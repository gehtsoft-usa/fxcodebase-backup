...
function CreateParameters() 
    ...
    strategy.parameters:addGroup("QQE"); 
...
function CreateEntryIndicators(source)
    ...
    indi = core.indicators:create("HALFTREND WITH ALERT", source, instance.parameters.Amplitude, true);

    local profile = core.indicators:findIndicator("QQE");
    assert(profile ~= nil, "Please, download and install " .. "QQE" .. ".LUA indicator");
    qqe = core.indicators:create("QQE", source, instance.parameters.RF, instance.parameters.RSP, instance.parameters.AP, instance.parameters.F, instance.parameters.S);
end
...
function IsEntryLong(source, period)
    if not indi.signal:hasData(period - 1) then
        return false;
    end
    return indi.signal[period - 1] == 1
        and qqe.QQE[period] > qqe.TS1[period]
end
...
function Prepare(nameOnly)
    ...
    entry_execution_type = "EndOfTurn";
