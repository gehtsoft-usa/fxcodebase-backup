...
--+------------------------------------------------------------------+
function Init()
    strategy:name("KST Signal");
	strategy:setTag("NonOptimizableParameters", "Email,SendEmail,SoundFile,RecurrentSound,PlaySound,ShowAlert");
    strategy:description("KST Signal");
    ...
    strategy.parameters:addInteger("RSIN", "RSIN", "", 14, 2, 200);
	...
	strategy.parameters:addBoolean("UseRSI", "Use RSI", "", true);
...
function ExtUpdate(id, source, period)
    ...
    if id == 1 and first then
        ...
        indicator = iprofile:createInstance(tsource.close, iparams);	
		 RSI = core.indicators:create("RSI", tsource.close, RSIN);
    ...
    elseif id == 2 and period > 1 then
		...
		local mailMessage;
                ...
                return;
					...
					if  core.crossesUnder(indicator.KST, indicator2.BL, period) and RSI.DATA[period] > 50 then
					mailMessage = instance.bid:instrument() .. instance.bid[NOW] .. "RSI Enter Short" .. instance.bid:date(NOW);
							if ShowAlert then
							...
							if Email ~= nil then
							terminal:alertEmail (Email, "Enter Short", "KST have give Enter Short signal " .. mailMessage)
							end
							
					elseif  core.crossesOver(indicator.KST, indicator2.TL, period) and RSI.DATA[period] < 50 then
					mailMessage = instance.bid:instrument() .. instance.bid[NOW] .. "RSI Enter Long" .. instance.bid:date(NOW);
						-- switch to long
							if ShowAlert then
							...
							if Email ~= nil then
							terminal:alertEmail (Email, "Enter Long", "KST have give Enter Long signal " .. mailMessage)
							end
							
					end
				else
					if  core.crossesUnder(indicator.KST, indicator2.BL, period) then
					  mailMessage = instance.bid:instrument() .. instance.bid[NOW] .. "Enter Short" .. instance.bid:date(NOW);
							if ShowAlert then
					...
					mailMessage = instance.bid:instrument() .. instance.bid[NOW] .. "Enter Long" .. instance.bid:date(NOW);
						-- switch to long
							if ShowAlert then
