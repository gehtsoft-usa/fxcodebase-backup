...
local ich, source, Tenkan, Kijun, Senkou;
...
{ 
    {
        ...
        UpCondition = function (period)
            if period <= ich.SL:first() then
                return false;
            end
            return core.crossesOver(ich.SL, ich.TL, period);
        end,
        DownCondition = function (period)
            if period <= ich.SL:first() then
                return false;
            end
            return core.crossesUnder(ich.SL, ich.TL, period);
        end,
    ...
    },
    {
        ...
        UpCondition = function (period)
            core.host:trace(2);
            if period <= ich.CS:first() or period < Kijun + 1 then
                return false;
            end
            return core.crossesOver(ich.CS, source.close, period - Kijun);
        end,
        DownCondition = function (period)
            if period <= ich.CS:first() or period < Kijun + 1 then
                return false;
            end
            return core.crossesUnder(ich.CS, source.close, period - Kijun);
        end,
        FormatMessage = function(source, period, level, label, isUp, metadata)
            return string.format(
                "Label: %s\013\010" ..
    ...
    },
    {
        Stage = 2,
        UpCondition = function (period)
            if period <= ich.SA:first() then
                return false;
            end
            return math.max(ich.SA[period], ich.SB[period]) < source.close[period] and
                math.max(ich.SA[period - 1], ich.SB[period - 1]) > source.close[period - 1]
        end,
        DownCondition = function (period)
            if period <= ich.SA:first() then
                return false;
            end
            return math.min(ich.SA[period], ich.SB[period]) > source.close[period] and
                math.min(ich.SA[period - 1], ich.SB[period - 1]) < source.close[period - 1]
        end,
        FormatMessage = function(source, period, level, label, isUp, metadata)
            return string.format(
                "Label: %s\013\010" ..
...
function Init()
    indicator:name("Ichimoku Kinko Hyo");
    indicator:description("");
    indicator:requiredSource(core.Bar);
    indicator:type(core.Indicator);
    
    indicator.parameters:addInteger("T", "Tenkan Period", "Tenkan Period", 9)
    indicator.parameters:addInteger("K", "Kijun Period", "Kijun Period", 26)
    indicator.parameters:addInteger("S", "Senkou Period", "Senkou Period", 52)
...
function Prepare(nameOnly)
    ...
    ich = core.indicators:create("ICH", source, instance.parameters.T, instance.parameters.K, instance.parameters.S);
