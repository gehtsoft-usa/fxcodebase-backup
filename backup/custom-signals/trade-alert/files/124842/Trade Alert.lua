...
-- http://fxcodebase.com/code/viewtopic.php?f=29&t=67762
...
local use_webhooks = true;

function Init()
    strategy:name("Trade Alert");
...
function signaler:AsyncOperationFinished(cookie, success, message, message1, message2)
    if cookie == self._advanced_alert_timer and #self._alerts > 0 and (self.last_req == nil or not self.last_req:loading()) then
        ...
        if use_webhooks then
            local alert_to_send = self._alerts[1];
            table.remove(self._alerts, 1);
            
            self.last_req = http_lua.createRequest();
            self.last_req:start("https://profitrobots.com/webhook/" .. self._advanced_alert_key, "POST", alert_to_send.Text);
        else
