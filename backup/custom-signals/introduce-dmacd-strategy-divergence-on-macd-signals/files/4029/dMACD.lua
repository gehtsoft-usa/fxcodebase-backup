...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=1985
...
function Init()
   ...
   -- strategy.parameters:addBoolean("isNeedAutoTrading", resources:get("R_isNeedAutoTrading"), "", false);
...
function Update()
	...
	if size > 1 then
		...
		if serial ~= lastSerial then
			local period = size - 2;
			local hasData = MACD.DATA:hasData(period);
...
function checkTrendDown(period) -- Check trend down
    if isPeak(period) then
        ...
        if prev ~= nil then
			if source.high[curr] > source.high[prev] and MACD.DATA[curr] < MACD.DATA[prev] then -- True divergence (turn trend down)
				trueDivergence = true;
			end
			
			if source.high[curr] < source.high[prev] and MACD.DATA[curr] > MACD.DATA[prev] then -- Hide divergence (confirmation down trend)
				hideDivergence = true;
			end
...
function checkTrendUp(period) -- Check trend up
    if isTrough(period) then
        ...
        if prev ~= nil then
			if source.low[curr] < source.low[prev] and MACD.DATA[curr] > MACD.DATA[prev] then -- True divergence (turn trend up)
				trueDivergence = true;
			end
			
			if source.low[curr] > source.low[prev] and MACD.DATA[curr] < MACD.DATA[prev] then -- Hide divergence (confirmation up trend)
				hideDivergence = true;
			end
