...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=10452
...
function Init() --The strategy profile initialization
    strategy:name("Percentage Crossover Channel strategy")
    strategy:description("Percentage Crossover Channel strategy")
    strategy:setTag("Version", "2")
    strategy:setTag("NonOptimizableParameters", "Email,SendEmail,SoundFile,RecurrentSound,PlaySound, ShowAlert")

    strategy.parameters:addGroup("Parameters")
    strategy.parameters:addDouble("percent", "percent", "", 1)
    ...
    strategy.parameters:addString("Cross", "Cross", "", "Upper/Lower")
    strategy.parameters:addStringAlternative("Cross", "Upper/Lower", "", "Upper/Lower")
    strategy.parameters:addStringAlternative("Cross", "Middle", "", "Middle")
...
function Prepare(nameOnly)
    ...
    assert(
        core.indicators:findIndicator("PERCENTAGE_CROSSOVER_CHANNEL") ~= nil,
        "Please, download and install PERCENTAGE_CROSSOVER_CHANNEL.LUA indicator"
    )

    Source = ExtSubscribe(2, nil, instance.parameters.TF, true, "bar")
    Price = instance.parameters.Price
    if Price == "close" then
        PCC = core.indicators:create("PERCENTAGE_CROSSOVER_CHANNEL", Source.close, instance.parameters.percent)
    elseif Price == "open" then
        PCC = core.indicators:create("PERCENTAGE_CROSSOVER_CHANNEL", Source.open, instance.parameters.percent)
    elseif Price == "high" then
        PCC = core.indicators:create("PERCENTAGE_CROSSOVER_CHANNEL", Source.high, instance.parameters.percent)
    elseif Price == "low" then
        PCC = core.indicators:create("PERCENTAGE_CROSSOVER_CHANNEL", Source.low, instance.parameters.percent)
    elseif Price == "typical" then
        PCC = core.indicators:create("PERCENTAGE_CROSSOVER_CHANNEL", Source.typical, instance.parameters.percent)
    elseif Price == "median" then
        PCC = core.indicators:create("PERCENTAGE_CROSSOVER_CHANNEL", Source.median, instance.parameters.percent)
    else
        PCC = core.indicators:create("PERCENTAGE_CROSSOVER_CHANNEL", Source.weighted, instance.parameters.percent)
    end
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    PCC:update(core.UpdateLast)

    -- Check that we have enough data
    if (PCC.DATA:first() > (period - 1)) then
        return
    end
    ...
    if instance.parameters.Cross == "Upper/Lower" then
        if source.low[period - 1] > PCC.LowerBuff[period - 1] and source.low[period] <= PCC.LowerBuff[period] then
        ...
        if source.high[period - 1] < PCC.UpperBuff[period - 1] and source.high[period] >= PCC.UpperBuff[period] then
            if instance.parameters.TypeSignal == "direct" then
                MustOpenS = true
            else
                MustOpenB = true
        ...
        if source.close[period - 1] > PCC.MiddleBuff[period - 1] and source.close[period] < PCC.MiddleBuff[period] then
            if instance.parameters.TypeSignal == "direct" then
                MustOpenB = true
            else
                MustOpenS = true
