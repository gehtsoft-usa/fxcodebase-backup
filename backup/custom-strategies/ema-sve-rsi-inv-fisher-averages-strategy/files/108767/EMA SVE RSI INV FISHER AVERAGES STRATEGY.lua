...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=64021
...
function Init() --The strategy profile initialization
    strategy:name("EMA SVE RSI INV FISHER AVERAGES STRATEGY")
    ...
    strategy.parameters:addInteger("MA_Period", "MA Period", "", 100)
    ...
    strategy.parameters:addGroup(" Smoothed RSI Inverse Fisher Transform Calculation")
    strategy.parameters:addInteger("RSI_N", "Number of periods for RSI", "", 7, 2, 1000)
    strategy.parameters:addInteger("MA_N", "Number of periods for MA", "", 7, 2, 1000)
    ...
    strategy.parameters:addStringAlternative("Method", "ARSI", "", "ARSI")
    strategy.parameters:addStringAlternative("Method", "VIDYA", "", "VIDYA")
    strategy.parameters:addStringAlternative("Method", "HPF", "", "HPF")
    strategy.parameters:addStringAlternative("Method", "VAMA", "", "VAMA")

    strategy.parameters:addDouble("BL", "Buy Level", "", 10)
    strategy.parameters:addDouble("SL", "Sell Level", "", 90)
...
local first
local Indicator, RSI_N, MA_N, Method
...
function Prepare(nameOnly)
    ...
    --SVE_RSI_InvFisher Averages
    --SVE_RainbowAverage Averages
    assert(
        core.indicators:findIndicator("SVE_RSI_INVFISHER AVERAGES") ~= nil,
        "Please, download and install SVE_RSI_INVFISHER AVERAGES.LUA indicator"
    )
    assert(
        core.indicators:findIndicator("SVE_RAINBOWAVERAGE AVERAGES") ~= nil,
        "Please download and install SVE_RAINBOWAVERAGE AVERAGES.lua indicator"
    )
    ...
    Indicator = core.indicators:create("SVE_RSI_INVFISHER AVERAGES", Source.close, RSI_N, MA_N, Method)
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if not Indicator.DATA:hasData(period) or not MA.DATA:hasData(period) then
        return
    end

    -- only buy if we have a fast cross over slow and the price is above the moving averages.
    if Source.close[period] > MA.DATA[period] and Indicator.DATA[period] > BL and Indicator.DATA[period - 1] <= BL then
        if Direction then
            BUY()
        else
            SELL()
    ...
    elseif Source.close[period] < MA.DATA[period] and Indicator.DATA[period] < SL and Indicator.DATA[period - 1] >= SL then
        if Direction then
            SELL()
        else
            BUY()
