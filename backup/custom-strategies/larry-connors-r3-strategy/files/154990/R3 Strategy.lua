-- Available @ https://fxcodebase.com/code/viewtopic.php?f=31&t=74772
...
function Init() --The strategy profile initialization
    ...
    strategy.parameters:addInteger("Period", "RSI Period", "", 2);  
    strategy.parameters:addInteger("MVAPeriod", "MVA Period", "", 200);  	
	
    strategy.parameters:addDouble("BuyLevel1", "1. Buy Level", "", 60); 	
    strategy.parameters:addDouble("BuyLevel2", "2. Buy Level", "", 30);	

    strategy.parameters:addDouble("SellLevel1", "1. Sell Level", "", 40); 	
    strategy.parameters:addDouble("SellLevel2", "2. Sell Level", "", 70);	
	
    strategy.parameters:addDouble("BuyExitLevel", "Buy Exit Level", "", 70); 	
    strategy.parameters:addDouble("SellExitLevel", "Sell Exit Level", "", 30);		
...
function CreateTradingParameters()
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "R3S");
...
function Prepare( nameOnly)
    ...
    RSI = core.indicators:create("RSI", Source.close, instance.parameters.Period ); 
    MVA = core.indicators:create("MVA", Source.close, instance.parameters.MVAPeriod ); 	
...
SELL AT MARKET
		            ...
		            if    RSI.DATA[period]> instance.parameters.BuyExitLevel then
					  ...
					  if    RSI.DATA[period]< instance.parameters.SellExitLevel then
...
BUY 1 SHARES AT MARKET
    ...
    if 	RSI.DATA[period] > RSI.DATA[period-1]
	and RSI.DATA[period-1] > RSI.DATA[period-2]
	and RSI.DATA[period-2]< instance.parameters.BuyLevel1
	and RSI.DATA[period]< instance.parameters.BuyLevel2	
	...
	then	
		...
		Return=true;
    elseif RSI.DATA[period] < RSI.DATA[period-1]
	and RSI.DATA[period-1] < RSI.DATA[period-2]
	and RSI.DATA[period-2]> instance.parameters.SellLevel1
	and RSI.DATA[period]> instance.parameters.SellLevel2	
	and Source.close[period]< MVA.DATA[period]	
	then
            if Direction then
                SELL();
