...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=68534
...
local STRATEGY_NAME = "TII Strategy";
...
function CreateParameters() 
    strategy.parameters:addInteger("Period", "Period", "Period", 30);
    strategy.parameters:addInteger("MAPeriod", "Ma Period", "Period", 60);
    ...
    strategy.parameters:addGroup("OB/OS Levels");	
    strategy.parameters:addDouble("overbought", "Overbought Level","", 80);
    strategy.parameters:addDouble("oversold","Oversold Level","", 20);

    strategy.parameters:addGroup("Actions");	
    AddAction("over_0", "Cross over 0");
    AddAction("under_0", "Cross under 0");

    AddAction("over_20", "Cross over 20");
    AddAction("under_20", "Cross under 20");

    AddAction("over_80", "Cross over 80");
    AddAction("under_80", "Cross under 80");

    AddAction("over_100", "Cross over 100");
    AddAction("under_100", "Cross under 100");
	
	 AddAction("over_50", "Cross over 50");
    AddAction("under_50", "Cross under 50");
end

function CreateIndicators(source) 
    local profile = core.indicators:findIndicator("TII");
    assert(profile ~= nil, "Please, download and install " .. "TII" .. ".LUA indicator");
    local indicatorParams = profile:parameters();
    indicatorParams:setInteger("Period", instance.parameters.Period);
    indicatorParams:setInteger("MAPeriod", instance.parameters.MAPeriod);
    indicatorParams:setString("Method", instance.parameters.Method);
    indi = core.indicators:create("TII", source, indicatorParams)
end
...
function CreateCustomActions()
    local action, isEntry = CreateAction("over_0");
    action.Data = nil;
    action.IsPass = function (source, period, periodFromLast, data) 
        if not indi.DATA:hasData(period - 1) then
            return;
        end
        return core.crossesOver(indi.DATA, 0, period); 
    end
    if isEntry then
        EntryActions[#EntryActions + 1] = action;
    else
        ExitActions[#ExitActions + 1] = action;
    end

    local action, isEntry = CreateAction("under_0");
    action.Data = nil;
    action.IsPass = function (source, period, periodFromLast, data) 
        if not indi.DATA:hasData(period - 1) then
            return;
        end
        return core.crossesUnder(indi.DATA, 0, period); 
    end
    if isEntry then
        EntryActions[#EntryActions + 1] = action;
    else
    ...
    local action, isEntry = CreateAction("over_20");
    action.Data = nil;
    action.IsPass = function (source, period, periodFromLast, data) 
        if not indi.DATA:hasData(period - 1) then
            return;
        end
        return core.crossesOver(indi.DATA, 20, period); 
    end
    if isEntry then
        EntryActions[#EntryActions + 1] = action;
    else
    ...
    local action, isEntry = CreateAction("under_20");
    action.Data = nil;
    action.IsPass = function (source, period, periodFromLast, data) 
        if not indi.DATA:hasData(period - 1) then
            return;
        end
        return core.crossesUnder(indi.DATA, 20, period); 
    end
    if isEntry then
        EntryActions[#EntryActions + 1] = action;
    else
    ...
    local action, isEntry = CreateAction("over_80");
    action.Data = nil;
    action.IsPass = function (source, period, periodFromLast, data) 
        if not indi.DATA:hasData(period - 1) then
            return;
        end
        return core.crossesOver(indi.DATA, 80, period); 
    end
    if isEntry then
        EntryActions[#EntryActions + 1] = action;
    else
    ...
    local action, isEntry = CreateAction("under_80");
    action.Data = nil;
    action.IsPass = function (source, period, periodFromLast, data) 
        if not indi.DATA:hasData(period - 1) then
            return;
        end
        return core.crossesUnder(indi.DATA, 80, period); 
    end
    if isEntry then
        EntryActions[#EntryActions + 1] = action;
    else
    ...
    local action, isEntry = CreateAction("over_100");
    action.Data = nil;
    action.IsPass = function (source, period, periodFromLast, data) 
        if not indi.DATA:hasData(period - 1) then
            return;
        end
        return core.crossesOver(indi.DATA, 100, period); 
    end
    if isEntry then
        EntryActions[#EntryActions + 1] = action;
    else
    ...
    local action, isEntry = CreateAction("under_100");
    action.Data = nil;
    action.IsPass = function (source, period, periodFromLast, data) 
        if not indi.DATA:hasData(period - 1) then
            return;
        end
        return core.crossesUnder(indi.DATA, 100, period); 
    end
    if isEntry then
        EntryActions[#EntryActions + 1] = action;
    else
	...
	local action, isEntry = CreateAction("over_50");
    action.Data = nil;
    action.IsPass = function (source, period, periodFromLast, data) 
        if not indi.DATA:hasData(period - 1) then
            return;
        end
        return core.crossesOver(indi.DATA, 50, period); 
    end
    if isEntry then
        EntryActions[#EntryActions + 1] = action;
    else
    ...
    local action, isEntry = CreateAction("under_50");
    action.Data = nil;
    action.IsPass = function (source, period, periodFromLast, data) 
        if not indi.DATA:hasData(period - 1) then
            return;
        end
        return core.crossesUnder(indi.DATA, 50, period); 
    end
    if isEntry then
        EntryActions[#EntryActions + 1] = action;
    else
