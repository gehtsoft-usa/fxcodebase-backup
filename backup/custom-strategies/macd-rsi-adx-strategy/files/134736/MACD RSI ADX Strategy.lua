...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=69986
...
function Init() --The strategy profile initialization
    strategy:name("MACD, RSI, ADX Strategy");
    ...
    strategy.parameters:addInteger("ADX_Period", "ADX Period", "", 20);
	strategy.parameters:addDouble("ADX_Level", "ADX Level", "", 18);
 
	  strategy.parameters:addGroup("RSI Calculation");
    strategy.parameters:addInteger("RSI_Period", "RSI Period", "", 20);
	strategy.parameters:addDouble("RSI_Buy_Top_Level", "RSI Buy Top_Level", "", 58);
	strategy.parameters:addDouble("RSI_Buy_Bottom_Level", "RSI Buy Bottom Level", "", 50);
	strategy.parameters:addDouble("RSI_Sell_Top_Level", "RSI Sell Top Level", "", 50);
	strategy.parameters:addDouble("RSI_Sell_Bottom_Level", "RSI Sell Bottom Level", "", 42);
	
	
	 strategy.parameters:addGroup("MACD Calculation");
    strategy.parameters:addInteger("MACD_Short", "MACD  Short Period", "", 12);
	strategy.parameters:addInteger("MACD_Long", "MACD Long Period", "", 26);
	strategy.parameters:addInteger("MACD_Signal", "MACD Signal Period", "", 9);
 
 
    strategy.parameters:addDouble("MACD_Buy_Top_Level", "MACD Buy Top_Level", "", 0.00015);
	strategy.parameters:addDouble("MACD_Buy_Bottom_Level", "MACD Buy Bottom_Level", "", -0.00015);
	strategy.parameters:addDouble("MACD_Sell_Top_Level", "MACD Sell Top Level", "", 0.00015);
	strategy.parameters:addDouble("MACD_Sell_Bottom_Level", "MACD Sell Bottom Level", "", -0.00015);
...
--	strategy.parameters:addStringAlternative("ExitExecutionType", "Live", "", "Live");
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "MRAS");
    ...
    strategy.parameters:addInteger("Limit", "Limit Order in pips", "", 20);
    strategy.parameters:addBoolean("SetStop", "Set Stop Orders", "", false);
    strategy.parameters:addInteger("Stop", "Stop Order in pips", "", 10);
...
local ADX, RSI, MACD;
local ADX_Level, RSI_Buy_Top_Level, RSI_Buy_Botom_Level, RSI_Sell_Top_Level, RSI_Sell_Bottom_Level;
local MACD_Buy_Top_Level, MACD_Buy_Botom_Level,MACD_Sell_Top_Level, MACD_Sell_Bottom_Level;
...
--	  Exit= instance.parameters.Exit;
	...
	RSI_Buy_Top_Level= instance.parameters.RSI_Buy_Top_Level;
	RSI_Buy_Bottom_Level= instance.parameters.RSI_Buy_Bottom_Level;
	RSI_Sell_Top_Level= instance.parameters.RSI_Sell_Top_Level;
	RSI_Sell_Bottom_Level= instance.parameters.RSI_Sell_Bottom_Level;
    
	
	MACD_Buy_Top_Level= instance.parameters.MACD_Buy_Top_Level;
	MACD_Buy_Bottom_Level= instance.parameters.MACD_Buy_Bottom_Level;
	MACD_Sell_Top_Level= instance.parameters.MACD_Sell_Top_Level;
	MACD_Sell_Bottom_Level= instance.parameters.MACD_Sell_Bottom_Level;
...
--	or ExitExecutionType== "Live"
	...
	first=math.max(ADX.DATA:first(), RSI.DATA:first(),  MACD.SIGNAL:first())+1; 
...
function EntryFunction( id, now,period)
    ...
    if  ADX.DATA[period]<= ADX_Level
	and RSI.DATA[period]<= RSI_Buy_Top_Level
	and RSI.DATA[period]>= RSI_Buy_Bottom_Level
	and MACD.DATA[period]>= MACD.SIGNAL[period]
	and MACD.DATA[period-1]<= MACD.SIGNAL[period-1]
	and MACD.DATA[period]<= MACD_Buy_Top_Level
	and MACD.DATA[period]>= MACD_Buy_Bottom_Level	
	then	
          if Direction then
                BUY();
            else
		...
		Return=true;
    elseif  ADX.DATA[period]<= ADX_Level
	and RSI.DATA[period]<= RSI_Sell_Top_Level
	and RSI.DATA[period]>= RSI_Sell_Bottom_Level
	and MACD.DATA[period]<= MACD.SIGNAL[period]
	and MACD.DATA[period-1]>= MACD.SIGNAL[period-1]
	and MACD.DATA[period]<= MACD_Sell_Top_Level
	and MACD.DATA[period]>= MACD_Sell_Bottom_Level	
		then
            if Direction then
                SELL();
            else
