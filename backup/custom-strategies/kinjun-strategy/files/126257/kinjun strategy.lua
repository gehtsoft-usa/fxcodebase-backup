...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=68444
...
local STRATEGY_NAME = "Kinjun Name";
...
local small_source, big_source;
function CreateParameters()
    strategy.parameters:addBoolean("is_bid", "Price Type", "", true);
    strategy.parameters:setFlag("is_bid", core.FLAG_BIDASK);
    strategy.parameters:addString("timeframe", "Trading Time frame #1", "", "m5");
    strategy.parameters:setFlag("timeframe", core.FLAG_BARPERIODS);
    strategy.parameters:addString("timeframe2", "Trading Time frame #2", "", "H1");
    strategy.parameters:setFlag("timeframe2", core.FLAG_BARPERIODS);

    strategy.parameters:addInteger("TenkanSenPeriod", "Ichimuku Tenkan-sen period", "Tenkan-sen period", 9, 1, 1000);
    strategy.parameters:addInteger("KijunSenPeriod", "Ichimuku Kijun-sen period", "Kijun-sen period", 26, 1, 1000);
    strategy.parameters:addInteger("SenkouSpanPeriod", "Ichimuku Senkou Span B period", "Senkou Span B period", 52, 1, 1000);
    strategy.parameters:addInteger("rsi_period", "RSI Period", "", 14);
    strategy.parameters:addDouble("rsi_buy_level", "RSI Buy Level", "", 65);
    strategy.parameters:addDouble("rsi_sell_level", "RSI Sell Level", "", 35);
	strategy.parameters:addInteger("MACD_Period1", "MACD Fast Period", "", 12);
    strategy.parameters:addInteger("MACD_Period2", "MACD Slow Period", "", 26);
	strategy.parameters:addInteger("MACD_Period3", "MACD Signal Period", "", 9);
    strategy.parameters:addDouble("SAR_Step", "Step", "", 0.02, 0.001, 1);
    strategy.parameters:addDouble("SAR_Max", "Max", "", 0.2, 0.001, 10);
end
local ich, rsi, macd, sar, KijunSenPeriod, rsi_buy_level, rsi_sell_level;
function CreateIndicators(source) 
    small_source = trading_logic:SubscribeHistory(source:instrument(), instance.parameters.timeframe, source:isBid());
    big_source = trading_logic:SubscribeHistory(source:instrument(), instance.parameters.timeframe2, source:isBid());
    KijunSenPeriod = instance.parameters.KijunSenPeriod;
    rsi_buy_level = instance.parameters.rsi_buy_level;
    rsi_sell_level = instance.parameters.rsi_sell_level;

    ich = core.indicators:create("ICH", small_source, 
        instance.parameters.TenkanSenPeriod,
        KijunSenPeriod,
        instance.parameters.SenkouSpanPeriod);
    rsi = core.indicators:create("RSI", small_source, instance.parameters.rsi_period);
    macd = core.indicators:create("MACD", small_source, 
        instance.parameters.MACD_Period1,
        instance.parameters.MACD_Period2,
        instance.parameters.MACD_Period3);
    sar = core.indicators:create("SAR", big_source, 
        instance.parameters.SAR_Step,
        instance.parameters.SAR_Max);
end
...
function CreateCustomActions()
    ...
    enterLongAction.IsPass = function (source, period, periodFromLast, data)
        local index = core.findDate(big_source, source:date(period), false);
        if index == -1 
            or not ich.TL:hasData(period - 1) 
            or not ich.SL:hasData(period - 1) 
            or not ich.SB:hasData(period - KijunSenPeriod) 
            or not rsi.DATA:hasData(period) 
            or not sar.DATA:hasData(period) 
            or not macd.MACD:hasData(period - 1)
        then
            return;
        end
        return core.crossesOver(source.close, ich.TL, period)
            and rsi.DATA[period] < rsi_buy_level
            and macd.MACD[period] >= macd.MACD[period - 1]
            and ich.SL[period] > ich.SL[period - 1]
            and ich.TL[period] > ich.SB[period - KijunSenPeriod]
            and source.close[period] > sar.DATA[index];
    end
    ...
    enterShortAction.IsPass = function (source, period, periodFromLast, data)
        ...
        return core.crossesUnder(source.close, ich.TL, period)
            and rsi.DATA[period] > rsi_sell_level
            and macd.MACD[period] <= macd.MACD[period - 1]
            and ich.SL[period] < ich.SL[period - 1]
            and ich.TL[period] < ich.SB[period - KijunSenPeriod]
            and source.close[period] < sar.DATA[index];
