...
function Init()
    strategy:name("Trend Magic Direction Strategy");
	strategy:setTag("NonOptimizableParameters", "Email,SendEmail,SoundFile,RecurrentSound,PlaySound,ShowAlert");
    strategy:description("The strategy trades using Trend Magic indicators. It trades when Trend Magic changes it's direction, not color");

    strategy.parameters:addGroup("Trend Magic Parameters");
    strategy.parameters:addInteger("CCI", "CCI", "", 40);
    ...
    strategy.parameters:addInteger("Limit", "Limit Order in pips", "", 30, 1, 100000);
    ...
    strategy.parameters:addInteger("IncStep", "Step to increase in lots", "", 100, 1, 10000);
    strategy.parameters:addInteger("IncMax", "Maximum trade size in lots", "", 5, 1, 10000);
...
function Prepare(onlyName)
    ...
    if AllowTrade then
        ...
        -- check whether we already have a position and whether it's locked
        local enum, row;
        enum = core.host:findTable("trades"):enumerator();
        row = enum:next();
        while count == 0 and row ~= nil do
            if row.AccountID == Account and
               row.OfferID == Offer then
                if row.BS == "B" then
                    POS = BUY;
                else
                    POS = SELL;
...
function ExtUpdate(id, source, period)
    if id == 2 then
        ...
        if TM.SIG[period] == 1 and POS ~= BUY and getTM(period) > getTM(period - 1) then
            -- switch to long
            if ShowAlert then
            ...
            POS = BUY;
        elseif TM.SIG[period] == -1 and POS ~= SELL and getTM(period) < getTM(period - 1) then
            -- switch to short
            if ShowAlert then
...
function getTM(period)
    if TM.SIG[period] == 1 then
        return TM.UP[period];
    elseif TM.SIG[period] == -1 then
        return TM.DN[period];
    else
        return 0;
    end
