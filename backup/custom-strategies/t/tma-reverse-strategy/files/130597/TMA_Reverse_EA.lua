...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=69294
...
local STRATEGY_NAME = "TMA_Reverse_EA";
...
function CreateEntryIndicators(source)
    local profile = core.indicators:findIndicator("TMA_REVERSE");
    assert(profile ~= nil, "Please, download and install " .. "TMA REVERSE" .. ".LUA indicator");
    indi = core.indicators:create("TMA_REVERSE", source, indicatorParams)
end
...
function CreateCustomActions()
    if not DISABLE_EXIT then
        ...
        exitLongAction.IsPass = function (source, period, periodFromLast, data)
            if not indi.Middle:hasData(period - 1) then
                return;
            end
            return core.crossesOver(source.close, indi.Middle, period);
        end
        ...
        exitShortAction.IsPass = function (source, period, periodFromLast, data)
            ...
            return core.crossesUnder(source.close, indi.Middle, period);
        end
    ...
    enterLongAction.IsPass = function (source, period, periodFromLast, data)
        if not indi.Lower:hasData(period) then
            return;
        end
    ...
    enterShortAction.IsPass = function (source, period, periodFromLast, data)
        if not indi.Upper:hasData(period) then
            return;
        end
