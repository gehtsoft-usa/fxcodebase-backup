...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=65626
...
function Init() --The strategy profile initialization
    strategy:name("Two Regression Lines MACD Strategy");
    ...
    strategy.parameters:addGroup("1. Regression Lines Calculation");
    strategy.parameters:addInteger("N1", "Number of periods", "Number of periods", 14, 2, 1000);
	strategy.parameters:addGroup("2. Regression Lines Calculation");
    strategy.parameters:addInteger("N2", "Number of periods", "Number of periods", 33, 2, 1000);
	
	 strategy.parameters:addGroup("MACD Calculation");
	 
	 strategy.parameters:addBoolean("UseMACD", "Use MACD Filter", "", true);   
	 strategy.parameters:addInteger("P1", "Fast Period", "", 12, 2, 1000);
	 strategy.parameters:addInteger("P2", "Slow Period", "", 26, 2, 1000);
	 strategy.parameters:addInteger("P3", "Signal Period", "", 9, 2, 1000);
...
function CreateTradingParameters()
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "TRLMACDS");
...
local Indicator1, Indicator2, N1,N2;
local MACD, UseMACD, P1, P2, P3;
...
function Prepare( nameOnly)
	...
	UseMACD = instance.parameters.UseMACD;
	...
	assert(core.indicators:findIndicator("REGRESSION_LINE") ~= nil, "Please, download and install REGRESSION_LINE.LUA indicator");          
    ...
    Indicator1= core.indicators:create("REGRESSION_LINE", Source.close,  N1 , core.rgb(0, 255, 0), core.rgb(255, 0, 0) );
	Indicator2= core.indicators:create("REGRESSION_LINE", Source.close,  N2  , core.rgb(0, 255, 0), core.rgb(255, 0, 0)  );
	MACD= core.indicators:create("MACD", Source.close,  P1, P2, P3  );
	first=math.max(Indicator1.DATA:first(),Indicator2.DATA:first(),MACD.SIGNAL:first()); 
...
--sition if: RLFast < RLSlow AND RLFast is Falling (Red)
		            if    Indicator1.DATA[period]< Indicator2.DATA[period] 
					and Indicator1.DATA:colorI(period) ==  core.rgb(255, 0, 0)
					then
					...
					if    Indicator1.DATA[period]> Indicator2.DATA[period] 
					and Indicator1.DATA:colorI(period) ==  core.rgb(0, 255, 0)
					then
...
function EntryFunction( now,period)
	...
	and Indicator1.DATA[period]> Indicator2.DATA[period]
	and Source.close[period] > Indicator2.DATA[period]
	and ((UseMACD and MACD.MACD[period]<MACD.SIGNAL[period] ) or not UseMACD)
	then	
          if Direction then
                BUY();
            else
