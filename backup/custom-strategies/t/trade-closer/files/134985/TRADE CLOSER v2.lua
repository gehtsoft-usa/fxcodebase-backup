...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=69856
...
function AddPrice(id, name)
    strategy.parameters:addDouble("Price" .. id, name .. " Price Source", "", 0)
    strategy.parameters:setFlag("Price" .. id, core.FLAG_PRICE)
end
...
function Init()
    ...
    AddPrice("1", "Long")
    strategy.parameters:addDouble("Factor1", "Factor1", "", 1);
    strategy.parameters:addInteger("atr_period", "ATR 1 period", "", 14);
    strategy.parameters:addString("tf1", "Timeframe 1", "", "m5");
    strategy.parameters:setFlag("tf1", core.FLAG_PERIODS);
    AddPrice("2", "Short")
    strategy.parameters:addDouble("Factor2", "Factor2", "", 1);
    strategy.parameters:addInteger("atr_period_2", "ATR 2 period", "", 21);
    strategy.parameters:addString("tf2", "Timeframe 2", "", "m5");
    strategy.parameters:setFlag("tf2", core.FLAG_PERIODS);
    
    strategy.parameters:addString("tf3", "Time frame 3", "", "m1");
    strategy.parameters:setFlag("tf3", core.FLAG_BARPERIODS);
    
    strategy.parameters:addString("TF", "Close Time frame", "", "m1");
...
function Prepare(nameOnly)
    ...
    Source3 = ExtSubscribe(3, nil, instance.parameters.tf3, instance.parameters.type == "Bid", "bar");
    local profile = core.indicators:findIndicator("LONGER SHORTER INDICATOR");
    assert(profile ~= nil, "Please, download and install " .. "LONGER SHORTER INDICATOR" .. ".LUA indicator");
    local indicatorParams = profile:parameters();
    indicatorParams:setString("tf1", instance.parameters.tf1);
    indicatorParams:setString("tf2", instance.parameters.tf2);
    indicatorParams:setDouble("Price1", instance.parameters.Price1);
    indicatorParams:setDouble("Price2", instance.parameters.Price2);
    ...
    indicatorParams:setInteger("atr_period_2", instance.parameters.atr_period_2);
    Indicator = core.indicators:create("LONGER SHORTER INDICATOR", Source3, indicatorParams)
...
function DoMoveStop(trade)
    if commands[trade.TradeID] ~= nil or Source.close:size() == 0 or Indicator.exit_sell:size() == 0 then
        return;
    end
    local newStop;
    if (trade.BS == "B") then
        if core.crossesUnder(Source.close, Indicator.exit_buy, Source.close:size() - 1, Indicator.exit_buy:size() - 1) then
            commands[trade.TradeID] = trading:Close(trade);
        end
    else
        if core.crossesOver(Source.close, Indicator.exit_sell, Source.close:size() - 1, Indicator.exit_sell:size() - 1) then
            commands[trade.TradeID] = trading:Close(trade);
        end
    end
end
