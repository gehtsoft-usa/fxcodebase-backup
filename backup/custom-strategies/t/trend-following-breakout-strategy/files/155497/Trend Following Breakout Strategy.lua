-- Available @ https://fxcodebase.com/code/viewtopic.php?f=31&t=74913
...
function Init() --The strategy profile initialization
    strategy:name("Trend Following Breakout Strategy");
    ...
    strategy.parameters:addInteger("EntryPeriod", "ENTRY H/L Period", "", 100, 1, 2000);
    strategy.parameters:addInteger("ExitPeriod", "EXIT H/L Period", "", 50, 1, 2000);
...
function CreateTradingParameters()
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "TFBS");
...
function Prepare( nameOnly)
	...
	assert(core.indicators:findIndicator("TREND FOLLOWING BREAKOUT") ~= nil, "Please, download and install TREND FOLLOWING BREAKOUT.LUA indicator");          
    ...
    Indicator = core.indicators:create("TREND FOLLOWING BREAKOUT", Source, EntryPeriod, ExitPeriod );   
...
--	period3= core.findDate (Source3, TickSource:date(period), false);
	...
	--/////////////////////////////////////////////////////////////////////
		            ...
		            if     core.crossesOver(Source.close,lowestpricelong, period )
					then
					  ...
					  if   core.crossesOver(Source.close,highestpriceshort, period )
					  then
...
function EntryFunction( id, now,period)
    ...
    if  core.crossesOver(Source.close,highestpricelong, period )
	then	
          if Direction then
                BUY();
            else
		...
		Return=true;
    elseif  core.crossesUnder(Source.close,lowestpriceshort, period )
		then
            if Direction then
                SELL();
            else
