-- Available @ https://fxcodebase.com/code/viewtopic.php?f=31&t=76482
--
-- ── Author ─────────────────────────────────────────────────────────────────────
...
function Init() --The strategy profile initialization
    ...
    strategy.parameters:addInteger("HalfLength", "Half Length", "", 56);
    strategy.parameters:addDouble("BandDeviations", "Band Deviations", "", 2.5);
    strategy.parameters:addBoolean("Interpolate", "Interpolate", "", true);
...
function CreateTradingParameters()
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "TMACGS");
...
local Price, HalfLength, BandDeviations, Interpolate;
...
function Prepare( nameOnly)
	...
	assert(core.indicators:findIndicator("TMA_CG") ~= nil, "Please, download and install  TMA_CG.LUA indicator");          
    ...
    Indicator = core.indicators:create("TMA_CG", Source, Price, HalfLength, BandDeviations, Interpolate  );
...
function EntryFunction( now,period)
    ...
    if   Source.high[period-1]>Indicator.upBuffer[period-1] and Source.close[period-1]>Source.open[period-1] and Source.close[period]<Source.open[period]
	then	
          if Direction then
                BUY();
            else
		...
		Return=true;
    elseif Source.low[period-1]<Indicator.dnBuffer[period-1] and Source.close[period-1]<Source.open[period-1] and Source.close[period]>Source.open[period]
		then
            if Direction then
                SELL();
            else
