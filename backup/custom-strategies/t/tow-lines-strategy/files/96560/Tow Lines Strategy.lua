...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=61332
...
function Init() --The strategy profile initialization
    strategy:name("Tow Lines Strategy")
    ...
    strategy.parameters:addInteger("Period1", "A Line Period", "Period", 20, 1, 2000)
    strategy.parameters:addInteger("Period2", "B Line Period", "Period", 6, 1, 2000)
    strategy.parameters:addInteger("Period3", "C Line Period", "Period", 5, 1, 2000)

    strategy.parameters:addString("Mode", "Trade Mode", "", "AB")
    strategy.parameters:addStringAlternative("Mode", "A/B Cross", "", "AB")
    strategy.parameters:addStringAlternative("Mode", "C/A Cross", "", "CA")
    strategy.parameters:addStringAlternative("Mode", "C/B Cross", "", "CB")
    strategy.parameters:addStringAlternative("Mode", "C/A&B Cross", "", "C/A&B")
...
function Prepare(nameOnly)
    ...
    assert(core.indicators:findIndicator("TOW LINES") ~= nil, "Please, download and install TOW LINES.LUA indicator")
    ...
    name =
        ...
        "( " ..
            instance.bid:name() ..
                "," .. Period1 .. "," .. Period2 .. "," .. Period3 .. "," .. Mode .. "," .. CustomID .. " )"
    instance:name(name)
    ...
    Tow = core.indicators:create("TOW LINES", Source, Period1, Period2, Period3)
    A = Tow:getStream(0)
    B = Tow:getStream(1)
    C = Tow:getStream(2)
    first = math.max(A:first(), B:first(), B:first()) + 1
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if Mode == "CA" then
        if core.crossesOver(C, A, period) then
            BUY()
            ONE = Source:serial(period)
        elseif core.crossesUnder(C, A, period) then
            SELL()
            ONE = Source:serial(period)
        end
    elseif Mode == "CB" then
        if core.crossesOver(C, B, period) then
            BUY()
            ONE = Source:serial(period)
        elseif core.crossesUnder(C, B, period) then
            SELL()
            ONE = Source:serial(period)
        end
    elseif Mode == "AB" then
        if core.crossesOver(A, B, period) then
            BUY()
            ONE = Source:serial(period)
        elseif core.crossesUnder(A, B, period) then
            SELL()
            ONE = Source:serial(period)
        end
    elseif Mode == "C/A&B" then
        if core.crossesOver(C, A, period) and core.crossesOver(C, B, period) then
            BUY()
            ONE = Source:serial(period)
        elseif core.crossesUnder(C, A, period) and core.crossesUnder(C, B, period) then
            SELL()
            ONE = Source:serial(period)
        end
    end
