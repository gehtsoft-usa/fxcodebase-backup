...
-- https://fxcodebase.com/code/viewtopic.php?f=31&t=73164
...
function Init() --The strategy profile initialization
    strategy:name("The Bollinger in Trend strategy");
    ...
    strategy.parameters:addInteger("Period", "BB Period", "", 10);  
    strategy.parameters:addDouble("Deviation", "BB Deviation", "", 1); 

	strategy.parameters:addInteger("Period1", "Fast MA Period", "", 20);  
 	strategy.parameters:addInteger("Period2", "Slow MA Period", "", 100);  
 	strategy.parameters:addInteger("Period3", "Trend MA Period", "", 200); 	
...
function CreateTradingParameters()
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "TBITS");
...
local Indicator;
local Deviation,Period1,Period1, Period2;
...
function Prepare( nameOnly)
    ...
    MA1= core.indicators:create("MVA", Source.close, Period1 );	 
    MA2= core.indicators:create("MVA", Source.close, Period2 );	 
    MA3= core.indicators:create("MVA", Source.close, Period3 );	 	
	first=math.max(BB.DATA:first(),MA1.DATA:first(),MA2.DATA:first(),MA3.DATA:first()); 
...
function EntryFunction( id, now,period)
	...
	local PB0 = (Source.close[period] - BB.BL[period]) / (BB.TL[period] - BB.BL[period])
	local PB1 = (Source.close[period-1] - BB.BL[period-1]) / (BB.TL[period-1] - BB.BL[period-1])	 
	...
	if  MA1.DATA[period] >MA3.DATA[period] then
	ca2 =true;
	else
	ca2 =false;	
	end	
	
 	if  PB0 < 0.2 and PB1 < 0.2  then
	ca3 =true;
	else
	ca3 =false;	
	end	
