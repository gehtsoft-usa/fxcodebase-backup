-- Available @ https://fxcodebase.com/code/viewtopic.php?f=31&t=74703
...
function Init() --The strategy profile initialization
    strategy:name("Triple RSI Trading Strategy");
    ...
    strategy.parameters:addInteger("OB", "OB Level", "", 70); 	
    strategy.parameters:addInteger("OS", "OS Level", "", 30); 		
...
function CreateTradingParameters()
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "TRTS");
...
function Prepare( nameOnly)
	...
	MVA = core.indicators:create("MVA", Source.close, MVAPeriod );    
	first=math.max(Indicator.DATA:first(), MVA.DATA:first()); 
...
--	period3= core.findDate (Source3, TickSource:date(period), false);
	...
	--/////////////////////////////////////////////////////////////////////
		            ...
		            if    Indicator.DATA[period]> 50  then
...
function EntryFunction( id, now,period)
    ...
    if   Indicator.DATA[period]<OS	
	and  Indicator.DATA[period] < Indicator.DATA[period-1] and Indicator.DATA[period-1] < Indicator.DATA[period-2]  and Indicator.DATA[period-2] < Indicator.DATA[period-3]
	and Indicator.DATA[period-3]<60
	...
	then	
		...
		Return=true;
    elseif   Indicator.DATA[period]>OB	
	and  Indicator.DATA[period] > Indicator.DATA[period-1] and Indicator.DATA[period-1] > Indicator.DATA[period-2]  and Indicator.DATA[period-2] > Indicator.DATA[period-3]
	and Indicator.DATA[period-3]>40
	and Source.close[period]< MVA.DATA[period] 
	then
            if Direction then
                SELL();
