...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=62395
...
function Init() --The strategy profile initialization
    strategy:name("Trading Magnum Strategy")
    ...
    strategy.parameters:addGroup("Trend direction")
    ...
    strategy.parameters:addGroup("Trend filter")
    strategy.parameters:addInteger("cp1", "Trend CCI Period", "", 50)
    strategy.parameters:addInteger("cp2", "Signal CCI Period", "", 14)

    strategy.parameters:addGroup("Joker Filter")
    strategy.parameters:addInteger("SmoothPeriod", "Smooth Period", "Smooth Period", 5)
    strategy.parameters:addInteger("SmoothPhase", "Smooth Phase", "Smooth Phase", 0)
...
function Prepare(nameOnly)
    ...
    assert(
        core.indicators:findIndicator("TREND DIRECTION") ~= nil,
        "Please, download and install TREND DIRECTION.LUA indicator"
    )
    assert(
        core.indicators:findIndicator("TREND FILTER") ~= nil,
        "Please, download and install TREND FILTER.LUA indicator"
    )
    assert(
        core.indicators:findIndicator("JOKER FILTER") ~= nil,
        "Please, download and install JOKER FILTER.LUA indicator"
    )
    ...
    TrendDirection =
        core.indicators:create("TREND DIRECTION", Source.close, Period, OB, OS, color.Lime, color.Red, color.Gray)
    TrendFilter =
    ...
    JokerFilter =
        core.indicators:create("JOKER FILTER", Source, SmoothPeriod, SmoothPhase, color.Lime, color.Red, color.Gray)
    first = math.max(HA.DATA:first(), TrendDirection.DATA:first(), TrendFilter.DATA:first(), JokerFilter.DATA:first())
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    TrendDirection:update(core.UpdateLast)
    TrendFilter:update(core.UpdateLast)
    JokerFilter:update(core.UpdateLast)
    ...
    if
        HA.close[period] > HA.open[period] and TrendDirection.DATA:colorI(period) == color.Lime and
            TrendFilter.DATA:colorI(period) == color.Lime and
            JokerFilter.DATA:colorI(period) == color.Lime
     then
        if Direction then
            BUY()
        else
    ...
    elseif
        HA.close[period] < HA.open[period] and TrendDirection.DATA:colorI(period) == color.Red and
            TrendFilter.DATA:colorI(period) == color.Red and
            JokerFilter.DATA:colorI(period) == color.Red
     then
        if Direction then
            SELL()
        else
