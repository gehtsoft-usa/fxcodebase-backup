...
function Init() --The strategy profile initialization
    strategy:name("Tenkan-sen MA Strategy")
    ...
    strategy.parameters:addInteger("Period", "TMA period", "", 34)
...
function Prepare(nameOnly)
    ...
    ICH = core.indicators:create("ICH", Source, T, K, S)
    first = math.max(MA.DATA:first(), ICH.DATA:first())
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if not MA.DATA:hasData(period) or not ICH.DATA:hasData(period) then
        return
    end
...
function ExitFunction(period, now)
    ...
    if ICH.DATA[period] > ICH.DATA[period - 1] and ICH.DATA[period - 1] <= ICH.DATA[period - 2] then
        if Direction then
            if exithaveTrades("B") then
                exitSpecific("B")
                Signal("Close Long")
            end
        else
            if exithaveTrades("S") then
                exitSpecific("S")
                Signal("Close Short")
            end
        end
...
function EntryFunction(period, now)
    ...
    if ICH.DATA[period] < MA.DATA[period] then
        if Direction then
            BUY()
        else
            SELL()
    ...
    elseif ICH.DATA[period] > MA.DATA[period] then
        if Direction then
            SELL()
        else
            BUY()
...
function BUY()
    if AllowTrade then
        if CloseOnOpposite and exithaveTrades("S") then
...
function SELL()
    if AllowTrade then
        if CloseOnOpposite and exithaveTrades("B") then
            -- close on opposite signal
            exitSpecific("B")
            Signal("Close Long")
        end
...
function exithaveTrades(BuySell)
    local enum, row
    local found = false
    enum = core.host:findTable("trades"):enumerator()
    row = enum:next()
