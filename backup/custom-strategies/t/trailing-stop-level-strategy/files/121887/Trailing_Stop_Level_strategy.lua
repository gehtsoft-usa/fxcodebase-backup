...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=66878
...
function Init()
    strategy:name("Trailing Stop Level Strategy");
    strategy:description("v1 2018-10-31");
...
function Prepare(name_only)
    ...
    assert(core.indicators:findIndicator("TRAILING_STOP_LEVEL") ~= nil, "Please, download and install TRAILING_STOP_LEVEL.LUA indicator");
    indicator = core.indicators:create("TRAILING_STOP_LEVEL", trading_logic.MainSource, 
        instance.parameters.Percent);
...
function IsBuyCondition(source, period)
    return indicator.DATA:color(period) == core.rgb(0, 255, 0)
        and indicator.DATA:color(period - 1) ~= core.rgb(0, 255, 0);
end

function IsSellCondition(source, period)
    return indicator.DATA:color(period) == core.rgb(255, 0, 0)
        and indicator.DATA:color(period - 1) ~= core.rgb(255, 0, 0);
end
