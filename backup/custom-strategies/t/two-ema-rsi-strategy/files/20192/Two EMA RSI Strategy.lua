...
function Init() --The strategy profile initialization
    strategy:name("Two EMA RSI Strategy")
    ...
    strategy.parameters:addInteger("SP", "Short EMA Period", "", 5, 2, 2000)
    strategy.parameters:addInteger("LP", "Long EMA Period", "", 10, 2, 2000)
...
function Prepare(nameOnly)
    ...
    Indicator[2] = core.indicators:create("EMA", Source[Price], LP)
    Short[2] = Indicator[2]:getStream(0)

    Indicator[3] = core.indicators:create("RSI", Source[Price], RP)
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if core.crossesOver(Short[1], Short[2], period) and Short[3][period] > 50 then
        if Direction then
            BUY()
        else
            SELL()
        end
    elseif core.crossesUnder(Short[1], Short[2], period) and Short[3][period] < 50 then
        if Direction then
            SELL()
        else
            BUY()
