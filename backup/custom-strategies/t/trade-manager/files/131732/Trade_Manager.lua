...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=66946
...
function Init()
    ...
    strategy.parameters:addBoolean("set_stop", "Set stop", "", true);
    strategy.parameters:addInteger("stop", "Stop, pips", "", 10);
end

local OFFERS_UPDATE = 1;
local TRADES_UPDATE = 2;
local custom_id;
local levels = {};
local set_stop, stop;
function Prepare(name_only)
    for _, module in pairs(Modules) do module:Prepare(nameOnly); end
    set_stop = instance.parameters.set_stop;
    ...
    if PositionsCount == 1 then
        levels[#levels + 1] = CreatePositionStrategy("");
    else
        for i = 1, PositionsCount do
            if instance.parameters:getBoolean("use_position_" .. i) then
                levels[#levels + 1] = CreatePositionStrategy("_" .. i);
            end
        end
    end
    core.host:execute("subscribeTradeEvents", OFFERS_UPDATE, "offers");
    core.host:execute("subscribeTradeEvents", TRADES_UPDATE, "trades");
    trading:FindTrade():Do(function (trade) 
        if trade.Instrument ~= instance.ask:instrument() then
            return;
        end
        CreateControllers(trade); 
    end);
end

function CreatePositionStrategy(id)
    local position_strategy = {};
    position_strategy.TriggerLevel = instance.parameters:getDouble("breakeven_when" .. id);
    if instance.parameters:getBoolean("breakeven_move_stop" .. id) then
...
function CreateControllers(trade)
    if not instance.parameters.allow_trade then
        return;
    end
    local afterPartialClose = trading:FindClosedTrade()
        :WhenTradeIDRemain(trade.TradeID)
        :Any();
    if afterPartialClose then
        return;
    end
    for _, level in ipairs(levels) do
        local controller = breakeven:CreateController()
            :SetTrade(trade)
            :SetWhen(level.TriggerLevel);
        if level.BreakevenTo ~= nil then
            controller:SetTo(level.BreakevenTo);
        end
        if level.BreakevenCloseAmount ~= nil then
            controller:SetPartialClose(level.BreakevenCloseAmount);
        end
    end
end
...
function ExtAsyncOperationFinished(cookie, success, message, message1, message2)
    ...
    elseif cookie == TRADES_UPDATE then
        ...
        if not close_trade then
            ...
            if trade.BS == "B" then
                trading:MoveStop(trade, trade.Open - stop * instance.bid:pipSize());
            else
                trading:MoveStop(trade, trade.Open + stop * instance.bid:pipSize());
            end
            CreateControllers(trade);
...
function trading:AddPositionParameters(parameters, id)
    parameters:addDouble("breakeven_when" .. id, "Target profit, in pips", "", 10);
    parameters:addBoolean("breakeven_move_stop" .. id, "Move stop", "", false);
    parameters:addDouble("breakeven_to" .. id, "Move stop to, in pips", "", 0);
    parameters:addBoolean("breakeven_close" .. id, "Partial close", "", false);
...
function trading:Init(parameters, count)
    ...
    if count == nil or count == 1 then
        ...
        for i = 1, count do
            ...
            parameters:addBoolean("use_position_" .. i, "Use level #" .. i, "", i == 1);
