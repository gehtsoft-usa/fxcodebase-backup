...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=69352
...
function Init()
    ...
    strategy.parameters:addInteger("lifetime", "Lifetime, min", "", 60);
end
...
local H1_SOURCE_ID = 3;
local TIMER_ID = 2;
local main_source, source_h1;
local custom_id;
local OpenTime, db, AllowTrade, Account, Amount, lifetime;
function Prepare(nameOnly)
    ...
    lifetime = instance.parameters.lifetime / 24 / 60;
    ...
    source_h1 = ExtSubscribe(H1_SOURCE_ID, nil, "H1", instance.parameters.type, "bar")
...
function DoTrading()
    local server_time = core.host:execute("getServerTime");
    local trades_present = false;
    trading:FindTrade()
        :WhenCustomID(custom_id)
        :Do(function (trade) 
            if closing[trade.TradeID] == nil and trade.Time + lifetime < server_time then
                trading:Close(trade); 
                closing[trade.TradeID] = true;
            else
                trades_present = true;
            end
        end);
    if trades_present or not AllowTrade then
        return;
    end
    local date = math.floor(server_time);
    local time = server_time - date;
    local last_date = math.floor(db:get("last_date", 0));
    local check = time >= OpenTime;
    local last_check = db:get("last_check", "1") == "1";
    if check and (not last_check or last_date ~= date) and main_source:size() > 5 then
        local hh = mathex.max(main_source.close, main_source:size() - 1 - 5, main_source:size() - 1);
        local ll = mathex.min(main_source.open, main_source:size() - 1 - 5, main_source:size() - 1);
        --if hh > ll and source_h1.high[NOW - 1] < source_h1.high[NOW - 3] then
            trading:MarketOrder(main_source:instrument())
                :SetSide("S")
                :SetAmount(Amount)
