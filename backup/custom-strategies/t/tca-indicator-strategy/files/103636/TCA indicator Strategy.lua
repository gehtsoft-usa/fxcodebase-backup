...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=62929
...
function Init() --The strategy profile initialization
    strategy:name("TCA indicator Strategy")
    ...
    strategy.parameters:addGroup("Calculation")
    strategy.parameters:addInteger("N", "Periods", "", 50, 2, 1000)
    strategy.parameters:addInteger("M", "Periods for Smooth", "", 7)
    strategy.parameters:addInteger("D", "Distance from center", "", 100)
    strategy.parameters:addInteger("C", "Coefficient percentage", "", 100)
    strategy.parameters:addDouble("BuyLevel", "Buy Level", "", -100)
    strategy.parameters:addDouble("SellLevel", "Sell Level", "", 100)
...
function Prepare(nameOnly)
    ...
    assert(core.indicators:findIndicator("TCA") ~= nil, "Please, download and install TCA.LUA indicator")
    ...
    Indicator = core.indicators:create("TCA", Source, N, M, D, C)

    first = math.max(Indicator.T:first(), Indicator.M:first(), Indicator.S:first())
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if
        Indicator.S[period] < BuyLevel and Indicator.T[period] > Indicator.S[period] and
            Indicator.T[period - 1] <= Indicator.S[period - 1]
     then
        if Direction then
            BUY()
        else
    ...
    elseif
        Indicator.S[period] > SellLevel and Indicator.T[period] < Indicator.S[period] and
            Indicator.T[period - 1] >= Indicator.S[period - 1]
     then
        if Direction then
            SELL()
