...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=15504
...
function Init() --The strategy profile initialization
    ...
    strategy:description("Two MA cross strategy")

    strategy.parameters:addGroup("Parameters")
    strategy.parameters:addInteger("Period1", "Period of MA1", "", 100)
    strategy.parameters:addString("Method1", "Method of MA1", "", "MVA")
    ...
    strategy.parameters:addString("Price1", "Price of MA1", "", "close")
    ...
    strategy.parameters:addInteger("Period2", "Period of MA2", "", 30)
    strategy.parameters:addString("Method2", "Method of MA2", "", "MVA")
    strategy.parameters:addStringAlternative("Method2", "MVA", "", "MVA")
    strategy.parameters:addStringAlternative("Method2", "EMA", "", "EMA")
    strategy.parameters:addStringAlternative("Method2", "KAMA", "", "KAMA")
    ...
    strategy.parameters:addString("Price2", "Price of MA2", "", "close")
    ...
    strategy.parameters:addInteger("Period3", "Period of entry MA", "", 5)
    strategy.parameters:addString("Method3", "Method of entry MA", "", "MVA")
    strategy.parameters:addStringAlternative("Method3", "MVA", "", "MVA")
    strategy.parameters:addStringAlternative("Method3", "EMA", "", "EMA")
    strategy.parameters:addStringAlternative("Method3", "KAMA", "", "KAMA")
    ...
    strategy.parameters:addString("Price3", "Price of entry MA", "", "close")
    ...
    strategy.parameters:addInteger("Entry_Shift", "Entry shift, in pips", "", 2)
...
function Prepare()
    ...
    TMAC =
        ...
        instance.parameters.Price1,
        instance.parameters.Period2,
        instance.parameters.Method2,
        instance.parameters.Price2
    )
    EntryMA =
        core.indicators:create(
        "AVERAGES",
        Source[instance.parameters.Price3],
        instance.parameters.Method3,
        instance.parameters.Period3,
        false
    )
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    TMAC:update(core.UpdateLast)
    EntryMA:update(core.UpdateLast)

    -- Check that we have enough data
    if (TMAC.DATA:first() > (period - 1)) then
        return
    end
    ...
    if TMAC.MA1_Buff[period - 1] > TMAC.MA2_Buff[period - 1] and TMAC.MA1_Buff[period] < TMAC.MA2_Buff[period] then
                ...
                if row.BS == "B" then
                    if MustOpenS then
                        ...
                        if AllowTrade then
                            Close(row)
                            if instance.parameters.AllowDirection ~= "Long" then
                                Open("S", EntryMA.DATA[period] - Entry_Shift)
                            end
                        end
                    end
                elseif row.BS == "S" then
                    if MustOpenB then
                        ...
                        if AllowTrade then
                            Close(row)
                            if instance.parameters.AllowDirection ~= "Short" then
                                Open("B", EntryMA.DATA[period] + Entry_Shift)
                            end
                        end
                    end
                end
...
function Open(side, Price)
    ...
    local CurPrice = Source.close[Source.close:size() - 1]

    if (side == "B" and Price > CurPrice) or (side == "S" and Price < CurPrice) then
