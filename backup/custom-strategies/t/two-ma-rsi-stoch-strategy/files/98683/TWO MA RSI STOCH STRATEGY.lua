...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=61827
...
function Init() --The strategy profile initialization
    strategy:name("TWO MA RSI STOCH STRATEGY")
    ...
    strategy.parameters:addInteger("Period1", "Period", "Period", 5)
    ...
    strategy.parameters:addInteger("Period2", "Period", "Period", 10)
    strategy.parameters:addString("Method2", "Method", "Method", "EMA")
    ...
    strategy.parameters:addInteger("RSIPeriod", "Period", "Period", 14)
    strategy.parameters:addDouble("RSIBuy", "Buy Level", "Level", 61)
    strategy.parameters:addDouble("RSISell", "Sell Level", "Level", 38)

    strategy.parameters:addGroup("Stochastic Calculation")
    strategy.parameters:addInteger("K", "%K Period", "", 14, 2, 1000)
...
local first
local MA1, MA2, Method2, Method1, Period1, Period2
local RSIPeriod, RSI, RSIBuy, RSISell
local Direction
local CustomID
local A1, A2, K, D, SD
local OS, OB, Stochastic
...
function Prepare(nameOnly)
    ...
    RSIBuy = instance.parameters.RSIBuy
    RSISell = instance.parameters.RSISell
    ...
    Stochastic = core.indicators:create("STOCHASTIC", Source, K, SD, D, A1, A2)
    first = math.max(MA1.DATA:first(), MA2.DATA:first(), RSI.DATA:first(), Stochastic.D:first()) + 1
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if
        RSI.DATA[period] > RSIBuy and Stochastic.K[period] > Stochastic.D[period] and Stochastic.K[period] < OB and
            core.crossesOver(MA1.DATA, MA2.DATA, period)
     then
        if Direction then
            BUY()
        else
    ...
    elseif
        RSI.DATA[period] < RSISell and Stochastic.K[period] < Stochastic.D[period] and Stochastic.K[period] > OS and
            core.crossesUnder(MA1.DATA, MA2.DATA, period)
     then
        if Direction then
            SELL()
        else
    ...
    if Exit then
        if core.crossesUnder(MA1.DATA, MA2.DATA, period) or core.crossesUnder(RSI.DATA, RSISell, period) then
            if Direction then
        ...
        if core.crossesOver(MA1.DATA, MA2.DATA, period) or core.crossesOver(RSI.DATA, RSIBuy, period) then
            if Direction then
                if haveTrades("S") then
                    exitSpecific("S")
                    Signal("Close Short")
