...
-- https://fxcodebase.com/code/viewtopic.php?f=31&t=73077
...
function Init() --The strategy profile initialization
    strategy:name("Trend following strategy with volatility filter");
    ...
    strategy.parameters:addGroup("RSI Caclulation");
    strategy.parameters:addInteger("RSI_Period", "RSI Period", "", 8);  
    strategy.parameters:addDouble("RSI_Level", "RSI Level", "", 30);  	
    strategy.parameters:addGroup("ATR Caclulation");	
    strategy.parameters:addInteger("ATR_Period", "ATR Period", "", 16);  	
    strategy.parameters:addDouble("ATR_Level", "ATR Level", "", 0.0002);  	 
...
function CreateTradingParameters()
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "TFSWVF");
...
local RSI, RSI_Period, RSI_Level; 
local ATR_Period, ATR, ATR_Level; 
function Prepare( nameOnly)
	...
	first=math.max(RSI.DATA:first(),ATR.DATA:first()); 
...
function EntryFunction( id, now,period)
    ...
    if  RSI.DATA[period]>(100-RSI_Level) and ATR.DATA[period]> ATR_Level
	then	
          if Direction then
                BUY();
            else
		...
		Return=true;
    elseif RSI.DATA[period]<RSI_Level and ATR.DATA[period]> ATR_Level 
		then
            if Direction then
                SELL();
            else
