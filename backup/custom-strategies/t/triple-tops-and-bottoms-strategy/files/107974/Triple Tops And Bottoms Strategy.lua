...
function ExtUpdate(id, source, period)  -- The method called every time when a new bid or ask price appears.
   ...
   -- get only time
	...
	local TradeFlag, Target =Calculate(period);
...
local BC=0;
			...
			local Target= (max-min)/Source:pipSize();
		...
		--	and source.high[period] < max+Delta;
			...
			if Source.close[Start]> max 
			and Source.close[Start-1]<= max 
			then
            return 1, Target;
  			elseif Source.close[Start]< min
			and  Source.close[Start-1]>= min
			then
            return -1, Target;   
            else
            return 0,0;     			
...
function enter(BuySell,Target)
    ...
    return MarketOrder(BuySell,Target);
end


-- enter into the specified direction
function MarketOrder(BuySell,Target)
