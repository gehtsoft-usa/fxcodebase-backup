...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=66272
-- Id: 
...
function Init()
    strategy:name("Trailing limit monitor");
    strategy:description("Trailing limit monitor");
...
local OFFERS_UPDATE = 3;
function Prepare(onlyName)
    trading:Prepare(onlyName);
...
function HandleNewTrade(trade_id)
    ...
    core.host:trace("New trade 2");
    trades[#trades + 1] = CreateController(trade);
end

function MoveLimits()
    for _, controller in ipairs(trades) do
        if controller.Finished ~= true then
            local trade = core.host:findTable("trades"):find("TradeID", controller.TradeID);
            if trade ~= nil then
                local offer = core.host:findTable("offers"):find("Instrument", trade.Instrument);
                if trade.BS == "S" then
                    core.host:trace(dist .. " > " .. controller.TrailingSize .. " = " .. tostring(dist >= controller.TrailingSize));
                    if dist >= controller.TrailingSize then
                        trading:MoveLimit(trade, trade.Limit + controller.TrailingSize * offer.PointSize);
                    end
                else
                    local dist = ((trade.Limit - trade.Close) / offer.PointSize) - controller.InitialLimit;
                    if dist >= controller.TrailingSize then
                        trading:MoveLimit(trade, trade.Limit - controller.TrailingSize * offer.PointSize);
                    end
                end
