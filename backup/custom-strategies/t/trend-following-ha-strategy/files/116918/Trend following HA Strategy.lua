...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=65613
...
function Init() --The strategy profile initialization
    strategy:name("Trend following HA Strategy");
...
function CreateTradingParameters()
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "TFHAS");
...
function Prepare( nameOnly)
    ...
    Indicator = core.indicators:create("HA", Source);
	 ...
	 fastMA = core.indicators:create("WMA",Indicator.close, Period);
	 slowMA = core.indicators:create("MVA",Indicator.close, Period);
	 
	first=math.max(Indicator.DATA:first(),fastMA.DATA:first(),slowMA.DATA:first() ); 
...
function EntryFunction( now,period)
    ...
    if   fastMA.DATA[period]> slowMA.DATA[period]
	and fastMA.DATA[period-1]<= slowMA.DATA[period-1]
	then	
          if Direction then
                BUY();
            else
		...
		Return=true;
    elseif   fastMA.DATA[period]< slowMA.DATA[period]
	and fastMA.DATA[period-1]>= slowMA.DATA[period-1]
 	 then
            if Direction then
                SELL();
