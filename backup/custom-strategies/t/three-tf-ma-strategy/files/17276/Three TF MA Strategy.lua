...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=7765
...
function Init() --The strategy profile initialization
    strategy:name("Three TF MA Strategy")
    ...
    Parameters(1, "m15", "MVA", 14)
    Parameters(2, "H1", "MVA", 14)
    Parameters(3, "H4", "MVA", 14)
...
function Parameters(id, TF, MVA, PERIOD)
    ...
    strategy.parameters:addInteger("PERIOD" .. id, "MVA Period", "", PERIOD, 2, 2000)

    strategy.parameters:addString("MVA" .. id, "Moving average method", "", MVA)
    strategy.parameters:addStringAlternative("MVA" .. id, "MVA", "", "MVA")
    strategy.parameters:addStringAlternative("MVA" .. id, "EMA", "", "EMA")
    strategy.parameters:addStringAlternative("MVA" .. id, "LWMA", "", "LWMA")
    strategy.parameters:addStringAlternative("MVA" .. id, "TMA", "", "TMA")
    strategy.parameters:addStringAlternative("MVA" .. id, "KAMA", "", "KAMA")
    strategy.parameters:addStringAlternative("MVA" .. id, "WMA", "", "WMA")
    strategy.parameters:addStringAlternative("MVA" .. id, "VIDYA", "", "VIDYA")
...
function Prepare(nameOnly)
    ...
    assert(core.indicators:findIndicator(MVA[i]) ~= nil, MVA[i] .. " indicator must be installed");
        Indicator[i] = core.indicators:create(MVA[i], Source[i][Price], PERIOD[i])
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if
        core.crossesOver(Source[1]["close"], Short[1], Short[1]:size() - 2) and
            Source[2]["close"][Short[2]:size() - 2] > Short[2][Short[2]:size() - 2] and
            Source[3]["close"][Short[3]:size() - 2] > Short[3][Short[3]:size() - 2]
     then
        if Direction then
            BUY()
        else
    ...
    elseif
        core.crossesUnder(Source[1]["close"], Short[1], Short[1]:size() - 2) and
            Source[2]["close"][Short[2]:size() - 2] < Short[2][Short[2]:size() - 2] and
            Source[3]["close"][Short[3]:size() - 2] < Short[3][Short[3]:size() - 2]
     then
        if Direction then
