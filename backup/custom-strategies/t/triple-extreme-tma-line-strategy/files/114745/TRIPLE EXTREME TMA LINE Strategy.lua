...
--http://fxcodebase.com/code/viewtopic.php?f=31&t=65062
...
function Init() --The strategy profile initialization
    strategy:name("TRIPLE EXTREME TMA LINE Strategy")
    ...
    strategy.parameters:addDouble("ATR_Mult1", "1. ATR multiplier", "", 2)
    strategy.parameters:addDouble("ATR_Mult2", "2. ATR multiplier", "", 4)
    strategy.parameters:addDouble("ATR_Mult3", "3. ATR multiplier", "", 6)
    strategy.parameters:addDouble("TrendThreshold", "TrendThreshold", "", 0.5)
    -- strategy.parameters:addBoolean("ShowTMA", "Show TMA", "", true);
    strategy.parameters:addBoolean("Redraw", "Redraw", "", true)

    strategy.parameters:addGroup("Selector")
    strategy.parameters:addBoolean("S1", "Use 1. Line Signals", "", true)
    strategy.parameters:addBoolean("S2", "Use 2. Line Signals", "", true)
    strategy.parameters:addBoolean("S3", "Use 3. Line Signals", "", true)
...
local Indicator, TMA_Period, ATR_Period, ATR_Mult1, ATR_Mult2, ATR_Mult3, TrendThreshold, Redraw
local S1, S2, S3
...
function Prepare(nameOnly)
    ...
    assert(
        core.indicators:findIndicator("TRIPLE EXTREME_TMA_LINE") ~= nil,
        "Please, download and install TRIPLE EXTREME_TMA_LINE.LUA indicator"
    )
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if not Source.close:hasData(period) or period < (first + 1) then
        return
    end
...
function EntryFunction(now, period)
    ...
    if
        not Indicator.Upper1:hasData(period) or not Indicator.Upper2:hasData(period) or
            not Indicator.Upper2:hasData(period)
     then
        return
    end
    ...
    if
        (Source.close[period] > Indicator.Upper1[period] and Source.close[period - 1] <= Indicator.Upper1[period - 1] and
            S1) or
            (Source.close[period] > Indicator.Upper2[period] and
                Source.close[period - 1] <= Indicator.Upper2[period - 1] and
                S2) or
            (Source.close[period] > Indicator.Upper3[period] and
                Source.close[period - 1] <= Indicator.Upper3[period - 1] and
                S3)
     then
        if Direction then
            BUY()
        ...
        Return = true
    elseif
        (Source.close[period] < Indicator.Lower1[period] and Source.close[period - 1] >= Indicator.Lower1[period - 1] and
            S1) or
            (Source.close[period] < Indicator.Lower2[period] and
                Source.close[period - 1] >= Indicator.Lower2[period - 1] and
                S2) or
            (Source.close[period] < Indicator.Lower3[period] and
                Source.close[period - 1] >= Indicator.Lower3[period - 1] and
                S3)
     then
        if Direction then
            SELL()
