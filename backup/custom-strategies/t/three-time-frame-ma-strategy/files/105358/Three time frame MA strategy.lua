...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=63265
...
function Init() --The strategy profile initialization
    strategy:name("Three time frame MA strategy")
    ...
    strategy.parameters:addInteger("Period1", "Short MA period", "", 7)
    strategy.parameters:addInteger("Period2", "Long MA period", "", 14)
...
local first
local Method, Period1, Period2, TF1, TF2, TF3
local Short_MA = {}
local Long_MA = {}
local Direction
local CustomID
...
function Prepare(nameOnly)
    ...
    Short_MA[1] = core.indicators:create(Method, Source1, Period1)
    Long_MA[1] = core.indicators:create(Method, Source1, Period2)
    Short_MA[2] = core.indicators:create(Method, Source2, Period1)
    Long_MA[2] = core.indicators:create(Method, Source2, Period2)
    Short_MA[3] = core.indicators:create(Method, Source3, Period1)
    Long_MA[3] = core.indicators:create(Method, Source3, Period2)
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    -- update indicators.
    Short_MA[1]:update(core.UpdateLast)
    Long_MA[1]:update(core.UpdateLast)
    Short_MA[2]:update(core.UpdateLast)
    Long_MA[2]:update(core.UpdateLast)
    Short_MA[3]:update(core.UpdateLast)
    Long_MA[3]:update(core.UpdateLast)

    if
        period1 < math.max(Short_MA[1].DATA:first() - 1, Long_MA[1].DATA:first() - 1) or
            period2 < math.max(Short_MA[2].DATA:first() - 1, Long_MA[2].DATA:first() - 1) or
            period3 < math.max(Short_MA[3].DATA:first() - 1, Long_MA[3].DATA:first() - 1)
     then
        return
    end
    ...
    if
        Short_MA[1].DATA[period1] > Long_MA[1].DATA[period1] and Short_MA[2].DATA[period2] > Long_MA[2].DATA[period2] and
            Short_MA[3].DATA[period3] > Long_MA[3].DATA[period3]
     then
        if Direction then
            BUY()
        else
