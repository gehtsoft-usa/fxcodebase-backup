...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=13522
...
function Init() --The strategy profile initialization
    strategy:name("TDI Trend Gap Strategy")
    ...
    strategy.parameters:addInteger("RSI_N", "RSI Periods", "Recommended values are in 8-25 range", 50, 2, 1000)
    ...
    strategy.parameters:addGroup("Level")
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if core.crossesOver(Short[1], Short[5], period) and Short[1][period] < Short[4][period] and Short[5][period] > BL then
        if Direction then
            BUY()
        else
            SELL()
    ...
    elseif
        core.crossesUnder(Short[1], Short[5], period) and Short[1][period] > Short[4][period] and Short[5][period] < SL
     then
        if Direction then
            SELL()
        else
    ...
    --exit buy : buff 4 cross under buff 5 and buff 4 cross under buy level 50
    --exit sell: buff 4 cross over buff 5 and buff 4 cross over sell level 50

    if core.crossesUnder(Short[1], Short[5], period) or core.crossesUnder(Short[1], BL, period) then
        if Direction then
    ...
    if core.crossesOver(Short[4], Short[5], period) or core.crossesOver(Short[4], SL, period) then
        if Direction then
            if haveTrades("S") then
                exit("S")
                Signal("Close Short")
