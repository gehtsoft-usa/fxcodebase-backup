...
-- https://fxcodebase.com/code/viewtopic.php?f=31&t=72163
...
function Init() --The strategy profile initialization
    strategy:name("TMA TMA Min Max Strategy");
    ...
    strategy.parameters:addInteger("Period1", "TMA Period", "", 14);  
    strategy.parameters:addInteger("Period2", "Min/Max Period", "", 14);  	
...
function CreateTradingParameters()
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "TTMMS");
...
function Prepare( nameOnly)
    ...
    Indicator = core.indicators:create("TMA", Source.close, Period1 );	
...
function ExtUpdate(id, source, period)  -- The method called every time when a new bid or ask price appears.
   ...
   Indicator:update(core.UpdateLast);
    ...
    local min1,max1=mathex.minmax(Indicator.DATA, period-1-Period2+1, period-1);	
    local min2,max2=mathex.minmax(Indicator.DATA, period-2-Period2+1, period-2);	
    EntryFunction(id,now,period,min1,max1,min2,max2);	
    ExitFunction(id, now,period,min1,max1);	
	
end


function ExitFunction(id, now,period, min, max)
...
function EntryFunction( id, now,period, min1, max1, min2, max2)
    ...
    if   Indicator.DATA[period]> max1
	and  Indicator.DATA[period-1]<=  max2
	then	
          if Direction then
                BUY();
            else
		...
		Return=true;
    elseif   Indicator.DATA[period]< min1
	and  Indicator.DATA[period-1]>=  min2
	then
            if Direction then
                SELL();
            else
