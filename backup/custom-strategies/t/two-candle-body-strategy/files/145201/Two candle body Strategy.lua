...
-- https://fxcodebase.com/code/viewtopic.php?f=31&t=71928
...
function Init() --The strategy profile initialization
    strategy:name("Two candle body Strategy");
...
function Prepare( nameOnly)
   ...
   --Indicator = core.indicators:create("MVA", Source.close, instance.parameters.Period );
    --Indicator = core.indicators:create("MVA", Source.close, Period );	
...
function EntryFunction( id, now,period)
   ...
   local LastCandleBody=math.abs(Source.open[period]-Source.close[period]);
   local PreviousCandleBody=math.abs(Source.open[period-1]-Source.close[period-1]);
   --[[
	
	Buy signal:
	Last candle body - previous candle body > 0
    ...
    if  LastCandleBody> PreviousCandleBody
	then	
          if Direction then
                BUY();
            else
		...
		Return=true;
    elseif  LastCandleBody< PreviousCandleBody
	then
            if Direction then
                SELL();
            else
