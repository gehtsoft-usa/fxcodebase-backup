...
function Init() --The strategy profile initialization
    strategy:name("Trend Clipper Strategy");
    ...
    strategy.parameters:addDouble("Multiplier", "Multiplier", "", 4, 0.5, 2000);
    strategy.parameters:addInteger("ExitPeriod", "Exit Period", "", 3);
...
function Prepare( nameOnly)
	...
	assert(core.indicators:findIndicator("TREND CLIPPER") ~= nil, "Please, download and install TREND CLIPPER.LUA indicator");          
    ...
    Indicator = core.indicators:create("TREND CLIPPER", Source, instance.parameters.Period, instance.parameters.Multiplier );	
 
	first=math.max(ExitPeriod, Indicator.DATA:first()+1); 
...
--	period3= core.findDate (Source3, TickSource:date(period), false);
    ...
    if period <= ExitPeriod then
	return;
	end
	
	
	local min,max= mathex.minmax( Source, period-1- ExitPeriod+1, period-1);
