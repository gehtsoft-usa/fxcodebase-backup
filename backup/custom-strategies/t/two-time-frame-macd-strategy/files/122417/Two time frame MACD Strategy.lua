...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=67024
...
function Init() --The strategy profile initialization
    strategy:name("Two time frame MACD Strategy");
	...
	strategy.parameters:addInteger("A1", "Fast Period", "", 12);
	strategy.parameters:addInteger("A2", "Slow Period", "", 26);
	strategy.parameters:addInteger("A3", "Signal Period", "", 9);
	
	strategy.parameters:addBoolean("Cross1", "Entry Cross Only", "", true);   
    strategy.parameters:addBoolean("Cross3", "Exit Cross Only", "", false);   

    strategy.parameters:addGroup("2. Time Frame Calculation");
	
	 strategy.parameters:addBoolean("Second", "Use Second Time Frame", "", true);   
	strategy.parameters:addString("TF2", "Time frame", "", "H1");
    strategy.parameters:setFlag("TF2", core.FLAG_PERIODS);
	
	strategy.parameters:addInteger("B1", "Fast Period", "", 12);
	strategy.parameters:addInteger("B2", "Slow Period", "", 26);
	strategy.parameters:addInteger("B3", "Signal Period", "", 9);
	
	strategy.parameters:addBoolean("Cross2", "Cross Only", "", false);  
...
function CreateTradingParameters()
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "TTFMACDS");
...
local  A,A1,A2,A3;
local  B,B1,B2,B3;
local Cross1, Cross2,Cross3,Second;
...
function Prepare( nameOnly)
	...
	Cross3= instance.parameters.Cross3;
	...
	A  = core.indicators:create("MACD", Source1.close, A1,A2,A3);
	
	if Second then
	Source2 = ExtSubscribe(3, nil, TF2, instance.parameters.Type == "Bid", "bar");
    B  = core.indicators:create("MACD", Source2.close, B1,B2,B3);
	end
...
function ExtUpdate(id, source, period)  -- The method called every time when a new bid or ask price appears.
   ...
   -- get only time
    now = now - math.floor(now);
	    ...
	    -- update indicators.
    A:update(core.UpdateLast);
	
	
		if Second then
		B:update(core.UpdateLast);
		
			if   not B.SIGNAL:hasData( period2)	then
			return;
			end
		end

	if not A.SIGNAL:hasData( period1)	 
	then
        return;
    end
    ...
    ExitFunction(now,period1 );
	end

end


function ExitFunction( now,period1 )
	 ...
	 return;
		            ...
		            if    A.MACD[period1]< A.SIGNAL[period1]
					and (not Cross3 or ( A.MACD[period1-1]>= A.SIGNAL[period1-1]))
					then
...
function EntryFunction( now,period1,period2)
	...
	if Second then
		if  B.MACD[period2]> B.SIGNAL[period2]
		and ( not Cross2 or ( B.MACD[period2-1]<= B.SIGNAL[period2-1]))
		then
		ItIs=1;
		elseif  B.MACD[period2]< B.SIGNAL[period2]
		and (not Cross2 or ( B.MACD[period2-1]>= B.SIGNAL[period2-1]))
		then
		ItIs=-1;
		end
    ...
    if   A.MACD[period1]> A.SIGNAL[period1]
	and (not Cross1 or ( A.MACD[period1-1]<= A.SIGNAL[period1-1]))
	and  (ItIs==1 or not Second ) 
	then	
          if Direction then
                BUY();
            else
		...
		Return=true;
    elseif   A.MACD[period1]< A.SIGNAL[period1]
	and (not Cross1 or ( A.MACD[period1-1]>= A.SIGNAL[period1-1])) 
	and  (ItIs==-1 or not Second ) 
	then
            if Direction then
