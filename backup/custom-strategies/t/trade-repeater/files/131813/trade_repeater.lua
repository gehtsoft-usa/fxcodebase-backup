...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=65821
...
local Modules = {};
function Init()
    strategy:name("Trading Repeater");
    strategy:description("v.1.0");
end
local result;
...
function ParseTrade(trade)
    local desc = {};
    desc.AccountID = trade.AccountID;
    desc.Instrument = trade.Instrument;
    desc.AmountK = trade.AmountK;
    desc.BS = trade.BS;
    local metadata = trading:GetTradeMetadata(trade);
    if metadata == nil then
        desc.Open = trade.Open;
    else
        desc.Open = metadata["rate"];
    end
    local stop_order = trading:FindStopOrder(trade);
    if stop_order ~= nil then
        desc.Stop = stop_order.Rate;
        desc.StopTrailing = stop_order.TrlMinMove;
    else
        desc.Stop = nil;
    end
    local limit_order = trading:FindLimitOrder(trade);
    if limit_order ~= nil then
        desc.Limit = limit_order.Rate;
    else
        desc.Limit = nil;
    end
    return desc;
...
function InitTrades()
    ...
    while (trade ~= nil) do
        trades[trade.TradeID] = ParseTrade(trade);
        trade = enum:next();
    end
end
function RepeatTrade(trade)
    core.host:trace("Repeating trade " .. tostring(trade.AccountID));
    local base_size = core.host:execute("getTradingProperty", "baseUnitSize", trade.Instrument, trade.AccountID);
    local command = trading:EntryOrder(trade.Instrument)
        :SetAccountID(trade.AccountID)
        :SetAmount(trade.Lot / base_size)
        :SetSide(trade.BS)
        :SetRate(trade.Open)
        :AddMetadata("rate", trade.Open);

    if trade.Stop ~= nil and trade.Stop ~= 0 then
        command = command:SetStop(trade.Stop);
    end
    if trade.Limit ~= nil and trade.Limit ~= 0 then
        command = command:SetLimit(trade.Limit);
    end

    result = command:Execute();
    if result.Finished then
        core.host:trace(tostring(result.Error));
        result = nil;
    end
end
function UpdateTrades()
    ...
    local trade = enum:next();
    while (trade ~= nil) do
        local desc = trades[trade.TradeID];
        if desc ~= nil then
            local stop_order = trading:FindStopOrder(trade);
            if stop_order ~= nil then
                desc.Stop = stop_order.Rate;
    ...
    elseif cookie == TRADES_UPDATE then
        ...
        if close_trade then
            ...
            if closed_trade ~= nil then
                local trade = trades[closed_trade.TradeID];
                if trade ~= nil then
                    trade.Lot = closed_trade.Lot;
                    RepeatTrade(trade);
                end
            end
        else
...
function trading:RegisterModule(modules) for _, module in pairs(modules) do self:OnNewModule(module); module:OnNewModule(self); end modules[#modules + 1] = self; self._ids_start = (#modules + 1) * 100; end
...
function trading:AsyncOperationFinished(cookie, success, message, message1, message2)
    ...
    if res ~= nil then
        ...
        if not success then
            core.host:trace(string.format("Failed request %s", tostring(message)));
        end
