...
-- http://fxcodebase.com/code//viewtopic.php?f=31&t=64253
...
function Init() --The strategy profile initialization
    strategy:name("Tilson T3 Oscillator Strategy");
    ...
    strategy.parameters:addInteger("F1", "First T3 Period", "First T3 Period", 4,2,2000);
	strategy.parameters:addInteger("F2", "Second T3 Period", "Second T3 Period", 5,2,2000);
	strategy.parameters:addInteger("N", "Normalize Period", "Normalize Period", 10,0,2000);
	strategy.parameters:addDouble("VF", "Volume Factor", "Volume Factor", 0.7); 
    strategy.parameters:addDouble("BL", "Buy Level", "", 20); 
     strategy.parameters:addDouble("SL", "Sell Level", "", 80); 
...
function CreateTradingParameters()
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "T3OS");
...
function Prepare( nameOnly)
	...
	assert(core.indicators:findIndicator("T3O") ~= nil, "Please, download and install T3O.LUA indicator");  
    ...
    Indicator = core.indicators:create("T3O", Source.close, F1, F2, N, VF );
	 
	first=Indicator.T3B:first(); 
...
function EntryFunction(period,now)
    ...
    if   Indicator.T3A[period]>  Indicator.T3B[period]
	and Indicator.T3B[period] > BL
	and Indicator.T3B[period-1]<=BL
	then	
          if Direction then
                BUY();
            else
		...
		Return=true;
    elseif  Indicator.T3A[period]< Indicator.T3B[period]
	and Indicator.T3B[period] < SL
	and Indicator.T3B[period-1] >= SL
	then
            if Direction then
                SELL();
            else
