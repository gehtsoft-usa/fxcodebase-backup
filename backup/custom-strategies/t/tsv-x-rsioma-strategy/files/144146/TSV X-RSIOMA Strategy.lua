...
-- https://fxcodebase.com/code/viewtopic.php?f=31&t=71577
...
function Init() --The strategy profile initialization
    strategy:name("TSV X-RSIOMA Strategy");
    ...
    strategy.parameters:addGroup("TSV Calculation");	
    strategy.parameters:addInteger("Period", "Period", "Period", 14);
	
    strategy.parameters:addGroup("1. Filter");	
	strategy.parameters:addString("HTF", "Hihger Time frame", "", "m1");
    strategy.parameters:setFlag("HTF", core.FLAG_PERIODS);    
	strategy.parameters:addBoolean("First", "Use First Filter", "", true)	
    strategy.parameters:addInteger("Min", "Min Bias", "", 1);	
	strategy.parameters:addBoolean("Second", "Use Second Filter", "", true)	

    strategy.parameters:addGroup("X-RSIOMA Calculation");
    strategy.parameters:addInteger("RSIOMA", "RSIOMA", "", 10)
    strategy.parameters:addInteger("Ma_RSIOMA", "Ma_RSIOMA", "", 14)
    strategy.parameters:addBoolean("rsi_use_obos", "Use RSI OB/OS", "", true)
    strategy.parameters:addInteger("rsi_os", "RSI Oversold", "", 10)
    strategy.parameters:addInteger("rsi_ob", "RSI Overbought", "", 90)
...
function CreateTradingParameters()
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "TXS");
...
local HTF, First,Min, HTF_Source; 
local Second;
function Prepare( nameOnly)
    ...
    s2, e2 = core.getcandle(HTF, 0, 0, 0);
    ...
    assert(HTF ~= "t1", "The time frame must not be tick"); 
 
   
    PrepareTrading();
 
	assert(core.indicators:findIndicator("TSV") ~= nil, "Please, download and install TSV.LUA indicator");          
	assert(core.indicators:findIndicator("X-RSIOMA") ~= nil, "Please, download and install X-RSIOMA.LUA indicator"); 	
    ...
    Source1 = ExtSubscribe(2, nil, TF, instance.parameters.Type == "Bid", "bar");
    Source2 = ExtSubscribe(3, nil, HTF, instance.parameters.Type == "Bid", "bar");	
    TSV = core.indicators:create("TSV", Source1, instance.parameters.Period  ); 
    XRSIOMA = core.indicators:create("X-RSIOMA", Source1, instance.parameters.RSIOMA,instance.parameters.Ma_RSIOMA, instance.parameters.rsi_use_obos, instance.parameters.rsi_os, instance.parameters.rsi_ob  ); 	
    HTF_XRSIOM = core.indicators:create("X-RSIOMA",  Source2, instance.parameters.RSIOMA,instance.parameters.Ma_RSIOMA, instance.parameters.rsi_use_obos, instance.parameters.rsi_os, instance.parameters.rsi_ob  ); 	
	
	
	first=math.max(TSV.DATA:first(),XRSIOMA.b:first()); 
...
function ExtUpdate(id, source, period)  -- The method called every time when a new bid or ask price appears.
   ...
   -- get only time
    now = now - math.floor(now);
	    ...
	    -- update indicators.
    TSV:update(core.UpdateLast);
    XRSIOMA:update(core.UpdateLast);
    HTF_XRSIOM:update(core.UpdateLast);
...
function ExitFunction(id, now,period)
	...
	--/////////////////////////////////////////////////////////////////  
	if  LastExit == Source:serial(period1)
	...
	--/////////////////////////////////////////////////////////////////////
		            ...
		            if    CloseLong then
								...
								LastExit= Source:serial(period1);
					  end
					  if  CloseShort then
...
function EntryFunction( id, now,period)
	...
	--/////////////////////////////////////////////////////////////////    
     if LastEntry == Source1:serial(period1)  
     --or not MA.DATA:hasData(period)
	 then
	 return;
	 end	
    ...
    local B1, B2=CountB(period2)    
	  
    -- only buy if we have a fast cross over slow and the price is above the moving averages.
    if   TSV.DATA[period1]> 0 
    and XRSIOMA.b[period1]> 0 	
	and (TSV.DATA[period1-1]<= 0 
	or XRSIOMA.b[period1-1]<= 0 )	
	and (B1 > Min and First or not First or (B1< B2 and Second ) )
	then	
          if Direction then
                BUY();
            else
...
function CountB(period2)

    local Last=period2;
    local B={0,0};
	
	for period = period2-1, 1, -1 do
	
	
	  if (HTF_XRSIOM.RSI[Last]> 50 
	  and  HTF_XRSIOM.RSI[period]< 50 )
	  or (HTF_XRSIOM.RSI[Last]< 50 
	  and  HTF_XRSIOM.RSI[period]> 50 )  
	  and B[1]~=0
	  and B[2]==0	  
	  then
