...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=67276
...
function Init() --The strategy profile initialization
    strategy:name("Two MA RSI ADX Strategys");
	...
	strategy.parameters:addInteger("Period1", "MA Period", "Period" , 5);
	...
	strategy.parameters:addInteger("Period2", "MA Period", "Period" , 10);
	...
	strategy.parameters:addInteger("ADX_Period", "ADX Period", "Period" , 14);
	strategy.parameters:addDouble("ADX_Level", "ADX Level", "Level" , 20);
...
function CreateTradingParameters()
   ...
   -- strategy.parameters:addStringAlternative("ExitExecutionType", "End of Turn", "", "EndOfTurn");
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "TMRAS");
...
function Prepare( nameOnly)
	...
	first=math.max(MA1.DATA:first(),MA2.DATA:first(),ADX.DATA:first(),RSI.DATA:first()); 
...
function EntryFunction( now,period)
	...
	and ((ADX.DATA[period] >=ADX_Level and Use_ADX) or not Use_ADX)
	then	
          if Direction then
                BUY();
            else
