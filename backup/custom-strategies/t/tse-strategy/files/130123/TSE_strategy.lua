...
--http://fxcodebase.com/code/viewtopic.php?f=31&t=69205
...
local STRATEGY_NAME = "TSE strategy";
...
function CreateParameters()
    strategy.parameters:addInteger("ema1_period", "EMA 1 period", "", 18);
    strategy.parameters:addInteger("ema2_period", "EMA 2 period", "", 50);
    strategy.parameters:addInteger("ema3_period", "EMA 3 period", "", 200);
    strategy.parameters:addDouble("entry_shift", "Entry shift, pips", "", 3);
    strategy.parameters:addString("rule_4", "Rule 4 mode", "", "EMA1")
    strategy.parameters:addStringAlternative("rule_4", "EMA1", "", "EMA1");
    strategy.parameters:addStringAlternative("rule_4", "EMA2", "", "EMA2");
end
...
local fractal, ema1, ema2, ema3, entry_shift, rule_4;
function CreateEntryIndicators(source)
    entry_shift = instance.parameters.entry_shift;
    rule_4 = instance.parameters.rule_4;
    fractal = core.indicators:create("FRACTAL", source);
    ema1 = core.indicators:create("EMA", source, instance.parameters.ema1_period);
    ema2 = core.indicators:create("EMA", source, instance.parameters.ema2_period);
    ema3 = core.indicators:create("EMA", source, instance.parameters.ema3_period);
end
function CreateExitIndicators(source) end

function UpdateIndicators()
    fractal:update(core.UpdateLast);
    ema1:update(core.UpdateLast);
    ema2:update(core.UpdateLast);
    ema3:update(core.UpdateLast);
end

function GetEntryRate(source, bs, period) 
    return source.close[period] + entry_shift * source:pipSize(); 
end
function SetCustomStop(position_desc, command, period, periods_from_last, source) 
    local engulfing, side = GetEngulfing(source, period);
    if position_desc.Side == "B" then
        command:SetStop(nil, source.low[engulfing] - position_desc.stop_shift * source:pipSize());
    else
        command:SetStop(nil, source.high[engulfing] + position_desc.stop_shift * source:pipSize());
    end
    return true;
end
...
function GetValue(stream, period)
    for i = period, 0, -1 do
        if stream:hasData(i) then
            local level, _ = stream:get(i);
            return level, i;
        end
    end
    return nil, nil;
...
function GetEngulfing(source, period)
    for i = period, 1, -1 do
        if source.low[i] <= math.min(source.low[i - 1], source.high[i - 1]) 
            and source.close[i] > math.max(source.low[i - 1], source.high[i - 1])
        then
            return i, 1;
        end
        if source.high[i] >= math.max(source.low[i - 1], source.high[i - 1]) 
            and source.close[i] < math.min(source.low[i - 1], source.high[i - 1])
        then
            return i, -1;
        end
    end
...
function CreateCustomActions()
    ...
    enterLongAction.IsPass = function (source, period, periodFromLast, data)
        local up_fractal0, period0 = GetValue(fractal:getTextOutput(0), period);
        if up_fractal0 == nil or period0 == 0 then
            return false;
        end
        local up_fractal1 = GetValue(fractal:getTextOutput(0), period0 - 1);
        if up_fractal1 == nil then
            return false;
        end
        if up_fractal0 <= up_fractal1 then
            return false;
        end

        local down_fractal0, period0 = GetValue(fractal:getTextOutput(1), period);
        if down_fractal0 == nil or period0 == 0 then
            return false;
        end
        local down_fractal1 = GetValue(fractal:getTextOutput(1), period0 - 1);
        if down_fractal1 == nil then
            return false;
        end
        if down_fractal0 <= down_fractal1 then
            return false;
        end

        local engulfing, side = GetEngulfing(source, period);

        return source.close[period] > ema1.DATA[period]
            and ema1.DATA[period] > ema2.DATA[period]
            and ema2.DATA[period] > ema3.DATA[period]
            and ((source.low[period] < ema1.DATA[period] and rule_4 == "EMA1") or (source.low[period] < ema2.DATA[period] and rule_4 == "EMA2"))
            and side == 1;
    end
    ...
    enterShortAction.IsPass = function (source, period, periodFromLast, data)
        ...
        return source.close[period] < ema1.DATA[period]
            and ema1.DATA[period] < ema2.DATA[period]
            and ema2.DATA[period] < ema3.DATA[period]
            and ((source.high[period] > ema1.DATA[period] and rule_4 == "EMA1") or (source.high[period] > ema2.DATA[period] and rule_4 == "EMA2"))
            and side == -1;
    end
