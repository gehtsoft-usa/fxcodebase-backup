...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=64467
...
function Init() --The strategy profile initialization
    strategy:name("True Strength Index Tilson T3 Average Cross Strategy")
    ...
    strategy.parameters:addInteger("Entry_BL", "Entry Long Level", "", 0)
    strategy.parameters:addInteger("Entry_SL", "Entry Short Level", "", 0)
    strategy.parameters:addInteger("Exit_BL", "Exit Long Level", "", 0)
    strategy.parameters:addInteger("Exit_SL", "Exit Short Level", "", 0)
    ...
    strategy.parameters:addInteger("N", "Smooth 1", "", 7, 2, 1000)
    strategy.parameters:addInteger("M", "Smooth 2", "", 14, 2, 1000)
...
local VF, F, T3
local TSI, N, M
local Exit_BL, Exit_SL
local Entry_BL, Entry_SL
function Prepare(nameOnly)
    ...
    Exit_BL = instance.parameters.Exit_BL
    Entry_BL = instance.parameters.Entry_BL
    Exit_SL = instance.parameters.Exit_SL
    Entry_SL = instance.parameters.Entry_SL
    ...
    TSI = core.indicators:create("TSI", Source.close, N, M)
    first = math.max(T3.DATA:first(), TSI.DATA:first())
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if not TSI.DATA:hasData(period) or period < first then
        return
    end
...
function ExitFunction(now, period)
    ...
    if Source.close[period] < T3.DATA[period] or TSI.DATA[period] < Exit_BL then
        if Direction then
    ...
    if Source.close[period] > T3.DATA[period] or TSI.DATA[period] > Exit_SL then
        if Direction then
            if haveTrades("S") then
                exitSpecific("S")
                Signal("Close Short")
...
function EntryFunction(now, period)
    ...
    if Source.close[period] > T3.DATA[period] and TSI.DATA[period] > Entry_BL and TSI.DATA[period - 1] <= Entry_BL then
        if Direction then
            BUY()
        else
            SELL()
        ...
        Return = true
    elseif Source.close[period] < T3.DATA[period] and TSI.DATA[period] < Entry_SL and TSI.DATA[period - 1] >= Entry_SL then
        if Direction then
            SELL()
        else
            BUY()
