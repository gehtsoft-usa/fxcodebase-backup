...
function Init() --The strategy profile initialization
	...
	strategy.parameters:addDouble("OS", "Oversold Level", "", 25);
	 ...
	 strategy.parameters:addBoolean("Reverse", "Trade only during Oversold Overbought conditions", "", true);
...
function ExtUpdate(id, source, period)  -- The method called every time when a new bid or ask price appears.
    ...
    now = now - math.floor(now);
      ...
      return ;
		...
		and  ((	Short[1][period]  > OB and  Reverse)
		or  (	Short[1][period]  < OB and not Reverse))	
		then
		...
		and  ((	Short[1][period] < OS and   Reverse)
		or  (	Short[1][period]  > OS and  not Reverse))
		 then
            if Direction then
                SELL();
            else
