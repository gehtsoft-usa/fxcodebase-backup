-- Available @ https://fxcodebase.com/code/viewtopic.php?f=31&t=74439
...
function Init() --The strategy profile initialization
    strategy:name("Nadaraya-Watson_Smoothers Strategy");
    ...
    strategy.parameters:addDouble("param1", "Bandwidth", "", 8., 0);
    strategy.parameters:addString("param2", "Source", "", "close");
    ...
    strategy.parameters:addBoolean("param3", "Repainting Smoothing", "", false);
...
function CreateTradingParameters()
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "NWSS");
...
function Prepare( nameOnly)
	...
	assert(core.indicators:findIndicator("NADARAYA-WATSON_SMOOTHERS") ~= nil, "Please, download and install NADARAYA-WATSON_SMOOTHERS.LUA indicator");          
    ...
    Indicator = core.indicators:create("NADARAYA-WATSON_SMOOTHERS", Source , instance.parameters.param1  , instance.parameters.param2 , instance.parameters.param3, core.colors().Green, core.colors().Red);
...
--	period3= core.findDate (Source3, TickSource:date(period), false);
	...
	--/////////////////////////////////////////////////////////////////////
		            ...
		            if     Indicator.DATA:colorI(period) ~= core.colors().Green
					then
					  ...
					  if   Indicator.DATA:colorI(period) ~=  core.colors().Red
					  then
...
function EntryFunction( id, now,period)
    ...
    if  Indicator.DATA:colorI(period) == core.colors().Green
	and Indicator.DATA:colorI(period-1) ~= core.colors().Green
	then	
          if Direction then
                BUY();
            else
		...
		Return=true;
    elseif  Indicator.DATA:colorI(period) == core.colors().Red
	and Indicator.DATA:colorI(period-1) ~= core.colors().Red
	then
            if Direction then
                SELL();
            else
