...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=24509
...
function Init() --The strategy profile initialization
    strategy:name("Hammer / Hanging Man Patterns Strategy")
    ...
    strategy.parameters:addInteger("ATR_Period", "ATR Period", "ATR Period", 14)
    strategy.parameters:addDouble("ATR_Multiplier", "ATR Multiplier", "ATR Multiplier", 2)

    strategy.parameters:addGroup("Calculation")
    strategy.parameters:addDouble(
        "Ratio",
        "Minimal Lower Shadow / Body  Ratio",
        "Minimal Lower Shadow / Body  Ratio",
        2
    )
    strategy.parameters:addDouble(
        "Upper",
        "Minimal Body / Upper Shadow  Ratio",
        "Minimal Body / Upper Shadow  Ratio",
        10
    )
    strategy.parameters:addBoolean("Body", "Use Body orientation Filter ", "", false)
    strategy.parameters:addBoolean("Reversal", "Use Trend Reversal Filter ", "", true)
    strategy.parameters:addInteger("Period", "Trend Reversal Filter Period", "Trend Reversal Filter Period", 5)

    strategy.parameters:addGroup("Trade Selection")
    strategy.parameters:addBoolean("P1", "Trade Hammer", "", true)
    strategy.parameters:addBoolean("P2", "Trade Hanging Man", "", true)
...
local first
local Period, P1, P2, Ratio, Upper, Body, Reversal
local Indicator = {}
local Short = {}
local Direction
local CustomID
...
local ATR, ATR_Multiplier, ATR_Period
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if
        (math.abs(Source.open[period] - Source.close[period]) * Ratio) >
            (math.min(Source.open[period], Source.close[period]) - Source.low[period])
     then
        return
    end
    ...
    if
        (math.abs(Source.open[period] - Source.close[period])) <
            ((Source.high[period] - math.max(Source.open[period], Source.close[period])) * Upper)
     then
        return
    end
