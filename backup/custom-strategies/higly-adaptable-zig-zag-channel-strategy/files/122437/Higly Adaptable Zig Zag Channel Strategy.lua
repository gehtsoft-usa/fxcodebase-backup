...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=67032
...
local HISTORY_PRELOAD_BARS = 5000;

local STRATEGY_NAME = "Highly Adaptable Zig Zag Channel Strategy";
...
function CreateParameters()
    strategy.parameters:addString("zz_timeframe", "Time frame", "", "m5");
    strategy.parameters:setFlag("zz_timeframe", core.FLAG_PERIODS);
    ...
    strategy.parameters:addInteger("ZZPeriod", "Period", "Period", 1);
...
function CreateCustomActions()
    ...
    action1.IsPass = function (source, period)
        if not indi.Top:hasData(NOW - 1) then
            return;
        end
        return core.crossesOver(source.close, indi.Top, period - 1, NOW - 1); 
    end
    if isEntry1 then
    ...
    action2.IsPass = function (source, period) 
        ...
        return core.crossesUnder(source.close, indi.Top, period - 1, NOW - 1); 
    end
    if isEntry2 then
    ...
    action3.IsPass = function (source, period) 
        if not indi.Bottom:hasData(NOW - 1) then
            return;
        end
        return core.crossesOver(source.close, indi.Bottom, period - 1, NOW - 1); 
    end
    if isEntry3 then
    ...
    action4.IsPass = function (source, period) 
        ...
        return core.crossesUnder(source.close, indi.Bottom, period - 1, NOW - 1); 
    end
    if isEntry4 then
...
local indicator_source;
function CreateIndicators(source)
    indicator_source = ExtSubscribe(3, source:instrument(), instance.parameters.zz_timeframe, source:isBid(), "bar");
    EnsureIndicatorInstalled("TICK ZIGZAG CHANNEL WITH OUTPUT");
    indi = core.indicators:create("TICK ZIGZAG CHANNEL WITH OUTPUT", indicator_source.close
        , instance.parameters.Depth
        , instance.parameters.Deviation
        , instance.parameters.Backstep
        , instance.parameters.ZZPeriod, 0);
end
...
function signaler:FormatEmail(source, period, message)
    ...
    local subject = message .. "(" .. source.close:instrument() .. ")";
    --format email text
    local delim = "\013\010";
    local signalDescr = "Signal: " .. (self.StrategyName or "");
    local symbolDescr = "Symbol: " .. source.close:instrument();
...
function signaler:Signal(label, source)
    ...
    if self._show_alert then
        terminal:alertMessage(source.close:instrument(), source[NOW], label, source:date(NOW));
    end
    ...
    if self._advanced_alert_key ~= nil then
        self:AlertTelegram(label, source.close:instrument(), source:barSize());
    end
