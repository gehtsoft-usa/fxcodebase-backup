...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=2760
...
function Init()
    strategy:name("Aroon oscillator strategy")
    strategy:description("Aroon oscillator strategy")
    ...
    strategy.parameters:addInteger("Period", "Period of Aroon oscillator", "", 25, 3, 1000)
    strategy.parameters:addDouble("Level", "Level for open/close orders", "", 50, 1, 100)
...
function Prepare(nameOnly)
    ...
    assert(
        core.indicators:findIndicator("AROON OSCILLATOR") ~= nil,
        "Please, download and install AROON OSCILLATOR.LUA indicator"
    )

    Level = instance.parameters.Level
    gSource = ExtSubscribe(1, nil, instance.parameters.TF, instance.parameters.Type == "Bid", "bar")
    Aroon = core.indicators:create("AROON OSCILLATOR", gSource, Period)
end
...
function ExtUpdate(id, source, period)
    ...
    local MustB = false
    local MustS = false
    if Aroon.DATA[period - 1] < Level and Aroon.DATA[period] > Level then
        MustB = true
    end
    if Aroon.DATA[period - 1] > -Level and Aroon.DATA[period] < -Level then
        MustS = true
