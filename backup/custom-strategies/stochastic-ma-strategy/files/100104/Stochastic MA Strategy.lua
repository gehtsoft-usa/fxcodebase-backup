...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=62156
...
function Init() --The strategy profile initialization
    strategy:name("Stochastic MA Strategy")
    ...
    strategy.parameters:addStringAlternative("A1", "MetaTrader", "", "FS")
...
function Prepare(nameOnly)
    ...
    Stochastic =
        ...
        instance.parameters.A1,
        instance.parameters.A2
    )
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if MA1.DATA[period] > MA2.DATA[period] and Stochastic.K[period] < OS and Stochastic.K[period - 1] >= OS then
        if Direction then
            BUY()
        else
            SELL()
    ...
    elseif MA1.DATA[period] < MA2.DATA[period] and Stochastic.K[period] > OB and Stochastic.K[period - 1] <= OB then
        if Direction then
            SELL()
        else
            BUY()
    ...
    if Exit then
        if Stochastic.K[period] > OB then
            if Direction then
        ...
        if Stochastic.K[period] < OS then
            if Direction then
                if haveTrades("S") then
                    exitSpecific("S")
                    Signal("Close Short")
