...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=66858
...
function Init()
    strategy:name("AlgoWizar1540218343269");
    strategy:description("v2 2018-11-17");

    strategy.parameters:addInteger("Period", "Period", "", 50, 1, 10000);
    strategy.parameters:addDouble("Deviation", "Deviation", "", 2.1, 0.0001, 1000.0);
    strategy.parameters:addInteger("stdev_periods", "St.Dev. periods", "", 50);
    strategy.parameters:addInteger("ADXPeriod", "Period ADX", "Number of Periods", 40);
...
function Prepare(name_only)
    ...
    stdev_periods = instance.parameters.stdev_periods;
    local valid;
    ...
    bb = core.indicators:create("BB", trading_logic.MainSource, instance.parameters.Period, instance.parameters.Deviation);
    adx = core.indicators:create("ADX", trading_logic.MainSource, instance.parameters.ADXPeriod);
end

function IsBuyCondition(source, period)
    if bb.AL:size() < 3 or not bb.AL:hasData(period - 2) then
        return false;
    end
    return core.crossesOver(source.open, bb.AL, period, period - 1);
end
...
function IsSellCondition(source, period)
    ...
    return core.crossesUnder(source.open, bb.AL, period, period - 1);
end

function IsExitBuyCondition(source, period)
    if adx.DATA:size() < 5 or not adx.DATA:hasData(period - 4) then
        return false;
    end
    return mathex.stdev(source.typical, period - stdev_periods - 5, period - 5) > mathex.stdev(source.typical, period - stdev_periods - 4, period - 4)
        and mathex.stdev(source.typical, period - stdev_periods - 4, period - 4) < mathex.stdev(source.typical, period - stdev_periods - 3, period - 3)
        and adx.DATA[period - 4] > adx.DATA[period - 3]
        and adx.DATA[period - 3] < adx.DATA[period - 2];
end
