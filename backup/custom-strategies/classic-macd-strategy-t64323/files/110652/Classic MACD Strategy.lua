...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=64323
...
function Init() --The strategy profile initialization
    strategy:name("Classic MACD Strategy")
    ...
    strategy.parameters:addInteger("SN", "Short EMA Period", "", 12, 2, 1000)
    strategy.parameters:addInteger("LN", "Long EMA Period", "", 26, 2, 1000)
    strategy.parameters:addInteger("IN", "Signal MA Period", "", 9, 2, 1000)
    strategy.parameters:addInteger("Confirmation", "Confirmation Period", "", 0, 0, 1000)
...
function Prepare(nameOnly)
    ...
    assert(
        core.indicators:findIndicator("CLASSIC MACD") ~= nil,
        "Please, download and install CLASSIC MACD.LUA indicator"
    )
    ...
    Indicator = core.indicators:create("CLASSIC MACD", Source.close, SN, LN, IN)
...
function ItIs(Start, Flag)
    if Indicator.MACD[Start] < Indicator.Signal[Start] and Flag == 1 then
        return false
    end
    ...
    for period = Start, Start - Confirmation, -1 do
        if Flag == 1 then
            if
                Indicator.MACD[period] > Indicator.Signal[period] and
                    Indicator.MACD[period - 1] <= Indicator.Signal[period - 1]
             then
                Cross = (Start - period)
                break
            end

            if Indicator.MACD[period] < Indicator.Signal[period] then
                Cross = 0
                break
            end
        else
