...
function Init() --The strategy profile initialization
    strategy:name("Previous candle RangeBreakOut Strategy");
    ...
    strategy.parameters:addInteger("Period", "MA Period", "", 34);  
...
function Prepare( nameOnly)
    ...
    MA = core.indicators:create("MVA", Source.close, instance.parameters.Period ); 
	first=MA.DATA:first(); 
...
function EntryFunction( id, now,period)
    ...
    local Range=math.abs(Source.close[period-1]-Source.open[period-1])/2;     
    local Max=math.max(Source.close[period-1],Source.open[period-1])  
    local Min=math.min(Source.close[period-1],Source.open[period-1])  
	
    -- only buy if we have a fast cross over slow and the price is above the moving averages.
    if   Source.close[period]>  Max +Range
	and  Source.close[period-1]<=  Max +Range
	...
	then	
		...
		Return=true;
    elseif   Source.close[period]<  Min -Range
	and  Source.close[period-1] >=  Min -Range
	and  Source.close[period] < MA.DATA[period]	
		then
            if Direction then
                SELL();
