...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=64314
...
function Init() --The strategy profile initialization
    strategy:name("RLW SSD Strategy");
...
function CreateTradingParameters()
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "RLWSSDS");
...
function Prepare( nameOnly)
	...
	first=math.max(RLW.DATA:first(), SSD.DATA:first()); 
...
function EntryFunction(period,now)
    ...
    if  RLW.DATA[period]> BL  
	and  RLW.DATA[period-1] <= BL 
	then	
          if Direction then
                BUY();
            else
		...
		Return=true;
    elseif  SSD.DATA[period]< SL  
	and  SSD.DATA[period-1] >= SL 
	then
            if Direction then
                SELL();
            else
