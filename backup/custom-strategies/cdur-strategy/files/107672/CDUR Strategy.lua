...
function Prepare(nameOnly)
    ...
    assert(core.indicators:findIndicator("CDUR") ~= nil, "Please, download and install CDUR.LUA indicator")
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if
        Indicator.CDURUTS[period] > Indicator.CDURUTC[period] and
            Indicator.CDURUTS[period - 1] <= Indicator.CDURUTC[period - 1]
     then
        if Direction then
            BUY()
        else
