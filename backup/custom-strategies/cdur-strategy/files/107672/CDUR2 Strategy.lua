...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=63777
...
function Init() --The strategy profile initialization
    ...
    strategy.parameters:addInteger("Period11", "Period", "", 9, 2, 2000)
    strategy.parameters:addInteger("Period12", "Period", "", 19, 2, 2000)
    strategy.parameters:addInteger("Period13", "Period", "", 6, 2, 2000)
    strategy.parameters:addInteger("Period14", "Period", "", 5, 2, 2000)

    strategy.parameters:addGroup("MA Calculation")
    strategy.parameters:addInteger("Period21", "Period", "", 45, 2, 2000)
    strategy.parameters:addInteger("Period22", "Period", "", 95, 2, 2000)
    strategy.parameters:addInteger("Period23", "Period", "", 6, 2, 2000)
    strategy.parameters:addInteger("Period24", "Period", "", 8, 2, 2000)
...
function Prepare(nameOnly)
    ...
    assert(core.indicators:findIndicator("CDUR2") ~= nil, "Please, download and install CDUR2.LUA indicator")
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if
        Indicator.CDURUTS:colorI(period) == core.rgb(0, 255, 0) and
            Indicator.CDURUTC:colorI(period) == core.rgb(0, 255, 0) and
            (Indicator.CDURUTS:colorI(period - 1) ~= core.rgb(0, 255, 0) or
                Indicator.CDURUTC:colorI(period - 1) ~= core.rgb(0, 255, 0))
     then
        if Direction then
            BUY()
        else
    ...
    elseif
        Indicator.CDURUTS:colorI(period) == core.rgb(255, 0, 0) and
            Indicator.CDURUTC:colorI(period) == core.rgb(255, 0, 0) and
            (Indicator.CDURUTS:colorI(period - 1) ~= core.rgb(255, 0, 0) or
                Indicator.CDURUTC:colorI(period - 1) ~= core.rgb(255, 0, 0))
     then
        if Direction then
            SELL()
        else
    ...
    if Exit then
        if
            Indicator.CDURUTS:colorI(period) ~= core.rgb(0, 255, 0) or
                Indicator.CDURUTC:colorI(period) ~= core.rgb(0, 255, 0)
         then
            if Direction then
        ...
        if
            Indicator.CDURUTS:colorI(period) ~= core.rgb(255, 0, 0) or
                Indicator.CDURUTC:colorI(period) ~= core.rgb(255, 0, 0)
         then
            if Direction then
                if haveTrades("S") then
                    exitSpecific("S")
