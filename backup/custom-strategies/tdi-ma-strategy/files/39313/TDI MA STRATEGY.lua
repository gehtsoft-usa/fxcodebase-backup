...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=22783
...
function Init() --The strategy profile initialization
    ...
    strategy.parameters:addStringAlternative("RSI_P_M", "VIDYA", "", "VIDYA")
    ...
    strategy.parameters:addStringAlternative("RSI_P_M", "TMA", "", "TMA")
    ...
    strategy.parameters:addStringAlternative("TS_M", "VIDYA", "", "VIDYA")
    ...
    strategy.parameters:addStringAlternative("TS_M", "TMA", "", "TMA")

    strategy.parameters:addGroup("MA Calculation")
    strategy.parameters:addInteger("MP", "MA Period", "", 34)

    strategy.parameters:addString("MM", "MA Method", "", "MVA")
    strategy.parameters:addStringAlternative("MM", "MVA(SMA)", "", "MVA")
    strategy.parameters:addStringAlternative("MM", "EMA", "", "EMA")
    strategy.parameters:addStringAlternative("MM", "LWMA", "", "LWMA")
    strategy.parameters:addStringAlternative("MM", "VIDYA", "", "VIDYA")
    strategy.parameters:addStringAlternative("MM", "SMMA", "", "SMMA")
    strategy.parameters:addStringAlternative("MM", "WMA(Wilders)", "", "WMA")
    strategy.parameters:addStringAlternative("MM", "KAMA(Kaufman)", "", "KAMA")
    strategy.parameters:addStringAlternative("MM", "TMA", "", "TMA")

    strategy.parameters:addGroup("Entry/Exit Levels")

    strategy.parameters:addDouble("BENTRY", "BUY ENTRY LEVEL", "", 50)
    strategy.parameters:addDouble("SENTRY", "SELL ENTRY LEVEL", "", 50)

    strategy.parameters:addDouble("BEXIT", "BUY EXIT LEVEL", "", 70)
    strategy.parameters:addDouble("SEXIT", "SELL EXIT LEVEL", "", 30)
...
local MP, MM
local BENTRY, BEXIT, SENTRY, SEXIT
...
function Prepare(nameOnly)
    ...
    BENTRY = instance.parameters.BENTRY
    BEXIT = instance.parameters.BEXIT
    SENTRY = instance.parameters.SENTRY
    SEXIT = instance.parameters.SEXIT
    ...
    Indicator[1] =
        core.indicators:create("TRADERSDYNAMICINDEX", Source.close, RSI_N, VB_N, VB_W, RSI_P_N, RSI_P_M, TS_N, TS_M)
    Short[4] = Indicator[1]:getStream(0)
    Short[1] = Indicator[1]:getStream(1)
    Short[3] = Indicator[1]:getStream(2)
    Short[2] = Indicator[1]:getStream(3)
    Short[5] = Indicator[1]:getStream(4)

    assert(core.indicators:findIndicator(MM) ~= nil, MM .. " indicator must be installed");
    Indicator[2] = core.indicators:create(MM, Source.high, MP)
    MA[1] = Indicator[2]:getStream(0)
    Indicator[3] = core.indicators:create(MM, Source.low, MP)
    MA[2] = Indicator[3]:getStream(0)
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    --2.BUFF5>BUFF2
    --3.BUFF4< BUFF1
    --4.BUFF4> BUY ENTRY LEVEL
    --5.BUFF4< BUY EXIT LEVEL

    --ALONG WITH ABOVE CONDITIONS PRICE CROSS OVER MVA 34 PRICE APPLIED TO HIGH

    if
        Short[4][period] > Short[5][period] and Short[5][period] > Short[2][period] and
            Short[4][period] < Short[1][period] and
            Short[4][period] > BENTRY and
            Short[4][period] < BEXIT and
            Source.close[period] > MA[1][period]
     then
        --	SELLING CONDITION:
        --IN TDI FOLLOWING CONDITION SHOULD BE SATISFIED
        --1.BUFF4< BUFF 5
        --2.BUFF5<BUFF2
        --3.BUFF4 >BUFF 3
        --4.BUFF4< SELL ENTRY LEVEL
        --5.BUFF4> SELL EXIT LEVEL
    ...
    elseif
        Short[4][period] < Short[5][period] and Short[5][period] < Short[2][period] and
            Short[4][period] > Short[3][period] and
            Short[4][period] < SENTRY and
            Short[4][period] > SEXIT and
            Source.close[period] < MA[2][period]
     then
        if Direction then
            SELL()
        else
    ...
    --EXIT SELL:
    --BUFF 4 CROSS OVER BUFF5

    if Short[4][period] < Short[5][period] then
        if Direction then
