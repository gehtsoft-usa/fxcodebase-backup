...
function Prepare(onlyName)
    ...
    local name = profile:id() .. "(" .. instance.bid:instrument() .. "[" .. instance.parameters.Period .. "], " .. instance.parameters.Trade .. ")"
...
function breakeven:CreateController()
    ...
    function controller:getTo()
        ...
        core.host:trace(tostring(self._to));
        if trade.BS == "B" then
...
function breakeven:CreatePriceController()
    ...
    function controller:DoBreakeven()
        ...
        if Source.close:size() > 1 and core.crosses(Source.close, self._when, Source:size() - 1) then
            self._parent._trading:MoveStop(self:getTo(), trade);
            self._executed = true;
            return false;
        end
