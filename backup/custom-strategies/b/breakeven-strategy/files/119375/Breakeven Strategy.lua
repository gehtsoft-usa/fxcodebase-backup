...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=13498&sid=fe28f995d8f09bf16c1d106adeed9199
...
function Init()
    ...
    strategy.parameters:addString("StopSelector", "Stop Selector", "", "Profit");
    strategy.parameters:addStringAlternative("StopSelector", "Profit", "", "Profit");
    strategy.parameters:addStringAlternative("StopSelector", "Price", "", "Price");
...
function Prepare(onlyName)
    ...
    name = profile:id() .. "(" .. instance.bid:instrument() .. "[" .. instance.parameters.Period .. "], " .. instance.parameters.Trade .. ")"
    ...
    local trade = core.host:findTable("trades"):find("TradeID", tradeId);
    if instance.parameters.StopSelector == "Profit" then
        breakeven:CreateController()
            :SetWhen(Profit)
            :SetTo(0)
            :SetTrade(trade)
    else
        breakeven:CreatePriceController()
            :SetWhen(Price)
            :SetTo(0)
            :SetTrade(trade)
    end
...
function ExtAsyncOperationFinished(id, success, message) for _, module in pairs(Modules) do if module.AsyncOperationFinished ~= nil then module:AsyncOperationFinished(cookie, success, message, message1, message2); end end end
...
function breakeven:CreatePriceController()
    local controller = {};
    controller._parent = self;
    controller._executed = false;
    controller._distStopTrade = core.host:execute("getTradingProperty", "conditionalDistanceStopForTrade", instance.bid:instrument());
    ...
    function controller:getTo()
        ...
        local offer = core.host:findTable("offers"):find("Instrument", trade.Instrument);
        if trade.BS == "B" then
            local maxAvaliablePrice = offer.Bid - (self._distStopTrade + 0.1  - self._to) * offer.PointSize;
            if self._when < maxAvaliablePrice then 
                return self._when
            end
            return maxAvaliablePrice;
        else
            local minAvaliablePrice = offer.Ask + (self._distStopTrade + 0.1 - self._to) * offer.PointSize;
            if self._when > minAvaliablePrice then 
                return self._when 
            end
            return minAvaliablePrice;
        end        
    end
    ...
    function controller:DoBreakeven()
        ...
        if core.crosses(Source.close, self._when, Source:size() - 1) then
            self._parent._trading:MoveStop(self:getTo(), trade);
            self._executed = true;
            return false;
        end
