...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=15782
...
function Init() --The strategy profile initialization
    ...
    strategy.parameters:addGroup("%B Parameters")
    strategy.parameters:addInteger("N", "Number of periods", "", 20)
    strategy.parameters:addDouble("Dev", "Number of standard deviations", "", 2)
    strategy.parameters:addDouble("OB", "Overbought Level", "", 80)
    strategy.parameters:addDouble("OS", "Oversold Level", "", 20)
    strategy.parameters:addInteger("Trigger", "Trigger periods", "", 3)

    strategy.parameters:addGroup("MA Parameters")
    strategy.parameters:addInteger("Period", "Number of periods", "Number of periods", 200)
    strategy.parameters:addString("Method", "Average Method", "", "MVA")
...
function Prepare(nameOnly)
    ...
    name =
        name ..
        ", " .. N .. ", " .. Dev .. ", " .. OB .. ", " .. OS .. ", " .. Trigger .. ", " .. Period .. ", " .. Method

    assert(core.indicators:findIndicator("BB_PER1") ~= nil, "Please, download and install BB_PER1.LUA indicator")
    ...
    Indicator[1] = core.indicators:create("BB_PER1", Source[Price], N, Dev)
    Short[1] = Indicator[1]:getStream(0)

    assert(core.indicators:findIndicator(Method) ~= nil, "Method indicator must be installed");
    Indicator[2] = core.indicators:create(Method, Source[Price], Period)
...
function Check(period)
    ...
    for i = period, period - Trigger + 1, -1 do
        if Short[1][i] > OS then
            UP = false
        end
        if Short[1][i] < OB then
            DOWN = false
        end
    end
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if period < first + Trigger then
        return
    end

    local UP, DOWN

    UP, DOWN = Check(period)

    if Source.close[period] > Short[2][period] and UP then
        if Direction then
            BUY()
        else
            SELL()
        end
    elseif Source.close[period] < Short[2][period] and DOWN then
        if Direction then
            SELL()
        else
            BUY()
