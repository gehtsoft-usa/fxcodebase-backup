...
function Init() --The strategy profile initialization
    strategy:name("Bollinger Band Squeeze Strategy");
    ...
    strategy.parameters:addInteger("Period1", "1. Period", "", 10, 1, 2000); 
    strategy.parameters:addInteger("Period2", "2. Period", "", 5, 1, 2000); 	
    strategy.parameters:addDouble("Multiplier", "Multiplier", "", 2, 0, 2000); 
...
function CreateTradingParameters()
   ...
   -- strategy.parameters:addStringAlternative("ExitExecutionType", "End of Turn", "", "EndOfTurn");
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "BBSS");
...
function Prepare( nameOnly)
	...
	assert(core.indicators:findIndicator("VOLATILITY BAND") ~= nil, "Please, download and install VOLATILITY BAND.LUA indicator");          
    ...
    VOLATILITY_BAND = core.indicators:create("VOLATILITY BAND", Source.close, instance.parameters.Period1,  instance.parameters.Multiplier );   
    RSI = core.indicators:create("RSI", VOLATILITY_BAND.DATA, instance.parameters.Period1 );   
	
	 
	first=math.max(RSI.DATA:first(),Period2); 
...
function ExtUpdate(id, source, period)  -- The method called every time when a new bid or ask price appears.
   ...
   -- get only time
    now = now - math.floor(now);
	    ...
	    -- update indicators.
   VOLATILITY_BAND:update(core.UpdateLast);
...
function EntryFunction( id, now,period)
    ...
    local min,max= mathex.minmax(Source, period-Period2, period-1);    
	  
    -- only buy if we have a fast cross over slow and the price is above the moving averages.
    if  RSI.DATA[period]< 45 
	and Source.close[period]>max
	then	
          if Direction then
                BUY();
            else
		...
		Return=true;
    elseif RSI.DATA[period]> 55 
	and Source.close[period]<min	
	then
            if Direction then
                SELL();
            else
