...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=27417
...
function Init() --The strategy profile initialization
    strategy:name("BB Aroon Strategy")
    ...
    strategy.parameters:addGroup("Bollinger Calculation")

    strategy.parameters:addString("P", "Price Source", "", "close")
    strategy.parameters:addStringAlternative("P", "OPEN", "", "open")
    strategy.parameters:addStringAlternative("P", "HIGH", "", "high")
    strategy.parameters:addStringAlternative("P", "LOW", "", "low")
    strategy.parameters:addStringAlternative("P", "CLOSE", "", "close")
    strategy.parameters:addStringAlternative("P", "MEDIAN", "", "median")
    strategy.parameters:addStringAlternative("P", "TYPICAL", "", "typical")
    strategy.parameters:addStringAlternative("P", "WEIGHTED", "", "weighted")

    strategy.parameters:addInteger("N1", "Period", "Period", 50)
    strategy.parameters:addDouble("D", "Standard deviation", "Standard deviation", 2)

    strategy.parameters:addGroup("Aroon Calculation")
    strategy.parameters:addInteger("N2", "Period", "Period", 200)
    strategy.parameters:addDouble("S", "Strong Trend Level", "Strong Trend Level", 85)
    strategy.parameters:addDouble("W", "Weak Trend Level", "Weak Trend Level", 15)

    strategy.parameters:addGroup("Exit Option")
    strategy.parameters:addBoolean("Exit", "Use Optinal Exit", "", true)

    strategy.parameters:addBoolean("O1", "Use Long Trade Top Line Exit Option", "", false)
    strategy.parameters:addBoolean("O2", "Use Long Trade Bottom Line Exit Option", "", false)
    strategy.parameters:addBoolean("O3", "Use Long Trade Central Line Exit Option", "", true)
    strategy.parameters:addBoolean("O4", "Use Short Trade Top Line Exit Option", "", false)
    strategy.parameters:addBoolean("O5", "Use Short Trade Bottom Line Exit Option", "", false)
    strategy.parameters:addBoolean("O6", "Use Short Trade Central Line Exit Option", "", true)
...
function Prepare(nameOnly)
    ...
    On[1] = instance.parameters.O1
    On[2] = instance.parameters.O2
    On[3] = instance.parameters.O3
    On[4] = instance.parameters.O4
    On[5] = instance.parameters.O5
    On[6] = instance.parameters.O6
    ...
    Indicator[1] = core.indicators:create("BB", Source[P], N1, D)
    Short["T"] = Indicator[1]:getStream(0)
    Short["B"] = Indicator[1]:getStream(1)
    Short["C"] = Indicator[1]:getStream(2)

    Indicator[2] = core.indicators:create("AROON", Source, N2)
    Short["U"] = Indicator[2]:getStream(0)
    Short["D"] = Indicator[2]:getStream(1)

    first = math.max(Short["T"]:first(), Short["C"]:first(), Short["U"]:first())
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if Short["U"][period] > S and Short["D"][period] < W and core.crossesOver(Source.close, Short["C"], period) then
        if Direction then
            BUY()
        else
            SELL()
    ...
    elseif Short["D"][period] > S and Short["U"][period] < W and core.crossesUnder(Source.close, Short["C"], period) then
        if Direction then
            SELL()
        else
            BUY()
    ...
    if Exit then
        if
            (core.crossesUnder(Source.close, Short["T"], period) and On[1]) or
                (core.crossesUnder(Source.close, Short["B"], period) and On[2]) or
                (core.crossesUnder(Source.close, Short["C"], period) and On[3])
         then
            if Direction then
        ...
        if
            (core.crossesOver(Source.close, Short["T"], period) and On[4]) or
                (core.crossesOver(Source.close, Short["B"], period) and On[5]) or
                (core.crossesOver(Source.close, Short["C"], period) and On[6])
         then
            if Direction then
                if haveTrades("S") then
                    exitSpecific("S")
