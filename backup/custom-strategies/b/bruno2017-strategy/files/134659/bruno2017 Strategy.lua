...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=69977
--+------------------------------------------------------------------+
...
local STRATEGY_NAME = "bruno2017 Strategy";
...
function IsEntryLong(source, period)
    ...
    return core.crossesOver(source.close, indi_1.DATA, period)
        and indi_1.DATA[period] > indi_1.DATA[period - 1]
        and indi_2.DATA[period] > indi_2.DATA[period - 1];
end
function IsEntryShort(source, period)
    if not indi_1.DATA:hasData(period - 1) or not indi_2.DATA:hasData(period - 1) then
        return false;
    end
    return core.crossesUnder(source.close, indi_1.DATA, period)
        and indi_1.DATA[period] < indi_1.DATA[period - 1]
        and indi_2.DATA[period] < indi_2.DATA[period - 1];
end
...
function Init()
    ...
    strategy.parameters:addInteger("stop_shift", "Stop shift", "", 1);
...
local use_stop, use_limit, limit_pips, entry_execution_type, use_trailing, trailing, use_position_limit, position_limit, stop_shift;
...
function OpenTrade(side)
    ...
    if side == "B" then
        valuemap.Rate = main_source.high[NOW];
    else
        valuemap.Rate = main_source.low[NOW];
    end
    ...
    if use_stop then
        if side == "B" then
            valuemap.RateStop = main_source.low[NOW - stop_shift];
        else
            valuemap.RateStop = main_source.high[NOW - stop_shift];
        end
