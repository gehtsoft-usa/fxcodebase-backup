...
--http://fxcodebase.com/code/viewtopic.php?f=31&t=69371
...
function Init()
    ...
    strategy.parameters:addString("breakeven_trailing", "Trailing after breakeven", "", "default");
    strategy.parameters:addStringAlternative("breakeven_trailing", "Do not change", "", "default");
    strategy.parameters:addStringAlternative("breakeven_trailing", "Set trailing", "", "set");
    strategy.parameters:addStringAlternative("breakeven_trailing", "Previous wick trailing", "", "wick");
...
local ExecutionType;
local TF, offer;
local Indicator, breakeven_when, breakeven_to, breakeven_trailing, trailing;
local commands = {};
...
function GetBreakevenTarget(trade, stop)
    if stop ~= nil then
        if trade.BS == "B" then
            if (stop.Rate - trade.Open) / offer.PointSize >= breakeven_to then
                return;
            end
        else
            if (trade.Open - stop.Rate) / offer.PointSize >= breakeven_to then
                return;
            end
        end
    end
    if trade.BS == "B" then
        return trade.Open - breakeven_to * offer.PointSize;
    end
    return trade.Open + breakeven_to * offer.PointSize;
end
...
function DoMoveStop(trade)
    ...
    local newStop, newTrailing;
    local stop = trading:FindStopOrder(trade);
    if trade.PL >= breakeven_when then
        newStop = GetBreakevenTarget(trade, stop);
        if newStop == nil then
            if breakeven_trailing == "wick" then
                if trade.BS == "B" then
                    newStop = Source.low[NOW - 2];
                    if stop ~= nil and stop.Rate > newStop then
                        return;
                    end
                else
                    newStop = Source.high[NOW - 2];
                    if stop ~= nil and stop.Rate < newStop then
                        return;
                    end
                end
    ...
    if stop == nil or stop.Rate ~= newStop and newStop ~= nil then
        commands[trade.TradeID] = trading:MoveStop(trade, newStop, newTrailing);
    end
end
