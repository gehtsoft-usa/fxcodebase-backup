...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=67041
...
function Init() --The strategy profile initialization
    strategy:name("BSTrend Strategy");
    ...
    strategy.parameters:addInteger("Period", "Period", "", 12, 1, 2000);
...
function CreateTradingParameters()
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "BSTS");
...
function Prepare( nameOnly)
	...
	assert(core.indicators:findIndicator("BSTREND") ~= nil, "Please, download and install BSTREND.LUA indicator");          
    ...
    Indicator = core.indicators:create("BSTREND", Source, Period,  core.rgb(0, 255, 0),  core.rgb(255, 0, 0), core.rgb(128, 128, 128) );
...
function ExitFunction( now,period)
	 ...
	 return;
										   ...
										   if Indicator.DATA:colorI(period) ==  core.rgb(128, 128, 128 )
										   and Indicator.DATA:colorI(period-1) ~=  core.rgb(128, 128, 128 )
										   and  haveTrades("B") then
											   exitSpecific("B");
											   Signal ("Close Long");
