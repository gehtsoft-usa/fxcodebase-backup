...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=64183
...
function Init() --The strategy profile initialization
    ...
    strategy.parameters:addInteger("RLW_Period", "Period", "", 20)
...
local BB, Period, Deviation
local RLW, RLW_Period, OB, OS
...
function Prepare(nameOnly)
    ...
    BB = core.indicators:create("BB", RLW.DATA, Period, Deviation)
    first = BB.DATA:first()
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if not BB.DATA:hasData(period) or not RLW.DATA:hasData(period) then
        return
    end
...
function EntryFunction(period, now)
    ...
    --BUY: BB TOP LINE < BUY LEVEL AND RLW CROSS OVER TOP LINE
    --SELL : BB LOW LINE > SELL LEVEL AND RLW CROSS UNDER LOW LINE

    if BB.TL[period] < OB and RLW.DATA[period] > BB.TL[period] and RLW.DATA[period - 1] <= BB.TL[period - 1] then
        if Direction then
            BUY()
        else
            SELL()
        ...
        Return = true
    elseif BB.BL[period] > OS and RLW.DATA[period] < BB.BL[period] and RLW.DATA[period - 1] >= BB.BL[period - 1] then
        if Direction then
            SELL()
        else
            BUY()
