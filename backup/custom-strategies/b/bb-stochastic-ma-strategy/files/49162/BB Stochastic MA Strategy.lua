...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=28004
...
function Init() --The strategy profile initialization
    strategy:name("BB Stochastic MA Strategy")
    ...
    strategy.parameters:addInteger("MA_Period", "Period", " Period", 200)
    ...
    strategy.parameters:addDouble("BL", "Buy Level", " ", 40)
    strategy.parameters:addDouble("SL", "Sell Level", " ", 60)
...
local MA_Price, MA_Period, MA_Method
local BL, SL
...
function Prepare(nameOnly)
    ...
    Indicator[1] = core.indicators:create(MA_Method, Source[MA_Price], MA_Period)
    Short["MA"] = Indicator[1]:getStream(0)

    Indicator[2] = core.indicators:create("STOCHASTIC", Source, K, SD, D, MVAT_K, MVAT_D)
    Short["K"] = Indicator[2]:getStream(0)
    Short["D"] = Indicator[2]:getStream(1)

    Indicator[3] = core.indicators:create("BB", Short["K"], Length, Deviation)
    Short["T"] = Indicator[3]:getStream(0)
    Short["B"] = Indicator[3]:getStream(1)

    first = math.max(Short["MA"]:first(), Short["D"]:first(), Short["T"]:first())
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if core.crossesOver(Short["T"], BL, period) and source.close[period] > Short["MA"][period] then
        if Direction then
            BUY()
        else
            SELL()
        end
    elseif core.crossesUnder(Short["B"], SL, period) and source.close[period] < Short["MA"][period] then
        if Direction then
            SELL()
        else
            BUY()
    ...
    if core.crossesUnder(source.close, Short["MA"], period) then
        if Direction then
    ...
    if core.crossesOver(source.close, Short["MA"], period) then
        if Direction then
            if haveTrades("S") then
                exit("S")
                Signal("Close Short")
