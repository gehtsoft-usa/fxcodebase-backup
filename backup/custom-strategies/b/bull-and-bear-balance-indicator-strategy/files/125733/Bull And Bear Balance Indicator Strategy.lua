...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=68334
...
function Init() --The strategy profile initialization
    strategy:name("Bull And Bear Balance Indicator Strategy");
	...
	strategy.parameters:addInteger("Period1", "Raw Data Smooth Period", "Raw Data Smooth Period", 20);
	strategy.parameters:addString("Method1", "Raw MA Method", "Method" , "EMA");
    ...
    strategy.parameters:addInteger("Period2", "Smooth Period", "Smooth Period", 30); 
...
function CreateTradingParameters()
   ...
   -- strategy.parameters:addStringAlternative("ExitExecutionType", "End of Turn", "", "EndOfTurn");
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "BBBIS");
...
function Prepare( nameOnly)
	...
	assert(core.indicators:findIndicator("BBB") ~= nil, "Please, download and install BBB.LUA indicator");          
    ...
    Indicator = core.indicators:create("BBB", Source, Period1, Method1,Period2 );	 
...
function EntryFunction( id, now,period)
    ...
    if   Indicator.DATA[period]> 0 
	and  Indicator.DATA[period-1]<= 0 
	then	
          if Direction then
                BUY();
            else
		...
		Return=true;
    elseif   Indicator.DATA[period]< 0 
	and  Indicator.DATA[period-1]>= 0 
	then
            if Direction then
                SELL();
