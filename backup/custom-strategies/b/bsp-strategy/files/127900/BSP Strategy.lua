...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=68786
...
function Init() --The strategy profile initialization
    ...
    strategy.parameters:addStringAlternative("Method", "WMA", "WMA" , "WMA");
	 ...
	 strategy.parameters:addString("Type2", "Show", "", "Smoothed"); 
    strategy.parameters:addStringAlternative("Type2", "Unsmoothed", "", "Unsmoothed");
	strategy.parameters:addStringAlternative("Type2", "Smoothed", "", "Smoothed");
...
--	strategy.parameters:addStringAlternative("ExitExecutionType", "Live", "", "Live");
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "BSPS");
...
function Prepare( nameOnly)
    ...
    Indicator = core.indicators:create("BSP", Source,  Method,Period, "Both",Type2 );
	
	first=Indicator:getStream(1):first(); 
...
function EntryFunction( id, now,period)
    ...
    if   Indicator:getStream(0)[period] > Indicator:getStream(1)[period]
	and   Indicator:getStream(0)[period-1] <= Indicator:getStream(1)[period-1]
	then	
          if Direction then
                BUY();
            else
		...
		Return=true;
    elseif   Indicator:getStream(0)[period] < Indicator:getStream(1)[period]
	and   Indicator:getStream(0)[period-1] >= Indicator:getStream(1)[period-1]
		then
            if Direction then
                SELL();
