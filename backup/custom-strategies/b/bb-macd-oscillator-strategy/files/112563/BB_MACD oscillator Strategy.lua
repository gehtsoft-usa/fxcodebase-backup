...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=64684
...
function Init() --The strategy profile initialization
    strategy:name("BB_MACD oscillator Strategy")
    ...
    strategy.parameters:addInteger("FastEMA", "Fast EMA Period", "", 12)
    strategy.parameters:addInteger("SlowEMA", "Slow EMA Period", "", 26)
    strategy.parameters:addInteger("SignalSMA", "Signal EMA Period", "", 10)
    strategy.parameters:addInteger("ADX_Period", "ADX Period", "", 16)
    strategy.parameters:addDouble("StdDev_Coeff", "StdDev Coeff", "", 1)
...
function Prepare(nameOnly)
    ...
    MACD = core.indicators:create("MACD", Source.close, FastEMA, SlowEMA)
    Signal_SMA = core.indicators:create("EMA", MACD.DATA, SignalSMA)
    StdDev = core.indicators:create("STDDEV2", MACD.DATA, SignalSMA, "EMA")
    first = Signal_SMA.DATA:first()
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    Signal_SMA:update(core.UpdateLast)
...
function EntryFunction(now, period)
    ...
    Upper_Band = Signal_SMA.DATA[period] + StdDev.DATA[period] * StdDev_Coeff
    Lower_Band = Signal_SMA.DATA[period] - StdDev.DATA[period] * StdDev_Coeff

    PreviousUpper_Band = Signal_SMA.DATA[period - 1] + StdDev.DATA[period - 1] * StdDev_Coeff
    PreviousLower_Band = Signal_SMA.DATA[period - 1] - StdDev.DATA[period - 1] * StdDev_Coeff

    -- only buy if we have a fast cross over slow and the price is above the moving averages.
    if Lower_Band > BL and PreviousLower_Band <= BL then
        if Direction then
            BUY()
        else
            SELL()
        ...
        Return = true
    elseif Upper_Band < SL and PreviousUpper_Band >= SL then
        if Direction then
            SELL()
        else
            BUY()
