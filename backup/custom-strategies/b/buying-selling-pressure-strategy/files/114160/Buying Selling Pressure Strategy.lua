...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=64989
...
function Init() --The strategy profile initialization
    strategy:name("Buying Selling Pressure Strategy")
    ...
    strategy.parameters:addGroup("Calcultion")
    ...
    strategy.parameters:addGroup("Buying Selling Pressure  Calcultion")
    ...
    strategy.parameters:addInteger("Period", "Smoothing Period", "", 4)

    strategy.parameters:addGroup("ADX Calcultion")

    strategy.parameters:addInteger("ADXPeriod", "Period", "", 14)
    strategy.parameters:addDouble("M1", "1. Multiplier ", "", 0.5)
    strategy.parameters:addDouble("M2", "2. Multiplier ", "", 0.5)
...
function Prepare(nameOnly)
    ...
    assert(core.indicators:findIndicator("BSP") ~= nil, "Please, download and install BSP.LUA indicator")
    ...
    Indicator1 = core.indicators:create("BSP", Source1, Method, Period, "Both", "Both")
    Indicator2 = core.indicators:create("BSP", Source2, Method, Period, "Both", "Both")
    ATR2 = core.indicators:create("ATR", Source2, ADXPeriod)
    Indicator3 = core.indicators:create("BSP", Source3, Method, Period, "Both", "Both")
    ATR3 = core.indicators:create("ATR", Source3, ADXPeriod)
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    ATR2:update(core.UpdateLast)
    ATR3:update(core.UpdateLast)
...
function EntryFunction(now, period1, period2, period3)
    ...
    if
        Indicator1.SBP[period1] > Indicator1.SSP[period1] and Indicator1.SBP[period1 - 1] <= Indicator1.SSP[period1 - 1] and
            Indicator2.SBP[period2] > Indicator2.SSP[period2] and
            Indicator3.SBP[period3] > Indicator3.SSP[period3] and
            math.abs(Indicator2.SBP[period2] - Indicator2.SSP[period2]) > (M1 * ATR2.DATA[period2]) and
            math.abs(Indicator3.SBP[period3] - Indicator3.SSP[period3]) > (M2 * ATR3.DATA[period3])
     then
        if Direction then
            BUY()
        else
