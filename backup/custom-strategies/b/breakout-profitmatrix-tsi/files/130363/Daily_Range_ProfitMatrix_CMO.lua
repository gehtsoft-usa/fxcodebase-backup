...
local STRATEGY_NAME = "Daily Range ProfitMatrix CMO Strategy";
...
function CreateCustomActions()
    if not DISABLE_EXIT then
        ...
        exitLongAction.IsPass = function (source, period, periodFromLast, data)
            if not cmo.DATA:hasData(period - 1) then
                return;
            end
    ...
    enterLongAction.IsPass = function (source, period, periodFromLast, data)
        if not bo.DATA:hasData(period) or not MA1.DATA:hasData(period) or not MA2.DATA:hasData(period) then
            return;
        end
