...
local STRATEGY_NAME = "Daily Range ProfitMatrix LCS Strategy";
...
function CreateParameters() 
    strategy.parameters:addGroup("Daily Range");
    strategy.parameters:addInteger("OpenHour", "Open hour", "", 4, 0, 23);
    strategy.parameters:addInteger("OpenMinute", "Open minute", "", 15, 0, 59);
    ...
    strategy.parameters:addGroup("Leavitt Conv Slope");
    strategy.parameters:addInteger("Length", "Length", "", 61);
    strategy.parameters:addDouble("UnSure", "Unsure", "", 0.0002);
end

local bo, MA1, MA2, lcs
function CreateEntryIndicators(source) 
    local profile = core.indicators:findIndicator("DAILY RANGE");
    assert(profile ~= nil, "Please, download and install " .. "DAILY RANGE" .. ".LUA indicator");
    local indicatorParams = profile:parameters();
    indicatorParams:setInteger("OpenHour", instance.parameters.OpenHour);
    indicatorParams:setInteger("OpenMinute", instance.parameters.OpenMinute);
    bo = core.indicators:create("DAILY RANGE", source, indicatorParams)
    ...
    local profile = core.indicators:findIndicator("LEAVITT CONV SLOPE");
    assert(profile ~= nil, "Please, download and install " .. "LEAVITT CONV SLOPE" .. ".LUA indicator");
    lcs = core.indicators:create("LEAVITT CONV SLOPE", source, instance.parameters.Length, instance.parameters.UnSure);
end
...
function CreateCustomActions()
    if not DISABLE_EXIT then
        ...
        exitLongAction.IsPass = function (source, period, periodFromLast, data)
            if not lcs.DATA:hasData(period - 1) then
                return;
            end
            return lcs.DATA[period] < lcs.DATA[period - 1];
        end
    ...
    enterLongAction.GetLog = function (source, period, periodFromLast, data)
        return "Price CO DR: " .. tostring(core.crossesOver(source.close, bo.DATA, period))
            .. " MA1 > MA2: " .. tostring(MA1.DATA[period] > MA2.DATA[period]);
    end
    enterLongAction.IsPass = function (source, period, periodFromLast, data)
        if not bo.DATA:hasData(period - 1) or not MA1.DATA:hasData(period) or not MA2.DATA:hasData(period) then
            return;
        end
        return core.crossesOver(source.close, bo.DATA, period)
    ...
    enterShortAction.GetLog = function (source, period, periodFromLast, data)
        return "Price CU DR: " .. tostring(core.crossesUnder(source.close, bo.DATA, period))
            .. " MA1 < MA2: " .. tostring(MA1.DATA[period] < MA2.DATA[period]);
    end
    enterShortAction.IsPass = function (source, period, periodFromLast, data)
        ...
        return core.crossesUnder(source.close, bo.DATA, period)
            and MA1.DATA[period] < MA2.DATA[period];
    end
