...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=62597
...
function Init() --The strategy profile initialization
    strategy:name("Blau Stochastic Index Strategy")
    ...
    strategy.parameters:addInteger("Period1", "First smooth period", "", 5)
    strategy.parameters:addInteger("Period2", "Second smooth period", "", 15)
    strategy.parameters:addInteger("SignalPeriod", "Signal period", "", 10)
    ...
    strategy.parameters:addDouble("BuyLevel", "BuyLevel", "", 0)
    strategy.parameters:addDouble("SellLevel", "SellLevel", "", 0)
...
local Indicator
local Short
local Period, Period1, Period2, SignalPeriod, Method
local Direction
local CustomID
...
function Prepare(nameOnly)
    ...
    assert(
        core.indicators:findIndicator("BLAU_STOCHASTIC_INDEX") ~= nil,
        "Please, download and install BLAU_STOCHASTIC_INDEX.LUA indicator"
    )
    ...
    Indicator = core.indicators:create("BLAU_STOCHASTIC_INDEX", Source, Period, Period1, Period2, SignalPeriod, Method)
...
BUY: 
 SMOOTH 1 > SMOOTH 2
 AND SMOOTH 2 > BUY LEVEL
 AND SIGNAL > BUY LEVEL 
 AND SIGNAL(PERIOD)>SIGNAL (PERIOD-1)
 AND STOCHATIC CROSSES OVER SMOOTH 1

SELL:
 SMOOTH 1 < SMOOTH 2
 AND SMOOTH 2<SELL LEVEL 
 AND  SIGNAL <SELL LEVEL 
 AND SIGNAL(PERIOD)<SIGNAL(PERIOD-1) 
 AND STOCHATIC CROSSES UNDER SMOOTH 1
    ...
    if
        Indicator.Smooth1[period] > Indicator.Smooth2[period] and Indicator.Smooth2[period] > BuyLevel and
            Indicator.Signal[period] > BuyLevel and
            Indicator.Signal[period] > Indicator.Signal[period - 1] and
            Indicator.Stochastic[period] > Indicator.Smooth1[period] and
            Indicator.Stochastic[period - 1] <= Indicator.Smooth1[period - 1]
     then
        if Direction then
            BUY()
        else
    ...
    elseif
        Indicator.Smooth1[period] < Indicator.Smooth2[period] and Indicator.Smooth2[period] < SellLevel and
            Indicator.Signal[period] < SellLevel and
            Indicator.Signal[period] < Indicator.Signal[period - 1] and
            Indicator.Stochastic[period] < Indicator.Smooth1[period] and
            Indicator.Stochastic[period - 1] >= Indicator.Smooth1[period - 1]
     then
