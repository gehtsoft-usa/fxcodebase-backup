...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=69506
--+------------------------------------------------------------------+
...
local EXIT_TIMEFRAME_IN_PARAMS = true;
local DISABLE_EXIT = false;

local STRATEGY_NAME = "Breakout 2 MA Strategy";
...
function CreateParameters() 
    strategy.parameters:addGroup("BBOnMACD");
    strategy.parameters:addInteger("rapida", "Media Rapida", "", 8);
    strategy.parameters:addInteger("lenta", "Media Lenta", "", 26);
    strategy.parameters:addDouble("stdv", "Stdv", "", 0.8);
    ...
    strategy.parameters:addString("time_period", "Time period", "", "D");
    strategy.parameters:addStringAlternative("time_period", "Daily", "", "D");
    strategy.parameters:addStringAlternative("time_period", "Weekly (Monday)", "", "W");
    strategy.parameters:addStringAlternative("time_period", "Monthly", "", "M");

    strategy.parameters:addGroup("MA1");
    strategy.parameters:addInteger("ma1_period", "MA 1 Period", "", 14);
    AddAverages("ma1_method", "MA 1 Method", "MVA");
    strategy.parameters:addInteger("ma2_period", "MA 2 Period", "", 21);
    AddAverages("ma2_method", "MA 2 Method", "EMA");
end
local bbonmacd;
function CreateEntryIndicators(source)
    ...
    breakout = core.indicators:create("BREAKOUT", source, instance.parameters.PeriodBegin, 
        instance.parameters.PeriodEnd, instance.parameters.BoxEnd, instance.parameters.Type,
        instance.parameters.time_period);

    local profile = core.indicators:findIndicator("BBONMACD");
    assert(profile ~= nil, "Please, download and install " .. "BBONMACD" .. ".LUA indicator");
    local indicatorParams = profile:parameters();
    indicatorParams:setInteger("rapida", instance.parameters.rapida);
    indicatorParams:setInteger("lenta", instance.parameters.lenta);
    indicatorParams:setDouble("stdv", instance.parameters.stdv);
    bbonmacd = core.indicators:create("BBONMACD", source, indicatorParams)
...
local exit_ma1, exit_ma2;
function CreateExitIndicators(source)
    exit_ma1 = CreateAverages(instance.parameters.ma1_period, instance.parameters.ma1_method, source);
    exit_ma2 = CreateAverages(instance.parameters.ma2_period, instance.parameters.ma2_method, source);
end

function UpdateIndicators()
    bbonmacd:update(core.UpdateLast);
    breakout:update(core.UpdateLast);
    ma1:update(core.UpdateLast);
    ma2:update(core.UpdateLast);
    exit_ma1:update(core.UpdateLast);
    exit_ma2:update(core.UpdateLast);
end
...
function CreateCustomActions()
    if not DISABLE_EXIT then
        ...
        exitLongAction.IsPass = function (source, period, periodFromLast, data)
            if not ma1.DATA:hasData(period - 1) or not ma2.DATA:hasData(period - 1) then
                return false;
            end
            return core.crosses(ma1.DATA, ma2.DATA, period);
        end
    ...
    enterLongAction.IsPass = function (source, period, periodFromLast, data)
        if not ma1.DATA:hasData(period - 1) or not ma2.DATA:hasData(period - 1) or not breakout.H:hasData(period - 1) then
            return false;
        end
        return core.crossesOver(ma1.DATA, breakout.H, period) and ma2.DATA[period] > ma2.DATA[period - 1]
            and bbonmacd.BBMACD:colorI(period) == core.colors().Green;
    end
    ...
    enterShortAction.IsPass = function (source, period, periodFromLast, data)
        if not ma1.DATA:hasData(period - 1) or not ma2.DATA:hasData(period - 1) or not breakout.L:hasData(period - 1) then
            return false;
        end
        return core.crossesUnder(ma1.DATA, breakout.L, period) and ma2.DATA[period] < ma2.DATA[period - 1]
        and bbonmacd.BBMACD:colorI(period) == core.colors().Red;
    end
