...
local STRATEGY_NAME = "BBOnMACD Breakout Strategy";
...
function CreateEntryIndicators(source)
    ...
    bbonmacd = core.indicators:create("BBONMACD", source, instance.parameters.rapida, instance.parameters.lenta, instance.parameters.stdv)
...
function IsEntryLong(source, period)
    return core.crossesOver(source.close, breakout.H, period)
        and bbonmacd.BBMACD[period] > bbonmacd.banda_supe[period];
end
function IsEntryShort(source, period)
    return core.crossesUnder(source.close, breakout.L, period)
        and bbonmacd.BBMACD[period] < bbonmacd.banda_inf[period];
end
function IsExitLong(source, period)
    return bbonmacd.BBMACD[period] < bbonmacd.banda_supe[period];
end
function IsExitShort(source, period)
    return bbonmacd.BBMACD[period] > bbonmacd.banda_inf[period];
end
