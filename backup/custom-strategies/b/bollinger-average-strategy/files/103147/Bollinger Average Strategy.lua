...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=62849
...
function Init() --The strategy profile initialization
    strategy:name("Bollinger Average Strategy")
...
function AddBollinger(id, BollingerPeriod, DeviationPeriod, TF, color)
    strategy.parameters:addGroup(id .. ". Bollinger")

    strategy.parameters:addString("TF" .. id, "Time frame", "", TF)
    local iTF = {"Chart", "m1", "m5", "m15", "m30", "H1", "H2", "H3", "H4", "H6", "H8", "D1", "W1", "M1"}
    for i = 1, 14, 1 do
        strategy.parameters:addStringAlternative("TF" .. id, iTF[i], iTF[i], iTF[i])
    end

    strategy.parameters:addInteger("BollingerPeriod" .. id, "Bollinger Bands Period", "", BollingerPeriod)
    strategy.parameters:addDouble("DeviationPeriod" .. id, "Deviation Period", "", DeviationPeriod)

    strategy.parameters:addColor("color" .. id, id .. ". Line Color", "", color)
    strategy.parameters:addInteger("width" .. id, "Line Width", "", 1, 1, 5)
    strategy.parameters:addInteger("style" .. id, "Line Style", "", core.LINE_SOLID)
    strategy.parameters:setFlag("style" .. id, core.FLAG_LINE_STYLE)
end
...
function Prepare(nameOnly)
    ...
    assert(
        core.indicators:findIndicator("BOLLINGER AVERAGE") ~= nil,
        "Please, download and install BOLLINGER AVERAGE.LUA indicator"
    )
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if Source.close[period] < Indicator.Top[period] and Source.close[period - 1] >= Indicator.Top[period - 1] then
        if Direction then
            BUY()
        else
            SELL()
    ...
    elseif Source.close[period] > Indicator.Bottom[period] and Source.close[period - 1] <= Indicator.Bottom[period - 1] then
        if Direction then
            SELL()
        else
            BUY()
