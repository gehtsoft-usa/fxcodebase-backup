...
input double Buy_Level=30.;
input double Sell_Level=70.;
...
public:
   BBLongCondition(const string symbol, ENUM_TIMEFRAMES timeframe)
      :ACondition(symbol, timeframe)
   {
      _indi = iBands(symbol, timeframe, BB_Period, 0, BB_Deviation, Price);
   }
   ~BBLongCondition()
   {
   ...
   bool IsPass(const int period, const datetime date)
   {
      ...
      double BB_Bottom1 = buffer[0];
      return iHigh(_symbol, _timeframe, period) >= BB_Bottom1 && iLow(_symbol, _timeframe, period) <= BB_Bottom1;
   }
};
...
public:
   RSILongCondition(const string symbol, ENUM_TIMEFRAMES timeframe)
      :ACondition(symbol, timeframe)
   {
      _indi = iRSI(symbol, timeframe, RSI_Period, Price);
   }
   ~RSILongCondition()
   {
      IndicatorRelease(_indi);
...
public:
   ...
   {
      double buffer[1];
      if (CopyBuffer(_indi, 2, period, 1, buffer) != 1)
      {
         return 0;
      }
      double BB_Top1 = buffer[0];
      return iHigh(_symbol, _timeframe, period) >= BB_Top1 && iLow(_symbol, _timeframe, period) <= BB_Top1;
   }
};
