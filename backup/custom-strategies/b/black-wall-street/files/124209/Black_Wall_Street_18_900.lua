...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=67411
...
local IncludeTradingTime = false;
...
local STRATEGY_NAME = "Black Wall Street";
...
function CreateStopParameters() return true; end
function SetCustomLimit() end
...
function AddSequance(id)
    strategy.parameters:addGroup("Sequance #" .. id);
    strategy.parameters:addString("StartTime" .. id, "Start Time for Trading", "", "00:00:00")
    strategy.parameters:addString("StopTime" .. id, "Stop Time for Trading", "", "24:00:00")
    strategy.parameters:addBoolean("Enable_Target_Profit" .. id, "Enable Target Profit", "", true);
    strategy.parameters:addDouble("Target_Profit" .. id, "Target Profit", "", 100);
    strategy.parameters:addBoolean("Enable_Target_Loss" .. id, "Enable Target Loss", "", true);
    strategy.parameters:addDouble("Target_Loss" .. id, "Target Loss", "", -100);

    AddTimeframe(id, "1", "m1");
    AddTimeframe(id, "2", "m30");
    AddTimeframe(id, "3", "H1");
    AddTimeframe(id, "4", "H2");
    AddTimeframe(id, "5", "H3");

    strategy.parameters:addBoolean("Enable_MM" .. id, "Enable MM", "", false);
    strategy.parameters:addInteger("Risk_in_Percent" .. id, "Risk in Percent", "", 10);
    strategy.parameters:addDouble("StopLoss" .. id, "Stop Loss", "", 30);
    strategy.parameters:addDouble("TakeProfit" .. id, "Take Profit", "", 30);
    strategy.parameters:addBoolean("UseTrailing" .. id, "Use Trailing", "", true);
    strategy.parameters:addDouble("TrailStop" .. id, "Trail Stop", "", 50);
    strategy.parameters:addDouble("TrailStep" .. id, "Trail Step", "", 40);
    strategy.parameters:addString("qtxt" .. id, "Custom ID/Magic Number", "", "Black_Wall_Street_18_900_42");
end

function AddTimeframe(id, index, tf)
    strategy.parameters:addGroup("Timeframe #" .. index .. " (" .. id .. ")");
    strategy.parameters:addString("Stoch_AlertType" .. id .. "_" .. index, "Stoch Alert Type", "", "nothing");
    strategy.parameters:addStringAlternative("Stoch_AlertType" .. id .. "_" .. index, "Nothing", "", "nothing");
    strategy.parameters:addStringAlternative("Stoch_AlertType" .. id .. "_" .. index, "Above/Below", "", "aboveBelow");
    strategy.parameters:addStringAlternative("Stoch_AlertType" .. id .. "_" .. index, "Between High/Low", "", "betweenHighLow");
    strategy.parameters:addString("MACD_AlertType" .. id .. "_" .. index, "MACD Alert Type", "", "macd_nothing");
    strategy.parameters:addStringAlternative("MACD_AlertType" .. id .. "_" .. index, "Nothing", "", "macd_nothing");
    strategy.parameters:addStringAlternative("MACD_AlertType" .. id .. "_" .. index, "Above/Below", "", "macd_aboveBelow");
    strategy.parameters:addString("Seq_TimeFrame" .. id .. "_" .. index, "Timeframe", "", tf);
    strategy.parameters:setFlag("Seq_TimeFrame" .. id .. "_" .. index, core.FLAG_BARPERIODS);
    strategy.parameters:addInteger("Seq_Shift" .. id .. "_" .. index, "Shift", "", 0);
    strategy.parameters:addBoolean("Stoch_Seq_SignalEnabled" .. id .. "_" .. index, "Stoch Seq Signal Enabled", "", true);
    strategy.parameters:addBoolean("RSI_Seq_SignalEnabled" .. id .. "_" .. index, "RSI Seq Signal Enabled", "", false);
    strategy.parameters:addBoolean("MACD_Seq_SignalEnabled" .. id .. "_" .. index, "MACD Seq Signal Enabled", "", false);
    strategy.parameters:addBoolean("MA_Seq_SignalEnabled" .. id .. "_" .. index, "MA Seq Signal Enabled", "", false);
end

function CreateParameters()
    strategy.parameters:addInteger("Delayer_in_Min", "Delayer_in_Min", "", 1);
    AddSequance(1);
    AddSequance(2);
    AddSequance(3);
    AddSequance(4);
    ...
    strategy.parameters:addGroup("Stochastics Parameters");
    strategy.parameters:addString("Stoch", "Stochastic", "", "SSD");
    strategy.parameters:setFlag("Stoch", core.FLAG_INDICATOR);
    strategy.parameters:addDouble("StochHigh", "Stoch High", "", 80);
    strategy.parameters:addDouble("StochLow", "Stoch Low", "", 20);

    strategy.parameters:addGroup("RSI Parameters");
    strategy.parameters:addInteger("RSI_Period", "Period", "", 14);
    --input ENUM_APPLIED_PRICE      RSI_AppliedPrice=PRICE_CLOSE;           //applied price 

    strategy.parameters:addGroup("MACD Parameters");
    strategy.parameters:addInteger("MACD_FastEmaPeriod", "Fast EMA period", "", 12);
    strategy.parameters:addInteger("MACD_SlowEmaPeriod", "Slow EMA period", "", 26);
    strategy.parameters:addInteger("MACD_SignalPeriod", "Signal line period", "", 9);
    ...
    strategy.parameters:addInteger("MA_Shift", "MA shift", "", 26);
    ...
    strategy.parameters:addDouble("MinDistanceBeforeLinesInPoints", "Min Distance Before Lines In Pips", "", 10);
    strategy.parameters:addDouble("MinDistanceAfterLinesInPoints", "Min Distance After Lines In Pips", "", 5);
...
local pivot;
local MinDistanceBeforeLinesInPoints;
local MinDistanceAfterLinesInPoints;
local sequenses = {};
function CreateIndicators(source) 
    MinDistanceBeforeLinesInPoints = instance.parameters.MinDistanceBeforeLinesInPoints;
    MinDistanceAfterLinesInPoints = instance.parameters.MinDistanceAfterLinesInPoints;
    ...
    for sequance = 1, 8 do
        ...
        for tf = 1, 5 do
            local controller = {};
            controller.source = trading_logic:SubscribeHistory(source:instrument(), 
                instance.parameters:getString("Seq_TimeFrame" .. sequance .. "_" .. tf), source:isBid());
            controller.source_shift = instance.parameters:getInteger("Seq_Shift" .. sequance .. "_" .. tf);
            if instance.parameters:getBoolean("Stoch_Seq_SignalEnabled" .. sequance .. "_" .. tf) then
    assert(core.indicators:findIndicator(instance.parameters.Stoch) ~= nil, instance.parameters.Stoch .. " indicator must be installed");
                controller.stoch = core.indicators:create(instance.parameters.Stoch, controller.source, instance.parameters:getCustomParameters("Stoch"))
                controller.stoch_high = instance.parameters.StochHigh;
                controller.stoch_low = instance.parameters.StochLow;
            end
            if instance.parameters:getBoolean("RSI_Seq_SignalEnabled" .. sequance .. "_" .. tf) then
                controller.rsi = core.indicators:create("RSI", controller.source, instance.parameters.RSI_Period);
            end
            if instance.parameters:getBoolean("MACD_Seq_SignalEnabled" .. sequance .. "_" .. tf) then
                controller.macd = core.indicators:create("MACD", controller.source, 
                    instance.parameters.MACD_FastEmaPeriod,
                    instance.parameters.MACD_SlowEmaPeriod,
                    instance.parameters.MACD_SignalPeriod);
            end
            if instance.parameters:getBoolean("MA_Seq_SignalEnabled" .. sequance .. "_" .. tf) then
                controller.ma = CreateAverates(instance.parameters.MA_Method, controller.source, instance.parameters.MA_Period);
                controller.ma_shift = instance.parameters.MA_Shift;
            end
            controller.Stoch_AlertType = instance.parameters:getString("Stoch_AlertType" .. sequance .. "_" .. tf);
            controller.MACD_AlertType = instance.parameters:getString("MACD_AlertType" .. sequance .. "_" .. tf);
            data.controllers[#data.controllers + 1] = controller;
        end
        data.Enable_Target_Profit = instance.parameters:getBoolean("Enable_Target_Profit" .. sequance);
        data.Target_Profit = instance.parameters:getDouble("Target_Profit" .. sequance);
        data.Enable_Target_Loss = instance.parameters:getBoolean("Enable_Target_Loss" .. sequance);
        data.Target_Loss = instance.parameters:getDouble("Target_Loss" .. sequance);
        StartTime, valid = ParseTime(instance.parameters:getString("StartTime" .. sequance));
        data.StartTime = StartTime;
        assert(valid, "Time " .. instance.parameters:getString("StartTime" .. sequance) .. " is invalid");
        StopTime, valid = ParseTime(instance.parameters:getString("StopTime" .. sequance));
        data.StopTime = StopTime;
        assert(valid, "Time " .. instance.parameters:getString("StopTime" .. sequance) .. " is invalid");
        data.MagicNumber = instance.parameters:getString("qtxt" .. sequance);
        data.Enable_MM = instance.parameters:getBoolean("Enable_MM" .. sequance);
        data.Risk_in_Percent = instance.parameters:getInteger("Risk_in_Percent" .. sequance);
        data.StopLoss = instance.parameters:getDouble("StopLoss" .. sequance);
        data.TakeProfit = instance.parameters:getDouble("TakeProfit" .. sequance);
        data.UseTrailing = instance.parameters:getBoolean("UseTrailing" .. sequance);
        data.TrailStop = instance.parameters:getDouble("TrailStop" .. sequance);
        data.TrailStep = instance.parameters:getDouble("TrailStep" .. sequance);
        sequenses[#sequenses + 1] = data;
    end
end

function UpdateIndicators()
    pivot:update(core.UpdateLast);
    for __, data in ipairs(sequenses) do
        for _, controller in ipairs(data.controllers) do
            if controller.stoch ~= nil then
                controller.stoch:update(core.UpdateLast);
            end
            if controller.rsi ~= nil then
                controller.rsi:update(core.UpdateLast);
            end
            if controller.macd ~= nil then
                controller.macd:update(core.UpdateLast);
            end
            if controller.ma ~= nil then
                controller.ma:update(core.UpdateLast);
            end
        end
    end
end
...
function CreateCustomActions()
    for __, data in ipairs(sequenses) do
        ...
        exitLongAction.IsPass = function (source, period, periodFromLast)
            return false; -- TODO: implement
        end

        local exitShortAction = {};
        exitShortAction.ActOnSwitch = false;
        exitShortAction.IsPass = function (source, period, periodFromLast)
            return false; -- TODO: implement
        end
        local enterLongAction = {};
        enterLongAction.ActOnSwitch = true;
        enterLongAction.MinTimespan = instance.parameters.Delayer_in_Min * 1440;
        enterLongAction.Data = data;
        enterLongAction.IsPass = function (source, period, periodFromLast, data)
            ...
            for _, controller in ipairs(data.controllers) do
                if InRange(now, data.StartTime, data.StopTime) then
                    if controller.stoch ~= nil then
                        if not controller.stoch.D:hasData(NOW - periodFromLast) then
                            return;
                        end
                        local k = controller.stoch.K[NOW - periodFromLast];
                        if k <= controller.stoch.D[NOW - periodFromLast] then
                            return false;
                        end
                        if controller.Stoch_AlertType == "aboveBelow" and k >= controller.stoch_low then
                            return false;
                        elseif controller.Stoch_AlertType == "betweenHighLow" and (k <= controller.stoch_low or k >= controller.stoch_high) then
                            return false;
                        end
                    end
                    if controller.rsi ~= nil then
                        if not controller.rsi.DATA:hasData(NOW - periodFromLast) then
                            return;
                        end
                        if controller.rsi.DATA[NOW - periodFromLast] <= controller.rsi.DATA[NOW - periodFromLast] then
                            return false;
                        end
                    end
                    if controller.macd ~= nil then
                        if not controller.macd.MACD:hasData(NOW - periodFromLast) or not controller.rsi.DATA:hasData(NOW - periodFromLast) then
                            return;
                        end
                        local macd = controller.macd.MACD[NOW - periodFromLast];
                        if macd <= controller.rsi.DATA[NOW - periodFromLast] then
                            return false;
                        end
                        if controller.MACD_AlertType == "macd_aboveBelow" and macd <= 0 then
                            return false;
                        end
                        return false;
                    end
                    if controller.ma ~= nil then
                        if not controller.ma.DATA:hasData(NOW - periodFromLast) then
                            return;
                        end
                        if controller.ma.DATA[NOW - periodFromLast] >= source.close[period] then
                            return false;
                        end
                    end
                end
            end
            if not pivot.P:hasData(period) then
                return;
            end
            local p = math.abs(pivot.P[period] - source.close[period]) / source:pipSize();
            local r1 = math.abs(pivot.R1[period] - source.close[period]) / source:pipSize();
            local r2 = math.abs(pivot.R2[period] - source.close[period]) / source:pipSize();
            local r3 = math.abs(pivot.R3[period] - source.close[period]) / source:pipSize();
            local s1 = math.abs(pivot.S1[period] - source.close[period]) / source:pipSize();
            local s2 = math.abs(pivot.S2[period] - source.close[period]) / source:pipSize();
            local s3 = math.abs(pivot.S3[period] - source.close[period]) / source:pipSize();
            return (p > MinDistanceBeforeLinesInPoints or p > MinDistanceAfterLinesInPoints)
                and (r1 > MinDistanceBeforeLinesInPoints or r1 > MinDistanceAfterLinesInPoints)
                and (r2 > MinDistanceBeforeLinesInPoints or r2 > MinDistanceAfterLinesInPoints)
                and (r3 > MinDistanceBeforeLinesInPoints or r3 > MinDistanceAfterLinesInPoints)
                and (s1 > MinDistanceBeforeLinesInPoints or s1 > MinDistanceAfterLinesInPoints)
                and (s2 > MinDistanceBeforeLinesInPoints or s2 > MinDistanceAfterLinesInPoints)
                and (s3 > MinDistanceBeforeLinesInPoints or s3 > MinDistanceAfterLinesInPoints);
        end

        local enterShortAction = {};
        enterShortAction.ActOnSwitch = true;
        enterShortAction.MinTimespan = instance.parameters.Delayer_in_Min * 1440;
        enterShortAction.Data = data;
        enterShortAction.IsPass = function (source, period, periodFromLast, data)
            ...
            local p = math.abs(source.close[period] - pivot.P[period]) / source:pipSize();
            local r1 = math.abs(source.close[period] - pivot.R1[period]) / source:pipSize();
            local r2 = math.abs(source.close[period] - pivot.R2[period]) / source:pipSize();
            local r3 = math.abs(source.close[period] - pivot.R3[period]) / source:pipSize();
            local s1 = math.abs(source.close[period] - pivot.S1[period]) / source:pipSize();
            local s2 = math.abs(source.close[period] - pivot.S2[period]) / source:pipSize();
            local s3 = math.abs(source.close[period] - pivot.S3[period]) / source:pipSize();
            return (p > MinDistanceBeforeLinesInPoints or p > MinDistanceAfterLinesInPoints)
                and (r1 > MinDistanceBeforeLinesInPoints or r1 > MinDistanceAfterLinesInPoints)
                and (r2 > MinDistanceBeforeLinesInPoints or r2 > MinDistanceAfterLinesInPoints)
                and (r3 > MinDistanceBeforeLinesInPoints or r3 > MinDistanceAfterLinesInPoints)
        ...
        if instance.parameters.Direction == "direct" then
            ...
            enterLongAction.ExecuteData = CreateBuyPositions(data);
            enterShortAction.ExecuteData = CreateSellPositions(data);
        else
            ...
            enterLongAction.ExecuteData = CreateSellPositions(data);
            enterShortAction.ExecuteData = CreateBuyPositions(data);
        end
...
function CreateCustomBreakeven() end
...
function CreateBuyPositions(data)
    local positions = {};
    ...
    if PositionsCount == 1 then
        positions[#positions + 1] = CreatePositionStrategyCustom(trading_logic.MainSource, "B", "", data);
        return positions;
    end
    for i = 1, PositionsCount do
        if instance.parameters:getBoolean("use_position_" .. i) then
            positions[#positions + 1] = CreatePositionStrategyCustom(trading_logic.MainSource, "B", "_" .. i, data);
        end
    end
    return positions;
end

function CreateSellPositions(data)
    local positions = {};
    if instance.parameters.allow_side == "buy" then
        return positions;
    end
    if PositionsCount == 1 then
        positions[#positions + 1] = CreatePositionStrategyCustom(trading_logic.MainSource, "S", "", data);
        return positions;
    end
    for i = 1, PositionsCount do
        if instance.parameters:getBoolean("use_position_" .. i) then
            positions[#positions + 1] = CreatePositionStrategyCustom(trading_logic.MainSource, "S", "_" .. i, data);
        end
    end
    return positions;
end
...
function CreatePositionStrategyCustom(source, side, id, data)
    local position_strategy = {};
    position_strategy.StopType = "pips";
    position_strategy.LimitType = "pips";
    position_strategy.custom_id = data.MagicNumber;
    position_strategy.Id = id;
    position_strategy.Side = side;
    position_strategy.Source = source;
    if data.Enable_MM then
        position_strategy.AmountRisk = data.Risk_in_Percent;
    else
        position_strategy.Amount = instance.parameters:getInteger("amount" .. id);
    end
    position_strategy.Stop = data.StopLoss;
    position_strategy.Limit = data.TakeProfit;
    position_strategy.UseBreakeven = UseTrailing;
    position_strategy.BreakevenWhen = data.TrailStop;
    position_strategy.BreakevenTo = data.TrailStep;
    position_strategy.BreakevenTrailing = "set";
    position_strategy.BreakevenTrailingValue = data.TrailStop;
    ...
    function position_strategy:Open(period)
        ...
        if self.Amount ~= nil then
            ...
            command:SetPercentOfEquityAmount(self.AmountRisk);
        end
        command:SetSide(self.Side)
            :SetAccountID(instance.parameters.account)
            :SetCustomID(self.custom_id);
...
function GoLong(source, period, positions)
    if instance.parameters.allow_trade then
        DoCloseOnOpposite("B");
...
function GoShort(source, period, positions)
    if instance.parameters.allow_trade then
        DoCloseOnOpposite("S");
...
function EntryFunction(source, period)
    UpdateIndicators();
    local nowDate = core.host:execute("convertTime", core.TZ_EST, ToTime, core.host:execute("getServerTime"));
    local now = nowDate - math.floor(nowDate);
    if IncludeTradingTime then
    ...
    for _, action in ipairs(EntryActions) do
        if action.IsPass(source, period, periodFromLast, action.Data) 
            ...
            and (action.MinTimespan == nil or action.LastTrigger == nil or nowDate - action.LastTrigger >= action.MinTimespan)
        then
            action.Execute(source, period, action.ExecuteData);
            action.LastTrigger = nowDate;
        end
    end
end
...
function ExitFunction(source, period)
    ...
    for _, data in ipairs(sequenses) do
        local pl = trading:FindTrade()
            :WhenCustomID(data.MagicNumber)
            :Summ(function (trade) return trade.GrossPL; end );
        if (data.Enable_Target_Profit and pl >= data.Target_Profit)
            or (data.Enable_Target_Loss and pl <= -data.Target_Loss) 
        then
            trading:FindTrade()
                :WhenCustomID(data.MagicNumber)
...
function trading:AddPositionParameters(parameters, id)
    ...
    elseif CreateLimitParameters ~= nil then
...
function breakeven:CreateIndicatorTrailingController()
    ...
    function controller:DoStopTrailing(trade, streamPeriod)
        ...
        if self._stream_in_distance then
            ...
            if self._stream_y_shift ~= nil then
                tick = tick + self._stream_y_shift * self:GetOffer().Digits;
            end
            if trade.BS == "B" then
                new_level = breakeven:round(trade.Open - tick, self:GetOffer().Digits);
            else
                new_level = breakeven:round(trade.Open + tick, self:GetOffer().Digits);
            end
        else
            ...
            new_level = breakeven:round(self._stream:tick(streamPeriod), self:GetOffer().Digits);
        end
    ...
    function controller:DoLimitTrailing(trade, streamPeriod)
        ...
        if self._stream_in_distance then
            ...
            new_level = breakeven:round(tick, self:GetOffer().Digits);
        end
        if self._min_profit ~= nil then
            if trade.BS == "B" then
                if (trade.Open - new_level) / self:GetOffer().PointSize < self._min_profit then
