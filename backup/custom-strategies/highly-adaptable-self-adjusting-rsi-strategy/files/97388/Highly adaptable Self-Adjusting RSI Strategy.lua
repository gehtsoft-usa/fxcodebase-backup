...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=61544
...
function Init() --The strategy profile initialization
    ...
    strategy.parameters:addInteger("x", "Period", "Period", 14)
    strategy.parameters:addDouble("k1", "Standard Deviation", "Standard Deviation", 1.8, 0.1, 5)
    strategy.parameters:addDouble("c1", "SMA constant", "SMA constant", 2, 0.1, 5)

    strategy.parameters:addInteger("m1", "Method", "", 1)
    strategy.parameters:addIntegerAlternative("m1", "Standard Deviation", "", 1)
    strategy.parameters:addIntegerAlternative("m1", "Simple Moving Average", "", 2)
...
function Prepare(nameOnly)
    ...
    assert(
        core.indicators:findIndicator("SELF-ADJUSTING RSI") ~= nil,
        "Please, download and install SELF-ADJUSTING RSI.LUA indicator"
    )
    ...
    Indicator = core.indicators:create("SELF-ADJUSTING RSI", Source.close, x, k1, c1, m1)
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if core.crossesOver(Indicator:getStream(0), Indicator:getStream(1), period) then
        ACTION(1, "Top Line", "Over", period)
    end
    if core.crossesUnder(Indicator:getStream(0), Indicator:getStream(1), period) then
        ACTION(2, "Top Line", "Under", period)
    end
    if core.crossesOver(Indicator:getStream(0), Indicator:getStream(2), period) then
        ACTION(3, "Bottom Line", "Over", period)
    end
    if core.crossesUnder(Indicator:getStream(0), Indicator:getStream(2), period) then
        ACTION(4, "Bottom Line", "Under", period)
    end
end
