...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=66824
...
function Init()
    strategy:name("The Dragon Hedge");
    strategy:description("v1 2018-10-14");

    strategy.parameters:addDouble("DistanceToOrders", "Distance to orders, pips", "", 10);
...
function Prepare(name_only)
    ...
    D1Source = ExtSubscribe(1, nil, "D1", false, "open");
    DistanceToOrders = instance.parameters.DistanceToOrders;
...
function DoBuy(source, shift)
    local rate = D1Source:tick(NOW) + shift * D1Source:pipSize();
    local command = trading:EntryOrder(source:instrument())
        :SetSide("B")
        :SetRate(rate)
...
function IsDayStart()
    ...
    local s, e = core.getcandle("D1", now, day_offset, week_offset)
    if _lastOpenDate == nil then
        _lastOpenDate = s;
    end
    if _lastOpenDate ~= s then
        _lastOpenDate = date;
        return true;
    end
    return false;
...
function EntryFunction(source, period)
    if IsDayStart() then
        _upOrder = DoBuy(source, DistanceToOrders);
        _downOrder = DoSell(source, DistanceToOrders);
        signaler:Signal("Creating orders");
    else
        if _upOrder ~= nil and _upOrder.Finished then
            if not _upOrder.Success then
                _upOrder = nil;
            else
                local trade = _upOrder:GetTrade();
                if trade ~= nil then
                    signaler:Signal("Opening middle order (Sell)")
                    _middleOrder = DoSell(source, 0);
                    _middleOrderSide = -1;
                    _upOrder = nil;
                end
            end
        end
        if _downOrder ~= nil and _downOrder.Finished then
            if not _downOrder.Success then
                _downOrder = nil;
            else
                local trade = _downOrder:GetTrade();
                if trade ~= nil then
                    signaler:Signal("Opening middle order (Buy)")
                    _middleOrder = DoBuy(source, 0);
                    _middleOrderSide = 1;
                    _downOrder = nil;
                end
            end
        end
        if _middleOrder ~= nil and _middleOrder.Finished then
            if not _middleOrder.Success then
                _middleOrder = nil;
            else
                local trade = _middleOrder:GetTrade();
                if trade ~= nil then
                    _middleOrder = nil;
                    if _middleOrderSide == 1 then
                        signaler:Signal("Opening up order (Buy)")
                        _upOrder = DoBuy(source, DistanceToOrders);
                    else
                        signaler:Signal("Opening down order (Sell)")
                        _downOrder = DoSell(source, DistanceToOrders);
                    end
                end
            end
    ...
    if cookie == TRADES_UPDATE then
        ...
        if close_trade then
            ...
            if closed_trade ~= nil then
                ...
                if closing_order == "L" then
                    signaler:Signal("Positions was closed by a limit. Deleting or orders");
                    trading:FindOrder()
                        :WhenCustomID(custom_id)
                        :Do(function (order) trading:DeleteOrder(order) end);
                end
            end
        end
