-- Available @ https://fxcodebase.com/code/viewtopic.php?f=28&t=75383
...
local STRATEGY_NAME = "MA Price cross with filters";
...
function CreateParameters() 
    AddAverages("ma_method", "Averages type", "MVA");
    strategy.parameters:addInteger("ma_period", "Averages Period", "", 14);
    strategy.parameters:addInteger("cci_period", "CCI Period", "", 14);
    AddAverages("confirm_method", "Confirmation Averages Type", "HMA");
    strategy.parameters:addInteger("confirm_period", "Confirmation Averages Period", "", 14);
end

local ma, cci, confirm, buy_stop_ma, sell_stop_ma;
function CreateEntryIndicators(source)
    ...
    confirm = CreateAverages(instance.parameters.confirm_period, instance.parameters.confirm_method, source.high);
    buy_stop_ma = CreateAverages(instance.parameters.ma_period, instance.parameters.ma_method, source.low);
    sell_stop_ma = CreateAverages(instance.parameters.ma_period, instance.parameters.ma_method, source.high);
...
function UpdateIndicators()
    ...
    confirm:update(core.UpdateLast);
    buy_stop_ma:update(core.UpdateLast);
    sell_stop_ma:update(core.UpdateLast);
end
...
function IsEntryLong(source, period)
    if not ma.DATA:hasData(period - 1) or not cci.DATA:hasData(period) or not confirm.DATA:hasData(period - 1) then
        return false;
    end
    return core.crossesOver(source.close, ma.DATA, period) 
        and confirm.DATA[period - 1] < confirm.DATA[period]
        and cci.DATA[period] > 50;
end
function IsEntryShort(source, period)
    if not ma.DATA:hasData(period - 1) or not cci.DATA:hasData(period) or not confirm.DATA:hasData(period - 1) then
        return false;
    end
    return core.crossesUnder(source.close, ma.DATA, period) 
        and confirm.DATA[period - 1] > confirm.DATA[period]
        and cci.DATA[period] < 50;
end
...
local use_stop, use_limit, limit_pips, entry_execution_type, use_trailing, trailing, use_position_limit, position_limit;
...
function OpenTrade(side)
    ...
    if use_stop then
        if side == "B" then
            valuemap.RateStop = buy_stop_ma.DATA[NOW];
        else
            valuemap.RateStop = sell_stop_ma.DATA[NOW];
        end
