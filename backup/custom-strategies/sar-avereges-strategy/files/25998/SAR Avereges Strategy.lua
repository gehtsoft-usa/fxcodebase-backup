...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=13376
...
function Init() --The strategy profile initialization
    strategy:name("SAR Avereges Strategy");
...
function CreateTradingParameters()
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "SAS");
...
function Prepare( nameOnly)
    ...
    Indicator[1] = core.indicators:create("AVERAGES", Source[Price], Method,Period , true , core.rgb(0, 0, 0) , core.rgb(0, 255, 0), core.rgb(255, 0, 0) );
	...
	first=math.max(Short[3]:first(),Short[1]:first(),Short[2]:first());
...
function ExtUpdate(id, source, period)  -- The method called every time when a new bid or ask price appears.
   ...
   -- get only time
    ...
    if period < first +1  then
        ...
        and not(Short[2]:hasData(period - 1)) 
		...
		and not(Short[1]:hasData(period - 1)) 
		and Short[1]:hasData(period) 	
		 then
			...
			ONE= Source:serial(period);
				     	...
				     	if	Indicator[1].DATA:colorI(period) ==  core.rgb(0, 255, 0) 						
						and not Short[2]:hasData(period) 		
						then
								if   Direction then
					   ...
					   if	Indicator[1].DATA:colorI(period) ==  core.rgb(255, 0, 0) 						
						and not Short[1]:hasData(period) 
						then
								   if   Direction then    
										if haveTrades("S") then
										   exitSpecific("S");
