...
-- https://fxcodebase.com/code/viewtopic.php?f=31&t=27809
...
function Prepare(nameOnly)
    ...
    Indicator = core.indicators:create("ST", Source, Period, Shift, FILTER, core.rgb(0, 255, 0), core.rgb(255, 0, 0))
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if  Source.close[period] > Short[period]  
	and  Short[period-1] >  Short[period] 
	then
        if Direction then
            BUY()
        else
    ...
    elseif Source.close[period] < Short[period] 
	and Short[period-1] <  Short[period] 
	then
        if Direction then
            SELL()
