...
function CreateParameters() 
    strategy.parameters:addString("instrument", "Against Symbol", "", "EUR/USD");
    strategy.parameters:setFlag("instrument", core.FLAG_INSTRUMENTS);

    strategy.parameters:addInteger("up_lag", "Up Lag", "", 65);
    strategy.parameters:addInteger("dn_lag", "Down Lag", "", 100);

    strategy.parameters:addBoolean("use_cons", "Use cons", "", false);
    strategy.parameters:addInteger("Brs", "Brs", "", 3);
    strategy.parameters:addInteger("BCons", "BCons", "", 52);
    strategy.parameters:addInteger("Srs", "Srs", "", -6);
    strategy.parameters:addInteger("SCons", "SCons", "", 52);
end

local indi, Brs, BrsOnly, BCons, SCons, Srs, use_cons;
function CreateEntryIndicators(source)
    Brs = instance.parameters.Brs;
    BCons = instance.parameters.BCons;
    SCons = instance.parameters.SCons;
    Srs = instance.parameters.Srs;
    use_cons = instance.parameters.use_cons;
    local profile = core.indicators:findIndicator("UTILITY AVERAGE");
    assert(profile ~= nil, "Please, download and install " .. "UTILITY AVERAGE" .. ".LUA indicator");
    local indicatorParams = profile:parameters();
    indi = core.indicators:create("UTILITY AVERAGE", source, instance.parameters.instrument, instance.parameters.up_lag, instance.parameters.dn_lag);
end
...
function IsEntryLong(source, period)
    ...
    if use_cons then
        for i = 0, BCons - 1 do
            if i > period or not indi.UtDrB:hasData(period - i) or indi.UtDrB[period - i] <= 0 then
                return false;
            end
        end
    end
    return indi.UtRsB[period] >= Brs;
end
function IsEntryShort(source, period)
    ...
    if use_cons then
        for i = 0, BCons - 1 do
            if i > period or not indi.UtDUtDrSrB:hasData(period - i) or indi.UtDrS[period - i] >= 0 then
                return false;
            end
        end
    end
    return indi.UtRsS[period] <= Srs;
end
