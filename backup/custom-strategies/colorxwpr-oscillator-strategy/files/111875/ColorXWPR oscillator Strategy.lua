...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=64574
...
function Init() --The strategy profile initialization
    strategy:name("ColorXWPR oscillator Strategy")
    ...
    strategy.parameters:addStringAlternative("SignalType", "XWPR Slope", "", "XWPR")
    strategy.parameters:addStringAlternative("SignalType", "WPR / XWPR Cross", "", "Cross")

    strategy.parameters:addInteger("DPeriod", "DPeriod", "", 15)
    ...
    strategy.parameters:addInteger("SPeriod", "SPeriod", "", 7)
...
local DPeriod, Method, SPeriod, Price
local SignalType
...
function Prepare(nameOnly)
    ...
    assert(core.indicators:findIndicator("COLORXWPR") ~= nil, "Please, download and install COLORXWPR.LUA indicator")
...
function EntryFunction(now, period)
    ...
    if SignalType == "XWPR" then
        if
            Indicator.XWPR:colorI(period) == core.rgb(0, 255, 0) and
                Indicator.XWPR:colorI(period - 1) ~= core.rgb(0, 255, 0)
         then
            if Direction then
                BUY()
            else
            ...
            Return = true
        elseif
            Indicator.XWPR:colorI(period) == core.rgb(255, 0, 0) and
                Indicator.XWPR:colorI(period - 1) ~= core.rgb(255, 0, 0)
         then
            if Direction then
                SELL()
            else
        ...
        if Indicator.WPR[period] > Indicator.XWPR[period] and Indicator.WPR[period - 1] <= Indicator.XWPR[period - 1] then
            if Direction then
                BUY()
            else
                SELL()
            ...
            Return = true
        elseif
            Indicator.WPR[period] < Indicator.XWPR[period] and Indicator.WPR[period - 1] >= Indicator.XWPR[period - 1]
         then
            if Direction then
                SELL()
            else
