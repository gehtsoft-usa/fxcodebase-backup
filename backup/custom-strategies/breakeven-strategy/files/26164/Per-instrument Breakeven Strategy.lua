...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=13498&hilit=Breakeven
...
function AddBE(i)
    strategy.parameters:addBoolean("Use_" .. i, "Enabled", "", false);
    strategy.parameters:addString("Instrument_" .. i, "Symbol", "", "EUR-USD");
    strategy.parameters:setFlag("Instrument_" .. i, core.FLAG_INSTRUMENTS);
	strategy.parameters:addInteger("Profit_" .. i, "Min Profit", "", 10, 0, 1000000);
end
...
function Prepare(onlyName)
    ...
    for i = 1, 20 do
        if instance.parameters:getBoolean("Use_" .. i) then
            local instrument = instance.parameters:getString("Instrument_" .. i);
            local profit = instance.parameters:getInteger("Profit_" .. i);
            breakevens[instrument] = profit;
        end
    end
...
function ExtUpdate(id, source, period)
    ...
    local row = enum:next();
    while (row ~= nil) do
        local profit = breakevens[row.Instrument];
        if profit ~= nil and row.PL >= profit and moved[row.TradeID] == nil then
            trading:MoveStop(row.Open, row);
            moved[row.TradeID] = true;
        end
        row = enum:next();
    end
end
