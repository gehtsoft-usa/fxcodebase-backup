...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=64927
...
function Init() --The strategy profile initialization
    strategy:name("MVA TDI STRATEGY")
    ...
    strategy.parameters:addInteger("Period1", "1. MA Period", "Period", 14)
    ...
    strategy.parameters:addInteger("Period2", "2. MA Period", "Period", 50)
...
function Prepare(nameOnly)
    ...
    MA1 = core.indicators:create(Method1, Source2.close, Period1)
    assert(core.indicators:findIndicator(Method2) ~= nil, Method2 .. " indicator must be installed");
    MA2 = core.indicators:create(Method2, Source2.close, Period2)
...
function EntryFunction(now, period1, period2)
    ...
    if period1 < TDI.TS:first() or period2 < (math.min(MA1.DATA:first(), MA2.DATA:first()) + 1) then
        return
    end

    -- only buy if we have a fast cross over slow and the price is above the moving averages.
    if MA1.DATA[period2] > MA2.DATA[period2] and TDI.VBU[period1] > BL and TDI.VBU[period1 - 1] <= BL then
        if Direction then
            BUY()
        else
            SELL()
        ...
        Return = true
    elseif MA1.DATA[period2] < MA2.DATA[period2] and TDI.VBD[period1] < SL and TDI.VBD[period1 - 1] >= SL then
        if Direction then
            SELL()
        else
            BUY()
