...
local PreventCalculation=false;
local AccountType;
local SUPPORTED = {"TSI", "RSI", "EMA" , "MVA", "KAMA", "PPMA", "TMA", "ADX", "DMI", "AROON",
				  ...
				  "HASM", "DSS", "SMMA", "REGRESSION", "TRENDSTOP","ST", "HMA", "QQE", "LRS","LAGUERRE_RSI","LAGUERRE_FILTER", "HALFTREND", "SUPERTREND",  "ADVANCED FRACTAL SIGNAL"
				  };
			...
			if ON[i] then
				   ...
				   assert(core.indicators:findIndicator(instance.parameters:getString("IN"..i)) ~= nil, "Please, download Indicator");		
                    ...
                    for j = 1, #SUPPORTED, 1 do
						...
						if  j== #SUPPORTED and not Flag then
...
function EVALUATION(i,p)      
       ...
       elseif instance.parameters:getString("IN"..i) == "RSI" then
		...
		elseif instance.parameters:getString("IN"..i) == "HALFTREND" then
		
		        
		         STREAMS[0]=indicator[i]:getStream(0);               
              			
						
						 if   STREAMS[0]:colorI(p)==  core.rgb(0, 255, 0)  then
						 PLUS(i);
						 elseif  STREAMS[0]:colorI(p)==core.rgb(255, 0, 0)   then
						 MINUS(i);
						 end   
		...
		elseif instance.parameters:getString("IN"..i) == "ADVANCED FRACTAL SIGNAL" then
		
		        
		         STREAMS[0]=indicator[i]:getStream(0);               
		         STREAMS[1]=indicator[i]:getStream(1);                    			
						
						 if   STREAMS[0][p]== 1  then
						 PLUS(i);
						 elseif  STREAMS[1][p]== 1   then
						 MINUS(i);
						 end   						 				 
