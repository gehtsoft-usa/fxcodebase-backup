...
-- http://fxcodebase.com/code/viewtopic.php?f=17&t=24993
...
function Add(id, TF, Flag, Instrument)
	...
	indicator.parameters:addInteger("Period" .. id, "Number of periods", "", 14, 1, 2000)
end

function Init()
	indicator:name("MTF MCP CCI HEAT MAP")
...
function Prepare(nameOnly)
	...
	for i = 1, 13, 1 do
		...
		if instance.parameters:getBoolean("On" .. i) and (e1 - s1) <= (e2 - s2) then
			...
			Temp1 = core.indicators:create("CCI", source, Period[Number])
			...
			loading[Number] = true
			Indicator[Number] = core.indicators:create("CCI", SourceData[Number], Period[Number])
		end
	end

	instance:setLabelColor(Color)
	instance:ownerDrawn(true)
	signalUp = instance:addStream("signalUp", core.Line, "Signal", "Signal", core.colors().Green, 0, 0);
	signalDn = instance:addStream("signalDn", core.Line, "Signal", "Signal", core.colors().Red, 0, 0);
...
function Update(period)
	...
	signalUp[period] = countUp;
	signalDn[period] = countDn;
end
