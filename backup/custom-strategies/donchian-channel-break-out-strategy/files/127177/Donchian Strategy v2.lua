...
function Init() --The strategy profile initialization
    ...
    strategy.parameters:addString("BREAK", "Action on Breakout", "", "side");
    strategy.parameters:addStringAlternative("BREAK", "Do nothing", "", "no");
    strategy.parameters:addStringAlternative("BREAK", "Exit by side", "", "side");
    strategy.parameters:addStringAlternative("BREAK", "Exit all", "", "all");
    strategy.parameters:addStringAlternative("BREAK", "Enter", "", "enter");
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    then
        if BREAK == "enter" then
            EnterBuy(period);
        elseif BREAK == "side" then
            exitSpecific("S")
            Signal("Close Short")
        elseif BREAK == "all" then
            if core.crosses(Source.close, mid, period) then
                exitSpecific("B")
                exitSpecific("S")
                Signal("Close All")
