...
function Init() --The strategy profile initialization
    strategy:name("GRAB Strategy with Delta Filter")
    ...
    strategy.parameters:addDouble("Delta", "Delta (in Pips)", " Delta", 0)
...
function Prepare(nameOnly)
    ...
    Indicator = core.indicators:create("GRAB", Source, Period, Method)
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if Source.close[period] > (Indicator.High[period] + Delta * Source:pipSize()) then
        if Direction then
            BUY()
        else
            SELL()
    ...
    elseif Source.close[period] < (Indicator.Low[period] - Delta * Source:pipSize()) then
        if Direction then
            SELL()
        else
            BUY()
