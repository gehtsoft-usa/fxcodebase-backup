...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=64769
...
function Add(id, TF)
    ...
    strategy.parameters:addInteger("Period" .. id, "Period", "", 14)
    strategy.parameters:addString("SMethod" .. id, "Smooth method", "", "EMA")
    strategy.parameters:addStringAlternative("SMethod" .. id, "MVA", "", "MVA")
    strategy.parameters:addStringAlternative("SMethod" .. id, "EMA", "", "EMA")
    strategy.parameters:addStringAlternative("SMethod" .. id, "KAMA", "", "KAMA")
    strategy.parameters:addStringAlternative("SMethod" .. id, "Wilder", "", "Wilder")
    strategy.parameters:addStringAlternative("SMethod" .. id, "LWMA", "", "LWMA")
    strategy.parameters:addStringAlternative("SMethod" .. id, "SineWMA", "", "SineWMA")
    strategy.parameters:addStringAlternative("SMethod" .. id, "TriMA", "", "TriMA")
    strategy.parameters:addStringAlternative("SMethod" .. id, "LSMA", "", "LSMA")
    strategy.parameters:addStringAlternative("SMethod" .. id, "SMMA", "", "SMMA")
    strategy.parameters:addStringAlternative("SMethod" .. id, "HMA", "", "HMA")
    strategy.parameters:addStringAlternative("SMethod" .. id, "ZeroLagEMA", "", "ZeroLagEMA")
    strategy.parameters:addStringAlternative("SMethod" .. id, "DEMA", "", "DEMA")
    strategy.parameters:addStringAlternative("SMethod" .. id, "T3", "", "T3")
    strategy.parameters:addStringAlternative("SMethod" .. id, "ITrend", "", "ITrend")
    strategy.parameters:addStringAlternative("SMethod" .. id, "Median", "", "Median")
    strategy.parameters:addStringAlternative("SMethod" .. id, "GeoMean", "", "GeoMean")
    strategy.parameters:addStringAlternative("SMethod" .. id, "REMA", "", "REMA")
    strategy.parameters:addStringAlternative("SMethod" .. id, "ILRS", "", "ILRS")
    strategy.parameters:addStringAlternative("SMethod" .. id, "IE/2", "", "IE/2")
    strategy.parameters:addStringAlternative("SMethod" .. id, "TriMAgen", "", "TriMAgen")
    strategy.parameters:addStringAlternative("SMethod" .. id, "JSmooth", "", "JSmooth")
    strategy.parameters:addInteger("SPeriod" .. id, "Smooth period", "", 21)
    strategy.parameters:addInteger("MomPeriod" .. id, "Momentum period", "", 1)
    strategy.parameters:addInteger("HLevel" .. id, "Sell level", "", 20)
    strategy.parameters:addInteger("LLevel" .. id, "Buy level", "", -20)
end
...
function Prepare(nameOnly)
    ...
    for i = 1, 3, 1 do
        ...
        SMethod[i] = instance.parameters:getString("SMethod" .. i)
        SPeriod[i] = instance.parameters:getInteger("SPeriod" .. i)
        MomPeriod[i] = instance.parameters:getInteger("MomPeriod" .. i)
        HLevel[i] = instance.parameters:getInteger("HLevel" .. i)
        LLevel[i] = instance.parameters:getInteger("LLevel" .. i)
    end
    ...
    assert(core.indicators:findIndicator("RSIOMA") ~= nil, "Please, download and install RSIOMA.LUA indicator")
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if
        period1 < Indicator[1].Signal:size() - 1 or period2 < Indicator[2].Signal:size() - 1 or
            period3 < Indicator[3].Signal:size() - 1
     then
        return
    end
...
function EntryFunction(now, period1, period2, period3)
    ...
    if
        Indicator[2].RSIOMA[period2] > Indicator[2].Signal[period2] and
            Indicator[3].RSIOMA[period3] > Indicator[3].Signal[period3] and
            Indicator[2].Signal[period2] > HLevel[2] and
            Indicator[3].Signal[period3] > HLevel[3] and
            Indicator[1].RSIOMA[period1] > Indicator[1].Signal[period1] and
            Indicator[1].RSIOMA[period1 - 1] <= Indicator[1].Signal[period1 - 1] and
            Indicator[1].Signal[period1] < LLevel[1]
     then
        if Direction then
            BUY()
        else
    ...
    elseif
        Indicator[2].RSIOMA[period2] < Indicator[2].Signal[period2] and
            Indicator[3].RSIOMA[period3] < Indicator[3].Signal[period3] and
            Indicator[2].Signal[period2] < LLevel[2] and
            Indicator[3].Signal[period3] < LLevel[3] and
            Indicator[1].RSIOMA[period1] < Indicator[1].Signal[period1] and
            Indicator[1].RSIOMA[period1 - 1] >= Indicator[1].Signal[period1 - 1] and
            Indicator[1].Signal[period1] > HLevel[1]
     then
        if Direction then
            SELL()
        else
