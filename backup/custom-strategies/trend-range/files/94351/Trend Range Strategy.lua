...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=60787
...
function Init() --The strategy profile initialization
    strategy:name("Trend Range Strategye")
    ...
    strategy.parameters:addBoolean("Show_MA", "Show MA", "", true)
    ...
    strategy.parameters:addInteger("Period1", "MA Period", "", 50, 2, 2000)
    ...
    strategy.parameters:addGroup("Range Calculation")
    strategy.parameters:addDouble("Range", "Range", "", 25, 0, 100)
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if Source.close[period] > ma.DATA[period] and range >= (100 - Range) then
        BUY()
        ONE = Source:serial(period)
    elseif Source.close[period] < ma.DATA[period] and range <= Range then
        SELL()
        ONE = Source:serial(period)
    end
end
