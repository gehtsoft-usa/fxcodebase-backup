...
function Init() --The strategy profile initialization
    strategy:name("Bollinger in Trend Strategy");
	...
	strategy.parameters:addInteger("Period1", "1. Period", "", 20);
	strategy.parameters:addInteger("Period2", "2. Period", "", 100);
...
function Prepare( nameOnly)
    ...
    Indicator = core.indicators:create("BB", Source.close, instance.parameters.Period, instance.parameters.Deviation  );
 
	first=math.max(Indicator.DATA:first(),Period1,Period2); 
...
function ExitFunction( now,period)
     ...
     local pB = (Source.close[period] - Indicator.BL[period]) / (Indicator.TL[period] - Indicator.BL[period])
...
function EntryFunction( now,period)
    ...
    local pB = (Source.close[period] - Indicator.BL[period]) / (Indicator.TL[period] - Indicator.BL[period])
 	local Average1=mathex.avg(Source.close, period-Period1+1, period);
    local Average2=mathex.avg(Source.close, period-Period2+1, period);	

	
	
    if  Source.close[period] > Average2
	and Average1 > Average2
	and  pB < 0.2
	then	
          if Direction then
                BUY();
		...
		Return=true;
    elseif Source.close[period] < Average2
	and Average1 < Average2
	and  pB > 0.8
	then
            if Direction then
