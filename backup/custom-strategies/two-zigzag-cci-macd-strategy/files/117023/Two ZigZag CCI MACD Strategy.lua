...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=65628
...
function Init() --The strategy profile initialization
    strategy:name("Two ZigZag CCI MACD Strategy");
    ...
    strategy.parameters:addGroup("1. ZigZag Calculation");
    strategy.parameters:addInteger("Depth1", "Depth", "The minimum number of periods used to draw one ZigZag line.", 13, 1, 1000);
    strategy.parameters:addInteger("Deviation1", "Deviation", "The maximum distance in pips by which the current high/low must be lower/higher than the previous one to return Backstep periods back to check if the current high/low is a new max/min.", 8, 1, 1000);
    ...
    strategy.parameters:addGroup("2. ZigZag Calculation");
    strategy.parameters:addInteger("Depth2", "Depth", "The minimum number of periods used to draw one ZigZag line.", 34, 1, 1000);
    strategy.parameters:addInteger("Deviation2", "Deviation", "The maximum distance in pips by which the current high/low must be lower/higher than the previous one to return Backstep periods back to check if the current high/low is a new max/min.", 21, 1, 1000);
    strategy.parameters:addInteger("Backstep2", "Backstep", "The number of periods used to define a new min/max if the current high/low is lower/higher than the previous one by Deviation or less.", 12, 1, 1000);
	...
	strategy.parameters:addInteger("P1", "Fast Period", "", 12, 1, 1000);
	strategy.parameters:addInteger("P2", "Slow Period", "", 26, 1, 1000);
	strategy.parameters:addInteger("P3", "Signal Period", "", 9, 1, 1000);
...
function CreateTradingParameters()
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "TZZCCIMACDS");
...
local ZigZag1,ZigZag2, Depth1,Deviation1, Backstep1 , Depth2,Deviation2, Backstep2;
local CCI, Period, BL1, SL1;
local MACD, P1, P2, P3, BL2, SL2;
...
function Prepare( nameOnly)
    ...
    ZigZag1 = core.indicators:create("ZIGZAG", Source  , Depth1, Deviation1, Backstep1  );
	ZigZag2 = core.indicators:create("ZIGZAG", Source  , Depth2, Deviation2, Backstep2  );
	
	CCI=core.indicators:create("CCI", Source  , Period  );
	MACD=core.indicators:create("MACD", CCI.DATA  , P1, P2, P3  );
...
local Return=0;
       ...
       for i= period, Source:first(),-1 do
		 ...
		 Return=i;
		      ...
		      if Data.DATA[i]== Source.high[i]  
			 then
			 Signal=1;
			 elseif Data.DATA[i]== Source.low[i]  
			 then
			 Signal=-1;
			 end
...
function EntryFunction( now,period)
    ...
    local Signal1, p1=FindLast( ZigZag1, period); 
	local Signal2, p2=FindLast( ZigZag2, period); 
    ...
    if   Signal1==-1
	and   Signal2==-1
	and (ZigZag1.DATA[p1] == ZigZag2.DATA[p2] or ZigZag1.DATA[p1] > ZigZag2.DATA[p2] )
	and MACD.MACD[period]> MACD.SIGNAL[period]
	and MACD.SIGNAL[period]> BL2
	and  CCI.DATA[period]> BL1
	and  CCI.DATA[period-1]<= BL1
	then	
          if Direction then
                BUY();
            else
	...
	and (ZigZag1.DATA[p1] == ZigZag2.DATA[p2] or ZigZag1.DATA[p1] < ZigZag2.DATA[p2] )
	and MACD.MACD[period]< MACD.SIGNAL[period]
	and MACD.SIGNAL[period]< SL2
	and  CCI.DATA[period]< SL1
	and  CCI.DATA[period-1]>= SL1
	then
            if Direction then
                SELL();
            else
