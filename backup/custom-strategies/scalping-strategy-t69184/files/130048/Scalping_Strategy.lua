...
--http://fxcodebase.com/code/viewtopic.php?f=31&t=69184
...
local STRATEGY_NAME = "Scalping Strategy";
...
function CreateParameters() 
    strategy.parameters:addString("tf1", "Long Timeframe", "", "H1");
    strategy.parameters:setFlag("tf1", core.FLAG_BARPERIODS);
    strategy.parameters:addInteger("tf1_ema1", "Long timeframe EMA #1 period", "", 8)
    strategy.parameters:addInteger("tf1_ema2", "Long timeframe EMA #2 period", "", 21)
    strategy.parameters:addInteger("tf2_ema1", "Entry timeframe EMA #1 period", "", 8)
    strategy.parameters:addInteger("tf2_ema2", "Entry timeframe EMA #2 period", "", 13)
    strategy.parameters:addInteger("tf2_ema3", "Entry timeframe EMA #3 period", "", 21)
    strategy.parameters:addDouble("trigger_shift", "Trigger shift, pips", "", 3);
    strategy.parameters:addInteger("bars_lookback", "Lookback, bars", "", 5)
end
function CreateStopParameters(params, id) 
    strategy.parameters:addDouble("stop_shift" .. id, "Stop shift, pips", "", 3);
    return true; 
end
function SaveCustomStopParameters(position_strategy, id) 
    position_strategy.stop_shift = instance.parameters:getDouble("stop_shift" .. id)
end
local tf1;
local ema1_tf1;
local ema2_tf1;
...
function CreateEntryIndicators(source) 
    tf1 = trading_logic:SubscribeHistory(source.close:instrument(), instance.parameters.tf1, source:isBid());
    ema1_tf1 = core.indicators:create("EMA", tf1.close, instance.parameters.tf1_ema1);
    ema2_tf1 = core.indicators:create("EMA", tf1.close, instance.parameters.tf1_ema2)
    ema1_tf2 = core.indicators:create("EMA", source.close, instance.parameters.tf2_ema1);
    ema2_tf2 = core.indicators:create("EMA", source.close, instance.parameters.tf2_ema2);
    ema3_tf2 = core.indicators:create("EMA", source.close, instance.parameters.tf2_ema3);
end
function CreateExitIndicators(source) end

function UpdateIndicators()
    ema1_tf1:update(core.UpdateLast);
    ema2_tf1:update(core.UpdateLast);
    ema1_tf2:update(core.UpdateLast);
    ema2_tf2:update(core.UpdateLast);
    ema3_tf2:update(core.UpdateLast);
end

function FindLongTriggerBarIndex(source, period)
    for i = 0, 1000 do
        local long_tf_index = core.findDate(tf1, source:date(period - i), false);
        if long_tf_index < 0 then
            return nil;
        end
        if tf1.close[long_tf_index] <= ema1_tf1.DATA[long_tf_index]
            or ema1_tf1.DATA[long_tf_index] <= ema2_tf1.DATA[long_tf_index]
        then
            return nil;
        end
        if source.close[period - i] <= ema1_tf2.DATA[period - i]
            and ema1_tf2.DATA[period - i] <= ema2_tf2.DATA[period - i]
            and ema2_tf2.DATA[period - i] <= ema3_tf2.DATA[period - i]
        then
            return nil;
        end
        if source.low[period - i] < ema1_tf2.DATA[period - i] then
            return period - i;
        end
    end
    return nil;
...
function FindShortTriggerBarIndex(source, period)
    for i = 0, 1000 do
        local long_tf_index = core.findDate(tf1, source:date(period - i), false);
        if long_tf_index < 0 then
            return nil;
        ...
        if source.high[period - i] > ema1_tf2.DATA[period - i] then
            return period - i;
        end
    end
    return nil;
end
function SetCustomStop(position_desc, command, period, periods_from_last) 
    if position_desc.Side == "B" then
        local index = FindLongTriggerBarIndex(trading_logic.MainSource, period);
        command:SetStop(trading_logic.MainSource.low[index] - position_desc.stop_shift * trading_logic.MainSource:pipSize());
    else
        local index = FindShortTriggerBarIndex(trading_logic.MainSource, period);
        command:SetStop(trading_logic.MainSource.high[index] + position_desc.stop_shift * trading_logic.MainSource:pipSize());
    end
    return true; 
end
...
function CreateCustomActions()
    ...
    enterLongAction.IsPass = function (source, period, periodFromLast, data)
        local trigger_bar_index = FindLongTriggerBarIndex(source, period);
        if trigger_bar_index == nil then
            return false;
        end
        local entry_value = mathex.max(source.low, trigger_bar_index - instance.parameters.bars_lookback, trigger_bar_index) + instance.parameters.trigger_shift * source:pipSize();
        return core.crossesOver(source.close, entry_value, period);
    end
    ...
    enterShortAction.IsPass = function (source, period, periodFromLast, data)
        local trigger_bar_index = FindShortTriggerBarIndex(source, period);
        if trigger_bar_index == nil then
            return false;
        end
        local entry_value = mathex.min(source.high, trigger_bar_index - instance.parameters.bars_lookback, trigger_bar_index) - instance.parameters.trigger_shift * source:pipSize();
        return core.crossesUnder(source.close, entry_value, period);
    end
