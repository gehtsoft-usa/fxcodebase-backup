...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=67126
...
local STRATEGY_NAME = "MACD Dual Histogram";
...
local CustomTimeframeDefined = true;

function CreateParameters()
    strategy.parameters:addBoolean("is_bid", "Price Type", "", true);
    strategy.parameters:setFlag("is_bid", core.FLAG_BIDASK);
    strategy.parameters:addString("timeframe_1", "MACD #1 Time frame", "", "H1");
    strategy.parameters:setFlag("timeframe_1", core.FLAG_PERIODS);
    strategy.parameters:addInteger("macd_sn_1", "MACD #1 Short EMA", "", 12, 2, 1000);
    strategy.parameters:addInteger("macd_ln_1", "MACD #1 Long EMA", "", 26, 2, 1000);
    strategy.parameters:addInteger("macd_in_1", "MACD #1 Signal Line", "", 9, 2, 1000);

    strategy.parameters:addString("timeframe_2", "MACD #2 Time frame", "", "m15");
    strategy.parameters:setFlag("timeframe_2", core.FLAG_PERIODS);
    strategy.parameters:addInteger("macd_sn_2", "MACD #2 Short EMA", "", 12, 2, 1000);
    strategy.parameters:addInteger("macd_ln_2", "MACD #2 Long EMA", "", 26, 2, 1000);
    strategy.parameters:addInteger("macd_in_2", "MACD #2 Signal Line", "", 9, 2, 1000);

    strategy.parameters:addDouble("exit_diff", "MACD change threshold, %", "", 0.1);
end

local MACD_1_SOURCE_ID = 3;
local MACD_2_SOURCE_ID = 4;
local macd_1;
local macd_2;
function CreateIndicators(source)
    local macd_1_source = ExtSubscribe(MACD_1_SOURCE_ID, source:instrument(), instance.parameters.timeframe_1, instance.parameters.is_bid, "bar");
    local macd_2_source = ExtSubscribe(MACD_2_SOURCE_ID, source:instrument(), instance.parameters.timeframe_2, instance.parameters.is_bid, "bar");
    macd_1 = core.indicators:create("MACD", macd_1_source, instance.parameters.macd_sn_1, instance.parameters.macd_ln_1, instance.parameters.macd_in_1);
    macd_2 = core.indicators:create("MACD", macd_2_source, instance.parameters.macd_sn_2, instance.parameters.macd_ln_2, instance.parameters.macd_in_2);
end

function UpdateIndicators()
    macd_1:update(core.UpdateLast);
    macd_2:update(core.UpdateLast);
end
...
function CreateCustomActions()
    ...
    exitLongAction.IsPass = function (source, period, periodFromLast)
        if macd_2.HISTOGRAM:size() < math.abs(periodFromLast) + 1 or not macd_2.HISTOGRAM:hasData(periodFromLast - 1) then
            return false;
        end
        local nom = macd_2.HISTOGRAM[periodFromLast];
        if nom == 0.0 then
            nom = math.pow(10, -macd_2.HISTOGRAM:getDisplayPrecision());
        end
        local diff = 100 * (macd_2.HISTOGRAM[periodFromLast - 1] - macd_2.HISTOGRAM[periodFromLast]) / nom;
        return diff >= instance.parameters.exit_diff;
    end
    ...
    exitShortAction.IsPass = function (source, period, periodFromLast)
        ...
        local diff = 100 * (macd_2.HISTOGRAM[periodFromLast] - macd_2.HISTOGRAM[periodFromLast - 1]) / nom;
        return diff >= instance.parameters.exit_diff;
    end
    ...
    enterLongAction.IsPass = function (source, period, periodFromLast)
        if macd_2.HISTOGRAM:size() < math.abs(periodFromLast) + 1 or not macd_2.HISTOGRAM:hasData(periodFromLast - 1)
            or macd_1.HISTOGRAM:size() < math.abs(periodFromLast) + 1 or not macd_1.HISTOGRAM:hasData(periodFromLast - 1) 
        then
            return false;
        end
        local nom = macd_2.HISTOGRAM[periodFromLast];
        ...
        local diff = 100 * (macd_2.HISTOGRAM[periodFromLast] - macd_2.HISTOGRAM[periodFromLast - 1]) / nom;
        return macd_1.HISTOGRAM[periodFromLast] > macd_1.HISTOGRAM[periodFromLast - 1]
            and diff >= instance.parameters.exit_diff;
    end
