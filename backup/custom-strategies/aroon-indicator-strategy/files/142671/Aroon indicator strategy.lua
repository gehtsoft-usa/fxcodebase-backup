...
-- https://fxcodebase.com/code/viewtopic.php?f=31&t=71317
...
function Init() --The strategy profile initialization
    strategy:name("Aroon indicator strategy");
    ...
    strategy.parameters:addInteger("Period", "Hedge your Short exposure", "",25, 3, 1000);  
...
function CreateTradingParameters()
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "AIS");
...
function Prepare( nameOnly)
    ...
    Indicator = core.indicators:create("AROON", Source,Period  );
...
function EntryFunction( id, now,period)
    ...
    if   Indicator.UP[period]> Indicator.DOWN[period]
	and Indicator.UP[period-1]<= Indicator.DOWN[period-1]
	then	
          if Direction then
                BUY();
            else
		...
		Return=true;
    elseif   Indicator.UP[period]< Indicator.DOWN[period]
	and Indicator.UP[period-1]>= Indicator.DOWN[period-1]
	then
            if Direction then
                SELL();
