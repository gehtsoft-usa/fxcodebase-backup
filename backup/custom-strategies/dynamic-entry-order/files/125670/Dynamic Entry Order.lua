...
function Init()
    strategy:name("Dynamic Entry Order");
    strategy:description("");
    strategy:type(core.Both);

    strategy.parameters:addInteger("atr_period", "ATR Period", "", 10);
    strategy.parameters:addString("timeframe", "Timeframe", "", "m5");
...
function OnTrade(trade)
    if trade.QTXT == custom_id or not instance.parameters.allow_trade or source:instrument() ~= trade.Instrument then
        return;
    end
    local rate;
    atr:update(core.UpdateLast);
    if trade.BS == "B" then
        rate = source.low[NOW] - atr.DATA[NOW];
    else
        rate = source.high[NOW] + atr.DATA[NOW];
    end
    local result = trading:EntryOrder(trade.Instrument)
        :SetAccountID(trade.AccountID)
        :SetAmount(trade.Lot)
        :SetRate(rate)
        :SetSide(trading:getOppositeSide(trade.BS))
        :SetCustomID(custom_id)
        :Execute();
    breakeven:CreateOrderTrailingController()
        :SetRequestID(result.RequestID)
        :MoveUpOnly()
        :SetIndicatorStream(source.close, 1, false)
        :SetIndicatorStreamShift(0, (rate - source.close[NOW]) / source:pipSize());
end
...
function Prepare(name_only)
    ...
    source = ExtSubscribe(1, instance.bid:instrument(), instance.parameters.timeframe, instance.parameters.is_bid, "bar");
    t1_source = ExtSubscribe(2, instance.bid:instrument(), "t1", instance.parameters.is_bid);
    atr = core.indicators:create("ATR", source, instance.parameters.atr_period);
    tables_monitor:ListenTrade(OnTrade);
