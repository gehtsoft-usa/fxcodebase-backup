...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=7810
...
function Init() --The strategy profile initialization
    strategy:name("10 Day EMA System")
    ...
    strategy.parameters:addInteger("1", "Period", "", 10, 2, 1000)
    strategy.parameters:addInteger("2", "Period", "", 20, 2, 1000)

    strategy.parameters:addGroup("2. Filter")
    strategy.parameters:addInteger("3", "Period", "", 20, 2, 1000)
    strategy.parameters:addInteger("4", "Period", "", 50, 2, 1000)

    strategy.parameters:addGroup("3. Filter")
    strategy.parameters:addInteger("5", "Period", "", 50, 2, 1000)
    strategy.parameters:addInteger("6", "Period", "", 200, 2, 1000)

    strategy.parameters:addGroup("Second Criteria")
    strategy.parameters:addInteger("7", "Period", "", 10, 2, 1000)

    strategy.parameters:addGroup("ATR")
    strategy.parameters:addInteger("8", "Period", "", 14, 2, 1000)
    strategy.parameters:addDouble("Factor", "ATR Multiplier", "", 0.5, 0, 100)
    strategy.parameters:addGroup("Confirmation")
    strategy.parameters:addInteger("9", "Period", "", 10, 0, 1000)
...
function Prepare(nameOnly)
    ...
    for i = 1, 9, 1 do
        Parameters[i] = instance.parameters:getInteger(tostring(i))
    ...
    first = math.max(Source:first(), Parameters[9])

    for i = 1, 7, 1 do
        Indicator[i] = core.indicators:create("EMA", Source[Price], Parameters[i])
        Short[i] = Indicator[i]:getStream(0)

        first = math.max(first, Short[i]:first(), Parameters[i])
    end

    Indicator[8] = core.indicators:create("ATR", Source, Parameters[8])
    Short[8] = Indicator[8]:getStream(0)

    first = math.max(first, Short[8]:first()) + 1
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if
        Short[1][period] > Short[2][period] and Short[3][period] > Short[4][period] and
            Short[5][period] > Short[6][period] and
            TESTLONG(Parameters[9], period - 1) and
            Source.low[period] < Short[7][period]
     then
        if Direction then
            BUY()
        else
    ...
    elseif
        Short[1][period] < Short[2][period] and Short[3][period] < Short[4][period] and
            Short[5][period] < Short[6][period] and
            TESTSHORT(Parameters[9], period - 1) and
            Source.high[period] > Short[7][period]
     then
        if Direction then
            SELL()
        else
    ...
    -- trailing stop to exit trade =value of 10 day ema minus 50% ATR-

    if Source.low[period] < Short[7][period] - Short[8][period] * Factor then
        if Direction then
    ...
    elseif Source.high[period] > Short[7][period] + Short[8][period] * Factor then
        if Direction then
            if haveTrades("S") then
                exit("S")
                Signal("Close Short")
...
function TESTSHORT(PERIOD, period)
    RETURN = true

    local i

    for i = period - PERIOD, period, 1 do
        if Source.high[i] > Short[7][i] then
            RETURN = false
        end
    end
...
function TESTLONG(PERIOD, period)
    RETURN = true

    local i

    for i = period - PERIOD, period, 1 do
        if Source.low[i] < Short[7][i] then
            RETURN = false
        end
    end
