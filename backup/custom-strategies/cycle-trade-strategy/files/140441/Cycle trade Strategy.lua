...
--http://fxcodebase.com/code/viewtopic.php?f=31&t=70868
...
local AllowTrade, trade, stop, limit
function OnTradeClosed(closed_trade)
    baseUnitSize = core.host:execute("getTradingProperty", "baseUnitSize", closed_trade.Instrument, closed_trade.AccountID);
    if tables_monitor.closing_order_types[closed_trade.CloseOrderID] == "S" then
        local result = trading:MarketOrder(closed_trade.Instrument)
            :SetAccountID(closed_trade.AccountID)
            :SetAmount(closed_trade.Lot / baseUnitSize)
        ...
        local result = trading:MarketOrder(closed_trade.Instrument)
            :SetAccountID(closed_trade.AccountID)
            :SetAmount(closed_trade.Lot / baseUnitSize)
            :SetSide(trading:getOppositeSide(closed_trade.BS))
            :SetPipLimit(nil, limit)
            :SetPipStop(nil, stop)
            :Execute();
    end
...
function Prepare(nameOnly)
    ...
    local tradeRow = core.host:findTable("trades"):find("TradeID", trade);
    assert(tradeRow ~= nil, "Trade not found");
    local offer = core.host:findTable("offers"):find("Instrument", tradeRow.Instrument);
    if tradeRow.Stop > 0 then
        stop = math.abs(tradeRow.Stop - tradeRow.Open) / offer.PointSize;
    end
    if tradeRow.Limit > 0 then
        limit = math.abs(tradeRow.Limit - tradeRow.Open) / offer.PointSize;
    end
...
function Update(source)
    for _, module in pairs(Modules) do if module.ExtUpdate ~= nil then module:ExtUpdate(0, source, NOW); end end 
end

function AsyncOperationFinished(cookie, success, message, message1, message2)
    core.host:trace(tostring(message));
