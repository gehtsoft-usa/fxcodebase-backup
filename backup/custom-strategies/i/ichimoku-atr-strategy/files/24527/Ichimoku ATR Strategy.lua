...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=12424
...
function Init() --The strategy profile initialization
    strategy:name("Ichimoku ATR Strategy")
    ...
    strategy.parameters:addDouble("ENTRY", "Entry Multiplier", "", 2)
    strategy.parameters:addDouble("STOP", "Stop Multiplier", "", 1)
    strategy.parameters:addDouble("LIMIT", "Limit", "", 1.5)
...
function Prepare(nameOnly)
    ...
    Parameters["STOP"] = instance.parameters.STOP
    Parameters["LIMIT"] = instance.parameters.LIMIT
    Parameters["ENTRY"] = instance.parameters.ENTRY
    ...
    name =
        name ..
        ", " .. Parameters["X"] .. ", " .. Parameters["Y"] .. ", " .. Parameters["Z"] .. ", " .. Parameters["ATR"]
    ...
    Short["A"] = Indicator[1]:getStream(3)
    Short["B"] = Indicator[1]:getStream(4)

    Indicator[2] = core.indicators:create("ATR", Source, Parameters["ATR"])
    Short["ATR"] = Indicator[2]:getStream(0)

    first = math.max(Short["ATR"]:first(), Short["A"]:first())
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if
        core.crossesOver(Source.close, Short["A"], period) and Short["A"][period] > Short["B"][period] and
            (Short["A"][period] - Short["B"][period]) > Short["ATR"][period] * Parameters["ENTRY"]
     then
        if Direction then
            STOP = Source.close[period] - Short["ATR"][period] * Parameters["STOP"]
            LIMIT = Source.close[period] + Short["ATR"][period] * Parameters["LIMIT"]
    ...
    elseif
        core.crossesUnder(Source.close, Short["A"], period) and Short["A"][period] < Short["B"][period] and
            (Short["B"][period] - Short["A"][period]) > Short["ATR"][period] * Parameters["ENTRY"]
     then
        if Direction then
            SELL()
            STOP = Source.close[period] + Short["ATR"][period] * Parameters["STOP"]
    ...
    if STOP == nil or LIMIT == nil then
        return
    end

    if core.crossesOver(Source.close, LIMIT, period) or core.crossesUnder(Source.close, STOP, period) then
    ...
    if core.crossesOver(Source.close, STOP, period) or core.crossesUnder(Source.close, LIMIT, period) then
