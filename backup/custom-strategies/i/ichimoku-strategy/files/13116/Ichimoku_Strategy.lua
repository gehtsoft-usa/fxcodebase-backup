...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=5437
...
function Init() --The strategy profile initialization
    strategy:name("Ichimoku strategy")
    strategy:description("Ichimoku strategy")
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if (Ich.DATA:first() > (period - 1)) then
        return
    end
    if (Ich.CS:first() > (period - instance.parameters.KijunSenPeriod - 1)) then
        return
    end
    ...
    if
        Ich.SL[period - 1] < Ich.TL[period - 1] and
            Ich.CS[period - instance.parameters.KijunSenPeriod - 1] <
                source.close[period - instance.parameters.KijunSenPeriod - 1] and
            source.close[period - 1] < math.min(Ich.SA[period - 1], Ich.SB[period - 1])
     then
        MustS1 = true
    end
    if
        Ich.SL[period - 1] > Ich.TL[period - 1] and
            Ich.CS[period - instance.parameters.KijunSenPeriod - 1] >
                source.close[period - instance.parameters.KijunSenPeriod - 1] and
            source.close[period - 1] > math.max(Ich.SA[period - 1], Ich.SB[period - 1])
     then
        MustB1 = true
    end

    if
        Ich.SL[period] < Ich.TL[period] and
            Ich.CS[period - instance.parameters.KijunSenPeriod] <
                source.close[period - instance.parameters.KijunSenPeriod] and
            source.close[period] < math.min(Ich.SA[period], Ich.SB[period]) and
            MustS1 == false
     then
    ...
    if
        Ich.SL[period] > Ich.TL[period] and
            Ich.CS[period - instance.parameters.KijunSenPeriod] >
                source.close[period - instance.parameters.KijunSenPeriod] and
            source.close[period] > math.max(Ich.SA[period], Ich.SB[period]) and
            MustB1 == false
     then
        if instance.parameters.TypeSignal == "direct" then
            MustB = true
