...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=12319
...
local FRAMES={"H1", "H4", "H8", "D1" }
local TfNumber =4;

function Init() --The strategy profile initialization
    strategy:name("MTF ICH Cloud Strategy");
...
function TIMEFRAME(id, FRAME)
    ...
    strategy.parameters:addInteger(tostring(id*100 + 2), "Tenkan-sen period", "", 9, 1, 10000);
    strategy.parameters:addInteger(tostring(id*100 + 3), "Kijun-sen period", "", 26, 1, 10000);
    strategy.parameters:addInteger(tostring(id*100 + 4), "Kijun-sen period", "", 52, 1, 10000);
...
function Prepare(nameOnly)
	...
	assert(core.indicators:findIndicator("ICH CLOUD BAR OSCILLATOR") ~= nil, "Please, download and install ICH CLOUD BAR OSCILLATOR.LUA indicator");  
    ...
    Indicator[i] = core.indicators:create("ICH CLOUD BAR OSCILLATOR", Source[i], Parameters[tostring(i*100 + 2)], Parameters[tostring(i*100 + 3)], Parameters[tostring(i*100 + 4)] ,
	 core.rgb(0, 255, 0) , core.rgb(0, 255, 0),  core.rgb(255, 0, 0) , core.rgb(255, 0, 0),  core.rgb(0, 0, 0) , core.rgb(0, 0, 0) );
...
function ExtUpdate(id, source, period)  -- The method called every time when a new bid or ask price appears.
   ...
   -- get only time
	...
	return;
			...
			and Indicator[1].DATA:colorI(Indicator[1].DATA:size()-3) ~=  core.rgb(0,255, 0) 
           and Indicator[2].DATA:colorI(Indicator[2].DATA:size()-2) ==  core.rgb(0,255, 0) 	
           and Indicator[3].DATA:colorI(Indicator[3].DATA:size()-2) ==  core.rgb(0,255, 0) 			
          and Indicator[4].DATA:colorI(Indicator[4].DATA:size()-2) ==  core.rgb(0,255, 0) 					   
		   then
		  ...
		  elseif Indicator[1].DATA:colorI(Indicator[1].DATA:size()-2) ==  core.rgb(255,0, 0) 
			and Indicator[1].DATA:colorI(Indicator[1].DATA:size()-3) ~=  core.rgb(255,0, 0) 
           and Indicator[2].DATA:colorI(Indicator[2].DATA:size()-2) ==  core.rgb(255,0, 0) 		
          and Indicator[3].DATA:colorI(Indicator[3].DATA:size()-2) ==  core.rgb(255,0, 0) 
        and Indicator[4].DATA:colorI(Indicator[4].DATA:size()-2) ==  core.rgb(255,0, 0) 			  
		 then
            if Direction then
                SELL();
            else
