...
-- http://fxcodebase.com/code/viewtopic.php?f=17&t=69124
-- More information about this indicator can be found at:
...
local alerts = {
    {
        ...
        UpCondition = function(period)
            return (Investors[period] - Investors[period - 1]) >= move
        end,
        DownCondition = function(period)
            return (Investors[period - 1] - Investors[period]) >= move
        end,
        ...
        OnChange = true,
        Name = "Investors alert"
    },
    {
        Stage = 102,
        UpCondition = function(period)
            return (Speculators[period] - Speculators[period - 1]) >= move
        end,
        DownCondition = function(period)
            return (Speculators[period - 1] - Speculators[period]) >= move
        end,
        FormatMessage = function(source, period, level, label, isUp)
            return string.format(
                "Label: %s\013\010" ..
...
function Init()
    ...
    indicator.parameters:addInteger("move", "Move", "", 10)
...
function Prepare(nameOnly)
    ...
    if Type ~= "Speculators" then
        Investors =
            ...
            name .. ".Investors",
            "Investors",
            instance.parameters.Investors_color,
            first + Period
        )
    ...
    if Type ~= "Investors" then
        Speculators =
            ...
            name .. ".Speculators",
            "Speculators",
            instance.parameters.Speculators_color,
            first + Period
        )
