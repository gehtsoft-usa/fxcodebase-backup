...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=10448
...
function Init() --The strategy profile initialization
    strategy:name("i-Fractals-sig strategy")
    strategy:description("i-Fractals-sig strategy")
    strategy:setTag("Version", "1.1")
    strategy:setTag("NonOptimizableParameters", "Email,SendEmail,SoundFile,RecurrentSound,PlaySound, ShowAlert")

    strategy.parameters:addGroup("Parameters")
    strategy.parameters:addInteger("bd", "Last bar body length", "Last bar body length", 7)
    strategy.parameters:addInteger(
        "bdd",
        "Body lenght for double top/buttom bars",
        "Body lenght for double top/buttom bars",
        40
    )
    strategy.parameters:addInteger("sd", "Shadow difference for fractal bars", "Shadow difference for fractal bars", 11)
    strategy.parameters:addInteger(
        "sdd",
        "Shadow difference for double tops/buttoms bars",
        "Shadow difference for double tops/buttoms bars",
        6
    )
...
function Prepare(nameOnly)
    ...
    assert(
        core.indicators:findIndicator("I-FRACTALS-SIG") ~= nil,
        "Please, download and install I-FRACTALS-SIG.LUA indicator"
    )
    ...
    iFrac =
        ...
        "I-FRACTALS-SIG",
        Source,
        instance.parameters.bd,
        instance.parameters.bdd,
        instance.parameters.sd,
        instance.parameters.sdd
    )
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    iFrac:update(core.UpdateLast)

    -- Check that we have enough data
    if (iFrac.DATA:first() > (period - 1)) then
        return
    end
    ...
    if iFrac.DATA[period] == source.low[period] then
    ...
    if iFrac.DATA[period] == source.high[period] then
        if instance.parameters.TypeSignal == "direct" then
            MustOpenS = true
        else
            MustOpenB = true
