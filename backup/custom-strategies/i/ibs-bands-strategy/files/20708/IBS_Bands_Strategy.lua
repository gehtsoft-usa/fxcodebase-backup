...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=9714
...
function Init() --The strategy profile initialization
    strategy:name("IBS Bands strategy")
    strategy:description("IBS Bands strategy")
    strategy:setTag(
    ...
    strategy.parameters:addString("Method", "Method of MA", "", "MVA")
    ...
    strategy.parameters:addInteger("Period", "Period of MA", "", 5)
    strategy.parameters:addString("Price", "Price", "", "Close-Low")
    strategy.parameters:addStringAlternative("Price", "Close-Low", "", "Close-Low")
    strategy.parameters:addStringAlternative("Price", "High-Close", "", "High-Close")
    strategy.parameters:addInteger("ExtrPeriod", "Extremum Period", "", 25)
...
function Prepare(nameOnly)
                                        ...
                                        instance.parameters.Period ..
                                            ", " ..
                                                instance.parameters.Price ..
                                                    ", " .. instance.parameters.ExtrPeriod .. "))"
    instance:name(name)
    if nameOnly then
        return
    end
    ...
    assert(core.indicators:findIndicator("IBS_BANDS") ~= nil, "Please, download and install IBS_BANDS.LUA indicator")
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    IBS:update(core.UpdateLast)

    -- Check that we have enough data
    if (IBS.DATA:first() > (period - 1)) then
        return
    end
    ...
    if IBS.IBS[period] > IBS.TopBand[period] then
    ...
    if IBS.IBS[period] < IBS.BottomBand[period] then
        if instance.parameters.TypeSignal == "direct" then
            MustOpenS = true
        else
            MustOpenB = true
