...
function Prepare( nameOnly)
    ...
    Indicator = core.indicators:create("ICH", Source, Tenkan, Kijun, Senkou);
    
	
	SA=Indicator.SA;
	SB=Indicator.SB;
	 
	first=Indicator.SA:first(); 
...
function ExtUpdate(id, source, period)  -- The method called every time when a new bid or ask price appears.
   ...
   -- get only time
	...
	CS=Indicator.CS;
	local chikouPeriod = CS:size()- 1;
	
    if (HA.close[period] > math.max(SA[period], SB[period])
        and HA.close[period-1]<= math.max(SA[period-1], SB[period-1]))
	and 
       (CS[chikouPeriod] > math.max(SA[chikouPeriod], SB[chikouPeriod])
        and CS[chikouPeriod - 1]<= math.max(SA[chikouPeriod-1], SB[chikouPeriod-1]))
	then	
          if Direction then
                BUY();
            else
    ...
    elseif  (HA.close[period]< math.min(SA[period], SB[period])
            and HA.close[period-1]>= math.min(SA[period-1], SB[period-1]))
        and
            (CS[chikouPeriod] < math.min(SA[chikouPeriod], SB[chikouPeriod])
            and CS[chikouPeriod - 1]>= math.min(SA[chikouPeriod-1], SB[chikouPeriod-1]))
	then
            if Direction then
                SELL();
            else
