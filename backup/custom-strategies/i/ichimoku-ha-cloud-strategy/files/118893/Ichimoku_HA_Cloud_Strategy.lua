...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=65771
...
function Init() --The strategy profile initialization
    strategy:name("Ichimoku HA Cloud Strategy");
    ...
    strategy.parameters:addInteger("T", "Tenkan Period", "Tenkan Period", 6);
    strategy.parameters:addInteger("K", "Kijun Period", "Kijun Period", 15);
    strategy.parameters:addInteger("S", "Senkou Period", "Senkou Period", 34);
...
function CreateTradingParameters()
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "ICHACS");
...
function ExtUpdate(id, source, period)  -- The method called every time when a new bid or ask price appears.
   ...
   -- get only time
	...
	local _first = math.max( first["SA"],  first["SB"], first["CS"] + Kijun + 1 ,  first["SA"], first["SL"],  first["TL"]);
    if  period-1 > _first then  

        ---1---     
        if (math.max(ICH.SA[period], ICH.SB[period]) < Source.close[period] and  math.max(ICH.SA[period-1], ICH.SB[period-1]) > Source.close[period-1]) and
           (ICH.CS[period-Kijun-1] < math.max(ICH.SA[period-1-Kijun], ICH.SB[period-1-Kijun]) and ICH.CS[period-Kijun] > math.max(ICH.SA[period-Kijun], ICH.SB[period-Kijun])) and
            ICH.SL[period] > ICH.SL[period -1] and 
            ICH.TL[period] > ICH.TL[period -1] and
            ICH.SA[period] > ICH.SA[period -1] and
            ICH.SB[period] > ICH.SB[period -1] 
            then
            if Direction then
                BUY();
            else
        ...
        elseif         
            math.max(ICH.SA[period], ICH.SB[period]) >  Source.close[period] and  math.max(ICH.SA[period-1], ICH.SB[period-1]) < Source.close[period-1] and
		    ICH.CS[period-Kijun-1] > math.max(ICH.SA[period-1-Kijun], ICH.SB[period-1-Kijun]) and ICH.CS[period-Kijun] < math.max(ICH.SA[period-Kijun], ICH.SB[period-Kijun]) and
            ICH.SL[period] < ICH.SL[period -1] and 
            ICH.TL[period] < ICH.TL[period -1] and
            ICH.SA[period] < ICH.SA[period -1] and
        ...
        ---2---
        if ((HA.close[period] > math.max(ICH.SA[period], ICH.SB[period])
            and HA.close[period-1]<= math.max(ICH.SA[period-1], ICH.SB[period-1]))
        and 
           (ICH.CS[period-Kijun] > math.max(ICH.SA[period-Kijun], ICH.SB[period-Kijun])
            and ICH.CS[period-Kijun - 1]<= math.max(ICH.SA[period-Kijun-1], ICH.SB[period-Kijun-1])))
        ...
        then	
            ...
            ONE = Source:serial(period);
        elseif  (HA.close[period]< math.min(ICH.SA[period], ICH.SB[period])
                and HA.close[period-1]>= math.min(ICH.SA[period-1], ICH.SB[period-1]))
            and
                (ICH.CS[period-Kijun] < math.min(ICH.SA[period-Kijun], ICH.SB[period-Kijun])
                and ICH.CS[period-Kijun - 1]>= math.min(ICH.SA[period-Kijun-1], ICH.SB[period-Kijun-1]))
        then
            if Direction then
                SELL();
            else
...
function ExtAsyncOperationFinished(cookie, success, message)
    ...
    if cookie == 100 then
        ...
        if UseMandatoryClosing and AllowTrade then
            now = core.host:execute("getServerTime");
            now= core.host:execute ("convertTime", core.core.TZ_EST, ToTime, now);
            -- get only time
            now = now - math.floor(now);
