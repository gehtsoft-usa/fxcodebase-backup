...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=69225
...
local STRATEGY_NAME = "ICH trillionairemac Strategy";
...
local indi;
local KijunSenPeriod;
function CreateEntryIndicators(source)
    indi = core.indicators:create("ICH", source, instance.parameters.TenkanSenPeriod, instance.parameters.KijunSenPeriod, instance.parameters.SenkouSpanPeriod);
...
function CreateCustomActions()
    ...
    enterLongAction.AddHeaders = function (headers)
        headers[#headers + 1] = "Buy close > Span B";
        headers[#headers + 1] = "Buy Tenkan-sen > Kijun-sen";
        headers[#headers + 1] = "Buy Chinkou Span > Span B";
    end
    enterLongAction.AddLog = function (source, period, periodFromLast, data, values)
        values["Buy close > Span B"] = tostring(source.close[period] > indi.SB[period + KijunSenPeriod]);
        values["Buy Tenkan-sen > Kijun-sen"] = tostring(indi.SL[period] > indi.TL[period]);
        values["Buy Chinkou Span > Span B"] = tostring(indi.CS[period - KijunSenPeriod] > indi.SB[period + KijunSenPeriod]);
    end
    enterLongAction.IsPass = function (source, period, periodFromLast, data)
        if not indi.SB:hasData(period + KijunSenPeriod) 
            or not indi.SL:hasData(period)
            or not indi.TL:hasData(period)
            or not indi.CS:hasData(period - KijunSenPeriod)
            or not indi.SB:hasData(period + KijunSenPeriod)
        then
            return;
        end
        return source.close[period] > indi.SB[period + KijunSenPeriod]
            and indi.SL[period] > indi.TL[period]
            and indi.CS[period - KijunSenPeriod] > indi.SB[period + KijunSenPeriod];
    end
    ...
    enterShortAction.AddHeaders = function (headers)
        headers[#headers + 1] = "Sell close < Span A";
        headers[#headers + 1] = "Sell Tenkan-sen < Kijun-sen";
        headers[#headers + 1] = "Sell Chinkou Span < Span B";
    end
    enterShortAction.AddLog = function (source, period, periodFromLast, data, values)
        values["Sell close < Span A"] = tostring(source.close[period] < indi.SA[period + KijunSenPeriod]);
        values["Sell Tenkan-sen < Kijun-sen"] = tostring(indi.SL[period] < indi.TL[period]);
        values["Sell Chinkou Span < Span B"] = tostring(indi.CS[period - KijunSenPeriod] < indi.SB[period + KijunSenPeriod]);
    end
    enterShortAction.IsPass = function (source, period, periodFromLast, data)
        if not indi.SA:hasData(period + KijunSenPeriod) 
            or not indi.SL:hasData(period)
            or not indi.TL:hasData(period)
            or not indi.CS:hasData(period - KijunSenPeriod)
            or not indi.SB:hasData(period + KijunSenPeriod)
        ...
        return source.close[period] < indi.SA[period + KijunSenPeriod]
            and indi.SL[period] < indi.TL[period]
            and indi.CS[period - KijunSenPeriod] < indi.SB[period + KijunSenPeriod];
    end
