...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=64301
...
function Init() --The strategy profile initialization
    strategy:name("Inside Bar Pattern Price Action Strategy")
    ...
    strategy.parameters:addInteger("Period", "MA Period", "", 21)
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if
        Source.high[period - 1] < Source.high[period - 2] and Source.low[period - 1] > Source.low[period - 2] and
            Min == 0
     then
        Min = Source.low[period - 2]
        Max = Source.high[period - 2]
    end
    ...
    Stop = (Max - Min) / Source:pipSize()
    Limit = 2 * (Max - Min) / Source:pipSize()

    if EntryExecutionType == "Live" and id == 1 or EntryExecutionType ~= "Live" and id ~= 1 then
        EntryFunction(period, now)
    end

    if Source.high[period] > Max or Source.low[period] < Min then
        Min = 0
        Max = 0
    end
end
...
function Trade(Start, Flag)
    if Flag == 1 then
        if Source.close[Start] < MA.DATA[Start] then
            return 0
        end

        if Source.close[Start] > Max and Min ~= 0 then
            return 1
        end
    end
    ...
    if Flag == -1 then
        ...
        if Source.close[Start] < Min and Min ~= 0 then
            return -1
        end
    end
