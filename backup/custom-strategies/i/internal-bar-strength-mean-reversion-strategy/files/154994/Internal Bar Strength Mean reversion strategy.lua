-- Available @ https://fxcodebase.com/code/viewtopic.php?f=31&t=74776
...
function Init() --The strategy profile initialization
    strategy:name("Internal Bar Strength Mean reversion strategy");
    ...
    strategy.parameters:addInteger("Period", "MVA Period", "", 20);  
...
function CreateTradingParameters()
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "IBSMRS");
...
function Prepare( nameOnly)
	...
	assert(core.indicators:findIndicator("INTERNAL BAR STRENGTH") ~= nil, "Please, download and install INTERNAL BAR STRENGTH.LUA indicator");          
    ...
    Indicator = core.indicators:create("INTERNAL BAR STRENGTH", Source ); 
    MVA = core.indicators:create("MVA", Source.close, instance.parameters.Period ); 	
...
function EntryFunction( id, now,period)
    ...
    if  Indicator.DATA[period]< 0.1 
	and Indicator.DATA[period-1]>= 0.1 
	...
	if  Indicator.DATA[period]> 0.9 
	and Indicator.DATA[period-1]<= 0.9 
    and Source.close[period] > MVA.DATA[period]	
	then
            if Direction then
                SELL();
