...
local STRATEGY_NAME = "i_Sadukey Strategy";
function CreateParameters()
end

local indi;
function CreateEntryIndicators(source)
    local profile = core.indicators:findIndicator("I_SADUKEY OVERLAY");
    assert(profile ~= nil, "Please, download and install " .. "I_SADUKEY OVERLAY" .. ".LUA indicator");
    local indicatorParams = profile:parameters();
    indi = core.indicators:create("I_SADUKEY OVERLAY", source, core.rgb(0, 255, 0), core.rgb(255, 0, 0));
end
...
function IsEntryLong(source, period)
    return indi.DATA:colorI(period) == core.rgb(0, 255, 0) and source.close[period] > source.open[period];
end
function IsEntryShort(source, period)
    return indi.DATA:colorI(period) == core.rgb(255, 0, 0) and source.close[period] < source.open[period];
end
