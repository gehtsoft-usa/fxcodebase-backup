...
--http://fxcodebase.com/code/viewtopic.php?f=31&t=69816
...
local STRATEGY_NAME = "i_Sadukey_dot Strategy";
function CreateParameters() 
end

local indi;
function CreateEntryIndicators(source)
    local profile = core.indicators:findIndicator("I_SADUKEY_DOT");
    assert(profile ~= nil, "Please, download and install " .. "I_SADUKEY_DOT" .. ".LUA indicator");
    indi = core.indicators:create("I_SADUKEY_DOT", source)
end
...
function IsEntryShort(source, period)
    return indi.DN:hasData(period) and not indi.DN:hasData(period - 1);
end
