...
--https://fxcodebase.com/code/viewtopic.php?f=31&t=71283
...
function CreateOptions(id, name)
    strategy.parameters:addBoolean("Use" .. id, "Use section " .. name, "", true);
    strategy.parameters:addString("Option1" .. id, "Option 1 " .. name, "", "Open");
    strategy.parameters:addStringAlternative("Option1" .. id, "Open", "", "Open");
    strategy.parameters:addStringAlternative("Option1" .. id, "High", "", "High");
    strategy.parameters:addStringAlternative("Option1" .. id, "Low", "", "Low");
    strategy.parameters:addStringAlternative("Option1" .. id, "Close", "", "Close");
    strategy.parameters:addStringAlternative("Option1" .. id, "Median", "", "Median");
    strategy.parameters:addStringAlternative("Option1" .. id, "Typical", "", "Typical");
    strategy.parameters:addStringAlternative("Option1" .. id, "Weighted", "", "Weighted");
    strategy.parameters:addString("Option2" .. id, "Option 2 " .. name, "", ">");
    strategy.parameters:addStringAlternative("Option2" .. id, ">", "", ">");
    strategy.parameters:addStringAlternative("Option2" .. id, "<", "", "<");
    strategy.parameters:addString("Option3" .. id, "Option 3 " .. name, "", "Open");
    strategy.parameters:addStringAlternative("Option3" .. id, "Open", "", "Open");
    strategy.parameters:addStringAlternative("Option3" .. id, "High", "", "High");
    strategy.parameters:addStringAlternative("Option3" .. id, "Low", "", "Low");
    strategy.parameters:addStringAlternative("Option3" .. id, "Close", "", "Close");
    strategy.parameters:addStringAlternative("Option3" .. id, "Median", "", "Median");
    strategy.parameters:addStringAlternative("Option3" .. id, "Typical", "", "Typical");
    strategy.parameters:addStringAlternative("Option3" .. id, "Weighted", "", "Weighted");
    strategy.parameters:addInteger("N" .. id, "N " .. name, "", 1);
end

function CreateParameters() 
    strategy.parameters:addGroup("Enter Long")
    CreateOptions("a_el", "A");
    CreateOptions("b_el", "B");

    strategy.parameters:addGroup("Enter Short")
    CreateOptions("a_es", "A");
    CreateOptions("b_es", "B");

    strategy.parameters:addGroup("Close Long")
    CreateOptions("a_cl", "A");
    CreateOptions("b_cl", "B");

    strategy.parameters:addGroup("Close Short")
    CreateOptions("a_cs", "A");
    CreateOptions("b_cs", "B");
end

function GetPrice(source, period, type)
    if type == "Open" then
...
function CreateSubcondition(id)
    local condition = {};
    condition.Use = instance.parameters:getBoolean("Use" .. id);
    condition.Option1 = instance.parameters:getString("Option1" .. id);
    condition.Option2 = instance.parameters:getString("Option2" .. id);
    condition.Option3 = instance.parameters:getString("Option3" .. id);
    condition.N = instance.parameters:getInteger("N" .. id);
    function condition:IsPass(source, period)
        if not self.Use then
            return true;
        end
        if self.Option2 == ">" then
            return GetPrice(source, period, self.Option1) > GetPrice(source, period - self.N, self.Option3);
        end
        return GetPrice(source, period, self.Option1) < GetPrice(source, period - self.N, self.Option3);
    end
    return condition;
...
function CreateCondition(id)
    local condition = {};
    condition.ConditionA = CreateSubcondition("a_" .. id)
    condition.ConditionB = CreateSubcondition("b_" .. id)
    function condition:IsPass(source, period)
        return self.ConditionA:IsPass(source, period)
            and self.ConditionB:IsPass(source, period);
    end
    return condition;
end

local EntryLong, EntryShort, CloseLong, CloseShort;
function CreateEntryIndicators(source)
    EntryLong = CreateCondition("el");
    EntryShort = CreateCondition("es");
    CloseLong = CreateCondition("cl");
    CloseShort = CreateCondition("cs");
end
...
function IsEntryLong(source, period)
    return EntryLong:IsPass(source, period);
end
function IsEntryShort(source, period)
    return EntryShort:IsPass(source, period);
end
function IsExitLong(source, period)
    return CloseLong:IsPass(source, period);
end
function IsExitShort(source, period)
    return CloseShort:IsPass(source, period);
end
