...
--http://fxcodebase.com/code/viewtopic.php?f=31&t=65078
...
function Init()
    strategy:name("Pivot Cross Strategy");
    strategy:description("Pivot Cross Strategy");
...
function Prepare(name_only)
    ...
    assert(core.indicators:findIndicator("PIVOT") ~= nil, "Please, download and install PIVOT indicator");
    indicator_1 = core.indicators:create("PIVOT", trading_logic.MainSource, "D1", "Pivot", "HIST");
...
function EntryFunction(source, period)
    ...
    local _v2 = trading_logic:GetLastPeriod(period, source, indicator_1.S2);
    if ((_v1 ~= nil and _v2 ~= nil) and (source.close:size() >= 3 and indicator_1.S2:size() >= 2 and core.crossesOver(source.close, indicator_1.S2, _v1, _v2))) then
        trading:CreateMarketOrderCustom(source:instrument(), "B", nil, nil, indicator_1.R1:tick(NOW), nil, "default", nil);
    end
    local _v3 = trading_logic:GetLastPeriod(period, source, indicator_1.R2);
    if ((_v1 ~= nil and _v3 ~= nil) and (source.close:size() >= 3 and indicator_1.R2:size() >= 2 and core.crossesUnder(source.close, indicator_1.R2, _v1, _v3))) then
        trading:CreateMarketOrderCustom(source:instrument(), "S", nil, nil, indicator_1.S1:tick(NOW), nil, "default", nil);
    end
end
