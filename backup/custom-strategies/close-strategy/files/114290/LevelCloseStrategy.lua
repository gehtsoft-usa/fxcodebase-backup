...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=2901&start=70
--+------------------------------------------------------------------+
...
function Init()
    ...
    strategy.parameters:addBoolean("DeleteOrders", "Is Delete Orders too?", "", false);
...
function Prepare(nameOnly)
	...
	IsDeleteOrders=instance.parameters.DeleteOrders; 
...
function ExtUpdate(id, source, period)
    ...
    if haveTrades() then
        ...
        if (PL>=instance.parameters.ProfitLevel and instance.parameters.LevelType=="ABOVE") or (PL<=instance.parameters.ProfitLevel and instance.parameters.LevelType=="BELOW") then
            ...
            if IsDeleteOrders == true then
                local orders = core.host:findTable("orders");
                RemoveOrdersFromContingencyGroup(orders);
                RemoveOrders(orders);
            end
...
function RemoveOrdersFromContingencyGroup(orders)

    local enum = orders:enumerator();
    local row = enum:next();
    while (row ~= nil) do
        if row.ContingencyType ~= 0 and row.FixStatus == "W" and row.AccountID == Account and (All== true or row.Instrument==instance.parameters.Symbol) then
            
            if ShowAlert then
                ExtSignal(source, period, "RemoveOrderFromContingencyGroup", SoundFile, Email, RecurrentSound);
            end
            
            if AllowTrade then
                local valuemap = core.valuemap();
                valuemap.Command = "RemoveFromContingencyGroup";
...
function RemoveOrders(orders)
    ...
    local row = enum:next();
    while (row ~= nil) do
        if row.FixStatus == "W" and row.AccountID == Account and(All== true or row.Instrument==instance.parameters.Symbol) then
        
            if ShowAlert then
                ExtSignal(source, period, "RemoveOrder", SoundFile, Email, RecurrentSound);
            end
