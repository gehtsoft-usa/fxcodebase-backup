...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=2901
...
function Init()
    ...
    strategy.parameters:addDouble("EquityLevel", "Equity level for close orders in the account currency", "", 0);
    ...
    strategy.parameters:addBoolean("close_positions", "Close positions", "", true);
    strategy.parameters:addBoolean("delete_orders", "Delete orders", "", true);
...
function Prepare(nameOnly)
    ...
    close_positions = instance.parameters.close_positions;
    delete_orders = instance.parameters.delete_orders;
    ...
    local equityLevel = instance.parameters.EquityLevel
...
function ExtUpdate(id, source, period)
    ...
    if haveTrades() then
        ...
        local equityLevel =  instance.parameters.EquityLevel;
        local currentEquity = core.host:findTable("accounts"):find("AccountID", Account).Equity;
        if (currentEquity > equityLevel and instance.parameters.LevelType=="ABOVE") 
            or (currentEquity <= equityLevel and instance.parameters.LevelType=="BELOW")
        then
            if delete_orders then
                ...
                while (order ~= nil) do
                    ...
                    terminal:execute(100, valuemap);
                    order = enum:next();
                end
            end
            if close_positions then
                local enum = trades:enumerator();
                while true do
                    local row = enum:next();
                    if row == nil then break end
