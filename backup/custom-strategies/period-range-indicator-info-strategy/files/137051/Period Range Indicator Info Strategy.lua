...
function Init() --The strategy profile initialization
    strategy:name("Period Range Indicator Info Strategy");
    ...
    strategy.parameters:addGroup("Trade");
	 strategy.parameters:addBoolean("Shift", "Shift by one period", "", false);   
    strategy.parameters:addInteger("Period", "Number of Periods","" , 24);
	strategy.parameters:addDouble("Ratio", "Risk/Reward ","" , 1);
	strategy.parameters:addDouble("Position", "Position % ","" , 25);
...
function CreateTradingParameters()
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "PRIIS");
...
local Indicator,Shift;
local Period, Ratio, Position;
...
function Prepare( nameOnly)
	...
	assert(core.indicators:findIndicator("PERIOD RANGE INDICATOR INFO") ~= nil, "Please, download and install PERIOD RANGE INDICATOR INFO.LUA indicator");          
    ...
    Indicator = core.indicators:create("PERIOD RANGE INDICATOR INFO", Source, Period,Ratio, Position, true  );
...
function ExitFunction(id, now,period)
	...
	--/////////////////////////////////////////////////////////////////////
		            ...
		            if    Source.close[period]< Indicator.StopLong[period-xperiod]
					or   Source.close[period]> Indicator.LimitLong[period-xperiod]
					then
					 ...
					 if Source.close[period]> Indicator.StopShort[period-xperiod]
					or   Source.close[period]< Indicator.LimitShort[period-xperiod]
					 then
...
function EntryFunction( id, now,period)
    ...
    if Source.close[period]> Indicator.EntryLong[period-xperiod]
	and  Source.close[period-1]<= Indicator.EntryLong[period-1-xperiod]
	then	
          if Direction then
                BUY();
            else
		...
		Return=true;
    elseif  Source.close[period]<Indicator.EntryShort[period-xperiod]
	and  Source.close[period-1]>= Indicator.EntryShort[period-1-xperiod]
	then
            if Direction then
                SELL();
            else
