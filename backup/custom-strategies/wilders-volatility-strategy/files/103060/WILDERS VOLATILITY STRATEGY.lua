...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=62829&p
...
function Init() --The strategy profile initialization
    strategy:name("WILDERS VOLATILITY STRATEGY")
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    --[[
	BUY: Cross(C,Ref(LLV(C,Period),-1)+(Ref(ATR(Period),-1)*3))
   SELL: Cross(Ref(HHV(C,Period),-1)-(Ref(ATR(Period),-1)*3),C)
	]]
    local Value1 = mathex.min(Source.close, period - 1 - Period + 1, period - 1) + ATR.DATA[period - 1] * 3
    local Value2 = mathex.min(Source.close, period - 1 - 1 - Period + 1, period - 1 - 1) + ATR.DATA[period - 1 - 1] * 3

    local Value3 = mathex.max(Source.close, period - 1 - Period + 1, period - 1) - ATR.DATA[period - 1] * 3
    local Value4 = mathex.max(Source.close, period - 1 - 1 - Period + 1, period - 1 - 1) - ATR.DATA[period - 1 - 1] * 3

    -- only buy if we have a fast cross over slow and the price is above the moving averages.
    if Source.close[period] > Value1 and Source.close[period - 1] <= Value2 then
        if Direction then
            BUY()
        else
            SELL()
    ...
    elseif Source.close[period] < Value3 and Source.close[period - 1] >= Value4 then
        if Direction then
            SELL()
        else
            BUY()
