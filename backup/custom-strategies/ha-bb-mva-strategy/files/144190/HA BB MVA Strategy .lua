...
-- https://fxcodebase.com/code/viewtopic.php?f=31&t=71625
...
function Init() --The strategy profile initialization
    strategy:name("HA BB MVA Strategy");
    ...
    strategy.parameters:addGroup("MA Calculatione");
    strategy.parameters:addInteger("Period", "Period", "", 100);
	
	
	strategy.parameters:addGroup("BB Calculation")
    strategy.parameters:addInteger("BB_Period", "Period", "", 20)
    strategy.parameters:addDouble("Deviation", "Deviation", "", 0.5)
...
function Prepare( nameOnly)
    ...
    MA = core.indicators:create("MVA", HA.close, Period  ); 
    BB = core.indicators:create("BB", HA.close, BB_Period, Deviation)	
	first=math.max(MA.DATA:first(), HA.DATA:first(), BB.DATA:first()); 
...
--	period3= core.findDate (Source3, TickSource:date(period), false);
	...
	--/////////////////////////////////////////////////////////////////////
		            ...
		            if     HA.close[period]<  BB.BL[period]	
					then
					  ...
					  if   HA.close[period]>  BB.TL[period]	
					  then
...
function EntryFunction( id, now,period)
    ...
    if   HA.close[period]>  MA.DATA[period]
    and  HA.close[period]>  BB.TL[period]	
    and  HA.close[period-1]<=  BB.TL[period-1]		
	then	
          if Direction then
                BUY();
            else
		...
		Return=true;
    elseif   HA.close[period]<  MA.DATA[period]
    and  HA.close[period]<  BB.BL[period]	
    and  HA.close[period-1]>=  BB.BL[period-1]		
		then
            if Direction then
                SELL();
            else
