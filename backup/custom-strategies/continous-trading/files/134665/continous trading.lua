...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=69719
...
function Init()
    strategy:name("Continous Trading");
    ...
    strategy.parameters:addString("side", "Side to start", "", "B");
    strategy.parameters:addStringAlternative("side", "Buy", "", "B");
    strategy.parameters:addStringAlternative("side", "Sell", "", "S");
    
    strategy.parameters:addDouble("stop", "Stop, pips", "", 30);
    strategy.parameters:addDouble("limit", "Limit, pips", "", 30);
    strategy.parameters:addDouble("martignale_mult", "Lot multiplicator", "", 2);
end

local MAIN_SOURCE_ID = 1;
local main_source, stop, limit, mult, martingale_stop, martignale_mult;
function Prepare(nameOnly)
    ...
    martignale_mult = instance.parameters.martignale_mult;
    main_source = ExtSubscribe(MAIN_SOURCE_ID, nil, instance.parameters.timeframe, instance.parameters.type, "bar")
    tables_monitor:ListenCloseTrade(function (closed_trade)
        if tables_monitor.closing_order_types[closed_trade.CloseOrderID] == "S" then
            local base_size = core.host:execute("getTradingProperty", "baseUnitSize", closed_trade.Instrument, closed_trade.AccountID);
            local command = trading:EntryOrder(closed_trade.Instrument)
                :SetAccountID(closed_trade.AccountID)
                :SetAmount(closed_trade.Lot * martignale_mult * 2 / base_size)
                :SetSide(closed_trade.BS)
                :SetPipStop(nil, stop)
                :SetPipLimit(nil, limit)
                :SetRate(closed_trade.Open);
    
            result = command:Execute();
        elseif tables_monitor.closing_order_types[closed_trade.CloseOrderID] == "L" then
            if main_source.close[NOW] > main_source.open[NOW] then
                Open("B");
            else
                Open("S");
            end
...
function Open(side)
    trading:FindOrder()
        ...
        :WhenInstrument(instance.bid:instrument())
        :Do(function (order)
            if not trading:IsStopOrder(order) and not trading:IsLimitOrder(order) then
    ...
    local rate;
    if side == "B" then
        rate = instance.bid[NOW] - stop * instance.bid:pipSize();
    else
        rate = instance.ask[NOW] + stop * instance.ask:pipSize();
    end
    ...
    local base_size = core.host:execute("getTradingProperty", "baseUnitSize", instance.bid:instrument(), instance.parameters.Account);
    local command = trading:EntryOrder(instance.bid:instrument())
        :SetAccountID(instance.parameters.Account)
        :SetAmount(instance.parameters.Amount * martignale_mult)
        :SetSide(trading:getOppositeSide(side))
        :SetPipStop(nil, stop)
        :SetPipLimit(nil, limit)
        :SetRate(rate);
