...
function Init() --The strategy profile initialization
    strategy:name("TTC TDI STRATEGY")
    ...
    strategy.parameters:addGroup("Trix Trend Cycle")

    strategy.parameters:addInteger("TrixPeriod", "Period", "", 4)
    strategy.parameters:addDouble("SL", "Sell Level", "", 90)
    strategy.parameters:addDouble("BL", "Buy Level", "", 10)

    strategy.parameters:addGroup("Traders Dynamic Index Indicator")
    ...
    strategy.parameters:addInteger("BE", "Buy Entry Level", "", 35)
    strategy.parameters:addInteger("EB", "Buy Exit Level", "", 68)
    strategy.parameters:addInteger("SE", "Sell Level", "", 65)
    strategy.parameters:addInteger("ES", "Sell Exit Level", "", 32)
...
function Prepare(nameOnly)
    ...
    assert(core.indicators:findIndicator("TTC") ~= nil, "Please, download and install TTC.LUA indicator")
    assert(
    ...
    Indicator[1] = core.indicators:create("TTC", Source, TrixPeriod)
    Short["TTC"] = Indicator[1]:getStream(0)

    Indicator[2] =
        core.indicators:create("TRADERSDYNAMICINDEX", Source[Price], RSI_N, VB_N, VB_W, RSI_P_N, RSI_P_M, TS_N, TS_M)
    Short[1] = Indicator[2]:getStream(0)
    Short[2] = Indicator[2]:getStream(1)
    Short[3] = Indicator[2]:getStream(2)
    Short[4] = Indicator[2]:getStream(3)
    Short[5] = Indicator[2]:getStream(4)
...
1. PRICE LINE OF TDI > BUY ENTRY LEVEL (35) AND ok
2. PRICE LINE OF TDI > TRD SIG OF TDI ok
3. TRD SIG OF TDI> MKT BASE OF TDI ok
4. PRICE LINE OF TDI< BUY EXIT LEVEL(68)
5. PRICE LINE OF TDI >VOL BAND LOW
6. PRICE LINE OF TDI < VOL BAND HIGH
]]
    if
        core.crossesOver(Short["TTC"], BL, period) and Short[1][period] > BE and Short[1][period] > Short[5][period] and
            Short[5][period] > Short[4][period] and
            Short[1][period] < EB and
            Short[1][period] > Short[3][period] and
            Short[1][period] < Short[2][period]
     then
        --[[
SELLING CONDITIONS:
...
6. PRICE LINE OF TDI < VOL BAND HIGH]]
    ...
    elseif
        core.crossesUnder(Short["TTC"], SL, period) and Short[1][period] < SE and Short[1][period] < Short[5][period] and
            Short[5][period] < Short[4][period] and
            Short[1][period] > ES and
            Short[1][period] > Short[3][period] and
            Short[1][period] < Short[2][period]
     then
        if Direction then
    ...
    if EXIT then
        if core.crossesUnder(Short["TTC"], SL, period) then
            if Direction then
        ...
        if core.crossesOver(Short["TTC"], BL, period) then
            if Direction then
                if haveTrades("S") then
                    exit("S")
                    Signal("Close Short")
