...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=60728&p
...
function Init() --The strategy profile initialization
    strategy:name("UltraRSI Strategy")
    ...
    strategy.parameters:addInteger("RSI_Period", "RSI period", "", 13)
    strategy.parameters:addString("Method1", "Average method", "", "MVA")
    ...
    strategy.parameters:addInteger("Start", "Start length", "", 3)
    strategy.parameters:addInteger("Step", "Step length", "", 5)
    strategy.parameters:addInteger("StepsTotal", "Total steps", "", 10)
    strategy.parameters:addString("Method2", "Smoothing method", "", "MVA")
    ...
    strategy.parameters:addInteger("SmoothPeriod", "Smoothing period", "", 3)
    strategy.parameters:addInteger("Level", "Level", "", 2)
...
local CustomID
local Price
local RSI_Period, Method1, Start, Step, StepsTotal, Method2, SmoothPeriod
...
function Prepare(nameOnly)
    ...
    assert(core.indicators:findIndicator("ULTRARSI") ~= nil, "Please, download and install ULTRARSI.LUA indicator")
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if Indicator.DATA:colorI(period) == core.rgb(0, 255, 0) and Indicator.DATA:colorI(period - 1) ~= core.rgb(0, 255, 0) then
        BUY()
        ONE = Source:serial(period)
    elseif
        Indicator.DATA:colorI(period) == core.rgb(255, 0, 0) and
            Indicator.DATA:colorI(period - 1) ~= core.rgb(255, 0, 0)
     then
        SELL()
        ONE = Source:serial(period)
    end
