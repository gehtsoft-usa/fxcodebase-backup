...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=26036
...
function Init() --The strategy profile initialization
    strategy:name("LRS T3 BAND STRATEGY")
    ...
    strategy.parameters:addGroup("1. LRS Line Calculation")
    strategy.parameters:addInteger("P1", " Period", " Period", 34)
    ...
    strategy.parameters:addDouble("B1", "Buy Level", "", 0)
    strategy.parameters:addDouble("S1", "Sell Level", "", 0)

    strategy.parameters:addGroup("2. LRS Line Calculation")
    strategy.parameters:addInteger("P2", " Period", " Period", 100)
    ...
    strategy.parameters:addDouble("B2", "Buy Level", "", 0)
    strategy.parameters:addDouble("S2", "Sell Level", "", 0)

    strategy.parameters:addGroup("T3 Trend Bands Calculation")
    strategy.parameters:addInteger("BandBars", "Count of bars for band", "", 28)
    strategy.parameters:addDouble("Deviation", "Deviation", "", 3.5)
...
local B1, S1, P1, Price1
local B2, S2, P2, Price2

local BandBars, Deviation
...
function Prepare(nameOnly)
                                        ...
                                        ", " ..
                                            P2 ..
                                                ", " ..
                                                    Price2 ..
                                                        ", " ..
                                                            B2 .. ", " .. S2 .. ", " .. BandBars .. ", " .. Deviation

    assert(core.indicators:findIndicator("LRS") ~= nil, "Please, download and install LRS.LUA indicator")
    assert(
        core.indicators:findIndicator("T3_TREND_BANDS") ~= nil,
        "Please, download and install T3_TREND_BANDS.LUA indicator"
    )
    ...
    Indicator[1] = core.indicators:create("LRS", Source[Price1], P1)
    Short[1] = Indicator[1]:getStream(0)

    Indicator[2] = core.indicators:create("LRS", Source[Price2], P2)
    Short[2] = Indicator[2]:getStream(0)

    Indicator[3] = core.indicators:create("T3_TREND_BANDS", Source, BandBars, Deviation)
    Short["T"] = Indicator[3]:getStream(0)
    Short["M"] = Indicator[3]:getStream(1)
    Short["B"] = Indicator[3]:getStream(2)

    first = math.max(Short[1]:first(), Short[2]:first(), Short["T"]:first())
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if Short[1][period] > B1 and Short[2][period] > B2 and core.crossesOver(Source.close, Short["M"], period) then
        if Direction then
            BUY()
        else
            SELL()
        end
    elseif Short[1][period] < S1 and Short[2][period] < S2 and core.crossesUnder(Source.close, Short["M"], period) then
        if Direction then
            SELL()
        else
            BUY()
    ...
    --[[
		...
		1.price cross-under upper line or
		2. price cross under lower line

		exit sell: 

		1.price cross over lower line or
		2.price cross over upper line]]
    if core.crossesUnder(Source.close, Short["T"], period) or core.crossesUnder(Source.close, Short["B"], period) then
        if Direction then
    ...
    if core.crossesOver(Source.close, Short["T"], period) or core.crossesOver(Source.close, Short["B"], period) then
        if Direction then
            if haveTrades("S") then
                exit("S")
                Signal("Close Short")
