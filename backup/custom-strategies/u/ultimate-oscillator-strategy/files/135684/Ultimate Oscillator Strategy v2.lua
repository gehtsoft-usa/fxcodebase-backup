...
function Prepare(nameOnly)
    ...
    Indicator =
        core.indicators:create("UOSC", Source, instance.parameters.S, instance.parameters.M, instance.parameters.L)
...
function EntryFunction(id, now, period)
    ...
    if FindFirst("Long", period) and Indicator.DATA[period] < OS then
        if Direction then
            BUY()
        else
            SELL()
        ...
        Return = true
    elseif FindFirst("Short", period) and Indicator.DATA[period] > OB then
        if Direction then
            SELL()
        else
            BUY()
...
function FindFirst(Trade_Direction, period)
    ...
    if Trade_Direction == "Long" then
        for i = period, first, -1 do
            if Indicator.DATA[i] < OS and Source.close[i] > Source.open[i] then
                Count = Count + 1
            end
        end
    elseif Trade_Direction == "Short" then
        for i = period, first, -1 do
            if Indicator.DATA[i] > OB and Source.close[i] < Source.open[i] then
                Count = Count + 1
            end
        end
    end
