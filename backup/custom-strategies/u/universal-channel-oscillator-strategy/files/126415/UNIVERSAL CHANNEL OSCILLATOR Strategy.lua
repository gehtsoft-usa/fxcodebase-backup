...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=68476
...
function Init() --The strategy profile initialization
    strategy:name("UNIVERSAL CHANNEL OSCILLATOR Strategy");
    ...
    strategy.parameters:addInteger("UniversalPeriod", "Period", "", 14, 2, 2000);
	strategy.parameters:addInteger("UNIVERSALFILTER", "UNIVERSAL FILTER", "", 14, 2, 2000);
    strategy.parameters:addDouble("OVERBOUGHT", "OVERBOUGHT", "", 1.5);
	strategy.parameters:addDouble("OVERSOLD", "OVERSOLD", "", 1.5);
	strategy.parameters:addInteger("UniAvgPeriod", "UniAvgPeriod", "", 14);
	 
	strategy.parameters:addInteger("AveragePeriod", "AveragePeriod", "", 14);
	strategy.parameters:addInteger("ATRperiod", "ATRperiod", "", 14);
    strategy.parameters:addInteger("TightenChannel", "TightenChannel", "", 14);
...
function CreateTradingParameters()
   ...
   -- strategy.parameters:addStringAlternative("ExitExecutionType", "End of Turn", "", "EndOfTurn");
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "UCOA");
...
local Indicator;
local UniversalPeriod, UNIVERSALFILTER, OVERBOUGHT, OVERSOLD,UniAvgPeriod, AveragePeriod, ATRperiod, TightenChannel, Method;
...
function Prepare( nameOnly)
	...
	assert(core.indicators:findIndicator("UNIVERSAL CHANNEL OSCILLATOR") ~= nil, "Please, download and install UNIVERSAL CHANNEL OSCILLATOR.LUA indicator");          
    ...
    Indicator = core.indicators:create("UNIVERSAL CHANNEL OSCILLATOR", Source, UniversalPeriod, UNIVERSALFILTER, OVERBOUGHT, OVERSOLD,UniAvgPeriod, AveragePeriod, ATRperiod, TightenChannel, Method  );
 
	first=Indicator.UniAvgline:first(); 
...
function EntryFunction( id, now,period)
    ...
    if   Indicator.Buffer4[period] > Indicator.UniAvgline[period]
	and Indicator.Buffer4[period-1] <= Indicator.UniAvgline[period-1]
	then	
          if Direction then
                BUY();
            else
		...
		Return=true;
    elseif   Indicator.Buffer4[period] < Indicator.UniAvgline[period]
	and Indicator.Buffer4[period-1] >= Indicator.UniAvgline[period-1]
		then
            if Direction then
                SELL();
