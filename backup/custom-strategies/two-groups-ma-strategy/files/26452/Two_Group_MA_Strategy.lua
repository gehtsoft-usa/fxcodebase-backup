...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=13722
...
function Init() --The strategy profile initialization
    strategy:name("Two groups MA strategy")
    strategy:description("Two groups MA strategy")
    ...
    strategy.parameters:addString("Periods1", "Periods1", "", "2,5,10,13,17,21")
    ...
    strategy.parameters:addString("Periods2", "Periods2", "", "50,75,100,125,150,200")
...
function Prepare(nameOnly)
    ...
    assert(
        core.indicators:findIndicator("SET_OF_AVERAGES2") ~= nil,
        "Please, download and install SET_OF_AVERAGES2.LUA indicator"
    )
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    S_MA1:update(core.UpdateLast)
    S_MA2:update(core.UpdateLast)

    -- Check that we have enough data
    if (S_MA1.DATA:first() > (period - 1)) then
        return
    end
    if (S_MA2.DATA:first() > (period - 1)) then
        return
    end
    ...
    if (S_MA1.Max[period - 1] < S_MA2.Max[period - 1] and S_MA1.Max[period] > S_MA2.Max[period]) then
    ...
    if (S_MA1.Min[period - 1] > S_MA2.Min[period - 1] and S_MA1.Min[period] < S_MA2.Min[period]) then
        if instance.parameters.TypeSignal == "direct" then
            MustOpenS = true
        else
            MustOpenB = true
