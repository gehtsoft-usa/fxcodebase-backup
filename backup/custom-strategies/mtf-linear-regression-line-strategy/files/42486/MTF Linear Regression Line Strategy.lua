...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=24708
...
function Init() --The strategy profile initialization
    strategy:name("MTF Linear Regression Line Strategy");
...
function Parameters (id, TF,PERIOD)
	...
	strategy.parameters:addInteger("P".. id,   "MA Period", "Period", PERIOD);
...
function Prepare(nameOnly)
   ...
   assert(core.indicators:findIndicator("LSMA") ~= nil, "Please, download and install LSMA.LUA indicator");       
    ...
    P[i]=  instance.parameters:getInteger("P" .. i)
    ...
    assert(core.indicators:findIndicator( "LSMA") ~= nil,  "LSMA" .. " indicator must be installed");
    Indicator[i] = core.indicators:create( "LSMA", Source[i][Price[i]], P[i] );	
    ...
    now = now - math.floor(now);		
	 ...
	 if not InRange(now, OpenTime, CloseTime)
      ...
      return ;
		...
		if  core.crossesOver(Short[1], Short[2],Short[1]:size()-2, Short[2]:size()-2)    
        ...
        elseif   core.crossesUnder(Short[1], Short[2],Short[1]:size()-2, Short[2]:size()-2)
		then
            if Direction then
                SELL();
            else
				...
				if  core.crossesUnder(Source[1].close, Short[2], Source[1].close:size()-2, Short[2]:size()-2)  then
									  if   Direction then
			  ...
			  elseif core.crossesOver(Source[1].close, Short[2], Source[1].close:size()-2, Short[2]:size()-2) then
									  if   Direction then    
												if haveTrades("S") then
												   exit("S");
												   Signal ("Close Short");
