...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=65945
...
local Modules = {};
function Init()
    strategy:name("Breakout trading strategy on French CAC40");
    strategy:description("");
    strategy.parameters:addDouble("MaxAmplitude", "Max Amplitude", "", 58)
    strategy.parameters:addDouble("PourcentageMin", "Pourcentage Min", "", 30)
    strategy.parameters:addDouble("OrderDistance", "Order Distance", "", 4)
    strategy.parameters:addDouble("MinAmplitude", "Min Amplitude", "", 11)
    strategy.parameters:addString("OpenTime", "Open Time", "", "09:15:00")
    strategy.parameters:addString("CloseTime", "Close Time", "", "17:00:00")
    strategy.parameters:addString("FlatAfter", "Flat After", "", "19:45:00")
    trading_logic:Init(strategy.parameters);
    trading:Init(strategy.parameters);
end
local custom_id = "STRAT";
local OpenTime;
local CloseTime;
local FlatAfter;
local MaxAmplitude;
local PourcentageMin = 30
local OrderDistance = 4
local MinAmplitude = 11
...
function Prepare(name_only)
    ...
    MaxAmplitude = instance.parameters.MaxAmplitude;
    PourcentageMin = instance.parameters.PourcentageMin;
    OrderDistance = instance.parameters.OrderDistance;
    MinAmplitude = instance.parameters.MinAmplitude;
    -- Market scan begin with the 15 minute candlestick that closed at 9h15 AM
    OpenTime = ParseTime(instance.parameters.OpenTime) - 7.0 / 24.0;
    -- No new position will be initiated after the 5h00 PM candlestick
    CloseTime = ParseTime(instance.parameters.CloseTime) - 7.0 / 24.0;
    -- Position is closed at 7h45 PM, frenh time (in case of CAC40 trading)
    FlatAfter = ParseTime(instance.parameters.FlatAfter) - 7.0 / 24.0;
...
function InTerminationRange(now)
    -- from, to
    if FlatAfter < OpenTime then
        return now >= FlatAfter and now <= OpenTime;
    end
    if FlatAfter > OpenTime then
        return now > FlatAfter or now < OpenTime;
    end

    return now == FlatAfter;
end

function IsTradingDay(source, period)
    -- The 24th and 31th days of December will not be traded because market close before 7h45 PM
    local date = source:date(period);
    local date_struct = core.dateToTable(date);
    if (date_struct.month == 5 and date_struct.day == 1) or (date_struct.month == 12 and (date_struct.day == 24 or date_struct.day == 25 or date_struct.day == 26 or date_struct.day == 30 or date_struct.day == 31)) then
        return false;
    end
    return true;
end

local StartTradingDay = -1
local BuyTreshold = 0
local SellTreshold = 0
local DayStartIndex = 0

function GetDayMinMax(source, period)
    local min, max;
    for i = DayStartIndex, period do
        if min == nil or min > source.low[i] then
            min = source.low[i];
        end
        if max == nil or max < source.high[i] then
            max = source.high[i];
        end
    end
    return min, max;
...
function GetBuySell(source, period)
    local LowLevel, HighLevel = GetDayMinMax(source, period)
    -- Spread calculation between the higher and the 
    -- lower value of the instrument since StartHour
    local DaySpread = HighLevel - LowLevel
    ...
    if DaySpread <= MaxAmplitude then
        ...
        local MinSpread = DaySpread * PourcentageMin / 100

        local s = 0;
        if (source.close[period] - LowLevel) >= MinSpread then
            s = LowLevel + OrderDistance
        end
        local b = 0;
        if (HighLevel - source.close[period]) >= MinSpread then
            b = HighLevel - OrderDistance
        end
        return s, b;
    end
    return 0, 0;
...
function EntryFunction(source, period)
    local date = source:date(period);
    date = date - math.floor(date);
    -- Variables that can change in intraday are initiliazed at first bar on each new day
    if not InRange(date, OpenTime, CloseTime) then
        if StartTradingDay ~= 0 then
            BuyTreshold = 0
            SellTreshold = 0
            StartTradingDay = 0
        end
        ...
        if StartTradingDay == 0 then
            ...
            DayStartIndex = period
            StartTradingDay = 1
        elseif StartTradingDay == 1 then
            ...
            SellTreshold, BuyTreshold = GetBuySell(source, period)
            ...
            if SellTreshold > 0 and BuyTreshold > 0 and (BuyTreshold - SellTreshold) >= MinAmplitude then
                core.host:trace("trading");
                local order = trading:EntryOrder(source:instrument())
                    :SetRate(BuyTreshold)
                    :SetSide("B")
                    :SetDefaultAmount()
                    :SetStop(SellTreshold)
                    :SetCustomID(custom_id);
                local r1 = order:Execute();
                local order = trading:EntryOrder(source:instrument())
                    :SetRate(SellTreshold)
                    :SetSide("S")
                    :SetDefaultAmount()
                    :SetStop(BuyTreshold)
                    :SetCustomID(custom_id);
                local r2 = order:Execute();
                StartTradingDay = 2;
            end
        end
    end
    if InTerminationRange(date) then
        local orders = trading:FindOrder():WhenCustomID(custom_id):All();
        for i, order in ipairs(orders) do
            core.host:trace("termination")
            trading:DeleteOrder(order)
        end
        local trades = trading:FindTrade():WhenCustomID(custom_id):All();
...
function trading_logic:Prepare(name_only)
    ...
    self.MainSource = ExtSubscribe(self._ids_start + 2, nil, "m15", instance.parameters.is_bid, "bar");
end
