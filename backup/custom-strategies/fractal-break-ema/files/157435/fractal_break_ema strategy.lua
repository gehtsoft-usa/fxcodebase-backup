-- Available @ https://fxcodebase.com/code/viewtopic.php?f=17&t=75399
...
local STRATEGY_NAME = "Fractal Break EMA Strategy";
function CreateParameters() 
    strategy.parameters:addInteger("short_ema", "Short EMA Period", "", 7);
    strategy.parameters:addInteger("long_ema", "Long EMA Period", "", 21);
end

local short_ema, long_ema, fractal;
function CreateEntryIndicators(source)
    short_ema = core.indicators:create("EMA", source, instance.parameters.short_ema);
    long_ema = core.indicators:create("EMA", source, instance.parameters.long_ema);
    fractal = core.indicators:create("FRACTAL", source);
end

function UpdateIndicators()
    short_ema:update(core.UpdateLast);
    long_ema:update(core.UpdateLast);
...
function FindFractal(source, period)
...
function IsEntryLong(source, period)
    if not short_ema.DATA:hasData(period - 1) or not long_ema.DATA:hasData(period - 1) then
        return false;
    end
    local dir, p, price = FindFractal(source, period);
    return short_ema.DATA[period] > long_ema.DATA[period] 
        and dir == 1
        and period == p + 2;
end
