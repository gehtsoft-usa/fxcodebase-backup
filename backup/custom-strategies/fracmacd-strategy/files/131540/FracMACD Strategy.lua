...
local STRATEGY_NAME = "FracMACD Strategy";
function CreateParameters() 
    strategy.parameters:addInteger("ShortMACDLength", "ShortMACDLength", "", 14)
    strategy.parameters:addInteger("LongMACDLength", "LongMACDLength", "", 26)
    strategy.parameters:addInteger("MACDSmoothing", "MACDSmoothing", "", 9)
end

local indi;
function CreateEntryIndicators(source)
    local profile = core.indicators:findIndicator("FRACMACD");
    assert(profile ~= nil, "Please, download and install " .. "FRACMACD" .. ".LUA indicator");
    local indicatorParams = profile:parameters();
    indi = core.indicators:create("FRACMACD", source, instance.parameters.ShortMACDLength, 
        instance.parameters.LongMACDLength, instance.parameters.MACDSmoothing);
end
...
function IsEntryLong(source, period)
    if not indi.FracMACD:hasData(period - 1) or not indi.Smooth:hasData(period - 1) then
        return;
    end
    return core.crossesOver(indi.FracMACD, indi.Smooth, period);
end
function IsEntryShort(source, period)
    if not indi.FracMACD:hasData(period - 1) or not indi.Smooth:hasData(period - 1) then
        return;
    end
    return core.crossesUnder(indi.FracMACD, indi.Smooth, period);
end
