...
function Init() --The strategy profile initialization
    strategy:name("ASI MA Cross Strategy")
    ...
    strategy.parameters:addInteger("Period", "Period", "", 300)
...
local first
local ASI, MA
local Period, MA_Period, MA_Method
local Direction
local CustomID
...
function Prepare(nameOnly)
    ...
    ASI = core.indicators:create("ASI", Source, Period)
    assert(core.indicators:findIndicator(MA_Method) ~= nil, MA_Method .. " indicator must be installed");
    MA = core.indicators:create(MA_Method, ASI.DATA, MA_Period)
    first = MA.DATA:first()
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if ASI.DATA[period] > MA.DATA[period] and ASI.DATA[period - 1] <= MA.DATA[period - 1] then
        if Direction then
            BUY()
        else
            SELL()
    ...
    elseif ASI.DATA[period] < MA.DATA[period] and ASI.DATA[period - 1] >= MA.DATA[period - 1] then
        if Direction then
            SELL()
        else
            BUY()
