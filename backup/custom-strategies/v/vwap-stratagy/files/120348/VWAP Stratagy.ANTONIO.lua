...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=66167
...
function Init() --The strategy profile initialization
    strategy:name("VWAP Stratagy")
    strategy:description("ANTONIO v2")

    strategy:setTag("NonOptimizableParameters", "Email,SendEmail,SoundFile,RecurrentSound,PlaySound, ShowAlert")

    strategy.parameters:addInteger("ema_period", "EMA Period", "", 10);
    ...
    strategy.parameters:addGroup("Calculate")

    strategy.parameters:addString("Mode", "Mode", "", "Daily")
    strategy.parameters:addStringAlternative("Mode", "Daily", "", "Daily")
    strategy.parameters:addStringAlternative("Mode", "Weekly", "", "Weekly")
    strategy.parameters:addStringAlternative("Mode", "Monthly", "", "Monthly")
...
function Prepare(nameOnly)
    ...
    assert(core.indicators:findIndicator("VWAP") ~= nil, "Please, download and install VWAP.LUA indicator")
    ...
    if Mode == "Daily" then
        Indicator = core.indicators:create("VWAP", Source, true,1,1,1, false,1,1,1, false,1,1,1)
        stream = Indicator.Daily
    elseif Mode == "Weekly" then
        Indicator = core.indicators:create("VWAP", Source, false,1,1,1, true,1,1,1, false,1,1,1)
        stream = Indicator.Weekly
    elseif Mode == "Monthly" then
        Indicator = core.indicators:create("VWAP", Source, false,1,1,1, false,1,1,1, true,1,1,1)
        stream = Indicator.Monthly
    end
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if not ema.DATA:hasData(period) or period < first then
        return
    end
...
function EntryFunction(now, period)
    ...
    if ema.DATA[period] > stream[period] and ema.DATA[period - 1] <= stream[period - 1] then
        if Direction then
            BUY(period)
        else
            SELL(period)
        ...
        return true
    elseif ema.DATA[period] < stream[period] and ema.DATA[period - 1] >= stream[period - 1] then
        if Direction then
            SELL(period)
        else
            BUY(period)
...
function enter(BuySell, hCount, period)
    ...
    return MarketOrder(BuySell, hCount, period)
end

-- enter into the specified direction
function MarketOrder(BuySell, hCount, period)
    ...
    if SetStop then
        if BuySell == "B" then
            valuemap.PegPriceOffsetPipsStop = Source.low[period]
        else
            valuemap.PegPriceOffsetPipsStop = Source.high[period]
        end
    end
