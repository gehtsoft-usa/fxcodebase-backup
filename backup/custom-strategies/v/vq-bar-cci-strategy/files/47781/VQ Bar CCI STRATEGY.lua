...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=27336&p
...
function Init() --The strategy profile initialization
    strategy:name("VQ Bar CCI STRATEGY")
    ...
    strategy.parameters:addGroup("VQ Bar Calculation")
    strategy.parameters:addString("Method", "Method", "Method", "LWMA")
    ...
    strategy.parameters:addBoolean("Steady", "Steady", "", false)
    strategy.parameters:addInteger("Length", "Length", "", 7)
    strategy.parameters:addInteger("Smoothing", "Smoothing", "", 1)
    strategy.parameters:addDouble("Filter", "Filter", "", 7)

    strategy.parameters:addGroup("CCI Calculation")
    strategy.parameters:addInteger("Period", "Period", "", 34)
...
local Method, Steady, Length, Smoothing, Filter, Period
local BL, SL
...
function Prepare(nameOnly)
    ...
    Indicator[2] = core.indicators:create("CCI", Source, Period)
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if
        Indicator[1].DATA:colorI(period) == core.rgb(0, 255, 0) and
            Indicator[1].DATA:colorI(period - 1) == core.rgb(0, 255, 0) and
            Indicator[1].DATA:colorI(period - 2) == core.rgb(0, 255, 0) and
            Indicator[1].DATA:colorI(period - 3) == core.rgb(0, 255, 0) and
            Indicator[1].DATA:colorI(period - 4) == core.rgb(0, 255, 0) and
            Indicator[1].DATA:colorI(period - 5) == core.rgb(0, 255, 0) and
            Short[2][period] > BL and
            Short[2][period - 1] <= BL and
            Short[2][period - 2] <= BL
     then
        if Direction then
            BUY()
    ...
    if
        Indicator[1].DATA:colorI(period) == core.rgb(255, 0, 0) and
            Indicator[1].DATA:colorI(period - 1) == core.rgb(255, 0, 0) and
            Indicator[1].DATA:colorI(period - 2) == core.rgb(255, 0, 0) and
            Indicator[1].DATA:colorI(period - 3) == core.rgb(255, 0, 0) and
            Indicator[1].DATA:colorI(period - 4) == core.rgb(255, 0, 0) and
            Indicator[1].DATA:colorI(period - 5) == core.rgb(255, 0, 0) and
            Short[2][period] < SL and
            Short[2][period - 1] >= SL and
            Short[2][period - 2] >= SL
     then
        if Direction then
            SELL()
    ...
    if Indicator[1].DATA:colorI(period) ~= core.rgb(0, 255, 0) or Short[2][period] < SL then
        if Direction then
    ...
    if Indicator[1].DATA:colorI(period) ~= core.rgb(255, 0, 0) or Short[2][period] > BL then
        if Direction then
            if haveTrades("S") then
                exit("S")
                Signal("Close Short")
