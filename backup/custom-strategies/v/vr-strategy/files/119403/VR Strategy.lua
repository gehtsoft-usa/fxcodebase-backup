...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=66160
...
function Init() --The strategy profile initialization
    ...
    strategy.parameters:addInteger("Frame", "Period", "Period", 14, 2, 2000);
	strategy.parameters:addDouble("Threshold", "Threshold", "Threshold", 2);
    strategy.parameters:addInteger("ClosePeriod", "Close Period", "Close Period", 10);
...
function CreateTradingParameters()
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "VRS");
...
function Prepare( nameOnly)
	...
	ClosePeriod= instance.parameters.ClosePeriod;
	...
	assert(core.indicators:findIndicator("VR") ~= nil, "Please, download and install VR.LUA indicator");          
    ...
    Indicator = core.indicators:create("VR", Source, Frame  );
	...
	Candle= core.getcandle (TF, core.host:execute("getServerTime"), core.host:execute ("getTradingDayOffset"), core.host:execute ("getTradingWeekOffset"))
...
function EntryFunction( now,period)
    ...
    local Return=false;
      ...
      return Return;
	   ...
	   if Indicator.DATA[period]< Threshold
	  then  
      return Return;
      end
...
function exitSpecific(BuySell,period)
...
-- closes all positions of the specified direction (B for buy, S for sell)
    ...
    while true do
        ...
        if row.AccountID == Account and
		   ...
		   ((((row.Time- Source:date(period))/Candle)>=ExitTime )or period==0)
		   then
