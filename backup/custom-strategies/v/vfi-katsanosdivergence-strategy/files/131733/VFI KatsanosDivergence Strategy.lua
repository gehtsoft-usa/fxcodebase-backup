...
local STRATEGY_NAME = "VFI Katsanos Divergence Strategy";
function CreateParameters() 
    strategy.parameters:addInteger("Period", "Period", "Period", 130);
    strategy.parameters:addDouble("Coef", "Coef", "Coef", 0.2);
    strategy.parameters:addDouble("VCoef", "VCoef", "VCoef", 2.5);
    strategy.parameters:addInteger("SmoothingPeriods", "SmoothingPeriods", "SmoothingPeriods", 3);
end

local vfi, div, length;
function CreateEntryIndicators(source)
    length = instance.parameters.Period;
    local profile = core.indicators:findIndicator("VFI");
    assert(profile ~= nil, "Please, download and install " .. "VFI" .. ".LUA indicator");
    local indicatorParams = profile:parameters();
    vfi = core.indicators:create("VFI", source, instance.parameters.Period, instance.parameters.Coef, instance.parameters.VCoef, instance.parameters.SmoothingPeriods)

    local profile = core.indicators:findIndicator("KATSANOSDIVERGENCE");
    assert(profile ~= nil, "Please, download and install " .. "KATSANOSDIVERGENCE" .. ".LUA indicator");
    local indicatorParams = profile:parameters();
    div = core.indicators:create("KATSANOSDIVERGENCE", vfi.DATA, instance.parameters.Period);
end

function UpdateIndicators()
    vfi:update(core.UpdateLast);
    div:update(core.UpdateLast);
end

function IsEntryLong(source, period)
    if period < length or not div.DATA:hasData(period - 1) then
        return false;
    end
    local slope = mathex.lregSlope(vfi.DATA, period - length + 1, period);
    return core.crossesOver(div.DATA, 0, period) and slope > 0;
end
function IsEntryShort(source, period)
    ...
    local slope = mathex.lregSlope(vfi.DATA, period - length + 1, period);
    return core.crossesUnder(div.DATA, 0, period) and slope < 0;
end
