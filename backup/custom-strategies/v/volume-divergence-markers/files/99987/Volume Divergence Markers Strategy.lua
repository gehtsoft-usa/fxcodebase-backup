...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=62138&p
...
function Init() --The strategy profile initialization
    strategy:name("Volume Divergence Markers Strategy")
    ...
    strategy.parameters:addInteger("Volume", "Continuous Volume", "Continuous Volume", 2)
    strategy.parameters:addInteger("Price", "Continuous Price", "Continuous Price", 2)
    strategy.parameters:addBoolean("Live", "Live", "Live", false)
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if CalculateVolume(period, dir) == false then
        if Direction then
            BUY()
        else
            SELL()
    ...
    elseif CalculateVolume(period, dir) == true then
        if Direction then
            SELL()
        else
            BUY()
...
function CalculateVolume(X, dir)
    ...
    if (counter1 == Volume) then
        return true
    elseif (counter2 == Volume) then
        return false
    else
        return nil
    end
