...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=63961
...
function Init() --The strategy profile initialization
    strategy:name("HA EXTREME STRATEGY")
    ...
    strategy.parameters:addBoolean("Reverse", "On Reverse only", "", false)
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if HA.close[period] > HA.open[period] and (not Reverse or (Reverse and HA.close[period - 1] <= HA.open[period - 1])) then
        if Direction then
            BUY()
        else
            SELL()
    ...
    elseif
        HA.close[period] < HA.open[period] and
            (not Reverse or (Reverse and HA.close[period - 1] >= HA.open[period - 1]))
     then
        if Direction then
            SELL()
        else
