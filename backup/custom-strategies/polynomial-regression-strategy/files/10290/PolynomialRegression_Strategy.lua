...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=4105
...
function Init() --The strategy profile initialization
    strategy:name("Polynomial regression strategy")
    strategy:description("Polynomial regression strategy")
    strategy:setTag("NonOptimizableParameters", "Email,SendEmail,SoundFile,RecurrentSound,PlaySound, ShowAlert")

    strategy.parameters:addGroup("Polynomial regression parameters")
    strategy.parameters:addInteger("Period", "Period", "", 50)
    strategy.parameters:addInteger("Power", "Power", "", 2, 1, 9)
    strategy.parameters:addDouble("Deviation", "Deviation", "", 1)
    ...
    strategy.parameters:addDouble("Dist", "Distance", "Distance", 10)
...
function Prepare(nameOnly)
                        ...
                        "," ..
                            "PR(" ..
                                instance.parameters.Period ..
                                    "," .. instance.parameters.Power .. "," .. instance.parameters.Deviation .. "))"
    instance:name(name)
    if nameOnly then
        return
    end
    ...
    assert(
        core.indicators:findIndicator("POLYNOMIAL_REGRESSION") ~= nil,
        "Please, download and install POLYNOMIAL_REGRESSION.LUA indicator"
    )
    PR =
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    PR:update(core.UpdateLast)

    -- Check that we have enough data
    if (PR.DATA:first() > (period - 1)) then
        return
    end
    ...
    if
        (IndSource[period - 2] < PR.BuffBandUp[period - 2] + instance.parameters.Dist * source:pipSize() and
            IndSource[period - 1] >= PR.BuffBandUp[period - 1] + instance.parameters.Dist * source:pipSize())
     then
    ...
    if
        (IndSource[period - 2] > PR.BuffBandDn[period - 2] - instance.parameters.Dist * source:pipSize() and
            IndSource[period - 1] <= PR.BuffBandDn[period - 1] - instance.parameters.Dist * source:pipSize())
     then
        if (instance.parameters.TypeSignal == "direct") then
            MustB = true
        else
