...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=63734
...
function Init() --The strategy profile initialization
    strategy:name("Consecutive candle Count Strategy")
    ...
    strategy.parameters:addBoolean("Extend", "Ignore Zero Body Candles", "", false)

    strategy.parameters:addBoolean("Exit", "Use Optional Exit", "", true)
    strategy.parameters:addInteger("ExitPeriod", "Exit Period", "Period", 1)
...
function Prepare(nameOnly)
    ...
    assert(
        ...
        "Please, download and installCONSECUTIVE CANDLE COUNT.LUA indicator"
    )
    ...
    Indicator = core.indicators:create("CONSECUTIVE CANDLE COUNT", Source, Period, Extend)
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if Indicator.DATA[period] == Period and Indicator.DATA[period - 1] ~= Period then
        if Direction then
            BUY()
        else
            SELL()
    ...
    elseif Indicator.DATA[period] == -Period and Indicator.DATA[period - 1] ~= -Period then
        if Direction then
            SELL()
        else
            BUY()
    ...
    if Exit then
        if Indicator.DATA[period] <= ExitPeriod then
            if Direction then
