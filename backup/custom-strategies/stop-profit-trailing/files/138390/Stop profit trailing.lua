...
--http://fxcodebase.com/code/viewtopic.php?f=31&t=70554
...
function Init()
    strategy:name("Stop profit trailing");
    ...
    strategy.parameters:addDouble("profit_trigger", "Profit trigger, pips", "", 10);
    strategy.parameters:addDouble("profit_target", "Profit target, pips", "", 5);
    strategy.parameters:addDouble("loss_trigger", "Loss trigger, pips", "", 10);
    strategy.parameters:addDouble("loss_target", "Loss target, pips", "", 5);
end

local profit_trigger, profit_target, loss_trigger, loss_target
function Prepare(nameOnly)
    ...
    loss_target = instance.parameters.loss_target;
    loss_trigger = instance.parameters.loss_trigger;
    profit_target = instance.parameters.profit_target;
    profit_trigger = instance.parameters.profit_trigger;
    core.host:execute("subscribeTradeEvents", 1000, "Trades");
    main_source = ExtSubscribe(1, nil, "t1", false);
end
    ...
    if cookie == 1000 then
        ...
        if not close_trade then
            ...
            function controller:DoBreakeven()
                ...
                if trade.PL >= profit_trigger then
                    if trade.BS == "B" then
                        self._command = self._parent._trading:MoveStop(trade, offer.Bid - (trade.PL - profit_target) * offer.PointSize);
                    else
                        self._command = self._parent._trading:MoveStop(trade, offer.Ask + (trade.PL -  profit_target) * offer.PointSize);
                    end
                    
                    self._executed = true;
                    return false;
                end
                if trade.PL <= -loss_trigger then
                    core.host:trace("moving limit");
                    if trade.BS == "B" then
                        self._command = self._parent._trading:MoveLimit(trade, offer.Bid - (trade.PL + loss_target) * offer.PointSize);
                    else
                        self._command = self._parent._trading:MoveLimit(trade, offer.Ask + (trade.PL + loss_target) * offer.PointSize);
                    end
