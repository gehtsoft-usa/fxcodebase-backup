...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if
        period1 < Source[1].close:first(Source[1].close:size() - 1) or
            period2 < Source[2].close:first(Source[2].close:size() - 1) or
            period3 < Source[3].close:first(Source[3].close:size() - 1) or
            period4 < Source[4].close:first(Source[4].close:size() - 1)
     then
        return
    end
...
function EntryFunction(now, period1, period2, period3, period4)
    ...
    if
        not Indicator1[1].DATA:hasData(period1) or not Indicator2[1].DATA:hasData(period1) or
            not Indicator3[1].DATA:hasData(period1)
     then
        return
    end

    if
        not Indicator1[2].DATA:hasData(period2) or not Indicator2[2].DATA:hasData(period2) or
            not Indicator3[2].DATA:hasData(period2)
     then
        return
    end

    if
        not Indicator1[3].DATA:hasData(period3) or not Indicator2[3].DATA:hasData(period3) or
            not Indicator3[3].DATA:hasData(period3)
     then
        return
    end

    if
        not Indicator1[4].DATA:hasData(period4) or not Indicator2[4].DATA:hasData(period4) or
            not Indicator3[4].DATA:hasData(period4)
     then
        return
    end
    ...
    if
        Indicator1[1].DATA[period1] > Indicator2[1].DATA[period1] and
        Indicator2[1].DATA[period1] > Indicator3[1].DATA[period1] and
        Indicator1[1].DATA[period1 - 1] <= Indicator2[1].DATA[period1 - 1] and
        Indicator1[2].DATA[period2] > Indicator2[2].DATA[period2] and
        Indicator2[2].DATA[period2] > Indicator3[2].DATA[period2] and
        Indicator1[3].DATA[period3] > Indicator2[3].DATA[period3] and
        Indicator2[3].DATA[period3] > Indicator3[3].DATA[period3] and
        Indicator1[4].DATA[period4] > Indicator2[4].DATA[period4] and
        Indicator2[4].DATA[period4] > Indicator3[4].DATA[period4] 
        and last_signal ~= "B"
    then
        if Direction then
            BUY()
