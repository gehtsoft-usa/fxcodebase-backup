...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=63891
...
function Init() --The strategy profile initialization
    strategy:name("Two Line SuperTrend Strategy")
    ...
    strategy.parameters:addBoolean("Trend", "Use Trend Filter", "", true)
    strategy.parameters:addBoolean("Change", "Only on subtrend change", "", true)
    ...
    strategy.parameters:addInteger("N1", "Number of periods", "", 7)
    strategy.parameters:addDouble("M1", "Multiplier", "", 2.5)
    ...
    strategy.parameters:addInteger("N2", "Number of periods", "", 7)
    strategy.parameters:addDouble("M2", "Multiplier", "", 2.5)
...
function CreateTradingParameters()
    ...
    strategy.parameters:addInteger("MaxNumberOfPosition", "Max Number Of Position In One Direction", "", 1, 1, 10000)
...
function Prepare(nameOnly)
    ...
    Indicator[1] = core.indicators:create("SUPERTREND", Source1, N1, M1, core.rgb(0, 255, 0), core.rgb(255, 0, 0))
    Indicator[2] = core.indicators:create("SUPERTREND", Source2, N2, M2, core.rgb(0, 255, 0), core.rgb(255, 0, 0))
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if not Indicator[1].DATA:hasData(period1) or not Indicator[2].DATA:hasData(period2) then
        return
    end

    -- only buy if we have a fast cross over slow and the price is above the moving averages.
    if
        Indicator[1].DATA:colorI(period1) == core.rgb(0, 255, 0) and
            (not Change or (Change and Indicator[1].DATA:colorI(period1 - 1) ~= core.rgb(0, 255, 0))) and
            (not Trend or (Trend and Indicator[2].DATA:colorI(period2) == core.rgb(0, 255, 0)))
     then
        if Direction then
            BUY()
        else
    ...
    elseif
        Indicator[1].DATA:colorI(period1) == core.rgb(255, 0, 0) and
            (not Change or (Change and Indicator[1].DATA:colorI(period1 - 1) ~= core.rgb(255, 0, 0))) and
            (not Trend or (Trend and Indicator[2].DATA:colorI(period2) == core.rgb(255, 0, 0)))
     then
        if Direction then
            SELL()
        else
    ...
    if Exit then
        if
            Indicator[1].DATA:colorI(period1) ~= core.rgb(0, 255, 0) or
                Indicator[2].DATA:colorI(period2) ~= core.rgb(0, 255, 0)
         then
            if Direction then
        ...
        if
            Indicator[1].DATA:colorI(period1) ~= core.rgb(255, 0, 0) or
                Indicator[2].DATA:colorI(period2) ~= core.rgb(255, 0, 0)
         then
            if Direction then
                if haveTrades("S") then
                    exitSpecific("S")
