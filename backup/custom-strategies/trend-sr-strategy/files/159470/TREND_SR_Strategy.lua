-- Available @ https://fxcodebase.com/code/viewtopic.php?f=31&t=75996
...
local STRATEGY_NAME = "TREND SR Strategy";
function CreateParameters() 
    --create your algorithm strategy.parameters here
    strategy.parameters:addInteger("period", "Period", "", 3)
    strategy.parameters:addInteger("ht_frame", "HT Frame", "", 1);
end

local indi;
function CreateEntryIndicators(source)
    local profile = core.indicators:findIndicator("TREND SR.LUA");
    assert(profile ~= nil, "Please, download and install " .. "TREND SR.LUA" .. ".LUA indicator");
    indi = core.indicators:create("TREND SR.LUA", source, 0, 0, 0, 0, 0, instance.parameters.period, instance.parameters.ht_frame)
end
...
function IsEntryLong(source, period)
    return core.crossesOver(source, indi.LL, period);
end
function IsEntryShort(source, period)
    return core.crossesUnder(source, indi.UL, period);
end
