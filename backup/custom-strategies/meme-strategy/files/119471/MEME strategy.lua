...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=66174
-- Id: 
...
local Modules = {};
function Init()
    strategy:name("Multiple Entry/Multiple Exit(MEME) Strategy");
    strategy:description("");

    strategy.parameters:addString("Side", "Side", "Side for trading or signaling, can be Sell, Buy", "B");
    strategy.parameters:addStringAlternative("Side", "Buy", "", "B");
    strategy.parameters:addStringAlternative("Side", "Sell", "", "S");
    strategy.parameters:addDouble("distance", "Pip distance between levels", "", 25)

    for i = 1, 5 do
        strategy.parameters:addGroup("Trade #" .. i);
        if i == 3 then
            strategy.parameters:addDouble("rate_" .. i, "Anchor level", "", 1);
            strategy.parameters:setFlag("rate_" .. i, core.FLAG_PRICE);
        end
        strategy.parameters:addBoolean("open_trade_" .. i, "Open trade", "", false)
        strategy.parameters:addInteger("amount_" .. i, "Trade Amount in Lots", "", 1);
        strategy.parameters:addBoolean("set_stop_" .. i, "Set Stop", "", false);
        strategy.parameters:addInteger("stop_" .. i, "Stop, in pips", "", 30);
        strategy.parameters:addBoolean("use_trailing_" .. i, "Trailing stop order", "", false);
        strategy.parameters:addInteger("trailing_" .. i, "Trailing, in pips", "Use 1 for dynamic and 10 or greater for the fixed trailing", 1);
        strategy.parameters:addBoolean("set_limit_" .. i, "Set Limit", "", false);
        strategy.parameters:addInteger("limit_" .. i, "Limit, in pips", "", 30);
    end
...
function Prepare(name_only)
    ...
    local distance = instance.parameters.distance;
    
    for i = 1, 5 do
        local trade = {};
        trade.used = instance.parameters:getBoolean("open_trade_" .. i);
    ...
    trades[3].rate = instance.parameters:getDouble("rate_3");
    trades[2].rate = trades[3].rate + distance * instance.ask:pipSize();
    trades[1].rate = trades[2].rate + distance * instance.ask:pipSize();
    trades[4].rate = trades[3].rate - distance * instance.ask:pipSize();
    trades[5].rate = trades[4].rate - distance * instance.ask:pipSize();
...
function ExtUpdate(id, source, period)
    ...
    for _, t in ipairs(trades) do
        if t.used then
            local order = trading:EntryOrder(instance.ask:instrument())
                :SetSide(side)
                :SetAmount(t.amount)
                :SetRate(t.rate);
            if t.set_stop then
                order = order:SetPipStop(nil, t.stop, t.use_trailing and t.trailing or nil);
            end
            if t.set_limit then
                order = order:SetPipLimit(nil, t.limit);
            end
            order:Execute();
        end
    end
...
function trading_logic:Prepare(name_only)
    ...
    ExtSubscribe(self._trading_source_id, nil, "t1", true, "close");
    ...
    if self._trading_source_id ~= self._exit_source_id then
        ExtSubscribe(self._exit_source_id, nil, "t1", true, "close");
    end
    self:trace("Exit on tick (live data)");
    self.MainSource = ExtSubscribe(self._ids_start + 2, nil, "m1", true, "bar");
end
