...
local STRATEGY_NAME = "Zigzag Retracement";
...
local retrace_indi;
local breakout_indi;
local stop_limit_indi;
local retrace_source;
local stop_limit_source;

function CreateStopParameters(params, id) 
    params:addDouble("stop" .. id, "Stop Value, %", "", 25);
    return true;
end

function CreateLimitParameters(params, id) 
    params:addDouble("limit" .. id, "Limit Value, %", "", 30);
    return true;
end

function SaveCustomStopParameters(position_desc, id) 
    position_desc.Stop = instance.parameters:getDouble("stop" .. id);
end

function SaveCustomLimitParameters(position_desc, id) 
    position_desc.Limit = instance.parameters:getDouble("limit" .. id);
end

function SetCustomStop(position_desc, command, period)
    local crest_period, value = GetNextCrest(stop_limit_indi, period);
    local move = math.abs(position_desc.Source.close[period] - value) / position_desc.Source:pipSize();
    command:SetPipStop(nil, move * position_desc.Stop / 100);
    return true; 
end
...
function SetCustomLimit(position_desc, command, period)
    local crest_period, value = GetNextCrest(stop_limit_indi, period);
    local move = math.abs(position_desc.Source.close[period] - value) / position_desc.Source:pipSize();
    command:SetPipLimit(nil, move * position_desc.Limit / 100);
    return true; 
end

function CreateParameters() 
    strategy.parameters:addString("retrace_tf", "Retrace timeframe", "", "m1");
    strategy.parameters:addString("breakout_tf", "Breakout timeframe", "", "m1");
    strategy.parameters:addString("stop_limit_tf", "Stop/limit timeframe", "", "m1");

    strategy.parameters:addInteger("ZigZagDepth", "ZigZagDepth", "See ZIGZAG", 4);
    strategy.parameters:addInteger("ZigZagDeviation", "ZigZagDeviation", "See ZIGZAG", 5);
    strategy.parameters:addInteger("ZigZagBackstep", "ZigZagBackstep", "See ZIGZAG", 3);
    strategy.parameters:addInteger("retrace_level", "Retracement level, %", "", 45);

    strategy.parameters:addInteger("Frame", "AFBSR Number of fractals (Odd)", "Number of fractals (Odd)", 5, 5, 99)
end

function CreateIndicators(source)
    coeff = instance.parameters.retrace_level;
    retrace_source = trading_logic:SubscribeHistory(source.close:instrument(), instance.parameters.retrace_tf, source:isBid());
    breakout_source = trading_logic:SubscribeHistory(source.close:instrument(), instance.parameters.breakout_tf, source:isBid());
    stop_limit_source = trading_logic:SubscribeHistory(source.close:instrument(), instance.parameters.stop_limit_tf, source:isBid());
    retrace_indi = core.indicators:create("ZIGZAG", retrace_source, instance.parameters.ZigZagDepth, instance.parameters.ZigZagDeviation, instance.parameters.ZigZagBackstep);
    stop_limit_indi = core.indicators:create("ZIGZAG", stop_limit_source, instance.parameters.ZigZagDepth, instance.parameters.ZigZagDeviation, instance.parameters.ZigZagBackstep);
    local indi_profile = EnsureIndicatorInstalled("AFBSR BREAKOUT ALERT");
    local p = indi_profile:parameters();
    p:setInteger("Frame", instance.parameters.Frame)
    p:setBoolean("strategy_output", true)
    breakout_indi = core.indicators:create("AFBSR BREAKOUT ALERT", breakout_source, p);
end

function UpdateIndicators()
    retrace_indi:update(core.UpdateLast);
    breakout_indi:update(core.UpdateLast);
    stop_limit_indi:update(core.UpdateLast);
end

function GetNextCrest(indi, period)
    if not indi.DATA:hasData(period) then
        while period > 0 and not indi.DATA:hasData(period) do
            period = period - 1;
        end
        return period, indi.DATA[period];
    end
    local isascending = indi.DATA[period - 1] < indi.DATA[period];
    while period > 0 and indi.DATA[period - 1] < indi.DATA[period] == isascending do
        period = period - 1;
    end
    return period, indi.DATA[period];
end

function IsRetraceUp(indi, period)
    local last_period = GetNextCrest(indi, period);
    local last = indi.DATA[last_period];
    local d_period = GetNextCrest(indi, last_period);
    local d = indi.DATA[d_period];
    if last > d then
        return false;
    end
    local c_period = GetNextCrest(indi, d_period);
    local c = indi.DATA[c_period];
    local b_period = GetNextCrest(indi, c_period);
    local b = indi.DATA[b_period];
    local a_period = GetNextCrest(indi, b_period);
    local a = indi.DATA[a_period];
    return d > b and c > a and last < (d - (d - c) * coeff / 100);
end

function IsRetraceDown(indi, period)
    local last_period = GetNextCrest(indi, period);
    local last = indi.DATA[last_period];
    local d_period = GetNextCrest(indi, last_period);
    local d = indi.DATA[d_period];
    ...
    local a_period = GetNextCrest(indi, b_period);
    local a = indi.DATA[a_period];
    return d < b and c < a and last > (d + (c - d) * coeff / 100);
end
...
function CreateCustomActions()
    ...
    enterLongAction.IsPass = function (source, period, periodFromLast)
        return IsRetraceUp(retrace_indi, retrace_source:size() - 1 - periodFromLast)
            and breakout_indi.OUTPUT[breakout_source:size() - 1 - periodFromLast] == 1;
    end

    local enterShortAction = {};
    enterShortAction.ActOnSwitch = true;
    enterShortAction.IsPass = function (source, period, periodFromLast)
        return IsRetraceDown(retrace_indi, retrace_source:size() - 1 - periodFromLast)
            and breakout_indi.OUTPUT[breakout_source:size() - 1 - periodFromLast] == -1;
    end
