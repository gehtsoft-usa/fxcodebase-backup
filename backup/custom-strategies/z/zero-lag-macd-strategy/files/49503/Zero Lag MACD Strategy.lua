...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=28156&p
--+------------------------------------------------------------------+
...
function Init() --The strategy profile initialization
    strategy:name("Zero Lag MACD Strategy")
    ...
    strategy.parameters:addInteger("Short", "Short Period MA", "", 12)
    strategy.parameters:addInteger("Long", "Long Period MA", "", 24)
    strategy.parameters:addInteger("Signal", "Signal Period", "", 9)
...
function Prepare(nameOnly)
    ...
    ShortPeriod = instance.parameters.Short
    LongPeriod = instance.parameters.Long
    SignalPeriod = instance.parameters.Signal
    ...
    name = name .. ", " .. ShortPeriod .. ", " .. LongPeriod .. ", " .. SignalPeriod

    assert(
        core.indicators:findIndicator("ZEROLAGMACD") ~= nil,
        "Please, download and install ZeroLagMACD.LUA indicator"
    )
    ...
    Source = ExtSubscribe1(1, nil, instance.parameters.TF, 1000, instance.parameters.Type == "Bid", "bar");

    Indicator[1] = core.indicators:create("ZEROLAGMACD", Source[Price], ShortPeriod, LongPeriod, SignalPeriod)
    ShortName["MACD"] = Indicator[1]:getStream(0)
    ShortName["SIG"] = Indicator[1]:getStream(1)
    ShortName["HISTOGRAM"] = Indicator[1]:getStream(2)

    first = ShortName["HISTOGRAM"]:first()
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
	...
	core.host:trace("MACD : " .. ShortName["MACD"][period]);
    core.host:trace("SIG : " .. ShortName["SIG"][period]);
    core.host:trace("HISTOGRAM : " .. ShortName["HISTOGRAM"][period]);

    if period < first + 1 then
        return
    end

    if core.crossesOver(ShortName["MACD"], ShortName["SIG"], period) then
        if Direction then
            BUY()
        else
            SELL()
        end
    elseif core.crossesUnder(ShortName["MACD"], ShortName["SIG"], period) then
        if Direction then
            SELL()
        else
            BUY()
    ...
    if Exit then
        if ShortName["HISTOGRAM"][period] < ShortName["HISTOGRAM"][period - 1] then
            if Direction then
