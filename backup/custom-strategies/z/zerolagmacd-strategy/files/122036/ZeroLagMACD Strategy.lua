...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=66908
...
function Init() --The strategy profile initialization
    strategy:name("ZeroLagMACD Strategy");
    ...
    strategy.parameters:addInteger("FMA", "Fast EMA periods", "", 12, 1, 5000);
    strategy.parameters:addInteger("SMA", "Slow EMA Periods", "", 24, 1, 5000);
    strategy.parameters:addInteger("SigMA", "Signal EMA periods", "", 9, 1, 5000);
...
function CreateTradingParameters()
  ...
  --  strategy.parameters:addStringAlternative("ExitExecutionType", "End of Turn", "", "EndOfTurn");
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "ZLMACDS");
...
function Prepare( nameOnly)
    ...
    Indicator = core.indicators:create("ZEROLAGMACD", Source, FMA, SMA, SigMA );
	 
	first=Indicator.SIG:first(); 
...
function EntryFunction( now,period)
    ...
    if  Indicator.MACD[period]> Indicator.SIG[period]
	and  Indicator.MACD[period-1]<= Indicator.SIG[period-1]
	then	
          if Direction then
                BUY();
            else
		...
		Return=true;
    elseif  Indicator.MACD[period]< Indicator.SIG[period]
	and  Indicator.MACD[period-1]>= Indicator.SIG[period-1]
	then
            if Direction then
                SELL();
