...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=3703
...
function Prepare(nameOnly)
    ...
    if PositionsCount == 1 then
        buy_positions[#buy_positions + 1] = CreatePositionStrategy(gSource, "B", "");
        sell_positions[#sell_positions + 1] = CreatePositionStrategy(gSource, "S", "");
    else
        for i = 1, PositionsCount do
            if instance.parameters:getBoolean("use_position_" .. i) then
                buy_positions[#buy_positions + 1] = CreatePositionStrategy(gSource, "B", "_" .. i);
                sell_positions[#sell_positions + 1] = CreatePositionStrategy(gSource, "S", "_" .. i);
            end
        end
    end
end
...
function ExtUpdate(id, source, period)
    ...
    if ZZ1 ~= nil then
        ZZ1:update(core.UpdateNew)
    end
    if ZZ2 ~= nil then
        ZZ2:update(core.UpdateNew)
    end
    if ZZ3 ~= nil then
        ZZ3:update(core.UpdateNew)
    end
    ...
    local signal = ""
    if Use1 then
        signal = signal .. tostring(up1)
    end
    if Use2 then
        signal = signal .. tostring(up2)
    end
    if Use3 then
        signal = signal .. tostring(up3)
    end

    if signal ~= prevSignal then
        if (not Use1 or (Use1 and up1)) and (not Use2 or (Use2 and up2)) and (not Use3 or (Use3 and up3)) then
            trading:FindTrade()
                :WhenSide("B")
        ...
        elseif (not Use1 or (Use1 and not up1)) and (not Use2 or (Use2 and not up2)) and (not Use3 or (Use3 and not up3)) then
            trading:FindTrade()
                :WhenSide("S")
                :WhenCustomID(custom_id)
                :Do(function (trade) trading:Close(trade); end);
