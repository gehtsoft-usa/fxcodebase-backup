...
function Init() --The strategy profile initialization
    strategy:name("Zscore Strategy with selectable Time Frame")
    ...
    strategy.parameters:addString("TF", "Strategy Time frame", "", "H1")
    strategy.parameters:setFlag("TF", core.FLAG_PERIODS)

    strategy.parameters:addGroup("Strategy Indicator Parameters")

    strategy.parameters:addString("ITF", "Indicator Time frame", "", "H1")
    strategy.parameters:setFlag("ITF", core.FLAG_PERIODS)
...
function Prepare(nameOnly)
    ...
    s2, e2 = core.getcandle(ITF, core.now(), 0, 0)

    assert(not ((e1 - s1) > (e2 - s2)), "Indicator Time frame should be greater than or equal to Strategy time frame.")

    if nameOnly then
        return
    end

    Source = ExtSubscribe(1, nil, TF, instance.parameters.Type == "Bid", "close")
    IndicatorSource = ExtSubscribe(2, nil, ITF, instance.parameters.Type == "Bid", "close")
    zscore = core.indicators:create("ZSCORE", IndicatorSource, Period)
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    period = core.findDate(IndicatorSource, Source:date(period), false)
