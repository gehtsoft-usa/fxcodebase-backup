...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=68420
...
local STRATEGY_NAME = "IVFT Strategy";
...
function CreateParameters()
    ...
    strategy.parameters:addDouble("level_1_val", "Level 1", "", 0.5);
    AddAction(1, "Level 1 Cross Over");
    AddAction(2, "Level 1 Cross Under");
    strategy.parameters:addDouble("level_2_val", "Level 2", "", 0);
    AddAction(3, "Level 2 Cross Over");
    AddAction(4, "Level 2 Cross Under");
    strategy.parameters:addDouble("level_3_val", "Level 3", "", -0.5);
    AddAction(5, "Level 3 Cross Over");
    AddAction(6, "Level 3 Cross Under");

    strategy.parameters:addString("ivft_tf", "Timeframe for IVFT", "", "m5");
    strategy.parameters:setFlag("ivft_tf", core.FLAG_PERIODS);
    strategy.parameters:addString("tsi_tf", "Timeframe for TSI", "", "m5");
    strategy.parameters:setFlag("tsi_tf", core.FLAG_PERIODS);
    strategy.parameters:addBoolean("use_tsi", "Use TSI", "", true);
...
local indi;
local level_1_val, level_2_val, level_3_val, tsi;
function CreateIndicators(source)
    EnsureIndicatorInstalled("IVFT");
    ivft_source = trading_logic:SubscribeHistory(source:instrument(), instance.parameters.ivft_tf, source:isBid());
    indi = core.indicators:create("IVFT", ivft_source, instance.parameters.Period);
    level_1_val = instance.parameters.level_1_val;
    level_2_val = instance.parameters.level_2_val;
    level_3_val = instance.parameters.level_3_val;
    if instance.parameters.use_tsi then
        tsi_source = trading_logic:SubscribeHistory(source:instrument(), instance.parameters.tsi_tf, source:isBid());
...
function CreateCustomActions()
    ...
    action1.IsPass = function (source, period, periodFromLast, data) 
        if not indi.DATA:hasData(NOW - 1) then
            return;
        end
        if tsi ~= nil then
            if not tsi.DATA:hasData(NOW) then
                return;
            end
            if tsi.DATA[NOW] <= 0 then
                return false;
            end
        end
        return core.crossesOver(indi.DATA, level_1_val, NOW); 
    end
    if isEntry1 then
    ...
    action1, isEntry1 = CreateAction(2);
    action1.Data = nil;
    action1.IsPass = function (source, period, periodFromLast, data) 
        if not indi.DATA:hasData(NOW - 1) then
            return;
        ...
        return core.crossesUnder(indi.DATA, level_1_val, NOW);
    end
    if isEntry1 then
        EntryActions[#EntryActions + 1] = action1;
    else
    ...
    local action1, isEntry1 = CreateAction(3);
    action1.Data = nil;
    action1.IsPass = function (source, period, periodFromLast, data) 
        if not indi.DATA:hasData(NOW - 1) then
            return;
        ...
        return core.crossesOver(indi.DATA, level_2_val, NOW); 
    end
    if isEntry1 then
        EntryActions[#EntryActions + 1] = action1;
    else
    ...
    local action1, isEntry1 = CreateAction(4);
    action1.Data = nil;
    action1.IsPass = function (source, period, periodFromLast, data) 
        if not indi.DATA:hasData(NOW - 1) then
            return;
        ...
        return core.crossesUnder(indi.DATA, level_2_val, NOW); 
    end
    if isEntry1 then
        EntryActions[#EntryActions + 1] = action1;
    else
    ...
    local action1, isEntry1 = CreateAction(5);
    action1.Data = nil;
    action1.IsPass = function (source, period, periodFromLast, data) 
        if not indi.DATA:hasData(NOW - 1) then
            return;
        ...
        return core.crossesOver(indi.DATA, level_3_val, NOW);
    end
    if isEntry1 then
        EntryActions[#EntryActions + 1] = action1;
    else
    ...
    local action1, isEntry1 = CreateAction(6);
    action1.Data = nil;
    action1.IsPass = function (source, period, periodFromLast, data) 
        if not indi.DATA:hasData(NOW - 1) then
            return;
        ...
        return core.crossesUnder(indi.DATA, level_3_val, NOW);
    end
    if isEntry1 then
        EntryActions[#EntryActions + 1] = action1;
    else
