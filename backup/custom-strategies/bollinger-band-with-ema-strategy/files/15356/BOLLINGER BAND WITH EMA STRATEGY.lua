...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=6751
...
function Init() --The strategy profile initialization
    strategy:name("BOLLINGER BAND WITH EMA STRATEGY")
    ...
    strategy.parameters:addInteger("First", "First Period", "", 34)
    strategy.parameters:addInteger("Second", "Second Period", "", 70)
    strategy.parameters:addInteger("Third", "Third Period", "", 106)
    ...
    strategy.parameters:addBoolean("EXIT", "Exit on Central Line Cross ", "", true)
...
function Prepare(nameOnly)
    ...
    Period["1"] = instance.parameters.First
    Period["2"] = instance.parameters.Second
    Period["3"] = instance.parameters.Third
            ...
            instance.bid:name() ..
                "." ..
                    instance.parameters.TF ..
                        ", " ..
                            Period["1"] .. ", " .. Period["2"] .. ", " .. Period["3"] .. ", " .. N .. ", " .. DEV .. ")"
    instance:name(name)
    ...
    EMA["1"] = core.indicators:create("EMA", Source[Price], Period["1"])
    EMA["2"] = core.indicators:create("EMA", Source[Price], Period["2"])
    EMA["3"] = core.indicators:create("EMA", Source[Price], Period["3"])
    BB = core.indicators:create("BB", Source[Price], N, DEV)

    first = math.max(EMA["1"].DATA:first(), EMA["2"].DATA:first(), EMA["3"].DATA:first(), BB.DATA:first())
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    EMA["1"]:update(core.UpdateLast)
    EMA["2"]:update(core.UpdateLast)
    EMA["3"]:update(core.UpdateLast)
    BB:update(core.UpdateLast)

    -- buy conditions:
    --1. price> ema 34 [and]
    --2.ema34>ema70>ema105[and]
    --3.price cross above downwards the upper line of bollinger band (20,2.0)

    --sell conditions:
    --1 price <ema 34 [and]
    --2.ema 34<ema 70 <ema105 [and]
    --3.price cross below upwards the lower line of bollinger band (20,2.0)

    if
        Source[Price][period] > EMA["1"].DATA[period] and EMA["1"].DATA[period] > EMA["2"].DATA[period] and
            EMA["2"].DATA[period] > EMA["3"].DATA[period] and
            core.crossesOver(Source[Price], BB.AL, period)
     then
    ...
    elseif
        Source[Price][period] < EMA["1"].DATA[period] and EMA["1"].DATA[period] < EMA["2"].DATA[period] and
            EMA["2"].DATA[period] < EMA["3"].DATA[period] and
            core.crossesUnder(Source[Price], BB.AL, period)
     then
        if Direction == "direct" then
            SELL()
        else
    ...
    if EXIT then
        if core.crossesUnder(Source[Price], BB.AL, period) then
            if haveTrades("B") then
                exit("B")
                Signal("Close Long")
            end
        elseif core.crossesOver(Source[Price], BB.AL, period) then
