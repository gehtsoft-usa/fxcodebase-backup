...
--http://fxcodebase.com/code/viewtopic.php?f=31&t=69468
...
local STRATEGY_NAME = "Trend Cont Factor Strategy";
function CreateParameters() 
    strategy.parameters:addInteger("Length", "Period", "", 35)
end

local Length, indi;
function CreateEntryIndicators(source)
    Length = instance.parameters.Length;
    local profile = core.indicators:findIndicator("TREND CONT FACTOR");
    assert(profile ~= nil, "Please, download and install " .. "TREND CONT FACTOR" .. ".LUA indicator");
    local indicatorParams = profile:parameters();
    --set parameters here indicatorParams:setBoolean("name", val);
    indi = core.indicators:create("TREND CONT FACTOR", source, instance.parameters.Length);
end
...
function IsEntryLong(source, period)
    if not indi.PlusTCF:hasData(period - 1) then
        return;
    end
    return core.crossesOver(indi.PlusTCF, 0, period);
end
function IsEntryShort(source, period)
    if not indi.MinusTCF:hasData(period - 1) then
        return;
    end
    return core.crossesOver(indi.MinusTCF, 0, period);
end
