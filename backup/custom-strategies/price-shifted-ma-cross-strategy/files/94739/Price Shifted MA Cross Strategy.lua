...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=60865
...
function Init() --The strategy profile initialization
    strategy:name("Price Shifted MA Cross Strategy");
    ...
    strategy.parameters:addString("Method", "Method", "Method" , "Pips");
    strategy.parameters:addStringAlternative("Method", "Pips", "Pips" , "Pips");
    strategy.parameters:addStringAlternative("Method", "Value", "Value" , "Value");
    ...
    strategy.parameters:addDouble("SY", "Shift in points", "", 0);
...
function Prepare( nameOnly)
    ...
    local iparams ;	
	           ...
	           iprofile = core.indicators:findIndicator(instance.parameters:getString("Indicator" ));
    ...
    assert(core.indicators:findIndicator("SHIFTED") ~= nil, "SHIFTED" .. " indicator must be installed");
    Shifted = core.indicators:create("SHIFTED", Indicator.DATA, SX,SY);
    first =  Shifted.DATA:first() + 1;
...
function ExtUpdate(id, source, period)  -- The method called every time when a new bid or ask price appears.
   ...
   -- get only time
    ...
    Shifted:update(core.UpdateLast);
    ...
    if    core.crossesOver(Source.close, Shifted.DATA, period)  then
        BUY();
		ONE= Source:serial(period);
    elseif   core.crossesUnder(Source.close, Shifted.DATA, period)  then
        SELL();
		ONE= Source:serial(period);
    end
