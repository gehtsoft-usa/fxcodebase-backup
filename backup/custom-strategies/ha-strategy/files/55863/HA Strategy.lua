...
function Init() --The strategy profile initialization
    strategy:name("Heiken Ashi Strategy")
...
function Prepare(nameOnly)
    ...
    Indicator[1] = core.indicators:create("HA", Source)
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if
        Indicator[1].open[period] < Indicator[1].close[period] and
            Indicator[1].open[period - 1] > Indicator[1].close[period - 1]
     then
        if Direction then
            BUY()
        else
