...
function Init() --The strategy profile initialization
    ...
    strategy.parameters:addGroup("1. MACD Calculation")
    strategy.parameters:addInteger("SN1", "Short EMA", "", 12, 2, 1000)
    strategy.parameters:addInteger("LN1", "Long EMA", "", 26, 2, 1000)

    strategy.parameters:addGroup("2. MACD Calculation")
    strategy.parameters:addInteger("SN2", "Short EMA", "", 21, 2, 1000)
    strategy.parameters:addInteger("LN2", "Long EMA", "", 34, 2, 1000)

    strategy.parameters:addGroup("3. MACD Calculation")
    strategy.parameters:addInteger("SN3", "Short EMA", "", 34, 2, 1000)
    strategy.parameters:addInteger("LN3", "Long EMA", "", 144, 2, 1000)

    strategy.parameters:addDouble("BL", "Buy Level", "", 0)
    strategy.parameters:addDouble("SL", "Sell Level", "", 0)

    strategy.parameters:addDouble("BEL", "Buy Exit Level", "", 0.005)
    strategy.parameters:addDouble("SEL", "Sell Exit Level", "", -0.005)
...
function Prepare(nameOnly)
    ...
    MACD[1] = core.indicators:create("MACD", Source.close, instance.parameters.SN1, instance.parameters.LN1)
    MACD[2] = core.indicators:create("MACD", Source.close, instance.parameters.SN2, instance.parameters.LN2)
    MACD[3] = core.indicators:create("MACD", Source.close, instance.parameters.SN3, instance.parameters.LN3)

    first = math.max(MACD[1].DATA:first(), MACD[2].DATA:first(), MACD[3].DATA:first())
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
	...
	buy: macd3> buy level 
	and macd3< buy exit level
	and macd2> buy level and macd1 cross over macd 2
	sell: macd3< sell level and macd3> sell exit level and macd2<sell level and macd1 cross under macd2
   ]]
    -- update indicators.
    MACD[1]:update(core.UpdateLast)
    MACD[2]:update(core.UpdateLast)
    MACD[3]:update(core.UpdateLast)
    ...
    if
        MACD[3].DATA[period] > BL and MACD[3].DATA[period] < BEL and MACD[2].DATA[period] > BL and
            MACD[1].DATA[period] > MACD[2].DATA[period] and
            MACD[1].DATA[period - 1] <= MACD[2].DATA[period - 1]
     then
        if Direction then
            BUY()
        else
    ...
    elseif
        MACD[3].DATA[period] < SL and MACD[3].DATA[period] > SEL and MACD[2].DATA[period] < SL and
            MACD[1].DATA[period] < MACD[2].DATA[period] and
            MACD[1].DATA[period - 1] >= MACD[2].DATA[period - 1]
     then
        if Direction then
