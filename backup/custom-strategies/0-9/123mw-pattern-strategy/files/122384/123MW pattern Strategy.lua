...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=67016
...
function CreateParameters()
    ...
    strategy.parameters:addInteger("DeltaPips", "DeltaPips", "Typical trend delta ", 75);
    strategy.parameters:addDouble("TargetFactor", "TargetFactor", "Target factor ", 1);
    strategy.parameters:addBoolean("close_on_limit", "Close on Limit Line", "", true);
    strategy.parameters:addBoolean("close_on_stop", "Close on Stop Line", "", true);
end

local indi;
function CreateIndicator(source)
    EnsureIndicatorInstalled("123MW-0.3B");
    indi = core.indicators:create("123MW-0.3B", source
        , instance.parameters.ZigZagDepth
        , instance.parameters.ZigZagDeviation
        , instance.parameters.ZigZagBackstep
        , instance.parameters.DeltaPips
        , instance.parameters.TargetFactor);
end
...
function IsBuyCondition(source, period)
    if indi:getTextOutput(1):size() == 0 then
        return false;
    end
...
function IsSellCondition(source, period)
    if indi:getTextOutput(2):size() == 0 then
        return false;
    end
    return indi:getTextOutput(2):hasData(period);
end

function IsExitBuyCondition(source, period)
    if instance.parameters.close_on_limit then
        if indi.LimitLine:size() < 2 or not indi.LimitLine:hasData(period) then
            return;
        end
        if core.crosses(source.close, indi.LimitLine, period) then
            return true;
        end
    end
    if instance.parameters.close_on_stop then
        if indi.StopLine:size() < 2 or not indi.StopLine:hasData(period) then
            return;
        end
        if core.crosses(source.close, indi.StopLine, period) then
            return true;
        end
    end
    return false;
...
function Init()
    strategy:name("123MW pattern Strategy");
...
function Prepare(name_only)
    ...
    CreateIndicator(trading_logic.MainSource);
...
function DoCloseOnOpposite(side)
    if instance.parameters.close_on_opposite then
        trading:FindTrade()
            :WhenSide(trading:getOppositeSide(side))
...
function IsPositionLimitHit(side, side_limit)
    ...
    local side_positions = trading:FindTrade()
        :WhenSide(side)
        :WhenCustomID(custom_id)
        :Count();
...
function EntryFunction(source, period)
    ...
    if IsBuyCondition(source, period) then
        if Direction == "direct" then
            GoLong(source, period);
        else
            GoShort(source, period);
        end
