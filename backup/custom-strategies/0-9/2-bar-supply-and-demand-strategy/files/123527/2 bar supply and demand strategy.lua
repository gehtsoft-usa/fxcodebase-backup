...
local STRATEGY_NAME = "2 bar supply and demand strategy";
...
function CreateParameters()
    strategy.parameters:addString("trading_mode", "Trading mode", "", "ImmediateOrders");
    strategy.parameters:addStringAlternative("trading_mode", "Immediate", "", "ImmediateOrders");
    strategy.parameters:addStringAlternative("trading_mode", "Pending", "", "PendingOrders");

    strategy.parameters:addString("stop_mode", "Stop loss mode", "", "StopModeManual");
    strategy.parameters:addStringAlternative("stop_mode", "Manual", "", "StopModeManual");
    strategy.parameters:addStringAlternative("stop_mode", "Set 1 pip below the wide demand level", "", "StopMode1");
    strategy.parameters:addStringAlternative("stop_mode", "Set 1 pip below the narrow demand level", "", "StopMode2");
end

local indi;
function CreateIndicators(source)
    EnsureIndicatorInstalled("2 BAR SUPPLY AND DEMAND");
    indi = core.indicators:create("2 BAR SUPPLY AND DEMAND", source, true);
end
...
function GetEntryRate(bs, period)
    if instance.parameters.trading_mode == "ImmediateOrders" then
        return nil;
    end
    if bs == "B" then
        return indi.demand_low[period];
    end
    return indi.demand_high[period];
end

function GetCustomStopLevel(bs, period)
    if instance.parameters.stop_mode == "StopMode1" then
        if bs == "B" then
            return indi.demand_wide_low[period] - trading_logic.MainSource:pipSize();
        else
            return indi.demand_wide_high[period] + trading_logic.MainSource:pipSize();
        end
    elseif instance.parameters.stop_mode == "StopMode2" then
        if bs == "B" then
            return indi.demand_low[period] - trading_logic.MainSource:pipSize();
        else
            return indi.demand_high[period] + trading_logic.MainSource:pipSize();
        end
    else
        return nil;
    end
...
function CreatePositionStrategy(source, side, id)
    ...
    function position_strategy:Open(period)
        local rate = GetEntryRate(self.Side, period);
        local command;
        if rate == nil then
            command = trading:MarketOrder(position_strategy.Source.close:instrument());
        else
            command = trading:EntryOrder(position_strategy.Source.close:instrument())
                :SetRate(rate);
        end
        ...
        local custom_stop = GetCustomStopLevel(self.Side, period);
        if custom_stop == nil then
            ...
            command = command:SetStop(custom_stop, nil);
        end
