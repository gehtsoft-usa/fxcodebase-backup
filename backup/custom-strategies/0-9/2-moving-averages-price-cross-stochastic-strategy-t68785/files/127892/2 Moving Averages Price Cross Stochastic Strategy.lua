...
--http://fxcodebase.com/code/viewtopic.php?f=31&t=68785
...
function Init() --The strategy profile initialization
    strategy:name("2 Moving Averages Price Cross Stochastic Strategy")
...
local Indicator, K, SD, D, MVAT_K, MVAT_D
...
function Prepare(nameOnly)
    ...
    first = math.max(MA1.DATA:first(), MA2.DATA:first(), Indicator.D:first())
...
function EntryFunction(now, period)
	...
	20 EMA above 50 MA
	Bar has touched or crossed 50 MA at close
    ...
    if
        MA1.DATA[period] > MA2.DATA[period] and Source.close[period] > MA2.DATA[period] and
            Source.close[period - 1] <= MA2.DATA[period - 1] and
            Indicator.K[period] < 30 and
            Indicator.D[period] < 30
     then
        if Direction then
            BUY()
        else
    ...
    elseif
        MA1.DATA[period] < MA2.DATA[period] and Source.close[period] < MA2.DATA[period] and
            Source.close[period - 1] >= MA2.DATA[period - 1] and
            Indicator.K[period] > 70 and
            Indicator.D[period] > 70
     then
        if Direction then
            SELL()
        else
