-- Available @ https://fxcodebase.com/code/viewtopic.php?f=31&t=75994
...
local STRATEGY_NAME = "50V1 Strategy";
function CreateParameters() 
    --create your algorithm strategy.parameters here
    strategy.parameters:addInteger("N", "Pips", "How many Pips in the Box", 50, 2, 1000);
end

local indi;
function CreateEntryIndicators(source)
    local profile = core.indicators:findIndicator("50V1");
    assert(profile ~= nil, "Please, download and install " .. "50V1" .. ".LUA indicator");
    local indicatorParams = profile:parameters();
    indicatorParams:setInteger("N", instance.parameters.N);
    indi = core.indicators:create("50V1", source, indicatorParams)
end
...
function IsEntryLong(source, period)
    return indi.L1:hasData(period);
end
function IsEntryShort(source, period)
    return indi.L3:hasData(period);
end
