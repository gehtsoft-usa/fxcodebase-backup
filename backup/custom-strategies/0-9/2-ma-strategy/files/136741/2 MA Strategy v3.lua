...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=11075
--+------------------------------------------------------------------+
...
local STRATEGY_NAME = "2 MA Strategy";
...
function CreateParameters()
    ...
    strategy.parameters:addString("Method1", "Short Method", "Method", "EMA")
    ...
    strategy.parameters:addInteger("LP", "Long Period", "", 14)
    strategy.parameters:addString("Method2", "Short Method", "Method", "EMA")
...
function SetCustomStop(position_desc, command, period, periods_from_last, source) 
    if position_desc.Side == "B" then
        local stop = source.low[period];
        for i = period - 1, 0, -1 do
            if core.crossesUnder(Indicator[1].DATA, Indicator[2].DATA, i) then
                command:SetStop(stop);
                return true;
            end
            if stop > source.low[i] then
                stop = source.low[i];
            end
        end
    else
        local stop = source.high[period];
        for i = period, 0, -1 do
            if core.crossesOver(Indicator[1].DATA, Indicator[2].DATA, i) then
                command:SetStop(stop);
                return true;
            end
            if stop < source.high[i] then
                stop = source.high[i];
            end
        end
    end
...
function CreateEntryIndicators(source) 
    ...
    Indicator[1] = core.indicators:create(Method1, source[Price], SP)

    Method2 = instance.parameters.Method2;
    LP = instance.parameters.LP;
    assert(core.indicators:findIndicator(Method2) ~= nil, Method2 .. " indicator must be installed");
    Indicator[2] = core.indicators:create(Method2, Indicator[1].DATA, LP)
end
...
function CreateCustomActions()
    ...
    enterShortAction.IsPass = function (source, period, periodFromLast, data)
        return core.crossesUnder(Indicator[1].DATA, Indicator[2].DATA, period);
    end
