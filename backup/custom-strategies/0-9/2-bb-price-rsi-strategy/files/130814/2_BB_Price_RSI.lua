...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=69333
...
local STRATEGY_NAME = "2 BB + Price + RSI Strategy";
...
function CreateParameters()
    strategy.parameters:addInteger("bb_1_period", "BB 1 period", "", 20);
    strategy.parameters:addDouble("bb_1_deviations", "BB 1 deviations", "", 2.0);

    strategy.parameters:addInteger("bb_2_period", "BB 2 period", "", 50);
    strategy.parameters:addDouble("bb_2_deviations", "BB 2 deviations", "", 2.1);

    strategy.parameters:addInteger("rsi_period", "RSI period", "", 14);
    strategy.parameters:addInteger("rsi_os_level", "RSI oversold", "", 80);
    strategy.parameters:addInteger("rsi_ob_level", "RSI overbought", "", 20);
    
    strategy.parameters:addInteger("Depth", "Depth", "the minimal amount of bars where there will not be the second maximum", 192);
    strategy.parameters:addInteger("Deviation", "Deviation", "Distance in pips to eliminate the second maximum in the last Depth periods", 1);
    strategy.parameters:addInteger("Backstep", "Backstep", "The minimal amount of bars between maximums/minimums", 144);
end
function CreateStopParameters(params, id) 
    strategy.parameters:addInteger("stop_loopback", "Stop lookback", "", 10);
    strategy.parameters:addInteger("stop_shift", "Stop shift, pips", "", 1);
    return false; 
end

local bb1, bb2, rsi, zz;
local rsi_os_level, rsi_ob_level;
function CreateEntryIndicators(source)
    ...
    bb1 = core.indicators:create("BB", source, instance.parameters.bb_1_period, instance.parameters.bb_1_deviations);
    bb2 = core.indicators:create("BB", source, instance.parameters.bb_2_period, instance.parameters.bb_2_deviations);
    rsi = core.indicators:create("RSI", source, instance.parameters.rsi_period);
    zz = core.indicators:create("ZIGZAG", source, instance.parameters.Depth, instance.parameters.Deviation, instance.parameters.Backstep);
end
...
function SetCustomStop(position_desc, command, period, periods_from_last, source) 
    local ll, hh = mathex.minmax(source, period - position_desc.stop_loopback, period);
    if position_desc.Side == "B" then
        command:SetStop(ll - position_desc.stop_shift * source:pipSize());
    else
        command:SetStop(hh + position_desc.stop_shift * source:pipSize());
    end
    return true;
end

function SaveCustomStopParameters(position_desc, id) 
    position_desc.stop_loopback = instance.parameters.stop_loopback;
    position_desc.stop_shift = instance.parameters.stop_shift;
end

function GetZZSide(period)
    while (period > 0 and not zz.DATA:hasData(period)) do
        period = period - 1;
    end
    if period == 0 or not zz.DATA:hasData(period - 1) then
        return;
    end
    if zz.DATA[period] > zz.DATA[period - 1] then
        return 1;
    end
    return -1;
end
...
function CreateCustomActions()
    ...
    enterLongAction.IsPass = function (source, period, periodFromLast, data)
        if not bb1.BL:hasData(period - 1) 
            or not bb2.BL:hasData(period - 1)
            or not rsi.DATA:hasData(period)
        then
            return;
        end
        local zz_side = GetZZSide(period);
        if zz_side == nil then
            return;
        end

        return core.crosses(bb1.BL, bb2.BL, period)
            and core.crosses(source.close, bb1.BL, period)
            and rsi.DATA[period] <= rsi_os_level
            and zz_side == -1;
    end
    ...
    enterShortAction.IsPass = function (source, period, periodFromLast, data)
        ...
        return core.crosses(bb1.TL, bb2.TL, period)
            and core.crosses(source.close, bb1.TL, period)
            and rsi.DATA[period] >= rsi_ob_level
            and zz_side == 1;
    end
