...
-- https://fxcodebase.com/code/viewtopic.php?f=31&t=71891
...
function Init() --The strategy profile initialization
    strategy:name("3 moving average crossover Strategy with RSI Filter");
	...
	strategy.parameters:addBoolean("EntryFilter", "RSI Entry Filter", "", true);  	
	strategy.parameters:addBoolean("ExitMA", "Exit on MA Cross", "", true);   	
	strategy.parameters:addBoolean("ExitOBOS", "Exit on OB/OS", "", true);   	
    

    strategy.parameters:addGroup("1. MA Calculation");
    strategy.parameters:addInteger("Period1", "Period", "", 5);  
...
function Prepare( nameOnly)
    ...
    ExitOBOS = instance.parameters.ExitOBOS;    
    ExitMA= instance.parameters.ExitMA; 
...
function EntryFunction( id, now,period)
	...
	and ((RSI.DATA[period] <= OB and EntryFilter) or not EntryFilter)
	then	
          if Direction then
                BUY();
            else
	...
	and  MA1.DATA[period-1] >= MA2.DATA[period-1]
	and ((RSI.DATA[period] >= OS and EntryFilter) or not EntryFilter)	
	then
            if Direction then
                SELL();
            else
