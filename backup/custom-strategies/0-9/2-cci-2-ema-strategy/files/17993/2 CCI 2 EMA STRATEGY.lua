...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=8144
...
function Init() --The strategy profile initialization
    strategy:name("2 CCI 2 EMA STRATEGY")
    ...
    strategy.parameters:addString("TF1", "Fast MA time frame", "", "m5")
    strategy.parameters:setFlag("TF1", core.FLAG_PERIODS)

    strategy.parameters:addGroup("EMA")
    strategy.parameters:addInteger("N1", "Periods", "", 34, 1, 2000)

    strategy.parameters:addGroup("Short CCI")
    strategy.parameters:addInteger("N3", "Periods", "", 14, 1, 2000)
    strategy.parameters:addDouble("N3B", "Buy Level", "", 0)
    strategy.parameters:addDouble("N3S", "Sell Level", "", 0)

    strategy.parameters:addGroup("Long CCI")
    strategy.parameters:addInteger("N4", "CCI Long Periods", "", 50, 1, 2000)
    strategy.parameters:addDouble("N4B", "Buy Level", "", 0)
    strategy.parameters:addDouble("N4S", "Sell Level", "", 0)
...
local N1, N3, N4, N3B, N3S, N4B, N4S
...
function Prepare(nameOnly)
    ...
    N3B = instance.parameters.N3B
    N3S = instance.parameters.N3S
    N4S = instance.parameters.N4S
    N4B = instance.parameters.N4B
    ...
    Indicator[1] = core.indicators:create("EMA", Source[1].high, N1)
    Indicator[2] = core.indicators:create("EMA", Source[1].low, N1)
    Indicator[3] = core.indicators:create("CCI", Source[1], N3)
    Indicator[4] = core.indicators:create("CCI", Source[1], N4)
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    --     crossover functions
    if
        Short[1]:size() <= Short[1]:first() + 2 or Short[2]:size() <= Short[2]:first() + 2 or
            Short[3]:size() <= Short[2]:first() + 2 or
            Short[4]:size() <= Short[2]:first() + 2
     then
        return
    end
    ...
    --1.price > ema [period 34][applied to high] Ok
    --2.cci 50>buy level
    --3.cci 14 crossover buy level

    --sell when

    --1. price < ema[period 34][applied to low] Ok
    --2.cci 50 < sell level
    --3.cci 14 cross under sell level

    if core.crossesOver(source.close, Short[1], period) and Short[3][period] > N3B and Short[4][period] > N4B then
        if Direction then
            BUY()
        else
            SELL()
        end
    elseif core.crossesUnder(source.close, Short[2], period) and Short[3][period] < N3S and Short[4][period] < N4S then
        if Direction then
            SELL()
        else
            BUY()
    ...
    --exit buy when:
    --1.price cross under ema 34[ applied to high]
    --2.cci 14 cross under sell level
    --3. cci 50 cross under sell level

    --exit sell when:
    --1. price crossover ema 34[ applied to low]
    --2.cci 14 cross over buy level
    --3. cci 50 cross over buy level

    if source.close[period] < Short[1][period] or Short[3][period] < N3S or Short[4][period] < N4S then
        if Direction then
    ...
    elseif source.close[period] > Short[2][period] or Short[3][period] > N3B or Short[4][period] > N4B then
        if Direction then
            if haveTrades("S") then
                exit("S")
                Signal("Close Short")
