...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if core.crossesOver(Short[3], N3B, period) and Source[1].close[period] > Short[1][period] and Short[4][period] > N4B then
        if Direction then
            BUY()
        else
            SELL()
    ...
    elseif
        core.crossesUnder(Short[3], N3S, period) and Source[1].close[period] < Short[1][period] and
            Short[4][period] < N4S
     then
        if Direction then
            SELL()
    ...
    if Source[1].close[period] < Short[1][period] or Short[3][period] < N3S or Short[4][period] < N4S then
        if Direction then
    ...
    elseif Source[1].close[period] > Short[2][period] or Short[3][period] > N3B or Short[4][period] > N4B then
        if Direction then
            if haveTrades("S") then
                exit("S")
                Signal("Close Short")
