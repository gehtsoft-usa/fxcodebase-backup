...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=65144
...
function Init() --The strategy profile initialization
    strategy:name("3MA ICH Strategy")
    ...
    strategy.parameters:addInteger("Delta", "MA Break Delta", "Delta (In Pips)", 0)

    strategy.parameters:addGroup("1. MA Calculation")

    strategy.parameters:addInteger("Period1", "MA Period", "Method", 14)
    ...
    strategy.parameters:addInteger("Period2", "MA Period", "Method", 34)
    ...
    strategy.parameters:addInteger("Period3", "MA Period", "Method", 50)

    strategy.parameters:addString("Method3", "MA Method", "Method", "MVA")
    ...
    strategy.parameters:addGroup("HTF MA Confirmation")
    strategy.parameters:addBoolean("confirm_with_htf", "Confirm with HTF MA", "", false)
    strategy.parameters:addString("htf", "Higher Time Frame", "", "m1")
    strategy.parameters:setFlag("htf", core.FLAG_PERIODS)
    strategy.parameters:addInteger("period_htf", "MA Period", "Method", 50)
    strategy.parameters:addString("method_htf", "MA Method", "Method", "MVA")
    strategy.parameters:addStringAlternative("method_htf", "MVA", "MVA", "MVA")
    strategy.parameters:addStringAlternative("method_htf", "EMA", "EMA", "EMA")
    strategy.parameters:addStringAlternative("method_htf", "LWMA", "LWMA", "LWMA")
    strategy.parameters:addStringAlternative("method_htf", "TMA", "TMA", "TMA")
    strategy.parameters:addStringAlternative("method_htf", "SMMA", "SMMA", "SMMA")
    strategy.parameters:addStringAlternative("method_htf", "KAMA", "KAMA", "KAMA")
    strategy.parameters:addStringAlternative("method_htf", "VIDYA", "VIDYA", "VIDYA")
    strategy.parameters:addStringAlternative("method_htf", "WMA", "WMA", "WMA")

    strategy.parameters:addGroup("ICH Calculation")
    strategy.parameters:addInteger("TenkanSenPeriod", "Tenkan-sen period", "Tenkan-sen period", 9, 1, 1000)
    strategy.parameters:addInteger("KijunSenPeriod", "Kijun-sen period", "Kijun-sen period", 26, 1, 1000)
    strategy.parameters:addInteger("SenkouSpanPeriod", "Senkou Span B period", "Senkou Span B period", 52, 1, 1000)
...
function CreateTradingParameters()
    ...
    strategy.parameters:addInteger("Trailing", "Trailing stop size", "", 1)
...
local MA1, MA2, MA3, ICH, A, B, Delta
local htf_ma
...
function Prepare(nameOnly)
    ...
    core.host:trace(tostring(instance.parameters.confirm_with_htf))
    if instance.parameters.confirm_with_htf then
        htf_source = ExtSubscribe(3, nil, instance.parameters.htf, instance.parameters.Type == "Bid", "bar")
        htf_ma =
    assert(core.indicators:findIndicator(instance.parameters.method_htf) ~= nil, instance.parameters.method_htf .. " indicator must be installed");
            core.indicators:create(instance.parameters.method_htf, htf_source.close, instance.parameters.period_htf)
    end
    ...
    assert(core.indicators:findIndicator(instance.parameters.Method3) ~= nil, instance.parameters.Method3 .. " indicator must be installed");
    ...
    A = ICH:getStream(3)
    B = ICH:getStream(4)
    first = math.max(MA1.DATA:first(), MA2.DATA:first(), MA3.DATA:first(), A:first())
...
function ConfirmHTFForShort()
    if htf_ma == nil then
        return true
    end

    core.host:trace(tostring(htf_ma.DATA[NOW] > Source.close[NOW]) .. " Short")
    return htf_ma.DATA[NOW] > Source.close[NOW]
end

function ConfirmHTFForLong()
    if htf_ma == nil then
        return true
    end

    core.host:trace(tostring(htf_ma.DATA[NOW] < Source.close[NOW]) .. " Long")
    return htf_ma.DATA[NOW] < Source.close[NOW]
end
...
function EntryFunction(now, period)
    ...
    --When price and MA below cloud, and price breaks (UP)the three moving averages by a user defined amount of pips,
    if
        math.max(Source.close[period], MA1.DATA[period], MA2.DATA[period], MA3.DATA[period]) <
            math.min(A[period], B[period]) and
            Source.close[period] >
                (math.max(MA1.DATA[period], MA2.DATA[period], MA3.DATA[period]) + Delta * Source:pipSize()) and
            Source.close[period - 1] <=
                (math.max(MA1.DATA[period - 1], MA2.DATA[period - 1], MA3.DATA[period - 1]) + Delta * Source:pipSize()) and
            ConfirmHTFForLong()
     then
        if Direction then
            BUY()
        ...
        Return = true
    elseif
        math.min(Source.close[period], MA1.DATA[period], MA2.DATA[period], MA3.DATA[period]) >
            math.max(A[period], B[period]) and
            Source.close[period] <
                (math.min(MA1.DATA[period], MA2.DATA[period], MA3.DATA[period]) - Delta * Source:pipSize()) and
            Source.close[period - 1] >=
                (math.min(MA1.DATA[period - 1], MA2.DATA[period - 1], MA3.DATA[period - 1]) - Delta * Source:pipSize()) and
            ConfirmHTFForShort()
     then
        if Direction then
            SELL()
