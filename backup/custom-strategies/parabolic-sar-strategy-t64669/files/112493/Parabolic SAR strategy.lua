...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=64669
...
function Init() --The strategy profile initialization
    ...
    strategy.parameters:addInteger("Period", "Period", "Period", 34);
...
function CreateTradingParameters()
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "PSS");
...
local SAR, Step, Max;
local TF1, Filter, Period, MA, MA_Source,Method;
...
function Prepare( nameOnly)
    ...
    s2, e2 = core.getcandle(TF1, core.now(), 0, 0);
    assert ((e1 - s1) <= (e2 - s2), "MA Filter time frame must be equal to or bigger than the chart time frame!");
    ...
    assert(TF1 ~= "t1", "MA time frame must not be tick");
	...
	MA_Source = ExtSubscribe(3, nil, TF1, instance.parameters.Type == "Bid", "bar");
    SAR	=    core.indicators:create("SAR", Source, Step,  Max );
	first=math.max( SAR.UP:first(), SAR.DN:first()); 	
    assert(core.indicators:findIndicator(Method) ~= nil, Method .. " indicator must be installed");
	MA	=    core.indicators:create(Method, MA_Source.close, Period );
...
function ExtUpdate(id, source, period)  -- The method called every time when a new bid or ask price appears.
    ...
    then	
		...
		period1= core.findDate (MA_Source, TickSource:date(period), false);
		period= core.findDate (Source, TickSource:date(period), false);
			...
			period1= core.findDate (MA_Source, Source:date(period), false);
	end
   ...
   -- get only time
	...
	if MA.DATA:size()-1 < period1 then
	return;
	end
...
function EntryFunction( now,period,period1)
    ...
    if    SAR:getStream(1):hasData(period)
	and Source.close[period]> BuyLevel
	and Source.close[period-1]<= BuyLevel
	and(not Filter or (Filter and  Source.close[period]> MA.DATA[period1] ))
	then	
          if Direction then
                BUY();
            else
		...
		Return=true;
    elseif SAR:getStream(0):hasData(period)
	and Source.close[period]< SellLevel
	and Source.close[period-1]>= SellLevel
	and(not Filter or (Filter and  Source.close[period]<MA.DATA[period1] ))
		then
            if Direction then
                SELL();
