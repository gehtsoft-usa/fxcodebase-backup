...
--http://fxcodebase.com/code/viewtopic.php?f=31&t=69777
...
function CreateParameters() 
    strategy.parameters:addInteger("bars_back", "Start Bars Back", "Bars Back", 125)
    strategy.parameters:addInteger("m", "Order", "Order", 2)
    strategy.parameters:addDouble("kstd", "Multiplier", "Multiplier", 2)
end

local indi;
function CreateEntryIndicators(source)
    local profile = core.indicators:findIndicator("BCG");
    assert(profile ~= nil, "Please, download and install " .. "BCG" .. ".LUA indicator");
    local indicatorParams = profile:parameters();
    indicatorParams:setInteger("bars_back", instance.parameters.bars_back);
    indicatorParams:setInteger("m", instance.parameters.m);
    indicatorParams:setDouble("kstd", instance.parameters.kstd);
    indi = core.indicators:create("BCG", source, indicatorParams)
end
...
function IsEntryLong(source, period)
    if not indi.LC1:hasData(period - 1) then
        return false;
    end
    return core.crossesUnderOrTouch(source.close, indi.LC1, period);
end
function IsEntryShort(source, period)
    if not indi.UC1:hasData(period - 1) then
        return false;
    end
    return core.crossesOverOrTouch(source.close, indi.UC1, period);
end
