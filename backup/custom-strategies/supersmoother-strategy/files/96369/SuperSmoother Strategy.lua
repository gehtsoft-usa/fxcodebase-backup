...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=61298
...
function Init() --The strategy profile initialization
    strategy:name("SuperSmoother Strategy")
...
function Prepare(nameOnly)
    ...
    assert(
        core.indicators:findIndicator("SUPERSMOOTHER") ~= nil,
        "Please, download and install SUPERSMOOTHER.LUA indicator"
    )

    Source = ExtSubscribe(2, nil, instance.parameters.TF, instance.parameters.Type == "Bid", "bar")
    Indicator = core.indicators:create("SUPERSMOOTHER", Source[Price])
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if Indicator.DATA[period] > Indicator.DATA[period - 1] and Indicator.DATA[period - 2] >= Indicator.DATA[period - 1] then
        BUY()
        ONE = Source:serial(period)
    elseif
        Indicator.DATA[period] < Indicator.DATA[period - 1] and Indicator.DATA[period - 2] <= Indicator.DATA[period - 1]
     then
        SELL()
        ONE = Source:serial(period)
    end
