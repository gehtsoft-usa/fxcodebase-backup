...
-- http://fxcodebase.com/code/viewtopic.php?f=17&t=66425
...
function Init() --The strategy profile initialization
    strategy:name("Two Level CCI Cross Strategy");
    ...
    strategy.parameters:addInteger("Period", "Period", "", 14);
	 ...
	 strategy.parameters:addDouble("OB", "OB Level", "", 100);
	  ...
	  strategy.parameters:addDouble("OBC", "OB Confirmation Level", "", 250);
	  strategy.parameters:addDouble("OSC", "OS Confirmation Level", "", -250);
	  strategy.parameters:addInteger("ConfirmationPeriod", "Confirmation Period", "", 7);
...
function CreateTradingParameters()
  ...
  --  strategy.parameters:addStringAlternative("ExitExecutionType", "End of Turn", "", "EndOfTurn");
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "TLCCICS");
...
function EntryFunction( now,period)
	...
	and  Indicator.DATA[period-1]<= OS
	then	
          if Direction then
                BUY();
            else
	...
	and  Indicator.DATA[period-1]>= OB
	then
            if Direction then
                SELL();
            else
...
local X=false;
		...
		for i= (period-1), math.max(first, period-ConfirmationPeriod), -1 do


					if Indicator.DATA[period]> OS 
					and Indicator.DATA[i]> OSC 
					then  
					X= false;
					elseif Indicator.DATA[period]< OB
					and Indicator.DATA[i]< OBC 
					then
                    X= false;
					end
