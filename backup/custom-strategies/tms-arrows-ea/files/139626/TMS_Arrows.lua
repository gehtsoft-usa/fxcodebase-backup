...
--http://fxcodebase.com/code/viewtopic.php?f=31&t=70727
...
local source, ha;
local UpZone, MdZone, DnZone, MaBuf, MbBuf, ris, Volatility_Band, ssd;
...
{ 
    {
        ...
        UpCondition = function (period)
            ...
            return core.crossesOver(MaBuf, MbBuf, period)
                and ssd.K[period] > ssd.D[period]
                and ha.open[period] > ha.close[period]
                and (ha.close[period - 1] > ha.open[period - 1] or (ha.close[period - 1] < ha.open[period - 1] and ha.close[period - 2] > ha.open[period - 2]));
        end,
        DownCondition = function (period)
            if period < 2 then
                return false;
            end
            return core.crossesUnder(MaBuf, MbBuf, period)
                and ssd.K[period] < ssd.D[period]
                and ha.open[period] < ha.close[period]
                and (ha.close[period - 1] < ha.open[period - 1] or (ha.close[period - 1] > ha.open[period - 1] and ha.close[period - 2] < ha.open[period - 2]));
        end,
...
function Init()
    indicator:name("TMS Arrows");
    indicator:description("");
    indicator:requiredSource(core.Bar);
    indicator:type(core.Indicator);

    indicator.parameters:addInteger("RSI_Period", "RSI Period", "", 13);
    indicator.parameters:addInteger("Volatility_Band", "Volatility Band", "", 34);

    indicator.parameters:addInteger("rsi_price_period", "RSI Price Period", "", 2);
    AddAverages("rsi_price_method", "RSI Price Method", "MVA");
    indicator.parameters:addInteger("trade_signal_period", "Trade Signal Period", "", 7);
    AddAverages("trade_signal_method", "Trade Signal Method", "MVA");
...
function Prepare(nameOnly)
    ...
    Volatility_Band = instance.parameters.Volatility_Band;

    ha = core.indicators:create("HA", source);
    ssd = core.indicators:create("SSD", source, 8, 3, 3);
    ...
    rsi = core.indicators:create("RSI", source, instance.parameters.rsi_price_period);
    rsi_ma = core.indicators:create("MVA", rsi.DATA, Volatility_Band);
    rsi_price = CreateAverages(instance.parameters.rsi_price_period, instance.parameters.rsi_price_method, rsi.DATA);
    trade_signal = CreateAverages(instance.parameters.trade_signal_period, instance.parameters.trade_signal_method, rsi.DATA);
end
...
function Update(period, mode)
    ...
    ssd:update(mode);
    rsi_ma:update(mode);
    rsi_price:update(mode);
    trade_signal:update(mode);
    if period <= Volatility_Band or not rsi.DATA:hasData(period - Volatility_Band) then
        return;
    end
    local stdev = mathex.stdev(rsi.DATA, core.rangeTo(period, Volatility_Band));
    UpZone[period] = (rsi_ma.DATA[period] + (1.6185 * stdev));
    DnZone[period] = (rsi_ma.DATA[period] - (1.6185 * stdev));  
    MdZone[period] = ((UpZone[period] + DnZone[period]) / 2);
    MaBuf[period] = rsi_price.DATA[period];
    MbBuf[period] = trade_signal.DATA[period]; 
