...
local STRATEGY_NAME = "TMS Arrows Strategy";
function CreateParameters() 
    strategy.parameters:addInteger("RSI_Period", "RSI Period", "", 13);
    strategy.parameters:addInteger("Volatility_Band", "Volatility Band", "", 34);

    strategy.parameters:addInteger("rsi_price_period", "RSI Price Period", "", 2);
    AddAverages("rsi_price_method", "RSI Price Method", "MVA");
    strategy.parameters:addInteger("trade_signal_period", "Trade Signal Period", "", 7);
    AddAverages("trade_signal_method", "Trade Signal Method", "MVA");
end

local indi;
function CreateEntryIndicators(source)
    local profile = core.indicators:findIndicator("TMS_ARROWS");
    assert(profile ~= nil, "Please, download and install " .. "TMS_ARROWS" .. ".LUA indicator");
    local indicatorParams = profile:parameters();
    indicatorParams:setInteger("RSI_Period", instance.parameters.RSI_Period);
    indicatorParams:setInteger("Volatility_Band", instance.parameters.Volatility_Band);
    indicatorParams:setInteger("rsi_price_period", instance.parameters.rsi_price_period);
    indicatorParams:setInteger("trade_signal_period", instance.parameters.trade_signal_period);
    indicatorParams:setString("rsi_price_method", instance.parameters.rsi_price_method);
    indicatorParams:setString("trade_signal_method", instance.parameters.trade_signal_method);
    indicatorParams:setBoolean("strategy_output", true);
    indi = core.indicators:create("TMS_ARROWS", source, indicatorParams)
end
