...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=66935
...
function CreateParameters()
    strategy.parameters:addInteger("Renko_Size", "Renko brick size", "", 20);
    strategy.parameters:addInteger("ma_period", "MA period", "", 7);
    strategy.parameters:addInteger("min_bricks", "Min. bricks", "", 2);
end
...
function CreateIndicator(source)
    renko = core.indicators:create("RENKO_CANDLES", nil, source:instrument(),
        ...
        instance.parameters.is_bid,
        instance.parameters.Renko_Size);
    renko_candles = renko:getCandleOutput(0);
    ma = core.indicators:create("MVA", renko_candles, instance.parameters.ma_period);
end
...
function IsBuyCondition(source, period)
    if renko_candles.DATA:size() <= instance.parameters.min_bricks 
        or not ma.DATA:hasData(NOW - instance.parameters.min_bricks - 1) 
    then
        return false;
    end
    for i = 0, instance.parameters.min_bricks do
        if not ma.DATA:hasData(NOW - i) or renko_candles.close[NOW - i] < ma.DATA[NOW - i] then
            return false;
        end
    end
    return core.crossesOver(renko_candles.close, ma.DATA, NOW - instance.parameters.min_bricks);
end
...
function IsSellCondition(source, period)
    ...
    return core.crossesUnder(renko_candles.close, ma.DATA, NOW - instance.parameters.min_bricks);
end
...
function Init()
    strategy:name("Renko MA strategy");
...
function EntryFunction(source, period)
    UpdateIndicators();
    if renko_candles.close:size() < 1 or last_serial == renko_candles:date(NOW) then
        return;
    end
    ...
    if IsBuyCondition(source, period) then
        ...
        last_serial = renko_candles:date(NOW);
    end
...
function breakeven:CreateController()
    ...
    function controller:DoBreakeven()
        if self._executed then
            if self._close_percent ~= nil and self._command ~= nil and self._command.Finished then
