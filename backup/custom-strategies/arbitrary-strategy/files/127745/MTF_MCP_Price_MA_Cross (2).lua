...
function Init()
    indicator:name("MTF MCP  Price MA Cross");
	 ...
	 for i = 1, Count, 1 do	
      ...
      When[i]={};
		   ...
		   for j = 1, Num, 1 do	
			...
			Temp= core.indicators:create("MVA", source.close ,Period );
			first =  Temp.DATA:first()*2;  
		   Source[i][j]= core.host:execute("getSyncHistory", Pair[i], TF[j], source:isBid(),math.max(300,first*2),20000 + ID , 10000 +ID);
		   loading [i][j]=true;
		   
		   Indicator [i][j]= core.indicators:create("MVA", Source[i][j].close, Period);
...
function Draw(stage, context)
 ...
 function Find(i,j,x)
    ...
    if not Source[i][j].close:hasData(Source[i][j].close:size()-1-x)
	or not Source[i][j].close:hasData(Source[i][j].close:size()-1-x-1)
	or not Indicator[i][j].DATA:hasData(Indicator[i][j].DATA:size()-1-x)
	or not Indicator[i][j].DATA:hasData(Indicator[i][j].DATA:size()-1-x-1)
	then
	return 0;
	end
   
	 if  Source[i][j].close[Source[i][j].close:size()-1-x] > Indicator[i][j].DATA[Indicator[i][j].DATA:size()-1-x] 
	 and Source[i][j].close[Source[i][j].close:size()-1-x-1] <= Indicator[i][j].DATA[Indicator[i][j].DATA:size()-1-x-1] 
	 then
	 Direction[i][j]=1;
     When[i][j]=x;	 
	 elseif  Source[i][j].close[Source[i][j].close:size()-1-x] < Indicator[i][j].DATA[Indicator[i][j].DATA:size()-1-x]
	 and Source[i][j].close[Source[i][j].close:size()-1-x-1] >= Indicator[i][j].DATA[Indicator[i][j].DATA:size()-1-x-1]
	 then
	 Direction[i][j]=-1;
	 When[i][j]=x;
