...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=64481
...
function Init() --The strategy profile initialization
    strategy:name("MA Cross Strategy with Daily Open LineFilter")
    ...
    strategy.parameters:addGroup("Daily Open Line Calculation")
    strategy.parameters:addInteger("OpenHour", "Open hour", "", 0, 0, 23)

    strategy.parameters:addGroup("MA Calculation")
    strategy.parameters:addInteger("Period1", "Fast Period MA", "", 50, 2, 2000)
    ...
    strategy.parameters:addStringAlternative("Method1", "SQW_MA", "SQW_MA", "SQW_MA")

    strategy.parameters:addInteger("Period2", "Slow Period MA", "", 200, 2, 2000)
    ...
    strategy.parameters:addStringAlternative("Method2", "SQW_MA", "SQW_MA", "SQW_MA")
...
local Method1, Method2, Period1, Period2, MA1, MA2, DAY, OpenHour
...
function Prepare(nameOnly)
    ...
    assert(
        core.indicators:findIndicator("DAILY_OPEN_LINE") ~= nil,
        "Please, download and install DAILY_OPEN_LINE.LUA indicator"
    )
    ...
    MA2 = core.indicators:create("AVERAGES", Source.close, Method2, Period2, false)
    DAY = core.indicators:create("DAILY_OPEN_LINE", Source, OpenHour)
    first = math.max(MA1.DATA:first(), MA2.DATA:first(), DAY.DATA:first())
...
function EntryFunction(now, period)
    ...
    if
        MA1.DATA[period] > MA2.DATA[period] and MA1.DATA[period - 1] <= MA2.DATA[period - 1] and
            Source.close[period] > DAY.DATA[period]
     then
        if Direction then
            BUY()
        else
