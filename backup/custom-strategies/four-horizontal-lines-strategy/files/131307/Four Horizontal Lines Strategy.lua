...
--http://fxcodebase.com/code/viewtopic.php?f=31&t=69413
...
function CreateParameters() 
    ...
    strategy.parameters:addDouble("buy_top", "Buy Target Price", "", 0);
    strategy.parameters:setFlag("buy_top", core.FLAG_PRICE)
    strategy.parameters:addDouble("buy_bottom", "Buy Breakout Price", "", 0);
    strategy.parameters:setFlag("buy_bottom", core.FLAG_PRICE)
    strategy.parameters:addDouble("sell_bottom", "Sell Breakout Price", "", 0)
    strategy.parameters:setFlag("sell_bottom", core.FLAG_PRICE)
    strategy.parameters:addDouble("sell_top", "Sell Target Price", "", 0);
    strategy.parameters:setFlag("sell_top", core.FLAG_PRICE)
end

local buy_top, buy_bottom, sell_bottom, sell_top
function CreateEntryIndicators(source)
    buy_top = instance.parameters.buy_top;
    buy_bottom = instance.parameters.buy_bottom;
    sell_bottom = instance.parameters.sell_bottom;
    sell_top = instance.parameters.sell_top;
end
...
function IsEntryLong(source, period)
    return source.close[period] > buy_bottom and source.close[period] < buy_top and source.close[period] > source.high[period - 1];
end
function IsEntryShort(source, period)
    return source.close[period] < sell_bottom and source.close[period] > sell_top and source.close[period] < source.low[period - 1];
end
...
function OpenTrade(side)
    ...
    if side == "B" then
        valuemap.RateStop = main_source.low[NOW - 1];
    else
        valuemap.RateStop = main_source.high[NOW - 1];
    end
