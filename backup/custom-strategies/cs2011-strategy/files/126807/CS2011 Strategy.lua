...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=68553
...
function CreateParameters()
    strategy.parameters:addInteger("fast", "MACD Fast Period", "", 30);
    strategy.parameters:addInteger("slow", "MACD Slow Period", "", 500);
    strategy.parameters:addInteger("signal_smoothing", "MACD Signal Smoothing", "", 36);
end

local macd;
function CreateIndicators(source) 
    macd = core.indicators:create("MACD", source, instance.parameters.fast,
        instance.parameters.slow, instance.parameters.signal_smoothing);
end
...
function CreateCustomActions()
    ...
    enterLongAction.IsPass = function (source, period, periodFromLast, data)
        if not macd.MACD:hasData(period - 1) then
            return nil;
        end
        return core.crossesUnder(macd.MACD, 0, period)
            or (macd.MACD[period] > 0 and macd.SIGNAL[period] > macd.SIGNAL[period - 1] and macd.SIGNAL[period - 1] < macd.SIGNAL[period - 2]);
    end
