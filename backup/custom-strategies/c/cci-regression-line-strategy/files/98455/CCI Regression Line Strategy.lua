...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=61775
...
function Init() --The strategy profile initialization
    strategy:name("CCI Regression Line Strategy")
    ...
    strategy.parameters:addInteger("CCI_Period", "Period", "", 14)
    strategy.parameters:addDouble("OB", "OB Level", "", 100)
    strategy.parameters:addDouble("OS", "OS Level", "", -100)

    strategy.parameters:addGroup("Regression Calculation")
    strategy.parameters:addInteger("REGRESION_Period", "Period", "", 34)

    strategy.parameters:addString("SignalType", "Candle Definition", "", "OpenClose")
    strategy.parameters:addStringAlternative("SignalType", "OpenClose", "", "OpenClose")
    strategy.parameters:addStringAlternative("SignalType", "HighLow", "", "HighLow")
...
local first
local CCI_Period, CCI, OB, OS
local REGRESION, REGRESION_Period
local Direction
local CustomID
local SignalType
...
function Prepare(nameOnly)
    ...
    REGRESION_Period = instance.parameters.REGRESION_Period
    ...
    REGRESION = core.indicators:create("REGRESSION", Source.close, REGRESION_Period)
    first = math.max(CCI.DATA:first(), REGRESION.DATA:first()) + 1
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    -- update indicators.
    REGRESION:update(core.UpdateLast)
    ...
    if
        core.crossesOver(CCI.DATA, OS, period) and
            (math.max(Source.open[period], Source.close[period]) >= REGRESION.DATA[period] and
                math.min(Source.open[period], Source.close[period]) <= REGRESION.DATA[period] or
                SignalType == "HighLow") and
            (Source.high[period] >= REGRESION.DATA[period] and Source.low[period] <= REGRESION.DATA[period] or
                SignalType == "OpenClose") and
            Source.close[period] > Source.open[period]
     then
        if Direction then
            BUY()
        else
    ...
    elseif
        core.crossesUnder(CCI.DATA, OB, period) and
            (math.max(Source.open[period], Source.close[period]) >= REGRESION.DATA[period] and
                math.min(Source.open[period], Source.close[period]) <= REGRESION.DATA[period] or
                SignalType == "HighLow") and
            (Source.high[period] >= REGRESION.DATA[period] and Source.low[period] <= REGRESION.DATA[period] or
