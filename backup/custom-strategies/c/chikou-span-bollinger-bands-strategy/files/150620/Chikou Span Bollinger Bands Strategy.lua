...
-- https://fxcodebase.com/code/viewtopic.php?f=27&p=153704#p153704
...
function Init() --The strategy profile initialization
    strategy:name("Chikou Span Bollinger Bands Strategy")
    ...
    strategy.parameters:addGroup("Chikou Span Calculation")
    strategy.parameters:addString("TF1", "Time frame", "", "m1")
    strategy.parameters:setFlag("TF1", core.FLAG_PERIODS)
    strategy.parameters:addInteger("Chikou_Period", "Chikou Period", "", 26)

    strategy.parameters:addGroup("BB Calculation")
    strategy.parameters:addString("TF2", "Time frame", "", "m15")
    strategy.parameters:setFlag("TF2", core.FLAG_PERIODS)
    strategy.parameters:addInteger("Period1", "Period", "", 20)
    strategy.parameters:addDouble("Deviation1", "Deviation", "", 2)

    strategy.parameters:addGroup("Exit BB Calculation")
    strategy.parameters:addString("TF3", "Time frame", "", "m15")
    strategy.parameters:setFlag("TF3", core.FLAG_PERIODS)
    strategy.parameters:addInteger("Period2", "Period", "", 20)
    strategy.parameters:addDouble("Deviation2", "Deviation", "", 2)
...
function Prepare(nameOnly)
    ...
    --Indicator parameters
    Chikou_Period = instance.parameters.Chikou_Period
    ...
    assert((e1 - s1) <= (e3 - s3), "The chosen time frame must be equal to or bigger than the chart time frame!")
    ...
    HTF_Source1 = ExtSubscribe(3, nil, TF2, instance.parameters.Type == "Bid", "bar")
    HTF_Source2 = ExtSubscribe(4, nil, TF3, instance.parameters.Type == "Bid", "bar")
    BB1 = core.indicators:create("BB", HTF_Source1.close, Period1, Deviation1)
    BB2 = core.indicators:create("BB", HTF_Source2.close, Period2, Deviation2)
    ich = core.indicators:create("ICH", Source, 9, Chikou_Period, 52)
    first = Source.close:first() + Chikou_Period + 1
...
function ExitFunction(id, now, period)
    ...
    if (ExitExecutionType == "EndOfTurn" and id == 2) then
        period = period
        p1 = core.findDate(HTF_Source2, Source:date(period), false)
        p2 = core.findDate(HTF_Source2, Source:date(period - 1), false)
    elseif (ExitExecutionType == "Live" and id == 1) then
        period = core.findDate(Source, TickSource:date(period), false)
        p1 = core.findDate(HTF_Source2, TickSource:date(period), false)
        p2 = core.findDate(HTF_Source2, TickSource:date(period - 1), false)
    else
        return
    end

    if p1 == -1 or p2 == -1 or not BB2.DATA:hasData(p1) or not BB2.DATA:hasData(p2) then
        return
    end
    ...
    --and stop limit when CS (m1) crossunder BBTL (m5)
    --and stop limit when CS (m1) crossover BBBL (m5)

    if Source.close[period - Chikou_Period] < BB2.TL[p1] and Source.close[period - Chikou_Period - 1] >= BB2.TL[p2] then
        if Direction then
    ...
    if Source.close[period - Chikou_Period] > BB2.BL[p1] and Source.close[period - Chikou_Period - 1] <= BB2.BL[p2] then
        if Direction then
            if haveTrades("S") then
                exitSpecific("S")
                Signal("Close Short")
...
function EntryFunction(id, now, period)
    ...
    if (EntryExecutionType == "EndOfTurn" and id == 2) then
        period = period
        p1 = core.findDate(HTF_Source1, Source:date(period), false)
        p2 = core.findDate(HTF_Source1, Source:date(period - 1), false)
    elseif (EntryExecutionType == "Live" and id == 1) then
        period = core.findDate(Source, TickSource:date(period), false)
        p1 = core.findDate(HTF_Source1, TickSource:date(period), false)
        p2 = core.findDate(HTF_Source1, TickSource:date(period - 1), false)
    else
        return
    end

    if p1 == -1 or p2 == -1 or not BB1.DATA:hasData(p1) or not BB1.DATA:hasData(p2) then
        return
    end
    ...
    --Long when CS (m1) crossover BBTL (m15)
    --Short when CS (m1) crossunder BBBL (m15)

    -- only buy if we have a fast cross over slow and the price is above the moving averages.
    if ich.CS[period - Chikou_Period] > BB1.TL[p1] and ich.CS[period - Chikou_Period - 1] <= BB1.TL[p2] then
        if Direction then
            BUY()
        else
            SELL()
        ...
        Return = true
    elseif ich.CS[period - Chikou_Period] < BB1.BL[p1] and ich.CS[period - Chikou_Period - 1] >= BB1.BL[p2] then
        if Direction then
            SELL()
        else
            BUY()
