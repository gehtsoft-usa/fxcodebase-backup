...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=68329
...
function Init()
    strategy:name("Close Position After Retracement");
    strategy:description("Close Position After Retracement");
    strategy:type(core.Both);

    strategy.parameters:addInteger("min_hours", "Min. hours", "", 5);
    strategy.parameters:addDouble("max_loss", "Max. loss", "", -20);
...
function Prepare(onlyName)
    ...
    max_loss = instance.parameters.max_loss;
    min_hours = instance.parameters.min_hours / 24;
...
function ExtAsyncOperationFinished(cookie, success, message, message2, message3)
    ...
    if cookie == 1 then
        ...
        local row = enum:next();
        while row ~= nil do
            local last_action = actions[row.TradeID];
            core.host:trace((now - row.Time));
            if last_action == nil and (now - row.Time) >= min_hours then
                core.host:trace("test");
                if row.PL <= max_loss then
                    actions[row.TradeID] = trading:Close(row);
                end
            end
            row = enum:next();
        end
