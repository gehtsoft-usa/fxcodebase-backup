...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=15229
...
function Init() --The strategy profile initialization
    ...
    strategy.parameters:addInteger("CP", "CCI Period", "CCI Period", 50, 1, 2000)
    strategy.parameters:addInteger("TP", "T3 Period", "T3 Period", 5, 1, 2000)
    strategy.parameters:addDouble("B", "B", "B", 0.618)
    ...
    strategy.parameters:addString("SignalType", "Trade Type", "", "Zero")
    strategy.parameters:addStringAlternative("SignalType", "Zero Line Cross", "", "Zero")
    strategy.parameters:addStringAlternative("SignalType", "Slope Change", "", "Slope")
...
function Prepare(nameOnly)
    ...
    assert(core.indicators:findIndicator("CCI_T3") ~= nil, "Please, download and install CCI_T3.LUA indicator")
    ...
    Indicator[1] = core.indicators:create("CCI_T3", Source, CP, TP, B, core.rgb(0, 255, 0), core.rgb(255, 0, 0))
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if SignalType == "Zero" then
        if core.crossesOver(Short[1], 0, period) then
            if Direction then
                BUY()
            else
                SELL()
            end
        elseif core.crossesUnder(Short[1], 0, period) then
            if Direction then
                SELL()
            else
                BUY()
        ...
        if
            Indicator[1].DATA:colorI(period) == core.rgb(0, 255, 0) and
                Indicator[1].DATA:colorI(period - 1) ~= core.rgb(0, 255, 0)
         then
            if Direction then
                BUY()
            else
        ...
        elseif
            Indicator[1].DATA:colorI(period) == core.rgb(255, 0, 0) and
                Indicator[1].DATA:colorI(period - 1) ~= core.rgb(255, 0, 0)
         then
            if Direction then
                SELL()
            else
