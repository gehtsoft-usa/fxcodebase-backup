...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=64770
...
function Prepare(nameOnly)
    ...
    CCI = core.indicators:create("CCI", Source, instance.parameters.Period)
    MA = core.indicators:create("MVA", CCI.DATA, instance.parameters.MA_Period)
    first = MA.DATA:first()
...
function EntryFunction(now, period)
    ...
    if CCI.DATA[period] > 0 and CCI.DATA[period - 1] <= 0 and MA.DATA[period] > 0 then
        if Direction then
            BUY()
        else
            SELL()
        ...
        Return = true
    elseif CCI.DATA[period] < 0 and CCI.DATA[period - 1] >= 0 and MA.DATA[period] < 0 then
        if Direction then
            SELL()
        else
            BUY()
