...
-- https://fxcodebase.com/code/viewtopic.php?f=31&t=71644
...
function Init() --The strategy profile initialization
    strategy:name("Chandelier Trend Strategy ");
    ...
    strategy.parameters:addInteger("Period", "RSI Period", "", 14);
    strategy.parameters:addDouble("BL", "Buy Level" , "", 55);
    strategy.parameters:addDouble("SL", "Sell Level", "", 45);	
	
	
    strategy.parameters:addGroup("ChandelierExit Calculation");	 
...
function CreateTradingParameters()
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "CTS");
...
function Prepare( nameOnly)
    ...
    CE = core.indicators:create("CHANDELIEREXIT_SS", Source , 	instance.parameters.Range, instance.parameters.Shift, instance.parameters.ATRPeriod,  instance.parameters.ATRMultipl  );	
	
 
 
	first=math.max(RSI.DATA:first(),CE.DATA:first()); 
...
function ExitFunction( now,period)
	 ...
	 return;
		            ...
		            if     Source.close[period] < CE.DATA[period] then
...
function EntryFunction( now,period)
    ...
    if   RSI.DATA[period] > BL
	and Source.close[period] > CE.DATA[period]
	then	
          if Direction then
                BUY();
            else
