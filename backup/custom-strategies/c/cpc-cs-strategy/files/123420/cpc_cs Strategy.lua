...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=67279
-- More information about this indicator can be found at:
-- http://fxcodebase.com/
...
local STRATEGY_NAME = "CPC CS Name";
...
function CreateParameters()
    strategy.parameters:addInteger("ema_fast_periods", "EMA Fast Periods", "", 7);
    strategy.parameters:addInteger("ema_medium_periods", "EMA Medium Periods", "", 14);
    strategy.parameters:addInteger("ema_slow_periods", "EMA Slow Periods", "", 21);
    strategy.parameters:addDouble("sar_step", "SAR Step", "", 0.02, 0.001, 1);
    strategy.parameters:addDouble("sar_max", "SAR Max", "", 0.2, 0.001, 10);
    strategy.parameters:addDouble("max_spread", "Max Spread, pips", "", 5);
end
...
function CreateIndicators(source)
    EMAFast = core.indicators:create("EMA", source, instance.parameters.ema_fast_periods);
    EMAMedium = core.indicators:create("EMA", source, instance.parameters.ema_medium_periods);
    EMASlow = core.indicators:create("EMA", source, instance.parameters.ema_slow_periods);
    SAR = core.indicators:create("SAR", source, instance.parameters.sar_step, instance.parameters.sar_max);
end

function UpdateIndicators()
    EMAFast:update(core.UpdateLast);
    EMAMedium:update(core.UpdateLast);
    EMASlow:update(core.UpdateLast);
    SAR:update(core.UpdateLast);
end

function CreateCustomActions()
    local exitLongAction = {};
    exitLongAction.ActOnSwitch = true;
    exitLongAction.IsPass = function (source, period, periodFromLast)
        if not SAR.DATA:hasData(period) then
            return nil;
        end
    ...
    exitShortAction.ActOnSwitch = true;
    exitShortAction.IsPass = function (source, period, periodFromLast)
        if not SAR.DATA:hasData(period) then
            return nil;
        end
        --1) SAR Up changes to SAR Down
        return SAR.DATA[period] < source.close[period];
    end
    ...
    enterLongAction.IsPass = function (source, period, periodFromLast)
        if not EMAFast.DATA:hasData(period - 1) or not EMAMedium.DATA:hasData(period - 1) or not EMASlow.DATA:hasData(period - 1) then
            return;
        end
        -- Open Long:
        -- 1) Two Options:
        -- a. EMA Medium > EMA Slow and EMA Fast change from EMA Fast < EMA Medium to EMA Fast > EMA Medium
        -- OR
        -- b. EMA Fast > EMA Slow and EMA Medium change from EMA Medium < EMA Slow to EMA Medium > EMA Slow
        return (EMAMedium.DATA[period] > EMASlow.DATA[period] and core.crossesOver(EMAFast.DATA, EMAMedium.DATA, period)
            or EMAFast.DATA[period] > EMASlow.DATA[period] and core.crossesOver(EMAMedium.DATA, EMASlow.DATA, period))
        -- 2) No open trades (Long or Short)
        -- 3) Spread <= User Selected Value for that Instrument
            and (instance.ask[NOW] - instance.bid[NOW]) / source:pipSize() <= instance.parameters.max_spread;
    end
    ...
    enterShortAction.IsPass = function (source, period, periodFromLast)
        ...
        -- OR
        -- b. EMA Fast < EMA Slow and EMA Medium change from EMA Medium > EMA Slow to EMA Medium < EMA Slow
        return (EMAMedium.DATA[period] < EMASlow.DATA[period] and core.crossesUnder(EMAFast.DATA, EMAMedium.DATA, period)
            or EMAFast.DATA[period] < EMASlow.DATA[period] and core.crossesUnder(EMAMedium.DATA, EMASlow.DATA, period))
        -- 5) No open trades (Long or Short)
        -- 6) Spread <= User Selected Value for that Instrument
            and (instance.ask[NOW] - instance.bid[NOW]) / source:pipSize() <= instance.parameters.max_spread;
    end
...
function Prepare(name_only)
    ...
    custom_id = profile:id() .. "_" .. instance.bid:name() .. "_" .. trading_logic.MainSource:barSize();
