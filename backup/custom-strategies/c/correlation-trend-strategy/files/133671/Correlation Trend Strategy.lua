...
--http://fxcodebase.com/code/viewtopic.php?f=31&t=69818
...
local STRATEGY_NAME = "Correlation Trend Strategy";
function CreateParameters() 
    --create your algorithm strategy.parameters here
    strategy.parameters:addInteger("FastLength", "Fast Length", "", 20)
    strategy.parameters:addInteger("SlowLength", "Slow Length", "", 40);
    strategy.parameters:addDouble("BuyThreshold", "Buy Threshold", "", 0.5);
    strategy.parameters:addDouble("SellThreshold", "Sell Threshold", "", 0);
end

local CorrF, CorrS, BuyThreshold, SellThreshold;
function CreateEntryIndicators(source)
    ...
    local profile = core.indicators:findIndicator("CORRELATION TREND");
    assert(profile ~= nil, "Please, download and install " .. "CORRELATION TREND" .. ".LUA indicator");
    CorrF = core.indicators:create("CORRELATION TREND", source, instance.parameters.FastLength)
    CorrS = core.indicators:create("CORRELATION TREND", source, instance.parameters.SlowLength)
end

function UpdateIndicators()
    CorrF:update(core.UpdateLast);
    CorrS:update(core.UpdateLast);
end

function IsEntryLong(source, period)
    if not CorrF.DATA:hasData(period - 1) then
        return false;
    end
    return core.crossesOver(CorrF.DATA, BuyThreshold, period);
end
function IsEntryShort(source, period)
    if not CorrS.DATA:hasData(period - 1) then
        return false;
    end
    return core.crossesUnder(CorrS.DATA, SellThreshold, period);
end
