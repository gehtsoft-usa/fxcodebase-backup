...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=60974
...
function Init() --The strategy profile initialization
    strategy:name("CCI  Based Alternate Ichimoku Strategy")
    ...
    strategy.parameters:addInteger("W", "Period", "", 15, 2, 1000)
    strategy.parameters:addDouble("BL", "Buy Level", "", 0)
    strategy.parameters:addDouble("SL", "Sell Level", "", 0)
    strategy.parameters:addGroup("Ichimoku CCI Calculation")
    strategy.parameters:addInteger("X", "Period", "", 50, 2, 1000)

    strategy.parameters:addGroup("Ichimoku Calculation")
    strategy.parameters:addInteger("Y", "SSP", "Period Priority Line", 75)
    strategy.parameters:addInteger("Z", "SSK", "Tolerance Second Line", 75)
...
function Prepare(nameOnly)
    ...
    name =
        profile:id() ..
        "( " .. instance.bid:name() .. "," .. W .. "," .. X .. "," .. Y .. "," .. Z .. "," .. CustomID .. " )"
    instance:name(name)
    ...
    assert(core.indicators:findIndicator("CCI CANDLE") ~= nil, "Please, download and install CCI CANDLE.LUA indicator")
    assert(
        core.indicators:findIndicator("CCI ALTERNATE ICHIMOKU") ~= nil,
        "Please, download and install CCI ALTERNATE ICHIMOKU.LUA indicator"
    )
    assert(
        core.indicators:findIndicator("ALTERNATE ICHIMOKU") ~= nil,
        "Please, download and install ALTERNATE ICHIMOKU.LUA indicator"
    )
    ...
    ICH = core.indicators:create("CCI ALTERNATE ICHIMOKU", Source, X, Y, Z)
    CCI = core.indicators:create("CCI", Source, W)
    first = math.max(ICH.DATA:first(), CCI.DATA:first()) + 1
...
3.sl < sell level
4.sl<sb
5.entry cci cross under sa+]]
    if
        ICH.SA[period] > ICH.SB[period] and ICH.SA[period] > BL and ICH.SL[period] > BL and
            ICH.SL[period] > ICH.SB[period] and
            core.crossesOver(CCI.DATA, ICH.SA, period)
     then
        BUY()
        ONE = Source:serial(period)
    elseif
        ICH.SA[period] < ICH.SB[period] and ICH.SA[period] < SL and ICH.SL[period] < SL and
            ICH.SL[period] < ICH.SB[period] and
            core.crossesUnder(CCI.DATA, ICH.SA, period)
     then
        SELL()
        ONE = Source:serial(period)
    end
