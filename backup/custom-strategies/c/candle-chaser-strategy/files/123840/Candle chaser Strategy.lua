...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=67340
...
function Init() --The strategy profile initialization
    strategy:name("Candle chaser Strategy")
    ...
    strategy.parameters:addBoolean("Change", "Only on Sequence Change", "", false)
...
function CreateTradingParameters()
    ...
    strategy.parameters:setFlag("AllowTrade", core.FLAG_ALLOW_TRADE)
	 ...
	 strategy.parameters:addBoolean("DoNotUse", "Prevent re-trade", "", false)
...
function Prepare(nameOnly)
    ...
    DoNotUse = instance.parameters.DoNotUse;
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if ExecutionType == "Live" then
        if ONE == Source:serial(period) and DoNotUse then
            return
        end
    ...
    if
        Source.close[period] > Source.open[period] and
            ((Source.close[period - 1] < Source.open[period - 1] and Change) or (not Change))            
     then
        BUY()
