...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=64623
...
function Init() --The strategy profile initialization
    strategy:name("Corelation Indicator Strategy")
    ...
    strategy.parameters:addString("instrument_1", "Instrument 1", "", "EUR/CHF")
    strategy.parameters:addString("instrument_2", "Instrument 2", "", "EUR/USD")
    strategy.parameters:addString("instrument_3", "Instrument 2", "", "USD/CHF")

    strategy.parameters:addString("TF", "Time frame", "", "m1")
    strategy.parameters:setFlag("TF", core.FLAG_PERIODS)

    strategy.parameters:addGroup("Calculation")
    strategy.parameters:addInteger("Multiplier", "Multiplier", "", 10000)
    strategy.parameters:addDouble("Level", "Entry Level", "", 15)
    strategy.parameters:addDouble("SellLevel", "Exit Level", "", 0.5)
...
local TICK_SOURCE_3 = 3
local BAR_SOURCE_1 = 4
local BAR_SOURCE_2 = 5
local BAR_SOURCE_3 = 5
local UseExit = false
local Multiplier, Level, SellLevel
...
function Prepare(nameOnly)
    ...
    if EntryExecutionType == "Live" or ExitExecutionType == "Live" or UseBreakeven then
        ...
        TickSource_3 =
            ExtSubscribe(
            TICK_SOURCE_3,
            instance.parameters.instrument_3,
            "t1",
            instance.parameters.Type == "Bid",
            "close"
        )
    ...
    Source_3 =
        ExtSubscribe(BAR_SOURCE_3, instance.parameters.instrument_3, TF, instance.parameters.Type == "Bid", "bar")
...
function PrepareTrading()
    ...
    instrument_3.BaseSize =
        core.host:execute("getTradingProperty", "baseUnitSize", instance.parameters.instrument_3, Account)
    ...
    instrument_3.Offer = core.host:findTable("offers"):find("Instrument", instance.parameters.instrument_3).OfferID
    ...
    instrument_3.CanClose =
        core.host:execute("getTradingProperty", "canCreateMarketClose", instance.parameters.instrument_3, Account)
...
function IsTickSource(id)
    return id == TICK_SOURCE_1 or id == TICK_SOURCE_2 or id == TICK_SOURCE_3
end

function IsBarSource(id)
    return id == BAR_SOURCE_1 or id == BAR_SOURCE_2 or id == BAR_SOURCE_3
end
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if IsTickSource(id) and UseBreakeven then
        ...
        DoBreakevenLogic(id, source, period, instrument_3)
    end
    ...
    if EntryExecutionType == "Live" or ExitExecutionType == "Live" then
        ...
        period3 = core.findDate(Source_3.close, TickSource_1:date(period), false)
    else
        ...
        period3 = core.findDate(Source_3.close, Source_1:date(period), false)
    end
    ...
    if ExitExecutionType == "Live" and IsTickSource(id) or ExitExecutionType ~= "Live" and not IsTickSource(id) then
        ExitFunction(period1, period2, period3, now)
    end
end

function ExitFunction(period1, period2, period3, now)
    ...
    local Value = Multiplier * (Source_1.close[period1] - (Source_2.close[period2] * Source_3.close[period3]))

    if Value < -SellLevel then
        if IsDirectDirection then
            exitSpecific("B", instrument_1)
            exitSpecific("B", instrument_2)
            exitSpecific("S", instrument_3)
        else
            exitSpecific("S", instrument_1)
            exitSpecific("S", instrument_2)
            exitSpecific("B", instrument_3)
        end
    end
    ...
    if cookie == 100 then
        ...
        if UseMandatoryClosing and AllowTrade then
            ...
            if now >= ExitTime and now < ExitTime + (ValidInterval / 86400.0) then
                ...
                if haveTrades("B", instrument_3) then
                    exitSpecific("B", instrument_3)
                    Signal("Close Long")
                end
                ...
                if haveTrades("S", instrument_3) then
                    exitSpecific("S", instrument_3)
                    Signal("Close Short")
                end
            end
