...
function CreateTradingParameters()
    ...
    strategy.parameters:addInteger("MaxNumberOfPosition", "Max Number Of Position In One Direction", "", 100, 1, 100)
...
local Source, TickSource, trading_source
...
function Prepare(nameOnly)
    ...
    trading_source = ExtSubscribe(2, nil, instance.parameters.TradingTF, instance.parameters.Type == "Bid", "bar")
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if closeNeeded
        ...
        or core.crosses(Source.close, ICH.SA, period)
        or core.crosses(Source.close, ICH.SB, period)
    then
        trading:FindTrade()
        :WhenInstrument(source:instrument())
        :Do(
    ...
    if ONE == trading_source:serial(NOW) then
        return
    end
