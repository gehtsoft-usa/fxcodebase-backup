...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=14626
--+------------------------------------------------------------------+
...
function Init() --The strategy profile initialization
    strategy:name("Candle meter strategy")
    strategy:description("Candle meter strategy")
    strategy:setTag("NonOptimizableParameters", "Email,SendEmail,SoundFile,RecurrentSound,PlaySound, ShowAlert")

    strategy.parameters:addGroup("Strategy Parameters")
    strategy.parameters:addInteger("OpenNumber", "Number in the same direction for open", "", 4, 1, 100)
    strategy.parameters:addInteger("N", "Number of periods for SuperTrend indicator", "", 10)
    strategy.parameters:addDouble("M", "Multiplier for SuperTrend indicator", "", 1.5)
...
function Prepare()
    ...
    assert(
        core.indicators:findIndicator("CANDLEMETER") ~= nil,
        "Please, download and install CANDLEMETER.LUA indicator"
    )
    assert(
        core.indicators:findIndicator("SUPERTREND_DOT") ~= nil,
        "Please, download and install SUPERTREND_DOT.LUA indicator"
    )

    Source = ExtSubscribe(2, nil, instance.parameters.TF, true, "bar")
    CM = core.indicators:create("CANDLEMETER", Source)
    ST = core.indicators:create("SUPERTREND_DOT", Source, instance.parameters.N, instance.parameters.M)
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    CM:update(core.UpdateLast)
    ST:update(core.UpdateLast)

    -- Check that we have enough data
    if (CM.DATA:first() > (period - 1)) then
        return
    end
    ...
    if CM.DATA[period] == instance.parameters.OpenNumber and ST.UP[period] > ST.DN[period] then
    ...
    if CM.DATA[period] == -instance.parameters.OpenNumber and ST.DN[period] > ST.UP[period] then
        if instance.parameters.TypeSignal == "direct" then
            MustOpenS = true
        else
            MustOpenB = true
