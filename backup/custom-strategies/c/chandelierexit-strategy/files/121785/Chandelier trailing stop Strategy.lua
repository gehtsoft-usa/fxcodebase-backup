...
function ExtUpdate(id, source, period)  -- The method called every time when a new bid or ask price appears.
	...
	if Indicator.DATA:size() <= math.abs(x_shift) then
		return;
	end
        ...
        trading:FindTrade()
            ...
            :Do(
                function (trade)
                    ...
                    local newStop;
                    if (trade.BS == "B") then
                        newStop = Indicator.DATA[NOW + x_shift] + y_shift * core.host:findTable("offers"):find("Instrument", trade.Instrument).PointSize
                    else
                        newStop = Indicator.DATA[NOW + x_shift] - y_shift * core.host:findTable("offers"):find("Instrument", trade.Instrument).PointSize;
                    end
