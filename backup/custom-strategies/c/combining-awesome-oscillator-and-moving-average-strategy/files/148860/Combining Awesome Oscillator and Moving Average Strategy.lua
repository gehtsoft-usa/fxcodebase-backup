...
-- https://fxcodebase.com/code/viewtopic.php?f=31&t=73093
...
function Init() --The strategy profile initialization
    strategy:name("Combining Awesome Oscillator and Moving Average Strategy");
    ...
    strategy.parameters:addBoolean("Cross", "Cross Only", "", true);		
    strategy.parameters:addInteger("Period", "Period", "", 8);  	
    ...
    strategy.parameters:addBoolean("Fractal", "Fractal Only", "", true);		
    strategy.parameters:addInteger("Period1", "1. Period", "", 5);  
    strategy.parameters:addInteger("Period2", "2. Period", "", 34);  	
...
function CreateTradingParameters()
   ...
   -- strategy.parameters:addStringAlternative("ExitExecutionType", "End of Turn", "", "EndOfTurn");
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "CAOAMOS");
...
function Prepare( nameOnly)
    ...
    AO = core.indicators:create("AO", Source, Period1, Period2 );	
    MA = core.indicators:create(Method, Source.close, Period  );	 
	first=math.max(AO.DATA:first(),MA.DATA:first()); 
...
function EntryFunction( id, now,period)
	...
	and (AO.DATA[period-1]<  AO.DATA[period-2]or not Fractal)
	and Source.close[period]> MA.DATA[period]
	and (Source.close[period-1]<= MA.DATA[period-1]	or not Cross)
	then	
          if Direction then
                BUY();
            else
		...
		Return=true;
    elseif  AO.DATA[period]<  AO.DATA[period-1]
	and (AO.DATA[period-1]>  AO.DATA[period-2] or not Fractal)
	and Source.close[period]< MA.DATA[period]
	and (Source.close[period-1]>= MA.DATA[period-1]	or not Cross)	
	then
