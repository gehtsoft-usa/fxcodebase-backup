...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=65669
...
function Init() --The strategy profile initialization
    strategy:name("CCI with TEMA Filter Strategy ");
    ...
    strategy.parameters:addInteger("Frame", "Period", "", 50,2,2000);
	
	strategy.parameters:addGroup("CCI Calculation");
    strategy.parameters:addInteger("Period", "Period", "", 14,2,2000);
	strategy.parameters:addDouble("BL", "Buy Level", "", 100);
...
function CreateTradingParameters()
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "CCIWTEMAFS");
...
function Prepare(nameOnly)
    ...
    TEMA = core.indicators:create("TEMA", Source.close, Frame );
	CCI = core.indicators:create("CCI", Source, Period );
	first=math.max(TEMA.DATA:first(),CCI.DATA:first());
...
function EntryFunction( now,period)
    ...
    if   Source.close[period]> TEMA.DATA[period]
	and CCI.DATA[period] > BL
	and CCI.DATA[period-1] <= BL
	then	
          if Direction then
                BUY();
            else
		...
		Return=true;
    elseif Source.close[period]< TEMA.DATA[period]
	and CCI.DATA[period] < SL
	and CCI.DATA[period-1] >= SL
		then
            if Direction then
                SELL();
            else
