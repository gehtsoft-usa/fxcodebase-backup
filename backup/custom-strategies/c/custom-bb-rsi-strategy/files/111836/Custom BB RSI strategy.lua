...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=64567
...
function Init() --The strategy profile initialization
    strategy:name("Custom BB RSI strategy")
    ...
    strategy.parameters:addInteger("R1", "RSI Period", "", 14)
    strategy.parameters:addInteger("B1", "Bollinger Period", "", 20)
    strategy.parameters:addDouble("D1", "Bollinger Deviation", "", 1)
    ...
    strategy.parameters:addInteger("R2", "RSI Period", "", 14)
    strategy.parameters:addInteger("B2", "Bollinger Period", "", 20)
    strategy.parameters:addDouble("D2", "Bollinger Deviation", "", 1)
...
function Prepare(nameOnly)
    ...
    assert(TF1 ~= "t1", "1. time frame must not be tick")
    assert(TF2 ~= "t1", "2. time frame must not be tick")
    ...
    local s1, e1, s2, e2
    s1, e1 = core.getcandle(TF1, 0, 0, 0)
    s2, e2 = core.getcandle(TF2, 0, 0, 0)
    assert((e1 - s1) <= (e2 - s2), "2. time frame must be equal to or bigger than the 1. time frame!")
    ...
    RSI1 = core.indicators:create("RSI", Source1.close, R1)
    RSI2 = core.indicators:create("RSI", Source2.close, R2)
    BB1 = core.indicators:create("BB", RSI1.DATA, B1, D1)
    BB2 = core.indicators:create("BB", RSI2.DATA, B2, D2)
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if ExitExecutionType == "Live" and id == 1 or ExitExecutionType ~= "Live" and id ~= 1 then
        ExitFunction(now, period1, period2)
    end
end

function ExitFunction(now, period1, period2)
    if not Exit then
        return
    end
    ...
    if not RSI1.DATA:hasData(period1) or not RSI2.DATA:hasData(period2) then
        return
    end
    if not BB1.DATA:hasData(period1) or not BB2.DATA:hasData(period2) then
        return
    end
    ...
    if RSI1.DATA[period1] < BB1.TL[period1] or RSI2.DATA[period2] < BB2.TL[period2] then
        if Direction then
    ...
    if RSI1.DATA[period1] > BB1.BL[period1] and RSI2.DATA[period2] > BB2.BL[period2] then
        if Direction then
            if haveTrades("S") then
                exitSpecific("S")
                Signal("Close Short")
...
-- if the RSI is above upper standard deviation line for both time frames BUY
--If the RSI is below lower standard deviation line for both time frames SELL
...
function EntryFunction(now, period1, period2)
    ...
    if not (RSI1.DATA:hasData(period1) or RSI2.DATA:hasData(period2)) then
        return
    end
    if not (BB1.DATA:hasData(period1) or BB2.DATA:hasData(period2)) then
        return
    end

    -- only buy if we have a fast cross over slow and the price is above the moving averages.
    if
        RSI1.DATA[period1] > BB1.TL[period1] and RSI2.DATA[period2] > BB2.TL[period2] and
            (RSI1.DATA[period1 - 1] < BB1.TL[period1 - 1] or RSI2.DATA[period2 - 1] < BB2.TL[period2 - 1])
     then
        if Direction then
            BUY()
        else
        ...
        Return = true
    elseif
        RSI1.DATA[period1] < BB1.BL[period1] and RSI2.DATA[period2] < BB2.BL[period2] and
            (RSI1.DATA[period1 - 1] > BB1.BL[period1 - 1] or RSI2.DATA[period2 - 1] > BB2.BL[period2 - 1])
     then
        if Direction then
            SELL()
        else
