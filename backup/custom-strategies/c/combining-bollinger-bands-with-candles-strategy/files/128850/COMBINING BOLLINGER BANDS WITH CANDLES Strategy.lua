...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=68948
...
function Init() --The strategy profile initialization
    strategy:name("COMBINING BOLLINGER BANDS WITH CANDLES Strategy");
	...
	strategy.parameters:addDouble("Deviations", "Deviations", "", 2);
	
	strategy.parameters:addInteger("ATR_Period", "ATR Period", "", 20);
	strategy.parameters:addDouble("ATR_Multiplier", "ATR_Multiplier", "", 4);
	
	strategy.parameters:addDouble("RISK_REWARD_LIMIT", "RISK_REWARD_LIMIT", "", 1);
 
	 strategy.parameters:addString("EngulfingType", "Engulfing Type", "", "Close");
    strategy.parameters:addStringAlternative("EngulfingType", "Close", "", "Close");
	strategy.parameters:addStringAlternative("EngulfingType", "Open", "", "Open");
    strategy.parameters:addStringAlternative("EngulfingType", "High/Low", "", "HighLow");
...
function CreateTradingParameters()
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "CBBWCS");
...
local Indicator,RISK_REWARD,RISK_REWARD_LIMIT;
local EngulfingType;
local ATR,ATR_Multiplier;
function Prepare( nameOnly)
    ...
    EngulfingType= instance.parameters.EngulfingType;
	RISK_REWARD_LIMIT= instance.parameters.RISK_REWARD_LIMIT;
  ...
  --Risk Reward
	 ...
	 assert(core.indicators:findIndicator("RISK REWARD") ~= nil, "Please, download and install RISK REWARD.LUA indicator");          
    ...
    Indicator = core.indicators:create("BB", Source.close,  instance.parameters.Period, instance.parameters.Deviations  );
	RISK_REWARD = core.indicators:create("RISK REWARD", Source ,  instance.parameters.Period, instance.parameters.Deviations, instance.parameters.ATR_Period,ATR_Multiplier  ); 
	ATR = core.indicators:create("ATR", Source ,   instance.parameters.ATR_Period  );
	first=math.max(Indicator.DATA:first(),RISK_REWARD.DATA:first()); 
...
function ExtUpdate(id, source, period)  -- The method called every time when a new bid or ask price appears.
   ...
   RISK_REWARD:update(core.UpdateLast);
...
function ExitFunction(id, now,period)
	...
	local StopLong = Source.close[period-1] - (ATR_Multiplier * ATR.DATA[period]);
	local StopShort = Source.close[period-1] + (ATR_Multiplier * ATR.DATA[period]);
	...
	--/////////////////////////////////////////////////////////////////////
		            ...
		            if    Source.close[period]< StopLong  then
					  ...
					  if   Source.close[period] > StopShort then
...
function EntryFunction( id, now,period)
    ...
    if   Source.close[period]> Indicator.BL[period]
	and  Source.open[period]<Indicator.BL[period]
	...
	and (( Source.open[period] <= Source.close[period-1] and EngulfingType== "Close" ) 	
	or
	( Source.open[period] <= Source.open[period-1] and EngulfingType== "Open" ) 
    or
	( Source.open[period] <= Source.low[period-1] and EngulfingType== "HighLow" ) 
	
    )	
	and RISK_REWARD.Long[period]> RISK_REWARD_LIMIT
	then	
          if Direction then
                BUY();
            else
		...
		Return=true;
    elseif  Source.close[period]< Indicator.TL[period]
	and  Source.open[period]>Indicator.TL[period]
	and ( Source.close[period] < Source.open[period] )
	and ( Source.close[period-1] > Source.open[period-1] )
	and ( Source.close[period] < Source.open[period-1] )
	and (( Source.open[period] >= Source.close[period-1]and EngulfingType== "Close"  ) 
	...
	( Source.open[period] >= Source.open[period-1]and EngulfingType== "Open" )
	or
	( Source.open[period] >= Source.high[period-1]and EngulfingType== "HighLow" )
	)
	and RISK_REWARD.Short[period]> RISK_REWARD_LIMIT
