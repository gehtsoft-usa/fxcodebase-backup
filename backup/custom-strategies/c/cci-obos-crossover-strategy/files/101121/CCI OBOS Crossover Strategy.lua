...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=62352
...
function Init() --The strategy profile initialization
    strategy:name("CCI OBOS Crossover Strategy")
    ...
    strategy.parameters:addDouble("OB", "OB Level", "", 50)
    strategy.parameters:addDouble("OS", "OS Level", "", -50)
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if core.crossesOver(cci.DATA, obos.high, period) and (not Filter or (Filter and obos.low[period] < OS)) then
        if Direction then
            BUY()
        else
            SELL()
    ...
    elseif core.crossesUnder(cci.DATA, obos.low, period) and (not Filter or (Filter and obos.high[period] > OB)) then
        if Direction then
            SELL()
        else
            BUY()
