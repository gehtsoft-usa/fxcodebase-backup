...
function Init() --The strategy profile initialization
    strategy:name("Chinkou Span Strategy")
...
function CreateTradingParameters()
    ...
    strategy.parameters:addInteger("TotalNumberOfPosition", "Total number of trades executed by the strategy", "", 5, 1, 10000)	
    strategy.parameters:addDouble("TotalNumberOfPositionTime", "Total number of trades executed by the strategy (Time in Days)", "", 1, 1, 10000)	
...
local ToTime
local TotalNumberOfPositionTime,TotalNumberOfPosition;
...
function Prepare(nameOnly)
    ...
    Indicator = core.indicators:create("ICH", Source, T, K, S)
    first = Indicator.DATA:first() + K
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if Indicator.SL[period - K] < Indicator.CS[period - K] and Indicator.TL[period - K] < Indicator.CS[period - K] then
        if Direction then
            BUY(period)
        else
            SELL(period)
    ...
    elseif Indicator.SL[period - K] > Indicator.CS[period - K] and Indicator.TL[period - K] > Indicator.CS[period - K] then
        if Direction then
            SELL(period)
        else
            BUY(period)
...
function TotalNumberOfPositionCount(BuySell)
    local enum, row
    local count = 0
    ...
    while row ~= nil do
        if  row.AccountID == Account 
			...
			and row.Time >=  (instance.bid:date(NOW) - TotalNumberOfPositionTime)
         then
            count = count + 1
        end
    ...
    while row ~= nil do
		...
		and row.QTXT == CustomID
		and (row.BS == BuySell or BuySell == nil)
		and row.OpenTime >=  (instance.bid:date(NOW) - TotalNumberOfPositionTime)
        then
            count = count + 1
        end
...
function enter(BuySell, period)
	...
	or TotalNumberOfPositionCount(nil) >= TotalNumberOfPosition
	then
        return true
    end
