...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=64250
...
function Init() --The strategy profile initialization
    strategy:name("Chaikin Volatility Oscillator Strategy");
    ...
    strategy.parameters:addInteger("SmoothPeriod", "SmoothPeriod", "Smooth period", 10);
    strategy.parameters:addInteger("ROCPeriod", "ROCPeriod", "ROC period", 10);
...
function CreateTradingParameters()
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "CVOS");
...
local Indicator;
local SmoothPeriod, ROCPeriod, Method;
local BL,SL;
...
function Prepare( nameOnly)
    ...
    if nameOnly then
	 ...
	 assert(core.indicators:findIndicator("CHAIKIN VOLATILITY OSCILLATOR") ~= nil, "Please, download and install CHAIKIN VOLATILITY OSCILLATOR.LUA indicator");          
    ...
    Indicator = core.indicators:create("CHAIKIN VOLATILITY OSCILLATOR", Source, SmoothPeriod,ROCPeriod,Method  );
...
function EntryFunction(period,now)
    ...
    if  Indicator.DATA[period]> BL
	and  Indicator.DATA[period-1]<=BL
	then	
          if Direction then
                BUY();
            else
		...
		Return=true;
    elseif  Indicator.DATA[period]< SL
	and  Indicator.DATA[period-1]>=SL
		then
            if Direction then
                SELL();
            else
