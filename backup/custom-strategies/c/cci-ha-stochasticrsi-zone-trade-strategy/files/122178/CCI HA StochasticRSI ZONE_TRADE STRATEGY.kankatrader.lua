...
function Init() --The strategy profile initialization
    ...
    strategy:description("v.1 2018-11-17")
    ...
    strategy.parameters:addBoolean("use_3_zz", "Use 3-rd ZZ Semafor", "", true)
...
function Prepare(nameOnly)
	...
	use_3_zz=instance.parameters.use_3_zz;
    ...
    if TYPE == "AO" or TYPE == "ZONE" then
        indicator[1] =
            ...
            "AO",
            Source,
            instance.parameters:getInteger("AOFM"),
            instance.parameters:getInteger("AOSM")
        )
    ...
    if TYPE == "AC" or TYPE == "ZONE" then
        indicator[2] =
            ...
            "AC",
            Source,
            instance.parameters:getInteger("ACFM"),
            instance.parameters:getInteger("ACSM"),
            instance.parameters:getInteger("ACM")
        )
    ...
    if use_3_zz then
        ...
        zz = core.indicators:create("3_LEVEL_ZZ_SEMAFOR", Source, p);
    end
...
function GetLastSignal(period)


    if not use_3_zz then
	return 0;
	end
	
    local up = zz:getTextOutput(4);
    local down = zz:getTextOutput(5);
    for i = 0, up:size() - 1 do
        if up:hasData(period - i) then
            return 1, Source:date(period - i);
        end
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if UseStochasticRSI then
        if FindRSI(period) == 1 then
            Signal4 = 1
        elseif FindRSI(period) == -1 then
            Signal4 = -1
        end
    end
	...
	local signal = GetLastSignal(period);
   
    if
        ((Signal1 == 1 and (TYPE == "AO" or TYPE == "ZONE")) or (TYPE ~= "AO" and TYPE ~= "ZONE")) and
            ((Signal2 == 1 and (TYPE == "AC" or TYPE == "ZONE")) or (TYPE ~= "AC" and TYPE ~= "ZONE")) and
            ((Signal3 == 1 and UseHA) or not UseHA) and
            ((Signal4 == 1 and UseStochasticRSI) or not UseStochasticRSI) and
            ((Signal5 == 1 and UseCCI) or not UseCCI)
        and ((use_3_zz or signal == 1) or not use_3_zz)
     then
        if Direction then
            BUY()
        else
    ...
    elseif
        ((Signal1 == -1 and (TYPE == "AO" or TYPE == "ZONE")) or (TYPE ~= "AO" and TYPE ~= "ZONE")) and
            ...
            ((Signal4 == -1 and UseStochasticRSI) or not UseStochasticRSI) and
            ((Signal5 == -1 and UseCCI) or not UseCCI)
        and ((use_3_zz and signal == -1) or not use_3_zz)
     then
        if Direction then
            SELL()
        else
...
function FindRSI(period)
    ...
    for i = period - 1, Source:first(), -1 do
        if indicator[4].K[i] > OS and indicator[4].D[i] > OS and indicator[4].D[i - 1] < OS then
            Return = 1
            break
        end

        if indicator[4].K[i] < OB and indicator[4].D[i] < OB and indicator[4].D[i - 1] > OB then
            Return = -1
            break
        end
    end
