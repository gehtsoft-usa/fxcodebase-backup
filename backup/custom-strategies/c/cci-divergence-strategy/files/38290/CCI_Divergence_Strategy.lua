...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=22114
...
function Init() --The strategy profile initialization
    strategy:name("CCI Divergence strategy")
    strategy:description("CCI Divergence strategy")
...
function Prepare(nameOnly)
    ...
    assert(
        core.indicators:findIndicator("CCI_DIVERGENCE") ~= nil,
        "Please, download and install CCI_DIVERGENCE.LUA indicator"
    )
    Source = ExtSubscribe(2, nil, instance.parameters.TF, true, "bar")
    CCI_D = core.indicators:create("CCI_DIVERGENCE", Source, instance.parameters.CCI_Period, false)
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    CCI_D:update(core.UpdateLast)

    -- Check that we have enough data
    if (CCI_D.DATA:first() > (period - 1)) then
        return
    end
    ...
    local UP = CCI_D.UP[period - 2]
    if UP == nil then
        UP = 0
    end
    local DN = CCI_D.DN[period - 2]
    if DN == nil then
        DN = 0
    end
