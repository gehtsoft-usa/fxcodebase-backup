...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=61969
...
function Init() --The strategy profile initialization
    strategy:name("CCI TREND FRACTAL STRATEGY")
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    Up1, Up2, Down1, Down2 = Fractal(period)

    if Up1 == nil or Up2 == nil or Down1 == nil or Down2 == nil then
        return
    end

    if LastUp ~= Source:serial(Up1) then
        LastUp = Source:serial(Up1)
        UpFlag = true
    end
    if LastDown ~= Source:serial(Down1) then
        LastDown = Source:serial(Down1)
        DownFlag = true
    end

    -- only buy if we have a fast cross over slow and the price is above the moving averages.
    if CCI.DATA[period] > BL and Source.high[Up1] > Source.high[Up2] and UpFlag then
        if Direction then
            BUY()
        else
            SELL()
    ...
    elseif CCI.DATA[period] < SL and Source.low[Down1] < Source.low[Down2] and DownFlag then
        if Direction then
            SELL()
        else
            BUY()
...
function Fractal(period)
    ...
    for i = period, Source.close:first(), -1 do
        ...
        if UpCount == 2 and DownCount == 2 then
            break
        end
    end
