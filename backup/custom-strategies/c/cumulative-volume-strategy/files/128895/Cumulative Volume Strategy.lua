...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=68960
...
function Init() --The strategy profile initialization
    strategy:name("Cumulative Volume Strategy");
...
--	strategy.parameters:addStringAlternative("ExitExecutionType", "Live", "", "Live");
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "CVS");
...
--	  Exit= instance.parameters.Exit;
	...
	--Cumulative Volume
	assert(core.indicators:findIndicator("CUMULATIVE VOLUME") ~= nil, "Please, download and install CUMULATIVE VOLUME.LUA indicator");          
...
--	or ExitExecutionType== "Live"
    ...
    Indicator = core.indicators:create("CUMULATIVE VOLUME", Source, Period, false, true );
...
function EntryFunction( id, now,period)
    ...
    if   Indicator.P[period]> 50
	and   Indicator.P[period-1]<= 50
	then	
          if Direction then
                BUY();
            else
		...
		Return=true;
    elseif   Indicator.N[period]< -50
	and   Indicator.N[period-1]>= -50
		then
            if Direction then
                SELL();
            else
