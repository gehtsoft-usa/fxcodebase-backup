...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=65693
...
function Init() --The strategy profile initialization
    strategy:name("Centred moving average Channel Strategy");
...
function CreateTradingParameters()
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "CMACS");
	...
	strategy.parameters:addDouble("Profit", "Profit Exit Level in Base Currency", "", 1000);
...
function CalculateProfit(BuySell ) 
    local enum, row; 
	local Return=0;
...
function ExitFunction( now,period)
	 ...
	 return;
	  ...
	  local ProfitLevelLong=CalculateProfit("B");
      local ProfitLevelShort=CalculateProfit("S");

		            if    ProfitLevelLong > Profit then
					    ...
					    if    ProfitLevelShort > Profit then
...
--((source.open( 0 )<CMAC.Top( 0 )) .and. (source.close( 0 ) > CMAC.Top( 0 )))
    ...
    if   Source.open[period]> Indicator.Bottom[period]
	and Source.close[period]< Indicator.Bottom[period]
	then	
          if Direction then
                BUY();
            else
		...
		Return=true;
    elseif   Source.open[period]< Indicator.Top[period]
	and Source.close[period]> Indicator.Top[period]
		then
            if Direction then
                SELL();
            else
