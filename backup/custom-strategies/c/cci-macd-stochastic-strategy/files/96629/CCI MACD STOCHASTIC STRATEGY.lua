...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=61347
...
function Init() --The strategy profile initialization
    strategy:name("CCI MACD STOCHASTIC STRATEGY")
    ...
    strategy.parameters:addInteger("CCIPeriod", "Period", "", 50, 2, 1000)
    strategy.parameters:addDouble("CCIBuy", "Buy Level", "", 0, -100, 100)
    strategy.parameters:addDouble("CCISell", "Sell Level", "", 0, -100, 100)

    strategy.parameters:addGroup("MACD Parameters")
    strategy.parameters:addInteger("MACDShort", "Short Period", "", 12, 2, 1000)
    strategy.parameters:addInteger("MACDLong", "Long Period", "", 26, 2, 1000)
    strategy.parameters:addDouble("MACDBuy", "Buy Level", "", 0)
    strategy.parameters:addDouble("MACDSell", "Sell Level", "", 0)
    ...
    strategy.parameters:addDouble("StochasticOB", "Stochastic OverBought Level", "", 80, 0, 100)
    strategy.parameters:addDouble("StochasticOS", "Sell OverSold Level", "", 20, 0, 100)
...
local first
local CCIPeriod, CCIBuy, CCISell, CCI
local MACD, MACDShort, MACDLong, MACDBuy, MACDSell
local CustomID
local STOCHASTIC, averageTypeK, k, sd, d, averageTypeD
...
function Prepare(nameOnly)
    ...
    CCIBuy = instance.parameters.CCIBuy
    CCISell = instance.parameters.CCISell
    MACDLong = instance.parameters.MACDLong
    MACDShort = instance.parameters.MACDShort
    MACDBuy = instance.parameters.MACDBuy
    MACDSell = instance.parameters.MACDSell
    StochasticOB = instance.parameters.StochasticOB
    StochasticOS = instance.parameters.StochasticOS

    k = instance.parameters.K
    d = instance.parameters.K
    ...
    MACD = core.indicators:create("MACD", Source.close, MACDShort, MACDLong)
    STOCHASTIC = core.indicators:create("STOCHASTIC", Source, k, sd, d, averageTypeK, averageTypeD)
    first = math.max(CCI.DATA:first(), MACD.DATA:first(), STOCHASTIC.DATA:first()) + 1
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if
        CCI.DATA[period] > CCIBuy and MACD.DATA[period] > MACDBuy and
            core.crossesOver(STOCHASTIC.DATA, StochasticOB, period)
     then
        BUY()
        ONE = Source:serial(period)
    elseif
        CCI.DATA[period] < CCISell and MACD.DATA[period] < MACDSell and
            core.crossesUnder(STOCHASTIC.DATA, StochasticOS, period)
     then
        SELL()
        ONE = Source:serial(period)
    end
