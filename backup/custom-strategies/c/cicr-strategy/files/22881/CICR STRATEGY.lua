...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=11290
...
function Init() --The strategy profile initialization
    ...
    strategy.parameters:addInteger("EMA", "EMA Period", "", 14)
    strategy.parameters:addGroup("RSI Calculation")
    strategy.parameters:addInteger("RSI", "RSI Period", "", 3)
    strategy.parameters:addDouble("BuyLevel", "RSI Buy Level", "", 50)
    strategy.parameters:addDouble("SellLevel", "RSI Sell Level", "", 50)
    strategy.parameters:addGroup("ATR Calculation")
    strategy.parameters:addInteger("ATR", "ATR Period", "", 14)
    strategy.parameters:addDouble("ATRMultiplier", "ATR Multiplier", "", 2)
    strategy.parameters:addBoolean("Trailing", "Use Trailing Stop", "", false)

    strategy.parameters:addGroup("MACD Calculation")

    strategy.parameters:addInteger("One", "Short EMA", "", 12, 2, 1000)
    strategy.parameters:addInteger("Two", "Long EMA", "", 26, 2, 1000)
    strategy.parameters:addInteger("Three", "Signal line", "", 9, 2, 1000)
...
function Prepare(nameOnly)
    ...
    Parameters["EMA"] = instance.parameters.EMA

    Parameters["RSI"] = instance.parameters.RSI
    Parameters["BuyLevel"] = instance.parameters.BuyLevel
    Parameters["SellLevel"] = instance.parameters.SellLevel

    Parameters["ATR"] = instance.parameters.ATR
    Parameters["ATRMultiplier"] = instance.parameters.ATRMultiplier

    Parameters["One"] = instance.parameters.One
    Parameters["Two"] = instance.parameters.Two
    Parameters["Three"] = instance.parameters.Three
                            ...
                            Parameters["ATR"] ..
                                ", " ..
                                    Parameters["ATRMultiplier"] ..
                                        ", " ..
                                            Parameters["One"] ..
                                                ", " .. Parameters["Two"] .. ", " .. Parameters["Three"]
    ...
    Indicator["EMA"] = core.indicators:create("EMA", Source[Price], Parameters["EMA"])
    Short["EMA"] = Indicator["EMA"]:getStream(0)

    Indicator["RSI"] = core.indicators:create("RSI", Source[Price], Parameters["RSI"])
    Short["RSI"] = Indicator["RSI"]:getStream(0)

    Indicator["ATR"] = core.indicators:create("ATR", Source, Parameters["ATR"])
    Short["ATR"] = Indicator["ATR"]:getStream(0)

    Indicator["HA"] = core.indicators:create("HA", Source)
    Short["OPEN"] = Indicator["HA"]:getStream(0)
    Short["CLOSE"] = Indicator["HA"]:getStream(3)

    Indicator["MACD"] =
        core.indicators:create("MACD", Source[Price], Parameters["One"], Parameters["Two"], Parameters["Three"])
    Short["MACD"] = Indicator["MACD"]:getStream(0)
    Short["SIGNAL"] = Indicator["MACD"]:getStream(1)
    Short["HISTOGRAM"] = Indicator["MACD"]:getStream(2)
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    Indicator["HA"]:update(core.UpdateLast)
    Indicator["EMA"]:update(core.UpdateLast)
    ...
    --1. price> ema and
    --2. macd > 0 and
    --3. heikan ashi green in color
    --4. rsi cross over buy level{0}

    if
        Source[Price][period] > Short["EMA"][period] and Short["MACD"][period] > 0 and
            Indicator["HA"].close[period] > Indicator["HA"].open[period] and
            Short["RSI"][period] > Parameters["BuyLevel"]
     then
        TRADE = "Long"
        EXIT["Long"] = Source[Price][period] - Short["ATR"][period] * Parameters["ATRMultiplier"]
    ...
    elseif
        Source[Price][period] < Short["EMA"][period] and Short["MACD"][period] < 0 and
            Indicator["HA"].close[period] < Indicator["HA"].open[period] and
            Short["RSI"][period] < Parameters["SellLevel"]
     then
        TRADE = "Short"
        EXIT["Short"] = Source[Price][period] + Short["ATR"][period] * Parameters["ATRMultiplier"]
    ...
    if Trailing then
        if TRADE == "Long" and Source[Price][period] - Short["ATR"][period] * Parameters["ATRMultiplier"] > EXIT["Long"] then
            EXIT["Long"] = Source[Price][period] - Short["ATR"][period] * Parameters["ATRMultiplier"]
        elseif
            TRADE == "Short" and
                Source[Price][period] + Short["ATR"][period] * Parameters["ATRMultiplier"] < EXIT["Short"]
         then
            EXIT["Short"] = Source[Price][period] + Short["ATR"][period] * Parameters["ATRMultiplier"]
        end
    end

    if EXIT["Long"] ~= nil and core.crossesUnder(Source[Price], EXIT["Long"], period) and TRADE == "Long" then
        if Direction then
        ...
        TRADE = "NO"
    elseif EXIT["Short"] ~= nil and core.crossesOver(Source[Price], EXIT["Short"], period) and TRADE == "Short" then
        if Direction then
            if haveTrades("S") then
                exit("S")
                Signal("Close Short")
