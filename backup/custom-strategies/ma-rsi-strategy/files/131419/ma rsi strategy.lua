...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=69436
...
local STRATEGY_NAME = "MA RSI Strategy";
function CreateParameters() 
    ...
    strategy.parameters:addDouble("rsi_buy_level", "RSI Buy Level", "", 50);
    strategy.parameters:addDouble("rsi_sell_level", "RSI Sell Level", "", -50);
end

local ma, rsi, rsi_buy_level, rsi_sell_level;
...
function EntryLongRule(source, period)
    return source.close[period] > ma.DATA[period] and rsi.DATA[period] > rsi_buy_level;
end

function EntryShortRule(source, period)
    return source.close[period] < ma.DATA[period] and rsi.DATA[period] < rsi_sell_level;
end

function IsEntryLong(source, period)
    return EntryLongRule(source, period) and not EntryLongRule(source, period - 1);
end
function IsEntryShort(source, period)
    return EntryShortRule(source, period) and not EntryShortRule(source, period - 1);
end
function IsExitLong(source, period)
    return source.close[period] < ma.DATA[period] or rsi.DATA[period] < rsi_buy_level;
end
function IsExitShort(source, period)
    return source.close[period] > ma.DATA[period] or rsi.DATA[period] > rsi_sell_level;
end
