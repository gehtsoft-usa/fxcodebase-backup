...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=27800
...
function Init() --The strategy profile initialization
    strategy:name("Smoothed Candles Strategy")
...
function Prepare(nameOnly)
    ...
    Indicator["O"] = core.indicators:create(Method, Source.open, Period)
    Indicator["C"] = core.indicators:create(Method, Source.close, Period)
    Indicator["H"] = core.indicators:create(Method, Source.high, Period)
    Indicator["L"] = core.indicators:create(Method, Source.low, Period)
    Short["O"] = Indicator["O"]:getStream(0)
    Short["C"] = Indicator["C"]:getStream(0)
    Short["H"] = Indicator["H"]:getStream(0)
    Short["L"] = Indicator["L"]:getStream(0)
    first = Short["O"]:first()
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    Indicator["O"]:update(core.UpdateLast)
    Indicator["C"]:update(core.UpdateLast)
    Indicator["H"]:update(core.UpdateLast)
    Indicator["L"]:update(core.UpdateLast)

    if period < first + 1 then
        return
    end

    if Short["C"][period] > Short["O"][period] and Short["C"][period - 1] < Short["O"][period - 1] then
        if Direction then
            BUY()
        else
            SELL()
        end
    elseif Short["C"][period] < Short["O"][period] and Short["C"][period - 1] > Short["O"][period - 1] then
        if Direction then
            SELL()
        else
            BUY()
