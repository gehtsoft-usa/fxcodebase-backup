...
--http://fxcodebase.com/code/viewtopic.php?f=31&t=69416
...
function CreateParameters() 
    ...
    strategy.parameters:addDouble("Pcnt", "Pcnt", "", 0.15);
end

local indi, ama, l, h;
function CreateEntryIndicators(source)
    local profile = core.indicators:findIndicator("ADAPTIVE_MA FILTER");
    assert(profile ~= nil, "Please, download and install " .. "ADAPTIVE_MA FILTER" .. ".LUA indicator");
    local indicatorParams = profile:parameters();
    indi = core.indicators:create("ADAPTIVE_MA FILTER", source, instance.parameters.period, instance.parameters.Pcnt);
    ...
    local indicatorParams = profile:parameters();
    ama = core.indicators:create("ADAPTIVE_MA", source, instance.parameters.period);
end

function UpdateIndicators()
    indi:update(core.UpdateLast);
    ama:update(core.UpdateLast);
    if not ama.DATA:hasData(NOW - 1) then
        return;
    end
    if l == nil then
        l = ama.DATA[NOW];
        ...
        if ama.DATA[NOW] < ama.DATA[NOW - 1] then
            l = ama.DATA[NOW];
        end
        if ama.DATA[NOW] > ama.DATA[NOW - 1] then
            h = ama.DATA[NOW];
...
function IsEntryLong(source, period)
    ...
    return core.crossesUnder(indi.DATA, ama.DATA[period] - l, period);
end
function IsEntryShort(source, period)
    if h == nil then
        return;
    end
    return core.crossesUnder(indi.DATA, h - ama.DATA[period], period);
end
