...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=66293
...
function Init() --The strategy profile initialization
    strategy:name("Multi Z-Score Strategy");
	...
	strategy.parameters:addDouble("Definition", "definition", "", 100);
...
function CreateTradingParameters()
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "MZSS");
...
local Indicator;
local Method,Period,Definition,BL,SL;
...
function Prepare( nameOnly)
	...
	assert(core.indicators:findIndicator("MULTI Z-SCORE") ~= nil, "Please, download and install MULTI Z-SCORE.LUA indicator");          
    ...
    Indicator = core.indicators:create("MULTI Z-SCORE", Source.close, Method,Period,Definition  );
...
function EntryFunction( now,period)
    ...
    if   Indicator.DATA[period]>Indicator.Signal[period]
	and  Indicator.DATA[period-1]<=Indicator.Signal[period-1]
	and Indicator.Signal[period] > BL
	then	
          if Direction then
                BUY();
            else
		...
		Return=true;
    elseif   Indicator.DATA[period]<Indicator.Signal[period]
	and  Indicator.DATA[period-1]>=Indicator.Signal[period-1]
	and Indicator.Signal[period] < SL
		then
            if Direction then
                SELL();
            else
