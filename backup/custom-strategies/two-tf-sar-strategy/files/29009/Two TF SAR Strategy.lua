...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=15391&p
...
function Init() --The strategy profile initialization
    strategy:name("Two TF SAR Strategy")
...
function TIMEFRAME(id, FRAME)
    ...
    strategy.parameters:addDouble(tostring(id * 100 + 2), "Step", "", 0.02, 0.001, 1)
    strategy.parameters:addDouble(tostring(id * 100 + 3), "Max", "", 0.2, 0.001, 10)
end
...
function Prepare(nameOnly)
    ...
    for i = 1, TfNumber, 1 do
        ...
        Parameters[tostring(i * 100 + 2)] = instance.parameters:getDouble(tostring(i * 100 + 2))
        Parameters[tostring(i * 100 + 3)] = instance.parameters:getDouble(tostring(i * 100 + 3))
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    for i = 1, TfNumber, 1 do
        if
            Indicator[i].UP:size() <= Indicator[i].UP:first() + 3 or
                Indicator[i].DN:size() <= Indicator[i].DN:first() + 3
         then
            FLAG = false
        end
    end
    ...
    if
        Indicator[1].DN:hasData(Indicator[1].DN:size() - 2) and not Indicator[1].DN:hasData(Indicator[1].DN:size() - 3) and
            Indicator[2].DN:hasData(Indicator[2].DN:size() - 2)
     then
        if Direction then
            BUY()
        else
    ...
    elseif
        Indicator[1].UP:hasData(Indicator[1].UP:size() - 2) and not Indicator[1].UP:hasData(Indicator[1].UP:size() - 3) and
            Indicator[2].UP:hasData(Indicator[2].UP:size() - 2)
     then
        if Direction then
            SELL()
        else
    ...
    if Exit then
        if
            (not Indicator[1].DN:hasData(Indicator[1].DN:size() - 2) and (ExitType == "First" or ExitType == "Any")) or
                (not Indicator[2].DN:hasData(Indicator[2].DN:size() - 2) and (ExitType == "Second" or ExitType == "Any"))
         then
            if Direction then
        ...
        if
            (not Indicator[1].UP:hasData(Indicator[1].UP:size() - 2) and (ExitType == "First" or ExitType == "Any")) or
                (not Indicator[2].UP:hasData(Indicator[2].UP:size() - 2) and (ExitType == "Second" or ExitType == "Any"))
         then
            if Direction then
                if haveTrades("S") then
                    exit("S")
