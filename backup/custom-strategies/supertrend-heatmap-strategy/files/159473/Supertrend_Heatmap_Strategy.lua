-- Available @ https://fxcodebase.com/code/viewtopic.php?f=31&t=75997
...
local STRATEGY_NAME = "Supertrend Heatmap Strategy";
...
function CreateParameters() 
    strategy.parameters:addGroup("Supertrend Parameters");
    ...
    strategy.parameters:addGroup("Timeframe Selection");
    for _, tf in ipairs(timeframes) do
        strategy.parameters:addBoolean("use_" .. tf.name, "Use " .. tf.name, "", tf.enabled);
    end
end

function CreateEntryIndicators(source)
    --create your indicators here
    local profile = core.indicators:findIndicator("SUPERTREND");
    assert(profile ~= nil, "Please, download and install SUPERTREND.LUA indicator");
    
    for _, tf in ipairs(timeframes) do
        if instance.parameters["use_" .. tf.name] then
            local tf_source = ExtSubscribe(#indicators + 1, nil, tf.value, instance.parameters.type, "bar");
            local indicatorParams = profile:parameters();
            indicatorParams:setInteger("N", instance.parameters.N);
            indicatorParams:setDouble("M", instance.parameters.M);
            indicators[#indicators + 1] = core.indicators:create("SUPERTREND", tf_source, indicatorParams);
        end
    end
end
...
function IsEntryLong(source, period)
    for i, v in ipairs(indicators) do
        if not v.DATA:hasData(NOW) or  v.DATA:colorI(NOW) ~=  core.rgb(0, 255, 0) then
            return false;
        end
    end
...
function IsEntryShort(source, period)
    for i, v in ipairs(indicators) do
        if not v.DATA:hasData(NOW) or  v.DATA:colorI(NOW)  ~=  core.rgb(255, 0, 0) then
            return false;
        end
    end
