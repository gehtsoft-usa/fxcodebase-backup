...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=62587
...
function Init() --The strategy profile initialization
    strategy:name("MACD Histogram Level Cross Strategy")
    ...
    strategy.parameters:setFlag("TF", core.FLAG_BARPERIODS_EDIT)

    strategy.parameters:addGroup("Calculation")
    strategy.parameters:addDouble("BuyLevel", "Buy Level", " ", 0.0002)
    strategy.parameters:addDouble("SellLevel", "Sell Level", " ", -0.0002)
...
function Prepare(nameOnly)
    ...
    MACD = core.indicators:create("MACD", Source[PriceType], SN, LN, IN)

    if Filter then
        MA = core.indicators:create(Method, Source[PriceType], Period)
    end
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if MACD.HISTOGRAM[period] > BuyLevel and MACD.HISTOGRAM[period - 1] <= BuyLevel then
        exitSpecific("S");
        if (not Filter or (Filter and Source.close[period] > MA.DATA[period])) then
            if Direction then
                BUY()
            else
                SELL()
    ...
    if (not Filter or (Filter and Source.close[period] < MA.DATA[period])) then
        exitSpecific("B");
        if MACD.HISTOGRAM[period] < SellLevel and MACD.HISTOGRAM[period - 1] >= SellLevel then
            if Direction then
                SELL()
            else
                BUY()
