-- Available @ https://fxcodebase.com/code/viewtopic.php?f=31&t=76089
...
function Init() --The strategy profile initialization
    strategy:name("Previous day Bounce with MA Confirmation Strategy");
    ...
    strategy.parameters:addBoolean("PDHL", "Close > PDH Close < PDL", "", true);
    strategy.parameters:addBoolean("PDC", "Close > PDC Close < PDC", "", true);
    strategy.parameters:addBoolean("PDR", "Previous candle Reversal", "", true);	
    strategy.parameters:addBoolean("MA", "Close > MA  Close < MA", "", true);	
    strategy.parameters:addBoolean("Range", "PDHL Filter", "", true);		
    strategy.parameters:addDouble("Multiplier", "Multiplier", "", 0.25, 0, 1 );		
	
	
	
	strategy.parameters:addGroup("Calculation"); 	
	strategy.parameters:addString("TF2", "Bar Size to display High/Low", "", "D1");
...
function Prepare( nameOnly)
	...
	ma = core.indicators:create(Method, Source1[Price], Period);		
	first=ma.DATA:first();	 
...
function ExtUpdate(id, source, period)  -- The method called every time when a new bid or ask price appears.
   ...
   -- get only time
	...
	if not Source1.close:hasData( period1)	
	or period1 < first 
	then
        return;
    end
...
function EntryFunction( now,period1,  period2)
	...
	local range= math.max( Source2.high[period2-1]-Source2.low[period2-1])	 
	  
    if (Source1.close[period1]> ma.DATA[period1]  and MA or not MA)
		and (Source1.open[period1]< (ma.DATA[period1] + range* Multiplier)  and Range or not Range)
		and (Source1.close[period1]> Source1.open[period1]
		and Source1.close[period1-1]< Source1.open[period1-1] and PDR or not PDR)
		and (Source1.close[period1] > Source2.high[period2-1] and PDHL or not PDHL)
		and (Source1.close[period1] > Source2.close[period2-1] and PDC or not PDC)		
	...
	then	
		...
		Return=true;
    elseif (Source1.close[period1]< ma.DATA[period1]  and MA or not MA)
		and (Source1.open[period1]> (ma.DATA[period1] - range* Multiplier)  and Range or not Range)		
		and (Source1.close[period1]< Source1.open[period1]
		and Source1.close[period1-1]> Source1.open[period1-1]  and PDR or not PDR)	
		and (Source1.close[period1] < Source2.low[period2-1] and PDHL or not PDHL)
		and (Source1.close[period1] < Source2.close[period2-1] and PDC or not PDC)	
		then
            if Direction then
                SELL();
