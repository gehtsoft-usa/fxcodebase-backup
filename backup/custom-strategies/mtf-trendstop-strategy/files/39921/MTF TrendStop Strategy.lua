...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=23181
...
local FRAMES = {"m15", "H1", "H4", "D1"}
local TfNumber = 4

function Init() --The strategy profile initialization
    strategy:name("MTF TrendStop Strategy")
...
function TIMEFRAME(id, FRAME)
    ...
    strategy.parameters:addInteger("Period" .. id, "Period", "Period", 10)
    strategy.parameters:addString("CrossType" .. id, "Triger", "", "Close")
    strategy.parameters:addStringAlternative("CrossType" .. id, "Close", "", "Close")
    strategy.parameters:addStringAlternative("CrossType" .. id, "High/Low", "", "High")
end
...
function Prepare(nameOnly)
    ...
    for i = 1, TfNumber, 1 do
        ...
        Type[i] = instance.parameters:getString("CrossType" .. i)
        TF[i] = instance.parameters:getString("TF" .. i)

        assert(TF[i] ~= "t1", "The time frame must not be tick")

        name = name .. "(" .. TF[i] .. ", " .. Period[i] .. ", " .. Type[i] .. ") "

        assert(
            core.indicators:findIndicator("TRENDSTOP") ~= nil,
            "Please, download and install TRENDSTOP.BIN indicator"
        )

        Source[i] = ExtSubscribe(i, nil, TF[i], instance.parameters.Type == "Bid", "bar")

        Indicator[i] = core.indicators:create("TRENDSTOP", Source[i], Period[i], Type[i])
    end
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    --4.D1 Trendstop long
    --3.H4 Trendstop long
    --2.H1 Trendstop short
    --1.m15 Trendstop goes from short to long (this is my buy signal)

    if
        core.crossesOver(Source[1].close, Indicator[1].DATA, Source[1].close:size() - 2, Indicator[1].DATA:size() - 2) and
            (Indicator[2].DATA[Indicator[2].DATA:size() - 2] > Source[2].close[Source[2].close:size() - 2] or not ON[2]) and
            (Indicator[3].DATA[Indicator[3].DATA:size() - 3] < Source[3].close[Source[3].close:size() - 2] or not ON[3]) and
            (Indicator[4].DATA[Indicator[4].DATA:size() - 4] < Source[4].close[Source[4].close:size() - 2] or not ON[4])
     then
        --D1 Trendstop short
        --H4 Trendstop short
        --H1 Trendstop long
    ...
    elseif
        core.crossesUnder(Source[1].close, Indicator[1].DATA, Source[1].close:size() - 2, Indicator[1].DATA:size() - 2) and
            (Indicator[2].DATA[Indicator[2].DATA:size() - 2] < Source[2].close[Source[2].close:size() - 2] or not ON[2]) and
            (Indicator[3].DATA[Indicator[3].DATA:size() - 3] > Source[3].close[Source[3].close:size() - 2] or not ON[3]) and
            (Indicator[4].DATA[Indicator[4].DATA:size() - 4] > Source[4].close[Source[4].close:size() - 2] or not ON[4])
     then
