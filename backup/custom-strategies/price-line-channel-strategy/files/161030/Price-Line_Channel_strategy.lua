-- Available @ https://fxcodebase.com/code/viewtopic.php?f=31&t=76401
--
-- ── Author ─────────────────────────────────────────────────────────────────────
...
local STRATEGY_NAME = "Price-Line Channel Strategy";
function CreateParameters() 
    strategy.parameters:addInteger("length", "Period", "", 50, 1, 2000); 
    strategy.parameters:addBoolean("r", "Readjustement", "", true);	
    strategy.parameters:addInteger("Lookback", "Lookback period", "", 0);	
end

local indi_1;
function CreateEntryIndicators(source)
    indi_1 = core.indicators:create("PRICE-LINE CHANNEL", source, instance.parameters.length, instance.parameters.r, instance.parameters.Lookback);
end
...
function IsEntryLong(source, period)
    if not indi_1.DATA:hasData(period - 1) then
        return false;
    end
    return core.crossesOverOrTouch(source.close, indi_1.Lower, period);
end
function IsEntryShort(source, period)
    if not indi_1.DATA:hasData(period - 1) then
        return false;
    end
    return core.crossesUnderOrTouch(source.close, indi_1.Upper, period);
end
...
function Prepare(nameOnly)
    ...
    assert(core.indicators:findIndicator("PRICE-LINE CHANNEL") ~= nil, "Please, download and install PRICE-LINE CHANNEL.LUA indicator");	
