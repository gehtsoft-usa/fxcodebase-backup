...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=62554
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if
        Source.close[period] > math.max(SA[period], SB[period]) and
            Source.close[period - 1] <= math.max(SA[period - 1], SB[period - 1])
     then
        if Direction then
            BUY()
        else
    ...
    elseif
        Source.close[period] < math.min(SA[period], SB[period]) and
            Source.close[period - 1] >= math.min(SA[period - 1], SB[period - 1])
     then
        if Direction then
            SELL()
        else
    ...
    if Exit then
        if Source.close[period] < math.max(SA[period], SB[period]) then
            if Direction then
        ...
        if Source.close[period] > math.min(SA[period], SB[period]) then
            if Direction then
                if haveTrades("S") then
                    exitSpecific("S")
                    Signal("Close Short")
