-- Available @ https://fxcodebase.com/code/viewtopic.php?f=31&t=74303
...
local FRAMES = {"m15", "H2", "D1"}
local TfNumber = 3

function Init() --The strategy profile initialization
    strategy:name("YOUYOUMA MTF StochRSI Strategy");
	...
	strategy.parameters:addGroup("Overbought Oversold Levels");
	strategy.parameters:addDouble("OB", "Overbought", "",  80, 0, 100);
    strategy.parameters:addDouble("OS", "Oversold", "", 20, 0, 100);	
...
function TIMEFRAME(id, FRAME)
   strategy.parameters:addGroup(id .. ". Time Frame");
    ...
    strategy.parameters:addInteger(tostring(id * 100 + 2), "%K Period", "", 3, 1, 1000)
    strategy.parameters:addInteger(tostring(id * 100 + 3), "%D Periof", "", 3, 1, 1000)
    strategy.parameters:addInteger(tostring(id * 100 + 4), "%D slowing periods", "", 5, 1, 1000)


    strategy.parameters:addInteger(tostring(id * 100 + 5), "Oversold level", "", 20, 1, 100)
    strategy.parameters:addInteger(tostring(id * 100 + 6), "Overbought level", "", 80, 1, 100)
...
function Prepare( nameOnly)
    ...
    Indicator[tostring(i*100 + 10)] = core.indicators:create("STOCHASTIC", Source[i], Parameters[tostring(i*100 + 2)], Parameters[tostring(i*100 + 3)], Parameters[tostring(i*100 + 4)]);
...
function ExtUpdate(id, source, period)  -- The method called every time when a new bid or ask price appears.
   ...
   -- get only time
	...
	for i = 1, TfNumber , 1 do	
		if Indicator[tostring(i*100 + 10)].DATA:size() <= Indicator[tostring(i*100 + 10)].DATA:first() + 2 
		then
		   FLAG= false;
		end
	end
...
function ExitFunction(id, now,period)
	...
	--/////////////////////////////////////////////////////////////////  
	if  LastExit == Source[1]:serial(period1) 
	then
	return;
	end	  
	--/////////////////////////////////////////////////////////////////////
	  
    if  tradesCount("B") > 0 then            
                   if (core.crossesUnder(Indicator[tostring(2*100 + 10)].K, Indicator[tostring(2*100 + 10)].D, period2) and tradesProfit("B") > 0  )
				   or (core.crossesUnder(Indicator[tostring(3*100 + 10)].K, Indicator[tostring(3*100 + 10)].D, period3) and tradesProfit("B") < 0  )
				   then
								...
								LastExit= Source[1]:serial(period1);
					  end
	end

    if tradesCount("S") > 0 then	
					  
					      if ( core.crossesOver(Indicator[tostring(2*100 + 10)].K, Indicator[tostring(2*100 + 10)].D,period2)  and tradesProfit("S") > 0 )
						  or  ( core.crossesOver(Indicator[tostring(3*100 + 10)].K, Indicator[tostring(3*100 + 10)].D,period3)  and tradesProfit("S") < 0)
						  then  
...
function EntryFunction( id, now,period)
	...
	--/////////////////////////////////////////////////////////////////    
     if LastEntry == Source[1]:serial(period1)   
	 then
	 return;
	 end	
	 --/////////////////////////////////////////////////////////////////  
    ...
    if core.crossesOver(Indicator[tostring(1*100 + 10)].K, Indicator[tostring(1*100 + 10)].D, period1)
	and Indicator[tostring(1*100 + 10)].K[period1] < OS 
	and
	(
	Indicator[tostring(2*100 + 10)].K[ period2] > Indicator[tostring(2*100 + 10)].D[ period2]
	)
	and (
	Indicator[tostring(3*100 + 10)].K[ period3] > Indicator[tostring(3*100 + 10)].D[ period3]
	)  
	then		
         if Direction then
                BUY();
		...
		Return=true;
    elseif core.crossesUnder(Indicator[tostring(1*100 + 10)].K, Indicator[tostring(1*100 + 10)].D, period1)
	and Indicator[tostring(1*100 + 10)].K[ period1] > OB 
	and
	(
	Indicator[tostring(2*100 + 10)].K[  period2] < Indicator[tostring(2*100 + 10)].D[  period2]
	)
