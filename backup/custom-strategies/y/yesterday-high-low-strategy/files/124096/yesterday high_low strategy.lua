...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=67384
...
local STRATEGY_NAME = "Yesterday High Low";
...
function CreateParameters() 
    strategy.parameters:addString("Close_Candle_TF", "Close Candle TF", "", "m1");
    strategy.parameters:setFlag("Close_Candle_TF", core.FLAG_BARPERIODS);
    strategy.parameters:addInteger("StochKPeriod", "Stochastic K Period", "", 5);
    strategy.parameters:addInteger("StochDPeriod", "Stochastic D Period", "", 3);
    strategy.parameters:addInteger("StochSlowing", "Stochastic Slowing", "", 3);
    strategy.parameters:addInteger("MacdFastEmaPeriod", "MACD Fast Period", "", 12);
    strategy.parameters:addInteger("MacdSlowEmaPeriod", "MACD Slow Period", "", 26);
    strategy.parameters:addInteger("MacdSignalEmaPeriod", "MACD Signal Period", "", 9);
end

local stoch;
local macd;
function CreateIndicators(source)
    stoch = core.indicators:create("STOCHASTIC", source, instance.parameters.StochKPeriod, 
        instance.parameters.StochSlowing, instance.parameters.StochDPeriod);
    macd = core.indicators:create("MACD", source, instance.parameters.MacdFastEmaPeriod,
        instance.parameters.MacdSlowEmaPeriod, instance.parameters.MacdSignalEmaPeriod);
end
...
function CreateCustomActions()
    ...
    -- action1.IsPass = function (source, period) return core.crossesOver(source.close, indi.Top, period); end
    -- if isEntry1 then
    ...
    enterLongAction.IsPass = function (source, period, periodFromLast)
        return source.close[period] < source.low[period - 1] 
            and source.close[period - 1] >= source.low[period - 1] 
            and macd.SIGNAL[period] < 0 
            and stoch.D[period] > 50;
    end

    local enterShortAction = {};
    enterShortAction.ActOnSwitch = true;
    enterShortAction.IsPass = function (source, period, periodFromLast)
        return source.close[period] > source.high[period - 1] 
            and source.close[period - 1] <= source.high[period - 1] 
            and macd.SIGNAL[period] > 0 
            and stoch.D[period] < 50;
    end
