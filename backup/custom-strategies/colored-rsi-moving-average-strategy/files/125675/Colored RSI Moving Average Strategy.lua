...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=68323
...
function Init() --The strategy profile initialization
    strategy:name("Colored RSI Moving Average Strategy");
	...
	strategy.parameters:addDouble("LL", "Long Level", "", 50);
	strategy.parameters:addDouble("EL", "Long Level", "", 50);
	strategy.parameters:addDouble("SL", "Short Level", "", 50);
	strategy.parameters:addDouble("ES", "Short Level", "", 50);
    ...
    strategy.parameters:addString("MA_Method", "MA method", "", "EMA");
...
function CreateTradingParameters()
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "CRSIMAS");
...
local RSI, MA;
local RSI_Period, MA_Period, MA_Method,Price;
local LL, SL, EL,ES;
...
function Prepare( nameOnly)
    ...
    MA = core.indicators:create("AVERAGES", RSI.DATA, MA_Method, MA_Period );
	first=MA.DATA:first(); 
...
function EntryFunction( id, now,period)
    ...
    if   MA.DATA[period]> LL 
	and MA.DATA[period-1]<= LL 
	then	
          if Direction then
                BUY();
            else
		...
		Return=true;
    elseif   MA.DATA[period]<SL 
	and MA.DATA[period-1]>= SL 
	then
            if Direction then
                SELL();
            else
