...
function CreateParameters() 
    strategy.parameters:addInteger("eng_period", "Min Engulf Candles", "", 4);
end

local eng_period
function CreateEntryIndicators(source)
    eng_period = instance.parameters.eng_period;
end

function UpdateIndicators()
end

function GetMasterCandle(source, period)
    local CandleTop = source.high[period - eng_period];
    local CandleBottom = source.low[period - eng_period];
    for i = period - eng_period, period do
        if math.max(source.close[i], source.open[i]) > CandleTop or math.min(source.close[i], source.open[i]) < CandleBottom then
            return false;
        end
    end
    return true, source.high[period - eng_period], source.low[period - eng_period];
end

function GetLastHighLow(source, period)
    for i = period, eng_period, -1 do
        local res, high, low = GetMasterCandle(source, i);
        if res then
            return high, low;
        end
    end
...
function IsEntryLong(source, period)
    local high, low = GetLastHighLow(source, period);
    return high ~= nil and core.crossesOver(source.close, high, period);
end
function IsEntryShort(source, period)
    local high, low = GetLastHighLow(source, period);
    return low ~= nil and core.crossesUnder(source.close, low, period);
end
...
function OpenTrade(side)
    ...
    local high, low = GetLastHighLow(main_source, main_source:size() - 1);
    if use_stop then
        if side == "B" then
            valuemap.RateStop = low;
        else
            valuemap.RateStop = high;
        end
    end
    if use_limit then
        if side == "B" then
            valuemap.RateLimit = instance.ask[NOW] + (instance.bid[NOW] - low);
        else
            valuemap.RateLimit = instance.bid[NOW] + (high - instance.ask[NOW]);
        end
    end
