...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=65036
...
function Init() --The strategy profile initialization
    strategy:name("Transient Zones Strategy")
    ...
    strategy.parameters:addInteger("h_right", "H Right", "H Right", 10)
    strategy.parameters:addInteger("h_left", "H Left", "H Left", 10)
    strategy.parameters:addInteger("sample_period", "H Left", "H Left", 1000)
    strategy.parameters:addBoolean("show_ptz", "Sample bars for % TZ", "Sample bars for % TZ", true)

    strategy.parameters:addString("EntrySignal", "Entry Signal", "", "Minor")
    strategy.parameters:addStringAlternative("EntrySignal", "Major", "", "Major")
    strategy.parameters:addStringAlternative("EntrySignal", "Minor", "", "Minor")

    strategy.parameters:addString("ExitSignal", "Exit Signal", "", "Major")
    strategy.parameters:addStringAlternative("ExitSignal", "Major", "", "Major")
    strategy.parameters:addStringAlternative("ExitSignal", "Minor", "", "Minor")
...
local Indicator, h_right, h_left, sample_period, show_ptz
local ExitSignal, EntrySignal
...
function Prepare(nameOnly)
    ...
    ExitSignal = instance.parameters.ExitSignal
    EntrySignal = instance.parameters.EntrySignal
    ...
    assert(
        core.indicators:findIndicator("TRANSIENT ZONES") ~= nil,
        "Please, download and install TRANSIENT ZONES.LUA indicator"
    )
    ...
    Indicator = core.indicators:create("TRANSIENT ZONES", Source, h_righ, h_left, sample_period, false, show_ptz, true)
...
function ExitFunction(now, period)
    ...
    if
        (Indicator.X[period - h_right + 1] == 1 and ExitSignal == "Major") or
            (Indicator.J[period] == 1 and ExitSignal == "Minor")
     then
        if Direction then
    ...
    if
        (Indicator.Y[period - h_right + 1] == 1 and ExitSignal == "Major") or
            (Indicator.I[period] == 1 and ExitSignal == "Minor")
     then
        if Direction then
            if haveTrades("S") then
                exitSpecific("S")
...
function EntryFunction(now, period)
    ...
    if
        (Indicator.Y[period - h_right + 1] == 1 and EntrySignal == "Major") or
            (Indicator.I[period] == 1 and EntrySignal == "Minor")
     then
        if Direction then
            BUY()
        else
        ...
        Return = true
    elseif
        (Indicator.X[period - h_right + 1] == 1 and EntrySignal == "Major") or
            (Indicator.J[period] == 1 and EntrySignal == "Minor")
     then
        if Direction then
            SELL()
        else
