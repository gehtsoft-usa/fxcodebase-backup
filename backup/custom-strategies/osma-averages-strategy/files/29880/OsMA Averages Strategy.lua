...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=15927
...
function Init() --The strategy profile initialization
    strategy:name("OsMA Averages Strategy")
    ...
    strategy.parameters:addInteger("AveragesPeriod", "Period", "", 20)

    strategy.parameters:addGroup("OsMA Parameters")
    ...
    strategy.parameters:addGroup("Confirmation")
    strategy.parameters:addString("Confirmation", "Confirmation Type", "", "NO")
    strategy.parameters:addStringAlternative("Confirmation", "Don't use Confirmation ", "", "NO")
    strategy.parameters:addStringAlternative("Confirmation", "Price / Averages Position", "", "CROSS")
    strategy.parameters:addStringAlternative("Confirmation", "Averages Slope", "", "SLOPE")
    strategy.parameters:addStringAlternative("Confirmation", "OSMA Zero Line Position", "", "ZERO")
    strategy.parameters:addStringAlternative("Confirmation", "OSMA Slope", "", "EXTREME")

    strategy.parameters:addGroup("Signal")
    strategy.parameters:addString("Signal", "Signal Type", "", "CROSS")
    strategy.parameters:addStringAlternative("Signal", "Price / Averages Cross", "", "CROSS")
    strategy.parameters:addStringAlternative("Signal", "Averages Slope", "", "SLOPE")
    strategy.parameters:addStringAlternative("Signal", "OSMA Zero Line Cross", "", "ZERO")
    strategy.parameters:addStringAlternative("Signal", "OSMA Top/Bottom", "", "EXTREME")
    --strategy.parameters:addStringAlternative("Signal", "All", "", "ALL");

    strategy.parameters:addGroup("Exit")
    strategy.parameters:addString("EXIT", "Signal Type", "", "NO")
    strategy.parameters:addStringAlternative("EXIT", "Don't use Exit ", "", "NO")
    strategy.parameters:addStringAlternative("EXIT", "Price / Averages Cross", "", "CROSS")
    strategy.parameters:addStringAlternative("EXIT", "Averages Slope", "", "SLOPE")
    strategy.parameters:addStringAlternative("EXIT", "OSMA Zero Line Cross", "", "ZERO")
    strategy.parameters:addStringAlternative("EXIT", "OSMA Slope", "", "EXTREME")
    strategy.parameters:addStringAlternative("EXIT", "OSMA Top/Bottom", "", "OTB")
...
function Prepare(nameOnly)
    Entry = instance.parameters.Signal
    OsmaType = instance.parameters.OsmaType
    AveragesType = instance.parameters.AveragesType
    Price = instance.parameters.Price
    Confirmation = instance.parameters.Confirmation
    AveragesPeriod = instance.parameters.AveragesPeriod
    ...
    name =
        name ..
        ", " .. AveragesPeriod .. ", " .. Method .. ", " .. ShortFrame .. ", " .. LongFrame .. ", " .. SignalFrame
    ...
    Indicator[1] = core.indicators:create("AVERAGES", Source[Price], Method, AveragesPeriod, true)
    Short[1] = Indicator[1]:getStream(0)

    Indicator[2] = core.indicators:create("MACD", Source[Price], ShortFrame, LongFrame, SignalFrame)
    Short[2] = Indicator[2]:getStream(2)
...
function CONFIRMATION(period, type, mode)
    local FLAG = true

    if type == "CROSS" then
        if mode then
            if Source.close[period] > Short[1][period] then
                FLAG = true
            else
                FLAG = false
            end
    ...
    elseif type == "SLOPE" then
        if mode then
            if Short[1][period] > Short[1][period - 1] then
                FLAG = true
            else
                FLAG = false
            end
    ...
    elseif type == "EXTREME" then
        if mode then
            if Short[2][period] > Short[2][period - 1] then
                FLAG = true
            else
                FLAG = false
            end
    ...
    elseif type == "OTB" then
        if mode then
            if
                Short[2][period] > Short[2][period - 1] and Short[2][period - 2] > Short[2][period - 1] or
                    Short[2][period] > Short[2][period - 1] and Short[2][period - 2] == Short[2][period - 1]
             then
                FLAG = true
            else
                FLAG = false
...
function ENTRY(period, type, mode)
    local FLAG = false
    ...
    if type == "CROSS" then
        if mode then
            ...
            if core.crossesUnder(Source.close, Short[1], period) then
                FLAG = true
            else
                FLAG = false
            end
    ...
    elseif type == "SLOPE" then
        if mode then
            if
                Short[1][period] > Short[1][period - 1] and Short[1][period - 2] > Short[1][period - 1] or
                    Short[1][period] > Short[1][period - 1] and Short[1][period - 2] == Short[1][period - 1]
             then
                FLAG = true
            else
                FLAG = false
    ...
    elseif type == "ZERO" then
        if mode then
            if core.crossesOver(Short[2], 0, period) then
                FLAG = true
            else
                FLAG = false
            end
        else
            if core.crossesUnder(Short[2], 0, period) then
                FLAG = true
            else
                FLAG = false
            end
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if CONFIRMATION(period, Confirmation, true) and ENTRY(period, Entry, true) then
        if Direction then
            BUY()
        else
            SELL()
        end
    elseif CONFIRMATION(period, Confirmation, false) and ENTRY(period, Entry, false) then
        if Direction then
            SELL()
        else
            BUY()
    ...
    if CONFIRMATION(period, EXIT, false) and EXIT ~= "NO" then
        if Direction then
    ...
    if CONFIRMATION(period, EXIT, true) and EXIT ~= "NO" then
        if Direction then
            if haveTrades("S") then
                exit("S")
                Signal("Close Short")
