...
function EntryFunction(id, now, period)
    ...
    if RSI.DATA[period] > EMA.DATA[period] and RSI.DATA[period - 1] <= EMA.DATA[period - 1] and Zone == -1 then
        if Direction then
            BUY()
        else
            SELL()
        ...
        Return = true
    elseif RSI.DATA[period] < EMA.DATA[period] and RSI.DATA[period - 1] >= EMA.DATA[period - 1] and Zone == 1 then
        if Direction then
            SELL()
        else
            BUY()
