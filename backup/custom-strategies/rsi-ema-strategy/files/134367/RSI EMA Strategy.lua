...
--	strategy.parameters:addStringAlternative("ExitExecutionType", "Live", "", "Live");
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "RES");
...
function Prepare( nameOnly)
    ...
    EMA = core.indicators:create("EMA", RSI.DATA,  instance.parameters.EMA_Period  );
...
function EntryFunction( id, now,period)
    ...
    if RSI.DATA[period]> EMA.DATA[period]
	and  RSI.DATA[period-1]<= EMA.DATA[period-1]
	and Zone==-1
	then	
          if Direction then
                BUY();
		...
		Return=true;
    elseif RSI.DATA[period]< EMA.DATA[period]
	and  RSI.DATA[period-1]>= EMA.DATA[period-1]
	and Zone==1
		then
            if Direction then
