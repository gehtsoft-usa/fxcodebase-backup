...
function Init() --The strategy profile initialization
    strategy:name("3 in 1 Stochastic Strategy")
    ...
    strategy.parameters:addBoolean("Change", "Change only", "", false)
...
function AddTimeFrame(id, TF)
    strategy.parameters:addGroup(id .. ". Time Frame Calculation")

    strategy.parameters:addString("TF" .. id, "Time Frame", "", TF)
    strategy.parameters:setFlag("TF" .. id, core.FLAG_BARPERIODS)
    strategy.parameters:addInteger("K" .. id, "Number of periods for %K", "The number of periods for %K.", 5, 2, 1000)
    strategy.parameters:addInteger("SD" .. id, "%D slowing periods", "The number of periods for slow %D.", 3, 2, 1000)
    strategy.parameters:addInteger("D" .. id, "Number of periods for %D", "The number of periods for %D.", 3, 2, 1000)

    strategy.parameters:addString("MVAT_K" .. id, "Smoothing method for %K", "", "MVA")
    strategy.parameters:addStringAlternative("MVAT_K" .. id, "MVA", "", "MVA")
    strategy.parameters:addStringAlternative("MVAT_K" .. id, "EMA", "", "EMA")
    strategy.parameters:addStringAlternative("MVAT_K" .. id, "MetaTrader", "", "FS")

    strategy.parameters:addString("MVAT_D" .. id, "Smoothing method for %D", "", "MVA")
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if ExecutionType == "Live" and id == 1 then
        period1 = core.findDate(Source[1].close, TickSource:date(period), false)
        period2 = core.findDate(Source[2].close, TickSource:date(period), false)
        period3 = core.findDate(Source[3].close, TickSource:date(period), false)
    end

    if ExecutionType == "Live" then
        if ONE == Source[1]:serial(period1) then
            return
        end
        ...
        period1 = period
        period2 = core.findDate(Source[2].close, Source[1]:date(period), false)
        period3 = core.findDate(Source[3].close, Source[1]:date(period), false)
    end
    ...
    if
        period1 < Source[1].close:first() + 1 or period2 < Source[2].close:first() + 1 or
            period3 < Source[3].close:first() + 1
     then
        return
    end
    ...
    if
        Indicator[2].K[period2] > Indicator[2].D[period2] and Indicator[3].K[period3] > Indicator[3].D[period3] and
            Indicator[1].K[period1] > Indicator[1].D[period1] and
            (not Change or (Change and Indicator[1].K[period1 - 1] <= Indicator[1].D[period1 - 1]) or
                (Change and Indicator[2].K[period2 - 1] <= Indicator[2].D[period2 - 1]) or
                (Change and Indicator[3].K[period3 - 1] <= Indicator[3].D[period3 - 1]))
     then
        if Direction then
            BUY()
        else
