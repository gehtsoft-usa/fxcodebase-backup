...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=63480
...
function Init() --The strategy profile initialization
    strategy:name("MA OF MA DELTA STRATEGY")
    ...
    strategy.parameters:addGroup("MA Delta Calculation")
    strategy.parameters:addInteger("Period1", "Periods ", "Periods", 10, 1, 10000)
    ...
    strategy.parameters:addInteger("Period2", "Periods ", "Periods", 34, 1, 10000)
...
function Prepare(nameOnly)
    ...
    assert(core.indicators:findIndicator("MA DELTA") ~= nil, "Please, download and install MA DELTA.LUA indicator")
    ...
    MA1 = core.indicators:create("MA DELTA", Source.close, Period1, Method1)
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if MA1.DATA[period] > BL and MA1.DATA[period] > MA2.DATA[period] and MA1.DATA[period - 1] <= MA2.DATA[period - 1] then
        if Direction then
            BUY()
        else
            SELL()
    ...
    elseif
        MA1.DATA[period] < SL and MA1.DATA[period] < MA2.DATA[period] and MA1.DATA[period - 1] >= MA2.DATA[period - 1]
     then
        if Direction then
            SELL()
        else
    ...
    if Exit then
        if MA1.DATA[period] < BL and MA1.DATA[period - 1] >= BL then
            if Direction then
        ...
        if MA1.DATA[period] > SL and MA1.DATA[period - 1] <= SL then
            if Direction then
                if haveTrades("S") then
                    exitSpecific("S")
                    Signal("Close Short")
