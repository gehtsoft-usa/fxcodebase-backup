...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=68487
...
local STRATEGY_NAME = "Regression slope strategy";
...
function CreateParameters()
    strategy.parameters:addInteger("period", "Regression period", "", 7, 2, 100000);
    strategy.parameters:addInteger("smoothing_period", "Smoothing", "", 10);
end

local indi, smooth;
function CreateIndicators(source) 
    local profile = core.indicators:findIndicator("REGRESSION SLOPE");
    assert(profile ~= nil, "Please, download and install " .. "REGRESSION SLOPE" .. ".LUA indicator");
    local indicatorParams = profile:parameters();
    indicatorParams:setInteger("N", instance.parameters.period);
    indi = core.indicators:create("REGRESSION SLOPE", source, indicatorParams)
    smooth = core.indicators:create("EMA", indi.DATA, instance.parameters.smoothing_period);
end

function UpdateIndicators()
    indi:update(core.UpdateLast);
    smooth:update(core.UpdateLast);
end
...
function CreateCustomActions()
    ...
    enterLongAction.IsPass = function (source, period, periodFromLast, data)
        if not smooth.DATA:hasData(period - 1) then
            return;
        end
        return core.crossesOver(smooth.DATA, 0, period);
    end
    ...
    enterShortAction.IsPass = function (source, period, periodFromLast, data)
        ...
        return core.crossesUnder(smooth.DATA, 0, period);
    end
