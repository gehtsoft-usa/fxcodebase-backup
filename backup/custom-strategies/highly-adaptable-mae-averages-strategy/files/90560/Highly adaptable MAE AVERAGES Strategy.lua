...
function Init() --The strategy profile initialization
    strategy:name("Highly adaptable MAE AVERAGES Strategy")
...
function Prepare(nameOnly)
    ...
    assert(
        core.indicators:findIndicator("MAE AVERAGES") ~= nil,
        "Please, download and install MAE AVERAGES.LUA indicator"
    )
    ...
    Indicator = core.indicators:create("MAE AVERAGES", Source[Price], Method, Period, B, BWU, true)
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if period < first or not Indicator:getStream(0):hasData(period - 1) then
        return
    end
