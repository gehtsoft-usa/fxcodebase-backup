...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=67757
...
function CreateParameters()
    strategy.parameters:addInteger("BollingerPeriod", "Bollinger Period", "", 20)
    strategy.parameters:addDouble("BollingerDeviations", "Bollinger Deviations", "", 2)
    strategy.parameters:addInteger("IVFTperiod", "IVFT period", "", 10)
...
function CreateIndicators(source)
    Bollinger1 = core.indicators:create("BB", source, instance.parameters.BollingerPeriod, instance.parameters.BollingerDeviations);
    EnsureIndicatorInstalled("IVFT");
    IVFT1 = core.indicators:create("IVFT", source, instance.parameters.IVFTperiod);
end

function UpdateIndicators()
    Bollinger1:update(core.UpdateLast);
    IVFT1:update(core.UpdateLast);
end
...
function CreateCustomActions()
    ...
    exitLongAction.IsPass = function (source, period, periodFromLast, data)
        if not Bollinger1.AL:hasData(period - 1) then
            return;
        end
        return core.crossesOver(source.close, Bollinger1.AL, period);
    end
    ...
    exitShortAction.IsPass = function (source, period, periodFromLast, data)
        ...
        return core.crossesUnder(source.close, Bollinger1.AL, period);
    end
    ...
    enterLongAction.IsPass = function (source, period, periodFromLast, data)
        if not Bollinger1.TL:hasData(period - 1) or not IVFT1.DATA:hasData(period) then
            return;
        end
        return core.crossesUnder(source.close, Bollinger1.TL, period) and IVFT1.DATA[period] > 0.5;
    end
    ...
    enterShortAction.IsPass = function (source, period, periodFromLast, data)
        if not Bollinger1.BL:hasData(period - 1) or not IVFT1.DATA:hasData(period) then
            return;
        end
        return core.crossesOver(source.close, Bollinger1.BL, period) and IVFT1.DATA[period] < -0.5;
    end
