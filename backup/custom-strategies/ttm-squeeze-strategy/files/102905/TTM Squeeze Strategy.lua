...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=62795
...
function Init() --The strategy profile initialization
    strategy:name("TTM Squeeze Strategy")
    ...
    strategy.parameters:addDouble("Period", "Number of periods", "Number of periods", 20.0)
    strategy.parameters:addDouble("Dev", "Number of standard deviations", "Number of standard deviations", 2.0)

    strategy.parameters:addGroup("Keltner Calculation")
    strategy.parameters:addInteger("NM", "Number of the periods to smooth the center line", "", 20)
    strategy.parameters:addInteger("NB", "Number of periods to smooth deviation", "", 20)
    strategy.parameters:addDouble("F", "Factor which is used to apply the deviation", "", 2)
    ...
    strategy.parameters:addStringAlternative("MVAT_K", "Fast Smoothed", "", "FS")
...
local Direction
local CustomID
local Dev, Period, NB, NM, F, MS
local ATR, MA, Stochastic
...
function Prepare(nameOnly)
    ...
    first = math.max(MA.DATA:first(), ATR.DATA:first(), Stochastic.D:first())
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    local H = MA.DATA[period] + ATR.DATA[period] * F
    local L = MA.DATA[period] - ATR.DATA[period] * F

    local TL = ml + Dev * d
    local BL = ml - Dev * d

    -- only buy if we have a fast cross over slow and the price is above the moving averages.
    if TL < H and TL > L then
