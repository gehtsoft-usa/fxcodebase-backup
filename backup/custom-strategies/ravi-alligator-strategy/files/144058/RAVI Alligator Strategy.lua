...
-- https://fxcodebase.com/code/viewtopic.php?f=31&t=71596
...
function Init() --The strategy profile initialization
    strategy:name("RAVI Alligator Strategy");
    ...
    strategy.parameters:addGroup("RAVI Calculation");
    strategy.parameters:addInteger("Slow", "Slow moving averages Period", "Slow moving averages Period", 65);
    strategy.parameters:addInteger("Fast", "Fast moving averages Period", "Fast moving averages Period", 7);
...
function CreateTradingParameters()
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "RAS");
...
function Prepare( nameOnly)
	...
	assert(core.indicators:findIndicator("RAVI") ~= nil, "Please, download and install RAVI.LUA indicator");          
    ...
    RAVI = core.indicators:create("RAVI", Source.close, instance.parameters.Slow, instance.parameters.Fast );
	Indicator = core.indicators:create("ALLIGATOR", Source, JawN, JawS, TeethN, TeethS, LipsN, LipsS, MTH)
    first = math.max(Indicator.Jaw:first(), Indicator.Teeth:first(), Indicator.Lips:first(), RAVI.DATA:first()) 
...
--Lips < Jaw
		            ...
		            if   RAVI.DATA[period]< 0
					or   Indicator.Lips[period]< Indicator.Jaw[period]
					then
...
function EntryFunction( id, now,period)
    ...
    if   RAVI.DATA[period]> 0
	and Indicator.Lips[period]> Indicator.Teeth[period]
	and Indicator.Teeth[period]> Indicator.Jaw[period]	
	then	
          if Direction then
                BUY();
            else
