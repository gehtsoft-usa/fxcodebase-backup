...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=63251
...
function Init() --The strategy profile initialization
    strategy:name("Candles Strategy")
    ...
    strategy.parameters:addInteger("Duration", "Duration", "", 1)
...
function CreateTradingParameters()
    ...
    strategy.parameters:addString("Price", "Stop Level", "", "Pips")
    strategy.parameters:addStringAlternative("Price", "Pips", "", "Pips")
    strategy.parameters:addStringAlternative("Price", "Open/Close", "", "Open/Close")
    strategy.parameters:addStringAlternative("Price", "High/Low", "", "High/Low")
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    for i = 1, Period, 1 do
        if Source.close[period - i] > Source.open[period - i] and Source.close[period - i] > Source.open[period - i] then
            pCount = pCount + 1
        end
    ...
    if pCount == Period then
        Trend = 1
    end

    if nCount == Period then
        Trend = -1
    end

    -- only buy if we have a fast cross over slow and the price is above the moving averages.
    if Trend == 1 and Source.close[period] > Source.open[period] then
        if Direction then
            BUY(period - 1)
        else
            SELL(period - 1)
    ...
    elseif Trend == -1 and Source.close[period] < Source.open[period] then
        if Direction then
            SELL(period - 1)
        else
            BUY(period - 1)
...
function ExitIfWeHaveTrades(period)
    local enum, row
    enum = core.host:findTable("trades"):enumerator()
    row = enum:next()
    while (row ~= nil) do
        if row.AccountID == Account and row.OfferID == Offer and row.QTXT == CustomID then
            iperiod = core.findDate(Source.close, row.Time, false)

            if iperiod ~= -1 then
                TimeDelta = period - iperiod

                if TimeDelta > Duration then
                    exitTrade(row)
                end
            end
        end
...
function MarketOrder(BuySell, period)
    ...
    if SetStop then
        ...
        elseif Price == "Open/Close" then
            if BuySell == "B" then
                valuemap.RateStop = Source.open[period]
            else
                valuemap.RateStop = Source.close[period]
            end
        elseif Price == "High/Low" then
            if BuySell == "B" then
                valuemap.RateStop = Source.low[period]
            else
                valuemap.RateStop = Source.high[period]
            end
        end
    end
