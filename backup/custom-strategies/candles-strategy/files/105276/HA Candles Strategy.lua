...
function Init() --The strategy profile initialization
    strategy:name("HA Candles Strategy")
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    for i = 1, Period, 1 do
        if HA.close[period - i] > HA.open[period - i] and HA.close[period - i] > HA.open[period - i] then
            pCount = pCount + 1
        end
    ...
    if Trend == 1 and HA.close[period] > HA.open[period] then
        if Direction then
            BUY(period - 1)
        else
            SELL(period - 1)
    ...
    elseif Trend == -1 and HA.close[period] < HA.open[period] then
        if Direction then
            SELL(period - 1)
        else
            BUY(period - 1)
...
function MarketOrder(BuySell, period)
    ...
    if SetStop then
        ...
        elseif Price == "Open/Close" then
            if BuySell == "B" then
                valuemap.RateStop = HA.open[period]
            else
                valuemap.RateStop = HA.close[period]
            end
        elseif Price == "High/Low" then
            if BuySell == "B" then
                valuemap.RateStop = HA.low[period]
            else
                valuemap.RateStop = HA.high[period]
            end
        end
    end
