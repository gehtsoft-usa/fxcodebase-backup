...
function Init() --The strategy profile initialization
    ...
    strategy:description("MACD Strategy with histogram confirmation.");
	...
	strategy.parameters:addInteger("SN", "Short Period", "", 12, 1, 2000);
    strategy.parameters:addInteger("LN", "Long Period", "", 26, 1, 2000);
    strategy.parameters:addInteger("IN", "Signal Period", "", 9, 1, 2000);
	
	 strategy.parameters:addDouble("MB", "MACD Buy Level", "", 0);
	  strategy.parameters:addDouble("MS", "MACD Sell Level", "", 0);
	  	  strategy.parameters:addDouble("Trigger", "Trigger Level ", "", 0);
   ...
   -- get only time
	...
	if not InRange(now, OpenTime, CloseTime) 
      ...
      return ;
		...
		elseif core.crossesUnder( Short["MACD"], MS , period) 
		and math.abs(Short["Histogram"][period-1] - Short["Histogram"][period])  >  Trigger
		then 
		 ...
		 if core.crossesUnder( Short["MACD"], MB , period) 	
		and math.abs(Short["Histogram"][period-1] - Short["Histogram"][period])  >  Trigger 
		 then 
		 ...
		 if core.crossesOver( Short["MACD"], MS , period) 
		 and math.abs(Short["Histogram"][period-1] - Short["Histogram"][period])  >  Trigger
		 then 	
		       if   Direction then    
       			if haveTrades("S") then
