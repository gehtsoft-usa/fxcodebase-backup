...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=70549
...
function Init() --The strategy profile initialization
    strategy:name("SSD RSI Strategy")
    ...
    strategy.parameters:addGroup("SSD Calculation")
    strategy.parameters:addString("TF1", "Time frame", "", "m1")
    strategy.parameters:setFlag("TF1", core.FLAG_PERIODS)
    strategy.parameters:addBoolean("ssd_filter", "Use SSD filter", "", true);
    strategy.parameters:addDouble("OB1", "OB Level", "", 80)
    strategy.parameters:addDouble("OS1", "OS Level", "", 20)

    strategy.parameters:addBoolean("SF", "SSD Filter", "", true)

    strategy.parameters:addInteger("P1", "K Period", "", 5)
    strategy.parameters:addInteger("P2", "D Slowing Period", "", 3)
    strategy.parameters:addInteger("P3", "D period", "", 3)

    strategy.parameters:addGroup("RSI Calculation")

    strategy.parameters:addString("TF2", "Time frame", "", "m1")
    ...
    strategy.parameters:addBoolean("rsi_filter", "Use RSI filter", "", true);
    strategy.parameters:addDouble("OB2", "OB Level", "", 80)
    strategy.parameters:addDouble("OS2", "OS Level", "", 20)
...
function Prepare(nameOnly)
    ...
    rsi_filter = instance.parameters.rsi_filter;
    ssd_filter = instance.parameters.ssd_filter;
    ...
    SSD = core.indicators:create("SSD", Source1, instance.parameters.P1, instance.parameters.P2, instance.parameters.P3)
    RSI = core.indicators:create("X-RSIOMA", Source2, instance.parameters.RSIOMA, instance.parameters.Ma_RSIOMA)
...
function RSIBuyFilter(period)
    if not rsi_filter then
        return true;
    end
    return RSI.RSI[period] > OS2;
end

function SSDBuyFilter(period)
    if not ssd_filter then
        return true;
    end
    return SSD.K[period] > OS1;
end

function RSISellFilter(period)
    if not rsi_filter then
        return true;
    end
    return RSI.RSI[period] < OB2;
end

function SSDSellFilter(period)
    if not ssd_filter then
        return true;
    end
    return SSD.K[period] < OB1;
end
...
function EntryFunction(id, now, period)
    ...
    --/////////////////////////////////////////////////////////////////
    if LastEntry == Source1:serial(period) then
        --or not MA.DATA:hasData(period)
        return
    end
    --/////////////////////////////////////////////////////////////////
    ...
    if not RSI.marsioma:hasData(period2) or not SSD.D:hasData(period1) then
        return
    end

    -- only buy if we have a fast cross over slow and the price is above the moving averages.
    if SSD.K[period1] > SSD.D[period1] 
        and SSDBuyFilter(period1)
        and RSI.RSI[period2] > RSI.marsioma[period2] 
        and RSIBuyFilter(period2)
        and ((SF and SSD.K[period1] < 50) or not SF)
     then
        if Direction then
            BUY()
        else
        ...
        Return = true
    elseif SSD.K[period1] < SSD.D[period1] 
        and SSDSellFilter(period) 
        and RSI.RSI[period2] < RSI.marsioma[period2] 
        and RSISellFilter(period2) 
        and ((SF and SSD.K[period1] > 50) or not SF)
     then
        if Direction then
            SELL()
