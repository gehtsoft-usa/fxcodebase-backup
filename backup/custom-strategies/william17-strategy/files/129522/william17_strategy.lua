...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=69078
...
local STRATEGY_NAME = "william17 strategy";
...
function CreateParameters() 
    ...
    strategy.parameters:addDouble("rsi_entry_level", "RSI Entry Level", "", 50);
    strategy.parameters:addDouble("rsi_buy_exit_levell", "RSI Exit Buy Level", "", 70);
    strategy.parameters:addDouble("rsi_sell_exit_levell", "RSI Exit Sell Level", "", 30);
...
local Ichimoku, RSI, SAR, MACD
function CreateEntryIndicators(source)
    Ichimoku = core.indicators:create("ICH", source, instance.parameters.TenkanSenPeriod, 
...
local rsi_entry_level, rsi_buy_exit_levell, rsi_sell_exit_levell;
function UpdateIndicators()
    rsi_entry_level = instance.parameters.rsi_entry_level;
    rsi_buy_exit_levell = instance.parameters.rsi_buy_exit_levell;
    rsi_sell_exit_levell = instance.parameters.rsi_sell_exit_levell;
    Ichimoku:update(core.UpdateLast);
...
function CreateCustomActions()
    if not DISABLE_EXIT then
        ...
        exitLongAction.IsPass = function (source, period, periodFromLast, data)
            return MACD.MACD[period - 1] > MACD.MACD[period]
                or RSI.DATA[period] >= rsi_buy_exit_levell;
        end
        ...
        exitShortAction.IsPass = function (source, period, periodFromLast, data)
            return MACD.MACD[period - 1] < MACD.MACD[period]
                or RSI.DATA[period] <= rsi_sell_exit_levell;
        end
    ...
    enterLongAction.IsPass = function (source, period, periodFromLast, data)
        return SAR.DN:hasData(period) and not SAR.UP:hasData(period - 1)
    ...
    enterShortAction.IsPass = function (source, period, periodFromLast, data)
        return not SAR.DN:hasData(period) and SAR.UP:hasData(period - 1)
            and source.close[period] < Ichimoku.SA[period]
            and source.close[period] < Ichimoku.SB[period]
            and RSI.DATA[period - 1] > rsi_entry_level;
    end
