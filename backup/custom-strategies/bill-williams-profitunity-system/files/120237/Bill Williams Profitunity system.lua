...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=66412
...
function Init() --The strategy profile initialization
    strategy:name("Bill Williams Profitunity system");
...
--	strategy.parameters:addStringAlternative("ExitExecutionType", "Live", "", "Live");
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "BWPS");
...
local JawN ,JawS, TeethN,TeethS, LipsN,  LipsS1, MTH;
local Alligator ;
...
function ExtUpdate(id, source, period)  -- The method called every time when a new bid or ask price appears.
   ...
   -- get only time
    now = now - math.floor(now);
	    ...
	    -- update indicators.
    Alligator:update(core.UpdateLast);
...
function EntryFunction( now,period)
	...
	and Range > 50
	and Source.high[period] < math.min(Alligator.Jaw[period],Alligator.Teeth[period], Alligator.Lips[period])
	then	
          if Direction then
                BUY();
            else
	...
	and Range < 50
	and Source.low[period] > math.max(Alligator.Jaw[period],Alligator.Teeth[period], Alligator.Lips[period])
		then
            if Direction then
                SELL();
            else
