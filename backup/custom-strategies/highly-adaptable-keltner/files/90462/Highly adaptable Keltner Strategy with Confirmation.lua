...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=59755
...
function Init() --The strategy profile initialization
    strategy:name("Highly adaptable Keltner Strategy")
    ...
    strategy.parameters:addString("MATF", "Time frame", "", "H8")
    strategy.parameters:setFlag("MATF", core.FLAG_PERIODS)
    ...
    strategy.parameters:addGroup("1. Selector")
    ...
    strategy.parameters:addBoolean("Confirmation" .. 1, "Use MA Confirmation ", "", false)

    strategy.parameters:addGroup("2. Selector")
    ...
    strategy.parameters:addBoolean("Confirmation" .. 2, "Use MA Confirmation ", "", false)

    strategy.parameters:addGroup("3. Selector")
    ...
    strategy.parameters:addBoolean("Confirmation" .. 3, "Use MA Confirmation ", "", false)

    strategy.parameters:addGroup("4. Selector")
    ...
    strategy.parameters:addBoolean("Confirmation" .. 4, "Use MA Confirmation ", "", false)

    strategy.parameters:addGroup("5. Selector")
    ...
    strategy.parameters:addBoolean("Confirmation" .. 5, "Use MA Confirmation ", "", false)

    strategy.parameters:addGroup("6. Selector")
    ...
    strategy.parameters:addBoolean("Confirmation" .. 6, "Use MA Confirmation ", "", false)

    strategy.parameters:addGroup("7. Selector")
    strategy.parameters:addString("Action" .. 7, "Price MA Line  Cross Over", "", "NO")
    ...
    strategy.parameters:addGroup("8. Selector")
    strategy.parameters:addString("Action" .. 8, "Price MA Line Cross Under", "", "NO")
...
local Method, Period, MA, MASource, Price
--
function Prepare(nameOnly)
    ...
    for i = 1, 8, 1 do
        ...
        if i <= 6 then
            Confirmation[i] = instance.parameters:getBoolean("Confirmation" .. i)
        end
    end
    ...
    MASource = ExtSubscribe(3, nil, instance.parameters.MATF, instance.parameters.Type == "Bid", "bar")
    Source = ExtSubscribe(2, nil, instance.parameters.TF, instance.parameters.Type == "Bid", "bar")
    Indicator = core.indicators:create("KELTNER", Source, NM, NB, F, SRC, MS, MV)

    assert(core.indicators:findIndicator(Method) ~= nil, Method .. " indicator must be installed");
    MA = core.indicators:create(Method, MASource[Price], Period)
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if MA.DATA:size() <= MA.DATA:first() + 2 then
        return
    end
    ...
    if
        core.crossesOver(Source.close, Indicator:getStream(0), period) and
            (not Confirmation[1] or MA.DATA[MA.DATA:size() - 1] <= Source.close[period])
     then
        ACTION(1, 1, "Over", period)
    end
    if
        core.crossesUnder(Source.close, Indicator:getStream(0), period) and
            (not Confirmation[2] or MA.DATA[MA.DATA:size() - 1] >= Source.close[period])
     then
        ACTION(2, 1, "Under", period)
    end
    if
        core.crossesOver(Source.close, Indicator:getStream(1), period) and
            (not Confirmation[3] or MA.DATA[MA.DATA:size() - 1] <= Source.close[period])
     then
        ACTION(3, 2, "Over", period)
    end
    if
        core.crossesUnder(Source.close, Indicator:getStream(1), period) and
            (not Confirmation[4] or MA.DATA[MA.DATA:size() - 1] >= Source.close[period])
     then
        ACTION(4, 2, "Under", period)
    end
    if
        core.crossesOver(Source.close, Indicator:getStream(2), period) and
            (not Confirmation[5] or MA.DATA[MA.DATA:size() - 1] <= Source.close[period])
     then
        ACTION(5, 3, "Over", period)
    end

    if
        core.crossesUnder(Source.close, Indicator:getStream(2), period) and
            (not Confirmation[6] or MA.DATA[MA.DATA:size() - 1] >= Source.close[period])
     then
        ACTION(6, 3, "Under", period)
    end

    if Source.close[period] > MA.DATA[MA.DATA:size() - 1] and Source.close[period - 1] <= MA.DATA[MA.DATA:size() - 1] then
        ACTION(7, 4, "Over", period)
    end
    if Source.close[period] < MA.DATA[MA.DATA:size() - 1] and Source.close[period - 1] >= MA.DATA[MA.DATA:size() - 1] then
        ACTION(8, 4, "Under", period)
    end
end
