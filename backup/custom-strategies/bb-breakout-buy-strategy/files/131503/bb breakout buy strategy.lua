...
local STRATEGY_NAME = "BB Breakout Buy Strategy";
function CreateParameters() 
    strategy.parameters:addInteger("bb_period", "BB period", "", 20);
    strategy.parameters:addDouble("bb_deviations", "BB deviations", "", 2.0);
end

local indi;
function CreateEntryIndicators(source)
    indi = core.indicators:create("BB", source, instance.parameters.bb_period, instance.parameters.bb_deviations);
end
...
function IsEntryLong(source, period)
    if not indi.TL:hasData(period) or not indi.BL:hasData(period - 1) then
        return;
    end
    if source.close[period] >= indi.TL[period] then
        --core.host:execute("stop");
        return false;
    end
    return core.crossesUnderOrTouch(source.close, indi.BL, period);
end
...
local _ToTime, tradingWeekOffset, tradingWeekOffset;
...
local last_entry, last_exit, last_day;
function ExtUpdate(id, source, period)
    if id ~= entry_source_id then
        return;
    end
    local s = core.getcandle("D1", source:date(period), tradingDayOffset, tradingWeekOffset)
    if last_day == nil or last_day ~= s then
        last_day = s;
        CloseTrades();
    end
...
function OpenTrade(side, period)
    ...
    valuemap.RateStop = main_source.low[period - 1];
    valuemap.Rate = main_source.high[period - 1];
    if use_limit then
