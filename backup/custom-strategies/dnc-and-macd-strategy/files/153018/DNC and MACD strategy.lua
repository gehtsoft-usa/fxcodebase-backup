-- Available @ https://fxcodebase.com/code/viewtopic.php?f=31&t=74272
...
function Init() --The strategy profile initialization
    strategy:name("DNC and MACD strategy");
    ...
    strategy.parameters:addGroup("Donchain calculation");
    strategy.parameters:addInteger("Period", "Period", "", 20);  
    strategy.parameters:addInteger("TestPeriod", "Test Period", "", 5); 	
...
function CreateTradingParameters()
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "DMS");
...
function Prepare( nameOnly)
	...
	TestPeriod= instance.parameters.TestPeriod;
    ...
    DNC = core.indicators:create("DNC", Source, instance.parameters.Period);  
	first=math.max(DNC.DATA:first(), MACD.SIGNAL:first())+TestPeriod; 
...
function IsLow(period)


   local Return=false;
   
   for i= period, math.max(Source:first(), period-TestPeriod),  -1 do   
       if  DNC.DN[i] < DNC.DN[i-1] then   
	   Return=true;
	   break
	   end
   end
...
function IsHigh(period)

   local Return=false;
   
   for i= period, math.max(Source:first(), period-TestPeriod),  -1 do   
	   if  DNC.DU[i] > DNC.DU[i-1] then   
	   Return=true;
	   break
	   end
   end
