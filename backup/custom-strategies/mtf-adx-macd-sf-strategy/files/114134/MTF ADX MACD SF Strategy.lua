...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=64986
...
function Init() --The strategy profile initialization
    strategy:name("MTF ADX MACD SF Strategy")
    ...
    --default paremeters(fast ma period 12, slow ma period 26,signal line 9)

    strategy.parameters:addGroup("MACD Calcultion")
    strategy.parameters:addString("TF1", "Time frame", "", "m5")
    strategy.parameters:addInteger("M1", "Fast Period", "", 12)
    strategy.parameters:addInteger("M2", "Slow Period", "", 26)
    strategy.parameters:addInteger("M3", "Signal Period", "", 9)

    strategy.parameters:addDouble("Buy", "Buy Level", "", 0)
    strategy.parameters:addDouble("Sell", "Sell Level", "", 0)

    strategy.parameters:addGroup("SF Calcultion")
    strategy.parameters:addString("TF2", "Time frame", "", "H1")

    strategy.parameters:addInteger("Period", "Period", "", 24)

    strategy.parameters:addGroup("ADX Calcultion")
...
function Prepare(nameOnly)
    ...
    assert(core.indicators:findIndicator("SF") ~= nil, "Please, download and install SF.LUA indicator")

    TickSource = ExtSubscribe(1, nil, "t1", instance.parameters.Type == "Bid", "close")

    Source1 = ExtSubscribe(2, nil, TF1, instance.parameters.Type == "Bid", "bar")
    MACD = core.indicators:create("MACD", Source1.close, M1, M2, M3)

    Source2 = ExtSubscribe(3, nil, TF2, instance.parameters.Type == "Bid", "bar")
    SF = core.indicators:create("SF", Source2.close, Period)

    Source3 = ExtSubscribe(4, nil, TF3, instance.parameters.Type == "Bid", "bar")
    ADX = core.indicators:create("ADX", Source3, ADX_Period)
...
function EntryFunction(now, period1, period2, period3)
    ...
    --[[
	 buy: adx > entry level and signal < buy level and macd cross over signal and snakeforce force up>snake force resistance down
     sell: adx > entry level and signal > sell level and macd cross under signal and snake force force down > snake force resistance up
	 
	 ]]
    -- only buy if we have a fast cross over slow and the price is above the moving averages.
    if
        ADX.DATA[period3] > ADX_Level and MACD.SIGNAL[period1] < Buy and MACD.MACD[period1] > MACD.SIGNAL[period1] and
            MACD.MACD[period1 - 1] <= MACD.SIGNAL[period1 - 1] and
            (SF.ForceUp[period2] > SF.ResistanceDn[period2])
     then
        if Direction then
            BUY()
        else
        ...
        Return = true
    elseif
        ADX.DATA[period3] > ADX_Level and MACD.SIGNAL[period1] > Sell and MACD.MACD[period1] < MACD.SIGNAL[period1] and
            MACD.MACD[period1 - 1] >= MACD.SIGNAL[period1 - 1] and
            (SF.ForceDn[period2] < SF.ResistanceUp[period2])
     then
        if Direction then
            SELL()
        else
