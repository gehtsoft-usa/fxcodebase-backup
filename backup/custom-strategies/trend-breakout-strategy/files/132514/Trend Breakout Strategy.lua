...
local STRATEGY_NAME = "Trend Breakout Strategy";
...
function CreateEntryIndicators(source)
    local profile = core.indicators:findIndicator("TREND BREAKOUT");
    assert(profile ~= nil, "Please, download and install " .. "TREND BREAKOUT" .. ".LUA indicator");
    local indicatorParams = profile:parameters();
    indi = core.indicators:create("TREND BREAKOUT", source, indicatorParams)
end
...
function IsEntryLong(source, period)
    ...
    return core.crossesOver(source.close, indi.DATA, period);
end
function IsEntryShort(source, period)
    if not indi.DATA:hasData(period - 1) then
        return false;
    end
    return core.crossesUnder(source.close, indi.DATA, period);
end
