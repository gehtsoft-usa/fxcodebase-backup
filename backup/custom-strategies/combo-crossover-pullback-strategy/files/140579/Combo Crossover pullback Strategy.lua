...
--http://fxcodebase.com/code/viewtopic.php?f=31&t=70889
...
local STRATEGY_NAME = "Combo Crossover pullback Strategy";
function CreateParameters() 
    strategy.parameters:addInteger("fast_ma_period", "Fast MA Period", "", 6);
    AddAverages("fast_ma_method", "Fast MA Method", "EMA");

    strategy.parameters:addInteger("slow_ma_period", "Slow MA Period", "", 12);
    AddAverages("slow_ma_method", "Slow MA Method", "EMA");

    strategy.parameters:addInteger("trend_filter_ma_period", "Trend Filter MA Period", "", 89);
    AddAverages("trend_filter_ma_method", "Trend Filter MA Method", "MVA");
    
    strategy.parameters:addInteger("fast_hma_period", "Fast Hull MA Period", "", 9);
    AddAverages("fast_hma_method", "Fast Hull MA Method", "HMA");
    
    strategy.parameters:addInteger("trend_hma_period", "Trend HMA Period", "", 50);
    AddAverages("trend_hma_method", "Trend HMA Method", "HMA");
end

local fast_ma, slow_ma, trend_filter_ma, fast_hma, trend_hma;
function CreateEntryIndicators(source)
    fast_ma = CreateAverages(instance.parameters.fast_ma_period, instance.parameters.fast_ma_method, source);
    slow_ma = CreateAverages(instance.parameters.slow_ma_period, instance.parameters.slow_ma_method, source);
    trend_filter_ma = CreateAverages(instance.parameters.trend_filter_ma_period, instance.parameters.trend_filter_ma_method, source);
    fast_hma = CreateAverages(instance.parameters.fast_hma_period, instance.parameters.fast_hma_method, source);
    trend_hma = CreateAverages(instance.parameters.trend_hma_period, instance.parameters.trend_hma_method, source);
end

function UpdateIndicators()
    trend_hma:update(core.UpdateLast);
    fast_hma:update(core.UpdateLast);
    trend_filter_ma:update(core.UpdateLast);
...
function IsEntryLong(source, period)
    if not fast_ma.DATA:hasData(period - 1) 
        or not slow_ma.DATA:hasData(period - 1) 
        or not trend_filter_ma.DATA:hasData(period - 1) 
    then
        return false;
    end
    local condition1 = fast_ma.DATA[period] > slow_ma.DATA[period] and source.close[period] > trend_filter_ma.DATA[period];
    return condition1;
end
function IsEntryShort(source, period)
    ...
    return fast_ma.DATA[period] < slow_ma.DATA[period] and source.close[period] < trend_filter_ma.DATA[period];
end
