...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=67358
...
function Init()
    ...
    strategy.parameters:addString("direction", "Direction", "", "long");
    strategy.parameters:addStringAlternative("direction", "Buy/long", "", "long");
    strategy.parameters:addStringAlternative("direction", "Sell/short", "", "short");
    ...
    strategy.parameters:addDouble("stop", "Stop in Pips", "", 60);
    strategy.parameters:addDouble("limit", "Limit in Pips", "", 30);
    strategy.parameters:addDouble("distance", "Hedging Distance", "", 30);
end
...
local direction;
local offer;
local custom_id = "surefire";
function Prepare(onlyName)
    local name = string.format("%s (%s)", profile:id(), instance.bid:instrument());
    instance:name(name);
    ...
    source = ExtSubscribe1(SOURCE_ID, instance.bid:instrument(), "t1", 0, instance.parameters.is_bid, "bar");
end
...
function OpenEntryOrder()
    offer:refresh();
    local total_profit = trading:FindTrade()
        :WhenCustomID(custom_id)
        :Summ(function (trade) 
                local mult = 1;
                if last_side == trade.BS then
                    mult = -1;
                end
                local pl;
                if last_side == "B" then
                    pl = (trade.Open - math.min(original_stop, original_limit)) * trade.AmountK
                else
                    pl = (math.max(original_stop, original_limit) - trade.Open) * trade.AmountK
                end
                return pl * mult / offer.PointSize
            end);
    local order_amount = math.ceil(-total_profit / limit) + amount;
    local command = trading:EntryOrder(instance.bid:instrument());
    if last_side == "S" then
        command:SetSide("B")
            :SetStop(math.min(original_stop, original_limit))
            :SetLimit(math.max(original_stop, original_limit))
        last_side = "B";
    else
        command:SetSide("S")
            :SetStop(math.max(original_stop, original_limit))
            :SetLimit(math.min(original_stop, original_limit))
        last_side = "S";
    end
    command:SetAccountID(account)
        :SetAmount(order_amount)
        :SetCustomID(custom_id);
    if last_side == original_side then
        command:SetRate(original_rate)
    else
        command:SetRate(opposite_rate)
    end
...
function ExtUpdate(id, source, period) 
    ...
    if trade == nil then
        ...
        command:SetAccountID(account)
            :SetPipStop(nil, stop, nil)
            :SetPipLimit(nil, limit)
            :SetAmount(amount)
            :SetCustomID(custom_id);
        trade = command:Execute();
    end
    if original_rate == nil then
        local trade_row = trade:GetTrade();
        if trade_row ~= nil then
            original_rate = trade_row.Open;
            original_stop = trade_row.Stop;
            original_limit = trade_row.Limit;
            original_side = last_side;
            if last_side == "B" then
                opposite_rate = original_rate - distance * source:pipSize();
            else
                opposite_rate = original_rate + distance * source:pipSize();
            end
            OpenEntryOrder();
    ...
    if order ~= nil then
        if order:GetTrade() ~= nil then
            OpenEntryOrder();
        elseif trade:GetClosedTrade() ~= nil then
            local closed_trade = trade:GetClosedTrade();
            if tables_monitor.closing_order_types[closed_trade.CloseOrderID] == "L" then
                trade = nil;
            else
                trading:FindOrder():WhenCustomID(custom_id):Do(function (order) trading:DeleteOrder(order) end);
            end
        end
    end
end

function ExtAsyncOperationFinished(cookie, successful, message, message1, message2)
