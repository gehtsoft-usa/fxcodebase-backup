...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=69290
...
local STRATEGY_NAME = "HA Strategy with martingale-2";
...
function CreateParameters()
    strategy.parameters:addInteger("ha_bars", "HA bars", "", 3);
    strategy.parameters:addBoolean("use_mva", "Use MVA", "", true);
    strategy.parameters:addInteger("mva_period", "MVA period", "", 100);

    strategy.parameters:addDouble("lot_coeff", "Lot size coefficient", "", 2);
    strategy.parameters:addDouble("max_lot", "Max lot size", "", 100);
    strategy.parameters:addDouble("min_margin", "Min. Margin, %", "", 70);
end

local mva, ha_bars, max_lot, lot_coeff, current_lot;
local min_margin;
function CreateEntryIndicators(source) 
    min_margin = instance.parameters.min_margin / 100;
    lot_coeff = instance.parameters.lot_coeff;
    max_lot = instance.parameters.max_lot;
    if instance.parameters.use_mva then
        mva = core.indicators:create("MVA", source, instance.parameters.mva_period);
    end
    ha_bars = instance.parameters.ha_bars;
    
    function OnTradeClosed(trade)
        core.host:trace("test");
        if trade.PL < 0 then
            current_mult = current_mult * lot_coeff;
        else
            current_mult = 1;
        end
    end
    tables_monitor:ListenCloseTrade(OnTradeClosed);
end
function CreateExitIndicators(source) end

function EnoughtMargin(instrument, amount)
    local account = core.host:findTable("accounts"):find("AccountID", instance.parameters.account);
    local total = account.UsableMargin + account.UsedMargin;
    local emr = core.host:getTradingProperty("EMR", instrument, instance.parameters.account);
    local margin = account.UsableMargin - emr * amount;
    return min_margin < margin / total;
end
...
function CreateCustomActions()
    ...
    enterLongAction.IsPass = function (source, period, periodFromLast, data)
        ...
        if mva ~= nil then
            ...
            if mva.DATA[period] <= source.close[period] then
                return false;
            end
        end
        for i = 0, ha_bars - 1 do
...
function CreatePositionStrategy(source, side, id)
    ...
    function position_strategy:Open(period, periods_from_last)
        ...
        if self.Amount_Type == "lots" then
            command:SetAmount(math.min(max_lot, math.floor(self.Amount * current_mult + 0.5)))
        elseif self.Amount_Type == "risk_equity" then
            command:SetRiskPercentOfEquityAmount(math.min(max_lot, self.Amount * current_mult))
        elseif self.Amount_Type == "equity" then
            command:SetPercentOfEquityAmount(math.min(max_lot, self.Amount * current_mult))
        end
        ...
        command:FillFields();
        local base_unit_size = command:_GetBaseUnitSize()
        if not EnoughtMargin(self.Source.close:instrument(), command.valuemap.Quantity / base_unit_size) then
            return;
        end
...
function trading_logic:Init(parameters)
    if not CustomTimeframeDefined then
        ...
        if not DISABLE_HA_SOURCE then
            parameters:addBoolean("ha_as_source", "Use HA as a source", "", true);
        end
