...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=64638
...
function Init() --The strategy profile initialization
    strategy:name("SR Rate Strategy")
    ...
    strategy.parameters:addGroup("Short Calculation")
    strategy.parameters:addInteger("P11", "Smoothing", "Period", 21)
    strategy.parameters:addInteger("P12", "Period", "Period", 21)
    strategy.parameters:addDouble("B1", "Buy Level", "Level", 0.2)
    strategy.parameters:addDouble("S1", "Sell Level", "Level", 0.8)

    strategy.parameters:addGroup("Long Calculation")
    strategy.parameters:addInteger("P21", "Smoothing", "Smoothing", 50)
    strategy.parameters:addInteger("P22", "Period", "Period", 50)
    strategy.parameters:addDouble("B2", "Buy Level", "Level", 0.5)
    strategy.parameters:addDouble("S2", "Sell Level", "Level", 0.5)
...
function Prepare(nameOnly)
    ...
    --Indicator parameters
    P11 = instance.parameters.P11
    P12 = instance.parameters.P12
    P21 = instance.parameters.P21
    P22 = instance.parameters.P22
    ...
    assert(core.indicators:findIndicator("SR RATE") ~= nil, "Please, download and install SR RATE.LUA indicator")
    ...
    Short = core.indicators:create("SR RATE", Source, P11, P12)
    Long = core.indicators:create("SR RATE", Source, P21, P22)
...
function EntryFunction(now, period)
    ...
    -- BUY: BIG SR RATE > BUY LEVEL AND SMALL SR RATE CROSS OVER ITS BUY LEVEL
    -- SELL: BIG SR RATE< SELL LEVEL AND SMALL SR RATE CROSS UNDER ITS SELL LEVEL

    if Long.DATA[period] > B2 and Short.DATA[period] > B1 and Short.DATA[period - 1] <= B1 then
        if Direction then
            BUY()
        else
            SELL()
        ...
        Return = true
    elseif Long.DATA[period] < S2 and Short.DATA[period] < S1 and Short.DATA[period - 1] >= S1 then
        if Direction then
            SELL()
        else
            BUY()
