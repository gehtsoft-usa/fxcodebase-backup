...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=7207
...
function Init() --The strategy profile initialization
    strategy:name("BOLLIGER BAND AND 2 RSI STRATEGY")
    ...
    strategy.parameters:addInteger("One", "First RSI Period", "", 14)
    strategy.parameters:addInteger("Two", "Second RSI Period", "", 50)

    strategy.parameters:addGroup("Bollinger Calculation")
    strategy.parameters:addInteger("N", "Period", "", 20, 1, 10000)
    strategy.parameters:addDouble("Dev", "Deviation", "", 2.0, 0.0001, 1000.0)
    strategy.parameters:addInteger("BuyLevel", "BuyLevel", "", 50, 0, 100)
    strategy.parameters:addDouble("SellLevel", "SellLevel", "", 50, 0, 100)
...
function Prepare(nameOnly)
    ...
    name =
        ...
        "(" ..
            instance.bid:name() ..
                "." .. instance.parameters.TF .. ", " .. One .. ", " .. Two .. ", " .. N .. "," .. Dev .. ")"
    instance:name(name)
    ...
    Indicator["One"] = core.indicators:create("RSI", Source[Price], One)
    Indicator["Two"] = core.indicators:create("RSI", Source[Price], Two)
    Indicator["BB"] = core.indicators:create("BB", Indicator["One"].DATA, N, Dev)

    first = math.max(Indicator["One"].DATA:first(), Indicator["Two"].DATA:first(), Indicator["BB"].TL:first())
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    Indicator["One"]:update(core.UpdateLast)
    Indicator["Two"]:update(core.UpdateLast)
    Indicator["BB"]:update(core.UpdateLast)

    --BUYING CONDITION
    --RSI(PERIOD 14) CROSSOVER LL OF BOLLINGER BAND AND RSI (PERIOD 50) > 50

    --SELLING CONDITION
    --RSI(PERIOD 14) CROSS UNDER UL OF BOLLINGER BAND AND RSI (PERIOD 50) < 50

    if core.crossesOver(Indicator["One"].DATA, Indicator["BB"].BL, period) and Indicator["Two"].DATA[period] > BuyLevel then
    ...
    elseif
        core.crossesUnder(Indicator["One"].DATA, Indicator["BB"].TL, period) and
            Indicator["Two"].DATA[period] < SellLevel
     then
        if Direction == "direct" then
            SELL()
        else
    ...
    if
        core.crossesUnder(Indicator["One"].DATA, Indicator["BB"].TL, period) or
            core.crossesUnder(Indicator["Two"].DATA, BuyLevel, period)
     then
    ...
    elseif
        core.crossesOver(Indicator["One"].DATA, Indicator["BB"].BL, period) or
            core.crossesOver(Indicator["Two"].DATA, SellLevel, period)
     then
    ...
    --EXIT BUY CONDITIONS:
    --1.RSI[PERIOS14] CROSS UNDER UL OF BOLLINGERBAND [20,2]
    --{OR}
    --2.RSI [PERIOD50] CROSS UNER BUY LEVEL{50}

    --EXIT SELL CONDITIONS:
    --1. RSI[PERIOD 14] CROSSOVER LL OF BOLLINGERBAND [20,2]
    --{OR}
    --2 .RSI [PERIOD 50] CROSS OVER SELL LEVEL{50}
end
