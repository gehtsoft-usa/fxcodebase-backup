...
function Init()
	...
	strategy.parameters:addInteger("SC", "Slow MVA Period of Close", "Slow MVA Period of Close", 36);
...
function Prepare(nameOnly)
   ...
   Initialization();
	...
	indicator["SC"]  = core.indicators:create("MVA", gSource.close, SC);
	first = math.max(first,indicator["SC"].DATA:first());
	
	indicator["SL"]  = core.indicators:create("MVA", gSource.low, SL);
    first = math.max(first,indicator["SL"].DATA:first());	
	
	indicator["SH"]  = core.indicators:create("MVA", gSource.high, SH);
	first = math.max(first,indicator["SH"].DATA:first());
	
	indicator["FC"]  = core.indicators:create("MVA", gSource.close, FC);
