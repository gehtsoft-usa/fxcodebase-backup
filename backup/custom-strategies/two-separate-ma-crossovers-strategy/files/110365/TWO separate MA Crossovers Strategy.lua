...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=64268
...
function Init() --The strategy profile initialization
    strategy:name("TWO separate MA Crossovers Strategy")
    ...
    strategy.parameters:addGroup("NR SUPER TREND Calculation")
    strategy.parameters:addInteger("NR_Period", "Period", "", 10)
    strategy.parameters:addDouble("NR_Multiplier", "Multiplier", "", 4)

    strategy.parameters:addGroup("1. MA Calculation")

    strategy.parameters:addInteger("Period1", "MA Period", "Period", 1)
    ...
    strategy.parameters:addInteger("Period2", "MA Period", "Period", 1)
    ...
    strategy.parameters:addInteger("Period3", "MA Period", "Period", 1)
    ...
    strategy.parameters:addGroup("4. MA Calculation")

    strategy.parameters:addInteger("Period4", "MA Period", "Period", 1)
    strategy.parameters:addString("Method4", "MA Method", "Method", "MVA")
...
local Period1, Method1, Period2, Method2, Period3, Method3, Period4, Method4
local HA1, HA2
local NR_Period, NR_Multiplier, NR_SUPER_TREND
function Prepare(nameOnly)
    ...
    NR_Period = instance.parameters.NR_Period
    NR_Multiplier = instance.parameters.NR_Multiplier
    ...
    assert(
        core.indicators:findIndicator("NR_SUPER_TREND") ~= nil,
        "Please, download and install NR_SUPER_TREND.LUA indicator"
    )
    ...
    HA1 = core.indicators:create("HA", Source)
    HA2 = core.indicators:create("HA", HA1:getCandleOutput(0))

    NR_SUPER_TREND = core.indicators:create("NR_SUPER_TREND", HA1:getCandleOutput(0), NR_Period, NR_Multiplier)

    assert(core.indicators:findIndicator(Method1) ~= nil, Method1 .. " indicator must be installed");
    MA1 = core.indicators:create(Method1, HA2.close, Period1)
    assert(core.indicators:findIndicator(Method2) ~= nil, Method2 .. " indicator must be installed");
    MA2 = core.indicators:create(Method2, NR_SUPER_TREND.DATA, Period2)
    assert(core.indicators:findIndicator(Method3) ~= nil, Method3 .. " indicator must be installed");
    MA3 = core.indicators:create(Method3, HA1.close, Period3)
    assert(core.indicators:findIndicator(Method4) ~= nil, Method4 .. " indicator must be installed");
    MA4 = core.indicators:create(Method4, HA2.close, Period4)
    first =
        math.max(
        MA1.DATA:first(),
        MA2.DATA:first(),
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    HA2:update(core.UpdateLast)

    NR_SUPER_TREND:update(core.UpdateLast)
...
function EntryFunction(period, now)
    ...
    if MA1.DATA[period] > MA2.DATA[period] and MA3.DATA[period] > MA4.DATA[period] then
        if Direction then
            BUY()
        else
            SELL()
        ...
        Return = true
    elseif MA1.DATA[period] < MA2.DATA[period] and MA3.DATA[period] < MA4.DATA[period] then
        if Direction then
            SELL()
        else
            BUY()
...
function ExitFunction(period, now)
    ...
    if
        haveTrades("B") and
            ((MA1.DATA[period] > MA2.DATA[period] and MA3.DATA[period] < MA4.DATA[period]) or
                (MA1.DATA[period] < MA2.DATA[period] and MA3.DATA[period] > MA4.DATA[period]))
     then
        exitSpecific("B")
        Signal("Close Long")
