...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=19839
...
function Init() --The strategy profile initialization
    strategy:name("Highly adaptable Dochian Channel With ADX Strategy")
    strategy:description("")
    strategy:setTag("Version", "1");
    ...
    strategy.parameters:addDouble("adx_level", "ADX Level", "", 0);
    ...
    strategy.parameters:addString("Action" .. 3, "Bottom Line Cross Over", "", "NO")
    ...
    strategy.parameters:addString("Action" .. 4, "Bottom Cross Under", "", "NO")
    ...
    strategy.parameters:addString("Action" .. 7, "Top Sub Level Line Cross Over", "", "NO")
    ...
    strategy.parameters:addString("Action" .. 8, "Top Sub Level Line Cross  Under", "", "NO")
    ...
    strategy.parameters:addString("Action" .. 9, "Bottom Sub Level Line Cross Over", "", "NO")
    ...
    strategy.parameters:addString("Action" .. 10, "Bottom Sub Level Line Cross Under", "", "NO")
...
function Prepare(nameOnly)
    ...
    Indicators[1] = core.indicators:create("DNC_V3", Source, N, MD, "Both", true, true, true)
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if period < first or not Indicators[1].DU:hasData(period - 1) then
        return
    end

    if core.crossesOver(Source.close, Indicators[1].DU, period) and adx.DATA[period] > adx_level then
        ACTION(1)
        ONE = Source:serial(period)
    end
    if core.crossesUnder(Source.close, Indicators[1].DU, period) and adx.DATA[period] > adx_level then
        ACTION(2)
        ONE = Source:serial(period)
    end
    if core.crossesOver(Source.close, Indicators[1].DN, period) and adx.DATA[period] > adx_level then
        ACTION(3)
        ONE = Source:serial(period)
    end
    if core.crossesUnder(Source.close, Indicators[1].DN, period) and adx.DATA[period] > adx_level then
        ACTION(4)
        ONE = Source:serial(period)
    end
    if core.crossesOver(Source.close, Indicators[1].DM, period) and adx.DATA[period] > adx_level then
        ACTION(5)
        ONE = Source:serial(period)
    end
    if core.crossesUnder(Source.close, Indicators[1].DM, period) and adx.DATA[period] > adx_level then
        ACTION(6)
        ONE = Source:serial(period)
    end
    if core.crossesOver(Source.close, Indicators[1].high, period) and adx.DATA[period] > adx_level then
        ACTION(7)
        ONE = Source:serial(period)
    end
    if core.crossesUnder(Source.close, Indicators[1].high, period) and adx.DATA[period] > adx_level then
        ACTION(8)
        ONE = Source:serial(period)
    end

    if core.crossesOver(Source.close, Indicators[1].low, period) and adx.DATA[period] > adx_level then
        ACTION(9)
        ONE = Source:serial(period)
    end
    if core.crossesUnder(Source.close, Indicators[1].low, period) and adx.DATA[period] > adx_level then
        ACTION(10)
        ONE = Source:serial(period)
    end
end
