...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=60557
...
function Init() --The strategy profile initialization
    strategy:name("MACD BB Strategy")
    ...
    strategy.parameters:addInteger("Fast", "Fast period EMA", "", 12)
    strategy.parameters:addInteger("Slow", "Slow period EMA", "", 26)
    strategy.parameters:addInteger("Signal", "Signal period", "", 9)

    strategy.parameters:addGroup("BB indicator parameters")
    strategy.parameters:addInteger("Length", "Period", "Period", 20)
...
function Prepare(nameOnly)
    ...
    MACD = core.indicators:create("MACD", Source.close, Fast, Slow, signal)
    BB = core.indicators:create("BB", MACD.DATA, Length, Deviation)
    first = math.max(MACD.DATA:first(), BB.DATA:first()) + 1
end
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
	...
	long when MACD crosses over bb lower bollinger
and macd> 0
stop the long when MACD crosses under bb highter bollinger

short when macd crosses under bb highter bollinger
and close <close-1
stop short when macd crosses over bb lower bollinger
	]]
    if (MACD.DATA[period] > 0 and core.crossesOver(MACD.DATA, BB.BL, period)) then
        BUY()
    elseif (MACD.DATA[period] < 0 and core.crossesUnder(MACD.DATA, BB.TL, period)) then
        SELL()
    end

    if core.crossesUnder(MACD.DATA, BB.TL, period) then
        exitSpecific("B")
    end

    if core.crossesOver(MACD.DATA, BB.BL, period) then
        exitSpecific("S")
    end
end
