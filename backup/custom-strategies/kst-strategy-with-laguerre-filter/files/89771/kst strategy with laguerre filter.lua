...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=59596&p
...
function Init() --The strategy profile initialization
    strategy:name("kst strategy with laguerre filter")
    ...
    strategy.parameters:addGroup("KST Calculation")
    ...
    strategy.parameters:addInteger("BB_Period", "Bands Period", "", 20)
    strategy.parameters:addDouble("BB_Deviation", "Bands Deviation", "", 2)

    strategy.parameters:addGroup("Laguerre Filter Calculation")
    strategy.parameters:addDouble("GAMMA", "Gamma", "No description", 0.7, -0.5, 1.05)
    ...
    strategy.parameters:addBoolean("Use", "Use Price Filter", "", true)
...
local BaseSize
local GAMMA
local KST_Ind, BB_Ind
local Top, Bottom, KST
local Laguerre_Filter, Laguerre
local Source
local Use, Exit
local Direction
...
function Prepare(nameOnly)
    ...
    KST_Ind =
        ...
        instance.parameters.MA1,
        instance.parameters.MET1,
        instance.parameters.ROC2,
        instance.parameters.MA2,
        instance.parameters.MET2,
        instance.parameters.ROC3,
        instance.parameters.MA3,
        instance.parameters.MET3,
        instance.parameters.ROC4,
        instance.parameters.MA4,
        instance.parameters.MET4,
        2,
        "MVA"
    )
    BB_Ind = core.indicators:create("BB", KST_Ind.KST, instance.parameters.BB_Period, instance.parameters.BB_Deviation)
    Laguerre_Filter = core.indicators:create("LAGUERRE_FILTER", Source, GAMMA)

    KST = KST_Ind:getStream(0)
    Top = BB_Ind:getStream(0)
    Bottom = BB_Ind:getStream(1)
    Laguerre = Laguerre_Filter:getStream(0)

    first = math.max(Top:first(), Laguerre:first())
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    KST_Ind:update(core.UpdateLast)
    BB_Ind:update(core.UpdateLast)
    Laguerre_Filter:update(core.UpdateLast)

    if period < first + 1 then
        return
    end

    if core.crossesOver(KST, Bottom, period) and (Source.close[period] < Laguerre[period] or not Use) then
        if Direction then
            BUY()
        else
            SELL()
        end
    elseif core.crossesUnder(KST, Top, period) and (Source.close[period] > Laguerre[period] or not Use) then
        if Direction then
            SELL()
        else
            BUY()
    ...
    if Exit then
        if Source.close[period] > Laguerre[period] then
            if Direction then
