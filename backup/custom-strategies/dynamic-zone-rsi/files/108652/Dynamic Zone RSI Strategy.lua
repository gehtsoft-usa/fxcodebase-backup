...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=63990
...
function Init() --The strategy profile initialization
    strategy:name("Dynamic Zone RSI Strategy")
    ...
    strategy.parameters:addInteger("RF", "RSI Frame", "", 5)
    strategy.parameters:addInteger("BF", "Band Period", "", 30)
    strategy.parameters:addDouble("SD", "Standard deviation", "", 1.3185)
...
local first
local RF, BF, SD, Indicator
local BL, SL
local Direction
local CustomID
local PositionCap
local Method, MA, Period, Filter
...
function Prepare(nameOnly)
    ...
    assert(
        core.indicators:findIndicator("DYNAMIC ZONE RSI") ~= nil,
        "Please, download and install DYNAMIC ZONE RSI.LUA indicator"
    )
    ...
    Indicator = core.indicators:create("DYNAMIC ZONE RSI", Source, RF, BF, SD)
    first = math.max(Indicator.RSI:first(), Indicator.CENTRAL:first())
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if not Indicator.DATA:hasData(period) or (Filter and not MA.DATA:hasData(period)) then
        return
    end

    --[[ 
	buy: center line > buy level and rsi cross over bottom line
	sell: center line < sell level and rsi cross under top line
	]]
    if
        Indicator.CENTRAL[period] > BL and Indicator.RSI[period] > Indicator.DOWN[period] and
            Indicator.RSI[period - 1] <= Indicator.DOWN[period - 1] and
    ...
    elseif
        Indicator.CENTRAL[period] < SL and Indicator.RSI[period] < Indicator.UP[period] and
            Indicator.RSI[period - 1] >= Indicator.UP[period - 1] and
            (not Filter or (Filter and Source.close[period] < MA.DATA[period]))
     then
        if Direction then
            SELL()
