...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=61872
...
function Init() --The strategy profile initialization
    strategy:name("ADX FRACTAL STRATEGY")
    ...
    strategy.parameters:addInteger("DMI_Period", "Period", "", 14)

    strategy.parameters:addGroup("CCI Calculation")
    strategy.parameters:addInteger("CCI_Period", "Period", "", 200)
...
local first
local Exit
local ADX_Period, DMI_Period, ADX, DMI, CCI, CCI_Period, BL, SL
local Direction
local CustomID
...
function Prepare(nameOnly)
    ...
    first = math.max(CCI.DATA:first(), DMI.DATA:first(), ADX.DATA:first()) + 1
...
function Calculate(period)
    local Flag = 0
    local curr
    for i = period, Source.close:first(), -1 do
        curr = Source.high[i - 2]
        if
            (curr > Source.high[i - 4] and curr > Source.high[i - 3] and curr > Source.high[i - 1] and
                curr > Source.high[i])
         then
            Flag = -1
            break
        ...
        curr = Source.low[i - 2]
        if (curr < Source.low[i - 4] and curr < Source.low[i - 3] and curr < Source.low[i - 1] and curr < Source.low[i]) then
            Flag = 1
            break
        end
    end
...
function Level(period)
    local Up = -1
    local Down = -1

    for i = period, Source.close:size() - 1, -1 do
        curr = Source.high[i - 2]
        if
            (curr > Source.high[i - 4] and curr > Source.high[i - 3] and curr > Source.high[i - 1] and
                curr > Source.high[i])
...
EXIT BUY: CCI CROSS UNDER BUY LEVEL OR PRICE CROSS UNDER DOWN FRACTAL

EXIT SELL: CCI CROSS OVER SELL LEVEL OR PRICE CROSS OVER UP FRACAL
	]]
    local X1, Level1 = Calculate(period)
    local X2, Level2 = Calculate(period - 1)

    -- only buy if we have a fast cross over slow and the price is above the moving averages.
    if
        ADX.DATA[period] > ADX.DATA[period - 1] and ADX.DATA[period - 1] > ADX.DATA[period - 2] and
            DMI.DIP[period] > DMI.DIM[period] and
            (CCI.DATA[period - 2] > BL or CCI.DATA[period - 1] > BL or CCI.DATA[period] > BL) and
            (X1 == 1) and
            (X2 == -1)
     then
        if Direction then
        ...
        ONE = Source.close:serial(period)
    elseif
        ADX.DATA[period] > ADX.DATA[period - 1] and ADX.DATA[period - 1] > ADX.DATA[period - 2] and
            DMI.DIP[period] < DMI.DIM[period] and
            (CCI.DATA[period - 2] < SL or CCI.DATA[period - 1] < SL or CCI.DATA[period] < SL) and
            (X1 == -1) and
            (X2 == 1)
     then
        if Direction then
    ...
    local Up, Down = Level(period)

    if Exit then
        if core.crossesUnder(CCI.DATA, BL, period) or core.crossesUnder(Source.close, Down, period) then
            if Direction then
        ...
        if core.crossesOver(CCI.DATA, SL, period) or core.crossesOver(Source.close, Up, period) then
            if Direction then
                if haveTrades("S") then
                    exitSpecific("S")
                    Signal("Close Short")
