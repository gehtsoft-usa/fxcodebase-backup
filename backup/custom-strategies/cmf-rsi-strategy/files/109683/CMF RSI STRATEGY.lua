...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=64195
...
function Init() --The strategy profile initialization
    strategy:name("CMF RSI STRATEGY")
    ...
    strategy.parameters:addGroup("CMF Calculation")
    strategy.parameters:addInteger("N", "Period", "", 14, 1, 1000)
    strategy.parameters:addString("ADMethod", "Method", "", "CS")
    strategy.parameters:addStringAlternative("ADMethod", "Classic", "", "CS")
    strategy.parameters:addStringAlternative("ADMethod", "ClassicIncremental", "", "CI")
    strategy.parameters:addStringAlternative("ADMethod", "TradeStation", "", "TS")

    strategy.parameters:addDouble("CB", "Buy Level", "", 0)
    strategy.parameters:addDouble("CS", "Sell Level", "", 0)
    ...
    strategy.parameters:addDouble("RS", "Sell Level", "", 50)
...
local CMF, N, ADMethod, CB, CS
local RSI, Perios, RS, RB
local name
function Prepare(nameOnly)
    ...
    ADMethod = instance.parameters.ADMethod
    ...
    CMF = core.indicators:create("CMF", Source, N, ADMethod)
    RSI = core.indicators:create("RSI", Source, Period)
    first = math.max(CMF.DATA:first(), RSI.DATA:first())
...
function EntryFunction(period, now)
    ...
    if CMF.DATA[period] > CB and CMF.DATA[period - 1] <= CB and RSI.DATA[period] > RB then
        if Direction then
            BUY()
        else
            SELL()
        ...
        Return = true
    elseif CMF.DATA[period] < CS and CMF.DATA[period - 1] >= CS and RSI.DATA[period] < RS then
        if Direction then
            SELL()
        else
            BUY()
