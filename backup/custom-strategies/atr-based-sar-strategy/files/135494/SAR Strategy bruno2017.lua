...
local STRATEGY_NAME = "SAR Strategy";
...
function CreateEntryIndicators(source)
    indi = core.indicators:create("SAR", source, instance.parameters.sar_step, instance.parameters.sar_max);
end
...
local use_stop, stop_pips, use_limit, limit_pips, use_trailing, trailing, use_position_limit, position_limit;
...
local last_entry, last_exit, last_delete;
function ExtUpdate(id, source, period)
    indi:update(core.UpdateLast);
    if use_mandatory_closing and core.host.Trading:getTradingProperty("isSimulation") then
        DoMandatoryClosing();
    end
    if id ~= entry_source_id or indi.DATA:size() == 0 then
        return;
    end
    local entry_period = main_source:size() - 1;
    if last_delete ~= main_source:date(NOW) then
        last_delete = main_source:date(NOW);
        DeleteOrders();
    end
    ...
    if last_entry ~= main_source:date(NOW) then
        if AllowTrade then
            local up, dn;
            for i = indi.DATA:size() - 1, 0, -1 do
                if up == nil and indi.UP:hasData(i) then
                    up = indi.UP[i];
                end
                if dn == nil and indi.DN:hasData(i) then
                    dn = indi.DN[i];
                end
                if up ~= nil and dn ~= nil then
                    break;
                end
            end
            OpenTrade("B", up);
