...
-- https://fxcodebase.com/code/viewtopic.php?f=31&t=72949
...
function Init() --The strategy profile initialization
    strategy:name("Engulfing pattern Strategy");
    ...
    strategy.parameters:addInteger("Period1", "Period", "", 13);  
...
function CreateTradingParameters()
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "EPS");
...
function Prepare( nameOnly)
    ...
    MA1 = core.indicators:create(Method, Source1.close, Period1 );
    MA2 = core.indicators:create(Method, Source1.close, Period2 ); 
...
function EntryFunction( id, now,period)
	...
	if   Source2.close[period2]>  Source2.open[period2]
	and Source2.close[period2-1]<  Source2.open[period2-1]
	and Source2.close[period2] >= Source2.open[period2-1]
	and Source2.open[period2] <= Source2.close[period2-1]	
	then
	Engulfing=1;
	end
    ...
    if   MA1.DATA[period1]>  MA2.DATA[period1]
	and  MA1.DATA[period1-1]<= MA2.DATA[period1-1]	
	and Engulfing== 1
	then	
          if Direction then
                BUY();
		...
		Return=true;
    elseif   MA1.DATA[period1]<  MA2.DATA[period1]
	and  MA1.DATA[period1-1]>= MA2.DATA[period1-1]
	and Engulfing== -1	
	then
            if Direction then
