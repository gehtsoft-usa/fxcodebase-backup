...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=64488
...
function Init() --The strategy profile initialization
    strategy:name("Jubilee Bar Strategy")
    ...
    strategy.parameters:addInteger("CCI_Period", "CCI Period", "", 40)
    strategy.parameters:addDouble("CCI_Long", "CCI Long Level", "", 100)
    strategy.parameters:addDouble("CCI_Short", "CCI Short Level", "", -100)

    strategy.parameters:addGroup("BB Calculation")
    strategy.parameters:addInteger("BB_Period", "Number of periods", "Number of periods", 10)
    ...
    strategy.parameters:addDouble("BB_Long", "BB Long Level", "", 100)
    strategy.parameters:addDouble("BB_Short", "BB Short Level", "", 0)

    strategy.parameters:addGroup("DMI Calculation")

    strategy.parameters:addInteger("DMI_Period", "Number of periods", "Number of periods", 12)
    strategy.parameters:addDouble("DMI_Level", "DX Level", "DX Levels", 40)
...
local CCI, CCI_Period, CCI_Long, CCI_Short
local BB, BB_Period, BB_Deviations, BB_Long, BB_Short
local DMI_Period, DMI, DMI_Level
function Prepare(nameOnly)
    ...
    CCI_Long = instance.parameters.CCI_Long
    CCI_Short = instance.parameters.CCI_Short
    ...
    BB_Long = instance.parameters.BB_Long
    BB_Short = instance.parameters.BB_Short
    ...
    assert(
        core.indicators:findIndicator("BB-PERCENTAGES") ~= nil,
        "Please, download and install BB-PERCENTAGES.LUA indicator"
    )
    ...
    BB = core.indicators:create("BB-PERCENTAGES", Source.close, BB_Period, BB_Deviations)
    DMI = core.indicators:create("DMI", Source, DMI_Period)
    first = math.max(CCI.DATA:first(), BB.DATA:first(), DMI.DATA:first())
...
function EntryFunction(now, period)
    ...
    if (div == 0) then
        ...
        DX = 100 * (math.abs(plus - minus) / div)
    end

    -- only buy if we have a fast cross over slow and the price is above the moving averages.
    if CCI.DATA[period] > CCI_Long and BB.DATA[period] > BB_Long and DX > DMI_Level then
        if Direction then
            BUY()
        else
            SELL()
        ...
        Return = true
    elseif CCI.DATA[period] < CCI_Short and BB.DATA[period] < BB_Short and DX > DMI_Level then
        if Direction then
            SELL()
        else
            BUY()
