...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=67296
--+------------------------------------------------------------------+
...
function Init() --The strategy profile initialization
    strategy:name("ICHIMOKO KINKO HYO Strategy");
    ...
    strategy.parameters:addInteger("P1", "Tenkan-sen period", "Tenkan-sen period", 9, 1, 1000);
    strategy.parameters:addInteger("P2", "Kijun-sen period", "Kijun-sen period", 26, 1, 1000);
    strategy.parameters:addInteger("P3", "Senkou Span B period", "Senkou Span B period", 52, 1, 1000);
...
function CreateTradingParameters()
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "IKHS");
...
function Prepare( nameOnly)
    ...
    Indicator = core.indicators:create("ICH", Source, instance.parameters.P1,instance.parameters.P2,instance.parameters.P3  );
	Short["A"] = Indicator:getStream(3);
	Short["B"] = Indicator:getStream(4);
	Short["Tenkan"] = Indicator:getStream(0);
    Short["Kijun"] = Indicator:getStream(1);
	Short["Chinoku"] = Indicator:getStream(2);
	
	Shift=instance.parameters.P2;
...
function ExitFunction( now,period)
	 ...
	 return;
		            ...
		            if   Short["Tenkan"][period]< Short["Kijun"][period]
					then
...
function EntryFunction( now,period)
	...
	if not Short["Chinoku"]:hasData(period-Shift) 
	or not Short["A"]:hasData(period-Shift) 
	then
	return;
	end
    ...
    if  Source.close[period]  > math.max(Short["A"][period] , Short["B"][period])
	and Short["A"][period+Shift]  > Short["B"][period+Shift]
	and  Short["Tenkan"][period]> Short["Kijun"][period]
	and  Short["Chinoku"][period-Shift]> math.max(Short["A"][period-Shift] , Short["B"][period-Shift])
	then	
          if Direction then
                BUY();
            else
		...
		Return=true;
    elseif  Source.close[period]  < math.min(Short["A"][period] , Short["B"][period])
	and Short["A"][period+Shift]  < Short["B"][period+Shift]
	and  Short["Tenkan"][period]< Short["Kijun"][period]
	and  Short["Chinoku"][period-Shift]< math.max(Short["A"][period-Shift] , Short["B"][period-Shift])
	then
