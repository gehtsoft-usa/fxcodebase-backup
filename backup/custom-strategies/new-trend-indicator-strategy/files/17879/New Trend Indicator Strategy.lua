...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=8091
...
function Init() --The strategy profile initialization
    strategy:name("New Trend Indicator Strategy");
	...
	strategy.parameters:addGroup("Calculation");
	 ...
	 strategy.parameters:addBoolean("Confirmation", "Two Period Confirmation", "", false);
	strategy.parameters:addInteger("Parameters"..1, "ATR Period", "ATR Period", 12);
    strategy.parameters:addInteger("Parameters"..2, "MVA Period", "MVA Period", 14);
	
	strategy.parameters:addString("CLOSE", "Close on", "", "R");
    strategy.parameters:addStringAlternative("CLOSE", "Reversal", "", "R");
    strategy.parameters:addStringAlternative("CLOSE", "Indecision", "", "I");
...
function Prepare(nameOnly)
    ...
    for i= 1, 2, 1 do
    Parameters[i]= instance.parameters:getInteger("Parameters".. i);
	...
	assert(core.indicators:findIndicator("NEW TREND") ~= nil, "Please, download and install NEW TREND.LUA indicator");            
    ...
    Indicator[1] = core.indicators:create("NEW TREND", Source,  Parameters[1], Parameters[2], true,   core.rgb(0, 255, 0),  core.rgb(255, 0, 0) ,  core.rgb(255, 128, 0)   );
   ...
   -- get only time
    ...
    if period < first +1  then
		...
		(
		 ...
		 and   Indicator[1].DATA:colorI(period-1) ==  core.rgb(0, 255, 0) 
		  and   Indicator[1].DATA:colorI(period-2) ~=  core.rgb(0, 255, 0) 
		   and  Confirmation
		 	)
		then
		...
		or
		(
		Indicator[1].DATA:colorI(period) ==  core.rgb(255, 0, 0) 
		and Indicator[1].DATA:colorI(period-1) ==  core.rgb(255, 0, 0) 
		and Indicator[1].DATA:colorI(period-2) ~=  core.rgb(255, 0, 0) 
		 and  Confirmation
		)
	 ...
	 if   CLOSE == "R" then
					...
					if Indicator[1].DATA:colorI(period) ==  core.rgb(255, 0, 0) then
					 ...
					 elseif Indicator[1].DATA:colorI(period) ==  core.rgb(0, 255, 0)  then 
								   if   Direction then    
										if haveTrades("S") then
										   exit("S");
										   Signal ("Close Short");
		...
		elseif   CLOSE == "I" then
		           	...
		           	if Indicator[1].DATA:colorI(period) ~=  core.rgb(0, 255, 0) then
					 ...
					 elseif Indicator[1].DATA:colorI(period) ~=  core.rgb(255, 0, 0)  then 
								   if   Direction then    
										if haveTrades("S") then
										   exit("S");
										   Signal ("Close Short");
