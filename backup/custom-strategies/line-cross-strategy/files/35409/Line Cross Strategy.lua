...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=20142
...
function Init() --The strategy profile initialization
    strategy:name("Line Cross Strategy")
    strategy:description("")

    strategy:setTag("NonOptimizableParameters", "Email,SendEmail,SoundFile,RecurrentSound,PlaySound, ShowAlert")

    strategy.parameters:addGroup("Line Cross Strategy")
    ...
    strategy.parameters:addDouble("L1", "1. Level", "", 0)
    strategy.parameters:addDouble("L2", "2. Level", "", 0)
    strategy.parameters:addDouble("L3", "3. Level", "", 0)
    strategy.parameters:addDouble("L4", "4. Level", "", 0)
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if core.crossesOver(Source.close, L1, period) then
        ACTION(1)
    end
    if core.crossesUnder(Source.close, L1, period) then
        ACTION(2)
    end
    if core.crossesOver(Source.close, L2, period) then
        ACTION(3)
    end
    if core.crossesUnder(Source.close, L2, period) then
        ACTION(4)
    end
    if core.crossesOver(Source.close, L3, period) then
        ACTION(5)
    end
    if core.crossesUnder(Source.close, L3, period) then
        ACTION(6)
    end
    if core.crossesOver(Source.close, L4, period) then
        ACTION(7)
    end
    if core.crossesUnder(Source.close, L4, period) then
        ACTION(8)
    end
end
