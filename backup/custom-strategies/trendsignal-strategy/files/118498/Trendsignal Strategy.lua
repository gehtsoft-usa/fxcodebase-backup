...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=65877
...
function Init() --The strategy profile initialization
    strategy:name("Trendsignal Strategy")
    strategy:description("v2 2018-11-18")
    ...
    strategy.parameters:addInteger("RISK", "Risk", "", 3)
    strategy.parameters:addInteger("SSP", "SSP", "", 9)

    strategy.parameters:addGroup("MA Filter Calculation")

    strategy.parameters:addBoolean("Filter", "Use Filter", "", false)
    strategy.parameters:addInteger("P1", "Short MA Period", "", 20)
    strategy.parameters:addString("Method1", "Short MA Method", "Method", "EMA")
    ...
    strategy.parameters:addInteger("P2", "Long MA Period", "", 200)

    strategy.parameters:addString("Method2", "Long MA Method", "Method", "EMA")
    ...
    strategy.parameters:addBoolean("exit_on_signal", "Exit on opposite signal", "", false);
...
local RISK, SSP
local Last
local MA1, MA2, Filter, P1, P2, Method1, Method2
...
function Prepare(nameOnly)
    ...
    assert(
        core.indicators:findIndicator("TRENDSIGNAL") ~= nil,
        "Please, download and install TRENDSIGNAL.LUA indicator"
    )
    ...
    Indicator = core.indicators:create("TRENDSIGNAL", Source, RISK, SSP, true)

    assert(core.indicators:findIndicator(Method1) ~= nil, Method1 .. " indicator must be installed");
    MA1 = core.indicators:create(Method1, Source.close, P1)
    assert(core.indicators:findIndicator(Method2) ~= nil, Method2 .. " indicator must be installed");
    MA2 = core.indicators:create(Method2, Source.close, P2)
    first = math.max(Source.close:first(), MA1.DATA:first(), MA2.DATA:first())
...
function IsBuyCondition(source, period)
    if Indicator.Signal[period] == 1 and (not Filter or MA1.DATA[period] > MA2.DATA[period]) then
        return true;
    end
    return Filter and core.crossesOver(MA1.DATA, MA2.DATA, period) and FindLast(period) == 1;
end
...
function IsSellCondition(source, period)
    ...
    return Filter and core.crossesUnder(MA1.DATA, MA2.DATA, period) and FindLast(period) == -1;
end

function IsExitBuyCondition(source, period)
    return instance.parameters.exit_on_signal and Indicator.Signal[period] == -1;
end
...
function EntryFunction(now, period)
    ...
    if IsBuyCondition(Source, period) then
        ...
        Return = true
    elseif IsSellCondition(Source, period) then
        if Direction then
            SELL()
        else
            BUY()
