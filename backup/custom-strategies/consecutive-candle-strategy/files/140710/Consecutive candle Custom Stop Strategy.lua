...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=70278
...
function Init() --The strategy profile initialization
    strategy:name("Consecutive candle Strategy")
    ...
    strategy.parameters:addInteger("OpenPeriod", "Consecutive candle to Open", "", 2)
    strategy.parameters:addInteger("ClosePeriod", "Consecutive candle to Close", "", 2)
...
function CreateTradingParameters()
    ...
    strategy.parameters:addInteger("Stop", "Stop Order, bars", "", 5)
...
function Prepare(nameOnly)
    ...
    --Indicator parameters
    OpenPeriod = instance.parameters.OpenPeriod - 1
    ClosePeriod = instance.parameters.ClosePeriod - 1
    ...
    -- Indicator = core.indicators:create("EXTREME_TMA_LINE", Source, TMA_Period, ATR_Period, ATR_Mult, TrendThreshold, true,Redraw,  core.rgb(128, 128, 128),  core.rgb(0, 255, 0),  core.rgb(255, 0, 0)  );
...
function ExitFunction(id, now, period)
    ...
    local ConsecutiveNumber = CountFunction(period)
    ...
    if ConsecutiveNumber >= ClosePeriod and Source.close[period] < Source.open[period] then
        if Direction then
...
function EntryFunction(id, now, period)
    ...
    if ConsecutiveNumber >= OpenPeriod and Source.close[period] > Source.open[period] then
        if Direction then
            BUY()
        else
            SELL()
        ...
        Return = true
    elseif ConsecutiveNumber >= OpenPeriod and Source.close[period] < Source.open[period] then
        if Direction then
            SELL()
        else
            BUY()
...
function CountFunction(start_period)
    local Return = 0

    for period = start_period - 1, first, -1 do
        if Source.close[start_period] > Source.open[start_period] and Source.close[period] > Source.open[period] then
            Return = Return + 1
        elseif Source.close[start_period] < Source.open[start_period] and Source.close[period] < Source.open[period] then
            Return = Return + 1
        else
            break
        end
...
function MarketOrder(BuySell, hCount)
    ...
    if SetStop then
        if BuySell == "B" then
            local stop = mathex.min(Source, core.rangeTo(Source:size() - 1, Stop))
            valuemap.RateStop = stop
        else
            local stop = mathex.max(Source, core.rangeTo(Source:size() - 1, Stop))
            valuemap.RateStop = stop
        end
    end
