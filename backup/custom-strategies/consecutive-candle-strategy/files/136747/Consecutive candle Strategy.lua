...
function CreateTradingParameters()
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "CCS");
...
function ExitFunction(id, now,period)
	...
	--/////////////////////////////////////////////////////////////////////
				 ...
				 if    ConsecutiveNumber >= ClosePeriod
...
function EntryFunction( id, now,period)
    ...
    if   ConsecutiveNumber >= OpenPeriod 
	and   Source.close[period] > Source.open[period] 
	then	
          if Direction then
                BUY();
		...
		Return=true;
    elseif    ConsecutiveNumber >= OpenPeriod 
	and   Source.close[period] < Source.open[period] 
	then	
...
function CountFunction(start_period) 
  ...
  for period =start_period-1, first, -1 do
   ...
   if Source.close[start_period]>Source.open[start_period]
   and  Source.close[period]>Source.open[period]
   then   
   Return=Return+1;
   elseif Source.close[start_period]<Source.open[start_period]
   and  Source.close[period]<Source.open[period]
   then
   Return=Return+1;
   else
