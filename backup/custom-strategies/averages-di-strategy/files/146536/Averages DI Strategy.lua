...
function Init() --The strategy profile initialization
    strategy:name("Averages DI Strategy.lua");
	...
	strategy.parameters:addGroup("Averages DI Calculation");
    ...
    strategy.parameters:addInteger("Period", "Period", "Period", 13); 
	
	strategy.parameters:addGroup("1. Line Calculation");
    strategy.parameters:addInteger("Period1", "Period", "Period", 1); 
	...
	strategy.parameters:addGroup("2. Line Calculation");
    strategy.parameters:addInteger("Period2", "Period", "Period", 13); 
	strategy.parameters:addString("Method2", "Method", "", "MVA");
...
function CreateTradingParameters()
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "ADS");
...
function Prepare( nameOnly)
	...
	assert(core.indicators:findIndicator("AVERAGES DI") ~= nil, "Please, download and install AVERAGES DI.LUA indicator");          
    ...
    Indicator1 = core.indicators:create("AVERAGES DI", Source.close,  instance.parameters.Method,   instance.parameters.Period );
    Indicator2 = core.indicators:create("AVERAGES", Indicator1.DATA,  instance.parameters.Method1,   instance.parameters.Period1 ); 
	Indicator3 = core.indicators:create("AVERAGES", Indicator1.DATA,  instance.parameters.Method2,   instance.parameters.Period2 );
	
	first=math.max(Indicator2.DATA:first(),Indicator3.DATA:first()); 
...
function EntryFunction( id, now,period)
    ...
    if   Indicator2.DATA[period]>  Indicator3.DATA[period]
	and  Indicator2.DATA[period-1]<=  Indicator3.DATA[period-1]
	then	
          if Direction then
                BUY();
            else
		...
		Return=true;
    elseif Indicator2.DATA[period]<  Indicator3.DATA[period]
	and  Indicator2.DATA[period-1]>=  Indicator3.DATA[period-1]
		then
            if Direction then
                SELL();
