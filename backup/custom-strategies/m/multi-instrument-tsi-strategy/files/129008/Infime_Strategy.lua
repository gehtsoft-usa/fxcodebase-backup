...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=68453
...
local STRATEGY_NAME = "Infime Strategy";
...
function CreateParameters()
    strategy.parameters:addGroup("RLW parameters");
    strategy.parameters:addInteger("rlw_N", "Period", "Period", 14, 1, 1000);

    strategy.parameters:addGroup("TSI parameters");
    strategy.parameters:addInteger("Short", "Short Period", "", 7);
    strategy.parameters:addInteger("Long", "Long Period", "", 14);
    
    strategy.parameters:addGroup("Donchian channel of RSI parameters");
	strategy.parameters:addInteger("Period", "MA Period", "Period" , 14, 1, 2000);
...
function CreateIndicators(source)
    rlw = core.indicators:create("RLW", source, instance.parameters.rlw_N);
    tsi = core.indicators:create("TSI", source, instance.parameters.Short, instance.parameters.Long);

    local profile = core.indicators:findIndicator("DONCHIAN CHANNEL OF RSI");
    assert(profile ~= nil, "Please, download and install " .. "DONCHIAN CHANNEL OF RSI" .. ".LUA indicator");
    local indicatorParams = profile:parameters();
    --set parameters here indicatorParams:setBoolean("name", val);
    dcrsi = core.indicators:create("DONCHIAN CHANNEL OF RSI", source, instance.parameters.Period,
        instance.parameters.N, instance.parameters.AC, "yes");
end

function UpdateIndicators()
    rlw:update(core.UpdateLast);
    tsi:update(core.UpdateLast);
    dcrsi:update(core.UpdateLast);
end
...
function CreateCustomActions()
    ...
    exitLongAction.IsPass = function (source, period, periodFromLast, data)
        return tsi.DATA[period] < tsi.DATA[period - 1];
    end
    ...
    enterLongAction.IsPass = function (source, period, periodFromLast, data)
        return rlw.DATA[period] < -80
            and tsi.DATA[period] < 0
            and dcrsi.RSI[period] < dcrsi.DU[period] 
            and dcrsi.RSI[period - 1] == dcrsi.DU[period - 1];
    end
    ...
    enterShortAction.IsPass = function (source, period, periodFromLast, data)
        return rlw.DATA[period] > -20
            and tsi.DATA[period] > 0
            and dcrsi.RSI[period] > dcrsi.DN[period] 
            and dcrsi.RSI[period - 1] == dcrsi.DN[period - 1];
    end
