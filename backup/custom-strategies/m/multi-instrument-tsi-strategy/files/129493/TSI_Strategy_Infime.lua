...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=68453&start=10
...
local STRATEGY_NAME = "TSI Strategy Infime";
...
function createPatams(id)
    strategy.parameters:addString("tf" .. id, "Timeframe " .. id, "", "m5");
    strategy.parameters:setFlag("tf" .. id, core.FLAG_BARPERIODS);
    strategy.parameters:addInteger("TSI_Period_1_" .. id, "TSI Short Period", "", 7);
    strategy.parameters:addInteger("TSI_Period_2_"  .. id, "TSI Long Period", "", 14);

    strategy.parameters:addInteger("ssd_K_" .. id, "Number of periods for %K", "The number of periods for %K.", 10, 2, 1000);
    strategy.parameters:addInteger("ssd_SD_" .. id, "%D slowing periods", "The number of periods for slow %D.", 5, 2, 1000);
    strategy.parameters:addInteger("ssd_D_" .. id, "Number of periods for %D", "The number of periods for %D.", 5, 2, 1000);
    strategy.parameters:addDouble("ssd_buy_level" .. id, "Buy level", "", 50);
    strategy.parameters:addDouble("ssd_sell_level" .. id, "Sell level", "", 50);
end
...
local indis = {};
function createIndi(source, id)
    local indi = {};
    indi.source = trading_logic:SubscribeHistory(source.close:instrument(), instance.parameters:getString("tf" .. id), source:isBid());
    indi.tsi = core.indicators:create("TSI", indi.source, instance.parameters:getInteger("TSI_Period_1" .. id), 
        instance.parameters:getInteger("TSI_Period2" .. id));
    indi.ssd = core.indicators:create("SSD", indi.source, instance.parameters:getInteger("ssd_K_" .. id),
        instance.parameters:getInteger("ssd_SD_" .. id), instance.parameters:getInteger("ssd_D_" .. id));
    indi.ssd_buy_level = instance.parameters:getDouble("ssd_buy_level" .. id);
    indi.ssd_sell_level = instance.parameters:getDouble("ssd_sell_level" .. id);
    return indi;
end

function CreateEntryIndicators(source)
    indis[#indis + 1] = createIndi(source, 1);
    indis[#indis + 1] = createIndi(source, 2);
end
function CreateExitIndicators(source) 
end

function UpdateIndicators()
    for _, indi in pairs(indis) do
        indi.tsi:update(core.UpdateLast);
        indi.ssd:update(core.UpdateLast);
    end
end
...
function CreateCustomActions()
    if not DISABLE_EXIT then
        ...
        exitLongAction.IsPass = function (source, period, periodFromLast, data)
            for _, indi in pairs(indis) do
                if indi.tsi.DATA[NOW] < indi.tsi.DATA[NOW - 1] then
                    return true;
                end
            end
            return false;
    ...
    enterLongAction.IsPass = function (source, period, periodFromLast, data)
        for _, indi in pairs(indis) do
            if indi.ssd.D[NOW] > indi.ssd_buy_level then
                return false;
            end
            if not (indi.tsi.DATA[NOW] > indi.tsi.DATA[NOW - 1] and indi.tsi.DATA[NOW - 1] < indi.tsi.DATA[NOW - 2]) then
                return false;
            end
        end
        return true;
    ...
    enterShortAction.IsPass = function (source, period, periodFromLast, data)
        for _, indi in pairs(indis) do
            if indi.ssd.D[NOW] < indi.ssd_sell_level then
                return false;
            end
            if not (indi.tsi.DATA[NOW] < indi.tsi.DATA[NOW - 1] and indi.tsi.DATA[NOW - 1] > indi.tsi.DATA[NOW - 2]) then
                return false;
