...
function Init() --The strategy profile initialization
    strategy:name("Multi-Instrument TSI Strategy");
	...
	strategy.parameters:addString("Pair1", "Pair", "", "FRA40");
    strategy.parameters:setFlag("Pair1", core.FLAG_INSTRUMENTS);
	
	strategy.parameters:addString("Pair2", "Pair", "", "SPX500");
...
function Prepare( nameOnly)
	...
	Pair1 = instance.parameters.Pair1;
	Pair2 = instance.parameters.Pair2;
    ...
    Source1 = ExtSubscribe(2, Pair1, TF, instance.parameters.Type == "Bid", "bar");
	Source2 = ExtSubscribe(3, Pair2, TF, instance.parameters.Type == "Bid", "bar");
    Indicator1 = core.indicators:create("TSI", Source1[Price], instance.parameters.Short, instance.parameters.Long  );
	Indicator2 = core.indicators:create("TSI", Source2[Price], instance.parameters.Short, instance.parameters.Long  );
...
function EntryFunction( id, now,period)
	...
	if (EntryExecutionType== "Live" and id==1) 
	then
	period1= core.findDate (Source1, TickSource:date(period1), false);
	period2= core.findDate (Source2, TickSource:date(period1), false);
    end
    ...
    if   Indicator1.DATA[period1]>  Indicator2.DATA[period2] 
	and  Indicator1.DATA[period1-1]<=  Indicator2.DATA[period2-1] 
	then	
          if Direction then
                BUY();
            else
		...
		Return=true;
    elseif   Indicator1.DATA[period1]<  Indicator2.DATA[period2] 
	and  Indicator1.DATA[period1-1]>=  Indicator2.DATA[period2-1] 
	then
            if Direction then
                SELL();
