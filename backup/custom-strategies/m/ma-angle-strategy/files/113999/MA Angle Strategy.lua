...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=64964
-- Id: 18781
-- More information about this indicator can be found at:
--http://fxcodebase.com/code/viewtopic.php?f=31&t=64964
...
function Init() --The strategy profile initialization
    strategy:name("MA Angle Strategy")
    ...
    strategy.parameters:addStringAlternative("Method", "DEMA", "DEMA", "DEMA")
    strategy.parameters:addStringAlternative("Method", "TEMA", "TEMA", "TEMA")
    strategy.parameters:addInteger("F", "Period", "Period", 50, 2, 2000)
    strategy.parameters:addInteger("S", "Shift", "Shift", 6, 0, 2000)
    strategy.parameters:addDouble("T", "Treshold", "Treshold", 0.3, 0, 100)
...
function Prepare(nameOnly)
    ...
    local profile = core.indicators:findIndicator("MAANGLE");
    assert(profile ~= nil, "Please, download and install " .. "MAANGLE" .. ".LUA indicator");
    local indicatorParams = profile:parameters();
    indicatorParams:setString("Method", Method);
    indicatorParams:setInteger("F", F);
    indicatorParams:setInteger("S", S);
    indicatorParams:setDouble("T", T);
    Indicator = core.indicators:create("MAANGLE", Source, indicatorParams)
...
function ExitFunction(now, period)
    ...
    if
        Indicator.DATA:colorI(period) == core.rgb(255, 215, 0) and
            Indicator.DATA:colorI(period - 1) ~= core.rgb(255, 215, 0)
     then
        if Direction then
