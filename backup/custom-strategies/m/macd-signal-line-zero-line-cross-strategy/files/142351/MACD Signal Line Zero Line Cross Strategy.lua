...
-- https://fxcodebase.com/code/viewtopic.php?f=31&t=71239
...
function Init() --The strategy profile initialization
    strategy:name("MACD Signal Line Zero Line Cross Strategy");
    ...
    strategy.parameters:addInteger("Short", "Short Period", "", 12);
    strategy.parameters:addInteger("Long", "Long Period", "", 26);
...
function CreateTradingParameters()
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "MSLZLCS");
...
function Prepare( nameOnly)
    ...
    Indicator = core.indicators:create("MACD", Source.close,   instance.parameters.Short, instance.parameters.Long, instance.parameters.Signal  );
...
function EntryFunction( id, now,period)
    ...
    if   Indicator.SIGNAL[period]> 0 
	and   Indicator.SIGNAL[period-1]<= 0 
	then	
          if Direction then
                BUY();
            else
		...
		Return=true;
    elseif  Indicator.SIGNAL[period]< 0 
	and   Indicator.SIGNAL[period-1]>= 0 
		then
            if Direction then
                SELL();
