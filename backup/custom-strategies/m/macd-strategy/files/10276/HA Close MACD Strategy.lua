...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=4100
...
function Init() --The strategy profile initialization
    ...
    strategy.parameters:addInteger("Delay", "Signal Delay", "", 0, 0, 200)

    strategy.parameters:addBoolean("Filter", "Use Signal Filter", "", false)
    strategy.parameters:addDouble("Min", "Signal Min. Value", "", 0)
    strategy.parameters:addDouble("Max", "Signal Min. Value", "", 100000)

    strategy.parameters:addGroup("Strategy Entry")
    strategy.parameters:addString("Entry", "Entry Condition", "Zero", "ZERO")
    strategy.parameters:addStringAlternative("Entry", "Zero Line Cross", "", "ZERO")
    strategy.parameters:addStringAlternative("Entry", "Signal Line Cross", "", "SIGNAL")
    strategy.parameters:addStringAlternative("Entry", "Both", "", "BOTH")

    strategy.parameters:addGroup("Strategy Exit")
    strategy.parameters:addString("Exit", "Exit Condition", "Zero", "ZERO")
    strategy.parameters:addStringAlternative("Exit", "Zero Line Cross", "", "ZERO")
    strategy.parameters:addStringAlternative("Exit", "Signal Line Cross", "", "SIGNAL")
    strategy.parameters:addStringAlternative("Exit", "Both", "", "BOTH")
...
function Prepare(nameOnly)
    ...
    Delay = instance.parameters.Delay
    ...
    MACD = core.indicators:create("MACD", HA.close, SN, LN, IN)
    MACDDATA = MACD:getStream(0) -- macd
    SIGNALDATA = MACD:getStream(1) -- signal

    first = SIGNALDATA:first() + Delay + 1
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if Exit ~= "ZERO" then
        if
            core.crossesOver(MACDDATA, SIGNALDATA, period) and
                (not Filter or (Filter and math.abs(SIGNALDATA[period]) > Min)) and
                (not Filter or (Filter and math.abs(SIGNALDATA[period]) < Max))
         then
            if AllowTrade then
        ...
        elseif
            core.crossesUnder(MACDDATA, SIGNALDATA, period) and
                (not Filter or (Filter and math.abs(SIGNALDATA[period]) > Min)) and
                (not Filter or (Filter and math.abs(SIGNALDATA[period]) < Max))
         then
            if AllowTrade then
