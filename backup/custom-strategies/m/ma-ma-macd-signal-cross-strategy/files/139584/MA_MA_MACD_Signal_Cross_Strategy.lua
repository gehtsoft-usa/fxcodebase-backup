...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=67341
...
function Init() --The strategy profile initialization
    strategy:name("MA_MA MACD_Signal Cross Strategy")
    ...
    strategy.parameters:addBoolean("Use_MACD", "Use MACD", "", true)
    strategy.parameters:addBoolean("Use_MA", "Use MA", "", true)

    strategy.parameters:addGroup("MACD Calculation")
    strategy.parameters:addInteger("P1", "Fast Period", "", 12)
    strategy.parameters:addInteger("P2", "Slow Period", "", 26)
    strategy.parameters:addInteger("P3", "Signal Period", "", 9)

    strategy.parameters:addGroup("1. MA Calculation")
    strategy.parameters:addInteger("Period1", "MA Period", "", 14)
    ...
    strategy.parameters:addInteger("Period2", "MA Period", "", 34)
    ...
    strategy.parameters:addDouble("min_macd", "Min. MACD value", "", 0.00142)
    strategy.parameters:addDouble("max_macd", "Max. MACD value", "", 0.00183)
...
function Prepare(nameOnly)
    ...
    min_macd = instance.parameters.min_macd;
    max_macd = instance.parameters.max_macd;
    ...
    --Indicator parameters
    Use_MACD = instance.parameters.Use_MACD
    Use_MA = instance.parameters.Use_MA
    ...
    assert(
        core.indicators:findIndicator(instance.parameters.Method1) ~= nil,
        instance.parameters.Method1 .. " indicator must be installed"
    )
    MA1 = core.indicators:create(instance.parameters.Method1, Source.close, instance.parameters.Period1)
    assert(
        core.indicators:findIndicator(instance.parameters.Method2) ~= nil,
        instance.parameters.Method2 .. " indicator must be installed"
    )
    ...
    first = math.max(MA1.DATA:first(), MA2.DATA:first(), MACD.DATA:first())
...
function EntryFunction(now, period)
    ...
    if (not Use_MA and not Use_MACD) then
        return
    end

    -- only buy if we have a fast cross over slow and the price is above the moving averages.
    if
        ((Use_MA and MA1.DATA[period] > MA2.DATA[period] and MA1.DATA[period - 1] <= MA2.DATA[period - 1]) or not Use_MA) and
            ((Use_MA and MACD.MACD[period] > MACD.SIGNAL[period] and MACD.MACD[period - 1] <= MACD.SIGNAL[period - 1] and MACD.MACD[period] < min_macd) or
                not Use_MACD)
     then
        if Direction then
            BUY()
    ...
    elseif
        ((Use_MA and MA1.DATA[period] < MA2.DATA[period] and MA1.DATA[period - 1] >= MA2.DATA[period - 1]) or not Use_MA) and
            ((Use_MA and MACD.MACD[period] < MACD.SIGNAL[period] and MACD.MACD[period - 1] >= MACD.SIGNAL[period - 1] and MACD.MACD[period] > max_macd) or
                not Use_MACD)
     then
        if Direction then
            SELL()
