...
function CreateTradingParameters()
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "MAMAMACDCS");
...
function Prepare( nameOnly)
	...
	MACD = core.indicators:create("MACD", Source.close, instance.parameters.P1, instance.parameters.P2 , instance.parameters.P3  );
...
function EntryFunction( now,period)
    ...
    if   ((Use_MA and MA1.DATA[period]>MA2.DATA[period] and MA1.DATA[period-1]<=MA2.DATA[period-1] )or not Use_MA)
	and  ((Use_MA and MACD.MACD[period]>MACD.SIGNAL[period] and MACD.MACD[period-1]<=MACD.SIGNAL[period-1])or not Use_MACD)
	...
	then	
		...
		Return=true;
    elseif    ((Use_MA and MA1.DATA[period]<MA2.DATA[period] and MA1.DATA[period-1]>=MA2.DATA[period-1] )or not Use_MA)
	and  ((Use_MA and MACD.MACD[period]<MACD.SIGNAL[period] and MACD.MACD[period-1]>=MACD.SIGNAL[period-1])or not Use_MACD)
	then
            if Direction then
                SELL();
