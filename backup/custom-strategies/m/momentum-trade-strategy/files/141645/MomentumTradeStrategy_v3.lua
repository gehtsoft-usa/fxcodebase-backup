...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=2771
...
local STRATEGY_NAME = "Momentum trade strategy";
...
function CreateParameters()
    strategy.parameters:addString("MA_Method", "MA_Method", "", "EMA")
    ...
    strategy.parameters:addInteger("MA_Period", "MA_Period", "", 20)

    strategy.parameters:addInteger("MACD_fast", "MACD_fast", "", 12)
    strategy.parameters:addInteger("MACD_slow", "MACD_slow", "", 26)
    strategy.parameters:addInteger("MACD_signal", "MACD_signal", "", 9)
    ...
    strategy.parameters:addInteger("Open_Distance", "Open_Distance", "", 10)
    strategy.parameters:addInteger("MACD_Cross_Distance", "MACD_Cross_Distance", "", 5)
end

local MA, MACD, Price, Open_Distance, MACD_Cross_Distance;
function CreateEntryIndicators(source)
    Price = instance.parameters.Price;
    MACD_Cross_Distance = instance.parameters.MACD_Cross_Distance;
    Open_Distance = instance.parameters.Open_Distance;
    assert(core.indicators:findIndicator("AVERAGES") ~= nil, "AVERAGES indicator must be installed")
    MA = core.indicators:create("AVERAGES", source, instance.parameters.MA_Method, instance.parameters.MA_Period, false)
    MACD = core.indicators:create("MACD", source, instance.parameters.MACD_fast, instance.parameters.MACD_slow, instance.parameters.MACD_signal)
end
...
function IsBuyFilter(source, period)
    for i = period, 1, -1 do
        if core.crossesUnder(MA.DATA, source[Price][i], i) then
            for ii = i, math.max(0, i - MACD_Cross_Distance), -1 do
                if core.crossesUnder(MACD.HISTOGRAM, 0, ii) then
                    return true;
                end
            end
            return false;
        end
        if MA.DATA[i] > source[Price][i] and MACD.HISTOGRAM[i] < 0. then
            return false;
        end
    end
    return false;
...
function IsSellFilter(source, period)
    for i = period, 1, -1 do
        if core.crossesOver(MA.DATA, source[Price][i], i) then
            for ii = i, math.max(0, i - MACD_Cross_Distance), -1 do
                if core.crossesOver(MACD.HISTOGRAM, 0, ii) then
                    return true;
                end
            end
            return false;
...
function CreateCustomActions()
    ...
    enterLongAction.IsPass = function (source, period, periodFromLast, data)
        return source[Price][period] - MA.DATA[period] >= Open_Distance * source:pipSize() and IsBuyFilter(source, period);
    end
    ...
    enterShortAction.IsPass = function (source, period, periodFromLast, data)
        return MA.DATA[period] - source[Price][period] >= Open_Distance * instance.bid:pipSize() and IsSellFilter(source, period);
    end
    ...
    enterLongAction.IsPass = function (source, period, periodFromLast, data)
        ...
        return core.crossesOver(source[Price], MA.DATA, period) 
            and MACD.HISTOGRAM[period] < 0
            and it:Count() > 0;
    end
    ...
    enterShortAction.IsPass = function (source, period, periodFromLast, data)
        ...
        return core.crossesUnder(source[Price], MA.DATA, period) 
            and MACD.HISTOGRAM[period] > 0
            and it:Count() > 0;
    end
