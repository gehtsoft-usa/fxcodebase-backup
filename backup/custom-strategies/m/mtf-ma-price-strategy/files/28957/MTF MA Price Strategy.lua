...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=15360
...
function Init() --The strategy profile initialization
    strategy:name("MTF MA Price Strategy");
    ...
    TIMEFRAME(1, FRAMES[1], 100 );
	 TIMEFRAME(2, FRAMES[2], 50 );
    ...
    strategy.parameters:addStringAlternative("ExitType", "First Time Frame", "", "First");
	strategy.parameters:addStringAlternative("ExitType", "Second Time Frame", "", "Second");
...
function TIMEFRAME(id, FRAME, MP)
    ...
    strategy.parameters:addInteger(tostring(id*100 + 2), " Period", "", MP);

	strategy.parameters:addString(tostring(id*100 + 4), "Price Source", "", "close");
	...
	strategy.parameters:addString(tostring(id*100 + 3), "Method", "Method" , "MVA");
    strategy.parameters:addStringAlternative(tostring(id*100 + 3), "MVA", "MVA" , "MVA");
    strategy.parameters:addStringAlternative(tostring(id*100 + 3), "EMA", "EMA" , "EMA");
	strategy.parameters:addStringAlternative(tostring(id*100 + 3), "LWMA", "LWMA" , "LWMA");
    strategy.parameters:addStringAlternative(tostring(id*100 + 3), "TMA", "TMA" , "TMA");
    strategy.parameters:addStringAlternative(tostring(id*100 + 3), "SMMA", "SMMA" , "SMMA");
    strategy.parameters:addStringAlternative(tostring(id*100 + 3), "VIDYA", "VIDYA" , "VIDYA");
...
function Prepare(nameOnly)
   ...
   Exit = instance.parameters.Exit;
    ...
    Indicator[i] = core.indicators:create(Parameters[tostring(i*100 + 3)] , Source[i][Parameters[tostring(i*100 + 4)]], Parameters[tostring(i*100 + 2)]   );   
	 end
...
function ExtUpdate(id, source, period)  -- The method called every time when a new bid or ask price appears.
   ...
   -- get only time
	...
	for i = 1, TfNumber , 1 do	
		if Indicator[i].DATA:size() <= Indicator[i].DATA:first() + 4 
		or Source[i].close:size() <= Source[i].close:first() + 4
		then
	...
	return;
		   ...
		   if  Source[1].close[ Source[1].close:size()-2] >   Indicator[1].DATA[Indicator[1].DATA:size()-2]	
		   and Source[1].close[ Source[1].close:size()-3] <   Indicator[1].DATA[Indicator[1].DATA:size()-3]	
		   and Source[2].close[ Source[2].close:size()-2] >   Indicator[2].DATA[Indicator[2].DATA:size()-2]		 
		   then
		  ...
		  elseif Source[1].close[ Source[1].close:size()-2] <   Indicator[1].DATA[Indicator[1].DATA:size()-2]
		  and Source[1].close[ Source[1].close:size()-3] >   Indicator[1].DATA[Indicator[1].DATA:size()-3]
		   and Source[2].close[ Source[2].close:size()-2] <   Indicator[2].DATA[Indicator[2].DATA:size()-2]
		 then
            if Direction then
		...
		if Exit then
         		if (Source[1].close[ Source[1].close:size()-2] <   Indicator[1].DATA[Indicator[1].DATA:size()-2] and ( ExitType == "First"  or   ExitType == "Any")  ) 
				or (Source[2].close[ Source[2].close:size()-2] <   Indicator[2].DATA[Indicator[2].DATA:size()-2] and ( ExitType == "Second"  or   ExitType == "Any")  ) 
				then 			
								if   Direction then
					  ...
					  elseif  (Source[1].close[ Source[1].close:size()-2] >   Indicator[1].DATA[Indicator[1].DATA:size()-2] and ( ExitType == "First"  or   ExitType == "Any")  ) 
				      or (Source[2].close[ Source[2].close:size()-2] >  Indicator[2].DATA[Indicator[2].DATA:size()-2] and ( ExitType == "Second"  or   ExitType == "Any")  ) 
		               then
								   if   Direction then    
										if haveTrades("S") then
