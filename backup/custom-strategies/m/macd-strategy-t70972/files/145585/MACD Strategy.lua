...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=70972
...
function Init() --The strategy profile initialization
	...
	strategy.parameters:addInteger("P2", "Long Period", "", 26);
	strategy.parameters:addInteger("P3", "Signal Perios", "", 9);
	
	strategy.parameters:addBoolean("MS", "MACD/Signal Cross Signal", "", true);   
    strategy.parameters:addBoolean("MZ", "MACD/Zero Cross Signal ", "", true); 
	
    strategy.parameters:addBoolean("MSC", "MACD/Signal Confirmation", "", true);   
    strategy.parameters:addBoolean("MZC", "MACD/Zero Confirmation ", "", true); 
 
     strategy.parameters:addBoolean("Zero", "MACD/Zero Line Cross Only ", "", true); 
...
--	strategy.parameters:addStringAlternative("ExitExecutionType", "Live", "", "Live");
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "MACDS");
...
function Prepare( nameOnly)
    ...
    MSC = instance.parameters.MSC;
	MZC = instance.parameters.MZC;
    ...
    Indicator = core.indicators:create("MACD", Source.close, instance.parameters.P1,instance.parameters.P2,instance.parameters.P3  );	 
...
function EntryFunction( id, now,period)
    ...
    if
	((Indicator.MACD[period] > 0 and Indicator.MACD[period-1] <= 0 and  Zero) or not Zero )
	and 
	(( 
	(MS and Indicator.MACD[period]>Indicator.SIGNAL[period] and Indicator.MACD[period-1]<=Indicator.SIGNAL[period-1])
	and (MZC and Indicator.MACD[period]>0)
	)
	or (
	(MZ and Indicator.MACD[period]>0 and Indicator.MACD[period-1]<=0)
	and (MSC and Indicator.MACD[period]>Indicator.SIGNAL[period])
	))
	then	
          if Direction then
                BUY();
