...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=24271
...
function Init() --The strategy profile initialization
    ...
    strategy.parameters:addDouble("Level", "Level", " Level", 0)
...
function Prepare(nameOnly)
    ...
    Indicator[1] = core.indicators:create("MA DELTA", Source[Price], Period)
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if core.crossesOver(Indicator[1].DATA, -Level, period) then
        if Direction then
            BUY()
        else
            SELL()
        end
    elseif core.crossesUnder(Indicator[1].DATA, Level, period) then
        if Direction then
            SELL()
        else
            BUY()
