...
function Init() --The strategy profile initialization
    strategy:name("MA Cross with the confirmation by the distance between lines")
    strategy:description(
        "The strategy trades based on signal that checks for the Moving Average crosses confirmed by the distance between the MA lines"
    )
    ...
    strategy.parameters:addString(
        ...
        "The methods marked by an asterisk (*) require the appropriate strategys to be loaded.",
        "MVA"
    )
                                    ...
                                    instance.parameters.FMA_N ..
                                        ")," ..
                                            instance.parameters.SMA_M ..
                                                "(" .. instance.parameters.SMA_N .. ")," .. Dist .. "," .. ConfN .. ")"
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if CurrDist ~= nil and PrevDate < Source:date(period) then
        CurrDist = CurrDist + 1
    end

    PrevDate = Source:date(period)
    ...
    if CurrDist ~= nil and CurrDist >= ConfN then
        if iSignal > 0 then
            if (FMA.DATA[period] - SMA.DATA[period]) >= Dist and mAllowedSide ~= "Sell" then
                if Direction then
                    BUY()
                else
                    SELL()
