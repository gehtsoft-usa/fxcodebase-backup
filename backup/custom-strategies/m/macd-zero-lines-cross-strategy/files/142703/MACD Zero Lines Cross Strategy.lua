...
-- https://fxcodebase.com/code/viewtopic.php?f=31&t=71323
...
function CreateTradingParameters()
   ...
   -- strategy.parameters:addStringAlternative("ExitExecutionType", "End of Turn", "", "EndOfTurn");
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "MZS");
...
function EntryFunction( id, now,period)
    ...
    if   Indicator.MACD[period]> 0
	and Indicator.MACD[period-1]<= 0
	then	
          if Direction then
                BUY();
            else
		...
		Return=true;
    elseif  Indicator.MACD[period]< 0
	and Indicator.MACD[period-1]>= 0
	then
            if Direction then
                SELL();
