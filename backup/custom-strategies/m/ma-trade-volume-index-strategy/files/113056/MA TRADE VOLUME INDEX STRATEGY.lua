...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=64824
...
function Init() --The strategy profile initialization
    strategy:name("MA TRADE VOLUME INDEX STRATEGY")
    ...
    strategy.parameters:addInteger("MTV", "Minimum Tick Value", "Minimum Tick Value", 10)

    strategy.parameters:addGroup("Indicator MA Calculation")

    strategy.parameters:addInteger("Period1", "Period", "Period", 50)
    ...
    strategy.parameters:addGroup("Price MA Calculation")

    strategy.parameters:addInteger("Period2", "Period", "Period", 50)
...
function Prepare(nameOnly)
    ...
    assert(
        core.indicators:findIndicator("TRADE VOLUME INDEX") ~= nil,
        "Please, download and install TRADE VOLUME INDEX.LUA indicator"
    )
    ...
    TVI = core.indicators:create("TRADE VOLUME INDEX", Source, MTV)

    assert(core.indicators:findIndicator(Method1) ~= nil, Method1 .. " indicator must be installed");
    MA = core.indicators:create(Method1, TVI.DATA, Period1)
    assert(core.indicators:findIndicator(Method2) ~= nil, Method2 .. " indicator must be installed");
    High = core.indicators:create(Method2, Source.high, Period2)
    Low = core.indicators:create(Method2, Source.low, Period2)

    first = math.max(MA.DATA:first(), High.DATA:first())
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    High:update(core.UpdateLast)
    Low:update(core.UpdateLast)
...
function EntryFunction(now, period)
    ...
    if
        TVI.DATA[period] > MA.DATA[period] and Source.close[period] > Low.DATA[period] and
            Source.close[period - 1] <= Low.DATA[period - 1]
     then
        if Direction then
            BUY()
        else
        ...
        Return = true
    elseif
        TVI.DATA[period] < MA.DATA[period] and Source.close[period] < High.DATA[period] and
            Source.close[period - 1] >= High.DATA[period - 1]
     then
        if Direction then
            SELL()
        else
