...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=64906
...
-- Available @ http://fxcodebase.com/code/viewtopic.php?f=31&t=64906

function Init() --The strategy profile initialization
    strategy:name("MA difference Strategy")
    ...
    strategy.parameters:addGroup("1. Difference Calculation")
    strategy.parameters:addInteger("length11", "1. MA Period", "Period", 10, 1, 2000)
    strategy.parameters:addString("Method11", "1. MA Method", "Method", "MVA")
    ...
    strategy.parameters:addInteger("length12", "2. MA Period", "Period", 10, 1, 2000)
    ...
    strategy.parameters:addInteger("length1", "1. Difference Smoothing Period", "Period", 10, 1, 2000)
    ...
    strategy.parameters:addGroup("2. Difference Calculation")
    strategy.parameters:addInteger("length21", "1. MA Period", "Period", 34, 1, 2000)

    strategy.parameters:addString("Method21", "1. MA Method", "Method", "MVA")
    ...
    strategy.parameters:addInteger("length22", "2. MA Period", "Period", 34, 1, 2000)
    ...
    strategy.parameters:addInteger("length2", "2. Difference Smoothing Period", "Period", 10, 1, 2000)
...
local Indicator
local length11, Method11, length12, Method12, length1, Method1
local length21, Method21, length22, Method22, length2, Method2
...
function Prepare(nameOnly)
    ...
    assert(
        core.indicators:findIndicator("MA DIFFERENCE") ~= nil,
        "Please, download and install MA DIFFERENCE.LUA indicator"
    )
    ...
    first = math.max(Indicator.signal1:first(), Indicator.signal2:first())
...
function EntryFunction(now, period)
    ...
    -- buy: signal 2> signal 1 and differece 1< difference 2 and differnce 2 cross over signal 2
    --sell: signal 2< signal 1 and difference 1> difference 2 and difference2 cross under signal 2
    if
        Indicator.signal2[period] > Indicator.signal1[period] and
            Indicator.difference1[period] < Indicator.difference2[period] and
            Indicator.difference2[period] > Indicator.signal2[period] and
            Indicator.difference2[period - 1] <= Indicator.signal2[period - 1]
     then
        if Direction then
            BUY()
        else
