...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=66903
...
function Init() --The strategy profile initialization
    strategy:name("MBFX Timing Strategy");
	...
	strategy.parameters:addBoolean("Neutral", "Use Neutral Color", "", false);
...
function CreateTradingParameters()
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "MBFXS");
...
function Prepare( nameOnly)
    ...
    Indicator = core.indicators:create("MBFX TIMING", Source, Len, Filter,  core.rgb(0, 255, 0),  core.rgb(255, 0, 0) ,  core.rgb(0, 0, 255)  ); 
...
function ExitFunction( now,period)
	 ...
	 return;
		            ...
		            if 
					(not Neutral and Indicator.DATA:colorI(period)   ~=  core.rgb(0, 255, 0) )
					or  
					(Neutral and Indicator.DATA:colorI(period)   ==  core.rgb(255, 0, 0 )   )
					then
					 ...
					 if 
					(not Neutral and Indicator.DATA:colorI(period)   ~=  core.rgb(255,0 , 0) )
					or  
					(Neutral and Indicator.DATA:colorI(period)   ==  core.rgb(0, 255, 0 )   )
					then
...
function EntryFunction( now,period)
	...
	and
    (
	(not Neutral and Indicator.DATA:colorI(period-1)   ~=  core.rgb(0, 255, 0) )
	or  
	(Neutral and Indicator.DATA:colorI(period-1)   ==  core.rgb(255, 0, 0 )   )
	)
	then	
          if Direction then
                BUY();
	...
	and
    (
	(not Neutral and Indicator.DATA:colorI(period-1)   ~=  core.rgb(255, 0, 0) )
	or  
	(Neutral and Indicator.DATA:colorI(period-1)   ==  core.rgb(0, 255, 0 )   )
	)
	then
            if Direction then
                SELL();
