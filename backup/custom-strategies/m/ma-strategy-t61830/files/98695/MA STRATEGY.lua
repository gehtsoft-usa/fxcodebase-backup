...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=61830
...
function Init() --The strategy profile initialization
    ...
    strategy.parameters:addDouble("Delta", "Delta (in Pips)", "", 10)
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if core.crossesOver(Source.close, MA.DATA[period] + Delta * Source:pipSize(), period) then
        if Direction then
            BUY()
        else
            SELL()
    ...
    elseif core.crossesUnder(Source.close, MA.DATA[period] - Delta * Source:pipSize(), period) then
        if Direction then
            SELL()
        else
            BUY()
