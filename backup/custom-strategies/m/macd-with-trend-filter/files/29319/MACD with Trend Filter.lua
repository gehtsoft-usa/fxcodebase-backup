...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=15551
...
function Init() --The strategy profile initialization
    strategy:name("MACD with Trend Filter")
    ...
    strategy.parameters:addInteger("FastPeriod", "Short MA Period", "", 20)
    strategy.parameters:addInteger("SlowPeriod", "Long MA Period", "", 200)
...
--local ExitType;
--local Step, Max;
local FastPeriod, SlowPeriod, Method
local SN, LN, IN
...
function Prepare(nameOnly)
    ...
    name =
        ...
        ", " ..
            Price ..
                ", " .. SN .. ", " .. LN .. ", " .. IN .. ", " .. Method .. ", " .. FastPeriod .. ", " .. SlowPeriod
    ...
    Indicator[1] = core.indicators:create("AVERAGES", Source[Price], Method, FastPeriod)
    Short[1] = Indicator[1]:getStream(0)

    Indicator[2] = core.indicators:create("AVERAGES", Source[Price], Method, SlowPeriod)
    Short[2] = Indicator[2]:getStream(0)

    Indicator[3] = core.indicators:create("MACD", Source[Price], SN, LN, IN)
    Short[3] = Indicator[3]:getStream(0)
    Short[4] = Indicator[3]:getStream(1)

    first = math.max(Short[1]:first(), Short[2]:first(), Short[4]:first())
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if core.crossesOver(Short[3], Short[4], period) and Short[1][period] > Short[2][period] then
        if Direction then
            BUY()
        else
            SELL()
        end
    elseif core.crossesUnder(Short[3], Short[4], period) and Short[1][period] < Short[2][period] then
        if Direction then
            SELL()
        else
            BUY()
    ...
    if core.crossesOver(Short[3], Short[4], period) then
        if Direction then
            if haveTrades("S") then
                exit("S")
                Signal("Close Short")
