...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=68289
...
function Init() --The strategy profile initialization
    strategy:name("MTF RSI with ADX Strategy");
    ...
    strategy.parameters:addGroup("Lower Time Frame");
    strategy.parameters:addString("TF1", "Time frame", "", "H4");
    strategy.parameters:setFlag("TF1", core.FLAG_PERIODS);
	
	strategy.parameters:addInteger("Period_RSI", "1.RSI Period", "", 9);
	
	strategy.parameters:addDouble("OB", "OB Level", "", 70);
	strategy.parameters:addDouble("OS", "OS Level", "", 30);
    ...
    strategy.parameters:addInteger("Period_RSI_1", "1.RSI Period", "", 10);
    strategy.parameters:addInteger("Period_RSI_2", "2.RSI Period", "", 16);
	
	strategy.parameters:addBoolean("Use_ADX", "Use ADX", "", true);   
	strategy.parameters:addInteger("Period_ADX", "ADX Period", "", 14);
	strategy.parameters:addDouble("Level_ADX", "ADX Level", "", 40);
...
function CreateTradingParameters()
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "MTFRSIWADX");
...
local ADX, RSI1, RSI2, Period_RSI_1, Period_RSI_2,Period_ADX,Level_ADX;
local Period_RSI,OB,OS, RSI;
...
function Prepare( nameOnly)
	...
	--Indicator parameters
	Period_RSI_1 = instance.parameters.Period_RSI_1;
	Period_RSI_2 = instance.parameters.Period_RSI_2;
	Period_ADX = instance.parameters.Period_ADX;
	Level_ADX = instance.parameters.Level_ADX;
	
	Use_ADX = instance.parameters.Use_ADX;
	
	Period_RSI= instance.parameters.Period_RSI;
	...
	RSI = core.indicators:create("RSI", Source1.close, Period_RSI); 
    Source2 = ExtSubscribe(3, nil, TF2, instance.parameters.Type == "Bid", "bar");
	RSI1 = core.indicators:create("RSI", Source2.close, Period_RSI_1); 
	RSI2 = core.indicators:create("RSI", Source2.close, Period_RSI_2); 
	ADX = core.indicators:create("ADX", Source2 , Period_ADX); 
...
function EntryFunction( now,period1,period2)
	...
	if not RSI1.DATA:hasData(period2)
	or not RSI2.DATA:hasData(period2)
	or not ADX.DATA:hasData(period2)
	or not RSI.DATA:hasData(period1)
	then
	return;
	end
    ...
    if   RSI1.DATA[period2]> RSI2.DATA[period2]
	and ((( ADX.DATA[period2]< Level_ADX) and Use_ADX ) or not Use_ADX)
	and RSI.DATA[period1]> OS
	and RSI.DATA[period1-1]<= OS
	then	
          if Direction then
                BUY();
            else
		...
		Return=true;
    elseif RSI1.DATA[period2]< RSI2.DATA[period2]
	and ((( ADX.DATA[period2]< Level_ADX) and Use_ADX ) or not Use_ADX)
	and RSI.DATA[period1]< OB
	and RSI.DATA[period1-1]>= OB
	then
            if Direction then
                SELL();
            else
