-- Available @ https://fxcodebase.com/code/viewtopic.php?f=31&t=75408
...
local STRATEGY_NAME = "Multi Day Breakout Strategy";
function CreateParameters() 
    strategy.parameters:addInteger("LookBack", "LookBack", "", 4);
    strategy.parameters:addDouble("offset", "Entry Offset, pips", "", 5);
end

local LookBack, offset;
...
function IsEntryLong(source, period)
    for i = 1, LookBack do
        if source.close[period - i + 1] <= source.close[period - i] then
            return false;
        end
    end
    return true;
    ...
    if IsEntryLong(main_source, entry_period) and last_entry ~= main_source:date(NOW) then
        if AllowTrade then
            ...
            if closed_positions > 0 or not PositionsLimitHit() then
                if trade_direction == "Direct" then
                    CreateEntry(main_source.high[entry_period] + offset * main_source:pipSize(), "B");
                else
                    CreateEntry(main_source.low[entry_period] - offset * main_source:pipSize(), "S");
                end
            end
        end
        if trade_direction == "Direct" then
