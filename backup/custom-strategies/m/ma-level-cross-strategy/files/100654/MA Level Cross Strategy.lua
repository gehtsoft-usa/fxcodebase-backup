...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=62265
...
function Init() --The strategy profile initialization
    strategy:name("MA / Level Cross Strategy")
    ...
    strategy.parameters:addDouble("Level", "Level", "", 1.3)
...
function Prepare(nameOnly)
    ...
    Indicator = core.indicators:create("AVERAGES", Source.close, Method, Period)
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if Indicator.DATA[period] > Level and Indicator.DATA[period - 1] <= Level then
        if Direction then
            BUY()
        else
            SELL()
    ...
    elseif Indicator.DATA[period] < Level and Indicator.DATA[period - 1] >= Level then
        if Direction then
            SELL()
        else
            BUY()
