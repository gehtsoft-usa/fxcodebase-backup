...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=3656
...
function Init() --The strategy profile initialization
    strategy:name("MA trend strategy")
    strategy:description("MA trend strategy")
    strategy:setTag(
    ...
    strategy.parameters:addGroup("MA trend indicator parameters")
    ...
    strategy.parameters:addInteger("Start_Period", "Start period", "", 10)
    strategy.parameters:addInteger("Step_Period", "Step period", "", 10)
    strategy.parameters:addInteger("Count_MA", "Count of MAs", "", 50)
    ...
    strategy.parameters:addDouble("UP_Level", "UP_Level", "UP_Level", 0.7)
    strategy.parameters:addDouble("DN_Level", "DN_Level", "DN_Level", -0.7)
...
function Prepare(nameOnly)
                        ...
                        "," ..
                            "MA trend(" ..
                                instance.parameters.Method ..
                                    ", " ..
                                        instance.parameters.Start_Period ..
                                            ", " ..
                                                instance.parameters.Step_Period ..
                                                    ", " .. instance.parameters.Count_MA .. "))"
    instance:name(name)
    if nameOnly then
        return
    end
    ...
    assert(core.indicators:findIndicator("MA_TREND") ~= nil, "MA_TREND indicator must be installed")
    if Price == "close" then
        MT =
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    MT:update(core.UpdateLast)

    -- Check that we have enough data
    if (MT.DATA:first() > (period - 1)) then
        return
    end
    ...
    if MT.LineBuff[period - 1] < instance.parameters.UP_Level and MT.LineBuff[period] >= instance.parameters.UP_Level then
    ...
    if MT.LineBuff[period - 1] > instance.parameters.DN_Level and MT.LineBuff[period] <= instance.parameters.DN_Level then
        if instance.parameters.TypeSignal == "direct" then
            MustS = true
        else
            MustB = true
