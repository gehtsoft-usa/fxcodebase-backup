...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=64241
...
function Init() --The strategy profile initialization
    strategy:name("MeanReversion Indicator Strategy");
    ...
    strategy.parameters:addInteger("Length", "Length", "", 20);
    strategy.parameters:addInteger("MA_Length", "MA Length", "", 50);
...
function CreateTradingParameters()
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "MRIS");
...
local Indicator,Length, MA_Length;
function Prepare( nameOnly)
    ...
    PrepareTrading();
	 ...
	 assert(core.indicators:findIndicator("MEANREVERSION INDICATOR") ~= nil, "Please, download and install MEANREVERSION INDICATOR.LUA indicator");          
    ...
    Source = ExtSubscribe(2, nil, TF, instance.parameters.Type == "Bid", "bar");
     Indicator = core.indicators:create("MEANREVERSION INDICATOR", Source,  Length, MA_Length  );
	
	first=Indicator.MA:first(); 
...
function ExtUpdate(id, source, period)  -- The method called every time when a new bid or ask price appears.
   ...
   -- get only time
	...
	if not Indicator.MA:hasData(period)	
	then
        return;
    end
...
function EntryFunction(period,now)
	...
	--buy;DM>DN AND DM> MA AND PRICE CROSSESABOVE DM
-- SELL: DM<DU AND DM< MA AND PRICE CROSSES BELOW DM
    ...
    if   Indicator.DM[period]> Indicator.DN[period]
	and Indicator.DM[period]> Indicator.MA[period]
	and Source.close[period]> Indicator.DM[period]
	and Source.close[period-1]<=Indicator.DM[period-1]
	then	
          if Direction then
                BUY();
            else
		...
		Return=true;
    elseif   Indicator.DM[period]< Indicator.DU[period]
	and Indicator.DM[period]< Indicator.MA[period]
	and Source.close[period]< Indicator.DM[period]
	and Source.close[period-1]>=Indicator.DM[period-1]
		then
