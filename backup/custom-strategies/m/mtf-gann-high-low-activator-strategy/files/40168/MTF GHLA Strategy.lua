...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=23334
...
local FRAMES={"m5", "m15", "m30" , "H1" }
local TfNumber =4;

function Init() --The strategy profile initialization
    strategy:name("MTF Gann High Low Activator Strategy");
...
function TIMEFRAME(id, FRAME)
	...
	strategy.parameters:addInteger("Period"..id, "Period", "", 10);
...
function Prepare(nameOnly)
    ...
    Indicator[i] = core.indicators:create("GHLA", Source[i],Period[i]  );
    GHLA[i] = Indicator[i]:getStream(0);
...
function ExtUpdate(id, source, period)  -- The method called every time when a new bid or ask price appears.
   ...
   -- get only time
	...
	for i = 1, TfNumber , 1 do	
		if GHLA[i]:size() <= GHLA[i]:first() + 2  then
		   FLAG= false;
		end
	end
	...
	return;
		   ...
		   if core.crossesUnder(GHLA[1], Source[1].close, GHLA[1]:size()-2 ,Source[1].close:size()-2)  		
		   and  (GHLA[2][ GHLA[2]:size()-2] < Source[2].close[Source[2].close:size()-2] or not  ON[2]) 
		   and  (GHLA[3][ GHLA[3]:size()-2] < Source[3].close[Source[3].close:size()-2] or not  ON[3]) 
		   and  (GHLA[4][ GHLA[4]:size()-2] < Source[4].close[Source[4].close:size()-2] or not  ON[4]) 
		   then
		   ...
		   elseif core.crossesOver(GHLA[1], Source[1].close, GHLA[1]:size()-2 ,Source[1].close:size()-2)  		
		   and  (GHLA[2][ GHLA[2]:size()-2] > Source[2].close[Source[2].close:size()-2] or not  ON[2]) 
		   and  (GHLA[3][ GHLA[3]:size()-2] > Source[3].close[Source[3].close:size()-2] or not  ON[3]) 
		   and  (GHLA[4][ GHLA[4]:size()-2] > Source[4].close[Source[4].close:size()-2] or not  ON[4]) 
		 then
