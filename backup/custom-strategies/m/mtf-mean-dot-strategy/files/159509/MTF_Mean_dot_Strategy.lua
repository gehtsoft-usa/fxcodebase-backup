-- Available @ https://fxcodebase.com/code/viewtopic.php?f=31&t=76009
...
local STRATEGY_NAME = "MTF Mean Dot Strategy";
function CreateParameters() 
    strategy.parameters:addString("tf1", "Timeframe #1", "", "m1");
    ...
    strategy.parameters:addString("tf3", "Timeframe #3", "", "m5");
    strategy.parameters:setFlag("tf3", core.FLAG_PERIODS);
    strategy.parameters:addString("tf4", "Timeframe #3", "", "m5");
    strategy.parameters:setFlag("tf4", core.FLAG_PERIODS);
end

local indi_1, indi_2, indi_3, indi_4;
function CreateEntryIndicators(source)
    local profile = core.indicators:findIndicator("MEAN_DOT");
    assert(profile ~= nil, "Please, download and install " .. "MEAN_DOT" .. ".LUA indicator");
    
    tf1 = ExtSubscribe(2, nil, instance.parameters.tf1, instance.parameters.type, "bar")
    indi_1 = core.indicators:create("MEAN_DOT", tf1)
    tf2 = ExtSubscribe(3, nil, instance.parameters.tf2, instance.parameters.type, "bar")
    indi_2 = core.indicators:create("MEAN_DOT", tf2)
    tf3 = ExtSubscribe(4, nil, instance.parameters.tf3, instance.parameters.type, "bar")
    indi_3 = core.indicators:create("MEAN_DOT", tf3)
    tf4 = ExtSubscribe(5, nil, instance.parameters.tf4, instance.parameters.type, "bar")
    indi_4 = core.indicators:create("MEAN_DOT", tf4)
end

function UpdateIndicators()
    indi_1:update(core.UpdateLast);
    indi_2:update(core.UpdateLast);
    indi_3:update(core.UpdateLast);
    indi_4:update(core.UpdateLast);
end
...
function IsEntryLong(source, period)
    return core.crossesOver(source, indi_1.DATA, NOW)
        and indi_2.DATA[NOW] > source[NOW]
        and indi_3.DATA[NOW] > source[NOW]
        and indi_4.DATA[NOW] > source[NOW];
end
function IsEntryShort(source, period)
    return core.crossesUnder(source, indi_1.DATA, NOW)
        and indi_2.DATA[NOW] < source[NOW]
        and indi_3.DATA[NOW] < source[NOW]
        and indi_4.DATA[NOW] < source[NOW];
end
