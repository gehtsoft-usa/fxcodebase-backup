...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=64304
...
function Init() --The strategy profile initialization
    strategy:name("Median NR SuperTrend");
	...
	strategy.parameters:addGroup("MA");
	
	strategy.parameters:addInteger("Period", "Period", "", 1);
	strategy.parameters:addString("Price", "Price Source", "", "median");
    ...
    strategy.parameters:addGroup("1. Super Trend"); 
    strategy.parameters:addInteger("Period1", "Period", "", 1);
    strategy.parameters:addDouble("Multiplier1", "Multiplier", "", 0.2);
	
	strategy.parameters:addGroup("2. Super Trend"); 
    strategy.parameters:addInteger("Period2", "Period", "", 1);
    strategy.parameters:addDouble("Multiplier2", "Multiplier", "", 0.9);
...
function CreateTradingParameters()
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "MNRSTS");
...
local Indicator1, Period1, Multiplier1;
local Indicator2, Period2, Multiplier2;
local MA, Period, Price, Method; 
function Prepare( nameOnly)
   CustomID = instance.parameters.CustomID;
    ...
    Indicator1 = core.indicators:create("NR_SUPER_TREND", Source, Period1, Multiplier1  );
	Indicator2 = core.indicators:create("NR_SUPER_TREND", Source, Period2, Multiplier2  ); 
    assert(core.indicators:findIndicator(Method) ~= nil, Method .. " indicator must be installed");
	MA = core.indicators:create(Method, Source[Price], Period ); 
	first=math.max(Indicator1.DATA:first(),Indicator2.DATA:first(), MA.DATA:first()); 
...
function ExtUpdate(id, source, period)  -- The method called every time when a new bid or ask price appears.
	...
	if (EntryExecutionType== "Live" 	or ExitExecutionType== "Live" )
    then	
...
function ExitFunction(period, now)
    ...
    if not InRange(now, OpenTime, CloseTime)
	 ...
	 return;
	                ...
	                and  MA.DATA[period]  > Indicator2.DATA[period] )
					or 
					 (Indicator1.DATA[period] > Indicator2.DATA[period]
	                and  MA.DATA[period]  < Indicator2.DATA[period] )
					then
...
function EntryFunction(period,now)
	...
	and
	(
	 Indicator2.DATA[period-1]< Indicator2.DATA[period-1]
	 or   MA.DATA[period-1]  < Indicator2.DATA[period-1]
	)
	then	
          if Direction then
                BUY();
	...
	and
	(
	 Indicator1.DATA[period-1]> Indicator2.DATA[period-1]
	 or   MA.DATA[period-1]  > Indicator2.DATA[period-1]
	)
	then
            if Direction then
