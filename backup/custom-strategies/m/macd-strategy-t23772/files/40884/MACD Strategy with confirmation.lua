...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=23772
...
function Init() --The strategy profile initialization
    strategy:name("MACD Strategy confirmation")
    ...
    strategy.parameters:addBoolean("Use", "Use MA Filter", "", true)
    strategy.parameters:addInteger("Period", "MA Period", "", 10, 2, 1000)
    strategy.parameters:addDouble("Zone", "Validation Zone width (in Pips)", "", 10, 2, 1000)
...
function Prepare(nameOnly)
    ...
    name = name .. ", " .. SN .. ", " .. LN .. ", " .. IN .. ", " .. Period .. ", " .. Method .. ", " .. Zone
    ...
    first = math.max(Short[2]:first(), MA.DATA:first())
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if
        core.crossesOver(Short[1], Short[2], period) and
            ((Source.close[period] < MA.DATA[period] + Zone * source:pipSize()) or not Use)
     then
        if Direction then
            BUY()
        else
    ...
    elseif
        core.crossesUnder(Short[1], Short[2], period) and
            ((Source.close[period] > MA.DATA[period] - Zone * source:pipSize()) or not Use)
     then
        if Direction then
            SELL()
