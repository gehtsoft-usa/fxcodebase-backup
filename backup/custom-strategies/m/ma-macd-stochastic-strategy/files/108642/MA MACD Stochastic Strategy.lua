...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=63989
...
function Init() --The strategy profile initialization
    strategy:name("MA MACD Stochastic Strategy")
    ...
    strategy.parameters:addInteger("Period1", "MA Period", "Period", 7)
    ...
    strategy.parameters:addInteger("Period2", "MA Period", "Period", 7)
    ...
    strategy.parameters:addInteger("Period3", "MA Period", "Period", 14)
    ...
    strategy.parameters:addInteger("SF", "Short MA Period", "Period", 12)
    strategy.parameters:addInteger("LF", "Long MA Period", "Period", 26)
    ...
    strategy.parameters:addString("MVAT_K", "Smoothing method for %K", "", "MVA")
    strategy.parameters:addStringAlternative("MVAT_K", "MVA", "", "MVA")
    strategy.parameters:addStringAlternative("MVAT_K", "EMA", "", "EMA")
    strategy.parameters:addStringAlternative("MVAT_K", "MetaTrader", "", "FS")

    strategy.parameters:addString("MVAT_D", "Smoothing method for %D", "", "MVA")
...
local CustomID
local PositionCap
local Stochastic, MACD, SF, LF, OB, OS
...
function Prepare(nameOnly)
    ...
    MACD = core.indicators:create("MACD", Source.close, SF, LF)
    Stochastic = core.indicators:create("STOCHASTIC", Source, K, SD, D, MVAT_K, MVAT_D)

    first = math.max(MA1.DATA:first(), MA2.DATA:first(), MA3.DATA:first(), MACD.DATA:first(), Stochastic.DATA:first())
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if
        not MA1.DATA:hasData(period) or not MA2.DATA:hasData(period) or not MA3.DATA:hasData(period) or
            not MACD.DATA:hasData(period) or
            not Stochastic.DATA:hasData(period)
     then
        return
    end
    ...
    --[[
		...
		for buy EMA2 crosses above EMA3.
		MACD > 0.
		stochastic >= 80

		for sell EMA1 crosses below EMA3
		MACD < 0
		stochastic <= 20
    ...
    if
        MA2.DATA[period] > MA3.DATA[period] and MA2.DATA[period - 1] <= MA3.DATA[period - 1] and MACD.DATA[period] > 0 and
            Stochastic.DATA[period] >= OB
     then
        if Direction then
            BUY()
        else
    ...
    elseif
        MA1.DATA[period] < MA3.DATA[period] and MA1.DATA[period - 1] >= MA3.DATA[period - 1] and MACD.DATA[period] < 0 and
            Stochastic.DATA[period] <= OS
     then
        if Direction then
            SELL()
        else
