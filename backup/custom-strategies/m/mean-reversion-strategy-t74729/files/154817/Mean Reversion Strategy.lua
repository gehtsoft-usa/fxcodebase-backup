...
function Init() --The strategy profile initialization
    ...
    strategy.parameters:addInteger("Period1", "Period", "", 25, 1, 2000);
    strategy.parameters:addDouble("Multiplier", "Multiplier", "", 2.25);
...
function Prepare( nameOnly)
	...
	assert(core.indicators:findIndicator("MEAN REVERSION") ~= nil, "Please, download and install MEAN REVERSION.LUA indicator");          
    ...
    Indicator = core.indicators:create("MEAN REVERSION", Source , instance.parameters.Period1, instance.parameters.Multiplier ); 
...
function EntryFunction( id, now,period)
    ...
    if   Source.close[period]< Indicator.Low[period]
	and  Indicator.IBS[period-1]<=  0.6
	then	
          if Direction then
                BUY();
            else
		...
		Return=true;
    elseif   Source.close[period]>Indicator.High[period]
	and  Indicator.IBS[period-1]>=  0.4
	then
            if Direction then
                SELL();
            else
