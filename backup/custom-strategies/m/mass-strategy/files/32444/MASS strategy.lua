...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=17918
...
function Init() --The strategy profile initialization
    ...
    strategy.parameters:addInteger("SN", "Short EMA", "(SN)No Description", 12, 2, 1000)
    strategy.parameters:addInteger("LN", "Long EMA", "(LN)No Description", 26, 2, 1000)
    strategy.parameters:addInteger("IN", "Signal Line", "(IN)No Description", 9, 2, 1000)

    strategy.parameters:addGroup("Absolute Strength Calculation")
    strategy.parameters:addString("Method1", "Method1", "", "RSI")
    strategy.parameters:addStringAlternative("Method1", "RSI", "", "RSI")
    strategy.parameters:addStringAlternative("Method1", "Stoch", "", "Stoch")
    strategy.parameters:addStringAlternative("Method1", "ADX", "", "ADX")
    ...
    strategy.parameters:addInteger("Signal", "Period for Signal", "", 3)
    strategy.parameters:addInteger("Smoothing", "Period for Smoothing", "", 3)

    strategy.parameters:addDouble("OverBought", "OverBought", "", 0)
    strategy.parameters:addDouble("OverSold", "OverSold", "", 0)
...
local Smoothing, ISignal, Length
local MVAT_D, MVAT_K
local K, SD, D
...
function Prepare(nameOnly)
    ...
    ISignal = instance.parameters.Signal
    ...
    assert(core.indicators:findIndicator("AS") ~= nil, "Please, download and install AS.LUA indicator")
    ...
    Indicator[1] = core.indicators:create("MACD", Source[Price], SN, LN, IN)
    Short["MACD"] = Indicator[1]:getStream(0)
    ...
    Short["BU"] = Indicator[2]:getStream(0)
    Short["BUS"] = Indicator[2]:getStream(1)
    Short["BE"] = Indicator[2]:getStream(2)
    Short["BES"] = Indicator[2]:getStream(3)

    Indicator[3] = core.indicators:create("STOCHASTIC", Source, K, SD, D, MVAT_K, MVAT_D)
    Short["K"] = Indicator[3]:getStream(0)

    first = math.max(Short["MACD"]:first(), Short["BUS"]:first(), Short["K"]:first())
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if
        Short["MACD"][period] > 0 and Short["BU"][period] > Short["BUS"][period] and
            Short["BU"][period] > Short["BE"][period] and
            core.crossesOver(Short["K"], OS, period)
     then
        if Direction then
            BUY()
        else
    ...
    elseif
        Short["MACD"][period] < 0 and Short["BE"][period] < Short["BES"][period] and
            Short["BE"][period] < Short["BU"][period] and
            core.crossesUnder(Short["K"], OB, period)
     then
        if Direction then
            SELL()
        else
