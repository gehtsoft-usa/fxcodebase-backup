...
function Init()
	...
	strategy.parameters:addBoolean("ADX_ON", "Use ADX Filter", "", false);
	 strategy.parameters:addInteger("ADX_PERIOD", "ADX Period ", "", 14, 2, 1000);
	 strategy.parameters:addInteger("ADX_Level", "ADX Level ", "", 25, 0, 100);
     strategy.parameters:addBoolean("Weak", "Close if Trend is Weak", "", false);
...
function Prepare(onlyName)
	...
	ADX_ON = instance.parameters.ADX_ON;
...
function Update()
    if not(init) then
		...
		ADX= core.indicators:create("ADX", SRC, ADX_PERIOD);
	...
	if p <= ADX.DATA:first()  then
        return ;
    end
	
	
	
			if   core.crossesOver( ADX.DATA,ADX_Level, p)  then
			ADX_Flag= true;
			elseif core.crossesUnder( ADX.DATA,ADX_Level, p)  then
			ADX_Flag= false;
			end
		

	
	if not ADX_Flag  and Weak then
			if CANTRADE then
			 close("S"); 
			  close("B"); 	
			 end 	
    ...
    if core.crossesOver(FMADATA, SMADATA, p - FMASHIFT, p - SMASHIFT) and (ALLOWEDSIDE == "Both" or ALLOWEDSIDE == "Buy") and ADX_Flag then
    ...
    elseif core.crossesUnder(FMADATA, SMADATA, p - FMASHIFT, p - SMASHIFT) and (ALLOWEDSIDE == "Both" or ALLOWEDSIDE == "Sell") and ADX_Flag then
