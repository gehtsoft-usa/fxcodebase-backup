...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=11906
...
function Init() --The strategy profile initialization
    strategy:name("MA High/Low strategy")
    strategy:description("MA High/Low strategy")
...
function Prepare(nameOnly)
    ...
    MA_H =
        core.indicators:create("AVERAGES", Source.high, instance.parameters.Method, instance.parameters.Period, false)
    MA_L = core.indicators:create("AVERAGES", Source.low, instance.parameters.Method, instance.parameters.Period, false)
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    if id == 2 then
        MA_H:update(core.UpdateLast)
        MA_L:update(core.UpdateLast)
        if (MA_H.DATA:first() > (period - 1)) then
            LastH = nil
            LastL = nil
            return
        end
        if (MA_L.DATA:first() > (period - 1)) then
            LastH = nil
            LastL = nil
            return
        end
        if source.close[period] > MA_L.DATA[period] and source.close[period] < MA_H.DATA[period] then
            LastH = MA_H.DATA[period]
            LastL = MA_L.DATA[period]
        end
        return
    end
    ...
    if LastH ~= nil and source[period] > LastH then
        LastH = nil
        LastL = nil
    ...
    if LastL ~= nil and source[period] < LastL then
        LastH = nil
        LastL = nil
        if instance.parameters.TypeSignal == "direct" then
            MustOpenS = true
