...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=63998
...
function Init() --The strategy profile initialization
    strategy:name("MA STOCH_MACD STATEGY")
    ...
    strategy.parameters:addString("Price", "MA Source", "", "close")
    ...
    strategy.parameters:addInteger("Period", "MA Period", "", 200)
    ...
    strategy.parameters:addInteger("P1", "Fast Line", "", 12)
    strategy.parameters:addInteger("P2", "Slow Line", "", 26)
    strategy.parameters:addInteger("P3", "Signal Line", "", 9)
...
local first
local MA, Price, Method, Period
local Stochastic, K, SD, D, MVAT_K, MVAT_D
local MACD, P1, P2, P3, BL, SL
local Direction
local CustomID
local PositionCap
...
function Prepare(nameOnly)
    ...
    MACD = core.indicators:create("MACD", Stochastic.D, P1, P2, P3)

    first = math.max(MA.DATA:first(), Stochastic.D:first(), MACD.SIGNAL:first())
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if not MA.DATA:hasData(period) or not Stochastic.D:hasData(period) or not MACD.SIGNAL:hasData(period) then
        return
    end

    --  MACD > SIGNAL AND SIGNAL < BUY LEVEL {AND}
    if
        Source.close[period] > MA.DATA[period] and Source.close[period - 1] <= MA.DATA[period - 1] and
            Stochastic.K[period] > Stochastic.D[period] and
            Stochastic.K[period - 1] <= Stochastic.D[period - 1] and
            MACD.MACD[period] > MACD.SIGNAL[period] and
            MACD.SIGNAL[period] < BL
     then
        if Direction then
            BUY()
        else
