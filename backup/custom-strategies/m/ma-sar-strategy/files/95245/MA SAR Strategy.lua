...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=61008
...
function Init() --The strategy profile initialization
    ...
    strategy.parameters:addInteger("Period1", "Short Periods", "", 5, 2, 1000)
    ...
    strategy.parameters:addInteger("Period2", "Long Periods", "", 12, 2, 1000)
...
function Prepare(nameOnly)
    ...
    first = math.max(MA1.DATA:first(), MA2.DATA:first(), SAR:getStream(0):first(), SAR:getStream(1):first()) + 1
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if Short[2]:hasData(period) and haveTrades("S") then
        exitSpecific("S")
        Signal("SAR Close Short")
    elseif Short[1]:hasData(period) and haveTrades("B") then
        exitSpecific("B")
        Signal("SAR Close Long")
    end

    if SAR.DN[period] < source.close[period] and not Short[1]:hasData(period) and MA1.DATA[period] > MA2.DATA[period] then
        BUY()
        ONE = Source:serial(period)
    elseif
        SAR.UP[period] > source.close[period] and not Short[2]:hasData(period) and MA1.DATA[period] < MA2.DATA[period]
     then
        SELL()
        ONE = Source:serial(period)
    end
