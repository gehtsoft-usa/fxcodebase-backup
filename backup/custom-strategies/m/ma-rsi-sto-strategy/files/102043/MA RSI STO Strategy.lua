...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=62595
...
function Init() --The strategy profile initialization
    strategy:name("MA RSI STO Strategy")
    ...
    strategy.parameters:addStringAlternative("KS", "Fast Smoothed", "", "FS")
...
local first
local SlowN, FastN, FastMethod, SlowMethod, RSIN, STOK, STOD, STOSD, KS, DS
...
function Prepare(nameOnly)
    ...
    FastMA = core.indicators:create(FastMethod, Source.close, FastN)
    assert(core.indicators:findIndicator(SlowMethod) ~= nil, SlowMethod .. " indicator must be installed");
    SlowMA = core.indicators:create(SlowMethod, Source.close, SlowN)

    RSI = core.indicators:create("RSI", Source.close, RSIN)
    STO = core.indicators:create("STOCHASTIC", Source, STOK, STOSD, STOD, KS, DS)
    MACD = core.indicators:create("MACD", Source.close, P1, P2, P3)

    first =
        math.max(FastMA.DATA:first(), SlowMA.DATA:first(), RSI.DATA:first(), STO.D:first(), MACD.HISTOGRAM:first()) + 1
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if core.crossesOver(MACD.HISTOGRAM, 0, period) then
        FastFlag = "Buy"
        reset = 1
    end

    if core.crossesUnder(MACD.HISTOGRAM, 0, period) then
        FastFlag = "Sell"
        reset = 1
    end
