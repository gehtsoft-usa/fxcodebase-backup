...
-- https://fxcodebase.com/code/viewtopic.php?f=31&t=72412
...
function Init() --The strategy profile initialization
    strategy:name("Multiframe RSI of RSI Strategy");
    ...
    strategy.parameters:addDouble("Period0", "Multiplier Period", "", 2, 1, 2000);
    strategy.parameters:addInteger("Period1", "1. EMA Period", "", 130, 1, 2000);
    strategy.parameters:addInteger("Period2", "2. EMA Period", "", 26, 1, 2000);	 
    strategy.parameters:addInteger("Period3", "Deviation Period", "", 22, 1, 2000);
...
function CreateTradingParameters()
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "MRRS");
...
function Prepare( nameOnly)
	...
	assert(core.indicators:findIndicator("MULTIFRAME RSI OF RSI") ~= nil, "Please, download and install MULTIFRAME RSI OF RSI.LUA indicator");          
    ...
    Indicator = core.indicators:create("MULTIFRAME RSI OF RSI", Source.close, instance.parameters.Period0, instance.parameters.Period1,  instance.parameters.Period2, instance.parameters.Period3 ); 
...
function EntryFunction( id, now,period)
    ...
    if   Indicator.Line1[period]>  Indicator.Line2[period]
	and  Indicator.Line1[period-1]<=  Indicator.Line2[period-1]
	then	
          if Direction then
                BUY();
            else
		...
		Return=true;
    elseif Indicator.Line1[period]<  Indicator.Line2[period]
	and  Indicator.Line1[period-1]>=  Indicator.Line2[period-1]
		then
            if Direction then
                SELL();
