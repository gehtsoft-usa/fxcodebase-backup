...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=67322
...
function Init() --The strategy profile initialization
    strategy:name("MTF 3_MA_Cross Strategy");
...
function AddTimeFrame(id, TF)
    ...
    strategy.parameters:addString("TF"..id, id .. ". Time frame", "", TF);
    strategy.parameters:setFlag("TF"..id, core.FLAG_PERIODS);
	
	strategy.parameters:addInteger("Period1"..id, "1. MA Period", "Period" , 7);
	strategy.parameters:addString("Method1"..id, "1. MA Method", "Method" , "EMA");
	...
	strategy.parameters:addInteger("Period2"..id, "2. MA Period", "Period" , 14);
	strategy.parameters:addString("Method2"..id, "2. MA Method", "Method" , "EMA");
	...
	strategy.parameters:addInteger("Period3"..id, "3. MA Period", "Period" , 21);
	strategy.parameters:addString("Method3"..id, "3. MA Method", "Method" , "EMA");
    strategy.parameters:addStringAlternative("Method3"..id, "MVA", "MVA" , "MVA");
    strategy.parameters:addStringAlternative("Method3"..id, "EMA", "EMA" , "EMA");
    strategy.parameters:addStringAlternative("Method3"..id, "LWMA", "LWMA" , "LWMA");
    strategy.parameters:addStringAlternative("Method3"..id, "TMA", "TMA" , "TMA");
    strategy.parameters:addStringAlternative("Method3"..id, "SMMA", "SMMA" , "SMMA");
    strategy.parameters:addStringAlternative("Method3"..id, "KAMA", "KAMA" , "KAMA");
    strategy.parameters:addStringAlternative("Method3"..id, "VIDYA", "VIDYA" , "VIDYA");
    strategy.parameters:addStringAlternative("Method3"..id, "WMA", "WMA" , "WMA");
...
function CreateTradingParameters()
  ...
  --  strategy.parameters:addStringAlternative("ExitExecutionType", "End of Turn", "", "EndOfTurn");
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "MTF3MACS");
...
function Prepare( nameOnly)
	...
	Period3[i]= instance.parameters:getInteger("Period3" .. i);
	
	Method1[i]= instance.parameters:getString("Method1" .. i);
	Method2[i]= instance.parameters:getString("Method2" .. i);
	Method3[i]= instance.parameters:getString("Method3" .. i);
...
function ExtUpdate(id, source, period)  -- The method called every time when a new bid or ask price appears.
    ...
    then	
		    ...
		    period1= core.findDate (Source[1], TickSource:date(period), false);
			period2= core.findDate (Source[2], TickSource:date(period), false);
			period3= core.findDate (Source[3], TickSource:date(period), false);
			period4= core.findDate (Source[4], TickSource:date(period), false);
	else	
			if id ~= 2 then       
			return;
			end
			...
			period1= period;
			period2= core.findDate (Source[2], Source[1]:date(period), false);
			period3= core.findDate (Source[3], Source[1]:date(period), false);
			period4= core.findDate (Source[4], Source[1]:date(period), false);
   ...
   -- get only time
	...
	if 	period1 < Source[1].close:first( Source[1].close:size()-1)
	or period2 < Source[2].close:first( Source[2].close:size()-1)
	or period3 < Source[3].close:first( Source[3].close:size()-1)
	or period4 < Source[4].close:first( Source[4].close:size()-1)
	then
        return;
    end
    ...
    EntryFunction(now,period1, period2, period3, period4);	
	end
...
function EntryFunction( now, period1, period2, period3, period4)


    local Return=false;

    if not Indicator1[1].DATA:hasData(period1)
	or not Indicator2[1].DATA:hasData(period1)
	or not Indicator3[1].DATA:hasData(period1)
	then	
	return
	end
	
	if not Indicator1[2].DATA:hasData(period2)
	or not Indicator2[2].DATA:hasData(period2)
	or not Indicator3[2].DATA:hasData(period2)
	then	
	return
	end
	
	if not Indicator1[3].DATA:hasData(period3)
	or not Indicator2[3].DATA:hasData(period3)
	or not Indicator3[3].DATA:hasData(period3)
	then	
	return
	end
	
	if not Indicator1[4].DATA:hasData(period4)
	or not Indicator2[4].DATA:hasData(period4)
	or not Indicator3[4].DATA:hasData(period4)
	then	
	return
	end
     ...
     if ( LastEntry == Source[1]:serial(period1) )   then
	 return;
	 end	
	  
    -- only buy if we have a fast cross over slow and the price is above the moving averages.
    if  Indicator1[1].DATA[period1]> Indicator2[1].DATA[period1] and Indicator2[1].DATA[period1]> Indicator3[1].DATA[period1]
	and  Indicator1[1].DATA[period1-1]<= Indicator2[1].DATA[period1-1] 
	and Indicator1[2].DATA[period2]> Indicator2[2].DATA[period2] and Indicator2[2].DATA[period2]> Indicator3[2].DATA[period2]
	and Indicator1[3].DATA[period3]> Indicator2[3].DATA[period3] and Indicator2[3].DATA[period3]> Indicator3[3].DATA[period3]
	and Indicator1[4].DATA[period4]> Indicator2[4].DATA[period4] and Indicator2[4].DATA[period4]> Indicator3[4].DATA[period4]
	then	
          if Direction then
                BUY();
            else
		...
		LastEntry= Source[1]:serial(period1);
		Return=true;
    elseif  Indicator1[1].DATA[period1]< Indicator2[1].DATA[period1] and Indicator2[1].DATA[period1]< Indicator3[1].DATA[period1]
	and  Indicator1[1].DATA[period1-1]>= Indicator2[1].DATA[period1-1] 
	and Indicator1[2].DATA[period2]< Indicator2[2].DATA[period2] and Indicator2[2].DATA[period2]<Indicator3[2].DATA[period2]
	and Indicator1[3].DATA[period3]< Indicator2[3].DATA[period3] and Indicator2[3].DATA[period3]< Indicator3[3].DATA[period3]
	and Indicator1[4].DATA[period4]< Indicator2[4].DATA[period4] and Indicator2[4].DATA[period4]< Indicator3[4].DATA[period4]
