...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=63932
...
function Init() --The strategy profile initialization
    ...
    strategy.parameters:addInteger("MA_Period", "MA period", "", 200)
    ...
    strategy.parameters:addInteger("MACD_Short", "MACD Short period", "", 4)
    strategy.parameters:addInteger("MACD_Long", "MACD Short period", "", 5)

    strategy.parameters:addGroup("RSI Calculation")
    strategy.parameters:addInteger("RSI_Period", "RSI period", "", 14)
    strategy.parameters:addDouble("OB_Level", "OB level", "", 70)
    strategy.parameters:addDouble("OS_Level", "OS level", "", 30)
...
function CreateTradingParameters()
    ...
    strategy.parameters:addBoolean("ReInvest", "Re-Invest", "", true)
...
local first
local MA, MA_Method, MA_Period
local OB_Level, OS_Level, RSI, RSI_Period
local MACD, MACD_Short, MACD_Long
local ReInvest
local Direction
local CustomID
local PositionCap
...
function Prepare(nameOnly)
    ...
    ReInvest = instance.parameters.ReInvest
    ...
    MACD_Short = instance.parameters.MACD_Short
    MACD_Long = instance.parameters.MACD_Long
    ...
    MACD = core.indicators:create("MACD", Source.close, MACD_Short, MACD_Long)
    first = math.max(MA.DATA:first(), RSI.DATA:first(), MACD.DATA:first())
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if not MA.DATA:hasData(period) or not RSI.DATA:hasData(period) or not MACD.DATA:hasData(period) then
        return
    end

    -- only buy if we have a fast cross over slow and the price is above the moving averages.
    if Source.close[period] > MA.DATA[period] and MACD.DATA[period] >= 0 and RSI.DATA[period] >= OS_Level then
        if Direction then
            BUY()
        else
            SELL()
    ...
    elseif Source.close[period] < MA.DATA[period] and MACD.DATA[period] <= 0 and RSI.DATA[period] <= OB_Level then
        if Direction then
            SELL()
        else
            BUY()
...
function MarketOrder(BuySell)
    ...
    if ReInvest then
        ...
        if iProfit < 0 then
            ...
            iAmount = (iProfit / BaseSize)
            iAmount = round(iAmount, 0)
...
function Profit()
    ...
    while (not found) and (row ~= nil) do
        ...
        if row.AccountID == Account and row.OfferID == Offer and row.QTXT == CustomID then
            ProfitValue = ProfitValue + row.GrossPL
        end
