...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=65799
...
function Init() --The strategy profile initialization
    strategy:name("MTF ANN Strategy Indicator Strategy");
	...
	strategy.parameters:addDouble("threshold1", "Threshold", "Threshold", 0.0014);
	
	strategy.parameters:addString("TF2", "Time frame", "", "m5");
    strategy.parameters:setFlag("TF2", core.FLAG_PERIODS);
	
	strategy.parameters:addDouble("threshold2", "Threshold", "Threshold", 0.0014);
	
	strategy.parameters:addString("TF3", "Time frame", "", "m15");
    strategy.parameters:setFlag("TF3", core.FLAG_PERIODS);
	
	strategy.parameters:addDouble("threshold3", "Threshold", "Threshold", 0.0014);
...
function CreateTradingParameters()
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "MTFANNSIS");
...
local threshold1,threshold2,threshold3; 
...
function Prepare( nameOnly)
	...
	threshold1= instance.parameters.threshold1;
	threshold2= instance.parameters.threshold2;
	threshold3= instance.parameters.threshold3;
    ...
    PrepareTrading();
	 ...
	 assert(core.indicators:findIndicator("ANN STRATEGY INDICATOR") ~= nil, "Please, download and install ANN STRATEGY INDICATOR.LUA indicator");          
    ...
    Indicator1 = core.indicators:create("ANN STRATEGY INDICATOR", Source,  threshold1, TF1,core.rgb(0, 255, 0),core.rgb(255, 0, 0)   );
	Indicator2 = core.indicators:create("ANN STRATEGY INDICATOR", Source,  threshold2, TF2,core.rgb(0, 255, 0),core.rgb(255, 0, 0)   );
	Indicator3 = core.indicators:create("ANN STRATEGY INDICATOR", Source,  threshold3, TF3,core.rgb(0, 255, 0),core.rgb(255, 0, 0)   );
...
function EntryFunction( now,period)
    ...
    if  Indicator1.DATA:colorI(period) ==  core.rgb(0, 255, 0)
	and  Indicator2.DATA:colorI(period) ==  core.rgb(0, 255, 0)
	and  Indicator3.DATA:colorI(period) ==  core.rgb(0, 255, 0)
	then	
          if Direction then
                BUY();
            else
	...
	and  Indicator2.DATA:colorI(period) ==   core.rgb(255, 0, 0)
	and  Indicator3.DATA:colorI(period) ==  core.rgb(255, 0, 0)
		then
            if Direction then
                SELL();
            else
