...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=69385
...
local STRATEGY_NAME = "MACD Candle Strategy";
function CreateParameters() 
	...
	strategy.parameters:addString("Method", "Candle Method", "Method" , "MACD");
    strategy.parameters:addStringAlternative("Method", "MACD", "MACD" , "MACD");
    strategy.parameters:addStringAlternative("Method", "SIGNAL", "SIGNAL" , "SIGNAL");
    strategy.parameters:addStringAlternative("Method", "HISTOGRAM", "HISTOGRAM" , "HISTOGRAM");
end

local indi;
function CreateEntryIndicators(source)
    local profile = core.indicators:findIndicator("MACD CANDLE");
    assert(profile ~= nil, "Please, download and install " .. "MACD CANDLE" .. ".LUA indicator");
    local indicatorParams = profile:parameters();
    indi = core.indicators:create("MACD CANDLE", source, instance.parameters.SN, instance.parameters.LN, 
        instance.parameters.IN, instance.parameters.Method);
end
...
function IsEntryLong(source, period)
    if not indi.Open:hasData(period - 1) then
        return;
    end
    return indi.Close[period] > 0 and indi.Close[period - 1] <= 0;
end
...
local _show_alert, _sound_file, _recurrent_sound, _email, AllowedSide;
local _ToTime;
