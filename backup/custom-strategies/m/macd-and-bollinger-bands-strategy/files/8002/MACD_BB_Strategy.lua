...
function Prepare(nameOnly)
    ...
    BB = core.indicators:create("BB", Source, instance.parameters.Period, instance.parameters.Deviation, true)
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if MACD.MACD[period - 2] < MACD.SIGNAL[period - 2] and MACD.MACD[period - 1] > MACD.SIGNAL[period - 1] then
        if Source[period - 1] > BB.TL[period - 1] then
    ...
    if MACD.MACD[period - 2] > MACD.SIGNAL[period - 2] and MACD.MACD[period - 1] < MACD.SIGNAL[period - 1] then
        if Source[period - 1] < BB.BL[period - 1] then
            if instance.parameters.TypeSignal == "direct" then
                if Direction ~= -1 then
                    MustS = true
                    Direction = -1
