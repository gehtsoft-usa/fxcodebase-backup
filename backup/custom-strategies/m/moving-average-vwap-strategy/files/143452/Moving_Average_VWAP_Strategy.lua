...
--https://fxcodebase.com/code/viewtopic.php?f=31&t=71473
...
local STRATEGY_NAME = "Moving Average VWAP Strategy";
function CreateParameters() 
    strategy.parameters:addInteger("cumulativePeriod", "Period", "", 5);
end

local indi;
function CreateEntryIndicators(source)
    local profile = core.indicators:findIndicator("MOVING_AVERAGE_VWAP");
    assert(profile ~= nil, "Please, download and install " .. "MOVING AVERAGE VWAP" .. ".LUA indicator");
    indi = core.indicators:create("MOVING_AVERAGE_VWAP", source, instance.parameters.cumulativePeriod)
end
...
function IsEntryLong(source, period)
    ...
    return core.crossesOver(source.close, indi.plot1, period) and source.close[period] > indi.plot2[period];
end
function IsEntryShort(source, period)
    if not indi.DATA:hasData(period - 1) then
        return false;
    end
    return core.crossesUnder(source.close, indi.plot1, period) and source.close[period] > indi.plot2[period];
end
