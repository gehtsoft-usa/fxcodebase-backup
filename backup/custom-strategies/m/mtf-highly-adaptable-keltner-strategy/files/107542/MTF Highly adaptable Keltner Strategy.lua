...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=63744
...
function Init() --The strategy profile initialization
    strategy:name("MTF Highly adaptable Keltner Strategy")
    ...
    strategy.parameters:addString("TF1", "1. Time frame", "", "H1")
    strategy.parameters:setFlag("TF1", core.FLAG_PERIODS)

    strategy.parameters:addString("TF2", "2. Time frame", "", "H2")
    strategy.parameters:setFlag("TF2", core.FLAG_PERIODS)

    strategy.parameters:addInteger("TradeTF", "Trade Time Frame", "", 1)
    strategy.parameters:addIntegerAlternative("TradeTF", "1. Time frame", "", 1)
    strategy.parameters:addIntegerAlternative("TradeTF", "2. Time frame", "", 2)

    strategy.parameters:addGroup("Calculation")
    -- indicator parameters
    strategy.parameters:addInteger("NM", "Number of the periods to smooth the center line", "", 50)
    strategy.parameters:addInteger("NB", "Number of periods to smooth deviation", "", 50)
    strategy.parameters:addDouble("F", "Factor which is used to apply the deviation", "", 1)

    -- source method
    strategy.parameters:addString("SRC", "The center line source", "", "C")
    strategy.parameters:addStringAlternative("SRC", "Close", "", "C")
    strategy.parameters:addStringAlternative("SRC", "Median (H+L)/2", "", "M1")
    strategy.parameters:addStringAlternative("SRC", "Median (H+L+C)/3", "", "M2")

    -- source smoothing method
    strategy.parameters:addString("MS", "The center line smoothing method", "", "MVA")
    strategy.parameters:addStringAlternative("MS", "MVA", "", "MVA")
    strategy.parameters:addStringAlternative("MS", "EMA", "", "EMA")
    strategy.parameters:addStringAlternative("MS", "LWMA", "", "LWMA")
    strategy.parameters:addStringAlternative("MS", "SMMA", "", "SMMA")
    strategy.parameters:addStringAlternative("MS", "Wilders", "", "WMA")

    -- variation method
    strategy.parameters:addString("MV", "Variation Method", "", "AHL")
    strategy.parameters:addStringAlternative("MV", "Smoothed H-L", "", "AHL")
    strategy.parameters:addStringAlternative("MV", "ATR of source", "", "ATR")
    ...
    strategy.parameters:addString("Action" .. 3, "Cental Line Cross Over", "", "NO")
    ...
    strategy.parameters:addString("Action" .. 4, "Cental Line  Cross Under", "", "NO")
    ...
    strategy.parameters:addString("Action" .. 6, "Bottom Cross Under", "", "NO")
...
local first
local Action = {}
local NM, NB, F, SRC, MS, MV
local Indicator1, Indicator2
local Direction
local CustomID
local TF1, TF2, TradeTF
...
function Prepare(nameOnly)
    ...
    TradeTF = instance.parameters.TradeTF
    ...
    Indicator1 = core.indicators:create("KELTNER", Source1, NM, NB, F, SRC, MS, MV)
    Indicator2 = core.indicators:create("KELTNER", Source2, NM, NB, F, SRC, MS, MV)
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if ExecutionType == "Live" then
        ...
        if TradeTF == 1 then
            ...
            if ONE == Source2:serial(period2) then
                return
            end
        end
    ...
    if
        Indicator1.DATA:first() + 2 > (Indicator1.DATA:size() - 1) or
            Indicator2.DATA:first() + 2 > (Indicator2.DATA:size() - 1)
     then
        return
    end

    --if core.crossesOver(Source.close, Indicator:getStream(0), period)
    if
        (Source1.close[Source1.close:size() - 1] > Indicator1:getStream(0)[Indicator1:getStream(0):size() - 1] and
            Source1.close[Source1.close:size() - 1 - 1] <=
                Indicator1:getStream(0)[Indicator1:getStream(0):size() - 1 - 1] and
            Source2.close[Source2.close:size() - 1] > Indicator2:getStream(0)[Indicator2:getStream(0):size() - 1] or
            TradeTF ~= 1) and
            (Source2.close[Source2.close:size() - 1] > Indicator2:getStream(0)[Indicator2:getStream(0):size() - 1] and
                Source2.close[Source2.close:size() - 1 - 1] <=
                    Indicator2:getStream(0)[Indicator2:getStream(0):size() - 1 - 1] and
                Source1.close[Source1.close:size() - 1] > Indicator1:getStream(0)[Indicator1:getStream(0):size() - 1] or
                TradeTF ~= 2)
     then
        ACTION(1, 1, "Over")
    end
    ...
    -- if core.crossesOver(Source.close, Indicator:getStream(1), period)
    if
        (Source1.close[Source1.close:size() - 1] > Indicator1:getStream(1)[Indicator1:getStream(1):size() - 1] and
            Source1.close[Source1.close:size() - 1 - 1] <=
                Indicator1:getStream(1)[Indicator1:getStream(1):size() - 1 - 1] and
            Source2.close[Source2.close:size() - 1] > Indicator2:getStream(1)[Indicator2:getStream(1):size() - 1] or
            TradeTF ~= 1) and
            (Source2.close[Source2.close:size() - 1] > Indicator2:getStream(1)[Indicator2:getStream(1):size() - 1] and
                Source2.close[Source2.close:size() - 1 - 1] <=
                    Indicator2:getStream(1)[Indicator2:getStream(1):size() - 1 - 1] and
                Source1.close[Source1.close:size() - 1] > Indicator1:getStream(1)[Indicator1:getStream(1):size() - 1] or
                TradeTF ~= 2)
     then
        ACTION(3, 2, "Over")
    end
    --if core.crossesUnder(Source.close, Indicator:getStream(1), period)
    if
        (Source1.close[Source1.close:size() - 1] < Indicator1:getStream(1)[Indicator1:getStream(1):size() - 1] and
            Source1.close[Source1.close:size() - 1 - 1] >=
                Indicator1:getStream(1)[Indicator1:getStream(1):size() - 1 - 1] and
    ...
    --if core.crossesOver(Source.close, Indicator:getStream(2), period)
    if
        (Source1.close[Source1.close:size() - 1] > Indicator1:getStream(2)[Indicator1:getStream(2):size() - 1] and
            Source1.close[Source1.close:size() - 1 - 1] <=
                Indicator1:getStream(2)[Indicator1:getStream(2):size() - 1 - 1] and
            Source2.close[Source2.close:size() - 1] <= Indicator2:getStream(2)[Indicator2:getStream(2):size() - 1] or
            TradeTF ~= 1) and
            (Source2.close[Source2.close:size() - 1] > Indicator2:getStream(2)[Indicator2:getStream(2):size() - 1] and
                Source2.close[Source2.close:size() - 1 - 1] <=
                    Indicator2:getStream(2)[Indicator2:getStream(2):size() - 1 - 1] and
                Source1.close[Source1.close:size() - 1] > Indicator1:getStream(2)[Indicator1:getStream(2):size() - 1] or
                TradeTF ~= 2)
     then
        ACTION(5, 3, "Over")
    end
    --if core.crossesUnder(Source.close, Indicator:getStream(2), period)
    if
        (Source1.close[Source1.close:size() - 1] < Indicator1:getStream(2)[Indicator1:getStream(2):size() - 1] and
            Source1.close[Source1.close:size() - 1 - 1] >=
                Indicator1:getStream(2)[Indicator1:getStream(2):size() - 1 - 1] and
...
function ACTION(Flag, Line, Label)
    ...
    elseif Action[Flag] == "BUY" then
        ...
        if TradeTF == 1 then
            ONE = Source1:serial(Source1.close:size() - 1)
        else
            ONE = Source2:serial(Source2.close:size() - 1)
        end
