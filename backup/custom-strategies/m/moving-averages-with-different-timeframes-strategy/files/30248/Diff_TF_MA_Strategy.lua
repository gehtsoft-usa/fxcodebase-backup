...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=16210
...
function Init() --The strategy profile initialization
    strategy:name("Different TF Averages strategy")
    strategy:description("Different TF Averages strategy")
    strategy:setTag("Version", "2")
    strategy:setTag("NonOptimizableParameters", "Email,SendEmail,SoundFile,RecurrentSound,PlaySound, ShowAlert")

    strategy.parameters:addGroup("Parameters")
    strategy.parameters:addString("BS", "Secondary Time frame to calculate MA", "", "D1")
    ...
    strategy.parameters:addString("TF", "Master Time Frame", "", "H1")
...
function Prepare(nameOnly)
    ...
    s1, e1 = core.getcandle(instance.parameters.BS, core.now(), 0, 0)
    assert((e - s) <= (e1 - s1), "The chosen time frame must be bigger than the chart time frame!")
    TF_Coeff = (e1 - s1) / (e - s)
    Period2 = math.floor(instance.parameters.Period * TF_Coeff + 0.5)

    assert(core.indicators:findIndicator("AVERAGES") ~= nil, "Please, download and install AVERAGES.LUA indicator")

    Source = ExtSubscribe(2, nil, instance.parameters.TF, true, "bar")
    Source2 = ExtSubscribe(3, nil, instance.parameters.BS, true, "bar")
    Price = instance.parameters.Price
    if Price == "close" then
        Master_MA = core.indicators:create("AVERAGES", Source.close, instance.parameters.Method, Period2, false)
        Secondary_MA =
    ...
    elseif Price == "open" then
        Master_MA = core.indicators:create("AVERAGES", Source.open, instance.parameters.Method, Period2, false)
        Secondary_MA =
            core.indicators:create(
            "AVERAGES",
            Source2.open,
    ...
    elseif Price == "high" then
        Master_MA = core.indicators:create("AVERAGES", Source.high, instance.parameters.Method, Period2, false)
        Secondary_MA =
            core.indicators:create(
            "AVERAGES",
            Source2.high,
    ...
    elseif Price == "low" then
        Master_MA = core.indicators:create("AVERAGES", Source.low, instance.parameters.Method, Period2, false)
        Secondary_MA =
            core.indicators:create(
            "AVERAGES",
            Source2.low,
    ...
    elseif Price == "median" then
        Master_MA = core.indicators:create("AVERAGES", Source.median, instance.parameters.Method, Period2, false)
        Secondary_MA =
            core.indicators:create(
            "AVERAGES",
            Source2.median,
    ...
    elseif Price == "typical" then
        Master_MA = core.indicators:create("AVERAGES", Source.typical, instance.parameters.Method, Period2, false)
        Secondary_MA =
            core.indicators:create(
            "AVERAGES",
            Source2.typical,
        ...
        Master_MA = core.indicators:create("AVERAGES", Source.weighted, instance.parameters.Method, Period2, false)
        Secondary_MA =
            core.indicators:create(
            "AVERAGES",
            Source2.weighted,
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    Master_MA:update(core.UpdateLast)
    Secondary_MA:update(core.UpdateLast)

    -- Check that we have enough data
    if (Master_MA.DATA:first() > (period - 1)) then
        return
    end
    if (Secondary_MA.DATA:first() > (period - 1)) then
        return
    end

    if id == 2 then
        LastMasterValue = Master_MA.DATA[period]
        return
    end
    ...
    if LastMasterValue ~= nil then
        if Secondary_MA.DATA[period] > LastMasterValue and LastState ~= 1 then
            LastState = 1
