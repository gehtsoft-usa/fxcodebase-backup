...
function Init() --The strategy profile initialization
    strategy:name("Manual Entry Strategy")
    strategy:description("")
    strategy:setTag("Version", "3")
    strategy:setTag("NonOptimizableParameters", "Email,SendEmail,SoundFile,RecurrentSound,PlaySound, ShowAlert")

    strategy.parameters:addGroup("Price")
    strategy.parameters:addString("Type", "Price Type (main)", "", "Bid")
    strategy.parameters:addStringAlternative("Type", "Bid", "", "Bid")
    strategy.parameters:addStringAlternative("Type", "Ask", "", "Ask")
    strategy.parameters:addBoolean("CTS_type","Price Type (CTS)", "", true);
    strategy.parameters:setFlag("CTS_type", core.FLAG_BIDASK);
    ...
    strategy.parameters:addDouble("Entry", "Entry Level", "", 0, 0, 100000)

    strategy.parameters:addInteger("Bar", "Min Bar Size in Pips", "", 0, 0, 1000000)
    strategy.parameters:addInteger("Crossover", "Min Entry Level Crossover in Pips", "", 0, 0, 1000000)
...
function OnClosedTrade(closed_trade)
    if tables_monitor.closing_order_types[closed_trade.CloseOrderID] == "S" and closed_trade.PL < 0 then
        core.host:execute("stop");
    end
end

function OnCloseOrder(order)
    if order.Type == "S" then
        local closed_trade = trading:FindClosedTrade():WhenCloseOrderID(order.OrderID):First();
        if closed_trade ~= nil and closed_trade.PL < 0 then
            core.host:execute("stop");
        end
    end
end
...
function Prepare(nameOnly)
    ...
    CTS_source = ExtSubscribe(2, nil, instance.parameters.TF, instance.parameters.CTS_type, "bar")
    ...
    if UseMandatoryClosing then
        core.host:execute("setTimer", 99, math.max(ValidInterval / 2, 1))
    end
    Indicator = core.indicators:create("CHANDELIEREXIT_SS", CTS_source, Range, Shift, ATRPeriod, ATRMultipl, core.rgb(0, 255, 0), core.rgb(255, 0, 0));

    tables_monitor:ListenCloseTrade(OnClosedTrade)
    tables_monitor:ListenCloseOrder(OnCloseOrder)
end
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if
        Source[period] > Entry + Crossover * Source:pipSize() and
            Source[period - 1] <= Entry + Crossover * Source:pipSize() and
            (Source[period] - Source[period - 1]) >= (Bar * Source:pipSize())
     then
        if Direction then
            BUY()
        else
    ...
    elseif
        Source[period] < Entry - Crossover * Source:pipSize() and
            Source[period - 1] >= Entry - Crossover * Source:pipSize() and
            (Source[period - 1] - Source[period]) >= (Bar * Source:pipSize())
     then
        if Direction then
            SELL()
        else
...
function ExtAsyncOperationFinished(cookie, success, message, message1, message2)
    ...
    elseif cookie == 42 and not success then
    ...
    elseif cookie == 43 and not success then
        terminal:alertMessage(
            instance.bid:instrument(),
            instance.bid[instance.bid:size() - 1],
...
function MarketOrder(BuySell)
    ...
    if SetStop then
        command = command:SetPipStop(nil, Stop, TrailingStop and 1 or nil);
    end
    if SetLimit then
        command = command:SetPipLimit(nil, Limit);
    end
    ...
    local controller = breakeven:CreateIndicatorTrailingController()
        :SetTrailingTarget(breakeven.STOP_ID)
        :SetRequestID(result.RequestID)
        :SetIndicatorStream(Indicator.DATA, 1, false)
        :SetIndicatorStreamShift(x_shift, y_shift);
...
function exitTrade(tradeRow)
    ...
    success, msg = terminal:execute(43, valuemap)
...
function trading:CreateLimitOrder(trade, limit_rate)
    ...
    local success, msg = terminal:execute(42, valuemap);
    if not(success) then
