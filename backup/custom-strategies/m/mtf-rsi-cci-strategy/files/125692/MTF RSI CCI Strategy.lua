...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=68324
...
function Init() --The strategy profile initialization
    strategy:name("MTF RSI CCI Strategy")
    ...
    strategy.parameters:addBoolean("Use_RSI", "Use RSI", "", true)
    strategy.parameters:addBoolean("Use_REX", "Use REX", "", true)
    ...
    strategy.parameters:addString("TF2", "Time frame", "", "W1")
    strategy.parameters:setFlag("TF2", core.FLAG_PERIODS)

    strategy.parameters:addInteger("RSI_Period", "RSI_Period", "", 14)

    strategy.parameters:addInteger("SP", "Rex Smoothing Period", "Smoothing Period", 14)
    strategy.parameters:addInteger("SS", "Rex Signal Period", "Signal Period", 14)

    strategy.parameters:addInteger("ADX_Period", "ADX Period", "", 14)
    strategy.parameters:addDouble("ADX_Level", "ADX Level", "", 25)
...
local Use_RSI, Use_REX
local Rex, SP, SS

--Indicator parameters
local RSI, RSI_Period
local CCI_Period, CCI
local Use_ADX, ADX, ADX_Level, ADX_Period, Use_HA
local ha;
...
function ExitFunction(id, now, period)
    ...
    if
        (ExitExecutionType == "Live" and id ~= 1) or
            --/////////////////////////////////////////////////////////////////
            (ExitExecutionType == "EndOfTurn" and id ~= 2)
     then
        --/////////////////////////////////////////////////////////////////
        return
    end

    if (ExitExecutionType == "Live" and id == 1) then
...
function EntryFunction(id, now, period)
    ...
    if
        (EntryExecutionType == "Live" and id ~= 1) or
            --/////////////////////////////////////////////////////////////////
            (EntryExecutionType == "EndOfTurn" and id ~= 2)
     then
        --/////////////////////////////////////////////////////////////////
        return
    end

    if (EntryExecutionType == "Live" and id == 1) then
        period1 = core.findDate(Source1, TickSource:date(period), false)
        period2 = core.findDate(Source2, TickSource:date(period), false)
    end
    ...
    if
        not RSI.DATA:hasData(period2) or not CCI.DATA:hasData(period1) or not REX.DATA:hasData(period2) or
            not ADX.DATA:hasData(period2)
     then
        return
    end
    ...
    if
        ((RSI.DATA[period2] > 50 and Use_RSI) or not Use_RSI) and CCI.DATA[period1] > -100 and
            CCI.DATA[period1 - 1] <= -100 and
            LevelConfirmation(period1, 1) and
            ((REX.REX[period2] > REX.SIGNAL[period2] and Use_REX) or not Use_REX) and
            ((ADX.DATA[period2] > ADX_Level and Use_ADX) or not Use_ADX)
