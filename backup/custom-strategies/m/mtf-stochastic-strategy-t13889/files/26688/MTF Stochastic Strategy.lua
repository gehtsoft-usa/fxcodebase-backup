...
function Prepare(nameOnly)
    ...
    for i = 1, TfNumber, 1 do
        ...
        Indicator[i] = core.indicators:create("STOCHASTIC", Source[i], Parameters[tostring(i * 100 + 2)],
            Parameters[tostring(i * 100 + 3)], Parameters[tostring(i * 100 + 4)],
            Parameters[tostring(i * 100 + 5)], Parameters[tostring(i * 100 + 6)])
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if
        core.crossesOver(K[1], D[1], K[1]:size() - 2, D[1]:size() - 2) and
            ...
            K[1][K[1]:size() - 2] < Parameters[tostring(1 * 100 + 8)] and
            (K[2][K[2]:size() - 2] < Parameters[tostring(2 * 100 + 8)] or not ON[2]) and
            (K[3][K[3]:size() - 2] < Parameters[tostring(3 * 100 + 8)] or not ON[3])
     then
        if Direction then
            BUY()
        else
    ...
    elseif
        core.crossesUnder(K[1], D[1], K[1]:size() - 2, D[1]:size() - 2) and
            (K[2][K[2]:size() - 2] < D[2][D[2]:size() - 2] or not ON[2]) and
            (K[3][K[3]:size() - 2] < D[3][D[3]:size() - 2] or not ON[3]) and
            K[1][K[1]:size() - 2] > Parameters[tostring(1 * 100 + 7)] and
            (K[2][K[2]:size() - 2] > Parameters[tostring(2 * 100 + 7)] or not ON[2]) and
            (K[3][K[3]:size() - 2] > Parameters[tostring(3 * 100 + 7)] or not ON[3])
     then
        if Direction then
            SELL()
        else
