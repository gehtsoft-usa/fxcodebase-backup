...
function ConfirmLongMA(source, indi, period)
...
function ConfirmShortMA(source, indi, period)
    if not indi.Confirm then
        return true;
    end
    return source.close[period] < indi.ConfirmIndi.DATA[period];
...
function SignOfLongFalling(indi, period)
    ...
    return indi.Source.close[period] < indi.ConfirmIndi.DATA[period];
end
...
function IsBuyCondition(source, period, item)
    if item.ON then
        if item.IsConditionAbove then
            if item.K[NOW - 1] < item.D[NOW - 1] then
                return false;
            end
        else
            if item.K[NOW - 1] > item.D[NOW - 1] then
    ...
    return ConfirmLongMA(source, item, period)
        and not SignOfLongFalling(item, period)
        and ConfirmLongCross(item, period)
        and ConfirmLongTrend(source, period);
end

function IsSellCondition(source, period, item)
    if item.ON then
        if item.IsConditionAbove then
            if item.K[NOW - 1] < item.D[NOW - 1] then
                return false;
    ...
    return ConfirmShortMA(source, item, period)
        and not SignOfShortFalling(item, period)
        and ConfirmShortCross(item, period)
        and ConfirmShortTrend(source, period);
end
...
function TIMEFRAME(id, FRAME)
    ...
    strategy.parameters:addInteger(tostring(id * 100 + 3), "%D Period", "", 3, 1, 1000)
    ...
    strategy.parameters:addString("condition_" .. id, "Condition", "", "above")
    strategy.parameters:addStringAlternative("condition_" .. id, "Above", "", "above")
    strategy.parameters:addStringAlternative("condition_" .. id, "Below", "", "below")

    strategy.parameters:addBoolean("confirm" .. id, "Price should close over/under MA", "", false);
    strategy.parameters:addBoolean("confirm_reversal" .. id, "Price is over/under MA", "", false);
    ...
    for i = 1, TfNumber, 1 do
        ...
        if not nameOnly then
            ...
            item.Indicator = core.indicators:create("STOCHASTIC", item.Source, Parameters[tostring(i * 100 + 2)],
                Parameters[tostring(i * 100 + 3)], Parameters[tostring(i * 100 + 4)],
                Parameters[tostring(i * 100 + 5)], Parameters[tostring(i * 100 + 6)])
            item.IsConditionAbove = instance.parameters:getString("condition_" .. i) == "above"
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if IsBuyCondition(source, period, data[1])
        and IsBuyCondition(source, period, data[2])
        and IsBuyCondition(source, period, data[3])
        and (not IsBuyCondition(source, period - 1, data[1])
        or not IsBuyCondition(source, period - 1, data[2])
        or not IsBuyCondition(source, period - 1, data[3]))
    then
        if Direction then
            BUY()
        else
    ...
    elseif IsSellCondition(source, period, data[1])
        and IsSellCondition(source, period, data[2])
        and IsSellCondition(source, period, data[3])
        and (not IsSellCondition(source, period - 1, data[1])
        or not IsSellCondition(source, period - 1, data[2])
        or not IsSellCondition(source, period - 1, data[3]))
    then
        if Direction then
            SELL()
        else
