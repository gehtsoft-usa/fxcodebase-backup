...
function Prepare(nameOnly)
    ...
    for i = 1, TfNumber, 1 do
        ...
        if not nameOnly then
            ...
            if instance.parameters:getBoolean("confirm" .. i) then
...
function ConfirmLong(source, indi, period)
    ...
    return source.close[period] > indi.DATA[period];
end
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if
        core.crossesOver(data[1].K, data[1].D, data[1].K:size() - 2, data[1].D:size() - 2) and
            ...
            data[1].K[data[1].K:size() - 2] < Parameters[tostring(1 * 100 + 8)] and
            (data[2].K[data[2].K:size() - 2] < Parameters[tostring(2 * 100 + 8)] or not data[2].ON) and
            (data[3].K[data[3].K:size() - 2] < Parameters[tostring(3 * 100 + 8)] or not data[3].ON)
            and ConfirmLong(source, data[1].ConfirmIndi, NOW)
            and ConfirmLong(source, data[2].ConfirmIndi, NOW)
            and ConfirmLong(source, data[3].ConfirmIndi, NOW)
    then
        if Direction then
            BUY()
        else
    ...
    elseif
        core.crossesUnder(data[1].K, data[1].D, data[1].K:size() - 2, data[1].D:size() - 2) and
            (data[2].K[data[2].K:size() - 2] < data[2].D[data[2].D:size() - 2] or not data[2].ON) and
            (data[3].K[data[3].K:size() - 2] < data[3].D[data[3].D:size() - 2] or not data[3].ON) and
            data[1].K[data[1].K:size() - 2] > Parameters[tostring(1 * 100 + 7)] and
            (data[2].K[data[2].K:size() - 2] > Parameters[tostring(2 * 100 + 7)] or not data[2].ON) and
            (data[3].K[data[3].K:size() - 2] > Parameters[tostring(3 * 100 + 7)] or not data[3].ON)
            and ConfirmShort(source, data[1].ConfirmIndi, NOW)
            and ConfirmShort(source, data[2].ConfirmIndi, NOW)
            and ConfirmShort(source, data[3].ConfirmIndi, NOW)
     then
        if Direction then
            SELL()
        else
