...
function ExtUpdate(id, source, period)  -- The method called every time when a new bid or ask price appears.
   ...
   -- get only time
	...
	--1.m15 Trendstop goes from short to long (this is my buy signal)
		   ...
		   and (Indicator[2].DATA[Indicator[2].DATA:size()-2] > Source[2].close[Source[2].close:size()-2]  or not  ON[2]) 
		     and (Indicator[3].DATA[Indicator[3].DATA:size()-3] < Source[3].close[Source[3].close:size()-2]  or not  ON[3]) 
			   and (Indicator[4].DATA[Indicator[4].DATA:size()-4] < Source[4].close[Source[4].close:size()-2]  or not  ON[4]) 
		   then
...
function tradesCount(BuySell)
    ...
    while row ~= nil do
        if row.AccountID == Account and row.QTXT == CustomID and
