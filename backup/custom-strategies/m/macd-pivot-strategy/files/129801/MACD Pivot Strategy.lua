...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=69134
...
function Init() --The strategy profile initialization
    strategy:name("MACD Pivot Strategy")
    ...
    strategy.parameters:addGroup("MACD Caculation")

    strategy.parameters:addString("TF1", "1. Time frame", "", "H1")
    strategy.parameters:setFlag("TF1", core.FLAG_PERIODS)

    strategy.parameters:addString("TF2", "2. Time frame", "", "D1")
    strategy.parameters:setFlag("TF2", core.FLAG_PERIODS)

    strategy.parameters:addInteger("P1", "1. Period", "", 12)
    strategy.parameters:addInteger("P2", "2. Period", "", 26)
    strategy.parameters:addInteger("P3", "3. Period", "", 9)

    strategy.parameters:addGroup("Pivot")

    strategy.parameters:addInteger("Minimum", "Minimum", "", 100)
...
function CreateTradingParameters()
    ...
    strategy.parameters:addInteger("Limit", "Pivot Limit line index", "", 1, 1,3);
    strategy.parameters:addBoolean("SetStop", "Set Stop Orders", "", false)
    strategy.parameters:addInteger("Stop", "Stop line index", "", 1);
...
function Prepare(nameOnly)
    ...
    Pivot = core.indicators:create("PIVOT", Source1, instance.parameters.PivotTF, instance.parameters.CalcMode, "HIST")
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    -- update indicators.
    MACD1:update(core.UpdateLast)
    MACD2:update(core.UpdateLast)
...
function EntryFunction(id, now, period)
    ...
    if
        MACD1.MACD[period1] > MACD1.SIGNAL[period1] and MACD1.MACD[period1 - 1] <= MACD1.SIGNAL[period1 - 1] and
            MACD2.MACD[period2] > MACD2.SIGNAL[period2]
     then
        if Direction then
            local Top = FindTop(period1, Limit)
            local Bottom = FindBottom(period1, Stop)
            BUY(Top, Bottom)
        else
            local Top = FindTop(period1, Stop)
            local Bottom = FindBottom(period1, Limit)
            SELL(Bottom, Top)
        end
...
function FindBottom(period1, index)
    local Value = 0
    local count = 0;
    for i = 1, 4, 1 do
        if Pivot:getStream(i)[period1] < (Source1.low[period1] - Minimum * Source1:pipSize()) then
            Value = Pivot:getStream(i)[period1]
            count = count + 1;
        end
...
function FindTop(period1, index)
    local Value = 0
    local count = 0;
    for i = 5, 8, 1 do
        if Pivot:getStream(i)[period1] > (Source1.high[period1] + Minimum * Source1:pipSize()) then
            Value = Pivot:getStream(i)[period1]
            count = count + 1;
        end
...
function BUY(limit, stop)
    if AllowTrade then
        ...
        enter("B", 0, limit, stop)
    else
        Signal("Buy Signal")
    end
end

function SELL(limit, stop)
    if AllowTrade then
...
function enter(BuySell, hCount, limit, stop)
    ...
    return MarketOrder(BuySell, hCount, limit, stop)
end

-- enter into the specified direction
function MarketOrder(BuySell, hCount, limit, stop)
    ...
    if SetStop and stop ~= 0 and limit ~= 0 then
        if BuySell == "B" then
            valuemap.RateStop = math.min(limit, stop)
        else
            valuemap.RateStop = math.max(limit, stop)
        end
    end
    ...
    if SetLimit and limit ~= 0 and stop ~= 0 then
        if BuySell == "B" then
            valuemap.RateLimit = math.max(limit, stop)
        else
            valuemap.RateLimit = math.min(limit, stop)
        end
    end
