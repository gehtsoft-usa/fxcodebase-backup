...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=22644
...
function Init() --The strategy profile initialization
    strategy:name("McGinley Dynamic Strategy")
    ...
    strategy.parameters:addInteger("Frame", "Period", "", 9, 2, 2000)
    strategy.parameters:addInteger("Smoothing", "Smoothing", "", 125, 1, 2000)
...
function Prepare(nameOnly)
    ...
    Indicator[1] = core.indicators:create("MCGD", Source[Price], Frame, Smoothing)
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if core.crossesOver(Source.close, Indicator[1].DATA, period) then
        if Direction then
            BUY()
        else
            SELL()
        end
    elseif core.crossesUnder(Source.close, Indicator[1].DATA, period) then
        if Direction then
            SELL()
        else
            BUY()
