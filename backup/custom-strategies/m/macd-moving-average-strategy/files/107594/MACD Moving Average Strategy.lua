...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=63758
...
function Init() --The strategy profile initialization
    strategy:name("MACD Moving Average Strategy")
    ...
    strategy.parameters:addInteger("Period1", "Period", "", 200)
    ...
    strategy.parameters:addDouble("BuyLevel", "Buy Level", "", 0.00003)
    strategy.parameters:addDouble("SellLevel", "Sell Level", "", 0)
...
local MA
local MACD
local Period1, Method1, P1, P2, P3, BuyLevel, SellLevel
local Direction
local CustomID
...
function Prepare(nameOnly)
    ...
    Source = ExtSubscribe(2, nil, instance.parameters.TF, instance.parameters.Type == "Bid", "close")
    assert(core.indicators:findIndicator(Method1) ~= nil, Method1 .. " indicator must be installed");
    MA = core.indicators:create(Method1, Source, Period1)
    MACD = core.indicators:create("MACD", Source, P1, P2, P3)
    first = math.max(MA.DATA:first(), MACD.HISTOGRAM:first())
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
	...
	Price > MVA (200 Periods)
	Histogram > 0.00003

	And closes trade when these conditions not met. eg

	Price < MVA (200 Periods)
	Histogram < 0
	]]
    -- only buy if we have a fast cross over slow and the price is above the moving averages.
    if Source[period] > MA.DATA[period] and MACD.HISTOGRAM[period] > BuyLevel then
        if Direction then
            BUY()
        else
            SELL()
    ...
    elseif Source[period] < MA.DATA[period] and MACD.HISTOGRAM[period] < SellLevel then
        if Direction then
            SELL()
        else
            BUY()
    ...
    if Exit then
        if Source[period] < MA.DATA[period] or MACD.HISTOGRAM[period] < BuyLevel then
            if Direction then
        ...
        if Source[period] > MA.DATA[period] or MACD.HISTOGRAM[period] > SellLevel then
            if Direction then
                if haveTrades("S") then
                    exitSpecific("S")
                    Signal("Close Short")
