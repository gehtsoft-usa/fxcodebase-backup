...
--http://fxcodebase.com/code/viewtopic.php?f=31&t=63764
...
function Init() --The strategy profile initialization
    strategy:name("Middle-High-Low Moving Average Strategy");
    ...
    strategy.parameters:addInteger("MHLLength", "MHL Length", "MHL Length", 10);
    strategy.parameters:addInteger("MovAvgLength", "MovAvg Length", "MovAvg Length", 50);
...
function CreateTradingParameters()
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "MHLMAS");
...
local first;
local MHLLength, MovAvgLength, Method;	 
local Indicator;
...
function Prepare( nameOnly)
	...
	assert(core.indicators:findIndicator("MIDDLE-HIGH-LOW MOVING AVERAGE") ~= nil, "Please, download and install MIDDLE-HIGH-LOW MOVING AVERAGE.LUA indicator");          
    ...
    Indicator = core.indicators:create("MIDDLE-HIGH-LOW MOVING AVERAGE", Source, MHLLength, MovAvgLength, Method); 
...
function ExtUpdate(id, source, period)  -- The method called every time when a new bid or ask price appears.
   ...
   -- get only time
    ...
    if   Indicator.C[period]>  Indicator.HL[period]
	and  Indicator.C[period-1]<=  Indicator.HL[period-1]
	then	
          if Direction then
                BUY();
            else
    ...
    elseif   Indicator.C[period]<  Indicator.HL[period]
	and  Indicator.C[period-1]>=  Indicator.HL[period-1]
	then
            if Direction then
                SELL();
