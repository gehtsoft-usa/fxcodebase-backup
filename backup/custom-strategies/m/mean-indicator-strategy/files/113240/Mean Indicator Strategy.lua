...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=64850
...
function Init() --The strategy profile initialization
    strategy:name("Mean Indicator Strategy")
...
function Prepare(nameOnly)
    ...
    assert(
        core.indicators:findIndicator("MEAN CALCULATION") ~= nil,
        "Please, download and install MEAN CALCULATION.LUA indicator"
    )
    ...
    Indicator = core.indicators:create("MEAN CALCULATION", Source.close)

    first = Indicator.PrevBuff:first()
...
function EntryFunction(now, period)
    ...
    if
        Indicator.Mean[period] > Indicator.PrevBuff[period] and
            Indicator.Mean[period - 1] <= Indicator.PrevBuff[period - 1]
     then
        if Direction then
            BUY()
        else
