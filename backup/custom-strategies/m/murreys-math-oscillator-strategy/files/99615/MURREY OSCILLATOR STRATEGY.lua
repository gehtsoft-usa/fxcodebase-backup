...
function Init() --The strategy profile initialization
    strategy:name("MURREY OSCILLATOR STRATEGY");
...
function CreateTradingParameters()
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "MOS");
...
function ExtUpdate(id, source, period)  -- The method called every time when a new bid or ask price appears.
   ...
   -- get only time
    ...
    if   MMO.DATA[period] > 0
    and MMO.DATA[period-1] <= 0	
	then	
          if Direction then
                BUY();
            else
