...
local STRATEGY_NAME = "MTF Moving Average Cross Over";
...
function AddParams(id)
    strategy.parameters:addGroup("Timeframe #" .. id);
    strategy.parameters:addString("timeframe_" .. id, "Timeframe", "", "m1");
    strategy.parameters:setFlag("timeframe_" .. id, core.FLAG_PERIODS);
    strategy.parameters:addBoolean("is_bid_" .. id, "Price Type", "", true);
    strategy.parameters:setFlag("is_bid_" .. id, core.FLAG_BIDASK);
    strategy.parameters:addString("mva_condition_" .. id, "MVA condition", "", "above");
    strategy.parameters:addStringAlternative("mva_condition_" .. id, "MVA Above Price", "", "above");
    strategy.parameters:addStringAlternative("mva_condition_" .. id, "MVA Below Price", "", "below");
    strategy.parameters:addInteger("period_1_" .. id, "Period of MVA", "", 7);
    
    strategy.parameters:addString("fast_slow_condition_" .. id, "Fast/Slow condition", "", "above");
    strategy.parameters:addStringAlternative("fast_slow_condition_" .. id, "Fast Above Slow", "", "above");
    strategy.parameters:addStringAlternative("fast_slow_condition_" .. id, "Fast Below Slow", "", "below");
    strategy.parameters:addInteger("period_2_" .. id, "Period of Fast EMA", "", 7);
    strategy.parameters:addInteger("period_3_" .. id, "Period of Slow EMA", "", 14);

    strategy.parameters:addString("price_ema_condition_" .. id, "Price/EMA condition", "", "above");
    strategy.parameters:addStringAlternative("price_ema_condition_" .. id, "Price Above EMA", "", "above");
    strategy.parameters:addStringAlternative("price_ema_condition_" .. id, "Price Below EMA", "", "below");
    strategy.parameters:addInteger("period_4_" .. id, "Period of EMA #3", "", 14);

    strategy.parameters:addString("stoch_condition_" .. id, "Stochastic condition", "", "no");
    strategy.parameters:addStringAlternative("stoch_condition_" .. id, "Do not use", "", "no");
    strategy.parameters:addStringAlternative("stoch_condition_" .. id, "K > D", "", "above");
    strategy.parameters:addStringAlternative("stoch_condition_" .. id, "K < D", "", "below");
    strategy.parameters:addInteger("stoch_k_" .. id, "Stochastic %K Periods", "", 15, 1, 10000);
    strategy.parameters:addInteger("stoch_sd_" .. id, "Stochastic %D Slowing Periods", "", 7, 1, 10000);
    strategy.parameters:addInteger("stoch_d_" .. id, "Stochastic Periods ", "", 3, 1, 10000);
    strategy.parameters:addString("stoch_k_method_" .. id, "Stochastic %K Method", "", "MVA");
    strategy.parameters:addString("stoch_d_method_" .. id, "Stochastic %D Method", "", "MVA");
end
...
function CreateCustomActions()
    ...
    enterLongAction.IsPass = function (source, period)
        local shift = source:size() - 1 - period;
        for i = 1, 3 do
            if timeframes[i].Source:size() == 0 then
                return false;
            end
            if timeframes[i].MVAConditionAbove then
                if timeframes[i].Source.close[NOW - shift] >= timeframes[i].MVA.DATA[NOW - shift] then
                    if core.host.Trading:getTradingProperty("isSimulation") then
                        signaler:Signal("FAIL: Close < MVA; " .. timeframes[i].Source:barSize());
                    end
                    return false;
                end
            else
            ...
            if timeframes[i].FastAboveSlow then
                if timeframes[i].FastEMA.DATA[NOW - shift] <= timeframes[i].SlowEMA.DATA[NOW - shift] then
                    if core.host.Trading:getTradingProperty("isSimulation") then
                        signaler:Signal("FAIL: Fast EMA > Slow EMA; " .. timeframes[i].Source:barSize());
                    end
                    return false;
                end
            else
            ...
            if timeframes[i].PriceAboveEMA then
                if (timeframes[i].Source.close[NOW - shift] <= timeframes[i].FastEMA.DATA[NOW - shift])
                    or timeframes[i].Source.close[NOW - shift] <= timeframes[i].EMA3.DATA[NOW - shift]
                then
                    if core.host.Trading:getTradingProperty("isSimulation") then
                        signaler:Signal("FAIL: Close > Fast EMA and EMA; " .. timeframes[i].Source:barSize());
                    end
                    return false;
                end
            else
            ...
            if timeframes[i].Stoch ~= nil then
                if timeframes[i].StochKAboveD then
                    if timeframes[i].Stoch.K[NOW - shift] <= timeframes[i].Stoch.D[NOW - shift] then
                        if core.host.Trading:getTradingProperty("isSimulation") then
                            signaler:Signal("FAIL: Stoch K > D; " .. timeframes[i].Source:barSize());
                        end
                        return false;
                    end
                else
...
local TIMEFRAME_1_ID = 3;
local TIMEFRAME_2_ID = 4;
local TIMEFRAME_3_ID = 5;

function CreateIndicator(id, tf_id)
    local tf = {};
    tf.Source = ExtSubscribe(tf_id, instance.bid:instrument(), 
        instance.parameters:getString("timeframe_" .. id), 
        instance.parameters:getBoolean("is_bid_" .. id), "bar");
    tf.MVA = core.indicators:create("MVA", tf.Source, instance.parameters:getInteger("period_1_" .. id));
    tf.MVAConditionAbove = instance.parameters:getString("mva_condition_" .. id) == "above";

    tf.FastEMA = core.indicators:create("EMA", tf.Source, instance.parameters:getInteger("period_2_" .. id));
    tf.SlowEMA = core.indicators:create("EMA", tf.Source, instance.parameters:getInteger("period_3_" .. id));
    tf.FastAboveSlow = instance.parameters:getString("fast_slow_condition_" .. id) == "above";

    tf.EMA3 = core.indicators:create("EMA", tf.Source, instance.parameters:getInteger("period_4_" .. id));
    tf.PriceAboveEMA = instance.parameters:getString("price_ema_condition_" .. id) == "above";

    if instance.parameters:getString("price_ema_condition_" .. id) ~= "no" then
        tf.StochKAboveD = instance.parameters:getString("price_ema_condition_" .. id) == "above";
        tf.Stoch = core.indicators:create("STOCHASTIC", tf.Source, 
            instance.parameters:getInteger("stoch_k_" .. id),
            instance.parameters:getInteger("stoch_sd_" .. id),
            instance.parameters:getInteger("stoch_d_" .. id),
            instance.parameters:getString("stoch_k_method_" .. id),
            instance.parameters:getString("stoch_d_method_" .. id));
    end
    return tf;
end

function CreateIndicators(source)
    timeframes[1] = CreateIndicator(1, TIMEFRAME_1_ID);
    timeframes[2] = CreateIndicator(2, TIMEFRAME_2_ID);
    timeframes[3] = CreateIndicator(3, TIMEFRAME_3_ID);
end

function UpdateIndicators()
    for i = 1, 3 do
        timeframes[i].MVA:update(core.UpdateLast);
        timeframes[i].FastEMA:update(core.UpdateLast);
        timeframes[i].SlowEMA:update(core.UpdateLast);
        timeframes[i].EMA3:update(core.UpdateLast);
        if timeframes[i].Stoch ~= nil then
            timeframes[i].Stoch:update(core.UpdateLast);
        end
    end
end
...
function Init()
    ...
    strategy.parameters:addString("Direction", "Type of Signal / Trade", "", "buy");
