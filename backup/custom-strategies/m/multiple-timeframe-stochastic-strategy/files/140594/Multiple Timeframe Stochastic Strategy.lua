...
function CreateParameters() 
    strategy.parameters:addInteger("Period", "Period", "", 21, 1, 2000);
    strategy.parameters:addString("TF1", "1. Time Frame", "", "m30");
	...
	strategy.parameters:addString("TF3", "3. Time Frame", "", "H8");
	strategy.parameters:setFlag("TF3", core.FLAG_PERIODS);
	strategy.parameters:addString("TF4", "4. Time Frame", "", "D1");
	strategy.parameters:setFlag("TF4", core.FLAG_PERIODS);
	strategy.parameters:addString("TF5", "5. Time Frame", "", "W1");
	strategy.parameters:setFlag("TF5", core.FLAG_PERIODS);
	strategy.parameters:addString("TF6", "6. Time Frame", "", "M1");
	strategy.parameters:setFlag("TF6", core.FLAG_PERIODS);
    strategy.parameters:addDouble("entry_level", "Entry Level", "", 2.1);
end

local indi, entry_level;
function CreateEntryIndicators(source)
    local profile = core.indicators:findIndicator("MULTIPLE TIMEFRAME STOCHASTIC");
    assert(profile ~= nil, "Please, download and install " .. "MULTIPLE TIMEFRAME STOCHASTIC" .. ".LUA indicator");
    ...
    indicatorParams:setString("TF3", instance.parameters.TF3);
    indicatorParams:setString("TF4", instance.parameters.TF4);
    indicatorParams:setString("TF5", instance.parameters.TF5);
    indicatorParams:setString("TF6", instance.parameters.TF6);
    indi = core.indicators:create("MULTIPLE TIMEFRAME STOCHASTIC", source, indicatorParams)
    entry_level = instance.parameters.entry_level;
end
...
function IsEntryLong(source, period)
    ...
    return core.crossesOver(indi.BuyPressure, entry_level, period);
end
function IsEntryShort(source, period)
    if not indi.DATA:hasData(period - 1) then
        return false;
    end
    return core.crossesOver(indi.SellPressure, entry_level, period);
end
function IsExitLong(source, period)
    return core.crossesUnder(indi.BuyPressure, entry_level, period);
end
function IsExitShort(source, period)
    return core.crossesUnder(indi.SellPressure, entry_level, period);
end
