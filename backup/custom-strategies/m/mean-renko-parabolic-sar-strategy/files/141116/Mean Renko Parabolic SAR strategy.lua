...
--http://fxcodebase.com/code/viewtopic.php?f=31&t=70988
--+------------------------------------------------------------------+
...
local STRATEGY_NAME = "Mean Renko Parabolic SAR strategy";
...
function CreateParameters()
    ...
    strategy.parameters:addString("ma_tf", "MA Timeframe", "", "m5");
    strategy.parameters:setFlag("ma_tf", core.FLAG_BARPERIODS);
	
	strategy.parameters:addGroup("MA Filter Calculation");
    strategy.parameters:addBoolean("Filter", "Use Filter", "", true);
    
    strategy.parameters:addInteger("ma_period", "name Period", "", 34);
    AddAverages("ma_method", "name Method", "MVA");

    strategy.parameters:addGroup("Mean Renko Calculation")
    strategy.parameters:addInteger("Step_", "Step (Pips)", "", 2);
	strategy.parameters:addDouble("OpeningPricePercentage", "Opening Price Percentage (%)", "", 50, 0, 100);
	strategy.parameters:addDouble("ReversalPercentage", "Reversal Percentage (%)", "", 200, 0, 1000);
    strategy.parameters:addBoolean("Smoothed","Smoothed", "", true);
end
local SAR, ma_source, ma, renko;
function CreateEntryIndicators(source) 
    local profile = core.indicators:findIndicator("MEAN RENKO BARS VIEW");
    assert(profile ~= nil, "Please, download and install " .. "MEAN RENKO BARS VIEW" .. ".LUA strategy");
    local strategyParams = profile:parameters();
    strategyParams:setString("instrument", source:instrument());
    strategyParams:setString("frame", source:barSize());
    strategyParams:setBoolean("type", instance.parameters.Type == "Bid");
    strategyParams:setBoolean("Smoothed", instance.parameters.Smoothed);
    strategyParams:setInteger("Step", instance.parameters.Step_);
    strategyParams:setDate("from", core.now() - 500);
    strategyParams:setDate("to", 0);
    strategyParams:setDouble("OpeningPricePercentage", instance.parameters.OpeningPricePercentage);
    strategyParams:setDouble("ReversalPercentage", instance.parameters.ReversalPercentage);
    renko = core.indicators:create("MEAN RENKO BARS VIEW", source, strategyParams)

    SAR	= core.indicators:create("SAR", renko:getCandleOutput(0), instance.parameters.Step, instance.parameters.Max);
    ma_source = trading_logic:SubscribeHistory(source:instrument(), instance.parameters.ma_tf, instance.parameters.is_bid);
    ma = CreateAverages(instance.parameters.ma_period, instance.parameters.ma_method, ma_source);
end
...
function FindLast(Data, period)
    for i = period, Data:first(), -1 do
        if Data:hasData(i) then
            return Data[i];
        end
    end
    return 0;
...
function CreateCustomActions()
    ...
    enterLongAction.IsPass = function (source, period, periodFromLast, data)
        local BuyLevel = FindLast(SAR:getStream(0), period)
        return SAR:getStream(1):hasData(period)
            and source.close[period] > BuyLevel
            and (not instance.parameters.Filter or source.close[period] > ma.DATA[NOW]);
    end
    ...
    enterShortAction.IsPass = function (source, period, periodFromLast, data)
        local SellLevel = FindLast(SAR:getStream(1), period)
        return SAR:getStream(0):hasData(period)
            and source.close[period] < SellLevel
            and (not instance.parameters.Filter or source.close[period] < ma.DATA[NOW]);
    end
