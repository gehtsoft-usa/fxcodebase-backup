...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=61783
...
function Init() --The strategy profile initialization
    strategy:name("MA ATR FRACTAL STRATEGY")
    ...
    strategy.parameters:addDouble("ATR_Threshold", "ATR Threshold (in Pips)", "", 14)
    ...
    strategy.parameters:addInteger("CCI_Period", "CCI period", "", 200)
    strategy.parameters:addDouble("OB", "Buy Level", "", 0)
    strategy.parameters:addDouble("OS", "Sell Level", "", 0)
...
local first
local TMA_Period, ATR_Threshold, ATR
local MA_Period, MA
local CCI, CCI_Period, OB, OS
local Indicator = {}
local Short = {}
local Direction
local CustomID
...
function Prepare(nameOnly)
    ...
    ATR_Threshold = instance.parameters.ATR_Threshold
    ...
    MA = core.indicators:create("MVA", ATR.DATA, MA_Period)
    CCI = core.indicators:create("CCI", Source, CCI_Period)
    first = math.max(MA.DATA:first(), CCI.DATA:first()) + 1
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    --[[BUY: 
	ATR> MA AND ATR < THRESOLD AND PRICE CROSS OVER UPPER FRACTAL AND CCI > BUY LEVEL
	SELL: 
	ATR> MA AND ATR< THRESOLD AND PRICE CROSS UNDER LOWE FRACTAL AND CCI < SELL LEVEL
    ]]
    local Up, Down = Fractal(period)

    if
        ATR.DATA[period] > MA.DATA[period] and ATR.DATA[period] < ATR_Threshold * source:pipSize() and
            CCI.DATA[period] > OB and
            core.crossesOver(Source.close, Up, period)
     then
        if Direction then
            BUY()
        else
    ...
    elseif
        ATR.DATA[period] > MA.DATA[period] and ATR.DATA[period] < ATR_Threshold * source:pipSize() and
            CCI.DATA[period] < OS and
            core.crossesUnder(Source.close, Down, period)
     then
        if Direction then
            SELL()
        else
    ...
    --EXIT BUY: ATR CROSS UNDER MA
    --EXIT SELL: ATR CROSS UNDER MA
    if Exit then
        if core.crossesUnder(ATR.DATA, MA.DATA, period) then
            if Direction then
...
function Fractal(Last)
    local Up = 0
    local Down = 0
    local curr

    for period = Last, Source:first(), -1 do
        if Up == 0 then
