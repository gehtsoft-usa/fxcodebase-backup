...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=26441
...
function Init() --The strategy profile initialization
    strategy:name("MTF Schaff Trend Cycle Oscillator Strategy");
	...
	Parameters (1, "m15",5,95);
	Parameters (2, "H1" ,98,2);
    Parameters (3, "D1",99,1);
	 ...
	 ExitParameters(1, true,95,5)
	 ExitParameters(2, true,98,2)
	 ExitParameters(3, false,99,1)
...
function ExitParameters (id, Flag, One , Two)

 strategy.parameters:addBoolean("ON"..id, id ..". Use This Time Frame in  Exit Calculation", "", Flag);
 
 strategy.parameters:addDouble("EB"..id, "Buy Exit Level", "" , One );
 strategy.parameters:addDouble("ES"..id, "Sell Exit Level", "" , Two);

end

function Parameters (id, TF, LB, LS )
    ...
    strategy.parameters:addInteger("C"..id, "Schaff cycle periods", "", 10, 2, 10000);
    strategy.parameters:addInteger("S"..id, "Short periods", "", 23, 2, 10000);
    strategy.parameters:addInteger("L"..id, "Long periods", "", 50, 2, 10000);
	
	strategy.parameters:addDouble("LB"..id, "Buy Level", "" , LB);
	strategy.parameters:addDouble("LS"..id, "Sell Level", "" , LS);
end
...
function Prepare(nameOnly)
   ...
   assert(core.indicators:findIndicator("SCHTC") ~= nil, "Please, download and install SCHTC.LUA indicator");
	...
	LB[i]=  instance.parameters:getDouble("LB" .. i);
	LS[i]=  instance.parameters:getDouble("LS" .. i);
	EB[i]=  instance.parameters:getDouble("EB" .. i);
	ES[i]=  instance.parameters:getDouble("ES" .. i);
    ...
    assert(core.indicators:findIndicator( "SCHTC") ~= nil,  "SCHTC" .. " indicator must be installed");
    Indicator[i] = core.indicators:create( "SCHTC", Source[i][Price[i]], C[i], S[i],L[i] );	
...
function ExtUpdate(id, source, period)  -- The method called every time when a new bid or ask price appears.
   ...
   -- get only time
	...
	then
		...
		if    core.crossesOver(Short[1],  LB[1], Short[1]:size() - 2 )
		and   ((  Short[2][ Short[2]:size() - 2] >  LB[2]) or not USE[2])
		and   ((  Short[3][ Short[3]:size() - 2] >  LB[3]) or not USE[3])
        ...
        elseif   core.crossesUnder(Short[1],  LS[1], Short[1]:size() - 2 )
		and   ((  Short[2][ Short[2]:size() - 2] <  LS[2]) or not USE[2])
		and   ((  Short[3][ Short[3]:size() - 2] <  LS[3]) or not USE[3])
		...
		if EXIT then
		        ...
		        if    (core.crossesUnder(Short[1], EB[1], Short[1]:size() - 2 ) and   ON[1])
				or   (core.crossesUnder(Short[2], EB[2], Short[2]:size() - 2 ) and   ON[2])
				or   (core.crossesUnder(Short[3], EB[3], Short[3]:size() - 2 ) and   ON[3])
				then
				     if   Direction then
		        ...
		        if    (core.crossesOver(Short[1], ES[1], Short[1]:size() - 2 ) and   ON[1])
				or   (core.crossesOver(Short[2], ES[2], Short[2]:size() - 2 ) and   ON[2])
				or   (core.crossesOver(Short[3], ES[3], Short[3]:size() - 2 ) and   ON[3])
				then
				             if   Direction then    
										if haveTrades("S") then
										   exit("S");
