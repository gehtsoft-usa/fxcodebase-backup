...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=25883
...
function Init() --The strategy profile initialization
    strategy:name("MTF Alternate Ichimoku Strategy")
...
function Parameters(id, TF)
    ...
    strategy.parameters:addInteger("SSP" .. id, "SSP", "Period Priority Line", 75)
    strategy.parameters:addInteger("SSK" .. id, "SSK", "Tolerance Second Line", 75)
end
...
function Prepare(nameOnly)
    ...
    for i = 1, 4, 1 do
        ...
        SSP[i] = instance.parameters:getInteger("SSP" .. i)
        SSK[i] = instance.parameters:getInteger("SSK" .. i)
    ...
    assert(core.indicators:findIndicator("C_ICH_AL") ~= nil, "Please, download and install C_ICH_AL.LUA indicator")

    for i = 1, 4, 1 do
        Short[i] = {}

        Source[i] = ExtSubscribe(i, nil, TF[i], instance.parameters.Type == "Bid", "bar")
        Indicator[i] = core.indicators:create("C_ICH_AL", Source[i], SSP[i], SSK[i])
        Short[i]["A"] = Indicator[i]:getStream(3)
        Short[i]["B"] = Indicator[i]:getStream(4)
    end
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    --     crossover functions
    if
        Short[1]["A"]:size() <= Short[1]["A"]:first() + 3 or Short[2]["A"]:size() <= Short[2]["A"]:first() + 3 or
            Short[3]["A"]:size() <= Short[3]["A"]:first() + 3 or
            Short[4]["A"]:size() <= Short[4]["A"]:first() + 3
     then
        return
    end
    ...
    if
        Short[1]["A"][Short[1]["A"]:size() - 2] > Short[1]["B"][Short[1]["B"]:size() - 2] and
            ((Short[2]["A"][Short[2]["A"]:size() - 2] > Short[2]["B"][Short[2]["B"]:size() - 2]) or not USE[2]) and
            ((Short[3]["A"][Short[3]["A"]:size() - 2] > Short[3]["B"][Short[3]["B"]:size() - 2]) or not USE[3]) and
            ((Short[4]["A"][Short[4]["A"]:size() - 2] > Short[4]["B"][Short[4]["B"]:size() - 2]) or not USE[4]) and
            ((Source[2].close[Source[2].close:size() - 2] > Short[2]["A"][Short[2]["A"]:size() - 2]) or not USE[2]) and
            ((Source[3].close[Source[3].close:size() - 2] > Short[3]["A"][Short[3]["A"]:size() - 2]) or not USE[2]) and
            ((Source[4].close[Source[4].close:size() - 2] > Short[4]["A"][Short[4]["A"]:size() - 2]) or not USE[2]) and
            (Source[1].close[Source[1].close:size() - 2] > Short[1]["A"][Short[1]["A"]:size() - 2]) and
            (Source[1].close[Source[1].close:size() - 3] < Short[1]["A"][Short[1]["A"]:size() - 3])
     then
        if Direction then
            BUY()
        else
    ...
    if EXIT then
        if
            ((Source[2].close[Source[2].close:size() - 2] < Short[2]["A"][Short[2]["A"]:size() - 2]) and ON[2]) and
                ((Source[3].close[Source[3].close:size() - 2] < Short[3]["A"][Short[3]["A"]:size() - 2]) and ON[3]) and
                ((Source[4].close[Source[4].close:size() - 2] < Short[4]["A"][Short[4]["A"]:size() - 2]) and ON[4]) and
                ((Source[1].close[Source[1].close:size() - 2] < Short[1]["A"][Short[1]["A"]:size() - 2]) and ON[4])
         then
            if Direction then
