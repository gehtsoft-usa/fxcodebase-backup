...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=10449
...
function Init() --The strategy profile initialization
    strategy:name("MBKAsctrend3 strategy")
    strategy:description("MBKAsctrend3 strategy")
    strategy:setTag("Version", "1.1")
    strategy:setTag("NonOptimizableParameters", "Email,SendEmail,SoundFile,RecurrentSound,PlaySound, ShowAlert")

    strategy.parameters:addGroup("Parameters")
    strategy.parameters:addInteger("WPRPeriod1", "Period for WPR1", "", 9)
    strategy.parameters:addInteger("WPRPeriod2", "Period for WPR2", "", 33)
    strategy.parameters:addInteger("WPRPeriod3", "Period for WPR3", "", 77)
    strategy.parameters:addInteger("Swing", "Swing", "", 3)
    strategy.parameters:addInteger("AverSwing", "Average swing", "", -5)
    strategy.parameters:addInteger("W1", "Weight for WPR1", "", 1)
    strategy.parameters:addInteger("W2", "Weight for WPR2", "", 3)
    strategy.parameters:addInteger("W3", "Weight for WPR3", "", 1)
...
function Prepare(nameOnly)
    ...
    assert(
        core.indicators:findIndicator("MBKASCTREND3") ~= nil,
        "Please, download and install MBKASCTREND3.LUA indicator"
    )
    ...
    MBKA =
        ...
        "MBKASCTREND3",
        Source,
        instance.parameters.WPRPeriod1,
        instance.parameters.WPRPeriod2,
        instance.parameters.WPRPeriod3,
        instance.parameters.Swing,
        instance.parameters.AverSwing,
        instance.parameters.W1,
        instance.parameters.W2,
        instance.parameters.W3
    )
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    MBKA:update(core.UpdateLast)

    -- Check that we have enough data
    if (MBKA.DATA:first() > (period - 1)) then
        return
    end
    ...
    if MBKA.DATA[period] > source.low[period] / 2 and MBKA.DATA[period] < source.low[period] then
    ...
    if MBKA.DATA[period] > source.high[period] then
        if instance.parameters.TypeSignal == "direct" then
            MustOpenS = true
        else
            MustOpenB = true
