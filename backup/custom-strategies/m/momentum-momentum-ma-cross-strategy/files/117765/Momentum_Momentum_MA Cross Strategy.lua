...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=65737
...
function Init() --The strategy profile initialization
    strategy:name("Momentum_Momentum_MA Cross Strategy");
	...
	strategy.parameters:addInteger("MA_Period", "Period", "", 14);
...
--	strategy.parameters:addStringAlternative("ExitExecutionType", "Live", "", "Live");
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "MMMACS");
...
function Prepare( nameOnly)
    ...
    Source = ExtSubscribe(2, nil, TF, instance.parameters.Type == "Bid", "bar");
     Indicator = core.indicators:create("MOMENTUM", Source.close, instance.parameters.Period );
    assert(core.indicators:findIndicator( instance.parameters.MA_Method) ~= nil,  instance.parameters.MA_Method .. " indicator must be installed");
	 MA = core.indicators:create( instance.parameters.MA_Method, Indicator.DATA, instance.parameters.MA_Period );
	first=MA.DATA:first(); 
...
function EntryFunction( now,period)
	...
	then	
		...
		Return=true;
    elseif   Indicator.DATA[period]< MA.DATA[period]
	and  Indicator.DATA[period-1]>= MA.DATA[period-1]
	 then
            if Direction then
                SELL();
