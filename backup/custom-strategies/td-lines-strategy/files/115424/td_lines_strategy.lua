...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=65015
...
trading._stop = nil;
trading._customID = nil;
trading.close_on_opposite = nil;
...
function trading:Init(parameters)
    ...
    parameters:addString("customID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "TD_LINES_ST");
...
function trading:Prepare(name_only)
    ...
    self._customID = instance.parameters.customID;
    self._close_on_opposite = instance.parameters.close_on_opposite;
    if self.AddAmountParameter then
...
function trading:CloseSideForInstrumentAndAccount(instrument, side, account_id)
    ...
    valuemap.CustomID = self._customID;
...
trading_logic._start_time = nil;
trading_logic._stop_time = nil;
...
function trading_logic:Init(parameters)
    ...
    parameters:addGroup("Time Parameters");
    parameters:addString("start_time", "Start Time for Trading", "", "00:00:00");
    parameters:addString("stop_time", "Stop Time for Trading", "", "24:00:00");
end
...
function trading_logic:Prepare(name_only)
    ...
    self._start_time, valid = ParseTime(instance.parameters.start_time);
    assert(valid, "Time " .. instance.parameters.start_time .. " is invalid");
    self._stop_time, valid = ParseTime(instance.parameters.stop_time);
    assert(valid, "Time " .. instance.parameters.stop_time .. " is invalid");
end
function trading_logic:ExtUpdate(id, source, period)
    if id == self._trading_source_id and self.DoTrading ~= nil then
        ...
        if now >= self._start_time and now <= self._stop_time then
...
function Prepare(name_only)
    ...
    instance: name (profile: id () .. "(" .. instance.bid:name () .. "(" .. instance.parameters.customID .. ")");
...
function DoAction(instrument, action)
    if action == "buy" then
        if trading._close_on_opposite == true then
            trading.CloseSideForInstrument(trading, instrument, "S")
        end 
        return trading:CreateMarketOrder(instrument, "B");
    elseif action == "sell" then
        if trading._close_on_opposite == true then
            trading.CloseSideForInstrument(trading, instrument, "B")
        end 
