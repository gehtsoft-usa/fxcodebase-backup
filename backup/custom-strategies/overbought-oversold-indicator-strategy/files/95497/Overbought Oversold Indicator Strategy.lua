...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=61064
...
function Init() --The strategy profile initialization
    strategy:name("Overbought/Oversold Indicator Strategy")
    ...
    strategy.parameters:addInteger("OB", "OB Level", "", 100)
    strategy.parameters:addInteger("OS", "OS Level", "", -100)

    strategy.parameters:addBoolean("BuyOne", "Buy on OB Level Signal", "", true)
    strategy.parameters:addBoolean("BuyTwo", "Buy on Central Level Signal", "", true)
    strategy.parameters:addBoolean("BuyThree", "Buy on OS Level Signal", "", true)

    strategy.parameters:addBoolean("SellOne", "Sell on OB Level Signal", "", true)
    strategy.parameters:addBoolean("SellTwo", "Sell on Central Level Signal", "", true)
    strategy.parameters:addBoolean("SellThree", "Sell on OS Level Signal", "", true)
...
local CustomID
local Indicator
local BuyThree, BuyTwo, BuyOne
local SellThree, SellTwo, SellOne
...
function Prepare(nameOnly)
    ...
    BuyThree = instance.parameters.BuyThree
    BuyTwo = instance.parameters.BuyTwo
    BuyOne = instance.parameters.BuyOne

    SellThree = instance.parameters.SellThree
    SellTwo = instance.parameters.SellTwo
    SellOne = instance.parameters.SellOne
    ...
    name =
        profile:id() ..
        "( " .. instance.bid:name() .. "," .. Period .. "," .. OB .. "," .. OS .. "," .. CustomID .. " )"
    instance:name(name)
    ...
    Indicator = core.indicators:create("OBOS", Source, Period)
    first = math.max(Indicator.DATA:first()) + 1
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if BuyOne and core.crossesOver(Indicator.close, OB, period) then
        BUY()
        ONE = Source:serial(period)
    end

    if BuyTwo and core.crossesOver(Indicator.close, 0, period) then
        BUY()
        ONE = Source:serial(period)
    end

    if BuyThree and core.crossesOver(Indicator.close, OS, period) then
        BUY()
        ONE = Source:serial(period)
    end

    if SellOne and core.crossesUnder(Indicator.close, OB, period) then
        SELL()
        ONE = Source:serial(period)
    end

    if SellTwo and core.crossesUnder(Indicator.close, 0, period) then
        SELL()
        ONE = Source:serial(period)
    end

    if SellThree and core.crossesUnder(Indicator.close, OS, period) then
        SELL()
        ONE = Source:serial(period)
    end
end
