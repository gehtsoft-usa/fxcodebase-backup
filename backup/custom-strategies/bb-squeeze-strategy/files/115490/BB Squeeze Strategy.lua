...
--http://fxcodebase.com/code/viewtopic.php?f=31&t=65184
...
function Init() --The strategy profile initialization
    strategy:name("BB Squeeze Strategy")
    ...
    strategy.parameters:addInteger("BP", "Bollinger Period", " ", 20)
    strategy.parameters:addDouble("BD", "Bollinger Deviations", " ", 2)

    strategy.parameters:addInteger("KP", "Keltner Period", " ", 20)
    strategy.parameters:addDouble("KF", "Keltner Factor", " ", 1.5)

    strategy.parameters:addDouble("MP", "Momentum Period", " ", 12)
    strategy.parameters:addString("MS", "Momentum smoothing method", "", "MVA")
    ...
    strategy.parameters:addDouble("MSP", "Momentum Smoothing Period", " ", 20)
...
local Indicator
local BP, BD, KP, KF, MP, MS, MSP, BB
...
function Prepare(nameOnly)
    ...
    Indicator = core.indicators:create("BBSQUEEZE", Source, BP, BD, KP, KF, MP, MS, MSP, true)
    BB = core.indicators:create("BB", Source.close, BP, BD)
    first = math.max(Indicator.BBSR:first(), BB.DATA:first())
...
function EntryFunction(now, period)
    ...
    if
        Source.close[period] > BB.TL[period] and Source.close[period - 1] <= BB.TL[period - 1] and
            Indicator.BBSR[period] > 100
     then
        if Direction then
            BUY()
        else
        ...
        Return = true
    elseif
        Source.close[period] < BB.BL[period] and Source.close[period - 1] >= BB.BL[period - 1] and
            Indicator.BBSR[period] < 100
     then
        if Direction then
            SELL()
