...
function Prepare(nameOnly)
    ...
    Indicator = core.indicators:create("LONGER SHORTER INDICATOR", Source, indicatorParams)
...
function DoMoveStop(trade)
    if commands[trade.TradeID] ~= nil then
        return;
    end
    local newStop;
    if (trade.BS == "B") then
        if core.crossesUnder(Source.close, Indicator.exit_buy, NOW) then
            commands[trade.TradeID] = trading:Close(trade);
        end
    else
        if core.crossesOver(Source.close, Indicator.exit_sell, NOW) then
            commands[trade.TradeID] = trading:Close(trade);
        end
    end
end
