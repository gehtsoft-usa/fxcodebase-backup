...
local STRATEGY_NAME = "Greezly Strategy";
function CreateParameters() 
    strategy.parameters:addGroup("AC");
    strategy.parameters:addBoolean("use_ac", "Use AC", "", true);
    strategy.parameters:addInteger("FastPeriod", "Fast period for AC", "", 5);
    strategy.parameters:addInteger("SlowPeriod", "Slow period for AC", "", 34);
    strategy.parameters:addInteger("MAPeriod", "MA period for AC", "", 5);
    strategy.parameters:addBoolean("use_speed", "Use Speed", "", true);
    strategy.parameters:addBoolean("use_flat", "Use Flat", "", true);
    strategy.parameters:addInteger("count_bars", "Count Bars", "", 10);
    strategy.parameters:addInteger("min_width", "Min Width", "", 10);
    strategy.parameters:addInteger("max_width", "Max Width", "", 100);
    strategy.parameters:addInteger("iFL_MinExtremumsCount", "Min Extremums Count", "", 0);
end

local ac, use_speed, use_flat, count_bars, min_width, max_width, iFL_MinExtremumsCount;
function CreateEntryIndicators(source)
    iFL_MinExtremumsCount = instance.parameters.iFL_MinExtremumsCount;
    max_width = instance.parameters.max_width;
    min_width = instance.parameters.min_width;
    use_speed = instance.parameters.use_speed;
    use_flat = instance.parameters.use_flat;
    count_bars = instance.parameters.count_bars;
    if instance.parameters.use_ac then
        miAccelerator = core.indicators:create("AC", source, instance.parameters.FastPeriod, instance.parameters.SlowPeriod, instance.parameters.MAPeriod);
    end
end

function UpdateIndicators()
    if miAccelerator ~= nil then
        miAccelerator:update(core.UpdateLast);
    end
end

function IsFlat(source, period)
    local ll, hh = mathex.minmax(source, core.rangeTo(period, count_bars))
    local width = hh - ll;
    if width >= min_width and width <= max_width then
        local prev = 0;
        local count_low = 0;
        local count_high = 0;
        for i = period, period - count_bars, -1 do
            if prev >= 0 and source.low[i] < ll + width / 4 then
                prev = -1;
                count_low = count_low + 1;
            end
            if prev <= 0 and source.high[i] > hh - width / 4 then
                prev = 1;
                count_high = count_high + 1;
            end
        end
        return count_low >= iFL_MinExtremumsCount and count_high >= iFL_MinExtremumsCount, ll, hh, width;
    end
    return false, 0, 0, 0;
end

function IsEntryLong(source, period)
    if (miAccelerator ~= nil and miAccelerator.DATA[period] < 0) then
        return false;
    end
    if use_speed and (source.close[period] - source.open[period]) / source.open[period] < 0 then
        return false;
    end
    if use_flat then
        local isFlat, iFL_Low, iFL_High, iFL_Width = IsFlat(source, period);
        local pass = source.open[period] >= source.close[period - 1] and source.close[period - 1] > source.close[period - 2]
            and source.close[period - 2] > source.close[period - 3]
            and source.close[period] > iFL_Low and source.close[period] <= (iFL_Low + iFL_Width / 4)
            and isFlat;
        if not pass then
            return false;
        end
...
function IsEntryShort(source, period)
    ...
    if use_flat then
        ...
        local pass = source.open[period] <= source.close[period - 1] and source.close[period - 1] < source.close[period - 2]
            and source.close[period - 2] < source.close[period - 3]
            and source.close[period] < iFL_High and source.close[period] >= (iFL_High - iFL_Width / 4)
            and isFlat;
        if not pass then
            return false;
        end
