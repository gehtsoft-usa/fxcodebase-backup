...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=60597
...
function Init() --The strategy profile initialization
    strategy:name("Simple Ichimoku Strategy")
...
function Prepare(nameOnly)
    ...
    ICH = core.indicators:create("ICH", Source, Parameters[1], Parameters[2], Parameters[3])

    Short["TL"] = ICH:getStream(0)
    Short["KL"] = ICH:getStream(1)
    Short["CS"] = ICH:getStream(2)
    Short["SA"] = ICH:getStream(3)
    Short["SB"] = ICH:getStream(4)

    first = math.max(Short["TL"]:first(), Short["KL"]:first(), Short["SA"]:first()) + 2
end
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    local priceAbove = Source.close[period] > math.max(Short["SA"][period], Short["SB"][period])
    local priceBelow = Source.close[period] < math.min(Short["SA"][period], Short["SB"][period])

    -- only buy if we have a fast cross over slow and the price is above the moving averages.
    if (priceAbove and core.crossesOver(Short["TL"], Short["KL"], period)) then
        BUY()
    end

    if (priceBelow and core.crossesUnder(Short["TL"], Short["KL"], period)) then
        SELL()
    end

    if core.crossesUnder(Source.close, Short["TL"], period) then
        if AllowTrade then
    ...
    if core.crossesOver(Source.close, Short["TL"], period) then
        if AllowTrade then
