...
local STRATEGY_NAME = "Martingale Strategy";
local STRATEGY_VERSION = "9";
...
function CreateParameters() 
    strategy.parameters:addString("CustomID", "Custom ID", "", "Martingale");
    strategy.parameters:addInteger("rsi_period", "RSI Period", "", 14);
    strategy.parameters:addDouble("rsi_level", "RSI Level", "", 50);
    strategy.parameters:addInteger("failed_trades", "# Failed trades", "", 0);
    strategy.parameters:addGroup("Martingale")
    strategy.parameters:addDouble("martingale_lot_sizing_val", "Lot sizing", "", 2);
end
local failed_trades, martingale_lot_sizing_val, db, current_amount;
local wait_failed = true;
local CustomID;
local rsi;
local rsi_level;
function CreateIndicators(source) 
    failed_trades = instance.parameters.failed_trades;
    martingale_lot_sizing_val = instance.parameters.martingale_lot_sizing_val;
    current_amount = instance.parameters.amount;
...
function CountFailedTrades()
    ...
    local unsortedData = trading:JsonToObject(json);
    ...
    if last_failed ~= failed then
        last_failed = failed;
        core.host:trace("Failed trades: " .. last_failed);
    end
    return failed;
end
...
function CreateCustomActions()
    ...
    enterLongAction.IsPass = function (source, period, periodFromLast, data)
        ...
        if (failed_trades == 0) then
            return core.crossesOver(rsi.DATA, rsi_level, period);
        end
        return CountFailedTrades() >= failed_trades;
    end
    ...
    enterShortAction.IsPass = function (source, period, periodFromLast, data)
        ...
        if (failed_trades == 0) then
            return core.crossesUnder(rsi.DATA, rsi_level, period);
        end
        return CountFailedTrades() >= failed_trades;
    end
...
function CreatePositionStrategy(source, side, id)
    ...
    function position_strategy:Open(period)
        ...
        command:SetSide(self.Side)
            ...
            :SetAmount(math.ceil(current_amount))
...
function IsPositionLimitHit(side, side_limit)
    if ENFORCE_POSITION_CAP == true then
        ...
        if UseOwnPositionsOnly then
            sideIt:WhenCustomID(CustomID);
            allIt:WhenCustomID(CustomID)
        end
        ...
        if failed_trades == 0 then
            ...
            while (row ~= nil) do
                if row.OQTXT == CustomID then 
                    ...
                    dataRows[#dataRows + 1] = dataRow;
                end
                row = enum:next();
            end
            local json = trading:ObjectToJson(dataRows);
            db:put("closed_trades", json);
        else
            ...
            if close_trade then
                ...
                if closed_trade ~= nil then
                    if closed_trade.PL >= 0 then
                        ...
                        wait_failed = false;
                        current_amount = current_amount * instance.parameters.martingale_lot_sizing_val;
                    end
                end
            end
        end
...
function trading:Init(parameters, count)
    ...
    parameters:addString("allow_side", "Allow side", "", "buy");
