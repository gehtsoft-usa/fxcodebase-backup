...
function Init() --The strategy profile initialization
    strategy:name("MACD with SAR")
    strategy:description("Classic MACD with SAR")
    ...
    strategy.parameters:addDouble("Step", "Step", "Step", 0.02, 0.001, 1)
    strategy.parameters:addDouble("Max", "Max", "Max", 0.2, 0.001, 10)

    strategy.parameters:addGroup("Strategy Parameters")
    strategy.parameters:addInteger(
        "MACDOpenLevel",
        "MACD Open Level",
        "Minimal MACD level for open position.",
        3,
        1,
        1000
    )
    strategy.parameters:addInteger("MACDCloseLevel", "MACD Close Level", "MACD level for closing position", 2, 1, 100)
    strategy.parameters:addBoolean(
        "ConfirmTrend",
        "Cofirm Trend with MA",
        "Do we need to confirm MACD signal with Moving Average trend",
        false
    )
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if (MACD.SIGNAL:first() > (period - 1) or EMA.EMA:first() > (period - 1) or SAR.DATA:first() > (period - 1)) then
        return
    end
    ...
    local confirmSAR = nil
    if SAR.UP[period] > SAR.DN[period] then
        confirmSAR = -1
    elseif SAR.UP[period] < SAR.DN[period] then
        confirmSAR = 1
    end
    ...
    if (not haveTrades) then
        ...
        if
            (macd[period] < 0 and core.crossesOver(macd, signal, period) and math.abs(macd[period]) > openLevel and
                (not confirmTrend or ma[period] > ma[period - 1]) and
                confirmSAR == 1)
         then
            if ShowAlert then
        ...
        -- confirmTrend flag is 'true'
        if
            (macd[period] > 0 and core.crossesUnder(macd, signal, period) and macd[period] > openLevel and
                (not confirmTrend or ma[period] < ma[period - 1]) and
                confirmSAR == -1)
         then
            if ShowAlert then
        ...
        while true do
            ...
            if row.AccountID == Account and row.OfferID == Offer then
                ...
                elseif row.BS == "S" then
                    if
                        (macd[period] < 0 and core.crossesOver(macd, signal, period) and
                            math.abs(macd[period]) > closeLevel)
                     then
                        if ShowAlert then
                            ExtSignal(source, period, "BUY", SoundFile, Email, RecurrentSound)
                        end
