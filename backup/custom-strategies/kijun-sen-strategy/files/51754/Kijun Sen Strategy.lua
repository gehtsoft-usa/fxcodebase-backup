...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=30025
...
function Init() --The strategy profile initialization
    strategy:name("Kijun Sen Strategy")
    ...
    strategy.parameters:addInteger("Period", " Period", " Period", 26)
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    local ll, hh
    ll, hh = mathex.minmax(Source, period - Period + 1, period)
    local TL = (hh + ll) / 2
    ...
    if core.crossesOver(Source.close, TL, period) then
        if Direction then
            BUY()
        else
            SELL()
        end
    elseif core.crossesUnder(Source.close, TL, period) then
        if Direction then
            SELL()
        else
            BUY()
    ...
    if Exit then
        if Source.close[period] < TL then
            if Direction then
