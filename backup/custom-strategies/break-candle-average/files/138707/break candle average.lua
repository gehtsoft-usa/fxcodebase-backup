...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=70602
...
local ENFORCE_POSITION_CAP = true;
...
local STRATEGY_NAME = "Break Candle & Average";
...
function CreateCustomBreakeven(position_desc, result, period, periods_from_last)
    local controller = breakeven:CreateBreakeven()
        ...
        :SetWhen(position_desc.BreakevenWhen)
        :SetTo(position_desc.BreakevenTo);
    controller:SetPartialClose(position_desc.BreakevenCloseAmount);

    local controller = breakeven:CreateBreakeven()
        :SetRequestID(result.RequestID)
        :SetWhen(instance.parameters.TP2)
        :SetTo(position_desc.BreakevenTo);
    controller:SetPartialClose(instance.parameters.TP2Close);
    return true;
end
...
function CreateParameters()
    strategy.parameters:addInteger("mm_period", "MM Period", "", 7);
    AddAverages("mm_method", "MM Method", "MVA");
    strategy.parameters:addInteger("macd_SN", "MACD Short EMA", "", 12, 2, 1000);
    strategy.parameters:addInteger("macd_LN", "MACD Long EMA", "", 26, 2, 1000);
    strategy.parameters:addInteger("macd_IN", "MACD Signal", "", 9, 2, 1000);
end
local mm, pivot, macd;
function CreateEntryIndicators(source)
    mm = CreateAverages(instance.parameters.mm_period, instance.parameters.mm_method, source);
...
function CreateCustomActions()
    ...
    enterLongAction.IsPass = function (source, period, periodFromLast, data)
        return (source.close[period] - source.high[period - 1]) / source:pipSize() > 0.3
            and source.close[period] > mm.DATA[period] 
            and mm.DATA[period] > mm.DATA[period - 1]
    ...
    enterShortAction.IsPass = function (source, period, periodFromLast, data)
        return (source.low[period - 1] - source.close[period]) / source:pipSize() > 0.3
            and source.close[period] < mm.DATA[period]
            and mm.DATA[period] < mm.DATA[period - 1]
            and source.close[period] < pivot.P[period]
            and macd.HISTOGRAM[period] < 0;
...
function trading:AddPositionParameters(parameters, id, section_id)
    ...
    if CreateLimitParameters == nil or not CreateLimitParameters(parameters, id) then
        ...
        parameters:addDouble("breakeven_when" .. id, "Limit #1, in pips", "", 10);
        ...
        parameters:addDouble("TP2", "Limit #2, pips", "", 30);
        parameters:addDouble("TP2Close", "Partial close amount, %", "", 25);
        parameters:addDouble("limit" .. id, "Limit #3, pips", "In pips", 50);
    end
end
