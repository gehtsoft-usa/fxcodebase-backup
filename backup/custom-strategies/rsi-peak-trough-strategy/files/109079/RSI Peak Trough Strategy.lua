...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=64098
...
function Init() --The strategy profile initialization
    strategy:name("RSI Peak Trough Strategy")
...
local Period1, Period2, Indicator
...
function Prepare(nameOnly)
    ...
    first = math.max(Indicator.DATA:first() + Period2)
...
function EntryFunction(period, now)
    ...
    --[[
	 ...
	 Open Long
		cross(c,p) and C > C(at time when have peak rsi)
		Open Short
		cross(t,c) and C < C(at time when have trough rsi)
	 ]]
    local min, max, minpos, maxpos = mathex.minmax(Indicator.DATA, period - Period2 + 1, period)

    -- only buy if we have a fast cross over slow and the price is above the moving averages.
    if Source.close[period] > Source.close[maxpos] and Source.close[period - 1] <= Source.close[maxpos] then
        if Direction then
            BUY()
        else
            SELL()
        ...
        Return = true
    elseif Source.close[period] < Source.close[minpos] and Source.close[period - 1] >= Source.close[minpos] then
        if Direction then
            SELL()
        else
            BUY()
    ...
    while (not found) and (row ~= nil) do
        ...
        if
            row.AccountID == Account and row.OfferID == Offer and --and ExitInstrument))
                row.QTXT == CustomID and -- and ExitCustomID))
                (row.BS == BuySell or BuySell == nil)
         then
            exitTrade(row)
        end
