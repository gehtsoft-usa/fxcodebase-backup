...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=63892
...
function Init() --The strategy profile initialization
    strategy:name("SVE_RSI_InvFisher Strategy");
    ...
    strategy.parameters:addInteger("RSI_N", "Number of periods for RSI", "", 4, 2, 1000);
    strategy.parameters:addInteger("EMA_N", "Number of periods for EMA", "", 4, 2, 1000);
	
	strategy.parameters:addGroup("Levels");
    strategy.parameters:addInteger("L1", "Level1", "", 12, 1, 100);
    strategy.parameters:addInteger("L2", "Level2", "", 88, 1, 100); 
...
function CreateTradingParameters()
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "SRIFS");
...
function Prepare( nameOnly)
   ...
   -- Exit= instance.parameters.Exit;
	...
	assert(core.indicators:findIndicator("SVE_RSI_INVFISHER") ~= nil, "Please, download and install SVE_RSI_INVFISHER.LUA indicator"); 
    ...
    Indicator = core.indicators:create("SVE_RSI_INVFISHER", Source.close, RSI_N, EMA_N  ); 
...
now = core.host:execute("getServerTime");
   ...
   -- get only time
    ...
    if   Indicator.DATA[period]> L1 
	and Indicator.DATA[period-1]<= L1
	then	
          if Direction then
                BUY();
            else
    ...
    elseif   Indicator.DATA[period]< L2 
	and Indicator.DATA[period-1]>= L2
	then
            if Direction then
                SELL();
            else
