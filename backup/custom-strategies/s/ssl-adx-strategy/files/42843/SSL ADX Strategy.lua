...
function Init() --The strategy profile initialization
    strategy:name("SSL ADX Strategy");
    ...
    strategy.parameters:addInteger("SP", "SSL Period", "", 10);
...
function Prepare(nameOnly)
    ...
    Indicator[1] = core.indicators:create("ADX", Source, AP  );
	Short[1] = Indicator[1]:getStream(0);
	
	Indicator[2] = core.indicators:create("SSL", Source, SP  );
...
function ExtUpdate(id, source, period)  -- The method called every time when a new bid or ask price appears.
   ...
   -- get only time
    ...
    if period < first +1  then
		...
		if  EXIT then 
		            if      core.crossesUnder(source.close, Short[2]  ,period)
					then 			
								if   Direction then
