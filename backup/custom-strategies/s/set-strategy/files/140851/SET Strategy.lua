...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=64020
...
function Init() --The strategy profile initialization
    ...
    strategy.parameters:addInteger("Period", "TMA period", "", 200)
    ...
    strategy.parameters:addGroup("Schaff Trend Cycle Oscillator Calculation")
    strategy.parameters:addInteger("C", "Schaff cycle periods", "", 20, 2, 10000)
    strategy.parameters:addInteger("S", "Short periods", "", 23, 2, 10000)
    strategy.parameters:addInteger("L", "Long periods", "", 50, 2, 10000)

    strategy.parameters:addDouble("BL1", "Buy Level", "", 50)
    strategy.parameters:addDouble("SL1", "Sell Level", "", 50)

    strategy.parameters:addGroup("TDI Calculation")
    ...
    strategy.parameters:addDouble("BL2", "Buy Level", "", 50)
    strategy.parameters:addDouble("SL2", "Sell Level", "", 50)

    strategy.parameters:addBoolean("use_filter", "Use MVA Filter", "", true);
    strategy.parameters:addInteger("mva_filter_period", "MVA Filter Period", "", 200);
    AddAverages("mva_filter_method", "MVA Filter Method", "MVA");
...
local CustomID
local PositionCap
local Schaff, C, S, L, BL1, SL1
local TDI, RSI_N, VB_N, VB_W, RSI_P_N, RSI_P_M, TS_N, TS_M, BL2, SBL2

local OpenTime, CloseTime, ExitTime
local ValidInterval, UseMandatoryClosing
local ToTime, mva_filter
...
function Prepare(nameOnly)
    ...
    if instance.parameters.use_filter then
        mva_filter = CreateAverages(instance.parameters.mva_filter_period, instance.parameters.mva_filter_method, Source);
    end

    assert(core.indicators:findIndicator(Method) ~= nil, Method .. " indicator must be installed");
    MA = core.indicators:create(Method, Source.close, Period)
    Schaff = core.indicators:create("SCHTC", Source.close, C, S, L)
    TDI = core.indicators:create("TRADERSDYNAMICINDEX", Source.close, RSI_N, VB_N, VB_W, RSI_P_N, RSI_P_M, TS_N, TS_M)
    first = math.max(MA.DATA:first(), Schaff.DATA:first(), TDI.DATA:first())
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    -- update indicators.
    if mva_filter ~= nil then
        mva_filter:update(core.UpdateLast);
    end
    MA:update(core.UpdateLast)
    Schaff:update(core.UpdateLast)
    TDI:update(core.UpdateLast)

    if
        not MA.DATA:hasData(period) or not Schaff.DATA:hasData(period) or not TDI.MB:hasData(period) or
            not TDI.RSI_P:hasData(period) or
            not TDI.TS:hasData(period)
     then
        return
    end

    -- only buy if we have a fast cross over slow and the price is above the moving averages.
    if Schaff.DATA[period] > BL1 then
        core.host:trace("1");
    end
    if TDI.RSI_P[period - 1] <= TDI.MB[period - 1] then
        core.host:trace("2");
    end
    if TDI.TS[period] > BL2 then
        core.host:trace("3");
    end
    if
        Source.close[period] < MA.DATA[period] 
            and Schaff.DATA[period] > BL1 
            and TDI.RSI_P[period - 1] <= TDI.MB[period - 1] 
            and TDI.TS[period] > BL2
            and (mva_filter == nil or Source.close[period] > mva_filter.DATA[period])
     then
        if Direction then
            BUY()
        else
    ...
    elseif
        Source.close[period] > MA.DATA[period] 
            and Schaff.DATA[period] < SL1 
            and TDI.RSI_P[period - 1] >= TDI.MB[period - 1] 
            and TDI.MB[period] < SL2
            and (mva_filter == nil or Source.close[period] < mva_filter.DATA[period])
     then
        if Direction then
            SELL()
