...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=64646
...
function Init() --The strategy profile initialization
    strategy:name("Stochastics Bands Strategy")
...
local K, SD, D, MVAT_K, MVAT_D, Indicator
...
function Prepare(nameOnly)
    ...
    assert(
        core.indicators:findIndicator("STOCHASTICS BANDS") ~= nil,
        "Please, download and install STOCHASTICS BANDS.LUA indicator"
    )
    ...
    Indicator = core.indicators:create("STOCHASTICS BANDS", Source, true, true, true, K, SD, D, MVAT_K, MVAT_D)
...
function EntryFunction(now, period)
    ...
    -- buy: k>d and source .low (period-1) < os line and source.close(period)>os line

    --sell: k< d and source.high(period-1)>ob line and source.close(period)<ob line

    -- only buy if we have a fast cross over slow and the price is above the moving averages.
    if Indicator.K[period] > Indicator.D[period] and --and   Indicator.K[period-1]<= Indicator.D[period-1]
            Source.low[period] < Indicator.OS[period] and Source.close[period] > Indicator.OS[period] then
        if Direction then
            BUY()
        else
            SELL()
        ...
        Return = true
    elseif Indicator.K[period] < Indicator.D[period] and --and   Indicator.K[period-1]>= Indicator.D[period-1]
            Source.high[period] > Indicator.OB[period] and Source.close[period] < Indicator.OB[period] then
        if Direction then
            SELL()
        else
            BUY()
