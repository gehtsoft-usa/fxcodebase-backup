...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=66919
...
function Init()
    strategy:name("Semafor Stochastic HeikenAshiZone Strategy");
    strategy:description("v1 2018-11-13");
    ...
    strategy.parameters:addGroup("HEIKEN ASHI ZONE TRADE");
    strategy.parameters:addString("Source", "AO/AC Indicator Source Data", "Source Data" , "Price");
    strategy.parameters:addStringAlternative("Source", "Price", "Price" , "Price");
    strategy.parameters:addStringAlternative("Source", "HA", "HA" , "HA")
    strategy.parameters:addInteger("FM", "AC/AO fast moving average", "", 5);
    strategy.parameters:addInteger("SM", "AC/AO slow moving average", "", 35);
    strategy.parameters:addInteger("AM", "A/C moving average", "", 5);
    strategy.parameters:addInteger("MT" , "Zone Trade", "" , 1);
    strategy.parameters:addIntegerAlternative("MT", "Awesome oscillator(AO)", "ZONE" , 2);
    strategy.parameters:addIntegerAlternative("MT", "Acceleration/Deceleration (AC)", "" , 3);
    strategy.parameters:addIntegerAlternative("MT", "B.W. Zone AC + AO", "" , 1);
    strategy.parameters:addString("Method", "Presentation Method", "Method" , "Heiken Ashi");
    strategy.parameters:addStringAlternative("Method", "Heiken Ashi", "Heiken Ashi" , "Heiken Ashi");
    strategy.parameters:addStringAlternative("Method", "Price", "Price" , "Price")

    strategy.parameters:addGroup("3_LEVEL_ZZ_SEMAFOR");
    strategy.parameters:addInteger("Depth3", "Depth", "The minimum number of periods used to draw one ZigZag line.", 34);
    strategy.parameters:addInteger("Deviation3", "Deviation", "The maximum distance in pips by which the current high/low must be lower/higher than the previous one to return Backstep periods back to check if the current high/low is a new max/min.", 21);
    strategy.parameters:addInteger("Backstep3", "Backstep", "The number of periods used to define a new min/max if the current high/low is lower/higher than the previous one by Deviation or less.", 12);
    
    strategy.parameters:addGroup("CCI_HISTOGRAM");
    strategy.parameters:addInteger("CCI_Period", "CCI Period", "", 45);
    strategy.parameters:addString("Type", "Signal Type", "", "CCI Trigger Level");
    strategy.parameters:addStringAlternative("Type", "CCI Trigger Level", "", "CCI Trigger Level");
    strategy.parameters:addStringAlternative("Type", "CCI MA", "", "CCI MA");
    ...
    strategy.parameters:addGroup("STOCHASTIC RSI WITH ALERT");
    ...
    strategy.parameters:addInteger("D2", "%D Slowing Stochastic Periods", "", 3, 1, 200);
...
function Prepare(name_only)
    ...
    EnsureIndicatorInstalled("HEIKEN ASHI ZONE TRADE");
    local zz_profile = EnsureIndicatorInstalled("3_LEVEL_ZZ_SEMAFOR");
    EnsureIndicatorInstalled("CCI_HISTOGRAM");
    EnsureIndicatorInstalled("STOCHASTIC RSI WITH ALERT");

    ha = core.indicators:create("HEIKEN ASHI ZONE TRADE", trading_logic.MainSource,
        instance.parameters.Source,
        instance.parameters.FM,
        instance.parameters.SM,
        instance.parameters.AM,
        instance.parameters.MT,
        instance.parameters.Method);
    local p = zz_profile:parameters();
    p:setInteger("Depth3", instance.parameters.Depth3);
    p:setInteger("Deviation3", instance.parameters.Deviation3);
    p:setInteger("Backstep3", instance.parameters.Backstep3);
    p:setBoolean("Show3", true);
    zz = core.indicators:create("3_LEVEL_ZZ_SEMAFOR", trading_logic.MainSource, p);
    cci = core.indicators:create("CCI_HISTOGRAM", trading_logic.MainSource,
        instance.parameters.CCI_Period,
        instance.parameters.Type,
        instance.parameters.CCI_Trigger_Level,
        instance.parameters.Period,
        instance.parameters.cci_Method);
    srsi = core.indicators:create("STOCHASTIC RSI WITH ALERT", trading_logic.MainSource,
        instance.parameters.N,
        instance.parameters.K,
        instance.parameters.KS,
        instance.parameters.D2);
...
function UpdateIndicators()
    ...
    zz:update(core.UpdateLast);
    cci:update(core.UpdateLast);
    srsi:update(core.UpdateLast);
end

function GetLastSignal(indi, period)
    if indi == nil then
        return true;
    end
    local up = indi:getTextOutput(4);
    local down = indi:getTextOutput(5);
    for i = 0, up:size() - 1 do
        if up:hasData(period - i) then
            return 1, period - i;
        end
        if down:hasData(period - i) then
            return -1, period - i;
        end
    end
    return 0;
...
function IsBuyCondition(source, period)
    zz_signal, zz_period = GetLastSignal(zz, period);
    if zz_period == nil then
        return;
    end
    local price;
    if zz_signal == 1 then
        price = source.low[zz_period];
    else
        price = source.high[zz_period];
    end
    if not ha.low:hasData(period) or not cci.DATA:hasData(period) or not srsi.DATA:hasData(period) then
        return;
    end
    return price < ha.low[period] 
        and source.open[period] < source.close[period]
        and cci.DATA:colorI(period) == core.rgb(0, 255, 0)
        and srsi.DATA[period] > 50;
end
...
function DoBuy(source)
    if instance.parameters.allow_trade then
        ...
        if instance.parameters.stop_type == "pips" then
            stop_value = instance.parameters.stop;
...
function EntryFunction(source, period)
    ...
    if IsBuyCondition(source, period) and not IsBuyCondition(source, period - 1) then
        if Direction == "direct" then
            DoBuy(source);
        else
            DoSell(source);
    ...
    if IsSellCondition(source, period) and not IsSellCondition(source, period - 1) then
        if Direction == "direct" then
            DoSell(source);
        else
            DoBuy(source);
...
function trading:Init(parameters)
    ...
    if self.AddStopParameter then
        parameters:addString("stop_type", "Stop Order", "", "no");
        parameters:addStringAlternative("stop_type", "No stop", "", "no");
        parameters:addStringAlternative("stop_type", "In Pips", "", "pips");
        parameters:addStringAlternative("stop_type", "ATR", "", "atr");
        parameters:addDouble("stop", "Stop Value", "In pips or ATR period", 30);
        parameters:addDouble("atr_stop_mult", "ATR Stop Multiplicator", "", 2.0);
