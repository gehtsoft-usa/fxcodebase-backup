...
function CreateTradingParameters()
   ...
   -- strategy.parameters:addStringAlternative("ExitExecutionType", "End of Turn", "", "EndOfTurn");
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "SSDRSIS");
...
function EntryFunction( id, now,period)
	...
	--/////////////////////////////////////////////////////////////////    
     if LastEntry == Source1:serial(period)  
     --or not MA.DATA:hasData(period)
	 then
	 return;
	 end	
    ...
    local Return=false;
     ...
     if not RSI.marsioma:hasData(period2)
	 or  not SSD.D:hasData(period1)
	 then
	 return;
	 end
	...
	and SSD.K[period1] > OS1 
	and RSI.RSI[period2] > RSI.marsioma[period2]
	and RSI.RSI[period2] > OS2	
	...
	and  SSD.K[period1]<  OB1 
	and RSI.RSI[period2] < RSI.marsioma[period2]
	and RSI.RSI[period2] < OB2
	and ((SF and SSD.K[period1] > 50 )or not SF)	
	then
            if Direction then
                SELL();
