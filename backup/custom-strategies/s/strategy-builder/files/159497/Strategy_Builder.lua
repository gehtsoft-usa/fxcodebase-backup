...
function EVALUATION(i, p)
	...
	elseif instance.parameters:getString("IN" .. i) == "SAR_HEAT_MAP_EX" then
		STREAMS[0] = indicator[i].signalUp
		STREAMS[1] = indicator[i].signalDn

		if STREAMS[0][p] >= 3 then
			PLUS(i)
		elseif STREAMS[1][p] >= 3 then
			MINUS(i)
		end
	elseif instance.parameters:getString("IN" .. i) == "SSNONLAGMA" then
		STREAMS[0] = indicator[i]:getStream(0)
	...
	elseif instance.parameters:getString("IN" .. i) == "MTF MCP CCI HEAT MAP EX" then
		STREAMS[0] = indicator[i].signalUp
		STREAMS[1] = indicator[i].signalDn
...
function enter(BuySell)
	...
	if amount_type == "lots" then
        ...
        valuemap.Quantity = math.floor(used_equity / emr) * BaseSize;
    end
