...
function Prepare(nameOnly)
	...
	for i = 1, 13, 1 do
		...
		if instance.parameters:getBoolean("On" .. i) and (e1 - s1) <= (e2 - s2) then
			...
			Temp1 = core.indicators:create("SAR", source, Step[Number], Max[Number])
			...
			loading[Number] = true
			Indicator[Number] = core.indicators:create("SAR", source, Step[Number], Max[Number])
		end
	end
...
function Update(period)
	...
	for j = 1, Number, 1 do
		...
		if p ~= false then
			if Indicator[j].UP:hasData(p) or Indicator[j].DN:hasData(p) then
				if Indicator[j].UP:hasData(p) then
					countUp = countUp + 1;
				elseif Indicator[j].DN:hasData(p) then
					countDn = countDn + 1;
				end
			end
		end
