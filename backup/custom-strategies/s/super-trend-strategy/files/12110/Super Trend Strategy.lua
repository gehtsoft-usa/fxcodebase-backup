...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=3583
...
function Prepare(nameOnly)
    ...
    name =
        ...
        "(" ..
            instance.bid:name() ..
                "." ..
                    instance.parameters.TF .. ", " .. Parameters["Period"] .. ", " .. Parameters["Multiplier"] .. " )"
    instance:name(name)
    if nameOnly then
        return;
    end
    ...
    if UseMandatoryClosing then
        core.host:execute("setTimer", 200, math.max(ValidInterval / 2, 1))
    end
end
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if SuperTrend.DATA[period] < Source.close[period] and SuperTrend.DATA[period - 1] > Source.close[period - 1] then
        if Direction == "direct" then
            BUY()
        else
            SELL()
        end
    elseif SuperTrend.DATA[period] > Source.close[period] and SuperTrend.DATA[period - 1] < Source.close[period - 1] then
        if Direction == "direct" then
            SELL()
        else
            BUY()
...
function ExtAsyncOperationFinished(cookie, success, message)
    ...
    elseif cookie == 100 and not success then
        terminal:alertMessage(
            instance.bid:instrument(),
            instance.bid[instance.bid:size() - 1],
    ...
    elseif cookie == 101 and not success then
        terminal:alertMessage(
            instance.bid:instrument(),
            instance.bid[instance.bid:size() - 1],
