...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=70788
...
function CreateParameters() 
    ...
    for i = 1, count do
        strategy.parameters:addDouble("Dev" .. i, i .. ". Standard deviation","Number of Standard deviations", i / 10.0, 0.0001, 1000.0);
    end
end
...
function CreateEntryIndicators(source)
    ...
    for i = 1, count do
        bb[i] = core.indicators:create("BB", source, instance.parameters.N, instance.parameters:getDouble("Dev" .. i))
    end
end

function UpdateIndicators()
    sar:update(core.UpdateLast);
    for i, indi in ipairs(bb) do
...
function IsEntryLong(source, period)
    if period == 0 or not sar.DATA:hasData(period - 1) then
        return false;
    end
    ...
    for i, indi in ipairs(bb) do
        ...
        if core.crossesOver(source.close, indi.TL, period)
            or core.crossesOver(source.close, indi.BL, period)
            or core.crossesOver(source.close, indi.AL, period)
        then
            return true;
        end
    end
...
function IsEntryShort(source, period)
    ...
    for i, indi in ipairs(bb) do
        ...
        if core.crossesUnder(source.close, indi.TL, period)
            or core.crossesUnder(source.close, indi.BL, period)
            or core.crossesUnder(source.close, indi.AL, period)
        then
            return true;
        end
    end
