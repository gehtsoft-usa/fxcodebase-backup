...
local STRATEGY_NAME = "Support and resistances auto Strategy";
...
function CreateParameters() 
    strategy.parameters:addDouble("zone_fuzzfactor", "zone_fuzzfactor", "", 0.75);
    strategy.parameters:addDouble("fractal_fast_factor", "fractal_fast_factor", "", 3.0);
    strategy.parameters:addDouble("fractal_slow_factor", "fractal_slow_factor", "", 6.0);
end
function CreateStopParameters(params, id) 
    params:addBoolean("use_trailing" .. id, "Trailing stop order", "", false);
    params:addInteger("trailing" .. id, "Trailing in pips", "Use 1 for dynamic and 10 or greater for the fixed trailing", 1);
    return true;
end
...
local atr, indi;
local zones = {};
local zone_fuzzfactor, fractal_fast_factor, fractal_slow_factor;
function CreateIndicators(source) 
    zone_fuzzfactor = instance.parameters.zone_fuzzfactor;
    fractal_fast_factor = instance.parameters.fractal_fast_factor;
    fractal_slow_factor = instance.parameters.fractal_slow_factor;
    atr = core.indicators:create("ATR", source, 7);
    local profile = core.indicators:findIndicator("SUPPORT AND RESISTANCES AUTO");
    assert(profile ~= nil, "Please, download and install " .. "SUPPORT AND RESISTANCES AUTO" .. ".LUA indicator");
    local indicatorParams = profile:parameters();
    indicatorParams:setDouble("zone_fuzzfactor", instance.parameters.zone_fuzzfactor);
    indicatorParams:setDouble("fractal_fast_factor", instance.parameters.fractal_fast_factor);
    indicatorParams:setDouble("fractal_slow_factor", instance.parameters.fractal_slow_factor);
    indi = core.indicators:create("SUPPORT AND RESISTANCES AUTO", source, indicatorParams)

    SlowDnPts = indi:getTextOutput(0);
    SlowUpPts = indi:getTextOutput(1);
    FastDnPts = indi:getTextOutput(2);
    FastUpPts = indi:getTextOutput(3);
end

function UpdateIndicators()
    local source = trading_logic.MainSource;
...
function FindEntryRate(side, period)
    local source = trading_logic.MainSource;
    for _, zone in ipairs(zones) do
        if zone.strength == ZONE_PROVEN or zone.strength == ZONE_VERIFIED then
            if side == "B" and core.crossesOver(source.close, zone.lo, period) then
                return zone.lo;
            end
            if side == "S" and core.crossesUnder(source.close, zone.hi, period) then
                return zone.hi;
            end
        end
    end
...
function SetCustomStop(position_desc, command, period)
    local entry = FindEntryRate(position_desc.Side, period);
    local stop;
    if position_desc.Side == "B" then
        for _, zone in ipairs(zones) do
            if zone.strength == ZONE_PROVEN or zone.strength == ZONE_VERIFIED and zone.hi < entry then
                if stop == nil or stop < zone.hi then
                    stop = zone.hi;
                end
            end
        end
        ...
        for _, zone in ipairs(zones) do
            if zone.strength == ZONE_PROVEN or zone.strength == ZONE_VERIFIED and zone.lo > entry then
                if stop == nil or stop > zone.lo then
                    stop = zone.lo;
                end
            end
        end
    ...
    if (stop ~= nil) then
        command:SetStop(stop, position_desc.Trailing);
    end
    return true; 
end
...
function SetCustomLimit(position_desc, command, period) 
    local entry = FindEntryRate(position_desc.Side, period);
    local limit;
    if position_desc.Side == "S" then
        for _, zone in ipairs(zones) do
            if zone.strength == ZONE_PROVEN or zone.strength == ZONE_VERIFIED and zone.hi < entry then
                if limit == nil or limit < zone.hi then
                    limit = zone.hi;
                end
            end
        end
        ...
        for _, zone in ipairs(zones) do
            if zone.strength == ZONE_PROVEN or zone.strength == ZONE_VERIFIED and zone.lo > entry then
                if limit == nil or limit > zone.lo then
                    limit = zone.lo;
                end
            end
        end
...
function CreateCustomActions()
    ...
    enterLongAction.IsPass = function (source, period, periodFromLast, data)
        for _, zone in ipairs(zones) do
            if zone.strength == ZONE_PROVEN or zone.strength == ZONE_VERIFIED then
                if core.crossesOver(source.close, zone.lo, period) then
                    return true;
                end
            end
        end
    ...
    enterShortAction.IsPass = function (source, period, periodFromLast, data)
        for _, zone in ipairs(zones) do
            if zone.strength == ZONE_PROVEN or zone.strength == ZONE_VERIFIED then
                if core.crossesUnder(source.close, zone.hi, period) then
                    return true;
                end
            end
        end
