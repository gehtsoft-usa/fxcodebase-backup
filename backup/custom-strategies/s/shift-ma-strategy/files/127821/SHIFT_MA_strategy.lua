...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=68768
...
local STRATEGY_NAME = "SHIFT MA Strategy";
...
function CreateParameters()
    ...
    strategy.parameters:addInteger("SX", "Shift in periods", "Postive is future, negative is past", 0);
    strategy.parameters:addInteger("SY", "Shift in points", "", 0);
end

local indi;
function CreateIndicators(source) 
    local profile = core.indicators:findIndicator("SHIFT_MA");
    assert(profile ~= nil, "Please, download and install " .. "SHIFT_MA" .. ".LUA indicator");
    local indicatorParams = profile:parameters();
    indicatorParams:setInteger("N", instance.parameters.N);
    indicatorParams:setInteger("SX", instance.parameters.SX);
    indicatorParams:setInteger("SY", instance.parameters.SY);
    indicatorParams:setString("MA", instance.parameters.MA);
    indi = core.indicators:create("SHIFT_MA", source, indicatorParams);
end
...
function CreateCustomActions()
    ...
    enterLongAction.IsPass = function (source, period, periodFromLast, data)
        return indi.DATA[period] > indi.DATA[period - 1];
    end
