...
function Init() --The strategy profile initialization
    strategy:name("SuperTrend Band Strategy")
    ...
    strategy.parameters:addInteger("Step", "Step (Pips)", "", 2);
...
function Prepare(nameOnly)
    ...
    assert(
        core.indicators:findIndicator("SUPERTREND BAND") ~= nil,
        "Please, download and install SUPERTREND BAND.LUA strategy"
    )
    ...
    strategyParams:setString("frame", TF);
    ...
    strategyParams:setInteger("Step", instance.parameters.Step);
    strategyParams:setDate("from", core.now() - 5000);
    ...
    strategy = core.indicators:create("SUPERTREND BAND", renko:getCandleOutput(0), N, M)

    first = strategy.DATA:first()
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    local candles = renko:getCandleOutput(0);
    if period < 0 or candles.open:size() == 0 then
        return
    end
    ...
    -- update indicators.
    strategy:update(core.UpdateLast)
...
function ExitFunction(id, now, period)
    ...
    --/////////////////////////////////////////////////////////////////
    local current_serial = renko:getCandleOutput(0):date(NOW);
    if LastExit == current_serial then
        --or not MA.DATA:hasData(period)
        return
    end
    --/////////////////////////////////////////////////////////////////////

    if Source.close[period] < strategy.UP[NOW - shift] then
        if Direction then
        ...
        LastExit = current_serial
    end
    if Source.close[period] > strategy.DN[NOW - shift] then
        if Direction then
            if haveTrades("S") then
                exitSpecific("S")
                Signal("Close Short")
...
function EntryFunction(id, now, period)
    ...
    --/////////////////////////////////////////////////////////////////
    local current_serial = renko:getCandleOutput(0):date(NOW);
    if LastEntry == current_serial then
        --or not MA.DATA:hasData(period)
        return
    end
    --/////////////////////////////////////////////////////////////////
    ...
    if Source.close[period] > strategy.DN[NOW - shift] then
        ...
        LastEntry = current_serial
        Return = true
    elseif Source.close[period] < strategy.UP[NOW - shift] then
        if Direction then
            SELL()
        else
            BUY()
