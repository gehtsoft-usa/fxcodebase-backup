...
function CreateTradingParameters()
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "STBS");
...
function Prepare( nameOnly)
    ...
    PrepareTrading();
	 ...
	 assert(core.indicators:findIndicator("SUPERTREND BAND") ~= nil, "Please, download and install SUPERTREND BAND.LUA indicator");          
    ...
    Indicator = core.indicators:create("SUPERTREND BAND", Source,N, M  );
...
function ExitFunction(id, now,period)
	...
	--/////////////////////////////////////////////////////////////////////
		            ...
		            if  Source.close[period]<Indicator.UP[period] then
					  ...
					  if   Source.close[period]> Indicator.DN[period] then
...
function EntryFunction( id, now,period)
    ...
    if  Source.close[period]> Indicator.DN[period]
	then	
          if Direction then
                BUY();
            else
		...
		Return=true;
    elseif  Source.close[period]<Indicator.UP[period]
		then
            if Direction then
                SELL();
            else
