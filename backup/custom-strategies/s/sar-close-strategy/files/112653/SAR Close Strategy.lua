...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=64698
...
function Init() --The strategy profile initialization
    strategy:name("SAR Close Strategy")
...
function ExitFunction(now, period)
    ...
    if SAR:getStream(0):hasData(period) and not SAR:getStream(0):hasData(period - 1) then
        if Direction then
    ...
    if SAR:getStream(1):hasData(period) and not SAR:getStream(1):hasData(period - 1) then
        if Direction then
            if haveTrades("S") then
                exitSpecific("S")
                Signal("Close Short")
...
function tradesCount(BuySell)
    ...
    row = enum:next()
    while row ~= nil do
        if row.AccountID == Account and row.OfferID == Offer and --and row.QTXT == CustomID
