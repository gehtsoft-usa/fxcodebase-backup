...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=14741
...
function Init() --The strategy profile initialization
    strategy:name("Schaff Trend Cycle Oscillator Strategy")
    ...
    strategy.parameters:addInteger("C", "Schaff cycle periods", "", 10, 2, 10000)
    strategy.parameters:addInteger("S", "Short periods", "", 23, 2, 10000)
    strategy.parameters:addInteger("L", "Long periods", "", 50, 2, 10000)

    strategy.parameters:addDouble("OB", "Overbought Level", "", 70, 0, 100)
    strategy.parameters:addDouble("OS", "Oversold Level", "", 30, 0, 100)
...
function Prepare(nameOnly)
    ...
    Indicator[1] = core.indicators:create("SCHTC", Source[Price], C, S, L)
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if core.crossesOver(Short[1], OS, period) then
        if Direction then
            BUY()
        else
            SELL()
        end
    elseif core.crossesUnder(Short[1], OB, period) then
        if Direction then
            SELL()
        else
            BUY()
