...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=5436
...
function Init() --The strategy profile initialization
    strategy:name("Silver trend strategy")
    strategy:description("Silver trend strategy")
    strategy:setTag(
    ...
    strategy.parameters:addGroup("Silver trend parameters")
    strategy.parameters:addInteger("Border", "Border", "", 30)
...
function Prepare(nameOnly)
            ...
            instance.bid:name() ..
                "." ..
                    instance.parameters.TF ..
                        "," .. "STS(" .. instance.parameters.Border .. ", " .. instance.parameters.Period .. "))"
    instance:name(name)
    if nameOnly then
        return
    end
    ...
    assert(
        core.indicators:findIndicator("SILVER_TREND_SIG") ~= nil,
        "Please, download and install SILVER_TREND_SIG.LUA indicator"
    )

    Source = ExtSubscribe(2, nil, instance.parameters.TF, true, "bar")
    STS = core.indicators:create("SILVER_TREND_SIG", Source, instance.parameters.Border, instance.parameters.Period)
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    STS:update(core.UpdateLast)

    -- Check that we have enough data
    if (STS.DATA:first() > (period - 1)) then
        return
    end
    ...
    if STS.BuffUP[period] > source.high[period] then
    ...
    if STS.BuffDN[period] > source.low[period] / 2 then
        if instance.parameters.TypeSignal == "direct" then
            MustB = true
        else
            MustS = true
