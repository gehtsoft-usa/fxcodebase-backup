...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=70760
--+------------------------------------------------------------------+
...
function Init()
    strategy:name("Stinger 66 Strategy");
...
function DoClose(side)
    ...
    trading:FindTrade()
        ...
        :Do(function (trade)
            if trade.PL > 0 and trade.BS == side then
                toClose = toClose + trade.PL * trade.Lot;
                trading:Close(trade);
            end
        end)
    if toClose > 0 then
        trading:FindTrade()
            :WhenInstrument(main_source:instrument())
            :Do(function (trade)
                if trade.PL < 0 and toClose > 0 and trade.BS == side then
                    local amount = math.abs(trade.PL) * trade.Lot;
                    if amount < toClose then
                        toClose = toClose - amount;
                        trading:Close(trade);
                    else
                        trading:PartialClose(trade, math.floor(trade.Lot * (amount / toClose) + 0.5));
                        toClose = 0;
                    end
                end
            end)
