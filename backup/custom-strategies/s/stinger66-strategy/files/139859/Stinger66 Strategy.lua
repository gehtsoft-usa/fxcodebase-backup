...
function ExtUpdate(id, source, period)
    ...
    if toMove > 0 then
        trading:FindTrade()
            :WhenInstrument(source:instrument())
            :Do(function (trade)
                if trade.PL < 0 and toMove > 0 then
                    local stopDistance = math.abs(trade.Stop - trade.Open) / source:pipSize();
                    local newStopDistance = math.min(stopDistance, toMove);
                    if trade.BS == "B" then
                        trading:MoveStop(trade, trade.Stop + newStopDistance * source:pipSize())
                    else
                        trading:MoveStop(trade, trade.Stop - newStopDistance * source:pipSize())
                    end
                    
                    toMove = toMove + newStopDistance;
                end
            end)
    end
end
