...
function Init() --The strategy profile initialization
    strategy:name("Stochastic Fast Strategy With Filters")
    ...
    strategy.parameters:addInteger("D", "%D Period", "", 5, 2, 1000)
...
function Prepare(nameOnly)
    ...
    local PARAMETARS = instance.parameters.K .. "," .. instance.parameters.D .. "," .. OS .. "," .. OB
    ...
    local iprofile = core.indicators:findIndicator("SFK")
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if instance.parameters.L == "K" then
        if core.crossesOver(Line1, OS, period) and (not MA or (MA and Source.close[period] > ma.DATA[period])) then
            BUY()
            ONE = Source:serial(period)
        elseif core.crossesUnder(Line1, OB, period) and (not MA or (MA and Source.close[period] < ma.DATA[period])) then
            SELL()
            ONE = Source:serial(period)
        end
    end
    ...
    if instance.parameters.L == "D" then
        if core.crossesOver(Line2, OS, period) and (not MA or (MA and Source.close[period] > ma.DATA[period])) then
            BUY()
            ONE = Source:serial(period)
        elseif core.crossesUnder(Line2, OB, period) and (not MA or (MA and Source.close[period] < ma.DATA[period])) then
            SELL()
            ONE = Source:serial(period)
        end
    end
    ...
    if (instance.parameters.L == "C") then
        if core.crossesOver(Line1, Line2, period) and (not MA or (MA and Source.close[period] > ma.DATA[period])) then
            BUY()
            ONE = Source:serial(period)
        elseif core.crossesUnder(Line1, Line2, period) and (not MA or (MA and Source.close[period] < ma.DATA[period])) then
            SELL()
            ONE = Source:serial(period)
        end
    end
