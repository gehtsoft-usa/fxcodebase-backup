...
--http://fxcodebase.com/code/viewtopic.php?f=31&t=70140
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if instance.parameters.L == "K" then
        if core.crossesOver(Line1, OS, period) and IsRSIBuy() then
            BUY()
            ONE = Source:serial(period)
        elseif core.crossesUnder(Line1, OB, period) and IsRSISell() then
            SELL()
            ONE = Source:serial(period)
        end
    end
    ...
    if instance.parameters.L == "D" then
        if core.crossesOver(Line2, OS, period) and IsRSIBuy() then
            BUY()
            ONE = Source:serial(period)
        elseif core.crossesUnder(Line2, OB, period) and IsRSISell() then
            SELL()
            ONE = Source:serial(period)
        end
    end
    ...
    if (instance.parameters.L == "C") then
        if core.crossesOver(Line1, Line2, period) and IsRSIBuy() then
            BUY()
            ONE = Source:serial(period)
        elseif core.crossesUnder(Line1, Line2, period) and IsRSISell() then
            SELL()
            ONE = Source:serial(period)
        end
    end
    ...
    if (instance.parameters.L == "B") then
        if core.crossesOver(Line1, Line2, period) and Line1[period - 1] < OS and Line2[period] < OS and IsRSIBuy() then
            BUY()
            ONE = Source:serial(period)
        elseif core.crossesUnder(Line1, Line2, period) and Line1[period - 1] > OB and Line2[period] > OB and IsRSISell() then
            SELL()
            ONE = Source:serial(period)
        end
    end
