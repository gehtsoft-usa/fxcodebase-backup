...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=66843
-- TASC Nov 2018
...
function Init()
    strategy:name("Stiffness Strategy");
    strategy:description("v1 2018-10-17");

    strategy.parameters:addInteger("BuyThreshold", "Buy Threshold", "", 90);
    strategy.parameters:addInteger("SellThreshold", "Sell Threshold", "", 50);
    strategy.parameters:addInteger("ExitAfterBars", "Exit After Bars", "", 84);

    strategy.parameters:addGroup("MA Calculation"); 
    strategy.parameters:addInteger("Period1", "Period", "", 100, 1, 2000);
    strategy.parameters:addString("Price1", "Power Price", "", "close");
    ...
    strategy.parameters:addInteger("Period3", "Summation Period", "", 60, 1, 2000);
    strategy.parameters:addGroup("Signal MA Calculation"); 
    strategy.parameters:addInteger("Period2", "Period", "", 3, 1, 2000);
...
function Prepare(name_only)
    ...
    BuyThreshold = instance.parameters.BuyThreshold;
    SellThreshold = instance.parameters.SellThreshold;
    ExitAfterBars = instance.parameters.ExitAfterBars;
    ...
    MarketTrendAvg = core.indicators:create(instance.parameters.Method1, trading_logic.MainSource, instance.parameters.Period1);

    assert(core.indicators:findIndicator("STIFFNESS INDICATOR") ~= nil, "Please, download and install STIFFNESS INDICATOR.LUA indicator");
    indicator = core.indicators:create(
        "STIFFNESS INDICATOR", 
        trading_logic.MainSource,
...
function IsBuyCondition(source, period)
    if not MarketTrendAvg.DATA:hasData(period - 2) or not indicator.Signal:hasData(period - 1) then
        return;
    end
    local MarketTrendOK = MarketTrendAvg.DATA[period] >= MarketTrendAvg.DATA[period - 2];
    return core.crossesOver(indicator.Signal, BuyThreshold, period) and  MarketTrendOK;
end
...
function IsSellCondition(source, period)
    ...
    local MarketTrendOK = MarketTrendAvg.DATA[period] < MarketTrendAvg.DATA[period - 2];
    return core.crossesUnder(indicator.Signal, SellThreshold, period) and  MarketTrendOK;
end

local last_serial;
function EntryFunction(source, period)
    MarketTrendAvg:update(core.UpdateLast);
    indicator:update(core.UpdateLast);
    if last_serial == source.close:serial(period) then
        return;
    end
    if (period - ExitAfterBars > 0) then
        local minDate = source:date(period - ExitAfterBars);
        trading:FindTrade()
            :WhenCustomID(custom_id)
            :Do(function (trade) 
                if (trade.Time < minDate) then
                    trading:Close(trade);
                end
            end);
    end
