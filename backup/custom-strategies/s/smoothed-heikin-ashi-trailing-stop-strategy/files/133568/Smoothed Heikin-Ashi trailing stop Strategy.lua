...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=69696
-- More information about this indicator can be found at:
--http://fxcodebase.com/code/viewtopic.php?f=31&t=69696
...
function Init()
    strategy:name("Smoothed Heikin-Ashi trailing stop Strategy");
    strategy:description("");
    strategy:setTag("Version", "1.0");
    strategy:setTag("strategy_type", "Money management");
    
    strategy.parameters:addGroup("Smoother HA");
    strategy.parameters:addInteger("shift", "Period shift", "", 1);
    ...
    strategy.parameters:addString("price_type", "Price type", "", "highlow");
    strategy.parameters:addStringAlternative("price_type", "High-low", "", "highlow");
    strategy.parameters:addStringAlternative("price_type", "Open", "", "open");
...
function Prepare(nameOnly)
    ...
    price_type = instance.parameters.price_type;
    ...
    Source = ExtSubscribe(2, nil, TF, instance.parameters.type, "bar");
    local profile = core.indicators:findIndicator("HEIKEN_ASHI_SMOOTHED");
    assert(profile ~= nil, "Please, download and install " .. "HEIKEN_ASHI_SMOOTHED" .. ".LUA indicator");
    ha = core.indicators:create("HEIKEN_ASHI_SMOOTHED", Source, instance.parameters.Period, instance.parameters.Method)
    
    if ExecutionType == "Live" then
        TickSource = ExtSubscribe(1, nil, "t1", instance.parameters.type, "close");
    end
end
...
function DoMoveStop(trade)
    ...
    local newStop;
    if trade.BS == "B" then
        if price_type == "highlow" then
            newStop = ha.low[NOW - shift];
        else
            newStop = ha.open[NOW - shift];
        end
    else
        if price_type == "highlow" then
            newStop = ha.high[NOW - shift];
        else
            newStop = ha.open[NOW - shift];
        end
    end
    ...
    if stop == nil or stop.Rate ~= newStop then
        core.host:trace("moving stop to " .. newStop)
