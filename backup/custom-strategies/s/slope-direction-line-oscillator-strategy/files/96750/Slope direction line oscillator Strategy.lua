...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=61382
...
function Init() --The strategy profile initialization
    strategy:name("Slope direction line oscillator Strategy")
    ...
    strategy.parameters:addStringAlternative("Method1", "TMA", "", "TMA")
    ...
    strategy.parameters:addStringAlternative("Method2", "TMA", "", "TMA")
...
function CreateTradingParameters()
    ...
    strategy.parameters:addBoolean("Exit", "Exit On Neutral", "", true)
...
function Prepare(nameOnly)
    ...
    assert(
        core.indicators:findIndicator("SLOPE DIRECTION LINE OSCILLATOR") ~= nil,
        "Please, download and install SDLO.LUA indicator"
    )
    assert(
        core.indicators:findIndicator("SLOPE_DIRECTION_LINE") ~= nil,
        "Please, download and install SLOPE_DIRECTION_LINE.LUA indicator"
    )
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if SDLO.DATA:colorI(period) == core.rgb(255, 0, 0) and SDLO.DATA:colorI(period - 1) ~= core.rgb(255, 0, 0) then
        Flag = "Buy"
        BUY()
        ONE = Source:serial(period)
    end

    if SDLO.DATA:colorI(period) == core.rgb(0, 255, 0) and SDLO.DATA:colorI(period - 1) ~= core.rgb(0, 255, 0) then
        SELL()
        ONE = Source:serial(period)
    end

    if
        SDLO.DATA:colorI(period) == core.rgb(128, 128, 128) and SDLO.DATA:colorI(period - 1) ~= core.rgb(128, 128, 128) and
            Exit
     then
