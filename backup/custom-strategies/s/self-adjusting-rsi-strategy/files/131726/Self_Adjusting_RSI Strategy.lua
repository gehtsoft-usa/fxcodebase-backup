...
--http://fxcodebase.com/code/viewtopic.php?f=31&t=69495
...
local STRATEGY_NAME = "Self Adjusting RSI Strategy";
function CreateParameters() 
    strategy.parameters:addInteger("Period", "Period", "", 12);
    strategy.parameters:addDouble("Deviation", "Deviation", "", 2);
    strategy.parameters:addBoolean("Smooth", "Smooth", "", true);
end

local indi;
function CreateEntryIndicators(source)
    local profile = core.indicators:findIndicator("SELF_ADJUSTING_RSI");
    assert(profile ~= nil, "Please, download and install " .. "SELF_ADJUSTING_RSI" .. ".LUA indicator");
    local indicatorParams = profile:parameters();
    indi = core.indicators:create("SELF_ADJUSTING_RSI", source, instance.parameters.Period, instance.parameters.Deviation, instance.parameters.Smooth);
end
...
function IsEntryLong(source, period)
    ...
    return core.crossesUnder(indi.RSI, indi.Oversold, period);
end
function IsEntryShort(source, period)
    if not indi.DATA:hasData(period - 1) then
        return;
    end
    return core.crossesOver(indi.RSI, indi.Overbought, period);
end
