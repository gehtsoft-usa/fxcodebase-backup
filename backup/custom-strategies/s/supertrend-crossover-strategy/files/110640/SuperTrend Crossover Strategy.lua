...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=64320
...
function Init() --The strategy profile initialization
    strategy:name("SuperTrend Crossover Strategy")
    ...
    strategy.parameters:addGroup("1. ST Calculation")
    strategy.parameters:addInteger("N1", "Number of periods", "", 10)
    strategy.parameters:addDouble("M1", "Multiplier", "", 1.5)

    strategy.parameters:addGroup("2. ST Calculation")
    strategy.parameters:addInteger("N2", "Number of periods", "", 20)
...
function Prepare(nameOnly)
    ...
    ST1 = core.indicators:create("ST", Source, N1, M1)
    ST2 = core.indicators:create("ST", Source, N2, M2)
    first = math.max(ST1.DATA:first(), ST2.DATA:first())
...
function EntryFunction(period, now)
    ...
    if ST1.DATA[period] > ST2.DATA[period] and ST1.DATA[period - 1] <= ST2.DATA[period - 1] then
        if Direction then
            BUY()
        else
            SELL()
        ...
        Return = true
    elseif ST1.DATA[period] < ST2.DATA[period] and ST1.DATA[period - 1] >= ST2.DATA[period - 1] then
        if Direction then
            SELL()
        else
            BUY()
