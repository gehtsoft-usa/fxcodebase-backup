...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=10554
...
function Init() --The strategy profile initialization
    strategy:name("Sensitive strategy")
    strategy:description("Sensitive strategy")
    strategy:setTag("Version", "2")
    strategy:setTag("NonOptimizableParameters", "Email,SendEmail,SoundFile,RecurrentSound,PlaySound, ShowAlert")

    strategy.parameters:addGroup("Parameters")
    strategy.parameters:addInteger("Period", "Period", "", 150)
    strategy.parameters:addInteger("Sensitive", "Sensitive", "", 7)
...
function Prepare(nameOnly)
    ...
    assert(core.indicators:findIndicator("SENSITIVE") ~= nil, "Please, download and install SENSITIVE.LUA indicator")

    Source = ExtSubscribe(2, nil, instance.parameters.TF, true, "bar")
    Sens = core.indicators:create("SENSITIVE", Source, instance.parameters.Period, instance.parameters.Sensitive)
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    Sens:update(core.UpdateLast)

    -- Check that we have enough data
    if (Sens.DATA:first() > (period - 1)) then
        return
    end
    ...
    if Sens.DATA[period - 1] <= 0 and Sens.DATA[period] > 0 then
