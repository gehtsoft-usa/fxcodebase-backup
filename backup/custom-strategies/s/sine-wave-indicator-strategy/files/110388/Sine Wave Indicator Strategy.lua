...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=64272
...
function Init() --The strategy profile initialization
    strategy:name("Sine Wave Indicator Strategy")
...
function Prepare(nameOnly)
    ...
    assert(core.indicators:findIndicator("JESINEWAVE") ~= nil, "Please, download and install JESINEWAVE.LUA indicator")
    ...
    Indicator = core.indicators:create("JESINEWAVE", Source, alpha)

    first = Indicator.LS:first()
...
function EntryFunction(period, now)
    ...
    if
        Indicator.BS[period] > Indicator.LS[period] and Indicator.BS[period - 1] <= Indicator.LS[period - 1] and
            Indicator.BS[period] < BL
     then
        if Direction then
            BUY()
        else
