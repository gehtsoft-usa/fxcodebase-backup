-- Available @ https://fxcodebase.com/code/viewtopic.php?f=31&t=73538
...
function Init() --The strategy profile initialization
    strategy:name("Support and Resistance Levels with Breaks Strategy");
    ...
    strategy.parameters:addInteger("p1", "leftBars", "", 15, 1, 2000);
    strategy.parameters:addInteger("p2", "rightBars", "", 15, 1, 2000);	
    strategy.parameters:addInteger("p3", "volumeThresh", "", 20, 1, 2000); 
...
function CreateTradingParameters()
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "SRLBS");
...
function Prepare( nameOnly)
    ...
    Indicator = core.indicators:create("SUPPORT AND RESISTANCE LEVELS WITH BREAKS", Source, false , instance.parameters.p1 , instance.parameters.p2 , instance.parameters.p2 ); 
...
function EntryFunction( id, now,period)
    ...
    --local LL=Low(period);
   -- local LH=High(period);	

    --if 	LL== nil or LH== nil then
	--return;
	--end
	
    -- only buy if we have a fast cross over slow and the price is above the moving averages.
    if   Source.close[period]>  Indicator.Top[period-instance.parameters.p2]
	and  Source.close[period-1] <=   Indicator.Top[period-instance.parameters.p2]
	then	
          if Direction then
                BUY();
            else
		...
		Return=true;
    elseif Source.close[period]< Indicator.Bottom[period-instance.parameters.p2]
	and  Source.close[period-1]>=  Indicator.Top[period-instance.parameters.p2]
		then
            if Direction then
                SELL();
...
function Low(period)
   ...
   for i= period, first, -1 do
	   if Indicator.Bottom[i]  ~= nil then
	   ReturnValue=Indicator.Bottom[i];
	   break;   
	   end
   end
...
function High(period)
   ...
   for i= period, first, -1 do
	   if Indicator.Top[i] ~= nil then
	   ReturnValue=Indicator.Top[i];
	   break;   
	   end
   end
