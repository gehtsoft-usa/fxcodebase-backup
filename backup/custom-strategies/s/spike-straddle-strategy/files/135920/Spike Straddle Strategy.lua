...
local STRATEGY_NAME = "Spike Straddle Strategy";
function CreateParameters() 
    strategy.parameters:addInteger("atr_period", "ATR Period", "", 11);
    strategy.parameters:addInteger("CandleSize", "Candle Size, %", "", 150)
    strategy.parameters:addInteger("StradleSize", "Stradle Size, %", "", 10);
end

local atr, CandleSize, StradleSize;
...
function IsEntry(source, period)
    ...
    return math.abs(source.open[period] - source.close[period]) > atr.DATA[period] * CandleSize / 100.0;
end
...
function Init()
    ...
    strategy.parameters:addDouble("stop_pips", "Stop, %", "", 50);
    ...
    strategy.parameters:addDouble("limit_pips", "Limit, %", "", 50);
...
local base_size, offer_id, Account, Amount, amount_type, AllowTrade, close_on_opposite, custom_id;
...
function OpenTrade(side)
    ...
    if side == "B" then
        valuemap.Rate = source.close[NOW] + atr.DATA[NOW] * StradleSize / 100.0;
    else
        valuemap.Rate = source.close[NOW] - atr.DATA[NOW] * StradleSize / 100.0;
    end
    ...
    if use_stop then
        ...
        if side == "B" then
            valuemap.PegPriceOffsetPipsStop = (-atr.DATA[NOW] / instance.bid:pipSize()) * stop_pips / 100.0;
        else
            valuemap.PegPriceOffsetPipsStop = (atr.DATA[NOW] / instance.bid:pipSize()) * stop_pips / 100.0;
        end
    ...
    if use_limit then
        ...
        if side == "B" then
            valuemap.PegPriceOffsetPipsLimit = (atr.DATA[NOW] / instance.bid:pipSize()) * limit_pips / 100.0;
        else
            valuemap.PegPriceOffsetPipsLimit = (-atr.DATA[NOW] / instance.bid:pipSize()) * limit_pips / 100.0;
        end
    end
