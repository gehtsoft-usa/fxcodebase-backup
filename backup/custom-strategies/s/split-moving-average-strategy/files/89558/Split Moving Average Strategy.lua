...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=59520
...
function Init() --The strategy profile initialization
    strategy:name("Split Moving Average Strategy")
    ...
    strategy.parameters:addString("iType", "Indicator Used", "", "SPLIT MOVING AVERAGE")
    strategy.parameters:addStringAlternative("iType", "Split Moving Average", "", "SPLIT MOVING AVERAGE")
    strategy.parameters:addStringAlternative(
        "iType",
        "Split Moving Average of Volume",
        "",
        "SPLIT MOVING AVERAGE OF VOLUME"
    )
    ...
    --strategy.parameters:addString("Flag", "Fill the gap", "Type" , "Zero");
    -- strategy.parameters:addStringAlternative("Flag", "Zero", "Zero" , "Zero");
    --strategy.parameters:addStringAlternative("Flag", "Previous / Last Value", "Previous" , "Previous");
...
function Prepare(nameOnly)
    ...
    Type = instance.parameters.iType
    ...
    name = name .. ", " .. Type .. ", " .. Period .. ", " .. Method

    assert(
        core.indicators:findIndicator("SPLIT MOVING AVERAGE") ~= nil,
        "Please, download and install Split Moving Average.LUA indicator"
    )
    assert(
        core.indicators:findIndicator("SPLIT MOVING AVERAGE OF VOLUME") ~= nil,
        "Please, download and install Split Moving Average of Volume.LUA indicator"
    )
    ...
    if Type == "SPLIT MOVING AVERAGE OF VOLUME" then
    assert(core.indicators:findIndicator(Type) ~= nil, Type .. " indicator must be installed");
        Indicator[1] = core.indicators:create(Type, Source, Period, Method)
    else
        Indicator[1] = core.indicators:create(Type, Source[Price], Period, Method)
    end
