...
--http://fxcodebase.com/code/viewtopic.php?f=31&t=64950
...
function Init() --The strategy profile initialization
    strategy:name("Simultaneous MACD Histogram Strategy")
    ...
    strategy.parameters:addInteger("A1", "Fast MA Period", "", 12)
    strategy.parameters:addInteger("A2", "Slow MA Period", "", 26)
    strategy.parameters:addInteger("A3", "Signal MA Period", "", 9)

    strategy.parameters:addGroup("2. MACD Calculation")
    strategy.parameters:addInteger("B1", "Fast MA Period", "", 8)
    strategy.parameters:addInteger("B2", "Slow MA Period", "", 17)
    strategy.parameters:addInteger("B3", "Signal MA Period", "", 9)
...
function CreateTradingParameters()
    ...
    strategy.parameters:addBoolean(
        "StopWhenProfitableDay",
        "Stop When Profitable Day",
...
local StopWhenProfitableDay
local LastProfitableDay = nil
local AcctRow = nil
...
function Prepare(nameOnly)
    ...
    A = core.indicators:create("MACD", Source.close, A1, A2, A3)
    B = core.indicators:create("MACD", Source.close, B1, B2, B3)
    first = math.max(A.SIGNAL:first(), B.SIGNAL:first())
...
function PrepareTrading()
    ...
    StopWhenProfitableDay = instance.parameters.StopWhenProfitableDay

    AcctRow = core.host:findTable("accounts"):find("AccountID", Account)
end
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    local currentTime = core.host:execute("convertTime", core.TZ_EST, ToTime, now)
    -- get only time
    now = currentTime - math.floor(currentTime)

    if StopWhenProfitableDay == true then
        if LastProfitableDay == nil or currentTime - LastProfitableDay >= 1 then
            local DayPL = AcctRow:cell("DayPL")
            if DayPL > 0.0 then
                LastProfitableDay = math.floor(currentTime)
                exitSpecific("B")
                exitSpecific("S")
                return
            end
...
function EntryFunction(now, period)
    ...
    if
        A.HISTOGRAM[period] > 0 and A.HISTOGRAM[period - 1] <= 0 and B.HISTOGRAM[period] > 0 and
            B.HISTOGRAM[period - 1] <= 0
     then
        if Direction then
            BUY()
        else
