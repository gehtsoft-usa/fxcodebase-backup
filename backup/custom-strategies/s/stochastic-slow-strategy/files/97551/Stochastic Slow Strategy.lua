...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=61578
...
function Init() --The strategy profile initialization
    strategy:name("Stochastic Slow Strategy")
...
function Prepare(nameOnly)
    ...
    Indicator = core.indicators:create("SSD", Source, K, SD, D)
    first = Indicator.D:first() + 1
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if core.crossesOver(Indicator.K, Indicator.D, period) then
        if Direction then
            BUY()
        else
            SELL()
    ...
    elseif core.crossesUnder(Indicator.K, Indicator.D, period) then
        if Direction then
            SELL()
        else
            BUY()
