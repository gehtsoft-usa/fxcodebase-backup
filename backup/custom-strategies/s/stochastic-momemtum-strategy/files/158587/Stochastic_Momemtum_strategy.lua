...
function CreateEntryIndicators(source)
    local profile = core.indicators:findIndicator("SMI");
    assert(profile ~= nil, "Please, download and install " .. "SMI" .. ".LUA indicator");
    local indicatorParams = profile:parameters();
    indicatorParams:setInteger("Period_Q", instance.parameters.Period_Q);
    indicatorParams:setInteger("Period_R", instance.parameters.Period_R);
    indicatorParams:setInteger("Period_S", instance.parameters.Period_S);
    indicatorParams:setInteger("Period_Signal", instance.parameters.Period_Signal);
    indi = core.indicators:create("SMI", source, indicatorParams)
end
...
function IsEntryLong(source, period)
    ...
    return indi.SignalBuff[period] < -45 and core.crossesOver(indi.DataBuff, indi.SignalBuff, period);
end
function IsEntryShort(source, period)
    if not indi.DATA:hasData(period - 1) then
        return false;
    end
    return indi.SignalBuff[period] > 45 and core.crossesUnder(indi.DataBuff, indi.SignalBuff, period);
end
