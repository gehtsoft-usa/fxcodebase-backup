...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=10686
...
function Init() --The strategy profile initialization
    strategy:name("SinTrend strategy")
    strategy:description("SinTrend strategy")
...
function Prepare(nameOnly)
    ...
    assert(core.indicators:findIndicator("SINTREND") ~= nil, "Please, download and install SINTREND.LUA indicator")

    if Price == "close" then
        SinTrend = core.indicators:create("SINTREND", Source.close, instance.parameters.Period)
    elseif Price == "open" then
        SinTrend = core.indicators:create("SINTREND", Source.open, instance.parameters.Period)
    elseif Price == "high" then
        SinTrend = core.indicators:create("SINTREND", Source.high, instance.parameters.Period)
    elseif Price == "low" then
        SinTrend = core.indicators:create("SINTREND", Source.low, instance.parameters.Period)
    elseif Price == "median" then
        SinTrend = core.indicators:create("SINTREND", Source.median, instance.parameters.Period)
    elseif Price == "typical" then
        SinTrend = core.indicators:create("SINTREND", Source.typical, instance.parameters.Period)
    else
        SinTrend = core.indicators:create("SINTREND", Source.weighted, instance.parameters.Period)
    end
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    SinTrend:update(core.UpdateLast)

    -- Check that we have enough data
    if (SinTrend.DATA:first() > (period - 1)) then
        return
    end
    ...
    if SinTrend.DATA[period - 1] <= 0 and SinTrend.DATA[period] > 0 then
