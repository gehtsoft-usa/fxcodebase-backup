...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=16211
...
function Init() --The strategy profile initialization
    strategy:name("Serial MA strategy")
    strategy:description("Serial MA strategy")
...
function Prepare(nameOnly)
    ...
    assert(core.indicators:findIndicator("SERIALMA") ~= nil, "Please, download and install SERIALMA.LUA indicator")

    Source = ExtSubscribe(2, nil, instance.parameters.TF, true, "bar")
    Price = instance.parameters.Price
    if Price == "close" then
        SerialMA = core.indicators:create("SERIALMA", Source.close)
    elseif Price == "open" then
        SerialMA = core.indicators:create("SERIALMA", Source.open)
    elseif Price == "high" then
        SerialMA = core.indicators:create("SERIALMA", Source.high)
    elseif Price == "low" then
        SerialMA = core.indicators:create("SERIALMA", Source.low)
    elseif Price == "median" then
        SerialMA = core.indicators:create("SERIALMA", Source.median)
    elseif Price == "typical" then
        SerialMA = core.indicators:create("SERIALMA", Source.typical)
    else
        SerialMA = core.indicators:create("SERIALMA", Source.weighted)
    end
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    SerialMA:update(core.UpdateLast)

    -- Check that we have enough data
    if (SerialMA.DATA:first() > (period - 1)) then
        return
    end
    ...
    if SerialMA.SerialMA[period - 1] == SerialMA.Cross[period - 1] then
        if source.close[period] > SerialMA.SerialMA[period] then
            if instance.parameters.TypeSignal == "direct" then
                MustOpenB = true
            else
                MustOpenS = true
            end
        elseif source.close[period] < SerialMA.SerialMA[period] then
            if instance.parameters.TypeSignal == "direct" then
                MustOpenS = true
            else
                MustOpenB = true
