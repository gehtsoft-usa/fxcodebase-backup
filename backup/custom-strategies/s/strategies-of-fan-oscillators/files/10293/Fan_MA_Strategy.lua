...
function Init() --The strategy profile initialization
    strategy:name("Fan MA strategy")
    strategy:description("Fan MA strategy")
    ...
    strategy.parameters:addGroup("Fan MA indicator parameters")
...
function Prepare(nameOnly)
    ...
    assert(core.indicators:findIndicator("FAN_MA2") ~= nil, "FAN_MA2 indicator must be installed");
    if Price == "close" then
        FMA =
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if (FMA.DATA:first() > (period - 1)) then
        return
    end
    ...
    if (FMA.BuffLine[period - 1] < instance.parameters.LevelB and FMA.BuffLine[period] >= instance.parameters.LevelB) then
    ...
    if (FMA.BuffLine[period - 1] > instance.parameters.LevelS and FMA.BuffLine[period] <= instance.parameters.LevelS) then
        if instance.parameters.TypeSignal == "direct" then
            MustS = true
        else
            MustB = true
