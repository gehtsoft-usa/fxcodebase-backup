...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=18180
...
function Init() --The strategy profile initialization
    strategy:name("Small inside bar strategy")
    strategy:description("Small inside bar strategy")
...
function Prepare(nameOnly)
    ...
    assert(
        core.indicators:findIndicator("SMALL_INSIDE_BAR") ~= nil,
        "Please, download and install SMALL_INSIDE_BAR.LUA indicator"
    )
    Source = ExtSubscribe(2, nil, instance.parameters.TF, true, "bar")
    SIB = core.indicators:create("SMALL_INSIDE_BAR", Source, "Both")
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    SIB:update(core.UpdateLast)

    -- Check that we have enough data
    if (SIB.DATA:first() > (period - 1)) then
        return
    end
    ...
    if SIB.Bullish[period] == source.low[period] then
    ...
    if SIB.Bearish[period] == source.high[period] then
        if instance.parameters.TypeSignal == "direct" then
            MustOpenS = true
        else
            MustOpenB = true
