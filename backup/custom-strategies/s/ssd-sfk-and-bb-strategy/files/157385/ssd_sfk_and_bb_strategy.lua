-- Available @ https://fxcodebase.com/code/viewtopic.php?f=31&t=75382
...
local STRATEGY_NAME = "SSD SFK and BB Strategy";
function CreateParameters() 
    strategy.parameters:addString("ssd_tf", "SSD Timeframe", "", "m5");
    strategy.parameters:setFlag("ssd_tf", core.FLAG_PERIODS);
    strategy.parameters:addInteger("ssd_K", "Number of periods for %K", "", 20, 2, 1000);
    strategy.parameters:addInteger("ssd_SD", "%D slowing periods", "", 10, 2, 1000);
    strategy.parameters:addInteger("ssd_ob", "SSD Overbought", "", 70)
    strategy.parameters:addInteger("ssd_os", "SSD Oversold", "", 30)

    strategy.parameters:addString("bb_tf", "BB/SFK Timeframe", "", "m15");
    strategy.parameters:setFlag("bb_tf", core.FLAG_PERIODS);
    strategy.parameters:addInteger("bb_period", "BB Period", "", 20);
    strategy.parameters:addDouble("bb_deviations", "BB Deviations", "", 0.75);
    strategy.parameters:addInteger("sfk_KPeriod", "SFK K Period", "", 5, 2, 1000)
    strategy.parameters:addInteger("sfk_DPeriod", "SFK D Period", "", 5, 2, 1000)
    strategy.parameters:addInteger("sfk_ob", "SFK Overbought", "", 50)
    strategy.parameters:addInteger("sfk_os", "SFK Oversold", "", 0)
    
    strategy.parameters:addString("timeframe", "Main SFK Timeframe", "", "H1");
    strategy.parameters:setFlag("timeframe", core.FLAG_PERIODS);
    strategy.parameters:addInteger("msfk_KPeriod", "Main SFK K Period", "", 5, 2, 1000)
    strategy.parameters:addInteger("msfk_DPeriod", "Main SFK D Period", "", 5, 2, 1000)
end

local ssd, ssd_ob, ssd_os;
local bb, sfk, sfk_ob, sfk_os;
local msfk;
function CreateEntryIndicators(source)
    ssd_source = ExtSubscribe(3, nil, instance.parameters.ssd_tf, instance.parameters.type, "bar");
    ssd = core.indicators:create("SSD", ssd_source, instance.parameters.ssd_K, instance.parameters.ssd_SD);
    ssd_ob = instance.parameters.ssd_ob;
    ssd_os = instance.parameters.ssd_os;

    bb_source = ExtSubscribe(4, nil, instance.parameters.bb_tf, instance.parameters.type, "bar");
    bb = core.indicators:create("BB", bb_source, instance.parameters.bb_period, instance.parameters.bb_deviations);
    sfk = core.indicators:create("SFK", bb_source, instance.parameters.sfk_KPeriod, instance.parameters.sfk_DPeriod);
    sfk_ob = instance.parameters.sfk_ob;
    sfk_os = instance.parameters.sfk_os;

    msfk = core.indicators:create("SFK", source, instance.parameters.msfk_KPeriod, instance.parameters.msfk_DPeriod);
end

function UpdateIndicators()
    ssd:update(core.UpdateLast);
    bb:update(core.UpdateLast);
    sfk:update(core.UpdateLast);
    msfk:update(core.UpdateLast);
end
...
function IsEntryLong(source, period)
    if not msfk.DATA:hasData(period) 
        or not sfk.K:hasData(NOW) 
        or not bb.TL:hasData(NOW) 
        or not ssd.K:hasData(NOW - 1) 
    then
        return false;
    end
    return msfk.K[period] > sfk_ob
        and sfk.K[NOW] > sfk_ob
        and source.close[period] > bb.TL[NOW]
        and core.crossesOver(ssd.K, ssd_ob, NOW)
...
function IsEntryShort(source, period)
    ...
    return msfk.K[period] < sfk_os
        and sfk.K[NOW] < sfk_os
        and source.close[period] < bb.TL[NOW]
        and core.crossesUnder(ssd.K, ssd_os, NOW)
        and source.close[period] < source.close[period - 1];
end
function IsExitLong(source, period)
    return core.crossesUnder(ssd.K, ssd_ob, NOW)
end
function IsExitShort(source, period)
    return core.crossesOver(ssd.K, ssd_os, NOW)
end
...
function Init()
    ...
    strategy.parameters:addDouble("stop_pips", "Stop, %", "", 3);
...
function OpenTrade(side)
    ...
    if use_stop then
        ...
        if side == "B" then
            valuemap.PegPriceOffsetPipsStop = source.close[NOW] - source.close[NOW] * stop_pips / 100.0;
        else
            valuemap.PegPriceOffsetPipsStop = source.close[NOW] + source.close[NOW] * stop_pips / 100.0;
        end
