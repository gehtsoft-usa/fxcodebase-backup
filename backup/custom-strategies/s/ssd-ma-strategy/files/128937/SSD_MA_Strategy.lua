...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=68971
...
local STRATEGY_NAME = "SSD MA Strategy";
...
function CreateParameters()
    strategy.parameters:addString("ssd_tf", "SSD Timeframe", "", "H4");
	...
	strategy.parameters:addInteger("OS_level", "Oversold level", "", 20);
    strategy.parameters:addInteger("OB_level", "Overbought level", "", 80);

    strategy.parameters:addString("ma_tf", "MA Timeframe", "", "H1");
    strategy.parameters:setFlag("ma_tf", core.FLAG_PERIODS);
    strategy.parameters:addInteger("fast_ma_period", "Fast MA period", "", 7);
    AddAverages("fast_ma_method", "Fast MA method", "MVA");
    strategy.parameters:addInteger("slow_ma_period", "Slow MA period", "", 14);
    AddAverages("slow_ma_method", "Slow MA method", "MVA");
end

local ssd;
local ma_fast;
local ma_slow;
local OS_level, OB_level;
function CreateIndicators(source)
    OB_level = instance.parameters.OB_level;
    OS_level = instance.parameters.OS_level;
    local ssd_source = trading_logic:SubscribeHistory(source:instrument(), instance.parameters.ssd_tf, source:isBid());
    ssd = core.indicators:create("SSD", ssd_source, instance.parameters.ssd_K, instance.parameters.ssd_SD, instance.parameters.ssd_D);

    local ma_source = trading_logic:SubscribeHistory(source:instrument(), instance.parameters.ma_tf, source:isBid());
    ma_fast = CreateAverages(instance.parameters.fast_ma_method, ma_source, instance.parameters.fast_ma_period);
    ma_slow = CreateAverages(instance.parameters.slow_ma_method, ma_source, instance.parameters.slow_ma_period);
end

function UpdateIndicators()
    ssd:update(core.UpdateLast);
    ma_fast:update(core.UpdateLast);
    ma_slow:update(core.UpdateLast);
end
...
function CreateCustomActions()
    ...
    enterLongAction.IsPass = function (source, period, periodFromLast, data)
        if not ma_fast.DATA:hasData(NOW - 1) 
            or not ma_slow.DATA:hasData(NOW - 1)
            or not ssd.D:hasData(NOW - 1)
        then
            return;
        end
        return core.crossesOver(ma_fast.DATA, ma_slow.DATA, NOW)
            and ssd.K[NOW] > ssd.D[NOW]
            and ssd.K[NOW] < OS_level;
    end
    ...
    enterShortAction.IsPass = function (source, period, periodFromLast, data)
        ...
        return core.crossesUnder(ma_fast.DATA, ma_slow.DATA, NOW)
            and ssd.K[NOW] < ssd.D[NOW]
            and ssd.K[NOW] > OB_level;
    end
