...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=67180
...
function Init() --The strategy profile initialization
	...
	strategy.parameters:addDouble("RSI_Buy", "RSI Buy Level", "", 66);
	strategy.parameters:addDouble("RSI_Sell", "RSI Sell Level", "", 34);
	
	
	 strategy.parameters:addGroup("MACD Calculation");
    
	strategy.parameters:addInteger("MACD_Period_1", "Short Period", "", 12);
	strategy.parameters:addInteger("MACD_Period_2", "Long Period", "", 26);
	strategy.parameters:addInteger("MACD_Period_3", "Signal Period", "", 9);
...
function CreateTradingParameters()
   ...
   -- strategy.parameters:addStringAlternative("ExitExecutionType", "End of Turn", "", "EndOfTurn");
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "SIMRS");
...
local RSI, RSI_Period, RSI_Buy, RSI_Sell;
local MACD, MACD_Period_1, MACD_Period_2, MACD_Period_3;
local SAR, Step, Max;
local ICH, Parameters1, Parameters2, Parameters3, TL, KL, CS, SA,SB;
...
function Prepare( nameOnly)
	...
	RSI_Buy = instance.parameters.RSI_Buy;
	RSI_Sell = instance.parameters.RSI_Sell;
	
	MACD_Period_1= instance.parameters.MACD_Period_1;
	MACD_Period_2= instance.parameters.MACD_Period_2;
	MACD_Period_3= instance.parameters.MACD_Period_3;
	...
	MACD = core.indicators:create("MACD", Source.close, MACD_Period_1,MACD_Period_2,MACD_Period_3   ); 
	...
	first=math.max(RSI.DATA:first(), MACD.HISTOGRAM:first(),SAR.UP:first(), SAR.DN:first(), ICH.TL:first()); 
...
function EntryFunction( now,period)
    ...
    if   RSI.DATA[period]< RSI_Buy
	and MACD.HISTOGRAM[period]> MACD.HISTOGRAM[period-1]
	and SAR:getStream(1):hasData(period)
	and not SAR:getStream(1):hasData(period-1)
	and TL[period]> TL[period-1]
	and Source.close[period]> KL[period]
	...
	then	
		...
		Return=true;
    elseif RSI.DATA[period]> RSI_Sell
	and MACD.HISTOGRAM[period]< MACD.HISTOGRAM[period-1]
	and SAR:getStream(0):hasData(period)
	and not SAR:getStream(0):hasData(period-1)
	and ICH.TL[period]< ICH.TL[period-1]
	and Source.close[period]< TL[period]
	and Source.close[period]< SA[period]
	and Source.close[period]< SB[period]
	then
