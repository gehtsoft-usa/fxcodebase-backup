...
-- https://fxcodebase.com/code/viewtopic.php?f=31&t=72413
...
function Init() --The strategy profile initialization
    strategy:name("Stochastic Equilibrium Strategy");
    ...
    strategy.parameters:addInteger("Period", "Period", "", 20, 1, 2000);
    strategy.parameters:addInteger("N", "N", "", 10, 1, 2000);
    strategy.parameters:addInteger("K", "K", "", 5, 1, 2000);	
    strategy.parameters:addInteger("MA_Period", "MA Line Period", "", 10, 1, 2000);		
...
function CreateTradingParameters()
   ...
   -- strategy.parameters:addStringAlternative("ExitExecutionType", "End of Turn", "", "EndOfTurn");
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "SES");
...
function Prepare( nameOnly)
	...
	assert(core.indicators:findIndicator("STOCHASTIC EQUILIBRIUM") ~= nil, "Please, download and install STOCHASTIC EQUILIBRIUM.LUA indicator");          
    ...
    Indicator = core.indicators:create("STOCHASTIC EQUILIBRIUM", Source , instance.parameters.Period, instance.parameters.N, instance.parameters.K );
    MA= core.indicators:create("MVA", Indicator.DATA  , instance.parameters.MA_Period );
	first=MA.DATA:first(); 
...
function EntryFunction( id, now,period)
    ...
    if   Indicator.D_Line[period]>  MA.DATA[period]
	and  Indicator.D_Line[period-1]<=  MA.DATA[period-1]
	then	
          if Direction then
                BUY();
            else
		...
		Return=true;
    elseif Indicator.D_Line[period]<  MA.DATA[period]
	and  Indicator.D_Line[period-1]>=  MA.DATA[period-1]
		then
            if Direction then
                SELL();
