...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=62789
...
function Init() --The strategy profile initialization
    strategy:name("STOCH BB RSI MA STRATEGY")
    ...
    strategy.parameters:addGroup("Stoc+BB+RSI indicator Calculation")
    ...
    strategy.parameters:addGroup("StdDev Calculation")
    strategy.parameters:addInteger("N", "Period", "", 20)

    strategy.parameters:addGroup("MA Calculation")
    strategy.parameters:addInteger("Period", "MA Period", "Period", 160)
...
local first
local N, STO_KPeriod, STO_DPeriod, STO_Slowing, STO_K, STO_D, BB_Period, BB_Deviation, RSI_Period, RSI_Price
local Indicator
local StdDev
local Direction
local CustomID
...
function Prepare(nameOnly)
    ...
    StdDev = core.indicators:create("STDDEV", Source.close, N)
    assert(core.indicators:findIndicator(Method) ~= nil, Method .. " indicator must be installed");
    MA = core.indicators:create(Method, Source.close, Period)
    first = math.max(Indicator.STO:first(), Indicator.STO_Signal:first(), StdDev.DATA:first(), MA.DATA:first())
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    --[[
	BUY:
	1. PRICE IS ABOVE EMA AND
	2.RSI >STO SIGNAL AND STO SIGNAL > BUY LEVEL AND STO CROSSS OVER RSI

	SELL:
	1.PRICE IS BELOW EMA AND
	2.RSI< STOSIGNAL AND STO SIGNAL < SELL LEVEL AND STO CROSS UNDER RSI
	]]
    -- only buy if we have a fast cross over slow and the price is above the moving averages.
    if
        Source.close[period] > MA.DATA[period] and Indicator.RSI[period] > Indicator.STO_Signal[period] and
            Indicator.STO_Signal[period] > BL and
            Indicator.STO[period] > Indicator.RSI[period] and
            Indicator.STO[period - 1] <= Indicator.RSI[period - 1]
     then
        if Direction then
            BUY()
        else
    ...
    elseif
        Source.close[period] < MA.DATA[period] and Indicator.RSI[period] < Indicator.STO_Signal[period] and
            Indicator.STO_Signal[period] < SL and
            Indicator.STO[period] < Indicator.RSI[period] and
            Indicator.STO[period - 1] >= Indicator.RSI[period - 1]
     then
        if Direction then
