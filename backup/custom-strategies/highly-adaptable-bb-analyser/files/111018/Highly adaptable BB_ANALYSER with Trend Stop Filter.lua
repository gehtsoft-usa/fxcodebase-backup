...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=64447
...
function Init() --The strategy profile initialization
    strategy:name("Highly adaptable BB_ANALYSER")
    ...
    strategy.parameters:addString("Method", "Method to smooth", "", "HMA")
    ...
    strategy.parameters:addInteger("Smooth", "Number of periods to smooth", "", 3, 1, 10000)
    strategy.parameters:addDouble(
        "SlopeConfirmBubble",
        "Minimum Slope to confirm Bubble (pip/minute)",
        "",
        0.18,
        0.0,
        20.0
    )
    strategy.parameters:addInteger("ConfirmEndSausage", "Confirm end of Sausage (%)", "", 25, 1, 100)

    strategy.parameters:addGroup("Selector")

    strategy.parameters:addString("Action" .. 1, "Bubble Up", "", "NO")
    ...
    strategy.parameters:addString("Action" .. 2, "Sausage Up", "", "NO")
    ...
    strategy.parameters:addString("Action" .. 3, "Bubble Down", "", "NO")
    ...
    strategy.parameters:addString("Action" .. 4, "Sausage Down", "", "NO")
    ...
    strategy.parameters:addString("Action" .. 5, "Squeeze", "", "NO")
    ...
    strategy.parameters:addString("trendstop_tf", "Trendstop's Time Frame", "", "H4")
    strategy.parameters:setFlag("trendstop_tf", core.FLAG_PERIODS)
    strategy.parameters:addInteger("trendstop_period", "Trendstop's Period", "Period", 20)
...
local Period, Deviation, Method, Smooth, SlopeConfirmBubble, ConfirmEndSausage
local trend_stop
...
function Prepare(nameOnly)
    ...
    SlopeConfirmBubble = instance.parameters.SlopeConfirmBubble
    ConfirmEndSausage = instance.parameters.ConfirmEndSausage
    ...
    assert(
        core.indicators:findIndicator("BB_ANALYSER_2") ~= nil,
        "Please, download and install BB_ANALYSER_2.LUA indicator"
    )
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if Indicator.BubbleUp[period] > 0 and (Indicator.BubbleUp[period - 1] == 0 or Indicator.BubbleUp[period - 1] == nil) then
        ACTION(1, 1, "Bubble Up", period)
    end
    if
        Indicator.SausageUp[period] > 0 and
            (Indicator.SausageUp[period - 1] == 0 or Indicator.SausageUp[period - 1] == nil)
     then
        ACTION(2, 1, "Sausage Up", period)
    end
    if Indicator.BubbleDn[period] > 0 and (Indicator.BubbleDn[period - 1] == 0 or Indicator.BubbleDn[period - 1] == nil) then
        ACTION(3, 2, "Bubble Dn", period)
    end
    if
        Indicator.SausageDn[period] > 0 and
            (Indicator.SausageDn[period - 1] == 0 or Indicator.SausageDn[period - 1] == nil)
     then
        ACTION(4, 2, "Sausage Dn", period)
    end
    if Indicator.Squeeze[period] > 0 and (Indicator.Squeeze[period - 1] == 0 or Indicator.Squeeze[period - 1] == nil) then
        ACTION(5, 3, "Squeeze", period)
    end
end
