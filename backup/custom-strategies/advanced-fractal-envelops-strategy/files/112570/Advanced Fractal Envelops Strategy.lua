...
function Init() --The strategy profile initialization
    strategy:name("Advanced Fractal Envelops Strategy")
...
function Prepare(nameOnly)
    ...
    Indicator = core.indicators:create("ADVANCED FRACTAL ENVELOPS", Source, Period, Frame)

    first = math.max(Indicator.TOP:first(), Indicator.BOTTOM:first())
...
function ExitFunction(now, period)
    ...
    if Source.close[period] > Indicator.BOTTOM[period] and Source.close[period - 1] <= Indicator.BOTTOM[period - 1] then
        if Direction then
            if haveTrades("S") then
                exitSpecific("S")
                Signal("Close Short")
...
function EntryFunction(now, period)
    ...
    if Source.close[period] > Indicator.CENTRAL[period] and Source.close[period - 1] <= Indicator.CENTRAL[period - 1] then
        if Direction then
            BUY()
        else
            SELL()
        ...
        Return = true
    elseif
        Source.close[period] < Indicator.CENTRAL[period] and Source.close[period - 1] >= Indicator.CENTRAL[period - 1]
     then
        if Direction then
            SELL()
        else
