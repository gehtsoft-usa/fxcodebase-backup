...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=63024
...
function Init() --The strategy profile initialization
    strategy:name("DMI Strategy with ADX Filter")
    ...
    strategy.parameters:addDouble("EntryLevel", "EntryLevel", "", 20)
    strategy.parameters:addDouble("ConfirmationLevel", "Confirmation Level", "", 20)

    strategy.parameters:addGroup("ADX Calculation")
    strategy.parameters:addInteger("Period1", "Period", "", 14)
    strategy.parameters:addDouble("EntryLevel1", "EntryLevel", "", 20)
    strategy.parameters:addDouble("ExitLevel1", "Exit Level", "", 20)
...
local first
local Period
local ConfirmationLevel, EntryLevel
local Indicator
local Direction
local CustomID

local ADX, Period1, EntryLevel1, ExitLevel1, Exit
...
function Prepare(nameOnly)
    ...
    EntryLevel = instance.parameters.EntryLevel
    ConfirmationLevel = instance.parameters.ConfirmationLevel

    Period1 = instance.parameters.Period1
    ExitLevel1 = instance.parameters.ExitLevel1
    EntryLevel1 = instance.parameters.EntryLevel1
    ...
    first = math.max(Indicator.DATA:first(), ADX.DATA:first()) + 1
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if
        Indicator.DIP[period] > EntryLevel and Indicator.DIM[period] < ConfirmationLevel and
            (Indicator.DIP[period - 1] <= EntryLevel and Indicator.DIM[period - 1] >= ConfirmationLevel) and
            ADX.DATA[period] > EntryLevel1
     then
        if Direction then
            BUY()
        else
    ...
    elseif
        Indicator.DIM[period] > EntryLevel and Indicator.DIP[period] < ConfirmationLevel and
            (Indicator.DIP[period - 1] >= ConfirmationLevel and Indicator.DIM[period - 1] <= EntryLevel) and
            ADX.DATA[period] > EntryLevel1
     then
        if Direction then
            SELL()
    ...
    if Exit then
        if ADX.DATA[period] < ExitLevel1 and ADX.DATA[period - 1] >= ExitLevel1 then
