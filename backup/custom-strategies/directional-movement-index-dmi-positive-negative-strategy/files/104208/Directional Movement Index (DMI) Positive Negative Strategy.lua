...
function Init() --The strategy profile initialization
    strategy:name("Directional Movement Index (DMI) Positive Negative Strategy")
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if
        Indicator.DIP[period] > EntryLevel and Indicator.DIM[period] < ConfirmationLevel and
            (Indicator.DIP[period - 1] <= EntryLevel and Indicator.DIM[period - 1] >= ConfirmationLevel)
     then
        if Direction then
            BUY()
        else
    ...
    elseif
        Indicator.DIM[period] > EntryLevel and Indicator.DIP[period] < ConfirmationLevel and
            (Indicator.DIP[period - 1] >= ConfirmationLevel and Indicator.DIM[period - 1] <= EntryLevel)
     then
        if Direction then
            SELL()
        else
