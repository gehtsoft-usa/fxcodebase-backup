...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=7953
...
function Init() --The strategy profile initialization
    strategy:name("MTF EMA & Stoch Strategy");
    ...
    strategy.parameters:addInteger("SD", "D Slowing Period", "", 3, 2, 1000);
    strategy.parameters:addInteger("D", "D Period", "", 3, 2, 1000);

	 
    strategy.parameters:addString("MVAT_K", "Smoothing Type for % K", "", "MVA");
    ...
    strategy.parameters:addString("MVAT_D", "Smoothing Type for % D", "", "MVA");
    strategy.parameters:addStringAlternative("MVAT_D", "MVA","", "MVA");
    strategy.parameters:addStringAlternative("MVAT_D", "EMA", "", "EMA");
	
    strategy.parameters:addGroup("Short Time Frame Calculation");
	
    strategy.parameters:addString("TF1", "Fast MA time frame", "", "H1");
    strategy.parameters:setFlag("TF1", core.FLAG_PERIODS);
    strategy.parameters:addInteger("N1", "1. EMA periods", "", 10, 1, 2000);
	strategy.parameters:addInteger("N2", "2. EMA periods", "", 20, 1, 2000);	
	
    strategy.parameters:addGroup("Long Time Frame Calculation");
    strategy.parameters:addString("TF2", "Slow MA time frame", "", "D1");
    strategy.parameters:setFlag("TF2", core.FLAG_PERIODS); 
	
	strategy.parameters:addInteger("N3", "3. EMA periods", "", 5, 1, 2000);
	strategy.parameters:addInteger("N4", "4. EMA periods", "", 10, 1, 2000);
    strategy.parameters:addInteger("N5", "5. EMA periods", "", 50, 1, 2000);
...
function Prepare(nameOnly)
    ...
    Direction = instance.parameters.Direction == "direct";
	 ...
	 N4  = instance.parameters.N4;
	 N5  = instance.parameters.N5;
    ...
    Source[1] = ExtSubscribe(1, nil, instance.parameters.TF1, instance.parameters.Type == "Bid", "bar");
	Source[2] = ExtSubscribe(2, nil, instance.parameters.TF2, instance.parameters.Type == "Bid", "bar");
	
	
    Indicator[1] = core.indicators:create("EMA", Source[1][Price], instance.parameters.N1);
    Short[1] = Indicator[1]:getStream(0);	
	
    Indicator[2] = core.indicators:create("EMA", Source[1][Price], instance.parameters.N2);
    Short[2] = Indicator[2]:getStream(0);
	
	
	
	  Indicator[3] = core.indicators:create("EMA", Source[2][Price], instance.parameters.N3);
    Short[3] = Indicator[3]:getStream(0);	
	
    Indicator[4] = core.indicators:create("EMA", Source[2][Price], instance.parameters.N4);
    Short[4] = Indicator[4]:getStream(0);
	
	Indicator[5] = core.indicators:create("EMA", Source[2][Price], instance.parameters.N5);
    Short[5] = Indicator[5]:getStream(0);
	
  	
   	Indicator[6] = core.indicators:create("STOCHASTIC", Source[2], K, SD, D,MVAT_K , MVAT_D );
    Short[6] = Indicator[6]:getStream(0);	
	Short[7] = Indicator[6]:getStream(1);	
	
	
	Indicator[7] = core.indicators:create("STOCHASTIC", Source[1], K, SD, D,MVAT_K , MVAT_D );
    Short[8] = Indicator[7]:getStream(0);	
	Short[9] = Indicator[7]:getStream(1);	
...
function ExtUpdate(id, source, period)  -- The method called every time when a new bid or ask price appears.
    ...
    Indicator[6]:update(core.UpdateLast);
	Indicator[7]:update(core.UpdateLast);
	...
	or Short[5]:size() <= Short[5]:first() + 2
	or Short[6]:size() <= Short[6]:first() + 2
	or Short[7]:size() <= Short[7]:first() + 2
	or Short[8]:size() <= Short[8]:first() + 2
	or Short[9]:size() <= Short[9]:first() + 2
	then
    ...
    now = now - math.floor(now);
      ...
      return ;
          ...
          -- On the daily chart: 
			...
			--Stoch (10,3,3): The fast line must be above the slow, (in other words, bullish), and must not be overbought.

			--On the hourly chart:
			--EMAs: Buy whenever the 10 crosses the 20 (bullish) as long as:
			--Stoch (10,3,3): bullish indication and not overbought.
 
         
        if Short[3][Short[3]:size()-2]  > Short[4][Short[4]:size()-2] 
		and Short[4][Short[4]:size()-2]  > Short[5][Short[5]:size()-2] 
		and Short[6][Short[6]:size()-2]  > Short[7][Short[7]:size()-2]
		and Short[6][Short[6]:size()-2]  <   80
		
		and Short[8][Short[8]:size()-2]  > Short[9][Short[9]:size()-2] 
		and Short[9][Short[9]:size()-2]  <   80
		
		and Short[1][Short[1]:size()-2] > Short[2][Short[2]:size()-2] 
        ...
        elseif Short[3][Short[3]:size()-2]  < Short[4][Short[4]:size()-2] 
		and Short[4][Short[4]:size()-2]  < Short[5][Short[5]:size()-2]
		and Short[6][Short[6]:size()-2]  < Short[7][Short[7]:size()-2] 
		and Short[6][Short[6]:size()-2]  >   20
		
		and Short[8][Short[8]:size()-2]  < Short[9][Short[9]:size()-2] 
		and Short[9][Short[9]:size()-2]  >   20
