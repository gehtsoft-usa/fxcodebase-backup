...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=63743
...
function Init() --The strategy profile initialization
    strategy:name("BB ADX MA Strategy")
    ...
    strategy.parameters:addString("bb_price", "BB Price", "", "close")
    ...
    strategy.parameters:addInteger("bb_period", "Period", "Period", 20)
    strategy.parameters:addDouble("bb_deviation", "Deviation", "Deviation", 2)

    strategy.parameters:addGroup("ADX parameters")
    strategy.parameters:addInteger("ADX_Period", "Period", "", 20)
    strategy.parameters:addDouble("ADX_Level", "Level ADX", "", 25)

    strategy.parameters:addGroup("MA parameters")

    strategy.parameters:addString("ma_price", "BB Price", "", "close")
    ...
    strategy.parameters:addInteger("ma_period", "Period", "", 20)
...
local first
local Exit
local bb_price, bb_period, bb_deviation, bb, ma, ma_price, ma_method, ma_period, adx, ADX_Level, ADX_Period
local Direction
local CustomID
...
function Prepare(nameOnly)
    ...
    bb_price = instance.parameters.bb_price
    ...
    ADX_Period = instance.parameters.ADX_Level
    ...
    bb = core.indicators:create("BB", Source[bb_price], bb_period, bb_deviation)
    first = math.max(adx.DATA:first(), ma.DATA:first(), bb.DATA:first())
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if
        Source.close[period] > ma.DATA[period] and adx.DATA[period] > ADX_Level and Source.close[period] > bb.TL[period] and
            Source.close[period - 1] <= bb.TL[period - 1]
     then
        if Direction then
            BUY()
        else
    ...
    elseif
        Source.close[period] < ma.DATA[period] and adx.DATA[period] > ADX_Level and Source.close[period] < bb.BL[period] and
            Source.close[period - 1] >= bb.BL[period - 1]
     then
        if Direction then
            SELL()
        else
