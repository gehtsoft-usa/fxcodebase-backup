...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=3606
...
function Init() --The strategy profile initialization
    strategy:name("Brain trend strategy")
    strategy:description("Brain trend strategy")
    strategy:setTag(
    ...
    strategy.parameters:addGroup("Brain trend indicator parameters")
    strategy.parameters:addInteger("Stoch_K", "Stochastic K", "", 9)
    strategy.parameters:addInteger("Stoch_SD", "Stochastic SD", "", 9)
    strategy.parameters:addInteger("Stoch_Level", "Stochastic Level", "", 40)
    strategy.parameters:addInteger("ATR_Period", "ATR period", "", 7)
    strategy.parameters:addInteger("Shift", "Shift", "", 2)
    strategy.parameters:addDouble("Range", "Range", "", 2)
...
function Prepare(nameOnly)
            ...
            instance.bid:name() ..
                "." ..
                    instance.parameters.TF ..
                        "," ..
                            "BrainTrend(" ..
                                instance.parameters.Stoch_K ..
                                    ", " ..
                                        instance.parameters.Stoch_SD ..
                                            ", " ..
                                                instance.parameters.Stoch_Level ..
                                                    ", " ..
    ...
    assert(core.indicators:findIndicator("BRAINTREND") ~= nil, "BRAINTREND indicator must be installed")
    BT =
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    BT:update(core.UpdateLast)

    -- Check that we have enough data
    if (BT.DATA:first() > (period - 1)) then
        return
    end
    ...
    if BT.BuffUP[period - 1] ~= Source.low[period - 1] and BT.BuffUP[period] == Source.low[period] then
    ...
    if BT.BuffDN[period - 1] ~= Source.high[period - 1] and BT.BuffDN[period] == Source.high[period] then
        if instance.parameters.TypeSignal == "direct" then
            MustS = true
        else
            MustB = true
