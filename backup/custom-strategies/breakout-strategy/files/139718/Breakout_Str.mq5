...
// http://fxcodebase.com/code/viewtopic.php?f=31&t=2639
...
public:
   EntryLongCondition(string symbol, ENUM_TIMEFRAMES timeframe)
      :ACondition(symbol, timeframe)
   {
      _indi = iCustom(symbol, timeframe, "Breakout", PeriodBegin, PeriodEnd);
   }
   ...
   {
      double BrU[1];
      if (CopyBuffer(_indi, 0, period, 1, BrU) != 1)
      {
         return false;
      }
      double BrL[1];
      if (CopyBuffer(_indi, 0, period, 1, BrL) != 1)
      {
         return false;
      }
      return iLow(_symbol, _timeframe, 1) < BrL[0] && iClose(_symbol, _timeframe, 1) > BrU[0];
   }
};
...
public:
   ...
   virtual bool IsPass(const int period, const datetime date)
   {
      ...
      return iHigh(_symbol, _timeframe, 1) > BrU[0] && iClose(_symbol, _timeframe, 1) < BrL[0];
   }
};
