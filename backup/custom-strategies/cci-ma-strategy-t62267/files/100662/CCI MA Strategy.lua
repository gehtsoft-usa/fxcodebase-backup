...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=62267
...
local first
local MA_Period, CCI_Period, Exit, Price, Method
local CCI, MA
local Direction
local CustomID
local OS, OB
...
function Prepare(nameOnly)
    ...
    first = math.max(MA.DATA:first(), CCI.DATA:first())
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if Source.close[period] > MA.DATA[period] and CCI.DATA[period] > OB and CCI.DATA[period - 1] <= OB then
        if Direction then
            BUY()
        else
            SELL()
    ...
    elseif Source.close[period] < MA.DATA[period] and CCI.DATA[period] < OS and CCI.DATA[period - 1] >= OS then
        if Direction then
            SELL()
        else
            BUY()
