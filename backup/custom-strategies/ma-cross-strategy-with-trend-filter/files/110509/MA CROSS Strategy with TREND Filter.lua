...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=64296
...
function Init() --The strategy profile initialization
    strategy:name("MA CROSS  Strategy with TREND Filter")
    ...
    strategy.parameters:addInteger("Period2", "MA Period", "Period", 20)
    ...
    strategy.parameters:addGroup("Trend MA Calculation")
    strategy.parameters:addInteger("Period3", "MA Period", "Period", 50)
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if not MA1.DATA:hasData(period) or not MA2.DATA:hasData(period) or not MA3.DATA:hasData(period) then
        return
    end
...
function EntryFunction(period, now)
    ...
    if
        Source.close[period] > MA3.DATA[period] and MA1.DATA[period] > MA2.DATA[period] and
            MA1.DATA[period - 1] <= MA2.DATA[period - 1]
     then
        if Direction then
            BUY()
        else
