...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=64958
...
function Init() --The strategy profile initialization
    strategy:name("Alligator MA Cross Strategy")
    ...
    strategy.parameters:addString("Selector", "Signal Selector", "", "Jaw")
    strategy.parameters:addStringAlternative("Selector", "MA/Jaw", "", "Jaw")
    strategy.parameters:addStringAlternative("Selector", "MA/Teeth", "", "Teeth")
    strategy.parameters:addStringAlternative("Selector", "MA/Lips", "", "Lips")

    strategy.parameters:addGroup("Alligator Calculation")
    strategy.parameters:addInteger("JawN", "Alligator Jaw smoothing periods", "", 13, 1, 300)
    strategy.parameters:addInteger("JawS", "Alligator Jaw shifting periods", "", 8, 0, 300)

    strategy.parameters:addInteger("TeethN", "Alligator Teeth smoothing periods", "", 8, 1, 300)
    strategy.parameters:addInteger("TeethS", "Alligator Teeth shifting periods", "", 5, 0, 300)

    strategy.parameters:addInteger("LipsN", "Alligator Lips smoothing periods", "", 5, 1, 300)
    strategy.parameters:addInteger("LipsS", "Alligator Lips shifting periods", "", 3, 0, 300)

    strategy.parameters:addString("MTH", "Smoothing method", "", "SMMA")
    strategy.parameters:addStringAlternative("MTH", "MVA", "", "MVA")
    strategy.parameters:addStringAlternative("MTH", "EMA", "", "EMA")
    strategy.parameters:addStringAlternative("MTH", "LWMA", "", "LWMA")
    strategy.parameters:addStringAlternative("MTH", "LSMA", "", "REGRESSION")
    strategy.parameters:addStringAlternative("MTH", "SMMA", "", "SMMA")
    strategy.parameters:addStringAlternative("MTH", "Vidya1995", "", "VIDYA")
    strategy.parameters:addStringAlternative("MTH", "Vidya1992", "", "VIDYA92")
    strategy.parameters:addStringAlternative("MTH", "Wilders", "", "WMA")
    ...
    strategy.parameters:addGroup("Deviation Filter")

    strategy.parameters:addBoolean("DeviationFilter", "Use Filter", "", true)
    strategy.parameters:addInteger("DeviationPeriod", "Deviation Period", "Period", 50)

    strategy.parameters:addString("DeviationType", "Deviation Type", "", "Price")
    strategy.parameters:addStringAlternative("DeviationType", "MA", "", "MA")
    strategy.parameters:addStringAlternative("DeviationType", "Price", "", "Price")

    strategy.parameters:addDouble("DeviationMinimum", "Deviation Minimum", "Value", 0.0001)
...
local TeethN, TeethS
local LipsN, LipsC
local MTH, alligator, Selector
local Price, Method, Period, ma

local DeviationFilter, DeviationPeriod, DeviationType, DeviationMinimum
...
function Prepare(nameOnly)
    ...
    DeviationFilter = instance.parameters.DeviationFilter
    DeviationPeriod = instance.parameters.DeviationPeriod
    DeviationType = instance.parameters.DeviationType
    DeviationMinimum = instance.parameters.DeviationMinimum
    ...
    alligator = core.indicators:create("ALLIGATOR", Source, JawN, JawS, TeethN, TeethS, LipsN, LipsC, MTH)
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    alligator:update(core.UpdateAll)
...
function EntryFunction(now, period)
    if DeviationFilter then
        if DeviationType == "Price" then
            if period <= Source.close:first() + DeviationPeriod then
                return
            end

            Deviation = mathex.stdev(Source.close, period - DeviationPeriod + 1, period)
        else
            if period <= ma.DATA:first() + DeviationPeriod then
                return
            end

            Deviation = mathex.stdev(ma.DATA, period - DeviationPeriod + 1, period)
        end

        if Deviation < DeviationMinimum then
            return
        end
    end
    ...
    if Selector == "Lips" and (not alligator.Lips:hasData(period) or not alligator.Lips:hasData(period - 1)) then
        return
    end

    if Selector == "Jaw" and (not alligator.Jaw:hasData(period) or not alligator.Jaw:hasData(period - 1)) then
        return
    end

    if Selector == "Teeth" and (not alligator.Teeth:hasData(period) or not alligator.Teeth:hasData(period - 1)) then
        return
    end

    -- only buy if we have a fast cross over slow and the price is above the moving averages.
    if
        (Selector == "Lips" and ma.DATA[period] > alligator.Lips[period] and
            ma.Lips[period - 1] <= alligator.Lips[period - 1]) or
            (Selector == "Jaw" and ma.DATA[period] > alligator.Jaw[period] and
                ma.DATA[period - 1] <= alligator.Jaw[period - 1]) or
            (Selector == "Teeth" and ma.DATA[period] > alligator.Teeth[period] and
                ma.DATA[period - 1] <= alligator.Teeth[period - 1])
     then
        if Direction then
            BUY()
        else
