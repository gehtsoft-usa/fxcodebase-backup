...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=6910
...
function Init() --The strategy profile initialization
    strategy:name("Regression CCI Super Trend Cross Strategy");
    ...
    strategy.parameters:addGroup("Regression");
	strategy.parameters:addInteger("RF", "Regression Period", "", 14, 2, 1000);	
	
	 strategy.parameters:addGroup("Super Trend");
	strategy.parameters:addInteger("N", "Number of periods", "", 10);
    strategy.parameters:addDouble("M", "Multiplier", "", 1.5);
	strategy.parameters:addInteger("C", "CCI periods", "", 50);
...
function Prepare( nameOnly)
   ...
   assert(core.indicators:findIndicator("CCI_SUPERTREND") ~= nil, "Please, download and install CCI_SUPERTREND.LUA indicator");
    ...
    local name;
    name = profile:id() .. "(" .. instance.bid:name() .. "." .. instance.parameters.TF..", " .. N ..", " .. M..", " .. C ..", " .. RF ..  ")";
    instance:name(name);
	...
	indicator[1]  = core.indicators:create("REGRESSION", Source.close, RF);
	indicator[2]  = core.indicators:create("CCI_SUPERTREND", Source, N,M, C);
    ...
    now = now - math.floor(now);
      ...
      return ;
	                ...
	                indicator[2]:update(core.UpdateLast);
							...
							if core.crossesOver( indicator[1].DATA, indicator[2].DATA , period) then 
