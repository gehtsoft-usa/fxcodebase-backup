...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=4881
...
function Init() --The strategy profile initialization
    ...
    strategy.parameters:addStringAlternative("SignalType", "Histogram Top/Bottom", "", "TOPBOTTOM")
    strategy.parameters:addStringAlternative("SignalType", "Histogram Reverse", "", "Reverse")
    strategy.parameters:addStringAlternative("SignalType", "All", "", "All")
    ...
    strategy.parameters:addString("PriceType", "Price Type", "", "close")
    strategy.parameters:addStringAlternative("PriceType", "OPEN", "", "open")
    strategy.parameters:addStringAlternative("PriceType", "HIGH", "", "high")
    strategy.parameters:addStringAlternative("PriceType", "LOW", "", "low")
    strategy.parameters:addStringAlternative("PriceType", "CLOSE", "", "close")
    strategy.parameters:addStringAlternative("PriceType", "MEDIAN", "", "median")
    strategy.parameters:addStringAlternative("PriceType", "TYPICAL", "", "typical")
    strategy.parameters:addStringAlternative("PriceType", "WEIGHTED", "", "weighted")
...
function Prepare(nameOnly)
    ...
    OSMA = core.indicators:create("OSMA", Source[PriceType], Method, SN, LN, IN)
    first = OSMA.DATA:first() + 2
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if not OSMA.DATA:hasData(period - 2) or not OSMA.DATA:hasData(period - 1) or not OSMA.DATA:hasData(period) then
        return
    end

    if SignalType == "CROSS" or SignalType == "All" then
        if core.crossesOver(OSMA.DATA, 0, period) then
            if Direction then
                BUY()
            else
                SELL()
        ...
        if core.crossesUnder(OSMA.DATA, 0, period) then
            if Direction then
                SELL()
            else
                BUY()
    ...
    if (SignalType == "TOPBOTTOM" or SignalType == "All") then
        if
            OSMA.DATA[period] > OSMA.DATA[period - 1] and OSMA.DATA[period - 1] < 0 and
                OSMA.DATA[period - 2] > OSMA.DATA[period - 1]
         then
            if Direction then
                BUY()
            else
    ...
    if (SignalType == "Reverse" or SignalType == "All") then
        if OSMA.DATA[period] > OSMA.DATA[period - 1] and OSMA.DATA[period - 2] > OSMA.DATA[period - 1] then
            if Direction then
                BUY()
            else
                SELL()
