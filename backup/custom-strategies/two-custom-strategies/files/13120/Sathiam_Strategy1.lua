...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=5440
...
function Init() --The strategy profile initialization
    strategy:name("Sathiam strategy 1")
    strategy:description("Sathiam strategy 1")
    strategy:setTag(
    ...
    strategy.parameters:addInteger("EMA1_Period", "EMA1_Period", "", 5)
    strategy.parameters:addInteger("EMA2_Period", "EMA2_Period", "", 21)
    strategy.parameters:addInteger("EMA3_Period", "EMA3_Period", "", 55)
    strategy.parameters:addInteger("SMA_Period", "SMA_Period", "", 8)
    strategy.parameters:addInteger("SSD_PeriodK", "SSD_PeriodK", "", 8)
    strategy.parameters:addInteger("SSD_SlowPeriodD", "SSD_SlowPeriodD", "", 5)
    strategy.parameters:addInteger("SSD_PeriodD", "SSD_PeriodD", "", 3)
...
function Prepare(nameOnly)
            ...
            instance.bid:name() ..
                "." ..
                    instance.parameters.TF ..
                        "," ..
                            "EMA1(" ..
                                instance.parameters.EMA1_Period ..
                                    "), EMA2(" ..
                                        instance.parameters.EMA2_Period ..
                                            "), EMA3(" ..
                                                instance.parameters.EMA3_Period ..
                                                    "), SMA(" ..
                                                        instance.parameters.SMA_Period ..
                                                            "), SSD(" ..
                                                                instance.parameters.SSD_PeriodK ..
                                                                    ", " ..
                                                                        instance.parameters.SSD_SlowPeriodD ..
                                                                            ", " ..
                                                                                instance.parameters.SSD_PeriodD .. "))"
    instance:name(name)
    if nameOnly then
        return
    end
    ...
    EMA1 = core.indicators:create("EMA", Source_, instance.parameters.EMA1_Period)
    EMA2 = core.indicators:create("EMA", Source_, instance.parameters.EMA2_Period)
    EMA3 = core.indicators:create("EMA", Source_, instance.parameters.EMA3_Period)
    SMA = core.indicators:create("MVA", Source_, instance.parameters.SMA_Period)
    SSD =
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if (EMA1.DATA:first() > (period - 1)) then
        return
    end
    if (EMA2.DATA:first() > (period - 1)) then
        return
    end
    if (EMA3.DATA:first() > (period - 1)) then
        return
    end
    if (SMA.DATA:first() > (period - 1)) then
        return
    end
    if (SSD.DATA:first() > (period - 1)) then
        return
    end
    ...
    if
        EMA1.DATA[period - 1] < SMA.DATA[period - 1] and EMA1.DATA[period] > SMA.DATA[period] and
            EMA2.DATA[period] > EMA3.DATA[period] and
            SSD.K[period] < instance.parameters.Level and
            SSD.D[period] < instance.parameters.Level
     then
    ...
    if SSD.K[period - 1] > SSD.D[period - 1] and SSD.K[period] < SSD.D[period] then
        if instance.parameters.TypeSignal == "direct" then
            MustCloseB = true
        else
            MustCloseS = true
