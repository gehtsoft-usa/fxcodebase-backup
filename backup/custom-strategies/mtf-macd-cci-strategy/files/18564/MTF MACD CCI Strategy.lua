...
function Init() --The strategy profile initialization
    strategy:name("MTF MACD CCI Strategy")
    ...
    strategy.parameters:addBoolean("EXIT", "Use Exit Rules", "", false)
...
function Parameters(id, TF)
    ...
    strategy.parameters:addInteger("CN" .. id, "CCI Period", "", 14, 2, 1000)
    strategy.parameters:addDouble("CB" .. id, "CCI Buy Level", "", 0)
    strategy.parameters:addDouble("CS" .. id, "CCI Sell Level", "", 0)

    strategy.parameters:addGroup("MACD Calculation")
    strategy.parameters:addInteger("SN" .. id, "Short Period", "", 12, 1, 2000)
    strategy.parameters:addInteger("LN" .. id, "Long Period", "", 26, 1, 2000)
    strategy.parameters:addInteger("IN" .. id, "Signal Period", "", 9, 1, 2000)
    strategy.parameters:addDouble("MB" .. id, "CCI Buy Level", "", 0)
    strategy.parameters:addDouble("MS" .. id, "CCI Sell Level", "", 0)
end
...
function Prepare(nameOnly)
    ...
    for i = 1, 4, 1 do
        ...
        CN[i] = instance.parameters:getInteger("CN" .. i)
        SN[i] = instance.parameters:getInteger("SN" .. i)
        LN[i] = instance.parameters:getInteger("LN" .. i)
        IN[i] = instance.parameters:getInteger("IN" .. i)
        CB[i] = instance.parameters:getDouble("CB" .. i)
        CS[i] = instance.parameters:getDouble("CS" .. i)
        MB[i] = instance.parameters:getDouble("MB" .. i)
        MS[i] = instance.parameters:getDouble("MS" .. i)

        assert(TF[i] ~= "t1", "The time frame must not be tick")
        name = name .. "(" .. TF[i] .. ", " .. CN[i] .. ", " .. SN[i] .. ", " .. LN[i] .. ", " .. IN[i] .. ") "
    end
    ...
    for i = 1, 4, 1 do
        ...
        Indicator["CCI"][i] = core.indicators:create("CCI", Source[i], CN[i])
        Short["CCI"][i] = Indicator["CCI"][i]:getStream(0)

        Indicator["MACD"][i] = core.indicators:create("MACD", Source[i][Price], SN[i], LN[i], IN[i])
        Short["MACD"][i] = Indicator["MACD"][i]:getStream(0)
        Short["SIGNAL"][i] = Indicator["MACD"][i]:getStream(1)
    end
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    local i
    for i = 1, 4, 1 do
        Indicator["MACD"][i]:update(core.UpdateLast)
        Indicator["CCI"][i]:update(core.UpdateLast)
    end
    ...
    for i = 1, 4, 1 do
        if
            Short["SIGNAL"][i]:size() <= Short["SIGNAL"][i]:first() + 2 or
                Short["MACD"][i]:size() <= Short["MACD"][i]:first() + 2 or
                Short["CCI"][i]:size() <= Short["CCI"][i]:first() + 2
         then
            return
        end
    end
    ...
    --2.	macd[time frame m5] crossover signal [time frame m5] {and}
    ...
    if
        Short["CCI"][1][Short["CCI"][1]:size() - 1] > CB[1] and
            Short["MACD"][1][Short["MACD"][1]:size() - 1] > Short["SIGNAL"][1][Short["SIGNAL"][1]:size() - 1] and
            Short["MACD"][1][Short["MACD"][1]:size() - 1] < MB[1] and
            Short["CCI"][2][Short["CCI"][2]:size() - 1] > CB[2] and
            Short["MACD"][2][Short["MACD"][2]:size() - 1] > Short["SIGNAL"][2][Short["SIGNAL"][2]:size() - 1] and
            Short["CCI"][3][Short["CCI"][3]:size() - 1] > CB[3] and
            Short["MACD"][3][Short["MACD"][3]:size() - 1] > Short["SIGNAL"][3][Short["SIGNAL"][3]:size() - 1] and
            Short["CCI"][4][Short["CCI"][4]:size() - 1] > CB[4] and
            Short["MACD"][4][Short["MACD"][4]:size() - 1] > Short["SIGNAL"][4][Short["SIGNAL"][4]:size() - 1]
     then
        --Selling conditions:
        --1.	cci [timeframe m5] < sell level
        --2.	macd [time frame m5] cross under signal [time frame m5] {and}
    ...
    elseif
        Short["CCI"][1][Short["CCI"][1]:size() - 1] < CS[1] and
            Short["MACD"][1][Short["MACD"][1]:size() - 1] < Short["SIGNAL"][1][Short["SIGNAL"][1]:size() - 1] and
            Short["MACD"][1][Short["MACD"][1]:size() - 1] > MS[1] and
            Short["CCI"][2][Short["CCI"][2]:size() - 1] < CS[2] and
            Short["MACD"][2][Short["MACD"][2]:size() - 1] < Short["SIGNAL"][2][Short["SIGNAL"][2]:size() - 1] and
            Short["CCI"][3][Short["CCI"][3]:size() - 1] < CS[3] and
            Short["MACD"][3][Short["MACD"][3]:size() - 1] < Short["SIGNAL"][3][Short["SIGNAL"][3]:size() - 1] and
            Short["CCI"][4][Short["CCI"][4]:size() - 1] < CS[4] and
            Short["MACD"][4][Short["MACD"][4]:size() - 1] < Short["SIGNAL"][4][Short["SIGNAL"][4]:size() - 1]
     then
        if Direction then
            SELL()
    ...
    if EXIT then
        ...
        if
            core.crossesUnder(Short["CCI"][1], CB[1], Short["CCI"][1]:size() - 1) or
                core.crossesUnder(Short["MACD"][1], Short["SIGNAL"][1], Short["SIGNAL"][1]:size() - 1) or
                core.crossesUnder(Short["CCI"][2], CB[2], Short["CCI"][2]:size() - 1) or
                core.crossesUnder(Short["MACD"][2], Short["SIGNAL"][2], Short["SIGNAL"][2]:size() - 1) or
                core.crossesUnder(Short["CCI"][3], CB[3], Short["CCI"][3]:size() - 1) or
                core.crossesUnder(Short["MACD"][3], Short["SIGNAL"][3], Short["SIGNAL"][3]:size() - 1) or
                core.crossesUnder(Short["CCI"][4], CB[4], Short["CCI"][4]:size() - 1) or
                core.crossesUnder(Short["MACD"][4], Short["SIGNAL"][4], Short["SIGNAL"][4]:size() - 1)
         then
            if Direction then
        ...
        if
            core.crossesOver(Short["CCI"][1], CS[1], Short["CCI"][1]:size() - 1) or
                core.crossesOver(Short["MACD"][1], Short["SIGNAL"][1], Short["SIGNAL"][1]:size() - 1) or
                core.crossesOver(Short["CCI"][2], CS[2], Short["CCI"][2]:size() - 1) or
                core.crossesOver(Short["MACD"][2], Short["SIGNAL"][2], Short["SIGNAL"][2]:size() - 1) or
                core.crossesOver(Short["CCI"][3], CS[3], Short["CCI"][3]:size() - 1) or
                core.crossesOver(Short["MACD"][3], Short["SIGNAL"][3], Short["SIGNAL"][3]:size() - 1) or
                core.crossesOver(Short["CCI"][4], CS[4], Short["CCI"][4]:size() - 1) or
                core.crossesOver(Short["MACD"][4], Short["SIGNAL"][4], Short["SIGNAL"][4]:size() - 1)
         then
            if Direction then
                if haveTrades("S") then
                    exit("S")
