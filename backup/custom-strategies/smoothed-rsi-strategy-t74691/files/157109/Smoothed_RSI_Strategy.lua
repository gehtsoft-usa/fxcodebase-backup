-- Available @ https://fxcodebase.com/code/viewtopic.php?f=31&t=74691
...
function Init() --The strategy profile initialization
    strategy:name("Smoothed RSI Strategy");
    ...
    strategy.parameters:addInteger("RSIFrame", "RSI Period", "No description", 14);
	
	strategy.parameters:addGroup("MA Parameters");
    strategy.parameters:addInteger("MAFrame", "MA Period", "No description", 10);
	strategy.parameters:addString("Method", "MA Method", "", "MVA");
	...
	strategy.parameters:addGroup("Buy/Sell Levels");
    strategy.parameters:addDouble("BuyLevel", "Buy Level", "", 50, 0, 100);
    strategy.parameters:addDouble("SellLevel", "Sell Level", "", 50, 0, 100);
	
	strategy.parameters:addGroup("Buy/Sell Exit Levels");
    strategy.parameters:addDouble("BuyLevelExit", "Buy Level", "", 70, 0, 100);
    strategy.parameters:addDouble("SellLevelExit", "Sell Level", "", 30, 0, 100);
...
function CreateTradingParameters()
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "SRSIS");
...
local Indicator;
local RSIFrame, MAFrame, Method, BuyLevel, SellLevel, BuyLevelExit, SellLevelExit;
...
function Prepare( nameOnly)
	...
	BuyLevelExit = instance.parameters.BuyLevelExit;
	SellLevelExit = instance.parameters.SellLevelExit; 
	...
	local profile = core.indicators:findIndicator("SMOOTHED RSI");
	assert(profile ~= nil, "Please, download and install SMOOTHED RSI.LUA indicator");          
    ...
    local params = profile:parameters();
    params:setInteger("RSIFrame", RSIFrame);
    params:setInteger("MAFrame", MAFrame);
    params:setString("Method", Method);
    params:setInteger("overbought", 70);
    params:setInteger("oversold", 30);
    params:setBoolean("strategy_mode", true);
    Indicator = profile:createInstance(Source.close, params);
...
--	period3= core.findDate (Source3, TickSource:date(period), false);
	...
	--/////////////////////////////////////////////////////////////////////
		             ...
		             if   Indicator.DATA[period]<  BuyLevelExit
	                 and  Indicator.DATA[period-1]>=  BuyLevelExit
					 then
					  ...
					  if Indicator.DATA[period]>  SellLevelExit
	                  and  Indicator.DATA[period-1]<=  SellLevelExit
					  then
...
function EntryFunction( id, now,period)
	...
	then	
		...
		Return=true;
    elseif Indicator.DATA[period]<  SellLevel
