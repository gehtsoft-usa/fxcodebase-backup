...
function Init() --The strategy profile initialization
    strategy:name("N-CANDLE PREDICTOR Strategy");
	...
	strategy.parameters:addString("TF1", "1. Time frame", "", "m15");
	...
	strategy.parameters:addString("TF3", "3. Time frame", "", "H1");
    strategy.parameters:setFlag("TF3", core.FLAG_PERIODS);

	strategy.parameters:addString("TF4", "4. Time frame", "", "H4");
    strategy.parameters:setFlag("TF4", core.FLAG_PERIODS);	

    strategy.parameters:addGroup("Trade");
    strategy.parameters:addInteger("Period", "Period", "", 10, 1, 10);
...
function CreateTradingParameters()
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "NCPS");
...
function Prepare( nameOnly)
	...
	assert(TF3 ~= "t1", "3. time frame must not be tick");
	assert(TF4 ~= "t1", "4. time frame must not be tick");
	...
	assert(core.indicators:findIndicator("N-CANDLE PREDICTOR SIGNAL") ~= nil, "Please, download and install N-CANDLE PREDICTOR SIGNAL.LUA indicator");          
    ...
    Indicator1 = core.indicators:create("N-CANDLE PREDICTOR SIGNAL", Source1, Period  ); 
    Indicator2 = core.indicators:create("N-CANDLE PREDICTOR SIGNAL", Source2, Period  ); 
    Indicator3 = core.indicators:create("N-CANDLE PREDICTOR SIGNAL", Source3, Period  ); 
    Indicator4 = core.indicators:create("N-CANDLE PREDICTOR SIGNAL", Source4, Period  ); 	
...
function ExtUpdate(id, source, period)  -- The method called every time when a new bid or ask price appears.
    ...
    then	
		    ...
		    period4= core.findDate (Source4, Source1:date(period), false); 
   ...
   -- get only time
    ...
    Indicator4:update(core.UpdateLast);	
    ...
    ExitFunction(now,period1, period2, period3, period4);
	end

end


function ExitFunction( now,period1, period2, period3, period4)
	...
	or not Indicator2.DATA:hasData( period2)
	or  not Indicator3.DATA:hasData( period3)
    or not Indicator4.DATA:hasData( period4)
	then
	return;
	end 
	 ...
	 return;
		            ...
		            if  Indicator1.DATA[period1]~= 1
					or Indicator2.DATA[period2]~= 1
					or Indicator3.DATA[period3]~= 1
					or Indicator4.DATA[period4]~= 1
					then
...
function EntryFunction( now,period1, period2, period3, period4)
    ...
    if   Indicator1.DATA[period1]== 1
	and Indicator2.DATA[period2]== 1
	and Indicator3.DATA[period3]== 1
	and Indicator4.DATA[period4]== 1	
	then	
          if Direction then
                BUY();
            else
		...
		Return=true;
    elseif   Indicator1.DATA[period1]== -1
	and Indicator2.DATA[period2]== -1
	and Indicator3.DATA[period3]== -1
	and Indicator4.DATA[period4]== -1
		then
