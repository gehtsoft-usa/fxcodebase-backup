...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=18806
...
function Init() --The strategy profile initialization
    strategy:name(" EC PIN BAR STRATEGY ")
    ...
    strategy.parameters:addInteger("EP", "EMA Period", "", 100)
    strategy.parameters:addInteger("CP", "CCI Period", "", 50)

    strategy.parameters:addDouble("BL", "CCI Buy Level", "", 0)
    strategy.parameters:addDouble("SL", "CCI Sell Level", "", 0)
    strategy.parameters:addInteger("Shift", "Pin Bar Shift", "", 0, 0, 1)
...
function Prepare(nameOnly)
    ...
    Indicator["CCI"] = core.indicators:create("CCI", Source, EP)
    Short["CCI"] = Indicator["CCI"]:getStream(0)

    first = math.max(Short["EMA"]:first(), Short["CCI"]:first())
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if
        Source.close[period] > Short["EMA"][period] and Short["CCI"][period] < BL and
            Source.close[period - Shift] == Source.high[period - Shift] and
            Source.close[period - Shift] > Source.median[period - Shift]
     then
        if Direction then
            BUY()
        else
    ...
    elseif
        Source.close[period] < Short["EMA"][period] and Short["CCI"][period] > SL and
            Source.close[period - Shift] == Source.low[period - Shift] and
            Source.close[period - Shift] < Source.median[period - Shift]
     then
        if Direction then
            SELL()
    ...
    if Source.close[period] < Short["EMA"][period] and Short["CCI"][period] < SL then
        if Direction then
    ...
    if Source.close[period] > Short["EMA"][period] and Short["CCI"][period] > BL then
        if Direction then
            if haveTrades("S") then
                exit("S")
                Signal("Close Short")
