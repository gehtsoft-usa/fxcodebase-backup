...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=24466
...
function Init() --The strategy profile initialization
    strategy:name("Engulfing Patterns Strategy")
    ...
    strategy.parameters:addBoolean("Complete", "Complete candle", "", true)
...
function Prepare(nameOnly)
    Complete = instance.parameters.Complete
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if Complete then
        if
            source.high[period - 1] <= source.close[period] and source.low[period - 1] >= source.open[period] and
        ...
        if
            source.high[period - 1] <= source.open[period] and source.low[period - 1] >= source.close[period] and
                source.close[period - 1] > source.open[period - 1] and
                source.open[period] > source.close[period]
         then
            Trade = "Short"
        ...
        if
            source.open[period - 1] <= source.close[period] and source.close[period - 1] >= source.open[period] and
                source.close[period - 1] < source.open[period - 1] and
                source.open[period] < source.close[period]
         then
            Trade = "Long"
        ...
        if
            source.close[period - 1] <= source.open[period] and source.open[period - 1] >= source.close[period] and
                source.close[period - 1] > source.open[period - 1] and
                source.open[period] > source.close[period]
         then
            Trade = "Short"
