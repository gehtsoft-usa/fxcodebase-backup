...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=66409
...
function Init() --The strategy profile initialization
    strategy:name("Ehlers Fisher Transform Ichimoku Chiokuo Span Strategy");
    ...
    strategy.parameters:addGroup("Ehlers Fisher Transform Ichimoku Calculation");
     strategy.parameters:addInteger("N", "Number of periods", "", 10);

     strategy.parameters:addString("Price", "Price source", "", "M2");
     strategy.parameters:addStringAlternative("Price", "Typical (H+L+C)/3", "", "M3");
     strategy.parameters:addStringAlternative("Price", "Median (H+L)/2", "", "M2");
     strategy.parameters:addStringAlternative("Price", "Close", "", "C");
	 
	 
	 
	 strategy.parameters:addGroup("Ichimoku Calculation");
...
function CreateTradingParameters()
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "EFTICSS");
...
function Prepare( nameOnly)
	...
	assert(core.indicators:findIndicator("EHLERS_FISHER_TRANSFORM") ~= nil, "Please, download and install EHLERS_FISHER_TRANSFORM.LUA indicator");          
    ...
    EFT = core.indicators:create("EHLERS_FISHER_TRANSFORM", Source, N, Price  );
	Ichimoku = core.indicators:create("ICH", Source, T,K,S  );
	first=math.max(Ichimoku.DATA:first(),EFT.Trigger:first()); 
...
function ExitFunction( now,period)
	 ...
	 return;
		            ...
		            if    Ichimoku.SA[period+K] < Ichimoku.SB[period+ K]  then
...
function EntryFunction( now,period)
    ...
    if  (EFT.Fisher[period] > EFT.Trigger[period] 
	     and EFT.Fisher[period-1] <= EFT.Trigger[period-1]
		 and Ichimoku.SA[period+K] > Ichimoku.SB[period+ K] 		 
		 )
		 
    or
	    (EFT.Fisher[period] > EFT.Trigger[period] 
		and  Ichimoku.SA[period+K] > Ichimoku.SB[period+ K] 	
		and  Ichimoku.SA[period+K-1] <= Ichimoku.SB[period+ K-1] 	
	     ) 
	then	
          if Direction then
                BUY();
		...
		Return=true;
    elseif  (EFT.Fisher[period] < EFT.Trigger[period] 
	     and EFT.Fisher[period-1] >= EFT.Trigger[period-1]
		 and Ichimoku.SA[period+K] < Ichimoku.SB[period+ K] 
		 )
