...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=62327
...
function Init() --The strategy profile initialization
    strategy:name("Elder Three Screen Strategy")
    ...
    strategy.parameters:addString("Method", "Price Source", "", "Three")
    strategy.parameters:addStringAlternative("Method", "Use One Time Frame", "", "One")
    strategy.parameters:addStringAlternative("Method", "Use Two Time Frames", "", "Two")
    strategy.parameters:addStringAlternative("Method", "Use Three Time Frames", "", "Three")
    ...
    strategy.parameters:addInteger("Period1", " Period", "Period", 14)
    ...
    strategy.parameters:addInteger("Period2", "Period", "Period", 14)
    ...
    strategy.parameters:addInteger("Period3", "Period", "Period", 14)
...
local first
local TF1, TF2, TF3, Method, Method1, Method2, Method3, Period1, Period2, Period3
local Indicator = {}
local Direction
local CustomID
...
function Prepare(nameOnly)
    ...
    Source[1] = ExtSubscribe(2, nil, TF1, instance.parameters.Type == "Bid", "bar")
    Source[2] = ExtSubscribe(3, nil, TF2, instance.parameters.Type == "Bid", "bar")
    Source[3] = ExtSubscribe(4, nil, TF2, instance.parameters.Type == "Bid", "bar")

    assert(core.indicators:findIndicator(Method1) ~= nil, Method1 .. " indicator must be installed");
    Indicator[1] = core.indicators:create(Method1, Source[1].close, Period1)
    assert(core.indicators:findIndicator(Method2) ~= nil, Method2 .. " indicator must be installed");
    Indicator[2] = core.indicators:create(Method2, Source[2].close, Period2)
    assert(core.indicators:findIndicator(Method3) ~= nil, Method3 .. " indicator must be installed");
    Indicator[3] = core.indicators:create(Method3, Source[3].close, Period3)
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if ExecutionType == "Live" then
        ...
        p[1] = core.findDate(Source[1].close, TickSource:date(period), false)
        p[2] = core.findDate(Source[2].close, TickSource:date(period), false)
        p[3] = core.findDate(Source[3].close, TickSource:date(period), false)

        if ONE == Source[1]:serial(p[1]) then
            return
        end
    else
        if id ~= 2 then
        ...
        p[1] = period
        p[2] = core.findDate(Source[2].close, Source[1]:date(period), false)
        p[3] = core.findDate(Source[3].close, Source[1]:date(period), false)
    end
    ...
    if
        not Indicator[1].DATA:hasData(p[1] - 2) or not Indicator[2].DATA:hasData(p[2] - 2) or
            not Indicator[3].DATA:hasData(p[3] - 2) or
            not Indicator[1].DATA:hasData(p[1] - 1) or
            not Indicator[2].DATA:hasData(p[2] - 1) or
            not Indicator[3].DATA:hasData(p[3] - 1) or
            not Indicator[1].DATA:hasData(p[1]) or
            not Indicator[2].DATA:hasData(p[2]) or
            not Indicator[3].DATA:hasData(p[3])
     then
        return
    end

    if p[1] <= 2 or p[2] <= 2 or p[3] <= 2 then
        return
    end

    -- only buy if we have a fast cross over slow and the price is above the moving averages.
    if
        Indicator[1].DATA[p[1]] > Indicator[1].DATA[p[1] - 1] and
            Indicator[1].DATA[p[1] - 1] <= Indicator[1].DATA[p[1] - 2] and
            Indicator[2].DATA[p[2]] > Indicator[2].DATA[p[2] - 1] and
            Indicator[3].DATA[p[3]] > Indicator[3].DATA[p[3] - 1]
     then
        if Direction then
            BUY()
        else
