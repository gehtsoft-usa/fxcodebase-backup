...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=64023
...
function Init() --The strategy profile initialization
    strategy:name("Extreme TMA Line Pivot Strategy")
    ...
    strategy.parameters:addGroup("Extreme TMA Line Slope Calculation")
    ...
    strategy.parameters:addDouble("BL", "Buy Level", "", 0.3)
    strategy.parameters:addDouble("SL", "Sell Level", "", -0.3)

    strategy.parameters:addGroup("Pivot Calculation")
    strategy.parameters:addString("BS", "Time Frame", "", "D1")
    strategy.parameters:setFlag("BS", core.FLAG_BARPERIODS)

    strategy.parameters:addString("CalcMode", "Mode", "", "Pivot")
...
local first
local TMA_Period, ATR_Period, ATR_Mult, TrendThreshold, Redraw
local TMA_Period1, ATR_Period1, ATR_Mult1, TrendThreshold1, Redraw1, Slope
local BL, SL
local Indicator
local Direction
local CustomID
...
function Prepare(nameOnly)
    ...
    first = math.max(Indicator.DATA:first(), Slope.DATA:first(), Pivot.DATA:first())
...
3. and price >daily pivot 
AND PRICE < RESISTANCE1


SELL;
1.EXTREME TMA SLOPE < SELL LEVEL 
2.and price cross under lower line 
3and price < daily pivot
 AND PRICE >SUPPORT1
    ...
    if not Indicator.DATA:hasData(period) or not Slope.DATA:hasData(period) or not Pivot.DATA:hasData(period) then
        return
    end

    -- only buy if we have a fast cross over slow and the price is above the moving averages.
    if
        Source.close[period] > Indicator.Upper[period] and Source.close[period - 1] <= Indicator.Upper[period - 1] and
            Slope.DATA[period] > BL and
            Source.close[period] > Pivot.DATA[period] and
            Source.close[period] < Pivot:getStream(5)[period]
     then
        if Direction then
            BUY()
        else
    ...
    elseif
        Source.close[period] < Indicator.Lower[period] and Source.close[period - 1] >= Indicator.Lower[period - 1] and
            Slope.DATA[period] < SL and
            Source.close[period] < Pivot.DATA[period] and
            Source.close[period] > Pivot:getStream(1)[period]
     then
        if Direction then
            SELL()
        else
