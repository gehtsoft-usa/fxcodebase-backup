...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=14745
...
function Init() --The strategy profile initialization
    strategy:name("EMA DMI STOCHASTIC STRATEGY")
    ...
    strategy.parameters:addInteger("SP", "Short Period", "", 12, 2, 1000)
    strategy.parameters:addInteger("MP", "Mid Period", "", 30, 2, 1000)
    strategy.parameters:addInteger("LP", "Long Period", "", 60, 2, 1000)
    strategy.parameters:addGroup("DMI Calculation")
    strategy.parameters:addInteger("DP", "Period", "", 14, 2, 1000)
    ...
    strategy.parameters:addDouble("OS", "Sell level", "", 50, 1, 100)
    strategy.parameters:addDouble("OB", "Buy level", "", 50, 1, 100)
...
local SP, MP, LP, DP, K, D, SD, A1, A2, OB, OS
...
function Prepare(nameOnly)
    ...
    Indicator[1] = core.indicators:create("EMA", Source[Price], SP)
    Short["SP"] = Indicator[1]:getStream(0)

    Indicator[2] = core.indicators:create("EMA", Source[Price], MP)
    Short["MP"] = Indicator[2]:getStream(0)

    Indicator[3] = core.indicators:create("EMA", Source[Price], LP)
    Short["LP"] = Indicator[3]:getStream(0)

    Indicator[4] = core.indicators:create("DMI", Source, DP)
    Short["DP"] = Indicator[4]:getStream(0)
    Short["DM"] = Indicator[4]:getStream(1)

    Indicator[5] = core.indicators:create("STOCHASTIC", Source, K, D, SD, A1, A2)
    Short["K"] = Indicator[5]:getStream(0)
    Short["D"] = Indicator[5]:getStream(1)

    first =
        math.max(Short["SP"]:first(), Short["MP"]:first(), Short["LP"]:first(), Short["DP"]:first(), Short["D"]:first())
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if
        Short["SP"][period] > Short["MP"][period] and Short["MP"][period] > Short["LP"][period] and
            Short["DP"][period] > Short["DM"][period] and
            Short["K"][period] < OB and
    ...
    if core.crossesUnder(Short["SP"], Short["MP"], period) then
        if Direction then
    ...
    if core.crossesOver(Short["SP"], Short["MP"], period) then
        if Direction then
            if haveTrades("S") then
                exit("S")
                Signal("Close Short")
