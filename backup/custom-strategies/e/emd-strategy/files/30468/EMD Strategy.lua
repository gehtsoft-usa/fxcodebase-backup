...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=16366
...
function Init() --The strategy profile initialization
    strategy:name("EMPIRICAL MODE DECOMPOSITION Strategy")
    ...
    strategy.parameters:addInteger("Period", "BandPass Filter Period", "", 20)
    strategy.parameters:addInteger("P2", "Threshold Period", "", 20)
    strategy.parameters:addDouble("Delta", "Delta", "", 0.5)
    strategy.parameters:addDouble("Fraction", "Fraction", "", 0.1)
...
function Prepare(nameOnly)
    ...
    name =
        name ..
        ", " .. tostring(Period) .. ", " .. tostring(P2) .. ", " .. tostring(Delta) .. ", " .. tostring(Fraction)

    assert(core.indicators:findIndicator("EMD") ~= nil, "Please, download and install EMD.LUA indicator")
    ...
    Indicator[1] = core.indicators:create("EMD", Source, Period, P2, Delta, Fraction)
    Short["C"] = Indicator[1]:getStream(0)
    Short["B"] = Indicator[1]:getStream(1)
    Short["T"] = Indicator[1]:getStream(2)

    first = math.max(Short["C"]:first(), Short["T"]:first())
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if core.crossesOver(Short["C"], Short["T"], period) then
        if Direction then
            BUY()
        else
            SELL()
        end
    elseif core.crossesUnder(Short["C"], Short["B"], period) then
        if Direction then
            SELL()
        else
            BUY()
    ...
    if core.crossesUnder(Short["C"], Short["T"], period) then
        if Direction then
    ...
    if core.crossesOver(Short["C"], Short["B"], period) then
        if Direction then
            if haveTrades("S") then
                exit("S")
                Signal("Close Short")
