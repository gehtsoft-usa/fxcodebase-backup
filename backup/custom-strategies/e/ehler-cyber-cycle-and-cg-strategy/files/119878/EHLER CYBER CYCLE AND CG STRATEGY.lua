...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=66254
...
function Init() --The strategy profile initialization
    strategy:name("EHLER CYBER CYCLE AND CG STRATEGY")
    ...
    strategy.parameters:addGroup("Ehlers CG Oscillator Calculation")
    strategy.parameters:addInteger("Length", "Length", "", 10)

    strategy.parameters:addDouble("BL1", "Buy Level", "", 0)
    strategy.parameters:addDouble("SL1", "Sell Level", "", 0)

    strategy.parameters:addGroup("Ehlers Cyber Cycle")
    strategy.parameters:addDouble("Alpha", "Alpha", "", 0.07)
...
function Prepare(nameOnly)
    ...
    assert(core.indicators:findIndicator("EHLERS_CG") ~= nil, "Please, download and install EHLERS_CG.LUA indicator")
    assert(
        core.indicators:findIndicator("EHLERS_CYBER_CYCLE") ~= nil,
        "Please, download and install EHLERS_CYBER_CYCLE.LUA indicator"
    )
    ...
    Indicator1 = core.indicators:create("EHLERS_CG", Source, Length)
    Indicator2 = core.indicators:create("EHLERS_CYBER_CYCLE", Source, Alpha)
...
function EntryFunction(now, period)
    ...
    if
        Indicator2.Buff1[period] > Indicator2.Buff2[period] and Indicator2.Buff2[period] > BL2 and
            Indicator1.Buff1[period] > Indicator1.Buff2[period] and
            Indicator1.Buff2[period] > BL1 and
            Indicator1.Buff2[period - 1] <= BL1
     then
        if Direction then
            BUY()
        else
        ...
        Return = true
    elseif
        Indicator2.Buff1[period] < Indicator2.Buff2[period] and Indicator2.Buff2[period] < SL2 and
            Indicator1.Buff1[period] < Indicator1.Buff2[period] and
            Indicator1.Buff2[period] < SL1 and
            Indicator1.Buff2[period - 1] >= SL1
     then
        if Direction then
            SELL()
        else
