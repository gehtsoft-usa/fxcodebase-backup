...
local STRATEGY_NAME = "EMA_RSI_STRATEGY";
function CreateParameters() 
    ...
    strategy.parameters:addDouble("rsi_buy", "Buy Level", "", 50);
    strategy.parameters:addDouble("rsi_sell", "Buy Level", "", 50);

    strategy.parameters:addInteger("atr_period", "ATR Period", "", 14)
end

local ma, rsi, rsi_buy, rsi_sell, atr;
...
function IsEntryLong(source, period)
    if not rsi.DATA:hasData(period) or not ma.DATA:hasData(period - 1) then
        return false;
    end
    return rsi.DATA[period] > rsi_buy and core.crossesOver(source.close, ma.DATA, period);
end
function IsEntryShort(source, period)
    if not rsi.DATA:hasData(period) or not ma.DATA:hasData(period - 1) then
        return false;
    end
    return rsi.DATA[period] < rsi_sell and core.crossesUnder(source.close, ma.DATA, period);
end
...
function Init()
    ...
    strategy.parameters:addDouble("trading_mult", "Trading Multiplier", "", 2.25);
    strategy.parameters:addDouble("max_lots", "Maximal Lots", "", 100);

    strategy.parameters:addGroup("Money Management");
    strategy.parameters:addBoolean("use_stop", "Set Stop", "", false);
    strategy.parameters:addDouble("stop_pips", "Stop multiplier", "", 1);
    ...
    strategy.parameters:addDouble("limit_pips", "Limit multiplier", "", 1);
...
local use_mandatory_closing, exit_time, current_amount, trading_mult, max_lots;
function Prepare(nameOnly)
    ...
    max_lots = instance.parameters.max_lots;
    trading_mult = instance.parameters.trading_mult;
    ...
    core.host:execute("subscribeTradeEvents", TRADES_ID, "trades")
end
    ...
    elseif cookie == TRADES_ID then
        ...
        if close_trade then
            ...
            if closed_trade ~= nil and closed_trade.OQTXT == custom_id then
                if closed_trade.PL < 0 then
                    current_amount = math.max(max_lots, current_amount * trading_mult);
                else
                    current_amount = Amount;
                end
            end
...
function OpenTrade(side)
    ...
    if amount_type == "lots" then
        valuemap.Quantity = math.floor(current_amount) * base_size;
    else
        local equity = core.host:findTable("accounts"):find("AccountID", valuemap.AcctID).Equity;
        local used_equity = equity * current_amount / 100.0;
    ...
    if use_stop then
        ...
        if side == "B" then
            valuemap.PegPriceOffsetPipsStop = -(atr.DATA[period] / atr.DATA:pipSize() * stop_pips);
        else
            valuemap.PegPriceOffsetPipsStop = (atr.DATA[period] / atr.DATA:pipSize() * stop_pips);
        end
    ...
    if use_limit then
        ...
        if side == "B" then
            valuemap.PegPriceOffsetPipsLimit = (atr.DATA[period] / atr.DATA:pipSize() * limit_pips);
        else
            valuemap.PegPriceOffsetPipsLimit = -(atr.DATA[period] / atr.DATA:pipSize() * limit_pips);
        end
    end
