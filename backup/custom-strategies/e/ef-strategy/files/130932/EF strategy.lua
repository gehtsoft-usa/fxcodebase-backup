...
--http://fxcodebase.com/code/viewtopic.php?f=31&t=69349
...
local STRATEGY_NAME = "EF Strategy";
...
function CreateParameters() 
    strategy.parameters:addInteger("FP", "Forward Testing Period ", "", 5);
    strategy.parameters:addInteger("BP", "Backward Testing Period", "", 5);
    strategy.parameters:addDouble("SIZE", "Current Candle Min. Size (pips)", "", 0);
    strategy.parameters:addDouble("NEXT", "Next Candle Min. Size (pips)", "", 0);
end
local indi;
function CreateEntryIndicators(source) 
    local profile = core.indicators:findIndicator("EF");
    assert(profile ~= nil, "Please, download and install " .. "EF" .. ".LUA indicator");
    local indicatorParams = profile:parameters();
    indicatorParams:setInteger("FP", instance.parameters.FP);
    indicatorParams:setInteger("BP", instance.parameters.BP);
    indicatorParams:setDouble("SIZE", instance.parameters.SIZE);
    indicatorParams:setDouble("NEXT", instance.parameters.NEXT);
    indi = core.indicators:create("EF", source, indicatorParams)
end
...
function CreateCustomActions()
    ...
    enterLongAction.IsPass = function (source, period, periodFromLast, data)
        local signal, date = GetLastSignal(source);
        return signal == 1;
    end
