...
-- https://fxcodebase.com/code/viewtopic.php?f=31&p=159577#p159577
...
function Init() --The strategy profile initialization
    strategy:name("Lines Cross Strategy")
...
function CreateTradingParameters()
    ...
    strategy.parameters:addDouble("loss_pos_mult", "Loss position amount multiplier", "", 1.0)
...
function Prepare(nameOnly)
    ...
    Indicator = core.indicators:create("MACD", Source.close, instance.parameters.P1, instance.parameters.P2)
...
function PrepareTrading()
    ...
    loss_pos_mult = instance.parameters.loss_pos_mult
...
function EntryFunction(id, now, period)
    ...
    if Indicator.MACD[period] > 0 and Indicator.MACD[period - 1] <= 0 then
        if Direction then
            BUY()
        else
            SELL()
        ...
        Return = true
    elseif Indicator.MACD[period] < 0 and Indicator.MACD[period - 1] >= 0 then
        if Direction then
            SELL()
        else
            BUY()
        ...
        if close_trade then
            ...
            if closed_trade ~= nil then
                ...
                if closed_trade.PL >= 0 then
                    ...
                    current_amount = math.min(max_lots, current_amount * loss_pos_mult);
                end
            end
        end
...
function MarketOrder(BuySell, hCount)
    ...
    if hCount > 0 then
        ...
        valuemap.Quantity = math.floor(current_amount) * BaseSize
    end
