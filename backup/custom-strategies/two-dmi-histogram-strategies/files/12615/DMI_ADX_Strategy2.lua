...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=5159
...
function Init() --The strategy profile initialization
    strategy:name("DMI ADX 2 strategy")
    strategy:description("DMI ADX 2 strategy")
    ...
    strategy.parameters:addDouble("ADXLevel", "Level ADX", "Level ADX", 20)
...
function Prepare(nameOnly)
    ...
    name =
        profile:id() ..
        "(" .. instance.bid:name() .. "." .. instance.parameters.TF .. "," .. "RSI(" .. instance.parameters.N .. "))"
    instance:name(name)
    if nameOnly then
        return;
    end
    ...
    assert(
        core.indicators:findIndicator("DMI_ADX_HISTOGRAM") ~= nil,
        "Please, download and install DMI_ADX_HISTOGRAM.LUA indicator"
    )

    Source = ExtSubscribe(2, nil, instance.parameters.TF, true, "bar")
    DMI_ADX = core.indicators:create("DMI_ADX_HISTOGRAM", Source, instance.parameters.N)
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    DMI_ADX:update(core.UpdateLast)

    -- Check that we have enough data
    if (DMI_ADX.DATA:first() > (period - 1)) then
        return
    end
    ...
    if DMI_ADX.BuffADX[period] > instance.parameters.ADXLevel and DMI_ADX.BuffDMI[period] > 0 then
    ...
    if DMI_ADX.BuffDMI[period - 1] > 0 and DMI_ADX.BuffDMI[period] < 0 then
        if instance.parameters.TypeSignal == "direct" then
            MustCloseB = true
        else
            MustCloseS = true
