...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=60818
...
function Init() --The strategy profile initialization
    ...
    strategy.parameters:addInteger("A1", "Period", "", 10)
    strategy.parameters:addDouble("A2", "K", "", 1)
    strategy.parameters:addString("A3", "Method", "", "High/Low")
    strategy.parameters:addStringAlternative("A3", "High/Low", "", "High/Low")
    strategy.parameters:addStringAlternative("A3", "Close", "", "Close")
    strategy.parameters:addInteger("A4", "Period_MA", "", 50)
    strategy.parameters:addInteger("A5", "Level", "", 20)
    strategy.parameters:addDouble("BuyLevel", "Buy Level", "", 0.5)
    strategy.parameters:addDouble("SellLevel", "Sell Level", "", 0.5)

    strategy.parameters:addGroup("Stochastic Parameters")

    strategy.parameters:addInteger("K", "%K Period", "", 5, 2, 1000)
    strategy.parameters:addInteger("D", "%D Periof", "", 3, 1, 1000)
    strategy.parameters:addInteger("SD", "%D slowing periods", "", 3, 1, 1000)

    strategy.parameters:addString("S1", "Smoothing method for %K", "", "MVA")
    strategy.parameters:addStringAlternative("S1", "MVA", "", "MVA")
    strategy.parameters:addStringAlternative("S1", "EMA", "", "EMA")
    strategy.parameters:addStringAlternative("S1", "MetaTrader", "", "MT")

    strategy.parameters:addString("S2", "Smoothing method for %D", "", "MVA")
    strategy.parameters:addStringAlternative("S2", "MVA", "", "MVA")
    strategy.parameters:addStringAlternative("S2", "EMA", "", "EMA")
...
function CreateTradingParameters()
    ...
    strategy.parameters:addString("ExecutionType", "End of Turn / Live", "", "Live")
    ...
    strategy.parameters:addInteger("MaxNumberOfPosition", "Max Number Of Position In One Direction", "", 100, 1, 200)
...
local first
local BuyLevel, SellLevel, A1, A2, A3, A4, A5
local ANINA
local CustomID
local K, D, OB, OS, Stochastic
...
function Prepare(nameOnly)
    ...
    A4 = instance.parameters.A4
    A5 = instance.parameters.A5
    ...
    ANINA = core.indicators:create("ANINA", Source, A1, A2, A3, A4, A5)
    ...
    iparams:setString("MVAT_K", instance.parameters:getString("S1"))
    iparams:setString("MVAT_D", instance.parameters:getString("S2"))
    Stochastic = iprofile:createInstance(Source, iparams)

    K = Stochastic:getStream(0) -- %K
    D = Stochastic:getStream(1) -- %D

    first = math.max(ANINA.BuffMin:first(), ANINA.BuffMid:first(), Stochastic.D:first()) + 1
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    --ANINA.BuffMin:first(),ANINA.BuffMid:first()
    if
        ANINA.BuffMin[period] > ANINA.BuffMid[period] and ANINA.BuffMin[period] > BuyLevel and
            core.crossesOver(D, OS, period)
     then
        --[[SELL:
    ...
    elseif
        ANINA.BuffMin[period] < ANINA.BuffMid[period] and ANINA.BuffMin[period] < SellLevel and
            core.crossesUnder(D, OB, period)
     then
        SELL()
        ONE = Source:serial(period)
    end
    ...
    if core.crossesUnder(ANINA.BuffMin, ANINA.BuffMid, period) or (core.crossesUnder(K, D, period) and D[period] > OB) then
        --[[EXIT SELL:
    ...
    elseif core.crossesOver(ANINA.BuffMin, ANINA.BuffMid, period) or (core.crossesOver(K, D, period) and D[period] < OS) then
