...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=62887
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if
        ((Fast.DATA[period] > Slow.DATA[period]) and (Fast.DATA[period - 1] < Slow.DATA[period - 1]) and
            (Fast.DATA[period + 1] > Slow.DATA[period + 1]))
     then
        if Direction then
            BUY()
        else
