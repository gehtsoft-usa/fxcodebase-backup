...
--http://fxcodebase.com/code/viewtopic.php?f=31&t=69189
...
local STRATEGY_NAME = "MA Cross Over with HTF Conformation";
...
function CreateParameters() 
    strategy.parameters:addInteger("ma1_1_period", "Average 1 Period", "", 14);
    AddAverages("ma1_1_method", "Average 1 Method", "MVA");

    strategy.parameters:addInteger("ma2_1_period", "Average 2 Period", "", 7);
    AddAverages("ma2_1_method", "Average 2 Method", "MVA");

    strategy.parameters:addString("higher_tf", "Higher Timeframe", "", "H1");
    strategy.parameters:setFlag("higher_tf", core.FLAG_PERIODS);
    strategy.parameters:addInteger("ma1_2_period", "Higher timeframe average 1 Period", "", 14);
    AddAverages("ma1_2_method", "Higher timeframe average 1 Method", "MVA");
    strategy.parameters:addInteger("ma2_2_period", "Higher timeframe average 2 Period", "", 7);
    AddAverages("ma2_2_method", "Higher timeframe average 2 Method", "MVA");
end
local ma1_tf1, ma2_tf1, ma1_tf2, ma2_tf2;
local tf2_source;
function CreateEntryIndicators(source) 
    ma1_tf1 = CreateAverages(instance.parameters.ma1_1_period, instance.parameters.ma1_1_method, source);
    ma2_tf1 = CreateAverages(instance.parameters.ma2_1_period, instance.parameters.ma2_1_method, source);
    tf2_source = trading_logic:SubscribeHistory(source.close:instrument(), instance.parameters.higher_tf, source:isBid());
    ma1_tf2 = CreateAverages(instance.parameters.ma1_2_period, instance.parameters.ma1_2_method, tf2_source);
    ma2_tf2 = CreateAverages(instance.parameters.ma2_2_period, instance.parameters.ma2_2_method, tf2_source);
end
function CreateExitIndicators(source) 
end

function UpdateIndicators()
    ma1_tf2:update(core.UpdateLast);
    ma2_tf2:update(core.UpdateLast);
    ma1_tf1:update(core.UpdateLast);
    ma2_tf1:update(core.UpdateLast);
end
...
function CreateCustomActions()
    if not DISABLE_EXIT then
        ...
        exitLongAction.IsPass = function (source, period, periodFromLast, data)
            return source.close[period] < source.open[period];
        end
    ...
    enterLongAction.IsPass = function (source, period, periodFromLast, data)
        core.host:trace(ma1_tf1.DATA[period] .. " " .. ma2_tf1.DATA[period] .. " " .. ma1_tf2.DATA[NOW] .. " " .. ma2_tf2.DATA[NOW])
        return ma1_tf1.DATA[period] > ma2_tf1.DATA[period]
            and ma1_tf2.DATA[NOW] > ma2_tf2.DATA[NOW];
    end
...
function EntryFunction(source, period)
    ...
    for _, action in ipairs(EntryActions) do
        ...
        core.host:trace(tostring(isPass));
