...
local STRATEGY_NAME = "Heiken Ashi Closing Strategy";
...
function IsEntryLong(source, period)
    return ha:getCandleOutput(0).open[period] < ha:getCandleOutput(0).close[period];
end
function IsEntryShort(source, period)
    return ha:getCandleOutput(0).open[period] > ha:getCandleOutput(0).close[period];
end
function IsExitLong(source, period)
    return ha:getCandleOutput(0).close[period - 1] > ha:getCandleOutput(0).close[period];
end
...
function OpenTrade(side)
    ...
    if use_stop then
        if side == "B" then
            valuemap.RateStop = main_source.low[NOW - 1] - main_source:pipSize() * stop_pips;
        else
            valuemap.RateStop = main_source.high[NOW - 1] + main_source:pipSize() * stop_pips;
        end
