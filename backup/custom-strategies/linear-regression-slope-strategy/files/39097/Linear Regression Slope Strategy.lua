...
function Init() --The strategy profile initialization
    strategy:name("Linear Regression Slope Strategy")
...
function Prepare(nameOnly)
    ...
    Indicator[1] = core.indicators:create("LRS", Source[Price], Period)
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if core.crossesOver(Indicator[1].DATA, 0, period) then
        if Direction then
            BUY()
        else
            SELL()
        end
    elseif core.crossesUnder(Indicator[1].DATA, 0, period) then
        if Direction then
            SELL()
        else
            BUY()
