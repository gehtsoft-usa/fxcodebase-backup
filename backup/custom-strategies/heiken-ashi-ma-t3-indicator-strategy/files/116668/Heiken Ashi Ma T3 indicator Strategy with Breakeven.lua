...
function CreateTradingParameters()
    ...
    strategy.parameters:addDouble("breakeven_when", "Breakeven Activation Value, in pips", "", 15);
    strategy.parameters:addDouble("breakeven_to", "Breakeven To, in pips", "", 5);
...
function ExtUpdate(id, source, period)  -- The method called every time when a new bid or ask price appears.
    ...
    breakeven:ExtUpdate(id, source, period);
...
function breakeven:CreateController()
    ...
    function controller:DoBreakeven()
        ...
        if trade.PL >= self._when then
            trading:MoveStop(self:getTo(), trade);
            self._executed = true;
            return false;
        end
        return true;
