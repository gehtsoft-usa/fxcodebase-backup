...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=60663
...
function Init() --The strategy profile initialization
    strategy:name("Macd Sar Stochastics Dmi Adx  Price Moving Average Strategy");
...
strategy:setTag("NonOptimizableParameters", "Email,SendEmail,SoundFile,RecurrentSound,PlaySound, ShowAlert");
    ...
    strategy.parameters:addGroup("SAR indicator parameters");   
	...
	strategy.parameters:addInteger("AdxPeriod", "Period", "", 14);
	strategy.parameters:addDouble("AdxLevel", "Level", "", 20);
	
	strategy.parameters:addGroup("DMI indicator parameters");   
	strategy.parameters:addInteger("DmiPeriod", "Period", "", 14);
	
	strategy.parameters:addGroup("MA indicator parameters");  
	strategy.parameters:addInteger("MaPeriod", "Period", "", 20);
	...
	strategy.parameters:addGroup("Stochastic indicator Calculation");
    strategy.parameters:addInteger("K", "Number of periods for %K","", 5, 2, 1000);
    strategy.parameters:addInteger("SD", "%D slowing periods","", 3, 2, 1000);
    strategy.parameters:addInteger("D", "Number of periods for %D","", 3, 2, 1000);

    strategy.parameters:addString("MVAT_K", "The type of smoothing algorithm for %K","", "MVA");
    ...
    strategy.parameters:addString("MVAT_D","The type of smoothing algorithm for %D","", "MVA");
...
function Prepare( nameOnly)
   CustomID = instance.parameters.CustomID;
    ...
    Source = ExtSubscribe(2, nil, instance.parameters.TF, instance.parameters.Type == "Bid", "bar");
     ...
     MACD = core.indicators:create("MACD", Source.close, instance.parameters.FastEMA, instance.parameters.SlowEMA, instance.parameters.Signal);
	 SAR =    core.indicators:create("SAR", Source, instance.parameters.Step ,  instance.parameters.Max );
	 ADX= core.indicators:create("ADX", Source, instance.parameters.AdxPeriod   );
	 DMI = core.indicators:create("DMI", Source, instance.parameters.DmiPeriod   );
    assert(core.indicators:findIndicator(instance.parameters.Method) ~= nil, instance.parameters.Method .. " indicator must be installed");
	 MA = core.indicators:create(instance.parameters.Method, Source.close, instance.parameters.MaPeriod   );
    ...
    first = math.max(MACD.HISTOGRAM:first(), SAR:getStream(0):first(), SAR:getStream(1):first(), ADX.DATA:first(), DMI.DATA:first(), MA.DATA:first(), Stochastic.D:first()) + 1;
...
function ExtUpdate(id, source, period)  -- The method called every time when a new bid or ask price appears.
   ...
   -- get only time
	...
	and (not(SAR.DN:hasData(period - 1)) and SAR.DN:hasData(period))
	and DMI.DIP[period]>DMI.DIM[period]
	and ADX.DATA[period]>AdxLevel
	...
	and (not(SAR.UP:hasData(period - 1)) and SAR.UP:hasData(period))
	and DMI.DIP[period]<DMI.DIM[period]
	and ADX.DATA[period]>AdxLevel
	and Source.close[period]< MA.DATA[period]
	and Stochastic.K[period]< Stochastic.D[period]
