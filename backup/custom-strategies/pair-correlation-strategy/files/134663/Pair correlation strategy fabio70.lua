...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=69920
...
function AddCondition(id)
    strategy.parameters:addString("cond_" .. id, "Condition " .. 1, "", ">");
    strategy.parameters:addStringAlternative("cond_" .. id, ">=", "", ">=");
    strategy.parameters:addStringAlternative("cond_" .. id, "<=", "", "<=");
end

-- START OF USER DEFINED SECTION
local STRATEGY_NAME = "Pair Correlation Strategy";
function CreateParameters() 
    strategy.parameters:addString("timeframe", "Timeframe 1", "", "m1");
    strategy.parameters:setFlag("timeframe", core.FLAG_PERIODS);
    strategy.parameters:addInteger("ma1_period", "MA1 Period", "", 14);
    AddAverages("ma1_method", "MA1 Method", "EMA");
    AddCondition(1);

    strategy.parameters:addString("instrument2", "Instrument 2", "", "EUR/USD");
    strategy.parameters:setFlag("instrument2", core.FLAG_INSTRUMENTS);
    strategy.parameters:addString("timeframe2", "Timeframe 2", "", "m1");
    strategy.parameters:setFlag("timeframe2", core.FLAG_PERIODS);
    strategy.parameters:addInteger("ma2_period", "MA2 Period", "", 14);
    AddAverages("ma2_method", "MA2 Method", "EMA");

    strategy.parameters:addString("instrument3", "Instrument 3", "", "EUR/USD");
    strategy.parameters:setFlag("instrument3", core.FLAG_INSTRUMENTS);
    strategy.parameters:addString("timeframe3", "Timeframe 3", "", "m1");
    strategy.parameters:setFlag("timeframe3", core.FLAG_PERIODS);
    strategy.parameters:addInteger("ma3_period", "MA3 Period", "", 14);
    AddAverages("ma3_method", "MA3 Method", "EMA");
end

local ma2, ma1, cond_1, ma3, source2, source3, source;
function CreateEntryIndicators(source)
    cond_1 = instance.parameters.cond_1;
    ma1 = CreateAverages(instance.parameters.ma1_period, instance.parameters.ma1_method, source);
    source2 = ExtSubscribe(4, instance.parameters.instrument2, instance.parameters.timeframe2, instance.parameters.type, "bar")
    ma2 = CreateAverages(instance.parameters.ma2_period, instance.parameters.ma2_method, source2);
    source3 = ExtSubscribe(5, instance.parameters.instrument3, instance.parameters.timeframe3, instance.parameters.type, "bar")
...
function IsEntryLong(source, period)
    if not ma1.DATA:hasData(NOW - 1) or not ma2.DATA:hasData(NOW - 1) then
        return false;
    end
    local cross1 = false;
    if cond_1 == "<=" then
        if ma1.DATA[period] < source.close[period] then
            return false;
        end
        if not core.crossesUnderOrTouch(source.close, ma1.DATA, period) then
            return false;
        end
        return source2.close[NOW] < ma2.DATA[NOW] 
            and source3.close[NOW] < ma3.DATA[NOW] 
    else
        if ma1.DATA[period] > source.close[period] then
            return false;
        end
        if not core.crossesOverOrTouch(source.close, ma1.DATA, period) then
            return false;
        end
        return source2.close[NOW] > ma2.DATA[NOW] 
            and source3.close[NOW] > ma3.DATA[NOW] 
...
function IsExitLong(source, period)
    ...
    if cond_1 == ">=" then
        ...
        cross1 = core.crossesUnderOrTouch(source.close, ma1.DATA, period);
    else
        if ma1.DATA[period] > source.close[period] then
            return false;
        end
        cross1 = core.crossesOverOrTouch(source.close, ma1.DATA, period);
    end
    local cross2 = false;
    if cond_2 == ">=" then
        if ma2.DATA[NOW] < source2.close[NOW] then
            return false;
        end
        cross2 = core.crossesUnderOrTouch(source2.close, ma2.DATA, NOW);
    else
        if ma2.DATA[NOW] > source2.close[NOW] then
            return false;
        end
        cross2 = core.crossesOverOrTouch(source2.close, ma2.DATA, period);
    end
