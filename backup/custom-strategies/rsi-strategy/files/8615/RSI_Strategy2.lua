...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=2828
...
function Init() --The strategy profile initialization
    ...
    strategy.parameters:addGroup("Strategy Parameters");
     strategy.parameters:addDouble("LevelB", "LevelB", "LevelB", 30);
    strategy.parameters:addDouble("LevelS", "LevelS", "LevelS", 70);
...
function CreateTradingParameters()
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "RSIS2");
...
function Prepare( nameOnly)
	...
	Direction = instance.parameters.Direction == "direct";
     ...
     LevelB = instance.parameters.LevelB;
    ...
    Source = ExtSubscribe(2, nil, instance.parameters.TF, instance.parameters.Type == "Bid", "bar");
     RSI = core.indicators:create("RSI", Source[Price], N);
...
function ExtUpdate(id, source, period)  -- The method called every time when a new bid or ask price appears.
   ...
   -- get only time
	...
	if period < first  then
	 ...
	 if RSI.DATA[period]>LevelB and RSI.DATA[period-1]<LevelB then
            if Direction then
                BUY();
            else
                SELL();
    ...
    if RSI.DATA[period]<LevelS and RSI.DATA[period-1]>LevelS  then
            if Direction then
                SELL();
            else
                BUY();
