...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=69698
--+------------------------------------------------------------------+
...
local UseOwnPositionsOnly = false;
...
local STRATEGY_NAME = "Longer Shorter Strategy";
...
function CreateCustomActions()
    if not DISABLE_EXIT then
        ...
        exitLongAction.IsPass = function (source, period, periodFromLast, data)
            if not indi.entry_sell:hasData(period - 1) then
                return;
            end
            return core.crossesUnder(source.close, indi.exit_buy, period);
        end
        ...
        exitShortAction.IsPass = function (source, period, periodFromLast, data)
            ...
            return core.crossesOver(source.close, indi.exit_sell, period);
        end
    ...
    enterLongAction.IsPass = function (source, period, periodFromLast, data)
        ...
        return core.crossesOver(source.close, indi.entry_buy, period);
    end
    ...
    enterShortAction.IsPass = function (source, period, periodFromLast, data)
        ...
        return core.crossesUnder(source.close, indi.entry_sell, period);
    end
