...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=18482
...
function Init() --The strategy profile initialization
    strategy:name("GHLA Super Trend Strategy")
    ...
    strategy.parameters:addInteger("G", "GHLA Period", "", 10)
    strategy.parameters:addInteger("S", "ST Period", "", 10)
    strategy.parameters:addDouble("M", "ST Multiplier", "", 1.5)
...
function CreateTradingParameters()
    ...
    --	strategy.parameters:addInteger("UpdatePeriod", "Live Execution Update Period (in Seconds)", "", 1);
...
function Prepare(nameOnly)
    ...
    --UpdatePeriod = instance.parameters.UpdatePeriod;
    LastEntry = nil
    LastExit = nil
    --*********************************************************************************************************
    ...
    GHLA = core.indicators:create("GHLA", Source, G)
    ST = core.indicators:create("ST", Source, S, M)

    first = math.max(GHLA.DATA:first(), ST.DATA:first())
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if not ST.DATA:hasData(period) or not GHLA.DATA:hasData(period) then
        return
    end
...
function ExitFunction(period, now)
    ...
    if Source.close[period] < ST.DATA[period] or Source.close[period] < GHLA.DATA[period] then
        if Direction then
...
function EntryFunction(period, now)
    ...
    if
        Source.close[period] > ST.DATA[period] and Source.close[period] > GHLA.DATA[period] and
            (Source.close[period - 1] < ST.DATA[period - 1] or Source.close[period - 1] < GHLA.DATA[period - 1])
     then
        if Direction then
            BUY()
        else
