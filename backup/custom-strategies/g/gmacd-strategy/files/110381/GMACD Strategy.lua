...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=64271
...
function Prepare(nameOnly)
    ...
    assert(core.indicators:findIndicator("GMACD") ~= nil, "Please, download and install GMACD.LUA indicator")
    ...
    Indicator = core.indicators:create("GMACD", Source.close, SN, LN, IN)
...
function ExitFunction(period, now)
    ...
    if Indicator.H[period] < Indicator.SIGNAL[period] and Indicator.H[period - 1] >= Indicator.SIGNAL[period - 1] then
        if Direction then
...
function EntryFunction(period, now)
    --  HISTOGRAM[period] > 0 and HISTOGRAM[period] > SIGNAL[period]
    -- HISTOGRAM[period] < 0 and HISTOGRAM[period] < SIGNAL[period]
    ...
    if
        Indicator.H[period] > 0 and Indicator.H[period] > Indicator.SIGNAL[period] and
            (Indicator.H[period - 1] <= 0 or Indicator.H[period - 1] <= Indicator.SIGNAL[period - 1])
     then
        if Direction then
            BUY()
        else
