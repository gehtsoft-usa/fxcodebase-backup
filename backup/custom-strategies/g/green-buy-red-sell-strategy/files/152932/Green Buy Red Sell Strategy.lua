...
function CreateTradingParameters()
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "GBRSS");
...
function EntryFunction( id, now,period)
	...
	then	
		...
		Return=true;
    elseif Source.close[period-1]< Source.open[period-1] 
    and  (not Filter or (Filter and Source.close[period] <  MA.DATA[period]))	
	then
            if Direction then
                SELL(period);
