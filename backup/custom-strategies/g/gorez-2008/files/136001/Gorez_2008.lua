...
local STRATEGY_NAME = "Gorez 2008 Strategy";
...
function CreateEntryIndicators(source)
    macd = core.indicators:create("MACD", source, instance.parameters.macd_SN, instance.parameters.macd_LN);
end
...
function IsEntryLong(source, period)
    ...
    return core.crossesOver(macd.MACD, 0, period) and source.close[period] > source.high[period - 1];
end
function IsEntryShort(source, period)
    if not macd.MACD:hasData(period - 1) then
        return;
    end
    return core.crossesUnder(macd.MACD, 0, period) and source.close[period] < source.low[period - 1];
end
function IsExitLong(source, period)
    if not macd.SIGNAL:hasData(period - 1) then
        return;
    end
    return core.crossesOver(macd.SIGNAL, 0, period);
end
function IsExitShort(source, period)
    if not macd.SIGNAL:hasData(period - 1) then
        return;
    end
    return core.crossesUnder(macd.SIGNAL, 0, period);
end
