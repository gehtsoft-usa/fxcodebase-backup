...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=62262
...
function Init() --The strategy profile initialization
    strategy:name("Generic Overlay Strategy")
    ...
    strategy.parameters:addGroup("Data Selection")
    strategy.parameters:addString("INDICATOR", "Indicator", "", "MVA")
    strategy.parameters:setFlag("INDICATOR", core.FLAG_INDICATOR)
    strategy.parameters:addInteger("Number", "Data Stream Number", "", 1, 1, 100)

    strategy.parameters:addString("IndicatorType", "Type", "Type", "Slope")
    strategy.parameters:addStringAlternative("IndicatorType", "Slope", "Slope", "Slope")
    strategy.parameters:addStringAlternative(
        "IndicatorType",
        "Position relation to the signal line",
        "Position relation to the signal line",
        "Signal"
    )
    strategy.parameters:addStringAlternative("IndicatorType", "Level", "Level", "Level")
    strategy.parameters:addStringAlternative(
        "IndicatorType",
        "Overbought / Oversold",
        "Overbought / Oversold",
    ...
    strategy.parameters:addInteger("Overbought", "Overbought Level", "", 80)
    strategy.parameters:addInteger("Oversold", "Oversold Level", "", 20)
    strategy.parameters:addInteger("Level", "Level", "", 0)

    strategy.parameters:addGroup("Signal Line")

    strategy.parameters:addInteger("Period", "Smoothing Period", "", 20, 1, 1000)
...
local INDICATOR, Number, Type, Overbought, Level, Period, Method, Exit
local INDEX, FIRST, MA
local Indicator
local Short = {}
local Direction
local CustomID
...
function Prepare(nameOnly)
    ...
    IndicatorType = instance.parameters.IndicatorType
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    local TradeSignal = nil
    local PreviousTradeSignal = nil
    ...
    elseif IndicatorType == "Slope" then
    ...
    elseif IndicatorType == "Level" then
    ...
    elseif IndicatorType == "Overbought/Oversold" then
    ...
    if IndicatorType == "Signal" then
        if INDEX[period - 1] > MA.DATA[period - 1] then
            PreviousTradeSignal = 1
        elseif INDEX[period - 1] < MA.DATA[period - 1] then
            PreviousTradeSignal = -1
        else
            PreviousTradeSignal = 0
        end
    elseif IndicatorType == "Slope" then
        if INDEX[period - 1] > INDEX[period - 2] then
            PreviousTradeSignal = 1
        elseif INDEX[period - 1] < INDEX[period - 2] then
            PreviousTradeSignal = -1
        else
            PreviousTradeSignal = 0
        end
    elseif IndicatorType == "Level" then
        if INDEX[period - 1] > Level then
            PreviousTradeSignal = 1
        elseif INDEX[period - 1] < Level then
            PreviousTradeSignal = -1
        else
            PreviousTradeSignal = 0
        end
    elseif IndicatorType == "Overbought/Oversold" then
        if INDEX[period - 1] > Overbought then
            PreviousTradeSignal = 1
        elseif INDEX[period - 1] < Oversold then
            PreviousTradeSignal = -1
        else
            PreviousTradeSignal = 0
        end
    ...
    elseif TradeSignal == -1 and PreviousTradeSignal ~= -1 then
        if Direction then
            SELL()
        else
            BUY()
