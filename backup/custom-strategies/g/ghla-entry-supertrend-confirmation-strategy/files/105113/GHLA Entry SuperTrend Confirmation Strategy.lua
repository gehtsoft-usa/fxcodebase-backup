...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=63214
...
function Init() --The strategy profile initialization
    strategy:name("GHLA Entry SuperTrend Confirmation Strategy")
    ...
    strategy.parameters:addGroup("Super Trend Calculation")
    strategy.parameters:addInteger("N", "Number of periods", "", 40)
    strategy.parameters:addDouble("M", "Multiplier", "", 7)

    strategy.parameters:addGroup("GHLA Calculation")
    strategy.parameters:addInteger("PERIOD", "Period", "", 40, 1, 2000)
...
function Prepare(nameOnly)
    ...
    assert(
        core.indicators:findIndicator("GHLA  TOUCHLINE") ~= nil,
        "Please, download and install GHLA  TOUCHLINE.LUA indicator"
    )
    ...
    GHLA = core.indicators:create("GHLA  TOUCHLINE", Source, PERIOD, Method, core.rgb(0, 255, 0), core.rgb(255, 0, 0))
    first = math.max(ST.DATA:first(), GHLA.DATA:first())
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    --[[
		BUY; 
	1. PRICE > SUPER TREND IN PAST 5 BARS AND
	2. GHLA GIVES BUY SIGNAL
	SELL;
	1.PRICE < SUPER TREND IN PAST 5 BARS AND
	2.GHLA GIVES SELL SIGNAL

	]]
    if
        ST.DATA:colorI(period) == core.rgb(0, 255, 0) and GHLA.DATA:colorI(period) == core.rgb(0, 255, 0) and
            GHLA.DATA:colorI(period - 1) ~= core.rgb(0, 255, 0)
     then
        if Direction then
            BUY()
        else
    ...
    elseif
        ST.DATA:colorI(period) == core.rgb(255, 0, 0) and GHLA.DATA:colorI(period) == core.rgb(255, 0, 0) and
            GHLA.DATA:colorI(period - 1) ~= core.rgb(255, 0, 0)
     then
        if Direction then
            SELL()
        else
    ...
    --[[
	EXIT BUY:SUPERTREND GIVES SELL SIGNAL
	EXIT SELL: SUPER TREND GIVES BUY SIGNAL
	]]
    if Exit then
        if ST.DATA:colorI(period) ~= core.rgb(0, 255, 0) then
            if Direction then
        ...
        if ST.DATA:colorI(period) ~= core.rgb(255, 0, 0) then
            if Direction then
                if haveTrades("S") then
                    exitSpecific("S")
                    Signal("Close Short")
