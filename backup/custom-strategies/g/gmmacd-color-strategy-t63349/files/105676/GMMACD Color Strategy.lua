...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=63349
...
function Init() --The strategy profile initialization
    strategy:name("GMMACD Color Strategy")
    ...
    strategy.parameters:addInteger("Period", "MA Period", "", 9, 1, 1000)
...
function CreateTradingParameters()
    ...
    strategy.parameters:addDouble("Percentage1", "1. Percentage", "", 1, 1, 100)
    strategy.parameters:addDouble("Percentage2", "2. Percentage", "", 2, 1, 100)
...
function Prepare(nameOnly)
    ...
    assert(
        core.indicators:findIndicator("GMMACD COLOR") ~= nil,
        "Please, download and install GMMACD COLOR.LUA indicator"
    )
    ...
    Indicator = core.indicators:create("GMMACD COLOR", Source, Period, true)
...
function PrepareTrading()
    ...
    Equity = core.host:findTable("accounts"):find("AccountID", Account).Equity
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if (Indicator.DATA[period] > 0 and Indicator.DATA[period] <= 0) then
        if Direction then
            BUY(Percentage1)
        else
            SELL(Percentage1)
    ...
    elseif
        (Indicator.DATA[period] > Indicator.SIGNAL[period] and
            Indicator.DATA[period - 1] <= Indicator.SIGNAL[period - 1])
     then
        if Direction then
            BUY(Percentage2)
        else
        ...
        ONE = Source:serial(period)
    elseif (Indicator.DATA[period] < 0 and Indicator.DATA[period] >= 0) then
        if Direction then
            SELL(Percentage1)
        else
            BUY(Percentage1)
    ...
    if Exit then
        if Indicator.DATA[period] < 0 or Indicator.DATA[period] < Indicator.SIGNAL[period] then
            if Direction then
...
function enter(BuySell, Percentage)
    ...
    return MarketOrder(BuySell, Percentage)
end

-- enter into the specified direction
function MarketOrder(BuySell, Percentage)
    ...
    local SubValue = win32.formatNumber((((Equity / 100) * Percentage) / BaseSize), false, 0)
    valuemap.Quantity = SubValue * BaseSize
    --Amount * BaseSize;
