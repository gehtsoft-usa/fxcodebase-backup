...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=62341
...
function Init() --The strategy profile initialization
    strategy:name("GHLA_EMA_Strategy")
    ...
    strategy.parameters:addInteger("Renko_Step", "Renko step", "", 5)
    strategy.parameters:addInteger("EMA_Period", "MA period", "", 15)
    ...
    strategy.parameters:addInteger("GHLA_Period", "GHLA period", "", 20)
...
function CreateTradingParameters()
    ...
    strategy.parameters:addInteger("Amount1", "Trade Amount in Lots for Trend trade", "", 10)
    strategy.parameters:addInteger("Amount2", "Trade Amount in Lots for Contra trend trade", "", 3)
...
local Direction
local CustomID
local Renko_Step, GHLA_Period, Method, Ind_EMA
local Indicator
local Short = {}
local Ind_EMA, Ind_GHLA, Prev_GHLA, pdir, Prev_Pr
...
function Prepare(nameOnly)
    ...
    GHLA_Period = instance.parameters.GHLA_Period
    ...
    Ind_EMA = core.indicators:create(Method, Indicator.close, instance.parameters.EMA_Period)
    Short[1] = Indicator.open
    Short[2] = Indicator.close
    pdir = nil
    Prev_GHLA = nil
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    Ind_EMA:update(core.UpdateLast)
    local EMA_L = Ind_EMA.DATA:size() - 1
    ...
    if P > 0 and LastRenkoTime ~= Short[1]:date(P) then
        ...
        local AvgHigh = mathex.avg(Indicator.high, P - GHLA_Period + 1, P)
        local AvgLow = mathex.avg(Indicator.low, P - GHLA_Period + 1, P)

        -- change direction in case the current close is above average high
        -- or below average low
        if Indicator.close[P] > AvgHigh then
            switch = 1
        elseif Indicator.close[P] < AvgLow then
            switch = -1
        end
        ...
        if Prev_GHLA ~= nil then
            if Indicator.close[P] > Ind_EMA.DATA[EMA_L] then
                if Prev_Pr <= Prev_GHLA and Indicator.close[P] > Ind_GHLA then
                    if CanClose then
                        exitSpecific("S")
                        --enter("B", Amount1-ordersCount());
                        if Direction then
                            BUY(Amount1 - ordersCount())
                        else
                            SELL(Amount1 - ordersCount())
                        end
                        ONE = P
                    else
                        -- enter("B", Amount1-ordersCount());
                ...
                elseif Prev_Pr >= Prev_GHLA and Indicator.close[P] < Ind_GHLA then
                    if CanClose then
                        exitSpecific("S")
                        -- enter("B", Amount2);
                        if Direction then
                        ...
                        -- enter("B", Amount2-ordersCount());
                        if Direction then
                            BUY(Amount2 - ordersCount())
                        else
                            SELL(Amount2 - ordersCount())
                        end
                        ONE = P
                    end
                end
...
function ordersCount()
    ...
    while row ~= nil do
        if row.AccountID == Account and row.OfferID == Offer and row.CustomID == CustomID then
            if row.BS == "B" then
                count = count + row.AmountK
            else
                count = count - row.AmountK
            end
        end
...
function enter(BuySell, Amount)
    ...
    return MarketOrder(BuySell, Amount)
end

-- enter into the specified direction
function MarketOrder(BuySell, Amount)
