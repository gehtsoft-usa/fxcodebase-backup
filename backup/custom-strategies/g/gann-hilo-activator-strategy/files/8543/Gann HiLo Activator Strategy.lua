...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=3562
...
function Init()
    strategy:name("Gann HiLo Activator strategy")
    strategy:description("Gann HiLo Activator strategy")
    ...
    strategy.parameters:addInteger("Frame", "Gann HiLo Activator Period", "", 10, 2, 10000)
...
function Prepare(nameOnly)
    ...
    indicator = core.indicators:create("GHLA", gSource, Frame)
...
function ExtUpdate(id, source, period)
    ...
    if core.crossesUnder(indicator.DATA, gSource.close, period) then
        BUY()
    elseif core.crossesOver(indicator.DATA, gSource.close, period) then
        SELL()
    end
end
