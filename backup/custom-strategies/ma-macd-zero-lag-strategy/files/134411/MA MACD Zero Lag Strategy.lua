...
--http://fxcodebase.com/code/viewtopic.php?f=31&t=69936
...
function CreateParameters() 
    ...
    strategy.parameters:addInteger("FMA", "Periode de l'EMA rapide", "", 12, 1, 1000);
    strategy.parameters:addInteger("SMA", "Periode de l'EMA lente", "", 26, 1, 1000);
    strategy.parameters:addInteger("SigMA", "Periode de l'EMA du Signal", "", 9, 1, 1000);
    strategy.parameters:addDouble("HIS_neutral", "Zone Neutre","", 1.0, 0, 100);
    
	strategy.parameters:addInteger("P_EMA", "Periode de cette EMA", "", 9, 1, 1000);
    strategy.parameters:addBoolean("HISTOFILTRE", "Filtrer l'histogramme ?", "Grise l'histogramme si sa couleur ne correspond pas à la couleur de la courbe MACD", true);
end
...
function CreateEntryIndicators(source)
    ...
    local profile = core.indicators:findIndicator("MACD_ZERO_LAG");
    assert(profile ~= nil, "Please, download and install " .. "MACD_ZERO_LAG" .. ".LUA indicator");
    local indicatorParams = profile:parameters();
    indicatorParams:setBoolean("ON1", true);
    indicatorParams:setBoolean("HISTOFILTRE", instance.parameters.HISTOFILTRE);
    indicatorParams:setInteger("P_EMA", instance.parameters.P_EMA);
    indicatorParams:setInteger("SigMA", instance.parameters.SigMA);
    indicatorParams:setInteger("SMA", instance.parameters.SMA);
    indicatorParams:setInteger("FMA", instance.parameters.FMA);
    indicatorParams:setDouble("HIS_neutral", instance.parameters.HIS_neutral);
    macd = core.indicators:create("MACD_ZERO_LAG", source, indicatorParams)
end
...
function IsEntryLong(source, period)
    if not macd.MACD:hasData(period - 1) or not ma.DATA:hasData(period) then
        return false;
    end
    return source.close[period] > ma.DATA[period]
	    and   macd.MACD[period] < 0
        and core.crossesOver(macd.MACD, macd.SIG, period);
end
function IsEntryShort(source, period)
    ...
    return source.close[period] < ma.DATA[period]
	    and   macd.MACD[period] > 0
        and core.crossesUnder(macd.MACD, macd.SIG, period);
end
