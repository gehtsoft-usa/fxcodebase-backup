...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=27944
...
function Init() --The strategy profile initialization
    strategy:name("FAN of EMA Strategy")
    ...
    strategy.parameters:addInteger("Period", "First period", "", 7)
    ...
    strategy.parameters:addInteger("K", "K", "", 2)
    strategy.parameters:addInteger("N", "Count of MA", "", 14)

    strategy.parameters:addGroup("Open Parameters")
    strategy.parameters:addInteger("OpenOne", "First Fan Line Number", "", 1)
    strategy.parameters:addInteger("OpenTwo", "Second Fan Line Number", "", 10)
    strategy.parameters:addInteger("OpenPrice", "Price Cross  Fan Line Number", "", 1)

    strategy.parameters:addGroup("Exit Parameters")
    strategy.parameters:addBoolean("Exit", "Use Optinal Exit", "", true)
    strategy.parameters:addBoolean("One", "Use MA/MA Exit", "", true)

    strategy.parameters:addInteger("ExitOne", "First Fan Line Number", "", 1)
    strategy.parameters:addInteger("ExitTwo", "Second Fan Line Number", "", 10)

    strategy.parameters:addBoolean("Two", "Use MA/Price Cross Exit", "", true)
    strategy.parameters:addInteger("ExitPrice", "Price Cross Fan Line Number", "", 1)
...
local Source
local Exit, ExitOne, ExitTwo, ExitPrice
local SoundFile = nil
...
local Period, Method, K, N
local OpenOne, OpenTwo, OpenPrice
...
function Prepare(nameOnly)
    ...
    OpenOne = instance.parameters.OpenOne
    OpenTwo = instance.parameters.OpenTwo
    OpenPrice = instance.parameters.OpenPrice
    ...
    ExitOne = instance.parameters.ExitOne
    ExitTwo = instance.parameters.ExitTwo
    ExitPrice = instance.parameters.ExitPrice

    -- NG: replace string comparison everytime in future.
    Direction = instance.parameters.Direction == "direct"

    assert(OpenOne <= N, " Open One must be less than N (" .. N .. ")")
    assert(OpenTwo <= N, " Open Two must be less than N (" .. N .. ")")
    assert(OpenPrice <= N, " Open Price must be less than N (" .. N .. ")")

    assert(ExitOne <= N, " Exit One must be less than N (" .. N .. ")")
    assert(ExitTwo <= N, " Exit Two must be less than N (" .. N .. ")")
    assert(ExitPrice <= N, " Exit Price must be less than N (" .. N .. ")")

    assert(ExitOne ~= ExitTwo, " Exit One and  Exit Two can not be the same.")
    assert(OpenOne ~= OpenTwo, " Open One and  Open Two can not be the same")
    ...
    name = name .. ", " .. Period .. ", " .. Method .. ", " .. K .. ", " .. N

    assert(core.indicators:findIndicator("FAN_MA") ~= nil, "Please, download and install FAN_MA.LUA indicator")
    ...
    Indicator[1] = core.indicators:create("FAN_MA", Source[Price], Period, Method, K, N)

    for i = 1, N, 1 do
        Short[i] = Indicator[1]:getStream(i - 1)
        first = math.max(first, Short[i]:first())
    end
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if Short[OpenOne][period] > Short[OpenTwo][period] and core.crossesOver(Source.close, Short[OpenPrice], period) then
        if Direction then
            BUY()
        else
            SELL()
        end
    elseif Short[OpenOne][period] < Short[OpenTwo][period] and core.crossesUnder(Source.close, Short[OpenPrice], period) then
        if Direction then
            SELL()
        else
            BUY()
    ...
    if Exit then
        if
            (Short[ExitOne][period] < Short[ExitTwo][period] and One) or
                (core.crossesUnder(Source.close, Short[ExitPrice], period) and Two)
         then
            if Direction then
        ...
        if
            (Short[ExitOne][period] > Short[ExitTwo][period] and One) or
                (core.crossesOver(Source.close, Short[ExitPrice], period) and Two)
         then
            if Direction then
                if haveTrades("S") then
                    exit("S")
