...
-- https://fxcodebase.com/code/viewtopic.php?f=31&t=72134
...
function Init() --The strategy profile initialization
    ...
    strategy.parameters:addInteger("Period", "Period", "", 21);  
    strategy.parameters:addInteger("Lookback", "Lookback", "", 2);  	
...
--	strategy.parameters:addStringAlternative("ExitExecutionType", "Live", "", "Live");
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "HMAS");
...
function Prepare( nameOnly)
    ...
    Indicator = core.indicators:create("HMA", Source.close,Period );
	first=Indicator.DATA:first()+Lookback; 
...
function EntryFunction( id, now,period)
    ...
    local delta = Indicator.DATA[period-1] - Indicator.DATA[period-1 - Lookback];
    local delta_per_bar = delta / Lookback;
    local next_bar = Indicator.DATA[period-1] + delta_per_bar;
    
	local  concavity =0;
	
    if Indicator.DATA[period] > next_bar then
	concavity = 1; 
	else
	concavity = -1;
    end 
	...
	local delta1 = Indicator.DATA[period-2] - Indicator.DATA[period - 2 - Lookback];
    local delta_per_bar1 = delta1 / Lookback;
    local next_bar1 = Indicator.DATA[period-2] + delta_per_bar1;
    
	local  concavity1 =0;
	
    if Indicator.DATA[period-1] > next_bar1 then
	concavity1 = 1; 
	else
	concavity1 = -1;
    end 
    ...
    if   concavity== 1 and concavity1~=concavity
	then	
          if Direction then
                BUY();
            else
		...
		Return=true;
    elseif concavity ==-1  and concavity1~=concavity
	then
            if Direction then
                SELL();
            else
