...
function Init() --The strategy profile initialization
    ...
    strategy.parameters:addBoolean("SignalBoolean", "MACD/Signal Signal", "", true);  
    strategy.parameters:addBoolean("HistogramBoolean", "Histogram Signal", "", true);  	
    strategy.parameters:addInteger("Period1", "Short Period", "", 12);  
    strategy.parameters:addInteger("Period2", "Long Period", "", 26);  
...
local MACD;
local HistogramBoolean, SignalBoolean;
...
function Prepare( nameOnly)
	...
	--Indicator parameters 
    HistogramBoolean= instance.parameters.HistogramBoolean;
	SignalBoolean= instance.parameters.SignalBoolean;
...
function EntryFunction( id, now,period)
    ...
    if  ( SignalBoolean and MACD.MACD[period] > MACD.SIGNAL[period]and MACD.MACD[period-1] <= MACD.SIGNAL[period-1] )
	or ( HistogramBoolean  and MACD.HISTOGRAM[period] > MACD.HISTOGRAM[period-1] and MACD.HISTOGRAM[period-1] <= MACD.HISTOGRAM[period-2] )
	then	
          if Direction then
                BUY();
            else
		...
		Return=true;
    elseif  ( SignalBoolean and MACD.MACD[period] < MACD.SIGNAL[period]and MACD.MACD[period-1] >= MACD.SIGNAL[period-1] )
	or ( HistogramBoolean  and MACD.HISTOGRAM[period] < MACD.HISTOGRAM[period-1] and MACD.HISTOGRAM[period-1] >= MACD.HISTOGRAM[period-2] )
		then
            if Direction then
                SELL();
