...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=14698
...
function Init() --The strategy profile initialization
    strategy:name("GHLA SuperTrend Strategy")
    ...
    strategy.parameters:addInteger("SSLFrame", "SSL Number of periods", "SSL The number of periods.", 10, 2, 1000)
    ...
    strategy.parameters:addString("EXIT", "Don't use Exit algo", "", "Not")
    strategy.parameters:addStringAlternative("EXIT", "Don't use Exit algo", "", "Not")
    strategy.parameters:addStringAlternative("EXIT", "Super Trend", "", "ST")
    strategy.parameters:addStringAlternative("EXIT", "Gann High Low Activator", "", "GHLA")
    strategy.parameters:addStringAlternative("EXIT", "Exit on GHLA or ST", "", "ANY")
...
function Prepare(nameOnly)
    ...
    name = name .. ", " .. SSLFrame .. ", " .. STFrame .. ", " .. Multiplier
    ...
    Indicator["GHLA"] = core.indicators:create("GHLA", Source, SSLFrame)
    Short["GHLA"] = Indicator["GHLA"]:getStream(0)

    Indicator["ST"] = core.indicators:create("ST", Source, STFrame, Multiplier)
    Short["ST"] = Indicator["ST"]:getStream(0)

    first = math.max(Short["GHLA"]:first(), Short["ST"]:first())
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    Indicator["GHLA"]:update(core.UpdateLast)
    Indicator["ST"]:update(core.UpdateLast)
    ...
    if
        (core.crossesOver(Source.close, Short["GHLA"], period) and Short["ST"][period] < Source.close[period]) or
            (core.crossesOver(Source.close, Short["ST"], period) and Short["GHLA"][period] < Source.close[period])
     then
        if Direction then
            BUY()
        else
    ...
    if EXIT == "ST" or EXIT == "ANY" then
        if core.crossesUnder(Source.close, Short["ST"], period) then
            if Direction then
        ...
        if core.crossesOver(Source.close, Short["ST"], period) then
            if Direction then
                if haveTrades("S") then
                    exit("S")
                    Signal("Close Short")
    ...
    if EXIT == "GHLA" or EXIT == "ANY" then
        if core.crossesUnder(Source.close, Short["GHLA"], period) then
            if Direction then
                if haveTrades("B") then
                    exit("B")
                    Signal("Close Long")
        ...
        if core.crossesOver(Source.close, Short["GHLA"], period) then
            if Direction then
                if haveTrades("S") then
                    exit("S")
                    Signal("Close Short")
