...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=63927
...
function Init() --The strategy profile initialization
    ...
    strategy.parameters:addInteger("CCI_Period", "Period", "", 44)
    strategy.parameters:addInteger("Min_Max_Period", "Min Max Period", "", 200)
    strategy.parameters:addInteger("Sum_Period", "Sum Period", "", 13)

    strategy.parameters:addGroup("Signal Calculation")
    strategy.parameters:addInteger("Signal_Period", "Period", "", 9)
...
local first
local CCI_Period, Min_Max_Period, Sum_Period, Signal_Period
local Indicator
local Direction
local CustomID
local PositionCap
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if Indicator.Raw[period] > Indicator.Signal[period] and Indicator.Raw[period - 1] <= Indicator.Signal[period - 1] then
        if Direction then
            BUY()
        else
            SELL()
    ...
    elseif
        Indicator.Raw[period] < Indicator.Signal[period] and Indicator.Raw[period - 1] >= Indicator.Signal[period - 1]
     then
        if Direction then
            SELL()
        else
