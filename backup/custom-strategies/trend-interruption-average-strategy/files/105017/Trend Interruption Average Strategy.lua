...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=63194
...
function Init() --The strategy profile initialization
    strategy:name("Trend Interruption Average Strategy")
...
function Prepare(nameOnly)
    ...
    assert(core.indicators:findIndicator("TIA") ~= nil, "Please, download and install TIA.LUA indicator")
    ...
    Indicator = core.indicators:create("TIA", Source.close, Method, Period, true, true, true)
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if Indicator.Up[period] > Indicator.Down[period] and Indicator.Up[period - 1] <= Indicator.Down[period - 1] then
        if Direction then
            BUY()
        else
            SELL()
    ...
    elseif Indicator.Up[period] < Indicator.Down[period] and Indicator.Up[period - 1] >= Indicator.Down[period - 1] then
        if Direction then
            SELL()
        else
            BUY()
