...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=70230
...
function CreateParameters()
    strategy.parameters:addInteger("ma_period", "MA Period", "", 14);
    AddAverages("ma_method", "MA Method", "MVA");
	strategy.parameters:addInteger("stoch_k", "Periods for %K", "", 10, 2, 1000);
    strategy.parameters:addInteger("stoch_sd", "%D slowing periods", "", 5, 2, 1000);
    strategy.parameters:addInteger("stoch_d", "Periods for %D", "", 5, 2, 1000);
    strategy.parameters:addString("tf1", "Confirmation Timeframe 1", "", "H1");
    strategy.parameters:setFlag("tf1", core.FLAG_PERIODS);
    strategy.parameters:addString("tf2", "Confirmation Timeframe 2", "", "D1");
    strategy.parameters:setFlag("tf2", core.FLAG_PERIODS);
end

local ma, ssd, atr_stop, atr_limit, stop_mult, limit_mult, confirmation_source1, confirmation_source2, MABuffer1, RSIBuffer1, marsioma1;
local confirmation_ma1, confirmation_ma2;
function CreateEntryIndicators(source)
    ...
    ssd = core.indicators:create("SSD", source, instance.parameters.stoch_k, instance.parameters.stoch_sd, instance.parameters.stoch_d);
    if instance.parameters.use_stop then
        atr_stop = core.indicators:create("ATR", source, instance.parameters.stop_pips);
    end
    if instance.parameters.use_limit then
        atr_limit = core.indicators:create("ATR", source, instance.parameters.limit_pips);
    end
    confirmation_source1 = ExtSubscribe(4, nil, instance.parameters.tf1, instance.parameters.type, "bar");
    confirmation_source2 = ExtSubscribe(5, nil, instance.parameters.tf2, instance.parameters.type, "bar");
    confirmation_ma1 = CreateAverages(instance.parameters.ma_period, instance.parameters.ma_method, confirmation_source1)
    confirmation_ma2 = CreateAverages(instance.parameters.ma_period, instance.parameters.ma_method, confirmation_source2);
...
function UpdateIndicators()
    confirmation_ma1:update(core.UpdateLast);
    confirmation_ma2:update(core.UpdateLast);
    ma:update(core.UpdateLast);
    ssd:update(core.UpdateLast);
    MABuffer1:update(core.UpdateLast);
    RSIBuffer1:update(core.UpdateLast);
    marsioma1:update(core.UpdateLast);
    if atr_stop ~= nil then
        atr_stop:update(core.UpdateLast);
    end
    if atr_limit ~= nil then
        atr_limit:update(core.UpdateLast);
    end
end
...
function IsEntryLong(source, period)
    if not ssd.D:hasData(period - 1) or not ma.DATA:hasData(period) then
        return false;
    end
    return source.close[period] > ma.DATA[period]
        and core.crossesOver(ssd.K, ssd.D, period)
        and source.close[period] > source.open[period - 1]
        and confirmation_ma1.DATA[NOW] < source.close[period] 
        and confirmation_ma2.DATA[NOW] < source.close[period] 
        and core.crossesUnder(RSIBuffer1.DATA, marsioma1.DATA, period)
end
function IsEntryShort(source, period)
    ...
    return source.close[period] < ma.DATA[period]
        and core.crossesUnder(ssd.K, ssd.D, period)
        and source.close[period] < source.open[period - 1]
        and confirmation_ma1.DATA[NOW] > source.close[period] 
        and confirmation_ma2.DATA[NOW] > source.close[period] 
        and core.crossesOver(RSIBuffer1.DATA, marsioma1.DATA, period)
end
...
function Init()
    ...
    strategy.parameters:addInteger("stop_pips", "Stop, period of ATR", "", 10);
    strategy.parameters:addDouble("stop_mult", "Stop multiplicator", "", 1.0);
    ...
    strategy.parameters:addInteger("limit_pips", "Limit, period of ATR", "", 20);
    strategy.parameters:addDouble("limit_mult", "Limit multiplicator", "", 1.0);
...
function OpenTrade(side)
    ...
    if use_stop then
        ...
        if side == "B" then
            valuemap.PegPriceOffsetPipsStop = -(atr_stop.DATA[NOW] / main_source:pipSize()) * stop_mult;
        else
            valuemap.PegPriceOffsetPipsStop = (atr_stop.DATA[NOW] / main_source:pipSize()) * stop_mult;
        end
    ...
    if use_limit then
        ...
        if side == "B" then
            valuemap.PegPriceOffsetPipsLimit = (atr_limit.DATA[NOW] / main_source:pipSize()) * limit_mult;
        else
            valuemap.PegPriceOffsetPipsLimit = -(atr_limit.DATA[NOW] / main_source:pipSize()) * limit_mult;
        end
    end
