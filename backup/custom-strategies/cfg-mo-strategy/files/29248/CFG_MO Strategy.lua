...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=15522
...
function Init() --The strategy profile initialization
    strategy:name("Cardwell Financial Group Momentum Oscillator Strategy")
    ...
    strategy.parameters:addInteger("A", "Slow RSI Period", "", 14, 2, 2000)
    strategy.parameters:addInteger("B", "Fast RSI Period", "", 3, 2, 2000)
    strategy.parameters:addInteger("C", "Momentum Period", "", 9, 2, 2000)
    strategy.parameters:addInteger("D", "Composite index Smoothing Period", "", 3, 2, 2000)
    strategy.parameters:addInteger("E", "Second Smoothing Period", "", 13, 2, 2000)
    strategy.parameters:addInteger("F", "Third Smoothing Period", "", 33, 2, 2000)

    strategy.parameters:addString("G", "Method For Average", "", "MVA")
    strategy.parameters:addStringAlternative("G", "MVA", "", "MVA")
    strategy.parameters:addStringAlternative("G", "EMA", "", "EMA")
    strategy.parameters:addStringAlternative("G", "LWMA", "", "LWMA")
    strategy.parameters:addStringAlternative("G", "VIDYA", "", "VIDYA")
    strategy.parameters:addStringAlternative("G", "SMMA", "", "SMMA")
    strategy.parameters:addStringAlternative("G", "WMA", "", "WMA")
    ...
    strategy.parameters:addString("SignalType", "Signal Type", "", "LF")
    strategy.parameters:addStringAlternative("SignalType", "MO / SM2", "", "LF")
    strategy.parameters:addStringAlternative("SignalType", "MO / SM3", "", "LS")
    strategy.parameters:addStringAlternative("SignalType", "SM2 / SM3", "", "FS")
    strategy.parameters:addStringAlternative("SignalType", "1. Algorithm", "", "Algorithm")

    strategy.parameters:addGroup("Exit Parameters")
    strategy.parameters:addBoolean("Exit", "Use Exit", "", false)

    strategy.parameters:addString("ExitType", "Signal Type", "", "LF")
    strategy.parameters:addStringAlternative("ExitType", "MO / SM2", "", "LF")
    strategy.parameters:addStringAlternative("ExitType", "MO / SM3", "", "LS")
    strategy.parameters:addStringAlternative("ExitType", "SM2 / SM3", "", "FS")
...
function Prepare(nameOnly)
    ...
    name = name .. ", " .. A .. ", " .. B .. ", " .. C .. ", " .. D .. ", " .. E .. ", " .. F .. ", " .. G

    assert(core.indicators:findIndicator("CFG_MO") ~= nil, "Please, download and install CFG_MO.LUA indicator")
    ...
    Indicator[1] = core.indicators:create("CFG_MO", Source[Price], A, B, C, D, E, F, G)
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if SignalType == "LF" then
        First = Short[1]
        Second = Short[2]
    elseif SignalType == "LS" then
        First = Short[1]
        Second = Short[3]
    elseif SignalType == "FS" then
        First = Short[2]
        Second = Short[3]
    end

    if SignalType == "LF" or SignalType == "LS" or SignalType == "FS" then
        if core.crossesOver(First, Second, period) then
            if Direction then
                BUY()
            else
                SELL()
            end
        elseif core.crossesUnder(First, Second, period) then
            if Direction then
                SELL()
            else
                BUY()
        ...
        if
            Short[2][period] > Short[3][period] and Short[3][period] > BL and Short[1][period] > Short[2][period] and
                Short[1][period - 1] <= Short[2][period - 1]
         then
            if Direction then
                BUY()
            else
        ...
        if
            Short[2][period] < Short[3][period] and Short[3][period] < SL and Short[1][period] < Short[2][period] and
                Short[1][period - 1] >= Short[2][period - 1]
         then
            if Direction then
                BUY()
    ...
    if Exit then
        ...
        if core.crossesUnder(First, Second, period) then
            if Direction then
