...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=64127
...
function Init() --The strategy profile initialization
    strategy:name("BB ATR Oscillator Strategy")
    ...
    strategy.parameters:addGroup("1. TF Indicator")

    strategy.parameters:addInteger("ATR_Period1", "ATR Period", "", 14)
    strategy.parameters:addDouble("ATR_Multiplier1", "ATR Multiplier", "", 2)
    strategy.parameters:addInteger("BB_Period1", "BB Period", "", 20)
    strategy.parameters:addDouble("BB_Deviation1", "BB Deviation", "", 2)
    strategy.parameters:addDouble("Line11", "Top Line", "", 0)
    strategy.parameters:addDouble("Line12", "Bottom Line", "", 0)

    strategy.parameters:addGroup("2. TF Indicator")

    strategy.parameters:addInteger("ATR_Period2", "ATR Period", "", 14)
    strategy.parameters:addDouble("ATR_Multiplier2", "ATR Multiplier", "", 2)

    strategy.parameters:addInteger("BB_Period2", "BB Period", "", 20)
    strategy.parameters:addDouble("BB_Deviation2", "BB Deviation", "", 2)
    strategy.parameters:addDouble("Line21", "Top Line", "", 0)
    strategy.parameters:addDouble("Line22", "Bottom Line", "", 0)
...
local ATR_Period1, ATR_Multiplier1, BB_Period1, BB_Deviation1, Line11, Line12
local ATR_Period2, ATR_Multiplier2, BB_Period2, BB_Deviation2, Line21, Line22
...
function Prepare(nameOnly)
    ...
    ATR_Multiplier1 = instance.parameters.ATR_Multiplier1
    BB_Period1 = instance.parameters.BB_Period1
    BB_Deviation1 = instance.parameters.BB_Deviation1
    Line11 = instance.parameters.Line11
    Line12 = instance.parameters.Line12
    ATR_Period2 = instance.parameters.ATR_Period2
    ATR_Multiplier2 = instance.parameters.ATR_Multiplier2
    BB_Period2 = instance.parameters.BB_Period2
    BB_Deviation2 = instance.parameters.BB_Deviation2
    Line21 = instance.parameters.Line21
    Line22 = instance.parameters.Line22
    ...
    assert(
        core.indicators:findIndicator("BB ATR OSCILLATOR") ~= nil,
        "Please, download and install BB ATR OSCILLATOR.LUA indicator"
    )
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if period1 < 0 or period2 < 0 then
        return
    end
    ...
    if not Indicator1.DATA:hasData(period1) or not Indicator2.DATA:hasData(period2) then
        return
    end

    local Return = EntryFunction(period1, period2, now)
end
...
function EntryFunction(period1, period2, now)
    ...
    -- BUY: BAO>BAO LEVEL1 IN H4 AND BAO CROSS OVER BAO LEVEL1IN M15
    --SELL:BAO< BAO LEVEL2 IN H4 AND BAO CROSS UNDER BAO LEVEL IN M15

    -- only buy if we have a fast cross over slow and the price is above the moving averages.
    if
        Indicator2.DATA[period2] > Line21 and Indicator1.DATA[period1] > Line11 and
            Indicator1.DATA[period1 - 1] <= Line11
     then
        if Direction then
            BUY()
        else
        ...
        Return = true
    elseif
        Indicator2.DATA[period2] < Line22 and Indicator1.DATA[period1] < Line12 and
            Indicator1.DATA[period1 - 1] >= Line12
     then
        if Direction then
            SELL()
        else
...
function exitSpecific(BuySell)
    ...
    if (CanClose) then
        ...
        exitTradeAll(BuySell)
    end
end

function exitTradeAll(side)
    -- if trade_in_progress then
    --return;
    --end
    ...
    if not (AllowTrade) or (CanClose) then
        return true
    end
...
function exitTrade(tradeRow)
    ...
    if not (AllowTrade) or not (CanClose) then
        return true
    end
