...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=64594&p
...
function Init() --The strategy profile initialization
    strategy:name("Vqi Rsi Ma Strategy")
    ...
    strategy.parameters:addInteger("RP1", "Period", "", 14)
    strategy.parameters:addInteger("RP2", "Period", "", 23)
    strategy.parameters:addInteger("RP3", "Period", "", 15)
    strategy.parameters:addDouble("RB", "Buy Level", "", 50)
    strategy.parameters:addDouble("RS", "Sell Levelel", "", 50)

    strategy.parameters:addGroup("VQI Calculation")
    strategy.parameters:addInteger("V1", "Short Smoothing Period", "", 9)
    strategy.parameters:addInteger("V2", "Long Smoothing Period", "", 200)

    strategy.parameters:addString("V3", "Algorithm Selection", "", "One")
    strategy.parameters:addStringAlternative("V3", "One", "", "One")
    strategy.parameters:addStringAlternative("V3", "Two", "", "Two")
...
function Prepare(nameOnly)
    ...
    RP3 = instance.parameters.RP3
    RB = instance.parameters.RB
    RS = instance.parameters.RS

    V1 = instance.parameters.V1
    V2 = instance.parameters.V2
    V3 = instance.parameters.V3
    ...
    assert(core.indicators:findIndicator("VQI") ~= nil, "Please, download and install  VQI.LUA indicator")
    ...
    RSI = core.indicators:create("RSI_MA", Source.close, RP1, RP2, RP3)
    VQI = core.indicators:create("VQI", Source, V1, V2, V3)

    first = math.max(RSI.BuffMARSI:first(), VQI.S3:first())
...
function EntryFunction(now, period)
    ...
    --buy: s2>s3 and rsi cross over rsi ma and rsi ma < buy level
    --sell: s2<s3 and rsi cross under rsima and rsima > sell level

    -- only buy if we have a fast cross over slow and the price is above the moving averages.
    if
        VQI.S2[period] > VQI.S3[period] and RSI.BuffRSI[period] > RSI.BuffMARSI[period] and
            RSI.BuffRSI[period - 1] <= RSI.BuffMARSI[period - 1] and
            RSI.BuffMARSI[period] < RB
     then
        if Direction then
            BUY()
        else
