...
function Init() --The strategy profile initialization
    strategy:name("Price TMA Min Max Strategy");
...
function CreateTradingParameters()
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "TMAMMS");
...
function EntryFunction( id, now,period, min1, max1, min2, max2)
    ...
    if   Source.close[period]> max1
	and  Source.close[period-1]<=  max2
	then	
          if Direction then
                BUY();
            else
		...
		Return=true;
    elseif   Source.close[period]< min1
	and  Source.close[period-1]>=  min2
	then
            if Direction then
                SELL();
            else
