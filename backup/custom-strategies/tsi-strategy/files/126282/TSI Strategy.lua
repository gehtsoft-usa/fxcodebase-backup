...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=68452
...
function CreateParameters()
    strategy.parameters:addInteger("smooth1", "Smooth 1", "", 20);
    strategy.parameters:addInteger("smooth2", "Smooth 2", "", 50);
    strategy.parameters:addDouble("max_spread", "Max. spread", "", 2);
end

local indi, max_spread;
function CreateIndicators(source)
    indi = core.indicators:create("TSI", source, instance.parameters.smooth1, instance.parameters.smooth2);
    max_spread = instance.parameters.max_spread;
end
...
function CreateCustomActions()
    ...
    enterLongAction.IsPass = function (source, period, periodFromLast, data)
        return indi.DATA[period] > 0 and (instance.ask[NOW] - instance.bid[NOW]) / source:pipSize() <= max_spread;
    end
...
function CreatePositionStrategy(source, side, id)
    ...
    if CreateCustomBreakeven == nil then
        ...
        if position_strategy.UseBreakeven and tick_source == nil then
            tick_source, TICKS_SOURCE_ID = trading_logic:SubscribeHistory(position_strategy.Source:instrument(), "t1", true);
        end
    end
...
function GoLong(source, period, positions, log)
    ...
    local message = "Buy " .. source:instrument()
    if log ~= nil then
        message = message .. "\r\nSignal info: " .. log;
    end
...
function GoShort(source, period, positions, log)
    ...
    local message = "Sell " .. source:instrument()
    if log ~= nil then
        message = message .. "\nSignal info: " .. log;
    end
    signaler:Signal(message, source);
...
function trading_logic:Init(parameters)
    if not CustomTimeframeDefined then
        ...
        parameters:addString("timeframe", "Trading Time frame", "", "t1");
