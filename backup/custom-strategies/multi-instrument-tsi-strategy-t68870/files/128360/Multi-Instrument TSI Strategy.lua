...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=68870
...
function Prepare(nameOnly)
    ...
    MA1 = core.indicators:create("MVA", Source1, instance.parameters.Period1)
    TSI = core.indicators:create("TSI", Source1.close, instance.parameters.TSI_Period1, instance.parameters.TSI_Period1)
    RLW = core.indicators:create("RLW", Source1, instance.parameters.RLW_Period)
    MA2 = core.indicators:create("MVA", Source2, instance.parameters.Period2)
    first = Source1:first()
...
function ExitFunction(id, now, period)
    ...
    if (ExitExecutionType == "Live" and id == 2) or (ExitExecutionType == "EndOfTurn" and id == 1) then
        if (ExitExecutionType == "EndOfTurn" and id == 1) then
            return
        end
    ...
    if
        (TSI.DATA[period1] < 15 and TSI.DATA[period1 - 1] >= 15) or
            (TSI.DATA[period1] < 34 and TSI.DATA[period1 - 1] >= 34)
     then
        if Direction then
...
function EntryFunction(id, now, period)
    ...
    if
        MA2.DATA[period2] > MA2.DATA[period2 - 1] and Source1.close[period1] > MA1.DATA[period1] and
            Source1.close[period1 - 1] <= MA1.DATA[period1 - 1] and
            TSI.DATA[period1] < 0 and
            RLW.DATA[period1] < -80
     then
        if Direction then
            BUY()
