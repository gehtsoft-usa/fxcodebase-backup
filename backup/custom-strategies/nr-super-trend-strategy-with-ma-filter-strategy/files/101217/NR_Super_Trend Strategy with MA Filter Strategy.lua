...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=62377
...
function Init() --The strategy profile initialization
    strategy:name("NR_Super_Trend Strategy with MA Filter Strategy");
	...
	strategy.parameters:addInteger("MA_Period", "Period", "", 50);
...
local first;
local Multiplier, Period,Exit;
local Indicator;
local MA;
local MA_Period, Method, Price;
local Direction;
local CustomID;
...
function Prepare( nameOnly)
    ...
    Indicator = core.indicators:create("NR_SUPER_TREND", Source, Period, Multiplier , core.rgb(0, 255, 0) , core.rgb(255, 0, 0)   );	
...
function ExtUpdate(id, source, period)  -- The method called every time when a new bid or ask price appears.
   ...
   -- get only time
	...
	if period < first  then
		...
		elseif  Indicator.DATA:colorI(period) ==  core.rgb(0, 255, 0) 
