...
--http://fxcodebase.com/code/viewtopic.php?f=31&t=70440
...
local ANCHOR_SOURCE_ID = 4;
local anchor_ema_8, anchor_ema_21, anchor_bars, trading_bars;
local ema_8, ema_13, ema_21;
...
local STRATEGY_NAME = "Two Timeperiod Twp Target Scalping Strategy";
...
function CreateStopParameters(params, id) return true; end
function CreateLimitParameters(params, id) return true; end

function GetEntryRate(source, bs, period) 
    core.host:trace(period);
    if bs == "B" then
        return mathex.max(source, core.rangeTo(period - 1, trading_bars)) + 3 * source:pipSize();
    end
    return mathex.min(source, core.rangeTo(period - 1, trading_bars)) - 3 * source:pipSize();
end
function SetCustomStop(position_desc, command, period, periods_from_last, source) 
    if position_desc.Side == "B" then
        command:SetStop(source.low[period] - 3 * source:pipSize());
        return true;
    end
    command:SetStop(source.high[period] + 3 * source:pipSize());
    return true;
end
function SetCustomLimit(position_desc, command, period, periods_from_last, source) 
    local dist = math.abs(command.valuemap.RateStop - command.valuemap.Rate);
    command:SetPipLimit(nil, dist * 2);
    return true; 
end
function SaveCustomStopParameters(position_desc, id) end
function SaveCustomLimitParameters(position_desc, id) end
function CreateCustomBreakeven(position_desc, result, period, periods_from_last, command) 
    local dist = math.abs(command.valuemap.RateStop - command.valuemap.Rate);
...
function CreateParameters()
    strategy.parameters:addString("anchor_tf", "Anchor Timeframe", "", "H1");
    strategy.parameters:setFlag("anchor_tf", core.FLAG_BARPERIODS);
    strategy.parameters:addInteger("anchor_bars", "Anchor, bars", "", 2);
    strategy.parameters:addInteger("trading_bars", "Trading, bars", "", 5);
end
function CreateEntryIndicators(source)
    trading_bars = instance.parameters.trading_bars;
    anchor_source = trading_logic:SubscribeHistory(source.close:instrument(), instance.parameters.anchor_tf, source:isBid());
    anchor_bars = instance.parameters.anchor_bars;

    anchor_ema_8 = core.indicators:create("EMA", anchor_source, 8);
    anchor_ema_21 = core.indicators:create("EMA", anchor_source, 21);
    ema_8 = core.indicators:create("EMA", source, 8);
    ema_13 = core.indicators:create("EMA", source, 13);
    ema_21 = core.indicators:create("EMA", source, 21);
end
function CreateExitIndicators(source) end

function UpdateIndicators()
    anchor_ema_8:update(core.UpdateLast);
    anchor_ema_21:update(core.UpdateLast);
    ema_8:update(core.UpdateLast);
    ema_13:update(core.UpdateLast);
    ema_21:update(core.UpdateLast);
end
...
function CreateCustomActions()
    if not DISABLE_EXIT then
        ...
        exitLongAction.IsPass = function (source, period, periodFromLast, data)
            return core.crossesUnder(source.close, ema_21.DATA, period);
        end
        ...
        exitShortAction.IsPass = function (source, period, periodFromLast, data)
            return core.crossesOver(source.close, ema_21.DATA, period);
        end
    ...
    enterLongAction.IsPass = function (source, period, periodFromLast, data)
        for i = 0, anchor_bars - 1 do
            if anchor_source.close[NOW - i] < anchor_ema_8.DATA[NOW - i] 
                or anchor_ema_8.DATA[NOW - i] < anchor_ema_21.DATA[NOW - i]
            then
                return false;
            end
        end
        for i = 1, trading_bars do
            if source.close[period - i] < ema_8.DATA[period - i] 
                or ema_8.DATA[period - i] < ema_13.DATA[period - i]
                or ema_13.DATA[period - i] < ema_21.DATA[period - i]
            then
                return false;
            end
        end
        return core.crossesUnder(source.low, ema_8.DATA, period);
    end
    ...
    enterShortAction.IsPass = function (source, period, periodFromLast, data)
        ...
        return core.crossesOver(source.high, ema_8.DATA, period);
    end
...
function CreatePositionStrategy(source, side, id)
    ...
    function position_strategy:Open(period, periods_from_last)
        ...
        local default_breakeven = CreateCustomBreakeven == nil or not CreateCustomBreakeven(self, result, period, periods_from_last, command);
