...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=2759
...
function Init()
    strategy:name("Simple adaptive strategy")
    strategy:description("Simple adaptive strategy")
    strategy:setTag(
    ...
    strategy.parameters:addString("FirstOrder", "Type of first order", "", "BUY")
    strategy.parameters:addStringAlternative("FirstOrder", "BUY", "", "BUY")
    strategy.parameters:addStringAlternative("FirstOrder", "SELL", "", "SELL")
    strategy.parameters:addInteger("OrdersNumber", "Number of orders for decision", "", 1)
...
function ExtUpdate(id, source, period)
    ...
    if not haveTrades() then
        ...
        if CountOrders == 0 then
            if instance.parameters.FirstOrder == "BUY" then
                side = "B"
            else
                side = "S"
            end
        else
            local enum, row, profit
            local CountB = 0
            local CountS = 0
            local BS
            for i = 1, CountOrders, 1 do
                enum = core.host:findTable("closed trades"):enumerator()
                row = enum:next()
                profit = 0
                BS = nil
                while row ~= nil do
                    if row.OpenOrderReqID == LastOrders[i] then
                        profit = profit + row.PL
                        BS = row.BS
                    end
                    row = enum:next()
                end
                if (profit > 0 and BS == "B") or (profit < 0 and BS == "S") then
                    CountB = CountB + 1
                elseif (profit > 0 and BS == "S") or (profit < 0 and BS == "B") then
                    CountS = CountS + 1
                end
            end

            if CountB > CountS then
                side = "B"
            elseif CountS > CountB then
                side = "S"
            else
                if instance.parameters.FirstOrder == "BUY" then
                    side = "B"
