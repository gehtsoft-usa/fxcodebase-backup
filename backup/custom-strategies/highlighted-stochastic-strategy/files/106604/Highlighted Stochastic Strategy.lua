...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=63560
...
function Init() --The strategy profile initialization
    strategy:name("Highlighted Stochastic Strategy")
...
function Prepare(nameOnly)
    ...
    assert(
        core.indicators:findIndicator("HIGHLIGHTED STOCHASTIC") ~= nil,
        "Please, download and install HIGHLIGHTED STOCHASTIC.LUA indicator"
    )
    ...
    Indicator = core.indicators:create("HIGHLIGHTED STOCHASTIC", Source, K, SD, D, MVAT_K, MVAT_D)
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if Indicator.K[period] > Indicator.D[period] and Indicator.K[period - 1] <= Indicator.D[period - 1] then
        if Direction then
            BUY()
        else
            SELL()
    ...
    elseif Indicator.K[period] < Indicator.D[period] and Indicator.K[period - 1] >= Indicator.D[period - 1] then
        if Direction then
            SELL()
        else
            BUY()
