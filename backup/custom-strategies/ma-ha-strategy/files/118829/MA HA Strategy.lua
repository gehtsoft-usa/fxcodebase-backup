...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=65476
...
function Init()
    strategy:name("MA HA Strategy");
    strategy:description("MA HA Strategy");
    strategy.parameters:addBoolean("allow_long", "Allow Long Trade", "", true);
    strategy.parameters:addBoolean("allow_short", "Allow Short Trade", "", true);
    strategy.parameters:addDouble("amount_per", "% of Account to Risk per trade", "", 5);
    strategy.parameters:addInteger("N", "Periods", "", 14);
    strategy.parameters:addString("MA", "Smoothing Method", "The methods marked by an asterisk (*) require the appropriate indicators to be loaded.", "MVA");
    ...
    strategy.parameters:addStringAlternative("MA", "Vidya (1992)*", "", "VIDYA92");
    strategy.parameters:addStringAlternative("MA", "Wilders*", "", "WMA");
    strategy.parameters:addInteger("SY", "Shift in points", "", 0);
    strategy.parameters:addInteger("SX", "Shift in bars", "", 1);
    strategy.parameters:addBoolean("use_trade", "Work on trade", "", false);
    strategy.parameters:addString("trade", "Trade", "", "");
    strategy.parameters:setFlag("trade", core.FLAG_TRADE);
    strategy.parameters:addBoolean("use_central_band", "Use central band", "", false);
...
local centre_band_bid = nil;
local centre_band_ask = nil;
local bid;
local ask;
local allow_long;
local allow_short;
...
function Prepare(name_only)
    ...
    tradeID = instance.parameters.use_trade and instance.parameters.trade or nil;
    allow_long = instance.parameters.allow_long;
    allow_short = instance.parameters.allow_short;
    amount_per = instance.parameters.amount_per;
    use_central_band = instance.parameters.use_central_band;
    ...
    assert(core.indicators:findIndicator("HA") ~= nil, "Please, download and install HA indicator");
    ha_ask = core.indicators:create("HA", ask);
    ha_bid = core.indicators:create("HA", bid);
    assert(core.indicators:findIndicator("SHIFT_MA") ~= nil, "Please, download and install SHIFT_MA indicator");
    upper_band = core.indicators:create("SHIFT_MA", ha_ask.high, instance.parameters.N, instance.parameters.MA, instance.parameters.SX, instance.parameters.SY);
    lower_band = core.indicators:create("SHIFT_MA", ha_bid.low, instance.parameters.N, instance.parameters.MA, instance.parameters.SX, -instance.parameters.SY);
    centre_band_bid = core.indicators:create("SHIFT_MA", ha_bid.open, instance.parameters.N, instance.parameters.MA, instance.parameters.SX, 0);
    centre_band_ask = core.indicators:create("SHIFT_MA", ha_ask.open, instance.parameters.N, instance.parameters.MA, instance.parameters.SX, 0);

    trading.CustomID = "ha mva strategy";
    trading_logic.DoTrading = EntryFunction;
end
function DoTradeLogic(trade, period, source)
    if trade.BS == "B" then
        if trade.PL >= 0 and use_central_band then
            if source.close[period] < centre_band_ask.DATA[NOW] then
                trading:Close(trade);
            else
                trading:MoveStop(centre_band_ask.DATA[NOW], trade);
            end
        else
            if source.close[period] < lower_band.DATA[NOW] then
                trading:Close(trade);
            else
                trading:MoveStop(lower_band.DATA[NOW], trade);
            end
        end
    else
        if trade.PL >= 0 and use_central_band then
            if source.close[period] > centre_band_bid.DATA[NOW] then
                trading:Close(trade);
            else
                trading:MoveStop(centre_band_bid.DATA[NOW], trade);
            end
        else
            if source.close[period] > upper_band.DATA[NOW] then
                trading:Close(trade);
            else
                trading:MoveStop(upper_band.DATA[NOW], trade);
            end
        end
    end
end
function EntryFunction(source, period)
    ha_ask:update(core.UpdateLast);
    ha_bid:update(core.UpdateLast);
    upper_band:update(core.UpdateLast);
    lower_band:update(core.UpdateLast);
    centre_band_bid:update(core.UpdateLast);
    centre_band_ask:update(core.UpdateLast);

    if tradeID ~= nil and tradeID ~= "" then
        local trade = core.host:findTable("trades"):find("TradeID", tradeID);
        if trade == nil then
            tradeID = nil;
        else
            DoTradeLogic(trade, period, source);
            return;
        end
    end

    local orders = trading:FindOrder():SetCustomID(trading.CustomID):All();
    for _, order in ipairs(orders) do
        trading:DeleteOrder(order);
    end
    if allow_long then
        local trades = trading:FindTrade():SetSide("B"):SetCustomID(trading.CustomID):All();
        for _, trade in ipairs(trades) do
            DoTradeLogic(trade, period, source);
        end

        if #trades < 1 then
            trading:EntryOrder(bid:instrument())
                :SetSide("B")
                :SetRate(upper_band.DATA[NOW])
                :SetPercentOfEquityAmount(amount_per)
                :SetStop(lower_band.DATA[NOW])
                :UseDefaultCustomId()
                :Execute();
        end
    end
    ...
    if allow_short then
        local trades = trading:FindTrade():SetSide("S"):SetCustomID(trading.CustomID):All();
        for _, trade in ipairs(trades) do
            DoTradeLogic(trade, period, source);
        end
        if #trades < 1 then
            trading:EntryOrder(bid:instrument())
                :SetSide("S")
                :SetRate(lower_band.DATA[NOW])
                :SetPercentOfEquityAmount(amount_per)
                :SetStop(upper_band.DATA[NOW])
                :UseDefaultCustomId()
                :Execute();
        end
    end
...
trading.Version = "1.9.0";
trading.Debug = false;
trading.AddAmountParameter = false;
...
function trading:FindLimitOrder(trade)
    ...
    if can_close then
        ...
        elseif self._request_id ~= nil then
            self:trace("Searching limit order by request id: " .. tostring(self._request_id));
...
function trading_logic:Prepare(name_only)
    ...
    self:trace(string.format("Trading on %s bar", "m1"));
    self.MainSource = ExtSubscribe(self._ids_start + 2, nil, "m1", instance.parameters.is_bid, "bar");
end
