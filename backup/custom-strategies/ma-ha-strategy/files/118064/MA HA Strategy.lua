...
function DoTradeLogic(trade, period, source)
    if trade.BS == "B" then
        if trade.PL >= 0 and use_central_band then
            if source.close[period] < centre_band_ask.DATA[period] then
                trading:Close(trade);
            else
                trading:MoveStop(centre_band_ask.DATA[period], trade);
            end
        else
            if source.close[period] < lower_band.DATA[period] then
                trading:Close(trade);
            else
                trading:MoveStop(lower_band.DATA[period], trade);
            end
        end
    else
        if trade.PL >= 0 and use_central_band then
            if source.close[period] > centre_band_bid.DATA[period] then
                trading:Close(trade);
            else
                trading:MoveStop(centre_band_bid.DATA[period], trade);
            end
        else
            if source.close[period] > upper_band.DATA[period] then
                trading:Close(trade);
            else
                trading:MoveStop(upper_band.DATA[period], trade);
            end
        end
    end
end
function EntryFunction(source, period)
    ...
    if allow_long then
        ...
        if #trades < 1 then
            trading:EntryOrder(bid:instrument())
                :SetSide("B")
                :SetRate(upper_band.DATA[period])
                :SetPercentOfEquityAmount(amount_per)
                :SetStop(lower_band.DATA[period])
                :UseDefaultCustomId()
                :Execute();
        end
    end
    ...
    if allow_short then
        ...
        if #trades < 1 then
            trading:EntryOrder(bid:instrument())
                :SetSide("S")
                :SetRate(lower_band.DATA[period])
                :SetPercentOfEquityAmount(amount_per)
                :SetStop(upper_band.DATA[period])
                :UseDefaultCustomId()
                :Execute();
        end
    end
