...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=61816
...
function Init() --The strategy profile initialization
    strategy:name("TWO CCI MA STRATEGY")
    ...
    strategy.parameters:addGroup("1. CCI Calculation")
    strategy.parameters:addInteger("Period1", "Period", "", 14)
    strategy.parameters:addInteger("Buy1", "Buy Level", "", 0)
    strategy.parameters:addInteger("Sell1", "Sell Level", "", 0)
    ...
    strategy.parameters:addGroup("2. CCI Calculation")
    strategy.parameters:addInteger("Period2", "Period", "", 50)
    strategy.parameters:addInteger("Buy2", "Buy Level", "", 0)
    strategy.parameters:addInteger("Sell2", "Sell Level", "", 0)
...
local first
local Period1, Period2, Buy1, Buy2, Sell1, Sell2
local Indicator = {}
local Short = {}
local Direction
local CustomID
...
function Prepare(nameOnly)
    ...
    Buy1 = instance.parameters.Buy1
    Buy2 = instance.parameters.Buy2
    Sell1 = instance.parameters.Sell1
    Sell2 = instance.parameters.Sell2
    ...
    CCI1 = core.indicators:create("CCI", Source, Period1)
    CCI2 = core.indicators:create("CCI", Source, Period2)
    assert(core.indicators:findIndicator(Method) ~= nil, Method .. " indicator must be installed");
    MA = core.indicators:create(Method, CCI1.DATA, Period)

    first = math.max(CCI2.DATA:first(), MA.DATA:first())
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if MA.DATA[period] > Buy1 and CCI2.DATA[period] > Buy2 and CCI1.DATA[period] > Buy1 and CCI1.DATA[period - 1] < Buy1 then
        if Direction then
            BUY()
        else
            SELL()
    ...
    elseif
        MA.DATA[period] < Buy1 and CCI2.DATA[period] < Buy2 and CCI1.DATA[period] < Buy1 and
            CCI1.DATA[period - 1] > Buy1
     then
        if Direction then
            SELL()
        else
    ...
    if Exit then
        if CCI2.DATA[period] > Buy2 and CCI2.DATA[period - 1] < Buy2 then
            if Direction then
