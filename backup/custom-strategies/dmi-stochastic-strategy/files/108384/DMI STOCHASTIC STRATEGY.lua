...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=63930
...
function Init() --The strategy profile initialization
    strategy:name("DMI STOCHASTIC STRATEGY")
    ...
    strategy.parameters:addInteger("DMI_Period", "TMA period", "", 14)
...
local first
local DMI, STOCHASTIC, DMI_Period, K, SD, D, MVAT_K, MVAT_D
local Direction
local CustomID
local PositionCap
local OS, OB
...
function Prepare(nameOnly)
    ...
    DMI = core.indicators:create("DMI", Source1, DMI_Period)
    STOCHASTIC = core.indicators:create("STOCHASTIC", Source2, K, SD, D, MVAT_K, MVAT_D)
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if not DMI.DATA:hasData(period1) or not STOCHASTIC.DATA:hasData(period2) then
        return
    end

    if
        DMI.DIP[period1] > DMI.DIM[period1] and STOCHASTIC.K[period2] > STOCHASTIC.D[period2] and
            STOCHASTIC.D[period2] < OB
     then
        if Direction then
            BUY()
        else
