-- Available @ https://fxcodebase.com/code/viewtopic.php?f=31&t=75919
...
function Init() --The strategy profile initialization
    strategy:name("MA Cross CCI Strategy");
	...
	strategy.parameters:addGroup("MAs");	
	strategy.parameters:addBoolean("MA_Off", "Don't use MA cross", "", false);  


	strategy.parameters:addGroup("1. MA");
    strategy.parameters:addString("Price1", "Price Source", "", "median");
	...
	strategy.parameters:addInteger("Period1", "Period", "Period" , 10);	

	strategy.parameters:addString("Method1", "Method", "Method" , "VIDYA");
	...
	strategy.parameters:addGroup("2. MA");
    strategy.parameters:addString("Price2", "Price Source", "", "median");
	...
	strategy.parameters:addInteger("Period2", "Period", "Period" , 12);	

	strategy.parameters:addString("Method2", "Method", "Method" , "VIDYA");
	...
	strategy.parameters:addGroup("CCI");
	strategy.parameters:addInteger("Period", "Period", "Period" , 10);	
	strategy.parameters:addDouble("OB", "OB Level", "Level" , 100);		
	strategy.parameters:addDouble("OS", "OS Level", "Level" , -100);	
    strategy.parameters:addBoolean("CCI_Off", "Don't use CCI cross", "", false);  
...
function Prepare( nameOnly)
	...
	CCI_Off = instance.parameters.CCI_Off;
	MA_Off = instance.parameters.MA_Off;
	...
	first=math.max(MA1.DATA:first(),MA2.DATA:first(),CCI.DATA:first()); 
...
function EntryFunction( now,period)
    ...
    if   ( MA1.DATA[period]> MA2.DATA[period]  and ( MA1.DATA[period-1]<= MA2.DATA[period-1] or MA_Off))
	and  ( CCI.DATA[period]> OS  and ( CCI.DATA[period-1]<= OS or  CCI_Off))
	then	
          if Direction then
                BUY();
            else
		...
		Return=true;
    elseif   ( MA1.DATA[period]< MA2.DATA[period]  and ( MA1.DATA[period-1]>= MA2.DATA[period-1] or MA_Off))
	and  ( CCI.DATA[period]< OB  and (CCI.DATA[period-1]>= OB or  CCI_Off))
	then
            if Direction then
                SELL();
            else
