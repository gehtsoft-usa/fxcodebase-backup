...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=63922
...
function Init() --The strategy profile initialization
    ...
    strategy.parameters:addInteger("ADX_Period", "ADX period", "", 14);
	 strategy.parameters:addDouble("ADX_Level", "ADX level", "", 14);
	
   	strategy.parameters:addGroup("DMI Calculation");
    strategy.parameters:addInteger("DMI_Period", "DMI period", "", 18);
...
function CreateTradingParameters()
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "MIS");
...
local ADX_Period,ADX, DMI,DMI_Period,ADX_Level;
...
function Prepare( nameOnly)
	...
	first=math.max(MA.DATA:first(), DMI.DATA:first(),  MACD.DATA:first()); 
...
function ExtUpdate(id, source, period)  -- The method called every time when a new bid or ask price appears.
   ...
   -- get only time
	...
	or  not DMI.DATA:hasData(period)	
	or  not ADX.DATA:hasData(period)	
	or  not MACD.DATA:hasData(period)	
	then
        return;
    end
	...
	and  MACD.DATA[period] > 0
	and ADX.DATA[period] > ADX_Level
	and DMI.DIP[period] >  DMI.DIM[period]
	then	
          if Direction then
                BUY();
            else
