...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=68860
...
local STRATEGY_NAME = "Multitimeframe SAR strategy";
...
function CreateCustomBreakeven(position, result, period, periods_from_last)
    local stop_rate = stop_sar.DATA[NOW - periods_from_last];
    local stop;
    if (position.Side == "B") then
        stop = math.abs(instance.bid[NOW] - stop_rate) / instance.ask:pipSize();
    else
        stop = math.abs(instance.ask[NOW] - stop_rate) / instance.ask:pipSize();
    end

    local limit;
    if position.LimitType == "pips" then
        limit = position.Limit / 2;
    elseif position.LimitType == "stop" then
        limit = stop_value * position.Limit / 2;
    else
        return true;
    end
...
function CreateParameters()
    ...
    strategy.parameters:addString("tf_1", "Timeframe #1", "", "m5");
    strategy.parameters:setFlag("tf_1", core.FLAG_PERIODS);
    strategy.parameters:addString("tf_2", "Timeframe #2", "", "m15");
    strategy.parameters:setFlag("tf_2", core.FLAG_PERIODS);
    strategy.parameters:addString("tf_3", "Timeframe #3", "", "m30");
    strategy.parameters:setFlag("tf_3", core.FLAG_PERIODS);
    strategy.parameters:addString("tf_4", "Timeframe #4", "", "H1");
    strategy.parameters:setFlag("tf_4", core.FLAG_PERIODS);
    strategy.parameters:addString("tf_5", "Timeframe #5", "", "H4");
    strategy.parameters:setFlag("tf_5", core.FLAG_PERIODS);
end

function CreateStopParameters(params, id) 
    strategy.parameters:addString("tf_stop", "Stop Timeframe", "", "H1");
    strategy.parameters:setFlag("tf_stop", core.FLAG_PERIODS);
    return true; 
end

function SetCustomStop(position_desc, command, period, periods_from_last) 
    command:SetStop(stop_sar.DATA[NOW - periods_from_last]);
    return false; 
end

local indis = {};
function CreateIndicators(source)
    local source_stop = trading_logic:SubscribeHistory(source:instrument(), instance.parameters.tf_stop, source:isBid());
    stop_sar = core.indicators:create("SAR", source_stop, instance.parameters.Step, instance.parameters.Max);

    local source_1 = trading_logic:SubscribeHistory(source:instrument(), instance.parameters.tf_1, source:isBid());
    indis[#indis + 1] = core.indicators:create("SAR", source_1, instance.parameters.Step, instance.parameters.Max);

    local source_2 = trading_logic:SubscribeHistory(source:instrument(), instance.parameters.tf_2, source:isBid());
    indis[#indis + 1] = core.indicators:create("SAR", source_2, instance.parameters.Step, instance.parameters.Max);
    
    local source_3 = trading_logic:SubscribeHistory(source:instrument(), instance.parameters.tf_3, source:isBid());
    indis[#indis + 1] = core.indicators:create("SAR", source_3, instance.parameters.Step, instance.parameters.Max);
    
    local source_4 = trading_logic:SubscribeHistory(source:instrument(), instance.parameters.tf_4, source:isBid());
    indis[#indis + 1] = core.indicators:create("SAR", source_4, instance.parameters.Step, instance.parameters.Max);
    
    local source_5 = trading_logic:SubscribeHistory(source:instrument(), instance.parameters.tf_5, source:isBid());
    indis[#indis + 1] = core.indicators:create("SAR", source_5, instance.parameters.Step, instance.parameters.Max);
end
...
function UpdateIndicators()
    ...
    stop_sar:update(core.UpdateLast);
end
...
function CreateCustomActions()
    ...
    exitLongAction.IsPass = function (source, period, periodFromLast, data)
        local count = 0;
        for _, indi in ipairs(indis) do
            if not indi.DATA:hasData(NOW - periodFromLast) then
                return;
            end
            if indi.DATA[NOW - periodFromLast] < source.close[period] then
                count = count + 1;
            end
        end
        return count >= 3;
