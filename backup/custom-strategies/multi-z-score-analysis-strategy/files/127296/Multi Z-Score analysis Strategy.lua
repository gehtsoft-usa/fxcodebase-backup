...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=68646
...
function Init() --The strategy profile initialization
    strategy:name("Multi Z-Score analysis Strategy");
    ...
    strategy.parameters:addInteger("Definition", "Definition", "", 20, 1, 2000);
...
--	strategy.parameters:addStringAlternative("ExitExecutionType", "Live", "", "Live");
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "MZSAS");
...
local Indicator;
local Period, Method, Definition;
...
function Prepare( nameOnly)
	...
	assert(core.indicators:findIndicator("MULTI Z-SCORE ANALYSIS") ~= nil, "Please, download and install MULTI Z-SCORE ANALYSIS.LUA indicator");          
    ...
    Indicator = core.indicators:create("MULTI Z-SCORE ANALYSIS", Source.close, Period, Method, Definition  );
	first=Indicator.Average:first(); 
...
function EntryFunction( id, now,period)
	...
	--/////////////////////////////////////////////////////////////////    
     ...
     or not Indicator.Average:hasData(period)
	 then
	 return;
	 end	
	 --/////////////////////////////////////////////////////////////////  
    ...
    if   Indicator.Line[period] >  Indicator.Average[period]
	and   Indicator.Line[period-1] <=  Indicator.Average[period-1]
	then	
          if Direction then
                BUY();
            else
		...
		Return=true;
    elseif  Indicator.Line[period] <  Indicator.Average[period]
	and   Indicator.Line[period-1] >=  Indicator.Average[period-1]
	then
            if Direction then
                SELL();
