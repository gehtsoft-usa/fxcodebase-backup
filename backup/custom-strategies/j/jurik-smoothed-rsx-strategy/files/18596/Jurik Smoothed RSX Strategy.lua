...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=8410
...
function Init() --The strategy profile initialization
    strategy:name("Jurik Smoothed RSX Strategy")
    ...
    strategy.parameters:addInteger("N1", "Number of periods to price delta", "", 8, 1, 10000)
    strategy.parameters:addInteger("N2", "Number of periods to final smooth", "", 3, 1, 10000)
    strategy.parameters:addInteger("P", "Phase to final smooth", "", 100, -1000, 1000)

    strategy.parameters:addGroup("Strategy Parameters")

    strategy.parameters:addString("Trade", "Trade Type", "", "Both")
    strategy.parameters:addStringAlternative("Trade", "Both", "", "Both")
    strategy.parameters:addStringAlternative("Trade", "Slope", "", "Slope")
    strategy.parameters:addStringAlternative("Trade", "Cross", "", "Cross")
...
function Prepare(nameOnly)
    ...
    assert(core.indicators:findIndicator("JJRSX") ~= nil, "JJRSX indicator must be installed")
    ...
    Indicator[1] = core.indicators:create("JJRSX", Source[Price], N1, N2, P, core.rgb(0, 255, 0), core.rgb(255, 0, 0))
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if
        (Short[1]:colorI(period - 1) ~= Short[1]:colorI(period) and Short[1]:colorI(period) == core.rgb(0, 255, 0) and
            Trade ~= "Cross") or
            (core.crossesOver(Indicator[1].DATA, 0, period) and Trade ~= "Slope")
     then
        if Direction then
            BUY()
        else
    ...
    if
        (Short[1]:colorI(period - 1) ~= Short[1]:colorI(period) and Short[1]:colorI(period) == core.rgb(255, 0, 0) and
            Trade ~= "Cross") or
            (core.crossesUnder(Indicator[1].DATA, 0, period) and Trade ~= "Slope")
     then
        if Direction then
            SELL()
        else
