...
local ENFORCE_entry_execution_type = "EndOfTurn"; -- Live/EndOfTurn
local ENFORCE_exit_execution_type = "EndOfTurn"; -- Live/EndOfTurn
local EXIT_TIMEFRAME_IN_PARAMS = false;
local DISABLE_EXIT = false;

local STRATEGY_NAME = "Three Timeframe MAs BB Strategy";
...
function CreateParameters()
    strategy.parameters:addString("higher_tf", "Higher Timeframe", "", "H1");
    strategy.parameters:setFlag("higher_tf", core.FLAG_PERIODS);
    strategy.parameters:addInteger("higher_ma_period", "Higher MA Period", "", 14);
    AddAverages("higher_ma_method", "Higher MA Method", "EMA");
    strategy.parameters:addString("medium_tf", "Medium Timeframe", "", "m30");
    strategy.parameters:setFlag("medium_tf", core.FLAG_PERIODS);
    strategy.parameters:addInteger("medium_tf_period", "Medium MA Period", "", 14);
    AddAverages("medium_tf_method", "Medium MA Method", "EMA");
    strategy.parameters:addInteger("bb_period", "BB Period", "", 14, 2, 1000);
    strategy.parameters:addDouble("bb_deviations", "BB Deviations", "", 2.0, 0.1, 1000);
end
...
local higher_ma, medium_ma, bb;
function CreateEntryIndicators(source)
    higher_source = trading_logic:SubscribeHistory(nil, instance.parameters.higher_tf, instance.parameters.is_bid);
    higher_ma = CreateAverages(instance.parameters.higher_ma_period, instance.parameters.higher_ma_method, higher_source.close);
    medium_source = trading_logic:SubscribeHistory(nil, instance.parameters.medium_tf, instance.parameters.is_bid);
    medium_ma = CreateAverages(instance.parameters.medium_ma_period, instance.parameters.medium_ma_method, medium_source.close);
    bb = core.indicators:create("BB", source.close, instance.parameters.bb_period, instance.parameters.bb_deviations);
end
function CreateExitIndicators(source)
end

function UpdateIndicators()
    higher_ma:update(core.UpdateLast);
    medium_ma:update(core.UpdateLast);
...
function OnNewBar(source, period)
    ...
    it:Do(function (order) trading:DeleteOrder(order); end);
    it = trading:FindTrade();
    if UseOwnPositionsOnly then
        it:WhenCustomID(custom_id);
    end
    it:Do(function (trade) 
        local stop = trading:FindStopOrder(trade);
        if stop == nil then
            local index = core.findDate(source, trade.Time, false);
            if index >= 0 then
                if trade.BS == "B" then
                    local low = mathex.min(source, core.rangeTo(period, 3));
                    trading:MoveStop(trade, low, nil);
                else
                    local high = mathex.max(source, core.rangeTo(period, 3));
                    trading:MoveStop(trade, high, nil);
                end
            end
        end
    end);
...
function CreateCustomController()
    local controller = breakeven:CreateBaseController();
    ...
    function controller:DoBreakeven()
        if self._executed then
            ...
            if self._command2 == nil then
                ...
                if core.crosses(source.close, medium_ma.DATA, NOW, NOW) then
                    trading:Close(trade);
                    self._command2 = true;
                end
            end
        ...
        if trade.BS == "B" then
            if core.crossesOver(trading_logic.MainSource.close, bb.TL, NOW) then
            ...
            if core.crossesUnder(trading_logic.MainSource.close, bb.BL, NOW) then
                if self._to ~= nil then
                    self._command = self._parent._trading:MoveStop(trade, self:getTo(), self._trailing);
                end
                self._executed = true;
    ...
    breakeven._controllers[#breakeven._controllers + 1] = controller;
    return controller;
end

function SetCustomStop(position_desc, command, period, periods_from_last, source) return true; end
function SetCustomLimit(position_desc, command, period, periods_from_last, source) return true; end
function SaveCustomStopParameters(position_desc, id) end
function SaveCustomLimitParameters(position_desc, id) end
function CreateCustomBreakeven(position_desc, result, period, periods_from_last) 
    local controller = CreateCustomController()
...
function LongBBCondition(source, period)
    for i = period, 0, -1 do
        if not bb.AL:hasData(i) or source.close[i] >= bb.AL[i] then
            return false;
        end
        if source.low[i] <= bb.BL[i] then
            return true;
        end
    end
    return false;
...
function ShortBBCondition(source, period)
    for i = period, 0, -1 do
        if not bb.AL:hasData(i) or source.close[i] <= bb.AL[i] then
            return false;
        end
        if source.high[i] >= bb.TL[i] then
            return true;
        end
    end
    return false;
...
function CreateCustomActions()
    ...
    enterLongAction.IsPass = function (source, period, periodFromLast, data)
        return source.close[period] > higher_ma.DATA[NOW]
            and source.close[period] > medium_ma.DATA[NOW]
            and source.close[period] < bb.AL[period]
            and LongBBCondition(source, period)
    end
    ...
    enterShortAction.IsPass = function (source, period, periodFromLast, data)
        return source.close[period] < higher_ma.DATA[NOW]
            and source.close[period] < medium_ma.DATA[NOW]
            and source.close[period] > bb.AL[period]
            and ShortBBCondition(source, period)
    end
