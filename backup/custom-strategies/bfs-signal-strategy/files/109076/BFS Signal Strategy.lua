...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=64097
...
function Init() --The strategy profile initialization
    strategy:name("BFS Signal Strategy")
    ...
    strategy.parameters:addGroup("BFS Calculation")
    strategy.parameters:addDouble("PeriodWATR", "Period WATR", "", 7)
    strategy.parameters:addDouble("Kwatr", "Kwatr", "", 0.7)
    strategy.parameters:addBoolean("HighLow", "High Low", "", false)

    strategy.parameters:addGroup("Corrected Average Calculation")
    strategy.parameters:addInteger("Period", "Period", "", 5)

    strategy.parameters:addString("Method", "Method", "Method", "SMMA")
...
local Exit
local Indicator, PeriodWATR, Kwatr, HighLow
local AVERAGE, Period, Method
function Prepare(nameOnly)
    ...
    assert(core.indicators:findIndicator("BFS SIGNAL") ~= nil, "Please, download and install BFS SIGNAL.LUA indicator")
    assert(
        core.indicators:findIndicator("CORRECTED AVERAGE") ~= nil,
        "Please, download and install CORRECTED AVERAGE.LUA indicator"
    )
    ...
    Indicator = core.indicators:create("BFS SIGNAL", Source, PeriodWATR, Kwatr, HighLow)
    AVERAGE = core.indicators:create("CORRECTED AVERAGE", Source.close, Period, Method)
    first = math.max(Indicator.Line2:first(), AVERAGE.DATA:first())
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if not Indicator.Line2:hasData(period) or not AVERAGE.DATA:hasData(period) then
        return
    end
...
close > corrected averages
    ...
    if Source.close[period] > AVERAGE.DATA[period] and Indicator.Line1[period] > Indicator.Line2[period] then
        if Direction then
            BUY()
        else
            SELL()
        ...
        Return = true
    elseif Source.close[period] < AVERAGE.DATA[period] and Indicator.Line1[period] < Indicator.Line2[period] then
        if Direction then
            SELL()
        else
            BUY()
