...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=69115
...
function Init()
    ...
    strategy.parameters:addInteger("SAR_Step", "SAR Step", "", 30);
    strategy.parameters:addInteger("TrailingStep", "Trailing Step", "", 5);
end

local MAIN_SOURCE_ID = 1
local main_source;
local AllowTrade, Account, Amount, SAR_Step, TrailingStep;
local TicketB, TicketS, TicketBS, TicketSS;
function Prepare(nameOnly)
    ...
    SAR_Step = instance.parameters.SAR_Step * instance.ask:pipSize();
    TrailingStep = instance.parameters.TrailingStep * instance.ask:pipSize();

    main_source = ExtSubscribe(MAIN_SOURCE_ID, nil, instance.parameters.timeframe, instance.parameters.type, "bar")
end

local TicketBS_command, TicketSS_command;
...
function ExtUpdate(id, source, period)
    ...
    local row = enum:next();
    while row ~= nil do
        if row.Instrument == instance.ask:instrument() then
            if row.BS == "B" then 
                TicketB = row;
            else
                TicketS = row;
    ...
    local row = enum:next();
    while row ~= nil do
        if row.Instrument == instance.ask:instrument() and row.Type == "SE" then
            if row.BS == "B" then 
                TicketBS = row;
            else
                TicketSS = row;
    ...
    if TicketB ~= nil then
        New_Level = instance.bid[NOW] - SAR_Step;
        Old_Level = TicketB.Stop;
        if New_Level - Old_Level >= TrailingStep then
            New_Level = Old_Level + math.floor((New_Level - Old_Level) / TrailingStep) * TrailingStep;

            trading:MoveStop(TicketB, New_Level);
            P_Level = (New_Level + SAR_Step - instance.bid[NOW] + instance.ask[NOW]);
            if TicketSS == nil then
                trading:EntryOrder(instance.ask:instrument())
                ...
                trading:ChangeOrder(TicketSS, New_Level);
                trading:MoveStop(TicketSS, P_Level);
            end
        end
    end

    if TicketS ~= nil then
        New_Level = (instance.ask[NOW] + SAR_Step);
        Old_Level = TicketS.Stop;
        if Old_Level - New_Level >= TrailingStep then
            New_Level = Old_Level - math.floor((Old_Level - New_Level) / TrailingStep) * TrailingStep;
            
            trading:MoveStop(TicketS, New_Level);
            P_Level = (New_Level - SAR_Step - instance.ask[NOW] + instance.bid[NOW]);
            if TicketBS == nil then
                local res = trading:EntryOrder(instance.ask:instrument())
                    :SetAccountID(Account)
                    :SetAmount(Amount)
                    :SetSide("B")
                    :SetRate(New_Level)
                    :SetStop(P_Level)
                    :Execute();
                if res.Finished and not res.Success then
                    core.host:trace(res.Error);
                end
            else
                trading:ChangeOrder(TicketBS, New_Level);
                trading:MoveStop(TicketBS, P_Level);
            end
        end
    end
    
    if TicketB == nil and TicketS == nil then
        if TicketBS_command == nil or TicketBS_command.Finished then
            New_Level = instance.ask[NOW] + SAR_Step / 2;
            P_Level = New_Level - SAR_Step - instance.ask[NOW] + instance.bid[NOW];
            if TicketBS == nil then
                TicketBS_command = trading:EntryOrder(instance.ask:instrument())
                    :SetAccountID(Account)
                    :SetAmount(Amount)
                    :SetSide("B")
                    :SetRate(New_Level)
                    :SetStop(P_Level)
                    :Execute();
                if TicketBS_command.Finished and not TicketBS_command.Success then
                    core.host:trace(TicketBS_command.Error);
                end
            else
                Old_Level = TicketBS.Rate;
                if (Old_Level - New_Level >= TrailingStep) then
                    New_Level = Old_Level - math.floor((Old_Level - New_Level) / TrailingStep) * TrailingStep;
        ...
        if TicketSS_command == nil or TicketSS_command.Finished then
            New_Level = instance.bid[NOW] - SAR_Step / 2;
            P_Level = New_Level + SAR_Step - instance.bid[NOW] + instance.ask[NOW];
            if TicketSS == nil then
                TicketSS_command = trading:EntryOrder(instance.ask:instrument())
                    :SetAccountID(Account)
                    :SetAmount(Amount)
                    :SetSide("S")
                    :SetRate(New_Level)
                    :SetStop(P_Level)
                    :Execute();
                if TicketSS_command.Finished and not TicketSS_command.Success then
                    core.host:trace(TicketSS_command.Error);
                end
            else
                Old_Level=TicketSS.Rate;
                if (New_Level-Old_Level>=TrailingStep) then
                    New_Level=Old_Level+math.floor((New_Level-Old_Level)/TrailingStep)*TrailingStep;
