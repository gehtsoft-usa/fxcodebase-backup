...
-- https://fxcodebase.com/code/viewtopic.php?f=31&p=159071#p159071
...
local alerts = {
    {
        ...
        UpCondition = function(period)
            if period < a:first() + 1 then
                return false
            end
            return a[period] > a[period - 1] and Close[period] > Average[period];
        end,
        ...
        OnChange = true,
        Name = "Upper line cross"
    },
    {
        Stage = 2,
        UpCondition = function(period)
            if period < b:first() + 1 then
                return false
            end
            return b[period] > b[period - 1] and Close[period] < Average[period];
        end,
        DownCondition = function(period)
            if period < b:first() + 1 then
                return false
