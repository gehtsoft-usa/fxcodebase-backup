-- Available @ https://fxcodebase.com/code/viewtopic.php?f=31&t=75732
...
local STRATEGY_NAME = "G-Channel Trend Direction Strategy";
...
function CreateParameters()
    strategy.parameters:addInteger("Period", "Period", "", 100, 1, 2000);
    strategy.parameters:addBoolean("Signal", "Signal Mode", "", false);
    strategy.parameters:addBoolean("use_rollback", "Close positions on rollback", "", false);
    strategy.parameters:addDouble("rollback", "Rollback, pips", "", 10);
end
...
function CreateEntryIndicators(source)
    local profile = core.indicators:findIndicator("G-CHANNEL TREND DETECTION");
    assert(profile ~= nil, "Please, download and install " .. "G-CHANNEL TREND DETECTION" .. ".LUA indicator");
    local indicatorParams = profile:parameters();
    indicatorParams:setBoolean("Signal", instance.parameters.Signal);
    indicatorParams:setInteger("Period", instance.parameters.Period);
    indi = core.indicators:create("G-CHANNEL TREND DETECTION", source, indicatorParams)
end
...
function PriceTouchedABefore(source, period)
    local i = period;
    while i > 0 do
        if source[i] >= indi.a[i] then
            return true;
        end
        if source[i] <= indi.b[i] then
            return false;
        end
        i = i - 1;
    end
...
function PriceTouchedBBefore(source, period)
    local i = period;
    while i > 0 do
        if source[i] <= indi.b[i] then
            return true;
...
function CreateCustomActions()
    ...
    -- end
    if (instance.parameters.use_rollback) then
        local action = {};
        action.ActOnSwitch = false;
        action.Cache = {};
        action.IsPass = function (source, period, periodFromLast, data)
            return core.crossesUnder(source, indi.a, period) and PriceTouchedABefore(source, period);
        end
        if instance.parameters.Direction == "direct" then
            action.Execute = CloseLong;
        else
            action.Execute = CloseShort;
        end
        ...
        action.Cache = {};
        action.IsPass = function (source, period, periodFromLast, data)
            return core.crossesOver(source, indi.b, period) and PriceTouchedBBefore(source, period);
        end
        if instance.parameters.Direction == "direct" then
            action.Execute = CloseShort;
        else
