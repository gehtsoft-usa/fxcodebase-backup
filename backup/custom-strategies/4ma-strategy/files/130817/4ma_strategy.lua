...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=69284
...
local STRATEGY_NAME = "4MA Strategy";
...
function CreateParameters()
    ...
    if not DISABLE_HA_SOURCE then
        strategy.parameters:addBoolean("ha_as_source", "Use HA as source", "", false);
    end
    strategy.parameters:addString("timeframe", "Entry Time frame", "", "m5");
    if USE_CUSTOM_TIMEFRAMES then
        strategy.parameters:setFlag("timeframe", core.FLAG_BARPERIODS_EDIT);
    else
        strategy.parameters:setFlag("timeframe", core.FLAG_BARPERIODS);
    end
    strategy.parameters:addInteger("ma1_period", "MA1 Period", "", 14);
    AddAverages("ma1_method", "MA1 Method", "MVA");

    strategy.parameters:addString("tf2", "Timeframe 2", "", "m15");
    strategy.parameters:setFlag("tf2", core.FLAG_PERIODS);
    strategy.parameters:addInteger("ma2_period", "MA2 Period", "", 14);
    AddAverages("ma2_method", "MA2 Method", "MVA");

    strategy.parameters:addString("tf3", "Timeframe 3", "", "m30");
    strategy.parameters:setFlag("tf3", core.FLAG_PERIODS);
    strategy.parameters:addInteger("ma3_period", "MA3 Period", "", 14);
    AddAverages("ma3_method", "MA3 Method", "MVA");

    strategy.parameters:addString("tf4", "Timeframe 4", "", "H1");
    strategy.parameters:setFlag("tf4", core.FLAG_PERIODS);
    strategy.parameters:addInteger("ma4_period", "MA4 Period", "", 14);
    AddAverages("ma4_method", "MA4 Method", "MVA");

    strategy.parameters:addString("tf_vpc", "Timeframe VPC", "", "H1");
    strategy.parameters:setFlag("tf_vpc", core.FLAG_PERIODS);
    strategy.parameters:addInteger("vpc_length", "Period", "", 15, 2, 2000);
	strategy.parameters:addInteger("vpc_price_smoothing", "Price Smoothing", "", 15);
	strategy.parameters:addInteger("vpc_signal_smoothing", "Signal Smoothing", "", 15);
end
local ma1, ma2, ma3, ma4, vpc;
function CreateEntryIndicators(source)
    ...
    source2 = trading_logic:SubscribeHistory(source.close:instrument(), instance.parameters.tf2, source:isBid());
    ma2 = CreateAverages(instance.parameters.ma2_period, instance.parameters.ma2_method, source2);

    source3 = trading_logic:SubscribeHistory(source.close:instrument(), instance.parameters.tf3, source:isBid());
    ma3 = CreateAverages(instance.parameters.ma3_period, instance.parameters.ma3_method, source3);

    source4 = trading_logic:SubscribeHistory(source.close:instrument(), instance.parameters.tf4, source:isBid());
    ma4 = CreateAverages(instance.parameters.ma4_period, instance.parameters.ma4_method, source4);

    source_vpc = trading_logic:SubscribeHistory(source.close:instrument(), instance.parameters.tf_vpc, source:isBid());
    local profile = core.indicators:findIndicator("VOLUME PRICE CHANGE");
    assert(profile ~= nil, "Please, download and install " .. "VOLUME PRICE CHANGE" .. ".LUA indicator");
    vpc = core.indicators:create("VOLUME PRICE CHANGE", source_vpc, 
        instance.parameters.vpc_length, instance.parameters.vpc_price_smoothing, 
...
function CreateCustomActions()
    ...
    enterLongAction.IsPass = function (source, period, periodFromLast, data)
        if ma1.DATA:size() == 0 or not ma1.DATA:hasData(NOW)
            or ma2.DATA:size() == 0 or not ma2.DATA:hasData(NOW)
            or ma3.DATA:size() == 0 or not ma3.DATA:hasData(NOW)
            or ma4.DATA:size() == 0 or not ma4.DATA:hasData(NOW)
            or not vpc.DATA:hasData(NOW)
        then
            return nil;
        end
        return ma1.DATA[NOW] > source.close[period]
            and ma2.DATA[NOW] > source.close[period]
            and ma3.DATA[NOW] > source.close[period]
            and ma4.DATA[NOW] > source.close[period]
            and vpc.vpc[NOW] < vpc.signal[NOW];
    end
