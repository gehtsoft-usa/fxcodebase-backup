...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=23836
...
function Init() --The strategy profile initialization
    ...
    strategy.parameters:addInteger("SP", "Signal Period", "", 14)
    strategy.parameters:addInteger("LP", "Confirmation Period", "", 28)

    strategy.parameters:addString("Method1", "Signal Method", "Method", "MVA")
    ...
    strategy.parameters:addStringAlternative("Method1", "PPMA", "PPMA", "PPMA")

    strategy.parameters:addString("Method2", "Confirmation Method", "Method", "MVA")
    ...
    strategy.parameters:addStringAlternative("Method2", "PPMA", "PPMA", "PPMA")

    strategy.parameters:addInteger("Filter", "Filter Mode", "", 1)
    strategy.parameters:addIntegerAlternative("Filter", "MA Slope", "", 1)
    strategy.parameters:addIntegerAlternative("Filter", "MA Position", "", 2)
    strategy.parameters:addIntegerAlternative("Filter", "Do not use filter", "", 3)

    strategy.parameters:addGroup("Exit")
    strategy.parameters:addBoolean("Exit", "Use Exit", "", true)

    strategy.parameters:addString("EMO", "Exit Mode", "", "Any")
    strategy.parameters:addStringAlternative("EMO", "Signal", "", "Short")
    strategy.parameters:addStringAlternative("EMO", "Confirmation", "", "Long")
    strategy.parameters:addStringAlternative("EMO", "Any", "", "Any")
...
local Direction
local CustomID
local Exit, LP, SP, Method1, Method2, Price, Filter, EMO
local Indicator2, Indicator1, Short1, Short2
...
function Prepare(nameOnly)
    ...
    assert(core.indicators:findIndicator("PPMA") ~= nil, "Please, download and install PPMA.LUA indicator")
    ...
    if Method1 == "PPMA" then
    assert(core.indicators:findIndicator(Method1) ~= nil, Method1 .. " indicator must be installed");
        Indicator1 = core.indicators:create(Method1, Source, SP)
    else
        Indicator1 = core.indicators:create(Method1, Source[Price], SP)
    end
    Short1 = Indicator1:getStream(0)
    if Method2 == "PPMA" then
    assert(core.indicators:findIndicator(Method2) ~= nil, Method2 .. " indicator must be installed");
        Indicator2 = core.indicators:create(Method2, Source, LP)
    else
        Indicator2 = core.indicators:create(Method2, Source[Price], LP)
    end

    Short1 = Indicator1:getStream(0)
    Short2 = Indicator2:getStream(0)

    first = math.max(Short1:first(), Short2:first())
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if
        core.crossesOver(Source.close, Short1, period) and ((Short2[period] > Short2[period - 1]) or Filter ~= 1) and
            ((Source.close[period] > Short2[period]) or Filter ~= 2)
     then
        if Direction then
            BUY()
        else
    ...
    elseif
        core.crossesUnder(Source.close, Short1, period) and ((Short2[period] < Short2[period - 1]) or Filter ~= 1) and
            ((Source.close[period] < Short2[period]) or Filter ~= 2)
     then
        if Direction then
            SELL()
    ...
    if Exit then
        if EMO ~= "Second" and Source.close[period] < Short1[period] then
            if Direction then
        ...
        if EMO ~= "First" and Source.close[period] > Short2[period] then
            if Direction then
                if haveTrades("S") then
                    exitSpecific("S")
                    Signal("Close Short")
