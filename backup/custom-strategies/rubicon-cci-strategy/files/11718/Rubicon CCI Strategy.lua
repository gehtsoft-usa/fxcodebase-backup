...
function Init()
    ...
    strategy:description(
        "Signals when CCI crosses +/-100 and the signal is confirmed by EMA(CCI) cross of the same level"
    )
...
function Prepare(nameOnly)
    ...
    name =
        name .. ", " .. instance.parameters.TF .. "(" .. ", " .. Parameters["EMA"] .. ", " .. Parameters["CCI"] .. ")"
    ...
    Indicator["RubCCI"] = core.indicators:create("RUBCCI", gSource, instance.parameters.CCI, instance.parameters.EMA)
    Indicator["CCI"] = Indicator["RubCCI"].CCI
    Indicator["EMA"] = Indicator["RubCCI"].EMA

    first = math.max(Indicator["CCI"]:first() + 6, Indicator["EMA"]:first() + 6)
...
function ExtUpdate(id, source, period)
    ...
    if core.crossesOver(Indicator["EMA"], -100, period) then
    ...
    elseif core.crossesUnder(Indicator["EMA"], 100, period) then
        for i = period, period - 5, -1 do
