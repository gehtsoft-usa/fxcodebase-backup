...
-- https://fxcodebase.com/code/viewtopic.php?f=31&t=72218
...
function Init() --The strategy profile initialization
    strategy:name("Fisher Momentum Shift Strategy");
    ...
    strategy.parameters:addGroup("Fisher Calculation");
...
function CreateTradingParameters()
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "FMSS");
...
function Prepare( nameOnly)
	...
	assert(core.indicators:findIndicator("FISHER") ~= nil, "Please, download and install FISHER.LUA indicator");          
    ...
    Indicator = core.indicators:create("FISHER", Source, 	instance.parameters.RangePeriods,  instance.parameters.PriceSmoothing,  instance.parameters.IndexSmoothing,  core.rgb(0, 255, 0),  core.rgb(255, 0, 0)  );
    MA = core.indicators:create(instance.parameters.Method, Source.close, 	instance.parameters.Period);
...
function EntryFunction( now,period)
    ...
    and  Indicator.DATA:colorI(period) ==  core.rgb(0, 255, 0)
	then	
          if Direction then
                BUY();
            else
    ...
    and  Indicator.DATA:colorI(period) ==  core.rgb(255, 0, 0)
	then
            if Direction then
                SELL();
            else
