...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=62895
...
function Init() --The strategy profile initialization
    strategy:name("3 MA Cross Strategy with QQE Confirmation ")
    ...
    strategy.parameters:addBoolean("Filter", "Use QQE Filter", "", true)

    strategy.parameters:addString("QQE_TF", "Time frame", "", "m1")
    strategy.parameters:setFlag("QQE_TF", core.FLAG_PERIODS)
...
local Fast, Medium, Slow
local Filter, QQE, QQE_Source, QQE_TF
local RF, RSP, AP, F, S
...
function Prepare(nameOnly)
    ...
    QQE_TF = instance.parameters.QQE_TF
    ...
    if Filter then
        ...
        s2, e2 = core.getcandle(QQE_TF, core.now(), 0, 0)
    ...
    assert(core.indicators:findIndicator(instance.parameters.Fast_Method) ~= nil, instance.parameters.Fast_Method .. " indicator must be installed");
    Fast = core.indicators:create(instance.parameters.Fast_Method, Source.close, instance.parameters.Fast_Period)
    assert(core.indicators:findIndicator(instance.parameters.Medium_Method) ~= nil, instance.parameters.Medium_Method .. " indicator must be installed");
    Medium = core.indicators:create(instance.parameters.Medium_Method, Source.close, instance.parameters.Medium_Period)
    assert(core.indicators:findIndicator(instance.parameters.Slow_Method) ~= nil, instance.parameters.Slow_Method .. " indicator must be installed");
    Slow = core.indicators:create(instance.parameters.Slow_Method, Source.close, instance.parameters.Slow_Period)

    first = math.max(Fast.DATA:first(), Medium.DATA:first(), Slow.DATA:first())
    QQE_Source = ExtSubscribe(3, nil, QQE_TF, instance.parameters.Type == "Bid", "bar")
    QQE = core.indicators:create("QQE", QQE_Source.close, RF, RSP, AP, F, S, true, true)
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if
     ...
     then
        ...
        if Filter then
            period1 = core.findDate(QQE_Source, TickSource:date(period), false)
        end
        ...
        if Filter then
            period1 = core.findDate(QQE_Source, Source:date(period), false)
        end
    end
    ...
    if EntryExecutionType == "Live" and id == 1 or EntryExecutionType ~= "Live" and id ~= 1 then
        EntryFunction(now, period, period1)
    end
...
function ExitFunction(now, period)
    ...
    if (LastExit == Source:serial(period)) or not Slow.DATA:hasData(period) then
        return
    end
    if Source.close[period] < Slow.DATA[period] then
        if Direction then
...
function EntryFunction(now, period, period1)
    local Return = false
    ...
    if not Filter then
        ...
        if not QQE.QQE:hasData(period1) or not QQE.TS1:hasData(period1) then
            return;
        end
        if QQE.QQE[period1] > QQE.TS1[period1] then
            ItIs = 1
        else
            ItIs = -1
        end
    ...
    if not Fast.DATA:hasData(period)
        or not Medium.DATA:hasData(period)
        or not Slow.DATA:hasData(period)
    then
        return;
    end
    -- only buy if we have a fast cross over slow and the price is above the moving averages.
    if
        Fast.DATA[period] < Medium.DATA[period] and Fast.DATA[period - 1] >= Medium.DATA[period - 1] and
            Fast.DATA[period] > Slow.DATA[period] and
            Medium.DATA[period] > Slow.DATA[period] and
            (ItIs == 1 or ItIs == 0)
     then
        if Direction then
            BUY()
