...
function Init()
    ...
    strategy.parameters:addInteger("wait_time", "Wait time, sec", "", 5);
...
function Prepare(name_only)
    wait_time = instance.parameters.wait_time / 86400.0;
...
function EntryFunction(source, period)
    ...
    for _, action in ipairs(EntryActions) do
        ...
        if action.Cache[current_serial] ~= isPass then
            action.LastTime = core.now();
        end
        ...
        if isPass and (not action.ActOnSwitch or action.Cache[action.LastSerial] == false) and (core.now() - action.LastTime) > wait_time then
