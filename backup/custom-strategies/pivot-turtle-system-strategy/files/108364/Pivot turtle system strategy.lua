...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=63923
...
function Init() --The strategy profile initialization
    strategy:name("Pivot turtle system strategy");
...
function CreateTradingParameters()
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "PTSS");
...
function ExtUpdate(id, source, period)  -- The method called every time when a new bid or ask price appears.
   ...
   -- get only time
	...
	if  not  Pivot.DATA:hasData(period)
	then
        return;
    end
	
    local DATA, Number=FindLastPivot(period);
    ...
    if DATA[1]> DATA[3] 
	and   Pivot.DATA[period]~= Pivot.DATA[period-1]	
	then	
          if Direction then
                BUY();
            else
...
function FindLastPivot(period)
local DATA={};
local Number=0;

Number=Number+1;
DATA[Number]=Pivot.DATA[period];

     for Index= period-1, first, -1 do
    
	
			if Pivot.DATA[Index]~= Pivot.DATA[Index-1] then
			Number=Number+1;
			DATA[Number]=Pivot.DATA[period-1];
			end
