...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=64088
...
function Init() --The strategy profile initialization
    strategy:name("Normalized LWMA Slope Strategy");
    ...
    strategy.parameters:addInteger("LWMA_Period1", "LWMA Period", "", 56);
    ...
    strategy.parameters:addInteger("LWMA_Period2", "LWMA Period", "", 56);
...
function CreateTradingParameters()
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "NLWMAS");
...
local TF1, LWMA_Period1, ATR_Period1,TF2, LWMA_Period2, ATR_Period2;
local LWMA1,LWMA2;
...
function Prepare( nameOnly)
   CustomID = instance.parameters.CustomID;
	...
	LWMA_Period1= instance.parameters.LWMA_Period1;
	ATR_Period1= instance.parameters.ATR_Period1;
	TF2= instance.parameters.TF2;
	LWMA_Period2= instance.parameters.LWMA_Period2;
    ...
    LWMA1 = core.indicators:create("NORMALIZED LWMA SLOPE", Source1, LWMA_Period1, ATR_Period1  );
    LWMA2 = core.indicators:create("NORMALIZED LWMA SLOPE", Source2, LWMA_Period2, ATR_Period2  );
...
function ExtUpdate(id, source, period)  -- The method called every time when a new bid or ask price appears.
   ...
   return;
    ...
    -- update indicators.
    LWMA1:update(core.UpdateLast);
	LWMA2:update(core.UpdateLast);

	if not LWMA1.DATA:hasData(period1)
    or not LWMA2.DATA:hasData(period2)
	then
        return;
    end
...
2. NORMALIZED LWMA SLOPE IN M15 CROSS UNDER SELL LEVEL
     ...
     if ( LastEntry == Source1:serial(period1) and   EntyExecutionType== "Live")   then
	 return;
	 end	
	  
    -- only buy if we have a fast cross over slow and the price is above the moving averages.
    if   LWMA1.DATA[period1] > BL1
	and LWMA1.DATA[period1-1] <= BL1
	and LWMA2.DATA[period2] > BL2
	then	
          if Direction then
                BUY();
            else
    ...
    elseif   LWMA1.DATA[period1] < SL1
	and LWMA1.DATA[period1-1] >= SL1
	and LWMA2.DATA[period2] < SL2
	then
            if Direction then
                SELL();
            else
...
function haveTrades(BuySell) 
    ...
    while (row ~= nil) do
        if row.AccountID == Account
		and row.OfferID == Offer and PositionCapInstrument
		and  row.QTXT == CustomID  and PositionCapCustomID
		and (row.BS == BuySell or BuySell == nil) then
            found = true;
            break;
        end
...
function exitSpecific(BuySell)
    ...
    while (not found) and (row ~= nil) do
        ...
        if row.AccountID == Account 
		and row.OfferID == Offer and ExitInstrument
		and  row.QTXT == CustomID  and ExitCustomID
