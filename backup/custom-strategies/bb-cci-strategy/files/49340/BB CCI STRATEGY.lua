...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=28088
...
function Init() --The strategy profile initialization
    ...
    strategy.parameters:addInteger("CP", "CCI Period", "", 20)
    ...
    strategy.parameters:addDouble("BM", "Bollinger Deviation", "", 2)
...
function Prepare(nameOnly)
    ...
    Indicator[1] = core.indicators:create("BB", Source[Price], BP, BM)
    Short["T"] = Indicator[1]:getStream(0)
    Short["B"] = Indicator[1]:getStream(1)

    Indicator[2] = core.indicators:create("CCI", Source, CP)
    Short["CCI"] = Indicator[2]:getStream(0)

    first = math.max(Short["T"]:first(), Short["CCI"]:first())
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if Source.close[period] > Short["T"][period] and core.crossesOver(Short["CCI"], BL, period) then
        if Direction then
            BUY()
        else
            SELL()
        end
    elseif Source.close[period] < Short["B"][period] and core.crossesUnder(Short["CCI"], SL, period) then
        if Direction then
            SELL()
        else
            BUY()
    ...
    if core.crossesUnder(Source.close, Short["B"], period) then
        if Direction then
    ...
    if core.crossesOver(Source.close, Short["T"], period) then
        if Direction then
            if haveTrades("S") then
                exit("S")
                Signal("Close Short")
