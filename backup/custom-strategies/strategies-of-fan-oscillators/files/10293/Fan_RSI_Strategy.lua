...
function Init() --The strategy profile initialization
    strategy:name("Fan RSI strategy")
    strategy:description("Fan RSI strategy")
    ...
    strategy.parameters:addGroup("Fan RSI indicator parameters")
...
function Prepare(nameOnly)
    ...
    assert(core.indicators:findIndicator("FAN_RSI2") ~= nil, "FAN_RSI2 indicator must be installed");
    if Price == "close" then
        FRSI =
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if (FRSI.DATA:first() > (period - 1)) then
        return
    end
    ...
    if (FRSI.BuffLine[period - 1] < instance.parameters.LevelB and FRSI.BuffLine[period] >= instance.parameters.LevelB) then
    ...
    if (FRSI.BuffLine[period - 1] > instance.parameters.LevelS and FRSI.BuffLine[period] <= instance.parameters.LevelS) then
        if instance.parameters.TypeSignal == "direct" then
            MustS = true
        else
            MustB = true
