...
--http://fxcodebase.com/code/viewtopic.php?f=31&t=69414
...
local STRATEGY_NAME = "TFS Strategy";
function CreateParameters() 
    strategy.parameters:addInteger("TetherLen", "TetherLen", "", 50);
    strategy.parameters:addInteger("OscAvgLength", "OscAvgLength", "", 7);
    strategy.parameters:addInteger("MBOFastAvg", "MBOFastAvg", "", 25);
    strategy.parameters:addInteger("MBOSlowAvg", "MBOSlowAvg", "", 200);
end

local TetherLen, OscAvgLength, MBOFastAvg, MBOSlowAvg;
function CreateEntryIndicators(source)
    TetherLen = instance.parameters.TetherLen;
    OscAvgLength = instance.parameters.OscAvgLength;
    MBOFastAvg = instance.parameters.MBOFastAvg;
    MBOSlowAvg = instance.parameters.MBOSlowAvg;
    
    indi_1 = core.indicators:create("MVA", source, instance.parameters.MBOFastAvg);
    indi_2 = core.indicators:create("MVA", source, instance.parameters.MBOSlowAvg);
end
...
function GetVol(source, period)
    if period < OscAvgLength then
        return;
    end
    local VolAccum = 0;
    for i = 0, OscAvgLength - 1 do
        if source.close[period - i] > source.open[period - i] then
            VolAccum = VolAccum + source.volume[period - i];
        else
            VolAccum = VolAccum - source.volume[period - i];
        end
    end
    return VolAccum / OscAvgLength;
end

function GetTether(source, period)
    if period < TetherLen then
        return nil;
    end
    core.host:trace(period .. " " .. tostring(source:barSize()))
    local ll, hh = mathex.minmax(source, period - TetherLen, period);
    return (hh + ll) / 2;
end

function IsEntryLong(source, period)
    if not indi_1.DATA[period - 1] or not indi_2.DATA[period - 1] then
        return;
    end
    local MBO = indi_1.DATA[period] - indi_2.DATA[period];
    local MBO_1 = indi_1.DATA[period - 1] - indi_2.DATA[period - 1];
    local tether = GetTether(source, period);
    if tether == nil then
        return;
    end
    local vol = GetVol(source, period);
    if vol == nil then
        return;
    end
    return core.crossesOver(source.close, tether, period) and vol > 0 and MBO > MBO_1;
end
function IsEntryShort(source, period)
    ...
    return core.crossesUnder(source.close, tether, period) and vol < 0 and MBO < MBO_1;
end
function IsExitLong(source, period)
    ...
    return core.crossesUnder(source.close, tether, period);
end
function IsExitShort(source, period)
    ...
    return core.crossesOver(source.close, tether, period);
end
