...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=62971
...
function Init() --The strategy profile initialization
    strategy:name("Extrem TMA Line MA Strategy")
    ...
    strategy.parameters:addGroup("Extrem TMA Line Calculation")
...
local first
local TMA_Period, ATR_Period, ATR_Mult, TrendThreshold, Redraw, Exit
local Indicator, MA, Period, Method
local Direction
local CustomID
local Price
...
function Prepare(nameOnly)
    ...
    assert(
        core.indicators:findIndicator("TICK_EXTREME_TMA_LINE") ~= nil,
        "Please, download and install TICK_EXTREME_TMA_LINE.LUA indicator"
    )
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if MA.DATA[period] > Indicator.DATA[period] and MA.DATA[period - 1] <= Indicator.DATA[period - 1] then
        if Direction then
            BUY()
        else
            SELL()
    ...
    elseif MA.DATA[period] < Indicator.DATA[period] and MA.DATA[period - 1] >= Indicator.DATA[period - 1] then
        if Direction then
            SELL()
        else
            BUY()
