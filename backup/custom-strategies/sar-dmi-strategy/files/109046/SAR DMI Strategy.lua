...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=64094
...
function Init() --The strategy profile initialization
    strategy:name("SAR DMI Strategy");
	...
	strategy.parameters:addBoolean("Cross", "On Cross Only", "", true)
...
function Prepare( nameOnly)
	...
	first=math.max(DMI.DATA:first(), SAR.DN:first(), SAR.UP:first());
...
function ExtUpdate(id, source, period)  -- The method called every time when a new bid or ask price appears.
   ...
   -- get only time
	...
	if not (SAR.UP:hasData(period) or SAR.DN:hasData(period)) 	
...
function ExitFunction(period, now)
      ...
      return ;
	   ...
	   if ( LastExit == Source:serial(period) and   ExitExecutionType== "Live")   then
	 return;
	 end	  
	  
    

		            if   not SAR:getStream(1):hasData(period)
					then 			
								if   Direction then
					  ...
					  if not SAR:getStream(0):hasData(period)
		               then
								   if   Direction then    
										if haveTrades("S") then
										   exitSpecific("S");
...
function EntryFunction(period,now)
	...
	and (not Cross or  (not SAR:getStream(1):hasData(period-1 ) and Cross))
	...
	and (not Cross or not (SAR:getStream(0):hasData(period-1 ) and Cross))
	and DMI.DIP[period]< DMI.DIM[period]
	then
            if Direction then
                SELL();
