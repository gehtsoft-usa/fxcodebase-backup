...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=63607
...
function Init() --The strategy profile initialization
    ...
    strategy.parameters:addBoolean("Filter", "Use ADX Entry Filter", "", true)
    strategy.parameters:addBoolean("Exit", "Use ADX Exit Filter", "", true)
...
local first
local DMI, ADX
local DMI_Period, ADX_Period, ADX_Level, Filter
local Direction
local CustomID
local Exit
...
function Prepare(nameOnly)
    ...
    if Filter then
        ...
        first = math.max(ADX.DATA:first(), first)
    end
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if DMI.DIP[period] > DMI.DIM[period] and DMI.DIP[period - 1] <= DMI.DIM[period - 1] and (ADX_Flag or not Filter) then
        if Direction then
            BUY()
        else
            SELL()
    ...
    elseif DMI.DIP[period] < DMI.DIM[period] and DMI.DIP[period - 1] >= DMI.DIM[period - 1] and (ADX_Flag or not Filter) then
        if Direction then
            SELL()
        else
            BUY()
