...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=11360
...
function Init() --The strategy profile initialization
    strategy:name("2cci ema strategy")
    ...
    strategy.parameters:addInteger("SP", "Short CCI Period", "", 14)
    strategy.parameters:addInteger("LP", "Long CCI Period", "", 50)
...
function Prepare(nameOnly)
    ...
    Indicator[1] = core.indicators:create("CCI", Source, SP)
    Short["Short"] = Indicator[1]:getStream(0)

    Indicator[2] = core.indicators:create("CCI", Source, LP)
    Short["Long"] = Indicator[2]:getStream(0)

    Indicator[3] = core.indicators:create("EMA", Source.high, EP)
    Short["High"] = Indicator[2]:getStream(0)

    Indicator[4] = core.indicators:create("EMA", Source.low, EP)
    Short["Low"] = Indicator[4]:getStream(0)

    first = math.max(Short["Short"]:first(), Short["Long"]:first(), Short["High"]:first(), Short["Low"]:first())
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if
        Source.close[period] > Short["High"][period] and Short["Long"][period] > BL and
            core.crossesOver(Short["Short"], BL, period)
     then
        if Direction then
            BUY()
        else
    ...
    elseif
        Source.close[period] < Short["Low"][period] and Short["Long"][period] < SL and
            core.crossesUnder(Short["Short"], SL, period)
     then
        if Direction then
            SELL()
        else
    ...
    if
        core.crossesUnder(Source.close, Short["High"], period) or core.crossesUnder(Short["Long"], BL, period) or
            core.crossesUnder(Short["Short"], BL, period)
     then
        if Direction then
    ...
    elseif
        core.crossesOver(Source.close, Short["Low"], period) or core.crossesOver(Short["Long"], SL, period) or
            core.crossesOver(Short["Short"], SL, period)
     then
        if Direction then
            if haveTrades("S") then
                exit("S")
