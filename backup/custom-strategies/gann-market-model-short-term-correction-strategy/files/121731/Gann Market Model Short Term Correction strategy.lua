...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=66855
...
function Init() --The strategy profile initialization
    strategy:name("Gann Market Model Short Term Correction strategy");
	...
	strategy.parameters:addInteger("Period1", "1. Period", "", 50, 1, 2000); 
    strategy.parameters:addInteger("Period2", "2. Period", "", 100, 1, 2000); 
	strategy.parameters:addInteger("Period3", "3. Period", "", 200, 1, 2000); 
...
function Prepare( nameOnly)
	...
	assert(core.indicators:findIndicator("GANN MARKET MODEL") ~= nil, "Please, download and install GANN MARKET MODEL.LUA indicator");          
    ...
    Indicator = core.indicators:create("GANN MARKET MODEL", Source, Period1, Period2, Period3  );
...
function EntryFunction( now,period)
    ...
    if   Indicator.DATA[period]> 2
	and  Indicator.DATA[period-1]<= 2
	then	
          if Direction then
                BUY();
            else
		...
		Return=true;
    elseif   Indicator.DATA[period]< 3
	and  Indicator.DATA[period-1]>= 3
	then
            if Direction then
                SELL();
            else
