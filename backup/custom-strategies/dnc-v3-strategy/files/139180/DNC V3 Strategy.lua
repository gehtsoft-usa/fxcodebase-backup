...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=70633
...
function CreateParameters() 
	...
	strategy.parameters:addString("MD", "Show Close or High/Low", "", "Close");
    strategy.parameters:addStringAlternative("MD", "Close", "", "Close");
    strategy.parameters:addStringAlternative("MD", "High/Low", "", "HighLow");
	
	strategy.parameters:addGroup("Selector");
    strategy.parameters:addString("SHL", "Show High/Low lines", "Show High/Low lines", "Both"); 
    strategy.parameters:addStringAlternative("SHL", "Both lines", "", "Both"); 
    strategy.parameters:addStringAlternative("SHL", "High only", "", "High"); 
    strategy.parameters:addStringAlternative("SHL", "Low only", "", "Low"); 
	strategy.parameters:addBoolean("AC", "Analyze the current period", "", true);  
end

local main_source;
local dnc;
function CreateEntryIndicators(source)
    local profile = core.indicators:findIndicator("DNC_V3");
    assert(profile ~= nil, "Please, download and install " .. "DNC_V3" .. ".LUA indicator");
    local indicatorParams = profile:parameters();
    indicatorParams:setInteger("N", instance.parameters.N);
    indicatorParams:setString("MD", instance.parameters.MD);
    indicatorParams:setString("SHL", instance.parameters.SHL);
    indicatorParams:setBoolean("SM", true);
    indicatorParams:setBoolean("SUB", true);
    indicatorParams:setBoolean("AC", instance.parameters.AC);
    dnc = core.indicators:create("DNC_V3", source, indicatorParams)
end
...
function IsEntryLong(source, period)
    if not dnc.DATA:hasData(period - 1) then
        return false;
    end
    local position_exists = PositionExists("B");
    return core.crossesOver(source.close, dnc.low, period)
        or (core.crossesOver(source.close, dnc.DM, period) and position_exists)
        or (core.crossesOver(source.close, dnc.high, period) and position_exists)
        or (core.crossesOver(source.close, dnc.DU, period) and position_exists);
end
function IsEntryShort(source, period)
    if not dnc.DATA:hasData(period - 1) then
        return false;
    end
    local position_exists = PositionExists("S");
    return core.crossesUnder(source.close, dnc.high, period)
        or (core.crossesUnder(source.close, dnc.DM, period) and position_exists)
        or (core.crossesUnder(source.close, dnc.low, period) and position_exists)
        or (core.crossesUnder(source.close, dnc.DN, period) and position_exists)
end
...
function CreateStopMove()
    ...
    function controller:DoBreakeven()
        ...
        if trade.BS == "B" then
            if core.crossesOver(main_source.close, dnc.DM, NOW) or main_source.high[NOW] > dnc.DM[NOW] then
                self._command = self._parent._trading:MoveStop(trade, dnc.low[NOW]);
                return false;
            end
            if core.crossesOver(main_source.close, dnc.high, NOW) or main_source.high[NOW] > dnc.high[NOW] then
                self._command = self._parent._trading:MoveStop(trade, dnc.DM[NOW]);
                return false;
            end
            if core.crossesOver(main_source.close, dnc.DU, NOW) or main_source.high[NOW] > dnc.DU[NOW] then
                self._command = self._parent._trading:MoveStop(trade, dnc.high[NOW]);
                return false;
            end
        else
            if core.crossesUnder(main_source.close, dnc.DM, NOW) or main_source.low[NOW] < dnc.DM[NOW] then
                self._command = self._parent._trading:MoveStop(trade, dnc.high[NOW]);
                return false;
            end
            if core.crossesUnder(main_source.close, dnc.low, NOW) or main_source.low[NOW] < dnc.low[NOW] then
                self._command = self._parent._trading:MoveStop(trade, dnc.DM[NOW]);
                return false;
            end
            if core.crossesUnder(main_source.close, dnc.DN, NOW) or main_source.low[NOW] < dnc.DN[NOW] then
                self._command = self._parent._trading:MoveStop(trade, dnc.low[NOW]);
                return false;
            end
        end
...
function OpenTrade(side)
    ...
    if side == "B" then
        valuemap.RateStop = dnc.DN[NOW];
    else
        valuemap.RateStop = dnc.DU[NOW];
    end
    ...
    if success then
        local controller = CreateStopMove()
            :SetRequestID(msg)
    end
end
