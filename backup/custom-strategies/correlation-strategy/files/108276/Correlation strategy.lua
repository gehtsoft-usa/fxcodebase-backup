...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=63902
...
function Init() --The strategy profile initialization
    strategy:name("Correlation strategy")
    ...
    strategy.parameters:addGroup("Prices")
    strategy.parameters:addInteger("N", "Number of bars", "", 14, 5, 100)
    strategy.parameters:addString("Src", "The source stream", "", "close")
    ...
    strategy.parameters:addString("INST", "The instrument to compare with", "", "")
    strategy.parameters:setFlag("INST", core.FLAG_INSTRUMENTS)
    strategy.parameters:addString("Dst", "The  stream to compare width", "", "close")
    strategy.parameters:addStringAlternative("Dst", "open", "", "open")
    strategy.parameters:addStringAlternative("Dst", "high", "", "high")
    strategy.parameters:addStringAlternative("Dst", "low", "", "low")
    strategy.parameters:addStringAlternative("Dst", "close", "", "close")
    strategy.parameters:addStringAlternative("Dst", "median", "", "median")
    strategy.parameters:addStringAlternative("Dst", "typical", "", "typical")
    strategy.parameters:addStringAlternative("Dst", "weighted", "", "weighted")
...
function Prepare(nameOnly)
    ...
    assert(
        core.indicators:findIndicator("CORRELATION") ~= nil,
        "Please, download and install CORRELATION.LUA indicator"
    )
    ...
    Indicator = core.indicators:create("CORRELATION", Source, N, Src, INST, Dst)
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if (Indicator.DATA[period] > 0 and Flag == 1) or (Indicator.DATA[period] < 0 and Flag == -1) then
        if Direction then
            BUY()
        else
            SELL()
    ...
    elseif (Indicator.DATA[period] > 0 and Flag == -1) or (Indicator.DATA[period] < 0 and Flag == 1) then
        if Direction then
            SELL()
        else
            BUY()
