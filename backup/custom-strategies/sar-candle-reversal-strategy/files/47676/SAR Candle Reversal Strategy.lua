...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=27300
...
function Init() --The strategy profile initialization
    strategy:name("SAR Candle Reversal Strategy");
    ...
    strategy.parameters:addBoolean("One", "Use SAR Exit", "", true);
	strategy.parameters:addBoolean("Two", "Use Price Exit", "", true);
...
function Prepare(nameOnly)
    ...
    Indicator[1] = core.indicators:create("SAR", Source, Step, Max   );
...
function ExtUpdate(id, source, period)  -- The method called every time when a new bid or ask price appears.
   ...
   -- get only time
    ...
    if period < first +1  then
        ...
        if    not( Indicator[1].DN:hasData(period - 1)) and  Indicator[1].DN:hasData(period)
		...
		elseif not( Indicator[1].UP:hasData(period - 1)) and  Indicator[1].UP:hasData(period)  
		...
		if Exit then
		            ...
		            if     ( not  Indicator[1].DN:hasData(period)   and  One) 
					or  (   Source.close[period] < Source.low[period-1]   and   Source.close[period] < Source.low[period-2]   and  Two) 
					then 			
								if   Direction then
					  ...
					  if   ( not  Indicator[1].UP:hasData(period)   and One) 
					  or  (   Source.close[period] > Source.high[period-1]   and  Source.close[period] > Source.high[period-2]   and  Two) 
		               then
								   if   Direction then    
										if haveTrades("S") then
										   exit("S");
