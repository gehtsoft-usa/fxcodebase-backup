...
function Init()
    ...
    strategy.parameters:addGroup("Asymmetric Fractal Parameters");
    strategy.parameters:addInteger("FrameBefore", "Number of bars before fractal", "Number of bars before fractal", 5);
    strategy.parameters:addInteger("FrameAfter", "Number of bars after fractal", "Number of bars after fractal", 2);
...
function ExtUpdate(id, source, period)
    if id ~= 1 or not tradeExist then
	return;
	end
	
	

        if period < tsource:first()+FrameBefore then
		return;
		end
            ...
            if trade.BS == "B" then
                for i=1,FrameBefore,1 do
                 if  tsource.low[period-FrameAfter]>=tsource.low[period-i-FrameAfter] then
				  
                  Fl=false;
                 end
                end
                if Fl==true then
                 stopValue=tsource.low[period-FrameBefore]-instance.parameters.Indent*instance.bid:pipSize();
                ...
                for i=1,FrameBefore,1 do
                 if  tsource.high[period-FrameAfter]<=tsource.high[period-i-FrameAfter] then
                  Fl=false;
                 end
                end
                if Fl==true then
                 stopValue=tsource.high[period-FrameBefore]+instance.parameters.Indent*instance.bid:pipSize();
                 if instance.parameters.MoveBack==false then
                  if trade.StopOrderID ~= "" and trade.StopOrderID ~= nil then
                   if stopValue>trade.Stop then
                    return ;
   ...
   for i=1, FrameAfter, 1 do
	   if tsource.high[period-FrameAfter]<=tsource.high[period-FrameAfter+i] then
		return ;
	   end
	   if tsource.low[period-FrameAfter]>=tsource.low[period-FrameAfter+i] then
		return ;
	   end
  end
