...
function AddSL(i)
    ...
    strategy.parameters:addBoolean("use_" .. i, i .. ". Use this level", "", false);
    
    strategy.parameters:addDouble("pl_" .. i, "P/L for Partial Close, in pips", "Negative for stops", (i + 1) * 5);
    strategy.parameters:addDouble("price_" .. i, "Rate for Partial Close", "", 0);  
    strategy.parameters:setFlag("price_" .. i, core.FLAG_PRICE);
    strategy.parameters:addBoolean("in_pl_" .. i, "Use Profit level in pips", "", false);
    strategy.parameters:addInteger("amount_" .. i, "Amount for Partial Close", "", 1);
    strategy.parameters:addBoolean("move_stop_" .. i, "Move stop", "", true);
    strategy.parameters:addDouble("stop_" .. i, "Stop level, pips", "", i * 5);
    
    strategy.parameters:addString("amount_size_selector_" .. i, i ..". Amount Type Selector", "", "Lots");
...
function Prepare(nameOnly)
    ...
    for i = 1, 5 do
        if instance.parameters:getBoolean("use_" .. i) then
            local level = {};
            if instance.parameters:getBoolean("in_pl_" .. i) then
                if trade.BS == "B" then
                    level.rate = trade.Open + instance.parameters:getDouble("pl_" .. i) * source:pipSize();
                else
                    level.rate = trade.Open - instance.parameters:getDouble("pl_" .. i) * source:pipSize();
                end
            else
                level.rate = instance.parameters:getDouble("price_" .. i);
            end
            ...
            if instance.parameters:getBoolean("move_stop_" .. i) then
...
function trading:MoveStop(trade, stop_rate)
