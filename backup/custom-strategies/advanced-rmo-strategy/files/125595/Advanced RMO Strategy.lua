...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=68305
...
function Init() --The strategy profile initialization
    strategy:name("Advanced RMO Strategy");
    ...
    strategy.parameters:addInteger("Steps", "Steps", "Steps",10);
	...
	strategy.parameters:addInteger("Period", "Base Period", "Period", 2);
...
local BaseSize;
--local Method;
local Period1,Period2,Steps, Period;
...
function Prepare(nameOnly)
    ...
    Indicator  = core.indicators:create("ADVANCED RMO", Source[Price],Steps,Period1, Period2, Period  );
	--Short[1] = Indicator[1]:getStream(0);
	Short[1] = Indicator:getStream(1);
	Short[2] = Indicator:getStream(2);
...
function ExtUpdate(id, source, period)  -- The method called every time when a new bid or ask price appears.
   ...
   -- get only time
	...
	--"RMO/1.Signal"   "RMO/2.Signal" "1.Signal/2.Signal"  "Any"
        ...
        and Short[1][period-1]<=  Short[2][period-1]			
		then
