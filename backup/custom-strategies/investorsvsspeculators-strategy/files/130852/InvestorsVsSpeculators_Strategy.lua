...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=69270
...
local DISABLE_HA_SOURCE = true;

local STRATEGY_NAME = "InvestorsVsSpeculators Strategy";
...
function CreateParameters() 
    ...
    strategy.parameters:addString("Method", "Calculation Method", "", "CS")
    strategy.parameters:addStringAlternative("Method", "Classic", "", "CS")
    strategy.parameters:addStringAlternative("Method", "TradeStation", "", "TS")

    strategy.parameters:addInteger("move", "Move", "", 10)

    strategy.parameters:addString("tf2", "Confirmation Timeframe", "", "m5");
    strategy.parameters:setFlag("tf2", core.FLAG_PERIODS);
end
local indi, confirm_indi;
function CreateEntryIndicators(source) 
    local profile = core.indicators:findIndicator("INVESTORSVSSPECULATORS WITH  CHANGE ALERT");
    assert(profile ~= nil, "Please, download and install " .. "INVESTORSVSSPECULATORS WITH  CHANGE ALERT" .. ".LUA indicator");
    local indicatorParams = profile:parameters();
    indicatorParams:setInteger("Period", instance.parameters:getInteger("Period"));
    indicatorParams:setInteger("move", instance.parameters:getInteger("move"));
    indicatorParams:setString("Method", instance.parameters:getString("Method"));
    indicatorParams:setString("Type", "Both");
    indicatorParams:setBoolean("strategy_output", true);
    indicatorParams:setBoolean("Show", false);
    indicatorParams:setBoolean("ShowAlert", false);
    indicatorParams:setBoolean("PlaySound", false);
    indicatorParams:setBoolean("SendEmail", false);
    indi = core.indicators:create("INVESTORSVSSPECULATORS WITH  CHANGE ALERT", source, indicatorParams)

    local confirm_source = trading_logic:SubscribeHistory(source.close:instrument(), instance.parameters.tf2, source:isBid());
    local indicatorParams = profile:parameters();
    indicatorParams:setInteger("Period", instance.parameters:getInteger("Period"));
    indicatorParams:setInteger("move", instance.parameters:getInteger("move"));
    indicatorParams:setString("Method", instance.parameters:getString("Method"));
    ...
    indicatorParams:setBoolean("PlaySound", false);
    indicatorParams:setBoolean("SendEmail", false);
    confirm_indi = core.indicators:create("INVESTORSVSSPECULATORS WITH  CHANGE ALERT", confirm_source, indicatorParams)
end
...
function UpdateIndicators()
    ...
    confirm_indi:update(core.UpdateLast);
end
...
function CreateCustomActions()
    ...
    enterLongAction.IsPass = function (source, period, periodFromLast, data)
        if confirm_indi.DATA:size() == 0 then
            return nil;
        end
        return indi.strat_signal_1[period] == 1 and confirm_indi.Investors[NOW] > confirm_indi.Speculators[NOW];
    end
