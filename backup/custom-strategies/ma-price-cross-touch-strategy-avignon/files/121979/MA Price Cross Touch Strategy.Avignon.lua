...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if CrossType == "Cross" then
        if ((core.crossesUnder(Source[PIN], indicator[1].DATA, period) and Confirmation ~= "Two") or
            (core.crossesUnder(Source[PIN], indicator[2].DATA, period) and Confirmation ~= "One"))
            and (not instance.parameters.confirm_long or indicator.LongMA.DATA[period] < indicator.LongMA.DATA[period - 1])
        then
            if Direction then
                SELL()
            else
        ...
        elseif ((core.crossesOver(Source[PIN], indicator[1].DATA, period) and Confirmation ~= "Two") or
            (core.crossesOver(Source[PIN], indicator[2].DATA, period) and Confirmation ~= "One"))
            and (not instance.parameters.confirm_long or indicator.LongMA.DATA[period] > indicator.LongMA.DATA[period - 1])
        then
            if Direction then
                BUY()
