...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=13893
...
function Init() --The strategy profile initialization
    strategy:name("Trading day average strategy")
    strategy:description("Trading day average strategy")
    strategy:setTag("NonOptimizableParameters", "Email,SendEmail,SoundFile,RecurrentSound,PlaySound, ShowAlert")

    strategy.parameters:addGroup("Parameters")
    strategy.parameters:addString("BeginTime", "Begin time of the trading day", "", "03:00")
    strategy.parameters:addBoolean("CloseAtEnd", "Close orders at begin of new trading day", "", true)
...
function Prepare(nameOnly)
    ...
    assert(
        core.indicators:findIndicator("TRADING_DAY_AVERAGE") ~= nil,
        "Please, download and install TRADING_DAY_AVERAGE.LUA indicator"
    )
    Source = ExtSubscribe(2, nil, instance.parameters.TF, true, "bar")
    TDA = core.indicators:create("TRADING_DAY_AVERAGE", Source, instance.parameters.BeginTime)
    ...
    local BeginM = string.sub(BeginTime, Pos + 1)
    BeginT = 3600 * BeginH + 60 * BeginM
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    TDA:update(core.UpdateLast)

    -- Check that we have enough data
    if (TDA.DATA:first() > (period - 1)) then
        return
    end
    ...
    if source.high[period - 1] < TDA.HighAvg[period - 1] and source.high[period] > TDA.HighAvg[period] then
    ...
    if source.low[period - 1] > TDA.LowAvg[period - 1] and source.low[period] < TDA.LowAvg[period] then
        if instance.parameters.TypeSignal == "direct" then
            MustOpenS = true
        else
            MustOpenB = true
    ...
    if MustOpenB and MustOpenS then
        MustOpenB = false
        MustOpenS = false
    end
...
function CheckCloseTime(period)
    if instance.parameters.CloseAtEnd == true then
        ...
        local CurTime = table.hour * 3600 + table.min * 60
        d, t = Source:date(period - 1)
        local table = core.dateToTable(d)
        local PrevTime = table.hour * 3600 + table.min * 60
        if PrevTime < BeginT and CurTime >= BeginT then
            return true
        end
    end
    return false
