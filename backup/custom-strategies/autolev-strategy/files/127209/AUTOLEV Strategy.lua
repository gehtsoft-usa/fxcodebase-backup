...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=68339
...
local STRATEGY_NAME = "AUTOLEV Strategy";
...
function CreateParameters() 
    ...
    strategy.parameters:addString("autolev_tf", "AUTOLEV Timeframe", "", "H4");
    strategy.parameters:setFlag("autolev_tf", core.FLAG_BARPERIODS);
    strategy.parameters:addInteger("N", "Number of periods to find H/L", "", 50)
    strategy.parameters:addBoolean("Flip", "Use L-H instead H-L", "", false)
    strategy.parameters:addDouble("L1", "1. Level", "", -0.236)
    AddAction(1, "Level 1 Cross Over Action");
    AddAction(2, "Level 1 Cross Under Action");
    strategy.parameters:addDouble("L2", "2. Level", "", 0)
    AddAction(3, "Level 2 Cross Over Action");
    AddAction(4, "Level 2 Cross Under Action");
    strategy.parameters:addDouble("L3", "3. Level", "", 0.236)
    AddAction(5, "Level 3 Cross Over Action");
    AddAction(6, "Level 3 Cross Under Action");
    strategy.parameters:addDouble("L4", "4. Level", "", 0.382)
    AddAction(7, "Level 4 Cross Over Action");
    AddAction(8, "Level 4 Cross Under Action");
    strategy.parameters:addDouble("L5", "5. Level", "", 0.5)
    AddAction(9, "Level 5 Cross Over Action");
    AddAction(10, "Level 5 Cross Under Action");
    strategy.parameters:addDouble("L6", "6. Level", "", 0.618)
    AddAction(11, "Level 6 Cross Over Action");
    AddAction(12, "Level 6 Cross Under Action");
    strategy.parameters:addDouble("L7", "7. Level", "", 0.764)
    AddAction(13, "Level 7 Cross Over Action");
    AddAction(14, "Level 7 Cross Under Action");
    strategy.parameters:addDouble("L8", "8. Level", "", 1)
    AddAction(15, "Level 8 Cross Over Action");
    AddAction(16, "Level 8 Cross Under Action");
    strategy.parameters:addDouble("L9", "9. Level", "", 1.272)
    AddAction(17, "Level 9 Cross Over Action");
    AddAction(18, "Level 9 Cross Under Action");
end

local autolev_source;
local N, Flip;
function CreateIndicators(source) 
    N = instance.parameters.N;
    Flip = instance.parameters.Flip;
    autolev_source = trading_logic:SubscribeHistory(source:instrument(), instance.parameters.autolev_tf, source:isBid())
end

function UpdateIndicators()
end

function GetLevelPrice(source, period, level)
    min, max = mathex.minmax(source, math.max(0, period - N + 1), period)
    d = max - min
    if Flip then
        return max - d * level
    end
...
function CreateCustomActions()
    for i = 1, 18 do
        local action1, isEntry1 = CreateAction(i);
        action1.Data = instance.parameters:getDouble("L" .. math.floor(i / 2 + 0.5));
        if i % 2 == 1 then
            action1.IsPass = function (source, period, periodFromLast, data) 
                if period < N + 1 or autolev_source:size() - 1 - periodFromLast < 0 then
                    return;
                end
                return core.crossesOver(source.close, GetLevelPrice(autolev_source, autolev_source:size() - 1 - periodFromLast, data), period); 
            end
        else
            action1.IsPass = function (source, period, periodFromLast, data) 
                if period < N + 1 or autolev_source:size() - 1 - periodFromLast < 0 then
                ...
                return core.crossesUnder(source.close, GetLevelPrice(autolev_source, autolev_source:size() - 1 - periodFromLast, data), period); 
            end
        end
        if isEntry1 then
