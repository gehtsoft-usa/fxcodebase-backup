...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=68421
...
function Init()
    ...
    strategy.parameters:addInteger("Limit", "Limit shift, pips", "", 1, 1, 10000)
    strategy.parameters:addBoolean("SetStop", "Set Stop Order", "", true)
    strategy.parameters:addInteger("Stop", "Stop shift, pips", "", 1, 1, 10000)
...
function Prepare(nameOnly)
    ...
    I =
        ...
        "BREAKOUT",
        gSource,
        instance.parameters.PeriodBegin,
        instance.parameters.PeriodEnd,
        instance.parameters.BoxEnd,
        instance.parameters.Type2,
        false
    )
...
function enter(BuySell, distance)
    ...
    if CanClose and SetLimit then
        -- set limit order
        if BuySell == "B" then
            valuemap.RateLimit = instance.ask[NOW] + distance + Limit
        else
            valuemap.RateLimit = instance.bid[NOW] - distance - Limit
        end
    end
    ...
    if CanClose and SetStop then
        -- set limit order
        if BuySell == "B" then
            valuemap.RateStop = instance.ask[NOW] - distance - Stop
        else
            valuemap.RateStop = instance.bid[NOW] + distance - Stop
        end
    ...
    if not (CanClose) and SetLimit then
        if BuySell == "B" then
            fifoSL("LE", "S", instance.ask[NOW] + distance + Limit, false)
        else
            fifoSL("LE", "B", instance.ask[NOW] - distance - Limit, false)
        end
    end

    if not (CanClose) and SetStop then
        if BuySell == "B" then
            fifoSL("SE", "S", instance.ask[NOW] - distance - Stop, TrailingStop)
        else
            fifoSL("SE", "B", instance.ask[NOW] + distance + Stop, TrailingStop)
        end
    end
