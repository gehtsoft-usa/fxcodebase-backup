...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=63430
...
function Init() --The strategy profile initialization
    strategy:name("Highly adaptable Bollinger Bands Convergence Divergence Strategy")
    ...
    strategy.parameters:addString("Action" .. 1, "Top Line Up Slope", "", "NO")
    ...
    strategy.parameters:addString("Action" .. 2, "Top Line Down Slope", "", "NO")
    ...
    strategy.parameters:addString("Action" .. 3, "Central Line Up Slope", "", "NO")
    ...
    strategy.parameters:addString("Action" .. 4, "Central Line Down Slope", "", "NO")
    ...
    strategy.parameters:addString("Action" .. 5, "Bottom Line Up Slope", "", "NO")
    ...
    strategy.parameters:addString("Action" .. 6, "Bottom Line Down Slope", "", "NO")
    ...
    strategy.parameters:addString("Action" .. 7, "Convergence", "", "NO")
    ...
    strategy.parameters:addString("Action" .. 8, "Divergence", "", "NO")
...
function Prepare(nameOnly)
    ...
    Indicator = core.indicators:create("BB", Source.close, N, Dev)
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if InRange(now, OpenTime, CloseTime) then
        ...
        if Indicator.TL[period] > Indicator.TL[period - 1] and Indicator.TL[period - 1] <= Indicator.TL[period - 2] then
            ACTION(1, 1, "Over")
        end
        if Indicator.TL[period] < Indicator.TL[period - 1] and Indicator.TL[period - 1] >= Indicator.TL[period - 2] then
            ACTION(2, 1, "Under")
        ...
        if Indicator.AL[period] > Indicator.AL[period - 1] and Indicator.AL[period - 1] <= Indicator.AL[period - 2] then
            ACTION(3, 2, "Over")
        end
        if Indicator.AL[period] < Indicator.AL[period - 1] and Indicator.AL[period - 1] >= Indicator.AL[period - 2] then
            ACTION(4, 2, "Under")
        ...
        if Indicator.BL[period] > Indicator.BL[period - 1] and Indicator.BL[period - 1] <= Indicator.BL[period - 2] then
            ACTION(5, 3, "Over")
        end
        ...
        local D0 = Indicator.TL[period] - Indicator.AL[period]
        local D1 = Indicator.TL[period - 1] - Indicator.AL[period - 1]
        local D2 = Indicator.TL[period - 2] - Indicator.AL[period - 2]

        if D0 > D1 and D1 <= D2 then
            ACTION(7, 4, "Divergence")
        end

        if D0 < D1 and D1 >= D2 then
            ACTION(8, 4, "Convergence")
        end
    end
end
