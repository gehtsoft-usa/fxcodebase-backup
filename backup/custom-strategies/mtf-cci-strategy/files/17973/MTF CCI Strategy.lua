...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=8136
...
function Init() --The strategy profile initialization
    strategy:name("MTF CCI Strategy")
    ...
    strategy.parameters:addInteger("OpenLevel", "Open Level", "", 5, 3, 5)
    strategy.parameters:addInteger("CloseLevel", "Close Level", "", 3, 2, 5)
...
function Parameters(id, frame, EMA)
    ...
    strategy.parameters:addInteger("PERIOD" .. id, "CCI Period ", "", EMA)
end
...
function Prepare(nameOnly)
    ...
    for i = 1, 5, 1 do
        ...
        PERIOD[i] = instance.parameters:getString("PERIOD" .. i)

        assert(TF[i] ~= "t1", i .. ". The time frame must not be tick")
        name = name .. "(" .. TF[i] .. ", " .. PERIOD[i] .. ") "
    end
    ...
    for i = 1, 5, 1 do
        ...
        Indicator[i] = core.indicators:create("CCI", Source[i], PERIOD[i])
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if
        Short[1]:size() <= Short[1]:first() + 2 or Short[2]:size() <= Short[2]:first() + 2 or
            Short[3]:size() <= Short[3]:first() + 2 or
            Short[4]:size() <= Short[4]:first() + 2 or
            Short[5]:size() <= Short[5]:first() + 2
     then
        return
    end
    ...
    for i = 1, 5, 1 do
        if Short[i][Short[i]:size() - 1] > Short[i][Short[i]:size() - 2] then
            Buy = Buy + 1
        elseif Short[i][Short[i]:size() - 1] < Short[i][Short[i]:size() - 2] then
            Sell = Sell + 1
        end
    end

    if Buy >= OpenLevel then
        if Direction then
            BUY()
        else
            SELL()
        end
    elseif Sell >= OpenLevel then
        if Direction then
            SELL()
        else
            BUY()
