...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=18865
...
function Init() --The strategy profile initialization
    ...
    strategy:description("Price extreme strategy")
    strategy:setTag("Version", "2")
    strategy:setTag("NonOptimizableParameters", "Email,SendEmail,SoundFile,RecurrentSound,PlaySound, ShowAlert")

    strategy.parameters:addGroup("Parameters")
    strategy.parameters:addInteger("Multiplier", "Multiplier", "", 5)
    strategy.parameters:addString("CrossType", "CrossType", "", "Close")
...
function Prepare(nameOnly)
    ...
    assert(
        core.indicators:findIndicator("PRICE_EXTREME_INDICATOR") ~= nil,
        "Please, download and install PRICE_EXTREME_INDICATOR.LUA indicator"
    )
    Source = ExtSubscribe(2, nil, instance.parameters.TF, true, "bar")
    PEI = core.indicators:create("PRICE_EXTREME_INDICATOR", Source, instance.parameters.Multiplier)
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    PEI:update(core.UpdateLast)

    -- Check that we have enough data
    if (PEI.DATA:first() > (period - 1)) then
        return
    end
    ...
    if
        (source.close[period] > PEI.HighBorder[period] and CrossType == "Close") or
            (source.high[period] > PEI.HighBorder[period] and CrossType == "High/Low")
     then
    ...
    if
        (source.close[period] < PEI.LowBorder[period] and CrossType == "Close") or
            (source.low[period] < PEI.LowBorder[period] and CrossType == "High/Low")
     then
        if instance.parameters.TypeSignal == "direct" then
            MustOpenS = true
        else
