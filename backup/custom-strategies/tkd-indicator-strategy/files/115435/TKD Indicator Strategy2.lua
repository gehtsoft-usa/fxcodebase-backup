...
function Init() --The strategy profile initialization
    strategy:name("TKD Indicator Strategy");
    ...
    strategy.parameters:addInteger("N", "Periods for %K", "", 30, 2, 1000);
    strategy.parameters:addInteger("M", "Periods for Smooth", "", 3);
    strategy.parameters:addInteger("D", "Distance percentage", "", 100);
    strategy.parameters:addInteger("C", "Coefficient percentage", "", 100);
	
	 strategy.parameters:addInteger("Filter", "Signal Type", "", 1);
    strategy.parameters:addIntegerAlternative("Filter", "T/F Cross", "", 1);
    strategy.parameters:addIntegerAlternative("Filter", "T/S Cross", "", 2);

    strategy.parameters:addBoolean("TFExit", "T/F cross exit", "", true);
	
	strategy.parameters:addGroup("RSI Calculation");
	 
	strategy.parameters:addInteger("RSI_Period", "Periods for RSI", "", 14, 2, 1000); 
	strategy.parameters:addInteger("Period", "Periods for MA", "", 28, 2, 1000);
...
function CreateTradingParameters()
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "TKDIS");
...
local N,M, D,C;
local Filter;
local RSI_Period, Method, Period;
local TFExit;
...
function Prepare( nameOnly)
    ...
    TFExit= instance.parameters.TFExit;
	...
	assert(core.indicators:findIndicator("TKD") ~= nil, "Please, download and install TKD.LUA indicator");          
    ...
    TKD = core.indicators:create("TKD", Source,  N,M, D,C  );
	RSI  = core.indicators:create("RSI", Source.close,  RSI_Period  );
    assert(core.indicators:findIndicator(Method) ~= nil, Method .. " indicator must be installed");
	MA  = core.indicators:create(Method, RSI.DATA,  Period  );
	first=math.max(TKD.S:first(), MA.DATA:first()); 
...
function ExtUpdate(id, source, period)  -- The method called every time when a new bid or ask price appears.
   ...
   -- get only time
	...
	if TFExit then
		if TKD.T[period]>TKD.F[period] and TKD.T[period-1]<=TKD.F[period-1] then
			if Direction then
				exitSpecific("S");
			else
				exitSpecific("B");
			end
		elseif TKD.T[period]<TKD.F[period] and TKD.T[period-1]>=TKD.F[period-1] then
			if Direction then
				exitSpecific("B");
			else
				exitSpecific("S");
    ...
    if   (TKD.T[period]> TKD.F[period]
	and TKD.T[period-1]<= TKD.F[period-1] and Filter== 1)
	or
	 (TKD.T[period]> TKD.S[period]
	and TKD.T[period-1]<= TKD.S[period-1] and Filter== 2 )
    ...
    elseif   (TKD.T[period]< TKD.F[period]
	and TKD.T[period-1]>= TKD.F[period-1] and Filter== 1)
	or
	 (TKD.T[period]< TKD.S[period]
	and TKD.T[period-1]>= TKD.S[period-1] and Filter== 2 )
	...
	if Exit then
		            if     RSI.DATA [period]< MA.DATA[period]
					then 			
								if   Direction then
