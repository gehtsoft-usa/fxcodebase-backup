...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=63370
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if
        (TKD.T[period] > TKD.F[period] and TKD.T[period - 1] <= TKD.F[period - 1] and Filter == 1) or
            (TKD.T[period] > TKD.S[period] and TKD.T[period - 1] <= TKD.S[period - 1] and Filter == 2)
     then
        if Direction then
            BUY()
        else
    ...
    if Exit then
        if RSI.DATA[period] < MA.DATA[period] then
            if Direction then
