...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=68999
...
function Prepare(nameOnly)
    ...
    Indicator = CreateAverages(instance.parameters.ma_method, Source, instance.parameters.ma_period);
...
function ExtUpdate(id, source, period)  -- The method called every time when a new bid or ask price appears.
    ...
    if not core.crosses(Source.close, Indicator.DATA, period) then
        return;
    end
    if all_trades then
        trading:FindTrade()
