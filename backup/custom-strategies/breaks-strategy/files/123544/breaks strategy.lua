...
local STRATEGY_NAME = "Breaks strategy";
...
function CreateStopParameters(params, id)
    strategy.parameters:addBoolean("auto_stop" .. id, "Automatic stop", "", false);
    return false;
end
...
function CreateIndicators(source) 
    ma = core.indicators:create("MVA", source, instance.parameters.ma_period);
end
...
function SetCustomStop(position_desc, command, period)
    if instance.parameters:getBoolean("auto_stop" .. position_desc.Id) then
        if position_desc.Side == "B" then
            local lastSignalPeriod = GetUpPeriod(position_desc.Source, period);
            local stop_value = position_desc.Source.low[lastSignalPeriod];
            command:SetStop(stop_value, nil);
        else
            local lastSignalPeriod = GetDownPeriod(position_desc.Source, period);
            local stop_value = position_desc.Source.high[lastSignalPeriod];
            command:SetStop(stop_value, nil);
        end
        return true;
    end
