...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=62320&p
...
function Init() --The strategy profile initialization
    strategy:name("Two Time Frame Super Trend Strategy")
    ...
    strategy.parameters:addGroup("1. Time Fame Calculation")
    strategy.parameters:addString("TF1", "Time frame", "", "m5")
    strategy.parameters:setFlag("TF1", core.FLAG_PERIODS)

    strategy.parameters:addInteger("N1", "Number of periods", "", 10)
    strategy.parameters:addDouble("M1", "Multiplier", "", 1.5)

    strategy.parameters:addGroup("2. Time Fame Calculation")
    strategy.parameters:addString("TF2", "Time frame", "", "m15")
    strategy.parameters:setFlag("TF2", core.FLAG_PERIODS)
    strategy.parameters:addInteger("N2", "Number of periods", "", 10)
    strategy.parameters:addDouble("M2", "Multiplier", "", 1.5)
...
function Prepare(nameOnly)
    ...
    Source1 = ExtSubscribe(1, nil, TF1, instance.parameters.Type == "Bid", "bar")
    ST1 = core.indicators:create("ST", Source1, N1, M1, core.rgb(0, 255, 0), core.rgb(255, 0, 0))

    Source2 = ExtSubscribe(2, nil, TF2, instance.parameters.Type == "Bid", "bar")
    ST2 = core.indicators:create("ST", Source2, N2, M2, core.rgb(0, 255, 0), core.rgb(255, 0, 0))
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    -- update indicators.
    ST1:update(core.UpdateLast)
    ST2:update(core.UpdateLast)

    if ST1.DATA:size() <= ST1.DATA:first() + 2 or ST2.DATA:size() <= ST2.DATA:first() + 2 then
        return
    end

    local Period = core.findDate(Source2.close, Source1:date(period), false)

    -- only buy if we have a fast cross over slow and the price is above the moving averages.
    if
        ST1.DATA:colorI(period) == core.rgb(0, 255, 0) and ST1.DATA:colorI(period - 1) ~= core.rgb(0, 255, 0) and
            ST2.DATA:colorI(Period) == core.rgb(0, 255, 0) and
            ST2.DATA:colorI(Period - 1) ~= core.rgb(0, 255, 0)
     then
        if Direction then
            BUY()
        else
    ...
    elseif
        ST1.DATA:colorI(period) == core.rgb(255, 0, 0) and ST1.DATA:colorI(period - 1) ~= core.rgb(255, 0, 0) and
            ST2.DATA:colorI(Period) == core.rgb(255, 0, 0) and
            ST2.DATA:colorI(Period - 1) ~= core.rgb(255, 0, 0)
     then
        if Direction then
            SELL()
        else
    ...
    if Exit then
        if ST1.DATA:colorI(period) ~= core.rgb(0, 255, 0) or ST2.DATA:colorI(Period) ~= core.rgb(0, 255, 0) then
            if Direction then
        ...
        if ST1.DATA:colorI(period) ~= core.rgb(255, 0, 0) or ST2.DATA:colorI(Period) ~= core.rgb(255, 0, 0) then
            if Direction then
                if haveTrades("S") then
                    exitSpecific("S")
                    Signal("Close Short")
