-- Available @ https://fxcodebase.com/code/viewtopic.php?f=31&t=74033
...
local STRATEGY_NAME = "Cleanow Strategy";
function CreateParameters() 
    strategy.parameters:addInteger("ma_period", "MA Period", "", 7);
    AddAverages("ma_method", "MA Method", "MVA");
end

local ma_low, ma_high, ma_period;
function CreateEntryIndicators(source)
    ma_period = instance.parameters.ma_period;
    ma_low = CreateAverages(instance.parameters.ma_period, instance.parameters.ma_method, source.low);
    ma_high = CreateAverages(instance.parameters.ma_period, instance.parameters.ma_method, source.high);
end

function UpdateIndicators()
    ma_low:update(core.UpdateLast);
    ma_high:update(core.UpdateLast);
end
...
function IsEntryLong(source, period)
    if not ma_low.DATA:hasData(period - 1) or not ma_high.DATA:hasData(period - 1) then
        return false;
    end
    return mathex.max(source.high, core.rangeTo(period, ma_period)) == source.high[period] 
        and ma_low.DATA[period] > ma_high.DATA[period];
end
function IsEntryShort(source, period)
    if not ma_low.DATA:hasData(period - 1) or not ma_high.DATA:hasData(period - 1) then
        return false;
    end
    return mathex.min(source.low, core.rangeTo(period, ma_period)) == source.low[period] 
        and ma_low.DATA[period] < ma_high.DATA[period];
end
