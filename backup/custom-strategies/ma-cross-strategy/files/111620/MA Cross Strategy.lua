...
function CreateTradingParameters()
    ...
    strategy.parameters:addString("SetLimit", "Limit Order Type", "", "auto")
    strategy.parameters:addStringAlternative("SetLimit", "Do not set", "", "no")
    strategy.parameters:addStringAlternative("SetLimit", "Auto", "", "auto")
    strategy.parameters:addStringAlternative("SetLimit", "In pips", "", "pips")
    strategy.parameters:addInteger("Limit", "Limit Order in pips", "", 30);
    strategy.parameters:addString("SetStop", "Stop Order Type", "", "auto")
    strategy.parameters:addStringAlternative("SetStop", "Do not set", "", "no")
    strategy.parameters:addStringAlternative("SetStop", "Auto", "", "auto")
    strategy.parameters:addStringAlternative("SetStop", "In pips", "", "pips")
...
function Prepare(nameOnly)
    ...
    Ask = ExtSubscribe(3, nil, TF, false, "bar")
    Bid = ExtSubscribe(4, nil, TF, true, "bar")
...
function EntryFunction(now, period)
    ...
    if Source.close[period] > MA.DATA[period] and Source.close[period - 1] <= MA.DATA[period - 1] then
        BUY(Bid.low[period])

        LastEntry = Source:serial(period)
        Return = true
    elseif Source.close[period] < MA.DATA[period] and Source.close[period - 1] >= MA.DATA[period - 1] then
        SELL(Ask.high[period])
...
function enter(BuySell, hCount, Stop)
    ...
    return MarketOrder(BuySell, hCount, Stop)
end

-- enter into the specified direction
function MarketOrder(BuySell, hCount)
    local offer = core.host:findTable("offers"):find("Instrument", instance.bid:instrument())
    ...
    if SetStop == "auto" then
        if BuySell == "B" then
            valuemap.Rate = offer.Bid - (Bid.close[NOW] - Bid.low[NOW])

            if math.abs(Bid.close[NOW] - valuemap.Rate) < offer.PointSize then
                valuemap.Rate = offer.Bid - offer.PointSize
            end
        else
            valuemap.Rate = offer.Ask + (Ask.high[NOW] - Ask.close[NOW])

            if math.abs(Ask.close[NOW] - valuemap.Rate) < offer.PointSize then
                valuemap.Rate = offer.Ask + offer.PointSize
            end
        end
    elseif SetStop == "pips" then
    ...
    if SetLimit == "auto" then
        if BuySell == "B" then
            valuemap.PegPriceOffsetPipsLimit = ((Bid.close[NOW] - Bid.low[NOW]) / Bid:pipSize())
        else
            valuemap.PegPriceOffsetPipsLimit = -((Ask.high[NOW] - Ask.close[NOW]) / Ask:pipSize())
        end
    elseif SetLimit == "pips" then
        if BuySell == "B" then
