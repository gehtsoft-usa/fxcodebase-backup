...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=2793
...
function Init()
    strategy:name("Set stop by candle shadows")
    strategy:description("Strategy changes stop followiung to the candle shadows")
    ...
    strategy.parameters:addInteger("BarsNumber", "Number of bars", "", 3, 0, 1000)
    strategy.parameters:addInteger("Indent", "Indent from shadow", "", 0, 0, 1000)
            ...
            instance.bid:instrument() ..
                "[" ..
                    instance.parameters.Period ..
                        "]" ..
                            "," ..
                                instance.parameters.BarsNumber ..
...
function ExtUpdate(id, source, period)
    if id == 1 and tradeExist then
        if period > tsource:first() + instance.parameters.BarsNumber then
            ...
            if trade.BS == "B" then
                stopValue =
                    core.max(tsource.low, core.rangeTo(period, instance.parameters.BarsNumber)) -
                ...
                stopValue =
                    core.min(tsource.high, core.rangeTo(period, instance.parameters.BarsNumber)) +
                    instance.parameters.Indent * instance.bid:pipSize()
                if instance.parameters.MoveBack == false then
                    if trade.StopOrderID ~= "" and trade.StopOrderID ~= nil then
                        if stopValue > trade.Stop then
