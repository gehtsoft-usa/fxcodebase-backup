...
local STRATEGY_NAME = "Alexander Elder Triple Screen Trading System";
...
function CreateParameters() 
    ...
    strategy.parameters:addString("timeframe", "Short Time frame", "", "m5");
    ...
    strategy.parameters:addString("long_timeframe", "Long Time frame", "", "m15");
    strategy.parameters:setFlag("long_timeframe", core.FLAG_PERIODS);
    strategy.parameters:addGroup("MACD Calculation")
    strategy.parameters:addInteger("fast", "MACD Fast Period", "", 12);
    strategy.parameters:addInteger("slow", "MACD Slow Period", "", 26);
    strategy.parameters:addInteger("signal_smoothing", "MACD Signal Smoothing", "", 9);
end
...
function CreateIndicators(source) 
    ...
    local short_source = trading_logic:SubscribeHistory(source:instrument(), instance.parameters.timeframe, instance.parameters.is_bid);
    fi = core.indicators:create("AEFI", short_source, indicatorParams)

    local long_source = trading_logic:SubscribeHistory(source:instrument(), instance.parameters.long_timeframe, instance.parameters.is_bid);
    macd = core.indicators:create("MACD", long_source, instance.parameters.fast, instance.parameters.slow, instance.parameters.signal_smoothing);
end
...
function CreateCustomActions()
    ...
    exitLongAction.IsPass = function (source, period, periodFromLast, data)
        if macd.HISTOGRAM:size() < 2 or not macd.HISTOGRAM:hasData(NOW - periodFromLast) then
            return nil;
        end
        return macd.HISTOGRAM[NOW - periodFromLast] < macd.HISTOGRAM[NOW - periodFromLast - 1]
            or fi.DATA[NOW - periodFromLast] < 0;
    end
    ...
    enterLongAction.IsPass = function (source, period, periodFromLast, data)
        ...
        return macd.HISTOGRAM[NOW - periodFromLast] > macd.HISTOGRAM[NOW - periodFromLast - 1] 
            and core.crossesOver(fi.DATA, 0, NOW - periodFromLast);
    end
    ...
    enterShortAction.IsPass = function (source, period, periodFromLast, data)
        ...
        return macd.HISTOGRAM[NOW - periodFromLast] < macd.HISTOGRAM[NOW - periodFromLast - 1] 
            and core.crossesUnder(fi.DATA, 0, NOW - periodFromLast);
    end
...
function trading:AddPositionParameters(parameters, id)
    if self.AddAmountParameter then
        ...
        parameters:addString("amount_type" .. id, "Amount Type", "", "equity");
