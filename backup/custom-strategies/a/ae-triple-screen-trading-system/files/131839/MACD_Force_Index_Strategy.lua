...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=68549
--+------------------------------------------------------------------+
...
local STRATEGY_NAME = "MACD Force Index Strategy";
function CreateParameters() 
    ...
    strategy.parameters:addBoolean("force_index_exit", "Force index exit", "", true);

    strategy.parameters:addGroup("MACD Calculation")
    strategy.parameters:addString("macd_tf", "MACD Timeframe", "", "H1");
    strategy.parameters:setFlag("macd_tf", core.FLAG_PERIODS);
    strategy.parameters:addInteger("SN1", "Short EMA", "", 12, 2, 1000)
    strategy.parameters:addInteger("LN1", "Long EMA", "", 26, 2, 1000)
end

local macd, macd_source, fi;
function CreateEntryIndicators(source)
    macd_source = ExtSubscribe(3, source:instrument(), instance.parameters.macd_tf, source:isBid(), "bar");
    macd = core.indicators:create("MACD", macd_source, instance.parameters.SN1, instance.parameters.LN1);
    
    local profile = core.indicators:findIndicator("AEFI");
    assert(profile ~= nil, "Please, download and install " .. "AEFI" .. ".LUA indicator");
    local indicatorParams = profile:parameters();
    indicatorParams:setBoolean("SM", instance.parameters.SM);
    indicatorParams:setInteger("N", instance.parameters.N);
    fi = core.indicators:create("AEFI", source, indicatorParams)
end

function UpdateIndicators()
    macd:update(core.UpdateLast);
    fi:update(core.UpdateLast);
end

function IsEntryLong(source, period)
    if macd.DATA:size() < 2 then
        return;
    end
    return macd.HISTOGRAM[NOW] > macd.HISTOGRAM[NOW - 1]
        and fi.DATA[period] < 0;
end
...
function IsExitLong(source, period)
    ...
    if instance.parameters.force_index_exit and core.crossesUnder(fi.DATA, 0, period) then
        return true;
    end
    return macd.HISTOGRAM[NOW] < macd.HISTOGRAM[NOW - 1];
end
function IsExitShort(source, period)
    ...
    if instance.parameters.force_index_exit and core.crossesOver(fi.DATA, 0, period) then
        return true;
    end
    return macd.HISTOGRAM[NOW] > macd.HISTOGRAM[NOW - 1];
end
...
function ExtUpdate(id, source, period)
    ...
    local row = enum:next();
    while row ~= nil do
        if row.Instrument == main_source:instrument() and not trading:IsStopOrder(row) and not trading:IsLimitOrder(row) then
            trading:DeleteOrder(row);
        end
        row = enum:next();
    end
    ...
    local row = enum:next();
    while row ~= nil do
        if row.Instrument == main_source:instrument() and moved[row.TradeID] == nil and trading:FindStopOrder(row) == nil then
            if row.BS == "B" then
                moved[row.TradeID] = trading:MoveStop(row, math.min(main_source.low[NOW - 1], main_source.low[NOW - 2]));
            else
                moved[row.TradeID] = trading:MoveStop(row, math.max(main_source.high[NOW - 1], main_source.high[NOW - 2]));
            end
        end
        row = enum:next();
    end
...
function OpenTrade(side)
    ...
    local command = trading:EntryOrder(main_source:instrument())
        :SetSide(side)
        :SetAccountID(Account)
        :SetAmount(Amount)
        :SetCustomID(custom_id)
    if side == "B" then
        command:SetRate(main_source.high[NOW - 1]);
    else
        command:SetRate(main_source.low[NOW - 1]);
    end
    if use_limit then
        command:SetpipLimit(nil, limit_pips)
    end
    command:Execute();
end
