...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=23862
...
function Init() --The strategy profile initialization
    strategy:name("ADX DMI Strategy")
    ...
    strategy.parameters:addInteger("Period1", "DMI Period", "Period", 14, 2, 1000)
    strategy.parameters:addInteger("Period2", "ADX Period", "Period", 14, 2, 1000)
    strategy.parameters:addDouble("Level", "ADX Level", "Level", 25, 0, 100)

    strategy.parameters:addGroup("Exit Condition")
    strategy.parameters:addInteger("Exit", "Exit Type", "", 3)
    strategy.parameters:addIntegerAlternative("Exit", "Weak ADX", "", 1)
    strategy.parameters:addIntegerAlternative("Exit", "DI+/DI- Cross", "", 2)
    strategy.parameters:addIntegerAlternative("Exit", "DMI Cross or Weak ADX ", "", 3)
    strategy.parameters:addIntegerAlternative("Exit", "Don't Use Exit", "", 3)
...
function Prepare(nameOnly)
    ...
    Indicator[1] = core.indicators:create("DMI", Source, Period)
    Short[1] = Indicator[1]:getStream(0)
    Short[2] = Indicator[1]:getStream(1)

    Indicator[2] = core.indicators:create("ADX", Source, Period)
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if
        (core.crossesOver(Short[3], Level, period) and (Short[1][period] > Short[2][period])) or
            (core.crossesOver(Short[1], Short[2], period) and Short[3][period] > Level)
     then
        if Direction then
            BUY()
        else
    ...
    elseif
        (core.crossesOver(Short[3], Level, period) and (Short[1][period] < Short[2][period])) or
            (core.crossesUnder(Short[1], Short[2], period) and Short[3][period] > Level)
     then
        if Direction then
            SELL()
        else
    ...
    if Exit ~= 2 and Exit ~= 4 then
        if Short[3][period] < Level then
            if Direction then
    ...
    elseif core.crossesOver(Short[1], Short[2], period) then
        if Direction then
            if haveTrades("S") then
                exit("S")
                Signal("Close Short")
