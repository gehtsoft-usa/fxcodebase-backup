...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=15349
...
function Init() --The strategy profile initialization
    strategy:name("Aroon_Strategy_with SAR_ Avereges_Filter")
    ...
    strategy.parameters:addInteger("AP", "Period", "", 25)
    strategy.parameters:addDouble("AL", "Aroon Signal Level", "", 75)
    ...
    strategy.parameters:addStringAlternative("ExitType", "Aroon", "", "Aroon")
    strategy.parameters:addStringAlternative("ExitType", "Averages", "", "Averages")
    strategy.parameters:addStringAlternative("ExitType", "SAR", "", "SAR")
...
function Prepare(nameOnly)
    ...
    name = name .. ", " .. Step .. ", " .. Max .. ", " .. Period .. ", " .. Method .. ", " .. AP
    ...
    Short[3] = Indicator[1]:getStream(0)

    Indicator[2] = core.indicators:create("SAR", Source, Step, Max)
    Short[1] = Indicator[2]:getStream(0)
    Short[2] = Indicator[2]:getStream(1)

    Indicator[3] = core.indicators:create("AROON", Source, AP)

    first = math.max(Short[3]:first(), Short[1]:first(), Short[2]:first(), Indicator[3].DATA:first())
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if
        Indicator[1].DATA:colorI(period) == core.rgb(0, 255, 0) and Short[2]:hasData(period) and
            core.crossesOver(Indicator[3].UP, AL, period)
     then
        if Direction then
            BUY()
        else
    ...
    elseif
        Indicator[1].DATA:colorI(period) == core.rgb(255, 0, 0) and Short[1]:hasData(period) and
            core.crossesOver(Indicator[3].DOWN, AL, period)
     then
        if Direction then
            SELL()
        else
    ...
    if Exit then
        if
            (Indicator[1].DATA:colorI(period) == core.rgb(255, 0, 0) and (ExitType == "Avegares" or ExitType == "Any")) or
                (Short[1]:hasData(period) and (ExitType == "SAR" or ExitType == "Any")) or
                (Indicator[3].UP[period] < AL and (ExitType == "Aroon" or ExitType == "Any"))
         then
            if Direction then
        ...
        if
            (Indicator[1].DATA:colorI(period) == core.rgb(0, 255, 0) and (ExitType == "Avegares" or ExitType == "Any")) or
                (Short[2]:hasData(period) and (ExitType == "SAR" or ExitType == "Any")) or
                (Indicator[3].DOWN[period] < AL and (ExitType == "Aroon" or ExitType == "Any"))
         then
            if Direction then
                if haveTrades("S") then
                    exit("S")
