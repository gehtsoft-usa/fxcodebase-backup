...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=68704
...
local STRATEGY_NAME = "Advanced Support Resistance Strategy";
...
function CreateParameters() 
    ...
    strategy.parameters:addString("indi_timeframe", "Timeframe", "", "m5");
    strategy.parameters:setFlag("indi_timeframe", core.FLAG_PERIODS);
    strategy.parameters:addInteger("Qualified", "Min Number to Qualify", "Min Number to Qualify", 1);
	strategy.parameters:addInteger("Tolerance", "Tolerance", "Tolerance", 0);
	strategy.parameters:addInteger("MinLength", "Min Length", "Min Length", 10);
	strategy.parameters:addInteger("MaxLength", "Max Length", "Max Length", 100);
    strategy.parameters:addInteger("Zone", "Zone width (in Pips)", "Zone width (in Pips)", 10);
    strategy.parameters:addInteger("Fractal", "Fractal", "", 2, 1 , 100);

	strategy.parameters:addString("Break", "Break On ", "Body ", "Body");	
    strategy.parameters:addStringAlternative("Break", "Body", "", "Body");
    strategy.parameters:addStringAlternative("Break", "Wick", "", "Wick");
end

local indi;
function CreateIndicators(source)
    local indi_source = trading_logic:SubscribeHistory(source:instrument(), instance.parameters.indi_timeframe, source:isBid());
    local profile = core.indicators:findIndicator("ADVANCED SUPPORT RESISTANCE");
    assert(profile ~= nil, "Please, download and install " .. "ADVANCED SUPPORT RESISTANCE" .. ".LUA indicator");
    local indicatorParams = profile:parameters();
    indicatorParams:setInteger("Qualified", instance.parameters.Qualified);
    indicatorParams:setInteger("Tolerance", instance.parameters.Tolerance);
    indicatorParams:setInteger("MinLength", instance.parameters.MinLength);
    indicatorParams:setInteger("MaxLength", instance.parameters.MaxLength);
    indicatorParams:setInteger("Zone", instance.parameters.Zone);
    indicatorParams:setInteger("Fractal", instance.parameters.Fractal);
    indicatorParams:setString("Find", "Both");
    indicatorParams:setString("Break", instance.parameters.Break);
    indicatorParams:setBoolean("strategy_mode", true);
    indicatorParams:setBoolean("ShowAlert", false);
    indicatorParams:setBoolean("PlaySound", false);
    indi = core.indicators:create("ADVANCED SUPPORT RESISTANCE", indi_source, indicatorParams)
end
...
function CreateCustomActions()
    ...
    enterLongAction.IsPass = function (source, period, periodFromLast, data)
        if indi.Down:size() < 2 or not indi.Down:hasData(period - 1) then
            return nil;
        end
        local index = indi.Down:size() - 1;
        return core.crossesUnder(source.close, indi.Down, period, index);
    end
    ...
    enterShortAction.IsPass = function (source, period, periodFromLast, data)
        if indi.Up:size() < 2 or not indi.Up:hasData(period - 1) then
            return nil;
        end
        local index = indi.Up:size() - 1;
        return core.crossesOver(source.close, indi.Up, period, index);
    end
