...
-- https://fxcodebase.com/code/viewtopic.php?f=31&t=72420
...
function Init() --The strategy profile initialization
    strategy:name("Adaptive Stochastic Strategy");
    ...
    strategy.parameters:addDouble("CycPart", "Cycle Part", "", 1, 0.1, 2000);
    strategy.parameters:addInteger("min", "Minimum", "", 6, 1, 2000);	
    strategy.parameters:addInteger("max", "Maximum", "", 50, 3, 2000);		
	
  	strategy.parameters:addGroup("Signal Calculation");	
    strategy.parameters:addInteger("Period", "Period", "", 7, 1, 2000);  
    ...
    strategy.parameters:addStringAlternative("Method1", "WMA", "WMA" , "WMA");
  	...
  	strategy.parameters:addGroup("Trend Calculation");	
    strategy.parameters:addInteger("TrendPeriod", "Period", "", 14, 1, 2000); 
...
function CreateTradingParameters()
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "ASS");
...
function Prepare( nameOnly)
    ...
    PrepareTrading();
	 ...
	 assert(core.indicators:findIndicator("ADAPTIVE STOCHASTIC") ~= nil, "Please, download and install ADAPTIVE STOCHASTIC.LUA indicator");          
    ...
    Trend = core.indicators:create(instance.parameters.Method2, Source.close,   instance.parameters.TrendPeriod );	
 
    Indicator = core.indicators:create("ADAPTIVE STOCHASTIC", Source , instance.parameters.CycPart, instance.parameters.min, instance.parameters.max );
    MA = core.indicators:create( instance.parameters.Method1	, Indicator.DATA,   instance.parameters.Period );	
 
	first=math.max(MA.DATA:first(), Trend.DATA:first()); 
...
--	period3= core.findDate (Source3, TickSource:date(period), false);
	...
	--/////////////////////////////////////////////////////////////////////
		            ...
		            if   MA.DATA[period] < 60
					or Source.close[period]<   Trend.DATA[period] 
					then
		            ...
		            if   Indicator.DATA[period] >   MA.DATA[period]
	                and MA.DATA[period] > 60 
					or Source.close[period] >    Trend.DATA[period] 					
					then
...
function EntryFunction( id, now,period)
	...
	--  Indicator.DATA[period]>  MA.DATA[period]
	--and  Indicator.DATA[period-1]<=  MA.DATA[period-1]
	and MA.DATA[period] > 50
	and Source.low[period]> Trend.DATA[period] 
	then	
	...
	--and  Indicator.DATA[period-1]>=  MA.DATA[period-1]
	and MA.DATA[period] < 50	
	and Source.high[period]< Trend.DATA[period] 	
	then
