...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=64685
...
function Init() --The strategy profile initialization
    strategy:name("Advanced Fractal Envelops Strategy Middle")
    ...
    strategy.parameters:addInteger("Period", "Fractal Envelop Period", "", 3, 1, 100)
...
local AccountType
local SourceBid, SourceAsk, MidleSource, TickSource
...
local Period, Frame
local IndicatorBid, IndicatorAsk
...
function Prepare(nameOnly)
    ...
    assert(
        core.indicators:findIndicator("ADVANCED FRACTAL ENVELOPS") ~= nil,
        "Please, download and install ADVANCED FRACTAL ENVELOPS.LUA indicator"
    )
    ...
    SourceBid = ExtSubscribe(2, nil, TF, true, "bar")
    SourceAsk = ExtSubscribe(3, nil, TF, false, "bar")
    IndicatorBid = core.indicators:create("ADVANCED FRACTAL ENVELOPS", SourceBid, Period, Frame)
    IndicatorAsk = core.indicators:create("ADVANCED FRACTAL ENVELOPS", SourceAsk, Period, Frame)

    first = math.max(IndicatorBid.TOP:first(), IndicatorBid.BOTTOM:first())
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    then
        ...
        periodBid = core.findDate(SourceBid, TickSource:date(period), false)
        periodAsk = core.findDate(SourceAsk, TickSource:date(period), false)
		
        if periodBid ~= periodAsk then
            return
        else
            period = periodBid
        end
		...
		periodBid = core.findDate(SourceBid, SourceAsk:date(period), false)
        periodAsk = period;
    ...
    -- update indicators.
    IndicatorBid:update(core.UpdateLast)
    IndicatorAsk:update(core.UpdateLast)

    if not SourceBid.close:hasData(period) or not SourceAsk.close:hasData(period) or period < first then
        return
    end
...
function ExitFunction(now, period)
    ...
    if (LastExit == SourceBid:serial(period)) then
        return
    end

    if
        CalcMidle(SourceBid.close, SourceAsk.close, period) < CalcMidle(IndicatorBid.TOP, IndicatorAsk.TOP, period) and
            CalcMidle(SourceBid.close, SourceAsk.close, period - 1) >=
                CalcMidle(IndicatorBid.TOP, IndicatorAsk.TOP, period - 1)
     then
        if Direction then
        ...
        LastExit = SourceBid:serial(period)
    end

    if
        CalcMidle(SourceBid.close, SourceAsk.close, period) >
            CalcMidle(IndicatorBid.BOTTOM, IndicatorAsk.BOTTOM, period) and
            CalcMidle(SourceBid.close, SourceAsk.close, period - 1) <=
                CalcMidle(IndicatorBid.BOTTOM, IndicatorAsk.BOTTOM, period - 1)
     then
        if Direction then
            if haveTrades("S") then
                exitSpecific("S")
...
function EntryFunction(now, period)
    ...
    if (LastEntry == SourceBid:serial(period)) then
        return
    end
    ...
    if
     ...
     then
        ...
        LastEntry = SourceBid:serial(period)
        Return = true
    elseif
        CalcMidle(SourceBid.close, SourceAsk.close, period) < CalcMidle(IndicatorBid.TOP, IndicatorAsk.TOP, period) and
            CalcMidle(SourceBid.close, SourceAsk.close, period - 1) >=
...
function CalcMidle(source1, source2, period)
    return (source1[period] + source2[period]) / 2
end
