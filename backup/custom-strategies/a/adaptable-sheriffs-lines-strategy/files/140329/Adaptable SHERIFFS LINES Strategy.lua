...
local STRATEGY_NAME = "Adaptable Sheriff's Lines Strategy";
function CreateParameters() 
    strategy.parameters:addInteger("ma_period", "MA Period", "", 14);
    AddAverages("ma_method", "MA Method", "MVA");
    strategy.parameters:addString("ATR_timeframe", "ATR Timeframe", "", "m5");
    strategy.parameters:setFlag("ATR_timeframe", core.FLAG_BARPERIODS);
    strategy.parameters:addInteger("ATR_Period", "ATR_Period", "", 9, 1, 2000);
	strategy.parameters:addInteger("MA_Period", "MA_Period", "", 9, 1, 2000);
	strategy.parameters:addBoolean("Previous", "Use Previous period ATR", "", true);
    strategy.parameters:addDouble("Factor1", "1. Factor", "", 5.0);
    strategy.parameters:addDouble("Factor2", "2. Factor", "", 5.0);
	strategy.parameters:addDouble("Factor3", "3. Factor", "", 5.0);
	strategy.parameters:addDouble("Factor4", "4. Factor", "", 5.0);
	strategy.parameters:addDouble("Factor5", "5. Factor", "", 5.0);
	strategy.parameters:addDouble("Factor6", "6. Factor", "", 5.0);
end
...
function CreateEntryIndicators(source)
    ...
    local profile = core.indicators:findIndicator("ADAPTABLE SHERIFFS LINES");
    assert(profile ~= nil, "Please, download and install " .. "ADAPTABLE SHERIFFS LINES" .. ".LUA indicator");
    local indicatorParams = profile:parameters();
    indicatorParams:setBoolean("Previous", instance.parameters.Previous);
    ...
    indicatorParams:setString("TF", instance.parameters.ATR_timeframe);
    indicatorParams:setDouble("Factor1", instance.parameters.Factor1);
    indicatorParams:setDouble("Factor2", instance.parameters.Factor2);
    indicatorParams:setDouble("Factor3", instance.parameters.Factor3);
    indicatorParams:setDouble("Factor4", instance.parameters.Factor4);
    indicatorParams:setDouble("Factor5", instance.parameters.Factor5);
    indicatorParams:setDouble("Factor6", instance.parameters.Factor6);
    indi = core.indicators:create("ADAPTABLE SHERIFFS LINES", source, indicatorParams)
end
...
function IsEntryLong(source, period)
    ...
    return core.crossesOver(ma.DATA, indi.Line1, period);
end
function IsEntryShort(source, period)
    if not indi.DATA:hasData(period - 1) then
        return false;
    end
    return core.crossesUnder(ma.DATA, indi.Line4, period);
end
function IsExitLong(source, period)
    return core.crossesUnder(ma.DATA, indi.Line3, period);
end
function IsExitShort(source, period)
    return core.crossesOver(ma.DATA, indi.Line2, period);
end
