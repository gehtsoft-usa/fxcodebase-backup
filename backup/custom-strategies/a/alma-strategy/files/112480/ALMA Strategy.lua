...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=64666
...
function Init() --The strategy profile initialization
    ...
    strategy.parameters:addGroup("ALMA Calculation")
    strategy.parameters:addString("Selector", "Indicator Selector", "", "ALMA")
    strategy.parameters:addStringAlternative("Selector", "ALMA", "", "ALMA")
    strategy.parameters:addStringAlternative("Selector", "ALMA2", "", "ALMA2")

    strategy.parameters:addString("SignalType", "SignalType Selector", "", "Slope")
    strategy.parameters:addStringAlternative("SignalType", "Indicator Slope", "", "Slope")
    strategy.parameters:addStringAlternative("SignalType", "Price/Indicator Cross", "", "Cross")

    strategy.parameters:addInteger("Period", "Period", "", 9)
    strategy.parameters:addDouble("Sigma", "Sigma", "", 6)
    strategy.parameters:addDouble("Offset", "Offset", "", 0.85)
    strategy.parameters:addDouble("DampingFactor", "Damping factor", "", 1)
    strategy.parameters:addDouble("PctFilter", "Pct filter", "", 0)
...
local Period, Sigma, Offset, DampingFactor, PctFilter
local Selector, Signal
...
function Prepare(nameOnly)
    ...
    assert(
        core.indicators:findIndicator(Selector) ~= nil,
        "Please, download and install " .. Selector .. ".LUA indicator"
    )
...
function EntryFunction(now, period)
    ...
    if
        (SignalType == "Cross" and Source.close[period] > Indicator.DATA[period] and
            Source.close[period - 1] <= Indicator.DATA[period - 1]) or
            (SignalType == "Slope" and Indicator.DATA:colorI(period) == core.rgb(0, 255, 0) and
                Indicator.DATA:colorI(period - 1) ~= core.rgb(0, 255, 0))
     then
        if Direction then
            BUY()
        else
    ...
    elseif
        (SignalType == "Cross" and Source.close[period] < Indicator.DATA[period] and
            Source.close[period - 1] >= Indicator.DATA[period - 1]) or
            (SignalType == "Slope" and Indicator.DATA:colorI(period) == core.rgb(255, 0, 0) and
