...
function Prepare(nameOnly)
    ...
    ASCTrend = core.indicators:create("ASCTREND2", Source, RISK, true)
    first = ASCTrend.buffDot:first()
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if ASCTrend.buffDot[period - 1] == Source.low[period - 1] then
        if Direction then
            BUY()
        else
            SELL()
    ...
    elseif ASCTrend.buffDot[period - 1] == Source.high[period - 1] then
        if Direction then
            SELL()
        else
            BUY()
