...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=3859
...
function Init() --The strategy profile initialization
    strategy:name("Averages Strategy")
    ...
    strategy.parameters:addString("Mode", "Strategy Mode", "", "SLOPE")
    strategy.parameters:addStringAlternative("Mode", "Slope", "", "SLOPE")
    strategy.parameters:addStringAlternative("Mode", "MA/Price Cross", "", "CROSS")
...
local first
local Exit
local PriceType, Mode, Type, PERIOD, Method, PRICE
local indicator
local Direction
local CustomID
...
function Prepare(nameOnly)
    ...
    name = name .. ", " .. "(" .. Mode .. ", " .. Type .. ", " .. Method .. ", " .. PERIOD .. ")"
    ...
    indicator = core.indicators:create("AVERAGES", PRICE, Method, PERIOD, true)
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if Mode == "SLOPE" then
        if not indicator.DATA:hasData(period - 2) then
            return
        end

        if
            indicator.DATA[period] > indicator.DATA[period - 1] and
                indicator.DATA[period - 2] > indicator.DATA[period - 1] or
                indicator.DATA[period] > indicator.DATA[period - 1] and
                    indicator.DATA[period - 2] == indicator.DATA[period - 1]
         then
