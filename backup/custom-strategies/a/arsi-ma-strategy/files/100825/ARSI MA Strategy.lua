...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=62298
...
function Init() --The strategy profile initialization
    strategy:name("ARSI MA Strategy")
    ...
    strategy.parameters:addGroup("ARSI Calculation")
    strategy.parameters:addString("ARSI_Price", "Price Source", "", "close")
    strategy.parameters:addStringAlternative("ARSI_Price", "OPEN", "", "open")
    strategy.parameters:addStringAlternative("ARSI_Price", "HIGH", "", "high")
    strategy.parameters:addStringAlternative("ARSI_Price", "LOW", "", "low")
    strategy.parameters:addStringAlternative("ARSI_Price", "CLOSE", "", "close")
    strategy.parameters:addStringAlternative("ARSI_Price", "MEDIAN", "", "median")
    strategy.parameters:addStringAlternative("ARSI_Price", "TYPICAL", "", "typical")
    strategy.parameters:addStringAlternative("ARSI_Price", "WEIGHTED", "", "weighted")

    strategy.parameters:addInteger("ARSI_Period", "ARSI Period", "", 14)

    strategy.parameters:addGroup("MA Calculation")

    strategy.parameters:addString("MA_Price", "Price Source", "", "close")
    ...
    strategy.parameters:addString("MA_Method", "MA Method", "Method", "MVA")
...
local first
local ARSI_Price, ARSI_Period, MA_Method, MA_Period, MA_Price
local MA, ARSI
local Direction
local CustomID
...
function Prepare(nameOnly)
    ...
    ARSI_Price = instance.parameters.ARSI_Price
    ARSI_Period = instance.parameters.ARSI_Period
    ...
    ARSI = core.indicators:create("ARSI", Source[ARSI_Price], ARSI_Period)
    first = math.max(ARSI.DATA:first(), MA.DATA:first())
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if ARSI.DATA[period] > MA.DATA[period] and ARSI.DATA[period - 1] <= MA.DATA[period - 1] then
        if Direction then
            BUY()
        else
            SELL()
    ...
    elseif ARSI.DATA[period] < MA.DATA[period] and ARSI.DATA[period - 1] >= MA.DATA[period - 1] then
        if Direction then
            SELL()
        else
            BUY()
