...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=5531
...
function Init() --The strategy profile initialization
    strategy:name("ADX trend strategy")
    strategy:description("ADX trend strategy")
    strategy:setTag(
    ...
    strategy.parameters:addGroup("ADX trend parameters")
    strategy.parameters:addInteger("ADX1_Period", "ADX 1 Period", "", 10)
    strategy.parameters:addInteger("ADX2_Period", "ADX 2 Period", "", 14)
    strategy.parameters:addInteger("ADX3_Period", "ADX 3 Period", "", 20)
    strategy.parameters:addDouble("Level1", "Level 1", "", 35)
    strategy.parameters:addDouble("Level2", "Level 2", "", 30)
...
function Prepare(nameOnly)
            ...
            instance.bid:name() ..
                "." ..
                    instance.parameters.TF ..
                        "," ..
                            "ADX_Trend(" ..
                                instance.parameters.ADX1_Period ..
                                    ", " ..
                                        instance.parameters.ADX2_Period ..
                                            ", " ..
                                                instance.parameters.ADX3_Period ..
                                                    ", " ..
    ...
    assert(core.indicators:findIndicator("ADX_TREND") ~= nil, "Please, download and install ADX_TREND.LUA indicator")
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ADX_Trend:update(core.UpdateLast)

    -- Check that we have enough data
    if (ADX_Trend.DATA:first() > (period - 1)) then
        return
    end
    ...
    if ADX_Trend.DATA[period] == source.high[period] then
        if Trigger == -1 then
    ...
    if ADX_Trend.DATA[period] == source.low[period] then
        if Trigger == 1 then
            if instance.parameters.TypeSignal == "direct" then
                MustS = true
            else
