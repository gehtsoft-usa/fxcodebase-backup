...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=10687
...
function Init() --The strategy profile initialization
    strategy:name("AC_Sig strategy")
    strategy:description("AC_Sig strategy")
...
function Prepare(nameOnly)
    ...
    assert(core.indicators:findIndicator("AC_SIG") ~= nil, "Please, download and install AC_SIG.LUA indicator")
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    AC_Sig:update(core.UpdateLast)

    -- Check that we have enough data
    if (AC_Sig.DATA:first() > (period - 1)) then
        return
    end
    ...
    if AC_Sig.DATA[period] > Source.low[period] / 2 and AC_Sig.DATA[period] <= Source.low[period] then
    ...
    if AC_Sig.DATA[period] >= Source.high[period] then
        if instance.parameters.TypeSignal == "direct" then
            MustOpenS = true
        else
            MustOpenB = true
