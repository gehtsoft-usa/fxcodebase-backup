...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=69605
...
function Init() --The strategy profile initialization
    strategy:name("Asymmetric Ease of move Strategy");
	...
	strategy.parameters:addString("TF1", "Lower Time frame", "", "H1");
    strategy.parameters:setFlag("TF1", core.FLAG_PERIODS);
    
	
	strategy.parameters:addString("TF2", "Lower Time frame", "", "D1");
    strategy.parameters:setFlag("TF2", core.FLAG_PERIODS);

    strategy.parameters:addGroup("Trade");
    strategy.parameters:addInteger("P1", "Lower Time Frame Period", "", 2);
    strategy.parameters:addInteger("P2", "Higher Time Frame Period", "", 2);
...
function CreateTradingParameters()
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "AEOMS");
...
function Prepare( nameOnly)
	...
	assert(core.indicators:findIndicator("ASYMMETRIC EASE OF MOVE") ~= nil, "Please, download and install ASYMMETRIC EASE OF MOVE.LUA indicator");          
    ...
    Indicator1 = core.indicators:create("ASYMMETRIC EASE OF MOVE", Source1.close,  instance.parameters.P1   ); 
	Indicator2 = core.indicators:create("ASYMMETRIC EASE OF MOVE", Source2.close, instance.parameters.P2   ); 
...
function ExitFunction(id, now,period)
	...
	--/////////////////////////////////////////////////////////////////////
		            ...
		            if   Indicator2.AverageUp[p2]~=1  then
					  ...
					  if   Indicator2.AverageDown[p2]~=1  then
...
function EntryFunction( id, now,period)
	...
	--/////////////////////////////////////////////////////////////////    
     if LastEntry == Source1:serial(p1)  
     --or not MA.DATA:hasData(period)
	 then
	 return;
	 end	
    ...
    if    Indicator2.AverageUp[p2] == 1 
	and Indicator1.AverageUp[p1] == 1 
	and Indicator1.AverageUp[p1-1] ~= 1 
	then	
          if Direction then
                BUY();
            else
		...
		Return=true;
    elseif  Indicator2.AverageDown[p2] == 1 
	and Indicator1.AverageDown[p1] == 1 
	and Indicator1.AverageDown[p1-1] ~= 1 
		then
            if Direction then
                SELL();
            else
