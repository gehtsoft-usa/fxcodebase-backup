...
function Prepare(nameOnly)
    ...
    first = math.max(Indicator["ARSI"].DATA:first(), Indicator["AVERAGES"].DATA:first()) + 1
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if core.crossesOver(Indicator["ARSI"].DATA, Indicator["AVERAGES"].DATA, period) then
        if Direction then
            BUY()
        else
            SELL()
        end
    elseif core.crossesUnder(Indicator["ARSI"].DATA, Indicator["AVERAGES"].DATA, period) then
        if Direction then
            SELL()
        else
            BUY()
