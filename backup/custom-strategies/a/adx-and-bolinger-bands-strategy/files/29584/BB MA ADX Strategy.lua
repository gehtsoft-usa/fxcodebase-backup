...
function Init() --The strategy profile initialization
    strategy:name("BB MA ADX Strategy")
    ...
    strategy.parameters:addInteger("MAP", "Number od Period", "", 200, 1, 1000)
    strategy.parameters:addString("MAM", "Method", "Method", "MVA")
    strategy.parameters:addStringAlternative("MAM", "MVA", "MVA", "MVA")
    strategy.parameters:addStringAlternative("MAM", "EMA", "EMA", "EMA")
    strategy.parameters:addStringAlternative("MAM", "LWMA", "LWMA", "LWMA")
    strategy.parameters:addStringAlternative("MAM", "TMA", "TMA", "TMA")
    strategy.parameters:addStringAlternative("MAM", "SMMA", "SMMA", "SMMA")
    strategy.parameters:addStringAlternative("MAM", "KAMA", "KAMA", "KAMA")
    strategy.parameters:addStringAlternative("MAM", "HMA", "HMA", "HMA")
    strategy.parameters:addStringAlternative("MAM", "WMA", "WMA", "WMA")
    strategy.parameters:addStringAlternative("MAM", "VIDYA", "VIDYA", "VIDYA")

    strategy.parameters:addGroup("ADX Calculation")
    strategy.parameters:addInteger("AP", "Number od Period", "", 14)
    strategy.parameters:addDouble("AL", "Adx Level", "", 20)

    strategy.parameters:addGroup("BB Calculation")
    strategy.parameters:addInteger("BP", "Number od Period", "", 20)
    strategy.parameters:addDouble("BD", "Deviation", "", 2)
...
function Prepare(nameOnly)
    ...
    assert(core.indicators:findIndicator(MAM) ~= nil, MAM .. " indicator must be installed");
    Indicator[1] = core.indicators:create(MAM, Source[Price], MAP)
    Short[1] = Indicator[1]:getStream(0)

    Indicator[2] = core.indicators:create("ADX", Source, AP)
    Short[2] = Indicator[2]:getStream(0)

    Indicator[3] = core.indicators:create("BB", Source[Price], BP, BD)
    Short[3] = Indicator[3]:getStream(0)
    Short[4] = Indicator[3]:getStream(1)
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if
        source.close[period] > Short[1][period] and Short[2][period] > AL and source.close[period] > Short[3][period] and
            source.close[period - 1] < Short[3][period - 1]
     then
        if Direction then
            BUY()
        else
    ...
    elseif
        source.close[period] < Short[1][period] and Short[2][period] > AL and source.close[period] < Short[4][period] and
            source.close[period - 1] > Short[4][period - 1]
     then
        if Direction then
            SELL()
        else
    ...
    if source.close[period] < Short[3][period] or source.close[period] < Short[1][period] then
        if Direction then
    ...
    if source.close[period] > Short[4][period] or source.close[period] > Short[1][period] then
        if Direction then
            if haveTrades("S") then
                exit("S")
                Signal("Close Short")
