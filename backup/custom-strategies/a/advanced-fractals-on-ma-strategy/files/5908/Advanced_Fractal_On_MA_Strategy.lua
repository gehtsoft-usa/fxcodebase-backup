...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=2624
...
function Init() --The strategy profile initialization
    strategy:name("Advanced_Fractal_On_MA_Strategy");
    ...
    strategy.parameters:addString("up_price", "up_price", "", "high")
    strategy.parameters:addStringAlternative("up_price", "close", "", "close")
    strategy.parameters:addStringAlternative("up_price", "open", "", "open")
    strategy.parameters:addStringAlternative("up_price", "high", "", "high")
    strategy.parameters:addStringAlternative("up_price", "low", "", "low")
    strategy.parameters:addStringAlternative("up_price", "median", "", "median")
    strategy.parameters:addStringAlternative("up_price", "typical", "", "typical")
    strategy.parameters:addStringAlternative("up_price", "weighted", "", "weighted")

    strategy.parameters:addString("dn_price", "dn_price", "", "low")
    strategy.parameters:addStringAlternative("dn_price", "close", "", "close")
    strategy.parameters:addStringAlternative("dn_price", "open", "", "open")
    strategy.parameters:addStringAlternative("dn_price", "high", "", "high")
    strategy.parameters:addStringAlternative("dn_price", "low", "", "low")
    strategy.parameters:addStringAlternative("dn_price", "median", "", "median")
    strategy.parameters:addStringAlternative("dn_price", "typical", "", "typical")
    strategy.parameters:addStringAlternative("dn_price", "weighted", "", "weighted")
...
function CreateTradingParameters()
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "AFOMAS");
...
function Prepare( nameOnly)
    ...
    MA_up = core.indicators:create("AVERAGES", Source[instance.parameters.up_price], instance.parameters.Method, instance.parameters.Period, false)
	MA_dn = core.indicators:create("AVERAGES", Source[instance.parameters.dn_price], instance.parameters.Method, instance.parameters.Period, false)
	first= MA_up.DATA:first() ;
...
function ExtUpdate(id, source, period)  -- The method called every time when a new bid or ask price appears.
   ...
   -- get only time
    now = now - math.floor(now);
	    ...
	    -- update indicators.
    MA_up:update(core.UpdateLast)
    MA_dn:update(core.UpdateLast)
	

	 if (MA_up.DATA:first() > period - math.floor((Frame - 1) / 2)) then
        return
    end
    if (MA_dn.DATA:first() > period - math.floor((Frame - 1) / 2)) then
        return
    end
...
function EntryFunction( now,period)
	...
	local up, dn = MA_up.DATA, MA_dn.DATA
    ...
    for i = 1, math.floor((Frame - 1) / 2), 1 do
        ...
        if dn[period - i - Shift] <= dn[period - Shift] or dn[period + i - Shift] <= dn[period - Shift] then
            
               
               MustB = false;   
        end
        if up[period - i - Shift] >= up[period - Shift] or up[period + i - Shift] >= up[period - Shift] then
