...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=65704
...
function signaler:AsyncOperationFinished(cookie, success, message, message1, message2)
    if cookie == self._telegram_timer and #self._alerts > 0 then
        ...
        core.host:trace("Alerts:" .. tostring(query));
...
function trading:Init(parameters)
    ...
    parameters:addString("side", "Side", "", "B");
    parameters:addStringAlternative("side", "Buy", "", "B");
    parameters:addStringAlternative("side", "Sell", "", "S");
...
function trading:FindOrder()
    local search = {};
    function search:SetCustomID(custom_id) self.CustomID = custom_id; return self; end
    function search:SetSide(bs) self.Side = bs; return self; end
    function search:SetInstrument(instrument) self.Instrument = instrument; return self; end
    function search:SetAccountID(account_id) self.AccountID = account_id; return self; end
    function search:SetRate(rate) self.Rate = rate; return self; end
...
function trading:FindTrade()
    ...
    function search:SetInstrument(instrument) self.Instrument = instrument; return self; end
    function search:SetAccountID(account_id) self.AccountID = account_id; return self; end
    function search:SetOpen(open) self.Open = open; return self; end
    function search:PassFilter(row)
        return (row.Instrument == self.Instrument or not self.Instrument)
            and (row.BS == self.Side or not self.Side)
            and (row.AccountID == self.AccountID or not self.AccountID)
...
function trading:CreateEntryOrderSuccessResult(msg)
    ...
    function res:GetClosedTrade()
        if self._trade == nil then
            self._trade = core.host:findTable("closed trades"):find("OpenOrderReqID", self.RequestID);
            if self._trade == nil then return nil; end
        end
        if not self._trade:refresh() then return nil; end
        return self._trade;
...
function trading:EntryOrder(instrument)
    ...
    function builder:SetStop(stop) self.valuemap.RateStop = stop; return self; end
...
function Init()
    strategy:name("David Molina Navarro");
...
function Prepare(name_only)
    ...
    core.host:execute("subscribeTradeEvents", 43, "orders");
...
function EntryFunction(source, period)
    if not trade_opened then
        trade_opened = true;
        core.host:trace(instance.parameters.stop);
        trade1 = trading:MarketOrder(source:instrument())
            :SetSide(instance.parameters.side)
            :SetPipStop(nil, instance.parameters.stop)
            :SetPipLimit(nil, instance.parameters.limit)
            :SetDefaultAmount()
            :Execute();
        trade2 = trading:MarketOrder(source:instrument())
            :SetSide(instance.parameters.side)
            :SetDefaultAmount()
            :Execute();
    end
...
function ExtUpdate(id, source, period)
    ...
    if id == 1 and trade1 ~= nil then
        if trade1.Finished and trade1.Success then
            local closed_trade = trade1:GetClosedTrade();
            local trade = trade2:GetTrade();
            if closed_trade ~= nil and trade ~= nil and trade2.StopCreate ~= true then
                if orders_types[closed_trade.CloseOrderID] == "L" then
                    signaler:Signal("Trade 1 was closed by limit. Moving stop");
                    trading:CreateStopOrder(trade.Open, trade, instance.parameters.trailing);
                    trade2.StopCreate = true;
                end
            end
        end
    end
...
function ExtAsyncOperationFinished(cookie, success, message, message1, message2)
    if cookie == 43 and message ~= nil then
        local order_id = message;
        local order = core.host:findTable("orders"):find("OrderID", order_id);
        if order ~= nil and order.Stage == "C" then
            orders_types[order.OrderID] = order.Type;
        end
    end
