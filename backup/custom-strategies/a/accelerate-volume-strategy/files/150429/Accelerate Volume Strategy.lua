-- Available @ https://fxcodebase.com/code/viewtopic.php?f=17&t=73604
...
function Init() --The strategy profile initialization
    strategy:name("Accelerate Volume Strategy");
    ...
    strategy.parameters:addInteger("Period1", "Fast Period", "", 4);  
...
function CreateTradingParameters()
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "AVS");
...
function Prepare( nameOnly)
	...
	assert(core.indicators:findIndicator("ACCELERATE VOLUME") ~= nil, "Please, download and install ACCELERATE VOLUME.LUA indicator");          
    ...
    Indicator = core.indicators:create("ACCELERATE VOLUME", Source , instance.parameters.Period1, instance.parameters.Period2); 
...
function EntryFunction( id, now,period)
	...
	if Indicator.AverageVolBuy[period] > Indicator.AverageVolSell[period]
	and Indicator.AverageVolBuy[period-1] <= Indicator.AverageVolSell[period-1]
	then
	a=true
	else
	a=false	
	...
	if Indicator.AverageVolBuy[period] > Indicator.AverageVolBuy[period-1] then
	b=true
	else
	b=false	
	end	  
	  
	if Indicator.MACDVolume[period] > Indicator.AverageVolSell[period] then
	c=true
	else
	c=false	
	end
...
// short position conditions
	...
	if Indicator.AverageVolSell[period] > Indicator.AverageVolBuy[period]
	and Indicator.AverageVolSell[period-1] <= Indicator.AverageVolBuy[period-1]
	then
	d=true
	else
	d=false	
	...
	if Indicator.AverageVolSell[period] > Indicator.AverageVolSell[period-1] then
	e=true
	else
	e=false	
	end	  
	  
	if Indicator.MACDVolume[period] > Indicator.AverageVolBuy[period] then
	f=true
	else
	f=false	
	end
