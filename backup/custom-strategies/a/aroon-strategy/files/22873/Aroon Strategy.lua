...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=11284
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if Consolidation then
        if Short[1][period] < Buy or Short[2][period] > Sell then
            if Direction then
        ...
        elseif Short[2][period] < Buy or Short[1][period] > Sell then
            if Direction then
                if haveTrades("S") then
                    exit("S")
                    Signal("Close Short")
