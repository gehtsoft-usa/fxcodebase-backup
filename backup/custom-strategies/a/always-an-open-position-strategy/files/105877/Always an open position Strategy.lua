...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=63396
...
function Init() --The strategy profile initialization
    strategy:name("Always an open position Strategy")
    ...
    strategy.parameters:addInteger("Period", "MA Period", "", 8)
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    -- update indicators.
    Open:update(core.UpdateLast)
    Close:update(core.UpdateLast)
    ...
    if Close.DATA[period] > Open.DATA[period] and Close.DATA[period - 1] <= Open.DATA[period - 1] then
        if Direction then
            BUY()
        else
            SELL()
    ...
    elseif Close.DATA[period] < Open.DATA[period] and Close.DATA[period - 1] >= Open.DATA[period - 1] then
        if Direction then
            SELL()
        else
            BUY()
