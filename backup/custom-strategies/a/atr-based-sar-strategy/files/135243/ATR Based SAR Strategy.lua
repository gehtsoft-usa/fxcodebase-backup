...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=70068
...
function Init() --The strategy profile initialization
    strategy:name("ATR Based SAR Strategy");
	...
	strategy.parameters:addInteger("P1", "Period", "", 14 );
	strategy.parameters:addDouble("P2", "Step Multiplier", "", 20 );
    strategy.parameters:addDouble("P3", "Max Multiplier", "", 40 );
...
function CreateTradingParameters()
   ...
   -- strategy.parameters:addStringAlternative("ExitExecutionType", "End of Turn", "", "EndOfTurn");
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "ABSS");
  ...
  --  strategy.parameters:addInteger("Stop", "Stop Order in pips", "", 30);
    strategy.parameters:addString("TRAILING_STOP_TYPE", "Trailing stop type", "",  "No");
...
function Prepare( nameOnly)
    ...
    PrepareTrading();
	 ...
	 assert(core.indicators:findIndicator("ATR BASED SAR") ~= nil, "Please, download and install ATR BASED SAR.LUA indicator");          
    ...
    Indicator = core.indicators:create("ATR BASED SAR", Source, P1, P2,P3  ); 
...
function ExtUpdate(id, source, period)  -- The method called every time when a new bid or ask price appears.
   ...
   return;
	...
	UpdateStop(Indicator.DATA[period]);
    ...
    while true do
        ...
        if row.AccountID == Account and
		   ...
		   then
		    ...
		    minChange = core.host:findTable("offers"):find("Instrument", row.Instrument).PointSize;
		   
            -- if trade has to be closed
	 
			   
			   
			    if math.abs(stopValue - row.Rate) < minChange then
				return;
				end
...
function EntryFunction( id, now,period)
	...
	then	
          if Direction then
                BUY(Indicator.DATA[period]);
            else
                SELL(Indicator.DATA[period]);
            end
...
function enter(BuySell, hCount,Level)
    ...
    return MarketOrder(BuySell,hCount,Level);
end


-- enter into the specified direction
function MarketOrder(BuySell,hCount,Level)
