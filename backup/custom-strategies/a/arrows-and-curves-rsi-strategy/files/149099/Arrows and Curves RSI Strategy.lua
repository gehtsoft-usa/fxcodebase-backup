...
-- https://fxcodebase.com/code/viewtopic.php?f=31&t=73205
...
function Init() --The strategy profile initialization
    strategy:name("Arrows and Curves RSI Strategy.");
    ...
    strategy.parameters:addDouble("BuyLevel", "Buy Level", "", 55);  	
    strategy.parameters:addDouble("SellLevel", "Sell Level", "", 45);  

 	strategy.parameters:addGroup("Arrows and Curves Calculation");	
    strategy.parameters:addInteger("Period1", "SSP", "", 6, 1, 2000);
    strategy.parameters:addInteger("Period2", "SkyCh", "", 13, 1, 2000);
...
function CreateTradingParameters()
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "ACRS");
...
function Prepare( nameOnly)
	...
	assert(core.indicators:findIndicator("ARROWS & CURVES") ~= nil, "Please, download and install ARROWS & CURVES.LUA indicator");          
    ...
    Signal_Indicator = core.indicators:create("ARROWS & CURVES", Source, Period1, Period2, true ); 	 
	first=math.max(RSI.DATA:first(),Signal_Indicator.DATA:first()); 
...
function ExtUpdate(id, source, period)  -- The method called every time when a new bid or ask price appears.
   ...
   Signal_Indicator:update(core.UpdateLast);
...
function EntryFunction( id, now,period)
    ...
    if  Signal_Indicator.DATA[period]==1
	and  RSI.DATA[period]> BuyLevel
	then	
          if Direction then
                BUY();
            else
		...
		Return=true;
    elseif  Signal_Indicator.DATA[period]==-1	
	and  RSI.DATA[period]< SellLevel	
	then
            if Direction then
                SELL();
            else
