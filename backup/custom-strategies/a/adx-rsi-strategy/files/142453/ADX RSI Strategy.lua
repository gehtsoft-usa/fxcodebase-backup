...
-- https://fxcodebase.com/code/viewtopic.php?f=31&t=71216
...
function Init() --The strategy profile initialization
    strategy:name("ADX RSI Strategy");
	...
	strategy.parameters:addBoolean("MA_Filter", "MA Filter", "", false); 
	strategy.parameters:addString("TF1", "Time frame", "", "m1");
    strategy.parameters:setFlag("TF1", core.FLAG_PERIODS);
	strategy.parameters:addInteger("Period1", "1. MA Period", "Period" , 9);
	...
	strategy.parameters:addInteger("Period2", "2. MA Period", "Period" , 39);
    ...
    strategy.parameters:addDouble("RSI_Buy_Level_1", "1. Buy Level", "", 55);
	strategy.parameters:addDouble("RSI_Buy_Level_2", "2. Buy Level", "", 75);
	
	strategy.parameters:addDouble("RSI_Sell_Level_1", "1. Sell Level", "", 45);
	strategy.parameters:addDouble("RSI_Sell_Level_2", "2. Sell Level", "", 25);
		
    strategy.parameters:addDouble("RSI_Buy_Close_Level", "Buy Close Level", "", 50);
	strategy.parameters:addDouble("RSI_Sell_Close_Level", "Sell Close Level", "", 50);
	
    strategy.parameters:addGroup("ADX Calculation");
    strategy.parameters:addInteger("ADX_Period", "Period", "", 14);
	strategy.parameters:addDouble("ADX_Level", "Level", "", 25);
...
function CreateTradingParameters()
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "ARS");
...
local RSI, RSI_Period,   ADX,  ADX_Period, ADX_Level;
local RSI_Buy_Level_1,  RSI_Sell_Level_1;
local RSI_Buy_Level_2,  RSI_Sell_Level_2;
local MA1,MA2;
local MA_Filter;
function Prepare( nameOnly)
	...
	RSI_Buy_Level_1 = instance.parameters.RSI_Buy_Level_1;
	RSI_Sell_Level_1 = instance.parameters.RSI_Sell_Level_1;
	RSI_Buy_Level_2 = instance.parameters.RSI_Buy_Level_2;
	RSI_Sell_Level_2 = instance.parameters.RSI_Sell_Level_2;
	
	RSI_Buy_Close_Level = instance.parameters.RSI_Buy_Close_Level;
	RSI_Sell_Close_Level = instance.parameters.RSI_Sell_Close_Level;
    ...
    MA1 = core.indicators:create(instance.parameters.Method1, Source1.close , instance.parameters.Period1   );
	MA2 = core.indicators:create(instance.parameters.Method2, Source1.close , instance.parameters.Period2   );
	first=math.max(RSI.DATA:first(),ADX.DATA:first()); 
...
function ExitFunction(id, now,period)
	...
	RSI Cross Over RSI Sell Close Level
		            ...
		            if    RSI.DATA[period] < RSI_Buy_Close_Level  then
					  ...
					  if   RSI.DATA[period]  >  RSI_Sell_Close_Level then
...
RSI < RSI Sell Level (for exemple 45), but, if RSI < second level, it's a long (for exemple 25)
    ...
    if RSI.DATA[period] > RSI_Buy_Level_1
	and RSI.DATA[period] < RSI_Buy_Level_2
	and   ADX.DATA[period] > ADX_Level
	and   ADX.DATA[period-1] <= ADX_Level
	and  ((MA1.DATA[period1]>  MA2.DATA[period1] and MA_Filter)or not MA_Filter)
	then	
          if Direction then
                BUY();
            else
		...
		Return=true;
    elseif RSI.DATA[period] < RSI_Sell_Level_1
	and RSI.DATA[period] > RSI_Sell_Level_2
	and   ADX.DATA[period] > ADX_Level
	and   ADX.DATA[period-1] <= ADX_Level
	and  ((MA1.DATA[period1]<  MA2.DATA[period1] and MA_Filter)or not MA_Filter)
	then
