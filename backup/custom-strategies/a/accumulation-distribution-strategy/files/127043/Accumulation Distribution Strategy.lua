...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=68592
...
function Init() --The strategy profile initialization
    strategy:name("Accumulation Distribution Strategy");
    ...
    strategy.parameters:addDouble("Up_Value", "Up Value", "", 0.0001);
    strategy.parameters:addDouble("Down_Value", "Down Value", "", -0.0001);
...
local Indicator;
local Up_Value,Down_Value,Price;
...
function Prepare( nameOnly)
	...
	assert(core.indicators:findIndicator("ACCUMULATION DISTRIBUTION") ~= nil, "Please, download and install ACCUMULATION DISTRIBUTION.LUA indicator");          
    ...
    Indicator = core.indicators:create("ACCUMULATION DISTRIBUTION", Source[Price], Up_Value, Down_Value  );	
...
function EntryFunction( id, now,period)
    ...
    if   Indicator.Accumulation[period]> Indicator.Distribution[period]
	and  Indicator.Accumulation[period-1]<= Indicator.Distribution[period-1]
	then	
          if Direction then
                BUY();
            else
		...
		Return=true;
    elseif   Indicator.Accumulation[period]< Indicator.Distribution[period]
	and  Indicator.Accumulation[period-1]>= Indicator.Distribution[period-1]
		then
            if Direction then
                SELL();
