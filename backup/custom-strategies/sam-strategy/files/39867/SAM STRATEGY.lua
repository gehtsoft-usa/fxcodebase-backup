...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=23139
...
function Init() --The strategy profile initialization
    ...
    strategy.parameters:addInteger("MP", "Period", " Period", 200);
	strategy.parameters:addString("MM", "Method", "Method" , "MVA");
    strategy.parameters:addStringAlternative("MM", "MVA", "MVA" , "MVA");
    strategy.parameters:addStringAlternative("MM", "EMA", "EMA" , "EMA");
	strategy.parameters:addStringAlternative("MM", "LWMA", "LWMA" , "LWMA");
    strategy.parameters:addStringAlternative("MM", "TMA", "TMA" , "TMA");
    strategy.parameters:addStringAlternative("MM", "SMMA", "SMMA" , "SMMA");
    strategy.parameters:addStringAlternative("MM", "WMA", "WMA" , "WMA");
	strategy.parameters:addStringAlternative("MM", "KAMA", "KAMA" , "KAMA");
	strategy.parameters:addStringAlternative("MM", "VIDYA", "VIDYA", "VIDYA"); 
	
    strategy.parameters:addGroup("ADX Calculation");
	strategy.parameters:addInteger("AP", "Period", " Period", 14);
	strategy.parameters:addDouble("AL", "Entry Level", "Entry  Level", 20);
	
	strategy.parameters:addGroup("Stochastic Calculation");
	strategy.parameters:addInteger("K", "Number of periods for %K", "The number of periods for %K.", 9, 2, 1000);
	...
	strategy.parameters:addDouble("OB", "OB Level", "OB  Level", 80);
	strategy.parameters:addDouble("OS", "OS Level", "OS  Level", 20);
...
local MM, MP, AP, AL;
local K, SD,D, MVAT_K,MVAT_D, OB, OS;
...
function Prepare(nameOnly)
    ...
    local i;
	   ...
	   name = name .. ", " ..  MP.. ", " .. MM.. ", " .. AP.. ", " .. AL
	               .. ", " .. K.. ", " .. SD.. ", " ..D.. ", " .. MVAT_K.. ", " ..MVAT_D.. ", " .. OB .. ", " .. OS   ;
    ...
    Indicator["MA"] = core.indicators:create(MM, Source[Price],MP  );
	 Indicator["ADX"] = core.indicators:create("ADX", Source, AP  );
	  Indicator["STO"] = core.indicators:create("STOCHASTIC", Source, K,SD,D, MVAT_K,MVAT_D  );

	
	first= math.max(Indicator["MA"].DATA:first(),Indicator["ADX"].DATA:first(), Indicator["STO"].D:first());
...
function ExtUpdate(id, source, period)  -- The method called every time when a new bid or ask price appears.
    ...
    Indicator["MA"]:update(core.UpdateLast);
   ...
   -- get only time
    now = now - math.floor(now);
      ...
      return ;
        ...
        if  Source.close[period]  > Indicator["MA"].DATA[period]
		and    Indicator["ADX"].DATA[period] < AL
		and Indicator["STO"].K[period]< OS
		and core.crossesOver(Indicator["STO"].K, Indicator["STO"].D,period) 
		then
		...
		elseif Source.close[period] < Indicator["MA"].DATA[period]
		and    Indicator["ADX"].DATA[period] < AL
		and   Indicator["STO"].K[period] > OB
		and core.crossesOver(Indicator["STO"].K, Indicator["STO"].D,period) 
		 then
            if Direction then
                SELL();
                ...
                BUY();
		            ...
		            if    core.crossesUnder(Source.close, Indicator["MA"].DATA,period) 
					or 
					(
					Indicator["STO"].K[period]> OB 
					and
					core.crossesUnder(Indicator["STO"].K, Indicator["STO"].D,period)
					)
					then 			
								if   Direction then
					  ...
					  if core.crossesOver(Source.close, Indicator["MA"].DATA,period)
					  or  
					  (
					  Indicator["STO"].K[period]< OS
					  and
					  core.crossesOver(Indicator["STO"].K, Indicator["STO"].D,period)
					  )
		              then
								   if   Direction then    
										if haveTrades("S") then
