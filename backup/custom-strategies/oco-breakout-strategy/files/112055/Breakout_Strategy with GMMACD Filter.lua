...
function Init()
    strategy:name("Breakout_Strategy with GMMACD Filter")
...
function Prepare(nameOnly)
    ...
    G = core.indicators:create("GMMACD", gTick)
...
function ExtUpdate(id, source, period)
    ...
    if check and P:hasData(NOW) and period >= stream:first() + 1 then
        ...
        if P[NOW] == BE_BOX then
            if
                ((prev <= H[NOW] + Threshold) and (curr > H[NOW] + Threshold)) and tradesCount("B") == 0 and
                    (Multiple or (not todayBuy)) and
                    G.DATA[NOW] > 0
             then
                if (AllowTrade) then
