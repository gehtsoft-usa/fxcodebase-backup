...
function Init() --The strategy profile initialization
    strategy:name("Ehlers Loops Strategy");
    ...
    strategy.parameters:addInteger("periodLP", "Low-Pass Period", "", 20, 7, 2000);
    strategy.parameters:addInteger("periodHP", "High-Pass Period", "", 125, 20, 2000);
    strategy.parameters:addInteger("periodRMS", "RMS Period", "", 80, 1, 2000);	
...
function CreateTradingParameters()
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "ELS");
...
function Prepare( nameOnly)
	...
	assert(core.indicators:findIndicator("EHLERS LOOPS") ~= nil, "Please, download and install EHLERS LOOPS.LUA indicator");          
    ...
    Indicator = core.indicators:create("EHLERS LOOPS", Source, instance.parameters.periodLP , instance.parameters.periodHP, instance.parameters.periodRMS   ); 
...
function EntryFunction( now,period)
    ...
    if   Indicator.PriceRMS[period] >   Indicator.VolRMS[period]
	and    Indicator.PriceRMS[period-1] <=   Indicator.VolRMS[period-1]
	then	
          if Direction then
                BUY();
            else
		...
		Return=true;
    elseif   Indicator.PriceRMS[period] <   Indicator.VolRMS[period]
	and    Indicator.PriceRMS[period-1] >=   Indicator.VolRMS[period-1]
		then
            if Direction then
                SELL();
