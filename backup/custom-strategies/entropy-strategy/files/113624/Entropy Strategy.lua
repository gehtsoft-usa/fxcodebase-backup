...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=64931
...
function Init() --The strategy profile initialization
    strategy:name("Entropy Strategy")
    ...
    strategy.parameters:addBoolean("S1", "Bullish divergece", "", true)
    strategy.parameters:addBoolean("S2", "Positive divergece", "", true)
    strategy.parameters:addBoolean("S3", "Bearish divergence", "", true)
    strategy.parameters:addBoolean("S4", "Negative divergence", "", true)

    strategy.parameters:addGroup("Calculation")
    strategy.parameters:addInteger("Period", "Period", "Period", 14)
    strategy.parameters:addDouble("BL", "Buy Level", "Level", 0)
    strategy.parameters:addDouble("SL", "Sell Level", "Level", 0)
...
local S1, S2, S3, S4, BL, SL, Period, Indicator
...
function Prepare(nameOnly)
    ...
    assert(core.indicators:findIndicator("ENTROPY") ~= nil, "Please, download and install ENTROPY.LUA indicator")
    ...
    Indicator = core.indicators:create("ENTROPY", Source.close, Period)
...
function EntryFunction(now, period)
    ...
    if (iSignal == 1 and S1) or (iSignal == 2 and S2) then
        ...
        Return = true
    elseif (iSignal == 3 and S3) or (iSignal == 4 and S4) then
        if Direction then
            SELL()
        else
            BUY()
...
function ItIs(period)
    ...
    local T1, T2, B1, B2 = Fractal(period)

    if
        B1 ~= 0 and B2 ~= 0 and B2 == period - 2 and Indicator.DATA[B2] < BL and Source.low[B1] > Source.low[B2] and
            Indicator.DATA[B1] < Indicator.DATA[B2]
     then
        Is = 1
    end
    ...
    if
        T1 ~= 0 and T2 ~= 0 and T2 == period - 2 and Indicator.DATA[T2] > SL and Source.high[T1] < Source.high[T2] and
            Indicator.DATA[T1] > Indicator.DATA[T2]
     then
        Is = 3
    end
...
function Fractal(period)
    ...
    for i = period, first, -1 do
        ...
        if T1 ~= 0 and T2 ~= 0 and B1 ~= 0 and B2 ~= 0 then
            break
        end
    end
