...
--http://fxcodebase.com/code/viewtopic.php?f=31&t=70726
...
local STRATEGY_NAME = "Plastic strategy";
function CreateParameters() 
    strategy.parameters:addInteger("volumeMultiple", "VolumeMultiple", "", 3)
end

local volumeMultiple, vol, ema;
function CreateEntryIndicators(source)
    volumeMultiple = instance.parameters.volumeMultiple;
    ema = core.indicators:create("EMA", source, 239);
    vol = core.indicators:create("MVA", source.volume, 40);
end

function UpdateIndicators()
    ema:update(core.UpdateLast);
    vol:update(core.UpdateLast);
end
...
function IsEntryLong(source, period)
    ...
    local ll, hh = mathex.minmax(source, core.rangeTo(period - 1, 9));
    local ll50, hh50 = mathex.minmax(source, core.rangeTo(period - 1, 50));
    local ll20, hh20 = mathex.minmax(source, core.rangeTo(period - 1, 20));
    local ll40, hh40 = mathex.minmax(source, core.rangeTo(period - 1, 40));
    local up = ((source.high[period] > hh) and (source.low[period] > ll) and (source.close[period] > hh50) and ((ll20 < (ll40 * 1.05)) or (ll20 <= (ll40 + source:pipSize()))) and ((hh20 > (hh40 * 0.95)) or (hh20 >= (hh40 - source:pipSize()))) and (source.volume[period] > vol.DATA[period - 1] * volumeMultiple));
    return source.close[period] > ema.DATA[period] and up;
end
...
function IsEntryShort(source, period)
    ...
    local ll20, hh20 = mathex.minmax(source, core.rangeTo(period - 1, 20));
    local ll40, hh40 = mathex.minmax(source, core.rangeTo(period - 1, 40));
    local dn = ((source.high[period] < hh) and (source.low[period] < ll) and (source.close[period] < hh50) and ((ll20 > (ll40 * 1.05)) or (ll20 >= (ll40 + source:pipSize()))) and ((hh20 < (hh40 * 0.95)) or (hh20 <= (hh40 - source:pipSize()))) and (source.volume[period] > vol.DATA[period - 1] * volumeMultiple));
    return source.low[period] < ema.DATA[period] and dn;
end
