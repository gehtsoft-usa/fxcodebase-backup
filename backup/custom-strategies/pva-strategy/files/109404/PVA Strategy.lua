...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=64169
...
function Init() --The strategy profile initialization
    ...
    strategy.parameters:addInteger("PVA_Climax_Period", "PVA Climax Period", "PVA Climax Period", 10)
    strategy.parameters:addInteger("PVA_Rising_Period", "PVA Rising Period", "PVA Rising Period", 10)
    strategy.parameters:addDouble("PVA_Rising_Factor", "PVA Rising Factor", "PVA Rising Factor", 1)
    strategy.parameters:addDouble("PVA_Extreme_Factor", "PVA Extreme Factor", "PVA Extreme Factor", 2)
...
local PVA_Climax_Period, PVA_Rising_Period, PVA_Rising_Factor, PVA_Extreme_Factor
local PVA
...
function ExitFunction(period, now)
    ...
    if not InRange(now, OpenTime, CloseTime) and ManageExit then
        return
    end
    ...
    if PVA.DATA:colorI(period) ~= core.rgb(0, 255, 0) then
        if Direction then
    ...
    if PVA.DATA:colorI(period) ~= core.rgb(255, 0, 0) then
        if Direction then
            if haveTrades("S") then
                exitSpecific("S")
                Signal("Close Short")
...
function EntryFunction(period, now)
    ...
    if PVA.DATA:colorI(period) == core.rgb(0, 255, 0) and PVA.DATA:colorI(period - 1) ~= core.rgb(0, 255, 0) then
        if Direction then
            BUY()
        else
            SELL()
        ...
        Return = true
    elseif PVA.DATA:colorI(period) == core.rgb(255, 0, 0) and PVA.DATA:colorI(period - 1) ~= core.rgb(255, 0, 0) then
        if Direction then
            SELL()
        else
            BUY()
