...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=63489
...
function Init() --The strategy profile initialization
    strategy:name("Extreme TMA  Strategy")
    ...
    strategy.parameters:addInteger("Period", "MA Period", "Period", 200)
    ...
    strategy.parameters:addGroup("Extreme TMA Line Calculation")
    ...
    strategy.parameters:addGroup("Extreme TMA Slope Calculation")
...
local TMA_Period2, ATR_Period2, TrendThreshold2, Redraw2
local Slope, Line
local Direction
local CustomID
...
function Prepare(nameOnly)
    ...
    assert(
        core.indicators:findIndicator("EXTREME_TMA_SLOPE") ~= nil,
        "Please, download and install EXTREME_TMA_SLOPE.LUA indicator"
    )
    ...
    first = math.max(MA.DATA:first(), Line.DATA:first(), Slope.DATA:first())
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    -- update indicators.
    Line:update(core.UpdateLast)
    Slope:update(core.UpdateLast)
    ...
    --a. Extreme Slope or Extreme Line turns red,    then Extreme Slope or Extreme Line turns red on closing candle, SELL MARKET.

    -- only buy if we have a fast cross over slow and the price is above the moving averages.
    if
        Source.close[period] > MA.DATA[period] and Slope.DATA:colorI(period) == core.rgb(0, 255, 0) and
            Line.DATA:colorI(period) == core.rgb(0, 255, 0)
     then
        if Direction then
            BUY()
        else
    ...
    elseif
        Source.close[period] < MA.DATA[period] and Slope.DATA:colorI(period) == core.rgb(255, 0, 0) and
            Line.DATA:colorI(period) == core.rgb(255, 0, 0)
     then
        if Direction then
            SELL()
        else
    ...
    --  b. If Extreme Slope or Extreme Line turns green on candle close, STOP MARKET.
    -- c. If price closes above 200 EMA on candle close, STOP MARKET .

    if Exit then
        if
            Slope.DATA:colorI(period) ~= core.rgb(0, 255, 0) or Line.DATA:colorI(period) ~= core.rgb(0, 255, 0) or
                Source.close[period] < MA.DATA[period]
         then
            if Direction then
        ...
        if
            Slope.DATA:colorI(period) ~= core.rgb(255, 0, 0) or Line.DATA:colorI(period) ~= core.rgb(255, 0, 0) or
                Source.close[period] > MA.DATA[period]
         then
            if Direction then
                if haveTrades("S") then
