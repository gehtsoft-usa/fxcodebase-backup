...
function CreateTradingParameters()
    ...
    strategy.parameters:addBoolean("Exit", "Optional Exit", "", true)
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if Line.DATA:colorI(period) == core.rgb(0, 255, 0) and Line.DATA:colorI(period - 1) ~= core.rgb(0, 255, 0) then
        if Direction then
            BUY()
        else
            SELL()
    ...
    elseif Line.DATA:colorI(period) == core.rgb(255, 0, 0) and Line.DATA:colorI(period - 1) ~= core.rgb(255, 0, 0) then
        if Direction then
            SELL()
        else
            BUY()
    ...
    if Exit then
        if Line.DATA:colorI(period) == core.rgb(128, 128, 128) then
