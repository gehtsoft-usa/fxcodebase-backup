...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=6571&p
...
function Prepare(nameOnly)
    ...
    assert(core.indicators:findIndicator("VORTEX") ~= nil, "Please, download and install VORTEX indicator")
    assert(instance.parameters.TF ~= "t1", "The time frame must not be tick")

    local name
    name = profile:id() .. "(" .. instance.bid:name() .. "." .. instance.parameters.TF .. ", " .. MA_Period .. ")"
    instance:name(name)
    ...
    Vortex = core.indicators:create("VORTEX", Source, MA_Period)

    first = Vortex.DATA:first()
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    Vortex:update(core.UpdateLast)

    if core.crossesOver(Vortex.VIP, Vortex.VIM, period) then
        if Direction == "direct" then
            BUY()
        else
            SELL()
        end
    elseif core.crossesUnder(Vortex.VIP, Vortex.VIM, period) then
        if Direction == "direct" then
            SELL()
        else
            BUY()
