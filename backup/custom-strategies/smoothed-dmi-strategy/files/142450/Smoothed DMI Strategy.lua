...
-- https://fxcodebase.com/code/viewtopic.php?f=31&t=71261
...
function Init() --The strategy profile initialization
    strategy:name("Smoothed DMI Strategy");
    ...
    strategy.parameters:addInteger("MA_Period", "Period", "", 14, 1, 1000);
	...
	strategy.parameters:addGroup("Pre-Calculation");
    ...
    strategy.parameters:addInteger("Period1", "Period", "", 14, 1, 1000);
	...
	strategy.parameters:addGroup("Post-Smoothing Calculation");
	strategy.parameters:addInteger("Period2", "MA Period", "Period" , 14);
...
function CreateTradingParameters()
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "SDS");
...
function Prepare( nameOnly)
	...
	assert(core.indicators:findIndicator("SMOOTHED DMI") ~= nil, "Please, download and install SMOOTHED DMI.LUA indicator");          
    ...
    Indicator = core.indicators:create("SMOOTHED DMI", Source,Period,Method,Period1,Method1 ,Period2,Method2    );	 
...
function EntryFunction( id, now,period)
    ...
    if   Indicator.DIP[period]> Indicator.DIM[period]
	and  Indicator.DIP[period-1]<= Indicator.DIM[period-1]
	and ((Filter and Source.close[period]> MA.DATA[period])or not Filter)
	then	
          if Direction then
                BUY();
            else
		...
		Return=true;
    elseif  Indicator.DIP[period]< Indicator.DIM[period]
	and  Indicator.DIP[period-1]>= Indicator.DIM[period-1]
	and ((Filter and Source.close[period]< MA.DATA[period])or not Filter)
	then
            if Direction then
