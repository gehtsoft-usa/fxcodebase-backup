...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=63659
...
function ExtUpdate(id, source, period)  -- The method called every time when a new bid or ask price appears.
    ...
    if instance.parameters.Exit then
        if core.crossesOver(Indicator.PB, Indicator.NRMS, period) then
            if haveTrades("B") then
                exitSpecific("B");
                Signal("Exit Long");
            end
        elseif core.crossesUnder(Indicator.PB, Indicator.PRMS, period) then
...
function ExtAsyncOperationFinished(cookie, success, message, message1, message2) for _, module in pairs(Modules) do if module.AsyncOperationFinished ~= nil then module:AsyncOperationFinished(cookie, success, message, message1, message2); end end
    if cookie == 100 then
        -- timer
