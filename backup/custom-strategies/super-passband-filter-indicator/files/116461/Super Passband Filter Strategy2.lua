...
function Init() --The strategy profile initialization
    strategy:name("Super Passband Filter Strategy");
...
strategy:setTag("NonOptimizableParameters", "Email,SendEmail,SoundFile,RecurrentSound,PlaySound, ShowAlert");
    ...
    strategy.parameters:addGroup("Super PassBand Calculation");
    strategy.parameters:addInteger("Period1", "1. Period", "", 40 );
	strategy.parameters:addInteger("Period2", "2. Period", "", 60 );
    strategy.parameters:addInteger("Average", "Average Period", "", 50 );
	
	
	strategy.parameters:addGroup("MA Calculation");
	
	strategy.parameters:addBoolean("Filter", "Use MA Filter", "", true);  
    strategy.parameters:addInteger("Period", "Period", "", 200 );
...
function CreateTradingParameters()
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "SPFS");
    ...
    strategy.parameters:addGroup("Reversal levels");
    strategy.parameters:addBoolean("UseReversal", "Use reversal levels", "", true);
    strategy.parameters:addDouble("ReversalB", "Reversal level for Buy", "", 0.0002);
    strategy.parameters:addDouble("ReversalS", "Reversal level for Sell", "", -0.0002);
...
local UseReversal, ReversalB, ReversalS;
...
function Prepare( nameOnly)
    ...
    UseReversal=instance.parameters.UseReversal;
    ReversalB=instance.parameters.ReversalB;
    ReversalS=instance.parameters.ReversalS;
	...
	assert(core.indicators:findIndicator("SUPER PASSBAND FILTER") ~= nil, "Please, download and install SUPER PASSBAND FILTER.LUA indicator");          
    ...
    Indicator = core.indicators:create("SUPER PASSBAND FILTER", Source.close, Period1, Period2, Average  );
    assert(core.indicators:findIndicator(Method) ~= nil, Method .. " indicator must be installed");
	MA = core.indicators:create(Method, Source.close, Period  ); 
	first=math.max(Indicator.PRMS:first(),MA.DATA:first()); 
...
function ExtUpdate(id, source, period)  -- The method called every time when a new bid or ask price appears.
   ...
   -- get only time
	...
	and Indicator.PB[period] > Indicator.PRMS[period]
	and Indicator.PB[period-1] <= Indicator.PRMS[period-1]
	then	
          if Direction then
                BUY();
            else
        ...
        Fl=false;
    elseif  (not Filter or ( Filter and Source.close[period]< MA.DATA[period]))
	and Indicator.PB[period] < Indicator.NRMS[period]
	and Indicator.PB[period-1] >= Indicator.NRMS[period-1]
		then
            if Direction then
                SELL();
            else
    ...
    if UseReversal and Fl then
        if Indicator.PB[period-1]<ReversalB and Indicator.PB[period]>ReversalB then
            if Direction then
                BUY();
            else
                SELL();
        ...
        if Indicator.PB[period-1]>ReversalS and Indicator.PB[period]<ReversalS then
            if Direction then
                SELL();
            else
                BUY();
