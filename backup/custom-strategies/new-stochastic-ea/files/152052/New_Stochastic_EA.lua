-- Available @ https://fxcodebase.com/code/viewtopic.php?f=31&t=74035
...
local STRATEGY_NAME = "New Stochastic Strategy";
...
function IsEntryLong(source, period)
    if not ssd.K:hasData(period - 1) or not ssd.D:hasData(period - 1) then
        return false;
    end
    return core.crossesOver(ssd.K, ssd.D, period);
end
function IsEntryShort(source, period)
    if not ssd.K:hasData(period - 1) or not ssd.D:hasData(period - 1) then
        return false;
    end
    return core.crossesUnder(ssd.K, ssd.D, period);
end
