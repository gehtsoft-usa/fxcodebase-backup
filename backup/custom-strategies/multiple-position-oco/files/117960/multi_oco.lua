...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=65793
...
local Modules = {};
function Init()
    strategy:name("Multiple OCO");
    strategy:description("");
    strategy.parameters:addDouble("buy_rate", "Buy order rate", "", 0);
    strategy.parameters:setFlag("buy_rate", core.FLAG_PRICE);
    strategy.parameters:addDouble("sell_rate", "Sell order rate", "", 0);
    strategy.parameters:setFlag("sell_rate", core.FLAG_PRICE);
...
function Prepare(name_only)
    ...
    buy_rate = instance.parameters.buy_rate;
    sell_rate = instance.parameters.sell_rate;
    ExtSubscribe(1, nil, "t1", instance.parameters.is_bid);
...
function ExtUpdate(id, source, period)
    ...
    if not created then
        ...
        for _, t in ipairs(trading.Trades) do
            if t.used then
                local buy_order = trading:EntryOrder(instrument)
                    :SetSide("B")
                    :SetAmount(t.amount)
                    :SetRate(buy_rate);
                if t.set_stop then
                    buy_order = buy_order:SetPipStop(nil, t.stop, t.use_trailing and t.trailing or nil);
                end
                if t.set_limit then
                    buy_order = buy_order:SetPipLimit(nil, t.limit);
                end
                local sell_order = trading:EntryOrder(instrument)
                    :SetSide("S")
                    :SetAmount(t.amount)
                    :SetRate(sell_rate);
                if t.set_stop then
                    sell_order = sell_order:SetPipStop(nil, t.stop, t.use_trailing and t.trailing or nil);
                end
                if t.set_limit then
                    sell_order = sell_order:SetPipLimit(nil, t.limit);
                end
                local command = core.valuemap();
                command.Command = "CreateOCO";
                command:append(buy_order:GetValueMap());
                command:append(sell_order:GetValueMap());
                local success, msg = terminal:execute(101, command);
                if not success then
                    signaler:Signal("Failed to create orders: " .. msg);
                end
            end
        end
    end
...
function AddTradeParams(parameters, index)
    parameters:addGroup("Trade " .. index);
    parameters:addBoolean("trade_" .. index, "Open Trade " .. index, "", true);
    parameters:addInteger("trade_" .. index .. "_amount", "Trade Amount in Lots", "", 1);
    parameters:addBoolean("trade_" .. index .. "_set_stop", "Set stop for Trade " .. index, "", false);
    parameters:addInteger("trade_" .. index .. "_stop", "Stop for Trade " .. index .. ", in pips", "", 30);
    parameters:addBoolean("trade_" .. index .. "_use_trailing", "Trailing stop order for Trade " .. index, "", false);
    parameters:addInteger("trade_" .. index .. "_trailing", "Trailing for Trade " .. index .. ", in pips", "Use 1 for dynamic and 10 or greater for the fixed trailing", 1);
    parameters:addBoolean("trade_" .. index .. "_set_limit", "Set Limit for Trade " .. index, "", false);
    parameters:addInteger("trade_" .. index .. "_limit", "Limit for Trade " .. index .. ", in pips", "", 30);
end
...
function trading:Init(parameters)
    ...
    AddTradeParams(parameters, 1);
    AddTradeParams(parameters, 2);
    AddTradeParams(parameters, 3);
    AddTradeParams(parameters, 4);
    AddTradeParams(parameters, 5);
end
...
function trading:Prepare(name_only)
    ...
    for index = 1, 5 do
        local trade = {};
        trade.used = instance.parameters:getBoolean("trade_" .. index);
        trade.set_stop = instance.parameters:getBoolean("trade_" .. index .. "_set_stop");
        trade.use_trailing = instance.parameters:getBoolean("trade_" .. index .. "_use_trailing");
        trade.set_limit = instance.parameters:getBoolean("trade_" .. index .. "_set_limit");
        trade.stop = instance.parameters:getInteger("trade_" .. index .. "_stop");
        trade.trailing = instance.parameters:getInteger("trade_" .. index .. "_trailing");
        trade.limit = instance.parameters:getInteger("trade_" .. index .. "_limit");
        trade.amount = instance.parameters:getInteger("trade_" .. index .. "_amount");
...
function trading:EntryOrder(instrument)
    ...
    function builder:_GetBaseUnitSize() if self._base_size == nil then self._base_size = core.host:execute("getTradingProperty", "baseUnitSize", self.Instrument, self.Parent._account); end return self._base_size; end
...
function signaler:AsyncOperationFinished(cookie, success, message, message1, message2)
    if cookie == self._telegram_timer and #self._alerts > 0 then
...
function signaler:Signal(label, source)
    ...
    if self._telegram_key ~= nil then
        self:AlertTelegram(profile:id() .. " : " .. label, source:instrument(), source:barSize());
    end
end
