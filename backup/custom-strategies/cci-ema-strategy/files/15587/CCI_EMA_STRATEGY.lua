...
function Init() --The strategy profile initialization
    ...
    strategy.parameters:addInteger("MA_Period", "EMA_Period", "", 200)
...
function Prepare(nameOnly)
    ...
    indicator["MA"] = core.indicators:create(Method, Source[Price], MA_Period)
    indicator["CCI"] = core.indicators:create("CCI", Source, CCI_Period)
    first = math.max(indicator["MA"].DATA:first(), indicator["CCI"].DATA:first()) + 1
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if Source[Price][period] > indicator["MA"].DATA[period] and core.crossesOver(indicator["CCI"].DATA, Buy, period) then
    ...
    if Source[Price][period] < indicator["MA"].DATA[period] and core.crossesUnder(indicator["CCI"].DATA, Sell, period) then
        if Direction == "direct" then
            SELL()
        else
            BUY()
    ...
    if
        core.crossesOver(Source[Price], indicator["MA"].DATA, period) or
    ...
    if
        core.crossesUnder(Source[Price], indicator["MA"].DATA, period) or
