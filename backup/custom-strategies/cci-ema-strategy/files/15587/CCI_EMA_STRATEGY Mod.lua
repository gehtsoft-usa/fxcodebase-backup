...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=6919
...
function Init() --The strategy profile initialization
    strategy:name("CCI EMA STRATEGY")
    ...
    strategy.parameters:addInteger("Buy", "CCI Buy Entry Level", "", -100)
    strategy.parameters:addInteger("Sell", "CCI Sell Entry Level", "", 100)
    ...
    strategy.parameters:addString("MATF", "Time frame", "", "H1")
    ...
    strategy.parameters:addInteger("MA_Period", "MA_Period", "", 200)
...
function Prepare(nameOnly)
    ...
    indicator["MA"] = core.indicators:create(Method, MASource[Price], MA_Period)
    indicator["CCI"] = core.indicators:create("CCI", Source, CCI_Period)
    first = indicator["CCI"].DATA:first() + 1
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if indicator["MA"].DATA:size() <= indicator["MA"].DATA:first() + 2 then
        return
    end
    ...
    if
        Source[Price][period] > indicator["MA"].DATA[indicator["MA"].DATA:size() - 1] and
            core.crossesOver(indicator["CCI"].DATA, Buy, period)
     then
    ...
    if
        Source[Price][period] < indicator["MA"].DATA[indicator["MA"].DATA:size() - 1] and
            core.crossesUnder(indicator["CCI"].DATA, Sell, period)
     then
        if Direction == "direct" then
            SELL()
        else
    ...
    if
        (Source[Price][period] > indicator["MA"].DATA[indicator["MA"].DATA:size() - 1] and
            Source[Price][period] < indicator["MA"].DATA[indicator["MA"].DATA:size() - 2]) or
            core.crossesOver(indicator["CCI"].DATA, Buy, period)
     then
        if Direction == "direct" then
