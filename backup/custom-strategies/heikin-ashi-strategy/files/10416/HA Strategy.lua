...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if HA.open[period] < HA.close[period] and HA.open[period - 1] > HA.close[period - 1] then
        LAST["PRICE"] = HA.open[period]
        LAST["SIDE"] = "LONG"
        LAST["ON"] = "OFF"
    elseif HA.open[period] > HA.close[period] and HA.open[period - 1] < HA.close[period - 1] then
        LAST["PRICE"] = HA.open[period]
        LAST["SIDE"] = "SHORT"
        LAST["ON"] = "OFF"
    end
    ...
    if Trigger and Level ~= 0.0 and LAST["ON"] == "OFF" then
        if Mode == "HA" then
            if Relativ == "PIP" then
                if LAST["SIDE"] == "LONG" and (HA.close[period] - LAST["PRICE"]) / gSource:pipSize() > Level then
                    BuySignal = true
                    LAST["ON"] = "ON"
                end
                if LAST["SIDE"] == "SHORT" and ((HA.close[period] - LAST["PRICE"]) / gSource:pipSize() < -Level) then
                    SellSignal = true
                    LAST["ON"] = "ON"
                end
            elseif Relativ == "Percentage" then
                if LAST["SIDE"] == "LONG" and (HA.close[period] - LAST["PRICE"]) / (LAST["PRICE"] / 100) > Level then
                    BuySignal = true
                    LAST["ON"] = "ON"
                end
                if LAST["SIDE"] == "SHORT" and (HA.close[period] - LAST["PRICE"]) / (LAST["PRICE"] / 100) < -Level then
                    SellSignal = true
                    LAST["ON"] = "ON"
                end
            end
        elseif Mode == "PRICE" then
            if Relativ == "PIP" then
                if LAST["SIDE"] == "LONG" and (Source.close[period] - LAST["PRICE"]) / gSource:pipSize() > Level then
                    BuySignal = true
                    LAST["ON"] = "ON"
                end
                if LAST["SIDE"] == "SHORT" and (Source.close[period] - LAST["PRICE"]) / gSource:pipSize() < -Level then
                    SellSignal = true
                    LAST["ON"] = "ON"
                end
            elseif Relativ == "Percentage" then
