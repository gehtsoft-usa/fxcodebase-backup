...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=64228
...
function Init() --The strategy profile initialization
    strategy:name("BB-Percentages-Bandwidth STRATEGY")
    ...
    strategy.parameters:addDouble("SL", "Sell Level", "", 100)
...
function Prepare(nameOnly)
    ...
    Percentages = core.indicators:create("BB-PERCENTAGES", Source.close, Period, Deviation)
    Bandwidth = core.indicators:create("BB-BANDWIDTH", Source.close, Period, Deviation)
    first = Percentages.DATA:first()
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    -- update indicators.
    Bandwidth:update(core.UpdateLast)
    Percentages:update(core.UpdateLast)
...
function EntryFunction(period, now)
    ...
    if Bandwidth.DATA[period] < Bandwidth.DATA[period - 1] then
        return Return
    end
    ...
    if Percentages.DATA[period] > BL and Percentages.DATA[period - 1] <= BL then
        if Direction then
            BUY()
        else
            SELL()
        ...
        Return = true
    elseif Percentages.DATA[period] < SL and Percentages.DATA[period - 1] >= SL then
        if Direction then
            SELL()
        else
            BUY()
