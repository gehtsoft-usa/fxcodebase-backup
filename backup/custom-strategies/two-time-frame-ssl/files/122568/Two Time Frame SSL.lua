...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=67065
...
function Init() --The strategy profile initialization
    strategy:name("Two Time Frame SSL");
	...
	strategy.parameters:addGroup("1. SSL Calculation");
	
	strategy.parameters:addBoolean("Use1", "Use 1. Time Frame ", "", true);   
	strategy.parameters:addBoolean("Cross1", "Cross Only ", "", true); 
	strategy.parameters:addString("TF1", "Time frame", "", "H1");
    strategy.parameters:setFlag("TF1", core.FLAG_PERIODS);
	
	
	strategy.parameters:addInteger("Period1", "Period", "", 10);
	
	strategy.parameters:addGroup("2. SSL Calculation");
	...
	strategy.parameters:addInteger("Period2", "Period", "", 10);
...
function CreateTradingParameters()
  ...
  --  strategy.parameters:addStringAlternative("ExitExecutionType", "End of Turn", "", "EndOfTurn");
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "TTFSSL");
...
function Prepare( nameOnly)
    ...
    Indicator1 = core.indicators:create("SSL", Source1, Period1 ,  core.rgb(0, 255, 0),  core.rgb(255, 0, 0)  );
	Indicator2 = core.indicators:create("SSL", Source2, Period2,  core.rgb(0, 255, 0),  core.rgb(255, 0, 0)  );
...
function EntryFunction( now,period1,period2)
    ...
    if ( ((Use1 and  Indicator1.DATA:colorI(period1) ==  core.rgb(0, 255, 0))
	and ((Cross1 and  Indicator1.DATA:colorI(period1-1) ~=  core.rgb(0, 255, 0))
	or not Cross1)		
	or not Use1)
	
	and  ((Use2 and  Indicator2.DATA:colorI(period2) ==  core.rgb(0, 255, 0))
	and ((Cross2 and  Indicator2.DATA:colorI(period2-1) ~=  core.rgb(0, 255, 0))
	or not Cross2)	
	) or not Use2)
	then	
          if Direction then
		...
		Return=true;
    elseif ( ((Use1 and  Indicator1.DATA:colorI(period1) ==  core.rgb(255, 0, 0))
	and ((Cross1 and  Indicator1.DATA:colorI(period1-1) ~=  core.rgb(255, 0, 0))
	or not Cross1)		
	or not Use1)
	
	and  ((Use2 and  Indicator2.DATA:colorI(period2) ==  core.rgb(255, 0, 0))
	and ((Cross2 and  Indicator2.DATA:colorI(period2-1) ~=  core.rgb(255, 0, 0))
	or not Cross2)	
	) or not Use2)
	then
            if Direction then
