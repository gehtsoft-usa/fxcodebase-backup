...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=63839
...
function CreateTradingParameters()
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "MPCTS");
	
	strategy.parameters:addInteger("MaxNumberOfPositionInAnyDirection", "Max Number Of Open Position In Any Direction", "", 2, 1, 10000);
	 ...
	 if period < first  then
        return;
		    ...
		    if CrossType == "Cross" then		
					...
					if  (core.crossesUnder(Source[PIN], indicator[1].DATA,  period) and Confirmation~="Two")
					or (core.crossesUnder(Source[PIN], indicator[2].DATA,  period) and Confirmation~="One")
					...
					elseif  (core.crossesOver(Source[PIN], indicator[1].DATA,  period) and Confirmation~="Two")
					or (core.crossesOver(Source[PIN], indicator[2].DATA,  period) and Confirmation~="One")
			...
			elseif CrossType =="Touch" then						     				 
					       if     (Source.high[period] > indicator[1].DATA[period] and Source.low[period] < indicator[1].DATA[period]	and Confirmation~="Two")
                           or  (Source.high[period] > indicator[2].DATA[period] and Source.low[period] < indicator[2].DATA[period]	and Confirmation~="One")						   
						   then	

						        if  ( Confirmation~="Two" and Source.close[period-1] < indicator[1].DATA[period-1])
								or   ( Confirmation~="One" and Source.close[period-1] < indicator[2].DATA[period-1])
								then
										if Direction then
											BUY();
										else
