...
function Prepare(nameOnly)
    ...
    Indicators[1] = core.indicators:create("ICH", Source, Parameters[1], Parameters[2], Parameters[3])

    first =
        math.max(
        Indicators[1]:getStream(0):first(),
        Indicators[1]:getStream(1):first(),
        Indicators[1]:getStream(2):first(),
        Indicators[1]:getStream(3):first(),
        Indicators[1]:getStream(4):first()
    )
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if
        Indicators[1].SL[period] > Indicators[1].TL[period] and
            Indicators[1].CS[period - Parameters[2]] > source.close[period - Parameters[2]] and
            Indicators[1].SL[period] > math.max(Indicators[1].SA[period], Indicators[1].SB[period]) and
            Indicators[1].SL[period] > math.max(Indicators[1].SA[period], Indicators[1].SB[period]) and
            TREND ~= "Buy"
     then
        TREND = "Buy"
    ...
    elseif
        Indicators[1].SL[period] < Indicators[1].TL[period] and
            Indicators[1].CS[period - Parameters[2]] < source.close[period - Parameters[2]] and
            Indicators[1].SL[period] < math.min(Indicators[1].SA[period], Indicators[1].SB[period]) and
            Indicators[1].SL[period] < math.min(Indicators[1].SA[period], Indicators[1].SB[period]) and
            TREND ~= "Sell"
     then
        TREND = "Sell"
