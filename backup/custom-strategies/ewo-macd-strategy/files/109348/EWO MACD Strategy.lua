...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=64156
...
function Init() --The strategy profile initialization
    strategy:name("EWO MACD Strategy")
    ...
    strategy.parameters:addGroup("EWO Calculation")
    strategy.parameters:addInteger("FastN", "Fast MA Period", "", 5, 2, 1000)
    strategy.parameters:addInteger("SlowN", "Slow MA Period", "", 35, 2, 1000)

    strategy.parameters:addString("EWO_Source", "Price Source", "", "M2")
    strategy.parameters:addStringAlternative("EWO_Source", "Median", "", "M2")
    strategy.parameters:addStringAlternative("EWO_Source", "Typical", "", "M3")
    strategy.parameters:addStringAlternative("EWO_Source", "Close", "", "C")
    ...
    strategy.parameters:addInteger("P1", "Fast MA Period", "", 12, 2, 1000)
    strategy.parameters:addInteger("P2", "Slow MA Period", "", 26, 2, 1000)
    strategy.parameters:addInteger("P3", "Signal MA Period", "", 9, 2, 1000)
...
local SendEmail
local BaseSize
local EntryExecutionType, ExitExecutionType
...
local FastN, SlowN, EWO_Source, Method, EWO
local P1, P2, P3, MACD
function Prepare(nameOnly)
    ...
    EWO_Source = instance.parameters.EWO_Source
    ...
    EWO = core.indicators:create("EWO", Source, FastN, SlowN, EWO_Source, Method)
    MACD = core.indicators:create("MACD", Source.close, P1, P2, P3)
    first = math.max(EWO.DATA:first(), MACD.HISTOGRAM:first())
...
function EntryFunction(period, now)
	...
	and EWO's histogram (current -2) > (current-1)
	and (current -1) < (current).
    ...
    if
        MACD.HISTOGRAM[period] > 0 and EWO.DATA[period] > EWO.DATA[period - 1] and
            EWO.DATA[period - 1] < EWO.DATA[period - 2]
     then
        if Direction then
            BUY()
        else
    ...
    if Hedge then
        if haveTrades("S") or haveTrades("B") then
            HEDGE()
            Signal("Hedge")
        end
