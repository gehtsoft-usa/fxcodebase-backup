...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=67423
...
local STRATEGY_NAME = "ZZ EMA TRIAGLE AND WEDGE STRATEGY";
...
function CreateParameters() 
    strategy.parameters:addGroup("ZIG ZAG");
    ...
    strategy.parameters:addGroup("EMA");
    strategy.parameters:addInteger("ema_period", "Period", "", 7);

    strategy.parameters:addGroup("Triangle");
    strategy.parameters:addString("k1", "Triangle", "Triangle  Y/N" , "Yes");
    strategy.parameters:addStringAlternative("k1", "Yes", "Triangle Yes" , "Yes");
	strategy.parameters:addStringAlternative("k1", "No", "Triangle No", "No");
	strategy.parameters:addString("k2", "Expanding Triangle", "Expanding Triangle  Y/N" , "No");
    strategy.parameters:addStringAlternative("k2", "Yes", "Expanding Triangle Yes" , "Yes");
	strategy.parameters:addStringAlternative("k2", "No", "Expanding Triangle No" , "No");
	strategy.parameters:addString("k5", "Wedges", "Up Wedges  Y/N" , "No");
    strategy.parameters:addStringAlternative("k5", "Yes", "Wedges  Yes" , "Yes");
	strategy.parameters:addStringAlternative("k5", "No", "Wedges  No" , "No");
	strategy.parameters:addString("k6", "Down Wedges", "Wedges  Y/N" , "No");
    strategy.parameters:addStringAlternative("k6", "Yes", "Wedges  Yes" , "Yes");
	strategy.parameters:addStringAlternative("k6", "No", "Wedges  No" , "No");
	strategy.parameters:addInteger("x", "Max. Base / Side Ratio", "Max. Base / Side Ratio", 5);
	strategy.parameters:addInteger("y", "Max. Side / Side Ratio", "Max. Side / Side Ratio", 5);
    strategy.parameters:addInteger("frame", "Frame Size", "Frame Size", 50);
end
...
function CreateIndicators(source) 
    zz = core.indicators:create("ZIGZAG", source, instance.parameters.Depth,
        instance.parameters.Deviation, instance.parameters.Backstep);
    ema = core.indicators:create("EMA", source, instance.parameters.ema_period);
    local indi_profile = EnsureIndicatorInstalled("TRIANGLE");
    local p = indi_profile:parameters();
    p:setString("k1", instance.parameters.k1)
    p:setString("k2", instance.parameters.k2)
    p:setString("k5", instance.parameters.k5)
    p:setString("k6", instance.parameters.k6)

    p:setInteger("x", instance.parameters.x)
    p:setInteger("y", instance.parameters.y)
    p:setInteger("frame", instance.parameters.frame)
    p:setBoolean("strategy_output", true)
    triangle = core.indicators:create("TRIANGLE", source, p);
end

function UpdateIndicators()
    triangle:update(core.UpdateLast);
...
function CreateCustomActions()
    ...
    enterLongAction.IsPass = function (source, period, periodFromLast, data)
        local crest_index, val = GetNextCrest(zz, period);
        local _, val2 = GetNextCrest(zz, crest_index);
        return source.close[period] > ema.DATA[period]
            and val < val2
            and core.crossesOver(source.close, triangle.OUTPUT_UP, period);
    end
    ...
    enterShortAction.IsPass = function (source, period, periodFromLast, data)
        ...
        return source.close[period] < ema.DATA[period]
            and val > val2
            and core.crossesUnder(source.close, triangle.OUTPUT_DOWN, period);
    end
