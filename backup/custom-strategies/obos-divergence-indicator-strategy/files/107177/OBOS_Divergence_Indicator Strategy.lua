...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=63680
...
function Init() --The strategy profile initialization
    strategy:name("OBOS_Divergence_Indicator Strategy");
    ...
    strategy.parameters:addInteger("period1", "Primary OBOS period", "", 10,1,9999);
    strategy.parameters:addInteger("period2", "Secondary OBOS period", "", 15,1,9999);
    strategy.parameters:addInteger("obLevel", "Over-bought level", "", 100);
    strategy.parameters:addInteger("osLevel", "Over-sold level", "", -100);

    strategy.parameters:addInteger("window", "Window", "", 10, 1, 1000);
    strategy.parameters:addInteger("leftStrength", "Left strength", "", 1);
    strategy.parameters:addInteger("rightStrength", "Right strength", "", 1);

    strategy.parameters:addBoolean("zeroFilter", "Zero filter", "", true);
    strategy.parameters:addBoolean("redFilter", "Red-line filter", "", true);
    strategy.parameters:addBoolean("blueFilter", "Blue-line filter", "", true);

    strategy.parameters:addInteger("duration", "Trade duration", "", 10, 1, 1000);
...
function CreateTradingParameters()
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "OBOSDIS");
...
local first;
local period1, period2, obLevel, osLevel,window,leftStrength,rightStrength,zeroFilter,redFilter,blueFilter,duration;
local Indicator;
...
function Prepare( nameOnly)
   CustomID = instance.parameters.CustomID;
	...
	zeroFilter = instance.parameters.zeroFilter;
	redFilter = instance.parameters.redFilter;
	blueFilter = instance.parameters.blueFilter;
    ...
    assert(core.indicators:findIndicator("OBOS_DIVERGENCE_INDICATOR_SIGNAL") ~= nil, "Please, download and install OBOS_DIVERGENCE_INDICATOR_SIGNAL indicator");
    ...
    assert(core.indicators:findIndicator( "OBOS_DIVERGENCE_INDICATOR_SIGNAL") ~= nil,  "OBOS_DIVERGENCE_INDICATOR_SIGNAL" .. " indicator must be installed");
    Indicator = core.indicators:create( "OBOS_DIVERGENCE_INDICATOR_SIGNAL", Source,
	period1, period2, obLevel, osLevel,window,leftStrength,rightStrength,zeroFilter,redFilter,blueFilter,duration); 
...
function ExtUpdate(id, source, period)  -- The method called every time when a new bid or ask price appears.
   ...
   -- get only time
    ...
    if   Indicator.Signal[period] ==  1
	and    Indicator.Signal[period-1] ~=  1
	then	
          if Direction then
                BUY();
            else
    ...
    elseif   Indicator.Signal[period] ==  -1
	and    Indicator.Signal[period-1] ~=  -1
	then
            if Direction then
                SELL();
