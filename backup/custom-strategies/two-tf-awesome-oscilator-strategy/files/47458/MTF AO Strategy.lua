...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=27217
...
function Init() --The strategy profile initialization
    ...
    local i
    for i = 1, 2, 1 do
        strategy.parameters:addBoolean("ON" .. i, i .. ".  Time Frame Color Exit", "", true)
    end
...
function Parameters(id, TF)
    ...
    strategy.parameters:addString("SP" .. id, "Short MA Period", "", 5)
    strategy.parameters:addString("LP" .. id, "Long MA Period", "", 35)

    strategy.parameters:addDouble("BL" .. id, "Buy Level", "", 0)
    strategy.parameters:addDouble("SL" .. id, "Sell Level", "", 0)
end
...
function Prepare(nameOnly)
    ...
    for i = 1, 2, 1 do
        ...
        SP[i] = instance.parameters:getInteger("SP" .. i)
        LP[i] = instance.parameters:getInteger("LP" .. i)
    ...
    for i = 1, 2, 1 do
        ...
        Indicator[i] = core.indicators:create("AO", Source[i], SP[i], LP[i], core.rgb(0, 255, 0), core.rgb(255, 0, 0))
    end
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if
        Indicator[1].DATA:size() <= Indicator[1].DATA:first() + 2 or
            Indicator[2].DATA:size() <= Indicator[2].DATA:first() + 2
     then
        return
    end
    ...
    if
        Indicator[1].DATA[Indicator[1].DATA:size() - 2] < BL[1] and
            Indicator[1].DATA:colorI(Indicator[1].DATA:size() - 2) == core.rgb(0, 255, 0) and
            Indicator[1].DATA:colorI(Indicator[1].DATA:size() - 3) ~= core.rgb(0, 255, 0) and
            ((Indicator[2].DATA[Indicator[2].DATA:size() - 2] > BL[2]) or not USE[2]) and
            ((Indicator[2].DATA:colorI(Indicator[2].DATA:size() - 2) == core.rgb(0, 255, 0)) or not USE[2])
     then
        if Direction then
            BUY()
        else
    ...
    elseif
        Indicator[1].DATA[Indicator[1].DATA:size() - 2] > SL[1] and
            Indicator[1].DATA:colorI(Indicator[1].DATA:size() - 2) == core.rgb(255, 0, 0) and
            Indicator[1].DATA:colorI(Indicator[1].DATA:size() - 3) ~= core.rgb(255, 0, 0) and
            ((Indicator[2].DATA[Indicator[2].DATA:size() - 2] < SL[2]) or not USE[2]) and
            ((Indicator[2].DATA:colorI(Indicator[2].DATA:size() - 2) == core.rgb(255, 0, 0)) or not USE[2])
     then
        if Direction then
            SELL()
        else
    ...
    if EXIT then
        if
            ((Indicator[1].DATA:colorI(Indicator[1].DATA:size() - 2) ~= core.rgb(0, 255, 0)) and ON[1]) or
                ((Indicator[2].DATA:colorI(Indicator[2].DATA:size() - 2) ~= core.rgb(0, 255, 0)) and ON[2])
         then
            if Direction then
        ...
        if
            ((Indicator[1].DATA:colorI(Indicator[1].DATA:size() - 2) ~= core.rgb(255, 0, 0)) and ON[1]) or
                ((Indicator[2].DATA:colorI(Indicator[2].DATA:size() - 2) ~= core.rgb(255, 0, 0)) and ON[2])
         then
            if Direction then
                if haveTrades("S") then
                    exit("S")
