...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=59451
...
function Init() --The strategy profile initialization
    strategy:name("Mohindar Oscillator Strategy");
    ...
    strategy.parameters:addInteger("Period1", "1. Smoothing Period", "Period", 30);
	strategy.parameters:addInteger("Period2", "2. Smoothing Period", "Period", 30);
	
	
	
    strategy.parameters:addGroup("Strategy Parameters");
    
	strategy.parameters:addString("Method", "Signal Method", "Method" , "RMO/1.Signal");
    strategy.parameters:addStringAlternative("Method", "RMO/1.Signal Cross", "RMO/1.Signal" , "RMO/1.Signal");
    strategy.parameters:addStringAlternative("Method", "RMO/2.Signal Cross", "RMO/2.Signal" , "RMO/2.Signal");
    strategy.parameters:addStringAlternative("Method", "1.Signal/2.Signal Cross", "1.Signal/2.Signal" , "1.Signal/2.Signal");
	strategy.parameters:addStringAlternative("Method", "Any", "Any" , "Any");
...
function Prepare(nameOnly)
    ...
    local i;
	 	...
	 	assert(core.indicators:findIndicator("RMO") ~= nil, "Please, download and install RMO.LUA indicator");          
    ...
    Indicator[1] = core.indicators:create("RMO", Source[Price],Period1, Period2  );
...
function ExtUpdate(id, source, period)  -- The method called every time when a new bid or ask price appears.
   ...
   -- get only time
	...
	--"RMO/1.Signal"   "RMO/2.Signal" "1.Signal/2.Signal"  "Any"
	
        if   (core.crossesOver(Short[1], Short[2],period) and  (Method== "RMO/1.Signal" or Method== "Any" ) )		
		then
			...
			Signal ("RMO/1.Signal CrossOver");
			end
			
		end
		
		if  (core.crossesUnder(Short[1], Short[2],period) and (Method== "RMO/1.Signal" or Method== "Any" ) )		
		 then
		    ...
		    if AllowTrade then
			 ...
			 Signal ("RMO/1.Signal CrossUnder");
		...
		if (core.crossesOver(Short[1], Short[3],period)and (Method== "RMO/2.Signal"  or   Method== "Any" )) then
		     if AllowTrade then
					 if Direction then
						BUY();
					else
		...
		if (core.crossesOver(Short[1], Short[3],period)and (Method== "RMO/2.Signal"  or Method== "Any" ) ) then
		    if AllowTrade then
			 ...
			 Signal ("RMO/2.Signal CrossUnder");
             end			 
		end
		
		
		  if (core.crossesOver(Short[2], Short[3],period)and (Method==  "1.Signal/2.Signal"  or Method== "Any" ) ) then
				...
				Signal ("1.Signal/2.Signal CrossOver");
			...
			if (core.crossesOver(Short[2], Short[3],period)and (Method==  "1.Signal/2.Signal"  or Method== "Any" ) ) then
			  ...
			  Signal ("1.Signal/2.Signal CrossUnder");
              end			  
			end
