...
--http://fxcodebase.com/code/viewtopic.php?f=31&t=69497
...
local STRATEGY_NAME = "Gimelfarb TRsV Strategy";
function CreateParameters() 
    strategy.parameters:addInteger("length", "Length", "", 10);
    strategy.parameters:addDouble("Threshold", "Threshold", "", 1.5);
end

local trsv, bop, Threshold;
function CreateEntryIndicators(source)
    local profile = core.indicators:findIndicator("TRSV VOLUME");
    assert(profile ~= nil, "Please, download and install " .. "TRSV VOLUME" .. ".LUA indicator");
    local indicatorParams = profile:parameters();
    trsv = core.indicators:create("TRSV VOLUME", source, instance.parameters.length)
    Threshold = instance.parameters.Threshold;

    local profile = core.indicators:findIndicator("BULL-BEAR BALANCE OF POWER");
    assert(profile ~= nil, "Please, download and install " .. "BULL-BEAR BALANCE OF POWER" .. ".LUA indicator");
    local indicatorParams = profile:parameters();
    bop = core.indicators:create("BULL-BEAR BALANCE OF POWER", source, indicatorParams)
end

function UpdateIndicators()
    trsv:update(core.UpdateLast);
    bop:update(core.UpdateLast);
end

function IsEntryLong(source, period)
    if period < 1 or not trsv.DATA:hasData(period - 1) then
        return;
    end
    return trsv.DATA[period] / trsv.DATA[period - 1] > Threshold and bop.DATA[period] < 1;
end
...
function OpenTrade(side)
    ...
    if side == "B" then
        valuemap.Rate = main_source.high[NOW - 1];
    else
        valuemap.Rate = main_source.low[NOW - 1];
    end
