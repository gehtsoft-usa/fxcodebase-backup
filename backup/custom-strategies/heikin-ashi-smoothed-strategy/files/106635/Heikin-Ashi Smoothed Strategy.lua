...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=63566
...
function Init() --The strategy profile initialization
    strategy:name("Heikin-Ashi Smoothed Strategy")
...
function Prepare(nameOnly)
    ...
    HA = core.indicators:create("HASM", Source, Method1, N1, Method2, N2, core.rgb(0, 255, 0), core.rgb(255, 0, 0))
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if HA.DATA:colorI(period) == core.rgb(0, 255, 0) and HA.DATA:colorI(period - 1) ~= core.rgb(0, 255, 0) then
        if Direction then
            BUY()
        else
            SELL()
    ...
    elseif HA.DATA:colorI(period) == core.rgb(255, 0, 0) and HA.DATA:colorI(period - 1) ~= core.rgb(255, 0, 0) then
        if Direction then
            SELL()
        else
            BUY()
    ...
    if Exit then
        if HA.DATA:colorI(period) ~= core.rgb(0, 255, 0) then
            if Direction then
        ...
        if HA.DATA:colorI(period) ~= core.rgb(255, 0, 0) then
            if Direction then
                if haveTrades("S") then
                    exitSpecific("S")
                    Signal("Close Short")
