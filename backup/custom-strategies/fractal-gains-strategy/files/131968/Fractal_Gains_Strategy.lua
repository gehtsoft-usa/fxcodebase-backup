...
local FirstStart, close_on_opposite
    ...
    if (id == 5 and FirstStart) or id == 3 then
        ...
        if UpFrValue ~= nil and DnFrValue ~= nil then
            ...
            if DO.Buff1[_period - 1] < DO.Buff2[_period - 1] and DO.Buff1[_period] > DO.Buff2[_period] then
                ...
                if FractalPrice > instance.ask[NOW] + core.host:execute("getTradingProperty", "conditionalDistanceEntryStop", instance.bid:instrument(), Account) then
                    signaler:SendCommand("action=open side=B");
                    signaler:Signal("Place entry BUY to " .. FractalPrice, source);
                    if AllowTrade then
                        if close_on_opposite then
                            trading:FindTrade()
                                :WhenInstrument(instance.ask:instrument())
                                :WhenSide("S")
                                :WhenAccountID(Account)
                                :WhenCustomID(CID)
                                :Do(function (trade)
                                    trading:Close(trade);
                                end);
            ...
            if DO.Buff2[_period - 1] < DO.Buff1[_period - 1] and DO.Buff2[_period] > DO.Buff1[_period] then
                ...
                if FractalPrice < instance.bid[NOW] - core.host:execute("getTradingProperty", "conditionalDistanceEntryStop", instance.bid:instrument(), Account) then
