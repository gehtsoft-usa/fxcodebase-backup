...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=23859
...
function Init() --The strategy profile initialization
    strategy:name("Tick DMI Strategy");
    ...
    strategy.parameters:addInteger("Period1", "DMI Period", "Period", 14, 2 , 1000);
	 strategy.parameters:addInteger("Period2", "ADX Period", "Period", 14, 2 , 1000);
	  strategy.parameters:addBoolean("Use", "Use Adx Filter", "", false);
	  strategy.parameters:addDouble("Level", "ADX Level", "Level", 20, 0 , 100);
...
function Prepare(nameOnly)
    ...
    local i;
	   ...
	   name = name .. ", " ..  Period1.. ", " ..  Period2;
	 	...
	 	assert(core.indicators:findIndicator("TICK DMIADX") ~= nil, "Please, download and install TICK DMIADX.LUA indicator");          
    ...
    Indicator[1] = core.indicators:create("TICK DMIADX", Source[Price], Period  );
...
function ExtUpdate(id, source, period)  -- The method called every time when a new bid or ask price appears.
   ...
   -- get only time
    ...
    if period < first +1  then
		...
		and ((Short[3][period] > Level) or not Use )
		then
