-- Available @ https://fxcodebase.com/code/viewtopic.php?f=31&t=74858
...
function Init() --The strategy profile initialization
    strategy:name("Two Moving Average Cross Strategy");
    ...
    strategy.parameters:addInteger("Period1", "1. Period", "", 10);  
    strategy.parameters:addInteger("Period2", "2. Period", "", 20);  	
	
	
	
    strategy.parameters:addBoolean("Forecast", "Forecast", "", true);   
    strategy.parameters:addInteger("Forecasted", "Forecasted Cross", "", 5);  	
...
function CreateTradingParameters()
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "TMAS");
...
function Prepare( nameOnly)
	...
	--Indicator parameters
    Forecast= instance.parameters.Forecast;
    Forecasted= instance.parameters.Forecasted;
...
function EntryFunction( id, now,period)
    ...
    local ForecastedPeriod=ForecastFunction(period);
	  
    -- only buy if we have a fast cross over slow and the price is above the moving averages.
    if  ( not Forecast and Indicator1.DATA[period] > Indicator2.DATA[period] and  Indicator1.DATA[period-1] <= Indicator2.DATA[period-1] )	or (Forecast and ForecastedPeriod > 0  and math.abs(ForecastedPeriod) < Forecasted)
	then	
          if Direction then
                BUY();
            else
		...
		Return=true;
    elseif ( not Forecast and Indicator1.DATA[period] < Indicator2.DATA[period] and  Indicator1.DATA[period-1] >= Indicator2.DATA[period-1] )	or (Forecast and ForecastedPeriod < 0  and math.abs(ForecastedPeriod) < Forecasted)
	then
            if Direction then
                SELL();
            else
