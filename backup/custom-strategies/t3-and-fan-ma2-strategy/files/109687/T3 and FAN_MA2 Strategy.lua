...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=64196
...
function Init() --The strategy profile initialization
    strategy:name("T3 and FAN_MA2 Strategy")
    ...
    strategy.parameters:addDouble("VF", "Volume Factor", "Volume Factor", 0.618, 0, 1)
    strategy.parameters:addInteger("F", "Period", "Period", 38, 2, 2000)

    strategy.parameters:addGroup("FAN MA Calculation")
    strategy.parameters:addInteger("Period_1", "First period", "", 5)
    ...
    strategy.parameters:addInteger("N", "Count of MA", "", 5)

    strategy.parameters:addInteger("CountOf", "Count of Bars To Trade", "", 3)
...
local T3, VF, F
local FAN_MA2, Period_1, Method, K, N, CountOf
local name
function Prepare(nameOnly)
    ...
    Period_1 = instance.parameters.Period_1
    Method = instance.parameters.Method
    K = instance.parameters.K
    N = instance.parameters.N
    CountOf = instance.parameters.CountOf
    ...
    assert(core.indicators:findIndicator("FAN_MA2") ~= nil, "Please, download and install FAN_MA2.LUA indicator")
    ...
    T3 = core.indicators:create("T3", Source.close, VF, F)
    FAN_MA2 = core.indicators:create("FAN_MA2", Source.close, Period_1, Method, K, N)

    first = math.max(T3.DATA:first(), FAN_MA2.DATA:first())
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    FAN_MA2:update(core.UpdateLast)

    if not T3.DATA:hasData(period) or not FAN_MA2.DATA:hasData(period) then
        return
    end
...
function EntryFunction(period, now)
    ...
    if Source.close[period] > T3.DATA[period] and ItIs(period, 1) then
        if Direction then
            BUY()
        else
            SELL()
        ...
        Return = true
    elseif Source.close[period] < T3.DATA[period] and ItIs(period, -1) then
        if Direction then
            SELL()
        else
            BUY()
...
function ItIs(period, Flag)
    --CountOf

    local ReturnFlag = true

    for i = 0, CountOf - 1, 1 do
        if Flag == 1 then
            if FAN_MA2.DATA[period - i] < 0 then
                ReturnFlag = false
                break
            end
        elseif Flag == -1 then
    ...
    if Flag == 1 then
        if FAN_MA2.DATA[period - CountOf] > 0 then
            ReturnFlag = false
        end
    elseif Flag == -1 then
        if FAN_MA2.DATA[period - CountOf] < 0 then
