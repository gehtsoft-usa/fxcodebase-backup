...
--https://fxcodebase.com/code/viewtopic.php?f=31&t=71744
...
function CreateParameters() 
    strategy.parameters:addInteger("ma1_period", "MA 1 Period", "", 7);
...
function IsEntryLong(source, period)
    ...
    return source.close[period] < source.open[period]
        ...
        or ma2.DATA[period] < source.close[period]);
end
...
function IsExitLong(source, period)
    return ma1.DATA[period] > source.close[period]
        and ma2.DATA[period] > source.close[period];
end
...
function OpenTrade(side, period)
    ...
    if side == "S" then
        valuemap.Rate = main_source.high[period]
    else
        valuemap.Rate = main_source.low[period]
    end
