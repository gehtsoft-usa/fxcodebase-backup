...
function CreateParameters() 
    ...
    strategy.parameters:addDouble("BuyLine", "Buy Line", "", 0.005);
    strategy.parameters:addDouble("SellLine", "Sell Line", "", -0.005);
end

local ift, BuyLine, SellLine;
function CreateEntryIndicators(source)
    BuyLine = instance.parameters.BuyLine;
    SellLine = instance.parameters.SellLine;
    local profile = core.indicators:findIndicator("INVFISHER-CYBERCYCLE");
    assert(profile ~= nil, "Please, download and install " .. "INVFISHER-CYBERCYCLE" .. ".LUA indicator");
    local indicatorParams = profile:parameters();
    ift = core.indicators:create("INVFISHER-CYBERCYCLE", source, instance.parameters.Alpha);
end
