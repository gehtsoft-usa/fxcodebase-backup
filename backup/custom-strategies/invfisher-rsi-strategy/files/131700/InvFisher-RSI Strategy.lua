...
function CreateParameters() 
    strategy.parameters:addDouble("Alpha", "Alpha", "", 0.1);
    strategy.parameters:addInteger("RSILength", "RSI Length", "", 5);
    strategy.parameters:addInteger("WAverageLen", "WAverageLen", "", 9);
    strategy.parameters:addDouble("BuyLine", "Buy Line", "", 0.5);
    strategy.parameters:addDouble("SellLine", "Sell Line", "", -0.5);
end

local rsi, ift, BuyLine, SellLine;
function CreateEntryIndicators(source)
    BuyLine = instance.parameters.BuyLine;
    SellLine = instance.parameters.SellLine;
    rsi = core.indicators:create("RSI", source, instance.parameters.RSILength);
    local profile = core.indicators:findIndicator("INVERSE FISHER TRANSFORM");
    assert(profile ~= nil, "Please, download and install " .. "INVERSE FISHER TRANSFORM" .. ".LUA indicator");
    local indicatorParams = profile:parameters();
    ift = core.indicators:create("INVERSE FISHER TRANSFORM", rsi.DATA, instance.parameters.WAverageLen, instance.parameters.Alpha, 50);
end

function UpdateIndicators()
    rsi:update(core.UpdateLast);
    ift:update(core.UpdateLast);
end

function IsEntryLong(source, period)
    return core.crossesOver(ift.DATA, BuyLine, period);
end
function IsEntryShort(source, period)
    return core.crossesUnder(ift.DATA, SellLine, period);
end
