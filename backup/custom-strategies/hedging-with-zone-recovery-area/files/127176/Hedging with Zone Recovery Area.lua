...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=67418
...
function Init()
    strategy:name("Hedging with Zone Recovery Area");
    strategy:description("");
    strategy:setTag("group", "Samples");
    strategy:type(core.Both);
    
    strategy.parameters:addInteger("area", "Zone recovery area", "", 35);
    strategy.parameters:addInteger("tp", "Take profit", "", 80);
    ...
    strategy.parameters:addDouble("amount_multiplicator", "Amount multiplicator", "", 2);
end

local amount_multiplicator, last_amount;
local allow_trade;
local account;
local area;
local tp;
local custom_id = "Simple Strategy";
...
function Prepare(onlyName)
    ...
    area = instance.parameters.area;
    tp = instance.parameters.tp;
    amount_multiplicator = instance.parameters.amount_multiplicator;
    last_amount = 1;
...
function ExtAsyncOperationFinished(cookie, success, message, message2, message3)
    if cookie == TRADES_UPDATE then
        ...
        if close_trade then
            ...
            if closed_trade ~= nil then
                trading:FindTrade():WhenInstrument(closed_trade.Instrument):Do(function (trade) trading:Close(trade) end);
                trading:FindOrder():WhenInstrument(closed_trade.Instrument):Do(function (order) trading:DeleteOrder(order) end);
            end
        else
            local new_trade = core.host:findTable("trades"):find("TradeID", trade_id);
            if new_trade ~= nil then
                local offer = core.host:findTable("offers"):find("Instrument", new_trade.Instrument);
                if original_rate == nil then
                    original_rate = new_trade.Open;
                    if new_trade.BS == "B" then
                        opposite_rate = original_rate - area * offer.PointSize;
                        stops["B"] = opposite_rate - tp * offer.PointSize;
                        stops["S"] = original_rate + tp * offer.PointSize;
                    else
                        opposite_rate = original_rate + area * offer.PointSize;
                        stops["S"] = opposite_rate + tp * offer.PointSize;
                        stops["B"] = original_rate - tp * offer.PointSize;
                    end
                end
                local rate;
                if math.abs(new_trade.Open - original_rate) < math.abs(new_trade.Open - opposite_rate) then
                    rate = opposite_rate;
                else
                    rate = original_rate;
                end
                local total_profit = trading:FindTrade()
                    :WhenInstrument(new_trade.Instrument)
                    :Summ(
                        function (trade) 
                            local mult = 1;
                            if new_trade.BS == trade.BS then
                                mult = -1;
                            end
                            local pl;
                            if new_trade.BS == "B" then
                                pl = (trade.Open - stops[trade.BS]) * trade.AmountK
                            else
                                pl = (stops[trade.BS] - trade.Open) * trade.AmountK
                            end
                            return pl * mult / offer.PointSize
                        end) + area;
                local order_amount = math.max(1, math.ceil(-total_profit / tp * last_amount));
                last_amount = last_amount * amount_multiplicator;
                trading:EntryOrder(new_trade.Instrument)
                    :SetSide(trading:getOppositeSide(new_trade.BS))
                    :SetRate(rate)
                    :SetPipLimit(nil, tp)
