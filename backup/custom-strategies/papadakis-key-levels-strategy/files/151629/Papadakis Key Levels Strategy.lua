-- Available @ https://fxcodebase.com/code/viewtopic.php?f=31&t=73927
...
function Init() --The strategy profile initialization
    strategy:name("Papadakis Key Levels Strategy");
    ...
    strategy.parameters:addInteger("Start", "Session Start", "", 2, 0, 23);
	
 	strategy.parameters:addGroup("Papadakis Key Lines");	 
	strategy.parameters:addDouble("Level", "Line Level", "", 1.618); 
...
function CreateTradingParameters()
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "PKLS");
...
function Prepare( nameOnly)
    ...
    Source = ExtSubscribe(2, nil, "H1", instance.parameters.Type == "Bid", "bar");
...
function EntryFunction( id, now,period)
	...
	data_and_time= sesion_data_and_time;	
 	...
 	local p=GetPeriod(data_and_time);
	if not p then
	return;
	end	
    ...
    if   Source.close[period]> High+Level*Range 
	and Source.close[period-1]<= High+Level*Range 
	then	
          if Direction then
                BUY();
            else
		...
		Return=true;
    elseif   Source.close[period]< Low-Level*Range 
	and Source.close[period-1]>= Low-Level*Range 
		then
            if Direction then
                SELL();
            else
...
function GetPeriod(data_and_time)
