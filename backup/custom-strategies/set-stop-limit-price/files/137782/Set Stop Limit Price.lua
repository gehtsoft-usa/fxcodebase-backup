...
local Modules = {};
local STRATEGY_NAME = "Set Stop Limit Price Strategy";
...
function Init()
    ...
    strategy.parameters:addDouble("StopLevel", "Stop level", "Stop level for set stop command", 0)
    strategy.parameters:setFlag("StopLevel", core.FLAG_PRICE)
    strategy.parameters:addDouble("LimitLevel", "Limit level", "Limit level for set limit command", 0)
    strategy.parameters:setFlag("LimitLevel", core.FLAG_PRICE)
end
...
function Prepare(nameOnly)
    ...
    main_source = ExtSubscribe(1, nil, instance.parameters.timeframe, false, "bar")
end
...
function ExtUpdate(id, source, period)
    ...
    trading:MoveStop(trade, instance.parameters.StopLevel)
    trading:MoveLimit(trade, instance.parameters.LimitLevel);
    core.host:execute("stop");
end
