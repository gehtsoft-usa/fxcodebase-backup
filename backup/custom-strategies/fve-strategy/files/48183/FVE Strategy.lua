...
function Init() --The strategy profile initialization
    ...
    strategy.parameters:addGroup("FVE Calculation")
    strategy.parameters:addInteger("Period1", "Period", "Period", 22)
    ...
    strategy.parameters:addGroup("FVE Smoothing Calculation")
    strategy.parameters:addInteger("Period2", "Period", "Period", 22)
    ...
    strategy.parameters:addGroup("Entry Parameters")

    strategy.parameters:addString("EntryType", "Entry Type", "", "Zero")
    strategy.parameters:addStringAlternative("EntryType", "FVE /(B/S) Level", "", "Zero")
    strategy.parameters:addStringAlternative("EntryType", "FVE / Signal", "", "Signal")
    strategy.parameters:addStringAlternative("EntryType", "FVE / Signal / Level", "", "Zone")

    strategy.parameters:addGroup("Exit Parameters")

    strategy.parameters:addString("ExitType", "Entry Type", "", "Zero")
    strategy.parameters:addStringAlternative("ExitType", "FVE /(B/S) Level", "", "Zero")
    strategy.parameters:addStringAlternative("ExitType", "FVE / Signal", "", "Signal")
...
function Prepare(nameOnly)
    ...
    name =
        name ..
        ", " .. tostring(Period1) .. ", " .. tostring(Method1) .. ", " .. tostring(Period2) .. ", " .. tostring(Method2)

    assert(core.indicators:findIndicator("FVE") ~= nil, "Please, download and install FVE.LUA indicator")
    ...
    Indicator[1] = core.indicators:create("FVE", Source, Period1, Method1, Period2, Method)
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    elseif EntryType == "Signal" then
        ...
        if core.crossesOver(Short[1], BL, period) and Short[1][period] > BL then
            if Direction then
                BUY()
            else
                SELL()
            end
        elseif core.crossesUnder(Short[1], SL, period) and Short[1][period] < SL then
            if Direction then
                SELL()
            else
                BUY()
    ...
    if ExitType == "Signal" then
        if core.crossesUnder(Short[1], Short[2], period) then
            if Direction then
    ...
    elseif ExitType == "Zero" then
        if core.crossesUnder(Short[1], BL, period) then
            if Direction then
                if haveTrades("B") then
                    exit("B")
                    Signal("Close Long")
        ...
        if core.crossesOver(Short[1], SL, period) then
            if Direction then
                if haveTrades("S") then
                    exit("S")
                    Signal("Close Short")
