...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=67301
...
-- Enable stop loss parameters?
local EnableDefaultStopLoss = false;

local STRATEGY_NAME = "Pinbar Strategy";
...
function CreateParameters()
    strategy.parameters:addDouble("entry_shift", "Entry shift, pips", "", 0);
end
function CreateStopParameters(params, id)
    strategy.parameters:addDouble("stop" .. id, "Stop, pips", "", 0);
    strategy.parameters:addBoolean("use_trailing" .. id, "Trailing stop order", "", false);
    strategy.parameters:addInteger("trailing" .. id, "Trailing in pips", "Use 1 for dynamic and 10 or greater for the fixed trailing", 1);
end
function CreateLimitParameters(params, id)
    strategy.parameters:addInteger("limit_ratio" .. id, "Risk to reward", "", 2);
    strategy.parameters:addBoolean("move_stop_each_level" .. id, "Move stop on each level", "", false);
end
...
function GetEntryRate(source, bs, period)
    if bs == "B" then
        return source.high[period] + instance.parameters.entry_shift * source:pipSize();
    else
        return source.low[period] - instance.parameters.entry_shift * source:pipSize();
    end
end

function SetCustomStop(position_desc, command, period)
    local trailing = nil;
    if instance.parameters:getBoolean("use_trailing" .. position_desc.Id) then
        trailing = instance.parameters:getInteger("trailing" .. position_desc.Id);
    end
    local barSize = (position_desc.Source.high[period] - position_desc.Source.low[period]) / position_desc.Source:pipSize();
    local stop = instance.parameters:getDouble("stop" .. position_desc.Id);
    command:SetPipStop(nil, barSize + stop, trailing);
    return true; 
end

function SetCustomLimit(position_desc, command, period)
    local limit_ratio = instance.parameters:getInteger("limit_ratio" .. position_desc.Id);
    local barSize = (position_desc.Source.high[period] - position_desc.Source.low[period]) / position_desc.Source:pipSize();
    local stop = instance.parameters:getDouble("stop" .. position_desc.Id);
    command:SetPipLimit(nil, (barSize + stop) * limit_ratio);
    return true;
end

function CreateCustomBreakeven(position_desc, result, period)
    local limit_ratio = instance.parameters:getInteger("limit_ratio" .. position_desc.Id);
    if instance.parameters:getBoolean("move_stop_each_level" .. position_desc.Id) and limit_ratio >= 2 then
        local barSize = (position_desc.Source.high[period] - position_desc.Source.low[period]) / position_desc.Source:pipSize();
        local stop = instance.parameters:getDouble("stop" .. position_desc.Id) + barSize;
        local initialStop = -stop;
        for i = 1, limit_ratio do
            local controller = breakeven:CreateController()
                :SetRequestID(result.RequestID)
                :SetWhen(stop * i)
                :SetTo(initialStop + stop * i);
        end
    end
    return true;
end
...
function CreateCustomActions()
    ...
    enterLongAction.IsPass = function (source, period, periodFromLast)
        local barCenter = (source.high[period] + source.low[period]) / 2;
        local shadow = source.open[period] - source.low[period];
        local body = source.close[period] - source.open[period];
        return source.open[period] < source.close[period]
            and barCenter < source.open[period]
            and shadow > body * 3;
    end
    ...
    enterShortAction.IsPass = function (source, period, periodFromLast)
        local barCenter = (source.high[period] + source.low[period]) / 2;
        local shadow = source.high[period] - source.open[period];
...
function Init()
    ...
    trading.AddStopParameter = SetCustomStop == nil;
...
function CreatePositionStrategy(source, side, id)
    ...
    function position_strategy:Open(period)
        ...
        local command;
        if rate == nil then
            command = trading:MarketOrder(self.Source:instrument());
        else
            command = trading:EntryOrder(self.Source:instrument())
                :SetRate(rate);
        end
...
function trading:AddPositionParameters(parameters, id)
    ...
    elseif CreateStopParameters ~= nil then
        CreateStopParameters(parameters, id);
    end
