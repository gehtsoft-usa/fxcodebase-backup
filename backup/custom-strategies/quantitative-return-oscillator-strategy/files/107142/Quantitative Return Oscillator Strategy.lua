...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=63670
...
function Init() --The strategy profile initialization
    strategy:name("Quantitative Return Oscillator Strategy")
    ...
    strategy.parameters:addInteger("Period", "Period", "Period", 5)
    strategy.parameters:addDouble("Multiplier", "Multiplier", "Multiplier", 100)

    strategy.parameters:addDouble("SL", "Sell Level", "", 0.25)
    strategy.parameters:addDouble("BL", "Buy Level", "", -0.25)
...
function Prepare(nameOnly)
    ...
    assert(
        core.indicators:findIndicator("QUANTITATIVE RETURN OSCILLATOR") ~= nil,
        "Please, download and install QUANTITATIVE RETURN OSCILLATOR.LUA indicator"
    )
    ...
    Indicator = core.indicators:create("QUANTITATIVE RETURN OSCILLATOR", Source.close, Period, Multiplier)
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if core.crosses(Indicator.DATA, BL, period) then
        if Direction then
            BUY()
        else
            SELL()
    ...
    elseif core.crosses(Indicator.DATA, SL, period) then
        if Direction then
            SELL()
        else
            BUY()
