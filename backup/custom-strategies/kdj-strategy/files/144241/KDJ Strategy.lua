...
function Init() --The strategy profile initialization
    ...
    strategy.parameters:addInteger("Period", "nPeriod", "nPeriod", 9);
    strategy.parameters:addDouble("factor1", "factor_1", "factor_1", 0.6666666);
    strategy.parameters:addDouble("factor2", "factor_2", "factor_2", 0.3333333);
...
function CreateTradingParameters()
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "KDJS");
...
function Prepare( nameOnly)
    ...
    assert(core.indicators:findIndicator("KDJ") ~= nil, "Please, download and install KDJ.LUA indicator");          
    ...
    Indicator = core.indicators:create("KDJ", Source,  instance.parameters.Period, instance.parameters.factor1 , instance.parameters.factor2   );
...
function EntryFunction( id, now,period)
    ...
    if   Indicator.buff_K[period]>  Indicator.buff_J[period]
	and   Indicator.buff_K[period-1]<=  Indicator.buff_J[period-1]
	and   Indicator.buff_K[period-1] < OS
	then	
          if Direction then
                BUY();
            else
		...
		Return=true;
    elseif Indicator.buff_K[period]<  Indicator.buff_J[period]
	and   Indicator.buff_K[period-1]>=  Indicator.buff_J[period-1]
	and   Indicator.buff_K[period-1] > OB
	then
            if Direction then
                SELL();
            else
