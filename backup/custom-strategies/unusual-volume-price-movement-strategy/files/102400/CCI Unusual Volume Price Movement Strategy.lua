...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=62668&p
...
function Init() --The strategy profile initialization
    strategy:name("CCI Unusual Volume Price Movement Strategy")
    ...
    strategy.parameters:addInteger("CCI_Period", "CCI Period", "Period", 50)
    strategy.parameters:addDouble("CCI_Buy", "CCI Buy Level", "Level", 0)
    strategy.parameters:addDouble("CCI_Sell", "CCI Sell Level", "Level", 0)
    strategy.parameters:addGroup("Volume Calculation")
    strategy.parameters:addInteger("Average_Period", "Volume Average Period", "Period", 25)
    strategy.parameters:addDouble("Volume_Multiplier", "Volume Multiplier", "Multiplier", 2)

    strategy.parameters:addGroup("Price Calculation")

    strategy.parameters:addString("Method", "Pip/Percentage", "Method", "Pips")
    strategy.parameters:addStringAlternative("Method", "Pips", "Pips", "Pips")
    strategy.parameters:addStringAlternative("Method", "Percentage", "Percentage", "Percentage")
    strategy.parameters:addInteger("Price_Period", "Price Period", "Period", 25)
    strategy.parameters:addDouble("Value", "Value", "Value", 0)
...
local first
local Average_Period, Volume_Multiplier, Method, Price_Period, Value
local Direction
local CustomID
local CCI_Buy, CCI_Sell, CCI, CCI_Period
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if period < math.max(first, Average_Period) then
        return
    end

    local Average_Volume = mathex.avg(Source.volume, period - 1 - Average_Period + 1, period - 1)

    if Source.volume[period] < Average_Volume * Volume_Multiplier then
        return
    end

    if period < first + Price_Period then
        return
    end
    ...
    if Source.close[period] > Top and CCI.DATA[period] > CCI_Buy then
        if Direction then
            BUY()
        else
            SELL()
    ...
    elseif Source.close[period] < Bottom and CCI.DATA[period] < CCI_Sell then
        if Direction then
            SELL()
        else
            BUY()
