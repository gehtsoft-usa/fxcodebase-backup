-- Available @ https://fxcodebase.com/code/viewtopic.php?f=31&t=73595
...
function Init() --The strategy profile initialization
    strategy:name("Trendilo Strategy");
    ...
    strategy.parameters:addInteger("p1", "Smoothing", "", 1, 1, 2000);
    strategy.parameters:addInteger("p2", "Lookback", "", 50, 1, 2000);
    strategy.parameters:addDouble("p3", "ALMA Offset", "", 0.85, 0, 2000);	  

    strategy.parameters:addInteger("p4", "ALMA Sigma", "", 6, 0, 2000);	
	strategy.parameters:addDouble("p5", "Band Multiplier", "", 1, 1, 2000);	

	strategy.parameters:addBoolean("p6", "Custom Band Length ? (Else same as Lookback)", "", false);	
	strategy.parameters:addInteger("p7", "Custom Band Length", "", 20, 1, 2000);
...
function CreateTradingParameters()
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "TS");
...
function Prepare( nameOnly)
	...
	assert(core.indicators:findIndicator("TRENDILO") ~= nil, "Please, download and install TRENDILO.LUA indicator");          
    ...
    Indicator = core.indicators:create("TRENDILO", Source.close, p1, p2, p3, p4, p5,p6, p7 ); 
	first=Indicator.Line:first(); 
...
function EntryFunction( id, now,period)
    ...
    if   Indicator.Line[period]>  0
	and  Indicator.Line[period-1]<=  0
	then	
          if Direction then
                BUY();
            else
		...
		Return=true;
    elseif Indicator.Line[period]<  0
	and  Indicator.Line[period-1]>=  0
		then
            if Direction then
                SELL();
