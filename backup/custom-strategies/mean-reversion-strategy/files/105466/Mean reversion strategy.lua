...
function Init() --The strategy profile initialization
	...
	strategy.parameters:addString("TF", "Time frame", "", "H1");
     ...
     strategy.parameters:addBoolean("S1", "Use Stochastic Filter", "", false);
     strategy.parameters:addBoolean("S2", "Use RSI Filter", "", false);
 
    strategy.parameters:addGroup("RSI Parameters");
    strategy.parameters:addInteger("RSIPeriod", "RSI Period", "", 14, 2, 1000);
  
   strategy.parameters:addGroup("Stochastic Parameters");
    strategy.parameters:addInteger("KPeriod", "K Period", "", 5, 2, 1000);
    strategy.parameters:addInteger("DPeriod", "D Period", "", 5, 2, 1000);
	
	strategy.parameters:addGroup("Bollinger Band Parameters");
	strategy.parameters:addInteger("BBPeriod", "Period", "", 20, 2, 1000);
	strategy.parameters:addDouble("BBDeviation", "Deviation", "", 2  );
...
function CreateTradingParameters()
    ...
    strategy.parameters:addBoolean("Reverse", "Reverse Signals", "Reverse the trading signals.", false);
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "MRS");
...
local first;
local RSI,RSIPeriod;
local KPeriod, DPeriod, Stochastic;
local BB, BBDeviation, BBPeriod;
local CustomID;
local S1, S2;
...
function Prepare( nameOnly)
	...
	Stochastic= core.indicators:create("SFK", Source , KPeriod, DPeriod);
	BB= core.indicators:create("BB", Source.close , BBPeriod, BBDeviation);
    first = math.max(RSI.DATA:first(), Stochastic.DATA:first(), BB.DATA:first()) + 1;
...
function ExtUpdate(id, source, period)  -- The method called every time when a new bid or ask price appears.
 ...
 long entry:
price closes below the 2 SD lower Bollinger band
fast stochastic <30 

(ie there is an upturn in momentum on both indicators)
exit when the price = 20 period Bollinger mid band.


and the opposite for a short....

short entry:
Price closes above the 2 SD upper Bollinger band 
Fast stochastic >70
The following period-
RSI must be lower than previous period
an the same for fast stochastic.
(ie there is a down turn in momentum on both indicators) 
Exit when price=20 period Bollinger mid band 
]]
   
    if  Source.close[period-1]<  BB.BL[period-1] 
	and (Stochastic.K[period-1]< 30 or not S1 )
	and (RSI.DATA[period]> RSI.DATA[period-1] or not S2 )
	and (Stochastic.K[period]> Stochastic.K[period-1] or not S1 )
	then 
        if Reverse then
            SELL();
        else
    ...
    elseif Source.close[period-1] >  BB.TL[period-1]
	and (Stochastic.K[period-1] > 70 or not S1)
	and (RSI.DATA[period]< RSI.DATA[period-1]or not S2)
	and (Stochastic.K[period]< Stochastic.K[period-1]or not S1)
	then
        if Reverse then
    ...
    if Reverse then
	  ...
	  elseif Source.close[period] <  BB.AL[period] then
