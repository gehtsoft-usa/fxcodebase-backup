...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=13149
...
function Init() --The strategy profile initialization
    strategy:name("MTF Two EMA Strategy");
...
function TIMEFRAME(id, FRAME)
    ...
    strategy.parameters:addInteger(tostring(id*100 + 2), " Period", "", 5);
    strategy.parameters:addInteger(tostring(id*100 + 3), "Period", "",10);
...
function Prepare(nameOnly)
    ...
    One[i] = core.indicators:create("EMA", Source[i][Price], Parameters[tostring(i*100 + 2)]   );
    Two[i] = core.indicators:create("EMA", Source[i][Price], Parameters[tostring(i*100 + 3)]   );
...
function ExtUpdate(id, source, period)  -- The method called every time when a new bid or ask price appears.
	...
	local i;
	for i = 1, TfNumber , 1 do	
    One[i]:update(core.UpdateLast);
	Two[i]:update(core.UpdateLast);
	end

    local FLAG = true;
	
	for i = 1, TfNumber , 1 do	
		if One[i].DATA:size() <= One[i].DATA:first() + 2  then
		   FLAG= false;
		end
	end
   ...
   -- get only time
    now = now - math.floor(now);		
      ...
      return ;
		   ...
		   if core.crossesOver(One[1].DATA,  Two[1].DATA, One[1].DATA:size() - 2) 
		   and One[2].DATA [ One[2].DATA:size() - 2 ] >   Two[2].DATA[ One[2].DATA:size() - 2]
		   and One[3].DATA [ One[3].DATA:size() - 2 ] >   Two[3].DATA[ One[3].DATA:size() - 2]
		   and One[4].DATA [ One[4].DATA:size() - 2 ] >   Two[4].DATA[ One[4].DATA:size() - 2]
		   then
		  ...
		  elseif  core.crossesUnder(One[1].DATA,  Two[1].DATA, One[1].DATA:size() - 2) 
		  and One[2] .DATA[ One[2].DATA:size() - 2 ] <   Two[2].DATA[ One[2].DATA:size() - 2]
		   and One[3].DATA [ One[3].DATA:size() - 2 ] <   Two[3].DATA[ One[3].DATA:size() - 2]
		   and One[4].DATA [ One[4].DATA:size() - 2 ] <   Two[4].DATA[ One[4].DATA:size() - 2]
		 then
            if Direction then
                ...
                BUY();
					...
					if core.crossesUnder(One[1].DATA,  Two[1].DATA, One[1].DATA:size() - 2) 
					then 			
								if   Direction then
					  ...
					  elseif core.crossesOver(One[1].DATA,  Two[1].DATA, One[1].DATA:size() - 2) 
		               then
								   if   Direction then    
										if haveTrades("S") then
										   exit("S");
