...
function Init() --The strategy profile initialization
    ...
    strategy.parameters:addDouble("adx_level", "ADX Level", "", 50);
...
function CreateTradingParameters()
    ...
    strategy.parameters:addDouble("profit_limit", "Profit limit", "", 100)
    strategy.parameters:addDouble("loss_limit", "Loss limit", "", 100)
...
local ToTime
local adx, adx_level, profit_limit, loss_limit;
--
function Prepare(nameOnly)
    profit_limit = instance.parameters.profit_limit;
    loss_limit = instance.parameters.loss_limit;
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    local accountRow = core.host:findTable("accounts"):find("AccountID", Account);
    local total_pl = accountRow.Equity - accountRow.Balance;
    if total_pl >= profit_limit or total_pl <= -loss_limit then
    ...
    if core.crossesOver(Source.close, Indicators:getStream(0), period) and adx.DATA[period] > adx_level then
        ACTION(1, "Top ", "Over")
        ONE = Source:serial(period)
    end
    if core.crossesUnder(Source.close, Indicators:getStream(0), period) and adx.DATA[period] > adx_level then
        ACTION(2, "Top ", "Under")
        ONE = Source:serial(period)
    end
    if core.crossesOver(Source.close, Indicators:getStream(2), period) and adx.DATA[period] > adx_level then
        ACTION(3, "Central ", "Over")
        ONE = Source:serial(period)
    end
    if core.crossesUnder(Source.close, Indicators:getStream(2), period) and adx.DATA[period] > adx_level then
        ACTION(4, "Central ", "Under")
        ONE = Source:serial(period)
    end
    if core.crossesOver(Source.close, Indicators:getStream(1), period) and adx.DATA[period] > adx_level then
        ACTION(5, "Bottom ", "Over")
        ONE = Source:serial(period)
    end
    if core.crossesUnder(Source.close, Indicators:getStream(1), period) and adx.DATA[period] > adx_level then
