...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=63134
...
function Init() --The strategy profile initialization
    strategy:name("Proportional Direction Strategy")
    ...
    strategy.parameters:addGroup("Indicator Selection")
    ...
    strategy.parameters:addString("Direction", "Direction", "Direction", "Direct")
    strategy.parameters:addStringAlternative("Direction", "Direct", "Direct", "Direct")
    strategy.parameters:addStringAlternative("Direction", "Reverse Indicator", "ReverseIndicator", "ReverseIndicator")
    strategy.parameters:addStringAlternative("Direction", "Reverse Price", "ReversePrice", "ReversePrice")
...
function CreateTradingParameters()
    ...
    strategy.parameters:addString("TradeDirection", "Type of Signal / Trade", "", "direct")
    strategy.parameters:addStringAlternative("TradeDirection", "Direct", "", "direct")
    strategy.parameters:addStringAlternative("TradeDirection", "Reverse", "", "reverse")
...
function Prepare(nameOnly)
    ...
    TradeDirection = instance.parameters.TradeDirection == "direct"
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if Direction == "Direct" then
        if Source.close[period] > Source.open[period] and INDEX[period] > INDEX[period - 1] then
            if TradeDirection then
                BUY()
            else
                SELL()
            end
            ONE = Source:serial(period)
        elseif Source.close[period] < Source.open[period] and INDEX[period] < INDEX[period - 1] then
            if TradeDirection then
                SELL()
            else
                BUY()
