-- Available @ https://fxcodebase.com/code/viewtopic.php?f=31&t=74034
...
local STRATEGY_NAME = "Momentum Signal Strategy";
function CreateParameters() 
    --create your algorithm strategy.parameters here
    strategy.parameters:addGroup("Calculation");  
    strategy.parameters:addInteger("CciPeriod", "Cci Period", "Cci Period", 6);
    strategy.parameters:addInteger("AtrPeriod", "Atr Period", "Atr Period", 12);
    strategy.parameters:addInteger("MomentumPeriod", "Momentum Period", "Momentum Period", 7);
    strategy.parameters:addInteger("RsiPeriod", "Rsi Period", "Rsi Period", 7);
    strategy.parameters:addInteger("AdxPeriod", "Adx Period", "Adx Period", 7);
    strategy.parameters:addDouble("SubtractFromSignal", "Subtract From Signal", "Subtract From Signal", -5);
    strategy.parameters:addDouble("SubtractFromIndicator", "Subtract From Indicator", "SubtractFromIndicator", -1);
end

local indi;
function CreateEntryIndicators(source)
    local profile = core.indicators:findIndicator("MOMENTUM SIGNAL");
    assert(profile ~= nil, "Please, download and install " .. "MOMENTUM SIGNAL" .. ".LUA indicator");
    local indicatorParams = profile:parameters();
    indicatorParams:setInteger("CciPeriod", instance.parameters.CciPeriod);
    indicatorParams:setInteger("AtrPeriod", instance.parameters.AtrPeriod);
    indicatorParams:setInteger("MomentumPeriod", instance.parameters.MomentumPeriod);
    indicatorParams:setInteger("RsiPeriod", instance.parameters.RsiPeriod);
    indicatorParams:setInteger("AdxPeriod", instance.parameters.AdxPeriod);
    indicatorParams:setDouble("SubtractFromSignal", instance.parameters.SubtractFromSignal);
    indicatorParams:setDouble("SubtractFromIndicator", instance.parameters.SubtractFromIndicator);
    indi = core.indicators:create("MOMENTUM SIGNAL", source, indicatorParams)
end
...
function IsEntryLong(source, period)
    if not indi.Momentum:hasData(period - 1) or not indi.Signal:hasData(period - 1) then
        return false;
    end
    return core.crossesOver(indi.Momentum, indi.Signal, period);
end
function IsEntryShort(source, period)
    if not indi.Momentum:hasData(period - 1) or not indi.Signal:hasData(period - 1) then
        return false;
    end
    return core.crossesUnder(indi.Momentum, indi.Signal, period);
end
