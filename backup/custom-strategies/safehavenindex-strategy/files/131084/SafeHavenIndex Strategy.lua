...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=69379
...
function Init() --The strategy profile initialization
    strategy:name("Safe Haven Index");
    ...
    strategy.parameters:addInteger("Period", "Period", "", 50, 2, 2000);
    strategy.parameters:addInteger("Smoothing", "Smoothing Period", "", 7, 2, 2000);
	 strategy.parameters:addDouble("Shift", "Shift", "", 0);
...
function CreateTradingParameters()
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "SHIS");
...
function Prepare( nameOnly)
	...
	assert(core.indicators:findIndicator("SAFEHAVENINDEX") ~= nil, "Please, download and install SAFEHAVENINDEX.LUA indicator");          
    ...
    Indicator = core.indicators:create("SAFEHAVENINDEX", Source, instance.parameters.Period, instance.parameters.Smoothing );
...
function EntryFunction( id, now,period)
    ...
    if   Indicator.DATA[period]>   -Shift
	and Indicator.DATA[period-1]<=  -Shift
	then	
          if Direction then
                BUY();
            else
		...
		Return=true;
    elseif   Indicator.DATA[period]< Shift 
	and Indicator.DATA[period-1]>= Shift 
	then
            if Direction then
                SELL();
