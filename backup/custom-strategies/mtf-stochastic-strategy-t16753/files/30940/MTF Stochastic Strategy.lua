...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=16753
...
function TIMEFRAME(id, FRAME)
    ...
    strategy.parameters:addInteger(tostring(id * 100 + 7), "Buy level", "", 50, 1, 100)
    strategy.parameters:addInteger(tostring(id * 100 + 8), "Sell level", "", 50, 1, 100)
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if
        core.crossesOver(K[1], Parameters[tostring(1 * 100 + 7)], K[1]:size() - 2) and
            (K[2][K[2]:size() - 2] > Parameters[tostring(2 * 100 + 7)] or not ON[2]) and
            (K[3][K[3]:size() - 2] > Parameters[tostring(3 * 100 + 7)] or not ON[3]) and
            (K[4][K[4]:size() - 2] > Parameters[tostring(4 * 100 + 7)] or not ON[4])
     then
        if Direction then
            BUY()
        else
    ...
    elseif
        core.crossesUnder(K[1], Parameters[tostring(1 * 100 + 8)], K[1]:size() - 2) and
            (K[2][K[2]:size() - 2] < Parameters[tostring(2 * 100 + 8)] or not ON[2]) and
            (K[3][K[3]:size() - 2] < Parameters[tostring(3 * 100 + 8)] or not ON[3]) and
            (K[4][K[4]:size() - 2] < Parameters[tostring(4 * 100 + 8)] or not ON[4])
     then
        if Direction then
            SELL()
        else
    ...
    if K[1][K[1]:size() - 2] < Parameters[tostring(1 * 100 + 7)] then
        if Direction then
    ...
    if K[1][K[1]:size() - 2] > Parameters[tostring(1 * 100 + 8)] then
        if Direction then
            if haveTrades("S") then
                exit("S")
                Signal("Close Short")
