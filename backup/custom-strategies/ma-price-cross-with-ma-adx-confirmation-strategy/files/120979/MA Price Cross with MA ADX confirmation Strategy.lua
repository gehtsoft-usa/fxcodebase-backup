...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=66625
...
function Init() --The strategy profile initialization
    strategy:name("MA Price Cross with MA ADX confirmation Strategy");
    ...
    strategy.parameters:addStringAlternative("Method1", "DEMA", "DEMA" , "DEMA");
	strategy.parameters:addStringAlternative("Method1", "TEMA", "TEMA" , "TEMA");  
	strategy.parameters:addStringAlternative("Method1", "PAR_MA", "PAR_MA" , "PAR_MA");  

	
	strategy.parameters:addGroup("ADX Filter Calculation");	
	
	strategy.parameters:addBoolean("ADXFilter", "Use ADX Filter", "", true);
	strategy.parameters:addInteger("ADX_Period", "ADX Period", "", 14);
	
	strategy.parameters:addGroup("Filter Calculation");
	strategy.parameters:addBoolean("MAFilter", "Use MA Filter", "", true);
    ...
    strategy.parameters:addStringAlternative("Method2", "DEMA", "DEMA" , "DEMA");
	strategy.parameters:addStringAlternative("Method2", "TEMA", "TEMA" , "TEMA");  
	strategy.parameters:addStringAlternative("Method2", "PAR_MA", "PAR_MA" , "PAR_MA");  
...
function CreateTradingParameters()
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "MAPCMAADXCS");
...
local Price2, Price1, Method1, Method2, MAFilter, Period1, Period2, ADX_Period, ADXFilter;
local fast, ADX,slow; 
...
function Prepare( nameOnly)
	...
	ADXFilter= instance.parameters.ADXFilter;
...
function ExtUpdate(id, source, period)  -- The method called every time when a new bid or ask price appears.
   ...
   -- get only time
    now = now - math.floor(now);
	 ...
	 if MAFilter then
		slow:update(core.UpdateLast); 
			...
			if  not slow.DATA:hasData(period)
	        or not slow.DATA:hasData(period-1)
			then
			return;
			end
	...
	if not ADX.DATA:hasData(period-1)
	or not ADX.DATA:hasData(period)
	or not fast.DATA:hasData(period)
	or not fast.DATA:hasData(period-1)
	or not Source.close:hasData( period) 
	then
	return;
	end
...
--period
   ...
   if  Source.close[period] > fast.DATA[period] 
   and  Source.close[period-1] <= fast.DATA[period-1] 
   and (not ADXFilter or  ADX.DATA[period]> ADX.DATA[period-1])
   and (not MAFilter or  Source.close[period] > slow.DATA[period]  ) 
	then	
          if Direction then
                BUY();
            else
		...
		Return=true;
   elseif  Source.close[period] < fast.DATA[period] 
   and   Source.close[period-1] >= fast.DATA[period-1] 
   and (not ADXFilter or ADX.DATA[period]> ADX.DATA[period-1])
   and (not MAFilter or  Source.close[period] < slow.DATA[period]  )
