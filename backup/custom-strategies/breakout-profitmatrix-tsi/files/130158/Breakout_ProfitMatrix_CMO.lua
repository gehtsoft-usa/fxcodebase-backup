...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=69168
--+------------------------------------------------------------------+
...
local STRATEGY_NAME = "Breakout ProfitMatrix CMO Strategy";
...
function CreateParameters() 
    strategy.parameters:addGroup("Breakout");
    strategy.parameters:addString("bo_PeriodBegin", "The hour and minute when the period begins", "", "00:00");
    strategy.parameters:addString("bo_PeriodEnd", "The hour and minute when the period ends", "", "05:30");
    strategy.parameters:addString("bo_BoxEnd", "The hour and minute when the box ends", "", "23:00");
    strategy.parameters:addString("bo_Type", "The time type", "", "TD");
    strategy.parameters:addStringAlternative("bo_Type", "Local Time", "", "LT");
    strategy.parameters:addStringAlternative("bo_Type", "EST Time", "", "EST");
    strategy.parameters:addStringAlternative("bo_Type", "GMT Time", "", "GMT");
    strategy.parameters:addStringAlternative("bo_Type", "Trading Day time", "", "TD");
    
    strategy.parameters:addGroup("ProfitMatrix");
    strategy.parameters:addInteger("pm_Period1", "Period", "", 7, 2, 2000);
	strategy.parameters:addString("pm_Method1", "MA Method", "Method" , "MVA");
    strategy.parameters:addStringAlternative("pm_Method1", "MVA", "MVA" , "MVA");
    strategy.parameters:addStringAlternative("pm_Method1", "EMA", "EMA" , "EMA");
    strategy.parameters:addStringAlternative("pm_Method1", "LWMA", "LWMA" , "LWMA");
    strategy.parameters:addStringAlternative("pm_Method1", "TMA", "TMA" , "TMA");
    strategy.parameters:addStringAlternative("pm_Method1", "SMMA", "SMMA" , "SMMA");
    strategy.parameters:addStringAlternative("pm_Method1", "KAMA", "KAMA" , "KAMA");
    strategy.parameters:addStringAlternative("pm_Method1", "VIDYA", "VIDYA" , "VIDYA");
    strategy.parameters:addStringAlternative("pm_Method1", "WMA", "WMA" , "WMA");
    strategy.parameters:addInteger("pm_Period2", "Period 2", "", 21, 2, 2000);
	strategy.parameters:addString("pm_Method2", "MA Method 2", "Method" , "MVA");
    strategy.parameters:addStringAlternative("pm_Method2", "MVA", "MVA" , "MVA");
    strategy.parameters:addStringAlternative("pm_Method2", "EMA", "EMA" , "EMA");
    strategy.parameters:addStringAlternative("pm_Method2", "LWMA", "LWMA" , "LWMA");
    strategy.parameters:addStringAlternative("pm_Method2", "TMA", "TMA" , "TMA");
    strategy.parameters:addStringAlternative("pm_Method2", "SMMA", "SMMA" , "SMMA");
    strategy.parameters:addStringAlternative("pm_Method2", "KAMA", "KAMA" , "KAMA");
    strategy.parameters:addStringAlternative("pm_Method2", "VIDYA", "VIDYA" , "VIDYA");
    strategy.parameters:addStringAlternative("pm_Method2", "WMA", "WMA" , "WMA");

    strategy.parameters:addGroup("CMO");
    strategy.parameters:addInteger("cmo_period", "Period", "", 7);
end

local bo, MA1, MA2, cmo
function CreateEntryIndicators(source) 
    local profile = core.indicators:findIndicator("BREAKOUT");
    assert(profile ~= nil, "Please, download and install " .. "BREAKOUT" .. ".LUA indicator");
    local indicatorParams = profile:parameters();
    indicatorParams:setString("bo_PeriodBegin", instance.parameters.bo_PeriodBegin);
    indicatorParams:setString("bo_PeriodEnd", instance.parameters.bo_PeriodEnd);
    indicatorParams:setString("bo_BoxEnd", instance.parameters.bo_BoxEnd);
    indicatorParams:setString("bo_Type", instance.parameters.bo_Type);
    bo = core.indicators:create("BREAKOUT", source, indicatorParams)

    MA1 = core.indicators:create(instance.parameters.pm_Method1, source, instance.parameters.pm_Period1);
    MA2 = core.indicators:create(instance.parameters.pm_Method2, source, instance.parameters.pm_Period2);

    cmo = core.indicators:create("CMO", source, instance.parameters.cmo_period);
end

function CreateExitIndicators(source) 
end

function Updatestrategys()
    bo:update(core.UpdateLast);
    MA1:update(core.UpdateLast);
    MA2:update(core.UpdateLast);
    cmo:update(core.UpdateLast);
end
...
function CreateCustomActions()
    if not DISABLE_EXIT then
        ...
        exitLongAction.IsPass = function (source, period, periodFromLast, data)
            if period < 2 or not cmo.DATA:hasData(period - 1) then
                return false;
            end
            return core.crosses(cmo.DATA, 0, period);
        end
    ...
    enterLongAction.GetLog = function (source, period, periodFromLast, data)
        return "Price CO BO.H: " .. tostring(core.crossesOver(source.close, bo.PH, period))
            .. " MA1 > MA2: " .. tostring(MA1.DATA[period] > MA2.DATA[period]);
    end
    enterLongAction.IsPass = function (source, period, periodFromLast, data)
        if period < 2 or not bo.PH:hasData(period - 1) then
            return false;
        end
        return core.crossesOver(source.close, bo.PH, period)
    ...
    enterShortAction.GetLog = function (source, period, periodFromLast, data)
        return "Price CU BO.L: " .. tostring(core.crossesUnder(source.close, bo.PL, period))
            .. " MA1 < MA2: " .. tostring(MA1.DATA[period] < MA2.DATA[period]);
    end
    enterShortAction.IsPass = function (source, period, periodFromLast, data)
        if period < 2 or not bo.PL:hasData(period - 1) then
            return false;
        end
        return core.crossesUnder(source.close, bo.PL, period)
            and MA1.DATA[period] < MA2.DATA[period];
    end
...
function CreatePositionStrategy(source, side, id)
    ...
    if SetCustomStop == nil then
        ...
        if position_strategy.StopType == "atr" then
            position_strategy.StopATR = core.strategys:create("ATR", source, position_strategy.Stop);
    ...
    if SetCustomLimit == nil then
        ...
        if position_strategy.LimitType == "atr" then
            position_strategy.LimitATR = core.strategys:create("ATR", source, position_strategy.Limit);
    ...
    function position_strategy:Open(period, periods_from_last)
        ...
        if default_stop then
            if self.StopType == "atr" then
                ...
                breakeven:CreatestrategyTrailingController()
                    :SetRequestID(result.RequestID)
                    :SetTrailingTarget(breakeven.STOP_ID)
                    :SetstrategyStream(self.StopATR.DATA, self.AtrStopMult, true);
            end
        end
        if default_limit then
            if self.LimitType == "atr" then
                ...
                breakeven:CreatestrategyTrailingController()
                    ...
                    :SetstrategyStream(self.LimitATR.DATA, self.AtrLimitMult, true);
            end
...
function EnsurestrategyInstalled(name)
    local profile = core.strategys:findstrategy(name);
    assert(profile ~= nil, "Please, download and install " .. name .. ".LUA strategy");
    return profile;
end
