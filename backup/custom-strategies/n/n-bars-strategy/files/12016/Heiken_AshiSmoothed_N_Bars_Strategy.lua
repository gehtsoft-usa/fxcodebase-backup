...
function Init() --The strategy profile initialization
    strategy:name("Heiken AshiSmoothed N Bars Strategy")
    ...
    strategy.parameters:addGroup("Heikin-Ashi Smoothed Parameters")
...
function Prepare(nameOnly)
    ...
    assert(core.indicators:findIndicator("HASM") ~= nil, "HASM" .. " indicator must be installed");
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    local TREND = 0
    local i
    for i = period, period - N, -1 do
        if i == period then
            if HA.open[i] < HA.close[i] then
                TREND = TREND + 1
            elseif HA.open[i] > HA.close[i] then
                TREND = TREND - 1
            end

            if HA.open[i] < HA.close[i] and TREND < 0 then
                return
            end
