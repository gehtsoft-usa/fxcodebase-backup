...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=15812
...
function Init() --The strategy profile initialization
	...
	strategy.parameters:addInteger("Period", "Number of candles to qualify", "Number of periods", 3);
...
local DOWN = true;
	...
	for i =period , period- Period +1, -1 do
			...
			or Source.high[period] > Source.high[period-1]
			then
			UP = false;
			end
			if  Source.low[period] < Source.low[period-1]
...
function ExtUpdate(id, source, period)  -- The method called every time when a new bid or ask price appears.
   ...
   -- get only time
	...
	UP,DOWN= Check (period) ;
		...
		--and core.crossesOver(Short[1], OS, period)
		and UP
		then
		...
		--and core.crossesUnder(Short[1], OS, period)
		and DOWN
		 then
            if Direction then
                SELL();
