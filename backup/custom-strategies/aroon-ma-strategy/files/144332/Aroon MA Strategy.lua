...
-- https://fxcodebase.com/code/viewtopic.php?f=31&t=71666
...
function Init() --The strategy profile initialization
    strategy:name("Aroon MA Strategy");
    ...
    strategy.parameters:addString("RSI_Price", "Price Source", "", "close");
	...
	strategy.parameters:addInteger("RSI_Period", "Period", "Period" , 14);		
	...
	strategy.parameters:addInteger("Period1", "Short Period", "Period" , 12);	
 	strategy.parameters:addInteger("Period2", "Long Period", "Period" , 26);	
	strategy.parameters:addInteger("Period3", "Signal Period", "Period" , 9);	
	
	
	strategy.parameters:addGroup("AROON Calculation");	
	strategy.parameters:addInteger("Aroon_Period", "Period", "Period" , 25);	
	
	strategy.parameters:addGroup("Exit Option");	
		
	strategy.parameters:addBoolean("Exit", "Use Exit", "", true);
    strategy.parameters:addBoolean("E1", "Use Exit Option 1", "", true);
    strategy.parameters:addBoolean("E2", "Use Exit Option 2", "", true);	
...
function CreateTradingParameters()
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "AMS");
...
function Prepare( nameOnly)
    ...
    RSI = core.indicators:create("RSI", Source[instance.parameters.RSI_Price], instance.parameters.RSI_Period ); 
    MACD = core.indicators:create("MACD", Source[instance.parameters.MACD_Price], instance.parameters.Period1, instance.parameters.Period2, instance.parameters.Period3 ); 	
    AROON = core.indicators:create("AROON", Source , instance.parameters.Aroon_Period); 	
	first=math.max(MA.DATA:first(),RSI.DATA:first(),MACD.SIGNAL:first(),AROON.DATA:first()); 
...
--	period3= core.findDate (Source3, TickSource:date(period), false);
	...
	--/////////////////////////////////////////////////////////////////////
		            ...
		            if   (  MACD.MACD[period]< MACD.SIGNAL[period] and  RSI.DATA[period]< 50 and E2)
					or   (MACD.MACD[period]< MACD.SIGNAL[period] and  RSI.DATA[period]< 70 and E1)
					then
					  ...
					  if ( MACD.MACD[period]> MACD.SIGNAL[period] and  RSI.DATA[period]> 50 and E2)
					or  ( MACD.MACD[period]< MACD.SIGNAL[period] and  RSI.DATA[period]> 30 and E1)	 				  
					  then
...
function EntryFunction( id, now,period)
	...
	and AROON.UP[period]> AROON.DOWN[period]
	then	
          if Direction then
                BUY();
            else
