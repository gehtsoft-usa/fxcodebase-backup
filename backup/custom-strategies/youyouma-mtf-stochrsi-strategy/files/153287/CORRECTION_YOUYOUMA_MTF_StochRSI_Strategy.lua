...
function Prepare(nameOnly)
    ...
    for i = 1, TfNumber, 1 do
        ...
        for i = 1, TfNumber, 1 do
            ...
            Parameters[tostring(i * 100 + 7)] = instance.parameters:getInteger(tostring(i * 100 + 7))
            Parameters[tostring(i * 100 + 8)] = instance.parameters:getInteger(tostring(i * 100 + 8))

            assert(Parameters[tostring(i * 100 + 1)] ~= "t1", "The time frame must not be tick")

            Source[i] =
                ExtSubscribe(i, nil, Parameters[tostring(i * 100 + 1)], instance.parameters.Type == "Bid", "bar")
            ...
            K[i] = Indicator[i]:getStream(0)
            D[i] = Indicator[i]:getStream(1)
        end
...
function EntryFunction(id, now, period)
    ...
    if
        core.crossesOver(K[1], D[1], K[1]:size() - 2, D[1]:size() - 2) and
            (K[2][K[2]:size() - 2] > D[2][D[2]:size() - 2] or not ON[2]) and
            (K[3][K[3]:size() - 2] > D[3][D[3]:size() - 2] or not ON[3]) and
            K[1][K[1]:size() - 2] < Parameters[tostring(1 * 100 + 7)]
     then
        if Direction then
            BUY()
        else
    ...
    elseif
        core.crossesUnder(K[1], D[1], K[1]:size() - 2, D[1]:size() - 2) and
            (K[2][K[2]:size() - 2] < D[2][D[2]:size() - 2] or not ON[2]) and
            (K[3][K[3]:size() - 2] < D[3][D[3]:size() - 2] or not ON[3]) and
            K[1][K[1]:size() - 2] > Parameters[tostring(1 * 100 + 8)]
     then
        if Direction then
            SELL()
        else
...
function ExitFunction(id, now, period)
    ...
    if tradesCount("B") > 0 then
        if
            core.crossesUnder(K[2], D[2], K[2]:size() - 2, D[2]:size() - 2) or
                core.crossesUnder(K[3], D[3], K[3]:size() - 2, D[3]:size() - 2)
         then
            if Direction then
                if haveTrades("B") then
                    exitSpecific("B")
        ...
        if tradesCount("S") > 0 then
            if
                core.crossesOver(K[2], D[2], K[2]:size() - 2, D[2]:size() - 2) or
                    core.crossesOver(K[3], D[3], K[3]:size() - 2, D[3]:size() - 2)
             then
                if Direction then
                    if haveTrades("S") then
                        exitSpecific("S")
