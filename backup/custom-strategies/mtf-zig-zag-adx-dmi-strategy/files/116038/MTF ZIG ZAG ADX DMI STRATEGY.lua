...
--http://fxcodebase.com/code/viewtopic.php?f=31&t=65364
--+------------------------------------------------------------------+
...
function Init() --The strategy profile initialization
    strategy:name("MTF ZIG ZAG ADX DMI STRATEGY")
    ...
    strategy.parameters:addInteger("DMI_Period", "DMI Period", "", 14)

    strategy.parameters:addDouble("ADX_Buy", "ADX Buy Level", "", 25)
    strategy.parameters:addDouble("ADX_Exit", "ADX Exit Level", "", 50)
    ...
    strategy.parameters:addString("TF4", "Time frame", "", "D1")
...
local ADX, DMI, ADX_Period, DMI_Period, ADX_Buy, ADX_Exit
local TF, TF1, TF2, TF3, TF4

local ZIGZAG1, ZIGZAG2, ZIGZAG3, ZIGZAG4
...
function Prepare(nameOnly)
    ...
    ADX_Buy = instance.parameters.ADX_Buy
    ADX_Exit = instance.parameters.ADX_Exit
    ...
    assert(TF4 ~= "t1", "The time frame must not be tick")
    ...
    Source1 = ExtSubscribe(3, nil, TF1, instance.parameters.Type == "Bid", "bar")
    Source2 = ExtSubscribe(4, nil, TF2, instance.parameters.Type == "Bid", "bar")
    Source3 = ExtSubscribe(5, nil, TF3, instance.parameters.Type == "Bid", "bar")
    Source4 = ExtSubscribe(6, nil, TF4, instance.parameters.Type == "Bid", "bar")
    ...
    ZIGZAG3 =
        ...
        "ZIGZAG",
        Source3,
        instance.parameters.Depth3,
        instance.parameters.Deviation3,
        instance.parameters.Backstep3
    )
    ZIGZAG4 =
        core.indicators:create(
        "ZIGZAG",
        Source4,
        instance.parameters.Depth4,
        instance.parameters.Deviation4,
        instance.parameters.Backstep4
    )
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if EntryExecutionType == "Live" then
        ...
        period4 = core.findDate(Source4, TickSource:date(period), false)
    else
        if id ~= 2 then
            return
        end
        ...
        period3 = core.findDate(Source3, Source:date(period), false)
        period4 = core.findDate(Source4, Source:date(period), false)
    end
    ...
    ZIGZAG3:update(core.UpdateAll)
    ZIGZAG4:update(core.UpdateAll)

    if not ADX.DATA:hasData(period0) or not DMI.DATA:hasData(period0) or not ADX.DATA:hasData(period0 - 1) then
        return
    end

    if EntryExecutionType == "Live" and id == 1 or EntryExecutionType ~= "Live" and id ~= 1 then
        EntryFunction(now, period0, period1, period2, period3, period4)
    end
end

function FindLast(period1, period2, period3, period4)
    local ZZ1 = 0
    local ZZ2 = 0
    local ZZ3 = 0
    local ZZ4 = 0

    for i = period1, Source1:first(), -1 do
        if ZIGZAG1.DATA:hasData(i) and ZIGZAG1.DATA[i] ~= 0 and ZIGZAG1.DATA[i] ~= nil then
            if ZIGZAG1.DATA[i] == Source1.high[i] then
                ZZ1 = 1
            end

            if ZIGZAG1.DATA[i] == Source1.low[i] then
                ZZ1 = -1
            end
    ...
    for i = period2, Source2:first(), -1 do
        if ZIGZAG2.DATA:hasData(i) and ZIGZAG2.DATA[i] ~= 0 and ZIGZAG2.DATA[i] ~= nil then
            if ZIGZAG2.DATA[i] == Source2.high[i] then
                ZZ2 = 1
            end

            if ZIGZAG2.DATA[i] == Source2.low[i] then
                ZZ2 = -1
            end
    ...
    for i = period3, Source3:first(), -1 do
        if ZIGZAG3.DATA:hasData(i) and ZIGZAG3.DATA[i] ~= 0 and ZIGZAG3.DATA[i] ~= nil then
            if ZIGZAG3.DATA[i] == Source3.high[i] then
                ZZ3 = 1
            end

            if ZIGZAG3.DATA[i] == Source3.low[i] then
                ZZ3 = -1
            end
    ...
    for i = period4, Source4:first(), -1 do
        if ZIGZAG4.DATA:hasData(i) and ZIGZAG4.DATA[i] ~= 0 and ZIGZAG4.DATA[i] ~= nil then
            if ZIGZAG4.DATA[i] == Source4.high[i] then
                ZZ4 = 1
            end

            if ZIGZAG4.DATA[i] == Source4.low[i] then
                ZZ4 = -1
            end
    ...
    return ZZ1, ZZ2, ZZ3, ZZ4
end

function EntryFunction(now, period0, period1, period2, period3, period4)
    local Return = false
    ...
    local ZZ1, ZZ2, ZZ3, ZZ4 = FindLast(period1, period2, period3, period4)
...
6. dmi positive > dmi negative]]
    ...
    if
        ADX.DATA[period0] > ADX_Buy and ADX.DATA[period0] < ADX_Exit and ADX.DATA[period0] > ADX.DATA[period0 - 1] and
            DMI.DIP[period0] > DMI.DIM[period0] and
            ZZ1 == 1 and
            ZZ2 == 1 and
            ZZ3 == 1 and
            ZZ4 == -1
