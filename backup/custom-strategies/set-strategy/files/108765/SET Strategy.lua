...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if
        Source.close[period] < MA.DATA[period] and Schaff.DATA[period] > BL1 and TDI.RSI_P[period] > TDI.MB[period] and
            TDI.RSI_P[period - 1] <= TDI.MB[period - 1] and
            TDI.TS[period] > BL2
     then
        if Direction then
            BUY()
    ...
    elseif
        Source.close[period] > MA.DATA[period] and Schaff.DATA[period] < SL1 and
            TDI.RSI_P[period - 1] >= TDI.MB[period - 1] and
            TDI.MB[period] < SL2
     then
        if Direction then
