...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=66891
...
function Init()
    strategy:name("DCS Strategy");
    strategy:description("v1 2018-11-04");

    strategy.parameters:addInteger("DMI_period", "DMI/ADX Period", "", 14);
...
function Prepare(name_only)
    ...
    assert(instance.parameters.limit_type ~= "stop" or instance.parameters.stop_type == "pips" or instance.parameters.stop_type == "custom", "To use limit based on stop you need to set stop in pips");

    dmi = core.indicators:create("DMI", trading_logic.MainSource, instance.parameters.DMI_period);
    adx = core.indicators:create("ADX", trading_logic.MainSource, instance.parameters.DMI_period);
    sar = core.indicators:create("SAR", trading_logic.MainSource, instance.parameters.SAR_Step,
...
function IsBuyCondition(source, period)
    if not sar.DN:hasData(period) then
        return false;
    end
    if dmi.DIP[period] < dmi.DIM[period] then
        return false;
    end
    if adx.DATA[period] < dmi.DIP[period] then
        return false;
    end
...
function IsSellCondition(source, period)
    if not sar.UP:hasData(period) then
        return false;
    end
    if dmi.DIP[period] > dmi.DIM[period] then
        return false;
    end
    if adx.DATA[period] > dmi.DIM[period] then
        return false;
    end
...
function DoBuy(source, period)
    if instance.parameters.allow_trade then
        ...
        elseif instance.parameters.stop_type == "custom" then
            local sar_value;
            if sar.UP:hasData(period) then
                sar_value = sar.UP[period];
            else
                sar_value = sar.DN[period];
            end
            stop_value = math.abs(instance.ask:tick(NOW) - sar_value) / instance.ask:pipSize();
            command = command:SetPipStop(nil, stop_value, instance.parameters.use_trailing and instance.parameters.trailing);
        end
        ...
        breakeven:CreateIndicatorTrailingController()
            ...
            :SetTrailingTarget(breakeven.STOP_ID)
            :SetMinProfit(0)
            :SetIndicatorStream(sar.UP, nil, false);
...
function DoSell(source, period)
    if instance.parameters.allow_trade then
        ...
        elseif instance.parameters.stop_type == "custom" then
            ...
            stop_value = math.abs(instance.bid:tick(NOW) - sar_value) / instance.bid:pipSize();
            command = command:SetPipStop(nil, stop_value, instance.parameters.use_trailing and instance.parameters.trailing);
        end
        if instance.parameters.limit_type == "pips" then
            command = command:SetPipLimit(nil, instance.parameters.limit);
        ...
        breakeven:CreateIndicatorTrailingController()
            ...
            :SetTrailingTarget(breakeven.STOP_ID)
            :SetMinProfit(0)
            :SetIndicatorStream(sar.DN, nil, false);
        if instance.parameters.use_breakeven then
            breakeven:CreateController()
                :SetRequestID(result.RequestID)
                :SetWhen(instance.parameters.breakeven_when)
...
function breakeven:CreateIndicatorTrailingController()
    ...
    function controller:DoBreakeven()
        ...
        if self._target_id == breakeven.STOP_ID then
            ...
            if self._min_profit ~= nil and trade.PL < self._min_profit then
                return true;
            end
...
function trading:Init(parameters)
    ...
    if self.AddStopParameter then
        parameters:addString("stop_type", "Stop Order", "", "custom");
        ...
        parameters:addStringAlternative("stop_type", "% of distance to SAR", "", "custom");
    ...
    if self.AddLimitParameter then
        parameters:addString("limit_type", "Limit Order", "", "stop");
