...
function Init() --The strategy profile initialization
    ...
    strategy.parameters:addGroup("DNC indicator parameters")
    strategy.parameters:addInteger("N", "Period", "Period", 20, 1, 1000)
...
function Prepare(nameOnly)
    ...
    name =
        profile:id() ..
        "(" .. instance.bid:name() .. "." .. instance.parameters.TF .. "," .. "DNC(" .. instance.parameters.N .. "))"
    instance:name(name)
    if nameOnly then
        return;
    end
    ...
    DNC = core.indicators:create("DNC", Source, instance.parameters.N, "yes")
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if (DNC.DATA:first() > (period - 1)) then
        return
    end
    ...
    local MustS = false
    if
        (DNC.DU[period] == source.low[period] and instance.parameters.TypeSignal == "direct") or
            (DNC.DN[period] == source.high[period] and instance.parameters.TypeSignal == "reverse")
     then
        MustB = true
    end
    if
        (DNC.DN[period] == source.high[period] and instance.parameters.TypeSignal == "direct") or
            (DNC.DU[period] == source.low[period] and instance.parameters.TypeSignal == "reverse")
     then
        MustS = true
    end
