...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=2829
-- Id: 23869
-- More information about this indicator can be found at:
-- http://fxcodebase.com/
...
function ParseTime(time)
    ...
    return (h / 24.0 +  m / 1440.0 + s / 86400.0),                          
           ((h >= 0 and h < 24 and m >= 0 and m < 60 and s >= 0 and s < 60) or (h == 24 and m == 0 and s == 0)); 
end
...
function ExtUpdate(id, source, period)  
 ...
 if (StopTime>StartTime and now >= StartTime and now <= StopTime) or (StopTime<StartTime and (now >= StartTime or now <= StopTime)) then
  local p = period;
...
function ExtAsyncOperationFinished(cookie, success, message)
    if cookie == 200 and not success then
        ...
        terminal:alertMessage(instance.bid:instrument(), instance.bid[instance.bid:size() - 1], "Limit Entry order failed" .. message, instance.bid:date(instance.bid:size() - 1));
        elseif cookie == 203 and not success then
        terminal:alertMessage(instance.bid:instrument(), instance.bid[instance.bid:size() - 1], "Limit Entry Remove order failed " .. message, instance.bid:date(instance.bid:size() - 1));
 elseif cookie == 204 and not success then
        terminal:alertMessage(instance.bid:instrument(), instance.bid[instance.bid:size() - 1], "Stop order failed" .. message, instance.bid:date(instance.bid:size() - 1));
 elseif cookie == 205 and not success then
 elseif cookie == 206 and not success then
        terminal:alertMessage(instance.bid:instrument(), instance.bid[instance.bid:size() - 1], "Modify Stop order failed" .. message, instance.bid:date(instance.bid:size() - 1));
