...
local STRATEGY_NAME = "Donchian Channel Band Strategy";
...
function CreateEntryIndicators(source)
    local profile = core.indicators:findIndicator("DONCHIAN CHANNEL BAND");
    assert(profile ~= nil, "Please, download and install " .. "DONCHIAN CHANNEL BAND" .. ".LUA indicator");
    indi = core.indicators:create("DONCHIAN CHANNEL BAND", source, instance.parameters.N);
end
...
function IsEntryLong(source, period)
    return source[period] >= indi.DU[period];
end
function IsEntryShort(source, period)
    return source[period] <= indi.DN[period];
end
