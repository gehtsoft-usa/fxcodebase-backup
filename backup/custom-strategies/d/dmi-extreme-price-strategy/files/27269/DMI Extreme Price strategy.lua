...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=14355
...
function Init() --The strategy profile initialization
    strategy:name("DMI Extreme Price strategy")
    ...
    strategy.parameters:addInteger("LP", " Period", " Period", 14)
    strategy.parameters:addDouble("Threshold", " Threshold (in Pips)", " ", 0)
...
function Prepare(nameOnly)
    ...
    Indicator[1] = core.indicators:create("DMI", Source, LP)
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if HIGH ~= nil and core.crossesOver(Source.close, HIGH + Threshold * source:pipSize(), period) then
        if Direction then
            BUY()
        else
            SELL()
        end
    elseif LOW ~= nil and core.crossesUnder(Source.close, LOW - Threshold * source:pipSize(), period) then
        if Direction then
            SELL()
        else
            BUY()
