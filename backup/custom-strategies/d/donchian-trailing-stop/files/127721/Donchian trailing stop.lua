...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=68742
...
function Init()
    strategy:name("Donchian trailing stop");
    ...
    strategy.parameters:addString("stream", "Stream to use", "", "DU");
    strategy.parameters:addStringAlternative("stream", "Up", "", "DU");
    strategy.parameters:addStringAlternative("stream", "Down", "", "DN");
...
function Prepare(nameOnly)
    ...
    local profile = core.indicators:findIndicator("DOCHIANCHANNEL");
    assert(profile ~= nil, "Please, download and install " .. "DOCHIANCHANNEL" .. ".LUA indicator");

    Source = ExtSubscribe(2, nil, TF, instance.parameters.Type == "Bid", "bar");
    Indicator = core.indicators:create("DOCHIANCHANNEL", Source, 
        instance.parameters.N,
        instance.parameters.MD);
    if (instance.parameters.stream == "DN") then
        stream = Indicator.DN;
    else
        stream = Indicator.DU;
    end
...
function DoMoveStop(trade)
    if commands[trade.TradeID] ~= nil and not commands[trade.TradeID].Finished or stream:size() == 0 then
        return;
    end
    local newStop = stream[NOW];
