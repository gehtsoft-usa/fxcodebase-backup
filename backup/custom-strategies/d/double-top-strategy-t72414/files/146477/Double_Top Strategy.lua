...
function Init() --The strategy profile initialization
    strategy:name("Double_Top Strategy");
...
function CreateTradingParameters()
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "DTS");
...
function Prepare( nameOnly)
	...
	assert(core.indicators:findIndicator("DOUBLE_TOP") ~= nil, "Please, download and install DOUBLE_TOP.LUA indicator");          
    ...
    Indicator = core.indicators:create("DOUBLE_TOP", Source , 	MinHeight, MaxDist, MinBars   ); 
	first=Source.close:first()+MinBars; 
...
function EntryFunction( id, now,period)
    ...
    if    Indicator.Bottom:hasData(period - MinBars)
	then	
          if Direction then
                BUY();
            else
		...
		Return=true;
    elseif     Indicator.Top:hasData(period - MinBars)
		then
            if Direction then
                SELL();
            else
