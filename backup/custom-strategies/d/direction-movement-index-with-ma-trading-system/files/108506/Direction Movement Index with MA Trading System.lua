...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=63953
...
function Init() --The strategy profile initialization
    strategy:name("Direction Movement Index with MA Trading System")
    ...
    strategy.parameters:addInteger("Period1", "DMI period", "", 5)

    strategy.parameters:addString("Method1", "MA Method", "Method", "WMA")
    ...
    strategy.parameters:addInteger("Period2", "DMI period", "", 11)
...
local first
local Exit
local DMI, DMI_Period, Method1, Method2, Period1, Period2, MA1, MA2, SAR, Step, Max
local Direction
local CustomID
local PositionCap
...
function Prepare(nameOnly)
    ...
    first =
        ...
        MA1.DATA:first(),
        MA2.DATA:first(),
        SAR:getStream(0):first(),
        SAR:getStream(1):first()
    )
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if
        not DMI.DATA:hasData(period) or not MA1.DATA:hasData(period) or not MA2.DATA:hasData(period) or
            (not SAR:getStream(0):first() or not SAR:getStream(1):first())
     then
        return
    end
    ...
    if MA1.DATA[period] > MA2.DATA[period] and DMI.DIP[period] > DMI.DIM[period] and SAR:getStream(1):hasData(period) then
        if Direction then
            BUY()
        else
            SELL()
    ...
    elseif
        MA1.DATA[period] < MA2.DATA[period] and DMI.DIP[period] < DMI.DIM[period] and SAR:getStream(0):hasData(period)
     then
        if Direction then
            SELL()
        else
    ...
    if Exit then
        if SAR:getStream(0):hasData(period) then
            if Direction then
        ...
        if SAR:getStream(1):hasData(period) then
            if Direction then
                if haveTrades("S") then
                    exitSpecific("S")
                    Signal("Close Short")
