...
-- https://fxcodebase.com/code/viewtopic.php?f=31&t=71699
...
function Init() --The strategy profile initialization
    strategy:name("DNC Perfect trend line Strategy");
    ...
    strategy.parameters:addInteger("Period", "DNC Signal Valid Period", "", 5)
		
	strategy.parameters:addInteger("N", "Number of periods", "", 20, 2, 10000)
	strategy.parameters:addString("AC", "Analyze the current period", "", "yes")
	strategy.parameters:addStringAlternative("AC", "no", "", "no")
	strategy.parameters:addStringAlternative("AC", "yes", "", "yes")
	
	
	
	strategy.parameters:addGroup("PTL Calculation");
	strategy.parameters:addColor("SlowLength", "Slow Length", "Slow Length", 7);
	strategy.parameters:addDouble("SlowPipDisplace", "Slow Pip Displace", "Slow PipD isplace", 0);
	strategy.parameters:addColor("FastLength", "Fast Length", "Fast Length", 3);
	strategy.parameters:addDouble("FastPipDisplace", "Fast Pip Displace", "Fast PipD isplace", 0);
...
function CreateTradingParameters()
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "DPS");
...
function Prepare( nameOnly)
	...
	assert(core.indicators:findIndicator("PERFECT TREND LINE") ~= nil, "Please, download and install PERFECT TREND LINE.LUA indicator");   
    ...
    DNC = core.indicators:create("DNC", Source, N,AC   );
    PTL = core.indicators:create("PERFECT TREND LINE", Source, SlowLength, SlowPipDisplace,FastLength, FastPipDisplace, true   );
	first=math.max(DNC.DATA:first(),PTL.DATA:first()); 
...
function ItIsUP(period)

local Return=0;


		for i= period, period- Period+1, -1 do
			if Source.low[period] <= DNC.DN[i]  then
			Return=1;
			end
...
function ItIsDOWN(period)

local Return=0;


		for i= period, period- Period+1, -1 do
			if Source.high[period] >= DNC.DU[i] 
			then
			Return=-1;
			end
