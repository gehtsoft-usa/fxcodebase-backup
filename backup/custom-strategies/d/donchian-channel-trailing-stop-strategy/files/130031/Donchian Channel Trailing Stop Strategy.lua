...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=69178
...
function Init() --The strategy profile initialization
    strategy:name("Donchian Channel Trailing Stop Strategy")
    ...
    strategy.parameters:addInteger("Period", "Min Max Period", "", 20)
...
function Prepare(nameOnly)
    ...
    assert(
        core.indicators:findIndicator("DONCHIAN CHANNEL TRAILING STOP") ~= nil,
        "Please, download and install DONCHIAN CHANNEL TRAILING STOP.LUA indicator"
    )
    ...
    Short = core.indicators:create("MVA", Source.close, instance.parameters.Short)
    Indicator = core.indicators:create("DONCHIAN CHANNEL TRAILING STOP", Source, instance.parameters.Period)
    first = math.max(Short.DATA:first(), Indicator.DATA:first())
...
function EntryFunction(id, now, period)
    ...
    if LastEntry == Source:serial(period) or period < instance.parameters.Period then
        return
    end

    local Return = false

    local min, max = mathex.minmax(Source, period - instance.parameters.Period + 1, period)
    local Percantage = (Source.close[period] - min) / ((max - min) / 100)
    ...
    if Source.close[period] > Indicator.DATA[period] and Source.close[period - 1] <= Indicator.DATA[period - 1] then
        if Direction then
            BUY()
        else
            SELL()
        ...
        Return = true
    elseif Source.close[period] < Indicator.DATA[period] and Source.close[period - 1] >= Indicator.DATA[period - 1] then
        if Direction then
            SELL()
        else
            BUY()
