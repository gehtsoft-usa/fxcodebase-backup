...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=63481
...
function Init() --The strategy profile initialization
    strategy:name("DAY CHANGE WITH SAR STRATEGY")
    ...
    strategy.parameters:addDouble("BL", "Buy Level  ", "Level", 0.25)
    strategy.parameters:addDouble("SL", "Sell Level ", "Level", -0.25)

    strategy.parameters:addGroup("SAR Calculation")
    strategy.parameters:addDouble("Step", "Step  ", "Level", 0.02)
    strategy.parameters:addDouble("Max", "Max  ", "Level", 0.2)
...
local Source, TickSource, DaySource
...
function Prepare(nameOnly)
    ...
    DaySource = ExtSubscribe(2, nil, "D1", instance.parameters.Type == "Bid", "bar")
    ...
    Short[1] = SAR:getStream(0)
    Short[2] = SAR:getStream(1)
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if ExecutionType == "Live" and id == 1 then
        period1 = core.findDate(Source.close, TickSource:date(period), false)
        period2 = core.findDate(DaySource.close, TickSource:date(period), false)
    end
    ...
    if ExecutionType == "Live" then
        ...
        period1 = period
        period2 = core.findDate(DaySource.close, Source:date(period1), false)
    end
    ...
    if
        ((DaySource.close[period2] - DaySource.open[period2]) / (DaySource.open[period2] / 100)) > BL and
            not (Short[2]:hasData(period - 1)) and
            Short[2]:hasData(period)
     then
        if Direction then
            BUY()
        else
    ...
    elseif
        ((DaySource.close[period2] - DaySource.open[period2]) / (DaySource.open[period2] / 100)) < SL and
            not (Short[1]:hasData(period - 1)) and
            Short[1]:hasData(period)
     then
        if Direction then
            SELL()
        else
