...
local Modules = {};
function Init()
    strategy:name("DMI+RSI");
    strategy:description("");
    strategy.parameters:addInteger("lene", "lene", "", 12);
    strategy.parameters:addInteger("lensig", "lensig", "", 12);
    strategy.parameters:addInteger("len", "Length", "", 12);
...
function Prepare(name_only)
    ...
    assert(core.indicators:findIndicator("DMI_RSI_INDICATOR_SIGNAL") ~= nil, "DMI_RSI_INDICATOR_SIGNAL" .. " indicator must be installed");
    indi = core.indicators:create("DMI_RSI_INDICATOR_SIGNAL", trading_logic.MainSource, lene, lensig, len)
...
function EntryFunction(source, period)
    ...
    if indi.DATA[period] == 1 then
        local trades = trading:FindTrade():WhenSide("S"):All()
    ...
    elseif indi.DATA[period] == -1 then
        local trades = trading:FindTrade():WhenSide("B"):All()
        for i, trade in ipairs(trades) do
            trading:Close(trade)
        end
        local command = trading:MarketOrder(source:instrument())
