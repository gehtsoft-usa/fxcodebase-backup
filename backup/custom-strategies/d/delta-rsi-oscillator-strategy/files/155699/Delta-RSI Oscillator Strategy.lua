-- Available @ https://fxcodebase.com/code/viewtopic.php?f=31&t=74959
...
function Init() --The strategy profile initialization
    strategy:name("Delta-RSI Oscillator");
    ...
    strategy.parameters:addInteger("Period", "RSI Period", "", 14, 1, 2000);   
    strategy.parameters:addInteger("Degree", "Polynomial Order", "", 2, 1, 2000); 
    strategy.parameters:addInteger("Window", "Frame Length (must be greater than Order)", "", 18, 2, 2000);	
    strategy.parameters:addInteger("SignalLength", "D-RSI Signal Length", "", 9, 2, 2000);	 
    strategy.parameters:addBoolean("IsSignal", "Show D-RSI Signal Line", "", true);
    strategy.parameters:addBoolean("IsRSIFittingError", "Show RSIFittingError Line", "", false);
    strategy.parameters:addBoolean("usenrmse", "Filter by Means of Root-Mean-Square Error of RSI Fitting", "", false);
    strategy.parameters:addDouble("rmse_thrs", "RSI fitting Error Threshold", "", 10, 0, 2000);	 
	
    strategy.parameters:addString("buycond", "Long Condition", "", "Signal");
	strategy.parameters:addStringAlternative("buycond", "Direction Change", "", "DirectionChange");
    strategy.parameters:addStringAlternative("buycond", "Zero Crossing", "", "ZeroCrossing");
    strategy.parameters:addStringAlternative("buycond", "Signal", "", "Signal"); 
	
	
    strategy.parameters:addString("goshort", "Long Condition", "", "Signal");
	strategy.parameters:addStringAlternative("goshort", "Direction Change", "", "DirectionChange");
    strategy.parameters:addStringAlternative("goshort", "Zero Crossing", "", "ZeroCrossing");
    strategy.parameters:addStringAlternative("goshort", "Signal", "", "Signal");
	
    strategy.parameters:addString("endcond", "Exit Condition", "", "Signal");
	strategy.parameters:addStringAlternative("endcond", "Direction Change", "", "DirectionChange");
    strategy.parameters:addStringAlternative("endcond", "Zero Crossing", "", "ZeroCrossing");
    strategy.parameters:addStringAlternative("endcond", "Signal", "", "Signal");	
...
function Prepare( nameOnly)
	...
	--Indicator parameters
	 usenrmse= instance.parameters.usenrmse;
     rmse_thrs= instance.parameters.rmse_thrs /100;
     buycond= instance.parameters.buycond;
     sellcond= instance.parameters.sellcond;
     endcond= instance.parameters.endcond; 
	...
	assert(core.indicators:findIndicator("DELTA-RSI OSCILLATOR") ~= nil, "Please, download and install DELTA-RSI OSCILLATOR.LUA indicator");          
    ...
    Indicator = core.indicators:create("DELTA-RSI OSCILLATOR", Source ,instance.parameters.Period,instance.parameters.Degree, instance.parameters.Window, instance.parameters.SignalLength, true, true); 
...
--	period3= core.findDate (Source3, TickSource:date(period), false);
	...
	filter_rmse = Indicator.RSIFittingError[period]<rmse_thrs
	 
	dirchangeup = (Indicator.PolynomialDifferentiator[period]>Indicator.PolynomialDifferentiator[period-1]) and (Indicator.PolynomialDifferentiator[period-1]<Indicator.PolynomialDifferentiator[period-2]) and Indicator.PolynomialDifferentiator[period-1]<0.0
	dirchangedw = (Indicator.PolynomialDifferentiator[period]<Indicator.PolynomialDifferentiator[period-1]) and (Indicator.PolynomialDifferentiator[period-1]>Indicator.PolynomialDifferentiator[period-2]) and Indicator.PolynomialDifferentiator[period-1]>0.0
	crossup = core.crossesOver(Indicator.PolynomialDifferentiator,0.0, period)
	crossdw = core.crossesUnder(Indicator.PolynomialDifferentiator,0.0, period)
	crosssignalup = core.crossesOver(Indicator.PolynomialDifferentiator,Indicator.Signal, period)
	crosssignaldw = core.crossesUnder(Indicator.PolynomialDifferentiator,Indicator.Signal, period)   	  
  


    if buycond=="DirectionChange" and dirchangedw then
	endlong=true;
    elseif buycond=="ZeroCrossing" and crossdw then
	endlong=true;	
	elseif buycond=="Signal" and crosssignaldw then
	endlong=true	
	else
	endlong=false
	end
	
    if sellcond=="DirectionChange" and dirchangeup then
	endshort=true;
    elseif sellcond=="ZeroCrossing" and crossup then
	endshort=true;
    elseif sellcond=="Signal" and  crosssignalup then	
	endshort=true	
	else
	endshort=false
	end	
		            ...
		            if    endlong 
					and ( not usenrmse or (usenrmse and filter_rmse))
					then
...
function EntryFunction( id, now,period)
    ...
    if buycond=="DirectionChange" and dirchangeup then
	golong=true;
    elseif buycond=="ZeroCrossing" and crossup then
	golong=true;	
	elseif buycond=="Signal" and crosssignalup then
	golong=true	
	else
	golong=false
	end
	
    if sellcond=="DirectionChange" and dirchangedw then
	goshort=true;
    elseif sellcond=="ZeroCrossing" and crossdw then
	goshort=true;
    elseif sellcond=="Signal" and  crosssignaldw then	
	goshort=true	
	else
	goshort=false
	end	
