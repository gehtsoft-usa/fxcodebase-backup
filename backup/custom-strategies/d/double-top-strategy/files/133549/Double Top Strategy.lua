...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=69799
...
local STRATEGY_NAME = "Double Top Strategy";
function CreateParameters() 
    AddAction(1, "Top");
    AddAction(2, "Bottom");
    AddAction(3, "Double Top");
    AddAction(4, "Double Bottom");
    strategy.parameters:addInteger("MinHeight", "Min. height of peak (in pips)", "", 10)
    strategy.parameters:addInteger("MaxDist", "Max. distance between peaks (in bars)", "", 20)
    strategy.parameters:addInteger("MinBars", "Min. number of bars after the peak (in bars)", "", 3)
end

local actions = {};
local indi;
function CreateEntryIndicators(source)
    local profile = core.indicators:findIndicator("DOUBLE_TOP ALERT");
    assert(profile ~= nil, "Please, download and install " .. "DOUBLE_TOP ALERT" .. ".LUA indicator");
    ...
    indi = core.indicators:create("DOUBLE_TOP ALERT", source, indicatorParams)

    actions[1] = 
    {
        IsPass = function (period)
            if period < instance.parameters.MinBars then
                return false;
            end
            return indi.Top:hasData(period - instance.parameters.MinBars);
        end
    };
    actions[2] = 
    {
        IsPass = function (period)
            ...
            return indi.Bottom:hasData(period - instance.parameters.MinBars);
        end
    };
    actions[3] = 
    {
        IsPass = function (period)
            ...
            return indi.DoubleTop:hasData(period - instance.parameters.MinBars);
        end
    };
    actions[4] = 
    {
        IsPass = function (period)
            ...
            return indi.DoubleBottom:hasData(period - instance.parameters.MinBars);
        end
    };
end
