...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=2572
...
function Init() --The strategy profile initialization
    ...
    strategy.parameters:addInteger("Period", "DMI Period", "", 14)
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if DMI.DIP[period] > DMI.DIM[period] and DMI.DIP[period - 1] <= DMI.DIM[period - 1] then
        if Direction then
            BUY()
        else
            SELL()
    ...
    elseif DMI.DIP[period] < DMI.DIM[period] and DMI.DIP[period - 1] >= DMI.DIM[period - 1] then
        if Direction then
            SELL()
        else
            BUY()
