...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=62622
...
function Init() --The strategy profile initialization
    strategy:name("Delta Indicator Strategy")
...
function Prepare(nameOnly)
    ...
    Indicator = core.indicators:create("DELTA", Source)
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if Indicator.Price[period] > Indicator.Delta[period] and Indicator.Price[period - 1] <= Indicator.Delta[period - 1] then
        if Direction then
            BUY()
        else
            SELL()
    ...
    elseif
        Indicator.Price[period] < Indicator.Delta[period] and Indicator.Price[period - 1] >= Indicator.Delta[period - 1]
     then
        if Direction then
            SELL()
        else
