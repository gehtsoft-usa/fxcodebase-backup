...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=70733
...
function Init() --The strategy profile initialization
    strategy:name("Distance from the moving average Strategy");
    ...
    strategy.parameters:addString("Method1", "Method1", "Method", "MVA");
...
function CreateTradingParameters()
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "DFMAS");
...
local Indicator;
local Period1, Method1 , Period2, Method2;
...
function Prepare( nameOnly)
	...
	assert(core.indicators:findIndicator("DISTANCE FROM THE MOVING AVERAGE") ~= nil, "Please, download and install DISTANCE FROM THE MOVING AVERAGE.LUA indicator");          
   ...
   Indicator = core.indicators:create("DISTANCE FROM THE MOVING AVERAGE", Source.close, Period1, Method1, true, Period2, Method2, "Value"  );
...
function EntryFunction( id, now,period)
    ...
    if   Indicator.Distance[period]> Indicator.Smoothed[period]
	and  Indicator.Distance[period-1]<= Indicator.Smoothed[period-1]
	then	
          if Direction then
                BUY();
            else
		...
		Return=true;
    elseif    Indicator.Distance[period]< Indicator.Smoothed[period]
	and  Indicator.Distance[period-1]>= Indicator.Smoothed[period-1]
		then
            if Direction then
                SELL();
