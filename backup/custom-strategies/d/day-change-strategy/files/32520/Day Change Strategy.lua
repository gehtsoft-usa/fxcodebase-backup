...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=17969
...
function Init() --The strategy profile initialization
    strategy:name("Day Change Strategy")
    ...
    strategy.parameters:addString("Mode", "The Indicator Mode", "", "N")
    strategy.parameters:addStringAlternative("Mode", "Net Change", "", "N")
    strategy.parameters:addStringAlternative("Mode", "Percent Change", "", "P")

    strategy.parameters:addDouble("Level", "Level", "", 0, 0, 1000)

    strategy.parameters:addBoolean("Filter", "Filter", "", false)
...
function Prepare(nameOnly)
    ...
    assert(core.indicators:findIndicator("DAY_CHANGE") ~= nil, "Please, download and install DAY_CHANGE.LUA indicator")
    ...
    Indicator = core.indicators:create("DAY_CHANGE", Source, Mode)

    assert(core.indicators:findIndicator(Method) ~= nil, Method .. " indicator must be installed");
    MA = core.indicators:create(Method, Source[Price], Period)

    first = math.max(Indicator.DATA:first(), MA.DATA:first()) + 1
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if core.crossesOver(Indicator.DATA, Level, period) and (Source.close[period] > MA.DATA[period] or not Filter) then
        if Direction then
            BUY()
        else
            SELL()
    ...
    elseif
        core.crossesUnder(Indicator.DATA, -1 * Level, period) and (Source.close[period] < MA.DATA[period] or not Filter)
     then
        if Direction then
            SELL()
        else
