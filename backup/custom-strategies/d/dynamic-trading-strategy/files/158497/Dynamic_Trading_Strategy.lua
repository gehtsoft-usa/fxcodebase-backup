...
local STRATEGY_NAME = "Dynamic Trading Strategy";
function CreateParameters() 
    --create your algorithm strategy.parameters here
    strategy.parameters:addInteger("param1", "Lookback Period for Key Levels", "", 20);
    strategy.parameters:addInteger("param2", "ATR Period", "", 14);
    strategy.parameters:addDouble("param3", "SL ATR Multiplier", "", 1.5);
    strategy.parameters:addDouble("param4", "TP1 ATR Multiplier", "", 1.5);
    strategy.parameters:addDouble("param5", "TP2 ATR Multiplier", "", 2.0);
    strategy.parameters:addDouble("param6", "Reward to Risk Ratio", "", 2.0);
end
...
function CreateEntryIndicators(source)
    ...
    assert(core.indicators:findIndicator("DYNAMIC TRADING INDICATOR") ~= nil, "Please, download and install DYNAMIC TRADING INDICATOR.LUA indicator");
    --create your indicators here
    indi = core.indicators:create("DYNAMIC TRADING INDICATOR", source, 
        instance.parameters.param1,
        instance.parameters.param2,
        instance.parameters.param3,
        instance.parameters.param4,
        instance.parameters.param5,
        instance.parameters.param6);
end
...
function FindBuyEntryPeriod(period)
    for i = 0, 10 do
        if indi.buy_entry:hasData(period - i) then
            return period - i;
        end
    end
    return nil;
end
function FindSellEntryPeriod(period)
    for i = 0, 10 do
        if indi.sell_entry:hasData(period - i) then
            return period - i;
        end
    end
    return nil;
...
function IsEntryLong(source, period)
    local entry_period = FindBuyEntryPeriod(period);
    if entry_period == nil then
        return false;
    end
    return core.crossesUnder(source.close, indi.buy_entry[entry_period], period);
end
function IsEntryShort(source, period)
    local entry_period = FindSellEntryPeriod(period);
    if entry_period == nil then
        return false;
    end
    return core.crossesOver(source.close, indi.sell_entry[entry_period], period);
end
...
local use_stop, use_limit, entry_execution_type, use_trailing, trailing, use_position_limit, position_limit;
...
function OpenTrade(side)
    ...
    if use_stop then
        if side == "B" then
            local buy_entry = FindBuyEntryPeriod(NOW);
            valuemap.RateStop = indi.buy_sl[buy_entry];
        else
            local sell_entry = FindSellEntryPeriod(NOW);
            valuemap.RateStop = indi.sell_sl[sell_entry];
        end
    ...
    if use_limit then
        if side == "B" then
            local buy_entry = FindBuyEntryPeriod(NOW);
            valuemap.RateLimit = indi.buy_tp1[buy_entry];
        else
            local sell_entry = FindSellEntryPeriod(NOW);
            valuemap.RateLimit = indi.sell_tp1[sell_entry];
        end
    end
