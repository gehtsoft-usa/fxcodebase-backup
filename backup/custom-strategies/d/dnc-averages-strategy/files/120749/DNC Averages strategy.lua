...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=66593
...
function Init() --The strategy profile initialization
    strategy:name("DNC Averages strategy");
	...
	strategy.parameters:addGroup("DNC Calculation");
    strategy.parameters:addInteger("N", "Number of periods", "", 20, 2, 10000);
 
	
	
 
    strategy.parameters:addString("ExitType", "Exit Type", "", "Any");
...
function CreateTradingParameters()
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "DNCAS");
...
local MA, Method, Period,Lookback;
local DNC, N;
local ExitType;
function Prepare( nameOnly)
    ...
    MA = core.indicators:create("AVERAGES", Source.close, Method, Period ); 
    assert(core.indicators:findIndicator("DNC") ~= nil, "DNC" .. " indicator must be installed");
	DNC = core.indicators:create("DNC", Source , N, "yes", "yes", false );
	first=math.max(MA.DATA:first(), DNC.DM:first()); 
...
function ExitFunction( now,period)
	 ...
	 return;
		            ...
		            if    (Source.close[period] < DNC.DM[period] and Source.close[period-1] >= DNC.DM[period-1] and ExitType~= "Top/Bottom")
					or   (Source.close[period] < DNC.DU[period] and Source.close[period-1] >= DNC.DU[period-1] and ExitType~= "Central" )
					then
					  ...
					  if    (Source.close[period] > DNC.DM[period] and Source.close[period-1] <= DNC.DM[period-1] and ExitType~= "Top/Bottom")
					  or  (Source.close[period] > DNC.DN[period] and Source.close[period-1] <= DNC.DN[period-1]  and ExitType~= "Central")
					  then
...
function ItIsTop(period)

local Return =false;

		for i= period, first,-1 do

				if Source.high[i]==DNC.DU[i] then
				Return = true;
				break;
				end
				
				
				if Source.low[i]==DNC.DN[i] then
				Return = true;
				break;
				end
		end
...
function EntryFunction( now,period)
	...
	and MA.DATA[period-1]<= MA.DATA[period-2]
	and ItIsBottom(period)
	then	
          if Direction then
                BUY();
            else
