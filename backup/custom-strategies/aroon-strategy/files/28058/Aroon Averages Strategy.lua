...
function Init() --The strategy profile initialization
    strategy:name("Aroon Averages Strategy")
    ...
    strategy.parameters:addInteger("Period", " Period", " Period", 25)
    strategy.parameters:addDouble("Buy", "Buy level", " Period", 50)
    strategy.parameters:addDouble("Sell", "Sell level", " Period", 50)
    strategy.parameters:addBoolean("Consolidation", "Exit On Consolidation", "", true)
...
function Prepare(nameOnly)
    ExitBuy = instance.parameters.ExitBuy
    ExitSell = instance.parameters.ExitSell
    Period = instance.parameters.Period
    Buy = instance.parameters.Buy
    Sell = instance.parameters.Sell
    Consolidation = instance.parameters.Consolidation
    ...
    name = name .. ", " .. Period .. ", " .. Method .. ", " .. PERIOD

    assert(core.indicators:findIndicator("AROON") ~= nil, "Please, download and install AROON.LUA indicator")
    ...
    Indicator[1] = core.indicators:create("AROON", Source, Period)
    ...
    Short[3] = Indicator[2]:getStream(0)
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if
        (Short[1][period] > Buy and core.crossesUnder(Short[2], Sell, period)) or
            (core.crossesOver(Short[1], Buy, period) and Short[2][period] < Sell)
     then
        if Indicator[2].DATA:colorI(period) == core.rgb(0, 255, 0) then
            if Direction then
                BUY()
            else
                SELL()
    ...
    elseif
        (Short[2][period] > Buy and core.crossesUnder(Short[1], Sell, period)) or
            (core.crossesOver(Short[2], Buy, period) and Short[1][period] < Sell)
     then
        if Indicator[2].DATA:colorI(period) == core.rgb(255, 0, 0) then
            if Direction then
                SELL()
            else
                BUY()
    ...
    if Consolidation then
        if Short[1][period] < Buy or Short[2][period] > Sell or Indicator[2].DATA:colorI(period) == core.rgb(255, 0, 0) then
            if Direction then
        ...
        elseif
            Short[2][period] < Buy or Short[1][period] > Sell or Indicator[2].DATA:colorI(period) == core.rgb(0, 255, 0)
         then
            if Direction then
                if haveTrades("S") then
                    exit("S")
