...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=6308
...
function Init() --The strategy profile initialization
    strategy:name("RSI-MACD-BB strategy")
    strategy:description("RSI-MACD-BB strategy")
    strategy:setTag("NonOptimizableParameters", "Email,SendEmail,SoundFile,RecurrentSound,PlaySound, ShowAlert")
    strategy:setTag("Version", "1.1")

    strategy.parameters:addGroup("RSI indicator parameters")
    ...
    strategy.parameters:addDouble("RSI_Level_B", "RSI level for long", "RSI level for long", 30)
    strategy.parameters:addDouble("RSI_Level_S", "RSI level for short", "RSI level for short", 70)
    strategy.parameters:addDouble("BB_Gap_B", "BB gap for long", "BB gap for long", 40)
    strategy.parameters:addDouble("BB_Gap_S", "BB gap for short", "BB gap for short", 20)
...
function Prepare(nameOnly)
    ...
    BB = core.indicators:create("BB", Source, instance.parameters.Period, instance.parameters.Deviation, false)
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if (RSI.DATA:first() > (period - 1)) then
        return
    end
    ...
    if
        RSI.DATA[period] > instance.parameters.RSI_Level_B and MACD.MACD[period] > MACD.SIGNAL[period] and
            BB.TL[period] - BB.BL[period] >= instance.parameters.BB_Gap_B * pipSize and
            Source[period] > BB.AL[period]
     then
    ...
    if
        RSI.DATA[period] < instance.parameters.RSI_Level_S and MACD.MACD[period] < MACD.SIGNAL[period] and
            BB.TL[period] - BB.BL[period] >= instance.parameters.BB_Gap_S * pipSize and
            Source[period] < BB.AL[period]
     then
        if instance.parameters.TypeSignal == "direct" then
            MustS = true
