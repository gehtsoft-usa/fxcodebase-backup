...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=62140
...
function Init() --The strategy profile initialization
    ...
    strategy.parameters:addInteger("FastPeriod", "Fast MA Period", "MA Period", 21)
    strategy.parameters:addString("FastMethod", "Fast MA Method", "Method", "EMA")
    ...
    strategy.parameters:addInteger("SlowPeriod", "Slow MA Period", "MA Period", 50)
    strategy.parameters:addString("SlowMethod", "Slow MA Method", "Method", "EMA")
    ...
    strategy.parameters:addInteger("OBVPeriod", "OVB MA Period", "MA Period", 21)
    strategy.parameters:addString("OBVMethod", "OVB MA Method", "Method", "EMA")
    strategy.parameters:addStringAlternative("OBVMethod", "MVA", "MVA", "MVA")
    strategy.parameters:addStringAlternative("OBVMethod", "EMA", "EMA", "EMA")
    strategy.parameters:addStringAlternative("OBVMethod", "LWMA", "LWMA", "LWMA")
    strategy.parameters:addStringAlternative("OBVMethod", "TMA", "TMA", "TMA")
    strategy.parameters:addStringAlternative("OBVMethod", "SMMA", "SMMA", "SMMA")
    strategy.parameters:addStringAlternative("OBVMethod", "KAMA", "KAMA", "KAMA")
    strategy.parameters:addStringAlternative("OBVMethod", "VIDYA", "VIDYA", "VIDYA")
    strategy.parameters:addStringAlternative("OBVMethod", "WMA", "WMA", "WMA")
...
local first
local FastPeriod, FastMethod
local SlowPeriod, SlowMethod
local Indicator = {}
local Short = {}
local Direction
local CustomID
local OBV, FastMA, SlowMA
local OBVMA, OBVPeriod, OBVMethod
...
function Prepare(nameOnly)
    ...
    OBVPeriod = instance.parameters.OBVPeriod
    OBVMethod = instance.parameters.OBVMethod
    ...
    assert(core.indicators:findIndicator(OBVMethod) ~= nil, OBVMethod .. " indicator must be installed");
    OBVMA = core.indicators:create(OBVMethod, OBV.DATA, OBVPeriod)
    assert(core.indicators:findIndicator(SlowMethod) ~= nil, SlowMethod .. " indicator must be installed");
    SlowMA = core.indicators:create(SlowMethod, Source.close, SlowPeriod)
    assert(core.indicators:findIndicator(FastMethod) ~= nil, FastMethod .. " indicator must be installed");
    FastMA = core.indicators:create(FastMethod, Source.close, FastPeriod)
    first = math.max(OBVMA.DATA:first(), SlowMA.DATA:first(), FastMA.DATA:first()) + 1
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    OBV:update(core.UpdateLast)
    OBVMA:update(core.UpdateLast)
    ...
    if
        FastMA.DATA[period] > SlowMA.DATA[period] and FastMA.DATA[period - 1] <= SlowMA.DATA[period - 1] and
            OBV.DATA[period] > OBVMA.DATA[period]
     then
        if Direction then
            BUY()
        else
    ...
    if Exit then
        if OBV.DATA[period] < OBVMA.DATA[period] then
            if Direction then
