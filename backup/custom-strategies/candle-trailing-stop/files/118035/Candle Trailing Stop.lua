...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=65802
...
function Init()
    strategy:name("Candle Trailing Stop");
    ...
    strategy.parameters:addInteger("shift", "Shift from high/low (pips)", "", 2, 0.1, 1000);
    ...
    if trade ~= nil then
        if trade.BS == "B" then
            if source.close[period] > source.open[period] then
                trading:MoveStop(source.low[period] - shift * source:pipSize(), trade);
            end
        else
            if source.close[period] < source.open[period] then
                trading:MoveStop(source.high[period] + shift * source:pipSize(), trade);
            end
        end
    end
end
