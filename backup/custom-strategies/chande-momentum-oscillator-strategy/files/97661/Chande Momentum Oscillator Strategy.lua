...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=61589
...
function Init() --The strategy profile initialization
    strategy:name("Chande Momentum Oscillator Strategy")
    ...
    strategy.parameters:addInteger("Period", "TMA period", "", 9)
...
function Prepare(nameOnly)
    ...
    Indicator = core.indicators:create("CMO", Source[Price], Period)
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if core.crossesOver(Indicator.DATA, 0, period) then
        if Direction then
            BUY()
        else
            SELL()
    ...
    elseif core.crossesUnder(Indicator.DATA, 0, period) then
        if Direction then
            SELL()
        else
            BUY()
