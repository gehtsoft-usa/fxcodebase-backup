...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=61992
...
function Init() --The strategy profile initialization
    strategy:name("Keltner Band MA Strategy")
    ...
    strategy.parameters:addInteger("Period1", "Period", "Period", 7)
    ...
    strategy.parameters:addGroup("Keltner Calculation")
...
local first
local Price
local NM, NB, F, SRC, MS, MV, Period
...
function Prepare(nameOnly)
    ...
    KELTNER = core.indicators:create("KELTNER", Source, NM, NB, F, SRC, MS, MV)
    ...
    first = math.max(KELTNER.DATA:first(), MA1.DATA:first(), MA2.DATA:first()) + 1
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    KELTNER:update(core.UpdateLast)

    if period < first then
        return
    end

    if core.crossesOver(MA1.DATA, MA2.DATA, period) and Source.close[period] < KELTNER.L[period] then
        if Direction then
            BUY()
        else
            SELL()
        end
    elseif core.crossesUnder(MA1.DATA, MA2.DATA, period) and Source.close[period] > KELTNER.H[period] then
        if Direction then
            SELL()
        else
            BUY()
