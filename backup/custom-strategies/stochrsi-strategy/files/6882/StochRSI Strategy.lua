...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=2990
...
function Init()
    strategy:name("StochRSI strategy")
    strategy:description("StochRSI strategy")
    ...
    strategy.parameters:addInteger("Frame", "StochRSI Period", "", 14, 2, 10000)
    ...
    strategy.parameters:addInteger("BuyLevel", "BuyLevel", "", 40, 0, 100)
    strategy.parameters:addInteger("SellLevel", "SellLevel", "", 60, 0, 100)
    Trading_Parameters()
end
...
function Prepare(nameOnly)
    ...
    elseif PriceType == "M" then
        Price = gSource.median
    elseif PriceType == "T" then
        Price = gSource.typical
    elseif PriceType == "W" then
        Price = gSource.weighted
    else
        Price = gSource.close
    end

    assert(core.indicators:findIndicator("STOCHRSI") ~= nil, "Please, download and install StochRSI indicator")

    Indicator = core.indicators:create("STOCHRSI", Price, Frame, K, KS, D)
...
function ExtUpdate(id, source, period)
    ...
    if core.crossesOver(Indicator.DATA, BuyLevel, period) then
        BUY()
    elseif core.crossesUnder(Indicator.DATA, SellLevel, period) then
        SELL()
    end

    if core.crossesUnder(Indicator.DATA, BuyLevel, period) then
        if haveTrades("B") then
            exit("B")
        end
    elseif core.crossesOver(Indicator.DATA, SellLevel, period) then
