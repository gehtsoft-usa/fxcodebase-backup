...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=69943
...
function Init() --The strategy profile initialization
    strategy:name("Ultimate Oscillator Strategy");
    ...
    strategy.parameters:addInteger("S", "Short Period", "", 7);
    strategy.parameters:addInteger("M", "Medium Period", "", 14);
    strategy.parameters:addInteger("L", "Long Period", "", 28);
...
--	  Exit= instance.parameters.Exit;
    ...
    PrepareTrading();
	 ...
	 assert(core.indicators:findIndicator("UOSC") ~= nil, "Please, download and install UOSC.LUA indicator");          
   ...
   Indicator = core.indicators:create("UOSC", Source, instance.parameters.S, instance.parameters.M, instance.parameters.L  );
...
function EntryFunction( id, now,period)
    ...
    if  FindFirst("Long", period)
	and Indicator.DATA[period]<OS
	then	
          if Direction then
                BUY();
            else
		...
		Return=true;
    elseif FindFirst("Short", period)
	and Indicator.DATA[period] > OB
	then
            if Direction then
                SELL();
            else
...
function FindFirst(Trade_Direction, period)
    ...
    if Trade_Direction == "Long" then
			  ...
			  for i = period, first, -1 do
				  ...
				  if Indicator.DATA[i] > OS then
				  break;
				  end
				  
				  if Indicator.DATA[i] < OS 
				  and Source.close[i]> Source.open[i]
				  then
				  Count=Count+1;
				  end
	...
	elseif Trade_Direction == "Short" then
 
                 for i = period, first, -1 do
			  
				  if Indicator.DATA[i] < OB then
				  break;
				  end
				  
				  if Indicator.DATA[i] > OB 
				  and Source.close[i]< Source.open[i]
				  then
				  Count=Count+1;				 
				  end
