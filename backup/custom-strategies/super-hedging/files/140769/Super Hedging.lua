...
--http://fxcodebase.com/code/viewtopic.php?f=31&t=70935
...
local STRATEGY_NAME = "Super Hedging Strategy";
function CreateParameters() 
    strategy.parameters:addInteger("ma_period", "MA Period", "", 9);
    AddAverages("ma_method", "MA Method", "EMA");
    strategy.parameters:addString("side1", "Side 1", "", "B");
    strategy.parameters:addStringAlternative("side1", "Buy", "", "B");
    strategy.parameters:addStringAlternative("side1", "Sell", "", "S");
    strategy.parameters:addString("instrument2", "Instrument2", "", "EUR/USD");
    strategy.parameters:setFlag("instrument2", core.FLAG_INSTRUMENTS);
    strategy.parameters:addString("side2", "Side 2", "", "B");
    strategy.parameters:addStringAlternative("side2", "Buy", "", "B");
    strategy.parameters:addStringAlternative("side2", "Sell", "", "S");
    strategy.parameters:addDouble("profit_target", "Total Profit", "", 100);
end

local SOURCE2_ID = 4;
local ma11, ma12, ma21, ma22, side1, side2, source2, custom_id, profit_target;
function CreateEntryIndicators(source)
    ma11 = CreateAverages(instance.parameters.ma_period, instance.parameters.ma_method, source.close);
    ma12 = CreateAverages(instance.parameters.ma_period, instance.parameters.ma_method, source.open);
    source2 = ExtSubscribe(SOURCE2_ID, instance.parameters.instrument2, instance.parameters.timeframe, instance.parameters.type, "bar")
    ma21 = CreateAverages(instance.parameters.ma_period, instance.parameters.ma_method, source2.close);
    ma22 = CreateAverages(instance.parameters.ma_period, instance.parameters.ma_method, source2.open);
    side1 = instance.parameters.side1;
    side2 = instance.parameters.side2;
    profit_target = instance.parameters.profit_target;
end

function CloseOnProfit()
    local enum = core.host:findTable("trades"):enumerator();
    local trade = enum:next();
    local profit = 0;
    while trade ~= nil do
        if trade.QTXT == custom_id then
            profit = profit + trade.GrossPL;
        end
        trade = enum:next();
    end
    if profit > profit_target then
        local enum = core.host:findTable("trades"):enumerator();
        local trade = enum:next();
        while trade ~= nil do
            if trade.QTXT == custom_id then
...
function IsEntry(source, period)
    if not ma11.DATA:hasData(source:size() - 1) or not ma21.DATA:hasData(source2:size() - 1) then
        return false;
    end
    if not core.crossesUnder(ma11.DATA, ma12.DATA, source:size() - 1) 
        and not core.crossesOver(ma21.DATA, ma22.DATA, source2:size() - 1)
    then
        return false;
    end
    return ma11.DATA[NOW] < ma12.DATA[NOW] and ma21.DATA[NOW] > ma22.DATA[NOW];
end
...
local main_source;
local base_size, offer_id, Account, Amount, amount_type, AllowTrade, AllowedSide;
    ...
    if IsEntry(main_source, entry_period) and last_entry ~= main_source:date(NOW) then
        if AllowTrade then
            ...
            if closed_positions > 0 or not PositionsLimitHit() then
                OpenTrade(main_source:instrument(), side1);
                OpenTrade(source2:instrument(), side2);
            end
        end
        Signal("Entry", main_source);
...
function OpenTrade(symbol, side)
    ...
    valuemap.OfferID = core.host:findTable("offers"):find("Instrument", symbol).OfferID;
    valuemap.AcctID = Account;
    if amount_type == "lots" then
        valuemap.Quantity = Amount * core.host:execute("getTradingProperty", "baseUnitSize", symbol, Account);
    else
        local equity = core.host:findTable("accounts"):find("AccountID", valuemap.AcctID).Equity;
        local used_equity = equity * Amount / 100.0;
        local emr = core.host:getTradingProperty("EMR", symbol, valuemap.AcctID);
        valuemap.Quantity = math.floor(used_equity / emr) * core.host:execute("getTradingProperty", "baseUnitSize", symbol, Account);
    end
