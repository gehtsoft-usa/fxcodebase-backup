...
--http://fxcodebase.com/code/viewtopic.php?f=31&t=65831
...
function Init()
    strategy:name("Trailing Bar Stop MTF");
    ...
    strategy.parameters:addInteger("target_level", "Target Profit Level, pips", "", 0);
    strategy.parameters:addDouble("buffer_zone", "Buffer Zone, pips", "", 3);
    strategy.parameters:addString("bars_filter", "Bars Filter", "", "none");
    strategy.parameters:addStringAlternative("bars_filter", "None", "", "none");
    strategy.parameters:addStringAlternative("bars_filter", "Open-close", "", "oc");
    strategy.parameters:addStringAlternative("bars_filter", "High-low", "", "hl");
    strategy.parameters:addDouble("bar_size", "Bar Size, pips", "", 5);
    ...
    strategy.parameters:addBoolean("use_partial_close", "Partial Close on Profit", "", false);
    strategy.parameters:addDouble("pl_partial_close", "P/L for Partial Close", "", 10);
    strategy.parameters:addInteger("amount_partial_close", "Amount for Partial Close, in Lots", "", 1);
end
...
function Prepare(nameOnly)
    ...
    local name = profile:id() .. "(" .. instance.bid:instrument()  .. "[" .. instance.parameters.Trade   .. "], " 
        .. instance.parameters.breakeven_level .. "," .. instance.parameters.target_level .. ")";

    buffer_zone = instance.parameters.buffer_zone;
    bars_filter = instance.parameters.bars_filter;
    min_bar_size = instance.parameters.bar_size;
    use_partial_close = instance.parameters.use_partial_close;
    pl_partial_close = instance.parameters.pl_partial_close;
    ...
    base_size = core.host:execute("getTradingProperty", "baseUnitSize", trade.Instrument, trade.AccountID);
    amount_partial_close = instance.parameters.amount_partial_close * base_size;
    ...
    breakeven:CreateController()
        ...
        :SetTrade(trade)
        :SetTo(instance.parameters.target_level);
end
...
function ExtUpdate(id, source, period)
    ...
    for _, controller in ipairs(breakeven._controllers) do
        controller:DoAfterBreakeven(source, period);
        controller:DoPartialClose(source, period);
    end
end
...
function breakeven:OnClosedTrade(closed_trade)
    for _, controller in ipairs(self._controllers) do
        if controller.TradeID == closed_trade.TradeIDOrigin then
            instance.parameters.Trade = closed_trade.TradeIDRemain;
...
function breakeven:CreateController()
    ...
    function controller:DoAfterBreakeven(source, period)
        if self._executed then
            return false;
        end
        if not self._breakeven_moved then
            return true;
        end
        local trade = self:GetTrade();
        if trade == nil then
        ...
        if bars_filter == "oc" then
            local bar_size = math.abs(source.close[period] - source.open[period]) / source:pipSize();
            if bar_size <= min_bar_size then
                return true;
            end
        elseif bars_filter == "hl" then
            local bar_size = math.abs(source.high[period] - source.low[period]) / source:pipSize();
            if bar_size <= min_bar_size then
                return true;
            end
        end
        if trade.BS == "B" then
            if source.close[period] > source.open[period] and self._level < source.low[period] then
                self._level = source.low[period] - buffer_zone * source:pipSize();
                trading:MoveStop(self._level, trade);
            end
        else
            if source.close[period] < source.open[period] and self._level > source.high[period] then
                self._level = source.high[period] + buffer_zone * source:pipSize();
                trading:MoveStop(self._level, trade);
            end
        end
    ...
    function controller:DoPartialClose()
        if self._executed or not use_partial_close then
            return false;
        end
        if self._partially_closed then
            return false;
        end
        local trade = self:GetTrade();
        if trade == nil then
        ...
        if trade.PL >= pl_partial_close then
            trading:ParialClose(trade, amount_partial_close);
            self._partially_closed = true;
            return false;
        end
        return true;
    end
    function controller:DoBreakeven()
        ...
        if trade.PL >= self._when then
            self._level = self:getTo();
            if instance.parameters.allow_trade then
                self._parent._trading:MoveStop(self._level, trade);
            end
