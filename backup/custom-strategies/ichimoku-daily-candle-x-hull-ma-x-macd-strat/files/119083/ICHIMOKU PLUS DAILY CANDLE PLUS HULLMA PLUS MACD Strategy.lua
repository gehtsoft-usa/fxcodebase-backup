...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=66104
-- Id: 
...
local Modules = {};
function Init()
    strategy:name("Ichimoku + Daily-Candle_X + HULL-MA_X + MacD");
    strategy:description("");

    strategy.parameters:addInteger("keh", "Double HullMA", "", 14);
    strategy.parameters:addDouble("dt", "Decision Threshold (0.001)", "", 0.001);
    strategy.parameters:addDouble("SL", "Stop Loss in $", "", -500);
    strategy.parameters:addDouble("TP", "Target Point in $", "", 25000);
    strategy.parameters:addInteger("conversionPeriods", "Conversion Line Periods", "", 9);
    strategy.parameters:addInteger("basePeriods", "Base Line Periods", "", 26);
    strategy.parameters:addInteger("laggingSpan2Periods", "Lagging Span 2 Periods", "", 52);
    strategy.parameters:addInteger("displacement", "Displacement", "", 26);
    strategy.parameters:addInteger("MACD_Length", "MACD Length", "", 9);
    strategy.parameters:addInteger("MACD_fastLength", "MACD Fast Length", "", 12);
    strategy.parameters:addInteger("MACD_slowLength", "MACD Slow Length", "", 26);
...
function Prepare(name_only)
    ...
    custom_id = "Ichimoku + Daily-Candle_X + HULL-MA_X + MacD_" .. instance.bid:instrument();
    source_d1 = ExtSubscribe(1, nil, "D1", instance.parameters.is_bid, "close");
    ...
    indi = core.indicators:create("ICHIMOKU PLUS DAILY CANDLE PLUS HULLMA PLUS MACD", trading_logic.MainSource.close, keh);
    macd = core.indicators:create("MACD", trading_logic.MainSource, MACD_fastLength, MACD_slowLength, MACD_Length);
...
function donchian(period, len)
    local lowest = mathex.min(trading_logic.MainSource.low, period - len, period);
    local highest = mathex.max(trading_logic.MainSource.high, period - len, period);
...
function EntryFunction(source, period)
    ...
    local serial = source:serial(period);
    if serial == last_serial or not indi.N1:hasData(period) or not indi.N2:hasData(period)
        or not macd.MACD:hasData(period) or not macd.SIGNAL:hasData(period)
    then
        return;
    end
    local confidence = (source_d1[NOW] - source_d1[NOW - 1]) / source_d1[NOW - 1];
    ...
    local LS = trading_logic.MainSource.close[period];
    ...
    local positionsCount = #trades;
    if indi.N1[period] < indi.N2[period] 
        and confidence < dt
        and LS < indi.N2[period] 
        or pl < SL 
        or pl > TP
    then
        CloseTrades("B");
    ...
    if indi.N1[period] > indi.N2[period] 
        ...
        and trading_logic.MainSource.open[period] < LS
        and MACD > aMACD
        and positionsCount < ot
    then
        last_serial = serial;
        ...
        if instance.parameters.set_stop then 
            command = command:SetPipStop(nil, instance.parameters.stop, instance.parameters.use_trailing and instance.parameters.trailing or nil);
        end
