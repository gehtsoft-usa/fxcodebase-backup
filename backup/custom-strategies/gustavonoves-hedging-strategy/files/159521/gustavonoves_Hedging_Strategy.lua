-- Available @ https://fxcodebase.com/code/viewtopic.php?f=31&t=76013
...
function Init()
    strategy:name("gustavonoves Hedging Strategy");
    ...
    strategy.parameters:addInteger("mult", "Failed trade amount miltiplier", "", 2);
    strategy.parameters:addDouble("Stop", "Stop Value", "In pips", 30);
    strategy.parameters:addDouble("Limit", "Limit Value", "In pips", 30);

    strategy.parameters:addString("custom_id", "ID for strades", "", "GHS");
end

local TRADES_UPDATE = 1;
local ORDERS_UPDATE = 2;
local TIMER_ID = 3;
local MAIN_SOURCE_ID = 4;
local main_source;
local mode = "start";
local b_amount;
local s_amount;
function Prepare(nameOnly)
    ...
    b_amount = instance.parameters.Amount;
    s_amount = instance.parameters.Amount;
...
function Update(source, period)
    ...
    elseif mode == "reopen" then
        if not trading:FindTrade()
            :WhenCustomID(instance.parameters.custom_id)
            :Any() 
        then
            mode = "start";
        end
...
function AsyncOperationFinished(cookie, success, message, message1, message2)
    trading:AsyncOperationFinished(cookie, success, message, message1, message2);
        ...
        if close_trade then
            ...
            if closed_trade ~= nil then
                if closed_trade.PL < 0 then
                    mode = "reopen";
                    if closed_trade.BS == "B" then
                        b_amount = b_amount * instance.parameters.mult;
                    else
                        s_amount = s_amount * instance.parameters.mult;
                    end
                end
            end
        end
