...
function Update(id, source, period)
    ...
    local row = enum:next();
    while row ~= nil do
        local stop_order = trading:FindStopOrder(row);
        if stop_order ~= nil then
            if trades_stop[row.TradeID] == nil then
                trades_stop[row.TradeID] = stop_order.Rate;
            else
                if trades_stop[row.TradeID] ~= stop_order.Rate and not processed[row.TradeID] then
                    if allow_trade then
                        CloseTrade(row);
                    end
                    Signal("Stop for " .. row.TradeID .. " has been moved");
                    processed[row.TradeID] = true;
                end
            end
        end
        local limit_order = trading:FindLimitOrder(row);
        if limit_order ~= nil then
            if trades_limit[row.TradeID] == nil then
                trades_limit[row.TradeID] = limit_order.Rate;
            else
                if trades_limit[row.TradeID] ~= limit_order.Rate and not processed[row.TradeID] then
                    if allow_trade then
                        CloseTrade(row);
                    end
                    Signal("Limit for " .. row.TradeID .. " has been moved");
                    processed[row.TradeID] = true;
                end
            end
        end
