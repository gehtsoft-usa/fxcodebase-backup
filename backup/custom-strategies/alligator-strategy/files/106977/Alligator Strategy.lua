...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=63638
...
function Init() --The strategy profile initialization
    strategy:name("Alligator Strategy")
    ...
    strategy.parameters:addDouble("Delta", "Candle Delta", "", 10, 0, 100000)
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if
        Lips[period] > Jaw[period] and Teeth[period] > Jaw[period] and
            (Lips[period - 1] <= Jaw[period - 1] or Teeth[period - 1] <= Jaw[period - 1]) and
            (Source.close[period] - Source.open[period]) > (Source:pipSize() * Delta)
     then
        if Direction then
            BUY()
        else
    ...
    elseif
        Lips[period] < Jaw[period] and Teeth[period - 1] < Jaw[period - 1] and
            (Lips[period - 1] >= Jaw[period - 1] or Teeth[period - 1] >= Jaw[period - 1]) and
            (Source.open[period] - Source.close[period]) > (Source:pipSize() * Delta)
     then
        if Direction then
            SELL()
        else
    ...
    --[[
	 Conditions : When the curves "lips " and " teeth" of Alligator Indicator crossed the curve " Jaw " + at least 10 pips candlesticks upward or down

Exits :
- Stop and Limit : When the curves the curves "lips " and " teeth" crossed the curve " Jaw " 
	]]
    if Exit then
        if Lips[period] < Jaw[period] and Teeth[period] < Jaw[period] then
            if Direction then
