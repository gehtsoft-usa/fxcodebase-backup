...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=2633
...
function Init() --The strategy profile initialization
    strategy:name("ASCTrend strategy")
    strategy:description("ASCTrend strategy")
...
function Prepare(nameOnly)
    ...
    name =
        ...
        "(" ..
            instance.bid:name() ..
                "." .. instance.parameters.TF .. "," .. "ASCTrend(" .. instance.parameters.RISK .. "))"
    instance:name(name)
    if nameOnly then
        return;
    end
    ...
    assert(core.indicators:findIndicator("ASCTREND2") ~= nil, "Please, download and install ASCTREND2.LUA indicator")

    Source = ExtSubscribe(2, nil, instance.parameters.TF, true, "bar")
    ASCTrend = core.indicators:create("ASCTREND2", Source, instance.parameters.RISK, true)
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ASCTrend:update(core.UpdateLast)

    -- Check that we have enough data
    if (ASCTrend.buffDot:first() > (period - 1)) then
        return
    end

    local buff = ASCTrend.buffDot
    ...
    local MustS = false
    if
        (buff[period - 1] == Source.high[period - 1] and instance.parameters.TypeSignal == "direct") or
            (buff[period - 1] == Source.low[period - 1] and instance.parameters.TypeSignal == "reverse")
     then
        MustS = true
    end
    if
        (buff[period - 1] == Source.low[period - 1] and instance.parameters.TypeSignal == "direct") or
            (buff[period - 1] == Source.high[period - 1] and instance.parameters.TypeSignal == "reverse")
     then
        MustB = true
    end
