...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=59633
...
function Init() --The strategy profile initialization
    strategy:name("TRO's Dymamic SR Strategy")
    ...
    strategy.parameters:addInteger("NR", "Number of periods for Resistance", "", 5, 2, 10000)
    strategy.parameters:addInteger("NS", "Number of periods for Support", "", 5, 2, 10000)
    strategy.parameters:addInteger("Threshold", "Threshold (in pips)", "", 0, 0, 10000)
    strategy.parameters:addInteger("Trigger", "Trigger value (in pips)", "", 5, 2, 10000)
    strategy.parameters:addBoolean("ShowTriggers", "Show Triggers", "", true)
    strategy.parameters:addBoolean("ResetOnNewDay", "Reset support/resistance on new day", "", false)
...
local NR, NS, Threshold, Trigger, ShowTriggers, ResetOnNewDay
...
function Prepare(nameOnly)
    ...
    ResetOnNew = instance.parameters.DayResetOnNewDay
    ...
    name = name .. ", " .. NR .. ", " .. NS .. ", " .. Threshold .. ", " .. Trigger

    assert(
        core.indicators:findIndicator("TRO_DYNAMIC_SR") ~= nil,
        "Please, download and install TRO_DYNAMIC_SR.LUA indicator"
    )
    ...
    Indicator[1] =
        core.indicators:create("TRO_DYNAMIC_SR", Source, NR, NS, Threshold, Trigger, true, ResetOnNewDay, true)
    Short[1] = Indicator[1]:getStream(2)
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if Short[1][period - 1] == 1 and Short[1][period - 1] ~= 1 then
        if Direction then
            BUY()
        else
            SELL()
        end
    elseif Short[1][period - 1] == -1 and Short[1][period - 1] ~= -1 then
        if Direction then
            SELL()
        else
            BUY()
