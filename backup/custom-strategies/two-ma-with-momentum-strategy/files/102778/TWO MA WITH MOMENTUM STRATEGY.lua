...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=62771
...
function Init() --The strategy profile initialization
    strategy:name("TWO MA WITH MOMENTUM STRATEGY")
    ...
    strategy.parameters:addInteger("Period1", "MA Period", "MA Period", 10)
    ...
    strategy.parameters:addInteger("Period2", "MA Period", "MA Period", 20)
    ...
    strategy.parameters:addInteger("MP", "Momentum Period", "Momentum Period", 12)
    strategy.parameters:addString("MS", "Momentum smoothing method", "Momentum smoothing method", "MVA")
    strategy.parameters:addStringAlternative("MS", "No smoothing", "", "NO")
    ...
    strategy.parameters:addInteger("MSP", "Momentum Smoothing Period", "Momentum Smoothing Period", 20)
    strategy.parameters:addDouble("BL", "Buy Level", "Buy Level", 100)
    strategy.parameters:addDouble("SL", "Sell Level", "Sell Level", 100)
...
function Prepare(nameOnly)
    ...
    assert(
        core.indicators:findIndicator("SMOOTHED MOMENTUM") ~= nil,
        "Please, download and install SMOOTHED MOMENTUM.LUA indicator"
    )
    ...
    Momentum = core.indicators:create("SMOOTHED MOMENTUM", Source.close, MP, MS, MSP)
    ...
    first = math.max(Momentum.DATA:first(), MA1.DATA:first(), MA2.DATA:first())
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    -- update indicators.
    Momentum:update(core.UpdateLast)
    ...
    if
        Momentum.DATA[period] > BL and Momentum.DATA[period] > Momentum.DATA[period - 1] and
            MA1.DATA[period] > MA2.DATA[period] and
            Source.close[period] > MA1.DATA[period] and
            Source.close[period - 1] <= MA1.DATA[period - 1]
     then
        if Direction then
            BUY()
        else
    ...
    elseif
        Momentum.DATA[period] < SL and Momentum.DATA[period] < Momentum.DATA[period - 1] and
            MA1.DATA[period] < MA2.DATA[period] and
            Source.close[period] < MA1.DATA[period] and
            Source.close[period - 1] >= MA1.DATA[period - 1]
     then
