...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=64582
...
function Init() --The strategy profile initialization
    strategy:name("ColorXWPR Strategy")
...
function EntryFunction(now, period)
    ...
    if
        Indicator.WPR[period] > Indicator.XWPR[period] and Indicator.WPR[period - 1] <= Indicator.XWPR[period - 1] and
            Indicator.XWPR[period] > Indicator.XWPR[period - 1] and
            Indicator.XWPR[period] < BL
     then
        if Direction then
            BUY()
        else
