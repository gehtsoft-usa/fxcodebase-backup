...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=64966
...
function Init() --The strategy profile initialization
    strategy:name("Pinocchio Bar Strategy")
    ...
    strategy.parameters:addGroup("Pin Bar Calculation")

    strategy.parameters:addInteger("BB", "Body Lengt", "Body Lengt", 33)
    strategy.parameters:addInteger("BP", "Body Position", "Body Position", 33)
    strategy.parameters:addInteger("NB", "Nose Lengt", "Nose Lengt", 33)
    strategy.parameters:addBoolean("LA", "Left Eye Check", "Left Eye Check", true)

    strategy.parameters:addGroup("Alligator Filter Calculation")
...
local Nose, Body, Position, Left, Filter
...
function Calculation(period)
    if not alligator.Teeth:hasData(period) then
        return 0
    end
    ...
    if Left then
        if math.abs(Source.close[period] - Source.open[period]) <= Bar * Body then
            if math.max(Source.close[period], Source.open[period]) <= Source.low[period] + Bar * Position then
                if Source.high[period] - Source.high[period - 1] >= Nose * Bar then
                    if
                        Source.low[period - 1] < math.min(Source.open[period], Source.close[period]) and
                            Source.high[period - 1] > math.max(Source.open[period], Source.close[period]) and
                            (not Filter or (Filter and Source.close[period] > alligator.Teeth[period]))
                     then
                        return -1
                    end
                end
            ...
            if math.min(Source.close[period], Source.open[period]) >= Source.high[period] - Bar * Position then
                if Source.low[period - 1] - Source.low[period] >= Nose * Bar then
                    if
                        Source.high[period - 1] > math.max(Source.open[period], Source.close[period]) and
                            Source.low[period - 1] < math.max(Source.open[period], Source.close[period]) and
                            (not Filter or (Filter and Source.close[period] < alligator.Teeth[period]))
                     then
                        return 1
                    end
        ...
        if math.abs(Source.close[period] - Source.open[period]) <= Bar * Body then
            if math.max(Source.close[period], Source.open[period]) <= Source.low[period] + Bar * Position then
                if
                    Source.high[period] - Source.high[period - 1] >= Nose * Bar and
                        (not Filter or (Filter and Source.close[period] > alligator.Teeth[period]))
                 then
                    return -1
                end
            ...
            if math.min(Source.close[period], Source.open[period]) >= Source.high[period] - Bar * Position then
                if
                    Source.low[period - 1] - Source.low[period] >= Nose * Bar and
                        (not Filter or (Filter and Source.close[period] < alligator.Teeth[period]))
                 then
                    return 1
                end
...
function EntryFunction(now, period)
    ...
    if Calculation(period) == 1 then
        ...
        Return = true
    elseif Calculation(period) == -1 then
        if Direction then
            SELL()
        else
            BUY()
