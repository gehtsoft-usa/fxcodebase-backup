...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=70445
--+------------------------------------------------------------------------+
...
local Modules = {};
local STRATEGY_NAME = "Pivot MA Price Cloud Strategy";
function CreateParameters() 
    strategy.parameters:addGroup("Calculation")
    strategy.parameters:addInteger("Period1", "1.MA Period", "", 100)
    ...
    strategy.parameters:addInteger("Period2", "2.MA Period", "", 130)
    ...
    strategy.parameters:addInteger("Period3", "3.MA Period", "", 200)
    ...
    strategy.parameters:addInteger("K", "Number of periods for %K", "The number of periods for %K.", 11, 2, 1000)
    ...
    strategy.parameters:addStringAlternative("MVAT_K", "FS", "FS", "FS")
    ...
    strategy.parameters:addBoolean("close_when_losing", "Close trade when loging X bars", "", true);
    strategy.parameters:addInteger("close_bars", "X bars", "", 5);
end

local indi, close_when_losing, close_bars;
function CreateEntryIndicators(source)
    close_when_losing = instance.parameters.close_when_losing;
    close_bars = instance.parameters.close_bars;
    local profile = core.indicators:findIndicator("PIVOT-MA-PRICE CLOUD");
    assert(profile ~= nil, "Please, download and install " .. "PIVOT-MA-PRICE CLOUD" .. ".LUA indicator");
    local indicatorParams = profile:parameters();
    indi = core.indicators:create("PIVOT-MA-PRICE CLOUD", source, source:barSize(), false, false, 
        instance.parameters.Period1, instance.parameters.Method1,
        instance.parameters.Period2, instance.parameters.Method2,
        instance.parameters.Period3, instance.parameters.Method3,
        instance.parameters.K, instance.parameters.SD, instance.parameters.D,
        instance.parameters.MVAT_K, instance.parameters.MVAT_D);
end
...
function OnNewBar(source, period)
    if not close_when_losing then
        return;
    end
    local low, high = mathex.minmax(source, core.rangeTo(period, close_bars));
    trading:FindTrade():WhenCustomID(CustomID)
        :Do(function (trade)
            if trade.PL < 0 then
                if trade.BS == "B" then
                    if high < trade.Open then
                        trading:Close(trade);
                    end
                else
                    if low > trade.Open then
                        trading:Close(trade);
                    end
                end
            end
