...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=66864
...
function Init()
    strategy:name("Previous Week High-Low Fibo Strategy");
    strategy:description("v1 2018-10-28");

    strategy.parameters:addDouble("fib1", "Fibonacci Level #1", "", 0.382);
    strategy.parameters:addDouble("fib2", "Fibonacci Level #2", "", 1.27);
...
function Prepare(name_only)
    ...
    assert(core.indicators:findIndicator("PREVIOUS WEEK HIGH-LOW FIBO INDI") ~= nil, "PREVIOUS WEEK HIGH-LOW FIBO INDI" .. " indicator must be installed");
    indi = core.indicators:create("PREVIOUS WEEK HIGH-LOW FIBO INDI", trading_logic.MainSource, 
        instance.parameters.fib1, instance.parameters.fib2);
...
function DoBuy(source)
    if instance.parameters.allow_trade then
        ...
        command = command:SetLimit(indi.F2[NOW]);
...
function EntryFunction(source, period)
    ...
    local date = core.getcandle("W1", source:date(period), trading_day_offset, trading_week_offset);
    if last_date ~= date then
        last_date = date;
        trading:FindTrade()
            :WhenCustomID(custom_id)
            :Do(function (trade) trading:Close(trade); end);
        if indi.F1[period] < indi.F2[period] then
            DoBuy(source);
        else
            DoSell(source);
        end
