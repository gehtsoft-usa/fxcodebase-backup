...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=63006
...
function Init() --The strategy profile initialization
    strategy:name("Price-to-MA shrinking-Gap Exit automation Strategy")
    ...
    strategy.parameters:addInteger("Period", "MA Period", "", 56)
    ...
    strategy.parameters:addDouble("Number", "Number of Pips", "", 10)
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    --[[
	Enter Buy at market when: EMA(-1) – Close(-1) >= № of Pip, and EMA – Close < № of Pip;
    Enter Sell at market when: Close(-1) – EMA(-1) >= № of Pip, and Close – EMA < № of Pip;
	]]
    -- only buy if we have a fast cross over slow and the price is above the moving averages.
    if
        ((MA.DATA[period - 1] - source.close[period - 1]) / Source:pipSize()) >= Number and
            ((MA.DATA[period] - source.close[period]) / Source:pipSize()) < Number
     then
        if Direction then
            BUY()
        else
    ...
    elseif
        ((source.close[period - 1] - MA.DATA[period - 1]) / Source:pipSize()) >= Number and
            ((source.close[period] - MA.DATA[period]) / Source:pipSize()) < Number
     then
        if Direction then
            SELL()
        else
