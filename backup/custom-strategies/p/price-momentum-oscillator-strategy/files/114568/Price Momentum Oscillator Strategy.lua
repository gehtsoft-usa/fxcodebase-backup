...
--http://fxcodebase.com/code/viewtopic.php?f=31&t=65039
...
function Init() --The strategy profile initialization
    strategy:name("Price Momentum Oscillator Strategy")
    ...
    strategy.parameters:addInteger("one", "Short Period", "Period", 20)
    strategy.parameters:addInteger("two", "Long Period", "Period", 35)
    strategy.parameters:addInteger("Period", "Signal Period", "Period", 10)
...
local Indicator, one, two, Period, BL, SL
...
function Prepare(nameOnly)
    ...
    Indicator = core.indicators:create("PMO", Source.close, one, two, Period)
...
function EntryFunction(now, period)
    ...
    if
        Indicator.SIGNAL[period] > BL and Indicator.PMO[period] > Indicator.SIGNAL[period] and
            Indicator.PMO[period - 1] <= Indicator.SIGNAL[period - 1]
     then
        if Direction then
            BUY()
        else
        ...
        Return = true
    elseif
        Indicator.SIGNAL[period] < SL and Indicator.PMO[period] < Indicator.SIGNAL[period] and
            Indicator.PMO[period - 1] >= Indicator.SIGNAL[period - 1]
     then
        if Direction then
            SELL()
