...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=61488
...
function Init() --The strategy profile initialization
    strategy:name("Price MA Cross Strategy with Confirmation")
    ...
    strategy.parameters:addInteger("Period", "MA Period", "", 14, 1, 1000)
    ...
    strategy.parameters:addDouble("Threshold", "Confirmation Threshold", "", 0, 0, 1000)
...
local first
local Method, Period, Price, Threshold
local MA
local Direction
local CustomID
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if period < first or not MA.DATA:hasData(period - 1) then
        return
    end
    local Long = MA.DATA[period] + Threshold * Source:pipSize()
    local Short = MA.DATA[period] - Threshold * Source:pipSize()
    -- only buy if we have a fast cross over slow and the price is above the moving averages.
    if core.crossesOver(Source.close, Long, period) and core.crossesOver(Source.close, MA.DATA, period) then
        if Direction then
            BUY()
        else
            SELL()
    ...
    elseif core.crossesUnder(Source.close, Short, period) and core.crossesUnder(Source.close, MA.DATA, period) then
        if Direction then
            SELL()
        else
            BUY()
