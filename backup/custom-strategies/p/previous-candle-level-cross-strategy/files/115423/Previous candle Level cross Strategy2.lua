...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=64285
...
function Init() --The strategy profile initialization
    strategy:name("Previous candle Level cross Strategy")
    ...
    strategy.parameters:addString("TF2", "Level candle Time frame", "", "H1")
    strategy.parameters:setFlag("TF2", core.FLAG_PERIODS)

    strategy.parameters:addGroup("Selector")

    strategy.parameters:addString("Action" .. 1, "High Price Cross Over", "", "NO")
    ...
    strategy.parameters:addString("Action" .. 2, "High Price Cross Under", "", "NO")
    ...
    strategy.parameters:addString("Action" .. 3, "Low Price Cross Over", "", "NO")
    ...
    strategy.parameters:addString("Action" .. 4, "Low Price Cross Under", "", "NO")
    ...
    strategy.parameters:addString("Action" .. 5, "Close Price Cross Over", "", "NO")
    ...
    strategy.parameters:addString("Action" .. 6, "Close Price Cross Under", "", "NO")
    ...
    strategy.parameters:addString("Action" .. 7, "Open Price Cross Over", "", "NO")
    ...
    strategy.parameters:addString("Action" .. 8, "Open Price Cross  Under", "", "NO")
...
function ExitFunction(period, now)
    ...
    local S2Prev = Source2:size() - 2
    if S2Prev < 2 then
        return;
    end

    if core.crossesOver(Source.close, Source2.high, period, S2Prev) then
        ExitACTION(1, period)
    end
    if core.crossesUnder(Source.close, Source2.high, period, S2Prev) then
        ExitACTION(2, period)
    end
    if core.crossesOver(Source.close, Source2.low, period, S2Prev) then
        ExitACTION(3, period)
    end
    if core.crossesUnder(Source.close, Source2.low, period, S2Prev) then
        ExitACTION(4, period)
    end
    if core.crossesOver(Source.close, Source2.close, period, S2Prev) then
        ExitACTION(5, period)
    end
    if core.crossesUnder(Source.close, Source2.close, period, S2Prev) then
        ExitACTION(6, period)
    end
    if core.crossesOver(Source.close, Source2.open, period, S2Prev) then
        ExitACTION(7, period)
    end
    if core.crossesUnder(Source.close, Source2.open, period, S2Prev) then
        ExitACTION(8, period)
    end
end
...
function EntryFunction(period, now)
    ...
    if core.crossesOver(Source.close, Source2.high, period, S2Prev) then
        EntryACTION(1, period)
    end
    if core.crossesUnder(Source.close, Source2.high, period, S2Prev) then
        EntryACTION(2, period)
    end
    if core.crossesOver(Source.close, Source2.low, period, S2Prev) then
        EntryACTION(3, period)
    end
    if core.crossesUnder(Source.close, Source2.low, period, S2Prev) then
        EntryACTION(4, period)
    end
    if core.crossesOver(Source.close, Source2.close, period, S2Prev) then
        EntryACTION(5, period)
    end
    if core.crossesUnder(Source.close, Source2.close, period, S2Prev) then
        EntryACTION(6, period)
    end
    if core.crossesOver(Source.close, Source2.open, period, S2Prev) then
        EntryACTION(7, period)
    end
    if core.crossesUnder(Source.close, Source2.open, period, S2Prev) then
        EntryACTION(8, period)
    end

    return Return
end

function EntryACTION(Flag, period)
...
function ExitACTION(Flag, period)
    if Action[Flag] ~= "CLOSE" then
        return
