...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=63747
...
function Init() --The strategy profile initialization
    strategy:name("Pinbar MA Cross Strategy")
    ...
    strategy.parameters:addBoolean("Filter", "Use MA Filter", "MA Filter", true)
                ...
                if Source.high[period] - Source.high[period - 1] >= Nose * Bar then
                    if
                        Source.low[period - 1] < math.min(Source.open[period], Source.close[period]) and
                            Source.high[period - 1] > math.max(Source.open[period], Source.close[period])
                     then
                        return 1
                    end
                end
                ...
                if Source.low[period - 1] - Source.low[period] >= Nose * Bar then
                    if
                        Source.high[period - 1] > math.max(Source.open[period], Source.close[period]) and
                            Source.low[period - 1] < math.max(Source.open[period], Source.close[period])
                     then
                        return -1
                    end
                end
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if
        Calculation(period - 1) == 1 and
            (MA.DATA[period] >= Source.low[period] and MA.DATA[period] <= Source.high[period] or not Filter)
     then
        if Direction then
            BUY()
        else
