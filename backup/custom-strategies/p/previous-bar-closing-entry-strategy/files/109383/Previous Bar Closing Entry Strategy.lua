...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=64166
...
function Init() --The strategy profile initialization
    strategy:name("Previous Bar Closing Entry Strategy ")
    ...
    strategy.parameters:addDouble("Delta", "Delta in pips", "", 1)
...
function CreateTradingParameters()
    ...
    strategy.parameters:addInteger("Trailing", "Trailing in pips", "", 1)
...
local CloseLong, CloseShort, Hedge
local Delta
...
function moveStop(rate, side, row)
    ...
    if order == nil then
        ...
        if math.abs(rate - order.Rate) > minChange then
            ...
            --core.host:trace("DEBUG: Moving stop for the %s to %d", trade.TradeID, rate);
...
function EntryFunction(period, now)
    ...
    -- If the previous bar close higher than opening price,
    -- put a "buy position 1 pips above the previous bar closing"

    if
        Source.close[period - 1] > Source.open[period - 1] and
            Source.close[period] > (Source.close[period - 1] + Delta * Source:pipSize())
     then
        if Direction then
            BUY()
        else
