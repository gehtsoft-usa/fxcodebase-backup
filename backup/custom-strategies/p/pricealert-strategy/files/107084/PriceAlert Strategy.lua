...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=63660
...
function Init() --The strategy profile initialization
    strategy:name("PriceAlert Strategy")
    ...
    strategy.parameters:addString("Smooth", "Smoot Type", "", "NO")
    strategy.parameters:addStringAlternative("Smooth", "NO", "", "NO")
    ...
    strategy.parameters:addInteger("SmoothN", "Smooth Period", "", 7, 1, 300)
    strategy.parameters:addDouble("Price", "Price", "", 0)
    strategy.parameters:setFlag("Price", core.FLAG_PRICE)
    strategy.parameters:addString("Condition", "Condition", "", "C")
    strategy.parameters:addStringAlternative("Condition", "ConditionCross", "", "C")
    strategy.parameters:addStringAlternative("Condition", "ConditionCrossOrTouch", "", "CT")
    strategy.parameters:addStringAlternative("Condition", "ConditionCrossAbove", "", "CA")
    strategy.parameters:addStringAlternative("Condition", "ConditionCrossAboveOrTouches", "", "CAT")
    strategy.parameters:addStringAlternative("Condition", "ConditionCrossBelow", "", "CB")
    strategy.parameters:addStringAlternative("Condition", "ConditionCrossBelowOrTouches", "", "CBT")

    strategy.parameters:addString("Direction", "Type Signal / Trade", "", "Buy")
    strategy.parameters:addStringAlternative("Direction", "Buy", "", "Buy")
    strategy.parameters:addStringAlternative("Direction", "Sell", "", "Sell")
    strategy.parameters:addStringAlternative("Direction", "Alert", "", "Alert")
...
local gCondition
local Price
local Type, Smooth, SmoothN, Condition
local gSmooth, gData, SIGNAL
...
function Prepare(nameOnly)
    ...
    if Smooth ~= "NO" then
        assert(core.indicators:findIndicator(Smooth) ~= nil, "Please, download and install " .. Smooth .. " indicator")
    end
    ...
    if Smooth == "NO" then
        ...
        gSmooth = core.indicators:create(Smooth, Source.close, SmoothN)
        gData = gSmooth.DATA
    end
    first = gData:first() + 1
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if Direction ~= "Alert" then
        ...
        elseif
            gCondition == CT_CROSSTOUCH and (core.crosses(gData, Price, period) or core.touches(gData, Price, period))
         then
            if Direction == "Buy" then
                BUY()
            else
        ...
        elseif gCondition == CT_CROSSABOVETOUCH and core.crossesOverOrTouch(gData, Price, period) then
            --ExtSignal(gSource, period, SIGNAL, SoundFile, Email, RecurrentSound);
            Signal("CROSSABOVETOUCH")
        elseif gCondition == CT_CROSSBELOW and core.crossesUnder(gData, Price, period) then
            --ExtSignal(gSource, period, SIGNAL, SoundFile, Email, RecurrentSound);
            Signal("CROSSBELOW")
        elseif gCondition == CT_CROSSBELOWTOUCH and core.crossesUnderOrTouch(gData, Price, period) then
            --ExtSignal(gSource, period, SIGNAL, SoundFile, Email, RecurrentSound);
            Signal("CROSSBELOWTOUCH")
        end
    end
end
