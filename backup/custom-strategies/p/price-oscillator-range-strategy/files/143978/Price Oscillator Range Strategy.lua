...
-- https://fxcodebase.com/code/viewtopic.php?f=31&t=71585
...
function Init() --The strategy profile initialization
    strategy:name("Price Oscillator Range Strategy");
    ...
    strategy.parameters:addInteger("Period1", "Trend Period", "Period", 14);
    strategy.parameters:addInteger("Period2", "Momentum Period", "Period", 14);	
    strategy.parameters:addInteger("Period3", "Range Period", "Period", 28);
    strategy.parameters:addInteger("Period4", "Major Trend Period", "Period", 34);
...
function CreateTradingParameters()
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "PORS");
...
function Prepare( nameOnly)
	...
	assert(core.indicators:findIndicator("PRICE OSCILLATOR RANGE") ~= nil, "Please, download and install PRICE OSCILLATOR RANGE.LUA indicator");          
    ...
    Indicator = core.indicators:create("PRICE OSCILLATOR RANGE", Source.close,  instance.parameters.Period1,  instance.parameters.Period2,  instance.parameters.Period3, true, true  );
    MA = core.indicators:create("MVA", Source.close,  instance.parameters.Period4 );	
...
--	period3= core.findDate (Source3, TickSource:date(period), false);
	...
	--/////////////////////////////////////////////////////////////////////
		            ...
		            if  Source.low[period]< MA.DATA[period]	
					then
					  ...
					  if  Source.high[period]>  MA.DATA[period]				  
					  then
...
function EntryFunction( id, now,period)
	...
	if   ( (Indicator.DATA[period-1]   <  0.1 )
    or ( Indicator.DATA[period-1] > 0.9  ))
    and Source.close[period] > Source.high[period-1] 
	...
	then	
		...
		Return=true;
	elseif  ( (Indicator.DATA[period-1] <  0.1 )
    or ( Indicator.DATA[period-1] > 0.9  ))
    and Source.close[period] < Source.low[period-1] 
    and Source.close[period] < MA.DATA[period]			
	then
            if Direction then
                SELL();
