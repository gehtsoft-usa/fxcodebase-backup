...
function EntryFunction(now, period, period1)
    ...
    if
        SAR:getStream(1):hasData(period) and Source.close[period] > BuyLevel and Source.close[period - 1] <= BuyLevel and
            (not Filter or (Filter and Source.close[period] > MA.DATA[period1]))
     then
        if Direction then
            BUY()
        else
        ...
        Return = true
    elseif
        SAR:getStream(0):hasData(period) and Source.close[period] < SellLevel and Source.close[period - 1] >= SellLevel and
            (not Filter or (Filter and Source.close[period] < MA.DATA[period1]))
     then
        if Direction then
            SELL()
