...
function Init()
    strategy:name("Ichimoku Kijun or Tenkan trailing stop");
    strategy:description("");
    strategy:setTag("Version", "2");
    
    strategy.parameters:addDouble("trailing_size", "Trailing size, pips", "", 2);
...
function Prepare(nameOnly)
    ...
    trailing_size = instance.parameters.trailing_size;
...
function DoStopMove(trade)
    ...
    local newStop = stop.Rate;
    local offer = core.host:findTable("offers"):find("Instrument", TickSource:instrument());
    if last_price[trade.TradeID] == nil then
        if (trade.BS == "B") then
            last_price[trade.TradeID] = offer.Bid;
        else
            last_price[trade.TradeID] = offer.Ask;
        end
    end
    if (trade.BS == "B") then
        local movements = math.floor((offer.Bid - last_price[trade.TradeID]) / (trailing_size * TickSource:pipSize()));
        if (movements > 0) then
            newStop = newStop + movements * trailing_size * TickSource:pipSize();
            last_price[trade.TradeID] = last_price[trade.TradeID] + movements * trailing_size * TickSource:pipSize();
        end
    else
        local movements = math.floor((last_price[trade.TradeID] - offer.Ask) / (trailing_size * TickSource:pipSize()));
        if (movements > 0) then
            newStop = newStop - movements * trailing_size * TickSource:pipSize();
            last_price[trade.TradeID] = last_price[trade.TradeID] - movements * trailing_size * TickSource:pipSize();
        end
