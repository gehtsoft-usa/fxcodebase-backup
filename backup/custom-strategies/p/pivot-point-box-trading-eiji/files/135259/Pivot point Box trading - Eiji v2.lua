...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=69983
-- More information about this indicator can be found at:
--http://fxcodebase.com/code/viewtopic.php?f=31&t=69983
...
function Init()
    strategy:name("Pivot Point Box Trading Eiji");
    ...
    strategy.parameters:addString("timeframe", "Timeframe", "", "D1");
    ...
    strategy.parameters:addBoolean("live", "Live execution", "", true);

    strategy.parameters:addInteger("box_size", "SIze of trading box, pips", "", 200)
    strategy.parameters:addDouble("tp1", "Take profit 1", "", 1);
    strategy.parameters:addDouble("tp1_amount", "Take profit 1 Amount, %", "", 30);
    strategy.parameters:addBoolean("breaneven", "Breakeven on TP1", "", true);
    strategy.parameters:addDouble("breakeven_to", "Breakeven to, pips", "", 2);
    strategy.parameters:addDouble("tp2", "Take profit 2", "", 2);
    strategy.parameters:addDouble("tp2_amount", "Take profit 2 Amount, %", "", 20);
    strategy.parameters:addDouble("trailing_activation", "Trailing activation", "", 200);
end

local MAIN_SOURCE_ID = 1;
local main_source, pivot, box_size, tp1, tp1_amount, breaneven, breakeven_to, tp2, tp2_amount, trailing_activation;
function Prepare(nameOnly)
    ...
    box_size = instance.parameters.box_size;
    tp1 = instance.parameters.tp1;
    tp1_amount = instance.parameters.tp1_amount;
    breaneven = instance.parameters.breaneven;
    breakeven_to = instance.parameters.breakeven_to;
    tp2 = instance.parameters.tp2;
    tp2_amount = instance.parameters.tp2_amount;
    trailing_activation = instance.parameters.trailing_activation;
    main_source = ExtSubscribe(MAIN_SOURCE_ID, nil, instance.parameters.timeframe, instance.parameters.type, "bar")
    if instance.parameters.live then
        ExtSubscribe(MAIN_SOURCE_ID + 1, nil, "t1", instance.parameters.type)
    end
    pivot = core.indicators:create("PIVOT", main_source, instance.parameters.timeframe, "Pivot", "HIST");
end
...
function ExtUpdate(id, source, period)
    ...
    if core.crosses(main_source.close, pivot.P, NOW) and last_date ~= main_source:date(NOW) then
        last_date = main_source:date(NOW);
        trading:FindOrder()
            :WhenInstrument(main_source:instrument())
            :Do(function (order)
                trading:DeleteOrder(order);
            end)
        local order1 = trading:EntryOrder(main_source:instrument())
            :SetAccountID(instance.parameters.Account)
            :SetAmount(instance.parameters.Amount)
            :SetSide("B")
            :SetStop(pivot.P[NOW])
            :SetRate(pivot.P[NOW] + box_size / 2 * main_source:pipSize())
            :Execute();
        local order2 = trading:EntryOrder(main_source:instrument())
            :SetAccountID(instance.parameters.Account)
            :SetAmount(instance.parameters.Amount)
            :SetSide("S")
            :SetStop(pivot.P[NOW])
        ...
        breakeven:PartialClose()
            :SetRequestID(order1.RequestID)
            :SetWhen(box_size * tp1)
            :SetPartialClose(tp1_amount)
        breakeven:PartialClose()
        ...
        breakeven:PartialClose()
            :SetRequestID(order1.RequestID)
            :SetWhen(box_size * tp2)
            :SetPartialClose(tp2_amount)
        breakeven:PartialClose()
            :SetRequestID(order2.RequestID)
            :SetWhen(box_size * tp2)
            :SetPartialClose(tp2_amount)
