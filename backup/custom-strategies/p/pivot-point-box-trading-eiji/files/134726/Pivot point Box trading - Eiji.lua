...
function ExtUpdate(id, source, period)
    ...
    if core.crosses(source.close, pivot.P, period) and last_date ~= source:date(period) then
        trading:FindOrder()
            :WhenInstrument(source:instrument())
            :Do(function (order)
                trading:DeleteOrder(order);
            end)
        local order1 = trading:EntryOrder(source:instrument())
            :SetAccountID(instance.parameters.Account)
            :SetAmount(instance.parameters.Amount)
            :SetSide("B")
            :SetStop(pivot.P[period] - box_size / 2 * source:pipSize())
            :SetRate(pivot.P[period] + box_size / 2 * source:pipSize())
            :Execute();
        local order2 = trading:EntryOrder(source:instrument())
            :SetAccountID(instance.parameters.Account)
            :SetAmount(instance.parameters.Amount)
            :SetSide("S")
            :SetStop(pivot.P[period] + box_size / 2 * source:pipSize())
