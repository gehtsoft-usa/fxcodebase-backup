...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=68535
...
local STRATEGY_NAME = "Price Action Doji Harami Strategy";
...
function CreateParameters() 
    strategy.parameters:addInteger("pctDw", "Doji, Min % of Range of Candle for Wicks", "", 60, 0, 90);
    strategy.parameters:addInteger("pipMin", "Doji, Previous Candle Min Pip Body Size", "", 0, 0)
    strategy.parameters:addBoolean("sname", "Show Price Action Bar Names", "", true)
    strategy.parameters:addBoolean("cbar", "Highlight Harami & Doji Bars", "", false)
    strategy.parameters:addBoolean("sHm", "Show Only Harami Style Doji's", "", false)
    strategy.parameters:addInteger("bars", "Doji, Number of Lookback Bars", "", 3, 1, 3);
end

local indi;
function CreateIndicators(source) 
    local profile = core.indicators:findIndicator("PRICE ACTION DOJI HARAMI");
    assert(profile ~= nil, "Please, download and install " .. "PRICE ACTION DOJI HARAMI" .. ".LUA indicator");
    local indicatorParams = profile:parameters();
    indicatorParams:setInteger("pctDw", instance.parameters.pctDw);
    indicatorParams:setInteger("pipMin", instance.parameters.pipMin);
    indicatorParams:setInteger("bars", instance.parameters.bars);
    indicatorParams:setBoolean("sname", instance.parameters.sname);
    indicatorParams:setBoolean("cbar", instance.parameters.cbar);
    indicatorParams:setBoolean("sHm", instance.parameters.sHm);
    indi = core.indicators:create("PRICE ACTION DOJI HARAMI", source, indicatorParams);
end
...
function CreateCustomActions()
    ...
    enterLongAction.IsPass = function (source, period, periodFromLast, data)
        return indi:getTextOutput(0):hasData(period) or indi:getTextOutput(2):hasData(period);
    end
    ...
    enterShortAction.IsPass = function (source, period, periodFromLast, data)
        return indi:getTextOutput(1):hasData(period) or indi:getTextOutput(3):hasData(period);
    end
