...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=60836
...
function Prepare( nameOnly)
    ...
    local name;
    name = profile:id() .. "( " .. instance.bid:name() .. "," .. Method .. "," .. Period.. "," .. Threshold .. "," .. CustomID ..  " )";
    instance:name(name);
...
function ExtUpdate(id, source, period)  -- The method called every time when a new bid or ask price appears.
   ...
   -- get only time
    ...
    if   core.crossesOver(Source.close, ma.DATA , period) and Source.volume[period] >=  Threshold then
        BUY();
		ONE= Source:serial(period);
    elseif    core.crossesUnder(Source.close,ma.DATA , period) and Source.volume[period] >=  Threshold   then
        SELL();
		ONE= Source:serial(period);
    end
