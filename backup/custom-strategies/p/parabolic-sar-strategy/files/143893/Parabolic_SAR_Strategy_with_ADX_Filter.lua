...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=11912
-- Id: 5699
--+------------------------------------------------------------------+
...
function Init() --The strategy profile initialization
    ...
    strategy.parameters:addString("adx_tf", "ADX Timeframe", "", "m5");
    strategy.parameters:setFlag("adx_tf", core.FLAG_BARPERIODS);
    strategy.parameters:addInteger("Period", "ADX Period", "", 14)
    strategy.parameters:addDouble("Level", "ADX Level", "", 20)
    strategy.parameters:addBoolean("Use", "Use ADX Filter", "", true)
...
function Prepare(nameOnly)
    ...
    ADXSource = ExtSubscribe(3, nil, instance.parameters.adx_tf, instance.parameters.Type == "Bid", "bar")
    Indicator[1] = core.indicators:create("SAR", Source, Parameters["Step"], Parameters["Max"])
    Short[1] = Indicator[1]:getStream(0)
    Short[2] = Indicator[1]:getStream(1)

    Indicator[2] = core.indicators:create("ADX", ADXSource, Period)
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if not Use then
        TRADE = true
    elseif Short[3][NOW] > Level then
        TRADE = true
    end

    -- only buy if we have a fast cross over slow and the price is above the moving averages.
    if not (Short[2]:hasData(period - 1)) and Short[2]:hasData(period) and TRADE then
        if Direction then
            BUY(TimeFlag)
        else
            SELL(TimeFlag)
    ...
    elseif not (Short[1]:hasData(period - 1)) and Short[1]:hasData(period) and TRADE then
        if Direction then
            SELL(TimeFlag)
        else
            BUY(TimeFlag)
    ...
    if Exit then
        if not (Short[2]:hasData(period)) then
            if Direction then
        ...
        if not (Short[1]:hasData(period)) then
            if Direction then
                if haveTrades("S") then
                    exitSpecific("S")
                    Signal("Close Short")
...
function BUY(iFlag)
    if AllowTrade then
        if CloseOnOpposite and haveTrades("S") and (Applies == "Entry" or (Applies ~= "Entry" and iFlag == true)) then
        ...
        if (Applies == "Exit" or (Applies ~= "Exit" and iFlag == true)) then
            enter("B")
        end
    else
        Signal("Buy Signal")
...
function SELL(iFlag)
    if AllowTrade then
        if CloseOnOpposite and haveTrades("B") and (Applies == "Entry" or (Applies ~= "Entry" and iFlag == true)) then
            -- close on opposite signal
            exitSpecific("B")
            Signal("Close Long")
        end
