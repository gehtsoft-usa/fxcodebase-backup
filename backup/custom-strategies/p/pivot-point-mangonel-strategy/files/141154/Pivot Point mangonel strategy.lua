...
--http://fxcodebase.com/code/viewtopic.php?f=31&t=71000
...
local STRATEGY_NAME = "Pivot Point Mangonel Strategy";
function CreateParameters() 
    ...
    strategy.parameters:setFlag("pivot_tf", core.FLAG_BARPERIODS);
end

local PIVOT, tradingDayOffset, tradingWeekOffset;
...
function GetR1(source, period)
    if not PIVOT.R1:hasData(PIVOT.R1:size() - 2) then
        return nil;
    end
    local s, e = core.getcandle(instance.parameters.pivot_tf, source:date(period), tradingDayOffset, tradingWeekOffset);
    local startIndex = core.findDate(source, s, false);
    if startIndex <= 0 then
        return nil;
    end
    return math.min(PIVOT.R1[startIndex], PIVOT.R1[startIndex - 1]);
end
function GetS1(source, period)
    if not PIVOT.S1:hasData(PIVOT.S1:size() - 2) then
        return nil;
    end
    local s, e = core.getcandle(instance.parameters.pivot_tf, source:date(period), tradingDayOffset, tradingWeekOffset);
    local startIndex = core.findDate(source, s, false);
    ...
    return math.max(PIVOT.S1[startIndex], PIVOT.S1[startIndex - 1]);
end
function GetR1O(source, period)
    if not PIVOT.R1:hasData(PIVOT.R1:size() - 2) then
        return nil;
    end
    local s, e = core.getcandle(instance.parameters.pivot_tf, source:date(period), tradingDayOffset, tradingWeekOffset);
    ...
    return math.max(PIVOT.R1[startIndex], PIVOT.R1[startIndex - 1]);
end
function GetS1O(source, period)
    if not PIVOT.S1:hasData(PIVOT.S1:size() - 2) then
        return nil;
    end
    local s, e = core.getcandle(instance.parameters.pivot_tf, source:date(period), tradingDayOffset, tradingWeekOffset);
    ...
    return math.min(PIVOT.S1[startIndex], PIVOT.S1[startIndex - 1]);
end

function IsEntryLong(source, period)
    local r1 = GetR1(source, period);
    if r1 == nil then
        return false;
    end
