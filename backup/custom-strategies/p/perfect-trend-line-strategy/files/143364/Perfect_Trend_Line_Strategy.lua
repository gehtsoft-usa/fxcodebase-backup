...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=71443
-- More information about this indicator can be found at:
-- https://fxcodebase.com/code/viewtopic.php?f=31&t=71443
--+------------------------------------------------------------------------+
...
function CreateParameters() 
	...
	strategy.parameters:addInteger("SlowLength", "Slow Length", "Slow Length", 7);
	strategy.parameters:addDouble("SlowPipDisplace", "Slow Pip Displace", "Slow PipD isplace", 0);
	strategy.parameters:addInteger("FastLength", "Fast Length", "Fast Length", 3);
	strategy.parameters:addDouble("FastPipDisplace", "Fast Pip Displace", "Fast PipD isplace", 0);
    strategy.parameters:addInteger("ma_period", "Filter Period", "", 14);
    AddAverages("ma_method", "Filter Method", "MVA");
end

local indi, ma;
function CreateEntryIndicators(source)
    local profile = core.indicators:findIndicator("PERFECT TREND LINE");
    assert(profile ~= nil, "Please, download and install " .. "PERFECT TREND LINE" .. ".LUA indicator");
    indi = core.indicators:create("PERFECT TREND LINE", source, 
        instance.parameters.SlowLength,
        instance.parameters.SlowPipDisplace,
        instance.parameters.FastLength,
        instance.parameters.FastPipDisplace)
...
function ConfirmLong(period)
    local stream = indi:getTextOutput(0);
    local levelFirst = stream:get(period);
    if not stream:hasData(period) then
        return false;
    end
    for i = period - 1, 0, -1 do
        if stream:hasData(i) then
            local level, label, tooltip, color = stream:get(i);
            return levelFirst > level;
        end
    end
    return false;
end
...
function ConfirmShort(period)
    local stream = indi:getTextOutput(1);
    local levelFirst = stream:get(period);
    if not stream:hasData(period) then
        return false;
    end
...
function IsEntryLong(source, period)
    return ConfirmLong(period) and source.close[period] > ma.DATA[period];
end
function IsEntryShort(source, period)
    return ConfirmShort(period) and source.close[period] < ma.DATA[period];
end
function IsExitLong(source, period)
    return source.close[period] < indi.line1[period];
end
