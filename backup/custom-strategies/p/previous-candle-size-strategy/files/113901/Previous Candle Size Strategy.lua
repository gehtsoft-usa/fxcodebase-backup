...
--http://fxcodebase.com/code/viewtopic.php?f=31&t=64957
--+------------------------------------------------------------------+
...
function Init() --The strategy profile initialization
    strategy:name("Previous Candle Size Strategy")
    ...
    strategy.parameters:addDouble("LSize", "Long Trade Candle Size (In Pips)", "", 10)
    strategy.parameters:addDouble("SSize", "Short Trade Candle Size (In Pips)", "", 10)
...
function Prepare(nameOnly)
    ...
    --Indicator parameters
    LSize = instance.parameters.LSize
    SSize = instance.parameters.SSize
...
function EntryFunction(now, period)
    ...
    if ((Source.close[period - 1] - Source.open[period - 1]) / Source:pipSize()) > LSize then
        if Direction then
            BUY()
        else
            SELL()
        ...
        Return = true
    elseif ((Source.open[period - 1] - Source.close[period - 1]) / Source:pipSize()) > SSize then
        if Direction then
            SELL()
        else
            BUY()
