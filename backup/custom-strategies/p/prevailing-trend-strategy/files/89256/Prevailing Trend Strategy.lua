...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=59426
...
local STRATEGY_NAME = "Prevailing Trend Strategy";
...
function CreateIndicators(source) 
    local profile = core.indicators:findIndicator("TREND");
    assert(profile ~= nil, "Please, download and install " .. "TREND" .. ".LUA indicator");
    indi = core.indicators:create("TREND", source, Period, Method)
    Short[1] = indi:getStream(0)
    Short[2] = indi:getStream(1)
end
...
function CreateCustomActions()
    ...
    enterLongAction.IsPass = function (source, period, periodFromLast, data)
        if not Short[1]:hasData(period - 1) or not Short[2]:hasData(period - 1) then
            return;
        end
        return core.crossesOver(Short[1], Short[2], period);
    end
    ...
    enterShortAction.IsPass = function (source, period, periodFromLast, data)
        ...
        return core.crossesUnder(Short[1], Short[2], period);
    end
