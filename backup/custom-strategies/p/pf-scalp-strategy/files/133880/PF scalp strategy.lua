...
--http://fxcodebase.com/code/viewtopic.php?f=31&t=69855
...
local STRATEGY_NAME = "P&F scalp strat";
function CreateParameters() 
    strategy.parameters:addInteger("box_size", "Box size", "", 10);
    strategy.parameters:addInteger("reversal", "Reversal", "", 3);
end

local indi;
function CreateEntryIndicators(source)
    indi = core.indicators:create("POINT_AND_FIGURE", source, source:instrument(), source:barSize(), source:isBid(), 
        instance.parameters.box_size, instance.parameters.reversal, core.now() - 5000, 0);
end
...
function IsEntryLong(source, period)
    if indi.DATA:size() == 0 then
        return false;
    end
    core.host:trace(indi.close[NOW] .. " " .. indi.open[NOW]);
    core.host:trace(indi.close[NOW - 1] .. " " .. indi.open[NOW - 1]);
    return core.crossesOver(indi.close, indi.open, indi.DATA:size() - 1);
end
function IsEntryShort(source, period)
    if indi.DATA:size() == 0 then
        return false;
    end
    return core.crossesUnder(indi.close, indi.open, indi.DATA:size() - 1);
end
...
function ExtUpdate(id, source, period)
    ...
    if IsExitLong(main_source, entry_period) and last_exit ~= indi.DATA:date(NOW) then
        if AllowTrade then
            CloseTrades("B");
        end
        Signal("Exit long", main_source);
        last_exit = indi.DATA:date(NOW);
    end
    if IsExitShort(main_source, entry_period) and last_exit ~= indi.DATA:date(NOW) then
        if AllowTrade then
            CloseTrades("S");
        end
    ...
    if IsEntryLong(main_source, entry_period) and last_entry ~= indi.DATA:date(NOW) and not PositionsLimitHit() then
        if AllowTrade then
        ...
        last_entry = indi.DATA:date(NOW);
    end
    if IsEntryShort(main_source, entry_period) and last_entry ~= indi.DATA:date(NOW) and not PositionsLimitHit() then
        if AllowTrade then
            if close_on_opposite then
                CloseTrades("B");
            end
