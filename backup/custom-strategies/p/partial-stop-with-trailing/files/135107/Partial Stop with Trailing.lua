...
--http://fxcodebase.com/code/viewtopic.php?f=31&t=70044
...
function Init()
    ...
    strategy.parameters:addDouble("to_close", "To Close, %", "", 50, 1, 100);
    strategy.parameters:addDouble("trailing_start", "Trailing start, pips", "", 10);
    strategy.parameters:addDouble("trailing_step", "Trailing step, pips", "", 10, 1);
...
local Indicator;
local commands = {};
local to_close, trailing_start, trailing_step;
...
function Prepare(nameOnly)
    ...
    to_close = instance.parameters.to_close;
    trailing_start = instance.parameters.trailing_start;
    trailing_step = instance.parameters.trailing_step;
...
function DoMoveStop(trade)
    if commands[trade.TradeID] ~= nil and not commands[trade.TradeID].Finished or trade.PL < trailing_start then
        return;
    end
    ...
    local stopDesc = stops[trade.TradeID];
    if stopDesc == nil then
        stopDesc = {};
        stopDesc.Current = stop.Rate;
        if (trade.BS == "B") then
            stopDesc.Distance = trade.Open - stop.Rate;
        else
            stopDesc.Distance = stop.Rate - trade.Open;
        end
        stops[trade.TradeID] = stopDesc;
        return;
    end
    if stopDesc.Finished then
        return;
    end
    if (trade.BS == "B") then
        while ((instance.bid[NOW] - stopDesc.Current - stopDesc.Distance) / instance.bid:pipSize() >= trailing_step) do
            stopDesc.Current = stopDesc.Current + trailing_step * instance.bid:pipSize();
        end
        if stopDesc.Current >= instance.bid[NOW] then
            local base_size = core.host:execute("getTradingProperty", "baseUnitSize", trade.Instrument, trade.AccountID);
            local amount = breakeven:round(trade.Lot * to_close / 100.0 / base_size) * base_size;
            trading:PartialClose(trade, amount);
            stopDesc.Finished = true;
        end
    else
        while ((stopDesc.Current - instance.ask[NOW] - stopDesc.Distance) / instance.bid:pipSize() >= trailing_step) do
            stopDesc.Current = stopDesc.Current - trailing_step * instance.bid:pipSize();
        end
        if stopDesc.Current <= instance.ask[NOW] then
            local base_size = core.host:execute("getTradingProperty", "baseUnitSize", trade.Instrument, trade.AccountID);
            local amount = breakeven:round(trade.Lot * to_close / 100.0 / base_size) * base_size;
            trading:PartialClose(trade, amount);
            stopDesc.Finished = true;
