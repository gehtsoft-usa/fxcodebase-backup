...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=63216
...
function Init() --The strategy profile initialization
    strategy:name("Price MA Cross Strategy ");
	...
	strategy.parameters:addDouble("Delta", "Delta", "Delta" , 10);
...
function CreateTradingParameters()
	...
	strategy.parameters:addInteger("SignalMode", "Trade Algorithm ", "", 1);
    strategy.parameters:addIntegerAlternative("SignalMode", "1. Algorithm", "", 1);
	strategy.parameters:addIntegerAlternative("SignalMode", "2. Algorithm", "", 2);
	
	strategy.parameters:addBoolean("Exit", "Close Strategy after first Trade Executions", "", true);
	
	strategy.parameters:addBoolean("CloseOnOpposite", "Close On Opposite", "", true);
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "PMCS");
...
function ExtUpdate(id, source, period)  -- The method called every time when a new bid or ask price appears.
   ...
   -- get only time
    ...
    if 
	(( Source.close[period]  + Delta*Source:pipSize()) > (MA.DATA[period] + Delta*Source:pipSize())
	and (Source.close[period-1] + Delta*Source:pipSize()) <= (MA.DATA[period-1]  + Delta*Source:pipSize())and SignalMode== 1)
	or (( Source.close[period] ) > (MA.DATA[period] + Delta*Source:pipSize())
	and (Source.close[period-1] ) <= (MA.DATA[period-1]  + Delta*Source:pipSize())and SignalMode== 2)
    ...
    elseif  ((Source.close[period] - Delta*Source:pipSize())< (MA.DATA[period] - Delta*Source:pipSize())
	and (Source.close[period-1] - Delta*Source:pipSize())>=(MA.DATA[period] - Delta*Source:pipSize())  and SignalMode== 1)
	or((Source.close[period] )< (MA.DATA[period] - Delta*Source:pipSize())
	and (Source.close[period-1])>=(MA.DATA[period] - Delta*Source:pipSize())  and SignalMode== 2)
	then
            if Direction then
                SELL();
            else
