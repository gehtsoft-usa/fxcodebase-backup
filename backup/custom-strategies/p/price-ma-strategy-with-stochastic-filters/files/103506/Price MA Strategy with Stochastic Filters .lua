...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=62911
...
function Init() --The strategy profile initialization
    strategy:name("Price MA Strategy with Stochastic Filters")
    ...
    strategy.parameters:addStringAlternative("L", "%K / %D Position", "", "C")
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if instance.parameters.L == "K" then
        if core.crossesOver(Source.close, ma.DATA, period) and (Line1[period] > OS) then
            BUY()
            ONE = Source:serial(period)
        elseif core.crossesUnder(Source.close, ma.DATA, period) and (Line1[period] < OB) then
            SELL()
            ONE = Source:serial(period)
        end
    end
    ...
    if (instance.parameters.L == "C") then
        if core.crossesOver(Source.close, ma.DATA, period) and Line1[period] > Line2[period] then
            BUY()
            ONE = Source:serial(period)
        end
        if core.crossesUnder(Source.close, ma.DATA, period) and Line1[period] < Line2[period] then
            SELL()
            ONE = Source:serial(period)
        end
    end
