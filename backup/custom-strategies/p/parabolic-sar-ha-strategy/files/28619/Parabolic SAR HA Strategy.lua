...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=15116
...
function Init() --The strategy profile initialization
    strategy:name("Parabolic SAR HA Strategy")
    ...
    strategy.parameters:addGroup("Trend Strength")
    strategy.parameters:addInteger("Period", "Period", "", 14)
    strategy.parameters:addDouble("Threshold", "Threshold (pips)", "", 0)
    ...
    strategy.parameters:addString("ExitType", "Exit Type", "", "Both")
    strategy.parameters:addStringAlternative("ExitType", "Both", "", "Both")
    strategy.parameters:addStringAlternative("ExitType", "SAR", "", "SAR")
    strategy.parameters:addStringAlternative("ExitType", "HA", "", "HA")
...
function Prepare(nameOnly)
    ...
    name = name .. ", " .. Parameters["Step"] .. ", " .. Parameters["Max"]
    ...
    first = math.max(Short[1]:first(), Short[2]:first(), Short[3]:first(), Period)
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if
        not (Short[2]:hasData(period - 1)) and Short[2]:hasData(period) and
            Indicator[2].close[period] > Indicator[2].open[period] and
            math.abs(Source.close[period] - Source.open[period - Period]) / source:pipSize() > Threshold
     then
        if Direction then
            BUY()
        else
    ...
    elseif
        not (Short[1]:hasData(period - 1)) and Short[1]:hasData(period) and
            Indicator[2].close[period] < Indicator[2].open[period] and
            math.abs(Source.close[period] - Source.open[period - Period]) / source:pipSize() > Threshold
     then
        if Direction then
    ...
    if Exit then
        if
            (not (Short[2]:hasData(period)) and ExitType ~= "HA") or
                (Indicator[2].close[period] < Indicator[2].open[period] and ExitType ~= "SAR")
         then
            if Direction then
        ...
        if
            (not (Short[1]:hasData(period)) and ExitType ~= "HA") or
                (Indicator[2].close[period] > Indicator[2].open[period] and ExitType ~= "SAR")
         then
            if Direction then
                if haveTrades("S") then
