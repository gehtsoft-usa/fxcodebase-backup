...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=68419
...
local STRATEGY_NAME = "Price action Strategy";
...
function CreateParameters() 
    strategy.parameters:addDouble("distance", "Distance, pips", "", 50);
end
local distance;
...
function SetCustomStop(position_desc, command, period)
    local trade = trading:FindTrade()
        :WhenInstrument(position_desc.Instrument)
        :WhenSide(position_desc.Side)
        :WhenCustomID(custom_id)
        :First();
    if trade == nil then
        return false;
    end
    local stopOrder = trading:FindStopOrder(trade);
    if stopOrder == nil then
        return false;
    end
    command:SetStop(stopOrder.Rate, position_desc.Trailing);
    return true; 
end
function SetCustomLimit(position_desc, command, period) 
    ...
    local limitOrder = trading:FindLimitOrder(trade);
    if limitOrder == nil then
        return false;
    end
    command:SetLimit(limitOrder.Rate);
    return true; 
end
...
function CreateCustomActions()
    ...
    enterLongAction.IsPass = function (source, period, periodFromLast, data)
        local DistanceB = (source.close[period] - source.open[period]) / source:pipSize();
        if DistanceB < distance then
            return false;
        end

        if trading:FindTrade()
            :WhenInstrument(source:instrument())
            :WhenSide("S")
            :WhenCustomID(custom_id):Any()
        then
            return false;
        end
        ...
        trading:FindTrade()
            ...
            :WhenCustomID(custom_id)
            :Do(function (trade)
                if last_trade == nil or trade.Time > last_trade.Time then
                    last_trade = trade;
                end
            end);
        ...
        return last_trade.Open < source.close[period];
    end
    ...
    enterShortAction.IsPass = function (source, period, periodFromLast, data)
        local DistanceB = (source.open[period] - source.close[period]) / source:pipSize();
        if DistanceB < distance then
            return false;
        end
...
function CreatePositionStrategy(source, side, id)
    ...
    if SaveCustomStopParameters ~= nil then
        SaveCustomStopParameters(position_strategy, id);
    end
    position_strategy.LimitType = instance.parameters:getString("limit_type" .. id);
    assert((position_strategy.LimitType ~= "stop" and position_strategy.LimitType ~= nil) or position_strategy.StopType == "pips", "To use limit based on stop you need to set stop in pips");
    if SaveCustomLimitParameters ~= nil then
