...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=62354
...
function Init() --The strategy profile initialization
    strategy:name("Parabolic SAR Channel Strategy")
    ...
    strategy.parameters:addString("TradeType", "Trade Type", "", "Any")
    strategy.parameters:addStringAlternative("TradeType", "Trend Continuation", "", "TrendContinuation")
    strategy.parameters:addStringAlternative("TradeType", "Trend Reversal", "", "TrendReversal")
    strategy.parameters:addStringAlternative("TradeType", "Any", "", "Any")
...
function Prepare(nameOnly)
    ...
    assert(core.indicators:findIndicator("PSARC") ~= nil, "Please, download and install PSARC.LUA indicator")
    ...
    Indicator = core.indicators:create("PSARC", Source, Step, Max)
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if
        Source.close[period] > Indicator.Top[period] and Source.close[period - 1] <= Indicator.Top[period - 1] and
            TradeType ~= "TrendContinuation"
     then
        if Direction then
            BUY()
        else
    ...
    if
        Source.close[period] > Indicator.Bottom[period] and Source.close[period - 1] <= Indicator.Bottom[period - 1] and
            TradeType ~= "TrendReversal"
     then
        if Direction then
            BUY()
