...
--http://fxcodebase.com/code/viewtopic.php?f=31&t=69520
...
local STRATEGY_NAME = "Martinelli Strategy";
function CreateParameters() 
    strategy.parameters:addInteger("ForecastLen", "ForecastLen", "", 3);
    strategy.parameters:addInteger("StdDevLen", "StdDevLen", "", 7);
    strategy.parameters:addDouble("Cutoff", "Cutoff", "", 1.04);
end

local indi, Cutoff;
function CreateEntryIndicators(source)
    Cutoff = instance.parameters.Cutoff;
    local profile = core.indicators:findIndicator("MARTINELLI");
    assert(profile ~= nil, "Please, download and install " .. "MARTINELLI" .. ".LUA indicator");
    local indicatorParams = profile:parameters();
    indi = core.indicators:create("MARTINELLI", source, instance.parameters.ForecastLen, instance.parameters.StdDevLen);
end
...
function IsEntryLong(source, period)
    ...
    return core.crossesOver(indi.DATA, Cutoff, period);
end
function IsEntryShort(source, period)
    if not indi.DATA:hasData(period - 1) then
        return;
    end
    return core.crossesUnder(indi.DATA, -Cutoff, period);
end
