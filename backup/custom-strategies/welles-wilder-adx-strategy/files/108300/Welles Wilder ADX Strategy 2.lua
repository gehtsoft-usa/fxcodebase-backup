...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=63910
...
function Init() --The strategy profile initialization
    strategy:name("Welles Wilder ADX Strategy")
    ...
    strategy.parameters:addBoolean("ADX_Filter", "Use ADX Filter", "", true)
    strategy.parameters:addDouble("ADX_Level", "ADX Level", "", 20)

    strategy.parameters:addDouble("DMI_Level", "DMI Entry Level", "", 20)
    strategy.parameters:addDouble("DMI_Confirmation", "DMI Confirmation Level", "", 10)
...
local Direction
local CustomID
local ADX_Level, ADX_Filter
local DMI_Level, DMI_Confirmation
...
function Prepare(nameOnly)
    ...
    DMI_Confirmation = instance.parameters.DMI_Confirmation
    ...
    assert(
        core.indicators:findIndicator("WELLES WILDER ADX") ~= nil,
        "Please, download and install WELLES WILDER ADX.LUA indicator"
    )
    ...
    Indicator = core.indicators:create("WELLES WILDER ADX", Source, Period)
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if
        (not ADX_Filter or (Indicator.ADX[period] > ADX_Level and ADX_Filter)) and Indicator.DP[period] > DMI_Level and
            Indicator.DP[period - 1] <= DMI_Level and
            Indicator.DN[period] < DMI_Confirmation
     then
        if Direction then
            BUY()
        else
    ...
    elseif
        (not ADX_Filter or (Indicator.ADX[period] > ADX_Level and ADX_Filter)) and Indicator.DN[period] > DMI_Level and
            Indicator.DN[period - 1] <= DMI_Level and
            Indicator.DP[period] < DMI_Confirmation
     then
        if Direction then
            SELL()
        else
