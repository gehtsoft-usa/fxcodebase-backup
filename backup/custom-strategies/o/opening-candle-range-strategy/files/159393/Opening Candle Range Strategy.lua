-- Available @ https://fxcodebase.com/code/viewtopic.php?f=31&t=75978&p=159393#p159393
...
function Init() --The strategy profile initialization
    strategy:name("Opening Candle Range Strategy");
    strategy:description("Cross Out of the Box");
    ...
    strategy.parameters:setFlag("TF1", core.FLAG_PERIODS);
  	...
  	strategy.parameters:addString("TF2", "2. Time frame", "", "m30");
    strategy.parameters:setFlag("TF2", core.FLAG_PERIODS);

    strategy.parameters:addGroup("Calculation");
	
    strategy.parameters:addInteger("hour", "Hour", "Hour at start of which the line must be drawn", 9, 0, 23); 
	strategy.parameters:addInteger("minute", "Minutes", "Minute at start of which the line must be drawn", 30, 0, 59);
	
    strategy.parameters:addBoolean("CrossOut", "Cross Out of Range", "", true); 		
    strategy.parameters:addBoolean("BigCandle", "Bigger than the previous candle", "", true); 		
	strategy.parameters:addDouble("Multiplier", "Bigger than the previous candle Multiplier", "", 4);	
    strategy.parameters:addBoolean("Second", "Second cross Only", "", true);	

	
    strategy.parameters:addString("PriceType", "Cross Line Type", "", "Later");
    strategy.parameters:addStringAlternative("PriceType", "Initial Range", "", "Initial");
    strategy.parameters:addStringAlternative("PriceType", "Extended Range", "", "Later");	
	
    strategy.parameters:addString("ExitType", "Exit Type", "", "Later");
    strategy.parameters:addStringAlternative("ExitType", "Initial Range", "", "Initial");
    strategy.parameters:addStringAlternative("ExitType", "Shifted Range", "", "Shifted");		
    strategy.parameters:addStringAlternative("ExitType", "Exit is NOT used", "", "No");	
...
function CreateTradingParameters()
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "K1S");
...
function Prepare( nameOnly)
	...
	BigCandle = instance.parameters.BigCandle;
	Multiplier = instance.parameters.Multiplier;
	CrossOut = instance.parameters.CrossOut;
    ...
    s, e = core.getcandle(TF1,0, 0, 0);
	TF1Size= e-s;
	
    s, e = core.getcandle(TF2,0, 0, 0);
	TF2Size= e-s;	
...
function ExtUpdate(id, source, period)  -- The method called every time when a new bid or ask price appears.
   ...
   -- get only time
    ...
    if not Source1:hasData(period1) then
	return;
	end
 
    local date= Source1:date(period1);   
    local begin = core.getcandle("D1", date,0,0);
   
    local startdate= begin+hour*HourTimeFrameSize+minute*MinuteTimeFrameSize; 
   
    local period3=core.findDate (Source2, startdate, false)  



    if not Source2:hasData(period3) or  not Source2:hasData(period2) then
	return;
	end
    ...
    ExitFunction(now,period1, period2, period3);
	end

end


function ExitFunction( now,period1, period2, period3)
      ...
      return ;
	   ...
	   if ( LastExit == Source1:serial(period1))   then
	 return;
	 end	  
	  
    

		            if  ( Source1.close[period1] < Source2.low[period3] and ExitType=="Initial" )
					or  ( Source1.close[period1] < Source2.high[period3] and ExitType=="Shifted" ) 
					then
								...
								LastExit= Source1:serial(period1);
					  end
					  if (Source1.close[period1] > Source2.high[period3] and ExitType =="Initial" )  
					  or  ( Source1.close[period1] > Source2.low[period3] and ExitType=="Shifted" ) 
					  then
...
function EntryFunction( now,period1, period2, period3)
	...
	if PriceType ~= "Initial" and period3 < period2  then
	max= Source2.high[period3] 
	min= Source2.low[period3]
	
		for i= period3, period2, 1 do
				if 	Source2.close[i] > max 
				and Source2.close[i-1] < max				
				then
				max=mathex.max(Source2.high, period3, i)
				break;
				end
				
				if 	Source2.close[i] < min 
				and Source2.close[i-1] > min				
				then
				min=mathex.min(Source2.low, period3, i)
				break;				
				end
		end
	else		
    ...
    if  ( Source1.close[period1] > max and Source1.close[period1-1] <= max and CrossOut or not CrossOut ) and (Second and Source2.high[period3] < max or not Second)
	and (BigCandle and (math.abs(Source1.close[period1]- Source1.open[period1]) > math.abs(Source1.close[period1 -1 ]- Source1.open[period1 -1 ]) * Multiplier)or not BigCandle)
	then	
          if Direction then
                BUY();
            else
		...
		Return=true;
    elseif  ( Source1.close[period1] < min and Source1.close[period1-1] >= min and CrossOut or not CrossOut )  and (Second and Source2.low[period3] > min or not Second)
	and (BigCandle and (math.abs(Source1.close[period1]- Source1.open[period1]) > math.abs(Source1.close[period1 -1 ]- Source1.open[period1 -1 ])*Multiplier )or not BigCandle)	
	then
            if Direction then
                SELL();
