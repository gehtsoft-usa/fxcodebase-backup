...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=69543
...
local STRATEGY_NAME = "Open Close Strategy";
function CreateParameters() 
    strategy.parameters:addInteger("period_1", "OMA Period", "", 14)
    strategy.parameters:addInteger("period_2", "CMA Period", "", 14);
    strategy.parameters:addInteger("Period", "Wilders Trailing Stop Period", "", 5);
    strategy.parameters:addDouble("Coeff", "Wilders Trailing Stop Coeff", "", 3.5);
    strategy.parameters:addInteger("rsi_period", "RSI period", "", 14);
    strategy.parameters:addInteger("ema_rsi_period", "EMA of RSI period", "", 14);
end

local HA, OMA, CMA, wtt, rsi, ema_rsi;
function CreateEntryIndicators(source)
    HA = core.indicators:create("HA", source);
    OMA = core.indicators:create("EMA", HA.open, instance.parameters.period_1);
    CMA = core.indicators:create("EMA", HA.close, instance.parameters.period_2);
    rsi = core.indicators:create("RSI", source, instance.parameters.rsi_period);
    ema_rsi = core.indicators:create("EMA", rsi.DATA, instance.parameters.ema_rsi_period);

    local profile = core.indicators:findIndicator("WILDERS_TRAILING_STOP");
    assert(profile ~= nil, "Please, download and install " .. "WILDERS_TRAILING_STOP" .. ".LUA indicator");
    local indicatorParams = profile:parameters();
    wtt = core.indicators:create("WILDERS_TRAILING_STOP", source, instance.parameters.Period, instance.parameters.Coeff);
end

function UpdateIndicators()
    HA:update(core.UpdateLast);
    wtt:update(core.UpdateLast);
    OMA:update(core.UpdateLast);
    CMA:update(core.UpdateLast);
    rsi:update(core.UpdateLast);
    ema_rsi:update(core.UpdateLast);
end

function IsEntryLong(source, period)
    if not CMA.DATA:hasData(period - 1) or not OMA.DATA:hasData(period - 1) then
        return;
    end
    return core.crossesOver(CMA.DATA, OMA.DATA, period) and wtt.DATA[period] < source.close[period];
end
function IsEntryShort(source, period)
    if not CMA.DATA:hasData(period - 1) or not OMA.DATA:hasData(period - 1) then
        return;
    end
    return core.crossesUnder(CMA.DATA, OMA.DATA, period) and wtt.DATA[period] > source.close[period];
end
function IsExitLong1(source, period)
    if not ema_rsi.DATA:hasData(period - 1) then
        return;
    end
    return core.crossesOver(ema_rsi.DATA, rsi.DATA, period) and rsi.DATA[period] > 75;
end
function IsExitLong2(source, period)
    return core.crossesUnder(CMA.DATA, OMA.DATA, period);
end
function IsExitShort1(source, period)
    if not ema_rsi.DATA:hasData(period - 1) then
        return;
    end
    return core.crossesUnder(ema_rsi.DATA, rsi.DATA, period) and rsi.DATA[period] > 75;
end
function IsExitShort2(source, period)
    return core.crossesOver(CMA.DATA, OMA.DATA, period);
end
...
function ExtUpdate(id, source, period)
    ...
    if IsExitLong1(main_source, entry_period) and last_exit ~= main_source:date(NOW) then
        if AllowTrade then
            CloseTrades("B");
    ...
    if IsExitLong2(main_source, entry_period) and last_exit ~= main_source:date(NOW) then
        if AllowTrade then
            local enum = core.host:findTable("trades"):enumerator();
            local row = enum:next();
            while row ~= nil do
                if row.Instrument == main_source:instrument() and row.QTXT == custom_id .. "1" or row.QTXT == custom_id .. "2" or row.QTXT == custom_id .. "3" then
                    trading:Close(row);
                end
                row = enum:next();
            end
    ...
    if IsExitShort1(main_source, entry_period) and last_exit ~= main_source:date(NOW) then
        if AllowTrade then
            CloseTrades("S");
    ...
    if IsExitShort2(main_source, entry_period) and last_exit ~= main_source:date(NOW) then
        if AllowTrade then
            local enum = core.host:findTable("trades"):enumerator();
            local row = enum:next();
            while row ~= nil do
...
function ExtAsyncOperationFinished(cookie, success, message, message1, message2)
    if cookie == 2000 then
        ...
        if close_trade then
            ...
            if closed_trade ~= nil and closed_trade.OQTXT == custom_id .. "1" then
                local enum = core.host:findTable("trades"):enumerator();
                local row = enum:next();
                while row ~= nil do
                    if row.QTXT == custom_id .. "2" or row.QTXT == custom_id .. "3" then
                        trading:MoveStop(row, math.max(row.Open, wtt.DATA[NOW]))
                    end
                    row = enum:next();
                end
            end
...
function OpenTrade(side, id, allow_limit)
    ...
    valuemap.CustomID = custom_id .. id;
    if use_stop then
        local stop = math.max(stop_pips, (main_source.close[NOW] - wtt.DATA[NOW]) / main_source:pipSize());
    ...
    if use_limit and allow_limit then
