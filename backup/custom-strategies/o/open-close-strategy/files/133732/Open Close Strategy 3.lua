...
local HA, OMA, CMA, rsi, ema_rsi, tp, sl;
...
function Init()
    ...
    strategy.parameters:addDouble("limit1_pips", "Limit 1, pips", "", 20);
    strategy.parameters:addDouble("limit2_pips", "Limit 2, pips", "", 40);
    strategy.parameters:addDouble("limit3_pips", "Limit 3, pips", "", 60);
...
local _show_alert, _sound_file, _recurrent_sound, _email, limit1_pips, limit2_pips, limit3_pips;
local _ToTime;
function Prepare(nameOnly)
    ...
    limit1_pips = instance.parameters.limit1_pips;
    limit2_pips = instance.parameters.limit2_pips;
    limit3_pips = instance.parameters.limit3_pips;
...
function ExtUpdate(id, source, period)
    ...
    if IsEntryLong(main_source, entry_period) and last_entry ~= main_source:date(NOW) and not PositionsLimitHit() then
        if AllowTrade then
            ...
            OpenTrade("B", 1, limit1_pips);
            OpenTrade("B", 2, limit2_pips);
            OpenTrade("B", 3, limit3_pips);
        else
    ...
    if IsEntryShort(main_source, entry_period) and last_entry ~= main_source:date(NOW) and not PositionsLimitHit() then
        if AllowTrade then
            ...
            OpenTrade("S", 1, limit1_pips);
            OpenTrade("S", 2, limit2_pips);
            OpenTrade("S", 3, limit3_pips);
        else
    ...
    if cookie == 2000 then
        ...
        if close_trade then
            ...
            if closed_trade ~= nil then
                if closed_trade.OQTXT == custom_id .. "1" then
                    local enum = core.host:findTable("trades"):enumerator();
                    local row = enum:next();
                    while row ~= nil do
                        if row.QTXT == custom_id .. "2" or row.QTXT == custom_id .. "3" then
                            trading:MoveStop(row, row.Open)
                        end
                        row = enum:next();
                    end
                elseif closed_trade.OQTXT == custom_id .. "2" then
                    local enum = core.host:findTable("trades"):enumerator();
                    local row = enum:next();
                    while row ~= nil do
                        if row.QTXT == custom_id .. "3" then
                            if row.BS == "B" then
                                trading:MoveStop(row, row.Open + limit1_pips * main_source:pipSize())
                            else
                                trading:MoveStop(row, row.Open - limit1_pips * main_source:pipSize())
                            end
                        end
...
function OpenTrade(side, id, limit)
