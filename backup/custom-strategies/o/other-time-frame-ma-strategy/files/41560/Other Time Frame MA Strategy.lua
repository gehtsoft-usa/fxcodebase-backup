...
function Init() --The strategy profile initialization
    strategy:name("Other Time Frame MA Strategy");
   ...
   local i;
     ...
     strategy.parameters:addGroup("Price Time Frame");
	
    strategy.parameters:addString("PTF", "Price Time Frame", "", "D1");
	 strategy.parameters:setFlag( "PTF", core.FLAG_PERIODS);
	
	
	
	strategy.parameters:addString("MTF", "MA Time Frame", "", "M1");
	strategy.parameters:setFlag( "MTF", core.FLAG_PERIODS);
    ...
    strategy.parameters:addStringAlternative("Method", "WMA", "WMA" , "WMA");
	 ...
	 strategy.parameters:addInteger("Period", "Period", "", 12, 1, 10000);
...
function Prepare(nameOnly)
	...
	PTF = instance.parameters.PTF;
	MTF = instance.parameters.MTF;
	Price = instance.parameters.Price;
	Period = instance.parameters.Period
     Method = instance.parameters.Method;
	
   assert(PTF ~= "t1", "The time frame must not be tick");
   assert(MTF ~= "t1", "The time frame must not be tick");
   
    local s, e, s1, e1;
    s, e = core.getcandle(PTF, core.now(), 0, 0);
    s1, e1 = core.getcandle(MTF, core.now(), 0, 0);
	...
	Source[1] = ExtSubscribe(1, nil, PTF, instance.parameters.Type == "Bid", "bar");
    Source[2] = ExtSubscribe(2, nil, MTF, instance.parameters.Type == "Bid", "bar");
	
    assert(core.indicators:findIndicator(Method) ~= nil, Method .. " indicator must be installed");
    Indicator[1] = core.indicators:create(Method, Source[2][Price],  Period  );
...
function ExtUpdate(id, source, period)  -- The method called every time when a new bid or ask price appears.
   ...
   -- get only time
    ...
    local FLAG = true;
		...
		if Source[1].close:size() <= Source[1].close:first() + 2  
		or Indicator[1].DATA:size() <= Indicator[1].DATA:first() + 2 
		then
		   FLAG= false;
		end
