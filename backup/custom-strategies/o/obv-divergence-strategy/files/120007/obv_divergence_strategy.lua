...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=66283
...
local Modules = {};
function Init()
    strategy:name("OBV Divergence strategy");
...
function Prepare(name_only)
    ...
    assert(core.indicators:findIndicator("OBV_DIVERGENCE") ~= nil, "Please, download and install OBV_DIVERGENCE indicator");
    indi = core.indicators:create("OBV_DIVERGENCE", trading_logic.MainSource);
...
function EntryFunction(source, period)
    ...
    if period < 3 or last_serial == source.close:serial(period) then
        return;
    end

    if indi:getTextOutput(1):hasData(period - 3) then
        if instance.parameters.allow_trade then
            ...
            local order = trading:MarketOrder(source:instrument())
                :SetSide("B")
                :SetDefaultAmount()
                :SetCustomID(custom_id);
            if instance.parameters.set_stop then
                order = order:SetPipStop(nil, instance.parameters.stop, instance.parameters.use_trailing and instance.parameters.trailing);
            end
            if instance.parameters.set_limit then
                order = order:SetPipLimit(nil, instance.parameters.limit);
            end
            order:Execute();
        end
        last_serial = source.close:serial(period);
    end
    if indi:getTextOutput(0):hasData(period - 3) then
        if instance.parameters.allow_trade then
            if instance.parameters.close_on_opposite then
                trading:FindTrade()
                    :WhenSide("B")
...
function trading_logic:Init(parameters)
    ...
    parameters:addString("exit_execution_type", "Exit Execution Type", "", "Live");
