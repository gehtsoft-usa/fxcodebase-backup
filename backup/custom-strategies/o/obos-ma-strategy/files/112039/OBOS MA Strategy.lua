...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=64602
...
function Init() --The strategy profile initialization
    strategy:name("OBOS MA Strategy");
   ...
   strategy.parameters:addGroup("OB OS  Calculation");
    strategy.parameters:addInteger("OBOS_Period", "Period", "Period" , 9);
    strategy.parameters:addGroup("MA Calculation");
	 strategy.parameters:addInteger("Period", "Period", "Period" , 14);
    strategy.parameters:addString("Method", "MA Method", "Method" , "KAMA");
...
function CreateTradingParameters()
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "OBOSMAS");
...
function Prepare( nameOnly)
	...
	OBOS_Period= instance.parameters.OBOS_Period;
	...
	assert(core.indicators:findIndicator("OBOS INDICATOR") ~= nil, "Please, download and install OBOS INDICATOR.LUA indicator");    
    ...
    OBOS = core.indicators:create("OBOS INDICATOR", Source, OBOS_Period  );	
    assert(core.indicators:findIndicator(Method) ~= nil, Method .. " indicator must be installed");
	MA = core.indicators:create(Method, OBOS.close, Period  );
	first=MA.DATA:first(); 
...
function EntryFunction( now,period)
    ...
    if   OBOS.close[period]> MA.DATA[period]
	and   OBOS.close[period-1]<= MA.DATA[period-1]
	then	
          if Direction then
                BUY();
            else
		...
		Return=true;
    elseif   OBOS.close[period]< MA.DATA[period]
	and   OBOS.close[period-1]>= MA.DATA[period-1]
	then
            if Direction then
                SELL();
