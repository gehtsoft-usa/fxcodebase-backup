...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=63316
...
function Init() --The strategy profile initialization
    strategy:name("Open a position on New Candle Strategy")
    ...
    strategy.parameters:addString("TradeDirection", "Trade Direction", "", "Previous")
    strategy.parameters:addStringAlternative("TradeDirection", "Buy", "", "Buy")
    strategy.parameters:addStringAlternative("TradeDirection", "Sell", "", "Sell")
    strategy.parameters:addStringAlternative("TradeDirection", "Previous Candle Direction", "", "Previous")

    strategy.parameters:addBoolean("CloseAfterFirstTrade", "After First Trade close Strategy", "", false)
...
local Direction
local CustomID
local TradeDirection, CloseAfterFirstTrade
...
function Prepare(nameOnly)
    ...
    CloseAfterFirstTrade = instance.parameters.CloseAfterFirstTrade
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    local iTradeDirection

    if TradeDirection == "Previous" then
        if Source.close[period - 1] > Source.open[period - 1] then
            iTradeDirection = "Buy"
        else
            iTradeDirection = "Sell"
        end
    else
        iTradeDirection = TradeDirection
    end

    -- only buy if we have a fast cross over slow and the price is above the moving averages.
    if ONE ~= Source:serial(period) and iTradeDirection == "Buy" then
        if Direction then
            BUY()
        else
            SELL()
        ...
        if CloseAfterFirstTrade then
            core.host:execute("stop")
        end
    elseif ONE ~= Source:serial(period) and iTradeDirection == "Sell" then
        if Direction then
            SELL()
        else
            BUY()
