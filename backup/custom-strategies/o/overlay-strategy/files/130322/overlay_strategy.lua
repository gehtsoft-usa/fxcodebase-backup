...
--http://fxcodebase.com/code/viewtopic.php?f=31&t=69244
...
local STRATEGY_NAME = "Overlay Strategy";
...
function CreateParameters()
    strategy.parameters:addString("instrument", "Instrument", "", "EUR/USD");
    strategy.parameters:setFlag("instrument", core.FLAG_INSTRUMENTS);

    strategy.parameters:addInteger("bars_back", "Period", "", 300);
end
local counter_source, bars_back;
function CreateEntryIndicators(source) 
    bars_back = instance.parameters.bars_back;
    counter_source = trading_logic:SubscribeHistory(instance.parameters.instrument, source:barSize(), true);
end
...
function CreateCustomActions()
    ...
    enterLongAction.IsPass = function (source, period, periodFromLast, data)
        if period <= bars_back then
            return nil;
        end
        local index = core.findDate(counter_source, source:date(period - bars_back), false);
        if index < periodFromLast or index >= counter_source:size() - 1 - periodFromLast then
            return nil;
        end
        local min1, max1 = mathex.minmax(source, period - bars_back, period);
        local range1 = max1 - min1;
        local price1 = (source.close[period] - min1) / range1;

        local min2, max2 = mathex.minmax(counter_source, index, counter_source:size() - 1 - periodFromLast);
        local range2 = max2 - min2;
        local price2 = (counter_source.close[counter_source:size() - 1 - periodFromLast] - min2) / range2;
