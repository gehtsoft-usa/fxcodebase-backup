...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=63552
...
function Init() --The strategy profile initialization
    ...
    strategy.parameters:addInteger("Period", "Period", "Period", 8)
    strategy.parameters:addInteger("Signal_Period", "Signal Period", "Signal Period", 5)
...
local first
local Period, Signal_Period, Method
local Indicator
local Direction
local CustomID
...
function Prepare(nameOnly)
    ...
    assert(core.indicators:findIndicator("OSCAR") ~= nil, "Please, download and install OSCAR.LUA indicator")
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if
        Indicator.Oscar[period] > Indicator.Signal[period] and
            Indicator.Oscar[period - 1] <= Indicator.Signal[period - 1]
     then
        if Direction then
            BUY()
        else
