...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=61489
...
function Prepare(nameOnly)
    ...
    Indicator =
        core.indicators:create("OBOS", Source, Period, core.rgb(0, 255, 0), core.rgb(255, 0, 0), core.rgb(0, 0, 255))
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if Indicator.DATA:colorI(period) == core.rgb(255, 0, 0) and Indicator.DATA:colorI(period - 1) ~= core.rgb(255, 0, 0) then
        SELL()
        ONE = Source:serial(period)
    end

    if Indicator.DATA:colorI(period) == core.rgb(0, 0, 255) and Indicator.DATA:colorI(period - 1) ~= core.rgb(0, 0, 255) then
        if Exit and haveTrades("B") then
            exitSpecific("B")
            Signal("Close Long")
        end

        if Exit and haveTrades("S") then
            exitSpecific("S")
            Signal("Close Short")
        end
    end
