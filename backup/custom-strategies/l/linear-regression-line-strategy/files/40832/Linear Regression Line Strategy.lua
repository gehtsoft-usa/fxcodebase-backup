...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=23741
...
function Init() --The strategy profile initialization
    strategy:name("Linear Regression Line Strategy")
    ...
    strategy.parameters:addInteger("SP", "Short Period", "Perios", 20)
    strategy.parameters:addInteger("LP", "Long Period", "Perios", 50)
...
function Prepare(nameOnly)
    ...
    assert(
        core.indicators:findIndicator("LINEAR REGRESSION LINE") ~= nil,
        "Please, download and install Linear Regression Line.LUA indicator"
    )
    ...
    Indicator[1] = core.indicators:create("LINEAR REGRESSION LINE", Source[Price], SP)
    Indicator[2] = core.indicators:create("LINEAR REGRESSION LINE", Source[Price], LP)
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if Source.close[period] < Short[2][period] then
        if Direction then
