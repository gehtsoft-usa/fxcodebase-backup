...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=63406
...
function Init() --The strategy profile initialization
    strategy:name("Level Stop Reverse / Smoothed RSI  Strategy")
    ...
    strategy.parameters:addGroup("Smoothed RSI  Parameters")
    ...
    strategy.parameters:addGroup("Level Stop Reverse  Calculation")
    strategy.parameters:addString("Mode", "Method", "Method", "ATR")
    strategy.parameters:addStringAlternative("Mode", "ATR", "", "ATR")
    strategy.parameters:addStringAlternative("Mode", "Smoothed ATR", "", "SATR")
    strategy.parameters:addStringAlternative("Mode", "PIP", "", "PIP")

    strategy.parameters:addGroup("ATR")
    strategy.parameters:addDouble("AP", "ATR Period", "", 14)
    strategy.parameters:addDouble("Multiplier", "Multiplier", "", 3)

    strategy.parameters:addGroup("Smoothed ATR")
    strategy.parameters:addDouble("MP", "Smoothed Period", "", 14)
    strategy.parameters:addDouble("SMultiplier", "SmoothedMultiplier", "", 2.824)

    strategy.parameters:addGroup("Pip")
    strategy.parameters:addDouble("Pip", "Pip Distance", "", 50)
...
local first
local Method, RSI_Period, MA_Period
local Indicator, LSR
local Direction
local CustomID
local Mode, AP, Multiplier, MP, SMultiplier, Pip
...
function Prepare(nameOnly)
    ...
    assert(
        core.indicators:findIndicator("SMOOTHED RSI") ~= nil,
        "Please download and install SMOOTHED RSI.lua Indicator!"
    )
    assert(core.indicators:findIndicator("LSR") ~= nil, "Please download and install LSR.lua Indicator!")
    ...
    Indicator = core.indicators:create("SMOOTHED RSI", Source.close, RSI_Period, MA_Period, Method)
    LSR = core.indicators:create("LSR", Source, Mode, AP, Multiplier, MP, SMultiplier, Pip)
    first = math.max(Indicator.DATA:first(), LSR.DATA:first())
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if
        Indicator.DATA[period] > 50 and Source.close[period] > LSR.DATA[period] and
            Source.close[period - 1] <= LSR.DATA[period - 1]
     then
        if Direction then
            BUY()
        else
