...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=4050
...
function Init() --The strategy profile initialization
    strategy:name("Logarithmic regression strategy")
    strategy:description("Logarithmic regression strategy")
    strategy:setTag(
    ...
    strategy.parameters:addGroup("Logarithmic regression parameters")
...
function Prepare(nameOnly)
            ...
            instance.bid:name() ..
                "." ..
                    instance.parameters.TF ..
                        "," .. "LR(" .. instance.parameters.Period .. "," .. instance.parameters.Deviation .. "))"
    instance:name(name)
    if nameOnly then
        return
    end
    ...
    assert(
        core.indicators:findIndicator("LOGARITHMIC_REGRESSION") ~= nil,
        "Please, download and install LOGARITHMIC_REGRESSION.LUA indicator"
    )
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    LR:update(core.UpdateLast)

    -- Check that we have enough data
    if (LR.DATA:first() > (period - 1)) then
        return
    end
    ...
    if
        (source.low[period - 2] < LR.BuffBandUp[period - 2] + instance.parameters.Dist * source:pipSize() and
            source.high[period - 1] >= LR.BuffBandUp[period - 1] + instance.parameters.Dist * source:pipSize())
     then
    ...
    if
        (source.high[period - 2] > LR.BuffBandDn[period - 2] - instance.parameters.Dist * source:pipSize() and
            source.low[period - 1] <= LR.BuffBandDn[period - 1] - instance.parameters.Dist * source:pipSize())
     then
        if (instance.parameters.TypeSignal == "direct") then
            MustB = true
        else
