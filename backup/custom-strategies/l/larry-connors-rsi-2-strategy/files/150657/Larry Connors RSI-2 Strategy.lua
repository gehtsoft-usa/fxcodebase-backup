-- Available @ https://fxcodebase.com/code/viewtopic.php?f=31&t=73667
...
function Init() --The strategy profile initialization
    strategy:name("Larry Connors RSI-2 Strategy");
    ...
    strategy.parameters:addInteger("RSI_Period", "RSI Period", "", 2);  
    strategy.parameters:addDouble("Delta", "RSI Delta", "", 10, 0, 50);  	
 
    strategy.parameters:addInteger("Fast_Period", "Fast MA Period", "", 5); 
    strategy.parameters:addInteger("Slow_Period", "Slow MA Period", "", 200); 	
...
function CreateTradingParameters()
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "LCR2S");
...
function Prepare( nameOnly)
    ...
    Fast = core.indicators:create("MVA", Source.close, instance.parameters.Fast_Period );
    Slow = core.indicators:create("MVA", Source.close, instance.parameters.Slow_Period );	
	first=math.max(RSI.DATA:first(),Fast.DATA:first(),Slow.DATA:first()); 
...
--	period3= core.findDate (Source3, TickSource:date(period), false);
	...
	--/////////////////////////////////////////////////////////////////////
		            ...
		            if  Source.close[period] > Fast.DATA[period] then
...
function EntryFunction( id, now,period)
	...
	c1=1
	elseif Source.close[period] < Fast.DATA[period] then
	c1=-1	
	end
	...
	if RSI.DATA[period]< Delta then
	c3=1
	elseif RSI.DATA[period]>100- Delta then
	c3=-1	
	end
