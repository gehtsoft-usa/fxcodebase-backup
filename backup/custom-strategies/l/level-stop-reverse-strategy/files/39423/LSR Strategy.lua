...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=22850
...
function Init() --The strategy profile initialization
    strategy:name("Level Stop Reverse Strategy")
    ...
    strategy.parameters:addGroup("Selection")
...
function Prepare(nameOnly)
    ...
    Indicator = core.indicators:create("LSR", Source, Mode, AP, Multiplier, MP, SMultiplier, Pip)
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if period < first or not Short:hasData(period - 1) then
        return
    end

    -- only buy if we have a fast cross over slow and the price is above the moving averages.
    if core.crossesOver(Source.close, Short, period) then
        if Direction then
            BUY()
        else
            SELL()
    ...
    elseif core.crossesUnder(Source.close, Short, period) then
        if Direction then
            SELL()
        else
            BUY()
