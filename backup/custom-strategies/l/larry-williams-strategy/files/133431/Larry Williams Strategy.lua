...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=69665
...
function Init() --The strategy profile initialization
    strategy:name("Larry Williams Strategy")
    ...
    strategy.parameters:addGroup("Trade Time Frame")
    ...
    strategy.parameters:addGroup("Higher Time Frame")

    strategy.parameters:addBoolean("HigherTimeFrame", "HigherTimeFrame", "", true)
    strategy.parameters:addString("TF2", "Time frame", "", "D1")
    ...
    strategy.parameters:addString("Action" .. 1, "-20 Line Cross Over", "", "NO")
    ...
    strategy.parameters:addString("Action" .. 2, "-20 Line Cross Under", "", "NO")
    ...
    strategy.parameters:addString("Action" .. 3, "-50 Line Cross Over", "", "NO")
    ...
    strategy.parameters:addString("Action" .. 4, "-50 Cross Under", "", "NO")
    ...
    strategy.parameters:addString("Action" .. 5, "-80 Line  Cross Over", "", "NO")
    ...
    strategy.parameters:addString("Action" .. 6, "-80 Line Cross Under", "", "NO")
...
function Prepare(nameOnly)
    ...
    HigherTimeFrame = instance.parameters.HigherTimeFrame
    ...
    assert(TF1 ~= "t1", "Trade time frame must not be tick")
    assert(TF2 ~= "t1", "Higher time frame must not be tick")
    assert(TF1 > TF2, "Higher time frame should be higher then Trade time frame")
    ...
    Indicator1 = core.indicators:create("RLW", Source1, instance.parameters.Period1)
    Indicator2 = core.indicators:create("RLW", Source2, instance.parameters.Period2)
...
function EntryFunction(id, now, period)
    ...
    if not Indicator1:getStream(0):hasData(period1) or not Indicator2:getStream(0):hasData(period2) then
        return
    end

    if core.crossesOver(Indicator1:getStream(0), -20, period1) then
        ACTION(1, "-20", "Over", period1, period2)
    end
    if core.crossesUnder(Indicator1:getStream(0), -20, period1) then
        ACTION(2, "-20", "Under", period1, period2)
    end
    if core.crossesOver(Indicator1:getStream(0), -50, period1) then
        ACTION(3, "-50", "Over", period1, period2)
    end
    if core.crossesUnder(Indicator1:getStream(0), -50, period1) then
        ACTION(4, "-50", "Under", period1, period2)
    end
    if core.crossesOver(Indicator1:getStream(0), -80, period1) then
        ACTION(5, "-80", "Over", period1, period2)
    end
    if core.crossesUnder(Indicator1:getStream(0), -80, period1) then
        ACTION(6, "-80", "Under", period1, period2)
    end

    return Return
end

function ACTION(Flag, Line, Label, period1, period2)
    if Action[Flag] == "NO" then
        return
    elseif Action[Flag] == "BUY" and ((HigherTimeFrame and Indicator2.DATA[period2] > -50) or not HigherTimeFrame) then
        BUY()
    elseif Action[Flag] == "SELL" and ((HigherTimeFrame and Indicator2.DATA[period2] < -50) or not HigherTimeFrame) then
        SELL()
