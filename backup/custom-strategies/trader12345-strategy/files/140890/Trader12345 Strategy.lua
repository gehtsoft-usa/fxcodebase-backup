...
--http://fxcodebase.com/code/viewtopic.php?f=31&t=70953
...
local STRATEGY_NAME = "Trader12345 Strategy";
function CreateParameters() 
    strategy.parameters:addInteger("step", "Step, pips", "", 10);
end
...
local entry_execution_type, use_position_limit, position_limit;
local _show_alert, _sound_file, _recurrent_sound, _email;
local _ToTime, OpenTime, CloseTime;
local use_mandatory_closing, exit_time, trade_direction, step;
...
local last_entry, last_exit, last_bar, first_order_created;
function ExtUpdate(id, source, period)
    ...
    if first_order_created or OrderExists(RoundPrice(side, main_source.close[NOW])) or PositionExists(side, main_source.close[NOW]) then
        return;
    end
    CreateOrderForPrice(side, main_source.close[NOW]);
    first_order_created = true;
end

function RoundPrice(side, price)
    local close_in_pips = price / main_source:pipSize();
    close_in_pips = math.floor(close_in_pips / step + 0.5) * step;
    price = close_in_pips * main_source:pipSize();
    if side == "S" then
        if price <= price then
            price = price + step * main_source:pipSize();
        end
    else
        if price >= price then
            price = price - step * main_source:pipSize();
...
function PositionExists(side, price)
    price = RoundPrice(side, price);
    ...
    while row ~= nil do
        ...
        then
            if math.abs(row.Open - price) / main_source:pipSize() < step / 2 then
                return true;
            end
        end
        row = enum:next();
...
function CreateOrderForPrice(side, price)
    ...
    valuemap.Rate = RoundPrice(side, price);
    if side == "S" then
        valuemap.RateLimit = valuemap.Rate - step * main_source:pipSize();
    else
        valuemap.RateLimit = valuemap.Rate + step * main_source:pipSize();
    end
    ...
    core.host:trace("creating order")
end

function OrderExists(price)
    local enum = core.host:findTable("orders"):enumerator()
    local row = enum:next()
    while row ~= nil do
        if row.AccountID == Account and row.Instrument == main_source:instrument() and (row.QTXT == custom_id or custom_id == "") and row.Rate == price then
            core.host:trace("order exitst")
            return true;
        end
        row = enum:next()
    end
...
function DeleteOrders()
    ...
    local row = enum:next()
    while row ~= nil do
        if row.AccountID == Account and row.Instrument == main_source:instrument() and (row.QTXT == custom_id or custom_id == "") then
            local valuemap = core.valuemap()
