...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=2449
...
function Init()
    strategy:name("Set stop by SAR")
    strategy:description("Strategy changes stop followiung to the SAR indicator")
                        ...
                        "]" ..
                            "," ..
                                instance.parameters.Step ..
                                    "," .. "," .. instance.parameters.Max .. "," .. instance.parameters.Trade .. ")"
    instance:name(name)
    if onlyName then
        return
    end
...
function ExtUpdate(id, source, period)
    if id == 1 and tradeExist and instance.parameters.AllowTrade then
        sar:update(core.UpdateLast)
        period = sar.DATA:size() - 1
        if period >= sar.DATA:first() then
