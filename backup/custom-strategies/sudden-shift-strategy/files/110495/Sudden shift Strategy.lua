...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=64293
...
function Init() --The strategy profile initialization
    strategy:name("Sudden shift Strategy")
    ...
    strategy.parameters:addInteger("Period", "Period (in seconds)", "", 60)
    strategy.parameters:addDouble("Level", "Alert Level (in pips)", "", 10)

    strategy.parameters:addString("TradeDirection", "Trade Direction", "", "Direction")
    strategy.parameters:addStringAlternative("TradeDirection", "In Shift Direction", "", "Direction")
    strategy.parameters:addStringAlternative("TradeDirection", "Opposite to Shift Direction", "", "Opposite")
    strategy.parameters:addStringAlternative("TradeDirection", "Close All ", "", "Close")
    strategy.parameters:addStringAlternative("TradeDirection", "Close In Shift Direction", "", "CloseDirection")
    strategy.parameters:addStringAlternative("TradeDirection", "Close Opposite to Shift Direction", "", "CloseOpposite")
    strategy.parameters:addBoolean("StrategyStop", "Strategy Stop, after Execution ", "", true)
...
local Period, Level, TradeDirection, StrategyStop  --Direction,Opposite,Long,Short,Close;
local BaseUnit
local TimeFlagDown = nil
local TimeFlagUp = nil
...
function Prepare(nameOnly)
    ...
    StrategyStop = instance.parameters.StrategyStop
    ...
    Source = ExtSubscribe(1, nil, "t1", instance.parameters.Type == "Bid", "close")
    first = Source:first()

    assert(Source:barSize() == "t1", "The time frame must be tick")
...
function EntryFunction(period, now)
    ...
    -- local Period, Level, TradeDirection, Stop; --Direction,Opposite,Close;
    ...
    if (TradeDirection == "Direction" and Trade == 1) or (TradeDirection == "Opposite" and Trade == -1) then
        if Direction then
            BUY()
        else
            SELL()
    ...
    elseif (TradeDirection == "Direction" and Trade == -1) or (TradeDirection == "Opposite" and Trade == 1) then
        if Direction then
            SELL()
        else
            BUY()
    ...
    if (TradeDirection == "CloseDirection" and Trade == 1) or (TradeDirection == "CloseOpposite" and Trade == -1) then
