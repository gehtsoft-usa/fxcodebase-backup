...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=64953
...
function Init() --The strategy profile initialization
    strategy:name("ImpulseBar Strategy")
    ...
    strategy.parameters:addInteger("FastEMA", "FastEMA", "", 12)
    strategy.parameters:addInteger("SlowEMA", "SlowEMA", "", 26)
    strategy.parameters:addInteger("SignalEMA", "SignalEMA", "", 9)
...
local FastEMA, SlowEMA, SignalEMA, app_price, Indicator
...
function ExitFunction(now, period)
    ...
    if Indicator.open:colorI(period) ~= core.rgb(0, 255, 0) then
        if Direction then
    ...
    if Indicator.open:colorI(period) ~= core.rgb(255, 0, 0) then
        if Direction then
            if haveTrades("S") then
                exitSpecific("S")
                Signal("Close Short")
...
function EntryFunction(now, period)
    ...
    if Indicator.open:colorI(period) == core.rgb(0, 255, 0) and Indicator.open:colorI(period - 1) ~= core.rgb(0, 255, 0) then
        if Direction then
            BUY()
        else
            SELL()
        ...
        Return = true
    elseif
        Indicator.open:colorI(period) == core.rgb(255, 0, 0) and
            Indicator.open:colorI(period - 1) ~= core.rgb(255, 0, 0)
     then
        if Direction then
            SELL()
        else
