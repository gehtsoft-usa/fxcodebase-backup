...
function Init() --The strategy profile initialization
    strategy:name("GHLA MURREY OSSILATOR STRATEGY");
...
function CreateTradingParameters()
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "GMOS");
...
function Prepare( nameOnly)
    ...
    GHLA = core.indicators:create("GHLA  TOUCHLINE", Source, Period, Method  );
    MMO = core.indicators:create("MURREYS MATH OSCILLATOR", Source, length, mult  );
    first=math.max(GHLA.DATA:first(), MMO.DATA:first()-1)+1;
...
function ExtUpdate(id, source, period)  -- The method called every time when a new bid or ask price appears.
   ...
   -- get only time
    ...
    if   MMO.DATA[period] > BuyLevel 
	and Source.close[period] > GHLA.DATA[period]
	and   Source.close[period-1] <= GHLA.DATA[period-1]
	then	
          if Direction then
                BUY();
            else
		...
		ONE= Source:serial(Source:size()-1);
    elseif  MMO.DATA[period] < SellLevel 
	and Source.close[period] < GHLA.DATA[period]
	and   Source.close[period-1] >= GHLA.DATA[period-1]
	then
            if Direction then
