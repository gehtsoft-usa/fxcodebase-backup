...
--http://fxcodebase.com/code/viewtopic.php?f=31&t=62072
...
function Init() --The strategy profile initialization
    strategy:name("MTF GHLA MURREY OSCILLATOR STRATEGY");
    ...
    strategy.parameters:addGroup("GHLA Calculation");
	
	strategy.parameters:addString("TF1", "Time frame", "", "m15");
    strategy.parameters:setFlag("TF1", core.FLAG_PERIODS);
	
    strategy.parameters:addInteger("Period", "Period", "", 34, 1, 2000);
	
	strategy.parameters:addString("Method", "Method", "Method" , "EMA");
	...
	strategy.parameters:addGroup("Murreys Math Oscillator  Calculation");
	strategy.parameters:addString("TF2", "Time frame", "", "H4");
    strategy.parameters:setFlag("TF2", core.FLAG_PERIODS);
	strategy.parameters:addInteger("length", "Look back Length", "Look back Length", 100);
    strategy.parameters:addDouble("mult", "Mutiplier", "Mutiplier", 0.125);
	
	strategy.parameters:addDouble("BuyLevel", "Buy Level", "Buy Level", 0.75);
	strategy.parameters:addDouble("SellLevel", "Sell Level", "Sell Level", -0.75);
...
function CreateTradingParameters()
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "MGMOS");
...
function Prepare( nameOnly)
	...
	assert(core.indicators:findIndicator("GHLA  TOUCHLINE") ~= nil, "Please, download and install GHLA  TOUCHLINE.LUA indicator");   
    ...
    GHLA = core.indicators:create("GHLA  TOUCHLINE", Source1, Period, Method  );
    MMO = core.indicators:create("MURREYS MATH OSCILLATOR", Source2, length, mult  );
...
function ExtUpdate(id, source, period)  -- The method called every time when a new bid or ask price appears.
   ...
   -- get only time
	...
	MMO:update(core.UpdateLast);

	--BUY: MURRAY MATH OSSILATOR > BUY LEVEL IN H4 AND PRICE CROSS OVER GHLA IN M15
     --SELL: MURRAY MATH OSSILATOR <SELL LEVEL IN H4 AND PRICE CROSS UNDER GHLA IN M15     
	
    
    -- only buy if we have a fast cross over slow and the price is above the moving averages.
    if   MMO.DATA[period2] > BuyLevel 
	and Source1.close[period1] > GHLA.DATA[period1]
	and   Source1.close[period1-1] <= GHLA.DATA[period1-1]
	then	
          if Direction then
                BUY();
            else
		...
		ONE= Source1:serial(Source1:size()-1);
    elseif   MMO.DATA[period2]  < SellLevel 
	and  Source1.close[period1] < GHLA.DATA[period1]
	and   Source1.close[period1-1] >= GHLA.DATA[period1-1]
	then
            if Direction then
