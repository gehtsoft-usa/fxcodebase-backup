...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=66896
...
function Init()
    ...
    strategy:description("v1 2018-11-07");

    strategy.parameters:addInteger("Renko_Step", "Renko step", "", 5);
    strategy.parameters:addInteger("renko_count", "Renko count", "", 2);
...
function Prepare(name_only)
    ...
    renko = core.indicators:create("RENKO_CANDLES_NEW", nil, instance.bid:instrument(), 
        instance.parameters.timeframe, instance.parameters.is_bid, instance.parameters.Renko_Step, false);
    local valid;
...
function IsBuyCondition(source, period)
    core.host:trace(tostring(renko.open:size() ));
    if renko.open:size() < instance.parameters.renko_count then
        return false;
    end
    for shift = 0, instance.parameters.renko_count - 1 do
        if renko.open[NOW - shift] > renko.close[NOW - shift] then
            return false;
        end
    end
    return last_serial ~= renko.open:serial(NOW);
end
...
function IsExitBuyCondition(source, period)
    return renko.open[NOW] > renko.close[NOW];
end
...
function EntryFunction(source, period)
    ...
    if IsBuyCondition(source, period) then
        ...
        last_serial = renko.open:serial(NOW);
    end
