...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=66212
...
function handleHistory(cookie)
    ...
    if (s ~= -1) then
        local to = history.close:date(history.close:first());

        if (cookie ~= 3001 and current < barLimit and lastLoaded ~= to) and not core.host.Trading:getTradingProperty("isSimulation") then
