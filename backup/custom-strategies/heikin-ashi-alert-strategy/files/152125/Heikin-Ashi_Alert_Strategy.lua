...
-- https://fxcodebase.com/code/viewtopic.php?f=31&t=74046
...
local STRATEGY_NAME = "Heikin-Ashi Alert Strategy";

function AddInstrumentParams(symbol, index)
    strategy.parameters:addString("symbol_" .. index, "Symbol #" .. index, "", symbol);
    strategy.parameters:addBoolean("inv_" .. index, "Invert signal for #" .. index, "", false);
end
function CreateParameters() 
    AddInstrumentParams("EUR/USD", 1);
    AddInstrumentParams("EUR/USD", 2);
    AddInstrumentParams("EUR/USD", 3);
    AddInstrumentParams("EUR/USD", 4);
    AddInstrumentParams("EUR/USD", 5);
    AddInstrumentParams("EUR/USD", 6);
end
...
function CreateEntryIndicators(source)
    ...
    for i = 1, 6, 1 do
        local symbol = instance.parameters:getString("symbol_" .. i);
        inv[i] = instance.parameters:getBoolean("inv_" .. i);
        symbols[i] = core.host:execute("getHistory1", 100 + i, symbol, source:barSize(), 300, 0, source:isBid());

        local profile = core.indicators:findIndicator("HEIKIN-ASHI_ALERT");
        assert(profile ~= nil, "Please, download and install HEIKIN-ASHI_ALERT.LUA indicator");
        local indicatorParams = profile:parameters();
        indicatorParams:setBoolean("in_strategy", true);
        indi[i] = core.indicators:create("HEIKIN-ASHI_ALERT", symbols[i], indicatorParams);
    end
end
...
function IsEntryLong(source, period)
    for i = 1, 6, 1 do
        if not indi[i].Alert3:hasData(period - 1) then
            return false;
        end
        local signal = inv[i] and -1 or 1;
        if indi[i].Alert3[period - 1] ~= signal then
            return false;
        end
    end
    return true;
...
function Init()
    ...
    strategy.parameters:addString("entry_execution_type", "Execution Type", "Once per bar close or on every tick", "EndOfTurn");
