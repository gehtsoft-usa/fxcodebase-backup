...
function Init()
	indicator:name("Heikin-Ashi Alert")
	...
	Parameters(1, "HA Trend Change", false)
	Parameters(2, "HA Trend Change (without wicks)", false)
	Parameters(3, "HA Trend Change (without wicks) First only", true)

	indicator.parameters:addBoolean("in_strategy", "Works in strategy", "For strategy use only. Do not change manually", false);
end
...
function Prepare(nameOnly)
	...
	for i = 1, Number, 1 do
		if not instance.parameters.in_strategy then
			Alert[i] = instance:addInternalStream(0, 0)
		else
			Alert[i] = instance:addStream("Alert" .. i, core.Line, "Alert", "Alert", core.colors().Red, 0, 0)
		end
...
function Activate(id, period)
	...
	-- "HA Trend Change"
	--"HA Trend Change (without wicks)"
	--"HA Trend Change (without wicks) First only"

	if id == 1 and ON[id] then
		if HA.close[period] > HA.open[period] and HA.close[period - 1] <= HA.open[period - 1] then
			Alert[id][period] = 1
			AlertLevel[id][period] = HA.low[period]
		...
		elseif HA.close[period] < HA.open[period] and HA.close[period - 1] >= HA.open[period - 1] then
			Alert[id][period] = -1
			AlertLevel[id][period] = HA.high[period]
	...
	if id == 2 and ON[id] then
		if HA.close[period] > HA.open[period] and HA.open[period] == HA.low[period] then
			Alert[id][period] = 1
			AlertLevel[id][period] = HA.low[period]
		...
		elseif HA.close[period] < HA.open[period] and HA.open[period] == HA.high[period] then
			Alert[id][period] = -1
			AlertLevel[id][period] = HA.high[period]
	...
	if id == 3 and ON[id] then
		if HA.close[period] > HA.open[period] then
			if HA.close[period - 1] < HA.open[period - 1] then
				Trend[period] = 0
			end

			if HA.open[period] == HA.low[period] and Trend[period] ~= 1 then
				Alert[id][period] = 1
				AlertLevel[id][period] = HA.low[period]
				Trend[period] = 1
		...
		elseif HA.close[period] < HA.open[period] then
			...
			if HA.open[period] == HA.high[period] and Trend[period] ~= -1 then
				Alert[id][period] = -1
				AlertLevel[id][period] = HA.high[period]
				Trend[period] = -1
				U[id] = nil
