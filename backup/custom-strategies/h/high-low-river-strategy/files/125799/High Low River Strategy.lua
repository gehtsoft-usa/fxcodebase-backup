...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=68351
...
function Init() --The strategy profile initialization
    strategy:name("High Low River Strategy");
	...
	strategy.parameters:addGroup("Signal Selector"); 
	strategy.parameters:addBoolean("Trend", "Trend change signal", "", true);   
	strategy.parameters:addBoolean("Pullback", "Pullback signal", "", true);   
	
    strategy.parameters:addGroup("MA Calculation"); 
    strategy.parameters:addInteger("Period1", "Period", "", 14, 2, 2000);
 
	
	strategy.parameters:addString("Method1", "Method", "Method" , "EMA");
	...
	strategy.parameters:addGroup("Price Smoothing Calculation"); 
    strategy.parameters:addInteger("Period2", "Period", "", 5, 2, 2000);
 
	
	strategy.parameters:addString("Method2", "Method", "Method" , "MVA");
	...
	strategy.parameters:addGroup("Offset Calculation"); 
	
	strategy.parameters:addInteger("Period", "Period", "", 14, 1, 2000);
	strategy.parameters:addString("Method", "Method", "Method" , "ATR");
    strategy.parameters:addStringAlternative("Method", "ATR", "ATR" , "ATR");
    strategy.parameters:addStringAlternative("Method", "Standard Deviation", "Standard Deviation" , "Standard Deviation");
	
	strategy.parameters:addDouble("Multiplier", "Multiplier", "", 0 );
	
	strategy.parameters:addGroup("Offset Smoothing Calculation"); 
    strategy.parameters:addInteger("Period3", "Period", "", 5, 2, 2000); 
	
	strategy.parameters:addString("Method3", "Method", "Method" , "MVA");
...
function CreateTradingParameters()
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "HLRS");
...
local Indicator;
local Period1, Method1, Period2, Method2, Period, Method,Multiplier,Period3,Method3;
local Trend, Pullback;
function Prepare( nameOnly)
	...
	assert(core.indicators:findIndicator("HIGH LOW RIVER") ~= nil, "Please, download and install HIGH LOW RIVER.LUA indicator");          
    ...
    Indicator = core.indicators:create("HIGH LOW RIVER", Source, Period1, Method1, Period2, Method2, Period, Method,Multiplier,Period3,Method3,  core.rgb(0, 255, 0),  core.rgb(255, 0, 0),  core.rgb(128, 128, 128) ,true, 100 );
...
function ExitFunction(id, now,period)
	...
	--/////////////////////////////////////////////////////////////////    
	or (ExitExecutionType== "EndOfTurn" and id~=2)
	--/////////////////////////////////////////////////////////////////    
	then
	return;
	end
...
function EntryFunction( id, now,period)
	...
	--/////////////////////////////////////////////////////////////////    
	or (EntryExecutionType== "EndOfTurn" and id~=2)
	--/////////////////////////////////////////////////////////////////    
	then
	return;
	end
    ...
    if  (( ( Trend and Indicator.DATA:colorI(period) ==  core.rgb(0, 255, 0)
	and   Indicator.DATA:colorI(period-1) ~=  core.rgb(0, 255, 0)
	) or not Trend)
	or ( ( Pullback and Indicator.DATA:colorI(period-1) ==  core.rgb(0, 255, 0) 
    and Source.close[period]> Indicator.Top:colorI(period)
	and Source.close[period-1]<= Indicator.Top:colorI(period-1)
	) or not Pullback)
	)
	and(Trend or Pullback)	
	then	
          if Direction then
                BUY();
            else
		...
		Return=true;
    elseif  (( ( Trend and Indicator.DATA:colorI(period) ==  core.rgb(255, 0, 0)
	and   Indicator.DATA:colorI(period-1) ~=  core.rgb(255, 0, 0) ) or not Trend)
	or ( ( Pullback and Indicator.DATA:colorI(period-1) ==  core.rgb(255, 0, 0)
	 and Source.close[period]< Indicator.Bottom:colorI(period)
	and Source.close[period-1]>= Indicator.Bottom:colorI(period-1)
	) or not Pullback)
	)
	and(Trend or Pullback)	
		then
