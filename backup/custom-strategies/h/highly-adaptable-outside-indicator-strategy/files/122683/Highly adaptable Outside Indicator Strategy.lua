...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=67136
--+------------------------------------------------------------------+
...
function Init() --The strategy profile initialization
    strategy:name("Highly adaptable Outside Indicator Strategy");
	...
	strategy.parameters:addString("Action".. 1, "Weak Up Candle", "", "BUY");
	...
	strategy.parameters:addString("Action".. 2, "Strong Up Candle", "", "BUY");
	...
	strategy.parameters:addString("Action".. 3, "Weak Down Candle", "", "SELL");
	...
	strategy.parameters:addString("Action".. 4, "Strong Down Candle", "", "SELL");
	...
	strategy.parameters:addString("Action".. 5, "Up Candle Pinbar", "", "CLOSE");
	...
	strategy.parameters:addString("Action".. 6, "Down Candle Pinbar", "", "CLOSE");
...
function CreateTradingParameters()
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "HAOIS");
...
function Prepare( nameOnly)
	...
	assert(core.indicators:findIndicator("OUTSIDE INDICATOR") ~= nil, "Please, download and install OUTSIDE INDICATOR.LUA indicator");
	...
	Indicator= core.indicators:create("OUTSIDE INDICATOR", Source, true);
   ...
   -- get only time
	...
	then
        return;
								...
								if   Indicator:getStream(0)[period]==1   then									
									ACTION(1  ,"Weak Up", period);								
								end
								if Indicator:getStream(0)[period]==2  then		
                                    ACTION(2  ,"Strong Up", period);	  
                                end									
					            if Indicator:getStream(0)[period]==-1  then	
								     ACTION(3 ,"Weak Down", period);	
								end	  
								if Indicator:getStream(0)[period]==-2  then	
								     ACTION(4  ,"Strong Down", period);	
								end	  
								if Indicator:getStream(0)[period]==3  then	
								     ACTION(5  ,"Up Pinbar", period);
                                end									 
								if Indicator:getStream(0)[period]==-3  then	
								    ACTION(6  ,"Down Pinbar", period);
					            end				
