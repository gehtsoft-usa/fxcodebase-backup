...
function Init() --The strategy profile initialization
    strategy:name("Outside Bar Strategy")
    ...
    strategy.parameters:addBoolean("OpenLong", "Allow Long", "", true)
    strategy.parameters:addBoolean("OpenShort", "Allow Short", "", true)
    strategy.parameters:addBoolean("Volume", "Use High Volume Filter", "", false)
    strategy.parameters:addBoolean("UseBreakeven", "Use Breakeven", "", true)
    strategy.parameters:addDouble("Minimum", "Minimum Profit for Breakeven", "", 5)
...
function moveStop(rate, side, row)
    ...
    if rate > instance.ask[NOW] and row.BS == "B" then
        return
    elseif rate < instance.bid[NOW] and row.BS == "S" then
        return
    end
    ...
    if order == nil then
        ...
        if CanClose then
            local trade = core.host:findTable("trades"):find("TradeID", trade.TradeID)
        ...
        -- =======================================================================
        if (math.abs(rate - order.Rate) / Source:pipSize()) > 1 then
            -- stop exists
            valuemap = core.valuemap()
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    --local NotFlag=false;
    if id == 1 and UseBreakeven and AllowTrade then
        ...
        row = enum:next()
        while (row ~= nil) do
            if row.AccountID == Account and row.OfferID == Offer and row.QTXT == CustomID and row.PL >= Minimum then
                --Ask (Kupujes)
                --Bid (Prodajes)
...
function EntryFunction(now, period)
    ...
    if
        Source.high[period] > Source.high[period - 1] and Source.low[period] < Source.low[period - 1] and
            Source.close[period] > Source.open[period] and
            (not Volume or (Volume and Source.volume[period] > Source.volume[period - 1]))
     then
        Signal = 1
    end
    ...
    if OpenLong and Signal == 1 then
        BUY(Source.low[period])

        LastEntry = Source:serial(period)
        Return = true
    elseif OpenShort and Signal == -1 then
        SELL(Source.high[period])
...
-- enter into the specified direction
function MarketOrder(BuySell, hCount, Stop)
    ...
    if SetLimit then
        if BuySell == "B" then
            valuemap.RateLimit = Stop + Limit * Source:pipSize()
        else
            valuemap.RateLimit = Stop - Limit * Source:pipSize()
        end
    end
