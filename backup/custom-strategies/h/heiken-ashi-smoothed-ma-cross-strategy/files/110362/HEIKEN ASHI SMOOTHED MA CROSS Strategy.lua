...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=64267
...
function Init() --The strategy profile initialization
    strategy:name("HEIKEN ASHI SMOOTHED MA CROSS Strategy")
    ...
    strategy.parameters:addString(
        "Method1",
        "The smoothing method for prices",
        "The methods marked by the star (*) requires to have approriate indicators installed",
        "MVA"
    )
    ...
    strategy.parameters:addString(
        "Method2",
        "The smoothing method for candles",
        "The methods marked by the star (*) requires to have approriate indicators installed",
        "MVA"
    )
    ...
    strategy.parameters:addString("pType", "MA Price Source", "", "Price")
    strategy.parameters:addStringAlternative("pType", "Reglar Candle", "", "Price")
    strategy.parameters:addStringAlternative("pType", "HASM Candle", "", "HASM")

    strategy.parameters:addString("Definition", "Candle Definition", "", "HL")
    strategy.parameters:addStringAlternative("Definition", "High Low", "", "HL")
    strategy.parameters:addStringAlternative("Definition", "Open Close", "", "OC")
    ...
    strategy.parameters:addStringAlternative("Method3", "Vidya (1995)*", "", "VIDYA")
    strategy.parameters:addStringAlternative("Method3", "Vidya (1992)*", "", "VIDYA92")
    strategy.parameters:addStringAlternative("Method3", "Wilders*", "", "WMA")

    strategy.parameters:addInteger("N3", "Periods to smooth prices", "", 6, 1, 1000)
...
local Method1, N1, Method2, N2
local Method3, N3, MA
local pType, Definition
function Prepare(nameOnly)
    ...
    pType = instance.parameters.pType
    ...
    assert(core.indicators:findIndicator("HASM") ~= nil, "Please, download and install HASM.LUA indicator")
    assert(
        core.indicators:findIndicator(Method1) ~= nil,
        "Please, download and install " .. Method1 .. ".LUA indicator"
    )
    assert(
        core.indicators:findIndicator(Method2) ~= nil,
        "Please, download and install " .. Method2 .. ".LUA indicator"
    )
    assert(
        core.indicators:findIndicator(Method3) ~= nil,
        "Please, download and install " .. Method3 .. ".LUA indicator"
    )
    ...
    Indicator =
        core.indicators:create("HASM", Source, Method1, N1, Method2, N2, core.rgb(0, 255, 0), core.rgb(255, 0, 0))

    if pType == "Price" then
        MA = core.indicators:create(Method3, Source.close, N3)
    else
        MA = core.indicators:create(Method3, Indicator.close, N3)
    end

    first = math.max(Indicator.DATA:first(), MA.DATA:first())
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if period < 0 or period < first then
        return
    end
    ...
    if not Source.close:hasData(period) then
        return
    end
...
function EntryFunction(period, now)
    ...
    if Definition == "HL" then
        -- only buy if we have a fast cross over slow and the price is above the moving averages.
        if
            MA.DATA[period] > math.max(Indicator.high[period], Indicator.low[period]) and
                MA.DATA[period - 1] <= math.max(Indicator.high[period - 1], Indicator.low[period - 1])
         then
            if Direction then
                BUY()
            else
            ...
            Return = true
        elseif
            MA.DATA[period] < math.min(Indicator.high[period], Indicator.low[period]) and
                MA.DATA[period - 1] >= math.min(Indicator.high[period - 1], Indicator.low[period - 1])
         then
            if Direction then
                SELL()
            else
        ...
        if
         ...
         then
            ...
            Return = true
        elseif
            MA.DATA[period] < math.min(Indicator.close[period], Indicator.open[period]) and
                MA.DATA[period - 1] >= math.min(Indicator.close[period - 1], Indicator.open[period - 1])
         then
            if Direction then
                SELL()
            else
