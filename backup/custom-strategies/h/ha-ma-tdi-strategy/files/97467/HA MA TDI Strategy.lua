...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=61565
...
function Init() --The strategy profile initialization
    strategy:name("HA MA TDI Strategy")
...
function Prepare(nameOnly)
    ...
    first = math.max(TDI.TS:first(), HA.DATA:first(), MA.DATA:first()) + 1
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    --cross above 68 or below 32 no alert or signal
    ...
    if
        HA.close[period] > HA.open[period] and HA.close[period] > MA.DATA[period] and
            MA.DATA[period] > MA.DATA[period - 1] and
            TDI.RSI_P[period] > TDI.MB[period] and
            core.crossesOver(TDI.RSI_P, TDI.TS, period) and
            (not Filter or TDI.RSI_P[period] < 68) and
            (not Filter or TDI.RSI_P[period] > 32)
     then
        if Direction then
            BUY()
        else
    ...
    elseif
        HA.close[period] < HA.open[period] and HA.close[period] < MA.DATA[period] and
            MA.DATA[period] < MA.DATA[period - 1] and
            TDI.RSI_P[period] < TDI.MB[period] and
            core.crossesUnder(TDI.RSI_P, TDI.TS, period) and
            (not Filter or TDI.RSI_P[period] < 68) and
            (not Filter or TDI.RSI_P[period] > 32)
     then
        if Direction then
