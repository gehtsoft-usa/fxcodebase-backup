...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=63255
...
function Init() --The strategy profile initialization
    strategy:name("HA OBOS Strategy")
    ...
    strategy.parameters:addInteger("Period", "Period", "", 9)
    strategy.parameters:addDouble("ExtremeOB", "ExtremeOB", "", 80)
    strategy.parameters:addDouble("ExtremeOS", "ExtremeOS", "", -80)
    strategy.parameters:addString("Filter", "Filter Out", "", "Nothing")
    strategy.parameters:addStringAlternative("Filter", "Inside", "Inside", "Inside")
    strategy.parameters:addStringAlternative("Filter", "Outside", "Outside", "Outside")
    strategy.parameters:addStringAlternative("Filter", "Nothing", "Nothing", "Nothing")

    strategy.parameters:addString("SignalMethod", "Signal Method", "", "Direction")
    strategy.parameters:addStringAlternative("SignalMethod", "Direction", "Direction", "Direction")
    strategy.parameters:addStringAlternative("SignalMethod", "OB/OS Cross In", "OB/OS Cross", "CrossIn")
    strategy.parameters:addStringAlternative("SignalMethod", "OB/OS Cross Out", "OB/OS Cross", "CrossOut")
...
local Direction
local CustomID
local Period, ExtremeOB, ExtremeOS, Filter
...
function Prepare(nameOnly)
    ...
    OBOS = core.indicators:create("OBOS", HA:getCandleOutput(0), Period, core.rgb(0, 255, 0), core.rgb(255, 0, 0))
    first = OBOS.DATA:first()
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    OBOS:update(core.UpdateLast)

    if period < first then
        return
    end

    if Filter == "Outside" and ((OBOS.open[period] > ExtremeOB) or (OBOS.open[period] < ExtremeOS)) then
        return
    end

    if Filter == "Inside" and ((OBOS.open[period] < ExtremeOB) and (OBOS.open[period] > ExtremeOS)) then
        return
    end

    if SignalMethod == "Direction" then
        if OBOS.DATA:colorI(period) == core.rgb(0, 255, 0) and OBOS.DATA:colorI(period - 1) ~= core.rgb(0, 255, 0) then
            if Direction then
                BUY()
            else
                SELL()
        ...
        elseif OBOS.DATA:colorI(period) == core.rgb(255, 0, 0) and OBOS.DATA:colorI(period - 1) ~= core.rgb(255, 0, 0) then
            if Direction then
                SELL()
            else
                BUY()
    ...
    elseif SignalMethod == "CrossIn" then
        if OBOS.close[period] < ExtremeOS and OBOS.close[period - 1] >= ExtremeOS then
            if Direction then
                BUY()
            else
                SELL()
            ...
            ONE = Source:serial(period)
        elseif OBOS.close[period] > ExtremeOB and OBOS.close[period - 1] <= ExtremeOB then
            if Direction then
                SELL()
            else
                BUY()
    ...
    elseif SignalMethod == "CrossOut" then
        if OBOS.close[period] > ExtremeOS and OBOS.close[period - 1] <= ExtremeOS then
            if Direction then
                BUY()
            else
