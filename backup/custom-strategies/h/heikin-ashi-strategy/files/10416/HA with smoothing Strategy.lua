...
function Init() --The strategy profile initialization
    strategy:name("HA with smoothing Strategy")
    ...
    strategy.parameters:addGroup("Smoothing Parameters")
    strategy.parameters:addBoolean("Use", "Use Smoothing ", "", false)
...
function Prepare(nameOnly)
    ...
    assert(core.indicators:findIndicator(Method) ~= nil, Method .. " indicator must be installed");
        MAo = core.indicators:create(Method, HA.open, Period)
        MAc = core.indicators:create(Method, HA.close, Period)
        Short["open"] = MAo.DATA
        Short["close"] = MAc.DATA
    else
        Short["open"] = HA.open
        Short["close"] = HA.close
    end

    first = Short["open"]:first()
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if Use then
        MAo:update(core.UpdateLast)
        MAc:update(core.UpdateLast)
    end
    ...
    if Trigger and Level ~= 0.0 and LAST["ON"] == "OFF" then
        if Mode == "HA" then
            if Relativ == "PIP" then
                if LAST["SIDE"] == "LONG" and (Short["close"][period] - LAST["PRICE"]) / Source:pipSize() > Level then
                    BuySignal = true
                    LAST["ON"] = "ON"
                end
                if LAST["SIDE"] == "SHORT" and ((Short["close"][period] - LAST["PRICE"]) / Source:pipSize() < -Level) then
                    SellSignal = true
                    LAST["ON"] = "ON"
                end
            elseif Relativ == "Percentage" then
                if LAST["SIDE"] == "LONG" and (Short["close"][period] - LAST["PRICE"]) / (LAST["PRICE"] / 100) > Level then
                    BuySignal = true
                    LAST["ON"] = "ON"
                end
                if LAST["SIDE"] == "SHORT" and (Short["close"][period] - LAST["PRICE"]) / (LAST["PRICE"] / 100) < -Level then
                    SellSignal = true
                    LAST["ON"] = "ON"
                end
            end
        elseif Mode == "PRICE" then
            if Relativ == "PIP" then
                if LAST["SIDE"] == "LONG" and (Source.close[period] - LAST["PRICE"]) / Source:pipSize() > Level then
                    BuySignal = true
                    LAST["ON"] = "ON"
                end
                if LAST["SIDE"] == "SHORT" and (Source.close[period] - LAST["PRICE"]) / Source:pipSize() < -Level then
                    SellSignal = true
                    LAST["ON"] = "ON"
                end
            elseif Relativ == "Percentage" then
                if LAST["SIDE"] == "LONG" and (Source.close[period] - LAST["PRICE"]) / (LAST["PRICE"] / 100) > Level then
                    BuySignal = true
                    LAST["ON"] = "ON"
                end
                if LAST["SIDE"] == "SHORT" and (Source.close[period] - LAST["PRICE"]) / (LAST["PRICE"] / 100) < -Level then
                    SellSignal = true
                    LAST["ON"] = "ON"
                end
            end
        ...
        if Short["open"][period] < Short["close"][period] and Short["open"][period - 1] > Short["close"][period - 1] then
            BuySignal = true
        elseif Short["open"][period] > Short["close"][period] and Short["open"][period - 1] < Short["close"][period - 1] then
            SellSignal = true
        end
    end
