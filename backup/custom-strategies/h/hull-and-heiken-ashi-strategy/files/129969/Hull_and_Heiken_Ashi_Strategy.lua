...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=69154
...
local STRATEGY_NAME = "Hull and Heiken Ashi Strategy";
...
function CreateParameters()
    strategy.parameters:addInteger("hma_period", "Period", "Period", 35, 1, 2000);
end
...
function CreateEntryIndicators(source)
    ...
    indi = core.indicators:create("HMA", source, instance.parameters.hma_period);
end
...
function CreateCustomActions()
    ...
    enterLongAction.GetLog = function (source, period, periodFromLast, data)
        return "MA Rising: " .. tostring(indi.DATA[period] > indi.DATA[period - 1]) .. " Ascending candle: " .. tostring(source.close[period] > source.open[period]);
    end
    enterLongAction.IsPass = function (source, period, periodFromLast, data)
        return indi.DATA[period] > indi.DATA[period - 1] and source.close[period] > source.open[period];
    end
    ...
    enterShortAction.GetLog = function (source, period, periodFromLast, data)
        return "MA Falling: " .. tostring(indi.DATA[period] < indi.DATA[period - 1]) .. " Descending candle: " .. tostring(source.close[period] < source.open[period]);
    end
