...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=66914
...
function Init()
    strategy:name("Highly adaptable Belkhayates Center Of Gravity Strategy v4");
    ...
    strategy.parameters:addInteger("Len", "Len", "", 7, 1, 2000);
    strategy.parameters:addDouble("Filter", "Filter", "", 0);

    strategy.parameters:addBoolean("confirm_zz", "Confirm using ZZ Semafor", "", false);
    strategy.parameters:addInteger("Depth", "Depth", "The minimum number of periods used to draw one ZigZag line.", 5);
    strategy.parameters:addInteger("Deviation", "Deviation", "The maximum distance in pips by which the current high/low must be lower/higher than the previous one to return Backstep periods back to check if the current high/low is a new max/min.", 1);
    strategy.parameters:addInteger("Backstep", "Backstep", "The number of periods used to define a new min/max if the current high/low is lower/higher than the previous one by Deviation or less.", 3);
...
function Prepare(name_only)
    ...
    assert(core.indicators:findIndicator("MBFX TIMING") ~= nil, "Please, download and install MBFX TIMING.LUA indicator")
    
    instance:name(profile:id() .. "(" .. instance.bid:name() ..  ")");
    if name_only then return ; end

    indi = core.indicators:create("BELCOG", trading_logic.MainSource, N, O, E);
    mbfx = core.indicators:create("MBFX TIMING", trading_logic.MainSource, 
        instance.parameters.Len,
        instance.parameters.Filter);
    zz = core.indicators:create("ZZ_SEMAFOR", trading_logic.MainSource, 
        instance.parameters:getInteger("Depth"),
        instance.parameters:getInteger("Deviation"),
        instance.parameters:getInteger("Backstep"));
...
function UpdateIndicators()
    ...
    mbfx:update(core.UpdateLast);
    zz:update(core.UpdateLast);
end

function Rising(stream, period)
...
function IsBuyCondition(source, period)
    if period < 3 or not mbfx.DATA:hasData(period - 2) or not indi:getStream(6):hasData(period - 1) then
        return false;
    end
    if Rising(mbfx.DATA, period) == Rising(mbfx.DATA, period - 1) then
        return false;
    end
    if instance.parameters.confirm_zz and not HasSignal(zz, period) then
        return false;
    end
    return core.crossesOverOrTouch(source.close, indi:getStream(6), period)
        or core.crossesOverOrTouch(source.close, indi:getStream(5), period);
end

function IsSellCondition(source, period)
    if period < 3 or not mbfx.DATA:hasData(period - 2) or not indi:getStream(2):hasData(period - 1) then
        return false;
    end
    if Rising(mbfx.DATA, period) == Rising(mbfx.DATA, period - 1) then
        return false;
    ...
    return core.crossesOverOrTouch(source.close, indi:getStream(2), period)
        or core.crossesOverOrTouch(source.close, indi:getStream(3), period);
end

function IsExitBuyCondition(source, period)
    if period < 2 or not indi:getStream(0):hasData(period - 1) then
        return false;
    end
    return core.crosses(source.close, indi:getStream(0), period)
        or source.close[period] == indi:getStream(0):tick(period);
end
