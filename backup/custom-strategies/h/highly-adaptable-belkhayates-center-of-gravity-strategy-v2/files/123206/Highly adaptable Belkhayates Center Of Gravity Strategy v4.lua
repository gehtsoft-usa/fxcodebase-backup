...
function IsBuyCondition(source, period)
    if not mbfx.DATA:hasData(period - 2) then
        return;
    end
    ...
    return (indi:getStream(6):hasData(period - 1) and core.crossesOverOrTouch(source.close, indi:getStream(6), period))
        or (indi:getStream(5):hasData(period - 1) and core.crossesOverOrTouch(source.close, indi:getStream(5), period));
end
...
function IsSellCondition(source, period)
    ...
    return (indi:getStream(2):hasData(period - 1) and core.crossesOverOrTouch(source.close, indi:getStream(2), period))
        or (indi:getStream(3):hasData(period - 1) and core.crossesOverOrTouch(source.close, indi:getStream(3), period));
end

function IsExitBuyCondition(source, period)
    if not indi:getStream(0):hasData(period - 1) then
        return;
    end
