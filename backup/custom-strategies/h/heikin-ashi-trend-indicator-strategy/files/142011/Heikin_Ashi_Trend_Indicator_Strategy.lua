...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=69699
...
function Init() --The strategy profile initialization
    strategy:name("Heikin Ashi Trend Indicator Strategy")
    ...
    strategy.parameters:addInteger("adx_level", "ADX Level", "", 25);
...
function Prepare(nameOnly)
    ...
    assert(
        core.indicators:findIndicator("HEIKIN ASHI TREND INDICATOR") ~= nil,
        "Please, download and install HEIKIN ASHI TREND INDICATOR.LUA indicator"
    )
    ...
    Indicator = core.indicators:create("HEIKIN ASHI TREND INDICATOR", Source, true)
...
function EntryFunction(id, now, period)
    ...
    if Indicator.DATA[period] == 1 and Indicator.DATA[period - 1] ~= 1 then
        ...
        Return = true
    elseif Indicator.DATA[period] == -1 and Indicator.DATA[period - 1] ~= -1 then
        if Direction then
            SELL()
        else
            BUY()
