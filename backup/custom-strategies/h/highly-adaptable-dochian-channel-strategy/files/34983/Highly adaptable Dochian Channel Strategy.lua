...
function Init() --The strategy profile initialization
    strategy:name("Highly adaptable Dochian Channel Strategy")
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if core.crossesOver(Source.close, Indicators[1].DU, period) then
        ACTION(1)
        ONE = Source:serial(period)
    end
    if core.crossesUnder(Source.close, Indicators[1].DU, period) then
        ACTION(2)
        ONE = Source:serial(period)
    end
    if core.crossesOver(Source.close, Indicators[1].DN, period) then
        ACTION(3)
        ONE = Source:serial(period)
    end
    if core.crossesUnder(Source.close, Indicators[1].DN, period) then
        ACTION(4)
        ONE = Source:serial(period)
    end
    if core.crossesOver(Source.close, Indicators[1].DM, period) then
        ACTION(5)
        ONE = Source:serial(period)
    end
    if core.crossesUnder(Source.close, Indicators[1].DM, period) then
        ACTION(6)
        ONE = Source:serial(period)
    end
    if core.crossesOver(Source.close, Indicators[1].DU, period) then
    ...
    if core.crossesOver(Source.close, Indicators[1].high, period) then
        ACTION(9)
        ONE = Source:serial(period)
    end
    if core.crossesUnder(Source.close, Indicators[1].low, period) then
        ACTION(10)
        ONE = Source:serial(period)
    end
end
