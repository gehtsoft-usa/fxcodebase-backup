...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if core.crossesOver(Source.close, Indicators[1]:getStream(1), period) then
        ACTION(3, 2, "Over")
    end
    if core.crossesUnder(Source.close, Indicators[1]:getStream(1), period) then
        ACTION(4, 2, "Under")
    end
    if core.crossesOver(Source.close, Indicators[1]:getStream(2), period) then
        ACTION(5, 3, "Over")
    end
    if core.crossesUnder(Source.close, Indicators[1]:getStream(2), period) then
        ACTION(6, 3, "Under")
    end
    if core.crossesOver(Source.close, Indicators[1]:getStream(3), period) then
        ACTION(7, 4, "Over")
    end
    if core.crossesUnder(Source.close, Indicators[1]:getStream(3), period) then
        ACTION(8, 4, "Under")
    end

    if core.crossesOver(Source.close, Indicators[1]:getStream(4), period) then
        ACTION(9, 5, "Over")
    end
    if core.crossesUnder(Source.close, Indicators[1]:getStream(4), period) then
        ACTION(10, 5, "Under")
    end

    if core.crossesOver(Source.close, Indicators[1]:getStream(5), period) then
        ACTION(11, 6, "Over")
    end
    if core.crossesUnder(Source.close, Indicators[1]:getStream(5), period) then
        ACTION(12, 6, "Under")
    end

    if core.crossesOver(Source.close, Indicators[1]:getStream(6), period) then
        ACTION(13, 7, "Over")
    end
    if core.crossesUnder(Source.close, Indicators[1]:getStream(6), period) then
        ACTION(14, 7, "Under")
    end
end
