...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=66619
...
function Init() --The strategy profile initialization
    strategy:name("Higher Highs Lower Lows Strategy");
    ...
    strategy.parameters:addInteger("Length", "Period", "", 20);
...
function CreateTradingParameters()
  ...
  --  strategy.parameters:addStringAlternative("ExitExecutionType", "End of Turn", "", "EndOfTurn");
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "HHLLS");
...
function Prepare( nameOnly)
	...
	assert(core.indicators:findIndicator("HIGHER HIGHS LOWER LOWS") ~= nil, "Please, download and install HIGHER HIGHS LOWER LOWS.LUA indicator");          
    ...
    Indicator = core.indicators:create("HIGHER HIGHS LOWER LOWS", Source, Length  );
...
function EntryFunction( now,period)
    ...
    if   Indicator.HHS[period] > Indicator.LLS[period]
	and Indicator.HHS[period-1] <= Indicator.LLS[period-1]
	then	
          if Direction then
                BUY();
            else
		...
		Return=true;
    elseif   Indicator.HHS[period] < Indicator.LLS[period]
	and Indicator.HHS[period-1] >= Indicator.LLS[period-1]
		then
            if Direction then
                SELL();
