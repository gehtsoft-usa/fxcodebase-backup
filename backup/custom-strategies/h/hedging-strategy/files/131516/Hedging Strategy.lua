...
--http://fxcodebase.com/code/viewtopic.php?f=31&t=69459
...
local STRATEGY_NAME = "Hedging Strategy";
...
function IsEntryLong(source, period)
    if not ma.DATA:hasData(period - 1) then
        return false;
    end
    return core.crossesOver(ma.DATA, source.close, period);
end
function IsEntryShort(source, period)
    if not ma.DATA:hasData(period - 1) then
        return false;
    end
    return core.crossesUnder(ma.DATA, source.close, period);
end
function IsEntryLongHedging(source, period)
    return ha.open[period] > ha.close[period] and ha.open[period - 1] < ha.close[period - 1];
end
function IsEntryShortHedging(source, period)
    return ha.open[period] < ha.close[period] and ha.open[period - 1] > ha.close[period - 1];
end
...
local main_source;
local base_size, offer_id, Account, Amount, AllowTrade, close_on_opposite, custom_id, custom_id_hedging;
...
function ExtUpdate(id, source, period)
    ...
    if IsEntryLong(main_source, entry_period) and last_entry ~= main_source:date(NOW) then
        if AllowTrade then
            if close_on_opposite then
                CloseTrades("S", custom_id);
            end
            OpenTrade("B", custom_id);
        else
    ...
    if IsEntryShort(main_source, entry_period) and last_entry ~= main_source:date(NOW) then
        if AllowTrade then
            if close_on_opposite then
                CloseTrades("B", custom_id);
            end
            OpenTrade("S", custom_id);
        else
            Signal("Entry short", main_source);
        end
        last_entry = main_source:date(NOW);
    end
    if IsEntryLongHedging(main_source, entry_period) and last_entry ~= main_source:date(NOW) then
        if AllowTrade then
            if close_on_opposite then
                CloseTrades("S", custom_id_hedging);
            end
            OpenTrade("B", custom_id_hedging);
        else
            Signal("Entry long", main_source);
        end
        last_entry = main_source:date(NOW);
    end
    if IsEntryShortHedging(main_source, entry_period) and last_entry ~= main_source:date(NOW) then
        if AllowTrade then
            if close_on_opposite then
                CloseTrades("B", custom_id_hedging);
            end
            OpenTrade("S", custom_id_hedging);
        else
            Signal("Entry short", main_source);
        end
        last_entry = main_source:date(NOW);
