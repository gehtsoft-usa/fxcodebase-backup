...
-- https://fxcodebase.com/code/viewtopic.php?f=31&t=72127
...
function Init() --The strategy profile initialization
    strategy:name("HTF candle biased LTF ARS Strategy");
	...
	strategy.parameters:addString("TF1", "Lower Time frame", "", "m1");
    strategy.parameters:setFlag("TF1", core.FLAG_PERIODS);
    
	strategy.parameters:addString("TF2", "Higher Time frame", "", "m5");
...
function Prepare( nameOnly)
    ...
    Indicator = core.indicators:create("ARSI", Source1.close, instance.parameters.Period );
...
function EntryFunction( id, now,period)
    ...
    local Body= math.abs(Source2.close[period2]-Source2.close[period2]);
	local Top=Source2.high[period2] - math.max(Source2.close[period2],Source2.open[period2]);
	local Bottom=math.min(Source2.close[period2],Source2.open[period2])-Source2.low[period2];
    if Body< Top or Body < Bottom then
	ItIs=false;
    end
    ...
    if  Source1.close[period1]>  Indicator.DATA[period1] 
	and ItIs
	then	
          if Direction then
                BUY();
		...
		Return=true;
    elseif  Source1.close[period1]<  Indicator.DATA[period1]
	and ItIs 
	then
            if Direction then
                SELL();
