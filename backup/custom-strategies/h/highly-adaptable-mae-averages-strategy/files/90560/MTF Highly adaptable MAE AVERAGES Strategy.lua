...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=59802
...
function Init() --The strategy profile initialization
    strategy:name("MTF Highly adaptable MAE AVERAGES Strategy");
	...
	strategy.parameters:addString("TF1", "Chart Time frame", "", "H1");
    strategy.parameters:setFlag("TF1", core.FLAG_PERIODS);
	
	
	strategy.parameters:addString("TF2", "MAE Time frame", "", "H1");
	...
	strategy.parameters:addString("Action".. 1, "Cental Line Cross Over", "", "NO");
	...
	strategy.parameters:addString("Action".. 2, "Cental Line  Cross Under", "", "NO");
	...
	strategy.parameters:addString("Action".. 3, "Top Line Cross Over", "", "NO");
	...
	strategy.parameters:addString("Action".. 4, "Top Line Cross Under", "", "NO");
...
function CreateTradingParameters()
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "HAMAS");
...
--
    ...
    s1, e1 = core.getcandle(TF2, 0, 0, 0);
    assert ((e - s) <= (e1 - s1), "The MAE time frame must be equal to or bigger than the chart time frame!");
	 

	assert(core.indicators:findIndicator("MAE AVERAGES") ~= nil, "Please, download and install MAE AVERAGES.LUA indicator");
	...
	Indicator= core.indicators:create("MAE AVERAGES", Source2[Price], Method, Period, B, BWU, true);	
  ...
  now = core.host:execute("getServerTime");
   ...
   -- get only time
	...
	if  ExecutionType ==  "Live" and  id == 1 then
			...
			periodx1= core.findDate (Source2.close, TickSource:date(period), false );	
			periodx2= core.findDate (Source2.close, TickSource:date(period-1), false );	
   else
   period1= Source1.close:size()-1;
 
    periodx1= core.findDate (Source2.close, Source1:date(period1), false );	
	periodx2= core.findDate (Source2.close,Source1:date(period1-1), false );	
	end
    ...
    Indicator:update(core.UpdateLast);
	 ...
	 if Indicator.DATA:size() <= Indicator.DATA:first() + 2 
	 or Source1.close:size() <= Source1.close:first() + 2
	 then
	 return;
	 end
								...
								if Source1.close[period1] > Indicator:getStream(0)[periodx2]
								and Source1.close[period1-1] <= Indicator:getStream(0)[periodx1]
								then									
									ACTION(1,1 ,"Over");	
					            ...
					            if Source1.close[period1] > Indicator:getStream(1)[periodx2] 
								and Source1.close[period1-1] <= Indicator:getStream(1)[periodx1]
								then	
								     ACTION(3,2 ,"Over");	
									 ONE = Source1:serial(period1) 
								end	  
								...
								if Source1.close[period1] > Indicator:getStream(2)[periodx2]
								and Source1.close[period1-1] <= Indicator:getStream(2)[periodx1]
								then	
								     ACTION(5,3 ,"Over");
									 ONE = Source1:serial(period1) 
                                end									 
