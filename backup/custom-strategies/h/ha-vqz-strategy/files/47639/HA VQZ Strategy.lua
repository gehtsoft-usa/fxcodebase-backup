...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=27285
...
function Init() --The strategy profile initialization
    ...
    strategy.parameters:addGroup("VQZ Calculation")
    strategy.parameters:addInteger("Period", "Period", "", 7)
    ...
    strategy.parameters:addInteger("Smoothing", "Smoothong", "", 1)
    strategy.parameters:addInteger("Filter", "Filter", "", 5)

    strategy.parameters:addGroup("Optinal Exit")
    strategy.parameters:addBoolean("One", "Use VQZ Exit", "", true)
    strategy.parameters:addBoolean("Two", "Use HA Exit", "", true)
...
local Period, Method, Smoothing, Filter
local Exit, One, Two
...
function Prepare(nameOnly)
    ...
    name = name .. ", " .. Period .. ", " .. Method .. ", " .. Smoothing .. ", " .. Filter

    assert(core.indicators:findIndicator("AVERAGES") ~= nil, "Please, download and install AVERAGES.LUA indicator")
    assert(core.indicators:findIndicator("VQZI") ~= nil, "Please, download and install VQZI.LUA indicator")
    ...
    Indicator[2] = core.indicators:create("HA", Source)
    Short["O"] = Indicator[2]:getStream(0)
    Short["C"] = Indicator[2]:getStream(3)

    first = math.max(Short[1]:first(), Short["O"]:first())
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if
        Indicator[1].DATA:colorI(period) == core.rgb(0, 255, 0) and Short["C"][period] > Short["O"][period] and
            Short["C"][period - 1] < Short["O"][period - 1]
     then
        if Direction then
            BUY()
        else
    ...
    elseif
        Indicator[1].DATA:colorI(period) == core.rgb(255, 0, 0) and Short["C"][period] < Short["O"][period] and
            Short["C"][period - 1] > Short["O"][period - 1]
     then
        if Direction then
            SELL()
    ...
    if Exit then
        if
            (Indicator[1].DATA:colorI(period) ~= core.rgb(0, 255, 0) and One) and
                (Short["C"][period] > Short["O"][period] and Two)
         then
            if Direction then
        ...
        if
            (Indicator[1].DATA:colorI(period) ~= core.rgb(255, 0, 0) and One) and
                (Short["C"][period] < Short["O"][period] and Two)
         then
            if Direction then
                if haveTrades("S") then
