-- Available @https://fxcodebase.com/code/viewtopic.php?f=31&t=75381
...
local STRATEGY_NAME = "Heikin Ashi Bar Color Strategy";
...
function IsEntryLong(source, period)
    if not ha.DATA:hasData(period) then
        return false;
    end
    return ha.open[period] < ha.close[period];
end
...
function Init()
    ...
    strategy.parameters:addDouble("long_stop_pips", "Stop for long, pips", "", 10);
    strategy.parameters:addDouble("short_stop_pips", "Stop for short, pips", "", 10);
    ...
    strategy.parameters:addDouble("long_limit_pips", "Limit for long, pips", "", 20);
    strategy.parameters:addDouble("short_limit_pips", "Limit for short, pips", "", 20);
...
local use_stop, long_stop_pips, short_stop_pips, use_limit, long_limit_pips, short_limit_pips, entry_execution_type, use_trailing, trailing, use_position_limit, position_limit;
...
function Prepare(nameOnly)
    ...
    long_limit_pips = instance.parameters.long_limit_pips;
    short_limit_pips = instance.parameters.short_limit_pips;
    use_limit = instance.parameters.use_limit;
    use_stop = instance.parameters.use_stop;
    long_stop_pips = instance.parameters.long_stop_pips;
    short_stop_pips = instance.parameters.short_stop_pips;
...
function OpenTrade(side)
    ...
    if use_stop then
        ...
        if side == "B" then
            valuemap.PegPriceOffsetPipsStop = -long_stop_pips;
        else
            valuemap.PegPriceOffsetPipsStop = short_stop_pips;
        end
    ...
    if use_limit then
        ...
        if side == "B" then
            valuemap.PegPriceOffsetPipsLimit = long_limit_pips;
        else
            valuemap.PegPriceOffsetPipsLimit = -short_limit_pips;
        end
    end
