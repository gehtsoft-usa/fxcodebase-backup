...
function Init()
    ...
    strategy:name("Highly adaptable Renko CCI Strategy")
...
function Prepare(nameOnly)
    ...
    Renko =
        ...
        instance.parameters.PT == "Bid",
        instance.parameters.Renko_Size
    )
    assert(core.indicators:findIndicator("TICK_CCI") ~= nil, "TICK_CCI" .. " indicator must be installed");
    CCI = core.indicators:create("TICK_CCI", Renko.DATA, CCIP)
...
function ExtUpdate(id, source, period)
    ...
    local R_size = Renko.DATA:size() - 1
    if R_size <= 1 then
        return
    end
