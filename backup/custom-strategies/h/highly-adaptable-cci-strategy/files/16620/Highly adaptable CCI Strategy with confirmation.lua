...
function Init()
    ...
    strategy:name("Highly adaptable CCI Strategy with confirmation")
    strategy:description("")
    strategy:setTag("NonOptimizableParameters", "Email,SendEmail,SoundFile,RecurrentSound,PlaySound, ShowAlert")

    -- Fast and slow moving average parameters

    strategy.parameters:addGroup("The main CCI Selector")

    strategy.parameters:addString("Action" .. 1, "Overbought Level CrossOver Action", "", "NO")
    ...
    strategy.parameters:addString("Action" .. 2, "Overbought Level CrossUnder Action", "", "NO")
    ...
    strategy.parameters:addString("Action" .. 3, "Zero Line CrossOver Action", "", "NO")
    ...
    strategy.parameters:addString("Action" .. 4, "Zero Line CrossUnder Action", "", "NO")
    ...
    strategy.parameters:addString("Action" .. 5, "Oversold  Level CrossOver Action", "", "NO")
    ...
    strategy.parameters:addString("Action" .. 6, "Oversold  Level CrossUnder Action", "", "NO")
    ...
    strategy.parameters:addGroup("The main CCI  Parameter")
    strategy.parameters:addInteger("CCIP", "Indicator periods", "", 14, 2, 2000)

    strategy.parameters:addGroup("Main CCI Levels")
    strategy.parameters:addInteger("OB", "Overbought Level", "", 100)
    strategy.parameters:addInteger("OS", " Oversold Level", "", -100)

    strategy.parameters:addGroup("Confirmation CCI  Parameter")
    strategy.parameters:addString("Action" .. 7, "Overbought Confirmation Level permissible positions", "", "BOTH")
    strategy.parameters:addStringAlternative("Action" .. 7, "Both", "", "BOTH")
    strategy.parameters:addStringAlternative("Action" .. 7, "Sell", "", "SELL")
    strategy.parameters:addStringAlternative("Action" .. 7, "Buy", "", "BUY")

    strategy.parameters:addString("Action" .. 8, "Oversold Confirmation Level permissible positions", "", "BOTH")
    strategy.parameters:addStringAlternative("Action" .. 8, "Both", "", "BOTH")
    strategy.parameters:addStringAlternative("Action" .. 8, "Sell", "", "SELL")
    strategy.parameters:addStringAlternative("Action" .. 8, "Buy", "", "BUY")

    strategy.parameters:addGroup("Confirmation CCI Levels")
    strategy.parameters:addInteger("CCCIP", "Indicator periods", "", 50, 2, 2000)

    strategy.parameters:addInteger("COB", "Overbought Level", "", 75)
    strategy.parameters:addInteger("COS", " Oversold Level", "", -75)
...
function Prepare(nameOnly)
    ...
    CCCIP = instance.parameters.CCCIP
    local i
    ...
    Source = ExtSubscribe(1, nil, instance.parameters.TF, instance.parameters.PT == "Bid", "bar")
    CCI = core.indicators:create("CCI", Source, CCIP)
    CCCI = core.indicators:create("CCI", Source, CCCIP)

    first = math.max(CCI.DATA:first(), CCCI.DATA:first())
...
function ExtUpdate(id, source, period)
    ...
    CCCI:update(core.UpdateLast)

    if CCCI.DATA[period] > COB then
        if Action[7] == "BUY" then
            FLAG = "BUY"
        elseif Action[7] == "SELL" then
            FLAG = "SELL"
        else
            FLAG = "BOTH"
        end
    elseif CCCI.DATA[period] < COS then
        if Action[8] == "BUY" then
            FLAG = "BUY"
        elseif Action[8] == "SELL" then
            FLAG = "SELL"
        else
            FLAG = "BOTH"
        end
    ...
    elseif core.crossesUnder(CCI.DATA, OB, period) then
        ACTION(2)
    elseif core.crossesOver(CCI.DATA, 0, period) then
        ACTION(3)
    elseif core.crossesUnder(CCI.DATA, 0, period) then
        ACTION(4)
    elseif core.crossesOver(CCI.DATA, OS, period) then
        ACTION(5)
...
function ACTION(Flag)
    if Action[Flag] == "NO" then
        return
    elseif Action[Flag] == "BUY" and FLAG ~= "SELL" and FLAG ~= nil then
    ...
    elseif Action[Flag] == "SELL" and FLAG ~= "BUY" and FLAG ~= nil then
