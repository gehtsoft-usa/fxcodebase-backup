...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=62731
...
function Init() --The strategy profile initialization
    strategy:name("HA Extreme TMA Line Strategy")
...
function Prepare(nameOnly)
    ...
    first = math.max(Indicator.DATA:first(), HA.DATA:first())
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if
        HA.close[period] > HA.open[period] and HA.close[period - 1] < HA.open[period - 1] and
            HA.close[period] < Indicator.Lower[period]
     then
        if Direction then
            BUY()
        else
    ...
    elseif
        HA.close[period] < HA.open[period] and HA.close[period - 1] > HA.open[period - 1] and
            HA.close[period] > Indicator.Upper[period]
     then
        if Direction then
            SELL()
        else
