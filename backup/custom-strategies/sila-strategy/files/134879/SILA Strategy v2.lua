...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=69992
--+------------------------------------------------------------------+
...
local STRATEGY_NAME = "SILA Strategy";
...
function CreateParameters() 
    strategy.parameters:addInteger("sensup", "Uptrend-sensivity", "", 5, -8, 8);
    strategy.parameters:addInteger("sensdn", "Downtrend-sensivity", "", 5, -8, 8);
    strategy.parameters:addBoolean("usewow", "Use trend-indicator WOW?", "", true)
    strategy.parameters:addBoolean("usebma", "Use trend-indicator BestMA?", "", true)
    strategy.parameters:addBoolean("usebc", "Use trend-indicator BarColor?", "", true)
    strategy.parameters:addBoolean("usest", "Use trend-indicator SuperTrend?", "", true)
    strategy.parameters:addBoolean("usedi", "Use trend-indicator DI?", "", true)
    strategy.parameters:addBoolean("usetts", "Use trend-indicator TTS?", "", true)
    strategy.parameters:addBoolean("usersi", "Use trend-indicator RSI?", "", true)
    strategy.parameters:addBoolean("usewto", "Use trend-indicator WTO?", "", true)
    strategy.parameters:addInteger("dist", "Distance SILA-lines", "", 100, 0, 100);
    strategy.parameters:addBoolean("usemon", "Need money?", "", true)
end
local indi;
function CreateEntryIndicators(source)
    local profile = core.indicators:findIndicator("SILA INDICATOR");
    assert(profile ~= nil, "Please, download and install " .. "SILA INDICATOR" .. ".LUA indicator");
    local indicatorParams = profile:parameters();
    indicatorParams:setBoolean("usewow", instance.parameters.usewow);
    indicatorParams:setBoolean("usebma", instance.parameters.usebma);
    indicatorParams:setBoolean("usebc", instance.parameters.usebc);
    indicatorParams:setBoolean("usest", instance.parameters.usest);
    indicatorParams:setBoolean("usedi", instance.parameters.usedi);
    indicatorParams:setBoolean("usetts", instance.parameters.usetts);
    indicatorParams:setBoolean("usersi", instance.parameters.usersi);
    indicatorParams:setBoolean("usewto", instance.parameters.usewto);
    indicatorParams:setBoolean("usemon", instance.parameters.usemon);
    indicatorParams:setBoolean("strategy_output", true);
    indicatorParams:setInteger("sensup", instance.parameters.sensup);
    indicatorParams:setInteger("sensdn", instance.parameters.sensdn);
    indicatorParams:setInteger("dist", instance.parameters.dist);
    indi = core.indicators:create("SILA INDICATOR", source, indicatorParams)
end
...
function CreateCustomActions()
    ...
    enterLongAction.IsPass = function (source, period, periodFromLast, data)
        return indi.strat_signal_1[period] == 1;
    end
