...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=62319
...
function Init() --The strategy profile initialization
    strategy:name("CCI OBOS Crossover with OBOS confirmation Strategy")
    ...
    strategy.parameters:addGroup("OBOS Calculation")
    strategy.parameters:addInteger("OBOS_Period", "OBOS Period", "", 9)
...
local first
local OBOS_Period, CCI_Period, obos, cci
local Direction
local CustomID
local Exit
...
function Prepare(nameOnly)
    ...
    obos = core.indicators:create("OBOS", Source, OBOS_Period, core.rgb(0, 255, 0), core.rgb(255, 0, 0))
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if core.crossesOver(cci.DATA, obos.high, period) and obos.open:colorI(period) == core.rgb(0, 255, 0) then
        if Direction then
            BUY()
        else
            SELL()
    ...
    elseif core.crossesUnder(cci.DATA, obos.low, period) and obos.open:colorI(period) == core.rgb(255, 0, 0) then
        if Direction then
            SELL()
        else
            BUY()
