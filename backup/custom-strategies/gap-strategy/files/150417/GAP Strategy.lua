-- Available @ https://fxcodebase.com/code/viewtopic.php?f=31&t=73600
...
function Init() --The strategy profile initialization
    ...
    strategy.parameters:addDouble("Minimal", "Minimal GAP in pips", "", 10);  
...
function CreateTradingParameters()
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "GS");
...
function EntryFunction( id, now,period)
	...
	if (math.abs(Source.open[period] - Source.close[period-1]) / Source:pipSize())<Minimal then
	return;
	end
    ...
    if   Source.close[period]>  Source.open[period-1]
	then	
          if Direction then
                BUY();
            else
		...
		Return=true;
    elseif Source.close[period]< Source.open[period-1]
	then
            if Direction then
                SELL();
            else
