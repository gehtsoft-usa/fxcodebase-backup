...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=70528
--+------------------------------------------------------------------+
...
function CreateParameters() 
    ...
    strategy.parameters:addDouble("Price1", "1 Price Source", "", 0)
    strategy.parameters:setFlag("Price1", core.FLAG_PRICE)
    ...
    strategy.parameters:addDouble("Price2", "2 Price Source", "", 0)
    strategy.parameters:setFlag("Price2", core.FLAG_PRICE)
...
function CreateEntryIndicators(source)
    ...
    indi = core.indicators:create("LONGER SHORTER INDICATOR", source, indicatorParams)
...
function IsEntryLong(source, period)
    if not indi.DATA:hasData(period - 1) or not ma.DATA:hasData(period - 1) then
        return false;
    end
    return core.crossesOver(indi.entry_buy, ma.DATA, period);
end
function IsEntryShort(source, period)
    if not indi.DATA:hasData(period - 1) or not ma.DATA:hasData(period - 1) then
        return false;
    end
    return core.crossesUnder(indi.entry_sell, ma.DATA, period);
end
function IsExitLong(source, period)
    if not indi.DATA:hasData(period - 1) or not ma.DATA:hasData(period - 1) then
        return false;
    end
    return core.crossesUnder(indi.exit_buy, ma.DATA, period);
end
function IsExitShort(source, period)
    if not indi.DATA:hasData(period - 1) or not ma.DATA:hasData(period - 1) then
        return false;
    end
    return core.crossesOver(indi.exit_sell, ma.DATA, period);
end
