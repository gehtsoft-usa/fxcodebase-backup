...
function AddPrice(id, name)
    indicator.parameters:addDouble("Price" .. id, name .. " Price Source", "", 0)
    indicator.parameters:setFlag("Price" .. id, core.FLAG_PRICE)
end
...
function Prepare(nameOnly)
    ...
    long_src = instance.parameters.Price1;
    short_src = instance.parameters.Price2;
...
function Update(period, mode)
    ...
    LONGDNX = math.max(TRAILINGUP[period], long_src);
    ...
    if source.close[period] < short_src then
        local Dn1 = source.close[period] + Factor1 * atr.DATA[atr_period];
        TRAILINGDOWN[period] = math.max(Dn1, TRAILINGDOWN[period - 1]);
    else
        TRAILINGDOWN[period] = short_src;
    end
    SHORTUPX = math.min(TRAILINGDOWN[period], short_src);
    ...
    SHORTDNX = math.min(TRAILINGDOWN[period], short_src);
    ...
    if source.close[period] < SHORTDN1[period] then
        exit_sell[period] = math.min(SHORTUP, SHORTDN1[period - 1]);
    else
