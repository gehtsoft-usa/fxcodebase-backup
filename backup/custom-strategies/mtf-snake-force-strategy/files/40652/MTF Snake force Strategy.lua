...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=23619
...
function Init() --The strategy profile initialization
    strategy:name("MTF Snake force Strategy")
...
function TIMEFRAME(id, FRAME)
    ...
    strategy.parameters:addInteger("Period" .. id, " Period", "", 24)
    strategy.parameters:addDouble("OB" .. id, "Top Level (in Pips)", "", 2.5)
    strategy.parameters:addDouble("OS" .. id, "Bottom Level (in Pips)", "", 2.5)
end
...
function Prepare(nameOnly)
    ...
    for i = 1, TfNumber, 1 do
        ...
        assert(core.indicators:findIndicator("SF") ~= nil, "Please, download and install SF.LUA indicator")
            ...
            Indicator[i] = core.indicators:create("SF", Source[i][Price], Period[i])
...
function ExtUpdate(id, source, the_period) -- The method called every time when a new bid or ask price appears.
    if AllowTrade then
	...
	then
		for i = 1, TfNumber, 1 do
			...
			period[i] = core.findDate (Source[i] , Source[1]:date(the_period), false); 
			end
		end		
	elseif (EntryExecutionType== "Live" and id==1) 
	then
		for i = 1, TfNumber, 1 do
		period[i] = core.findDate (Source[i] , TickSource:date(the_period), false); 
		end
	else
	return;	
    end
    ...
    --BUY:
    --1.	1.FORCE UP > TOP LEVEL AND RESISTANCE DOWN CROSS OVER BOTTOM LEVEL IN TIME FRAME H1 { AND}
    --2.	2.FORCE UP > TOP LEVEL AND RESISTANCE DOWN =0 IN TIME FRAME D1

    if
        Indicator[1].ForceUp[period[1]] > OB[1] * source:pipSize()
		and core.crossesOver(Indicator[1].ResistanceDn, (-OS[1] * source:pipSize()), period[1])
		and (Indicator[2].ForceUp[period[2]] > OB[2] * source:pipSize() or not ON[2])
		and  (Indicator[2].ResistanceDn[period[2]] == 0 or not ON[2])
     then
        --SELL:
        --1.	FORCE DOWN <BOTTOM LEVEL AND RESISTANCE UP CROSS UNDER TOP LEVEL IN TIME FRAME H1 { AND}
        --2.	FORCE DOWN < BOTTOM LEVEL AND RESISTANCE UP =0 IN TIME FRAME D1
        if Direction then
            BUY()
        else
            SELL()
        end
    elseif  Indicator[1].ForceDn[period[1]] < -OS[1] * source:pipSize()
	        and core.crossesUnder(Indicator[1].ResistanceUp,(OB[1] * source:pipSize()), period[1] )
			and (Indicator[2].ForceDn[period[2]] < -OS[2] * source:pipSize() or not ON[2])
			and (Indicator[2].ResistanceUp[period[2]] == 0 or not ON[2])
     then
        if Direction then
            SELL()
        else
