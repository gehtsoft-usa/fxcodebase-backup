...
-- https://fxcodebase.com/code/viewtopic.php?f=31&t=71219
...
function Init() --The strategy profile initialization
    strategy:name("Quantum MACD Strategy");
	...
	strategy.parameters:addInteger("Period0", "Slope Period", "Period", 14);
    ...
    strategy.parameters:addGroup("Quantum Calculation");
	 ...
	 strategy.parameters:addBoolean("SignalMode", "SignalMode", "", false); 
...
function CreateTradingParameters()
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "QMS");
...
local QUANTUM,MACD;
local Period, ReversalOnly,SignalMode; 
local Period1,Period2,Period3,Period0;
function Prepare( nameOnly)
	...
	assert(core.indicators:findIndicator("QUANTUM SIGNAL OSCILLATOR") ~= nil, "Please, download and install QUANTUM SIGNAL OSCILLATOR.LUA indicator");          
	assert(core.indicators:findIndicator("MACD SLOPE") ~= nil, "Please, download and install MACD SLOPE.LUA indicator");   
    ...
    QUANTUM = core.indicators:create("QUANTUM SIGNAL OSCILLATOR", Source, Period, ReversalOnly,SignalMode  );
	MACD = core.indicators:create("MACD SLOPE", Source, Period0, Period1, Period2,Period3 ); 
    first=math.max(QUANTUM.DATA:first(), MACD.DATA:first()); 
...
local Return=0;
	...
	for i= period, first, -1 do
		if QUANTUM.DATA[i]~=0 then
		Return=QUANTUM.DATA[i];
		break;
		end 
...
function SELL ()		
    if AllowTrade then
        ...
        local bCount= tradesCount("S"); 	
				
        enter("S",bCount+Amount);
    else
        Signal ("Sell Signal");	
    end
end
