-- Available @ https://fxcodebase.com/code/viewtopic.php?f=31&t=74976
...
function Init() --The strategy profile initialization
    strategy:name("Quantum Entry MA Exit Strategy");
	...
	strategy.parameters:addBoolean("EntryFilter", "Use Entry Filter", "", false);	
    strategy.parameters:addString("PositionPosition", "Price position in relation to thea MA (Long Positions)", "", "Under");
    strategy.parameters:addStringAlternative("PositionPosition", "Over", "", "Over");
    strategy.parameters:addStringAlternative("PositionPosition", "Under", "", "Under");
	...
	strategy.parameters:addInteger("MA_Period", "Period", "Period", 34);	
...
function CreateTradingParameters()
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "QEMES");
...
function Prepare( nameOnly)
    ...
    PositionPosition= instance.parameters.PositionPosition;
    EntryFilter= instance.parameters.EntryFilter;	
	...
	MA  = core.indicators:create(Method, Source[Price], MA_Period );
	
	
	first=  math.max(MA.DATA:first(), Period) ; 
...
function EntryFunction( now,period)
    ...
    local Return=false;
	 ...
	 return;
	   ...
	   --PositionPosition, EntryFilter;	
	 
	 
	 if Direction then
	  
				-- only buy if we have a fast cross over slow and the price is above the moving averages.
				if   Quantum.Signal[period]==-1		
                and (not PositionPosition or (Source.close[period]< MA.DATA[period]))			
				then	
					...
					Return=true;
				elseif  Quantum.Signal[period]==1	
                and (not PositionPosition or (Source.close[period]> MA.DATA[period]))					
				then
