...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=9147
...
function Init() --The strategy profile initialization
    strategy:name("Qualitative Quantitative Estimation Strategy with HPF Smoothing")
    ...
    strategy.parameters:addString("Select", "Select Signal Line", "", "F")
    strategy.parameters:addStringAlternative("Select", "Fast trailing stop", "", "F")
    strategy.parameters:addStringAlternative("Select", "Slow trailing stop", "", "S")
    strategy.parameters:addBoolean("Use", "Use Confirmation Level", "", false)
    strategy.parameters:addDouble("Level", "Confirmation Level", "Confirmation Level", 50)

    strategy.parameters:addGroup("HPF Calculation")
    strategy.parameters:addInteger("Filter", "HP Filter Period", "", 50)
    strategy.parameters:addInteger("Bars", "Max Bars to calculate", "", 1000, 100, 1000)
...
local CustomID
local QQE
local Filter, Bars, Use, Level, Select, RF, RSP, AP, F, S
...
function Prepare(nameOnly)
    ...
    name = name .. ", " .. RF .. ", " .. RSP .. ", " .. AP .. ", " .. F .. ", " .. S .. ", " .. Level
    assert(core.indicators:findIndicator("QQE") ~= nil, "Please, download and install QQE.LUA indicator")
    assert(core.indicators:findIndicator("HPF") ~= nil, "Please, download and install HPF.LUA indicator")
    name = name .. " )"
    instance:name(name)
    ...
    Indicator[2] = core.indicators:create("HPF", Source, Filter)
    Indicator[1] = core.indicators:create("QQE", Indicator[2].DATA, RF, RSP, AP, F, S, true)
    Short[1] = Indicator[1]:getStream(0)
    Short[2] = Indicator[1]:getStream(1)
    Short[3] = Indicator[1]:getStream(2)

    first = math.max(Short[2]:first(), Short[1]:first())
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if period < first or Short[1]:hasData(period - 1) or not Short[2]:hasData(period - 1) or not Short[3]:hasData(period - 1) then
        return
    end

    if Select == "F" then
        if core.crossesOver(Short[1], Short[2], period) and (Short[1][period] > Level or not Use) then
            if Direction then
                BUY()
            else
                SELL()
        ...
        elseif core.crossesUnder(Short[1], Short[2], period) and (Short[1][period] < Level or not Use) then
            if Direction then
                SELL()
            else
                BUY()
        ...
        if core.crossesOver(Short[1], Short[3], period) and (Short[1][period] > Level or not Use) then
            if Direction then
                BUY()
            else
                SELL()
            ...
            ONE = Source:serial(period)
        elseif core.crossesUnder(Short[1], Short[3], period) and (Short[1][period] < Level or not Use) then
            if Direction then
                SELL()
            else
                BUY()
