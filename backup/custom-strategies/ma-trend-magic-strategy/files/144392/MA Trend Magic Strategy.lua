...
function Init() --The strategy profile initialization
    strategy:name("MA Trend Magic Strategy");
    ...
    strategy.parameters:addGroup("Trend Magic Calculation");
    strategy.parameters:addInteger("Period1", "CCI period", "", 50, 2, 2000);
    strategy.parameters:addInteger("Period2", "ATR Period", "", 5, 2, 2000);
...
function CreateTradingParameters()
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "MATMS");
...
function Prepare( nameOnly)
    ...
    TM= core.indicators:create("MAGIC TREND" , Source, instance.parameters.Period1, instance.parameters.Period2  ); 	
	first=math.max(MA.DATA:first(),TM.DATA:first()) ; 
...
--	period3= core.findDate (Source3, TickSource:date(period), false);
	...
	--/////////////////////////////////////////////////////////////////////
					...
					or  Source.close[period]< TM.DATA[period]
					then
...
function EntryFunction( id, now,period)
    ...
    if  Source.close[period]> MA.DATA[period]
	and  Source.close[period]> TM.DATA[period]
	then	
          if Direction then
                BUY();
            else
