...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=61828
...
function Init() --The strategy profile initialization
    strategy:name("On_Balance_Real_Volume MA Strategy");
...
function CreateTradingParameters()
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "OMS");
...
function Prepare( nameOnly)
    ...
    assert(core.indicators:findIndicator("ON_BALANCE_REAL_VOLUME") ~= nil, "Please, download and install ON_BALANCE_REAL_VOLUME.LUA indicator");    
    ...
    OBV = core.indicators:create("ON_BALANCE_REAL_VOLUME", Source    );
    assert(core.indicators:findIndicator(Method) ~= nil, Method .. " indicator must be installed");
	MA = core.indicators:create(Method, OBV.DATA , Period  ); 
...
function ExtUpdate(id, source, period)  -- The method called every time when a new bid or ask price appears.
   ...
   -- get only time
    ...
    if    core.crossesOver(OBV.DATA, MA.DATA, period) 
	then	
          if Direction then
                BUY();
            else
    ...
    elseif   core.crossesUnder(OBV.DATA, MA.DATA, period) 
	then
            if Direction then
                SELL();
            else
