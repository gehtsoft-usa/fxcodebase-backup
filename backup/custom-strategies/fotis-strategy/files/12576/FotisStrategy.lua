...
function Init()
    ...
    strategy.parameters:addStringAlternative("Type", "Ask", "", "Ask");
	 ...
	 strategy.parameters:addBoolean("EMA", "Use EMA", "" , true);
	 strategy.parameters:addBoolean("RSI", "Use RSI", "" , true);
	 strategy.parameters:addBoolean("TMACD", "Use TMACD", "" , true);
...
function Prepare(nameOnly)
    ...
    local i; 
	  ON["TMACD"] = instance.parameters.TMACD;
	  ON["PRICE"] =  instance.parameters.EMA;
	  ON["RSI"] =  instance.parameters.RSI;
...
function ExtUpdate(id, source, period)
	...
	if AllowTrade then
		...
		if not  ON["TMACD"]  then 
		FLAG["TMACD"]= "BUY";
		end 
		
		if not  ON["PRICE"]  then 
		FLAG["PRICE"]= "BUY";
		end 
		

		  if ON["PRICE"] or ON["TMACD"] or  ON["RSI"] then

					if  FLAG["Postition"]~= "BUY"
					and  FLAG["RSI"]== "BUY" 
					and FLAG["TMACD"]== "BUY"  
					and FLAG["PRICE"]== "BUY"
					and gSource.close[period] > gSource.open[period] then 
...
function Trading_Parameters()
    ...
    strategy.parameters:addStringAlternative("ALLOWEDSIDE", "Sell", "", "Sell");	
	 ...
	 strategy.parameters:addBoolean("RESET", "Resume Trading After Exit", "If the conditions are still met", false);
