...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=66699
...
function Init()
    ...
    strategy:description("[v2] Strategy changes stop following to the fractals");
...
function Prepare(onlyName)
    ...
    if instance.parameters.ExecutionType == "Live" then
        ExtSubscribe(1, nil, "t1", instance.parameters.Type == "Bid");
...
function ExtUpdate(id, source, period)
    ...
    if source:barSize() == "t1" then
        period = tsource:size() - 1;
        source = tsource;
    end
...
function MoveStop(trade, period)
    ...
    if trade.BS == "B" then
        ...
        if not IsFlSell(period) and trade.StopOrderID ~= "" then
            return;
        end
    ...
    if instance.parameters.AllowTrade then
        commands[trade.TradeID] = trading:MoveStop(trade, stopValue);
    end
end

function ExtAsyncOperationFinished(id, success, message)
    for _, module in pairs(Modules) do if module.AsyncOperationFinished ~= nil then module:AsyncOperationFinished(id, success, message, message1, message2); end end
end
...
function trading:AsyncOperationFinished(cookie, success, message, message1, message2)
    ...
    if res ~= nil then
        ...
        if self._signaler ~= nil then
            self._signaler:Signal(error_message);
        else
