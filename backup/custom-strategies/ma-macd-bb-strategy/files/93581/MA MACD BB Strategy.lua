...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=60556
...
function Init() --The strategy profile initialization
    strategy:name("MA MACD BB Strategy")
    ...
    strategy.parameters:addInteger("Period", "MA Periods", "", 200, 2, 1000)
...
function Prepare(nameOnly)
    ...
    first = math.max(MA.DATA:first(), MACD.DATA:first(), BB.DATA:first()) + 1
end
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    local priceAbove = currentPrice > currentMA
    local priceBelow = currentPrice < currentMA

    if (priceAbove and core.crossesOver(MACD.DATA, BB.TL, period)) then
        BUY()
    elseif (priceBelow and core.crossesUnder(MACD.DATA, BB.BL, period)) then
        SELL()
    end
end
