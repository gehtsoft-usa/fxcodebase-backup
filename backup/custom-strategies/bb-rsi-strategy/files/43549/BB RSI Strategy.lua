...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=25331
...
function Init() --The strategy profile initialization
    ...
    strategy.parameters:addInteger("BP", "Period", " ", 17)
    strategy.parameters:addDouble("SD", "Standard Deviation", " ", 2)
    ...
    strategy.parameters:addDouble("BL", "Buy Level", " ", 30)
    strategy.parameters:addDouble("SL", "Sell Level", " ", 70)
...
function Prepare(nameOnly)
    ...
    Indicator[1] = core.indicators:create("BB", Source[Price1], BP, SD)
    Short["T"] = Indicator[1]:getStream(0)
    Short["B"] = Indicator[1]:getStream(1)

    Indicator[2] = core.indicators:create("RSI", Source[Price2], RP)
    Short["RSI"] = Indicator[2]:getStream(0)

    first = math.max(Short["T"]:first(), Short["RSI"]:first())
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if
        Source.high[period] > Short["B"][period] and Source.low[period] < Short["B"][period] and
            Short["RSI"][period] < BL
     then
        if Direction then
            BUY()
    ...
    elseif
        Source.high[period] > Short["T"][period] and Source.low[period] < Short["T"][period] and
            Short["RSI"][period] > SL
     then
        if Direction then
            SELL()
