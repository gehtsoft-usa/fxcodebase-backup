...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=61408
...
function Init() --The strategy profile initialization
    strategy:name("Extreme_TMA_Line Breakout Strategy")
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if core.crossesOver(Source.close, Indicator[1].Upper, period) then
        if Direction then
            BUY()
        else
            SELL()
    ...
    elseif core.crossesUnder(Source.close, Indicator[1].Lower, period) then
        if Direction then
            SELL()
        else
            BUY()
