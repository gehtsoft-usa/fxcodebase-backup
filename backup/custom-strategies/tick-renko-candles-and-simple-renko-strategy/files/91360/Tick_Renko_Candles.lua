...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=60066
--+------------------------------------------------------------------+
...
function Init()
    indicator:name("Tick Renko candles");
    indicator:description("Tick Renko candles");
	  
		
    indicator:requiredSource(core.Tick);
    indicator:type(core.View);

    indicator.parameters:addGroup("Calculations");
    indicator.parameters:addString("instrument", "Instrument name", "Instrument name", "EUR/USD");
    ...
    indicator.parameters:addInteger("Step", "Brick size", "The brick size in pips.", 10);

    indicator.parameters:addGroup("Range");
    indicator.parameters:addDate("from", "Date from", "Date from", -1000);
    indicator.parameters:addDate("to", "Date to", "Date to", 0);
...
function Prepare(onlyName)
    ...
    assert(row ~= nil, "Error: instrument not found");
    ...
    open = instance:addStream("open", core.Line, name .. "." .. "Open price", "open", 0, 0, 0);
    high = instance:addStream("high", core.Line, name .. "." .. "High price", "high", 0, 0, 0);
    low = instance:addStream("low", core.Line, name .. "." .. "Low price", "low", 0, 0, 0);
    close = instance:addStream("close", core.Line, name .. "." .. "Close price", "close", 0, 0, 0);
