...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=15659
...
function Init() --The strategy profile initialization
    strategy:name("NonLagDot SAR Strategy");
	...
	strategy.parameters:addGroup("NLD Calculation");	
	...
	strategy.parameters:addString("Trade", "Signal is generated by", "", "NLD");
    strategy.parameters:addStringAlternative("Trade", "NLS", "", "NLD");
    strategy.parameters:addStringAlternative("Trade", "SAR", "", "SAR");
	strategy.parameters:addStringAlternative("Trade", "Mutual Confirmation", "", "BOTH");
...
local Direction;
local Length, Filter ,ColorBarBack,Deviation;
local first;
local Price;
local Exit;
local Trade;
...
function Prepare(nameOnly)
	...
	Indicator[2] = core.indicators:create("NLD",  Source[Price] ,Length,Filter,ColorBarBack, Deviation, core.rgb(0, 255, 0), core.rgb(255, 0, 0));
...
function ExtUpdate(id, source, period)  -- The method called every time when a new bid or ask price appears.
   ...
   -- get only time
	...
	if Trade == "NLD" then
				if   Indicator[2].DATA:colorI(period) ==  core.rgb(0, 255, 0) 
				and   Indicator[2].DATA:colorI(period-1) ~=  core.rgb(0, 255, 0) 
				...
				elseif  Indicator[2].DATA:colorI(period) ==  core.rgb(255, 0, 0) 
				and   Indicator[2].DATA:colorI(period-1) ~=  core.rgb(255, 0, 0) 
     	...
     	elseif  Trade == "SAR" then	 
	           ...
	           if   Indicator[2].DATA:colorI(period) ==  core.rgb(0, 255, 0) 				
				and Short[2]:hasData(period) 
				and  (  Indicator[2].DATA:colorI(period-1) ~=  core.rgb(0, 255, 0)  or  not Short[2]:hasData(period-1))
				then
				...
				elseif  Indicator[2].DATA:colorI(period) ==  core.rgb(255, 0, 0) 				
				and Short[1]:hasData(period)
				 and  (not Short[1]:hasData(period-1) 	or    Indicator[2].DATA:colorI(period-1) ~=  core.rgb(255, 0, 0) )		
				 then
					if Direction then
						SELL();
					else
		...
		if Exit then
					...
					then 			
					  ...
					  if Indicator[2].DATA:colorI(period) ~=  core.rgb(255, 0, 0) 
					  or not Short[1]:hasData(period)
		               then
								   if   Direction then    
										if haveTrades("S") then
										   exit("S");
