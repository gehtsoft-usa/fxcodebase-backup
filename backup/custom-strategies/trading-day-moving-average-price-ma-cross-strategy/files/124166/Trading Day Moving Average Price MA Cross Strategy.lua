...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=67395
...
function Init() --The strategy profile initialization
    strategy:name("Trading Day Moving Average Price MA Cross Strategy");
	...
	strategy.parameters:addGroup("TDMA Parameters");
...
function CreateTradingParameters()
   ...
   -- strategy.parameters:addStringAlternative("ExitExecutionType", "End of Turn", "", "EndOfTurn");
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "TDMAPMACS");
...
local Method,MA_Price, Period ,MA; 
function Prepare( nameOnly)
	...
	assert(core.indicators:findIndicator("TRADING DAY MOVING AVERAGE") ~= nil, "Please, download and install TRADING DAY MOVING AVERAGE.LUA indicator");          
    ...
    Indicator = core.indicators:create("TRADING DAY MOVING AVERAGE", Source, Price  ); 
    assert(core.indicators:findIndicator(Method) ~= nil, Method .. " indicator must be installed");
	MA = core.indicators:create(Method, Source[MA_Price], Period  ); 
...
function EntryFunction( now,period)
    ...
    if   MA.DATA[period]> Indicator.DATA[period]
	and  MA.DATA[period-1]<= Indicator.DATA[period-1]
	then	
          if Direction then
                BUY();
            else
		...
		Return=true;
    elseif   MA.DATA[period]< Indicator.DATA[period]
	and  MA.DATA[period-1]>= Indicator.DATA[period-1]
	then
            if Direction then
                SELL();
