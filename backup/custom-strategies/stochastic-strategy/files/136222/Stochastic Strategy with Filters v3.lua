...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=2533
...
function Init() --The strategy profile initialization
    strategy:name("Stochastic Strategy With Filters")
    ...
    strategy.parameters:addGroup("MA Filter Selector")
    strategy.parameters:addBoolean("MA", "USE MA Filter", "", false)
    ...
    strategy.parameters:addInteger("MAPeriod", "MAPeriod", "MAPeriod", 50, 2, 2000)

    strategy.parameters:addGroup("ADX Filter Selector")
    strategy.parameters:addBoolean("ADX", "USE ADX Filter", "", false)

    strategy.parameters:addInteger("ADXPeriod", "ADX Period", "", 14, 2, 2000)
    strategy.parameters:addDouble("ADXLevel", "ADX Level", "", 25, 0, 100)

    strategy.parameters:addGroup("Extreme TMA Filter");
    strategy.parameters:addBoolean("use_extreme_tma", "Use Extreme TMA", "", true)
...
local OS, OB
local PARAMETARS
local ma, adx, MAPeriod, MAMethod
local ADXLevel, ADXPariod
...
function Prepare(nameOnly)
    ...
    if instance.parameters.use_extreme_tma then
        local profile = core.indicators:findIndicator("EXTREME_TMA_LINE");
        assert(profile ~= nil, "Please, download and install " .. "EXTREME_TMA_LINE" .. ".LUA indicator");
        local indicatorParams = profile:parameters();
        indicatorParams:setInteger("TMA_Period", instance.parameters.TMA_Period);
        indicatorParams:setInteger("ATR_Period", instance.parameters.ATR_Period);
        indicatorParams:setDouble("ATR_Mult", instance.parameters.ATR_Mult);
        indicatorParams:setDouble("TrendThreshold", instance.parameters.TrendThreshold);
        etma = core.indicators:create("EXTREME_TMA_LINE", Source, indicatorParams);
    end
    ...
    first = math.max(Line2:first(), ma.DATA:first(), adx.DATA:first()) + 1
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if period < first or (ADX and adx.DATA[period] < ADXLevel) then
        return
    end

    if instance.parameters.L == "K" then
        if core.crossesOver(Line1, OS, period) and (not MA or (MA and Source.close[period] > ma.DATA[period])) and etma.DATA:colorI(period) == core.rgb(0, 255, 0) then
            BUY()
            ONE = Source:serial(period)
        elseif core.crossesUnder(Line1, OB, period) and (not MA or (MA and Source.close[period] < ma.DATA[period])) and etma.DATA:colorI(period) == core.rgb(255, 0, 0) then
            SELL()
            ONE = Source:serial(period)
        end
    end
    ...
    if instance.parameters.L == "D" then
        if core.crossesOver(Line2, OS, period) and (not MA or (MA and Source.close[period] > ma.DATA[period])) and etma.DATA:colorI(period) == core.rgb(0, 255, 0) then
            BUY()
            ONE = Source:serial(period)
        elseif core.crossesUnder(Line2, OB, period) and (not MA or (MA and Source.close[period] < ma.DATA[period])) and etma.DATA:colorI(period) == core.rgb(255, 0, 0) then
            SELL()
            ONE = Source:serial(period)
        end
    end
    ...
    if (instance.parameters.L == "C") then
        if core.crossesOver(Line1, Line2, period) and (not MA or (MA and Source.close[period] > ma.DATA[period])) and etma.DATA:colorI(period) == core.rgb(0, 255, 0) then
            BUY()
            ONE = Source:serial(period)
        elseif core.crossesUnder(Line1, Line2, period) and (not MA or (MA and Source.close[period] < ma.DATA[period])) and etma.DATA:colorI(period) == core.rgb(255, 0, 0) then
            SELL()
            ONE = Source:serial(period)
        end
    end
    ...
    if (instance.parameters.L == "B") then
        if
            core.crossesOver(Line1, Line2, period) and Line1[period - 1] < OS and Line2[period] < OS and
                (not MA or (MA and Source.close[period] > ma.DATA[period]))
                and etma.DATA:colorI(period) == core.rgb(0, 255, 0)
         then
            BUY()
            ONE = Source:serial(period)
        elseif
            core.crossesUnder(Line1, Line2, period) and Line1[period - 1] > OB and Line2[period] > OB and
                (not MA or (MA and Source.close[period] < ma.DATA[period]))
                and etma.DATA:colorI(period) == core.rgb(255, 0, 0)
         then
            SELL()
            ONE = Source:serial(period)
        end
