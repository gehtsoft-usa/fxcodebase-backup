...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=60190
...
function Init() --The strategy profile initialization
    ...
    strategy.parameters:addString("Execution", "Execution Type", "", "EOT")
    strategy.parameters:addStringAlternative("Execution", "End Of Turn", "", "EOT")
    strategy.parameters:addStringAlternative("Execution", "Live", "", "Live")

    strategy.parameters:addString("TF", "Time frame", "", "m5")
    strategy.parameters:setFlag("TF", core.FLAG_PERIODS)

    strategy.parameters:addGroup("ARSI Parameters")
    ...
    strategy.parameters:addInteger("Fast", "Fast Period ARSI", "", 14)
    ...
    strategy.parameters:addInteger("Slow", "Slow Period ARSI", "", 28)
...
function Prepare(nameOnly)
    Execution = instance.parameters.Execution
    ...
    Indicator[1] = core.indicators:create("ARSI", Source[Price1], Fast)
    Short["Fast"] = Indicator[1]:getStream(0)

    Indicator[2] = core.indicators:create("ARSI", Source[Price2], Slow)
    Short["Slow"] = Indicator[2]:getStream(0)

    first = math.max(Short["Fast"]:first(), Short["Slow"]:first())
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if Execution == "EOT" and id == 2 then
        ...
        if core.crossesOver(Short["Fast"], Short["Slow"], period) then
            if Direction then
                BUY()
            else
                SELL()
        ...
        if core.crossesUnder(Short["Fast"], Short["Slow"], period) then
            if Direction then
                SELL()
            else
                BUY()
    ...
    elseif Execution == "Live" then
        ...
        elseif core.crossesUnder(Short["Fast"], Short["Slow"], period) then
            ONE = Source:serial(period)
            if Direction then
                SELL()
            else
