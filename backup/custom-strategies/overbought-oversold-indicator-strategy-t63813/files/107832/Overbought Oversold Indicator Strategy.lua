...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=63813
...
function CreateTradingParameters()
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "OOIS3");
...
function Prepare( nameOnly)
    ...
    Indicator = core.indicators:create("OBOS", Source , Period,   core.rgb(0, 255, 0),core.rgb(255,0 , 0), core.rgb(0, 0, 255));
...
function ExtUpdate(id, source, period)  -- The method called every time when a new bid or ask price appears.
   ...
   Indicator:update(core.UpdateLast);
	...
	min = math.min(Indicator:getStream(0)[period], Indicator:getStream(1)[period], Indicator:getStream(2)[period], Indicator:getStream(3)[period]);
	max = math.max(Indicator:getStream(0)[period], Indicator:getStream(1)[period], Indicator:getStream(2)[period], Indicator:getStream(3)[period]);
   
    if  Indicator.DATA:colorI(period)== core.rgb(0, 255, 0) 
	and Indicator.DATA:colorI(period-1)~= core.rgb(0, 255, 0) 
	and min  < OS and max < OS
	then
        BUY();
	...
	if  Indicator.DATA:colorI(period)== core.rgb(255, 0, 0) 
	and Indicator.DATA:colorI(period-1)~= core.rgb(255, 0, 0)
	and min  > OB and max > OB
	then
         SELL();
		ONE= Source:serial(period);
	end
	...
	if  Exit then 
		if  Indicator.DATA:colorI(period)== core.rgb(0, 0, 255) 
