...
--http://fxcodebase.com/code/viewtopic.php?f=31&t=3482
...
function Init() --The strategy profile initialization
    ...
    strategy.parameters:addGroup("RLW indicator parameters");
    strategy.parameters:addInteger("Period", "Period", "Period", 14, 1, 1000);
  
    strategy.parameters:addDouble("BuyLevel", "Level BUY", "Level BUY", -80);
    strategy.parameters:addDouble("SellLevel", "Level SELL", "Level SELL", -20);
	
	 strategy.parameters:addInteger("EntryMethod", "Entry Method", "Entry Method" , 2);
    strategy.parameters:addIntegerAlternative("EntryMethod", "In the zone cross", "In the zone cross" , 1);	
    strategy.parameters:addIntegerAlternative("EntryMethod", "Out of the zone cross", "Out of the zone cross" , 2);
	
	
	 strategy.parameters:addInteger("ExitMethod", "Exit Method", "Exit Method" , 1);
	strategy.parameters:addIntegerAlternative("ExitMethod", "In the zone cross", "In the zone cross" , 1);
    strategy.parameters:addIntegerAlternative("ExitMethod", "Out of the zone cross", "Out of the zone cross" , 2);
    strategy.parameters:addIntegerAlternative("ExitMethod", "Price / MA Cross", "Price / MA Cross" , 3);
	strategy.parameters:addIntegerAlternative("ExitMethod", "Any of the above", "Any of the above" , 4);
	strategy.parameters:addIntegerAlternative("ExitMethod", "Don't use optinal Exit", "Don't use optinal Exit" , 5);
	
	
	strategy.parameters:addGroup("MA Calculation");
	
	strategy.parameters:addBoolean("USE1", "Use 1. MA", "", true);
	...
	strategy.parameters:addInteger("P1", "1. MA Period", "", 14);
	
	strategy.parameters:addBoolean("USE2", "Use 2. MA", "", true);
	...
	strategy.parameters:addInteger("P2", "2. MA Period", "", 28);
	
	
	strategy.parameters:addBoolean("USE3", "Use 3. MA", "", true);
	...
	strategy.parameters:addInteger("P3", "3. MA Period", "", 42);
...
function CreateTradingParameters()
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "RLWS");
...
local first;
local Exit;
local Period, BuyLevel,SellLevel,EntryMethod, ExitMethod;

local Method1, Method2, Method3;
local USE1, USE2, USE3;
...
function Prepare( nameOnly)
    ...
    Indicator = core.indicators:create("RLW", Source, Period);	
	
    assert(core.indicators:findIndicator(Method1) ~= nil, Method1 .. " indicator must be installed");
	MA1 = core.indicators:create(Method1, Source[Price1], P1 );
    assert(core.indicators:findIndicator(Method2) ~= nil, Method2 .. " indicator must be installed");
	MA2 = core.indicators:create(Method2, Source[Price2], P2 ); 	
    assert(core.indicators:findIndicator(Method3) ~= nil, Method3 .. " indicator must be installed");
	MA3 = core.indicators:create(Method3, Source[Price3], P3 );	
	
	first=math.max(Indicator.DATA:first(), MA1.DATA:first(),  MA2.DATA:first(),  MA3.DATA:first());
...
function ExtUpdate(id, source, period)  -- The method called every time when a new bid or ask price appears.
   ...
   -- get only time
	...
	local BuySignal= false;
	local SellSignal= false;
	
	if EntryMethod == 1 then
		
	 
		if Indicator.DATA[period]< BuyLevel 
		and Indicator.DATA[period-1]>= BuyLevel 
		then
		BuySignal = true; 
		end
		if Indicator.DATA[period]>  SellLevel
		and Indicator.DATA[period-1]<=  SellLevel
		then
		SellSignal =true; 
		end
    ...
    if  BuySignal == true
	and((Source.close[period] > MA1.DATA[period]) or not USE1)
	and((Source.close[period] > MA2.DATA[period]) or not USE2)
	and((Source.close[period] > MA3.DATA[period]) or not USE3)
	then	
          if Direction then
                BUY();
            else
	...
	if ExitMethod == 1 or ExitMethod == 4 then
	    if Indicator.DATA[period]> SellLevel 
		and Indicator.DATA[period-1]<= SellLevel 
		then
		ExitBuySignal = true; 
	...
	if ExitMethod == 2 or ExitMethod == 4 then
	...
	if   ExitMethod == 3 or ExitMethod == 4 then
           if	((Source.close[period] < MA1.DATA[period]) and USE1)
			or((Source.close[period] < MA2.DATA[period]) and USE2)
			or((Source.close[period] < MA3.DATA[period]) and USE3)
	        then
			ExitBuySignal= true;
	         elseif  ((Source.close[period] > MA1.DATA[period]) and USE1)
			or((Source.close[period] > MA2.DATA[period]) and USE2)
			or((Source.close[period] > MA3.DATA[period]) and USE3)
	        then
		    ExitSellSignal= true;   
