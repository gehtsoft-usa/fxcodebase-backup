...
function Init() --The strategy profile initialization
    ...
    strategy:description("RLW strategy")
    strategy:setTag("NonOptimizableParameters", "Email,SendEmail,SoundFile,RecurrentSound,PlaySound, ShowAlert")

    strategy.parameters:addGroup("RLW indicator parameters")
    strategy.parameters:addInteger("N", "Period", "Period", 14, 1, 1000)

    strategy.parameters:addGroup("Strategy Parameters")
    strategy.parameters:addDouble("LevelB", "Level BUY", "Level BUY", -80)
    strategy.parameters:addDouble("LevelS", "Level SELL", "Level SELL", -20)
    ...
    strategy.parameters:addDouble("Limit", "Limit Order in pips", "", 0.5, 0.1, 10000)
    strategy.parameters:addBoolean("SetStop", "Set Stop Orders", "", false)
    strategy.parameters:addDouble("Stop", "Stop Order in pips", "", 0.5, 0.1, 10000)
...
function Prepare()
    ...
    name =
        profile:id() ..
        "(" .. instance.bid:name() .. "." .. instance.parameters.TF .. "," .. "RLW(" .. instance.parameters.N .. "))"
    instance:name(name)
    ...
    Source = ExtSubscribe(2, nil, instance.parameters.TF, true, "close")

    assert(core.indicators:findIndicator("RLW TICK") ~= nil, "RLW TICK" .. " indicator must be installed")
    RLW = core.indicators:create("RLW TICK", Source, instance.parameters.N)
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if (RLW.DATA:first() > (period - 1)) then
        return
    end
    ...
    if RLW.DATA[period - 1] < instance.parameters.LevelB and RLW.DATA[period] > instance.parameters.LevelB then
    ...
    if RLW.DATA[period - 1] > instance.parameters.LevelS and RLW.DATA[period] < instance.parameters.LevelS then
        if instance.parameters.TypeSignal == "direct" then
            MustS = true
        else
            MustB = true
