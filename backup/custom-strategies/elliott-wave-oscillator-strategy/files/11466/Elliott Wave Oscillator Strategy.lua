...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=4645
...
function Init() --The strategy profile initialization
    strategy:name("Elliott Wave Oscillator Strategy")
    ...
    strategy.parameters:addInteger("Fast", "Fast Moving Average", "", 5, 2, 1000)
    strategy.parameters:addInteger("Slow", "Slow Moving Average", "", 35, 2, 1000)

    strategy.parameters:addString("Source", "Price Source", "", "M2")
    strategy.parameters:addStringAlternative("Source", "Typical", "", "M2")
    strategy.parameters:addStringAlternative("Source", "Median", "", "M3")
    strategy.parameters:addStringAlternative("Source", "Close", "", "C")

    strategy.parameters:addString("Method", "Smoothing method", "", "MVA")
    ...
    strategy.parameters:addBoolean("CO", "Show CrossOver", "", true)
    strategy.parameters:addBoolean("CU", "Show CrossUnder", "", true)
    strategy.parameters:addBoolean("TOP", "Show Top", "", true)
    strategy.parameters:addBoolean("BOTTOM", "Show Bottom", "", true)
...
function Prepare(nameOnly)
    ...
    Parameters["Source"] = instance.parameters.Source
    Parameters["Method"] = instance.parameters.Method
    Parameters["Fast"] = instance.parameters.Fast
    Parameters["Slow"] = instance.parameters.Slow

    Parameters["CO"] = instance.parameters.CO
    Parameters["CU"] = instance.parameters.CU
    Parameters["TOP"] = instance.parameters.TOP
    Parameters["BOTTOM"] = instance.parameters.BOTTOM
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if period < first or not Indicator.DATA:hasData(period - 2) then
        return
    end

    if
        Indicator.DATA[period - 1] < Indicator.DATA[period] and Indicator.DATA[period - 2] >= Indicator.DATA[period - 1] and
            Parameters["BOTTOM"] and
            Indicator.DATA[period - 1] < 0
     then
    ...
    if core.crossesOver(Indicator.DATA, 0, period) and Parameters["CO"] then
        ONE = Source:serial(period)
        ...
        elseif ShowAlert then
            Signal("Zero Line Cross Over")
        end
    elseif core.crossesUnder(Indicator.DATA, 0, period) and Parameters["CU"] then
        ONE = Source:serial(period)
