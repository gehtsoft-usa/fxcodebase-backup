...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=5119
...
function Init() --The strategy profile initialization
    strategy:name("Adaptive Relative Strength Index, Dynamic Trend Strategy")
    ...
    strategy.parameters:addGroup("Dynamic Trend Indicator Parameters  ")
    strategy.parameters:addInteger("Percent", "Percent", "Percent", 1)
    strategy.parameters:addInteger("MaxPeriod", "MaxPeriod", "MaxPeriod", 14)
...
function Prepare(nameOnly)
    ...
    assert(
        core.indicators:findIndicator("DYNAMICTREND") ~= nil,
        "Please, download and install DYNAMICTREND.LUA indicator"
    )

    Indicator["ARSI"] = core.indicators:create("ARSI", Source, Parameters["N"])
    Indicator["DYNAMICTREND"] =
        core.indicators:create("DYNAMICTREND", Source, Parameters["Percent"], Parameters["MaxPeriod"], true)

    first = math.max(Indicator["ARSI"].DATA:first(), Indicator["DYNAMICTREND"].DATA:first()) + 1
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    Indicator["DYNAMICTREND"]:update(core.UpdateLast)

    if period < first then
        return
    end

    if core.crossesOver(Indicator["ARSI"].DATA, Indicator["DYNAMICTREND"].DATA, period) then
        if Direction == "direct" then
            BUY()
        else
            SELL()
        end
    elseif core.crossesUnder(Indicator["ARSI"].DATA, Indicator["DYNAMICTREND"].DATA, period) then
        if Direction == "direct" then
            SELL()
        else
            BUY()
