...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=63562
...
function Init() --The strategy profile initialization
    strategy:name("Bollinger Strategy")
    ...
    strategy.parameters:addDouble("Deviations", "Number of standard deviations", "", 2)
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if Source.close[period] > BB.TL[period] and Source.close[period - 1] <= BB.TL[period - 1] then
        if Direction then
            BUY()
        else
            SELL()
    ...
    elseif Source.close[period] < BB.BL[period] and Source.close[period - 1] >= BB.BL[period - 1] then
        if Direction then
            SELL()
        else
            BUY()
