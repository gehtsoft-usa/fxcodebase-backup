...
--http://fxcodebase.com/code/viewtopic.php?f=31&t=70467
...
local STRATEGY_NAME = "MA Trend Strategy";
...
function CreateAverages(period, method, source)
    ...
    then
        ...
        return core.indicators:create(method, source[instance.parameters.Price], period);
    end
    assert(core.indicators:findIndicator("AVERAGES") ~= nil, "Please, download and install AVERAGES indicator");
    return core.indicators:create("AVERAGES", source[instance.parameters.Price], method, period);
end
...
function IsEntryLong(source, period)
    if not ma.DATA:hasData(period - 2) then
        return false;
    end
    return ma.DATA[period] > ma.DATA[period - 1] and ma.DATA[period - 1] <= ma.DATA[period - 2];
end
