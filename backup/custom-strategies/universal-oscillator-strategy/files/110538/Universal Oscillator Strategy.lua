...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=64303&p
...
function Init() --The strategy profile initialization
    strategy:name("Universal Oscillator Strategy")
    ...
    strategy.parameters:addInteger("BandEdge", "BandEdge", "BandEdge", 20, 2, 100)
...
function Prepare(nameOnly)
    ...
    assert(
        core.indicators:findIndicator("UNIVERSAL OSCILLATOR") ~= nil,
        "Please, download and install UNIVERSAL OSCILLATOR.LUA indicator"
    )
    ...
    Indicator = core.indicators:create("UNIVERSAL OSCILLATOR", Source, BandEdge)
...
function EntryFunction(period, now)
    ...
    if Indicator.DATA[period] > 0 and Indicator.DATA[period - 1] <= 0 then
        if Direction then
            BUY()
        else
            SELL()
        ...
        Return = true
    elseif Indicator.DATA[period] < 0 and Indicator.DATA[period - 1] >= 0 then
        if Direction then
            SELL()
        else
            BUY()
