...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=63401
...
function Init() --The strategy profile initialization
    strategy:name("Fx Snipers Ergodic CCI Trigger Strategy")
...
function Prepare(nameOnly)
    ...
    assert(
        core.indicators:findIndicator("FX_SNIPERS_ERGODIC_CCI_TRIGGER") ~= nil,
        "Please, download and install FX_SNIPERS_ERGODIC_CCI_TRIGGER.LUA indicator"
    )
    ...
    Indicator =
        ...
        Source,
        instance.parameters.pq,
        instance.parameters.pr,
        instance.parameters.ps,
        instance.parameters.trigger,
        instance.parameters.MA_Method
    )

    first = Indicator.buff2:first()
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if Indicator.buff1[period] > Indicator.buff2[period] and Indicator.buff1[period - 1] <= Indicator.buff2[period - 1] then
        if Direction then
            BUY()
        else
            SELL()
    ...
    elseif
        Indicator.buff1[period] < Indicator.buff2[period] and Indicator.buff1[period - 1] >= Indicator.buff2[period - 1]
     then
        if Direction then
            SELL()
        else
