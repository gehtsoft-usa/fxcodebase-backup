...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=10555
...
function Init() --The strategy profile initialization
    strategy:name("FP Channel strategy")
    strategy:description("FP Channel strategy")
    strategy:setTag("Version", "1.1")
    strategy:setTag("NonOptimizableParameters", "Email,SendEmail,SoundFile,RecurrentSound,PlaySound, ShowAlert")

    strategy.parameters:addGroup("Parameters")
    strategy.parameters:addInteger("Period", "Period", "", 100, 3, 1000)
...
function Prepare(nameOnly)
    ...
    assert(core.indicators:findIndicator("FP_CHANNEL") ~= nil, "Please, download and install FP_CHANNEL.LUA indicator")

    Source = ExtSubscribe(2, nil, instance.parameters.TF, true, "bar")
    FP = core.indicators:create("FP_CHANNEL", Source, instance.parameters.Period)
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    FP:update(core.UpdateLast)

    -- Check that we have enough data
    if (FP.DATA:first() > (period - 1)) then
        return
    end
    ...
    if source.close[period] <= FP.BottomBuff[period] then
    ...
    if source.close[period] >= FP.TopBuff[period] and source.close[period] / 2 < FP.TopBuff[period] then
        if instance.parameters.TypeSignal == "direct" then
            MustOpenS = true
        else
            MustOpenB = true
