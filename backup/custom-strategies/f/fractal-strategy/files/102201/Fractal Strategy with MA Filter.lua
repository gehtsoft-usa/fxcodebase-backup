...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=62633
...
function Init() --The strategy profile initialization
    strategy:name("Fractal Strategy with MA Filter")
    ...
    strategy.parameters:addInteger("Period", "MA Period", "Period", 34)
...
function Prepare(nameOnly)
    ...
    first = math.max((Source.close:first() + 6), MA.DATA:first())
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if
        (curr > Source.high[period - 4] and curr > Source.high[period - 3] and curr > Source.high[period - 1] and
            curr > Source.high[period]) and
            Source.close[period - 2] < MA.DATA[period - 2]
     then
        if Direction then
            SELL()
        else
