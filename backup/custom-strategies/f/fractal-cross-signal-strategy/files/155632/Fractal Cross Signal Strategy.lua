...
-- https://fxcodebase.com/code/viewtopic.php?f=31&t=71423
...
function Init() --The strategy profile initialization
    strategy:name("Fractal Cross Signal Strategy");
...
function CreateTradingParameters()
	...
	--*********************************************************************************************************
	 ...
	 strategy.parameters:addGroup("ATR Filter Parameters");
     strategy.parameters:addBoolean("Filter", "Use Entry Filter", "", true);	
     strategy.parameters:addInteger("Period", "ATR Period", "", 14, 1, 200);
     strategy.parameters:addDouble("Level", "ATR Level", "", 20, 1, 200);	 
	 
	 strategy.parameters:addString("AllowTrading", "Allow Trading if ATR is", "", "Under");
     strategy.parameters:addStringAlternative("AllowTrading", "Under", "", "Under");
     strategy.parameters:addStringAlternative("AllowTrading", "Over", "", "Over");	
	 
	 strategy.parameters:addGroup("Trade Parameters");
	
	strategy.parameters:addBoolean("CloseOnOpposite", "Close On Opposite", "", true);
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "FCSS");
...
function Prepare( nameOnly)
	...
	assert(core.indicators:findIndicator("FRACTAL CROSS SIGNAL") ~= nil, "Please, download and install FRACTAL CROSS SIGNAL.LUA indicator");          
    ...
    Indicator = core.indicators:create("FRACTAL CROSS SIGNAL", Source  ); 
    ADX = core.indicators:create("ADX", Source, Period  ); 	
	first=math.max(Indicator.DATA:first(),ADX.DATA:first()); 
...
function ExitFunction(id, now,period)
	...
	--/////////////////////////////////////////////////////////////////////
		            ...
		            if     Indicator.down[period]==1  then
...
function EntryFunction( id, now,period)
    ...
    local Return=false;
	 ...
	 if Filter and AllowTrading == "Under" and ADX.DATA[period] > Level then
	 return
	 end
	 
	 if Filter and AllowTrading == "Over" and ADX.DATA[period] < Level then
	 return
	 end     
	  
    -- only buy if we have a fast cross over slow and the price is above the moving averages.
    if   Indicator.up[period]==1  
	then	
          if Direction then
                BUY();
            else
		...
		Return=true;
    elseif Indicator.down[period]==1 
		then
            if Direction then
                SELL();
            else
