...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=10450
--+------------------------------------------------------------------+
...
function Prepare()
    ...
    assert(
        core.indicators:findIndicator("FIBO_TRIGGER") ~= nil,
        "Please, download and install FIBO_TRIGGER.LUA indicator"
    )
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if FT.DATA[period] == source.low[period] and CVT then
    ...
    if FT.DATA[period] == source.high[period] and CVT then
        if instance.parameters.TypeSignal == "direct" then
            MustOpenS = true
        else
            MustOpenB = true
