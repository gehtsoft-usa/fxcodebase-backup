...
--http://fxcodebase.com/code/viewtopic.php?f=31&t=70597
...
local STRATEGY_NAME = "Future Renko + DNC v3";
...
function CreateParameters()
    strategy.parameters:addInteger("Step", "Step", "", 100, 1, 1000);
...
function CreateEntryIndicators(source)
    local profile = core.indicators:findIndicator("FUTURE RENKO");
    assert(profile ~= nil, "Please, download and install " .. "FUTURE RENKO" .. ".LUA indicator");
    local indicatorParams = profile:parameters();
    indicatorParams:setString("instrument", source:instrument());
    indicatorParams:setString("frame", source:barSize());
    indicatorParams:setBoolean("type", source:isBid());
    indicatorParams:setInteger("Step", instance.parameters.Step);
    indicatorParams:setDate("from", core.now() - 5000);
    indicatorParams:setDate("to", 0);
    renko = core.indicators:create("FUTURE RENKO", source, indicatorParams)
    ...
    dnc = core.indicators:create("DNC_V3", renko:getCandleOutput(0), indicatorParams);
end
...
function CreateCustomActions()
    if not DISABLE_EXIT then
        ...
        exitLongAction.IsPass = function (source, period, periodFromLast, data)
            if dnc.DATA:size() < 2 then
                return;
            end
            return renko.close[NOW - 1] < dnc.high[NOW] and renko.close[NOW] < renko.open[NOW];
        end
        ...
        exitShortAction.IsPass = function (source, period, periodFromLast, data)
            ...
            return renko.close[NOW - 1] > dnc.low[NOW] and renko.close[NOW] > renko.open[NOW];
        end
    ...
    enterLongAction.IsPass = function (source, period, periodFromLast, data)
        ...
        return renko.close[NOW] >= dnc.DU[NOW] and renko.close[NOW] > renko.open[NOW];
    end
    AddLongCondition(enterLongAction);
    ...
    enterShortAction.IsPass = function (source, period, periodFromLast, data)
        ...
        return renko.close[NOW] <= dnc.DN[NOW] and renko.close[NOW] < renko.open[NOW];
    end
    AddShortCondition(enterShortAction);
end
