...
local STRATEGY_NAME = "Fred Tam F1 Indicator Strategy";
...
function CreateEntryIndicators(source)
    local profile = core.indicators:findIndicator("FRED TAM F1 INDICATOR WITH ALERT");
    assert(profile ~= nil, "Please, download and install " .. "FRED TAM F1 INDICATOR WITH ALERT" .. ".LUA indicator");
    local indicatorParams = profile:parameters();
    indicatorParams:setInteger("Buy", instance.parameters.Buy);
    indicatorParams:setInteger("Sell", instance.parameters.Sell);
    indicatorParams:setBoolean("strategy_output", true);
    indi = core.indicators:create("FRED TAM F1 INDICATOR WITH ALERT", source, indicatorParams)
end
