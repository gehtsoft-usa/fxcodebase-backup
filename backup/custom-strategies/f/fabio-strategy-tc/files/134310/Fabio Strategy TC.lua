...
--http://fxcodebase.com/code/viewtopic.php?f=31&t=69919
...
function CreateParameters() 
    strategy.parameters:addDouble("lots_multiplier", "Lots multiplier", "", 2);
    strategy.parameters:addInteger("rsi_period", "RSI Period", "", 14)
    strategy.parameters:addDouble("RsiMinimum", "RsiMinimum", "", 20);
    strategy.parameters:addDouble("RsiMaximum", "RsiMaximum", "", 80);
    strategy.parameters:addInteger("DEL", "Del", "", 3);

    strategy.parameters:addString("cci_tf", "CCI Timeframe", "", "H1");
    strategy.parameters:setFlag("cci_tf", core.FLAG_PERIODS);
    strategy.parameters:addInteger("cci_period", "CCI Period", "", 14);
    strategy.parameters:addDouble("drop", "Drop", "", 500);
end

local rsi, RsiMinimum, RsiMaximum, drop, DEL, cci, lots_multiplier;
function CreateEntryIndicators(source)
    rsi = core.indicators:create("RSI", source, instance.parameters.rsi_period);
    cci_source = ExtSubscribe(4, nil, instance.parameters.cci_tf, instance.parameters.type, "bar")
    cci = core.indicators:create("CCI", cci_source, instance.parameters.cci_period);
    drop = instance.parameters.drop;
    DEL = instance.parameters.DEL;
    lots_multiplier = instance.parameters.lots_multiplier;
    RsiMinimum = instance.parameters.RsiMinimum;
    RsiMaximum = instance.parameters.RsiMaximum;
end
...
function IsEntryLong(source, period)
    if not rsi.DATA:hasData(period) then
        return false;
    end
    return source.close[period] < source.close[period - 1]
        and rsi.DATA[period] > RsiMinimum;
end
function IsEntryShort(source, period)
    ...
    return source.close[period] > source.close[period - 1]
        and rsi.DATA[period] < RsiMaximum;
end
function IsExitLong(source, period)
    return cci.DATA[NOW] > drop;
end
...
function Init()
    ...
    strategy.parameters:addString("timeframe", "Timeframe", "", "m15");
...
function ExtUpdate(id, source, period)
    ...
    if IsEntryLong(main_source, entry_period) and last_entry ~= main_source:date(NOW) and not PositionsLimitHit() then
        if AllowTrade then
            ...
            local requestId = OpenTrade("B", Amount);
            local controller = breakeven:CreateMartingale(OpenTrade);
            controller:SetRequestID(requestId);
            controller:SetLotSizingValue(lots_multiplier);
        end
    ...
    if IsEntryShort(main_source, entry_period) and last_entry ~= main_source:date(NOW) and not PositionsLimitHit() then
        if AllowTrade then
            ...
            local requestId = OpenTrade("S", Amount);
            local controller = breakeven:CreateMartingale(OpenTrade);
            controller:SetRequestID(requestId);
            controller:SetLotSizingValue(lots_multiplier);
        end
...
function OpenTrade(side, amount, parent_trade)
...
function breakeven:CreateMartingale(openFunction)
    ...
    function controller:DoBreakeven()
        ...
        self._step = math.max(12, (main_source.high[NOW - 1] - main_source.low[NOW - 1]) / pipSize / DEL);
        if trade.BS == "B" then
