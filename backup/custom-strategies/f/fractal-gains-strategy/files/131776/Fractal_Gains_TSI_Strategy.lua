...
function Init()
    strategy:name("Fractal Gains TSI Strategy")
    strategy:description("Fractal Gains TSI Strategy")

    strategy.parameters:addGroup("TSI parameters")
    strategy.parameters:addInteger("tsi_1_length", "TSI Smooth 1", "", 7)
    strategy.parameters:addInteger("tsi_2_length", "TSI Smooth 2", "", 14)
    ...
    strategy.parameters:addBoolean("use_breakeven", "Use breakeven", "", true);
...
function Prepare(nameOnly)
    ...
    tsi = core.indicators:create("TSI", Source_Entry, instance.parameters.tsi_1_length, instance.parameters.tsi_2_length);
...
function ExtUpdate(id, source, period)
            ...
            while true do
                ...
                if trade.BS == "B" then
                    if DnFrValue ~= nil then
                        ...
                        trading:MoveStop(trade, stopValue)
                    end
                else
    ...
    if (id == 5 and FirstStart) or id == 3 then
        ...
        if UpFrValue ~= nil and DnFrValue ~= nil then
            ...
            local conditionalDistanceEntryStop = core.host:execute("getTradingProperty", "conditionalDistanceEntryStop", instance.bid:instrument(), Account);
            if DO.Buff1[_period - 1] < DO.Buff2[_period - 1] and DO.Buff1[_period] > DO.Buff2[_period] then
                ...
                if FractalPrice > instance.ask[NOW] + conditionalDistanceEntryStop and tsi.DATA[period] > 0 then
            ...
            if DO.Buff2[_period - 1] < DO.Buff1[_period - 1] and DO.Buff2[_period] > DO.Buff1[_period] then
                ...
                if FractalPrice < instance.bid[NOW] - conditionalDistanceEntryStop and tsi.DATA[period] < 0 then
