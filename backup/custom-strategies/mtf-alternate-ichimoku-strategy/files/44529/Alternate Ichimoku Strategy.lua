...
function Init() --The strategy profile initialization
    strategy:name("Alternate Ichimoku Strategy")
    ...
    strategy.parameters:addInteger("SSP", "SSP", "Period Priority Line", 75)
    strategy.parameters:addInteger("SSK", "SSK", "Tolerance Second Line", 75)
...
function Prepare(nameOnly)
    ...
    Indicator[1] = core.indicators:create("C_ICH_AL", Source, SSP, SSK)
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if Short["A"][period] > Short["B"][period] and core.crossesOver(Source.close, Short["A"], period) then
        if Direction then
            BUY()
        else
            SELL()
        end
    elseif Short["A"][period] < Short["B"][period] and core.crossesUnder(Source.close, Short["A"], period) then
        if Direction then
            SELL()
        else
            BUY()
    ...
    if EXIT then
        if Source.close[period] < Short["A"][period] then
            if Direction then
