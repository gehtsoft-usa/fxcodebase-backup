...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=63975
...
function Init() --The strategy profile initialization
    ...
    strategy.parameters:addInteger("Period", "MA Period", "Period", 40)
    ...
    strategy.parameters:addInteger("Period1", "MA Period", "Period", 30)
    ...
    strategy.parameters:addInteger("Period2", "MA Period", "Period", 50)
    ...
    strategy.parameters:addInteger("Period3", "MA Period", "Period", 100)
...
function Prepare(nameOnly)
    ...
    assert(
        core.indicators:findIndicator("HEIKEN_ASHI_SMOOTHED") ~= nil,
        "Please, download and install HEIKEN_ASHI_SMOOTHED.LUA indicator"
    )
    ...
    MA3 = core.indicators:create(Method3, Source.close, Period3)
    first = math.max(HA.DATA:first(), MA1.DATA:first(), MA2.DATA:first(), MA3.DATA:first())
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if
        not HA.DATA:hasData(period) or not MA1.DATA:hasData(period) or not MA2.DATA:hasData(period) or
            not MA3.DATA:hasData(period)
     then
        return
    end

    --[[
	Criteria #1: Heiken Ashi candlestick has to close above the 100 period SMA AND 30 MA >50MA >100MA
	]]
    if
        (HA.DATA:colorI(period) == core.rgb(0, 255, 0) and HA.DATA:colorI(period - 1) ~= core.rgb(0, 255, 0)) and
            MA1.DATA[period] > MA2.DATA[period] and
            MA2.DATA[period] > MA3.DATA[period] and
            HA.close[period] > MA3.DATA[period]
     then
        if Direction then
            BUY()
        else
    ...
    elseif
        (HA.DATA:colorI(period) == core.rgb(255, 0, 0) and HA.DATA:colorI(period - 1) ~= core.rgb(255, 0, 0)) and
            MA1.DATA[period] < MA2.DATA[period] and
            MA2.DATA[period] < MA3.DATA[period] and
            HA.close[period] < MA3.DATA[period]
     then
