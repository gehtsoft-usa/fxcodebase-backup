...
-- https://fxcodebase.com/code/viewtopic.php?f=31&t=71251
...
function Init() --The strategy profile initialization
    strategy:name("DNC PSAR Strategy");
    ...
    strategy.parameters:addGroup("DNC Trade");
    strategy.parameters:addInteger("N", "Period", "", 20, 2, 10000)
    strategy.parameters:addInteger("MyLimit", "Maximum Entry Limit Period", "", 20, 0, 10000)	
	
	    strategy.parameters:addGroup("PSAR Calculation")
...
function Prepare( nameOnly)
	...
	MyLimit= instance.parameters.MyLimit;
	...
	assert(core.indicators:findIndicator("DNC CONTINUATION") ~= nil, "Please, download and install DNC CONTINUATION.LUA indicator");          
    ...
    DNC = core.indicators:create("DNC CONTINUATION", Source, N  ); 
	
	SAR = core.indicators:create("SAR", Source, Step, Max)
    Line[1] = SAR:getStream(0)
    Line[2] = SAR:getStream(1)
	
	first=math.max(DNC.DATA:first(),Line[1]:first(), Line[2]:first()); 
...
function EntryFunction( id, now,period)
    ...
    elseif H~=0 
	and H< MyLimit
	and not (Line[2]:hasData(period - 1)) and Line[2]:hasData(period) 
	then
            if Direction then
                SELL();
            else
