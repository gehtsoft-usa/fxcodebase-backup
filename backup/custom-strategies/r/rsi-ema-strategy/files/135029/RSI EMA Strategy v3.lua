...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=69929
...
function Init() --The strategy profile initialization
    strategy:name("RSI EMA Strategy")
    ...
    strategy.parameters:addInteger("ema_period", "MA Period", "", 14);
    AddAverages("ema_method", "EMA Method", "MVA");
    strategy.parameters:addInteger("ma2_period", "MA 2 Period", "", 14);
    AddAverages("ma2_method", "MA 2 Method", "EVA");

    strategy.parameters:addDouble("OB", "OB Level", "", 70)
    strategy.parameters:addDouble("OS", "OS Level", "", 30)
    strategy.parameters:addInteger("Look_Back_Period", "Look_Back_Period 0- Unlimited", "", 0)
...
function Prepare(nameOnly)
    ...
    Look_Back_Period = instance.parameters.Look_Back_Period
    ...
    ma2 = CreateAverages(instance.parameters.ma2_period, instance.parameters.ma2_method, RSI.DATA);
    EMA = CreateAverages(instance.parameters.ema_period, instance.parameters.ema_method, RSI.DATA);
...
function EntryFunction(id, now, period)
    ...
    local Zone = Last_Zone(period)

    -- only buy if we have a fast cross over slow and the price is above the moving averages.
    if RSI.DATA[period] > EMA.DATA[period] and RSI.DATA[period - 1] <= EMA.DATA[period - 1] and EMA.DATA[period] > ma2.DATA[period] and Zone == -1 then
        if Direction then
            BUY()
        else
            SELL()
        ...
        Return = true
    elseif RSI.DATA[period] < EMA.DATA[period] and RSI.DATA[period - 1] >= EMA.DATA[period - 1] and EMA.DATA[period] < ma2.DATA[period] and Zone == 1 then
        if Direction then
            SELL()
        else
            BUY()
...
function Last_Zone(period)
    local Return = 0

    for i = period, math.max(Source:first(), period - Look_Back_Period), -1 do
