...
--http://fxcodebase.com/code/viewtopic.php?f=31&t=69421
...
local STRATEGY_NAME = "RSMK Strategy";
function CreateParameters() 
    strategy.parameters:addInteger("ema_period", "EMA period", "", 3);
    strategy.parameters:addString("instrument", "Comparison Index", "", "SPX500");
    strategy.parameters:setFlag("instrument", core.FLAG_INSTRUMENTS);
    strategy.parameters:addInteger("shift", "Shift, bars", "", 90)
end

local indi;
function CreateEntryIndicators(source)
    local profile = core.indicators:findIndicator("RSMK");
    assert(profile ~= nil, "Please, download and install " .. "RSMK" .. ".LUA indicator");
    local indicatorParams = profile:parameters();
    indi = core.indicators:create("RSMK", source, instance.parameters.ema_period, instance.parameters.instrument, instance.parameters.shift);
end
