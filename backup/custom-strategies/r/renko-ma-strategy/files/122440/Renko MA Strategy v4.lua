...
function FindCross()
    for i = renko_candles:size() - 1, 1, -1 do
        if core.crossesOver(renko_candles.close, ma.DATA, i) then
            return 1, i;
        end
        if core.crossesUnder(renko_candles.close, ma.DATA, i) then
            return -1, i;
        end
    end
end
...
function IsBuyCondition(source, period)
    if ma.DATA:size() < instance.parameters.min_bricks + 2 or not ma.DATA:hasData(NOW - instance.parameters.min_bricks - 2) then
        return false;
    end
    ...
    return (last_serial == nil or renko_candles:date(p) > last_serial) 
        and renko_candles:size() - 1 - p >= instance.parameters.min_bricks;
end
