...
--http://fxcodebase.com/code/viewtopic.php?f=31&t=65011
...
function Init() --The strategy profile initialization
    strategy:name("Reverse EMA Period Input Strategy")
    ...
    strategy.parameters:addInteger("Trend", "Trend", "", 39)
    strategy.parameters:addInteger("Cycle", "Cycle", "", 6)
...
function Prepare(nameOnly)
    ...
    assert(
        core.indicators:findIndicator("REVERSE EMA PERIOD INPUT") ~= nil,
        "Please, download and install REVERSE EMA PERIOD INPUT.LUA indicator"
    )
    ...
    Trend = core.indicators:create("REVERSE EMA PERIOD INPUT", Source.close, instance.parameters.Trend)
    Cycle = core.indicators:create("REVERSE EMA PERIOD INPUT", Source.close, instance.parameters.Cycle)
    first = math.max(Trend.DATA:first(), Cycle.DATA:first())
...
function ExitFunction(now, period)
    ...
    if
        (Cycle.DATA[period] < 0 and Cycle.DATA[period - 1] >= 0) or
            (Trend.DATA[period] < 0 and Trend.DATA[period - 1] >= 0)
     then
        if Direction then
...
function EntryFunction(now, period)
    ...
    if Trend.DATA[period] > 0 and Cycle.DATA[period] > 0 and Cycle.DATA[period - 1] <= 0 then
        if Direction then
            BUY()
        else
            SELL()
        ...
        Return = true
    elseif Trend.DATA[period] < 0 and Cycle.DATA[period] < 0 and Cycle.DATA[period - 1] >= 0 then
        if Direction then
            SELL()
        else
            BUY()
