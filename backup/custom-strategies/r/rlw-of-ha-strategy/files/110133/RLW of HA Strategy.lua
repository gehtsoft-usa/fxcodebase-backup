...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=64220
...
function Init() --The strategy profile initialization
    strategy:name("RLW of HA Strategy")
    ...
    strategy.parameters:addDouble("OB", "OB", "", -20)
    strategy.parameters:addDouble("OS", "OS", "", -80)
...
function Prepare(nameOnly)
    ...
    RLW = core.indicators:create("RLW", HA:getCandleOutput(0), Period)
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if period < Period or period < first then
        return
    end

    RLW:update(core.UpdateLast)

    if not RLW.DATA:hasData(period) then
        return
    end
...
function EntryFunction(period, now)
    ...
    if RLW.DATA[period] > OS and RLW.DATA[period - 1] <= OS then
        if Direction then
            BUY()
        else
            SELL()
        ...
        Return = true
    elseif RLW.DATA[period] < OB and RLW.DATA[period - 1] >= OB then
        if Direction then
            SELL()
        else
            BUY()
