...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=68488
...
local STRATEGY_NAME = "Renko Averages Strategy";
...
function CreateParameters() 
    AddAverages("ma_method", "Method", "EMA");
    strategy.parameters:addInteger("ma_period", "Period", "", 7);
...
function CreateIndicators(source)
    EnsureIndicatorInstalled("RENKO_CANDLES_NEW");
    renko = core.indicators:create("RENKO_CANDLES_NEW", nil, source:instrument(), source:barSize(),
        source:isBid(), instance.parameters.Renko_Size, false);
    indi = CreateAverates(instance.parameters.ma_method, renko.close, instance.parameters.ma_period);
end
...
function CreateCustomActions()
    ...
    enterLongAction.IsPass = function (source, period, periodFromLast, data)
        if renko.close:size() < 2 or not indi.DATA:hasData(period - 1) then
            return false;
        end
        return core.crossesOver(renko.close, indi.DATA, renko.close:size() - 1, period);
    end
    ...
    enterShortAction.IsPass = function (source, period, periodFromLast, data)
        ...
        return core.crossesUnder(renko.close, indi.DATA, renko.close:size() - 1, period);
    end
...
function GoLong(source, period, positions, log)
    ...
    last_serial = renko.close:date(renko.close:size() - 1);
end
...
function EntryFunction(source, period)
    UpdateIndicators();
    if renko.close:size() > 0 and last_serial == renko.close:date(renko.close:size() - 1) then
        return;
    end
