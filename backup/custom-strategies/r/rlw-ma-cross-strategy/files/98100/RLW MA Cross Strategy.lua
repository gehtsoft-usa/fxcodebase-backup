...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=61707
...
function Init() --The strategy profile initialization
    strategy:name("RLW MA Cross Strategy");
	...
	strategy.parameters:addString("Algorithm", "Strategy Algorithm", "", "Cross");
    strategy.parameters:addStringAlternative("Algorithm", "Cross", "", "Cross");
    strategy.parameters:addStringAlternative("Algorithm", "Position+Direction", "", "Position_Direction");

    
	strategy.parameters:addGroup("RLW Calculation");
    strategy.parameters:addInteger("rlw", "RLW Period", "", 14);
...
local first;
local Method, Period, MA, rlw,RLW;
local Direction;
local CustomID;
local Algorithm;
...
function Prepare( nameOnly)
    ...
    RLW = core.indicators:create("RLW", Source, rlw  );
    assert(core.indicators:findIndicator(Method) ~= nil, Method .. " indicator must be installed");
	MA = core.indicators:create(Method, RLW.DATA, Period);	 
...
function ExtUpdate(id, source, period)  -- The method called every time when a new bid or ask price appears.
   ...
   -- get only time
    ...
    if ( core.crossesOver (RLW.DATA, MA.DATA,period) and Algorithm== "Cross")
	or (RLW.DATA[period] > MA.DATA[period] and Source.close[period]  >  Source.open[period] and Algorithm=="Position_Direction")
	then	
          if Direction then
                BUY();
            else
    ...
    elseif  (core.crossesUnder (RLW.DATA, MA.DATA,period) and Algorithm == "Cross")
	or (RLW.DATA[period] < MA.DATA[period]and Source.close[period]  <  Source.open[period] and Algorithm=="Position_Direction")
    then
            if Direction then
                SELL();
