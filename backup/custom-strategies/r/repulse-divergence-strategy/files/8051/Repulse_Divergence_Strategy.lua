...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=3363
...
function Init() --The strategy profile initialization
    strategy:name("Repulse divergence strategy")
    strategy:description("Repulse divergence strategy")
    strategy:setTag("Version", "2")
    strategy:setTag("NonOptimizableParameters", "Email,SendEmail,SoundFile,RecurrentSound,PlaySound, ShowAlert")

    strategy.parameters:addGroup("Repulse indicator parameters")
    strategy.parameters:addInteger("RepulsePeriod1", "RepulsePeriod1", "", 1)
    strategy.parameters:addInteger("RepulsePeriod2", "RepulsePeriod2", "", 5)
    strategy.parameters:addInteger("RepulsePeriod3", "RepulsePeriod3", "", 15)
    strategy.parameters:addInteger("RepulseLine", "RepulseLine", "", 1)
...
function Prepare(nameOnly)
            ...
            instance.bid:name() ..
                "." ..
                    instance.parameters.TF ..
                        "," ..
                            "Repulse Div(" ..
                                instance.parameters.RepulsePeriod1 ..
                                    ", " ..
                                        instance.parameters.RepulsePeriod2 ..
                                            ", " ..
                                                instance.parameters.RepulsePeriod3 ..
                                                    ", " .. instance.parameters.RepulseLine .. "))"
    instance:name(name)
    if nameOnly then
        return
    end
    ...
    assert(
        core.indicators:findIndicator("REPULSE_DIVERGENCE") ~= nil,
        "Please, download and install REPULSE_DIVERGENCE.LUA indicator"
    )
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    RD:update(core.UpdateLast)

    -- Check that we have enough data
    if (RD.DATA:first() > (period - 1)) then
        return
    end
    ...
    if RD:getStream(3):hasData(period - 2) then
        v = RD:getStream(3)[period - 2]
        if v ~= 0 then
    ...
    if RD:getStream(4):hasData(period - 2) then
        v = RD:getStream(4)[period - 2]
        if v ~= 0 then
            if instance.parameters.TypeSignal == "direct" then
                MustS = true
            else
