...
--http://fxcodebase.com/code/viewtopic.php?f=31&t=70954
...
function CreateParameters() 
    ...
    strategy.parameters:addGroup("Moving Average Parameter");
    strategy.parameters:addInteger("Fast_MA", "Fast MA", "", 5);
    strategy.parameters:addInteger("Slow_MA", "Slow MA", "", 10);
    strategy.parameters:addGroup("ADX Parameter");
    strategy.parameters:addInteger("ADX_Period", "ADX Period", "", 14);
    strategy.parameters:addGroup("CCI Parameter");
    strategy.parameters:addInteger("Period_CCI", "Period CCI", "", 14);
end

local sar, macd, fast_ma, slow_ma, dmi, cci;
function CreateEntryIndicators(source)
    ...
    fast_ma = core.indicators:create("MVA", source, instance.parameters.Fast_MA);
    slow_ma = core.indicators:create("MVA", source, instance.parameters.Slow_MA);
    dmi = core.indicators:create("DMI", source, instance.parameters.ADX_Period);
    cci = core.indicators:create("CCI", source, instance.parameters.Period_CCI);
end
...
function IsEntryLong(source, period)
    ...
    if fast_ma.DATA[period] <= slow_ma.DATA[period] then
        return false;
    end
