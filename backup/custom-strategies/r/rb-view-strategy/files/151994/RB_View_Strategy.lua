-- Available @ https://fxcodebase.com/code/viewtopic.php?f=31&t=74019
...
function CreateParameters() 
    ...
    strategy.parameters:addGroup("RB View parameters");
    strategy.parameters:addInteger("RangeSize", "Range of bar in pips", "", 10);

    strategy.parameters:addGroup("Tick Extreme TMA Line parameters");
...
function CreateEntryIndicators(source)
    local profile = core.indicators:findIndicator("RB_VIEW");
    assert(profile ~= nil, "Please, download and install " .. "RB_VIEW" .. ".LUA indicator");
    local indicatorParams = profile:parameters();
    indicatorParams:setString("instrument", source:instrument());
    indicatorParams:setString("frame", instance.parameters.timeframe);
    indicatorParams:setBoolean("type", instance.parameters.type);
    indicatorParams:setInteger("RangeSize", instance.parameters.RangeSize);
    rb = core.indicators:create("RB_VIEW", source, indicatorParams);

    local profile = core.indicators:findIndicator("TICK_EXTREME_TMA_LINE");
    assert(profile ~= nil, "Please, download and install " .. "TICK_EXTREME_TMA_LINE" .. ".LUA indicator");
    local indicatorParams = profile:parameters();
    ...
    tma = core.indicators:create("TICK_EXTREME_TMA_LINE", rb:getCandleOutput(0), indicatorParams)

    ma = CreateAverages(instance.parameters.ma_period, instance.parameters.ma_method, rb:getCandleOutput(0));
end

function UpdateIndicators()
    --update your indicators here
    rb:update(core.UpdateLast);
...
function IsEntryLong(source, period)
    if not ma.DATA:hasData(NOW - 1) then
        return false;
    end
    return core.crossesOverOrTouch(ma.DATA, tma.Upper, NOW);
end
function IsEntryShort(source, period)
    if not ma.DATA:hasData(NOW - 1) then
        return false;
    end
    return core.crossesUnderOrTouch(ma.DATA, tma.Lower, NOW);
end
