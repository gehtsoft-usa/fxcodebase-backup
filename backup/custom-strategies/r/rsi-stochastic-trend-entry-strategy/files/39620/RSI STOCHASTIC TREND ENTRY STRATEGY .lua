...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=22984
...
function Init() --The strategy profile initialization
    strategy:name("RSI STOCHASTIC TREND ENTRY STRATEGY");
	...
	strategy.parameters:addGroup("SMA Calculation");
    strategy.parameters:addInteger("MP", " Period", " Period", 150);
   
	strategy.parameters:addGroup("STO Calculation");
	strategy.parameters:addInteger("AP", "Period", " Period", 7);
	strategy.parameters:addDouble("AM", "Multiplier", " Period", 1);
	
	strategy.parameters:addGroup("RSI Calculation");
	strategy.parameters:addInteger("RP", "Period", " Period", 3);
	strategy.parameters:addDouble("RB", "OB Level" , "", 70);
	strategy.parameters:addDouble("RS", "OS Level" , "", 30);
	...
	strategy.parameters:addDouble("SB", "OB Level" , "", 80);
	strategy.parameters:addDouble("SS", "OS Level" , "", 20);
...
local MP, AP,AM,RP,RB,RS, K,D,SD, MVAT_K, MVAT_D,SB,SS;
...
function Prepare(nameOnly)
    ...
    Indicator["SMA"] = core.indicators:create("MVA", Source[Price], MP   );
	Indicator["ATR"] = core.indicators:create("ATR", Source , AP   );
	 ...
	 L1 = Indicator["STO"]:getStream(0);
	  L2 = Indicator["STO"]:getStream(1);
	
	first=math.max( Indicator["SMA"].DATA:first(),Indicator["ATR"].DATA:first(),Indicator["RSI"].DATA:first(), L2:first() );
...
function ExtUpdate(id, source, period)  -- The method called every time when a new bid or ask price appears.
   ...
   Indicator["SMA"] :update(core.UpdateLast);
   ...
   -- get only time
    now = now - math.floor(now);
      ...
      return ;
        ...
        if   Source.close[period]> Indicator["SMA"].DATA[period] 
	    and	 Source.close[period]< Indicator["SMA"].DATA[period]  + Indicator["ATR"].DATA[period] 
		and Indicator["RSI"].DATA[period]< RS
		and L1[period]< SS
		and core.crossesOver(L1, L2,period)
		then
		...
		elseif Source.close[period]> Indicator["SMA"].DATA[period] 
		and Source.close[period] >  Indicator["SMA"].DATA[period]  -Indicator["ATR"].DATA[period] 
		and Indicator["RSI"].DATA[period] > RB
		and L1[period] > SB
		and core.crossesUnder(L1, L2,period)
		 then
            if Direction then
                SELL();
            else
