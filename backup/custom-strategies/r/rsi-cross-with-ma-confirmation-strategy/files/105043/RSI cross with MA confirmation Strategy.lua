...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=63197
...
function Init() --The strategy profile initialization
    strategy:name("RSI cross with MA confirmation Strategy")
    ...
    strategy.parameters:addDouble("BL", "Long Entry Level", "Level", 50)
    strategy.parameters:addDouble("SL", "Short Entry Level", "Level", 50)

    strategy.parameters:addDouble("BTP", "Long Take profit Level", "Level", 60)
    strategy.parameters:addDouble("STP", "Short Take profit Level", "Level", 40)

    strategy.parameters:addGroup("MA Calculation")
    strategy.parameters:addInteger("Period1", "1. MA Period", "Period", 13)
    ...
    strategy.parameters:addInteger("Period2", "2. MA Period", "Period", 48)
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if MA1.DATA[period] > MA2.DATA[period] and RSI.DATA[period] > BL and RSI.DATA[period - 1] <= BL then
        if Direction then
            BUY()
        else
            SELL()
    ...
    elseif MA1.DATA[period] < MA2.DATA[period] and RSI.DATA[period] < SL and RSI.DATA[period - 1] >= SL then
        if Direction then
            SELL()
        else
            BUY()
    ...
    if Exit then
        if RSI.DATA[period] > BTP or RSI.DATA[period] < BL then
            if Direction then
        ...
        if RSI.DATA[period] < STP or RSI.DATA[period] > SL then
            if Direction then
                if haveTrades("S") then
                    exitSpecific("S")
                    Signal("Close Short")
