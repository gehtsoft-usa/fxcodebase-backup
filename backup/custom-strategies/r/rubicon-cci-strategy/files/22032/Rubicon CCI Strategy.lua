...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=4734
...
function Init() --The strategy profile initialization
    strategy:name("Rubicon CCI Strategy")
    ...
    strategy.parameters:addInteger("CCI", "CCI Period", "", 10)
    strategy.parameters:addInteger("EMA", "EMA Period", "", 3)

    strategy.parameters:addGroup("Filter")
    strategy.parameters:addInteger("Period", "Long Period", "", 34)
    strategy.parameters:addString("Method", "Method", "Method", "MVA")
    ...
    strategy.parameters:addStringAlternative("Method", "HMA", "HMA", "HMA")
...
function Prepare(nameOnly)
    ...
    Parameters["Method"] = instance.parameters.Method
    Parameters["Period"] = instance.parameters.Period
    Parameters["Filter"] = instance.parameters.Filter
    ...
    name =
        ...
        ", " ..
            instance.parameters.TF ..
                "(" .. ", " .. Parameters["EMA"] .. ", " .. Parameters["CCI"] .. ", " .. Parameters["Method"] .. ")"
    ...
    assert(
        core.indicators:findIndicator(Parameters["Method"]) ~= nil,
        "Please, download and install" .. Parameters["Method"] .. "indicator"
    )
    assert(core.indicators:findIndicator("RUBCCI") ~= nil, "Please, download and install RUBCCI.LUA indicator")
    ...
    Indicator["RubCCI"] = core.indicators:create("RUBCCI", Source, instance.parameters.CCI, instance.parameters.EMA)
    Indicator["CCI"] = Indicator["RubCCI"].CCI
    Indicator["EMA"] = Indicator["RubCCI"].EMA

    assert(core.indicators:findIndicator(Parameters["Method"]) ~= nil, Parameters["Method"] .. " indicator must be installed");
    Indicator["FILTER"] = core.indicators:create(Parameters["Method"], Source[Price], Parameters["Period"])

    first = math.max(Indicator["CCI"]:first() + 6, Indicator["EMA"]:first() + 6, Indicator["FILTER"].DATA:first() + 6)
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    Indicator["RubCCI"]:update(core.UpdateLast)
    Indicator["FILTER"]:update(core.UpdateLast)
    ...
    if core.crossesOver(Indicator["EMA"], -100, period) and Up then
        for i = period, period - 5, -1 do
            if core.crossesOver(Indicator["CCI"], -100, i) then
                --Trade
    ...
    elseif core.crossesUnder(Indicator["EMA"], 100, period) and Down then
        for i = period, period - 5, -1 do
            if core.crossesUnder(Indicator["CCI"], 100, i) then
                --Trade
