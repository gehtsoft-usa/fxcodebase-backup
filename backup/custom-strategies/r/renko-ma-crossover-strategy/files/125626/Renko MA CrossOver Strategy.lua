...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=68311
...
local STRATEGY_NAME = "Renko MA CrossOver";
...
function CreateParameters()
    ...
    strategy.parameters:addInteger("fast_ema_period", "Fast EMA Period", "", 7);
    strategy.parameters:addInteger("slow_ema_period", "Slow EMA Period", "", 14);
    strategy.parameters:addInteger("exit_ema_period", "Exit EMA Period", "", 10);
end

local fast_ema, slow_ema, exit_ema;
local renko;
...
function CreateIndicators(source)
    ...
    fast_ema = core.indicators:create("EMA", source, instance.parameters.fast_ema_period);
    slow_ema = core.indicators:create("EMA", source, instance.parameters.slow_ema_period);
    exit_ema = core.indicators:create("EMA", source, instance.parameters.exit_ema_period);
end

function UpdateIndicators()
    fast_ema:update(core.UpdateLast);
    slow_ema:update(core.UpdateLast);
    exit_ema:update(core.UpdateLast);
end
...
function CreateCustomActions()
    ...
    exitLongAction.IsPass = function (source, period, periodFromLast, data)
        if not slow_ema.DATA:hasData(NOW - periodFromLast - 1) then
            return;
        end
        return core.crossesUnder(source.close, slow_ema.DATA[NOW - periodFromLast], period);
    end
    ...
    exitShortAction.IsPass = function (source, period, periodFromLast, data)
        ...
        return core.crossesOver(source.close, slow_ema.DATA[NOW - periodFromLast], period);
    end
    ...
    enterLongAction.IsPass = function (source, period, periodFromLast, data)
        ...
        return fast_ema.DATA[NOW - periodFromLast] > slow_ema.DATA[NOW - periodFromLast]
            and core.crossesOver(source.close, slow_ema.DATA[NOW - periodFromLast], period);
    end
    ...
    enterShortAction.IsPass = function (source, period, periodFromLast, data)
        ...
        return fast_ema.DATA[NOW - periodFromLast] < slow_ema.DATA[NOW - periodFromLast]
            and core.crossesUnder(source.close, slow_ema.DATA[NOW - periodFromLast], period);
    end
