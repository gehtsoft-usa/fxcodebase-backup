...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=64105
...
function Init() --The strategy profile initialization
    ...
    strategy.parameters:addInteger("Period1", "RSI Period", "", 14)
...
function Prepare(nameOnly)
    ...
    Indicator = core.indicators:create("RSI", Source.close, Period1)
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    local Return = EntryFunction(period, now)
end
...
function EntryFunction(period, now)
    ...
    local peakRSIvalue, peakRSIperiod, troughRSIvalue, troughRSIperiod = FindPeakTrough(period)

    if peakRSIperiod == 0 or troughRSIperiod == 0 then
        return
    end

    if not Source.close:hasData(peakRSIperiod) or not Source.close:hasData(troughRSIperiod) then
        return
    end

    if
        Source.close[period] > Source.close[peakRSIperiod] and Indicator.DATA[period] > Indicator.DATA[peakRSIperiod] and
            Indicator.DATA[period - 1] <= Indicator.DATA[peakRSIperiod]
     then
        if Direction then
            BUY()
        else
        ...
        Return = true
    elseif
        Source.close[period] < Source.close[troughRSIperiod] and
            Indicator.DATA[period] < Indicator.DATA[troughRSIperiod] and
            Indicator.DATA[period - 1] >= Indicator.DATA[troughRSIperiod]
     then
        if Direction then
            SELL()
        else
...
function FindPeakTrough(Start)
    local peakRSIvalue = 0
    local peakRSIperiod = 0
    local troughRSIvalue = 0
    local troughRSIperiod = 0

    local P
    for P = Start - 1, Indicator.DATA:first() + 1, -1 do
        if
            peakRSIperiod == 0 and Indicator.DATA[P] > Indicator.DATA[P + 1] and
                Indicator.DATA[P] > Indicator.DATA[P - 1]
         then
            peakRSIperiod = P
        end

        if
            troughRSIperiod == 0 and Indicator.DATA[P] < Indicator.DATA[P + 1] and
                Indicator.DATA[P] < Indicator.DATA[P - 1]
         then
            troughRSIperiod = P
        end

        if peakRSIperiod ~= 0 and troughRSIperiod ~= 0 then
            break
        end
    end

    return peakRSIvalue, peakRSIperiod, troughRSIvalue, troughRSIperiod
end
