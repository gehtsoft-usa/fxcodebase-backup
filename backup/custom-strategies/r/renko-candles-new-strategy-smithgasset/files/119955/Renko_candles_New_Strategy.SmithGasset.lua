...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=66270
...
function Init()
    ...
    strategy.parameters:addInteger("Renko_Step", "Brick size", "The brick size in pips", 10);
...
local BullishCount = 0;
local BearishCount = 0;
local ema21;
local ema50;
...
function Prepare(nameOnly)
    ...
    local name = profile:id() .. "( " .. instance.bid:name() .. ", " .. Renko_Step .. " )";
    instance:name(name);
    if nameOnly then
        return ;
    end

    Source = ExtSubscribe(1, nil, instance.parameters.TF, instance.parameters.Type, "bar");
    TickSource = ExtSubscribe(2, nil, "t1", instance.parameters.Type);

    assert(core.indicators:findIndicator("RENKO_CANDLES_NEW") ~= nil, "Please, download and install RENKO_CANDLES_NEW.lua indicator");          
    Indicator = core.indicators:create("RENKO_CANDLES_NEW", nil, instance.bid:instrument(), instance.parameters.TF, instance.parameters.Type == "Bid", Renko_Step, core.now() - 1, 0);
    ema21 = core.indicators:create("EMA", Source, 21);
    ema50 = core.indicators:create("EMA", Source, 50);
    first = math.max(Indicator.open:first(), Indicator.close:first());
end
...
function ExtUpdate(id, source, period) 
    ...
    ema21:update(core.UpdateLast);
    ema50:update(core.UpdateLast);
    if Indicator.open:size() > 1 and LastRenkoTime ~= Indicator.open:date(NOW) then
        if Indicator.open[NOW] < Indicator.close[NOW] then
            CloseTrades("S");

            BullishCount = BullishCount + 1;
            BearishCount = 0;
            if BullishCount == Renko_Count and ema21.DATA[NOW] < Source.close[NOW] and ema50.DATA[NOW] < Source.close[NOW] then
                if instance.parameters.allow_trade then
                    local t = trading:MarketOrder(source:instrument())
                        :SetSide("B")
                        :SetAccountID(instance.parameters.account)
                        :SetAmount(instance.parameters.amount);
                    if instance.parameters.use_stop then
                        t = t:SetPipStop(nil, instance.parameters.stop, instance.parameters.trailing_stop and instance.parameters.trailing or nil);
                    end
                    if instance.parameters.set_limit then
                        t = t:SetPipLimit(nil, instance.parameters.limit);
                    end
                    t:Execute();
                end
                signaler:Signal("Buy");
            end
        elseif Indicator.open[NOW] > Indicator.close[NOW] then
            CloseTrades("B");

            BearishCount = BearishCount + 1;
            BullishCount = 0;
            if BearishCount == Renko_Count and ema21.DATA[NOW] > Source.close[NOW] and ema50.DATA[NOW] > Source.close[NOW] then
                if instance.parameters.allow_trade then
                    local t = trading:MarketOrder(source:instrument())
                        :SetSide("S")
                        :SetAccountID(instance.parameters.account)
        ...
        LastRenkoTime = Indicator.open:date(NOW);
    end
end
