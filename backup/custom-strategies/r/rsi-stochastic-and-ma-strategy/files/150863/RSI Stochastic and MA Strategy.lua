...
function CreateTradingParameters()
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "RSMS");
...
function Prepare( nameOnly)
    ...
    Indicator = core.indicators:create("MVA", Source.close, instance.parameters.Period );
    Fast = core.indicators:create(instance.parameters.Method, Source.close, instance.parameters.Period1 );	
    Slow = core.indicators:create(instance.parameters.Method, Source.close, instance.parameters.Period2 );	 
    RSI = core.indicators:create("RSI", Source.close, instance.parameters.Period3 );		
	...
	first=math.max(Fast.DATA:first(), Slow.DATA:first(), RSI.DATA:first(), Stochastic.D:first(), SignalLine.DATA:first()); 
...
function EntryFunction( id, now,period)
	...
	and (Stochastic.K[period]-Stochastic.D[period])>Delta
	and RSI.DATA[period]>RB	
	and SignalLine.DATA[period]< SB
	then	
          if Direction then
                BUY();
            else
	...
	and  Fast.DATA[period-1]>=  Slow.DATA[period-1]
	and (Stochastic.K[period]-Stochastic.D[period])<-Delta	
	and RSI.DATA[period]<RS		
	and SignalLine.DATA[period]> SS	
	then
            if Direction then
                SELL();
            else
