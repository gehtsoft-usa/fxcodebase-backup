...
-- Available @ https://fxcodebase.com/code/viewtopic.php?f=31&t=73729
...
function Init() --The strategy profile initialization
    strategy:name("RSI , Stochastic and MA Strategy")
    ...
    strategy.parameters:addString("TF", "Trading Time frame", "", "m15")
    strategy.parameters:setFlag("TF", core.FLAG_PERIODS)
    strategy.parameters:addString("ma_TF", "MA Time frame", "", "m15")
    strategy.parameters:setFlag("ma_TF", core.FLAG_PERIODS)

    strategy.parameters:addGroup("MA Calculation")
    strategy.parameters:addInteger("Period1", "Fast Period", "", 5)
    strategy.parameters:addInteger("Period2", "Slow Period", "", 10)
    ...
    strategy.parameters:addString("rsi_TF", "RSI Time frame", "", "m1")
    strategy.parameters:setFlag("rsi_TF", core.FLAG_PERIODS)
    strategy.parameters:addInteger("Period3", "Period", "", 14)

    strategy.parameters:addInteger("RB", "Buy Level", "", 45)
    strategy.parameters:addInteger("RS", "Sell level", "", 55)

    strategy.parameters:addGroup("Stochastic Calculation")
    strategy.parameters:addString("stoch_TF", "Stochastic Time frame", "", "m5")
    strategy.parameters:setFlag("stoch_TF", core.FLAG_PERIODS)
    
    strategy.parameters:addInteger("S1", "K Period", "", 14)
    strategy.parameters:addInteger("S2", "D Period", "", 3)
    strategy.parameters:addInteger("S3", "1. D MA Period", "", 5)
    strategy.parameters:addInteger("S4", "2. D MA Period", "", 10)
    strategy.parameters:addInteger("Delta", "K/D Difference", "", 10)
    ...
    strategy.parameters:addString("M3", "2. D MA Method", "Method", "MVA")
    strategy.parameters:addStringAlternative("M3", "MVA", "MVA", "MVA")
    strategy.parameters:addStringAlternative("M3", "EMA", "EMA", "EMA")

    strategy.parameters:addInteger("SB", "Buy Level", "", 45)
    strategy.parameters:addInteger("SS", "Sell level", "", 55)
...
local AccountType
local ma_Source, TickSource, stoch_Source, rsi_Source, Source
...
function Prepare(nameOnly)
    ...
    ma_TF = instance.parameters.ma_TF
    stoch_TF = instance.parameters.stoch_TF
    rsi_TF = instance.parameters.rsi_TF
    ...
    ma_Source = ExtSubscribe(2, nil, ma_TF, instance.parameters.Type == "Bid", "bar")
    rsi_Source = ExtSubscribe(3, nil, rsi_TF, instance.parameters.Type == "Bid", "bar")
    stoch_Source = ExtSubscribe(4, nil, stoch_TF, instance.parameters.Type == "Bid", "bar")
    Source = ExtSubscribe(5, nil, TF, instance.parameters.Type == "Bid", "bar")
    Indicator = core.indicators:create("MVA", ma_Source.close, instance.parameters.Period)
    Fast = core.indicators:create(instance.parameters.Method, ma_Source.close, instance.parameters.Period1)
    Slow = core.indicators:create(instance.parameters.Method, ma_Source.close, instance.parameters.Period2)
    RSI = core.indicators:create("RSI", rsi_Source.close, instance.parameters.Period3)
    Stochastic =
    ...
    SignalLine = core.indicators:create(instance.parameters.M3, Stochastic.K, instance.parameters.S4)

    first =
        math.max(Fast.DATA:first(), Slow.DATA:first(), RSI.DATA:first(), Stochastic.D:first(), SignalLine.DATA:first())
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    SignalLine:update(core.UpdateLast)
...
function ExitFunction(id, now, period)
    ...
    if (ExitExecutionType == "EndOfTurn" and id == 5) then
        --	period1= period;
...
function EntryFunction(id, now, period)
    ...
    if
        Fast.DATA[NOW] > Slow.DATA[NOW] and Fast.DATA[NOW - 1] <= Slow.DATA[NOW - 1] and
            (Stochastic.K[NOW] - Stochastic.D[NOW]) > Delta and
            RSI.DATA[NOW] > RB and
            SignalLine.DATA[NOW] < SB
     then
        if Direction then
            BUY()
        else
