...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=62575
...
function Init() --The strategy profile initialization
    strategy:name("ROC RLW MA Strategy");
    ...
    strategy.parameters:addInteger("MA_Period", "MA Period", "", 50);
    ...
    strategy.parameters:addInteger("RLW_Period", "RLW Period", "", 50);
	strategy.parameters:addDouble("RLW_Buy", "RLW Buy Level", "", -80);
	strategy.parameters:addDouble("RLW_Sell", "RLW Sell Level", "", -20);
	
	strategy.parameters:addGroup("ROC Calculation");
    strategy.parameters:addInteger("ROC_Period", "ROC Period", "", 50);
	strategy.parameters:addDouble("ROC_Buy", "ROC Buy Level", "", 0);
	strategy.parameters:addDouble("ROC_Sell", "ROC Sell Level", "", 0);
...
function CreateTradingParameters()
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "ROCRLWMA");
...
local RLW;
local ROC;
local MA_Method,MA_Period, RLW_Period,RLW_Buy,RLW_Sell,ROC_Period,ROC_Buy,ROC_Sell;
local Direction;
local CustomID;
...
function Prepare( nameOnly)
	...
	RLW_Buy = instance.parameters.RLW_Buy;
	RLW_Sell = instance.parameters.RLW_Sell;
	ROC_Period = instance.parameters.ROC_Period;
	ROC_Buy= instance.parameters.ROC_Buy;
	ROC_Sell= instance.parameters.ROC_Sell;
	...
	ROC = core.indicators:create("ROC", Source.close, ROC_Period  );
	first=math.max(RLW.DATA:first(), MA.DATA:first(),ROC.DATA:first()); 
...
function ExtUpdate(id, source, period)  -- The method called every time when a new bid or ask price appears.
   ...
   -- get only time
    ...
    --BUY: ROC> BUY LEVEL 0 AND PRICE > MVA AND WPR CROSS OVER BUY LEVEL -90
    --SELL: ROC<SELL LEVEL 0 AND PRICE < MVA AND WPR CROSS UNDER SELL LEVEL -10


    -- only buy if we have a fast cross over slow and the price is above the moving averages.
    if  ROC.DATA[period]> ROC_Buy 
	and Source.close[period]> MA.DATA[period]
    and RLW.DATA[period]> RLW_Buy
	and RLW.DATA[period-1]<= RLW_Buy
	then	
          if Direction then
                BUY();
            else
    ...
    elseif  ROC.DATA[period]< ROC_Sell 
	and Source.close[period]< MA.DATA[period]
	and RLW.DATA[period]< RLW_Sell
	and RLW.DATA[period-1]>= RLW_Sell
	then
            if Direction then
                SELL();
            else
	...
	if Exit then
	                --EXIT BUY: PRICE CROSS UNDER MVA OR ROC CROSSUNDER BUY LEVEL 
		            if     Source.close[period] < MA.DATA[period]
					or    ROC.DATA[period]< ROC_Buy
					then 			
								if   Direction then
					   ...
					   --EXIT SELL: PRICE CROSS OVER MVA OR ROC CROSS OVER SELL LEVEL
						  
					  if Source.close[period]> MA.DATA[period]
					  or    ROC.DATA[period]> ROC_Sell
		              then
								   if   Direction then    
										if haveTrades("S") then
										   exitSpecific("S");
