...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=64823
...
function Init() --The strategy profile initialization
    ...
    strategy.parameters:addGroup("RLW Calulation")
    strategy.parameters:addInteger("RLW_Period", "Period", "", 34)
    strategy.parameters:addDouble("BL", "Buy Level", "", -50)
    strategy.parameters:addDouble("SL", "Sell Level", "", -50)

    strategy.parameters:addGroup("RLW MA Calulation")

    strategy.parameters:addInteger("RLW_MA_Period", "Period", "", 50)

    strategy.parameters:addString("RLW_MA_Method", "MA Method", "Method", "MVA")
    strategy.parameters:addStringAlternative("RLW_MA_Method", "MVA", "MVA", "MVA")
    strategy.parameters:addStringAlternative("RLW_MA_Method", "EMA", "EMA", "EMA")
    strategy.parameters:addStringAlternative("RLW_MA_Method", "LWMA", "LWMA", "LWMA")
    strategy.parameters:addStringAlternative("RLW_MA_Method", "TMA", "TMA", "TMA")
    strategy.parameters:addStringAlternative("RLW_MA_Method", "SMMA", "SMMA", "SMMA")
    strategy.parameters:addStringAlternative("RLW_MA_Method", "KAMA", "KAMA", "KAMA")
    strategy.parameters:addStringAlternative("RLW_MA_Method", "VIDYA", "VIDYA", "VIDYA")
    strategy.parameters:addStringAlternative("RLW_MA_Method", "WMA", "WMA", "WMA")

    strategy.parameters:addGroup("Price MA Calulation")

    strategy.parameters:addInteger("Period", "Period", "", 50)
...
local RLW_Period, BL, SL, RLW
local Price_MA, Price, Method, Period
local RLW_MA_Period, RLW_MA_Method, MA
...
function Prepare(nameOnly)
    ...
    RLW_MA_Period = instance.parameters.RLW_MA_Period
    RLW_MA_Method = instance.parameters.RLW_MA_Method
    ...
    assert(core.indicators:findIndicator(RLW_MA_Method) ~= nil, RLW_MA_Method .. " indicator must be installed");
    MA = core.indicators:create(RLW_MA_Method, RLW.DATA, RLW_MA_Period)
    assert(core.indicators:findIndicator(Method) ~= nil, Method .. " indicator must be installed");
    Price_MA = core.indicators:create(Method, Source[Price], Period)
    first = math.max(RLW.DATA:first(), Price_MA.DATA:first(), MA.DATA:first())
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    Price_MA:update(core.UpdateLast)
...
function EntryFunction(now, period)
    ...
    if
        RLW.DATA[period] > MA.DATA[period] and RLW.DATA[period - 1] <= MA.DATA[period - 1] and MA.DATA[period] < BL and
            Source.close[period] > Price_MA.DATA[period]
     then
        if Direction then
            BUY()
        else
        ...
        Return = true
    elseif
        RLW.DATA[period] < MA.DATA[period] and RLW.DATA[period - 1] >= MA.DATA[period - 1] and MA.DATA[period] > SL and
            Source.close[period] < Price_MA.DATA[period]
     then
        if Direction then
            SELL()
