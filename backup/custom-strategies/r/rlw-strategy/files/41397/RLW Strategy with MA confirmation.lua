...
function Init() --The strategy profile initialization
    strategy:name("RLW Strategy with MA confirmation");
    ...
    strategy.parameters:addInteger("SP", "Short Period", "", 14);
	...
	strategy.parameters:addInteger("Mode", "Exit Node", "", 3);
    strategy.parameters:addIntegerAlternative("Mode", "RLW", "", 1);
    strategy.parameters:addIntegerAlternative("Mode", "MA", "", 2);
	strategy.parameters:addIntegerAlternative("Mode", "Both", "", 3);
...
function Prepare(nameOnly)
    ...
    local i;
	   ...
	   name = name .. ", " ..  SP.. ", " .. OB.. ", " .. OS.. ", " .. Price1.. ", " .. Method1.. ", " .. P1.. ", " .. Price2.. ", " .. Method2.. ", " .. P2.. ", " .. Price3.. ", " .. Method3.. ", " .. P3;
    ...
    Indicator[1] = core.indicators:create("RLW", Source, SP );
	Short[1] = Indicator[1]:getStream(0);
	
    assert(core.indicators:findIndicator(Method1) ~= nil, Method1 .. " indicator must be installed");
	Indicator[2] = core.indicators:create(Method1, Source[Price1], P1 );
	Short[2] = Indicator[2]:getStream(0);
	
    assert(core.indicators:findIndicator(Method2) ~= nil, Method2 .. " indicator must be installed");
	Indicator[3] = core.indicators:create(Method2, Source[Price2], P2 );
	Short[3] = Indicator[3]:getStream(0);
	
    assert(core.indicators:findIndicator(Method3) ~= nil, Method3 .. " indicator must be installed");
	Indicator[4] = core.indicators:create(Method3, Source[Price3], P3 );
...
function ExtUpdate(id, source, period)  -- The method called every time when a new bid or ask price appears.
   ...
   -- get only time
    ...
    if period < first +1  then
        ...
        if    core.crossesOver(Short[1], OS,period)
		and((Source.close[period] > Short[2][period]) or not USE1)
		and((Source.close[period] > Short[3][period]) or not USE2)
		and((Source.close[period] > Short[4][period]) or not USE3)
		then
		...
		elseif   core.crossesUnder(Short[1], OB,period)
		and((Source.close[period] < Short[2][period]) or not USE1)
		and((Source.close[period] < Short[3][period]) or not USE2)
		and((Source.close[period] < Short[4][period]) or not USE3)
		...
		if EXIT then
		     ...
		     if Mode ~= 2 then
				  ...
				  if       core.crossesUnder(Short[1], OB,period)
					then 			
								if   Direction then
			 ...
			 if Mode ~= 1 then
			        ...
			        if     ((Source.close[period] < Short[2][period]) and USE1)
							or((Source.close[period] < Short[3][period]) and USE2)
							or((Source.close[period] < Short[4][period]) and USE3)
					then 			
								if   Direction then
										   if haveTrades("B") then
											   exit("B");
