...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=40934
...
function Init() --The strategy profile initialization
    strategy:name("Rs Dynamic Line Strategy")
...
function Prepare(nameOnly)
    ...
    assert(
        core.indicators:findIndicator("RSDYNAMIC_LINE") ~= nil,
        "Please, download and install RSDYNAMIC_LINE.LUA indicator"
    )
    ...
    Indicator[1] = core.indicators:create("RSDYNAMIC_LINE", Source[Price], Period)
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if Short[3][period] > Short[4][period] and core.crossesOver(Source.close, Short[1], period) then
        if Direction then
            BUY()
        else
            SELL()
        end
    elseif Short[3][period] < Short[4][period] and core.crossesUnder(Source.close, Short[2], period) then
        if Direction then
            SELL()
        else
            BUY()
