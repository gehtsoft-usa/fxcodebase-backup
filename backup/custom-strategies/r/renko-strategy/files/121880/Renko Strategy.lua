...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=66876
...
function Init()
    ...
    strategy:description("v1 2018-10-29");
...
function Prepare(name_only)
    ...
    renko = core.indicators:create("RENKO_CANDLES_NEW", nil, instance.bid:instrument(), 
        instance.parameters.timeframe, instance.parameters.is_bid, Renko_Step, false);
    local valid;
...
function IsBuyCondition(source, period)
    return last_serial ~= renko.open:serial(NOW) and renko.open[NOW] < renko.close[NOW];
end
