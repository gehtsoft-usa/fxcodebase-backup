...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=67318
...
function Init()
    strategy:name("Risk Management Strategy");
    strategy:description("");
    strategy:setTag("group", "Samples");
    strategy:type(core.Both);
    
    strategy.parameters:addDouble("risk", "Risk, %", "", 1);
...
local allow_trade;
local risk;
local custom_id = "Risk Management Strategy";
    ...
    if cookie == OFFERS_UPDATE then
        ...
        trading:FindTrade()
            ...
            :Do(function (trade) 
                    ...
                    local base_size = core.host:execute("getTradingProperty", "baseUnitSize", trade.Instrument, instance.parameters.account);
                    local offer = core.host:findTable("offers"):find("OfferID", trade.OfferID);
                    if trade.BS == "B" then
                        loss = loss + (trade.Lot / base_size) * offer.PipCost * (stop.Rate - offer.Bid) / offer.PointSize
                    else
                        loss = loss + (trade.Lot / base_size) * offer.PipCost * (offer.Ask - stop.Rate) / offer.PointSize
                    end
                end);
        local lossPr = 100 * loss / equity;
        if lossPr >= risk then
            signaler:Signal("Risk target of account equity is reached " .. tostring(risk) .. "%. Closing positions")
            trading:FindTrade()
                :WhenAccountID(instance.parameters.account)
