...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=69100
...
function Init()
    ...
    strategy.parameters:addInteger("bars_back", "Bars back", "", 1);
...
function Prepare(nameOnly)
    ...
    local profile = core.indicators:findIndicator("RENKO_CANDLES_NEW");
    assert(profile ~= nil, "Please, download and install " .. "RENKO_CANDLES_NEW" .. ".LUA indicator");
    local indicatorParams = profile:parameters();
    Indicator = core.indicators:create("RENKO_CANDLES_NEW", nil, instance.bid:instrument(),
...
function DoMoveStop(trade)
    ...
    renko_candles = Indicator:getCandleOutput(0);
    core.host:trace("moving stop");
    local newStop;
    local stop = trading:FindStopOrder(trade);
    if (trade.BS == "B") then
        newStop = renko_candles.close[NOW - bars_back];
...
function ExtUpdate(id, source, period)  -- The method called every time when a new bid or ask price appears.
    ...
    renko_candles = Indicator:getCandleOutput(0);
    core.host:trace(tostring(renko_candles:size()))
