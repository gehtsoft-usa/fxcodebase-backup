...
--http://fxcodebase.com/code/viewtopic.php?f=31&t=69565
...
local STRATEGY_NAME = "Range Filter Strategy";
function CreateParameters() 
    strategy.parameters:addInteger("Sampling_Period", "Sampling Period", "", 100, 1, 2000);
    strategy.parameters:addDouble("Range_Multiplier", "Range_Multiplier", "", 3, 0, 2000);
end

local indi;
function CreateEntryIndicators(source)
    local profile = core.indicators:findIndicator("RANGE FILTER");
    assert(profile ~= nil, "Please, download and install " .. "RANGE FILTER" .. ".LUA indicator");
    local indicatorParams = profile:parameters();
    indicatorParams:setInteger("Sampling_Period", instance.parameters.Sampling_Period);
    indicatorParams:setDouble("Range_Multiplier", instance.parameters.Range_Multiplier);
    indi = core.indicators:create("RANGE FILTER", source, indicatorParams)
end
...
function IsEntryLong(source, period)
    return indi.Central:colorI(period) == core.rgb(0, 255, 0) 
        and indi.Central:colorI(period - 1) ~= core.rgb(0, 255, 0);
end
function IsEntryShort(source, period)
    return indi.Central:colorI(period) == core.rgb(255, 0, 0) 
        and indi.Central:colorI(period - 1) ~= core.rgb(255, 0, 0);
end
...
function OpenTrade(side)
    ...
    if use_stop then
        ...
        if side == "B" then
            valuemap.RateStop = indi.Bottom[NOW] - stop_pips * soruce:pipSize();
        else
            valuemap.RateStop = indi.Top[NOW] + stop_pips * soruce:pipSize();
        end
