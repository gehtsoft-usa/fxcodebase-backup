...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=5994
...
function Init() --The strategy profile initialization
    strategy:name("RSI of MA Strategy")
    ...
    strategy.parameters:addInteger("MA_Period", "MA_Period", "", 14)
    strategy.parameters:addInteger("RSI_Period", "RSI_Period", "", 21)
    strategy.parameters:addInteger("MA_RSI_Period", "MA_RSI_Period", "", 15)
...
function Prepare(nameOnly)
    ...
    --  assert(core.indicators:findIndicator("RSI_MA") ~= nil, "Please, download and install RSI_MA indicator");
            ...
            instance.bid:name() ..
                "." ..
                    instance.parameters.TF ..
                        ", " .. MA_Period .. ", " .. RSI_Period .. ", " .. MA_RSI_Period .. "," .. ")"
    instance:name(name)
    if nameOnly then
        return;
    end
    ...
    MA = core.indicators:create("EMA", Source[Price], MA_Period)
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    MA_RSI:update(core.UpdateLast)

    if core.crossesOver(RSI.DATA, MA_RSI.DATA, period) then
        if Direction == "direct" then
            BUY()
        else
            SELL()
        end
    elseif core.crossesUnder(RSI.DATA, MA_RSI.DATA, period) then
        if Direction == "direct" then
            SELL()
        else
            BUY()
