...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=69382
...
local STRATEGY_NAME = "RenkoStop Strategy";
...
function CreateParameters()
    strategy.parameters:addString("renko_tf", "RenkoStop Timeframe", "", "m5");
    strategy.parameters:setFlag("renko_tf", core.FLAG_PERIODS);
    strategy.parameters:addInteger("loopback", "Loopback", "", 5);
    strategy.parameters:addInteger("decay", "Decay", "Decay", 250);
    strategy.parameters:addInteger("detection", "Detection", "Detection", 1);
    strategy.parameters:addInteger("smooth", "Smooth", "Smooth", 2);
	strategy.parameters:addInteger("rma_smooth", "RMA Smooth", "RMA Smooth", 12);
end
local indi, loopback;
function CreateEntryIndicators(source)
    local renko_source = trading_logic:SubscribeHistory(source:instrument(), instance.parameters.renko_tf, source:isBid());
    loopback = instance.parameters.loopback;

    local profile = core.indicators:findIndicator("RENKOSTOP");
    assert(profile ~= nil, "Please, download and install " .. "RENKOSTOP" .. ".LUA indicator");
    local indicatorParams = profile:parameters();
    indi = core.indicators:create("RENKOSTOP", renko_source, instance.parameters.decay, 
        instance.parameters.detection, instance.parameters.smooth, instance.parameters.rma_smooth);
end
...
function CreateCustomActions()
    if not DISABLE_EXIT then
        ...
        exitLongAction.IsPass = function (source, period, periodFromLast, data)
            return indi.DATA:colorI(NOW) == core.rgb(255, 0, 0);
        end
        ...
        exitShortAction.IsPass = function (source, period, periodFromLast, data)
            return indi.DATA:colorI(NOW) == core.rgb(0, 255, 0);
        end
    ...
    enterLongAction.IsPass = function (source, period, periodFromLast, data)
        if indi.DATA:size() < loopback then
            return;
        end
        for i = 0, loopback do
            if not indi.DATA:hasData(NOW - i) then
                return;
            end
            if indi.DATA:colorI(NOW - i) ~= core.rgb(0, 255, 0) then
                return false;
            end
        end
        return true;
    ...
    enterShortAction.IsPass = function (source, period, periodFromLast, data)
        ...
        for i = 0, loopback do
            ...
            if indi.DATA:colorI(NOW - i) ~= core.rgb(255, 0, 0) then
                return false;
            end
        end
        return true;
