...
-- https://fxcodebase.com/code/viewtopic.php?f=31&t=72676
...
function Init() --The strategy profile initialization
    strategy:name("Recursive Moving Average Strategy");
    ...
    strategy.parameters:addInteger("N", "RMA Periods", "", 10, 1, 2000);
    strategy.parameters:addInteger("P", "Smoothing Periods", "", 6, 1, 2000); 
...
function CreateTradingParameters()
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "RMAS");
...
function Prepare( nameOnly)
	...
	assert(core.indicators:findIndicator("RECURSIVE MOVING AVERAGE") ~= nil, "Please, download and install RECURSIVE MOVING AVERAGE.LUA indicator");          
    ...
    Indicator = core.indicators:create("RECURSIVE MOVING AVERAGE", Source.close, instance.parameters.N, instance.parameters.P );
...
function EntryFunction( id, now,period)
    ...
    if   Indicator.RMA[period]>  Indicator.SIGNAL[period]
	and   Indicator.RMA[period-1]<=  Indicator.SIGNAL[period-1]
	then	
          if Direction then
                BUY();
            else
		...
		Return=true;
    elseif Indicator.RMA[period]<  Indicator.SIGNAL[period]
	and  Indicator.RMA[period-1]>=  Indicator.SIGNAL[period-1]
		then
            if Direction then
                SELL();
