...
--http://fxcodebase.com/code/viewtopic.php?f=31&t=69182
...
local STRATEGY_NAME = "Daily Scalping";
...
function CreateParameters()
    strategy.parameters:addInteger("usable_margin_limit", "Usable margin limit", "", 1000);
    strategy.parameters:addBoolean("use_stoch", "Use Stochastic", "", true);
    ...
    strategy.parameters:addDouble("rsi_low", "RSI low limit", "", 10);
    strategy.parameters:addDouble("rsi_high", "RSI high limit", "", 90);
end
local usable_margin_limit;
local stoch;
local rsi, rsi_low, rsi_high;
function CreateEntryIndicators(source) 
    usable_margin_limit = instance.parameters.usable_margin_limit;
    if instance.parameters.use_stoch then
        stoch = core.indicators:create("SSD", source, instance.parameters.stoch_k, instance.parameters.stoch_sd, instance.parameters.stoch_d);
    end
    if instance.parameters.use_rsi then
        rsi = core.indicators:create("RSI", source, instance.parameters.rsi_period);
        rsi_low = instance.parameters.rsi_low;
        rsi_high = instance.parameters.rsi_high;
    end
end
...
function CreateCustomActions()
    ...
    enterLongAction.IsPass = function (source, period, periodFromLast, data)
        if stoch ~= nil and not core.crossesOver(stoch.K, stoch.D, period) then
            return false;
        end
        if rsi ~= nil and rsi.DATA[period] <= rsi_low then
            return false;
        end
        local usablemargin = core.host:findTable("accounts"):find("AccountID", instance.parameters.account).UsableMargin;
        return usablemargin >= usable_margin_limit;
    end
    ...
    enterShortAction.IsPass = function (source, period, periodFromLast, data)
        if stoch ~= nil and not core.crossesUnder(stoch.K, stoch.D, period) then
            return false;
        end
        if rsi ~= nil and rsi.DATA[period] >= rsi_high then
            return false;
        end
        local usablemargin = core.host:findTable("accounts"):find("AccountID", instance.parameters.account).UsableMargin;
        return usablemargin >= usable_margin_limit;
