...
function Prepare(nameOnly)
    ...
    name =
        name ..
        ", " ..
            instance.parameters.RF ..
                ", " ..
                    instance.parameters.RSP ..
                        ", " ..
                            instance.parameters.AP ..
                                ", " .. instance.parameters.F .. ", " .. instance.parameters.S .. "," .. CustomID
    name = name .. " )"
    instance:name(name)
    ...
    QQE =
        ...
        instance.parameters.RSP,
        instance.parameters.AP,
        instance.parameters.F,
        instance.parameters.S,
        false
    )
    first = QQE:getStream(1):first()
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if period < first or not QQE.QQE:hasData(period - 1) or not QQE.TS1:hasData(period - 1) then
        return
    end

    -- only buy if we have a fast cross over slow and the price is above the moving averages.
    if core.crossesOver(QQE.QQE, QQE.TS1, period) then
        if Direction then
            BUY()
        else
            SELL()
    ...
    elseif core.crossesUnder(QQE.QQE, QQE.TS1, period) then
        if Direction then
            SELL()
        else
            BUY()
