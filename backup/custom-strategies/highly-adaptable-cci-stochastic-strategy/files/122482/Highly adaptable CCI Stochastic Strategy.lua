...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=67043
...
function Init() --The strategy profile initialization
    strategy:name("Highly adaptable CCI Stochastic Strategy");
    ...
    strategy.parameters:addInteger("CCIPeriod", "CCI Period", "CCI Period", 14);
    strategy.parameters:addInteger("StochPeriod", "Stochastic Period", "Stochastic Period", 14);
    strategy.parameters:addInteger("StochSmooth", "Stochastic Smooth", "Stochastic Smooth", 3);
	
	
	strategy.parameters:addDouble("OB", "OB Level", "Level", 80);
    strategy.parameters:addDouble("OS", "OS Level", "Level", 20);
    
	strategy.parameters:addGroup("Selector");
	
	strategy.parameters:addString("Action".. 1, "Top Line Cross Over", "", "BUY");
	...
	strategy.parameters:addString("Action".. 2, "Top Line Cross Under", "", "SELL");
	...
	strategy.parameters:addString("Action".. 3, "Central Line Cross Over", "","BUY");
	...
	strategy.parameters:addString("Action".. 4, "Central Line Cross Under", "", "SELL");
	...
	strategy.parameters:addString("Action".. 5, "Bottom Line  Cross Over", "", "BUY");
	...
	strategy.parameters:addString("Action".. 6, "Bottom Line Cross Under", "", "SELL");
...
function CreateTradingParameters()
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "CCISS");
...
local Indicators, CCIPeriod, StochPeriod, StochSmooth,OB,OS;
...
function Prepare( nameOnly)
	...
	assert(core.indicators:findIndicator("SIMPLE CCI STOCHASTIC") ~= nil, "Please, download and install SIMPLE CCI STOCHASTIC.LUA indicator");
	...
	Indicators= core.indicators:create("SIMPLE CCI STOCHASTIC", Source, CCIPeriod, StochPeriod, StochSmooth);
...
function ExtUpdate(id, source, period)  -- The method called every time when a new bid or ask price appears.
   ...
   -- get only time
	...
	then
	 ...
	 if not Indicators.DATA:hasData(period)	
	then
        return;
    end
								...
								if core.crossesOver(  Indicators:getStream(0),OB,  period)then									
									ACTION(1,1 ,"Over", period);								
								end
								if core.crossesUnder(  Indicators:getStream(0),OB,  period)then		
                                    ACTION(2,1 ,"Under", period);	  
                                end									
					            if core.crossesOver( Indicators:getStream(0),50,  period) then	
								     ACTION(3,2 ,"Over", period);	
								end	  
								if core.crossesUnder(  Indicators:getStream(0),50,  period) then	
								     ACTION(4,2 ,"Under", period);	
								end	  
								if core.crossesOver( Indicators:getStream(0),OS,  period) then	
								     ACTION(5,3 ,"Over", period);
                                end									 
								if core.crossesUnder( Indicators:getStream(0),OS,  period) then	
...
function ACTION (Flag, Line , Text,period)
                                ...
                                elseif Action[Flag] ==  "Alert" then 										
									Signal (LabelNote[Line] .. " Line "  .. Text  );				 		
								end
