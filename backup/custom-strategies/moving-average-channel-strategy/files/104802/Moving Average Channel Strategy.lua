...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=63150
...
function Init() --The strategy profile initialization
    strategy:name("Moving Average Channel Strategy")
    ...
    strategy.parameters:addGroup("Heikin-Ashi Smoothed Calculation")
    ...
    strategy.parameters:addGroup("Moving Average Channel Calculation")

    strategy.parameters:addInteger("Period", "Fast Period MA", "", 6, 2, 2000)
    strategy.parameters:addString("Method", "MA Method", "Method", "TMA")
    ...
    strategy.parameters:addGroup("Central Line Calculation")

    strategy.parameters:addBoolean("UseFilter", "Use Filter", "", true)
    strategy.parameters:addInteger("CentralPeriod", "Fast Period MA", "", 6, 2, 2000)
    strategy.parameters:addString("CentralMethod", "MA Method", "Method", "TMA")
    strategy.parameters:addStringAlternative("CentralMethod", "MVA", "MVA", "MVA")
    strategy.parameters:addStringAlternative("CentralMethod", "EMA", "EMA", "EMA")
    strategy.parameters:addStringAlternative("CentralMethod", "LWMA", "LWMA", "LWMA")
    strategy.parameters:addStringAlternative("CentralMethod", "TMA", "TMA", "TMA")
    strategy.parameters:addStringAlternative("CentralMethod", "SMMA", "SMMA", "SMMA")
    strategy.parameters:addStringAlternative("CentralMethod", "KAMA", "KAMA", "KAMA")
    strategy.parameters:addStringAlternative("CentralMethod", "VIDYA", "VIDYA", "VIDYA")
    strategy.parameters:addStringAlternative("CentralMethod", "WMA", "WMA", "WMA")
...
local Direction
local CustomID
local CentralMethod, CentralPeriod
local Method, Period
local top, bottom, central
local UseFilter, HA
local Method1, Method2, N1, N2
...
function Prepare(nameOnly)
    ...
    CentralMethod = instance.parameters.CentralMethod
    CentralPeriod = instance.parameters.CentralPeriod
    ...
    assert(
        core.indicators:findIndicator(CentralMethod) ~= nil,
        "Please, download and install" .. CentralMethod .. ".LUA indicator"
    )
    ...
    HA = core.indicators:create("HASM", Source, Method1, N1, Method2, N2)

    top = core.indicators:create(Method, HA.high, Period)
    central = core.indicators:create(CentralMethod, HA.close, CentralPeriod)
    bottom = core.indicators:create(Method, HA.low, Period)

    first = math.max(top.DATA:first(), central.DATA:first()) + 1
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    top:update(core.UpdateLast)
    bottom:update(core.UpdateLast)
    central:update(core.UpdateLast)
    ...
    if
        HA.close[period] > top.DATA[period] and HA.close[period - 1] <= top.DATA[period - 1] and
            (not UseFilter or (central.DATA[period] > central.DATA[period - 1]))
     then
        if Direction then
            BUY()
        else
    ...
    elseif
        HA.close[period] < bottom.DATA[period] and HA.close[period - 1] >= bottom.DATA[period - 1] and
            (not UseFilter or (central.DATA[period] < central.DATA[period - 1]))
     then
        if Direction then
            SELL()
    ...
    if Exit then
        if central.DATA[period] < central.DATA[period - 1] then
            if Direction then
