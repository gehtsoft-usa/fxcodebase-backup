-- Available @ https://fxcodebase.com/code/viewtopic.php?f=31&t=74883
...
function Init() --The strategy profile initialization
    strategy:name("Intraday Momentum Strategy");
	...
	strategy.parameters:addGroup("Bands Calculation");
    strategy.parameters:addInteger("Period", "Period", "", 10, 1, 2000);
    strategy.parameters:addDouble("Multiplier", "Multiplier", "", 1, 0, 2000);	
	
	strategy.parameters:addGroup("VWAP Calculation");	
    strategy.parameters:addString("src", "Source", "", "typical");
    strategy.parameters:addStringAlternative("src", "Open", "", "open");
    strategy.parameters:addStringAlternative("src", "High", "", "high");
    strategy.parameters:addStringAlternative("src", "Low", "", "low");
    strategy.parameters:addStringAlternative("src", "Close", "", "close");
    strategy.parameters:addStringAlternative("src", "Median", "", "median");
    strategy.parameters:addStringAlternative("src", "Typical", "", "typical");
    strategy.parameters:addStringAlternative("src", "Weighted", "", "weighted");
   -- strategy.parameters:addInteger("offset", "Offset", "", 0);
    --strategy.parameters:addBoolean("showBands", "Calculate Bands", "", true);
   -- strategy.parameters:addDouble("stDevMultiplier", "Bands Multiplier", "", 1);
    strategy.parameters:addString("anchor", "Anchor", "", "Session");
    strategy.parameters:addStringAlternative("anchor", "Session", "", "Session");
    strategy.parameters:addStringAlternative("anchor", "Week", "", "Week");
    strategy.parameters:addStringAlternative("anchor", "Month", "", "Month");
    strategy.parameters:addStringAlternative("anchor", "Quarter", "", "Quarter");
    strategy.parameters:addStringAlternative("anchor", "Year", "", "Year");	
...
function CreateTradingParameters()
   ...
   -- strategy.parameters:addStringAlternative("ExitExecutionType", "End of Turn", "", "EndOfTurn");
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "IMS");
...
function Prepare( nameOnly)
	...
	assert(core.indicators:findIndicator("INTRADAY MOMENTUM STRATEGY INDICATOR") ~= nil, "Please, download and install INTRADAY MOMENTUM STRATEGY INDICATOR.LUA indicator");          
    ...
    INTRADAY_MOMENTUM_STRATEGY_INDICATOR = core.indicators:create("INTRADAY MOMENTUM STRATEGY INDICATOR", Source , instance.parameters.Period  , instance.parameters.Multiplier); 
	VWAP = core.indicators:create("VWAP", Source ,  instance.parameters.src, 0, false, 1, instance.parameters.anchor); 
	first=math.max(INTRADAY_MOMENTUM_STRATEGY_INDICATOR.DATA:first(), VWAP.DATA:first()); 
...
function ExtUpdate(id, source, period)  -- The method called every time when a new bid or ask price appears.
   ...
   -- get only time
    now = now - math.floor(now);
	    ...
	    -- update indicators.
   INTRADAY_MOMENTUM_STRATEGY_INDICATOR:update(core.UpdateLast);
   --VWAP:update(core.UpdateLast);
...
function EntryFunction( id, now,period)
	...
	--/////////////////////////////////////////////////////////////////    
     ...
     or not INTRADAY_MOMENTUM_STRATEGY_INDICATOR.Top:hasData(period)
     --or not  VWAP.VWAP:hasData(period)	 
	 then
	 return;
	 end	
	 --/////////////////////////////////////////////////////////////////  
    ...
    if Source.close[period] >  INTRADAY_MOMENTUM_STRATEGY_INDICATOR.Top[period] 
	--and  Source.close[period] >  VWAP.VWAP[period] 
	then	
          if Direction then
                BUY();
            else
		...
		Return=true;
    elseif Source.close[period] < INTRADAY_MOMENTUM_STRATEGY_INDICATOR.Bottom[period] 
	--and Source.close[period] <  VWAP.VWAP[period] 
		then
            if Direction then
                SELL();
