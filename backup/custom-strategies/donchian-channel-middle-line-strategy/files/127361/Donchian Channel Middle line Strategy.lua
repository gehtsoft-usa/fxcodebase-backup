...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=68575
...
local STRATEGY_NAME = "Doncahin Channel Middle line Strategy";
...
function CreateParameters()
    ...
    AddAverages("ma", "Moving average filter", "MVA");
    strategy.parameters:addInteger("ma_period", "Moving average period", "", 14);
end

local indi, ma;
function CreateIndicators(source)
    local profile = core.indicators:findIndicator("DONCHIAN CHANNEL WITH ALERT");
    assert(profile ~= nil, "Please, download and install " .. "DONCHIAN CHANNEL WITH ALERT" .. ".LUA indicator");
    ...
    indi = core.indicators:create("DONCHIAN CHANNEL WITH ALERT", source, indicatorParams);
    ma = CreateAverates(instance.parameters.ma, source, instance.parameters.ma_period)
end
...
function CreateCustomActions()
    ...
    exitLongAction.IsPass = function (source, period, periodFromLast, data)
        return source.close[period] > indi.DU[period] and indi.DM[period] > ma.DATA[period];
    end

    local exitShortAction = {};
    exitShortAction.ActOnSwitch = false;
    exitShortAction.IsPass = function (source, period, periodFromLast, data)
        return source.close[period] < indi.DN[period] and indi.DM[period] < ma.DATA[period];
    end
    ...
    enterLongAction.IsPass = function (source, period, periodFromLast, data)
        return source.close[period] > indi.DM[period] and indi.DM[period] < ma.DATA[period];
    end
