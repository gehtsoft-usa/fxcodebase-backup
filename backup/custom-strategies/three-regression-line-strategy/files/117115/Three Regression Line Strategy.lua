...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=65646
...
function Init() --The strategy profile initialization
    strategy:name("Three Regression Line Strategy");
    ...
    strategy.parameters:addInteger("N1", "1.Line Period", "Number of periods", 14, 2, 1000);
	strategy.parameters:addInteger("N2", "2.Line Period", "Number of periods", 34, 2, 1000);
	strategy.parameters:addInteger("N3", "3.Line Period", "Number of periods", 200, 2, 1000);
...
function CreateTradingParameters()
	...
	strategy.parameters:addInteger("MaxNumberOfPositionInAnyDirection", "Max Number Of Open Position In Any Direction", "", 3);
	strategy.parameters:addInteger("MaxNumberOfPosition", "Max Number Of Position In One Direction", "", 3);
...
function Prepare( nameOnly)
	...
	Indicator3 = core.indicators:create("REGRESSION_LINE", Source.close, N3,     core.rgb(0, 255, 0),  core.rgb(255, 0, 0)  );
	first=math.max(Indicator1.DATA:first(),Indicator2.DATA:first(),Indicator3.DATA:first())+1; 
...
function ExitFunction( now,period)
   ...
   -- Exit Long if R1 is Red AND if R2 is Red AND R1<R3
					...
					and Indicator1.DATA[period]< Indicator3.DATA[period]
					then
...
function EntryFunction( now,period)
    ...
    if   Indicator3.DATA:colorI(period)==  core.rgb(0, 255, 0)
	and  Indicator2.DATA:colorI(period)==  core.rgb(0, 255, 0)
	and  Indicator1.DATA:colorI(period)==  core.rgb(0, 255, 0)
	and  Indicator1.DATA[period]> Indicator3.DATA[period]
	and  Indicator2.DATA[period]> Indicator3.DATA[period]
	then	
          if Direction then
                BUY();
            else
		...
		Return=true;
    elseif   Indicator3.DATA:colorI(period)==  core.rgb(255, 0, 0)
	and Indicator2.DATA:colorI(period)==  core.rgb(255, 0, 0)
