...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=65337
...
function Init() --The strategy profile initialization
    ...
    strategy.parameters:addInteger("Period1", "1. DMIPeriod", "", 34, 1, 2000);
	 strategy.parameters:addInteger("Period2", "2. DMIPeriod", "", 100, 1, 2000);
    strategy.parameters:addGroup("First ZigZag Calculation");
	 strategy.parameters:addBoolean("Use1", "Use ZigZag Filter", "", true);   
    strategy.parameters:addInteger("Depth1", "Depth", "The minimum number of periods used to draw one ZigZag line.", 10, 1, 1000);
    strategy.parameters:addInteger("Deviation1", "Deviation", "The maximum distance in pips by which the current high/low must be lower/higher than the previous one to return Backstep periods back to check if the current high/low is a new max/min.", 3, 1, 1000);
    strategy.parameters:addInteger("Backstep1", "Backstep", "The number of periods used to define a new min/max if the current high/low is lower/higher than the previous one by Deviation or less.", 5, 1, 1000);
    strategy.parameters:addGroup("Second ZigZag Calculation");
     strategy.parameters:addBoolean("Use2", "Use ZigZag Filter", "", true);   
    strategy.parameters:addInteger("Depth2", "Depth", "The minimum number of periods used to draw one ZigZag line.", 20, 1, 1000);
    strategy.parameters:addInteger("Deviation2", "Deviation", "The maximum distance in pips by which the current high/low must be lower/higher than the previous one to return Backstep periods back to check if the current high/low is a new max/min.", 6, 1, 1000);
    strategy.parameters:addInteger("Backstep2", "Backstep", "The number of periods used to define a new min/max if the current high/low is lower/higher than the previous one by Deviation or less.", 10, 1, 1000);
...
function CreateTradingParameters()
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "ZZDS");
...
local ZZ1, ZZ2;
local Depth1, Deviation1, Backstep1;
local Depth2, Deviation2, Backstep2;
local DMI1, Period1;
local DMI2, Period2; 
...
function Prepare( nameOnly)
	...
	--Indicator parameters
	Depth1= instance.parameters.Depth1;
	Deviation1= instance.parameters.Deviation1;
	Backstep1= instance.parameters.Backstep1;
	Depth2= instance.parameters.Depth2;
	Deviation2= instance.parameters.Deviation2;
	Backstep2= instance.parameters.Backstep2;
    ...
    DMI1 = core.indicators:create("DMI", Source , Period1  );
	DMI2 = core.indicators:create("DMI", Source , Period2  );
	ZZ1 = core.indicators:create("ZIGZAG", Source , Depth1, Deviation1, Backstep1  );
	ZZ2 = core.indicators:create("ZIGZAG", Source , Depth2, Deviation2, Backstep2  );
	
	first=math.max(DMI1.DATA:first(),DMI2.DATA:first() , ZZ1.DATA:first(),ZZ2.DATA:first()); 
...
function ExtUpdate(id, source, period)  -- The method called every time when a new bid or ask price appears.
   ...
   -- get only time
    now = now - math.floor(now);
	    ...
	    -- update indicators.
	if Use1 then
    ZZ1:update(core.UpdateAll);
	end
	if Use2 then
	ZZ2:update(core.UpdateAll);
	end
...
for i= period , 0 , -1 do
 ...
 if ZZ1.DATA:hasData(i) 
 and ZZ1.DATA[i]~=0
 and ZZ1.DATA[i]~=nil
 then
 
 if ZZ1.DATA[i]== Source.high[i] then
 Return=1;
 end
 
  if ZZ1.DATA[i]== Source.low[i] then
 Return=-1;
 end
...
for i= period , 0 , -1 do
	 ...
	 if ZZ2.DATA:hasData(i) 
	 and ZZ2.DATA[i]~=0
	 and ZZ2.DATA[i]~=nil
	 then
	 
	      if ZZ2.DATA[i]== Source.high[i] then
		 Return=1;
		 end
		 
		  if ZZ2.DATA[i]== Source.low[i] then
		 Return=-1;
		 end
...
function GetLastIndicatorValue(indiInstance, period)

    for i= period , 0 , -1 do
        if indiInstance.DATA:hasData(i) 
            and indiInstance.DATA[i]~=0
            and indiInstance.DATA[i]~=nil then
            return indiInstance.DATA[i];
        end
    end
...
function EntryFunction( now,period)
    ...
    if   DMI1.DIP[period]> DMI1.DIM[period]
	and DMI1.DIP[period-1]<= DMI1.DIM[period-1]
	and DMI2.DIP[period]> DMI2.DIM[period]
	and (not Use1 or (Use1 and Z1==-1))
	and (not Use2 or (Use2 and Z2==-1))
	and (not Use1 or not Use2 or (Use1 and Use2 and GetLastIndicatorValue(ZZ1, period) > GetLastIndicatorValue(ZZ2, period)))
	then	
          if Direction then
                BUY();
            else
		...
		Return=true;
    elseif   DMI1.DIP[period]< DMI1.DIM[period]
	and DMI1.DIP[period-1]>= DMI1.DIM[period-1]
	and DMI2.DIP[period]< DMI2.DIM[period]
	and (not Use1 or (Use1 and Z1==1))
	and (not Use2 or (Use2 and Z2==1))
