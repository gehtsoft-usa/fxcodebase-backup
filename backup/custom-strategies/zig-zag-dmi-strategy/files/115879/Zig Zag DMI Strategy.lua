...
function Find1(period)
    ...
    for i = period, 0, -1 do
        if ZZ1.DATA:hasData(i) and ZZ1.DATA[i] ~= 0 and ZZ1.DATA[i] ~= nil then
...
function Find2(period)
    ...
    for i = period, 0, -1 do
        if ZZ2.DATA:hasData(i) and ZZ2.DATA[i] ~= 0 and ZZ2.DATA[i] ~= nil then
...
function EntryFunction(now, period)
    ...
    if
        DMI1.DIP[period] > DMI1.DIM[period] and DMI1.DIP[period - 1] <= DMI1.DIM[period - 1] and
            DMI2.DIP[period] > DMI2.DIM[period] and
            (not Use1 or (Use1 and Z1 == -1)) and
            (not Use2 or (Use2 and Z2 == -1))
     then
        if Direction then
            BUY()
        else
