...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=59430
...
function Init() --The strategy profile initialization
    ...
    strategy.parameters:addInteger("RP", " Period", " Period", 16)
    ...
    strategy.parameters:addInteger("UP", " Period", " Period", 16)
    ...
    strategy.parameters:addInteger("MP", " Period", " Period", 32)
...
local BaseSize
local Method
local RP, Price1, Price2, Price3, UP, MP
...
function Prepare(nameOnly)
    ...
    name =
        name ..
        ", " .. Price1 .. ", " .. RP .. ", " .. Price2 .. ", " .. UP .. ", " .. Price3 .. ", " .. MP .. ", " .. Method

    assert(
        core.indicators:findIndicator("UNSMOOTHED RSI") ~= nil,
        "Please, download and install UNSMOOTHED RSI.LUA indicator"
    )
    ...
    Indicator[1] = core.indicators:create("RSI", Source[Price1], RP)
    Short[1] = Indicator[1]:getStream(0)

    Indicator[2] = core.indicators:create("UNSMOOTHED RSI", Source[Price2], UP)
    Short[2] = Indicator[2]:getStream(0)

    assert(core.indicators:findIndicator(Method) ~= nil, Method .. " indicator must be installed");
    Indicator[3] = core.indicators:create(Method, Source[Price3], MP)
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if
        (Short[1][period] <= 30 and core.crossesOver(Short[2], 30, period)) or
            (Short[2][period] <= 30 and core.crossesOver(Short[1], 30, period))
     then
        if Direction then
            BUY()
        else
    ...
    elseif
        (Short[1][period] >= 70 and core.crossesUnder(Short[2], 70, period)) or
            (Short[2][period] >= 70 and core.crossesUnder(Short[1], 70, period))
     then
        if Direction then
            SELL()
        else
    ...
    if core.crossesUnder(Source.close, Short[3], period) then
        if Direction then
    ...
    if core.crossesOver(Source.close, Short[3], period) then
        if Direction then
            if haveTrades("S") then
                exit("S")
                Signal("Close Short")
