...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=9651
...
function Init() --The strategy profile initialization
    strategy:name("ADX DMI CCI Strategy")
    ...
    strategy.parameters:addDouble("CL", "CCI Level", "", 0)

    strategy.parameters:addInteger("AP", "ADX Period", "", 14)
    strategy.parameters:addDouble("AL", "ADX Level", "", 0)
    strategy.parameters:addInteger("DP", "DMI Period", "", 14)
...
function Prepare(nameOnly)
    ...
    Indicator[1] = core.indicators:create("CCI", Source, CP)
    Short["CCI"] = Indicator[1]:getStream(0)

    Indicator[2] = core.indicators:create("ADX", Source, AP)
    Short["ADX"] = Indicator[2]:getStream(0)

    Indicator[3] = core.indicators:create("DMI", Source, DP)
    Short["DIP"] = Indicator[3]:getStream(0)
    Short["DIM"] = Indicator[3]:getStream(1)

    first = math.max(Short["CCI"]:first(), Short["ADX"]:first(), Short["DIP"]:first())
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if
        Short["ADX"][period] > AL and Short["DIP"][period] > Short["DIM"][period] and
            Short["DIM"][period] < Short["ADX"][period] and
            Short["DIP"][period] > Short["ADX"][period] and
            core.crossesOver(Short["CCI"], CL, period)
     then
        --sell when:
        --1.adx>entry level ,
        --2. dmi->dmi+,
    ...
    elseif
        Short["ADX"][period] > AL and Short["DIP"][period] < Short["DIM"][period] and
            Short["DIM"][period] > Short["ADX"][period] and
            Short["DIP"][period] < Short["ADX"][period] and
            core.crossesUnder(Short["CCI"], CL, period)
     then
        --and   Short["CCI"][period] <  CL
        if Direction then
            SELL()
    ...
    --exit buy :
    --1. dmi+ cross under dmi -,
    --2. cci cross under sell level

    --exit sell:

    --1. dmi- cross under dmi +,
    --2. cci cross over buy level

    if core.crossesUnder(Short["DIP"], Short["DIM"], period) or core.crossesUnder(Short["CCI"], CL, period) then
        if Direction then
    ...
    elseif core.crossesOver(Short["DIP"], Short["DIM"], period) or core.crossesOver(Short["CCI"], CL, period) then
        if Direction then
            if haveTrades("S") then
                exit("S")
                Signal("Close Short")
