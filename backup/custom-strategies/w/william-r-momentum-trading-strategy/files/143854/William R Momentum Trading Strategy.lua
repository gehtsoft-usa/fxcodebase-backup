...
-- https://fxcodebase.com/code/viewtopic.php?f=31&t=71544
...
function Init() --The strategy profile initialization
    strategy:name("William R Momentum Trading Strategy");
...
function CreateTradingParameters()
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "WRMTS");
...
function Prepare( nameOnly)
	...
	first=math.max(RLW.DATA:first(),MA.DATA:first()); 
...
function EntryFunction( id, now,period)
    ...
    if Source.close[period]> MA.DATA[period]
	and RLW.DATA[period]> RLW.DATA[period-1]
	and RLW.DATA[period-1] <= RLW.DATA[period-2]
	then	
          if Direction then
                BUY();
            else
