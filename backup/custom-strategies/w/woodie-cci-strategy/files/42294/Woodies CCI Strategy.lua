...
function Init() --The strategy profile initialization
    strategy:name("Woodies CCI Strategy")
    ...
    strategy.parameters:addInteger("CP", "CCI Period", "CCI Period", 50)
    strategy.parameters:addInteger("NP", "Neutral Period", "Neutral Period", 5, 1, 1000)
    strategy.parameters:addDouble("BL", "Buy Level", "Buy Level", 0)
    strategy.parameters:addDouble("SL", "Sell Level", "Sell Level", 0)
...
function TEST(Flag, period)
    ...
    if Flag then
        for i = period, period - NP, -1 do
            if Short[1][i] < BL then
                FLAG = false
            end
        end

        if FLAG and Short[1][period - NP - 1] < BL then
            return true
        end
    else
        for i = period, period - NP, -1 do
            if Short[1][i] > SL then
                FLAG = false
            end
        end

        if FLAG and Short[1][period - NP - 1] > SL then
            return true
        end
    end
end
