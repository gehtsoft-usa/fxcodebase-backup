...
function Init() --The strategy profile initialization
    ...
    strategy.parameters:addBoolean("Filter", "Use Adx Filter", "", true)
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if
        Indicator.DP[period] > Indicator.DN[period] and Indicator.DP[period - 1] <= Indicator.DN[period - 1] and
            (not Filter or (Indicator.ADX[period] > ADX_Level and Filter))
     then
        if Direction then
            BUY()
        else
