...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=64170
...
function Init() --The strategy profile initialization
    strategy:name("William VIX FIX Strategy")
    ...
    strategy.parameters:addInteger("Period", "Period", "Period", 22)
    strategy.parameters:addInteger("bbl", "Bolinger Band Length", "Bolinger Band Length", 20)
    strategy.parameters:addDouble("mult", "Bollinger Band Standard Devaition", "Bollinger Band Standard Devaition", 2)

    strategy.parameters:addDouble("lb", "Look Back Period", "Look Back Period", 50)
    strategy.parameters:addDouble("ph", "Highest Percentile", "Highest Percentile", 0.85)
    strategy.parameters:addDouble("pl", "Lowest Percentile", "Lowest Percentile", 1.01)
...
local Indicator
local Period, bbl, mult, lb, ph, pl
...
function Prepare(nameOnly)
    ...
    Hedge = instance.parameters.Hedge
    ...
    assert(
        core.indicators:findIndicator("WILLIAM VIX FIX") ~= nil,
        "Please, download and install WILLIAM VIX FIX indicator"
    )
...
function EntryFunction(period, now)
    ...
    if
        Indicator.rangeHigh[period] < Indicator.upperBand[period] and Source.close[period] > Source.open[period] and
            Indicator.WVF[period] > Indicator.rangeHigh[period] and
            Indicator.WVF[period - 1] <= Indicator.rangeHigh[period - 1]
     then
        if Direction then
            BUY()
        else
