-- https://fxcodebase.com/code/viewtopic.php?f=31&t=74009
...
function Init() --The strategy profile initialization
    strategy:name("Wildturkey Strategy");
    ...
    strategy.parameters:addGroup("ATR_Channels Calculation");
    strategy.parameters:addInteger("ATR_Period", "Period of ATR", "", 26);
    strategy.parameters:addInteger("MA_Period", "Period of MA", "", 16);
    ...
    strategy.parameters:addString("MA_Price", "Price of MA", "", "close");
    strategy.parameters:addStringAlternative("MA_Price", "close", "", "close");
    strategy.parameters:addStringAlternative("MA_Price", "open", "", "open");
    strategy.parameters:addStringAlternative("MA_Price", "high", "", "high");
    strategy.parameters:addStringAlternative("MA_Price", "low", "", "low");
    strategy.parameters:addStringAlternative("MA_Price", "median", "", "median");
    strategy.parameters:addStringAlternative("MA_Price", "typical", "", "typical");
    strategy.parameters:addStringAlternative("MA_Price", "weighted", "", "weighted");
    strategy.parameters:addDouble("Mult_Factor1", "Mult_Factor1", "", 1.6);
    strategy.parameters:addDouble("Mult_Factor2", "Mult_Factor2", "", 3.5);
    strategy.parameters:addDouble("Mult_Factor3", "Mult_Factor3", "", 4.8);
	
	
    strategy.parameters:addGroup("Stochastic Calculation");
    strategy.parameters:addInteger("S1", "K Period", "", 14);  	
    strategy.parameters:addInteger("S2", "D Period", "", 3);  	
    strategy.parameters:addInteger("S3", "D MA Period", "", 5);  
	
    strategy.parameters:addGroup("Cross Selection");	
	strategy.parameters:addBoolean("Cross1", "Stochastic Line Cross", "", true);
	strategy.parameters:addBoolean("Cross2", "ATR_Channels Line Cross", "", false);	
 
	
	strategy.parameters:addString("M1", "K MA Method", "Method" , "MVA");
    strategy.parameters:addStringAlternative("M1", "MVA", "MVA" , "MVA");	
    strategy.parameters:addStringAlternative("M1", "EMA", "EMA" , "EMA");
    strategy.parameters:addStringAlternative("M1", "FS", "FS" , "FS");
 

	strategy.parameters:addString("M2", "1. D MA Method", "Method" , "MVA");
    strategy.parameters:addStringAlternative("M2", "MVA", "MVA" , "MVA");
    strategy.parameters:addStringAlternative("M2", "EMA", "EMA" , "EMA"); 	
...
function CreateTradingParameters()
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "WTS");
...
function Prepare( nameOnly)
    ...
    Cross1 = instance.parameters.Cross1;
	Cross2 = instance.parameters.Cross2;
	...
	assert(core.indicators:findIndicator("ATR_CHANNELS") ~= nil, "Please, download and install ATR_CHANNELS.LUA indicator");    
    ...
    ATR_CHANNEL = core.indicators:create("ATR_CHANNELS", Source, 	ATR_Period, MA_Period, MA_Method, MA_Price, Mult_Factor1, Mult_Factor2, Mult_Factor3 );	
	Stochastic = core.indicators:create("STOCHASTIC", Source, instance.parameters.S1 , instance.parameters.S2, instance.parameters.S3,instance.parameters.M1,instance.parameters.M2);	 
	first=math.max(ATR_CHANNEL.DATA:first(),Stochastic.D:first()); 
...
function ExtUpdate(id, source, period)  -- The method called every time when a new bid or ask price appears.
   ...
   -- get only time
    ...
    ATR_CHANNEL:update(core.UpdateLast);
	if period <= first
	then
        return;
    end	
...
function EntryFunction( id, now,period)
    ...
    if   Stochastic.D[period] < 20
	and  Stochastic.K[period] > Stochastic.D[period]  
	and (Stochastic.K[period-1]<= Stochastic.D[period-1]or not Cross1)	
	and Source.close[period]< ATR_CHANNEL.LowerBand2[period]
	and (Source.close[period-1]>= ATR_CHANNEL.LowerBand2[period-1]or not Cross2)	
	then	
          if Direction then
                BUY();
            else
		...
		Return=true;
    elseif Stochastic.D[period] > 80
	and  Stochastic.K[period]< Stochastic.D[period]  
	and (Stochastic.K[period-1]>= Stochastic.D[period-1]  or not Cross1)	 
	and Source.close[period]> ATR_CHANNEL.UpperBand2[period]	
	and (Source.close[period-1]<= ATR_CHANNEL.UpperBand2[period-1] or not Cross2)	
	then
            if Direction then
                SELL();
            else
	...
	Sell Signal - if price closes above upper line 2 and Stochastics K and D line are above 80 and stochastic K line crosses below D line = Sell

     Buy Signal - if price closes below lower line 2 and stochastics K and D line are below 20 and stochastic K line crosses above d line = Buy.
