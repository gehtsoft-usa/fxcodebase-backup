...
function Init() --The strategy profile initialization
    strategy:name("Whistlers Active Volatility Energy Strategy");
	...
	strategy.parameters:addString("SignalType", " Signal / Trade Type", "", "Line");
    strategy.parameters:addStringAlternative("SignalType", "Line Cross", "", "Line");
    strategy.parameters:addStringAlternative("SignalType", "Middle", "", "Middle");
	
    strategy.parameters:addInteger("ShortBandsPeriod", "Short bands period", "", 14, 1, 300);
    strategy.parameters:addInteger("ShortBandsShift", "Short bands shift", "", 0, 0, 100);
    strategy.parameters:addDouble("ShortBandsDeviations", "Short bands deviations", "", 2.2, 0.00001, 100);

    strategy.parameters:addInteger("LongBandsPeriod", "Long bands period", "", 55, 1, 300);
    strategy.parameters:addInteger("LongBandsShift", "Long bands shift", "", 0, 0, 100);
    strategy.parameters:addDouble("LongBandsDeviations", "Long bands deviations", "", 2.2, 0.00001, 100);

    strategy.parameters:addInteger("Chars", "Characteristics", "", 100, 1, 300);
...
function CreateTradingParameters()
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "WAVES");
...
local first;
local Indicator;
local ShortBandsPeriod, ShortBandsShift, ShortBandsDeviations,LongBandsPeriod , LongBandsShift , LongBandsDeviations , Chars;
local Direction;
local CustomID;
local SignalType;
...
function Prepare( nameOnly)
    ...
    Indicator = core.indicators:create("WAVE-PM", Source.close, ShortBandsPeriod, ShortBandsShift, ShortBandsDeviations,LongBandsPeriod , LongBandsShift , LongBandsDeviations , Chars  );
	 
	first=math.max(Indicator.Long:first(),Indicator.Short:first()); 
...
function ExtUpdate(id, source, period)  -- The method called every time when a new bid or ask price appears.
   ...
   -- get only time
    ...
    if SignalType == "Line" then
    
			if   Indicator.Short[period] >  Indicator.Long[period]
			and  Indicator.Short[period-1] <=  Indicator.Long[period-1]
			then	
				  if Direction then
						BUY();
					else
			...
			elseif   Indicator.Short[period] <  Indicator.Long[period]
			and  Indicator.Short[period-1] >=  Indicator.Long[period-1]
			then
					if Direction then
						SELL();
			...
			if   Indicator.Short[period] >  0.5
			and  Indicator.Short[period-1] <=  0.5
			then	
				  if Direction then
						BUY();
					else
				...
				ONE= Source:serial(period);
			elseif   Indicator.Short[period] <  0.5
			and  Indicator.Short[period-1] >=  0.5
			then
					if Direction then
						SELL();
