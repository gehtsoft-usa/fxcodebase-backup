...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=64104
...
function Init() --The strategy profile initialization
    strategy:name("WADDAH ATTAR ATR TS STRATEGY")
    ...
    strategy.parameters:addGroup("ATR TS Calculation")
    ...
    strategy.parameters:addGroup("WADDAH ATTAR Calculation")
    ...
    strategy.parameters:addDouble("BLevel", "Buy Level", "", 0)
    strategy.parameters:addDouble("SLevel", "Sell Level", "", 0)
...
function Prepare(nameOnly)
    ...
    BLevel = instance.parameters.BLevel
    SLevel = instance.parameters.SLevel
    ...
    assert(core.indicators:findIndicator("ATRTS") ~= nil, "Please, download and install ATRTS.LUA indicator")
    assert(
        core.indicators:findIndicator("WADDAH_ATTAR_EXPLOSION") ~= nil,
        "Please, download and install WADDAH_ATTAR_EXPLOSION.LUA indicator"
    )
    ...
    ATR = core.indicators:create("ATRTS", Source1, AP, AM)
    WA = core.indicators:create("WADDAH_ATTAR_EXPLOSION", Source2, Sen)
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    WA:update(core.UpdateLast)

    if not ATR.DATA:hasData(period1) or not WA.TR:hasData(period2) or not WA.TG:hasData(period2) then
        return
    end
...
function EntryFunction(period1, period2, now)
    ...
    if
        Source1.close[period1] > ATR.DATA[period1] and Source1.close[period1 - 1] <= ATR.DATA[period1 - 1] and
            WA.TG[period2] > BLevel and
            WA.TG[period2] < WA.E[period2] and
            WA.TG[period2] == 0
     then
        if Direction then
            BUY()
    ...
    elseif
        Source1.close[period1] < ATR.DATA[period1] and Source1.close[period1 - 1] >= ATR.DATA[period1 - 1] and
            WA.TR[period2] > SLevel and
            WA.TR[period2] < WA.E[period2] and
            WA.TG[period2] == 0
     then
        if Direction then
            SELL()
