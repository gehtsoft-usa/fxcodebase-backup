...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=67345
...
function CreateParameters() 
    strategy.parameters:addInteger("bb1_Period", "BB Period", "", 20, 1, 10000);
    strategy.parameters:addDouble("bb1_Deviation", "BB Deviation", "", 2.0, 0.0001, 1000.0);
...
function CreateIndicators(source) 
    BB1 = core.indicators:create("BB", source, instance.parameters.bb1_Period, instance.parameters.bb1_Deviation);
    WR = core.indicators:create("RLW", source, instance.parameters.rlw_period);
end

function UpdateIndicators()
    BB1:update(core.UpdateLast);
    WR:update(core.UpdateLast);
end
...
function CreateCustomActions()
    ...
    exitLongAction.IsPass = function (source, period, periodFromLast)
        if not BB1.AL:hasData(period - 1) then
            return;
        end
        return core.crossesOver(source.close, BB1.AL, period);
    end
    ...
    exitShortAction.IsPass = function (source, period, periodFromLast)
        ...
        return core.crossesUnder(source.close, BB1.AL, period);
    end

    local enterLongAction = {};
    enterLongAction.ActOnSwitch = true;
    enterLongAction.IsPass = function (source, period, periodFromLast)
        if not BB1.TL:hasData(period - 1) or not WR.DATA:hasData(period - 1) then
            return;
        end
        return source.close[period] < BB1.TL[period] and WR.DATA[period] > -20;
    end

    local enterShortAction = {};
    enterShortAction.ActOnSwitch = true;
    enterShortAction.IsPass = function (source, period, periodFromLast)
        if not BB1.BL:hasData(period - 1) or not WR.DATA:hasData(period - 1) then
            return;
        end
        return source.close[period] > BB1.BL[period] and WR.DATA[period] < -80;
    end
