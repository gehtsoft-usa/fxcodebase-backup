...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=62793
...
function Prepare(nameOnly)
    ...
    assert(
        core.indicators:findIndicator("RB_CLEAR") ~= nil,
        "The RB_CLEAR.LUA indicator must be downloaded and installed"
    )
    ...
    Indicator = core.indicators:create("RB_CLEAR", Source)
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if Indicator.Up:hasData(period) and Indicator.Dn:hasData(period - 1) then
        if Direction then
            BUY()
        else
            SELL()
    ...
    elseif Indicator.Dn:hasData(period) and Indicator.Up:hasData(period - 1) then
        if Direction then
            SELL()
        else
            BUY()
