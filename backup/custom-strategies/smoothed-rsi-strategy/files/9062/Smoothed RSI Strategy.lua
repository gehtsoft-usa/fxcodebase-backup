...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=3731
...
function Prepare(nameOnly)
    ...
    assert(core.indicators:findIndicator("SMOOTHED RSI") ~= nil, "Please download and install SMOOTHED RSI Indicator!")
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if period < first or not Indicator.DATA:hasData(period - 1) then
        return
    end

    -- only buy if we have a fast cross over slow and the price is above the moving averages.
    if core.crossesOver(Indicator.DATA, BL, period) then
        if Direction then
            BUY()
        else
            SELL()
    ...
    elseif core.crossesUnder(Indicator.DATA, SL, period) then
        if Direction then
            SELL()
        else
            BUY()
