...
-- https://fxcodebase.com/code/viewtopic.php?f=31&t=71597
...
function Init() --The strategy profile initialization
    strategy:name("RVI BBands Strategy");
    ...
    strategy.parameters:addGroup("BBands Stop Calculation"); 
    ...
    strategy.parameters:addGroup("RVI Calculation");	
...
function CreateTradingParameters()
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "RBS");
...
function Prepare( nameOnly)
	...
	assert(core.indicators:findIndicator("BBANDS_STOP") ~= nil, "Please, download and install BBANDS_STOP.LUA indicator");          
	assert(core.indicators:findIndicator("ERVI") ~= nil, "Please, download and install ERVI.LUA indicator");        	
    ...
    BB = core.indicators:create("BBANDS_STOP",Source,  instance.parameters.Length, instance.parameters.Deviation, instance.parameters.MoneyRisk,1,1  );
    RVI = core.indicators:create("ERVI", Source,   instance.parameters.Frame  );	 
	first=math.max(BB.DATA:first(),RVI.DATA:first()); 
...
--	period3= core.findDate (Source3, TickSource:date(period), false);
	...
	--/////////////////////////////////////////////////////////////////////
		            ...
		            if  not BB.UP_L:hasData(period)
					and  BB.DN_L:hasData(period) 
					then
					  ...
					  if   not BB.DN_L:hasData(period) 
					  and BB.UP_L:hasData(period)
					  then
...
function EntryFunction( id, now,period)
    ...
    if  BB.UP_L:hasData(period)
	and RVI.DATA[period]>0
	then	
          if Direction then
                BUY();
            else
