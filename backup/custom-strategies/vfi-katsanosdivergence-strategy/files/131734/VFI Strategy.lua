...
local STRATEGY_NAME = "VFI Strategy";
...
function IsEntryLong(source, period)
    if not vfi.DATA:hasData(period - 1) then
        return false;
    end
    return core.crossesOver(vfi.DATA, 50, period);
end
function IsEntryShort(source, period)
    if not vfi.DATA:hasData(period - 1) then
        return false;
    end
    return core.crossesUnder(vfi.DATA, 50, period);
end
