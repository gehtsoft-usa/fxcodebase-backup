...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=69928
...
local STRATEGY_NAME = "BB Threshold Strategy";
...
local bb, threshold, last_stopped_position;
...
function CreateParameters()
    ...
    strategy.parameters:addInteger("threshold", "Threshold, pips", "", 10);
    ...
    strategy.parameters:addInteger("adx_level", "ADX level", "", 50);
end

function OnClosedTrade(closed_position)
    if tables_monitor.closing_order_types[closed_trade.CloseOrderID] == "S" then
        last_stopped_position = closed_trade.BS;
    else
        last_stopped_position = nil;
    end
end

local ma, adx, adx_level;
...
function CreateCustomActions()
    ...
    enterLongAction.IsPass = function (source, period, periodFromLast, data)
        if not bb.DATA:hasData(period - 1) then
            return false;
        end
        return (bb.BL[period] - source.close[period]) / source:pipSize() >= threshold
            and (bb.BL[period - 1] - source.close[period - 1]) / source:pipSize() < threshold
            and ma.DATA[period] < source.close[period]
            and adx.DATA[period] < adx_level
            and last_stopped_position ~= "B";
    end
    ...
    enterShortAction.IsPass = function (source, period, periodFromLast, data)
        ...
        return (source.close[period] - bb.TL[period]) / source:pipSize() >= threshold
            and (source.close[period - 1] - bb.TL[period - 1]) / source:pipSize() < threshold
            and ma.DATA[period] > source.close[period]
            and adx.DATA[period] < adx_level
            and last_stopped_position ~= "S";
    end
