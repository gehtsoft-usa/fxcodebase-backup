...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=62264
...
function Init() --The strategy profile initialization
    strategy:name("Higher Time Frame Open Price Overlay Strategy")
    ...
    strategy.parameters:addString("Higher", "HigherTime frame", "", "D1")
    strategy.parameters:setFlag("Higher", core.FLAG_PERIODS)
...
function Prepare(nameOnly)
    ...
    HigherSource = ExtSubscribe(3, nil, instance.parameters.Higher, instance.parameters.Type == "Bid", "bar")
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if Source.close[Source.close:size() - 1] > HigherSource.open[HigherSource.open:size() - 1] then
        if Direction then
            BUY()
        else
            SELL()
    ...
    elseif Source.close[Source.close:size() - 1] < HigherSource.open[HigherSource.open:size() - 1] then
        if Direction then
            SELL()
        else
            BUY()
