...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=24324
...
function Init() --The strategy profile initialization
    strategy:name("LeMan's Variation Strategy")
    ...
    strategy.parameters:addDouble("EB", "Exit Long", "", 0)
    strategy.parameters:addDouble("ES", "Exit Short", "", 0)
...
function Prepare(nameOnly)
    ...
    assert(core.indicators:findIndicator("LEMANVAR") ~= nil, "Please, download and install LemanVar.LUA indicator")
    ...
    Indicator[1] = core.indicators:create("LEMANVAR", Source[Price], Period)
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if core.crossesUnder(Short[1], EB, period) then
        if Direction then
    ...
    if core.crossesOver(Short[1], ES, period) then
        if Direction then
            if haveTrades("S") then
                exit("S")
                Signal("Close Short")
