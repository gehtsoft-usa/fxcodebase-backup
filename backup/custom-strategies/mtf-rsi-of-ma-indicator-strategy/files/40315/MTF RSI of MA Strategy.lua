...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=23417
...
function Init() --The strategy profile initialization
    strategy:name("MTF RSI of MA indicator Strategy");
...
function TIMEFRAME(id, FRAME)
	...
	strategy.parameters:addBoolean("ON".. id , "Use Confirmation", "", true);
	 strategy.parameters:addString("TF"..id, "Time frame", "", FRAME);
    strategy.parameters:setFlag( "TF"..id, core.FLAG_PERIODS);
	
	strategy.parameters:addInteger("MA_Period"..id, "MA_Period", "", 14);
    strategy.parameters:addInteger("RSI_Period"..id, "RSI_Period", "", 21);
    strategy.parameters:addInteger("MA_RSI_Period"..id, "MA_RSI_Period", "", 15);

	
	strategy.parameters:addDouble("CB"..id, "Buy Level", "", 50); 
	strategy.parameters:addDouble("CS"..id, "Sell Level", "", 50); 
	

	else	
    strategy.parameters:addGroup("Signal Time Frame");
    ...
    strategy.parameters:addInteger("MA_RSI_Period"..id, "MA_RSI_Period", "", 15);
		...
		--BUY ENTRY LEVEL 30
		--SELL ENTRY LEVEL 70
		strategy.parameters:addDouble("SB"..id, "Buy Level", "", 30); 
	  strategy.parameters:addDouble("SS"..id, "Sell Level", "", 70); 
		
		
		--BUY EXIT LEVEL 80
		--SELL EXIT LEVEL 20
		
		strategy.parameters:addDouble("BE"..id, "Buy Exit Level", "", 80); 
	  strategy.parameters:addDouble("SE"..id, "Sell Exit Level", "", 20); 
	end
...
function Prepare(nameOnly)
   ...
   assert(core.indicators:findIndicator("RSI_MA") ~= nil, "Please, download and install RSI_MA.LUA indicator");
	...
	SB[i]=instance.parameters:getDouble("SB"..i);
	SS[i]=instance.parameters:getDouble("SS"..i);
    
	BE[i]=instance.parameters:getDouble("BE"..i);
	SE[i]=instance.parameters:getDouble("SE"..i);	
	end
	TF[i]= instance.parameters:getString("TF"..i);
	
	RSI_Period[i]=  instance.parameters:getInteger("RSI_Period"..i);
    MA_Period[i]=  instance.parameters:getInteger("MA_Period"..i);
   MA_RSI_Period[i]=  instance.parameters:getInteger("MA_RSI_Period"..i);
    ...
    Indicator[i] = core.indicators:create("RSI_MA", Source[i][Price],MA_Period[i], RSI_Period[i],  MA_RSI_Period[i]  );
    Short1[i] = Indicator[i]:getStream(0);
	Short2[i] = Indicator[i]:getStream(1);
	 end
...
function ExtUpdate(id, source, period)  -- The method called every time when a new bid or ask price appears.
   ...
   -- get only time
	...
	for i = 1, TfNumber , 1 do	
		if Short1[i]:size() <= Short1[i]:first() + 2
		or Short2[i]:size() <= Short2[i]:first() + 2
		then
		   FLAG= false;
		end
	end
	...
	return;
		...
		--2. RSI CROSS OVER MA IN M15 AND RSI >BUY ENTRY LEVEL AND RSI < BUY EXIT LEVEL
		   ...
		   if core.crossesOver(Short1[1],Short2[1] , Short1[1]:size()-2 ,Short2[1]:size()-2)  	
            and  (Short1[1][ Short1[1]:size()-2] >  SB[1] )	
            and  (Short1[1][ Short1[1]:size()-2] <  BE[1] )			
		   and  (Short1[2][ Short1[2]:size()-2] > Short2[2][ Short2[2]:size()-2] or not  ON[2]) 
		    and  (Short1[2][ Short1[2]:size()-2] >  CB[2]  or not  ON[2])  
		   then
		...
		--2. RSI CROSS UNDER MA IN M15 AND RSI < SELL ENTRY LEVEL AND RSI> SELL EXIT LEVEL
		   ...
		   elseif core.crossesUnder(Short1[1],Short2[1] , Short1[1]:size()-2 ,Short2[1]:size()-2)  	
            and  (Short1[1][ Short1[1]:size()-2] <  SS[1] 	)
            and  (Short1[1][ Short1[1]:size()-2] >  SE[1] 	)		   
		    and  (Short1[2][ Short1[2]:size()-2] < Short2[2][ Short2[2]:size()-2] or not  ON[2]) 
			and  (Short1[2][ Short1[2]:size()-2] <  CS[2]  or not  ON[2])  
        ...
        -- EXIT BUY:
        if core.crossesUnder(Short1[1],Short2[1] , Short1[1]:size()-2 ,Short2[1]:size()-2)   then
        ...
        elseif core.crossesOver(Short1[1],Short2[1] , Short1[1]:size()-2 ,Short2[1]:size()-2)  		 then
		                        if   Direction then    
										if haveTrades("S") then
										   exit("S");
										   Signal ("Close Short");
