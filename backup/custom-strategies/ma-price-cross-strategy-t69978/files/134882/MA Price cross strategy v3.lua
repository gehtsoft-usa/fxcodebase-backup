...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=69978
-- More information about this indicator can be found at:
-- http://fxcodebase.com/code/viewtopic.php?f=27&t=69557
--+------------------------------------------------------------------+
...
local STRATEGY_NAME = "MA Price Cross Strategy";
...
function CreateParameters()
    ...
    AddAction(1, "Cross")
    AddAction(2, "Cross over")
    AddAction(3, "Cross over or touch")
    AddAction(4, "Cross under")
    AddAction(5, "Cross under or touch")
...
local ma, min_magrin, check_margin;
function CreateEntryIndicators(source)
    ...
    min_magrin = instance.parameters.min_magrin;
...
function MargingPass()
    ...
    return (account.UsableMargin / account.Equity) * 100 > min_magrin;
end
...
function CreateCustomActions()
    ...
    action1.IsPass = function (source, period, periodFromLast, data)
        if not ma.DATA:hasData(period - 1) or not MargingPass() then
            return;
        end
        return core.crosses(source.close, ma.DATA, period);
    end
    if isEntry1 then
    ...
    local action1, isEntry1 = CreateAction(2);
    action1.Data = nil;
    action1.ActOnSwitch = true;
    action1.AddLog = function (source, period, periodFromLast, data, values)
        --values["Buy value name"] = value;
    ...
    action1.IsPass = function (source, period, periodFromLast, data)
        ...
        return core.crossesOverOrTouch(source.close, ma.DATA, period);
    end
    if isEntry1 then
        EntryActions[#EntryActions + 1] = action1;
    else
    ...
    action1.IsPass = function (source, period, periodFromLast, data)
        ...
        return core.crossesUnderOrTouch(source.close, ma.DATA, period);
    end
    if isEntry1 then
        EntryActions[#EntryActions + 1] = action1;
    else
