...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=26326
...
function Init() --The strategy profile initialization
    strategy:name("Inverse Fisher Transform Stochastic Oscillator / CCI Strategy")
    ...
    strategy.parameters:addGroup(" IFTSO Calculation")
    ...
    strategy.parameters:addInteger("P0", "MA Period", "MA Period", 2)
    strategy.parameters:addInteger("P1", "Stochastic Period", "Stochastic Period", 30)
    strategy.parameters:addInteger("P2", "Slowing Period", "Slowing Period", 5)

    strategy.parameters:addDouble("B1", "Buy Level", "Buy Level", 20)
    strategy.parameters:addDouble("S1", "Sell Level", "Sell Level", 80)

    strategy.parameters:addGroup("CCI Calculation")

    strategy.parameters:addBoolean("Use", "Use CCI Filter", "", true)
    strategy.parameters:addInteger("CP", "Period", "Period", 200)
    strategy.parameters:addDouble("B2", "Buy Level", "Buy Level", 0)
    strategy.parameters:addDouble("S2", "Sell Level", "Sell Level", 0)

    strategy.parameters:addGroup("Exit")
    strategy.parameters:addBoolean("Exit", "Use Exit", "", true)
    strategy.parameters:addBoolean("E1", "Use IFTSO Exit", "", true)
    strategy.parameters:addBoolean("E2", "Use CCI Exit", "", true)
...
function Prepare(nameOnly)
    ...
    assert(core.indicators:findIndicator("IFTSO") ~= nil, "Please, download and install IFTSO.LUA indicator")
    ...
    Indicator[1] = core.indicators:create("IFTSO", Source[Price1], P0, P1, P2)
    Short[1] = Indicator[1]:getStream(0)

    Indicator[2] = core.indicators:create("CCI", Source, CP)
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if core.crossesOver(Short[1], B1, period) and (not Use or Short[2][period] > B2) then
        if Direction then
            BUY()
        else
            SELL()
        end
    elseif core.crossesUnder(Short[1], S1, period) and (not Use or Short[2][period] < S2) then
        if Direction then
            SELL()
        else
            BUY()
    ...
    if Exit then
        if (E1 and core.crossesUnder(Short[1], S1, period)) or (E2 and core.crossesUnder(Short[2], S2, period)) then
            if Direction then
        ...
        if (E1 and core.crossesOver(Short[1], B1, period)) or (E2 and core.crossesOver(Short[2], B2, period)) then
            if Direction then
                if haveTrades("S") then
                    exit("S")
                    Signal("Close Short")
