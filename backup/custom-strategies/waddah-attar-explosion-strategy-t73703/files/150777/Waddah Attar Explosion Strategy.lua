-- Available @ https://fxcodebase.com/code/viewtopic.php?f=31&t=73703
...
function Init() --The strategy profile initialization
    ...
    strategy.parameters:addInteger("Period", "ATR Period", "", 7);	
    strategy.parameters:addInteger("Period1", "Fast Period", "", 20);	
    strategy.parameters:addInteger("Period2", "Slow Period", "", 40);	
    strategy.parameters:addInteger("Period3", "BB Period", "", 20);	
    strategy.parameters:addInteger("Period4", "ATR Period", "", 100);
	strategy.parameters:addInteger("Sensitivity", "Sensitivity", "", 150); 
    strategy.parameters:addDouble("Multiplier", "Multiplier", "", 2);	
    strategy.parameters:addDouble("ATR_Multiplier", "ATR Multiplier", "", 3.7);
...
function CreateTradingParameters()
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "WAES");
...
function Prepare( nameOnly)
	...
	assert(core.indicators:findIndicator("WADDAH ATTAR EXPLOSION") ~= nil, "Please, download and install WADDAH ATTAR EXPLOSION.LUA indicator");          
    ...
    Indicator = core.indicators:create("WADDAH ATTAR EXPLOSION", Source,  Period,Period1, Period2, Period3, Period4,Sensitivity, Multiplier,ATR_Multiplier); 
	first=Indicator.DeadZoneLine:first(); 
...
--	period3= core.findDate (Source3, TickSource:date(period), false);
    ...
    --Signal for EXIT_BUY: Exit when green histo crosses below Explosion line.
		            ...
		            if   (Indicator.Trend[period] > 0 or Indicator.Trend[period-1] > 0 )
					and Indicator.Trend[period] < Indicator.ExplosionLine[period]
					then
...
function EntryFunction( id, now,period)
    ...
    if   Indicator.Trend[period] >  0
	and Indicator.Trend[period]> Indicator.Trend[period-1]
	and Indicator.Trend[period] >  Indicator.ExplosionLine[period]
	and Indicator.ExplosionLine[period]> Indicator.ExplosionLine[period-1]
	and Indicator.ExplosionLine[period] > Indicator.DeadZoneLine[period]
	and Indicator.Trend[period] > Indicator.DeadZoneLine[period]	
	then	
          if Direction then
                BUY();
            else
    ...
    elseif Indicator.Trend[period] <  0
	and Indicator.Trend[period]< Indicator.Trend[period-1]	
	and Indicator.Trend[period] >  Indicator.ExplosionLine[period]	
	and Indicator.ExplosionLine[period]> Indicator.ExplosionLine[period-1]	
	and Indicator.ExplosionLine[period] > Indicator.DeadZoneLine[period]
