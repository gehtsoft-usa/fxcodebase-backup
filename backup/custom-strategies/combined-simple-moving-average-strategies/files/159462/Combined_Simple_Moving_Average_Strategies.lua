-- Available @ https://fxcodebase.com/code/viewtopic.php?f=31&t=75992
...
local STRATEGY_NAME = "Combined Simple Moving Average Strategies";
...
function CreateParameters() 
    ...
    strategy.parameters:addInteger("x", "X", "", 0)
    strategy.parameters:addInteger("fast_period", "Fast Period", "", 7);
    AddAverages("fast_method", "Fast Method", "MVA");
    strategy.parameters:addInteger("slow_period", "Slow Period", "", 14);
    AddAverages("slow_method", "Slow Method", "MVA");
    strategy.parameters:addString("first_allowed_side", "First Allowed Side", "Set to Buy, Sell or Either", "Either");
    strategy.parameters:addStringAlternative("first_allowed_side", "Buy", "", "Buy");
    strategy.parameters:addStringAlternative("first_allowed_side", "Sell", "", "Sell");
    strategy.parameters:addStringAlternative("first_allowed_side", "Either", "", "Either");
end

local fast, slow, x, first_allowed_side, max_balance;
function CreateEntryIndicators(source)
    fast = CreateAverages(instance.parameters.fast_period, instance.parameters.fast_method, source);
    slow = CreateAverages(instance.parameters.slow_period, instance.parameters.slow_method, source);
    x = instance.parameters.x;
    first_allowed_side = instance.parameters.first_allowed_side;
    if trading:FindTrade():WhenInstrument(source:instrument()):Any() then
        first_allowed_side = "Either";
    end
    
    -- Load max_balance from persistent storage
	require("storagedb");
    db = storagedb.get_db("CSMAS");	
    max_balance = tonumber(db:get("max_balance", 0));
end
...
function UpdateIndicators()
    ...
    if account.Balance > max_balance then
        max_balance = account.Balance;
        -- Save to persistent storage
        db:put("max_balance", max_balance);
    end
end
...
function IsEntryLong(source, period)
    if first_allowed_side == "Sell" then
        return false;
    end
    if x == 0 then
        return core.crossesOver(fast.DATA, slow.DATA, period);
    end
    return RiseXBarsInARow(slow, period);
end
function IsEntryShort(source, period)
    if first_allowed_side == "Buy" then
        return false;
    end
    if x == 0 then
        return core.crossesUnder(fast.DATA, slow.DATA, period);
    end
    return FallXBarsInARow(slow, period);
end
...
function RiseXBarsInARow(ma, period)
    if x <= 0 then return false; end
    
    -- Check if we have enough bars to analyze
    if period < x then return false; end
    
    -- Check if MA has risen for X consecutive bars
    for i = 0, x - 1 do
        if ma.DATA[period - i] <= ma.DATA[period - i - 1] then
            return false;
        end
    end
    return true;
...
function FallXBarsInARow(ma, period)
    if x <= 0 then return false; end
    
    -- Check if we have enough bars to analyze
    if period < x then return false; end
    
    -- Check if MA has fallen for X consecutive bars
    for i = 0, x - 1 do
        if ma.DATA[period - i] >= ma.DATA[period - i - 1] then
            return false;
        end
