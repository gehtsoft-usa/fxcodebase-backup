...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=62594
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if
        Indicator.SL[period] > math.max(SA[period], SB[period]) and
            Indicator.SL[period - 1] <= math.max(SA[period - 1], SB[period - 1])
     then
        if Direction then
            BUY()
        else
    ...
    elseif
        Indicator.SL[period] < math.min(SA[period], SB[period]) and
            Indicator.SL[period - 1] >= math.min(SA[period - 1], SB[period - 1])
     then
        if Direction then
            SELL()
        else
    ...
    if Exit then
        if Indicator.SL[period] < math.max(SA[period], SB[period]) then
            if Direction then
        ...
        if Indicator.SL[period] > math.min(SA[period], SB[period]) then
            if Direction then
                if haveTrades("S") then
                    exitSpecific("S")
                    Signal("Close Short")
