...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=63290
...
function Init() --The strategy profile initialization
    strategy:name("TWO MA WITH MACD STRATEGY")
    ...
    strategy.parameters:addInteger("Period1", "Short period MA", "", 12)
    ...
    strategy.parameters:addInteger("Period2", "Short period MA", "", 26)
...
function Prepare(nameOnly)
    ...
    Period2 = instance.parameters.Perios2
    ...
    Fast = core.indicators:create(Method1, Source.close, Period1)
    assert(core.indicators:findIndicator(Method2) ~= nil, Method2 .. " indicator must be installed");
    Slow = core.indicators:create(Method2, Source.close, Period2)
    MACD = core.indicators:create("MACD", Source.close, SN, LN, IN)
    first = math.max(Fast.DATA:first(), Slow.DATA:first(), MACD.SIGNAL:first())
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    --[[
		...
		1.FAST MA>SLOW MA AND
		2.MACD> SIGNAL AND
		3. MACD> BUY LEVEL AND
		4. PRICE CROSS OVER FAST MA
		...
		2.MACD< SIGNAL AND
		3. MACD< SELL LEVEL
		4. PRICE CROSS UNDER FAST MA

	]]
    -- only buy if we have a fast cross over slow and the price is above the moving averages.
    if
        Fast.DATA[period] > Slow.DATA[period] and MACD.MACD[period] > MACD.SIGNAL[period] and MACD.MACD[period] > BL and
            Source.close[period] > Fast.DATA[period] and
            Source.close[period - 1] <= Fast.DATA[period - 1]
     then
        if Direction then
            BUY()
        else
    ...
    elseif
        Fast.DATA[period] < Slow.DATA[period] and MACD.MACD[period] < MACD.SIGNAL[period] and MACD.MACD[period] < SL and
            Source.close[period] < Fast.DATA[period] and
            Source.close[period - 1] >= Fast.DATA[period - 1]
     then
        if Direction then
    ...
    if Exit then
        if Fast.DATA[period] < Slow.DATA[period] then
            if Direction then
