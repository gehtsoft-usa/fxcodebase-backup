...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=62769
...
function Init() --The strategy profile initialization
    strategy:name("Speed of Trade Strategy");
    ...
    strategy.parameters:addGroup("Claculation");	
    strategy.parameters:addString("iType", "Candle Type", "", "OC");
    strategy.parameters:addStringAlternative("iType", "Open / Close", "", "OC");
    strategy.parameters:addStringAlternative("iType", "High / Low", "", "HL");
...
function CreateTradingParameters()
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "SOTS");
...
function Prepare( nameOnly)
	...
	iType=instance.parameters.iType;
	...
	assert(core.indicators:findIndicator("SOT") ~= nil, "Please, download and install SOT indicator");          
    ...
    SOT = core.indicators:create("SOT", Source,  iType,Period,Trigger  ); 
	first=SOT.AVG:first(); 
...
function ExtUpdate(id, source, period)  -- The method called every time when a new bid or ask price appears.
   ...
   -- get only time
    ...
    -- update indicators.
    SOT:update(core.UpdateLast);

	if period < first  then
        return;
    end
	
	if  SOT.SOT[period]  < (SOT.AVG[period] *Trigger)   then
	return;
	end
