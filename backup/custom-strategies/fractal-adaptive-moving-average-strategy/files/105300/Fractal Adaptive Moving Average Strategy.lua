...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=63253
...
function Init() --The strategy profile initialization
    strategy:name("Fractal Adaptive Moving Average Strategy")
    ...
    strategy.parameters:addString("Indicator", "Indicator Type", "", "FRAMA")
    strategy.parameters:addStringAlternative("Indicator", "FRAMA", "", "FRAMA")
    strategy.parameters:addStringAlternative("Indicator", "MODIFIED FRAMA", "", "MODIFIED FRAMA")

    strategy.parameters:addGroup("FRAMA Calculation")
    strategy.parameters:addInteger("Period", "Period", "", 56)

    strategy.parameters:addGroup("Modified Calculation")
    strategy.parameters:addInteger("First", "Frama Period", "Frama Period", 10, 2, 2000)
    strategy.parameters:addInteger("Fast", "Fast Frama Period", "Fast Period", 2, 2, 2000)
    strategy.parameters:addInteger("Slow", "Slow Frama Period", "Frama Period", 100, 2, 2000)
...
function Prepare(nameOnly)
    ...
    if Indicator == "FRAMA" then
        assert(core.indicators:findIndicator("FRAMA") ~= nil, "Please, download and install FRAMA.LUA indicator")
        MA = core.indicators:create("FRAMA", Source, Period)
    else
        assert(
            core.indicators:findIndicator("MODIFIED FRAMA") ~= nil,
            "Please, download and install MODIFIED FRAMA indicator"
        )
        MA = core.indicators:create("FRAMA", Source, First, Fast, Slow)
    end
