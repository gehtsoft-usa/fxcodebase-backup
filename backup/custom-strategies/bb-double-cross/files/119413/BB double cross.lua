...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=66161
-- Id: 
...
local Modules = {};
function Init()
    strategy:name("BB double cross");
    strategy:description("");

    strategy.parameters:addGroup("BB Calculation");
    strategy.parameters:addInteger("Period", "Period", "", 20, 1, 10000);
    strategy.parameters:addDouble("Deviation", "Deviation", "", 2.0, 0.0001, 1000.0);

    strategy.parameters:addBoolean("trade_all", "Trade all instruments", "", false);

    strategy.parameters:addDouble("distance", "Distance from TL/BL, pips", "", 100);
...
function Prepare(name_only)
    ...
    trade_all = instance.parameters.trade_all;
    if trade_all then
    ...
    if trade_all then
        ...
        while (row ~= nil) do
            indi = {};
            indi.source = ExtSubscribe(#indicators + 1, row.Instrument, instance.parameters.timeframe, instance.parameters.is_bid, "bar");
            indi.bb = core.indicators:create("BB", indi.source, instance.parameters.Period, instance.parameters.Deviation);
            indi.custom_id = profile:id() .. "_" .. indi.source:name();
            indicators[#indicators + 1] = indi;
            row = enum:next();
        end
    else
        indi = {};
        indi.source = ExtSubscribe(#indicators + 1, nil, instance.parameters.timeframe, instance.parameters.is_bid, "bar");
        indi.bb = core.indicators:create("BB", indi.source, instance.parameters.Period, instance.parameters.Deviation);
        indi.custom_id = profile:id() .. "_" .. indi.source:name();
        indicators[#indicators + 1] = indi;
    end
...
function ExtUpdate(id, source, period)
    ...
    if data == nil or not data.bb.TL:hasData(period - 1) then
        return;
    end
    data.bb:update(core.UpdateLast);
    if core.crossesOver(data.source.close, data.bb.TL, period) and core.crossesOver(data.source.close, data.bb.AL, period)
    and (data.source.close[period] - data.bb.TL[period]) / data.source:pipSize() >= distance
    then
        local current_serial = data.source:serial(period);
        if (data.last_serial ~= current_serial) then
            data.last_serial = current_serial;
            if (instance.parameters.close_on_opposite) then
                local trades = trading:FindTrade():WhenSide("B"):WhenCustomID(data.custom_id):All();
                for _, trade in ipairs(trades) do
                    trading:Close(trade);
                end
            end
            local command = trading:MarketOrder(data.source:instrument()):SetSide("S"):SetDefaultAmount():SetCustomID(data.custom_id);
            ...
            command = command:SetLimit(data.bb.TL[period], instance.parameters.trailing_stop and instance.parameters.trailing or nil);
            command:Execute();
        end
    end
    if core.crossesUnder(data.source.close, data.bb.BL, period) and core.crossesUnder(data.source.close, data.bb.AL, period)
    and (data.bb.BL[period] - data.source.close[period]) / data.source:pipSize() >= distance
    then
        local current_serial = data.source:serial(period);
        if (data.last_serial ~= current_serial) then
            data.last_serial = current_serial;
            if (instance.parameters.close_on_opposite) then
                local trades = trading:FindTrade():WhenSide("S"):WhenCustomID(data.custom_id):All();
                for _, trade in ipairs(trades) do
                    trading:Close(trade);
                end
            end
            local command = trading:MarketOrder(data.source:instrument()):SetSide("B"):SetDefaultAmount():SetCustomID(data.custom_id);
            if (instance.parameters.set_stop) then
                command = command:SetPipStop(nil, instance.parameters.stop, instance.parameters.trailing_stop and instance.parameters.trailing or nil);
            end
            command = command:SetLimit(data.bb.BL[period], instance.parameters.trailing_stop and instance.parameters.trailing or nil);
            command:Execute();
        end
    end
end
