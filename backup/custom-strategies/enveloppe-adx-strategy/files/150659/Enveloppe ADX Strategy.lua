-- Available @ https://fxcodebase.com/code/viewtopic.php?f=31&t=73669
...
function Init() --The strategy profile initialization
    strategy:name("Enveloppe ADX Strategy");
    ...
    strategy.parameters:addInteger("ADX_Period", "ADX Period", "", 40); 
    strategy.parameters:addDouble("TrendStrength", "ADX Trend  Strength", "", 10);
    strategy.parameters:addDouble("Percentage", "Enveloppe Percentage", "", 0.004);
...
function CreateTradingParameters()
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "EADXS");
...
function Prepare( nameOnly)
	...
	TrendStrength = instance.parameters.TrendStrength;
	...
	first=math.max(MA.DATA:first(),DMI.DATA:first()); 
...
function EntryFunction( id, now,period)
    ...
    local EnveloppeSup = MA.DATA[period]*(1 + Percentage )
	local EnveloppeInf = MA.DATA[period]*(1 - Percentage)
     
	  
    -- only buy if we have a fast cross over slow and the price is above the moving averages.
    if   ADX.DATA[period] > TrendStrength
	and  DMI.DIP[period]>  DMI.DIM[period]
	and  Source.high[period] > EnveloppeSup and Source.close[period] < EnveloppeSup
	then	
          if Direction then
                BUY();
            else
		...
		Return=true;
    elseif    ADX.DATA[period] > TrendStrength
	and  DMI.DIP[period]<  DMI.DIM[period]	
	and Source.low[period] < EnveloppeInf and Source.close[period] > EnveloppeInf
	then
            if Direction then
                SELL();
            else
