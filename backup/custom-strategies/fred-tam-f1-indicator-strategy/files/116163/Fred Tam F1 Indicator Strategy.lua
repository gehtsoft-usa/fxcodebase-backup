...
function Init()
    strategy:name("Fred Tam F1 Indicator Strategy")
    ...
    strategy.parameters:addGroup("Fred Tam F1 Indicator Parameters")

    strategy.parameters:addInteger("Buy", "Buy Signal Period", "", 4, 2, 2000)
    strategy.parameters:addInteger("Sell", "Sell Signal Period", "", 4, 2, 2000)

    strategy.parameters:addString("CrossType", "Cross Type", "", "High/Low")
    strategy.parameters:addStringAlternative("CrossType", "High/Low", "", "High/Low")
    strategy.parameters:addStringAlternative("CrossType", "Close", "", "Close")
...
function Prepare(nameOnly)
    ...
    first = gSource:first() + math.max(Buy, Sell) + 1
...
function ExtUpdate(id, source, period)
    ...
    if ExecutionType == "Live" and id == 1 then
        period = core.findDate(gSource.close, TickSource:date(period), false)
    end

    if ExecutionType == "Live" then
        if ONE == gSource:serial(period) then
            return
        end
    ...
    local min, max
    if CrossType == "High/Low" then
        max = mathex.max(gSource.high, period - Buy - 1, period - 1)
        min = mathex.min(gSource.low, period - Sell - 1, period - 1)
    else
        min = mathex.min(gSource.close, period - Sell - 1, period - 1)
        max = mathex.max(gSource.close, period - Buy - 1, period - 1)
    end

    if core.crossesOver(gSource.close, max, period) and FLAG ~= "LONG" then
        BUY()
        FLAG = "LONG"
        ONE = gSource:serial(period)
    end

    if core.crossesUnder(gSource.close, min, period) and FLAG ~= "SHORT" then
        SELL()
        FLAG = "SHORT"
        ONE = gSource:serial(period)
    end
