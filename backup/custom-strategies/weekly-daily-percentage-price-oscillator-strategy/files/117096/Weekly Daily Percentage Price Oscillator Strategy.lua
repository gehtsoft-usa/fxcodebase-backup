...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=65642
...
function Init() --The strategy profile initialization
    strategy:name("Weekly & Daily Percentage Price Oscillator Strategy");
    ...
    strategy.parameters:addInteger("Length1", "Length 1", "Period", 60);
	strategy.parameters:addInteger("Length2", "Length 2", "Period", 130);
	strategy.parameters:addInteger("Length3", "Length 3", "Period", 12);
	strategy.parameters:addInteger("Length4", "Length 4", "Period", 26);
...
function Prepare( nameOnly)
   ...
   assert(core.indicators:findIndicator("WEEKLY AND DAILY PERCENTAGE PRICE OSCILLATOR") ~= nil, "Please, download and install WEEKLY AND DAILY PERCENTAGE PRICE OSCILLATOR.LUA indicator");          
    ...
    Indicator = core.indicators:create("WEEKLY AND DAILY PERCENTAGE PRICE OSCILLATOR", Source.close, Length1, Length2, Length3, Length4 , true, true, false );    
	first=Indicator.DPP:first()+1; 
...
function EntryFunction( now,period)
    ...
    if  Indicator.WPP[period]< Indicator.DPP[period]
	and Indicator.WPP[period-1]>= Indicator.DPP[period-1]
	then	
          if Direction then
                BUY();
            else
		...
		Return=true;
    elseif  Indicator.WPP[period]> Indicator.DPP[period]
	and Indicator.WPP[period-1]<= Indicator.DPP[period-1]
	then
            if Direction then
                SELL();
