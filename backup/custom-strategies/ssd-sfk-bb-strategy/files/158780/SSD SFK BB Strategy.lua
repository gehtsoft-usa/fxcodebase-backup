-- Available @ https://fxcodebase.com/code/viewtopic.php?f=31&t=75758
...
function Init() --The strategy profile initialization
    strategy:name("SSD SFK BB Strategy");
	...
	strategy.parameters:addString("TradeDirection", "Trade Direction", "", "Long");
    strategy.parameters:addStringAlternative("TradeDirection", "Long", "", "Long");
	strategy.parameters:addStringAlternative("TradeDirection", "Short", "", "Short");
	strategy.parameters:addStringAlternative("TradeDirection", "Any", "", "Any");
	
	strategy.parameters:addString("TF1", "Time frame", "", "m5");
    strategy.parameters:setFlag("TF1", core.FLAG_PERIODS);
	
 	strategy.parameters:addInteger("SSD11", "K Period", "", 20);
 	strategy.parameters:addInteger("SSD12", "D Slowind Period", "", 10);	
 	strategy.parameters:addInteger("SSD13", "D Period", "", 10);
    ...
    strategy.parameters:setFlag("TF2", core.FLAG_PERIODS);
 	...
 	strategy.parameters:addInteger("SFK21", "K Period", "", 5);
 	strategy.parameters:addInteger("SFK22", "D Period", "", 5);
	
	
 	strategy.parameters:addInteger("BB21", "BB Period", "", 20);
 	strategy.parameters:addDouble("BB22", "BB Deviation", "", 0.75);	
    ...
    strategy.parameters:setFlag("TF3", core.FLAG_PERIODS);	
 	...
 	strategy.parameters:addInteger("SFK31", "K Period", "", 5);
 	strategy.parameters:addInteger("SFK32", "D Period", "", 5);	
...
function CreateTradingParameters()
    ...
    strategy.parameters:addString("CustomID", "Custom Identifier", "The identifier that can be used to distinguish strategy instances", "SSBS");
...
function Prepare( nameOnly)
	...
	--Indicator parameters
    SFK21 = instance.parameters.SFK21;
	SFK22 = instance.parameters.SFK22;
	SFK31 = instance.parameters.SFK31;
	SFK32 = instance.parameters.SFK32;
	SSD11 = instance.parameters.SSD11;
	SSD12 = instance.parameters.SSD12;
	SSD13 = instance.parameters.SSD13;
	BB21 = instance.parameters.BB21;
	BB22 = instance.parameters.BB22;
	
	TradeDirection = instance.parameters.TradeDirection;
    ...
    Source2 = ExtSubscribe(2, nil, TF2, instance.parameters.Type == "Bid", "bar");
    Source3 = ExtSubscribe(2, nil, TF3, instance.parameters.Type == "Bid", "bar");	
	
    SSD1 = core.indicators:create("SSD", Source1, SSD11, SSD12, SSD13  );	
    SKF2 = core.indicators:create("SFK", Source2, SKF21, SFK22  );
    SKF3 = core.indicators:create("SFK", Source3, SKF31, SFK32  );
	
	BB2=core.indicators:create("BB", Source2.close, BB21, BB22  );
...
function ExtUpdate(id, source, period)  -- The method called every time when a new bid or ask price appears.
    ...
    then	
		    ...
		    p1= core.findDate (Source1, TickSource:date(period), false);
		    p2= core.findDate (Source2, TickSource:date(period), false);
		    p3= core.findDate (Source3, TickSource:date(period), false);			
	else	
			if id ~= 2 then       
			return;
			end
		    ...
		    p1= period;
		    p2= core.findDate (Source2, Source1:date(period), false);
		    p3= core.findDate (Source3, Source1:date(period), false);			
	end
   ...
   -- get only time
    ...
    SKF2:update(core.UpdateLast);
    SKF3:update(core.UpdateLast);
    BB2:update(core.UpdateLast);	
	
	if not Source1.close:hasData( period)	
	or period < first 
	then
        return;
    end
    ...
    EntryFunction(now,period, p1,p2,p3);	
	end
	
	if ExitExecutionType== "Live" and id==1 
	or ExitExecutionType~= "Live" and id~=1 then
    ExitFunction(now,period, p1,p2,p3);
	end

end


function ExitFunction( now,period, p1,p2,p3)
      ...
      return ;
	   ...
	   if ( LastExit == Source1:serial(p1))   then
	 return;
	 end	  
	  
    

		            if    SSD1.DATA[p1]< 70 and TradeDirection~= "Short" then
								...
								LastExit= Source1:serial(p1);
					  end
					  if  SSD1.DATA[p1] > 30  and TradeDirection~= "Long" then
...
function EntryFunction( now,period, p1,p2,p3)
    ...
    local Return=false;
     ...
     if ( LastEntry == Source1:serial(p1) )   then
	 return;
	 end	
	...
	and  SKF3.DATA[p3]> 50
	and SSD1.DATA[p1]> 70
	and SSD1.DATA[p1-1]<= 70	
	and Source2.close[p2]> BB2.TL[p2]
	and Source2.close[p2] > Source2.close[p2-1]
	and TradeDirection~= "Short"
	then	
          if Direction then
                BUY();
            else
		...
		LastEntry= Source1:serial(p1);
		Return=true;
    elseif  SKF2.DATA[p2]< 50
	and  SKF3.DATA[p3]< 50
	and SSD1.DATA[p1]< 30
	and SSD1.DATA[p1-1]>= 30	
	and Source2.close[p2]< BB2.BL[p2]	
	and Source2.close[p2] < Source2.close[p2-1]	
	and TradeDirection~= "Long"	
	then
            if Direction then
                SELL();
            else
