...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=68644
...
function Init()
    strategy:name("Renko Fixed Trailing Stop");
...
function Prepare(nameOnly)
    ...
    Indicator = core.indicators:create("RENKO_CANDLES_NEW", nil, instance.bid:instrument(), instance.parameters.TF,
        instance.parameters.type, instance.parameters.Renko_Step, false)
...
function DoMoveStop(trade)
    if commands[trade.TradeID] ~= nil and not commands[trade.TradeID].Finished or last_date == Indicator.close:date(NOW) then
        return;
    end
    ...
    local newStop;
    if (trade.BS == "B") then
        if Indicator.close[NOW] < Indicator.open[NOW] then
            return;
        end
        newStop = stop.Rate + Indicator.close[NOW] - Indicator.open[NOW];
    else
        if Indicator.close[NOW] > Indicator.open[NOW] then
            return;
        end
        newStop = stop.Rate - Indicator.open[NOW] - Indicator.close[NOW];
    end
    commands[trade.TradeID] = trading:MoveStop(trade, newStop);
    last_date = Indicator.close:date(NOW);
end
...
function ExtUpdate(id, source, period)  -- The method called every time when a new bid or ask price appears.
    ...
    if Indicator.close:size() == 0 then
        return;
    end
