...
--http://fxcodebase.com/code/viewtopic.php?f=31&t=69824
...
local STRATEGY_NAME = "SSL Strategy";
function CreateParameters() 
    strategy.parameters:addInteger("N1", "Number of periods 1", "The number of periods.", 10, 2, 1000);
    strategy.parameters:addInteger("N2", "Number of periods 2", "The number of periods.", 20, 2, 1000);
end

local indi_1, indi_2;
function CreateEntryIndicators(source)
    local profile = core.indicators:findIndicator("SSL");
    assert(profile ~= nil, "Please, download and install " .. "SSL" .. ".LUA indicator");
    indi_1 = core.indicators:create("SSL", source, instance.parameters.N1);
    indi_2 = core.indicators:create("SSL", source, instance.parameters.N2);
end
...
function IsEntryLong(source, period)
    if not indi_2.DATA:hasData(period - 1) then
        return false;
    end
    return core.crossesOver(source.close, indi_2.DATA, period);
end
function IsEntryShort(source, period)
    if not indi_2.DATA:hasData(period - 1) then
        return false;
    end
    return core.crossesUnder(source.close, indi_2.DATA, period);
end
function IsExitLong(source, period)
    return indi_1.DATA[period] < source.close[period];
end
