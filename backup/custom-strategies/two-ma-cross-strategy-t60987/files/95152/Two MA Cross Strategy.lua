...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=60987
...
function Init() --The strategy profile initialization
    ...
    strategy.parameters:addInteger("Period1", "Short MA Periods", "", 25, 2, 1000)
    ...
    strategy.parameters:addInteger("Period2", "Long MA Periods", "", 50, 2, 1000)
...
function Prepare(nameOnly)
    ...
    first = math.max(short.DATA:first(), long.DATA:first()) + 1
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    local Up = short.DATA[period] > long.DATA[period]
    local Down = short.DATA[period] < long.DATA[period]

    local Top = Source.high[period] > short.DATA[period] and Source.close[period] < short.DATA[period]
    local Bottom = Source.low[period] < short.DATA[period] and Source.close[period] > short.DATA[period]

    if Up and Bottom then
        BUY()
        ONE = Source:serial(period)
    elseif Down and Top then
        SELL()
        ONE = Source:serial(period)
    end
end
