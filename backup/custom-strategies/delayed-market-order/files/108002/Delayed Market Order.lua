...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=63844
...
function Init()
    strategy:name("Delayed Market Order")
	strategy:setTag("NonOptimizableParameters", "Email,SendEmail,SoundFile,RecurrentSound,PlaySound,ShowAlert");
    strategy:description("")

    strategy.parameters:addGroup("Entry")
    strategy.parameters:addString("Direction", "Type of  Trade", "", "Buy")
    ...
    strategy.parameters:addString("ExitTime", "Entry Time", "", "23:59:00")
    strategy.parameters:addInteger("ValidInterval", "Valid interval for operation in second", "", 60)

    strategy.parameters:addBoolean("Exit", "Use Optional Exit", "", true)

    strategy.parameters:addString("ExitType", "Exit Type", "", "All")
    strategy.parameters:addStringAlternative("ExitType", "All", "", "All")
    strategy.parameters:addStringAlternative("ExitType", "Currency Pair", "", "Currency Pair")
    strategy.parameters:addStringAlternative("ExitType", "Custom Identifier", "", "Custom Identifier")
...
local Offer, Account, CanClose  -- Source;
local ExitTime, ValidInterval, ExitType
    ...
    while (not found) and (row ~= nil) do
        if
            row.AccountID == Account and
                ((row.OfferID == Offer and ExitType == "Currency Pair") or ExitType ~= "Currency Pair") and
                ((row.QTXT == CustomID and ExitType == "Custom Identifier") or ExitType ~= "Custom Identifier")
         then
            return true
        end
        row = enum:next()
...
function ExtAsyncOperationFinished(id, success, message)
    --[[if id == 100 then
				...
				if now >= ExitTime and now < ExitTime + ValidInterval
				then
									if haveTrades()  then
									CloseAll();
									end
...
function BUY()
    if AllowTrade then
        if CloseOnOpposite and haveMyTrades("S") then
...
function SELL()
    if AllowTrade then
        if CloseOnOpposite and haveMyTrades("B") then
            -- close on opposite signal
            exitSpecific("B")
            Signal("Close Long")
        end
