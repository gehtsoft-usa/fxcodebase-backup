...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=27987
...
function Init() --The strategy profile initialization
    strategy:name("Smoothed True Money Flow Index Strategy")
    ...
    strategy.parameters:addInteger("N", "Periods", "", 14, 1, 1000)

    strategy.parameters:addString("Method", "Smoothing Method", "Method", "MVA")
    ...
    strategy.parameters:addInteger("Period", "Smoothing Periods", "", 14, 1, 1000)

    strategy.parameters:addGroup("Trade Parameters")

    strategy.parameters:addBoolean("Confirmation", "Use Confirmation", "", false)
    strategy.parameters:addDouble("BL", "Buy Confirmation Level", "", 60)
    strategy.parameters:addDouble("SL", "Sell Confirmation Level", "", 40)
    strategy.parameters:addBoolean("Exit", "Use Optinal Exit", "", false)
    strategy.parameters:addString("ExitType", "Any", "", "Any")
    strategy.parameters:addStringAlternative("ExitType", "Any", "", "Any")
    strategy.parameters:addStringAlternative("ExitType", "TMFI/Signal", "", "One")
    strategy.parameters:addStringAlternative("ExitType", "TMFI/Levels", "", "Two")
...
function Prepare(nameOnly)
    ...
    assert(core.indicators:findIndicator("STMFI") ~= nil, "Please, download and install STMFI.LUA indicator")
    ...
    Indicator[1] = core.indicators:create("STMFI", Source, N, Method, Period)
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if (core.crossesOver(Short[1], Short[2], period)) and (Short[1][period] > BL or not Confirmation) then
        if Direction then
            BUY()
        else
            SELL()
        end
    elseif (core.crossesUnder(Short[1], Short[2], period)) and (Short[1][period] < SL or not Confirmation) then
        if Direction then
            SELL()
        else
            BUY()
    ...
    if Exit then
        if
            (core.crossesUnder(Short[1], Short[2], period) and ExitType ~= "Two") or
                (core.crossesUnder(Short[1], SL, period) and ExitType ~= "One")
         then
            if Direction then
        ...
        if
            (core.crossesOver(Short[1], Short[2], period) and ExitType ~= "Two") or
                (core.crossesOver(Short[1], BL, period) and ExitType ~= "One")
         then
            if Direction then
                if haveTrades("S") then
                    exit("S")
