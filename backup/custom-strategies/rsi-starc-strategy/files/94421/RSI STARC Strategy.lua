...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=60807
...
function Init() --The strategy profile initialization
    strategy:name("RSI STARC Strategy")
    ...
    strategy.parameters:addInteger("RP", "RSI Period", "RSI Period", 14)
    strategy.parameters:addInteger("AP", "ATR Period", "ATR Period", 14)
    strategy.parameters:addInteger("MP", "MA Period", "MA Period", 14)
    strategy.parameters:addDouble("TM", "Top Line Multiplier", "Top Line Multiplier", 2)
    strategy.parameters:addDouble("BM", "Bottom  Line Multiplier", "Bottom  Line Multiplier", 2)
...
function Prepare(nameOnly)
    ...
    assert(core.indicators:findIndicator("RSI STARC") ~= nil, "Please, download and install RSI STARC.LUA indicator")
    ...
    RSISTARC = core.indicators:create("RSI STARC", Source, RP, AP, MP, TM, BM)

    first = RSISTARC.DATA:first() + 1
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    -- update indicators.
    RSISTARC:update(core.UpdateLast)
    ...
    if core.crossesOver(RSISTARC.RSI, RSISTARC.TOP, period) then
        BUY()
        ONE = Source:serial(period)
    elseif core.crossesUnder(RSISTARC.RSI, RSISTARC.BOTTOM, period) then
        SELL()
        ONE = Source:serial(period)
    end
end
