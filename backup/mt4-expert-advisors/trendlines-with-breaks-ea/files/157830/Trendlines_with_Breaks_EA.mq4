//Available @  https://fxcodebase.com/code/viewtopic.php?f=38&t=75505
...
#define POSITION_CAP_FEATURE 
   ...
   string EA_NAME = "[Trendlines with Breaks EA]";
#endif
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      for (int i = 0; i < 100; ++i)
      {
         double value = iCustom(_symbol, _timeframe, "Trendlines_with_Breaks_[LuxAlgo]", length, k, method, 0, period + i);
         if (value != EMPTY_VALUE && value != 0)
         {
            return true;
         }
         value = iCustom(_symbol, _timeframe, "Trendlines_with_Breaks_[LuxAlgo]", length, k, method, 1, period + i);
         if (value != EMPTY_VALUE && value != 0)
         {
            return false;
         }
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      for (int i = 0; i < 100; ++i)
      {
         double value = iCustom(_symbol, _timeframe, "Trendlines_with_Breaks_[LuxAlgo]", length, k, method, 1, period + i);
         if (value != EMPTY_VALUE && value != 0)
         {
            return true;
         }
         value = iCustom(_symbol, _timeframe, "Trendlines_with_Breaks_[LuxAlgo]", length, k, method, 0, period + i);
         if (value != EMPTY_VALUE && value != 0)
         {
            return false;
         }
...
int OnInit()
{
   double temp = iCustom(NULL, 0, "Trendlines_with_Breaks_[LuxAlgo]", 0, 0);
   if (GetLastError() == ERR_INDICATOR_CANNOT_LOAD)
   {
      Alert("Please, install the 'Trendlines_with_Breaks_[LuxAlgo]' indicator");
      return INIT_FAILED;
   }
