...
//https://fxcodebase.com/code/viewtopic.php?f=38&t=72663
...
int checkTL(int bar)
  {
   string objName = "LTChannel_2_0_";
   double tlPriceCurr = ObjectGetValueByShift(objName, bar);
   double tlPricePrev = ObjectGetValueByShift(objName, bar + 1);
   double tlPrice0 = ObjectGetDouble(0, objName, OBJPROP_PRICE, 0);
   double tlPrice1 = ObjectGetDouble(0, objName, OBJPROP_PRICE, 1);
   if(Close[bar] > Open[bar] && Close[bar] > tlPriceCurr && Close[bar + 1] < tlPricePrev && tlPriceCurr > 0 && tlPrice0 > tlPrice1)
     {
      return 1;
     }
   if(Close[bar] < Open[bar] && Close[bar] < tlPriceCurr && Close[bar + 1] > tlPricePrev && tlPriceCurr > 0 && tlPrice0 < tlPrice1)
