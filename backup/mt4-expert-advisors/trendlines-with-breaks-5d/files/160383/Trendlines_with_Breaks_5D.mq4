//Available @  https://fxcodebase.com/code/viewtopic.php?f=38&t=76276
                ...
                const int &spread[])
{
   ...
   for (int pos = MathMin(bars_limit, rates_total - 1 - MathMax(prev_calculated - 1, toSkip)); pos >= 0 && !IsStopped(); --pos)
   {
      ...
      PlotShape::Set(plot3, pos, "absolute", (SafeGreater(upos[pos], upos[pos + 1]) ? low[pos] : EMPTY_VALUE), high, low, 0, upCss);
      if (pos + 1 > (rates_total - 1)) { continue; }
      PlotShape::Set(plot4, pos, "absolute", (SafeGreater(dnos[pos], dnos[pos + 1]) ? high[pos] : EMPTY_VALUE), high, low, 0, dnCss);
      if (pos + 1 > (rates_total - 1)) { continue; }
