...
input double First_Coeff=0.5;
input int First_Price=0;    // Applied price
                            // 0 - Close
                            // 1 - Open
                            // 2 - High
                            // 3 - Low
                            ...
                            // 5 - Typical
                            // 6 - Weighted  
input double Second_Coeff=0.3;
input int Second_Price=0;    // Applied price
                            // 0 - Close
                            // 1 - Open
                            // 2 - High
                            // 3 - Low
...
int OnInit()
{
     double temp = iCustom(NULL, 0, "Coral", 0, 0);
   if (GetLastError() == ERR_INDICATOR_CANNOT_LOAD)
   {
       Alert("Please, install the 'Coral' indicator");
       return INIT_FAILED;
   }
   LastBar=0;
...
void OnTick()
{
 ...
 double C1_0, C1_1, C2_0, C2_1;
 
 C1_0=iCustom(NULL, 0, "Coral", First_Coeff, First_Price, 0, 1);
 C1_1=iCustom(NULL, 0, "Coral", First_Coeff, First_Price, 0, 2);
 C2_0=iCustom(NULL, 0, "Coral", Second_Coeff, Second_Price, 0, 1);
 C2_1=iCustom(NULL, 0, "Coral", Second_Coeff, Second_Price, 0, 2);
 
 int CO;
 int res;
 double SL, TP;
 
 if (C1_1<C2_1 && C1_0>C2_0)
 {
  _Alert("Buy");
  CO=CalculateOrders();
  if (CO<=0)
