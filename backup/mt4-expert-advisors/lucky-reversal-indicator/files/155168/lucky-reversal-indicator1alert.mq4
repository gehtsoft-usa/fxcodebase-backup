//Available @  https://fxcodebase.com/code/viewtopic.php?f=38&p=155114#p155114
...
int init()
  {
   ...
   SetIndexBuffer(4, Up);
   SetIndexStyle(4, DRAW_ARROW, EMPTY, 7); //Green up arrow
   SetIndexArrow(4, 233);
   SetIndexEmptyValue(4, 0.0);
   SetIndexBuffer(5, Dn);
   SetIndexStyle(5, DRAW_ARROW, EMPTY, 7); //Red down arrow
   SetIndexArrow(5, 234);
...
void checkAlert()
  {
   ...
   if(nb)
      alerted = false;
   if(notificationsOn == true && !alerted)
     {
      if(Up[0] > 0 && Up[0] != EMPTY_VALUE)
        {
         Notify(1);
         alerted = true;
        }
      if(Dn[0] > 0 && Dn[0] != EMPTY_VALUE)
        {
         Notify(2);
         alerted = true;
        }
