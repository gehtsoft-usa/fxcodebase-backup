...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=75247
...
input const string indicator_file = "Extreme_Spike"; // Indicator File:
//---
int  bufferToBuy              = 1;
int  bufferToSell             = 0;
bool indicator_init           = true;
input int  candles_back_for_signals = 5;
//---
int  handle = 0;
void setHandle() { handle = iCustom(Symbol(), NULL, indicator_file, 
    NoRepaint,
    MinorMinExtremeHeightATRs,
    MajorToMinorHeightRatio,
    MinorMinExtremeWidth,
    MajorMinExtremeWidth,
    AlertMajorBottomEnabled,
    AlertMajorTopEnabled,
    AlertMinorTopEnabled,
  ...
  public:
    bool evaluate()
    {
        for (int i = 1; i <= candles_back_for_signals; i++) {
            ...
            if (indi == -1 && indi != EMPTY_VALUE && indi != lastSignal) {
                lastSignal = iTime(NULL, 0, i);
                Print("line: ",__LINE__," lastSignal: ",lastSignal);
                return true;
            }
        }
        return false;
