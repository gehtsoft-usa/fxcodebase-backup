...
void DrawLine(const string id, const double rate1, const int pos1, const double rate2, const int pos2, const color clr)
{
   ...
   if (show_leg_volume_total)
      label = label + "Total volume: " + IntegerToString((int)vol[pos2]) + ";";
   if (show_leg_volume_avg)
      label = label + "Avg volume: " + IntegerToString((int)MathCeil(vol[pos2] / (pos1 - pos2))) + ";";
   if (show_time)
      label = label + "Time: " + IntegerToString((int)MathCeil((Time[pos2] - Time[pos1]) / 60 )) + ";";
   ...
   switch (show_leg_size)
   {
      case DoNotShow:
         break;
      case ShowPips:
         label = label + "Leg size: " + DoubleToStr(MathAbs(Low[pos1] - High[pos2]) / pipSize, 1) + " pips;";
         break;
      case ShowBars:
         label = label + "Leg size: " + IntegerToString(pos1 - pos2) + " bars;";
         break;
   }
   if (label != "")
   {
