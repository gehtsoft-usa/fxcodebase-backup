...
input int volume_divisor = 10; // Volume divisor
...
int start()
{
   ...
   while (pos >= 0)
   {
      ...
      else
      {
         ...
         string id = IndicatorObjPrefix + "idValue" + TimeToString(Time[pos + 1]);
         ObjectCreate(0, id, OBJ_TEXT, 0, Time[pos + 1], wave[pos] == 1 ? Low[pos + 1] : High[pos + 1]);
         ObjectSetString(0, id, OBJPROP_TEXT, DoubleToString(vol[pos + 1] / volume_divisor, 0));
         ObjectSetString(0, id, OBJPROP_FONT, "Arial");
         ObjectSetInteger(0, id, OBJPROP_FONTSIZE, 12);
         ObjectSetInteger(0, id, OBJPROP_COLOR, labels_color);
         ObjectSetInteger(0, id, OBJPROP_ANCHOR, wave[pos] == 1 ? ANCHOR_UPPER : ANCHOR_LOWER);
      }
   ...
   if (show_current_volume)
   {
      ...
      ObjectCreate(0, id, OBJ_TEXT, 0, Time[0], wave[0] == 1 ? Low[0] : High[0]);
      ObjectSetString(0, id, OBJPROP_TEXT, DoubleToString(vol[0] / volume_divisor, 0));
      ObjectSetString(0, id, OBJPROP_FONT, "Arial");
      ObjectSetInteger(0, id, OBJPROP_FONTSIZE, 12);
      ObjectSetInteger(0, id, OBJPROP_COLOR, labels_color);
      ObjectSetInteger(0, id, OBJPROP_ANCHOR, wave[0] == 1 ? ANCHOR_UPPER : ANCHOR_LOWER);
      ObjectMove(0, id, 0, Time[0], wave[0] == 1 ? Low[0] : High[0]);
   }
   return(0);
}
