...
public:
   ...
   {
      double buffer[1];
      if (CopyBuffer(wt, 3, period, 1, buffer) == 1)
      {
         return false;
      }
...
public:
   BBLongCondition(const string symbol, ENUM_TIMEFRAMES timeframe)
      :ACondition(symbol, timeframe)
   {
      _indi = iBands(_symbol, _timeframe, bb_period, bb_deviation, bb_shift, PRICE_CLOSE);
   }
   ...
   {
      double buffer[1];
      if (CopyBuffer(_indi, UPPER_BAND, period, 1, buffer) == 1)
      {
         return false;
      }
      double bbValue = buffer[0];
...
public:
   ...
   {
      double buffer[1];
      if (CopyBuffer(wt, 2, period, 1, buffer) == 1)
      {
         return false;
      }
      double value = buffer[0];
...
public:
   ...
   {
      double buffer[1];
      if (CopyBuffer(_indi, LOWER_BAND, period, 1, buffer) == 1)
      {
         return false;
      }
      double bbValue = buffer[0];
...
public:
   ...
   {
      double buffer[1];
      if (CopyBuffer(wt, 0, period, 1, buffer) == 1)
      {
         return false;
      }
      double value = buffer[0];
...
int OnInit()
{
   wt = iCustom(_Symbol, (ENUM_TIMEFRAMES)_Period, "WilliamsTrade", wperiod, short_level, long_level, ma_period, valid_bars_back);
