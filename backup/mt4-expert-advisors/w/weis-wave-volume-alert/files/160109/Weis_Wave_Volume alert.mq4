//Available @  https://fxcodebase.com/code/viewtopic.php?f=38&t=76208
...
enum Method
{
   ...
   MethodPart // Part of Price
};
enum PriceSource
{
   PriceClose, // Close
   PriceOpenClose, // Open / Cloe
   PriceHighLow // High / Low
};
input Method      method             = MethodATR; // Renko Assignment Method
input double      methodvalue        = 14.0; // Value
input PriceSource pricesource        = PriceClose; // Price Source
input bool        useTrueRange       = true; // Use True Range instead of Volume
input bool        isOscillating      = false; // Oscillating
input bool        normalize          = false; // Normalize
input color       UpHistogramColor   = clrGreen;
input color       DownHistogramColor = clrRed;
input int         HistogramWidth     = 1;
// Stream v.3.0
...
int init()
{
   ...
   IndicatorShortName("Weis Wave Volume");

   IndicatorBuffers(7);

   out.RegisterStream(0, UpHistogramColor, "", DRAW_HISTOGRAM,STYLE_SOLID,HistogramWidth);
   out.RegisterStream(1, DownHistogramColor, "", DRAW_HISTOGRAM,STYLE_SOLID,HistogramWidth);
 
   out.RegisterInternalStream(2);
   SetIndexEmptyValue(0, EMPTY_VALUE);
   SetIndexEmptyValue(1, EMPTY_VALUE);
   SetIndexEmptyValue(2, EMPTY_VALUE);

   SetIndexStyle(5, DRAW_NONE);
   SetIndexBuffer(5, currclose);
                ...
                const int &spread[])
{
   if (prev_calculated <= 0 || prev_calculated > rates_total)
   {
      ...
      ArrayInitialize(currclose, 0);
      ArrayInitialize(direction, 0);
      ArrayInitialize(vol, 0);
      ArrayInitialize(barcount, 1);
   }
   ...
   for (int pos = rates_total - 1 - MathMax(prev_calculated, toSkip); pos >= 0 && !IsStopped(); --pos)
   {
      ...
      out.Set(isOscillating && direction[pos] < 0 ? -res : res, pos, direction[pos] > 0 ? 0 : 1);
   }
...
void checkAlert()
  {
   ...
   if(notificationsOn == 1 && !alerted)
     {
      if(out._streams[0].Stream[0] > 0 &&  out._streams[0].Stream[0] != EMPTY_VALUE && out._streams[1].Stream[0] == EMPTY_VALUE && out._streams[1].Stream[1] != EMPTY_VALUE)
        {
         Notify(1);
         alerted = true;
        }
      if(out._streams[1].Stream[0] > 0 &&  out._streams[1].Stream[0] != EMPTY_VALUE && out._streams[0].Stream[0] == EMPTY_VALUE && out._streams[0].Stream[1] != EMPTY_VALUE)
        {
         Notify(2);
         alerted = true;
        }
   ...
   if(notificationsOn == 2 && nb)
     {
      if(out._streams[0].Stream[1] > 0 &&  out._streams[0].Stream[1] != EMPTY_VALUE && out._streams[1].Stream[1] == EMPTY_VALUE && out._streams[1].Stream[2] != EMPTY_VALUE)
        {
         Notify(11);
        }
       
      if(out._streams[1].Stream[1] > 0 &&  out._streams[1].Stream[1] != EMPTY_VALUE && out._streams[0].Stream[1] == EMPTY_VALUE && out._streams[0].Stream[2] != EMPTY_VALUE)
        {
         Notify(22);
        }
     }
