...
//--- indicator buffers mapping
   ...
   IndicatorName = GenerateIndicatorName("WeisWaveRM4_avg_volume");
                ...
                const int &spread[])
{
   ...
   for(int i = limit; i>=0; i--)
   {
      ...
      if (trendDirection[i] != waveDirection[i+1] && waveDifference >= Difference) 
      {
         ...
         for (int k = waveChangeBar - 1; k>=i; k--) 
         {
            ...
            if (waveDirection[i] == -1) {
               upVolumeBuffer[k] = 0;
               dnVolumeBuffer[k] = volumeTracker[k] / volumeCount[k];
         ...
         if (ShowVolumeLabels == true)
         {
            ...
            {
               if (prevVolLabelSide == waveDirection[i])
                  ObjectDelete(0, prevVolLabel);
               string lastVolLabel = IndicatorObjPrefix + TimeToString(time[waveChangeBar], TIME_DATE|TIME_MINUTES) + "-VOLD";
               ...
               prevVolLabelSide = (int)waveDirection[i];
               prevVolLabel = lastVolLabel;
            }
            else if (waveDirection[i] == -1 && upPipBuffer[waveChangeBar] > 0)
            {
               if (prevVolLabelSide == waveDirection[i])
                  ObjectDelete(0, prevVolLabel);
               string lastVolLabel = IndicatorObjPrefix + TimeToString(time[waveChangeBar], TIME_DATE|TIME_MINUTES) + "-VOLU";
      ...
      if (waveDirection[i] == -1) {
         upVolumeBuffer[i] = 0;
         dnVolumeBuffer[i] = volumeTracker[i] / volumeCount[i];
         upPipBuffer[i]=0;
   ...
   if (ShowVolumeLabels == true) {
      ...
      if (ObjectFind(0, IndicatorObjPrefix) >= 0)
      {
         ObjectSet(IndicatorObjPrefix, OBJPROP_TIME1, time[0]);
         ObjectSet(IndicatorObjPrefix, OBJPROP_PRICE1, price);
      }
      else
      {
         ObjectCreate(0, IndicatorObjPrefix, OBJ_TEXT, 0, time[0], price);
         ObjectSet(IndicatorObjPrefix, OBJPROP_ANGLE, 0);
         ObjectSet(IndicatorObjPrefix, OBJPROP_ANCHOR, ANCHOR_LEFT);
      }
      ObjectSetText(IndicatorObjPrefix, StringConcatenate(DoubleToString(MathAbs(dnVolumeBuffer[0]-upVolumeBuffer[0])/1000,1), "K"), FontSize, NULL, FontColorNow);
   }  
