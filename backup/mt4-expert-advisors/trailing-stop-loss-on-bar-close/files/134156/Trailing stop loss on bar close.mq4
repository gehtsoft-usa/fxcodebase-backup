...
input int ticket_id = 0; // Ticket id
...
TrailingPipsAction* action;
OrderByTicketId* order;

int OnInit()
{
   order = new OrderByTicketId(ticket_id);
...
void OnTick()
{
   if (action == NULL)
   {
      ...
      action = new TrailingPipsAction(order, distance, trailing_step);
      last_date = iTime(_Symbol, _Period, 0);
   }
   if (last_date != iTime(_Symbol, _Period, 0))
   {
      last_date = iTime(_Symbol, _Period, 0);
      action.DoAction(0, last_date);
   }
}