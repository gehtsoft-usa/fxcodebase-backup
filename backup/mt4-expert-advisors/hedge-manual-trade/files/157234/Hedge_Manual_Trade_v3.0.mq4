...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=75105
...
class PositionsMannagement
{
  ...
  public:
    ...
    void set(string sym = NULL, int magi = 0)
    {
        _symbol = sym;
        _magic  = magi;
    }
        ...
        for (int i = OrdersTotal() - 1; i >= 0; i--) {
            if (OrderSelect(i, SELECT_BY_POS) && OrderSymbol() == _Symbol) {
                if(OrderType() != OP_BUY && OrderType() != OP_SELL)
                    int r = OrderDelete(OrderTicket(), clrBlack);
            }
        }
        return true;
    }
};
ActionDeletePendings acDeletePendings;
...
ModeEntry    modeEntry   = PendingStop;         // Mode Entry:
input int    magico      = 1982;                // Magic Number:
int          candlesback = 10;                  // candles back to search high / low:

#ifdef pending_orders_on
input double uHedgeDistance  = 20;   // Pips distance to Hedge Order:
input bool   uHedgeUnlimited = true; // Hedge Unlimited:
#endif

// MARK: funcion Price
double Price(string side, int pips = 0, string _symbol = "", bool hedge = false)
{
    ...
    if (!hedge) {
        if (side == "buy") {
            ...
            case PendingStop:
                // price += distance;
                price = iHigh(NULL, 0, iHighest(NULL, 0, MODE_HIGH, candlesback, 1));
                break;
            }
        }
        ...
        if (side == "sell") {
            ...
            case PendingStop:
                // price -= distance;
                price = iLow(NULL, 0, iLowest(NULL, 0, MODE_LOW, candlesback, 1));
                break;
            }
        }
    }
...
enum_lot_mode lot_mode   = multiply_original;           // Lot Calculation Mode
input double  m          = 2;                           // Original lot Multiplier:
...
double LotsCalculation()
{
    ...
    case multiply_original:
        ...
        lots = NormalizeDouble(originalLots * m, 2);
        break;
    }
    //
    return lots;
...
class OrderCreator : public iActions
{
  ...
  public:
    OrderCreator(const string Symbol, const string Side, const int Magic, bool hedge = false)
    {
        _symbol = Symbol;
        _side   = Side;
        _magic  = Magic;
    ...
    void setPrice() { _price = ::Price(_side, uHedgeDistance, NULL, _hedge); }
    ...
    bool execute()
    {
        ...
        string com = _hedge ? "hedge" : "";

        int result = OrderSend(_symbol, _cmd, _lots, _price, _slippage, _sl, _tp, com, _magic, 0, clrNONE);
...
OrderCreator buyhedge(_Symbol, "buy", magico, true);
OrderCreator sellhedge(_Symbol, "sell", magico, true);

#endif

#define Section_Hedge
#ifdef Section_Hedge
// Mark: Section_Hedge

class ConditionNewTradeBuy : public iConditions
{
    int lastCount;
  ...
  public:
    bool evaluate()
    {
        ...
        for (int i = OrdersTotal() - 1; i >= 0; i--) {
            if (OrderSelect(i, SELECT_BY_POS) && OrderSymbol() == _Symbol) {
                if (OrderType() == OP_BUY && OrderComment() == "") count++;
            }
        }
...
ConditionNewTradeBuy cdNewTradeBuy;
class ConditionNewTradeSell : public iConditions
{
    int lastCount;
...
ConditionNewTradeSell cdNewTradeSell;

Strategy stHedgeBuy;
Strategy stHedgeSell;

void OnInit_Hedge()
{
    stHedgeSell.set(&cdNewTradeSell, &buyhedge);
    stHedgeBuy.set(&cdNewTradeBuy, &sellhedge);

    Trader.strategys.add(&stHedgeBuy);
    Trader.strategys.add(&stHedgeSell);
}

#endif

#define Section_hedgeUnlimited
#ifdef Section_hedgeUnlimited
// Mark: Section_hedgeUnlimited

class ConditionhedgeUnlimited : public iConditions
{
  public:
    bool evaluate()
    {
        ...
        for (int i = OrdersTotal() - 1; i >= 0; i--) {
            if (OrderSelect(i, SELECT_BY_POS, MODE_TRADES) && OrderSymbol() == _Symbol) {
                ...
                string          order_com   = OrderComment();
                int             esHedge     = StringFind(order_com, "hedge", 0);                
                if (esHedge >= 0) continue;

                bool            haveHedge   = false;
                string          original_tk = (string)OrderTicket();
                ENUM_ORDER_TYPE ot          = OrderType();
                double          order_price = OrderOpenPrice();
                double          order_lots  = OrderLots();

                for (int j = OrdersTotal() - 1; j >= 0; j--) {
                    if (OrderSelect(j, SELECT_BY_POS, MODE_TRADES) && OrderSymbol() == _Symbol) {
                        string comi = OrderComment();
                        if (StringFind(comi, original_tk, 0) >= 0) {
                            haveHedge = true;
                            break;
                        }
                    }
                ...
                if (!haveHedge) {
                    string com      = "hedge " + original_tk;
                    double distance = uHedgeDistance * 10 * _Point;
                    double h_lots = m > 0 ? order_lots * m : order_lots;

                    if (ot == OP_BUY) {
                        double _price = order_price - distance;
                        double sl = StopLoss(_price, "sell", _Symbol, h_lots); 
                        double tp = TakeProfit(_price, "sell", _Symbol, h_lots); 
                        int r = OrderSend(NULL, OP_SELLSTOP, h_lots, order_price - distance,0, sl, tp, com, 0, 0, clrMagenta);
                    }
                    if (ot == OP_SELL) {
                        double _price = order_price - distance;
                        double sl = StopLoss(_price, "buy", _Symbol, h_lots); 
                        double tp = TakeProfit(_price, "buy", _Symbol, h_lots); 
                        int r = OrderSend(NULL, OP_BUYSTOP, h_lots, order_price + distance, 0, sl, tp, com, 0, 0, clrMagenta);
                    }
                }
            }
        }
...
ConditionhedgeUnlimited cdhedgeUnlimited;

class ActionhedgeUnlimited : public iActions
{
  public:
    bool execute()
    {
        // string com      = "hedge " + (string)OrderTicket();
        // double distance = uHedgeDistance * 10 * _Point;
        // double pr       = OrderType() == OP_BUY ? OrderOpenPrice() - distance : OrderOpenPrice() + distance;

        // if (OrderType() == OP_BUY) {
        //     int r = OrderSend(NULL, OP_SELLSTOP, OrderLots(), pr, 0, 0, 0, com, 0, 0, clrMagenta);
        // }
        // if (OrderType() == OP_SELL) {
        //     int r = OrderSend(NULL, OP_BUYSTOP, OrderLots(), pr, 0, 0, 0, com, 0, 0, clrMagenta);
        // }

        return true;
    }
};
ActionhedgeUnlimited achedgeUnlimited;

Strategy sthedgeUnlimited;

void OnInit_hedgeUnlimited()
{
    sthedgeUnlimited.set(&cdhedgeUnlimited, &achedgeUnlimited);
    // Trader.management.add(&sthedgeUnlimited);
    Trader.strategys.add(&sthedgeUnlimited);
}
...
#define Section_CloseByMoney_One_Trade
#ifdef Section_CloseByMoney_One_Trade
// Mark: Section_CloseByMoney_One_Trade

input string tcloseOneTrade         = "== Close Trade by Money Setup =="; // ————————————————————————
input bool   closeOneTrade_on       = false;                   // Close Trade by hedge money on?
input bool deletePendingsWhenCloseOn = true;                   // Delete Pendings when close ?
input double profit_limit_one_trade = 100;                     // Profit Limit (0 = off)
input double loss_limit_one_trade   = 100;                     // Loss Limit (0 = off)

class ConditionCloseByMoneyOneTrade : public iConditions
{
  public:
    bool evaluate()
    {
...
ConditionCloseByMoneyOneTrade cdCloseByMoneyOneTrade;

class ActionCloseByMoneyOneTrade : public iActions
{
  public:
    bool execute()
    {
        ...
        for (int i = OrdersTotal() - 1; i >= 0; i--) {
            ...
            if (profitAcum >= profit_limit_one_trade && profit_limit_one_trade > 0) { r = true; }
            if (fabs(profitAcum) >= fabs(loss_limit_one_trade) && loss_limit_one_trade != 0 && profitAcum < 0) { r = true; }

                double price = OrderType() == OP_BUY ? Bid : Ask;
                if(r == true) { 
                    int    re    = OrderClose(OrderTicket(), OrderLots(), price, 0, clrGray); 
                    if(deletePendingsWhenCloseOn) acDeletePendings.execute();                    
                }
            }        
        }
        return true;
...
ActionCloseByMoneyOneTrade acCloseByMoneyOneTrade;

Strategy stCloseByMoneyOneTrade;

void OnInit_CloseByMoneyOneTrade()
{
    if (!closeOneTrade_on) return;
    stCloseByMoneyOneTrade.set(&cdCloseByMoneyOneTrade, &acCloseByMoneyOneTrade);
    Trader.management.add(&stCloseByMoneyOneTrade);
}
...
#define Section_DeletePendingsIfNotTrade
#ifdef Section_DeletePendingsIfNotTrade
// Mark: Section_DeletePendingsIfNotTrade

class ConditionDeletePendingsIfNotTrade : public iConditions
{
  public:
    bool evaluate()
    {
        int count = 0;
        for (int i = OrdersTotal() - 1; i >= 0; i--) {
            if (OrderSelect(i, SELECT_BY_POS) && OrderSymbol() == _Symbol && (OrderType()==OP_BUY || OrderType()==OP_SELL))
                count ++;
            }
        return (count == 0);
    }
};
ConditionDeletePendingsIfNotTrade cdDeletePendingsIfNotTrade;

class ActionDeletePendingsIfNotTrade : public iActions
{
  public:
    bool execute()
    {
...
ActionDeletePendingsIfNotTrade acDeletePendingsIfNotTrade;

Strategy stDeletePendingsIfNotTrade;

void OnInit_DeletePendingsIfNotTrade()
{
    stDeletePendingsIfNotTrade.set(&cdDeletePendingsIfNotTrade, &acDeletePendingsIfNotTrade);
    Trader.strategys.add(&stDeletePendingsIfNotTrade);
}
...
int OnInit()
{
    ...
    Trader.management.set(_Symbol, 0);

    if (!uHedgeUnlimited) OnInit_Hedge();
    if (uHedgeUnlimited) OnInit_hedgeUnlimited();
    OnInit_CloseByMoneyOneTrade();
    OnInit_DeletePendingsIfNotTrade();
