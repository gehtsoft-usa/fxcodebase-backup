...
extern int ActionType=0;   // 0 - Close, 1 - High/Low
...
public:
   ...
   bool IsPass(const int period)
   {
      if (ActionType == 0)
      {
         return Close[period] > TrendStop[period + 1] && !Trend[period + 1];
      }
      return High[period] > TrendStop[period + 1] && !Trend[period + 1];
   }
};
...
public:
   ...
   bool IsPass(const int period)
   {
      ...
      return Low[period] < TrendStop[period + 1] && Trend[period + 1];
   }
};
...
int init()
{
   ...
   IndicatorName = GenerateIndicatorName("Trend stop");
...
int start()
{
   ...
   while(pos>=0)
   {
      ...
      if (ActionType==0)
      {
         ...
         {
            if (Close[pos]>TrendStop[pos+1] && !Trend[pos])
            {
               Min_=min;
               Max_=max;
               Trend[pos] = true;
      ...
      else
      {
         ...
         {
            if (High[pos]>TrendStop[pos+1] && !Trend[pos])
            {
               Min_=min;
               Max_=max;
               Trend[pos]=true;
            ...
            {
               if (Low[pos]<TrendStop[pos+1] && Trend[pos])
               {
                  Min_=min;
                  Max_=max;
                  Trend[pos]=false;
      ...
      if (Trend[pos] && Stop>TrendStop[pos+1] && !Flag)
         TrendStop[pos]=Stop;
      else
      {
         if (!Trend[pos] && Stop<TrendStop[pos+1] && !Flag)
