//Available @  https://fxcodebase.com/code/viewtopic.php?f=38&t=76125
...
int OnCalculate(const int rates_total, const int prev_calculated, const datetime &time[], const double &open[], const double &high[], const double &low[], const double &close[], const long &tick_volume[], const long &volume[], const int &spread[])
{
    ...
    for (i = periods; i >= 0; i--) {
            ...
            for (int n = 1; n < _back; n++) {
                ...
                if (close[i + 1] < swHigh[j] && close[i + 1] < open[i + 1]) {
                    ...
                    if (zz_line[u] > swHigh[j]) {
                        if ((pinbarOn == true && isPinbar("dn", u)) || (pinbarOn == false)) {
                            Sell[u] = high[u];                         
            ...
            for (int n = 1; n < _back; n++) {
                ...
                if (close[i + 1] > swLow[j] && close[i + 1] > open[i + 1]) {
                    ...
                    if (zz_line[u] < swLow[j]) {
                        if ((pinbarOn == true && isPinbar("up", u)) || (pinbarOn == false)) {
                            Buy[u] = low[u];
        ...
        // si en la vela anterior hay una flecha, notificar
        if(newCandle.IsNewCandle()) {
            for(int j=1; j <= 3; j++)
            {

                if (Buy[j] != 0.0 && Buy[j] != EMPTY_VALUE && Buy[j] != lastbuy) {
                    Notifications(0);
                    lastbuy = Buy[j];
                    break;
                }
                if (Sell[j] != 0.0 && Sell[j] != EMPTY_VALUE && Sell[j] != lastsell) {
                    Notifications(1);
                    lastsell = Sell[j];
                    break;
                }
