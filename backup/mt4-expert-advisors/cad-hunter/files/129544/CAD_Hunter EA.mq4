...
input int indi_period = 8; // Period
input int indi_smooth = 13; // Smooth
input int indi_minmaxperiod = 25; // Min/max period
input double indi_uplevel = 90; // Up level
input double indi_downlevel = 10; // Down level
...
public:
   ...
   {
      double value00 = iCustom(_symbol, _timeframe, "CAD_Hunter_indicator", 
         indi_period, indi_smooth, indi_minmaxperiod, indi_uplevel, indi_downlevel, 0, period);
      double value01 = iCustom(_symbol, _timeframe, "CAD_Hunter_indicator", 
         indi_period, indi_smooth, indi_minmaxperiod, indi_uplevel, indi_downlevel, 0, period + 1);
      double value10 = iCustom(_symbol, _timeframe, "CAD_Hunter_indicator", 
         indi_period, indi_smooth, indi_minmaxperiod, indi_uplevel, indi_downlevel, 1, period);
      double value11 = iCustom(_symbol, _timeframe, "CAD_Hunter_indicator", 
         indi_period, indi_smooth, indi_minmaxperiod, indi_uplevel, indi_downlevel, 1, period + 1);
      return value00 >= value10 && value01 < value11;
   }
};
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      ...
      double value01 = iCustom(_symbol, _timeframe, "CAD_Hunter_indicator", 
         indi_period, indi_smooth, indi_minmaxperiod, indi_uplevel, indi_downlevel, 0, period + 1);
      double value10 = iCustom(_symbol, _timeframe, "CAD_Hunter_indicator", 
         indi_period, indi_smooth, indi_minmaxperiod, indi_uplevel, indi_downlevel, 2, period);
      double value11 = iCustom(_symbol, _timeframe, "CAD_Hunter_indicator", 
         indi_period, indi_smooth, indi_minmaxperiod, indi_uplevel, indi_downlevel, 2, period + 1);
      return value00 <= value10 && value01 > value11;
   }
};
...
int OnInit()
{
   ...
   stats = NULL;
#endif
   double temp = iCustom(NULL, 0, "CAD_Hunter_indicator", 0, 0);
   if (GetLastError() == ERR_INDICATOR_CANNOT_LOAD)
   {
      Alert("Please, install the 'CAD_Hunter_indicator' indicator");
      return INIT_FAILED;
   }
   if (!IsDllsAllowed() && advanced_alert)
   {
