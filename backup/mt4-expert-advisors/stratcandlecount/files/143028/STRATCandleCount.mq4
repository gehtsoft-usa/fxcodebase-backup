...
//https://fxcodebase.com/code/viewtopic.php?f=38&t=71384
...
input double shift_arrows_pips = 0.1; // Shift
input color p1_color = White; // Pattern 1 color
input color p2_color = White; // Pattern 2 color
input color p3_color = Aqua; // Pattern 3 color
input color p4_color = Red; // Pattern 4 color
...
int CreateAlert(int id, ICondition* condition, IAction* action, string text, string message, color clr, PriceType priceType, int sign)
{
   ...
   static int lastId = 1;
   id = conditions[size].RegisterText(id, message, IndicatorObjPrefix + IntegerToString(lastId++), text, clr, highStream, font_size, sign == 1 ? ANCHOR_LOWER : ANCHOR_UPPER);
   highStream.Release();
   return id;
}
...
{
   {
      ...
      upCondition.Add(new P1Condition(_Symbol, tf), false);
      ...
      id = CreateAlert(id, upCondition, upAction, "1", "1 " + TimeframeToString(tf), p1_color, PriceLow, -1);
      upCondition.Release();
      if (upAction != NULL)
      {
         upAction.Release();
   ...
   {
      AndCondition* upCondition = new AndCondition();
      upCondition.Add(new P2Condition(_Symbol, tf), false);
      SetLastSignalAction* upAction = NULL;
      if (filter_consecutive)
      {
         upCondition.Add(new LastSignalNotCondition(2), false);
         upAction = new SetLastSignalAction(2);
      }
      id = CreateAlert(id, upCondition, upAction, "3", "3 " + TimeframeToString(tf), p2_color, PriceLow, -1);
      upCondition.Release();
      if (upAction != NULL)
      {
         upAction.Release();
   ...
   {
      AndCondition* upCondition = new AndCondition();
      upCondition.Add(new P3Condition(_Symbol, tf), false);
      SetLastSignalAction* upAction = NULL;
      if (filter_consecutive)
      {
         upCondition.Add(new LastSignalNotCondition(3), false);
         upAction = new SetLastSignalAction(3);
      }
      id = CreateAlert(id, upCondition, upAction, "2", "2 " + TimeframeToString(tf), p3_color, PriceHigh, 1);
      upCondition.Release();
      if (upAction != NULL)
      {
         upAction.Release();
   ...
   {
      AndCondition* upCondition = new AndCondition();
      upCondition.Add(new P4Condition(_Symbol, tf), false);
      SetLastSignalAction* upAction = NULL;
      if (filter_consecutive)
      {
         upCondition.Add(new LastSignalNotCondition(4), false);
         upAction = new SetLastSignalAction(4);
      }
      id = CreateAlert(id, upCondition, upAction, "2", "2 " + TimeframeToString(tf), p4_color, PriceLow, -1);
      upCondition.Release();
      if (upAction != NULL)
      {
         upAction.Release();
...
class P1Condition : public ACondition
{
public:
   P1Condition(const string symbol, ENUM_TIMEFRAMES timeframe)
      :ACondition(symbol, timeframe)
   {
   ...
   bool IsPass(const int period, const datetime date)
   {
      return iHigh(_symbol, _timeframe, period) <= iHigh(_symbol, _timeframe, period + 1)
         && iLow(_symbol, _timeframe, period) >= iLow(_symbol, _timeframe, period + 1);
   }
};
class P2Condition : public ACondition
{
public:
   P2Condition(const string symbol, ENUM_TIMEFRAMES timeframe)
      :ACondition(symbol, timeframe)
   {
...
class P3Condition : public ACondition
{
public:
   P3Condition(const string symbol, ENUM_TIMEFRAMES timeframe)
      :ACondition(symbol, timeframe)
   {
...
class P4Condition : public ACondition
{
public:
   P4Condition(const string symbol, ENUM_TIMEFRAMES timeframe)
      :ACondition(symbol, timeframe)
   {
...
int init()
{
   ...
   {
      id = CreateAlert(id, (ENUM_TIMEFRAMES)_Period, 0, 0);
   }
