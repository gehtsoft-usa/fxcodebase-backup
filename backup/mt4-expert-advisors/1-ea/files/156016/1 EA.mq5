...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=75035
...
class ConditionNewTradeOpen : public iConditions
{
    int count;
    public:
    bool evaluate()
    {
        ...
        if (count == 0 || currentPositions > count){
            count = currentPositions;
            r = true;
        }
        if(currentPositions < count)
        {
            count = currentPositions;
        }
...
ConditionNewTradeOpen cdNewTradeOpen;
...
Strategy stDeletePendingsWhenNewTrade(&cdNewTradeOpen, &acDeletePendings);

void oninit_delete_pendings_strategy()
{
        Trader.management.add(&stDeletePendingsWhenNewTrade);
}
...
string TmodeEntry = "== Entry Setup =="; // ————————————————————————
ModeEntry    modeEntry  = PendingStop;         // Mode Entry:
int    magico     = 1982;                // Magic Number:

#ifdef pending_orders_on
double uEntryDistance = 1; // Pips distance for pending orders:
#endif
...
#ifdef pending_orders_on
    double points   = (double)SymbolInfoDouble(symbol, SYMBOL_POINT);
    double distance = uEntryDistance * 10 * points;
...
input enum_lot_mode lot_mode   = lot_equity_percent;          // Lot Calculation Mode
input double        uLotsValue = 1;                           // value to calculate Lots:
...
bool setExpiration()
{
    int oneCandeTime = (int)iTime(NULL,0,1) - iTime(NULL,0,2);
    _expiration = TimeCurrent() + oneCandeTime;
    return true;
}
    bool execute()
    {
        ...
        if(_cmd == ORDER_TYPE_BUY_STOP) 
            if (!trade.BuyStop(_lots, _price, _symbol, _sl, _tp, ORDER_TIME_SPECIFIED, _expiration)) { Print(__FUNCTION__, " ", "Cannot Send Order, error: ", GetLastError()); return false;}
        if(_cmd == ORDER_TYPE_SELL_STOP)
            if (!trade.SellStop(_lots, _price, _symbol, _sl, _tp,  ORDER_TIME_SPECIFIED, _expiration)) { Print(__FUNCTION__, " ", "Cannot Send Order, error: ", GetLastError()); return false;}

        if(_cmd == ORDER_TYPE_BUY || _cmd == ORDER_TYPE_SELL) 
            if(!trade.PositionOpen(_symbol, _cmd, _lots, _price, _sl, _tp, "")) { Print(__FUNCTION__, " ", "Cannot Send Order, error: ", GetLastError()); return false; }
...
input string ttradingtm = "== Trading Time =="; //
input string trade1_tm  = "02:15";              // Trade Time 1 (Broker Time)
input string trade2_tm  = "03:15";              // Trade Time 2 (Broker Time)
input string trade3_tm  = "04:15";              // Trade Time 3 (Broker Time)
input string trade4_tm  = "05:15";              // Trade Time 4 (Broker Time)
input string trade5_tm  = "06:15";              // Trade Time 5 (Broker Time)

string allow_trades_tm = "00:00"; // allow trades
bool   trade1_buy_on   = true;
bool   trade2_buy_on   = true;
bool   trade3_buy_on   = true;
bool   trade4_buy_on   = true;
bool   trade5_buy_on   = true;
bool   trade1_sell_on  = true;
bool   trade2_sell_on  = true;
bool   trade3_sell_on  = true;
bool   trade4_sell_on  = true;
bool   trade5_sell_on  = true;
...
class BUYcondition1 : public iConditions
{
  public:
    bool evaluate()
    {
        ...
        if (trade1_buy_on)
            if (TimeCurrent() == ConvertUserTime(trade1_tm)) {
                trade1_buy_on = false;
                return true;
            }
        if (trade2_buy_on)
            if (TimeCurrent() == ConvertUserTime(trade2_tm)) {
                trade2_buy_on = false;
                return true;
            }
        if (trade3_buy_on)
            if (TimeCurrent() == ConvertUserTime(trade3_tm)) {
                trade3_buy_on = false;
                return true;
            }
        if (trade4_buy_on)
            if (TimeCurrent() == ConvertUserTime(trade4_tm)) {
                trade4_buy_on = false;
                return true;
            }
        if (trade5_buy_on)
            if (TimeCurrent() == ConvertUserTime(trade5_tm)) {
                trade5_buy_on = false;
                return true;
            }

        if (TimeCurrent() >= ConvertUserTime(allow_trades_tm)) {
            trade1_buy_on = true;
            trade2_buy_on = true;
            trade3_buy_on = true;
            trade4_buy_on = true;
...
class SELLcondition1 : public iConditions
{
  public:
    bool evaluate()
    {
        ...
        if (trade1_sell_on)
            if (TimeCurrent() == ConvertUserTime(trade1_tm)) {
                trade1_sell_on = false;
                return true;
            }
        if (trade3_sell_on)
            if (TimeCurrent() == ConvertUserTime(trade3_tm)) {
                trade3_sell_on = false;
                return true;
            }
        if (trade2_sell_on)
            if (TimeCurrent() == ConvertUserTime(trade2_tm)) {
                trade2_sell_on = false;
                return true;
            }
        if (trade4_sell_on)
            if (TimeCurrent() == ConvertUserTime(trade4_tm)) {
                trade4_sell_on = false;
                return true;
            }
        if (trade5_sell_on)
            if (TimeCurrent() == ConvertUserTime(trade5_tm)) {
                trade5_sell_on = false;
                return true;
            }
...
datetime ConvertUserTime(string time)
{
    MqlDateTime tm;
    TimeCurrent(tm);
    tm.hour = (int)StringSubstr(time, 0, 2);
    tm.min  = (int)StringSubstr(time, 3, 2);
...
int OnInit()
{
    ...
    oninit_delete_pendings_strategy();
...
void OnTimer(void) { Trader.doTrading(); }
