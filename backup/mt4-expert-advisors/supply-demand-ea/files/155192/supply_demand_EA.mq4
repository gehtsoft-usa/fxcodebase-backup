...
string file_custom_indicator = "a-supply-demand";
int    bufferToSell          = 1;
int    bufferToBuy           = 4;
...
double Support()
{
    ...
    while (value == 0 && i < 1000) {
        double in  = iCustom(NULL, 0, file_custom_indicator, bufferToBuy, i);
        double in2 = iCustom(NULL, 0, file_custom_indicator, bufferToBuy + 1, i);
...
double Resistance()
{
    ...
    double value = 0;
    while (value == 0 && i < 1000) {
        double in  = iCustom(NULL, 0, file_custom_indicator, bufferToSell, i);
        double in2 = iCustom(NULL, 0, file_custom_indicator, bufferToSell - 1, i);
        if (in > 0 && in != EMPTY_VALUE && in2 > 0 && in2 != EMPTY_VALUE) {
            value = in;
        }
        i++;
...
class SellOppositeCondition : public iConditions
{
    double lastSignal;
...
SellOppositeCondition sellOppositeCondition;
...
class BuyOppositeCondition : public iConditions
{
    double lastSignal;
...
BuyOppositeCondition buyOppositeCondition;
  ...
  public:
    bool evaluate()
    {
        if (closeAllInOpositeSignal)
            if (buyOppositeCondition.evaluate())
                return true;
...
class ConditionToCloseSell : public iConditions
{
  public:
    ...
    {
        if (closeAllInOpositeSignal)
            if (sellOppositeCondition.evaluate())
                return true;
...
void OnTick()
{
    ...
    // ——————————————————————————————————————————————————————————————————
    // NOTE: close conditions en ontick    
    if (mainOrders.qnt() > 0) {
        if (!uTradeReverse) {
