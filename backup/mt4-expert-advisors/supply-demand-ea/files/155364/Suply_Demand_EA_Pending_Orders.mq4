...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=74826
...
ActionCloseSell actionCloseSell;
...
void            BasicStrategy_Iniciate()
{
    ...
    StrategyToCloseBuy.addCondition(&conditionCloseBuy);
    StrategyToCloseBuy.addAction(&actionCloseBuy);

    StrategyToCloseSell.addCondition(&conditionCloseSell);
    StrategyToCloseSell.addAction(&actionCloseSell);
...
input const string indicator_file  = "a-supply-demand";           // Indicator File:




int                bufferToSell_SL = 0;
int                bufferToSell    = 1;
int                bufferToBuy     = 4;
int                bufferToBuy_SL  = 3;
bool               indicator_init  = true;

double             Support(string mode = "")
{
    int    i     = 1;
    double value = 0;
    while (value == 0 && i < 1000) {
        double in  = iCustom(NULL, 0, indicator_file, bufferToBuy, i);
        double in2 = iCustom(NULL, 0, indicator_file, bufferToBuy_SL, i);
        if (in > 0 && in != EMPTY_VALUE && in2 > 0 && in2 != EMPTY_VALUE) {
            value = in;
            if (mode == "sl")
                value = in2;
        }
...
double Resistance(string mode = "")
{
    int    i     = 1;
    double value = 0;
    while (value == 0 && i < 1000) {
        double in  = iCustom(NULL, 0, indicator_file, bufferToSell, i);
        double in2 = iCustom(NULL, 0, indicator_file, bufferToSell_SL, i);
        if (in > 0 && in != EMPTY_VALUE && in2 > 0 && in2 != EMPTY_VALUE) {
            value = in;
            if (mode == "sl")
                value = in2;
...
class ConditionIndicatorBuy : public iConditions
{
  ...
  public:
    ...
    {
        switch (modeEntry) {
        case Market            : return evaluateMarket();
        case PendigsByIndicator: return evaluatePendings();
        }
        return false;
    }

    bool evaluatePendings()
    {
        double s            = Support();
        double upperBlueBox = Resistance("sl");
        ...
        if (Bid > upperBlueBox)
            if (lastSignal != upperBlueBox) {
                lastSignal = upperBlueBox;
                return true;
            }
    ...
    bool evaluateMarket()
    {
        ...
        if (
            lastSignal != s &&
            iOpen(NULL, 0, 1) > s && iLow(NULL, 0, 1) <= s &&
            iLow(NULL, 0, 0) < s && iClose(NULL, 0, 0) >= s) {
            lastSignal = s;
            return true;
        }
        return false;
...
class ConditionIndicatorSell : public iConditions
{
  ...
  public:
    ...
    {
        double r             = Resistance();
        double lowerBrownBox = Support("sl");
        ...
        if (Ask < lowerBrownBox)
            if (lastSignal != lowerBrownBox) {
                lastSignal = lowerBrownBox;
                return true;
            }
    ...
    bool evaluateMarket()
    {
        ...
        if (
            lastSignal != r &&
            iOpen(NULL, 0, 1) < r && iHigh(NULL, 0, 1) >= r &&
            iHigh(NULL, 0, 0) > r && iClose(NULL, 0, 0) <= r) {
            lastSignal = r;
            return true;
        }
        return false;
...
Strategy               IndicatorStrategyLong;
Strategy               IndicatorStrategyShort;



class ConditionNotBox : public iConditions
{
  public:
    bool evaluate()
    {
...
ConditionNotBox conditionNotBox;

class ActionDeletePending : public iActions
{
  public:
    bool execute()
    {
        bool r = OrderDelete(OrderTicket(), White);
        return r;
    }
};
ActionDeletePending actionDeletePending;

Strategy            CleanPendingsWhenNotBox;
...
bool                OnInit_CustomIndicator()
{
    ...
    IndicatorStrategyLong.addCondition(&conditionIndicatorBuy);
    IndicatorStrategyLong.addAction(&buy);
    Trader.strategys.add(&IndicatorStrategyLong);
    
    IndicatorStrategyShort.addCondition(&conditionIndicatorSell);
    IndicatorStrategyShort.addAction(&sell);
    Trader.strategys.add(&IndicatorStrategyShort);

    CleanPendingsWhenNotBox.addCondition(&conditionNotBox);
    CleanPendingsWhenNotBox.addAction(&actionDeletePending);
    if(clean_pendings) Trader.management.add(&CleanPendingsWhenNotBox);
...
// MARK: Section Entry
    ...
    #define pending_orders_based_indicator
...
enum ModeEntry {
    ...
    #ifdef pending_orders_based_indicator
        PendigsByIndicator, // By Indicator (Pendings)
    #endif
...
input bool      clean_pendings = true;                // Delete Pendings when box desapear:
...
double Price(string side, int pips = 0, string _symbol = "")
{
    ...
    if (modeEntry == PendigsByIndicator) {
        if (side == "buy") {
            return Support();
        }
        if (side == "sell") {
...
double StopLoss(const double price, const string side, const string symbol, const double lot)
{
    ...
    case sl_by_indicator: return StopLossByIndicator(price, side, symbol);
...
input double         uStopLossValue = 100;               // SL value:
input double         sl_extra_pips  = 5;                 // Extra pips (mode Indicator)
// ------------------------------------------------------------------
...
double StopLossByIndicator(const double price, const string side, const string symbol)
{
    double point    = MarketInfo(symbol, MODE_POINT);
    double distance = sl_extra_pips * 10 * point;

    if (side == "buy") {
        return Support("sl") - distance;
    }
    if (side == "sell") {
        return Resistance("sl") + distance;
    }
    return 0;
}
...
double TakeProfit(const double price, const string side, const string symbol, const double lot)
{
    ...
    case tp_by_indicator: return TakeProfitByIndicator(price, side, symbol);
    }
...
input double           uTakeProfitValue = 3;                   // TP value
...
double TakeProfitByIndicator(const double price, const string side, const string symbol)
{
...
void oninit_counters_filters()
{
    ...
    IndicatorStrategyShort.addCondition(&cCountPositions);
    IndicatorStrategyLong.addCondition(&cCountPositions);
    
    IndicatorStrategyShort.addCondition(&cCountSells);
    IndicatorStrategyLong.addCondition(&cCountBuys);
}
