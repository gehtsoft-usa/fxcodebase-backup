...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=73199
...
string file_hull = "Hull.ex4";
string file_qqe  = "QQE.ex4";
string file_fli  = "FLI_indicator.ex4";
...
enum Arrows { Hide_Arrows,
              Simple_Arrows,
              Open_Cose_Median,
              High_Low_Open_Close };
input string             TFli         = "== FLI indicator Setup ==";  // ————————————
input int                BarsCount    = 10000;                        // Bars:
input int                BBperiod     = 21;                           // BB Period:
input double             BBdeviations = 1;                            // BB Desv:
input int                MAperiod     = 21;                           // MA Period:
input int                ATRperiod    = 5;                            // ATR period:
input bool               UseATRfilter = false;                        // Use ATR Filter:
bool                     AlertON      = false;
Arrows                   TypeOfArrows = Simple_Arrows;
input string             THULL        = "== HULL indicator Setup ==";  // ————————————
input int                inpPeriod    = 20;                            // Period
input double             inpDivisor   = 2.0;                           // Divisor ("speed")
input ENUM_APPLIED_PRICE inpPrice     = PRICE_CLOSE;                   // Price
input string             TQQE         = "== QQE indicator Setup ==";   // ————————————
input int                SF           = 5;                             // SF:
...
class BUYcondition1 : public iConditions
{
 public:
  bool evaluate()
  {
    ...
    double upSignal1 = iCustom(NULL, 0, file_fli, BarsCount, BBperiod, BBdeviations, MAperiod, ATRperiod, UseATRfilter, AlertON, TypeOfArrows, 2, 1);
    double upSignal2 = iCustom(NULL, 0, file_fli, BarsCount, BBperiod, BBdeviations, MAperiod, ATRperiod, UseATRfilter, AlertON, TypeOfArrows, 2, 2);
    return upSignal1 != EMPTY_VALUE || upSignal2 != EMPTY_VALUE;
    // return false;
  }
};
 ...
 public:
  bool evaluate()
  {
    // TODO: condition Buy 2
		double upSignal = iCustom(NULL, 0, file_hull, 1, 1);
    return upSignal == EMPTY_VALUE;
    // return false;
  }
};
 ...
 public:
  bool evaluate()
  {
    // TODO: condition Buy 3
		double value1 = iCustom(NULL, 0, file_qqe, SF, 0, 1);
		double value2 = iCustom(NULL, 0, file_qqe, SF, 1, 1);
...
class SELLcondition1 : public iConditions
{
 public:
  bool evaluate()
  {
    ...
    double dnSignal1 = iCustom(NULL, 0, file_fli, BarsCount, BBperiod, BBdeviations, MAperiod, ATRperiod, UseATRfilter, AlertON, TypeOfArrows, 3, 1);
    double dnSignal2 = iCustom(NULL, 0, file_fli, BarsCount, BBperiod, BBdeviations, MAperiod, ATRperiod, UseATRfilter, AlertON, TypeOfArrows, 3, 2);
    return dnSignal1 != EMPTY_VALUE || dnSignal2 != EMPTY_VALUE;
    ;
 ...
 public:
  bool evaluate()
  {
    // TODO: condition sell 2
		double dnSignal = iCustom(NULL, 0, file_hull, 1, 1);
    return dnSignal != EMPTY_VALUE;
    // return false;
  }
};
...
int OnInit()
{
  ...
  double temp = iCustom(NULL, 0, file_hull, 0, 0);
  if (GetLastError() == ERR_INDICATOR_CANNOT_LOAD)
  {
    Alert("Please, install the: " + file_hull + " indicator");
    return INIT_FAILED;
  }
  temp = iCustom(NULL, 0, file_fli, 0, 0);
  if (GetLastError() == ERR_INDICATOR_CANNOT_LOAD)
  {
    Alert("Please, install the: " + file_fli + " indicator");
    return INIT_FAILED;
  }
  temp = iCustom(NULL, 0, file_qqe, 0, 0);
  if (GetLastError() == ERR_INDICATOR_CANNOT_LOAD)
  {
    Alert("Please, install the: " + file_qqe + " indicator");
    return INIT_FAILED;
  }
#endif
