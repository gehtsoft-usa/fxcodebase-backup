...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=75080
...
#define Section_Custom_Indicator_Special_Indicator
...
#ifdef Section_Custom_Indicator_Special_Indicator
...
input const string indicator_file = "Special_Indicator";                          // Indicator File:
//---
int  bufferToBuy              = 1;
int  bufferToSell             = 0;
  ...
  public:
    bool evaluate()
    {
        for (int i = 1; i <= candles_back_for_signals; i++) {
            double buBuy = Indi(bufferToBuy, i);
            double buBuyPrev = Indi(bufferToBuy, i-1);
            if (buBuy > 0 && buBuy != EMPTY_VALUE && buBuy != lastSignal && buBuyPrev == EMPTY_VALUE) {
                lastSignal = buBuy;
                return true;
            }
        }
...
class ConditionIndicatorSell : public iConditions
{
  ...
  public:
    ...
    {
        for (int i = 1; i <= candles_back_for_signals; i++) {
            double buSell = Indi(bufferToSell, i);
            double buSellPrev = Indi(bufferToSell, i-1);
            if (buSell > 0 && buSell != EMPTY_VALUE && buSell != lastSignal && buSellPrev == EMPTY_VALUE) {
                lastSignal = buSell;
                return true;
            }
        }
...
input const string indicator_file_wave = "Wave Reversal Indicator.ex4";                          // Indicator File:
int  bufferToBuy_wave              = 0;
int  bufferToSell_wave             = 1;
bool indicator_init_wave           = true;

double Indi_wave(int buffer, int candle = 1) { return iCustom(NULL, 0, indicator_file_wave, buffer, candle); }

class ConditionIndicatorBuy_wave : public iConditions
{
    double lastSignal;
  ...
  public:
    ...
    {
        for (int i = 1; i <= candles_back_for_signals; i++) {
            double buBuy = Indi_wave(bufferToBuy_wave, i);
            if (buBuy == 0 && buBuy != EMPTY_VALUE) {
                lastSignal = buBuy;
                return true;
            }
        }
...
ConditionIndicatorBuy_wave conditionIndicatorBuy_wave;

class ConditionIndicatorSell_wave : public iConditions
{
    double lastSignal;
  ...
  public:
    ...
    {
        for (int i = 1; i <= candles_back_for_signals; i++) {
            double buSell = Indi_wave(bufferToSell_wave, i);
            if (buSell == 0 && buSell != EMPTY_VALUE) {
                lastSignal = buSell;
                return true;
            }
        }
...
ConditionIndicatorSell_wave conditionIndicatorSell_wave;

bool OnInit_CustomIndicator_wave()
{
    double temp = iCustom(NULL, 0, indicator_file_wave, 0, 0);
    if (GetLastError() == ERR_INDICATOR_CANNOT_LOAD) {
        MessageBox("THIS EA NEED AN INDICATOR\n install the file:\n" + indicator_file_wave + "\ninto the folder:\nMQL4/Indicators.", "Important Information", MB_ICONINFORMATION);
        Alert("THIS EA NEED AN INDICATOR, install the file: " + indicator_file_wave + " into the folder: MQL4/Indicators.");
        indicator_init_wave = false;
    }

    if (!indicator_init_wave) return false;

    ShortStrategy.addCondition(&conditionIndicatorSell_wave);
    LongStrategy.addCondition(&conditionIndicatorBuy_wave);
...
#ifdef Section_Custom_Indicator_Special_Indicator
    if (!OnInit_CustomIndicator()) { return INIT_FAILED; }
#endif

#ifdef Section_Custom_Indicator
    if (!OnInit_CustomIndicator_wave()) { return INIT_FAILED; }
#endif
