...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=73314
...
input int                consecutives   = 7;   // Nro of Candles in same direction:
// ------------------------------------------------------------------
...
class FinderPattern
{
 ...
 public:
  ...
  bool havePatternUp(int i, ENUM_TIMEFRAMES tf)
  {
    ...
    int n = 0;
    for (; n < consecutives; n++) {
      if (iClose(_symbol, tf, i + n) > iOpen(_symbol, tf, i + n)){ continue;}
      else{ break; }
                }
    if (n == consecutives) return true;
