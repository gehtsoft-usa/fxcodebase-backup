// Available @  https://fxcodebase.com/code/viewtopic.php?f=38&t=74528
...
int account_number = 519976;
// int account_number = 2100212550;
bool controlAcount = true;
...
//+------------------------------------------------------------------+
string ColorNote      = "--- Fibonacci colors ---";
color  UpperFiboColor = clrWhite;
color  MainFiboColor  = clrYellow;
color  LowerFiboColor = clrRed;
string TimeFrameNote  = "--- Timeframe for the high and low ---";
string TimeFrameNot2  = "{1=M1, 5=M5, 15=M15, ..., 1440=D1, 10080=W1, 43200=MN1}";
int    TimeFrame      = PERIOD_D1;
string OpenTimeNote2  = "--- Open hour for daily timeframe ---";
int    OpenTime       = 0;
string TimeZoneNote   = "{0=ServerTime, 1=GMT, 2=LocalTime}";
int    TimeZone       = 0;

// NOTE: inputs
input double slPoints            = 200;   // Sl points
input double tpPoints            = 100;   // Tp Points
input bool   AutomaticChangeTPSL = false; // If change TP change SL:
...
string MagicsList      = "2023";                 // Magics numbers (separate by comma ","):
int    magico          = 2023;
...
class TslByPips : public iTSL
{
  ...
  public:
    ...
    double newSL(Order* order)
    {
        ...
        Print(__FUNCTION__," pointsToMove: ",pointsToMove);
        if (order.type() == OP_SELL) {
            pointsToMove *= -1;
        }

        double newSl = order.tslNext() - pointsToMove;
        Print(__FUNCTION__," newSl: ",newSl);
...
int OnInit()
{
    ...
    if (TimeZone == 1) {
        if (IsDllsAllowed()) {
            ...
            if (result != 0)
                GmtLocalOffset = TZInfoArray[0]; //	Difference between your local time and GMT in minutes (winter time)
            if (result == 2)
                GmtLocalOffset += TZInfoArray[42]; //	Current difference between your local time and GMT in minutes
    ...
    if (FirstHour > 0)
        ...
        LastHour = 23;
    LastHour += 24 + timeShift;
    LastHour %= 24;
    FirstHour += 24 + timeShift;
    FirstHour %= 24;
...
void OnTick()
{
    ...
    if (TslON)
        tsl.doTSL();
    if (AutomaticChangeTPSL == true && TslON == false)
        AutomaticTPSLControl();
    ...
    //	Use daily, weekly, whatever candles
    {
        ...
        if (TimeFrame == PERIOD_D1 && TimeDayOfWeek(StartTime) == 0 /*Sunday*/) { // Add fridays high and low
    ...
    //	Use hourly candles
    {
        ...
        int shift = 1;
        while (TimeHour(iTime(NULL, PERIOD_D1, shift)) != LastHour)
            shift++;
        //----
        //	find first candle of the period
        int startShift = shift;
        while (TimeHour(iTime(NULL, PERIOD_D1, startShift)) != FirstHour || TimeDayOfWeek(iTime(NULL, PERIOD_D1, startShift)) == 0 /*Sunday*/)
            startShift++;
        while (TimeHour(iTime(NULL, PERIOD_D1, startShift)) == FirstHour)
            startShift++;
        startShift--;

        //----
        //	get the highest high and lowest low of the period
        HiPrice   = iHigh(NULL, PERIOD_D1, iHighest(NULL, PERIOD_D1, MODE_HIGH, startShift - shift + 1, shift));
        LoPrice   = iLow(NULL, PERIOD_D1, iLowest(NULL, PERIOD_D1, MODE_LOW, startShift - shift + 1, shift));
        StartTime = iTime(NULL, PERIOD_D1, startShift);
        EndTime   = iTime(NULL, PERIOD_D1, shift);
    }
    ...
    if (controlAcount)
        if (AccountNumber() != account_number || TimeCurrent() > dateEnd) {
            Alert("Entre em contato: +551199447-9494");
            return;
        }
    ...
    if (CrossingBuyLevels()) {
        string side     = "buy";
        actionSendOrder = new SendNewOrder(side, Lots(), "", 0, SL(side), TP(side), magico);
...
double Lots()
{
    return NormalizeDouble(AccountBalance() / 15000, 2);
}
...
//|                                                                  |
//+------------------------------------------------------------------+
string hdSup        = "------ Setting STR_AlertSignal ----- ";
int    ExtDepth     = 31;
int    ExtDeviation = 3;
int    ExtBackstep  = 31;
bool   AlertOn      = false;
...
int    takeprofit = 150;
double volume     = 0.2;
string hdvlS      = "------ Setting Volume -----";
double volume_0   = 0.04; // Lot size at -100
// extern double volume_1 = 0;//Lot size at -76.4
double volume_2 = 0.03; // Lot size at -61.8
// extern double volume_3 = 0;//Lot size at -50.0
double volume_4 = 0.02; // Lot size at -38.2
double volume_5 = 0.01; // Lot size at -23.6
...
double volume_13 = 0.01; // Lot size at 123.6
double volume_14 = 0.02; // Lot size at 138.2
// extern double volume_15 = 0.0;//Lot size at 150.0
double volume_16 = 0.03; // Lot size at 161.8
// extern double volume_17 = 0;//Lot size at 176.7
double volume_18 = 0.04; // Lot size at 200
...
int DrawFibo()
{
    ...
    ObjectSetFiboDescription("FiboUp", 6, "(Regiao Venda) -  %$");

    // ObjectSet("FiboUp", OBJPROP_FIRSTLEVEL + 7 , 1.236);   ObjectSetFiboDescription("FiboUp", 7 , "(223.6%) -  %$");
    // ObjectSet("FiboUp", OBJPROP_FIRSTLEVEL + 8 , 1.382);   ObjectSetFiboDescription("FiboUp", 8 , "(238.2%) -  %$");
    // ObjectSet("FiboUp", OBJPROP_FIRSTLEVEL + 9 , 1.500);   ObjectSetFiboDescription("FiboUp", 9 , "(250.0%) -  %$");
    // ObjectSet("FiboUp", OBJPROP_FIRSTLEVEL + 10, 1.618);   ObjectSetFiboDescription("FiboUp", 10, "(261.8%) -  %$");
    // ObjectSet("FiboUp", OBJPROP_FIRSTLEVEL + 11, 1.764);   ObjectSetFiboDescription("FiboUp", 11, "(276.4%) -  %$");
    ObjectSet("FiboUp", OBJPROP_FIRSTLEVEL + 12, 2.000);
    ObjectSetFiboDescription("FiboUp", 12, "(Regiao Venda) -  %$");

    ObjectSet("FiboUp", OBJPROP_RAY, true);
    ObjectSet("FiboUp", OBJPROP_BACK, true);

    string va1   = ObjectGetFiboDescription("FiboUp", 12);
    ...
    // NOTE: new fibo levels
    // up
    //   va2236 = NormalizeDouble(((HiPrice - LoPrice) * 2.236 + LoPrice), digit);
    //   va2382 = NormalizeDouble(((HiPrice - LoPrice) * 2.382 + LoPrice), digit);
    //   va2500 = NormalizeDouble(((HiPrice - LoPrice) * 2.500 + LoPrice), digit);
    //   va2618 = NormalizeDouble(((HiPrice - LoPrice) * 2.618 + LoPrice), digit);
    //   va2764 = NormalizeDouble(((HiPrice - LoPrice) * 2.764 + LoPrice), digit);
    va3000 = NormalizeDouble(((HiPrice - LoPrice) * 3.000 + LoPrice), digit);
    // dn
    //   va1236_dn = NormalizeDouble((LoPrice - (HiPrice - LoPrice) * 1.236), digit);
    //   va1382_dn = NormalizeDouble((LoPrice - (HiPrice - LoPrice) * 1.382), digit);
    //   va1500_dn = NormalizeDouble((LoPrice - (HiPrice - LoPrice) * 1.500), digit);
    //   va1618_dn = NormalizeDouble((LoPrice - (HiPrice - LoPrice) * 1.618), digit);
    //  va1764_dn = NormalizeDouble((LoPrice - (HiPrice - LoPrice) * 1.764), digit);
    va2000_dn = NormalizeDouble((LoPrice - (HiPrice - LoPrice) * 2.000), digit);
    //---

    // van ordenados de abajo hacia arriba ?

    arrValue[0] = va2000_dn;
    //    arrValue[1] = va1764_dn;
    //    arrValue[2] = va1618_dn;
    //    arrValue[3] = va1500_dn;
    //    arrValue[4] = va1382_dn;
    //    arrValue[5] = va1236_dn;
    ...
    ab = "FiboUp\n" +
         ...
         "\nLevel 300: " + va3000 + " ||  OpenS: " + arrStatus[30] + " || OpenB: " + arrStatusB[30] +
         //        "\nLevel 276: " + va2764 + " ||  OpenS: " + arrStatus[29] + " || OpenB: " + arrStatusB[29] +
         //        "\nLevel 261: " + va2618 + " ||  OpenS: " + arrStatus[28] + " || OpenB: " + arrStatusB[28] +
         //        "\nLevel 250: " + va2500 + " ||  OpenS: " + arrStatus[27] + " || OpenB: " + arrStatusB[27] +
         //        "\nLevel 238: " + va2382 + " ||  OpenS: " + arrStatus[26] + " || OpenB: " + arrStatusB[26] +
         //        "\nLevel 223: " + va2236 + " ||  OpenS: " + arrStatus[25] + " || OpenB: " + arrStatusB[25] +

         "\nLevel 200: " + va200 + " ||  OpenS: " + arrStatus[24] + " || OpenB: " + arrStatusB[24] +
         //        "\nLevel 1764: " + va1764 + " || OpenS: "      + arrStatus[23] + " || OpenB: " + arrStatusB[23] +
         //        "\nLevel 1618: " + va1618 + " || OpenS: "      + arrStatus[22] + " || OpenB: " + arrStatusB[22] +
         //        "\nLevel 150: " + va150 + " || OpenS: "        + arrStatus[21] + " || OpenB: " + arrStatusB[21] +
         //        "\nLevel 1382: " + va1382 + " || OpenS: "      + arrStatus[20] + " || OpenB: " + arrStatusB[20] +
         //        "\nLevel 1236: " + va1236 + " || OpenS: "      + arrStatus[19] + " || OpenB: " + arrStatusB[19] +
         "\nLevel 100: " + arrValue[18] + " || OpenS: " + arrStatus[18] + " || OpenB: " + arrStatusB[18] +
         "\n----------" +
         //        "\nLevel 764: " + va764 + " || OpenS: "     + arrStatus[17] + " || OpenB: " + arrStatusB[17] +
         //        "\nLevel 618: " + va618 + " || OpenS: "     + arrStatus[16] + " || OpenB: " + arrStatusB[16] +
         //        "\nLevel 508: " + va50  + " || OpenS: "     + arrStatus[15] + " || OpenB: " + arrStatusB[15] +
         //        "\nLevel 382: " + va382 + " || OpenS: "     + arrStatus[14] + " || OpenB: " + arrStatusB[14] +
         //        "\nLevel 236: " + va236 + " || OpenS: "     + arrStatus[13] + " || OpenB: " + arrStatusB[13] +
         "\nLevel 0: " + arrValue[12] + " || OpenS: " + arrStatus[12] + " || OpenB: " + arrStatusB[12] +
         "\n---  Fibo Low ----" +

         //        "\nLevel -236: " + va236a + " || OpenS: " + arrStatus[11] + " || OpenB: " + arrStatusB[11] +
         //        "\nLevel -382: " + va382a + " || OpenS: " + arrStatus[10] + " || OpenB: " + arrStatusB[10] +
         //        "\nLevel -508: " + va50a +  " || OpenS: " + arrStatus[9]  + " || OpenB: " + arrStatusB[9] +
         //        "\nLevel -618: " + va618a + " || OpenS: " + arrStatus[8]  + " || OpenB: " + arrStatusB[8] +
         //        "\nLevel -764: " + va764a + " || OpenS: " + arrStatus[7]  + " || OpenB: " + arrStatusB[7] +
         "\nLevel -100: " + va100a + " || OpenS: " + arrStatus[6] + " || OpenB: " + arrStatusB[6] +

         "\nLevel -123: " + va100a + " || OpenS: " + arrStatus[5] + " || OpenB: " + arrStatusB[5] +
         //        "\nLevel -132: " + va100a + " || OpenS: " + arrStatus[4] + " || OpenB: " + arrStatusB[4] +
         //        "\nLevel -150: " + va100a + " || OpenS: " + arrStatus[3] + " || OpenB: " + arrStatusB[3] +
         //        "\nLevel -161: " + va100a + " || OpenS: " + arrStatus[2] + " || OpenB: " + arrStatusB[2] +
         //        "\nLevel -176: " + va100a + " || OpenS: " + arrStatus[1] + " || OpenB: " + arrStatusB[1] +
         "\nLevel -200: " + va100a + " || OpenS: " + arrStatus[0] + " || OpenB: " + arrStatusB[0];
    // Comment(ab);
    //----
    ...
    ObjectSet("FiboDn", OBJPROP_FIBOLEVELS, 13);
    ...
    ObjectSet("FiboDn", OBJPROP_FIRSTLEVEL + 6, 1.000);
    ObjectSetFiboDescription("FiboDn", 6, "(Regiao Compra) -  %$");
    ...
    ObjectSet("FiboDn", OBJPROP_FIRSTLEVEL + 12, 2.000);
    ObjectSetFiboDescription("FiboDn", 12, "(Regiao Compra) -  %$");
...
void fIsGetSTR_AlertSignal()
{
    ...
    for (int i = 0; i < arrSize; i++) {
        ...
        if (arrStatusB[i] == false && arrValue[i] != 0) {
            ...
            if (i > 0) {
                if ((diCusDn_0 <= arrValue[i] && diCusDn_0 >= arrValue[i - 1] && diCusDn_0 != 0 && diCusDn_0 != EMPTY_VALUE) || (diCusDn_0_1 <= arrValue[i] && diCusDn_0_1 >= arrValue[i - 1] && diCusDn_0_1 != 0 && diCusDn_0_1 != EMPTY_VALUE)) {
                    double ask = MarketInfo(sym, MODE_ASK);
                ...
                if ((diCusDn_0 <= arrValue[i] && diCusDn_0 != 0 && diCusDn_0 != EMPTY_VALUE) || (diCusDn_0_1 <= arrValue[i] && diCusDn_0_1 != 0 && diCusDn_0_1 != EMPTY_VALUE)) {
    ...
    for (int i = arrSize - 1; i >= 0; i--) {
        ...
        if (arrStatus[i] == false) {
            // Print("1");
            if (i == (arrSize - 1)) {
                if ((diCusDn_1 >= arrValue[i] && diCusDn_1 != 0 && diCusDn_1 != EMPTY_VALUE) || (diCusDn_1_1 >= arrValue[i] && diCusDn_1_1 != 0 && diCusDn_1_1 != EMPTY_VALUE)) {
                    Print("Vao lenh o day nao : " + i + " => " + arrValue[i]);
                    double bid = MarketInfo(sym, MODE_BID);
                ...
                if ((diCusDn_1 >= arrValue[i] && diCusDn_1 <= arrValue[i + 1] && diCusDn_1 != 0 && diCusDn_1 != EMPTY_VALUE) || (diCusDn_1_1 >= arrValue[i] && diCusDn_1_1 <= arrValue[i + 1] && diCusDn_1_1 != 0 && diCusDn_1_1 != EMPTY_VALUE)) {
                    double bid = MarketInfo(sym, MODE_BID);
                    double sl  = bid + stoploss * point;
                    double tp  = bid - takeprofit * point;
                    if (stoploss == 0) {
...
string hdCO              = "------ Use close opposite -----";
string hdCO1             = "------ Open Sell/Close Buy. Reverse -----";
bool   flagCloseOpposite = true; // Use close opposite
//+------------------------------------------------------------------+
int    b_count       = 0;
int    s_count       = 0;
int    pending_count = 0;
...
//+------------------------------------------------------------------+
string ______________   = "---- Display EA ----";
bool   ShowTradeComment = TRUE;
int    magic            = 2024536; // Magic Number
int    MaxOpenOrders    = 15;
double SafeEquityRisk   = 0.5;
bool   FreezeAfterTP    = FALSE;
...
bool IsTradeTime()
{
    ...
    int day         = DayOfWeek();
    if (day < StartingTradeDay || day > EndingTradeDay)
        AllowTrade = false;
    int minuntes = 60 * TimeHour(TimeCurrent()) + TimeMinute(TimeCurrent());
    if (day <= StartingTradeDay && gStartMinutes >= minuntes)
        AllowTrade = false;
    if (day >= EndingTradeDay && gStopMinutes < minuntes)
        AllowTrade = false;
    return (AllowTrade);
}
//+------------------------------------------------------------------+
...
void f0_3()
{
    ...
    if (balanceDeviation() > MaxDD)
        MaxDD = balanceDeviation();
    ls_3 = "Margin Usage:                            " + DoubleToStr(100 - (AccountFreeMargin() / AccountBalance() * 100), 2) + "%\n";
    if (!IsTradeTime())
        ls_1 = "New Trades disallowed by scheduler";
    if (FreezeAfterTP)
        ls_1 = "Freeze AfterTP Enabled";
...
double calculatePLBalance()
{
    ...
    for (int li_4 = 0; li_4 < li_0; li_4++) {
        ...
        if (OrderMagicNumber() == magic)
            gd_TotalPL += OrderProfit() + OrderSwap() + OrderCommission();
    }
    return (gd_TotalPL);
}
//+------------------------------------------------------------------+
...
void f0_13()
{
    if (ShowTradeComment) {
        ...
        if (ObjectFind("BG") < 0) {
            ...
            ObjectSetText("BG", "g", 210, "Webdings", DodgerBlue);
        ...
        if (ObjectFind("BG1") < 0) {
            ...
            ObjectSetText("BG1", "g", 210, "Webdings", Black);
        ...
        if (ObjectFind("BG2") < 0) {
            ...
            ObjectSetText("BG2", "g", 210, "Webdings", Black);
        ...
        if (ObjectFind("NAME") < 0) {
            ...
            ObjectSetText("NAME", "MYWALLETFX - " + Symbol(), 9, "Arial Bold", White);
        ...
        if (ObjectFind("BG3") < 0) {
            ...
            ObjectSetText("BG3", "g", 110, "Webdings", Black);
        ...
        if (ObjectFind("BG5") < 0) {
            ...
            ObjectSetText("BG5", "g", 210, "Webdings", Black);
...
void AutomaticTPSLControl()
{
    for (int i = OrdersTotal() - 1; i >= 0; i--) {
        if (OrderSelect(i, SELECT_BY_POS) && OrderSymbol() == _Symbol && OrderMagicNumber() == magico) {

            double distanceTP = fabs(OrderTakeProfit() - OrderOpenPrice());
            double distanceSL = fabs(OrderStopLoss() - OrderOpenPrice());
            double newSL;
            if (distanceTP != distanceSL) {
                if (OrderType() == OP_BUY) {
                    newSL = OrderOpenPrice() - distanceTP;
                }
