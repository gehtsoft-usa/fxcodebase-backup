...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=75388
...
input bool open_new_buy_on  = true; // Open New Buy
input bool open_new_sell_on = true; // Open New Sell
bool       av_to_buy        = true;
bool       av_to_sell       = true;
...
#define Section_OpenTradesByTime
#ifdef Section_OpenTradesByTime
// Mark: Section_OpenTradesByTime

input string Topentrades          = "== Open Trades by Time =="; // ————————————————————————
input bool   open_trades_by_tm_on = true;                        // Open New trade by time?
input int    tmBtwTrades          = 5;                           // Minutes between trades:
bool         av_to_buy_by_tm      = false;
bool         av_to_sell_by_tm     = false;

class ConditionOpenBuysByTime : public iConditions
{
    int      tkOriginal;
    datetime nextTm;
  ...
  public:
    bool evaluate()
    {
        ...
        if (tkOriginalWasClose()) {
            av_to_buy_by_tm = false;
            acCloseAll.execute();
            tkOriginal=0;
            nextTm=0;
        }

        if (!av_to_buy_by_tm) haveBuy();

        if (av_to_buy_by_tm) {
            if (TimeCurrent() >= nextTm) {
                nextTm += tmBtwTrades * 60;
                return true;
            }
        }
        return false;
    ...
    {
        for (int i = OrdersTotal() - 1; i >= 0; i--) {
            if (OrderSelect(i, SELECT_BY_POS) && OrderSymbol() == _Symbol && OrderMagicNumber() != magico && OrderTicket() != tkOriginal) {
                if (OrderType() == OP_BUY) {
                    tkOriginal      = OrderTicket();
                    av_to_buy_by_tm = true;
                    datetime openTm = TimeCurrent();
                    Print("line: ", __LINE__, " openTm: ", openTm);
                    nextTm = openTm + tmBtwTrades * 60;
    ...
    bool tkOriginalWasClose()
    {
        if (OrderSelect(tkOriginal, SELECT_BY_TICKET)) {
            if (OrderCloseTime() > 0) {
                return true;
            }
        }
...
ConditionOpenBuysByTime cdOpenBuysByTime;

class ConditionOpenSellsByTime : public iConditions
{
  ...
  public:
    ...
    {
        if (tkOriginalWasClose()) {
            av_to_sell_by_tm = false;
            acCloseAll.execute();
            tkOriginal=0;
            nextTm=0;
        }

        if (!av_to_sell_by_tm) haveSell();
...
ConditionOpenSellsByTime cdOpenSellsByTime;

Strategy stOpenBuysByTime;
Strategy stOpenSellsByTime;

void OnInit_OpenTradesByTime()
{
    if (!open_trades_by_tm_on) return;
    stOpenBuysByTime.set(&cdOpenBuysByTime, &buy);
    stOpenSellsByTime.set(&cdOpenSellsByTime, &sell);
    Trader.strategys.add(&stOpenBuysByTime);
    Trader.strategys.add(&stOpenSellsByTime);
}
...
class BUYcondition1 : public iConditions
{
  ...
  public:
    bool evaluate()
    {
        ...
        // si hay un buy en el gráfico o se abre un nuevo
        if (open_new_buy_on) {
            if (!av_to_buy) return false;
            if (haveBuy()) return true;
        }
        return false;
    }
...
class SELLcondition1 : public iConditions
{
  ...
  public:
    bool evaluate()
    {
        ...
        if (open_new_sell_on) {
            if (!av_to_sell) return false;
            if (haveSell()) return true;
        }
        return false;
    }
...
void BasicStrategy_Iniciate()
{
    ...
    // StrategyToCloseBuy.set(&conditionCloseBuy, &acCloseAll);
    // StrategyToCloseSell.set(&conditionCloseSell, &acCloseAll);
