...
public:
   ...
   virtual bool IsPass(const int period, const datetime date)
   {
      ...
      double stochk = iStochastic(_symbol, _timeframe, Stoch_K, Stoch_D, Stoch_Slowing, MODE_SMA, 0, MODE_MAIN, 0);
      double stochd = iStochastic(_symbol, _timeframe, Stoch_K, Stoch_D, Stoch_Slowing, MODE_SMA, 0, MODE_SIGNAL, 0);
      double stochrsid = iCustom(_symbol, _timeframe, "Stochastic_RSI_MTF_basic", _timeframe, 14, 5, 3, 3, 1, 0);
      double rlw = iWPR(_symbol, _timeframe, Williams_Percent_Periods, 0);
      return dmip > dmim && stochk > stochd && stochd < Stoch_OB_Level && stochrsid < StochRSI_OB_Level && rlw < Williams_Percent_OB_Level;
   }
};
...
public:
   ...
   virtual bool IsPass(const int period, const datetime date)
   {
      ...
      double stochrsid = iCustom(_symbol, _timeframe, "Stochastic_RSI_MTF_basic", _timeframe, 14, 5, 3, 3, 1, 0);
      double rlw = iWPR(_symbol, _timeframe, Williams_Percent_Periods, 0);
      return dmim > dmip && stochk < stochd && stochd > Stoch_OS_Level && stochrsid > StochRSI_OS_Level && rlw > Williams_Percent_OS_Level;
   }
};
...
int init()
{
   ...
   IndicatorName = GenerateIndicatorName("MTF_MCP_Scanner");
