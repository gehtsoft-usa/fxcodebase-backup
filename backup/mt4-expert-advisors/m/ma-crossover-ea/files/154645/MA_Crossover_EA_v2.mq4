...
class BUYcondition1 : public iConditions
{
  public:
    bool evaluate()
    {
        ...
        bool filter, crossover;
        filter    = maFilterOn == true ? maSlow.index(1) > maFilter.index(1) : true;
        crossover = maFast.index(1) > maSlow.index(1) && maFast.index(2) <= maSlow.index(2);

        return filter && crossover;
    }
};
    ...
    bool evaluate()
    {
        ...
        for (int i = OrdersTotal() - 1; i >= 0; i--) {
            if (OrderSelect(i, SELECT_BY_POS) && OrderSymbol() == _Symbol && OrderMagicNumber() == magico && OrderType() == OP_BUY) {
                ...
                {
                    if(profitLastTrade>0) return false;
                }
                
                if(Distancia(Ask, precioUltimoTrade) < Martingale_gap) return false;
            }
        }
        return true;
    }
...
double floatingEA(string side = "")
{
    ...
    if (side == "buy") return profitBuy;
    if (side == "sell") return profitSell;
