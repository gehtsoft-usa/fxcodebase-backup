...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=74582
...
enum CloseAllMode {
    ...
    // CloseByPips            // By Pips
    CloseBySide,         // by profit on Side
    CloseBySideOnOposite // by Profit when oposite
};
...
string       file_custom_indicator = "Scalper X2";
int          bufferToBuy           = 0;
int          bufferToSell          = 1;

input string Tindi                 = "== Indicator Setup =="; // ————————————
input int    scalperx2_par         = 7;                       // Scalper x2 par

double       Indi(int buffer, int candle)
{
    return iCustom(NULL, 0, file_custom_indicator,
                   scalperx2_par, 1500, false, false, false,
                   buffer, candle);
}
#endif
...
input double       closeMoneyBuy           = 100;                       // $ to close Buy
input double       closeMoneySell          = 100;                       // $ to close Sell
...
input string tMatingale                = "== Martingale Setup =="; // == Martingale Setup ==
input bool   MartingaleON              = true;                     // Use Martingale:
input bool   Martingale_lastIsNegative = true;                     // Open another position on loss:
...
input int    Martingale_gap            = 20;                       // Min Pips Between Orders:
...
class MartingaleIncrementLot
{
  ...
  public:
    ...
    void setLastValues(string side = "")
    {
        for (int i = OrdersTotal() - 1; i >= 0; i--) {
            if (OrderSelect(i, SELECT_BY_POS, MODE_TRADES) && OrderSymbol() == _symbol && OrderMagicNumber() == _magic) {
                if (side == "buy" && OrderType() == OP_BUY) {
                    _lastLot = OrderLots();
                    return;
                } else if (side == "sell" && OrderType() == OP_SELL) {
                    _lastLot = OrderLots();
                    return;
                }
            }
...
class BUYcondition1 : public iConditions
{
  ...
  public:
    bool evaluate()
    {
        ...
        double buySignal = Indi(bufferToBuy, 1);
        double prev_buySignal = Indi(bufferToBuy, 2);
        if (buySignal != EMPTY_VALUE && buySignal > 0 && buySignal != lastSignal && prev_buySignal == -1) {
...
class BUYcondition2 : public iConditions
{
  public:
    bool evaluate()
    {
        ...
        double precioUltimoTrade = 0;
        for (int i = OrdersTotal() - 1; i >= 0; i--) {
            if (OrderSelect(i, SELECT_BY_POS) && OrderSymbol() == _Symbol && OrderMagicNumber() == magico && OrderType() == OP_BUY) {
                precioUltimoTrade      = OrderOpenPrice();

                double profitLastTrade = OrderProfit();
                if (Martingale_lastIsNegative) {
                    if (profitLastTrade > 0)
                        return false;
                }

                if (Distancia(Ask, precioUltimoTrade) < Martingale_gap)
                    return false;
            }
        }
        return true;
...
class SELLcondition1 : public iConditions
{
  ...
  public:
    bool evaluate()
    {
        ...
        double sellSignal      = Indi(bufferToSell, 1);
        double prev_SellSignal = Indi(bufferToSell, 2);
        if (sellSignal != EMPTY_VALUE && sellSignal > 0 && sellSignal != lastSignal && prev_SellSignal == -1) {
  ...
  public:
    bool evaluate()
    {
        ...
        if (closeAllControlON)
            if (CloseAllControl("") || CloseAllControl("buy"))
                return true;
...
class ConditionToCloseSell : public iConditions
{
  public:
    bool evaluate()
    {
        ...
        if (closeAllControlON)
            if (CloseAllControl("") || CloseAllControl("sell"))
                return true;
...
void OnTick()
{
    ...
    // string profitFromStart= (string)stats.ProfitFromStart();
    // string profitToday= (string)stats.ProfitToday();
    // string equity= AccountInfoDouble(ACCOUNT_EQUITY);

    string txt = "---- Stats ----\n" +
                 "PROFITS FLOATING: " + (string)stats.open_Profit() + "\n" +
                 "MAX DROWDOWN: " + (string)stats.MaxDrowDown() + "\n" +
                 "PROFITS TODAY: " + (string)stats.ProfitToday() + "\n" +
                 "PROFITS YESTERDAY: " + (string)stats.ProfitYesterday() + "\n" +
                 "PROFITS EVER: " + (string)stats.ProfitFromStart() + "\n" +
    ...
    if (!uTradeReverse) {
        if (conditionsToBuy.EvaluateConditions()) {
            if (modeEntry == Market) {
                actionSendOrder = new SendNewOrder("buy", Lots("buy"), "", 0, SL("buy"), TP("buy"), magico);
            } else {
                double pr       = Price("buy", uEntryDistance, _Symbol);
                actionSendOrder = new SendNewOrder("buy", Lots("buy"), "", pr, SL("buy", pr), TP("buy", pr), magico);
            }
        ...
        if (conditionsToSell.EvaluateConditions()) {
            if (modeEntry == Market) {
                actionSendOrder = new SendNewOrder("sell", Lots("sell"), "", 0, SL("sell"), TP("sell"), magico);
            } else {
                double pr       = Price("sell", uEntryDistance, _Symbol);
                actionSendOrder = new SendNewOrder("sell", Lots("sell"), "", pr, SL("sell", pr), TP("sell", pr), magico);
            }
            if (actionSendOrder.doAction()) {
                mainOrders.GetMarketOrders();
...
double floatingEA(string side = "")
{
    ...
    for (int i = OrdersTotal() - 1; i >= 0; i--) {
        if (OrderSelect(i, SELECT_BY_POS) && OrderSymbol() == _Symbol && OrderMagicNumber() == magico) {
            ...
            if (side == "sell" && OrderType() == OP_SELL)
...
// clang-format off
bool CloseAllControl(string side ="")
{
   switch (closeBy)
   {
      case CloseByMoney:
         ...
         case CloseBySide:
         if (side =="buy" && floatingEA("buy") >= closeMoneyBuy && closeMoneyBuy>0) { return true; }
         if (side =="sell" && floatingEA("sell") >= closeMoneySell && closeMoneySell>0) { return true; }
         break;

         case CloseBySideOnOposite:
         if (side =="buy" && floatingEA("buy") >= closeMoneyBuy && closeMoneyBuy>0 && sellCondition1.evaluate()==true) { return true; }
         if (side =="sell" && floatingEA("sell") >= closeMoneySell && closeMoneySell>0 && buyCondition1.evaluate()==true ) { return true; }
         break;
