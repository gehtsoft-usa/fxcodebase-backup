...
class ActionCloseBuys : public iActions
{
  public:
    bool execute()
    {
...
ActionCloseBuys acCloseBuys;

class ActionCloseSells : public iActions
{
  public:
    bool execute()
    {
...
input stop_loss_mode sl_mode        = sl_null;       // SL Mode:
...
input const double grid_step         = 10;                   // Grid Step:
input double       grid_m            = 2;                    // Grid Lot multiplier:
input const int    grid_max_steps    = 5;                    // Grid Max Steps:
double       grid_close_profit = 0;                   // Profit to close (0 = off):
double       grid_close_loss   = 0;                   // Loss to close (0 = off):
...
class ActionGrid : public iActions
{
  public:
        ...
        if (OrderType() == OP_BUY) {
            for (int n = 1; n <= grid_max_steps; n++) {
                ...
                if (Ask <= last_step && !HaveOrderInThisStep(grid_id)) {
                    int r = OrderSend(NULL, OP_BUY, GridLot(origin_lot, (double)n), Ask, 10000, 0, 0, grid_id, magico, 0, SkyBlue);
                }
            }
        }
        ...
        if (OrderType() == OP_SELL) {
            for (int n = 1; n <= grid_max_steps; n++) {
                ...
                if (Bid >= last_step && !HaveOrderInThisStep(grid_id)) {
                    int r = OrderSend(NULL, OP_SELL, GridLot(origin_lot, (double)n), Bid, 10000, 0, 0, grid_id, magico, 0, Pink);
                }
            }
        }
    ...
    bool HaveOrderInThisStep(string grid_id)
    {
        ...
        for (int i = OrdersTotal() - 1; i >= 0; i--) {
            if (OrderSelect(i, SELECT_BY_POS) && OrderSymbol() == _Symbol && OrderMagicNumber() == magico) {
                if (StringFind(OrderComment(), grid_id) >= 0) {
...
class ConditionCloseGridByProfit : public iConditions
{
  public:
    bool evaluate()
    {
        ...
        for (int i = OrdersTotal() - 1; i >= 0; i--) {
            if (OrderSelect(i, SELECT_BY_POS) && OrderSymbol() == _Symbol && OrderMagicNumber() == magico && OrderComment() == grid_id) {
                current_profit += OrderProfit() + OrderCommission() + OrderSwap();
            }
        }

        // vuelvo a seleccionar el tk donde estaba antes del recorrido:
        int s = OrderSelect(tk, SELECT_BY_TICKET);

        if (grid_close_profit > 0 && current_profit >= grid_close_profit) return true;
        if (grid_close_loss > 0 && current_profit <= -grid_close_loss) return true;
...
class ActionCloseGrid : public iActions
{
  public:
    bool execute()
    {
        ...
        for (int i = OrdersTotal() - 1; i >= 0; i--) {
            if (OrderSelect(i, SELECT_BY_POS) && OrderSymbol() == _Symbol && OrderMagicNumber() == magico && (StringFind(OrderComment(), grid_id) >= 0 || OrderTicket() == tk)) {
                double price = OrderType() == OP_BUY ? Bid : Ask;
...
Strategy stCloseGridByProfit(&cdCloseGridByProfit, &acCloseGrid);
Strategy stCloseGridBecauseOriginalTradeIsClose(&cdOriginalTkIsClose, &acCloseGrid);

void OnInit_Grid()
{
    if (!grid_on) return;
    Trader.management.add(&stGrid);
    Trader.management.add(&stCloseGridByProfit);
...
#define Section_grid_p_Positive
#ifdef Section_grid_p_Positive
// Mark: Section_grid_p_Positive

input string       tGrid_p             = "== Grid Setup =="; // ————————————————————————
...
double       grid_p_close_profit = 0;                   // Profit to close (0 = off):
double       grid_p_close_loss   = 0;                   // Loss to close (0 = off):
...
class ActionGridPositive : public iActions
{
  public:
    ...
    {
        // Lot increment by multiplier
        double l = lot * pow(grid_p_m, (double)n);
        return l;
    }
};
...
Strategy stCloseGridPositiveBecauseOriginalTradeIsClose(&cdOriginalTkIsClosePositive, &acCloseGrid);
...
#define Section_CloseByMoney
#ifdef Section_CloseByMoney
// Mark: Section_CloseByMoney

input string tcloseall    = "== Close All Setup =="; // ————————————————————————
input bool   closeAll_on  = false;                    // Close All on?
input double profit_limit = 100;                     // Profit Limit (0 = off)
input double loss_limit   = 100;                     // Loss Limit (0 = off)

class ConditionCloseByMoney : public iConditions
{
  public:
    bool evaluate()
    {
        ...
        if (profitAcum >= profit_limit && profit_limit != 0)
        {
            return true;
        }
        if (fabs(profitAcum) >= fabs(loss_limit) && loss_limit != 0 && profitAcum < 0)
        {
            return true;
        }
        return false;
...
ConditionCloseByMoney cdCloseByMoney;

class ActionCloseByMoney : public iActions
{
  public:
    bool execute() { 
        for (int i = OrdersTotal() - 1; i >= 0; i--) {
...
ActionCloseByMoney acCloseByMoney;

Strategy stCloseByMoney;

void OnInit_CloseByMoney()
{
    if(!closeAll_on) return;
    stCloseByMoney.set(&cdCloseByMoney, &acCloseByMoney);
    Trader.management.add(&stCloseByMoney);
}
...
input const string indicator_file = "MILLIONTRADE";              // Indicator File:
input double opti                  = 1.0; // Optimization:
input bool indi_alerts_on          = false; // Alerts:
input int arrow_size               = 8; // Chose the arrow size;
...
//---
double Indi(int buffer, int candle = 1) { return iCustom(NULL, 0, indicator_file, opti, indi_alerts_on, arrow_size, buffer, candle); }
...
int indicator_direction = 0;

ConditionIndicatorSell indicatorSell;
ConditionIndicatorBuy indicatorBuy;
...
class ConditionCloseOnOppositeBuy : public iConditions
{
  public:
    bool evaluate()
    {
        bool r = false;
        if (OrderType() == OP_BUY) {
            r = indicatorSell.evaluate();
            // if(indicator_direction == -1) r = true;
        }
        return r;
    }
};
...
{
    closeOppositeBuy.set(&cdCloseOnOppositeBuy, &acCloseBuys);
    closeOppositeSell.set(&cdCloseOnOppositeSell, &acCloseSells);
