...
//http://fxcodebase.com/code/viewtopic.php?f=38&t=70529
...
input int ma_1_period = 50; // MA 1 period
input ENUM_MA_METHOD ma_1_method = MODE_EMA; // Smoothing method
input int ma_2_period = 100; // MA 2 period
input ENUM_MA_METHOD ma_2_method = MODE_EMA; // Smoothing method
input int tenkan = 9; // Tenkan
input int kijun = 26; // Kijun
input int senkou = 52; // Senkou
...
ICondition* CreateLongCondition(string symbol, ENUM_TIMEFRAMES timeframe)
{
   ...
   condition.Add(new MAMACondition(symbol, timeframe, FirstAboveSecond, ma_1_method, ma_1_period, PRICE_CLOSE, ma_2_method, ma_2_period, PRICE_CLOSE), false);
...
ICondition* CreateLongFilterCondition(string symbol, ENUM_TIMEFRAMES timeframe)
{
   ...
   AndCondition* condition = new AndCondition();
   IStream* stream1 = new IchimokuStdCloudStream(symbol, timeframe, tenkan, kijun, senkou, false);
   IStream* stream2 = new PriceStream(symbol, timeframe, PriceClose);
   condition.Add(new StreamStreamCondition(symbol, timeframe, FirstAboveSecond, stream1, stream2, "Price", "Cloud"), false);
   stream1.Release();
   stream2.Release();
   return condition;
}
...
ICondition* CreateShortCondition(string symbol, ENUM_TIMEFRAMES timeframe)
{
   ...
   AndCondition* condition = new AndCondition();
   condition.Add(new MAMACondition(symbol, timeframe, FirstBelowSecond, ma_1_method, ma_1_period, PRICE_CLOSE, ma_2_method, ma_2_period, PRICE_CLOSE), false);
   #ifdef ACT_ON_SWITCH_CONDITION
      ActOnSwitchCondition* switchCondition = new ActOnSwitchCondition(symbol, timeframe, (ICondition*) condition);
      condition.Release();
      return switchCondition;
...
ICondition* CreateShortFilterCondition(string symbol, ENUM_TIMEFRAMES timeframe)
{
   ...
   AndCondition* condition = new AndCondition();
   IStream* stream1 = new IchimokuStdCloudStream(symbol, timeframe, tenkan, kijun, senkou, true);
   IStream* stream2 = new PriceStream(symbol, timeframe, PriceClose);
   condition.Add(new StreamStreamCondition(symbol, timeframe, FirstBelowSecond, stream1, stream2, "Price", "Cloud"), false);
   stream1.Release();
   stream2.Release();
   return condition;
}
