...
input string symbol_1 = "EURUSD"; // MFI Symbol 1
input int mfi_period_1 = 14; // MFI Period #1
input ENUM_TIMEFRAMES tf_1 = PERIOD_CURRENT; // MFI Timeframe #1
input string symbol_2 = "USDJPY"; // MFI Symbol 2
input int mfi_period_2 = 14; // MFI Period #2
input ENUM_TIMEFRAMES tf_2 = PERIOD_CURRENT; // MFI Timeframe #2
...
public:
   ...
   {
      return mfi1[period] > mfi2[period];
   }
};
...
int init()
{
   ...
   IndicatorBuffers(10);
   IndicatorObjPrefix = GenerateIndicatorPrefix("mfica");
   IndicatorShortName("2 MFI Cross Alert");
   ...
   SetIndexBuffer(id, mfi1);
   SetIndexLabel(id, "MFI #1");
   ++id;
   SetIndexStyle(id, DRAW_LINE);
   SetIndexBuffer(id, mfi2);
   SetIndexLabel(id, "MFI #2");
   ++id;
...
int start()
{
   ...
   {
      ArrayInitialize(mfi1, EMPTY_VALUE);
      ArrayInitialize(mfi2, EMPTY_VALUE);
      if (customStream != NULL)
      {
         customStream.Init();
      }
   ...
   {
      int index1 = pos == 0 ? 0 : iBarShift(symbol_1, tf_1, Time[pos]);
      if (index1 >= 0)
      {
         mfi1[pos] = iCustom(symbol_1, tf_1, "MFI", mfi_period_1, 0, index1);
      }
      int index2 = pos == 0 ? 0 : iBarShift(symbol_2, tf_2, Time[pos]);
      if (index2 >= 0)
      {
         mfi2[pos] = iCustom(symbol_2, tf_2, "MFI", mfi_period_2, 0, index2);
      }
      if (customStream != NULL)
      {
         customStream.SetValue(pos, mfi1[pos]);
      }
      for (int i = 0; i < ArraySize(conditions); ++i)
      {
         AlertSignal* item = conditions[i];
