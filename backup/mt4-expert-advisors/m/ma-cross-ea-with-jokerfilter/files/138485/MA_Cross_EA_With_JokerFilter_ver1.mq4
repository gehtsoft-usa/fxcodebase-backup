...
//http://fxcodebase.com/code/viewtopic.php?f=38&t=70444
...
enum AlgoType
{
   AlgoDouble, // Double Confirmation
   AlgoFilter, // Only Filter
   AlgoCross // Only Moving Average Crossover
};

input int short_ma_period = 14; // Short MA Period
input int shift = 2; // Placing Order at which Candle
input ENUM_MA_METHOD short_ma_method = MODE_SMA; // Smoothing method
input ENUM_APPLIED_PRICE short_ma_price = PRICE_CLOSE; // Price type
input int long_ma_period = 21; // Long MA Period
input ENUM_MA_METHOD long_ma_method = MODE_SMA; // Smoothing method
input ENUM_APPLIED_PRICE long_ma_price = PRICE_CLOSE; // Price type
input AlgoType algo_type = AlgoDouble; // Mode
...
input bool smart_move = true; // Smart Move
input double lots_increase = 0.1; // Smart move increase;
input int max_smart_move = 3; // How many times Smart move
...
enum CloseAtProfitType
{
   CAPDoNotClose, // Nothing
   CAPFixed, // Fixed value
   CAPEquityPercent, // Equity Percentage
   CAPEquityLevel // Equity should reach to
};
input CloseAtProfitType close_at_profit = CAPDoNotClose; // Close at profit
input double close_at_profit_value = 100; // Close at profit value
enum AfterCloseAtProfitType
{
   ACAPRestart, // Restart
   ACAPStop // Stop   
};
input AfterCloseAtProfitType after_close_at_profit = ACAPRestart; // After Close at Profit
...
public:
   ...
   virtual bool IsPass(const int period, const datetime date)
   {
      if (_percentage)
      {
         return AccountBalance() * (1 + _totalProfit / 100.0) <= AccountEquity();
      }
...
public:
   ...
   virtual double GetValue(int period, double entryPrice)
   {
      ...
      return _calculator.GetLots(_lotsType, current_lots, MathAbs(sl - entryPrice));
   }
};
...
public:
   ...
   virtual double GetValue(int period, double entryPrice)
   {
      return _calculator.GetLots(_lotsType, current_lots, 0.0);
   }
};
...
public:
   ...
   {
      int row = 0;
      cells0.Add("Left till close", color_text, _fontName, _fontSize, 0, row);
      double left;
      switch (close_at_profit)
      {
         case CAPFixed:
            left = close_at_profit_value - (AccountEquity() - AccountBalance());
            break;
         case CAPEquityPercent:
            left = (AccountBalance() * (1 + close_at_profit_value / 100.0)) - AccountEquity();
            break;
         case CAPEquityLevel:
            left = close_at_profit_value - AccountEquity();
            break;
      }
      cells0.Add(DoubleToString(left, 2), color_text, _fontName, _fontSize, 1, row++);
...
class StopAction : public AAction
{
public:
   StopAction()
   {
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      double value = iCustom(_symbol, _timeframe, "Filter", 1, period + shift - 1);
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      double value = iCustom(_symbol, _timeframe, "Filter", 0, period + shift - 1);
      return value != 0 && value != EMPTY_VALUE;
   }
};
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      ...
      double value10 = iMA(_symbol, _timeframe, short_ma_period, 0, short_ma_method, short_ma_price, period + shift);
      double value11 = iMA(_symbol, _timeframe, short_ma_period, 0, short_ma_method, short_ma_price, period + 1 + shift);
      double value20 = iMA(_symbol, _timeframe, long_ma_period, 0, long_ma_method, long_ma_price, period + shift);
      double value21 = iMA(_symbol, _timeframe, long_ma_period, 0, long_ma_method, long_ma_price, period + 1 + shift);
...
ICondition* CreateLongCondition(string symbol, ENUM_TIMEFRAMES timeframe)
{
   ...
   if (algo_type != AlgoCross)
   {
      condition.Add(new LongCondition(symbol, timeframe), false);
   }
   else
   {
      condition.Add(new MAMACondition(symbol, timeframe, FirstAboveSecond, short_ma_method, short_ma_period, short_ma_price, long_ma_method, long_ma_period, long_ma_price, shift - 2, shift - 2), false);
   }
...
ICondition* CreateShortCondition(string symbol, ENUM_TIMEFRAMES timeframe)
{
   ...
   {
      condition.Add(new MAMACondition(symbol, timeframe, FirstBelowSecond, short_ma_method, short_ma_period, short_ma_price, long_ma_method, long_ma_period, long_ma_price, shift - 2, shift - 2), false);
   }
   #ifdef ACT_ON_SWITCH_CONDITION
      ActOnSwitchCondition* switchCondition = new ActOnSwitchCondition(symbol, timeframe, (ICondition*) condition);
      condition.Release();
...
TradingController *CreateController(const string symbol, const ENUM_TIMEFRAMES timeframe, string algoId, string &error)
{
   ...
   IAction* closeAllAction = new CloseAllAction(magic_number, slippage_points);
   if (mandatory_closing)
   {
      NotCondition* condition = new NotCondition(tradingTimeCondition);
      actions.AddActionOnCondition(closeAllAction, condition);
      condition.Release();
   }
   switch (close_at_profit)
   {
      ...
      case CAPFixed:
         {
            HitTotalProfitCondition* condition = new HitTotalProfitCondition(close_at_profit_value, false);
            actions.AddActionOnCondition(closeAllAction, condition);
            if (after_close_at_profit == ACAPStop)
            {
               StopAction* stopAction = new StopAction();
               actions.AddActionOnCondition(stopAction, condition);
               stopAction.Release();
            }
            condition.Release();
         }
      ...
      case CAPEquityPercent:
         {
            HitTotalProfitCondition* condition = new HitTotalProfitCondition(close_at_profit_value, true);
            actions.AddActionOnCondition(closeAllAction, condition);
            if (after_close_at_profit == ACAPStop)
            {
               StopAction* stopAction = new StopAction();
      ...
      case CAPEquityLevel:
         {
            HitEquityLevelCondition* condition = new HitEquityLevelCondition(close_at_profit_value);
            actions.AddActionOnCondition(closeAllAction, condition);
            if (after_close_at_profit == ACAPStop)
            {
               StopAction* stopAction = new StopAction();
...
int OnInit()
{
   double temp = iCustom(NULL, 0, "Filter", 0, 0);
   if (GetLastError() == ERR_INDICATOR_CANNOT_LOAD)
   {
      Alert("Please, install the 'Filter' indicator");
      return INIT_FAILED;
   }
   #ifdef SHOW_ACCOUNT_STAT
      stats = NULL;
   ...
   #ifdef SHOW_ACCOUNT_STAT
      if (close_at_profit != CAPDoNotClose)
      {
         stats = new AccountStatistics(EA_NAME);
      }
   #endif
   OnTradeClosedAction* onTradeClosedAction = new OnTradeClosedAction();
...
public:
   ...
   virtual bool DoAction(const int period, const datetime date)
   {
      ...
      if (OrderClosePrice() == OrderStopLoss() || StringFind(OrderComment(), "[sl]", 0) != -1)
      {
         if (mult_times < max_smart_move)
         {
            ++mult_times;
            current_lots += lots_increase;
         }
      }
