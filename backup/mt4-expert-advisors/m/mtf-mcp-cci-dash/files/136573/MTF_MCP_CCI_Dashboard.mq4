...
// based on Se7enTrader http://fxcodebase.com/code/viewtopic.php?f=38&t=65346
#property description "Dashboard shows CCI values on multi-timeframes and multi-currency, with 2 threshold notification"

//--- input parameters
input int                CCI_Period = 14;  // CCI Period
input ENUM_APPLIED_PRICE CCIApplied = 5;   // CCI Applied Price: 0 - Close, 1 - Open, 2 - High, 3 - Low, 4 - Median, 5 - Typical, 6 - Weighted
input double             MinCCI     = 30;  // CCI OverThreshold1 Level
input double             MaxCCI     = 100; // CCI OverThreshold2 Leve2
...
public:
   ...
   {
      double cciValue = iCCI(_symbol, _timeframe, CCI_Period, CCIApplied, period);
      return cciValue <= MinCCI;
   }
};
...
class IndicatorTextFormatter : public AValueFormatter
{
   string _text;
   color _clr;
   color _bgClr;
   ...
   ENUM_TIMEFRAMES _timeframe;
public:
   IndicatorTextFormatter(const string symbol, const ENUM_TIMEFRAMES timeframe, string text, color clr, color bgClr)
   {
      _bgClr = bgClr;
      _text = text;
      _clr = clr;
   ...
   {
      double cciValue = iCCI(_symbol, _timeframe, CCI_Period, CCIApplied, period);
      string value = DoubleToString(cciValue, 2);
      clr = _clr;
      bgColor = _bgClr;
      return value + " " + _text;
   }
...
public:
   ...
   {
      IValueFormatter* defaultValue = new IndicatorTextFormatter(symbol, timeframe, "-", GetTextColor(_neutralColor), GetBackgroundColor(_neutralColor));
      ...
      IValueFormatter* upValue = new IndicatorTextFormatter(symbol, timeframe, "Buy", GetTextColor(_upColor), GetBackgroundColor(_upColor));
      cell.AddCondition(upCondition, upValue, NULL, upValue);
      upCondition.Release();
      upValue.Release();
      
      ICondition* downCondition = new DownCondition(symbol, timeframe);
      IValueFormatter* downValue = new IndicatorTextFormatter(symbol, timeframe, "Sell", GetTextColor(_downColor), GetBackgroundColor(_downColor));
      cell.AddCondition(downCondition, downValue, NULL, downValue);
...
string IndicatorName = "MTF NCP CCI Dashboard";
int init()
{
   ...
   IndicatorObjPrefix = GenerateIndicatorPrefix("ccid");
