...
input string             Iema           = "== Moving Average 1 Setup =="; // == Moving Average 1 Setup ==
...
input string             Iema3           = "== Moving Average 3 Setup =="; // == Moving Average 3 Setup ==
input int                ma3Period       = 10;                             // Period
...
int  handle_ma3 = 0;
void setHandle_ma3() { handle_ma3 = iMA(NULL, 0, ma3Period, ma3Shift, ma3Method, ma3AppliedPrice); }

double MA3(int candle = 1)
{
    double value[1];
    // int    shift = Bars(_Symbol, _Period) - candle;
    int shift = candle;
    int copy  = CopyBuffer(handle_ma3, 0, shift, 1, value);
...
input bool   close_by_strategy_on = false;                   // Close positions by ema cross:
input bool closeWhenCrossWithEma2 = true;                    // Close when ema 1 cross ema 2:
input bool closeWhenCrossWithEma3 = true;                    // Close when ema 1 cross ema 3:
  ...
  public:
    bool evaluate()
    {
        ...
        if (PositionGetInteger(POSITION_TYPE) == POSITION_TYPE_BUY) {
            ...
            if(closeWhenCrossWithEma2)
            {
                return MA1(1) <  MA2(1) && MA1(2) >=  MA2(2);
            }
            if(closeWhenCrossWithEma3)
            {
                return MA1(1) <  MA3(1) && MA1(2) >=  MA3(2);
            }
        }
        return r;
    }
