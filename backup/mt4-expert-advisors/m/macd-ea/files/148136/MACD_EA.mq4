...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=72893
...
input double uPoints = 200; // Points between Trades:
...
input bool         closeAllInOpositeSignal = true;                       // Close in Oposite Signal
#else
string        TtpOptions              = "== Close Options ==";  // == Close Options ==
...
bool          closeAllInOpositeSignal = true;                       // Close In Oposite Signal
#endif
...
class BUYcondition1 : public iConditions
{
 public:
  bool evaluate()
  {
    ...
    if (macd.macd_line(1) > macd.signal_line(1) && macd.macd_line(2) < macd.signal_line(2) && macd.macd_line(1) > 0 && macd.signal_line(1) > 0) return true;
...
// el precio de apertura tiene que ser menor al min menos x pips (user)
if(Ask <= minPrice("buy")-uPoints*_Point) { return true; }
if(Ask >= maxPrice("buy")+uPoints*_Point) { return true; }

// ó el precio de apertura tiene que ser mayor al max, más x pips (user)
...
if(Bid <= minPrice("sell")-uPoints*_Point) { return true; }
if(Bid >= maxPrice("sell")+uPoints*_Point) { return true; }
    return false;
  }
};
...
double minPrice(string side)
{
  ...
  for (int i = OrdersTotal() - 1; i >= 0; i--)
  {
    if (OrderSelect(i, SELECT_BY_POS) && OrderSymbol() == _Symbol && OrderMagicNumber() == magico)
    {
      ...
      {
        if (price == 0 || OrderOpenPrice() < price)
        {
          price = OrderOpenPrice();
        }
      }
  ...
  // devolver el precio minimo al que se abrió un trade, según el side
	Print(__FUNCTION__," MIN price: ",price);
  return price;
}

double maxPrice(string side)
{
  double price = 0;
  ...
  for (int i = OrdersTotal() - 1; i >= 0; i--)
  {
    if (OrderSelect(i, SELECT_BY_POS) && OrderSymbol() == _Symbol && OrderMagicNumber() == magico)
    {
				Print(__FUNCTION__," ", i, " OrderOpenPrice: ",OrderOpenPrice());

      if (price == 0 || OrderOpenPrice() > price)
      {
				if (side == "buy" && OrderType() == OP_BUY) { price = OrderOpenPrice(); }
	      if (side == "sell" && OrderType() == OP_SELL) { price = OrderOpenPrice(); }
	    }
    }
  }

  // devolver el precio maximo al que se abrió un trade, según el side

	Print(__FUNCTION__," MAX price: ",price);
  return price;
}
