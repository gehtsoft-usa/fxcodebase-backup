//Available @  https://fxcodebase.com/code/viewtopic.php?f=38&p=156064#p156064
...
#define Section_EMA_Filter
#ifdef Section_EMA_Filter
// Mark: Section_EMA_Filter

input string             Iema           = "== Moving Average Setup =="; // == Moving Average Setup ==
input bool filter_by_ema_on = true; // Filter By EMA
...
int  handle_ma = 0;
void setHandleEMA() { handle_ma = iMA(NULL, 0, maPeriod, maShift, maMethod, maAppliedPrice); }
...
class Condition_close_up_ema : public iConditions
{
  public:
    bool evaluate()
    {
        return iClose(NULL,0,1) > MA(1);
    }
};
Condition_close_up_ema cd_close_up_ema;

class Condition_close_dn_ema : public iConditions
{
  public:
    bool evaluate()
    {
...
Condition_close_dn_ema cd_close_dn_ema;


void OnInit_EMA_Filter()
{
    if(!filter_by_ema_on) return;
    setHandleEMA();
    LongStrategy.addCondition(&cd_close_up_ema);
    ShortStrategy.addCondition(&cd_close_dn_ema);
}
...
input const string indicator_file = "MyDayIni_MT5.ex5";                          // Indicator File:
input int range = 5; // Range:
//---
int  bufferToBuy              = 4;
int  bufferToSell             = 5;
...
//---
int  handle = 0;
void setHandle() { handle = iCustom(Symbol(), NULL, indicator_file, range); }
...
ConditionIndicatorSell conditionToSell;
ConditionIndicatorBuy conditionToBuy;
