...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=73664
...
#property description "5. Allow \"Opposite direction\" to make the grid against the original trade"
...
input GridDirection gridDirection       = same;                // Grid Direction:
...
class Grid
{
   ...
   public:
    Grid(string Symbol, string Side, double LastPrice, double Gap, double Multiplier, int MaxQnt, double MaxLot, double InitialLot, int magic, bool simbolFilterOn = true, bool magicFilterOn = true, double tp = 0, double sl = 0, GridDirection direction = same)
    {
        _symbol     = Symbol;
        _side       = Side;
        ...
        // Set Conditions:
        int mode     = _direction == same ? 1 : 0;
        cdMatchPrice = new ConditionMatchPrice(_symbol, _side, _nextPrice, mode);
    ...
    void direction(GridDirection inpdirection)
    {
        _direction = inpdirection;
        if (_direction == opposite) cdMatchPrice.mode(0);
    }
    GridDirection direction(void) { return _direction; }

    void magico(int grid_magic)
    {
        _magico = grid_magic;
    }
        ...
        if (conditionsToOpenNewTrade.EvaluateConditions())
        {
            if (_direction == same)
            {
                if (_side == "buy")
                {
                    ...
                    {
                        if (gridOrders.GetLastMarketOrder("grid")) setNextTrade();
                    }
                    delete openTrade;
                }
                if (_side == "sell")
...
void CheckearOrdernesyGenerarGrids()
{
    ...
    if (!thisOrderHaveGrid(mainOrders.last().id()))
    {
        if (mainOrders.last().type() == OP_BUY)
        {
            ...
            {
                grids[t] = new Grid(_Symbol, "buy", mainOrders.last().price(), GridUser_gap, GridUser_multiplier, GridUser_maxCount, GridUser_maxLot, mainOrders.last().lot(), magico, true, true, 0, 0, gridDirection);
        ...
        if (mainOrders.last().type() == OP_SELL)
        {
            ...
            if (ArrayResize(grids, t + 1))
            {
                grids[t] = new Grid(_Symbol, "sell", mainOrders.last().price(), GridUser_gap, GridUser_multiplier, GridUser_maxCount, GridUser_maxLot, mainOrders.last().lot(), magico, true, true, 0, 0, gridDirection);
                grids[t].tk(mainOrders.last().id());
                grids[t].magico(1234);
            }
        }
