...
public:
   ...
   {
      double lowerValue0 = iBands(_symbol, _timeframe, _period, _dev, 0, PRICE_CLOSE, MODE_UPPER, period);
      double lowerValue1 = iBands(_symbol, _timeframe, _period, _dev, 0, PRICE_CLOSE, MODE_UPPER, period + 1);
...
public:
   ...
   bool IsPass(const int period)
   {
      double upperValue0 = iBands(_symbol, _timeframe, _period, _dev, 0, PRICE_CLOSE, MODE_LOWER, period);
      double upperValue1 = iBands(_symbol, _timeframe, _period, _dev, 0, PRICE_CLOSE, MODE_LOWER, period + 1);
      double close0 = iClose(_symbol, _timeframe, period);
      double close1 = iClose(_symbol, _timeframe, period + 1);
...
ICondition* CreateUpCondition(string symbol, ENUM_TIMEFRAMES timeframe, double deviations)
{
   return new BandCrossOverCondition(symbol, timeframe, Bollinger_Bands_Periods, deviations);
}

ICondition* CreateDownCondition(string symbol, ENUM_TIMEFRAMES timeframe, double deviations)
{
   return new BandCrossUnderCondition(symbol, timeframe, Bollinger_Bands_Periods, deviations);
}
...
public:
   TrendValueCell(const string id, const int x, const int y, const string symbol, const ENUM_TIMEFRAMES timeframe, double deviations)
   { 
      _signaler = new Signaler(symbol, timeframe);
      _signaler.SetMessagePrefix(symbol + "/" + _signaler.GetTimeframeStr() + "; Dev " + DoubleToString(deviations, 1) + ": ");
      _id = id; 
      _x = x; 
      _y = y; 
      _symbol = symbol; 
      _timeframe = timeframe; 
      _upCondition = CreateUpCondition(_symbol, _timeframe, deviations);
      _downCondition = CreateDownCondition(_symbol, _timeframe, deviations);
   }
      ...
      if (Time[0] != _lastSignalDate)
      {
         switch (direction)
         {
            case ENTER_BUY_SIGNAL:
               _signaler.SendNotifications("Cross over");
               _lastSignalDate = Time[0];
               break;
            case ENTER_SELL_SIGNAL:
               _signaler.SendNotifications("Cross under");
               _lastSignalDate = Time[0];
               break;
         }
      }
...
public:
   ...
   {
      _symbolsCount = 3;
      ArrayResize(_symbols, 3);
      ArrayResize(_devs, 3);
      _symbols[0] = DoubleToString(Bollinger_Bands_Deviations_1, 1);
      _symbols[1] = DoubleToString(Bollinger_Bands_Deviations_2, 1);
      _symbols[2] = DoubleToString(Bollinger_Bands_Deviations_3, 1);
      _devs[0] = Bollinger_Bands_Deviations_1;
      _devs[1] = Bollinger_Bands_Deviations_2;
      _devs[2] = Bollinger_Bands_Deviations_3;
   ...
   void AddTimeframe(const string label, const ENUM_TIMEFRAMES timeframe)
   {
      if (_verticalMode)
      {
         ...
         for (int i = 0; i < _symbolsCount; i++)
         {
            ...
            row.Add(new TrendValueCell(id, x, yIterator.GetNext(), _Symbol, timeframe, _devs[i]));
         }
      }
      else
      {
         ...
         {
            string id = IndicatorObjPrefix + _symbols[i] + "_" + label;
            row.Add(new TrendValueCell(id, xIterator.GetNext(), y, _Symbol, timeframe, _devs[i]));
         }
      }
   }
...
int init()
{
   ...
   IndicatorName = GenerateIndicatorName("Multi_bb_dashboard");
