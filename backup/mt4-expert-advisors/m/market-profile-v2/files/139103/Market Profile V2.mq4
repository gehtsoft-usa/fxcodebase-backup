...
//http://fxcodebase.com/code/viewtopic.php?f=38&t=70656
...
// based on Copyright © 2011, ahmad.yani@hotmail.com
...
//---input vars
int        LookBack;
input int        _LookBack                = 3;
input bool       UseVolumeProfile        = false;
input string     ProfileTimeframeInfo    = "use D, W, or M";
input string     ProfileTimeframe        = "W";
input int        DayStartHour            = 0;
input double     VATPOPercent            = 70.0;
input int        TickSize                = 1;
input int        ExtendedPocLines        = 3;

input string     spr0                    = "on/off settings..";
input bool       ShowPriceHistogram      = true;
input bool       ShowValueArea           = false;
input bool       ShowVAHVALLines         = false;
input bool       ShowOpenCloseArrow      = false;

int HistoHeight;
input string     spr1                    = "design & colors..";
input double     VolAmplitudePercent     = 40.0;
input int        _HistoHeight             = 2;
input color      HistoColor1             = Black;
input color      HistoColor2             = Black;
input color      OpenColor               = Black;
input color      CloseColor              = Black;
input color      POCColor                = White;
input color      VirginPOCColor          = White;
input color      VAColor                 = Black;
input color      VALinesColor            = Black;
input color      InfoColor               = Black;

input string     spr2                    = "Profile Data.............";
input int        DailyProfileDataTf      = 60;
input int        WeeklyProfileDataTf     = 240;
input int        MonthlyProfileDataTf    = 1440;
...
int init()
{
   visibility.Init("market_profile", "mp", "Show/Hide", button_x, button_y);
...
void drawInfo()
{
   ...
   ObjectSetText(gsPref+"lblinfo1", info, 3, "Impact", InfoColor);
   ...
   if (giProfileTf == PERIOD_D1)
   {
      ...
      ObjectSetText(gsPref+"lblinfo2", "DayStartHour: " + DayStartHour, 3, "Impact", InfoColor);
...
void createText(string name, datetime t, double p, string text
   , int size=3, string font="Impact", color c=White)
{
   name = gsPref + name;
