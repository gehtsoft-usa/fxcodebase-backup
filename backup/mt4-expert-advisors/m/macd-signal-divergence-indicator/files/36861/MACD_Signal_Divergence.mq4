...
int init()
  {
   IndicatorName = GenerateIndicatorName("MACD Signal Divergence");
...
void DrawLine(int First, int Second, bool BullFl)
{
 ...
 if (ObjectFind(ObjName)==-1)
 {
  ...
  if (WindowNumber!=-1)
  {
   ObjectCreate(ObjName, OBJ_TREND, WindowNumber, Time[First], MACD[First], Time[Second], MACD[Second]);
   ObjectSet(ObjName, OBJPROP_RAY, false);
   ObjectCreate(ObjName+"A", OBJ_ARROW, WindowNumber, Time[Second], MACD[Second]);
   if (BullFl)
   {
...
int start()
{
 ...
 while(pos>=0)
 {
  MACD[pos]=iMACD(NULL, 0, Fast, Slow, Signal, Price, MODE_SIGNAL, pos);
