...
//https://fxcodebase.com/code/viewtopic.php?f=38&t=71248
...
input int fast_ma_period = 7; // Fast MA period
input int slow_ma_period = 14; // Slow MA period
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      double macdValue = iMACD(_symbol, _timeframe, fast_ema_period, slow_ema_period, signal_period, PRICE_CLOSE, MODE_MAIN, period);
      return macdValue > 0;
   }
};
...
ICondition* CreateLongCondition(string symbol, ENUM_TIMEFRAMES timeframe)
{
   ...
   condition.Add(new MAMACondition(symbol, timeframe, FirstAboveSecond, 
      MODE_EMA, fast_ma_period, PRICE_CLOSE,
      MODE_EMA, slow_ma_period, PRICE_CLOSE), false);
...
ICondition* CreateShortCondition(string symbol, ENUM_TIMEFRAMES timeframe)
{
   ...
   condition.Add(new MAMACondition(symbol, timeframe, FirstBelowSecond, 
      MODE_EMA, fast_ma_period, PRICE_CLOSE,
      MODE_EMA, slow_ma_period, PRICE_CLOSE), false);
   #ifdef ACT_ON_SWITCH_CONDITION
      ActOnSwitchCondition* switchCondition = new ActOnSwitchCondition(symbol, timeframe, (ICondition*) condition);
