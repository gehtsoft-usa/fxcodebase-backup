...
int start()
  {
   ...
   for(i=limit; i>=0; i--){
      Fan_1[i]  = Fan_MA(Period_1, 0, K, i, Method);
      Fan_2[i]  = Fan_MA(Period_1, 1, K, i, Method);
      Fan_3[i]  = Fan_MA(Period_1, 2, K, i, Method);
      Fan_4[i]  = Fan_MA(Period_1, 3, K, i, Method);
      Fan_5[i]  = Fan_MA(Period_1, 4, K, i, Method);
      Fan_6[i]  = Fan_MA(Period_1, 5, K, i, Method);
      Fan_7[i]  = Fan_MA(Period_1, 6, K, i, Method);
      Fan_8[i]  = Fan_MA(Period_1, 7, K, i, Method);
      Fan_9[i]  = Fan_MA(Period_1, 8, K, i, Method);
      Fan_10[i] = Fan_MA(Period_1, 9, K, i, Method);
      Fan_11[i] = Fan_MA(Period_1, 10, K, i, Method);
      Fan_12[i] = Fan_MA(Period_1, 11, K, i, Method);
      Fan_13[i] = Fan_MA(Period_1, 12, K, i, Method);
      Fan_14[i] = Fan_MA(Period_1, 13, K, i, Method);
      Fan_15[i] = Fan_MA(Period_1, 14, K, i, Method);
      Fan_16[i] = Fan_MA(Period_1, 15, K, i, Method);
      Fan_17[i] = Fan_MA(Period_1, 16, K, i, Method);
      Fan_18[i] = Fan_MA(Period_1, 17, K, i, Method);
      Fan_19[i] = Fan_MA(Period_1, 18, K, i, Method);
      Fan_20[i] = Fan_MA(Period_1, 19, K, i, Method);
   }
...
double Fan_MA(int Periodo, int Step, int KK, int num, int Metodo){
   
   double MA, Previous_MA;
   
   if (Step==0){
      MA = iMA(NULL,0,Periodo,0,MODE_EMA,PRICE_CLOSE,num);
      Previous_MA = iMA(NULL,0,Periodo,0,MODE_EMA,PRICE_CLOSE,num+1);
   }else if (Metodo==1){
      MA = iMA(NULL,0,Periodo+(Step*KK),0,MODE_EMA,PRICE_CLOSE,num);
      Previous_MA = iMA(NULL,0,Periodo+(Step*KK),0,MODE_EMA,PRICE_CLOSE,num+1);
   }else{
      MA = iMA(NULL,0,Periodo*(Step*KK),0,MODE_EMA,PRICE_CLOSE,num);
      Previous_MA = iMA(NULL,0,Periodo*(Step*KK),0,MODE_EMA,PRICE_CLOSE,num+1);
   }
   ...
   if (num==eval){
      ArrayFill(current,Step,1,MA);
      ArrayFill(previous,Step,1,Previous_MA);
   }
