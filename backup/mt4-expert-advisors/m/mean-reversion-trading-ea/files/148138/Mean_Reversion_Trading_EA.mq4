...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=71894
...
enum ModeEntry { Market,
                 ...
                 PendingLimit };
// ------------------------------------------------------------------
input int          uCandlesBack            = 6; // Candles Back to compare:
...
input bool      uMaxPendingsOn    = true;                             // Limit the # of Pendings Orders On?
input int       uMaxPendings      = 1;                                // Max Pendings Orders:
...
class ConditionCountPendings : public iConditions
{
   int _max;

  public:
   ConditionCountPendings(int max)
   {
      _max = max;
   }
   ~ConditionCountPendings() { ; }
...
ConditionCountPendings* countPendings;
...
int OnInit()
{
   ...
   if(uMaxPendingsOn)conditionsToBuy.AddCondition(countPendings = new ConditionCountPendings(uMaxPendings));
   ...
   conditionsToSell.AddCondition(countSells = new ConditionCountSells(1));
	 if(uMaxPendingsOn)conditionsToSell.AddCondition(countPendings = new ConditionCountPendings(uMaxPendings));
...
double maxCandlesBack()
{
   double candlesBack[];
   ArrayResize(candlesBack, uCandlesBack);
   for (int i = 0; i < ArraySize(candlesBack); i++)
   {
      candlesBack[i] = iClose(Symbol(),Period(),i+2); 
  	}
   return candlesBack[ArrayMaximum(candlesBack, 0)];
}
double minCandlesBack()
{
   double candlesBack[];
   ArrayResize(candlesBack, uCandlesBack);
   for (int i = 0; i < ArraySize(candlesBack); i++)
   ...
   return candlesBack[ArrayMinimum(candlesBack, 0)];
}
