...
// http://fxcodebase.com/code/viewtopic.php?f=38&t=65105

//+------------------------------------------------------------------+
//|                                          MTF_MCP_RSI_HeatMap.mq4 |
...
enum metodo{ RSI_OBOS=1, RSI_Center_Line=2, RSI_Slope=3 };

input  metodo Compare_Method = RSI_OBOS;
extern int    RSI_Periods    = 12;
extern double RSI_OB_Level   = 70.0;
extern double RSI_OS_Level   = 30.0;
...
int start(){
   ...
   if (Period() < 43200){
      ...
      for(i=limit ; i>=0; i--){
         ...
         rsi0 = iRSI(Symbolo,period,RSI_Periods,PRICE_CLOSE,current);
         rsi1 = iRSI(Symbolo,period,RSI_Periods,PRICE_CLOSE,current+1);
         w1_up[i] = -1.0;
         w1_dn[i] = -1.0;
         w1_nt[i] = -1.0;
         if (Compare_Method==1){
            if (rsi0 > RSI_OB_Level)
               w1_up[i] = 1.0;
            else if (rsi0 < RSI_OS_Level)
               w1_dn[i] = 1.0;
            else
               w1_nt[i] = 1.0;
         }
