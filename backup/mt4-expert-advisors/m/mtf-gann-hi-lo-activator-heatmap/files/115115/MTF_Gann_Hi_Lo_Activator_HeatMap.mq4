...
// http://fxcodebase.com/code/viewtopic.php?f=38&t=65121
...
int start(){
   ...
   double SMA_H, SMA_L, ssl, Hld, Hlv;
   ...
   if (Period() < 43200){
      ...
      for(i=limit ; i>=0; i--){
         ...
         SMA_H = iMA(NULL,period,Periods,0,MODE_SMA,PRICE_HIGH,i+1);
         SMA_L = iMA(NULL,period,Periods,0,MODE_SMA,PRICE_LOW,i+1);
         
         if(iClose(NULL,period,i) > SMA_H)
            Hld=1;
         else{
            if(iClose(NULL,period,i) < SMA_L)
               Hld=-1;
            else
               Hld=0;
         }
