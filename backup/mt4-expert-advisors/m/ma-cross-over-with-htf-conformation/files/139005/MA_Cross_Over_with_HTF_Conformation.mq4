...
input double reentry = 2; // Reentry, pips
input ENUM_TIMEFRAMES tf2 = PERIOD_CURRENT; // Timeframe #2
input string lines_id = "ZZCWO"; // Line ids

input int ma1_period = 7; // MA 1 period
input ENUM_MA_METHOD ma1_method = MODE_SMA; // Smoothing method
input int ma2_period = 14; // MA 2 period
input ENUM_MA_METHOD ma2_method = MODE_SMA; // Smoothing method
...
double GetZZLines(string symbol, ENUM_TIMEFRAMES timeframe, string id, int period)
{
   int x1 = iBarShift(symbol, timeframe, ObjectGetInteger(0, id, OBJPROP_TIME, 0));
   int x2 = iBarShift(symbol, timeframe, ObjectGetInteger(0, id, OBJPROP_TIME, 1));
   double y1 = ObjectGetDouble(0, id, OBJPROP_PRICE, 0);
   double y2 = ObjectGetDouble(0, id, OBJPROP_PRICE, 1);
   if (x1 > x2)
   {
      datetime temp = x1;
      x1 = x2;
...
class LongPullbackCondition : public ACondition
{
   double pipSize;
public:
   LongPullbackCondition(const string symbol, ENUM_TIMEFRAMES timeframe)
      :ACondition(symbol, timeframe)
   {
   ...
   bool IsPass(const int period, const datetime date)
   {
      ...
      {
         double maValue = iMA(_symbol, _timeframe, ma2_period, 0, ma2_method, PRICE_CLOSE, i);
         if (iLow(_symbol, _timeframe, i) - reentry * pipSize <= maValue)
         {
            return true;
         }
      }
...
class LongMa12Condition : public ACondition
{
public:
   LongMa12Condition(const string symbol, ENUM_TIMEFRAMES timeframe)
      :ACondition(symbol, timeframe)
   {
   }
...
class LongMa12_2Condition : public ACondition
{
public:
   LongMa12_2Condition(const string symbol, ENUM_TIMEFRAMES timeframe)
      :ACondition(symbol, timeframe)
   {
   }
   ...
   bool IsPass(const int period, const datetime date)
   {
      double ma1Value = iMA(_symbol, tf2, ma1_period, 0, ma1_method, PRICE_CLOSE, 0);
      double ma2Value = iMA(_symbol, _timeframe, ma2_period, 0, ma2_method, PRICE_CLOSE, 0);
      return ma1Value > ma2Value;
   }
};

class LongSourceZZCondition : public ACondition
{
public:
   LongSourceZZCondition(const string symbol, ENUM_TIMEFRAMES timeframe)
      :ACondition(symbol, timeframe)
   {
   }
   ...
   bool IsPass(const int period, const datetime date)
   {
      double up = GetZZLines(_symbol, _timeframe, lines_id + "Up", period);
      double down = GetZZLines(_symbol, _timeframe, lines_id + "Down", period);
      return iClose(_symbol, _timeframe, period) > MathMax(up, down);
   }
};

class LongSourceMACondition : public ACondition
{
public:
   LongSourceMACondition(const string symbol, ENUM_TIMEFRAMES timeframe)
      :ACondition(symbol, timeframe)
   {
   }
...
class LongVPCCondition : public ACondition
{
public:
   LongVPCCondition(const string symbol, ENUM_TIMEFRAMES timeframe)
      :ACondition(symbol, timeframe)
   {
   }
   ...
   bool IsPass(const int period, const datetime date)
   {
      double vpc = iCustom(_symbol, _timeframe, "Volume Price Change", 0, period);
      double signal = iCustom(_symbol, _timeframe, "Volume Price Change", 1, period);
      return vpc > signal;
   }
};

class ShortPullbackCondition : public ACondition
{
   double pipSize;
public:
   ShortPullbackCondition(const string symbol, ENUM_TIMEFRAMES timeframe)
      :ACondition(symbol, timeframe)
   {
      double point = MarketInfo(_symbol, MODE_POINT);
      int digits = (int)MarketInfo(_symbol, MODE_DIGITS);
   ...
   bool IsPass(const int period, const datetime date)
   {
      ...
      {
         double maValue = iMA(_symbol, _timeframe, ma2_period, 0, ma2_method, PRICE_CLOSE, i);
         if (iHigh(_symbol, _timeframe, i) + reentry * pipSize >= maValue)
         {
            return true;
         }
      }
...
class ShortMa12Condition : public ACondition
{
public:
   ShortMa12Condition(const string symbol, ENUM_TIMEFRAMES timeframe)
      :ACondition(symbol, timeframe)
   {
   }
...
class ShortMa12_2Condition : public ACondition
{
public:
   ShortMa12_2Condition(const string symbol, ENUM_TIMEFRAMES timeframe)
      :ACondition(symbol, timeframe)
   {
   }
...
class ShortSourceZZCondition : public ACondition
{
public:
   ShortSourceZZCondition(const string symbol, ENUM_TIMEFRAMES timeframe)
      :ACondition(symbol, timeframe)
   {
   }
...
class ShortSourceMACondition : public ACondition
{
public:
   ShortSourceMACondition(const string symbol, ENUM_TIMEFRAMES timeframe)
      :ACondition(symbol, timeframe)
   {
   }
...
class ShortVPCCondition : public ACondition
{
public:
   ShortVPCCondition(const string symbol, ENUM_TIMEFRAMES timeframe)
      :ACondition(symbol, timeframe)
   {
   }
...
ICondition* CreateLongCondition(string symbol, ENUM_TIMEFRAMES timeframe)
{
   ...
   AndCondition* condition2 = new AndCondition();
   condition2.Add(new LongSourceMACondition(symbol, timeframe), false);
   condition2.Add(new LongPullbackCondition(symbol, timeframe), false);
   OrCondition* or = new OrCondition();
   or.Add(new LongSourceZZCondition(symbol, timeframe), false);
   or.Add(condition2, false);
   condition.Add(new LongMa12Condition(symbol, timeframe), false);
   condition.Add(new LongMa12_2Condition(symbol, timeframe), false);
   condition.Add(new LongVPCCondition(symbol, timeframe), false);
   condition.Add(or, false);
...
ICondition* CreateShortCondition(string symbol, ENUM_TIMEFRAMES timeframe)
{
   ...
   AndCondition* condition = new AndCondition();
   AndCondition* condition2 = new AndCondition();
   condition2.Add(new ShortSourceMACondition(symbol, timeframe), false);
   condition2.Add(new ShortPullbackCondition(symbol, timeframe), false);
   OrCondition* or = new OrCondition();
   or.Add(new ShortSourceZZCondition(symbol, timeframe), false);
   or.Add(condition2, false);
   condition.Add(new ShortMa12Condition(symbol, timeframe), false);
   condition.Add(new ShortMa12_2Condition(symbol, timeframe), false);
   condition.Add(new ShortVPCCondition(symbol, timeframe), false);
   condition.Add(or, false);
   #ifdef ACT_ON_SWITCH_CONDITION
      ActOnSwitchCondition* switchCondition = new ActOnSwitchCondition(symbol, timeframe, (ICondition*) condition);
      condition.Release();
...
int OnInit()
{
   double temp = iCustom(NULL, 0, "Volume Price Change", 0, 0);
   if (GetLastError() == ERR_INDICATOR_CANNOT_LOAD)
   {
      Alert("Please, install the 'Volume Price Change' indicator");
      return INIT_FAILED;
   }
   #ifdef SHOW_ACCOUNT_STAT
      stats = NULL;
