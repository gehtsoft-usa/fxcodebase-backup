...
//---- indicators
	...
	WindowName = "MTF Price Bar ("+Symbol()+")";
   ...
   TF_count = ArraySize(TF_arr);
	...
	for (int j=0; j < TF_count; j++) {
      ...
      period = StringToInteger(TF_arr[j]);
   	...
   	// Bulish Candle
   	if (iOpen(NULL,period,0)<=iClose(NULL,period,0)){
   	
   	   Bullish_Wick_High[i]   = iHigh(NULL,period,0);
   	   Bullish_Candle_High[i] = iClose(NULL,period,0);
   	   Bullish_Candle_Low[i] = iOpen(NULL,period,0);
   	   Bullish_Wick_Low[i]    = iOpen(NULL,period,0);
   	...
   	// Bearish Candle
   	   ...
   	   Bearish_Wick_High[i]   = iHigh(NULL,period,0);
   	   Bearish_Candle_High[i] = iOpen(NULL,period,0);
   	   Bearish_Candle_Low[i] = iClose(NULL,period,0);
   	   Bearish_Wick_Low[i]    = iClose(NULL,period,0);
	   
   	
   	}
   	
   	// Down Empty
   	Empty[i] = iLow(NULL,period,0);
	Empty[i+1] = iHigh(NULL,period,0);
