...
input int Period1 = 7; // Period #1
input ENUM_MA_METHOD Method1 = MODE_SMA; // MA Method #1
input int Period2 = 14; // Period
input ENUM_MA_METHOD Method2 = MODE_SMA; // MA Method #2
input int NM = 50; // Number of the periods to smooth the center line
input int NB = 50; // Number of periods to smooth deviation
input double F = 1; // Factor which is used to apply the deviation
input ENUM_APPLIED_PRICE SRC = PRICE_CLOSE; // The center line source
input bool SC = true; // Show the center line
input ENUM_MA_METHOD MS = MODE_SMA; // The center line smoothing method
enum VariationMethod
{
   AHL, // Smoothed H-L
   ATR // ATR of source
};
input VariationMethod MV = AHL; // Variation Method
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      double ma1 = iMA(_symbol, _timeframe, Period1, 0, Method1, PRICE_CLOSE, period);
      double ma2 = iMA(_symbol, _timeframe, Period2, 0, Method2, PRICE_CLOSE, period);
      return ma1 > ma2;
   }
};
...
class FilterLongCondition : public ACondition
{
public:
   FilterLongCondition(const string symbol, ENUM_TIMEFRAMES timeframe)
      :ACondition(symbol, timeframe)
   {
   ...
   bool IsPass(const int period, const datetime date)
   {
      double value = iCustom(_symbol, _timeframe, "Keltner1", NM, NB, F, SRC, SC, MS, MV, 2, period);
      return iClose(_symbol, _timeframe, period) < value;
   }
};

class FilterShortCondition : public ACondition
{
public:
   FilterShortCondition(const string symbol, ENUM_TIMEFRAMES timeframe)
      :ACondition(symbol, timeframe)
   {
   ...
   bool IsPass(const int period, const datetime date)
   {
      double value = iCustom(_symbol, _timeframe, "Keltner1", NM, NB, F, SRC, SC, MS, MV, 1, period);
      return iClose(_symbol, _timeframe, period) > value;
   }
};
