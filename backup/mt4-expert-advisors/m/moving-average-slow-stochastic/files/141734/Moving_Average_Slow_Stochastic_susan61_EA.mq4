...
//https://fxcodebase.com/code/viewtopic.php?f=38&t=71142
...
input int stoch_k = 10; // Periods for %K
input int stoch_sd = 5; // %D slowing periods
input int stoch_d = 5; // Periods for %D
input ENUM_TIMEFRAMES tf1 = PERIOD_H1; // Confirmation Timeframe 1
input ENUM_TIMEFRAMES tf2 = PERIOD_D1; // Confirmation Timeframe 2
...
class MALongCondition : public ACondition
{
public:
   MALongCondition(const string symbol, ENUM_TIMEFRAMES timeframe)
      :ACondition(symbol, timeframe)
   {
   ...
   bool IsPass(const int period, const datetime date)
   {
      double maValue = iMA(_symbol, _timeframe, ma_period, 0, ma_method, PRICE_CLOSE, 0);
      return iClose(_symbol, _timeframe, 0) > maValue;
   }
};
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      double k = iStochastic(_symbol, _timeframe, stoch_k, stoch_d, stoch_sd, MODE_SMA, 0, MODE_MAIN, period);
      double d = iStochastic(_symbol, _timeframe, stoch_k, stoch_d, stoch_sd, MODE_SMA, 0, MODE_SIGNAL, period);
      return k > d;
   }
};

class PriceLongCondition : public ACondition
{
public:
   PriceLongCondition(const string symbol, ENUM_TIMEFRAMES timeframe)
      :ACondition(symbol, timeframe)
   {
   ...
   bool IsPass(const int period, const datetime date)
   {
      return iClose(_symbol, _timeframe, period) > iOpen(_symbol, _timeframe, period + 1);
   }
};
double RSIOMA = 10;
double Ma_RSIOMA = 14;
...
{
   RSIStream* RSIBuffer1;
   SmaOnStream* marsioma1;
public:
   RSILongCondition(const string symbol, ENUM_TIMEFRAMES timeframe)
      :ACondition(symbol, timeframe)
   {
      BarStream* source = new BarStream(symbol, timeframe);
      PriceStream* price = new PriceStream(source, PriceClose);
      source.Release();
      LwmaOnStream* maBuffer1 = new LwmaOnStream(price, RSIOMA);
      price.Release();
      RSIBuffer1 = new RSIStream(maBuffer1, RSIOMA);
      maBuffer1.Release();
      marsioma1 = new SmaOnStream(RSIBuffer1, Ma_RSIOMA);
   }
   ~RSILongCondition()
   {
      RSIBuffer1.Release();
   ...
   {
      double rsi, ma;
      if (!RSIBuffer1.GetValue(period, rsi) || !marsioma1.GetValue(period, ma))
      {
         return false;
      }
      return rsi < ma;
...
class MAShortCondition : public ACondition
{
public:
   MAShortCondition(const string symbol, ENUM_TIMEFRAMES timeframe)
      :ACondition(symbol, timeframe)
   {
...
class PriceShortCondition : public ACondition
{
public:
   PriceShortCondition(const string symbol, ENUM_TIMEFRAMES timeframe)
      :ACondition(symbol, timeframe)
   {
...
ICondition* CreateLongCondition(string symbol, ENUM_TIMEFRAMES timeframe)
{
   ...
   condition.Add(new MALongCondition(symbol, timeframe), false);
   condition.Add(new SSDLongCondition(symbol, timeframe), false);
   condition.Add(new PriceLongCondition(symbol, timeframe), false);
   condition.Add(new RSILongCondition(symbol, timeframe), false);
   condition.Add(new MAShortCondition(symbol, tf1), false);
   condition.Add(new MAShortCondition(symbol, tf2), false);
...
ICondition* CreateShortCondition(string symbol, ENUM_TIMEFRAMES timeframe)
{
   ...
   AndCondition* condition = new AndCondition();
   condition.Add(new MAShortCondition(symbol, timeframe), false);
   condition.Add(new SSDShortCondition(symbol, timeframe), false);
   condition.Add(new PriceShortCondition(symbol, timeframe), false);
   condition.Add(new RSIShortCondition(symbol, timeframe), false);
   condition.Add(new MALongCondition(symbol, tf1), false);
   condition.Add(new MALongCondition(symbol, tf2), false);
   #ifdef ACT_ON_SWITCH_CONDITION
      ActOnSwitchCondition* switchCondition = new ActOnSwitchCondition(symbol, timeframe, (ICondition*) condition);
      condition.Release();
      return switchCondition;
