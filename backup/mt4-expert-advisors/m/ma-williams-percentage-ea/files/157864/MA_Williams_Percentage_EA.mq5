//Available @  https://fxcodebase.com/code/viewtopic.php?f=38&t=75512&p=157864#p157864
...
input int os_level = -85; // Oversold level
input int ob_level = -15; // Overbought level
...
public:
   EntryLongCondition(string symbol, ENUM_TIMEFRAMES timeframe)
      :ACondition(symbol, timeframe)
   {
      IStream* close = PriceStreamFactory::Create(symbol, timeframe, PriceClose);
      IStream* high = PriceStreamFactory::Create(symbol, timeframe, PriceHigh);
      IStream* low = PriceStreamFactory::Create(symbol, timeframe, PriceLow);
      IStream* williams = new WilliamsRStream(close, high, low, length);
      ema = new EMAOnStream(williams, length);
      close.Release();
      high.Release();
      low.Release();
   }
   ...
   {
      double val[1];
      if (!ema.GetSeriesValues(period, 1, val))
      {
         return false;
      }
      return val[0] < os_level;
   }
};
