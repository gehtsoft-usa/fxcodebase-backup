...
input int SlowEMA = 24; // MACD Slow EMA
input int SignalEMA = 9; // MACD Signal
input int ma_periods = 200; // EMA Period
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      double value = iCustom(_symbol, _timeframe, "ZeroLagMACD", FastEMA, SlowEMA, SignalEMA, 0, period);
      return value > 0;
   }
};
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      double value = iCustom(_symbol, _timeframe, "VWAP", 0, period);
      double value1 = iCustom(_symbol, _timeframe, "VWAP", 0, period + 1);
      return value > value1 && value < iClose(_symbol, _timeframe, period);
   }
};
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      double value = iMA(_symbol, _timeframe, ma_periods, 0, MODE_EMA, PRICE_CLOSE, period);
      double value1 = iMA(_symbol, _timeframe, ma_periods, 0, MODE_EMA, PRICE_CLOSE, period + 1);
      return value > value1 && value < iClose(_symbol, _timeframe, period);
   }
};
