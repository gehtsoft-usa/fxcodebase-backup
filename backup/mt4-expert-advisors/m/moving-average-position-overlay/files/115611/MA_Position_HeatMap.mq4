...
// http://fxcodebase.com/code/viewtopic.php?f=38&t=65205

//+------------------------------------------------------------------+
//|                                          MA_Position_HeatMap.mq4 |
...
input  e_method MA1_Method     = EMA;
input  e_price  MA1_Price      = CLOSE;
extern int      MA1_Periods    = 5;
input  e_method MA2_Method     = EMA;
input  e_price  MA2_Price      = CLOSE;
extern int      MA2_Periods    = 10;
input  e_method MA3_Method     = EMA;
input  e_price  MA3_Price      = CLOSE;
extern int      MA3_Periods    = 20;
...
int start(){
   ...
   if (Period() < 43200){
      ...
      for(i=limit ; i>=0; i--){
         ...
         ma1 = iMA(Symbolo,period,MA1_Periods,0,ENUM_MA_METHOD(MA1_Method),ENUM_APPLIED_PRICE(MA1_Price),current);
         ma2 = iMA(Symbolo,period,MA2_Periods,0,ENUM_MA_METHOD(MA2_Method),ENUM_APPLIED_PRICE(MA2_Price),current);
         ma3 = iMA(Symbolo,period,MA3_Periods,0,ENUM_MA_METHOD(MA3_Method),ENUM_APPLIED_PRICE(MA3_Price),current);
         w1_up[i] = -1.0;
         w1_dn[i] = -1.0;
         w1_nt[i] = -1.0;
         if (ma1 > ma2 && ma2 > ma3)
