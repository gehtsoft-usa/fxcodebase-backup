...
string file_custom_indicator = "Donchian_Channel.ex5";
int    donchian;
...
input string       Tindi                      = "== Donchian Setup ==";        // **********
input int  Length     = 20;
...
input CloseAllMode closeBy                 = CloseByDonchian;           // Close All Mode:
 ...
 public:
  bool evaluate()
  {
    // TODO: condition Buy 1
		if(MainOrders.qnt()<1)
		{
	    double indi = index(donchian, 0, 1);
			Print(__FUNCTION__," indi: ",indi);
    	return Ask() > indi && Bid() <= indi;
		}
		if(MainOrders.qnt()>1)
		{
			return false;
 ...
 public:
  bool evaluate()
  {
    // TODO: condition Buy2
		if(MainOrders.qnt()==1)
		{
			double indi = index(donchian, 1, 1);
			Print(__FUNCTION__," indi: ",indi);
    	return Ask() > indi && Bid() <= indi;
		}
		if(MainOrders.qnt()>1)
 ...
 public:
  bool evaluate()
  {
    // NOTE: condition sell 1
		if(MainOrders.qnt()<1)
		{
			double indi = index(donchian, 4, 1);
			Print(__FUNCTION__," donchian: ",donchian);
			Print(__FUNCTION__," indi: ",indi);
    	return Ask() >= indi && Bid() < indi;
		}
    if(MainOrders.qnt()>1)
 ...
 public:
  bool evaluate()
  {
    // NOTE: condition sell 2
		if(MainOrders.qnt()==1)
		{
			double indi = index(donchian, 3, 1);
			Print(__FUNCTION__," donchian: ",donchian);
			Print(__FUNCTION__," indi: ",indi);
    	return Ask() >= indi && Bid() < indi;
		}
...
int OnInit()
{
  ...
  donchian = iCustom(NULL, 0, file_custom_indicator,Length, Mode, true);
...
void OnTick()
{
  ...
  if (uTradeReverse) {
    if (conditionsToCloseSell.EvaluateConditions()) { closeAll("buy"); }
    if (conditionsToCloseBuy.EvaluateConditions()) { closeAll("sell"); }
  }
  ...
  switch (closeBy) {
    ...
    case CloseByAccountPercent: {
      ...
      break;			
		...
		// NOTE: close by indicator
		case CloseByDonchian:{
		  double indi = index(donchian, 2, 1);
    	return Ask() > indi && Bid() <= indi;
			break;
			}
  }
