...
input int MinPipsForAddOnOrders = 5; // Min pips for addon orders
...
public:
   LongCondition(const string symbol, ENUM_TIMEFRAMES timeframe)
      :ACondition(symbol, timeframe)
   {
      _indi = iCustom(_symbol, _timeframe, "2 bar supply and demand", true);
   }
   ...
   {
      double buffer[1];
      if (CopyBuffer(_indi, 9, period + 1, 1, buffer) != 1)
      {
         return false;
      }
      return buffer[0] != EMPTY_VALUE;
   }
};
...
public:
   ...
   {
      double buffer[1];
      if (CopyBuffer(_indi, 5, period + 1, 1, buffer) != 1)
      {
         return false;
      }
      return buffer[0] != EMPTY_VALUE;
...
class LongEntryDistanceCondition : public ACondition
{
public:
   LongEntryDistanceCondition(const string symbol, ENUM_TIMEFRAMES timeframe)
      :ACondition(symbol, timeframe)
   {
   ...
   bool IsPass(const int period, const datetime date)
   {
      ...
      while (it.Next())
      {
         if ((instrument.GetAsk() - it.GetOpenPrice()) / instrument.GetPipSize() < MinPipsForAddOnOrders)
         {
            return false;
         }
      }
...
class ShortEntryDistanceCondition : public ACondition
{
public:
   ShortEntryDistanceCondition(const string symbol, ENUM_TIMEFRAMES timeframe)
      :ACondition(symbol, timeframe)
   {
   ...
   bool IsPass(const int period, const datetime date)
   {
      ...
      while (it.Next())
      {
         if ((it.GetOpenPrice() - instrument.GetBid()) / instrument.GetPipSize() < MinPipsForAddOnOrders)
         {
            return false;
         }
      }
...
ICondition* CreateLongCondition(string symbol, ENUM_TIMEFRAMES timeframe)
{
   ...
   condition.Add(new LongEntryDistanceCondition(symbol, timeframe), false);
   #ifdef ACT_ON_SWITCH_CONDITION
...
ICondition* CreateShortCondition(string symbol, ENUM_TIMEFRAMES timeframe)
{
   ...
   condition.Add(new ShortEntryDistanceCondition(symbol, timeframe), false);
   #ifdef ACT_ON_SWITCH_CONDITION
      ActOnSwitchCondition* switchCondition = new ActOnSwitchCondition(symbol, timeframe, (ICondition*) condition);
      condition.Release();
      return switchCondition;
