//Available @  https://fxcodebase.com/code/viewtopic.php?f=38&p=159995#p159995
...
int OnInit()
{
   ...
   ArrayResize(ValueBuffer, period);
   ArrayResize(Fish1Buffer, period);
                ...
                const int &spread[])
{
   ...
   {
      //--- Find highest/lowest
      double MaxH = high[iHighest(_Symbol, PERIOD_CURRENT, MODE_CLOSE, period, i)];
      double MinL = low[iLowest(_Symbol, PERIOD_CURRENT, MODE_CLOSE, period, i)];
      ...
      if(Arrow && i >= SIGNAL_BAR + 1)
      {
         if(ExtBuffer[i-SIGNAL_BAR-1] > 0.0 && ExtBuffer[i-SIGNAL_BAR] < 0.0)
            CreateArrow(time[i-2], open[i-2], close[i-2], clArrowSell, 234, false);
            
         if(ExtBuffer[i-SIGNAL_BAR-1] < 0.0 && ExtBuffer[i-SIGNAL_BAR] > 0.0)
            CreateArrow(time[i-2], open[i-2], close[i-2], clArrowBuy, 233, true);
      }
