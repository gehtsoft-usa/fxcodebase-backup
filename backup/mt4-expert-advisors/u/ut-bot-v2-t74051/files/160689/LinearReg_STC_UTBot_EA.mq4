...
string file_custom_indicator = "Linear_Regression_Bar.ex4";
int bufferToBuy = 0;
int bufferToSell = 4;


input string Tindi1 = "== Linear regression Setup ==";  // ————————————
input int            InpLRLPeriod = 50;    // Linear Regression Period
input int            maPeriod = 11;            // MA Period;
input ENUM_MA_METHOD maMethod = MODE_EMA;    // MA Method;
input bool           hideCandle = true;       // Hide Chart's Candle
input color          BullColor = clrBlue;   // Bull Candle Color
input color          BearColor = clrRed;    // Bear Candle Color
input color          maColor = clrYellow;   // MA Color
input int            maWidth = 2;             // MA Width

// ------------------------------------------------------------------
string file_custom_indicator2 = "UT_Bot_v2.ex4";
int bufferToBuy2 = 0;
int bufferToSell2 = 1;
...
// ------------------------------------------------------------------
input string Tindi2 = "== UT Bot Setup ==";  // ————————————
...
string T_0 = "== Draw Trade ==";  // ————————————
bool   drawTradeON = false; // Draw Trade On?

// ------------------------------------------------------------------
string file_custom_indicator3 = "STC_Indicator.ex4";
int bufferToBuy3 = 2;
int bufferToSell3 = 2;

input string Tindi3 = "== STC Setup ==";  // ————————————
input int    stc_avgPeriod   = 30; // Average period
input double stc_sensitivity = 3;  // Sensitivity
...
int                _maPeriod       = 20;                            // Period
int                _maShift        = 0;                             // Ma Shift
ENUM_MA_METHOD     _maMethod       = MODE_EMA;                      // Method
ENUM_APPLIED_PRICE _omaAppliedPrice = PRICE_CLOSE;                   // Applied Price
 ...
 public:
  bool evaluate()
  {
    if (closeAllInOpositeSignal) 
			if (sellCondition1.evaluate() && sellCondition2.evaluate() && sellCondition3.evaluate()) return true;
...
class ConditionToCloseSell : public iConditions
{
 public:
  ...
  {
    if (closeAllInOpositeSignal)
      if (buyCondition1.evaluate() && buyCondition2.evaluate() && buyCondition3.evaluate()) return true;
...
int OnInit()
{
  ...
  temp = iCustom(NULL, 0, file_custom_indicator2, 0, 0);
  if(GetLastError() == ERR_INDICATOR_CANNOT_LOAD)
  {
  ...
  temp = iCustom(NULL, 0, file_custom_indicator3, 0, 0);
  if(GetLastError() == ERR_INDICATOR_CANNOT_LOAD)
  {
...
double Indi(int buffer, int _candle)
{
...
double UT_Bot(int buffer, int _candle)
{
    ...
    return iCustom(NULL, 0, file_custom_indicator2,
        Tindi2, m, atrPeriods, h, s, p, b, T_0, drawTradeON,
        buffer, _candle);
       
}

double STC_indicator(int buffer, int _candle)
{
    // TODO: copiar la lista de INPUTS para custom indicator:    
    return iCustom(NULL, 0, file_custom_indicator3,
    stc_avgPeriod,
    stc_sensitivity,
    buffer, _candle);
}
