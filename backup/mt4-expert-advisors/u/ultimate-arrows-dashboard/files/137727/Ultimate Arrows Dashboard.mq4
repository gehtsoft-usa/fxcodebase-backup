...
input int ExtDepth=115;//105;//21;
input int ExtDeviation=130;//120;//24;
input int ExtBackstep=80;//70;//14;
...
public:
   ...
   {
      double value = iCustom(_symbol, _timeframe, "Ultimate Arrows", ExtDepth, ExtDeviation, ExtBackstep, 0, period);
...
public:
   ...
   virtual bool IsPass(const int period, const datetime date)
   {
      double value = iCustom(_symbol, _timeframe, "Ultimate Arrows", ExtDepth, ExtDeviation, ExtBackstep, 1, period);
      return value != 0 && value != EMPTY_VALUE;
   }
};
...
string IndicatorName = "Ultimate Arrows Dashboard";
int init()
{
   double temp = iCustom(NULL, 0, "Ultimate Arrows", 0, 0);
   if (GetLastError() == ERR_INDICATOR_CANNOT_LOAD)
   {
      Alert("Please, install the 'Ultimate Arrows' indicator");
      return INIT_FAILED;
   }
   ...
   IndicatorObjPrefix = GenerateIndicatorPrefix("uad");
