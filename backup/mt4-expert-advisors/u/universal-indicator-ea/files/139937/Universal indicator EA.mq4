...
//http://fxcodebase.com/code/viewtopic.php?f=38&t=70770
...
// DEFINED BY THE USER [START]
input string indicator_name = "averages"; // Indicator name
...
{
   return iCustom(symbol, tf, indicator_name, /*insert parameters, */stream, period);
}
// DEFINED BY THE USER [END]
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      double value = GetSignal(_symbol, _timeframe, period, buy_stream_index);
      switch (buy_signal)
      {
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      double value = GetSignal(_symbol, _timeframe, period, sell_stream_index);
      switch (sell_signal)
      {
         case STCrossOverPrice:
            return value > iClose(_symbol, _timeframe, period);
