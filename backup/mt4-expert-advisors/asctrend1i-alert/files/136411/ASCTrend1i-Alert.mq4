...
//---- input parameters
input int Risk = 6; /* Risk */                                                 // Risk level. WPR_Period=3+Risk*2 or WPR_Period=ALT_PERIOD for a large move
input int BarCount_ = 0; /* Bar Count */                                        // 0 for full chart buffer
int BarCount;
...
public:
   ...
   {
      return up_sig[period] > 0 && up_sig[period + 1] <= 0;
   }
};
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      return dn_sig[period] > 0 && dn_sig[period + 1] <= 0;
   }
};
...
int init()
{
   ...
   IndicatorBuffers(11);
   IndicatorObjPrefix = GenerateIndicatorPrefix("asct1i");
