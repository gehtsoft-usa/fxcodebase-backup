...
int OnInit()
{
     double temp = iCustom(NULL, 0, "Average_Of_Averages", 0, 0);
   if (GetLastError() == ERR_INDICATOR_CANNOT_LOAD)
   {
       Alert("Please, install the 'Average_Of_Averages' indicator");
       return INIT_FAILED;
   }
   LastBar=0;
...
void OnTick()
{
 ...
 double S_MA0, S_MA1, F_MA0, F_MA1;
 S_MA0=iCustom(NULL, 0, "Average_Of_Averages", Slow_MA_Length, Price, 0, 1);
 S_MA1=iCustom(NULL, 0, "Average_Of_Averages", Slow_MA_Length, Price, 0, 2);
 F_MA0=iCustom(NULL, 0, "Average_Of_Averages", Fast_MA_Length, Price, 0, 1);
 F_MA1=iCustom(NULL, 0, "Average_Of_Averages", Fast_MA_Length, Price, 0, 2);
 
 int CO;
 int res;
 double SL, TP;
 
 if (F_MA1<S_MA1 && F_MA0>S_MA0)
 {
  _Alert("Buy");
  CO=CalculateOrders();
  if (CO<=0)
