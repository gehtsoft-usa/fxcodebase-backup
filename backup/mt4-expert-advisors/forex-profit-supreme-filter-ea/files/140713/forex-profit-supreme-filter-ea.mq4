...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      double val = iCustom(_symbol, _timeframe, "forex-profit-supreme-filter", "current time frame", SmoothPeriod, SmoothPhase, 0, period);
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      double val = iCustom(_symbol, _timeframe, "forex-profit-supreme-filter", "current time frame", SmoothPeriod, SmoothPhase, 1, period);
      return val != 0 && val != EMPTY_VALUE;
   }
};
...
int OnInit()
{
   double temp = iCustom(NULL, 0, "forex-profit-supreme-filter", "current time frame", SmoothPeriod, SmoothPhase, 0, 0);
   if (GetLastError() == ERR_INDICATOR_CANNOT_LOAD)
   {
      Alert("Please, install the 'forex-profit-supreme-filter' indicator");
      return INIT_FAILED;
   }
