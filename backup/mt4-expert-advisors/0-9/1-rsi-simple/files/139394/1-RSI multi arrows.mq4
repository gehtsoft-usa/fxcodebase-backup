...
input DisplayType Type = Lines; // Presentation Type
input double shift_arrows_pips = 0.1; // Shift arrows
input bool     Include_M1 = false; // Include M1
input color   up_color_M1 = Blue; // Up color
input color down_color_M1 = Red; // Down color
input bool     Include_M5 = false; // Include M5
input color   up_color_M5 = Blue; // Up color
input color down_color_M5 = Red; // Down color
input bool     Include_M15 = false; // Include M15
input color   up_color_M15 = Blue; // Up color
input color down_color_M15 = Red; // Down color
input bool     Include_M30 = false; // Include M30
input color   up_color_M30 = Blue; // Up color
input color down_color_M30 = Red; // Down color
input bool     Include_H1 = true; // Include H1
input color   up_color_H1 = Blue; // Up color
input color down_color_H1 = Red; // Down color
input bool     Include_H4 = false; // Include H4
input color   up_color_H4 = Blue; // Up color
input color down_color_H4 = Red; // Down color
input bool     Include_D1 = true; // Include D1
input color   up_color_D1 = Blue; // Up color
input color down_color_D1 = Red; // Down color
input bool     Include_W1 = true; // Include W1
input color   up_color_W1 = Blue; // Up color
input color down_color_W1 = Red; // Down color
input bool     Include_MN1 = false; // Include MN1
input color   up_color_MN1 = Blue; // Up color
input color down_color_MN1 = Red; // Down color
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      ...
      double value = iCustom(_symbol, _timeframe, "1-RSI_simple", _timeframe, RsiPeriod, MaType, MaPeriod, Interpolate, 2, index);
      return value == 1;
   }
};
...
int init()
{
   ...
   IndicatorBuffers(8);
   IndicatorObjPrefix = GenerateIndicatorPrefix("rsima");
   IndicatorShortName("RSI Multi Arrows");
   ...
   if (Include_M1)
   {
      id = CreateAlert(id, PERIOD_M1, up_color_M1, down_color_M1);
   }
   if (Include_M5)
   {
      id = CreateAlert(id, PERIOD_M5, up_color_M5, down_color_M5);
   }
   if (Include_M15)
   {
      id = CreateAlert(id, PERIOD_M15, up_color_M15, down_color_M15);
   }
   if (Include_M30)
   {
      id = CreateAlert(id, PERIOD_M30, up_color_M30, down_color_M30);
   }
   if (Include_H1)
   {
      id = CreateAlert(id, PERIOD_H1, up_color_H1, down_color_H1);
   }
   if (Include_H4)
   {
      id = CreateAlert(id, PERIOD_H4, up_color_H4, down_color_H4);
   }
   if (Include_D1)
   {
      id = CreateAlert(id, PERIOD_D1, up_color_D1, down_color_D1);
   }
   if (Include_W1)
   {
      id = CreateAlert(id, PERIOD_W1, up_color_W1, down_color_W1);
   }
   if (Include_MN1)
   {
      id = CreateAlert(id, PERIOD_MN1, up_color_MN1, down_color_MN1);
   }
...
int start()
{
   ...
   int minBars = 1;
   int limit = MathMin(bars_limit, MathMin(Bars - 1 - minBars, Bars - counted_bars - 1));
