...
//http://fxcodebase.com/code/viewtopic.php?f=38&t=69768
...
input int period1 = 13; // Length 1
input ENUM_MA_METHOD method1 = MODE_EMA; // Smoothing method 1
input int period2 = 50; // Length 2
input ENUM_MA_METHOD method2 = MODE_EMA; // Smoothing method 2
...
public:
   ...
   virtual bool IsPass(const int period, const datetime date)
   {
      ...
      double ma0 = iMA(_symbol, _timeframe, period1, 0, method1, PRICE_CLOSE, period);
      if (ma0 > close0)
      {
         return false;
      }
      ...
      {
         close0 = iClose(_symbol, _timeframe, period + i);
         ma0 = iMA(_symbol, _timeframe, period1, 0, method1, PRICE_CLOSE, period + i);
         double ma1 = iMA(_symbol, _timeframe, period2, 0, method2, PRICE_CLOSE, period + i);
         if (ma0 > close0 || ma1 < ma0)
         {
            return false;
         }
         if (ma1 > close0)
...
int init()
{
   ...
   IndicatorName = GenerateIndicatorName("2 EMA Cross Arrows");
...
int start()
{
   ...
   {
      out1[pos] = iMA(_Symbol, _Period, period1, 0, method1, PRICE_CLOSE, pos);
      out2[pos] = iMA(_Symbol, _Period, period2, 0, method2, PRICE_CLOSE, pos);
      if (customStream != NULL)
      {
