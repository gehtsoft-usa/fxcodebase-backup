//Available @  https://fxcodebase.com/code/viewtopic.php?f=38&t=72087
...
string IndicatorName = "GHLAMNCD";
string IndicatorShortName = "GHLAMNCD";
...
input int xoffset                       = 0; // X offset (+/-)
input int yoffset                       = 0; // Y offset (+/-)
...
public:
   ...
   {
      double value = iCustom(_symbol, _timeframe, "4-time-frame-gann-high-low-activator-nmc", "Current time frame", 
         "Current time frame", "Current time frame", "Current time frame", LookBack, 
         "4 Time Gann HiLo", 0, DarkGray, 5, 1.5, false, false, false, false, 0, period);
...
public:
   ...
   virtual bool IsPass(const int period, const datetime date)
   {
      double value = iCustom(_symbol, _timeframe, "4-time-frame-gann-high-low-activator-nmc", "Current time frame", 
         "Current time frame", "Current time frame", "Current time frame", LookBack, 
         "4 Time Gann HiLo", 0, DarkGray, 5, 1.5, false, false, false, false, 1, period);
      return value != EMPTY_VALUE;
   }
};
...
protected:
   ...
   { 
      ...
      ObjectSet(nm, OBJPROP_XDISTANCE, xoff+xoffset); 
      ObjectSet(nm, OBJPROP_YDISTANCE, yoff+yoffset); 
...
public:
   ...
   virtual void Draw(int x, int y) 
   {
      if (_withBackground)
      {
         ...
         ObjectSetInteger(0, _id + "rect", OBJPROP_XDISTANCE, x+xoffset);
         ObjectSetInteger(0, _id + "rect", OBJPROP_YDISTANCE, y+yoffset);
...
private:
   ...
   void DrawItem(int x, int y, string text, color textColor, color bgColor, string font)
   {
      ...
      else
      {
         ...
         ObjectSet(id, OBJPROP_XDISTANCE, x+xoffset);
         ObjectSet(id, OBJPROP_YDISTANCE, y+yoffset);
...
int init()
{
   ...
   double temp = iCustom(NULL, 0, "4-time-frame-gann-high-low-activator-nmc", 0, 0);
   if (GetLastError() == ERR_INDICATOR_CANNOT_LOAD)
   {
      Alert("Please, install the '4-time-frame-gann-high-low-activator-nmc' indicator (http://fxcodebase.com/code/viewtopic.php?f=38&t=72087)");
      return INIT_FAILED;
   }
