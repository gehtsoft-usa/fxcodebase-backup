...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=74605
...
string file_custom_indicator = "4 Level ZZ Semafor mtf alerts SOLO4";
int    bufferToBuy           = 2;
int    bufferToBuy1          = 4;
int    bufferToBuy2          = 6;
...
class BUYcondition1 : public iConditions
{
  public:
    bool evaluate()
    {
        ...
        for(int i=1;i < 3;i++)
        {
            if((Indi(bufferToBuy, i) != EMPTY_VALUE && Indi(bufferToBuy, i)>0
            && Indi(bufferToBuy1, i) != EMPTY_VALUE && Indi(bufferToBuy1,i)>0
            && Indi(bufferToBuy2, i) != EMPTY_VALUE && Indi(bufferToBuy2,i)>0) == true)
            return true;
        }
        return false;
    }
  ...
  public:
    bool evaluate()
    {
        // TODO: condition sell 1
           for(int i=1;i < 3;i++)
           {
               if((Indi(bufferToSell, i) != EMPTY_VALUE && Indi(bufferToSell, i)>0
               && Indi(bufferToSell1, i) != EMPTY_VALUE && Indi(bufferToSell1,i)>0
               && Indi(bufferToSell2, i) != EMPTY_VALUE && Indi(bufferToSell2,i)>0)==true) return true;              
           }
           return false;
    }
};
