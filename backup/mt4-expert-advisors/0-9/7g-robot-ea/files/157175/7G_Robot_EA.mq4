...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=73850
...
input double GridUser_maxLot     = 1;                  // Max lot value:
input double GridUser_multiplier = 1.2;                // Multiplier:
...
input bool               filterMa       = true;                         // Filter by MA On?
...
class ConditionSide : public iConditions
{
    OrdersList* _orders;
    string      side;

  public:
    ConditionSide(OrdersList* Orders, string Side)
    {
        _orders = Orders;
        side    = Side;
    }
    ...
    bool evaluate()
    {
        ...
        string currentSide = _orders.last().type() == OP_BUY ? "buy" : "sell";

        return currentSide == side;
    }
};
ConditionSide* conditionSideBuy;
ConditionSide* conditionSideSell;
...
int OnInit()
{
    ...
    if (filterMa) conditionsToBuy.AddCondition(buyCondition2 = new BUYcondition2());
    conditionsToBuy.AddCondition(conditionSideBuy = new ConditionSide(&mainOrders, "buy"));
    ...
    if (filterMa) conditionsToSell.AddCondition(sellCondition2 = new SELLcondition2());
    conditionsToSell.AddCondition(conditionSideSell = new ConditionSide(&mainOrders, "sell"));
...
//--- SESSIONS CONTROL:
#ifdef TIMER_FULL sesionControl.setTimeZone(uTimeZone);
    if (day1_On)
