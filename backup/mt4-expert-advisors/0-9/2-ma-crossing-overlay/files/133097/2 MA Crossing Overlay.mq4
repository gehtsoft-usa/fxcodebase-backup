...
//http://fxcodebase.com/code/viewtopic.php?f=38&t=69718
...
#property indicator_label2  "MA with Shift"
...
public:
   ...
   {
      return ma[period] >= maShifted[period] && ma[period + 1] < maShifted[period + 1];
   }
};
...
int init()
{
   ...
   IndicatorName = GenerateIndicatorName("2 MA Crossing Overlay");
...
int start()
{
   ...
   int minBars = ma_shift + ma_period;
   int limit = MathMin(Bars - 1 - minBars, Bars - counted_bars - 1);
   for (int pos = limit; pos >= 0; --pos)
   {
      ma[pos] = iMA(_Symbol, _Period, ma_period, 0, ma_method, PRICE_CLOSE, pos);
      maShifted[pos] = iMA(_Symbol, _Period, ma_period, ma_shift, ma_method, PRICE_CLOSE, pos);
      if (customStream != NULL)
      {
