...
// http://fxcodebase.com/code/viewtopic.php?f=38&t=69810
...
input ENUM_TIMEFRAMES tf1 = PERIOD_M5; // Timeframe 1
input int period1 = 14; // Period 1
input ENUM_MA_METHOD method1 = MODE_SMA; // Smoothing method 1

input ENUM_TIMEFRAMES tf2 = PERIOD_M15; // Timeframe 2
input int period2 = 14; // Period 2
input ENUM_MA_METHOD method2 = MODE_SMA; // Smoothing method 2

input ENUM_TIMEFRAMES tf3 = PERIOD_M30; // Timeframe 3
input int period3 = 14; // Period 3
input ENUM_MA_METHOD method3 = MODE_SMA; // Smoothing method 3

input ENUM_TIMEFRAMES tf4 = PERIOD_H1; // Timeframe 4
input int period4 = 14; // Period 4
input ENUM_MA_METHOD method4 = MODE_SMA; // Smoothing method 4

input ENUM_TIMEFRAMES tf_vpc = PERIOD_H1; // Timeframe
input int length = 15; // VPC Period
input int price_smoothing = 15; // VPC Price Smoothing
input int signal_smoothing = 15; // VPC Signal Smoothing
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      double vpc = iCustom(_symbol, _timeframe, "Volume Price Change", length, price_smoothing, signal_smoothing, 0, period);
      double signal = iCustom(_symbol, _timeframe, "Volume Price Change", length, price_smoothing, signal_smoothing, 1, period);
      return vpc < signal;
   }
};
...
ICondition* CreateLongCondition(string symbol, ENUM_TIMEFRAMES timeframe)
{
   ...
   condition.Add(new MAPriceCondition(symbol, tf1, FirstAboveSecond, method1, period1, PRICE_CLOSE), false);
   condition.Add(new MAPriceCondition(symbol, tf2, FirstAboveSecond, method2, period2, PRICE_CLOSE), false);
   condition.Add(new MAPriceCondition(symbol, tf3, FirstAboveSecond, method3, period3, PRICE_CLOSE), false);
   condition.Add(new MAPriceCondition(symbol, tf4, FirstAboveSecond, method4, period4, PRICE_CLOSE), false);
   condition.Add(new LongCondition(symbol, tf_vpc), false);
...
ICondition* CreateShortCondition(string symbol, ENUM_TIMEFRAMES timeframe)
{
   ...
   AndCondition* condition = new AndCondition();
   condition.Add(new MAPriceCondition(symbol, tf1, FirstBelowSecond, method1, period1, PRICE_CLOSE), false);
   condition.Add(new MAPriceCondition(symbol, tf2, FirstBelowSecond, method2, period2, PRICE_CLOSE), false);
   condition.Add(new MAPriceCondition(symbol, tf3, FirstBelowSecond, method3, period3, PRICE_CLOSE), false);
   condition.Add(new MAPriceCondition(symbol, tf4, FirstBelowSecond, method4, period4, PRICE_CLOSE), false);
   condition.Add(new ShortCondition(symbol, tf_vpc), false);
   #ifdef ACT_ON_SWITCH_CONDITION
      ActOnSwitchCondition* switchCondition = new ActOnSwitchCondition(symbol, timeframe, (ICondition*) condition);
      condition.Release();
      return switchCondition;
