...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=73663
...
string file_custom_indicator = "3 Tier London Breakout V.3.2b";
...
input bool closeCandle = true; // Close Candle Mode:
...
input bool tpByIndicator = true; // TP By Fibo Level On?
input int uTpLevel = 2; // Select the level to tp (from 0 to 5):
...
input bool slByIndicator = true; // SL By Fibo Level On?
input int uSlLevel = 2; // Select the level to sl (from 0 to 5):
#ifdef SPREAD_FILTER
...
input bool uDailyQntTradesOn = true; // Daily count of trades On ?
input int uMaxTradesToday = 2; // Max # of trades daily:
...
#define __martingaleIncrementingLots
#ifdef __martingaleIncrementingLots
...
class CountTradesAtDay : public iConditions
{
    int _max;
    int _magic;
    ...
    public:
    CountTradesAtDay(int max, int magic) : _max(max), _magic(magic) { ; }
    ~CountTradesAtDay() { ; }
    ...
    int TodayTrades()
    {
        ...
        datetime endDay = iniDay + (24 * 60 * 60);
        int      count = 0;
        ...
        for(int i = OrdersTotal() - 1; i >= 0; i--)
        {
            ...
            {
                if(OrderOpenTime() >= iniDay && OrderOpenTime() <= endDay)
                {
                    count++;
                }
            }
...
bool CloseCandleMode = closeCandle;  // meter en la clase CloseCandle
...
class BUYcondition1 : public iConditions
{
    ...
    bool evaluate()
    {
        ...
        if(closeCandle)
            if(prBuy != 0 && iClose(NULL, 0, 1) > prBuy && iClose(NULL, 0, 2) < prBuy) return true;
        if(!closeCandle)
            if(prBuy != 0 && Ask > prBuy && Bid < prBuy) return true;
...
class SELLcondition1 : public iConditions
{
    ...
    bool evaluate()
    {
        ...
        if(closeCandle)
            if(prSell != 0 && iClose(NULL, 0, 1) < prSell && iClose(NULL, 0, 2) > prSell) return true;
        if(!closeCandle)
            if(prBuy != 0 && Bid < prSell && Ask > prSell) return true;
        return false;
    }
};
...
int OnInit()
{
    ...
    martin = new MartingaleIncrementLot(magico, _Symbol);

    //--- CONDITIONS TO OPEN TRADES:
    //--- buys:
    conditionsToBuy.AddCondition(buyCondition1 = new BUYcondition1());
    if(uDailyQntTradesOn) conditionsToBuy.AddCondition(cTradesToday = new CountTradesAtDay(uMaxTradesToday, magico));
    ...
    if(uDailyQntTradesOn) conditionsToSell.AddCondition(cTradesToday = new CountTradesAtDay(uMaxTradesToday, magico));
...
void OnTick()
{
    ...
    iCustom(NULL, 0, file_custom_indicator, Info, StartTime, EndTime, SessionEndTime, SessionColor, NumDays, MinBoxSizeInPips, MaxBoxSizeInPips, LimitBoxToMaxSize, StickBoxToLatestExtreme, StickBoxOusideSRlevels, TP1Factor, TP3Factor, TP5Factor, TP2_help, TP4_help, LevelsResizeFactor, BoxColorOK, BoxColorNOK, BoxColorMAX, LevelColor, FibLength, showProfitZone, ProfitColor, objPrefix, 0, 0);
    updatePrices();
...
double SL(string side, double price = 0)
{
    ...
    if(slByIndicator) { result = side == "buy" ? buysl[uSlLevel] : sellsl[uSlLevel]; }
...
double TP(string side, double price = 0)
{
    ...
    if(tpByIndicator) { result = side == "buy" ? sellsl[uSlLevel] : buysl[uSlLevel]; }
...
double Lots()
{
    ...
    if(MartingaleON)
    {
        if(martin.Lot() > 0) lots = martin.Lot();
    }
...
void updatePrices()
{
    ...
    {
        name = ObjectName(i);
        int type = ObjectGet(name, OBJPROP_TYPE);
        if(type == 10)
        {

            double newSellPrice, newBuyPrice;

            newSellPrice = ObjectGetDouble(0, name, OBJPROP_PRICE1);
            if(newSellPrice != prSell)
            {
                prSell = newSellPrice;
                Print(__FUNCTION__, " prSell: ", prSell);
                updateTargets();
            }

            newBuyPrice = ObjectGetDouble(0, name, OBJPROP_PRICE2);
            if(newBuyPrice != prBuy)
            {
                prBuy = newBuyPrice;
                Print(__FUNCTION__, " prBuy: ", prBuy);
                updateTargets();
            }
...
void updateTargets()
{
    ...
    for(int i = obj_total - 1; i >= 0; i--)
    {
        ...
        if(type == OBJ_FIBO)
        {
            ...
            double level0 = ObjectGetDouble(0, name, OBJPROP_LEVELVALUE, 0);
            double level1 = ObjectGetDouble(0, name, OBJPROP_LEVELVALUE, 1);
            double level2 = ObjectGetDouble(0, name, OBJPROP_LEVELVALUE, 2);
            double level3 = ObjectGetDouble(0, name, OBJPROP_LEVELVALUE, 3);
            double level4 = ObjectGetDouble(0, name, OBJPROP_LEVELVALUE, 4);
            double level5 = ObjectGetDouble(0, name, OBJPROP_LEVELVALUE, 5);
            double level6 = ObjectGetDouble(0, name, OBJPROP_LEVELVALUE, 6);
            double level7 = ObjectGetDouble(0, name, OBJPROP_LEVELVALUE, 7);
            double level8 = ObjectGetDouble(0, name, OBJPROP_LEVELVALUE, 8);
            double level9 = ObjectGetDouble(0, name, OBJPROP_LEVELVALUE, 9);
            double level10 = ObjectGetDouble(0, name, OBJPROP_LEVELVALUE, 10);
            double level11 = ObjectGetDouble(0, name, OBJPROP_LEVELVALUE, 11);
            
            
            double dist = prBuy - prSell;
            sellsl[0] = prBuy;
            sellsl[1] = prSell + (dist * level3);
            sellsl[2] = prSell + (dist * level5);
            sellsl[3] = prSell + (dist * level7);
            sellsl[4] = prSell + (dist * level9);
            sellsl[5] = prSell + (dist * level11);

            buysl[0] = prSell;
            buysl[1] = prSell + (dist * level2);
            buysl[2] = prSell + (dist * level4);
            buysl[3] = prSell + (dist * level6);
            buysl[4] = prSell + (dist * level8);
            buysl[5] = prSell + (dist * level10);

            //   for (int i = 0; i < ArraySize(sellsl); i++) { Print(__FUNCTION__, " sell sl: ",i," ", sellsl[i]); }
            //   for (int i = 0; i < ArraySize(sellsl); i++) { Print(__FUNCTION__, " buy sl: ",i," ", buysl[i]); }
