...
Version:     
Date:        
Repository:  Available @ https://fxcodebase.com/code/viewtopic.php?f=38&p=160416#p160416
License:     GNU
...
string file_custom_indicator = "SuperTrend.ex4";
int    bufferToBuy           = 0;
int    bufferToSell          = 1;
int    signalBuffer          = 2;
...
input bool         uCloseOnOpposite = true;       // Close On Opposite Signal:
...
input int                maSlowPeriod         = 200;                                 // Period
...
class BUYcondition1 : public iConditions
{
  public:
    bool evaluate()
    {
        ...
        bool closingPrice, crossover;
        crossover = maFast.index(1)>maSlow.index(1) && maFast.index(2)<=maSlow.index(2);
        closingPrice = iClose(NULL,0,1)>maSlow.index(1);

        return crossover && closingPrice;
    }
};
...
class BUYcondition2 : public iConditions
{
  public:
    bool evaluate()
    {
        ...
        double signal = Indi(signalBuffer, 1);
...
class SELLcondition1 : public iConditions
{
  public:
    bool evaluate()
    {
        ...
        bool crossover;
        crossover = maFast.index(1)<maSlow.index(1) && maFast.index(2)>=maSlow.index(2);
        bool closingPrice = iClose(NULL,0,1)<maSlow.index(1);
...
void OnTick()
{
    ...
    // ——————————————————————————————————————————————————————————————————
    // Close on opposite signal logic
    if (uCloseOnOpposite) {
        if (!uTradeReverse) {
            // Normal mode: close buy orders when sell signal appears
            if (conditionsToCloseBuy.EvaluateConditions()) {
                Print("Close on Opposite Signal: Closing BUY orders due to SELL signal");
                closeAll("buy");
            }
            // Normal mode: close sell orders when buy signal appears  
            if (conditionsToCloseSell.EvaluateConditions()) {
                Print("Close on Opposite Signal: Closing SELL orders due to BUY signal");
                closeAll("sell");
            }
        }
        if (uTradeReverse) {
            // Reverse mode: close buy orders when buy signal appears (reverse logic)
            if (conditionsToCloseSell.EvaluateConditions()) {
                Print("Close on Opposite Signal (Reverse): Closing BUY orders due to BUY signal");
                closeAll("buy");
            }
            // Reverse mode: close sell orders when sell signal appears (reverse logic)
            if (conditionsToCloseBuy.EvaluateConditions()) {
                Print("Close on Opposite Signal (Reverse): Closing SELL orders due to SELL signal");
                closeAll("sell");
            }
        }
    }
