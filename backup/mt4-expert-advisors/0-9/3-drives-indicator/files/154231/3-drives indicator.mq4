// Available @  https://fxcodebase.com/code/viewtopic.php?f=38&t=74579
...
//--- input parameters
string T0                    = "== ZigZag Setup ==";  // ZigZag Setup
bool   zzOn                  = false;                 // Draw ZigZag Line?
input int    InpDepth              = 10;                    // Depth
int    InpDeviation          = 5;                     // Deviation
int    InpBackstep           = 3;                     // Backstep
color  zzClr                 = Navy;                  // ZigZag Color
...
void DrawLabel(int index, string side, string txt)
{
    ...
    if(txt == "BUY")
    {
        BMSup[shift] = price - (vShift * _Point);
    }
    if(txt == "SELL")
    {
        BMSdn[shift] = price + (vShift * _Point);
    }
...
void FindDrives()
{
    if (price(1) > price(3) && price(3) > price(5) && price(2) < price(1) && price(4) < price(3)) {
            DrawLabel(1, "dn", "SELL");                
            // Notifications(1, _bar);
    }
