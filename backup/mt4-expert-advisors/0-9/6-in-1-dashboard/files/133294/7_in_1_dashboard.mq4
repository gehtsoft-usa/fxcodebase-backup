...
public:
   ...
   virtual bool IsPass(const int period, const datetime date)
   {
      ...
      {
         double value = iCustom(_symbol, _timeframe, "ZigZag", ZZLength, ZZPatternLength, ZZPatternWidth, 0, period + i);
...
public:
   ...
   virtual bool IsPass(const int period, const datetime date)
   {
      double value = iCustom(_symbol, _timeframe, "MEGA_FX", false, 0, period);
      return value > 0;
   }
};
...
int init()
{
   ...
   temp = iCustom(NULL, 0, "ZigZag", 0, 0);
   if (GetLastError() == ERR_INDICATOR_CANNOT_LOAD)
   {
      Alert("Please, install the ZigZag indicator");
      return INIT_FAILED;
   }
   temp = iCustom(NULL, 0, "MEGA_FX", 0, 0);
   if (GetLastError() == ERR_INDICATOR_CANNOT_LOAD)
   {
   ...
   temp = iCustom(NULL, 0, "5-stochasti", 0, 0);
   if (GetLastError() == ERR_INDICATOR_CANNOT_LOAD)
   {
