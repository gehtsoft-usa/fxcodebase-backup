...
class F30UpCondition : public ACondition
{
public:
   F30UpCondition(const string symbol, ENUM_TIMEFRAMES timeframe)
   ...
   {
      double value = iCustom(_symbol, _timeframe, "1-F30", 0, period);
      return value < iClose(_symbol, _timeframe, period);
   }
};

class F30DownCondition : public ACondition
{
public:
   F30DownCondition(const string symbol, ENUM_TIMEFRAMES timeframe)
      :ACondition(symbol, timeframe)
   {
   }
...
ICondition* CreateUpCondition(string symbol, ENUM_TIMEFRAMES timeframe)
{
   if (symbol == "F30")
   {
      return new F30UpCondition(_Symbol, timeframe);
   }
...
ICondition* CreateDownCondition(string symbol, ENUM_TIMEFRAMES timeframe)
{
   if (symbol == "F30")
   {
      return new F30DownCondition(_Symbol, timeframe);
   }
   else if (symbol == "ZigZag")
   {
...
int init()
{
   double temp = iCustom(NULL, 0, "1-F30", 0, 0);
   if (GetLastError() == ERR_INDICATOR_CANNOT_LOAD)
   {
      Alert("Please, install the '1-F30' indicator");
      return INIT_FAILED;
   }
