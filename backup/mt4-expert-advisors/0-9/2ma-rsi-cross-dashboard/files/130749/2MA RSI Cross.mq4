...
extern int            SlowMaPeriod    = 21;            // Slow moving average period
...
extern int            ArrowCodeUp     = 225;          // Arrow code up
extern int            ArrowCodeDn     = 226;          // Arrow code down
...
int init()
{
   ...
   IndicatorShortName(timeFrameToString(TimeFrame)+  " FXitute ");
return(0);
}
...
void doAlert(string doWhat)
{
   ...
   string message;
      ...
      if (previousAlert != doWhat || previousTime != Time[0]) {
          ...
          message = timeFrameToString(_Period)+" "+_Symbol+" at "+TimeToStr(TimeLocal(),TIME_SECONDS)+" myComp "+doWhat;
             if (alertsMessage)     Alert(message);
             if (alertsPushNotif )  SendNotification(message);
             if (alertsEmail)       SendMail(_Symbol+" myComp ",message);
