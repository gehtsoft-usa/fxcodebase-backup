...
input int       wperiod=2000;//williams percent
input int       short_level=-60;//williams level to go short from
input int       long_level=-40;//williams level to go long from
input int       ma_period=21;//williams averaging curve
input int       valid_bars_back=30;//Bars to look back as needed to be below or aabove trigger point
...
input double lowline = -40; // Low line
input double highline = -60; // High line
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      double value = iCustom(_symbol, _timeframe, "WilliamsTrade", wperiod, short_level, long_level, ma_period, valid_bars_back, 3, period);
...
{
public:
   BBLongCondition(const string symbol, ENUM_TIMEFRAMES timeframe)
      :ACondition(symbol, timeframe)
   {
   ...
   bool IsPass(const int period, const datetime date)
   {
      double bbValue = iBands(_symbol, _timeframe, bb_period, bb_deviation, bb_shift, PRICE_CLOSE, MODE_UPPER, period);
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      double value = iCustom(_symbol, _timeframe, "WilliamsTrade", wperiod, short_level, long_level, ma_period, valid_bars_back, 2, period);
      return value != 0 && value != EMPTY_VALUE;
   }
};
...
{
public:
   BBShortCondition(const string symbol, ENUM_TIMEFRAMES timeframe)
      :ACondition(symbol, timeframe)
   {
   ...
   bool IsPass(const int period, const datetime date)
   {
      double bbValue = iBands(_symbol, _timeframe, bb_period, bb_deviation, bb_shift, PRICE_CLOSE, MODE_LOWER, period);
      return bbValue < iClose(_symbol, _timeframe, period);
   }
};
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      double value = iCustom(_symbol, _timeframe, "WilliamsTrade", wperiod, short_level, long_level, ma_period, valid_bars_back, 0, period);
      return value < lowline;
   }
};
...
ICondition* CreateLongCondition(string symbol, ENUM_TIMEFRAMES timeframe)
{
   ...
   condition.Add(new BBLongCondition(symbol, timeframe), false);
...
ICondition* CreateShortCondition(string symbol, ENUM_TIMEFRAMES timeframe)
{
   ...
   condition.Add(new BBShortCondition(symbol, timeframe), false);
   #ifdef ACT_ON_SWITCH_CONDITION
      ActOnSwitchCondition* switchCondition = new ActOnSwitchCondition(symbol, timeframe, (ICondition*) condition);
      condition.Release();
      return switchCondition;
