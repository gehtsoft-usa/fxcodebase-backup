...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      ...
      return maOnArray[index] > maOnArray[index - 1] 
         && maOnArray[index] > long_level 
         && maOnArray[index - 1] <= long_level 
         && maOnArray[index - valid_bars_back] < long_level - 10;
   }
};
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      int index = iBars(_symbol, _timeframe) - 1 - period;
      return maOnArray[index] < maOnArray[index - 1] 
         && maOnArray[index] < short_level 
         && maOnArray[index - 1] >= short_level 
         && maOnArray[index - valid_bars_back] > short_level + 10;
   }
};
...
int OnInit(void)
{
   IndicatorObjPrefix = GenerateIndicatorPrefix("Williams_Trade");
   IndicatorSetString(INDICATOR_SHORTNAME, "Williams_Trade");
   ...
   SetIndexBuffer(id, ma, INDICATOR_DATA);
   PlotIndexSetInteger(id, PLOT_DRAW_TYPE, DRAW_LINE);
   PlotIndexSetInteger(id, PLOT_LINE_COLOR, Red);
   ++id;

   SetIndexBuffer(id, maOnArray, INDICATOR_DATA);
   PlotIndexSetInteger(id, PLOT_DRAW_TYPE, DRAW_LINE);
   PlotIndexSetInteger(id, PLOT_LINE_COLOR, DodgerBlue);
   ++id;
   ...
   lowStream.Release();
   highStream.Release();
   wpr = iWPR(_Symbol, _Period, wperiod);
   maWpr = iMA(_Symbol, _Period, ma_period, 0, MODE_SMA, wpr);
...
{
   IndicatorRelease(wpr);
   IndicatorRelease(maWpr);
   delete mainSignaler;
   mainSignaler = NULL;
...
)
{
   if (prev_calculated <= 0 || prev_calculated > rates_total)
   {
      ...
      ArrayInitialize(maOnArray, EMPTY_VALUE);
      for (int i = 0; i < ArraySize(alerts); ++i)
      {
         alerts[i].Init();
      }
   }
   int first = valid_bars_back;
   for (int pos = MathMax(first, prev_calculated - 1); pos < rates_total; ++pos)
   {
      int oldIndex = rates_total - 1 - pos;
      double buffer[1];
      if (CopyBuffer(wpr, 0, oldIndex, 1, buffer) != 1)
      {
         continue;
      }
      ma[pos] = buffer[0];
      if (CopyBuffer(maWpr, 0, oldIndex, 1, buffer) != 1)
      {
         continue;
      }
      maOnArray[pos] = buffer[0];
      for (int i = 0; i < ArraySize(alerts); ++i)
      {
