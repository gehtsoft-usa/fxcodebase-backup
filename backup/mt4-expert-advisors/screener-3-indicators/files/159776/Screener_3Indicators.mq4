...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=76100
...
input int  back    = 5;    // Candles Back To Search:
input bool capOn   = true; // Cap Filter On:
input bool peakOn  = true; // Peak Filter On:
input bool cycleOn = true; // Cycle Filter On:
...
int CapPosition(string symbol, int timeframe, int shift = 0)
{
    if (!capOn) return 2;

    string cap_channel_file = "Cap Channel Trading Indicator MT4.ex4";
    int    bufferUpper      = 1; // Buffer de la banda superior
    int    bufferLower      = 0; // Buffer de la banda inferior

    double upper = iCustom(symbol, timeframe, cap_channel_file, bufferUpper, shift);
    // //Print("line: ",__LINE__," upper: ",upper, " symbol: ", symbol);
    double lower = iCustom(symbol, timeframe, cap_channel_file, bufferLower, shift);
...
int PeakPosition(string symbol, int timeframe, int shift = 0)
{
    if (!peakOn) return 2;

    // Cambia el nombre del archivo y los índices de buffer según corresponda
    string indi_file    = "Peak.ex4";
    int    buffer_green = 0; // Buffer Green
    int    buffer_red   = 1; // Buffer Rojo

    double _green = iCustom(symbol, timeframe, indi_file, buffer_green, shift);
    double _red   = iCustom(symbol, timeframe, indi_file, buffer_red, shift);

    if (_green > 25) return 1;
    if (_red > 25) return -1;
    return 0;
}
int CyclePosition(string symbol, int timeframe, int shift = 0)
{
    if (!cycleOn) return 2;

    string indi_file   = "Cycle identifier rsi.ex4";
    int    buffer_blue = 3; // Buffer Azul
    int    buffer_red  = 2; // Buffer Rojo

    double _blue = iCustom(symbol, timeframe, indi_file, buffer_blue, shift);
    double _red  = iCustom(symbol, timeframe, indi_file, buffer_red, shift);
...
class FinderPattern
{
  ...
  public:
    FinderPattern(string Symbol) { _symbol = Symbol; }
    ~FinderPattern() { ; }
    ...
    {
        if (capOn == false && peakOn == false && cycleOn == false) return false;

        // TODO: patrón alcista
        int cap, cycle, peak;

        cap = CapPosition(_symbol, tf, i);
        if (cap == 1 || cap == 2) {
            peak = PeakPosition(_symbol, tf, i);
            if (peak == 1 || peak == 2) {
                cycle = CyclePosition(_symbol, tf, i);
                if (cycle == 1) {
                return true;
                }
            }
...
void doActions()
{
	...
	for(int i=0;i < ArraySize(finders) ; i++)
	{
		...
		for (int j = 0; j < ArraySize(TF); j++) 
		{
			...
			value[j] = StringSubstr(patCode,0,2);
		...
		for(int n=0;n < 16;n++)
		{
	 		...
	 		if(!esPar(n)) { int pos = n/2; results[n] = value[pos]; }
		}
...
//--- Draw arrow in current timeframe
	...
	if(rowSymbol!= -1)
	{
	if(Period() == PERIOD_M1 ) candle = table.getValue(rowSymbol, 2) ;
	if(Period() == PERIOD_M5 ) candle = table.getValue(rowSymbol, 4) ;
	if(Period() == PERIOD_M15) candle = table.getValue(rowSymbol, 6) ;
	if(Period() == PERIOD_M30) candle = table.getValue(rowSymbol, 8) ;
	if(Period() == PERIOD_H1 ) candle = table.getValue(rowSymbol, 10);
	if(Period() == PERIOD_H4 ) candle = table.getValue(rowSymbol, 12);
	if(Period() == PERIOD_D1 ) candle = table.getValue(rowSymbol, 14);
	if(Period() == PERIOD_W1 ) candle = table.getValue(rowSymbol, 16);
