...
input int distance = 10; // Distance to orders, pips. Use 0 for market
input string opposite_symbol = "EUR/USD"; // Opposite symbol
...
input bool order_expiry = false;    // Order expiry at end of the day
input int order_expiry_days = 0; // Order expiry in days
...
#import
datetime todayDate, oed, expiringDate;
class Signaler
  {
   string            _prefix;
public:
...
public:
   ...
   bool              GetValue(const int period, double &val)
     {
      val = iClose(_symbol, _timeframe, period) + distance * _instrument.GetPipSize();
      return true;
     }
  };
         ...
         case ERR_INVALID_STOPS:
           {
            ...
            if(newStopLoss != 0.0 && MathRound(MathAbs(rate - newStopLoss) / point) < minStopDistancePoints)
               ...
               if(newTakeProfit != 0.0 && MathRound(MathAbs(rate - newTakeProfit) / point) < minStopDistancePoints)
   ...
   int               Execute(string &errorMessage)
     {
      ...
      if(order_expiry)
        {
         oed = order_expiry_days;
         if(order_expiry_days < 1)
            oed = 1;
         todayDate = iTime(Symbol(), PERIOD_D1, 0);
         expiringDate = todayDate + order_expiry_days * 1440 * 60;
        }
      if(_ecnBroker)
         order = OrderSend(_instrument, orderType, _amount, rate, _slippage, 0, 0, _comment, _magicNumber, expiringDate);
      else
         order = OrderSend(_instrument, orderType, _amount, rate, _slippage, sl, tp, _comment, _magicNumber, expiringDate);
      if(order == -1)
        {
         int error = GetLastError();
         switch(error)
         ...
         if(_ecnBroker && (_stopLoss != 0 || _takeProfit != 0))
           {
      ...
      for(int i = 0; i < _retries; ++i)
        {
         ...
         if(_ecnBroker)
            ...
            order = OrderSend(_instrument, orderType, _amount, rate, _slippage, _stopLoss, _takeProfit, _comment, _magicNumber, expiringDate);
         if(order == -1)
           {
            int error = GetLastError();
            switch(error)
   ...
   bool              IsPass(const int period, const datetime date)
     {
      for(int i = 0; i < 1000; ++i)
        {
         double buyValue = iCustom(_symbol, _timeframe, "HOLY GRAIL 1.6", MAIN_PERIOD, ADX_PERIOD, 1, period);
         double sellValue = iCustom(_symbol, _timeframe, "HOLY GRAIL 1.6", MAIN_PERIOD, ADX_PERIOD, 0, period);
         if(sellValue != EMPTY_VALUE)
           {
            return false;
           }
         if(buyValue != EMPTY_VALUE)
           {
            return true;
           }
        }
...
TradingController *CreateController(const string symbol, const ENUM_TIMEFRAMES timeframe, string algoId, string &error)
  {
   ...
   if(distance == 0)
   ...
   else
     {
      ...
      entryStrategy = new PendingEntryStrategy(symbol, magic_number, slippage_points, longPrice, shortPrice, actions, ecn_broker);
     }
   IEntryStrategy* entryStrategyOpposite;
   if(distance == 0)
     {
      entryStrategyOpposite = new MarketEntryStrategy(opposite_symbol, magic_number, slippage_points, actions, ecn_broker);
     }
   else
     {
      AStream *longPrice = new LongEntryStream(opposite_symbol, timeframe);
      AStream *shortPrice = new ShortEntryStream(opposite_symbol, timeframe);
      entryStrategyOpposite = new PendingEntryStrategy(opposite_symbol, magic_number, slippage_points, longPrice, shortPrice, actions, ecn_broker);
     }
   ...
   EntryPositionController* oppositeLongPosition = new EntryPositionController(SellSide, longCondition, longFilterCondition,
         entryStrategyOpposite, closeOnOpposite, orderHandlers, signaler, "", "Sell");
   EntryPositionController* oppositeShortPosition = new EntryPositionController(BuySide, shortCondition, shortFilterCondition,
         entryStrategyOpposite, closeOnOpposite, orderHandlers, signaler, "", "Buy");
   ...
   TradingController* oppositeController = new TradingController(tradingCalculator, timeframe, timeframe, oppositeLongPosition, oppositeShortPosition, actions, signaler, algoId);
...
// #endif
   ...
   switch(logic_direction)
     {
      case DirectLogic:
         ...
         oppositeController.SetExitLongCondition(exitLongCondition);
         oppositeController.SetExitShortCondition(exitShortCondition);
         break;
      case ReversalLogic:
         controller.SetExitLongCondition(exitShortCondition);
         controller.SetExitShortCondition(exitLongCondition);
         oppositeController.SetExitLongCondition(exitShortCondition);
         oppositeController.SetExitShortCondition(exitLongCondition);
         break;
     }
   ...
   IMoneyManagementStrategy* oppositeLongMoneyManagement = CreateMoneyManagementStrategy(tradingCalculator, opposite_symbol, timeframe, true,
         lots_type, lots_value, stop_loss_type, stop_loss_value, stop_loss_atr_multiplicator, take_profit_type, take_profit_value, take_profit_atr_multiplicator);
   IMoneyManagementStrategy* oppositeShortMoneyManagement = CreateMoneyManagementStrategy(tradingCalculator, opposite_symbol, timeframe, false,
         lots_type, lots_value, stop_loss_type, stop_loss_value, stop_loss_atr_multiplicator, take_profit_type, take_profit_value, take_profit_atr_multiplicator);
   oppositeLongPosition.AddMoneyManagement(oppositeLongMoneyManagement);
   oppositeShortPosition.AddMoneyManagement(oppositeShortMoneyManagement);
#ifdef NET_STOP_LOSS_FEATURE
   ...
   oppositeController.SetEntryLogic(entry_logic);
   oppositeController.SetEntryStrategy(entryStrategyOpposite);
   entryStrategy.Release();
   entryStrategyOpposite.Release();
   if(print_log)
     {
   ...
   controllers[controllersCount++] = oppositeController;
   return controller;
  }
...
int OnInit()
  {
   double temp = iCustom(NULL, 0, "HOLY GRAIL 1.6", MAIN_PERIOD, ADX_PERIOD, 0, 0);
   if(GetLastError() == ERR_INDICATOR_CANNOT_LOAD)
     {
      Alert("Please, install the 'HOLY GRAIL 1.6' indicator");
      return INIT_FAILED;
     }
#ifdef SHOW_ACCOUNT_STAT
   stats = NULL;
