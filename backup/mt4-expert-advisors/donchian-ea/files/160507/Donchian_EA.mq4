...
Version:     
Date:        
Repository:  Available @ https://fxcodebase.com/code/viewtopic.php?f=38&p=160373#p160373
License:     GNU
...
// Reverse/Spread/Slippage
enum ModeDirection { Direct, Reverse };
input string        TexecExtras          = "== Execution Extras =="; // ————————————————————————
input ModeDirection direction            = Direct;                    // Trade Direction: Direct/Reverse
input double        max_spread_pips      = 3;                         // Max allowed spread (pips)
input int           user_slippage_pips   = 2;                         // Slippage (pips)
...
// Helpers: pip/points, spread, slippage
int PointsPerPip(string symbol = "")
{
    string s = (symbol == "" ? Symbol() : symbol);
...
{
    string s = (symbol == "" ? Symbol() : symbol);
    return (int)MathRound(user_slippage_pips * PointsPerPip(s));
}

double CurrentSpreadPips(string symbol = "")
{
    string s   = (symbol == "" ? Symbol() : symbol);
    double ask = SymbolInfoDouble(s, SYMBOL_ASK);
    double bid = SymbolInfoDouble(s, SYMBOL_BID);
    double pip = MarketInfo(s, MODE_POINT) * PointsPerPip(s);
...
#define Section_Martingale
#ifdef Section_Martingale
// Mark: martingale

enum incrementMode {
    mode_multiply, // Multiplier
    mode_addition, // Addition
};

input string        tMartingale                 = "== Martingale Setup =="; // ————————————————————————
input bool          MartingaleON                = true;                     // Use Martingale:
input incrementMode martingale_mode             = mode_multiply;            // Increment Mode:
input bool          considerOpenTrades          = true;                     // Consider Open Trades ?
input int           MartingaleUser_maxCount     = 5;                        // Max steps:
input double        MartingaleUser_maxLot       = 10;                       // Max lot value:
input double        MartingaleUser_multiplier   = 2;                        // Multiplier:
input double        MartingaleUser_additionLots = 0.10;                     // Addition Lots:
...
class MartingaleIncrementLot
{
  ...
  public:
    ...
    double Lot()
    {
        setLastValues();
        double next = 0;
        if (martingale_mode == mode_multiply) {
            next = NormalizeDouble(_lastLot * _multiplier, 2);
        }
        if (martingale_mode == mode_addition) {
            next = NormalizeDouble(_lastLot + MartingaleUser_additionLots, 2);
        }
...
#define Section_grid_Positive
#ifdef Section_grid_Positive
// Mark: Section_grid_Positive

enum enum_addition_mode {
    grid_mode_multiplier, // Multiplier
    grid_mode_addition    // Addition
};

input string       tGrid               = "== Grid Setup ==";   // ————————————————————————
input const bool   grid_p_on           = false;                // Grid ON:
input const double grid_p_step         = 10;                   // Grid Step:
input enum_addition_mode grid_add_mode      = grid_mode_multiplier;      // Increment lots mode:
input double       grid_p_m            = 2;                    // Grid Lot Multiplier:
input double       grid_p_a            = 0.1;                  // Grid Lot Addition:
input const int    grid_p_max_steps    = 5;                    // Grid Max Steps:
input double       grid_p_close_profit = 50;                   // Profit to close (0 = off):
input double       grid_p_close_loss   = 50;                   // Loss to close (0 = off):

class ConditionGridPositive : public iConditions
{
  public:
    bool evaluate()
    {
...
ConditionGridPositive cdGridPositive;

class ActionGridPositive : public iActions
{
  public:
    bool execute()
    {
        double mPoints    = MarketInfo(OrderSymbol(), MODE_POINT);
        double step       = grid_p_step * mPoints * 10;
        string grid_p_id    = "grid_p_" + (string)OrderTicket();
        double origin_lot = OrderLots();
        double open_price = OrderOpenPrice();

        if (OrderType() == OP_BUY) {
            for (int n = 1; n <= grid_p_max_steps; n++) {
                double last_step = open_price + (step * (double)n);
                if (Ask > open_price + (step * ((double)n + 1))) continue;

                if (Ask >= last_step && !HaveOrderInThisStep(grid_p_id)) {
                    int r = OrderSend(NULL, OP_BUY, GridLot(origin_lot, (double)n), Ask, 10000, 0, 0, grid_p_id, magico, 0, SkyBlue);
                }
            }
        }
        ...
        if (OrderType() == OP_SELL) {
            for (int n = 1; n <= grid_p_max_steps; n++) {
                double last_step = open_price - (step * (double)n);
                if (Bid < open_price - (step * ((double)n + 1))) continue;

                if (Bid <= last_step && !HaveOrderInThisStep(grid_p_id)) {
                    int r = OrderSend(NULL, OP_SELL, GridLot(origin_lot, (double)n), Bid, 10000, 0, 0, grid_p_id, magico, 0, Pink);
                }
            }
        }
    ...
    bool HaveOrderInThisStep(string grid_p_id)
    {
        double mPoints = MarketInfo(OrderSymbol(), MODE_POINT);
        double step    = grid_p_step * mPoints * 10;
        ...
        for (int i = OrdersTotal() - 1; i >= 0; i--) {
            if (OrderSelect(i, SELECT_BY_POS) && OrderSymbol() == _Symbol && OrderMagicNumber() == magico) {
                if (StringFind(OrderComment(), grid_p_id) >= 0) {
    ...
    double GridLot(double lot, double n)
    {
        ...
        switch (grid_add_mode)            
        {
            case grid_mode_multiplier:
                l = lot * pow(grid_p_m, n);
            break;
        
            case grid_mode_addition:
                l = lot + (grid_p_a * n);
            break;
        }
        return l;
...
ActionGridPositive acGridPositive;

Strategy   stGridPositive(&cdGridPositive, &acGridPositive);

// Mark: Section_CloseGrid_Positive
class ConditionOriginalTkIsClosePositive : public iConditions
{
  public:
    ...
    bool evaluate()
    {
        if (StringFind(OrderComment(), "grid") < 0) return false;
        string grid_p_id = OrderComment();
        return OriginalIsClose(grid_p_id);
    }
};
ConditionOriginalTkIsClosePositive cdOriginalTkIsClosePositive;

class ConditionCloseGridPositiveByProfit : public iConditions
{
  public:
    bool evaluate()
    {
        if (StringFind(OrderComment(), "grid_p") >= 0) return false;

        string grid_p_id        = "grid_p" + (string)OrderTicket();
        double current_profit = OrderProfit() + OrderCommission() + OrderSwap(); // profit de la original

        for (int i = OrdersTotal() - 1; i >= 0; i--) {
            if (OrderSelect(i, SELECT_BY_POS) && OrderSymbol() == _Symbol && OrderMagicNumber() == magico && StringFind(OrderComment(), grid_p_id) >= 0) {
                current_profit += OrderProfit() + OrderCommission() + OrderSwap();
            }
        }

        if (grid_p_close_profit > 0 && current_profit >= grid_p_close_profit) return true;
        if (grid_p_close_loss > 0 && current_profit <= -grid_p_close_loss) return true;

        return false;
    }
};
ConditionCloseGridPositiveByProfit cdCloseGridPositiveByProfit;

class ActionCloseGridPositive : public iActions
{
  public:
    bool execute()
    {
        int    tk      = OrderTicket();
        string grid_p_id = "grid_p" + (string)tk;

        for (int i = OrdersTotal() - 1; i >= 0; i--) {
            if (OrderSelect(i, SELECT_BY_POS) && OrderSymbol() == _Symbol && OrderMagicNumber() == magico && (StringFind(OrderComment(), grid_p_id) >= 0 || OrderTicket() == tk)) {
                double price = OrderType() == OP_BUY ? Bid : Ask;
                int    r     = OrderClose(OrderTicket(), OrderLots(), price, 10000, clrGray);
            }
        }
        return true;
    }
};
ActionCloseGridPositive acCloseGridPositive;

Strategy stCloseGridPositiveByProfit(&cdCloseGridPositiveByProfit, &acCloseGridPositive);
Strategy stCloseGridPositiveBecauseOriginalTradeIsClose(&cdOriginalTkIsClosePositive, &acCloseGridPositive);

void OnInit_Grid_Positive()
{
    Trader.management.add(&stGridPositive);
    Trader.management.add(&stCloseGridPositiveByProfit);
    Trader.management.add(&stCloseGridPositiveBecauseOriginalTradeIsClose);
}
...
#define Section_Traling_Stop_byMoney
#ifdef Section_Traling_Stop_byMoney

input string       money_ttsl         = "== TrailingStop Setup =="; // ————————————————————————
input const bool   money_tsl_on       = false;                      // Money TSL ON:
input const double money_tsl_start    = 10;                         // Money TSL Start:
double money_tsl_step                 = 1;                        // Money TSL Step:
input const double money_tsl_distance = 20;                         // Money TSL Distance:


class ConditionMoneyTSL : public iConditions
{
  public:
    bool evaluate()
    {
...
ConditionMoneyTSL conditionMoneyTSL;

double SL_ByMoney(double money_sl, string side, string symbol, double lot)
{
    double _tickValue    = MarketInfo(symbol, MODE_TICKVALUE);
    double _modeCalc     = MarketInfo(symbol, MODE_PROFITCALCMODE);
    double _contractSize = SymbolInfoDouble(symbol, SYMBOL_TRADE_CONTRACT_SIZE);
    ...
    // FOREX
    if (_modeCalc == 0) {
        _pips = NormalizeDouble(money_sl / (lot * _tickValue), (int)_digits);
        Print("line: ",__LINE__," _pips: ",_pips);
    }

    // FUTUROS
    if (_modeCalc == 1 && _step != 1.0) {
        double c = _contractSize * _step;
        _pips    = NormalizeDouble((money_sl / c / lot), 2);
    }

    // FUTUROS SIN DECIMALES
    if (_modeCalc == 1 && _step == 1.0) {
        double c = _contractSize * _step;
        _pips    = MathFloor((money_sl / c / lot) / 100);
    }
...
class ActionMoneyTSL : public iActions
{
  public:
    bool execute()
    {
        // Trailing Stop by Money
        // sumar el total de profit para las ordenes abiertas en el symbolo
        double flotante = 0;
        double lots= 0;
        string side = "";
        for (int i = OrdersTotal() - 1; i >= 0; i--)
        ...
        // para todas las ordenes abiertas en el chart, si el  flotante el mayor a money_tsl_start
        // ---- REVISAR ----
        if (flotante > money_tsl_start) // 
        {
            // double dinero_sl = flotante - money_tsl_distance;
            double new_sl = SL_ByMoney(money_tsl_distance, side, Symbol(), lots);
            Print("line: ",__LINE__," new_sl: ",new_sl);
            // Print("line: ",__LINE__," dinero_sl: ",dinero_sl);

            // modificar el stop loss de todas las ordenes abiertas en el chart por new_sl 
            // si el nuevo stop loss es mayor al actual en caso de Buys o si el nuevo stop loss es menor al actual en caso de sells
            for (int i = OrdersTotal() - 1; i >= 0; i--)
            {
                if (OrderSelect(i, SELECT_BY_POS) && OrderSymbol() == Symbol())
                {
                    if (OrderType() == OP_BUY)
                    {
                        ...
                        {
                            int r = OrderModify(OrderTicket(), OrderOpenPrice(), new_sl, OrderTakeProfit(), OrderExpiration(), clrNONE);
                        }
                    }
                    if (OrderType() == OP_SELL)
                    {
...
ActionMoneyTSL actionMoneyTSL;

Strategy Money_TSL;

void     Initiate_Money_TSL()
{
    Money_TSL.addCondition(&conditionMoneyTSL);
    Money_TSL.addAction(&actionMoneyTSL);
    Trader.management.add(&Money_TSL);
}
...
class OrderCreator : public iActions
{
  ...
  public:
    ...
    bool execute()
    {
        // determine effective side considering Reverse mode
        string side_exec = _side;
        if (direction == Reverse) side_exec = (_side == "buy" ? "sell" : "buy");

        // compute price/lots/sl/tp based on effective side
        _price = ::Price(side_exec);
        setLots();
        _sl = ::StopLoss(_price, side_exec, _symbol, _lots);
        _tp = ::TakeProfit(_price, side_exec, _symbol, _lots);

        // enforce broker stop level safety for SL/TP
        if (_sl > 0) _sl = StopLossSafety(side_exec, _symbol, _sl);
        if (_tp > 0) _tp = tpSafety(side_exec, _symbol, _tp);

        // set order type according to effective side and mode
        // clang-format off
        double ask = SymbolInfoDouble(_symbol, SYMBOL_ASK);
        double bid = SymbolInfoDouble(_symbol, SYMBOL_BID);

        if (modeEntry == Market) {
            if (side_exec == "buy")  { _cmd = OP_BUY; }
            if (side_exec == "sell") { _cmd = OP_SELL; }
        } else {
            if (side_exec == "buy") {
                if (_price > ask)    { _cmd = OP_BUYSTOP; }
                if (_price < ask)    { _cmd = OP_BUYLIMIT; }
            }
            if (side_exec == "sell") {
                if (_price > bid)    { _cmd = OP_SELLLIMIT; }
                if (_price < bid)    { _cmd = OP_SELLSTOP; }
            }
        }
        // clang-format on

        // spread check
        double spr = CurrentSpreadPips(_symbol);
        if (max_spread_pips > 0 && spr > max_spread_pips) {
            Print("Spread too high (", DoubleToString(spr, 1), ") > max (", DoubleToString(max_spread_pips, 1), ")");
            return false;
        }

        int _slippage = SlippagePoints(_symbol);
...
void BasicStrategy_Iniciate()
{
    ...
    {
        LongStrategy.set(&conditionSell, &buy);
        ShortStrategy.set(&conditionBuy, &sell);
    }
...
input const string indicator_file = "Donchian Breakout Syste";                          // Indicator File:
//---
int  bufferToBuy              = 0;
int  bufferToSell             = 1;
...
class ConditionDayLimit : public iConditions
{
  public:
    bool evaluate()
    {
        ...
        double result = 0;
        for (int i = OrdersHistoryTotal() - 1; i >= 0; i--) {
            if (OrderSelect(i, SELECT_BY_POS, MODE_HISTORY) && OrderSymbol() == _Symbol && OrderMagicNumber() == magico && OrderCloseTime() > iTime(NULL, PERIOD_D1, 0)) {
...
void OnInit_DayLimit()
{
    if (!daily_limits_on) return;
    ShortStrategy.addCondition(&cdDayLimit);
    LongStrategy.addCondition(&cdDayLimit);
}
...
if(money_tsl_on) Initiate_Money_TSL();
