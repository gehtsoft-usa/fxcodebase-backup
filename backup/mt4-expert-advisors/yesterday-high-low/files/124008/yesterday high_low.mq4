...
// http://fxcodebase.com/code/viewtopic.php?f=38&t=67367
...
extern ENUM_TIMEFRAMES Close_Candle_TF = PERIOD_CURRENT; // Close Candle TF
extern int StochKPeriod = 5; // Stochastic K Period
extern int StochDPeriod = 3; // Stochastic D Period
extern int StochSlowing = 3; // Stochastic Slowing
extern int MacdFastEmaPeriod = 12; // MACD Fast Period
extern int MacdSlowEmaPeriod = 26; // MACD Slow Period
extern int MacdSignalEmaPeriod = 9; // MACD Signal Period
...
public:
   ...
   bool IsPass(const int period)
   {
      ...
      double macd = iMACD(_symbol, _timeframe, MacdFastEmaPeriod, MacdSlowEmaPeriod, MacdSignalEmaPeriod, PRICE_CLOSE, MODE_SIGNAL, period);
      double stoch = iStochastic(_symbol, _timeframe, StochKPeriod, StochDPeriod, StochSlowing, MODE_SMA, 0, MODE_MAIN, period);
      return close0 < low && close1 >= low && macd < 0 && stoch > 50;
   }
};
...
public:
   ...
   bool IsPass(const int period)
   {
      ...
      double macd = iMACD(_symbol, _timeframe, MacdFastEmaPeriod, MacdSlowEmaPeriod, MacdSignalEmaPeriod, PRICE_CLOSE, MODE_SIGNAL, period);
      double stoch = iStochastic(_symbol, _timeframe, StochKPeriod, StochDPeriod, StochSlowing, MODE_SMA, 0, MODE_MAIN, period);
      return close0 > high && close1 <= high && macd > 0 && stoch < 50;
   }
};
...
int OnInit()
{
   ...
   string error;
   TradeController *controller = CreateController(_Symbol, Close_Candle_TF, error);
   if (controller == NULL)
   {
      Print(error);
      return INIT_FAILED;
