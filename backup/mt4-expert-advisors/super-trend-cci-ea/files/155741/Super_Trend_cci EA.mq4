...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=74973
...
input bool   close_by_arrows_on   = false;                   // Close positions by Arrows Indicator:
...
class ConditionCloseByNRP_Sell : public iConditions
{
  public:
    bool evaluate()
    {
...
ConditionCloseByNRP_Sell cCloseByNrpSell;

class ConditionCloseByNRP_Buy : public iConditions
{
  public:
    bool evaluate()
    {
...
ConditionCloseByNRP_Buy cCloseByNrpBuy;

Strategy           stCloseByNrpBuy(&cCloseByNrpBuy, &acCloseBuy);
Strategy           stCloseByNrpSell(&cCloseByNrpSell, &acCloseSell);
...
void               oninit_close_strategy()
{
    ...
    if(close_by_arrows_on){
        Trader.management.add(&stCloseByNrpBuy);
        Trader.management.add(&stCloseByNrpSell);
...
public:
    bool evaluate()
    {
        ...
        if (OrderType() == OP_BUY)
        {
            r = cTrendSell.evaluate() && cNrpSell.evaluate();
        }
        return r;
    }
};
...
public:
    bool evaluate()
    {
        ...
        if (OrderType() == OP_SELL)
        {
            r = cTrendBuy.evaluate() && cNrpBuy.evaluate();
        }
        return r;
    }
};
...
input const string indicator_file0 = "! SuperTrend - cci (mtf + alerts + arrows + candles + btn).ex4";  // Indicator File:
input const string indicator_file1 = "100% Non Repaint Indicator V22.0.ex4"; // Indicator File:
int bufferToBuy0 = 8;
int bufferToSell0 = 8;
...
// ------------------------------------------------------------------
double nrp_i(int buffer, int candle = 1) { return iCustom(NULL, 0, indicator_file1, buffer, candle); }
int                nrp__buBuy      = 3;
int                nrp__buSel      = 4;
...
{
    return iCustom(NULL, 0, indicator_file0, 
                   buffer, candle);
}

class ConditionIndicatorTrendBuy : public iConditions
{
public:
    bool evaluate()
    {
        double indi = Indi(bufferToBuy0, 1);
        return indi == EMPTY_VALUE;
    }
};
ConditionIndicatorTrendBuy cTrendBuy;
...
public:
    bool evaluate()
    {
        double indi = Indi(bufferToSell0, 1);
        return indi > 0 && indi != EMPTY_VALUE;
    }
};
ConditionIndicatorSell cTrendSell;


bool OnInit_CustomIndicator()
{
    double temp = iCustom(NULL, 0, indicator_file0, 0, 0);
    if (GetLastError() == ERR_INDICATOR_CANNOT_LOAD)
    {
        MessageBox("THIS EA NEED AN INDICATOR\n install the file:\n" + indicator_file0 + "\ninto the folder:\nMQL4/Indicators.", "Important Information", MB_ICONINFORMATION);
        Alert("THIS EA NEED AN INDICATOR, install the file: " + indicator_file0 + " into the folder: MQL4/Indicators.");
    ...
    LongStrategy.addCondition(&cTrendBuy);
    LongStrategy.addCondition(&cNrpBuy);
    ShortStrategy.addCondition(&cTrendSell);
    ShortStrategy.addCondition(&cNrpSell);
