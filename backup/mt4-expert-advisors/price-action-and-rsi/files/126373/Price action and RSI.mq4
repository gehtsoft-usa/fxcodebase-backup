...
// http://fxcodebase.com/code/viewtopic.php?f=38&t=68472
...
MARTINGALE_FEATURE MartingaleType martingale_type = MartingaleOnLoss; // Martingale type
...
NET_STOP_LOSS_FEATURE StopLimitType net_stop_loss_type = StopLimitPips; // Net stop loss type
...
NET_TAKE_PROFIT_FEATURE StopLimitType net_take_profit_type = StopLimitPips; // Net take profit type
...
class APattern : public IPattern
{
protected:
   string _symbol;
   ENUM_TIMEFRAMES _timeframe;
public:
   APattern(const string symbol, const ENUM_TIMEFRAMES timeframe)
   {
      _symbol = symbol;
      _timeframe = timeframe;
   }
...
class InsideBarPattern : public APattern
{
public:
   InsideBarPattern(const string symbol, const ENUM_TIMEFRAMES timeframe) 
   : APattern(symbol, timeframe)
   {
   }

   virtual bool IsPass(const int period)
   {
      return IsPass(_symbol, _timeframe, period);
   }
   
   static bool IsPass(const string symbol, const ENUM_TIMEFRAMES timeframe, const int period)
   {
      return iHigh(symbol, timeframe, period) < iHigh(symbol, timeframe, period + 1)
         && iLow(symbol, timeframe, period) > iLow(symbol, timeframe, period + 1);
   }
};
...
public:
   ABaseCondition(const string symbol, ENUM_TIMEFRAMES timeframe)
   {
      ...
      _timeframe = timeframe;
      _symbol = symbol;
      _pattern = new InsideBarPattern(symbol, timeframe);
   }
   ~ABaseCondition()
   {
      delete _pattern;
...
public:
   ...
   {
      double rsi = iRSI(_symbol, _timeframe, rsi_period, PRICE_CLOSE, period);
      if (rsi < rsi_overbought)
         return false;

      return _pattern.IsPass(period);
   }
};
