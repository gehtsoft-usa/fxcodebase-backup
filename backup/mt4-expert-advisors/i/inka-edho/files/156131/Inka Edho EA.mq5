...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=75063
...
input int uQntGrid = 1; // #Pending orders
...
// MARK: funcion Price
double Price(string side,  int n=1, int pips = 0, string _symbol = "")
{
    ...
    double distance = uEntryDistance * n * 10 * points;
...
int  handle_ma = 0;
void setHandleMA() { handle_ma = iMA(NULL, 0, maPeriod, maShift, maMethod, maAppliedPrice); }
...
double MA(int candle = 1)
{
    ...
    int shift = candle;
    int copy  = CopyBuffer(handle_ma, 0, shift, 1, value);
...
input bool   close_by_strategy_on = false;                   // Close positions by EMA:
...
class OrderCreator : public iActions
{
  ...
  public:
    ...
    void setPrice(int n) { _price = ::Price(_side, n); }
...
// clang-format off
    bool execute()    
    {
        ...
        if (_cmd == ORDER_TYPE_BUY_STOP) 
            if (!trade.BuyStop(_lots, _price, _symbol, _sl, _tp)) { Print(__FUNCTION__, " ", "Cannot Send Order, error: ", GetLastError()); return false; }
        if (_cmd == ORDER_TYPE_SELL_STOP)
            if (!trade.SellStop(_lots, _price, _symbol, _sl, _tp)) { Print(__FUNCTION__, " ", "Cannot Send Order, error: ", GetLastError()); return false; }
...
class ConditionCountPendingsBuys : public iConditions
{
  public:
    bool evaluate()
    {
...
ConditionCountPendingsBuys cCountPendingsBuys;
class ConditionCountPendingsSells : public iConditions
{
  public:
    bool evaluate()
    {
...
ConditionCountPendingsSells cCountPendingsSells;
...
class ActionDeletePendingsBuy : public iActions
{
  public:
    bool execute()
    {
...
ActionDeletePendingsBuy acDeletePendingsBuy;

class ActionDeletePendingsSell : public iActions
{
  public:
    bool execute()
    {
...
ActionDeletePendingsSell acDeletePendingsSell;
...
void BasicStrategy_Iniciate()
{
    ...
    LongStrategy.addCondition(&cCountPendingsBuys);
    LongStrategy.addAction(&acDeletePendingsSell);
    
    ShortStrategy.set(&conditionSell, &sell);
    ShortStrategy.addCondition(&cCountPendingsSells);
    ShortStrategy.addAction(&acDeletePendingsBuy);
...
void oninit_counters_filters()
{
    ...
    // Trader.filters.add(&cCountPendings);
