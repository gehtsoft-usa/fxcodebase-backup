// More information about this indicator can be found at: // https://fxcodebase.com/code/viewtopic.php?f=38&t=75133
...
CButton bt1, bt2;
#define BUTTON1_NAME "Close All"
#define BUTTON2_NAME "On/Off Labels"

int OnInitButtons()
{

    if (!Create_button(BUTTON1_NAME, 10, 100, 17, 100, bt1)) return (INIT_FAILED);
    //   if (!Create_button(BUTTON2_NAME, 10, 120, 17, 100, bt2)) return (INIT_FAILED);
...
void OnChartEventButtons(const int id, const long &lparam, const double &dparam, const string &sparam)
{
    if (id == CHARTEVENT_OBJECT_CLICK && sparam == BUTTON1_NAME) {
        ...
        // ActionBt1();
        //   Print("presionando bt 1");
        closeAll();
    }

    if (id == CHARTEVENT_OBJECT_CLICK && sparam == BUTTON2_NAME) {
        // TODO: event del button 2
        // ActionBt2();
        Print("presionando bt 2");
    }
}
#endif
...
{
    instant, //     Instant
    everycandle, // Every Candle
    normal, // Normal
};
input enum_Mode_Open mode_open = instant; // Mode Open trades:

enum slModeLevels {
    slFixPips,           // Fix Pips
    slbyMoney,           // Money
    slPipsFromOpenCandle // Pips from Candle
};
enum tpModeLevels {
    tpFixPips,            // Fix Pips
    tpbyMoney,            // Money
    tpPipsFromOpenCandle, // Pips from Candle
    tpByRatio             // By Ratio RR
};
...
string file_custom_indicator = "Indicador_TV_to_MT4.ex4";
int    bufferToBuy           = 0;
int    bufferToSell          = 1;
...
double Indi(int buffer, int candle)
{
    ...
    return iCustom(NULL, 0, file_custom_indicator, m, atrPeriods, h, buffer, candle);
}
// ------------------------------------------------------------------
#endif
...
input tpModeLevels modeTP       = tpFixPips;         // Mode Take Profit:
input int          userTPpips   = 20;                // Pips TP
input double       userTPmoney  = 15;                // Money TP
input double       uRatio       = 2;                 // Ratio RR
input string       T02          = "= Stop Loss =";   // ————————————
input bool         stopLossOn   = true;              // Stop Loss On:
input slModeLevels modeSL       = slFixPips;         // Mode Stop Loss:
...
input string       TtpOptions              = "== Equity Monitor =="; // ————————————
input bool         closeAllControlON       = false;                     // Control On:
input CloseAllMode closeBy                 = CloseByMoney;              // Mode:
...
input bool    AutotradingON     = false;                                // Turn Off Autotrading when close all?

void DeactiveAutoTrading()
{
    int main = GetAncestor(WindowHandle(Symbol(), Period()), 2);
    // si está habilitado lo desactiva:
...
class ByRatio : public iLevels
{
  ...
  public:
    ByRatio(string inpSymbol, string inpSide, double inpPriceSL, double inpRatio, string inpMode, Levels *sl)
    {
        _symbol  = inpSymbol;
        _side    = inpSide;
        _mode    = inpMode;
    ...
    void setSide(string inpSide) { _side = inpSide; }
    ...
    void setPips() { _pips = _sl.pips() * _ratio; }
...
class InstantModeBuy : public iConditions
{
  public:
    bool evaluate()
    {
        ...
        int shift_buy = 0;
        while (Indi(bufferToBuy, shift_buy) == EMPTY_VALUE)
        {
            shift_buy++;            
        }
        int shift_sell = 0;
        while (Indi(bufferToSell, shift_sell) == EMPTY_VALUE)
        {
            shift_sell++;            
        }
        
        if (shift_buy < shift_sell)
        {
            return true;
        }
...
InstantModeBuy *instantModeBuy;

class InstantModeSell : public iConditions
{
  public:
    bool evaluate()
    {
...
InstantModeSell *instantModeSell;
...
int OnInit()
{
    ...
    if(mode_open == instant)
    {
        CloseCandleMode = false;
        conditionsToBuy.AddCondition(instantModeBuy = new InstantModeBuy());
        conditionsToSell.AddCondition(instantModeSell = new InstantModeSell());
        #ifdef MAX_TRADES_AT_SAME_TIME
        conditionsToBuy.AddCondition(countTrades = new ConditionCountTrades(uMaxTrades));
        conditionsToSell.AddCondition(countTrades = new ConditionCountTrades(uMaxTrades));
        #endif
    }
    if(mode_open == everycandle)
    {
        CloseCandleMode = true;
        conditionsToBuy.AddCondition(instantModeBuy = new InstantModeBuy());
        conditionsToSell.AddCondition(instantModeSell = new InstantModeSell());        
        #ifdef MAX_TRADES_AT_SAME_TIME
        conditionsToBuy.AddCondition(countTrades = new ConditionCountTrades(1000));
        conditionsToSell.AddCondition(countTrades = new ConditionCountTrades(1000));
        #endif
    }
...
void OnTick()
{
    ...
    if (!uTradeReverse) {
        if (conditionsToBuy.EvaluateConditions()) {
            if (modeEntry == Market) {
                double sl =  SL("buy");
                actionSendOrder = new SendNewOrder("buy", Lots(), "", 0,sl, TP("buy"), magico);
            } else {
                double pr       = Price("buy", uEntryDistance, _Symbol);
                double sl =  SL("buy", pr);
                actionSendOrder = new SendNewOrder("buy", Lots(), "", pr, sl, TP("buy", pr), magico);
            }
        ...
        if (conditionsToSell.EvaluateConditions()) {
            if (modeEntry == Market) {
              double sl = SL("sell");
                actionSendOrder = new SendNewOrder("sell", Lots(), "", 0, sl , TP("sell"), magico);
            } else {
                double pr       = Price("sell", uEntryDistance, _Symbol);
                double sl = SL("sell", pr);
                actionSendOrder = new SendNewOrder("sell", Lots(), "", pr, sl, TP("sell", pr), magico);
            }
            if (actionSendOrder.doAction()) {
                mainOrders.GetMarketOrders();
...
double TP(string side, double price = 0)
{
    ...
    if (takeProfitOn) switch (modeTP) {
        ...
        case tpPipsFromOpenCandle:
            levelTP = new Levels(new ByPipsFromCandle(_Symbol, side, userSLpips, "TP", 0, 1));
            result  = levelTP.calculateLevel();
            break;

        case tpByRatio:
            levelTP = new Levels(new ByRatio(_Symbol, side, 0, uRatio, "TP", levelSL));
            result  = levelTP.calculateLevel();
            break;
        }
    return result;
...
void closeAll(string side = "")
{
    ...
    if(AutotradingON) { DeactiveAutoTrading(); }
}
