...
// http://fxcodebase.com/code/viewtopic.php?f=38&t=68557
...
input ENUM_TIMEFRAMES Timeframe_1 = PERIOD_M1; // Timeframe 1
input ENUM_TIMEFRAMES Timeframe_2 = PERIOD_M5; // Timeframe 2
...
input double Step            = 0.02; // PSAR Step
input double Max             = 0.2; // PSAR Max
...
public:
   ...
   bool IsPass(const int period)
   {
      ...
      double saValue = iIchimoku(_symbol, _timeframe, _tenkanSen, _kijunSen, _senkoiSpanB, MODE_SENKOUSPANA, index);
      double sbValue = iIchimoku(_symbol, _timeframe, _tenkanSen, _kijunSen, _senkoiSpanB, MODE_SENKOUSPANB, index);
...
public:
   ...
   {
      int index = iBarShift(_symbol, _timeframe, Time[period]);
      double rsiValue = iRSI(_symbol, _timeframe, _rsiPeriod, PRICE_CLOSE, index);
...
public:
   ...
   bool IsPass(const int period)
   {
      int index = iBarShift(_symbol, _timeframe, Time[period]);
      double close = iClose(_symbol, _timeframe, index);
      double sarValue = iSAR(_symbol, _timeframe, _step, _max, index);
...
ICondition* CreateLongCondition(string symbol, ENUM_TIMEFRAMES timeframe)
{
   ...
   condition.Add(new PriceAboveKumhoCondition(symbol, Timeframe_1, Tenkan_Sen_Period, Kijun_Sen_Period, Senkou_Span_B_Period));
   condition.Add(new PriceAboveKumhoCondition(symbol, Timeframe_2, Tenkan_Sen_Period, Kijun_Sen_Period, Senkou_Span_B_Period));
   condition.Add(new PriceAboveKumhoCondition(symbol, Timeframe_3, Tenkan_Sen_Period, Kijun_Sen_Period, Senkou_Span_B_Period));
   condition.Add(new RSIAboveLevelCondition(symbol, timeframe, RSI_Period, RSI_Level));
   condition.Add(new PSARBelowPriceCondition(symbol, timeframe, Step, Max));
   return condition;
}
...
ICondition* CreateShortCondition(string symbol, ENUM_TIMEFRAMES timeframe)
{
   ...
   AndCondition* condition = new AndCondition();
   condition.Add(new PriceBelowKumhoCondition(symbol, Timeframe_1, Tenkan_Sen_Period, Kijun_Sen_Period, Senkou_Span_B_Period));
   condition.Add(new PriceBelowKumhoCondition(symbol, Timeframe_2, Tenkan_Sen_Period, Kijun_Sen_Period, Senkou_Span_B_Period));
   condition.Add(new PriceBelowKumhoCondition(symbol, Timeframe_3, Tenkan_Sen_Period, Kijun_Sen_Period, Senkou_Span_B_Period));
   condition.Add(new RSIBelowLevelCondition(symbol, timeframe, RSI_Period, RSI_Level));
   condition.Add(new PSARAbovePriceCondition(symbol, timeframe, Step, Max));
   return condition;
}
