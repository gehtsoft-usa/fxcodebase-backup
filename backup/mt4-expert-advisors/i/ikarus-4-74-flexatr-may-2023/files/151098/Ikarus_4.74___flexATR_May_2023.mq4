//Available @ https://fxcodebase.com/code/viewtopic.php?f=38&t=73791
...
// https://www.forexfactory.com/thread/1070898-free-grid-ea-ikarus-from-zero-to?page=110
// ------------------------------------------------------------------------------------------------
//
...
#define versionNo "4.74"
#define versionBMI "Icarus " + versionNo
#define versionOld "based on Super Money Grid v1.41"

#property version versionNo
#property strict
#property copyright "Freeware. Forex Factory"
#property link      "https://www.forexfactory.com/thread/post/14016738#post14016738"
#property description "From zero to hero"
#property description " "
#property description "Time Frame = M15.  ( min balance = 500 )"
#property description " "
#property description "Caution = EA is not designed to cope with Major news impact  (Eg. 2021 Aug 06-08th. NPF)"
...
// EXTERN VARS
// ------------------------------------------------------------------------------------------------
extern int magic = 0;  // 0 : magic number is calculated in automatic

// ------------------------------------------------------------------------------------------------
int user_slippage = 2;

extern double grid_size       = 30;       // (20) Grid_Size (orders' distance)
extern int gs_progression     = 3;        // Grid_Step Mode (0=fixed. 1=sum. 2=martingale. 3=fibonacci)
extern double gs_multiplicator  =3;       // Martingale coefficient for grid step
extern double take_profit     = 25;       // Take_Profit
extern double profit_lock     = 1.00;     // Profit_Lock
extern double min_lots        = 0.01;     // Min_lots
extern double equity_warning  = 0.20;     // Equity_Warning (0.2=20%)
extern double account_risk    = 1.00;     // Account_Risk (1=1%)
extern bool useTradeCycleRisk = false;    // Allow Buy & Sell at the same time
extern double cycleEquityRisk = 0.10;     // Buy OR Sell Cycle Equity Risk Percentage (0.1=10%)
extern int progression        = 3;        // Lot size Mode (0=fixed. 1=sum. 2=martingale. 3=fibonacci)
extern double lot_multiplicator =1.0;     // Martingale lot size multiplier
...
extern bool both_cycle        = false;     // Initiate Both Cycle when indicators is used
extern bool hedging_active    = false;    // Hedging Order to balance is used
extern int grid_partially_close = 4;      // Grid partial close ( >4 trades close last & 1st trade in profit)
...
string Bollinger_Bands; //*****   Bollinger Bands   *****
bool Use_BB       = true;          // Bollinger Bands is used
bool BB_invert    = false;         // Invert Trigger
ENUM_TIMEFRAMES BB_tf = PERIOD_M15; // Time Frame
int BB_Period     = 20;            // Period
double BB_Dev     = 3.0;           // Deviation
int BB_Shift      = 0;             // Shift
BB_options BB_Mod = ASK_BID;       // Option

string Stochastic_Oscullator; //*****   Stochastic Oscullator   *****
bool Use_Stoch    = true; //Stochastic Oscullator is used
bool STO_invert   = false; //Invert Trigger
ENUM_TIMEFRAMES Stoch_tf = PERIOD_M15; //Time Frame
int Stoch_K       = 14; //Period of the %K line
int Stoch_D       = 5;  //Period of the %D line
int Stoch_Slowing = 3;  //Slowing value
ENUM_MA_METHOD Stoch_Method = MODE_SMA;  //Moving Average method
ENUM_STO_PRICE Stoch_Price = STO_LOWHIGH; //Price field parameter
int Stoch_Shift   = 0;  //Shift
double Stoch_Lower = 30; //Lower level
double Stoch_Upper = 70; //Upper level

string Relative_Strength_Index; //*****   Relative Strength Index   *****
bool Use_RSI      = true;   //RSI is used
bool RSI_invert   = false;  //Invert Trigger
ENUM_TIMEFRAMES RSI_tf = PERIOD_M15; //Time Frame
int RSI_Period    = 11;  //Period
double RSI_Lower  = 30;  //Lower level
double RSI_Upper  = 70;  //Upper level
int RSI_Shift     = 0;   //Shift

string Commodity_Channel_Index; //*****   Commodity Channel Index   *****
bool Use_CCI      = true;   //CCI is used
bool CCI_invert   = false;  //Invert Trigger
ENUM_TIMEFRAMES CCI_tf = PERIOD_M15; //Time Frame
int CCI_Period    = 25;  //Period
double CCI_Lower  = -100;  //Lower level
double CCI_Upper  = 100;  //Upper level
int CCI_Shift     = 0;   //Shift

sinput string ATR_for_Grid_Size; //*****   Average True Range   *****
extern bool Use_ATR           = true;      //Using ATR for Grid Size
extern ENUM_TIMEFRAMES ATR_tf = PERIOD_H1;  //Time Frame
extern int ATR_Period         = 9;          //Period
   ...
   if(show_forecast)
     {
      ...
      Write("panel_1_03", DoubleToString(CalculatePriceByTickDiff(relativeVolume, ter_tick_size * 10), 2) + " " + ter_currencySymbol + " / Pip", 5, 58, "Arial", 10, colFontWhite);
      Write("panel_1_04", "Balance: " + DoubleToString(AccountBalance(), 2) + " " + ter_currencySymbol, 5, 74, "Arial", 10, colFontWhite);
      Write("panel_1_05", "Equity: " + DoubleToString(AccountEquity(), 2) + " " + ter_currencySymbol, 5, 90, "Arial", 10, colFontWhite);
      Write("panel_1_06", "Free Margin: " + DoubleToString(AccountFreeMargin(), 2) + " " + ter_currencySymbol, 5, 106, "Arial", 10, colFontWhite);
      ...
      Write("panel_1_11", "Pips2Go B : " + DoubleToString(pips2go_Buys, 0) + " S " + DoubleToStr(pips2go_Sells, 0), 5, 192, "Arial", 10, colFontWhite);
      Write("panel_1_11", "Magic N.  : " + IntegerToString(magic, 0), 5, 192, "Arial", 12, colFontYellow);

      //---------------------if(showComment)

      Write("panel_1_14", "BUY ORDERS", 5, 300, "Arial", 10, colFontWhite);
      Write("panel_1_15", "Number of orders: " + (string)buys, 5, 316, "Arial", 10, colFontWhite);
      Write("panel_1_16", "Total lots: " + DoubleToStr(total_buy_lots, 2), 5, 332, "Arial", 10, colFontWhite);
      Write("panel_1_17", "Profit goal: " + ter_currencySymbol + DoubleToStr(aux_tp_buy, 2), 5, 348, "Arial", 10, colFontWhite);
      Write("panel_1_18", "Maximum profit reached: " + ter_currencySymbol + DoubleToStr(buy_max_profit, 2), 5, 364, "Arial", 10, colFontWhite);
      Write("panel_1_19", "Profit locked: " + ter_currencySymbol + DoubleToStr(buy_close_profit, 2), 5, 380, "Arial", 10, colFontWhite);
      Write("panel_1_20", "SellHedging: " + ter_currencySymbol + DoubleToStr(total_hedge_sell_profit, 2), 5, 396, "Arial", 10, colFontWhite);
      Write("panel_1_21", "SellHedging_Orders: " + (string)hedge_sells, 5, 412, "Arial", 10, colFontWhite);
      Write("panel_1_22", "SellHedging_Lots: " + DoubleToStr(total_hedge_sell_lots, 2), 5, 428, "Arial", 10, colFontWhite);

      Write("panel_1_23", "SELL ORDERS", 5, 460, "Arial", 10, colFontWhite);
      Write("panel_1_24", "Number of orders: " + (string)sells, 5, 476, "Arial", 10, colFontWhite);
      Write("panel_1_25", "Total lots: " + DoubleToStr(total_sell_lots, 2), 5, 492, "Arial", 10, colFontWhite);
      Write("panel_1_26", "Profit goal: " + ter_currencySymbol + DoubleToStr(aux_tp_sell, 2), 5, 508, "Arial", 10, colFontWhite);
      Write("panel_1_27", "Maximum profit reached: " + ter_currencySymbol + DoubleToStr(sell_max_profit, 2), 5, 524, "Arial", 10, colFontWhite);
      Write("panel_1_28", "Profit locked: " + ter_currencySymbol + DoubleToStr(sell_close_profit, 2), 5, 540, "Arial", 10, colFontWhite);
      Write("panel_1_29", "BuyHedging: " + ter_currencySymbol + DoubleToStr(total_hedge_buy_profit, 2), 5, 556, "Arial", 10, colFontWhite);
      Write("panel_1_30", "BuyHedging_Orders: " + (string)hedge_buys, 5, 572, "Arial", 10, colFontWhite);
      Write("panel_1_31", "BuyHedging_Lots: " + DoubleToStr(total_hedge_buy_lots, 2), 5, 588, "Arial", 10, colFontWhite);




      Write("panel_1_32", "Current drawdown: " + DoubleToString((max_equity - AccountEquity()), 2) + " " + ter_currencySymbol + " (" + DoubleToString((max_equity - AccountEquity()) / max_equity * 100, 2) + " %)", 5, 620, "Arial", 10, colFontWhite);
      Write("panel_1_33", "Max. drawdown: " + DoubleToString(max_float, 2) + " " + ter_currencySymbol, 5, 636, "Arial", 10, colFontWhite);

      Write("panel_1_34", "SETTINGS: ", 5, 668, "Arial", 10, colFontWhite);
      Write("panel_1_35", "Grid size: " + (string)grid_size, 5, 684, "Arial", 10, colFontWhite);
      Write("panel_1_36", info_gs_progression, 5, 700, "Arial", 10, colFontWhite);
      Write("panel_1_37", "Take profit: " + (string)take_profit, 5, 716, "Arial", 10, colFontWhite);
      Write("panel_1_38", "Profit locked: " + DoubleToStr(100 * profit_lock, 2) + "%", 5, 732, "Arial", 10, colFontWhite);
      Write("panel_1_39", "Minimum lots: " + DoubleToStr(min_lots, 2), 5, 748, "Arial", 10, colFontWhite);
      Write("panel_1_40", "Equity warning: " + DoubleToStr(100 * equity_warning, 2) + "%", 5, 764, "Arial", 10, colFontWhite);
      Write("panel_1_41", "Account risk: " + DoubleToStr(100 * account_risk, 2) + "%", 5, 780, "Arial", 10, colFontWhite);
      Write("panel_1_42", cycleRisttext, 5, 796, "Arial", 10, colFontWhite);
      Write("panel_1_43", "Progression: " + info_money_management[progression] + (string)lot_multiplicator, 5, 812, "Arial", 10, colFontWhite);
      Write("panel_1_44", "Max Positions: " + (string)max_positions, 5, 828, "Arial", 10, colFontWhite);
      Write("panel_1_45", "AveregeFrom: " + (string)grid_partially_close, 5, 844, "Arial", 10, colFontWhite);
