...
// http://fxcodebase.com/code/viewtopic.php?f=38&t=68516
...
extern string ICH_Section = ""; // == Ichimoku ==
input int tenkan_sen = 9; // Tenkan sen
input int kijun_sen = 26; // Kijun sen
input int senkoi_span_b = 52; // Senkoi span b

extern string Stoch_Section = ""; // == Stochastic ==
input int k_period = 8; // K period
input int d_period = 3; // D period
input int slowing_period = 3; // Slowing period

extern string Osma_Section = ""; // == OsMA ==
input int fast_ema = 12; // Fast EMA
input int slow_ema = 26; // Slow EMA
input int signal = 9; // Signal
...
class OSMAStream : public AStream
{
   int _fastEMA;
   int _slowEMA;
   int _signal;
public:
   OSMAStream(const string symbol, const ENUM_TIMEFRAMES timeframe, int fastEMA, int slowEMA, int signalParam)
      :AStream(symbol, timeframe)
   {
      _fastEMA = fastEMA;
      _slowEMA = slowEMA;
   ...
   bool GetValue(const int period, double &val)
   {
      val = iCustom(_symbol, _timeframe, "OsMA", _fastEMA, _slowEMA, _signal, 4, period);
      return true;
   }
};
...
class PriceUnderIchimokuCloudCondition : public ABaseCondition
{
public:
   PriceUnderIchimokuCloudCondition(const string symbol, ENUM_TIMEFRAMES timeframe)
   ...
   {
      double spanA = iIchimoku(_symbol, _timeframe, tenkan_sen, kijun_sen, senkoi_span_b, MODE_SENKOUSPANA, period);
      double spanB = iIchimoku(_symbol, _timeframe, tenkan_sen, kijun_sen, senkoi_span_b, MODE_SENKOUSPANB, period);
      double close = iClose(_symbol, _timeframe, period);
      
      return close < MathMin(spanA, spanB);
   }
};

class PriceOverIchimokuCloudCondition : public ABaseCondition
{
public:
   PriceOverIchimokuCloudCondition(const string symbol, ENUM_TIMEFRAMES timeframe)
      :ABaseCondition(symbol, timeframe)
   {
   ...
   int Execute(string &errorMessage)
   {
      ...
      if (order == -1)
      {
         ...
         switch (error)
         {
            ...
            case ERR_INVALID_STOPS:
               {
                  Print((_orderSide == BuySide ? "B " : "S ") + DoubleToStr(rate) + " " + DoubleToStr(_stop));
                  double point = SymbolInfoDouble(_instrument, SYMBOL_POINT);
...
ICondition* CreateLongCondition(string symbol, ENUM_TIMEFRAMES timeframe)
{
   ...
   and.Add(or);
   and.Add(new PriceUnderIchimokuCloudCondition(symbol, timeframe));

   PriceStream* highStream = new PriceStream(symbol, timeframe, PriceHigh);
   PriceStream* lowStream = new PriceStream(symbol, timeframe, PriceLow);

   StochasticStream* stochasticStream = new StochasticStream(symbol, timeframe, k_period, d_period, slowing_period, 0);
   or.Add(new DivergencePattern(stochasticStream, highStream, lowStream, DivergenceHiddenBulish));
   stochasticStream.Release();

   OSMAStream* osmaStream = new OSMAStream(symbol, timeframe, fast_ema, slow_ema, signal);
   or.Add(new DivergencePattern(osmaStream, highStream, lowStream, DivergenceHiddenBulish));
   osmaStream.Release();

   highStream.Release();
   lowStream.Release();
   return (ICondition *)and;
}
...
ICondition* CreateShortCondition(string symbol, ENUM_TIMEFRAMES timeframe)
{
   ...
   OrCondition* or = new OrCondition();
   and.Add(or);
   and.Add(new PriceOverIchimokuCloudCondition(symbol, timeframe));

   PriceStream* highStream = new PriceStream(symbol, timeframe, PriceHigh);
   PriceStream* lowStream = new PriceStream(symbol, timeframe, PriceLow);

   StochasticStream* stochasticStream = new StochasticStream(symbol, timeframe, k_period, d_period, slowing_period, 0);
   or.Add(new DivergencePattern(stochasticStream, highStream, lowStream, DivergenceHiddenBearish));
   stochasticStream.Release();

   OSMAStream* osmaStream = new OSMAStream(symbol, timeframe, fast_ema, slow_ema, signal);
   or.Add(new DivergencePattern(osmaStream, highStream, lowStream, DivergenceHiddenBearish));
   osmaStream.Release();
