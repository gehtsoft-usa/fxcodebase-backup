...
int init()
{
   visibility.Init("show_hide_ich", "ICH", "Show/Hide", button_x, button_y);
   IndicatorBuffers(13);
   ...
   if (TimeFrame!=_Period)
   {
      ...
      for (i=limit;i>=0 && !_StopFlag; i--)
      {
         if (visibility.IsVisible())
         {
            ...
            Tenkan_Buffer[i]  = _mtfCall(0,y);
            Kijun_Buffer[i]   = _mtfCall(1,y);
            SpanA_Buffer[i]   = _mtfCall(2,y);
            SpanB_Buffer[i]   = _mtfCall(3,y);
            Chinkou_Buffer[i] = _mtfCall(4,y);
            SpanA2_Buffer[i]  = _mtfCall(5,y);
            SpanB2_Buffer[i]  = _mtfCall(6,y);
   ...
   for(i=limit; i>=0; i--)
   {
      if (visibility.IsVisible())
      {
         ...
         if (i<Bars-1)
         {
            ...
            trend4[i] = trend4[i+1];
            trend5[i] = trend5[i+1];
            if (Close[i] > MathMax(SpanA_Buffer[i+Kijun],SpanB_Buffer[i+Kijun])) trend1[i] = 1;
            if (Close[i] < MathMin(SpanA_Buffer[i+Kijun],SpanB_Buffer[i+Kijun])) trend1[i] =-1;
            if (Tenkan_Buffer[i]>Kijun_Buffer[i])                                trend2[i] = 1;
            if (Tenkan_Buffer[i]<Kijun_Buffer[i])                                trend2[i] =-1;
            if (Close[i]>Kijun_Buffer[i])                                        trend3[i] = 1;
            if (Close[i]<Kijun_Buffer[i])                                        trend3[i] =-1;
            if (Chinkou_Buffer[i]<Tenkan_Buffer[i])                              trend4[i] = 1;
            if (Chinkou_Buffer[i]>Tenkan_Buffer[i])                              trend4[i] =-1;
            if (SpanA2_Buffer[i]>SpanB2_Buffer[i])                               trend5[i] = 1;
            if (SpanA2_Buffer[i]<SpanB2_Buffer[i])                               trend5[i] =-1;
         }
      }
      else
      {
         ...
         trend3[i] = EMPTY_VALUE;
         trend4[i] = EMPTY_VALUE;
         trend5[i] = EMPTY_VALUE;
      }
   }
   manageAlerts();
   return(0);
