                ...
                const int&      spread[])
{
    ...
    for (i = start; i >= 0; i--) {
      ...
      if (lookfor == 1 && LineUp[i+1]!=EMPTY_VALUE && MACD_Main(i+1) > MACD_Signal(i+1) && MACD_Main(i+2) > MACD_Signal(i+2)) {
            lookfor    = 0;
            ArrowUp[i+1] = Low[i+1];
            notify(0);
        }
        ...
        if (lookfor == 0 && LineDn[i+1]!=EMPTY_VALUE && MACD_Main(i+1) < MACD_Signal(i+1) && MACD_Main(i+2) > MACD_Signal(i+2)) {
