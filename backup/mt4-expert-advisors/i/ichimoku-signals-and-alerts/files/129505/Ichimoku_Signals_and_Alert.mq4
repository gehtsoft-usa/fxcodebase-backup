...
#property description "- Tenkan-sen with Price"
...
extern bool Show_Tenkan_Sen_Price        = true; // Show alerts for Tenkan-sen/Price
...
double TENKANSEN_Price_up[], TENKANSEN_Price_dn[];
...
int init(){
   ...
   SetIndexBuffer(12,TENKANSEN_Price_up);
   SetIndexStyle (12,DRAW_ARROW);
   SetIndexArrow (12,233);
   SetIndexBuffer(13,TENKANSEN_Price_dn);
   SetIndexStyle (13,DRAW_ARROW);
   SetIndexArrow (13,234);
   
   SetIndexBuffer(17,TENKANSEN);
   SetIndexBuffer(18,KIJUNSEN);
...
int start()
  {
   ...
   for(i=limit ; i>=0; i--){
      ...
      if (Show_Tenkan_Sen_Price){
         if (TENKANSEN[i] > Close[i] && TENKANSEN[i+1] < Close[i + 1])
         {
            TENKANSEN_Price_up[i] = TENKANSEN[i];
            if (i==0) Alerts+= " TENKANSEN/Price (up) ";
         }
         if (TENKANSEN[i] < Close[i] && TENKANSEN[i+1] > Close[i+1]){
            TENKANSEN_Price_dn[i] = TENKANSEN[i];
            if (i==0) Alerts+= " TENKANSEN/Price (down) ";
         }
      }
