...
// ——————————————————————————————————————————————————————————————————
enum ModeLevels {
    MidleLineStatic, // Midle Line Static
    HighLineStatic, // High/Low Line Static
    FixPips,            // Fix Pips
    byMoney,            // Money
...
string file_custom_indicator = "C_.ex4";
int bufferToBuy = 5;
int bufferToSell = 6;
...
input string Tindi = "== Indicator C_ Setup ==";  // ————————————
...
extern color           ColorNe = clrCrimson;      // Color for neutral and "zero" line
extern bool            Interpolate = true;            // Interpolate in multi time frame mode?

// NOTE: mladen setup
string file_custom_indicator2 = "mladen_indicator.ex4";
int CentralBand = 0;
int UpBand = 1;
int DnBand = 2;
int mladen_bufferToBuy = 3;
int mladen_bufferToSell = 4;

input string              Tindi2 = "== Indicator mladen Setup ==";  // ————————————
extern string             indi2_TimeFrame = "current time frame";
extern int                HalfLength = 20;
extern ENUM_APPLIED_PRICE indi2_Price = PRICE_WEIGHTED; //  Price:
extern double             BandsDeviations = 2.3;        
extern bool               indi2_Interpolate = false; // Interpolate
...
input bool closeInMidleLine = true; // Close in midle line Dinamic:
...
class BUYcondition1 : public iConditions
{
    ...
    bool evaluate()
    {
        ...
        return C_(bufferToBuy, 1) != EMPTY_VALUE;
        return false;
    }
};
...
class BUYcondition2 : public iConditions
{
    ...
    bool evaluate()
    {
        ...
        double m = mladen(CentralBand, 2);
        //   double m2 = mladen(CentralBand, 2);
        Print(__FUNCTION__, " BUY       mladen: ", m);
        Print(__FUNCTION__, " BUY       close2: ", iClose(NULL, 0, 2));
        //   Print(__FUNCTION__,"       mladen: ",      m2);

        //   return mladen(CentralBand, 1) > mladen(CentralBand, 2);
        return mladen(CentralBand, 2) > iClose(NULL, 0, 2);
        //   return false;
    }
};
...
class SELLcondition1 : public iConditions
{
    ...
    bool evaluate()
    {
        ...
        // return iClose(NULL, 0, 1) < ma.index(1);
        return C_(bufferToSell, 1) != EMPTY_VALUE;
        // return false;
    }
};
...
class SELLcondition2 : public iConditions
{
    ...
    bool evaluate()
    {
      ...
      //   return mladen(CentralBand, 1) < mladen(CentralBand, 2);
        ...
        double m = mladen(CentralBand, 2);
        Print(__FUNCTION__, "  SELL     mladen: ", m);
        Print(__FUNCTION__, "  SELL     close2: ", iClose(NULL, 0, 2));
        //   double m2 = mladen(CentralBand, 2);
        //   Print(__FUNCTION__,"       mladen: ",      m2);

      //   return mladen(CentralBand, 1) < Bid;
        return mladen(CentralBand, 2) < iClose(NULL, 0, 2);
        //   return false;
    }
};
...
class ConditionToCloseBuy : public iConditions
{
    ...
    bool evaluate()
    {
        ...
        if(closeInMidleLine)
        {
            if(Bid < mladen(CentralBand, 0) && iOpen(NULL, 0, 0) > mladen(CentralBand, 0))
                return true;
        }
...
class ConditionToCloseSell : public iConditions
{
    ...
    bool evaluate()
    {
        ...
        if(closeInMidleLine)
        {
            if(Ask > mladen(CentralBand, 0) && iOpen(NULL, 0, 0) < mladen(CentralBand, 0))
                return true;
        }
...
double SL(string side, double price = 0)
{
    ...
    if(stopLossOn)
        switch(modeSL)
        {
            ...
            case MidleLineStatic:
                {
                    if(side == "buy")
                    {
                        double band = NormalizeDouble(mladen(CentralBand, 2), _Digits);
                        result = band < Ask ? band : Ask - 30 * _Point;
                    }
                    if(side == "sell")
                    {
                        double band = NormalizeDouble(mladen(CentralBand, 2), _Digits);
                        result = band > Bid ? band : Bid + 30 * _Point;
                    }
                }
                // break;
           case HighLineStatic:
                {
                    if(side == "buy")  result = NormalizeDouble(mladen(DnBand, 2), _Digits);
                    if(side == "sell") result = NormalizeDouble(mladen(UpBand, 2), _Digits);
                    // break;
                }
        }
    return result;
        ...
        switch(modeTP)
        {
            ...
            case MidleLineStatic:
                ...
                if(side == "buy")
                    {
                        double band = NormalizeDouble(mladen(CentralBand, 2), _Digits);
                        Print(__FUNCTION__," Centralband: ",band);
                        result = band > Ask ? band : Ask + 30 * _Point;
                    }
                if(side == "sell")
                {
            ...
            case HighLineStatic:
                {
                    if(side == "buy")  
                    {
                        double upband = NormalizeDouble(mladen(UpBand, 2), _Digits);
                        Print(__FUNCTION__, " upband ", upband);
                        result = upband;

                    }
                    if(side == "sell") 
                    {
                        double dnband = NormalizeDouble(mladen(DnBand, 2), _Digits);
                        Print(__FUNCTION__, " dnband ", dnband);
                        result = dnband;
                    }
                    // break;
                }
...
double C_(int buffer, int candle)
{
...
double mladen(int buffer, int candle)
{
    // TODO: copiar la lista de INPUTS para custom indicator:    
