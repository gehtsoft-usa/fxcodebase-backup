...
input double RSI_os_Level = 20; // RSI Oversold Level
input double RSI_ob_Level = 80; // RSI Overbought Level
...
ICondition* CreateUpCondition(string symbol, ENUM_TIMEFRAMES timeframe)
{
   ...
   condition.Add(new PriceAboveKumhoCondition(symbol, timeframe, Tenkan_Sen_Period, Kijun_Sen_Period, Senkou_Span_B_Period), false);
   condition.Add(new RSIBelowLevelCondition(symbol, timeframe, RSI_Period, RSI_os_Level), false);
   return condition;
}
...
{
   AndCondition* condition = new AndCondition();
   condition.Add(new PriceBelowKumhoCondition(symbol, timeframe, Tenkan_Sen_Period, Kijun_Sen_Period, Senkou_Span_B_Period), false);
   condition.Add(new RSIAboveLevelCondition(symbol, timeframe, RSI_Period, RSI_ob_Level), false);
   return condition;
}
...
int init()
{
   ...
   IndicatorName = GenerateIndicatorName("Ichimoku RSI Scanner");
