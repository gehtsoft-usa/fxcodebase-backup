...
class BUYcondition1 : public iConditions
{
 ...
 public:
  bool evaluate()
  {
    ...
    double indiBuy = Indi(bufferToBuy, 1);
    double indiSell = Indi(bufferToSell, 1);
    
    if(indiSell > 0 && indiSell != EMPTY_VALUE){ 
      signal = false;
      return false;
    }
    
    if(indiBuy > 0 && indiBuy != EMPTY_VALUE) signal = true;
...
class SELLcondition1 : public iConditions
{
 ...
 public:
  bool evaluate()
  {
    ...
    if(indiBuy > 0 && indiBuy != EMPTY_VALUE){ 
      signal = false;
      return false;
    }

    if(indiSell > 0 && indiSell != EMPTY_VALUE) signal = true;
