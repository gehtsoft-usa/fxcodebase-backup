...
//https://fxcodebase.com/code/viewtopic.php?f=38&t=72481
...
for(int i=0;i < ArraySize(ichimokus) ; i++)
{
	...
	for (int j = 0; j < ArraySize(TF); j++) 
	{
		...
		int trend = -1;
		// Up
		if(Bid > tenkan && tenkan > kinjun && kinjun > senkouA) { trend = 0; }
		// Down
		if(Bid < tenkan && tenkan < kinjun && kinjun < senkouA) { trend = 1; }
		// pullback
