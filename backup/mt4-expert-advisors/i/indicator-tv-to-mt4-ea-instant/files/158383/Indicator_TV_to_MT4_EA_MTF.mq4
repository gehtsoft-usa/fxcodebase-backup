...
input ENUM_TIMEFRAMES open_signal_tf = PERIOD_CURRENT; // Open Signal TimeFrame:
input ENUM_TIMEFRAMES close_opposite_tf = PERIOD_CURRENT; // Close Opposite TimeFrame:
 ...
 public:
  bool evaluate()
  {
    // TODO: condition Buy 1
      return Indi(bufferToBuy, 1, open_signal_tf) != EMPTY_VALUE;
  }
};
...
class SELLcondition1 : public iConditions
{
 public:
  bool evaluate()
  {
    ...
    return Indi(bufferToSell, 1, open_signal_tf) != EMPTY_VALUE;
  }
};
 ...
 public:
  bool evaluate()
  {
    if (closeAllInOpositeSignal) 
      if(Indi(bufferToSell, 1, close_opposite_tf) != EMPTY_VALUE) return true;
...
class ConditionToCloseSell : public iConditions
{
 public:
  ...
  {
    if (closeAllInOpositeSignal)
    if(Indi(bufferToBuy, 1, close_opposite_tf) != EMPTY_VALUE) return true;
