...
//http://fxcodebase.com/code/viewtopic.php?f=38&t=70808
...
public:
   ...
   {
      int index = iBars(_symbol, _timeframe) - 1 - period;
      if (index < 1)
      {
         return false;
      }
      return ExtTenkanBuffer[index] <= iClose(_symbol, _timeframe, period) && ExtTenkanBuffer[index - 1] > iClose(_symbol, _timeframe, period + 1);
   }
};
...
int OnInit(void)
{
   IndicatorObjPrefix = GenerateIndicatorPrefix("ika");
   IndicatorSetString(INDICATOR_SHORTNAME, "Ichimoku Kijun Alert");
   ...
   lowStream.Release();
   highStream.Release();
   SetIndexBuffer(id + 0,ExtTenkanBuffer,INDICATOR_DATA);
   SetIndexBuffer(id + 1,ExtKijunBuffer,INDICATOR_DATA);
   SetIndexBuffer(id + 2,ExtSpanABuffer,INDICATOR_DATA);
   SetIndexBuffer(id + 3,ExtSpanBBuffer,INDICATOR_DATA);
   PlotIndexSetInteger(id + 0,PLOT_DRAW_BEGIN,InpTenkan);
   PlotIndexSetInteger(id + 1,PLOT_DRAW_BEGIN,InpKijun);
   PlotIndexSetInteger(id + 2,PLOT_DRAW_BEGIN,InpSenkou-1);
   PlotIndexSetInteger(id + 2,PLOT_SHIFT,InpKijun);
   PlotIndexSetInteger(id + 3,PLOT_SHIFT,-InpKijun);
   PlotIndexSetString(id + 0,PLOT_LABEL,"Tenkan-sen("+string(InpTenkan)+")");
   PlotIndexSetString(id + 1,PLOT_LABEL,"Kijun-sen("+string(InpKijun)+")");
   PlotIndexSetString(id + 2,PLOT_LABEL,"Senkou Span A;Senkou Span B("+string(InpSenkou)+")");
   SetIndexBuffer(id + 4,ExtChikouBuffer,INDICATOR_DATA);
...
)
{
   ...
   {
      ArrayInitialize(ExtTenkanBuffer, EMPTY_VALUE);
      ArrayInitialize(ExtKijunBuffer, EMPTY_VALUE);
      ArrayInitialize(ExtSpanABuffer, EMPTY_VALUE);
      ArrayInitialize(ExtSpanBBuffer, EMPTY_VALUE);
      ArrayInitialize(ExtChikouBuffer, EMPTY_VALUE);
      for (int i = 0; i < ArraySize(alerts); ++i)
      {
         alerts[i].Init();
      }
   ...
   for (int pos = MathMax(first, prev_calculated - 1); pos < rates_total; ++pos)
   {
      ...
      ExtChikouBuffer[pos] = close[pos];
      //--- tenkan sen
      double price_max=Highest(high,InpTenkan,pos);
      double price_min=Lowest(low,InpTenkan,pos);
      ExtTenkanBuffer[pos]=(price_max+price_min)/2.0;
      //--- kijun sen
      price_max=Highest(high,InpKijun,pos);
      price_min=Lowest(low,InpKijun,pos);
      ExtKijunBuffer[pos]=(price_max+price_min)/2.0;
      //--- senkou span a
      ExtSpanABuffer[pos]=(ExtTenkanBuffer[pos]+ExtKijunBuffer[pos])/2.0;
      //--- senkou span b
      price_max=Highest(high,InpSenkou,pos);
      price_min=Lowest(low,InpSenkou,pos);
      ExtSpanBBuffer[pos]=(price_max+price_min)/2.0;
