...
string             SymbolsList;                                          // Symbols (separate by comma ","):
...
string fx_handling_ichimoku(string symbol_mapping, int timeframe)
{
   ...
   if (fx_l1(CURRENT_SENKOUSPANA, CURRENT_SENKOUSPANB, symbol_mapping) == true 
		//  && fx_l2(TENKANSEN, KIJUNSEN, CURRENT_SENKOUSPANA, CURRENT_SENKOUSPANB, symbol_mapping) 
		&& fx_l3(TENKANSEN, KIJUNSEN, symbol_mapping)) 
		// && fx_l4(CHIKOUSPAN, CURRENT_SENKOUSPANA, CURRENT_SENKOUSPANB, symbol_mapping, timeframe))
   {
      return "BUY";
   }

   if (fx_l1_sell(CURRENT_SENKOUSPANA, CURRENT_SENKOUSPANB, symbol_mapping) == true 
	//  && fx_l2_sell(TENKANSEN, KIJUNSEN, CURRENT_SENKOUSPANA, CURRENT_SENKOUSPANB, symbol_mapping) 
		&& fx_l3_sell(TENKANSEN, KIJUNSEN, symbol_mapping))
	// && fx_l4_sell(CHIKOUSPAN, CURRENT_SENKOUSPANA, CURRENT_SENKOUSPANB, symbol_mapping, timeframe))
   {
      return "SELL";
   }
...
OrdersList mainOrders(filterMagicsOn, (string)magico, filterSymbolsOn, SymbolsList);
...
class BUYcondition1 : public iConditions
{
  public:
   bool evaluate()
   {
      ...
      {
         if (signals[i].symbol == global_symbol)
         {
...
class BUYcondition2 : public iConditions
{
  public:
   bool evaluate()
   {
      ...
      rsi = new RSI(global_symbol, 0);
			if(rsi.index(1) >= rsiLevelUp)
			{
         delete rsi;
         return true;				
			}
...
class ConditionCountBuys : public iConditions
{
  ...
  public:
   ...
   bool evaluate()
   {
      ...
      for (int i = OrdersTotal() - 1; i >= 0; i--)
      {
         if (OrderSelect(i, SELECT_BY_POS) && OrderSymbol() == global_symbol && OrderMagicNumber() == magico)
         {
            if (OrderType() == OP_BUY)
            {
               count += 1;
...
void OnTick()
{
   ...
   for (int i = 0; i < ArraySize(signals); i++)
   {
      global_symbol = signals[i].symbol;
      ...
      {
         actionSendOrder = new SendNewOrder("buy", Lots(), global_symbol, 0, SL("buy"), TP("buy"), magico);
      ...
      {
         actionSendOrder = new SendNewOrder("sell", Lots(), global_symbol, 0, SL("sell"), TP("sell"), magico);
         if (actionSendOrder.doAction())
         {
            mainOrders.GetMarketOrders();
...
{
   double ask = SymbolInfoDouble(global_symbol, SYMBOL_ASK);
   double bid = SymbolInfoDouble(global_symbol, SYMBOL_BID);
...
double SL(string side)
{
   ...
   if (stopLossOn)
      ...
      {
         case FixPips:
            levelSL = new Levels(new ByFixPips(global_symbol, side, userSLpips, "SL"));
            result  = levelSL.calculateLevel();
            break;

         case byMoney:
            levelSL = new Levels(new ByMoney(global_symbol, side, userLots, userSLmoney, "SL"));
            result  = levelSL.calculateLevel();
            break;
         case PipsFromOpenCandle:
            levelSL = new Levels(new ByPipsFromCandle(global_symbol, side, userSLpips, "SL", 0, 1));
            result  = levelSL.calculateLevel();
      }
   return result;
}
double TP(string side)
{
   ...
   if (takeProfitOn)
      ...
      {
         case FixPips:
            levelTP = new Levels(new ByFixPips(global_symbol, side, userTPpips, "TP"));
            result  = levelTP.calculateLevel();
            break;

         case byMoney:
            levelTP = new Levels(new ByMoney(global_symbol, side, userLots, userTPmoney, "TP"));
            result  = levelTP.calculateLevel();
            break;
         case PipsFromOpenCandle:
            levelSL = new Levels(new ByPipsFromCandle(global_symbol, side, userSLpips, "TP", 0, 1));
            result  = levelSL.calculateLevel();
      }
   return result;
}
double Lots()
{
   lotProvider = new LotCalculator(global_symbol);
   double lots = -1;
   switch (modeCalcLots)
   {
      case Money:
...
{
   string text = "";
   if (type == 0) text += global_symbol + " " + GetTimeFrame(_Period) + " BUY ";
   else text += global_symbol + " " + GetTimeFrame(_Period) + " SELL ";

   text += " ";

   if (!notifications) return;
   
	 if (desktop_notifications){ Print("sending Alert");  Alert(text);}
   if (push_notifications)   { Print("sending Push");   SendNotification(text);}
   if (email_notifications)  { Print("sending e-Mail"); SendMail("MetaTrader Notification", text);}
}
...
void OnInit_Signals()
{
   ArrayResize(signals, ArraySize(currency_pair_mapping));
   for (int i = 0; i < ArraySize(signals); i++)
   {
      signals[i].symbol = currency_pair_mapping[i];
      signals[i].signal = "N";
   }
}
void getSignals()
{
   for (int j = 0; j < ArraySize(currency_pair_mapping); j++)
   {
      ...
      for (int i = 0; i < ArraySize(time_array); i++)
      {
         if (i == 0)
         {
            signal = fx_mapping_data(time_array[i], currency_pair_mapping[j]);
         }
         
				 // avanza un tf y lo compara con la signal inicial
         if (i > 0 && i < ArraySize(time_array) - 1)
         {
            if (fx_mapping_data(time_array[i], currency_pair_mapping[j]) == signal)
            {
               continue;
            } else
            {
               break;
				 ...
				 // si llegó al último i y sigue siendo igual, seteo la signal
         if (i == ArraySize(time_array) - 1 && fx_mapping_data(time_array[i], currency_pair_mapping[j]) == signal)
         {
            signals[j].signal = signal;
         }
      }
   }
}
...
void PrintSignals()
{
   for (int i = 0; i < ArraySize(signals); i++)
   {
      Print(signals[i].symbol);
      Print(signals[i].signal);
   }
}
