...
//http://fxcodebase.com/code/viewtopic.php?f=38&t=70271
//+------------------------------------------------------------------+
...
input int sma_period = 10; // RSI/average period
input int exit_sma_period = 10; // Exit RSI/average period
input int sma_shift = 0; // RSI/average shift
input ENUM_MA_METHOD sma_method = MODE_SMA; // RSI/average method
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      ...
      double maRsi = iCustom(_symbol, _timeframe, "MA_on_RSI", rsi_period, sma_period, sma_shift, sma_method, 0, period);
      return rsiValue > maRsi;
   }
};
...
public:
   ...
   {
      double rsiValue = iRSI(_symbol, _timeframe, rsi_period, PRICE_CLOSE, period);
      double maRsi = iCustom(_symbol, _timeframe, "MA_on_RSI", rsi_period, exit_sma_period, sma_shift, sma_method, 0, period);
      return rsiValue < maRsi;
   }
};
...
int OnInit()
{
   double temp = iCustom(NULL, 0, "MA_on_RSI", 0, 0);
   if (GetLastError() == ERR_INDICATOR_CANNOT_LOAD)
   {
      Alert("Please, install the 'MA_on_RSI' indicator");
      return INIT_FAILED;
   }
   #ifdef SHOW_ACCOUNT_STAT
      stats = NULL;
