...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=72464&p=155570#p155570
...
input double MaxLots                 = 5;                         // Maximum Lots:
double       userMoney               = 10;                        // Setup Lots by "Money":
double       userBalancePer          = 0.1;                       // Setup Lots by "Account Percent":
int    nTradesForIncrementLot  = 1;                         // Consecutive Trades For Increment Lot:
double uLotMultiplier          = 1;                         // Lot Multiplier:
...
input double takeProfitReduction     = 25;                        // Take profit Reduction%:
input int    nTradesToStartReduction = 2;                         // Trades to Start Reduction of TP:
...
class Reduction
{
  ...
  public:
    ...
    double value()
    {
        ...
        if (openTrades() >= _nextCountToReduction.current()) {
  ...
  public:
    ...
    double value()
    {
        if (openTrades(_side) >= _nextCount.current()) {
            ...
            _count.sum();                                     // suma 1 a count. Está contando la cantidad de incrementos que se hicieron, es para calcular el lotaje
            _nextCount.sum(_iniCount);                        // suma _iniCount a _nextCount. _iniCount es la cantidad de trades que tienen que pasar hasta el próximo incremento
            _lastValue = MathPow(_percent, _count.current()); // calcula el nuevo lotaje
            return _lastValue;
...
ConcurrentConditions conditionsToOpenBoth;
...
// ------------------------------------------------------------------
class ConditionOpenBoth : public iConditions
{
    // TODO: condition Both
  public:
    bool evaluate()
    {
        if (countBuys.evaluate() == true && countSells.evaluate() == true) {
            return true;
        }
        return false;
    }
};
ConditionOpenBoth* conditionOpenBoth;
...
class BUYcondition1 : public iConditions
{
  public:
    bool evaluate()
    {
        ...
        // if(countBuys.evaluate() || countSells.evaluate())
        return true;
...
incrementorSell = new IncrementerLots(nTradesForIncrementLot, uLotMultiplier, "sell");
    ...
    conditionsToOpenBoth.AddCondition(conditionOpenBoth = new ConditionOpenBoth());
    ...
    countBuys = new ConditionCountBuys(1, magico, ORDER_TYPE_BUY);
    // conditionsToBuy.AddCondition(countBuys);
    ...
    countSells = new ConditionCountSells(1, magico, ORDER_TYPE_SELL);
    //   conditionsToSell.AddCondition(countSells);
...
void OnDeinit(const int reason)
{
  ...
  delete incrementorBuy;
  delete incrementorSell;
}
...
void OnTick()
{
    ...
    // NOTE: evaluate conditions ontick
    if(conditionsToOpenBoth.EvaluateConditions())
    {
...
string lastWinDirection()
{
  ...
  datetime dateIni = dateFin - 10*24*60*60;
  ...
  for (int i = 0; i < total; i++) {
    ...
    if(profit == 0 || magic != magico) continue;
...
void deleteGrid(string side)
{
    if(side == "buy")
    if (CheckPointer(gridBuy) != POINTER_INVALID) { delete gridBuy; }
    
    if(side == "sell")
    if (CheckPointer(gridSell) != POINTER_INVALID) { delete gridSell; }
}
    ...
    for (int i = 0; i < PositionsTotal(); i++) {
        ...
        if (PositionGetSymbol(i) == _Symbol && PositionGetInteger(POSITION_MAGIC) == magico) {
            if (PositionGetInteger(POSITION_TYPE) == POSITION_TYPE_BUY) {
                trade.PositionModify(tk, 0.0, newTPbuy);
            }
            if (PositionGetInteger(POSITION_TYPE) == POSITION_TYPE_SELL) {
                trade.PositionModify(tk, 0.0, newTPsell);
            }
        }
    }
}
