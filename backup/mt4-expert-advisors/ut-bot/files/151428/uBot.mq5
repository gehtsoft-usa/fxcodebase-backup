//Available @ https://fxcodebase.com/code/viewtopic.php?f=38&t=73874
...
int OnInit()
{
    ...
    atrHandle = iATR(NULL, 0, atrPeriods);
                ...
                const int& spread [])
{
    ...
    for(i = start; i < rates_total && !IsStopped(); i++)
    {
        ...
        xATRTrailingStop[i] =
            cl > xATRTrailingStop[i - 1] && cl1 > xATRTrailingStop[i - 1] ?
            fmax(xATRTrailingStop[i - 1], cl - nLoss) : cl < xATRTrailingStop[i - 1] && cl1 < xATRTrailingStop[i - 1] ?
            fmin(xATRTrailingStop[i - 1], cl + nLoss) : cl > xATRTrailingStop[i - 1] ? cl - nLoss : cl + nLoss;
        ...
        {
            ArrowDn[i - s] = high[i - s];
        }

    }
    if(ArrowUp[1 - s] != EMPTY_VALUE && newCandle.IsNewCandle()) { Notifications(0); }
    if(ArrowDn[1 - s] != EMPTY_VALUE && newCandle.IsNewCandle()) { Notifications(1); }
...
double ATR(int _shift)
{
    double value[1];
    int shift = iBars(NULL, 0) - _shift - 1;
    int    copy = CopyBuffer(atrHandle, 0, shift, 1, value);
