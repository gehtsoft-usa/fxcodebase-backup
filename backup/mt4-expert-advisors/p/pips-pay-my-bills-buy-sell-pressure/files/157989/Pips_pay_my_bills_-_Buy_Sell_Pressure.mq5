//Available @  https://fxcodebase.com/code/viewtopic.php?f=38&p=146755#p146755
...
void OnInit()
{
   ...
   IndicatorSetString(INDICATOR_SHORTNAME, "Pips pay my bills - Buy Sell Pressure");
   IndicatorSetInteger(INDICATOR_DIGITS, Digits());
   id = plot1.RegisterInternalStreams(id);
   if (btf != PERIOD_CURRENT)
   {
      indi = iCustom(_Symbol, btf, "Pips_pay_my_bills_-_Buy_Sell_Pressure", bars_limit);
      if (indi == INVALID_HANDLE)
      {
         Print("Failed to create BTF indi");
      }
   }
   else
   {
                ...
                const int &spread[])
{
   ...
   for (int pos = MathMax(rates_total - 1 - bars_limit, MathMax(first, prev_calculated - 1)); pos < rates_total; ++pos)
   {
      if (indi != 0)
      {
         ...
         double upVal[1];
         if (CopyBuffer(indi, 0, btfPos, 1, upVal) == -1)
         {
            return 0;
         }
         double clr[1];
         CopyBuffer(indi, 1, btfPos, 1, clr);
         plot1.Set(pos, upVal[0], clr[0] == 0 ? Green : Red);
         continue;
      }
      int oldPos = rates_total - pos - 1;
      double tmp_sp = high[pos] - close[pos];
      double tmp_bp = close[pos] - low[pos];
      ema1Source.SetValue(pos, tmp_sp);
      double ema1Value[1];
      if (!ema1.GetValues(pos, 1, ema1Value)) { ema1Value[0] = EMPTY_VALUE; }
      double tmp_sp_ma = ema1Value[0];
      double nsp = ((tmp_sp_ma == 0) ? 0 : (SafeDivide(tmp_sp, tmp_sp_ma)));
      ema2Source.SetValue(pos, tmp_bp);
      double ema2Value[1];
      if (!ema2.GetValues(pos, 1, ema2Value)) { ema2Value[0] = EMPTY_VALUE; }
      double tmp_bp_ma = ema2Value[0];
      double nbp = ((tmp_bp_ma == 0) ? 0 : (SafeDivide(tmp_bp, tmp_bp_ma)));
      ema3Source.SetValue(pos, nbp);
      double ema3Value[1];
      if (!ema3.GetValues(pos, 1, ema3Value)) { ema3Value[0] = EMPTY_VALUE; }
      ema4Source.SetValue(pos, nsp);
      double ema4Value[1];
      if (!ema4.GetValues(pos, 1, ema4Value)) { ema4Value[0] = EMPTY_VALUE; }
      double diff = SafeMinus(ema3Value[0], ema4Value[0]);
      if ((SafeGreater(diff, 0)))
      {
         double bp = diff;
      }
      if ((SafeLess(diff, 0)))
      ...
      double plot1Value = plot1.Set(pos, diff, (SafeGreater(diff, 0) ? Green : Red));
   }
   return rates_total;
}
//Available @  https://fxcodebase.com/code/viewtopic.php?f=38&p=146755#p146755
