...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=74287
...
enum ModeTP {
    TP_IndicatorTP1, // Indi TP1
    TP_IndicatorTP2, // Indi TP2
    TP_FixPips,            // Fix Pips
    TP_byMoney,            // Money
...
// string file_custom_indicator = "PipFinite Trend PRO_fix.ex4";
// string file_custom_indicator = "\\Indicators\\PipFinite Trend PRO_fix.ex4";
// #resource "\\Indicators\\PipFinite Trend PRO_fix.ex4";
#resource "\\Indicators\\PipFinite_Trend_PRO_fix.ex4";
...
double Indi(int buffer, int candle)
{
    ...
    return iCustom(NULL, 0, "::Indicators\\PipFinite_Trend_PRO_fix.ex4", buffer, candle); 
}
#endif
...
input ModeTP       modeTP = TP_IndicatorTP1;            // Mode Take Profit:
...
input bool   BkvOnTP1    = true;             // Free Risk When TP1 is hit:
...
class BreackevenConditionTP1 : public iConditions
{
    Order* _order;
    ...
    bool evaluate()
    {
        ...
        // double ask = SymbolInfoDouble(_order.symbol(), SYMBOL_ASK);
        // double bid = SymbolInfoDouble(_order.symbol(), SYMBOL_BID);
        double tp1WasHit = Indi(20, 1);

        // if(_order.type() == OP_BUY)
        // {
            // if(bid >= tp1WasHit) return true;
            if(tp1WasHit>0) return true;
        // }

        // if(_order.type() == OP_SELL)
        // {
            // if(ask <= tp1WasHit) return true;
        // }

        return false;
    }
};
BreackevenConditionTP1* breackevenConditionTP1;
...
int OnInit()
{
    ...
    double temp = iCustom(NULL, 0, "::Indicators\\PipFinite_Trend_PRO_fix.ex4", 0, 0); 
    ...
    if( BkvOnTP1 ) conditionsToBreackeven.AddCondition(breackevenConditionTP1 = new BreackevenConditionTP1());
    else conditionsToBreackeven.AddCondition(breackevenCondition = new BreackevenCondition());
...
double TP(string side, double price = 0)
{
    ...
    if(takeProfitOn)
        switch(modeTP)
        {
            case TP_IndicatorTP1:
                result = Indi(12,1);
                break;
            case TP_IndicatorTP2:
                result = Indi(13,1);
                break;
            case TP_FixPips:
...
void doBreackevenAction()
{
    for(int i = mainOrders.qnt() - 1; i >= 0; i--)
    {
        ...
        {
            if( BkvOnTP1 ) breackevenConditionTP1.setOrder(mainOrders.index(i));
            else breackevenCondition.setOrder(mainOrders.index(i));
