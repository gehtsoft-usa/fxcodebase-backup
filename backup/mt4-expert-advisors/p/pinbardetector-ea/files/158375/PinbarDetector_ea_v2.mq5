//Available @  https://fxcodebase.com/code/viewtopic.php?f=38&t=75627
...
public:
     ...
     {
      refs = 1;
      indi = iCustom(symbol, timeframe, "PinbarDetector", CountBars, DisplayDistance, false, false, false, false, CustomMaxNoseBodySize,
                     CustomNoseBodyPosition, CustomLeftEyeOppositeDirection, CustomNoseSameDirection,
                     CustomNoseBodyInsideLeftEyeBody, CustomLeftEyeMinBodySize, CustomNoseProtruding,
                     CustomNoseBodyToLeftEyeBody, CustomNoseLengthToLeftEyeLength, CustomLeftEyeDepth,
                     CustomMinimumNoseLength);
     }
                    ~EntryStreamData()
     {
   ...
   double            Get(int period)
     {
      double buffer[1];
      if(CopyBuffer(indi, 0, period, 1, buffer) != 1)
        {
         return EMPTY_VALUE;
        }
      return buffer[0];
...
input bool maTouchFilter = true;
input bool maTrendFilter = true;
//
input int maFastPeriod = 10;
...
int ma()
  {
   if(!maTouchFilter && !maTrendFilter)
      return 0;
   int b = 0;
   if(entry_logic == TradingModeOnBarClose)
      b = 1;
   double fastMA = iMAMQL4(Symbol(), Period(), maFastPeriod, 0, maFastMethod, maFastPrice, b);
   double slowMA = iMAMQL4(Symbol(), Period(), maSlowPeriod, 0, maSlowMethod, maSlowPrice, b);
   int dir;
   bool mtf = true;
   if(maTouchFilter)
     {
      if(iHigh(Symbol(), PERIOD_CURRENT, b) >= fastMA && iLow(Symbol(), PERIOD_CURRENT, b) <= fastMA)
         mtf = true;
      else
         mtf = false;
     }
   ...
   virtual bool      IsPass(const int period, const datetime date)
     {
      ...
      return value < iClose(_symbol, _timeframe, period) && (ma() == 0 || ma() == 1);
     }
  };
   ...
   virtual bool      IsPass(const int period, const datetime date)
     {
      ...
      return value > iClose(_symbol, _timeframe, period) && (ma() == 0 || ma() == 2);
     }
  };
