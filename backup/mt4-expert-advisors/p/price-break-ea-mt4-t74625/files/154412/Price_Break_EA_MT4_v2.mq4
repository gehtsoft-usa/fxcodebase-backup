...
enum ModeLevelsTP {
    TP_ByRatioRR, // Risk Reward Ratio
    TP_FixPips    // Fix Pips
};
enum TSLMode {
    byPips, // By Pips
...
string file_custom_indicator = "BetterVolume 1.6";
// int bufferToBuy = ;
// int bufferToSell = ;

input string Tindi      = "== Indicator Setup =="; // ————————————
input bool   filterBVon = true;                    // Filter with Better Volume?

enum enum_signalBV {
    ClimaxHigh  = 0, // Climax High
    LowVolume   = 2, // Low Volume
    HighChurn   = 3, // High Churn
    ClimaxLow   = 4, // Climax Low
    ClimaxChurn = 5  // Climax Churn
};
input enum_signalBV BV_signal = ClimaxHigh; // Signal to Filter:
...
input int uHighExtra = 10; // Extra pips to buy in High
input int uLowExtra  = 10; // Extra pips to sell in Low
#endif
...
int                magico        = 202223;        // Magic Number:
...
input ModeLevelsTP modeTP       = TP_ByRatioRR;      // Mode Take Profit:
input double       uRatioTP     = 2;                 // Risk Reward Ratio
input double       userTPpips   = 20;                // Pips TP
...
input double     userSLpips  = 5;                  // Pips SL
input double     userSLmoney = 15;                 // Money SL

input bool whenCandleClose        = true; // Close When Candle Close?
bool       allowTradeOnThisCandle = true;
...
class ByFixPips : public iLevels
{
  ...
  public:
    ByFixPips(string inpSymbol, string inpSide, double inpPips, string inpMode, double Price = 0)
    {
        _pips   = inpPips;
        _symbol = inpSymbol;
        _side   = inpSide;
...
class ByPipsFromCandle : public iLevels
{
    ...
    int    _tfCandle;
    int    _shiftCandle;
    double _pipsFromCandle;
  
  public:
    ByPipsFromCandle(string inpSymbol, string inpSide, double inpPips, string inpMode, int timeFrameCandle, int shiftCandle)
    {
        _pips        = inpPips;
        _symbol      = inpSymbol;
        _side        = inpSide;
    ...
    {
        double mPoint   = MarketInfo(_symbol, MODE_POINT);
        double distance = _pipsFromCandle * 10 * mPoint;
        double result   = 0;
        ...
        if (_side == "buy") {
            if (_mode == "TP"){ result= high + distance;}
            if (_mode == "SL"){ result= low - distance;}
        }
        if (_side == "sell") {
            if (_mode == "TP") result= low - distance;
            if (_mode == "SL") result= high + distance;
        }
...
class BUYcondition1 : public iConditions
{
  public:
    bool evaluate()
    {
        ...
        return Ask > iHigh(NULL, 0, 1) + uHighExtra * 10 * _Point;
    }
};
...
class BUYcondition2 : public iConditions
{
  public:
    bool evaluate()
    {
        ...
        double indi = Indi(BV_signal, 1);
        return indi != EMPTY_VALUE && indi != 0;
    }
};
...
class SELLcondition1 : public iConditions
{
  public:
    bool evaluate()
    {
        ...
        return Bid < iLow(NULL, 0, 1) - uLowExtra * 10 * _Point;
    }
};
...
//--- SESSIONS CONTROL:
    ...
    switch (modeSL) {
    case FixPips:
        levelSL = new Levels(new ByFixPips(_Symbol, " ", userSLpips, "SL"));
        break;
    case byMoney:
        levelSL = new Levels(new ByMoney(_Symbol, " ", userLots, userSLmoney, "SL"));
        break;
    case PipsFromOpenCandle:
        levelSL = new Levels(new ByPipsFromCandle(_Symbol, " ", userSLpips, "SL", 0, 1));
        break;
    }

    switch (modeTP) {
    case TP_FixPips:
        levelTP = new Levels(new ByFixPips(_Symbol, " ", userTPpips, "TP"));
        break;
    case TP_ByRatioRR:
        levelTP = new Levels(new ByRatio(_Symbol, " ", 0, uRatioTP, "TP", levelSL.level()));
        break;
...
void OnTick()
{
    ...
    if (newCandle.IsNewCandle()) {
        allowTradeOnThisCandle = true;
        Print(__FUNCTION__, " allowTradeOnThisCandle: ", allowTradeOnThisCandle);
        if (whenCandleClose)
            closeAll();
    }
    // ——————————————————————————————————————————————————————————————————
    ...
    if (uTradeReverse == false && allowTradeOnThisCandle == true) {
        // NOTE: BUY normal
        if (conditionsToBuy.EvaluateConditions()) {
            if (modeEntry == Market) {
               double sl = SL("buy");
               double tp = TP("buy");
                actionSendOrder = new SendNewOrder("buy", Lots(), "", 0, sl, tp, magico);
            } else {
                double pr       = Price("buy", uEntryDistance, _Symbol);
                double sl = SL("buy", pr);
                double tp = TP("buy", pr);
                actionSendOrder = new SendNewOrder("buy", Lots(), "", pr, sl, tp, magico);
            }

            if (actionSendOrder.doAction()) {
                mainOrders.GetMarketOrders();
                allowTradeOnThisCandle = false;
        ...
        if (conditionsToSell.EvaluateConditions()) {
            if (modeEntry == Market) {
                double sl = SL("sell");
                double tp = TP("sell");
                actionSendOrder = new SendNewOrder("sell", Lots(), "", 0, sl, tp, magico);
            } else {
                double pr       = Price("sell", uEntryDistance, _Symbol);
                double sl = SL("sell", pr);
                double tp = TP("sell", pr);
                actionSendOrder = new SendNewOrder("sell", Lots(), "", pr, sl, tp, magico);
            }
            if (actionSendOrder.doAction()) {
                mainOrders.GetMarketOrders();
                allowTradeOnThisCandle = false;
