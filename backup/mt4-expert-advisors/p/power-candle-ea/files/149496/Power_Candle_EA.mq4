...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=73336
...
// para calcular las variaciones de precios
double       priceVariation = 0;
double       lastPrice      = 0;
double       currentPrice   = 0;

input string T_       = "== Power Bar Setup ==";  // ————————————
input uint   uSeconds = 5;                        // Seconds:
input int    uVarPoints  = 20;                    // Points:
input string             Iema           = "== Moving Average Setup ==";  // ————————————
input bool               emaFilterOn    = true;                          // Ema filter On:
...
#ifdef MARTINGALE
// NOTE: Martingale input
input string TMartingale           = "- Martingale -";    // Setup Martingale
input bool   martingaleOn          = true;                // Martingale On:
input double Martingale_Multiplier = 2;                   // Multiplier:
input double Martingale_MaxLots    = 10;                  // Max lots:
...
class Martingale
{
 ...
 public:
  Martingale(string symbol, int magic, double multiplicator, double maxLots) : _symbol(symbol), _magic(magic), _multi(multiplicator), _maxLots(maxLots) { ; }
  ~Martingale() { ; }
  ...
  bool lastWasLoss()
  {
    for (int i = OrdersHistoryTotal() - 1; i >= 0; i--)
    {
      ...
      {
        _lastLot = OrderLots();
        return OrderProfit() + OrderCommission() + OrderSwap() < 0;
      }
    }
  ...
  {
    if(Yesterday_lastWasLoss())
    {
      double lotCalculated = NormalizeDouble(_lastLot * _multi, 2);
      if (lotCalculated >= _maxLots){ return _maxLots;} 
      	
			return lotCalculated;
    }
    return lot;
  }
};
...
input int sesion0_trades  = 1; // Trades:
...
input int sesion1_trades  = 1; // Trades:
...
input int sesion2_trades  = 1; // Trades:
...
input int sesion3_trades  = 1; // Trades:
...
input int sesion4_trades  = 1; // Trades:
...
input int sesion5_trades  = 1; // Trades:
...
input int sesion6_trades  = 1; // Trades:
...
input int sesion7_trades  = 1; // Trades:
...
input int sesion8_trades  = 1; // Trades:
...
input int sesion9_trades  = 1; // Trades:
...
class Timer
{
   ...
   public:
    Timer(int Seconds)
    {
      initm     = TimeCurrent();
      seconds   = Seconds;
      nextEvent = TimeCurrent() + seconds;
    }
  ~Timer() { ; }

	// tiempo trascurrido desde que se inició
	int Elapsed()
	{
    	return TimeCurrent() - initm;
	}

	bool Event()
	{
		if(TimeCurrent() >= nextEvent)
		{
	  	nextEvent = TimeCurrent()+seconds;
			return true;
		}
 ...
 public:
  bool evaluate()
  {
    // TODO: condition Buy 1
		...
		// si la variacion es mayor a lo que el usuario quiere
    return priceVariation > uVarPoints;
  }
};
 ...
 public:
  bool evaluate()
  {
    // TODO: condition sell 1
		// si la variacion es menor a lo que el usuario quiere
    return priceVariation < -uVarPoints;
  }
};
...
int OnInit()
{
  ...
  timer = new Timer(uSeconds);

maxTrades[0]=sesion0_trades;
maxTrades[1]=sesion1_trades;
maxTrades[2]=sesion2_trades;
maxTrades[3]=sesion3_trades;
maxTrades[4]=sesion4_trades;
maxTrades[5]=sesion5_trades;
maxTrades[6]=sesion6_trades;
maxTrades[7]=sesion7_trades;
maxTrades[8]=sesion8_trades;
maxTrades[9]=sesion9_trades;

#ifdef MARTINGALE
martin = new Martingale(Symbol(), magico, Martingale_Multiplier, Martingale_MaxLots);
#endif
...
void OnTick()
{
  ...
  if (timer.Event())
  {
		// comparás actual con anterior
    priceVariation = lastPrice==0? 0 : Bid - lastPrice;
    priceVariation /= _Point;
    priceVariation = NormalizeDouble(priceVariation, 2);
    // actualizo el ultimo precio
		lastPrice      = Bid;
...
// si la cantidad de trades que se abrieron en la sessión llegó al maximo no permitir abrir más:
  if(sesionControl.CurrentSession()>=0)
	if (trades_in_session.current() == maxTrades[sesionControl.CurrentSession()]) return;
