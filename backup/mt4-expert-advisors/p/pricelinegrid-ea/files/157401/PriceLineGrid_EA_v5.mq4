...
enum ModeLevels {
    ...
    ByPriceLine         // by Price Line Indicator
};
enum TSLMode {
    byPips, // By Pips
...
enum ModeEntry { Market, PendingStop, PendingLimit, PriceLine };
...
enum ModeRobot {
    ...
    oneSide,    // One Side Mode
};

input ModeRobot mode = oneSide; // Robot Mode:

int direction = 0; // 0 nada 1:buy, -1:sell

void TrendControl()
{
    int pendingbuys, pendingsells, openBuys, openSells;
    ...
    int pendingtotal = pendingbuys + pendingsells;
    if ((pendingbuys == 1 && pendingsells == 1) || (pendingbuys == 0 && pendingsells == 0)) direction = 0;
    if ((openBuys >= 1 && openSells == 0)) direction = 1;
    if ((openBuys == 0 && openSells >= 1)) direction = -1;
    // Print(__FUNCTION__, " direction: ", direction);
    if ((openBuys + openSells) == 0) limitTrades.ResetCounter();
}

#ifdef CONTROL_CUSTOM_INDICATOR_FILE

string file_custom_indicator = "!AFP_PriceLineGrid.ex4";
int    bufferToBuy           = 0;
int    bufferToSell          = 0;

input string Tindi              = "== Indicator Setup =="; // ————————————
input int    NumLinesAboveBelow = 10;
...
double Indi(int buffer, int candle)
{
    ...
    return iCustom(NULL, 0, file_custom_indicator, NumLinesAboveBelow, SuperLevels, LineColorSuper, LineStyleSuper, LineWidthSuper, MSuperLevels, LineColorMSuper, LineStyleMSuper, LineWidthMSuper,
                   MainLevels, LineColorMain, LineStyleMain, LineWidthMain, ShowSubLevels, SubLevels, LineColorSub, LineStyleSub, LineWidthSub, 0, 0);
}
...
void takeAllPrices()
{
    ...
    for (int i = 0; i < total; i++) {
        ...
        if (StringSubstr(name, 0, 11) == "[SweetSpot]") price = ObjectGetDouble(0, name, OBJPROP_PRICE);

        int t = ArraySize(allPrices);
        if (ArrayResize(allPrices, t + 1)) {
            allPrices[t] = price;
        }
    }
}

double distanceBtwPrices() { return fabs(allPrices[1] - allPrices[0]); }

double UpPrice()
{
    takeAllPrices();
    for (int i = 1; i < ArraySize(allPrices)-1; i++) {
        if (allPrices[i] > Ask && allPrices[i - 1] < Ask) {
            return allPrices[i];
        }
    }
    return 0;
...
{
    takeAllPrices();
    for (int i = 0; i < ArraySize(allPrices)-1; i++) {
        if (allPrices[i] < Bid && allPrices[i + 1] > Bid) {
            return allPrices[i];
        }
    }
    return 0;
...
bool cuttingLevel()
{
    takeAllPrices();
    for (int i = 0; i < ArraySize(allPrices)-1; i++) {
        if (allPrices[i] <= Ask && allPrices[i] >= Bid) {
            return true;
        }
    }
    return false;
...
input ModeEntry modeEntry         = PriceLine;                       // Mode Entry:
...
input bool         repeat        = true; // Repeat Trade if SL ?
...
input ModeLevels modeTP       = ByPriceLine;       // Mode Take Profit:
...
input ModeLevels modeSL       = ByPriceLine;       // Mode Stop Loss:
...
MartingaleIncrementLot martin(magico, _Symbol);
...
input double GridUser_multiplier = 1;                  // Multiplier:
  ...
  public:
    ...
    bool doAction()
    {
        ...
        if(CheckPointer(newOrder)!= POINTER_INVALID)
         tk = OrderSend(newOrder.symbol(), newOrder.type(), newOrder.lot(), newOrder.price(), 1000, newOrder.sl(), newOrder.tp(), newOrder.comment(), newOrder.magic(), newOrder.expireTime(), clrNONE);
...
class BUYcondition2 : public iConditions
{
  public:
    bool evaluate()
    {
        ...
        if (direction != 0) return cuttingLevel();
    ...
    bool evaluate()
    {
        ...
        for (int i = OrdersTotal() - 1; i >= 0; i--) {
            if (OrderSelect(i, SELECT_BY_POS) && OrderSymbol() == _Symbol && OrderMagicNumber() == magico) {
                ...
                if (Ask < OrderOpenPrice() + (distanceBtwPrices() * 0.50) && Ask > OrderOpenPrice() - (distanceBtwPrices() * 0.50)) return false;
            }
        }
        return true;
    }
...
class ConditionCountTrades : public iConditions
{
  ...
  public:
    ...
    bool evaluate()
    {
        int count = 0;
        for (int i = OrdersTotal() - 1; i >= 0; i--) {
            if (OrderSelect(i, SELECT_BY_POS) && OrderSymbol() == _Symbol && OrderMagicNumber() == magico && (OrderType() == OP_BUY || OrderType() == OP_SELL)) {
                count += 1;
...
class ConditionLimitTrades : public iConditions
{
    int _max;
    int _countAcum;
    int _count;
  ...
  public:
    ConditionLimitTrades(int maxTrades)
    {
        _max       = maxTrades;
        _countAcum = 0;
    }
    ~ConditionLimitTrades() { ; }

    void IncreaseCounter() { _count += 1; }
    ...
    bool evaluate()
    {
        ...
        if (_count > _countAcum) {
            _countAcum = _count;
        }
        if (currentcount == 0) {
            _countAcum = currentcount;
            _count     = 0;
        }
...
ConditionLimitTrades *limitTrades;
  ...
  public:
    bool evaluate()
    {
        ...
        for (int i = OrdersTotal() - 1; i >= 0; i--) {
            if (OrderSelect(i, SELECT_BY_POS) && OrderSymbol() == _Symbol && OrderMagicNumber() == magico) {
                //   recorrer las ordenes
                //    los precios tienen que estar al menos al 50% de distancia de cualquier orden
                if (Ask < OrderOpenPrice() + (distanceBtwPrices() * 0.50) && Ask > OrderOpenPrice() - (distanceBtwPrices() * 0.50)) return false;
            }
        }
        return true;
...
int OnInit()
{
    ...
    if (mode == oneSide) {
        conditionsToBuy.AddCondition(buyCondition1 = new BUYcondition1()); // direccion != -1
        conditionsToBuy.AddCondition(buyCondition2 = new BUYcondition2()); // corte de nivel
    ...
    if (mode == oneSide) {
        conditionsToSell.AddCondition(sellCondition1 = new SELLcondition1()); // direccion != 1
        conditionsToSell.AddCondition(sellCondition2 = new SELLcondition2()); // corte de nivel
    ...
    conditionsToBuy.AddCondition(limitTrades = new ConditionLimitTrades(uMaxTrades));
    conditionsToSell.AddCondition(limitTrades = new ConditionLimitTrades(uMaxTrades));
#endif
...
void OnTick()
{
    ...
    if (!uTradeReverse) {
        if (conditionsToBuy.EvaluateConditions()) {
            if (modeEntry == Market || direction == 1) {
    ...
    if(repeat && lastCloseWasStopLoss() && lastCloseCMD() == OP_BUY) {        
        actionSendOrder = new SendNewOrder("buy", Lots(), "", 0, SL("buy"), TP("buy"), magico);
        }
    if(repeat && lastCloseWasStopLoss() && lastCloseCMD() == OP_SELL) {        
        actionSendOrder = new SendNewOrder("sell", Lots(), "", 0, SL("sell"), TP("sell"), magico);
        }
    }
...
double Price(string direction, int pips = 0, string _symbol = "")
{
    ...
    if (direction == "buy") {
        ...
        if (modeEntry == PriceLine) {
            price = UpPrice();

            if (mode == oneSide && direction == 1) price = Ask;
        }
    ...
    if (direction == "sell") {
        ...
        if (modeEntry == PriceLine) {
            price = DnPrice();
            if (mode == oneSide && direction == -1) price = Bid;
        }
...
double SL(string side, double price = 0)
{
    ...
    if (modeSL == ByPriceLine) {
        if (side == "buy") {
            // takeUPLines();
            // takeAllPrices();
            // double distance = UpPrices[1] - UpPrices[0];
            // result = price - distance;
            result = price - distanceBtwPrices();
        }
        if (side == "sell") {
            // takeUPLines();
            // takeAllPrices();
...
void Notifications(int type)
{
    // time Control
    if (timeNextNotify != 0)
        if (TimeCurrent() < timeNextNotify) return;
...
void closePendingIfHaveAnOpenTrade()
{
    countTrades = new ConditionCountTrades(1);
    if (!countTrades.evaluate()) {
        actionDeletePendingsBuys = new ActionDeletePendings("buy", magico);
        actionDeletePendingsBuys.doAction();
        delete actionDeletePendingsBuys;
        actionDeletePendingsSells = new ActionDeletePendings("sell", magico);
