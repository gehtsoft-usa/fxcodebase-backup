...
//https://fxcodebase.com/code/viewtopic.php?f=38&t=72559
		...
		for (int i = 1; i < 99; i++)
    {
      ...
      if (ratio < 0.30)
      {
      	...
      	double position = 0; (op - lo) / size;
				if(size > 0) position = (op - lo) / size;

				if (position < 0.35) { direction = "q";havePinbar = true;}
      	if (position > 0.75) { direction = "p";havePinbar = true;}
      }
 ...
 public:
  ...
  void setValues(int rowNumber, string& values[])
  {
    for (int j = 0; j < ArraySize(values); j++)
    {
      ...
      // Down Format
      if (values[j] == "q") { _rows[rowNumber + 1, j + 1].TextColor(Tomato, 255); }
...
int OnInit()
{
  ...
  {
    AddPinbar(symbols.currentSymbol());
  }
                ...
                const int&      spread[])
{
	...
	if(newCandle.IsNewCandle()) { doActions(); }
...
{
	// clang-format off
	for(int i=0;i < ArraySize(pinbars) ; i++)
	{
		...
		{
			string patCode = pinbars[i].FindPattern(TF[j]);	
	...
	if(direction == "q") { clr = Tomato; code = 234; dir = false; }
...
void AddPinbar(string sym)
{
  int t = ArraySize(pinbars);
  if (ArrayResize(pinbars, t + 1))
  {
    pinbars[t] = new Pinbar(sym);
  }
}
