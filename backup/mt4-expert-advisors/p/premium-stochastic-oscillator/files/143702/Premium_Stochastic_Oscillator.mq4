...
input double inpLevel1       = 0.9;
input double inpLevel2       = 0.2;

input string Custom_Indicator = "Premium Stochastic Oscillator";
...
int CreateAlert(int id, ICondition* condition, IAction* action, int code, string codeStr, string message, color clr, PriceType priceType, int sign)
{
   ...
   switch (Type)
   {
      ...
      case Text:
         {
            highStream = new SimplePriceStream(_Symbol, (ENUM_TIMEFRAMES)_Period, priceType);
            highStream.SetShift(shift_arrows_pips * sign);
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      return Level__2[period] != 0 && Level__2[period] != EMPTY_VALUE;
   }
};
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      return Level__4[period] != 0 && Level__4[period] != EMPTY_VALUE;
   }
};
...
int init() {
   ...
   IndicatorObjPrefix = GenerateIndicatorPrefix("pso");
   IndicatorShortName("pso");
...
int start() {
   ...
   for (pos = Bars - gi_116 - 1; pos >= 0; pos--)
   {
      ...
      if (customStream != NULL)
      {
         customStream.SetValue(pos, val[pos]);
      }
