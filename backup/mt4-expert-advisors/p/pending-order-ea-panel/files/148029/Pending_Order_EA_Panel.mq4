...
//https://fxcodebase.com/code/viewtopic.php?f=38&t=72871
...
class GUI : public CAppDialog
{
 ...
 public: 
  ...
  ~GUI() 
	...
	CButton bt1, bt2, bt3, bt4, bt5, bt6;
	...
	bool Create(const long chart, const string name, const int subwin, const int x1, const int y1, const int x2, const int y2)
  {
    ...
    if (!Create_button("SELL Stop",                 Col(0), Row(0), _high, _width, bt1)) return false; 
    if (!Create_button("BUY Stop",                  Col(1), Row(0), _high, _width, bt2)) return false; 
    if (!Create_button("BUY & SELL stop",           Col(0), Row(1), _high, _width*2+_gapH, bt3)) return false; 
    if (!Create_button("Close Lossers",             Col(0), Row(2), _high, _width, bt4)) return false; 
    if (!Create_button("Close Winners",             Col(1), Row(2), _high, _width, bt5)) return false; 
    if (!Create_button("Close All Orders",          Col(0), Row(3), _high, _width*2+_gapH, bt6)) return false; 
...
input int          magico         = 22669;              // Magic Number:
...
int OnInit()
{
	...
	{
		if (!gui.Create(0, "Pending Orders Panel",0, 10,10,240,250))
	  {
	...
	gui.setButton2Action(buyStopPrevious = new BuyStopPrevious());
	gui.setButton3Action(buyAndSellStopPrevious = new BuyAndSellStopPrevious());
	gui.setButton4Action(closeLossers = new ActionCloseByProfit("loss",magico,_Symbol));
	gui.setButton5Action(closeWinners = new ActionCloseByProfit("win",magico,_Symbol));
	gui.setButton6Action(closeAll = new ActionCloseByProfit("all",magico,_Symbol));
...
class BuyAndSellStopPrevious : public iActions
{
	bool doAction()
	{
      ...
      delete actionSendOrder;
		  pr       = PricePrevious("sell", uEntryDistance, _Symbol);
      actionSendOrder = new SendNewOrder("sell", Lots(), "", pr, SL("sell", pr), TP("sell", pr), magico,"previous");
      if(!actionSendOrder.doAction())
			{
        Print("Can't Send order");
...
BuyAndSellStopPrevious* buyAndSellStopPrevious;
