...
// https://fxcodebase.com/code/viewtopic.php?f=38&p=147223
...
bool         reverse        = true;                 // Reverse Mode:
...
input CloseAllMode closeBy                 = CloseByEma;                 // Close All Mode:
input int          emaToClose              = 8;                          // Ema to Close all:
...
class BUYcondition1 : public iConditions
{
 public:
  bool evaluate()
  {
    ...
    double indiBuySignal = iCustom(NULL, 0, "Price MA Cross Alert", "", 20, 0, 1);
    if (indiBuySignal > 0 && indiBuySignal != EMPTY_VALUE) return true;
...
class SELLcondition1 : public iConditions
{
 public:
  bool evaluate()
  {
    ...
    double indiSellSignal = iCustom(NULL, 0, "Price MA Cross Alert", "", 20, 1, 1);
    if (indiSellSignal > 0 && indiSellSignal != EMPTY_VALUE) return true;
...
class ConditionToCloseBuy : public iConditions
{
 public:
  ...
  {		
		...
		{ 
			// if(closeBy == CloseByEma)
			// {
				double ema = iMA(NULL, 0, emaToClose, 0, MODE_EMA, PRICE_CLOSE, 0);
				double l = iLow(NULL, 0, 0);
				Print("l: ",l," line: ", 2526);
				Print("ema: ",ema," line: ", 2528);
				Print("Bid: ",Bid," line: ",2530);
				if(Bid >= ema && l < ema ) return true;
			// }
		}
 ...
 public:
  bool evaluate()
  {
		...
		{ 
			...
			// {
				double ema = iMA(NULL, 0, emaToClose, 0, MODE_EMA, PRICE_CLOSE, 0);
				Print("ema: ",ema," line. ", 2554);
				double h = iHigh(NULL, 0, 0);
				Print("h: ",h," line. ", 2556);
				Print("Ask: ",Ask," line.  ",2530);
				if(Ask <= ema && h > ema) return true;
			// }
		}
    if (closeAllInOpositeSignal)
    {
...
int OnInit()
{
  ...
  if(reverse)
    conditionsToBuy.AddCondition(countSells = new ConditionCountSells(1));
  else
  ...
  if(reverse)
		conditionsToSell.AddCondition(countBuys = new ConditionCountBuys(1));
	else 
...
//--- CANDLE CLOSE:
  ...
  {
    if (reverse) OpenSell(); else OpenBuy();
  }
  if (conditionsToSell.EvaluateConditions())
  {
		if (reverse) OpenBuy(); else OpenSell();
  }
}
      ...
      case CloseByPips: 
      {
         ...
         break;
			...
			case CloseByEma:
			{
				...
				// if(Ask >= ema && Bid <= ema) return true;
