...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      double entry = iCustom(_symbol, _timeframe, "Period Range Indicator Info", Period, Ratio, Position, 0, period);
      return iClose(_symbol, _timeframe, period) > entry;
   }
};
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      double entry = iCustom(_symbol, _timeframe, "Period Range Indicator Info", Period, Ratio, Position, 1, period);
      return iClose(_symbol, _timeframe, period) < entry;
   }
};

class LongFSCondition : public ACondition
{
public:
   LongFSCondition(const string symbol, ENUM_TIMEFRAMES timeframe)
      :ACondition(symbol, timeframe)
   {
   ...
   bool IsPass(const int period, const datetime date)
   {
      double val = iCustom(_symbol, _timeframe, "freescalperindicator", Length, 0, period);
      return val != 0 && val != EMPTY_VALUE;
   }
};

class ShortFSCondition : public ACondition
{
public:
   ShortFSCondition(const string symbol, ENUM_TIMEFRAMES timeframe)
      :ACondition(symbol, timeframe)
   {
   ...
   bool IsPass(const int period, const datetime date)
   {
      double val = iCustom(_symbol, _timeframe, "freescalperindicator", Length, 1, period);
      return val != 0 && val != EMPTY_VALUE;
   }
};
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      double stop = iCustom(_symbol, _timeframe, "Period Range Indicator Info", Period, Ratio, Position, 2, period);
      double limit = iCustom(_symbol, _timeframe, "Period Range Indicator Info", Period, Ratio, Position, 4, period);
      return iClose(_symbol, _timeframe, period) < stop
         || iClose(_symbol, _timeframe, period) > limit;
   }
};
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      double stop = iCustom(_symbol, _timeframe, "Period Range Indicator Info", Period, Ratio, Position, 3, period);
      double limit = iCustom(_symbol, _timeframe, "Period Range Indicator Info", Period, Ratio, Position, 5, period);
      return iClose(_symbol, _timeframe, period) > stop
         || iClose(_symbol, _timeframe, period) < limit;
   }
};
...
ICondition* CreateLongFilterCondition(string symbol, ENUM_TIMEFRAMES timeframe)
{
   ...
   AndCondition* condition = new AndCondition();
   condition.Add(new LongFSCondition(symbol, timeframe), false);
   return condition;
}
...
ICondition* CreateShortFilterCondition(string symbol, ENUM_TIMEFRAMES timeframe)
{
   ...
   AndCondition* condition = new AndCondition();
   condition.Add(new ShortFSCondition(symbol, timeframe), false);
   return condition;
}
...
int OnInit()
{
   double temp = iCustom(NULL, 0, "Period Range Indicator Info", 0, 0);
   if (GetLastError() == ERR_INDICATOR_CANNOT_LOAD)
   {
      Alert("Please, install the 'Period Range Indicator Info' indicator");
      return INIT_FAILED;
   }
   temp = iCustom(NULL, 0, "freescalperindicator", 0, 0);
   if (GetLastError() == ERR_INDICATOR_CANNOT_LOAD)
   {
      Alert("Please, install the 'freescalperindicator' indicator");
      return INIT_FAILED;
   }
   #ifdef SHOW_ACCOUNT_STAT
      stats = NULL;
