...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=74021
...
input bool         TPbySLdistance = true;                        // TP by SL distance
input double       TPSLmulti      = 2.0;                         // SL distance multipler
...
public:
                     TPbySL(string inpSymbol, string inpSide, int inpPips, string inpMode, double Price = 0)
     {
      _pips   = inpPips;
      _symbol = inpSymbol;
      _side   = inpSide;
   ...
   if(stopLossOn)
     {
      ...
      if(TPbySLdistance)
         slDist =  MathAbs(Ask - result)/Point;
     }
   return result;
  }
//+------------------------------------------------------------------+
   ...
   if(takeProfitOn)
     {
      if(TPbySLdistance)
        {
        Comment(slDist);
         levelTP = new Levels(new TPbySL(_Symbol, side, userTPpips, "TP", price));
         return levelTP.calculateLevel();
        }
      switch(modeTP)
        {
         case FixPips:
         ...
         case byMoney:
            ...
            break;
         case PipsFromOpenCandle:
            levelSL = new Levels(new ByPipsFromCandle(_Symbol, side, userTPpips, "TP", 0, 1));
            result  = levelTP.calculateLevel();
        }
     }
   return result;
