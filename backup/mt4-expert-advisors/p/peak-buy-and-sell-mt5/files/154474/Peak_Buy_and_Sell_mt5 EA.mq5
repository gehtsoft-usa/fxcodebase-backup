...
#property description "This EA need the file \"Peak_Buy_and_Sell_v2.ex5\" in the folder MQL5/Indicators"
...
string file_custom_indicator = "Peack_Buy_and_Sell_v2";
int handle = 0;
...
input string       T_ = "== Peak Buy & Sell Setup ==";                  // ————————————
input int     Len = 100;
...
class BUYcondition1 : public iConditions
{
    ...
    bool evaluate()
    {
        ...
        {
           if(custom_indicator(0, i) > UrovenSignal && custom_indicator(0, i+1) <= UrovenSignal)return true;
        }
        // return 
        return false;
    }
...
class SELLcondition1 : public iConditions
{
    ...
    bool evaluate()
    {
        ...
        for(int i=10 - 1;i >= 0 ;i--)
        {
            if(custom_indicator(1, i) > UrovenSignal && custom_indicator(1, i+1) <= UrovenSignal)return true;
        }
        return false;
    }
};
...
void setHandle()
{
    handle = iCustom(Symbol(), NULL, file_custom_indicator, Len, HistoryBars, TF1, TF2, ModeHL, ModeOnline, ModeinFile, ModeHistory, alert, sound, email, GV, UrovenSignal );
}
