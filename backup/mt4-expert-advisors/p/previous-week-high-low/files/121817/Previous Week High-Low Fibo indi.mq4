...
// http://fxcodebase.com/code/viewtopic.php?f=38&p=121819
...
double week_high[], week_low[], fib1[], fib2[];
Signaler *_signaler;
...
int init()
{
   IndicatorName = GenerateIndicatorName("Previous Week High-Low Fibo");
   IndicatorObjPostfix = "__" + IndicatorName + "__";
   IndicatorShortName(IndicatorName);

   SetIndexBuffer(0, week_high);
   SetIndexBuffer(1, week_low);
   SetIndexBuffer(2, fib1);
   SetIndexBuffer(3, fib2);
...
int start()
{
   ...
   if (last_bars != iBars(_Symbol, PERIOD_W1))
   {
      last_bars = iBars(_Symbol, PERIOD_W1);
      limit = Bars - 2;
   }  
   int pos = limit;
   while (pos >= 0)
   {
      int w1_pos = iBarShift(_Symbol, PERIOD_W1, Time[pos]);
      if (w1_pos >= 0 && iBars(_Symbol, PERIOD_W1) > w1_pos + 1)
      {
         week_high[pos] = iHigh(_Symbol, PERIOD_W1, w1_pos + 1);
         week_low[pos] = iLow(_Symbol, PERIOD_W1, w1_pos + 1);
         double range = week_high[pos] - week_low[pos];
         if (iOpen(_Symbol, PERIOD_W1, w1_pos + 1) > iClose(_Symbol, PERIOD_W1, w1_pos + 1))
         {
            fib1[pos] = week_low[pos] + range * Fib1Level;
            fib2[pos] = week_low[pos] + range * Fib2Level;
            if (pos == 0)
            {
               if (fib1[0] <= Close[0] && fib1[1] > Close[1])
               {
         ...
         {
            fib1[pos] = week_high[pos] - range * Fib1Level;
            fib2[pos] = week_high[pos] - range * Fib2Level;
            if (pos == 0)
            {
               if (fib1[0] >= Close[0] && fib1[1] < Close[1])
               {
