...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=74365
...
class Timer
{
    ...
    public:
    Timer(uint seconds) : ciclo(seconds) {};
    ~Timer() { ; }
    ...
    void stop() { inicio = 0; next = 0; }
    void setNext() { next += ciclo; }

};

input int BuyTimerSeconds = 1;// BUY Timer Minutes:
input int SellTimerSeconds = 1;// SELL Timer Minutes:
int buyTimerSeconds = BuyTimerSeconds * 60;
int sellTimerSeconds = SellTimerSeconds * 60;

Timer buytimer(buyTimerSeconds);
Timer selltimer(sellTimerSeconds);
...
bool OnInit_GUI()
{
    ...
    {
        res = gui.Create(0, "Panel EA", 0, 400, 0, 500, 200);
        if(res) gui.Run();
        news.OnInit();
    }
...
{
    gui.reason(reason);
    if(gui.reason() != REASON_CHARTCHANGE && gui.reason() != REASON_PARAMETERS && gui.reason() != REASON_TEMPLATE)
    {
        gui.Destroy(reason);
...
class MyButton : public CButton
{
    ...
    MyButton()
    {
        active_clr = PaleGreen;
    }
   ~MyButton() { ; }

   // state control
   bool State() { return state; }
   void State(bool st) { state = st; SetStyle(); }
   void changeBtnState()
   {
        state = !state;
        // // Print(__FUNCTION__, " ", " State: ", state);
        SetStyle();
   }

   // style control
   void setHoverClr(color clr) { hover_clr = clr; }
   void SetStyle()
   {
       if(State())
           ColorBackground(active_clr);
       else
   ...
   {
       if(IsActive())
           ColorBorder(LimeGreen);
       else
           ColorBorder(CONTROLS_BUTTON_COLOR_BORDER);
   }
...
class GUI : public CAppDialog
{
    ...
    void DoActions()
    {
        ...
        {
            if(buytimer.ok())
            button1Action.doAction();
        }
        if(bt2.State())
        {
            if(selltimer.ok())
            button2Action.doAction();
        }
    }
    ...
    bool Create(const long chart, const string name, const int subwin, const int x1, const int y1, const int x2, const int y2)
    {
        ...
        if(!Create_button("Stop EA", Col(0), Row(3), _high, _width, bt4)) return false;
...
ON_EVENT(ON_CLICK, bt1, bt1action)
ON_EVENT(ON_CLICK, bt2, bt2action)
ON_EVENT(ON_CLICK, bt3, bt3action)
ON_EVENT(ON_CLICK, bt4, bt4action)
...
#ifdef TIMER_MINI
input string T1 = "== Trading Sessions ==";  // ————————————
input double   uTimeZone = -9;               // Set your GMT zone:
...
class ScheduleController
{
    ...
    bool doSessionControl()  // control day and hours for every session
    {
        ...
        datetime dtCurrent = (datetime) actual;
        string dtc = TimeToString(dtCurrent, TIME_MINUTES);
