...
// http://fxcodebase.com/code/viewtopic.php?f=38&t=68979
...
input double D1 = 1.0; // Deviation for inner band
input double D2 = 2.0; // Deviation for mid band
input double D3 = 3.0; // Deviation for outer band
...
input bool alert_center = true; // Alert on Center Line
input bool alert_inner = true; // Alert on Inner Line
input bool alert_middle = true; // Alert on Middle Line
input bool alert_outer = true; // Alert on Outer Line
...
class CenterUpAlertCondition : public ABaseCondition
{
public:
   CenterUpAlertCondition(const string symbol, ENUM_TIMEFRAMES timeframe)
   ...
   {
      return iClose(_symbol, _timeframe, period) > M[period]
         && iClose(_symbol, _timeframe, period + 1) <= M[period + 1];
   }
};

class CenterDownAlertCondition : public ABaseCondition
{
public:
   CenterDownAlertCondition(const string symbol, ENUM_TIMEFRAMES timeframe)
      :ABaseCondition(symbol, timeframe)
   {
...
class InnerUpAlertCondition : public ABaseCondition
{
public:
   InnerUpAlertCondition(const string symbol, ENUM_TIMEFRAMES timeframe)
      :ABaseCondition(symbol, timeframe)
   {
   ...
   bool IsPass(const int period)
   {
      return iClose(_symbol, _timeframe, period) > IU[period]
         && iClose(_symbol, _timeframe, period + 1) < IU[period + 1];
   }
};

class InnerDownAlertCondition : public ABaseCondition
{
public:
   InnerDownAlertCondition(const string symbol, ENUM_TIMEFRAMES timeframe)
      :ABaseCondition(symbol, timeframe)
   {
   ...
   bool IsPass(const int period)
   {
      return iClose(_symbol, _timeframe, period) > ID[period]
         && iClose(_symbol, _timeframe, period + 1) < ID[period + 1];
   }
};

class MiddleUpAlertCondition : public ABaseCondition
{
public:
   MiddleUpAlertCondition(const string symbol, ENUM_TIMEFRAMES timeframe)
      :ABaseCondition(symbol, timeframe)
   {
   ...
   bool IsPass(const int period)
   {
      return iClose(_symbol, _timeframe, period) > MU[period]
         && iClose(_symbol, _timeframe, period + 1) < MU[period + 1];
   }
};

class MiddleDownAlertCondition : public ABaseCondition
{
public:
   MiddleDownAlertCondition(const string symbol, ENUM_TIMEFRAMES timeframe)
      :ABaseCondition(symbol, timeframe)
   {
   ...
   bool IsPass(const int period)
   {
      return iClose(_symbol, _timeframe, period) > MD[period]
         && iClose(_symbol, _timeframe, period + 1) < MD[period + 1];
   }
};

class OuterUpAlertCondition : public ABaseCondition
{
public:
   OuterUpAlertCondition(const string symbol, ENUM_TIMEFRAMES timeframe)
      :ABaseCondition(symbol, timeframe)
   {
   ...
   bool IsPass(const int period)
   {
      return iClose(_symbol, _timeframe, period) > OU[period]
         && iClose(_symbol, _timeframe, period + 1) < OU[period + 1];
   }
};

class OuterDownAlertCondition : public ABaseCondition
{
public:
   OuterDownAlertCondition(const string symbol, ENUM_TIMEFRAMES timeframe)
      :ABaseCondition(symbol, timeframe)
   {
   ...
   bool IsPass(const int period)
   {
      return iClose(_symbol, _timeframe, period) > OD[period]
         && iClose(_symbol, _timeframe, period + 1) < OD[period + 1];
   }
};

double M[], IU[], ID[], MU[], MD[], OU[], OD[];
...
int init()
{
   ...
   IndicatorName = GenerateIndicatorName("Triple Bollinger Bands");
   ...
   IStream* price = new PriceStream(_Symbol, (ENUM_TIMEFRAMES)_Period, PriceClose);
   stdDev = new StDevStream(price, N);
   price.Release();
   ...
   SetIndexBuffer(1, IU);
   SetIndexLabel(1, "Inner Up");

   SetIndexStyle(2, DRAW_LINE);
   SetIndexBuffer(2, ID);
   SetIndexLabel(2, "Inner Down");

   SetIndexStyle(3, DRAW_LINE);
   SetIndexBuffer(3, MU);
   SetIndexLabel(3, "Middle Up");

   SetIndexStyle(4, DRAW_LINE);
   SetIndexBuffer(4, MD);
   SetIndexLabel(4, "Middle Down");

   SetIndexStyle(5, DRAW_LINE);
   SetIndexBuffer(5, OU);
   SetIndexLabel(5, "Outer Up");

   SetIndexStyle(6, DRAW_LINE);
   SetIndexBuffer(6, OD);
   SetIndexLabel(6, "Outer Down");
   ...
   {
      id = CreateAlert(id,
         new CenterUpAlertCondition(_Symbol, (ENUM_TIMEFRAMES)_Period), 
         new CenterDownAlertCondition(_Symbol, (ENUM_TIMEFRAMES)_Period));
   }
   if (alert_inner)
   {
      id = CreateAlert(id,
         new InnerUpAlertCondition(_Symbol, (ENUM_TIMEFRAMES)_Period), 
         new InnerDownAlertCondition(_Symbol, (ENUM_TIMEFRAMES)_Period));
   }
   if (alert_middle)
   {
      id = CreateAlert(id,
         new MiddleUpAlertCondition(_Symbol, (ENUM_TIMEFRAMES)_Period), 
         new MiddleDownAlertCondition(_Symbol, (ENUM_TIMEFRAMES)_Period));
   }
   if (alert_outer)
   {
      id = CreateAlert(id,
         new OuterUpAlertCondition(_Symbol, (ENUM_TIMEFRAMES)_Period), 
         new OuterDownAlertCondition(_Symbol, (ENUM_TIMEFRAMES)_Period));
   }
...
int start()
{
   ...
   {
      double m = iMA(_Symbol, (ENUM_TIMEFRAMES)_Period, N, 0, MODE_SMA, PRICE_CLOSE, pos);
      double d;
      if (!stdDev.GetValue(pos, d))
         return false;
