...
//https://fxcodebase.com/code/viewtopic.php?f=38&t=72891
...
input string TEndActions         = "== Actions when session end ==";  // == Actions when session end ==
input bool   uEndAction_CloseAll = true;                                  // Close all open orders and pending orders after end time:
input bool   uEndAction_CloseProfitAndPendings = false;                    // Close all profit orders and pending orders after end time
input bool   uEndAction_CloseLossersAndPendings = false;                   // Close all losing orders and pending orders after end time
...
class BUYcondition1 : public iConditions
{
 public:
  bool evaluate()
  {
    ...
    return zz.price(0) < zz.price(1) && zz.price(1) > zz.price(2) && zz.price(2) < zz.price(3) && zz.price(1) > zz.price(3);
    return zz.price(1) > zz.price(3);
    // return false;
  }
};
...
class BUYcondition2 : public iConditions
{
 public:
  bool evaluate()
  {
    ...
    return iClose(NULL, 0, 1) > ma.index(1) && ma.index(1) > ma.index(3);
    // return false;
  }
};
...
void OnTick()
{
  ...
  // ------------------------------------------------------------------
  // borrar las pendientes si hay un nuevo HH-LL
  if (newHH())
  {
  ...
  if (direction == "buy")
  {
    ...
    if (modeEntry == PendingStop)
    {
      ...
      // {
				...
				price = zz.price(1) + distance;
        if (price < Ask) price = 0;
...
bool newHH()
{
  if (zz.price(0) < zz.price(1) && zz.price(1) > zz.price(2) && zz.price(2) < zz.price(3) && zz.price(1) > zz.price(3))
  {
    double Price_NewTrade = Price("buy", uEntryDistance, _Symbol);
		// Print(__FUNCTION__," Price_NewTrade: ",Price_NewTrade);
    double Price_PendingOrder;
    ...
    for (int i = OrdersTotal() - 1; i >= 0; i--)
    {
      if (OrderSelect(i, SELECT_BY_POS) && OrderSymbol() == _Symbol && OrderMagicNumber() == magico)
      {
        ...
        {
          Price_PendingOrder = OrderOpenPrice();
        }
      }
    }


		// Print(__FUNCTION__," Price_PendingOrder: ",Price_PendingOrder);

    if (Price_NewTrade > Price_PendingOrder && Price_PendingOrder!=0)
    {
      return true;
    }
  }
...
bool newLL()
{
  if (zz.price(0) > zz.price(1) && zz.price(1) < zz.price(2) && zz.price(2) > zz.price(3) && zz.price(1) < zz.price(3))
  {
    double Price_NewTrade = Price("sell", uEntryDistance, _Symbol);
		// Print(__FUNCTION__," Price_NewTrade: ",Price_NewTrade);
    double Price_PendingOrder;
