                ...
                const int &spread[])
{
   ...
   {
      double upValue = iCustom(_Symbol, _Period, "TMA+CG mladen NRP", TimeFrame, HalfLength, Price, BandsDeviations, Interpolate, 3, pos);
      int colorIndex = out.GetColorIndex(pos + 1);
      if (upValue != EMPTY_VALUE)
      {
         colorIndex = 0;
      }
      double downValue = iCustom(_Symbol, _Period, "TMA+CG mladen NRP", TimeFrame, HalfLength, Price, BandsDeviations, Interpolate, 4, pos);
