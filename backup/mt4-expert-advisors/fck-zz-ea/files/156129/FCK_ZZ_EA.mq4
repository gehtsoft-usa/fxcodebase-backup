//Available @  https://fxcodebase.com/code/viewtopic.php?f=38&t=75062
//+------------------------------------------------------------------------------------------------+
...
input const string indicator_file = "FCK_ZZ";                    // Indicator File:
//---
int    bufferToBuy              = 0;
int    bufferToSell             = 1;
bool   indicator_init           = true;
int    candles_back_for_signals = 10;
double zz[4];

double Indi(int buffer, int candle = 1) { return iCustom(NULL, 0, indicator_file, buffer, candle); }

double ZZPoint(int n)
{
    // llenar los valores del zz
    int c = 0;
    int i = 0;
    while (c < 4) {
        double indi = Indi(0, i);
...
class ConditionIndicatorBuy : public iConditions
{
  ...
  public:
    bool evaluate()
    {
        double static lastSignal = ZZPoint(1);        
        if(ZZPoint(0) > ZZPoint(1) && ZZPoint(2) > ZZPoint(1) && ZZPoint(2) > ZZPoint(3) && ZZPoint(0) > ZZPoint(2) && ZZPoint(1) > ZZPoint(3) &&  ZZPoint(1) != lastSignal)
        {
            lastSignal = ZZPoint(1);            
            return true;
        }
