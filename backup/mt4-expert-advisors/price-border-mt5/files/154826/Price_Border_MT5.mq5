//Available @  https://fxcodebase.com/code/viewtopic.php?f=38&t=74733
...
void OnInit()
{
   ...
   IndicatorSetString(INDICATOR_SHORTNAME, "Price Border");
                ...
                const int &spread[])
{
   ...
   for (int pos = MathMax(veryFirst, MathMax(first, prev_calculated - 1)); pos < rates_total; ++pos)
   {
      ...
      if(close[pos-1]>upBuffer[pos-1]) { trend[pos]=-1; }
      if(close[pos-1]<dnBuffer[pos-1]) { trend[pos]=1;  }
      
      if(allowSignal==1 || allowSignal==0)
      if(trend[pos]==1 && upBuffer[pos-2]<upBuffer[pos-1]) { arrowUp[pos-1]=dnBuffer[pos-1]; allowSignal=-1; }
      
      if(allowSignal==-1|| allowSignal==0)
      if(trend[pos]==-1 && dnBuffer[pos-2]>dnBuffer[pos-1]) { arrowDn[pos-1]=upBuffer[pos-1];allowSignal=1; }
   }
   return rates_total;
}
//+------------------------------------------------------------------------------------------------+
