//Available @  https://fxcodebase.com/code/viewtopic.php?f=38&t=74178&p=152633#p152633
...
#define INDICATOR_NAME "MarketProfile"
//+------------------------------------------------------------------+
            ...
            if(Session == Intraday)
              {
               ...
               if((Session == Rectangle) && (SeamlessScrollingMode))  // No point in seamless scrolling mode with rectangle sessions.
                 {
...
// New bars arrived?
   if((EnableDevelopingPOC) && (Bars - IndicatorCounted() > 1) && (CleanedUpOn != Bars))
     {
      // Initialize the indicator buffers.
      for(int i = Bars - IndicatorCounted(); i < Bars; i++)
        {
         DevelopingPOC_1[i] = EMPTY_VALUE;
         DevelopingPOC_2[i] = EMPTY_VALUE;
        }
      CleanedUpOn = Bars; // To prevent cleaning up the buffers again and again when the platform just starts.
     }
   CheckAlerts();
...
// Recalculate everything if there were missing bars or something like that. Or if RightToLeft is on and a new right-most session arrived.
   if((Bars - IndicatorCounted() > 1) || (NeedToRestartDrawing))
     {
      FirstRunDone = false;
      ObjectCleanup();
...
// We begin from the oldest session coming to the current session or to StartFromDate.
   for(int i = SessionToStart; i < _SessionsToCount; i++)
     {
      if(Session == Intraday)
      ...
      else
        {
         ...
         if(!ProcessSession(sessionstart, sessionend, i))
            return 0;
        }
   ...
   if((ShowValueAreaRays != None) || (ShowMedianRays != None) || ((HideRaysFromInvisibleSessions) && (SinglePrintRays)))
      CheckRays();
   FirstRunDone = true;
   Timer = (int)TimeLocal();
   return 0;
                           ...
                           while((x < Bars) && (TimeHour(Time[x]) * 60 + TimeMinute(Time[x]) >= IDEndTime[intraday_i]))
                             {
                              ...
                              if(TimeDayOfWeek(Time[x] + TimeShiftMinutes * 60) > 1)
                                 return FindDayStart(n);
                             }
                           return (x - 1);
                          }
   ...
   while((x < Bars) && (TimeAbsoluteDay(date + TimeShiftMinutes * 60) < TimeAbsoluteDay(Time[x] + TimeShiftMinutes * 60)))
     {
      ...
      if(SaturdaySunday == Append_Saturday_Sunday)
        {
         if(((TimeDayOfWeek(Time[x] + TimeShiftMinutes * 60) == 6) || (TimeDayOfWeek(Time[x] + TimeShiftMinutes * 60) == 1)) && (TimeAbsoluteDay(Time[x] + TimeShiftMinutes * 60) - TimeAbsoluteDay(date + TimeShiftMinutes * 60) == 1))
            break;
        }
      x++;
     }
      ...
      if(SaturdaySunday == Append_Saturday_Sunday)
        {
         ...
         if((time_x_day_of_week == 6) && (TimeDay(Time[x] + TimeShiftMinutes * 60) == 1) && (TimeYear(Time[x] + TimeShiftMinutes * 60) * 12 + TimeMonth(Time[x] + TimeShiftMinutes * 60) - TimeYear(date + TimeShiftMinutes * 60) * 12 - TimeMonth(date + TimeShiftMinutes * 60) == 1))
            break;
         ...
         if((TimeDayOfWeek(date + TimeShiftMinutes * 60) == 0) && (TimeYear(Time[x] + TimeShiftMinutes * 60) * 12 + TimeMonth(Time[x] + TimeShiftMinutes * 60) - TimeYear(date + TimeShiftMinutes * 60) * 12 - TimeMonth(date + TimeShiftMinutes * 60) == 1))
            break;
        }
      x++;
      if(x >= Bars)
      ...
      for(int i = obj_total - 1; i >= 0; i--)
        {
         ...
         if(StringSubstr(obj, 0, StringLen(rectangle_prefix + "MP" + Suffix)) != rectangle_prefix + "MP" + Suffix)
            continue;
         // Previous bar's dot found.
         if(StringSubstr(obj, 0, StringLen(rectangle_prefix + "MP" + Suffix + LastNameStart)) != rectangle_prefix + "MP" + Suffix + LastNameStart)
            break;
         // Change color.
            ...
            if(converted_time < Time[0])
              {
               ...
               converted_time -= PeriodSeconds(); // While beyond the current candle, subtract fixed time periods to move left on the time scale.
        }
   ...
   if(ObjectFind(0, rectangle_prefix + "MP" + Suffix + LastName) >= 0)  // Need to move the rectangle.
     {
      ...
      ObjectCreate(0, rectangle_prefix + "MP" + Suffix + LastName, OBJ_RECTANGLE, 0, time_start, price, time_end, price - onetick);
   ...
   if(enable)
     {
      ...
      if(IDEndTime[IntradaySessionCount] < IDStartTime[IntradaySessionCount])
      ...
      if(SinglePrintRays)
        {
         ...
         if(range == 1)
            ...
            SinglePrintTracking_array[index] = false;
        }
     }
// Single Print Rays
   ...
   if(SinglePrintRays)
     {
      ...
      if((HideRaysFromInvisibleSessions) && (Time[WindowFirstVisibleBar()] >= Time[sessionstart]))
         ...
         if(SaturdaySunday == Ignore_Saturday_Sunday)
           {
            ...
            // Day start is on Friday.
               ...
               if(TimeDayOfWeek(Time[remember_sessionstart] + TimeShiftMinutes * 60) == 5)
                 {
      ...
      if(SaturdaySunday == Append_Saturday_Sunday)
        {
         ...
         // The intraday session ends on 00:00 or otherwise captures midnight, and remember_sessionstart points to Friday:
            ...
            if(((IDEndTime[intraday_i] == 24 * 60) || (IDStartTime[intraday_i] > IDEndTime[intraday_i])) && (TimeDayOfWeek(Time[remember_sessionstart] + TimeShiftMinutes * 60) == 5))
              {
            ...
            if(IDStartTime[intraday_i] > IDEndTime[intraday_i])
              {
               ...
               // If Ignore_Saturday_Sunday is on and it is Monday before the session's end:
                  ...
                  if((SaturdaySunday == Ignore_Saturday_Sunday) && (TimeDayOfWeek(Time[0] + TimeShiftMinutes * 60) == 1) && (time < IDEndTime[intraday_i]))
                    {
                     // Then we can use remember_sessionstart as the session's start.
                     sessionstart = remember_sessionstart;
                    }
         ...
         if(IDStartTime[intraday_i] < IDEndTime[intraday_i])
           {
            ...
            // Intraday session starts after the today's actual session ended (for Friday/Saturday cases).
               ...
               if(TimeHour(Time[remember_sessionend] + TimeShiftMinutes * 60) * 60 + TimeMinute(Time[remember_sessionend] + TimeShiftMinutes * 60) < IDStartTime[intraday_i])
                  continue;
            ...
            // Intraday session ends before the today's actual session starts (for Sunday cases).
               ...
               if(TimeHour(Time[remember_sessionstart] + TimeShiftMinutes * 60) * 60 + TimeMinute(Time[remember_sessionstart] + TimeShiftMinutes * 60) >= IDEndTime[intraday_i])
                  continue;
            ...
            // If Append_Saturday_Sunday is on and the session starts on 00:00 and the session end points to Sunday (it is current Sunday session , no Monday bars yet):
               ...
               if((SaturdaySunday == Append_Saturday_Sunday) && (IDStartTime[intraday_i] == 0) && (TimeDayOfWeek(Time[sessionend] + TimeShiftMinutes * 60) == 0))
                 {
               ...
               // Otherwise find the session end.
                  ...
                  while((sessionend < Bars) && ((TimeHour(Time[sessionend] + TimeShiftMinutes * 60) * 60 + TimeMinute(Time[sessionend] + TimeShiftMinutes * 60) >= IDEndTime[intraday_i]) || ((TimeDayOfWeek(Time[sessionend] + TimeShiftMinutes * 60) == 6) && (SaturdaySunday == Append_Saturday_Sunday))))
                    {
                     sessionend++;
                    }
            if(sessionend == Bars)
            ...
            if(IDStartTime[intraday_i] > IDEndTime[intraday_i])
              {
               ...
               // If Ignore_Saturday_Sunday is on and it is Monday: (and it is not a special additional intra-Monday session.)
                  ...
                  if((SaturdaySunday == Ignore_Saturday_Sunday) && (TimeDayOfWeek(Time[remember_sessionstart] + TimeShiftMinutes * 60) == 1) && (!ContinuePreventionFlag))
                    {
                     // Then we can use remember_sessionstart as the session's start.
                     sessionstart = remember_sessionstart;
               ...
               // If Append_Saturday_Sunday is on and the start points to Sunday, use a simple method to find the end.
                  ...
                  // If Ignore_Saturday_Sunday is on and the session starts on Friday:
                     ...
                     if((SaturdaySunday == Ignore_Saturday_Sunday) && (TimeDayOfWeek(Time[remember_sessionstart] + TimeShiftMinutes * 60) == 5))
                       {
         ...
         for(int j = 0; j < obj_total; j++)
           {
            ...
            if(StringSubstr(obj_name, 0, StringLen(rec_name + "MPSPR" + suffix + last_name)) != rec_name + "MPSPR" + suffix + last_name)
         ...
         if(!((HideRaysFromInvisibleSessions) && (Time[WindowFirstVisibleBar()] >= median_time)))
           {
            ...
            ObjectDelete(0, rec_name + "Median Ray" + suffix + last_name); // Delete the ray that starts from behind the screen.
        }
            ...
            for(int j = 0; j < obj_total; j++)
              {
               ...
               if(StringSubstr(obj_name, 0, StringLen(obj_prefix)) != obj_prefix)
                  continue; // Not a Single Print ray (or not this seesion's).
         ...
         if(AlertForValueArea)
           {
            ...
            if(ObjectFind(ChartID(), obj_prefix) >= 0)  // Exists and visible.
              {
               if(!FindAtLeastOneArrowForRay(obj_prefix))
                  CheckHistoricalArrowsForNonMPSPRRays(bar_start, obj_prefix);
              }
            else
              {
               DeleteArrowsByPrefix(obj_prefix); // Delete all arrows generated by this ray.
   ...
   for(int i = 0; i < objects_total; i++)
     {
      ...
      if(StringFind(obj_name, ray_name) != -1)
         return true; // Found an rrrow for a given ray.
     }
         ...
         if(CurrentBarDirection == Bullish)
           {
            ...
            if(CurrentBarDirection == Bearish)
              {
               colour = clrDarkRed;
              }
            else
               if(CurrentBarDirection == Neutral)
                 {
                  colour = clrPink;
                 }
         break;
...
void OnTimer()
  {
   if(GetTickCount() - LastRecalculationTime < 500)
      ...
      if(!name_found)
        {
         ...
         int k = 0;
         while((k < mpr_total) && (MPR_Array[k].RectangleTimeMin < t))
            k++;
         // Now k points either to the first newer rectangle or to beyond the end of the array.
         mpr_total++;
         ArrayResize(MPR_Array, mpr_total);
      ...
      if(rectangle_price_changed)
        {
         ...
         if((max_index != -1) && (min_index != -1))
           {
            if((RectanglePriceMax > High[max_index]) && (RectanglePriceMin < Low[min_index]) && (prev_RectanglePriceMax > High[max_index]) && (prev_RectanglePriceMin < Low[min_index]))
...
// Used only for Arrows:
   if(Time[0] < RectangleTimeMax)
      RememberSessionEnd[i] = Time[0];
   else
      RememberSessionEnd[i] = RectangleTimeMax;
   if((!new_bars_are_not_within_rectangle) || (current_bar_changed_within_boundaries) || (rectangle_changed_and_recalc_is_due) || ((Number != i) && ((RaysUntilIntersection != Stop_No_Rays) && ((ShowMedianRays != None) || (ShowValueAreaRays != None)))))
      ProcessSession(sessionstart, sessionend, i, &this);
   Number = i;
  }
...
// If already there - ignore.
   if(ObjectFind(0, rectangle_prefix + "MPSP" + Suffix + LastName) >= 0)
      return;
...
void PutSinglePrintRay(const double price, const int sessionstart, const string rectangle_prefix, const color spr_color)
  {
   ...
   string LastName = LastNameStart + DoubleToString(price, _Digits);
// If already there - ignore.
   if(ObjectFind(0, rectangle_prefix + "MPSPR" + Suffix + LastName) >= 0)
      return;
   ...
   for(int max_bar = sessionstart; max_bar >= sessionend; max_bar--)
     {
      ...
      // Buffer #1 already had a value,
         ...
         if(DevelopingPOC_1[max_bar + 1] != EMPTY_VALUE)
           {
...
// No need to check further if no alert method is chosen.
   if((!AlertNative) && (!AlertEmail) && (!AlertPush) && (!AlertArrows))
      return;
...
// Skip alerts if only closed bar should be checked and it has already been done.
   if((AlertCheckBar == CheckPreviousBar) && (LastAlertTime == Time[0]))
      return;
   ...
   for(int i = 0; i < obj_total; i++)
     {
      ...
      if(!(((AlertForMedian) && (StringFind(object_name, "Median Ray") > -1)) ||
           ...
           ((AlertForSinglePrint) && (StringFind(object_name, "MPSPR") > -1) && ((color)ObjectGetInteger(ChartID(), object_name, OBJPROP_COLOR) != clrNONE))))
         continue;
            ...
            if((Close_prev != EMPTY_VALUE) && (((Close[0] >= level) && (Close_prev < level)) || ((Close[0] <= level) && (Close_prev > level))))
              {
               ...
               if(AlertArrows)
                  CreateArrowObject("ArrPB" + object_name, Time[0], Close[0], AlertArrowCodePB, AlertArrowColorPB, AlertArrowWidthPB, "Price Break");
              }
            Close_prev = Close[0];
           }
         if(AlertOnCandleClose)  // Candle close alerts.
            ...
            if(((Close[0] >= level) && (Close[1] < level)) || ((Close[0] <= level) && (Close[1] > level)))
              {
               ...
               if(AlertArrows)
                  CreateArrowObject("ArrCC" + object_name, Time[0], Close[0], AlertArrowCodeCC, AlertArrowColorCC, AlertArrowWidthCC, "Candle Close");
              }
           }
         if(AlertOnGapCross)  // Gap cross alerts.
           {
            if(((Open[0] > level) && (High[1] < level)) || ((Open[0] < level) && (Low[1] > level)))
              {
               ...
               if(AlertArrows)
                  CreateArrowObject("ArrGC" + object_name, Time[0], level, AlertArrowCodeGC, AlertArrowColorGC, AlertArrowWidthGC, "Gap Cross");
              }
           }
        }
      // Price breaks (using pre-previous High and previous Close), candle closes, and gap crosses using Close[1].
      else
         if(AlertCheckBar == CheckPreviousBar)
           {
            if(AlertOnPriceBreak)  // Price break alerts.
              {
               if(((High[1] >= level) && (Close[1] < level) && (Close[2] < level)) || ((Low[1] <= level) && (Close[1] > level) && (Close[2] > level)))
                 {
                  DoAlerts(PriceBreak, object_name);
                  if(AlertArrows)
                     CreateArrowObject("ArrPB" + object_name, Time[1], Close[1], AlertArrowCodePB, AlertArrowColorPB, AlertArrowWidthPB, "Price Break");
                 }
              }
            if(AlertOnCandleClose)  // Candle close alerts.
              {
               if(((Close[1] >= level) && (Close[2] < level)) || ((Close[1] <= level) && (Close[2] > level)))
                 {
                  DoAlerts(CandleCloseCrossover, object_name);
                  if(AlertArrows)
                     CreateArrowObject("ArrCC" + object_name, Time[1], Close[1], AlertArrowCodeCC, AlertArrowColorCC, AlertArrowWidthCC, "Candle Close");
                 }
              }
            if(AlertOnGapCross)  // Gap cross alerts.
              {
               if(((Low[1] > level) && (High[2] < level)) || ((Low[2] > level) && (High[1] < level)))
                 {
                  DoAlerts(GapCrossover, object_name);
                  if(AlertArrows)
                     CreateArrowObject("ArrGC" + object_name, Time[1], level, AlertArrowCodeGC, AlertArrowColorGC, AlertArrowWidthGC, "Gap Cross");
                 }
              }
...
// Remember that this alert has already been sent. For CheckPreviousBar, this is done in CheckAlerts().
   if((alert_type == CandleCloseCrossover) && (AlertCheckBar == CheckCurrentBar))
      ...
      if((alert_type == GapCrossover) && (AlertCheckBar == CheckCurrentBar))
