...
#property indicator_label1 "Smoothed Volume Weighted Moving Average"

enum MA_Types{ SMA=1,  EMA=2, SMMA=3,LWMA=4 };
 
input  MA_Types MA_Type = EMA;
...
int init()
{
   ...
   IndicatorName = GenerateIndicatorName("Smoothed Volume Weighted Moving Average");
...
int start()
{
   ...
   {  
		...
		VSum =iMAOnArray(VolumeArray,0,Volume_MA_Period,0,(MA_Type-1),pos);
