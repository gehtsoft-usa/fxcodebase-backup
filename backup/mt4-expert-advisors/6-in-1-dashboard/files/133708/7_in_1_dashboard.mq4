...
input int    Period_HL = 21; // Mega RK Period HL
...
public:
   ...
   {
      double value = iCustom(_symbol, _timeframe, "1-RSI", GetTimeframeStr(), RsiPeriod, MaType, MaPeriod, Interpolate, false, "", "", 0, 0, "", false, false, "", false, "", "", "", false, 0, period);
      return value < iClose(_symbol, _timeframe, period);
   }
};
...
public:
   ...
   virtual bool IsPass(const int period, const datetime date)
   {
      double value = iCustom(_symbol, _timeframe, "Mega_rk BTF", _timeframe, Lime, Red, Period_HL, false, 0, period);
      return value > 0;
   }
};
...
public:
   ...
   virtual bool IsPass(const int period, const datetime date)
   {
      double value = iCustom(_symbol, _timeframe, "Mega_rk BTF", _timeframe, Lime, Red, Period_HL, false, 1, period);
      return value < 0;
   }
};
...
int init()
{
   ...
   temp = iCustom(NULL, 0, "Mega_rk BTF", 0, Lime, Red, Period_HL, false, 0, 0);
   if (GetLastError() == ERR_INDICATOR_CANNOT_LOAD)
   {
      Alert("Please, install the 'Mega_rk BTF' indicator");
      return INIT_FAILED;
   }
