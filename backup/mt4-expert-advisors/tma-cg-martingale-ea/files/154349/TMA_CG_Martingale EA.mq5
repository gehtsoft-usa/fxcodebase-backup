...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=74609
...
string file_custom_indicator = "TMA_CG_mladen";
int    handle                = 0;
int    handle2               = 0;
...
void setHandle()
{
  handle = iCustom(Symbol(), TF1, file_custom_indicator,
                   indi_HalfLength,indi_Price,indi_BandsDeviations);
}
void setHandle2()
{
  handle2 = iCustom(Symbol(), TF2, file_custom_indicator,
                   indi_HalfLength,indi_Price,indi_BandsDeviations);
}

double indi_tf1(int buffer, int shift)
{
  double value[1];
...
double indi_tf2(int buffer, int shift)
{
  double value[1];
  int    copy = CopyBuffer(handle2, buffer, shift, 1, value);
...
class BUYcondition1 : public iConditions
{
 public:
  bool evaluate()
  {
    ...
    return iLow(_Symbol, TF1, 1) < indi_tf1(bufferToBuy, 1); 
        // && iClose(_Symbol, TF1, 2) < iOpen(_Symbol, TF1, 2) 
        // && iClose(_Symbol, TF1, 1) > iOpen(_Symbol, TF1, 1);
  }
};
...
class SELLcondition1 : public iConditions
{
 public:
  bool evaluate()
  {
    ...
    return iHigh(_Symbol, TF1, 1) > indi_tf1(bufferToSell, 1);
        // && iClose(_Symbol, TF1, 2) > iOpen(_Symbol, TF1, 2) 
        // && iClose(_Symbol, TF1, 1) < iOpen(_Symbol, TF1, 1);
