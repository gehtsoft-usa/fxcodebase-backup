...
// ------------------------------------------------------------------
string file_custom_indicator = "^Pivot_PP.ex4";
...
input double extraPoints = 10; // Extra Points to Breakout:
...
input bool closeWhenNewDayOn = false; // Close all at the end of the day:
...
bool CloseCandleMode = closeCandleMode;  // meter en la clase CloseCandle
...
class CNewCandle
{
    ...
    CNewCandle(string symbol, ENUM_TIMEFRAMES tf)
    {
        m_symbol = symbol;
        m_tf = tf;
        velasInicio = iBars(m_symbol, m_tf);
        Print(__FUNCTION__, " m_symbol: ", m_symbol);
        Print(__FUNCTION__, " m_tf: ", m_tf);
        Print(__FUNCTION__, " velasInicio: ", velasInicio);
    };
    ~CNewCandle() { ; }
    ...
    bool IsNewCandle()
    {
        ...
        Print(__FUNCTION__, " velasActuales: ", velasActuales);
...
// {
//     res2 = iCustom(NULL, 0, file_custom_indicator, Formula, 0, 0);
//     res1 = iCustom(NULL, 0, file_custom_indicator, Formula, 1, 0);
//     pivot0 = iCustom(NULL, 0, file_custom_indicator, Formula, 2, 0);
//     sop1 = iCustom(NULL, 0, file_custom_indicator, Formula, 3, 0);
//     sop2 = iCustom(NULL, 0, file_custom_indicator, Formula, 4, 0);
...
//     // Print(__FUNCTION__," res2: ",res2);
//     // Print(__FUNCTION__," res1: ",res1);
//     // Print(__FUNCTION__," pivot0: ",pivot0);
//     // Print(__FUNCTION__," sop1: ",sop1);
//     // Print(__FUNCTION__," sop2: ",sop2);

//     SetDailyDirection();
// }

// void SetDailyDirection()
// {
//     double dailyOpen = iOpen(NULL, PERIOD_D1, 0);
//     direction = dailyOpen < pivot0 ? 1 : 0;
//     Print(__FUNCTION__, " dir: 1-alcista 0-bajista", direction);
// }
...
class BUYcondition1 : public iConditions
{
    ...
    bool evaluate()
    {
        ...
        // esta formula sería en live
        // if (Ask > pivot0 && Bid < pivot0) return true;

        // esta formula para candle close

        // for (int i = 0; i < ArraySize(indiLevels); i++)
        // {
        //     if (iClose(NULL, 0, 1) > indiLevels[i] && iOpen(NULL, 0, 1) < indiLevels[i]){ return true;}
        // }

        pivot0 = iCustom(NULL, 0, file_custom_indicator, Formula, 0, 0);
        double pivotShifted =  pivot0 + (extraPoints * _Point);
        if(iClose(NULL, 0, 1) > pivotShifted && iOpen(NULL, 0, 1) < pivot0) { return true; }
...
class ConditionToCloseBuy : public iConditions
{
    ...
    bool evaluate()
    {
        ...
        if (closeWhenNewDayOn)
            if (day.isnewday()) { return true; }
...
int OnInit()
{
    ...
    D1candle = new CNewCandle(Symbol(), PERIOD_D1);
