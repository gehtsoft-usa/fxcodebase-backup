//Available @ https://fxcodebase.com/code/viewtopic.php?f=38&t=74052
...
string file_custom_indicator = "Daily_Currency_Strength_Indicator.ex4";
...
input	ENUM_TIMEFRAMES	Indi_TimeFrame = PERIOD_D1;
input double level1 = 0.1; // Level 1:
input double level2 = 0.2; // Level 2:
input double level3 = 0.3; // Level 3:
...
bool   filterSymbolsOn = false;                    // Symbols filter On:
...
class BUYcondition1 : public iConditions
{
    ...
    bool evaluate()
    {
        ...
        int BuySim = SimbolToBuy();
        int SellSim = SimbolToSell();

        if(BuySim > 0 && SellSim > 0)
        {
            Print("BUY: ", MySymbol(BuySim, SellSim));
            SymToBuy = MySymbol(BuySim, SellSim);
            return true;
        }
...
class SELLcondition1 : public iConditions
{
    ...
    bool evaluate()
    {
        ...
        if(BuySim > 0 && SellSim > 0)
        {
            SymToSell = MySymbol(SellSim, BuySim);
            Print("SELL: ", MySymbol(SellSim, BuySim));
        }
...
void OnTick()
{
    ...
    if(!uTradeReverse)
    {
        if(conditionsToBuy.EvaluateConditions())
        {
            if(modeEntry == Market)
            {
                actionSendOrder = new SendNewOrder("buy", Lots(), SymToBuy, 0, SL("buy"), TP("buy"), magico);
            }
            else
            {
        ...
        if(conditionsToSell.EvaluateConditions())
        {
            if(modeEntry == Market)
            {
                actionSendOrder = new SendNewOrder("sell", Lots(), SymToSell, 0, SL("sell"), TP("sell"), magico);
            }
            else
            {
...
int SimbolToBuy()
{
    for(int i = 0; i < 7; i++)
    {
        Print("last Indi Pair: shift 1: ",CurrencyIndex_List[i]," ", Indi(i, 1));
        Print("last Indi Pair: shift 2: ", CurrencyIndex_List[i], " ", Indi(i, 2));
        
        if(Indi(i, 1) > level3 && Indi(i, 2) < level3)
        {
            return i;
        }
    }
...
string MySymbol(int a, int b)
{
    string sym;
    sym = CurrencyIndex_List[a] + CurrencyIndex_List[b];
