...
// http://fxcodebase.com/code/viewtopic.php?f=38&t=70642
//+------------------------------------------------------------------+
...
input bool backtest_mode = false; // Backtest mode
...
{
public:
   EntryLongCondition(const string symbol, ENUM_TIMEFRAMES timeframe)
      :ACondition(symbol, timeframe)
   {
   ...
   bool IsPass(const int period, const datetime date)
   {
      for (int k = ObjectsTotal(0); k >= 0; k--)
      {
         ...
         if (time >= date && StringFind(name, "YBM_Signaler_ arr b") == 0)
         {
            return true;
         }
      }
...
{
public:
   EntryShortCondition(const string symbol, ENUM_TIMEFRAMES timeframe)
      :ACondition(symbol, timeframe)
   {
   ...
   bool IsPass(const int period, const datetime date)
   {
      for (int k = ObjectsTotal(0); k >= 0; k--)
      {
         string name = ObjectName(0, k);
         datetime time = (datetime)ObjectGetInteger(0, name, OBJPROP_TIME);
         if (time >= date && StringFind(name, "YBM_Signaler_ arr s") == 0)
         {
            return true;
         }
      }
...
int OnInit()
{
   if (backtest_mode)
   {
      indi = iCustom(_Symbol, _Period, "YBM_Signaller");
   }
   string error;
   string sym_arr[];
   sym_arr[0] = _Symbol;
