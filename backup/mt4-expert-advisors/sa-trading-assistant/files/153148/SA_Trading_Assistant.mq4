...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=74311
    ...
    bool doAction()
    {
        for(int i = OrdersTotal() - 1; i >= 0; i--)
        {
            if(OrderSelect(i, SELECT_BY_POS) && OrderSymbol() == _symbol && OrderMagicNumber() == _magic)
            {
                ...
                if(_profitState == "all" && OrderType() != OP_BUY && OrderType() != OP_SELL)
                {
...
class BuyAction : public iActions
{
    // abre el buy original, y agregar el buy auxiliar y el sell auxiliar
    public:
    bool doAction()
    {
        SendNewOrder* SendOrder;

        // BUY Original:        (acomodar que cuando mando a mercado me la tome)
        double pr = Price("buy", 0, (double) gui.editPrice.Text());
        
        if(gui.comboTypes.Value() == 1)
        {
            pr = Ask;
            SendOrder = new SendNewOrder("buy", Lots(), "", 0, SL("buy", pr, userSLpips), TP("buy", pr, userTPpips), magico, dayComent(), expiration());
        } else {
            SendOrder = new SendNewOrder("buy", Lots(), "", pr, SL("buy", pr, userSLpips), TP("buy", pr, userTPpips), magico, dayComent(), expiration());
        }
        bool result = SendOrder.doAction();
        delete SendOrder;

        // // BUY Auxiliar:
        // int distance = (int) gui.edit1.Text();
        // double pr2 = Price("buy", distance, pr);
        // SendOrder = new SendNewOrder("buy", Lots(), "", pr2, SL("buy", pr2, userSLpips2), TP("buy", pr2, userTPpips2), magico);
        // result = SendOrder.doAction();
        // delete SendOrder;

        // // SELL Auxiliar:
        // distance = (int) gui.edit2.Text();
        // double pr3 = Price("sell", distance, pr);
        // SendOrder = new SendNewOrder("sell", Lots(), "", pr3, SL("sell", pr3, userSLpips2), TP("sell", pr3, userTPpips3), magico);
        // result = SendOrder.doAction();
        // delete SendOrder;
...
BuyAction btnBuyAction();

class SellAction : public iActions
{
    public:
    bool doAction()
    {
        ...
        // SELL Original:        
        double pr = Price("sell", 0, (double) gui.editPrice.Text());
        if(gui.comboTypes.Value() == 1){
            pr = Bid;
            SendOrder = new SendNewOrder("sell", Lots(), "", 0, SL("sell", pr, userSLpips), TP("sell", pr, userTPpips), magico, dayComent(), expiration());
        } else {
            SendOrder = new SendNewOrder("sell", Lots(), "", pr, SL("sell", pr, userSLpips), TP("sell", pr, userTPpips), magico, dayComent(), expiration());
        }
        bool result = SendOrder.doAction();
        delete SendOrder;
...
// NOTE: refresh    
    void RefreshPrice()
    {
        if(comboTypes.Value() == 1)
        {
            string tx = (string) Price("buy", 0, Ask);
            editPrice.Text(tx);
        }
    }
    ...
    CLabel labelPrice;
    CButton bt1, bt2, bt3;
    CButton btBuy, btSell;
    CEdit   edit1, edit2, editPrice;
    CComboBox comboTypes;
    ...
    bool Create(const long chart, const string name, const int subwin, const int x1, const int y1, const int x2, const int y2)
    {
        ...
        if(!Create_button("BUY", Col(0), Row(2), _high, _width, btBuy)) return false;
        if(!Create_label("pips buy", Col(2), Row(2), _high, _width, lb1, "pips buy")) return false;
        if(!Create_button("SELL", Col(0), Row(3), _high, _width, btSell)) return false;
        if(!Create_label("pips sell", Col(2), Row(3), _high, _width, lb2, "pips sell")) return false;
        if(!Create_Edit("Pips buy", Col(1), Row(2), _high, _width, edit1)) return false;
        if(!Create_Edit("Pips sell", Col(1), Row(3), _high, _width, edit2)) return false;

        if(!Create_button("Close Win", Col(0), Row(5), _high, _width, bt1)) return false;
        if(!Create_button("Close Loss", Col(1), Row(5), _high, _width, bt2)) return false;
        if(!Create_button("Close All", Col(2), Row(5), _high, _width, bt3)) return false;

        if(!Create_combo("comboType", Col(0), Row(0), _high + 5, _width, comboTypes)) return false;
        if(!Create_Edit("editPrice", Col(1), Row(0), _high + 5, _width, editPrice)) return false;

        // if(!Create_label("labelPrice", Col(2), Row(0),_high+5, _width, labelPrice, "Price")) return false;
        // if (!Create_label("loss",       Col(1), Row(1), _high, _width, lb2)) return false;
        // if (!Create_label("all" ,       Col(1), Row(2), _high, _width, lb3)) return false;
        // if (!Create_label("% win",      Col(2)-20, Row(0), _high, _width, lb4)) return false;
        // if (!Create_label("% loss",     Col(2)-20, Row(1), _high, _width, lb5)) return false;
        // if (!Create_label("% all",      Col(2)-20, Row(2), _high, _width, lb6)) return false;


            // if (!Create_label("Expire Days: ",      Col(0), Row(4), _high, _width, lb7)) return false;
            // if (!Create_label("days",      Col(1), Row(4), _high, _width, lb8)) return false;
            // if (!Create_label("PhoneNumber",    Col(0), Row(5), _high, _width, lb9)) return false;
    ...
    void HoverEvents(const int id, const long& lparam, const double& dparam, const string& sparam)
    {
        ...
        if(btSell.IsActive()) btSell.ColorBackground(Crimson);  else btSell.ColorBackground(CONTROLS_BUTTON_COLOR_BG);
    ...
    bool Create_combo(string name, int x1, int y1, int high, int width, CComboBox& combo)
    {
        int x2 = x1 + width;
        int y2 = y1 + high;

        if(!combo.Create(m_chart_id, name, 0, x1, y1, x2, y2))
            Print("cant create the comboBox, Error:", GetLastError());
    ...
    bool Create_Edit(string name, const int x1, const int y1, const int high, const int width, CEdit& ed)
    {
        ...
        if(name == "editPrice")
        {
            tx = (string) Price("buy", (int) edit1.Text(), Ask);
        }
        ed.Text(tx);
        ed.Font("Calibri");
        ed.FontSize(10);
...
EVENT_MAP_BEGIN(GUI)
ON_EVENT(ON_CLICK, btBuy, btnBuyAction.doAction)
ON_EVENT(ON_CLICK, btSell, btnSellAction.doAction)
ON_EVENT(ON_CLICK, bt1, button1Action.doAction)
ON_EVENT(ON_CLICK, bt2, button2Action.doAction)
ON_EVENT(ON_CLICK, bt3, button3Action.doAction)
...
#endif
input int          magico = 2023;               // Magic Number:
input bool oneByCandle = true; // One Trade By Candle?
...
input int          userTPpips  = 10;                 // Pips TP1
input int          userTPpips2 = 30;                 // Pips TP2
input int          userTPpips3 = 50;                 // Pips TP3
...
input int          userSLpips = 30;                 // Pips SL 1:
input int          userSLpips2 = 20;                 // Pips SL 2:
...
input int    userTslInitialStep = 5;                           // TSL Initial Step:
...
input LotMode lotMode = aditionMode;       // Mode to Increase Volumen:
...
input double uAditioner = 0.01;            // Lots To Addition:
...
bool   filterMagicsOn = false;                    // Use magic number filter?
...
class Grid
{
    ...
    public:
    Grid(string Symbol, string Side, double LastPrice, double Gap, double Multiplier, int MaxQnt, double MaxLot, double InitialLot, int magic, bool simbolFilterOn = true, bool magicFilterOn = true, double aditioner = 0)
    {
        _symbol = Symbol;
        _side = Side;
    ...
    double nextLot(void)
    {
        switch(lotMode)
        {
            case multiplierMode: return NormalizeDouble(_initialLot * pow(_multiplier, qnt()), 2);
            case aditionMode: return NormalizeDouble(_initialLot + (_aditioner * qnt()), 2);
        }
...
bool CloseCandleMode = oneByCandle;  // meter en la clase CloseCandle
...
class BUYcondition1 : public iConditions
{
    ...
    bool evaluate()
    {
        ...
        //   return Indi(bufferToBuy, 1) != EMPTY_VALUE;
        return IsTesting() && OrdersTotal() == 0;
        //   return false;
    }
};
...
class SELLcondition1 : public iConditions
{
    ...
    bool evaluate()
    {
        ...
        // return Indi(bufferToSell, 1) != EMPTY_VALUE;
        return false;
    }
};
...
int OnInit()
{
    ...
    if(i_reason != REASON_CHARTCHANGE && i_reason != REASON_TEMPLATE && i_reason != REASON_PARAMETERS)
    {
        ...
        int rows = 8;
        int rowSize = 18;
        int high = 20 + (rows * rowSize);

        int colums = 4;
        int columnSize = 75;
        int width = colums * columnSize;

        gui.Create(0, "SA Trading Assistant", 0, x1, y1, x1 + width, y1 + high);
        gui.Run();
    }
    gui.setButton1Action(closeWinners = new ActionCloseByProfit("win", magico));
    gui.setButton2Action(closeLossers = new ActionCloseByProfit("loss", magico));
    gui.setButton3Action(closeAll = new ActionCloseByProfit("all", magico));
    ...
    if(!uTradeReverse)
    {
        if(conditionsToBuy.EvaluateConditions())
        {
            if(modeEntry == Market)
            {
                ...
                string com = day(dt.day_of_week) +"("+(string)CountTrades()+")";
                Print(__FUNCTION__," com: ",com);
                actionSendOrder = new SendNewOrder("buy", Lots(), "", 0, SL("buy"), TP("buy"), magico, com);
            }
            else
            {
            ...
            if(actionSendOrder.doAction())
            {
                ...
                if(GridON == true && CheckPointer(gridBuy) == POINTER_INVALID)
                {
                    gridBuy = new Grid(_Symbol, "buy", mainOrders.last().price(), GridUser_gap, GridUser_multiplier, GridUser_maxCount, GridUser_maxLot, mainOrders.last().lot(), magico, true, true, uAditioner);
        ...
        if(conditionsToSell.EvaluateConditions())
        {
            ...
            if(actionSendOrder.doAction())
            {
                ...
                if(GridON == true && CheckPointer(gridSell) == POINTER_INVALID)
                {
                    gridSell = new Grid(_Symbol, "sell", mainOrders.last().price(), GridUser_gap, GridUser_multiplier, GridUser_maxCount, GridUser_maxLot, mainOrders.last().lot(), magico, true, true, uAditioner);
...
double Price(string direction, int pips, double originalPrice)
{
    if (originalPrice==0) { return -1; }
    ...
    if(direction == "buy")
    {
        // double ask = SymbolInfoDouble(_symbol, SYMBOL_ASK);
        // double price = distance == 0 ? ask : ask + distance;
        double price = originalPrice - distance;
...
// NOTE: stop loss
double SL(string side, double price = 0, int pips = 0)
{
    pips==0? userSLpips : pips;
    double result = 0;
    if(stopLossOn)
        switch(modeSL)
        {
...
// NOTE: take profit
double TP(string side, double price = 0, int pips=0)
{
    double result = 0;
    pips == 0 ? userTPpips : pips;
    if(takeProfitOn)
        switch(modeTP)
        {
            case FixPips:
...
void HandleTP()
{
    for(int i = 0; i < mainOrders.qnt(); i++)
    {
        ...
        // Print("Handle tp: ", mainOrders.index(i).id());
        int _id = mainOrders.index(i).id();

        OrderSelect(_id, SELECT_BY_TICKET);
        // if (mainOrders.index(i).tp() != newTPbuy)

        if(OrderTakeProfit() != newTPbuy)
        {
            if(mainOrders.index(i).type() == OP_BUY)
            {
                if(ModifyOrder(_id, newTPbuy, 0))
                {
                    mainOrders.index(i).tp(newTPbuy);
                }
            }

            if(mainOrders.index(i).type() == OP_SELL)
            {
                if(ModifyOrder(_id, newTPsell, 0))
                {
                    mainOrders.index(i).tp(newTPsell);
                }
            }
        }
...
bool ModifyOrder(int id, double newTP, double newSL)
{
    double _tp = newTP;
    double _sl = newSL;
    ...
    {
        if(_tp == 0) _tp = OrderTakeProfit();
        if(_sl == 0) _sl = OrderStopLoss();
    }
    return OrderModify(id, OrderOpenPrice(), _sl, _tp, OrderExpiration(), 0);
}
...
double BuyAveragePrice()
{
    ...
    if(_Digits == 5 || _Digits == 3) PipAdjust = 10;
    else if(_Digits == 4 || _Digits == 2) PipAdjust = 1;
...
string day(const int nr)
{
    if(nr == 0) return "Synday";
    if(nr == 1) return "Monday";
    if(nr == 2) return "Tuesday";
    if(nr == 3) return "Wednesday";
    if(nr == 4) return "Thursday";
    if(nr == 5) return "Friday";
...
datetime expiration()
{
    datetime exp = iTime(NULL, PERIOD_D1, 0) + (24*60*60);
    return exp;
}
