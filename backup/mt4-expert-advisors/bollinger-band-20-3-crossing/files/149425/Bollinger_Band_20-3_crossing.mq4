...
string file_custom_indicator = "bbsqueezemodified.ex4";
...
input ModeEntry modeEntry         = PendingLimit;                           // Mode Entry:
...
input string             Tsqueeze       = "== Squeeze Setup ==";  // ————————————
input int       bolPrd=20;
input double    bolDev=2.0;
input int       keltPrd=18;//20
input double    keltFactor=1.8;//1.5
input int       momPrd=14;//12


input string             TBB              = "== Bands Setup ==";  // ————————————
input int                bb1_period       = 20;                   // Band 1: averaging period
input double             bb1_deviation    = 3;                    // Band 1: standard deviations
int                      bb1_bands_shift  = 0;                    // Band 1: bands shift
input ENUM_APPLIED_PRICE bb1_AppliedPrice = PRICE_TYPICAL;        // Band 1: Applied Price
input int                bb2_period       = 20;                   // Band 2: averaging period
input double             bb2_deviation    = 2;                    // Band 2: standard deviations
int                      bb2_bands_shift  = 0;                    // Band 2: bands shift
input ENUM_APPLIED_PRICE bb2_AppliedPrice = PRICE_TYPICAL;        // Band 2: Applied Price
...
input ModeLevels_TP   modeTP      = tp_ByBand;            // Mode Take Profit:
...
input ModeLevels_SL   modeSL      = PipsFromOpenCandle; // Mode Stop Loss:
input int          userSLpips     = 5;                  // Pips SL
 ...
 public:
  bool evaluate()
  {
    // TODO: condition Buy 1
		// double bb2_prev = iBands(NULL, 0, bb2_period, bb2_deviation, bb2_bands_shift, bb2_AppliedPrice, 1, 1);
    double bb1_prev = iBands(NULL, 0, bb1_period, bb1_deviation, bb1_bands_shift, bb1_AppliedPrice, 2, 1);    
    return iLow(NULL, 0, 1) < bb1_prev;
...
class BUYcondition2 : public iConditions
{
 public:
  bool evaluate()
  {
    ...
    return iCustom(NULL, 0, file_custom_indicator, bolPrd, bolDev, keltPrd, keltFactor, momPrd, 2, 1) != EMPTY_VALUE;

    // return Ask > bb1 && Ask < bb2;
    // return false;
  }
};
...
class SELLcondition1 : public iConditions
{
 public:
  bool evaluate()
  {
		...
		double bb1_prev = iBands(NULL, 0, bb1_period, bb1_deviation, bb1_bands_shift, bb1_AppliedPrice, 1, 1);    
    return iHigh(NULL, 0, 1) > bb1_prev;
  }
};
 ...
 public:
  bool evaluate()
  {
    // TODO: condition sell 2
		...
		// double bb1 = iBands(NULL, 0, bb1_period, bb1_deviation, bb1_bands_shift, bb1_AppliedPrice, 1, 0);    
		// double bb2 = iBands(NULL, 0, bb2_period, bb2_deviation, bb2_bands_shift, bb2_AppliedPrice, 1, 0);
    // return Bid < bb1 && Bid > bb2;
...
void OnTick()
{
  ...
  if (uTradeReverse)
  {
	...
	if(modeTP==tp_ByBand) { handleTP_ByBands(); }
}
    ...
    switch (modeTP)
    {
      ...
      case tp_PipsFromOpenCandle:
        ...
        result  = levelSL.calculateLevel();
				break;
      case tp_ByBand:
        result  = iBands(NULL, 0, bb1_period, bb1_deviation, bb1_bands_shift, bb1_AppliedPrice, 0, 0);    
				break;
...
for(int i=OrdersTotal()-1;i>=0;i--)
{
	 if(OrderSelect(i,SELECT_BY_POS) && OrderSymbol() == _Symbol && OrderMagicNumber()== magico)
	 {
		double tp  = iBands(NULL, 0, bb1_period, bb1_deviation, bb1_bands_shift, bb1_AppliedPrice, 0, 0);    
		if(tp != OrderTakeProfit())
			OrderModify(OrderTicket(), OrderOpenPrice(), OrderStopLoss(), tp, OrderExpiration(), 0);			
	 }
}
}
