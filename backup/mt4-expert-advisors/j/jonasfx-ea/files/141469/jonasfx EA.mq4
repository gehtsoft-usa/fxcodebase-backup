...
//https://fxcodebase.com/code/viewtopic.php?f=38&t=71081
...
public:
   virtual bool DoAction(const int period, const datetime date)
   {
      ...
      double price = MathFloor((Bid / pipSize) / distance) * distance * pipSize;
      ...
      if (OrderType() == OP_BUY)
      {
         double entryPrice = OrderOpenPrice() - distance * pipSize;
         orderBuilder
            .SetRate(entryPrice)
            .SetSide(SellSide)
...
int OnInit()
{
   ...
   tradingMonitor.SetOnOrderChanged(onNewTradeAction);
...
void OnTick()
{
   ...
   if (first)
   {
      ...
      int mult = digits == 3 || digits == 5 ? 10 : 1;
      double pipSize = point * mult;
      double entryPrice = MathFloor((Bid / pipSize) / distance) * distance * pipSize;
      OrderBuilder* orderBuilder = new OrderBuilder(actions);
      string error;
      int order = orderBuilder
         ...
         .SetSlippage(slippage)
         .SetMagicNumber(magic_number)
         .SetStopLoss(direction ? entryPrice - stop_loss * pipSize : entryPrice + stop_loss * pipSize)
         .SetTakeProfit(direction ? entryPrice + take_profit * pipSize : entryPrice - take_profit * pipSize)
         .Execute(error);
      delete orderBuilder;
   }
}
