...
//https://fxcodebase.com/code/viewtopic.php?f=38&t=71212
...
input int adx_level = 30; // ADX Level
input int max_trades_day = 2; // Max trades in a day
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      double adxValue = iADX(_symbol, _timeframe, adx_period, PRICE_CLOSE, MODE_MAIN, period);
      return adx_level < adxValue;
   }
};
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      double dp = iADX(_symbol, _timeframe, adx_period, PRICE_CLOSE, MODE_PLUSDI, period);
      double dm = iADX(_symbol, _timeframe, adx_period, PRICE_CLOSE, MODE_MINUSDI, period);
      return dp > dm;
   }
};
...
ICondition* CreateLongFilterCondition(string symbol, ENUM_TIMEFRAMES timeframe)
{
   ...
   condition.Add(new DayTradesLimitCondition(symbol, magic_number, max_trades_day), false);
   return condition;
}
...
{
   AndCondition* condition = new AndCondition();
   ICondition* main = new ADXCondition(symbol, timeframe);
   condition.Add(new NotCondition(main), false);
   main.Release();
   #ifdef ACT_ON_SWITCH_CONDITION
      ActOnSwitchCondition* switchCondition = new ActOnSwitchCondition(symbol, timeframe, (ICondition*) condition);
      condition.Release();
