...
string file_custom_indicator = "TrendIndicator.ex4";
...
input string         ashi               = "== Indicator Setup =="; // ————————————
input int            SignalCandle       = 1;
input string         MA1Setting         = "MA1 Setting";
input int            MA1_Period         = 8;
input ENUM_MA_METHOD MA1_Mode           = MODE_EMA;
input string         MA2Setting         = "MA2 Setting";
input int            MA2_Period         = 13;
input ENUM_MA_METHOD MA2_Mode           = MODE_EMA;
input bool           UseMATrendFilter   = true;
double               ArrowGapMultiplier = .5;
...
input EnumSides    enumSides      = Both_Side;          // Side:
input int          max_sells      = 3;                  // Max Sells:
input int          max_buys       = 3;                  // Max Buys:
...
class BUYcondition1 : public iConditions
{
 public:
  bool evaluate()
  {
	...
	double value = iCustom(NULL,0,file_custom_indicator, ashi, SignalCandle, MA1Setting, MA1_Period, MA1_Mode, MA2Setting, MA2_Period, MA2_Mode, UseMATrendFilter, ArrowGapMultiplier, BuyArrowColor, SellArrowColor, ArrowSize, 5,1);
...
class SELLcondition1 : public iConditions
{
 public:
  bool evaluate()
  {
    ...
    double value = iCustom(NULL,0,file_custom_indicator, ashi, SignalCandle, MA1Setting, MA1_Period, MA1_Mode, MA2Setting, MA2_Period, MA2_Mode, UseMATrendFilter, ArrowGapMultiplier, BuyArrowColor, SellArrowColor, ArrowSize, 4,1);
...
int OnInit()
{
  ...
  conditionsToBuy.AddCondition(countBuys = new ConditionCountBuys(max_buys));
  ...
  conditionsToSell.AddCondition(countSells = new ConditionCountSells(max_sells));
...
void OnTick()
{
  ...
  if (!uTradeReverse)
  {
		if(enumSides==Both_Side || enumSides==Buy_Side)
    if (conditionsToBuy.EvaluateConditions())
    {
      ...
      delete levelSL;
		...
		// NOTE: SELL normal
		if(enumSides==Both_Side || enumSides==Sell_Side)
