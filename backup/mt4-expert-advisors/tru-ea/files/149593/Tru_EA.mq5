...
input string       tTailingStop            = "== Trailireturn Stop Setup ==";  // == Trailing Stop Setup ==
...
#ifdef MOVING_AVERAGE_ON
  ma   = new MovingAverage(_Symbol, Period());
  ma.setSetup(ma_Period, ma_Shift, ma_Method, ma_AppliedPrice);
#endif
...
/*
	...
	ma.index(i) => is a moving average object. This is setting by user in inputs "Moving Average I Setup"
  so the next function make this: (close - simple_moving_average(length))/simple_moving_average(length)
*/
double detrend(int i)
{
  return (iClose(NULL, 0, i) - (ma.index(i))) / ma.index(i);
}
