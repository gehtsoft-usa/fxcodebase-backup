...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=74426
...
#property description "Expert Advisor Recovery Zone"
#property description "Be sure to have the TP at least the double of size than Recovery Zone"
#property strict
...
input int          magico        = 2024;          // Magic Number:
...
#endif
#ifdef GRID_ON
input string tGrid               = "== RecoveryZone Setup =="; // == RecoveryZone Setup ==
input bool   GridON              = true;                       // Use RecoveryZone:
...
input bool   closeGridOn         = true;                       // Use Close RecoveryZone?
input double closeGridTP         = 100;                        // Take Profit RecoveryZone $
input double closeGridSL         = -100;                       // Stop Loss RecoveryZone -$
#else
...
class OrdersList
{
    ...
    bool SomeOrderCloseByTP()
    {
        for(int i = 0; i < qnt(); i++)
        {
            if(isCloseByTP(i)) return true;
        }
        return false;
    }
  ...
  public:
    ...
    double nextPrice(double inpLastPrice = 0)
    {
        ...
        if (inpLastPrice > 0) {
            ...
            if (_side == "buy")
                _nextPrice = _sellPrice;
            if (_side == "sell")
                _nextPrice = _buyPrice;
        }
    ...
    void doGrid()
    {
        if (conditionsToOpenNewTrade.EvaluateConditions()) {
            if (_side == "sell") {
                openTrade = new SendNewOrder("buy", Lots(), "", 0, gridSL("buy"), gridTP("buy"), magico);
            ...
            if (_side == "buy") {
                openTrade = new SendNewOrder("sell", Lots(), "", 0, gridSL("sell"), gridTP("sell"), magico);
                if (openTrade.doAction()) {
                    if (gridOrders.GetLastMarketOrder()) {
    ...
    double gridSL(string side)
    {
        return SL(side);
    }
    double gridTP(string side)
    {
        return TP(side);
    }
...
RecoveryZone* gridBuy;
RecoveryZone* gridSell;
...
class ConditionGridActive : public iConditions
{
  ...
  public:
    ConditionGridActive(RecoveryZone* grid)
    {
        _grid = grid;
    }
    ...
    if (!uTradeReverse) {
        if (conditionsToBuy.EvaluateConditions()) {
            ...
            if (actionSendOrder.doAction()) {
                ...
                if (GridON == true && CheckPointer(gridBuy) == POINTER_INVALID) {
                    gridBuy = new RecoveryZone(_Symbol, "buy", mainOrders.last().price(), GridUser_gap, GridUser_multiplier, GridUser_maxCount, GridUser_maxLot, mainOrders.last().lot(), magico);
        ...
        if (conditionsToSell.EvaluateConditions()) {
            ...
            if (actionSendOrder.doAction()) {
                ...
                if (GridON == true && CheckPointer(gridSell) == POINTER_INVALID) {
                    ...
                    gridSell = new RecoveryZone(_Symbol, "sell", mainOrders.last().price(), GridUser_gap, GridUser_multiplier, GridUser_maxCount, GridUser_maxLot, mainOrders.last().lot(), magico);
...
void doCloseGridControl()
{
    ...
    if (mainOrders.SomeOrderCloseByTP()) {
        if (CheckPointer(gridBuy) != POINTER_INVALID) {
            gridBuy.closeGrid();
            delete gridBuy;
        }
