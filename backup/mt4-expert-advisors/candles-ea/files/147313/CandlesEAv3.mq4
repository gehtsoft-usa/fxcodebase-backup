...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=71990
...
input bool               uUseEMAFilter  = false;                         // Use Moving Average Filter:
input int                maPeriod       = 50;                            // Period
int                      maShift        = 0;                             // Ma Shift
input ENUM_MA_METHOD     maMethod       = MODE_SMMA;                     // Method
...
input bool         closeAllInOpositeSignal = true;                         // CLose All In Oposite Signal
...
input string       tIncreaseLots           = "== Increase Lots Setup ==";  // == Increase Lots Setup ==
input bool         MartingaleON            = true;                         // Increase Lots ON:
...
class SendNewOrder : public iActions
{
 ...
 public:
  ...
  bool doAction()
  {
    if (isCountMaxTrade(newOrder.type()) == false)
    {
...
int OnInit()
{
  ...
  if (uUseEMAFilter) conditionsToBuy.AddCondition(buyCondition2 = new BUYcondition2());
  ...
  if (uUseEMAFilter) conditionsToSell.AddCondition(sellCondition2 = new SELLcondition2());
  ...
  lastCandle = new CCandle(_Symbol, Period());

  // Note: Indicators
  emaFilter.setSetup(maPeriod, maShift, maMethod, maAppliedPrice);
  ...
  if (MartingaleON)
  {
    ...
    for (int i = OrdersHistoryTotal() - 1; i >= 0; i--)
    {
      if (OrderSelect(i, SELECT_BY_POS, MODE_HISTORY) && OrderSymbol() == _Symbol && OrderMagicNumber() == magico)
      {
        ...
        {
          waslosser = true;
          // determinar el lotaje multiplicando el lotaje de ese trade por el lotMultiplier
          lots = NormalizeDouble(OrderLots() * lotMultiplier, 2);
					break;
        } else { break; }
      }
    }
	...
	if(MartingaleON==true && waslosser==true)
	{
    lots = lots >= maxLots ? maxLots : lots;
    return lots;
  }


  // Forma tradicional de calcular el lotaje:
  // ------------------------------------------------------------------
...
bool isCountMaxTrade(int order_type)
{
  ...
  for (int i = 0; i < OrdersTotal(); i++)
  {
    ...
    {
      if (OrderType() == order_type && OrderSymbol() == Symbol() && OrderMagicNumber() == magico)
      {
