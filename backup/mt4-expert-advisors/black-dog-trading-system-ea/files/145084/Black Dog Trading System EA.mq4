...
class BlackDog : public iIndicators
{
   string _symbol;
   int    _tf;
   string _file;
  ...
  public:
   BlackDog()
	{
		_symbol = _Symbol;
      _tf     = Period();
      _file   = "BD Crossover BDv8_12_31.ex4";
	}
	BlackDog(string Symbol, int TimeFrame)
   {
      _symbol = Symbol;
      _tf     = TimeFrame;
      _file   = "BD Crossover BDv8_12_31.ex4";
   ...
   double calculate(int shift, int buffer = 0)
   {
      ...
      return iCustom(_symbol, _tf, _file, FasterEMA1, SlowerEMA1, FasterEMA2, SlowerEMA2, SoundON, buffer, shift);
   }
   ...
   double lastValue(int buffer, bool candle = false)
   {
      ...
      int    i     = 0;
      while (value == EMPTY_VALUE ||value == 0 || i == 2000)
      {
   ...
   int actualSide()
   {
      if (actualSideBD() == 0 && actualSideSES() == 0)
         return 0;
      if (actualSideBD() == 1 && actualSideSES() == 1)
         return 1;
   ...
   int actualSideBD()
   {
      int lastBuy  = (int)lastValue(2, true);
      int lastSell = (int)lastValue(3, true);
      if (lastBuy < lastSell)
         return 0;  // BUY
      else
         return 1;  // SELL
   ...
   int BDSignal()
   {
      if (calculate(1, 2) != EMPTY_VALUE) return 0;
		if (calculate(1, 3) != EMPTY_VALUE) return 1;
   ...
   int SESSignal()
   {
      if (calculate(1, 0) != EMPTY_VALUE) return 0;
		if (calculate(1, 1) != EMPTY_VALUE) return 1;
...
class BUYcondition1 : public iConditions
{
  public:
   bool evaluate()
   {
      ...
      if (blackDog.BDSignal() == 0 || blackDog.SESSignal()==0)
      {
         return true;
      }
      return false;
  ...
  public:
   bool evaluate()
   {
      // TODO: condition Buy 2
		if (blackDog.actualSideBD() == 0 && blackDog.SESSignal()== 0)
		{
			return true;
		}
      return false;
...
class SELLcondition1 : public iConditions
{
  public:
   bool evaluate()
   {
      ...
      if (blackDog.BDSignal() == 1 || blackDog.SESSignal()==1)
      {
         return true;
      }
      return false;
  ...
  public:
   bool evaluate()
   {
      // TODO: condition sell 2
		if (blackDog.actualSideBD() == 1 && blackDog.SESSignal()==1)
      {
         return true;
      }
      return false;
