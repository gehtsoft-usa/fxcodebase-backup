...
class ATR
{
 ...
 public:
  ATR(){;}
  ATR(string Symbol, ENUM_TIMEFRAMES TimeFrame = PERIOD_CURRENT, int Period = 14)
  {
    Setup(Symbol, TimeFrame, Period);
  }
  ~ATR() { ; }

  void Setup(string Symbol = NULL, ENUM_TIMEFRAMES TimeFrame = PERIOD_CURRENT, int Period = 14)
  {
...
int OnInit()
{
  ...
  SetIndexBuffer(2, bATR, INDICATOR_CALCULATIONS);

  atr.Setup(Symbol(), Period(), atrPeriods);
                ...
                const int&      spread[])
{
  ...
  for (i = start; i < rates_total && !IsStopped(); i++) {
    ...
    // clang-format off
        bATR[i] = 
            cl > bATR[i - 1] && cl1 > bATR[i - 1] ? 
            fmax(bATR[i - 1], cl - matr) :
            cl < bATR[i - 1] && cl1 < bATR[i - 1] ? 
            fmin(bATR[i - 1], cl + matr) :
            cl > bATR[i - 1] ? cl - matr : cl + matr;

        bool crossUp = cl > bATR[i] && cl1 < bATR[i - 1];
        bool crossDn = cl < bATR[i] && cl1 > bATR[i - 1];
    // clang-format on

    if (cl > bATR[i] && crossUp == true) {
      ArrowUp[i] = low[i] - cATR / 2;
      if (ArrowUp[i] != EMPTY_VALUE) { notify("buy"); }
    }

    if (cl < bATR[i] && crossDn == true) {
      ArrowDn[i] = high[i] + cATR / 2;
...
void notify(string side)
{
  ...
  if (side == "buy") Notifications(0);
  if (side == "sell") Notifications(1);
}
