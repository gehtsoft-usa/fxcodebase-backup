//Available @  https://fxcodebase.com/code/viewtopic.php?f=38&t=75509 
...
#property indicator_label1 "OBMFI"
...
int init()
{
   ...
   IndicatorShortName("On-balance MFI");
                ...
                const int &spread[])
{
   ...
   {
      mfi1Series.SetValue(pos, close[pos]);
      double mfi1Value;
      if (!mfi1.GetValue(pos, mfi1Value)) { mfi1Value = EMPTY_VALUE; }
      double mfi = mfi1Value;
      if (pos + 1 > (rates_total - 1)) { continue; }
      cum1X.SetValue(pos, (SafeGreater(close[pos], close[pos + 1]) ? mfi : InvertSign(mfi)));
      double cum1Value;
      if (!cum1.GetValue(pos, cum1Value)) { cum1Value = EMPTY_VALUE; }
      double obmfi = cum1Value;
      plot1[pos] = obmfi;
   }
