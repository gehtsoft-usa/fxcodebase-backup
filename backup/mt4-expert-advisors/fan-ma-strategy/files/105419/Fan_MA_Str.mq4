...
int OnInit()
{
     double temp = iCustom(NULL, 0, "Fan_MA2", 0, 0);
   if (GetLastError() == ERR_INDICATOR_CANNOT_LOAD)
   {
       Alert("Please, install the 'Fan_MA2' indicator");
       return INIT_FAILED;
   }
   LastBar=0;
...
void OnTick()
{
 ...
 FMA0=iCustom(NULL, 0, "Fan_MA2", First_Length, Increment_Type_Str, Increment_Type, Length_Step, MA_Count, Method, Price, Average_Length, Average_Method, 2, 1);
 FMA1=iCustom(NULL, 0, "Fan_MA2", First_Length, Increment_Type_Str, Increment_Type, Length_Step, MA_Count, Method, Price, Average_Length, Average_Method, 2, 2);
 
 int CO;
 int res;
 double SL, TP;
 
 if ((FMA1<Buy_Level && FMA0>=Buy_Level && !Inverse_Direction) || (FMA1>Sell_Level && FMA0<=Sell_Level && Inverse_Direction))
 {
  _Alert("Buy");
  CO=CalculateOrders();
  if (CO<=0)
 ...
 if ((FMA1>Sell_Level && FMA0<=Sell_Level && !Inverse_Direction) || (FMA1<Buy_Level && FMA0>=Buy_Level && Inverse_Direction))
 {
  _Alert("Sell");
  CO=CalculateOrders();
  if (CO>=0)
