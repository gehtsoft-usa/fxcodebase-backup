...
// http://fxcodebase.com/code/viewtopic.php?f=38&t=69163
...
public:
   ...
   int temp_start[1000];
   int temp_hits[1000];
   int temp_strength[1000];
   int temp_count;
   bool temp_turn[1000];
   bool temp_merge[1000];

   double zone_hi[1000],zone_lo[1000];
   int zone_start[1000],zone_hits[1000],zone_type[1000],zone_strength[1000],zone_count;
   bool zone_turn[1000];
   
   double FastDnPts[1000],FastUpPts[1000];
   double SlowDnPts[1000],SlowUpPts[1000];
   ...
   void FindZones(int from)
   {
      ...
      for(int shift = from; shift > 5; shift--)
      {
         ...
         {
            double hival = High[shift];
            if (zone_extend)
               hival += fu;
            double loval = MathMax(MathMin(Close[shift], High[shift] - fu), High[shift] - fu * 2);
            
            bool isWeak = SlowUpPts[shift] <= 0.001;
            if (!IsBurst(shift, false, loval, hival, isWeak, testcount, hasturned))
            {
               Save(loval, hival, hasturned, testcount, shift, isWeak);
            }
         }
         else if (FastDnPts[shift] > 0.001)
         {
            double loval = Low[shift];
            if (zone_extend)
               loval -= fu;
            double hival = MathMin(MathMax(Close[shift], Low[shift] + fu), Low[shift] + fu * 2);

            bool isWeak = SlowDnPts[shift] <= 0.001;
            if (!IsBurst(shift, true, loval, hival, isWeak, testcount, hasturned))
            {
               Save(loval, hival, hasturned, testcount, shift, isWeak);
            }
         }
...
private:
   bool IsBurst(int shift, bool turned, double loval, double hival, bool isWeak, int& testcount, bool& hasturned)
   {
      hasturned = false;
      testcount = 0;
      int bustcount = 0;
      ...
      {
         if ((!turned && FastUpPts[i] >= loval && FastUpPts[i] <= hival) 
            || (turned && FastDnPts[i] <= hival && FastDnPts[i] >= loval))
         {
            // Potential touch, just make sure its been 10+candles since the prev one
            if (IsTouchOk(i, turned, loval, hival))
            {
         ...
         if ((!turned && High[i] > hival) || (turned && Low[i] < loval))
         {
   ...
   void Save(double loval, double hival, bool hasturned, int testcount, int shift, bool isWeak)
   {
   ...
   bool IsTouchOk(int period, bool turned, double loval, double hival)
   {
      for (int j = period + 1; j < period + 11; j++)
      {
         if ((!turned && FastUpPts[j] >= loval && FastUpPts[j] <= hival) 
            || (turned && FastDnPts[j] <= hival && FastDnPts[j] >= loval))
         {
            return false;
         }
      }
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      ...
      {
         if (zones.zone_strength[i] == ZONE_PROVEN || zones.zone_strength[i] == ZONE_VERIFIED)
         {
            double close0 = iClose(_symbol, _timeframe, period);
            double close1 = iClose(_symbol, _timeframe, period + 1);
            if (close0 > zones.zone_lo[i] && close1 <= zones.zone_lo[i])
               return true;
         }
      }
      return false;
   ...
   bool IsPass(const int period, const datetime date)
   {
      for (int i = 0; i < zones.zone_count; ++i)
      {
         if (zones.zone_strength[i] == ZONE_PROVEN || zones.zone_strength[i] == ZONE_VERIFIED)
         {
            double close0 = iClose(_symbol, _timeframe, period);
            double close1 = iClose(_symbol, _timeframe, period + 1);
            if (close0 < zones.zone_hi[i] && close1 >= zones.zone_hi[i])
               return true;
         }
      }
      return false;
...
void OnTick()
{
   zones.FastFractals(MathMin(Bars - 1, 999));
   zones.SlowFractals(MathMin(Bars - 1, 999));
   zones.FindZones(MathMin(Bars - 1, 999));
