...
//https://fxcodebase.com/code/viewtopic.php?f=38&t=71330
...
IndicatorOutputStream* highest1Source;
IndicatorOutputStream* x;
IStream* x_ma;
IStream* highest1;
IStream* wma2;
IStream* wma3;
IndicatorOutputStream* lowest4Source;
IStream* lowest4;
IStream* wma5;
IStream* wma6;
int init()
{
   ...
   IndicatorShortName("Auto-Dispersion Bands");
   IndicatorBuffers(6);
   SetIndexBuffer(0, plot1);
   SetIndexBuffer(1, plot2);
   SetIndexBuffer(2, plot3);
   highest1Source = new IndicatorOutputStream(_Symbol, (ENUM_TIMEFRAMES)_Period);
   highest1Source.RegisterInternalStream(3);
   highest1 = new HighestHighStream(highest1Source, length);
   wma2 = new WMAOnStream(highest1, length);
   wma3 = new WMAOnStream(wma2, smooth);
   lowest4Source = new IndicatorOutputStream(_Symbol, (ENUM_TIMEFRAMES)_Period);
   lowest4Source.RegisterInternalStream(4);
   lowest4 = new LowestLowStream(lowest4Source, length);
   wma5 = new WMAOnStream(lowest4, length);
   wma6 = new WMAOnStream(wma5, smooth);
   
   x = new IndicatorOutputStream(_Symbol, (ENUM_TIMEFRAMES)_Period);
   x.RegisterInternalStream(5);
   x_ma = new SmaOnStream(x, length);
                ...
                const int &spread[])
{
   ...
   {
      x._data[pos] = MathPow(close[pos] - close[pos + length], 2);
      double ma_val;
      if (!x_ma.GetValue(pos, ma_val))
      {
         continue;
      }
      double sq = MathSqrt(ma_val);

      highest1Source._data[pos] = close[pos] + sq;
      lowest4Source._data[pos] = close[pos] - sq;
      double wma3Value;
