...
//http://fxcodebase.com/code/viewtopic.php?f=38&t=70509
...
int Complect70 = 70;
int Complect80 = 80;
input int BackSteph = 0; // number of steps back (h)
input int BackStepl = 0; // the number of steps ago l
input int BackStep = 0;  // number of steps backward
input int qSteps = 1;    // the number of steps, not more than 3
input int LevDPl = 5;    // level of Demark points; 2 = central bar is above (below) the 2 bars to the left
input int LevDPr = 1;    // level of Demark points; 2 = central bar is above (below) the 2 bars on the right
input int period = 8;
...
input double b = 0.7;
input int showBars_ = 1000; // If = 0, then the indicator is displayed for all graphics
...
int qBars;
double qTime = 0; // variables to eliminate glitches when loading
...
void SendAlerts()
{
   ...
   {
      LineCrossCondition* item = conditions[i];
      if (item.IsPass(0, Time[0]))
      {
         signaler.SendNotifications("Line cross");
         conditions[i].Release();
         conditions[i] = NULL;
         ++toRemove;
         for (int ii = i; ii < ArraySize(conditions) - 1; ++ii)
         {
            conditions[ii] = conditions[ii + 1];
         }
         conditions[ArraySize(conditions) - 1] = NULL;
      }
   }
   if (toRemove > 0)
   {
      int size = ArraySize(conditions);
      ArrayResize(conditions, size - toRemove);
   }
}
//+------------------------------------------------------------------+
...
int start()
{
   ...
   for (shift = 0; shift <= limit; shift++)
   {
      ...
      case 29:
         ...
         break;
      case 30:
         Oscil[shift] = iWPR(NULL, 0, period, shift);
         break;
      default:
         Oscil[shift] = iRSI(NULL, 0, period, applied_price, shift);
      }
   ...
   if (qTime == Time[0])
      return (0);
   qTime = Time[0]; // runs only on the first tick
   int showBars = showBars_;
...
void CreateCondition(string id)
{
   if (LineExists(id))
   {
      return;
   ...
   conditions[size] = new LineCrossCondition(id);
}
//+------------------------------------------------------------------+
//|                                                                  |
//+------------------------------------------------------------------+
string TDMain(int Step)
{
   ...
   if (Trend <= 0)
   {
      ...
      if (ChannelLine)
      {
         ...
         ObjectSet(Rem, OBJPROP_PRICE1, t3_Oscil[qExt]);
         ObjectSet(Rem, OBJPROP_PRICE2, t3_Oscil[qExt] - qTL * qExt);
