...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=74556
...
CNewCandle* newCandleBOB;
CNewCandle* newCandleBOFB;
CNewCandle* newCandleBOTB;
CNewCandle* newCandleSOB;
CNewCandle* newCandleSOFB;
CNewCandle* newCandleSOTB;

int Distancia(double precioA, double precioB, string mode = "pips")
{
    ...
    if (mode == "points")
        return (dist / mPoints);
    if (mode == "pips")
        return ((dist / mPoints) / 10);
    return -1;
}
//+------------------------------------------------------------------+

#define STRATEGYS_SELECTOR
#ifdef STRATEGYS_SELECTOR

input string          TBOB                   = "== Buy on Breackout Setup ==";        // ————————————
input bool            BOB_ON                 = true;                                  // BOB ON:
input ENUM_TIMEFRAMES BOB_tf                 = PERIOD_CURRENT;                        // BOB TF:
input double          BOB_Price              = 1.20;                                  // BOB Price:
input bool            BOB_ControlQntOn       = true;                                  // BOB Control #Orders?
input int             BOB_QntOrders          = 1;                                     // BOB Max #Orders:
input bool            BOB_ControlDistanceOn  = true;                                  // BOB Control Distance?
input int             BOB_MaxPips            = 10;                                    // BOB Max distance:
input string          TBOFB                  = "== Buy on False Breackout Setup ==";  // ————————————
input bool            BOFB_ON                = true;                                  // BOFB ON:
input ENUM_TIMEFRAMES BOFB_tf                = PERIOD_CURRENT;                        // BOFB TF:
input double          BOFB_Price             = 1.20;                                  // BOFB Price:
input bool            BOFB_ControlQntOn      = true;                                  // BOFB Control #Orders?
input int             BOFB_QntOrders         = 1;                                     // BOFB Max #Orders:
input bool            BOFB_ControlDistanceOn = true;                                  // BOFB Control Distance?
input int             BOFB_MaxPips           = 10;                                    // BOFB Max distance:
input string          TBOTB                  = "== Buy on Throwback Setup ==";        // ————————————
input bool            BOTB_ON                = true;                                  // BOTB ON:
input ENUM_TIMEFRAMES BOTB_tf                = PERIOD_CURRENT;                        // BOTB TF:
input double          BOTB_Price             = 1.20;                                  // BOTB Price:
input bool            BOTB_ControlQntOn      = true;                                  // BOTB Control #Orders?
input int             BOTB_QntOrders         = 1;                                     // BOTB #Orders:
input bool            BOTB_ControlDistanceOn = true;                                  // BOTB Control Distance?
input int             BOTB_MaxPips           = 10;                                    // BOTB Max distance:
input int             BOTB_Candles           = 10;                                    // BOTB candles thereafter:
input string          TSOB                   = "== Sell on Breackout Setup ==";       // ————————————
input bool            SOB_ON                 = true;                                  // SOB ON:
input ENUM_TIMEFRAMES SOB_tf                 = PERIOD_CURRENT;                        // SOB TF:
input double          SOB_Price              = 1.20;                                  // SOB Price:
input bool            SOB_ControlQntOn       = true;                                  // SOB Control #Orders?
input int             SOB_QntOrders          = 1;                                     // SOB Max #Orders:
input bool            SOB_ControlDistanceOn  = true;                                  // SOB Control Distance?
input int             SOB_MaxPips            = 10;                                    // SOB Max distance:
input string          TSOFB                  = "== Sell on False Breackout Setup =="; // ————————————
input bool            SOFB_ON                = true;                                  // SOFB ON:
input ENUM_TIMEFRAMES SOFB_tf                = PERIOD_CURRENT;                        // SOFB TF:
input double          SOFB_Price             = 1.20;                                  // SOFB Price:
input bool            SOFB_ControlQntOn      = true;                                  // SOFB Control #Orders?
input int             SOFB_QntOrders         = 1;                                     // SOFB Max #Orders:
input bool            SOFB_ControlDistanceOn = true;                                  // SOFB Control Distance?
input int             SOFB_MaxPips           = 10;                                    // SOFB Max distance:
input string          TSOTB                  = "== Sell on Throwback Setup ==";       // ————————————
input bool            SOTB_ON                = true;                                  // SOTB ON:
input ENUM_TIMEFRAMES SOTB_tf                = PERIOD_CURRENT;                        // SOTB TF:
input double          SOTB_Price             = 1.20;                                  // SOTB Price:
input bool            SOTB_ControlQntOn      = true;                                  // SOTB Control #Orders?
input int             SOTB_QntOrders         = 1;                                     // SOTB #Orders:
input bool            SOTB_ControlDistanceOn = true;                                  // SOTB Control Distance?
input int             SOTB_MaxPips           = 10;                                    // SOTB Max distance:
input int             SOTB_Candles           = 10;                                    // SOTB candles thereafter:
#endif
...
ConditionsModeOneTrue conditionsToBuy;
ConditionsModeOneTrue conditionsToSell;
...
class BUYcondition1 : public iConditions
{
  ...
  public:
    ...
    // TODO: condition Buy 1
    {
        ...
        if (!newCandleBOB.IsNewCandle())
            return false;

        double op = iOpen(NULL, BOB_tf, 1);
        double cl = iClose(NULL, BOB_tf, 1);

        if (BOB_ControlDistanceOn)
            if (Distancia(cl, BOB_Price) > BOB_MaxPips)
                return false;

        if (BOB_ControlQntOn)
            if (countOrders("BOB") >= BOB_QntOrders)
                return false;

        if (cl > BOB_Price && op < BOB_Price) {
            strategy = "BOB";
            return true;
        }
        strategy = "";
...
class BUYcondition2 : public iConditions
{
  public:
    bool evaluate()
    {
        ...
        if (!BOFB_ON)
            return false;
        if (!newCandleBOFB.IsNewCandle())
            return false;

        double op = iOpen(NULL, BOFB_tf, 1);
        double cl = iClose(NULL, BOFB_tf, 1);
        double lo = iLow(NULL, BOFB_tf, 1);

        if (BOFB_ControlDistanceOn)
            if (Distancia(cl, BOFB_Price) > BOFB_MaxPips)
                return false;

        if (BOFB_ControlQntOn)
            if (countOrders("BOFB") >= BOFB_QntOrders)
                return false;

        if (cl > BOFB_Price && op > BOFB_Price && lo < BOFB_Price) {
            strategy = "BOFB";
            return true;
        }
        strategy = "";
...
class BUYcondition3 : public iConditions
{
  ...
  public:
    bool evaluate()
    {
        ...
        if (!BOTB_ON)
            return false;
        if (!newCandleBOTB.IsNewCandle())
            return false;

        for (int i = 1; i <= BOTB_Candles; i++) {
            double op = iOpen(NULL, BOTB_tf, i);
            double cl = iClose(NULL, BOTB_tf, i);
            double lo = iLow(NULL, BOTB_tf, i);

            if (BOTB_ControlDistanceOn)
                if (Distancia(cl, BOTB_Price) > BOTB_MaxPips)
                    continue;

            if (BOTB_ControlQntOn)
                if (countOrders("BOTB") >= BOTB_QntOrders)
                    continue;

            if (cl > BOTB_Price && op > BOTB_Price && lo < BOTB_Price && iTime(NULL, BOTB_tf, i) != lastSignal) {
                strategy     = "BOTB";
                expireSignal = TimeCurrent() + (BOTB_tf * 60 * BOTB_Candles - i);
                lastSignal   = iTime(NULL, BOTB_tf, i);
                return true;
            }
        }
        strategy = "";
...
class SELLcondition1 : public iConditions
{
  public:
    bool evaluate()
    {
        ...
        if (!SOB_ON)
            return false;
        if (!newCandleSOB.IsNewCandle())
            return false;

        double op = iOpen(NULL, SOB_tf, 1);
        double cl = iClose(NULL, SOB_tf, 1);

        if (SOB_ControlDistanceOn)
            if (Distancia(cl, SOB_Price) > SOB_MaxPips)
                return false;
        if (SOB_ControlQntOn)
            if (countOrders("SOB") >= SOB_QntOrders)
                return false;

        if (cl < SOB_Price && op > SOB_Price) {
            strategy = "SOB";
            return true;
        }
        strategy = "";
...
class SELLcondition2 : public iConditions
{
  public:
    bool evaluate()
    {
        ...
        if (!SOFB_ON)
            return false;
        if (!newCandleSOFB.IsNewCandle())
            return false;

        double op = iOpen(NULL, SOFB_tf, 1);
        double cl = iClose(NULL, SOFB_tf, 1);
        double hi = iHigh(NULL, SOFB_tf, 1);

        if (SOFB_ControlDistanceOn)
            if (Distancia(cl, SOFB_Price) > SOFB_MaxPips)
                return false;
        if (SOFB_ControlQntOn)
            if (countOrders("SOFB") >= SOFB_QntOrders)
                return false;

        if (cl < SOFB_Price && op < SOFB_Price && hi > SOFB_Price) {
            strategy = "SOFB";
            return true;
        }
        strategy = "";
...
class SELLcondition3 : public iConditions
{
  ...
  public:
    bool evaluate()
    {
        ...
        if (!SOTB_ON)
            return false;
        if (!newCandleSOTB.IsNewCandle())
            return false;

        for (int i = 1; i <= SOTB_Candles; i++) {
            double op = iOpen(NULL, SOTB_tf, i);
            double cl = iClose(NULL, SOTB_tf, i);
            double hi = iHigh(NULL, SOTB_tf, i);
            
            Print(__FUNCTION__," cl: ",cl);

            if (SOTB_ControlDistanceOn)
                if (Distancia(cl, SOTB_Price) > SOTB_MaxPips)
                    continue;

            if (SOTB_ControlQntOn)
                if (countOrders("SOTB") >= SOTB_QntOrders)
                    continue;

            if (cl < SOTB_Price && op < SOTB_Price && hi > SOTB_Price && iTime(NULL, SOTB_tf, i) != lastSignal) {
                strategy     = "SOTB";
                expireSignal = TimeCurrent() + (SOTB_tf * 60 * SOTB_Candles - i);
                lastSignal   = iTime(NULL, SOTB_tf, i);
                return true;
            }
        }
        strategy = "";
...
int OnInit()
{
    ...
    // to control de closes of candles in dif tf:
    newCandleBOB  = new CNewCandle(Symbol(), BOB_tf);
    newCandleBOFB = new CNewCandle(Symbol(), BOFB_tf);
    newCandleBOTB = new CNewCandle(Symbol(), BOTB_tf);
    newCandleSOB  = new CNewCandle(Symbol(), SOB_tf);
    newCandleSOFB = new CNewCandle(Symbol(), SOFB_tf);
    newCandleSOTB = new CNewCandle(Symbol(), SOTB_tf);

    //--- CONDITIONS TO OPEN TRADES:
    //--- buys:
    conditionsToBuy.AddCondition(BOB = new BUYcondition1());
    conditionsToBuy.AddCondition(BOFB = new BUYcondition2());
    ...
    //--- sell:
    conditionsToSell.AddCondition(SOB = new SELLcondition1());
    conditionsToSell.AddCondition(SOFB = new SELLcondition2());
...
void OnTick()
{
    ...
    if (!uTradeReverse) {
        if (conditionsToBuy.EvaluateConditions()) {
            if (modeEntry == Market && strategy != "BOTB") {
                actionSendOrder = new SendNewOrder("buy", Lots(), "", 0, SL("buy"), TP("buy"), magico, strategy);
            } else {
                // double pr       = Price("buy", uEntryDistance, _Symbol);
                double pr       = BOTB_Price;
                actionSendOrder = new SendNewOrder("buy", Lots(), "", pr, SL("buy", pr), TP("buy", pr), magico, strategy, expireSignal);
            }
        ...
        if (conditionsToSell.EvaluateConditions()) {
            if (modeEntry == Market && strategy != "SOTB") {
                actionSendOrder = new SendNewOrder("sell", Lots(), "", 0, SL("sell"), TP("sell"), magico, strategy);
            } else {
                // double pr       = Price("sell", uEntryDistance, _Symbol);
                double pr       = SOTB_Price;
                actionSendOrder = new SendNewOrder("sell", Lots(), "", pr, SL("sell", pr), TP("sell", pr), magico, strategy, expireSignal);
            }
            if (actionSendOrder.doAction()) {
                mainOrders.GetMarketOrders();
