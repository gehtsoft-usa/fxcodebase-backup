...
public:
   virtual bool DoAction(const int period, const datetime date)
   {
      if (current_lots == lots_value * 4 || OrderMagicNumber() != _magicNumber)
      {
         return true;
      }
      ...
      else if (OrderClosePrice() == OrderTakeProfit() || StringFind(OrderComment(), "[tp]", 0) != -1)
      {
         current_lots *= 2;
...
int OnInit()
{
   ...
   monitor.SetClosedTradeAction(closeTradeAction);
...
void OpenTrade(double amount, OrderSide side)
{
   ...
   IMoneyManagementStrategy* moneyManagement = CreateMoneyManagementStrategy(tradingCalculator, _Symbol, (ENUM_TIMEFRAMES)_Period, side == BuySide, 
      lots_type, amount, stop_loss_type, stop_loss_value, stop_loss_atr_multiplicator, take_profit_type, take_profit_value, take_profit_atr_multiplicator);
