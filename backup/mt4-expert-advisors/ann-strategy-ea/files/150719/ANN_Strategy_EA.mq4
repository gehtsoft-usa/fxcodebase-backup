...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=73687
...
string file_custom_indicator = "Higher Time Frame ANN Strategy Indicator.ex4";
...
input double          threshold  = 0.0014;
input color           High_Color  = clrDeepSkyBlue;
input color           Low_Color   = clrRed;
input ENUM_TIMEFRAMES TimeFrame = PERIOD_H4; // Upper timeframe
input string             Iema           = "== Moving Average Setup ==";  // ————————————
input bool MovingAverageFilterOn = true; // EMA Filter On?
 ...
 public:
  bool evaluate()
  {
    // TODO: condition Buy 1
      int current_bar = 1;
      int buy_current = iCustom(NULL, 0, file_custom_indicator, threshold, High_Color, Low_Color, TimeFrame, 0, current_bar);
      int sell_current = iCustom(NULL, 0, file_custom_indicator, threshold, High_Color, Low_Color, TimeFrame, 1, current_bar);

    int buy_prev = iCustom(NULL, 0, file_custom_indicator, threshold, High_Color, Low_Color, TimeFrame, 0, 2);
    int sell_prev = iCustom(NULL, 0, file_custom_indicator, threshold, High_Color, Low_Color, TimeFrame, 1, 2);

    if (buy_prev == 0 && sell_prev == -1 && buy_current == 1 && sell_current == 0 ) return true;
 ...
 public:
  bool evaluate()
  {
    // TODO: condition sell 1
      int indi = iCustom(NULL, 0, file_custom_indicator, threshold, High_Color, Low_Color, TimeFrame, 1, 1);
      int indi2 = iCustom(NULL, 0, file_custom_indicator, threshold, High_Color, Low_Color, TimeFrame, 0, 2);
      if (indi2 == 1 && indi == -1) return true;
  ...
  bool evaluate()
  {
    if (closeAllInOpositeSignal) {
        if (sellCondition1.evaluate()) {
            if (MovingAverageFilterOn) {
                if (sellCondition2.evaluate()) { return true; }
            } // si no hay que evaluar la ema:
            else { return true; }           
        }
    }
  ...
  bool evaluate()
  {
      if (closeAllInOpositeSignal) {
          if (buyCondition1.evaluate()) {
              if (MovingAverageFilterOn) {
                  if (buyCondition2.evaluate()) { return true; }
              }
              else { return true; }
          }
        }
...
int OnInit()
{
  ...
  if(MovingAverageFilterOn) conditionsToBuy.AddCondition(buyCondition2 = new BUYcondition2());
  ...
  if(MovingAverageFilterOn) conditionsToSell.AddCondition(sellCondition2 = new SELLcondition2());
