...
Version:     2.00
Date:        2025
Repository:  Available @ https://fxcodebase.com/code/viewtopic.php?f=38&p=160832#p160832
License:     GNU
...
void DetectZoneBreakouts(int rates_total)
  {
   ...
   for(int i = 0; i < ArraySize(bu_ob_boxes); i++)
     {
      string obj_name = bu_ob_boxes[i].name;
      if(ObjectFind(0, obj_name) == -1)
         continue;
      double zone_high = MathMax(bu_ob_boxes[i].price1, bu_ob_boxes[i].price2);
      double zone_low = MathMin(bu_ob_boxes[i].price1, bu_ob_boxes[i].price2);
      datetime zone_start_time = bu_ob_boxes[i].time1;
   ...
   for(int i = 0; i < ArraySize(be_ob_boxes); i++)
     {
      string obj_name = be_ob_boxes[i].name;
      if(ObjectFind(0, obj_name) == -1)
         continue;
      double zone_high = MathMax(be_ob_boxes[i].price1, be_ob_boxes[i].price2);
      double zone_low = MathMin(be_ob_boxes[i].price1, be_ob_boxes[i].price2);
      datetime zone_start_time = be_ob_boxes[i].time1;
      int start_bar = iBarShift(NULL, 0, zone_start_time);
      if(start_bar <= 1)
         continue;
      for(int bar = start_bar; bar >= 1; bar--)
   ...
   for(int i = 0; i < ArraySize(bu_bb_boxes); i++)
     {
      string obj_name = bu_bb_boxes[i].name;
      if(ObjectFind(0, obj_name) == -1)
         continue;
      double zone_high = MathMax(bu_bb_boxes[i].price1, bu_bb_boxes[i].price2);
      double zone_low = MathMin(bu_bb_boxes[i].price1, bu_bb_boxes[i].price2);
      datetime zone_start_time = bu_bb_boxes[i].time1;
      int start_bar = iBarShift(NULL, 0, zone_start_time);
      if(start_bar <= 1)
         continue;
      for(int bar = start_bar; bar >= 1; bar--)
   ...
   for(int i = 0; i < ArraySize(be_bb_boxes); i++)
     {
      string obj_name = be_bb_boxes[i].name;
      if(ObjectFind(0, obj_name) == -1)
         continue;
      double zone_high = MathMax(be_bb_boxes[i].price1, be_bb_boxes[i].price2);
      double zone_low = MathMin(be_bb_boxes[i].price1, be_bb_boxes[i].price2);
      datetime zone_start_time = be_bb_boxes[i].time1;
      int start_bar = iBarShift(NULL, 0, zone_start_time);
      if(start_bar <= 1)
         continue;
      for(int bar = start_bar; bar >= 1; bar--)
   ...
   for(int obj = ObjectsTotal() - 1; obj >= 0; obj--)
     {
      ...
      if(StringFind(obj_name, "MSB_Line_") != 0)
         continue;
