//Available @  https://fxcodebase.com/code/viewtopic.php?f=38&p=144651#p144651
...
input string FastMASection = ""; // == Fast MA ==
input ENUM_PRICE        FPrice                =           0;       // Apply To
input int               FMA_Period            =          14;       // Period
input int               FMA_Shift             =           0;       // Shift
input ENUM_MA_MODE      FMA_Method            =         SMA;       // Method
input string SlowMASection = ""; // == Slow MA ==
input ENUM_PRICE        SPrice                =           0;       // Apply To
input int               SMA_Period            =          28;       // Period
input int               SMA_Shift             =           0;       // Shift
input ENUM_MA_MODE      SMA_Method            =         SMA;       // Method
...
public:
   ...
   {
      refs = 1;
      fast = iCustom(symbol, timeframe, "All Averages 4.9", 0,  FPrice, FMA_Period, FMA_Shift, FMA_Method);
      slow = iCustom(symbol, timeframe, "All Averages 4.9", 0,  SPrice, SMA_Period, SMA_Shift, SMA_Method);
      adx = iADX(symbol, timeframe, 14);
      rsi = iRSI(symbol, timeframe, 14, PRICE_CLOSE);
   }
   ~EntryStreamData()
   {
   ...
   {
      double buffer[1];
      if (CopyBuffer(adx, 0, period, 1, buffer) == 1)
      {
         return buffer[0];
      }
      return EMPTY_VALUE;
...
class LongADXFilterCondition : public ACondition
{
   EntryStreamData* data;
public:
   LongADXFilterCondition(const string symbol, ENUM_TIMEFRAMES timeframe, EntryStreamData* data)
      :ACondition(symbol, timeframe)
   {
      this.data = data;
      data.AddRef();
   }
   ~LongADXFilterCondition()
   {
      data.Release();
   }
   ...
   bool IsPass(const int period, const datetime date)
   {
      double adxValue = data.GetADX(period);
      return adxValue > 30 && adxValue < 45;
   }
};

class LongRSIFilterCondition : public ACondition
{
   EntryStreamData* data;
public:
   LongRSIFilterCondition(const string symbol, ENUM_TIMEFRAMES timeframe, EntryStreamData* data)
      :ACondition(symbol, timeframe)
   {
      this.data = data;
      data.AddRef();
   }
   ~LongRSIFilterCondition()
   {
      data.Release();
   }
   ...
   bool IsPass(const int period, const datetime date)
   {
      double rsiValue = data.GetRSI(period);
      return rsiValue > 50 && rsiValue < 70;
   }
};

class ShortADXFilterCondition : public ACondition
{
   EntryStreamData* data;
public:
   ShortADXFilterCondition(const string symbol, ENUM_TIMEFRAMES timeframe, EntryStreamData* data)
      :ACondition(symbol, timeframe)
   {
      this.data = data;
      data.AddRef();
   }
   ~ShortADXFilterCondition()
   {
      data.Release();
   }
...
class ShortRSIFilterCondition : public ACondition
{
   EntryStreamData* data;
public:
   ShortRSIFilterCondition(const string symbol, ENUM_TIMEFRAMES timeframe, EntryStreamData* data)
      :ACondition(symbol, timeframe)
   {
      this.data = data;
      data.AddRef();
   }
   ~ShortRSIFilterCondition()
   {
      data.Release();
   }
   ...
   {
      double rsiValue = data.GetRSI(period);
      return rsiValue > 30 && rsiValue < 50;
   }
};
...
ICondition* CreateLongFilterCondition(string symbol, ENUM_TIMEFRAMES timeframe, EntryStreamData* data)
{
   ...
   AndCondition* condition = new AndCondition();
   condition.Add(new LongADXFilterCondition(symbol, timeframe, data), false);
   condition.Add(new LongRSIFilterCondition(symbol, timeframe, data), false);
   return condition;
}
...
ICondition* CreateShortFilterCondition(string symbol, ENUM_TIMEFRAMES timeframe, EntryStreamData* data)
{
   ...
   AndCondition* condition = new AndCondition();
   condition.Add(new ShortADXFilterCondition(symbol, timeframe, data), false);
   condition.Add(new ShortRSIFilterCondition(symbol, timeframe, data), false);
   return condition;
}
