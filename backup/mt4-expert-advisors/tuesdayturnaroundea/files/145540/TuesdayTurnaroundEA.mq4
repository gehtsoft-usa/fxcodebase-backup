// If on Friday the DAX closes below its moving average in 34 days, a buy position is open, at market price, on Monday evening.
// The position is closed at the market price during the Tuesday evening.

// More information about this indicator can be found at:
// https://fxcodebase.com/code/viewtopic.php?f=38&t=72037
...
input int                emaClose_maPeriod       = 34;                            // Period
int                      emaClose_maShift        = 0;                             // Ma Shift
input ENUM_MA_METHOD     emaClose_maMethod       = MODE_EMA;                      // Method
...
input string       timeEnd                 = "16:00:00";                 // On Tuesday time to close (GMT time):
...
class BUYcondition1 : public iConditions
{
  public:
   bool evaluate()
   {
      ...
      if (iClose(NULL, PERIOD_D1, 1) < emaClose.index(1))
      {
         return true;
      }
      return false;
...
class SELLcondition2 : public iConditions
{
  public:
   bool evaluate()
   {
      // TODO: condition sell 2- ES LUNES
      if (DayOfWeek() == 1)
      {
         return true;
      }
...
class ConditionToCloseBuy : public iConditions
{
  public:
   bool evaluate()
   {
      ...
      // que cierre a una determinada hora marcada por el usuario
      if (DayOfWeek() == 2 && sesionControl.doSessionControl() == false)
      {
         return true;
      }
      return false;
...
int OnInit()
{
   ...
   // TODO: set Indicators
   emaClose = new MovingAverage(_Symbol, PERIOD_D1);
