...
#property strict
// Includes
enum ModeCalcLots { Money, AccountPercent, UserLots };

input string       T0                    = "== Trade Setup ==";    // == Trade Setup ==
input ModeCalcLots modeCalcLots          = Money;                  // Mode to Win:
input double       userMoney             = 10;                     // Money:
input double       userBalancePer        = 0.1;                    // Balance Percent:
input double       userLots              = 0.01;                   // Lots Initial
input int          userTPpips            = 20;                     // Pips TP
input int          userSLpips            = 20;                     // Pips SL
// input int          BR                    = 3;                      // Ratio Risk Reguard
...
input int    magico                = 1028;                   // Magic Number:
...
class LotCalculator
{
  ...
  public:
   ...
   double LotsByMoney(double Money, double Distance)
   {
      ...
      Print(__FUNCTION__," ","risk"," ",risk);
      Print(__FUNCTION__," ","Money"," ",Money);
      return CalculateLots(risk, Distance);
   }
...
class CustomConditionSELL : public iConditions
{
  public:
   bool evaluate()
   {
      double highDaily      = iHigh(Symbol(), PERIOD_D1, 0);
      double highLastCandle = iHigh(Symbol(), Period(), 1);
      if (highLastCandle == highDaily)
      {
         return true;
      }
...
CustomConditionSELL* lastHighEqualDailyHigh;
class CustomConditionSELL2 : public iConditions
{
  public:
   ...
   {
	double highLastCandle = iHigh(Symbol(),Period(),1);
	double highActualCandle = iHigh(Symbol(),Period(),0);
   double gap           = 2 * 10 * _Point;
	   if (highActualCandle >= highLastCandle - gap && highActualCandle <= highLastCandle + gap )
      {
         return true;
      }
      return false;
...
CustomConditionSELL2* testLastHigh;
class CustomConditionSELL3 : public iConditions
{
  public:
   bool evaluate()
   {
	double openActualCandle = iOpen(Symbol(),Period(),0);
   
	   if (Bid <= openActualCandle)
      {
         return true;
      }
      return false;
...
CustomConditionSELL3* returnToActualOpenSell;
...
class CustomConditionBUY : public iConditions
{
  public:
   bool evaluate()
   {
	double lowLastCandle = iLow(Symbol(),Period(),1);
	double lowDaily = iLow(Symbol(),PERIOD_D1,0);   
   if (lowDaily == lowLastCandle)
	{
		return true;
	}	
      return false;
...
CustomConditionBUY* lastLowEqualDailyLow;
class CustomConditionBUY2 : public iConditions
{
  public:
   ...
   {
	double lowLastCandle = iLow(Symbol(),Period(),1);
	double lowActualCandle = iLow(Symbol(),Period(),0);
	double gap           = 2 * 10 * _Point;
	if (lowActualCandle <= lowLastCandle + gap && lowActualCandle >= lowLastCandle - gap)
	{
		return true;
	}
...
CustomConditionBUY2* testLastLow;
class CustomConditionBUY3 : public iConditions
{
  public:
   bool evaluate()
   {
	double openActualCandle = iOpen(Symbol(),Period(),0);
	if (Ask == openActualCandle)
	{
		return true;
	}
...
CustomConditionBUY3* returnToActualOpenBuy;
...
int OnInit()
{
   ...
   conditionsToSell.AddCondition(lastHighEqualDailyHigh = new CustomConditionSELL());
   conditionsToSell.AddCondition(testLastHigh = new CustomConditionSELL2());
   conditionsToSell.AddCondition(returnToActualOpenSell = new CustomConditionSELL3());
   conditionsToSell.AddCondition(countSells = new ConditionCountSells());
   conditionsToBuy.AddCondition(lastLowEqualDailyLow = new CustomConditionBUY());
   conditionsToBuy.AddCondition(testLastLow = new CustomConditionBUY2());
   conditionsToBuy.AddCondition(returnToActualOpenBuy = new CustomConditionBUY3());
   conditionsToBuy.AddCondition(countBuys = new ConditionCountBuys());
...
double Lots()
{
   ...
   {
      case Money:
         return lotProvider.LotsByMoney(userMoney, userTPpips);
         break;

      case AccountPercent:
         return lotProvider.LotsByBalancePercent(userBalancePer, userTPpips);
         break;
