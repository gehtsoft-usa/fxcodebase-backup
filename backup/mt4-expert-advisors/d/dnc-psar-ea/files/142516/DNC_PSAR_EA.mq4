...
input int my_limit = 20; // Maximum Entry Limit Period
input double Step = 0.02; // SAR step
input double Max = 0.2; // SAR max
...
class DNCLongCondition : public ACondition
{
public:
   DNCLongCondition(const string symbol, ENUM_TIMEFRAMES timeframe)
      :ACondition(symbol, timeframe)
   {
   ...
   bool IsPass(const int period, const datetime date)
   {
      ...
      {
         double value = iCustom(_symbol, _timeframe, "DNC Continuation", n, AC, 0, period + i);
         if (value != 1)
         {
            return true;
         }
...
class DNCShortCondition : public ACondition
{
public:
   DNCShortCondition(const string symbol, ENUM_TIMEFRAMES timeframe)
      :ACondition(symbol, timeframe)
   {
   ...
   bool IsPass(const int period, const datetime date)
   {
      for (int i = 0; i < my_limit; ++i)
      {
         double value = iCustom(_symbol, _timeframe, "DNC Continuation", n, AC, 1, period + i);
         if (value != -1)
         {
            return true;
         }
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      ...
      return sarValue > iClose(_symbol, _timeframe, period);
   }
};
...
ICondition* CreateLongCondition(string symbol, ENUM_TIMEFRAMES timeframe)
{
   ...
   condition.Add(new DNCLongCondition(symbol, timeframe), false);
...
ICondition* CreateShortCondition(string symbol, ENUM_TIMEFRAMES timeframe)
{
   ...
   AndCondition* condition = new AndCondition();
   condition.Add(new DNCShortCondition(symbol, timeframe), false);
...
int OnInit()
{
   double temp = iCustom(NULL, 0, "DNC Continuation", 0, 0);
   if (GetLastError() == ERR_INDICATOR_CANNOT_LOAD)
   {
      Alert("Please, install the 'DNC Continuation' indicator");
      return INIT_FAILED;
   }
