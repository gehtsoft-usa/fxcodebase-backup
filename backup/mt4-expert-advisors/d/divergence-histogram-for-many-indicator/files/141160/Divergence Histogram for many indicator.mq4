...
//http://fxcodebase.com/code/viewtopic.php?f=38&t=71003
...
input int lrb = 5; // Pivot Point Period
input int mindiv = 1; // Minimum #Num of Divergence for Alert
input bool chcut = false; // Check Cut-Through in indicators
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      return totaldivUp[period] != EMPTY_VALUE && totaldivUp[period] >= mindiv;
   }
};
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      return totaldivDown[period] != EMPTY_VALUE && totaldivDown[period] <= -mindiv;
   }
};
...
double topc[], botc[], rsi[], macd[], moment[], cci[], stk[], DI[], maFast[], maSlow[], vwmacd[], cmf[], Mfi[];
int init()
{
   ...
   {
      Alert("Please, install the 'Volume Weighted Moving Average' indicator");
      return INIT_FAILED;
   }
   temp = iCustom(NULL, 0, "Chaikin money flow", 0, 0);
   if (GetLastError() == ERR_INDICATOR_CANNOT_LOAD)
   {
      Alert("Please, install the 'Chaikin money flow' indicator");
      return INIT_FAILED;
   }
   ...
   IndicatorShortName("Divergence Histogram for many indicator");
   IndicatorBuffers(23);
   SetIndexBuffer(0, totaldivUp);
   SetIndexBuffer(1, totaldivDown);
   ...
   SetIndexStyle(id, DRAW_NONE);
   SetIndexBuffer(id, topc);
   ++id;
   SetIndexStyle(id, DRAW_NONE);
   SetIndexBuffer(id, botc);
   ++id;
   SetIndexStyle(id, DRAW_NONE);
   SetIndexBuffer(id, rsi);
   ++id;
   SetIndexStyle(id, DRAW_NONE);
   SetIndexBuffer(id, macd);
   ++id;
   SetIndexStyle(id, DRAW_NONE);
   SetIndexBuffer(id, moment);
   ++id;
   SetIndexStyle(id, DRAW_NONE);
   SetIndexBuffer(id, cci);
   ++id;
   SetIndexStyle(id, DRAW_NONE);
   SetIndexBuffer(id, stk);
   ++id;
   SetIndexStyle(id, DRAW_NONE);
   SetIndexBuffer(id, DI);
   ++id;
   SetIndexStyle(id, DRAW_NONE);
   SetIndexBuffer(id, maFast);
   ++id;
   SetIndexStyle(id, DRAW_NONE);
   SetIndexBuffer(id, maSlow);
   ++id;
   SetIndexStyle(id, DRAW_NONE);
   SetIndexBuffer(id, vwmacd);
   ++id;
   SetIndexStyle(id, DRAW_NONE);
   SetIndexBuffer(id, cmf);
   ++id;
   SetIndexStyle(id, DRAW_NONE);
   SetIndexBuffer(id, Mfi);
   ++id;
                ...
                const int &spread[])
{
   ...
   {
      ArrayInitialize(totaldivUp, EMPTY_VALUE);
      ArrayInitialize(totaldivDown, EMPTY_VALUE);
      ArrayInitialize(topc, 0);
      ArrayInitialize(botc, 0);
      ArrayInitialize(rsi, EMPTY_VALUE);
      ArrayInitialize(macd, EMPTY_VALUE);
      ArrayInitialize(moment, EMPTY_VALUE);
      ArrayInitialize(cci, EMPTY_VALUE);
      ArrayInitialize(stk, EMPTY_VALUE);
      ArrayInitialize(DI, EMPTY_VALUE);
      ArrayInitialize(maFast, EMPTY_VALUE);
      ArrayInitialize(maSlow, EMPTY_VALUE);
      ArrayInitialize(vwmacd, EMPTY_VALUE);
   ...
   int toSkip = MathMax(lrb * 2, 1);
   for (int pos = MathMin(bars_limit, rates_total - 1 - MathMax(prev_calculated, toSkip)); pos >= 0 && !IsStopped(); --pos)
   {
      rsi[pos] = iRSI(_Symbol, _Period, 14, PRICE_CLOSE, pos);
      macd[pos] = iMACD(_Symbol, _Period, 12, 26, 9, PRICE_CLOSE, MODE_MAIN, pos) - iMACD(_Symbol, _Period, 12, 26, 9, PRICE_CLOSE, MODE_SIGNAL, pos);
      moment[pos] = iMomentum(_Symbol, _Period, 10, PRICE_CLOSE, pos);
      cci[pos] = iCCI(_Symbol, _Period, 10, PRICE_CLOSE, pos);
      stk[pos] = iStochastic(_Symbol, _Period, 14, 3, 3, MODE_SMA, 0, MODE_MAIN, pos);
      DI[pos] = (high[pos] - high[pos + 1]) + (low[pos] - low[pos + 1]);
      double maFast = iCustom(_Symbol, _Period, "Volume Weighted Moving Average", 12, 0, pos);
      double maSlow = iCustom(_Symbol, _Period, "Volume Weighted Moving Average", 26, 0, pos);
      vwmacd[pos] = maFast - maSlow;
      cmf[pos] = iCustom(_Symbol, _Period, "Chaikin money flow", 21, 0, pos);
      Mfi[pos] = iCustom(_Symbol, _Period, "MFI", 14, 0, pos);
      double top = pivothigh(pos, lrb, lrb);
      double bot = pivotlow(pos, lrb, lrb);
      topc[pos] = top != EMPTY_VALUE ? lrb : topc[pos + 1] + 1;
      botc[pos] = bot != EMPTY_VALUE ? lrb : botc[pos + 1] + 1;
      double newtop = pivothigh(pos, lrb, 0);
      bool emptyh = true;
      if (newtop != EMPTY_VALUE && newtop > high[pos + (int)topc[pos]])
      {
         double diff = (newtop - high[pos + (int)topc[pos]]) / topc[pos];
         double hline = newtop - diff;
         for (int x = 1; x < topc[pos]; ++x)
         {
            if (close[pos + x] > hline)
            {
               emptyh = false;
               break;
            }
      ...
      bool rsiokn = nocut1(rsi, topc[pos], pos);
      bool macdokn = nocut1(macd, topc[pos], pos);
      bool deltamacdokn = nocut1(macd, topc[pos], pos);
      bool momentokn = nocut1(moment, topc[pos], pos);
      bool cciokn = nocut1(cci, topc[pos], pos);
      bool stkokn = nocut1(stk, topc[pos], pos);
      bool vwmacdokn = nocut1(vwmacd, topc[pos], pos);
      bool cmfokn = nocut1(cmf, topc[pos], pos);
      bool mfiokn = nocut1(Mfi, topc[pos], pos);

      double newbot = pivotlow(pos, lrb, 0);
      bool emptyl = true;
      if (newbot != EMPTY_VALUE && newbot < low[pos + (int)botc[pos]])
      {
         double diff = (newbot - low[pos + (int)botc[pos]]) / botc[pos];
         double lline = newbot - diff;
         for (int x = 1; x < botc[pos]; ++x)
         {
            if (close[pos + x] < lline)
            {
               emptyl = false;
               break;
            }
      ...
      bool rsiokp = nocut2(rsi, botc[pos], pos);
      bool macdokp = nocut2(macd, botc[pos], pos);
      bool deltamacdokp = nocut2(macd, botc[pos], pos);
      bool momentokp = nocut2(moment, botc[pos], pos);
      bool cciokp = nocut2(cci, botc[pos], pos);
      bool stkokp = nocut2(stk, botc[pos], pos);
      bool vwmacdokp = nocut2(vwmacd, botc[pos], pos);
      bool cmfokp = nocut2(cmf, botc[pos], pos);
      bool mfiokp = nocut2(Mfi, botc[pos], pos);
      double indi1 = iff(posidiv(rsi, rsiokp, pos, emptyl, newbot), 1, iff(negadiv(rsi, rsiokn, pos, emptyh, newtop), -1, 0));
      double indi2 = iff(posidiv(macd, macdokp, pos, emptyl, newbot), 1, iff(negadiv(macd, macdokn, pos, emptyh, newtop), -1, 0));
      double indi3 = iff(posidiv(macd, deltamacdokp, pos, emptyl, newbot), 1, iff(negadiv(macd, deltamacdokn, pos, emptyh, newtop), -1, 0));
      double indi4 = iff(posidiv(moment, momentokp, pos, emptyl, newbot), 1, iff(negadiv(moment, momentokn, pos, emptyh, newtop), -1, 0));
      double indi5 = iff(posidiv(cci, cciokp, pos, emptyl, newbot), 1, iff(negadiv(cci, cciokn, pos, emptyh, newtop), -1, 0));
      double indi7 = iff(posidiv(stk, stkokp, pos, emptyl, newbot), 1, iff(negadiv(stk, stkokn, pos, emptyh, newtop), -1, 0));
      double indi9 = iff(posidiv(vwmacd, vwmacdokp, pos, emptyl, newbot), 1, iff(negadiv(vwmacd, vwmacdokn, pos, emptyh, newtop), -1, 0));
      double indi10 = iff(posidiv(cmf, cmfokp, pos, emptyl, newbot), 1, iff(negadiv(cmf, cmfokn, pos, emptyh, newtop), -1, 0));
      double indi11 = iff(posidiv(Mfi, mfiokp, pos, emptyl, newbot), 1, iff(negadiv(Mfi, mfiokn, pos, emptyh, newtop), -1, 0));
      //double indi6, indi8;
      double totaldiv = indi1 + indi2 + indi3 + indi4 + indi5 + indi7 + indi9 + indi10 + indi11;
      if (totaldiv > 0)
      {
         totaldivUp[pos] = totaldiv;
      }
      else
      {
         totaldivDown[pos] = totaldiv;
      }
      if (customStream != NULL)
      {
         customStream.SetValue(pos, totaldiv);
      }
...
bool posidiv(double &indi[], bool isok, int pos, bool emptyl, double newbot)
{
   return (emptyl && newbot != EMPTY_VALUE && indi[pos + (int)botc[pos]] < indi[pos] && (!chcut || isok));
}

bool nocut2(double &indi[], int len, int pos)
{
   double diff = (indi[pos] - indi[pos + len]) / len;
   double ln = indi[pos] - diff;
   for (int x = 1; x < len; ++x)
   {
      if (indi[pos + x] < ln)
      {
...
bool negadiv(double &indi[], bool isok, int pos, bool emptyh, double newtop)
{
   return (emptyh && newtop != EMPTY_VALUE && indi[pos + (int)topc[pos]] > indi[pos] && (!chcut || isok));
}

// check cut-through in indicators
bool nocut1(double &indi[], int len, int pos)
{
   double diff = (indi[pos] - indi[pos + len]) / len;
   double ln = indi[pos] - diff;
   for (int x = 1; x < len; ++x)
