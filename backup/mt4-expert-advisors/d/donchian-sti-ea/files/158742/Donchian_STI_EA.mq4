//Available @  https://fxcodebase.com/code/viewtopic.php?f=38&t=75749
...
#define Section_Custom_Indicator_STI
...
input const string indicator_file = "DonchianArrow";                          // Indicator File:
//---
int  bufferToBuy              = 0;
int  bufferToSell             = 1;
...
#ifdef Section_Custom_Indicator_STI
// MARK: Custom Indicator


input string       Tcustom2        = "==== Indicator Setup ===="; // ————————————————————————
input const string indicator_file2 = "STI_Trend Hunter";                          // Indicator File:
//---
int  bufferToBuy2              = 5;
int  bufferToSell2             = 6;
int  candles_back_for_signals2 = 1;
//---
double Indi2(int buffer, int candle = 1) { return iCustom(NULL, 0, indicator_file2, buffer, candle); }

class ConditionIndicatorBuy2 : public iConditions
{
    double lastSignal;
  ...
  public:
    bool evaluate()
    {
        for (int i = 1; i <= candles_back_for_signals2; i++) {
            double indi = Indi2(bufferToBuy2, i);
            if (indi > 0 && indi != EMPTY_VALUE && indi != lastSignal) {
                lastSignal = indi;
                return true;
            }
...
ConditionIndicatorBuy2 conditionIndicatorBuy2;

class ConditionIndicatorSell2 : public iConditions
{
    double lastSignal;
  ...
  public:
    ...
    {
        for (int i = 1; i <= candles_back_for_signals2; i++) {
            double indi = Indi2(bufferToSell2, i);
            if (indi > 0 && indi != EMPTY_VALUE && indi != lastSignal) {
                lastSignal = indi;
                return true;
            }
...
ConditionIndicatorSell2 conditionIndicatorSell2;

bool OnInit_CustomIndicator2()
{

    ShortStrategy.addCondition(&conditionIndicatorSell2);
    LongStrategy.addCondition(&conditionIndicatorBuy2);
