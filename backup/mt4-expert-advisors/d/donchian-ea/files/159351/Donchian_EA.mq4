...
// https://fxcodebase.com/code/viewtopic.php?f=38&p=159134#p159134
...
double SL_ByMoney(double money_sl, string side, string symbol, double lot)
{
    ...
    // FOREX
    if (_modeCalc == 0) {
        _pips = NormalizeDouble(money_sl / (lot * _tickValue), _digits);
            ...
            for (int i = OrdersTotal() - 1; i >= 0; i--)
            {
                if (OrderSelect(i, SELECT_BY_POS) && OrderSymbol() == Symbol())
                {
                    if (OrderType() == OP_BUY)
                    {
                        ...
                        {
                            OrderModify(OrderTicket(), OrderOpenPrice(), new_sl, OrderTakeProfit(), OrderExpiration(), clrNONE);
                        }
                    }
                    if (OrderType() == OP_SELL)
                    {
