...
//https://fxcodebase.com/code/viewtopic.php?f=38&t=73053
...
#ifdef CLOSE_ALL_ON
string       TtpOposite              = "== Close Opposite ==";     // ————————————
bool         closeAllInOpositeSignal = false;                       // Close all in opposite signal
...
// NOTE: GRID input
input string tGrid                 = "== Grid for SAME SIDE ==";     // ————————————
...
input bool   GridTPOn              = true;                           // TP On?
input double GridTpVariation       = 0;                              // TP percent: (+)Increase (-)Decrease:
input bool   GridSLOn              = true;                           // SL On?
input double GridSlVariation       = 0;                              // SL percent: (+)Increase (-)Decrease:
input bool   closeGridOn           = true;                           // Use Close Grid?
input double closeGridTP           = 100;                            // Take Profit Grid $ (zero deactive)
input double closeGridSL           = -100;                           // Stop Loss Grid -$ (zero deactive)
input string tGridA                = "== Grid for AGAINST SIDE ==";  // ————————————
input bool   a_GridON              = false;                          // Grid On:
input int    a_GridUser_maxCount   = 5;                              // Max attempts:
input double a_GridUser_maxLot     = 10;                             // Max lot value:
input double a_GridUser_multiplier = 1.5;                            // Multiplier:
input int    a_GridUser_gap        = 30;                             // Gap betwen orders (pips):
input bool   a_GridTPOn            = true;                           // TP On?
input double a_GridTpVariation     = 0;                              // TP percent: (+)Increase (-)Decrease:
input bool   a_GridSLOn            = true;                           // SL On?
input double a_GridSlVariation     = 0;                              // SL percent: (+)Increase (-)Decrease:
input bool   a_closeGridOn         = true;                           // Use Close Grid?
input double a_closeGridTP         = 100;                            // Take Profit Grid $ (zero deactive)
input double a_closeGridSL         = -100;                           // Stop Loss Grid -$ (zero deactive)
#else
...
class Grid
{
  ...
  double     tp_VariationPercent;
  double     sl_VariationPercent;
  bool       tp_on;
  bool       sl_on;

 public:
  Grid(string Symbol, string Side, double LastPrice, double Gap, double Multiplier, int MaxQnt, double MaxLot, double InitialLot, int magic, bool simbolFilterOn = true, bool magicFilterOn = true, double Tp_VariationPercent = 0, double Sl_VariationPercent = 0, bool Tp_on = false, bool Sl_on = false)
  {
    _symbol             = Symbol;
    _side               = Side;
    ...
    _nextLot            = nextLot();
    _magico             = magic;
    tp_VariationPercent = Tp_VariationPercent;
    sl_VariationPercent = Sl_VariationPercent;
    tp_on               = Tp_on;
    sl_on               = Sl_on;
  ...
  void   side(string inpside)             { _side = inpside;             }
  void   symbol(string inpsymbol)         { _symbol = inpsymbol;         }
  int    qnt()                            { return gridOrders.qnt();     }
  double nextLot(void)                    { return NormalizeDouble(_initialLot * pow(_multiplier, qnt()), 2); };
  ...
  void doGrid()
  {
    if (conditionsToOpenNewTrade.EvaluateConditions())
    {
      if (_side == "buy")
      {
        openTrade = new SendNewOrder("buy", Lots(), "", 0, SL("buy",_nextPrice), TP("buy",_nextPrice), _magico);
      ...
      if (_side == "sell")
      {
        openTrade = new SendNewOrder("sell", Lots(), "", 0, SL("sell", _nextPrice), TP("sell", _nextPrice), _magico);
        if (openTrade.doAction())
        {
          // Print("pointer de la ultima orden: ", openTrade.lastOrder());
          // if (gridOrders.AddOrder(openTrade.lastOrder()))
  ...
  double SL(string side, double price)
  {
		int pips = userTPpips;
		pips    *= 1 + (sl_VariationPercent/100*qnt()); 
		
		levelSL = new Levels(new ByFixPips(_Symbol, side, pips, "SL", price));
    double result  = levelSL.calculateLevel();
    delete levelSL;
	...
	double TP(string side, double price)
  {
		...
		// NOTE: 
		int pips  = userTPpips;
		pips     *= 1+(tp_VariationPercent/100*qnt()); 


    levelTP = new Levels(new ByFixPips(_Symbol, side, pips, "TP", price));
    double result  = levelTP.calculateLevel();
    delete levelTP;
...
class ConditionToCloseBuy : public iConditions
{
 public:
  bool evaluate()
  {
    ...
    {
      return sellCondition1.evaluate();
    }
...
void OnTick()
{
  ...
  if (GridON == true && CheckPointer(grid_SameSide) != POINTER_INVALID)
  {
    grid_SameSide.doGrid();
    mainOrders.GetMarketOrders();
  }
	if(a_GridON== true && CheckPointer(grid_Against) != POINTER_INVALID) 
	{ 
		grid_Against.doGrid();
	  mainOrders.GetMarketOrders();
	}
  ...
  if (GridON == true && closeGridOn == true ||a_GridON == true && a_closeGridOn == true)
  {
    doCloseGridControl();
  }
  ...
  if (!uTradeReverse)
  {
    if (conditionsToBuy.EvaluateConditions())
    {
      ...
      if (actionSendOrder.doAction())
      {
        ...
        if (GridON == true && CheckPointer(grid_SameSide) == POINTER_INVALID)        
				{
          grid_SameSide = new Grid(_Symbol, "buy", mainOrders.last().price(), GridUser_gap, GridUser_multiplier, GridUser_maxCount, GridUser_maxLot, mainOrders.last().lot(), magico,true, true, GridTpVariation, GridSlVariation, GridTPOn, GridSLOn);
          conditionsToBuy.AddCondition(gridActiveCondition = new ConditionGridActive(grid_SameSide));
        }
		
				if(a_GridON== true && CheckPointer(grid_Against) == POINTER_INVALID)
				{
					grid_Against = new Grid(_Symbol, "sell", mainOrders.last().price(), a_GridUser_gap, a_GridUser_multiplier, a_GridUser_maxCount, a_GridUser_maxLot, mainOrders.last().lot(), magico,true,true, a_GridTpVariation, a_GridSlVariation, a_GridTPOn, a_GridSLOn);
	         conditionsToSell.AddCondition(gridActiveCondition = new ConditionGridActive(grid_Against));
				}
    ...
    if (conditionsToSell.EvaluateConditions())
    {
      ...
      if (actionSendOrder.doAction())
      {
        ...
        {
					// NOTE: GRID SELL					
          grid_SameSide = new Grid(_Symbol, "sell", mainOrders.last().price(), GridUser_gap, GridUser_multiplier, GridUser_maxCount, GridUser_maxLot, mainOrders.last().lot(), magico,true,true, GridTpVariation, GridSlVariation, GridTPOn, GridSLOn);
          conditionsToSell.AddCondition(gridActiveCondition = new ConditionGridActive(grid_SameSide));
        }

				if(a_GridON == true && CheckPointer(grid_Against) == POINTER_INVALID)
				{
						grid_Against = new Grid(_Symbol, "buy", mainOrders.last().price(), a_GridUser_gap, a_GridUser_multiplier, a_GridUser_maxCount, a_GridUser_maxLot, mainOrders.last().lot(), magico,true,true, a_GridTpVariation, a_GridSlVariation, a_GridTPOn, a_GridSLOn);
	          conditionsToBuy.AddCondition(gridActiveCondition = new ConditionGridActive(grid_Against));
       	}
  ...
  if (uTradeReverse)
  {
    ...
    if (conditionsToSell.EvaluateConditions())
    {
      ...
      if (actionSendOrder.doAction())
      {
        ...
        if (GridON == true && CheckPointer(grid_SameSide) == POINTER_INVALID)
        {
          grid_SameSide = new Grid(_Symbol, "buy", mainOrders.last().price(), GridUser_gap, GridUser_multiplier, GridUser_maxCount, GridUser_maxLot, mainOrders.last().lot(), magico);
          conditionsToBuy.AddCondition(gridActiveCondition = new ConditionGridActive(grid_SameSide));
        }
    ...
    if (conditionsToBuy.EvaluateConditions())
    {
      ...
      if (actionSendOrder.doAction())
      {
        ...
        if (GridON == true && CheckPointer(grid_Against) == POINTER_INVALID)
        {
          Print(__FUNCTION__, " ", "Voy a setear la grid_Against");
          grid_Against = new Grid(_Symbol, "sell", mainOrders.last().price(), GridUser_gap, GridUser_multiplier, GridUser_maxCount, GridUser_maxLot, mainOrders.last().lot(), magico);
          Print(__FUNCTION__, " ", "pointer de la grid", GetPointer(grid_Against));
          conditionsToSell.AddCondition(gridActiveCondition = new ConditionGridActive(grid_Against));
        }
...
void CheckearOrdernesyGenerarGrids()
{
  if (mainOrders.qnt() == 1)
  {
    if (mainOrders.last().type() == OP_BUY && CheckPointer(grid_SameSide) == POINTER_INVALID)
    {
      grid_SameSide = new Grid(_Symbol, "buy", mainOrders.last().price(), GridUser_gap, GridUser_multiplier, GridUser_maxCount, GridUser_maxLot, mainOrders.last().lot(), magico);
    }
    if (mainOrders.last().type() == OP_SELL && CheckPointer(grid_Against) == POINTER_INVALID)
    {
      grid_Against = new Grid(_Symbol, "sell", mainOrders.last().price(), GridUser_gap, GridUser_multiplier, GridUser_maxCount, GridUser_maxLot, mainOrders.last().lot(), magico);
    }
  }
...
void deleteGrid()
{
  if (CheckPointer(grid_Against) != POINTER_INVALID)
  {
    delete grid_Against;
  }
  if (CheckPointer(grid_SameSide) != POINTER_INVALID)
  {
    delete grid_SameSide;
  }
}
...
void closeAll(string side)
{
  if (side == "buy")
  {
    ...
    if (GridON && CheckPointer(grid_SameSide) != POINTER_INVALID)
    {
      grid_SameSide.closeGrid();
      delete grid_SameSide;
    }
  ...
  if (side == "sell")
  {
    ...
    if (GridON && CheckPointer(grid_Against) != POINTER_INVALID)
    {
      grid_Against.closeGrid();
      delete grid_Against;
    }
...
{   
   if(closeGridTP>0 || a_closeGridTP>0)
   {
      if(CheckPointer(grid_SameSide) != POINTER_INVALID) if(grid_SameSide.profit() >= closeGridTP) {grid_SameSide.closeGrid(); delete grid_SameSide; }
			if(CheckPointer(grid_Against) != POINTER_INVALID) if(grid_Against.profit() >= a_closeGridTP) {grid_Against.closeGrid(); delete grid_Against; }
   }

   if(closeGridSL<0 || a_closeGridSL<0)
   {
      if(CheckPointer(grid_SameSide) != POINTER_INVALID)
      if(grid_SameSide.profit() <= closeGridSL) {grid_SameSide.closeGrid(); delete grid_SameSide; }
      
			if(CheckPointer(grid_Against) != POINTER_INVALID)
      if(grid_Against.profit() <= a_closeGridSL) {grid_Against.closeGrid(); delete grid_Against; }
   }
}
// clang-format on
...
double CurrentResult(string mode)
{
  double winners = 0;
  double lossers = 0;
  double all     = 0;
  ...
  for (int i = OrdersTotal() - 1; i >= 0; i--)
  {
    if (OrderSelect(i, SELECT_BY_POS) && OrderSymbol() == Symbol() && OrderMagicNumber() == magico)
    {
      double profit = OrderProfit() - OrderSwap() - OrderCommission();

      all += profit;
      if (profit > 0) winners += profit;
      if (profit < 0) lossers += profit;
    }
  }

  if (mode == "win") return winners;
  if (mode == "loss") return lossers;
