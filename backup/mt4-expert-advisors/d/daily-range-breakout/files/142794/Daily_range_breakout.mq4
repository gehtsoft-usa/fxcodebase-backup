...
//https://fxcodebase.com/code/viewtopic.php?f=38&t=71342
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      int index = iBarShift(_symbol, PERIOD_D1, date);
      if (index < 0)
      {
         return false;
      }
      return iHigh(_symbol, PERIOD_D1, index + 1) <= iClose(_symbol, _timeframe, period);
   }
};
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      ...
      return iLow(_symbol, PERIOD_D1, index + 1) >= iClose(_symbol, _timeframe, period);
   }
};
