//Available @  https://fxcodebase.com/code/posting.php?mode=reply&f=38&t=74734
...
// ------------------------------------------------------------------
input string ICustom        = "== Irchannel Setup ==";  // == Irchannel Setup ==
input int    uPeriodM1    = 150;                       // M1 Period:
input int    uPeriodM5    = 150;                       // M5 Period:
input int    uPeriodM15   = 150;                       // M15 Period:
input int    uPeriodM30   = 150;                       // M30 Period:
input int    uPeriodH1    = 150;                       // H1 Period:
input int    uPeriodH4    = 150;                       // H4 Period:
input int    uPeriodD1    = 150;                       // D1 Period:
input int    uPeriodW1    = 150;                       // W1 Period:
int Periods[8];

input string Iposition        = "== Pannel Position ==";  // == Pannel Position ==
input int uPositionx = 5; // Position X:
input int uPositiony = 30;// Position y:

// ------------------------------------------------------------------
// input string TitlePairs = "==== PAIRS ====";  // ————————————
// string uSymbols   = "GBPUSD,EURUSD, USDCAD";    // Symbols (add separate by comma ","):
string uSymbols   = "";  // solo para el simbolo actual
// ------------------------------------------------------------------
...
class Irchannel
{
 ...
 public:
  ...
  Irchannel(string Symbol, ENUM_TIMEFRAMES TimeFrame, int period)
  {
    _symbol = Symbol;
    _tf     = TimeFrame;
    _period = period;
  ...
  void setHandle()
  {
    _handle = iCustom(_symbol,_tf,"lrchannel.ex5",_period);
  }
...
// clang-format off
  ...
  double main(int shift)      { return calculate(shift, 0); }
  double upLine(int shift)    { return calculate(shift, 1); }
  double dnLine(int shift)    { return calculate(shift, 2); }
  double highLine(int shift)  { return calculate(shift, 3); }
  double lowLine(int shift)   { return calculate(shift, 4); }
};
...
class FinderPattern
{
 ...
 public:
  FinderPattern(string Symbol)
  {
    ...
    {
			handles[i] = new Irchannel(_symbol, TF[i], Periods[i]);
	  }
  }
  ~FinderPattern() {
	for(int i=0; i < ArraySize(handles);i++) 
  ...
  bool havePatternUp(int i, int tf_index)
  {
    // TODO: escribir el patrón alcista
		// Up
    // GREEN: show green if current price is below the LOWER lines of the LNR, 
    
      return  iClose(NULL, TF[tf_index], i) < handles[tf_index].dnLine(i);
  }

  bool havePatternDn(int i, int tf_index)
  {
    // TODO: escribir el patrón bajista
    //   RED: and to show red if current price is above the UPPER lines of the LNR. 
    return  iClose(NULL, TF[tf_index], i) > handles[tf_index].upLine(i);
  }
};
     ...
     if (id == CHARTEVENT_MOUSE_MOVE)
    {
      ...
      if (sparam == 1 && Focus(mousex, mousey))
      {
        EventChartCustom(0,PRIORITY_CONTROL,0, mousey, _name);
			...
			// if(WasPressed()==true){ MoveSpecial(mousey,sparam);}
			if(WasPressed()==true){ Move(mousey);}
...
int OnInit()
{
  ...
  Periods[0] = uPeriodM1;
  Periods[1] = uPeriodM5;
  Periods[2] = uPeriodM15;
  Periods[3] = uPeriodM30;
  Periods[4] = uPeriodH1;
  Periods[5] = uPeriodH4;
  Periods[6] = uPeriodD1;
  Periods[7] = uPeriodW1;
  
  string syms = uSymbols == "" ? Symbol() : uSymbols;
	symbols = new SymbolsList(syms);

  for (int i = symbols.ini(); i < symbols.qnt(); i = symbols.next())
  {
    AddFinder(symbols.currentSymbol());
    Print("add finder to: ", symbols.currentSymbol());
  }
...
void doActions()
{
	...
	for(int i=0;i < ArraySize(finders) ; i++)
	{
		...
		table.setValues(i,patternDirection);
	}
}
...
void OnChartEvent(const int id, const long& lparam, const double& dparam, const string& sparam)
{
  ...
  table.OnEvent(id, lparam, dparam, sparam);
}
