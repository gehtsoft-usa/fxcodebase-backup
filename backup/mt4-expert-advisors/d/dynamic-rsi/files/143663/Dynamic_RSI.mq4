...
// http://fxcodebase.com/code/viewtopic.php?f=27&t=68193
//+------------------------------------------------------------------------------------------------+
...
input int Look_Back_Length=60;
input int RSI_Period=14;
input double Buy_Zone_Probability=0.1;
input double Sell_Zone_Probability=0.1; 

enum MA_Types{ SMA=1,  EMA=2, SMMA=3,LWMA=4 };
 
input  MA_Types MA_Type = SMA;
 
input double OB_Level     = 70;
input double OS_Level     = 30;

input string Custom_Indicator = "Dynamic RSI";
...
int CreateAlert(int id, ICondition* condition, IAction* action, int code, string codeStr, string message, color clr, PriceType priceType, int sign)
{
   ...
   switch (Type)
   {
      ...
      case Text:
         {
            SimplePriceStream* highStream1 = new SimplePriceStream(_Symbol, (ENUM_TIMEFRAMES)_Period, priceType);
            highStream1.SetShift(shift_arrows_pips * sign);
            static int lastId2 = 1;
            id = conditions[size].RegisterArrows(id, message, IndicatorObjPrefix + IntegerToString(lastId2++), codeStr, clr, highStream1, font_size);
            highStream1.Release();
         }
         break;
   }
...
int init()
{
   ...
   IndicatorObjPrefix = GenerateIndicatorPrefix("drsi");
   IndicatorShortName("drsi");
...
int start() 
{
   ...
   //hipotenusa
      out = (MathArctan(A / C)*180.0/Pi)*(-1);

// Para voltar a valer dois RSI tem que colocar o [pos] na frente deles novamente
      double j=(WindowPriceMax()-WindowPriceMin())/20;
      if (out>0) drawtext(pos, High[pos]+j, DoubleToStr(out,0), ColorBull);
      if (out<0) drawtext(pos, High[pos]+j, DoubleToStr(out,0), ColorBull);

      if (customStream != NULL)
      {
         customStream.SetValue(pos, 50);
      }
