...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=73364
...
input int                maFilterPeriod       = 136;                            // Period
...
class FinderPattern
{
 ...
 public:
  ...
  bool havePatternUp(int i, ENUM_TIMEFRAMES tf)
  {
    ...
    maFast.Set_Symbol_TF(_symbol, tf);
    maSlow.Set_Symbol_TF(_symbol, tf);
    maFilter.Set_Symbol_TF(_symbol, tf);
    
    return maFast.index(i) > maSlow.index(i) && maFast.index(i + 1) <= maSlow.index(i + 1) 
      && maFast.index(i) > maFilter.index(i) && maSlow.index(i) > maFilter.index(i);
  }
