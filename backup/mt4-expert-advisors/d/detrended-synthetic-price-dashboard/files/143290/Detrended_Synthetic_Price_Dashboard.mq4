...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=71434
...
public:
   ...
   {
      double value = iCustom(_symbol, _timeframe, "Detrended Synthetic Price (oscillator)", DspPeriod, Price, SignalPeriod, ColorOn, 3, period);
...
public:
   ...
   virtual bool IsPass(const int period, const datetime date)
   {
      double value = iCustom(_symbol, _timeframe, "Detrended Synthetic Price (oscillator)", DspPeriod, Price, SignalPeriod, ColorOn, 5, period);
      return value != 0 && value != EMPTY_VALUE;
   }
};
...
string IndicatorName = "DSPS";
int init()
{
   double temp = iCustom(NULL, 0, "Detrended Synthetic Price (oscillator)", 0, 0);
   if (GetLastError() == ERR_INDICATOR_CANNOT_LOAD)
   {
      Alert("Please, install the 'Detrended Synthetic Price (oscillator)' indicator");
      return INIT_FAILED;
   }
   ...
   IndicatorObjPrefix = GenerateIndicatorPrefix("DSPS");
