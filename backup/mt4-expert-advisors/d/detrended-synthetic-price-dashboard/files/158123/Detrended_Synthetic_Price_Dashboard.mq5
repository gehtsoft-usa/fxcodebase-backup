//Available @  https://fxcodebase.com/code/viewtopic.php?f=38&t=71434
// +------------------------------------------------------------------------------------------------+
...
public:
   ...
   {
      indi = iCustom(symbol, timeframe, "Detrended Synthetic Price (oscillator)", DspPeriod, DspPrice, SignalPeriod, ColorOn);
   }
   ~UpCondition()
   {
   ...
   {
      double upLevel[1];
      if (CopyBuffer(indi, 0, period, 1, upLevel) != 1)
      {
         return false;
      }
      double value[1];
      if (CopyBuffer(indi, 2, period, 1, value) != 1)
      {
         return false;
      }
      return value[0] > upLevel[0];
   }
};
...
public:
   ...
   {
      double dnLevel[1];
      if (CopyBuffer(indi, 1, period, 1, dnLevel) != 1)
      {
         return false;
      }
      double value[1];
