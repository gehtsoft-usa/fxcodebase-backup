...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      double value = iCustom(_symbol, _timeframe, "Holy Grail075", PERIOD_CURRENT, Length, barsback, 1, period);
      return value == 1;
   }
};
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      double value = iCustom(_symbol, _timeframe, "Holy Grail075", PERIOD_CURRENT, Length, barsback, 0, period);
      Print(value);
      return value == 1;
   }
};
...
int OnInit()
{
   double temp = iCustom(NULL, 0, "Holy Grail075", 0, Length, barsback, 0, 0);
   if (GetLastError() == ERR_INDICATOR_CANNOT_LOAD)
   {
      Alert("Please, install the 'Holy Grail075' indicator");
      return INIT_FAILED;
   }
   #ifdef SHOW_ACCOUNT_STAT
      stats = NULL;
