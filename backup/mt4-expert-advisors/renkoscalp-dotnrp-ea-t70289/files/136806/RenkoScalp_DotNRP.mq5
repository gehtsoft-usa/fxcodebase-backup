...
//http://fxcodebase.com/code/viewtopic.php?f=38&t=70289
...
void OnInit()
{
   IndicatorObjPrefix = GenerateIndicatorPrefix("rsdnrp");
   IndicatorSetString(INDICATOR_SHORTNAME, "RenkoScalp DotNRP");
   ...
   lowStream.Release();
   highStream.Release();
   SetIndexBuffer(id, Li_40, INDICATOR_CALCULATIONS);
}
                ...
                const int &spread[])
{
   if (prev_calculated <= 0 || prev_calculated > rates_total)
   {
      ...
      ArrayInitialize(Li_40, EMPTY_VALUE);
      up.Init();
      down.Init();
   }
   int first = Length + 1;
   for (int pos = MathMax(first, prev_calculated - 1); pos < rates_total; ++pos)
   {
      ...
      int highestIndex = ArrayMaximum(high, pos - Length - 1, Length);
      double high_12 = high[highestIndex];
      int lowestIndex = ArrayMinimum(low, pos - Length - 1, Length);
      double low_4 = low[lowestIndex];
      Li_40[pos] = low_4;
      if (!(low[pos] < low_4 && high[pos] > high_12))
      {
         if (low[pos] < low_4)
         {
            Li_40[pos] = -1;
      ...
      up.Update(oldPos, time[pos]);
      down.Update(oldPos, time[pos]);
   }
   return rates_total;
}