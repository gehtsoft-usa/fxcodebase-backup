...
string file_custom_indicator = "IMACD.ex4";
...
enum ModeEntry { Market,
                 ...
                 PendingLimit };
// ——————————————————————————————————————————————————————————————————
input string Timacd       = "== IMACD Setup ==";  // ————————————
...
input TSLMode   userTslMode     = byMA;                      // TSL Mode:
input int    userTslInitialStep = 1;                           // TSL Initial Step:
input int    userTslStep        = 1;                           // TSL Step:
input int    userTslDistance    = 20;                          // TSL Distance:
input string          tTslByATR      = "-- TSL By ATR Setup --";  // -- TSL By ATR Setup --
input ENUM_TIMEFRAMES uTslATRTf      = PERIOD_H1;                 // TSL ATR time Frame:
input int             uTslATRPeriod  = 14;                        // TSL ATR Period:
int                   uTslATRShift   = 0;                         // TSL ATR Shift:
input double          uATRmultiplier = 2.5;                       // ATR multiplier:

input string             tTslByMA          = "-- TSL By MA Setup --";  // -- TSL By MA Setup --
input int                uInitialStep      = 5;                        // TSL MA Initial Step:
input int                uTslStep          = 1;                        // TSL MA Step:
input ENUM_TIMEFRAMES    userTslMaTf       = PERIOD_H1;                // TSL MA time Frame:
input int                tslMaPeriod       = 50;                       // TSL MA Period
input int                tslMaShift        = 0;                        // TSL MA Shift
input ENUM_MA_METHOD     tslMaMethod       = MODE_EMA;                 // TSL MA Method
input ENUM_APPLIED_PRICE tslMaAppliedPrice = PRICE_CLOSE;              // TSL MA Applied Price

input string          tTslByCandles     = "-- TSL By High/Low Candles Back --";  // --
input int             ByCandlesIniStep = 5;                                     // TSL Initial Step:
input int             ByCandlesStep     = 1;                                     // TSL Step:
input ENUM_TIMEFRAMES ByCandlesTf         = PERIOD_CURRENT;                        // TSL TF:
input int             candlesBack  = 10;                                    // TSL Candles Back:
...
class TslByATR : public iTSL
{
  ...
  public:
   TslByATR(ENUM_TIMEFRAMES TslATRTf, int TslATRPeriod, int TslATRShift, double ATRmultiplier)
   {
      _Tf     = TslATRTf;
      _Period = TslATRPeriod;
      _Shift  = TslATRShift;
      _m      = ATRmultiplier;
   }
   ~TslByATR(){ ;}

   double atr(Order* order)
   {
      double digits = SymbolInfoInteger(order.symbol(), SYMBOL_DIGITS);
      double atr = NormalizeDouble(iATR(order.symbol(), _Tf, _Period, _Shift),digits);
      return atr;
   }

   void setInitialStep(Order* order)
   {
      double pointsToMove = atr(order) * _m;
      if (order.type() == OP_SELL)
      {
         pointsToMove *= -1;
      }
      double initialStep = order.price() + pointsToMove;
      order.tslNext(order.price() + pointsToMove);
   }
   ...
   void setNextStep(Order* order)
   {
      ...
      double tslNext = order.tslNext() + pointsToMove;
...
class TslByMA : public iTSL
{
 ...
 public:
  TslByMA(int uInitialStep, int uTslStep, ENUM_TIMEFRAMES MaTf, int MaPeriod, int MaShift, ENUM_MA_METHOD MaMethod, ENUM_APPLIED_PRICE MaAppliedPrice)
  {
    _InitialStep  = uInitialStep;
    _TslStep      = uTslStep;
    _Tf           = MaTf;
    _Period       = MaPeriod;
    _Shift        = MaShift;
    _Method       = MaMethod;
    _AppliedPrice = MaAppliedPrice;
  }
  ~TslByMA(){;}

  double ma(Order* order)
  {
    double digits = SymbolInfoInteger(order.symbol(), SYMBOL_DIGITS);
    double ma     = NormalizeDouble(iMA(order.symbol(), _Tf, _Period, _Shift, _Method, _AppliedPrice, 1), digits);
    return ma;
  }
...
class TslByCandles : public iTSL
{
  int                _InitialStep;
  int                _TslStep;
  ENUM_TIMEFRAMES    _Tf;
 ...
 public:
  TslByCandles(int InitialStep, int TslStep, ENUM_TIMEFRAMES tf, int CandlesBack)
  {
    _InitialStep  = InitialStep;
    _TslStep      = TslStep;
    _Tf           = tf;
    _candles      = CandlesBack;
  }
  ~TslByCandles() { ; }
  ...
  double newSL(Order* order)
  {
    ...
    if (order.type() == OP_BUY)
    {
      double lowest = iLow(order.symbol(), _Tf, iLowest(order.symbol(), _Tf,MODE_LOW, _candles, 1));

      if ((lowest > order.sl()) || order.sl()==0)
      {
        newsl = lowest;
      }
    }
    ...
    if (order.type() == OP_SELL)
    {
      double highest = iHigh(NULL, 0, iHighest(order.symbol(), _Tf,MODE_HIGH, _candles, 1));

      if ((highest < order.sl()) || order.sl()==0)
      {
        newsl = highest;
      }
    }
  ...
  TrailingStop(OrdersList* uOrders, TSLMode mode)
  {
    ...
    switch (mode)
    {
      case byPips:
        ...
        break;
      case byMA:
        _TslMode = new TslByMA(uInitialStep, uTslStep,userTslMaTf, tslMaPeriod, tslMaShift, tslMaMethod, tslMaAppliedPrice);
        break;
      case byATR:
        _TslMode = new TslByATR(uTslATRTf, uTslATRPeriod, uTslATRShift, uATRmultiplier);
	      break;
      case byCandles:
        _TslMode = new TslByCandles(ByCandlesIniStep, ByCandlesStep, ByCandlesTf, candlesBack);
	      break;
    }
  }
  ~TrailingStop()
...
class BUYcondition1 : public iConditions
{
 public:
  bool evaluate()
  {
    ...
    double macd1 = iCustom(NULL, 0, file_custom_indicator, FastMAPeriod, SlowMAPeriod, SignalMAPeriod, 0, 1);
    double signal1 = iCustom(NULL, 0, file_custom_indicator, FastMAPeriod, SlowMAPeriod, SignalMAPeriod, 1, 1);
    double macd2 = iCustom(NULL, 0, file_custom_indicator, FastMAPeriod, SlowMAPeriod, SignalMAPeriod, 0, 2);
    double signal2 = iCustom(NULL, 0, file_custom_indicator, FastMAPeriod, SlowMAPeriod, SignalMAPeriod, 1, 2);

    return macd1 > signal1 && macd2 <= signal2;
    // return false;
  }
};
