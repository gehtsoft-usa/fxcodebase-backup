...
//http://fxcodebase.com/code/viewtopic.php?f=38&t=69345
...
input double lots_multiplicator = 2; // Position size multiplicator
...
input TradingSide trading_side = BothSides; // What trades should be taken
   ...
   input bool position_cap = true; // Position Cap
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      double value = iCustom(_symbol, _timeframe, "MACD_Histogram", FastMAPeriod, SlowMAPeriod, SignalMAPeriod, 3, period + 2);
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      double value = iCustom(_symbol, _timeframe, "MACD_Histogram", FastMAPeriod, SlowMAPeriod, SignalMAPeriod, 4, period + 2);
      return value != 0 && value != EMPTY_VALUE;
   }
};
...
class MultiplicatorLotsProvider : public ILotsProvider
{
   TradingCalculator* _calculator;
public:
   MultiplicatorLotsProvider(TradingCalculator* calculator)
   {
      _calculator = calculator;
   }
   ...
   virtual double GetLots(double stopLoss)
   {
      return _calculator.GetLots(lots_type, current_lot, 0.0);
   }
};
   ...
   ENUM_TIMEFRAMES timeframe, bool isBuy)
{
   ...
   switch (lots_type)
   {
      case PositionSizeRisk:
         break;
      default:
         lots = new MultiplicatorLotsProvider(tradingCalculator);
         break;
   }
...
public:
   virtual bool DoAction()
   {
      ...
      {
         current_lot = instrument.NormalizeLots(current_lot * lots_multiplicator);
      }
      else
      {
...
int OnInit()
{
   ...
   current_lot = lots_value;
   double temp = iCustom(NULL, 0, "MACD_Histogram", 0, 0);
   if (GetLastError() == ERR_INDICATOR_CANNOT_LOAD)
   {
      Alert("Please, install the 'MACD_Histogram' indicator");
      return INIT_FAILED;
   }
   #ifdef SHOW_ACCOUNT_STAT
      stats = NULL;
