...
enum StopLimitType
{
   ...
   StopLossType stop_loss_type = SLPips; // Stop loss type
...
public:
   ...
   virtual double GetValue(const int period, double entryPrice)
   {
      InstrumentInfo instrument(_calculator.GetSymbol());
      double value0 = iCustom(_symbol, _timeframe, "Fractals average breakout Indicator", Loopback, _isBuy ? 2 : 0, period + 1);
      return instrument.AddPips(value0, _isBuy ? -stop_loss_value : stop_loss_value);
   }
};
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      double value0 = iCustom(_symbol, _timeframe, "Fractals average breakout Indicator", Loopback, 0, period + 1);
      return iClose(_symbol, _timeframe, period) >= value0;
   }
};
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      double value0 = iCustom(_symbol, _timeframe, "Fractals average breakout Indicator", Loopback, 2, period + 1);
      return iClose(_symbol, _timeframe, period) <= value0;
   }
};
...
int OnInit()
{
   double temp = iCustom(NULL, 0, "Fractals average breakout Indicator", 0, 0);
   if (GetLastError() == ERR_INDICATOR_CANNOT_LOAD)
   {
      Alert("Please, install the 'Fractals average breakout Indicator' indicator");
      return INIT_FAILED;
   }
   #ifdef SHOW_ACCOUNT_STAT
      stats = NULL;
