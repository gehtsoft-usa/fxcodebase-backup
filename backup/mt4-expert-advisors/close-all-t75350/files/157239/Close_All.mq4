...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=75350
...
#define Section_CloseByAccountPercent
#ifdef Section_CloseByAccountPercent
// Mark: Section_CloseByAccountPercent

input string tawlir           = "== Actions when limit is reached =="; // ————————————————————————
input bool   deletePendingsOn = true;                                  // Delete Pendings
input bool   closePlatformOn  = true;                                  // Close Platform

// cp = close percent
input string tcp             = "== Close all by account percent =="; // ————————————————————————
input bool   cp_on           = true;                                 // Close all by account on?
input double cp_profit_limit = 0.1;                                  // Profit Limit (0 = off)
input double cp_loss_limit   = 0;                                    // Loss Limit (0 = off)

class ConditionCloseByAccountPercent : public iConditions
{
  public:
    bool evaluate()
    {
        ...
        double acc_percent_total = (profitAcum / acc) * 100;
        if (acc_percent_total >= cp_profit_limit && cp_profit_limit != 0) {
            return true;
        }
        if (fabs(acc_percent_total) >= fabs(cp_loss_limit) && cp_loss_limit != 0 && acc_percent_total < 0) {
            return true;
        }
...
ConditionCloseByAccountPercent cdCloseByAccountPercentAll;

Strategy stCloseByAccountPercentAll;

void OnInit_CloseByAccountPercent()
{
    if(!cp_on) return;
    stCloseByAccountPercentAll.set(&cdCloseByAccountPercentAll, &acCloseAll);
    
    if(deletePendingsOn)
        stCloseByAccountPercentAll.addAction(&acDeletePendings);
    if(closePlatformOn)
        stCloseByAccountPercentAll.addAction(&acClosePlatform);

    Trader.strategys.add(&stCloseByAccountPercentAll);
...
input bool   closeAll_on  = false;                      // Close all by money on?
...
void OnInit_CloseByMoney()
{
    ...
    if(closePlatformOn)
        stCloseByMoney.addAction(&acClosePlatform);

    Trader.strategys.add(&stCloseByMoney);
}
...
int OnInit()
{
    ...
    // Trader.management.set(_Symbol);

    OnInit_CloseByAccountPercent();
    OnInit_CloseByMoney();
