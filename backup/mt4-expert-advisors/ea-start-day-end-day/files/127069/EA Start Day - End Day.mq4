...
// http://fxcodebase.com/code/viewtopic.php?f=38&t=68597
...
input double pips_for_opposite = 300; // Open oppisite on profit, pips
...
class OpenOppositeAction : public AAction
{
   IOrder* _order;
public:
   OpenOppositeAction(IOrder* order)
   {
      _order = order;
      _order.AddRef();
   }
   ...
   virtual bool DoAction()
   {
      ...
      int type = OrderType();
      MarketOrderBuilder orderBuilder;
      orderBuilder.SetInstrument(OrderSymbol())
         .SetAmount(OrderLots() * 2)
         .SetSlippage(slippage_points)
         .SetMagicNumber(magic_number)
         .SetComment("Opposite trade");
      if (type == OP_BUY)
         orderBuilder.SetSide(SellSide);
      else if (type == OP_SELL)
         orderBuilder.SetSide(BuySide);
      else
         return false;

      string error;
      int order = orderBuilder.Execute(error);
      if (order == -1)
      {
      ...
      if (_longCondition.IsPass(tradePeriod))
      {
         for (int i = 0; i < ArraySize(_longMoneyManagement); ++i)
         {
            ...
            if (order >= 0)
            {
               ...
               int isBuy = true;
               double basePrice = OrderOpenPrice();
               double triggerValue = _calculator.CalculateTakeProfit(isBuy, pips_for_opposite, StopLimitPips, OrderLots(), basePrice);
               IOrder *orderEntity = new OrderByMagicNumber(OrderMagicNumber());
               HitProfitCondition* condition = new HitProfitCondition();
               condition.Set(orderEntity, triggerValue);
               
               IAction* action = new OpenOppositeAction(orderEntity);
               if (!_actions.AddActionOnCondition(action, condition))
               {
                  delete action;
                  delete condition;
...
ICondition* CreateLongCondition(string symbol, ENUM_TIMEFRAMES timeframe, TradingTime* tradingTime)
{
   if (trading_side == ShortSideOnly)
      return (ICondition *)new DisabledCondition();

   return (ICondition *)new LongCondition(symbol, timeframe, tradingTime);
}
...
TradingController *CreateController(const string symbol, const ENUM_TIMEFRAMES timeframe, string &error)
{
   ...
   ICondition *longCondition = CreateLongCondition(symbol, timeframe, tradingTime);
   ...
   ICondition *exitLongCondition = new ExitLongCondition(symbol, timeframe, tradingTime);
   ICondition *exitShortCondition = new ExitShortCondition(symbol, timeframe, tradingTime);
