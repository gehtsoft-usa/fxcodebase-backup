...
class Strategy
{
  ...
  public:
    ...
    void addCondition(iConditions* aCondition)
    {
        conditions.add(aCondition);
    }
    void addAction(iActions* Action)
    {
        actions.add(Action);
    }
...
void                   oninit_basic_conditions()
{
    Trader.filters.add(&conditionOncePerCandle);
}
...
double Price(string side, int pips = 0, string _symbol = "")
{
    ...
    if (side == "buy") {
        switch (modeEntry) {
        case PendingStop : price += distance; break;
        case PendingLimit: price -= distance; break;
        }
    }
...
double StopLoss(const double price, const string side, const string symbol, const double lot)
{
    ...
    case sl_null    : return 0.00;
    case sl_fix_pips: return StopLossByPips(price, side, symbol);

    #ifdef sl_money_on
    case sl_money: return StopLossByMoney(price, side, symbol, lot);
    #endif

    #ifdef sl_candles_on
    case sl_candles: return StopLossByCandles(side, symbol);
    #endif
    }
...
input stop_loss_mode sl_mode        = sl_candles;        // SL Mode:
...
double TakeProfit(const double price, const string side, const string symbol, const double lot)
{
    ...
    switch (tp_mode) {
    case tp_null    : return 0.00;
    case tp_fix_pips: return TakeProfitByPips(price, side, symbol);

    #ifdef tp_money_on
    case tp_money: return TakeProfitByMoney(price, side, symbol, lot);
    #endif

    #ifdef tp_ratio_on
    case tp_by_ratio: return TakeProfitByRatio(price, side, symbol);
    #endif
    }
...
input bool   close_on_opposite_flat_on = true;                      // Close On Opposite by Flat Trend:

class ConditionCloseOnOppositeBuy_flat : public iConditions
{
  public:
    bool evaluate()
    {
        bool r = false;
        if (OrderType() == OP_BUY) {
            r = cd_flat_trend_sell.evaluate();
        }
        return r;
    }
};
ConditionCloseOnOppositeBuy_flat cdCloseOnOppositeBuy_flat;

class ConditionCloseOnOppositeSell_flat : public iConditions
{
  public:
    bool evaluate()
    {
        bool r = false;
        if (OrderType() == OP_SELL) {
            r = cd_flat_trend_buy.evaluate();
        }
        return r;
    }
};
ConditionCloseOnOppositeSell_flat cdCloseOnOppositeSell_flat;

Strategy                          closeOppositeBuy_flat(&cdCloseOnOppositeBuy_flat, &acCloseBuy);
Strategy                          closeOppositeSell_flat(&cdCloseOnOppositeSell_flat, &acCloseSell);

void                              oninit_close_on_opposite_flat()
{
    if (close_on_opposite_flat_on) {
        Trader.management.add(&closeOppositeBuy_flat);
        Trader.management.add(&closeOppositeSell_flat);
    }
}


//---
input bool close_on_opposite_super_on = true; // Close On Opposite by Super Trader Trend:

class ConditionCloseOnOppositeBuy_super : public iConditions
{
  public:
    bool evaluate()
    {
        ...
        if (OrderType() == OP_BUY) {
            ...
            r = cd_super_trader_sell.evaluate();
        }
        return r;
    }
};
ConditionCloseOnOppositeBuy_super cdCloseOnOppositeBuy_super;

class ConditionCloseOnOppositeSell_super : public iConditions
{
  public:
    bool evaluate()
    {
        bool r = false;
        if (OrderType() == OP_SELL) {
            r = cd_super_trader_buy.evaluate();
        }
        return r;
    }
};
ConditionCloseOnOppositeSell_super cdCloseOnOppositeSell_super;

Strategy                           closeOppositeBuy_super(&cdCloseOnOppositeBuy_super, &acCloseBuy);
Strategy                           closeOppositeSell_super(&cdCloseOnOppositeSell_super, &acCloseSell);

void                               oninit_close_on_opposite_super()
{
    if (close_on_opposite_super_on) {
        Trader.management.add(&closeOppositeBuy_super);
        Trader.management.add(&closeOppositeSell_super);
    }
}
...
input bool   close_by_strategy_on = false;                   // Close positions by RSI:
input int    rsi_period           = 8;                       // RSI periods
  ...
  public:
    bool evaluate()
    {
        ...
        if (OrderType() == OP_BUY) {
            ...
            r = iRSI(NULL, 0, rsi_period, PRICE_CLOSE, 2) >= 70 && iRSI(NULL, 0, rsi_period, PRICE_CLOSE, 1) < 70;
        }
        return r;
    }
};
  ...
  public:
    bool evaluate()
    {
        ...
        if (OrderType() == OP_SELL) {
            ...
            r = iRSI(NULL, 0, rsi_period, PRICE_CLOSE, 2) <= 30 && iRSI(NULL, 0, rsi_period, PRICE_CLOSE, 1) > 30;
        }
        return r;
    }
};
...
input bool   partial1_on     = true;                  // Partial on RSI:
input double percentToClose1 = 50;                    // Percent to close:
bool         partial2_on     = false;                 // Partial 2:
double       percentToClose2 = 50;                    // Percent to close


class ConditionPartial1 : public iConditions
{
  public:
    bool evaluate()
    {
        // TODO: condition to close partial 1
        bool r = false;
        if (OrderType() == OP_BUY)
            r = iRSI(NULL, 0, rsi_period, PRICE_CLOSE, 2) >= 70 && iRSI(NULL, 0, rsi_period, PRICE_CLOSE, 1) < 70;
        if (OrderType() == OP_SELL)
...
ConditionPartial1 cdPartial1;

class ConditionPartial2 : public iConditions
{
  public:
    bool evaluate()
    {
        // TODO: condition to close partial 2
        bool r = false;
        if (OrderType() == OP_BUY)
            r = iRSI(NULL, 0, rsi_period, PRICE_CLOSE, 2) >= 70 && iRSI(NULL, 0, rsi_period, PRICE_CLOSE, 1) < 70;
        if (OrderType() == OP_SELL)
...
ConditionPartial2 cdPartial2;

class PartialClose1 : public iActions
{
    int tk_list_partial_1[];
  ...
  public:
    ...
    double lots(int n = 1)
    {
        ...
        if (n == 1) {
            l = NormalizeDouble(l * percentToClose1 / 100, 2);
        }
        return l;
    }
    ...
    bool execute()
    {
        ...
        if (OrderType() == OP_BUY) {
            if (OrderClose(tk, lots(1), price(), 10000, Green)) {
                savetk(newtk(tk));
                return true;
            }
        }
    ...
    bool CheckList(int tk)
    {
        int p = 0;
        p     = ArrayBsearch(tk_list_partial_1, tk);
        if (p < 0)
            return false;
        return tk_list_partial_1[p] == tk;
        return false;
    }
    ...
    void savetk(int tk)
    {
        ...
        int t = ArraySize(tk_list_partial_1);
        if (ArrayResize(tk_list_partial_1, t + 1)) {
            tk_list_partial_1[t] = tk;
        }
    }
...
PartialClose1 acPartialClose1;

class PartialClose2 : public iActions
{
    int tk_list_partial_2[];
  ...
  public:
    ...
    double lots(int n = 2)
    {
        ...
        if (n == 2) {
            l = OrderLots() / (percentToClose1 / 100);
            l = NormalizeDouble(l * percentToClose2 / 100, 2);
        }
        return l;
    }
    ...
    bool execute()
    {
        ...
        if (OrderType() == OP_BUY) {
            if (OrderClose(tk, lots(2), price(), 10000, Green)) {
                savetk(newtk(tk));
                return true;
            }
        }
        ...
        if (OrderType() == OP_SELL) {
            if (OrderClose(tk, lots(2), price(), 10000, Green)) {
                savetk(newtk(tk));
                acPartialClose1.savetk(newtk(tk));
                return true;
            }
        }
        return false;
    ...
    {
        int p = 0;
        p     = ArrayBsearch(tk_list_partial_2, tk);
        if (p < 0)
            return false;
        return tk_list_partial_2[p] == tk;
    }

    void savetk(int tk)
    {
        int t = ArraySize(tk_list_partial_2);
        if (ArrayResize(tk_list_partial_2, t + 1)) {
            tk_list_partial_2[t] = tk;
        }
    }
...
PartialClose2 acPartialClose2;

Strategy      stPartialClose1(&cdPartial1, &acPartialClose1);
Strategy      stPartialClose2(&cdPartial2, &acPartialClose2);

void          oninit_Partials()
{
    Trader.management.add(&stPartialClose1);
    // Trader.management.add(&stPartialClose2);
}
...
class OrderCreator : public iActions
{
  ...
  public:
    ...
    void setLots()
    {
        _lots = ::LotsCalculation();
    }

    void setStopLoss()
    {
        _sl = ::StopLoss(_price, _side, _symbol, _lots);
    }

    void setTakeProfit()
    {
        _tp = ::TakeProfit(_price, _side, _symbol, _lots);
    }
...
Strategy           LongStrategy(&conditionBuy, &buy);
Strategy           ShortStrategy(&conditionSell, &sell);
Strategy           StrategyToCloseBuy(&conditionCloseBuy, &acCloseBuy);
Strategy           StrategyToCloseSell(&conditionCloseSell, &acCloseSell);
...
#define Section_Super_Trader_Trend
#ifdef Section_Super_Trader_Trend
// MARK: super trend indicator
...
input const string supertradertrend_file = "SuperTraderTrend.ex4";      // Indicator File:
input bool supertradertrend_on = true; // Supertradertrend On:
int                super_bufferToBuy     = 0;
int                super_bufferToSell    = 1;
int                super_bufferTrend     = 2; // puede ser 1 o -1
bool               super_indicator_init  = true;


double             super_trader_trend(int buffer, int candle = 1)
{
    return iCustom(NULL, 0, supertradertrend_file, buffer, candle);
}

class CDSuperTraderTrendBuy : public iConditions
{
  public:
    bool evaluate()
    {
        double indi = super_trader_trend(super_bufferToBuy, 1);
        // double t    = super_trader_trend(super_bufferTrend, 1);
        return indi > 0 && indi != EMPTY_VALUE;
    }
};
CDSuperTraderTrendBuy cd_super_trader_buy;

class CDSuperTraderTrenSell : public iConditions
{
  public:
    bool evaluate()
    {
        double indi = super_trader_trend(super_bufferToSell, 1);
        // double t    = super_trader_trend(super_bufferTrend, 1);
        return indi > 0 && indi != EMPTY_VALUE;
    }
};
CDSuperTraderTrenSell cd_super_trader_sell;

Strategy              st_indicators_long;
Strategy              st_indicators_short;


bool                  OnInit_SuperTraderTrend_Indicator()
{
    double temp = iCustom(NULL, 0, supertradertrend_file, 0, 0);
    if (GetLastError() == ERR_INDICATOR_CANNOT_LOAD) {
        MessageBox("THIS EA NEED AN INDICATOR\n install the file:\n" + supertradertrend_file + "\ninto the folder:\nMQL4/Indicators.", "Important Information", MB_ICONINFORMATION);
        Alert("THIS EA NEED AN INDICATOR, install the file: " + supertradertrend_file + " into the folder: MQL4/Indicators.");
        super_indicator_init = false;
    }

    if (!super_indicator_init)
        return false;

    if(supertradertrend_on){
        st_indicators_long.addCondition(&cd_super_trader_buy);
        st_indicators_short.addCondition(&cd_super_trader_sell);
    }
...
#ifdef Section_flat_trend
// MARK: flat trend indicator

input const string flat_trend_file         = "FlatTrend w MACD_alertsfix (1).ex4"; // Indicator File:
input int          Minutes                 = 60;
input int          MACD_Fast               = 8;
input int          MACD_Slow               = 21;
input int          MACD_MA                 = 9;

int                flat_trend_bufferToSell = 0;
int                flat_trend_bufferToBuy  = 1;
bool               flat_trend_init         = true;


double             FlatTrend(int buffer, int candle = 1)
{
    return iCustom(NULL, 0, flat_trend_file,
                   Minutes,
                   MACD_Fast,
                   MACD_Slow,
                   MACD_MA,
...
class CDFlatTrendBuy : public iConditions
{
    bool newSignal;
  ...
  public:
    ...
    {
        bool r;
        double indi = FlatTrend(flat_trend_bufferToBuy, 1);
        double indi2 = super_trend_cci(0,1);
        
        if(!supertrendcci_on)
        r = indi > 0 && indi != EMPTY_VALUE;
        else
        r = indi > 0 && indi != EMPTY_VALUE && indi2 > 0 && indi2 != EMPTY_VALUE;
        
        if(r== true && newSignal==true){
            newSignal = false;        
            return true;
        } 
        if(r== false){
...
CDFlatTrendBuy cd_flat_trend_buy;

class CDFlatTrendSell : public iConditions
{
    bool newSignal;
  public:
    bool evaluate()
    {
        bool r;
        double indi = FlatTrend(flat_trend_bufferToSell, 1);
        double indi2 = super_trend_cci(1,1);
...
CDFlatTrendSell cd_flat_trend_sell;



bool            OnInit_CustomIndicator()
{
    double temp = iCustom(NULL, 0, flat_trend_file, 0, 0);
    if (GetLastError() == ERR_INDICATOR_CANNOT_LOAD) {
        MessageBox("THIS EA NEED AN INDICATOR\n install the file:\n" + flat_trend_file + "\ninto the folder:\nMQL4/Indicators.", "Important Information", MB_ICONINFORMATION);
        Alert("THIS EA NEED AN INDICATOR, install the file: " + flat_trend_file + " into the folder: MQL4/Indicators.");
        flat_trend_init = false;
    }

    if (!flat_trend_init)
        return false;
    st_indicators_long.addCondition(&cd_flat_trend_buy);
    st_indicators_short.addCondition(&cd_flat_trend_sell);

    st_indicators_long.addAction(&buy);
    st_indicators_short.addAction(&sell);

    Trader.strategys.add(&st_indicators_long);
    Trader.strategys.add(&st_indicators_short);
...
input const string supertrendcci_file = "!!supertrend - cci histo (mtf + arrows + alerts).ex4";      // Indicator File:
input bool supertrendcci_on = true; // Supertrendcci On:
int                supercci_bufferToBuy     = 0;
int                supercci_bufferToSell    = 1;
 bool               supercci_indicator_init  = true;


double             super_trend_cci(int buffer, int candle = 1)
{
    return iCustom(NULL, 0, supertrendcci_file, buffer, candle);
}
 
bool                  OnInit_SuperTrend_CCI_Indicator()
{
    double temp = iCustom(NULL, 0, supertrendcci_file, 0, 0);
    if (GetLastError() == ERR_INDICATOR_CANNOT_LOAD) {
        MessageBox("THIS EA NEED AN INDICATOR\n install the file:\n" + supertrendcci_file + "\ninto the folder:\nMQL4/Indicators.", "Important Information", MB_ICONINFORMATION);
        Alert("THIS EA NEED AN INDICATOR, install the file: " + supertrendcci_file + " into the folder: MQL4/Indicators.");
        supercci_indicator_init = false;
    }

    if (!supercci_indicator_init)
        return false;
...
input const string trsifilter         = "== RSI Filter =="; // ————————————————————————
input bool         rsi_filter_on      = true;               // RSI filter to open:
input int          rsi_filter_periods = 8;                  // RSI filter periods:
input double       rsi_filter_high    = 70;                 // RSI high level:
input double       rsi_filter_low     = 30;                 // RSI filter low leve:

class ConditionRsiFilter : public iConditions
{
  public:
    bool evaluate()
    {
        return iRSI(NULL, 0, rsi_period, PRICE_CLOSE, 0) < rsi_filter_high && iRSI(NULL, 0, rsi_filter_periods, PRICE_CLOSE, 0) > rsi_filter_low;
    }
};
ConditionRsiFilter conditionRsiFilter;


void               oninit_rsi_filter()
{
    if (!rsi_filter_on)
        return;
    Trader.filters.add(&conditionRsiFilter);
}
#endif
...
void                    oninit_counters_filters()
{
    ...
    Trader.filters.add(&cCountSells);
    Trader.filters.add(&cCountBuys);
}
...
int OnInit()
{
    ...
    if (!OnInit_SuperTraderTrend_Indicator()) {
        return INIT_FAILED;
    }
...
if (!OnInit_SuperTrend_CCI_Indicator()) {
        return INIT_FAILED;
    ...
    #ifdef Section_Close_Opposite
    oninit_close_on_opposite_flat();
    oninit_close_on_opposite_super();
    #endif
