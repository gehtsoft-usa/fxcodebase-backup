...
// https://fxcodebase.com/code/viewtopic.php?f=38&p=147616
...
CButton bt1, bt2;
#define BUTTON1_NAME "On/Off Line"
...
input string Tlabels               = "== Labels Setup ==";   // Lebels
input int    uFontSize             = 9;                      // Label Size
input color  Text_color_Top        = LimeGreen;              // Label Up Color
input color  Text_color_Bottom     = Red;                    // Label Dn Color
input color  uBMSColor             = Magenta;                // BMS color
...
double vShift    = 0;                      // Vertical Label shift
...
void ActionBt2()
{
  ...
  if (!drawLabels.Now())
  {
    ...
    {
      int    index = iBars(NULL, 0) - i;
      string id    = ObjPrefix + index;
      ObjectSetInteger(0, id, OBJPROP_COLOR, clrNONE);
    }
  }
  ...
  if (drawLabels.Now())
  {
    for (int i = 0; i < 5000; i++)
    {
      ...
      string id    = ObjPrefix + index;
      color  clr;
      if (ObjectDescription(id) == "HH") clr = Text_color_Top;
      if (ObjectDescription(id) == "HL") clr = Text_color_Top;
      if (ObjectDescription(id) == "LL") clr = Text_color_Bottom;
      if (ObjectDescription(id) == "LH") clr = Text_color_Bottom;
      if (ObjectDescription(id) == "BMS") clr = uBMSColor;
      ObjectSetInteger(0, id, OBJPROP_COLOR, clr);
    }
  }
