...
//http://fxcodebase.com/code/viewtopic.php?f=38&t=70645
...
input double distance = 500; // Distance to orders
...
public:
   virtual bool DoAction(const int period, const datetime date)
   {
      if (OrderSymbol() != _Symbol || !Trade(OrderType()))
      {
         return false;
      }
      OpenOrders(MathFloor(OrderOpenPrice() / pipSize));
      return true;
   }
};
...
bool Exists(bool isBuy, double rate)
{
   OrdersIterator orders;
   orders.WhenSymbol(_Symbol);
   orders.WhenTrade();
   orders.WhenSide(isBuy ? BuySide : SellSide);
   orders.WhenMagicNumber(magic_number);
   while (orders.Next())
   {
      if (OrderOpenPrice() == rate)
      {
         return true;
      }
   }
...
void OpenOrder(bool isBuy, double rate)
{
   if (Exists(isBuy, rate))
   {
      return;
   }
   string error;
   OrderBuilder *orderBuilder = new OrderBuilder(actions);
...
void OpenOrders(double price)
{
   OpenOrder(true, (price + distance) * pipSize);
   OpenOrder(false, (price + distance) * pipSize);
   OpenOrder(true, (price - distance) * pipSize);
   OpenOrder(false, (price - distance) * pipSize);
}
...
void OnTick()
{
   ...
   {
      first = false;
      OpenOrders(MathFloor(iClose(_Symbol, (ENUM_TIMEFRAMES)_Period, 0) / pipSize));
   }
}
