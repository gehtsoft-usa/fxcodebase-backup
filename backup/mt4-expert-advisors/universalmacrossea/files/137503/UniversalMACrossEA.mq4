...
// http://fxcodebase.com/code/viewtopic.php?f=38&t=70416
//+------------------------------------------------------------------+
...
// Based on 2006, firedavem http://www.fx-review.com
...
int MagicNumber;
input int
         _MagicNumber          = 1234; //Needed if running multiple EAs on the same account
double StopLoss, TakeProfit;
input double 
         _StopLoss             = 20,  // Use 0 (zero) if you don't like to use Stop Loss.
         _TakeProfit           = 30;  // Use 0 (zero) if you like to use open target.
...
input int         
         ...
         _TrailingStop         = 15; //Use 0 (zero) if you don't use trailing stop
...
int MinCrossDistance;
input int
         _MinCrossDistance     = 0,    //Set the pip distance between FastMA and SlowMA to be consider as a valid cross. Use 0 (zero) to disable this filter.
...
bool StopAndReverse;
input bool
         _StopAndReverse       = true,  //If set to TRUE, will exit any trade and reverse position when signal change.
...
input bool
         UseThirdMA           = true, //If set to TRUE (and UseCounterTrend = FALSE), will only trade according to ThirdMA direction, above for BUY and below for SELL.
...
input int
         ThirdMAPeriod        = 62,  //Third Moving Average Period.
...
input double max_distance_pips = 10; // Maximum price distance in pips
...
input int     
         StartHour            = 8, //Time when the EA start active (use with UseHourTrade = TRUE).
         EndHour              = 19; //Time when the EA stop active (use with UseHourTrade = TRUE).
         
input string  
         MM_Parameters        = "---------- Money Management";
double Lots;
input double 
         _Lots                 = 1.0; //Number of lot per trade.
input bool 
...
//----------------------- SOURCE : PENGIE
   MagicNumber    = subGenerateMagicNumber(_MagicNumber, Symbol(), Period());
	TicketComment  = StringConcatenate(TicketComment, "-", Symbol(), "-", Period());

//----------------------- SET MinCrossDistance ALWAYS POSITIVE
   MinCrossDistance = MathAbs(_MinCrossDistance);
...
//----------------------- NO STOP LOSS, NO TAKE PROFIT, NO TRAILING STOP
   ...
   else
   {
      ...
      StopAndReverse = _StopAndReverse;
   }
...
//+------------------------------------------------------------------+
   ...
   double distance = (iClose(_Symbol, _Period, 0) - SlowMACurrent) / pipSize;
   if (BuyCondition)
   {
      BuyCondition = distance < max_distance_pips && distance > 0;
   }
   else if (SellCondition)
   {
      SellCondition = (-distance) < max_distance_pips && distance < 0;
   }
