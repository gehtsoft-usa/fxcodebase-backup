//Available @ https://fxcodebase.com/code/viewtopic.php?f=38&t=74098
...
extern string             UniquebuttonId        = "FSR";
...
int getLowerFractalBar(int timeframe, int starting_bar)
  {
   for(int bar = starting_bar; bar < Bars; bar++)
      if(isLowerFractal(timeframe, bar))
         return(bar);
   return (-1);
  }
...
bool isLowerFractal(int timeframe, int bar)
  {
   for(int offset = -2; offset <= 2; offset++)
      if((offset != 0) && (iLow(Symbol(), timeframe, bar + offset) < iLow(Symbol(), timeframe, bar)))
         return(false);
   return (true);
  }
...
void drawResistance(int bar_index, int timeframe_index)
  {
   if(bar_index > 0)
      drawTrendLine(getTrendLineName(RESISTANCE, timeframes[timeframe_index]), iHigh(Symbol(), timeframes[timeframe_index], bar_index), colors[timeframe_index], styles[timeframe_index]);
  }
...
void drawSupport(int bar_index, int timeframe_index)
  {
   if(bar_index > 0)
      drawTrendLine(getTrendLineName(SUPPORT, timeframes[timeframe_index]), iLow(Symbol(), timeframes[timeframe_index], bar_index), colors[timeframe_index], styles[timeframe_index]);
  }
