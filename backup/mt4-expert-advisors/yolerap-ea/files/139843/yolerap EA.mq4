...
class AnotherPosOrderAction : public AOrderAction
{
   IAction* _openLongAction;
   IAction* _openShortAction;
   ActionOnConditionLogic* _actions;
   string _symbol;
   ENUM_TIMEFRAMES _timeframe;
public:
   AnotherPosOrderAction(string symbol, ENUM_TIMEFRAMES timeframe, IAction* openLongAction, IAction* openShortAction, ActionOnConditionLogic* actions)
   {
      _symbol = symbol;
      _timeframe = timeframe;
      _openLongAction = openLongAction;
      _openLongAction.AddRef();
      _openShortAction = openShortAction;
      _openShortAction.AddRef();
      _actions = actions;
   }

   ~AnotherPosOrderAction()
   {
      _openLongAction.Release();
      _openShortAction.Release();
   }
   ...
   virtual bool DoAction(const int period, const datetime date)
   {
      ...
      IOrder *order = new OrderByTicketId(_currentTicket);
      MinDistanceSinceTradeCondition* condition = new MinDistanceSinceTradeCondition(_symbol, _timeframe, candles_another_pos, order);
      order.Release();
      _actions.AddActionOnCondition(TradingCalculator::IsBuyOrder() ? _openLongAction : _openShortAction, condition);
      condition.Release();
      return true;
   }
};
...
TradingController *CreateController(const string symbol, const ENUM_TIMEFRAMES timeframe, string algoId, string &error)
{
   ...
   if (candles_another_pos > 0)
   {
      AnotherPosOrderAction* orderAction = new AnotherPosOrderAction(symbol, timeframe, openLongAction, openShortAction, actions);
      orderHandlers.AddOrderAction(orderAction);
      orderAction.Release();
   }
   openLongAction.Release();
