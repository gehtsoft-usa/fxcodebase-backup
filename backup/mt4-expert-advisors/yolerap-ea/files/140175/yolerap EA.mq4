...
// http://fxcodebase.com/code/viewtopic.php?f=38&t=70748
...
input int close_loss_pos_after = 60; // Close losing position after, minutes. 0 to disable
input int candles_another_pos = 10; // Open another position after candles. 0 to disable
...
class MinDistanceSinceTradeCondition : public ACondition
{
   int _minBars;
   IOrder* _order;
public:
   MinDistanceSinceTradeCondition(const string symbol, ENUM_TIMEFRAMES timeframe, int minBars, IOrder* order)
      :ACondition(symbol, timeframe, "Min disatance since trade")
   {
      _order = order;
      _order.AddRef();
      _minBars = minBars;
   ...
   ~MinDistanceSinceTradeCondition()
   {
      _order.Release();
   }
   ...
   bool IsPass(const int period, const datetime date)
   {
      ...
      int openBar = iBarShift(_symbol, _timeframe, OrderOpenTime());
      return openBar >= _minBars;
   }
};
...
class TradeOlderThanCondition : public AConditionBase
{
   IOrder* _order;
   double _minutes;
public:
   TradeOlderThanCondition(IOrder* order, double minutes)
   {
      _order = order;
      _order.AddRef();
      _minutes = minutes;
   ...
   virtual bool IsPass(const int period, const datetime date)
   {
      ...
      return (TimeCurrent() - OrderOpenTime()) / 60 >= _minutes;
   }
};
...
class CloseLosingAction : public AOrderAction
{
   ActionOnConditionLogic* _actions;
public:
   CloseLosingAction(ActionOnConditionLogic* actions)
   {
      _actions = actions;
   }
   ...
   void RestoreActions(string symbol, int magicNumber)
   {
      ...
      while (trades.Next())
      {
         int ticketId = trades.GetTicket();
         string ticketIdStr = IntegerToString(ticketId);
         double after = GlobalVariableGet("cl_" + ticketIdStr + "_after");
         if (after == 0)
         {
            continue;
         }
         OrderByTicketId* order = new OrderByTicketId(ticketId);
         CloseOrderAction* action = new CloseOrderAction(ticketId, slippage_points);
         AndCondition* condition = new AndCondition();
         condition.Add(new ProfitInRangeCondition(order, -1000000, 0), false);
         condition.Add(new TradeOlderThanCondition(order, after), false);
         _actions.AddActionOnCondition(action, condition);
         condition.Release();
         action.Release();
         order.Release();
   ...
   virtual bool DoAction(const int period, const datetime date)
   {
      ...
      string ticketIdStr = IntegerToString(_currentTicket);
      GlobalVariableSet("cl_" + ticketIdStr + "_after", close_loss_pos_after);
      
      CloseOrderAction* action = new CloseOrderAction(_currentTicket, slippage_points);
      AndCondition* condition = new AndCondition();
      condition.Add(new ProfitInRangeCondition(order, -1000000, 0), false);
      condition.Add(new TradeOlderThanCondition(order, close_loss_pos_after), false);
      _actions.AddActionOnCondition(action, condition);
      condition.Release();
      action.Release();
...
ICondition* CreateLongFilterCondition(string symbol, ENUM_TIMEFRAMES timeframe)
{
   ...
   AndCondition* condition = new AndCondition();
   condition.Add(new MinDistanceSinceLastTradeCondition(symbol, timeframe, candles_another_pos, magic_number), false);
   return condition;
}
...
TradingController *CreateController(const string symbol, const ENUM_TIMEFRAMES timeframe, string algoId, string &error)
{
   ...
   if (close_loss_pos_after > 0)
   {
      AOrderAction* closeLosingAction = new CloseLosingAction(actions);
      orderHandlers.AddOrderAction(closeLosingAction);
      closeLosingAction.Release();
   }
