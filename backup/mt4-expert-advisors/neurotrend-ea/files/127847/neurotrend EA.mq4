...
input int ma_exit_period = 100; // MA exit period
input ENUM_MA_METHOD ma_exit_method = MODE_SMA; // MA exit method
...
public:
   ...
   {
      double maValue = iMA(_symbol, _timeframe, ma_exit_period, 0, ma_exit_method, PRICE_CLOSE, period);
      double close = iClose(_symbol, _timeframe, period);

      double value = iCustom(_symbol, _timeframe, "neurotrend", z, z1, s, s1, 4, period);
      return value != EMPTY_VALUE && maValue > close;
   }
};
...
public:
   ...
   bool IsPass(const int period)
   {
      ...
      double value = iCustom(_symbol, _timeframe, "neurotrend", z, z1, s, s1, 5, period);
      return value != EMPTY_VALUE && maValue < close;
   }
};
...
int OnInit()
{
   double temp = iCustom(NULL, 0, "neurotrend", 0, 0);
   if (GetLastError() == ERR_INDICATOR_CANNOT_LOAD)
   {
      Alert("Please, install the 'neurotrend' indicator");
      return INIT_FAILED;
   }
