...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=74193
...
string file_custom_indicator = "Profit Indicator";
int lineGreen = 0;
int lineRed = 1;
int lineYellow = 4;

string Tindi = "== Indicator Setup ==";  // ————————————
...
bool buySignal()
{
    if((Indi(lineYellow, 2) >= Indi(lineRed, 2) && Indi(lineYellow, 1) < Indi(lineRed, 1) && Indi(lineYellow, 1) < 0) ||
       (Indi(lineYellow, 3) >= Indi(lineRed, 3) && Indi(lineYellow, 1) < Indi(lineRed, 1) && Indi(lineYellow, 1) < 0))
        return true;

    return false;    
    // return Indi(lineYellow, 2) < Indi(lineGreen, 2) && Indi(lineYellow, 1) >= Indi(lineGreen, 1);
}

bool sellSignal()
{
    if((Indi(lineYellow, 2) <= Indi(lineGreen, 2) && Indi(lineYellow, 1) > Indi(lineGreen, 1) && Indi(lineYellow, 1) > 0) ||
       (Indi(lineYellow, 3) <= Indi(lineGreen, 3) && Indi(lineYellow, 1) > Indi(lineGreen, 1) && Indi(lineYellow, 1) > 0))
        return true;

    return false;
    // return Indi(lineYellow, 2) > Indi(lineRed, 2) && Indi(lineYellow, 1) <= Indi(lineRed, 1);
}
...
class BUYcondition1 : public iConditions
{
 public:
  bool evaluate()
  {
    ...
    //   for(int i = 0;i < 5;i++)
    //   {
    //       double indi = Indi(lineGreen, i);
    //       if(indi != EMPTY_VALUE && indi != lastBuyPrice)
    //       {
    //           lastBuyPrice = indi;
    //           return true;
    //       }
...
class SELLcondition1 : public iConditions
{
 public:
  bool evaluate()
  {
    ...
    //   for(int i=0;i < 5;i++)
    //   {
    //     double indi = Indi(lineRed, i);
    //       if(indi != EMPTY_VALUE && indi != lastSellPrice)
    //       {
    //           lastSellPrice = indi;
    //           return true;
    //       }
