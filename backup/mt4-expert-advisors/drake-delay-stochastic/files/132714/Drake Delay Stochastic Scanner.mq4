...
// http://fxcodebase.com/code/viewtopic.php?f=38&t=69643
//+------------------------------------------------------------------+
...
input double Slw = 8;
input double Pds = 13;
input double Slwsignal = 9;
input int    Barcount = 2000;
...
public:
   ...
   {
      return iCustom(_symbol, _timeframe, "Drake Delay Stochastic", Slw, Pds, Slwsignal, Barcount, os, ob, 2, period) != EMPTY_VALUE;
   }
};
...
public:
   ...
   virtual bool IsPass(const int period, const datetime date)
   {
      return iCustom(_symbol, _timeframe, "Drake Delay Stochastic", Slw, Pds, Slwsignal, Barcount, os, ob, 3, period) != EMPTY_VALUE;
   }
};
...
public:
   ...
   { 
      ...
      _signaler.SetMessagePrefix("Drake: " + symbol + "/" + _signaler.GetTimeframeStr() + ": ");
      _id = id; 
      _x = x; 
      _y = y; 
      _symbol = symbol;
...
int init()
{
   double temp = iCustom(NULL, 0, "Drake Delay Stochastic", 0, 0);
   if (GetLastError() == ERR_INDICATOR_CANNOT_LOAD)
   {
      Alert("Please, install the 'Drake Delay Stochastic' indicator");
      return INIT_FAILED;
   }
   ...
   IndicatorName = GenerateIndicatorName("Drake Delay Stochastic Scanner");
