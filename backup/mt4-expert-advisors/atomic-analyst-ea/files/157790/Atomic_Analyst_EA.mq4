//Available @  https://fxcodebase.com/code/viewtopic.php?f=38&t=75493
...
double StopLossByIndi(string side)
{
    double sl=0;
    if (side == "buy") {
        sl = Indi(0, 0);
...
enum take_profit_mode {
    tp_null,     // Off
    tp_indi_1, // Analyst TP1
    tp_indi_2, // Analyst TP2
    tp_indi_3, // Analyst TP3
    tp_indi_4, // Analyst TP4
    tp_indi_5, // Analyst TP5
...
double TakeProfit(const double price, const string side, const string symbol, const double lot)
{
    ...
    case tp_indi_1: return TakeProfitByAnalyst(price, side, 1);
    case tp_indi_2: return TakeProfitByAnalyst(price, side, 2);
    case tp_indi_3: return TakeProfitByAnalyst(price, side, 3);
    case tp_indi_4: return TakeProfitByAnalyst(price, side, 4);
    case tp_indi_5: return TakeProfitByAnalyst(price, side, 5);
...
double TakeProfitByAnalyst(double price, string side, int tp_nro)
{
    double sl       = StopLossByIndi(side);
    double tp       = 0;
    double pr = iClose(NULL, 0, 1);
    double distance = fabs(pr - sl);
// clang-format off
    if (side == "buy") {
        if (tp_nro == 1) { tp = pr + distance * 0.236; }
        if (tp_nro == 2) { tp = pr + distance * 0.618; }
        if (tp_nro == 3) { tp = pr + distance * 1.000; }
        if (tp_nro == 4) { tp = pr + distance * 1.382; }
        if (tp_nro == 5) { tp = pr + distance * 2.000; }
    }
...
input const string indicator_file = "Atomic Analyst V40";                          // Indicator File:
//---
int  bufferToBuy              = 22;
int  bufferToSell             = 23;
...
input bool         filter_on       = true;                        // Filter On:
input const string indicator_file2 = "bb stops (multiple) 1.5";   // Indicator Filter File:
//---
int  bufferToBuy2              = 11;
int  bufferToSell2             = 12;
//---
double BBStops(int buffer, int candle = 1) { return iCustom(NULL, 0, indicator_file2, buffer, candle); }

class ConditionFilterBuy : public iConditions
{
    public:
    bool evaluate()
    {
        double indi = BBStops(bufferToBuy2, 0);
        return indi > 0 && indi != EMPTY_VALUE;
    }
};
ConditionFilterBuy cdFilterBuy;

class ConditionFilterSell : public iConditions
{
    public:
    bool evaluate()
    {
        double indi = BBStops(bufferToSell2, 0);
        return indi > 0 && indi != EMPTY_VALUE;
    }
};
ConditionFilterSell cdFilterSell;
...
bool OnInit_CustomIndicator()
{
    ...
    if(filter_on) {
        LongStrategy.addCondition(&cdFilterBuy);
        ShortStrategy.addCondition(&cdFilterSell);        
    }
