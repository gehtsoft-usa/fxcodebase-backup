//Available @  https://fxcodebase.com/code/viewtopic.php?f=38&t=75837
...
string file_custom_indicator = "HalfTrend-1.02+push_1461306941";
int    bufferToBuy           = 4;
int    bufferToSell          = 5;
...
double Indi(int buffer, int candle)
{
    ...
    return iCustom(NULL, 0, file_custom_indicator,
                   Amplitude, showBars, showArrows, alerts, alertss, buffer, candle);
}
#endif
...
input int                maFilterPeriod       = 50;                                  // Period
...
class BUYcondition2 : public iConditions
{
  public:
    bool evaluate()
    {
        ...
        return iClose(NULL, 0, 1) > maFilter.index(1);
    }
};
  ...
  public:
    bool evaluate()
    {
        if (closeAllInOpositeSignal) {
            bool cond1 = false;
            bool cond2 = true;
            cond1      = sellCondition1.evaluate();
            if (maFilterOn)
                cond2 = sellCondition2.evaluate();
            return cond1 && cond2;
        }
  ...
  public:
    bool evaluate()
    {
        if (closeAllInOpositeSignal) {
            bool cond1 = false;
            bool cond2 = true;
            cond1      = buyCondition1.evaluate();
            if (maFilterOn)
                cond2 = buyCondition2.evaluate();
...
double WinLossRate()
{
    ...
    for (int i = OrdersHistoryTotal() - 1; i >= 0; i--) {
        if (OrderSelect(i, SELECT_BY_POS, MODE_HISTORY) && OrderSymbol() == _Symbol && OrderMagicNumber() == magico) {
            if (OrderCloseTime() >= initial_time) {
                profit += OrderProfit() + OrderSwap() + OrderCommission();
    ...
    return NormalizeDouble(WinCount/LossCount, 2)*100;
}
//Available @  https://fxcodebase.com/code/viewtopic.php?f=38&t=75837
