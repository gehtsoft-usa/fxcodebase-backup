//Available @ https://fxcodebase.com/code/viewtopic.php?f=38&t=73801
...
int init()
{
   IndicatorObjPrefix = GenerateIndicatorPrefix("ZLSMA");
   IndicatorShortName("ZLSMA - Zero Lag LSMA");
   IndicatorBuffers(1);
   int id = 0;
   ...
   linreg1 = new LinearRegressionOnStream(linreg1Source, length, offset);
   linreg2Source = new CustomStream(_Symbol, (ENUM_TIMEFRAMES)_Period);
   linreg2 = new LinearRegressionOnStream(linreg2Source, length, offset);
                ...
                const int &spread[])
{
   ...
   for (int pos = MathMin(bars_limit, rates_total - 1 - MathMax(prev_calculated - 1, toSkip)); pos >= 0 && !IsStopped(); --pos)
   {
      ...
      linreg1Source.SetValue(pos, srcInputstreamValue);
      double linreg1Value;
      if (!linreg1.GetValue(pos, linreg1Value))
      {
         continue;
      }
      double lsma = linreg1Value;
      linreg2Source.SetValue(pos, lsma);
      double linreg2Value;
      if (!linreg2.GetValue(pos, linreg2Value))
      {
         continue;
      }
      double lsma2 = linreg2Value;
