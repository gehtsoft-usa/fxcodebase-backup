...
public:
   virtual bool DoAction(const int period, const datetime date)
   {
      ...
      if (OrderClosePrice() == OrderStopLoss() || StringFind(OrderComment(), "[sl]", 0) != -1)
      {
         ...
         double amount = OrderLots() * 4;
         double takeProfit = calc.CalculateTakeProfit(OrderType() == OP_BUY, limit, StopLimitPips, amount, OrderOpenPrice());
         double stopLoss = calc.CalculateStopLoss(OrderType() == OP_BUY, stop, StopLimitPips, amount, OrderOpenPrice());
         delete calc;

         string error;
         OrderBuilder order(actions);
         int ticket = order.SetSide(OrderType() == OP_BUY ? BuySide : SellSide)
            .SetInstrument(OrderSymbol())
            .SetAmount(OrderLots() * 4)
            .SetRate(OrderOpenPrice())
...
void Open()
{
   ...
   double rate;
   double marketrate;
   bool isBuy = Close[0] > Open[0];
   if (isBuy)
   {
      marketorder.SetSide(BuySide);
      order.SetSide(SellSide);
      rate = instrument.AddPips(instrument.GetBid(), -stop);
      marketrate = instrument.GetAsk();
   }
   else
   {
      marketorder.SetSide(SellSide);
      order.SetSide(BuySide);
      rate = instrument.AddPips(instrument.GetAsk(), stop);
      marketrate = instrument.GetBid();
   }
   TradingCalculator* calc = TradingCalculator::Create(_Symbol);
   order.SetStopLoss(calc.CalculateStopLoss(!isBuy, stop, StopLimitPips, Amount, rate));
   order.SetTakeProfit(calc.CalculateTakeProfit(!isBuy, limit, StopLimitPips, Amount, rate));
   marketorder.SetStopLoss(calc.CalculateStopLoss(isBuy, stop, StopLimitPips, Amount, marketrate));
