...
input const string indicator_file = "Driver";                // Indicator File:
input int          rsi_der        = 15;
//---
int  buDriver                 = 0;
bool indicator_init           = true;
int  candles_back_for_signals = 1;
//---
double Indi(int buffer, int candle = 1) { return iCustom(NULL, 0, indicator_file, rsi_der, buffer, candle); }
  ...
  public:
    bool evaluate()
    {
        for (int i = 1; i <= candles_back_for_signals; i++) {
            double indi  = Indi(buDriver, i);
            double indi2 = Indi(buDriver, i + 1);
            if (indi > 0 && indi2 <= 0) {
                lastSignal = indi;
                return true;
            }
...
input string             t_ma          = "== Moving Average Setup =="; // ————————————————————————
input bool               ma_on         = true;                         // Filter by MA:
input int                ma_period     = 5;                            // Periods:
...
class ConditionBarCrossEmaUp : public iConditions
{
  public:
    bool evaluate()
    {
        ...
        double ema = iMA(NULL, 0, ma_period, ma_shift, ma_method, applied_price, 1);
...
// Mark: Section_Envelopes

input string             env_title         = "== Envelopes Setup =="; // ————————————————————————
input bool               env_on            = true;                    // Envelopes On
input int                env_period        = 54;                      // Periods:
input ENUM_MA_METHOD     env_method        = MODE_SMMA;               // Method:
input int                env_shift         = 0;                       // Shift:
input ENUM_APPLIED_PRICE env_applied_price = PRICE_CLOSE;             // Applied Price:
input double             env_desv          = 0.14;                    // Desviation:
...
class ConditionEnvelopesUp : public iConditions
{
  public:
    ...
    {
        double cl        = iClose(NULL, 0, 1);
        double env_low = iEnvelopes(NULL, 0, env_period, env_method, env_shift, env_applied_price, env_desv, MODE_LOWER, 1);
        return cl < env_low;
    }
};
...
class ConditionEnvelopesDn : public iConditions
{
  public:
    ...
    {
        double cl      = iClose(NULL, 0, 1);
        double env_upper = iEnvelopes(NULL, 0, env_period, env_method, env_shift, env_applied_price, env_desv, MODE_UPPER, 1);
        return cl > env_upper;
    }
};
