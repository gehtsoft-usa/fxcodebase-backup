...
input const string indicator_file = "Driver_MT5";                // Indicator File:
input int          rsi_per        = 15;
//---
int  buDriver                 = 0;
int  bufferToSell             = 0;
...
//---
int  handle_driver = 0;
void setHandleDriver() { handle_driver = iCustom(Symbol(), NULL, indicator_file, "", rsi_per); }

double Driver(int buffer, int candle = 1)
{
    double value[1];
    int    copy = CopyBuffer(handle_driver, buffer, candle, 1, value);
  ...
  public:
    bool evaluate()
    {
        for (int i = 1; i <= candles_back_for_signals; i++) {
            double indi  = Driver(buDriver, i);
            double indi2 = Driver(buDriver, i + 1);
            if (indi > 0 && indi2 <= 0) {
                lastSignal = indi;
                return true;
            }
...
#define Section_BarCrossEma
#ifdef Section_BarCrossEma
// Mark: Section_BarCrossEma

input string             Iema            = "== Moving Average Setup =="; // == Moving Average Setup ==
input bool               ma_on           = true;                         // Filter by MA:
input int                ma1Period       = 5;                            // Period
...
class ConditionBarCrossEmaUp : public iConditions
{
  public:
    bool evaluate()
    {
        ...
        double ema = MA(1);
        return cl > lo && cl > ema && lo <= ema;
    }
};
ConditionBarCrossEmaUp cdBarCrossEmaUp;

class ConditionBarCrossEmaDn : public iConditions
{
  public:
    bool evaluate()
    {
        ...
        double cl  = iClose(NULL, 0, 1);
        double ema = MA(1);
        return cl < hi && cl < ema && hi >= ema;
    }
};
ConditionBarCrossEmaDn cdBarCrossEmaDn;

void oninitma()
{
    setHandle_ma1();
    ShortStrategy.addCondition(&cdBarCrossEmaDn);
    LongStrategy.addCondition(&cdBarCrossEmaUp);
}

#endif

#define Section_Envelopes
#ifdef Section_Envelopes
// MARK: section envelopes indicator

input string             env1_title         = "== Envelopes Setup =="; // ————————————————————————
input bool               env1_on            = true;                    // Envelopes On
input int                env1_period        = 54;                      // Periods:
input ENUM_MA_METHOD     env1_method        = MODE_SMMA;               // Method:
input int                env1_shift         = 0;                       // Shift:
input ENUM_APPLIED_PRICE env1_applied_price = PRICE_CLOSE;             // Applied Price:
input double             env1_desv          = 0.14;                    // Desviation:

int  handle_env1 = 0;
void setHandle_env1() { handle_env1 = iEnvelopes(NULL, 0, env1_period, env1_shift, env1_method, env1_applied_price, env1_desv); }

double Envelopes_Upper(int candle = 1)
{
    ...
    // int    shift = Bars(_Symbol, _Period) - candle;
    int shift = candle;
    int copy  = CopyBuffer(handle_env1, 0, shift, 1, value);
...
double Envelopes_Lower(int candle = 1)
{
    double value[1];
    // int    shift = Bars(_Symbol, _Period) - candle;
    int shift = candle;
    int copy  = CopyBuffer(handle_env1, 1, shift, 1, value);
...
class ConditionEnvelopesUp : public iConditions
{
  public:
    bool evaluate()
    {
        double cl      = iClose(NULL, 0, 1);
        double env_low = Envelopes_Lower(1);
        return cl < env_low;
    }
};
ConditionEnvelopesUp cdEnvelopesup;

class ConditionEnvelopesDn : public iConditions
{
  public:
    bool evaluate()
    {
        double cl        = iClose(NULL, 0, 1);
        double env_upper = Envelopes_Upper(1);
        return cl > env_upper;
    }
};
ConditionEnvelopesDn cdEnvelopesdn;

void OnInit_Envelopes()
{
    if (env1_on) {
        setHandle_env1();
        ShortStrategy.addCondition(&cdEnvelopesdn);
        LongStrategy.addCondition(&cdEnvelopesup);
    }
}
...
void OnTick()
{
    ...
    // Print("line: ",__LINE__," credit: ",credit);
    // double margin = AccountInfoDouble(ACCOUNT_MARGIN);
    // Print("line: ",__LINE__," margin: ",margin);
    // double margin_level = AccountInfoDouble(ACCOUNT_MARGIN_LEVEL);
    // Print("line: ",__LINE__," margin_level: ",margin_level);
    // double margin_so_so = AccountInfoDouble(ACCOUNT_MARGIN_SO_SO);
    // Print("line: ",__LINE__," ACCOUNT_MARGIN_SO_SO: ",ACCOUNT_MARGIN_SO_SO);
    // double margin_so_so = AccountInfoDouble(ACCOUNT_MARGIN_SO_SO)/100;
    // double margin_so_call = AccountInfoDouble(ACCOUNT_MARGIN_SO_CALL)/100;
    // double margin_level = AccountInfoDouble(ACCOUNT_MARGIN_LEVEL);
    // Print("line: ",__LINE__," margin_level: ",margin_level);
    // Print("line: ",__LINE__," margin_so_so: ",margin_so_so);
    // Print("line: ",__LINE__," margin_so_call: ",margin_so_call);
}
