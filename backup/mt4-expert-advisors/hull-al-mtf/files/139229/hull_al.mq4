...
public:
   ...
   {
      return ind_buffer0.GetColorIndex(period) == 0;
   }
};
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      return ind_buffer0.GetColorIndex(period) == 1;
   }
};
...
int init()
{
   ...
   ind_buffer0 = new ColoredStream(_Symbol, (ENUM_TIMEFRAMES)_Period);
   //---- indicator buffers mapping
   IndicatorBuffers(14);
   int id = 0;
   id = ind_buffer0.RegisterStream(id, DeepSkyBlue, "HullALMA Uptrend", DRAW_LINE, STYLE_SOLID, 1);
   id = ind_buffer0.RegisterStream(id, OrangeRed, "HullALMA Dntrend", DRAW_LINE, STYLE_SOLID, 1);
   id = ind_buffer0.RegisterInternalStream(id);
...
int start()
{
   ...
   if (counted_bars <= 0 || counted_bars > Bars)
   {
      ...
      ArrayInitialize(Dntrend, EMPTY_VALUE);
   ...
   {
      double v1Val, v2Val;
      if (!v1.GetValue(pos, v1Val) || !v2.GetValue(pos, v2Val))
      {
         continue;
      }
      ind_buffer1._data[pos] = v1Val + DampingFactor * (v1Val - v2Val);
      double indiVal;
      if (!ind_buffer0_stream.GetValue(pos, indiVal))
      {
         continue;
      }
      ...
      if (customStream != NULL)
      {
         customStream.SetValue(pos, indiVal);
      }
      double Filter = 0;
      if (PctFilter > 0)
      {
         Del[pos] = MathAbs(indiVal - ind_buffer0._data[pos + 1]);
         ...
         if (MathAbs(indiVal - ind_buffer0._data[pos + 1]) < Filter)
         {
            indiVal = ind_buffer0._data[pos + 1];
         }
      }
      int colorIndex = ind_buffer0.GetColorIndex(pos + 1);
      if (indiVal - ind_buffer0._data[pos + 1] > Filter)
      {
         colorIndex = 0;
      }
      else if (ind_buffer0._data[pos + 1] - indiVal > Filter)
      {
         colorIndex = 1;
      }
      ind_buffer0.Set(indiVal, pos, colorIndex);
