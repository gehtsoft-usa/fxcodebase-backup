...
int init()
{
   signaler = new Signaler(symbol, timeframe);
...
int start()
{
   ...
   for (int i = limit; i >= 0; i--)
   {
      ...
      {
         int index = iBarShift(symbol, 0, Time[i]);
      ...
      {
         int index = iBarShift(symbol, timeframe, Time[i]);
         ExtBuffer0[i] = iCustom(symbol, timeframe, "Up_and_Down HTF", symbol, PERIOD_CURRENT, period, 0, index);
...
void manageArr(int j, color clr, int theCode, bool up)   
{
   ...
   if ( up )
   {
      ...
      if (j == 0 && last_alert != iTime(symbol, timeframe, 0))
      {
         last_alert = iTime(symbol, timeframe, 0);
