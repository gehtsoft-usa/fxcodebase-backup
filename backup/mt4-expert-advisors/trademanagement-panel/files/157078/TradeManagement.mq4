...
enum control_winloss_type {
    ctrl_by_money,   // by money
    ctrl_by_percent, // by account percent
};

input control_winloss_type control_type = ctrl_by_money; // Win Loss Control by:

// 
input bool   control_win_on  = false; // Control Win On?
input double win_limit       = 100;   // Win Limit
input double percent_win     = 1;     // Account Percent Win:
input bool   closeallwhenwin = true;  // Close all trades when reach the win limit?

input bool   control_loss_on = false; // Control Loss On?
input double loss_limit      = 100;   // Loss Limit
input double percent_loss    = 1;     // Account Percent Loss:
input bool   closeallwhenloss = true;  // Close all trades when reach the loss limit?

void ControlLossOn()
{
    // control_loss_on = !control_loss_on;
}
void ControlWinOn()
{
    // control_win_on = !control_win_on;
}

input bool   tp_on     = false; // Put TP:
input double u_tp_pips = 50;    // TP Pips:
double       tp_pips   = u_tp_pips;
double u_rb      = 2; //  Ratio RR:

double rb = u_rb;

input bool   sl_on     = false; // Put SL:
input double u_sl_pips = 50;    // SL Pips
double       sl_pips   = u_sl_pips;

input bool bkv_on = false; // Breakeven On:
...
class ConditionControlLoss : public iConditions
{
  public:
    bool evaluate()
    {
        ...
        // double loss_limit = (double)gui.eControlLoss.Text();
        ...
        double limit = loss_limit;
        if (control_type == ctrl_by_percent) {
            double account = AccountInfoDouble(ACCOUNT_BALANCE);
            limit          = account * percent_loss / 100;
        }
...
class ActionControlLoss : public iActions
{
  public:
    bool execute()
    {
        if (closeallwhenloss) acCloseAll.execute();
        return true;
    }
};
...
class ConditionControlWin : public iConditions
{
  public:
    bool evaluate()
    {
        ...
        // double  win_limit = (double)gui.eControlWin.Text();
        ...
        double limit = win_limit;
        if (control_type == ctrl_by_percent) {
            double account = AccountInfoDouble(ACCOUNT_BALANCE);
            limit          = account * percent_win / 100;
        }
...
class ActionControlWin : public iActions
{
  public:
    bool execute()
    {
        if (closeallwhenwin) acCloseAll.execute();
        return true;
    }
};
...
input bool   tsl_on         = false;                      // TSL ON:
input double u_tsl_start    = 1;                          // TSL Start:
double       u_tsl_step     = 1;                          // TSL Step:
input double u_tsl_distance = 20;                         // TSL Distance:
double       tsl_start      = u_tsl_start;
double       tsl_step       = u_tsl_step;
double       tsl_distance   = u_tsl_distance;
...
int OnInit()
{
    ...
    TpOn();
    SlOn();
    if(bkv_on)acBreackeven.execute();
