...
#property description "Extreme Entry with Mean Reversion and Trend Filter"
#property strict
...
input ENUM_TIMEFRAMES calcTimeframe = PERIOD_CURRENT; // Timeframe to calculate bands (MTF)
//--- buffers
...
//--- starting calculation
   if(prev_calculated>1)
      ...
      pos=0;
   //--- main cycle
   if(calcTimeframe==PERIOD_CURRENT)
   {
      for(i=pos; i<rates_total && !IsStopped(); i++)
      {
         //--- middle line (chart timeframe)
   ...
   else
   {
      for(i=pos; i<rates_total && !IsStopped(); i++)
      {
         int mtfIndex = iBarShift(_Symbol, calcTimeframe, time[i], true);
         if(mtfIndex<0)
         {
            ExtMovingBuffer[i]=EMPTY_VALUE;
         ...
         int bars = iBars(NULL,0)-1;

         double middle = iMA(_Symbol, calcTimeframe, InpBandsPeriod, InpBandsShift, MODE_SMA, PRICE_CLOSE, mtfIndex);    
      
         double stddev = iStdDev(_Symbol, calcTimeframe, InpBandsPeriod, InpBandsShift, MODE_SMA, PRICE_CLOSE, mtfIndex);

         ExtMovingBuffer[bars-i]=middle;
         ExtStdDevBuffer[bars-i]=stddev;
         ExtUpperBuffer[bars-i]=middle + InpBandsDeviations * stddev;
         ExtLowerBuffer[bars-i]=middle - InpBandsDeviations * stddev;
      }
   }
