...
// https://fxcodebase.com/code/viewtopic.php?f=38&p=148723
...
void NotifyControl()
{
  for (int r = 1; r < table.nRows(); r++)
  {
	...
	for (int c = 1; c <= 7; c++)
    {
        if(c == 1) { value = table.cellText(r, c); continue; }
        if(c < 7)  { if(value == table.cellText(r, c)) continue; else break; }
...
// if(newCandle.IsNewCandle()==false) return;
if(newCandleM5.IsNewCandle()==false) return;
...
// if(tf == "W1" && newCandleW1.IsNewCandle()==false) return;
    ...
    if (type == 0) text += sym + " All TimeFrames in BUY ";
    // if (type == 1) text += sym + " " + tf + " SELL ";
    if (type == 1) text += sym + " All TimeFrames in SELL ";
