...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=75387
...
input enumDirection Direction = openBuy; // Direction
...
input int barstoclose = 10; // # Bars to close
  ...
  public:
    bool evaluate()
    {
        if (OrderType() == OP_BUY) {
            ...
            int b = iBarShift(NULL, PERIOD_M1, OrderOpenTime());
            if(b >= barstoclose) {
                return true;
            }
        }
...
void oninit_close_strategy()
{
    ...
    stCloseBuy.set(&cdCloseBuy, &acCloseAll);
    stCloseSell.set(&cdCloseSell, &acCloseAll);
...
class BUYcondition1 : public iConditions
{
  ...
  public:
    bool evaluate()
    {
        ...
        if(Direction == openSell) return false;

        datetime currTm = iTime(NULL, PERIOD_M1, 0);
        if(lastTm == 0) lastTm = currTm;
...
class SELLcondition1 : public iConditions
{
  ...
  public:
    bool evaluate()
    {
        ...
        if(Direction == openBuy) return false;
