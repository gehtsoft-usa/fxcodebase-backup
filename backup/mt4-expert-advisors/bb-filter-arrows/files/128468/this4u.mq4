...
// http://fxcodebase.com/code/viewtopic.php?f=38&t=68890
...
input int        Bollinger_Bands_Periods       = 22; // BB Period
...
public:
   ...
   bool IsPass(const int period)
   {
      return buffer2[period] == -1 && buffer2[period + 1] != -1;
   }
};
...
int init()
{
   ...
   upCondition.Add(new UpAlertCondition(_Symbol, (ENUM_TIMEFRAMES)_Period));
   upCondition.Add(new PriceOutsideBandCondition(_Symbol, (ENUM_TIMEFRAMES)_Period, Bollinger_Bands_Periods, Bollinger_Bands_Deviations));
   AndCondition* downCondition = new AndCondition();
   downCondition.Add(new DownAlertCondition(_Symbol, (ENUM_TIMEFRAMES)_Period));
   downCondition.Add(new PriceOutsideBandCondition(_Symbol, (ENUM_TIMEFRAMES)_Period, Bollinger_Bands_Periods, Bollinger_Bands_Deviations));
   ...
   SetIndexBuffer(8, buffer);
   SetIndexStyle(9, DRAW_NONE);
   SetIndexBuffer(9, buffer2);
   SetIndexStyle(10, DRAW_NONE);
   SetIndexBuffer(10, buffer3);
...
int start()
{
   ...
   {
      buffer[pos] = MathMax(
         MathMax(MathAbs(Low[pos] - Close[pos + 1]), MathAbs(spread + High[pos] - Close[pos + 1])), 
         spread + High[pos] - Low[pos]);
      if (pos > Bars - 8)
         continue;
      ...
      if (buffer2[pos] != -1)
      {
         if (Low[pos] < buffer3[pos] - ld_44)
         {
            buffer2[pos] = -1;
            buffer3[pos] = spread + High[pos];
         }
      }
      else
      {
         if (spread + High[pos] > buffer3[pos] + ld_44)
         {
            buffer2[pos] = 1;
            buffer3[pos] = Low[pos];
         }
      ...
      if (buffer2[pos] != -1 && Low[pos] > buffer3[pos])
         buffer3[pos] = Low[pos];
      if (buffer2[pos] != 1 && spread + High[pos] < buffer3[pos])
         buffer3[pos] = spread + High[pos];
