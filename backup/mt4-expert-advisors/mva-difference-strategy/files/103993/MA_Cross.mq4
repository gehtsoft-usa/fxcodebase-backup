...
input int Short_Length=10;
input int Long_Length=50;
...
void OnTick()
{
 ...
 double MA_S0, MA_S1, MA_L0, MA_L1;
 MA_S0=iMA(NULL, 0, Short_Length, 0, MODE_SMA, PRICE_CLOSE, 1);
 MA_S1=iMA(NULL, 0, Short_Length, 0, MODE_SMA, PRICE_CLOSE, 2);
 MA_L0=iMA(NULL, 0, Long_Length, 0, MODE_SMA, PRICE_CLOSE, 1);
 MA_L1=iMA(NULL, 0, Long_Length, 0, MODE_SMA, PRICE_CLOSE, 2);
 
 int CO;
 int res;
 double SL, TP;
 
 if (MA_S1<=MA_L1 && MA_S0>MA_L0)
 {
  _Alert("Buy");
  CO=CalculateOrders();
  if (CO<=0)
