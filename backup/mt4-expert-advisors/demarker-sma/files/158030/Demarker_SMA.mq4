//Available @  https://fxcodebase.com/code/viewtopic.php?f=38&t=75555
...
// ------------------------------------------------------------------
input int demPeriods = 14; // Demarker Periods:
input int    smaPeriods               = 20;
...
int OnInit()
{
    SetIndexBuffer(0, Dem, INDICATOR_DATA);
                ...
                const long &tick_volume[], const long &volume[], const int &spread[])
{
    ...
    for (i = start; i >= 0; i--) {
            ...
            Dem[i] =  iDeMarker(NULL,0,demPeriods,i);
            Sma[i] = smaDemarker(i);

        if (Dem[i]<Sma[i] && Dem[i+1]>=Sma[i+1]) {
            ArrowUp[i] = Dem[i];
            notify(0);
        }
        if (Dem[i]>Sma[i] && Dem[i+1]<=Sma[i+1]) {
...
double smaDemarker(int i)
{
    double sum = 0;
    for (int j = 0; j < smaPeriods; j++) {
        sum += iDeMarker(NULL,0,demPeriods,j+i);
    }
    return sum / smaPeriods;
}
