...
// http://fxcodebase.com/code/viewtopic.php?f=38&t=71322
...
input int      Periods_to_Smooth       = 3;
input string   Comment0                = "- Slope: Minimum Slope to Confirm Bubble (pip/minute) -";
input double   Slope                   = 0.18;
input string   Comment1                = "- Percent: To Confirm end of sausage (%) -";
input double   Percent                 = 25;
input bool     Show_BB_Middle_Line     = true;
input double   Slope_Detection         = 0.5;
input int      Bars_to_Calculate_Slope = 1;
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      double value = iCustom(_symbol, _timeframe, "BB_Analyzer", 
         Periods, Deviation, Method_to_Smooth, Periods_to_Smooth, "", Slope, "", Percent, Show_BB_Middle_Line, Slope_Detection, Bars_to_Calculate_Slope, 
         4, period);
...
int OnInit()
{
   double temp = iCustom(NULL, 0, "BB_Analyzer", 0, 0);
   if (GetLastError() == ERR_INDICATOR_CANNOT_LOAD)
   {
      Alert("Please, install the 'BB_Analyzer' indicator");
      return INIT_FAILED;
   }
