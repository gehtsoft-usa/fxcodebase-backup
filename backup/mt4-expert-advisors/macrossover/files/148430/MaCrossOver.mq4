...
input string             IemaFast            = "== MA fast Setup ==";  // ————————————
input ENUM_TIMEFRAMES    maFast_TF           = PERIOD_CURRENT;         // Time Frame:
...
input string             IemaSlow            = "== MA Slow Setup ==";  // ————————————
input ENUM_TIMEFRAMES    maSlow_TF           = PERIOD_CURRENT;         // Time Frame:
...
class BUYcondition2 : public iConditions
{
 public:
  bool evaluate()
  {
    ...
    return maFast.index(1) > maSlow.index(1) && iClose(NULL, 0, 1) > maFast.index(1);
    // return false;
  }
};
...
//--- SESSIONS CONTROL:
  ...
  // Note: Indicators
  maFast = new MovingAverage(_Symbol, maFast_TF);
  maFast.setSetup(maFast_Period, maFast_Shift, maFast_Method, maFast_AppliedPrice);

	maSlow = new MovingAverage(_Symbol, maSlow_TF);
