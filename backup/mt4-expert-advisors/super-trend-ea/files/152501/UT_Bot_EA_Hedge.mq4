...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=73876
...
#define BUTTON2_NAME "Hedging All"
...
string file_custom_indicator = "UT_Bot_v2.ex4";
int bufferToBuy = 0;
int bufferToSell = 1;
...
input string Ttrade = "== Draw Trade ==";  // ————————————
...
input int uMaxTrades = 100;  // Max Trades At Same Time:
#endif
...
input bool               HedgeBeforeNews = true; // Hedge Before News:
...
class ActionHedging : public iActions
{
    ...
    public:
    ActionHedging(int magic = 0, string symbol = "", int slippage = 10000)
    {
...
void setSideAndLots()
{
    ...
    for(int i = OrdersTotal() - 1; i >= 0; i--)
    {
       if(OrderSelect(i, SELECT_BY_POS) && OrderSymbol() == _symbol && OrderMagicNumber() == _magic && OrderType() == OP_BUY)
       {
           buyLots += OrderLots();
       }
       if(OrderSelect(i, SELECT_BY_POS) && OrderSymbol() == _symbol && OrderMagicNumber() == _magic && OrderType() == OP_SELL)
       {
           sellLots += OrderLots();
       }
    }
    ...
    {
        _type = OP_SELL;
        _lots = buyLots - sellLots;
    } else 
    {
        _type = OP_BUY;
        _lots = sellLots - buyLots;
    }
    ...
    double setPrice()
    {
        if(_type == OP_BUY) { return SymbolInfoDouble(_symbol, SYMBOL_ASK); }
        if(_type == OP_SELL) { return SymbolInfoDouble(_symbol, SYMBOL_BID); }
        return 0;
    }
    ...
    bool doAction()
    {
        ...
        string side = _type == OP_BUY ? "buy" : "sell";
        
        actionSendOrder = new SendNewOrder(side, _lots, "", 0, 0, 0, magico);
        if(CheckPointer(actionSendOrder)!=POINTER_INVALID && actionSendOrder.doAction())
        {
...
double Indi(int buffer, int _candle)
{
    ...
    m, atrPeriods, h, s, p, b, Ttrade, drawTradeON, userSL, userTP1, userTP2, userTP3,
    buffer, _candle);
}
