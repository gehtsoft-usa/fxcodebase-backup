...
input string Tindi = "== SuperTrend Setup ==";  // ————————————
input ENUM_TIMEFRAMES indi_tf = PERIOD_CURRENT; // Indicator Time Frame:
input int periods = 10; // Periods:
input double multiplier = 3.0; // Multiplier:


input string Tindi2 = "== SuperTrend Filter Setup ==";  // ————————————
input bool FilterSuperTrendON = true; // Filter TF Superior ON?
input ENUM_TIMEFRAMES indi_tf2 = PERIOD_CURRENT; // Indicator Time Frame:
input int             periods2 = 10; // Periods:
input double          multiplier2 = 3.0; // Multiplier:
...
class SELLcondition1 : public iConditions
{
 public:
  bool evaluate()
  {
    ...
    // return iClose(NULL, 0, 1) < ma.index(1);
    return IndiMTF(bufferToSell, 2) == -1;
    // return false;
  }
};
...
int OnInit()
{
  ...
  if(FilterSuperTrendON)conditionsToBuy.AddCondition(buyCondition3 = new BUYcondition3());
  ...
  if(FilterSuperTrendON)conditionsToSell.AddCondition(sellCondition3 = new SELLcondition3());
...
double Indi(int buffer, int candle)
{
    ...
    return iCustom(NULL, indi_tf, file_custom_indicator,
    periods,
    multiplier,
    buffer, candle);
}

double IndiMTF(int buffer, int candle)
{
    // TODO: copiar la lista de INPUTS para custom indicator:    
    return iCustom(NULL, indi_tf2, file_custom_indicator,
    periods2,
    multiplier2,
    buffer, candle);
}
