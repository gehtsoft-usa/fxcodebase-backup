...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      ...
      return ind_buffer1[index] > UpLineValue;
   }
};
...
public:
   ...
   {
      int index = iBars(_symbol, _timeframe) - period - 1;
      return ind_buffer1[index] < DownLineValue;
   }
};
...
)
{
   ...
   {
      ArrayInitialize(ind_buffer1, 0);
      ArrayInitialize(ind_buffer2, 0);
      for (int i = 0; i < ArraySize(alerts); ++i)
      {
         alerts[i].Init();
      }
   ...
   for (int pos = MathMax(first, prev_calculated - 1); pos < rates_total; ++pos)
   {
      ...
      if (resh == 0)
         deltah = 0;
      ind_buffer1[pos] = deltah;

      deltal = ind_buffer2[pos - 1];
      if (dif < 0)
      {
         resh = 0;
         if (resl == 0)
