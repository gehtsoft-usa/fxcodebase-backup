...
input string CurrentSybmol = "EURUSD";
input int norm_period = 10; // Normalization period
...
int init()
{
   double temp = iCustom(NULL, 0, "DS_ver1", 0, 0);
   if (GetLastError() == ERR_INDICATOR_CANNOT_LOAD)
   {
      Alert("Please, install the 'DS_ver1' indicator");
      return INIT_FAILED;
   }
   IndicatorShortName("Demand/Supply oscillator");
   IndicatorDigits(Digits);
   IndicatorBuffers(5);
   ds = new ColoredStream(_Symbol, (ENUM_TIMEFRAMES)_Period);
   int id = ds.RegisterStream(0, Green);
   id = ds.RegisterStream(id, Red);
   id = ds.RegisterInternalStream(id);
   SetIndexBuffer(id++, Raw);
...
int start()
{
   ...
   {
      data[pos] = iCustom(CurrentSybmol, Timeframe, "DS_ver1", Length, Method, Timeframe, CurrentSybmol, 0, pos);
      if (pos + norm_period >= Bars)
      {
         continue;
      }
      
      int highestIndex = ArrayMaximum(data, norm_period, pos);
      double highest = data[highestIndex];
      int lowestIndex = ArrayMinimum(data, norm_period, pos);
      double lowest = data[lowestIndex];

      double value = (data[pos] - lowest) / (highest - lowest);
      if (value < ds._data[pos + 1])
      {
         ds.Set(value, pos, 1);
      }
      else
