...
// ------------------------------------------------------------------
input int table_position = 0;   // Table Position:
input int table_size     = 560; // Table Size:

input string TitlePairs = "==== PAIRS ===="; // Set your pairs
input string uSymbols   = "GBPUSD,EURUSD";   // Symbols (add separate by comma ","):
// ------------------------------------------------------------------
input string TitlePatern              = "==== Indicator Setup ===="; // ————————————
input int    rsi_periods              = 14;                          // RSI periods:
input double highLevel                = 70;                          // Buy Level
input double lowLevel                 = 30;                          // Sell Level
input int    candles_back_for_signals = 10;                          // Search in candles back:
...
class RSI
{
  ...
  public:
    ...
    double value(int shift) { return iRSI(_symbol, _tf, _periods, PRICE_CLOSE, shift); }
};
RSI rsi();
...
class FinderPattern
{
  ...
  public:
    ...
    {
        // TODO: escribir el patrón alcista (cruce hacia abajo del lowlevel)
        rsi.Set_Symbol_TF(_symbol, tf);
        return rsi.value(i + 2) >= lowLevel && rsi.value(i + 1) < lowLevel;
    }

    bool havePatternDn(int i, ENUM_TIMEFRAMES tf)
    {
        // TODO: escribir el patrón bajista (cruce hacia arriba del highlevel)
        rsi.Set_Symbol_TF(_symbol, tf);
        return rsi.value(i + 2) <= highLevel && rsi.value(i + 1) > highLevel;
    }
};
...
class Table : CObjectBase
{
  ...
  void Create()
  {
    ...
    for (int i = 0; i < _nRows; i++)
    {
      ...
      _rows[i, 0].Text("");
      _rows[i, 0].Width(70);
      // _rows[i, 0].Width(table_size / _nColumns);
      _rows[i, 0].X(x);
      _rows[i, 0].Y(y);
      ...
      for (int j = 1; j < _nColumns; j++)
      {
        ...
        // _rows[i, j].Width(35);
        _rows[i, j].Width(table_size / _nColumns);
        _rows[i, j].X(x);
        _rows[i, j].Y(y);
        _rows[i, j].Create();
        // x += 36;
        x += table_size / _nColumns +1;
      }
      y += 17;
    }
  }
...
void doActions()
{
	...
	for(int i=0;i < ArraySize(finders) ; i++)
	{
		...
		for (int j = 0; j < ArraySize(TF); j++) 
		{
			...
			adx_value[j] = StringSubstr(patCode,0,2);
		...
		for(int n=0;n < 16;n++)
		{
	 		...
	 		if(!esPar(n)) { int pos = n/2; results[n] = adx_value[pos]; }
		}
