...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=75036
...
input const string indicator_file           = "RSI.ex5";                        // Indicator File:
input string       note                     = "in the folder: MQL5/Indicators"; // The Indicator must to have intalled
input double       highLevel                = 70;                               // Buy Level
input double       lowLevel                 = 30;                               // Sell Level
input int          bufferToBuy              = 0;                                // Buy Signal buffer:
input int          bufferToSell             = 0;                                // Sell Signal buffer:
input int          candles_back_for_signals = 10;                               // Search in candles back:
// ------------------------------------------------------------------

int consecutives = 7; // Nro of Candles in same direction:
...
class FinderPattern
{
  ...
  public:
    ...
    {
        for (int i = 0; i < 8; i++) {
            handles[i] = iCustom(_symbol, TF[i], indicator_file);
        }
    }

    double Indi(int buffer, int candle = 1, int pos = 0)
    {
        double value[1];
        int    copy = CopyBuffer(handles[pos], buffer, candle, 1, value);
        if (copy > 0) {
            return value[0];
        }
    ...
    bool havePatternUp(int i, ENUM_TIMEFRAMES tf)
    {
        ...
        double indi  = Indi(bufferToBuy, i, p);
        double indi2 = Indi(bufferToBuy, i + 1, p);
        if (indi < lowLevel && indi2 >= lowLevel) return true;
    ...
    bool havePatternDn(int i, ENUM_TIMEFRAMES tf)
    {
        ...
        double indi  = Indi(bufferToSell, i, p);
        double indi2 = Indi(bufferToSell, i + 1, p);
        if (indi > highLevel && indi2 <= highLevel) return true;
