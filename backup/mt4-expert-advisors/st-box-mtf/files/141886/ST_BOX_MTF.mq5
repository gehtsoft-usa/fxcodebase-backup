...
//https://fxcodebase.com/code/viewtopic.php?f=38&t=71170
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      ...
      double buffer[3];
      if (CopyBuffer(cci, 0, index, 3, buffer) != 3)
      {
         return false;
      }
      return buffer[0] > buffer[1] && buffer[0] > buffer[2];
   }
};
...
int OnInit(void)
{
   cci = iCCI(_Symbol, tf, pCCIPer, PRICE_CLOSE);
   IndicatorObjPrefix = GenerateIndicatorPrefix("stbox");
   IndicatorSetString(INDICATOR_SHORTNAME, "+ST Box");
