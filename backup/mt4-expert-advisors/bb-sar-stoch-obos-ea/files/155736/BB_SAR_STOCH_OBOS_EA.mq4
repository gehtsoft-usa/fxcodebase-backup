...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=74971
...
input enum_tsl_mode tslmode     = tsl_by_sar;                 // TSL Mode:
...
Strategy  st_TSL_by_pips(&conditionTSL, &actionTSL);



class ConditionTslBySar : public iConditions
{
    public:
    bool evaluate()
    {
...
ConditionTslBySar cdTslBySar;

class ActionTslBySar : public iActions
{
    public:
    bool execute()
    {
...
ActionTslBySar acTslBySar;

Strategy  st_TSL_by_sar(&cdTslBySar, &acTslBySar);


void      Initiate_TSL()
{
    if(tslmode == tsl_by_pips){ Trader.management.add(&st_TSL_by_pips);}
    if(tslmode == tsl_by_sar) { Trader.management.add(&st_TSL_by_sar);}
}
...
input string       Tcustom        = "==== Stochastic Setup ===="; // ————————————————————————
input int KPeriod=8;
input int DPeriod=3;
input int Slowing=3;
//--- 
int bu_UP_divergence   = 0;
int bu_DN_divergence   = 1;
int bu_line_k = 0;
int bu_line_D = 1;

bool StochDivergenceOn = true; // Signals in Stoch Divergence?
input bool StochCrossOn = true; // Signals in MVA cross?
input bool cLevelsOn = true; // Cross signals must respect the levels?
input double stoch_buy_level = 20; // Lower Level:
input double stoch_sell_level = 80; // Upper Level:

//--- 
bool               indicator_init = true;
//--- 

double Stoch(int buffer, int candle = 1)
{
    // return iCustom(NULL, 0, indicator_file, KPeriod, DPeriod, Slowing, buffer, candle);
    return iStochastic(NULL, 0, KPeriod, DPeriod, Slowing, MODE_SMA, 0, buffer, candle);
}

class ConditionDivergenceBuy : public iConditions
{
  public:
    bool evaluate()
    {
        double indi = Stoch(bu_UP_divergence, 1);
        return indi > 0 && indi != EMPTY_VALUE;
    }
};
ConditionDivergenceBuy cdDivergenceBuy;

class ConditionDivergenceSell : public iConditions
{
  public:
    bool evaluate()
    {
        double indi = Stoch(bu_DN_divergence, 1);
        return indi > 0 && indi != EMPTY_VALUE;
    }
};
ConditionDivergenceSell cdDivergenceSell;

class ConditionCrossBuy : public iConditions
{
  public:
    bool evaluate()
    {
        double k = Stoch(bu_line_k, 1);
        double k2 = Stoch(bu_line_k, 2);
        double d = Stoch(bu_line_D, 1);
        double d2 = Stoch(bu_line_D, 2);

        bool cCross = k>d && k2<=d2;
    
        if(cLevelsOn)
        {
            bool cLevels = k < stoch_buy_level && d < stoch_buy_level;
            return cCross && cLevels;
        }
        return cCross;
    }
};
ConditionCrossBuy cdCrossBuy;

class ConditionCrossSell : public iConditions
{
  public:
    bool evaluate()
    {
        ...
        if(cLevelsOn)
        {
            bool cLevels = k > stoch_sell_level && d > stoch_sell_level;
            return cCross && cLevels;
        }
        return cCross;
    }
};
ConditionCrossSell cdCrossSell;
...
input string       Tobos        = "==== OBOS Setup ===="; // ————————————————————————
input const string obos_file = "OBOS.ex4";                  // Indicator File:
input bool ObosFilterOn = true; // Filter by Obos?
input int Length=9;
input int MaxBars=100;
int bu_obos_fast = 0;
int bu_obos_slow = 1;

double obos(int buffer, int candle = 1)
{
    return iCustom(NULL, 0, obos_file, Length, MaxBars, buffer, candle);
}

class ConditionOBOS_Buy : public iConditions
{
    public:
    bool evaluate()
    {
        double f = obos(bu_obos_fast, 1);
        double s = obos(bu_obos_slow, 1);
        return f < s && f<0 && s<0;        
    }
};
ConditionOBOS_Buy cdOBOS_Buy;

class ConditionOBOS_Sell : public iConditions
{
    public:
    bool evaluate()
    {
...
ConditionOBOS_Sell cdOBOS_Sell;
...
input string       Tsar        = "==== SAR Setup ===="; // ————————————————————————
input bool SarFilterOn = true; // Filter by SAR?
input double sar_step=0.02; // Step
input double sar_max=0.2;// Maximum

double sar(int buffer, int candle = 1)
{
    return iSAR(NULL,0, sar_step, sar_max, candle);
}

class ConditionSAR_Buy : public iConditions
{
    public:
    bool evaluate()
    {
        return sar(0, 1) < iLow(NULL,0,1);
    }
};
ConditionSAR_Buy cdSAR_Buy;

class ConditionSAR_Sell : public iConditions
{
    public:
    bool evaluate()
    {
           return sar(0, 1) > iHigh(NULL,0,1);
    }
};
ConditionSAR_Sell cdSAR_Sell;




input string       Tbb        = "==== Bollinguer Bands Setup ===="; // ————————————————————————
input bool BBFilterOn = true; // Filter by BB?
input int bb_period = 20; // BB Period:
input double bb_desv = 2;// BB Desviation:
int bb_shift = 0;
ENUM_APPLIED_PRICE bb_applied_price = PRICE_CLOSE;
int bu_main = 0;
int bu_up = 1;
int bu_low = 2;

double bb(int buffer, int candle = 1)
{
    return iBands(NULL, 0, bb_period,bb_desv,bb_shift,bb_applied_price, buffer, candle);
}

class ConditionBB_Buy : public iConditions
{
    public:
    bool evaluate()
    {
        // abajo de la low
        // return bb(bu_low, 1) > iClose(NULL, 0, 1);

        // abajo de la main
        return bb(bu_main, 1) > iClose(NULL, 0, 1);
    }
};
ConditionBB_Buy cdBB_Buy;

class ConditionBB_Sell : public iConditions
{
    public:
    bool evaluate()
    {
        // arriba del high
        // return bb(bu_up, 1) < iClose(NULL, 0, 1);

        // arriba de la main
        return bb(bu_main, 1) < iClose(NULL, 0, 1);
    }
};
ConditionBB_Sell cdBB_Sell;





// Mark: OnInit_CustomIndicator
bool OnInit_CustomIndicator()
{
    double temp = iCustom(NULL, 0, obos_file, 0, 0);
    if (GetLastError() == ERR_INDICATOR_CANNOT_LOAD) {
        MessageBox("THIS EA NEED AN INDICATOR\n install the file:\n" + obos_file + "\ninto the folder:\nMQL4/Indicators.", "Important Information", MB_ICONINFORMATION);
        Alert("THIS EA NEED AN INDICATOR, install the file: " + obos_file + " into the folder: MQL4/Indicators.");
    ...
    if(StochDivergenceOn) 
    {
        LongStrategy.addCondition(&cdDivergenceBuy);
        ShortStrategy.addCondition(&cdDivergenceSell);
    }
    if(StochCrossOn)
    {
        LongStrategy.addCondition(&cdCrossBuy);
        ShortStrategy.addCondition(&cdCrossSell);        
    }

    if(ObosFilterOn)
    {
        LongStrategy.addCondition(&cdOBOS_Buy);
        ShortStrategy.addCondition(&cdOBOS_Sell);        
    }
    
    if(SarFilterOn)
    {
        LongStrategy.addCondition(&cdSAR_Buy);
        ShortStrategy.addCondition(&cdSAR_Sell);        
    }

    if(BBFilterOn)
    {
            LongStrategy.addCondition(&cdBB_Buy);
            ShortStrategy.addCondition(&cdBB_Sell);        
    }

    // Trader.strategys.add(&IndicatorStrategyLong);
    // Trader.strategys.add(&IndicatorStrategyShort);
