//Available @  https://fxcodebase.com/code/viewtopic.php?f=38&p=157742
...
class PositionsMannagement
{
  ...
  public:
    ...
    void set(string sym, int magi=0)
    {
        _symbol = sym;
        _magic  = magi;
    }
...
double GetAverageOpenPrice(string symbol, int magic)
{
    double totalLots = 0;
    double totalPrice = 0;
    for (int i = OrdersTotal() - 1; i >= 0; i--)
...
void DrawHorizontalLine(string name, double price, color lineColor = clrDarkGray, int width = 1, int style = STYLE_DOT)
{
...
void DrawAveragePrice(string symbol, int magic)
{
    
    if(OrdersTotal() == 0)
    {
        if (ObjectFind("AverageOpenPrice") != -1) ObjectDelete("AverageOpenPrice");
    }
    double averagePrice = GetAverageOpenPrice(symbol, magic);
    if (averagePrice != 0)
    {
        DrawHorizontalLine("AverageOpenPrice", averagePrice);
    }
}

void HandleStopLosses()
{
    // si no hay ordenes abiertas, salir
    if (OrdersTotal() == 0) return;

    double averagePrice = GetAverageOpenPrice(_Symbol, magico);
    
    // recorrer las ordenes
    // cambiar el sl al sl basado en el promedio
    for (int i = OrdersTotal() - 1; i >= 0; i--) {
        if (OrderSelect(i, SELECT_BY_POS, MODE_TRADES)) {
            if (OrderSymbol() == _Symbol) {
                string side         = OrderType() == OP_BUY ? "buy" : "sell";
                double newStopLoss  = StopLoss(averagePrice, side, _Symbol, OrderLots());
                    if(OrderStopLoss() == newStopLoss) continue;
                    if (newStopLoss != 0) {
...
int LastOrderClosedBySL_Candles()
{
    int lastOrder = OrdersHistoryTotal() - 1;
    if (OrderSelect(lastOrder, SELECT_BY_POS, MODE_HISTORY)) {
        if (OrderProfit() < 0) {
            ...
            int    candles    = 0;
            // contar las velas desde el cierre de la orden hasta ahora
            for (int i = 0; i < Bars; i++) {
                if (iTime(NULL, 0, i) > closeTime) {
                    candles++;
                }
            }
            return candles;
...
int CountCandlesSincePriceCrossedMA(string side)
{
    int count = 0;
    // hace cuantas velas está arriba de la ema
    if(side == "up")
    {
        for (int i = 1; i < Bars; i++) {
            double maValue = iMA(_Symbol, 0, ma1_period, 0, MODE_EMA, PRICE_CLOSE, i);
            if (iClose(_Symbol, 0, i) > maValue) {
                count++;
            }
            if (iClose(_Symbol, 0, i) < maValue) break;
        }
    }
    if(side == "down")
    {
...
/*
 función que recorre las ordenes abiertas y determina la dirección de la primer orden abierta, 
 luego llama a la funcion de contar velas desde que se cruzó la media movil,
 si la dirección de la primer orden abierta es buy, contar velas desde que el precio cruzó la media movil hacia abajo
 si la dirección de la primer orden abierta es sell, contar velas desde que el precio cruzó la media movil hacia arriba
 si la cantidad de velas es mayor a 5, entonces cerrar todas las ordenes abiertas
*/
void CloseOrdersIfCandlesCrossedMA()
{
    if (OrdersTotal() == 0) return;

    string firstOrderSide = "";
    for (int i = OrdersTotal() - 1; i >= 0; i--) {
        if (OrderSelect(i, SELECT_BY_POS, MODE_TRADES)) {
            if (OrderSymbol() == _Symbol) {
                firstOrderSide = OrderType() == OP_BUY ? "buy" : "sell";
                break;
            }
        }
    }

    int candlesSinceCross = 0;
    if (firstOrderSide == "buy") {
        candlesSinceCross = CountCandlesSincePriceCrossedMA("down");
        Print("line: ",__LINE__," candlesSinceCross DOWN: ",candlesSinceCross);
    } else if (firstOrderSide == "sell") {
        candlesSinceCross = CountCandlesSincePriceCrossedMA("up");
        Print("line: ",__LINE__," candlesSinceCross UP: ",candlesSinceCross);
    }

    if (candlesSinceCross > nCandlesToClose) {
        for (int i = OrdersTotal() - 1; i >= 0; i--) {
            if (OrderSelect(i, SELECT_BY_POS, MODE_TRADES)) {
                if (OrderSymbol() == _Symbol) {
                    OrderClose(OrderTicket(), OrderLots(), MarketInfo(_Symbol, MODE_BID), 3);
                }
            }
        }
    }
...
{
    Print("line: ",__LINE__," SL price: ",price);
...
input int            nCandlesToWait = 5;                  // Candles to wait after SL
// ------------------------------------------------------------------
...
const double tsl_step     = 1;                          // TSL Step:
...
input bool               ma1_on               = true;                           // MA Filter ON:
...
input bool               closeAllIfCrossedEma = true;                           // Close all orders if price crossed ema
input int                nCandlesToClose      = 5;                              // Number of candles to close all after crossing ema

double ma1(int shift) { return iMA(NULL, 0, ma1_period, ma1_shift, ma1_method, ma1_applied_price, shift); }

class ConditionAboveEma : public iConditions
{
  public:
    bool evaluate() { return iClose(NULL, 0, 1) > ma1(1); }
};
ConditionAboveEma cdPriceAboveEma;

class ConditionBelowEma : public iConditions
{
  public:
      bool evaluate() { return iClose(NULL, 0, 1) < ma1(1); }
};
ConditionBelowEma cdPriceBelowEma;
...
int init()
{
    ...
    magico = I_i_0;
    Trader.management.set(_Symbol, (int)magico);
...
int start()
{
    ...
    Trader.doTrading();
    DrawAveragePrice(_Symbol, (int)magico);
    HandleStopLosses();

    // si la ultima orden cerrada fue por sl, no abrir nuevas ordenes durante x velas
    if (LastOrderClosedBySL()) {
        int candles = LastOrderClosedBySL_Candles();
        if (candles < nCandlesToWait) {
            return 0;
        }
    }

    if (closeAllIfCrossedEma) {
        CloseOrdersIfCandlesCrossedMA();
    }
    ...
    */
    ...
    L_s_2  = "false";
    G_i_75 = I_b_11;
    if ((G_i_75 == 0) || (I_b_11 && I_i_78 > I_i_79 && Hour() >= I_i_79 && Hour() <= I_i_78) || (I_i_79 > I_i_78 && Hour() < I_i_78 && Hour() > I_i_79)) {
        L_s_1 = "true";
    }
    if (I_b_11) {
        if ((I_i_78 > I_i_79 && Hour() < I_i_79 && Hour() > I_i_78) || (I_i_79 > I_i_78 && Hour() >= I_i_78 && Hour() <= I_i_79)) {
            L_s_2 = "true";
        }
    }
    if (I_b_12) {
...
int f0_15(int Fa_i_00, double Fa_d_01, double Fa_d_02, int Fa_i_03, double Fa_d_04, double Fa_d_05, int Fa_i_06, string Fa_s_07, int Fa_i_08, int Fa_i_09, int Fa_i_0A)
{
    ...
    if (I_i_1 == 2) {
        ...
        do {
            ...
            if (ma1_on == true && cdPriceAboveEma.evaluate() == true)
            {                
                double sl = StopLoss(Fa_d_02, "buy",_Symbol, Fa_d_01 );
                L_i_11 = OrderSend(_Symbol, 2, Fa_d_01, Fa_d_02, Fa_i_03, sl, G_d_3, Fa_s_07, Fa_i_08, Fa_i_09, G_i_31);            
            } 
            if (!ma1_on)
            {
                double sl = StopLoss(Fa_d_02, "buy",_Symbol, Fa_d_01 );
    ...
    if (I_i_1 == 4) {
        ...
        do {
            ...
            {                
                double sl = StopLoss(Fa_d_02, "buy",_Symbol, Fa_d_01 );
                L_i_11 = OrderSend(_Symbol, 4, Fa_d_01, Fa_d_02, Fa_i_03, sl, G_d_109, Fa_s_07, Fa_i_08, Fa_i_09, G_i_33);            
            }
            if (!ma1_on)
            {
                double sl = StopLoss(Fa_d_02, "buy",_Symbol, Fa_d_01 );            
                L_i_11 = OrderSend(_Symbol, 4, Fa_d_01, Fa_d_02, Fa_i_03, sl, G_d_111, Fa_s_07, Fa_i_08, Fa_i_09, G_i_33);
            }
    ...
    if (I_i_1 == 0) {
        ...
        do {
            ...
            if (ma1_on == true && cdPriceAboveEma.evaluate() == true)
            {                
                double sl = StopLoss(Ask, "buy", _Symbol, Fa_d_01 );
                L_i_11 = OrderSend(_Symbol, 0, Fa_d_01, Ask, Fa_i_03, sl, G_d_115, Fa_s_07, Fa_i_08, Fa_i_09, G_i_36);            
            } 
            if (!ma1_on)
            {
                double sl = StopLoss(Ask, "buy", _Symbol, Fa_d_01 );            
    ...
    if (I_i_1 == 3) {
        ...
        do {
            ...
            if (ma1_on == true && cdPriceBelowEma.evaluate()== true)
            {                
                double sl = StopLoss(Fa_d_02, "sell",_Symbol, Fa_d_01 );
                L_i_11 = OrderSend(_Symbol, 3, Fa_d_01, Fa_d_02, Fa_i_03, sl, G_d_118, Fa_s_07, Fa_i_08, Fa_i_09, G_i_38);            
            } 
            if (!ma1_on)
            {
                double sl = StopLoss(Fa_d_02, "sell",_Symbol, Fa_d_01);
    ...
    if (I_i_1 == 5) {
        ...
        do {
            ...
            {                
                double sl = StopLoss(Fa_d_02, "sell",_Symbol, Fa_d_01 );
                L_i_11 = OrderSend(_Symbol, 5, Fa_d_01, Fa_d_02, Fa_i_03, sl, G_d_121, Fa_s_07, Fa_i_08, Fa_i_09, G_i_41);            
            } 
            if (!ma1_on)
            {
                double sl = StopLoss(Fa_d_02, "sell",_Symbol, Fa_d_01 );            
    ...
    if (I_i_1 == 1) {
        ...
        do {
            ...
            {                
                double sl = StopLoss(Fa_d_02, "sell",_Symbol, Fa_d_01 );
                L_i_11 = OrderSend(_Symbol, 1, Fa_d_01, Bid, Fa_i_03, sl, G_d_124, Fa_s_07, Fa_i_08, Fa_i_09, G_i_44);            
            } 
            if (!ma1_on)
            {
                double sl = StopLoss(Fa_d_02, "sell",_Symbol, Fa_d_01 );
