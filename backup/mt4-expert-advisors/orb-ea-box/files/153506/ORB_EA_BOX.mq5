...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=74393
...
int LSMAShortPeriod = 7;  //
int LSMALongPeriod = 16;
int PipsDifference = 16;

input string       T_ = "== ORB Setup ==";        // ————————————
input ENUM_TIMEFRAMES MainTF = PERIOD_M15; // TimeFrame:
input int minOrbSize = 100; // Min Size:
input int maxOrbSize = 600; // Max Size:
input double offset = 100;// Offset Points:
input bool continueMode = false; // Continue Mode:

input string       T0 = "== Trade Setup ==";        // ————————————
input int      magico = 1234;                       // Magic Number:
input ModeBar barMode = Live;                       // Trade Mode:
...
string       tTslBypips = "-- TSL By Pips Setup --";  // —
...
input double             closeAllMoney = 10;                        // Close by Money Winning $(+)
...
input string       timeStart = "08:00:00";          // Time Start GMT
input string       timeEnd = "14:00:00";            // Time End GMT
...
class ORB
{
    ...
    bool filterSize()
    {
        double size = (iHigh(NULL, PERIOD_CURRENT, 1) - iLow(NULL, PERIOD_CURRENT, 1)) / _Point;
        Print(__FUNCTION__, " size: ", size);
    ...
    void setORB()
    {
        string bar = (string)iBars(NULL, PERIOD_CURRENT);
        VLineCreate(0, "ORB_" + bar, 0, iTime(NULL, PERIOD_CURRENT, 1));


        if(filterSize()){
            high = iHigh(NULL, PERIOD_CURRENT, 1) + offset * _Point;
            Print(__FUNCTION__, " high: ", high);

            MqlDateTime dt;
            TimeGMT(dt);// Current time GMT
            Print("hour: " + dt.hour + " Min: " + dt.min + " Sec: " + dt.sec);

            low = iLow(NULL, PERIOD_CURRENT, 1) - offset * _Point;
...
class BUYcondition1 : public iConditions
{
    ...
    bool evaluate()
    {
        ...
        return iClose(Symbol(), 0, 1) > orb.high && iOpen(Symbol(), 0, 1) < orb.high && orb.high != 0;
...
class SELLcondition1 : public iConditions
{
    ...
    bool evaluate()
    {
        ...
        return iClose(Symbol(), 0, 1) < orb.low && iOpen(Symbol(), 0, 1) > orb.low && orb.low != 0;
        // return false;
    }
};
...
datetime StartORB()
{
    ...
    int ss = (int) StringSubstr(timeStart, 6, 2);
    ...
    dt.sec = ss;
    // Print("hour: " + dt.hour + " Min: " + dt.min + " Sec: " + dt.sec);
    return StructToTime(dt)+PeriodSeconds();
}
...
int OnInit()
{
    ...
    ChartSetSymbolPeriod(0, Symbol(), MainTF);
...
void OnTick()
{
    ...
    // CM true: tiene que chequear todas las velas mientras que
    // cm false: solo la primera vela de la session

    if(continueMode == false && orb.tradingOn == false) return;


    datetime orbStart = timeStart + PeriodSeconds(); // time when close the first candle of the session
    if(TimeGMT() >= StartORB() && orb.high == 0) orb.setORB();
    ...
    if(!uTradeReverse){
        ...
        if(conditionsToBuy.EvaluateConditions()) {
            ...
            if(actionSendOrder.doAction()) {
                ...
                {
                    // GridUser_gap
                    gridBuy = new Grid(_Symbol, "buy", MainOrders.last().price(), orb.distance / 10, GridUser_multiplier, GridUser_maxCount, GridUser_maxLot, MainOrders.last().lot(), magico);
        ...
        if(conditionsToSell.EvaluateConditions()) {
            ...
            if(actionSendOrder.doAction()) {
                ...
                if(GridON == true && CheckPointer(gridSell) == POINTER_INVALID)
                {
                    ...
                    gridSell = new Grid(_Symbol, "sell", MainOrders.last().price(), orb.distance / 10, GridUser_multiplier, GridUser_maxCount, GridUser_maxLot, MainOrders.last().lot(), magico);
...
void CloseAndDeleteGrid(string side="")
{
    if(side == "" || side == "buy")
    {
        if(CheckPointer(gridBuy) != POINTER_INVALID) {
