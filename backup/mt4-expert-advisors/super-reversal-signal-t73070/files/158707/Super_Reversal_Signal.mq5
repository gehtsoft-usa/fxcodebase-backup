...
// https://fxcodebase.com/code/viewtopic.php?f=38&p=158521#p158521
...
//
   int i, limit;
   int tr = 0;
   limit = MathMin(rates_total - 4, BarLimit);
   for(i = limit; i > 0 && !IsStopped() && i < rates_total; i--)
     {
      double gap = 3.0 * iATRMQL4(Symbol(), Period(), 20, i) / 4.0;
      Print(i + "  " + gap);
      body[i] = MathAbs(open[i] - close[i]);
      trend[i] = trend[i + 1];
      if(!ArrowOnlyTrendChange)
         tr = 0;
      if(LagBar == 1)
        {
      ...
      if(LagBar != 1)
        {
         ...
         trend[i] = trend[i + 1];
         body[i] = MathAbs(open[i] - close[i]);
         if((high[i + 1] <= close[i]) && (body[i] > body[i + 1]) && (body[i] > body[i + 2]) && (body[i] > body[i + 3]))
           {
            if(tr == 0 || tr == -1)
              {
               trend[i] = 1;
         ...
         if((low[i + 1] >= close[i]) && (body[i] > body[i + 1]) && (body[i] > body[i + 2]) && (body[i] > body[i + 3]))
           {
            if(tr == 0 || tr == 1)
              {
               trend[i] = -1;
