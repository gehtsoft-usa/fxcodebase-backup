...
int init()
{
   IndicatorObjPrefix = GenerateIndicatorPrefix("adx_s_s_c_o");
   IndicatorShortName("ADX strenght meter Currency Oscillator");
   ...
   while (pos >= 0)
   {
      for (int i = 0; i < ArraySize(lines); ++i)
      {
         ...
         for (int index = 0; index < ArraySize(lines[i].Symbols); index++)
         {
            ...
            double adx = iADX(symbolData.Symbol, _Period, adx_period, PRICE_CLOSE, MODE_MAIN, pos);
