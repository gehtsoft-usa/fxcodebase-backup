...
// http://fxcodebase.com/code/viewtopic.php?f=38&t=70786
//+------------------------------------------------------------------+
...
input SingalMode signal_mode = SingalModeOnBarClose; // Signal mode
...
public:
   ...
   {
      return ExtTenkanBuffer[period] <= Close[period] && ExtTenkanBuffer[period + 1] > Close[period + 1];
   }
};
...
int init()
{
   ...
   IndicatorName = GenerateIndicatorName("Ichimoku Kijun Alert");
...
int start()
{
   ...
   int minBars = tenkan_sen;
   int limit = MathMin(Bars - 1 - minBars, Bars - counted_bars - 1);
   for (int pos = limit; pos >= 0; --pos)
   {
      ExtTenkanBuffer[pos] = iIchimoku(_Symbol, _Period, tenkan_sen, kijun_sen, senkoi_span_b, MODE_TENKANSEN, pos);
      ExtKijunBuffer[pos] = iIchimoku(_Symbol, _Period, tenkan_sen, kijun_sen, senkoi_span_b, MODE_KIJUNSEN, pos);
      ExtSpanA_Buffer[pos] = iIchimoku(_Symbol, _Period, tenkan_sen, kijun_sen, senkoi_span_b, MODE_SENKOUSPANA, pos);
      ExtSpanB_Buffer[pos] = iIchimoku(_Symbol, _Period, tenkan_sen, kijun_sen, senkoi_span_b, MODE_SENKOUSPANB, pos);
      ExtChikouBuffer[pos] = iIchimoku(_Symbol, _Period, tenkan_sen, kijun_sen, senkoi_span_b, MODE_CHIKOUSPAN, pos);
      ExtSpanA2_Buffer[pos] = iIchimoku(_Symbol, _Period, tenkan_sen, kijun_sen, senkoi_span_b, MODE_SENKOUSPANA, pos);
      ExtSpanB2_Buffer[pos] = iIchimoku(_Symbol, _Period, tenkan_sen, kijun_sen, senkoi_span_b, MODE_SENKOUSPANB, pos);
      if (customStream != NULL)
      {
