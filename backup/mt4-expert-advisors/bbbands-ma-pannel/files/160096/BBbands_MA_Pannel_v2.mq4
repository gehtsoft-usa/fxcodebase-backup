...
// https://fxcodebase.com/code/viewtopic.php?f=38&p=147611#p147611
...
string activeTFNames[];
int activeTFCount = 0;

// ------------------------------------------------------------------
input string             TitlePairs     = "==== PAIRS ====";       // Set your pairs
input string             uSymbols       = "GBPUSD,EURUSD";         // Symbols (add separate by comma ","):
input string             TitleIndi      = "==== INDICATORS ====";  // Set your indicators
input int                bbPeriods      = 50;                      // BB Periods:
input int                bbDesviation   = 3;                       // BB Desviation:
input int                maPeriods      = 200;                     // MA Periods:
input ENUM_MA_METHOD     maMethod       = MODE_SMA;                // MA Method:
input ENUM_APPLIED_PRICE maAppliedPrice = PRICE_CLOSE;             // MA Applied Price
// ------------------------------------------------------------------
input string TitleTF               = "==== TIMEFRAMES ====";  // Timeframe Selection
input bool   ShowM1                = true;                    // Show M1
...
input bool   ShowW1                = true;                    // Show W1
// ------------------------------------------------------------------
...
class FinderPattern
{
 ...
 public:
  ...
  bool havePatternUp(int i, ENUM_TIMEFRAMES tf)
  {
    ...
    double lowBand     = iBands(_symbol, tf, bbPeriods, bbDesviation, 0, PRICE_CLOSE, 2, i);
    double lowBandPrev = iBands(_symbol, tf, bbPeriods, bbDesviation, 0, PRICE_CLOSE, 2, i + 1);
    double ema         = iMA(_symbol, tf, maPeriods, 0, maMethod, maAppliedPrice, i);
    double emaPrev     = iMA(_symbol, tf, maPeriods, 0, maMethod, maAppliedPrice, i + 1);

    if (lowBand < ema && lowBandPrev >= emaPrev) return true;
    if (lowBand > ema && lowBandPrev <= emaPrev) return true;
    return false;
  }
  ...
  bool havePatternDn(int i, ENUM_TIMEFRAMES tf)
  {
    ...
    double hiBand     = iBands(_symbol, tf, bbPeriods, bbDesviation, 0, PRICE_CLOSE, 1, i);
    double hiBandPrev = iBands(_symbol, tf, bbPeriods, bbDesviation, 0, PRICE_CLOSE, 1, i + 1);
    double ema        = iMA(_symbol, tf, maPeriods, 0, maMethod, maAppliedPrice, i);
    double emaPrev    = iMA(_symbol, tf, maPeriods, 0, maMethod, maAppliedPrice, i + 1);

    if (hiBand < ema && hiBandPrev >= emaPrev) return true;
    if (hiBand > ema && hiBandPrev <= emaPrev) return true;
    return false;
  }
};
...
class Table : CObjectBase
{
 ...
 public:
    ...
    if (id == CHARTEVENT_OBJECT_CLICK)
    {
			...
			for (int r = 1; r < _nRows; r++)
      {
	        ...
	        if(_rows[r, c].Name() == sparam) {
						// Find which timeframe column was clicked
						int tfIndex = (c - 1) / 2;
						if(tfIndex < activeTFCount)
						{
							ChartSetSymbolPeriod(0, _rows[r, 0].Text(), activeTF[tfIndex]);
							break;
						}
	       	}
        }
  ...
  void Create()
  {
    ...
    // nColumns is now set dynamically in CreateTable()
    int y = 30;
    int x = 5;
...
CNewCandle newCandle(_Symbol, PERIOD_M1);
...
int OnInit()
{
  ...
  // Initialize active timeframes
  InitializeActiveTimeframes();
...
// Add new function to initialize active timeframes
void InitializeActiveTimeframes()
{
  activeTFCount = 0;
  
  // Count active timeframes
  if(ShowM1) activeTFCount++;
  if(ShowM5) activeTFCount++;
  if(ShowM15) activeTFCount++;
  if(ShowM30) activeTFCount++;
  if(ShowH1) activeTFCount++;
  if(ShowH4) activeTFCount++;
  if(ShowD1) activeTFCount++;
  if(ShowW1) activeTFCount++;
  
  // Resize arrays
  ArrayResize(activeTF, activeTFCount);
  ArrayResize(activeTFNames, activeTFCount);
  
  // Fill arrays with active timeframes
  int index = 0;
  if(ShowM1) { activeTF[index] = PERIOD_M1; activeTFNames[index] = "M1"; index++; }
  if(ShowM5) { activeTF[index] = PERIOD_M5; activeTFNames[index] = "M5"; index++; }
  if(ShowM15) { activeTF[index] = PERIOD_M15; activeTFNames[index] = "M15"; index++; }
  if(ShowM30) { activeTF[index] = PERIOD_M30; activeTFNames[index] = "M30"; index++; }
  if(ShowH1) { activeTF[index] = PERIOD_H1; activeTFNames[index] = "H1"; index++; }
  if(ShowH4) { activeTF[index] = PERIOD_H4; activeTFNames[index] = "H4"; index++; }
  if(ShowD1) { activeTF[index] = PERIOD_D1; activeTFNames[index] = "D1"; index++; }
  if(ShowW1) { activeTF[index] = PERIOD_W1; activeTFNames[index] = "W1"; index++; }
}
...
void CreateTable()
{
  int sym = symbols.qnt();
  table.nRows(sym);
  table.nColumns(activeTFCount * 2 + 1); // Update column count based on active timeframes
  table.Create();
  
  // Create dynamic headers
  string headers[];
  ArrayResize(headers, activeTFCount * 2 + 1);
  headers[0] = " ";
  
  for(int i = 0; i < activeTFCount; i++)
  {
    headers[i * 2 + 1] = activeTFNames[i];
    headers[i * 2 + 2] = " ";
  }
                ...
                const int&      spread[])
{
	...
	if(newCandle.IsNewCandle()) { doActions(); NotifyControl();}
...
void doActions()
{
	...
	for(int i=0;i < ArraySize(finders) ; i++)
	{
		...
		ArrayResize(candleNumber, activeTFCount);
		ArrayResize(patternDirection, activeTFCount);
		ArrayResize(results, activeTFCount * 2);
	
		for (int j = 0; j < activeTFCount; j++) 
		{
			string patCode = finders[i].FindPattern(activeTF[j]);	
			candleNumber[j] = StringSubstr(patCode,0,2);
			patternDirection[j] = StringSubstr(patCode,2,1);
		}
		
		for(int n=0;n < activeTFCount * 2;n++)
		{
	...
	if(rowSymbol!= -1)
	{
		// Find the current period in active timeframes
		int currentTFIndex = -1;
		for(int i = 0; i < activeTFCount; i++)
		{
			if(Period() == activeTF[i])
			{
				currentTFIndex = i;
				break;
			}
		...
		if(currentTFIndex != -1)
		{
			candle = table.getValue(rowSymbol, currentTFIndex * 2 + 2);
			direction = table.getValue(rowSymbol, currentTFIndex * 2 + 1);
...
void NotifyControl()
{
  for (int r = 1; r < table.nRows(); r++)
  {
    ...
    for (int c = 1; c < activeTFCount * 2 + 1; c += 2)
    {
      value = table.cellText(r, c);
      string sym = table.cellText(r, 0);
      if (c + 1 < activeTFCount * 2 + 1)
      {
        string tfName = table.cellText(0, c);
        if (value == "p" && table.cellText(r, c+1)=="01") Notifications(0, sym, tfName);
        if (value == "q" && table.cellText(r, c+1)=="01") Notifications(1, sym, tfName);
      }
    }
  }
}
...
// if(tf == "M1" && newCandle.IsNewCandle()==false) return;
