//Available @ https://fxcodebase.com/code/viewtopic.php?f=38&t=74170
...
input string Iindi = "== Indicator Setup ==";    // == Indicator Setup ==
input uint SlowLength=42; // период медленного зигзага
input uint FastLength=6; // период быстрого зигзага

string file_custom_indicator = "Boa ZigZag Arrows Duplex MT5 Indicator";
int handle = 0;

void setHandle()
{
    handle = iCustom(Symbol(), NULL, file_custom_indicator, SlowLength, FastLength );
}
...
input int          userSLpips = 50;                          // Pips SL
...
class BUYcondition1 : public iConditions
{
    ...
    bool evaluate()
    {
        ...
        for (int i = 0; i <= 3; i++)
        {
            ...
            // Print(__FUNCTION__, custom_indicator(2, i));
            double indi = custom_indicator(2, i);
            if(indi > 0 && indi != lastBuyPrice)
            {
                lastBuyPrice = indi;
                return true;
            }
...
class SELLcondition1 : public iConditions
{
    ...
    bool evaluate()
    {
        ...
        for(int i = 0; i <= 3; i++)
        {
            // Print(__FUNCTION__, custom_indicator(3, i));
            double indi = custom_indicator(3, i);
            if( indi > 0 && indi != lastSellPrice) 
            {
                lastSellPrice = indi;
                return true;
            }
