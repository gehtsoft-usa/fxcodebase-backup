...
// https://fxcodebase.com/code/viewtopic.php?f=38&p=158823#p158823
...
)
{
    ...
    if (prev_calculated > rates_total || prev_calculated < 0)
        ...
        if (prev_calculated > 0) to_copy += 10;
    }
    if (!RefreshBuffers(iMAHigh, iMALow, iATRx, ind_mahi, ind_malo, ind_atr, to_copy)) return (0);
...
//+------------------------------------------------------------------+
bool RefreshBuffers(double &hi_buffer[], double &lo_buffer[], double &atr_buffer[], int hi_handle, int lo_handle, int atr_handle, int amount)
{
    ResetLastError();
...
void doAlert(int forBar, string doWhat)
{
    ...
    if (previousAlert != doWhat || previousTime != iTime(NULL, 0, forBar)) {
        ...
        message        = StringFormat("%s at %s", Symbol() + TimeToString(TimeLocal(), TIME_SECONDS), " HalfTrend signal " + PeriodString() + doWhat);
        message_mobile = StringFormat("%s at %s", Symbol() + TimeToString(TimeLocal(), TIME_SECONDS), " HalfTrend signal " + PeriodString() + doWhat);
        if (alertsMessage) Alert(message);
        if (alertMobile) SendNotification(message_mobile);
