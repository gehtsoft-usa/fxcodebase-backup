...
// http://fxcodebase.com/code/viewtopic.php?f=27&t=70207
//+------------------------------------------------------------------+
...
public:
   ...
   {
      return k[period] <= os && d[period] <= os
         && !(k[period + 1] <= os && d[period + 1] <= os);
   }
};
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      return k[period] >= ob && d[period] >= ob
         && !(k[period + 1] >= ob && d[period + 1] >= ob);
   }
};
...
int init()
{
   ...
   IndicatorBuffers(8);
   IndicatorObjPrefix = GenerateIndicatorPrefix("emastoch");
   IndicatorShortName("EMA Stoch OBOS");
...
int start()
{
   ...
   {
      k[pos] = iStochastic(_Symbol, _Period, K_periods, D_periods, Slowing, method, 0, MODE_MAIN, pos);
      d[pos] = iMAOnArray(k, 0, ma_periods, 0, MODE_EMA, pos);
      if (customStream != NULL)
      {
         customStream.SetValue(pos, k[pos]);
      }
      for (int i = 0; i < ArraySize(conditions); ++i)
      {
         AlertSignal* item = conditions[i];
