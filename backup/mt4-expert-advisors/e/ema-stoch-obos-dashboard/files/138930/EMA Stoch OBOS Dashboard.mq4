...
// http://fxcodebase.com/code/viewtopic.php?f=38&t=70634
//+------------------------------------------------------------------+
...
input int ma_periods = 14; // MA period
...
public:
   ...
   {
      double value = iCustom(_symbol, _timeframe, "EMA Stoch OBOS", K_periods, D_periods, Slowing, method, os, ob, ma_periods, 2, period);
...
public:
   ...
   virtual bool IsPass(const int period, const datetime date)
   {
      double value = iCustom(_symbol, _timeframe, "EMA Stoch OBOS", K_periods, D_periods, Slowing, method, os, ob, ma_periods, 3, period);
      return value != 0 && value != EMPTY_VALUE;
   }
};
...
string IndicatorName = "EMA Stich OBOS Dashboard";
int init()
{
   double temp = iCustom(NULL, 0, "EMA Stoch OBOS", 0, 0);
   if (GetLastError() == ERR_INDICATOR_CANNOT_LOAD)
   {
      Alert("Please, install the 'EMA Stoch OBOS' indicator");
      return INIT_FAILED;
   }
   ...
   IndicatorObjPrefix = GenerateIndicatorPrefix("emasobosd");
