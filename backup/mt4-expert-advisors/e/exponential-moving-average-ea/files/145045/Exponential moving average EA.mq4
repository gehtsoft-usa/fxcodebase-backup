...
//https://fxcodebase.com/code/viewtopic.php?f=38&t=71886
//+------------------------------------------------------------------------------------------------+
...
input string IPFE       = "== PFE Setup ==";  // == PFE Setup ==
...
input string             Iema1          = "== Moving Average Setup ==";  // == Moving Average Setup ==
input int                maPeriod1      = 36;                            // Period
...
input string             Iema3          = "== Moving Average Setup ==";  // == Moving Average Setup ==
input int                maPeriod3      = 633;                           // Period
int                      maShift3       = 0;                             // Ma Shift
input ENUM_MA_METHOD     maMethod3      = MODE_EMA;                      // Method
input ENUM_APPLIED_PRICE maAppliedPrice3= PRICE_CLOSE;                   // Applied Price
...
class PFE : public iIndicators
{
  ...
  public:
   PFE(string Symbol, int TimeFrame)
   {
      _symbol = Symbol;
      _tf     = TimeFrame;
      _file   = "00jPFE_v100.ex4";
   }
   ~PFE() { ; }

   PFE* file(string setfile)
   {
      _file = setfile;
      return &this;
   }
   ...
   int actualSide()
   {
      if (calculate(0, 1) >= 0) { return 0; } else { return 1; }
   }
   int lastSide()
   {
      if (calculate(0, 2) >= 0) { return 0; } else { return 1; }
   }
...
class BUYcondition1 : public iConditions
{
  public:
   bool evaluate()
   {
      ...
      if (pfe.actualSide() == 0 && pfe.lastSide()==1)
      {
         return true;
      }
      return false;
  ...
  public:
   bool evaluate()
   {
      // NOTE: condition Buy 2
		if(ma1.index(1) > ma2.index(1) && ma2.index(1) > ma3.index(1) && ma3.index(1)> ma4.index(1))
		{
			return true;
		}
      return false;
...
class SELLcondition1 : public iConditions
{
  public:
   bool evaluate()
   {
      ...
      if (pfe.actualSide() == 1 && pfe.lastSide()==0)
      {
         return true;
      }
      return false;
...
int OnInit()
{
   ...
   ma2 = new MovingAverage(_Symbol, Period());
	...
	ma4 = new MovingAverage(_Symbol, Period());
   ma1.setSetup(maPeriod, maShift, maMethod, maAppliedPrice);
   ma2.setSetup(maPeriod1, maShift1, maMethod1, maAppliedPrice1);
   ma3.setSetup(maPeriod2, maShift2, maMethod2, maAppliedPrice2);
   ma4.setSetup(maPeriod3, maShift3, maMethod3, maAppliedPrice3);
