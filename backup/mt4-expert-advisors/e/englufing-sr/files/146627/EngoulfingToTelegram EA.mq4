...
//https://fxcodebase.com/code/viewtopic.php?f=38&t=72114&p=146627#p146627
...
//+------------------------------------------------------------------+
// clang-format off
#define Telegram_Files
#ifdef Telegram_Files 
...
void GetCustomInfo(TCustomInfo& info, const int _error_code, const ENUM_LANGUAGE _lang)
{
...
string           InpTemplates      = "";                                                // List of Templates:
// clang-format on

//+------------------------------------------------------------------+
// NOTE: TELEGRAM Input parameters
//+------------------------------------------------------------------+
input string te             = "== Telegram Seup == ";                            //----------------------
...
// ------------------------------------------------------------------
input string Tindicator      = "== Indicator Setup ==";  // Indicator Setup
color        colorBulls      = Blue;
color        colorBears      = Red;
input int    Distance        = 20;
bool         DrawLevel       = true;
input int    MaxPrevLevel    = 3;
input int    LevelExtendBar  = 15;
color        SupportColor    = RoyalBlue;
int          SupportWidth    = 2;
int          SupportStlye    = 0;
color        ResistanceColor = Orange;
int          ResistanceWidth = 2;
int          ResistanceStlye = 0;
...
int OnInit()
{
  if (OnInit_Telegram() == INIT_FAILED)
  {
    return INIT_FAILED;
  }
...
void Notifications(int type)
{
  ...
  if (telegramOn)
  {
    ...
    if (sendPic) bot.SendScreenShot(Channel_id, _Symbol, 0);
  }
}
...
{
  int    buffer = 0;
  double value  = iCustom(NULL, 0, "Englufing+sr.ex4", colorBulls, colorBears, Distance, DrawLevel, MaxPrevLevel, LevelExtendBar, SupportColor, SupportWidth, SupportStlye, ResistanceColor, ResistanceWidth, ResistanceStlye, buffer, 1);
  if (value > 0)
  {
    return true;
  }
