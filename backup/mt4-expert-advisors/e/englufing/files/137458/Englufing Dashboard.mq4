...
public:
   ...
   {
      double value = iCustom(_symbol, _timeframe, "Englufing", 0, period);
...
public:
   ...
   virtual bool IsPass(const int period, const datetime date)
   {
      double value = iCustom(_symbol, _timeframe, "Englufing", 1, period);
      return value != 0 && value != EMPTY_VALUE;
   }
};
...
string IndicatorName = "ed";
int init()
{
   double temp = iCustom(NULL, 0, "Englufing", 0, 0);
   if (GetLastError() == ERR_INDICATOR_CANNOT_LOAD)
   {
      Alert("Please, install the 'Englufing' indicator");
      return INIT_FAILED;
   }
   ...
   IndicatorObjPrefix = GenerateIndicatorPrefix("Engulfing Dashboard");
