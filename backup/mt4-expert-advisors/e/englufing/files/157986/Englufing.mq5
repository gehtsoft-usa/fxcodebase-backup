...
// http://fxcodebase.com/code/viewtopic.php?f=38&t=70407
...
public:
   ...
   void Update(int chartPeriod, int period, datetime date)
   {
      if (!_condition.IsPass(period, date))
      {
         _signals[chartPeriod] = EMPTY_VALUE;
         return;
      }
      Print(chartPeriod);
      if (period == 0)
...
public:
   ...
   {
      return iHigh(_symbol, _timeframe, period + 1) < iHigh(_symbol, _timeframe, period) 
         && iLow(_symbol, _timeframe, period + 1) > iClose(_symbol, _timeframe, period) 
         && iOpen(_symbol, _timeframe, period + 1) < iClose(_symbol, _timeframe, period + 1) 
         && iOpen(_symbol, _timeframe, period) > iClose(_symbol, _timeframe, period);
   }
};
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      return iLow(_symbol, _timeframe, period + 1) > iLow(_symbol, _timeframe, period) 
         && iHigh(_symbol, _timeframe, period + 1) < iClose(_symbol, _timeframe, period) 
         && iOpen(_symbol, _timeframe, period + 1) > iClose(_symbol, _timeframe, period + 1) 
         && iOpen(_symbol, _timeframe, period) < iClose(_symbol, _timeframe, period);
   }
};
...
int OnInit(void)
{
   IndicatorObjPrefix = GenerateIndicatorPrefix("Engulfing");
   IndicatorSetString(INDICATOR_SHORTNAME, "Engulfing");
   IndicatorSetInteger(INDICATOR_DIGITS, Digits());
   
   //register outputs
   ENUM_TIMEFRAMES timeframe = htf;
   int id = 0;
...
)
{
   ...
   int first = 0;
   for (int pos = /*MathMax(first, prev_calculated - 1)*/ rates_total - 5000; pos < rates_total - 1; ++pos)
   {
      ...
      {
         index = iBarShift(_Symbol, htf, date);
         if (index == iBarShift(_Symbol, htf, time[pos + 1]))
         {
            continue;
         }
      }
      for (int i = 0; i < ArraySize(alerts); ++i)
      {
         alerts[i].Update(oldIndex, index, date);
      }
   }
   return rates_total;
}
