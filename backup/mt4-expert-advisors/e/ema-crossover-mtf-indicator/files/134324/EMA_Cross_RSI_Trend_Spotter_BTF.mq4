...
// http://fxcodebase.com/code/viewtopic.php?f=38&t=69923
//+------------------------------------------------------------------+
...
#property description "Based on Jason Robinson (jnrtrading) http://www.jnrtrading.co.uk"
...
input int FasterEMA = 5; // Fast EMA Period
input int SlowerEMA = 200; // Slow EMA Period
input int RSIPeriod = 12; // RSI Period
input int barsBack = 600; // Bars back
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      ...
      double fasterEMAnow = iMA(_Symbol, tf, FasterEMA, 0, MODE_EMA, PRICE_CLOSE, index);
      double slowerEMAnow = iMA(_Symbol, tf, SlowerEMA, 0, MODE_EMA, PRICE_CLOSE, index);
      double RSInow = iRSI(_Symbol, tf, RSIPeriod, PRICE_CLOSE, index);
      return fasterEMAnow > slowerEMAnow && RSInow > 50;
   }
};
...
int init()
{
   ...
   IndicatorBuffers(8);
   IndicatorObjPrefix = GenerateIndicatorPrefix("emacrsits");
   IndicatorShortName("EMA Cross RSI Trend Spotter BTF");
