...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=67152
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      ...
      {
         double buffer = iCustom(_symbol, _timeframe, "Extreme_Spike", 
            MinorMinExtremeHeightATRs, MajorToMinorHeightRatio, MinorMinExtremeWidth, MajorMinExtremeWidth, 4, period + i);
         if (buffer != 0)
         {
            datetime signalDate = iTime(_symbol, _timeframe, period + i);
            if (signalDate > _lastSignal)
            {
               _lastSignal = signalDate;
               return buffer == 1;
            }
            return false;
         }
...
int OnInit()
{
   double temp = iCustom(NULL, 0, "Extreme_Spike", 0, 0);
   if (GetLastError() == ERR_INDICATOR_CANNOT_LOAD)
   {
      Alert("Please, install the 'Extreme_Spike' indicator");
      return INIT_FAILED;
   }
