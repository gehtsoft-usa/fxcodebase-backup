...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=74147
...
enum Enum_Condition {
    NotEmpty,     // Buffer not empty
    MoreThanZero, // Buffer more than zero
};

#ifdef CONTROL_CUSTOM_INDICATOR_FILE

input string         Tindi                 = "== Indicator Setup =="; // ————————————
input string         file_custom_indicator = "";                      // Indicator File:
input int            bufferToBuy           = 0;                       // Buffer to Buy:
input int            bufferToSell          = 1;                       // Buffer to Sell:
input Enum_Condition conditionToBuy        = NotEmpty;                // Condition to Buy:
input Enum_Condition conditionToSell       = NotEmpty;                // Condition to Sell:
...
input int uMaxBuys = 1;// # Buys:
input int uMaxSells = 1; // # Sells:
#endif
bool               uTradeReverse = false;         // Trade Reverse:
input int          magico        = 2022;          // Magic Number:
input bool         infinity      = true;          // Infinity Mode:

enum ShiftMode  
{
    current_candle, // Current candle
    next_candle // Next candle after signal
};

input ShiftMode shift_mode = current_candle; // Shift mode:
...
ConcurrentConditions conditionsToBuyFilter;
ConcurrentConditions conditionsToSellFilter;
...
class BUYcondition1 : public iConditions
{
  public:
    bool evaluate()
    {
        ...
        int i = 1;
        if(shift_mode ==current_candle) i = 0;
        
        if (conditionToBuy == NotEmpty)
            return Indi(bufferToBuy, i) != EMPTY_VALUE;
        
        if (conditionToBuy == MoreThanZero)
            return Indi(bufferToBuy, i) > 0;
...
class BUYcondition2 : public iConditions
{
  public:
    bool evaluate()
    {
        ...
        return lastTradeWasBuy() && lastTradeWasWinner();
    }
};
...
class SELLcondition1 : public iConditions
{
  public:
    bool evaluate()
    {
        ...
        if (conditionToSell == NotEmpty)
            return Indi(bufferToSell, i) != EMPTY_VALUE;
        if (conditionToSell == MoreThanZero)
            return Indi(bufferToSell, i) > 0;
...
class SELLcondition2 : public iConditions
{
  public:
    bool evaluate()
    {
        ...
        return lastTradeWasSell() && lastTradeWasWinner();
        return false;
    }
};
...
int OnInit()
{
    ...
    if (infinity) conditionsToSell.AddCondition(sellCondition2 = new SELLcondition2());

    // countTrades = new ConditionCountTrades(1);

#ifdef MAX_TRADES_AT_SAME_TIME
    conditionsToBuyFilter.AddCondition(countTrades = new ConditionCountTrades(uMaxTrades));
    conditionsToSellFilter.AddCondition(countTrades = new ConditionCountTrades(uMaxTrades));
    
    conditionsToSellFilter.AddCondition(countSells = new ConditionCountSells(uMaxSells));
    conditionsToBuyFilter.AddCondition(countBuys = new ConditionCountBuys(uMaxBuys));
#endif
...
if(OpenTradeOnThisBar())return;
    ...
    // NOTE: BUY normal
    if (!uTradeReverse) {
        if(conditionsToBuyFilter.EvaluateConditions())
        ...
        // NOTE: SELL normal
        if(conditionsToSellFilter.EvaluateConditions())
...
bool OpenTradeOnThisBar()
{
    for (int i = OrdersTotal() - 1; i >= 0; i--) {
        if (OrderSelect(i, SELECT_BY_POS) && OrderSymbol() == _Symbol && OrderMagicNumber() == magico) {
            return OrderOpenTime() >= iTime(NULL, 0, 0);
        }
    }
    return false;
}
