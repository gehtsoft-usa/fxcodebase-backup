...
//https://fxcodebase.com/code/viewtopic.php?f=38&t=72656
...
enum ModeLevels {
  FixPips,            // Fix Pips
  byMoney,            // Money
  PipsFromOpenCandle  // Pips ATR from Candle
};
enum TSLMode {
  byPips,  // By Pips
  byATR    // By ATR
...
input int                maPeriod1               = 100;                                      // Period
int                      maShift1                = 0;                                        // Ma Shift
input ENUM_MA_METHOD     maMethod1               = MODE_EMA;                                 // Method
input ENUM_APPLIED_PRICE maAppliedPrice1         = PRICE_CLOSE;                              // Applied Price
input string             Iema2                   = "===== Moving Average Slow Setup =====";  // ===== Moving Average Setup =====
input int                maPeriod2               = 200;                                      // Period
int                      maShift2                = 0;                                        // Ma Shift
input ENUM_MA_METHOD     maMethod2               = MODE_EMA;                                 // Method
input ENUM_APPLIED_PRICE maAppliedPrice2         = PRICE_CLOSE;                              // Applied Price
input string             Icci                    = "===== CCi Setup =====";                  // ===== CCi Setup =====
input int                cciPeriods              = 50;                                       // Periods
input ENUM_APPLIED_PRICE cciAppliedPrice         = PRICE_CLOSE;                              // Applied Price
input string             IATR                    = "===== ATR Setup =====";                  // ===== ATR Setup =====
input int                atrPeriods              = 14;                                       // Periods
input string             T0                      = "===== Trade Setup =====";                // ===== Trade Setup =====
input int                uCountBuys              = 1;                                        // Buys At same time
input int                uCountSells             = 1;                                        // Sells At same time
...
input double             atrX                    = 1;                                        // ATR's X SL:
...
input TSLMode            userTslMode             = byATR;                                   // TSL Mode:
...
input ENUM_TIMEFRAMES    uTslATRTf               = PERIOD_CURRENT;                           // TSL ATR time Frame:
input int                uTslATRPeriod           = 14;                                       // TSL ATR Period:
int                      uTslATRShift            = 0;                                        // TSL ATR Shift:
input double             uATRmultiplier          = 1;                                        // ATR multiplier:
...
class BUYcondition1 : public iConditions
{
 public:
  bool evaluate()
  {
    ...
    return maFast.index(1) > maSlow.index(1) && cciBuySignal() == true && Bid > maSlow.index(1);
  }
};
...
class SELLcondition1 : public iConditions
{
 public:
  bool evaluate()
  {
    ...
    return maFast.index(1) < maSlow.index(1) && cciSellSignal() == true && Bid < maSlow.index(1);
...
int OnInit()
{
  ...
  if (userTslMode == byPips) tsl = new TrailingStop(GetPointer(mainOrders), byPips);
  if (userTslMode == byATR) tsl = new TrailingStop(GetPointer(mainOrders), byATR);
  ...
  conditionsToBuy.AddCondition(countBuys = new ConditionCountBuys(uCountBuys));
  ...
  conditionsToSell.AddCondition(countSells = new ConditionCountSells(uCountSells));
  ...
  // Note: Indicators
  maFast = new MovingAverage(_Symbol, Period());
  maSlow = new MovingAverage(_Symbol, Period());
  maFast.setSetup(maPeriod1, maShift1, maMethod1, maAppliedPrice1);
  maSlow.setSetup(maPeriod2, maShift2, maMethod2, maAppliedPrice2);
  ...
  if (stopLossOn)
    switch (modeSL)
    {
      ...
      case PipsFromOpenCandle:
        atrPips = iATR(NULL, 0, atrPeriods, 1) / (_Point * 10) * atrX;
        Print("atrPips: ", atrPips, " ", 2739, "");
        levelSL = new Levels(new ByPipsFromCandle(_Symbol, side, atrPips, "SL", 0, 1));
...
bool cciBuySignal()
{
  return iCCI(NULL, 0, cciPeriods, cciAppliedPrice, 1) > 0 && iCCI(NULL, 0, cciPeriods, cciAppliedPrice, 2) < 0;
}
bool cciSellSignal()
{
  return iCCI(NULL, 0, cciPeriods, cciAppliedPrice, 1) < 0 && iCCI(NULL, 0, cciPeriods, cciAppliedPrice, 2) > 0;
