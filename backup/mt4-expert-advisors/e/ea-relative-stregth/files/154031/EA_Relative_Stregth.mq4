...
// CONFLUENCE 1 
// Activate? ( )YES NO
// Term? - entiendo que es el TF
// Direction? ( ) ANY ( ) GOING AGAINST
...
string tconfluence1 = "Confluence 1";
input bool Activate1 = true; // Activate:
input ENUM_TIMEFRAMES uTFconfluence1 = PERIOD_CURRENT; // TF:
input DirectionIndicator uDirectionConfluence1 = Any; // Direction:
string tconfluence2 = "Confluence 2";
input bool Activate2 = true; // Activate:
input ENUM_TIMEFRAMES uTFconfluence2 = PERIOD_CURRENT; // TF:
input DirectionIndicator uDirectionConfluence2 = Any; // Direction:
string tconfluence3 = "Confluence 3";
input bool Activate3 = true; // Activate:
input ENUM_TIMEFRAMES uTFconfluence3 = PERIOD_CURRENT; // TF:
input DirectionIndicator uDirectionConfluence3 = Any; // Direction:
string tconfluence4 = "Confluence 4";
input bool Activate4 = true; // Activate:
input ENUM_TIMEFRAMES uTFconfluence4 = PERIOD_CURRENT; // TF:
input DirectionIndicator uDirectionConfluence4 = Any; // Direction:

class Confluence
{
    bool Activate;
    ENUM_TIMEFRAMES TFconfluence;
    DirectionIndicator uDirectionConfluence;


    public:
    Confluence(bool activate, ENUM_TIMEFRAMES tfc, DirectionIndicator direction)
    {
        Activate = activate;
        TFconfluence = tfc;
        uDirectionConfluence = direction;
    }
    ~Confluence() { ; }
    ...
    bool check()
    {
        ...
        double aUp = iCustom(NULL, 0, "CCFp mtf 2.10- Simplied by R", TFconfluence, 8, 1);
        double aDn = iCustom(NULL, 0, "CCFp mtf 2.10- Simplied by R", TFconfluence, 9, 1);

        if(uDirectionConfluence == Any)
        {
            return aUp > aDn && aUp != EMPTY_VALUE && aDn != EMPTY_VALUE;
        }
        else
        {
            return aUp < aDn && aUp != EMPTY_VALUE && aDn != EMPTY_VALUE;
...
Confluence* confluences[4];

string file_custom_indicator = "CCFp mtf 2.10- Simplied by R";
int bufferToSell = 9;
int bufferToBuy = 8;

input string texit = "== Exit Rule ==";  // ————————————
input bool         closeAllInOpositeSignal = true;                       // Close in opposite signal
...
class BUYcondition1 : public iConditions
{
    ...
    bool evaluate()
    {
        ...
        // if(uDirectionConfluence1 == Any)
        for(int i = 0; i < ArraySize(confluences); i++) {
              if(!confluences[i].check())return false;
          }
          return true;

          // return Indi(bufferToBuy, 1) > Indi(bufferToSell, 1) && Indi(bufferToBuy, 1) != EMPTY_VALUE && Indi(bufferToSell, 1) != EMPTY_VALUE;
        //   return false;
    }
};
BUYcondition1* buyCondition1;

class ExitBUYcondition : public iConditions
{
    public:
    bool evaluate()
    {
...
class ExitSELLcondition : public iConditions
{
    public:
    bool evaluate()
    {
...
class ConditionToCloseBuy : public iConditions
{
    ...
    bool evaluate()
    {
        if(closeAllInOpositeSignal)
            if(exitBuy.evaluate()) return true;
...
class ConditionToCloseSell : public iConditions
{
    ...
    {
        if(closeAllInOpositeSignal)
            if(exitSell.evaluate())return true;
...
int OnInit()
{
    ...
    confluences[0] = new Confluence(Activate1, uTFconfluence1, uDirectionConfluence1);
    confluences[1] = new Confluence(Activate2, uTFconfluence2, uDirectionConfluence2);
    confluences[2] = new Confluence(Activate3, uTFconfluence3, uDirectionConfluence3);
    confluences[3] = new Confluence(Activate4, uTFconfluence4, uDirectionConfluence4);
    ...
    exitBuy = new ExitBUYcondition();
    exitSell = new ExitSELLcondition();
