...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=74244
...
input int                Stoch_K_Periods = 3;                 // K-Periods
input int                Stoch_D_Periods = 3;                 // D-Periods
input int                Stoch_Slowing    = 15;                // Slowing
input ENUM_MA_METHOD     Stoch_MA_Method  = MODE_SMA;          // Mode MA
input ENUM_STO_PRICE     Stoch_PriceField = STO_CLOSECLOSE;    // Stoch Price
input double             Stoch_LevelUp    = 75;                // Level Over Bougth
input double             Stoch_LevelDn    = 25;                // Level Over Sold
...
class Stoch
{
    ...
    Stoch(
        ...
        ENUM_STO_PRICE     PriceField = STO_CLOSECLOSE,
        double             LevelUp = 75,
        double             LevelDn = 25)
    {
        _handle = iStochastic(Symbol, TimeFrame, K_Periods, D_Periods, Slowing, MA_Method, PriceField);
        _levelUp = LevelUp;
        _levelDn = LevelDn;
    }
    ~Stoch() {;}
    ...
    double K_Line(int shift) { return calculate(1, shift); }
    double D_Line(int shift) { return calculate(0, shift); }

    bool iskOverB(int shift) { return (calculate(0, shift) >= _levelUp); }
    bool iskOverS(int shift) { return (calculate(0, shift) <= _levelDn); }

    double CrossLevelUp(int shift) { return (K_Line(shift) > _levelUp && K_Line(shift + 1) <= _levelUp); }
    double CrossLevelDn(int shift) { return (K_Line(shift) < _levelDn && K_Line(shift + 1) >= _levelDn); }
    bool isRising(int shift) { return (K_Line(shift) > K_Line(shift + 1)); }
    bool isFalling(int shift) { return (K_Line(shift) < K_Line(shift + 1)); }
...
input int                maFast_Period = 144;                                   // Period
...
class BUYcondition1 : public iConditions
{
    ...
    bool evaluate()
    {
        ...
        return iClose(NULL, 0, 1) > emaFast.index(1);
    }
};
...
class BUYcondition2 : public iConditions
{
    ...
    {
        // TODO: condition Buy2
        // return stoch.D_Line(1) > stoch.K_Line(1) && stoch.D_Line(2) <= stoch.K_Line(2) && stoch.iskOverS(1);
        return stoch.D_Line(1) > stoch.K_Line(1) && stoch.iskOverS(1);
    }
};
...
class SELLcondition2 : public iConditions
{
    ...
    bool evaluate()
    {
        ...
        // return stoch.D_Line(1) < stoch.K_Line(1) && stoch.D_Line(2) >= stoch.K_Line(2) && stoch.iskOverB(1);
        return stoch.D_Line(1) < stoch.K_Line(1) && stoch.iskOverB(1);
    }
};
...
stoch = new Stoch(_Symbol, NULL, Stoch_K_Periods, Stoch_D_Periods, Stoch_Slowing, Stoch_MA_Method, Stoch_PriceField, Stoch_LevelUp, Stoch_LevelDn);
