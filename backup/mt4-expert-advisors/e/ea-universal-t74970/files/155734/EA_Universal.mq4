...
class ConditionCloseOnOppositeBuy : public iConditions {
  ...
  public:
    bool evaluate() {
        double indi;
        for (int i = 1; i < maxCandlesback; i++) {
            indi = Indi(bufferToSell, 1);
...
//---
input int bufferToBuy    = 0; // Buffer To Buy Signals
input int bufferToSell   = 1; // Buffer To Sell Signals
input int maxCandlesback = 5; // Allowed Signal Candles back:

bool indicator_init = true;

double Indi(int buffer, int candle = 1) {
    return iCustom(NULL, 0, indicator_file, buffer, candle);
}
