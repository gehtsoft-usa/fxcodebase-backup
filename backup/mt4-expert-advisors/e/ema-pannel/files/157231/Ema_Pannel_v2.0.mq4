...
{
    bool res = true;
        res = gui.Create(0, "Moving Average Pannel", 0, 400, 0, 650, 110);
...
class GUI : public CAppDialog
{
    ...
    CEdit   edit1, edit2;
    CComboBox cbFastType, cbSlowType;
    ...
    bool Create(const long chart, const string name, const int subwin, const int x1, const int y1, const int x2, const int y2)
    {
        ...
        string ema_modes[4] = {"MODE SMA", "MODE EMA", "MODE SMMA", "MODE LWMA"};
        if(!Create_ComboBox("cbFast", Col(4), Row(0), _high, _width, cbFastType, ema_modes)) return false;
        if(!Create_ComboBox("cbSlow", Col(4), Row(1), _high, _width, cbSlowType, ema_modes)) return false;
    ...
    bool Create_ComboBox(string name, const int x1, const int y1, const int high, const int width, CComboBox& cb, string &items[])
    {
        int x2 = x1 + width*3;
        int y2 = y1 + high;
                ...
                const int& spread [])
{
    ...
    if(m1 != gui.cbFastType.Value())Recalculate();
    if(m2 != gui.cbSlowType.Value())Recalculate();
    ...
    for(i = start; i >= 0; i--)
    {
        LineFast[i] = valueLineFast(i);
        LineSlow[i] = valueLineSlow(i);
...
{
    ENUM_MA_METHOD method = gui.cbFastType.Value();
    return iMA(NULL, 0, fastPer, 0, method, PRICE_CLOSE, i);
}

double valueLineSlow(int i)
{
    ENUM_MA_METHOD method = gui.cbSlowType.Value();
    return iMA(NULL, 0, slowPer, 0, method, PRICE_CLOSE, i);    
}
