...
input int TMA_Period=56;
input int ATR_Period=100;
input double ATR_Mult=2;
input double TrendThreshold=0.5;
input double second_entry = 15; // Another entry if floating, pips
input double third_entry = 30; // Another entry if floating #2, pips
...
input TradingMode entry_logic = TradingModeLive; // Entry logic
   ...
   input bool exit_by_arrow = false; // Exit by arrow?
#endif
...
// Automatically saves data about breakeven levels as globals.
// You need to call RestoreActions() after creation of this object to restore tracking of old trades.
class OpenAnotherOrderAction : public AOrderAction
{
   ActionOnConditionLogic* _actions;
public:
   OpenAnotherOrderAction(ActionOnConditionLogic* actions)
   {
      _actions = actions;
   }

   ~OpenAnotherOrderAction()
   {
   }
   ...
   void RestoreActions(string symbol, int magicNumber)
   {
      ...
      while (trades.Next())
      {
         int ticketId = trades.GetTicket();
         string ticketIdStr = IntegerToString(ticketId);
         double trigger = GlobalVariableGet("oa_" + ticketIdStr + "_tr");
         if (trigger == 0)
         {
            continue;
         }
         CreateAnother(ticketId, trigger);
      }
   }
   ...
   virtual bool DoAction(const int period, const datetime date)
   {
      ...
      CreateAnother(_currentTicket, second_entry);
      CreateAnother(_currentTicket, third_entry);
      return true;
   }
private:
   void CreateAnother(const int ticketId, const double val)
   {
      if (!OrderSelect(ticketId, SELECT_BY_TICKET, MODE_TRADES) || OrderComment() == "Secondary")
      {
         return;
      }
      string ticketIdStr = IntegerToString(ticketId);
      GlobalVariableSet("oa_" + ticketIdStr + "_tr", val);
      IOrder *order = new OrderByTicketId(_currentTicket);
      HitProfitCondition* condition = new HitProfitCondition();
      condition.Set(order, val);
      order.Release();

      IAction* action = CreateAction(TradingCalculator::Create(OrderSymbol()), OrderSymbol(),
         (ENUM_TIMEFRAMES)_Period, TradingCalculator::IsBuyOrder() ? BuySide : SellSide, 
         orderHandlers, true, _actions, NULL, true);
      if (!_actions.AddActionOnCondition(action, condition))
      {
      }
      condition.Release();
...
class LongCondition : public ACondition
{
   int _shift;
public:
   LongCondition(const string symbol, ENUM_TIMEFRAMES timeframe, int shift)
      :ACondition(symbol, timeframe)
   {
      _shift = shift;
   }
   ...
   bool IsPass(const int period, const datetime date)
   {
      ...
      int mult = digits == 3 || digits == 5 ? 10 : 1;
      double pipSize = point * mult;
      double value = iCustom(_symbol, _timeframe, "extreme tma line", TMA_Period, ATR_Period, ATR_Mult, TrendThreshold, 4, period) - _shift * pipSize;
      return iClose(_symbol, _timeframe, period) <= value;
   }
};
...
class ShortCondition : public ACondition
{
   int _shift;
public:
   ShortCondition(const string symbol, ENUM_TIMEFRAMES timeframe, int shift)
      :ACondition(symbol, timeframe)
   {
      _shift = shift;
   }
   ...
   bool IsPass(const int period, const datetime date)
   {
      ...
      int mult = digits == 3 || digits == 5 ? 10 : 1;
      double pipSize = point * mult;
      double value = iCustom(_symbol, _timeframe, "extreme tma line", TMA_Period, ATR_Period, ATR_Mult, TrendThreshold, 3, period) + _shift * pipSize;
      return iClose(_symbol, _timeframe, period) >= value;
   }
};

class LongFilterCondition : public ACondition
{
public:
   LongFilterCondition(const string symbol, ENUM_TIMEFRAMES timeframe)
      :ACondition(symbol, timeframe)
   {
   ...
   bool IsPass(const int period, const datetime date)
   {
      double value = iCustom(_symbol, _timeframe, "ema line", 0, period);
      return value != 0 && value != EMPTY_VALUE;
   }
};

class ShortFilterCondition : public ACondition
{
public:
   ShortFilterCondition(const string symbol, ENUM_TIMEFRAMES timeframe)
      :ACondition(symbol, timeframe)
   {
   ...
   bool IsPass(const int period, const datetime date)
   {
      double value = iCustom(_symbol, _timeframe, "ema line", 1, period);
      return value != 0 && value != EMPTY_VALUE;
   }
};
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      double value = iCustom(_symbol, _timeframe, "extreme tma line", TMA_Period, ATR_Period, ATR_Mult, TrendThreshold, 3, period);
      return iClose(_symbol, _timeframe, period) >= value;
   }
};
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      double value = iCustom(_symbol, _timeframe, "extreme tma line", TMA_Period, ATR_Period, ATR_Mult, TrendThreshold, 4, period);
      return iClose(_symbol, _timeframe, period) <= value;
   }
};

class ExitLongCondition2 : public ACondition
{
public:
   ExitLongCondition2(const string symbol, ENUM_TIMEFRAMES timeframe)
      :ACondition(symbol, timeframe)
   {
   ...
   bool IsPass(const int period, const datetime date)
   {
      double value = iCustom(_symbol, _timeframe, "ema line", 4, period);
      return value != EMPTY_VALUE;
   }
};

class ExitShortCondition2 : public ACondition
{
public:
   ExitShortCondition2(const string symbol, ENUM_TIMEFRAMES timeframe)
      :ACondition(symbol, timeframe)
   {
   ...
   bool IsPass(const int period, const datetime date)
   {
      double value = iCustom(_symbol, _timeframe, "ema line", 3, period);
      return value != EMPTY_VALUE;
   }
};
...
ICondition* CreateLongCondition(string symbol, ENUM_TIMEFRAMES timeframe)
{
   ...
   or.Add(new LongCondition(symbol, timeframe, 0), false);
...
ICondition* CreateLongFilterCondition(string symbol, ENUM_TIMEFRAMES timeframe)
{
   ...
   AndCondition* condition = new AndCondition();
   condition.Add(new LongFilterCondition(symbol, timeframe), false);
   return condition;
}
...
ICondition* CreateShortCondition(string symbol, ENUM_TIMEFRAMES timeframe)
{
   ...
   AndCondition* condition = new AndCondition();
   OrCondition* or = new OrCondition();
   or.Add(new ShortCondition(symbol, timeframe, 0), false);
   condition.Add(or, false);
   #ifdef ACT_ON_SWITCH_CONDITION
      ActOnSwitchCondition* switchCondition = new ActOnSwitchCondition(symbol, timeframe, (ICondition*) condition);
      condition.Release();
...
ICondition* CreateShortFilterCondition(string symbol, ENUM_TIMEFRAMES timeframe)
{
   ...
   AndCondition* condition = new AndCondition();
   condition.Add(new ShortFilterCondition(symbol, timeframe), false);
   return condition;
}
...
ICondition* CreateExitLongCondition(string symbol, ENUM_TIMEFRAMES timeframe)
{
   ...
   if (exit_by_arrow)
   {
      condition.Add(new ExitLongCondition2(symbol, timeframe), false);
   }
   else
   {
...
ICondition* CreateExitShortCondition(string symbol, ENUM_TIMEFRAMES timeframe)
{
   ...
   if (exit_by_arrow)
   {
      condition.Add(new ExitShortCondition2(symbol, timeframe), false);
   }
   else
   {
...
IAction* CreateAction(TradingCalculator* tradingCalculator, string symbol,
   ...
   ActionOnConditionLogic* actions, IStream* price, bool singleAction)
{
   ...
   IAction* action = new EntryAction(entryStrategy, orderSide, moneyManagement, "Secondary", orderHandlers, singleAction);
...
TradingController *CreateController(const string symbol, const ENUM_TIMEFRAMES timeframe, string algoId, string &error)
{
   ...
   OpenAnotherOrderAction* orderAction = new OpenAnotherOrderAction(actions);
   orderAction.RestoreActions(_Symbol, magic_number);
   orderHandlers.AddOrderAction(orderAction);
   orderAction.Release();
   #ifdef TWO_LEVEL_TP
...
int OnInit()
{
   orderHandlers = new OrderHandlers();
      ...
      stats = NULL;
   #endif
   double temp = iCustom(NULL, 0, "extreme tma line", TMA_Period, ATR_Period, ATR_Mult, TrendThreshold, 4, 0);
   if (GetLastError() == ERR_INDICATOR_CANNOT_LOAD)
   {
      Alert("Please, install the 'extreme tma line' indicator");
      return INIT_FAILED;
   }
   if (exit_by_arrow)
   {
      temp = iCustom(NULL, 0, "ema line", 0, 0);
      if (GetLastError() == ERR_INDICATOR_CANNOT_LOAD)
      {
         Alert("Please, install the 'ema line' indicator");
         return INIT_FAILED;
      }
   }
