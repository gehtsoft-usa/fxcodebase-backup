...
int init()
{
   ...
   double temp_1 = iCustom(NULL, 0, "EWO", Fast_MA_Length, Slow_MA_Length, Price, Smoothing_Method, 0, 1);
   if (GetLastError() == ERR_INDICATOR_CANNOT_LOAD)
   {
      Alert("Please, install the 'EWO' indicator");
      return INIT_FAILED;
   } 
   double diff = (temp - temp_1)/Point;
   IndicatorObjPrefix = GenerateIndicatorPrefix("ewohm");
   IndicatorShortName("EWO Heat Map "+"Divergency = "+DoubleToStr(diff,2));
