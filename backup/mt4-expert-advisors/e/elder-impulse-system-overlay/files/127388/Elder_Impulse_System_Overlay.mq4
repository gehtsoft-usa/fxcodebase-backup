...
// http://fxcodebase.com/code/viewtopic.php?f=38&t=64430
...
input ENUM_MA_METHOD macd_avg = MODE_SMA; // Smoothing method
...
int start(){   
   ...
   for(i = limit; i>=0; i--)
   {
      ...
      MACD_src[i] = iMA(NULL, 0, MACD_Fast_EMA, 0, macd_avg, PRICE_CLOSE, i) - iMA(NULL, 0, MACD_Slow_EMA, 0, macd_avg, PRICE_CLOSE, i);
      double Signal = iMAOnArray(MACD_src, 0, MACD_Signal_EMA, 0, macd_avg, i);
      if (MACD_Signal_Difference)
         MACD[i] = MACD_src[i] - Signal;
      else
         MACD[i] = MACD_src[i];
      if (i == limit)
         continue;
