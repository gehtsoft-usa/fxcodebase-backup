...
// http://fxcodebase.com/code/viewtopic.php?f=38&t=70115
// More information about this indicator can be found at:
//http://fxcodebase.com/code/viewtopic.php?f=38&t=70115
...
input int ma_period = 89; // MA Period
...
input bool show_historical_values = true; // Show historical values
...
public:
   ...
   {
      double ma0 = iMA(_symbol, _timeframe, ma_period, 0, ma_method, PRICE_CLOSE, period);
      double ma1 = iMA(_symbol, _timeframe, ma_period, 0, ma_method, PRICE_CLOSE, period + 1);
      return ma0 < iClose(_symbol, _timeframe, period)
         && ma1 > iLow(_symbol, _timeframe, period + 1);
   }
};
...
public:
   ...
   virtual bool IsPass(const int period, const datetime date)
   {
      ...
      double ma1 = iMA(_symbol, _timeframe, ma_period, 0, ma_method, PRICE_CLOSE, period + 1);
      return ma0 > iClose(_symbol, _timeframe, period)
         && ma1 < iHigh(_symbol, _timeframe, period + 1);
   }
};
...
public:
   ...
   virtual ICell* Create(const string id, const int x, const int y, const string symbol, const ENUM_TIMEFRAMES timeframe)
   {
      ...
      if (show_historical_values)
      {
         historyUpValue = new FixedTextFormatter("Buy", _historicalUpColor);
      }
      ...
      if (show_historical_values)
      {
         historyDownValue = new FixedTextFormatter("Sell", _historicalDownColor);
      }
...
string IndicatorName = "EMA Bounce Dashboard";
int init()
{
   ...
   IndicatorObjPrefix = GenerateIndicatorPrefix("ebd");
