...
#define MARTINGALE_IN_OPEN_TRADES
// #define MOVING_AVERAGE_ON
// #define RSI_ON
// #define ADX_ON
// #define SPREAD_FILTER
// ------------------------------------------------------------------
input int magico = 4477;  // Magic Number:
...
string file_custom_indicator = "BB3";
int    handle                = 0;

void setHandle()
{
  handle = iCustom(Symbol(), NULL, file_custom_indicator,
                   uPeriod, uShift, uDeviation1, uDeviation2, uDeviation3);
}
...
// clang-format off
double MIBand(int s) { return custom_indicator(0, s); }
double H1Band(int s) { return custom_indicator(1, s); }
double H2Band(int s) { return custom_indicator(2, s); }
double H3Band(int s) { return custom_indicator(3, s); }
double L1Band(int s) { return custom_indicator(4, s); }
double L2Band(int s) { return custom_indicator(5, s); }
double L3Band(int s) { return custom_indicator(6, s); }
// clang-format on
...
input string tMatingale                = "== Martingale ==";         // == Martingale Setup ==
...
input string tTailingStop              = "== TailingStop ==";        // ————————————
...
double       closeByPipsWin            = 0;                          // Close Pips Win:
double       closeByPipsLoss           = 0;                          // Close Pips Loss:


#ifdef MARTINGALE_IN_OPEN_TRADES
...
class MartingaleOpenTrades
{
 ...
 public:
  ...
  {
    double next = NormalizeDouble(_lot * MathPow(_multiplier, openTrades(side)), 2);
...
class OneTrueConditions
{
 protected:
  iConditions* _conditions[];

 public:
  OneTrueConditions(void) {}
  ~OneTrueConditions(void) { releaseConditions(); }
...
OneTrueConditions conditionsToBuy;
OneTrueConditions conditionsToSell;
...
class BUYcondition1 : public iConditions
{
 public:
  bool evaluate()
  {
    ...
    // if (CountSells() >= 1) return false;

    if (CountBuys() == 0)
      return iOpen(NULL, PERIOD_M1, 0) > L1Band(0) && Bid() < L1Band(0) && Ask() >= L1Band(0);
...
class BUYcondition2 : public iConditions
{
 public:
  bool evaluate()
  {
    ...
    // TODO: condition Buy2
    if (CountBuys() == 1)
      return iOpen(NULL, PERIOD_M1, 0) > L2Band(0) && Bid() < L2Band(0) && Ask() >= L2Band(0);
...
class BUYcondition3 : public iConditions
{
 public:
  bool evaluate()
  {
    ...
    if (CountBuys() == 2)
      return iOpen(NULL, PERIOD_M1, 0) > L3Band(0) && Bid() < L2Band(0) && Ask() >= L2Band(0);
...
class SELLcondition1 : public iConditions
{
 public:
  bool evaluate()
  {
    ...
    // if (CountBuys() >= 1) return false;

    if (CountSells() == 0)
      return iOpen(NULL, PERIOD_M1, 0) < H1Band(0) && Ask() >= H1Band(0);
...
class SELLcondition2 : public iConditions
{
 public:
  bool evaluate()
  {
    ...
    if (CountSells() == 1)
      return iOpen(NULL, PERIOD_M1, 0) < H2Band(0) && Ask() >= H2Band(0);
...
class SELLcondition3 : public iConditions
{
 public:
  bool evaluate()
  {
    ...
    if (CountSells() == 2)
      //   return iOpen(NULL, 0, 1) < H2Band(0) && Ask() >= H2Band(0);
      return iOpen(NULL, PERIOD_M1, 0) < H3Band(0) && Ask() >= H3Band(0);
...
double Lots(string side="all")
{
    ...
    if(MartingaleON)
    {
        double mlot = martingale.Lot(side);
        if(mlot!=0) lots = mlot;
    }
