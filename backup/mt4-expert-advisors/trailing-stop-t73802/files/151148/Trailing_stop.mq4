//Available @ https://fxcodebase.com/code/viewtopic.php?f=38&t=73802
...
input double offset = 0.2; // Offset %
input int offbars = 0; // Offset Bars
input int sumbars = 6; // Number of bars for Average
enum PriceType
{
...
input PriceType srcts = PriceClose; // Source for Trailing Stop
input PriceType srctrigger = PriceClose; // Source for Stop-Trigger
...
SimplePriceStream* srctsInputstream;
SimplePriceStream* srctriggerInputstream;
...
int init()
{
   IndicatorObjPrefix = GenerateIndicatorPrefix("Trailing stop");
   IndicatorShortName("Trailing stop");
   IndicatorBuffers(3);
   int id = 0;
   ...
   srctsInputstream = new SimplePriceStream(_Symbol, (ENUM_TIMEFRAMES)_Period, srcts);
   srctriggerInputstream = new SimplePriceStream(_Symbol, (ENUM_TIMEFRAMES)_Period, srctrigger);
   sum1Source = new CustomStream(_Symbol, (ENUM_TIMEFRAMES)_Period);
   sum1 = new SumOnStream(sum1Source, sumbars);
...
int deinit()
{
   ...
   srctsInputstream.Release();
   srctriggerInputstream.Release();
   sum1Source.Release();
   sum1.Release();
                ...
                const int &spread[])
{
   if (prev_calculated <= 0 || prev_calculated > rates_total)
   {
      ...
      ArrayInitialize(tsl, EMPTY_VALUE);
   }
   ...
   {
      double srctsInputstreamValue;
      if (!srctsInputstream.GetValue(pos, srctsInputstreamValue))
      {
         continue;
      }
      sum1Source.SetValue(pos, srctsInputstreamValue);
      double sum1Value;
      if (!sum1.GetValue(pos, sum1Value))
      {
         continue;
      }
      tsl[pos] = (sum1Value / sumbars) * (1 - (offset / 100));
      double srctriggerInputstreamValue;
      if (!srctriggerInputstream.GetValue(pos, srctriggerInputstreamValue))
      {
         continue;
      }
      crossunder1X.SetValue(pos, srctriggerInputstreamValue);
      crossunder1Y.SetValue(pos, tsl[pos + offbars]);
      double crossunder1Value;
      if (!crossunder1.GetValue(pos, crossunder1Value))
      {
         continue;
      }
      bool signal = crossunder1Value;
      double out = tsl[pos + offbars];
      plot1[pos] = out;
      if (signal)
         plot2[pos] = high[pos];
   }
