//Available @  https://fxcodebase.com/code/viewtopic.php?f=38&t=76282
...
#endif
input bool param1 = true; // 
input int param2 = 5; // Fractals Length
input int param3 = 3; // Support Fractals Qty
input color param4 = Lime; // 
input int param5 = 3; // Resistance Fracrtal Qty
input color param6 = Orange; // 
input string param7 = "average"; // Type
...
uint colorRes;
int typeZone;
ISimpleTypeArray<double>* FracrtalUp;
ISimpleTypeArray<double>* FracrtalLow;
FloatStream* highest1Source;
FloatStream* lowest1Source;
class drawFractalLevel_faS_i_bStream
{
   TIStream<ISimpleTypeArray<double>*>* Fracrtal;
   int countAmount;
   int side;
   ATRStream* atr1;
   double startIndex[];
   double startIndex_DEFAULT_VALUE;
   Line* level;
   Box* zone;
   bool _initialized;
   string IndicatorObjPrefix;
public:
   drawFractalLevel_faS_i_bStream(TIStream<ISimpleTypeArray<double>*>* Fracrtal, int countAmount, int side, string indicatorObjPrefix)
   {
      _initialized = false;
      IndicatorObjPrefix = indicatorObjPrefix;
      this.Fracrtal = Fracrtal;
      Fracrtal.AddRef();
      this.countAmount = countAmount;
      this.side = side;
      atr1 = new ATRStream(200);
   }
   ~drawFractalLevel_faS_i_bStream()
   {
      Fracrtal.Release();
      atr1.Release();
   }
   ...
   bool GetValue(const int pos)
   {
      if (!_initialized)
      {
         startIndex_DEFAULT_VALUE = ((iBars(_Symbol, (ENUM_TIMEFRAMES)_Period) - 1) - pos);
         ArrayInitialize(startIndex, startIndex_DEFAULT_VALUE);
         level = (Line*)(NULL);
         zone = (Box*)(NULL);
         _initialized = true;
      }
      startIndex[pos] = pos < (iBars(_Symbol, (ENUM_TIMEFRAMES)_Period) - 1) ? startIndex[pos + 1] : ((iBars(_Symbol, (ENUM_TIMEFRAMES)_Period) - 1) - pos);
      double atr1Value;
      if (!atr1.GetValue(pos, atr1Value)) { atr1Value = EMPTY_VALUE; }
      double atr = atr1Value;
      uint _color = (side ? colorSup : colorRes);
      int count = 0;
      ISimpleTypeArray<double>* FracrtalValue;
      if (!Fracrtal.GetValue(pos, FracrtalValue)) { FracrtalValue = NULL; }
      if (SafeGreater(Array::Size<int, ISimpleTypeArray<double>*>(FracrtalValue, INT_MIN), countAmount))
      {
         Array::Shift<double, ISimpleTypeArray<double>*>(FracrtalValue, EMPTY_VALUE);
      }
      if (SafeGE(Array::Size<int, ISimpleTypeArray<double>*>(FracrtalValue, INT_MIN), countAmount) && (((iBars(_Symbol, (ENUM_TIMEFRAMES)_Period) - 1) - pos) - startIndex[pos] > 100))
      {
         double lastF = Array::Last<double, ISimpleTypeArray<double>*>(FracrtalValue, EMPTY_VALUE);
         double upper = SafePlus(lastF, atr);
         double lower = SafeMinus(lastF, atr);
         int for1_from = 0;
         int for1_to = (-countAmount);
         ...
         {
            if (SafeGreater(Array::Get<double, ISimpleTypeArray<double>*, int>(FracrtalValue, i, EMPTY_VALUE), lower) && SafeLess(Array::Get<double, ISimpleTypeArray<double>*, int>(FracrtalValue, i, EMPTY_VALUE), upper))
            {
               count = count + 1;
            }
         }
         ...
         {
            double fractalLevel = (typeZone ? Array::Avg(FracrtalValue) : ((side ? Array::Min(FracrtalValue, 0) : Array::Max(FracrtalValue, 0))));
            level = LinesCollection::Create(IndicatorObjPrefix + "line_1_id", ((iBars(_Symbol, (ENUM_TIMEFRAMES)_Period) - 1) - pos) - 50, fractalLevel, ((iBars(_Symbol, (ENUM_TIMEFRAMES)_Period) - 1) - pos) + 20, fractalLevel, iTime(_Symbol, (ENUM_TIMEFRAMES)_Period, pos)).SetColor(_color).SetWidth(1).SetStyle("solid").SetExtend("none");
            zone = BoxesCollection::Create(IndicatorObjPrefix + "box_1_id", ((iBars(_Symbol, (ENUM_TIMEFRAMES)_Period) - 1) - pos) - 50, fractalLevel, ((iBars(_Symbol, (ENUM_TIMEFRAMES)_Period) - 1) - pos) + 10, SafeMinus(fractalLevel, ((side ? atr : InvertSign(atr)))), iTime(_Symbol, (ENUM_TIMEFRAMES)_Period, pos))
               .SetBgColor(AddTransparency(_color, 80))
               .SetBorderColor((uint)(INT_MAX))
               .SetExtend("none")
               .SetText(Str::ToString(countAmount))
               .SetTextColor(ChartGetInteger(0, CHART_COLOR_FOREGROUND))
               .SetTextHAlign("center")
               .SetTextVAlign("center")
               .SetTextSize("auto");
            SetStream(startIndex, pos, ((iBars(_Symbol, (ENUM_TIMEFRAMES)_Period) - 1) - pos), startIndex_DEFAULT_VALUE);
         }
      }
      Line::SetX2(level, ((iBars(_Symbol, (ENUM_TIMEFRAMES)_Period) - 1) - pos) + 20);
      Box::SetRight(zone, ((iBars(_Symbol, (ENUM_TIMEFRAMES)_Period) - 1) - pos) + 10);
      return true;
   }
};
FloatArrayStream* drawFractalLevel_faS_i_b1_param1;
drawFractalLevel_faS_i_bStream* drawFractalLevel_faS_i_b1;
FloatArrayStream* drawFractalLevel_faS_i_b2_param1;
drawFractalLevel_faS_i_bStream* drawFractalLevel_faS_i_b2;
double plot1[];
double plot2[];
...
int init()
{
   ...
   resCount = param5;
   colorRes = param6;
   int LeftRange = fractalLen;
   int RightRange = fractalLen;
   ...
   SetIndexStyle(id++, DRAW_ARROW, STYLE_SOLID, 1, GetColorOnly(AddTransparency(colorRes, 50)));
   SetIndexBuffer(id, plot2);
   SetIndexShift(id, (-5));
   SetIndexArrow(id, 217);
   SetIndexStyle(id++, DRAW_ARROW, STYLE_SOLID, 1, GetColorOnly(AddTransparency(colorSup, 50)));
   LinesCollection::SetMaxLines(500);
   BoxesCollection::SetMaxBoxes(50);
   IndicatorObjPrefix = GenerateIndicatorPrefix("");
   IndicatorShortName("Fractal Support and Resistance [BigBeluga]");
   drawFractalLevel_faS_i_b1_param1 = new FloatArrayStream(_Symbol, (ENUM_TIMEFRAMES)_Period, NULL);
   drawFractalLevel_faS_i_b1 = new drawFractalLevel_faS_i_bStream(drawFractalLevel_faS_i_b1_param1, resCount, false, IndicatorObjPrefix + "_1");
   id = drawFractalLevel_faS_i_b1.Init(id);
   drawFractalLevel_faS_i_b2_param1 = new FloatArrayStream(_Symbol, (ENUM_TIMEFRAMES)_Period, NULL);
   drawFractalLevel_faS_i_b2 = new drawFractalLevel_faS_i_bStream(drawFractalLevel_faS_i_b2_param1, supCount, true, IndicatorObjPrefix + "_2");
   id = drawFractalLevel_faS_i_b2.Init(id);
...
int deinit()
{
   ...
   if (FracrtalUp != NULL) FracrtalUp.Release();
   if (FracrtalLow != NULL) FracrtalLow.Release();
   highest1Source.Release();
   lowest1Source.Release();
   drawFractalLevel_faS_i_b1_param1.Release();
   delete drawFractalLevel_faS_i_b1;
   drawFractalLevel_faS_i_b2_param1.Release();
   delete drawFractalLevel_faS_i_b2;
                ...
                const int &spread[])
{
   if (prev_calculated <= 0 || prev_calculated > rates_total)
   {
      ...
      ISimpleTypeArray<double>* __array1 = new FloatArray(100, 0);
      if (FracrtalUp != NULL) FracrtalUp.Release();
      FracrtalUp = __array1;
      FracrtalUp.AddRef();
      ISimpleTypeArray<double>* __array2 = new FloatArray(100, 0);
      if (FracrtalLow != NULL) FracrtalLow.Release();
      FracrtalLow = __array2;
      FracrtalLow.AddRef();
      highest1Source.Init();
      lowest1Source.Init();
      drawFractalLevel_faS_i_b1_param1.Init();
      drawFractalLevel_faS_i_b1.Clear();
      drawFractalLevel_faS_i_b2_param1.Init();
      drawFractalLevel_faS_i_b2.Clear();
   ...
   {
      typeZone = (param7 == "average");
      int RightRange = fractalLen;
      int LeftRange = fractalLen;
      if (pos + RightRange > (rates_total - 1)) { continue; }
      highest1Source.SetValue(pos, high[pos]);
      double highest1Value;
      if (!HighestHighStream::GetValue(pos, highest1Value, highest1Source, LeftRange + RightRange + 1)) { highest1Value = EMPTY_VALUE; }
      int upperF = SafeGE(high[pos + RightRange], highest1Value);
      if (pos + RightRange > (rates_total - 1)) { continue; }
      lowest1Source.SetValue(pos, low[pos]);
      double lowest1Value;
      if (!LowestLowStream::GetValue(pos, lowest1Value, lowest1Source, LeftRange + RightRange + 1)) { lowest1Value = EMPTY_VALUE; }
      int lowerF = SafeLE(low[pos + RightRange], lowest1Value);
      if (upperF)
      {
         if (pos + RightRange > (rates_total - 1)) { continue; }
         Array::Push<ISimpleTypeArray<double>*, double>(FracrtalUp, high[pos + RightRange]);
      }
      if (lowerF)
      {
         if (pos + RightRange > (rates_total - 1)) { continue; }
         Array::Push<ISimpleTypeArray<double>*, double>(FracrtalLow, low[pos + RightRange]);
      }
      drawFractalLevel_faS_i_b1_param1.SetValue(pos, FracrtalUp);
      if (!drawFractalLevel_faS_i_b1.GetValue(pos)) { }
      drawFractalLevel_faS_i_b2_param1.SetValue(pos, FracrtalLow);
      if (!drawFractalLevel_faS_i_b2.GetValue(pos)) { }
      PlotShape::SetBool(plot1, pos, "abovebar", upperF, high, low, 5, AddTransparency(colorRes, 50));
      PlotShape::SetBool(plot2, pos, "belowbar", lowerF, high, low, 5, AddTransparency(colorSup, 50));
   }
