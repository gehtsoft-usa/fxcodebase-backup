...
//https://fxcodebase.com/code/viewtopic.php?f=38&t=76501
...
input int TimeAfterNews = 240; // Minutes after news to breakout range
...
input bool      NEWS_IMPOTANCE_MEDIUM = false;                           // Medium
...
class News : public iConditions
{
  ...
  public:
    ...
    //+------------------------------------------------------------------+
    //| Devuelve max y min en un rango de tiempo antes de una noticia     |
    //+------------------------------------------------------------------+
    bool GetMaxMinBeforeNews(const sNews &l_news, int minutesBefore, int timeframe, double &outMaxPrice, datetime &outMaxTime, double &outMinPrice, datetime &outMinTime)
    {
        if (minutesBefore <= 0) return(false);
        datetime news_time = l_news.dTime + (datetime)TIME_CORRECTION; // usar misma corrección que otras funciones de noticias
        datetime t_start   = news_time - (datetime)(minutesBefore * 60);

        // buscar barras cerradas estrictamente anteriores a la noticia
        int startShift = iBarShift(_Symbol, timeframe, t_start, true);
        int endShift   = iBarShift(_Symbol, timeframe, news_time - 1, true); // última barra antes de la noticia

        // if (startShift == -1 || endShift == -1) return(false);
        // if (startShift < endShift) return(false); // no hay barras en el rango
        ...
        for (int s = startShift; s >= endShift; s--) {
            double h = iHigh(_Symbol, timeframe, s);
            double l = iLow(_Symbol, timeframe, s);
            datetime t = iTime(_Symbol, timeframe, s);
            if (h > outMaxPrice) { outMaxPrice = h; outMaxTime = t; }
            if (l < outMinPrice) { outMinPrice = l; outMinTime = t; }
        }

        if (outMaxTime == 0 || outMinTime == 0) return(false);
        return(true);
    }

    //+------------------------------------------------------------------+
    //| Dibuja un rectángulo en el chart que representa el rango dado     |
    //+------------------------------------------------------------------+
    bool DrawRangeRectangle(string name, datetime t_start, datetime t_end, double priceHigh, double priceLow, color clrRect = clrOrange, int style = STYLE_SOLID, int width = 1, bool sendBehind = false)
    {
        if (t_start <= 0 || t_end <= 0) return(false);
        if (priceHigh <= priceLow) { double tmp = priceHigh; priceHigh = priceLow; priceLow = tmp; }
        // normalizar orden de tiempos
        if (t_start > t_end) { datetime tmp = t_start; t_start = t_end; t_end = tmp; }

        if (StringLen(name) == 0) name = "NEWS_RANGE_" + TimeToString(t_start, TIME_DATE | TIME_MINUTES) + "_" + TimeToString(t_end, TIME_DATE | TIME_MINUTES);

        // eliminar si ya existe
        if (ObjectFind(0, name) >= 0) ObjectDelete(0, name);

        // crear rectángulo: parámetros (chart, name, OBJ_RECTANGLE, subwindow, time1, price1, time2, price2)
        if (!ObjectCreate(0, name, OBJ_RECTANGLE, 0, t_start, priceHigh, t_end, priceLow)) return(false);

        ObjectSetInteger(0, name, OBJPROP_COLOR, clrRect);
        ObjectSetInteger(0, name, OBJPROP_STYLE, style);
        ObjectSetInteger(0, name, OBJPROP_WIDTH, width);
        ObjectSetInteger(0, name, OBJPROP_BACK, sendBehind ? 1 : 0);
        ObjectSetInteger(0, name, OBJPROP_SELECTABLE, false);
        ObjectSetInteger(0, name, OBJPROP_SELECTED, false);
        ObjectSetInteger(0, name, OBJPROP_HIDDEN, true);
        ObjectSetInteger(0, name, OBJPROP_ZORDER, 0);
        ObjectSetString(0, name, OBJPROP_TEXT, "Active");

        return(true);
    }


    bool GetMaxMinBeforeNewsByIndex(int newsIndex, int minutesBefore, int timeframe, double &outMaxPrice, datetime &outMaxTime, double &outMinPrice, datetime &outMinTime)
    {
        if (newsIndex < 0 || newsIndex >= ArraySize(NEWS_TABLE)) return(false);
        return GetMaxMinBeforeNews(NEWS_TABLE[newsIndex], minutesBefore, timeframe, outMaxPrice, outMaxTime, outMinPrice, outMinTime);
    }

    
    void UpdateToNextNews()
    {
        if (ArraySize(news.NEWS_TABLE) <= 0) return;
        int index = -1; 

        datetime currentTime = TimeCurrent();
        int qntNews = ArraySize(news.NEWS_TABLE);
        datetime lastNewsTm = -1;
        for (int i = 0; i < qntNews; i++) {
            datetime newsTime = news.NEWS_TABLE[i].dTime;
            if(i-1 >=0) lastNewsTm = news.NEWS_TABLE[i-1].dTime;
        ...
        // Tomar Máximo y minimo del periodo anterior y dibujar rectángulo
        if(index >= 0)
        if((TimeCurrent() >= lastNewsTm + TimeAfterNews * 60) || (lastNewsTm == -1 )) {
            news.DeactivateRect("NewsRange");
            double maxPrice, minPrice;
            datetime maxTime, minTime;
            news.GetMaxMinBeforeNews(news.NEWS_TABLE[index], 1440, PERIOD_M15, maxPrice, maxTime, minPrice, minTime);
            // Print("Max Price before news: ", maxPrice, " at ", TimeToString(maxTime), "; Min Price before news: ", minPrice, " at ", TimeToString(minTime));
            news.DrawRangeRectangle("NewsRange", minTime, maxTime, maxPrice, minPrice, LightSteelBlue, STYLE_DASH, 2, true);
        }
    }
    ...
    //+------------------------------------------------------------------+
    //| Devuelve 1 si la última vela cerrada está por encima del rectángulo|
    //| llamado `rectName`, -1 si está por debajo, 0 si dentro/no existe.  |
    //+------------------------------------------------------------------+
    int LastClosedPositionRelativeToRect(const string rectName = "NewsRange")
    {
        if (ObjectGetString(0, rectName, OBJPROP_TEXT) != "Active") return(0);
        if (StringLen(rectName) == 0) return(0);
        if (ObjectFind(rectName) < 0) return(0);

        double p1 = ObjectGet(rectName, OBJPROP_PRICE1);
        double p2 = ObjectGet(rectName, OBJPROP_PRICE2);
        double top = MathMax(p1, p2);
        double bot = MathMin(p1, p2);

        double lastClose = iClose(_Symbol, PERIOD_CURRENT, 1);
        if (lastClose > top) { DeactivateRect(rectName); return(1);}
        if (lastClose < bot) { DeactivateRect(rectName); return(-1);}
        return(0);
    }
    
    void DeactivateRect(string name)
    {
        ObjectSetString(0, name, OBJPROP_TEXT, "Inactive");
        ObjectSetInteger(0, name, OBJPROP_COLOR, C'80,80,80');
    }
...
class BUYcondition1 : public iConditions
{
  public:
    bool evaluate()
    {
        ...
        int lastCandleTm = iTime(NULL, 0, 1);        
        if(lastCandleTm > CurrentNewsTime + TimeAfterNews * 60) {
            news.UpdateToNextNews();
        }
        
        int upOrDown = news.LastClosedPositionRelativeToRect("NewsRange");
