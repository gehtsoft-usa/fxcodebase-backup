...
// http://fxcodebase.com/code/viewtopic.php?f=38&t=68780
...
input bool Complet = false; // Use Complet candle
...
class UpAlertConditionComplet : public ABaseCondition
{
public:
   UpAlertConditionComplet(const string symbol, ENUM_TIMEFRAMES timeframe)
   ...
   {
      return Open[period] <= MathMin(Open[period + 1], Close[period + 1]) && Close[period] > MathMax(Open[period + 1], Close[period + 1]);
   }
};

class DownAlertConditionComplet : public ABaseCondition
{
public:
   DownAlertConditionComplet(const string symbol, ENUM_TIMEFRAMES timeframe)
      :ABaseCondition(symbol, timeframe)
   {
   ...
   bool IsPass(const int period)
   {
      return Open[period] >= MathMax(Open[period + 1], Close[period + 1]) && Close[period] < MathMin(Open[period + 1], Close[period + 1]);
   }
};
...
public:
   ...
   bool IsPass(const int period)
   {
      return Low[period] <= Low[period + 1] && Close[period] > High[period + 1];
   }
};
...
public:
   ...
   bool IsPass(const int period)
   {
      return High[period] >= High[period + 1] && Close[period] < Low[period + 1];
   }
};
...
int init()
{
   ...
   IndicatorName = GenerateIndicatorName("Engulfing Pattern");
   IndicatorObjPrefix = "__" + IndicatorName + "__";
   IndicatorShortName(IndicatorName);

   mainSignaler = new Signaler(_Symbol, (ENUM_TIMEFRAMES)_Period);

   ICondition* upCondition = Complet ? (ICondition*)new UpAlertConditionComplet(_Symbol, (ENUM_TIMEFRAMES)_Period) : (ICondition*)new UpAlertCondition(_Symbol, (ENUM_TIMEFRAMES)_Period);
   ICondition* downCondition = Complet ? (ICondition*)new DownAlertConditionComplet(_Symbol, (ENUM_TIMEFRAMES)_Period) : (ICondition*)new DownAlertCondition(_Symbol, (ENUM_TIMEFRAMES)_Period);
