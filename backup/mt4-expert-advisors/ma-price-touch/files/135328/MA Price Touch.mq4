...
//http://fxcodebase.com/code/viewtopic.php?f=38&t=70081
...
public:
   ...
   {
      double maValue0 = customStream._stream[period];
      return maValue0 <= iHigh(_symbol, _timeframe, period) && maValue0 >= iLow(_symbol, _timeframe, period) && maValue0 < iClose(_symbol, _timeframe, period);
   }
};
...
int init()
{
   ...
   IndicatorBuffers(8);
   IndicatorObjPrefix = GenerateIndicatorPrefix("mapricet");
   IndicatorShortName("MA Price Touch");
   ...
   {
      id = customStream.RegisterStream(id, Yellow, 1, STYLE_SOLID, "MA");
   }
...
int start()
{
   ...
   for (int pos = limit; pos >= 0; --pos)
   {
      if (customStream != NULL)
      {
         customStream._stream[pos] = iMA(_Symbol, _Period, ma_period, 0, ma_method, PRICE_CLOSE, pos);
      }
      for (int i = 0; i < ArraySize(conditions); ++i)
      {
         AlertSignal* item = conditions[i];
