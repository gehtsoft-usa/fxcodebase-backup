...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=75108
...
input const int    grid_max_steps    = 5;                    // Grid Max Trades:
...
class ActionGrid : public iActions
{
  public:
    ...
    bool execute()
    {
        ...
        if (OrderType() == OP_BUY) {
            for (int n = 1; n <= grid_max_steps-1; n++) {
    ...
    bool evaluate()
    {
        ...
        for (int i = OrdersTotal() - 1; i >= 0; i--) {
            if (OrderSelect(i, SELECT_BY_POS) && OrderSymbol() == _Symbol && OrderMagicNumber() == magico && OrderComment() == grid_id) {
                ...
                Print("line: ",__LINE__, " pos: ",i , " tk: ",OrderTicket(), " grid_id: ", grid_id, " current_profit: ",current_profit);
            }
        }
        ...
        Print("line: ",__LINE__,"-------> vuelve a seleccionar el tk original tk: ",tk);
...
void OnInit_Grid()
{
    ...
    stCloseGridBecauseOriginalTradeIsClose.set(&cdOriginalTkIsClose, &acCloseGrid);
...
#ifdef timer_mini
input string T1             = "== Trading Sessions =="; // ————————————————————————
input bool   sessioncontrol = true;                     // Timer On:
input string timeStart      = "00:00:00";               // Time Start GMT
input string timeEnd        = "23:59:59";               // Time End GMT
input double   uTimeZone    = -1;                       // Set your GMT zone (+ / - hs):
input bool close_all_on_tm_end = true;                  // Close All on time end:
...
string tcountfilter        = "== Count Trades Filter =="; // ————————————————————————
int    count_max_positions = 1;                           // # Total positions:
int    count_max_buys      = 1;                           // # Buys limit:
int    count_max_sells     = 1;                           // # Sells limit:
