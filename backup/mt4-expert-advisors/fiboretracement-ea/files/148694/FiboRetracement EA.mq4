...
string file_custom_indicator = "TradeZones.ex4";
...
string    Tpom              = "== Pending or Market Setup ==";  // ————————————
...
input int fibPeriod = 240; // Periods:
...
class BUYcondition1 : public iConditions
{
 public:
  bool evaluate()
  {
	...
	//  buyZone.highPrice =  ObjectGet("BuyZone", 3);
	 buyZone.lowPrice  =  ObjectGet("BuyZone", 1);
  //  return iClose(NULL, 0, 1) < buyZone.highPrice && iClose(NULL, 0, 1) > buyZone.lowPrice;
  //  return iClose(NULL, 0, 1) < buyZone.lowPrice+20*_Point && iClose(NULL, 0, 1) > buyZone.lowPrice;
   return iLow(NULL, 0, 1) < buyZone.lowPrice+50*_Point && iLow(NULL, 0, 1) > buyZone.lowPrice;
  }
};
 ...
 public:
  bool evaluate()
  {
    // TODO: condition sell 1
		sellZone.highPrice =  ObjectGet("SellZone", 3);
		// sellZone.lowPrice  =  ObjectGet("SellZone", 1);
   	
		// return iClose(NULL, 0, 1) < sellZone.highPrice && iClose(NULL, 0, 1) > sellZone.lowPrice;
		// return iClose(NULL, 0, 1) > sellZone.highPrice && iClose(NULL, 0, 1) < sellZone.highPrice+50*_Point;
		return iHigh(NULL, 0, 1) > sellZone.highPrice-50*_Point && iHigh(NULL, 0, 1) < sellZone.highPrice;
  }
};
...
void OnTick()
{
  ...
  double temp = iCustom(NULL, 0, file_custom_indicator, fibPeriod, 0, 0);
...
double Price(string direction, int pips = 0, string _symbol = "")
{
  ...
  if (direction == "buy")
  {
    ...
    // }
		...
		{
      price = buyZone.lowPrice;
    }
		if (modeEntry == PendingStop)
		{
      price = buyZone.highPrice;
    }
  ...
  if (direction == "sell")
  {
    ...
    if (modeEntry == PendingLimit)
		{
			price = sellZone.highPrice;
		}
		if (modeEntry == PendingStop)
		{
      price = sellZone.lowPrice;
    }
...
void ModifyPendings()
{
	for(int i=OrdersTotal()-1;i>=0;i--)
	{
		 if(OrderSelect(i,SELECT_BY_POS) && OrderSymbol() == _Symbol && OrderMagicNumber()== magico)
		 {
				...
				{
          OrderModify(OrderTicket(), buyZone.highPrice, SL("buy", buyZone.highPrice), OrderTakeProfit(), OrderExpiration(), CLR_NONE);
        }
				if(OrderType()==OP_SELLSTOP)
				{
					OrderModify(OrderTicket(), sellZone.lowPrice, SL("sell", sellZone.lowPrice), OrderTakeProfit(), OrderExpiration(), CLR_NONE);
				}
		 }
	}
}
