...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=69434&start=10
...
input int          magico                  = 26;                         // Magic Number:
...
class BamsBum : public iIndicators
{
   string          _symbol;
   int             _tf;
   string          _file;
  ...
  public:
   BamsBum(string Symbol, int TimeFrame)
   {
      _symbol = Symbol;
      _tf     = TimeFrame;
      _file = "#BAMSBUNG-NO REPAINT-3.ex4";
   }
   ~BamsBum() { ;}

   BamsBum* file(string setfile)
   {
      _file = setfile;
      return &this;
   }
...
class CustomConditionBUY : public iConditions
{
  public:
   bool evaluate()
   {
      ...
      Print(__FUNCTION__, " ", "value3: 1", " ", bamsbum.calculate(2, 1));  // blanca
      Print(__FUNCTION__, " ", "value4: 1", " ", bamsbum.calculate(3, 1));  // roja
      double red1   = bamsbum.calculate(3, 1);
      double red2   = bamsbum.calculate(3, 3);
      double white1 = bamsbum.calculate(2, 1);
      double white2 = bamsbum.calculate(2, 3);

      if (white1 > red1 && white2 <= red2)
      {
         return true;
      }
      return false;
...
class CustomConditionSELL : public iConditions
{
  public:
   ...
   {
      // TODO: dev condition
      if (bamsbum.calculate(2, 1) < bamsbum.calculate(3, 1) && bamsbum.calculate(2, 3) >= bamsbum.calculate(3, 3))
      {
         return true;
      }
      return false;
