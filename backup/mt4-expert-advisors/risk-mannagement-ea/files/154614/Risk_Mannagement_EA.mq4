...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=74673
...
input ModeCalcLots modeCalcLots   = AccountPercent;// Mode to Calc Lots:
ModeCalcLots modeLots= modeCalcLots;
input double       userLots       = 0.01;          // Fixed Lots:
input double       userEquityPer  = 1;             // Setup Lots by "Balance Percent":
...
input double     userSLmoney  = 15;                // Money Risk:


double balancePercent = userEquityPer;
double lots          = userLots;
double SLmoney       = userSLmoney;
double SLpips        = userSLpips;
double TPmoney       = userTPmoney;
double TPpips        = userTPpips;
...
input string             IemaFast             = "== Moving Average Fast Setup ==";   // ————————————
input int                maFastPeriod         = 20;                                  // Period
input int                maFastShift          = 0;                                   // MA Shift
input ENUM_MA_METHOD     maFastMethod         = MODE_EMA;                            // Method
input ENUM_APPLIED_PRICE maFastAppliedPrice   = PRICE_CLOSE;                         // Applied Price
input string             IemaSlow             = "== Moving Average Slow Setup ==";   // ————————————
input int                maSlowPeriod         = 50;                                  // Period
input int                maSlowShift          = 0;                                   // MA Shift
input ENUM_MA_METHOD     maSlowMethod         = MODE_EMA;                            // Method
input ENUM_APPLIED_PRICE maSlowAppliedPrice   = PRICE_CLOSE;                         // Applied Price
input string             IemaFilter           = "== Moving Average Filter Setup =="; // ————————————
input bool               maFilterOn           = false;                               // MA Filter On ?
...
bool OnInit_GUI()
{
    ...
    {
        res = gui.Create(0, "Panel Buy - Sell", 0, 400, 0, 620, 270);
        if(res) gui.Run();
    }
    return res;
}
...
class BuyAction : public iActions
{
    bool doAction()
    {
        ...
        int distance = 0;
        double pr = distance == 0 ? 0 : Price("buy", distance, _Symbol);        
        
        SendOrder = new SendNewOrder("buy", Lots(), "", pr, SL("buy", pr), TP("buy", pr), magico);
...
class SellAction : public iActions
{
    bool doAction()
    {
        SendNewOrder* SendOrder;
        int distance = 0;
        double pr = distance == 0 ? 0 : Price("sell", distance, _Symbol);
        SendOrder = new SendNewOrder("sell", Lots(), "", pr, SL("sell", pr), TP("sell", pr), magico);
        bool result = SendOrder.doAction();
        delete SendOrder;
        return result;
    }
...
class GUI : public CAppDialog
{
    ...
    CLabel  lb1, lb2, lb3, lb_Money_TP, lb_Percent_TP, lb_Money_SL, lb_Percent_SL, lb8, lb9, lb10;
    CButton bt_Buy, bt_Sell, bt3;
    CEdit   edit_Balance, edit_Points_TP, edit_Points_SL, edit_Money_Risk, edit_Percent_Risk, edit_Volume;    
    CRadioGroup radio;
    ...
    bool Create(const long chart, const string name, const int subwin, const int x1, const int y1, const int x2, const int y2)
    {
        ...
        if(!Create_Label("Balance:", Col(0), Row(0), _high, _width, lb1)) return false;
        if(!Create_Edit("Balance",   Col(1), Row(0), _high, _width, edit_Balance)) return false;
        
        if(!Create_Label("Pips TP:", Col(0), Row(2), _high, _width, lb2)) return false;
        if(!Create_Edit("Points TP",   Col(1), Row(2), _high, _width, edit_Points_TP)) return false;
        // if(!Create_Label("Money TP",   Col(0), Row(2), _high, _width, lb_Money_TP)) return false;
        // if(!Create_Label("% TP",       Col(1), Row(2), _high, _width, lb_Percent_TP)) return false;

        if(!Create_Label("Pips SL:", Col(0), Row(3), _high, _width, lb3)) return false;
        if(!Create_Edit("Points SL",   Col(1), Row(3), _high, _width, edit_Points_SL)) return false;
        // if(!Create_Label("Money SL",   Col(0), Row(4), _high, _width, lb_Money_SL)) return false;
        // if(!Create_Label("% SL",       Col(1), Row(4), _high, _width, lb_Percent_SL)) return false;

        if(!Create_Label("Money Risk:", Col(0), Row(5), _high, _width, lb8)) return false;
        if(!Create_Edit("MoneyRisk",    Col(1), Row(5), _high, _width, edit_Money_Risk)) return false;
        
        if(!Create_Radio("radio",       Col(2), Row(5), _high*2+_gapV, _width/3, radio)) return false;
        
        if(!Create_Label("% Risk:",     Col(0), Row(6), _high, _width, lb9)) return false;
        if(!Create_Edit("PerRisk",      Col(1), Row(6), _high, _width, edit_Percent_Risk)) return false;
        
        if(!Create_Label("Volume:",     Col(0), Row(7), _high, _width, lb10)) return false;
        if(!Create_Edit("volume",       Col(1), Row(7), _high, _width, edit_Volume)) return false;

        if(!Create_button("BUY",        Col(0), Row(9), _high, _width, bt_Buy)) return false;
        if(!Create_button("SELL",       Col(1), Row(9), _high, _width, bt_Sell)) return false;
    ...
    {
        if(bt_Buy.IsActive()) bt_Buy.ColorBackground(PaleGreen); else bt_Buy.ColorBackground(CONTROLS_BUTTON_COLOR_BG);
        if(bt_Sell.IsActive()) bt_Sell.ColorBackground(Crimson); else bt_Sell.ColorBackground(CONTROLS_BUTTON_COLOR_BG);        
    }

    protected:
    void OnEndEdit_edit_Balance() { Print("edit: balance "+edit_Balance.Text()); }
    void OnEndEdit_edit_Points_TP() { TPpips = edit_Points_TP.Text(); Print("edit: points TP "+edit_Points_TP.Text()); }
    void OnEndEdit_edit_Points_SL() { SLpips = edit_Points_SL.Text(); Refresh(); Print("edit: points SL "+edit_Points_SL.Text()); }
    void OnEndEdit_edit_Money_Risk() { SLmoney = edit_Money_Risk.Text(); Refresh(); Print("edit: edit_Money_Risk "+edit_Money_Risk.Text()); }
    void OnEndEdit_edit_Percent_Risk() { balancePercent = edit_Percent_Risk.Text(); Refresh(); Print("edit: edit_Percent_Risk "+edit_Percent_Risk.Text()); }
    void OnEndEdit_edit_Volume() {lots = edit_Volume.Text(); Refresh(); Print("edit: edit_Volume "+edit_Volume.Text()); }
    ...
    bool Create_Edit(string name, const int x1, const int y1, const int high, const int width, CEdit& ed)
    {
        ...
        if(name == "volume" || name== "Balance")ed.ReadOnly(true);

        Add(ed);
        return true;
    }
    bool Create_Radio(string name, const int x1, const int y1, const int high, const int width, CRadioGroup& rg)
    {
        int x2 = x1 + width;
        int y2 = y1 + high;


        rg.Create(m_chart_id, name, m_subwin, x1, y1, x2, y2);
        rg.AddItem("item1",0);
        rg.AddItem("item2",1);
        rg.Value(1);
        Add(rg);
    ...
    void Start()
    {
        ...
        edit_Balance.Text(bal);
        
        edit_Points_TP.Text(userTPpips);
        edit_Points_SL.Text(userSLpips);
        edit_Money_Risk.Text(userSLmoney);
        edit_Percent_Risk.Text(userEquityPer);
        edit_Volume.Text(Lots());
    }
    ...
    void onChange_Radio()
    {
        ...
        // Balance Percent Risk
        if(radio.Value()==1)
        {
            modeLots = AccountPercent;
...
EVENT_MAP_BEGIN(GUI)
ON_EVENT(ON_END_EDIT, edit_Balance, OnEndEdit_edit_Balance)
ON_EVENT(ON_END_EDIT, edit_Points_TP, OnEndEdit_edit_Points_TP)
ON_EVENT(ON_END_EDIT, edit_Points_SL, OnEndEdit_edit_Points_SL)
ON_EVENT(ON_END_EDIT, edit_Money_Risk, OnEndEdit_edit_Money_Risk)
ON_EVENT(ON_END_EDIT, edit_Percent_Risk, OnEndEdit_edit_Percent_Risk)
ON_EVENT(ON_END_EDIT, edit_Volume, OnEndEdit_edit_Volume)
ON_EVENT(ON_CLICK, bt_Buy, button1Action.doAction)
ON_EVENT(ON_CLICK, bt_Sell, button2Action.doAction)
ON_EVENT(ON_CLICK, bt3, button3Action.doAction)
ON_EVENT(ON_CHANGE, radio, onChange_Radio)
  ...
  public:
    bool evaluate()
    {
        if (closeAllInOpositeSignal)
            if (sellCondition1.evaluate() && sellCondition2.evaluate())
                return true;
...
class ConditionToCloseSell : public iConditions
{
  public:
    ...
    {
        if (closeAllInOpositeSignal)
            if (buyCondition1.evaluate() && buyCondition2.evaluate())
                return true;
...
int OnInit()
{
    ...
    gui.setButton1Action(&btnBuyAction);
    gui.setButton2Action(&btnSellAction);
    gui.Start();
...
double SL(string side, double price = 0)
{
    ...
    if (stopLossOn)
        switch (modeSL) {
        case FixPips:
            levelSL = new Levels(new ByFixPips(_Symbol, side, SLpips, "SL", price));
            result  = levelSL.calculateLevel();
            break;

        case byMoney:
            levelSL = new Levels(new ByMoney(_Symbol, side, userLots, SLmoney, "SL"));
            result  = levelSL.calculateLevel();
            break;

        case PipsFromOpenCandle:
            levelSL = new Levels(new ByPipsFromCandle(_Symbol, side, SLpips, "SL", 0, 1));
            result  = levelSL.calculateLevel();
            break;
        }
    return result;
...
double TP(string side, double price = 0)
{
    ...
    if (takeProfitOn)
        switch (modeTP) {
        case FixPips:
            levelTP = new Levels(new ByFixPips(_Symbol, side, TPpips, "TP", price));
            result  = levelTP.calculateLevel();
            break;

        case byMoney:
            levelTP = new Levels(new ByMoney(_Symbol, side, userLots, TPmoney, "TP"));
            result  = levelTP.calculateLevel();
            break;
        case PipsFromOpenCandle:
            levelSL = new Levels(new ByPipsFromCandle(_Symbol, side, SLpips, "TP", 0, 1));
            result  = levelSL.calculateLevel();
        }
    return result;
}
double Lots()
{
    ...
    switch (modeLots) {
        case Money:
          _lots = lotProvider.LotsByMoney(SLmoney, SLpips);
          break;

    case AccountPercent:
         _lots = lotProvider.LotsByBalancePercent(balancePercent, SLpips);
         break;
    ...
    // case EquityPercent:
    //     _lots = lotProvider.LotsByEquityPercent(balancePercent);
    //     break;
    }
    delete lotProvider;
    return _lots;
