...
// http://fxcodebase.com/code/viewtopic.php?f=38&t=70002
//+------------------------------------------------------------------+
...
public:
   ...
   { 
      ...
      {
         double a = iCustom(_symbol, _timeframe, "PG", MinorMinExtremeHeightATRs, MajorToMinorHeightRatio, MinorMinExtremeWidth, MajorMinExtremeWidth, 0, i);
         if (a != 0 && a != EMPTY_VALUE)
         {
            ObjectMakeLabel(_id, _x, _y, "A", Labels_Color, 1, WindowNumber, "Arial", font_size); 
            return;
         }
         double b = iCustom(_symbol, _timeframe, "PG", MinorMinExtremeHeightATRs, MajorToMinorHeightRatio, MinorMinExtremeWidth, MajorMinExtremeWidth, 4, i);
         if (b != 0 && b != EMPTY_VALUE)
         {
            ObjectMakeLabel(_id, _x, _y, "B", Labels_Color, 1, WindowNumber, "Arial", font_size); 
            return;
         }
         double c = iCustom(_symbol, _timeframe, "PG", MinorMinExtremeHeightATRs, MajorToMinorHeightRatio, MinorMinExtremeWidth, MajorMinExtremeWidth, 2, i);
         if (c != 0 && c != EMPTY_VALUE)
         {
            ObjectMakeLabel(_id, _x, _y, "C", Labels_Color, 1, WindowNumber, "Arial", font_size); 
            return;
         }
      }
      ObjectMakeLabel(_id, _x, _y, "-", Labels_Color, 1, WindowNumber, "Arial", font_size); 
   }
};
...
int init()
{
   double temp = iCustom(NULL, 0, "PG", MinorMinExtremeHeightATRs, MajorToMinorHeightRatio, MinorMinExtremeWidth, MajorMinExtremeWidth, 0, 0);
   if (GetLastError() == ERR_INDICATOR_CANNOT_LOAD)
   {
      Alert("Please, install the 'PG' indicator");
      return INIT_FAILED;
   }

   IndicatorObjPrefix = GenerateIndicatorPrefix("pgd");
   IndicatorShortName("PG Dashboard");
...
int start()
{
   WindowNumber = MathMax(0, WindowFind("PG Dashboard"));
   grid.Draw();
