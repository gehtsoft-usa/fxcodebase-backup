//Available @ https://fxcodebase.com/code/viewtopic.php?f=38&t=73691
...
// tiene que permitirme buscar un min o max en uno de los arrays de precios
...
class SearchExtremes
{
    ...
    double Maximum(int iniTm, int endTm, string _mode, string symbol = NULL, ENUM_TIMEFRAMES tf = 0)
    {
...
Line lineBuy("buy_line");
Line lineSell("sell_line");
...
input string    Tpom = "== London Session Setup ==";  // ————————————
input string London_night = "21:00:00";       // London Night (GMT)
input string London_open = "09:00:00";       // London Open (GMT)
int qntBuys = 1; // # Buys:
int qntSells = 1; // # Sells:
// input double    uEntryDistance    = 150;                              // Pips distance for pending orders:
int pipsBtw = 50; // Pips Between Orders:
...
string Ti = "== Indicator Setup ==";  // ————————————
int Broker_GMT_Shift = 1;
int London_Night = 21;
int London_Open = 9;
bool Currency_Auto_Setup = true;

input string T0 = "== Trade Setup ==";  // ————————————
#ifdef MAX_TRADES_AT_SAME_TIME
int uMaxTrades = 2;  // Max Trades At Same Time:
#endif
...
input ModeLevels   modeTP = Indicator;            // Mode Take Profit:
...
input ModeLevels   modeSL = Indicator;            // Mode Stop Loss:
...
ScheduleController LondonControl;
...
class ConditionCountTradesPendings : public iConditions
{
    int _max;
    int _count;

    public:
    ConditionCountTradesPendings(int maxTrades)
    {
        _max = maxTrades;
    }
    ~ConditionCountTradesPendings() { ; }
...
ConditionCountTradesPendings countPendings(2);

class ConditionCountTradesOpen : public iConditions
{
    int _max;
    int _count;

    public:
    ConditionCountTradesOpen(int maxTrades)
    {
        _max = maxTrades;
    }
    ~ConditionCountTradesOpen() { ; }
...
ConditionCountTradesOpen* countTradesOpen;
...
class BUYcondition1 : public iConditions
{
    ...
    bool evaluate()
    {
        ...
        // int qnt = mainOrders.qnt();
        // Print(__FUNCTION__," qnt: ",qnt);
        if(countPendings.count() < 1) return true;
...
class BUYcondition2 : public iConditions
{
    ...
    bool evaluate()
    {
        ...
        if (buyPrice > 0 && sellPrice > 0 && buyTP > 0 && sellTP > 0) return true;
...
class SELLcondition1 : public iConditions
{
    ...
    bool evaluate()
    {
        // TODO: condition sell 1
            // return iClose(NULL, 0, 1) < ma.index(1);
        if (countPendings.count() <= 1) return true;
        // if(mainOrders.qnt()==0) return true;
        return false;
    }
};
...
int OnInit()
{
    ...
    conditionsToBuy.AddCondition(countTradesOpen = new ConditionCountTradesOpen(1));
    ...
    conditionsToSell.AddCondition(countTradesOpen = new ConditionCountTradesOpen(1));
    ...
    LondonControl.AddSession(London_open, London_night);
    // #endif
...
void OnTick()
{
   ...
   // Close controls:
    ...
    if(!uTradeReverse)
    {
        ...
        if(!LondonControl.doSessionControl())
        {
            closeAll();
            get_MinMax_from_London();
            allowTrade = true;
            
            lineSell.iniTime(search.ConvertTime(London_night) - 24 * 3600);
            lineSell.price(sellPrice).clr(Crimson).redraw();

            lineBuy.iniTime(search.ConvertTime(London_night) - 24 * 3600);
            lineBuy.price(buyPrice).clr(Green).redraw();
    ...
    if(LondonControl.doSessionControl()== true && allowTrade== true)    
    {
        // NOTE: BUY normal
        if(!uTradeReverse)
        {
            if(conditionsToBuy.EvaluateConditions())
                for(int i = 1; i <= qntBuys; i++)
                {
                    {
                        double pr = buyPrice;
...
double SL(string side, double price = 0)
{
    ...
    if(stopLossOn)
        switch(modeSL)
        {
            ...
            case Indicator:
                result = side == "buy" ? sellPrice : buyPrice;
                break;
        }
    return result;
}
double TP(string side, double price = 0)
{
    ...
    if(takeProfitOn)
        switch(modeTP)
        {
            ...
            case Indicator:
                result = side == "buy" ? buyPrice + gapRange : sellPrice - gapRange;
                break;
        }
    return result;
}
...
bool get_MinMax_from_London()
{

    double max = search.Maximum(London_night, London_open, "high");
    buyPrice = max;

    double min = search.Minimum(London_night, London_open, "low");
    sellPrice = min;

    gapRange = max - min;

    if(buyPrice > 0 && sellPrice > 0) return true;
