...
input string    uComment = "";                       // Custom Comment:
...
#define Section_Notifications
#ifdef Section_Notifications

input string tnotif                = "== Notifications =="; // === Notifications ===
...
class OrderCreator : public iActions
{
  ...
  public:
    OrderCreator(const string Symbol, const string Side, const int Magic, string comment="")
    {
        _symbol = Symbol;
        _side   = Side;
        _magic  = Magic;
    ...
    bool execute()
    {
        ...
        int result = OrderSend(_symbol, _cmd, _lots, _price, _slippage, _sl, _tp, _comment, _magic, 0, clrNONE);

    if(result > 0) {
        if(_cmd == OP_BUY) { Notifications(0); }
        if(_cmd == OP_SELL) { Notifications(1); }
    }
...
OrderCreator buy(_Symbol, "buy", magico, uComment);
OrderCreator sell(_Symbol, "sell", magico, uComment);
...
input const string indicator_file = "#ALICE-[NRP][+]UltradeFX-MasterEntry"; // Indicator File (put your indicator name here):
//---
...
class ConditionCloseGridByProfit : public iConditions
{
  public:
    bool evaluate()
    {
        ...
        for (int i = OrdersTotal() - 1; i >= 0; i--) {
            if (OrderSelect(i, SELECT_BY_POS) && OrderSymbol() == _Symbol && OrderMagicNumber() == magico && StringFind(OrderComment(), grid_id) >= 0) {
...
input bool   MartingaleON              = false;                    // Use Martingale:
...
#define Section_ReOpen
#ifdef Section_ReOpen
// Mark: Section_ReOpen

input string       treopen     = "== Re-Open Setup =="; // ————————————————————————
input const bool   reopen_on   = false;                 // Re-Open ON:
input const double reopen_step = 10;                    // Re-Open Step:

class ConditionReOpen : public iConditions
{
  public:
    bool evaluate()
    {
        bool   r       = false;
        double mPoints = MarketInfo(OrderSymbol(), MODE_POINT);
        double step    = reopen_step * mPoints * 10;
        double price   = 0;

        if (OrderType() == OP_BUY) {
            price = OrderOpenPrice() + step;
            if (Ask >= price) { r = true; }
        }
...
ConditionReOpen cdReOpen;

class ActionReOpen : public iActions
{
    // cerrar el trade original
    // abrir otro igual
  ...
  public:
    bool execute() { 
        ...
        // if (!OrderClose(OrderTicket(), OrderLots(), ask, 0, Black)) { Print("Error when try to close order ", GetLastError()); }
        // if (OrderType() == OP_SELL) return false;
        if (OrderType() == OP_SELL)
        {
            acCloseSell.execute();
        ...
        // double bid = SymbolInfoDouble(OrderSymbol(), SYMBOL_BID);
        // if (!OrderClose(OrderTicket(), OrderLots(), bid, 0, Black)) { Print("Error when try to close order ", GetLastError()); }
        // return true;
        
        return true; 
    }
};
ActionReOpen acReOpen;

Strategy stReOpen(&cdReOpen, &acReOpen);

void OnInit_ReOpen() {
    Trader.management.add(&stReOpen);
}
