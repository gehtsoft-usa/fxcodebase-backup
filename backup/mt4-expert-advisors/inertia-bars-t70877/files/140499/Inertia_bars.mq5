...
//http://fxcodebase.com/code/viewtopic.php?f=38&t=70877
...
{
   double Body = MathAbs(iClose(_Symbol, (ENUM_TIMEFRAMES)_Period, pos) - iOpen(_Symbol, (ENUM_TIMEFRAMES)_Period, pos));
   if (Method == InPips)
   {
      return Body / pipSize;
   }

   double Candle = MathAbs(iHigh(_Symbol, (ENUM_TIMEFRAMES)_Period, pos) - iLow(_Symbol, (ENUM_TIMEFRAMES)_Period, pos));
...
public:
   ...
   {
      double Value = GetValue(period);
      if (iClose(_symbol, _timeframe, period) >= iOpen(_symbol, _timeframe, period))
      {
         return Value >= Level;
      }
      return false;
...
int OnInit(void)
{
   ...
   int mult = digit == 3 || digit == 5 ? 10 : 1;
   pipSize = point * mult;
   IndicatorObjPrefix = GenerateIndicatorPrefix("ibi");
   IndicatorSetString(INDICATOR_SHORTNAME, "Inertia bars indicator");
