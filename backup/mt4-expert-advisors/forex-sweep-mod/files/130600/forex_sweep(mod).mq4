...
int start()
{
   ...
   int limit = MathMin(bars_limit, Bars - counted_bars - 1);
   for (int i = limit; i >= 0; i--)
   {
      if (tf == _Period || tf == PERIOD_CURRENT)
      {
         ...
         {
            if(buffer3[i] > 0 && buffer3[i + 1] < 0) 
               MA_sUP[i] = INDICATOR_MAXIMUM;
            if(buffer3[i] < 0 && buffer3[i + 1] > 0) 
               MA_sDN[i] = -INDICATOR_MINIMUM;
         }
         if (NumLine == 2) 
         {
            if(MA1buffer[i] > 0 && MA1buffer[i + 1] < 0) 
               MA_sUP[i] = INDICATOR_MAXIMUM;
            if(MA1buffer[i] < 0 && MA1buffer[i + 1] > 0) 
               MA_sDN[i] = -INDICATOR_MINIMUM;
         }
         if (NumLine == 3) 
         {
            if(MA2buffer[i] > 0 && MA2buffer[i + 1] < 0) 
               MA_sUP[i] = INDICATOR_MAXIMUM;
            if(MA2buffer[i] < 0 && MA2buffer[i + 1] > 0) 
               MA_sDN[i] = -INDICATOR_MINIMUM;
         }
      ...
      else
      {
         ...
         buffer1[i] = iCustom(_Symbol, tf, "forex_sweep(mod)", period, price, false, MA1period, MA2period, "", "", TypeMA1, TypeMA2, NumLine, 0, index);
         buffer2[i] = iCustom(_Symbol, tf, "forex_sweep(mod)", period, price, false, MA1period, MA2period, "", "", TypeMA1, TypeMA2, NumLine, 1, index);
         buffer3[i] = iCustom(_Symbol, tf, "forex_sweep(mod)", period, price, false, MA1period, MA2period, "", "", TypeMA1, TypeMA2, NumLine, 2, index);
         MA1buffer[i] = iCustom(_Symbol, tf, "forex_sweep(mod)", period, price, false, MA1period, MA2period, "", "", TypeMA1, TypeMA2, NumLine, 3, index);
         MA2buffer[i] = iCustom(_Symbol, tf, "forex_sweep(mod)", period, price, false, MA1period, MA2period, "", "", TypeMA1, TypeMA2, NumLine, 4, index);
         MA_sUP[i] = iCustom(_Symbol, tf, "forex_sweep(mod)", period, price, false, MA1period, MA2period, "", "", TypeMA1, TypeMA2, NumLine, 5, index);
         MA_sDN[i] = iCustom(_Symbol, tf, "forex_sweep(mod)", period, price, false, MA1period, MA2period, "", "", TypeMA1, TypeMA2, NumLine, 6, index);
      }
   } 
   if (MA_sDN[0] != EMPTY_VALUE && MA_sDN[1] != EMPTY_VALUE)
   {   
      if (Time[0] > alertBar)
      {
   ...
   if (MA_sUP[0] != EMPTY_VALUE && MA_sUP[1] != EMPTY_VALUE)
   {
      if (Time[0] > alertBar) 
      {
