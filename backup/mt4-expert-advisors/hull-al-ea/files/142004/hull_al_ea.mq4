...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=71185
...
input int     FPrice             =    0;  // Fast price
input int     FLength            =   14;  // Fast length
input double  FSigma             =  6.0;  // Fast sigma parameter 
input double  Fffset            = 0.85;  // Fast offset of Gaussian distribution (0...1)
input double  FDampingFactor    =    1;  // 0...1.0(ex.0.7)
input double  FPctFilter         =    0;  // Fast dynamic filter in decimal
input int     FShift             =    0;  // Fast shift

input int     SPrice             =    0;  // Slow price
input int     SLength            =   21;  // Slow length
input double  SSigma             =  6.0;  // Slow sigma parameter 
input double  Sffset            = 0.85;  // Slow offset of Gaussian distribution (0...1)
input double  SDampingFactor    =    1;  // 0...1.0(ex.0.7)
input double  SPctFilter         =    0;  // Slow dynamic filter in decimal
input int     SShift             =    0;  // Slow shift
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      double fast = iCustom(_symbol, _timeframe, "hull_al", FPrice, FLength, FSigma, Fffset, FDampingFactor, FPctFilter, FShift, true, 1, period);
      double slow = iCustom(_symbol, _timeframe, "hull_al", SPrice, SLength, SSigma, Sffset, SDampingFactor, SPctFilter, SShift, true, 1, period);
      return fast != EMPTY_VALUE && slow != EMPTY_VALUE;
   }
};
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      double fast = iCustom(_symbol, _timeframe, "hull_al", FPrice, FLength, FSigma, Fffset, FDampingFactor, FPctFilter, FShift, true, 2, period);
      double slow = iCustom(_symbol, _timeframe, "hull_al", SPrice, SLength, SSigma, Sffset, SDampingFactor, SPctFilter, SShift, true, 2, period);
      return fast != EMPTY_VALUE && slow != EMPTY_VALUE;
   }
};
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      double fast = iCustom(_symbol, _timeframe, "hull_al", FPrice, FLength, FSigma, Fffset, FDampingFactor, FPctFilter, FShift, 2, period);
      double slow = iCustom(_symbol, _timeframe, "hull_al", SPrice, SLength, SSigma, Sffset, SDampingFactor, SPctFilter, SShift, 2, period);
      return fast != EMPTY_VALUE || slow != EMPTY_VALUE;
   }
};
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      double fast = iCustom(_symbol, _timeframe, "hull_al", FPrice, FLength, FSigma, Fffset, FDampingFactor, FPctFilter, FShift, 1, period);
      double slow = iCustom(_symbol, _timeframe, "hull_al", SPrice, SLength, SSigma, Sffset, SDampingFactor, SPctFilter, SShift, 1, period);
      return fast != EMPTY_VALUE || slow != EMPTY_VALUE;
   }
};
...
int OnInit()
{
   double temp = iCustom(NULL, 0, "hull_al", 0, 0);
   if (GetLastError() == ERR_INDICATOR_CANNOT_LOAD)
   {
      Alert("Please, install the 'hull_al' indicator");
      return INIT_FAILED;
   }
