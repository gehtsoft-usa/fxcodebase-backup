...
//http://fxcodebase.com/code/viewtopic.php?f=38&t=70684
...
input int DI_Length=5; // Demand index length
input int CMF_Periods=20; // CMF period
input int SRC_Range=14; // SRC Range
input bool SRC_direction = true; // SRC Direction
...
public:
   ...
   {
      return iCustom(_symbol, _timeframe, "Demand_Index", DI_Length, 0, period) > 0
         && iCustom(_symbol, _timeframe, "CMF", CMF_Periods, 0, period) > 0
         && iCustom(_symbol, _timeframe, "Spearman_Rank_Correlation", SRC_Range, SRC_direction, 0, period) > 0;
   }
};
...
int init()
{
   ...
   double temp = iCustom(NULL, 0, "Demand_Index", 0, 0);
   if (GetLastError() == ERR_INDICATOR_CANNOT_LOAD)
   {
      Alert("Please, install the 'Demand_Index' indicator");
      return INIT_FAILED;
   }
   temp = iCustom(NULL, 0, "CMF", 0, 0);
   if (GetLastError() == ERR_INDICATOR_CANNOT_LOAD)
   {
      Alert("Please, install the 'CMF' indicator");
      return INIT_FAILED;
   }
   temp = iCustom(NULL, 0, "Spearman_Rank_Correlation", 0, 0);
   if (GetLastError() == ERR_INDICATOR_CANNOT_LOAD)
   {
      Alert("Please, install the 'Spearman_Rank_Correlation' indicator");
      return INIT_FAILED;
   }
   IndicatorBuffers(8);
   IndicatorObjPrefix = GenerateIndicatorPrefix("3oa");
   IndicatorShortName("3 oscillator arrows");
