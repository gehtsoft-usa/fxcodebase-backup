...
double Blau_SM_Stoch[], Signal[];
double HH[], EMA1[], EMA2[];
double Half_HH[], Half_EMA1[], Half_EMA2[];

int init()
{
 IndicatorShortName("William Blau Stochastic Momentum Oscillator");
 IndicatorDigits(Digits);
 SetIndexStyle(0,DRAW_HISTOGRAM);
 SetIndexBuffer(0,Blau_SM_Stoch);
 ...
 SetIndexBuffer(5,Half_HH);
 SetIndexStyle(6,DRAW_NONE);
 SetIndexBuffer(6,Half_EMA1);
 SetIndexStyle(7,DRAW_NONE);
 SetIndexBuffer(7,Half_EMA2);
...
int start()
{
 ...
 while(pos>=0)
 {
  ...
  if (Half_EMA3>0)
  {
   Blau_SM_Stoch[pos]=100.*EMA3/Half_EMA3;
  }
  else
  {
   Blau_SM_Stoch[pos]=0;
 ...
 while(pos>=0)
 {
  Signal[pos]=iMAOnArray(Blau_SM_Stoch, 0, Signal_Length, 0, MODE_EMA, pos);
  pos--;
 }
