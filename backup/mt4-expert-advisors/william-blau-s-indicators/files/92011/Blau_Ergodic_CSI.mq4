...
double Blau_CSI[], Signal[];
double CM[], CM_EMA1[], CM_EMA2[];
double HH[], HH_EMA1[], HH_EMA2[];

int init()
{
 IndicatorShortName("William Blau Ergodic Candlestick Index");
 IndicatorDigits(Digits);
 SetIndexStyle(0,DRAW_HISTOGRAM);
 SetIndexBuffer(0,Blau_CSI);
 SetIndexStyle(1,DRAW_LINE);
 SetIndexBuffer(1,Signal);
 SetIndexStyle(2,DRAW_NONE);
 SetIndexBuffer(2,CM);
 SetIndexStyle(3,DRAW_NONE);
 SetIndexBuffer(3,CM_EMA1);
 SetIndexStyle(4,DRAW_NONE);
 SetIndexBuffer(4,CM_EMA2);
...
int start()
{
 ...
 while(pos>=0)
 {
  CM_EMA1[pos]=iMAOnArray(CM, 0, Smooth_Length1, 0, MODE_EMA, pos);
 ...
 while(pos>=0)
 {
  CM_EMA2[pos]=iMAOnArray(CM_EMA1, 0, Smooth_Length2, 0, MODE_EMA, pos);
  HH_EMA2[pos]=iMAOnArray(HH_EMA1, 0, Smooth_Length2, 0, MODE_EMA, pos);
  pos--;
 }  

 double CM_EMA3, HH_EMA3;
 pos=limit;
 while(pos>=0)
 {
  CM_EMA3=iMAOnArray(CM_EMA2, 0, Smooth_Length3, 0, MODE_EMA, pos);
  HH_EMA3=iMAOnArray(HH_EMA2, 0, Smooth_Length3, 0, MODE_EMA, pos);
  if (HH_EMA3>0)
  {
   Blau_CSI[pos]=100.*CM_EMA3/HH_EMA3;
  }
  else
  {
   Blau_CSI[pos]=0;
 ...
 while(pos>=0)
 {
  Signal[pos]=iMAOnArray(Blau_CSI, 0, Signal_Length, 0, MODE_EMA, pos);
  pos--;
 }  
