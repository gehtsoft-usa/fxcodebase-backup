...
//---- set dynamic array as as a buffer for data saving   
   SetIndexBuffer(2,TimeSeriesBuffer,INDICATOR_DATA);
   ...
   for(bar=limit; bar>=0 && !IsStopped(); bar--)
     {
      ...
      double sum=0;
      for(int n = (int)ma_periods; n>0; n--) {
         // if(bar+n>=ArraySize(IndBuffer)) continue;
         if (IndBuffer[bar+n] == 0) { continue; }         
         sum += IndBuffer[bar+n];            
       }
...
//----     
  ...
  //     Print("line: ",__LINE__," bar: ",bar);
  //     Print("line: ",__LINE__," IndBuffer[bar] = ",IndBuffer[bar]);
  //     Print("line: ",__LINE__," MaBuffer[bar] = ",MaBuffer[bar]);
  //     if(bar>=ArraySize(IndBuffer)) continue;
  //     if(bar>0 && IndBuffer[bar]>MaBuffer[bar] && IndBuffer[bar+1]<=MaBuffer[bar+1]) {
...
//---- return the value of prev_calculated for the next call
   return(rates_total);
  }
//+------------------------------------------------------------------+
//Available @  https://fxcodebase.com/code/viewtopic.php?f=38&t=75700
