...
//https://fxcodebase.com/code/viewtopic.php?f=38&t=71474
...
public:
   ...
   void Get(const int period, const double entryPrice, double &amount, double &stopLoss, double &takeProfit)
   {
       ...
       // NOTE: lot calc martingale:
       double l = martin.Lot();
       if(MartingaleON==true && l != 0) amount = martin.Lot();
...
public:
   ...
   virtual double GetValue(const int period, double entryPrice)
   {
      return _isBuy ? iLow(_symbol, _timeframe, period + 1) : iHigh(_symbol, _timeframe, period + 1);
   }
};
#endif
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      return iClose(_symbol, _timeframe, period) >= iHigh(_symbol, _timeframe, period + 1);
   }
};
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      return iClose(_symbol, _timeframe, period) <= iLow(_symbol, _timeframe, period + 1);
   }
};
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      return iClose(_symbol, _timeframe, period) > iMA(_symbol, _timeframe, ma_period, 0, ma_method, PRICE_CLOSE, period);
   }
};
...
MartingaleIncrementLot martin(magic_number, _Symbol);
