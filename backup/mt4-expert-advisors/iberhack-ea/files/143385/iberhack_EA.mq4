...
// http://fxcodebase.com/code/viewtopic.php?f=38&t=71451
...
input int x_bar = 1; // X, bars
input bool close_after_x_bars = true; // Limit lifetime of a trade
input int trade_lifetime = 10; // Trade lifetime in bars
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      int highestIndex = iHighest(_symbol, _timeframe, MODE_HIGH, x_bar, period + 1);
      double highest = iHigh(_symbol, _timeframe, highestIndex);
      return iClose(_symbol, _timeframe, period) > highest;
   }
};
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      int lowestIndex = iLowest(_symbol, _timeframe, MODE_LOW, x_bar, period + 1);
      double lowest = iLow(_symbol, _timeframe, lowestIndex);
      return iClose(_symbol, _timeframe, period) < lowest;
   }
};
