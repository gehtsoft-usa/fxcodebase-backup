...
//http://fxcodebase.com/code/viewtopic.php?f=38&t=68924
...
input int Short_Length = 12; // MACD short
input int Long_Length = 26; // MACD long
input int Signal_Length = 9; // MACD signal
...
public:
   ...
   {
      double macdValue = iMACD(_symbol, _timeframe, Short_Length, Long_Length, Signal_Length, PRICE_CLOSE, MODE_MAIN, period)
         - iMACD(_symbol, _timeframe, Short_Length, Long_Length, Signal_Length, PRICE_CLOSE, MODE_SIGNAL, period);
      double macdValue1 = iMACD(_symbol, _timeframe, Short_Length, Long_Length, Signal_Length, PRICE_CLOSE, MODE_MAIN, period + 1)
         - iMACD(_symbol, _timeframe, Short_Length, Long_Length, Signal_Length, PRICE_CLOSE, MODE_SIGNAL, period + 1);
      return iClose(_symbol, _timeframe, period) > iHigh(_symbol, _timeframe, period - 1) 
         && macdValue < macdValue1;
   }
};
...
int init()
{
   ...
   IndicatorName = GenerateIndicatorName("MACD Histogram Reversal");
