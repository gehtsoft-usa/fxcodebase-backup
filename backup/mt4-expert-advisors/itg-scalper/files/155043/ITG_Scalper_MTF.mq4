...
input ENUM_TIMEFRAMES TF_Sup     = PERIOD_H4; // Time Frame:
ENUM_TIMEFRAMES       tf;
                ...
                const int&      spread[])
{
    ...
    for (i = start; i >= 0; i--) {
        int isup     = iBarShift(NULL, tf, time[i], true);
        EmaOrigin[i] = iMA(NULL, tf, EMA_period, 0, MODE_EMA, PRICE_CLOSE, isup); // escalonado
    }

    for (i = start; i >= 0; i--) {
        int isup  = iBarShift(NULL, tf, time[i], false);
        lastEma   = iMA(NULL, tf, EMA_period, 0, MODE_EMA, PRICE_CLOSE, isup+1);

        if (i - 2 >= 0)
            if (EmaOrigin[i] != EmaOrigin[i+1]) {
                currentEma = EmaOrigin[i];

                for (int x = i+1; x < start; x++) {
                    if (EmaOrigin[x] == lastEma) barsBtw++;
                        else break;
                }

                if (barsBtw > 0)
                    for (int p = 1; p <= barsBtw; p++) {
                        double distance = currentEma - lastEma;
                        double portion  = distance / barsBtw;
                        
                        Ema[i + p] = currentEma - (p * portion);
                    }
                barsBtw = 0;            
            }        
    }
...
// ---- draw lineas and arrows
    ...
    for (i = start; i >= 0; i--) {
        ...
        if (TemaBuffer[i] >= TemaBuffer[i + 1]) {
          ...
          if (lookfor == 1 && LineUp[i+1]!=EMPTY_VALUE && MACD_Main(isup+1) > MACD_Signal(isup+1) && MACD_Main(isup+2) > MACD_Signal(isup+2)) {
                lookfor    = 0;
                ArrowUp[i+1] = Low[i+1];
                notify(0);
            }
        ...
        if (TemaBuffer[i] < TemaBuffer[i + 1]) {
            ...
            if (lookfor == 0 && LineDn[i]!=EMPTY_VALUE && MACD_Main(isup+1) < MACD_Signal(isup+1) && MACD_Main(isup+2) > MACD_Signal(isup+2)) {
...
{
    return iMACD(NULL, tf, fast_ema_period, slow_ema_period, signal_period, applied_price, 0, i);
}
double MACD_Signal(int i)
{
    return iMACD(NULL, tf, fast_ema_period, slow_ema_period, signal_period, applied_price, 1, i);
}
