...
// http://fxcodebase.com/code/viewtopic.php?f=38&t=69128
...
input int FastN = 5; // Fast Period
input int SlowN = 10; // Slow Period
input int RSIN = 14; // RSI Period
input int STOK = 14; // K% Line Period
input int STOD = 3; // D% Line Period
input int STOSD = 3; // Slow %D Period
...
public:
   ...
   {
      double fastMA0 = iMA(_symbol, _timeframe, FastN, 0, Method, PRICE_CLOSE, period);
      double fastMA1 = iMA(_symbol, _timeframe, FastN, 0, Method, PRICE_CLOSE, period + 1);
      double slowMA0 = iMA(_symbol, _timeframe, SlowN, 0, Method, PRICE_CLOSE, period);
      double slowMA1 = iMA(_symbol, _timeframe, SlowN, 0, Method, PRICE_CLOSE, period + 1);

      double rsi0 = iRSI(_symbol, _timeframe, RSIN, PRICE_CLOSE, period);
      double rsi1 = iRSI(_symbol, _timeframe, RSIN, PRICE_CLOSE, period + 1);
      double stochK0 = iStochastic(_symbol, _timeframe, STOK, STOD, STOSD, Method, 0, MODE_MAIN, period);
      double stochK1 = iStochastic(_symbol, _timeframe, STOK, STOD, STOSD, Method, 0, MODE_MAIN, period + 1);
      double stochD0 = iStochastic(_symbol, _timeframe, STOK, STOD, STOSD, Method, 0, MODE_SIGNAL, period);
      double stochD1 = iStochastic(_symbol, _timeframe, STOK, STOD, STOSD, Method, 0, MODE_SIGNAL, period + 1);

      return fastMA0 >= slowMA0 && fastMA1 < slowMA1 && fastMA0 > fastMA1
         && rsi0 >= 50 && rsi1 < 50 && rsi0 < 70
         && stochK0 >= stochD0 && stochK1 < stochD1 && stochK0 < 80;
   }
};
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      ...
      return fastMA0 <= slowMA0 && fastMA1 > slowMA1 && fastMA0 < fastMA1
         && rsi0 <= 50 && rsi1 > 50 && rsi0 > 30
         && stochK0 <= stochD0 && stochK1 > stochD1 && stochK0 > 20;
   }
};
...
int init()
{
   ...
   IndicatorName = GenerateIndicatorName("EMA RSI STO Signal");
