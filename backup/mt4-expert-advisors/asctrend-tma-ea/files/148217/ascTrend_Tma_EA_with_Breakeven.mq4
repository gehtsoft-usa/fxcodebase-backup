...
string AscTrend_file = "asc-trend.ex4";
string TMA_file = "TMA+CG.ex4";
...
// NOTE: Indi setups
// ------------------------------------------------------------------
input string Tindi1          = "== Asc Trend Setup ==";  // == Asc Trend Setup ==
input int    uRisk           = 2;                        // RISK:
input int    uBars           = 300;                      // CountBars:
input string Tindi2          = "== TMA Setup ==";        // == TMA Setup ==
...
class Asc
{
 public:
  ...
  {
    return iCustom(NULL, 0, AscTrend_file, uRisk, uBars, buffer, shift);
  }
  ...
  int currentSide()
  {
    int lastBuy  = (int)lastValue(1, true);
    int lastSell = (int)lastValue(0, true);
...
input int          magico         = 2022610;            // Magic Number:
...
class BUYcondition2 : public iConditions
{
 public:
  bool evaluate()
  {
    ...
    double redCross = iCustom(NULL, 0, TMA_file, TimeFrame, HalfLength, Price, BandsDeviations, Interpolate, 3, 1);
    return (redCross > 0 && redCross != EMPTY_VALUE);
 ...
 public:
  bool evaluate()
  {
    // TODO: condition sell 2
		double blueCross = iCustom(NULL, 0, TMA_file, TimeFrame, HalfLength, Price, BandsDeviations, Interpolate, 4, 1);
    return (blueCross > 0 && blueCross != EMPTY_VALUE);
  }
};
...
int OnInit()
{
  ...
  double temp1 = iCustom(NULL, 0, AscTrend_file, 0, 0);
  if (GetLastError() == ERR_INDICATOR_CANNOT_LOAD)
  {
    Alert("Please, install the: " + AscTrend_file + " indicator");
    return INIT_FAILED;
  }

  double temp2 = iCustom(NULL, 0, TMA_file, 0, 0);
  if (GetLastError() == ERR_INDICATOR_CANNOT_LOAD)
  {
    Alert("Please, install the: " + TMA_file + " indicator");
    return INIT_FAILED;
  }
#endif
