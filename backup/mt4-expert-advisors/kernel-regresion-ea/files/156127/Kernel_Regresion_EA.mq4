...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=75061
...
input const string indicator_file = "Kernel_Regression"; // Indicator File:
//---
int  bufferToBuy    = 0;
int  bufferToSell   = 1;
bool indicator_init = true;
input int candles_back_for_signals = 10; // Candles back singals:
//---
  ...
  public:
    bool evaluate()
    {
        for (int i = 1; i <= candles_back_for_signals; i++) {
            ...
            double indi_prev = Indi(bufferToSell, i-1);
            if (indi > 0 && indi != EMPTY_VALUE && indi != lastSignal && indi_prev == EMPTY_VALUE) {
                lastSignal = indi;
                return true;
            }
        }
  ...
  public:    
    bool evaluate()
    {
        for (int i = 1; i <= candles_back_for_signals; i++) {
            ...
            double buSell_curr = Indi(bufferToSell, i);
            double buSell_prev = Indi(bufferToSell, i-1);

            // if ((indi > 0 && indi != EMPTY_VALUE && indi != lastSignal) && (buSell_prev > 0 && buSell_prev != EMPTY_VALUE) && buSell_curr==EMPTY_VALUE)
            if (indi > 0 && indi != EMPTY_VALUE && indi != lastSignal && buSell_prev != EMPTY_VALUE && buSell_curr == EMPTY_VALUE)
            {
                lastSignal = indi;
                return true;
            }
