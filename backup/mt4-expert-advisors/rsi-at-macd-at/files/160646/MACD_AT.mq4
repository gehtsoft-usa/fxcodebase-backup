...
double macd(int i) { return iMACD(NULL, 0, macd_fast_periods, macd_slow_periods, macd_signal_periods, applied_price, 0, i); }
double macd_signal(int i) { return iMACD(NULL, 0, macd_fast_periods, macd_slow_periods, macd_signal_periods, applied_price, 1, i); }

bool MACD_Venta(int i)
{
    double macd1 = macd(i + 1);
    double macd2 = macd(i + 2);
    double macd3 = macd(i + 3);

    return macd1 < macd2 && macd2 > macd3;
}
bool MACD_Compra(int i)
{
    double macd1 = macd(i + 1);
    double macd2 = macd(i + 2);
    double macd3 = macd(i + 3);
...
int OnCalculate(const int rates_total, const int prev_calculated, const datetime &time[], const double &open[], const double &high[], const double &low[], const double &close[], const long &tick_volume[], const long &volume[], const int &spread[])
{
    ...
    // clang-format off
    if (prev_calculated == 0) { start = rates_total - macd_slow_periods+1; } else { start = rates_total - (prev_calculated - 1); }
    // clang-format on

    for (i = start; i >= 0; i--) {
        MACD[i] = iMACD(NULL, 0, macd_fast_periods, macd_slow_periods, macd_signal_periods, applied_price, 0, i);
