                ...
                const int&      spread[])
{
  ...
  for (i = start; i > 0; i--){
      if (btf != PERIOD_CURRENT)
      {
         ...
         if (btfPos != btfPosPrev)
         {
            ArrowUp[i] = iCustom(_Symbol, btf, "Indicador_TV_to_MT4", m, atrPeriods, h, 0, btfPos);
            ArrowDn[i] = iCustom(_Symbol, btf, "Indicador_TV_to_MT4", m, atrPeriods, h, 1, btfPos);
         }
         btfPosPrev = btfPos;
         xATRTrailingStop[i] = iCustom(_Symbol, btf, "Indicador_TV_to_MT4", m, atrPeriods, h, 2, btfPos);
         continue;
      }
