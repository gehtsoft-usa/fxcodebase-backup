...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=71933
...
enum ModeLevels {
   ...
   PipsFromOpenCandle,  // Pips from Candle
	ByIndicator // From Ultimate Pro Scalper
};
enum TSLMode {
   byPips,  // By Pips
...
input string IWave = "== Ultimate Pro Scalper Setup ==";  // == Ultimate Pro Scalper Setup ==
//--------------------------------------------------------------------
input int  PeriodValue = 13;
input int  SL_pips     = 148;
input int  TP_pips     = 188;
...
//--------------------------------------------------------------------
class UltimateProScalper : public iIndicators
{
   string _symbol;
   int    _tf;
   string _file;
  ...
  public:
   UltimateProScalper()
   {
      _symbol = _Symbol;
      _tf     = Period();
      _file   = "UltimateProScalper.ex4";
   }
   UltimateProScalper(string Symbol, int TimeFrame)
   {
      _symbol = Symbol;
      _tf     = TimeFrame;
      _file   = "UltimateProScalper.ex4";
   ...
   UltimateProScalper* file(string setfile)
   {
      _file = setfile;
      return &this;
   }
   ...
   // buffer 4 _ buy signal (-1 no signal) dibuja el rombo cuando pasa de -1 a un valor
   // buffer 5 _ sell signal (-1 no signal) dibuja el rombo cuando pasa de -1 a un valor
   // buffer 6 SL
   // buffer 7 TP

   int actualSide()
   {
      int lastBuy  = (int)lastValue(4, true);
      int lastSell = (int)lastValue(5, true);
      if (lastBuy < lastSell)
      {
         return 0;  // BUY
      } else
   ...
   int side(int shift)
   {
      if (calculate(shift, 4) > -1)
      {
         return 0;
      } 
		
		if (calculate(shift, 5) > -1)
      {
         return 1;
      }
   ...
   double SL()
   {
      return calculate(1, 6);
   }
   double TP()
   {
      return calculate(1, 7);		
   }

};
// ------------------------------------------------------------------
UltimateProScalper ultimateScalper();
// ------------------------------------------------------------------
interface IOrders {
  public:
   virtual void Add()     = 0;
  ...
  public:
   bool evaluate()
   {
      // TODO: condition Buy 1
		if(ultimateScalper.side(1) ==0 && ultimateScalper.side(2) ==1)
		{
			return true;
		}
      return false;
  ...
  public:
   bool evaluate()
   {
      // TODO: condition sell 1
		if(ultimateScalper.side(1) ==1 && ultimateScalper.side(2) ==0)
		{
			return true;
		}
      return false;
