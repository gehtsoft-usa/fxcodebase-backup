...
int start()
{
   ...
   string propability = DoubleToString((ups[0] / bars_limit) * 100.0, 0) + "%";
   if (ObjectFind(0, id) == -1)
   {
      if (!ObjectCreate(0, id, OBJ_TEXT, 0, Time[0], Close[0]))
      {
         Print(__FUNCTION__, ". Error: ", GetLastError());
         return 0;
      }
   ...
   ObjectSetDouble(0, id, OBJPROP_PRICE1, Close[0]);
