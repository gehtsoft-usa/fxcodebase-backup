...
input CandleType Type = CandleOC; // Candle Type
input int Period = 14; // MA Period
input int Trigger = 2; // Trigger Level
...
public:
   ...
   {
      return sot._data[period] > avg[period] * Trigger && iClose(_symbol, _timeframe, period) > iOpen(_symbol, _timeframe, period);
   }
};
...
void OnInit()
{
   IndicatorObjPrefix = GenerateIndicatorPrefix("sot");
   IndicatorSetString(INDICATOR_SHORTNAME, "Speed of Trade");
   IndicatorSetInteger(INDICATOR_DIGITS, Digits());

   ENUM_TIMEFRAMES timeframe = (ENUM_TIMEFRAMES)_Period;
   sot = new IndicatorOutputStream(_Symbol, timeframe);
   int id = 0;
   id = sot.RegisterStream(id, Red, "SoT");
   ...
   id = up.RegisterStreams(id, "Up", 217, up_color, sot);
   id = down.RegisterStreams(id, "Down", 218, down_color, sot);
}
                ...
                const int &spread[])
{
   ...
   for (int pos = MathMax(first, prev_calculated - 1); pos < rates_total; ++pos)
   {
      ...
      if (Type == CandleOC)
      {
         sot._data[pos] = ((MathAbs(open[pos] - close[pos])) / pipSize) / tick_volume[pos]; 
      }
      else
      {
         sot._data[pos] = ((MathAbs(high[pos] - low[pos])) / pipSize) / volume[pos];
      }
   }
   SimpleMAOnBuffer(rates_total, prev_calculated, 0, Period, sot._data, avg);
   for (int pos = MathMax(first, prev_calculated - 1); pos < rates_total; ++pos)
   {
