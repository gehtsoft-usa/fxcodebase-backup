...
int GetNext(int period)
{
   for (int i = period + 1; i < iBars(_Symbol, (ENUM_TIMEFRAMES)_Period); ++i)
   {
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      int first = GetNext(period);
      if (first == -1)
      {
         return false;
      }
      int second = GetNext(period);
      if (second == -1)
      {
         return false;
      }
      if (ZigzagBuffer[first] < ZigzagBuffer[second])
      {
         return false;
      }
      return true;
...
int init()
{
   visibility.Init("hhlh", "hhlh", "Show/Hide", button_x, button_y);
   ...
   IndicatorShortName("ZuluFX HHLHLLHL2");
...
int start()
{
   ...
   for (int pos = limit; pos >= 0 && !IsStopped(); --pos)
   {
      if (customStream != NULL)
      {
         customStream.SetValue(pos, ZigzagBuffer[pos]);
      }
...
void DrawHighLabel(int shift)
{
   ...
   for (int i = shift + 1; i < MathMin(Bars, shift + 1000); i++)
   {
