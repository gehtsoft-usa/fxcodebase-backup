...
//http://fxcodebase.com/code/viewtopic.php?f=38&t=70379
...
#property indicator_color1 DeepSkyBlue, Tomato  // up[]
#property indicator_width1 2

#define ACT_ON_SWITCH

input int Amplitude = 2; // Period
...
double up[], upColor[], atrlo[], atrhi[], trend[];
int lowma, highma, atr;

int OnInit(void)
{
   IndicatorObjPrefix = GenerateIndicatorPrefix("htwa");
   IndicatorSetString(INDICATOR_SHORTNAME, "HalfTrend with Alert");
   ...
   mainSignaler.SetMessagePrefix(_Symbol + "/" + mainSignaler.GetTimeframeStr() + ": ");
   {
      ...
      id = alerts[size].RegisterStreams(id, 1, "Up", 217, up_color, highStream);
      id = alerts[size + 1].RegisterStreams(id, 1, "Down", 218, down_color, lowStream);
   }
   lowStream.Release();
   highStream.Release();
   ...
   SetIndexBuffer(id++, atrlo, INDICATOR_CALCULATIONS);
   SetIndexBuffer(id++, atrhi, INDICATOR_CALCULATIONS);

   lowma = iMA(_Symbol, _Period, Amplitude, 0, MODE_SMA, PRICE_LOW);
   highma = iMA(_Symbol, _Period, Amplitude, 0, MODE_SMA, PRICE_HIGH);
...
{
   IndicatorRelease(lowma);
   IndicatorRelease(highma);
...
)
{
   ...
   for (int pos = MathMax(first, prev_calculated - 1); pos < rates_total; ++pos)
   {
      ...
      double lowmaValue[1];
      if (CopyBuffer(lowma, 0, oldPos, 1, lowmaValue) != 1)
      {
         continue;
      }
      double highmaValue[1];
      if (CopyBuffer(highma, 0, oldPos, 1, highmaValue) != 1)
      {
         continue;
      }
      double atrValue[1];
      ...
      int highestIndex = ArrayMaximum(high, pos - Amplitude, Amplitude);
      double highprice_i = high[highestIndex];
      int lowestIndex = ArrayMaximum(low, pos - Amplitude, Amplitude);
      double lowprice_i = low[lowestIndex];
      if (trend[pos - 1] == 0)
      {
         maxlowprice = MathMax(lowprice_i, maxlowprice);
         if (highmaValue[0] < maxlowprice && close[pos] < low[pos - 1])
         {
            trend[pos] = 1.0;
      ...
      if (trend[pos - 1] == 1)
      {
         ...
         if (lowmaValue[0] > minhighprice && close[pos] > high[pos - 1])
         {
            trend[pos] = 0.0;
      ...
      if (trend[pos] == 0.0)
      {
         up[pos] = MathMax(maxlowprice, up[pos - 1]);
         upColor[pos] = 0;
         atrhi[pos] = up[pos] - atrValue[0];
         atrlo[pos] = up[pos];
      }
      else
      {
         up[pos] = MathMin(minhighprice, up[pos - 1]);
         upColor[pos] = 1;
         atrhi[pos] = up[pos] + atrValue[0];
         atrlo[pos] = up[pos];
      }
