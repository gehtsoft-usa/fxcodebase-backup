   ...
   if (tipo == Both || tipo == Buy) {
      if (HabilitadoParaIniciar) {
         if (Ask >= PrecioBuy) {
            ...
            //mando la orden
            int tk = OrderSend(Symbol(), OP_BUY, LoteAuto, Ask, 25, 0, TPauto, NULL, Magico, 0, clrNONE);
            // HabilitarCiclo();
   ...
   if (tipo == Both || tipo == Sell) {
      if (HabilitadoParaIniciar) {
         if (Bid <= PrecioSell) {
            ...
            //mando la orden
            int tk = OrderSend(Symbol(), OP_SELL, LoteAuto, Bid, 25, 0, TPauto, NULL, Magico, 0, clrNONE); 
            // HabilitarCiclo();
            if (tk > 0) HabilitadoParaIniciar = false;
            bool sel = OrderSelect(tk, SELECT_BY_TICKET);
            //calcular precios de coberturas:
      ...
      for (int i = OrdersTotal() - 1; i >= 0; i--) {
         if (OrderSelect(i, SELECT_BY_POS, MODE_TRADES) && OrderSymbol() == Symbol()&& OrderMagicNumber() == Magico) {
            ...
            if (OrderType() == OP_BUY) {
               ...
               if (!OrderSend(Symbol(), OP_SELLSTOP, OrderLots() * multiplicadorLotaje, PrecioCobVenta, 50, 0, TPcob, "cobertura", Magico, 0, clrNONE)) {}
               break;
            }
            if (OrderType() == OP_SELL) {
               double TPcob = PrecioCobCompra + (pipTP * 10 * _Point);
               if (!OrderSend(Symbol(), OP_BUYSTOP, OrderLots() * multiplicadorLotaje, PrecioCobCompra, 50, 0, TPcob, "cobertura", Magico, 0, clrNONE)) {}
               break;
            }
         }
      }
