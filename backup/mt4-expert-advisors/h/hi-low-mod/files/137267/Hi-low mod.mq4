...
// http://fxcodebase.com/code/viewtopic.php?f=38&t=70370
...
public:
   ...
   {
      return down[period] != EMPTY_VALUE;
   }
};
...
int init()
{
   ...
   IndicatorObjPrefix = GenerateIndicatorPrefix("hlm");
   IndicatorShortName("Hi-Low mod");
                ...
                const int &spread[])
{
   ...
   for (int pos = MathMin(bars_limit, rates_total - 1 - MathMax(prev_calculated, toSkip)); pos >= 0 && !IsStopped(); --pos)
   {
      ...
      double h = iMA(_Symbol, tf, inpPeriod, 1, MODE_SMA, PRICE_HIGH, index);
      double l = iMA(_Symbol, tf, inpPeriod, 1, MODE_SMA, PRICE_LOW, index);
      state[pos] = state[pos + 1];
      if (close[pos] < h)
         state[pos] = -1;
      if (close[pos] > l)
         state[pos] = 1;
      up[pos] = (state[pos] == -1) ? h + 2 * _Point : EMPTY_VALUE;
      down[pos] = (state[pos] == 1) ? l - 2 * _Point : EMPTY_VALUE;
      if (customStream != NULL)
      {
