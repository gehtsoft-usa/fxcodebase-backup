...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=74766
...
class Line
{
  ...
  public:
    Line(string Name="line", datetime IniTm = 0, datetime EndTm = 0, double Price = 0, color Clr = Black, string Txt = "")
    {
        _name  = Name;
        _iniTm = IniTm;
        _endTm = EndTm;
...
CCandle candle3(_Symbol, PERIOD_CURRENT);
CCandle candle2(_Symbol, PERIOD_CURRENT);
CCandle candle1(_Symbol, PERIOD_CURRENT);
...
class BUYcondition1 : public iConditions
{
  public:
    bool evaluate()
    {
        ...
        // sino hay patron alcista buscarlo:
        // pat: dos velas alcistas y una bajista:
        if(signal == "buy" && signalPrice >0) return true;


        candle3.setCandle(3);
        candle2.setCandle(2);
        candle1.setCandle(1);
    if(candle3.Direction()=="up" && candle2.Direction()=="up" && candle1.Direction()=="down" && candle1.Size()>100)
    {
        // buscar el high del pattern
        int c;
        if(candle3.High()>candle2.High() && candle3.High() > candle1.High()) {signalPrice =candle3.High(); c= 3;}
        if(candle2.High()>candle3.High() && candle2.High() > candle1.High()) {signalPrice =candle2.High(); c= 2;}
        if(candle1.High()>candle3.High() && candle1.High() > candle2.High()) {signalPrice =candle1.High(); c= 1;}

        signal = "buy";
        datetime tm = iTime(NULL, 0,1);
        line.price(signalPrice).iniTime(tm).width(2).clr(Blue).redraw();
        line.price(signalPrice).fromCandle(c).width(2).clr(Blue).redraw();
        return true;
    }
...
class BUYcondition2 : public iConditions
{
  public:
    bool evaluate()
    {
        ...
        if(cl > signalPrice && op < signalPrice) return true;
        return false;
    }
};
...
class SELLcondition1 : public iConditions
{
  public:
    bool evaluate()
    {
        ...
        // sino hay patron alcista buscarlo:
        // pat: dos velas alcistas y una bajista:
        if(signal == "sell" && signalPrice >0) return true;


        candle3.setCandle(3);
        candle2.setCandle(2);
        candle1.setCandle(1);
    if(candle3.Direction()=="down" && candle2.Direction()=="down" && candle1.Direction()=="up"&& candle1.Size()>100)
    {
        int c;
        if(candle3.Low()<candle2.Low() && candle3.Low() < candle1.Low()) {signalPrice =candle3.Low();c=3;}
        if(candle2.Low()<candle3.Low() && candle2.Low() < candle1.Low()) {signalPrice =candle2.Low();c=2;}
        if(candle1.Low()<candle3.Low() && candle1.Low() < candle2.Low()) {signalPrice =candle1.Low();c=1;}

        signal = "sell";
        datetime tm = iTime(NULL, 0,1);
        // line.price(signalPrice).iniTime(tm).width(2).clr(Red).redraw();
        line.price(signalPrice).fromCandle(c).width(2).clr(Red).redraw();
        return true;
    }
