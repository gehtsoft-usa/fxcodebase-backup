...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=73609
...
string file_custom_indicator = "heiken-ashi-smoothedLine.ex5";
...
input bool emaOn = true; // MA Filter On?
input int                maFast_Period = 50;                                   // Period:
input int                maFast_Shift = 0;                                    // Ma Shift:
...
input string             TcloseOptions     = "== Close by Oposite ==";  // == Oposite Setup ==
...
class BUYcondition1 : public iConditions
{
 public:
  bool evaluate()
  {
    ...
    double indiPrev = index(heiken, 1, 2);
    double indi = index(heiken, 1, 1);
    return indiPrev==1 && indi==0;
  }
};
...
class BUYcondition2 : public iConditions
{
 public:
  ...
  {
      // TODO: condition Buy2
      if (!emaOn)return true;
      
      if (iClose(NULL, 0, 1) > emaFast.index(1)) return true;
    return false;
  }
};
...
class ConditionToCloseBuy : public iConditions
{
 public:
  bool evaluate()
  {
    if (closeAllInOpositeSignal) { if(sellCondition1.evaluate())return true; }
    if (closeAllControlON)       { if(CloseALlControl())return true;         }
    return false;
  }
};
...
class ConditionToCloseSell : public iConditions
{
 public:
  bool evaluate()
  {
    if (closeAllInOpositeSignal) { if(buyCondition1.evaluate()) return true; }
    if (closeAllControlON)       { if(CloseALlControl())return true;         }    
		return false;
  }
};
...
int OnInit()
{
  ...
  heiken = iCustom(NULL, 0, file_custom_indicator, inpMaPeriod, inpMaMetod, inpStep,inpBetterFormula);
  if (GetLastError() == ERR_INDICATOR_CANNOT_CREATE) {
    Alert("Please, install the: " + file_custom_indicator + " indicator, into the folder MQL5/Indicators");
    return INIT_FAILED;
  }
#endif
