...
input int                inpDivisor            = 2;                        // Divisor ("speed")
input ENUM_APPLIED_PRICE inpPrice              = PRICE_CLOSE;              // Price
input int                inpLookBack           = 2;                        // Sensibility
...
class CustomConditionBUY : public iConditions
{
  public:
   bool evaluate()
   {
      if (iCustom(Symbol(), Period(), "HullMovingAverage.ex4", inpPeriod,inpDivisor,inpPrice,inpLookBack, 1, 1) > 0)
      {
         return true;
      }
      return false;
...
class CustomConditionBUY2 : public iConditions
{
  public:
   ...
   {
      // TODO: dev condition
      //   no tiene que abrir 2 ordenes en la misma vela
      if (OrderSelect(OrdersTotal() - 1, SELECT_BY_POS, MODE_TRADES) && OrderMagicNumber() == magico)
      {
         if (OrderOpenTime() >= iTime(_Symbol, _Period, 0))
         {
            return false;
         }
      }
...
class CustomConditionSELL : public iConditions
{
  public:
   bool evaluate()
   {
      if (iCustom(Symbol(), Period(), "HullMovingAverage.ex4", inpPeriod,inpDivisor,inpPrice,inpLookBack, 2, 1) > 0)
      {
         return true;
      }
...
CustomConditionSELL2* countSells;

int OnInit()
{
   newCandle = new CNewCandle();
   conditionsToBuy.AddCondition(entryBuy = new CustomConditionBUY());
   conditionsToBuy.AddCondition(countBuys = new CustomConditionBUY2());
   conditionsToSell.AddCondition(entrySell = new CustomConditionSELL());
   conditionsToSell.AddCondition(countSells = new CustomConditionSELL2());
...
bool CalculateIndicators()
{
   ...
   double buySignal = iCustom(_Symbol, _Period, "HullMovingAverage.ex4", 20, 2.0, 2, 21, 1, 0);
   if (buySignal != 0)
   {
      Print(__FUNCTION__, " ", "buySignal", " ", buySignal);
   }

   double sellSignal = iCustom(_Symbol, _Period, "HullMovingAverage.ex4", 20, 2.0, 2, 21, 2, 0);
   if (sellSignal != 0)
   {
