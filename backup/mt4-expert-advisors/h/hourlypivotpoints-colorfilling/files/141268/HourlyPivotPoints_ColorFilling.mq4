...
//http://fxcodebase.com/code/viewtopic.php?f=38&t=71032
...
// based on Dean Feng, ayhomer@outlook.com"
#property strict
...
input int os_level = -20; // Oversold level
input int ob_level = -80; // Overbought level
input int wpr_periods = 14; // R% periods
...
int CreateAlert(int id, ENUM_TIMEFRAMES tf, color upColor, color downColor)
{
   ...
   upCondition.Add(new WPRUpCondition(_Symbol, tf), false);
   ...
   downCondition.Add(new WPRDownCondition(_Symbol, tf), false);
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      return iClose(_symbol, _timeframe, period) > R3;
   }
};
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      double wprValue = iWPR(_symbol, _timeframe, wpr_periods, period);
      return wprValue > os_level;
   }
};
...
int OnInit()
{
   ...
   IndicatorBuffers(8);
   IndicatorObjPrefix = GenerateIndicatorPrefix("hppvf");
   IndicatorShortName("HourlyPivotPoints_ColorFilling");
