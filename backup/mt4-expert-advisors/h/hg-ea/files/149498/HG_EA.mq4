...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=73337
...
string file_custom_indicator = "HG.ex4";
...
input double Set1=3.0;
input int    Set2=2;
input int    Set3=8;
bool         Alertformessage=false;
bool         Alertformobile=false;
bool         Alertforemail=false;
int          arrowSize                = 2;
...
input ModeCalcLots modeCalcLots   = LotsbyRange;        // Mode to Calc Lots:
input double       uRange         = 500;                // Range:
...
class BUYcondition1 : public iConditions
{
 public:
  bool evaluate()
  {
    ...
    return iCustom(NULL, 0, file_custom_indicator, Set1, Set2, Set3, Alertformessage, Alertformobile, Alertforemail, arrowSize,0, 1) != EMPTY_VALUE;
    // return false;
  }
};
 ...
 public:
  bool evaluate()
  {
    // TODO: condition sell 1
		return iCustom(NULL, 0, file_custom_indicator, Set1, Set2, Set3, Alertformessage, Alertformobile, Alertforemail, arrowSize, 1, 1) != EMPTY_VALUE;
  }
};
...
double Lots()
{
  ...
  switch (modeCalcLots)
  {
    case Money: lots = lotProvider.LotsByMoney(userMoney, levelSL.pips()); break;
    case AccountPercent: lots = lotProvider.LotsByBalancePercent(userBalancePer, levelSL.pips()); break;
    case FixLots: lots = userLots; break;
    case LotsbyRange: lots = NormalizeDouble(userLots * AccountInfoDouble(ACCOUNT_BALANCE)/uRange,2) ; break;
  }
	
	#ifdef MARTINGALE
	if(martingaleOn)
	if(martin.lastWasLoss())
	{
		// TODO: implementar la martingale
		// agarra el ultimo lotaje y lo multiplica
    double lastLot;
    for (int i = OrdersHistoryTotal() - 1; i >= 0; i--)
    {
        if (!OrderSelect(i, SELECT_BY_POS, MODE_HISTORY) && OrderSymbol() == Symbol() && OrderMagicNumber() == magico) continue;
        lastLot = OrderLots();
        break;
    }
	    double lotsm = NormalizeDouble(lastLot * Martingale_Multiplier, 2);
    	lots = lotsm >= Martingale_MaxLots ? Martingale_MaxLots : lotsm;
    }
		#endif
