...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=73810
...
// Gobal Variables
//+------------------------------------------------------------------+
input string tmode = "==== Mode ====";
input Modalidad   Modo = SemiAutomatico;  //Mode:
input string note = "Automatic Mode: open the first trade based on moving average breakout"; // Note:
input string _note = "Mannual Mode: the first trade should be putted mannually "; // Note:
input string _note2 = "and you must to put a Take Profit and the EA will use this to make the hedge trade "; // Note:
input CompraVenta tipo = Ambos;           // First Operation in Automatic mode:
input int         MM = 30;                // Moving Average for Automatic mode:
input double      LoteAuto = 0.10;        // Initial lot in Automatic mode:
input int         pipAuto = 60;           // Pips Take Profit Automatic mode:
input string      s1 = "==== Time Control ====";       //******
input bool        ControlaHora = false;    // Timer Control On?:
input int         HoraIni = 0000;         // Start Time (hhmm) =
input int         HoraFin = 0000;         // End Time (hhmm) =
// extern string     HoraIni = "00:00";        //Hora Inicio (formato hh:mm) =
// extern string     HoraFin = "00:00";        //Hora fin (formato hh:mm) =
input int AjusteHora;  // Diference to server time (+/- Hrs):
...
void OnTick()
{
    if(ControlHorario()) {
        ...
        Comment("Fuera de Horario / EA Apagado");
    }
...
void CloseAll()
{
    for(int i = OrdersTotal() - 1; i >= 0; i--) {
        if(OrderSelect(i, SELECT_BY_POS, MODE_TRADES) && OrderSymbol() == Symbol()) {
            if(OrderType() != OP_BUY && OrderType() != OP_SELL) { if(OrderDelete(OrderTicket())){} }
            if(OrderType() == OP_BUY) { if(OrderClose(OrderTicket(), OrderLots(), Bid, 30, clrNONE)){} }
            if(OrderType() == OP_SELL) { if(OrderClose(OrderTicket(), OrderLots(), Ask, 30, clrNONE)){} }
        }
    }
