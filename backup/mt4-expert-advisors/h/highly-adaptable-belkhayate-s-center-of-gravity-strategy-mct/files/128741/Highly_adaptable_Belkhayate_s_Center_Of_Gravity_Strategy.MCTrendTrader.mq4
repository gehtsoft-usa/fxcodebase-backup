...
// http://fxcodebase.com/code/viewtopic.php?f=38&t=68930
...
#ifndef CustomAction_IMP
#define CustomAction_IMP

enum CustomAction
{
   CustomActionNo, // No action
   CustomActionLong, // Long
   CustomActionShort, // Short
   CustomActionCloseLong, // Close Long
   CustomActionCloseShort // Close Short
};
#endif

input string ZZSection = ""; // == 3-rd ZZ Semafor ==
...
input string BelcogSection = ""; // == Belcog ==
input int N = 180; // Number of bars
input int O = 3; // Order
input double E = 1.61803399; // Eccart value
input string ActionsSection = ""; // == Actions ==
input CustomAction L1LineCrossOver = CustomActionNo; // L1 Line Cross Over
input CustomAction L1LineCrossUnder = CustomActionNo; // L1 Line Cross Under
input CustomAction L2LineCrossOver = CustomActionNo; // L2 Line Cross Over
input CustomAction L2LineCrossUnder = CustomActionNo; // L2 Line Cross Under
input CustomAction L3LineCrossOver = CustomActionNo; // L3 Line Cross Over
input CustomAction L3LineCrossUnder = CustomActionNo; // L3 Line Cross Under
input CustomAction L4LineCrossOver = CustomActionNo; // L4 Line Cross Over
input CustomAction L4LineCrossUnder = CustomActionNo; // L4 Line Cross Under
input CustomAction L5LineCrossOver = CustomActionNo; // L5 Line Cross Over
input CustomAction L5LineCrossUnder = CustomActionNo; // L5 Line Cross Under
input CustomAction L6LineCrossOver = CustomActionNo; // L6 Line Cross Over
input CustomAction L6LineCrossUnder = CustomActionNo; // L6 Line Cross Under
input CustomAction L7LineCrossOver = CustomActionNo; // L7 Line Cross Over
input CustomAction L7LineCrossUnder = CustomActionNo; // L7 Line Cross Under
...
class LineCrossOverCondition : public ABaseCondition
{
   int _stream;
public:
   LineCrossOverCondition(string symbol, ENUM_TIMEFRAMES timeframe, int stream)
   ...
   virtual bool IsPass(const int period)
   {
      double belcog0 = iCustom(_symbol, _timeframe, "BELCOG", N, O, E, _stream, period);
      double belcog1 = iCustom(_symbol, _timeframe, "BELCOG", N, O, E, _stream, period + 1);
      double close0 = iClose(_symbol, _timeframe, period);
      double close1 = iClose(_symbol, _timeframe, period + 1);
      return close0 > belcog0 && close1 <= belcog1;
   }
};

class LineCrossUnderCondition : public ABaseCondition
{
   int _stream;
public:
   LineCrossUnderCondition(string symbol, ENUM_TIMEFRAMES timeframe, int stream)
      :ABaseCondition(symbol, timeframe)
   {
      _stream = stream;
   }
...
class LastZZSignalCondition : public ABaseCondition
{
   int _signalStream;
   int _oppositeSignalStream;
public:
   LastZZSignalCondition(string symbol, ENUM_TIMEFRAMES timeframe, int signalStream, int oppositeSignalStream)
      : ABaseCondition(symbol, timeframe)
   {
      _signalStream = signalStream;
      _oppositeSignalStream = oppositeSignalStream;
   }
   ...
   virtual bool IsPass(const int period)
   {
      ...
      {
         double upValue = iCustom(_symbol, _timeframe, "Semaphor_3Level_ZZ", Period_1, Period_2, Period_3, Deviation_1, Step_1, Deviation_2, Step_2, Deviation_3, Step_3, _signalStream, period + i);
         double downValue = iCustom(_symbol, _timeframe, "Semaphor_3Level_ZZ", Period_1, Period_2, Period_3, Deviation_1, Step_1, Deviation_2, Step_2, Deviation_3, Step_3, _oppositeSignalStream, period + i);
...
ICondition* CreateL1OverCondition(string symbol, ENUM_TIMEFRAMES timeframe)
{
   return new LineCrossOverCondition(symbol, timeframe, 0);
}

ICondition* CreateL1UnderCondition(string symbol, ENUM_TIMEFRAMES timeframe)
{
   return new LineCrossUnderCondition(symbol, timeframe, 0);
}
...
ICondition* CreateL2OverCondition(string symbol, ENUM_TIMEFRAMES timeframe)
{
   AndCondition* and = new AndCondition();
   and.Add(new LineCrossOverCondition(symbol, timeframe, 1));
   and.Add(new LastZZSignalCondition(symbol, timeframe, 4, 5));
   return and;
}

ICondition* CreateL2UnderCondition(string symbol, ENUM_TIMEFRAMES timeframe)
{
   AndCondition* and = new AndCondition();
   and.Add(new LineCrossUnderCondition(symbol, timeframe, 1));
   and.Add(new LastZZSignalCondition(symbol, timeframe, 4, 5));
   return and;
}

ICondition* CreateL3OverCondition(string symbol, ENUM_TIMEFRAMES timeframe)
{
   AndCondition* and = new AndCondition();
   and.Add(new LineCrossOverCondition(symbol, timeframe, 2));
   and.Add(new LastZZSignalCondition(symbol, timeframe, 4, 5));
   return and;
}

ICondition* CreateL3UnderCondition(string symbol, ENUM_TIMEFRAMES timeframe)
{
   AndCondition* and = new AndCondition();
   and.Add(new LineCrossUnderCondition(symbol, timeframe, 2));
   and.Add(new LastZZSignalCondition(symbol, timeframe, 4, 5));
   return and;
}

ICondition* CreateL4OverCondition(string symbol, ENUM_TIMEFRAMES timeframe)
{
   AndCondition* and = new AndCondition();
   and.Add(new LineCrossOverCondition(symbol, timeframe, 3));
   and.Add(new LastZZSignalCondition(symbol, timeframe, 4, 5));
   return and;
}

ICondition* CreateL4UnderCondition(string symbol, ENUM_TIMEFRAMES timeframe)
{
   AndCondition* and = new AndCondition();
   and.Add(new LineCrossUnderCondition(symbol, timeframe, 3));
   and.Add(new LastZZSignalCondition(symbol, timeframe, 4, 5));
   return and;
}

ICondition* CreateL5OverCondition(string symbol, ENUM_TIMEFRAMES timeframe)
{
   AndCondition* and = new AndCondition();
   and.Add(new LineCrossOverCondition(symbol, timeframe, 4));
   and.Add(new LastZZSignalCondition(symbol, timeframe, 5, 4));
   return and;
}

ICondition* CreateL5UnderCondition(string symbol, ENUM_TIMEFRAMES timeframe)
{
   AndCondition* and = new AndCondition();
   and.Add(new LineCrossUnderCondition(symbol, timeframe, 4));
   and.Add(new LastZZSignalCondition(symbol, timeframe, 5, 4));
   return and;
}

ICondition* CreateL6OverCondition(string symbol, ENUM_TIMEFRAMES timeframe)
{
   AndCondition* and = new AndCondition();
   and.Add(new LineCrossOverCondition(symbol, timeframe, 5));
   and.Add(new LastZZSignalCondition(symbol, timeframe, 5, 4));
   return and;
}

ICondition* CreateL6UnderCondition(string symbol, ENUM_TIMEFRAMES timeframe)
{
   AndCondition* and = new AndCondition();
   and.Add(new LineCrossUnderCondition(symbol, timeframe, 5));
   and.Add(new LastZZSignalCondition(symbol, timeframe, 5, 4));
   return and;
}

ICondition* CreateL7OverCondition(string symbol, ENUM_TIMEFRAMES timeframe)
{
   AndCondition* and = new AndCondition();
   and.Add(new LineCrossOverCondition(symbol, timeframe, 6));
   and.Add(new LastZZSignalCondition(symbol, timeframe, 5, 4));
   return and;
}

ICondition* CreateL7UnderCondition(string symbol, ENUM_TIMEFRAMES timeframe)
{
   AndCondition* and = new AndCondition();
   and.Add(new LineCrossUnderCondition(symbol, timeframe, 6));
   and.Add(new LastZZSignalCondition(symbol, timeframe, 5, 4));
   return and;
}

ICondition* CreateCondition(string symbol, ENUM_TIMEFRAMES timeframe, CustomAction action)
{
   OrCondition* condition = new OrCondition();
   if (L1LineCrossOver == action)
      condition.Add(CreateL1OverCondition(symbol, timeframe));
   if (L1LineCrossUnder == action)
      condition.Add(CreateL1UnderCondition(symbol, timeframe));
   if (L2LineCrossOver == action)
      condition.Add(CreateL2OverCondition(symbol, timeframe));
   if (L2LineCrossUnder == action)
      condition.Add(CreateL2UnderCondition(symbol, timeframe));
   if (L3LineCrossOver == action)
      condition.Add(CreateL3OverCondition(symbol, timeframe));
   if (L3LineCrossUnder == action)
      condition.Add(CreateL3UnderCondition(symbol, timeframe));
   if (L4LineCrossOver == action)
      condition.Add(CreateL4OverCondition(symbol, timeframe));
   if (L4LineCrossUnder == action)
      condition.Add(CreateL4UnderCondition(symbol, timeframe));
   if (L5LineCrossOver == action)
      condition.Add(CreateL5OverCondition(symbol, timeframe));
   if (L5LineCrossUnder == action)
      condition.Add(CreateL5UnderCondition(symbol, timeframe));
   if (L6LineCrossOver == action)
      condition.Add(CreateL6OverCondition(symbol, timeframe));
   if (L6LineCrossUnder == action)
      condition.Add(CreateL6UnderCondition(symbol, timeframe));
   if (L7LineCrossOver == action)
      condition.Add(CreateL7OverCondition(symbol, timeframe));
   if (L7LineCrossUnder == action)
      condition.Add(CreateL7UnderCondition(symbol, timeframe));
   return condition;
}
...
ICondition* CreateLongCondition(string symbol, ENUM_TIMEFRAMES timeframe)
{
   ...
   OrCondition* condition = CreateCondition(symbol, timeframe, CustomActionLong);
...
ICondition* CreateShortCondition(string symbol, ENUM_TIMEFRAMES timeframe)
{
   ...
   OrCondition* condition = CreateCondition(symbol, timeframe, CustomActionShort);
   #ifdef ACT_ON_SWITCH_CONDITION
   return new ActOnSwitchCondition(condition);
   #else
   return (ICondition *)condition;
...
{
   OrCondition* condition = CreateCondition(symbol, timeframe, CustomActionCloseLong);
   #ifdef ACT_ON_SWITCH_CONDITION
   return new ActOnSwitchCondition(condition);
   #else
   return (ICondition *)condition;
...
{
   OrCondition* condition = CreateCondition(symbol, timeframe, CustomActionCloseShort);
   #ifdef ACT_ON_SWITCH_CONDITION
   return new ActOnSwitchCondition(condition);
   #else
   return (ICondition *)condition;
...
int OnInit()
{
   double temp = iCustom(NULL, 0, "BELCOG", 0, 0);
   if (GetLastError() == ERR_INDICATOR_CANNOT_LOAD)
   {
      Alert("Please, install the 'BELCOG' indicator");
      return INIT_FAILED;
   }
   temp = iCustom(NULL, 0, "Semaphor_3Level_ZZ", 0, 0);
   if (GetLastError() == ERR_INDICATOR_CANNOT_LOAD)
   {
