...
// http://fxcodebase.com/code/viewtopic.php?f=38&t=70269
//+------------------------------------------------------------------+
...
input int loopback = 10; // Loopback
...
public:
   ...
   {
      string Flag = GetFlag(period);
      return SignatureUp.Get(Flag) > SignatureDown.Get(Flag);
   }
};
...
int init()
{
   ...
   IndicatorShortName("Candle Direction Predictor");
   ...
   SetIndexStyle(id, DRAW_NONE);
   SetIndexBuffer(id, ups);
   ++id;
   SetIndexStyle(id, DRAW_NONE);
   SetIndexBuffer(id, downs);
   ++id;
...
StringIntMap SignatureUp;
StringIntMap SignatureDown;

string GetFlag(int pos)
{
   string Flag = "";
...
int start()
{
   ...
   last_run = Time[0];
   int minBars = Period + 1;
   int limit = MathMin(Bars - 1 - minBars, Bars - counted_bars - 1);
   for (int pos = limit; pos >= 1; --pos)
   {
      string Flag = GetFlag(pos);
      if (Flag != "")
      {
         if (Close[pos] > Open[pos])
         {
            SignatureUp.Set(Flag, SignatureUp.Get(Flag) + 1);
         }
         else if (Close[pos] < Open[pos])
         {
            SignatureDown.Set(Flag, SignatureDown.Get(Flag) + 1);
         }
      }
      if (customStream != NULL)
      {
      ...
      for (int i = MathMin(Bars - 1 - minBars, pos + loopback); i >= pos; --i)
      {
