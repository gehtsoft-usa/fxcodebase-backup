...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=75144
...
#endif
#ifdef sl_candles_on
    sl_candles, // Low/High "x" candles back
#endif
};
...
input double         uStopLossValue = 20;                // SL value:
...
input const string indicator_file = "Heiken Ashi";               // Indicator File:
input bool onlyFirst = true; // Only First Retracement?
//---
int  bOpen                    = 2;
int  bClose                   = 3;
bool indi_init                = true;
...
class ConditionHeikenAshiBuy : public iConditions
{
    double lastSignal;
  ...
  public:
    bool evaluate()
    {
        for (int i = 1; i <= candles_back_for_signals; i++) {
            double op  = Indi(bOpen, i);
            double cl  = Indi(bClose, i);
            double op2 = Indi(bOpen, i + 1);
            double cl2 = Indi(bClose, i + 1);

            // if (op > 0 && op != EMPTY_VALUE && op != lastSignal) {
            if (cl > op && cl2 < op2) {
                lastSignal = op;
                return true;
            }
...
ConditionHeikenAshiBuy cdHeikenAshiBuy;

class ConditionHeikenAshiSell : public iConditions
{
    double lastSignal;
...
ConditionHeikenAshiSell cdHeikenAshiSell;

//
//
//

input string       Twcustom        = "==== Indicator Setup ===="; // ————————————————————————
input const string indicator_file2 = "SuperWoodieCCI";            // Indicator File:
input int          CCI_period      = 50;                          // CCI periods:
input int          TCCI_period     = 20;                          // TCCI periods:

//---
int       histo                           = 0;
int       cciline                         = 1;
input int candles_back_for_signals_woodie = 1;

//---
double Wodie(int buffer, int candle = 1) { return iCustom(NULL, 0, indicator_file2, CCI_period, TCCI_period, buffer, candle); }
...
class ConditionIndicatorBuy : public iConditions
{
  ...
  public:
    bool evaluate()
    {
        for (int i = 1; i <= candles_back_for_signals_woodie; i++) {
            double indi = Wodie(histo, i);
            double line = Wodie(cciline, i);
            if (indi > 0 && indi != EMPTY_VALUE && indi != lastSignal && line > 0 && line != EMPTY_VALUE && line != lastSignal) {
                lastSignal = indi;
                return true;
            }
        }
...
ConditionIndicatorBuy cdWoodieBuy;
...
ConditionIndicatorSell cdWoodieSell;

//
//
//

class ConditionPersistanceSignalBuy : public iConditions
{

  public:
    bool evaluate()
    {
        // buscar hora del ultimo cambio de señal del indicador a buy
        // buscar hora del ultimo trade buy abierto,
        // si la hora del trade es posterior a la de la señal return false

        int t = 0;
        while (Wodie(histo, t) > 0 && Wodie(cciline, t) > 0) {
            t++;
        }
        if (t == 0) return false;
        datetime tmLastSignal = iTime(NULL, 0, t);
        Print("line: ",__LINE__," tmLastSignal: ",tmLastSignal, " t: ", t);

        datetime tmTrade;
        for (int i = OrdersTotal() - 1; i >= 0; i--) {
            if (OrderSelect(i, SELECT_BY_POS) && OrderSymbol() == _Symbol && OrderMagicNumber() == magico && OrderType() == OP_BUY) {
                tmTrade = OrderOpenTime();
                if (tmTrade > tmLastSignal) {
                    return false;
                }
            }
        }
...
ConditionPersistanceSignalBuy cdPersistanceSignalBuy;

class ConditionPersistanceSignalSell : public iConditions
{
...
ConditionPersistanceSignalSell cdPersistanceSignalSell;
...
bool OnInit_CustomIndicator()
{
    ...
    temp = iCustom(NULL, 0, indicator_file2, 0, 0);
    if (GetLastError() == ERR_INDICATOR_CANNOT_LOAD) {
        MessageBox("THIS EA NEED AN INDICATOR\n install the file:\n" + indicator_file2 + "\ninto the folder:\nMQL4/Indicators.", "Important Information", MB_ICONINFORMATION);
    ...
    if (!indi_init) return false;

    // Woodie
    LongStrategy.addCondition(&cdWoodieBuy);
    ShortStrategy.addCondition(&cdWoodieSell);
    
    if(onlyFirst){
        LongStrategy.addCondition(&cdPersistanceSignalBuy);
        ShortStrategy.addCondition(&cdPersistanceSignalSell);
    }

    // Heiken Ashi patterns
    LongStrategy.addCondition(&cdHeikenAshiBuy);
    ShortStrategy.addCondition(&cdHeikenAshiSell);
...
input bool   close_by_strategy_on = false;                   // Close by Heiken Ashi:

ConditionHeikenAshiSell cdHeikenAshiCloseBuy;
ConditionHeikenAshiBuy cdHeikenAshiCloseSell;
...
{
    stCloseBuy.set(&cdHeikenAshiCloseBuy, &acCloseBuy);
    stCloseSell.set(&cdHeikenAshiCloseSell, &acCloseSell);
