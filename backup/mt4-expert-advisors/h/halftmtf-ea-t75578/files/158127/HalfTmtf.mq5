...
ATRStream* atr1;
double maxlowprice[];
double maxlowprice_DEFAULT_VALUE;
double minhighprice[];
double minhighprice_DEFAULT_VALUE;
ColoredPlot* plot1;
double plot2[];
double plot3[];
...
void OnInit()
{
   int id = 0;
   Amplitude = param1;
   lowest1 = new LowestLowStream(_Symbol, (ENUM_TIMEFRAMES)_Period, Amplitude);
   highest1 = new HighestHighStream(_Symbol, (ENUM_TIMEFRAMES)_Period, Amplitude);
   sma1Source = new FloatStream(_Symbol, (ENUM_TIMEFRAMES)_Period);
   sma1 = new SmaOnStream(sma1Source, Amplitude);
   sma2Source = new FloatStream(_Symbol, (ENUM_TIMEFRAMES)_Period);
   sma2 = new SmaOnStream(sma2Source, Amplitude);
   atr1 = new ATRStream(100);
   ...
   IndicatorSetString(INDICATOR_SHORTNAME, "HalfTmtf");
                ...
                const int &spread[])
{
   if (prev_calculated <= 0 || prev_calculated > rates_total)
   {
      ...
      maxlowprice_DEFAULT_VALUE = EMPTY_VALUE;
      ArrayInitialize(maxlowprice, maxlowprice_DEFAULT_VALUE);
      minhighprice_DEFAULT_VALUE = EMPTY_VALUE;
      ArrayInitialize(minhighprice, minhighprice_DEFAULT_VALUE);
      plot1.Init();
   ...
   for (int pos = MathMax(rates_total - 1 - bars_limit, MathMax(first, prev_calculated - 1)); pos < rates_total; ++pos)
   {
      ...
      maxlowprice[pos] = pos > 0 ? maxlowprice[pos - 1] : EMPTY_VALUE;
      minhighprice[pos] = pos > 0 ? minhighprice[pos - 1] : EMPTY_VALUE;
      double lowest1Value[1];
      if (!lowest1.GetValues(pos, 1, lowest1Value)) { lowest1Value[0] = EMPTY_VALUE; }
      double lowprice_i = lowest1Value[0];
      double highest1Value[1];
      if (!highest1.GetValues(pos, 1, highest1Value)) { highest1Value[0] = EMPTY_VALUE; }
      double highprice_i = highest1Value[0];
      sma1Source.SetValue(pos, low[pos]);
      double sma1Value[1];
      if (!sma1.GetValues(pos, 1, sma1Value)) { sma1Value[0] = EMPTY_VALUE; }
      double lowma = sma1Value[0];
      sma2Source.SetValue(pos, high[pos]);
      double sma2Value[1];
      if (!sma2.GetValues(pos, 1, sma2Value)) { sma2Value[0] = EMPTY_VALUE; }
      double highma = sma2Value[0];
      double atr1Value[1];
      if (!atr1.GetValues(pos, 1, atr1Value)) { atr1Value[0] = EMPTY_VALUE; }
      double atr = SafeDivide(atr1Value[0], 2);
      if (((trend[pos] == 1)))
      {
         if (pos - 1 < 0) { continue; }
         if ((!((maxlowprice[pos - 1]) == EMPTY_VALUE)))
         {
            if (pos - 1 < 0) { continue; }
            SetStream(maxlowprice, pos, SafeMathMax(lowprice_i, maxlowprice[pos - 1]), maxlowprice_DEFAULT_VALUE);
         }
         else
         {
            SetStream(maxlowprice, pos, lowprice_i, maxlowprice_DEFAULT_VALUE);
         }
         if (pos - 1 < 0) { continue; }
         if ((SafeLess(highma, maxlowprice[pos]) && SafeLess(close[pos], low[pos - 1])))
         {
            SetStream(trend, pos, 0, trend_DEFAULT_VALUE);
            SetStream(minhighprice, pos, highprice_i, minhighprice_DEFAULT_VALUE);
         }
      }
      else if (((trend[pos] == 0)))
      {
         if (pos - 1 < 0) { continue; }
         if ((!((minhighprice[pos - 1]) == EMPTY_VALUE)))
         {
            if (pos - 1 < 0) { continue; }
            SetStream(minhighprice, pos, SafeMathMin(highprice_i, minhighprice[pos - 1]), minhighprice_DEFAULT_VALUE);
         }
         else
         {
            SetStream(minhighprice, pos, highprice_i, minhighprice_DEFAULT_VALUE);
         ...
         if (pos - 1 < 0) { continue; }
         if ((SafeGreater(lowma, minhighprice[pos]) && SafeGreater(close[pos], high[pos - 1])))
         {
            SetStream(trend, pos, 1, trend_DEFAULT_VALUE);
            SetStream(maxlowprice, pos, lowprice_i, maxlowprice_DEFAULT_VALUE);
         }
      }
      double plot1Value = plot1.Set(pos, ((trend[pos] == 1) ? maxlowprice[pos] : minhighprice[pos]), ((trend[pos] == 1) ? Blue : Red));
      if (pos - 1 < 0) { continue; }
      int plotshape2_condition = (trend[pos] == 1) && (trend[pos - 1] == 0);
      if (plotshape2_condition == true) { plot2[pos] = low[pos]; }
      if (pos - 1 < 0) { continue; }
      int plotshape3_condition = (trend[pos] == 0) && (trend[pos - 1] == 1);
