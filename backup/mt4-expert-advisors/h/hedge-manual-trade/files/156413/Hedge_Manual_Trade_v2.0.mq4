        ...
        for (int i = OrdersTotal() - 1; i >= 0; i--) {
            if (OrderSelect(i, SELECT_BY_POS, MODE_TRADES) && OrderSymbol() == _Symbol) {
                ...
                if (!haveHedge) {
                    ...
                    if (ot == OP_BUY) {
                        int r = OrderSend(NULL, OP_SELLSTOP, h_lots, order_price - distance, 0, 0, 0, com, 0, 0, clrMagenta);
                    }
                    if (ot == OP_SELL) {
                        int r = OrderSend(NULL, OP_BUYSTOP, h_lots, order_price + distance, 0, 0, 0, com, 0, 0, clrMagenta);
                    }
                }
            }
        }
