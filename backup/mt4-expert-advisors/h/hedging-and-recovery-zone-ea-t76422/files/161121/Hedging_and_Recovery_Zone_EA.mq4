...
Version:     
Date:        
Repository:  Available @ https://fxcodebase.com/code/viewtopic.php?f=38&t=76422
License:     GNU
*/
...
input int       magico     = 0;                   // Magic Number (0 for mannual trades):
...
#define Section_Recovery_Zone
#ifdef Section_Recovery_Zone
// Mark: Section_Recovery_Zone

input string note_1            = "Consider the secuency of lot multiplier to recovery the trade is this: 1, 2, 4, 8, 16, 32..."; // NOTE:
input int    max_recovery_trys = 2;     // Attemps to Recovery:
bool         recovery_zone_on  = true;  // Recovery Zone on?
input double uLotMultiplier    = 2.0;   // Lot Multiplier 1, 1, 2, 4, 8, 16

void RecoveryZoneOn()
{
    // recovery_zone_on = !recovery_zone_on;
    recovery_zone_on = true;
    Recovery_Zone.resetTrys();
}

class ConditionRecovery_Zone : public iConditions
{
  public:
    bool evaluate() { return recovery_zone_on; }
};
ConditionRecovery_Zone cdRecovery_Zone;

class ActionRecovery_Zone : public iActions
{
    int             tk_to_recovery;
    int             try_nr;
    double          zone_size;
  ...
  public:
    double lots() { return last_lot * try_nr; }
    void   resetTrys() { try_nr = 0; }

    bool closeBySl()
    {
        if (OrderSelect(tk_to_recovery, SELECT_BY_TICKET)) {
            if (OrderCloseTime() > 0 && OrderProfit() < 0 && OrderStopLoss() != 0) return true;
        }
        return false;
    }
    ...
    void updateTk()
    {
        for (int i = OrdersTotal() - 1; i >= 0; i--) {
            if (OrderSelect(i, SELECT_BY_POS) && OrderSymbol() == _Symbol) {
                if (OrderTicket() != tk_to_recovery) {
                    tk_to_recovery = OrderTicket();
                }
                Print("line: ", __LINE__, " tk_to_recovery: ", tk_to_recovery);
                zone_size = fabs(OrderOpenPrice() - OrderStopLoss());
                Print("line: ", __LINE__, " zone_size: ", zone_size);
                last_lot = OrderLots();
                Print("line: ", __LINE__, " last_lot: ", last_lot);
    ...
    void OpenRecovery()
    {
        ENUM_ORDER_TYPE cmd   = last_cmd == OP_BUY ? OP_SELL : OP_BUY;
        double          price = 0, sl = 0, tp = 0, l = 0;

        // Lot Multiplier 1(original), 1, 2, 4, 8, 16

        if (cmd == OP_BUY) {
            sl = Ask - zone_size;
            tp = Ask + zone_size;
            if (try_nr == 1) l = last_lot;
            if (try_nr > 1) l = last_lot * 2;
            price = Ask;
        }
        if (cmd == OP_SELL) {
            sl = Bid + zone_size;
    ...
    {
        updateTk();
        if (closeBySl() && try_nr < max_recovery_trys) {
            try_nr++;
            OpenRecovery();
        }

        if (try_nr == max_recovery_trys) {
            RecoveryZoneOn();
        }
...
ActionRecovery_Zone Recovery_Zone;

Strategy stRecovery_Zone;

void OnInit_Recovery_Zone()
{
    stRecovery_Zone.set(&cdRecovery_Zone, &Recovery_Zone);
    Trader.strategys.add(&stRecovery_Zone);
}
...
// Controlar solo un trade Long por día
class ConditionOneLongTradePerDay : public iConditions
{
  private:
    datetime lastTradeDay;

  public:
    ConditionOneLongTradePerDay() { lastTradeDay = 0; }
    ...
    bool evaluate()
    {
        ...
        for (int i = OrdersHistoryTotal() - 1; i >= 0; i--) {
            if (OrderSelect(i, SELECT_BY_POS, MODE_HISTORY)) {
                if (OrderSymbol() == _Symbol && OrderMagicNumber() == magico && OrderType() == OP_BUY) {
                    datetime openDay = iTime(NULL, PERIOD_D1, iBarShift(NULL, PERIOD_D1, OrderOpenTime(), true));
                    if (openDay == today) {
                        lastTradeDay = today;
                        return false; // Ya hubo un trade BUY hoy
                    }
                }
            }
        }
        // También chequea trades abiertos hoy
        for (int i = OrdersTotal() - 1; i >= 0; i--) {
            if (OrderSelect(i, SELECT_BY_POS, MODE_TRADES)) {
                if (OrderSymbol() == _Symbol && OrderMagicNumber() == magico && OrderType() == OP_BUY) {
                    ...
                    if (openDay == today) {
                        lastTradeDay = today;
                        return false; // Ya hay un trade BUY abierto hoy
                    }
                }
            }
        }
        return true; // No hay trade BUY hoy
    }
};
ConditionOneLongTradePerDay cOneLongTradePerDay;

// Controlar solo un trade Short por día
class ConditionOneShortTradePerDay : public iConditions
{
  private:
    datetime lastTradeDay;

  public:
    ConditionOneShortTradePerDay() { lastTradeDay = 0; }
    ...
    bool evaluate()
    {
        ...
        for (int i = OrdersHistoryTotal() - 1; i >= 0; i--) {
            if (OrderSelect(i, SELECT_BY_POS, MODE_HISTORY)) {
                if (OrderSymbol() == _Symbol && OrderMagicNumber() == magico && OrderType() == OP_SELL) {
                    datetime openDay = iTime(NULL, PERIOD_D1, iBarShift(NULL, PERIOD_D1, OrderOpenTime(), true));
                    if (openDay == today) {
                        lastTradeDay = today;
                        return false; // Ya hubo un trade SELL hoy
                    }
                }
            }
        }
        ...
        for (int i = OrdersTotal() - 1; i >= 0; i--) {
            if (OrderSelect(i, SELECT_BY_POS, MODE_TRADES)) {
                if (OrderSymbol() == _Symbol && OrderMagicNumber() == magico && OrderType() == OP_SELL) {
                    ...
                    if (openDay == today) {
                        lastTradeDay = today;
                        return false; // Ya hay un trade SELL abierto hoy
                    }
                }
            }
        }
        return true; // No hay trade SELL hoy
    }
};
ConditionOneShortTradePerDay cOneShortTradePerDay;

// Estrategia básica que simula que el usuario abre un trade
class BUYcondition1 : public iConditions
{
  public:
    bool evaluate()
    {
        ...
        if (IsTesting() == true && OrdersTotal() == 0) {
            if (iMA(NULL, 0, 20, 0, MODE_EMA, PRICE_CLOSE, 0) > iMA(NULL, 0, 50, 0, MODE_EMA, PRICE_CLOSE, 0)) {
                RecoveryZoneOn();
                return true;
            }
        }
...
void BasicStrategy_Iniciate()
{
    ...
    LongStrategy.addCondition(&cOneLongTradePerDay);
    ShortStrategy.addCondition(&cOneShortTradePerDay);
...
Version:     
Date:        
Repository:  Available @ https://fxcodebase.com/code/viewtopic.php?f=38&t=76408
License:     GNU
*/
