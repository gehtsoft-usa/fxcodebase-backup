...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      double value = iCustom(_symbol, _timeframe, "Extreme_TMA_Line", TMA_Period, ATR_Period, ATR_Mult, TrendThreshold, 4, period);
      double close = iCustom(_symbol, _timeframe, "Heiken Ashi", 2, period);
      double close1 = iCustom(_symbol, _timeframe, "Heiken Ashi", 2, period + 1);
      double open = iCustom(_symbol, _timeframe, "Heiken Ashi", 3, period);
      double open1 = iCustom(_symbol, _timeframe, "Heiken Ashi", 3, period + 1);
      return close > open && close1 < open1 && close < value;
   }
};
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      double value = iCustom(_symbol, _timeframe, "Extreme_TMA_Line", TMA_Period, ATR_Period, ATR_Mult, TrendThreshold, 3, period);
      double close = iCustom(_symbol, _timeframe, "Heiken Ashi", 2, period);
      double close1 = iCustom(_symbol, _timeframe, "Heiken Ashi", 2, period + 1);
      double open = iCustom(_symbol, _timeframe, "Heiken Ashi", 3, period);
      double open1 = iCustom(_symbol, _timeframe, "Heiken Ashi", 3, period + 1);
