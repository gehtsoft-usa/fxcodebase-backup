...
input int    userTPpips = 30;                 // Pips TP
...
input int    userSLpips = 60;                // Pips SL
...
bool   GridON = true;                         // Use Grid:
int    GridUser_maxCount = 100;                           // Max attempts:
double GridUser_maxLot = 40;                           // Max lot value:
double GridUser_multiplier = 2;                           // Multiplier:
input string GridUserLots = "0.03,0.06,0.12,0.24,0.48,0.96,1.92,3.84"; // Lots Sequence:
input int    GridUser_gap = 30;                           // Racovery Zone Size:
input bool   closeGridByHitTP = true; // Close Grid When hit TP?
input bool   closeGridOn = true;                        // Close by profit / loss ?
input double closeGridTP = 100;                          // TP Grid $ (zero deactivated)
input double closeGridSL = -1000;                        // SL Grid -$ (zero deactivated)
#else
...
class OrdersList
{
    ...
    //   comprueba si se cerró por TP
    // ------------------------------------------------------------------
    bool isCloseByTP(int index)
    {
        if(notOverFlow(index))
        {
            if(OrderSelect(id(index), SELECT_BY_TICKET))
...
class Grid
{
    ...
    Grid(string Symbol, string Side, double LastPrice, double Gap, double Multiplier, int MaxQnt, double MaxLot, double InitialLot, int magic, bool simbolFilterOn = true, bool magicFilterOn = true, bool useTP = false, bool useSL = false, string Lots = "")
    {
        _symbol = Symbol;
        _side = Side;
        ...
        // Set Conditions:
        cdMatchPrice = new ConditionMatchPrice(_symbol, _side, _nextPrice, 0);
    ...
    void getLots(string uLots)
    {
        string lotes [];
        string sep = ",";
        ushort u_sep;
        u_sep = StringGetCharacter(sep, 0);
        int k = StringSplit(uLots, u_sep, lotes);
        ArrayResize(_lots, ArrayRange(lotes, 0), 0);
        for(int i = 0; i < ArrayRange(lotes, 0); i++)
        {
            _lots[i] = StringToDouble(lotes[i]);
            Print(__FUNCTION__, " _lots: ", i, " / ", _lots[i]);
        }
        if(ArrayRange(_lots, 0) > 0)
        {
            ArraySort(_lots, WHOLE_ARRAY, 0, MODE_ASCEND);
        }
    }

    double nextLot(void)
    {
        // pasar un array con los lotes
        // devolver el lotaje según la cantidad de trades abiertos
        Print("qnt: ", qnt());
        if(qnt() >= 1)
        {
            return NormalizeDouble(_lots[qnt() - 1], 2);
        }
    ...
    bool HitTP()
    {
        ...
        //   recorrer las ordenes y si alguna se cerró por tp, retornar true
        for(int i = 0; i < gridOrders.qnt(); i++)
        {
            // if(CheckPointer(gridOrders.index(i) != POINTER_INVALID))
            // {
            if(gridOrders.isCloseByTP(i)) return true;
            // }
        }
            return false;
...
class BUYcondition1 : public iConditions
{
    ...
    bool evaluate()
    {
        ...
        // return iClose(NULL, 0, 1) > ma.index(1) && iClose(NULL, 0, 2) < ma.index(2);
        return true;
    }
};
    ...
    if(!uTradeReverse)
    {
        if(conditionsToBuy.EvaluateConditions())
        {
            ...
            if(actionSendOrder.doAction())
            {
                ...
                if(GridON == true && CheckPointer(gridBuy) == POINTER_INVALID)
                {
                    gridBuy = new Grid(_Symbol, "buy", mainOrders.last().price(), GridUser_gap, GridUser_multiplier, GridUser_maxCount, GridUser_maxLot, mainOrders.last().lot(), magico, true, true, takeProfitOn, stopLossOn, GridUserLots);
        ...
        if(conditionsToSell.EvaluateConditions())
        {
            ...
            if(actionSendOrder.doAction())
            {
                ...
                if(GridON == true && CheckPointer(gridSell) == POINTER_INVALID)
                {
                    ...
                    gridSell = new Grid(_Symbol, "sell", mainOrders.last().price(), GridUser_gap, GridUser_multiplier, GridUser_maxCount, GridUser_maxLot, mainOrders.last().lot(), magico, true, true, takeProfitOn, stopLossOn, GridUserLots);
...
void doCloseGridControl()
{
    ...
    if(closeGridByHitTP)
    {
        if(CheckPointer(gridBuy) != POINTER_INVALID)
        {
            if(gridBuy.HitTP()) { gridBuy.closeGrid(); delete gridBuy; }
        }
        if(CheckPointer(gridSell) != POINTER_INVALID)
        {
            if(gridSell.HitTP()) { gridSell.closeGrid(); delete gridSell; }
        }
