...
double up_[], upColor[], atrhi[], atrlo[], trend[], atrcolor[], minhighprice[], maxlowprice[];
int atr, mahigh, malow;
int OnInit(void)
{
   IndicatorObjPrefix = GenerateIndicatorPrefix("HalfTrend");
   IndicatorSetString(INDICATOR_SHORTNAME, "HalfTrend");
   IndicatorSetInteger(INDICATOR_DIGITS, Digits());
   atr = iATR(_Symbol, _Period, 100);
   mahigh = iMA(_Symbol, _Period, Amplitude, 0, MODE_SMA, PRICE_HIGH);
   malow = iMA(_Symbol, _Period, Amplitude, 0, MODE_SMA, PRICE_LOW);
   ...
   SetIndexBuffer(id++, up_, INDICATOR_DATA);
   SetIndexBuffer(id++, upColor, INDICATOR_COLOR_INDEX);
   SetIndexBuffer(id++, atrhi, INDICATOR_DATA);
   SetIndexBuffer(id++, atrlo, INDICATOR_DATA);
   SetIndexBuffer(id++, atrcolor, INDICATOR_COLOR_INDEX);
   SetIndexBuffer(id++, trend, INDICATOR_CALCULATIONS);
   SetIndexBuffer(id++, maxlowprice, INDICATOR_CALCULATIONS);
   SetIndexBuffer(id++, minhighprice, INDICATOR_CALCULATIONS);
...
)
{
   if (prev_calculated <= 0 || prev_calculated > rates_total)
   {
      ...
      ArrayInitialize(up_, EMPTY_VALUE);
      ArrayInitialize(upColor, 0);
      ArrayInitialize(maxlowprice, EMPTY_VALUE);
      ArrayInitialize(minhighprice, EMPTY_VALUE);
      ArrayInitialize(atrlo, EMPTY_VALUE);
      ArrayInitialize(atrhi, EMPTY_VALUE);
      ArrayInitialize(atrcolor, 0);
      up.Init();
      down.Init();
   }
   int first = 100;
   for (int pos = MathMax(first, prev_calculated - 1); pos < rates_total; ++pos)
   {
      ...
      double lowprice_i = iLow(Symbol(),Period(),iLowest(Symbol(),Period(),MODE_LOW,Amplitude, oldIndex));
      double highprice_i = iHigh(Symbol(),Period(),iHighest(Symbol(),Period(),MODE_HIGH,Amplitude, oldIndex));
      double lowma[1];
      if (CopyBuffer(malow, 0, oldIndex, 1, lowma) != 1 || lowma[0] == EMPTY_VALUE)
      {
         continue;
      }
      double highma[1];
      if (CopyBuffer(mahigh, 0, oldIndex, 1, highma) != 1 || highma[0] == EMPTY_VALUE)
      {
         continue;
      }
      double atrValue[1];
      if (CopyBuffer(atr, 0, oldIndex, 1, atrValue) != 1 || atrValue[0] == EMPTY_VALUE)
      {
         continue;
      }
      maxlowprice[pos] = maxlowprice[pos - 1];
      minhighprice[pos] = minhighprice[pos - 1];
      if (trend[pos] != 1)
      {
         maxlowprice[pos] = MathMax(lowprice_i, maxlowprice[pos]);
         if (highma[0] < maxlowprice[pos] && close[pos] < low[pos - 1])
         {
            trend[pos] = 1.0;
            minhighprice[pos] = highprice_i;
         }
      }
      else
      {
         minhighprice[pos] = MathMin(highprice_i, minhighprice[pos]);
         if (lowma[0] > minhighprice[pos] && close[pos] > high[pos - 1])
         {
            trend[pos] = -1;
            maxlowprice[pos] = lowprice_i;
         }
      }
      if (trend[pos] == -1)
      {
         if(trend[pos - 1] == -1)
         {
            up_[pos] = MathMax(maxlowprice[pos], up_[pos - 1]);
         }
         upColor[pos] = 0;
         atrlo[pos] = up_[pos] - atrValue[0];
         atrcolor[pos] = 0;
      }
      else
      {
         if(trend[pos - 1] == 1)
         {
            up_[pos] = MathMin(minhighprice[pos], up_[pos - 1]);
         }
         upColor[pos] = 1;
         atrlo[pos] = up_[pos] + atrValue[0];
         atrcolor[pos] = 1;
