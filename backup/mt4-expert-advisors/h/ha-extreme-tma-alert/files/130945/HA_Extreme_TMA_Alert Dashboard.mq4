...
input ENUM_APPLIED_PRICE BBPrice = PRICE_CLOSE; // BB Price
...
public:
   ...
   {
      double bbValue = iBands(_symbol, _timeframe, BBLength, BBDeviation, BBShift, BBPrice, MODE_MAIN, period);
      double close = iClose(_symbol, _timeframe, period);
      double value = iCustom(_symbol, _timeframe, "HA_Extreme_TMA_Alert", TMA_Period, ATR_Period, AtrMult, 3, period);
      return value != 0 && value != EMPTY_VALUE && close > bbValue;
   }
};
...
public:
   ...
   virtual bool IsPass(const int period, const datetime date)
   {
      double bbValue = iBands(_symbol, _timeframe, BBLength, BBDeviation, BBShift, BBPrice, MODE_MAIN, period);
      double close = iClose(_symbol, _timeframe, period);
      double value = iCustom(_symbol, _timeframe, "HA_Extreme_TMA_Alert", TMA_Period, ATR_Period, AtrMult, 4, period);
      return value != 0 && value != EMPTY_VALUE && close < bbValue;
   }
};
...
int init()
{
   ...
   IndicatorName = GenerateIndicatorName("HA Extreme TMA Dashboard");
