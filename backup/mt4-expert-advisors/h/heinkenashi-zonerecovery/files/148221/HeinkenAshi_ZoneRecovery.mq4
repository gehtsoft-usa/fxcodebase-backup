...
// https://fxcodebase.com/code/viewtopic.php?f=38&p=148245
...
input string tHeiken             = "== Heiken Ashi Setup ==";  // == Heiken Ashi Setup ==
input int    uConsecutiveCandles = 3;                          // Consecutive Candles
input string tAlligator          = "== Alligator Setup==";     // == Alligator Setup ==
input bool   uAlligatorON        = true;                       // Alligator Condition On:
input int    uJawsPeriod         = 13;                         // JawsPeriod
input int    uJawsShift          = 8;                          // JawsShift
input int    uTeethPeriod        = 8;                          // TeethPeriod
input int    uTeethShift         = 5;                          // TeethShift
input int    uLipsPeriod         = 5;                          // LipsPeriod
input int    uLipsShift          = 3;                          // LipsShift
...
#endif
input bool   uTradeReverse  = false;              // Trade Reverse Signal:
...
input int    userTPpips     = 50;                 // Pips TP
...
input int    userSLpips     = 100;                // Pips SL
...
#endif
#ifdef GRID_ON
input string tGrid               = "== Recovery Zone Setup ==";  // ----------
input bool   GridON              = true;                         // Use Grid:
input int    GridUser_maxCount   = 10;                           // Max attempts:
input double GridUser_maxLot     = 5;                            // Max lot value:
input double GridUser_multiplier = 2.2;                          // Multiplier:
input int    GridUser_gap        = 50;                           // Pips betwen orders:
input bool   closeGridOn         = false;                        // Use Close Grid?
input double closeGridTP         = 100;                          // TP Grid $ (zero deactivated)
input double closeGridSL         = -100;                         // SL Grid -$ (zero deactivated)
#else
...
input bool     day1_3_On    = true;        // Session On:
input enumDays day1_3       = monday;      // Day
input string   day1_3_Start = "00:00:00";  // Time Start GMT
input string   day1_3_End   = "23:59:59";  // Time End GMT
...
input bool     day2_3_On    = true;        // Session On:
input enumDays day2_3       = tuesday;     // Day
input string   day2_3_Start = "00:00:00";  // Time Start GMT
input string   day2_3_End   = "23:59:59";  // Time End GMT
...
input bool     day3_3_On    = true;        // Session On:
input enumDays day3_3       = wednesday;   // Day
input string   day3_3_Start = "00:00:00";  // Time Start GMT
input string   day3_3_End   = "23:59:59";  // Time End GMT
...
input bool     day4_3_On    = true;        // Session On:
input enumDays day4_3       = thursday;    // Day
input string   day4_3_Start = "00:00:00";  // Time Start GMT
input string   day4_3_End   = "23:59:59";  // Time End GMT
...
input bool     day5_3_On    = true;        // Session On:
input enumDays day5_3       = friday;      // Day
input string   day5_3_Start = "00:00:00";  // Time Start GMT
input string   day5_3_End   = "23:59:59";  // Time End GMT
  ...
  string FindPattern(int _tf = 0)
  {
    ...
    for (int i = 1; i < 4; i++)
    {
      ...
      if (haveSize)
      {
        ...
        if (op2 < cl2 && cl1 < op1 && cl1 <= (op2 + body2 * 0.30))
        {
          direction  = "q";
          havePatern = true;
        }  // bajista
...
class HeikenAshi
{
 ...
 public:
  HeikenAshi(string Symbol, int TimeFrame)
  {
    _symbol = Symbol;
    _tf     = TimeFrame;
    _file   = "Heiken Ashi.ex4";
  }
  ~HeikenAshi() { ; }

  HeikenAshi* file(string setfile)
  {
    _file = setfile;
    return &this;
  }
  ...
  double calculate(int buffer, int shift)
  {
    return iCustom(_symbol, _tf, _file, buffer, shift);
  }

  double open(int i) { return calculate(2, i); }
  double close(int i) { return calculate(3, i); }
  bool   isUpCandle(int i) { return open(i) < close(i); }
  bool   isDownCandle(int i) { return open(i) > close(i); }

  double high(int i)
  {
    // si la vela es roja, High es el buffer 0
    if (isDownCandle(i)) return calculate(0, i);
    // si la vela es blanca, high es el buffer 1
    if (isUpCandle(i)) return calculate(1, i);

    return 0;
  }
  double low(int i)
  {
    // si la vela es roja, High es el buffer 1
    if (isDownCandle(i)) return calculate(1, i);
    // si la vela es blanca, high es el buffer 0
    if (isUpCandle(i)) return calculate(0, i);
...
class Grid
{
 ...
 public:
  Grid(string Symbol, string Side, double LastPrice, double Gap, double Multiplier, int MaxQnt, double MaxLot, double InitialLot, int magic, bool simbolFilterOn = true, bool magicFilterOn = true, bool useTP = false, bool useSL = false)
  {
    _symbol     = Symbol;
    _side       = Side;
  ...
  double nextPrice(double inpLastPrice = 0)
  {
    ...
    {
      if (_side == "buy") _nextPrice = _sellPrice;
      if (_side == "sell") _nextPrice = _buyPrice;
    }

    return _nextPrice;
  }

  void setBuyAndSellPrices()
  {
    double mPoint = MarketInfo(_symbol, MODE_POINT);

    if (_side == "buy")
    {
      _sellPrice = _lastPrice - (_gap * mPoint * 10);
      _buyPrice  = _lastPrice;
    }
    if (_side == "sell")
    {
      _sellPrice = _lastPrice;
      _buyPrice  = _lastPrice + (_gap * mPoint * 10);
    }
  }
  ...
  void doGrid()
  {
    if (conditionsToOpenNewTrade.EvaluateConditions())
    {
      if (_side == "sell")
      {
        openTrade = new SendNewOrder("buy", Lots(), "", 0, SL("buy", _buyPrice), TP("buy", _buyPrice), _magico);
        if (openTrade.doAction())
        {
          ...
          {
            side("buy");  // doy vuelta el side para que la próxima la abra para el otro lado
            setNextTrade();
          }
        }
        delete openTrade;
      ...
      if (_side == "buy")
      {
        openTrade = new SendNewOrder("sell", Lots(), "", 0, SL("sell", _sellPrice), TP("sell", _sellPrice), _magico);
        if (openTrade.doAction())
        {
          if (gridOrders.GetLastMarketOrder())
          {
            side("sell");  // doy vuelta el side para que la próxima la abra para el otro lado
            setNextTrade();
          }
        }
        delete openTrade;
  ...
  double SL(string side, double price = 0)
  {
    ...
    levelSL = new Levels(new ByFixPips(Symbol(), side, userSLpips, "SL", price));
  ...
  double TP(string side, double price)
  {
    double result = 0;
    if (!_useTP) return 0;

    levelTP = new Levels(new ByFixPips(Symbol(), side, userTPpips, "TP", price));
...
class BUYcondition1 : public iConditions
{
 public:
  bool evaluate()
  {
    ...
    for (int i = 1; i <= uConsecutiveCandles; i++)
    {
      if (!heiken.isUpCandle(i)) return false;
    }
    return true;
  }
};
BUYcondition1* buyCondition_Heiken;
...
class BUYcondition2 : public iConditions
{
 public:
  bool evaluate()
  {
    ...
    for (int i = 1; i <= uConsecutiveCandles; i++)
    {
      if (!aligatorUP(i)) return false;
      if (heiken.close(i) < aligatorLips(i)) return false;
    }

    return true;
  }
};
BUYcondition2* buyCondition_Alligator;
...
class SELLcondition1 : public iConditions
{
 public:
  bool evaluate()
  {
    ...
    for (int i = 0; i <= uConsecutiveCandles; i++)
    {
      if (!heiken.isDownCandle(i)) return false;
    }
    return true;
  }
};
SELLcondition1* sellHeikenCondition;
...
SELLcondition2* sellCondition_Alligator;
...
class ConditionToCloseBuy : public iConditions
{
 public:
  bool evaluate()
  {
    ...
    {
      return sellCondition_Alligator.evaluate() && sellHeikenCondition.evaluate();
    }
...
class ConditionToCloseSell : public iConditions
{
 public:
  bool evaluate()
  {
    if (closeAllInOpositeSignal)
    {
      return buyCondition_Alligator.evaluate() && buyCondition_Heiken.evaluate();
    }
    if (closeAllControlON)
    {
      return CloseAllControl();
...
int OnInit()
{
  ...
  //--- buys:
  conditionsToBuy.AddCondition(buyCondition_Heiken = new BUYcondition1());
  if (uAlligatorON) conditionsToBuy.AddCondition(buyCondition_Alligator = new BUYcondition2());
  ...
  //--- sell:
  conditionsToSell.AddCondition(sellHeikenCondition = new SELLcondition1());
  if (uAlligatorON) conditionsToSell.AddCondition(sellCondition_Alligator = new SELLcondition2());
...
//--- SESSIONS CONTROL:
  ...
  if (day1_3_On) sesionControl.AddSession(day1_3_Start, day1_3_End, day1_3);
  ...
  if (day2_3_On) sesionControl.AddSession(day2_3_Start, day2_3_End, day2_3);
  ...
  if (day3_3_On) sesionControl.AddSession(day3_3_Start, day3_3_End, day3_3);
  ...
  if (day4_3_On) sesionControl.AddSession(day4_3_Start, day4_3_End, day4_3);
  ...
  if (day5_3_On) sesionControl.AddSession(day5_3_Start, day5_3_End, day5_3);
  ...
  heiken = new HeikenAshi(_Symbol, 0);
  ...
  if (!uTradeReverse)
  {
    if (conditionsToBuy.EvaluateConditions())
    {
      ...
      if (actionSendOrder.doAction())
      {
        ...
        if (GridON == true && CheckPointer(gridBuy) == POINTER_INVALID)
        {
          gridBuy = new Grid(_Symbol, "buy", mainOrders.last().price(), GridUser_gap, GridUser_multiplier, GridUser_maxCount, GridUser_maxLot, mainOrders.last().lot(), magico, true, true, takeProfitOn, stopLossOn);
    ...
    if (conditionsToSell.EvaluateConditions())
    {
      ...
      if (actionSendOrder.doAction())
      {
        ...
        if (GridON == true && CheckPointer(gridSell) == POINTER_INVALID)
        {
          ...
          gridSell = new Grid(_Symbol, "sell", mainOrders.last().price(), GridUser_gap, GridUser_multiplier, GridUser_maxCount, GridUser_maxLot, mainOrders.last().lot(), magico, true, true, takeProfitOn, stopLossOn);
...
bool aligatorUP(int i)
{
  double lips = iAlligator(NULL, 0, uJawsPeriod, uJawsShift, uTeethPeriod, uTeethShift, uLipsPeriod, uLipsShift, MODE_SMA, PRICE_CLOSE, MODE_GATORLIPS, i);

  double teeth = iAlligator(NULL, 0, uJawsPeriod, uJawsShift, uTeethPeriod, uTeethShift, uLipsPeriod, uLipsShift, MODE_SMA, PRICE_CLOSE, MODE_GATORTEETH, i);

  double jaw = iAlligator(NULL, 0, uJawsPeriod, uJawsShift, uTeethPeriod, uTeethShift, uLipsPeriod, uLipsShift, MODE_SMA, PRICE_CLOSE, MODE_GATORJAW, i);

  return lips > teeth && teeth > jaw;
}

bool aligatorDN(int i)
{
  double lips = iAlligator(NULL, 0, uJawsPeriod, uJawsShift, uTeethPeriod, uTeethShift, uLipsPeriod, uLipsShift, MODE_SMA, PRICE_CLOSE, MODE_GATORLIPS, i);
...
double aligatorLips(int i)
{
  return iAlligator(NULL, 0, uJawsPeriod, uJawsShift, uTeethPeriod, uTeethShift, uLipsPeriod, uLipsShift, MODE_SMA, PRICE_CLOSE, MODE_GATORLIPS, i);
}
