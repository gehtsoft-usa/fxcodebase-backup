...
public:
   ...
   {
      return iCustom(_symbol, _timeframe, "HARMONIC PATTERN_WITH ALERT"
         , UseBat, UseGartley, UseCrab, UseButterfly, UseABCD, UseThreeDrives
         , Correction, Depth, Deviation, Backstep, 0, period) > 0;
   }
};
...
public:
   ...
   void Create(const int order, const double distancePips, const TrailingType trailingType
      , const double trailingStep, const double trailingStart)
   {
      if (!OrderSelect(order, SELECT_BY_TICKET, MODE_TRADES) || OrderCloseTime() != 0.0)
         return;
...
int OnInit()
{
   ...
   double temp = iCustom(NULL, 0, "HARMONIC PATTERN_WITH ALERT", 0, 0);
   if (GetLastError() == ERR_INDICATOR_CANNOT_LOAD)
   {
      Alert("Please, install the 'HARMONIC PATTERN_WITH ALERT' indicator");
      return INIT_FAILED;
   }
