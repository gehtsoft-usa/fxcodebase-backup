//Available @ https://fxcodebase.com/code/viewtopic.php?f=38&t=73831
...
extern int    PSR_MA_Period    = 30;
extern bool   ShowBars         = false;
extern bool   ShowArrows       = false;
extern bool   ShowPSRArrows    = true;
...
int init()
  {
   ...
   SetIndexBuffer(7, PSRarrup);
   SetIndexBuffer(8, PSRarrdwn);
   ...
   for(int i = Bars - 1; i >= 0; i--)
     {
      ...
      if(ShowPSRArrows)
        {
         double ema0 = iMA(Symbol(), Period(), PSR_MA_Period, 0, MODE_EMA, PRICE_CLOSE, i);
         double ema1 = iMA(Symbol(), Period(), PSR_MA_Period, 0, MODE_EMA, PRICE_CLOSE, i + 1);
         if(trend[i] == 0 && Close[i] > ema0 && Close[i + 1] < ema1)
            PSRarrup[i] = Low[i] - atr;
         else
            PSRarrup[i] = 0.0;
         if(trend[i] == 1 && Close[i] < ema0 && Close[i + 1] > ema1)
            PSRarrdwn[i] = High[i] + 2 * atr;
         else
            PSRarrdwn[i] = 0.0;
        }
     }
...
void manageAlerts()
  {
   if(alertsOn)
     ...
     /* if(arrup[whichBar]  != EMPTY_VALUE)
      ...
      if(PSRarrup[whichBar]  != 0.0 && PSRarrup[whichBar]  != EMPTY_VALUE)
         doAlert(whichBar, "up");
      if(PSRarrdwn[whichBar] != 0.0 && PSRarrdwn[whichBar] != EMPTY_VALUE)
         doAlert(whichBar, "down");
     }
  }
