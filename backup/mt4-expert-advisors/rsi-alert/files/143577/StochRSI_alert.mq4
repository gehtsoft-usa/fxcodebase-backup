...
//https://fxcodebase.com/code/viewtopic.php?f=38&t=71499
...
input int Percent_K=14;
input int Percent_D=9;
input int NumOfBars=0;
input double os_level = 10; // Oversold level
input double ob_level = 90; // Overbought level
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      double value = iCustom(_symbol, _timeframe, "StochRSI", RSI_Periods, Percent_K, Percent_D, NumOfBars, 1, period);
      return value < os_level;
   }
};
...
int init()
{
   double temp = iCustom(NULL, 0, "StochRSI", 0, 0);
   if (GetLastError() == ERR_INDICATOR_CANNOT_LOAD)
   {
      Alert("Please, install the 'StochRSI' indicator");
      return INIT_FAILED;
   }
   ...
   IndicatorBuffers(8);
   IndicatorObjPrefix = GenerateIndicatorPrefix("srsia");
   IndicatorShortName("srsia");
