...
// https://fxcodebase.com/code/viewtopic.php?f=38&p=157676#p157676
...
input const string indicator_file = "Mobo_Bands.ex4";            // Indicator File:
input int          mb_periods     = 13;                          // Periods
input int          mb_length      = 10;                          // Length
input double       mb_desv        = 1;                           // Standar Deviation Bands
...
//---
double Indi(int buffer, int candle = 1) { return iCustom(NULL, 0, indicator_file, mb_periods, mb_length, mb_desv, buffer, candle); }
  ...
  public:
    bool evaluate()
    {
        for (int i = 1; i <= candles_back_for_signals; i++) {
            ...
            Print("line: ",__LINE__," indicator BUY SIGNAL: ",indi);
  ...
  public:
    bool evaluate()
    {
        for (int i = 1; i <= candles_back_for_signals; i++) {
            ...
            Print("line: ",__LINE__," indicator SELL SIGNAL: ",indi);
            if (indi > 0 && indi != EMPTY_VALUE && indi != lastSignal) {
                lastSignal = indi;
                return true;
            }
...
//--- --------------------- ---------------------
input const string indicator_file_filter           = "super trend (profit max)1.1"; // Indicator Filter File:
input bool         filter_on                       = true;                          // Filter by super trend on?
int                filter_bufferToBuy              = 8;
int                filter_bufferToSell             = 8;
int                filter_candles_back_for_signals = 1;
input int          st_calc_period                  = 10; // Calculate Period:
input int          st_calc_dev                     = 3;  // Calculate Deviation:

//---
double IndiFilter(int buffer, int candle = 1) { return iCustom(NULL, 0, indicator_file_filter, 0, "", 14,0,0, 
st_calc_period, st_calc_dev,
buffer, candle); }

class ConditionIndicatorBuyFilter : public iConditions
{
    double lastSignal;
  ...
  public:
    bool evaluate()
    {
        for (int i = 1; i <= filter_candles_back_for_signals; i++) {
            double indi = IndiFilter(filter_bufferToBuy, i);
            if (indi == EMPTY_VALUE) {
                // lastSignal = indi;
                return true;
            }
...
ConditionIndicatorBuyFilter conditionIndicatorBuyFilter;

class ConditionIndicatorSellFilter : public iConditions
{
    double lastSignal;
  ...
  public:
    ...
    {
        for (int i = 1; i <= filter_candles_back_for_signals; i++) {
            double indi = IndiFilter(filter_bufferToSell, i);
...
ConditionIndicatorSellFilter conditionIndicatorSellFilter;
...
bool OnInit_CustomIndicator()
{
    ...
    double temp2 = iCustom(NULL, 0, indicator_file_filter, 0, 0);
    if (GetLastError() == ERR_INDICATOR_CANNOT_LOAD) {
        MessageBox("THIS EA NEED AN INDICATOR\n install the file:\n" + indicator_file + "\ninto the folder:\nMQL4/Indicators.", "Important Information", MB_ICONINFORMATION);
        Alert("THIS EA NEED AN INDICATOR, install the file: " + indicator_file + " into the folder: MQL4/Indicators.");
        indicator_init = false;
    ...
    if(filter_on) {
        ShortStrategy.addCondition(&conditionIndicatorSellFilter);
        LongStrategy.addCondition(&conditionIndicatorBuyFilter);
    }
...
input bool   close_on_opposite_on_Mobo       = false;          // Close On Opposite Mobo Bands:
input bool   close_on_opposite_on_Supertrend = false;          // Close On Opposite Supertrend:
...
ConditionIndicatorBuyFilter  cdCloseOnOppositeSupertrendSell;
ConditionIndicatorSellFilter  cdCloseOnOppositeSupertrendBuy;

Strategy stCloseOppositeSuperTrendBuy;
Strategy stCloseOppositeSuperTrendSell;
...
void oninit_close_on_opposite()
{
    ...
    stCloseOppositeSuperTrendBuy.set(&cdCloseOnOppositeSupertrendBuy, &acCloseBuy);
    stCloseOppositeSuperTrendSell.set(&cdCloseOnOppositeSupertrendSell, &acCloseSell);

    if (close_on_opposite_on_Mobo) {
        Trader.management.add(&stCloseOppositeBuy);
        Trader.management.add(&stCloseOppositeSell);
    }
    if (close_on_opposite_on_Supertrend) {
        Trader.management.add(&stCloseOppositeSuperTrendBuy);
        Trader.management.add(&stCloseOppositeSuperTrendSell);
    }
}
