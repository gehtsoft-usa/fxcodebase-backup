...
//https://fxcodebase.com/code/viewtopic.php?f=38&p=157512#p157512
...
input const string indicator_file = "Mobo_Bands.ex5";            // Indicator File:
input int    periods              = 13;                          // Periods
input int    moboLength           = 10;                          // Mobo Length :
input double nStd                 = 1;                           //  Standard Deviation Bands:

input bool normalEntryOn = true; // Normal Entry On?
...
int  handle = 0;
void setHandle() { handle = iCustom(Symbol(), NULL, indicator_file, periods, moboLength, nStd); }
...
input bool earlyEntryOn = true; // Early Entry On?
int lineup = 0;
int linedn = 1;
int lineYellow = 5;

class ConditionEarlyBuy : public iConditions
{
    double lastSignal;
    ...
    {
        for (int i = 1; i <= candles_back_for_signals; i++) {
            double yellow = Indi(lineYellow, i);
            double yellow2 = Indi(lineYellow, i+1);
            double lidn = Indi(linedn, i);
            double lidn2 = Indi(linedn, i+1);
            if (yellow > lidn && yellow2 <= lidn2 ) {
                lastSignal = yellow;
                return true;
            }
        }
...
ConditionEarlyBuy cdEarlyBuy;

class ConditionEarlySell : public iConditions
{
    double lastSignal;
    ...
    bool evaluate()
    {
        for (int i = 1; i <= candles_back_for_signals; i++) {
            double yellow = Indi(lineYellow, i);
            double yellow2 = Indi(lineYellow, i+1);
            double liup = Indi(lineup, i);
            double liup2 = Indi(lineup, i+1);
            if (yellow < liup && yellow2 >= liup2 ) {
                lastSignal = yellow;
                return true;
            }
        }
...
ConditionEarlySell cdEarlySell;
...
bool OnInit_CustomIndicator()
{
    ...
    if(earlyEntryOn)
    {
        LongStrategy.addCondition(&cdEarlyBuy);
        ShortStrategy.addCondition(&cdEarlySell);
    }
...
#define Section_2_EMAS_Filter
#ifdef Section_2_EMAS_Filter

// Mark: Section_2_EMAS_Filter

input string             Iema1          = "== Moving Average Setup =="; // == Moving Average Fast Setup ==
input bool FilterbyEmasOn = true; // Filter By EMAS On?
...
input string             Iema2          = "== Moving Average Setup =="; // == Moving Average Slow Setup ==
input int                maSlowPeriod       = 100;                      // Period
...
void setHandleEMAfast() { handle_ma_fast = iMA(NULL, 0, maPeriod, maShift, maMethod, maAppliedPrice); }
void setHandleEMAslow() { handle_ma_slow = iMA(NULL, 0, maSlowPeriod, maSlowShift, maSlowMethod, maSlowAppliedPrice); }

double MAFast(int candle = 1)
{
    double value[1];
    int shift = candle;
    int copy  = CopyBuffer(handle_ma_fast, 0, shift, 1, value);
...
double MASlow(int candle = 1)
{
    double value[1];
    int shift = candle;
    int copy  = CopyBuffer(handle_ma_slow, 0, shift, 1, value);
...
class ConditionEMA_Filter_Buy : public iConditions
{
  public:
    bool evaluate()
    {
        return iClose(NULL,0,1) > MAFast(1) && MAFast(1) > MASlow(1);
    }
};
ConditionEMA_Filter_Buy cd_ema_trend_up;

class ConditionEMA_Filter_Sell : public iConditions
{
  public:
    bool evaluate()
    {
...
ConditionEMA_Filter_Sell cd_ema_trend_dn;

void OnInit_EMA_Filter()
{
    if(FilterbyEmasOn) {
        setHandleEMAfast();
        setHandleEMAslow();

        LongStrategy.addCondition(&cd_ema_trend_up);
        ShortStrategy.addCondition(&cd_ema_trend_dn);        
    }
}
