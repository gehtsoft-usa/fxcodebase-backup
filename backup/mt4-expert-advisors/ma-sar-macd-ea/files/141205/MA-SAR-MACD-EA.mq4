...
//http://fxcodebase.com/code/viewtopic.php?f=38&t=71012
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      ...
      double close = iClose(_symbol, _timeframe, period);
      return maValue < close && close > sarValue;
   }
};
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      double mainValue = iMACD(_symbol, _timeframe, fast_ema_period, slow_ema_period, signal_period, PRICE_CLOSE, MODE_MAIN, period);
      return mainValue > 0;
   }
};
