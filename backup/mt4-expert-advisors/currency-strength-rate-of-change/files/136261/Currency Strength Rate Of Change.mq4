...
//http://fxcodebase.com/code/viewtopic.php?f=38&t=70219
...
public:
   ...
   {
      return rocjpyindex[period] >= rocgbpindex[period] && rocjpyindex[period + 1] < rocgbpindex[period + 1];
   }
};
...
double jpyindex[], gbpindex[], rocjpyindex[], rocgbpindex[];
...
int init()
{
   ...
   IndicatorObjPrefix = GenerateIndicatorPrefix("csroc");
   IndicatorShortName("Currency Strength Rate Of Change");

   IndicatorBuffers(12);

   SetIndexStyle(0, DRAW_LINE);
   SetIndexBuffer(0, rocjpyindex);
   SetIndexLabel(0, "rocjpyindex");

   SetIndexStyle(1, DRAW_LINE);
   SetIndexBuffer(1, rocgbpindex);
   SetIndexLabel(1, "rocgbpindex");
   ...
   SetIndexStyle(id, DRAW_NONE);
   SetIndexBuffer(id, jpyindex);
   ++id;

   SetIndexStyle(id, DRAW_NONE);
   SetIndexBuffer(id, gbpindex);
   ++id;
...
double OLHC4(string symbol, ENUM_TIMEFRAMES timeframe, int period)
{
   return (iOpen(symbol, timeframe, period) 
      + iClose(symbol, timeframe, period)
      + iHigh(symbol, timeframe, period)
      + iLow(symbol, timeframe, period)) / 4;
}
                ...
                const int &spread[])
{
   ...
   {
      ArrayInitialize(rocjpyindex, EMPTY_VALUE);
      ArrayInitialize(rocgbpindex, EMPTY_VALUE);
      ArrayInitialize(jpyindex, EMPTY_VALUE);
      ArrayInitialize(gbpindex, EMPTY_VALUE);
      if (customStream != NULL)
      {
         customStream.Init();
      }
   ...
   {
      double sourcejpy = OLHC4("GBPJPY", (ENUM_TIMEFRAMES)_Period, pos) * 2
         + OLHC4("USDJPY", (ENUM_TIMEFRAMES)_Period, pos) * 2
         + OLHC4("EURJPY", (ENUM_TIMEFRAMES)_Period, pos)
         + OLHC4("AUDJPY", (ENUM_TIMEFRAMES)_Period, pos)
         + OLHC4("CHFJPY", (ENUM_TIMEFRAMES)_Period, pos)
         + OLHC4("CADJPY", (ENUM_TIMEFRAMES)_Period, pos)
         + OLHC4("NZDJPY", (ENUM_TIMEFRAMES)_Period, pos);
      double sourcegbp = OLHC4("JPYGBP", (ENUM_TIMEFRAMES)_Period, pos) * 2
         + OLHC4("USDGBP", (ENUM_TIMEFRAMES)_Period, pos) * 2
         + OLHC4("EURGBP", (ENUM_TIMEFRAMES)_Period, pos)
         + OLHC4("AUDGBP", (ENUM_TIMEFRAMES)_Period, pos)
         + OLHC4("CHFGBP", (ENUM_TIMEFRAMES)_Period, pos)
         + OLHC4("CADGBP", (ENUM_TIMEFRAMES)_Period, pos)
         + OLHC4("NZDGBP", (ENUM_TIMEFRAMES)_Period, pos);

      jpyindex[pos] = sourcejpy / 9.0;
      gbpindex[pos] = sourcegbp / 9.0;
      if (rates_total - 1 - toSkip < pos + length)
      {
         continue;
      }
      rocjpyindex[pos] = jpyindex[pos + length] == 0.0 ? 0.0 : 100.0 * (jpyindex[pos] - jpyindex[pos + length]) / jpyindex[pos + length];
      rocgbpindex[pos] = gbpindex[pos + length] == 0.0 ? 0.0 : 100.0 * (gbpindex[pos] - gbpindex[pos + length]) / gbpindex[pos + length];

      if (customStream != NULL)
      {
         customStream.SetValue(pos, rocjpyindex[pos]);
      }
      for (int i = 0; i < ArraySize(conditions); ++i)
      {
         AlertSignal* item = conditions[i];
