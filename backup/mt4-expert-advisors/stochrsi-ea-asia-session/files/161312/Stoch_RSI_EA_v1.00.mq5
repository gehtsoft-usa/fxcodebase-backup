...
#define EANAME "Stoch_RSI_EA"
int iCustom_handle;
string custom_indicator = "Stoch_RSI";
...
#define CUSTOM_INDICATOR_PARAMS InpStockKPeriod, InpStockDPeriod, InpRSIPeriod, InpStochastikPeriod, InpRSIAppliedPrice
...
enum globalPL
  {
   live = 0,            // Running normal
   hist = 1,            // Running normal and grid
   liveGrid = 2,        // Running and closed normal (from the first opened)
   histGrid = 3         // Running and closed normal and grid (from the first opened)
  };
enum gridLotSize
  {
   dis = 0,             // Disabled
...
enum trailingAndBe
  {
   disabled = 0,                             // Off
   normal = 1,                               // Normal positions
   grid = 2                                  // Normal and grid positions
  };
enum lMode
  {
...
input double maxLot = 1.00;                  // Maximum size of lots (All Normal and Grid positions)
...
input string st = "========== Global Close  =========="; // ========== Global Close  ==========
...
input string gr = "========== Grid  =========="; // ========== Grid  ==========
input bool gridOn = false;                   // Use Grid (works only with first buy and sell positions)
input int  gridMax = 5;                      // Max attempts per symbol
input gridLotSize gridLot = 0;               // Grid lot sizing type
input double gridLotSizing = 2.0;            // Value of multipling or adding
input double gridGap = 10;                   // Grid distance pip/point (as set in "Pip/Point mode" setting)
input closeAllBy gridCloseBy = 1;            // Close grid positions per symbol by
input double gridProfit = 50;                // Take Profit Grid per symbol
input double gridLoss = 50;                  // Stop Loss Grid per symbol
...
input string finishTime = "00:00";          // Finish time GMT (hh:mm)
input bool mandatoryClose = false;          // Mandatory Close all positions at end of session
bool initOK = true;
string symbols[];
...
void gridTrading()
  {
   ulong ticket;
   int numSym = ArraySize(symbols);
   int numBuyPos[], numSellPos[];
   bool openedInitBuy[], openedInitSell[];
   double minBuyPrice[], maxSellPrice[];
   double profitBuySum[], profitSellSum[];
   double initBuyLotSize[], initSellLotSize[];
   double nextBuyLotSize[], nextSellLotSize[];
   double nextBuyPrice[], nextSellPrice[];
   double price, initLot, profit;
   double eq = AccountInfoDouble(ACCOUNT_EQUITY);
   ArrayResize(numBuyPos, numSym);
   ArrayResize(numSellPos, numSym);
   ArrayResize(openedInitBuy, numSym);
   ArrayResize(openedInitSell, numSym);
   ArrayResize(minBuyPrice, numSym);
   ArrayResize(maxSellPrice, numSym);
   ArrayResize(profitBuySum, numSym);
   ArrayResize(profitSellSum, numSym);
   ArrayResize(initBuyLotSize, numSym);
   ArrayResize(initSellLotSize, numSym);
   ArrayResize(nextBuyLotSize, numSym);
   ArrayResize(nextSellLotSize, numSym);
   ArrayResize(nextBuyPrice, numSym);
   ArrayResize(nextSellPrice, numSym);
         ...
         if((ticket = PositionGetTicket(pos)) > 0)
           {
            ...
            if(PositionGetString(POSITION_SYMBOL) ==  symbols[i] && PositionGetInteger(POSITION_MAGIC) == (int)MagicNumber && MagicNumber > 0)
              {
               SymbolInfoTick(symbols[i], p);
               price = PositionGetDouble(POSITION_PRICE_OPEN);
               profit = PositionGetDouble(POSITION_PROFIT);
               initLot = PositionGetDouble(POSITION_VOLUME);
               if(StringFind(comment, "_ip#1", 0) >= 0 && PositionGetInteger(POSITION_TYPE) == POSITION_TYPE_BUY)
                 {
                  openedInitBuy[i] = true;
                  initBuyLotSize[i] = initLot;
                  if(minBuyPrice[i] == 0)
                     minBuyPrice[i] = price;
                 }
               if(StringFind(comment, "_ip#1", 0) >= 0 && PositionGetInteger(POSITION_TYPE) == POSITION_TYPE_SELL)
                 {
                  openedInitSell[i] = true;
                  initSellLotSize[i] = initLot;
                  if(maxSellPrice[i] == 0)
                     maxSellPrice[i] = price;
                 }
               if(PositionGetInteger(POSITION_TYPE) == POSITION_TYPE_BUY)
                 {
                  numBuyPos[i]++;
                  if(gridCloseBy == 1)
                     profitBuySum[i] += (p.bid - price) / pip(symbols[i]);
                  if(gridCloseBy == 2)
                     profitBuySum[i] += profit;
                  if(gridCloseBy == 3)
                     profitBuySum[i] += 1 / (eq * profit) * 100;
                  if(minBuyPrice[i] > 0 && minBuyPrice[i] > price)
                     minBuyPrice[i] = price;
                 }
               if(PositionGetInteger(POSITION_TYPE) == POSITION_TYPE_SELL)
                 {
                  numSellPos[i]++;
                  if(gridCloseBy == 1)
                     profitSellSum[i] += (price - p.ask) / pip(symbols[i]);
                  if(gridCloseBy == 2)
                     profitSellSum[i] += profit;
                  if(gridCloseBy == 3)
                     profitSellSum[i] += 1 / (eq * profit) * 100;
                  if(maxSellPrice[i] > 0 && maxSellPrice[i] < price)
                     maxSellPrice[i] = price;
                 }
              }
           }
   ...
   for(int i = 0; i < numSym; i++)
     {
      SymbolInfoTick(symbols[i], p);
      if(openedInitBuy[i] && numBuyPos[i] < gridMax)
        {
         if(gridLot == 0)
            nextBuyLotSize[i] = initBuyLotSize[i];
         if(gridLot == 1)
            nextBuyLotSize[i] = initBuyLotSize[i] * MathPow(gridLotSizing, numBuyPos[i]);
         if(gridLot == 2)
            nextBuyLotSize[i] = initBuyLotSize[i] + gridLotSizing * numBuyPos[i];;
         nextBuyPrice[i] = minBuyPrice[i] - gridGap * pip(symbols[i]);
         if(p.ask <= nextBuyPrice[i] && nextBuyPrice[i] > 0)
            gridPosOpening(symbols[i], nextBuyLotSize[i], 1, numBuyPos[i]);
        }
      if(openedInitSell[i] && numSellPos[i] < gridMax)
        {
         if(gridLot == 0)
            nextSellLotSize[i] = initSellLotSize[i];
         if(gridLot == 1)
            nextSellLotSize[i] = initSellLotSize[i] * MathPow(gridLotSizing, numSellPos[i]);
         if(gridLot == 2)
            nextSellLotSize[i] = initSellLotSize[i] + gridLotSizing * numSellPos[i];
         nextSellPrice[i] = maxSellPrice[i] + gridGap * pip(symbols[i]);
         if(p.bid >= nextSellPrice[i] && nextSellPrice[i] > 0)
            gridPosOpening(symbols[i], nextSellLotSize[i], 2, numSellPos[i]);
        }
      if(numBuyPos[i] > 1 && (profitBuySum[i] > gridProfit || profitBuySum[i] < gridLoss * (-1)))
         closeAll(1, symbols[i]);
      if(numSellPos[i] > 1 && (profitSellSum[i] > gridProfit || profitSellSum[i] < gridLoss * (-1)))
         closeAll(2, symbols[i]);
     }
  }
...
//|                                                                  |
//+------------------------------------------------------------------+
void gridPosOpening(string sym, double lot, int dir, int count)
  {
   int ticket;
   int dig = (int)SymbolInfoInteger(sym, SYMBOL_DIGITS);
   ...
   if(dir == 1)
     {
      ...
      request.deviation = slp;
      request.type_filling = OrderFill;
      request.comment = EAName + "_gr#" + (string)count;
      request.magic = MagicNumber;
      ticket = OrderSend(request, result);
      if(ticket < 0)
        {
         Alert(EAName + ": Grid OrderSend on " + sym + " failed with error #", GetLastError(),
               " lot: " + DoubleToString(lot, 2) +
               " price: " + DoubleToString(p.ask, dig));
        }
     }
   if(dir == 2)
     {
      ...
      if(ticket < 0)
        {
         Alert(EAName + ": Grid OrderSend on " + sym + " failed with error #", GetLastError(),
               " lot: " + DoubleToString(lot, 2) +
               " price: " + DoubleToString(p.bid, dig));
        }
     }
  }
      ...
      if((ticket = HistoryDealGetTicket(pos)) > 0)
        {
         ...
         string ordersym = HistoryDealGetString(ticket, DEAL_SYMBOL);
         long openTime = HistoryDealGetInteger(ticket, DEAL_TIME);
         if(StringFind(comment, "_gr#", 0) >= 0  && (globalCalcMode == 0 || globalCalcMode == 2))
            continue;
