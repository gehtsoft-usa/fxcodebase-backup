...
// http://fxcodebase.com/code/viewtopic.php?f=38&t=20206&start=10
...
input bool use_averages_check = false; // Check Middle line with MA of price
enum AveragesMethod
{
   SMA = MODE_SMA, // SMA
   EMA = MODE_EMA, // EMA
...
input AveragesMethod averages_method = SMA; // Averages method
input int averages_period = 14; // Averages period
...
class MiddleLineOverMA : public ABaseCondition
{
public:
   MiddleLineOverMA(const string symbol, ENUM_TIMEFRAMES timeframe)
   ...
   {
      double value0 = iCustom(_symbol, _timeframe, "Dochian_Channel", Length, Mode, 1, period);
      double avg = iCustom(_symbol, _timeframe, "averages", averages_period, PRICE_CLOSE, averages_method, 0, period);
      return value0 > avg;
   }
};

class MiddleLineUnderMA : public ABaseCondition
{
public:
   MiddleLineUnderMA(const string symbol, ENUM_TIMEFRAMES timeframe)
      :ABaseCondition(symbol, timeframe)
   {
...
class NewHighCondition : public ABaseCondition
{
public:
   NewHighCondition(const string symbol, ENUM_TIMEFRAMES timeframe)
      :ABaseCondition(symbol, timeframe)
   {
   ...
   bool IsPass(const int period)
   {
      ...
      while (it.Next())
      {
         if (dt == 0 || dt > it.GetOpenTime())
            dt = it.GetOpenTime();
      }
      if (dt == 0)
         return false;

      int indexMain = iBarShift(_symbol, _timeframe, dt);
      double maxDonchian = 0.0;
      for (int i = indexMain; i >= period + 1; --i)
      {
         double value0 = iCustom(_symbol, _timeframe, "Dochian_Channel", Length, Mode, 1, i);
         if (maxDonchian < value0)
            maxDonchian = value0;
      }
      double donchianCurrent = iCustom(_symbol, _timeframe, "Dochian_Channel", Length, Mode, 1, period);
      if (donchianCurrent <= maxDonchian || maxDonchian == 0.0)
         return false;

      int indexD1 = iBarShift(_symbol, PERIOD_D1, dt);
      int index;
      if (indexD1 > period + 1)
      {
         double maxAtr = 0.0;
         for (int i = indexD1; i >= period + 1; --i)
         {
            double value = iATR(_symbol, _timeframe, atr_period, i);
            if (maxAtr < value)
               maxAtr = value;
         }
         if (maxAtr < iATR(_symbol, _timeframe, atr_period, period))
            return false;

         index = iBarShift(_symbol, PERIOD_M1, MathMax(iTime(_symbol, PERIOD_D1, 0), dt));
      }
      else
         index = iBarShift(_symbol, PERIOD_M1, dt);

      int highestIndex = iHighest(_symbol, _timeframe, MODE_HIGH, index, index);
      double highest = iHigh(_symbol, _timeframe, highestIndex);
      if (iHigh(_symbol, _timeframe, period) < highest)
         return false;
...
class NewLowCondition : public ABaseCondition
{
public:
   NewLowCondition(const string symbol, ENUM_TIMEFRAMES timeframe)
      :ABaseCondition(symbol, timeframe)
   {
   ...
   bool IsPass(const int period)
   {
      ...
      int indexMain = iBarShift(_symbol, _timeframe, dt);
      double minDonchian = DBL_MAX;
      for (int i = indexMain; i >= period + 1; --i)
      {
         double value0 = iCustom(_symbol, _timeframe, "Dochian_Channel", Length, Mode, 1, i);
         if (minDonchian > value0)
            minDonchian = value0;
      }
      double donchianCurrent = iCustom(_symbol, _timeframe, "Dochian_Channel", Length, Mode, 1, period);
      if (donchianCurrent >= minDonchian || minDonchian == DBL_MAX)
         return false;
      ...
      if (indexD1 > period + 1)
      {
         ...
         if (minAtr > iATR(_symbol, _timeframe, atr_period, period))
            return false;
      ...
      int lowestIndex = iLowest(_symbol, _timeframe, MODE_HIGH, index, index);
      double lowest = iLow(_symbol, _timeframe, lowestIndex);
      if (iLow(_symbol, _timeframe, period) > lowest)
         return false;
...
ICondition* CreateLongCondition(string symbol, ENUM_TIMEFRAMES timeframe)
{
   ...
   if (!use_averages_check)
      return (ICondition *)new CrossUnderBottomCondition(symbol, timeframe);

   AndCondition* and = new AndCondition();
   and.Add(new CrossUnderBottomCondition(symbol, timeframe));
   and.Add(new MiddleLineOverMA(symbol, timeframe));
   OrCondition* or = new OrCondition();
   or.Add(new NewHighCondition(symbol, timeframe));
   or.Add(and);
   return and;
}
...
ICondition* CreateShortCondition(string symbol, ENUM_TIMEFRAMES timeframe)
{
   ...
   AndCondition* and = new AndCondition();
   and.Add(new CrossOverTopCondition(symbol, timeframe));
   and.Add(new MiddleLineOverMA(symbol, timeframe));
   OrCondition* or = new OrCondition();
   or.Add(new NewLowCondition(symbol, timeframe));
   or.Add(and);
   return and;
}
