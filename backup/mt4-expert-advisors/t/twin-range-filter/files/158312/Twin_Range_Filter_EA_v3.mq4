...
// https://fxcodebase.com/code/viewtopic.php?f=38&p=156206#p156206
...
#ifdef tp_candles_on
      case tp_by_candles:
         return TakeProfitByCandles(side, symbol);
#endif
     }
   return 0.00;
  }
...
//|                                                                  |
//+------------------------------------------------------------------+
input int    tp_candles_shift    = 2; // Candles Shift (high/low)
input double tp_pips_from_candle = 5; // Pips From High Low

//+------------------------------------------------------------------+
//|                                                                  |
//+------------------------------------------------------------------+
double TakeProfitByCandles(const string side, const string symbol, ENUM_TIMEFRAMES tf = 0)
  {
   double r = 0;
   if(side == "sell")
     {
      r = iLow(symbol, tf, iLowest(symbol, tf, MODE_LOW, sl_candles_shift, 1));
      r -= tp_pips_from_candle * 10 * MarketInfo(symbol, MODE_POINT);
      // r = StopLossSafety("buy", symbol, r);
     }
   if(side == "buy")
     {
...
input string TclosePositions      = "== Close by RSI =="; // ————————————————————————
input bool   close_by_strategy_on = false;                   // Close by RSI on:
   ...
   bool              evaluate()
     {
      ...
      if(OrderType() == OP_BUY)
        {
         r = iRSI(NULL, 0, rsi_periods, PRICE_CLOSE, 1) > ob_level;
        }
      return r;
     }
  };
   ...
   bool              evaluate()
     {
      ...
      if(OrderType() == OP_SELL)
        {
         r = iRSI(NULL, 0, rsi_periods, PRICE_CLOSE, 1) < os_level;
        }
      return r;
     }
  };
   ...
   bool              execute()
     {
      ...
      Notifications(_side, _symbol, _price, _sl, _tp, "");
      return result > 0;
     }
  };
...
input const string indicator_file = "Twin_Range_Filter";         // Indicator File:
input int          source         = 0;                           // Source
input int          Fast_period    = 27;                          // Fast Period
input double       Fast_range     = 1.6;                         // Fast Range
input int          Slow_period    = 55;                          // Slow Period
input double       Slow_range     = 2.0;                         // Slow Range
input bool maFilter = true; // MA Filter
input int maPeriodFast = 50; // Fast MA Period
input ENUM_MA_METHOD maMethodFast = MODE_EMA; // Fast MA Method
input ENUM_APPLIED_PRICE maPriceFast = PRICE_CLOSE; // Fast MA Price
input int maPeriodSlow = 100; // Slow MA Period
input ENUM_MA_METHOD maMethodSlow = MODE_EMA; // Slow MA Method
input ENUM_APPLIED_PRICE maPriceSlow = PRICE_CLOSE; // Slow MA Price
input string al = "====  Alert Settings  ===="; // ————————————————————————
...
double Indi(int buffer, int candle = 1)
  {
   return iCustom(NULL, 0, indicator_file, source, Fast_period, Fast_range, Slow_period, Slow_range, buffer, candle);
  }
      ...
      for(int i = 1; i <= candles_back_for_signals; i++)
        {
         ...
         double indi2 = super_trend_cci(0, i);
         if(!supertrendcci_on)
           {
            if(indi > 0 && indi != EMPTY_VALUE && indi != lastSignal && maCheck(1, 1))
              {
               lastSignal = indi;
               return true;
              }
           ...
           {
            if(indi > 0 && indi != EMPTY_VALUE && indi > 0 && indi2 != EMPTY_VALUE && indi2 != lastSignal && maCheck(1, 1))
              {
               lastSignal = indi;
               return true;
              }
      ...
      for(int i = 1; i <= candles_back_for_signals; i++)
        {
         ...
         double indi2 = super_trend_cci(1, i);
         if(!supertrendcci_on)
           {
            if(indi > 0 && indi != EMPTY_VALUE && indi != lastSignal && maCheck(-1, 1))
              {
...
//|                                                                  |
//+------------------------------------------------------------------+
bool maCheck(int dir, int bar)
  {
   if(maFilter == false)
      return true;
   double maFast = iMA(Symbol(), PERIOD_CURRENT, maPeriodFast, 0, maMethodFast, maPriceFast, bar);
   double maSlow = iMA(Symbol(), PERIOD_CURRENT, maPeriodSlow, 0, maMethodSlow, maPriceSlow, bar);
   if(dir == 1 && maFast > maSlow)
      return true;
   if(dir == -1 && maFast < maSlow)
      return true;
   return false;
...
//|                                                                  |
//+------------------------------------------------------------------+
void Notifications(string type, string sym, double price, double stoploss, double takeprofit, string comment)
  {
   string text = "Twin Range Filter EA: ";
   if(type == "buy")
      text += sym + " " + GetTimeFrame(_Period) + " BUY (price: " + (string)price + " sl: " + (string)stoploss + " tp: " + (string)takeprofit + ")";
   if(type == "sell")
      text += sym + " " + GetTimeFrame(_Period) + " SELL (price: " + (string)price + " sl: " + (string)stoploss + " tp: " + (string)takeprofit + ")";
