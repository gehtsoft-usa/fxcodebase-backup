...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=75044
...
double             uLots         = userLots;          // Fixed Lots:
...
bool OnInit_GUI()
{
    ...
    if (gui.reason() != REASON_CHARTCHANGE && gui.reason() != REASON_TEMPLATE && gui.reason() != REASON_PARAMETERS) {
        ...
        int y1 = 0;
        int y2 = 250;
        res = gui.Create(0, "Panel Buy - Sell", 0, x1, y1, x2, y2);
        if (res) gui.Run();
    }
    return res;
}
...
class BuyAction : public iActions
{
    bool doAction()
    {
        ...
        SendOrder              = new SendNewOrder("buy", l, "", pr, SL("buy", pr), TP("buy", pr), magico);
...
class SellAction : public iActions
{
    bool doAction()
    {
        ...
        double l = (double)gui.editLots.Text();
        SendOrder              = new SendNewOrder("sell", l, "", pr, SL("sell", pr), TP("sell", pr), magico);
        bool result            = SendOrder.doAction();
        delete SendOrder;
        return result;
    }
...
class GUI : public CAppDialog
{
    ...
    CLabel  lb1, lb2, lb3, lb4, lbStartBalance, lbEndBalance, lb7, lb8, lb9;
    CButton bt1, bt2, bt3;
    CEdit   edit1, edit2, editLots;
    CEdit   editTPpips, editSLpips;
    ...
    void updateLabel(CLabel &lb, string tx)
    {
      lb.Text(tx);
    }
    ...
    bool Create(const long chart, const string name, const int subwin, const int x1, const int y1, const int x2, const int y2)
    {
        ...
        if(!Create_Edit("Pips TP",         Col(1), Row(2), _high, _width, editTPpips)) return false;
        if(!Create_Edit("Pips SL",         Col(1), Row(3), _high, _width, editSLpips)) return false;
        ...
        if(!Create_label("Lots:",          Col(0), Row(4), _high, _width, lb4)) return false;
        if(!Create_Edit("Lots",            Col(1), Row(4), _high, _width, editLots)) return false;
        editLots.Text(DoubleToString(uLots, 2));
  
        if(!Create_button("Close All", Col(0), Row(5), _high, _width, bt3)) return false;
        
        if(!Create_label("Start Balance:", Col(0), Row(6), _high, _width, lbStartBalance)) return false;
        if(!Create_label("End Balance:",   Col(0), Row(7), _high, _width, lbEndBalance)) return false;
    ...
    void HoverEvents(const int id, const long& lparam, const double& dparam, const string& sparam)
    {
        ...
        if(bt2.IsActive()) bt2.ColorBackground(Tomato); else bt2.ColorBackground(CONTROLS_BUTTON_COLOR_BG);
    ...
    void OnEndEdit_editLots()   { uLots      = (int) editLots.Text(); Print(uLots); }
    void OnClick_button1() { button1Action.doAction(); }
    void OnClick_button2() { button2Action.doAction(); }
    void OnClick_button3() 
    { 
      button3Action.doAction(); 
      string sBalance = DoubleToString(AccountInfoDouble(ACCOUNT_BALANCE), 2);
      lbEndBalance.Text("Balance End:       " + sBalance);
    }
...
string sBalance = DoubleToString(AccountInfoDouble(ACCOUNT_BALANCE), 2);
gui.lbStartBalance.Text("Balance Start:     " + sBalance);
gui.lbEndBalance.Text(  "Balance End:       " + "0.00");
