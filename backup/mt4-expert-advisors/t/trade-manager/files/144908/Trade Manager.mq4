...
//https://fxcodebase.com/code/viewtopic.php?f=38&t=71839
...
input string       T01                     = "== Filters Orders ==";     // == Filters Orders ==
input bool         filterSymbolsOn         = false;                      // Use symbols filter?
input string       UserSymbols             = "GBPUSD,EURUSD";            // Symbols (separate by comma ","):
input bool         filterMagicsOn          = false;                      // Use magic number filter?
input string       UserMagics              = "0";                        // Magics numbers (separate by comma ","):
...
input bool         partialCloseOn          = true;                       // Use Partial Close ?
...
input bool         breakevenOn             = true;                       // Use Breakeven?
...
class OrdersList
{
  ...
  public:
   ...
   OrdersList(bool useMagic,bool useSymbols)
   {
      _useMagic = useMagic;
      _useSymbols = useSymbols;
      
      if (useMagic) { magics = new FilterByMagics(UserMagics); }
      if (useSymbols) { symbols = new FilterBySymbols(UserSymbols); }
   ...
   void GetMarketOrders()
   {
      for (int i = OrdersTotal() - 1; i >= 0; i--)
      {
         if (OrderSelect(i, SELECT_BY_POS, MODE_TRADES))
         {
            ...
            if (_useMagic) if (!magics.control(OrderMagicNumber())) { continue; }
            if (_useSymbols) if (!symbols.control(OrderSymbol())) { continue; }
...
OrdersList ordersList(filterMagicsOn, filterSymbolsOn);
...
int OnInit()
{
   ...
   // conditionsToBuy.AddCondition(countBuys = new ConditionCountBuys(2));
   ...
   // conditionsToSell.AddCondition(countSells = new ConditionCountSells(2));
...
void doPartialCloseAction()
{
   for (int i = ordersList.qnt() - 1; i >= 0; i--)
   {
      if (ordersList.index(i).countPartials() == 0)
      {
         partialCloseCondition.setOrder(ordersList.index(i));
         if (conditionsToPartialClose.EvaluateConditions())
         {
            partialCloseAction = new PartialClose();
            partialCloseAction.order(ordersList.index(i)).percent(userPartialClosePercent);
