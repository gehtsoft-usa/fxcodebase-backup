...
// http://fxcodebase.com/code/viewtopic.php?f=27&t=69183
//+------------------------------------------------------------------+
...
input int bars_since_last_trade = 30; // Min bars since last trade
input int Fast_MA_Period = 30;
input ENUM_MA_METHOD Fast_MA_Method = MODE_SMA; // Fast MA Method
input ENUM_APPLIED_PRICE Fast_MA_Price_Type = PRICE_CLOSE;
input int Slow_MA_Period = 100;
input ENUM_MA_METHOD Slow_MA_Method = MODE_SMA; // Slow MA Method
input ENUM_APPLIED_PRICE Slow_MA_Price_Type = PRICE_CLOSE;
input ENUM_MA_METHOD third_ma_method = MODE_SMA; // Third MA method
input int third_ma_period = 10; // Third MA period
input ENUM_TIMEFRAMES third_ma_timeframe = PERIOD_CURRENT; // Third MA timeframe
input double Max_Pips_Away_From_Cross      = 10;
...
ICondition* CreateLongCondition(string symbol, ENUM_TIMEFRAMES timeframe)
{
   ...
   condition.Add(new MACrossOverMACondition(symbol, timeframe, Fast_MA_Method, Fast_MA_Period, Slow_MA_Method, Slow_MA_Period), false);
   condition.Add(new MAAbovePriceCondition(symbol, timeframe, Slow_MA_Method, Slow_MA_Period, Max_Pips_Away_From_Cross), false);
   condition.Add(new MAAbovePriceCondition(symbol, timeframe, Fast_MA_Method, Fast_MA_Period), false);
   condition.Add(new MAAboveMACondition(symbol, timeframe, Fast_MA_Method, Fast_MA_Period, third_ma_timeframe, third_ma_method, third_ma_period), false);
   condition.Add(new MAAboveMACondition(symbol, timeframe, Slow_MA_Method, Slow_MA_Period, third_ma_timeframe, third_ma_method, third_ma_period), false);
...
ICondition* CreateShortCondition(string symbol, ENUM_TIMEFRAMES timeframe)
{
   ...
   AndCondition* condition = new AndCondition();
   condition.Add(new MACrossUnderMACondition(symbol, timeframe, Fast_MA_Method, Fast_MA_Period, Slow_MA_Method, Slow_MA_Period), false);
   condition.Add(new MABelowPriceCondition(symbol, timeframe, Slow_MA_Method, Slow_MA_Period, Max_Pips_Away_From_Cross), false);
   condition.Add(new MABelowPriceCondition(symbol, timeframe, Fast_MA_Method, Fast_MA_Period), false);
   condition.Add(new MABelowMACondition(symbol, timeframe, Fast_MA_Method, Fast_MA_Period, third_ma_timeframe, third_ma_method, third_ma_period), false);
   condition.Add(new MABelowMACondition(symbol, timeframe, Slow_MA_Method, Slow_MA_Period, third_ma_timeframe, third_ma_method, third_ma_period), false);
   condition.Add(new MinDistanceSinceLastTradeCondition(symbol, timeframe, bars_since_last_trade, magic_number), false);
   #ifdef ACT_ON_SWITCH_CONDITION
      ActOnSwitchCondition* switchCondition = new ActOnSwitchCondition(symbol, timeframe, (ICondition*) condition);
      condition.Release();
