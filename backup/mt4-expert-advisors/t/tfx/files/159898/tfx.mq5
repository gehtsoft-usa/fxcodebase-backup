...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=76146
...
// #define Section_Candle_Close_Condition
#ifdef Section_Candle_Close_Condition
...
input entry_mode user_entry = ema_mode;            // Select Entry Mode:
input bool       Pyramid    = true;                // Pyramid:
input double     Next_Trade = 200;                 // Points Next Trade:

input int    bars_look_back    = 1;    // Bars Look Back
input double EntryLag_long     = 2;    // Extra Points Long
input double EntryLag_short    = 2;    // Extra Points Short
input bool   Last_Entry_Filter = true; // Last Entry Filter
...
input double        uLotsValue       = 0.1;                         // value to calculate Lots:
input bool          lot_increment_on = true;                        // Lot Incrementation On
input double        lot_increment    = 0.1;                         // Incrementation
...
double LotsCalculation()
{
    ...
    case lot_fix:
        lots = uLotsValue;
        if (lot_increment_on && countTrades.total() > 0) {
            lots = last_lot + lot_increment;
        }
        break;
#endif
    }
    ...
    bool execute()
    {
        for (int i = OrdersTotal() - 1; i >= 0; i--) {
            ...
            if (OrderSelect(tk)) {
                ...
                if (tp != ORDER_TYPE_BUY && tp != ORDER_TYPE_SELL) {
  ...
  public:
    bool evaluate()
    {
        ...
        for (int j = PositionsTotal() - 1; j >= 0; j--) {
            ...
            if (PositionGetSymbol(j) == Symbol() && PositionGetInteger(POSITION_MAGIC) == magico) {
                current_profit += PositionGetDouble(POSITION_PROFIT) + PositionGetDouble(POSITION_SWAP);
            }
        }
        if (current_profit >= profit_limit && profit_limit > 0) 
        {
            resetvalues();
            return true;
        }
        if (current_profit < 0 && -current_profit >= loss_limit && loss_limit > 0){
...
void OnInit_CloseByMoney()
{
    ...
    stCloseByMoney.set(&cdCloseByMoney, &acCloseAll);
    stCloseByMoney.addAction(&acDeletePendings);
...
#define Section_One_trade_BY_Candle
#ifdef Section_One_trade_BY_Candle
// Mark: Section_One_trade_BY_Candle

bool     can_open_buy     = true;
bool     can_open_sell    = true;
datetime last_candle_time = 0;

void TurnOff_Buy() { can_open_buy = false; }
void TurnOff_Sell() { can_open_sell = false; }

void OnTick_One_trade_By_Candle()
{
...
{
  public:
    // TODO: condition EMA BUY
    bool evaluate()
    {
        bool r = false;
        if (countTrades.total() == 0) {
            if (MA1(1) > MA1(2)) {
                r = true;
            } else {
                r = false;
...
BUYcondition1 cd_ema_buy;

class SELLcondition1 : public iConditions
{
  public:
    // TODO: condition EMA SELL
    bool evaluate()
    {
        bool r = false;
        if (countTrades.total() == 0) {
...
SELLcondition1 cd_ema_sell;

// busca el max y min de bars_look_back candles hacia atrás
// ask > a max + EntryLag_long --> BUY
// bid < a min - EntryLag_short --> sell
// !Pyramid invierte la lógica
double Maximun()
{
    double maximo = iHigh(_Symbol, _Period, 1); // Empieza en la vela anterior
    for (int i = 2; i <= bars_look_back; i++) {
...
double Minimum()
{
    double minimo = iLow(_Symbol, _Period, 1); // Empieza en la vela anterior
    for (int i = 2; i <= bars_look_back; i++) {
...
// Necesito que cuando se abra un trade me actualice esta data:
// Last_Buy: precio de la ultima compra
// Last_Sell: precop de la ultima venta
// Last_Type: typo de la ultima orden
double Last_Buy  = 0;
double Last_Sell = 0;
int    Last_Type = -1; // 0 = BUY, 1 = SELL
double last_lot  = 0;
...
void resetvalues()
{
    Last_Buy  = 0;
    Last_Sell = 1000000;
    Last_Type = -1; // 0 = BUY, 1 = SELL
    last_lot  = 0;
}

void UpdateLastTradeData(ulong ticket)
{
...
class ConditionKayvanBuy : public iConditions
{
  public:
    bool evaluate()
    {
        ...
        double Ask = SymbolInfoDouble(_Symbol, SYMBOL_ASK);
        if (countTrades.total() == 0) {
            if (Ask > Maximun() + EntryLag_long * _Point) {
                r = true;
            }
        } else {
            if (Ask > Maximun() + EntryLag_long * _Point) {
                if (Last_Entry_Filter) {
                    if (Ask > Last_Buy && Last_Type == 1) r = true;
                } else {
                    r = true;
                }
            }
...
ConditionKayvanBuy cd_kayvan_buy;
class ConditionKayvanSell : public iConditions
{
  public:
    bool evaluate()
    {
        ...
        double Bid = SymbolInfoDouble(_Symbol, SYMBOL_BID);
        if (countTrades.total() == 0) {
            if (Bid < Minimum() - EntryLag_short * _Point) {
                r = true;
            }
        } else {
            if (Bid < Minimum() - EntryLag_short * _Point) {
                if (Last_Entry_Filter) {
                    if (Bid < Last_Sell && Last_Type == 0) r = true;
                } else {
                    r = true;
                }
            }
...
ConditionKayvanSell cd_kayvan_sell;

// class ConditionOneBuyPerCandle : public iConditions
// {
//   public:
//     bool evaluate()
//     {
...
//         datetime current_candle_time = iTime(_Symbol, _Period, 0);
//         Print("line: ", __LINE__, " current_candle_time: ", current_candle_time);

//         // Si es una nueva vela, resetea el flag (no necesario flag, solo chequeo)
//         // if (current_candle_time != last_candle_time) {
//         // last_candle_time = current_candle_time;
//         // }
//         // Buscar si ya hay un BUY abierto en esta vela
//         for (int i = PositionsTotal() - 1; i >= 0; i--) {
//             if (PositionGetSymbol(i) != _Symbol) continue;
//             int      type      = (int)PositionGetInteger(POSITION_TYPE);
//             datetime open_time = (datetime)PositionGetInteger(POSITION_TIME);
//             if (type == POSITION_TYPE_BUY && open_time >= current_candle_time) {
//                 Print("line: ", __LINE__, " open_time: ", open_time);
//                 return false; // Ya hay un BUY abierto en esta vela
//             }
//         }

//         // Buscar si ya hubo un BUY cerrado en esta vela (en el historial)
...
//             // Solo BUY y solo aperturas (DEAL_ENTRY_IN)
//             if (symbol == _Symbol && entry == DEAL_ENTRY_IN && type == DEAL_TYPE_BUY && open_time >= current_candle_time) {
//                 return false; // Ya hubo un BUY (aunque esté cerrado) en esta vela
//             }
//         }

//         return true; // No hay ni hubo BUY en esta vela, se puede abrir
//     }
// };

// class ConditionOneSellPerCandle : public iConditions
// {
//   public:
//     bool evaluate()
//     {
...
//         // Buscar si ya hay un SELL abierto en esta vela
//         for (int i = PositionsTotal() - 1; i >= 0; i--) {
//             if (PositionGetSymbol(i) != _Symbol) continue;
//             int      type      = (int)PositionGetInteger(POSITION_TYPE);
//             datetime open_time = (datetime)PositionGetInteger(POSITION_TIME);
//             if (type == POSITION_TYPE_SELL && open_time >= current_candle_time) {
//                 return false; // Ya hay un SELL abierto en esta vela
//             }
//         }

//         // Buscar si ya hubo un SELL cerrado en esta vela (en el historial)
//         for (int i = HistoryDealsTotal() - 1; i >= 0; i--) {
//             ulong    ticket    = HistoryDealGetTicket(i);
//             string   symbol    = HistoryDealGetString(ticket, DEAL_SYMBOL);
//             int      entry     = (int)HistoryDealGetInteger(ticket, DEAL_ENTRY);
...
//             // Solo SELL y solo aperturas (DEAL_ENTRY_IN)
//             if (symbol == _Symbol && entry == DEAL_ENTRY_IN && type == DEAL_TYPE_SELL && open_time >= current_candle_time) {
//                 return false; // Ya hubo un SELL (aunque esté cerrado) en esta vela
//             }
//         }

//         return true; // No hay ni hubo SELL en esta vela, se puede abrir
//     }
// };
// ConditionOneBuyPerCandle  cdOneBuy;
// ConditionOneSellPerCandle cdOneSell;
...
{
    if (user_entry == ema_mode) {
        if (Pyramid) {
            LongStrategy.set(&cd_ema_buy, &buy);
            ShortStrategy.set(&cd_ema_sell, &sell);
        } else {
            LongStrategy.set(&cd_ema_buy, &sell);
            ShortStrategy.set(&cd_ema_sell, &buy);
        }
    }
    if (user_entry == kayvan_mode) {
        if (Pyramid) {
            LongStrategy.set(&cd_kayvan_buy, &buy);
            ShortStrategy.set(&cd_kayvan_sell, &sell);
        } else {
            LongStrategy.set(&cd_kayvan_buy, &sell);
            ShortStrategy.set(&cd_kayvan_sell, &buy);
        }
        // LongStrategy.addCondition(&cdOneBuy);
        // ShortStrategy.addCondition(&cdOneSell);
    }
...
double buy_price;
double sell_price;
void   UpdateNextPrices()
{
    double Ask = SymbolInfoDouble(_Symbol, SYMBOL_ASK);
    double Bid = SymbolInfoDouble(_Symbol, SYMBOL_BID);

    double gap = Next_Trade * _Point;
    buy_price  = Ask + gap;
    sell_price = Bid - gap;
}
...
input int    count_max_positions = 100;                         // # Total positions:
input int    count_max_buys      = 100;                         // # Buys limit:
input int    count_max_sells     = 100;                         // # Sells limit:
...
ConditionCountBuys cdCountBuys;
...
ConditionCountSells cdCountSells;
...
ConditionCountPositions cdCountPositions;
...
CountPositions countTrades;

void oninit_counters_filters()
{
    Trader.filters.add(&cdCountPositions);
    ShortStrategy.addCondition(&cdCountPositions);
    LongStrategy.addCondition(&cdCountPositions);

    ShortStrategy.addCondition(&cdCountSells);
    LongStrategy.addCondition(&cdCountBuys);
}
