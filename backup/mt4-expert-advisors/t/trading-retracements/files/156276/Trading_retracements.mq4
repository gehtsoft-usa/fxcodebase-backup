...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=75107
...
input double    pips_between = 20;                  // Pips Between Trades
...
input string tcloseall    = "== Close All by money =="; // ————————————————————————
input bool   closeAll_on  = true;                       // Close by money on?
...
#define Section_CloseByRetracement
#ifdef Section_CloseByRetracement
// Mark: Section_CloseByRetracement

input string tclosebyretracement     = "== Close by retracement =="; // ————————————————————————
input bool   close_by_retracement_on = true;                         // Close by Retracement on?
input double retracement_buy         = 25;                           // Retracement % Buy (0 = off)
input double retracement_sell        = 25;                           // Retracement % Sell (0 = off)

class ConditionCloseByRetracementBuy : public iConditions
{
  public:
    bool evaluate()
    {
        if (retracement_buy == 0) {
            return false;
        }
        ...
        if (Bid >= lo + range * (retracement_buy / 100)) {
            return true;
        }
...
ConditionCloseByRetracementBuy cdCloseByRetracement_buy;

class ConditionCloseByRetracementSell : public iConditions
{
  public:
    bool evaluate()
    {
        if (retracement_sell == 0) {
            return false;
        }
        ...
        // all sells are winning?
        for (int i = OrdersTotal() - 1; i >= 0; i--) {
            if (OrderSelect(i, SELECT_BY_POS) && OrderSymbol() == _Symbol && OrderMagicNumber() == magico) {
                if (OrderType() == OP_SELL && OrderProfit() < 0) return false;
            }
        }

        if (Ask <= hi - range * (retracement_sell / 100)) {
            return true;
        }
...
ConditionCloseByRetracementSell cdCloseByRetracement_sell;
    ...
    bool execute()
    {
        for (int i = OrdersTotal() - 1; i >= 0; i--) {
            if (OrderSelect(i, SELECT_BY_POS) && OrderSymbol() == _Symbol && OrderMagicNumber() == magico && OrderType() == OP_BUY) {
                int r = OrderClose(OrderTicket(), OrderLots(), Bid, 10000, clrGray);
            }
        }
        return true;
    }
    ...
    bool execute()
    {
        for (int i = OrdersTotal() - 1; i >= 0; i--) {
            if (OrderSelect(i, SELECT_BY_POS) && OrderSymbol() == _Symbol && OrderMagicNumber() == magico && OrderType() == OP_SELL) {
                int r = OrderClose(OrderTicket(), OrderLots(), Ask, 10000, clrGray);
            }
        }
        return true;
    }
...
Strategy stCloseByRetracementBuys;
Strategy stCloseByRetracementSells;

void OnInit_CloseByRetracement()
{
    stCloseByRetracementBuys.set(&cdCloseByRetracement_buy, &acCloseBuys);
    stCloseByRetracementSells.set(&cdCloseByRetracement_sell, &acCloseSells);
    Trader.management.add(&stCloseByRetracementBuys);
    Trader.management.add(&stCloseByRetracementSells);
}
...
class SELLcondition1 : public iConditions
{
  ...
  public:
    ...
    {
        double lastHigh = 0;
        int    iniDay   = iTime(NULL, PERIOD_D1, 0);
        double h        = 0;
        int    i        = 2;

        while (iTime(NULL, 0, i) >= iniDay) {
            h = iHigh(NULL, 0, i);
            if (h > lastHigh) {
                lastHigh = h;
            }
...
class ConditionSellDistance : public iConditions
{
  public:
    bool evaluate()
    {
        double distance = pips_between * _Point * 10;
        double maxPrice = 0;

        for (int i = OrdersTotal() - 1; i >= 0; i--) {
            if (OrderSelect(i, SELECT_BY_POS) && OrderSymbol() == _Symbol && OrderMagicNumber() == magico) {
                if (OrderType() == OP_SELL) {
                    double pr = OrderOpenPrice();
                    if (pr > maxPrice) {
                        maxPrice = pr;
                    }
                }
        ...
        return Bid >= maxPrice + distance;
    }
};
ConditionSellDistance cdSellDistance;

class ConditionBuyDistance : public iConditions
{
  public:
    bool evaluate()
    {
        ...
        return Ask <= minPrice - distance;
    }
};
ConditionBuyDistance cdBuyDistance;
...
void BasicStrategy_Iniciate()
{
    ...
    LongStrategy.addCondition(&cdBuyDistance);
    ShortStrategy.addCondition(&cdSellDistance);
...
int OnInit()
{
    ...
    if (close_by_retracement_on) {
        OnInit_CloseByRetracement();
    }
