...
bool normalSignal = false;

#ifdef CONTROL_CUSTOM_INDICATOR_FILE

input string             Tindicator           = "== Indicator Setup ==";  // ————————————
input int                indi_HalfLength      = 20;
input ENUM_APPLIED_PRICE indi_Price           = PRICE_WEIGHTED;
input double             indi_BandsDeviations = 2.3;  // 2.7; //1.618;

string file_custom_indicator = "TMA_CG_special";
int    handle                = 0;
int    bufferToBuy           = 3;
int    bufferToSell          = 4;
int    bufferToBuy_strong    = 5;
int    bufferToSell_strong   = 6;
int    bufferCentral         = 0;

void setHandle()
{
  handle = iCustom(Symbol(), 0, file_custom_indicator,
                   indi_HalfLength, indi_Price, indi_BandsDeviations);
}
...
input double       userBalancePerStrong = 0.1;                // Account % strong signal:
input double       userBalancePer       = 0.05;               // Account % normal signal:
...
input double       userTPpips           = 0;                  // Pips TP

enum SL_Mode {
  slmode_byPips,   // Pips
  slmode_byCandle  // Candle
};
input string  T02                = "- Stop Loss -";            //...
input bool    stopLossOn         = true;                       // Stop Loss On:
input SL_Mode slMode             = slmode_byCandle;            // Sl Mode:
input double  userSLpips         = 0;                          // Pips SL
...
input int     userTslDistance    = 15;                         // TSL Distance:
 ...
 public:
  ...
  void GetMarketOrders()
  {
    for (int i = PositionsTotal() - 1; i >= 0; i--) {
      ...
      if (_filterByMagicOn)
        if (_magics != magic) { continue; }
      if (_filterBySymbolsOn)
        if (sym != _symbols) { continue; }
...
class MoveTP : public iActions
{
 ...
 public:
  ...
  MoveTP* order(Order* or)
  {
    _order = or ;
    return &this;
  }
  MoveTP* newTP(double newTP)
  {
    _newTP = newTP;
    return &this;
  }
  ...
  {
    if (!controlPointer(_order)) {
      // Print(__FUNCTION__, " ", "Can't Move TP");
      return false;
    }

    if (moveTP(_order.id(), _newTP)) {
      return true;
    }
  ...
  bool moveTP(int tk, double newTP)
  {
    if (!trade.PositionModify(tk, _order.sl(), newTP)) {
      // Print(__FUNCTION__, " ", "error when move TP in TK: ", tk, " error:", GetLastError());
      return false;
    } else {
      _order.tp(_newTP);
      // Print(__FUNCTION__, " ", _order.id(), " Modify: new TP: ", _newTP);
      return true;
    }
...
class BUYcondition1 : public iConditions
{
 public:
  bool evaluate()
  {
    ...
    double in = indi(bufferToBuy, 1);
    if (in > 0 && in != EMPTY_VALUE) {
      normalSignal = true;
      virtualTP.percent(uPercentMid);
      return true;
    }

    double strong_in = indi(bufferToBuy_strong, 1);
    if (strong_in > 0 && strong_in != EMPTY_VALUE) {
      strongSignal = true;
      virtualTP.percent(uPercentFull);
      return true;
    }
    return false;
  }
...
class SELLcondition1 : public iConditions
{
 public:
  bool evaluate()
  {
    ...
    double strong_in = indi(bufferToSell_strong, 1);
    if (strong_in > 0 && strong_in != EMPTY_VALUE) {
      strongSignal = true;
      virtualTP.percent(uPercentFull);
      return true;
...
class ConditionMatchPrice : public iConditions
{
 ...
 public:
  ...
  ConditionMatchPrice(string Symbol, int Mode)
  {
    _symbol = Symbol;
    _mode   = Mode;
  }
...
input string tpartial    = "== Partial close Setup ==";  // ————————————
input bool   TPPartialOn = false;                        // Take Partial On:
input double uPercentMid  = 40;                           // Percent close midle risk :
input double uPercentFull = 60;                           // Percent close full risk:
double uPercent;

class VirtualTakeProfit
{
  ...
  PartialsDoit partialsDoit[];

 public:
  VirtualTakeProfit(double percent, string symbol = "")
  {
    _percent = percent;
    if (symbol == "") _symbol = Symbol();
    cdMatchPrice = new ConditionMatchPrice(symbol, 0);
  }
  ~VirtualTakeProfit() { ; }

  void   tk(long inptk) { _tk = inptk; }
  long   tk(void) { return _tk; }
  void   price(double Price) { _price = Price; }
  double price(void) { return _price; }
  void   percent(double Percent) { _percent = Percent; }
  double percent(void) { return _percent; }

  bool checkIfPartialWasDoit(long tk)
  {
    for (int i = 0; i < ArraySize(partialsDoit); i++) {
      if(partialsDoit[i].tk == tk)
      return true;
    }
    return false;
  }

  void registrarParcial(long tk)
  {
    int t = ArraySize(partialsDoit);
    if (ArrayResize(partialsDoit, t + 1)) {
      partialsDoit[t].tk            = tk;
      partialsDoit[t].countPartials = 1;
    }
  }

  bool doAction(long tk, double price)
  {
    if (checkIfPartialWasDoit(tk)) {
      return false;
    }
    ...
    cdMatchPrice.side(side);
    cdMatchPrice.price(price);

    if (cdMatchPrice.evaluate()) {
      double vol = PositionGetDouble(POSITION_VOLUME);
      double volumeToClose = NormalizeDouble(vol * _percent/100, 2);
      if (trade.PositionClosePartial(tk, volumeToClose)) {
        registrarParcial(tk);
        return true;
      } 
    }
...
VirtualTakeProfit virtualTP(uPercent);
...
class Grid
{
    ...
    iConditions *buyCondition;
    iConditions *sellCondition;
    ...
    Grid(string Symbol, string Side, double LastPrice, double Gap, double Multiplier, int MaxQnt, double MaxLot, double InitialLot, int magic, bool simbolFilterOn = true, bool magicFilterOn = true)
    {
        ...
        if(_side == "buy")  conditionsToOpenNewTrade.AddCondition(buyCondition = new BUYcondition1());
        if(_side == "sell") conditionsToOpenNewTrade.AddCondition(sellCondition = new SELLcondition1());
    }
    ~Grid()
    {
        delete cdMatchPrice;
...
void OnTick()
{
    ...
    if(!uTradeReverse){
        ...
        if(conditionsToBuy.EvaluateConditions()) {
            ...
            double sl = SL("buy");
            double vol = Lots();
            actionSendOrder = new SendNewOrder("buy", vol, "", 0, sl, tp, magico);
        ...
        if(conditionsToSell.EvaluateConditions()) {
            ...
            double sl = SL("sell");
            double vol = Lots();
            actionSendOrder = new SendNewOrder("sell", vol, "", 0, sl, tp, magico);
...
double SL(string direction)
{
    ...
    if(slMode==slmode_byCandle)
    {
      double candleSize=iHigh(Symbol(),0,1) - iLow(Symbol(),0,1);
      if(direction == "buy") {
          double ask = SymbolInfoDouble(_Symbol, SYMBOL_ASK);
          result = ask - candleSize;
          
          SLpips = candleSize/(_Point*10);
          return result;
      }

      if(direction == "sell") {
          double bid = SymbolInfoDouble(_Symbol, SYMBOL_BID);
          result = bid + candleSize;
          SLpips = candleSize/(_Point*10);
          return result;
      }
    }
...
double Lots()
{
    ...
    switch(modeCalcLots) {
        case Money:
            ...
            //
        case AccountPercent:
            if(normalSignal) lots = lotProvider.LotsByBalancePercent(userBalancePer, SLpips);
            if(strongSignal) lots = lotProvider.LotsByBalancePercent(userBalancePerStrong, SLpips);
            break;
            //
        case FixLots:
            lots = userLots;
...
void doVirtualTP()
{
  for (int i = MainOrders.qnt() - 1; i >= 0; i--) {
    long tk = MainOrders.id(i);
    PositionSelectByTicket(tk);
    double vol = PositionGetDouble(POSITION_VOLUME);
    if (vol == 0) continue;

    double tpPrice = NormalizeDouble(indi(bufferCentral, 0), _Digits);
    if (virtualTP.doAction(tk, tpPrice)) {
      doBreackevenAction();
    }
  }
}
...
void doTPControl()
{
  double buyTP  = indi(1, 1);
  double sellTP = indi(2, 1);

  for (int i = MainOrders.qnt() - 1; i >= 0; i--) {
    long tk = MainOrders.id(i);
    PositionSelectByTicket(tk);

    // tomar el tp y controlar que sea distinto al nuevo:
    double newTP = MainOrders.index(i).type() == POSITION_TYPE_BUY ? buyTP : sellTP;
    if (PositionGetDouble(POSITION_TP) != newTP) {
      tpControl = new MoveTP();
      tpControl.order(MainOrders.index(i)).newTP(newTP);
      tpControl.doAction();
      delete tpControl;
    }
  }
