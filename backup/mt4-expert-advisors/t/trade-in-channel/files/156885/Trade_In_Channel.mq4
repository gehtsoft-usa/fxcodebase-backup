...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=75261
...
input eModeToSignal mode_signal = channel_mode; // Mode Signal
input string        channelName = "Trade";      // Define your Channel Name:
...
bool ableToBuy = true;
bool ableToSell = true;

// if(ObjectFind(0,channelName)) Print("Está el channel");
double DownLine() { return ObjectGetValueByTime(0, channelName, TimeCurrent(), 0); } // down
double UpLine() { return ObjectGetValueByTime(0, channelName, TimeCurrent(), 1); }   // Up
...
class BUYcondition1 : public iConditions
{
  public:
    bool evaluate()
    {
        ...
        if (iLow(NULL, 0, 0) < DownLine() && iClose(NULL, 0, 0) > DownLine() && iClose(NULL, 0, 1) > DownLine() && iClose(NULL, 0, 2) > DownLine() && ableToBuy==true) {
            if (!cCountBuys.evaluate()) { return false; }
            ableToBuy = false;
            return true;
        }
...
BUYcondition1 conditionBuy_channel;
...
class SELLcondition1 : public iConditions
{
  public:
    bool evaluate()
    {
        ...
        if (iHigh(NULL, 0, 0) > UpLine() && iClose(NULL, 0, 0) < UpLine() && iClose(NULL, 0, 1) < UpLine() && iClose(NULL, 0, 2) < UpLine() && ableToSell == true) {
            if (!cCountSells.evaluate()) { return false; }            
            ableToSell = false;
            return true;
        }
...
SELLcondition1 conditionSell_channel;

// Line
double Line()
{
    double pr = ObjectGetDouble(0, channelName, OBJPROP_PRICE);
    return pr;
}
...
class BUYcondition2 : public iConditions
{
  public:
    ...
    {
        // TODO: condition Buy 1
        if (iLow(NULL, 0, 0) < Line() && iClose(NULL, 0, 0) > Line() && iClose(NULL, 0, 1) > Line() && iClose(NULL, 0, 2) > Line()) {
            return true;
        }
...
BUYcondition2 conditionBuy_line;
...
class SELLcondition2 : public iConditions
{
  public:
    ...
    {
        // TODO: condition sell 1
        if (iHigh(NULL, 0, 0) > Line() && iClose(NULL, 0, 0) < Line() && iClose(NULL, 0, 1) < Line() && iClose(NULL, 0, 2) < Line()) {
            return true;
        }
...
SELLcondition2 conditionSell_line;
...
void BasicStrategy_Iniciate()
{
    ...
    if (mode_signal == channel_mode) {
        LongStrategy.set(&conditionBuy_channel, &buy);
        ShortStrategy.set(&conditionSell_channel, &sell);
    }
    if (mode_signal == line_mode) {
        LongStrategy.set(&conditionBuy_line, &buy);
        ShortStrategy.set(&conditionSell_line, &sell);
    }
