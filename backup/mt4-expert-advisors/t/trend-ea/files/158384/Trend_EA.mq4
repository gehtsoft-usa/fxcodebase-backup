...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=75641
...
class BUYcondition1 : public iConditions
{
  public:
    bool evaluate()
    {
        ...
        double daylow = iLow(NULL, PERIOD_D1, 1);
        double l      = iLow(NULL, 0, 1);
        double c      = iClose(NULL, 0, 1);

        return l < daylow && c > daylow;
    }
};
BUYcondition1 cdTouchingLow;
...
class SELLcondition1 : public iConditions
{
  public:
    bool evaluate()
    {
        ...
        double dayhigh = iHigh(NULL, PERIOD_D1, 1);
        double h = iHigh(NULL, 0, 1);
        double c = iClose(NULL, 0, 1);

        return h > dayhigh && c < dayhigh;
    }
};
SELLcondition1 cdTouchingHigh;

class ConditionTrendBullish : public iConditions
{
    public:
    bool evaluate()
    {
       return iMA(NULL, 0, 20, 0, MODE_EMA, PRICE_CLOSE, 0) > iMA(NULL, 0, 50, 0, MODE_EMA, PRICE_CLOSE, 0);
    }
};
ConditionTrendBullish cdTrendBullish;

class ConditionTrendBearish : public iConditions
{
    public:
    bool evaluate()
    {
...
ConditionTrendBearish cdTrendBearish;
...
{
    LongStrategy.set(&cdTouchingLow, &buy);
    LongStrategy.addCondition(&cdTrendBullish);
    ShortStrategy.set(&cdTouchingHigh, &sell);
    ShortStrategy.addCondition(&cdTrendBearish);
