...
// ------------------------------------------------------------------
input string T0          = "== Set Trix ==";    // ————————————
input int    Depth       = 14;                  // Depth:
input int    candles     = 200;                 // Back Candles:
                ...
                const int&      spread[])
{
  ...
  {
    // to show in data window
    _open[i]  = iCustom(NULL, 0, "Trix.ex4", Depth, PRICE_OPEN, 0, i);
    _high[i]  = iCustom(NULL, 0, "Trix.ex4", Depth, PRICE_HIGH, 0, i);
    _low[i]   = iCustom(NULL, 0, "Trix.ex4", Depth, PRICE_LOW, 0, i);
    _close[i] = iCustom(NULL, 0, "Trix.ex4", Depth, PRICE_CLOSE, 0, i);

    // POSITIVE SIDE;
    if (_open[i] > 0 || _close[i]>0)
    {
      // Up Candle
      if (_open[i] < _close[i])
      {
        ...
        _up_basehigh[i] = _close[i];
        _up_baselow[i]  = _low[i] > _open[i] ? _open[i] : _low[i];
      }
      ...
      if (_open[i] > _close[i])
      {
        ...
        _dn_basehigh[i] = _open[i];
        _dn_baselow[i]  = _low[i] > _close[i] ? _close[i] : _low[i];
        // if (_open[i] < 0)
        // {
        //   _dn_baselow[i] = _low[i] < _close[i] ? _close[i] : _low[i];
        // }
    ...
    if (_open[i] < 0 && _close[i] < 0)
    {
      ...
      if (fabs(_open[i]) < fabs(_close[i]))
      {
        ...
        _dn_basehigh[i] = _close[i];
        _dn_baselow[i]  = _low[i] < _open[i] ? _open[i] : _low[i];
      }
      ...
      if (fabs(_open[i]) > fabs(_close[i]))
      {
        ...
        _up_low[i]      = _close[i];
        _up_basehigh[i] = _open[i];
        _up_baselow[i]  = _low[i] < _close[i] ? _close[i] : _low[i];
      }
    }
  }
  return (rates_total);
