...
Version:     
Date:        
Repository:  Available @ https://fxcodebase.com/code/viewtopic.php?f=38&t=76418
License:     GNU
*/
...
extern AlertType AlertOn = Current;
...
int start() {
   ...
   int i,limit;
   double Delta,Signal;
	static datetime lastAlertUpCurrent = 0;
	static datetime lastAlertDownCurrent = 0;
	static datetime lastAlertUpPrevious = 0;
	static datetime lastAlertDownPrevious = 0;
   ...
   for (i=limit;i>=0;i--)   {
	...
	if (UseAlerts)
	{
		double Delta0 = iMAOnArray(b1,0,Smoothing,0,MODE_SMA,0-1) - iMAOnArray(b1,0,Smoothing,0,MODE_SMA,0);
		double Signal0 = iADX(NULL,0,ADX_Period,PRICE_CLOSE,MODE_PLUSDI,0) - iADX(NULL,0,ADX_Period,PRICE_CLOSE,MODE_MINUSDI,0);
		double Delta1 = iMAOnArray(b1,0,Smoothing,0,MODE_SMA,1-1) - iMAOnArray(b1,0,Smoothing,0,MODE_SMA,1);
		double Signal1 = iADX(NULL,0,ADX_Period,PRICE_CLOSE,MODE_PLUSDI,1) - iADX(NULL,0,ADX_Period,PRICE_CLOSE,MODE_MINUSDI,1);
		...
		if (AlertOn == Current)
		{
			...
			{
				Alert(Symbol(), " ", Period(), ": Buy signal on current candle");
				lastAlertUpCurrent = Time[0];
			}
			if (isSell0 && lastAlertDownCurrent != Time[0])
			{
				Alert(Symbol(), " ", Period(), ": Sell signal on current candle");
		...
		if (AlertOn == Previous)
		{
			...
			{
				Alert(Symbol(), " ", Period(), ": Buy signal on previous candle close");
				lastAlertUpPrevious = Time[1];
			}
			if (isSell1 && lastAlertDownPrevious != Time[1])
			{
				Alert(Symbol(), " ", Period(), ": Sell signal on previous candle close");
