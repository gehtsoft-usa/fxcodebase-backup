...
int init()
{
   ...
   int id = 0;
   SetIndexStyle(id, DRAW_LINE, STYLE_SOLID, 1);
   SetIndexBuffer(id, ma1);
   SetIndexLabel(id, "MA1");
   ++id;
   SetIndexStyle(id, DRAW_LINE, STYLE_SOLID, 1);
   SetIndexBuffer(id, ma2);
   SetIndexLabel(id, "MA2");
   ++id;
   SetIndexStyle(id, DRAW_LINE, STYLE_SOLID, 1);
   SetIndexBuffer(id, ma3);
   SetIndexLabel(id, "MA3");
   ++id;
   ...
   {
      ICondition* upCondition = (ICondition*) new Up2Condition(_Symbol, (ENUM_TIMEFRAMES)_Period);
      ICondition* downCondition = (ICondition*) new Down2Condition(_Symbol, (ENUM_TIMEFRAMES)_Period);
      id = CreateAlert(id, upCondition, NULL, downCondition, NULL, 217, 218);
   }
                ...
                const int &spread[])
{
   ...
   int minBars = 0;
   int limit = MathMin(rates_total - 1 - minBars, rates_total - prev_calculated - 1);
   for (int pos = limit; pos >= 0 && !IsStopped(); --pos)
   {
      ma1[pos] = iMA(_Symbol, _Period, Period1, 0, Method1, PRICE_CLOSE, pos);
      ma2[pos] = iMA(_Symbol, _Period, Period2, 0, Method2, PRICE_CLOSE, pos);
      ma3[pos] = iMA(_Symbol, _Period, Period3, 0, Method3, PRICE_CLOSE, pos);
      if (customStream != NULL)
      {
