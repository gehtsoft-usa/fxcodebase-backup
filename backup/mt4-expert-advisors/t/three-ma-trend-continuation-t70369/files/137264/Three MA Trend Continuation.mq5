...
//http://fxcodebase.com/code/viewtopic.php?f=38&t=70369
...
input int Period1 = 10; // Fast Period MA
input ENUM_MA_METHOD Method1 = MODE_SMA; // MA Method
input int Period2 = 15; // Slow Period MA
input ENUM_MA_METHOD Method2 = MODE_SMA; // MA Method
input int Period3 = 50; // Slow Period MA
input ENUM_MA_METHOD Method3 = MODE_SMA; // MA Method
...
class Up2AlertCondition : public ACondition
{
public:
   Up2AlertCondition(const string symbol, ENUM_TIMEFRAMES timeframe)
      :ACondition(symbol, timeframe)
   {
...
class Down2AlertCondition : public ACondition
{
public:
   Down2AlertCondition(const string symbol, ENUM_TIMEFRAMES timeframe)
      :ACondition(symbol, timeframe)
   {
...
double ma1[], ma2[], ma3[];
int ma1Indi, ma2Indi, ma3Indi;
int OnInit(void)
{
   IndicatorObjPrefix = GenerateIndicatorPrefix("tmatc");
   IndicatorSetString(INDICATOR_SHORTNAME, "Three MA Trend Continuation");
   IndicatorSetInteger(INDICATOR_DIGITS, Digits());
   ma1Indi = iMA(_Symbol, _Period, Period1, 0, Method1, PRICE_CLOSE);
   ma2Indi = iMA(_Symbol, _Period, Period2, 0, Method2, PRICE_CLOSE);
   ma3Indi = iMA(_Symbol, _Period, Period3, 0, Method3, PRICE_CLOSE);
   
   int id = 0;
   SetIndexBuffer(id, ma1, INDICATOR_DATA);
   PlotIndexSetInteger(id, PLOT_DRAW_TYPE, DRAW_LINE);
   PlotIndexSetString(id, PLOT_LABEL, "MA1");
   ++id;
   SetIndexBuffer(id, ma2, INDICATOR_DATA);
   PlotIndexSetInteger(id, PLOT_DRAW_TYPE, DRAW_LINE);
   PlotIndexSetString(id, PLOT_LABEL, "MA2");
   ++id;
   SetIndexBuffer(id, ma3, INDICATOR_DATA);
   PlotIndexSetInteger(id, PLOT_DRAW_TYPE, DRAW_LINE);
   PlotIndexSetString(id, PLOT_LABEL, "MA3");
   ++id;
   ...
   {
      ICondition* upCondition = new Up2AlertCondition(_Symbol, timeframe);
      ICondition* downCondition = new Down2AlertCondition(_Symbol, timeframe);
      #ifdef ACT_ON_SWITCH
         ActOnSwitchCondition* upSwitch = new ActOnSwitchCondition(_Symbol, timeframe, upCondition);
         upCondition.Release();
         upCondition = upSwitch;
...
{
   IndicatorRelease(ma1Indi);
   IndicatorRelease(ma2Indi);
   IndicatorRelease(ma3Indi);
   delete mainSignaler;
   mainSignaler = NULL;
...
)
{
   ...
   for (int pos = MathMax(first, prev_calculated - 1); pos < rates_total; ++pos)
   {
      ...
      double buffer[1];
      if (CopyBuffer(ma1Indi, 0, oldIndex, 1, buffer) == 1)
      {
         ma1[pos] = buffer[0];
      }
      if (CopyBuffer(ma2Indi, 0, oldIndex, 1, buffer) == 1)
      {
         ma2[pos] = buffer[0];
      }
      if (CopyBuffer(ma3Indi, 0, oldIndex, 1, buffer) == 1)
      {
         ma3[pos] = buffer[0];
      }
      for (int i = 0; i < ArraySize(alerts); ++i)
