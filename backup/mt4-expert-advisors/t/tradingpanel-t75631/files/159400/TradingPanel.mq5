...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=75631
...
bool OnInit_GUI()
{
    ...
    if (gui.reason() != REASON_CHARTCHANGE && gui.reason() != REASON_TEMPLATE && gui.reason() != REASON_PARAMETERS) {
        ...
        double columns    = 2;
        double anchoPanel = columns * 75 + columns * 10 + (5 + 5);
        res               = gui.Create(0, "Trading Panel MT5", 0, 0, 0, anchoPanel, altoPanel);
        if (res) gui.Run();
    }
    return res;
}
...
class GUI : public CAppDialog
{
    ...
    int _magic;
    int _high, _width1, _width2, _width4;
    int _x, _y;
    int _gapV, _gapH;
    ...
    GUI(int magic = 0)
    {
        ...
        _gapH = 5;
        _magic = magic;
        _width2 = _width1 * 2 + _gapH;
        _width4 = _width1 * 4 + _gapH;
    }
    ~GUI() {}

    CButton btBuy, btSell, btBuyPending, btSellPending, btCloseAll, btTSL, btClosePartial;
    CEdit   eAsk, eBid;
    CEdit   ePendingPrice,  eTP,  eSL,  eLots,  eRisk, ePartial; 
    CLabel  lbPendingPrice, lbTP, lbSL, lbLots, lbRisk;
    CComboBox  cbLots;
    ...
    int Col(int c) { return _y + (c * _width1) + c * _gapH; }
    ...
    bool Create(const long chart, const string name, const int subwin, const int x1, const int y1, const int x2, const int y2)
    {
        ...
        if(!Create_Edit("eAsk",        Col(0), Row(0), _high, _width1, eAsk)) return false; eAsk.Text("0.00");
        if(!Create_Edit("eBid",        Col(1), Row(0), _high, _width1, eBid)) return false; eBid.Text("0.00");
        
        // entry
        if(!Create_button("Buy",        Col(0), Row(1), _high, _width1, btBuy)) return false;
        if(!Create_button("Sell",       Col(1), Row(1), _high, _width1, btSell)) return false;
        
        if(!Create_label("Pending Price", Col(0), Row(2), _high, _width1, lbPendingPrice)) return false; 
        if(!Create_Edit("ePendingPrice",  Col(1), Row(2), _high, _width1, ePendingPrice)) return false; ePendingPrice.Text("0.00");
        if(!Create_button("Buy Pending",  Col(0), Row(3), _high, _width1, btBuyPending)) return false;
        if(!Create_button("Sell Pending", Col(1), Row(3), _high, _width1, btSellPending)) return false;

        // string EntrysArray[] = {"Market","Pending at Close", "Stop Order",}; long EntrysArrayValue[] = {0,0,0};
        // if(!Create_Combobox("cbEntry",        Col(1), Row(25), _high, _width2, cbEntry, EntrysArray, EntrysArrayValue)) return false;
        // if(!Create_Edit("eEntry",             Col(3), Row(25), _high, _width1, eEntry)) return false;eEntry.Text("0");
        
        // SL
        if(!Create_label("SL:",               Col(0), Row(4), _high, _width1, lbSL)) return false;
        if(!Create_Edit("eSL",                Col(1), Row(4), _high, _width1, eSL)) return false;eSL.Text("20");
        // string SLArray[] = {"Pattern","Zone","Pips","ATR", "Swing"}; long SLArrayValue[] = {0,0,0,0,0};
        // if(!Create_Combobox("cbSL",           Col(1), Row(26), _high, _width2, cbSL, SLArray, SLArrayValue)) return false;
        
        // TP
        // string TPArray[] = {"Ratio RR","Zone","Pips"}; long TPArrayValue[] = {0,0,0};
        // if(!Create_Combobox("cbTP",           Col(1), Row(27), _high, _width2, cbTP, TPArray, TPArrayValue)) return false;
        if(!Create_label("TP:",               Col(0), Row(5), _high, _width1, lbTP)) return false;
        if(!Create_Edit("eTP",                Col(1), Row(5), _high, _width1, eTP)) return false;eTP.Text("40");
        
        // Lots
        string LotsArray[] = {"Risk Percent","Fix Lots"}; long LotsArrayValue[] = {0,0};
        if(!Create_Combobox("cbLots",         Col(0), Row(7), _high, _width2, cbLots, LotsArray, LotsArrayValue)) return false;
        if(!Create_label("Risk:",             Col(0), Row(8), _high, _width1, lbRisk)) return false;
        if(!Create_Edit("eRisk",              Col(1), Row(8), _high, _width1, eRisk)) return false;eRisk.Text("1");
        if(!Create_label("Lots:",             Col(0), Row(9), _high, _width1, lbLots)) return false;
        if(!Create_Edit("eLots",              Col(1), Row(9), _high, _width1, eLots)) return false;eLots.Text("0.01");
        
        if(!Create_button("Close All",        Col(0), Row(11), _high, _width2, btCloseAll)) return false;
        if(!Create_button("Close %",    Col(0), Row(12), _high, _width1, btClosePartial)) return false;
        if(!Create_Edit("Partial Close",      Col(1), Row(12), _high, _width1, ePartial)) return false;ePartial.Text("50");
        if(!Create_button("TSL",              Col(0), Row(13), _high, _width2, btTSL)) return false;
    ...
    void HoverEvents(const int id, const long& lparam, const double& dparam, const string& sparam)
    {
        ...
        if(btBuyPending.IsActive()) btBuyPending.ColorBackground(PaleGreen); else btBuyPending.ColorBackground(CONTROLS_BUTTON_COLOR_BG);
        if(btSellPending.IsActive()) btSellPending.ColorBackground(Tomato); else btSellPending.ColorBackground(CONTROLS_BUTTON_COLOR_BG);
    }
    ...
    void Update()
    {
        ...
        eAsk.Text(DoubleToString(Ask, _Digits));
        eBid.Text(DoubleToString(Bid, _Digits));
        
        double l = ::LotsCalculation(); 
        eLots.Text((string)NormalizeDouble(l, 2));        
    }
    ...
    bool Create_Combobox(string name, const int x1, const int y1, const int high, const int width, CComboBox& cb, string &items[], long &values[])
    {
        int x2 = x1 + width;
        int y2 = y1 + high;

        cb.Create(m_chart_id, name, m_subwin, x1, y1, x2, y2);
        
        for(int i=0;i < ArraySize(items);i++)
        {
            cb.AddItem(items[i], values[i]);
        }
        cb.Select(0);        
    ...
    bool Create_List(string name, int x1, int y1, int high, int width, CListView& lv)
    {
        int x2 = x1 + width;
        int y2 = y1 + high;
        lv.Create(m_chart_id, name, m_subwin, x1, y1, x2, y2);
        lv.AddItem("signals --- 4");
        lv.AddItem("signals --- 5");
        lv.AddItem("signals --- 6");
        lv.AddItem("signals --- 7");
        lv.AddItem("signals --- 8");
        Add(lv);
        return true;
    }
...
EVENT_MAP_BEGIN(GUI)
ON_EVENT(ON_CLICK, btBuy,       buy.execute)
ON_EVENT(ON_CLICK, btSell,      sell.execute)
ON_EVENT(ON_CLICK, btBuyPending, buyPending.execute)
ON_EVENT(ON_CLICK, btSellPending, sellPending.execute)
ON_EVENT(ON_CLICK, btCloseAll, acCloseAll.execute)
ON_EVENT(ON_CLICK, btClosePartial, acClosePartial.execute)
ON_EVENT(ON_CLICK, btTSL, TSL_ONOFF)
...
// ------------------------------------------------------------------
string  tvolumen   = "== Volumen Calculation =="; // ————————————————————————
enum_lot_mode lot_mode   = lot_fix;                     // Lot Calculation Mode
double  uLotsValue = 0.01;                        // value to calculate Lots:
...
double LotsCalculation()
{
    ...
    if (gui.cbLots.Select() == "Risk Percent")
    {
        lot_mode = lot_equity_percent;
    } else
    {
        lot_mode = lot_fix;
    }  
    ...
    case lot_equity_percent:
        ...
        lots = lotsProvider.LotsByEquityPercent((double)gui.eRisk.Text());
        break;
#endif
...
// ------------------------------------------------------------------
string         tstop          = "== Stop Loss =="; // ————————————————————————
stop_loss_mode sl_mode        = sl_fix_pips;       // SL Mode:
double         uStopLossValue = 50;                // SL value:
...
double StopLossByPips(const double price, const string side, const string symbol)
{
    ...
    uStopLossValue = (double)gui.eSL.Text();
...
// ------------------------------------------------------------------
string           ttakeprofit      = "== Take Profit =="; // ————————————————————————
take_profit_mode tp_mode          = tp_fix_pips;         // TP Mode
double           uTakeProfitValue = 2;                   // TP value
...
double TakeProfitByPips(const double price, const string side, const string symbol)
{
    ...
    uTakeProfitValue = (double)gui.eTP.Text();    
...
#define Section_ClosePartial
#ifdef Section_ClosePartial
// Mark: Section_ClosePartial

class ConditionClosePartial : public iConditions
{
  public:
    bool evaluate()
    {
...
ConditionClosePartial cdClosePartial;

class ActionClosePartial : public iActions
{
  public:
    bool execute()
    {
        ...
        for (int i = PositionsTotal(); i >= 0; i--) {
            ...
            if (PositionGetSymbol(i) == Symbol() && PositionGetInteger(POSITION_MAGIC) == magico) {
                ...
                double volume_to_close = PositionGetDouble(POSITION_VOLUME);
                double uPercent = StringToDouble(gui.ePartial.Text());
                uPercent /= 100;
                volume_to_close = NormalizeDouble(volume_to_close * uPercent, 2);
                
                Print("line: ",__LINE__,"  Closing Partial : vol:",   volume_to_close);
                 
                return trade.PositionClosePartial(Symbol(),volume_to_close);
                }
            }
...
ActionClosePartial acClosePartial;

Strategy stClosePartial;

void OnInit_ClosePartial()
{
    stClosePartial.set(&cdClosePartial, &acClosePartial); 
}
...
void TSL_ONOFF()
{
    ...
    if (tsl_On) {
        ...
        Print("Trailing Stop Loss Deactivated");
    }
}
...
class OrderCreator : public iActions
{
  ...
  public:
    OrderCreator(const string Symbol, const string Side, const int Magic, bool pending=false)
    {
        _symbol = Symbol;
        _side   = Side;
        _magic  = Magic;
    ...
    void setPrice() { 
        if(_pending)
        {
            _price = (double)gui.ePendingPrice.Text();
        } else
        {
            _price = ::Price(_side); }
        }
    ...
    void setLots() { 
        ...
        Print("line: ",__LINE__," _lots: ",_lots);        
    }
    ...
    bool execute()
    {
        ...
        if(_pending)
            if (!trade.OrderOpen(_symbol, _cmd, _lots, 0, _price, _sl, _tp, 0, 0, "")){
                Print(__FUNCTION__, " ", "Cannot Send Pending Order, error: ", GetLastError());            
                return false;
            }        
        return true;
    }
...
OrderCreator buy(_Symbol, "buy", magico, false);
OrderCreator sell(_Symbol, "sell", magico, false);
OrderCreator buyPending(_Symbol, "buy", magico, true);
OrderCreator sellPending(_Symbol, "sell", magico, true);
...
void OnTick() { 
    ...
    if (avprice != lastAvPrice) {
        lastAvPrice = avprice;
        DrawLine(avprice);
    }
    if(avprice == 0) { ObjectDelete(0, "AvPriceLine"); }
}
...
double GetAverageOpenPrice()
{
    ...
    if(totalLots > 0)
        return NormalizeDouble(totalPrice / totalLots, (int)SymbolInfoInteger(Symbol(), SYMBOL_DIGITS));
    else
        return 0;
}
...
{
    ObjectDelete(0, "AvPriceLine");
    string name="AvPriceLine";
    color clr = clrBlue;
    int width = 1;
    ENUM_LINE_STYLE style = STYLE_DOT;
    long chart_id = 0;
    datetime t1 = iTime(_Symbol, PERIOD_CURRENT, 0);
    datetime t2 = iTime(_Symbol, PERIOD_CURRENT, Bars(_Symbol, PERIOD_CURRENT) - 1);
