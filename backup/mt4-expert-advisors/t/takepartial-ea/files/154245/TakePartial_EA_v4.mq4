...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=73251
...
int   uEntryDistance = 0;       // Pips distance for pending orders:
bool      uDeletePendingsOn = false;   // Delete pendings when close all:
#endif

input string T0 = "== Trade Setup ==";  // ————————————
int          uMinSize = 2;                    // Minimum Size Candle Pattern (points):
...
input bool closePendings = true; // Delete Pendings when Close All?
...
input double userPartialClosePips = 10;                     // Partial Close Pips:
input color  clrPLine = clrNavy;                // Color Partial line:
...
input bool   breakevenOn = true;                     // Breakeven On:
double userBkvPips = 10;                       // Breakeven Pips
input double BkvExtraPips = 1; // Extra Pips: 
#else
...
bool OnInit_GUI()
{
    ...
    {
        res = gui.Create(0, "TakePartial EA v3", 0, 400, 0, 590, 110);
        if(res) gui.Run();
    }
    return res;
}
...
class GUI : public CAppDialog
{
    ...
    bool Create(const long chart, const string name, const int subwin, const int x1, const int y1, const int x2, const int y2)
    {
        ...
        if(!Create_button("Close all", Col(0), Row(2), _high, _width, bt3)) return false;
...
class ActionCloseAll : public iActions
{
    ...
    bool doAction()
    {
        ...
        if(closePendings)
        {
            deletePendingsBuys.doAction();
            deletePendingsSells.doAction();
        }
...
ActionCloseAll closeAllAction(magico);
...
ActionDeletePendings deletePendingsBuys("buy", magico);
ActionDeletePendings deletePendingsSells("sell", magico);
...
class BreackevenCondition : public iConditions
{
    ...
    bool evaluate()
    {
        ...
        if(CheckPointer(_order)!=POINTER_INVALID)
        {
        double mPoints = MarketInfo(_order.symbol(), MODE_POINT);// invalid pointer acá
        double ask = SymbolInfoDouble(_order.symbol(), SYMBOL_ASK);
        double bid = SymbolInfoDouble(_order.symbol(), SYMBOL_BID);
...
int OnInit()
{
    ...
    {
        gui.setButton1Action(GetPointer(btnBuyAction));
        gui.setButton2Action(GetPointer(btnSellAction));
        gui.setButton3Action(GetPointer(closeAllAction));
    }
...
void doBreackevenAction()
{
    for(int i = mainOrders.qnt() - 1; i >= 0; i--)
    {
        ...
        {
            if(CheckPointer(mainOrders.index(i)) == POINTER_INVALID)continue;
    ...
    for(int i = mainOrders.qnt() - 1; i >= 0; i--)
    {
        if(mainOrders.index(i).countPartials() < uQntPartials)
        {
            ...
            if(!mainOrders.index(i).breakevenWasDoIt())
                ...
                double pr = mainOrders.index(i).type() == OP_BUY ? mainOrders.index(i).price() + BkvExtraPips * _Point *10 :  mainOrders.index(i).price() - BkvExtraPips * _Point *10;
                breackevenAction.order(mainOrders.index(i)).newSL(pr);
                // breackevenAction.order(mainOrders.index(i)).newSL(mainOrders.index(i).price());
                breackevenAction.doAction();
                delete breackevenAction;
                }
...
void generate_pline()
{
    // de cada orden en main tiene que haber una linea
    // recorro las ordenes en main
    for(int i = mainOrders.qnt() - 1; i >= 0; i--)
    {
        // tomo el id
        string id = (string) mainOrders.id(i);

        // busco un objeto line con el nombre
        if(ObjectFind(0, id) != 0)
        {
            // no existe, generar la linea
            pline = new HLine(id, 0, partialPriceFromInput(i), clrPLine, id);
            pline.redraw();
        }
    }
}

double linePrice(int id)
{
    string _id = (string) id;
    if(ObjectFind(0, _id) == 0)
        return ObjectGetDouble(0, _id, OBJPROP_PRICE);

    return 0;
}

void cleanCloseLines()
{
    int count = 0;
    for(int i = OrdersTotal() - 1; i >= 0; i--)
    {
...
double partialPriceFromInput(int i)
{
    // double dist;
    double price = 0;
    double mPoints = MarketInfo(_Symbol, MODE_POINT);
    ...
    if(mainOrders.index(i).type() == OP_BUY)
        price = mainOrders.index(i).price() + dist;
