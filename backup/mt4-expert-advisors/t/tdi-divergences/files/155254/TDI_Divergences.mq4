...
//Available @  https://fxcodebase.com/code/viewtopic.php?f=38&t=74845
...
#property indicator_label7 "Arrow Up"
#property indicator_type7  DRAW_ARROW
#property indicator_color7 clrBlue
#property indicator_style7 STYLE_SOLID
#property indicator_width7 1
#property indicator_label8 "Arrow Down"
#property indicator_type8  DRAW_ARROW
#property indicator_color8 clrRed
#property indicator_style8 STYLE_SOLID
#property indicator_width8 1

#property indicator_label9 "Arrow Up"
#property indicator_type9  DRAW_ARROW
#property indicator_color9 clrGreen
#property indicator_style9 STYLE_SOLID
#property indicator_width9 1
#property indicator_label10 "Arrow Down"
...
extern enMaTypes       RsiPriceLineMAMode       = ma_sma;         // Signal line moving average method
extern double          RsiSignalLinePeriod      = 14;             // Rsi signal line period
extern enMaTypes       RsiSignalLineMAMode      = ma_sma;         // Signal line moving average method
...
int init()
{
    ...
    SetIndexBuffer(6, xhigh);
    SetIndexArrow(6, 159);
    SetIndexStyle(6, DRAW_ARROW, STYLE_SOLID, 1, Crimson);
    SetIndexBuffer(7, xlow);
    SetIndexArrow(7, 159);
    SetIndexStyle(7, DRAW_ARROW, STYLE_SOLID, 1, Blue);

    SetIndexBuffer(8, beard);
    SetIndexArrow(8, 234);
    SetIndexStyle(8, DRAW_ARROW, STYLE_SOLID, 1, Crimson);
    SetIndexBuffer(9, bulld);
    SetIndexArrow(9, 233);
    SetIndexStyle(9, DRAW_ARROW, STYLE_SOLID, 1, Blue);
    ...
    IndicatorShortName(timeFrameToString(TimeFrame) + " TDI of " + getRsiName((int)RsiMethod) + "  (" + (string)RsiPeriod + "," + (string)RsiPriceLinePeriod + ", " + (string)RsiSignalLinePeriod + ")");
...
int start()
{
    ...
    for (i = limit; i >= 0; i--) {
        ...
        rsi[i]        = iRsi(RsiMethod, prices, RsiPeriod, i);
        rsiPL[i]      = iCustomMa(RsiPriceLineMAMode, rsi[i], RsiPriceLinePeriod, i, Bars, 0);
        rsiSL[i]      = iCustomMa(RsiSignalLineMAMode, rsi[i], RsiSignalLinePeriod, i, Bars, 1);
        bandMi[i]     = 0;
        for (int k = 0; k < VolatilityBandPeriod && (i + k) < Bars; k++)
            bandMi[i] += rsi[i + k];
        bandMi[i] /= VolatilityBandPeriod;
        ...
        trend[i]         = (i < Bars - 1) ? (rsiPL[i] > rsiSL[i]) ? 1 : (rsiPL[i] < rsiSL[i]) ? -1
                                                                                              : trend[i + 1]
                                          : 0;
    ...
    if (alertsOn) {
        ...
        if (trend[whichBar] != trend[whichBar + 1]) {
            if (trend[whichBar] == 1)
                doAlert(whichBar, " crossed up");
            if (trend[whichBar] == -1)
                doAlert(whichBar, " crossed down");
        }
    }
...
bool isxlow(int i)
{
    double prev = rsiPL[i + 3];
    double curr = rsiPL[i + 2];
    double next = rsiPL[i + 1];

    return (prev > curr && next > curr && curr > 0 && curr != EMPTY_VALUE);
}
bool isxhigh(int i)
{
    double prev = rsiPL[i + 3];
...
bool BullDivergence(int index)
{
    index += 3;
    double last          = 0;
    double lastCandleLow = 0;
    double curr          = xlow[index-1];

    for (int i = index; i < 100; i++) {
        if (xlow[i] >= 0 && xlow[i] != EMPTY_VALUE) {
            last          = xlow[i];
            lastCandleLow = Low[i];
            break;
        }
    }
    if(last < curr && lastCandleLow > Low[index] && lastCandleLow != 0)return true;
    if(last > curr && lastCandleLow < Low[index] && lastCandleLow != 0)return true;

    return false;
}

bool BearDivergence(int index)
{
    index += 3;
    double last           = 0;
    double lastCandleHigh = 0;
    double curr           = xhigh[index-1];

    for (int i = index; i < 100; i++) {
        if (xhigh[i] >= 0 && xhigh[i] != EMPTY_VALUE) {
            last           = xhigh[i];
            lastCandleHigh = High[i];
            break;
        }
    }

    if(last > curr && lastCandleHigh < High[index] && lastCandleHigh != 0)return true;
    if(last < curr && lastCandleHigh > High[index] && lastCandleHigh != 0)return true;
