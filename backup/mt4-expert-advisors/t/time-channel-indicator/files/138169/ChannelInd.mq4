...
// http://fxcodebase.com/code/viewtopic.php?f=38&t=17247
//+------------------------------------------------------------------+
...
input int EndHour = 10; // end hour
input int EndMinute = 0; // End minute
input int AlertEndHour = 15; // End Hour for checking the break
input int AlertEndminute = 0; // End minute for checking the break
input ENUM_TIMEFRAMES tf = PERIOD_H1; // Timeframe
input int FontSize = 15;
input color TextColor = Yellow;
...
double GetHigh(int pos)
{
   for (int i = pos; i < Bars - 1; ++i)
   {
      if (UpperCh[i] != EMPTY_VALUE)
      {
         return UpperCh[i];
      }
   }
...
double GetLow(int pos)
{
   for (int i = pos; i < Bars - 1; ++i)
   {
      if (LowerCh[i] != EMPTY_VALUE)
      {
         return LowerCh[i];
      }
   }
...
public:
   ...
   {
      datetime EndTime = StrToTime(TimeToStr(date, TIME_DATE) + " " + IntegerToString(EndHour, 0) + ":" + IntegerToString(EndMinute));
      datetime AlertEndTime = StrToTime(TimeToStr(date, TIME_DATE) + " " + IntegerToString(AlertEndHour, 0) + ":" + IntegerToString(AlertEndminute));
      if (date >= EndTime && date < AlertEndTime)
      {
         double h = GetHigh(period);
         return iClose(_symbol, _timeframe, period) >= h && iClose(_symbol, _timeframe, period + 1) < h;
      }
      return false;
   }
};
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      ...
      if (date >= EndTime && date < AlertEndTime)
      {
         ...
         return iClose(_symbol, _timeframe, period) <= l && iClose(_symbol, _timeframe, period + 1) > l;
      }
      return false;
   }
};
...
int init()
{
   ...
   }
   {
      ...
      upCondition.Add(new UpCondition(_Symbol, (ENUM_TIMEFRAMES)_Period), false);
      upCondition.Add(new UpCondition(_Symbol, tf), false);
      AndCondition* downCondition = new AndCondition();
      downCondition.Add(new DownCondition(_Symbol, (ENUM_TIMEFRAMES)_Period), false);
...
int start()
{
   ...
   {
      datetime EndTime = StrToTime(TimeToStr(Time[pos], TIME_DATE) + " " + IntegerToString(EndHour, 0) + ":" + IntegerToString(EndMinute));
      if (Time[pos] >= EndTime && Time[pos + 1] < EndTime)
      {
         int pos_ = Time[pos] == EndTime ? pos : pos + 1;
         double MaxPrice = High[pos_];
         double MinPrice = Low[pos_];
         datetime StartTime = StrToTime(TimeToStr(Time[pos], TIME_DATE) + " " + IntegerToString(StartHour, 0) + ":" + IntegerToString(StartMinute));
