...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=76101
...
input take_profit_mode tp_mode          = tp_fix_pips;         // TP Mode
input double           uTakeProfitValue = 5;                   // TP value
...
input string tdeletependings             = "== Delete Pendings =="; // ————————————————————————
input bool   delete_pendings_on          = true;                    // Delete Pendings On?
input double distance_to_delete_pendings = 1000;                    // Delete Pendings Distance:

class ConditionDeletePendings : public iConditions
{
  public:
    bool evaluate() { return delete_pendings_on; }
};
  ...
  public:
    bool execute()
    {
        ...
        if (OrderType() == OP_BUYSTOP) {
            if (fabs(OrderOpenPrice() - Ask) > distance_to_delete_pendings * _Point) {
                OrderDelete(OrderTicket(), clrMagenta);
            }
        }
        if (OrderType() == OP_SELLSTOP) {
            if (fabs(OrderOpenPrice() - Bid) > distance_to_delete_pendings * _Point) {
                OrderDelete(OrderTicket(), clrMagenta);
            }
        }
        return true;
...
input int    count_max_positions = 40;                          // # Total positions:
input int    count_max_buys      = 20;                          // # Buys limit:
input int    count_max_sells     = 20;                          // # Sells limit:
