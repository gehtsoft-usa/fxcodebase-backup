...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=73273
...
input double             rsiLevelUpH           = 80;                                 // Buy RSI Level High
input double             rsiLevelUpL           = 50;                                 // Buy RSI Level Low
input double             rsiLevelDnH           = 50;                                 // Sell RSI Level High
input double             rsiLevelDnL           = 20;                                 // Sell RSI Level Low
int                      rsiCandlesDivergence = 50;                                 // RSI Candles Divergences
input string             Imfi                 = "== MFI Setup ==";                  // == MFI Setup ==
input int                mfiPeriod            = 14;                                 // Period
input double             mfiLevelUpH           = 80;                                 // Buy MFI Level High
input double             mfiLevelUpL           = 50;                                 // Buy MFI Level Low
input double             mfiLevelDnH           = 50;                                 // Sell MFI Level High
input double             mfiLevelDnL           = 20;                                 // Sell MFI Level Low
input string             Iema                 = "== Moving Average Fast Setup ==";  // ————————————
input int                maPeriod             = 7;                                  // Period
...
input int                ma_slowPeriod        = 20;                                 // Period
int                      ma_slowShift         = 0;                                  // Ma Shift
input ENUM_MA_METHOD     ma_slowMethod        = MODE_EMA;                           // Method
input ENUM_APPLIED_PRICE ma_slowAppliedPrice  = PRICE_CLOSE;                        // Applied Price
input string             Iline                = "== Line Setup ==";                 // ————————————
input int                candlesBack          = 10;                                 // Candles Back for last high:
   ...
   bool              UpDivergence(int shift)
     {
            ...
            if(index(i + 1) < index(i) && index(i + 1) < index(i + 2))
              {
               if(second_down == 0)
                 {
                  second_down = index(i + 1);
                  break;
                 }
               if(index(i + 1) < second_down)
                 {
                  second_down = index(i + 1);
                  break;
                 }
            ...
            if(index(j + 1) < index(j) && index(j + 1) < index(j + 2))
              {
               if(first_down == 0)
                 {
                  first_down = index(j + 1);
                 }
               if(index(j + 1) < first_down)
                 {
                  first_down = index(j + 1);
                  break;
                 }
      ...
      if(first_down == 0 || second_down == 0 || j == 51)
         return false;
      ...
      if(market_second >= iClose(NULL, 0, i + 2) || market_second >= iClose(NULL, 0, i))
         return false;
      market_first  = iClose(NULL, 0, j + 1);  // minimo anterior...
      if(market_first >= iClose(NULL, 0, j + 2) || market_first >= iClose(NULL, 0, j))
         return false;
      // ahora los cierres del mercado tienen que ser el segundo inferior al primero (hay nuevo min en el mercado)
      if(market_first <= market_second)
         return false;
      return true;
     }
   ...
   bool              evaluate()
     {
      ...
      return iClose(Symbol(), PERIOD_D1, 0) > iOpen(Symbol(), PERIOD_D1, 0);
      // return false;
     }
  };
   ...
   bool              evaluate()
     {
      ...
      return iRSI(NULL, 0, rsiPeriod, rsiAppliedPrice, 1) > rsiLevelUpL && iRSI(NULL, 0, rsiPeriod, rsiAppliedPrice, 1) < rsiLevelUpH;
     }
  };
   ...
   bool              evaluate()
     {
      ...
      return iMFI(NULL, 0, mfiPeriod, 1) > mfiLevelUpL && iMFI(NULL, 0, mfiPeriod, 1) < mfiLevelUpH;
     }
  };
   ...
   bool              evaluate()
     {
      ...
      return ma.index(1) > ma_slow.index(1);
     }
  };
BUYcondition4* buyCondition4;

//+------------------------------------------------------------------+
//|                                                                  |
//+------------------------------------------------------------------+
class BUYcondition5 : public iConditions
  {
public:
   bool              evaluate()
     {
      // TODO: condition Buy 5
      if(iLow(Symbol(), 0, 1) < ma.index(1) && iClose(Symbol(), 0, 1) > iOpen(Symbol(), 0, 2) && iClose(Symbol(), 0, 1) > iClose(Symbol(), 0, 2))
        {
         DrawLine("high");
         return true;
        }
...
BUYcondition5* buyCondition5;
class BUYcondition6 : public iConditions
  {
public:
   bool              evaluate()
     {
      // TODO: condition Buy 6
      return false;
     }
  };
BUYcondition6* buyCondition6;
   ...
   bool              evaluate()
     {
      ...
      return iRSI(NULL, 0, rsiPeriod, rsiAppliedPrice, 1) < rsiLevelDnH && iRSI(NULL, 0, rsiPeriod, rsiAppliedPrice, 1) > rsiLevelDnL;
     }
  };
   ...
   bool              evaluate()
     {
      ...
      return iMFI(NULL, 0, mfiPeriod, 1) < mfiLevelDnH && iMFI(NULL, 0, mfiPeriod, 1) > mfiLevelDnL;
     }
  };
...
SELLcondition4* sellCondition4;
class SELLcondition5 : public iConditions
  {
public:
   bool              evaluate()
     {
      // TODO: condition sell 5
      if(iHigh(Symbol(), 0, 1) > ma.index(1) && iClose(Symbol(), 0, 1) < iOpen(Symbol(), 0, 2) && iClose(Symbol(), 0, 1) < iClose(Symbol(), 0, 2))
        {
         DrawLine("low");
         return true;
        }
...
SELLcondition5* sellCondition5;
class SELLcondition6 : public iConditions
  {
public:
   bool              evaluate()
     {
      // TODO: condition sell 6
      return false;
     }
  };
SELLcondition6* sellCondition6;
...
public:
   ...
   Line*             price2(double Price)          { _price2 = Price;    return &this; }
   ...
   void              draw()
     {
      // draw line
      ObjectCreate(0, _name, OBJ_TREND, 0, _iniTm, _price, _endTm, _price2);
...
//|                                                                  |
//+------------------------------------------------------------------+
double LastHigh(string mode)
  {
   double   value = 0;
   datetime tm;
   for(int i = 3; i < candlesBack; i++)
     {
...
//|                                                                  |
//+------------------------------------------------------------------+
void DrawLine(string mode)
  {
   if(mode == "high")
     {
      line = new Line(Bars, LastHigh("time"), iTime(NULL, 0, 2), LastHigh("price"), clrBlue);
      double     op = iOpen(NULL, 0, 2);
      double     cl  = iClose(NULL, 0, 2);
      double     pr2 = op < cl ? cl : op;
      line.price2(pr2);
      line.draw();
      // delete line;
     }
   if(mode == "low")
     {
      line = new Line(Bars, LastLow("time"), iTime(NULL, 0, 2), LastLow("price"), clrCrimson);
      double     op = iOpen(NULL, 0, 2);
      double     cl  = iClose(NULL, 0, 2);
      double     pr2 = op > cl ? cl : op;
      line.price2(pr2);
...
//--- buys:
   ...
   conditionsToBuy.AddCondition(buyCondition4 = new BUYcondition4());
   conditionsToBuy.AddCondition(buyCondition5 = new BUYcondition5());
// conditionsToBuy.AddCondition(buyCondition6 = new BUYcondition6());
...
//--- sell:
   ...
   conditionsToSell.AddCondition(sellCondition4 = new SELLcondition4());
   conditionsToSell.AddCondition(sellCondition5 = new SELLcondition5());
// conditionsToSell.AddCondition(sellCondition6 = new SELLcondition6());
...
// Note: Indicators
   ...
   ma_slow = new MovingAverage();
   ma_slow.setSetup(ma_slowPeriod, ma_slowShift, ma_slowMethod, ma_slowAppliedPrice);
//rsi = new RSI();
      ...
      case CloseByPips:
        {
         ...
         if(floatingEA() >= moneyLimitWin)
           {
            return true;
           }
         if(floatingEA() < moneyLimitLoss)
           {
            return true;
           }
         break;
...
void doCloseGridControl()
  {
   if(closeGridTP > 0)
     {
      if(CheckPointer(gridBuy) != POINTER_INVALID)
         if(gridBuy.profit() >= closeGridTP)
           {
            gridBuy.closeGrid();
            delete gridBuy;
           }
      if(CheckPointer(gridSell) != POINTER_INVALID)
         if(gridSell.profit() >= closeGridTP)
           {
            gridSell.closeGrid();
            delete gridSell;
           }
   ...
   if(closeGridSL < 0)
     {
      if(CheckPointer(gridBuy) != POINTER_INVALID)
         if(gridBuy.profit() <= closeGridSL)
           {
            gridBuy.closeGrid();
            delete gridBuy;
           }
      if(CheckPointer(gridSell) != POINTER_INVALID)
         if(gridSell.profit() <= closeGridSL)
           {
            gridSell.closeGrid();
            delete gridSell;
           }
