...
extern color profit_color = Lime; // Profit color
extern color loss_color = Red; // Loss color
input int x = 15;
input int y = 5;
...
public:
   ...
   void Update()
   {
      ...
      while (it.Next())
      {
         if (it.IsBuy())
         {
            if (it.GetTakeProfit() != 0)
            {
               ...
               tp_curr += ((it.GetTakeProfit() - it.GetOpenPrice()) / tickSize) * unitCost * OrderLots() - OrderCommission() + OrderSwap();
            }
            if (it.GetStopLoss() != 0)
            {
               sl += it.GetStopLoss()- it.GetOpenPrice();
               sl_curr += ((it.GetStopLoss() - it.GetOpenPrice()) / tickSize) * unitCost * OrderLots() - OrderCommission() + OrderSwap();
            }
         }
         else
         {
            if (it.GetTakeProfit() != 0)
            {
               ...
               tp_curr += ((it.GetOpenPrice() - it.GetTakeProfit()) / tickSize) * unitCost * OrderLots() - OrderCommission() + OrderSwap();
            }
            if (it.GetStopLoss() != 0)
            {
               sl += it.GetOpenPrice() - it.GetStopLoss();
               sl_curr += ((it.GetOpenPrice() - it.GetStopLoss()) / tickSize) * unitCost * OrderLots() - OrderCommission() + OrderSwap();
            }
         }
      }
      ...
      string_window(_eaName + "Balance", x, y + 20, 0);
      ObjectSetText(_eaName + "Balance", "Stop loss: " + DoubleToStr(sl / pipSize, 1) + " pips (" + DoubleToStr(sl_curr, 2) + " " + AccountCurrency() + ")", _fontSize, _fontName, sl >= 0 ? profit_color : loss_color);
      ObjectSet(_eaName + "Balance", OBJPROP_CORNER, _textCorner);

      string_window(_eaName + "Equity", x, y + 35, 0);
      ObjectSetText(_eaName + "Equity", "Take profit: " + DoubleToStr(tp / pipSize, 1) + " pips (" + DoubleToStr(tp_curr, 2) + " " + AccountCurrency() + ")", _fontSize, _fontName, tp >= 0 ? profit_color : loss_color); 
