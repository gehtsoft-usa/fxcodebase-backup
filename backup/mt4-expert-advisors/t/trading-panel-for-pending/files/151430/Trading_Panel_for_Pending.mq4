...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=73875
...
bool OnInit_GUI()
{
    ...
    {
        res = gui.Create(0, "One Click Trading Panel", 0, 400, 0, 675, 110);
        if(res) gui.Run();
    }
    return res;
}
...
class BuyAction : public iActions
{
    bool doAction()
    {
        ...
        // double pr = distance == 0 ? 0 : NormalizeDouble((double)gui.edit3.Text(), _Digits);
...
class SellAction : public iActions
{
    bool doAction()
    {
        ...
        // double pr = distance == 0 ? 0 : NormalizeDouble((double)gui.edit4.Text(), _Digits);
...
class GUI : public CAppDialog
{
    ...
    GUI(int magic = 0)
    {
        ...
        _gapH = 10;
        _magic = magic;
        setButton1Action(&btnBuyAction);
        setButton2Action(&btnSellAction);
    }
    ~GUI()
    {
    ...
    bool Create(const long chart, const string name, const int subwin, const int x1, const int y1, const int x2, const int y2)
    {
        ...
        if(!Create_label("Pips", Col(1), Row(0), _high, _width, lb1)) return false;
        if(!Create_label("Price", Col(2), Row(0), _high, _width, lb2)) return false;
        if(!Create_button("Buy", Col(0), Row(1), _high, _width, bt1)) return false;
        if(!Create_button("Sell", Col(0), Row(2), _high, _width, bt2)) return false;
        ...
        if(!Create_Edit("Price Buy", Col(2), Row(1), _high, _width, edit3)) return false;
        if(!Create_Edit("Price Sell", Col(2), Row(2), _high, _width, edit4)) return false;
    ...
    void HoverEvents(const int id, const long& lparam, const double& dparam, const string& sparam)
    {
        ...
        if(bt2.IsActive()) bt2.ColorBackground(Crimson); else bt2.ColorBackground(CONTROLS_BUTTON_COLOR_BG);
    }
    ...
    void Refresh()
    {
        // Refresh Prices:
        string tx;
        tx = (string) NormalizeDouble(Price("buy", (int) edit1.Text(), _Symbol), _Digits);
        edit3.Text(tx);
        tx = (string) NormalizeDouble(Price("sell", (int) edit2.Text(), _Symbol), _Digits);
        edit4.Text(tx);

        // Refresh Buttons Text
        int d1 = (int) edit1.Text();
        int d2 = (int) edit2.Text();
        string buyType = d1 > 0 ? "Buy Stop" : d1 < 0 ? "Buy Limit" : "Buy";
        string sellType = d2 > 0 ? "Sell Limit" : d2 < 0 ? "Sell Stop" : "Sell";
    ...
    {
        string tx = (string)NormalizeDouble(Price("buy", (int) edit1.Text(), _Symbol), _Digits);
        edit3.Text(tx);
        Refresh();
    }
    void OnEndEdit_edit2()
    {
        string tx = (string) NormalizeDouble(Price("sell", (int) edit2.Text(), _Symbol), _Digits);
        edit4.Text(tx);
        Refresh();
    }
    void OnEndEdit_edit3() { edit1.Text((string) Pips("buy", (double) edit3.Text())); Refresh(); }
    void OnEndEdit_edit4() { edit2.Text((string) Pips("sell", (double) edit4.Text())); Refresh();}
    ...
    int Pips(string side, double price)
    {
        double distance = 0;
        int r = 0;
        if(side == "buy")
...
ON_EVENT(ON_END_EDIT, edit3, OnEndEdit_edit3)
ON_EVENT(ON_END_EDIT, edit4, OnEndEdit_edit4)
