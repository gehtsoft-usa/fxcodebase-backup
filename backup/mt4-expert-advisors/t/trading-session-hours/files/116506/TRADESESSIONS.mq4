...
void UpdateNY(const datetime dt)
{
    if (NY_S)
    {
        if (ny.SetDate(dt))
        {
            ...
            if (S_SE)
            {
                string id = "ny" + IndicatorObjPrefix + TimeToString(Time[start]) + "SE";
                if (ObjectFind(id) == -1)
                {
...
void UpdateLondon(const datetime dt)
{
    if (LO_S)
    {
        if (london.SetDate(dt))
        {
            ...
            if (S_SE)
            {
                string id = "london" + IndicatorObjPrefix + TimeToString(Time[start]) + "SE";
                if (ObjectFind(id) == -1)
                {
                    ObjectCreate(id, OBJ_TREND, 0, Time[start], Open[start], Time[end], Close[end]);
...
void UpdateTokyo(const datetime dt)
{
    if (TO_S)
    {
        if (tokyo.SetDate(dt))
        {
            ...
            if (S_SE)
            {
                string id = "tokyo" + IndicatorObjPrefix + TimeToString(Time[start]) + "SE";
                if (ObjectFind(id) == -1)
                {
                    ObjectCreate(id, OBJ_TREND, 0, Time[start], Open[start], Time[end], Close[end]);
...
void UpdateSydney(const datetime dt)
{
    if (SY_S)
    {
        if (sydney.SetDate(dt))
        {
            ...
            if (S_SE)
            {
                string id = "sydney" + IndicatorObjPrefix + TimeToString(Time[start]) + "SE";
                if (ObjectFind(id) == -1)
                {
                    ObjectCreate(id, OBJ_TREND, 0, Time[start], Open[start], Time[end], Close[end]);
