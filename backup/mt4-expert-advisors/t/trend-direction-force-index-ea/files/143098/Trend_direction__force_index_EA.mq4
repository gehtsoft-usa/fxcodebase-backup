...
// http://fxcodebase.com/code/viewtopic.php?f=38&t=71397
...
enum maTypes
{
   m_sma,     // simple moving average - SMA
   m_ema,     // exponential moving average - EMA
   m_dsema,   // double smoothed exponential moving average - DSEMA
   m_dema,    // double exponential moving average - DEMA
   m_tema,    // tripple exponential moving average - TEMA
   m_smma,    // smoothed moving average - SMMA
   m_lwma,    // linear weighted moving average - LWMA
   m_pwma,    // parabolic weighted moving average - PWMA
   m_alxma,   // Alexander moving average - ALXMA
   m_vwma,    // volume weighted moving average - VWMA
   m_hull,    // Hull moving average
   m_tma,     // triangular moving average
   m_sine,    // sine weighted moving average
   m_linr,    // linear regression value
   m_ie2,     // IE/2
   m_nlma,    // non lag moving average
   m_zlma,    // zero lag moving average
   m_lead,    // leader exponential moving average
   m_ssm,     // super smoother
   m_smoo     // smoother
};

input int             trendPeriod            = 20;
input double          TriggerUp              = 0.05; 
input double          TriggerDown            = -0.05; 
input maTypes         MaMethod               = 19;
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      double value = iCustom(_symbol, _timeframe, "Trend direction & force index - averages alerts 2", PERIOD_CURRENT, 
         trendPeriod, TriggerUp, TriggerDown, MaMethod, MaPeriod, 2, period);
      Print(value);
      return value > TriggerUp;
   }
};
...
int OnInit()
{
   double temp = iCustom(NULL, 0, "Trend direction & force index - averages alerts 2", 0, 0);
   if (GetLastError() == ERR_INDICATOR_CANNOT_LOAD)
   {
      Alert("Please, install the 'Trend direction & force index - averages alerts 2' indicator");
      return INIT_FAILED;
   }
