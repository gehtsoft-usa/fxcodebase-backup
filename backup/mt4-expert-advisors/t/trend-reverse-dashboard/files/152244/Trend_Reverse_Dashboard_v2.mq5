...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=73315
...
string file_custom_indicator = "TREND-REVERS.ex5";
...
// ------------------------------------------------------------------
input string             TitlePatern = "==== Trend Revers Indicator ====";     // ————————————
...
input int                ma1_Period = 200;                            // Period
...
input int                ma2_Period = 800;                            // Period
...
class MovingAverage
{
    ...
    double calculate(int buffer, int shift)
    {
        ...
        Print("EMA no se copio el buffer: error: ", GetLastError());
        return -1;
    }
...
class BBands
{
    ...
    double UpperBand(int shift) { return calculate(1, shift); }

    double LowerBand(int shift) { return calculate(2, shift); }

    double MidleBand(int shift) { return calculate(0, shift); }
};
...
class FinderPattern
{
    ...
    FinderPattern(string Symbol)
    {
        ...
        {
            handles[i] = iCustom(Symbol, TF[i], file_custom_indicator, Period1, Period2, Period3, Dev_Step_1, Dev_Step_2, Dev_Step_3, Symbol_1_Kod, Symbol_2_Kod, Symbol_3_Kod);

            ma1[i] = new MovingAverage(Symbol, TF[i]);
            ma1[i].setSetup(ma1_Period, ma1_Shift, ma1_Method, ma1_AppliedPrice);
            ma2[i] = new MovingAverage(Symbol, TF[i]);
            ma2[i].setSetup(ma2_Period, ma2_Shift, ma2_Method, ma2_AppliedPrice);

            bands[i] = new BBands(Symbol, TF[i], bbPeriods, bbDesviation, bbApliedPrice);

            // Print(__FUNCTION__," handles: ",i, " ",handles[i]);
    ...
    bool havePatternUp(int i, int tf_index)
    {
        ...
        double tReverse = index(handles[tf_index], 4, i);

        double ma1_value = ma1[tf_index].index(i);
        // Print(__FUNCTION__, " ma1_value: i:", i, " ", ma1_value);

        double ma2_value = ma2[tf_index].index(i);
        // Print(__FUNCTION__, " ma2_value: i:", i, " ", ma2_value);

        double lowBand = bands[tf_index].LowerBand(i);
        // Print(__FUNCTION__, " lowBand: ", lowBand);

        
        return  tReverse > 0 && ma1_value > ma2_value && iLow(_symbol,TF[tf_index],i) < lowBand;
    }
    ...
    bool havePatternDn(int i, int tf_index)
    {
        ...
        // double tReverse = index(handles[tf_index], 5, i);

        // double ma1_value = ma1[tf_index].index(i);
        // double ma2_value = ma2[tf_index].index(i);

        double upBand = bands[tf_index].UpperBand(i);
        // Print(__FUNCTION__," upBand: ",upBand);
        
        return  index(handles[tf_index], 5, i) > 0 && ma1[tf_index].index(i) < ma2[tf_index].index(i) && iHigh(_symbol,TF[tf_index],i) > upBand;
    }
    ...
    double index(int handle, int buffer, int shift)
    {
        ...
        Print("no se copio el buffer: error: ", GetLastError());
        return -1;
    }
};
