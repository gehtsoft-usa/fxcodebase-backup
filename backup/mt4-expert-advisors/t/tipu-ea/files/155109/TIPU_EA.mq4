...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=74801
...
string       file_custom_indicator = "Tipu_Trend";
int          bufferToBuy           = 0;
int          bufferToSell          = 1;
input string Tindi                 = "== TIPU - Setup =="; // ————————————
input bool   panel                 = false;                // Pannel On?
input string panelname             = "Trnd";               // Panel name:
string       title                 = "SRS";
input bool   showBox               = true; // Show Box
input bool   fillBox               = true; // Fill Box

// Exit Indicator
string tipu_rsi_file = "Tipu RSI";
int    bufRSIBuy     = 0;
int    bufRSISell    = 1;

input string             TipuRsiTitle           = "== RSI - Setup =="; // ————————————
input bool               closeByTipuRsiOn       = true;                     // Close By Tipu RSI?
input double             percenToCloseByTipuRsi = 50;                       // Percent to close %:
input int                RsiPeriod              = 14;                       // RSi Period
input ENUM_APPLIED_PRICE RsiApliedPrice         = PRICE_TYPICAL;            // Applied Price
input int                RsiOb                  = 70;                       // Overbougth
input int                RsiOs                  = 30;                       // Oversold
...
double Indi(int buffer, int candle)
{
    return iCustom(NULL, 0, file_custom_indicator,
                   panel, panelname, title, showBox, fillBox,
                   buffer, candle);
}

double Rsi(int buffer, int candle)
{
    // double rsiNow = iCustom(NULL, 0, tipu_rsi_file, RsiPeriod, RsiApliedPrice, RsiOb, RsiOs, buffer, candle);
    // Print(__FUNCTION__," rsiNow: ",rsiNow);
    // return rsiNow;
    double rsiNow = iRSI(NULL, 0, RsiPeriod, RsiApliedPrice, candle);
    Print(__FUNCTION__," rsiNow: ",rsiNow);
    return rsiNow;
}
  ...
  public:
    ...
    bool Now()
    {
        ...
        for (int i = ObjectsTotal(); i >= 0; i--) {
            nm = ObjectName(i);
            if (ObjectGetInteger(0, nm, OBJPROP_TYPE) == OBJ_RECTANGLE)
                break;
        }

        _ini = (datetime)ObjectGetInteger(0, nm, OBJPROP_TIME, 0);
        _end = (datetime)ObjectGetInteger(0, nm, OBJPROP_TIME, 1);

        bool haveBoxNow = _end >= iTime(NULL, 0, 0);

        if (haveBoxNow) {
            _active    = true;
            _buyPrice  = ObjectGetDouble(0, nm, OBJPROP_PRICE1);
            _sellPrice = ObjectGetDouble(0, nm, OBJPROP_PRICE2);
        }
        return haveBoxNow;
    }
    ...
    double BuyPrice() { return _buyPrice; }
    double SellPrice() { return _sellPrice; }
};
cBox box();
...
input bool   stopBox      = true;              // Stop Loss by Box?
...
class BUYcondition1 : public iConditions
{
  public:
    bool evaluate()
    {
        ...
        if (box.Active()) {
            return box.Now() == false && (iOpen(NULL, 0, 1) < box.BuyPrice()) && (Ask > box.BuyPrice());
        }
        if (!box.Active()) {
            return Indi(bufferToSell, 2) != EMPTY_VALUE && Indi(bufferToBuy, 1) > 0 && Indi(bufferToBuy, 1) != EMPTY_VALUE;
        }
        return false;
    }
};
...
class SELLcondition1 : public iConditions
{
  public:
    bool evaluate()
    {
        ...
        // return Indi(bufferToSell, 1) != EMPTY_VALUE;
        if (box.Active()) {
            return box.Now() == false && (iOpen(NULL, 0, 1) > box.SellPrice()) && (Bid < box.SellPrice());
        }
        if (!box.Active()) {
            return Indi(bufferToBuy, 2) != EMPTY_VALUE && Indi(bufferToSell, 1) > 0 && Indi(bufferToSell, 1) != EMPTY_VALUE;
        }
...
class PartialCloseConditionRSI : public iConditions
{
    // TODO: PC condition
    Order* _order;
  ...
  public:
    ...
    bool evaluate()
    {
        ...
        if (_order.type() == OP_BUY) {
            if (Rsi(0, 2) < RsiOb && Rsi(0, 1) > RsiOb)  {
                return true;
            }
        }
        if (_order.type() == OP_SELL) {
            if (Rsi(0, 2)>RsiOs && Rsi(0, 1) < RsiOs)  {
                return true;
            }
        }
...
PartialCloseConditionRSI* partialCloseCondition;
...
int OnInit()
{
    ...
    conditionsToPartialClose.AddCondition(partialCloseCondition = new PartialCloseConditionRSI());
    ...
    for (int i = mainOrders.qnt() - 1; i >= 0; i--) {
        if (mainOrders.index(i).countPartials() < uQntPartials) {
            ...
            if (conditionsToPartialClose.EvaluateConditions()) {
                ...
                partialCloseAction.order(mainOrders.index(i)).percent(percenToCloseByTipuRsi);
