...
// http://fxcodebase.com/code/viewtopic.php?f=38&t=69167
...
input bool show_outside = true; // Show Three Outside Signal
input bool show_inside = true; // Show Three Inside Signal
...
class ThreeOutsideBullishAlertCondition : public ABaseCondition
{
public:
   ThreeOutsideBullishAlertCondition(const string symbol, ENUM_TIMEFRAMES timeframe)
   ...
   bool IsPass(const int period, const datetime date)
   {
      ...
      double close2 = iClose(_symbol, _timeframe, period - 2);
      double open0 = iOpen(_symbol, _timeframe, period);
      double high0 = iHigh(_symbol, _timeframe, period);
      double high1 = iHigh(_symbol, _timeframe, period - 1);
      return close0 < open0 && close1 > high0 && close2 > high1;
   }
};

class ThreeOutsideBearishAlertCondition : public ABaseCondition
{
public:
   ThreeOutsideBearishAlertCondition(const string symbol, ENUM_TIMEFRAMES timeframe)
      :ABaseCondition(symbol, timeframe)
   {
   ...
   bool IsPass(const int period, const datetime date)
   {
      ...
      double close1 = iClose(_symbol, _timeframe, period - 1);
      double close2 = iClose(_symbol, _timeframe, period - 2);
      double low0 = iLow(_symbol, _timeframe, period);
      double low1 = iLow(_symbol, _timeframe, period - 1);
      double open0 = iOpen(_symbol, _timeframe, period);
      return close0 > open0 && close1 < low0 && close2 < low1;
   }
};

class ThreeInsideBullishAlertCondition : public ABaseCondition
{
public:
   ThreeInsideBullishAlertCondition(const string symbol, ENUM_TIMEFRAMES timeframe)
      :ABaseCondition(symbol, timeframe)
   {
...
class ThreeInsideBearishAlertCondition : public ABaseCondition
{
public:
   ThreeInsideBearishAlertCondition(const string symbol, ENUM_TIMEFRAMES timeframe)
      :ABaseCondition(symbol, timeframe)
   {
...
int init()
{
   ...
   IndicatorName = GenerateIndicatorName("Three Inside Outside Up Down candle pattern");
   ...
   if (show_outside)
   {
      ...
      ICondition* upCondition = (ICondition*) new ThreeOutsideBullishAlertCondition(_Symbol, (ENUM_TIMEFRAMES)_Period);
      ICondition* downCondition = (ICondition*) new ThreeOutsideBearishAlertCondition(_Symbol, (ENUM_TIMEFRAMES)_Period);
   ...
   {
      customStream = new CustomStream(_Symbol, (ENUM_TIMEFRAMES)_Period);
      ICondition* upCondition = (ICondition*) new ThreeInsideBullishAlertCondition(_Symbol, (ENUM_TIMEFRAMES)_Period);
      ICondition* downCondition = (ICondition*) new ThreeInsideBearishAlertCondition(_Symbol, (ENUM_TIMEFRAMES)_Period);
      id = CreateAlert(id, upCondition, downCondition);
      id = customStream.RegisterInternalStream(id);
   }
