...
// http://fxcodebase.com/code/viewtopic.php?f=38&t=70676
//+------------------------------------------------------------------+
...
input TradingSide trade_side = BothSides;
input double amount = 0.01; // Lots
input double entry_price = 0; // Entry price. 0 for auto
...
input double distance = 30; // Distance
...
public:
   ...
   void DoWork()
   {
      ...
      for (int i = 0; i < total; i++)
      {
         ...
         {
            changed = true;
            if (_newTradeAction != NULL && !_initial)
            {
               Print("111");
         ...
         else
         {
            ...
            {
               changed = true;
               if (_tradeChangedAction != NULL && !_initial)
      ...
      for (int index = 0; index < active_total; index++)
      {
         ...
         {
            changed = true;
            if (_closedTradeAction != NULL && OrderSelect(active_ticket[index], MODE_HISTORY) && !_initial)
...
CreateTrailingAction* trailingAction;

int OnInit()
{
   actions = new ActionOnConditionLogic();
   
   DeletePendingOrdersAction* onNewTradeAction = new DeletePendingOrdersAction(magic_number, slippage);
   tradingMonitor.SetOnNewTrade(onNewTradeAction);
   tradingMonitor.SetOnTradeChanged(onNewTradeAction);
   onNewTradeAction.Release();

   trailingAction = new CreateTrailingAction(trailing_start, trailing_step, actions);
...
void OnTick()
{
   ...
   if (first)
   {
      ...
      double entryPrice;
      int order;
      if (trade_side != ShortSideOnly)
      {
         orderBuilder = new OrderBuilder(actions);
         double price = entry_price == 0 ? iClose(_Symbol, PERIOD_D1, 1) : entry_price;
         entryPrice = price + distance * pipSize;
         order = orderBuilder
            .SetRate(entryPrice)
            ...
            .SetStopLoss(entryPrice - stop_loss * pipSize)
            .SetTakeProfit(entryPrice + take_profit * pipSize)
            .Execute(error);
         delete orderBuilder;
         if (order <= 0)
         {
         ...
         trailingAction.DoAction(order);
      }
      if (trade_side != LongSideOnly)
      {
         orderBuilder = new OrderBuilder(actions);
         double price = entry_price == 0 ? iClose(_Symbol, PERIOD_D1, 1) : entry_price;
         entryPrice = price - distance * pipSize;
