                ...
                const int &spread[])
{
   ...
   for (int pos = MathMin(bars_limit, rates_total - 1 - MathMax(prev_calculated - 1, toSkip)); pos >= 0 && !IsStopped(); --pos)
   {
      ...
      double pch = change1Value / srcInputStreamValue * 100;
      alma2Source._data[pos] = pch;
      double alma2Value;
      if (!alma2.GetValue(pos, alma2Value))
      {
         continue;
      }
      double avpch = alma2Value;
      sum3Source._data[pos] = avpch * avpch;
      double sum3Value;
      if (!sum3.GetValue(pos, sum3Value))
      {
         continue;
      }
      double rms  = bmult * MathSqrt(sum3Value / blength);
      plot1[pos] = avpch;
      plot2[pos] = rms;
      plot3[pos] = (-rms);
   }
