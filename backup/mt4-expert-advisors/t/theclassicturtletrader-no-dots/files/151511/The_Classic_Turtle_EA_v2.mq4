...
// https://fxcodebase.com/code/viewtopic.php?f=38&p=151300
...
string file_custom_indicator = "TheClassicTurtleTrader_no_dots.ex4";
int bufferToBuy = 0;
int bufferToSell = 1;
...
input bool StrictEntry = false;     // Apply strict entry parameters like the Turtles did
input bool StrictExit = false;     // Apply strict exit parameters like the Turtles did
input bool StrictStop = false;     // Apply strict stop-loss like the turtled did
input bool Greedy = false;     // Do not exit a trade unless it is in profit or the SL is hit
input bool EvaluateStoploss = true;      // Check if we have been stopped out and show future signals
input int  ATRPeriod = 20;        // ATRPeriod to set the stop-loss
input double ATRStopNumber = 2;         // N.Factor to calculate the stop-loss
input int lookBack = 1000;
input double offset = 5.0;
input color highLine = clrDodgerBlue;
input color lowLine = clrRed;
input bool showText = true;

// input alert  notificationsOn = 1;                      // Notifications
// input bool   arrowAlert = true;                             // Alert if arrow appears
// input bool   breakoutAlert = true;                          // Alert if breakdown
...
#ifdef CLOSE_ALL_ON
input string       TtpOposite = "== Close Conditions ==";     // ————————————
input bool         closeByBreakout = true;                    // Close when Breakout:
...
class PriceLine
{
    ...
    public:
    PriceLine(double price, double distance = 0, int candlesBack = 0, bool draw = false)
    {
        _price = price;
    ...
    bool CutUp()
    {
        ...
        {
            if(close(i) > pricePlus && open(i) < pricePlus)
            {
                _candleCutUp = i;
                _cutUp = true;
...
class PriceLineList
{
    ...
    void Add(double price, double distance)
    {
        ...
        prLines[t] = new PriceLine(price, distance, 1);
    }
    ...
    void clean()
    {
        ...
        ArrayResize(prLines, 0);
    }

};
PriceLineList buyLevels;
PriceLineList sellLevels;
...
class CloseSellCondition : public iConditions
{
    public:
    bool evaluate()
    {
        ...
        if(buyLevels.CutDn())
        {
            // Print("SELL Level - Breakout UP");
            return true;
        }
        return false;
    }
};
CloseSellCondition* closeSellCondition;
...
class CloseBuyCondition : public iConditions
{
    public:
    bool evaluate()
    {
        ...
        if(sellLevels.CutUp())
        {
            // Print("BUY Level - Breakout Dn");
            return true;
        }
        return false;
    }
};
CloseBuyCondition* closeBuyCondition;
...
class ConditionToCloseBuy : public iConditions
{
    ...
    bool evaluate()
    {
        ...
        if(closeByBreakout)
            if(closeBuyCondition.evaluate()) return true;
...
class ConditionToCloseSell : public iConditions
{
    ...
    bool evaluate()
    {
        ...
        if(closeByBreakout)
            if(closeSellCondition.evaluate()) return true;
...
int OnInit()
{
    ...
    closeSellCondition = new CloseSellCondition();
    ...
    closeBuyCondition = new CloseBuyCondition();
...
void OnTick()
{
    ...
    if(buyLevels.CutUp())
    {
        Print("BUY Level - Corte arriba");
    }
    //   if(sellLevels.CutDn())
    //   {
    //       Print("SELL Level - corte abajo");
    //   }
    //   buyLevels.Draw();
    //   sellLevels.Draw();
...
void GetLevels()
{
    ...
    for(int i = 0; i < t; i++)
    {
        ...
        int otype = ObjectGet(n, OBJPROP_TYPE);
        if(otype != 2)continue;

        datetime dt = ObjectGet(n, OBJPROP_TIME2);

        if(dt >= iTime(NULL, 0, 2))
        {
            double price = ObjectGet(n, OBJPROP_PRICE1);
            if(StringFind(n, "CCTH", 0) != -1) { sellLevels.Add(price, 0); }
            else { buyLevels.Add(price, 0); }
        }
    }
}
