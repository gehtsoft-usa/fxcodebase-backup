//Available @  https://fxcodebase.com/code/viewtopic.php?f=38&p=148103#p148103
...
void OnInit()
{
   ...
   sma1 = new SmaOnStream(sma1Source, Period);
   SetIndexBuffer(id++, plot1, INDICATOR_DATA);
   SetIndexBuffer(id++, plot1_dn, INDICATOR_DATA);
   ...
   SetIndexBuffer(id++, raw, INDICATOR_CALCULATIONS);
}
                ...
                const int &spread[])
{
   if (prev_calculated <= 0 || prev_calculated > rates_total)
   {
      ...
      ArrayInitialize(plot1_dn, EMPTY_VALUE);
      raw_DEFAULT_VALUE = EMPTY_VALUE;
      ArrayInitialize(raw, raw_DEFAULT_VALUE);
   ...
   for (int pos = MathMax(rates_total - 1 - bars_limit, MathMax(first, prev_calculated - 1)); pos < rates_total; ++pos)
   {
      ...
      if (pos - 1 < 0) { continue; }
      double temp = SafeMultiply(tick_volume[pos], (SafeMinus(close[pos], close[pos - 1])));
      sma1Source.SetValue(pos, temp);
      double sma1Value[1];
      if (!sma1.GetValues(pos, 1, sma1Value)) { sma1Value[0] = EMPTY_VALUE; }
      SetStream(raw, pos, sma1Value[0], raw_DEFAULT_VALUE);
      if (pos - 1 < 0) { continue; }
      plot1[pos] = (SafeGreater(raw[pos], raw[pos - 1]) ? raw[pos] : EMPTY_VALUE);
      plot1_dn[pos] = 0;
      if (pos - 1 < 0) { continue; }
      plot3[pos] = (SafeLE(raw[pos], raw[pos - 1]) ? raw[pos] : EMPTY_VALUE);
      plot3_dn[pos] = 0;
   }
   return rates_total;
}
