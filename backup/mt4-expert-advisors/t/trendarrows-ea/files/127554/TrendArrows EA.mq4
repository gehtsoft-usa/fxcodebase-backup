...
// http://fxcodebase.com/code/viewtopic.php?f=38&t=68712
...
public:
   ...
   {
      double value = iCustom(_symbol, _timeframe, "TrendArrows", ADX_Period, DeltaAdx, Smoothing, SignalGap, 2, period);
      return value != 0;
   }
};
...
public:
   ...
   bool IsPass(const int period)
   {
      double value = iCustom(_symbol, _timeframe, "TrendArrows", ADX_Period, DeltaAdx, Smoothing, SignalGap, 1, period);
      return value != 0;
   }
};
...
int OnInit()
{
   double temp = iCustom(NULL, 0, "TrendArrows", 0, 0);
   if (GetLastError() == ERR_INDICATOR_CANNOT_LOAD)
   {
      Alert("Please, install the 'TrendArrows' indicator");
      return INIT_FAILED;
   }
