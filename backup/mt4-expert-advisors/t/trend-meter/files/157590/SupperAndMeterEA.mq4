...
// https://fxcodebase.com/code/viewtopic.php?f=38&p=157520#p157520
...
input const string indicator_file = "SuperTrend";                          // Indicator File:
...
input string       tTrendMeter         = "==== Indicator Setup ===="; // ————————————————————————
input const string indicator_file_meter = "Trend_Meter";                          // Trend Meter File:
input bool filter_by_meter = true; // Filter by Trend Meter:

//---
int  bufferToBuy1              = 9;
int  bufferToBuy2              = 12;
int  bufferToBuy3              = 15;
int  bufferToSell1             = 10;
int  bufferToSell2             = 13;
int  bufferToSell3             = 16;
//---
double TrendMeter(int buffer, int candle = 1) { return iCustom(NULL, 0, indicator_file_meter, buffer, candle); }

class ConditionTrendMeterBuy : public iConditions
{
    double lastSignal;
  ...
  public:
    ...
    {
        for (int i = 1; i <= candles_back_for_signals; i++) {
            double indi1 = TrendMeter(bufferToBuy1, i);
            double indi2 = TrendMeter(bufferToBuy2, i);
            double indi3 = TrendMeter(bufferToBuy3, i);
    
            if(indi1 != EMPTY_VALUE && indi2 != EMPTY_VALUE && indi3 != EMPTY_VALUE) {                
                return true;
            }
        }
        return false;
...
ConditionTrendMeterBuy cdTrendMeterBuy;

class ConditionTrendMeterSell : public iConditions
{
    double lastSignal;
  ...
  public:
    ...
    {
        for (int i = 1; i <= candles_back_for_signals; i++) {
            double indi1 = TrendMeter(bufferToSell1, i);
            double indi2 = TrendMeter(bufferToSell2, i);
            double indi3 = TrendMeter(bufferToSell3, i);
...
ConditionTrendMeterSell cdTrendMeterSell;
...
bool OnInit_CustomIndicator()
{
    ...
    if(filter_by_meter) {
        LongStrategy.addCondition(&cdTrendMeterBuy);
        ShortStrategy.addCondition(&cdTrendMeterSell);        
    }
