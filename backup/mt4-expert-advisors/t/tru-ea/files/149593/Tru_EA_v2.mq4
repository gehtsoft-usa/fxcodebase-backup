...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=73365
...
enum CloseAllMode {
  ...
  CloseByPips,            // By Pips
  CloseByCandles,         // by Candles
  CloseByDesviation,      // by St. Desviation
	CloseByEMA              // by Moving Average
};
...
input int          uSleeppage     = 1000;               // Max Sleeppage:
...
input string TtpOptions              = "== Exit Rules ==";      // ————————————
CloseAllMode closeBy                 = CloseByEMA;              // Close All Mode:
input bool   closeAllControlON       = true;                    // Close Rules On:
input bool   closeByCandleOn         = true;                    // Close by candles on:
input int    candlestoclose          = 5;                       // Candles to close:
input bool   closeByDesviationOn     = true;                    // Close by desviation threshold on:
input double qDesviations            = 2.0;                     // St. Desviations:
input bool   closeByEMAOn            = true;                    // Close by ema on:
...
input string             T_              = "== Detrend Setup ==";            // ————————————
input int                uLength         = 10;                               // Length:
input string             Iema1           = "== Moving Average I Setup ==";   // ————————————
input int                ma1Period       = 10;                               // Periods
...
class SendNewOrder : public iActions
{
 ...
 public:
  ...
  bool doAction()
  {
    int tk = OrderSend(newOrder.symbol(), newOrder.type(), newOrder.lot(), newOrder.price(), uSleeppage, newOrder.sl(), newOrder.tp(), newOrder.comment(), newOrder.magic(), newOrder.expireTime(), clrNONE);
...
class BUYcondition2 : public iConditions
{
 public:
  bool evaluate()
  {
    ...
    return iClose(NULL, 0, 1) > ma1.index(1);
  }
};
...
if(closeByCandleOn)     { if(closeByCandles())     return true; }
if(closeByDesviationOn) { if(closeByDesviations()) return true; }
if(closeByEMAOn)        { if(closeByEma())         return true; }
...
return false;
	 ...
	 switch (closeBy)
   {
      ...
      case CloseByPips: 
      {
         ...
         if (floatingEA() < moneyLimitLoss) { return true; }         break;
      }
			case CloseByCandles: { return closeByCandles(); break; }			
			case CloseByDesviation: { return closeByDesviations(); break; }
   }
	return false;
}
// clang-format on
...
bool closeByCandles()
{
  // seleccionas la orden abierta
  for (int i = OrdersTotal() - 1; i >= 0; i--)
  {
    if (OrderSelect(i, SELECT_BY_POS) && OrderSymbol() == _Symbol && OrderMagicNumber() == magico)
    {
      // tomás la hora de apertura // la comparás con la hora de x velas hacia atrás
      return OrderOpenTime() < iTime(NULL, 0, candlestoclose - 1);
    }
  }
  return false;
}

bool closeByDesviations()
{
  double threshold = iStdDev(NULL, 0, ma1Period, 0, ma1Method, PRICE_CLOSE, 1) * qDesviations;
  return detrend(1) >= threshold;
}
...
// NOTE: Search the minimum value of "detrend" for all "length"
// next function return this: min(detrend,length)
double minDetrend()
{
  double min = 0;
  for (int i = 1; i <= uLength; i++)
  {
    double dtrend = detrend(i);
    if (dtrend < min || min == 0)
      min = dtrend;
  }
  return min;
...
// NOTE: Calculate detrend
/*
	iClose(NULL, 0, i) => is the close of the candle i
	ma1.index(i) => is a moving average object. This is setting by user in inputs "Moving Average I Setup"
  so the next function make this: (close - simple_moving_average(length))/simple_moving_average(length)
*/
double detrend(int i)
{
  return (iClose(NULL, 0, i) - (ma1.index(i))) / ma1.index(i);
}

// in the line 2721 I use this funtions to check the BUY Condition: detrend(1) <= minDetrend();
