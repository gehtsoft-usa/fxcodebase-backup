...
//https://fxcodebase.com/code/viewtopic.php?f=38&t=71195
...
double GetHighestOpen(string symbol, datetime date, int h1Period)
{
   int dayStartIndex = iBarShift(symbol, PERIOD_H1, date);
   if (dayStartIndex < 0)
   {
      return 0;
   }
   double open = iOpen(symbol, PERIOD_H1, dayStartIndex);
   for (int i = dayStartIndex - 1; i >= h1Period; --i)
   {
      if (open < iOpen(symbol, PERIOD_H1, i))
      {
         open = iOpen(symbol, PERIOD_H1, i);
      }
   }
   return open;
}

double GetLowestOpen(string symbol, datetime date, int h1Period)
{
   int dayStartIndex = iBarShift(symbol, PERIOD_H1, date);
   if (dayStartIndex < 0)
   {
...
public:
   ...
   {
      datetime time = iTime(_symbol, _timeframe, period);
      int index = period == 0 ? 0 : iBarShift(_symbol, PERIOD_H1, time);
      if (index < 0 || time == iTime(_symbol, PERIOD_H1, index))
      {
         return false;
      }
      int indexD1 = period == 0 ? 0 : iBarShift(_symbol, PERIOD_D1, time);
      if (indexD1 < 0)
      {
         return false;
      }
      double open = GetHighestOpen(_symbol, iTime(_symbol, PERIOD_D1, indexD1), index);

      return iOpen(_symbol, _timeframe, period) > open;
   }
};
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      ...
      double open = GetLowestOpen(_symbol, iTime(_symbol, PERIOD_D1, indexD1), index);
