...
string file_custom_indicator = "TDI-With_Alerts";
// NOTE: buffers
#define bbHigh 1
#define bbLow 3
#define rsi 4
...
enum ModeEntry { Market,
                 ...
                 PendingLimit };
// ——————————————————————————————————————————————————————————————————
input string Tindi       = "== TDI Setup ==";  // ————————————
input string NoteGeneral=" --- General Options --- ";
input bool Show_TrendVisuals=true;
input bool Show_SignalArrows=true;
input int SHIFT_Sideway=0;
input int SHIFT_Up_Down=0;
input string NoteIndic=" --- Indicator Options --- ";
input int RSI_Period=13;         //8-25
input int RSI_Price=0;           //0-6
input int Volatility_Band=34;    //20-40
input int RSI_Price_Line=2;
input int RSI_Price_Type=0;      //0-3
input int Trade_Signal_Line=7;
input bool SHOW_Trade_Signal_Line2=true;
input int Trade_Signal_Line2=18;
input int Trade_Signal_Type=0;   //0-3
input string NoteAlerts=" --- Alert Options --- ";
input bool BuySellAlerts=true;
input bool CautionAlerts=true;
...
class BUYcondition1 : public iConditions
{
 public:
  bool evaluate()
  {
      ...
      if(!allowBuy)
        if (indicator(rsi, 1) < indicator(bbLow, 1) && indicator(rsi, 2) >= indicator(bbLow, 2))allowBuy=true;
...
class BUYcondition2 : public iConditions
{
 public:
  bool evaluate()
  {
      ...
      if(allowBuy)
      {
        if(indicator(rsi, 1)>indicator(signal, 1) && indicator(rsi, 2)<=indicator(signal, 2))
        {
            allowBuy = false;
            return true;
        }
...
class SELLcondition1 : public iConditions
{
 public:
  bool evaluate()
  {
      ...
      if (!allowSell)
          if (indicator(rsi, 1) > indicator(bbHigh, 1) && indicator(rsi, 2) <= indicator(bbHigh, 2))allowSell = true;
...
class ConditionToCloseBuy1 : public iConditions
{
 public:
  bool evaluate()
  {
    if(indicator(rsi,1) < indicator(signal,1) && indicator(rsi,2) >= indicator(signal,2))return true;

    return false;
  }
};
ConditionToCloseBuy1* exitBuy;

class ConditionToCloseSell1 : public iConditions
{
 public:
  bool evaluate()
  {
...
ConditionToCloseSell1* exitSell;
...
int OnInit()
{
  ...
  conditionsToCloseBuy.AddCondition(exitBuy = new ConditionToCloseBuy1());
  conditionsToCloseBuy.AddCondition(conditionCloseBuy = new ConditionToCloseBuy());
  conditionsToCloseSell.AddCondition(exitSell = new ConditionToCloseSell1());
...
double indicator(int buffer, int index)
{
