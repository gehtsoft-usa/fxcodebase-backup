...
input color UpColor = Green; // Up color
input color DownColor = Red; // Down color
...
int init()
{
   IndicatorName = GenerateIndicatorName("TSF Rectangular Fill Bar");
...
int GetSignalStart(const int period)
{
...
int start()
{
   ...
   int pos = limit;
   if (ExtCountedBars > 1)
      pos = MathMin(pos, Bars - ExtCountedBars - 1);
   while (pos >= 1)
   {
      double a, b;
      LinReg(pos, a, b);
      TSF[pos] = b + (a * (Period - 1 - Period));
      Signal[pos] = TSF[pos] > TSF[pos + 1] ? 1 : -1;
      if (Signal[pos] != Signal[pos + 1])
      {
         int last = GetSignalStart(pos + 1);
         if (last != -1)
         {
            string id = IndicatorObjPrefix + TimeToString(Time[last]);
            double low = Low[iLowest(_Symbol, _Period, MODE_LOW, last - pos + 1, pos)];
            double high = High[iHighest(_Symbol, _Period, MODE_HIGH, last - pos + 1, pos)];
            ObjectCreate(0, id, OBJ_RECTANGLE, 0, Time[last], low, Time[pos], high);
            ObjectSetInteger(0, id, OBJPROP_COLOR, Signal[pos + 1] == 1 ? UpColor : DownColor);
            if (pos == 1 && lastDatetime != Time[0])
            {
               if (Signal[pos + 1] == 1)
                  _signaler.SendNotifications("New up zone");
               else
                  _signaler.SendNotifications("New down zone");
               lastDatetime = Time[0];
            }
         }
      }
