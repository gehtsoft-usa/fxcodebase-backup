...
// https://fxcodebase.com/code/viewtopic.php?f=38&p=147012
  ...
  public:
   bool evaluate()
   {
      // TODO: condition Buy 1
			double Curr = iCustom(NULL,0,"TrendSniper", showExtra, higherTime, std, candles, wicks, len, len2, RSI_extra_color, bars_limit, 5, 1);
			double Prev = iCustom(NULL,0,"TrendSniper", showExtra, higherTime, std, candles, wicks, len, len2, RSI_extra_color, bars_limit, 5, 2);
			// Print("Curr ", Curr);
			// Print("Prev ", Prev);

			if(Curr >= uLevelBuy && Prev < uLevelBuy) { return true; }
...
class SELLcondition1 : public iConditions
{
  public:
   bool evaluate()
   {
	...
	double Curr = iCustom(NULL,0,"TrendSniper.ex4", showExtra, higherTime, std, candles, wicks, len, len2, RSI_extra_color, bars_limit, 5, 1);
  double Prev = iCustom(NULL,0,"TrendSniper.ex4", showExtra, higherTime, std, candles, wicks, len, len2, RSI_extra_color, bars_limit, 5, 2);
    
		// Print("Curr ", Curr);
		// Print("Prev ", Prev);
			
			if(Curr <= uLevelSel && Prev > uLevelSel) { return true; }
