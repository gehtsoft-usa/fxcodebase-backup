...
int OnInit()
{
  ...
  SetIndexBuffer(id++, RSI_open, INDICATOR_CALCULATIONS);
	SetIndexBuffer(id++, RSI_high, INDICATOR_CALCULATIONS);
	SetIndexBuffer(id++, RSI_close, INDICATOR_CALCULATIONS);
  SetIndexBuffer(id++, RSI_low, INDICATOR_CALCULATIONS);
  ...
  ++id;
	...
	SetIndexBuffer(id++, gain_loss_close, INDICATOR_DATA); 
                ...
                const int&      spread[])
{
  ...
  if (pos >= rates_total) pos = rates_total - 1;
  for (; pos > 0; pos--)
  {
    ...
    if (showExtra)
    {
      ...
      if (index >= 0)
      {
        ...
        //   continue;
        // }
				rsi = iRSI(_Symbol, higherTime, len2, PRICE_CLOSE, index);
        RSI_extra[pos] = rsi;
