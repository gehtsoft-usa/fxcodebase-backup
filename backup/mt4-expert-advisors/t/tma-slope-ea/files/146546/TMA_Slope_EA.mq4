...
//https://fxcodebase.com/code/viewtopic.php?f=38&t=72439
...
// ------------------------------------------------------------------
// NOTE: Inputs indicator
input string T99                     = "== TMA Setup ==";        // == TMA Setup ==
input string OtherTimeFrames         = "Select below 0=current tf,1,5,15,30,60,240,1440,10080,43200";
input int    select_other_tf_to_show = 0;
input int    H_Pos_MTF               = 1;
input int    V_Pos_MTF               = 50;
input int    Font_Size_MTF           = 12;
input int    Corner_MTF              = 3;
input color  Font_Color_MTF          = Black;
input int    eintPeriod              = 20;
input double edblHigh1               = 0.4;
input double edblLow1                = -0.4;
...
double indicatorPrev=0;

bool BuySignal()
{
  string file           = "TMA_slope.ex4";
  int    buffer         = 6;
  int    shift          = 1;
  double valueToCompare = edblHigh1;
  double indicatorNow   = iCustom(NULL, 0, file, OtherTimeFrames, select_other_tf_to_show, H_Pos_MTF, V_Pos_MTF, Font_Size_MTF, Corner_MTF, Font_Color_MTF, eintPeriod, edblHigh1, edblLow1, atrPeriod, buffer, shift);
  // double indicatorPrev   = iCustom(NULL, 0, file, OtherTimeFrames, select_other_tf_to_show, H_Pos_MTF, V_Pos_MTF, Font_Size_MTF, Corner_MTF, Font_Color_MTF, eintPeriod, edblHigh1, edblLow1, atrPeriod, buffer, shift+2);

if(indicatorPrev == 0) { indicatorPrev = indicatorNow; }

Print("BuySignal: ", "indicatorPrev:", indicatorPrev, " indicatorNow: ", indicatorNow);

  if (indicatorNow >= valueToCompare && indicatorPrev < valueToCompare)
  {
	  return true;
	}
	
	// indicatorPrev = indicatorNow;
	return false;
}
...
bool SellSignal()
{
  ...
  int       buffer            = 6;
  int       shift             = 1;
  double    valueToCompare    = edblLow1;
  double    indicatorNow      = iCustom(NULL, 0, file, OtherTimeFrames, select_other_tf_to_show, H_Pos_MTF, V_Pos_MTF, Font_Size_MTF, Corner_MTF, Font_Color_MTF, eintPeriod, edblHigh1, edblLow1, atrPeriod, buffer, shift);
  // double indicatorPrev   = iCustom(NULL, 0, file, OtherTimeFrames, select_other_tf_to_show, H_Pos_MTF, V_Pos_MTF, Font_Size_MTF, Corner_MTF, Font_Color_MTF, eintPeriod, edblHigh1, edblLow1, atrPeriod, buffer, shift+2);
