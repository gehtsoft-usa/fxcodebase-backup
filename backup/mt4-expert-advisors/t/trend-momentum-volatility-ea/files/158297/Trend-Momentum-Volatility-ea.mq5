//Available @  https://fxcodebase.com/code/viewtopic.php?f=38&t=75625
...
input int fast_ema_length = 50; // Fast EMA length
input int slow_ema_length = 200; // Slow EMA length
...
input int macd_signal_period = 9; // MACD Signal Line Period
...
public:
   ...
   {
      refs = 1;
      fast = iMA(symbol, timeframe, fast_ema_length, 0, MODE_EMA, PRICE_CLOSE);
      slow = iMA(symbol, timeframe, slow_ema_length, 0, MODE_EMA, PRICE_CLOSE);
      rsi = iRSI(symbol, timeframe, rsi_period, PRICE_CLOSE);
      bb = iBands(symbol, timeframe, BB_Period, 0, BB_Deviation, PRICE_CLOSE);
      macd = iMACD(symbol, timeframe, macd_fast_ema_period, macd_slow_ema_period, macd_signal_period, PRICE_CLOSE);
   }
   ~EntryStreamData()
   {
   ...
   double GetFast(int period)
   {
      double buffer[1];
      if (CopyBuffer(fast, 0, period, 1, buffer) != 1)
      {
         return EMPTY_VALUE;
      }
      return buffer[0];
   }
   double GetSlow(int period)
   {
      double buffer[1];
      if (CopyBuffer(slow, 0, period, 1, buffer) != 1)
      {
         return EMPTY_VALUE;
      }
      return buffer[0];
   ...
   {
      double buffer[1];
      if (CopyBuffer(rsi, 0, period, 1, buffer) != 1)
      {
         return EMPTY_VALUE;
      }
      return buffer[0];
   }
   double GetBBUp(int period)
   {
      double buffer[1];
      if (CopyBuffer(bb, 0, period, 1, buffer) != 1)
      {
         return EMPTY_VALUE;
      }
      return buffer[0];
   }
   double GetBBDn(int period)
   {
      double buffer[1];
      if (CopyBuffer(bb, 1, period, 1, buffer) != 1)
      {
         return EMPTY_VALUE;
      }
      return buffer[0];
   }
   double GetMACD(int period)
   {
      double buffer[1];
      if (CopyBuffer(macd, MAIN_LINE, period, 1, buffer) != 1)
      {
         return EMPTY_VALUE;
      }
      return buffer[0];
   }
   double GetMACDSignal(int period)
   {
      double buffer[1];
      if (CopyBuffer(macd, SIGNAL_LINE, period, 1, buffer) != 1)
      {
         return EMPTY_VALUE;
      }
      return buffer[0];
...
public:
   ...
   virtual bool IsPass(const int period, const datetime date)
   {
      return data.GetFast(period) > data.GetSlow(period);
   }
};
...
class RSILongCondition : public ACondition
{
   EntryStreamData* data;
public:
   RSILongCondition(string symbol, ENUM_TIMEFRAMES timeframe, EntryStreamData* data)
      :ACondition(symbol, timeframe)
   {
      this.data = data;
      data.AddRef();
   ...
   virtual bool IsPass(const int period, const datetime date)
   {
      return data.GetRSI(period) > RSI_Oversold;
   }
};
class RSIShortCondition : public ACondition
{
   EntryStreamData* data;
public:
   RSIShortCondition(string symbol, ENUM_TIMEFRAMES timeframe, EntryStreamData* data)
      :ACondition(symbol, timeframe)
   {
      this.data = data;
      data.AddRef();
   ...
   virtual bool IsPass(const int period, const datetime date)
   {
      return data.GetRSI(period) < RSI_Overbought;
   }
};
class BBLongCondition : public ACondition
{
   EntryStreamData* data;
public:
   BBLongCondition(string symbol, ENUM_TIMEFRAMES timeframe, EntryStreamData* data)
      :ACondition(symbol, timeframe)
   {
      this.data = data;
      data.AddRef();
   ...
   virtual bool IsPass(const int period, const datetime date)
   {
      return data.GetBBDn(period) <= iClose(_symbol, _timeframe, period);
   }
};
class BBShortCondition : public ACondition
{
   EntryStreamData* data;
public:
   BBShortCondition(string symbol, ENUM_TIMEFRAMES timeframe, EntryStreamData* data)
      :ACondition(symbol, timeframe)
   {
      this.data = data;
      data.AddRef();
   ...
   virtual bool IsPass(const int period, const datetime date)
   {
      return data.GetBBUp(period) >= iClose(_symbol, _timeframe, period);
   }
};
class MACDLongCondition : public ACondition
{
   EntryStreamData* data;
public:
   MACDLongCondition(string symbol, ENUM_TIMEFRAMES timeframe, EntryStreamData* data)
      :ACondition(symbol, timeframe)
   {
      this.data = data;
      data.AddRef();
   ...
   virtual bool IsPass(const int period, const datetime date)
   {
      return data.GetMACD(period) > data.GetMACDSignal(period);
   }
};
class MACDShortCondition : public ACondition
{
   EntryStreamData* data;
public:
   MACDShortCondition(string symbol, ENUM_TIMEFRAMES timeframe, EntryStreamData* data)
      :ACondition(symbol, timeframe)
   {
      this.data = data;
      data.AddRef();
...
ICondition* CreateLongFilterCondition(string symbol, ENUM_TIMEFRAMES timeframe, EntryStreamData* data)
{
   ...
   AndCondition* condition = new AndCondition();
   condition.Add(new RSILongCondition(symbol, timeframe, data), false);
   condition.Add(new BBLongCondition(symbol, timeframe, data), false);
   condition.Add(new MACDLongCondition(symbol, timeframe, data), false);
   return condition;
}
...
ICondition* CreateShortFilterCondition(string symbol, ENUM_TIMEFRAMES timeframe, EntryStreamData* data)
{
   ...
   AndCondition* condition = new AndCondition();
   condition.Add(new RSIShortCondition(symbol, timeframe, data), false);
   condition.Add(new BBShortCondition(symbol, timeframe, data), false);
   condition.Add(new MACDShortCondition(symbol, timeframe, data), false);
   return condition;
}
