// More information about this indicator can be found at: // https://fxcodebase.com/code/viewtopic.php?f=38&t=73496
...
#property strict
#property description "Generate a grid increasing lots"
#property description "-------------------------------------------"
#property description "1. First trade must to maked mannually and the magic number must be zero (0)"
#property description "2. Allow multiple trades at same time"
#property description "3. Can control a grid for each trade"
#property description "4. In tester: the EA simulates to open random trades after x candles (you can setup this)"
...
input bool openNewByCandles = true; // Open New Trades after "x" candles ?
...
input double GridUser_multiplier = 2;                  // Multiplier:
input int    GridUser_gap        = 10;                 // Gap betwen orders (pips):
input bool   closeGridOn         = true;               // Use Close Grid?
input double closeGridTP         = 20;                 // Take Profit Grid $
input double closeGridSL         = -200;               // Stop Loss Grid -$
#else
  ...
  public:
    ...
    bool execute()
    {
        for (int i = OrdersTotal() - 1; i >= 0; i--) {
            if (OrderSelect(i, SELECT_BY_POS) && OrderSymbol() == _symbol && (OrderMagicNumber() == _magic || OrderMagicNumber() == 1234)) {
                for (int n = 0; n < ArraySize(_strategys); n++) {
                    Print(__FUNCTION__, " doing TSL in: ", OrderTicket());
...
input string             Iema           = "== Moving Average Filter Setup =="; // ————————————
...
input string       tEmaCross           = "== Enter By Ema Cross Setup =="; // ————————————
input bool         entreByEmaCrossOn   = true;                             // Enter by Ema Cross ?
input int          fast_maPeriod       = 12;                               // Fast Ema Period
int                fast_maShift        = 0;                                // Ma Shift
ENUM_MA_METHOD     fast_maMethod       = MODE_EMA;                         // Method
ENUM_APPLIED_PRICE fast_maAppliedPrice = PRICE_CLOSE;                      // Applied Price
input int          slow_maPeriod       = 89;                               // Slow Ema Period
int                slow_maShift        = 0;                                // Ma Shift
    ENUM_MA_METHOD     slow_maMethod       = MODE_EMA;                         // Method
ENUM_APPLIED_PRICE slow_maAppliedPrice = PRICE_CLOSE;                      // Applied Price
...
enum SourceNews { Manual, Auto, None };
...
class MovingAverage
{
  ...
  public:
    ...
    void setSetup(int set0, int set1, int set2, int set3)
    {
        _setup.setup0 = set0;
        _setup.setup1 = set1;
        _setup.setup2 = set2;
    ...
    double calculate(int buffer, int shift) { return iMA(_symbol, _tf, _setup.setup0, _setup.setup1, _setup.setup2, _setup.setup3, shift); }

    double index(int shift) { return calculate(0, shift); }
};
MovingAverage *ma;
MovingAverage *maFast;
MovingAverage *maSlow;

interface iIndicators { double calculate(int bufferNumber, int shift); };
interface IOrders
{
  public:
    virtual void Add()     = 0;
...
class Order
{
  ...
  public:
    Order(int id, string symbol, double price, double sl, double tp, double lot, int type, int magic, string comment, string strategy, datetime expireTime, datetime signalTime, double profit,
          double bkvWasDoIt, int countPartials)
        : _id(id), _symbol(symbol), _price(price), _sl(sl), _tp(tp), _lot(lot), _type(type), _magic(magic), _comment(comment), _strategy(strategy), _expireTime(expireTime), _signalTime(signalTime),
          _profit(profit), _bkvWasDoIt(bkvWasDoIt), _countPartials(countPartials)
    {
    }
...
void setOrdersList(bool magicOn, string magics, bool symbolsOn, string symbols)
{
   ...
   // ——————————————————————————————————————————————————————————————————
   void GetMarketOrders(string coment="")
   {
      for (int i = OrdersTotal() - 1; i >= 0; i--)
      {
         if (OrderSelect(i, SELECT_BY_POS, MODE_TRADES))
         {
            ...
            if (exist(OrderTicket()) == true) { continue; }
						...
						// filtro por comentario para separar las de las grid
						if(coment!="" && OrderComment()!=coment) { continue; }
   ...
   // ——————————————————————————————————————————————————————————————————
   bool GetLastMarketOrder(string coment="", int magi=0)
   {
      for (int i = OrdersTotal() - 1; i >= 0; i--)
      {
         if (OrderSelect(i, SELECT_BY_POS, MODE_TRADES))
...
class SendNewOrder : public iActions
{
  ...
  public:
    SendNewOrder(string side, double lots, string symbol = "", double price = 0, double sl = 0, double tp = 0, int magic = 0, string coment = "", datetime expire = 0)
    {
        ...
        newOrder.id(OrderTicket()).symbol(_symbol).type(_type).price(_price).sl(sl).tp(tp).lot(lots).magic(magic).comment(coment).expireTime(expire).profit(0);
    }

    ~SendNewOrder() { delete newOrder; }
    ...
    {
        int tk =
            OrderSend(newOrder.symbol(), newOrder.type(), newOrder.lot(), newOrder.price(), 1000, newOrder.sl(), newOrder.tp(), newOrder.comment(), newOrder.magic(), newOrder.expireTime(), clrNONE);
    ...
    Order *lastOrder() { return GetPointer(newOrder); }
};
...
class Grid
{
  ...
  public:
         ...
         double tp = 0, double sl = 0)
    {
        ...
        gridOrders.setOrdersList(magicFilterOn, IntegerToString(_magico) + ",1234", simbolFilterOn, _symbol);
    ...
    int  tk() { return _tk; }
    void tk(int tiket) { _tk = tiket; }

    void magico(int grid_magic) { _magico = grid_magic; }
    ...
    double profit()
    {
        ...
        for (int i = 0; i < qnt(); i++) {
            if (CheckPointer(gridOrders.index(i)) != POINTER_INVALID) gridResult += gridOrders.profit(i);
    ...
    double Lots() { return nextLot(); }

    double SL(string side) { return 0; }
    double TP(string side) { return 0; }
...
class ConditionSignalLimiter : public iConditions
{
  ...
  public:
    ConditionSignalLimiter(string Side) { _side = Side; }
    ~ConditionSignalLimiter() { ; }

    void lastSide(string lastSignal) { _lastSide = lastSignal; }
...
class ConditionGridActive : public iConditions
{
  ...
  public:
    ConditionGridActive(Grid *grid) { _grid = grid; }
...
class Levels
{
  ...
  public:
    Levels(iLevels *inpLevel) { _level = inpLevel; }
    ~Levels()
    {
        if (CheckPointer(_level) == 1) delete _level;
    }

    double calculateLevel() { return _level.calculateLevel(); }
    double pips() { return _level.pips(); }
};
Levels *levelTP;
Levels *levelSL;
...
class BUYcondition3 : public iConditions
{
  ...
  public:
    bool evaluate()
    {
        ...
        if(maFast.index(1) > maSlow.index(1) && maFast.index(2) <= maSlow.index(2) && maFast.index(1) != lastsignal)
        {
            lastsignal = maFast.index(1);
            return true;
        }
        return false;
    }
...
class ConditionCountBuys : public iConditions
{
  ...
  public:
    ConditionCountBuys(int maxBuys) { _maxBuys = maxBuys; }
    ~ConditionCountBuys() { ; }
...
class ConditionCountTrades : public iConditions
{
  ...
  public:
    ConditionCountTrades(int maxTrades) { _max = maxTrades; }
  ...
  public:
    ...
    int count()
    {
        if (_count < candlesTest) {
            ...
            _count = 0;
            Print(__FUNCTION__, " _count candles sell: ", _count);
        }
        return _count;
    }
};
...
class ConditionCountSells : public iConditions
{
  ...
  public:
    ConditionCountSells(int maxSells) { _maxSells = maxSells; }
    ~ConditionCountSells() { ; }
...
class BreackevenCondition : public iConditions
{
  ...
  public:
    void setOrder(Order * or) { _order = or ; }
...
//--- SESSIONS CONTROL:
    ...
    maFast.setSetup(fast_maPeriod, fast_maShift, fast_maMethod, fast_maAppliedPrice);

    maSlow = new MovingAverage();
    maSlow.setSetup(slow_maPeriod, slow_maShift, slow_maMethod, slow_maAppliedPrice);
...
void OnTick()
{
    ...
    if (CloseCandleMode) {
        ...
        if (newCandle.IsNewCandle()) {
            if (openNewByCandles == true && entreByEmaCrossOn == false ) {
                buyCondition2.count();
                sellCondition2.count();
            }
        }
    }
...
{
    if (CheckPointer(GetPointer(mainOrders)) == POINTER_INVALID) return;
    if (CheckPointer(mainOrders.last()) == POINTER_INVALID) return;

    // if (mainOrders.qnt() == 1)
    // {
    // lastOrder =

    if (!thisOrderHaveGrid(mainOrders.last().id())) {

        // if (mainOrders.last().type() == OP_BUY && CheckPointer(gridBuy) == POINTER_INVALID)
        if (mainOrders.last().type() == OP_BUY) {
            // NOTE: o
            int t = ArraySize(grids);
            if (ArrayResize(grids, t + 1)) {
                grids[t] = new Grid(_Symbol, "buy", mainOrders.last().price(), GridUser_gap, GridUser_multiplier, GridUser_maxCount, GridUser_maxLot, mainOrders.last().lot(), magico);
                grids[t].tk(mainOrders.last().id());
                grids[t].magico(1234);
            }
            // new Grid(_Symbol, "buy", mainOrders.last().price(), GridUser_gap, GridUser_multiplier, GridUser_maxCount, GridUser_maxLot, mainOrders.last().lot(), magico);
            // grid.tk(mainOrders.last().id());
            // addGrid(grid);
        ...
        if (mainOrders.last().type() == OP_SELL) {
            int t = ArraySize(grids);
            if (ArrayResize(grids, t + 1)) {
                grids[t] = new Grid(_Symbol, "sell", mainOrders.last().price(), GridUser_gap, GridUser_multiplier, GridUser_maxCount, GridUser_maxLot, mainOrders.last().lot(), magico);
                grids[t].tk(mainOrders.last().id());
                grids[t].magico(1234);
            }
            // delete grid;
...
void doGrids()
{
    for (int i = 0; i < ArraySize(grids); i++) {
        if (CheckPointer(grids[i]) != POINTER_INVALID) grids[i].doGrid();
    }
}

bool thisOrderHaveGrid(int tk)
{
    for (int i = 0; i < ArraySize(grids); i++) {
        if (CheckPointer(grids[i]) != POINTER_INVALID)
            if (grids[i].tk() == tk) {
                return true;
            }
    }
...
{
    for (int i = 0; i < ArraySize(grids); i++) {
        if (CheckPointer(grids[i]) != POINTER_INVALID) deleteGrid(i);
    }
    ArrayFree(grids);
}
...
void closeAll(string side = "")
{
    ...
    if (side == "sell" || side == "") {
        ...
        if (uDeletePendingsOn) {
            ...
            actionDeletePendingsSells.execute();
