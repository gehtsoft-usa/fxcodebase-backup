...
int OnInit()
{
  ...
  //---
	...
	if(uMaxTradesOn)conditionsToBuy.AddCondition(countTrades = new ConditionCountTrades(uMaxTrades));
  if(uMaxTradesOn)conditionsToSell.AddCondition(countTrades = new ConditionCountTrades(uMaxTrades));
...
void OnTick()
{
  ...
  if (!conditionsToBuy.EvaluateConditions()) return;
  if (!conditionsToSell.EvaluateConditions()) return;
