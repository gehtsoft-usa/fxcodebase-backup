...
public:
   ...
   {
      int index = iBarShift(_symbol, secondTf, date);
      return WTSUp[period] != EMPTY_VALUE && WTSUp[period + 1] == EMPTY_VALUE && iClose(_symbol, secondTf, index) > iOpen(_symbol, secondTf, index);
   }
};
...
public:
   ...
   {
      int index = iBarShift(_symbol, secondTf, date);
      return WTSDn[period] == EMPTY_VALUE && WTSDn[period + 1] != EMPTY_VALUE && iClose(_symbol, secondTf, index) < iOpen(_symbol, secondTf, index);
   }
};
...
input int Length=5;
input double Coeff=3.5;

double WTS[], WTSDn[], WTSUp[];
input ENUM_TIMEFRAMES TimeFrame = PERIOD_CURRENT; // Timeframe
input ENUM_TIMEFRAMES secondTf = PERIOD_CURRENT; // Second Timeframe
...
int init()
{
   ...
   IndicatorBuffers(8);
   IndicatorObjPrefix = GenerateIndicatorPrefix("wtsi");
