...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=76294
...
input double uLotMultiplier = 2;                        // Lot Multiplier:
...
class ConditionOpenBoth : public iConditions
{
    ...
    {
        if(countBuys.evaluate()==true || countSells.evaluate()==true) { return true; }
        return false;
    }
};
...
string lastWinDirection()
{
  ...
  for (int i = 0; i < total; i++) {
    ...
    Print("history[i].tk: ", tk);
    Print("history[i].profit: ", profit);
    Print("history[i].type: ", type);
    Print("history[i].POSITION id: ", id);
  }
  ...
  if (t == 1) {
    Print("la ultima ganadora fue SELL");
    return "sell";
  }
  ...
  for (int i = 0; i < PositionsTotal(); i++) {
    ...
    if (PositionGetSymbol(i) == _Symbol && PositionGetInteger(POSITION_MAGIC) == magico) {
      if (PositionGetInteger(POSITION_TYPE) == POSITION_TYPE_BUY) {
        if (trade.PositionModify(tk, 0.0, newTPbuy)) MainOrders.index(i).tp(newTPbuy);
      }
      if (PositionGetInteger(POSITION_TYPE) == POSITION_TYPE_SELL) {
        if (trade.PositionModify(tk, 0.0, newTPsell)) MainOrders.index(i).tp(newTPsell);
      }
    }
  }
}
