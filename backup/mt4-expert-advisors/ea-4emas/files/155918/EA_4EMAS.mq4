...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=75008
...
input int                ma2_period        = 10;                             // Periods:
...
input string             ma3_title         = "== Moving Average 2 Setup =="; // ————————————————————————
input int                ma3_period        = 15;                             // Periods:
input ENUM_MA_METHOD     ma3_method        = MODE_EMA;                       // Method:
input int                ma3_shift         = 0;                              // Shift:
input ENUM_APPLIED_PRICE ma3_applied_price = PRICE_CLOSE;                    // Applied Price:

double ma3(int shift) { return iMA(NULL, 0, ma3_period, ma3_shift, ma3_method, ma3_applied_price, shift); }

input string             ma4_title         = "== Moving Average 2 Setup =="; // ————————————————————————
input int                ma4_period        = 20;                             // Periods:
input ENUM_MA_METHOD     ma4_method        = MODE_EMA;                       // Method:
input int                ma4_shift         = 0;                              // Shift:
input ENUM_APPLIED_PRICE ma4_applied_price = PRICE_CLOSE;                    // Applied Price:

double ma4(int shift) { return iMA(NULL, 0, ma4_period, ma4_shift, ma4_method, ma4_applied_price, shift); }

class ConditionMaCrossUp : public iConditions
{
  public:
    bool evaluate() { return ma1(1) > ma4(1) && ma2(1) > ma4(1) && ma3(1) > ma4(1) && ma3(2) <= ma4(2); }
};
...
bool OnInit_MaCross()
{
    ...
    LongStrategy.addCondition(&cdMaCrossUp);
    ShortStrategy.addCondition(&cdMaCrossDn);
