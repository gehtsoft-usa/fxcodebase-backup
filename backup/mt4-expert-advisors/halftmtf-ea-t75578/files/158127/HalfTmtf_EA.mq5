//Available @  https://fxcodebase.com/code/viewtopic.php?f=38&t=75578
...
public:
   ...
   {
      refs = 1;
      indi = iCustom(symbol, timeframe, "HalfTmtf", param1);
   }
   ~EntryStreamData()
   {
   ...
   double GetBuy(int pos)
   {
      double buffer[1];
      if (CopyBuffer(indi, 0, pos, 1, buffer) == 1)
      {
         return buffer[0];
      }
      return EMPTY_VALUE;
   }
   double GetSell(int pos)
   {
      double buffer[1];
      if (CopyBuffer(indi, 1, pos, 1, buffer) == 1)
      {
         return buffer[0];
      }
      return EMPTY_VALUE;
...
public:
   ...
   virtual bool IsPass(const int period, const datetime date)
   {
      return data.GetBuy(period) != EMPTY_VALUE;
   }
};
...
public:
   ...
   virtual bool IsPass(const int period, const datetime date)
   {
      return data.GetSell(period) != EMPTY_VALUE;
   }
};
