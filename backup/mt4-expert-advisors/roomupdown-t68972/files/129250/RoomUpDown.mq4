...
// http://fxcodebase.com/code/viewtopic.php?f=38&t=68972
...
input string UpLevels                   = "75,100,125";
input string DownLevels                 = ".";
input int StartCandle                = 10;
input int LineLength                 = 5;
input int ADRdays                    = 30;
input bool FixFromOpen                = false;
input color LineColor = Green; // Line color

input ENUM_TIMEFRAMES adr_timeframe = PERIOD_D1; // ADR timeframe

input color vline_color = White; // Vertical line color
input ENUM_LINE_STYLE vline_style = STYLE_SOLID; // Vertical line style
input int vline_width = 1; // Vertical line width
...
int init()
{
   ...
   ndn = StrToDoubleArray(DownLevels == "." ? UpLevels : DownLevels, dnarr);
...
int start()
{
   ...
   for (int pos = limit; pos >= 0; --pos)
   {
      ...
      {
         double range = iHigh(_Symbol, adr_timeframe, k) - iLow(_Symbol, adr_timeframe, k);
      ...
      for (int i = 0; i < nup; ++i)
      {
         ...
         double level = 0;
         if (FixFromOpen)
            level = iOpen(_Symbol, adr_timeframe, 0) + adr * lvl / 100;
         else
            level = iLow(_Symbol, adr_timeframe, 0) + adr * lvl / 100;
         ResetLastError();
         ...
         ObjectSetString(0, id, OBJPROP_TOOLTIP, "+" + IntegerToString(lvl) + "% = " 
            + DoubleToString((adr * lvl / 100) / pipSize, 1) + " pips = " 
   ...
   int index = iBarShift(_Symbol, _Period, iTime(_Symbol, adr_timeframe, ADRdays));
   datetime time = Time[index];
   if (ObjectFind(0, id) == -1)
   {
      if (!ObjectCreate(0, id, OBJ_VLINE, 0, time, 0))
      {
         Print(__FUNCTION__, ". Error: ", GetLastError());
         return 0;
      }
      ObjectSetInteger(0, id, OBJPROP_COLOR, vline_color);
      ObjectSetInteger(0, id, OBJPROP_STYLE, vline_style);
      ObjectSetInteger(0, id, OBJPROP_WIDTH, vline_width);
   }
