...
//http://fxcodebase.com/code/viewtopic.php?f=38&t=70801
...
input int Lookback = 500; // Look Back Period
input int alertlevellow = 1; // Low Alert Level Threshold
input int alertlevelhigh = 99; // High Alert Level Threshold
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      return pctRank[period] <= alertlevellow;
   }
};
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      return pctRank[period] >= alertlevelhigh;
   }
};
...
int OnInit()
{
   IndicatorObjPrefix = GenerateIndicatorPrefix("cpr");
   IndicatorShortName("Close Percentile Rank");

   IndicatorBuffers(9);
   SetIndexStyle(0, DRAW_LINE);
   SetIndexBuffer(0, pctRank);
   SetIndexLabel(0, "pctRank");
...
double percentrank(int pos)
{
   int count = 0;
   for (int i = 1; i < Lookback; ++i)
   {
      if (iClose(_Symbol, tf, pos) >= iClose(_Symbol, tf, pos + i))
      {
         ++count;
      }
   }
                ...
                const int &spread[])
{
   ...
   {
      ArrayInitialize(pctRank, EMPTY_VALUE);
      if (customStream != NULL)
      {
         customStream.Init();
      }
