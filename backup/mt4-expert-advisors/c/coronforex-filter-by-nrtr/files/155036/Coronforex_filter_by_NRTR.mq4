...
string file_custom_indicator = "NRTR ATR Stop";
int    bufferUp              = 0;
int    bufferDn              = 1;
...
double NRTR(int buffer, int candle)
{
    return iCustom(NULL, 0, file_custom_indicator,
                   TimeFrame, ATR, Coeficient,
                   buffer, candle);
}
...
int start()
{
    ...
    for (i = limit, r = Bars - i - 1; i >= 0; i--, r++) {
        ...
        if (rsx[i] > levelOs && rsx[i] < levelOb)
            trend[i] = 0;
        cross[i] = (i < Bars - 1) ? (rsx[i] > levelOb) ? 1 : (rsx[i] < levelOs)                   ? -1
                                                         : (rsx[i] < levelOb && rsx[i] > levelOs) ? 0
                                                                                                  : cross[i + 1]
                                  : 0;
        if (trend[i] == 0)
            trArr[i] = 50.0;
        ...
        if (arrowsVisible) {
            ...
            if (i < (Bars - 1) && cross[i] != cross[i + 1]) {
                ...
                double indiDn = NRTR(bufferDn, i);
                
                if (indiDn != EMPTY_VALUE && indiDn > 0) {
                ...
                double indiUp = NRTR(bufferUp, i);

                if (indiUp != EMPTY_VALUE && indiUp > 0) {
                    if (arrowsOnZoneEnter && cross[i] == -1)
