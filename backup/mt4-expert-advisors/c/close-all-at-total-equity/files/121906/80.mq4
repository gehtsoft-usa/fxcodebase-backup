...
extern string Section1 = ""; // ------------------------------System1 parameters------------------------------
extern string Section1_1 = ""; // ST1 parameters
extern int ST_N = 10; // Number of periods
extern double ST_M = 1.0; // Multiplier
extern string Section1_2 = ""; // ST2 parameters
extern int ST_N2 = 10; // Number of periods
extern double ST_M2 = 1.0; // Multiplier
extern string Section1_3 = ""; // ST3 parameters
extern int ST_N3 = 10; // Number of periods
extern double ST_M3 = 1.0; // Multiplier
...
extern int no_of_positions = 1; // Max # of open positions
...
extern bool dieresi = false; // dieresi gia STOP
extern bool polaplasiasmos = true; // polaplasiasmos gia STOP
extern double Dieretis = 2.0; // Dia or Epi
...
extern bool dieresi2 = false; // dieresi gia LIMIT
extern bool polaplasiasmos2 = true; // polaplasiasmos gia LIMIT
extern double Dieretis2 = 4.0; // Dia or Epi
            ...
            case ERR_INVALID_TRADE_VOLUME:
               {
                  ...
                  {
                     Print("Volume of the lot is too low: " + DoubleToStr(_amount) + " Min lot is: " + DoubleToStr(minVolume));
                     break;
                  }
                  double maxVolume = SymbolInfoDouble(_instrument, SYMBOL_VOLUME_MAX);
                  if (_amount > maxVolume)
                  {
                     Print("Volume of the lot is too high: " + DoubleToStr(_amount) + " Max lot is: " + DoubleToStr(maxVolume));
                     break;
                  }
                  Print("Invalid volume: " + DoubleToStr(_amount));
               }
               break;
            case 4109:
...
public:
   ...
   void WhenOrderType(const int orderType)
   {
      _useOrderType = true;
      _orderType = orderType;
   }

   void WhenMegicNumber(const int magicNumber)
   {
...
class TradeController
{
   ...
   {
      double DistanceB = CalcDistanceB(period);
      double DistanceS = CalcDistanceS(period);
      if (DistanceB > 50 && DistanceS > 50 && DistanceB < 200 && DistanceS < 200)
      {
         double ST = iCustom(NULL, 0, "ST", ST_N, ST_M, 2, period);
         double ST2 = iCustom(NULL, 0, "ST", ST_N2, ST_M2, 2, period);
         double ST3 = iCustom(NULL, 0, "ST", ST_N3, ST_M3, 2, period);
         return ST == 1 && ST2 == 1 && ST3 == 1;
      }
      return false;
   }
...
public:
   ...
   void DoTrading()
   {
      ...
      if (IsLongCondition(0) && !IsLongCondition(1))
      {
         ...
         {
            OrdersIterator it1();
            it1.WhenMegicNumber(MagicNumber);
            it1.WhenOrderType(OP_BUY);
            int buy_positions = it1.Count();
            OrdersIterator it2();
            it2.WhenMegicNumber(MagicNumber);
...
private:
   double CalcDistanceS(const int period)
   {
      int k = 0;
      int l = 1;
      do
      {
         k++;
         l++;
         if (iClose(_calculator.GetSymbol(), _timeframe, period + k) < iOpen(_calculator.GetSymbol(), _timeframe, period + k)
            && iClose(_calculator.GetSymbol(), _timeframe, period + l) > iOpen(_calculator.GetSymbol(), _timeframe, period + l))
         {
            return MathAbs(iHigh(_calculator.GetSymbol(), _timeframe, period + k) - iLow(_calculator.GetSymbol(), _timeframe, period)) / _calculator.GetPipSize();
         }
      }
      while (true);
      return 0;
   ...
   double CalcDistanceB(const int period)
   {
      int k = 0;
      int l = 1;
      do
      {
         ...
         if (iClose(_calculator.GetSymbol(), _timeframe, period + k) > iOpen(_calculator.GetSymbol(), _timeframe, period + k)
            && iClose(_calculator.GetSymbol(), _timeframe, period + l) < iOpen(_calculator.GetSymbol(), _timeframe, period + l))
         {
            return MathAbs(iHigh(_calculator.GetSymbol(), _timeframe, period) - iLow(_calculator.GetSymbol(), _timeframe, period + k)) / _calculator.GetPipSize();
         }
      }
      while (true);
      return 0;
   ...
   bool IsTradingTime(datetime dt)
   {
      ...
      if (!TimeToStruct(dt, current_time))
         return false;
      if (current_time.day > 23 && current_time.mon == 12)
         return true;
   ...
   {
      double DistanceB = CalcDistanceB(0);
      double limit = 0.0;
      if (dieresi2)
         limit = _calculator.CalculateTakeProfit(true, DistanceB / Dieretis2, StopLimitPips, amount, MarketInfo(_calculator.GetSymbol(), MODE_ASK));
      if (polaplasiasmos2)
         limit = _calculator.CalculateTakeProfit(true, DistanceB * Dieretis2, StopLimitPips, amount, MarketInfo(_calculator.GetSymbol(), MODE_ASK));
      double stop = 0.0;
      if (dieresi)
         stop = _calculator.CalculateStopLoss(true, DistanceB / Dieretis, StopLimitPips, amount, MarketInfo(_calculator.GetSymbol(), MODE_ASK));
      if (polaplasiasmos)
         stop = _calculator.CalculateStopLoss(true, DistanceB * Dieretis, StopLimitPips, amount, MarketInfo(_calculator.GetSymbol(), MODE_ASK));
   ...
   {
      double DistanceS = CalcDistanceS(0);
      double limit = 0.0;
      if (dieresi2)
         limit = _calculator.CalculateTakeProfit(false, DistanceS / Dieretis2, StopLimitPips, amount, MarketInfo(_calculator.GetSymbol(), MODE_BID));
      if (polaplasiasmos2)
         limit = _calculator.CalculateTakeProfit(false, DistanceS * Dieretis2, StopLimitPips, amount, MarketInfo(_calculator.GetSymbol(), MODE_BID));
      double stop = 0.0;
      if (dieresi)
         stop = _calculator.CalculateStopLoss(false, DistanceS / Dieretis, StopLimitPips, amount, MarketInfo(_calculator.GetSymbol(), MODE_BID));
      if (polaplasiasmos)
         stop = _calculator.CalculateStopLoss(false, DistanceS * Dieretis, StopLimitPips, amount, MarketInfo(_calculator.GetSymbol(), MODE_BID));
      if (LotsType == PositionSizeRisk)
      {
