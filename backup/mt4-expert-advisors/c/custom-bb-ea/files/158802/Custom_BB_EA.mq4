//Available @  https://fxcodebase.com/code/viewtopic.php?f=38&t=75764
...
input const string indicator_file = "custom_BB";                          // Indicator File:
//---
int  bufferToBuy              = 4;
int  bufferToSell             = 4;
  ...
  public:
    bool evaluate()
    {
        for (int i = 1; i <= candles_back_for_signals; i++) {
            ...
            if (indi > 0 && indi != EMPTY_VALUE && lastSignal <= 0) {
                lastSignal = indi;
                return true;
            }
        }
  ...
  public:
    bool evaluate()
    {
        for (int i = 1; i <= candles_back_for_signals; i++) {
            ...
            Print("line: ",__LINE__,"--- BUSCANDO SELL --- indi: ",indi);
            if (indi < 0 && indi != EMPTY_VALUE && lastSignal >= 0) {
                lastSignal = indi;
                return true;
            }
...
// MARK: funcion OnTick
void OnTick() { 
    // signal = LastArrowType();
    // Print("line: ",__LINE__," signal: ",signal);
    Trader.doTrading(); 
}
...
int LastArrowType()
{
    ...
    // Recorre las velas hacia atrás
    for (int candleShift = 1; candleShift < 100; candleShift++)
    {
        int      candleTime = iTime(NULL, 0, candleShift);
        double   candleHigh = iHigh(NULL, 0, candleShift);
        double   candleLow  = iLow(NULL, 0, candleShift);

        // Recorre todos los objetos en el gráfico
        for (int i = 0; i < totalObjects; i++)
        {
            string objectName = ObjectName(0, i);

            // Verifica si el objeto es de tipo flecha
            if (ObjectType(objectName) == OBJ_ARROW)
            {
                int    objectTime  = ObjectGetInteger(0, objectName, OBJPROP_TIME1);
                double objectPrice = ObjectGetDouble(0, objectName, OBJPROP_PRICE1);

                if (objectTime == candleTime) 
                {
                    if (objectPrice <= candleLow)
                    {
                        if (lastSignalBuy == objectTime) // Si es igual a la última señal de compra
                        {
                            return 0;
                        }
                        lastSignalBuy = objectTime; // Actualiza la última señal de compra
                        return 1; // Señal de compra
                    }
                    
                    // Si la flecha está encima del high, es una señal de venta (sell)
                    Print("line: ",__LINE__," objectPrice: ",objectPrice);
                    Print("line: ",__LINE__," candleHigh: ",candleHigh);
                    if (objectPrice >= candleHigh)
                    {
                        if (lastSignalSell == objectTime) // Si es igual a la última señal de venta
                        {
                            // Print("line: ",__LINE__," lastSignalSell: ",(datetime)lastSignalSell);
                            return 0;
                        }
                        lastSignalSell = objectTime; // Actualiza la última señal de venta
    ...
    return 0; // No se encontró ninguna flecha
}
