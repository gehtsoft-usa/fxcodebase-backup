//Available @  https://fxcodebase.com/code/viewtopic.php?f=38&t=75191
...
#property indicator_label4 "Long Stop"
#property indicator_type4  DRAW_LINE
#property indicator_color4 Green
#property indicator_style4 STYLE_SOLID
#property indicator_width4 1
#property indicator_label5 "Long Stop Start"
#property indicator_type5  DRAW_ARROW
#property indicator_color5 Green
#property indicator_style5 STYLE_SOLID
#property indicator_width5 1
#property indicator_label6 "Buy Label"
#property indicator_type6  DRAW_ARROW
#property indicator_color6 Green
#property indicator_style6 STYLE_SOLID
#property indicator_width6 1
#property indicator_label7 "Short Stop"
#property indicator_type7  DRAW_LINE
#property indicator_color7 Red
#property indicator_style7 STYLE_SOLID
#property indicator_width7 1
#property indicator_label8 "Short Stop Start"
#property indicator_type8  DRAW_ARROW
#property indicator_color8 Red
#property indicator_style8 STYLE_SOLID
#property indicator_width8 1
#property indicator_label9 "Sell Label"
...
input int    length         = 22;     // ATR Period
input double mult           = 3;      // ATR Multiplier
input bool   showLabels     = true;   // Show Buy/Sell Labels ?
input bool   useClose       = true;   // Use Close Price for Extremums ?
input bool   highlightState = true;   // Highlight State ?
input int    bars_limit     = 100000; // Bars limit
double       longStop[], shortStop[];
double       nzfloat(double val, double replacement = 0) { return val == EMPTY_VALUE ? replacement : val; }
...
class SmaOnStream : public AOnStream
{
  ...
  public:
    ...
    {
        int totalBars = Bars;
        if (ArrayRange(_buffer, 0) != totalBars) ArrayResize(_buffer, totalBars);
...
class HighestHighStream : public AOnStream
{
  ...
  public:
    HighestHighStream(string symbol, ENUM_TIMEFRAMES timeframe, int loopback) : AOnStream(new SimplePriceStream(symbol, timeframe, PriceHigh)) { _source.Release(); }
    HighestHighStream(IStream *source, int loopback) : AOnStream(source) { _loopback = loopback; }

    bool GetValue(const int period, double &val)
    {
        if (!_source.GetValue(period, val)) return false;

        for (int i = 1; i < _loopback; ++i) {
            double value;
            if (!_source.GetValue(period + i, value)) return false;
...
class LowestLowStream : public AOnStream
{
  ...
  public:
    LowestLowStream(string symbol, ENUM_TIMEFRAMES timeframe, int loopback) : AOnStream(new SimplePriceStream(symbol, timeframe, PriceLow)) { _source.Release(); }
    LowestLowStream(IStream *source, int loopback) : AOnStream(source) { _loopback = loopback; }
...
IStream *          atr1;
SimplePriceStream *highest2Source;
IStream *          highest2;
IStream *          highest3;
SimplePriceStream *lowest4Source;
IStream *          lowest4;
IStream *          lowest5;
double             dir[];
double             longColor[];
...
int init()
{
    ...
    IndicatorShortName("Chandelier Exit");
    IndicatorBuffers(12);
    int id = 0;
    SetIndexBuffer(id++, dir);
    SetIndexBuffer(id++, longColor);
    SetIndexBuffer(id++, shortColor);
    SetIndexBuffer(id++, plot4);
    SetIndexBuffer(id, plot5);
    SetIndexArrow(id++, 108);
    ...
    SetIndexBuffer(id++, longStop);
    SetIndexBuffer(id++, shortStop);
    atr1           = new ATRStream(_Symbol, (ENUM_TIMEFRAMES)_Period, length);
    highest2Source = new SimplePriceStream(_Symbol, (ENUM_TIMEFRAMES)_Period, PriceClose);
    highest2       = new HighestHighStream(highest2Source, length);
    highest3       = new HighestHighStream(_Symbol, (ENUM_TIMEFRAMES)_Period, length);
    lowest4Source  = new SimplePriceStream(_Symbol, (ENUM_TIMEFRAMES)_Period, PriceClose);
    lowest4        = new LowestLowStream(lowest4Source, length);
    lowest5        = new LowestLowStream(_Symbol, (ENUM_TIMEFRAMES)_Period, length);
                ...
                const long &tick_volume[], const long &volume[], const int &spread[])
{
    ...
    for (int pos = MathMin(bars_limit, rates_total - 1 - MathMax(prev_calculated - 1, toSkip)); pos >= 0 && !IsStopped(); --pos) {
        ...
        longStop[pos]       = ((useClose ? highest2Value : highest3Value)) - atr;
        double longStopPrev = nzfloat(longStop[pos + 1], longStop[pos]);
        longStop[pos]       = ((close[pos + 1] > longStopPrev) ? MathMax(longStop[pos], longStopPrev) : longStop[pos]);
        double lowest4Value;
        ...
        shortStop[pos]       = ((useClose ? lowest4Value : lowest5Value)) + atr;
        double shortStopPrev = nzfloat(shortStop[pos + 1], shortStop[pos]);
        shortStop[pos]       = ((close[pos + 1] < shortStopPrev) ? MathMin(shortStop[pos], shortStopPrev) : shortStop[pos]);
        dir[pos]             = pos < rates_total - 1 ? dir[pos + 1] : 1;
        dir[pos]             = ((close[pos] > shortStopPrev) ? 1 : ((close[pos] < longStopPrev) ? (-1) : dir[pos]));
        longColor[pos]       = pos < rates_total - 1 ? longColor[pos + 1] : 1;
        shortColor[pos]      = pos < rates_total - 1 ? shortColor[pos + 1] : 1;
        double longStopPlot = plot4[pos] = ((dir[pos] == 1) ? longStop[pos] : EMPTY_VALUE);
        bool   buySignal                 = ((dir[pos] == 1) && (dir[pos + 1] == (-1)));
        plot5[pos]                       = (buySignal ? longStop[pos] : EMPTY_VALUE);
        plot6[pos]                       = ((buySignal && showLabels) ? longStop[pos] : EMPTY_VALUE);
        double shortStopPlot = plot7[pos] = ((dir[pos] == 1) ? EMPTY_VALUE : shortStop[pos]);
        bool   sellSignal                 = ((dir[pos] == (-1)) && (dir[pos + 1] == 1));
        plot8[pos]                        = (sellSignal ? shortStop[pos] : EMPTY_VALUE);
        plot9[pos]                        = ((sellSignal && showLabels) ? shortStop[pos] : EMPTY_VALUE);
        double midPricePlot = plot10[pos] = (open[pos] + high[pos] + low[pos] + close[pos]) / 4;
        color  longFillColor              = (highlightState ? (((dir[pos] == 1) ? longColor[pos] : EMPTY_VALUE)) : EMPTY_VALUE);
        color  shortFillColor             = (highlightState ? (((dir[pos] == (-1)) ? shortColor[pos] : EMPTY_VALUE)) : EMPTY_VALUE);
        bool   changeCond                 = (dir[pos] != dir[pos + 1]);
    }
