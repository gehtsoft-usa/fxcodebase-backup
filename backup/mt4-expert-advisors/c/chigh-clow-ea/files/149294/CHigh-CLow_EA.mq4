...
//https://fxcodebase.com/code/viewtopic.php?f=38&t=73271
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
			if (iClose(_symbol, _timeframe, period) == iHigh(_symbol, _timeframe, period))
      {
				if(iClose(Symbol(),0, 1) > ma.index(1))
				{
					return true;
				}
			}
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      if (iClose(_symbol, _timeframe, period) == iLow(_symbol, _timeframe, period))
      {
				if(iClose(Symbol(),0, 1) < ma.index(1))
				{
					return true;
...
public:
   ...
   {
      //TODO: implement
				if(iClose(Symbol(),0, 1) < ma.index(1) && iOpen(Symbol(),0, 1) < ma.index(1))
				{
					return true;
				}
      return false;
