...
//http://fxcodebase.com/code/viewtopic.php?f=38&t=69684
...
extern int  tenkan_sen                = 7; // Tenkan Sen Period
extern int  kijun_sen                 = 21; // Kijun Sen Period
extern int  senkou_span_b             = 48; // Senkou Span B Period
...
public:
   ...
   {
      double ichValue0 = chikou_out[period];
      return ichValue0 > iOpen(_symbol, _timeframe, period) && ichValue0 < iClose(_symbol, _timeframe, period);
   }
};
...
int init()
{
   ...
   IndicatorName = GenerateIndicatorName("chikou highlite");
   ...
   SetIndexBuffer(0, chikou_out);
   SetIndexLabel(0, "Chikou");
...
int start()
{
   ...
   int minBars = 1;
   int limit = MathMax(kijun_sen, MathMin(Bars - 1 - minBars, Bars - counted_bars - 1));
   for (int pos = limit; pos >= kijun_sen; --pos)
   {
      chikou_out[pos] = iIchimoku(_Symbol, 0, tenkan_sen, kijun_sen, senkou_span_b, MODE_CHIKOUSPAN, pos);
      if (customStream != NULL)
      {
