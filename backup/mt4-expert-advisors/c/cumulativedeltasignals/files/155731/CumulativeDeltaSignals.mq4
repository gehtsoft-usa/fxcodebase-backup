//Available @  https://fxcodebase.com/code/viewtopic.php?f=38&t=74969
...
extern double  confirmation_periods = 75; 
extern double  signal_periods = 14; 
...
int init() {
  ...
  SetIndexBuffer(17,confirmation);      SetIndexEmptyValue(17,0); SetIndexStyle(17,DRAW_LINE, STYLE_SOLID, 2, Gold);
  
  SetIndexBuffer(18,ArrowUp);            SetIndexEmptyValue(18,0); SetIndexStyle(18,DRAW_ARROW, STYLE_SOLID, 1, Blue); SetIndexArrow(18, 233);
  SetIndexBuffer(19,ArrowDn);            SetIndexEmptyValue(19,0); SetIndexStyle(19,DRAW_ARROW, STYLE_SOLID, 1, Red); SetIndexArrow(19, 234);
...
{
   if(confirmation_periods <=0 ) confirmation_periods = 1;
   double sum = 0;
      for (int p = confirmation_periods; p>=0; p--) {
         sum += closed[p];
      }    
      confirmation[0] = NormalizeDouble(sum/confirmation_periods, _Digits);
}
...
subida_media = ema de 
AvUp = suma de n Variaciones positivas
AvDn = suma de n Variaciones negativas
RS = AvUp / AvDn
RSI = 100 - (100 / (1 + RS) 
...
// void DrawSignal()
// {
//    double sumVarPositive = 0;
//    double sumVarNegative = 0;
//    double rs;

//       for (int p = signal_periods; p>=0; p--) {
//          double var = closed[p] - closed[p-1];
//          if(var >0) {
//             sumVarPositive += var;
//          } else {
//             sumVarNegative += fabs(var);
//          }
//       }    
//       double avUp = sumVarPositive / signal_periods;
//       double avDn = sumVarNegative / signal_periods;
//       if (avDn>0)
//       {
//          rs = avUp / avDn;
//          double rsi = NormalizeDouble(100 - (100 / (1 + rs)), 2);
//          if(rsi < 30) ArrowUp[0] = closed[1];
//          if(rsi > 70) ArrowDn[0] = closed[1];
//       }      
// }

void DrawSignal()
{
   double rsi_cu = iRSI(NULL, 0, signal_periods, PRICE_CLOSE, 1);
   double rsi_pr = iRSI(NULL, 0, signal_periods, PRICE_CLOSE, 2);

   if (confirmation[1] > confirmation[2] && closed[1]> confirmation[1])
      if(rsi_cu > 30 && rsi_pr <= 30) 
         ArrowUp[0] = closed[1]-closed[1]*0.10;
   
   if (confirmation[1] < confirmation[2] && closed[1] < confirmation[1])
      if(rsi_cu < 70 && rsi_pr >= 70) ArrowDn[0] = closed[1]+closed[1]*0.10;
}
