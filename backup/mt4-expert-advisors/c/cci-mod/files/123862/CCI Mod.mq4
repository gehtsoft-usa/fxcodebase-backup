...
extern double Constant = 0.015; // Constant
extern int SmoothingPeriod = 20; // Smoothing period
extern int DevLength = 14; // Deviation length
...
int init()
{
   IndicatorName = GenerateIndicatorName("CCI Mod");
   ...
   priceSource = new PriceStream(_Symbol, (ENUM_TIMEFRAMES)_Period, PRICE_TYPICAL);
   smoothing = new VwmaOnStream(priceSource, SmoothingPeriod);
   meanDev = new MeanDevStream(priceSource, DevLength, ma_vwma);
...
int start()
{
   ...
   while (pos >= 0)
   {
      ...
      double maValue;
      double meanDevValue;
      if (priceSource.GetValue(pos, priceValue) && smoothing.GetValue(pos, maValue) && meanDev.GetValue(pos, meanDevValue))
      {
         out[pos] = (priceValue - maValue) / (Constant * meanDevValue);
      }
      pos--;
   } 
   return 0;
