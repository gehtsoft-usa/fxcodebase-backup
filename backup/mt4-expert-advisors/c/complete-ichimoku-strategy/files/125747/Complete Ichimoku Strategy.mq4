...
// http://fxcodebase.com/code/viewtopic.php?f=38&t=68336
...
extern string IndicatorsSection = ""; // == Indicators == 
extern int  Tenkan_Sen_Period                = 7; // Tenkan Sen Period
extern int  Kijun_Sen_Period                 = 21; // Kijun Sen Period
extern int  Senkou_Span_B_Period             = 48; // Senkou Span B Period
...
public:
   ...
   {
      double aCurrent = iIchimoku(_symbol, _timeframe, Tenkan_Sen_Period, Kijun_Sen_Period, Senkou_Span_B_Period, MODE_SENKOUSPANA, period);
      double bCurrent = iIchimoku(_symbol, _timeframe, Tenkan_Sen_Period, Kijun_Sen_Period, Senkou_Span_B_Period, MODE_SENKOUSPANB, period);
      double aFuture = iIchimoku(_symbol, _timeframe, Tenkan_Sen_Period, Kijun_Sen_Period, Senkou_Span_B_Period, MODE_SENKOUSPANA, period - Kijun_Sen_Period);
      double bFuture = iIchimoku(_symbol, _timeframe, Tenkan_Sen_Period, Kijun_Sen_Period, Senkou_Span_B_Period, MODE_SENKOUSPANB, period - Kijun_Sen_Period);
      double sl = iIchimoku(_symbol, _timeframe, Tenkan_Sen_Period, Kijun_Sen_Period, Senkou_Span_B_Period, MODE_TENKANSEN, period);
      double tl = iIchimoku(_symbol, _timeframe, Tenkan_Sen_Period, Kijun_Sen_Period, Senkou_Span_B_Period, MODE_KIJUNSEN, period);
      double cs = iIchimoku(_symbol, _timeframe, Tenkan_Sen_Period, Kijun_Sen_Period, Senkou_Span_B_Period, MODE_CHIKOUSPAN, period + Kijun_Sen_Period);
      double close = iClose(_symbol, _timeframe, period);
      if (close < MathMax(aCurrent, bCurrent) 
         || aFuture < bFuture
         || sl < MathMax(aCurrent, bCurrent)
         || tl < MathMax(aCurrent, bCurrent)
         || sl < tl
         || cs < iClose(_symbol, _timeframe, period + Kijun_Sen_Period))
      {
         return false;
      }
      double slPrevious = iIchimoku(_symbol, _timeframe, Tenkan_Sen_Period, Kijun_Sen_Period, Senkou_Span_B_Period, MODE_TENKANSEN, period + 1);
      double tlPrevious = iIchimoku(_symbol, _timeframe, Tenkan_Sen_Period, Kijun_Sen_Period, Senkou_Span_B_Period, MODE_KIJUNSEN, period + 1);
      return slPrevious <= tlPrevious && sl > tl;
   }
};
...
public:
   ...
   bool IsPass(const int period)
   {
      ...
      double cs = iIchimoku(_symbol, _timeframe, Tenkan_Sen_Period, Kijun_Sen_Period, Senkou_Span_B_Period, MODE_CHIKOUSPAN, period + Kijun_Sen_Period);
      double close = iClose(_symbol, _timeframe, period);
      if (close > MathMin(aCurrent, bCurrent) 
         || aFuture > bFuture
         || sl > MathMin(aCurrent, bCurrent)
         || tl > MathMin(aCurrent, bCurrent)
         || sl > tl
         || cs > iClose(_symbol, _timeframe, period + Kijun_Sen_Period))
      {
         return false;
