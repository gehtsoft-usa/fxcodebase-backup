...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=74395
...
input ModeEntry modeEntry = PendingBoth;                 // Mode Entry:
...
int uMaxTrades = qntBuys + qntSells;  // Max Trades At Same Time:
#endif
...
void OnTick()
{
    ...
    if(!uTradeReverse)
    {
        if(conditionsToBuy.EvaluateConditions())
        {
            ...
            if(modeEntry == PendingLimit || modeEntry == PendingStop)
            {
                double pr = Price("buy", uEntryDistance, _Symbol);
                actionSendOrder = new SendNewOrder("buy", Lots(), "", pr, SL("buy", pr), TP("buy", pr), magico);
            }

            if(modeEntry == PendingBoth)
            {
                for(int i = 0; i < qntBuys; i++)
                {
                    double pr = Price("buy", uEntryDistance * (i + 1), _Symbol);
                    actionSendOrder = new SendNewOrder("buy", Lots(), "", pr, SL("buy", pr), TP("buy", pr), magico);
        ...
        if(conditionsToSell.EvaluateConditions())
        {
            ...
            if(modeEntry == PendingBoth)
            {
                for(int i = 0; i < qntSells; i++)
                {
                    double pr = Price("sell", -uEntryDistance * (i + 1), _Symbol);
                    actionSendOrder = new SendNewOrder("sell", Lots(), "", pr, SL("sell", pr), TP("sell", pr), magico);
