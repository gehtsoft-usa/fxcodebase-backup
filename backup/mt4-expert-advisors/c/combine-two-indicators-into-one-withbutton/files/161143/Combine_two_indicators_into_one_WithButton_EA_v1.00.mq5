...
Name:        Combine_two_indicators_into_one_WithButton_EA_v1.00
Version:     1.00
Date:        2025
Repository:  Available @ https://fxcodebase.com/code/viewtopic.php?f=38&p=152827#p152827
License:     GNU
...
#define EANAME "CTIIO_EA"
int iCustom_handle;
string custom_indicator = "Combine_two_indicators_into_one_WithButton";
input string is = "==========  Entry Settings  =========="; // ==========  Entry Settings  ==========
input int    Deviation = 4;     // Deviation
input double MoneyRisk = 1.00;  // Offset Factor
#define CUSTOM_INDICATOR_PARAMS Deviation, MoneyRisk
...
input bool ShowInfo = 1;                     // Show info on chart
input ENUM_BASE_CORNER Corner = 1;           // Info corner
input int sizeInfo = 12;                     // Info font size
input color colorInfo = clrYellow;           // Info font color
...
input string mpl = "==========  Max Daily PL  =========="; // ==========  Max Daily PL  ==========
input closeAllBy maxDailyPLCalc = 0;         // Max daily PL calculation mode
input double maxDailyLoss = 0;               // Max loss per day (0=unlimited)
input double maxDailyWin = 0;                // Max win positions per day (0=unlimited)
input bool closeCurrent = true;              // Close current positions if reach
...
void OP(string sym, int entrysig = 0, double lot = 0.0, double price = 0.0, double slPrice = 0.0, double tpPrice = 0.0)
  {
   ...
   if(maxDailyPLCalc > 0)
     {
      double PLHist = PLOfPositionsHistory(maxDailyPLCalc, 0, sym, iTime(sym, PERIOD_D1, 0));
      double PL = PLOfPositions(maxDailyPLCalc, 0, sym);
      if(closeCurrent)
        {
         if(PLHist + PL >= maxDailyWin || PLHist + PL <= maxDailyLoss * (-1))
           {
            closeAll(0, sym);
            return;
           }
        ...
        {
         if(PLHist >= maxDailyWin || PLHist <= maxDailyLoss * (-1))
            return;
        }
     }
   int entrySignal;
...
int checkEntry(int bar, string sym)
  {
   double arrUp = custom_indi_calc(2, bar);
   double arrDn = custom_indi_calc(3, bar);
   ChartRedraw();
   if(isVal(arrUp))
      return 1;
   if(isVal(arrDn))
