...
Version:     
Date:        
Repository:  Available @ https://fxcodebase.com/code/viewtopic.php?f=38&t=76352
License:     GNU
*/
...
int OnInit()
{
    ...
    bottom_price = ChartGetDouble(0,CHART_FIXED_MIN); 
    ...
    SetIndexBuffer(4, TrendUp, INDICATOR_DATA);
    SetIndexArrow(4, 159);
    SetIndexStyle(4, DRAW_ARROW, EMPTY, 1, ArrowUpClr);
    SetIndexLabel(4, "Arrow Up");

    SetIndexBuffer(5, TrendDn, INDICATOR_DATA);
                ...
                const long &tick_volume[], const long &volume[], const int &spread[])
{
    ...
    for (i = start; i >= 0; i--) {
        if (ConditionsToLineUp(i)) { LineFast[i] = valueLineUp(i); }
        if (ConditionsToLineDn(i)) { LineSlow[i] = valueLineDn(i); }

        if(LineFast[i] > LineSlow[i]) { TrendUp[i] =  bottom_price + 100 * Point; }
        if(LineFast[i] < LineSlow[i]) { TrendDn[i] =  bottom_price + 100 * Point; }
        
        if (haveSignalUp(i))   { ArrowUp[i+1] = Low[i+1]; notify(0); }
        if (haveSignalDown(i)) { ArrowDn[i+1] = High[i+1]; notify(1); }
    }
    return (rates_total);
}
...
{
    return iMA(NULL, 0, 50, 0, MODE_SMA, PRICE_CLOSE, i);
}
...
{
    // TODO: signal up
    return (iClose(NULL, 0, i + 2) > iOpen(NULL, 0, i + 2) && iClose(NULL, 0, i + 1) > LineFast[i + 1] &&  LineFast[i + 1] >  LineSlow[i + 1] && iOpen(NULL, 0, i + 1) < LineFast[i + 1]);
}
