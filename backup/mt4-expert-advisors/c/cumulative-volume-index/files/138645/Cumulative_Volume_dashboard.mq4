...
// http://fxcodebase.com/code/viewtopic.php?f=38&p=138638
...
public:
   ...
   {
      double value = iCustom(_symbol, _timeframe, "Cumulative_Volume", Length, method, true, Relative, invert, 0, period);
...
public:
   ...
   virtual bool IsPass(const int period, const datetime date)
   {
      double value = iCustom(_symbol, _timeframe, "Cumulative_Volume", Length, method, true, Relative, invert, 1, period);
      return value != 0 && value != EMPTY_VALUE;
   }
};
...
public:
   ...
   virtual ICell* Create(const string id, const int x, const int y, ENUM_BASE_CORNER corner, const string symbol, const ENUM_TIMEFRAMES timeframe, bool showHistorical)
   {
      ...
      IValueFormatter* upValue = new FixedTextFormatter("Green", GetTextColor(_upColor), GetBackgroundColor(_upColor));
      IValueFormatter* historyUpValue = NULL;
      if (showHistorical)
      {
         historyUpValue = new FixedTextFormatter("Green", GetTextColor(_historicalUpColor), GetBackgroundColor(_historicalUpColor));
      }
      ...
      IValueFormatter* downValue = new FixedTextFormatter("Red", GetTextColor(_downColor), GetBackgroundColor(_downColor));
      IValueFormatter* historyDownValue = NULL;
      if (showHistorical)
      {
         historyDownValue = new FixedTextFormatter("Red", GetTextColor(_historicalDownColor), GetBackgroundColor(_historicalDownColor));
      }
...
string IndicatorName = "Cumulative Volume Dashboard";
int init()
{
