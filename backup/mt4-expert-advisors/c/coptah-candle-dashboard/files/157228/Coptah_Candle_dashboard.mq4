...
string file_custom_indicator = "Coptah_Candle";
...
enTimeFrames       inpTimeFrame           = tf_cu;             // Time frame to use
int                inpTimeFrameCustom     = 0;                 // Custom time frame to use (if custom time frame used)  
int                B1Period               = 20;                // First band period
ENUM_APPLIED_PRICE B1Price                = PRICE_CLOSE;       // First band price
int                B2Period               = 40;                // Second band period
ENUM_APPLIED_PRICE B2Price                = PRICE_CLOSE;       // Second band price
...
class FinderPattern
{
 ...
 public:
  ...
  bool havePatternUp(int i, ENUM_TIMEFRAMES tf)
  {
    ...
    double indiUp = iCustom(_symbol, tf, file_custom_indicator, 0, 0);
    ...
    if (indiUp > 0 && indiUp != EMPTY_VALUE ) { return true; }
  ...
  bool havePatternDn(int i, ENUM_TIMEFRAMES tf)
  {
    ...
    double indiDn = iCustom(_symbol, tf, file_custom_indicator,  1, 0);
    
    if (indiDn > 0 && indiDn != EMPTY_VALUE) { return true; }
