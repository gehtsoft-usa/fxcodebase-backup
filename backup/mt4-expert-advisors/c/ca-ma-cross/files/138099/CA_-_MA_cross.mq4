...
//http://fxcodebase.com/code/viewtopic.php?f=38&t=70513
...
// Based on https://www.tradingview.com/script/bcWGvngm-Moving-Average-Cross-Alert-Multi-Timeframe-MTF-by-ChartArt/

input ENUM_TIMEFRAMES tf = PERIOD_CURRENT; // Timeframe
input int shortperiod = 50; // Short Period Moving Average
input int longperiod = 100; // Long Period Moving Average
input ENUM_MA_METHOD smoothInput = MODE_SMA; // Smoothing method
...
public:
   ...
   {
      return ma1._data[period] > ma2._data[period] 
         && ma1._data[period + 1] <= ma2._data[period + 1] 
         && iClose(_symbol, _timeframe, period) > iClose(_symbol, _timeframe, period + 1);
   }
};
...
int init()
{
   ...
   IndicatorBuffers(14);
   IndicatorObjPrefix = GenerateIndicatorPrefix("CAMACross");
   IndicatorShortName("CA MA Cross");
   mainSignaler = new Signaler();
   mainSignaler.SetMessagePrefix(_Symbol + "/" + TimeframeToString((ENUM_TIMEFRAMES)_Period) + ": ");

   int id = 0;
   ma1 = new ColoredStream(_Symbol, (ENUM_TIMEFRAMES)_Period);
   id = ma1.RegisterStream(id, up_color, "MA1");
   id = ma1.RegisterStream(id, down_color, "MA1");
   ma2 = new ColoredStream(_Symbol, (ENUM_TIMEFRAMES)_Period);
   id = ma2.RegisterStream(id, up_color, "MA2");
   id = ma2.RegisterStream(id, down_color, "MA2");
   ...
   id = ma1.RegisterInternalStream(id);
   id = ma2.RegisterInternalStream(id);
...
int start()
{
   ...
   {
      ma1.Init(EMPTY_VALUE);
      ma2.Init(EMPTY_VALUE);
      if (customStream != NULL)
      {
         customStream.Init();
      }
   ...
   int minBars = MathMax(shortperiod, longperiod);
   ...
   for (int pos = limit; pos >= 0 && !IsStopped(); --pos)
   {
      ...
      double maValue0 = iMA(_Symbol, tf, shortperiod, 0, smoothInput, PRICE_CLOSE, index);
      double maValue1 = iMA(_Symbol, tf, shortperiod, 0, smoothInput, PRICE_CLOSE, index + 1);
      ma1.Set(maValue0, pos, maValue0 > maValue1 ? 0 : 1);
      maValue0 = iMA(_Symbol, tf, longperiod, 0, smoothInput, PRICE_CLOSE, index);
      maValue1 = iMA(_Symbol, tf, longperiod, 0, smoothInput, PRICE_CLOSE, index + 1);
      ma2.Set(maValue0, pos, maValue0 > maValue1 ? 0 : 1);
      if (customStream != NULL)
      {
