...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=72593
...
class ZigZag
{
  ...
  int    _tf;
  string _file;
  int    _lastSignal;  // vela de la última señal
 ...
 public:
  ...
  {
    return iCustom(_symbol, _tf, _file, _setup.candle, _setup.length, buffer, shift);
  }
  ...
  // Controla que la señal sea nueva
  bool ValidateSignal(int curSignal)
  {
    if (candle(curSignal) > _lastSignal)
    {
      _lastSignal = candle(curSignal);
      return true;
    }
    return false;
  }
...
input int    uCandle = 480;                   // Candle
input int    uLength = 12;                    // Length
...
ConditionCountTrades* countOpenTrades;
...
int OnInit()
{
  ...
  conditionsToTrade.AddCondition(countOpenTrades = new ConditionCountTrades(1));
  //--- buys:
  ...
  // Note: Indicators
  zz = new ZigZag(_Symbol, 0);
  zz.setSetup(uCandle, uLength);
