...
input int PatternLength = 10; // Pattern Length
input int PatternWidth = 4; // Pattern Width
...
public:
   ...
   virtual bool IsPass(const int period, const datetime date)
   {
      ...
      {
         double value = iCustom(_symbol, _timeframe, "ZigZag_with_patterns_and_values mtf+alerts", "", 1000, Length, PatternLength, PatternWidth, 1, period + i);
         if (value == 1)
         {
            return true;
         }
...
int init()
{
   ...
   double temp = iCustom(NULL, 0, "ZigZag_with_patterns_and_values mtf+alerts", 0, 0);
   if (GetLastError() == ERR_INDICATOR_CANNOT_LOAD)
   {
      Alert("Please, install the 'ZigZag_with_patterns_and_values mtf+alerts' indicator");
      return INIT_FAILED;
   }

   IndicatorName = GenerateIndicatorName("ZigZag with patterns and values");
