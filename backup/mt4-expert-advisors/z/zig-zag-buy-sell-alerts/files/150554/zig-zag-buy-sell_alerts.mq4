...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=73638
...
input int    candles_back          = 5; // candles back for alerts
...
int start()
  {
   ...
   Shift_Zerro();
	...
	for(int j=0; j < candles_back; j++)
	{
      if (BuyOrdersBuffer[j]  > 0 ) {
				if(lastAlertPrice != BuyOrdersBuffer[j])
					{
						Notifications(0);
          	lastAlertPrice = BuyOrdersBuffer[j];
					}
      }
      
			if (SellOrdersBuffer[j] > 0 ) {
				if(lastAlertPrice != SellOrdersBuffer[j])
				{
					Notifications(1);
			    lastAlertPrice = SellOrdersBuffer[j];
				}
			}
  }
...
// MULTITIMEFRAME:
// te d? la correspondencia de la vela en tf inferior con la vela de tf superior:
