...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=73072
...
input double userLots       = 0.1;                // Setup Lots by "Fix Lots":
...
input double GridUser_maxLot     = 40;                           // Max lot value:
double GridUser_multiplier = 2;                            // Multiplier:
input string GridUserLots        = "0.3,0.6,1.2,2.4,4.8,9.6,19.2,38.4"; // Grid Lots
 ...
 public:
	...
	double nextLot(void)
  {
    ...
    if (qnt() >= 1 && qnt() <= max )
    {
		...
		if(qnt() > max )
		{
	    return NormalizeDouble(_lots[max - 1], 2);
		}
