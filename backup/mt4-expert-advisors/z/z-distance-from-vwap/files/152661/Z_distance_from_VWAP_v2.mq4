...
input double OB_Level1 = 2;
input double OB_Level2 = 2.5;

input double OS_Level1 = -2;
input double OS_Level2 = -2.5;

input double PositiveLevelSignal = 1.5; // Positve Level Signal
input double NegativeLevelSignal = -1.5; // Negative Level Signal

input string Custom_Indicator = "Z distance from VWAP";
input ENUM_MA_METHOD avg = MODE_SMA; // Smoothing method
...
int init()
{
   ...
   SetIndexBuffer(8, ArrowUp, INDICATOR_DATA);
    SetIndexArrow(8, 233);
    SetIndexStyle(8, DRAW_ARROW, EMPTY, 1, Green);
    SetIndexLabel(8, "Up Signal");

   SetIndexBuffer(9, ArrowDn, INDICATOR_DATA);
    SetIndexStyle(9, DRAW_ARROW, EMPTY, 1, Crimson);
    SetIndexArrow(9, 234);
    SetIndexLabel(9, "Down Signal");

    
    SetLevelValue(0, OB_Level1);
	SetLevelValue(1, OS_Level1);
	SetLevelValue(2, OB_Level2);
	SetLevelValue(3, OS_Level2);

// NOTE: nuevos levels    
    SetLevelValue(4, PositiveLevelSignal);
    SetLevelValue(5, NegativeLevelSignal);
   ...
   IndicatorObjPrefix = GenerateIndicatorPrefix("zdfvwap");
   IndicatorShortName("Z dist from VWAP");
                ...
                const int&      spread[])
{
	...
	for (pos = Bars - gi_116 - 1; pos >= 0; pos--)
	{
		...
		sma = iMAOnArray(V_Data, 0, Length, 0, avg, pos);

		if (sma != 0)
		{
			mean[pos] = (iMAOnArray(PV_Data, 0, Length, 0, avg, pos) * Length) / (sma * Length);
		}
		else
		{
			mean[pos] = 0;
	...
	for (pos = Bars - gi_116 - 1; pos >= 0; pos--)
	{
		...
		sma = iMAOnArray(Data_Data, 0, Length, 0, avg, pos);
		if (sma > 0)
		{
			vwapsd = MathSqrt(sma);
...
//    for(int pos = 50; pos > 0 && !IsStopped(); --pos)
  for(int pos = limit; pos >= 0 && !IsStopped(); --pos)
   {
       
       if(SignalUp(pos))
        {
            ArrowUp[pos] = PositiveLevelSignal;
        }

        if(SignalDn(pos))
        {
            ArrowDn[pos] = NegativeLevelSignal;
        }
    }
...
bool SignalUp(int i)
{
    ...
    Print(__FUNCTION__, " Oscillator[i + 1] ", Oscillator[i+1]);
    
    return (Oscillator[i + 1] < PositiveLevelSignal && Oscillator[i] >= PositiveLevelSignal);
}

bool SignalDn(int i)
{
    return (Oscillator[i + 1] > NegativeLevelSignal && Oscillator[i] <= NegativeLevelSignal);
}
//+------------------------------------------------------------------------------------------------+
