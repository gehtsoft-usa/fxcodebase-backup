...
// http://fxcodebase.com/code/viewtopic.php?f=38&t=68341
...
extern string MACDSection = ""; // == Zero Lag MACD ==
extern int MACDFastPeriod=12;
extern int MACDSlowPeriod=24;
extern int MACDSignalPeriod=9;
extern int MACDPrice=0;    // Applied price
                       // 0 - Close
                       // 1 - Open
                       // 2 - High
                       // 3 - Low
                       ...
                       // 5 - Typical
                       // 6 - Weighted  
extern double macd_buy_level = 0; // MACD buy level
extern double macd_sell_level = 0; // MACD sell level
...
extern MATypes ma_type = ma_ema; // MA type
extern int ma_period = 10; // MA period
...
public:
   ...
   {
      _source = new PriceStream(symbol, timeframe, PRICE_CLOSE);
      _ma = AveragesStreamFactory::Create(_source, ma_period, ma_type);
   }
   ...
   {
      double ma;
      if (!_ma.GetValue(period, ma) || iClose(_symbol, _timeframe, period) <= ma)
         return false;
      double val0 = iCustom(_symbol, _timeframe, "ZeroLagMACD", MACDFastPeriod, MACDSlowPeriod, MACDSignalPeriod, MACDPrice, period, 0);
      double val1 = iCustom(_symbol, _timeframe, "ZeroLagMACD", MACDFastPeriod, MACDSlowPeriod, MACDSignalPeriod, MACDPrice, period + 1, 0);
      return val0 > macd_buy_level && val1 <= macd_buy_level;
   }
};
...
public:
   ...
   bool IsPass(const int period)
   {
      ...
      double val0 = iCustom(_symbol, _timeframe, "ZeroLagMACD", MACDFastPeriod, MACDSlowPeriod, MACDSignalPeriod, MACDPrice, period, 0);
      double val1 = iCustom(_symbol, _timeframe, "ZeroLagMACD", MACDFastPeriod, MACDSlowPeriod, MACDSignalPeriod, MACDPrice, period + 1, 0);
      return val0 < macd_sell_level && val1 <= macd_sell_level;
   }
};
...
int OnInit()
{
   ...
   double temp = iCustom(NULL, 0, "ZeroLagMACD", 0, 0);
   if (GetLastError() == ERR_INDICATOR_CANNOT_LOAD)
   {
      Alert("Please, install the 'ZeroLagMACD' indicator (you can find it on FXCodeBase.com)");
      return INIT_FAILED;
   }
