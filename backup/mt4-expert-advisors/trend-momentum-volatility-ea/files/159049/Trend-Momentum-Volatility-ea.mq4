...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=75625
...
input double userBkvPips = 10;                      // Breakeven R size pips:
input double uBkvRCount = 2;                        // Breakeven R count:
input double userBkvStep = 0;                       // Breakeven extra pips:
...
class ConditionBreackeven : public iConditions
{
  public:
    bool evaluate()
    {
        ...
        double ask     = SymbolInfoDouble(OrderSymbol(), SYMBOL_ASK);
        double bid     = SymbolInfoDouble(OrderSymbol(), SYMBOL_BID);
        double dist    = userBkvPips * uBkvRCount * mPoints * 10;
...
const double tsl_start    = 0;                         // TSL Start:
input const double tsl_step     = 10;                         // TSL R size:
input const double tsl_distance = 2;                          // TSL R:
...
class ActionTSL : public iActions
{
  public:
    bool execute()
    {
        ...
        double distance  = step * tsl_distance;
...
input int                ma2_period        = 200;                            // Periods:
...
ConditionAboveEma cdEmaCrossUp;
...
ConditionBelowEma cdEmaCrossDown;
...
double bb(int buffer, int candle = 1) { return iBands(NULL, 0, bb_period, bb_desv, bb_shift, bb_applied_price, buffer, candle); }
...
class ConditionBB_Buy : public iConditions
{
  public:
    ...
    {
        // abajo de la low
        return bb(bu_low, 1) > iLow(NULL, 0, 1);
...
class ConditionBB_Sell : public iConditions
{
  public:
    ...
    {
        // arriba del high
        return bb(bu_up, 1) < iLow(NULL, 0, 1);
...
input bool  MACD_FILTER         = true;              // Filter by MACD?
...
class ConditionMACDBuy : public iConditions
{
  public:
    bool evaluate()
    {
...
ConditionMACDBuy cdMACDBuy;

class ConditionMACDSell : public iConditions
{
  public:
    bool evaluate() {
        return macd(1) < macd_signal(1); 
...
{
    LongStrategy.set(&cdEmaCrossUp, &buy);
    if(RSI_FILTER) LongStrategy.addCondition(&cd_Rsi_Not_Over_Sold);
    if(BBFilterOn) LongStrategy.addCondition(&cdBB_Buy);
    if(MACD_FILTER) LongStrategy.addCondition(&cdMACDBuy);
    
    ShortStrategy.set(&cdEmaCrossDown, &sell);
    if(RSI_FILTER) ShortStrategy.addCondition(&cd_Rsi_Not_Over_Bougth);
    if(BBFilterOn) ShortStrategy.addCondition(&cdBB_Sell);
    if(MACD_FILTER) ShortStrategy.addCondition(&cdMACDSell);
