...
double StopLoss(const double price, const string side, const string symbol, const double lot)
{
    ...
    case sl_by_atr:
        return StopLossByATR(price, side, symbol);
...
input string tatr       = "== ATR SL Setup =="; // ————————————————————————
input int    atr_period = 1;                 // ATR Periods

int  handle_atr = 0;
void setHandleAtr() { handle_atr = iATR(NULL, 0, atr_period); }
...
double StopLossByATR(const double price, const string side, const string symbol)
{
    long   digits   = SymbolInfoInteger(symbol, SYMBOL_DIGITS);
    double atr      = ATR(1);
    double distance = atr * uStopLossValue;
    double result   = 0;
...
input const string indicator_file = "Firestorm_indicator.ex5";       // Indicator File:
//---
int  bufferToBuy              = 2;
int  bufferToSell             = 3;
