...
void DrawDownArrow(int cnt)
{
   if (cnt <= 1 && last_signal != Time[cnt])
   {
      last_signal = cnt;
      signaler.SendNotifications("Down arrow");
   }
   ResetLastError();
   string id = IndicatorObjPrefix + TimeToString(Time[cnt]);
   if (ObjectFind(0, id) == -1)
   {
      if (!ObjectCreate(0, id, OBJ_TEXT, 0, Time[cnt], High[cnt]))
      {
   ...
   ObjectSetInteger(0, id, OBJPROP_TIME, Time[cnt]);
   ObjectSetDouble(0, id, OBJPROP_PRICE1, High[cnt]);
   ObjectSetString(0, id, OBJPROP_TEXT, CharToStr(217));
}

void DrawUpArrow(int cnt)
{
   if (cnt <= 1 && last_signal != Time[cnt])
   {
      last_signal = cnt;
      signaler.SendNotifications("Up arrow");
   }
   ResetLastError();
   string id = IndicatorObjPrefix + TimeToString(Time[cnt]);
   if (ObjectFind(0, id) == -1)
   {
      if (!ObjectCreate(0, id, OBJ_TEXT, 0, Time[cnt], Low[cnt]))
      {
         Print(__FUNCTION__, ". Error: ", GetLastError());
         return ;
      }
   ...
   ObjectSetInteger(0, id, OBJPROP_TIME, Time[cnt]);
   ObjectSetDouble(0, id, OBJPROP_PRICE1, Low[cnt]);
   ObjectSetString(0, id, OBJPROP_TEXT, CharToStr(218));
}
