...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=71268
...
input int signal_period = 14; // Signal period
...
class RStream : public AStreamBase
{
   string _symbol;
   ENUM_TIMEFRAMES _timeframe;
public:
   RStream(const string symbol, const ENUM_TIMEFRAMES timeframe)
   {
      _symbol = symbol;
      _timeframe = timeframe;
   }
   ...
   bool GetValue(const int period, double &val)
   {
      ...
      double close1 = iClose(_symbol, _timeframe, period);
      double close2 = iClose(_symbol, _timeframe, period + 1);
      double sign = (close1 > close2) ? 1 : (close1 < close2) ? -1 : 0;
      double vol = iVolume(_symbol, _timeframe, period);
      val = sign * vol;
      return true;
   }
};

class CustomOnStream : public AOnStream
{
   double _length;
   double _buffer[];
public:
   CustomOnStream(IStream *source, const int length)
      :AOnStream(source)
   {
      _length = 2.0 / (1.0 + length);
   }
   ...
   bool GetValue(const int period, double &val)
   {
      ...
      {
         _buffer[index] = _buffer[index - 1] + _length * (price - _buffer[index - 1]);
      }
      val = _buffer[index];
      return true;
   }
...
IndicatorOutputStream* vzo;
MinOnStream* vzoMin;
MaxOnStream* vzoMax;
SmaOnStream* vzo_smooth;
double levup[], levdn[], signal[];
int init()
{
   IndicatorObjPrefix = GenerateIndicatorPrefix("VZOFL");
   IndicatorShortName("Volume zone oscillator - fl");

   IndicatorBuffers(4);
   int id = 0;
   SetIndexStyle(id, DRAW_LINE, STYLE_SOLID);
   SetIndexBuffer(id, levup);
   ++id;
   SetIndexStyle(id, DRAW_LINE, STYLE_SOLID);
   SetIndexBuffer(id, levdn);
   ++id;
   vzo = new IndicatorOutputStream(_Symbol, (ENUM_TIMEFRAMES)_Period);
   id = vzo.RegisterStream(id, DarkGray, "VZO");
   vzo_smooth = new SmaOnStream(vzo, signal_period);
   vzoMin = new MinOnStream(vzo, inpFlLookBack);
   vzoMax = new MaxOnStream(vzo, inpFlLookBack);

   SetIndexStyle(id, DRAW_LINE, STYLE_SOLID);
   SetIndexBuffer(id, signal);
   ++id;
   
   RStream* R = new RStream(_Symbol, (ENUM_TIMEFRAMES)_Period);
   _vp = new CustomOnStream(R, inpPeriod);
   R.Release();
   SimplePriceStream* vol = new SimplePriceStream(_Symbol, (ENUM_TIMEFRAMES)_Period, PriceVolume);
   _tv = new CustomOnStream(vol, inpPeriod);
   vol.Release();
                ...
                const int &spread[])
{
   ...
   {
      double vp, tv;
      if (!_vp.GetValue(pos, vp) || !_tv.GetValue(pos, tv))
      {
         continue;
      }
      vzo._data[pos] = tv != 0 ? 100.0 * vp / tv : 0;
      int _start = MathMin(pos + inpFlLookBack, rates_total - 1);
      double min, max;
      if (!vzoMin.GetValue(pos, min) || !vzoMax.GetValue(pos, max))
      {
         continue;
      }
      double range = max - min;
      levup[pos] = min + inpFlLevelUp * range / 100.0;
      levdn[pos] = min + inpFlLevelDown * range / 100.0;
      double signalVal;
      if (!vzo_smooth.GetValue(pos, signalVal))
      {
         continue;
      }
      signal[pos] = signalVal;
   }
