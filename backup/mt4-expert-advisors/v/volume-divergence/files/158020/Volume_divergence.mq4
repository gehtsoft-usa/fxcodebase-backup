//Available @  https://fxcodebase.com/code/viewtopic.php?f=38&t=75551
...
int init()
{
   ...
   IndicatorShortName("Volume divergence");
                ...
                const int &spread[])
{
   ...
   {
      double shadow = high[pos] - low[pos];
      ...
      double volAvg = sma1Value;
      if (pos + 2 > (rates_total - 1)) { continue; }
      int signal = SafeLess(body, shadow * 0.05) && SafeGreater(tick_volume[pos], volAvg) && SafeGreater(tick_volume[pos], tick_volume[pos + 2]);
      int plotshape1_condition = signal;
      if (plotshape1_condition == true) { plot1[pos] = high[pos]; }
      if (signal) { _signaler.SendNotifications("Reversal!", "Reversal!"); }
   }
