...
#define MAGICMA  36455536

input int Short_Smoothing_Length=9;
input int Long_Smoothing_Length=200;
input string Algorithm_Str="Algorithm: 1 - One, 2 - Two";
input int Algorithm=1;
...
int OnInit()
{
     double temp = iCustom(NULL, 0, "VQI", 0, 0);
   if (GetLastError() == ERR_INDICATOR_CANNOT_LOAD)
   {
       Alert("Please, install the 'VQI' indicator");
       return INIT_FAILED;
   }
   LastBar=0;
...
void OnTick()
{
 ...
 double VQI2_0, VQI2_1, VQI3_0, VQI3_1;
 VQI2_0=iCustom(NULL, 0, "VQI", Short_Smoothing_Length, Long_Smoothing_Length, Algorithm_Str, Algorithm, 1, 1);
 VQI2_1=iCustom(NULL, 0, "VQI", Short_Smoothing_Length, Long_Smoothing_Length, Algorithm_Str, Algorithm, 1, 2);
 VQI3_0=iCustom(NULL, 0, "VQI", Short_Smoothing_Length, Long_Smoothing_Length, Algorithm_Str, Algorithm, 2, 1);
 VQI3_1=iCustom(NULL, 0, "VQI", Short_Smoothing_Length, Long_Smoothing_Length, Algorithm_Str, Algorithm, 2, 2);
 
 int CO;
 int res;
 double SL, TP;
 
 if (VQI2_0>VQI3_0 && VQI2_1<=VQI3_1)
 {
  _Alert("Buy");
  CO=CalculateOrders();
  if (CO<=0)
