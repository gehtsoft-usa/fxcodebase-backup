                ...
                const int &spread[])
{
   ...
   {
      int osIndex = iBarShift(symbol, (ENUM_TIMEFRAMES)_Period, time[pos]);
      if (osIndex < 0)
      {
         continue;
      }
      int htfStartIndex = iBarShift(symbol, tf, time[pos]);
      if (htfStartIndex < 0)
      {
         continue;
      }
      int startIndex = iBarShift(symbol, (ENUM_TIMEFRAMES)_Period, iTime(symbol, tf, htfStartIndex));
      if (startIndex < 0)
      {
         continue;
      }
      double vwapValue = 0, volumesSum = 0;
      for (int i = osIndex; i <= startIndex; i++)
      {
         vwapValue += iClose(symbol, (ENUM_TIMEFRAMES)_Period, i) * iVolume(symbol, (ENUM_TIMEFRAMES)_Period, i);
         volumesSum += iVolume(symbol, (ENUM_TIMEFRAMES)_Period, i);
      }
      vwapValue = volumesSum == 0 ? 0 : vwapValue / volumesSum;
      double vwapSTD = 0;
      for (int i = osIndex; i <= startIndex; i++)
      {
         vwapSTD += MathPow(iClose(symbol, (ENUM_TIMEFRAMES)_Period, i) - vwapValue, 2);
      }
      vwapSTD = startIndex - osIndex + 1 == 0 ? 0 : MathSqrt(vwapSTD / (startIndex - osIndex + 1));

      double c = vwapSTD == 0 ? 0 : (iClose(symbol, (ENUM_TIMEFRAMES)_Period, osIndex) - vwapValue) / vwapSTD;
      double o = vwapSTD == 0 ? 0 : (iOpen(symbol, (ENUM_TIMEFRAMES)_Period, osIndex) - vwapValue) / vwapSTD;
      double h = vwapSTD == 0 ? 0 : (iHigh(symbol, (ENUM_TIMEFRAMES)_Period, osIndex) - vwapValue) / vwapSTD;
      double l = vwapSTD == 0 ? 0 : (iLow(symbol, (ENUM_TIMEFRAMES)_Period, osIndex) - vwapValue) / vwapSTD;
      if (iClose(symbol, (ENUM_TIMEFRAMES)_Period, osIndex) > iOpen(symbol, (ENUM_TIMEFRAMES)_Period, osIndex))
      {
