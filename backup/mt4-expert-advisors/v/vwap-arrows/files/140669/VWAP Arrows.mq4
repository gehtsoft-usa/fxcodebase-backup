...
//http://fxcodebase.com/code/viewtopic.php?f=38&t=70913
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      return Daily[period] > iClose(_symbol, _timeframe, period);
   }
};
...
int init()
{
   ...
   IndicatorBuffers(8);
   IndicatorObjPrefix = GenerateIndicatorPrefix("vwapa");
   IndicatorShortName("VWAP Arrows");
...
double CalcWeekly(int pos)
{
   ...
   while (index < Bars - 1 && TimeDayOfWeek(Time[index+1])<=TimeDayOfWeek(Time[index]))
   {
      Sum1+=WP[index];
      Sum2+=Volume[index];
...
int start()
{
   ...
   for (pos = limit; pos >= 0 && !IsStopped(); --pos)
   {
      if (customStream != NULL)
      {
         customStream.SetValue(pos, Daily[pos]);
      }
      ...
      for (int i = 0; i < ArraySize(conditions); ++i)
      {
         AlertSignal* item1 = conditions[i];
         item1.Update(pos);
      }
   } 
   return(0);
