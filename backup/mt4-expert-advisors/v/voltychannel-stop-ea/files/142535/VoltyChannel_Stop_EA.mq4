...
//https://fxcodebase.com/code/viewtopic.php?f=38&t=71276
...
input int myBoxWidth = 3;
input int MA_Price = 0;
input int MA_Length = 1;
input int MA_Mode = 0;
input int ATR_Length = 10;
input double Kv = 4.0;
input double MoneyRisk = 1.0;
input bool usePrice_HiLoBreak = true;
input bool useMA_HiLoEnvolope = false;
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      double value = iCustom(_symbol, _timeframe, "VoltyChannel_Stop_v2.1", false, false, myBoxWidth, MA_Price, MA_Length, MA_Mode, ATR_Length, Kv, MoneyRisk, usePrice_HiLoBreak, useMA_HiLoEnvolope, 0, period);
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      double value = iCustom(_symbol, _timeframe, "VoltyChannel_Stop_v2.1", false, false, myBoxWidth, MA_Price, MA_Length, MA_Mode, ATR_Length, Kv, MoneyRisk, usePrice_HiLoBreak, useMA_HiLoEnvolope, 1, period);
      return value != 0 && value != EMPTY_VALUE;
   }
};
...
int OnInit()
{
   double temp = iCustom(NULL, 0, "VoltyChannel_Stop_v2.1", 0, 0);
   if (GetLastError() == ERR_INDICATOR_CANNOT_LOAD)
   {
      Alert("Please, install the 'VoltyChannel_Stop_v2.1' indicator");
      return INIT_FAILED;
   }
