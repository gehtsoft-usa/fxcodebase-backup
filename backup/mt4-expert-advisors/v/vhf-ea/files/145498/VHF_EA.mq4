...
input string ICustom     = "== VHF Setup ==";  // == VHF Setup ==
input int      uMain     = 11;                 // Main Sliding window size, in bars
input int      uWork     = 7;                  // Working Sliding window size, in bars
input int      Inp_VHF_N = 9;                  // VHF: averaging period
input bool     uReverse  = false;              // Reverse
// ------------------------------------------------------------------
...
input int          uMaxQntTrades           = 1;                          // Max Number of trade at same time:
...
class VHF : public iIndicators
{
  ...
  public:
   VHF(string Symbol, int TimeFrame)
   {
      _symbol = Symbol;
      _tf     = TimeFrame;
      _file   = "VHF.ex4";
   ...
   void setSetup(int set0, uchar uMainPeriod, uchar uWorkPeriod)
   {
      _setup.setup0 = set0;
      _mainPeriod = uMainPeriod;
      _workPeriod = uWorkPeriod;
   }

// void mainPeriod(uchar inpmainPeriod) { _mainPeriod = inpmainPeriod; }
// void workPeriod(uchar inpworkPeriod) { _workPeriod = inpworkPeriod; }

   double calculate(int shift, int buffer = 0)
   {
      return iCustom(_symbol, _tf, _file, _setup.setup0, buffer, shift);
   }
   ...
   void setMaxMin()
   {
      ...
      for (uchar i = 0; i < _mainPeriod; i++)
      {
         int t = ArraySize(main);
         if (ArrayResize(main, t + 1))
         {
            main[t] = calculate(i+1);           
         }      
      }

      _maxMain = main[ArrayMaximum(main, WHOLE_ARRAY, 0)];
      _minMain = main[ArrayMinimum(main, WHOLE_ARRAY, 0)];
      _maxWork = main[ArrayMaximum(main, _workPeriod, 0)];
      _minWork = main[ArrayMinimum(main, _workPeriod, 0)];
   }

   double avgMain() { return (_maxMain + _minMain) / 2; }
   double avgWork() { return (_maxWork + _minWork) / 2; }

   bool trendMain()
   {
      setMaxMin();
      if(calculate(0) > avgMain())      {
         return true;
      }
      return false;
   }
  ...
  public:
   bool evaluate()
   {
      // TODO: condition Buy 1
		if(vhf.trendMain()==true && vhf.trendWork()==true)
		{
			return true;
		}
      return false;
  ...
  public:
   bool evaluate()
   {
      // TODO: condition Buy 2
		if(iClose(Symbol(),Period(),1) > iClose(Symbol(),Period(),2))
		{
			return true;
		}
      return false;
...
int OnInit()
{
   ...
   //--- CONDITIONS TO OPEN TRADES:
	...
	if(!uReverse)
	{
	   ...
	   conditionsToBuy.AddCondition(conditionOrderCount = new ConditionOrderCount(GetPointer(mainOrders), uMaxQntTrades));
	} else 
	{
		conditionsToBuy.AddCondition(sellCondition1 = new SELLcondition1());
	   conditionsToBuy.AddCondition(sellCondition2 = new SELLcondition2());
      conditionsToBuy.AddCondition(conditionOrderCount = new ConditionOrderCount(GetPointer(mainOrders),uMaxQntTrades));
	}
   ...
   //--- sell:
	if(!uReverse)
	{
      ...
      conditionsToSell.AddCondition(conditionOrderCount = new ConditionOrderCount(GetPointer(mainOrders),uMaxQntTrades));
	}else
	{
	   conditionsToSell.AddCondition(buyCondition1 = new BUYcondition1());
	   conditionsToSell.AddCondition(buyCondition2 = new BUYcondition2());
      conditionsToSell.AddCondition(conditionOrderCount = new ConditionOrderCount(GetPointer(mainOrders),uMaxQntTrades));
	}
   ...
   //--- INDICATORS SETUPS:
   vhf.setSetup(Inp_VHF_N, uWork, uMain);
