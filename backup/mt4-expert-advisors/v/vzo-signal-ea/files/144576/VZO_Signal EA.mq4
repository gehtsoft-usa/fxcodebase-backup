...
input int          userPeriod1           = 6;                        // Period:
input int          userMA_Period         = 6;                        // MA Period:
input int          userMA_Method         = 0;                        // MA Method:
...
input int          magico                = 1032;                     // Magic Number:
...
class CustomConditionBUY : public iConditions
{
  public:
   bool evaluate()
   {
      ...
      {
         double value = iCustom(Symbol(), Period(), "VZO_Signal.ex4", userPeriod1, userMA_Period, userMA_Method, 1, i+1);
         if (value != EMPTY_VALUE) buffer[i] = value;
         Print(__FUNCTION__, " ", "buffer BUY ", i, " ", buffer[i]);
      }

       if (buffer[0] > 0 && buffer[1] > 0 && buffer[2] == 0){ return true;}
...
class CustomConditionSELL : public iConditions
{
  public:
   bool evaluate()
   {
      ...
      for (int i = 0; i < ArraySize(buffer); i++)
      {
         double value = iCustom(Symbol(), Period(), "VZO_Signal.ex4", userPeriod1, userMA_Period, userMA_Method, 2, i+1);
         if (value != EMPTY_VALUE) buffer[i] = value;
