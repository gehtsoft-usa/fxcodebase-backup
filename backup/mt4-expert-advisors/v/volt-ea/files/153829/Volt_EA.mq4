...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=74496
...
string file_custom_indicator = "ATR - Stops1.01 (mtf + alerts)";
int bufferToBuy = 6;
int bufferToSell = 7;

input string Tindi = "== ATR - Stops Setup ==";  // ————————————
#define _disLin 1
#define _disBar 2
#define _disCha 4
enum enDisplayType
{
   dis_01=_disLin,                  // Display atr stops 
   dis_02=_disBar,                  // Display atr stops candles
   dis_03=_disLin+_disBar,          // Display atr stops + candles 
   dis_04=_disCha,                  // Display channel
   dis_05=_disCha+_disBar,          // Display channel + candles
   dis_06=_disCha+_disBar+_disLin   // Display channel + candles + atr stops dots
};
enum enPrices
{
   pr_close,      // Close
...
input enTimeFrames TimeFrame       = tf_cu;  // Time frame to use
input int          MaPeriod        = 2;               // Ma period 
input enMaTypes    MaMethod        = ma_slwma;        // Moving average method   
input int          AtrPeriod       = 10;              // Atr period
input double       _Kv             = 2.0;             // Distance from price
input enPrices     Price           = pr_close;        // Price

double Indi(int buffer, int candle)
{
    // TODO: copiar la lista de INPUTS para custom indicator:    
    return iCustom(NULL, 0, file_custom_indicator, TimeFrame, MaPeriod, MaMethod, AtrPeriod, _Kv, Price, buffer, candle);
}
#endif
...
string file_custom_indicator2 = "Volty Channel Stop averages (mtf + arrows)";
int bufferToBuy2 = 0;
int bufferToSell2 = 1;

input string Tindi2 = "== Indicator Setup ==";  // ————————————
input ENUM_TIMEFRAMES TimeFrame2 = 0;
input int             AtrLength2 = 11;
input int             AtrMaMethod2 = 22;
input double          Kv2 = 1.3;
input int             MaPeriod2=5;
input int             MaMethod2=22;
input int             Price2=0;
input double          Risk2=0.15;


double Volty(int buffer, int candle)
{
    return iCustom(NULL, 0, file_custom_indicator2, TimeFrame2, AtrLength2, AtrMaMethod2, Kv2, MaPeriod2, MaMethod2, Price2, Risk2, buffer, candle);
}
#endif
 ...
 public:
  bool evaluate()
  {
    // TODO: condition Buy 1
      return Indi(bufferToBuy, 1) != EMPTY_VALUE && Volty(bufferToBuy2, 1)==1;
  }
};
...
class SELLcondition1 : public iConditions
{
 public:
  bool evaluate()
  {
    ...
    return Indi(bufferToSell, 1) != EMPTY_VALUE && Volty(bufferToSell2, 1)==1;
  }
};
