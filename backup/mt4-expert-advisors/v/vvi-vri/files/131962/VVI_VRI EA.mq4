...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      double value = iCustom(_symbol, _timeframe, "VVI_VRI", 
         "", modo_tipo, "", VRI_ON, VVI_ON, "", ma_1, ma_type, ma_len, ma_src, reaction_ma_1, ma_2, ma_type_b, ma_len_b, ma_src_b, reaction_ma_2,
         "", config_tend_alc, config_tend_baj, "", porc_cuerpo, "", porc_bar_cont, "", NPAMBI,
         6, period);
...
int OnInit()
{
   double temp = iCustom(NULL, 0, "VVI_VRI", 0, 0);
   if (GetLastError() == ERR_INDICATOR_CANNOT_LOAD)
   {
      Alert("Please, install the 'VVI_VRI' indicator");
      return INIT_FAILED;
   }
   #ifdef SHOW_ACCOUNT_STAT
      stats = NULL;
