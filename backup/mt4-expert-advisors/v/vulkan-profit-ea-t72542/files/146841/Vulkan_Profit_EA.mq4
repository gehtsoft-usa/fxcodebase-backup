...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=72542
      ...
      for(int i=0;i < candlesBack;i++)
		...
		for (int i = 0; i < ArraySize(values); i++) 
		{
         if(values[i] > 0)
         {
            ...
            checkLastBuy.signalTime(signalTm);// setea la hora de la ultima señal
            return true;
         }          
      }
...
class CustomConditionBUY2 : public iConditions
{
  ...
  public:
   bool evaluate()
   {
     ...
     if (reverseMode){ type = OP_SELL;} else { type=OP_BUY; }

     if (OrderSelect(OrdersTotal() - 1, SELECT_BY_POS, MODE_TRADES) && OrderMagicNumber() == magico && OrderType() == type)
     {
      ...
      {
         if(OrderSelect(i,SELECT_BY_POS, MODE_HISTORY) && OrderSymbol() == _Symbol && OrderMagicNumber() == magico && OrderType() == type)
         {
            if (OrderOpenTime() >= _signalTime)
            {
               return false;
...
class CustomConditionSELL2 : public iConditions
{
  ...
  public:
   bool evaluate()
   {
		...
		ENUM_ORDER_TYPE type;
    if (reverseMode){ type = OP_BUY;} else { type = OP_SELL; }
...
int OnInit()
{
   newCandle = new CNewCandle();
	...
	if(reverseMode)
	 {
	 conditionsToSell.AddCondition(entryBuy = new CustomConditionBUY());
   conditionsToSell.AddCondition(checkLastBuy = new CustomConditionBUY2());
   
	 conditionsToBuy.AddCondition(entrySell = new CustomConditionSELL());
   conditionsToBuy.AddCondition(checkLastSell= new CustomConditionSELL2());
	 } else
	 {
...
void OnTick()
{
   ...
   if (conditionsToBuy.EvaluateConditions())
   {
      ...
      if (actionSendOrder.doAction()) { Notifications(0); }
	 ...
	 if (conditionsToSell.EvaluateConditions())
   {
      ...
      if (actionSendOrder.doAction()) { Notifications(1); }
      delete actionSendOrder;
   }
}
