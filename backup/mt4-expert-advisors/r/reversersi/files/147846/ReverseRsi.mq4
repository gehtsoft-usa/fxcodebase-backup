...
//https://fxcodebase.com/code/viewtopic.php?f=38&t=72826
...
input bool               ReverseSignal   = true; // Reverse Signal:
// ------------------------------------------------------------------
...
input int          magico                  = 2022604;                    // Magic Number:
...
class BUYcondition2 : public iConditions
{
  public:
   bool evaluate()
   {
      ...
      return rsi.CrossAbove_LevelUp(1);
      // return false;
   }
};
...
class SELLcondition2 : public iConditions
{
  public:
   bool evaluate()
   {
      ...
      return rsi.CrossBelow_LevelDn(1);
      // return false;
   }
};
...
class ConditionToCloseBuy : public iConditions
{
  public:
   bool evaluate()
   {
      ...
      {
				if(!ReverseSignal) return sellCondition1.evaluate();
				if(ReverseSignal) return sellCondition2.evaluate();
      }
...
class ConditionToCloseSell : public iConditions
{
  public:
   bool evaluate()
   {
      if (closeAllInOpositeSignal)
      {
				if(!ReverseSignal) return buyCondition1.evaluate();
				if(ReverseSignal) return buyCondition2.evaluate();
      }
      if (closeAllControlON)
      {
         return CloseALlControl();
...
int OnInit()
{
  ...
  rsi.setSetup(rsiPeriod, rsiAppliedPrice, rsiLevelUp, rsiLevelDn);
   ...
   //--- buys:
   if(!ReverseSignal)conditionsToBuy.AddCondition(buyCondition1 = new BUYcondition1());
   if(ReverseSignal) conditionsToBuy.AddCondition(buyCondition2 = new BUYcondition2());
   ...
   //--- sell:
   if(!ReverseSignal)conditionsToSell.AddCondition(sellCondition1 = new SELLcondition1());
   if(ReverseSignal) conditionsToSell.AddCondition(sellCondition2 = new SELLcondition2());
