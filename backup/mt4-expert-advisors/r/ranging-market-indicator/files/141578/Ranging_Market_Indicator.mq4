...
//https://fxcodebase.com/code/viewtopic.php?f=38&t=71105
...
input bool S1 = true; // Use ADX Filter
input bool S2 = true; // Use ATR Filter
input bool S3 = true; // Use RSI Filter
input int ADX_Period = 14; // ADX Period
input double ADX_Level = 25; // ADX Level
input int ATR_Period = 14; // ATR Period
input int ATR_MA_Period = 20; // MA Period
input int RSI_Period = 14; // RSI Period
input double RSI_OB_Level = 60; // OB Level
input double RSI_OS_Level = 40; // OS Level
input int bars_limit = 100000; // Bars limit
input color Trend = Green; // Trend color
input color Range = Red; // Range color
...
ColoredStream* Oscillator;
double data[];
int init()
{
   IndicatorObjPrefix = GenerateIndicatorPrefix("rmi");
   IndicatorShortName("Ranging Market Indicator");

   IndicatorBuffers(4);

   int id = 0;
   Oscillator = new ColoredStream(_Symbol, (ENUM_TIMEFRAMES)_Period);
   id = Oscillator.RegisterStream(id, Range, "Range", DRAW_HISTOGRAM);
   id = Oscillator.RegisterStream(id, Trend, "Trend", DRAW_HISTOGRAM);
   id = Oscillator.RegisterInternalStream(id);

   SetIndexStyle(id, DRAW_NONE);
   SetIndexBuffer(id, data);
   ++id;
                ...
                const int &spread[])
{
   ...
   for (int pos = MathMin(bars_limit, rates_total - 1 - MathMax(prev_calculated - 1, toSkip)); pos >= 0 && !IsStopped(); --pos)
   {
      ...
      if (S1)
      {
         double adxValue = iADX(_Symbol, _Period, ADX_Period, PRICE_CLOSE, MODE_MAIN, pos);
         if (adxValue > ADX_Level)
         {
            val = 1;
         }
      }
      if (S2)
      {
         data[pos] = iATR(_Symbol, _Period, ATR_Period, pos);
         double maValue = iMAOnArray(data, 0, ATR_MA_Period, 0, MODE_SMA, pos);
         if (data[pos] > maValue)
         {
            val = 1;
         }
      ...
      if (S3)
      {
         double rsiValue = iRSI(_Symbol, _Period, RSI_Period, PRICE_CLOSE, pos);
         if (rsiValue < RSI_OS_Level || rsiValue > RSI_OB_Level)
         {
            val = 1;
         }
      }
      Oscillator.Set(1, pos, val);
   }
