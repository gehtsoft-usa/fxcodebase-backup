...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=75584
...
bool OnInit_GUI()
{
    ...
    if (gui.reason() != REASON_CHARTCHANGE && gui.reason() != REASON_TEMPLATE && gui.reason() != REASON_PARAMETERS) {
        ...
        res           = gui.Create(0, "", 0, 0, 0, 185, altoPanel);
        if (res) gui.Run();
    }
    return res;
}
...
class GUI : public CAppDialog
{
    ...
    CLabel  lbLots, lbSl, lbTp, lbCandles, lbRB, lbRisk, lbAccount;
    CButton btBuy, btSell, btCloseAll, btClosePartialPair, btClosePair, btReverse, btHedge, btBreackeven, btAutoTrade,btBuyEma, btSellEma, btLevel;
    CEdit   eCandles, edit2, eLots, eSl, eTp, eRisk, eRB, eBuyEma, eSellEma, edLevel;
    ...
    bool Create(const long chart, const string name, const int subwin, const int x1, const int y1, const int x2, const int y2)
    {
        ...
        if(!Create_label("Magic",             Col(0), Row(0), _high, _width, lbLabel1)) return false;
        if(!Create_Edit("edMagic",            Col(1), Row(0), _high, _width, edMagic)) return false; edMagic.Text(uOrderMagic);
        if(!Create_button("Reverse by Magic", Col(0), Row(1), _high, _widthFull, btButton1)) return false;
        if(!Create_label("Account Level",     Col(0), Row(3), _high, _width, lbAccount)) return false;
        if(!Create_Edit("edLevel",            Col(1), Row(3), _high, _width, edLevel)) return false; edLevel.Text(uReverseLevel);
        if(!Create_button("Reverse by Level", Col(0), Row(4), _high, _widthFull, btLevel)) return false;
    ...
    {
        if(btButton1.IsActive()) btButton1.ColorBackground(PaleGreen); else btButton1.ColorBackground(CONTROLS_BUTTON_COLOR_BG);
        
        if(ReverseByLevelOn) { btLevel.ColorBackground(PaleGreen); }
        if(ReverseByLevelOn==false && btLevel.IsActive()==true) btLevel.ColorBackground(PaleGreen);             
        if(ReverseByLevelOn==false && btLevel.IsActive()==false) btLevel.ColorBackground(CONTROLS_BUTTON_COLOR_BG);
...
EVENT_MAP_BEGIN(GUI)
ON_EVENT(ON_CLICK, btButton1, acReverseByMagic.execute)
ON_EVENT(ON_CLICK, btLevel, activateReverseByLevel)
...
// MARK: Section REVERSE

input string Treverse         = "== Reverse Orders Setup =="; // ————————————————————————
input int    uOrderMagic      = 1982;                         // Magic Number to reverse
input double uReverseLevel    = 1000;                         // Reverse if account is less than:
bool ReverseByLevelOn = false;

void activateReverseByLevel() { 
    ReverseByLevelOn = !ReverseByLevelOn; 
    Print("line: ",__LINE__," ReverseByLevelOn: ",ReverseByLevelOn);
}
...
class ConditionReverseByAccountLevel : public iConditions
{
  public:
    bool evaluate()
    {
        ...
        double guiLevel = (double)gui.edLevel.Text();
...
ConditionReverseByAccountLevel cdReverseByAccountLevel;

class ActionReverseByAccountLevel : public iActions
{
  public:
    bool execute()
    {
        if (!ReverseByLevelOn) { return false; }
        Print("Executing reversion by account level...");
        ...
        activateReverseByLevel();
        return true;
    }
};
ActionReverseByAccountLevel acReverseByAccountLevel;

class ActionReverseByMagic : public iActions
{
  public:
    bool execute()
    {
        Print("Executing reversion by magic number... ");

        // recorrer todos los trades
        for (int i = OrdersTotal() - 1; i >= 0; i--) {
            if (OrderSelect(i, SELECT_BY_POS)) {
                int tk = OrderTicket();
                int mag = OrderMagicNumber();
                int guiMagic = (int)gui.edMagic.Text();

                if (OrderMagicNumber() == guiMagic) {
                    Reverse(OrderTicket());
                }
            }
        }
...
ActionReverseByMagic acReverseByMagic;

void Reverse(int tk)
{
    // tenés que tener el trade a reversar seleccionado
    Print("line: ", __LINE__, " tk: ", tk);

    string          _symbol = OrderSymbol();
    ENUM_ORDER_TYPE cmd     = OrderType() == OP_BUY ? (ENUM_ORDER_TYPE)OP_SELL : (ENUM_ORDER_TYPE)OP_BUY;
    double          lot     = OrderLots();
    double          sl      = OrderTakeProfit() != 0 ? OrderTakeProfit() : 0;
    double          tp      = OrderStopLoss() != 0 ? OrderStopLoss() : 0;
    double          bid     = SymbolInfoDouble(_symbol, SYMBOL_BID);
    double          ask     = SymbolInfoDouble(_symbol, SYMBOL_ASK);
    double          price   = cmd == OP_BUY ? ask : bid;

    int r = OrderSend(_symbol, cmd, lot, price, 0, sl, tp, NULL, 0, 0, clrMagenta);

    if (r > 0) {
        if (cmd == OP_BUY) {
            // si el nuevo es Buy, hay que cerrar un sell
            double ask = SymbolInfoDouble(_symbol, SYMBOL_ASK);
            if (OrderClose(tk, lot, bid, 0, Black)) {
                ;
            }
        }
        if (cmd == OP_SELL) {
...
Strategy stReverseByAccountLevel;
...
void BasicStrategy_Iniciate()
{
    ...
    stReverseByAccountLevel.set(&cdReverseByAccountLevel, &acReverseByAccountLevel);
    Trader.strategys.add(&stReverseByAccountLevel);
