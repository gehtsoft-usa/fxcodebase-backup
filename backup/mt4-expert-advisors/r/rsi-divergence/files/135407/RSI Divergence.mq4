...
public:
   ...
   {
      int prev;
      return IsBullishDivergence(period + 2, prev);
   }
};
...
public:
   ...
   {
      int prev;
      return IsBearishDivergence(period + 2, prev);
   }
};
...
int init()
{
   ...
   IndicatorObjPrefix = GenerateIndicatorPrefix("rsid");
   IndicatorShortName("RSI Divergence");
...
bool IsBullishDivergence(int curr, int& prev)
{
   if (isTrough(curr))
   {
      prev = prevTrough(curr);
      if (prev!=EMPTY_VALUE)
      {
         ...
         else if (RSI[curr]<RSI[prev] && Low[curr]>Low[prev])
         {
            return true;
         }
      }
...
{
   int prev;
   if (IsBullishDivergence(bar, prev))
   {
      DrawLine(prev, bar, true);
   }
}  
...
{
   int prev;
   if (IsBearishDivergence(bar, prev))
   {
      DrawLine(prev, bar, false);
   }
}  

bool IsBearishDivergence(int curr, int& prev)
{
   if (isPeak(curr))
   {
      prev=prevPeak(curr);
      if (prev!=EMPTY_VALUE)
      {
         ...
         else if (RSI[curr]>RSI[prev] && High[curr]<High[prev]) 
         {
            return true;
         }
      }
...
int start()
{
   ...
   for (int pos = limit; pos >= 0; --pos)
   {
      ...
      if (customStream != NULL)
      {
         customStream._stream[pos] = RSI[pos];
      }
      for (int i = 0; i < ArraySize(conditions); ++i)
      {
         AlertSignal* item = conditions[i];
