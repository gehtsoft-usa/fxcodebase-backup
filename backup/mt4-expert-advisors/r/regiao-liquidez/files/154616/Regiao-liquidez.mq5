...
// NOTE: Inputs
// ------------------------------------------------------------------
input int    NumberOfDays = 2;             // Days:
input double PointsZone = 150; // Zone Points:
                ...
                const int&      spread[])
{
    ...
    ArrayInitialize(lineUp2, EMPTY_VALUE);
    ArrayInitialize(lineDn2, EMPTY_VALUE);

    int start;
    datetime tm = iTime(NULL, PERIOD_D1, NumberOfDays);
    int bar     = iBarShift(Symbol(), PERIOD_D1, tm, true);
    double hi   = iHigh(Symbol(), PERIOD_D1, bar);
    double lo   = iLow(Symbol(), PERIOD_D1, bar);
    int bar_ini = iBars(NULL, 0) -iBarShift(NULL, 0, tm, true);

//   if (prev_calculated > 1) start = prev_calculated - 1; else { start = bar_ini; }

    for(int i = bar_ini; i < rates_total; i++)
    {
        lineUp[i]  = hi;
        lineDn[i]  = lo;
        lineUp2[i] = hi + PointsZone * _Point;
        lineDn2[i] = lo - PointsZone * _Point;
    }
