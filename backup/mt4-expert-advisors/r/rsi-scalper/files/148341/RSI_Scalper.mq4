...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=72948
...
input double       userEquityPer           = 0.10;               // Setup Lots by "Equity Percent":
...
input string             tmaFast             = "== Moving Average Fast Setup ==";  // == Moving Average Setup ==
input int                maFast_Period       = 20;                            // MA Fast - Period 
int                      maFast_Shift        = 0;                             // MA Fast - Ma Shift
input ENUM_MA_METHOD     maFast_Method       = MODE_EMA;                      // MA Fast - Method
input ENUM_APPLIED_PRICE maFast_AppliedPrice = PRICE_CLOSE;                   // MA Fast - Applied Price
input string             tmaSlow             = "== Moving Average Slow Setup ==";  // == Moving Average Setup ==
input int                maSlow_Period       = 50;                            // MA Slow _ Period
int                      maSlow_Shift        = 0;                             // MA Slow _ Ma Shift
input ENUM_MA_METHOD     maSlow_Method       = MODE_EMA;                      // MA Slow _ Method
input ENUM_APPLIED_PRICE maSlow_AppliedPrice = PRICE_CLOSE;                   // MA Slow _ Applied Price
...
//--- SESSIONS CONTROL:
  ...
  maFast.setSetup(maFast_Period, maFast_Shift, maFast_Method, maFast_AppliedPrice);
	maSlow = new MovingAverage();
  maSlow.setSetup(maSlow_Period, maSlow_Shift, maSlow_Method, maSlow_AppliedPrice);

  rsi = new RSI();
  rsi.setSetup(rsiPeriod, rsiAppliedPrice, rsiLevelUp, rsiLevelDn);

	martingale = new Martingale(_Symbol, magico, Martingale_Multiplier, Martingale_MaxLots);
