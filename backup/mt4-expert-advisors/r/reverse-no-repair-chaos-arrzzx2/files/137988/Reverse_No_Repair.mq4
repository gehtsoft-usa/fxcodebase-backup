...
// https://fxcodebase.com/code/viewtopic.php?f=38&p=143308 
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      if (iLow(_symbol, _timeframe, period) > iLow(_symbol, _timeframe, period + 1)
         && iHigh(_symbol, _timeframe, period) > iHigh(_symbol, _timeframe, period + 1)
         && iClose(_symbol, _timeframe, period) > iClose(_symbol, _timeframe, period + 1))
      {
         int startIndex = period + 1 + 1;
         int endIndex = period + 1 + FilterCandle - 1;
         for(int i = startIndex; i <= endIndex; i++)
         {
            if (iLow(_symbol, _timeframe, i) < iLow(_symbol, _timeframe, period + 1))
            {
               return false;
            }
         }
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
         ...
         && iClose(_symbol, _timeframe, period) < iClose(_symbol, _timeframe, period + 1))
      {
         ...
         for(int i = startIndex; i <= endIndex; i++)
         {
            if (iHigh(_symbol, _timeframe, i) > iHigh(_symbol, _timeframe, period + 1))
            {
               return false;
            }
         }
...
int init()
{
   ...
   IndicatorBuffers(8);
   IndicatorObjPrefix = GenerateIndicatorPrefix("rnr");
   IndicatorShortName("rnr");
