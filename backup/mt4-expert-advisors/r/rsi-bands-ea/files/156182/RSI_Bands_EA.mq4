...
input const string       indicator_file = "RSI with channels";         // Indicator File:
...
//---
double Indi(int buffer, int candle = 1) { return iCustom(NULL, 0, indicator_file,inpRsiPeriod, inpPrice, inpSmoothing, buffer, candle); }
  ...
  public:
    bool evaluate()
    {
        for (int i = 1; i <= candles_back_for_signals; i++) {
            ...
            double rsi       = Indi(buRSI, i);
            double rsi_prev  = Indi(buRSI, i + 1);
            if (rsi < indi && rsi_prev >= indi_prev && indi != EMPTY_VALUE && indi != lastSignal) {
                lastSignal = indi;
                return true;
            }
        }
