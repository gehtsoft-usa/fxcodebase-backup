...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=74427
...
{
    int n = pos + 1;
    while(DnMap[n] == EMPTY_VALUE && n+1 < ArraySize(DnMap)){ n++; }
    return (DnMap[n] > iLow(NULL, 0, pos));
}
bool isHH(int pos)
{
    int n = pos + 1;
    while(UpMap[n] == EMPTY_VALUE && n+1 < ArraySize(UpMap)){ n++; }
