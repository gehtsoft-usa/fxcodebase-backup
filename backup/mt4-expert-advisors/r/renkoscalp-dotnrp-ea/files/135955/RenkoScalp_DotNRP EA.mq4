...
// http://fxcodebase.com/code/viewtopic.php?f=38&t=70176
//+------------------------------------------------------------------+
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      for (int i = 0; i < 100; ++i)
      {
         double value = iCustom(_symbol, _timeframe, "RenkoScalp_DotNRP", barn, Length, 0, period + i);
         if (value != 0)
         {
...
class DiverStochLongCondition : public ACondition
{
public:
   DiverStochLongCondition(const string symbol, ENUM_TIMEFRAMES timeframe)
      :ACondition(symbol, timeframe)
   {
   ...
   bool IsPass(const int period, const datetime date)
   {
      ...
      {
         double value1 = iCustom(_symbol, _timeframe, "DiverStoch", KPeriod, DPeriod, Slowing, 1, period + i);
         double value2 = iCustom(_symbol, _timeframe, "DiverStoch", KPeriod, DPeriod, Slowing, 2, period + i);
...
class DiverStochShortCondition : public ACondition
{
public:
   DiverStochShortCondition(const string symbol, ENUM_TIMEFRAMES timeframe)
      :ACondition(symbol, timeframe)
   {
...
ICondition* CreateLongFilterCondition(string symbol, ENUM_TIMEFRAMES timeframe)
{
   ...
   AndCondition* condition = new AndCondition();
   condition.Add(new DiverStochLongCondition(symbol, timeframe), false);
   return condition;
}
...
ICondition* CreateShortFilterCondition(string symbol, ENUM_TIMEFRAMES timeframe)
{
   ...
   AndCondition* condition = new AndCondition();
   condition.Add(new DiverStochShortCondition(symbol, timeframe), false);
   return condition;
}
...
int OnInit()
{
      ...
      stats = NULL;
   #endif
   double temp = iCustom(NULL, 0, "DiverStoch", KPeriod, DPeriod, Slowing, 0, 0);
   if (GetLastError() == ERR_INDICATOR_CANNOT_LOAD)
   {
      Alert("Please, install the 'DiverStoch' indicator");
      return INIT_FAILED;
   }
   temp = iCustom(NULL, 0, "RenkoScalp_DotNRP", 0, 0);
   if (GetLastError() == ERR_INDICATOR_CANNOT_LOAD)
   {
      Alert("Please, install the 'RenkoScalp_DotNRP' indicator");
      return INIT_FAILED;
   }
