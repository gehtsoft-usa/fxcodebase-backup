...
Name:        rsx_nrp_counter_signal
Version:     1.00
Date:        2025
Repository:  Available @ https://fxcodebase.com/code/viewtopic.php?f=38&t=76433
License:     GNU
...
extern bool               counterSignalsOn      = true;             // Enable counter signals?
extern int                counterBarsMax        = 5;                // Max bars for counter signal
extern double             counterUpperGap       = 1.0;              // Counter upper arrow gap
extern double             counterLowerGap       = 1.0;              // Counter lower arrow gap
extern color              counterUpColor        = clrBlue;          // Counter up arrow color
extern color              counterDnColor        = clrRed;           // Counter down arrow color
extern int                counterUpCode         = 233;              // Counter up arrow code
extern int                counterDnCode         = 234;              // Counter down arrow code
...
int init()
  {
   ...
   SetIndexStyle(0, EMPTY, EMPTY, LineWidth);
   SetIndexBuffer(1, shadowa);
   SetIndexStyle(1, EMPTY, EMPTY, shadowWidth, ShadowColor);
   SetIndexBuffer(2, shadowb);
   SetIndexStyle(2, EMPTY, EMPTY, shadowWidth, ShadowColor);
   SetIndexBuffer(3, buffer1ua);
   SetIndexStyle(3, EMPTY, EMPTY, LineWidth, ColorUp);
   SetIndexBuffer(4, buffer1ub);
   SetIndexStyle(4, EMPTY, EMPTY, LineWidth, ColorUp);
   SetIndexBuffer(5, buffer1da);
   SetIndexStyle(5, EMPTY, EMPTY, LineWidth, ColorDown);
...
int start()
  {
            ...
            if(preventRepeatingArrows)
              {
               ...
               for(int k = i; k < MathMin(Bars - 1, i + barsToIgnore); k++)
                 {
                  string name = arrowsIdentifier + ":" + (string)Time[k];
   ...
   if(counterSignalsOn && arrowsVisible)
     {
      for(i = limit; i >= 0; i--)
        {
         string counterName = arrowsIdentifier + ":counter:" + (string)Time[i];
         ObjectDelete(counterName);
         int buySignalBar = -1;
         double buySignalLow = 0;
         for(int k = i + 1; k <= MathMin(Bars - 1, i + counterBarsMax); k++)
           {
            string signalName = arrowsIdentifier + ":" + (string)Time[k];
            if(ObjectFind(0, signalName) >= 0 && ObjectGetDouble(0, signalName, OBJPROP_PRICE) <= Low[k])
              {
               buySignalBar = k;
               buySignalLow = Low[k];
               break;
         ...
         int sellSignalBar = -1;
         double sellSignalHigh = 0;
         for(int k = i + 1; k <= MathMin(Bars - 1, i + counterBarsMax); k++)
           {
            string signalName = arrowsIdentifier + ":" + (string)Time[k];
            if(ObjectFind(0, signalName) >= 0 && ObjectGetDouble(0, signalName, OBJPROP_PRICE) >= High[k])
              {
               sellSignalBar = k;
               sellSignalHigh = High[k];
               break;
              }
           }
         if(buySignalBar > 0 && Close[i] < buySignalLow)
           {
            bool counterExists = false;
            for(int c = i + 1; c < buySignalBar; c++)
              {
               string checkName = arrowsIdentifier + ":counter:" + (string)Time[c];
            ...
            if(!counterExists)
               drawCounterArrow(i, counterDnColor, counterDnCode, true);
           }
         if(sellSignalBar > 0 && Close[i] > sellSignalHigh)
           {
            bool counterExists = false;
            for(int c = i + 1; c < sellSignalBar; c++)
              {
               string checkName = arrowsIdentifier + ":counter:" + (string)Time[c];
               if(ObjectFind(0, checkName) >= 0)
                 {
            ...
            if(!counterExists)
               drawCounterArrow(i, counterUpColor, counterUpCode, false);
           }
        }
     }
   if(alertsOn)
     {
      ...
      if(alertsOnCurrent)
         whichBar = 0;
      string name = arrowsIdentifier + ":" + (string)Time[whichBar];
      string counterName = arrowsIdentifier + ":counter:" + (string)Time[whichBar];
      ...
      if(ObjectFind(0, counterName) >= 0)
        {
         if(ObjectGetDouble(0, counterName, OBJPROP_PRICE) >= High[whichBar])
            doAlert(whichBar, "sell counter");
         if(ObjectGetDouble(0, counterName, OBJPROP_PRICE) <= Low[whichBar])
            doAlert(whichBar, "buy counter");
        }
     }
   return(0);
  }
...
//|                                                                  |
//+------------------------------------------------------------------+
void drawCounterArrow(int i, color theColor, int theCode, bool up)
  {
   string name = arrowsIdentifier + ":counter:" + (string)Time[i];
   double gap  = iATR(NULL, 0, 20, i);
   ObjectCreate(name, OBJ_ARROW, 0, Time[i], 0);
   ObjectSet(name, OBJPROP_ARROWCODE, theCode);
   ObjectSet(name, OBJPROP_COLOR, theColor);
   if(up)
      ObjectSet(name, OBJPROP_PRICE1, High[i] + counterUpperGap * gap);
   else
      ObjectSet(name, OBJPROP_PRICE1, Low[i]  - counterLowerGap * gap);
  }
