...
// https://fxcodebase.com/code/viewtopic.php?f=38&p=158346
...
input entry_indicator entry_indicator_type     = supertrend;                  // Entry Indicator:
input const string    indicator_file           = "Dark Inversion";            // Indicator File:
int                   bufferToBuy              = 0;
int                   bufferToSell             = 1;
bool                  indicator_init           = true;
int                   candles_back_for_signals = 1;
input bool            filter_by_supertrend     = true; // Filter by SuperTrend:
ENUM_TIMEFRAMES       st_tf                    = 0;
string                st_note                  = 0;
int                   st_ma_period             = 10;
int                   st_ma_type               = 14;
int                   st_ma_calc_price         = 0;
int                   st_calc_usign            = 0;
input int             st_calculate_period      = 10; // Calculate Period:
input int             st_calculate_deviation   = 3;  // Calculate deviation:
...
ConditionIndicatorBuy darkinversion_entry_BUY;
...
ConditionIndicatorSell darkinversion_entry_SELL;
...
double SuperTrend(int buffer, int candle = 1) { return iCustom(NULL, 0, "super trend (profit max)1.1", 
st_tf,
st_note,
st_ma_period,
st_ma_type,
...
st_calc_usign,
st_calculate_period,
st_calculate_deviation,
buffer, candle); }


class ConditionSuperTrendBuy : public iConditions
{
    public:
    bool evaluate()
    {
        return (SuperTrend(8,1) == EMPTY_VALUE && SuperTrend(7,1) != EMPTY_VALUE && SuperTrend(7,1) > 0);
    }
};
ConditionSuperTrendBuy cdSuperTrendBuy;

class ConditionSuperTrendSell : public iConditions
{
    public:
    bool evaluate()
    {
...
ConditionSuperTrendSell cdSuperTrendSell;


class ConditionEntrySuperTrendBuy : public iConditions
{
    public:
    bool evaluate()
    {
        if(SuperTrend(15,2) < 0 && SuperTrend(15,1) > 0)
        {
            return true;
        }
        return false;
...
ConditionEntrySuperTrendBuy cdEntrySuperTrendBuy;

class ConditionEntrySuperTrendSell : public iConditions
{
    public:
    bool evaluate()
    {
...
ConditionEntrySuperTrendSell cdEntrySuperTrendSell;
...
bool OnInit_CustomIndicator()
{
    ...
    if(entry_indicator_type == supertrend) {
        ShortStrategy.addCondition(&cdEntrySuperTrendSell);
        LongStrategy.addCondition(&cdEntrySuperTrendBuy);            
    }
    
    if(entry_indicator_type == dark_inversion) {
        ShortStrategy.addCondition(&darkinversion_entry_SELL);
        LongStrategy.addCondition(&darkinversion_entry_BUY);
    }


    if(ma_filter_on) {
        ShortStrategy.addCondition(&cdEmasTrendDn);
        LongStrategy.addCondition(&cdEmasTrendUp);    
    }    
    
    if(filter_by_supertrend) {
        ShortStrategy.addCondition(&cdSuperTrendSell);
        LongStrategy.addCondition(&cdSuperTrendBuy);    
    }    
...
enum enum_opposite_types 
{
    op_by_dark_inversion, // Opposite by Dark Inversion
    op_by_super_trend     // Opposite by Super Trend
};

input string Tcloseopposite       = "==== Close On Opposite ===="; // ————————————————————————

input bool                close_on_opposite_on = false;          // Close On Opposite Signal:
input enum_opposite_types opposite_type        = op_by_dark_inversion; // Opposite Type:
...
// Mark: Section Close Opposite Super Trend
ConditionSuperTrendSell cd_CloseOpposite_SuperTrend_Buy;
ConditionSuperTrendBuy cd_CloseOpposite_SuperTrend_Sell;

Strategy stClose_Opposite_ST_Buy;
Strategy stClose_Opposite_ST_Sell;

void oninit_close_on_opposite()
{
    if(opposite_type == op_by_dark_inversion) {
    ...
    if(opposite_type == op_by_super_trend) {
        stClose_Opposite_ST_Buy.set(&cd_CloseOpposite_SuperTrend_Buy, &acCloseBuy);
        stClose_Opposite_ST_Sell.set(&cd_CloseOpposite_SuperTrend_Sell, &acCloseSell); 

        if (close_on_opposite_on) {
            Trader.management.add(&stClose_Opposite_ST_Buy);
            Trader.management.add(&stClose_Opposite_ST_Sell);
        }        
    }
}
...
input string             ma_titl           = "== Filter by moving average =="; // ————————————————————————
input bool               ma_filter_on      = true;                             // Ma Filter On ?
...
double ma2(int shift) { return iMA(NULL, 0, ma2_period, ma2_shift, ma2_method, ma2_applied_price, shift); }
