  ...
  public:
    bool execute()
    {
        ...
        if (close_percent < 100) {
            lots *= close_percent / 100;
            lots = NormalizeDouble(lots, 2);
            if (lots < minLot) { lots = minLot; }
        }
        if (!OrderClose(OrderTicket(), lots, bid, 0, Black)) {
...
class ActionCloseSell : public iActions
{
  public:
    bool execute()
    {
        ...
        if (!OrderClose(OrderTicket(), lots, ask, 0, Black)) {
            Print("Error when try to close order ", GetLastError());
        }
        return true;
    }
...
input string TclosePositions      = "== Close Positions by indicator =="; // ————————————————————————
input bool   close_by_strategy_on = false;                                // Close positions by Indicator:
input double close_percent        = 25;                                   // Percent to Close:

int bToCloseBuy  = 7;
int btoCloseSell = 8;
//---

class ConditionIndicatorCloseBuy : public iConditions
{
    double lastSignal;
    datetime candle;
  public:
    bool evaluate()
    {
        for (int i = 0; i < candles_back_for_signals; i++) {
            double indi = Indi(bToCloseBuy, i);
            if (indi > 0 && indi != EMPTY_VALUE && indi != lastSignal && iTime(NULL, 0,0) != candle ) {
                lastSignal = indi;
                candle = iTime(NULL, 0, 0);
                return true;
            }
        }
        return false;
...
ConditionIndicatorCloseBuy cdCloseBuy;

class ConditionIndicatorCloseSell : public iConditions
{
    double lastSignal;
    datetime candle;
  ...
  public:
    ...
    {
        for (int i = 0; i < candles_back_for_signals; i++) {
            double indi = Indi(btoCloseSell, i);
            if (indi > 0 && indi != EMPTY_VALUE && indi != lastSignal && iTime(NULL, 0,0) != candle ) {
                lastSignal = indi;
                candle = iTime(NULL, 0, 0);
                return true;
...
ConditionIndicatorCloseSell cdCloseSell;
...
class OrderCreator : public iActions
{
  ...
  public:
    ...
    bool setType()
    {
        ...
        if (modeEntry == Market || modeEntry == Reverse ) {
...
input const string indicator_file = "Trend_Strength";            // Indicator File:
...
//---
double IndiFilter(int buffer, int candle = 1) { return iCustom(NULL, 0, indicator_file_filter, buffer, candle); }
...
bool OnInit_CustomIndicator()
{
    ...
    if (modeEntry == Reverse) {
        ShortStrategy.addCondition(&conditionIndicatorBuy);
        LongStrategy.addCondition(&conditionIndicatorSell);
        
        if(filter_on) {
            ShortStrategy.addCondition(&conditionIndicatorBuyFilter);
            LongStrategy.addCondition(&conditionIndicatorSellFilter);
        }
    }
