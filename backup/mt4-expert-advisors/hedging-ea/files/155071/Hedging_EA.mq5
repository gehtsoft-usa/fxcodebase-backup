...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=74765
...
// NOTE: hedge inputs
input string THEDGE        = "== Hedging Setup ==";  // ————————————
input double m             = 2;                      // Lot Multiplier:
input double hedgeDistance = 200;                    // Hedging Distance
input int    magicoOrigin  = 1234;                   // Magic Number Origin:
input int    magico        = 4321;                   // Magic Number Hedging:

double       hedge_buyPrice = 0;
double       hedge_sellPrice         = 0;
...
class CountPositions
{
 ...
 public:
  CountPositions(ENUM_POSITION_TYPE t, ulong m) : _type(t), _magic(m) { ; }
  ~CountPositions() { ; }
  ...
  int count()
  {
    ...
    for (int i = PositionsTotal() - 1; i >= 0; i--) {
      ...
      if (PositionGetInteger(POSITION_TYPE) == _type && ControlMagic(PositionGetInteger(POSITION_MAGIC)) == true) {
        co += 1;
      }
    }
    return co;
  ...
  bool ControlMagic(ulong mag)
  {
    return mag == _magic || mag == magicoOrigin || mag == 0;
  }
};
CountPositions Buys(POSITION_TYPE_BUY, magico);
CountPositions Sells(POSITION_TYPE_SELL, magico);
...
class CountPendings
{
 ...
 public:
  CountPendings(ENUM_ORDER_TYPE t, ulong m) : _type(t), _magic(m) { ; }
  ~CountPendings() { ; }
  ...
  int count()
  {
    ...
    for (int i = OrdersTotal() - 1; i >= 0; i--) {
      ...
      if (OrderGetInteger(ORDER_TYPE) == _type && OrderGetInteger(ORDER_MAGIC) == _magic) {
        co += 1;
      }
    }
    return co;
...
CountPendings BuysPending(ORDER_TYPE_BUY_LIMIT, magico);
CountPendings SellsPending(ORDER_TYPE_SELL_LIMIT, magico);
...
class NewBuy
{
 ...
 public:
  ...
  bool evaluate()
  {
    int currentBuys = Buys.count();
    if (currentBuys > _BuyCount) {
      _BuyCount = currentBuys;
      return true;
    }
    if (currentBuys == 0) {
      _BuyCount = 0;
...
class NewSell
{
 ...
 public:
  ...
  bool evaluate()
  {
    int currentSells = Sells.count();
    if (currentSells > _SellCount) {
      _SellCount = currentSells;
      return true;
    }
    if (currentSells == 0) {
      _SellCount = 0;
...
class OrdersList
{
    ...
    void clearList()
    {
        ...
        Print(__FUNCTION__,"  cleaaning the list ");
  ...
  // ——————————————————————————————————————————————————————————————————
  bool cleanCloseOrders()
  {
    if (qnt() == 0) return false;

    bool someClose = false;
    for (int i = 0; i < qnt(); i++)
    {
      if (isClose(i))
      {
...
bool isClose(int index)
{
  ...
  long  deal_entry;    // tipo de movimiento
  ...
  for (int i = 0; i < deals; i++) {
    ...
    if (position_ID == tk && deal_entry == DEAL_ENTRY_OUT) {
      return true;
    }
  }
    return false;
 ...
 public:
  ...
  bool doAction()
  {
    if (CheckPointer(newOrder) == POINTER_INVALID) {
      Print(__FUNCTION__, " ", "Order Invalid");
      return false;
    }
    ...
    if (newOrder.type() == ORDER_TYPE_SELL || newOrder.type() == ORDER_TYPE_BUY) {
...
class BUYcondition1 : public iConditions
{
 public:
  bool evaluate()
  {
    ...
    // si se abrió un sell y no hay un buy pendiente

    if (newSell.evaluate() && BuysPending.count() == 0) {
      hedge_sellPrice = MainOrders.last().price();
      hedge_buyPrice  = NormalizeDouble(hedge_sellPrice + (hedgeDistance * Point()), _Digits);

      if (tkOrigin == 0) {
        tkOrigin = MainOrders.last().id();
        Print(__FUNCTION__, " tkOrigin: ", tkOrigin);
      }

      SendPending("buy", hedge_buyPrice);

      //   Print("ENVIAR PENDING BUY");
    }
    return false;
  }
};
...
class SELLcondition1 : public iConditions
{
 public:
  bool evaluate()
  {
    ...
    // si se abrió un buy y no hay un sell pendiente
    if (newBuy.evaluate() && SellsPending.count() == 0) {
      hedge_buyPrice  = MainOrders.last().price();
      hedge_sellPrice = NormalizeDouble(hedge_buyPrice - (hedgeDistance * Point()), _Digits);
      ...
      SendPending("sell", hedge_sellPrice);
      //   Print("ENVIAR PENDING SELL");
    }
    return false;
    // return false;
  }
...
void OnTick()
{
    ...
    {
        if(MainOrders.qnt()==0){
            actionSendOrder = new SendNewOrder("buy", Lots(), "", 0, SL("buy"), TP("buy"), magicoOrigin);
...
double Lots()
{
    ...
    if(CheckPointer(MainOrders.last()) != POINTER_INVALID){
        double lastlot = MainOrders.last().lot();
        if(m > 1) lots = NormalizeDouble(lastlot * m, 2);
    }
...
void closeAll(string side = "")
{
  if (side == "buy" || side == "") {
    ...
    actionCloseBuys = new ActionCloseOrdersByType("buy", magicoOrigin);
    actionCloseBuys.doAction();
    delete actionCloseBuys;
  }
  if (side == "sell" || side == "") {
    ...
    actionCloseSells = new ActionCloseOrdersByType("sell", magicoOrigin);
    actionCloseSells.doAction();
    delete actionCloseSells;
  }
  DeletePendings();
...
double floatingEA()
{
  ...
  for (int i = PositionsTotal() - 1; i >= 0; i--) {
    ulong tk = PositionGetTicket(i);
    if (PositionGetSymbol(i) == Symbol() && ControlMagic(PositionGetInteger(POSITION_MAGIC)) == true) {
...
bool ControlMagic(ulong mag)
{
    return mag == magico || mag == magicoOrigin || mag == 0;
}
...
void SendPending(string side, double price)
{
  actionSendOrder = new SendNewOrder(side, Lots(), "", price, SL(side), TP(side), magico);
  actionSendOrder.doAction();
  delete actionSendOrder;
}

ulong tkOrigin = 0;
void  ControlCloseOrigin()
{
  if (tkOrigin == 0) return;
    
    if (isClose(tkOrigin) || MainOrders.cleanCloseOrders()) 
    {
      closeAll();
      DeletePendings();
