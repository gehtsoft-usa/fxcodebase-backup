//Available @  https://fxcodebase.com/code/viewtopic.php?f=38&t=75640
...
input bool include_sma = true; // Include SMA
input bool include_wma = true; // Include WMA
input bool include_ema = true; // Include EMA
input bool include_linreg = true; // Include Linear regression
input bool include_tema = true; // Include TEMA
input bool include_lwma = true; // Include LWMA
input bool include_zlma = true; // Include ZLMA
input bool include_rma = true; // Include RMA
input bool include_zltema = true; // Include ZLTEMA
...
int init()
{
   ...
   if (include_sma)
   {
      builder.AddCell(Create(new ConditionFactory(ma_sma), "SMA"));
   }
   if (include_wma)
   {
      builder.AddCell(Create(new ConditionFactory(ma_wma), "WMA"));
   }
   if (include_ema)
   {
      builder.AddCell(Create(new ConditionFactory(ma_ema), "EMA"));
   }
   if (include_linreg)
   {
      builder.AddCell(Create(new ConditionFactory(ma_linreg), "LinReg"));
   }
   if (include_tema)
   {
      builder.AddCell(Create(new ConditionFactory(ma_tema), "TEMA"));
   }
   if (include_lwma)
   {
      builder.AddCell(Create(new ConditionFactory(ma_lwma), "LWMA"));
   }
   if (include_zlma)
   {
      builder.AddCell(Create(new ConditionFactory(ma_zlma), "ZLMA"));
   }
   if (include_rma)
   {
      builder.AddCell(Create(new ConditionFactory(ma_rma), "RMA"));
   }
   if (include_zltema)
   {
      builder.AddCell(Create(new ConditionFactory(ma_zltema), "ZLTEMA"));
   }
