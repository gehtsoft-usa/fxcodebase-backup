...
//http://fxcodebase.com/code/viewtopic.php?f=38&t=70101
...
#property indicator_label1  "Kijub"
#property indicator_type1   DRAW_LINE
#property indicator_color1  Blue
#property indicator_style1  STYLE_SOLID
#property indicator_width1  1
#property indicator_label2  "BB Top"
...
input int ich_shift = 0; // Ichimoku shift
...
public:
   ...
   {
      return (kijun[period] > bb_top[period] != kijun[period + 1] > bb_top[period + 1]);
   }
};
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      return (kijun[period] < bb_bottom[period] != kijun[period + 1] < bb_bottom[period + 1]);
   }
};
...
int init()
{
   ...
   IndicatorBuffers(8);
   IndicatorObjPrefix = GenerateIndicatorPrefix("ichbb");
   IndicatorShortName("Ichimoku BB");
   ...
   SetIndexBuffer(0, kijun);
   SetIndexLabel(0, "Kijub");

   SetIndexStyle(1, DRAW_LINE, STYLE_SOLID);
   SetIndexBuffer(1, bb_top);
   SetIndexLabel(1, "BB Top");

   SetIndexStyle(2, DRAW_LINE, STYLE_SOLID);
   SetIndexBuffer(2, bb_bottom);
...
int start()
{
   ...
   for (int pos = limit; pos >= 0; --pos)
   {
      ...
      kijun[pos] = iIchimoku(_Symbol, tf, tenkan_sen, kijun_sen, senkou_span_b, MODE_KIJUNSEN, index + ich_shift);
      bb_top[pos] = iBands(_Symbol, tf, bb_period, bb_deviation, bb_shift, bb_price, MODE_UPPER, index);
      bb_bottom[pos] = iBands(_Symbol, tf, bb_period, bb_deviation, bb_shift, bb_price, MODE_LOWER, index);
      if (customStream != NULL)
      {
