...
// http://fxcodebase.com/code/viewtopic.php?f=38&t=70261
...
input double ATRMultiplier7 = 3.236; // Multiplier
input int rsi_period = 14; // RSI Period
input int buy_level = 25; // RSI Buy Level
input int sell_level = 75; // RSI Sell Level
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      double value = iCustom(_symbol, _timeframe, "TMA ATR Channel", _timeframe, 
         TMAPeriod, Price, ATRPeriod, TrendThreshold, false, 1, 1, 1, 1, 1, 1, ATRMultiplier7, 13, period);
...
public:
   ...
   {
      double value = iCustom(_symbol, _timeframe, "TMA ATR Channel", _timeframe, 
         TMAPeriod, Price, ATRPeriod, TrendThreshold, false, 1, 1, 1, 1, 1, 1, ATRMultiplier7, 14, period);
      return iClose(_symbol, _timeframe, period) <= value;
   }
};
...
int OnInit()
{
   double temp = iCustom(NULL, 0, "TMA ATR Channel", 0, TMAPeriod, Price, ATRPeriod, TrendThreshold, false, 1, 1, 1, 1, 1, 1, ATRMultiplier7, 0, 0);
   if (GetLastError() == ERR_INDICATOR_CANNOT_LOAD)
   {
      Alert("Please, install the 'TMA ATR Channel' indicator");
      return INIT_FAILED;
   }
   #ifdef SHOW_ACCOUNT_STAT
      stats = NULL;
