...
input int fxperiod=10;
input int price=0; // 0 or other = (H+L)/2
                   // 1 = Open
                   // 2 = Close
                   // 3 = High
                   // 4 = Low
                   ...
                   // 6 = (O+C+H+L)/4
                   // 7 = (O+C)/2
input bool Mode_Fast= False;
input bool Signals= False;
input int MA1period=9, MA2period=45;
input string TypeHelp = "SMA- 0, EMA - 1, SMMA - 2, LWMA- 3";
input string TypeHelp2 = "John Hyden settings TypeMA1=0, TypeMA2=3";
input int TypeMA1=0;
input int TypeMA2=3;
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      double ma = iCustom(_symbol, _timeframe, "FX_FISH_2MA", fxperiod, price, Mode_Fast, false, MA1period, MA2period, "", "", TypeMA1, TypeMA2, 3, period);
      return ma > 0;
   }
};
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      double hist = iCustom(_symbol, _timeframe, "FX_FISH_2MA", fxperiod, price, Mode_Fast, false, MA1period, MA2period, "", "", TypeMA1, TypeMA2, 2, period);
      return hist > 0;
   }
};
...
ICondition* CreateLongFilterCondition(string symbol, ENUM_TIMEFRAMES timeframe)
{
   ...
   AndCondition* condition = new AndCondition();
   condition.Add(new FilterLongCondition(symbol, timeframe), false);
   return condition;
}
...
ICondition* CreateShortFilterCondition(string symbol, ENUM_TIMEFRAMES timeframe)
{
   ...
   AndCondition* condition = new AndCondition();
   condition.Add(new FilterShortCondition(symbol, timeframe), false);
   return condition;
}
...
int OnInit()
{
   double temp = iCustom(0, 0, "FX_FISH_2MA", fxperiod, price, Mode_Fast, false, MA1period, MA2period, "", "", TypeMA1, TypeMA2, 3, 0);
   if (GetLastError() == ERR_INDICATOR_CANNOT_LOAD)
   {
      Alert("Please, install the 'FX_FISH_2MA' indicator");
      return INIT_FAILED;
   }
   #ifdef SHOW_ACCOUNT_STAT
      stats = NULL;
