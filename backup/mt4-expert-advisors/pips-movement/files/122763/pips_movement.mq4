...
// http://fxcodebase.com/code/viewtopic.php?f=38&t=67154
...
extern double initial_price = 0.0; // Initial price
extern double entry_distance = 20; // Distance to open trade, pips
...
STOP_LOSS_FEATURE double breakeven_level = 0; // Breakeven targer
...
public:
   ...
   {
      return (iClose(_instrument.GetSymbol(), _timeframe, period) - initial_price) / _instrument.GetPipSize() >= entry_distance;
   }
};
...
public:
   ...
   bool IsPass(const int period)
   {
      return (initial_price - iClose(_instrument.GetSymbol(), _timeframe, period)) / _instrument.GetPipSize() >= entry_distance;
   }
};
...
// Entry strategy v.1.0
interface IEntryStrategy
{
public:
   virtual int OpenPosition(OrderSide side, IMoneyManagementStrategy *moneyManagement, const string comment, double &stopLoss) = 0;
...
public:
   ...
   void DoTrading()
   {
      ...
      if (_longCondition.IsPass(0) && !_longCondition.IsPass(1))
      {
         ...
         double stopLoss = 0.0;
         int order = _entryStrategy.OpenPosition(BuySide, _longMoneyManagement, "Main position", stopLoss);
         if (order >= 0)
         {
      ...
      if (_shortCondition.IsPass(0) && !_shortCondition.IsPass(1))
      {
         ...
         double stopLoss = 0.0;
         int order = _entryStrategy.OpenPosition(SellSide, _shortMoneyManagement, "Main position", stopLoss);
         if (order >= 0)
         {
            _lastbartime = iTime(NULL, _timeframe, 0);
...
private:
   void DoMartingale(IMartingaleStrategy *martingale)
   {
      ...
      {
         double stopLoss;
         int order = _entryStrategy.OpenPosition(anotherSide, martingale.GetMoneyManagement(), "Martingale position", stopLoss);
         if (order >= 0)
         {
...
public:
   AccountStatistics(string eaName)
   {
      ...
      ObjectSet(IndicatorObjPrefix + "EA_NAME", OBJPROP_CORNER, 3); 
      ObjectSetText(IndicatorObjPrefix + "EA_NAME", _eaName, _fontSize + 3, "Impact", header_color);
   }
   ...
   void Update()
   {
      ...
      ObjectSetText(IndicatorObjPrefix + "hari", hari + ", " + DoubleToStr(Day(), 0) + " - " + DoubleToStr(Month(), 0) + " - " + DoubleToStr(Year(), 0), _fontSize+ 1 , "Impact", header_color);
      ObjectSet(IndicatorObjPrefix +  "hari", OBJPROP_CORNER, text_corner);

      string_window( "Balance", 5, 15+20, 0); //
      ObjectSetText(IndicatorObjPrefix +  "Balance","Balance   : " + DoubleToStr(AccountBalance(), 2), _fontSize, "Cambria", color_text);
      ObjectSet(IndicatorObjPrefix +  "Balance", OBJPROP_CORNER,text_corner);  

      string_window( IndicatorObjPrefix + "Equity", 5, 30+20, 0); //
      ObjectSetText( IndicatorObjPrefix + "Equity","Equity     : " + DoubleToStr(AccountEquity(),2), _fontSize, "Cambria", equity_color); 
      ObjectSet( IndicatorObjPrefix + "Equity", OBJPROP_CORNER, text_corner);  
      
      string_window( "Profit", 5, 45+20, 0); 
      ObjectSetText(IndicatorObjPrefix +  "Profit", "Profit    : " + DoubleToStr( XYZ,2) , _fontSize, "Cambria", equity_color); 
      ObjectSet( IndicatorObjPrefix + "Profit", OBJPROP_CORNER, text_corner);
      
      string_window("Leverage", 5, 60 + 20, 0);
      ObjectSetText(IndicatorObjPrefix + "Leverage", "Leverage   : " + DoubleToStr(AccountLeverage(), 0), _fontSize, "Cambria", color_text);
      ObjectSet(IndicatorObjPrefix + "Leverage", OBJPROP_CORNER, text_corner);

      string_window( IndicatorObjPrefix + "Spread", 5,75+20, 0);
      ObjectSetText(IndicatorObjPrefix +  "Spread","Spread   : " + DoubleToStr(_symbol.GetSpread(), 1), _fontSize, "Cambria", color_text);
      ObjectSet( IndicatorObjPrefix + "Spread", OBJPROP_CORNER, text_corner);
      
      double Range = (iHigh(NULL, 1440, 0) - iLow(NULL, 1440, 0)) / _symbol.GetPipSize();
      string_window( "Range", 5, 90+20, 0); //
      ObjectSetText( IndicatorObjPrefix + "Range","Range : " + DoubleToStr(Range,1) , _fontSize, "Cambria", color_text); 
      ObjectSet( IndicatorObjPrefix + "Range", OBJPROP_CORNER, text_corner); 
      
      string_window( "Price", 5, 125, 0); //
      ObjectSetText(IndicatorObjPrefix + "Price", "Price : " + DoubleToStr(Bid, Digits), _fontSize, "Cambria", GetPriceColor()); 
      ObjectSet( IndicatorObjPrefix + "Price", OBJPROP_CORNER, text_corner); 
   }
private:
   color GetPriceColor()
   {
   ...
   {
      ObjectCreate(IndicatorObjPrefix + n, OBJ_LABEL, WindowToUse, 0, 0 );
      ObjectSet(IndicatorObjPrefix + n, OBJPROP_CORNER, 1 );
      ObjectSet(IndicatorObjPrefix + n, OBJPROP_XDISTANCE, xoff );
      ObjectSet(IndicatorObjPrefix + n, OBJPROP_YDISTANCE, yoff );
      ObjectSet(IndicatorObjPrefix + n, OBJPROP_BACK, true );
      return (0);
   }
};
...
int OnInit()
{
   IndicatorName = GenerateIndicatorName("pips_movement");
   ...
   if (close_on_opposite)
      controller.SetCloseOnOpposite(new DoCloseOnOppositeStrategy(magic_number, slippage_pips));
   else
   ...
   controller.SetEntryStrategy(new MarketEntryStrategy(_Symbol, magic_number, slippage_pips));
   if (mandatory_closing)
      controller.SetMandatoryClosing(new DoMandatoryClosing(magic_number, signaler, slippage_pips));
   else
