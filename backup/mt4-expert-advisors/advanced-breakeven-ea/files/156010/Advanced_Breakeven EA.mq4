...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=75032
...
void    update_lastbox(int side)
{
    ...
    for (int i = 2; i <= ChannelPeriod + 1; i++) {
        ...
        if (h > top) top = h;
        if (l < bottom || bottom == 0) bottom = l;
    }
    lastbox.top    = top;
    lastbox.bottom = bottom;
}

string file_custom_indicator = "DonchianBreakoutSystem_v1.1.2 600+.ex4";
int    bufferToBuy           = 0;
int    bufferToSell          = 1;
...
input bool       ShowAnalysis       = true;
input bool       ShowStatsComment   = true;
input bool       ShowArrows         = true;
input bool       ShowProfit         = true;
input bool       ShowExits          = true;
...
double          uEntryDistance    = 10;                              // Pips distance for pending orders:
input int       uExpireHs         = 48;                              // Expiration hours:
input bool      uDeletePendingsOn = true;                            // Delete pendings when close all:
input bool      checkBoxSize_on   = true;                            // Check Box Size:
input double    checkBoxSize      = 15;                              // Minimum Box Size:
#else
...
input string       userComment   = "...comment";  // Your Comment:
...
input string tMatingale                = "== Martingale Setup =="; // == Matingale Setup ==
...
input int    userTPpips   = 5;                 // Pips TP
...
class SendNewOrder : public iActions
{
  ...
  public:
    SendNewOrder(string side, double lots, string symbol = "", double price = 0, double sl = 0, double tp = 0, int magic = 0, string coment = "", datetime expire = 0)
    {
        ...
        Print(__LINE__, " newOrder.expireTime() ", newOrder.expireTime());
    }
...
class BUYcondition1 : public iConditions
{
  public:
    bool evaluate()
    {
        ...
        if (Indi(bufferToSell, 1) != EMPTY_VALUE) {
            update_lastbox(-1);
            return true;
        }
        return false;
...
class BUYcondition2 : public iConditions
{
  public:
    bool evaluate()
    {
        ...
        if (!checkBoxSize_on) return true;

        double box_size = (lastbox.top - lastbox.bottom) / 10 / _Point;
        if (box_size >= checkBoxSize) {
            return true;
        }
        return false;
    }
...
class SELLcondition1 : public iConditions
{
  public:
    bool evaluate()
    {
        ...
        if (Indi(bufferToBuy, 1) != EMPTY_VALUE) {
            update_lastbox(1);
            return true;
        }
        return false;
...
void OnTick()
{
    ...
    if (!uTradeReverse) {
        if (conditionsToBuy.EvaluateConditions()) {
            if (modeEntry == Market) {
                actionSendOrder = new SendNewOrder("buy", Lots(), "", 0, SL("buy"), TP("buy"), magico, userComment);
            } else {
                double tp = lastbox.top;
                double pr = lastbox.top - userTPpips * 10 * _Point;
                double sl = pr - userSLpips * 10 * _Point;

                datetime expireTm = TimeCurrent() + uExpireHs * 3600;
                actionSendOrder = new SendNewOrder("buy", Lots(), "", pr, sl, tp, magico, userComment,  expireTm);
            }
        ...
        if (conditionsToSell.EvaluateConditions()) {
            if (modeEntry == Market) {
                actionSendOrder = new SendNewOrder("sell", Lots(), "", 0, SL("sell"), TP("sell"), magico, userComment);
            } else {
                double tp = lastbox.bottom;
                double pr = lastbox.bottom + userTPpips * 10 * _Point;
                double sl = pr + userSLpips * 10 * _Point;
                datetime expireTm = TimeCurrent() + uExpireHs * 3600;
                actionSendOrder = new SendNewOrder("sell", Lots(), "", pr, sl, tp, magico, userComment, expireTm);
            }
            if (actionSendOrder.doAction()) {
                mainOrders.GetMarketOrders();
...
double Indi(int buffer, int candle)
{
    ...
    return iCustom(NULL, 0, file_custom_indicator, UniqueName, Price, ChannelPeriod, MaxChannelPeriod, Margin, MinChannelWidth, UpTrendColor, DnTrendColor, ShowFilledBoxes, ShowChannel, AnalysisSets,
                   ShowAnalysis, ShowStatsComment, ShowArrows, ShowProfit, ShowExits, WinColor, LossColor, alerts, AlertOn, AlertShift, SoundsNumber, SoundsPause, UpTrendSound, DnTrendSound, EmailOn,
                   EmailsNumber, PushNotificationOn, buffer, candle);
}
