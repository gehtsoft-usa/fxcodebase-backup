...
//https://fxcodebase.com/code/viewtopic.php?f=38&t=71555
...
#property indicator_label1  "MA of Volume"
...
#endif
input MATypes ma_method = ma_sma; // Smoothing method
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      ...
      return iVolume(_symbol, _timeframe, period) < mavol[period];
   }
};
...
int init()
{
   ...
   src = new SimplePriceStream(_Symbol, (ENUM_TIMEFRAMES)_Period, PriceVolume);
   ma = AveragesStreamFactory::Create(src, ma_period, ma_method);
                ...
                const int &spread[])
{
   ...
   {
      ArrayInitialize(mavol, EMPTY_VALUE);
      if (customStream != NULL)
      {
         customStream.Init();
      }
   ...
   for (int pos = MathMin(bars_limit, rates_total - 1 - MathMax(prev_calculated - 1, toSkip)); pos >= 0 && !IsStopped(); --pos)
   {
      ...
      if (customStream != NULL)
      {
         customStream.SetValue(pos, mavol[pos]);
      }
