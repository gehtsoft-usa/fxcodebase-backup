//Available @  https://fxcodebase.com/code/viewtopic.php?f=38&p=159260#p159260
...
input bool addArrow = true; // Confirmation arrow
input int arrNumber = 500; // Max bars for arrow
...
// 'BarNumber' changes from 1 to 'Bars', so the array index is [Bars-1 .. 0]
   for(; BarNumber < Bars - 1;)
     {
      BarNumber++;
      processBar();
     }
   for(int i = MathMin(Bars, 500) - 1; i >= 0; i--)
     {
      if(line5[i] == 1 && iClose(Symbol(), Period(), i + 1) < iClose(Symbol(), Period(), i))
         drawArrow(-1, i);
      else
         drawArrow(-11, i);
      if(line5[i] == -1 && iClose(Symbol(), Period(), i + 1) > iClose(Symbol(), Period(), i))
...
void drawArrow(int dir, int bar)
  {
   ...
   if(dir == -1)
     {
      ObjectCreate(0, "arrD" + (string)bar, OBJ_ARROW, 0, iTime(Symbol(), Period(), bar), iHigh(Symbol(), Period(), bar) + Distance * Point());
   ...
   if(dir == 1)
     {
      ObjectCreate(0, "arrU" + (string)bar, OBJ_ARROW, 0, iTime(Symbol(), Period(), bar), iLow(Symbol(), Period(), bar) - Distance * Point());
