...
//http://fxcodebase.com/code/viewtopic.php?f=38&t=70399
...
#property description "Tape chart of the 'Trading Made Simple' system oscillator"
...
#property indicator_color4 Yellow,Green,Red
...
input bool               Alert_Popup=false;         // Popup window & sound on alert
input bool               Alert_Notification=false;  // Send push notifiaction on alert
input bool               Alert_Email=false;         // Send email on alert
input string             Alert_Subject="Default";   // Email Subject. If "Default" the subject is set by the indicator
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      ...
      return gadTrendColor[index] == 1 && gadTrendColor[index - 1] != 1;
   }
};
...
public:
   ...
   {
      int index = iBars(_symbol, _timeframe) - period - 1;
      return gadTrendColor[index] == 2 && gadTrendColor[index - 1] != 2;
   }
};
...
void OnInit()
{
   ...
   SetIndexBuffer(1, gadRSISig, INDICATOR_DATA);
   PlotIndexSetInteger(1, PLOT_DRAW_TYPE, DRAW_LINE);
   PlotIndexSetString(1, PLOT_LABEL, "RSI Signal");
   
   SetIndexBuffer(2, gadTradeSig, INDICATOR_DATA);
   PlotIndexSetInteger(2, PLOT_DRAW_TYPE, DRAW_LINE);
   PlotIndexSetString(2, PLOT_LABEL, "Trade Signal");

   SetIndexBuffer(3, gadTrend, INDICATOR_DATA);
   PlotIndexSetInteger(3, PLOT_DRAW_TYPE, DRAW_COLOR_HISTOGRAM);
   SetIndexBuffer(4, gadTrendColor, INDICATOR_COLOR_INDEX);
                ...
                const int &spread[])
{
   if (prev_calculated <= 0 || prev_calculated > rates_total)
   {
      ...
      ArrayInitialize(gadTrend, EMPTY_VALUE);
      ArrayInitialize(gadTrendColor, 0);
      for (int i = 0; i < ArraySize(alerts); ++i)
      {
         alerts[i].Init();
      }
   ...
   for (int pos = MathMax(first, prev_calculated - 1); pos < rates_total; ++pos)
   {
      ...
      gadTrend[pos] = 1;
      if (gadRSISig[pos] > gadTradeSig[pos])
      {
         gadTrendColor[pos] = 1;
      }
      else if (gadRSISig[pos] < gadTradeSig[pos])
      {
         gadTrendColor[pos] = 2;
      }
      else
