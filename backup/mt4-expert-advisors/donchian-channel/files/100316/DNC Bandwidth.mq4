...
extern bool In_Pips=false;


double Up[], Dn[] , Bandwidth[];

int init()
{
 IndicatorShortName("DNC Bandwidth");
...
int start()
{
 ...
 while(pos>=0)
 {
      ...
      if (In_Pips)
	  {
	   Bandwidth [pos] = (Up[pos] -  Dn[pos])/(MarketInfo(Symbol(), MODE_TICKSIZE)*10);
	  }
	  else
	  {
	  Bandwidth [pos] = (Up[pos] -  Dn[pos]) ;
	  }
  pos--;
 } 
 return(0);
