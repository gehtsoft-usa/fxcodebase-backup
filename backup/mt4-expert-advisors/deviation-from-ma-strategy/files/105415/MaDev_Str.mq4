...
int OnInit()
{
     double temp = iCustom(NULL, 0, "MaDev", 0, 0);
   if (GetLastError() == ERR_INDICATOR_CANNOT_LOAD)
   {
       Alert("Please, install the 'MaDev' indicator");
       return INIT_FAILED;
   }
   LastBar=0;
...
void OnTick()
{
 ...
 double MaDev_Up, MaDev_Dn;
 MaDev_Up=iCustom(NULL, 0, "MaDev", Method, Price, Length, Frame, 1, 1, Shift+1);
 MaDev_Dn=iCustom(NULL, 0, "MaDev", Method, Price, Length, Frame, 1, 2, Shift+1);
 
 int CO;
 int res;
 double SL, TP;
 
 if (MathAbs(MaDev_Dn-Low[Shift+1])<=Point)
 {
  _Alert("Buy");
  CO=CalculateOrders();
  if (CO<=0)
 ...
 if (MathAbs(MaDev_Up-High[Shift+1])<=Point)
 {
  _Alert("Sell");
  CO=CalculateOrders();
  if (CO>=0)
