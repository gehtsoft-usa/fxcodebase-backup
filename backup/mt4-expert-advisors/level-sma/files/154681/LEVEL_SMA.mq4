//Available @  https://fxcodebase.com/code/viewtopic.php?f=38&t=74696
...
extern int            FastMaPeriod    = 200;            // Fast moving average period
...
extern int            FastMaShift     = 0;            // Fast moving average shift
extern int            SlowMaPeriod    = 44;            // Slow moving average period
...
extern int            SlowMaShift     = 0;            // Slow moving average shift
extern enDisplay      DisplayType     = en_lid;       // Display type
extern int            LinesWidth1      = 2;            // Lines width 1 (when lines are included in display)
extern int            LinesWidth2      = 2;            // Lines width 2 (when lines are included in display)
extern int            ArrowSize       = 3;            // Arrow size
extern bool           alertsOn        = true;        // Turn alerts on?
extern bool           alertsCross     = true;         // Alerts on crossing
extern bool           alertsTouch     = true;            // Alerts on touching
extern int            minutesNotify   = 120;             // Minutes between touch notify
...
extern bool           alertsPushNotif = true;        // Alerts should send push notification?
extern bool           alertsEmail     = false;        // Alerts should send email?
extern string         soundFile       = "alert2.wav"; // Sound file
extern int            ArrowCodeUp     = 222;          // Arrow code up
extern int            ArrowCodeDn     = 221;          // Arrow code down
extern double         ArrowGapUp      = -4;          // Gap for arrow up        
extern double         ArrowGapDn      = -4;          // Gap for arrow down
...
#define _mtfCall(_buff,_y) iCustom(NULL,TimeFrame,indicatorFileName,tf_cu,0,FastMaPeriod,FastMaMethod,FastMaPrice,0,SlowMaPeriod,SlowMaMethod,SlowMaPrice,0,DisplayType,0,0,ArrowSize,alertsOn,alertsOnCurrent,alertsMessage,alertsSound,alertsPushNotif,alertsEmail,soundFile,ArrowCodeUp,ArrowCodeDn,ArrowGapUp,ArrowGapDn,ArrowOnFirst,_buff,_y)
...
int init()
{
   ...
   SetIndexBuffer(2,fastMa);  SetIndexStyle(2,lstyle,EMPTY,LinesWidth1);
   SetIndexBuffer(3,slowMa);  SetIndexStyle(3,lstyle,EMPTY,LinesWidth2);
   ...
   SetIndexBuffer(5,count);  
         ...
         SetIndexShift(2, FastMaShift  * TimeFrame/_Period); 
         SetIndexShift(3, SlowMaShift  * TimeFrame/_Period);
  
   IndicatorShortName(timeFrameToString(TimeFrame)+  " Sunil Aloorkar ");
return(0);
}
   ...
   if (TimeFrame == _Period)
   {
     for(i=limit; i>=0; i--)
     {
      ...
      if (alertsOn)
      {
         ...
         {
            if (trend[whichBar] == 1) doAlertCross(" SELL ");
            if (trend[whichBar] ==-1) doAlertCross(" BUY ");       
         }    
         }
         if (alertsTouch) {    
            if ((Open[0]<slowMa[0] && Close[0]>slowMa[0]) || (Open[0]<slowMa[0] && Close[0]>slowMa[0]))
            doAlertTouch();
            }
          }
...
void doAlertCross(string doWhat)
{
   ...
   string message;
      ...
      if (previousAlert != doWhat || previousTime != Time[0]) {
          ...
          message = timeFrameToString(_Period)+" "+_Symbol+" at "+TimeToStr(TimeLocal(),TIME_SECONDS)+" SMA 44 & 200 =   "+doWhat;
             if (alertsMessage)     Alert(message);
             if (alertsPushNotif )  SendNotification(message);
             if (alertsEmail)       SendMail(_Symbol+" SMA ",message);
...
void doAlertTouch()
{
   ...
   string message;
      ...
      if ( previousTime + minutesNotify*60< TimeLocal()) {
          ...
          message = timeFrameToString(_Period)+" "+_Symbol+" at "+TimeToStr(TimeLocal(),TIME_SECONDS)+" SMA 44 & 200 = TOUCH ";
             if (alertsMessage)     Alert(message);
             if (alertsPushNotif )  SendNotification(message);
             if (alertsEmail)       SendMail(_Symbol+" SMA ",message);
             if (alertsSound)       PlaySound(soundFile);
