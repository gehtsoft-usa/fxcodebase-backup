      ...
      for(int i = 0; i < MAX_TIMEFRAMES; i++)
        {
         ...
         for(int j = 0; j < ArraySize(pairList); j++)
           {
            string sym = pairList[j];
            iCustom_handle = iCustom(sym, timeframes[i], fileName);
            val0 = 0;
            val1 = 0;
            c = 0;
            for(int k = 0; k < 20 && c <= 1; k++)
              {
               if(custom_indi_calc(0, k) > 0 && custom_indi_calc(0, k) != EMPTY_VALUE)
                 {
                  if(c == 0)
                    {
                     val0 = custom_indi_calc(0, k);
                     c++;
                     continue;
                    }
                  if(c == 1)
                    {
                     val1 = custom_indi_calc(0, k);
                     break;
                    }
                 }
              }
         
            string label = (val0 > val1) ? "Buy" : (val0 < val1) ? "Sell" : "-";
