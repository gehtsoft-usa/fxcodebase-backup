...
// Input parameters
input bool          UseRiskFree        = true;           // Enable/Disable risk-free mode
...
void OpenTradeOnTesterIfNoOrders()
{
    if (!IsTesting()) return;

    // Verifica si hay operaciones abiertas
    int openOrders = 0;
    for (int i = OrdersTotal() - 1; i >= 0; i--) {
        if (OrderSelect(i, SELECT_BY_POS, MODE_TRADES) && OrderSymbol() == Symbol()) openOrders++;
    }
    if (openOrders > 0) return;

    // Verifica la última vela
    double open  = iOpen(Symbol(), 0, 1);
    double close = iClose(Symbol(), 0, 1);

    double lot = 0.1; // Puedes ajustar el lote

    if (close > open) {
        // Última vela alcista, abre BUY
        OrderSend(Symbol(), OP_BUY, lot, Ask, 3, 0, 0, "TesterBuy", masterMagic, 0, clrGreen);
    } else if (close < open) {
        // Última vela bajista, abre SELL
        OrderSend(Symbol(), OP_SELL, lot, Bid, 3, 0, 0, "TesterSell", masterMagic, 0, clrCrimson);
    }
}
...
{
    for (int i = OrdersTotal() - 1; i >= 0; i--) {
        if (OrderSelect(i, SELECT_BY_POS, MODE_TRADES) && OrderSymbol() == Symbol() && OrderMagicNumber() == masterMagic) {
            return OrderTicket();
        }
    }
    return -1;
...
{
    if (OrderSelect(ticket, SELECT_BY_TICKET)) return OrderProfit() > 0;
    return false;
}
...
void ProcessGrid(int masterTicket)
{
    ...
    if ((direction == OP_BUY && Ask < gridPrice) || (direction == OP_SELL && Bid > gridPrice)) return;
    ...
    if (gridCount > 0) {
        ...
        if(UseRiskFree) AdjustPositionSL(prevTicket, gridPrice);
    } else {
        if(UseRiskFree) AdjustPositionSL(masterTicket, gridPrice);
    }
}

double CalculateNextGridPrice(int direction, double basePrice, int gridCount)
{
    double distance = (gridCount == 0) ? FirstGridDistance * pointMultiplier : GridDistance * pointMultiplier *gridCount;
...
void ApplyTrailingStop()
{
    for (int i = OrdersTotal() - 1; i >= 0; i--) {
        if (OrderSelect(i, SELECT_BY_POS, MODE_TRADES) && OrderSymbol() == Symbol()) {
            ...
            if (OrderType() == OP_BUY) {
                if (Bid - OrderStopLoss() > step * pointMultiplier) newSl = Bid - trail;
            } else if (OrderType() == OP_SELL) {
                if (OrderStopLoss() - Ask > step * pointMultiplier) newSl = Ask + trail;
            }

            if (newSl > 0 && MathAbs(OrderStopLoss() - newSl) > Point) {
...
int CountGridPositions()
{
    int count = 0;
    for (int i = OrdersTotal() - 1; i >= 0; i--) {
        if (OrderSelect(i, SELECT_BY_POS, MODE_TRADES) && OrderSymbol() == Symbol() && OrderMagicNumber() == gridMagic) {
            count++;
        }
    }
    return count;
...
int FindLastGridPosition()
{
    ...
    datetime lastTime   = 0;
    for (int i = OrdersTotal() - 1; i >= 0; i--) {
        if (OrderSelect(i, SELECT_BY_POS, MODE_TRADES) && OrderSymbol() == Symbol() && OrderMagicNumber() == gridMagic && OrderOpenTime() > lastTime) {
...
double GetLastGridLot()
{
    ...
    if (OrderSelect(ticket, SELECT_BY_TICKET)) return OrderLots();
    return 0;
}
// Available @  https://fxcodebase.com/code/viewtopic.php?f=38&t=76030
