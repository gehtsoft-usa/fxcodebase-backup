...
ulong FindMasterPosition()
{
    ...
    for (int i = PositionsTotal() - 1; i >= 0; i--) {
        ...
        if (PositionGetString(POSITION_SYMBOL) == _Symbol && PositionGetInteger(POSITION_MAGIC) == masterMagic) {
...
{
    if (PositionSelectByTicket(ticket)) return PositionGetDouble(POSITION_PROFIT) > 0;
    return false;
}
...
void ProcessGrid(ulong masterTicket)
{
    ...
    if ((direction == POSITION_TYPE_BUY && lastTick.ask < gridPrice) || (direction == POSITION_TYPE_SELL && lastTick.bid > gridPrice)) return;
...
void ApplyTrailingStop()
{
    for (int i = PositionsTotal() - 1; i >= 0; i--) {
        ...
        if (type == POSITION_TYPE_BUY) {
            ...
            if ((sl == 0 && price - PositionGetDouble(POSITION_PRICE_OPEN) >= trail) || (sl > 0 && price - sl >= step)) {
                newSl  = NormalizeDouble(trigger, _Digits);
                modify = (sl == 0 || MathAbs(newSl - sl) >= Point());
            }
        } else if (type == POSITION_TYPE_SELL) {
            double trigger = price + trail;
            if ((sl == 0 && PositionGetDouble(POSITION_PRICE_OPEN) - price >= trail) || (sl > 0 && sl - price >= step)) {
                newSl  = NormalizeDouble(trigger, _Digits);
                modify = (sl == 0 || MathAbs(newSl - sl) >= Point());
            }
        }
...
int CountGridPositions()
{
    ...
    for (int i = PositionsTotal() - 1; i >= 0; i--) {
        ...
        if (PositionGetString(POSITION_SYMBOL) == _Symbol && PositionGetInteger(POSITION_MAGIC) == gridMagic) {
            count++;
        }
    }
    return count;
...
ulong FindLastGridPosition()
{
    ...
    for (int i = PositionsTotal() - 1; i >= 0; i--) {
        ulong ticket = PositionGetTicket(i);
        if (PositionGetString(POSITION_SYMBOL) == _Symbol && PositionGetInteger(POSITION_MAGIC) == gridMagic && PositionGetInteger(POSITION_TIME) > lastTime) {
