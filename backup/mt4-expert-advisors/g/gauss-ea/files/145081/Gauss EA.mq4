...
// ------------------------------------------------------------------
input string           ICustom                 = "== Gaussian Setup ==";  // == Gaussian Setup ==
extern ENUM_TIMEFRAMES TimeFrame               = PERIOD_M15;              // Time frame
extern double          alpha                   = 0.14;
...
class Gaussian : public iIndicators
{
   string _symbol;
   int    _tf;
   string _file;
   struct Parameters
   {
      ...
      ENUM_TIMEFRAMES setup0;  // TimeFrame
      double          setup1;  // alpha
      int             setup2;  // maxbars
      double          setup3;  // ZoneUp
      double          setup4;  // ZoneDown
      bool            setup5;  // Interpolate
   };
   Parameters _setup;

  public:
   Gaussian(string uSymbol, int uTimeFrame)
   {
      _symbol = uSymbol;
      _tf     = uTimeFrame;
      _file   = "2nd Order Gaussian High Pass Filter mtf Zones.ex4";
   }
   ~Gaussian() { ; }

   Gaussian* file(string setfile)
   {
      _file = setfile;
      return &this;
   }

   void setSetup(ENUM_TIMEFRAMES set0, double set1, int set2, double set3, double set4, bool set5)
   {
      _setup.setup0 = set0;
      _setup.setup1 = set1;
      _setup.setup2 = set2;
...
class BUYcondition1 : public iConditions
{
  public:
   bool evaluate()
   {
      ...
      if (gaus.calculate(1) > 0 && gaus.calculate(2) < 0)
      {
         return true;
      }
      return false;
...
int OnInit()
{
   ...
   //--- INDICATORS SETUPS:
   gaus.setSetup(TimeFrame, alpha, maxbars, ZoneUp, ZoneDown, Interpolate);
