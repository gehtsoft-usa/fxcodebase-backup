...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=73689
...
input string timePendings = "06:00:00";       // Time to put orders (GMT)
input string timeClose    = "09:00:00";       // Time to close orders (GMT)
input int qntBuys  = 5; // # Buys:
input int qntSells = 5; // # Sells:
// input double    uEntryDistance    = 150;                              // Pips distance for pending orders:
input int pipsBtw  = 50; // Pips Between Orders:
...
// #ifdef TIMER_MINI
  ...
  sesionControl.AddSession(timePendings, timeClose);
// #endif
 ...
 // Close controls:
  ...
  if (!uTradeReverse)
  {
    if (conditionsToBuy.EvaluateConditions())
    {
			...
			{
				double pr       = Price("buy", i * pipsBtw, _Symbol);
        ...
        actionSendOrder.doAction();
			...
			{
				double pr       = Price("sell", i * pipsBtw, _Symbol);
