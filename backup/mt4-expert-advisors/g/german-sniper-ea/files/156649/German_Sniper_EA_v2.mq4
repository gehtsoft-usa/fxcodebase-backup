...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=73454
...
string file_custom_indicator = "German Sniper Indicator V1.ex4";
...
input int      uTimeZone  = -6;                         // Set your GMT zone:
  ...
  int closeAllInList()
  {
    ...
    for (int i = 0; i < ArraySize(orders); i++)
    {
      ...
      if (OrderSelect(tk, SELECT_BY_TICKET))
      {
        ...
        // chequiar que la orden no esté cerrada ya o que el tk sea valido...

        
        if (!OrderClose(OrderTicket(), OrderLots(), closePrice, 0, clrNONE))
        {
...
class Grid
{
 ...
 public:
  ...
  double SL(string side="") { return _sl; }
  double TP(string side="") { return _tp; }
	 
	 void sl(double inpsl) { _sl = inpsl; }
	 void tp(double inptp) { _tp = inptp; }
 ...
 public:
  bool evaluate()
  {
    // TODO: condition Buy 1
		return iClose(NULL, 0, 1) > iCustom(NULL, 0, file_custom_indicator, 0, 1) 
	      && iLow(NULL, 0, 1) < iCustom(NULL, 0, file_custom_indicator, 0, 1)
				&& iCustom(NULL, 0, file_custom_indicator, 0, 1) != EMPTY_VALUE;
 ...
 public:
  bool evaluate()
  {
    // TODO: condition sell 1
		return iClose(NULL, 0, 1) < iCustom(NULL, 0, file_custom_indicator, 1, 1) 
	    		&& iHigh(NULL, 0, 1) > iCustom(NULL, 0, file_custom_indicator, 1, 1)
				 && iCustom(NULL, 0, file_custom_indicator, 1, 1) != EMPTY_VALUE;
      ...
      if (actionSendOrder.doAction())
      {
        ...
        if (GridON == true && CheckPointer(gridBuy) == POINTER_INVALID)
        {
          ...
          conditionsToBuy.AddCondition(gridActiveCondition = new ConditionGridActive(gridBuy));
					gridBuy.tp(mainOrders.last().tp());
					gridBuy.sl(mainOrders.last().sl());
        }
      ...
      if (actionSendOrder.doAction())
      {
        ...
        if (GridON == true && CheckPointer(gridSell) == POINTER_INVALID)
        {
          ...
          conditionsToSell.AddCondition(gridActiveCondition = new ConditionGridActive(gridSell));
					...
					gridSell.tp(mainOrders.last().tp());
					gridSell.sl(mainOrders.last().sl());
        }
