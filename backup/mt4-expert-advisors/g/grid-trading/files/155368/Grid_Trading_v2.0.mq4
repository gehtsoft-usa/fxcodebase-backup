...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=73954
...
input StartGridMode startMode           = Immediately;        // Start Grid Mode:
input double        price_start_grid    = 0.00;               // Price to start:
input SideGrid      side_Grid           = Buy;                // Side Grid:
...
input grid_gap_mode gap_mode            = gap_by_money;       // Mode Gap:
input int           GridUser_gap        = 30;                 // Gap betwen orders (pips):
input double        user_gap_money      = 20;                 // Gap by money:
...
class Grid
{
  ...
  public:
    Grid(string Symbol, string Side, double LastPrice, double Gap, double Multiplier, int MaxQnt, double MaxLot, double InitialLot, int magic, bool simbolFilterOn = true, bool magicFilterOn = true)
    {
        ...
        Print("Grid Seting: ", _symbol);
        Print("Grid Seting: ", _side);
        Print("Grid Seting: ", _lastPrice);
        Print("Grid Seting: ", _nextPrice);
        Print("---------> Grid Seting GAP: ", _gap);
        Print("Grid Seting: ", _multiplier);
        Print("Grid Seting: ", _maxQnt);
        Print("Grid Seting: ", _maxLot);
        Print("Grid Seting: ", _initialLot);
        Print("Grid Seting: ", _nextLot);
    ...
    double pipsbymoney(double _lot)
    {
        double _tickValue    = MarketInfo(_symbol, MODE_TICKVALUE);
        double _modeCalc     = MarketInfo(_symbol, MODE_PROFITCALCMODE);
        double _contractSize = SymbolInfoDouble(_symbol, SYMBOL_TRADE_CONTRACT_SIZE);
        ...
        double _points       = MarketInfo(_symbol, MODE_POINT);
        double _digits       = MarketInfo(_symbol, MODE_DIGITS);
        double _money = fabs(user_gap_money);

        double divisor ;

        if (_modeCalc == 0) {
         divisor = _lot * _tickValue;
            if(divisor <= 0) divisor = 0.01;
            return NormalizeDouble(_money / divisor, 2);
        }
    ...
    double nextPrice(double inpLastPrice)
    {
        ...
        if (gap_mode == gap_by_money) {
            double gap = pipsbymoney(nextLot());
            Print(__FUNCTION__," gap: ",gap);
            
            if (_side == "buy") {
                _nextPrice = inpLastPrice - (gap/10 * mPoint );
  ...
  public:
    bool evaluate()
    {
        ...
        if (startMode == Immediately) {
            ...
            if (Ask > price_start_grid && Bid < price_start_grid)
                return true;
        }
