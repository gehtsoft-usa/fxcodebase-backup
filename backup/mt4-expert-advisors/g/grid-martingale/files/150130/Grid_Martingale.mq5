...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=73526
...
input string             Iema1                 = "== Moving Average Buy Setup ==";    // == Moving Average Buy Setup ==
input int                maFast_Period         = 200;                                   // Period
...
input string             Iema3                 = "== Moving Average Sell Setup ==";    // == Moving Average Sell Setup ==
...
MovingAverage* emaBuy;
MovingAverage* emaSell;
#endif
...
input int uMaxTrades = 20;  // Max Trades At Same Time:
...
input string tclose                = "== Close Setup ==";  // == Close Setup ==
input bool   closeBySideOn         = true; // Close by Side On?
input double close_buy_amount      = 3; // Amount to Close Buys:
input double close_sell_amount     = 3; // Amount to Close Sells:
...
class BUYcondition1 : public iConditions
{
 public:
  bool evaluate()
  {
    ...
    return iClose(NULL, 0, 1) > iClose(NULL, 0, 2);
  }
};
...
class BUYcondition2 : public iConditions
{
 public:
  ...
  {
    // TODO: condition Buy2
    return iClose(NULL, 0, 2) > emaBuy.index(2);
  }
};
 ...
 public:
  bool evaluate()
  {
    // NOTE: condition sell 2
		return iClose(NULL, 0, 2) < emaSell.index(2);
    // return false;
  }
};
  ...
  bool evaluate()
  {
    ...
    if (closeAllControlON) {
      return CloseALlControl();
		...
		if(closeBySideOn)
		{
			if( floatingEA("buy") > close_buy_amount )
			{
        closeAll("buy");
				return false;
      }
...
class ConditionToCloseSell : public iConditions
{
 public:
  bool evaluate()
  {
    ...
    if (closeAllControlON) { return CloseALlControl(); }

		if(closeBySideOn)
		{
			if( floatingEA("sell") > close_sell_amount )
			{
        closeAll("sell");
				return false;
      }
...
class MartingaleIncrementLot
{
 ...
 public:
  ...
  {
    setLastValues();
    double next = _lastLot ==0 ? userLots : NormalizeDouble(_lastLot * _multiplier, 2);
  ...
  void setLastValues()
  {
    ...
    int      diezDias = 60 * 60 * 24 * 10;
    datetime dateIni  = TimeCurrent() - diezDias;
    ...
    for (int i = total-1; i >= 0; i--) 
      ...
      int    magic  = HistoryDealGetInteger(tk, DEAL_MAGIC);
      string symbol = HistoryDealGetString(tk, DEAL_SYMBOL);
      double lots   = (float)HistoryDealGetDouble(tk, DEAL_VOLUME);

      if (symbol == _symbol && magic == _magic)
        if (profit < 0) 
				{
  
	        _lastTk     = tk;
          _lastProfit = profit;
          _lastLot    = NormalizeDouble(lots, 2);
	
					 		Print(__FUNCTION__," ","_lastTk"," ",_lastTk);
					 		Print(__FUNCTION__," ","_lastProfit"," ",_lastProfit);
					 		Print(__FUNCTION__," ","_lastLot"," ",_lastLot);
					break;				 	
	      } else {
          _lastTk     = 0;
          _lastProfit = 0;
  ...
  {
    ulong tk = HistoryDealGetTicket(i);  
    ENUM_DEAL_ENTRY   entry     = HistoryDealGetInteger(tk, DEAL_ENTRY);
    if (entry == 0) continue;
    ...
    if (symbol == _symbol && magic == _magic && entry == 1)
    {
      if (profit < 0) { count++; }
      if (profit >= 0) { break; }
    }
...
#ifdef MOVING_AVERAGE_ON
  emaBuy   = new MovingAverage(_Symbol, Period());
  emaSell   = new MovingAverage(_Symbol, Period());
  
	emaBuy.setSetup(maFast_Period, maFast_Shift, maFast_Method, maFast_AppliedPrice);
  emaSell.setSetup(maSlow_Period, maSlow_Shift, maSlow_Method, maSlow_AppliedPrice);
#endif
