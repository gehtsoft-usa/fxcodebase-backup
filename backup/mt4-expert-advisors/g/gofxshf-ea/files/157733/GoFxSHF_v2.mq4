...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=74823
...
string file_custom_indicator = "GoFxSHF";
int    bufferToBuy           = 2;
int    bufferToSell          = 3;

input string              Tindi          = "== Indicator Setup =="; // ————————————
extern int                FastMaPeriod   = 5;
extern int                MediumMaPeriod = 13;
extern int                SlowMaPeriod   = 34;
extern ENUM_MA_METHOD     MaMethod       = MODE_LWMA;
extern ENUM_APPLIED_PRICE MAAppliedPrice = PRICE_WEIGHTED; // 0=close, 1=open, 2=high, 3=low, 4=median(high+low)/2, 5=typical(high+low+close)/3, 6=weighted(high+low+close+close)/4
input bool                filterSupOn    = true;           // Filter by higher TF :
input ENUM_TIMEFRAMES     superiorTF     = PERIOD_H4;      // Higher TF:
...
double iSuperior(int buffer, int candle)
{
    // TODO: copiar la lista de INPUTS para custom indicator:
    return iCustom(NULL, superiorTF, file_custom_indicator,
                   FastMaPeriod,
                   MediumMaPeriod,
                   SlowMaPeriod,
                   MaMethod,
...
input bool   early_on      = false;         // Early Entry ?
...
input int                ma2_period        = 13;                             // Periods:
...
class BUYcondition1 : public iConditions
{
  public:
    bool evaluate()
    {
        ...
        bool SupCondition = true;
        if(filterSupOn)
        {
            double sup = iSuperior(bufferToBuy, 1);
            if(sup == 0 || sup == EMPTY_VALUE)
                SupCondition = false;
        }

        if(!early_on) {
            bool  CurCondition = false;
            double cur = indi(bufferToBuy, 1);        
            double cur2 = indi(bufferToBuy, 2);        
            if((cur > 0 && cur != EMPTY_VALUE) && (cur2 == 0 || cur2 == EMPTY_VALUE)) 
                CurCondition=true;

            return SupCondition && CurCondition;
        }

        if(early_on) 
        {
            // sell side
            if(indi(0, 1) == EMPTY_VALUE) 
            {
                bool  CurCondition = false;
                double cur = indi(bufferToSell, 1);        
                double cur2 = indi(bufferToSell, 2);        
                double signal = indi(1, 1);        
                double signal2 = indi(1, 2);        
                if((signal > cur && signal2 <= cur2 && cur != EMPTY_VALUE && cur > 0 && cur2 != EMPTY_VALUE && cur2 > 0))
                    CurCondition=true;
                return SupCondition && CurCondition;
            } else 
            {
                ...
                double cur = indi(bufferToBuy, 1);        
                double cur2 = indi(bufferToBuy, 2);        
                double signal = indi(0, 1);        
                double signal2 = indi(0, 2);        
...
class SELLcondition1 : public iConditions
{
  public:
    bool evaluate()
    {
        ...
        if(filterSupOn)
        {
            double sup = iSuperior(bufferToSell, 1);
            if(sup == 0 || sup == EMPTY_VALUE)
                SupCondition = false;
        }
...
bool CloseAllControl()
{
   switch (closeBy)
   {
      ...
      case CloseByEmas: 
      {
        for(int i=OrdersTotal()-1;i>=0;i--) {
            if(OrderSelect(i,SELECT_BY_POS) && OrderSymbol() == _Symbol && OrderMagicNumber()== magico) {
                if(OrderType() == OP_BUY) { return (cdMaCrossDn.evaluate()); }
                if(OrderType() == OP_SELL) { return (cdMaCrossUp.evaluate()); }
            }
        }
