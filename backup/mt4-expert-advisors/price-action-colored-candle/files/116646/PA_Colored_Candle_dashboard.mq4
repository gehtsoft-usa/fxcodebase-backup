...
private:
    ...
    int GetDirection()
    {
        if (iClose(_symbol, _timeframe, 0) > iLow(_symbol, _timeframe, 1))
        {
            return 1;
        }
        if (iClose(_symbol, _timeframe, 0) < iHigh(_symbol, _timeframe, 1))
        {
            return -1;
        }
        return 0;
...
int init()
{
    IndicatorName = GenerateIndicatorName("PA_Colored_Candle_dashboard");
   ...
   IndicatorShortName(IndicatorName);
    ...
    if (Include_M1)
    {
        ...
        for (i = 0; i < sym_count; i++)
        {
            rows[currentCell - 1].Add(new ValueCell(sym_arr[i] + "_M1", m1_x, ym1Iterator.GetNext(), sym_arr[i], PERIOD_M1));
        }
    }
    if (Include_M5)
    {
        ...
        for (i = 0; i < sym_count; i++)
        {
            rows[currentCell - 1].Add(new ValueCell(sym_arr[i] + "_M5", m5_x, ym5Iterator.GetNext(), sym_arr[i], PERIOD_M5));
        }
    }
    if (Include_M15)
    {
        ...
        for (i = 0; i < sym_count; i++)
        {
            rows[currentCell - 1].Add(new ValueCell(sym_arr[i] + "_M15", m15_x, ym15Iterator.GetNext(), sym_arr[i], PERIOD_M15));
        }
    }
    if (Include_M30)
    {
        ...
        for (i = 0; i < sym_count; i++)
        {
            rows[currentCell - 1].Add(new ValueCell(sym_arr[i] + "_M30", m30_x, ym30Iterator.GetNext(), sym_arr[i], PERIOD_M30));
        }
    }
    if (Include_H1)
    {
        ...
        for (i = 0; i < sym_count; i++)
        {
            rows[currentCell - 1].Add(new ValueCell(sym_arr[i] + "_H1", h1_x, yH1Iterator.GetNext(), sym_arr[i], PERIOD_H1));
        }
    }
    if (Include_H4)
    {
        ...
        for (i = 0; i < sym_count; i++)
        {
            rows[currentCell - 1].Add(new ValueCell(sym_arr[i] + "_H4", h4_x, yH4Iterator.GetNext(), sym_arr[i], PERIOD_H4));
        }
    }
    if (Include_D1)
    {
        ...
        for (i = 0; i < sym_count; i++)
        {
            rows[currentCell - 1].Add(new ValueCell(sym_arr[i] + "_D1", d1_x, yD1Iterator.GetNext(), sym_arr[i], PERIOD_D1));
        }
    }
    if (Include_W1)
    {
        ...
        for (i = 0; i < sym_count; i++)
        {
            rows[currentCell - 1].Add(new ValueCell(sym_arr[i] + "_W1", w1_x, yW1Iterator.GetNext(), sym_arr[i], PERIOD_W1));
        }
    }
    if (Include_MN1)
    {
        ...
        for (i = 0; i < sym_count; i++)
        {
            rows[currentCell - 1].Add(new ValueCell(sym_arr[i] + "_MN1", mn1_x, yMN1Iterator.GetNext(), sym_arr[i], PERIOD_MN1));
        }
    }
