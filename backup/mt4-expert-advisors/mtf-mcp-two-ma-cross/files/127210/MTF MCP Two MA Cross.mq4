...
// http://fxcodebase.com/code/viewtopic.php?f=38&t=68623
...
input int Period1 = 50; // Period #1
input ENUM_MA_METHOD Method1 = MODE_SMA; // MA Method #1
input int Period2 = 200; // Period #2
input ENUM_MA_METHOD Method2 = MODE_SMA; // MA Method #2
input bool Specific = true; // Base Instrument Specific
input string Instrument = "USD"; // Base Instrument
...
public:
   ...
   {
      double ma_1_1 = iMA(_symbol, _timeframe, Period1, 0, Method1, PRICE_CLOSE, period);
      double ma_1_2 = iMA(_symbol, _timeframe, Period1, 0, Method1, PRICE_CLOSE, period + 1);
      double ma_2_1 = iMA(_symbol, _timeframe, Period2, 0, Method2, PRICE_CLOSE, period);
      double ma_2_2 = iMA(_symbol, _timeframe, Period2, 0, Method2, PRICE_CLOSE, period + 1);
      return ma_1_1 > ma_2_1 && ma_1_2 <= ma_2_2;
   }
};
...
public:
   ...
   { 
      ...
      _symbol = symbol; 
      _timeframe = timeframe; 
      if (StringFind(_symbol, Instrument) == 0 || !Specific)
      {
      ...
      {
         _upCondition = new DownCondition(_symbol, _timeframe);
         _downCondition = new UpCondition(_symbol, _timeframe);
      }
   }
...
private:
   ...
   {
      for (barsBack = 0; barsBack < MathMin(iBars(_symbol, _timeframe) - 1, 500); ++barsBack)
      {
...
public:
   ...
   void AddTimeframe(const string label, const ENUM_TIMEFRAMES timeframe)
   {
      ...
      for (int i = 0; i < sym_count; i++)
      {
         row.Add(new BarsBackValueCell(IndicatorObjPrefix + sym_arr[i] + "_" + label, x, yIterator.GetNext(), sym_arr[i], timeframe));
      }
   }
...
int init()
{
   IndicatorName = GenerateIndicatorName("MTF MCP Two MA Cross");
