...
// http://fxcodebase.com/code/viewtopic.php?f=38&t=67309
...
string EA_NAME = "Triple Tops And Bottoms Strategy";
...
extern int Period = 100; // Period
extern double Threshold = 10; // Threshold
...
public:
   ...
   bool IsPass(const int period)
   {
      int TC = 0;
      int BC = 0;
      double min = iLow(_instrument.GetSymbol(), _timeframe, iLowest(_instrument.GetSymbol(), _timeframe, period + Period, period + 1));
      double max = iHigh(_instrument.GetSymbol(), _timeframe, iHighest(_instrument.GetSymbol(), _timeframe, period + Period, period + 1));
      double Delta = ((max - min) / 100) * Threshold;
      for (int i = period + 1; i <= period + Period; ++i)
      {
         double high = iHigh(_instrument.GetSymbol(), _timeframe, i);
         if (high > iHigh(_instrument.GetSymbol(), _timeframe, i + 1) 
            && high > iHigh(_instrument.GetSymbol(), _timeframe, i - 1)
            && high > max - Delta)
         {
            TC++;
         }
         double low = iLow(_instrument.GetSymbol(), _timeframe, i);
         if (low < iLow(_instrument.GetSymbol(), _timeframe, i + 1) 
            && low < iLow(_instrument.GetSymbol(), _timeframe, i - 1)
            && low < min + Delta)
         {
            BC++;
         }
      ...
      if ((BC < 2 && TC < 3) || (BC < 3 && TC < 2))
         return false;

      return iClose(_instrument.GetSymbol(), _timeframe, period) > max
         && iClose(_instrument.GetSymbol(), _timeframe, period + 1) <= max;
   }
};
...
public:
   ...
   bool IsPass(const int period)
   {
      ...
      return iClose(_instrument.GetSymbol(), _timeframe, period) < max
         && iClose(_instrument.GetSymbol(), _timeframe, period + 1) >= min;
   }
};
...
class LongMoneyManagementStrategy : public IMoneyManagementStrategy
{
   ...
   ENUM_TIMEFRAMES _timeframe;
public:
   LongMoneyManagementStrategy(TradeCalculator *calculator, const ENUM_TIMEFRAMES timeframe)
   {
      _calculator = calculator;
      _timeframe = timeframe;
   }
   ...
   void Get(const int period, const double entryPrice, double &amount, double &stopLoss, double &takeProfit)
   {
      ...
      double min = iLow(_calculator.GetSymbol(), _timeframe, iLowest(_calculator.GetSymbol(), _timeframe, period + Period, period + 1));
      double max = iHigh(_calculator.GetSymbol(), _timeframe, iHighest(_calculator.GetSymbol(), _timeframe, period + Period, period + 1));
      double Target = (max - min) / _calculator.GetPipSize();
      takeProfit = _calculator.CalculateTakeProfit(true, Target, StopLimitPips, amount, ask);
   }
};
...
class ShortMoneyManagementStrategy : public IMoneyManagementStrategy
{
   ...
   ENUM_TIMEFRAMES _timeframe;
public:
   ShortMoneyManagementStrategy(TradeCalculator *calculator, const ENUM_TIMEFRAMES timeframe)
   {
      _calculator = calculator;
      _timeframe = timeframe;
   }
   ...
   void Get(const int period, const double entryPrice, double &amount, double &stopLoss, double &takeProfit)
   {
      ...
      double max = iHigh(_calculator.GetSymbol(), _timeframe, iHighest(_calculator.GetSymbol(), _timeframe, period + Period, period + 1));
      double Target = (max - min) / _calculator.GetPipSize();
      takeProfit = _calculator.CalculateTakeProfit(true, Target, StopLimitPips, amount, bid);
   }
};
...
TradeController *CreateController(const string symbol, const ENUM_TIMEFRAMES timeframe, string &error)
{
   ...
   if (breakeven_type == StopLimitDoNotUse)
      ...
      controller.SetBreakeven(new BreakevenLogic(tradeCalculator, breakeven_type, breakeven_value, breakeven_level, signaler, timeframe));
   ...
   IMoneyManagementStrategy *longMoneyManagement = new LongMoneyManagementStrategy(tradeCalculator, timeframe);
   IMoneyManagementStrategy *shortMoneyManagement = new ShortMoneyManagementStrategy(tradeCalculator, timeframe);
