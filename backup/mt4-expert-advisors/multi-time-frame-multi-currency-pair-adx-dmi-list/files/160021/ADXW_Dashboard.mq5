...
// https://fxcodebase.com/code/viewtopic.php?f=38&p=156353#p156353
...
input double InpLevel = 20.0; // Level for ADXW
      ...
      for(int i = 0; i < MAX_TIMEFRAMES; i++)
        {
         ...
         for(int j = 0; j < ArraySize(pairList); j++)
           {
            string sym = pairList[j];
            iCustom_handle = iCustom(sym, timeframes[i], fileName, InpPeriodADXW);
            val0 = 0;
            val1 = 0;
            if(custom_indi_calc(0, 1) > InpLevel)
              {
               val0 = 1;
               val1 = 0;
              }
            ...
            string label = (val0 > val1) ? "Trend" : (val0 < val1) ? "Side" : "-";
            state += (label == "Trend") ? "t" : (label == "Side") ? "s" : "-";
            color clr = (val0 > val1) ? Confirmed_Up_Color : (val0 < val1) ? Confirmed_Dn_Color : Neutral_Color;
