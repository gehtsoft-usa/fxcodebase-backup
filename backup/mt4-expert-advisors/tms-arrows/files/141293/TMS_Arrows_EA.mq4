...
input string info1 = "Settings for TMS_Arrows indicator";         
input int RSI_Period = 13;         
input int RSI_Price = 0;           
input int Volatility_Band = 34;    
input int RSI_Price_Line = 2;      
input int RSI_Price_Type = 0;      
input int Trade_Signal_Line = 7;   
input int Trade_Signal_Type = 0;
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      double up = iCustom(Symbol(), Period(), "#TMS_Arrows", RSI_Period, RSI_Price, Volatility_Band, RSI_Price_Line, RSI_Price_Type, Trade_Signal_Line, Trade_Signal_Type, 6, period);
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      double dn = iCustom(Symbol(), Period(), "#TMS_Arrows", RSI_Period, RSI_Price, Volatility_Band, RSI_Price_Line, RSI_Price_Type, Trade_Signal_Line, Trade_Signal_Type, 7, period); 
      return dn != EMPTY_VALUE;
   }
};
...
int OnInit()
{
   double temp = iCustom(NULL, 0, "#TMS_Arrows", 0, 0);
   if (GetLastError() == ERR_INDICATOR_CANNOT_LOAD)
   {
      Alert("Please, install the '#TMS_Arrows' indicator");
      return INIT_FAILED;
   }
