...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=23440
...
// ------------------------------------------------------------------
input string IRSL = "== RSL Setup ==";  // == RSL Setup ==
extern int   P1   = 14;
...
input bool         closeByStrategyON       = true;                       // Close by Strategy ON:
...
class RSL
{
  ...
  public:
   ...
   RSL(string Symbol, int TimeFrame)
   {
      _symbol = Symbol;
      _tf     = TimeFrame;
      _file   = "RSL.ex4";
   ...
   {
      return iCustom(_symbol, _tf, _file, P1, USE, buffer, shift);
   }

   double lowerLine(int shift)
   {
      return calculate(shift, 0);
   }
   double upperLine(int shift)
   {
...
class BUYcondition1 : public iConditions
{
  public:
   bool evaluate()
   {
      ...
      double lower  = rsl.lowerLine(1);
      double upper  = rsl.upperLine(1);

      if (close1 > lower && close2 < lower)
      {
         return true;
      }
