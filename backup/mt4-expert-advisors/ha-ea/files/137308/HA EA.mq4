...
// http://fxcodebase.com/code/viewtopic.php?f=38&t=70376
...
TradingMode entry_logic = TradingModeOnBarClose; // Entry logic
...
input double lot_multiplier = 1.5; // Position size multiplier
...
public:
   LongCondition(const string symbol, ENUM_TIMEFRAMES timeframe)
      :ACondition(symbol, timeframe, "Green")
   {
   ...
   {
      double open, close;
      return ha.GetOpenClose(1, open, close) && open <= close;
   }
};
...
public:
   ShortCondition(const string symbol, ENUM_TIMEFRAMES timeframe)
      :ACondition(symbol, timeframe, "Red")
   {
...
class ReopenAction : public AAction
{
   IOrder* _order;
   InstrumentInfo* _instrument;
   ActionOnConditionLogic* _actions;
...
public:
   ReopenAction(IOrder* order, ActionOnConditionLogic* actions, IMoneyManagementStrategy* longMoneyManagement, IMoneyManagementStrategy* shortMoneyManagement)
   {
      _longMoneyManagement = longMoneyManagement;
      _shortMoneyManagement = shortMoneyManagement;
      _actions = actions;
      _order = order;
      _order.AddRef();
      _instrument = new InstrumentInfo(_Symbol);
   ...
   virtual bool DoAction(const int period, const datetime date)
   {
      ...
      double open, close;
      if (OrderOpenTime() > iTime(_Symbol, trading_timeframe, 0) || !ha.GetOpenClose(0, open, close))
      {
         return false;
      }

      double newamount = _instrument.RoundLots(OrderLots() * lot_multiplier);
      if (OrderType() == OP_BUY)
      {
         if (open > close)
         {
            string error;
            if (TradingCommands::CloseCurrentOrder(slippage_points, error))
            {
               double entryPrice = InstrumentInfo::GetBid(_Symbol);
               double amount;
               double takeProfit;
               double stopLoss;
               _shortMoneyManagement.Get(0, entryPrice, amount, stopLoss, takeProfit);
               if (amount == 0.0)
               {
                  return false;
               }
               ...
               {
                  _order.Release();
                  _order = new OrderByTicketId(order);
               }
               delete orderBuilder;
            }
         }
      ...
      if (open < close)
      {
         ...
         if (TradingCommands::CloseCurrentOrder(slippage_points, error))
         {
            double entryPrice = InstrumentInfo::GetAsk(_Symbol);
            double amount;
            double takeProfit;
            double stopLoss;
            _longMoneyManagement.Get(0, entryPrice, amount, stopLoss, takeProfit);
            if (amount == 0.0)
            {
               return false;
            }
...
class CreateReopenAction : public AOrderAction
{
   ActionOnConditionLogic* _actions;
   IMoneyManagementStrategy* _longMoneyManagement;
   IMoneyManagementStrategy* _shortMoneyManagement;
public:
   CreateReopenAction(ActionOnConditionLogic* actions, IMoneyManagementStrategy* longMoneyManagement, IMoneyManagementStrategy* shortMoneyManagement)
   {
      _longMoneyManagement = longMoneyManagement;
      _shortMoneyManagement = shortMoneyManagement;
      _actions = actions;
   ...
   virtual bool DoAction(const int period, const datetime date)
   {
      ...
      ReopenAction* action = new ReopenAction(order, _actions, _longMoneyManagement, _shortMoneyManagement);
...
TradingController *CreateController(const string symbol, const ENUM_TIMEFRAMES timeframe, string algoId, string &error)
{
   ...
   CreateReopenAction* createReopenAction = new CreateReopenAction(actions, longMoneyManagement, shortMoneyManagement);
   orderHandlers.AddOrderAction(createReopenAction);
   createReopenAction.Release();
...
int OnInit()
{
   ...
   #endif
   ha = new HABarStream(_Symbol, trading_timeframe);
