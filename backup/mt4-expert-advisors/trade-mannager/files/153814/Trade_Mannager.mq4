...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=74492
// ------------------------------------------------------------------
//+------------------------------------------------------------------------------------------------+
...
bool OnInit_GUI()
{
    ...
    {
        res = gui.Create(0, "Mannager Orders",10, 150, 10, 380, 250);
        if(res) gui.Run();
    }
    return res;
}
...
class BuyAction : public iActions
{
    bool doAction()
    {
        ...
        while(n < userNoTrades)
        {
            ...
            tp = TP("buy", pr);
            SendOrder = new SendNewOrder("buy", Lots(), "", pr, SL("buy", pr), tp, magico);
...
class SellAction : public iActions
{
    bool doAction()
    {
        ...
        while(n < userNoTrades)
        {
            ...
            SendNewOrder* SendOrder;
            SendOrder = new SendNewOrder("sell", Lots(), "", pr, SL("sell", pr), tp, magico);
            result = SendOrder.doAction();
            n++;
            delete SendOrder;            
        }
...
class GUI : public CAppDialog
{
    ...
    bool Create(const long chart, const string name, const int subwin, const int x1, const int y1, const int x2, const int y2)
    {
        ...
        if(!Create_label("Lots | Risk %", Col(1), Row(0), _high, _width, lb5)) return false;
        ...
        if(!Create_button("CLOSE ALL", Col(0), Row(6), _high, _width*2.14, bt3)) return false;
