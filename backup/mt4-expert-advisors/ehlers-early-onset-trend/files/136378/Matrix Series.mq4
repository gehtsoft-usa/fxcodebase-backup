...
public:
   ...
   {
      return up[period] > ob && up[period] > down[period];
   }
};
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      return down[period] < os && up[period] > down[period];
   }
};
...
double ys1[], rk5[], rk6[], up[], Value1[], ResistanceLine[], SupportLine[], down[];
int init()
{
   ...
   id = upCandles.RegisterStreams(id, Green);
   downCandles = new CandleStreams();
   id = downCandles.RegisterStreams(id, Red);
   SetIndexStyle(id, dynamic ? DRAW_LINE : DRAW_NONE, STYLE_SOLID, 1, Green);
   SetIndexBuffer(id, ResistanceLine);
   SetIndexLabel(id, "Resistance Line");
   SetIndexEmptyValue(id, EMPTY_VALUE);
   ++id;
   SetIndexStyle(id, dynamic ? DRAW_LINE : DRAW_NONE, STYLE_SOLID, 1, Red);
   SetIndexBuffer(id, SupportLine);
   SetIndexLabel(id, "Support Line");
   SetIndexEmptyValue(id, EMPTY_VALUE);
   ++id;
   ...
   SetIndexStyle(id, DRAW_NONE);
   SetIndexBuffer(id, ys1);
   ++id;
   SetIndexStyle(id, DRAW_NONE);
   SetIndexBuffer(id, rk5);
   ++id;
   SetIndexStyle(id, DRAW_NONE);
   SetIndexBuffer(id, rk6);
   ++id;
   SetIndexStyle(id, DRAW_NONE);
   SetIndexBuffer(id, up);
   ++id;
   SetIndexStyle(id, DRAW_NONE);
   SetIndexBuffer(id, Value1);
   ++id;
   SetIndexStyle(id, DRAW_NONE);
...
int start()
{
   ...
   if (counted_bars <= 0 || counted_bars > Bars)
   {
      upCandles.Init();
      downCandles.Init();
      ArrayInitialize(ys1, EMPTY_VALUE);
      ArrayInitialize(rk5, EMPTY_VALUE);
      ArrayInitialize(rk6, EMPTY_VALUE);
      ArrayInitialize(up, EMPTY_VALUE);
      ArrayInitialize(down, EMPTY_VALUE);
      ArrayInitialize(Value1, EMPTY_VALUE);
      ArrayInitialize(ResistanceLine, EMPTY_VALUE);
      ArrayInitialize(SupportLine, EMPTY_VALUE);
      if (customStream != NULL)
      {
         customStream.Init();
      }
   ...
   {
      ys1[pos] = (High[pos] + Low[pos] + Close[pos] * 2) / 4;
      double rk3 = iMAOnArray(ys1, 0, nn, 0, MODE_EMA, pos);
      double rk4 = StDev(ys1, nn, pos);
      rk5[pos] = (ys1[pos] - rk3) * 200 / rk4;
      rk6[pos] = iMAOnArray(rk5, 0, nn, 0, MODE_EMA, pos);
      up[pos] = iMAOnArray(rk6, 0, nn, 0, MODE_EMA, pos);
      down[pos] = iMAOnArray(up, 0, nn, 0, MODE_EMA, pos);
      double Oo = up[pos] < down[pos] ? up[pos] : down[pos];
      double Hh = Oo;
      double Ll = up[pos] < down[pos] ? down[pos] : up[pos];
      double Cc = Ll;
      upCandles.Clear(pos);
      downCandles.Clear(pos);
      if (Oo > Cc)
      {
         downCandles.Set(pos, Oo, Hh, Ll, Cc);
      }
      else
      {
         upCandles.Set(pos, Oo, Hh, Ll, Cc);
      }

      int Lookback = SupResPeriod;
      int PerCent = SupResPercentage;
      int Pds = PricePeriod;
      Value1[pos] = iCCI(_Symbol, _Period, Pds, PRICE_CLOSE, pos);
      int highestIndex = ArrayMaximum(Value1, Lookback, pos);
      double Value2 = Value1[highestIndex];
      int lowestIndex = ArrayMinimum(Value1, Lookback, pos);
      double Value3 = Value1[lowestIndex];
      double Value4 = Value2 - Value3;
      double Value5 = Value4 * (PerCent / 100.0);
      ResistanceLine[pos] = Value3 + Value5;
      SupportLine[pos] = Value2 - Value5;
      if (customStream != NULL)
      {
