//Available @  https://fxcodebase.com/code/viewtopic.php?f=38&t=75081
...
double Indi(int buffer, int candle)
{
    ...
    return iCustom(NULL, 0, file_custom_indicator, Inputs, buffer, candle);
}
#endif
...
input int uMaxTrades   = 1;   // Trades At Same Time:
input int GapBtwOrders = -20; // Gap Betwen Orders (pips):
#endif
...
bool OnInit_GUI()
{
    ...
    if (gui.reason() != REASON_CHARTCHANGE && gui.reason() != REASON_TEMPLATE && gui.reason() != REASON_PARAMETERS) {
        res = gui.Create(0, "Panel Grid", 0, 400, 0, 590, 300);
        if (res) gui.Run();
    }
    return res;
}
...
class BuyAction : public iActions
{
    bool doAction()
    {
        ...
        int    gap      = (int)gui.edit3.Text();
        int    steps    = (int)gui.editQnt.Text();

        do {
            SendNewOrder *SendOrder;

            if (n == 0) pr = distance == 0 ? 0 : Price("buy", distance, _Symbol);

            if (n > 0) {
                if (pr == 0) pr = Ask;
                pr = gap == 0 ? 0 : pr + (gap * 10 * Point());
            }
...
class SellAction : public iActions
{
    bool doAction()
    {
        ...
        do {
            ...
            if (n == 0) pr = distance == 0 ? 0 : Price("sell", distance, _Symbol);
...
class GUI : public CAppDialog
{
    ...
    iActions* buttonCloseLossAction;
    iActions* buttonCloseWinnersAction;
    iActions* buttonCloseAllAction;
    iActions* buttonDeletePendingsAction;
    ...
    ~GUI()
    {
        ...
        delete buttonCloseLossAction;
        delete buttonCloseWinnersAction;
        delete buttonCloseAllAction;
        delete buttonDeletePendingsAction;
    }

    CLabel  lb1, lb2, lb3, lb4, lb5, lb6, lb7, lb8, lb9;
    CButton bt1, bt2, bt3, btCloseLoss, btCloseWin, btCloseAll, btDeletePendings;
    CEdit   edit1, edit2, edit3;
    CEdit   editTPpips, editSLpips, editQnt;

    void setButton1Action(iActions* action) { button1Action = action; }
    void setButton2Action(iActions* action) { button2Action = action; }
    
    void setButtonCloseLossAction(iActions* action) { buttonCloseLossAction = action; }
    void setButtonCloseWinnersAction(iActions* action) { buttonCloseWinnersAction = action; }
    void setButtonCloseAllAction(iActions* action) { buttonCloseAllAction = action; }
    void setButtonDeletePendingsAction(iActions* action) { buttonDeletePendingsAction = action; }
    ...
    bool Create(const long chart, const string name, const int subwin, const int x1, const int y1, const int x2, const int y2)
    {
        ...
        if(!Create_button("BUY", Col(0), Row(0), _high, _width, bt1)) return false;
        if(!Create_button("SELL", Col(0), Row(1), _high, _width, bt2)) return false;
        if(!Create_Edit("Pips buy", Col(1), Row(0), _high, _width, edit1)) return false;
        if(!Create_Edit("Pips sell", Col(1), Row(1), _high, _width, edit2)) return false;
        if(!Create_Edit("Pips TP", Col(1), Row(2), _high, _width, editTPpips, (string)uTPpips)) return false;
        if(!Create_Edit("Pips SL", Col(1), Row(3), _high, _width, editSLpips, (string)uSLpips)) return false;
        if(!Create_label("TP pips", Col(0), Row(2), _high, _width, lb1)) return false;
        if(!Create_label("SL pips", Col(0), Row(3), _high, _width, lb2)) return false;
        
        if(!Create_label("Grid Steps", Col(0), Row(4), _high, _width, lb4)) return false;
        if(!Create_Edit("Steps", Col(1), Row(4), _high, _width, editQnt,(string)uMaxTrades)) return false;
        
        if(!Create_label("Gap betwen", Col(0), Row(5), _high, _width, lb3)) return false;
        if(!Create_Edit("Gap", Col(1), Row(5), _high, _width, edit3, (string)GapBtwOrders)) return false;
        
        if(!Create_button("Close Loss", Col(0), Row(6), _high, _width, btCloseLoss)) return false;
        if(!Create_button("Close Win", Col(1), Row(6), _high, _width, btCloseWin)) return false;
        if(!Create_button("Close All", Col(0), Row(7), _high, _width * 2.15, btCloseAll)) return false;
        if(!Create_button("Delete Pendings ", Col(0), Row(8), _high, _width * 2.15, btDeletePendings)) return false;
    ...
    void OnClick_buttonCloseLoss()      { buttonCloseLossAction.doAction(); }
    void OnClick_buttonCloseWinners()   { buttonCloseWinnersAction.doAction(); }
    void OnClick_buttonCloseAll()       { buttonCloseAllAction.doAction(); }
    void OnClick_buttonDeletePendings() { buttonDeletePendingsAction.doAction(); }
...
ON_EVENT(ON_CLICK, btCloseLoss, OnClick_buttonCloseLoss)
ON_EVENT(ON_CLICK, btCloseWin, OnClick_buttonCloseWinners)
ON_EVENT(ON_CLICK, btCloseAll, OnClick_buttonCloseAll)
ON_EVENT(ON_CLICK, btDeletePendings, OnClick_buttonDeletePendings)
...
gui.setButtonCloseLossAction(closeLossers = new ActionCloseByProfit("loss",magico,_Symbol));
gui.setButtonCloseWinnersAction(closeWinners = new ActionCloseByProfit("win",magico,_Symbol));
gui.setButtonCloseAllAction(closeAll = new ActionCloseByProfit("all",magico,_Symbol));
gui.setButtonDeletePendingsAction(deletePendings = new ActionCloseByProfit("all pendings",magico,_Symbol));
    ...
    bool doAction()
    {
        for (int i = OrdersTotal() - 1; i >= 0; i--) {
            if (OrderSelect(i, SELECT_BY_POS) && OrderSymbol() == _symbol && OrderMagicNumber() == _magic) {
                ...
                if (_profitState == "all pendings" && (_type != OP_BUY && _type != OP_SELL)) {
                    OrderDelete(OrderTicket());
                }
            }
        }
...
ActionCloseByProfit *deletePendings;
