...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=73915
...
enum TSLMode {
    byPips,             // By Pips
    byBars,             // By One Candle
    byBarsSameDirection // By Candle in Same Direction
...
input bool      uDeletePendingsOn = true;                            // Delete pendings after one bar:
#else
...
// input bool         notAllowInFirstAndLastCandleOfTheWeek = true; // Not Allow Open First and last bar of the week:
...
// NOTE: setup para tsl por una vela a la vez
string          tTslByCandles    = "-- TSL By High/Low Candles Back --"; // --
int             ByCandlesIniStep = 1;                                    // TSL Initial Step:
int             ByCandlesStep    = 1;                                    // TSL Step:
int             candlesBack      = 1;                                    // TSL Candles Back:
ENUM_TIMEFRAMES ByCandlesTf      = PERIOD_CURRENT;                       // TSL TF:
  ...
  public:
    SendNewOrder(string side, double lots, string symbol = "", double price = 0, double sl = 0, double tp = 0, int magic = 0, string coment = "", datetime expire = 0)
    {
        ...
        double _price  = setPrice(side, price, _symbol);
               _type   = SetType(side, price, _symbol);
...
class ByPipsFromCandle : public iLevels
{
    ...
    //   ByPipsFromCandle(_Symbol, side, userSLpips, "SL", 0, 1));
  public:
...
class TslByCandlesSameDirection : public iTSL
{
    int             _InitialStep;
    int             _TslStep;
    ENUM_TIMEFRAMES _Tf;
  ...
  public:
    TslByCandlesSameDirection(int InitialStep, int TslStep, ENUM_TIMEFRAMES tf)
    {
        _InitialStep = InitialStep;
        _TslStep     = TslStep;
        _Tf          = tf;
    ...
    double newSL(Order* order)
    {
        ...
        if (order.type() == OP_BUY) {
            ...
            double lowest = iLow(order.symbol(), _Tf, 1);

            double op = iOpen(order.symbol(), _Tf, 1);
            double cl = iClose(order.symbol(), _Tf, 1);
        ...
        if (order.type() == OP_SELL) {
            double highest = iHigh(order.symbol(), _Tf, 1);
  ...
  public:
    TrailingStop(OrdersList* uOrders, TSLMode mode)
    {
        ...
        case byBars:
            ...
            break;
        case byBarsSameDirection:
            _TslMode = new TslByCandlesSameDirection(ByCandlesIniStep, ByCandlesStep, ByCandlesTf);
            break;
    ...
    if (!uTradeReverse) {
        if (conditionsToBuy.EvaluateConditions()) {
            if (modeEntry == Market) {
                ...
                datetime expireTm = uDeletePendingsOn == true ? iTime(NULL, 0, 0)+_Period*60 : 0;
                
                actionSendOrder = new SendNewOrder("buy", Lots(), "", pr, SL("buy", pr), TP("buy", pr), magico, "", expireTm);
            }
        ...
        if (conditionsToSell.EvaluateConditions()) {
            if (modeEntry == Market) {
                ...
                datetime expireTm = uDeletePendingsOn == true ? iTime(NULL, 0, 0)+_Period*60 : 0;
                actionSendOrder = new SendNewOrder("sell", Lots(), "", pr, SL("sell", pr), TP("sell", pr), magico, "", expireTm);
            }
            if (actionSendOrder.doAction()) {
                mainOrders.GetMarketOrders();
...
double Price(string direction, int pips = 0, string _symbol = "")
{
    ...
    if (direction == "buy") {
        ...
        if (modeEntry == PendingStop) {
            ...
            if (cl > op) {
                // price = cl + distance;
                price = iHigh(symbol, 0, 1);
            } else {
                // price = op + distance;
                price = iLow(symbol, 0, 1);
            }
        }
...
double baseSL(string side, double price = 0)
{
    double result = 0;
    if (stopLossOn)
        switch (modeSL) {
...
double baseTP(string side, double price = 0)
{
    double result = 0;
    if (takeProfitOn)
        switch (modeTP) {
...
// bool checkFirstAndLasCandle()
// {
//     if (notAllowInFirstAndLastCandleOfTheWeek)
//     {
//         // buscar horario de cierre del mercado
//         // hora de apertura
//     }
