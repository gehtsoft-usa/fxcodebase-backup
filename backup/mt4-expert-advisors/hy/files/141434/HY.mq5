...
//https://fxcodebase.com/code/viewtopic.php?f=38&t=71071
...
public:
   ...
   {
      int pos = iBars(_symbol, _timeframe) - period - 1;
      return buffer1[pos] >= buffer3[pos];
   }
};
...
public:
   ...
   {
      int pos = iBars(_symbol, _timeframe) - period - 1;
      return buffer1[pos] <= buffer4[pos];
   }
};
...
void OnInit()
{
   ...
   rsi = iRSI(_Symbol, _Period, RsiLength, RsiPrice);
   IndicatorObjPrefix = GenerateIndicatorPrefix("HY");
   IndicatorSetString(INDICATOR_SHORTNAME, "HY");
   IndicatorSetInteger(INDICATOR_DIGITS, Digits());

   int id = 0;
   SetIndexBuffer(id, buffer1, INDICATOR_DATA);
   ++id;
   SetIndexBuffer(id, buffer2, INDICATOR_DATA);
   ++id;
   SetIndexBuffer(id, buffer3, INDICATOR_DATA);
   ++id;
   SetIndexBuffer(id, buffer4, INDICATOR_DATA);
   ++id;
   SetIndexBuffer(id, Arriba, INDICATOR_DATA);
   ...
   ++id;
   SetIndexBuffer(id, Abajo, INDICATOR_DATA);
   PlotIndexSetInteger(id, PLOT_DRAW_TYPE, DRAW_ARROW);
   ...
   mainSignaler.SetMessagePrefix(_Symbol + "/" + mainSignaler.GetTimeframeStr() + ": ");
   {
      ...
      id = alerts[size].RegisterStreams(id, 0, "Up", 217, up_color, customStream);
      id = alerts[size + 1].RegisterStreams(id, 0, "Down", 218, down_color, customStream);
   }
}
...
int OnCalculate(const int rates_total,
                ...
                const int &spread[])
{
   customStream.SetSize(rates_total);
   if (prev_calculated <= 0 || prev_calculated > rates_total)
   {
      ...
      ArrayInitialize(Arriba, EMPTY_VALUE);
      ArrayInitialize(Abajo, EMPTY_VALUE);
      for (int i = 0; i < ArraySize(alerts); ++i)
      {
         alerts[i].Init();
      }
   ...
   for (int pos = MathMax(rates_total - 1 - bars_limit, MathMax(first, prev_calculated - 1)); pos < rates_total; ++pos)
   {
      ...
      double rsiValue[1];
      if (CopyBuffer(rsi, 0, oldPos, 1, rsiValue) == 1)
      {
         buffer1[pos] = rsiValue[0];
      }
      customStream._data[pos] = buffer1[pos];
   }
   
   for (int pos = MathMax(rates_total - 1 - bars_limit, MathMax(first, prev_calculated - 1)); pos < rates_total; ++pos)
   {
      double dev = StDev(buffer1, DevPeriod, pos);
      double sum  = (HalfLength + 1) * buffer1[pos];
      ...
      for(int j = 1, k = HalfLength; j <= HalfLength; j++, k--)
      {
         sum  += k * buffer1[pos - j];
         sumw += k;
         if (j + pos < rates_total)
         {
            sum  += k * buffer1[pos + j];
            sumw += k;
         }
      }
      buffer2[pos] = sum / sumw;
      buffer3[pos] = buffer2[pos] + dev * Deviations;
      buffer4[pos] = buffer2[pos] - dev * Deviations;
      if (buffer1[pos] >= buffer3[pos])
      { 
         Arriba[pos] = high[pos];
      }
      if (buffer1[pos] <= buffer4[pos])
      {
         Abajo[pos] = low[pos];
      }
      int oldPos = rates_total - pos - 1;
