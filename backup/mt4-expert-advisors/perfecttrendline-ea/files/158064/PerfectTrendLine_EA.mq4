//Available @ https://fxcodebase.com/code/viewtopic.php?f=38&t=75563
...
input ENUM_MA_METHOD ma_type = MODE_SMA; // MA Smoothing method
input int ma_length = 7; // MA Length
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      double value = iCustom(_symbol, _timeframe, "PerfectTrendLine", Tlines, uSlowPer, uSlowPips, uFastPer, uFastPips, 4, period + 1);
      return value == 1;
   }
};
...
class LongFilterCondition : public ACondition
{
   EntryStreamData* data;
public:
   LongFilterCondition(const string symbol, ENUM_TIMEFRAMES timeframe, EntryStreamData* data)
      :ACondition(symbol, timeframe)
   {
      this.data = data;
      data.AddRef();
   ...
   bool IsPass(const int period, const datetime date)
   {
      double value = iMA(_symbol, _timeframe, ma_length, 0, ma_type, PRICE_CLOSE, period);
...
class ShortFilterCondition : public ACondition
{
   EntryStreamData* data;
public:
   ShortFilterCondition(const string symbol, ENUM_TIMEFRAMES timeframe, EntryStreamData* data)
      :ACondition(symbol, timeframe)
   {
      this.data = data;
      data.AddRef();
   }
   ~ShortFilterCondition()
   {
      data.Release();
   }
...
ICondition* CreateLongFilterCondition(string symbol, ENUM_TIMEFRAMES timeframe, EntryStreamData* data)
{
   ...
   AndCondition* condition = new AndCondition();
   condition.Add(new LongFilterCondition(symbol, timeframe, data), false);
   return condition;
}
...
ICondition* CreateShortFilterCondition(string symbol, ENUM_TIMEFRAMES timeframe, EntryStreamData* data)
{
   ...
   AndCondition* condition = new AndCondition();
   condition.Add(new ShortFilterCondition(symbol, timeframe, data), false);
   return condition;
}
...
TradingController *CreateController(const string symbol, const ENUM_TIMEFRAMES timeframe, string algoId, string &error)
{
   ...
   switch (logic_direction)
   {
      case DirectLogic:
         ...
         break;
      case ReversalLogic:
         shortFilterCondition.Add(CreateLongFilterCondition(symbol, timeframe, data), false);
         longFilterCondition.Add(CreateShortFilterCondition(symbol, timeframe, data), false);
...
int OnInit()
{
   ...
   double temp = iCustom(NULL, 0, "PerfectTrendLine", 0, 0);
   if (GetLastError() == ERR_INDICATOR_CANNOT_LOAD)
   {
      Alert("Please, install the 'PerfectTrendLine' indicator");
      return INIT_FAILED;
   }
