...
// http://fxcodebase.com/code/viewtopic.php?f=38&t=70771
...
public:
   ...
   {
		double a = iIchimoku(_symbol, _timeframe, tenkan_sen, kijun_sen, senkou_span_b, MODE_SENKOUSPANA, period - kijun_sen);
		double b = iIchimoku(_symbol, _timeframe, tenkan_sen, kijun_sen, senkou_span_b, MODE_SENKOUSPANB, period - kijun_sen);
      return a > b;
   }
};
...
class OverlapCondition : public ACondition
{
public:
   OverlapCondition(const string symbol, ENUM_TIMEFRAMES timeframe)
      :ACondition(symbol, timeframe)
   {
   }
...
class DynTextFormatter : public AValueFormatter
{
   color _clr;
   color _bgClr;
   OverlapCondition* _conditions[];
public:
   DynTextFormatter(color clr, color bgClr)
   {
      _bgClr = bgClr;
      _clr = clr;
   }
   ...
   void Add(OverlapCondition *condition, bool addRef)
   {
      int size = ArraySize(_conditions);
      ArrayResize(_conditions, size + 1);
      _conditions[size] = condition;
   ...
   virtual string FormatItem(const int period, const datetime date, color& clr, color& bgColor)
   {
      ...
      for (int i = 0; i < ArraySize(_conditions); ++i)
      {
         OverlapCondition* item = _conditions[i];
         if (item.IsPass(period, date))
         {
            ++count;
         }
      }
      if (count > 1)
      {
         return "RC" + IntegerToString(count) + " A & B";
      }
      return "RC A & B";
   }
};

class DynSignalTextFormatter : public AValueFormatter
{
   color _clr;
   color _bgClr;
   OverlapCondition* _conditions[];
public:
   DynSignalTextFormatter(color clr, color bgClr)
   {
      _bgClr = bgClr;
      _clr = clr;
   }
   ~DynSignalTextFormatter()
   {
      int size = ArraySize(_conditions);
      for (int i = 0; i < size; ++i)
      {
   ...
   virtual string FormatItem(const int period, const datetime date, color& clr, color& bgColor)
   {
      ...
      for (int i = 0; i < ArraySize(_conditions); ++i)
      {
         ...
         {
            ++count;
            message = message + ", " + TimeframeToString(item.GetTimeframe());
         }
      }
      return message + ": RC" + IntegerToString(count) + " in SPAN A & SPAN B";
   }
};

void addNeutral(OrCondition* or, DynTextFormatter* formatter, DynSignalTextFormatter* signal, OverlapCondition* condition)
{
   or.Add(condition, false);
   formatter.Add(condition, true);
   signal.Add(condition, true);
}
...
public:
   ...
   virtual ICell* Create(const string id, const int x, const int y, ENUM_BASE_CORNER corner, const string symbol, const ENUM_TIMEFRAMES timeframe, bool showHistorical)
   {
      ...
      {
         DynTextFormatter* value = new DynTextFormatter(GetTextColor(neutral_color), GetBackgroundColor(neutral_color));
         DynSignalTextFormatter* signal = new DynSignalTextFormatter(GetTextColor(neutral_color), GetBackgroundColor(neutral_color));
         ICondition* or = new OrCondition();
         if (Include_M1)
         {
            addNeutral(or, value, signal, new OverlapCondition(symbol, PERIOD_M1));
         }
         if (Include_M5)
         {
            addNeutral(or, value, signal, new OverlapCondition(symbol, PERIOD_M5));
         }
         if (Include_M15)
         {
            addNeutral(or, value, signal, new OverlapCondition(symbol, PERIOD_M15));
         }
         if (Include_M30)
         {
            addNeutral(or, value, signal, new OverlapCondition(symbol, PERIOD_M30));
         }
         if (Include_H1)
         {
            addNeutral(or, value, signal, new OverlapCondition(symbol, PERIOD_H1));
         }
         if (Include_H4)
         {
            addNeutral(or, value, signal, new OverlapCondition(symbol, PERIOD_H4));
         }
         if (Include_D1)
         {
            addNeutral(or, value, signal, new OverlapCondition(symbol, PERIOD_D1));
         }
         if (Include_W1)
         {
            addNeutral(or, value, signal, new OverlapCondition(symbol, PERIOD_W1));
         }
         if (Include_MN1)
         {
            addNeutral(or, value, signal, new OverlapCondition(symbol, PERIOD_MN1));
         }
         
         TrendValueCell* cell = new TrendValueCell(id, x, y, corner, symbol, timeframe, _alertShift, defaultValue, output_mode);
         defaultValue.Release();

         cell.AddCondition(or, value, NULL, signal);
         or.Release();
         value.Release();
...
int init()
{
   ...
   builder.AddTimeframe("Overlap", PERIOD_CURRENT);
