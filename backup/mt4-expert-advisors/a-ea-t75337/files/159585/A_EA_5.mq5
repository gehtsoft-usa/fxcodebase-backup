...
// https://fxcodebase.com/code/viewtopic.php?f=38&p=159334#p159334
    ...
    bool execute()
    {
        ...
        for (int i = PositionsTotal() - 1; i >= 0; i--) {
            ...
            if (PositionGetString(POSITION_COMMENT) == "") {
                ...
                if (grid_close_profit > 0 && current_profit >= grid_close_profit) closeGrid(tk);
...
int  handle_rsi = 0;
void setHandleRSI() { handle_rsi = iRSI(Symbol(), PERIOD_M15, rsi_period, PRICE_CLOSE); }
...
class BUYcondition1 : public iConditions
{
  public:
    bool evaluate()
    {
        // Condición de compra: cierre anterior < cierre actual y RSI < 36
        double prevClose = iClose(_Symbol, PERIOD_CURRENT, 2);
        double currClose = iClose(_Symbol, PERIOD_CURRENT, 1);
        // double rsi = iRSI(_Symbol, PERIOD_M15, 5, PRICE_CLOSE, 1);
        double rsi = RSI(1);
        
        return (prevClose < currClose && rsi < 36.0);
    }
};
...
class SELLcondition1 : public iConditions
{
  public:
    bool evaluate()
    {
        // Condición de venta: cierre anterior > cierre actual y RSI > 56
        double prevClose = iClose(_Symbol, PERIOD_CURRENT, 2);
        double currClose = iClose(_Symbol, PERIOD_CURRENT, 1);
        // double rsi = iRSI(_Symbol, PERIOD_M15, 5, PRICE_CLOSE, 1);
        double rsi = RSI(1);

        return (prevClose > currClose && rsi > 56.0);
    }
};
