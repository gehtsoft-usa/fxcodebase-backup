...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=74529
// ------------------------------------------------------------------
//+------------------------------------------------------------------------------------------------+
...
input int GapBtwOrders = 0; // Gap Betwen Orders (pips):
...
double userRisk = userEquityPer;
// double             userMoney               = 10;                 // Setup Lots by "Money":
input double             userBalancePer = 1;                // Setup Lots by "Account 
input double uLotMultiplier = 2; // Multiplier
double m = uLotMultiplier;
...
#endif
#ifdef BREAKEVEN_ON
string Tbk = "== TPModification Setup ==";  // ————————————
bool   TPModifyOn = false;                    // Breakeven On:
double userBkvPips = 10;                        // Breakeven Pips
double userBkvStep = 3;                        // Breakeven Step
#else
string        Tbk = "== TPModification Setup ==";    // == Breakeven Setup ==
bool          TPModifyOn = true;                      // Use Breakeven?
double        userBkvPips = 10;                        // Breakeven Pips
double        userBkvStep = 3;                        // Breakeven Step
#endif
    ...
    bool GetLastMarketOrder()
    {
        for(int i = OrdersTotal() - 1; i >= 0; i--)
        {
            if(OrderSelect(i, SELECT_BY_POS, MODE_TRADES))
            {
                ...
                if(AddOrder(newOrder))
                {
                    Print(__FUNCTION__, " ", "* New Market Order * ", id(i), "magic: ", magic(i));
                    // PrintOrder(i);
                    return true;
                }
            }
...
bool OnInit_GUI()
{
    ...
    {
        res = gui.Create(0, "Pending Orders EA", 10, 150, 10, 350, 300);
        if(res) gui.Run();
    }
    return res;
}
...
class BuyAction : public iActions
{
    bool doAction()
    {
        ...
        while(n < userNoTrades)
        {
            ...
            SendOrder = new SendNewOrder("buy", Lots(n), "", pr, SL("buy", pr), tp, magico);
...
class SellAction : public iActions
{
    bool doAction()
    {
        ...
        {
            SendNewOrder* SendOrder;
            SendOrder = new SendNewOrder("sell", Lots(n), "", pr, SL("sell", pr), tp, magico);
            result = SendOrder.doAction();
            n++;
            delete SendOrder;
        }
...
class SendBothAction : public iActions
{
    bool doAction()
    {
        bool result = false;
        ...
        if(gap > 0){
            distance = gap;
            pricebuy = Price("buy", distance, _Symbol);
            pricesell = Price("sell", -distance, _Symbol);
        }
        tpsell = TP("sell", pricesell);
        tpbuy = TP("buy", pricebuy);
        ...
        while(n < userNoTrades)
        {
            if(sw == 0){
                SendNewOrder* SendOrder;
                SendOrder = new SendNewOrder("sell", Lots(n), "", pricesell, SL("sell", pricesell), tpsell, magico);
                result = SendOrder.doAction();
                delete SendOrder;
                n++;
                sw = 1;
            ...
            {
                SendNewOrder* SendOrder;
                SendOrder = new SendNewOrder("buy", Lots(n), "", pricebuy, SL("buy", pricebuy), tpbuy, magico);
                result = SendOrder.doAction();
                delete SendOrder;
                n++;
                sw = 0;
...
SendBothAction btnSendOrdersAction();

class CloseAction : public iActions
{
    //   string _symbol;
    //   int    _magic;
    //   string _side;
...
CloseAction btnCloseAction();

class TrailingStopAction : public iActions
{
    bool doAction()
    {
        TslON;
...
TrailingStopAction btnTrailingAction();

// clang-format off
class TPBreakEvenAction : public iActions
{
    bool doAction()
    {
        TPModifyOn = !TPModifyOn;

        Print(__FUNCTION__, " TPModifyOn: ", TPModifyOn);
        //  doBreackevenAction();
       //@btnBuyAction();
       //@btnSellAction();
        return true;
...
TPBreakEvenAction btnTPBrKevenAction();
...
class GUI : public CAppDialog
{
    ...
    int _x, _y;
    int _gapV, _gapH;
    int _reason; // I'm going to use it for when the EA resets
    ...
    CLabel  lb1, lb2, lb3, lb4, lb5, lb6, lb7, lb8, lb9, lb10;
    CButton bt1, bt2, bt3, bt4, bt5, btnBoth;
    CEdit   edit1, edit2, edit3, editm;
    ...
    bool Create(const long chart, const string name, const int subwin, const int x1, const int y1, const int x2, const int y2)
    {
        ...
        if(!Create_label("Lots | Risk %", Col(0), Row(1), _high, _width, lb5)) return false;
        if(!Create_label("Multiplier", Col(0), Row(2), _high, _width, lb10)) return false;

        if(!Create_Edit("No of Orders", Col(1), Row(0), _high, _width, editNoOrders, (string) uMaxTrades)) return false;
        string txtToLotSize = modeCalcLots == FixLots ? (string) userLotSize : (string) userRisk;
        if(!Create_Edit("Lot size", Col(1), Row(1), _high, _width, editLots, txtToLotSize)) return false;
        if(!Create_Edit("editM", Col(1), Row(2), _high, _width, editm, (string) m)) return false;


        //if(!Create_Edit("Pips buy", Col(1), Row(1), _high, _width, edit1)) return false;
        //if(!Create_Edit("Pips sell", Col(1), Row(2), _high, _width, edit2)) return false;
        if(!Create_Edit("Pips TP", Col(1), Row(3), _high, _width, editTPpips, (string) uTPpips)) return false;
        if(!Create_Edit("Pips SL", Col(1), Row(4), _high, _width, editSLpips, (string) uSLpips)) return false;
        if(!Create_label("TP pips", Col(0), Row(3), _high, _width, lb1)) return false;
        if(!Create_label("SL pips", Col(0), Row(4), _high, _width, lb2)) return false;
        if(!Create_label("Gap ", Col(0), Row(5), _high, _width, lb3)) return false;
        if(!Create_Edit("Gap", Col(1), Row(5), _high, _width, editGappips, (string) GapBtwOrders)) return false;

        //if(!Create_button("Trailing STOP ON", Col(0), Row(7), _high, _width*2, bt4)) return false;
        // if(!Create_button("All Order Change TP ", Col(0), Row(8), _high, _width*2.14, bt5)) return false;

        if(!Create_button("BUY", Col(0), Row(7), _high, _width, bt1)) return false;
        if(!Create_button("SELL", Col(1), Row(7), _high, _width, bt2)) return false;

        if(!Create_button("All Orders", Col(0), Row(8), _high, _width * 2.14, btnBoth)) return false;
        if(!Create_button("CLOSE ALL", Col(0), Row(9), _high, _width * 2.14, bt3)) return false;
    ...
    {
        if(bt1.IsActive()) bt1.ColorBackground(Lime); else bt1.ColorBackground(CONTROLS_BUTTON_COLOR_BG);
        if(bt2.IsActive()) bt2.ColorBackground(Red); else bt2.ColorBackground(CONTROLS_BUTTON_COLOR_BG);
        ...
        if(btnBoth.IsActive()) btnBoth.ColorBackground(RoyalBlue); else btnBoth.ColorBackground(CONTROLS_BUTTON_COLOR_BG);
    }

    protected:
    void OnEndEdit_edit1() { Print("edit 1 ok"); }
    void OnEndEdit_edit2() { Print("edit 2 ok"); }
    void OnEndEdit_editLots()
    {
        if(modeCalcLots == FixLots){
            userLotSize = (double) editLots.Text(); Print(userLotSize);
        }

        if(modeCalcLots == AccountPercent){
            userRisk = (double) editLots.Text(); Print(userRisk);
        }
    }
    void OnEndEdit_editNoOrders() { userNoTrades = (int) editNoOrders.Text(); Print(userNoTrades); }
    void OnEndEdit_editTPpips() { userTPpips = (int) editTPpips.Text(); Print(userTPpips); }
    void OnEndEdit_editSLpips() { userSLpips = (int) editSLpips.Text(); Print(userSLpips); }
    void OnEndEdit_editGappips() { userGappips = (int) editGappips.Text(); Print(userGappips); }
    ...
    void OnClick_button5()
    {
        ...
        if(TPModifyOn)
            bt5.ColorBackground(RoyalBlue);
        else
            bt5.ColorBackground(CONTROLS_BUTTON_COLOR_BG);
    }
...
//Event Map (MACRO Substitutions)
EVENT_MAP_BEGIN(GUI)
ON_EVENT(ON_END_EDIT, edit1, OnEndEdit_edit1)
ON_EVENT(ON_END_EDIT, edit2, OnEndEdit_edit2)
ON_EVENT(ON_END_EDIT, editLots, OnEndEdit_editLots)
ON_EVENT(ON_END_EDIT, editNoOrders, OnEndEdit_editNoOrders)
ON_EVENT(ON_END_EDIT, editTPpips, OnEndEdit_editTPpips)
ON_EVENT(ON_END_EDIT, editSLpips, OnEndEdit_editSLpips)
ON_EVENT(ON_END_EDIT, editGappips, OnEndEdit_editGappips)
...
ON_EVENT(ON_CLICK, btnBoth, OnClick_button6)
...
class MoveSL : public iActions
{
    ...
    bool doAction()
    {
        ...
        {
            Print(__FUNCTION__, " ", "Can't Move takeprofit");
            return false;
        }
        if(OrderSelect(_order.id(), SELECT_BY_TICKET))
        {
            ...
            if(OrderModify(_order.id(), OrderOpenPrice(), OrderStopLoss(), _newSL, OrderExpiration(), clrNONE))
            {
                _order.tp(_newSL);
...
class BreackevenCondition : public iConditions
{
    ...
    bool evaluate()
    {
        // If the current price matches the time of making BK Ret True
...
int OnInit()
{
    ...
    gui.setButton3Action(&btnCloseAction);
    gui.setButton4Action(&btnTrailingAction);
    gui.setButton5Action(&btnTPBrKevenAction);
    gui.setButton6Action(&btnSendOrdersAction);
...
void OnTick()
{
    ...
    // ——————————————————————————————————————————————————————————————————
    if(TPModifyOn) doBreackevenAction();
    if(TslON) tsl.doTSL();
...
double Lots(int n = 1)
{
    ...
    switch(modeCalcLots)
    {
        ...
        case AccountPercent:
            lots = lotProvider.LotsByBalancePercent(userRisk, userSLpips);
            break;
...
void handleTP(string side, double newTP)
{
    for(int i = OrdersTotal() - 1;i >= 0;i--)
    {
        if(OrderSelect(i, SELECT_BY_POS) && OrderSymbol() == _Symbol && OrderMagicNumber() == magico)
