...
// http://fxcodebase.com/code/viewtopic.php?f=38&t=68541
...
input int BoxSize = 2; // Size of price box in pips
...
int init()
{
   IndicatorName = GenerateIndicatorName("marketP4");
   ...
   for (int pos = 100; pos >= 0; --pos)
   {
      ...
      for (int j = finish; j <= start; ++j)
      {
         ...
         for (double price = min; price <= max; price += BOX)
         {
            ...
            if ((price >= Low[j] && price + BOX <= High[j]) || 
               (price <= Low[j] && price + BOX >= Low[j]) ||
               (price <= High[j] && price + BOX >= High[j]))
            {
