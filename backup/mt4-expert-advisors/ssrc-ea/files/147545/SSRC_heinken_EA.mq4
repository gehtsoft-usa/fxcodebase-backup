...
class BUYcondition1 : public iConditions
{
  public:
   bool evaluate()
   {
      ...
      // if (ssrcSlow.calculate(0, 1) > -majorTrendLevel && ssrcSlow.calculate(0, 5) <= -majorTrendLevel)
      if (ssrcSlow.calculate(0, 1) > majorTrendLevel) // && ssrcSlow.calculate(0, 5) <= -majorTrendLevel)
      {
         return true;
      }
...
class BUYcondition2 : public iConditions
{
  public:
   bool evaluate()
   {
      ...
      if (ssrcFast.calculate(0, 1) > minorTrendLevel && ssrcFast.calculate(0, 5) <= minorTrendLevel)
      {
         return true;
      }
      return false;
  ...
  public:
   bool evaluate()
   {
      double open= iCustom(Symbol(),Period(),"Heiken Ashi.ex4",2,0);
		Print(__FUNCTION__," ","open"," ",open);
      double close = iCustom(Symbol(),Period(),"Heiken Ashi.ex4",3,0);
		Print(__FUNCTION__," ","close"," ",close);
		if(open < close)
		{
			return true;
		}
