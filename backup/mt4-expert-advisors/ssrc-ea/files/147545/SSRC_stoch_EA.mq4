...
string           Iema                    = "== Stochastic Setup ==";  // == Stochastic Setup ==
...
double           uStochBuyLevel          = 20;                        // Stoch Level to Buy:
double           uStochSellLevel         = 80;                        // Stoch Level to Sell:
input string           ICustom                 = "== SSRC Setup ==";        // == SSRC Setup ==
input int              TotalBars               = 700;
input int              SnakeRange              = 14;
input int              FilterPeriod            = 21;
input double           MartFiltr               = 2;
input int              PriceConst              = 6;
input double           uSsrcLevelBuy           = 1;                          // SSRC Level To Buy:
input double           uSsrcLevelSell          = -1;                         // SSRC Level To Sell:
...
input bool             closeOpositeSSRC        = false;                      // Close with oposite SSRC
...
class BUYcondition1 : public iConditions
{
  public:
   bool evaluate()
   {
      ...
      if (ssrc.calculate(0, 1) >= uSsrcLevelBuy)
      {
         return true;
      }
      return false;
...
class BUYcondition2 : public iConditions
{
  public:
   bool evaluate()
   {
      ...
      if (stoch.main(2) < uStochBuyLevel && stoch.main(1) > uStochBuyLevel)
      {
         return true;
      }
      return false;
...
class SELLcondition1 : public iConditions
{
  public:
   bool evaluate()
   {
      ...
      if (ssrc.calculate(0, 1) <= uSsrcLevelSell)
      {
         return true;
      }
      return false;
...
class SELLcondition2 : public iConditions
{
  public:
   bool evaluate()
   {
      ...
      if (stoch.main(2) > uStochSellLevel && stoch.main(1) < uStochSellLevel)
      {
         return true;
      }
      return false;
...
class ConditionToCloseBuy : public iConditions
{
  public:
   bool evaluate()
   {
      if(closeOpositeSSRC)
      {
         if (ssrc.calculate(0, 1) <= uSsrcLevelSell) return true;
      }
...
int OnInit()
{
  ...
  //  conditionsToSell.AddCondition(sellCondition2 = new SELLcondition2());
   ...
   ssrc.setSetup(TotalBars, SnakeRange, FilterPeriod, MartFiltr, PriceConst);
