...
// http://fxcodebase.com/code/viewtopic.php?f=38&t=68945
...
public:
   ...
   bool IsPass(const int period)
   {
      for (int i = 1; i <= 3; ++i)
      {
         double SENKOUSPANA = iIchimoku(_symbol, _timeframe, Tenkan_Sen_Period, Kijun_Sen_Period, Senkou_Span_B_Period, 3, period + i);
         double SENKOUSPANB = iIchimoku(_symbol, _timeframe, Tenkan_Sen_Period, Kijun_Sen_Period, Senkou_Span_B_Period, 4, period + i);
         double topKumo = MathMax(SENKOUSPANA, SENKOUSPANB);
         if (iLow(_symbol, _timeframe, period + i) <= topKumo)
            return false;
         if (iOpen(_symbol, _timeframe, period + i) >= iClose(_symbol, _timeframe, period + i))
            return false;
      }
      return iOpen(_symbol, _timeframe, period + 2) >= iClose(_symbol, _timeframe, period + 3);
   }
};
...
public:
   ...
   bool IsPass(const int period)
   {
      for (int i = 1; i <= 3; ++i)
      {
         double SENKOUSPANA = iIchimoku(_symbol, _timeframe, Tenkan_Sen_Period, Kijun_Sen_Period, Senkou_Span_B_Period, 3, period + i);
         double SENKOUSPANB = iIchimoku(_symbol, _timeframe, Tenkan_Sen_Period, Kijun_Sen_Period, Senkou_Span_B_Period, 4, period + i);
         double bottomKumo = MathMin(SENKOUSPANA, SENKOUSPANB);
         if (iHigh(_symbol, _timeframe, period + i) >= bottomKumo)
            return false;
         if (iOpen(_symbol, _timeframe, period + i) <= iClose(_symbol, _timeframe, period + i))
            return false;
      }
