...
public:
   ...
   {
      double value = iCustom(_symbol, _timeframe, "ICH With Alert Derekers", Tenkan_Sen_Period, Kijun_Sen_Period, Senkou_Span_B_Period, 0, period);
      Print(value);
...
public:
   ...
   bool IsPass(const int period)
   {
      double value = iCustom(_symbol, _timeframe, "ICH With Alert Derekers", Tenkan_Sen_Period, Kijun_Sen_Period, Senkou_Span_B_Period, 1, period);
      return value != EMPTY_VALUE;
   }
};
...
int OnInit()
{
   double temp = iCustom(NULL, 0, "ICH With Alert Derekers", 0, 0);
   if (GetLastError() == ERR_INDICATOR_CANNOT_LOAD)
   {
      Alert("Please, install the 'ICH With Alert Derekers' indicator");
      return INIT_FAILED;
   }
#ifdef SHOW_ACCOUNT_STAT
   stats = NULL;
