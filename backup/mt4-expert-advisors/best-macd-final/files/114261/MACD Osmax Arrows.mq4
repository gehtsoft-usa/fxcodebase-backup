...
// http://fxcodebase.com/code/viewtopic.php?f=38&t=65000
//+------------------------------------------------------------------+
...
double BuffLine[], BuffSignal[], BuffMACD[], BuffUP[], BuffDN[], BuffNE[];
...
int CreateAlert(int id, ENUM_TIMEFRAMES tf, color upColor, color downColor)
{
   ...
   id = CreateAlert(id, upCondition, upAction, 217, "Over Signal " + TimeframeToString(tf), upColor, PriceLow, -1);
   upCondition.Release();
   if (upAction != NULL)
   {
      upAction.Release();
   ...
   id = CreateAlert(id, downCondition, downAction, 218, "Under Signal " + TimeframeToString(tf), downColor, PriceHigh, 1);
...
int CreateAlert2(int id, ENUM_TIMEFRAMES tf, color upColor, color downColor)
{
   ...
   id = CreateAlert(id, upCondition, upAction, 217, "Over 0 " + TimeframeToString(tf), upColor, PriceLow, -1);
   upCondition.Release();
   if (upAction != NULL)
   {
      upAction.Release();
   ...
   id = CreateAlert(id, downCondition, downAction, 218, "Under 0 " + TimeframeToString(tf), downColor, PriceHigh, 1);
   downCondition.Release();
   if (downAction != NULL)
   {
      downAction.Release();
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      return BuffLine[period] > BuffSignal[period];
   }
};
...
int init()
{
   ...
   IndicatorBuffers(8);
   IndicatorObjPrefix = GenerateIndicatorPrefix("macd_osmax");
   IndicatorShortName("macd_osmax");
   ...
   SetIndexBuffer(1, BuffSignal);
   SetIndexStyle(2, DRAW_LINE);
   SetIndexBuffer(2, BuffMACD);
   SetIndexStyle(3, DRAW_HISTOGRAM);
   SetIndexBuffer(3, BuffUP);
   SetIndexStyle(4, DRAW_HISTOGRAM);
   SetIndexBuffer(4, BuffDN);
   SetIndexStyle(5, DRAW_HISTOGRAM);
   SetIndexBuffer(5, BuffNE);
...
int start()
{
   ...
   while (pos >= 0)
   {
      FMA = iMA(NULL, 0, FastEMA, 0, MODE_EMA, Price, pos);
      SMA = iMA(NULL, 0, SlowEMA, 0, MODE_EMA, Price, pos);

      BuffMACD[pos] = FMA - SMA;
   ...
   while (pos >= 0)
   {
      BuffSignal[pos] = iMAOnArray(BuffMACD, 0, SignalEMA, 0, MODE_EMA, pos);

      BuffLine[pos] = (BuffMACD[pos] - BuffSignal[pos]) * OsmaX;

      BuffUP[pos] = 0.;
      BuffDN[pos] = 0.;
      BuffNE[pos] = 0.;

      if (BuffMACD[pos] > BuffMACD[pos + 1])
      {
         if (BuffLine[pos] > BuffLine[pos + 1])
         {
            BuffUP[pos] = BuffLine[pos];
         }
         else
         {
            BuffNE[pos] = BuffLine[pos];
         }
      }
      else
      {
   ...
   for (int pos = limit; pos >= 0 && !IsStopped(); --pos)
   {
      if (customStream != NULL)
      {
         customStream.SetValue(pos, BuffLine[pos]);
      }
