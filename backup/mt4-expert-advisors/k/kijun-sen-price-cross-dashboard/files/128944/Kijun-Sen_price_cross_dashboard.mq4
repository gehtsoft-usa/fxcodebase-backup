...
// http://fxcodebase.com/code/viewtopic.php?f=38&t=68973
...
public:
   ...
   {
      double value = iCustom(_symbol, _timeframe, "Kijun-Sen price cross alerts 2.01", Kijun, 3, period);
      double value1 = iCustom(_symbol, _timeframe, "Kijun-Sen price cross alerts 2.01", Kijun, 3, period + 1);
      return value == 1 && value1 != 1;
   }
};
...
int init()
{
   double temp = iCustom(NULL, 0, "Kijun-Sen price cross alerts 2.01", 0, 0);
   if (GetLastError() == ERR_INDICATOR_CANNOT_LOAD)
   {
      Alert("Please, install the 'Kijun-Sen price cross alerts 2.01' indicator");
      return INIT_FAILED;
   }
   if (!IsDllsAllowed() && advanced_alert)
   {
