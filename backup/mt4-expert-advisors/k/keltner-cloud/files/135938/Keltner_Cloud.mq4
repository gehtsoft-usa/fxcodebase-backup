...
// http://fxcodebase.com/code/viewtopic.php?f=38&t=70173
...
input int SRC = 0;  // The center line source
                     // 0 - Close
                     // 1 - Open
                     // 2 - High
                     // 3 - Low
...
input int Method = 0; // The center line smoothing method
                       // 0 - Simple
                       // 1 - Exponential
                       // 2 - Smoothed
                       // 3 - Linear weighted
...
public:
   ...
   {
      return iClose(_symbol, _timeframe, period) <= LowerCloudBuff[period] && iClose(_symbol, _timeframe, period + 1) > LowerCloudBuff[period + 1];
   }
};
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      return iClose(_symbol, _timeframe, period) >= UpperCloudBuff[period] && iClose(_symbol, _timeframe, period + 1) < UpperCloudBuff[period + 1];
   }
};
...
int init()
{
   ...
   IndicatorBuffers(8);
   IndicatorObjPrefix = GenerateIndicatorPrefix("kc");
