...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=75980
// +------------------------------------------------------------------------------------------------+
...
// input double    pips_btw_orders = 10;              // Pips between orders:
...
// MARK: funcion Price
double Price(string side, double pips = 0, string _symbol = "", ModeEntry modeEntry = Market)
{
...
double StopLoss(const double price, const string side, const string symbol, const double lot, ModeEntry modeEntry = Market)
{
...
double TakeProfit(const double price, const string side, const string symbol, const double lot, ModeEntry modeEntry = Market) 
{
...
input bool   tm_close_all   = false;             // Mandatory Close all at end Time?
#endif
class Session
{
...
#define Section_Spread
#ifdef Section_Spread
// Mark: Section_Spread

input string tspread_filter   = "== Spread Filter Setup =="; // ————————————————————————
input bool   spread_filter_on = false;                      // Spread Filter On?
input int    max_spread       = 100;                        // Max Spread (points):

class ConditionSpread : public iConditions
{
  public:
    bool evaluate()
    {
        int _spread = (int)MarketInfo(_Symbol, MODE_SPREAD);
        return _spread <= max_spread;
    }
};
ConditionSpread cdSpread;


void OnInit_Spread()
{
    if (!spread_filter_on) return;
    ShortStrategy.addCondition(&cdSpread);
    LongStrategy.addCondition(&cdSpread);
}
...
class OrderCreator : public iActions
{
    ...
    OrderCreator(const string Symbol, const string Side, const int Magic, ModeEntry imodeEntry = Market)
    {
        _symbol = Symbol;
        _side   = Side;
        _magic  = Magic;
        modeEntry = imodeEntry;
    }
    ~OrderCreator() { ; }
    

    void setPrice() { _price = ::Price(_side, uEntryDistance, _symbol, modeEntry); }

    void setLots() { _lots = ::LotsCalculation(); }

    void setStopLoss() { _sl = ::StopLoss(_price, _side, _symbol, _lots, modeEntry); }

    void setTakeProfit() { _tp = ::TakeProfit(_price, _side, _symbol, _lots, modeEntry); }
    ...
    bool execute()
    {
        ...
        ObjectDelete("Line"); // delete the line if exists

        return result > 0;
    }
};

OrderCreator buy(_Symbol, "buy", magico, Market);
OrderCreator sell(_Symbol, "sell", magico, Market);
OrderCreator buyPending(_Symbol, "buy", magico, PendingStop);
OrderCreator sellPending(_Symbol, "sell", magico, PendingStop);

#endif

#define Section_Delete_Pending_Orders
#ifdef Section_Delete_Pending_Orders
// MARK: Section Delete Pending Orders

class ConditionDeletePendings : public iConditions
{
  public:
    bool evaluate()
    {
        ...
        for (int i = OrdersTotal() - 1; i >= 0; i--) {
            if (OrderSelect(i, SELECT_BY_POS) && OrderSymbol() == _Symbol && OrderMagicNumber() == magico) {
                if (OrderType() == OP_BUY || OrderType() == OP_SELL) 
                {
                    marketOrder = true; // hay una orden abierta
                    continue;
                }
                if (OrderType() == OP_BUYSTOP || OrderType() == OP_SELLSTOP)
                {
                    pendingOrder = true; // hay una orden pendiente
                    continue;
                }
            }
        }
        return marketOrder && pendingOrder; // si hay una orden abierta y una pendiente, entonces se ejecuta la acción
    }
};
ConditionDeletePendings cdHaveOpenTrade;
...
Strategy stDeletePendings;

void OnInit_Delete_Pending_Orders()
{
    stDeletePendings.set(&cdHaveOpenTrade, &acDeletePendings);
    Trader.management.add(&stDeletePendings);
}
...
class BUYcondition1 : public iConditions
{
  public:
    bool evaluate()
    {
        ...
        for (int i = OrdersTotal() - 1; i >= 0; i--) {
            if (OrderSelect(i, SELECT_BY_POS) && OrderSymbol() == _Symbol && OrderMagicNumber() == magico && (OrderType() == OP_SELL || OrderType() == OP_BUY)) {
                return false; 
            }
        }
        return true;
...
class SELLcondition1 : public iConditions
{
  public:
    bool evaluate()
    {
        ...
        for (int i = OrdersTotal() - 1; i >= 0; i--) {
            if (OrderSelect(i, SELECT_BY_POS) && OrderSymbol() == _Symbol && OrderMagicNumber() == magico && (OrderType() == OP_SELL || OrderType() == OP_BUY)) {
                return false; // Ya hay una orden abierta
            }
        }
        return true;
    }
...
// mientras hay una orden abierta, tiene que seguir al precio haciendo traling
class Condition_Follow_Price_to_Buy : public iConditions
{
    double _lowestPrice;
  ...
  public:
    bool evaluate()
    {
        bool haveOrderSell = false;
        bool haveOrderBuy = false;
        for (int i = OrdersTotal() - 1; i >= 0; i--) {
            if (OrderSelect(i, SELECT_BY_POS) && OrderSymbol() == _Symbol && OrderMagicNumber() == magico && OrderType() == OP_SELL) {
                haveOrderSell = true; // hay una orden SELL abierta
                continue;
            }
            if (OrderSelect(i, SELECT_BY_POS) && OrderSymbol() == _Symbol && OrderMagicNumber() == magico && OrderType() == OP_BUY) {
                haveOrderBuy = true; // hay una orden BUY abierta
                _lowestPrice = 0;
                continue;
            }
        }

        double distance         = 100 * _Point;    // 10 pips
        double curr_entry_price = Ask + distance; // precio de entrada actual

        if (haveOrderSell && !haveOrderBuy){
            if (curr_entry_price < _lowestPrice || _lowestPrice == 0) // si el precio actual es menor que el precio más bajo o si es la primera vez
            {
                _lowestPrice = curr_entry_price; // actualizar el precio más bajo

                // borrar y crear una nueva línea
                string lineName = "Line";
                ObjectDelete(lineName);
                ObjectCreate(0, lineName, OBJ_HLINE, 0, TimeCurrent(), _lowestPrice);
                ObjectSetInteger(0, lineName, OBJPROP_COLOR, clrBlue);
                ObjectSetInteger(0, lineName, OBJPROP_STYLE, STYLE_DASHDOT);
                ObjectSetInteger(0, lineName, OBJPROP_WIDTH, 1);
            }

            if (Ask >= _lowestPrice && _lowestPrice != 0) // si el precio actual es mayor o igual al precio más bajo
            {
                return true;
            }
        }
...
Condition_Follow_Price_to_Buy cdFollow_Price_Buy;

class Condition_Follow_Price_to_Sell : public iConditions
{
  ...
  public:
    bool evaluate()
    {
        ...
        // chequear si hay una orden abierta tipo BUY
        bool haveOrderBuy = false;
        bool haveOrderSell = false;
        for (int i = OrdersTotal() - 1; i >= 0; i--) {
            if (OrderSelect(i, SELECT_BY_POS) && OrderSymbol() == _Symbol && OrderMagicNumber() == magico && OrderType() == OP_BUY) {
        ...
        double distance         = 100 * _Point;    // 10 pips
        double curr_entry_price = Bid - distance; // precio de entrada actual

        if (haveOrderBuy && !haveOrderSell) {
            if (curr_entry_price > _highestPrice || _highestPrice == 0) // si el precio actual es mayor que el precio más alto o si es la primera vez
            {
                _highestPrice = curr_entry_price; // actualizar el precio más alto
                string lineName = "Line";
                ObjectDelete(lineName);
                ObjectCreate(0, lineName, OBJ_HLINE, 0, TimeCurrent(), _highestPrice);
                ObjectSetInteger(0, lineName, OBJPROP_COLOR, clrRed);
                ObjectSetInteger(0, lineName, OBJPROP_STYLE, STYLE_DASHDOT);
                ObjectSetInteger(0, lineName, OBJPROP_WIDTH, 1);
            }

            if (Bid <= _highestPrice && _highestPrice != 0) // si el precio actual es menor o igual al precio más alto
            {
                return true;
            }
        }
...
Condition_Follow_Price_to_Sell cdFollow_Price_Sell;

Strategy stFollow_Price_Buy;
Strategy stFollow_Price_Sell;

void BasicStrategy_Iniciate()
{
    LongStrategy.set(&conditionBuy, &buyPending);
    ShortStrategy.set(&conditionSell, &sellPending);
    StrategyToCloseBuy.set(&conditionCloseBuy, &acCloseBuy);
    StrategyToCloseSell.set(&conditionCloseSell, &acCloseSell);

    stFollow_Price_Buy.set(&cdFollow_Price_Buy, &buy);
    stFollow_Price_Sell.set(&cdFollow_Price_Sell, &sell);

    Trader.strategys.add(&LongStrategy);
    Trader.strategys.add(&stFollow_Price_Buy);
    Trader.strategys.add(&ShortStrategy);
    Trader.strategys.add(&stFollow_Price_Sell);
...
input int    count_max_positions = 2;                           // # Total positions:
