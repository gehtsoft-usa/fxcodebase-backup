...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      ...
      double value00 = out[pos];
      double value01 = out[pos - 1];
      double value10 = signal[pos];
      double value11 = signal[pos - 1];
      return value00 >= value10 && value01 < value11 && buffer[0] < close;
      return false;
   }
};
...
int OnInit(void)
{
   ...
   ma = iMA(_Symbol, _Period, confirm_period, 0, MODE_EMA, PRICE_CLOSE);
   IndicatorObjPrefix = GenerateIndicatorPrefix("Klinger Oscillator Alert");
   IndicatorSetString(INDICATOR_SHORTNAME, "Klinger Oscillator Alert");
   ...
   PlotIndexSetString(0, PLOT_LABEL, "KVO");
...
)
{
   ...
   for (int pos = MathMax(first, prev_calculated - 1); pos < rates_total; ++pos)
   {
      ...
      double buffer[1];
      if (CopyBuffer(indi, 0, oldIndex, 1, buffer) != 1)
      {
         continue;
      }
      out[pos] = buffer[0];
      if (CopyBuffer(indi, 1, oldIndex, 1, buffer) != 1)
      {
         continue;
      }
      signal[pos] = buffer[0];
