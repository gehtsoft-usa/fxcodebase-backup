...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      double value0 = iCustom(_symbol, _timeframe, "kaufman_ama_averages_filtered_ATR bands", 
         _timeframe, AMAPeriod, AMAPrice, Nfast, Nslow, GCoeff, PriceFilter, PriceFilterMode, Interpolate, 1, period);
      return value0 == EMPTY_VALUE;
   }
};
...
int OnInit()
{
   double temp = iCustom(NULL, 0, "kaufman_ama_averages_filtered_ATR bands", (ENUM_TIMEFRAMES)_Period, AMAPeriod, AMAPrice, Nfast, Nslow, GCoeff, PriceFilter, PriceFilterMode, Interpolate, 0, 0);
   if (GetLastError() == ERR_INDICATOR_CANNOT_LOAD)
   {
      Alert("Please, install the 'kaufman_ama_averages_filtered_ATR bands' indicator");
      return INIT_FAILED;
   }
   #ifdef SHOW_ACCOUNT_STAT
      stats = NULL;
