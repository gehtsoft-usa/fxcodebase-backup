...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=75410
...
input const string indicator_file = "Ozymandias_System_Alert";   // Indicator File:

input uint           Length      = 2;        // Length
input ENUM_MA_METHOD MAType      = MODE_SMA; // MA MATHOD
input uint           Shift       = 0;        // Shift
input uint           NumberofBar = 1;        // Bar
...
//---
int  handle = 0;
void setHandle() { handle = iCustom(Symbol(), NULL, indicator_file, Length, MAType, Shift, NumberofBar); }
  ...
  public:
    bool evaluate()
    {
        for (int i = 1; i <= candles_back_for_signals; i++) {
            ...
            double indi_prev = Indi(bufferToBuy, i+1);
            if (indi == 1 && indi_prev == 0 && indi != EMPTY_VALUE) {
                return true;
            }
        }
        return false;
  ...
  public:
    bool evaluate()
    {
        for (int i = 1; i <= candles_back_for_signals; i++) {
            double indi      = Indi(bufferToBuy, i);
            double indi_prev = Indi(bufferToBuy, i+1);
            if (indi == 0 && indi_prev == 1 && indi != EMPTY_VALUE) {
                return true;
            }
        }
        return false;
