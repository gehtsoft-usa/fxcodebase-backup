...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=75272
...
class ActionCloseTrade : public iActions
{
  public:
    bool execute()
    {
        ...
        if (!OrderClose(OrderTicket(), OrderLots(), price, 0, Black)) {;}
        return true;
    }
};
ActionCloseTrade acCloseTrade;
...
class ActionCloseWinners : public iActions
{
  public:
    bool execute()
    {
...
ActionCloseWinners acCloseWinners;

class ActionCloseLossers : public iActions
{
  public:
    bool execute()
    {
...
ActionCloseLossers acCloseLoosers;
...
int    sl_candles_shift    = 2; // Candles Shift (high/low)
double sl_pips_from_candle = 5; // Pips From High Low
...
take_profit_mode tp_mode          = tp_by_ratio;         // TP Mode
...
input bool   close_all_when_target     = true;                    // Close all when target reached ?
input double target                    = 100;                     // Profit Target:
input bool   close_loss_when_target    = true;                    // Close entries in loss only when profit reaches target ?
input bool   close_winners_when_target = true;                    // Close entries in winn only when profit reaches target ?
input bool   close_all_when_dd         = true;                    // Close all when Max Drawdown reached ?
input double drawdown                  = 100;                     // Drawdown Max:
input bool   close_trade_when_dd       = true;                    // Max Loss for one trade on ?
input double loss_one_trade            = 10;                      // Max loss for one trade:
...
class ConditionTargetReached : public iConditions
{
  public:
    bool evaluate()
    {
        if (GlobalProfit() >= target) {
            return true;
        }
        return false;
    }
};
ConditionTargetReached cdTargetReached;

class ConditionDrawdownReached : public iConditions
{
  public:
    bool evaluate()
    {
        if (GlobalProfit() <= -drawdown) {
            return true;
        }
        return false;
    }
};
ConditionDrawdownReached cdDrawdownReached;

Strategy stCloseAllWhenTarget;
Strategy stCloseLossWhenTarget;
Strategy stCloseWinnersWhenTarget;
Strategy stCloseAllWhenDrawdown;

class ConditionCloseTrade : public iConditions
{
  public:
    bool evaluate()
    {
        return OrderProfit() <= -loss_one_trade;
    }
};
ConditionCloseTrade cdCloseLossTrade;

Strategy stCloseLossTrade;

void oninit_close_strategy()
{
    stCloseAllWhenTarget.set(&cdTargetReached, &acCloseAll);
    stCloseLossWhenTarget.set(&cdTargetReached, &acCloseLoosers);
    stCloseWinnersWhenTarget.set(&cdTargetReached, &acCloseWinners);
    stCloseAllWhenDrawdown.set(&cdDrawdownReached, &acCloseAll);

    if (close_all_when_target) {
        Trader.strategys.add(&stCloseAllWhenTarget);
    }
    if (close_loss_when_target) {
        Trader.strategys.add(&stCloseLossWhenTarget);
    }
    if (close_all_when_dd) {
        Trader.strategys.add(&stCloseAllWhenDrawdown);
    }
    if(close_trade_when_dd)
    {
        stCloseLossTrade.set(&cdCloseLossTrade, &acCloseTrade);
        Trader.management.add(&stCloseLossTrade);
    }
}
...
ConditionTargetReached conditionCloseBuy;
