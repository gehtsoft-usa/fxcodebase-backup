...
// https://fxcodebase.com/code/viewtopic.php?f=17&t=71783
...
input string             I0                    = "== SLOPE LINE Setup ==";      // == SLOPE LINE Setup ==
input int                HalfLength            = 9;                             // HalfLength
input int                Price                 = 4;                             // Price
input int                LineSizeCen           = 4;                             // LineSizeCen
input int                LineStyleCen          = 0;                             // LineStyleCen
input int                CountBars             = 500;                           // CountBars
input string             Iema                  = "== Moving Average Setup ==";  // == Moving Average Setup ==
input int                Period                = 50;                            // Period
int                      maShift               = 0;                             // Ma Shift
input ENUM_MA_METHOD     Method                = MODE_SMMA;                     // Method
...
input int                magico                = 20;                             // Magic Number:
...
MovingAverage ema(_Symbol, _Period);

class SlopeLine : public iIndicators
{
   string _symbol;
   int    _tf;
   string _file;
   struct SlopeLineParameters
   {
      int setup0;  //  HalfLength=9
      int setup1;  //  Price=4
      int setup2;  //  LineSizeCen=4
      int setup3;  //  LineStyleCen=0
      int setup4;  //  CountBars=500
   };
   SlopeLineParameters _setup;

  public:
   SlopeLine(string Symbol, int TimeFrame)
   {
      _file   = "SLOPE LINE.ex4";
      _symbol = Symbol;
      _tf     = TimeFrame;
      setSetup(HalfLength, Price, LineSizeCen, LineStyleCen, CountBars);
   }
   ~SlopeLine() { ; }

   SlopeLine* file(string setfile)
   {
      _file = setfile;
      return &this;
   }
...
SlopeLine slope(_Symbol, _Period);
...
class CustomConditionBUY : public iConditions
{
  public:
   bool evaluate()
   {
      if (slope.index(1) > ema.index(1))
		if (slope.index(2) <= ema.index(2) || slope.index(3) <= ema.index(3))
      {
         return true;
      }
