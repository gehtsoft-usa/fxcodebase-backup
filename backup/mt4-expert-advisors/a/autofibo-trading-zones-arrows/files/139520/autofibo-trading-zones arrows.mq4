...
//http://fxcodebase.com/code/viewtopic.php?f=38&t=70714
...
input ENUM_MA_METHOD method1 = MODE_EMA; // Method #1
input int period1 = 20; // Period #1
input ENUM_MA_METHOD method2 = MODE_EMA; // Method #2
input int period2 = 50; // Period #2
...
public:
   ...
   {
      double maValue1 = iMA(_symbol, _timeframe, period1, 0, method1, PRICE_CLOSE, period);
      double maValue2 = iMA(_symbol, _timeframe, period2, 0, method2, PRICE_CLOSE, period);
      return maValue1 > maValue2 && iClose(_symbol, _timeframe, period) <= BuyZoneLow;
   }
};
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      double maValue1 = iMA(_symbol, _timeframe, period1, 0, method1, PRICE_CLOSE, period);
      double maValue2 = iMA(_symbol, _timeframe, period2, 0, method2, PRICE_CLOSE, period);
      return maValue1 < maValue2 && iClose(_symbol, _timeframe, period) >= SellZoneHigh;
   }
};
...
int init()
{
   ...
   IndicatorBuffers(10);
   IndicatorObjPrefix = GenerateIndicatorPrefix("aftza");
   IndicatorShortName("Autofibo Trading Zones");
   mainSignaler = new Signaler();
   mainSignaler.SetMessagePrefix(_Symbol + "/" + TimeframeToString((ENUM_TIMEFRAMES)_Period) + ": ");

   int id = 2;
   SetIndexStyle(0, Show_Channel ? DRAW_LINE : DRAW_NONE, 1);
   SetIndexStyle(1, Show_Channel ? DRAW_LINE : DRAW_NONE, 1);
   ...
   ObjectCreate(IndicatorObjPrefix + "FibLevels", OBJ_FIBO, 0, Time[0], High[0], Time[0], Low[0]);
   ObjectCreate(IndicatorObjPrefix + "BuyZone", OBJ_RECTANGLE, 0, 0, 0, 0);
   ObjectCreate(IndicatorObjPrefix + "SellZone", OBJ_RECTANGLE, 0, 0, 0, 0);
   
   if (Show_StartLine)
   {
      if (ObjectFind(IndicatorObjPrefix + StartLine)==-1)
      {
         ObjectCreate(IndicatorObjPrefix + StartLine,OBJ_VLINE,0,Time[Fib_Period],Close[0]);
         ObjectSet(IndicatorObjPrefix + StartLine,OBJPROP_COLOR,StartLine_Color);
      }
   } 
   if (Show_EndLine)
   {
      if (ObjectFind(IndicatorObjPrefix + EndLine)==-1)
      {
         ObjectCreate(IndicatorObjPrefix + EndLine,OBJ_VLINE,0,Time[0],Close[0]);
         ObjectSet(IndicatorObjPrefix + EndLine,OBJPROP_COLOR,EndLine_Color);
      }
   } 
...
int start()
{
   ...
   if (Show_StartLine)
   {   
      datetime HLineTime=ObjectGet(IndicatorObjPrefix + StartLine,OBJPROP_TIME1);
      if (HLineTime>=Time[0])
      {
         BarShift=0;
      }
   ...
   if (Show_EndLine)
   {   
      datetime HLine2Time=ObjectGet(IndicatorObjPrefix + EndLine,OBJPROP_TIME1);
   ...
   for (int i = limit; i >= 0; i--) 
   {
      ...
      if (Show_StartLine)
      {
         ObjectSet(IndicatorObjPrefix + "FibLevels", OBJPROP_TIME1, Time[BarShift]);
         ObjectSet(IndicatorObjPrefix + "FibLevels", OBJPROP_TIME2, Time[BarShift2]);
      }
      if (!Show_StartLine)
      {
         ObjectSet(IndicatorObjPrefix + "FibLevels", OBJPROP_TIME1, StartZoneTime);
      }
      ObjectSet(IndicatorObjPrefix + "FibLevels", OBJPROP_TIME2, Time[0]);
      if (Open[Fib_Period] < Open[0]) // Up
      { 
         if (Show_StartLine)
         {
            ObjectSet(IndicatorObjPrefix + "FibLevels", OBJPROP_PRICE1, SellZoneHigh);
            ObjectSet(IndicatorObjPrefix + "FibLevels", OBJPROP_PRICE2, BuyZoneLow);
         }
         if (!Show_StartLine)
         {
            ObjectSet(IndicatorObjPrefix + "FibLevels", OBJPROP_PRICE1, getPeriodHigh(Fib_Period,i));
            ObjectSet(IndicatorObjPrefix + "FibLevels", OBJPROP_PRICE2, getPeriodLow(Fib_Period,i));
         } 
      } 
      else 
      {
         if (Show_StartLine)
         {
            ObjectSet(IndicatorObjPrefix + "FibLevels", OBJPROP_PRICE1, BuyZoneLow);
            ObjectSet(IndicatorObjPrefix + "FibLevels", OBJPROP_PRICE2, SellZoneHigh);
         }
         if (!Show_StartLine)
         {
            ObjectSet(IndicatorObjPrefix + "FibLevels", OBJPROP_PRICE1, getPeriodLow(Fib_Period,i));
            ObjectSet(IndicatorObjPrefix + "FibLevels", OBJPROP_PRICE2, getPeriodHigh(Fib_Period,i));
         }
      }
      ObjectSet(IndicatorObjPrefix + "FibLevels", OBJPROP_LEVELCOLOR, Fib_Color);
      ObjectSet(IndicatorObjPrefix + "FibLevels", OBJPROP_STYLE, Fib_Style);
      ObjectSet(IndicatorObjPrefix + "FibLevels", OBJPROP_FIBOLEVELS, level_count);
      for(int j=0; j<level_count; j++)
      {
         ObjectSet(IndicatorObjPrefix + "FibLevels", OBJPROP_FIRSTLEVEL+j, level_array[j]);
         ObjectSetFiboDescription(IndicatorObjPrefix + "FibLevels",j,leveldesc_array[j]);
      }
   
      if (Show_StartLine)
      {
         ObjectSet(IndicatorObjPrefix + "BuyZone", OBJPROP_TIME2, Time[BarShift]);
         ObjectSet(IndicatorObjPrefix + "BuyZone", OBJPROP_TIME1, Time[BarShift2]);
      }
      if (!Show_StartLine)
      {
         ObjectSet(IndicatorObjPrefix + "BuyZone", OBJPROP_TIME2, StartZoneTime);
      }
      ObjectSet(IndicatorObjPrefix + "BuyZone", OBJPROP_TIME1, EndZoneTime);
      ObjectSet(IndicatorObjPrefix + "BuyZone", OBJPROP_PRICE1, BuyZoneLow);
      ObjectSet(IndicatorObjPrefix + "BuyZone", OBJPROP_PRICE2, BuyZoneHigh);
      ObjectSet(IndicatorObjPrefix + "BuyZone", OBJPROP_COLOR, BuyZone_Color);
      
      if (Show_StartLine)
      {
         ObjectSet(IndicatorObjPrefix + "SellZone", OBJPROP_TIME2, Time[BarShift]);
         ObjectSet(IndicatorObjPrefix + "SellZone", OBJPROP_TIME1, Time[BarShift2]);
      }
      if (!Show_StartLine)
      {
         ObjectSet(IndicatorObjPrefix + "SellZone", OBJPROP_TIME2, StartZoneTime);
      }
      ObjectSet(IndicatorObjPrefix + "SellZone", OBJPROP_TIME1, EndZoneTime);
      ObjectSet(IndicatorObjPrefix + "SellZone", OBJPROP_PRICE1, SellZoneLow);
      ObjectSet(IndicatorObjPrefix + "SellZone", OBJPROP_PRICE2, SellZoneHigh);
      ObjectSet(IndicatorObjPrefix + "SellZone", OBJPROP_COLOR, SellZone_Color);
   }
...
{
   double buffer = 0;
   for (int i = pos; i <= MathMin(Bars - 1, pos + period); i++) 
   {
      if (High[i] > buffer) 
      {
         buffer = High[i];
