//Available @  https://fxcodebase.com/code/viewtopic.php?f=38&t=75423
...
input int                ma1_period        = 5;                              // Periods:
...
input const string indicator_file = "! Advanced Dz Step RSI (mtf + alerts +arrows)";                          // Indicator File:
input bool earlyOn  = true; // Early entry On?:
input bool normalOn = true; // Normal entry On?:
  ...
  public:
    bool evaluate()
    {
        for (int i = 1; i <= candles_back_for_signals; i++) {
            double rsi = Indi(bufferRSI, i);
            double rsi2 = Indi(bufferRSI, i+1);
            double black = Indi(bufferBuy, i);
            double black2 = Indi(bufferBuy, i+1);
            if (rsi > black && rsi2 <= black2) {
                return true;
            }
        }
        return false;
  ...
  public:
    bool evaluate()
    {
        for (int i = 1; i <= candles_back_for_signals; i++) {
            double rsi = Indi(bufferRSI, i);
            double rsi2 = Indi(bufferRSI, i+1);
            double black = Indi(bufferSell, i);
            double black2 = Indi(bufferSell, i+1);
...
class ConditionIndicatorBuyNormal : public iConditions
{
    double lastSignal;
  ...
  public:
    ...
    {
        for (int i = 1; i <= candles_back_for_signals; i++) {
            double iBuy = Indi(3, i);
            if (iBuy > 0 && iBuy != EMPTY_VALUE && iBuy != lastSignal) {
                lastSignal = iBuy;
                return true;
            }
        }
...
ConditionIndicatorBuyNormal conditionIndicatorBuyNormal;

class ConditionIndicatorSellNormal : public iConditions
{
    double lastSignal;
  ...
  public:
    ...
    {
        for (int i = 1; i <= candles_back_for_signals; i++) {
            double iSell = Indi(4, i);
            if (iSell > 0 && iSell != EMPTY_VALUE && iSell != lastSignal) {
                lastSignal = iSell;
                return true;
            }
        }
...
ConditionIndicatorSellNormal conditionIndicatorSellNormal;
...
bool OnInit_CustomIndicator()
{
    ...
    if(normalOn){
        LongStrategy.addCondition(&conditionIndicatorBuyNormal);
        ShortStrategy.addCondition(&conditionIndicatorSellNormal);
    }
...
input bool   close_on_opposite_early_on = false;                     // Close On Opposite Early Signal:
input bool   close_on_opposite_normal_on = false;                    // Close On Opposite Normal Signal:

ConditionIndicatorSell cdCloseOnOppositeBuy;
ConditionIndicatorBuy cdCloseOnOppositeSell;
ConditionIndicatorSellNormal cdCloseOnOppositeNormalBuy;
ConditionIndicatorBuyNormal cdCloseOnOppositeNormalSell;

Strategy closeOppositeEarlyBuy;
Strategy closeOppositeEarlySell;
Strategy closeOppositeNormalBuy;
Strategy closeOppositeNormalSell;


void oninit_close_on_opposite()
{
    closeOppositeEarlyBuy.set(&cdCloseOnOppositeBuy, &acCloseBuy);
    closeOppositeEarlySell.set(&cdCloseOnOppositeSell, &acCloseSell);
    
    closeOppositeNormalBuy.set(&cdCloseOnOppositeNormalBuy, &acCloseBuy);
    closeOppositeNormalSell.set(&cdCloseOnOppositeNormalSell, &acCloseSell);
    
    if (close_on_opposite_early_on) {
        Trader.management.add(&closeOppositeEarlyBuy);
        Trader.management.add(&closeOppositeEarlySell);
    }
    if(close_on_opposite_normal_on) {
        Trader.management.add(&closeOppositeNormalBuy);
        Trader.management.add(&closeOppositeNormalSell);
