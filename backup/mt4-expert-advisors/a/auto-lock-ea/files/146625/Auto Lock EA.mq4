...
// //https://fxcodebase.com/code/viewtopic.php?f=38&t=72464
...
// ------------------------------------------------------------------
string           T0                      = "== Trade Setup ==";       // == Trade Setup ==
...
class Grid
{
  ...
  public:
    Grid(string Symbol, string Side, double LastPrice, double Gap, double Multiplier, int MaxQnt, double MaxLot, double InitialLot, int magic, bool simbolFilterOn = true, bool magicFilterOn = true)
    {
        ...
        // gridOrders.setOrdersList(false, IntegerToString(_magico), simbolFilterOn, _symbol);
...
void OnTick()
{
    ...
    if (ordersCount != mainOrders.qnt()) {
        ordersCount = mainOrders.qnt();
        HandleTP();
    }
    ...
    if (mainOrders.GetLastMarketOrder()) {
        if (GridON == true) {
            if (mainOrders.last().type() == OP_BUY && CheckPointer(gridBuy) == POINTER_INVALID) {
                gridBuy = new Grid(_Symbol, "buy", mainOrders.last().price(), GridUser_gap, GridUser_multiplier, GridUser_maxCount, GridUser_maxLot, mainOrders.last().lot(), magico, true, false);
            }
            if (mainOrders.last().type() == OP_SELL && CheckPointer(gridSell) == POINTER_INVALID) {
                gridSell = new Grid(_Symbol, "sell", mainOrders.last().price(), GridUser_gap, GridUser_multiplier, GridUser_maxCount, GridUser_maxLot, mainOrders.last().lot(), magico, true, false);
            }
        }
        Notifications(0);
    }
...
void doBreackevenAction()
{
    for (int i = mainOrders.qnt() - 1; i >= 0; i--) {
        if (!mainOrders.index(i).breakevenWasDoIt()) {
            ...
            if (breackevenCondition.evaluate()) {
    ...
    for (int i = OrdersTotal() - 1; i >= 0; i--) {
        if (OrderSelect(i, SELECT_BY_POS, MODE_TRADES) && OrderSymbol() == _Symbol && OrderMagicNumber() == magico) {
            ...
            if (OrderType() == OP_BUY) {
                ...
                if (NormalizeDouble(OrderTakeProfit(), _Digits) != NormalizeDouble(newTPbuy, _Digits)) {
                    if (OrderModify(OrderTicket(), OrderOpenPrice(), OrderStopLoss(), NormalizeDouble(newTPbuy, _Digits), OrderExpiration(), clrNONE)) {
                        Print(__FUNCTION__, " ok Order Modify: ", _id);
...
void handleSellsTP()
{
    for (int i = OrdersTotal() - 1; i >= 0; i--) {
        if (OrderSelect(i, SELECT_BY_POS, MODE_TRADES) && OrderSymbol() == _Symbol && OrderMagicNumber() == magico) {
            ...
            if (OrderType() == OP_SELL) {
                double newTPsell = NormalizeDouble(TP("sell", SellAveragePrice()), _Digits);
                if (NormalizeDouble(newTPsell, _Digits) > Bid) continue;

                if (NormalizeDouble(OrderTakeProfit(), _Digits) > NormalizeDouble(newTPsell, _Digits) && newTPsell > 0) {
                    if (OrderModify(OrderTicket(), OrderOpenPrice(), OrderStopLoss(), NormalizeDouble(newTPsell, _Digits), OrderExpiration(), clrNONE)) {
                        Print(__FUNCTION__, " ok Order Modify: ", _id);
...
double BuyAveragePrice()
{
    ...
    for (int i = 0; i < OrdersTotal(); i++) {
        ...
        {
            if (OrderType() == OP_BUY && OrderSymbol() == Symbol() && OrderMagicNumber() == magico) {
                Total_Buy_Trades++;
    ...
    double Average_Price = Bid - NormalizeDouble(distance, _Digits);
...
double SellAveragePrice()
{
    ...
    for (int i = 0; i < OrdersTotal(); i++) {
        int ord = OrderSelect(i, SELECT_BY_POS, MODE_TRADES);
        {
            if (OrderType() == OP_SELL && OrderSymbol() == Symbol() && OrderMagicNumber() == magico) {
                Total_Trades++;
    ...
    double Average_Price = Ask + NormalizeDouble(distance, _Digits);
...
void ShowStats()
{
    ...
    Comment("== Lucifer Fx ==", "\n",
            "-----------", "\n",
            "BUYS: profit: ", StringFormat("%+.2f", buyProfit), " (", buyqnt, ") ", "\n",
            "SELLS: profit: ", StringFormat("%+.2f", sellProfit), " (", sellqnt, ") ", "\n",
            "-----------", "\n");
}

bool CloseTradeOnThisBar()
{
    for (int i = OrdersHistoryTotal() - 1; i >= 0; i--) {
        if (OrderSelect(i, SELECT_BY_POS, MODE_HISTORY) && OrderSymbol() == _Symbol && OrderMagicNumber() == magico) {
            return OrderCloseTime() >= iTime(NULL, 0, 0);
        }
    }
    return false;
}
