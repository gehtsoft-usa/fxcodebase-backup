...
//http://fxcodebase.com/code/viewtopic.php?f=38&t=69692
...
input int Smooth1  = 7; // TSI Smooth 1
input int Smooth2 = 14; // TSI Smooth 2
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      double adxValue = iADX(_symbol, _timeframe, adx_period, PRICE_CLOSE, MODE_PLUSDI, period);
      return adxValue > 25;
   }
};
...
class TSILongCondition : public ACondition
{
public:
   TSILongCondition(const string symbol, ENUM_TIMEFRAMES timeframe)
      :ACondition(symbol, timeframe)
   {
   ...
   bool IsPass(const int period, const datetime date)
   {
      double tsi = iCustom(_symbol, _timeframe, "TSI", Smooth1, Smooth2, 0, 0);
      return tsi > 0;
   }
};
class TSIShortCondition : public ACondition
{
public:
   TSIShortCondition(const string symbol, ENUM_TIMEFRAMES timeframe)
      :ACondition(symbol, timeframe)
   {
...
class VWAPLongCondition : public ACondition
{
public:
   VWAPLongCondition(const string symbol, ENUM_TIMEFRAMES timeframe)
      :ACondition(symbol, timeframe)
   {
   ...
   bool IsPass(const int period, const datetime date)
   {
      double value0 = iCustom(_symbol, _timeframe, "Any Time Frame VWAP", _timeframe, 0, period);
      double value1 = iCustom(_symbol, _timeframe, "Any Time Frame VWAP", _timeframe, 0, period + 1);
...
class VWAPShortCondition : public ACondition
{
public:
   VWAPShortCondition(const string symbol, ENUM_TIMEFRAMES timeframe)
      :ACondition(symbol, timeframe)
   {
...
ICondition* CreateLongCondition(string symbol, ENUM_TIMEFRAMES timeframe)
{
   ...
   condition.Add(new TSILongCondition(symbol, timeframe), false);
   condition.Add(new VWAPLongCondition(symbol, timeframe), false);
...
ICondition* CreateShortCondition(string symbol, ENUM_TIMEFRAMES timeframe)
{
   ...
   condition.Add(new TSIShortCondition(symbol, timeframe), false);
   condition.Add(new VWAPShortCondition(symbol, timeframe), false);
   #ifdef ACT_ON_SWITCH_CONDITION
      ActOnSwitchCondition* switchCondition = new ActOnSwitchCondition(symbol, timeframe, (ICondition*) condition);
      condition.Release();
      return switchCondition;
...
int OnInit()
{
   ...
   temp = iCustom(NULL, 0, "Any Time Frame VWAP", 0, 0);
   if (GetLastError() == ERR_INDICATOR_CANNOT_LOAD)
   {
      Alert("Please, install the 'Any Time Frame VWAP' indicator");
      return INIT_FAILED;
   }
