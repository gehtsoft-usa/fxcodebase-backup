...
//http://fxcodebase.com/code/viewtopic.php?f=38&t=70772
...
#property indicator_label1  "DMI+"
#property indicator_type1   DRAW_LINE
#property indicator_color1  Green
#property indicator_style1  STYLE_SOLID
#property indicator_width1  2
#property indicator_label2  "DMI-"
...
input double threshold = 20; // Threshold
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      double adxValue = iADX(_symbol, _timeframe, adx_period, PRICE_CLOSE, MODE_MINUSDI, period);
      return adxValue < threshold;
   }
};
...
int init()
{
   ...
   IndicatorBuffers(8);
   IndicatorObjPrefix = GenerateIndicatorPrefix("adxa");
   IndicatorShortName("ADX Arrows");
   mainSignaler = new Signaler();
   mainSignaler.SetMessagePrefix(_Symbol + "/" + TimeframeToString((ENUM_TIMEFRAMES)_Period) + ": ");

   SetIndexBuffer(0, dmip);
   SetIndexBuffer(1, dmim);
   int id = 2;
...
int start()
{
   ...
   {
      ArrayInitialize(dmip, EMPTY_VALUE);
      ArrayInitialize(dmim, EMPTY_VALUE);
      if (customStream != NULL)
      {
         customStream.Init();
      }
   ...
   {
      dmip[pos] = iADX(_Symbol, _Period, adx_period, PRICE_CLOSE, MODE_PLUSDI, pos);
      dmim[pos] = iADX(_Symbol, _Period, adx_period, PRICE_CLOSE, MODE_MINUSDI, pos);
      if (customStream != NULL)
      {
         customStream.SetValue(pos, dmip[pos]);
      }
