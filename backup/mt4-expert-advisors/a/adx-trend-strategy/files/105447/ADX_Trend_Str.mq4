...
input int ADX1_Length=10;
input int ADX2_Length=14;
input int ADX3_Length=20;
input double Level1=35.;
input double Level2=30.;
...
int OnInit()
{
     double temp = iCustom(NULL, 0, "ADX_Trend", 0, 0);
   if (GetLastError() == ERR_INDICATOR_CANNOT_LOAD)
   {
       Alert("Please, install the 'ADX_Trend' indicator");
       return INIT_FAILED;
   }
   LastBar=0;
...
void OnTick()
{
 ...
 ADXT_Up=iCustom(NULL, 0, "ADX_Trend", ADX1_Length, ADX2_Length, ADX3_Length, Level1, Level2, Price, 1, 0, 1);
 ADXT_Dn=iCustom(NULL, 0, "ADX_Trend", ADX1_Length, ADX2_Length, ADX3_Length, Level1, Level2, Price, 1, 1, 1);
 ...
 if ((ADXT_Dn==Low[1] && Signal_Type==0) || (ADXT_Up==High[1] && Signal_Type==1))
 {
  _Alert("Buy");
  CO=CalculateOrders();
  if (CO<=0)
 ...
 if ((ADXT_Dn==Low[1] && Signal_Type==1) || (ADXT_Up==High[1] && Signal_Type==0))
 {
  _Alert("Sell");
  CO=CalculateOrders();
  if (CO>=0)
