...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=75174
...
class Trading
{
  public:
    ...
    Conditions           filters;
    Strategys            strategys;
    Strategys            strategysWithoutFilter;
    PositionsMannagement management;

    void doTrading()
    {
        management.execute();
        strategysWithoutFilter.execute();
        if (filters.evaluate()) {
            strategys.execute();
        }
    }
...
void oninit_basic_conditions() { if(modeEntry == CandleClose) Trader.filters.add(&conditionOncePerCandle); }
...
input bool      modeInstant = true;               // Mode instant (open a trade immediately when attach in chart)
...
input take_profit_mode tp_mode          = tp_null;            // TP Mode
...
input bool   daily_limits_close_all      = true;                       // Close all when limit reached?
input bool   daily_limits_close_platform = true;                       // Close platform when limit is reached?
  ...
  public:
    bool evaluate()
    {
        ...
        if (result >= daily_win_limit || result <= -daily_loss_limit) {
            Print("Daily limit reached: ", result);
            r = false;
        }
  ...
  public:
    bool execute()
    {
        while (OrdersTotal() > 0) {
            for (int i = OrdersTotal() - 1; i >= 0; i--) {
                if (OrderSelect(i, SELECT_BY_POS) && OrderSymbol() == _Symbol && OrderMagicNumber() == magico) {
                    if (OrderType() != OP_BUY && OrderType() != OP_SELL) int r = OrderDelete(OrderTicket(), clrBlack);
                }
            }
        }
        return true;
...
class ActionClosePlatform : public iActions
{
  public:
    bool execute()
    {
...
ActionClosePlatform acClosePlatform;

Strategy          stStopWhenLimits(false);
ConditionDayLimit cdDayLimitStop;
...
void OnInit_DayLimit()
{
    ...
    stStopWhenLimits.addCondition(&cdDayLimitStop);
    if (daily_limits_close_all) {
        stStopWhenLimits.addAction(&acCloseAll);
    }
    if (daily_limits_close_platform) {
        stStopWhenLimits.addAction(&acClosePlatform);
    }

    Trader.strategysWithoutFilter.add(&stStopWhenLimits);
}
...
input double uTimeZone      = -3;                       // Set your GMT zone:
input string timeStart      = "00:00:00";               // Time Start:
input string timeEnd        = "23:59:59";               // Time End:
#endif
class Session
{
...
input const string indicator_file = "AI_Trend_Navigator";        // Indicator File:
//---
int  bufferToBuy              = 0;
int  bufferToSell             = 1;
bool indicator_init           = true;
int  candles_back_for_signals = 2;
//---
...
class ConditionIndicatorBuy : public iConditions
{
  ...
  public:
    bool evaluate()
    {
        ...
        for (int i = ini; i <= candles_back_for_signals; i++) {
            double indi     = Indi(bufferToBuy, i);
            double indiPrev = Indi(bufferToBuy, i + 1);
            if (indi > 0 && indi != EMPTY_VALUE && indiPrev == EMPTY_VALUE && indi != lastSignal) {
                lastSignal = indi;
                return true;
            }
        }
  ...
  public:
    bool evaluate()
    {
        ...
        for (int i = ini; i <= candles_back_for_signals; i++) {
            ...
            double indiPrev = Indi(bufferToSell, i + 1);
            if (indi > 0 && indi != EMPTY_VALUE && indiPrev == EMPTY_VALUE && indi != lastSignal) {
                lastSignal = indi;
                return true;
            }
...
#define Section_ModeInstant
#ifdef Section_ModeInstant
// Mark: Section_ModeInstant



int excuted = 0;

class ConditionModeInstant_BUY : public iConditions
{
  ...
  public:
    bool evaluate()
    {
        for (int i = 1; i <= candles_back_for_signals; i++) {
            double indi = Indi(bufferToBuy, i);
            if (indi > 0 && indi != EMPTY_VALUE && excuted == 0) {
                excuted = 1;
                return true;
            }
        }
...
ConditionModeInstant_BUY cdModeInstant_BUY;

class ConditionModeInstant_SELL : public iConditions
{
  public:
    bool evaluate()
    {
...
ConditionModeInstant_SELL cdModeInstant_SELL;

Strategy stModeInstantBuy;
Strategy stModeInstantSell;

void OnInit_ModeInstant()
{
    if (!modeInstant) return;
    stModeInstantBuy.set(&cdModeInstant_BUY, &buy);
    stModeInstantSell.set(&cdModeInstant_SELL, &sell);
    Trader.strategysWithoutFilter.add(&stModeInstantBuy);
    Trader.strategysWithoutFilter.add(&stModeInstantSell);
}
