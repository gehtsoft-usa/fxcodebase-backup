...
// http://fxcodebase.com/code/viewtopic.php?f=38&t=68995
...
CustomStream* SigBulls;
CustomStream* SigBears;
double SmthBulls[];
double SmthBears[];
...
int CreateAlert(int id, ICondition* upCondition, ICondition* downCondition)
{
   ...
   if (Type == Arrows)
   {
      id = conditions[size].RegisterStreams(id, "Up", 217, up_color, (IStream*) SigBulls);
      id = conditions[size + 1].RegisterStreams(id, "Down", 218, down_color, (IStream*) SigBears);
   }
   else
   {
...
public:
   ...
   {
      return SmthBulls[period] != 0 && SmthBulls[period + 1] == 0;
   }
};
...
public:
   ...
   bool IsPass(const int period)
   {
      return SmthBears[period] != 0 && SmthBears[period + 1] == 0;
   }
};
//+------------------------------------------------------------------+
...
//---- indicators
   ...
   SigBulls = new CustomStream(_Symbol, (ENUM_TIMEFRAMES)_Period);
   SigBears = new CustomStream(_Symbol, (ENUM_TIMEFRAMES)_Period);
   SigBulls.RegisterStream(2, DeepSkyBlue, 1, STYLE_SOLID, "Bulls");
   SigBears.RegisterStream(3, Red, 1, STYLE_SOLID, "Bears");
   
   SetIndexBuffer(12,Bulls);
   SetIndexBuffer(13,Bears);
   SetIndexBuffer(14,AvgBulls);
   SetIndexBuffer(15,AvgBears);
   //
   ...
   IndicatorName = GenerateIndicatorName("AbsoluteStrengthHistogram");
...
//---- 
   ...
   if(counted_bars < 1)
   {
      for(int i=1;i<Length+Smooth+Signal;i++)
      {
         ...
         SmthBulls[Bars-i]=0;
         SmthBears[Bars-i]=0;
         SigBulls._stream[Bars-i]=0;
         SigBears._stream[Bars-i]=0;
      }
   }
   if(counted_bars>0) 
   ...
   limit--;
//----
   for(int shift = MathMin(barsLimit, limit); shift>=0; shift--)
   {
...
//----
      ...
      switch (Mode_Histo)
      {
         ...
         case 3:
            {
               SigBulls._stream[shift]=  SmthBulls[shift];
               SigBears._stream[shift]=  SmthBears[shift];
               if(SmthBulls[shift]-SmthBears[shift]>0)
                  SmthBears[shift]=0;
               else
                  SmthBulls[shift]=0;
