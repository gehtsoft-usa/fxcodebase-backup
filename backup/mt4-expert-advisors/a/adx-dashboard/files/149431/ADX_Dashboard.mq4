...
string file_custom_indicator = "DX_Wilder_(Alert)";
...
enum alert
  {
   Off = 0, // Off
   Main_Current = 1,  // Only ADX line at current bar
   Main_Previous = 2, // Only ADX line at previous closed bar
   All_Current = 3,   // All lines at current bar
   All_Previous = 4   // All lines at previous closed bar
  };
// ------------------------------------------------------------------
input string TitlePairs      = "==== PAIRS ====";            // Set your pairs
input string uSymbols        = "GBPUSD,EURUSD";              // Symbols (add separate by comma ","):
input string TitleIndi       = "==== DX Wilder Setup ====";  //
input int    Period_ADX      = 14;                           // Adx Periods:
alert  notificationsOn = 0;                                  // Notifications
input double alert_level     = 40;                           // Level for alerts
input double alert_levelDown = -40;                          // Level Down:
// ------------------------------------------------------------------
 ...
 public:
  ...
  string FindPattern(ENUM_TIMEFRAMES _tf)
  {
    ...
    string adx;
      ...
      if (havePatternUp(0, _tf))
      {
				int indi = iCustom(_symbol, _tf,file_custom_indicator, Period_ADX, notificationsOn, alert_level, alert_levelDown, 0 , 0);
				adx = indi < 10 ? "0" + (string)indi : (string)indi;
        return adx + up;
      }
      if (havePatternDn(0, _tf))
      {
        int indi    = fabs(iCustom(_symbol, _tf, file_custom_indicator, Period_ADX, notificationsOn, alert_level, alert_levelDown, 0, 0));
        adx = indi < 10 ? "0" + (string)indi : (string)indi;
        return adx + dn;
      }
  ...
  bool havePatternUp(int i, ENUM_TIMEFRAMES tf)
  {
    // TODO: escribir el patrón alcista
		double adx = iCustom(_symbol, tf,file_custom_indicator, Period_ADX, notificationsOn, alert_level, alert_levelDown, 0 , i);
    return adx > 0;
  }
...
void NotifyControl()
{
  for (int r = 1; r < table.nRows(); r++)
  {
		...
		for (int c = 1; c <= 16; c++)
    {
        ...
        if (value == "p" && StringToInteger(table.cellText(r, c+1))>=alert_level) Notifications(0, sym, table.cellText(0, c));
        if (value == "q" && StringToInteger(table.cellText(r, c+1))>=fabs(alert_levelDown)) Notifications(1, sym, table.cellText(0, c));
    }
  }
}
