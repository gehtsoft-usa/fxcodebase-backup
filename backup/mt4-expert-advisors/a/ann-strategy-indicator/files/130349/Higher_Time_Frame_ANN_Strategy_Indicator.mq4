...
// http://fxcodebase.com/code/viewtopic.php?f=38&t=66491
...
extern double   threshold  = 0.0014;
extern color High_Color  = clrDeepSkyBlue;
extern color Low_Color   = clrRed;

extern ENUM_TIMEFRAMES TimeFrame = PERIOD_H4; // Upper timeframe
...
int init()
{
   IndicatorShortName("Higher Time Frame ANN Strategy Indicator");
...
int start()
{
   ...
   for(i=limit; i>=0; i--)
   {
      ...
      double X= PineActivationFunctionLinear(Diff);	
      double l0_0 = X;
      double l0_1 = X;
      double l0_2 = X;
      double l0_3 = X;
      ...
      double l3_0 = PineActivationFunctionTanh(l2_0*0.3385061186 + l2_1*0.6218531956 + l2_2*-0.7790340983 + l2_3*0.1413078332 + l2_4*0.1857010624 + l2_5*-0.1769456351 + l2_6*-0.3242337911 + l2_7*-0.503944883 + l2_8*0.1540568869);
      High_Bar[i] = High_Bar[i+1];
      Low_Bar[i] = Low_Bar[i+1];
   ...
   if (last_alert != iTime(_Symbol, TimeFrame, 0))
   {
      if (High_Bar[0] == 1 && High_Bar[1] != 1)
      {
         signaler.SendNotifications("Bull");
         last_alert = iTime(_Symbol, TimeFrame, 0);
      }
      if (Low_Bar[0] == -1 && Low_Bar[1] != -1)
      {
         signaler.SendNotifications("Bear");
         last_alert = iTime(_Symbol, TimeFrame, 0);
      }
   }
...
double PineActivationFunctionLinear(double v) 
{
	if ((MathExp(v) + MathExp(-v)) !=0)
	{
...
double getDiff(int x_period)
{
    int btf_period = iBarShift(NULL, TimeFrame, Time[x_period], false);
	double yesterday = (iOpen(NULL, TimeFrame, btf_period + 1) + iClose(NULL, TimeFrame, btf_period + 1) + iHigh(NULL, TimeFrame, btf_period + 1) + iLow(NULL, TimeFrame, btf_period + 1)) / 4;
	double today = (iOpen(NULL, TimeFrame, btf_period) + iClose(NULL, TimeFrame, btf_period) + iHigh(NULL, TimeFrame, btf_period) + iLow(NULL, TimeFrame, btf_period)) / 4;
