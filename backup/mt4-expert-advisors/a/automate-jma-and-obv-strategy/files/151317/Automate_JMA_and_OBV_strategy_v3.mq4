...
string file_custom_indicator1 = "Jurik filter 1.02.ex4";
string file_custom_indicator2 = "OBV2.02_mtf+alerts nmc.ex4";
string file_custom_indicator3 = "ATR_Stop.ex4";
...
input string Tindi1 = "== Jurik Filter Setup ==";  // ————————————
input ENUM_TIMEFRAMES    Jurik_TimeFrame = PERIOD_CURRENT;   // Time frame
input int                Jurik_Length = 34;               // Jurik and filter period to use
input double             Jurik_Phase = 3.0;              // Jurik phase 
input bool               Jurik_Double = false;            // Jurik smooth double
input enPrices           Jurik_Price = pr_haweighted;    // Price to use
input double             Jurik_Filter = 0;                // Filter to use for filtering (<=0 for no filtering)
input enFilterType       Jurik_FilterType = flt_all;          // Filter should be applied to :
input enDisplay          Jurik_DisplayType = en_lin;           // Display type
input int                Jurik_LinesWidth = 3;                // Lines width (when lines are included in display)
input bool               Jurik_ArrowOnFirst = true;             // Arrow on first bars
input int                Jurik_UpArrowSize = 2;                // Up Arrow size
input int                Jurik_DnArrowSize = 2;                // Down Arrow size
input int                Jurik_UpArrowCode = 159;              // Up Arrow code
input int                Jurik_DnArrowCode = 159;              // Down arrow code
input double             Jurik_UpArrowGap = 0.5;              // Up Arrow gap        
input double             Jurik_DnArrowGap = 0.5;              // Dn Arrow gap
input color              Jurik_UpArrowColor = clrLime;          // Up Arrow Color
input color              Jurik_DnArrowColor = clrRed;           // Down Arrow Color
input int                Jurik_Shift = 0;                // JMA shift
input bool               Jurik_Interpolate = true;             // Interpolate in multi time frame mode?

input string Tindi2 = "== OBV Setup ==";  // ————————————
input string         OBV_TimeFrame = "Current time frame";
input int            OBV_SignalPeriod = 13;
input ENUM_MA_METHOD OBV_SignalMaMode = MODE_SMMA;
input bool           OBV_ShowMa = true;

input string Tindi3 = "== ATR Stop Setup ==";  // ————————————
input int    ATR_Period = 14;
input double ATR_Factor = 3;
input bool   ATR_MedianPrice = true;
input bool   ATR_MedianBase = true;
input bool   ATR_CloseBase = true;
input double ATR_distance = 0;
...
input bool closeCandleOn = false; // Close Candle Mode?
...
input TSLMode userTslMode = byATRStop;                   // TSL Mode:
...
input GridMode gridmode = Limit; // Grid Type: 
...
class Grid
{
    ...
    public:
    Grid(string Symbol, string Side, double LastPrice, double Gap, double Multiplier, int MaxQnt, double MaxLot, double InitialLot, int magic, bool simbolFilterOn = true, bool magicFilterOn = true, GridMode mode = Limit)
    {
        _symbol     = Symbol;
        _side       = Side;
        ...
        // Set Conditions:
        if(_mode == Limit) cdMatchPrice = new ConditionMatchPrice(_symbol, _side, _nextPrice, 1);
        if(_mode == Stop) cdMatchPrice = new ConditionMatchPrice(_symbol, _side, _nextPrice, 0);
    ...
    bool checkIfCloseByTpSl()
    {
        // .recorrer las ordenes
        for (int i = 0; i < qnt(); i++)
        {
            // .consultar si está cerrada
            if (CheckPointer(gridOrders.index(i)) != POINTER_INVALID)
                if(gridOrders.isClose(i)==true)
                {
                    // .preguntar si se cerró por tpsl
                    if (OrderSelect(gridOrders.index(i).id(), SELECT_BY_TICKET))
                    {
                        if (wasTPSL(OrderComment()) == true)
                            // .si es cierto mandar a cerrar todo
                        {
                            closeGrid();
                            return true;
                        }
    ...
    bool wasTPSL(string txt)
    {
        if (StringFind(txt, "sl")>0)return true;
        if (StringFind(txt, "tp")>0)return true;
        return false;
    }
...
class TslByATR_Stop : public iTSL
{
    int    _Period;
    double _Factor;
    bool   _MedianPrice;
    ...
    public:
    TslByATR_Stop(int Period, double Factor, bool MedianPrice, bool MedianBase, bool CloseBase, double distance)
    {
        _Period = Period;
        _Factor = Factor;
        _MedianPrice = MedianPrice;
        _MedianBase = MedianBase;
        _CloseBase = CloseBase;
        _distance = distance;
    }
    ~TslByATR_Stop() { ; }

    // retorna el precio del indicador de la ultima vela cerrada
    double atr_stop_buy(Order* order)
    {
        double atrStop = 0;
        atrStop = iCustom(NULL, 0, "ATR_Stop.ex4", _Period, _Factor, _MedianPrice, _MedianBase, _CloseBase, _distance, 0, 1);
        return NormalizeDouble(atrStop,_Digits);
    }

    double atr_stop_sell(Order* order)
    {
        double atrStop = 0;
        atrStop = iCustom(NULL, 0, "ATR_Stop.ex4", _Period, _Factor, _MedianPrice, _MedianBase, _CloseBase, _distance, 1, 1);
        return NormalizeDouble(atrStop, _Digits);
    }

    // setea el primer salto que tiene que dar el precio para hacer trailing stop
    void setInitialStep(Order* order)
    {
        double mPoint = MarketInfo(order.symbol(), MODE_POINT);
        double pointsToMove = 10 * mPoint;
        if (order.type() == OP_SELL)
        {
            pointsToMove *= -1;
        }
    ...
    // setea el siguiente salto que tiene que dar el precio para hacer trailing stop
    void setNextStep(Order* order)
    {
        double mPoint = MarketInfo(order.symbol(), MODE_POINT);
        double pointsToMove = 10 * mPoint;
    ...
    double newSL(Order* order)
    {
        ...
        {
            if (atr_stop_buy(order) > order.sl() && atr_stop_buy(order) != 0)
            {
                newSl = atr_stop_buy(order);
            }
        }

        if (order.type() == OP_SELL)
        {
            double atr_sl = atr_stop_sell(order);
            
            if (atr_sl != 0)
            if (atr_sl < order.sl() || order.sl() == 0)
            {
                newSl = atr_sl;
            }
        }
...
class TrailingStop
{
    ...
    TrailingStop(OrdersList* uOrders, TSLMode mode)
    {
        ...
        switch (mode)
        {
            ...
            case byATRStop:
                _TslMode = new TslByATR_Stop(ATR_Period, ATR_Factor, ATR_MedianPrice, ATR_MedianBase, ATR_CloseBase, ATR_distance);
                break;
        }
    }
    ~TrailingStop()
...
bool CloseCandleMode = closeCandleOn;  // meter en la clase CloseCandle
...
class BUYcondition1 : public iConditions
{
    ...
    bool evaluate()
    {
        ...
        return iCustom(NULL, 0, file_custom_indicator1,
            Jurik_TimeFrame,
            Jurik_Length,
            Jurik_Phase,
            Jurik_Double,
...
class BUYcondition2 : public iConditions
{
    ...
    bool evaluate()
    {
        ...
        return iCustom(NULL, 0, file_custom_indicator2,
            OBV_TimeFrame,
            OBV_SignalPeriod,
            OBV_SignalMaMode,
            OBV_ShowMa,
...
int OnInit()
{
    ...
    double temp1 = iCustom(NULL, 0, file_custom_indicator1, 0, 0);
    if (GetLastError() == ERR_INDICATOR_CANNOT_LOAD)
    {
        string txt = "THIS EA NEED AN INDICATOR\ninstall the file:\n" + file_custom_indicator1 + "\ninto the folder:\nMQL4/Indicators.";
        MessageBox(txt, "Important Information", MB_ICONINFORMATION);
        Alert(txt);
        return INIT_FAILED;
    }

    double temp2 = iCustom(NULL, 0, file_custom_indicator2, 0, 0);
    if (GetLastError() == ERR_INDICATOR_CANNOT_LOAD)
    {
        string txt = "THIS EA NEED AN INDICATOR\ninstall the file:\n" + file_custom_indicator2 + "\ninto the folder:\nMQL4/Indicators.";
        MessageBox(txt, "Important Information", MB_ICONINFORMATION);
        Alert(txt);
        return INIT_FAILED;
    }

    double temp3 = iCustom(NULL, 0, file_custom_indicator3, 0, 0);
    if (GetLastError() == ERR_INDICATOR_CANNOT_LOAD)
    {
        string txt = "THIS EA NEED AN INDICATOR\ninstall the file:\n" + file_custom_indicator3 + "\ninto the folder:\nMQL4/Indicators.";
        MessageBox(txt, "Important Information", MB_ICONINFORMATION);
        Alert(txt);
        return INIT_FAILED;
    }
    ...
    tsl = new TrailingStop(GetPointer(mainOrders), userTslMode);
...
void OnTick()
{
    ...
    if (GridON == true && CheckPointer(gridSell) != POINTER_INVALID)
    {
        gridSell.doGrid();
        mainOrders.GetMarketOrders();
        if(gridSell.checkIfCloseByTpSl()) { delete gridSell; }
    }
    if (GridON == true && CheckPointer(gridBuy) != POINTER_INVALID)
    {
        gridBuy.doGrid();
        mainOrders.GetMarketOrders();
        if(gridBuy.checkIfCloseByTpSl()){delete gridBuy;}
    }
    ...
    if (!uTradeReverse)
    {
        if (conditionsToBuy.EvaluateConditions())
        {
            ...
            if (actionSendOrder.doAction())
            {
                ...
                if (GridON == true && CheckPointer(gridBuy) == POINTER_INVALID)
                {
                    gridBuy = new Grid(_Symbol, "buy", mainOrders.last().price(), GridUser_gap, GridUser_multiplier, GridUser_maxCount, GridUser_maxLot, mainOrders.last().lot(), magico, true, true, gridmode);
        ...
        if (conditionsToSell.EvaluateConditions())
        {
            ...
            if (actionSendOrder.doAction())
            {
                ...
                if (GridON == true && CheckPointer(gridSell) == POINTER_INVALID)
                {
                    ...
                    gridSell = new Grid(_Symbol, "sell", mainOrders.last().price(), GridUser_gap, GridUser_multiplier, GridUser_maxCount, GridUser_maxLot, mainOrders.last().lot(), magico, true, true, gridmode);
