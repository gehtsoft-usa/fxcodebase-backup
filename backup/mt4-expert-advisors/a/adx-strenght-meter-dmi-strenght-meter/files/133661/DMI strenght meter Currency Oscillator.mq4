...
int init()
{
   IndicatorObjPrefix = GenerateIndicatorPrefix("dmi_s_s_c_o");
   IndicatorShortName("DMI strenght meter Currency Oscillator");
   ...
   while (pos >= 0)
   {
      for (int i = 0; i < ArraySize(lines); ++i)
      {
         ...
         for (int index = 0; index < ArraySize(lines[i].Symbols); index++)
         {
            ...
            double adxpValue = iADX(symbolData.Symbol, _Period, adx_period, PRICE_CLOSE, MODE_PLUSDI, pos);
            double adxmValue = iADX(symbolData.Symbol, _Period, adx_period, PRICE_CLOSE, MODE_MINUSDI, pos);
            double adx = adxmValue == 0 ? 0 : adxpValue / adxmValue;
         ...
         lines[i].Line[pos] = value / ArraySize(lines[i].Symbols);
      }
      pos--;
   } 
