...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=73750
...
string file_custom_indicator = "AllPivots_v5.4 600+.ex4";
string file_custom_indicator2 = "TMA ATR Channel.ex4";
...
enum ENUM_BASEPRICE
{
   ...
   typical,             // Previous Typical  
   wclose,              // Previous Weighted Close      
   wopen                // Previous Weighted Open
};
enum ENUM_DESCPLACE
{
   off,                 // off
...
//---- input parameters
input string            set1                    =  "";                  // ==== Indicator settings: ====
input ENUM_TIMEFRAMES   TimeFrame               =  D1;                  // TimeFrame 
input string            UniqueName              =  "Fibo";              // Unique Name
input int               Length                  =  5;                   // Length in periods(eg. 2= 2 days)  
input ENUM_PIVOTS       PivotMode               =  fibo_rel;            // Pivot Mode  
input string            PivotValues             =  "0;0.382;0.618;1;1.382;1.618";     // Pivot values(ratios)
input ENUM_BASEPRICE    BasePrice               =  open;                // Base Price
input int               BaseHourShift           =  0;                   // Base Hour Shift(eg.0-start of the day)
input bool              ShowMedians             =  false;               // Show Medians 
input ENUM_RANGE        RangeMode               =  hilo_r;              // Range Mode
input string            AvgPeriods              =  "1;5;10;20";         // Range Avgerage Periods
input int               SessionStartHour        =  0;                   // Session Start Hour(eg. 9 for DAX,0-for Forex)
input int               SessionEndHour          =  0;                   // Session End Hour(eg. 22 for DAX,0-for Forex)
input int               NumberOfDigits          =  4;                   // Number of digits after decimal point 
input string            set2                    =  "";                  // ==== Visual settings: ====
input bool              ShowPivots              =  true;                // Show Pivots
input bool              ShowGradientColor       =  true;                // Show Gradient Colors
input color             MainPivotColor          =  clrWhite;            // Main Pivot Color
input int               MainPivotWidth          =  2;                   // Main Pivot Width
input color             UpperLevelColor         =  clrRed;              // Upper Level Color         
input color             LowerLevelColor         =  clrLimeGreen;        // Lower Level Color
input ENUM_LINE_STYLE   LineStyle               =  STYLE_SOLID;         // Line Style
input int               LineWidth               =  1;                   // Line Width                   
input ENUM_LINE_STYLE   MedianStyle             =  STYLE_DOT;           // Median Style
input int               MedianWidth             =  0;                   // Median Width
input bool              ShowPivotValues         =  false;               // Show Pivot Values
input string            FontName                =  "Arial";             // Font Name
input int               FontSize                =  8;                   // Font Size(0-text disabled)
input ENUM_DESCPLACE    DescriptionPlace        =  left;                // Description Place
input bool              ShowComment             =  false;               // Show Comments
input bool              ShowPeriodSeparators    =  true;                // Show Period Separators
input color             PeriodSeparatorsColor   =  clrGray;             // Period Separators Color
#endif
...
input ENUM_TIMEFRAMES    tma_TimeFrame = PERIOD_CURRENT;
...
input string       TtpTMAclose              = "== Close by TMA ==";     // ————————————
input bool         closeByTmaON = false;                       // Close By TMA
...
input TSLMode userTslMode        = byPivots;                      // TSL Mode:
...
class TslByPivots : public iTSL
{
  ...
  void setInitialStep(Order* order)
  {
    ...
    // buf 4,5,6 S
      ...
      level = order.type() == OP_BUY ? 2 : 5;
      double initialPrice = indicator(level, 0);

    order.tslNext(initialPrice);
  ...
  {
      level += 1;
      Print(__FUNCTION__, " level: ", level);

    double PriceToMatch = indicator(level, 0);
    Print(__FUNCTION__," PriceToMatch: ", PriceToMatch);


    order.tslNext(PriceToMatch);
    Print(__FUNCTION__, " ", "TSL tslNext: ", " ", order.tslNext());
  }

  double newSL(Order* order)
  {
      double newSL = indicator(level - 1, 0);
      Print(__FUNCTION__," level: ",level-1, " newSL: ",newSL);
...
class BUYcondition1 : public iConditions
{
 public:
  bool evaluate()
  {
      ...
      if (Ask >= indicator(1, 0) && Bid < indicator(1, 0)) return true;
...
class BUYcondition2 : public iConditions
{
 public:
  bool evaluate()
  {
      ...
      if (Ask >= indicator(2, 0) && Bid < indicator(2, 0)) return true;
 ...
 public:
  bool evaluate()
  {
    ...
    //   SetIndexBuffer(17,neutral); 
      ...
      double sell_line = TMA_indicator(16, 1);
      double neutral_line = TMA_indicator(17, 1);
      if(sell_line != EMPTY_VALUE || neutral_line != EMPTY_VALUE)
      {
          return true;
      }
        return false;
...
BUYcondition3* closeBuyByTMA_condition;
 ...
 public:
  bool evaluate()
  {
    // TODO: condition sell 1
      ...
      if (Bid < indicator(7, 0) && Ask >= indicator(7, 0)) return true;
      return false;
  }
};
...
class SELLcondition2 : public iConditions
{
 public:
  bool evaluate()
  {
      ...
      if (Bid < indicator(8, 0) && Ask >= indicator(8, 0)) return true;
      return false;
  }
};
 ...
 public:
  bool evaluate()
  {
    // TODO: condition sell 3
      double buy_line = TMA_indicator(15, 1);
      double neutral_line = TMA_indicator(17, 1);
      if( buy_line != EMPTY_VALUE || neutral_line != EMPTY_VALUE)
      {
          return true;
      }
...
SELLcondition3* closeSellByTMA_condition;
 ...
 public:
  bool evaluate()
  {
    if (closeAllInOpositeSignal) 
      ...
      if(closeByTmaON)
            if(closeBuyByTMA_condition.evaluate())return true;
 ...
 public:
  bool evaluate()
  {
      if(closeAllInOpositeSignal)
        ...
        if(closeByTmaON)
            if(closeSellByTMA_condition.evaluate())return true;
...
int OnInit()
{
  ...
  closeBuyByTMA_condition = new BUYcondition3();
  ...
  closeSellByTMA_condition = new SELLcondition3();
...
//   conditionsToCloseSell.AddCondition(sellCondition2 = new SELLcondition2());
  
  conditionsToCloseBuy.AddCondition(conditionCloseBuy = new ConditionToCloseBuy());
//   conditionsToCloseBuy.AddCondition(buyCondition2 = new BUYcondition2());
...
// buf 1,2,3 R
// buf 7,8,9 S
double indicator(int buf, int index)
{
    double value = iCustom(NULL, 0, file_custom_indicator,"",
        TimeFrame,
        UniqueName,
        Length,
        PivotMode,
...
// buf 1,2,3 R
// buf 7,8,9 S
double TMA_indicator(int buf, int index)
{
    double value = iCustom(NULL, 0, file_custom_indicator2, tma_TimeFrame, TMAPeriod, Price, ATRPeriod, TrendThreshold, ShowCenterLine, ATRMultiplier, ATRMultiplier2, ATRMultiplier3, ATRMultiplier4, ATRMultiplier5, ATRMultiplier6, ATRMultiplier7, alertsOn, alertsMessage, alertsSound, alertsEmail, MoveEndpointEveryTick, MaxBarsBack, buf, index);
