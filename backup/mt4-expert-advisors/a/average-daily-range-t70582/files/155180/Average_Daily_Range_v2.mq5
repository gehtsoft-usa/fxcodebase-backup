...
int OnInit()
{
  sum_day    = iATR(_Symbol, PERIOD_D1, NumOfDays_D);
  sum_m      = iATR(_Symbol, PERIOD_D1, NumOfDays_M);
  sum_6m     = iATR(_Symbol, PERIOD_D1, NumOfDays_6M);
  sum_6m_add = iATR(_Symbol, PERIOD_D1, Recent_Days_Weighting);
                ...
                const int&      spread[])              // array Spread
{
  ...
  if (Daily_ADR_Line_ON)
    for (int i = 1; i < Daily_ADR_Line_History; i++) {
      ...
      {
        ObjectCreate(0, objname + "_ADR_Histo_" + i + "ADR high line", OBJ_TREND, 0, 0, _hi - m);
        ...
        ObjectSetDouble(0, objname + "_ADR_Histo_" + i + "ADR high line", OBJPROP_PRICE, 0, _hi - m);
        ObjectSetDouble(0, objname + "_ADR_Histo_" + i + "ADR high line", OBJPROP_PRICE, 1, _hi - m);
        ...
        ObjectCreate(0, objname + "_ADR_Histo_" + i + "ADR low line", OBJ_TREND, 0, 0, _lo + m);
        ...
        ObjectSetDouble(0, objname + "_ADR_Histo_" + i + "ADR low line", OBJPROP_PRICE, 0, _lo + m);
        ObjectSetDouble(0, objname + "_ADR_Histo_" + i + "ADR low line", OBJPROP_PRICE, 1, _lo + m);
  ...
  if (pnt > 0) {
    ...
    if (Weighting_to_ADR_percentage) {
      double WADR = ((iHigh(NULL, PERIOD_D1, 0) - iLow(NULL, PERIOD_D1, 0)) + (iHigh(NULL, PERIOD_D1, 1) - iLow(NULL, PERIOD_D1, 1)) + (iHigh(NULL, PERIOD_D1, 2) - iLow(NULL, PERIOD_D1, 2)) +
                     (iHigh(NULL, PERIOD_D1, 3) - iLow(NULL, PERIOD_D1, 3)) + (iHigh(NULL, PERIOD_D1, 4) - iLow(NULL, PERIOD_D1, 4))) /
                    5;
...
int DailyOpen(int offset, int lastbar, const double& open[], const datetime& time[])
{
  ...
  offset                    = MathMin(offset - 1, iBarShift(Symbol(), Period(), iTime(Symbol(), PERIOD_D1, Daily_Open_Line_History-1)));
