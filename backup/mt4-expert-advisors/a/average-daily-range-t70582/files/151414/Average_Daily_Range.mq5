...
int OnCalculate(const int rates_total,     // grandezza delle timeseries input
                ...
                const int &spread[])       // array Spread
  {
   ...
   if(CopyBuffer(sum_day, 0, 0, 1, sum_day_val) != 1)
     {
      return 0;
     }
   double sum_m_val[1];
   if(CopyBuffer(sum_m, 0, 0, 1, sum_m_val) != 1)
     {
      return 0;
     }
   double sum_6m_val[1];
   if(CopyBuffer(sum_6m, 0, 0, 1, sum_6m_val) != 1)
     {
      return 0;
     }
   double sum_6m_add_val[1];
   if(CopyBuffer(sum_6m_add, 0, 0, 1, sum_6m_add_val) != 1)
     {
      return 0;
     }
