...
public:
   ...
   {
      double value = iCustom(_symbol, _timeframe, "AdvancedFractals", Frames, 1, period + Frames + 1);
      return value > 0;
   }
};
...
public:
   ...
   bool IsPass(const int period)
   {
      double value = iCustom(_symbol, _timeframe, "AdvancedFractals", Frames, 0, period + Frames + 1);
      return value > 0;
   }
};
...
int OnInit()
{
    double temp = iCustom(NULL, 0, "AdvancedFractals", 0, 0);
    if (GetLastError() == ERR_INDICATOR_CANNOT_LOAD)
    {
       Alert("Please, install the 'AdvancedFractals' indicator");
       return INIT_FAILED;
    }
#ifdef SHOW_ACCOUNT_STAT
   stats = NULL;
