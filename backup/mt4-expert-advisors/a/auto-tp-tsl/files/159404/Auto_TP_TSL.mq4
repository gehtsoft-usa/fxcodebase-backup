//Available @  https://fxcodebase.com/code/viewtopic.php?f=38&p=159299#p159299
...
#define Section_ModifyOrders
#ifdef Section_ModifyOrders
// Mark: Section_ModifyOrders

class ConditionNewTrade : public iConditions
{
    int lastCount;
        ...
        for (int i = OrdersTotal() - 1; i >= 0; i--) {
            if (OrderSelect(i, SELECT_BY_POS) && OrderSymbol() == _Symbol) {
                if (OrderType() == OP_BUY || OrderType() == OP_SELL) {
                    count++;
                    Print("line: ",__LINE__," count: ",count);
                }
            }
        }
...
ConditionNewTrade cdNewTrade;

class ActionPutTPSL : public iActions
{
  public:
    bool execute()
    {
        for (int i = OrdersTotal() - 1; i >= 0; i--) {
            if (OrderSelect(i, SELECT_BY_POS) && OrderSymbol() == _Symbol) {
                if (OrderType() == OP_BUY || OrderType() == OP_SELL) {
                    // TODO: Implement Take Profit and Stop Loss modification logic
                    double newSL = 0; 
                    double newTP = 0; 
                    ...
                    if(OrderStopLoss() == 0 || OrderTakeProfit() == 0) {
                        if (OrderType() == OP_BUY) {
                            newSL = OrderStopLoss() == 0 ? StopLoss(pr, "buy", _Symbol, OrderLots()) : OrderStopLoss();
                            newTP = OrderTakeProfit() == 0 ? TakeProfit(pr, "buy", _Symbol, OrderLots()) : OrderTakeProfit();
                        } else if (OrderType() == OP_SELL) {
                            newSL = OrderStopLoss() == 0 ? StopLoss(pr, "sell", _Symbol, OrderLots()) : OrderStopLoss();
                            newTP = OrderTakeProfit() == 0 ? TakeProfit(pr, "sell", _Symbol, OrderLots()) : OrderTakeProfit();
                        }
                    }
                    
                    Print("line: ",__LINE__," newSL: ",newSL);
                    Print("line: ",__LINE__," newTP: ",newTP);

                    // Apply the new Take Profit and Stop Loss
                    int m = OrderModify(OrderTicket(), OrderOpenPrice(), newSL, newTP, 0, clrNONE);
                }
            }
        }
        return true;
...
ActionPutTPSL acPutTPSL;

Strategy stPutTPSL;

void OnInit_PutTPSL()
{
    stPutTPSL.set(&cdNewTrade, &acPutTPSL);
    Trader.management.add(&stPutTPSL); 
}
