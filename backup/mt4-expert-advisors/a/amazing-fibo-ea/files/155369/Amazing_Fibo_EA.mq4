...
#define Section_Basic_classes
#ifdef Section_Basic_classes
// MARK: Basic
...
{
    Strategy* _strategys[];
    bool      onetimebycandle;

  public:
    StrategysHub(bool _onetimebycandle) { onetimebycandle = _onetimebycandle; }
    ~StrategysHub() { release(); }
...
StrategysHub       TradingStrategys(true);
TradingMannagement PositionsMannager;
...
void            OnInit_Basic_Strategy()
{
    ...
    TradingStrategys.add(&LongStrategy);
    TradingStrategys.add(&ShortStrategy);
    ...
    PositionsMannager.add(&StrategyToCloseBuy);
    PositionsMannager.add(&StrategyToCloseSell);
}
...
input const string indicator_file = "FMCBR.ex4";                 // Indicator File:
color              BLUE           = clrAqua;
color              RED            = clrRed;
input int          inDepth        = 50;
input int          Limit          = 240;
...
bool                   OnInit_CustomIndicator()
{
    ...
    TradingStrategys.add(&IndicatorStrategyLong);
    TradingStrategys.add(&IndicatorStrategyShort);
...
#ifdef Section_Stop_expert

// MARK: Stop Expert
class StopTrading : public iActions
{
  public:
    StopTrading() { ; }
    ~StopTrading() { ; }
...
Strategy    StopExpert(false); // si alguna condition es false detiene el ea
...
input string    TmodeEntry = "== Entry to Market Setup =="; // ————————————
...
void            Initiate_Breakeven()
{
    ...
    PositionsMannager.add(&Breakeven);
}
...
int OnInit()
{
    ...
    #ifdef Section_Stop_expert
    StopExpert.addAction(&stopTrading);
    #endif

    PositionsMannager.set(_Symbol, magico);
...
void OnTick()
{
    PositionsMannager.execute();
    #ifdef Section_Stop_expert
        if (StopExpert.execute()) return;
    #endif    
    TradingStrategys.execute();
}
