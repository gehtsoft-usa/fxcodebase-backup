...
input color range_clr = White;
input color range_trend_clr = 0xEFDB2B;
input color range_strong_trend_clr = Orange;
input color range_weakening_trend_clr = Red;
...
int init()
{
   ...
   id = al_lowerbar.RegisterStream(id, Teal);
   id = al_lowerbar.RegisterStream(id, White);
   al_upperbar = new ColoredStream(_Symbol, (ENUM_TIMEFRAMES)_Period);
   id = al_upperbar.RegisterStream(id, Blue);
   id = al_upperbar.RegisterStream(id, 0x115193);
   id = al_upperbar.RegisterStream(id, White);
   al_rangebar = new ColoredStream(_Symbol, (ENUM_TIMEFRAMES)_Period);
   id = al_rangebar.RegisterStream(id, range_clr);
   id = al_rangebar.RegisterStream(id, range_trend_clr);
   id = al_rangebar.RegisterStream(id, range_strong_trend_clr);
   id = al_rangebar.RegisterStream(id, range_weakening_trend_clr);
                ...
                const int &spread[])
{
   if (prev_calculated <= 0 || prev_calculated > rates_total)
   {
      ...
      ArrayInitialize(asp_pval, EMPTY_VALUE);
   ...
   for (int pos = rates_total - 1 - MathMax(prev_calculated, toSkip); pos >= 0 && !IsStopped(); --pos)
   {
      ...
      for (int i = 1; i < range_length; ++i)
      {
         ...
         double val1 = Low[pos] <= Close[pos + 1] ? Low[pos] : Close[pos + 1];
         if (chop_ltl > val1)
         {
            chop_ltl = val1;
         }
         double val2 = High[pos] >= Close[pos + 1] ? High[pos] : Close[pos + 1];
         if (chop_hth < val2)
         {
            chop_hth = val2;
         }
      ...
      int lowestIndex = ArrayMinimum(af_stddev, pos, af_low_lookback);
      ...
      asp_pval[pos] = MathMax(asp_price_stdev[pos], 0);
      ...
      al_rangebar.Set(3, pos, range_index[pos] > range_max_val ? 0 : range_trend_condition ? 1 : range_strong_trend_condition ? 2 : range_weakening_trend_condition ? 3 : 0);
