...
// http://fxcodebase.com/code/viewtopic.php?f=38&t=68711
...
//---- input parameters
input int ADXcrossesPeriod = 14;
input bool act_on_close = true; // End of turn mode
//---- buffers
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      b4plusdi = iADX(NULL, 0, ADXcrossesPeriod, PRICE_CLOSE, MODE_PLUSDI, period + 1);
      nowplusdi = iADX(NULL, 0, ADXcrossesPeriod, PRICE_CLOSE, MODE_PLUSDI, period);
      b4minusdi = iADX(NULL, 0, ADXcrossesPeriod, PRICE_CLOSE, MODE_MINUSDI, period + 1);
      nowminusdi = iADX(NULL, 0, ADXcrossesPeriod, PRICE_CLOSE, MODE_MINUSDI, period);  
      return b4plusdi > b4minusdi && nowplusdi < nowminusdi;
   }
};
...
int init()
  {
   ...
   IndicatorBuffers(8);
   IndicatorObjPrefix = GenerateIndicatorPrefix("adxbs");
