...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=74847
...
input string Tentrys       = "== Entrys Setup =="; // ————————————
input bool   AdxOver_on    = false;                 //  ADX is over 25 (+DI/-DI determines direction)
input bool   DiCrossAdx_on = true;                 //  +DI/-DI crossovers with ADX level
input bool   DiCross_on    = true;                 //  Trade +DI/-DI crossovers
...
ConcurrentConditions AllowToTrading;
...
class BUYcondition1 : public iConditions
{
  public:
    bool evaluate()
    {
        return adx.Main(1) > AdxLevelMain && adx.bull(1);
    }
};
...
class BUYcondition2 : public iConditions
{
  public:
    bool evaluate()
    {
        ...
        return adx.Main(1) < adx.PlusDi(1) &&
               adx.Main(2) >= adx.PlusDi(2);
    }
};
...
class BUYcondition3 : public iConditions
{
  public:
    bool evaluate()
    {
        ...
        return adx.PlusDi(1) > adx.MinusDi(1) &&
               adx.PlusDi(2) <= adx.MinusDi(2);
    }
};
...
class SELLcondition1 : public iConditions
{
  public:
    bool evaluate()
    {
        return adx.Main(1) > AdxLevelMain && adx.bear(1);
    }
};
...
class SELLcondition2 : public iConditions
{
  public:
    bool evaluate()
    {
        ...
        return adx.Main(1) < adx.MinusDi(1) &&
               adx.Main(2) >= adx.MinusDi(2);
    }
};
...
#ifdef MAX_TRADES_AT_SAME_TIME
    AllowToTrading.AddCondition(countTrades = new ConditionCountTrades(uMaxTrades));
    AllowToTrading.AddCondition(countTrades = new ConditionCountTrades(uMaxTrades));
#endif
...
if(AllowToTrading.EvaluateConditions())
{
        // NOTE: BUY normal
        if (!uTradeReverse) {
