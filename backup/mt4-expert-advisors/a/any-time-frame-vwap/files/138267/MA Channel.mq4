...
public:
   ...
   {
      return iClose(_symbol, _timeframe, period) > up[period];
   }
};
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      return iClose(_symbol, _timeframe, period) < dn[period];
   }
};
...
int init()
{
   ...
   IndicatorBuffers(11);
   IndicatorObjPrefix = GenerateIndicatorPrefix("mac");
                ...
                const int &spread[])
{
   ...
   for (int pos = MathMin(bars_limit, rates_total - 1 - MathMax(prev_calculated, toSkip)); pos >= 0 && !IsStopped(); --pos)
   {
      ...
      if (customStream != NULL)
      {
         customStream.SetValue(pos, ma[pos]);
      }
      for (int i = 0; i < ArraySize(conditions); ++i)
      {
         AlertSignal* item = conditions[i];
