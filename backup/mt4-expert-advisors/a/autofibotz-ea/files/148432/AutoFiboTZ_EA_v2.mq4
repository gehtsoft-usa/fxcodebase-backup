...
string file_custom_indicator = "AutoFib TradeZones.ex4";
 ...
 public:
  bool evaluate()
  {
    // TODO: condition Buy 1
	 ...
	 buyZone.highPrice =  NormalizeDouble(ObjectGet("BuyZone", 3), _Digits);
	 buyZone.lowPrice  =  NormalizeDouble(ObjectGet("BuyZone", 1), _Digits);
 ...
 public:
  bool evaluate()
  {
    // TODO: condition sell 1
		sellZone.highPrice =  NormalizeDouble(ObjectGet("SellZone", 3),_Digits);
		sellZone.lowPrice  =  NormalizeDouble(ObjectGet("SellZone", 1),_Digits);
...
void ModifyPendings()
{
	for(int i=OrdersTotal()-1;i>=0;i--)
	{
		 if(OrderSelect(i,SELECT_BY_POS) && OrderSymbol() == _Symbol && OrderMagicNumber()== magico)
		 {
                ...
                {
                    if (OrderOpenPrice() == buyZone.highPrice) continue;
                    if(!OrderModify(OrderTicket(), buyZone.highPrice, SL("buy", buyZone.highPrice), OrderTakeProfit(), OrderExpiration(), CLR_NONE))
                    {
                        Print(GetLastError());
                    }
                }
                ...
                {
                    if(OrderOpenPrice() == sellZone.lowPrice) continue;
                    if (!OrderModify(OrderTicket(), sellZone.lowPrice, SL("sell", sellZone.lowPrice), OrderTakeProfit(), OrderExpiration(), CLR_NONE))
                    {
                        Print(GetLastError());
                    }
                }
