...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=75046
...
input const string indicator_file = "BBPercentile";              // Indicator File:
input IndiMode     indimode       = cross_zero_mode;             // Signal Mode:
//---
  ...
  public:
    bool evaluate()
    {
        for (int i = 1; i <= candles_back_for_signals; i++) 
        {
            ...
            double arrow_up = Indi(1, i);

            if (indimode == cross_zero_mode) {
                if (indi > 0 && indi2 <= 0) {
                    return true;
                }
            }
            
            if (indimode == arrow_mode) {
                if (arrow_up < 0 && arrow_up != EMPTY_VALUE && arrow_up != lastSignal) {
                    lastSignal = arrow_up;
                    return true;
                }
            }
  ...
  public:
    bool evaluate()
    {
        for (int i = 1; i <= candles_back_for_signals; i++) 
        {
            ...
            if (indimode == arrow_mode) {
                if (arrow_dn > 0 && arrow_dn != EMPTY_VALUE && arrow_dn != lastSignal) {
                    lastSignal = arrow_dn;
                    return true;
                }
            }
