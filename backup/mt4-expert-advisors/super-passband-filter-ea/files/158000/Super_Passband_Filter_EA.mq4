...
//https://fxcodebase.com/code/viewtopic.php?f=38&p=147588#p147588
...
input bool use_ma_filter = false; // Use MA Filter?
input int ma_filter_period = 200; // MA Filter Period
input ENUM_MA_METHOD ma_filter_method = MODE_SMA; // MA Filter Method
...
input TradingMode entry_logic = TradingModeLive; // Entry logic
   ...
   input bool exit_crossback = false; // True - exit on cross-back, false - exit on opposite signal
#endif
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      double pb = iCustom(_symbol, _timeframe, "Super Passband Filter", Period1, Period2, Average, 0, period);
      double prms = iCustom(_symbol, _timeframe, "Super Passband Filter", Period1, Period2, Average, 1, period);
      return pb > prms;
   }
};
...
public:
   ...
   {
      double pb = iCustom(_symbol, _timeframe, "Super Passband Filter", Period1, Period2, Average, 0, period);
      double nrms = iCustom(_symbol, _timeframe, "Super Passband Filter", Period1, Period2, Average, 2, period);
      return pb < nrms;
   }
};
...
class ExitLongCrossbackCondition : public ACondition
{
public:
   ExitLongCrossbackCondition(const string symbol, ENUM_TIMEFRAMES timeframe)
      :ACondition(symbol, timeframe)
   {
...
class ExitShortCrossbackCondition : public ACondition
{
public:
   ExitShortCrossbackCondition(const string symbol, ENUM_TIMEFRAMES timeframe)
      :ACondition(symbol, timeframe)
   {
...
ICondition* CreateLongFilterCondition(string symbol, ENUM_TIMEFRAMES timeframe)
{
   ...
   if (use_ma_filter)
   {
      condition.Add(new MAPriceCondition(symbol, timeframe, FirstBelowSecond, ma_filter_method, ma_filter_period, PRICE_CLOSE), false);
   }
   else
   {
...
ICondition* CreateShortFilterCondition(string symbol, ENUM_TIMEFRAMES timeframe)
{
   ...
   if (use_ma_filter)
   {
      condition.Add(new MAPriceCondition(symbol, timeframe, FirstAboveSecond, ma_filter_method, ma_filter_period, PRICE_CLOSE), false);
   }
   else
   {
      condition.Add(new NoCondition(), false);
...
ICondition* CreateExitLongCondition(string symbol, ENUM_TIMEFRAMES timeframe)
{
   ...
   if (exit_crossback)
   {
      condition.Add(new ExitLongCrossbackCondition(symbol, timeframe), false);
   }
   else
   {
...
ICondition* CreateExitShortCondition(string symbol, ENUM_TIMEFRAMES timeframe)
{
   ...
   if (exit_crossback)
   {
      condition.Add(new ExitShortCrossbackCondition(symbol, timeframe), false);
   }
   else
   {
...
int OnInit()
{
   double temp = iCustom(NULL, 0, "Super Passband Filter", 0, 0);
   if (GetLastError() == ERR_INDICATOR_CANNOT_LOAD)
   {
      Alert("Please, install the 'Super Passband Filter' indicator");
      return INIT_FAILED;
   }
   #ifdef SHOW_ACCOUNT_STAT
      stats = NULL;
