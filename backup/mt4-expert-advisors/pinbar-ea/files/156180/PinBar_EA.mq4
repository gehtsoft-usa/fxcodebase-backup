...
input const string indicator_file = "Pinbar";        // Indicator File:
//---
int  bufferToBuy              = 0;
int  bufferToSell             = 1;
bool indicator_init           = true;
int  candles_back_for_signals = 300;
//---
double Indi(int buffer, int candle = 1) { return iCustom(NULL, 0, indicator_file, buffer, candle); }
double Support_shift(int buffer = 6, int candle = 1) { return iCustom(NULL, 0, indicator_file, buffer, candle); }
double Resistance_shift(int buffer = 7, int candle = 1) { return iCustom(NULL, 0, indicator_file, buffer, candle); }
...
class ConditionIndicatorBuy : public iConditions
{
  ...
  public:
    bool evaluate()
    {
        int    i        = (int)Support_shift() + 1;
        double op       = iOpen(NULL, 0, i);
        double hi       = iHigh(NULL, 0, i);
        double lo       = iLow(NULL, 0, i);
        double cl       = iClose(NULL, 0, i);
        double wick_top = cl < op ? cl : op;
        double wick_med = lo + ((wick_top - lo) / 2);

        buyPrice = wick_med;

        if (Bid < buyPrice && buyPrice != lastSignal) {
            lastSignal = buyPrice;
            return true;
        }
        return false;
...
class ConditionIndicatorSell : public iConditions
{
  ...
  public:
    bool evaluate()
    {
        int    i        =  (int)Resistance_shift() + 1;
        double op       = iOpen(NULL, 0, i);
        double hi       = iHigh(NULL, 0, i);
        double lo       = iLow(NULL, 0, i);
        double cl       = iClose(NULL, 0, i);
        double wick_lo  = cl > op ? cl : op;
        double wick_med = wick_lo + ((hi - wick_lo) / 2);

        sellPrice = wick_med;

        if (Ask > sellPrice && sellPrice > 0 && sellPrice != EMPTY_VALUE && sellPrice != lastSignal) {
            lastSignal = sellPrice;
            return true;
        }
        return false;
    }
