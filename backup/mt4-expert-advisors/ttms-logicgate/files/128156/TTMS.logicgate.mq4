...
int init()
{
   ...
   highStream.Release();
   id = dots.RegisterStream(id, Green, "", DRAW_LINE, STYLE_DOT, 3);
   id = dots.RegisterStream(id, Red, "", DRAW_LINE, STYLE_DOT, 3);
   id = dots.RegisterInternal(id);
...
int start()
{
   ...
   while (pos>=0)
   {
      ...
      bool buy = TL > H && momHistVal > 0;
      bool sell = TL < H && momHistVal < 0;
      if (buy)
      ...
      if (buy && !sell)
         ...
         dots.Set(0, pos, dots.GetColorIndex(pos + 1));
