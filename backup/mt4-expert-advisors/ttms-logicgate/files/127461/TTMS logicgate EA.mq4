...
input int Keltner_Length=20;
input int Keltner_Smooth_Length=20;
input int Keltner_Smooth_Method=0;  // 0 - SMA
                                    // 1 - EMA
                                    // 2 - SMMA
                                    // 3 - LWMA
input double Keltner_Deviation=2;
input int momPeriod = 12; // Momemtum Period
input int momEMA = 5; // Momentum EMA Period
...
public:
   ...
   {
      double value = iCustom(_symbol, _timeframe, "TTMS.logicgate", BB_Length, BB_Deviation, Keltner_Length,
         Keltner_Smooth_Length, Keltner_Smooth_Method, Keltner_Deviation, momPeriod, momEMA, 4, period);
...
public:
   ...
   {
      double value = iCustom(_symbol, _timeframe, "TTMS.logicgate", BB_Length, BB_Deviation, Keltner_Length,
         Keltner_Smooth_Length, Keltner_Smooth_Method, Keltner_Deviation, momPeriod, momEMA, 5, period);
      return value != EMPTY_VALUE;
   }
};
...
int OnInit()
{
   double temp = iCustom(NULL, 0, "TTMS.logicgate", 0, 0);
   if (GetLastError() == ERR_INDICATOR_CANNOT_LOAD)
   {
      Alert("Please, install the 'TTMS.logicgate' indicator");
      return INIT_FAILED;
   }
#ifdef SHOW_ACCOUNT_STAT
   stats = NULL;
