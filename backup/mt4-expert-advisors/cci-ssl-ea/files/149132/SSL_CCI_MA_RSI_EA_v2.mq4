...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=70102
...
enum sslMode { OpenTrades,  // To Open Trades
               Trend        // Filter Trend
};
...
MovingAverage ma3();
// ------------------------------------------------------------------
input string             sslSetup         = "== SSL Setup ==";         // == SSL Setup ==
input bool               sslOn            = true;                      // SSL ON:
input int                Lb               = 10;                        // Lb:
sslMode                  sslmode          = OpenTrades;                // SSL Mode:
input string             TripleMASetup    = "== MA FILTER Setup ==";   // == MA System Setup ==
input bool               maSystemOn       = true;                      // Ma System On:
// input bool controlPrice = true;// Control Price respect EMA? (above to buy / below to sell)
input string             ma1t             = "== MA1 Setup ==";         // == MA1 Setup ==
input bool               ma1On            = true;                      // MA1 On:
input int                ma1Period        = 7;                         // Period
...
input bool               ma2On            = true;                      // MA2 On:
input int                ma2Period        = 60;                        // Period
...
input bool               ma3On            = true;                      // MA3 On:
input int                ma3Period        = 180;                       // Period
...
input string             cciSetup         = "== CCI FILTER Setup ==";  // == CCI Setup ==
input bool               cciOn            = true;                      // CCI ON:
input int                cci_period       = 20;                        // CCI Period
input double             cci_buy_level    = -100;                      // CCI buy level
input double             cci_sell_level   = 100;                       // CCI sell level
input string             RSIFilterSetup   = "== RSI FILTER Setup ==";  // == RSI System Setup ==
input bool               rsiOn            = true;                      // RSI Filter On:
input string             Irsi1            = "== RSI 1 Setup ==";       // == RSI 1 Setup ==
input int                rsi1Period       = 50;                        // Period
input ENUM_APPLIED_PRICE rsi1AppliedPrice = PRICE_CLOSE;               // Applied Price
input string             Irsi2            = "== RSI 2 Setup ==";       // == RSI 2 Setup ==
input int                rsi2Period       = 100;                       // Period
input ENUM_APPLIED_PRICE rsi2AppliedPrice = PRICE_CLOSE;               // Applied Price
...
input double          userLots                = 0.01;                       // Setup Lots:
...
input ENUM_TIMEFRAMES uATRtf_tp               = PERIOD_CURRENT;             // ATR TP Time Frame
input int             uATRperiod_tp           = 14;                         // ATR TP periods
input double          uATRmultiplier_tp       = 1;                          // ATR TP multiplier
...
input ENUM_TIMEFRAMES uATRtf                  = PERIOD_CURRENT;             // ATR SL Time Frame
input int             uATRperiod              = 14;                         // ATR SL periods
input double          uATRmultiplier          = 1;                          // ATR SL multiplier
input string          TtpOptions              = "== Close Setup ==";        // == Close Setup ==
bool                  closeAllControlON       = false;                      // Close All Control ON:
input bool            sslToClose              = false;                      // Close By SSL:
input bool            cciToClose              = false;                      // Close By CCI:
input bool            rsiToClose              = false;                      // Close By RSI:
...
input string          bloqSetup               = "== Security Setup ==";     // == Security Setup ==
input bool            countLossOn             = false;                      // Count Losses ON:
input int             uLossesLimit            = 5;                          // Losses Limit:
input bool            marginLimitOn           = false;                      // Margin Limit ON:
input double          uMarginLimit            = 0;                          // Margin % Limit:
...
input int             magico                  = 6282022;                    // Magic Number:
...
class ConcurrentConditions
{
 ...
 public:
  ...
  {
    if (ArraySize(_conditions) == 0) return false;
...
BUYcondition1* buyCondition1_ssl;
BUYcondition1* buyCondition1_ssl_close;
...
class BUYcondition2 : public iConditions
{
 public:
  bool evaluate()
  {
    ...
    return maSystemHaveUpTrend();
  }
};
BUYcondition2* buyCondition2_ma;
class BUYcondition3 : public iConditions
{
 public:
  bool evaluate()
  {
    ...
    return cciHaveBuyTrend();
  }
};
BUYcondition3* buyCondition3_cci;
BUYcondition3* buyCondition3_cci_close;

class BUYcondition4 : public iConditions
{
 public:
  bool evaluate()
  {
    // TODO: condition Buy 4
    return rsiTrendUp();
  }
};
BUYcondition4* buyCondition4_rsi;
BUYcondition4* buyCondition4_rsi_close;
...
SELLcondition1* sellCondition1_ssl;
SELLcondition1* sellCondition1_ssl_close;
...
class SELLcondition2 : public iConditions
{
 public:
  bool evaluate()
  {
    ...
    return maSystemHaveDnTrend();
  }
};
SELLcondition2* sellCondition2_ma;
...
class SELLcondition3 : public iConditions
{
 public:
  bool evaluate()
  {
    ...
    return cciHaveSellTrend();
  }
};
SELLcondition3* sellCondition3_cci;
SELLcondition3* sellCondition3_cci_close;

class SELLcondition4 : public iConditions
{
 public:
  bool evaluate()
  {
    // TODO: condition sell 4
    return rsiTrendDn();
  }
};
SELLcondition4* sellCondition4_rsi;
SELLcondition4* sellCondition4_rsi_close;
...
int OnInit()
{
  double temp = iCustom(NULL, 0, "ssl-channel-chart-alert-indicator", 0, 0);
  if (GetLastError() == ERR_INDICATOR_CANNOT_LOAD)
  {
    Alert("Please, install the 'ssl-channel-chart-alert-indicator' indicator");
    return INIT_FAILED;
  }

  newCandle   = new CNewCandle();
  tsl         = new TrailingStop(GetPointer(mainOrders), byPips);
  countLosses = new CounterLosses(uLossesLimit, magico);
  ...
  if (sslOn) conditionsToBuy.AddCondition(buyCondition1_ssl = new BUYcondition1());
  if (maSystemOn) conditionsToBuy.AddCondition(buyCondition2_ma = new BUYcondition2());
  if (cciOn) conditionsToBuy.AddCondition(buyCondition3_cci = new BUYcondition3());
  if (rsiOn) conditionsToBuy.AddCondition(buyCondition4_rsi = new BUYcondition4());
	// if (controlPrice) conditionsToBuy.AddCondition(buyCondition5_price = new BUYcondition5());
  ...
  //--- sell:
  if (sslOn) conditionsToSell.AddCondition(sellCondition1_ssl = new SELLcondition1());
  if (maSystemOn) conditionsToSell.AddCondition(sellCondition2_ma = new SELLcondition2());
  if (cciOn) conditionsToSell.AddCondition(sellCondition3_cci = new SELLcondition3());
  if (rsiOn) conditionsToSell.AddCondition(sellCondition4_rsi = new SELLcondition4());
	// if (controlPrice) conditionsToSell.AddCondition(sellCondition5_price = new SELLcondition5());
  ...
  if (sslToClose) conditionsToCloseBuy.AddCondition(sellCondition1_ssl_close = new SELLcondition1());
  if (sslToClose) conditionsToCloseSell.AddCondition(buyCondition1_ssl_close = new BUYcondition1());
  if (cciToClose) conditionsToCloseSell.AddCondition(buyCondition3_cci_close = new BUYcondition3());
  if (cciToClose) conditionsToCloseBuy.AddCondition(sellCondition3_cci_close = new SELLcondition3());
  if (rsiToClose) conditionsToCloseSell.AddCondition(buyCondition4_rsi_close = new BUYcondition4());
  if (rsiToClose) conditionsToCloseBuy.AddCondition(sellCondition4_rsi_close = new SELLcondition4());
...
void OnTick()
{
  ...
  {
    if(!reverseMode)actionSendOrder = new SendNewOrder("buy", Lots(), "", 0, SL("buy"), TP("buy"), magico);
    if(reverseMode) actionSendOrder = new SendNewOrder("sell", Lots(), "", 0, SL("sell"), TP("sell"), magico);
  ...
  if (stopLossOn)
    switch (modeSL)
    {
      ...
      case PipsFromOpenCandle:
        ...
        break;
      case byATR:
        levelSL = new Levels(new ByATR(_Symbol, side, "SL", uATRtf, uATRperiod, 1, uATRmultiplier));
        result  = levelSL.calculateLevel();
        break;
    }
  return result;
  ...
  if (takeProfitOn)
    switch (modeTP)
    {
      ...
      case PipsFromOpenCandle:
        ...
        break;
      case byATR:
        levelTP = new Levels(new ByATR(_Symbol, side, "TP", uATRtf_tp, uATRperiod_tp, 1, uATRmultiplier_tp));
        result  = levelTP.calculateLevel();
        break;
    }
  return result;
...
bool sslCrossUp()
{
  ...
  int    bufferUp = 1;
  int    shift    = 1;
  double up0      = iCustom(NULL, 0, "ssl-channel-chart-alert-indicator", Lb, bufferUp, shift);
  double up1      = iCustom(NULL, 0, "ssl-channel-chart-alert-indicator", Lb, bufferUp, shift + 1);
  double dn0      = iCustom(NULL, 0, "ssl-channel-chart-alert-indicator", Lb, bufferDn, shift);
  double dn1      = iCustom(NULL, 0, "ssl-channel-chart-alert-indicator", Lb, bufferDn, shift + 1);
...
// NOTE: CCI signals
// ------------------------------------------------------------------
bool cciHaveBuyTrend()
{
  int    n    = 1;
  double cci0 = iCCI(NULL, 0, cci_period, PRICE_CLOSE, n);
  double cci1 = iCCI(NULL, 0, cci_period, PRICE_CLOSE, n + 1);

  // condition: ascending and greater than the buy level
  return (cci0 > cci_buy_level && cci0 >= cci1);
}
bool cciHaveSellTrend()
{
  int    n    = 1;
  double cci0 = iCCI(NULL, 0, cci_period, PRICE_CLOSE, n);
  double cci1 = iCCI(NULL, 0, cci_period, PRICE_CLOSE, n + 1);

  // condition: descending and lower than the sell level
  return (cci0 < cci_sell_level && cci0 <= cci1);
}

// NOTE: MA SYSTEM
// ------------------------------------------------------------------
bool maSystemHaveUpTrend()
{
  bool t1 = true;
  bool t2 = true;
  bool t3 = true;
  ...
  if (t1 == false || t2 == false || t3 == false) return false;

  return true;
}
bool maSystemHaveDnTrend()
{
  bool t1 = true;
  bool t2 = true;
  bool t3 = true;
...
int trend_ma1()
{
  if (ma1.index(1) > ma1.index(2) && Bid > ma1.index(1)) return 0;
  if (ma1.index(1) < ma1.index(2) && Bid < ma1.index(1)) return 1;

  return -1;
}
int trend_ma2()
{
  if (ma2.index(1) > ma2.index(2) && Bid > ma2.index(1)) return 0;
  if (ma2.index(1) < ma2.index(2) && Bid < ma2.index(1)) return 1;

  return -1;
}
int trend_ma3()
{
  if (ma3.index(1) > ma3.index(2) && Bid > ma3.index(1)) return 0;
  if (ma3.index(1) < ma3.index(2) && Bid < ma3.index(1)) return 1;
...
bool rsiTrendUp()
{
  double rsi1 = iRSI(NULL, 0, rsi1Period, rsi1AppliedPrice, 1);
  double rsi2 = iRSI(NULL, 0, rsi2Period, rsi2AppliedPrice, 2);
...
class Counter
{
 ...
 public:
  ...
  Counter(int initial) { _value = initial; }
  ~Counter() { ; }

  int  current(void) { return _value; }
  void add(int value = 1) { _value += value; }
  void rest(int value = 1) { _value -= value; }
...
class CounterLosses
{
 ...
 public:
  CounterLosses(int limit, int magic) : _limit(limit), _magic(magic)
  {
    _bloqued = false;
  }
  ~CounterLosses() { ; }
  ...
  bool ReachLimit()
  {
    ...
    for (int i = OrdersHistoryTotal() - 1; i >= 0; i--)
    {
      if (OrderSelect(i, SELECT_BY_POS, MODE_HISTORY) && OrderSymbol() == _Symbol && OrderMagicNumber() == _magic)
      {
        ...
        {
          _count.add();
          if (_count.current() >= _limit && _limit > 0)
          {
            _bloqued = true;
            return true;
          }
...
CounterLosses* countLosses;

bool ReachMarginLimit()
{
  double marginLevel = AccountInfoDouble(ACCOUNT_MARGIN_LEVEL);
  if (marginLevel > 0)
    if (marginLevel <= uMarginLimit && uMarginLimit > 0)
    {
      return true;
    }

  // Print("margin: ", AccountInfoDouble(ACCOUNT_MARGIN));
  // Print("margin free: ", AccountInfoDouble(ACCOUNT_MARGIN_FREE));
