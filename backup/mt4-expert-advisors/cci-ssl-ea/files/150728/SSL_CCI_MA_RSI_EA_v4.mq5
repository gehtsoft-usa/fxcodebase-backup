...
string file_custom_indicator = "ssl-channel-chart-indicator.ex5";
...
double             rsi1LevelUp      = 70;
double             rsi1LevelDn      = 30;
...
double             rsi2LevelUp      = 70;
double             rsi2LevelDn      = 30;
...
RSI rsi1(NULL, 0, rsi1Period, rsi1AppliedPrice, rsi1LevelUp, rsi1LevelDn);
RSI rsi2(NULL, 0, rsi2Period, rsi2AppliedPrice, rsi2LevelUp, rsi2LevelDn);

#ifdef MAX_TRADES_AT_SAME_TIME
input int uMaxTrades = 1;  // Max Trades At Same Time:
#endif MAX_TRADES_AT_SAME_TIME

input int LSMAShortPeriod = 7;  //
input int LSMALongPeriod  = 16;
input int PipsDifference  = 16;
...
class Order
{
 ...
 public:
  ...
  //  double         profit()     { return _profit; }
	 ...
	 double         profit()
	 { 
		Print(__FUNCTION__," _id: ",_id);
		// if (OrderSelect(_id))
		//  { 
			// long tkPos = OrderGetInteger(ORDER_POSITION_ID); 
			// Print(__FUNCTION__," tkPos: ",tkPos);
	  	//  if(PositionSelectByTicket(tkPos)) 
...
//   ssl_handle = iCustom(NULL, 0, file_custom_indicator,Lb);
...
// #endif
	...
	ssl_handle = iCustom(NULL, 0, file_custom_indicator, Lb);
  cci_handle = iCCI(NULL, 0, cci_period, PRICE_CLOSE);
	rsi1_handle = iRSI(NULL, 0, rsi1Period, rsi1AppliedPrice);
	rsi2_handle = iRSI(NULL, 0, rsi2Period, rsi2AppliedPrice);
...
#ifdef MOVING_AVERAGE_ON
  ma1 = new MovingAverage(_Symbol, Period());
  ma2 = new MovingAverage(_Symbol, Period());
  ma3 = new MovingAverage(_Symbol, Period());
...
void OnTick()
{
  ...
  // NOTE: close control
  // ------------------------------------------------------------------
if(conditionsToCloseBuy.qntConditions()>0)
  if (conditionsToCloseBuy.EvaluateConditions()) {
      closeAll("buy");
  }

if (conditionsToCloseSell.qntConditions() > 0)
...
// NOTE: SSL signals
// ------------------------------------------------------------------
double value(int buffer=0, int shift=0)
{
  double value[1];
  int    copy = CopyBuffer(ssl_handle, buffer, shift, 1, value);
  if (copy > 0) { return value[0]; }
...
bool sslCrossUp()
{
  ...
  int    bufferUp = 1;
  int    shift    = 1;
  double up0      = value(bufferUp, shift);
  double up1      = value(bufferUp, shift+1);
  double dn0      = value(bufferDn, shift);
  double dn1      = value(bufferDn, shift + 1);

  if (up0 > dn0 && up1 <= dn1) { return true; }
...
// NOTE: CCI signals
// ------------------------------------------------------------------
double cci_value(int shift=0)
{
  double value[1];
  int    copy = CopyBuffer(cci_handle, 0, shift, 1, value);
  if (copy > 0) { return value[0]; }
...
{
  int    n    = 1;
  double cci0 = cci_value(n);
  double cci1 = cci_value(n+1);
...
// NOTE: RSI Filter
// ------------------------------------------------------------------
double rsi1Value(int shift=0)
{
  double value[1];
  int    copy = CopyBuffer(rsi1_handle, 0, shift, 1, value);
  if (copy > 0) { return value[0]; }
  
	return -1;
}
double rsi2Value(int shift=0)
{
  double value[1];
  int    copy = CopyBuffer(rsi2_handle, 0, shift, 1, value);
  if (copy > 0) { return value[0]; }
...
bool rsiTrendUp()
{
  double rsi1 = rsi1Value(1);
  double rsi2 = rsi2Value(2);
