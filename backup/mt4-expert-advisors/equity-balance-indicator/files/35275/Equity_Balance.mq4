...
int start()
  {
    ...
    ArrayInitialize(EquityArray,AccountEquity());
    ArrayInitialize(BalanceArray,AccountBalance());
    ...
    for (i=0;i<ArraysSize;i++)
    {
     ...
     for (i2=limit;i2>CloseBar;i2--)
     {
      if (i2<=OpenBar) d=(ClosePrices[i]-iClose(Symbols[i],0,i2))/(ClosePrices[i]-OpenPrices[i])*Profits[i]; else d=Profits[i];
      EquityArray[i2]=EquityArray[i2]-d;
      if (CloseBar!=0) BalanceArray[i2]=BalanceArray[i2]-Profits[i];
     }
    }
    for (i=limit;i>=0;i--)
    {
