...
#property indicator_label1 "LineUp"
#property indicator_type1  DRAW_NONE
#property indicator_color1 clrBlue
#property indicator_style1 STYLE_SOLID
#property indicator_width1 2

#property indicator_label2 "LineDn"
#property indicator_type2  DRAW_NONE
#property indicator_color2 clrRed
#property indicator_style2 STYLE_SOLID
#property indicator_width2 2

#property indicator_label3 "LineDn"
...
// NOTE: Inputs
// ------------------------------------------------------------------
input int    periods               = 5;                     // Range
input string T0                    = "== Set Arrows ==";    // Set Arrows
input bool   ArrowsOn              = true;                  // Arrows On?
input color  ArrowUpClr            = DodgerBlue;               // Arrow Up Color:
...
void OnInit()
{
    ...
    SetIndexBuffer(0, Draw_DnBand); 
    SetIndexBuffer(1, Draw_UpBand); 

    SetIndexBuffer(2, DnBand, INDICATOR_CALCULATIONS); PlotIndexSetInteger(2, PLOT_DRAW_TYPE, DRAW_NONE);
    SetIndexBuffer(3, UpBand, INDICATOR_CALCULATIONS); PlotIndexSetInteger(3, PLOT_DRAW_TYPE, DRAW_NONE);

    SetIndexBuffer(4, ArrowUp, INDICATOR_DATA);
    PlotIndexSetInteger(4, PLOT_ARROW, 233);
    PlotIndexSetInteger(4, PLOT_LINE_COLOR, DodgerBlue);

    SetIndexBuffer(5, ArrowDn, INDICATOR_DATA);
    PlotIndexSetInteger(5, PLOT_ARROW, 234);
    PlotIndexSetInteger(5, PLOT_LINE_COLOR, Tomato);
                ...
                const long &tick_volume[], const long &volume[], const int &spread[])
{
    ...
    for (int i = start; i < rates_total; i++) {              
        int s = iBars(NULL, 0) - i-1;
        DnBand[i]      = iLow(NULL, 0, iLowest(NULL, 0, MODE_LOW, periods, s));
        UpBand[i]      = iHigh(NULL, 0, iHighest(NULL, 0, MODE_HIGH, periods, s));
        Draw_DnBand[i] = EMPTY_VALUE;
        Draw_UpBand[i] = EMPTY_VALUE;
        ArrowUp[i]     = EMPTY_VALUE;
        ArrowDn[i]     = EMPTY_VALUE;
        Trend[i]       = Trend[i - 1] == EMPTY_VALUE ? 0 : Trend[i - 1];

        // determinar ta tendencia
        if (close[i] > UpBand[i - 1] && Trend[i - 1] == 0) {
            Trend[i] = 1;
        }
        if (close[i] < DnBand[i - 1] && Trend[i - 1] == 1) {
            Trend[i] = 0;
        }

        if (Trend[i] == 1) {
            Draw_DnBand[i] = DnBand[i - 1] < DnBand[i - 2] ? DnBand[i - 2] : DnBand[i - 1];
            if (Trend[i - 1] == 0) {
        ...
        if (Trend[i] == 0) {
            Draw_UpBand[i] = UpBand[i - 1] > UpBand[i - 2] ? UpBand[i - 2] : UpBand[i - 1];
            if (Trend[i - 1] == 1) {
