...
// https://fxcodebase.com/code/viewtopic.php?f=38&p=143717#p143717
// +------------------------------------------------------------------------------------------------+
...
class ACondition : public AConditionBase
{
  ...
  public:
    ACondition(const string symbol, ENUM_TIMEFRAMES timeframe, string name = "") : AConditionBase(name)
    {
...
class ActOnSwitchCondition : public ACondition
{
  ...
  public:
    ActOnSwitchCondition(string symbol, ENUM_TIMEFRAMES timeframe, ICondition *condition) : ACondition(symbol, timeframe)
    {
        _last        = false;
        _current     = false;
        _currentDate = 0;
    ...
    ~ActOnSwitchCondition() { _condition.Release(); }
    ...
    virtual string GetLogMessage(const int period, const datetime date) { return "Switch of (" + _condition.GetLogMessage(period, date) + (IsPass(period, date) ? ")=true" : ")=false"); }
};
...
class AndCondition : public AConditionBase
{
  ...
  public:
    ...
    virtual bool IsPass(const int period, const datetime date)
    {
        int size = ArraySize(_conditions);
        for (int i = 0; i < size; ++i) {
            if (!_conditions[i].IsPass(period, date)) return false;
        }
        return true;
    }
...
class SimplePriceStream : public AStream
{
  ...
  public:
    SimplePriceStream(const string symbol, const ENUM_TIMEFRAMES timeframe, const PriceType __price) : AStream(symbol, timeframe) { _price = __price; }
...
class Signaler
{
  ...
  public:
    ...
    void SendNotifications(const string subject, string message = NULL)
    {
        ...
        if (advanced_alert && advanced_key != "" && !IsTesting()) AdvancedAlert(advanced_key, message, "", "");
    }
};
...
class AlertSignalCandleColor : public IAlertSignalOutput
{
  ...
  public:
    AlertSignalCandleColor() { _candleStreams = new CandleStreams(); }

    ~AlertSignalCandleColor() { delete _candleStreams; }

    int Register(int id, color clr) { return _candleStreams.RegisterStreams(id, clr); }

    void Init() { _candleStreams.Init(); }

    virtual void Clear(int period) { _candleStreams.Clear(period); }

    virtual void Set(int period) { _candleStreams.Set(period, Open[period], High[period], Low[period], Close[period]); }
};
...
class AlertSignalArrow : public IAlertSignalOutput
{
  ...
  public:
    AlertSignalArrow() { _price = NULL; }

    ~AlertSignalArrow()
    {
        if (_price != NULL) _price.Release();
    }
    ...
    void Init() { ArrayInitialize(_signals, EMPTY_VALUE); }

    virtual void Clear(int period) { _signals[period] = EMPTY_VALUE; }

    virtual void Set(int period)
    {
        double price;
        if (!_price.GetValue(period, price)) return;
         
        int arrow_pos = period + 1;
        if (arrow_pos < ArraySize(_signals)) 
        _signals[arrow_pos] = price;
    }
};
...
class MainChartAlertSignalArrow : public IAlertSignalOutput
{
  ...
  public:
    ...
    virtual void Set(int period)
    {
      ...
      int arrow_pos = period + 1;
      if (arrow_pos >= Bars) return; // Evita desbordamiento

        ResetLastError();
        string id = _labelId + TimeToString(Time[arrow_pos]);
        if (ObjectFind(0, id) == -1) {
            if (!ObjectCreate(0, id, OBJ_TEXT, 0, Time[arrow_pos], price)) {
        ...
        ObjectSetInteger(0, id, OBJPROP_TIME, Time[arrow_pos]);
...
class StreamWrapper : public AStream
{
  ...
  public:
    StreamWrapper(const string symbol, const ENUM_TIMEFRAMES timeframe) : AStream(symbol, timeframe)
    {
        _symbol    = symbol;
        _timeframe = timeframe;
    }
...
class SetLastSignalAction : public AAction
{
  ...
  public:
    SetLastSignalAction(int signal) { _signal = signal; }
...
class LastSignalNotCondition : public AConditionBase
{
  ...
  public:
    LastSignalNotCondition(int signal) { _signal = signal; }

    bool IsPass(const int period, const datetime date) { return last_signal_side != _signal; }
};
...
int CreateAlert(int id, ENUM_TIMEFRAMES tf, color upColor, color downColor)
{
    ...
    upCondition.Add(new RSIUpCondition(_Symbol, tf), false);
    upCondition.Add(new StochUpCondition(_Symbol, tf), false);
    ...
    downCondition.Add(new RSIDownCondition(_Symbol, tf), false);
    downCondition.Add(new StochDownCondition(_Symbol, tf), false);
...
bool IsRed(string symbol, ENUM_TIMEFRAMES tf, int period) { return iClose(symbol, tf, period) < iOpen(symbol, tf, period); }
class UpCondition : public ACondition
{
  public:
    UpCondition(const string symbol, ENUM_TIMEFRAMES timeframe) : ACondition(symbol, timeframe) {}

    virtual bool IsPass(const int period, const datetime date)
    {
        return !IsRed(_symbol, _timeframe, period) && !IsRed(_symbol, _timeframe, period + 1) && IsRed(_symbol, _timeframe, period + 2) &&
               iMA(_symbol, _timeframe, ma_period, 0, ma_method, PRICE_CLOSE, period) < iClose(_symbol, _timeframe, period);
    }
};
...
{
  public:
    DownCondition(const string symbol, ENUM_TIMEFRAMES timeframe) : ACondition(symbol, timeframe) {}
...
{
  public:
    RSIUpCondition(const string symbol, ENUM_TIMEFRAMES timeframe) : ACondition(symbol, timeframe) {}
...
{
  public:
    RSIDownCondition(const string symbol, ENUM_TIMEFRAMES timeframe) : ACondition(symbol, timeframe) {}
...
{
  public:
    StochUpCondition(const string symbol, ENUM_TIMEFRAMES timeframe) : ACondition(symbol, timeframe) {}

    virtual bool IsPass(const int period, const datetime date)
    {
        double stochValue       = iStochastic(_symbol, _timeframe, K_periods, D_periods, Slowing, MODE_SMA, 0, MODE_MAIN, period);
        double stochSignalValue = iStochastic(_symbol, _timeframe, K_periods, D_periods, Slowing, MODE_SMA, 0, MODE_SIGNAL, period);
        return stochSignalValue < stochValue;
    }
};
...
{
  public:
    StochDownCondition(const string symbol, ENUM_TIMEFRAMES timeframe) : ACondition(symbol, timeframe) {}
...
int init()
{
    ...
    IndicatorBuffers(8);
    IndicatorObjPrefix = GenerateIndicatorPrefix("Bastos1983");
    IndicatorShortName("Bastos1983");
...
int start()
{
    ...
    int minBars = 1;
    int limit   = MathMin(Bars - 1 - minBars, Bars - counted_bars - 2);
    for (int pos = limit; pos >= 0 && !IsStopped(); --pos) {
        ...
        if (current_signal_date < Time[pos] && current_signal_side != 0) {
            ...
            current_signal_date = Time[signal_mode == SingalModeOnBarClose ? pos + 2 : pos+1];
        }
