...
//https://fxcodebase.com/code/viewtopic.php?f=38&t=72523
//+------------------------------------------------------------------------------------------------+
...
#property indicator_color5  DodgerBlue//C'0,128,255'
#property indicator_width5 3
#property indicator_color6  MediumVioletRed//C'192,0,192'
...
string  capture_time =    Time[i]; 
            
            if  (  alert_notification    ==  true  &&   StringCompare( capture_recent_time ,  capture_time)  == -1) {
             if   (  previousUpMove    == "NONE"    ||  previousUpMove   !=    Up[i])  {
             previousUpMove     =    Up[i];
             Alert  ( "Up  Move :"   ,Up[i] ) ;
             SendNotification  ( "Up  Move   :"   +    Up[i]) ; 
             }
      ...
      if(z<0 && Dn[i]>0)
        {
         ...
         if(i==1)
            pDn[0]=Dn[i];
             capture_time =    Time[i]; 
              if  (  alert_notification   == true  &&   StringCompare( capture_recent_time ,  capture_time)  == -1)  {
            if  (   previousDownMove    == "NONE"   ||   previousDownMove   !=   Dn[i])  {
            previousDownMove   =   Dn[i];
            Alert  ( "Down  Move  :"  ,   Dn[i])  ; 
            SendNotification  ( "Down  Move   :"   +    Dn[i]) ;
            }
...
string  previousUpMove   =      "NONE" ; 
string  previousDownMove  =   "NONE"; 
extern bool   alert_notification     =      true;   //  Alert Send Notification
...
int init()
  {
   ...
   SetIndexBuffer(4,Up);
   SetIndexStyle(4,DRAW_ARROW,EMPTY,3);//Green up arrow
   SetIndexArrow(4,108);
   SetIndexEmptyValue(4,0.0);
   SetIndexBuffer(5,Dn);
   SetIndexStyle(5,DRAW_ARROW,EMPTY,3);//Red down arrow
   SetIndexArrow(5,108);
   SetIndexEmptyValue(5,0.0);
   SetIndexBuffer(6,pUp);
   SetIndexStyle(6,DRAW_ARROW,EMPTY,3);//Green up markers
   SetIndexArrow(6,104);
   SetIndexEmptyValue(6,0.0);
   SetIndexBuffer(7,pDn);
   SetIndexStyle(7,DRAW_ARROW,EMPTY,3);//Red down markers
   SetIndexArrow(7,104);
