...
public:
   ...
   {
      double value = iCustom(_symbol, _timeframe, "MTF_Smart with inputs", 0, RSIPeriod, MFIPeriod, STOCHPeriodK, STOCHPeriodD, 
         STOCHSlowing, MfiHigh, MfiLow, RsiHigh, RsiLow, StochHigh, StochLow, 0, period);
...
public:
   ...
   {
      double value = iCustom(_symbol, _timeframe, "MTF_Smart with inputs", 0, RSIPeriod, MFIPeriod, STOCHPeriodK, STOCHPeriodD, 
         STOCHSlowing, MfiHigh, MfiLow, RsiHigh, RsiLow, StochHigh, StochLow, 1, period);
      return value != 0 && value != EMPTY_VALUE;
   }
};
...
int init()
{
   ...
   double temp = iCustom(NULL, 0, "MTF_Smart with inputs", 0, 0);
   if (GetLastError() == ERR_INDICATOR_CANNOT_LOAD)
   {
      Alert("Please, install the 'MTF_Smart with inputs' indicator");
      return INIT_FAILED;
   }

   IndicatorName = GenerateIndicatorName("Smart with inputs");
