...
// http://fxcodebase.com/code/viewtopic.php?f=38&t=68920
...
int FindLastPos(int period, string symbol, ENUM_TIMEFRAMES timeframe, double& val)
{
   for (int i = 0; i < 1000; ++i)
   {
      val = iCustom(symbol, timeframe, "ZigZag-Integer v1.1", Depth, Deviation, Backstep, 0, period + i);
      if (val != EMPTY_VALUE)
         return period + i;
      val = iCustom(symbol, timeframe, "ZigZag-Integer v1.1", Depth, Deviation, Backstep, 1, period + i);
      if (val != EMPTY_VALUE)
         return period + i;
   }
   return -1;
...
public:
   ...
   {
      double val;
      int pos = FindLastPos(period, _symbol, _timeframe, val);
      if (pos == -1 || iTime(_symbol, _timeframe, pos) == _lastEntry)
         return false;
      double prev_val;
      int prev_pos = FindLastPos(pos + 1, _symbol, _timeframe, prev_val);
      if (val < prev_val)
         return false;
      _lastEntry = iTime(_symbol, _timeframe, pos);
      return true;
   }
};
...
int OnInit()
{
   double temp = iCustom(NULL, 0, "ZigZag-Integer v1.1", 0, 0);
   if (GetLastError() == ERR_INDICATOR_CANNOT_LOAD)
   {
      Alert("Please, install the 'ZigZag-Integer v1.1' indicator");
      return INIT_FAILED;
   }
#ifdef SHOW_ACCOUNT_STAT
   stats = NULL;
