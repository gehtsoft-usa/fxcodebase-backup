...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=71885
...
enum Display {
   adaptative,   // Adaptative display algoritm
   allChannels,  // All Channels
};
input string          ICustom           = "== AutoTrendChannels Setup ==";  // == AutoTrendChannels Setup ==
input ENUM_TIMEFRAMES uTFLine           = PERIOD_H1;                        // Channel to Trade:
string                Channel_Settings  = "";
input int             BarAnalys         = 200;
input int             Accuracy          = 2;
input int             WidthLineFactor   = 0;
string                TColor_Options    = "";
input CScheme         ColorScheme       = Auto;
string                TAlgoritm_Display = "";
input Display         Algoritm_Display  = adaptative;
// ------------------------------------------------------------------
...
class AutoTrendChannels : public iIndicators
{
   string _symbol;
   int    _tf;
   string _file;
  ...
  public:
   AutoTrendChannels(string Symbol, int TimeFrame)
   {
      _symbol = Symbol;
      _tf     = TimeFrame;
      _file   = "AutoTrendChannels.ex4";
   }
   ~AutoTrendChannels() { ; }

   AutoTrendChannels* file(string setfile)
   {
      _file = setfile;
      return &this;
   }
   ...
   void setLines()
   {
      _buyLine  = "";
      _sellLine = "";
      // recorrer todos los objetos del chart // ver cual coindide le descripción con el tf del usuario
      int    i   = 0;
      string uTF = getTimeFrame(uTFLine);
      string txt = "";
      while (uTF != txt && i <= 20)
      {
         string name = StringConcatenate("HeaderCanal_", i);
         if (ObjectFind(0, name) >= 0)
         {
            ObjectGetString(0, name, OBJPROP_TEXT, 0, txt);
         }
         i++;
      }
      ...
      if (uTF == txt)
      {
         _buyLine  = StringConcatenate("LineChannel9_", i - 1, " DOWN3");
         _sellLine = StringConcatenate("LineChannel9_", i - 1, " UP3");
      }
   }
   double BuyPrice()
   {
      return ObjectGetValueByShift(_buyLine, 1);
   }
   double SellPrice()
   {
      return ObjectGetValueByShift(_sellLine, 1);
   }
   ...
   string getTimeFrame(int tf)
   {
      ...
      return IntegerToString(uTFLine);
   }

};
AutoTrendChannels* autoTrend;
...
class BUYcondition1 : public iConditions
{
  public:
   bool evaluate()
   {
      ...
      // autoTrend.calculate(0, 0);
      Print(autoTrend.BuyPrice());
      double buyPrice = NormalizeDouble(autoTrend.BuyPrice(), _Digits);
      if (Bid <= buyPrice && Ask > buyPrice)
      {
         return true;
      }
...
class SELLcondition1 : public iConditions
{
  public:
   bool evaluate()
   {
      ...
      Print(autoTrend.SellPrice());
      double sellPrice = NormalizeDouble(autoTrend.SellPrice(), _Digits);
      if (Ask >= sellPrice && Bid < sellPrice)
      {
         return true;
      }
...
int OnInit()
{
   ...
   //--- INDICATORS SETUPS:
   autoTrend = new AutoTrendChannels(_Symbol, Period());
   autoTrend.calculate(0, 0);
