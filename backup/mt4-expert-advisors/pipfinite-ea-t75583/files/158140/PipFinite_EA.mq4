...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=75583
...
input string    Indi = "== Indicator Setup =="; // ————————————————————————
string in = "";
input int per = 10; // Period
input double rf = 1.0; // Range Filter

#define INP in,per,rf,1
...
double TakeProfitByPipFinite()
{
    return Indi(bufferTP2, 1);   
}
...
#define Section_Take_Partial
#ifdef Section_Take_Partial
// MARK: Take Partial

input string TclosePartial    = "== Partial Close Setup =="; // ————————————————————————
input bool   closePartialOn   = true;                        // Take Partial On?
input double percent_to_close = 50;                          //  Percent to close:
int bufferPartial = 6;


class ConditionToTakePartialOnBuy : public iConditions
{
    double price;
  ...
  public:
    ...
    bool evaluate()
    {
        if (!closePartialOn) return false;
...
ConditionToTakePartialOnBuy cdToTakePartialOnBuy;

class ConditionToTakePartialOnSell : public iConditions
{
    double price;
...
ConditionToTakePartialOnSell cdToTakePartialOnSell;
...
class ActionClosePartial : public iActions
{
  ...
  public:
    bool execute()
    {
        ...
        double l     = NormalizeDouble(OrderLots() * percent_to_close / 100, 2);

        int r = OrderClose(OrderTicket(), l, price, 0, CLR_NONE);
        if (r >= 0) executed = true;
        return true;
    }

    void allowToTakePartial() { executed = false; }
};
ActionClosePartial acClosePartial;

Strategy stClosePartialBuy;
Strategy stClosePartialSell;


// . cuando se abre una nueva orden hay que setear el precio para la toma de parcial:
//     cdToTakePartialOnBuy.setPrice(precio_para_parcial_en_buy)
//     cdToTakePartialOnSell.setPrice(precio_para_parcial_en_sell)
// . y hay que activar el tomador de parciales para que solo lo haga una vez
//     acClosePartial.allowToTakePartial();

void OnInitClosePartial()
{
    stClosePartialBuy.set(&cdToTakePartialOnBuy, &acClosePartial);
    stClosePartialSell.set(&cdToTakePartialOnSell, &acClosePartial);

    Trader.management.add(&stClosePartialBuy);
    Trader.management.add(&stClosePartialSell);
}
...
class OrderCreator : public iActions
{
  ...
  public:
    ...
    bool execute()
    {
        ...
        // seteo la toma de parciales
        if(result >0){ 
           if(_cmd == OP_BUY) cdToTakePartialOnBuy.setPrice(Indi(bufferPartial, 1)); 
           if(_cmd == OP_SELL) cdToTakePartialOnSell.setPrice(Indi(bufferPartial, 1)); 
           acClosePartial.allowToTakePartial();
        }
...
input const string indicator_file = "PipFinite Breakout EDGE_fix";   // Indicator File:
//---
int  bufferToBuy              = 0;
int  bufferToSell             = 1;
...
//---
double Indi(int buffer, int candle = 1) { return iCustom(NULL, 0, indicator_file, INP, buffer, candle); }
...
{
    double temp = iCustom(NULL, 0, indicator_file, INP, 0, 0);
