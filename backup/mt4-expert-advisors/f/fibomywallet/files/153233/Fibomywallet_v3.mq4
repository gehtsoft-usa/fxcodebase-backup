//Available @  https://fxcodebase.com/code/viewtopic.php?f=38&t=74330
...
input bool AutomaticChangeTPSL = true; // If change TP change SL:
...
class TslByPips : public iTSL
{
    ...
    void setInitialStep(Order* order)
    {
        ...
        if(order.type() == OP_SELL) { pointsToMove *= -1; }
...
void OnTick()
{
    ...
    if(TslON) tsl.doTSL();
    if(AutomaticChangeTPSL) AutomaticTPSLControl();
    ...
    if(AccountNumber() == account_number)
    {
        ...
        else
        {
            ...
            if(CrossingBuyLevels())
            {
                ...
                if(actionSendOrder.doAction()) { mainOrders.GetMarketOrders(); }
...
int DrawFibo()
{
    ...
    ObjectSet("FiboUp", OBJPROP_FIRSTLEVEL + 12, 2.000);   ObjectSetFiboDescription("FiboUp", 12, "(300.0%) -  %$");
