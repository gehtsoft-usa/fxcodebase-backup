...
// https://fxcodebase.com/code/viewtopic.php?f=38&p=159117#p159117
         ...
         for(j = 0; j < ArraySize(pairList); j++)
           {
            ...
            val0 = 0;
            c = 0;
            if((custom_indi_calc(sym, timeframes[i], 2, 0) > 0 && custom_indi_calc(sym, timeframes[i], 2, 0) != EMPTY_VALUE) ||
               (custom_indi_calc(sym, timeframes[i], 0, 0) > 0 && custom_indi_calc(sym, timeframes[i], 0, 0) != EMPTY_VALUE))
              {
               val0 = 1;
              }
            if((custom_indi_calc(sym, timeframes[i], 3, 0) > 0 && custom_indi_calc(sym, timeframes[i], 3, 0) != EMPTY_VALUE) ||
               (custom_indi_calc(sym, timeframes[i], 1, 0) > 0 && custom_indi_calc(sym, timeframes[i], 1, 0) != EMPTY_VALUE))
              {
               val0 = -1;
              }
...
//|                                                                  |
//+------------------------------------------------------------------+
double custom_indi_calc(string sym, ENUM_TIMEFRAMES tf, int buffer, int shift)
  {
   return iCustom(sym, tf, "FollowLine", BarsCount, BBprice, BBperiod, BBdeviations, MAmethod, MAperiod, UseATRfilter, ATRperiod, buffer, shift);
  }
