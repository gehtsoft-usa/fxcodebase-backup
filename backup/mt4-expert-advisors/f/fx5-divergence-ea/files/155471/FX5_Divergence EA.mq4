// Available @  https://fxcodebase.com/code/viewtopic.php?f=38&t=74900
...
input const string indicator_file      = "fx5_dema_macd_divergence_v2"; // Indicator 1 File:
input string       separator1          = "--- MACD / OSMA Settings ---";
input double       fastEMA             = 8.3896;
input double       slowEMA             = 17.5185;
input double       signal              = 9.0503;
input bool         UseOSMA             = false;
input bool         UseDema             = true;
input string       separator2          = "---Indicator Settings---";
input double       positiveSensitivity = 0.0001;
input double       negativeSensitivity = -0.0001;

input string       Tcustom2            = "==== Indicator filter ===="; // ————————————————————————
input const bool   filter_by_ssrr_on   = false;                        // Filter by zones SSRR On:
input const string Tnote               = "filter by SSRR";             // If want to use this filter you must to put this indicator in the chart:
input const string indicator_file2     = "Supply Demand Pro (3)";      // Indicator 2 File: (Optional filter)
...
{
    return iCustom(NULL, 0, indicator_file,
                   separator1,
                   fastEMA,
                   slowEMA,
                   signal,
...
class ConditionIndicatorBuy : public iConditions
{
  public:
    bool evaluate()
    {
        double indi = Indi(bufferToBuy, 3);
...
class ConditionIndicatorSell : public iConditions
{
  public:
    bool evaluate()
    {
        double indi = Indi(bufferToSell, 3);
        return indi > 0 && indi != EMPTY_VALUE;
    }
};
...
class ConditionHaveSupport : public iConditions
{
  public:
    bool evaluate()
    {
        string prefix = "SSSR#S";
        for (int n = ObjectsTotal(OBJ_RECTANGLE); n >= 0; n--) {
            string _name = ObjectName(n);
            if(ObjectType(_name)!= OBJ_RECTANGLE)continue;
            Print(__FUNCTION__, " _name: ", _name);
            if (StringSubstr(_name, 0, StringLen(prefix)) == prefix) {
                double price1 = ObjectGetDouble(0, _name, OBJPROP_PRICE1);
                double price2 = ObjectGetDouble(0, _name, OBJPROP_PRICE2);
                if(iLow(NULL,0,1) <= price1 && iClose(NULL,0,1) > price2 && price1 != 0 && price2 != 0) return true;
            }
        }
        return false;
    }
};
ConditionHaveSupport conditionHaveSupport;

class ConditionHaveResistance : public iConditions
{
  public:
    bool evaluate()
    {

        string prefix = "SSSR#R";
        for (int n = ObjectsTotal(); n >= 0; n--) {
            ...
            if (StringSubstr(_name, 0, StringLen(prefix)) == prefix) {
                ...
                if (iHigh(NULL,0,1) > price2 && iClose(NULL,0,1) < price1 && price1 != 0 && price2 != 0)
                    return true;
            }
        }
        return false;
...
ConditionHaveResistance conditionHaveResistance;
...
bool                    OnInit_CustomIndicator()
{
    ...
    if (filter_by_ssrr_on) {
        IndicatorStrategyLong.addCondition(&conditionHaveSupport);
        IndicatorStrategyShort.addCondition(&conditionHaveResistance);
    }
...
input bool   close_on_opposite_on = true;                          // Close On Opposite Signal:
...
ConditionCloseOnOppositeBuy conditionCloseOnOppositeBuy;
...
ConditionCloseOnOppositeSell conditionCloseOnOppositeSell;
...
{
    closeOppositeBuy.addCondition(&conditionCloseOnOppositeBuy);
    closeOppositeBuy.addAction(&actionCloseBuy);

    closeOppositeSell.addCondition(&conditionCloseOnOppositeSell);
    closeOppositeSell.addAction(&actionCloseSell);
