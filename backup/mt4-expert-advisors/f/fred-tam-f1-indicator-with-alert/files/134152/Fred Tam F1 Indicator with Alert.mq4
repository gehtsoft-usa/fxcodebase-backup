...
//http://fxcodebase.com/code/viewtopic.php?f=38&t=69892
...
public:
   ...
   {
      double max;
      if (cross_type == CrossHighLow)
      {
         int highestIndex = iHighest(_symbol, _timeframe, MODE_HIGH, Buy, period);
         max = iHigh(_symbol, _timeframe, highestIndex);
      }
      else
      {
         int highestIndex = iHighest(_symbol, _timeframe, MODE_CLOSE, Buy, period);
         max = iClose(_symbol, _timeframe, highestIndex);
      }
      return iClose(_symbol, _timeframe, period) >= max 
         && iClose(_symbol, _timeframe, period + 1) < max;
   }
};
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      ...
      if (cross_type == CrossHighLow)
      {
         int highestIndex = iLowest(_symbol, _timeframe, MODE_LOW, Sell, period);
         max = iLow(_symbol, _timeframe, highestIndex);
      }
      else
      {
         int highestIndex = iLowest(_symbol, _timeframe, MODE_CLOSE, Sell, period);
         max = iClose(_symbol, _timeframe, highestIndex);
      }
      return iClose(_symbol, _timeframe, period) <= max 
         && iClose(_symbol, _timeframe, period + 1) > max;
...
int init()
{
   ...
   IndicatorObjPrefix = GenerateIndicatorPrefix("fredtf1");
   IndicatorShortName("Fred Tam F1 Indicator with Alert");
