...
// http://fxcodebase.com/code/viewtopic.php?f=38&t=69633
//+------------------------------------------------------------------+
...
input int X_long = 20; // X Long
input int X_short = 85; // X Short
input bool use_second_stoch = false; // Use second stochastic
input int K_periods2 = 5; // Stoch %K 2
input int D_periods2 = 3; // Stoch %D 2
input int Slowing2 = 3; // Stoch Slowing 2
input int X_long2 = 30; // Y Long
input int X_short2 = 70; // Y Short
...
input int exit_with_bands = true; // Exit with bands
...
class StochLongCondition : public ACondition
{
   ...
   double _os;
public:
   StochLongCondition(const string symbol, ENUM_TIMEFRAMES timeframe, int kPeriod, int dPeriod, int slowing, double os)
      :ACondition(symbol, timeframe)
   {
      _kPeriod = kPeriod;
      _dPeriod = dPeriod;
   ...
   bool IsPass(const int period, const datetime date)
   {
      double stochValue0 = iStochastic(_symbol, _timeframe, _kPeriod, _dPeriod, _slowing, MODE_SMA, 0, MODE_MAIN, period);
      double stochValue1 = iStochastic(_symbol, _timeframe, _kPeriod, _dPeriod, _slowing, MODE_SMA, 0, MODE_SIGNAL, period);
      return stochValue0 > stochValue1 && stochValue0 < _os && stochValue1 < _os;
   }
};
...
class StochShortCondition : public ACondition
{
   ...
   double _ob;
public:
   StochShortCondition(const string symbol, ENUM_TIMEFRAMES timeframe, int kPeriod, int dPeriod, int slowing, double ob)
      :ACondition(symbol, timeframe)
   {
      _kPeriod = kPeriod;
      _dPeriod = dPeriod;
   ...
   bool IsPass(const int period, const datetime date)
   {
      double stochValue0 = iStochastic(_symbol, _timeframe, _kPeriod, _dPeriod, _slowing, MODE_SMA, 0, MODE_MAIN, period);
      double stochValue1 = iStochastic(_symbol, _timeframe, _kPeriod, _dPeriod, _slowing, MODE_SMA, 0, MODE_SIGNAL, period);
      return stochValue0 < stochValue1 && stochValue0 > _ob && stochValue1 > _ob;
   }
};
...
class RSIMALongCondition : public ACondition
{
public:
   RSIMALongCondition(const string symbol, ENUM_TIMEFRAMES timeframe)
      :ACondition(symbol, timeframe)
   {
   ...
   bool IsPass(const int period, const datetime date)
   {
      ...
      double summ = rsiValue;
      for (int i = 1; i < ma_period; ++i)
      {
         summ += iRSI(_symbol, _timeframe, rsi_period, PRICE_CLOSE, period + i);
      }
      
      return rsiValue > (summ / ma_period)
         && rsiValue > 40
         && (summ / ma_period) < 40;
   }
};

class RSIMAShortCondition : public ACondition
{
public:
   RSIMAShortCondition(const string symbol, ENUM_TIMEFRAMES timeframe)
      :ACondition(symbol, timeframe)
   {
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      double bbValue = iBands(_symbol, _timeframe, bb_period, bb_deviation, bb_shift, bb_price, MODE_UPPER, period);
      return iClose(_symbol, _timeframe, period) >= bbValue;
   }
};
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      double bbValue = iBands(_symbol, _timeframe, bb_period, bb_deviation, bb_shift, bb_price, MODE_LOWER, period);
      return iClose(_symbol, _timeframe, period) <= bbValue;
   }
};
...
ICondition* CreateLongCondition(string symbol, ENUM_TIMEFRAMES timeframe)
{
   ...
   condition.Add(new StochLongCondition(symbol, timeframe, K_periods, D_periods, Slowing, X_long), false);
   if (use_second_stoch)
   {
      condition.Add(new StochLongCondition(symbol, timeframe, K_periods2, D_periods2, Slowing2, X_long2), false);
   }
   if (use_macd)
   {
      condition.Add(new MACDAboveLevelCondition(symbol, timeframe, fast_ema_period, slow_ema_period, signal_period, 0, 0), false);
   }
   condition.Add(new RSIMALongCondition(symbol, timeframe), false);
...
ICondition* CreateShortCondition(string symbol, ENUM_TIMEFRAMES timeframe)
{
   ...
   AndCondition* condition = new AndCondition();
   condition.Add(new StochShortCondition(symbol, timeframe, K_periods, D_periods, Slowing, X_short), false);
   if (use_second_stoch)
   {
      condition.Add(new StochShortCondition(symbol, timeframe, K_periods2, D_periods2, Slowing2, X_short2), false);
   }
   if (use_macd)
   {
      condition.Add(new MACDBelowLevelCondition(symbol, timeframe, fast_ema_period, slow_ema_period, signal_period, 0, 0), false);
   }
   condition.Add(new RSIMAShortCondition(symbol, timeframe), false);
   #ifdef ACT_ON_SWITCH_CONDITION
      ActOnSwitchCondition* switchCondition = new ActOnSwitchCondition(symbol, timeframe, (ICondition*) condition);
      condition.Release();
      return switchCondition;
