...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      double stochValue0 = iStochastic(_symbol, _timeframe, K_periods, D_periods, Slowing, MODE_SMA, 0, MODE_MAIN, period);
      double stochValue1 = iStochastic(_symbol, _timeframe, K_periods, D_periods, Slowing, MODE_SMA, 0, MODE_SIGNAL, period);
      return stochValue0 > stochValue1 && stochValue0 < X_long && stochValue1 < X_long;
   }
};
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      double stochValue0 = iStochastic(_symbol, _timeframe, K_periods, D_periods, Slowing, MODE_SMA, 0, MODE_MAIN, period);
      double stochValue1 = iStochastic(_symbol, _timeframe, K_periods, D_periods, Slowing, MODE_SMA, 0, MODE_SIGNAL, period);
      return stochValue0 < stochValue1 && stochValue0 > X_short && stochValue1 > X_short;
   }
};
