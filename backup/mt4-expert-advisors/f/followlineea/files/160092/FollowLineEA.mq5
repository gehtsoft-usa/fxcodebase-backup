...
input const string indicator_file = "Follow_Line";                          // Indicator File:

input bool MultiTF = true; // Use Multi Time Frame Indicator
input ENUM_TIMEFRAMES tframeHTF = PERIOD_H4; // Time Frame for HIGHER Time Frame
input ENUM_TIMEFRAMES tframeLTF = PERIOD_CURRENT; // Time Frame for LOWER Time Frame
...
//---
int  handle = 0;
void setHandle() { handle = iCustom(Symbol(), tframeLTF, indicator_file); }
int  handleHTF = 0;
void setHandleHTF() { handleHTF = iCustom(Symbol(), tframeHTF, indicator_file); }
...
class ConditionIndicatorBuyHTF : public iConditions
{
  public:
    bool evaluate()
    {
        for (int i = 1; i <= candles_back_for_signals; i++) {
            double indiHTF = IndiHTF(2, i);
            if (indiHTF > 0 && indiHTF != EMPTY_VALUE) {
                return true;
            }
        }
        return false;
...
ConditionIndicatorBuyHTF conditionIndicatorBuyHTF;

class ConditionIndicatorSellHTF : public iConditions
{
  public:
    bool evaluate()
    {
        for (int i = 1; i <= candles_back_for_signals; i++) {
            double indiHTF = IndiHTF(3, i);
...
ConditionIndicatorSellHTF conditionIndicatorSellHTF;
...
bool OnInit_CustomIndicator()
{
    ...
    setHandle(); setHandleHTF();
    ShortStrategy.addCondition(&conditionIndicatorSell);
    LongStrategy.addCondition(&conditionIndicatorBuy);
    
    if (MultiTF) {
        LongStrategy.addCondition(&conditionIndicatorBuyHTF);
        ShortStrategy.addCondition(&conditionIndicatorSellHTF);
    }
...
ConditionIndicatorBuy cSignalBuy;
ConditionIndicatorSell cSignalSell;
...
{
    closeOppositeBuy.set(&cSignalSell, &acCloseBuy);
    closeOppositeSell.set(&cSignalBuy, &acCloseSell);
