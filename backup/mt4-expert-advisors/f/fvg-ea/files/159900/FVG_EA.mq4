...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=76147
...
input int  uCandlesBack     = 2;    // Contrary Candles Before FVG:
input bool strategy1_on     = true; // Strategy 1:
input bool strategy2_on     = true; // Strategy 2:
input int  max_candles_st_2 = 50;   // Candles Limits for strategy 2 after 1:
...
input bool           ma_on             = true;                           // MA filter On:
input int            ma1_period        = 50;                             // Periods:
input ENUM_MA_METHOD ma1_method        = MODE_EMA;                       // Method:
int                  ma1_shift         = 0;                              // Shift:
ENUM_APPLIED_PRICE   ma1_applied_price = PRICE_CLOSE;                    // Applied Price:
...
input double         uStopLossValue = 3;                 // SL value:
...
void OnInit_Spread()
{
    ...
    stBull2.addCondition(&cdSpread);
    stBear2.addCondition(&cdSpread);
}
...
input enum_dl_mode dl_mode          = dl_by_money;              // Daily Limits Mode:
input double       daily_win_limit  = 1000;                      // Daily Win Limit:
input double       daily_loss_limit = 1000;                      // Daily Loss Limit:
...
void OnInit_DayLimit()
{
    ...
    stBull2.addCondition(&cdDayLimit);
    stBear2.addCondition(&cdDayLimit);
}
    ...
    bool control()
    {
        if (count_on) {
            if (maturity == duration) {
                maturity = 0;
                count_on = false;
                Print("line: ", __LINE__, " maturity: ", maturity);
            } else {
                maturity += 1;
                Print("line: ", __LINE__, " maturity: ", maturity);
            }
...
Edge bull_strategy_edge;
Edge bear_strategy_edge;
...
class BUYcondition1 : public iConditions
{
  ...
  public:
    bool evaluate()
    {
        ...
        if (st_side == -1) return false;

        if (on) {
            int c = CountBearishCandles(3);
            if (c == uCandlesBack)
                if (up_fvg()) {
                    bull_strategy_edge.start();
                    on      = false;
                    st_side = 1;
                    return true;
                }
...
class SELLcondition1 : public iConditions
{
  ...
  public:
    bool evaluate()
    {
        ...
        if (on) {
            int c = CountBullishCandles(3);
            if (c == uCandlesBack)
                if (dn_fvg()) {
                    bear_strategy_edge.start();
                    on      = false;
                    st_side = -1;
                    return true;
                }
...
class ConditionTouchFVGBuy : public iConditions
{
  public:
    bool evaluate()
    {
        ...
        if (bull_strategy_edge.control()) {
            double op = iOpen(NULL, 0, 1);
            double lo = iLow(NULL, 0, 1);
            double cl = iClose(NULL, 0, 1);

            if (cl > op &&
                lo < last_fvg_price_up && cl > last_fvg_price_up) {
                bull_strategy_edge.stop();
                conditionBuy.activate();
                return true;
            }
        }
        return false;
...
ConditionTouchFVGBuy cdTouchFVGBuy;

class ConditionTouchFVGSell : public iConditions
{
  public:
    bool evaluate()
    {
        if (st_side == 1) return false;        
        if (bear_strategy_edge.control()) {
            double op = iOpen(NULL, 0, 1);
            double hi = iHigh(NULL, 0, 1);
            double cl = iClose(NULL, 0, 1);
            if (cl < op &&
                hi > last_fvg_price_dn && cl < last_fvg_price_dn) {
                bear_strategy_edge.stop();
                conditionSell.activate();
                return true;
            }
        }
        return false;
...
ConditionTouchFVGSell cdTouchFVGSell;
...
void BasicStrategy_Iniciate()
{
    if (strategy1_on) {
        ...
        if (ma_on) {
            LongStrategy.addCondition(&cdPriceAboveEma);
            ShortStrategy.addCondition(&cdPriceBelowEma);
        }
    }
    
    if (strategy2_on) {
        stBull2.set(&cdTouchFVGBuy, &buy);
        stBear2.set(&cdTouchFVGSell, &sell);
        if (ma_on) {
            stBull2.addCondition(&cdPriceAboveEma);
            stBear2.addCondition(&cdPriceBelowEma);
        }
    }
    ...
    Trader.strategys.add(&stBull2);
    Trader.strategys.add(&stBear2);
...
input int    count_max_buys      = 2;                           // # Buys limit:
input int    count_max_sells     = 2;                           // # Sells limit:
...
void OnTick()
{
    if (st_side != 0 && CountTrades() == 0)
    { 
        st_side = 0;
        conditionSell.activate();
        conditionBuy.activate();
        Print("line: ",__LINE__," st_side: ",st_side);
    }
    Trader.doTrading();
}
