...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=74799
...
string file_custom_indicator = "Forex_Gump";
int    bufferToBuy           = 0;
int    bufferToSell          = 1;
int    bufferToCloseSell     = 4;
int    bufferToCloseBuy      = 5;

input string Tindi      = "== Indicator Setup =="; // ————————————
input bool   infoOn     = true;                    // Information
input double short_wave = 2;                       // Short Wave
input int    period     = 1;                       // Period
input int    period_ma  = 22;                      // Period Ma
input bool closeByIndicator = true; // Close Trades by Indicator?

double Indi(int buffer, int candle)
{
    // TODO: copiar la lista de INPUTS para custom indicator:
    return iCustom(NULL, utf, file_custom_indicator,
                   infoOn, short_wave, period, period_ma,
                   buffer, candle);
}
#endif
...
class BUYcondition1 : public iConditions
{
  public:
    bool evaluate()
    {
        ...
        double indi2 = Indi(bufferToBuy, 2);
        return (indi > 0 && indi != EMPTY_VALUE) && (indi2==0 || indi2 == EMPTY_VALUE);
...
class SELLcondition1 : public iConditions
{
  public:
    bool evaluate()
    {
        ...
        double indi2 = Indi(bufferToSell, 2);
        return (indi > 0 && indi != EMPTY_VALUE) && (indi2==0 || indi2 == EMPTY_VALUE);
        return false;
    }
};
...
class ConditionToCloseBuy : public iConditions
{
  public:
    bool evaluate()
    {
        if(closeByIndicator)
        {
            double indi = Indi(bufferToCloseBuy, 1);
...
class ConditionToCloseSell : public iConditions
{
  public:
    bool evaluate()
    {
        ...
        if(closeByIndicator)
        {
            double indi = Indi(bufferToCloseSell, 1);
            return (indi > 0 && indi != EMPTY_VALUE);
        }
