...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=73493
...
input int    GridUser_gap        = 5;                  // Gap betwen orders (pips) - deviation:
...
//--- SESSIONS CONTROL:
  ...
  maSlow.setSetup(maPeriod, maShift, maMethod, maAppliedPrice);
  ...
  if (!uTradeReverse)
  {
    if (conditionsToBuy.EvaluateConditions())
    {
      ...
      if (actionSendOrder.doAction())
      {
        ...
        if (GridON == true && CheckPointer(gridBuy) == POINTER_INVALID)
        {
          gridBuy = new Grid(_Symbol, "buy", mainOrders.last().price(), GridUser_gap, GridUser_multiplier, GridUser_maxCount, GridUser_maxLot, mainOrders.last().lot(), magico, true, true, mainOrders.last().tp(), mainOrders.last().sl());
    ...
    if (conditionsToSell.EvaluateConditions())
    {
      ...
      if (actionSendOrder.doAction())
      {
        ...
        if (GridON == true && CheckPointer(gridSell) == POINTER_INVALID)
        {
          gridSell = new Grid(_Symbol, "sell", mainOrders.last().price(), GridUser_gap, GridUser_multiplier, GridUser_maxCount, GridUser_maxLot, mainOrders.last().lot(), magico,true, true, mainOrders.last().tp(), mainOrders.last().sl());
