...
string file_custom_indicator = "FractalSupportResistance.ex4";
 ...
 public:
  bool evaluate()
  {
    // TODO: condition Buy 2
			...
			for (int i = 0; i < 5; i++) 
			{
         if(iClose(NULL, 0, 1) > levels[i] && iLow(NULL, 0, 2) < levels[i]) return true;
      }
			
			// Breackout on resistance
			for (int i = 5; i < 10; i++) 
			{
         if(iClose(NULL, 0, 1) > levels[i] && iLow(NULL, 0, 1) < levels[i]) return true;
      }
		// iClose(NULL, 0, 1) > ma.index(1);
    // return 
    return false;
  }
};
 ...
 public:
  bool evaluate()
  {
    // TODO: condition sell 2
		...
		// reverse on resistance
		for (int i = 5; i < ArraySize(levels); i++) 
			{
         if(iClose(NULL, 0, 1) < levels[i] && iHigh(NULL, 0, 2) > levels[i]) return true;
      }
		
		// breackout on support
		for (int i = 0; i < 5; i++) 
			{
         if(iClose(NULL, 0, 1) < levels[i] && iHigh(NULL, 0, 1) > levels[i]) return true;
      }
    return false;
  }
};
...
// NOTE: Indicator functionality
// ------------------------------------------------------------------

double linePrice(string _name)
{
...
void updateSR()
{
	...
	string modificators[]    = {"support", "resistance"};
  string timeframe_names[] = {"H1", "H4", "D1", "W1", "MN1"};

  string lineName;
	for (int j = 0; j < ArraySize(modificators); j++) 
	{
		for (int n = 0; n < ArraySize(timeframe_names); n++) 
		{
       lineName  = modificators[j] + timeframe_names[n];
       int pos   = j == 0 ? n : n + 5;
       levels[pos] = linePrice(lineName);
		}
	}    
}
...
bool touchSupport()
{
	for (int i = 0; i < 5; i++) 
	{
    if(iClose(NULL, 0, 1) > levels[i] && Bid <= levels[i]) return true;
    if(iClose(NULL, 0, 1) < levels[i] && Bid >= levels[i]) return true;
  }	
	return false;
}
bool touchResistance()
{
	for (int i = 5; i < 10; i++) 
	{
    if(iClose(NULL, 0, 1) < levels[i] && Bid >= levels[i]) return true;
