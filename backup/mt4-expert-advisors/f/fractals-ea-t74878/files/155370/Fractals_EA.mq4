...
//https://fxcodebase.com/code/viewtopic.php?f=38&t=74878
...
input int          uCandlesBack            = 5;                          // Valid Signal from candles back:
...
class BUYcondition1 : public iConditions
{
 ...
 public:
  bool evaluate()
  {
    ...
    {
      double indi = iCustom(Symbol(), 0, "Reverse No Repair.ex4", 0, i);
      if (indi != EMPTY_VALUE && indi!=lastsignalbuy) { 
        lastsignalbuy = indi;
        return true; 
      } 
    }
...
class SELLcondition1 : public iConditions
{
 ...
 public:
  bool evaluate()
  {
    ...
    for (int i = 0; i < uCandlesBack; i++)
    {
      double indi = iCustom(Symbol(), 0, "Reverse No Repair.ex4", 1, i);
      if (indi != EMPTY_VALUE && indi!=lastsignal) { 
        lastsignal = indi;
        return true;
        }
    }
...
ConditionCountOrders* countTrades;
...
int OnInit()
{
  ...
  //--- buys:
  conditionsToBuy.AddCondition(countTrades = new ConditionCountOrders(1,magico));
  ...
  //--- sell:
	conditionsToSell.AddCondition(countTrades = new ConditionCountOrders(1, magico));
  ...
  for (int i = mainOrders.qnt() - 1; i >= 0; i--)
  {
    if (mainOrders.index(i).countPartials() == 0)
    {
      ...
      if (conditionsToPartialClose.EvaluateConditions())
      {
        ...
        partialCloseA999999ction.order(mainOrders.index(i)).percent(userPartialClosePercent);
