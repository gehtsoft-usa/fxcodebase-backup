...
input int WPR_Period = 13;
input double Level_Up = 28;
input double Level_Dn = 72;
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      ...
      if (lda_60[pos] >= Level_Up)
      {
         return false;
      }
      int li_116 = 1;
      while (pos - li_116 > 0 && lda_60[pos - li_116] >= Level_Up && lda_60[pos - li_116] <= Level_Dn)
      {
         ++li_116;
      }
      return pos - li_116 > 0 && lda_60[pos - li_116] > Level_Dn;
   }
};
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      ...
      return pos - li_116 > 0 && lda_60[pos - li_116] < Level_Up;
   }
};
...
void OnInit()
{
   IndicatorObjPrefix = GenerateIndicatorPrefix("ivjempol");
   IndicatorSetString(INDICATOR_SHORTNAME, "ovjempol");
   IndicatorSetInteger(INDICATOR_DIGITS, Digits());

   wpr1 = iWPR(_Symbol, _Period, WPR_Period);
   wpr2 = iWPR(_Symbol, _Period, 4);

   int id = 0;
   SetIndexBuffer(id, g_ibuf_100, INDICATOR_DATA);
   ...
   ++id;
   SetIndexBuffer(id, g_ibuf_104, INDICATOR_DATA);
   PlotIndexSetInteger(id, PLOT_LINE_STYLE, STYLE_SOLID);
   PlotIndexSetInteger(id, PLOT_DRAW_TYPE, DRAW_HISTOGRAM);
   PlotIndexSetInteger(id, PLOT_LINE_WIDTH, 1);
   ++id;
   SetIndexBuffer(id, lda_60, INDICATOR_CALCULATIONS);
   ++id;
...
void OnDeinit(const int reason)
{
   ...
   IndicatorRelease(wpr1);
   IndicatorRelease(wpr2);
                ...
                const int &spread[])
{
   if (prev_calculated <= 0 || prev_calculated > rates_total)
   {
      ...
      ArrayInitialize(lda_60, EMPTY_VALUE);
      for (int i = 0; i < ArraySize(alerts); ++i)
      {
         alerts[i].Init();
      }
   }
   int first = MathMax(9, WPR_Period);
   for (int pos = MathMax(first, prev_calculated - 1); pos < rates_total; ++pos)
   {
      ...
      for (int i = 0; i < 9; i++)
      {
         ld_36 += MathAbs(high[pos - i] - low[pos - i]);
      }
      double ld_28 = ld_36 / 10.0;
      int indi = wpr1;
      for (int i = 0; i < 6; i++)
      {
         if (MathAbs(close[pos - i - 3] - close[pos - i]) >= 4.6 * ld_28)
         {
            indi = wpr2;
            break;
         }
      ...
      lda_60[pos] = 100 - MathAbs(buffer[0]);
      double ld_12 = 0;
      if (lda_60[pos] < Level_Up)
      {
         int li_116 = 1;
         while (lda_60[pos - li_116] >= Level_Up && lda_60[pos - li_116] <= Level_Dn)
         {
            ++li_116;
         }
         if (lda_60[pos - li_116] > Level_Dn)
         {
            ld_12 = high[pos] + ld_28 / 2.0;
            g_ibuf_100[pos] = ld_12;
         }
      }
      if (lda_60[pos] > Level_Dn)
      {
         ...
         if (lda_60[pos - li_116] < Level_Up)
         {
            ld_12 = low[pos] - ld_28 / 2.0;
            g_ibuf_104[pos] = ld_12;
         }
      }
      for (int i = 0; i < ArraySize(alerts); ++i)
      {
