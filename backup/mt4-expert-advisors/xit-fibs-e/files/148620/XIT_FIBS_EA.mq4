...
// https://fxcodebase.com/code/viewtopic.php?f=38&p=148640
...
string file_custom_indicator = "XIT_FIBS_with_Colour.ex4";
...
input bool         closeAllInOpositeSignal = true;                       // Close all in opposite signal
 ...
 public:
  bool evaluate()
  {
    // TODO: condition Buy 1
		if(fibo.trend==1)
		{
      double pr = CuttingFiboPrice("buy");
			Print(__FUNCTION__," CuttingFiboPrice: ",pr);

      return CuttingFiboPrice("buy") != 0;
    }
 ...
 public:
  bool evaluate()
  {
    // TODO: condition sell 1
		if(fibo.trend==-1)
		{
			double pr = CuttingFiboPrice("sell");
			Print(__FUNCTION__," CuttingFiboPrice: ",pr);

      return CuttingFiboPrice("sell") != 0;
    }
		return false;
  }
};
...
// NOTE: refresh indicator
// ——————————————————————————————————————————————————————————————————

if(TheZoomChange())
{
    Print("THE ZOOM CHANGE !!!!!!!!!!!");
    setFiboLevels();
}

if(newCandle.IsNewCandle()) { setFiboLevels(); }
    ...
    switch (modeSL)
    {
      ...
      case Fiboprices:
        for (int i = 0; i < 5; i++)
        {
          if (side == "buy")
          {
            if (Ask < FiboPrice(i) && Ask > FiboPrice(i + 1)) result = FiboPrice(i + 1);
          }
          if (side == "sell")
          {
            if (Bid > FiboPrice(i) && Bid < FiboPrice(i + 1)) result = FiboPrice(i + 1);
          }
	    	}
        break;
    }
...
bool TheZoomChange()
{
  int curZoom = WindowBarsPerChart();
	if(curZoom != zoom)
	{
		zoom = curZoom;
  	return true;
...
void setFiboLevels()
{
  ...
  double temp = iCustom(NULL, 0, file_custom_indicator, 0, 0);
	...
	// Print(ObjectGet("XIT_FIBO", OBJPROP_FIRSTLEVEL + 0));
	double fiboPrice1=ObjectGet("XIT_FIBO",OBJPROP_PRICE1);
	double fiboPrice2=ObjectGet("XIT_FIBO",OBJPROP_PRICE2);

	Print("fiboPrice1: "+fiboPrice1);
	Print("fiboPrice2: "+fiboPrice2);

	double fiboPriceDiff = fiboPrice2-fiboPrice1;
	fibo.price0    = NormalizeDouble(fiboPrice2-fiboPriceDiff*0.0, _Digits);
	fibo.price23   = NormalizeDouble(fiboPrice2-fiboPriceDiff*0.236, _Digits);
	fibo.price38   = NormalizeDouble(fiboPrice2-fiboPriceDiff*0.382, _Digits);
	fibo.price50   = NormalizeDouble(fiboPrice2-fiboPriceDiff*0.50, _Digits);
	fibo.price61   = NormalizeDouble(fiboPrice2-fiboPriceDiff*0.618, _Digits);
	fibo.price100  = NormalizeDouble(fiboPrice2-fiboPriceDiff*1.0, _Digits);

	color colorTrend = ObjectGet("XIT_FIBO",OBJPROP_LEVELCOLOR);
	fibo.trend = colorTrend == clrRed ? -1 : 1;
	Print("colorTrend: " + colorTrend+" fibo.trend: "+fibo.trend);
}

void PrintLevels()
{
	Print("fibo.price0  "+fibo.price0  );
	Print("fibo.price23 "+fibo.price23 );
	Print("fibo.price38 "+fibo.price38 );
	Print("fibo.price50 "+fibo.price50 );
	Print("fibo.price61 "+fibo.price61 );
	Print("fibo.price100"+fibo.price100);
	Print("fibo.trend   "+fibo.trend   );
}

double CuttingFiboPrice(string side)
{
	double op = iOpen(NULL,0,0);
	
	if(side == "buy")
	{
    // if (op > fibo.price0   && Ask <= fibo.price0)   return fibo.price0;
    // if (op > fibo.price23  && Ask <= fibo.price23)  return fibo.price23;
    // if (op > fibo.price38  && Ask <= fibo.price38)  return fibo.price38;
    // if (op > fibo.price50  && Ask <= fibo.price50)  return fibo.price50;
    // if (op > fibo.price61  && Ask <= fibo.price61)  return fibo.price61;
    if (op > fibo.price100 && Ask <= fibo.price100) return fibo.price100;
	}
	if(side == "sell")
	{
	  // if (op < fibo.price0   &&  Bid >= fibo.price0)   return fibo.price0;
	  // if (op < fibo.price23  &&  Bid >= fibo.price23)  return fibo.price23;
	  // if (op < fibo.price38  &&  Bid >= fibo.price38)  return fibo.price38;
	  // if (op < fibo.price50  &&  Bid >= fibo.price50)  return fibo.price50;
	  // if (op < fibo.price61  &&  Bid >= fibo.price61)  return fibo.price61;
	  if (op < fibo.price100 &&  Bid >= fibo.price100) return fibo.price100;
	}
  
	return 0;
}

double FiboPrice(int index)
{
  if (index == 0) return fibo.price0;
  if (index == 1) return fibo.price23;
  if (index == 2) return fibo.price38;
  if (index == 3) return fibo.price50;
  if (index == 4) return fibo.price61;
  if (index == 5) return fibo.price100;
