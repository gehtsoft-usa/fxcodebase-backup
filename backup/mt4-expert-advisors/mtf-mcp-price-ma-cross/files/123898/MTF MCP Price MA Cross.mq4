...
// http://fxcodebase.com/code/viewtopic.php?f=38&t=67351
...
public:
   ...
   virtual void Draw()
   {
      for (int i = 0; i < iBars(_symbol, _timeframe); ++i)
      {
         int direction = GetDirection(i);
         if (direction != 0)
         {
            ObjectMakeLabel(_id, _x, _y, IntegerToString(i), GetDirectionColor(direction), 1, WindowNumber, "Arial", 10);
            return;
         }
      }
   }
...
private:
   ...
   int GetDirection(const int period)
   {
      ...
      double currentIndi = iMA(_symbol, _timeframe, maPeriod, 0, MODE_SMA, PRICE_CLOSE, period);
      double previousIndi = iMA(_symbol, _timeframe, maPeriod, 0, MODE_SMA, PRICE_CLOSE, period + 1);
      if (currentClose >= currentIndi && previousClose < previousIndi)
...
int init()
{
   IndicatorName = GenerateIndicatorName("MTF MCP Price MA Cross");
