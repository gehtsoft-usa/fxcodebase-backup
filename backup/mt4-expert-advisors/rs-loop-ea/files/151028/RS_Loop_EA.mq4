...
#define priceLineListOn
#ifdef priceLineListOn
...
class PriceLine
{
    ...
    public:
    PriceLine(double price, double distance = 0, int candlesBack = 0, bool draw = true)
    {
        _price = price;
        _extraPoints = distance;
        _candlesBack = candlesBack;
        _reboundUp = false;
        _reboundDn = false;
    ...
    void   SetExtraPoints(double inpextraPoints) { _extraPoints = inpextraPoints; }
    double Price() { return _price; }

    void Draw()
    {
        string name = "PriceLine: " + (string) _price;
        line = new HLine(name, 0, _price, clrBlack);
        line.redraw();
    }

    // detecta si el nivel fue cortado hacia arriba from candlesBack time
    bool CutUp()
    {
        double pricePlus = _price + (_extraPoints * _Point);
        for(int i = _candlesBack; i >= 0; i--)
        {
            if(close(1) > pricePlus && close(2) < _price)
            {
                _candleCutUp = i;
                _cutUp = true;
                line.changeColor(clrBlue);
                return true;
            }
        }
        line.changeColor(clrBlack);
        _cutUp = false;
        return false;
    }
    bool dojiUp()
    {
        for(int i = _candlesBack; i > 0; i--)
        {
            if(open(i) > _price && low(i) < _price && close(i) > _price)
            {
                _candleDojiUp = i;
                return true;
            }
    ...
    bool dojiDn()
    {
        for(int i = _candlesBack; i > 0; i--)
        {
            if(open(i) < _price && high(i) > _price && close(i) < _price)
            {
                _candleDojiDn = i;
                return true;
            }
    ...
    bool ReboundUp()
    {
        if(dojiUp() == true && _candleDojiUp == 1)
        {
            _reboundUp = true;
            return true;
        }
        if(CutDn() == true && CutUp() == true && _candleCutUp < _candleCutDn && _candleCutUp == 1)
        {
            _reboundUp = true;
            return true;
        }
    ...
    bool ReboundDn()
    {
        if(dojiDn() == true && _candleDojiDn == 1)
        {
            _reboundDn = true;
            return true;
        }
        if(CutDn() == true && CutUp() == true && _candleCutUp > _candleCutDn && _candleCutDn == 1)
        {
            _reboundDn = true;
            return true;
        }
    ...
    {
        line.changeColor(clrBlack);
        if(_cutUp)line.changeColor(clrBlue);
        if(_cutDn)line.changeColor(clrOrange);
        if(_reboundDn)line.changeColor(clrRed);
        if(_reboundUp)line.changeColor(clrGreen);

    }

    void setComment()
    {
        Comment("PRICE: " + _price + "\n" + " CUT UP: " + _cutUp + "\n" + " CUT DN: " + _cutDn + "\n" + " Rebound UP: " + _reboundUp + "\n" + " Rebound DN: " + _reboundDn);
    }

    bool getReboudUp() { return _reboundUp; }
    bool getReboudDn() { return _reboundDn; }

    double close(int i) { return iClose(NULL, 0, i); }
    double open(int i) { return iOpen(NULL, 0, i); }
    double high(int i) { return iHigh(NULL, 0, i); }
    double low(int i) { return iLow(NULL, 0, i); }
};
...
class PriceLineList
{
    ...
    void Add(double price, double distance)
    {
        int t = ArraySize(prLines);
        ArrayResize(prLines, t + 1);
        prLines[t] = new PriceLine(price, distance);
    }

    void Draw()
    {
        for(int i = 0; i < ArraySize(prLines); i++)
        {
            prLines[i].Draw();
        }
    }
    ...
    bool CutUp()
    {
        for(int i = 0; i < ArraySize(prLines); i++)
        {
            if(prLines[i].CutUp()) return true;
        }
        return false;
    }
    ...
    bool CutDn()
    {
        for(int i = 0; i < ArraySize(prLines); i++)
        {
            if(prLines[i].CutDn()) return true;
        }
        return false;
    }
    ...
    void PrintAll()
    {
        for(int i = 0; i < ArraySize(prLines); i++)
        {
            Print("prLine i", i, " ", prLines[i].Price());
        }
    }

};

PriceLineList priceLevels();
#endif

input string Tentry = "== Levels Setup ==";  // ————————————
input double price1 = 1.5170; // Price 1:
input double distance1 = 50; // break distance 1 (points):
input double price2 = 1.5270; // Price 2:
input double distance2 = 50; // break distance 2 (points):
input double price3 = 1.5370; // Price 3:
input double distance3 = 50; // break distance 3 (points):
input double price4 = 1.5470; // Price 4:
input double distance4 = 50; // break distance 4 (points):
input double price5 = 1.5570; // Price 5:
input double distance5 = 50; // break distance 5 (points):
input double price6 = 1.5670; // Price 6:
input double distance6 = 50; // break distance 6 (points):
input double price7 = 1.5770; // Price 7:
input double distance7 = 50; // break distance 7 (points):

#define MoneyMannagementOn
#ifdef MoneyMannagementOn

input string t_mm = "== Money Management ==";// ————————————
input double lot0 = 0.01;   // Lots 1:
input double pipSl0 = 10;   // Pips Sl 1:
input double pipTp0 = 10;   // Pips Tp 1:
input double lot1 = 0.02;   // Lots 2:
input double pipSl1 = 10;   // Pips Sl 2:
input double pipTp1 = 10;   // Pips Tp 2:
input double lot2 = 0.03;   // Lots 3:
input double pipSl2 = 20;   // Pips Sl 3:
input double pipTp2 = 20;   // Pips Tp 3:
input double lot3 = 0.04;   // Lots 4:
input double pipSl3 = 30;   // Pips Sl 4:
input double pipTp3 = 30;   // Pips Tp 4:
input double lot4 = 0.05;   // Lots 5:
input double pipSl4 = 40;   // Pips Sl 5:
input double pipTp4 = 40;   // Pips Tp 5:
input double lot5 = 0.06;   // Lots 6:
input double pipSl5 = 50;   // Pips Sl 6:
input double pipTp5 = 50;   // Pips Tp 6:
input double lot6 = 0.07;   // Lots 7:
input double pipSl6 = 60;   // Pips Sl 7:
input double pipTp6 = 60;   // Pips Tp 7:
input double lot7 = 0.08;   // Lots 8:
input double pipSl7 = 70;   // Pips Sl 8:
input double pipTp7 = 70;   // Pips Tp 8:
input double lot8 = 0.09;   // Lots 9:
input double pipSl8 = 80;   // Pips Sl 9:
input double pipTp8 = 80;   // Pips Tp 9:
input double lot9 = 0.10;   // Lots 10:
input double pipSl9 = 90;   // Pips Sl 10:
input double pipTp9 = 90;   // Pips Tp 10:
input double lot10 = 0.11;  // Lots 11:
input double pipSl10 = 100; // Pips Sl 11:
input double pipTp10 = 100; // Pips Tp 11:
input double lot11 = 0.12;  // Lots 12:
input double pipSl11 = 110; // Pips Sl 12:
input double pipTp11 = 110; // Pips Tp 12:
input double lot12 = 0.13;  // Lots 13:
input double pipSl12 = 120; // Pips Sl 13:
input double pipTp12 = 120; // Pips Tp 13:
input double lot13 = 0.14;  // Lots 14:
input double pipSl13 = 130; // Pips Sl 14:
input double pipTp13 = 130; // Pips Tp 14:
input double lot14 = 0.15;  // Lots 15:
input double pipSl14 = 140; // Pips Sl 15:
input double pipTp14 = 140; // Pips Tp 15:
input double lot15 = 0.16;  // Lots 16:
input double pipSl15 = 150; // Pips Sl 16:
input double pipTp15 = 150; // Pips Tp 16:
input double lot16 = 0.17;  // Lots 17:
input double pipSl16 = 160; // Pips Sl 17:
input double pipTp16 = 160; // Pips Tp 17:
input double lot17 = 0.18;  // Lots 18:
input double pipSl17 = 170; // Pips Sl 18:
input double pipTp17 = 170; // Pips Tp 18:
input double lot18 = 0.19;  // Lots 19:
input double pipSl18 = 180; // Pips Sl 19:
input double pipTp18 = 180; // Pips Tp 19:
input double lot19 = 0.20;  // Lots 20:
input double pipSl19 = 180; // Pips Sl 20:
input double pipTp19 = 180; // Pips Tp 20:
input double lot20 = 0.21;  // Lots 21:
input double pipSl20 = 200; // Pips Sl 21:
input double pipTp20 = 200; // Pips Tp 21:
input double lot21 = 0.22;  // Lots 22:
input double pipSl21 = 210; // Pips Sl 22:
input double pipTp21 = 210; // Pips Tp 22:
input double lot22 = 0.23;  // Lots 23:
input double pipSl22 = 220; // Pips Sl 23:
input double pipTp22 = 220; // Pips Tp 23:
input double lot23 = 0.24;  // Lots 24:
input double pipSl23 = 230; // Pips Sl 24:
input double pipTp23 = 230; // Pips Tp 24:
input double lot24 = 0.25;  // Lots 25:
input double pipSl24 = 240; // Pips Sl 25:
input double pipTp24 = 240; // Pips Tp 25:
input double lot25 = 0.26;  // Lots 26:
input double pipSl25 = 250; // Pips Sl 26:
input double pipTp25 = 250; // Pips Tp 26:
input double lot26 = 0.27;  // Lots 27:
input double pipSl26 = 260; // Pips Sl 27:
input double pipTp26 = 260; // Pips Tp 27:
input double lot27 = 0.28;  // Lots 28:
input double pipSl27 = 270; // Pips Sl 28:
input double pipTp27 = 270; // Pips Tp 28:
input double lot28 = 0.29;  // Lots 29:
input double pipSl28 = 280; // Pips Sl 29:
input double pipTp28 = 280; // Pips Tp 29:
input double lot29 = 0.30;  // Lots 30:
input double pipSl29 = 290; // Pips Sl 30:
input double pipTp29 = 290; // Pips Tp 30:

double parameters[90];
...
void setParameters()
{
        ...
        parameters[30] = pipSl0;
        parameters[31] = pipSl1;
        parameters[32] = pipSl2;
        parameters[33] = pipSl3;
        parameters[34] = pipSl4;
        parameters[35] = pipSl5;
        parameters[36] = pipSl6;
        parameters[37] = pipSl7;
        parameters[38] = pipSl8;
        parameters[39] = pipSl9;
        parameters[40] = pipSl10;
        parameters[41] = pipSl11;
        parameters[42] = pipSl12;
        parameters[43] = pipSl13;
        parameters[44] = pipSl14;
        parameters[45] = pipSl15;
        parameters[46] = pipSl16;
        parameters[47] = pipSl17;
        parameters[48] = pipSl18;
        parameters[49] = pipSl19;
        parameters[50] = pipSl20;
        parameters[51] = pipSl21;
        parameters[52] = pipSl22;
        parameters[53] = pipSl23;
        parameters[54] = pipSl24;
        parameters[55] = pipSl25;
        parameters[56] = pipSl26;
        parameters[57] = pipSl27;
        parameters[58] = pipSl28;
        parameters[59] = pipSl29;

        parameters[60] = pipTp0;
        parameters[61] = pipTp1;
        parameters[62] = pipTp2;
        parameters[63] = pipTp3;
        parameters[64] = pipTp4;
        parameters[65] = pipTp5;
        parameters[66] = pipTp6;
        parameters[67] = pipTp7;
        parameters[68] = pipTp8;
        parameters[69] = pipTp9;
        parameters[70] = pipTp10;
        parameters[71] = pipTp11;
        parameters[72] = pipTp12;
        parameters[73] = pipTp13;
        parameters[74] = pipTp14;
        parameters[75] = pipTp15;
        parameters[76] = pipTp16;
        parameters[77] = pipTp17;
        parameters[78] = pipTp18;
        parameters[79] = pipTp19;
        parameters[80] = pipTp20;
        parameters[81] = pipTp21;
        parameters[82] = pipTp22;
        parameters[83] = pipTp23;
        parameters[84] = pipTp24;
        parameters[85] = pipTp25;
        parameters[86] = pipTp26;
        parameters[87] = pipTp27;
        parameters[88] = pipTp28;
        parameters[89] = pipTp29;
}
...
class MoneyManagement
{
    ...
    TradeSetup setups[30];

    int _i;
    int _magic;

    public:
    MoneyManagement(int magic, double &param[])
    {
        _magic = magic;
        
        setups[0].lots  = param[0];
        setups[1].lots  = param[1];
        setups[2].lots  = param[2];
        setups[3].lots  = param[3];
        setups[4].lots  = param[4];
        setups[5].lots  = param[5];
        setups[6].lots  = param[6];
        setups[7].lots  = param[7];
        setups[8].lots  = param[8];
        setups[9].lots  = param[9];
        setups[10].lots = param[10];
        setups[11].lots = param[11];
        setups[12].lots = param[12];
        setups[13].lots = param[13];
        setups[14].lots = param[14];
        setups[15].lots = param[15];
        setups[16].lots = param[16];
        setups[17].lots = param[17];
        setups[18].lots = param[18];
        setups[19].lots = param[19];
        setups[20].lots = param[20];
        setups[21].lots = param[21];
        setups[22].lots = param[22];
        setups[23].lots = param[23];
        setups[24].lots = param[24];
        setups[25].lots = param[25];
        setups[26].lots = param[26];
        setups[27].lots = param[27];
        setups[28].lots = param[28];
        setups[29].lots = param[29];

        setups[0].sl  = param[30];
        setups[1].sl  = param[31];
        setups[2].sl  = param[32];
        setups[3].sl  = param[33];
        setups[4].sl  = param[34];
        setups[5].sl  = param[35];
        setups[6].sl  = param[36];
        setups[7].sl  = param[37];
        setups[8].sl  = param[38];
        setups[9].sl  = param[39];
        setups[10].sl = param[40];
        setups[11].sl = param[41];
        setups[12].sl = param[42];
        setups[13].sl = param[43];
        setups[14].sl = param[44];
        setups[15].sl = param[45];
        setups[16].sl = param[46];
        setups[17].sl = param[47];
        setups[18].sl = param[48];
        setups[19].sl = param[49];
        setups[20].sl = param[50];
        setups[21].sl = param[51];
        setups[22].sl = param[52];
        setups[23].sl = param[53];
        setups[24].sl = param[54];
        setups[25].sl = param[55];
        setups[26].sl = param[56];
        setups[27].sl = param[57];
        setups[28].sl = param[58];
        setups[29].sl = param[59];

        setups[0].tp  = param[60];
        setups[1].tp  = param[61];
        setups[2].tp  = param[62];
        setups[3].tp  = param[63];
        setups[4].tp  = param[64];
        setups[5].tp  = param[65];
        setups[6].tp  = param[66];
        setups[7].tp  = param[67];
        setups[8].tp  = param[68];
        setups[9].tp  = param[69];
        setups[10].tp = param[70];
        setups[11].tp = param[71];
        setups[12].tp = param[72];
        setups[13].tp = param[73];
        setups[14].tp = param[74];
        setups[15].tp = param[75];
        setups[16].tp = param[76];
        setups[17].tp = param[77];
        setups[18].tp = param[78];
        setups[19].tp = param[79];
        setups[20].tp = param[80];
        setups[21].tp = param[81];
        setups[22].tp = param[82];
        setups[23].tp = param[83];
        setups[24].tp = param[84];
        setups[25].tp = param[85];
        setups[26].tp = param[86];
        setups[27].tp = param[87];
        setups[28].tp = param[88];
        setups[29].tp = param[89];
        ...
        // setups[8].lots = _lot8;
        // setups[9].lots = _lot9;
        // setups[10].lots = _lot10;
        // setups[11].lots = _lot11;
        // setups[12].lots = _lot12;
        // setups[13].lots = _lot13;
        // setups[14].lots = _lot14;
        // setups[15].lots = _lot15;
        // setups[16].lots = _lot16;
        // setups[17].lots = _lot17;
        // setups[18].lots = _lot18;
        // setups[19].lots = _lot19;
        // setups[20].lots = _lot20;
        // setups[21].lots = _lot21;
        // setups[22].lots = _lot22;
        // setups[23].lots = _lot23;
        // setups[24].lots = _lot24;
        // setups[25].lots = _lot25;
        // setups[26].lots = _lot26;
        // setups[27].lots = _lot27;
        // setups[28].lots = _lot28;
        // setups[29].lots = _lot29;
        ...
        // setups[8].sl = _pipSl8;
        // setups[9].sl = _pipSl9;
        // setups[10].sl = _pipSl10;
        // setups[11].sl = _pipSl11;
        // setups[12].sl = _pipSl12;
        // setups[13].sl = _pipSl13;
        // setups[14].sl = _pipSl14;
        // setups[15].sl = _pipSl15;
        // setups[16].sl = _pipSl16;
        // setups[17].sl = _pipSl17;
        // setups[18].sl = _pipSl18;
        // setups[19].sl = _pipSl19;
        // setups[20].sl = _pipSl20;
        // setups[21].sl = _pipSl21;
        // setups[22].sl = _pipSl22;
        // setups[23].sl = _pipSl23;
        // setups[24].sl = _pipSl24;
        // setups[25].sl = _pipSl25;
        // setups[26].sl = _pipSl26;
        // setups[27].sl = _pipSl27;
        // setups[28].sl = _pipSl28;
        // setups[29].sl = _pipSl29;
        ...
        // setups[8].tp = _pipTp8;
        // setups[9].tp = _pipTp9;
        // setups[10].tp = _pipTp10;
        // setups[11].tp = _pipTp11;
        // setups[12].tp = _pipTp12;
        // setups[13].tp = _pipTp13;
        // setups[14].tp = _pipTp14;
        // setups[15].tp = _pipTp15;
        // setups[16].tp = _pipTp16;
        // setups[17].tp = _pipTp17;
        // setups[18].tp = _pipTp18;
        // setups[19].tp = _pipTp19;
        // setups[20].tp = _pipTp20;
        // setups[21].tp = _pipTp21;
        // setups[22].tp = _pipTp22;
        // setups[23].tp = _pipTp23;
        // setups[24].tp = _pipTp24;
        // setups[25].tp = _pipTp25;
        // setups[26].tp = _pipTp26;
        // setups[27].tp = _pipTp27;
        // setups[28].tp = _pipTp28;
        // setups[29].tp = _pipTp29;
    ...
    // si el ultimo fue perdedor, incrementar el Index:
    // si el ultimo fue ganador, resetear el Index
    void MannageIndex()
    {
        if(LastWasLoss()) { Increase(); }
        else { Print("RESETEO! ------"); Reset(); }
    }

    void Increase()
    {
        _i += 1;
        int t = ArraySize(setups);
        if(_i >= t)
        {
            _i = t - 1;
        }
    ...
    double Sl() { return setups[Index()].sl; }
    double Tp() { return setups[Index()].tp; }
    double Lots() { return setups[Index()].lots; }
...
int OnInit()
{
    ...
    // Setup Money Mannagement:
    setParameters();
    mm = new MoneyManagement(magico, parameters);

    // Setup Price levels to enter trades:
    priceLevels.Add(price1, distance1);
    priceLevels.Add(price2, distance2);
    priceLevels.Add(price3, distance3);
    priceLevels.Add(price4, distance4);
    priceLevels.Add(price5, distance5);
    priceLevels.Add(price6, distance6);
    priceLevels.Add(price7, distance7);
...
double SL(string side, double price = 0)
{
    ...
    {
      case FixPips:
        levelSL = new Levels(new ByFixPips(_Symbol, side, pips, "SL", price));
...
double TP(string side, double price = 0)
{
    ...
    switch (modeTP)
    {
      case FixPips:
        ...
        levelTP = new Levels(new ByFixPips(_Symbol, side, pips, "TP", price));
...
double Lots()
{
    ...
    Print("mm.Index()", mm.Index());
    Print("mm.Lots()", mm.Lots());
