...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      double ANGLE = iCustom(_symbol, _timeframe, "Arctan", PeriodeA, nbChandelierA, PeriodeB, nbChandelierB, 0, period);
      double pente = iCustom(_symbol, _timeframe, "Arctan", PeriodeA, nbChandelierA, PeriodeB, nbChandelierB, 1, period);
      double trigger = iCustom(_symbol, _timeframe, "Arctan", PeriodeA, nbChandelierA, PeriodeB, nbChandelierB, 2, period);

      return ANGLE >= 45 && pente > trigger && pente < 0;
   }
};
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      ...
      return ANGLE <= -37 && pente < trigger && pente > -1;
   }
};
...
int OnInit()
{
   double temp = iCustom(NULL, 0, "Arctan", 0, 0);
   if (GetLastError() == ERR_INDICATOR_CANNOT_LOAD)
   {
      Alert("Please, install the 'Arctan' indicator");
      return INIT_FAILED;
   }
   #ifdef SHOW_ACCOUNT_STAT
      stats = NULL;
