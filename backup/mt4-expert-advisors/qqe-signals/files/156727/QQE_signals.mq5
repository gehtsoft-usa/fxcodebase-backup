//Available @ https://fxcodebase.com/code/viewtopic.php?f=38&t=75219
...
#property indicator_label1 "QQE long"
#property indicator_type1 DRAW_ARROW
#property indicator_style1 STYLE_SOLID
#property indicator_width1 1
#property indicator_label2 "QQE short"
...
input int param2 = 5; // RSI Smoothing
input double param3 = 4.238; // Fast QQE Factor
input int param4 = 10; // Thresh-hold
...
EMAOnStream* ema1;
double RsiMa[];
double RsiMa_DEFAULT_VALUE;
FloatStream* ema2Source;
EMAOnStream* ema2;
FloatStream* ema3Source;
EMAOnStream* ema3;
double RSIndex[];
double RSIndex_DEFAULT_VALUE;
double longband[];
double longband_DEFAULT_VALUE;
double shortband[];
double shortband_DEFAULT_VALUE;
FloatStream* cross1X;
FloatStream* cross1Y;
IBoolStream* cross1;
FloatStream* cross2X;
...
double QQExlong[];
double QQExlong_DEFAULT_VALUE;
double QQExshort[];
double QQExshort_DEFAULT_VALUE;
double FastAtrRsiTL[];
double FastAtrRsiTL_DEFAULT_VALUE;
double plot1[];
double plot2[];
Signaler* _signaler;
...
void OnInit()
{
   ...
   IndicatorSetString(INDICATOR_SHORTNAME, "QQE signals");
   IndicatorSetInteger(INDICATOR_DIGITS, Digits());
   SetIndexBuffer(id++, RsiMa, INDICATOR_CALCULATIONS);
   SetIndexBuffer(id++, RSIndex, INDICATOR_CALCULATIONS);
   SetIndexBuffer(id++, longband, INDICATOR_CALCULATIONS);
   SetIndexBuffer(id++, shortband, INDICATOR_CALCULATIONS);
   SetIndexBuffer(id++, trend, INDICATOR_CALCULATIONS);
   SetIndexBuffer(id++, QQExlong, INDICATOR_CALCULATIONS);
   SetIndexBuffer(id++, QQExshort, INDICATOR_CALCULATIONS);
   SetIndexBuffer(id++, FastAtrRsiTL, INDICATOR_CALCULATIONS);
                ...
                const int &spread[])
{
   if (prev_calculated <= 0 || prev_calculated > rates_total)
   {
      rsi1X.Init();
      ema1Source.Init();
      RsiMa_DEFAULT_VALUE = EMPTY_VALUE;
      ArrayInitialize(RsiMa, RsiMa_DEFAULT_VALUE);
      ema2Source.Init();
      ema3Source.Init();
      RSIndex_DEFAULT_VALUE = EMPTY_VALUE;
      ArrayInitialize(RSIndex, RSIndex_DEFAULT_VALUE);
      longband_DEFAULT_VALUE = 0.0;
      ArrayInitialize(longband, longband_DEFAULT_VALUE);
      shortband_DEFAULT_VALUE = 0.0;
      ArrayInitialize(shortband, shortband_DEFAULT_VALUE);
      cross1X.Init();
      cross1Y.Init();
      cross2X.Init();
      cross2Y.Init();
      ...
      QQExlong_DEFAULT_VALUE = 0;
      ArrayInitialize(QQExlong, QQExlong_DEFAULT_VALUE);
      QQExshort_DEFAULT_VALUE = 0;
      ArrayInitialize(QQExshort, QQExshort_DEFAULT_VALUE);
      FastAtrRsiTL_DEFAULT_VALUE = EMPTY_VALUE;
      ArrayInitialize(FastAtrRsiTL, FastAtrRsiTL_DEFAULT_VALUE);
   ...
   for (int pos = MathMax(rates_total - 1 - bars_limit, MathMax(first, prev_calculated - 1)); pos < rates_total; ++pos)
   {
      ...
      int Wilders_Period = RSI_Period * 2 - 1;
      rsi1X.SetValue(pos, src);
      double rsi1Value[1];
      if (!rsi1.GetValues(pos, 1, rsi1Value)) { rsi1Value[0] = EMPTY_VALUE; }
      double Rsi = rsi1Value[0];
      ema1Source.SetValue(pos, Rsi);
      double ema1Value[1];
      if (!ema1.GetValues(pos, 1, ema1Value)) { ema1Value[0] = EMPTY_VALUE; }
      SetStream(RsiMa, pos, ema1Value[0], RsiMa_DEFAULT_VALUE);
      if (pos - 1 < 0) { continue; }
      double AtrRsi = SafeMathAbs(SafeMinus(RsiMa[pos - 1], RsiMa[pos]));
      ema2Source.SetValue(pos, AtrRsi);
      double ema2Value[1];
      if (!ema2.GetValues(pos, 1, ema2Value)) { ema2Value[0] = EMPTY_VALUE; }
      double MaAtrRsi = ema2Value[0];
      ema3Source.SetValue(pos, MaAtrRsi);
      double ema3Value[1];
      if (!ema3.GetValues(pos, 1, ema3Value)) { ema3Value[0] = EMPTY_VALUE; }
      double dar = SafeMultiply(ema3Value[0], QQE);
      double DeltaFastAtrRsi = dar;
      SetStream(RSIndex, pos, RsiMa[pos], RSIndex_DEFAULT_VALUE);
      double newshortband = SafePlus(RSIndex[pos], DeltaFastAtrRsi);
      double newlongband = SafeMinus(RSIndex[pos], DeltaFastAtrRsi);
      if (pos - 1 < 0) { continue; }
      if (pos - 1 < 0) { continue; }
      if (pos - 1 < 0) { continue; }
      if (pos - 1 < 0) { continue; }
      SetStream(longband, pos, (SafeGreater(RSIndex[pos - 1], longband[pos - 1]) && SafeGreater(RSIndex[pos], longband[pos - 1]) ? SafeMathMax(longband[pos - 1], newlongband) : newlongband), longband_DEFAULT_VALUE);
      if (pos - 1 < 0) { continue; }
      if (pos - 1 < 0) { continue; }
      if (pos - 1 < 0) { continue; }
      if (pos - 1 < 0) { continue; }
      SetStream(shortband, pos, (SafeLess(RSIndex[pos - 1], shortband[pos - 1]) && SafeLess(RSIndex[pos], shortband[pos - 1]) ? SafeMathMin(shortband[pos - 1], newshortband) : newshortband), shortband_DEFAULT_VALUE);
      if (pos - 1 < 0) { continue; }
      cross1X.SetValue(pos, longband[pos - 1]);
      cross1Y.SetValue(pos, RSIndex[pos]);
      bool cross1Value[1];
      if (!cross1.GetValues(pos, 1, cross1Value)) { cross1Value[0] = EMPTY_VALUE; }
      bool cross_1 = cross1Value[0];
      if (pos - 1 < 0) { continue; }
      cross2X.SetValue(pos, RSIndex[pos]);
      cross2Y.SetValue(pos, shortband[pos - 1]);
      bool cross2Value[1];
      if (!cross2.GetValues(pos, 1, cross2Value)) { cross2Value[0] = EMPTY_VALUE; }
      if (pos - 1 < 0) { continue; }
      SetStream(trend, pos, (cross2Value[0] ? 1 : (cross_1 ? (-1) : Nz(trend[pos - 1], 1))), trend_DEFAULT_VALUE);
      SetStream(FastAtrRsiTL, pos, ((trend[pos] == 1) ? longband[pos] : shortband[pos]), FastAtrRsiTL_DEFAULT_VALUE);
      if (pos - 1 < 0) { continue; }
      SetStream(QQExlong, pos, Nz(QQExlong[pos - 1]), QQExlong_DEFAULT_VALUE);
      if (pos - 1 < 0) { continue; }
      SetStream(QQExshort, pos, Nz(QQExshort[pos - 1]), QQExshort_DEFAULT_VALUE);
      SetStream(QQExlong, pos, (SafeLess(FastAtrRsiTL[pos], RSIndex[pos]) ? QQExlong[pos] + 1 : 0), QQExlong_DEFAULT_VALUE);
      SetStream(QQExshort, pos, (SafeGreater(FastAtrRsiTL[pos], RSIndex[pos]) ? QQExshort[pos] + 1 : 0), QQExshort_DEFAULT_VALUE);
      if (pos - 1 < 0) { continue; }
      double qqeLong = ((QQExlong[pos] == 1) ? SafeMinus(FastAtrRsiTL[pos - 1], 50) : EMPTY_VALUE);
      if (pos - 1 < 0) { continue; }
      double qqeShort = ((QQExshort[pos] == 1) ? SafeMinus(FastAtrRsiTL[pos - 1], 50) : EMPTY_VALUE);
      bool plotshape1_condition = NumberToBool(qqeLong);
      if (plotshape1_condition) { plot1[pos] = low[pos]; }
      bool plotshape2_condition = NumberToBool(qqeShort);
      if (plotshape2_condition) { plot2[pos] = high[pos]; }
      if (qqeLong) { _signaler.SendNotifications("Long", "Long"); }
      if (qqeShort) { _signaler.SendNotifications("Short", "Short"); }
   }
   return rates_total;
}
//+------------------------------------------------------------------------------------------------+
