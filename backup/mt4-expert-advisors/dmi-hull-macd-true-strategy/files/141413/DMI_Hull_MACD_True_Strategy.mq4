...
class DILongCondition : public ACondition
{
public:
   DILongCondition(const string symbol, ENUM_TIMEFRAMES timeframe)
      :ACondition(symbol, timeframe)
   {
   ...
   bool IsPass(const int period, const datetime date)
   {
      double dip = iADX(_symbol, _timeframe, adx_period, PRICE_CLOSE, MODE_PLUSDI, period);
      double dim = iADX(_symbol, _timeframe, adx_period, PRICE_CLOSE, MODE_MINUSDI, period);
      return dip > dim;
   }
};

class DILongFilterCondition : public ACondition
{
public:
   DILongFilterCondition(const string symbol, ENUM_TIMEFRAMES timeframe)
      :ACondition(symbol, timeframe)
   {
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      double macd = iCustom(_symbol, _timeframe, "MACD True", FastMAPeriod, SlowMAPeriod, SignalMAPeriod, 0, period);
      double signal = iCustom(_symbol, _timeframe, "MACD True", FastMAPeriod, SlowMAPeriod, SignalMAPeriod, 1, period);
      return macd > signal;
   }
};

class HullLongCondition : public ACondition
{
public:
   HullLongCondition(const string symbol, ENUM_TIMEFRAMES timeframe)
      :ACondition(symbol, timeframe)
   {
   ...
   bool IsPass(const int period, const datetime date)
   {
      double curr = iCustom(_symbol, _timeframe, "Hull moving average", inpPeriod, inpDivisor, 0, period);
      double prev = iCustom(_symbol, _timeframe, "Hull moving average", inpPeriod, inpDivisor, 0, period + 1);
      return curr > prev;
   }
};

class DIShortCondition : public ACondition
{
public:
   DIShortCondition(const string symbol, ENUM_TIMEFRAMES timeframe)
      :ACondition(symbol, timeframe)
   {
...
class DIShortFilterCondition : public ACondition
{
public:
   DIShortFilterCondition(const string symbol, ENUM_TIMEFRAMES timeframe)
      :ACondition(symbol, timeframe)
   {
...
class HullShortCondition : public ACondition
{
public:
   HullShortCondition(const string symbol, ENUM_TIMEFRAMES timeframe)
      :ACondition(symbol, timeframe)
   {
...
ICondition* CreateLongCondition(string symbol, ENUM_TIMEFRAMES timeframe)
{
   ...
   condition.Add(new DILongCondition(symbol, timeframe), false);
   condition.Add(new DILongFilterCondition(symbol, timeframe), false);
   condition.Add(new MACDLongCondition(symbol, timeframe), false);
   condition.Add(new HullLongCondition(symbol, timeframe), false);
...
ICondition* CreateShortCondition(string symbol, ENUM_TIMEFRAMES timeframe)
{
   ...
   AndCondition* condition = new AndCondition();
   condition.Add(new DIShortCondition(symbol, timeframe), false);
   condition.Add(new DIShortFilterCondition(symbol, timeframe), false);
   condition.Add(new MACDShortCondition(symbol, timeframe), false);
   condition.Add(new HullShortCondition(symbol, timeframe), false);
   #ifdef ACT_ON_SWITCH_CONDITION
      ActOnSwitchCondition* switchCondition = new ActOnSwitchCondition(symbol, timeframe, (ICondition*) condition);
      condition.Release();
      return switchCondition;
...
int OnInit()
{
   double temp = iCustom(NULL, 0, "MACD True", 0, 0);
   if (GetLastError() == ERR_INDICATOR_CANNOT_LOAD)
   {
      Alert("Please, install the 'MACD True' indicator");
      return INIT_FAILED;
   }
   temp = iCustom(NULL, 0, "Hull moving average", 0, 0);
   if (GetLastError() == ERR_INDICATOR_CANNOT_LOAD)
   {
      Alert("Please, install the 'Hull moving average' indicator");
      return INIT_FAILED;
   }
