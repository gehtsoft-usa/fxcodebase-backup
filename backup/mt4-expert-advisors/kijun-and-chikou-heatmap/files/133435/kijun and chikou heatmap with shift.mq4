...
int init()
{
   ...
   if (bar_shift <0)
				{
				   Alert("bar_shif should be positive number");
				   return INIT_FAILED;
				}
...
int start()
{
   int minBars = bar_shift;
   int limit = MathMin(Bars - 1 - minBars, Bars - IndicatorCounted() - 1);
	   for (int i = limit ; i >= 0 ; i--)
	   {
      
			//  if (i<(limit - bar_shift) )
			//  {
					  out1[i+bar_shift] = EMPTY_VALUE;
					  out2[i+bar_shift] = EMPTY_VALUE;
					  out3[i+bar_shift] = EMPTY_VALUE;
