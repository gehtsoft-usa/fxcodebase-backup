...
int init(){
   ...
   IndName = "Aaron_Oscillator_HeatMap";
...
int start(){
   ...
   if (Period() < 43200){
      ...
      for(i=limit ; i>=0; i--){
         ...
         Aaron = 100*(iLowest(Symbolo, period, MODE_LOW, Aaron_Periods, current) - iHighest(Symbolo, period, MODE_HIGH, Aaron_Periods, current) ) / Aaron_Periods;
         w1_up[i] = -1.0;
         w1_dn[i] = -1.0;
         w1_nt[i] = -1.0;
         if (Aaron > Aaron_Filter)
            w1_up[i] = 1.0;
         else if (Aaron < -Aaron_Filter)
            w1_dn[i] = 1.0;
         else
            w1_nt[i] = 1.0;
