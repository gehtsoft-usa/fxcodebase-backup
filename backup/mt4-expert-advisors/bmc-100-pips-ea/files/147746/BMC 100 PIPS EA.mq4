...
string file_custom_indicator  = "BMC 100 PIPS.ex4";
string file_custom_indicator2 = "5TF Waddah Attar Explosion.ex4";
...
enum enumDays { sunday,
                ...
                EA_OFF };
enum SignalMode {
  OneCandle,    // One Candle
  TwoCandles,   // Two Candles
  TreeCandles,  // Third Candles
  FourCandles   // Four Candles
};

enum ModeEntry { Market,        // Market Order
                 PendingStop,   // Stop Order
                 PendingLimit,  // Limit Order
                 NoTrade        // No Trade
};

// ------------------------------------------------------------------
input string TCustom        = "== Indicator Setup ==";                    // == Setup ==
input int    BPERIOD        = 34;                                         // BPERIOD
input int    APERIOD        = 3;                                          // APERIOD
input int    aTake_Profit   = 88;                                         // Take Profit
input int    aStop_Loss     = 48;                                         // Stop Loss
input string TCustom2       = "== 5TF Whaddah Attar Explosion Setup ==";  // == Indicator 2 ==
input bool   uFilterIndi2On = true;                                       // Filter by Whaddah Attar On:
...
input string TimeFrame5     = "next4";
input int    Sensetive      = 150;

input string    TI3            = "== Sleep Page Setup ==";  // == Sleep Page Setup ==
input bool      uSleepOn       = true;                      // Sleep Page Filter On:
input ModeEntry modeEntry      = Market;                    // SleepPage Mode:
input int       uSleepMax      = 100;                       // SleepPage Max (points):
input double    uEntryDistance = 10;                        // Pips distance for pending orders:

input string     T0         = "== Trade Setup ==";  // == Trade Setup ==
input SignalMode signalMode = OneCandle;            // Signal Mode:
...
input bool     day1_2_On    = true;                       // Session On:
input enumDays day1_2       = monday;                     // Day
input string   day1_2_Start = "00:00:00";                 // Time Start GMT
input string   day1_2_End   = "23:59:59";                 // Time End GMT
...
input bool     day2_2_On    = true;                       // Session On:
input enumDays day2_2       = tuesday;                    // Day
input string   day2_2_Start = "00:00:00";                 // Time Start GMT
input string   day2_2_End   = "23:59:59";                 // Time End GMT
...
input bool     day3_2_On    = true;                       // Session On:
input enumDays day3_2       = wednesday;                  // Day
input string   day3_2_Start = "00:00:00";                 // Time Start GMT
input string   day3_2_End   = "23:59:59";                 // Time End GMT
...
input bool     day4_2_On    = true;                       // Session On:
input enumDays day4_2       = thursday;                   // Day
input string   day4_2_Start = "00:00:00";                 // Time Start GMT
input string   day4_2_End   = "23:59:59";                 // Time End GMT
...
input bool     day5_2_On    = true;                       // Session On:
input enumDays day5_2       = friday;                     // Day
input string   day5_2_Start = "00:00:00";                 // Time Start GMT
input string   day5_2_End   = "23:59:59";                 // Time End GMT
#endif
#ifdef TIMER_MINI
input string T1        = "== Trading Sessions  ==";  // Trading Sessions
...
input string TEndActions         = "== Actions when the session end ==";  // == Actions when the session end ==
input bool   uEndAction_CloseAll = true;                                  // Close all open orders and pending orders after end time:
input bool   uEndAction_CloseProfitAndPendings = true;                    // Close all profit orders and pending orders after end time
input bool   uEndAction_CloseLossersAndPendings = true;                   // Close all losing orders and pending orders after end time
...
ActionDeletePendings* actionDeletePendings;
...
ActionCloseByProfit actionCloseWinners("win", magico);
ActionCloseByProfit actionCloseLosers("loss",magico);
...
class BUYcondition1 : public iConditions
{
 public:
  bool evaluate()
  {
    ...
    double upCurr = iCustom(NULL, 0, file_custom_indicator, BPERIOD, APERIOD, aTake_Profit, aStop_Loss, true, true, 9, Yellow, Red, 0, 1);
    // double upPrev = iCustom(NULL,0, file_custom_indicator, BPERIOD, APERIOD, aTake_Profit, aStop_Loss,true, true, 9, Yellow, Red, 0,2);

    // if( upPrev == EMPTY_VALUE && upCurr != EMPTY_VALUE ) return true;
    if (upCurr != EMPTY_VALUE) return true;
    return false;
  }
};
...
class BUYcondition2 : public iConditions
{
 public:
  bool evaluate()
  {
    ...
    double indi1 = iCustom(NULL, 0, file_custom_indicator, BPERIOD, APERIOD, aTake_Profit, aStop_Loss, true, true, 9, Yellow, Red, 0, 1);
    double indi2 = iCustom(NULL, 0, file_custom_indicator, BPERIOD, APERIOD, aTake_Profit, aStop_Loss, true, true, 9, Yellow, Red, 0, 2);
    double indi3 = iCustom(NULL, 0, file_custom_indicator, BPERIOD, APERIOD, aTake_Profit, aStop_Loss, true, true, 9, Yellow, Red, 0, 3);
    double indi4 = iCustom(NULL, 0, file_custom_indicator, BPERIOD, APERIOD, aTake_Profit, aStop_Loss, true, true, 9, Yellow, Red, 0, 4);

    switch (signalMode)
    {
      case OneCandle:
        if (close1 < indi1 || indi1 == EMPTY_VALUE) return false;
        break;
      case TwoCandles:
        if (close1 < indi1 || close2 < indi2 || indi1 == EMPTY_VALUE || indi2 == EMPTY_VALUE) return false;
        if (close1 < close2) return false;
        break;
      case TreeCandles:
        if (close1 < indi1 || close2 < indi2 || close3 < indi3 || indi1 == EMPTY_VALUE || indi2 == EMPTY_VALUE || indi3 == EMPTY_VALUE) return false;
        if (close1 < close2 || close2 < close3) return false;
        break;
      case FourCandles:
        if (close1 < indi1 || close2 < indi2 || close3 < indi3 || close4 < indi4 || indi1 == EMPTY_VALUE || indi2 == EMPTY_VALUE || indi3 == EMPTY_VALUE || indi4 == EMPTY_VALUE) return false;
        if (close1 < close2 || close2 < close3 || close3 < close4) return false;
        break;
    }
...
class BUYcondition3 : public iConditions
{
 public:
  bool evaluate()
  {
    ...
    for (int i = 0; i < 9; i++)
    {
      double indi = iCustom(NULL, 0, file_custom_indicator2, TimeFrame1, TimeFrame2, TimeFrame3, TimeFrame4, TimeFrame5, Sensetive, "5TF WAE Trend", false, 3, false, false, false, 0, clrDarkGray, 5, 1.5, i, 1);
      i++;
      if (indi == EMPTY_VALUE || indi == 0) return false;
    }
    // double tf0 = iCustom(NULL, 0, file_custom_indicator2, TimeFrame1, TimeFrame2, TimeFrame3, TimeFrame4, TimeFrame5, Sensetive, 0, 1);
    // double tf1 = iCustom(NULL, 0, file_custom_indicator2, TimeFrame1, TimeFrame2, TimeFrame3, TimeFrame4, TimeFrame5, Sensetive, 2, 1);
    // double tf2 = iCustom(NULL, 0, file_custom_indicator2, TimeFrame1, TimeFrame2, TimeFrame3, TimeFrame4, TimeFrame5, Sensetive, 4, 1);
    // double tf3 = iCustom(NULL, 0, file_custom_indicator2, TimeFrame1, TimeFrame2, TimeFrame3, TimeFrame4, TimeFrame5, Sensetive, 6, 1);
    // double tf4 = iCustom(NULL, 0, file_custom_indicator2, TimeFrame1, TimeFrame2, TimeFrame3, TimeFrame4, TimeFrame5, Sensetive, 8, 1);

    // if( tf0 == EMPTY_VALUE || tf1 == EMPTY_VALUE || tf2 == EMPTY_VALUE || tf3 == EMPTY_VALUE || tf4 == EMPTY_VALUE) return false;
...
class SELLcondition1 : public iConditions
{
 public:
  bool evaluate()
  {
    ...
    double dnCurr = iCustom(NULL, 0, file_custom_indicator, BPERIOD, APERIOD, aTake_Profit, aStop_Loss, true, true, 9, Yellow, Red, 1, 1);
    // double dnPrev = iCustom(NULL,0, file_custom_indicator, BPERIOD, APERIOD, aTake_Profit, aStop_Loss,true, true, 9, Yellow, Red, 1,2);

    // if( dnPrev == EMPTY_VALUE && dnCurr != EMPTY_VALUE ) return true;
    if (dnCurr != EMPTY_VALUE) return true;
    return false;
  }
};
...
class SELLcondition2 : public iConditions
{
 public:
  bool evaluate()
  {
    ...
    double indi1 = iCustom(NULL, 0, file_custom_indicator, BPERIOD, APERIOD, aTake_Profit, aStop_Loss, true, true, 9, Yellow, Red, 1, 1);
    double indi2 = iCustom(NULL, 0, file_custom_indicator, BPERIOD, APERIOD, aTake_Profit, aStop_Loss, true, true, 9, Yellow, Red, 1, 2);
    double indi3 = iCustom(NULL, 0, file_custom_indicator, BPERIOD, APERIOD, aTake_Profit, aStop_Loss, true, true, 9, Yellow, Red, 1, 3);
    double indi4 = iCustom(NULL, 0, file_custom_indicator, BPERIOD, APERIOD, aTake_Profit, aStop_Loss, true, true, 9, Yellow, Red, 1, 4);
 ...
 public:
  bool evaluate()
  {
    if (closeAllInOpositeSignal)
    {
			if(uFilterIndi2On)
			{
        // if(sellCondition1.evaluate() == true && sellCondition2.evaluate()== true && sellCondition3.evaluate()== true) return true;
        if(sellCondition3.evaluate()) return true;
      } else
			{
				if(sellCondition1.evaluate() == true && sellCondition2.evaluate()== true) return true;
			}
    }
 ...
 public:
  bool evaluate()
  {
    if (closeAllInOpositeSignal)
    {
			if(uFilterIndi2On)
			{
        // if(buyCondition1.evaluate() == true && buyCondition2.evaluate()== true && buyCondition3.evaluate()== true) return true;
        if(buyCondition3.evaluate()) return true;
      } else
			{
				if(buyCondition1.evaluate() == true && buyCondition2.evaluate()== true) return true;
			}
    }
    if (closeAllControlON)
    {
...
int OnInit()
{
  ...
  if (uFilterIndi2On) conditionsToBuy.AddCondition(buyCondition3 = new BUYcondition3());
  ...
  if (uFilterIndi2On) conditionsToSell.AddCondition(sellCondition3 = new SELLcondition3());
...
//--- SESSIONS CONTROL:
  ...
  if (day1_On) sesionControl.AddSession(day1_Start, day1_End, day1);
	if (day1_2_On) sesionControl.AddSession(day1_2_Start, day1_2_End, day1_2);
  if (day2_On) sesionControl.AddSession(day2_Start, day2_End, day2);
  if (day2_2_On) sesionControl.AddSession(day2_2_Start, day2_2_End, day2_2);
  if (day3_On) sesionControl.AddSession(day3_Start, day3_End, day3);
  if (day3_2_On) sesionControl.AddSession(day3_2_Start, day3_2_End, day3_2);
  if (day4_On) sesionControl.AddSession(day4_Start, day4_End, day4);
  if (day4_2_On) sesionControl.AddSession(day4_2_Start, day4_2_End, day4_2);
  if (day5_On) sesionControl.AddSession(day5_Start, day5_End, day5);  
  if (day5_2_On) sesionControl.AddSession(day5_2_Start, day5_2_End, day5_2);
#endif
#ifdef TIMER_MINI
...
void OnTick()
{
  ...
  if (!sesionControl.doSessionControl())
  {
		if(EA_ON.isOn())
		{
			// ejecutar las acciones de apagado
			if(uEndAction_CloseAll) 
			{ 
				closeAll();
        deletePendings();
      }
      
			if(uEndAction_CloseProfitAndPendings)
			{
        actionCloseWinners.doAction();
        deletePendings();
      }
			
			if(uEndAction_CloseLossersAndPendings)
			{
	      actionCloseLosers.doAction();
        deletePendings();
      }

	    EA_ON.Off(); // se apaga (por lo que ya no ejecutará las acciones de apagado)
		}
    return;
  }
  EA_ON.On();// si el control de session está dando ok, se enciende el EA
  ...
  // ------------------------------------------------------------------
  if (controlSleepPage() == false && modeEntry == NoTrade) return;

  if (conditionsToBuy.EvaluateConditions())
  {
    if (controlSleepPage() == true || modeEntry == Market)
    {
...
bool controlSleepPage()
{
  if (!uSleepOn) return true;

  int spread = SymbolInfoInteger(_Symbol, SYMBOL_SPREAD);

  return spread <= uSleepMax;
}

void deletePendings()
{
	actionDeletePendings = new ActionDeletePendings("buy", magico);
  actionDeletePendings.doAction();
	delete actionDeletePendings;
	actionDeletePendings = new ActionDeletePendings("sell", magico);
  actionDeletePendings.doAction();
	delete actionDeletePendings;
}
