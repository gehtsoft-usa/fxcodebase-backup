// Available @  https://fxcodebase.com/code/viewtopic.php?f=38&p=157694#p157694
...
input string uMs = "... custom message"; // Message:
...
class CCustomBot
{
  private:
    ...
    //+------------------------------------------------------------------+
    void ArrayAdd(uchar &dest[], const string text)
    {
    ...
    void ProcessMessages(void)
    {
        ...
        for (int i = 0; i < m_chats.Total(); i++) {
            ...
            if (!chat.m_new_one.done) {
                ...
                //-- Start
                if (text == "/start") SendMessage(chat.m_id, "Hello, I am FX four_The$20s Bot.\n\n For more information reply to this message with /help");

                //-- Help
                if (text == "/help") SendMessage(chat.m_id, "FAQ \n ");
            }
        }
    }
};

input string InpChannelName = "@ttrPruebas";                                    // Channel Name
input string InpToken       = "5522964431:AAGisI-heJUa7uyPPQu1zbFj4CNiIZXyzw8"; // Bot Token
// input long   ChatID         ="";
...
void OnTick()
{
    ...
    //--- get time
    datetime time[1];
    if (CopyTime(NULL, 0, 0, 1, time) != 1) return;
}
...
void OnTradeTransaction(const MqlTradeTransaction &trans, const MqlTradeRequest &request, const MqlTradeResult &result)
{
    ...
    // Price Levels
    string trade_symbol = (string)trans.symbol; //--- the name of the symbol, for which a transaction was performed
    ...
    Print("line: ",__LINE__," stop_loss: ",stop_loss);
    Print("line: ",__LINE__," take_profit: ",take_profit);


    // string variables:
    // -----------------------------------------------------
    string stransaction_type = StringType(transaction_type); 

    // Orders
    string sorder_ID    = (string)order_ID;
    string sorder_type  = StringType(order_type);  // Order type
    string sorder_state = (string)order_state; // Order state

    // Deals
    string sdeal_ID   = (string)deal_ID;
    string sdeal_type = (string)deal_type;

    // Positions
    string sposition_ID = (string)position_ID;

    // Price Levels
    string strade_symbol = (string)trade_symbol;
    string slot_size     = (string)lot_size;
    string sentry_price  = (string)entry_price;
    string sstop_loss    = (string)stop_loss;
    string stake_profit  = (string)take_profit;
    ...
    // if (transaction_type == TRADE_TRANSACTION_ORDER_ADD || transaction_type == TRADE_TRANSACTION_ORDER_UPDATE || transaction_type == TRADE_TRANSACTION_ORDER_DELETE) {
    
    if (transaction_type == TRADE_TRANSACTION_ORDER_ADD) {
        string msg = StringFormat("NEW ORDER \nOrder Number: %s \nSymbol: %S \nOrder Type: %s \nLot Size: %s \n", 
        sorder_ID, strade_symbol, sorder_type, slot_size);
        // (string)order_ID, (string)trade_symbol, (string)order_type, (string)lot_size);
        ...
        if (order_type == ORDER_TYPE_BUY || order_type == ORDER_TYPE_BUY_LIMIT || order_type == ORDER_TYPE_BUY_STOP) {
            ...
            strade_symbol, sorder_type, sentry_price, sstop_loss, stake_profit);
            // trade_symbol, order_type, entry_price, stop_loss, take_profit);

            // Send Signal to named Channel
            if (SendScreenShot) {
                sendSnapShots(strade_symbol, ScreenShotTimeFrame, msg);  
            }
            if (!SendScreenShot) {
                int res = bot.SendMessage(InpChannelName, msg); 
            }
    ...
    if (transaction_type == TRADE_TRANSACTION_POSITION) {
        // string msg = StringFormat("Position Update: Position  #%d %s \nSymbol: %s \nNew Levels: SL=%.5f TP=%.5f", position_ID, trade_symbol, stop_loss, take_profit);
        string msg = StringFormat("Position Update: Position  %s \nSymbol: %s \nNew Levels: SL=%s TP=%s", 
        sposition_ID, strade_symbol, sstop_loss, stake_profit);
    ...
    // if (order_state == ORDER_STATE_PLACED || order_state == ORDER_STATE_FILLED || order_state == ORDER_STATE_CANCELED) {
    ...
    // if (transaction_type == TRADE_TRANSACTION_DEAL_ADD || transaction_type == TRADE_TRANSACTION_DEAL_UPDATE || transaction_type == TRADE_TRANSACTION_DEAL_DELETE) {
    ...
    //     string msg = StringFormat("Trade Signal \nSymbol: %s \nOrder Type: %s \nEntry Price: %s \nStop Loss: %s \nTake Profit: %s", trade_symbol, order_type, entry_price, stop_loss, take_profit);
...
void OnClickSendTelegram()
{
    ...
    sendSnapShots(Symbol(), PERIOD_CURRENT, ms);  
