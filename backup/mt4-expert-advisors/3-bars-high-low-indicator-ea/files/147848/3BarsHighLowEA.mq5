...
string file_custom_indicator = "3-bars-high-low-indicator";
int    indicator;
...
input string       TI                      = "== Indicator Setup ==";        // == Indicator Setup ==
input int          uPeriodcci              = 50;    // CCI period
input int          uATRperiod              = 5;     // ATR Period
...
Flag      allowToOpenBuys(false);
Flag      allowToOpenSells(false);
...
class BUYcondition1 : public iConditions
{
 public:
  bool evaluate()
  {
    ...
    int limit;
    if (signalMode == OneCandle) limit = 1;
    if (signalMode == TwoCandles) limit = 2;
    if (signalMode == TreeCandles) limit = 3;
    if (signalMode == FourCandles) limit = 4;
    ...
    for (int i = 1; i <= limit; i++) {
      double close = iClose(NULL, 0, i);
			Print(__FUNCTION__," iClose: i: ",close, " ", i);
    
		  // double indi = iCustom(NULL, 0, file_custom_indicator, uBars, uPeriodcci, uATRperiod, false, false, false, 0, i);
      double indi = calculate(indicator, 0, i);

      Print(__FUNCTION__," indi: ",indi);
      if (close < indi) return false;
		 }
...
int OnInit()
{
  ...
  adx.setSetup(AdxPeriod, AdxAppliedPrice);
	...
	indicator = iCustom(NULL,0,file_custom_indicator,uPeriodcci,uATRperiod);
...
double calculate(int handle, int buffer, int i)
{
	...
	//  int    copy = CopyBuffer(handle, buffer, shift, 1, value);
	 int    copy = CopyBuffer(handle, buffer, i, 1, value);
	 if (copy > 0) { return value[0]; }
	 //--- 
	 return -1;
}

void controlateSides()
{
			double close = iClose(NULL, 0, 1);
			double closePrev = iClose(NULL, 0, 2);
			double low = iLow(NULL,0,1);
			double hi = iHigh(NULL,0,1);
			// double indi = iCustom(NULL, 0, file_custom_indicator, uBars, uPeriodcci, uATRperiod, false, false, false, 0, 1);
      double indi = calculate(indicator, 0, 1);

      if(closePrev < indi && close > indi) { allowToOpenBuys.On(); }
			if(closePrev > indi && close < indi) { allowToOpenSells.On(); }
}
