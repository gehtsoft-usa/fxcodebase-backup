...
// https://fxcodebase.com/code/viewtopic.php?f=38&p=159439#p159439
...
input const double       grid_p_step         = 50;                   // Grid Step:
input enum_addition_mode grid_p_add_mode     = grid_mode_multiplier; // Increment lots mode:
input double             grid_p_m            = 2;                    // Grid Lot multiplier:
input double             grid_p_a            = 0.1;                  // Grid Lot Addition:
input const int          grid_p_max_steps    = 10;                   // Grid Max Steps:
input double             grid_p_close_profit = 200;                  // Profit to close grid (0 = off):
input double             grid_p_close_loss   = 0;                    // Loss to close grid (0 = off):
...
class ActionGridPositive : public iActions
{
  public:
    ...
    bool execute()
    {
        ...
        if (OrderType == POSITION_TYPE_BUY) {
            for (int n = 1; n <= grid_p_max_steps; n++) {
                ...
                Print("line: ",__LINE__," last_step: ",last_step);
    ...
    double GridLot(double lot, double n)
    {
        ...
        switch (grid_p_add_mode) {
            case grid_mode_multiplier:
                l = lot * pow(grid_p_m, (double)n);
                break;

            case grid_mode_addition:
                l = lot + (grid_p_a * (double)n);
                break;
        }
    ...
    bool execute()
    {
        ...
        for (int i = PositionsTotal() - 1; i >= 0; i--) {
            ...
            if (PositionGetString(POSITION_COMMENT) == "") {
                ...
                if (grid_p_close_profit > 0 && current_profit >= grid_p_close_profit) closeGrid(tk);
                if (grid_p_close_loss > 0 && current_profit <= -grid_p_close_loss) closeGrid(tk);
            }
        }
        return true;
    }
...
Strategy stCloseGridPositiveByProfit;
Strategy stCloseGridPositiveBecauseOriginalTradeIsClose;

void OnInit_Grid_Positive()
{
    if (!grid_p_on) return;
    stGridPositive.set(&cdGridPositive, &acGridPositive);
    stCloseGridPositiveByProfit.set(&cdCloseGridPositiveByProfit, &acCloseGridPositive);
    stCloseGridPositiveBecauseOriginalTradeIsClose.set(&cdOriginalTkIsClose, &acEmpty);
...
input string    Tbasic    = "== Strategy Setup ==";              // ————————————————————————
input string ureftm ="14:30"; // Reference Candle Open Time:
datetime reftm;
input int candlesFromRef = 5; // Number of candles to enter:

datetime Ref_tm()
{
    reftm = StringToTime(ureftm);
    datetime candleReftm = server_tm_from_local_tm(reftm); // hora de la vela ref
    return candleReftm;
}

datetime server_tm_from_local_tm(datetime localToConvert)
{
    datetime r = localToConvert += int(TimeTradeServer() - TimeLocal());
    return r;
}

double Ref_price()
{
    double ref_price = iOpen(NULL, 0, CandlesfromRefCandle());
    return ref_price;  
}

int CandlesfromRefCandle()
{
    int candlesfromRefCandle = iBarShift(NULL,0, Ref_tm()); 
    return candlesfromRefCandle;
}
...
class BUYcondition1 : public iConditions
{
  public:
    bool evaluate()
    {
        if(CandlesfromRefCandle() == candlesFromRef+1) // vela siguiente a las que pide el usuario
        {
            if(iClose(NULL,0,1) > Ref_price())
            {
                return true;
            }
        }
...
input bool   close_by_strategy_on = true;                   // Close by time on?:
input string uclosetm ="20:00"; // Close Time:
  ...
  public:
    bool evaluate()
    {
        ...
        if (PositionGetInteger(POSITION_TYPE) == POSITION_TYPE_BUY) {
            ...
            datetime closetm = StringToTime(uclosetm);
            if(TimeLocal() >= closetm) r = true;
        }
        return r;
    }
};
