...
// http://fxcodebase.com/code/viewtopic.php?f=38&t=67454
...
extern double LongDistance1 = 10; // Long #1 distance, pips
extern double LongLotSize2 = 0.1; // Long lot size #2
extern double LongDistance2 = 20; // Long #2 distance, pips
extern double LongLotSize3 = 0.1; // Long lot size #3
extern double LongDistance3 = 30; // Long #3 distance, pips
extern double LongLotSize4 = 0.1; // Long lot size #4
extern double LongDistance4 = 40; // Long #4 distance, pips
extern double LongLotSize5 = 0.1; // Long lot size #5
extern double LongDistance5 = 50; // Long #5 distance, pips
extern double LongLotSize6 = 0.1; // Long lot size #6
extern double LongDistance6 = 60; // Long #6 distance, pips
extern double ShortLotSize1 = 0.1; // Short lot size #1
extern double ShortDistance1 = 10; // Short #1 distance, pips
extern double ShortLotSize2 = 0.1; // Short lot size #2
extern double ShortDistance2 = 20; // Short #2 distance, pips
extern double ShortLotSize3 = 0.1; // Short lot size #3
extern double ShortDistance3 = 30; // Short #3 distance, pips
extern double ShortLotSize4 = 0.1; // Short lot size #4
extern double ShortDistance4 = 40; // Short #4 distance, pips
extern double ShortLotSize5 = 0.1; // Short lot size #5
extern double ShortDistance5 = 50; // Short #5 distance, pips
extern double ShortLotSize6 = 0.1; // Short lot size #6
extern double ShortDistance6 = 60; // Short #6 distance, pips
extern double LongDistance = 10; // Long distance, pips
extern double ShortDistance = 10; // Short distance, pips
extern int slippagePoints = 3; // Slippage, points
extern int magic_Number = 42; // Magic number
extern bool UseStopLoss = false; // Set stop loss?
extern double StopLossValue = 10; // Stop loss
...
public:
   ...
   void SetAction(const int order, IAction *action)
   {
      int count = ArraySize(_triggers);
      for (int i = 0; i < count; ++i)
      {
         if (_triggers[i].SetAction(order, action))
            return;
      }
      ArrayResize(_triggers, count + 1);
      _triggers[count] = new TradeTrigger();
      _triggers[count].SetAction(order, action);
   }
};

class BuyAction : public IAction
{
public:
   virtual void DoAction()
   {
      ...
      int order = orderBuilder
         .SetRate(price + LongDistance * instrument.GetPipSize())
         .SetSide(BuySide)
      ...
      if (order == -1)
         ...
         tradeTriggerManager.SetAction(order, &sellAction);
   }
};

class SellAction : public IAction
{
public:
   virtual void DoAction()
   {
      ...
      if (order == -1)
         ...
         tradeTriggerManager.SetAction(order, &buyAction);
   }
};
...
int CreateNewOrder(const string symbol, const bool isBuy, const double rate, const double amount)
{
   InstrumentInfo instrument(symbol);
   string error;
   OrderBuilder *orderBuilder = new OrderBuilder();
   if (UseStopLoss)
   {
      ...
      double stopLoss = calculator.CalculateStopLoss(isBuy, StopLossValue, StopLimitPips, amount, rate);
      delete calculator;
      orderBuilder.SetStopLoss(stopLoss);
   }
   int order = orderBuilder
      .SetRate(rate)
...
void OpenOrders()
{
   ...
   int newOrder = CreateNewOrder(symbol, true, rate + LongDistance1 * instrument.GetPipSize(), LongLotSize1);
   if (newOrder != -1)
      tradeTriggerManager.SetAction(newOrder, &buyAction);
   newOrder = CreateNewOrder(symbol, true, rate + LongDistance2 * instrument.GetPipSize(), LongLotSize2);
   if (newOrder != -1)
      tradeTriggerManager.SetAction(newOrder, &buyAction);
   newOrder = CreateNewOrder(symbol, true, rate + LongDistance3 * instrument.GetPipSize(), LongLotSize3);
   if (newOrder != -1)
      tradeTriggerManager.SetAction(newOrder, &buyAction);
   newOrder = CreateNewOrder(symbol, true, rate + LongDistance4 * instrument.GetPipSize(), LongLotSize4);
   if (newOrder != -1)
      tradeTriggerManager.SetAction(newOrder, &buyAction);
   newOrder = CreateNewOrder(symbol, true, rate + LongDistance5 * instrument.GetPipSize(), LongLotSize5);
   if (newOrder != -1)
      tradeTriggerManager.SetAction(newOrder, &buyAction);
   newOrder = CreateNewOrder(symbol, true, rate + LongDistance6 * instrument.GetPipSize(), LongLotSize6);
   if (newOrder != -1)
      tradeTriggerManager.SetAction(newOrder, &buyAction);

   newOrder = CreateNewOrder(symbol, false, rate - ShortDistance1 * instrument.GetPipSize(), ShortLotSize1);
   if (newOrder != -1)
      tradeTriggerManager.SetAction(newOrder, &sellAction);
   newOrder = CreateNewOrder(symbol, false, rate - ShortDistance2 * instrument.GetPipSize(), ShortLotSize2);
   if (newOrder != -1)
      tradeTriggerManager.SetAction(newOrder, &sellAction);
   newOrder = CreateNewOrder(symbol, false, rate - ShortDistance3 * instrument.GetPipSize(), ShortLotSize3);
   if (newOrder != -1)
      tradeTriggerManager.SetAction(newOrder, &sellAction);
   newOrder = CreateNewOrder(symbol, false, rate - ShortDistance4 * instrument.GetPipSize(), ShortLotSize4);
   if (newOrder != -1)
      tradeTriggerManager.SetAction(newOrder, &sellAction);
   newOrder = CreateNewOrder(symbol, false, rate - ShortDistance5 * instrument.GetPipSize(), ShortLotSize5);
   if (newOrder != -1)
      tradeTriggerManager.SetAction(newOrder, &sellAction);
   newOrder = CreateNewOrder(symbol, false, rate - ShortDistance6 * instrument.GetPipSize(), ShortLotSize6);
   if (newOrder != -1)
      tradeTriggerManager.SetAction(newOrder, &sellAction);
}
