//Available @ https://fxcodebase.com/code/viewtopic.php?f=38&t=75510
...
input MATypes ma_type = ma_sma; // MA type
input int ltma_1_length = 30; // Long term MA#1 period
input int ltma_2_length = 35; // Long term MA#2 period
input int ltma_3_length = 40; // Long term MA#3 period
input int ltma_4_length = 45; // Long term MA#4 period
input int ltma_5_length = 50; // Long term MA#5 period
input int ltma_6_length = 60; // Long term MA#6 period
input int stma_1_length = 3; // Short term MA#1 period
input int stma_2_length = 5; // Short term MA#2 period
input int stma_3_length = 8; // Short term MA#3 period
input int stma_4_length = 10; // Short term MA#4 period
input int stma_5_length = 12; // Short term MA#5 period
input int stma_6_length = 15; // Short term MA#6 period
...
public:
   EntryStreamData(const string symbol, const ENUM_TIMEFRAMES timeframe)
   {
      ...
      ArrayResize(ltma, 6);
      ltma[0] = AveragesStreamFactory::Create(source, ltma_1_length, ma_type);
      ltma[1] = AveragesStreamFactory::Create(source, ltma_2_length, ma_type);
      ltma[2] = AveragesStreamFactory::Create(source, ltma_3_length, ma_type);
      ltma[3] = AveragesStreamFactory::Create(source, ltma_4_length, ma_type);
      ltma[4] = AveragesStreamFactory::Create(source, ltma_5_length, ma_type);
      ltma[5] = AveragesStreamFactory::Create(source, ltma_6_length, ma_type);
      ArrayResize(stma, 6);
      stma[0] = AveragesStreamFactory::Create(source, stma_1_length, ma_type);
      stma[1] = AveragesStreamFactory::Create(source, stma_2_length, ma_type);
      stma[2] = AveragesStreamFactory::Create(source, stma_3_length, ma_type);
      stma[3] = AveragesStreamFactory::Create(source, stma_4_length, ma_type);
      stma[4] = AveragesStreamFactory::Create(source, stma_5_length, ma_type);
      stma[5] = AveragesStreamFactory::Create(source, stma_6_length, ma_type);
      source.Release();
   }
   ~EntryStreamData()
   {
      for (int i = 0; i < ArraySize(ltma); ++i)
      {
         ltma[i].Release();
      }
      for (int i = 0; i < ArraySize(stma); ++i)
      {
         stma[i].Release();
      }
   }
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      double st = EMPTY_VALUE;
      double lt = EMPTY_VALUE;
      for (int i = 0; i < 6; ++i)
      {
         double v;
         if (!data.ltma[i].GetValue(period, v))
         {
            return false;
         }
         if (lt == EMPTY_VALUE || lt < v)
         {
            lt = v;
         }
         if (!data.stma[i].GetValue(period, v))
         {
            return false;
         }
         if (st == EMPTY_VALUE || st > v)
         {
            st = v;
         }
      }
