...
input int Price=0;      // Applied price
                         ...
                         // 5 - Typical
                         // 6 - Weighted  
input int Average_Length=10;
input int Average_Method=0;  // 0 - SMA
                              // 1 - EMA
                              // 2 - SMMA
                              // 3 - LWMA
input double Buy_Level=0.5;
input double Sell_Level=-0.5; 
...
int OnInit()
{
     double temp = iCustom(NULL, 0, "Fan_CCI2", 0, 0);
   if (GetLastError() == ERR_INDICATOR_CANNOT_LOAD)
   {
       Alert("Please, install the 'Fan_CCI2' indicator");
       return INIT_FAILED;
   }
   LastBar=0;
...
void OnTick()
{
 ...
 FCCI0=iCustom(NULL, 0, "Fan_CCI2", First_Length, Increment_Type_Str, Increment_Type, Length_Step, CCI_Count, Price, Average_Length, Average_Method, 2, 1);
 FCCI1=iCustom(NULL, 0, "Fan_CCI2", First_Length, Increment_Type_Str, Increment_Type, Length_Step, CCI_Count, Price, Average_Length, Average_Method, 2, 2);
 
 int CO;
 int res;
 double SL, TP;
 
 if ((FCCI1<Buy_Level && FCCI0>=Buy_Level && !Inverse_Direction) || (FCCI1>Sell_Level && FCCI0<=Sell_Level && Inverse_Direction))
 {
  _Alert("Buy");
  CO=CalculateOrders();
  if (CO<=0)
 ...
 if ((FCCI1>Sell_Level && FCCI0<=Sell_Level && !Inverse_Direction) || (FCCI1<Buy_Level && FCCI0>=Buy_Level && Inverse_Direction))
 {
  _Alert("Sell");
  CO=CalculateOrders();
  if (CO>=0)
