...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      double value = iCustom(_symbol, _timeframe, "Magic Fx Formula", false, false, 1, period);
      return value != 0;
   }
};
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      double value = iCustom(_symbol, _timeframe, "Magic Fx Formula", false, false, 0, period);
      return value != 0;
   }
};
...
int OnInit()
{
   double temp = iCustom(NULL, 0, "Magic Fx Formula", 0, 0);
   if (GetLastError() == ERR_INDICATOR_CANNOT_LOAD)
   {
      Alert("Please, install the 'Magic Fx Formula' indicator");
      return INIT_FAILED;
   }
   #ifdef SHOW_ACCOUNT_STAT
      stats = NULL;
