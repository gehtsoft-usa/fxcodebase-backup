...
Version:     
Date:        
Repository:  Available @ https://fxcodebase.com/code/viewtopic.php?f=38&t=76429
License:     GNU
*/
...
input int    rsi_level_buy  = 10;                // Level Buy
input int    rsi_level_sell = 90;                // Level Sell
  ...
  public:
    bool evaluate()
    {
        ...
        for (int i = 1; i <= candles_back_for_signals; i++) {
            double rsi = RSI(i);
            double rsi_prev = RSI(i+1);
            if (rsi < rsi_level_buy && rsi_prev >= rsi_level_buy) {
                return true;
            }
        }
        return r;
  ...
  public:
    bool evaluate()
    {
        ...
        for (int i = 1; i <= candles_back_for_signals; i++) {
            double rsi = RSI(i);
            double rsi_prev = RSI(i+1);
            if (rsi > rsi_level_sell && rsi_prev <= rsi_level_sell) {
                return true;
            }
        }
        return r;
