...
// ------------------------------------------------------------------
input string             TtmaCentered              = "== TMA Centered Setup ==";  // == TMA Centered Setup ==
input ENUM_TIMEFRAMES    indi1_TimeFrame           = 0;                           // TimeFrame
input int                indi1_HalfLength          = 34;                          // HalfLength
input ENUM_MA_METHOD     indi1_HLma_method         = 0;                           // HLma_method
input ENUM_APPLIED_PRICE indi1_HLapplied_price     = PRICE_CLOSE;                 // HLapplied_price
input int                indi1_Shift               = 5;                           // Shift
input int                indi1_MA_Period           = 34;                          // MA_Period
input int                indi1_ATR_Period          = 900;                         // ATR_Period
input double             indi1_ATR_Multiplier_Band = 2.0;                         // ATR_Multiplier_Band
input double             indi1_Risk                = 1;                           // Risk
// ------------------------------------------------------------------
input string TtmaSlopeBands = "== TMA Slope Bands ==";  // == TMA Slope Bands ==
input int    indi2_HalfLength    = 60;                  // HalfLength
input int    indi2_Price         = PRICE_CLOSE;         // Price
input double indi2_ATRMultiplier = 3.0;                 // ATRMultiplier
input int    indi2_ATRPeriod     = 300;                 // ATRPeriod
// ------------------------------------------------------------------
...
bool haveSignalBuy()
{
  ...
  double valueNull = 0;
	...
	double indi1Buy = iCustom(NULL, indi1_TimeFrame, "TMA_Centered_H1.ex4", indi1_HalfLength, indi1_HLma_method, indi1_HLapplied_price, indi1_Shift, indi1_MA_Period, indi1_ATR_Period, indi1_ATR_Multiplier_Band, indi1_Risk, buffer, shift);

	Print("indi1Buy: ", indi1Buy);

	if (indi1Buy > valueNull) { return true; } return false; 
...
bool haveSignalSell()
{
  ...
  double indi1Sell = iCustom(NULL, indi1_TimeFrame, "TMA_Centered_H1.ex4", indi1_HalfLength, indi1_HLma_method, indi1_HLapplied_price, indi1_Shift, indi1_MA_Period, indi1_ATR_Period, indi1_ATR_Multiplier_Band, indi1_Risk, buffer, shift);

  Print("indi1Sell: ", indi1Sell);

  if (indi1Sell > valueNull)
  {
    return true;
  }
  return false;
...
bool haveUpTrend()
{
  ...
  if(iCustom(NULL, 0, "TMA_slope_bands.ex4", indi2_HalfLength, indi2_Price, indi2_ATRMultiplier, indi2_ATRPeriod, buffer, shift) == value)
  {
    return true;
  }
  return false;
