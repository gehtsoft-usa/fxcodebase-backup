...
enum ModeLevelsTP {
  tp_FixPips,             // Fix Pips
  tp_byMoney,             // Money
  tp_PipsFromOpenCandle,  // Pips from Candle
  tp_ByRatio              // TP by Ratio Risk Reward
};
enum TSLMode {
  byPips,  // By Pips
...
string file_custom_indicator = "Inside_Bar_Indicator.ex4";
int bufferToBuy = 0;
int bufferToSell = 1;
...
input ModeCalcLots modeCalcLots            = EquityPercent;      // Mode to Calc Lots:
...
input ModeLevelsTP modeTP         = tp_ByRatio;         // Mode Take Profit:
input int uRatioTP = 1; // Ratio Risk Reward:
...
input int maxSL = 40; // SL Max Distance:
input int minSL = 35; // SL Distance if Max Distance is reach:
...
input int      maxTradesBySession = 2;                  // Max Trades By Session:
input double   uTimeZone = -5;                          // Set your GMT zone:
input bool     day1_On    = true;                       // Session On:
input enumDays day1       = monday;                     // Day
input string   day1_Start = "14:00:00";                 // Time Start GMT
input string   day1_End   = "16:00:00";                 // Time End GMT
input bool     day2_On    = true;                       // Session On:
input enumDays day2       = tuesday;                    // Day
input string   day2_Start = "14:00:00";                 // Time Start GMT
input string   day2_End   = "16:00:00";                 // Time End GMT
input bool     day3_On    = true;                       // Session On:
input enumDays day3       = wednesday;                  // Day
input string   day3_Start = "14:00:00";                 // Time Start GMT
input string   day3_End   = "16:00:00";                 // Time End GMT
input bool     day4_On    = true;                       // Session On:
input enumDays day4       = thursday;                   // Day
input string   day4_Start = "14:00:00";                 // Time Start GMT
input string   day4_End   = "16:00:00";                 // Time End GMT
input bool     day5_On    = true;                       // Session On:
input enumDays day5       = friday;                     // Day
input string   day5_Start = "14:00:00";                 // Time Start GMT
input string   day5_End   = "16:00:00";                 // Time End GMT
#endif
#ifdef TIMER_MINI
input string T1        = "== Trading Sessions ==";  // ————————————
...
class ByRatio : public iLevels
{
  ...
  public:
   ...
   void setPips()
   {
       ...
       Print(__FUNCTION__," _sl.pips() ",_sl.pips());
 ...
 public:
  ...
  double calculateLevel()
  {
    ...
    if (_side == "buy")
    {
        ...
        if(_mode == "TP"){
            _pips = Distancia(Ask, high, _Symbol);
            return high + distance;
        }
        if(_mode == "SL"){ _pips = Distancia(Ask, low, _Symbol);  return low - distance; }
    }
    if (_side == "sell")
    {
      if (_mode == "TP"){_pips = Distancia(Bid, low, _Symbol); return low - distance; }
      if (_mode == "SL"){_pips = Distancia(Bid, high, _Symbol); return high + distance;}
    }
    return -1;
  }
};
 ...
 public:
  bool evaluate()
  {
    ...
    //   return Indi(bufferToBuy, 1) > 0;
      return Indi(bufferToBuy, 0) != EMPTY_VALUE;   
      //   return false;
  }
};
...
class SELLcondition1 : public iConditions
{
 public:
  bool evaluate()
  {
    ...
    return Indi(bufferToSell, 0) != EMPTY_VALUE;
    // return false;
  }
};
...
void OnTick()
{
  ...
  if (TradeOnThisCandle())
  {
      return;
  }

  if(CheckTradesOnThisSession())
  {
      return;
  }
  ...
  if (!uTradeReverse)
  {
    if (conditionsToBuy.EvaluateConditions())
    {
      if (modeEntry == Market)
      {
          double BuySL = SL("buy", Ask);
          double BuyTP = TP("buy", Ask);
          actionSendOrder = new SendNewOrder("buy", Lots(), "", 0, BuySL, BuyTP, magico);
      } else
      {
          double pr = Price("buy", uEntryDistance, _Symbol);
          double BuySL = SL("buy", pr);
          double BuyTP = TP("buy", pr);
          actionSendOrder = new SendNewOrder("buy", Lots(), "", pr, BuySL, BuyTP, magico);
      }

      if (actionSendOrder.doAction())
      {
        mainOrders.GetMarketOrders();
        Print(__FUNCTION__, " ", GetPointer(levelSL));
    ...
    if (conditionsToSell.EvaluateConditions())
    {
      if (modeEntry == Market)
      {
          double _SL = SL("sell", Bid);
          double _TP = TP("sell", Bid);
          actionSendOrder = new SendNewOrder("sell", Lots(), "", 0, _SL, _TP, magico);
      } else
      {
          double pr = Price("sell", uEntryDistance, _Symbol);
          double _SL = SL("buy", pr);
          double _TP = TP("buy", pr);
          actionSendOrder = new SendNewOrder("sell", Lots(), "", pr, _SL, _TP, magico);
      }
      if (actionSendOrder.doAction())
      {
        mainOrders.GetMarketOrders();
    ...
    switch (modeSL)
    {
      ...
      case byMoney:
        ...
        case PipsFromOpenCandle:
        {
            int candle = -1;
            for(int i = 0; i <= 4; i++) {
              if(Indi(2, i) != EMPTY_VALUE) {
                  candle = i; break;
              } 
          }

            Print(__FUNCTION__, " candle: ", candle);
            if(candle !=-1)
                levelSL = new Levels(new ByPipsFromCandle(_Symbol, side, userSLpips, "SL", 0, candle));
            else
                levelSL = new Levels(new ByFixPips(_Symbol, side, userSLpips, "SL", price));
            result = levelSL.calculateLevel();

        // si el result es mayor a la dstancia maxima permitida recortar a la minima
            if (side == "buy")
            {
                if(Distancia(Ask, result, _Symbol) > maxSL)
                {
                    result = Ask - (minSL * 10 * _Point);
                }                
            }
            if (side == "sell")
            {
                if(Distancia(Bid, result, _Symbol) > maxSL)
                {
                    result = Bid + (minSL * 10 * _Point);
                }                
            }            
            break;
        }
  ...
  if (takeProfitOn)
    switch (modeTP)
    {
      ...
      case tp_byMoney:
        ...
        result  = levelTP.calculateLevel();
        break;
      case tp_PipsFromOpenCandle:
        levelTP = new Levels(new ByPipsFromCandle(_Symbol, side, userSLpips, "TP", 0, 1));
        result = levelTP.calculateLevel();
        break;
      case tp_ByRatio:
          Print(__FUNCTION__ , " ", GetPointer(levelSL));
          levelTP = new Levels(new ByRatio(_Symbol, side, price, uRatioTP, "TP", levelSL.level()));
          result = levelTP.calculateLevel();
        break;
    }
        // si el result es mayor a la dstancia maxima permitida recortar a la minima
...
bool TradeOnThisCandle()
{
    for(int i = OrdersHistoryTotal() - 1;i >= 0;i--){
       if(OrderSelect(i,SELECT_BY_POS,MODE_HISTORY) && OrderSymbol() == _Symbol && OrderMagicNumber() == magico){
          if (OrderOpenTime() > iTime(_Symbol, 0, 0)){
              return true;
          }           
       }
       break;
...
bool CheckTradesOnThisSession()
{
    ...
    for(int i = OrdersHistoryTotal() - 1;i >= 0;i--)
    {
        if(OrderSelect(i, SELECT_BY_POS, MODE_HISTORY) && OrderSymbol() == _Symbol && OrderMagicNumber() == magico)
        {
            ...
            int cl_seg = sesionControl.at().endTime();
            datetime closeSession = dt + cl_seg;

            int op_seg = sesionControl.at().iniTime();
            datetime openSession = dt + op_seg;
            datetime openTime = OrderOpenTime() + uTimeZone * (60 * 60);

          if(openTime < closeSession && openTime > openSession)
          {
               count++;
          }           
       }
