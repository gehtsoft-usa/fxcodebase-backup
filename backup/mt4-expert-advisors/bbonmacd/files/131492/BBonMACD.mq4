...
//http://fxcodebase.com/code/viewtopic.php?f=38&t=69452
...
input int rapida = 8; // Media Rapida
input int lenta = 26; // Media Lenta
input double stdv = 0.8; // Stdv
input color color_1 = Red; // MACD color 1
input color color_2 = Green; // MACD color 2
input color color_3 = Blue; // MACD color 3
input color banda_supe_color = Green; // Banda Superior Color
input color banda_inf_color = Blue; // Banda Inferior Color
...
public:
   ...
   {
      return bbmacd._data[period] > banda_supe[period] 
         && bbmacd._data[period + 1] < banda_supe[period + 1];
   }
};
...
ColoredStream* bbmacd;
double banda_supe[], banda_inf[];
StDevStream* stdevStream;
...
int init()
{
   ...
   IndicatorName = GenerateIndicatorName("Bollinguer sobre Macd");
   ...
   int id = 0;
   bbmacd = new ColoredStream(_Symbol, (ENUM_TIMEFRAMES)_Period);
   id = bbmacd.RegisterStream(id, color_1, "BB MACD", DRAW_HISTOGRAM);
   id = bbmacd.RegisterStream(id, color_2, "BB MACD", DRAW_HISTOGRAM);
   id = bbmacd.RegisterStream(id, color_3, "BB MACD", DRAW_HISTOGRAM);

   SetIndexStyle(id, DRAW_LINE, STYLE_SOLID, 1, banda_supe_color);
   SetIndexBuffer(id, banda_supe);
   SetIndexLabel(id, "banda_supe");
   ++id;

   SetIndexStyle(id, DRAW_LINE, STYLE_SOLID, 1, banda_inf_color);
   SetIndexBuffer(id, banda_inf);
   SetIndexLabel(id, "banda_inf");
   ++id;

   stdevStream = new StDevStream(bbmacd, 9);
...
int start()
{
   ...
   {
      double m_rapida = iMA(_Symbol, _Period, rapida, 0, MODE_EMA, PRICE_CLOSE, pos);
      double m_lenta = iMA(_Symbol, _Period, lenta, 0, MODE_EMA, PRICE_CLOSE, pos);
      bbmacd._data[pos] = m_rapida - m_lenta;
      double Avg = iMAOnArray(bbmacd._data, 0, 9, 0, MODE_EMA, pos);
      bbmacd.Set(m_rapida - m_lenta, pos, 0);

      double SDev;
      if (stdevStream.GetValue(pos, SDev))
      {
         banda_supe[pos] = Avg + stdv * SDev;
         banda_inf[pos] = Avg - stdv * SDev;
      }
      if (bbmacd._data[pos] < banda_inf[pos])
         bbmacd.Set(m_rapida - m_lenta, pos, 0);
      else if (bbmacd._data[pos] > banda_supe[pos])
         bbmacd.Set(m_rapida - m_lenta, pos, 1);
      else
         bbmacd.Set(m_rapida - m_lenta, pos, 2);
      
      customStream._stream[pos] = bbmacd._data[pos];
      for (int i = 0; i < ArraySize(conditions); ++i)
      {
