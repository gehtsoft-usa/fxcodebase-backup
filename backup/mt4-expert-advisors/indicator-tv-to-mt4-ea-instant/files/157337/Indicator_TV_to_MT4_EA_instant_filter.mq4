...
input string TindiFilter = "== Indicator Filter ==";  // ————————————
input string file_filter_indicator = "AI_Trend_Navigator.ex4"; // Indicator File to Filter:
input bool   filterbyknnOn = true; // Filter By KNN?

double KNN_UP(int candle)
{
    return iCustom(NULL, 0, file_filter_indicator, 0, candle);
}
double KNN_DN(int candle)
{
    return iCustom(NULL, 0, file_filter_indicator, 1, candle);
}
...
input bool         open_instant   = false;               // Open a Trade Inmediatly ?
...
input bool         closeAllInOpositeSignal = false;                       // Close in opposite by signal:
input bool         closeInOpositeByKNN     = false;                       // Close in opposite by knn filter:
...
// Note: autotrading section
input string T1        = "== Trading Sessions ==";  // ————————————
input bool   tradingSessionOn = false;              // Session Timer On?
input string timeStart = "00:00:00";                // Time Start Local
input string timeEnd   = "23:59:59";                // Time End Local
input bool   openATradeAtStart = true; // Open a trade (if no trade open):
input bool   autotradingOffAtEndTm = true; // AutoTrading Off at end time:
input bool   closeAllAtEndTm = true;       // Close All trades at end time:
...
class ScheduleController
{
 ...
 public:
  ...
  void setCurrentDay()
  {
    ...
    _currentDay = TimeDay(TimeLocal() + _timeZone);  // return the day of the month 1-31
  }

  bool isNewDay()
  {
    // if (TimeDay(TimeGMT() + _timeZone) != _currentDay)
    if (TimeDay(TimeLocal() + _timeZone) != _currentDay)
    {
      setCurrentDay();
      return true;
    }
  ...
  bool doSessionControl()  // control day and hours for every session
  {
    ...
    int actual = (TimeHour(TimeLocal() + _timeZone) * 3600) + (TimeMinute(TimeLocal() + _timeZone) * 60);
    ...
    for (int i = 0; i < qnt(); i++)
    {
      ...
      {
        if (schedules[i].dayNumber() == TimeDayOfWeek(TimeLocal() + _timeZone))
        {
...
class BUYcondition2 : public iConditions
{
 public:
  bool evaluate()
  {
    ...
    if(filterbyknnOn) return KNN_UP(1) != EMPTY_VALUE && KNN_UP(1) > 0 && KNN_DN(1) == EMPTY_VALUE;
    // 
    return true;
  }
};
...
class SELLcondition2 : public iConditions
{
 public:
  bool evaluate()
  {
    ...
    if(filterbyknnOn) return KNN_DN(1) != EMPTY_VALUE && KNN_DN(1) > 0 &&  KNN_UP(1) == EMPTY_VALUE;
    // 
    return true;
  }
};
...
class ConditionToCloseBuy : public iConditions
{
 public:
  bool evaluate()
  {
    ...
    if (closeInOpositeByKNN) 
			if (sellCondition2.evaluate()) return true;
...
if(open_instant == true && executed == false) {
      int ib = 0;
      double indi = EMPTY_VALUE;
      while (indi == EMPTY_VALUE){
          indi = Indi(bufferToBuy, ib);
          Print("line: ",__LINE__," indi: ",indi);
          // double indi = Indi(bufferToBuy, ib) == EMPTY_VALUE) 
       ib ++; 
      }
      
      int is = 0;
      while (Indi(bufferToSell, is) == EMPTY_VALUE) { is ++; }
...
void OnTick()
{
  ...
  if(tradingSessionOn)
  {
    if (sesionControl.doSessionControl())
    {
      ...
      if(openATradeAtStart == true && countOpenTrades()==0) {
        
        if(buyCondition2.evaluate()) {
        actionSendOrder = new SendNewOrder("buy", Lots(), "", 0, SL("buy"), TP("buy"), magico);
    ...
    if (!sesionControl.doSessionControl())
    {
      ...
      bool allow = TerminalInfoInteger(TERMINAL_TRADE_ALLOWED);
      if(autotradingOffAtEndTm== true && allow==true) 
      {
        AutotradingOff();
      }
...
{
    bool allow =  TerminalInfoInteger(TERMINAL_TRADE_ALLOWED);
    Print("line: ",__LINE__," allow: ",allow);
