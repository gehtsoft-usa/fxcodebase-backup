...
input bool               filterByEmaOn  = true; // Filter By EMAS On?
...
input string             Iema_slow           = "== Moving Average Slow Setup ==";  // ————————————
input int                maPeriod_slow       = 200;                            // Period
input int                maShift_slow        = 0;                             // Ma Shift
input ENUM_MA_METHOD     maMethod_slow       = MODE_EMA;                      // Method
input ENUM_APPLIED_PRICE maAppliedPrice_slow = PRICE_CLOSE;                   // Applied Price
...
class BUYcondition2 : public iConditions
{
 public:
  bool evaluate()
  {
    ...
    if(mafast.index(1) > maslow.index(1)) return true;
    return false;
  }
};
...
int OnInit()
{
  ...
  //--- buys:
    ...
    if(filterByEmaOn)conditionsToBuy.AddCondition(buyCondition2 = new BUYcondition2());
  ...
  if(filterByEmaOn)conditionsToSell.AddCondition(sellCondition2 = new SELLcondition2());
...
//--- SESSIONS CONTROL:
  ...
  mafast.setSetup(maPeriod, maShift, maMethod, maAppliedPrice);
  maslow = new MovingAverage();
  maslow.setSetup(maPeriod_slow, maShift_slow, maMethod_slow, maAppliedPrice_slow);
