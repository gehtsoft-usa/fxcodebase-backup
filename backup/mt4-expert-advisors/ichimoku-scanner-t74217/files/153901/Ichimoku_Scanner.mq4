//Available @  https://fxcodebase.com/code/viewtopic.php?f=38&t=74217
...
int OnInit()
{
    ...
    if(USE_M1)  addTF(PERIOD_M1);
    if(USE_M5)  addTF(PERIOD_M5);
    if(USE_M15) addTF(PERIOD_M15);
    if(USE_M30) addTF(PERIOD_M30);
    if(USE_H1)  addTF(PERIOD_H1);
    if(USE_H4)  addTF(PERIOD_H4);
    if(USE_D1)  addTF(PERIOD_D1);
    if(USE_W1)  addTF(PERIOD_W1);
                ...
                const int& spread [])
{
    for(int i = 0;i < ArraySize(ichimokus); i++)
    {
        ...
        for(int j = 0; j < ArraySize(TF); j++)
        {
            ...
            if(ask > tenkan &&
                tenkan > kinjun &&
                tenkan > prev_tenkan && kinjun > prev_kinjun)
            {
                trend = 0;
            }
