...
input int    Tenkan                     = 5;              // Tenkan Period
input int    Kijun                      = 15;             // Kijun Period
input int    Senkou                     = 52;             // Senkou Period
input int direction_shift = 0; // Direction shift
input int quality_shift = 0; // Quality shift
input bool   alertsOn                   = false;           // Alerts On?
input bool   alertsOnKumoCloudCross     = false;          // Alerts on leaving Kumo cloud?
input bool   alertsOnTenkanKijunCross   = true;           // Alerts on tenkan and kijun cross?
input bool   alertsOnPriceKijunCross    = false;          // Alerts on price crossing kijun?
input bool   alertsOnChinkouTenkanCross = false;          // Alerts on Chinkou and Tenkan cross?
input bool   alertsOnSpanASpanBCross    = true;           // Alerts on SpanA and SpanB cross?
...
input bool   alertsNotify               = false;          // Alerts Send Notification?
input bool   alertsEmail                = false;          // Alerts send email?
input int    Shift                      = 26;             // Shift values should = Kijun 
input bool   Interpolate                = true;           // Interpolate values if multi timeframe is used

input bool   Draw_Tenkan                = true;           // Draw Tenkan option
input bool   Draw_Kijun                 = true;           // Draw Kijun option
input bool   Draw_Kumo                  = true;           // Draw Kumo cloud option
input bool   Draw_Chikou                = true;           // Draw Chinkou option
...
input color direction_button_color = Orange; // Direction button color
input color quality_button_color = Purple; // Quality button color
input int button_x = 20;
input int button_y = 30;
input int magic_number = 42;
...
public:
   ...
   {
      recalc = false;
      visibilityId = indicatorName + "_visibility" + IntegerToString(magic_number);
      double val;
...
int init()
{
   IndicatorObjPrefix = GenerateIndicatorPrefix("imtfa");
   IndicatorShortName(timeFrameToString(tf) + " Ichimoku");

   ich.Init(IndicatorObjPrefix + "show_hide_ich", "ich1", "Ichimoku", button_x, button_y, ichimoku_button_color);
   kijun.Init(IndicatorObjPrefix + "show_hide_kijun", "ich2", "Kijun", button_x, button_y + 70, kijun_button_color);
   tenkan.Init(IndicatorObjPrefix + "show_hide_tenkan", "ich3", "Tenkan", button_x, button_y + 70 * 2, tenkan_button_color);
   chikou.Init(IndicatorObjPrefix + "show_hide_chikou", "ich4", "Chikou", button_x, button_y + 70 * 4, chikou_button_color);
   kumo.Init(IndicatorObjPrefix + "show_hide_kumo", "ich5", "Kumo", button_x, button_y + 70 * 3, kumo_button_color);
   directionButton.Init(IndicatorObjPrefix + "show_hide_dir", "ich6", "Direction", button_x, button_y + 70 * 5, direction_button_color);
   qualityButton.Init(IndicatorObjPrefix + "show_hide_qual", "ich7", "Quality", button_x, button_y + 70 * 6, quality_button_color);
   IndicatorBuffers(22);
   ...
   SetIndexLabel(7, "Direction");
   SetIndexStyle(8, DRAW_LINE);
   SetIndexBuffer(8, quality);
   SetIndexLabel(8, "Quality");   

   SetIndexBuffer(20, trend1); 
   SetIndexBuffer(21, trend2); 
   ...
   tf         = fmax(TimeFrame,_Period); 
   SetIndexShift(2,Shift  * tf/_Period); 
   SetIndexShift(3,Shift  * tf/_Period);
   SetIndexShift(4,-Shift * tf/_Period);
   SetIndexShift(5,Shift  * tf/_Period);
   SetIndexShift(6,Shift  * tf/_Period);
   SetIndexShift(7, (Shift + direction_shift)  * tf / _Period);
   SetIndexShift(8, (Shift + quality_shift) * tf / _Period);
...
int deinit()
{
   ...
   kijun.DeInit();
   kumo.DeInit();
   directionButton.DeInit();
   qualityButton.DeInit();
                  ...
                  const string &sparam)
{
   if (ich.HandleButtonClicks() || tenkan.HandleButtonClicks() || kijun.HandleButtonClicks() || chikou.HandleButtonClicks() || kumo.HandleButtonClicks()
      || directionButton.HandleButtonClicks() || qualityButton.HandleButtonClicks())
   {
      start();
   }
}
...
int start()
{
   ...
   directionButton.HandleButtonClicks();
   qualityButton.HandleButtonClicks();
   ...
   if (ich.IsRecalcNeeded() || tenkan.IsRecalcNeeded() || kijun.IsRecalcNeeded() || chikou.IsRecalcNeeded() || kumo.IsRecalcNeeded()
      || directionButton.IsRecalcNeeded() || qualityButton.IsRecalcNeeded())
   {
      counted_bars = 0;
   }
   ...
   chikou.ResetRecalc();
   kumo.ResetRecalc();
   directionButton.ResetRecalc();
   qualityButton.ResetRecalc();
   if (tf!=_Period)
   {
      limit = (int)fmax(limit,fmin(Bars-1,_mtfCall(12,0)*tf/_Period));
      for (i=limit;i>=0 && !_StopFlag; i--)
      {
         ...
         if (directionButton.IsVisible() && ich.IsVisible())
         {
            direction[i] = Kijun_Buffer2[i];
         }
         else
         {
            direction[i] = EMPTY_VALUE;
         }
         if (qualityButton.IsVisible() && ich.IsVisible())
         {
            quality[i] = Kijun_Buffer2[i];
         }
         else
         {
            quality[i] = EMPTY_VALUE;
         }
         ...
         int n,j; datetime time = iTime(NULL,tf,y);
         ...
         for(j = 1; j<n && (i+n)<Bars && (i+j)<Bars; j++) 
         {
            ...
            _interpolate(direction);
            _interpolate(quality);
         }
      }
      return(0);
   }
