...
int init()
{
   ich.Init("show_hide_ich", "ich", "Ichimoku", button_x, button_y);
   kijun.Init("show_hide_kijun", "ich", "Kijun", button_x, button_y + 70);
   tenkan.Init("show_hide_tenkan", "ich", "Tenkan", button_x, button_y + 70 * 2);
   chikou.Init("show_hide_chikou", "ich", "Chikou", button_x, button_y + 70 * 4);
   IndicatorBuffers(20);
...
void OnChartEvent(const int id,
                  ...
                  const string &sparam)
{
   if (ich.HandleButtonClicks() || tenkan.HandleButtonClicks() || kijun.HandleButtonClicks() || chikou.HandleButtonClicks())
   {
      start();
   }
}
...
int start()
{
   ...
   if (ich.IsRecalcNeeded() || tenkan.IsRecalcNeeded() || kijun.IsRecalcNeeded() || chikou.IsRecalcNeeded())
   {
      counted_bars = 0;
   }
