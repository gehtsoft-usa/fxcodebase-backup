...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=74901
...
input const string indicator_file = "Entry1.ex4";                // Indicator File:
...
class ConditionIndicatorBuy : public iConditions
{
  public:
    bool evaluate()
    {
        double indi = Indi(bufferToBuy, 1);
...
class ConditionIndicatorSell : public iConditions
{
  public:
    bool evaluate()
    {
        double indi = Indi(bufferToSell, 1);
        return indi > 0 && indi != EMPTY_VALUE;
    }
};
...
double TakeProfit(const double price, const string side, const string symbol, const double lot)
{
    ...
    case tp_by_indicator: return TakeProfitIndicator(price, side, symbol);
...
double TakeProfitIndicator(const double price, const string side, const string symbol)
{
    double tp_indi = 0;
    double indi    = Indi(buffertp, 1);
    if (indi > 0 && indi != EMPTY_VALUE)
        tp_indi = indi;

    return tpSafety(side, symbol, tp_indi);
}
...
input bool   partial1_on     = true;                  // Partial 1:
input double percentToClose1 = 50;                    // Percent to close
input bool   partial2_on     = true;                  // Partial 2:
input double percentToClose2 = 50;                    // Percent to close
...
class PartialClose : public iActions
{
  ...
  public:
    ...
    double indicator_tp(int partial)
    {
        // tomo el baffer del indicador segun el nro de partial
        int buff = 12;
        if (partial == 2)
            buff = 13;
    ...
    bool execute()
    {
        ...
        if (OrderType() == OP_BUY) {
            if (partial1_on)
                if (price() >= indicator_tp(1) && indicator_tp(1) != 0 && !CheckList(tk, 1))
                    if (OrderClose(tk, lots(1), price(), 10000, Green)) {
                        savetk(newtk(tk), 1);
                        return true;
                    }
            if (partial2_on)
                if (price() >= indicator_tp(2) && indicator_tp(2) != 0 && !CheckList(tk, 2))
                    if (OrderClose(tk, lots(2), price(), 10000, Black)) {
                        savetk(newtk(tk), 1);
                        savetk(newtk(tk), 2);
                        return true;
                    }
...
PartialClose actionPartialClose;

class ConditionPartial : public iConditions
{
  public:
    bool evaluate()
    {
...
ConditionPartial conditionPartial;

Strategy         sPartialClose;

void             oninit_Partials()
{
    actionPartialClose.setMagic(magico);
    sPartialClose.addCondition(&conditionPartial);
    sPartialClose.addAction(&actionPartialClose);

    Trader.management.add(&sPartialClose);
}
