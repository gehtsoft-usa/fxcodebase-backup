...
// http://fxcodebase.com/code/viewtopic.php?f=38&t=70277
...
input double delta = 10; // Zone Delta (in Pips)
...
void PlotLine(string name,double value,double line_color,double style)
{
   ...
   {
      if (!ObjectCreate(0, id, OBJ_RECTANGLE, 0, time1, value + delta * pipSize, time2, value - delta * pipSize))
      {
   ...
   ObjectSetDouble(0, id, OBJPROP_PRICE1, value + delta * pipSize);
   ObjectSetDouble(0, id, OBJPROP_PRICE2, value - delta * pipSize);
...
int init()
{
   ...
   EAName = IndicatorObjPrefix;
    ...
    if (TimePeriod=="M1" || TimePeriod=="1") period=PERIOD_M1;
    else if (TimePeriod=="M5" || TimePeriod=="5") period=PERIOD_M5;
    else if (TimePeriod=="M15" || TimePeriod=="15") period=PERIOD_M15;
    else if (TimePeriod=="M30" || TimePeriod=="30") period=PERIOD_M30;
    else if (TimePeriod=="H1" || TimePeriod=="60") period=PERIOD_H1;
    else if (TimePeriod=="H4" || TimePeriod=="240") period=PERIOD_H4;
    else if (TimePeriod=="D1" || TimePeriod=="1440") period=PERIOD_D1;
    else if (TimePeriod=="W1" || TimePeriod=="10080") period=PERIOD_W1; 
    else if (TimePeriod=="MN" || TimePeriod=="43200") period=PERIOD_MN1;
    else
    {
...
int start()
{
   ...
   switch (Mode)
   {
      case Today:
         ...
         time2 = time1 + (time1 - iTime(NULL,period,i+1));
         DrawToday(i);
         break;
      case History:
         for (i = limit; i >= 0; --i)
