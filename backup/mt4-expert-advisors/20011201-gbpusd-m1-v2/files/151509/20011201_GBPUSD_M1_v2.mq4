//Posted on https://fxcodebase.com/code/viewtopic.php?f=38&t=73774
//+------------------------------------------------------------------------------------------------+
...
double Maximum_Amount  = 0.01; // Maximum position amount [lot]
double Adding_Amount   = 0.0; // Amount to add on addition [lot]
double Reducing_Amount = 0.0; // Amount to close on reduction [lot]
input int Stop_Loss   = 0; // Stop Loss [point]
input int Take_Profit = 0; // Take Profit [point]
input int Break_Even  = 0; // Break Even [point]
double Martingale_Multiplier = 0; // Martingale Multiplier

static input string t_new = "Lot Size New Logic"; // ---------
input bool newLotSizeOn = true;// Use New Lot Size Logic:
input double uBaseLot = 0.01; // Base Lot
input double uMultiplier = 2; // Multiplier:
...
input int Slot1IndParam0 = 55; // Smoothing period
input int Slot1IndParam1 = 50; // Level
static input string IndicatorName2 = "Moving Averages Crossover"; // ------ Indicator parameters ------
input int Slot2IndParam0 = 20; // Fast MA period
input int Slot2IndParam1 = 15; // Slow MA period
...
input int Slot3IndParam0 = 4; // Slow MA period
...
static input int Expert_Magic = 25141829; // Expert Magic Number
...
// NOTE: New Lot size logic
// ------------------------------------------------------------------
#define __newLotSizeLogic
#ifdef __newLotSizeLogic
class Flag
{
  bool _state;
...
class CheckLastCloseTrade
{
    ...
    public:
    CheckLastCloseTrade(string symbol, int magic) :_symbol(symbol), _magic(magic) { ; }
    ~CheckLastCloseTrade() { ; }
...
class LotSizeLogic
{
    CheckLastCloseTrade* chek;
    Flag WasIncremented;
    double multiplier;
    double baseLot;

    public:
    LotSizeLogic(CheckLastCloseTrade* ch, double Multiplier, double BaseLot)
    {
        chek = ch;
    ...
    double LotSize()
    {
        if(chek.LastWasWin())
        {
            if(WasIncremented.isOff())
            {
                WasIncremented.On();
                return baseLot * multiplier;
            }
            else if(WasIncremented.isOn())
            {
                WasIncremented.Off();
                return baseLot;
            }
        } else if(chek.LastWasLoss())
        {
            WasIncremented.Off();
        }
        return baseLot;
...
CheckLastCloseTrade* lastTrade;
LotSizeLogic* lotLogic;
#endif
...
int OnInit()
{
    ...
    lastTrade = new CheckLastCloseTrade(_Symbol, Expert_Magic);
    lotLogic  = new LotSizeLogic(lastTrade, uMultiplier, uBaseLot);
