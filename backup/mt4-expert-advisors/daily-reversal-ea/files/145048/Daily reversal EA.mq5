...
//https://fxcodebase.com/code/viewtopic.php?f=38&t=71887
...
// ------------------------------------------------------------------
input string       Tsignal                 = "== Signal Setup ==";        // == Signal Setup ==
input int          uDaysBack                = 5;// Days Back:
...
input bool         closeAllInOpositeSignal = false;                      // CLose in Oposite Signal
input int          daysToClose             = 3;                          // Close in Days:
...
class BUYcondition1 : public iConditions
{
 public:
  bool evaluate()
  {
	...
	// place buy stop order at high of prev day candle if prev day close is the lowset close for the past X days
   
	// tomar  el close del dia previo
	// comparar con x días anteriores
	// si es inferior a todos, habilitar buy 
	
	double closeYesterday =iClose(Symbol(),PERIOD_D1,1);
	double closePrevDays[];
	ArrayResize(closePrevDays, uDaysBack);
	for (int i = 0; i < ArraySize(closePrevDays); i++) {
		  closePrevDays[i] = iClose(Symbol(),PERIOD_D1,i+2);
	  }
	double minClose = closePrevDays[ArrayMinimum(closePrevDays,0)];

	if(closeYesterday < minClose) return true;
...
class BUYcondition2 : public iConditions
{
 public:
  ...
  {
    // TODO: condition Buy2 
	 // precio Buy será el High del día previo
   if(Ask() >= iHigh(Symbol(),PERIOD_D1,1)) return true;
...
class SELLcondition1 : public iConditions
{
 public:
  bool evaluate()
  {
	...
	double maxClose = closePrevDays[ArrayMaximum(closePrevDays,0)];

	if(closeYesterday > maxClose) return true;
 ...
 public:
  bool evaluate()
  {
    // NOTE: condition sell 2
	 if(Bid() <= iLow(Symbol(),PERIOD_D1,1)) return true;
   ...
   switch (closeBy)
   {
      ...
      case CloseByAccountPercent: 
		...
		case CloseByDays:
		{
			...
			{
		    	ulong tk = PositionGetTicket(i);
				datetime timeToClose = PositionGetInteger(POSITION_TIME)+daysToClose*86400;

				Print(__FUNCTION__," ","timeToClose"," ",timeToClose);
				Print(__FUNCTION__," ","TimeCurrent"," ",TimeCurrent());
				if(TimeCurrent() >= timeToClose)
				{
	     			trade.PositionClose(tk, 1000);
				}
	    	}  
			break;
		}
