...
input bool   close_by_strategy_on = false;                   // Close positions After x candles?:
input int    nrCandlesToClose     = 15;                      // # Candles to close
  ...
  public:
    bool evaluate()
    {
        ...
        if (PositionGetInteger(POSITION_TYPE) == POSITION_TYPE_BUY) {
            ...
            // contar las velas desde la actual hasta la de entrada
            datetime openTm = (datetime)PositionGetInteger(POSITION_TIME);

            int c = 0;
            while (iTime(NULL, 0, c) > openTm) {
                c++;
            }

            if (c >= nrCandlesToClose) r = true;
        }
        return r;
    }
};
  ...
  public:
    bool evaluate()
    {
        ...
        if (PositionGetInteger(POSITION_TYPE) == POSITION_TYPE_SELL) {
            ...
            // contar las velas desde la actual hasta la de entrada
            datetime openTm = PositionGetInteger(POSITION_TIME);
...
input const string indicator_file = "KueFx_Arrows";              // Indicator File:
//---
int  bufferToBuy              = 2;
int  bufferToSell             = 3;
...
#define Section_TwoTimeFrames
#ifdef Section_TwoTimeFrames
// Mark: Section_TwoTimeFrames

input string          Ttwotf          = "==== 2 TimeFrames Trading ===="; // ————————————————————————
input bool            twotimeframesOn = false;                            // Two TF On ?
input bool closeAllWhenChengHtf = true; // Close all when change Higher TF signal?
...
input ENUM_TIMEFRAMES ltf    = PERIOD_M1;                        // Lower TimeFrame: (open trades)
int                  inp_fast_length              = 14;                // Length
double               inp_fast_atr_multiplier      = 1.0;               // ATR Multiplier
ENUM_APPLIED_PRICE   inp_fast_price               = PRICE_CLOSE;       // Applied Price
ENUM_YES_NO          inp_fast_change_calc         = No;                // Use volume data ?

int    handlehtf = 0;
void   setHandlehtf() { 
        handlehtf = iCustom(Symbol(), htf, indicator_file, inp_length, inp_atr_multiplier, inp_price, inp_change_calc); }
double Indihtf(int buffer, int candle = 1)
{
    double value[1];
    int    copy = CopyBuffer(handlehtf, buffer, candle, 1, value);
...
int    handleltf = 0; 
void   setHandleltf() { 
    handleltf = iCustom(Symbol(), ltf, indicator_file, inp_fast_length, inp_fast_atr_multiplier, inp_fast_price, inp_fast_change_calc); }
double Indiltf(int buffer, int candle = 1)
{
    double value[1];
    int    copy = CopyBuffer(handleltf, buffer, candle, 1, value);
...
string lastsignalhtf()
{
    
    double buysignal = EMPTY_VALUE;
    int    b         = 1;
    while (buysignal == EMPTY_VALUE) {
        buysignal = Indihtf(bufferToBuy, b);
        b++;
    }
    
    double sellsignal = EMPTY_VALUE;
    int    s          = 1;
    while (sellsignal == EMPTY_VALUE) {
        sellsignal = Indihtf(bufferToSell, s);
        s++;
    }
    
    string lastsignal ="";
    if (b < s) lastsignal = "buy";
    if (b > s) lastsignal = "sell";
    
    return lastsignal;
}

class ConditionTwoTimeFramesBuy : public iConditions
{
    double lastSignal;
    datetime nextTm;
  ...
  public:
    bool evaluate()
    {
        if (lastsignalhtf() == "buy") {
            if(iTime(Symbol(),ltf,0) > nextTm) 
            {
                nextTm = iTime(Symbol(),ltf,0);
                return true;
            }
        }
        return false;
...
ConditionTwoTimeFramesBuy cdTwoTimeFramesBuy;

class ActionTwoTimeFramesBuy : public iActions
{
  public:
    bool execute() { return true; }
};
ActionTwoTimeFramesBuy acTwoTimeFramesBuy;

class ConditionTwoTimeFramesSell : public iConditions
{
    double lastSignal;
    datetime nextTm;
...
ConditionTwoTimeFramesSell cdTwoTimeFramesSell;

class ActionTwoTimeFramesSell : public iActions
{
  public:
    bool execute() { return true; }
};
ActionTwoTimeFramesSell acTwotimeFramesSell;

Strategy stTwoTimeFramesBuy;
Strategy stTwoTimeFramesSell; 


class ConditionCloseWhenSignalChangeBuy : public iConditions
{
    string lastSignal;
    datetime nextTm;
  ...
  public:
    bool evaluate()
    {
        if(lastsignalhtf() == "buy") lastSignal = "buy";
        
        if(lastsignalhtf() == "sell" && lastSignal == "buy" ) { 
          lastSignal = "sell";
          return true;
        }
...
ConditionCloseWhenSignalChangeBuy cdCloseBuy_;

class ConditionCloseWhenSignalChangeSell : public iConditions
{
    string lastSignal;
    datetime nextTm;
  ...
  public:
    bool evaluate()
    {
        if(lastsignalhtf() == "sell") lastSignal = "sell";
                    
        if(lastsignalhtf() == "buy" && lastSignal == "sell") {
          lastSignal = "buy";
          return true;
        }
...
ConditionCloseWhenSignalChangeSell cdCloseSell_;


Strategy stCloseAllWhenSignalChangeBuys;
Strategy stCloseAllWhenSignalChangeSells;


void OnInit_TwoTimeFrames_Close()
{
    stCloseAllWhenSignalChangeBuys.set(&cdCloseBuy_, &acCloseAll);
    stCloseAllWhenSignalChangeSells.set(&cdCloseSell_, &acCloseAll);
    Trader.strategys.add(&stCloseAllWhenSignalChangeBuys);
    Trader.strategys.add(&stCloseAllWhenSignalChangeSells);
}


void OnInit_TwoTimeFrames()
{
    setHandlehtf();
    setHandleltf();
    
    
    stTwoTimeFramesBuy.set(&cdTwoTimeFramesBuy, &buy);
    stTwoTimeFramesSell.set(&cdTwoTimeFramesSell, &sell);

    stTwoTimeFramesBuy.addCondition(&cCountPositions);
    stTwoTimeFramesSell.addCondition(&cCountPositions);

    stTwoTimeFramesBuy.addCondition(&cCountBuys);
    stTwoTimeFramesSell.addCondition(&cCountSells);

    Trader.strategys.add(&stTwoTimeFramesBuy);
    Trader.strategys.add(&stTwoTimeFramesSell);    
...
ConditionIndicatorSell cdCloseOppositeBuy;
ConditionIndicatorBuy  cdCloseOppositeSell;
...
class ConditionCloseOnOppositeBuy : public iConditions
{
  public:
    ...
    {
        if(twotimeframesOn) {
            if(lastsignalhtf() == "sell") return true;
        }
...
class ConditionCloseOnOppositeSell : public iConditions
{
  public:
    ...
    {
        if(twotimeframesOn) {
            if(lastsignalhtf() == "buy") return true;
        }
...
{
    closeOppositeBuy.set(&cdCloseOppositeBuy, &acCloseBuy);
    closeOppositeSell.set(&cdCloseOppositeSell, &acCloseSell);
...
input int    count_max_positions = 10;                           // # Total positions:
input int    count_max_buys      = 5;                           // # Buys limit:
input int    count_max_sells     = 5;                           // # Sells limit:
...
int OnInit()
{
    ...
    OnInit_TwoTimeFrames();
    OnInit_TwoTimeFrames_Close();
