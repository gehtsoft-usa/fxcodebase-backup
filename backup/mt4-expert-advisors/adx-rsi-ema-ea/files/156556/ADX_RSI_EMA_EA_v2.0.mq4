...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=74824
...
input string             Tsignal              = "== Signal Setup ==";              // ————————————
...
input int                rsiPeriod            = 3;                                 // Period
input ENUM_APPLIED_PRICE rsiAppliedPrice      = PRICE_CLOSE;                       // Applied Price
input double             rsiLevelUp           = 80;                                // RSI Level Over Bougth
input double             rsiLevelDn           = 20;                                // RSI Level Over Sold
int                      rsiCandlesDivergence = 50;                                // RSI Candles Divergences
input string             tADX                 = "== ADX Setup ==";                 // == ADX Setup ==
input int                AdxPeriod            = 5;                                 // Period
input ENUM_APPLIED_PRICE AdxAppliedPrice      = PRICE_CLOSE;                       // Applied Price
input double             AdxLevelMain         = 30;                                // Minimum Main Level
double                   AdxLevelBuy          = 0;                                 // Minimum Level to Buy
double                   AdxLevelSell         = 0;                                 // Minimum Level to Sell
...
input double       userEquityPer = 0.5;             // Setup Lots by "Equity Percent":
...
input ModeTP   modeTP = tp_ratio;           // Mode Take Profit:
input int    userTPpips   = 20;                // Pips TP
input double uTpRatio = 1; // Ratio RR
...
input int    userSLpips   = 2;                 // Pips SL
...
class BUYcondition1 : public iConditions
{
  public:
    bool evaluate()
    {
        ...
        return iClose(NULL, 0, 1) > maSlow.index(1) && // cierre arriba de la 50
               rsi.index(2) < rsiLevelDn && // rsi vela anterior abajo de 20
               rsi.index(1) > rsiLevelDn && // rsi vela actual arriba de 20
            //    rsi.isOverSold(1)==true &&  
               adx.Main(1) > AdxLevelMain &&
...
class SELLcondition1 : public iConditions
{
  public:
    bool evaluate()
    {
        return iClose(NULL, 0, 1) < maSlow.index(1) && 
        // rsi.isOverBougth(1)==true && 
        rsi.index(2) > rsiLevelUp && // rsi vela anterior arriba de 80
        rsi.index(1) < rsiLevelUp && // rsi vela actual abajo de 80
        adx.Main(1) > AdxLevelMain &&
        iClose(NULL,0,1) < iOpen(NULL,0,1);
    }
};
...
{
    if (modeTP == tp_ratio) {
        // recorrer las ordenes de par y la qe no tenga tp,
        // lo calculas en funcion del sl y le ponés el tp
        for (int i = OrdersTotal() - 1; i >= 0; i--) {
            if (OrderSelect(i, SELECT_BY_POS) && OrderSymbol() == _Symbol && OrderMagicNumber() == magico) {

                if (OrderStopLoss() != 0 && OrderTakeProfit() == 0) {
                    double sl_distance = fabs(OrderOpenPrice() - OrderStopLoss());
                    double tp_distance = sl_distance * uTpRatio;
                    double tp_price = OrderType() == OP_BUY ? OrderOpenPrice() + tp_distance : OrderOpenPrice() - tp_distance;
                    OrderModify(OrderTicket(), OrderOpenPrice(), OrderStopLoss(), tp_price, 0, clrNONE);
                }
            }
        }
    }
