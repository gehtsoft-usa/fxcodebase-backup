...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=75579
...
input int                FastPeriod   =15;               // Fast MA Period
input int                MeanPeriod   =18;               // Mean MA Period
...
input int max_distance = 0; // Max distance between crosses
...
public:
   ...
   void Update(int period, datetime date)
   {
      ...
      _signals[period] = MeanBuffer[period];
   }
};
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      ...
      if (FastBuffer[pos] > MeanBuffer[pos] && FastBuffer[pos + 1] <= MeanBuffer[pos + 1])
      {
         for (int i = period; i <= period + max_distance; ++i)
         {
            if (SlowBuffer[i] < MeanBuffer[i] && SlowBuffer[i + 1] >= MeanBuffer[i + 1])
            {
               return true;
            }
         }
      }
      if (SlowBuffer[pos] < MeanBuffer[pos] && SlowBuffer[pos + 1] >= MeanBuffer[pos + 1])
      {
         for (int i = period; i <= period + max_distance; ++i)
         {
            if (FastBuffer[i] > MeanBuffer[i] && FastBuffer[i + 1] <= MeanBuffer[i + 1])
            {
               return true;
            }
         }
...
int OnInit()
{
   IndicatorObjPrefix = GenerateIndicatorPrefix("macross");
   IndicatorSetString(INDICATOR_SHORTNAME, "macross");
...
//--- indicator buffers mapping
   ...
   IndicatorSetString(INDICATOR_SHORTNAME,"MACROSS("+IntegerToString(FastPeriod)+", "+IntegerToString(MeanPeriod)+", "+IntegerToString(SlowPeriod)+")");
...
int OnCalculate(const int rates_total,       // size of input time series
                ...
                )
  {
   ...
   limit=rates_total-MathMax(max_distance + 1, prev_calculated);
   if(prev_calculated==0) limit-=(MathMax(FastPeriod,MathMax(MeanPeriod,SlowPeriod)) + max_distance);

   int first = 0;
   for (int pos = MathMax(first, prev_calculated - 1); pos < rates_total; ++pos)
   {
      int oldIndex = rates_total - 1 - pos;
      CalculateMACROSS(pos);
      for (int i = 0; i < ArraySize(alerts); ++i)
      {
...
//|                                                                  |
//+------------------------------------------------------------------+
void CalculateMACROSS(const int shift=0)
  {
//---
