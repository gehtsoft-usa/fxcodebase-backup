...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=73069
...
input int    uMinSize = 20;                   // Minimum Size Candle Pattern (points):
...
string             ma3title        = "== Moving Average 3 Setup ==";// ————————————
int                ma3Period       = 100;                            // Period
int                      ma3Shift        = 0;                             // Ma Shift
ENUM_TIMEFRAMES    ma3tf           = PERIOD_H1;                     // TimeFrame:
ENUM_MA_METHOD     ma3Method       = MODE_EMA;                      // Method
ENUM_APPLIED_PRICE ma3AppliedPrice = PRICE_CLOSE;                   // Applied Price
string             ma4title        = "== Moving Average 4 Setup ==";// ————————————
int                ma4Period       = 200;                            // Period
int                      ma4Shift        = 0;                             // Ma Shift
ENUM_TIMEFRAMES    ma4tf           = PERIOD_H1;                     // TimeFrame:
ENUM_MA_METHOD     ma4Method       = MODE_EMA;                      // Method
ENUM_APPLIED_PRICE ma4AppliedPrice = PRICE_CLOSE;                   // Applied Price
...
input int stochMinLevel = 30;// Stoch Buy level:
input int stochMaxLevel = 70;// Stoch Sell level:
// ------------------------------------------------------------------
...
class Stoch
{
  ...
  public:
   ...
   void setSetup(int set1, int set2, int set3, ENUM_MA_METHOD set4, ENUM_PRICE_FIELD set5)
   {
      _setup.setup1 = set1;
      _setup.setup2 = set2;
      _setup.setup3 = set3;
 ...
 public:
  bool evaluate()
  {
    // TODO: condition Buy 1
		return iClose(NULL, ma1tf, 1) > ma1.index(1) && iClose(NULL, ma2tf, 1) > ma2.index(1);
    // return false;
  }
};
 ...
 public:
  bool evaluate()
  {
    // TODO: condition Buy 2    
		return stoch.main(1) > stoch.signal(1) && stoch.main(2) < stoch.signal(2) && stoch.main(2) < stochMinLevel && stoch.signal(2) < stochMinLevel;
    // return false;
  }
};
...
class SELLcondition2 : public iConditions
{
 public:
  bool evaluate()
  {
    ...
    return stoch.main(1) < stoch.signal(1) && stoch.main(2) > stoch.signal(2) && stoch.main(2) > stochMaxLevel && stoch.signal(2) > stochMaxLevel;
    // return false;
  }
};
...
//--- SESSIONS CONTROL:
  ...
  ma1 = new MovingAverage(_Symbol, ma1tf); ma1.setSetup(ma1Period, ma1Shift, ma1Method, ma1AppliedPrice);
  ma2 = new MovingAverage(_Symbol, ma2tf); ma2.setSetup(ma2Period, ma2Shift, ma2Method, ma2AppliedPrice);
  ma3 = new MovingAverage(_Symbol, ma3tf); ma3.setSetup(ma3Period, ma3Shift, ma3Method, ma3AppliedPrice);
  ma4 = new MovingAverage(_Symbol, ma4tf); ma4.setSetup(ma4Period, ma4Shift, ma4Method, ma4AppliedPrice);
