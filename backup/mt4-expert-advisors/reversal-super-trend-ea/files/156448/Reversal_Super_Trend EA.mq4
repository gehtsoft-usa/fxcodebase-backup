...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=75145
...
input const string indicator_file = "! SuperTrend  adaptive avgs filtered (mtf + alerts + arrows + btn)"; // Indicator File:

enum enum_signal_mode { normal, reversal };
input enum_signal_mode signal_mode = normal; // Signal Mode:
...
{
    if (!close_on_opposite_on) return;
    ...
    if(signal_mode == reversal){
        stCloseOppositeBuy.set(&cdCloseOnOppositeSell, &acCloseBuy);
        stCloseOppositeSell.set(&cdCloseOnOppositeBuy, &acCloseSell);
    }
