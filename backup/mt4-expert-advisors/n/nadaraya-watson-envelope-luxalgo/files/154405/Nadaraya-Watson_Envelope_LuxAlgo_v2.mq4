//Available @  https://fxcodebase.com/code/viewtopic.php?f=38&t=74624
...
int init()
{
   ...
   SetIndexStyle(id++, DRAW_ARROW, STYLE_SOLID, 1, (color)(EMPTY_VALUE));
   ...
   for (int pos = MathMin(bars_limit, rates_total - 1 - MathMax(prev_calculated - 1, toSkip)); pos >= 0 && !IsStopped(); --pos)
   {
      ...
      if ((pos == 0) && repaint)
      {
         ...
         for (int i = for6_from; i <= for6_to; i += 1)
         {
            if (i%2)
            {
               LinesCollection::Create(IndicatorObjPrefix + "line_2_id", n - i + 1, SafePlus(y1, sae), n - i, SafePlus(Array::Get(nwe, i), sae), time[pos]).SetColor(upCss).SetWidth(1);
               LinesCollection::Create(IndicatorObjPrefix + "line_3_id", n - i + 1, SafeMinus(y1, sae), n - i, SafeMinus(Array::Get(nwe, i), sae), time[pos]).SetColor(dnCss).SetWidth(1);
            }
            double srcValue_i;
            if (!src.GetValue(pos + i, srcValue_i)) { srcValue_i = EMPTY_VALUE; }
            double srcValue__1;
            if (!src.GetValue(pos + i + 1, srcValue__1)) { srcValue__1 = EMPTY_VALUE; }
            if (SafeGreater(srcValue_i, SafePlus(Array::Get(nwe, i), sae)) && SafeLess(srcValue__1, SafePlus(Array::Get(nwe, i), sae)))
            {
               LabelsCollection::Create(IndicatorObjPrefix + "label_1_id", n - i, srcValue_i, time[pos]).SetColor((color)(EMPTY_VALUE)).SetText("▼").SetTextColor(dnCss).SetStyle("down").SetSize("normal").SetYLoc("price");
            }
            double srcValue__2;
            if (!src.GetValue(pos + i + 1, srcValue__2)) { srcValue__2 = EMPTY_VALUE; }
            if (SafeLess(srcValue_i, SafeMinus(Array::Get(nwe, i), sae)) && SafeGreater(srcValue__2, SafeMinus(Array::Get(nwe, i), sae)))
            {
               LabelsCollection::Create(IndicatorObjPrefix + "label_2_id", n - i, srcValue_i, time[pos]).SetColor((color)(EMPTY_VALUE)).SetText("▲").SetTextColor(upCss).SetStyle("up").SetSize("normal").SetYLoc("price");
            }
            y1 = Array::Get(nwe, i);
         }
      }
