...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=74428
...
#property description "Be sure to have this indicators in the folder \"MQL4/Indicators\":" 
#property description "> BrainTrend2SigALERT5.ex4"
#property description "> JokerFilter.ex4"
#property strict
// NOTE: defines
// ——————————————————————————————————————————————————————————————————
...
string signal_indicator_file = "BrainTrend2SigALERT5.ex4";
int bufferToBuy = 0;
int bufferToSell = 1;
double SignalIndicator(int buffer, int candle)
{
    return iCustom(NULL, 0, signal_indicator_file, buffer, candle);
}


input string Tindi = "== JokerFilter Setup ==";  // ————————————
string filter_indicator_file = "JokerFilter.ex4";
input ENUM_TIMEFRAMES tf = PERIOD_CURRENT; // TimeFrame
input int SmoothPeriod = 5;// SmoothPeriod
input int SmoothPhase = 0;// SmoothPhase

double FilterIndicator(int buffer, int candle)
{
    return iCustom(NULL, 0, filter_indicator_file, tf, SmoothPeriod, SmoothPhase,
    buffer, candle);
}
...
class BUYcondition1 : public iConditions
{
 public:
  bool evaluate()
  {
      return SignalIndicator(bufferToBuy, 1) != 0;
  }
};
...
class BUYcondition2 : public iConditions
{
 public:
  bool evaluate()
  {
      return FilterIndicator(bufferToBuy, 1) == 1;
  }
};
...
class SELLcondition1 : public iConditions
{
 public:
  bool evaluate()
  {
    return SignalIndicator(bufferToSell, 1) != 0;
  }
};
...
class SELLcondition2 : public iConditions
{
 public:
  bool evaluate()
  {
    return FilterIndicator(bufferToSell, 1) == 1;
  }
};
...
#ifdef CONTROL_CUSTOM_INDICATOR_FILE
  double temp = iCustom(NULL, 0, signal_indicator_file, 0, 0);
	if (GetLastError() == ERR_INDICATOR_CANNOT_LOAD)
  {
        string txt = "THIS EA NEED AN INDICATOR\ninstall the file:\n" + signal_indicator_file + "\ninto the folder:\nMQL4/Indicators.";
