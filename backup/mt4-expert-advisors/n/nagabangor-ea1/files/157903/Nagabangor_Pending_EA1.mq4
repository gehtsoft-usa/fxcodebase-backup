//Available @ https://fxcodebase.com/code/viewtopic.php?f=38&p=148067#p148067
...
input int ma2_period = 8; // MA 2 period
input ENUM_MA_METHOD ma2_method = MODE_SMA; // Smoothing method
input int ma3_period = 34; // MA 3 period
...
input double entry_shift = 10; // Entry shift in pips
...
public:
   ...
   public:
      ...
      bool GetValue(const int period, double &val)
      {
         ...
         val = iHigh(_symbol, _timeframe, period) + pipSize * entry_shift;
         return true;
      }
   };
   ...
   public:
      ...
      bool GetValue(const int period, double &val)
      {
         ...
         int mult = digits == 3 || digits == 5 ? 10 : 1;
         double pipSize = point * mult;
         val = iLow(_symbol, _timeframe, period) + pipSize * entry_shift;
         return true;
      }
   };
#endif
...
ICondition* CreateLongCondition(string symbol, ENUM_TIMEFRAMES timeframe, EntryStreamData* data)
{
   ...
   condition.Add(new MAMACondition(symbol, timeframe, FirstAboveSecond, ma2_method, ma2_period, PRICE_CLOSE,
      ma3_method, ma3_period, PRICE_CLOSE), false);
   condition.Add(new MAMACondition(symbol, timeframe, FirstAboveSecond, ma1_method, ma1_period, PRICE_CLOSE,
      ma2_method, ma2_period, PRICE_CLOSE), false);
...
ICondition* CreateShortCondition(string symbol, ENUM_TIMEFRAMES timeframe, EntryStreamData* data)
{
   ...
   AndCondition* condition = new AndCondition();
   condition.Add(new MAMACondition(symbol, timeframe, FirstBelowSecond, ma2_method, ma2_period, PRICE_CLOSE,
      ma3_method, ma3_period, PRICE_CLOSE), false);
   condition.Add(new MAMACondition(symbol, timeframe, FirstBelowSecond, ma1_method, ma1_period, PRICE_CLOSE,
      ma2_method, ma2_period, PRICE_CLOSE), false);
   #ifdef ACT_ON_SWITCH_CONDITION
      ActOnSwitchCondition* switchCondition = new ActOnSwitchCondition(symbol, timeframe, (ICondition*) condition);
      condition.Release();
