...
//https://fxcodebase.com/code/viewtopic.php?f=38&t=75031
...
class LotCalculator
{
  ...
  public:
    ...
    {
        double lot = 1;
        double marginConsumido =
            AccountFreeMargin() - AccountFreeMarginCheck(Symbol(), OP_BUY, lot);
...
double StopLoss(const double price, const string side, const string symbol,
                const double lot)
{
...
double StopLossByPips(const double price, const string side,
                      const string symbol)
{
...
double StopLossByMoney(const double price, const string side,
...
double StopLossByCandles(const string side, const string symbol,
                         ENUM_TIMEFRAMES tf = 0)
{
    double r = 0;
    if (side == "buy") {
        r = iLow(symbol, tf,
                 iLowest(symbol, tf, MODE_LOW, sl_candles_shift, 1));
        r -= sl_pips_from_candle * 10 * MarketInfo(symbol, MODE_POINT);
        r = StopLossSafety("buy", symbol, r);
    }
    if (side == "sell") {
        r = iHigh(symbol, tf,
                  iHighest(symbol, tf, MODE_HIGH, sl_candles_shift, 1));
        r += sl_pips_from_candle * 10 * MarketInfo(symbol, MODE_POINT);
...
double TakeProfit(const double price, const string side, const string symbol,
                  const double lot)
{
...
double TakeProfitByPips(const double price, const string side,
                        const string symbol)
{
    double mPoint   = MarketInfo(symbol, MODE_POINT);
    double digits   = MarketInfo(symbol, MODE_DIGITS);
...
double TakeProfitByMoney(const double price, const string side,
                         const string symbol, const double lot)
{
    double _tickValue    = MarketInfo(symbol, MODE_TICKVALUE);
    double _modeCalc     = MarketInfo(symbol, MODE_PROFITCALCMODE);
...
double TakeProfitByRatio(const double price, const string side,
                         const string symbol)
{
    ...
    bool evaluate()
    {
        ...
        if (OrderType() == OP_BUY) {
            return (((OrderStopLoss() < OrderOpenPrice()) ||
                     (OrderStopLoss() == 0)) &&
                    (bid >= OrderOpenPrice() + dist));
        }
        if (OrderType() == OP_SELL) {
            return (((OrderStopLoss() > OrderOpenPrice()) ||
                     (OrderStopLoss() == 0)) &&
                    (ask <= OrderOpenPrice() - dist));
        }
        return false;
    }
};
    ...
    bool execute()
    {
        ...
        if (OrderType() == OP_BUY) {
            double buySl = NormalizeDouble(
                OrderOpenPrice() + userBkvStep * 10 * Point, digi);
            r = OrderModify(OrderTicket(), OrderOpenPrice(), buySl,
                            OrderTakeProfit(), OrderExpiration(), clrNONE);
        }
        if (OrderType() == OP_SELL) {
            double sellSl = NormalizeDouble(
                OrderOpenPrice() - userBkvStep * 10 * Point, digi);
            r = OrderModify(OrderTicket(), OrderOpenPrice(), sellSl,
                            OrderTakeProfit(), OrderExpiration(), clrNONE);
        }
        return r;
    }
  ...
  public:
    bool execute()
    {
        ...
        if (OrderType() == OP_BUY) {
            ...
            allowed = bid - allowed;
            newsl =
                MathMin(NormalizeDouble(last_step - distance, digi), allowed);
            if (currentsl < newsl || currentsl == 0) {
                return OrderModify(OrderTicket(), OrderOpenPrice(), newsl,
        ...
        if (OrderType() == OP_SELL) {
            ...
            allowed = ask + allowed;
            newsl =
                MathMax(NormalizeDouble(last_step + distance, digi), allowed);
            if (currentsl > newsl || currentsl == 0) {
                return OrderModify(OrderTicket(), OrderOpenPrice(), newsl,
                                   OrderTakeProfit(), OrderExpiration(),
                                   clrNONE);
...
class ScheduleController : public iConditions
{
  ...
  public:
    ...
    {
        _currentDay = TimeDay(
            TimeGMT() + (int)_timeZone); // return the day of the month 1-31
    }
    ...
    bool evaluate() // control day and hours for every session
    {
        ...
        int actual = (TimeHour(TimeGMT() + (int)_timeZone) * 3600) +
                     (TimeMinute(TimeGMT() + (int)_timeZone) * 60);
        ...
        for (int i = 0; i < qnt(); i++) {
            ...
            if (schedules[i].dayNumber() != 0) {
                if (schedules[i].dayNumber() ==
                    TimeDayOfWeek(TimeGMT() + (int)_timeZone)) {
                    if ((actual >= schedules[i].iniTime()) &&
                        actual <= schedules[i].endTime()) {
                        setActualSession(i);
...
input string note =
    "http://calendar.fxstreet.com/";           // You Must to allow this URL:
  ...
  public:
    ...
    int OnInit()
    {
        if (!MQLInfoInteger(MQL_TESTER) || !MQLInfoInteger(MQL_OPTIMIZATION)) {
            if (NEWS_FILTER == true && READ_NEWS(NEWS_TABLE) &&
                ArraySize(NEWS_TABLE) > 0)
    ...
    void ReadNews()
    {
        ...
        for (int i = 0; i < ArraySize(NEWS_TABLE); i++) {
            ...
            if (Importance_Check ||
                StringFind(Currencies_Check, NEWS_TABLE[i].currency, 0) == -1 ||
                (Check_Specific_News &&
                 (StringFind(NEWS_TABLE[i].news, Specific_News_Text) == -1)))
                continue;
            if ((news_time <= time &&
                 (news_time + (datetime)(START_AFTER_NEWS * 60)) >= time) ||
                (news_time >= time &&
                 (news_time - (datetime)(STOP_BEFORE_NEWS * 60)) <= time)) {
                NEWS_ON = 1;
                Comment("News Time...");
                break;
            } else {
    ...
    bool READ_NEWS(sNews &l_NewsTable[])
    {
        ...
        char   post[], result[];
        string tmpStr   = "";
        string st_date  = TimeToString(TimeCurrent(), TIME_DATE),
               end_date = TimeToString(
                   (TimeCurrent() + (datetime)(7 * 24 * 60 * 60)), TIME_DATE);
        StringReplace(st_date, ".", "");
        StringReplace(end_date, ".", "");
        string url =
            "http://calendar.fxstreet.com/EventDateWidget/GetMini?culture=" +
            LANG + "&view=range&start=" + st_date + "&end=" + end_date +
            "&timezone=UTC" +
            "&columns=date%2Ctime%2Ccountry%2Ccountrycurrency%2Cevent%"
            "2Cconsensus%2Cprevious%2Cvolatility%2Cactual&showcountryname="
            "false&showcurrencyname=true&isfree=true&_=1455009216444";
        ResetLastError();
        WebRequest("GET", url, cookie, referer, 10000, post, sizeof(post),
                   result, headers);
        if (ArraySize(result) <= 0) {
            ...
            if (er == 4060)
                MessageBox(
                    "YOU MUST ADD THE ADDRESS '" +
                        "http://calendar.fxstreet.com/" +
                        "' IN THE LIST OF ALLOWED URL IN THE TAB 'ADVISERS'",
        ...
        HEADS.importance =
            (st < end ? StringSubstr(tmpStr, st, fmin(end - st, 8)) : "");
        ...
        do {
            ...
            if (begin >= 0) {
                ...
                tmpStr =
                    StringSubstr(tmpStr, 0, begin) + StringSubstr(tmpStr, end);
            }
        } while (begin >= 0);
        ...
        for (uchar tr = 1; tr < 255; tr++) {
            ...
            for (int td = 0; td < ArraySize(arNews); td++) {
                ...
                if (st > 0) {
                    ...
                    int d = (int)StringToInteger(
                        StringSubstr(arNews[td], end - 4, end - st));
                    MqlDateTime time;
                    TimeCurrent(time);
                    if (d < (time.day - 5)) {
                        if (time.mon == 12) {
                ...
                l_NewsTable[td - BackShift].time =
                    StringSubstr(arNews[td], st1, end - st1);
                if (StringFind(l_NewsTable[td - BackShift].time, ":") > 0) {
                    l_NewsTable[td - BackShift].dTime =
                        StringToTime(TimeToString(date, TIME_DATE) + " " +
                                     StringSubstr(arNews[td], st1, end - st1));
                } else {
                ...
                l_NewsTable[td - BackShift].currency =
                    (st1 < end ? StringSubstr(arNews[td], st1, end - st1) : "");
                st1 = StringFind(arNews[td], "fxst-i-vol", st);
                st1 = StringFind(arNews[td], ">", st1) + 1;
                end = StringFind(arNews[td], "</td>", st1);
                StringInit(l_NewsTable[td - BackShift].importance,
                           (int)StringToInteger(
                               StringSubstr(arNews[td], st1, end - st1)),
                           '*');
                ...
                l_NewsTable[td - BackShift].news = StringSubstr(
                    arNews[td], st1, (end1 > 0 ? fmin(end, end1) : end) - st1);
                st1 = StringFind(arNews[td], "fxst-td-act", st);
                st1 = StringFind(arNews[td], ">", st1) + 1;
                end = StringFind(arNews[td], "</td>", st1);
                l_NewsTable[td - BackShift].Actual =
                    (end > st1 ? StringSubstr(arNews[td], st1, end - st1) : "");
                st1 = StringFind(arNews[td], "fxst-td-cons", st);
                st1 = StringFind(arNews[td], ">", st1) + 1;
                end = StringFind(arNews[td], "</td>", st1);
                l_NewsTable[td - BackShift].forecast =
                    (end > st1 ? StringSubstr(arNews[td], st1, end - st1) : "");
                st1 = StringFind(arNews[td], "fxst-td-prev", st);
                st1 = StringFind(arNews[td], ">", st1) + 1;
                end = StringFind(arNews[td], "</td>", st1);
                l_NewsTable[td - BackShift].previus =
                    (end > st1 ? StringSubstr(arNews[td], st1, end - st1) : "");
            }
            break;
        }
    ...
    void DRAW_NEWS(sNews &l_a_news[])
    {
        if (DRAW_NEWS_LINES || DRAW_NEWS_CHART) {
            ...
            for (int i = ArraySize(l_a_news) - 1; i >= 0; i--) {
                ...
                datetime t1 =
                    (l_a_news[i].dTime + (datetime)(START_AFTER_NEWS * 60));
                datetime t2 = ((TimeCurrent() - (datetime)TIME_CORRECTION));

                if (StringFind(Currencies_Check, l_a_news[i].currency) == -1 ||
                    t1 < t2 ||
                    (Check_Specific_News &&
                     (StringFind(l_a_news[i].news, Specific_News_Text) ==
                      -1))) {
                    DEL_ROW(l_a_news, i);
                    continue;
                }
                ...
                if ((!NEWS_IMPOTANCE_LOW && l_a_news[i].importance == "*") ||
                    (!NEWS_IMPOTANCE_MEDIUM &&
                     l_a_news[i].importance == "* *") ||
                    (!NEWS_IMPOTANCE_HIGH &&
                     l_a_news[i].importance == "* * *")) {
                    DEL_ROW(l_a_news, i);
                    continue;
                }
                string NAME = (" " + l_a_news[i].currency + " " +
                               l_a_news[i].importance + " " + l_a_news[i].news);
                if (DRAW_NEWS_LINES) {
                    if (ObjectFind(0, NAME) < 0) {
                        ObjectCreate(0, NAME, OBJ_VLINE, 0,
                                     l_a_news[i].dTime + TIME_CORRECTION, 0);
            ...
            if (DRAW_NEWS_CHART) {
                for (int l = 1; l <= 9 && Z < ArraySize(l_a_news); l++) {
                    ...
                    if (ObjectFind(0, NAME) < 0)
                        OBJECT_LABEL(
                            0, NAME, 0, X + 110, Y - (int)(18 * (l + 5)),
                            CORNER_LEFT_LOWER,
                            ((TimeToString(l_a_news[K].dTime + TIME_CORRECTION,
                                           TIME_DATE | TIME_MINUTES) +
                              " " + l_a_news[K].currency + " " +
                              l_a_news[K].importance + " " + l_a_news[K].news)),
                            News_Font, Font_Size, Font_Color, 0,
                            ANCHOR_LEFT_UPPER, false, false, true, 0);
                }
            }
            return;
        }
    ...
    bool OBJECT_LABEL(const long CHART_ID = 0, const string NAME = "",
                      const int SUB_WINDOW = 0, const int X_Axis = 0,
                      const int              Y_Axis = 0,
                      const ENUM_BASE_CORNER CORNER = CORNER_LEFT_UPPER,
                      const string TEXT = "", const string FONT = "",
                      const int               FONT_SIZE = 10,
                      const color             CLR       = color("255,0,0"),
                      const double            ANGLE     = 0.0,
                      const ENUM_ANCHOR_POINT ANCHOR    = ANCHOR_LEFT_UPPER,
                      const bool BACK = false, const bool SELECTION = false,
                      const bool HIDDEN = true, const long ZORDER = 0,
                      string TOOLTIP = "\n")
    {
        ResetLastError();
        if (ObjectFind(0, NAME) < 0) {
...
class OrderCreator : public iActions
{
  ...
  public:
    ...
    bool execute()
    {
        ...
        int result = OrderSend(_symbol, _cmd, _lots, _price, _slippage, _sl,
                               _tp, "", _magic, 0, clrNONE);
...
input int toleranceCandlesBack = 2; // Tolerance Candles back Signal:
// input const string indicator_file = "Noble Impulse V4 Licensed_fix_fix.ex4"; // Indicator File:
//---
int  bufferToBuy    = 0;
int  bufferToSell   = 1;
...
// . detectar ultima arrow buy
// . detectar ultima arrow sell
// . el que esté ubicado en la última vela
// . dar señal 
  ...
  public:
    bool evaluate()
    {
        ...
        for(int i=ObjectsTotal() - 1;i >= 0 ;i--)
        {
            ...
            Print("line: ",__LINE__," name: ",name);
                if(StringFind(name, buytext) >= 0){
                arrowTm = (datetime)ObjectGetInteger(0, name, OBJPROP_TIME);    
                arrowPrice= ObjectGetDouble(0, name, OBJPROP_PRICE);    
                break;
                }
        }
        
        Print("line: ",__LINE__," arrowTm: ",arrowTm);
        Print("line: ",__LINE__," arrowPrice: ",arrowPrice);
        
        if (arrowTm >= iTime(NULL, 0, toleranceCandlesBack) && arrowPrice != lastSignal)
        {
            lastSignal = arrowPrice;
            return true;
        }
...
bool OnInit_CustomIndicator()
{
    ...
    //                    indicator_file + "\ninto the folder:\nMQL4/Indicators.",
    //                "Important Information", MB_ICONINFORMATION);
    //     Alert("THIS EA NEED AN INDICATOR, install the file: " + indicator_file +
    //           " into the folder: MQL4/Indicators.");
...
class ConditionCountBuys : public iConditions
{
  public:
    bool evaluate()
    {
        ...
        for (int i = OrdersTotal() - 1; i >= 0; i--) {
            if (OrderSelect(i, SELECT_BY_POS) && OrderSymbol() == _Symbol &&
                OrderMagicNumber() == magico && OrderType() == OP_BUY) {
                count += 1;
            }
        }
...
class ConditionCountSells : public iConditions
{
  public:
    bool evaluate()
    {
        ...
        for (int i = OrdersTotal() - 1; i >= 0; i--) {
            if (OrderSelect(i, SELECT_BY_POS) && OrderSymbol() == _Symbol &&
                OrderMagicNumber() == magico && OrderType() == OP_SELL) {
                count += 1;
            }
        }
