...
input string Tindicator_nw = "== Indicator Setup ==";  // ————————————
input int     Length=500;              // Bars Count
input int     Bandwidth=17;                // Bandwidth
input double  Multiplayer=1.5;

string nadaraya_file = "Nadaraya Watson Envelope";
int    nadaraya_handle                = 0;

void setHandle()
{
  nadaraya_handle = iCustom(Symbol(), NULL, nadaraya_file, Length, Bandwidth, Multiplayer);
}

double Nadaraya(int buffer, int shift)
{
  double value[1];
  int    copy = CopyBuffer(nadaraya_handle, buffer, shift, 1, value);
...
input string Tindicator_dc = "== Donchian Channel Setup ==";  // ————————————
input int    dc_Length          = 20;
input int    dc_Mode            = 1;     // 0 - Close 1 - High/Low
bool   dc_show_inter      = false;  // Show 25% and 75% levels?

string donchian_file       = "Donchian_Channel";
int    donchian_handle     = 0;

void setHandle_dc()
{
  donchian_handle = iCustom(Symbol(), NULL, donchian_file, dc_Length, dc_Mode, dc_show_inter);
}

double Donchian(int buffer, int shift)
{
  double value[1];
  int    copy = CopyBuffer(donchian_handle, buffer, shift, 1, value);
...
class BUYcondition2 : public iConditions
{
 public:
  ...
  {
    // TODO: condition Buy2
    return iClose(NULL,0,1) < Nadaraya(0, 1);
  }
};
...
class BUYcondition3 : public iConditions
{
 public:
  bool evaluate()
  {
    ...
    return iClose(NULL,0,1) < Donchian(4, 1);
  }
};
...
class SELLcondition2 : public iConditions
{
 public:
  bool evaluate()
  {
    ...
    return iClose(NULL,0, 1) > Nadaraya(1, 1);
  }
};
...
class SELLcondition3 : public iConditions
{
 public:
  bool evaluate()
  {
    ...
    return iClose(NULL,0,1) > Donchian(0, 1);
  }
};
...
int OnInit()
{
    ...
    // double temp = iCustom(NULL, 0, nadaraya_file);
    setHandle();
    if(GetLastError() == ERR_INDICATOR_CANNOT_CREATE) {
        Alert("Please, install the: " + nadaraya_file + " indicator");
        return INIT_FAILED;
    }
    
    // double temp2 = iCustom(NULL, 0, donchian_file);
    setHandle_dc();
    if(GetLastError() == ERR_INDICATOR_CANNOT_CREATE) {
        Alert("Please, install the: " + donchian_file + " indicator");
        return INIT_FAILED;
    }
...
double index(int nadaraya_handle, int buffer, int shift)
{
    double value[1];
    int    qnt = CopyBuffer(nadaraya_handle, buffer, shift, 1, value);
