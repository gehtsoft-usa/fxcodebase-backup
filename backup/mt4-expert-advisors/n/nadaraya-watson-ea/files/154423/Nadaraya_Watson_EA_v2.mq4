...
// ------------------------------------------------------------------
input string NadarayaSetup = "== Nadaraya Setups ==";   // == Nadaraya Setups ==
input int    length = 50;               // length:
input int    h              = 8;                // Bandwidth:
input int    mult           = 3;                // Multiplier:

string file_custom_indicator = "NadarayaWatsonEnvelopes";
int bufferToSell = 0;
int bufferToBuy  = 1;

double Nadaraya(int buffer, int candle)
{
    return iCustom(NULL, 0, file_custom_indicator, length, h, mult, buffer, candle);
}

// ------------------------------------------------------------------
input string IDonchian   = "== Donchian Setup ==";  // == Donchian Setup ==
input int donchian_periods = 3; // Periods:

double Donchian(int buffer, int candle)
{
    return iCustom(NULL, 0, "Donchian.ex4", donchian_periods, buffer, candle);
}
...
class BUYcondition2 : public iConditions
{
 public:
  bool evaluate()
  {
      ...
      Print("Nadaraya 1: ", Nadaraya(1, 1));
      if(iClose(NULL, 0, 1) < Nadaraya(1, 1)) return true;
      return false;
  }
};
...
class BUYcondition3 : public iConditions
{
 public:
  bool evaluate()
  {
     ...
     //   Donchian
      Print("Donchian 0: ", Donchian(0, 1));
      if(iHigh(NULL, 0, 1) >= Donchian(0, 1)) return true;
      return false;
  }
};
...
class SELLcondition2 : public iConditions
{
 public:
  bool evaluate()
  {
      ...
      Print("Nadaraya 0: ", Nadaraya(0, 1));
      if(iClose(NULL, 0, 1) > Nadaraya(0, 1))return true;
      return false;
  }
};
...
class SELLcondition3 : public iConditions
{
 public:
  bool evaluate()
  {
      ...
      //   Donchian
      Print("Donchian 1: ", Donchian(0, 1));
      if(iLow(NULL, 0, 1) <= Donchian(0, 1)) return true;
      return false;
  }
};
