...
int init()
  {
       double temp = iCustom(NULL, 0, "NHNL", 0, 0);
   if (GetLastError() == ERR_INDICATOR_CANNOT_LOAD)
   {
       Alert("Please, install the 'NHNL' indicator");
       return INIT_FAILED;
   }
   IndicatorName = GenerateIndicatorName("NHNL Divergence");
   IndicatorObjPrefix = "__" + IndicatorName + "__";
   IndicatorShortName(IndicatorName);
   IndicatorDigits(Digits);
   SetIndexStyle(0,DRAW_LINE);
   SetIndexBuffer(0,NHNL);
...
void DrawLine(int First, int Second, bool BullFl)
{
 ...
 if (ObjectFind(ObjName)==-1)
 {
  ...
  if (WindowNumber!=-1)
  {
   ObjectCreate(ObjName, OBJ_TREND, WindowNumber, Time[First], NHNL[First], Time[Second], NHNL[Second]);
   ObjectSet(ObjName, OBJPROP_RAY, false);
   ObjectCreate(ObjName+"A", OBJ_ARROW, WindowNumber, Time[Second], NHNL[Second]);
   if (BullFl)
   {
...
{
 int i;
 if (NHNL[bar]<0 && NHNL[bar]<NHNL[bar+1] && NHNL[bar]<NHNL[bar-1])
 {
...
int prevTrough(int bar)
{
 ...
 {
  if (NHNL[i]<=NHNL[i+1] && NHNL[i]<NHNL[i+2] && NHNL[i]<=NHNL[i-1] && NHNL[i]<NHNL[i-2])
  {
   return (i);
  }
 }
...
void processBullish(int bar)
{
 if (isTrough(bar))
 {
  ...
  if (prev!=EMPTY_VALUE)
  {
   if (NHNL[curr]>NHNL[prev] && Low[curr]<Low[prev])
   {
...
void processBearish(int bar)
{
 if (isPeak(bar))
 {
  ...
  if (prev!=EMPTY_VALUE)
  {
   if (NHNL[curr]<NHNL[prev] && High[curr]>High[prev])
   {
...
int start()
{
 ...
 while(pos>=0)
 {
  NHNL[pos]=iCustom(NULL, 0, "NHNL", Length, 2, pos);
