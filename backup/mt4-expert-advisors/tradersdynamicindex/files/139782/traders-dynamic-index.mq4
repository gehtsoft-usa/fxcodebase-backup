...
public:
   ...
   {
      return RSIBuf[period] > DnZone[period];
   }
};
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      return RSIBuf[period] < UpZone[period];
   }
};
...
int init()
{
   ...
   IndicatorBuffers(8);
   IndicatorObjPrefix = GenerateIndicatorPrefix("tdi");
   IndicatorShortName("Traders Dynamic Index");
...
int start()
{
   ...
   int toSkip = 0;
   int limit = MathMin(bars_limit, Bars - 1 - MathMax(counted_bars, toSkip));
   for (int i = limit; i >= 0; i--)
   {
      switch (source_mode)
      {
         case SourceRSI:
            ...
            break;
         case SourceDMI:
            RSIBuf[i] = iADX(_Symbol, _Period, RSI_Period, RSI_Price, MODE_MAIN, i);
            break;
         case SourceDemarker:
            RSIBuf[i] = iDeMarker(_Symbol, _Period, RSI_Period, i);
            break;
         case SourceStochastic:
            RSIBuf[i] = iStochastic(_Symbol, _Period, RSI_Period, 3, 3, MODE_SMA, 0, MODE_MAIN, i);
            break;
      }
      switch (channel_mode)
      {
         ...
         case ChannelStDev:
            {
               double SD = StDev(RSIBuf, Volatility_Band, i);
               UpZone[i] = RSIBuf[i] + SD;
               DnZone[i] = RSIBuf[i] - SD;
               MdZone[i] = RSIBuf[i];
            }
            break;
         case ChannelDonchian:
            {
               int highestIndex = ArrayMaximum(RSIBuf, i - Volatility_Band, Volatility_Band);
               UpZone[i] = RSIBuf[highestIndex];
               int lowestIndex = ArrayMinimum(RSIBuf, i - Volatility_Band, Volatility_Band);
               DnZone[i] = RSIBuf[lowestIndex];
               MdZone[i] = (UpZone[i] + DnZone[i]) / 2;
            }
            break;
         case ChannelKeltner:
            {
               double summ = 0;
               for (int ii = 0; ii < Volatility_Band; ++ii)
               {
                  summ += RSIBuf[ii];
               }
               MdZone[i] = summ / Volatility_Band;
               DnZone[i] = MdZone[i] - MdZone[i] * StdDev;
               UpZone[i] = MdZone[i] + MdZone[i] * StdDev;
            }
            break;
      }
   }
   ...
   for (int pos = limit; pos >= 0 && !IsStopped(); --pos)
   {
      if (customStream != NULL)
      {
         customStream.SetValue(pos, RSIBuf[pos]);
      }
      for (i = 0; i < ArraySize(conditions); ++i)
      {
         item = conditions[i];
