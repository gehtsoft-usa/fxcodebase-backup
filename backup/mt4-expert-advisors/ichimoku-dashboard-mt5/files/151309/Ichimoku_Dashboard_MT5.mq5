...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=73848
 ...
 public:
  FinderPattern(string Symbol)
  {
    _symbol = Symbol;
		...
		{
			handles[i] = new Ichimoku(_symbol, TF[i]);
			handles[i].set(uTenkan_sen, uKijun_sen, uSenkou_span_b);
		}
  }
  ~FinderPattern() {
	for(int i=0; i < ArraySize(handles);i++) 
  ...
  bool havePatternUp(int i, int tf_index)
  {
    // TODO: escribir el patrón alcista
		// Up
      // return handles[tf_index].Tenkansen(i) > handles[tf_index].Tenkansen(i+1) && handles[tf_index].Kijunsen(i) > handles[tf_index].Kijunsen(i+1);
      
    //   bool Cross_kumo = handles[tf_index].SenkouSpanA(i) > handles[tf_index].SenkouSpanB(i) && handles[tf_index].SenkouSpanB(i + 1) >= handles[tf_index].SenkouSpanA(i + 1);

      bool Cross_kumo = iClose(NULL, TF[tf_index], i) > handles[tf_index].SenkouSpanB(i);
      bool tenkan_kijun = handles[tf_index].Tenkansen(i) > handles[tf_index].Kijunsen(i);

      return Cross_kumo && tenkan_kijun;
  }
