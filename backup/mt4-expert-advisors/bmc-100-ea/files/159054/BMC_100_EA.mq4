...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=75866
...
input const string indicator_file = "BMC_100_PIPS";                          // Indicator File:
//---
int  bufferToBuy              = 0;
int  bufferToSell             = 1;
  ...
  public:
    bool evaluate()
    {
        for (int i = 1; i <= candles_back_for_signals; i++) {
            ...
            double indi1 = Indi(bufferToBuy, i+1);
            if (indi > 0 && indi != EMPTY_VALUE && indi1 == EMPTY_VALUE && indi != lastSignal && indi1) {
                lastSignal = indi;
                return true;
            }
        }
  ...
  public:
    bool evaluate()
    {
        for (int i = 1; i <= candles_back_for_signals; i++) {
            ...
            double indi1 = Indi(bufferToSell, i+1);
            if (indi > 0 && indi != EMPTY_VALUE && indi1 == EMPTY_VALUE && indi != lastSignal) {
                lastSignal = indi;
                return true;
            }
        }
