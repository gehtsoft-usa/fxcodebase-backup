...
string file_custom_indicator = "Nadaraya_Watson_Envelope.ex5";
int    handle                = 0;

input string Tindi       = "== Nadaraya Watson Setup =="; // ————————————
input int    Length      = 500;                           // Bars Count
input int    Bandwidth   = 17;                            // Bandwidth
input double Multiplayer = 1.5;

void setHandle() { handle = iCustom(Symbol(), NULL, file_custom_indicator, Length, Bandwidth, Multiplayer); }

double indiUP(int shift)
{
    double value[1];
...
input double userBkvPips = 20;                      // Breakeven Pips
input double userBkvStep = 1;                       // Breakeven Step
#else
...
class Order
{
  ...
  public:
    Order(long id, string symbol, double price, double sl, double tp, double lot, ENUM_ORDER_TYPE type, int magic, string comment, string strategy, datetime expireTime, datetime signalTime,
...
class BUYcondition1 : public iConditions
{
  public:
    bool evaluate()
    {
        ...
        return iLow(NULL, 0, 1) < indiDN(1) && iOpen(NULL, 0, 0) > indiDN(0);
    }
};
...
class SELLcondition1 : public iConditions
{
  public:
    bool evaluate()
    {
        ...
        return iHigh(NULL, 0, 1) > indiUP(1) && iOpen(NULL, 0, 0) < indiUP(0);
    }
};
...
class CloseOnUpperBand : public iConditions
{
  public:
    bool evaluate()
    {
...
CloseOnUpperBand *closeOnUpperBand;
class CloseOnDownBand : public iConditions
{
  public:
    bool evaluate()
    {
...
CloseOnDownBand *closeOnDownBand;
...
int OnInit()
{
    ...
    conditionsToCloseBuy.AddCondition(closeOnUpperBand = new CloseOnUpperBand());

    conditionsToCloseSell.AddCondition(conditionCloseSell = new ConditionToCloseSell());
    conditionsToCloseSell.AddCondition(closeOnDownBand = new CloseOnDownBand());
