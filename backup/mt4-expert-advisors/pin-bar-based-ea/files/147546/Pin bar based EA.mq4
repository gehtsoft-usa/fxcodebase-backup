...
// //https://fxcodebase.com/code/viewtopic.php?f=38&t=72746 
...
input int          userEntrypips           = 5; // Pips from candle to entry:
...
double AverageSizeCandles(ENUM_TIMEFRAMES _tf = 0)
{
  double SumSize = 0;
  for (int i = 1; i < 101; i++)
  {
  ...
  string FindPattern(ENUM_TIMEFRAMES _tf = 0)
  {
		...
		for (int i = 1; i < 99; i++)
    {
      ...
      if (ratio < 0.30)
      {
      	...
      	double position = 0; //(op - lo) / size;
				...
				if( size>AverageSizeCandles(_tf)*0.80 )
				{
					if (position < 0.25) { direction = "q";havePinbar = true;}
  ...
  public:
   bool evaluate()
   {
      // TODO: condition Buy 1
			if( pinbar.FindPattern(0)=="01p" ) return true;
			return false;
   }
};
...
class BUYcondition2 : public iConditions
{
  public:
   bool evaluate()
   {
      ...
      double sma = iMA(_Symbol, 0, maPeriod, maShift , maMethod, maAppliedPrice, 1);
			double lo = iLow(Symbol(),0,1);
			double cl = iClose(Symbol(),0,1);

			if( sma > lo && sma < cl ) return true;
  ...
  public:
   bool evaluate()
   {
      // TODO: condition Buy 3
			levelEntry = new Levels(new ByPipsFromCandle(_Symbol, "buy", userEntrypips, "TP", 0, 1));
      double entryPrice  = levelEntry.calculateLevel();
      delete levelEntry;

      if (Ask >= entryPrice) return true;
      return false;
  ...
  public:
   bool evaluate()
   {
      // TODO: condition sell 1
			if( pinbar.FindPattern(0)=="01q" ) return true;
      return false;
   }
};
  ...
  public:
   bool evaluate()
   {
      // TODO: condition sell 3
			levelEntry = new Levels(new ByPipsFromCandle(_Symbol, "sell", userEntrypips, "TP", 0, 1));
      double entryPrice  = levelEntry.calculateLevel();
      delete levelEntry;
			
      if (Bid <= entryPrice) return true;
      return false;
   }
};
