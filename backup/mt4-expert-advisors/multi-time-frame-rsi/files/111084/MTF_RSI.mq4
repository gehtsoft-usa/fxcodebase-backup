...
extern int      OB_Level    = 70;
extern int      OS_Level    = 30;
...
int start(){
   ...
   if (Check(TimeFrame_1)==false && Check(TimeFrame_2)==false && Check(TimeFrame_3)==false){
      ...
      for(i=floor(limit/multiplier) ; i>=0; i--){
         RSI1[i*multiplier] = iRSI(NULL,period,RSI_Periods,PRICE_CLOSE,i);
      }
      ...
      for(i=floor(limit/multiplier) ; i>=0; i--){
         RSI2[i*multiplier] = iRSI(NULL,period,RSI_Periods,PRICE_CLOSE,i);
      }
      ...
      for(i=floor(limit/multiplier) ; i>=0; i--){
         RSI3[i*multiplier] = iRSI(NULL,period,RSI_Periods,PRICE_CLOSE,i);
      }
