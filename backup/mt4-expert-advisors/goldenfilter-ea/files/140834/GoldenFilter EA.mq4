...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      double value = iCustom(_symbol, _timeframe, "GoldenFilter", Short_MA_Period, Short_MA_Method, Short_MA_Price, Long_MA_Period, Long_MA_Method, Long_MA_Price, MACD_Fast_EMA, MACD_Slow_EMA, MACD_Signal, RSI_Period, ADX_Period, DeMarker_Period, Momentum_Period, Force_Index_Period, 0, period);
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      double value = iCustom(_symbol, _timeframe, "GoldenFilter", Short_MA_Period, Short_MA_Method, Short_MA_Price, Long_MA_Period, Long_MA_Method, Long_MA_Price, MACD_Fast_EMA, MACD_Slow_EMA, MACD_Signal, RSI_Period, ADX_Period, DeMarker_Period, Momentum_Period, Force_Index_Period, 1, period);
      return value != EMPTY_VALUE && value != 0.0;
   }
};
...
int OnInit()
{
   double temp = iCustom(NULL, 0, "GoldenFilter", Short_MA_Period, Short_MA_Method, Short_MA_Price, Long_MA_Period, Long_MA_Method, Long_MA_Price, MACD_Fast_EMA, MACD_Slow_EMA, MACD_Signal, RSI_Period, ADX_Period, DeMarker_Period, Momentum_Period, Force_Index_Period, 0, 0);
   if (GetLastError() == ERR_INDICATOR_CANNOT_LOAD)
   {
      Alert("Please, install the 'GoldenFilter' indicator");
      return INIT_FAILED;
   }
