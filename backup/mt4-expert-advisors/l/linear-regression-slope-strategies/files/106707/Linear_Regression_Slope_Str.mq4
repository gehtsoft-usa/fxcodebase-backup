// Id: 16227
//+------------------------------------------------------------------+
//|                                  Linear_Regression_Slope_Str.mq4 |
...
int OnInit()
{
     double temp = iCustom(NULL, 0, "Linear_Regression_Slope", 0, 0);
   if (GetLastError() == ERR_INDICATOR_CANNOT_LOAD)
   {
       Alert("Please, install the 'Linear_Regression_Slope' indicator");
       return INIT_FAILED;
   }
   LastBar=0;
...
void OnTick()
{
 ...
 LRS0=iCustom(NULL, 0, "Linear_Regression_Slope", Length, 0, 1);   
 LRS1=iCustom(NULL, 0, "Linear_Regression_Slope", Length, 0, 2);   
 ...
 if ((Signal_Type==0 && LRS0>0. && LRS1<=0.) || (Signal_Type==1 && LRS0<0. && LRS1>=0.))
 {
  _Alert("Buy");
  CO=CalculateOrders();
  if (CO<=0)
 ...
 if ((Signal_Type==1 && LRS0>0. && LRS1<=0.) || (Signal_Type==0 && LRS0<0. && LRS1>=0.))
 {
  _Alert("Sell");
  CO=CalculateOrders();
  if (CO>=0)
