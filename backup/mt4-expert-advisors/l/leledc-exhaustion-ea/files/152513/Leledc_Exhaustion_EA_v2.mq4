//Available @ https://fxcodebase.com/code/viewtopic.php?f=38&p=151987#p151987
...
string file_custom_indicator = "Leledc-ExhaustionBar1";
...
input bool FilterByMajorTF = true;// Filter By Superior TF On?:
input ENUM_TIMEFRAMES MajorTimeFrame = PERIOD_CURRENT; // Superior Time Frame:
 ...
 public:
  bool evaluate()
  {
    // TODO: condition Buy 1
		double indi = iCustom(Symbol(),MajorTimeFrame,file_custom_indicator, qual, len, Distance, Countbars, 0 ,1);
...
class BUYcondition2 : public iConditions
{
 public:
  bool evaluate()
  {
      ...
      double indi = iCustom(Symbol(),0,file_custom_indicator, qual, len, Distance, Countbars, 0 ,1);
		return indi != EMPTY_VALUE;
      return false;
  }
};
 ...
 public:
  bool evaluate()
  {
    // TODO: condition sell 1
		double indi = iCustom(Symbol(),MajorTimeFrame,file_custom_indicator, qual, len, Distance, Countbars, 1 ,1);
		return indi != EMPTY_VALUE;
  }
};
...
class SELLcondition2 : public iConditions
{
 public:
  bool evaluate()
  {
      ...
      double indi = iCustom(Symbol(),0,file_custom_indicator, qual, len, Distance, Countbars, 1 ,1);
		return indi != EMPTY_VALUE;
      return false;
  }
};
...
class ConditionToCloseBuy : public iConditions
{
 public:
  bool evaluate()
  {
    ...
    {
        if(FilterByMajorTF) return sellCondition1.evaluate()==true && sellCondition2.evaluate()==true;
        return sellCondition2.evaluate()==true;
    }
...
class ConditionToCloseSell : public iConditions
{
 public:
  bool evaluate()
  {
    if (closeAllInOpositeSignal)
    {
        if(FilterByMajorTF) return buyCondition1.evaluate()==true && buyCondition2.evaluate()==true;
        return buyCondition2.evaluate()==true;
    }
    if (closeAllControlON)
    {
      return CloseAllControl();
...
OnInitButtons();
  ...
  //--- buys:
  if(FilterByMajorTF) conditionsToBuy.AddCondition(buyCondition1 = new BUYcondition1());
  ...
  //--- sell:
  if(FilterByMajorTF)conditionsToSell.AddCondition(sellCondition1 = new SELLcondition1());
