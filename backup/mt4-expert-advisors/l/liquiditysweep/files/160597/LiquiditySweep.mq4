...
Version:     
Date:        
Repository:  Available @ https://fxcodebase.com/code/viewtopic.php?f=38&p=160418#p160418
License:     GNU
*/
...
input bool soundOn    = false; // Enable sound
...
static datetime lastBuyAlertBarTime  = 0;
static datetime lastSellAlertBarTime = 0;
static datetime lastAnyAlertBarTime  = 0; // unify once-per-bar across types
static bool alertsReady = false;
int gBackstep = 3;
...
int OnInit()
{
    gBackstep = InpBackstep;
    if (gBackstep >= InpDepth) gBackstep = InpDepth + 1;
...
int OnCalculate(const int rates_total, const int prev_calculated, const datetime &time[], const double &open[], const double &high[], const double &low[], const double &close[], const long &tick_volume[], const long &volume[], const int &spread[])
{
    ...
    if (rates_total < InpDepth || gBackstep >= InpDepth) return (0);
    ...
    for (i = limit; i >= 0; i--) {
        ...
        if (extremum == lastlow)
            ...
            if (low[i] - extremum > InpDeviation * Point)
                ...
                for (back = 1; back <= gBackstep; back++) {
                    pos = i + back;
                ...
                if (close[i + 1] < swHigh[j] && close[i + 1] < open[i + 1]) {
                    ...
                    if (zz_line[u] > swHigh[j]) {
                        ...
                        if (pinbarOn == true && isPinbar("dn", u)) {
                            ...
                            if(alertsOn && alertsReady) TriggerAlert("Sell", u, high[u]);
                        } else if (!pinbarOn) {
                            Sell[u] = high[u];
                            if (showLines) drawLine(high[j], j, u, Crimson);
                            if(alertsOn && alertsReady) TriggerAlert("Sell", u, high[u]);
                ...
                if (close[i + 1] > swLow[j] && close[i + 1] > open[i + 1]) {
                    ...
                    if (zz_line[u] < swLow[j]) {
                        if (pinbarOn == true && isPinbar("up", u)) {
                            ...
                            if(alertsOn && alertsReady) TriggerAlert("Buy", u, low[u]);
                        } else if (!pinbarOn) {
                            Buy[u] = low[u];
                            if (showLines) drawLine(low[j], j, u, Navy);
                            if(alertsOn && alertsReady) TriggerAlert("Buy", u, low[u]);
...
void TriggerAlert(string type, int shiftIndex, double price)
{
    // Only alert on just-closed bar (shift==1)
    if(shiftIndex!=1)
        return;

    // Ensure alert only fires once per bar (and per type as extra guard)
    datetime barTime = iTime(NULL, 0, shiftIndex);
    if(barTime==lastAnyAlertBarTime) return;
    if(type=="Buy"){
        if(barTime==lastBuyAlertBarTime) return;
        lastBuyAlertBarTime = barTime;
    } else if(type=="Sell"){
        if(barTime==lastSellAlertBarTime) return;
        lastSellAlertBarTime = barTime;
    }
    lastAnyAlertBarTime = barTime;

    string msg = StringFormat("LiquiditySweep %s %s @ %.5f (TF %s)", type, Symbol(), price, TimeframeToString());
    Alert(msg);
