...
input string               uComent             = ""; // User Comment
...
datetime timePrev;
double   SL, TP, MaxProfitTrail, TickSize, TickValue, Spread, StopLevel, MinLot, MaxLot, LotStep, Pnt;
int      order_type, T, nT, T_CloseOut, nBO, nSO, nBS, nSS, nBL, nSL, nO, nS, nL, _nBO, _nSO, _nBS, _nSS, _nBL, _nSL, _nO, _nS, _nL, Slip;
bool     IsProfitBE, CloseContinue, Ans, _Ans, Activate, FatalError, FreeMarginAlert, IsModify, IsTester, IsVisual;
int      tickets[];
...
class PartialClose
{
  ...
  public:
    ...
    double lots() { return NormalizeDouble((originalLot * percentToClose / 100), 2); }
...
class MartingaleIncrementLot
{
  ...
  public:
    ...
    double Lot()
    {
        ...
        if (next > _maxLot) return _maxLot;
        if (count() == _maxCount) return 0;
        return next;
    }
...
void close_by_reversals()
{
    ...
    if (cl_by_reversal_filter_on) {
        ...
        if (indi_buy != EMPTY_VALUE && indi_buy != 0) {
            current_filter_side = 1;
        }
        if (indi_sell != EMPTY_VALUE && indi_sell != 0) {
            current_filter_side = -1;
        }
    }
...
void OnTick()
{
    ...
    // xxxxx
    if (nO > 0) {
        if (UseCloseOut) CloseOut();
        // xxxxx
        if (TP_Mode == Individual) {
            if (SL_money > 0.0 || TP_money > 0.0) Check_SL_TP();
            if (UseBreakEven) Breakeven();
            if (UseTrailingStop) Trail();
        }
        // xxxxx
        else if (TP_Mode == Basket) {
    ...
    if (partialCloseOn) partialClose.takePartial();
    // xxxxx
    close_by_reversals();
}
// xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx//
void Trade()
{
    ...
    if (MO(order_type, Lot_Size, uComent) == -1) return;
...
int    candles_back_for_signals = 5;
double lastSignal_sell          = 0;
double lastSignal_buy           = 0;
// xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx//
int GetSign()
{
    ...
    for (int i = 1; i <= candles_back_for_signals; i++) {
        double indi = i_custom(Buy_Buffer_NO, i);
        if (indi > 0 && indi != EMPTY_VALUE && indi != lastSignal_buy) {
            if (filter_on && !filter_buy) return (0);
    ...
    for (int i = 1; i <= candles_back_for_signals; i++) {
        double indi = i_custom(Sell_Buffer_NO, i);
        if (indi > 0 && indi != EMPTY_VALUE && indi != lastSignal_sell) {
            if (filter_on && !filter_sell) return (0);

                signal          = -1;
                lastSignal_sell = indi;
...
void CloseOut()
{
    ...
    int    i = 0, j = 0, t_buy = -1, t_sell = -1;
    double pf = 0.0, pf_buy = 0.0, pf_sell = 0.0;
    if (CloseOutStyle == BE) {
        for (i = 0; i < nT; i++) {
        ...
        for (i = 0; i < nT; i++) {
            ...
            if (OrderType() == OP_BUY && (pf_buy == 0.0 || pf > pf_buy)) {
                pf_buy = pf;
                t_buy  = tickets[i];
            } else if (OrderType() == OP_SELL && (pf_sell == 0.0 || pf > pf_sell)) {
                pf_sell = pf;
                t_sell  = tickets[i];
            }
        }
...
void Check_SL_TP_basket()
{
    ...
    if ((SL_money != 0.0 && curr_result <= -fabs(SL_money)) || (TP_money > 0.0 && curr_result >= TP_money)) {
        CloseContinue = true;
        if (ClosePos()) CloseContinue = false;
        if (!IsTester) {
            GVS("CloseContinue", (double)CloseContinue);
            GlobalVariablesFlush();
        }
...
void Trail_basket()
{
    ...
    } else if (MaxProfitTrail > 0.0 && curr_result_basket > MaxProfitTrail) {
        MaxProfitTrail = curr_result_basket;
        if (!IsTester) {
            GVS("MaxProfitTrail", MaxProfitTrail);
            GlobalVariablesFlush();
        }
    } else if (MaxProfitTrail > 0.0 && curr_result_basket <= MaxProfitTrail - Trail_Step) {
        CloseContinue = true;
        if (ClosePos()) CloseContinue = false;
        if (!CloseContinue) {
            MaxProfitTrail = 0.0;
...
void Check_SL_TP()
{
    int    i  = 0;
    color  c  = clrNONE;
    double sl = 0.0, tp = 0.0, sl_lev = 0.0, tp_lev = 0.0, comm_and_swap = 0.0, comm_swap_pnt = 0.0;
    for (i = 0; i < nT; i++) {
        if (!OrderSelect(tickets[i], SELECT_BY_TICKET) || OrderCloseTime() > 0 || OrderType() > 1) continue;
...
int getTickets()
{
    ...
    for (pos = OrdersTotal() - 1; pos >= 0; pos--) {
        for (j = 0; j < 10; j++) {
            if (OrderSelect(pos, SELECT_BY_POS)) break; /*=>*/
            if (j < 9) Sleep(100);
        } /*for =>*/
        if (j == 10) continue;
...
void _numbOrders(int i = 0)
{
    ...
    for (pos = 0; pos < nT; pos++) {
        for (j = 0; j < 10; j++) {
            if (OrderSelect(tickets[pos], SELECT_BY_TICKET)) break; /*=>*/
            if (j < 9) Sleep(100);
        } /*for =>*/
        if (j == 10) continue;
...
{
    if (sl_lev == -1.0) sl_lev = sl();
    if (tp_lev == -1.0) tp_lev = tp();
    if (op_lev == -1.0) op_lev = op();
...
int numberOfPositions(int dir, string sym)
{
    ...
    for (int pos = OrdersTotal() - 1; pos >= 0; pos--) {
        ...
        if ((OrderSymbol() == sym || sym == "") && OrderMagicNumber() == (int)Magic && Magic > 0) {
            if (OrderType() == OP_BUY)
                if (dir == 1 || dir == 0) countPos++;
            if (OrderType() == OP_SELL)
                if (dir == 2 || dir == 0) countPos++;
        }
    }
    return countPos;
}
...
int MO(int type, double lot, string comment = "") // market order
{
    ...
    int    i = 0;
    T        = -1;
    if (numberOfPositions(0, Symbol()) >= maxPos && maxPos > 0) return (T);
    RefreshRates();
    if (type == OP_BUY) {
        if (numberOfPositions(1, Symbol()) >= maxBuyPos && maxBuyPos > 0) return (T);
        c     = clrBlue;
        price = Ask;
    } else if (type == OP_SELL) {
        if (numberOfPositions(2, Symbol()) >= maxSellPos && maxSellPos > 0) return (T);
        c     = clrRed;
        price = Bid;
    } else
        return (T);
    ...
    while (T < 0 && i < 5) {
        ...
        if (T < 0) {
            if (!Errors(GetLastError())) return (T);
        }
        i++;
    } /*while*/
    return (T);
    ...
    if (OrderType() == OP_BUY) {
        if (TP > 0.0) {
            if (tpType == 0)
                ...
                tp = TP;
            if (ND(tp - Bid) < StopLevel) tp = Bid + 1.5 * StopLevel;
        }
        if (SL > 0.0) {
            if (slType == 0)
                ...
                sl = SL;
            if (ND(Bid - sl) < StopLevel) sl = Bid - 1.5 * StopLevel;
        }
        c = clrBlue;
    } else if (OrderType() == OP_SELL) {
        if (TP > 0.0) {
            if (tpType == 0)
                ...
                tp = TP;
            if (ND(Ask - tp) < StopLevel) tp = Ask - 1.5 * StopLevel;
        }
        if (SL > 0.0) {
            if (slType == 0)
                sl = op + fmax(SL * Pnt, StopLevel);
                ...
                sl = SL;
            if (ND(sl - Ask) < StopLevel) sl = Ask + 1.5 * StopLevel;
        }
        c = clrRed;
    }
...
bool HistoryCheck(int n = 10, int sleep = 1000)
{
    ...
    if (i == n) { /*Comment("Update failed. Go to the next attempt.");*/
        Comment("");
        return (false);
    }
    Comment("");
...
// xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx//
double tickSize(string symb = "")
{
    if (symb == "") symb = Symbol();
    return (MarketInfo(symb, MODE_TICKSIZE));
}
//+------------------------------------------------------------------+
//|                                                                  |
//+------------------------------------------------------------------+
double tickValue(string symb = "")
{
    if (symb == "") symb = Symbol();
    return (MarketInfo(symb, MODE_TICKVALUE));
}
//+------------------------------------------------------------------+
//|                                                                  |
//+------------------------------------------------------------------+
double spread(string symb = "")
{
    if (symb == "") symb = Symbol();
    return (MarketInfo(symb, MODE_SPREAD));
}
//+------------------------------------------------------------------+
//|                                                                  |
//+------------------------------------------------------------------+
double _spread(string symb = "")
{
    if (symb == "") symb = Symbol();
    return (MarketInfo(symb, MODE_SPREAD) * point());
}
//+------------------------------------------------------------------+
//|                                                                  |
//+------------------------------------------------------------------+
double stopLevel(string symb = "")
{
    if (symb == "") symb = Symbol();
...
//|                                                                  |
//+------------------------------------------------------------------+
double _stopLevel(string symb = "")
{
    if (symb == "") symb = Symbol();
    return (MarketInfo(symb, MODE_STOPLEVEL) * point());
}
//+------------------------------------------------------------------+
//|                                                                  |
//+------------------------------------------------------------------+
double freezeLevel(string symb = "")
{
    if (symb == "") symb = Symbol();
    return (MarketInfo(symb, MODE_FREEZELEVEL));
}
//+------------------------------------------------------------------+
//|                                                                  |
//+------------------------------------------------------------------+
double _freezeLevel(string symb = "")
{
    if (symb == "") symb = Symbol();
    return (MarketInfo(symb, MODE_FREEZELEVEL) * point());
}
//+------------------------------------------------------------------+
//|                                                                  |
//+------------------------------------------------------------------+
double minLot(string symb = "")
{
    if (symb == "") symb = Symbol();
    return (MarketInfo(symb, MODE_MINLOT));
}
//+------------------------------------------------------------------+
//|                                                                  |
//+------------------------------------------------------------------+
double maxLot(string symb = "")
{
    if (symb == "") symb = Symbol();
    return (MarketInfo(symb, MODE_MAXLOT));
}
//+------------------------------------------------------------------+
//|                                                                  |
//+------------------------------------------------------------------+
double lotStep(string symb = "")
{
    if (symb == "") symb = Symbol();
    return (MarketInfo(symb, MODE_LOTSTEP));
}
//+------------------------------------------------------------------+
//|                                                                  |
//+------------------------------------------------------------------+
double bid(string symb = "")
{
    if (symb == "") symb = Symbol();
    return (MarketInfo(symb, MODE_BID));
}
//+------------------------------------------------------------------+
//|                                                                  |
//+------------------------------------------------------------------+
double ask(string symb = "")
{
    if (symb == "") symb = Symbol();
    return (MarketInfo(symb, MODE_ASK));
}
//+------------------------------------------------------------------+
//|                                                                  |
//+------------------------------------------------------------------+
...
{
    if (symb == "") symb = Symbol();
    return (MarketInfo(symb, MODE_POINT));
}
//+------------------------------------------------------------------+
//|                                                                  |
//+------------------------------------------------------------------+
int digits(string symb = "")
{
    if (symb == "") symb = Symbol();
    return ((int)MarketInfo(symb, MODE_DIGITS));
}
// xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx//
double NL(double L) { return (MathRound(MathMin(MathMax(L, MinLot), MaxLot) / LotStep) * LotStep); }
// xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx//
...
{
    if (IsTester)
        return ("_" + IntegerToString(Magic) + "_" + Symbol() + "_" + "Tester");
    else
        return ("_" + IntegerToString(Magic) + "_" + Symbol());
}
// xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx//
...
void TSL_info()
{
    ...
    if (MaxProfitTrail > 0.0) {
        drawLbl("TSL_lev", "TSL LEVEL : " + DoubleToString(MaxProfitTrail - Trail_Step, 2) + " " + AccountCurrency(), CORNER_LEFT_UPPER, 20, 20, 12, "Tahoma", clrGold);
    } else if (MaxProfitTrail <= 0.0) {
