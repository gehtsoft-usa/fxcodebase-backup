...
Version:     
Date:        
Repository:  Available @ https://fxcodebase.com/code/viewtopic.php?f=38&p=161266#p161266
License:     GNU
*/
...
input int             startMultiplierAtCount    = 2;                       // Start Multiplier at Count:
input double          firstLotSize              = 0.01;                    // First Lot Size:
input double          secondLotSize             = 0.01;                    // Second Lot Size:
...
class MartingaleIncrementLot
{
  ...
  public:
    ...
    double Lot()
    {
        ...
        // primeras dos llamadas fijas
        if(c == 0) return NormalizeDouble(firstLotSize, 2);
        if(c == 1) return NormalizeDouble(secondLotSize, 2);

        // si alcanzó maxCount, no más intentos
        if (c == _maxCount) return 0;

        // base para incrementar: si no hay _lastLot válido, usar secondLotSize
        double baseLot = _lastLot;
        if(baseLot <= 0.0) baseLot = secondLotSize;

        double next = baseLot;

        // comenzar a multiplicar solo cuando c >= startMultiplierAtCount
        if(c >= startMultiplierAtCount)
        {
            int steps = c - startMultiplierAtCount + 1;
            for(int i = 0; i < steps; i++)
                next *= _multiplier;
        }
        // si aún no llegó a startMultiplierAtCount, se mantiene el baseLot

        next = NormalizeDouble(next, 2);
