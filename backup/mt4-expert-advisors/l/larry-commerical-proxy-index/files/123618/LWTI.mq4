...
extern int Length=8;
extern MATypes SmoothingType = ma_sma; // Smoothing type
...
double LWTI[];
ArrayStream *smoothingSource;
IStream *smoothing;

int init()
{
   IndicatorShortName("Larry Large Trade Index");
   IndicatorDigits(Digits);
   SetIndexStyle(0,DRAW_LINE);
   SetIndexBuffer(0,LWTI);
   smoothingSource = new ArrayStream(1);
   SetIndexStyle(1,DRAW_NONE);
   smoothing = AveragesStreamFactory::Create(smoothingSource, Length, SmoothingType);
...
int start()
{
   ...
   if(ExtCountedBars>2)
      limit = MathMin(Length, Bars - ExtCountedBars - 1);
   int pos;
   pos=limit;
   while(pos>=0)
   {
      smoothingSource.Buffer[pos]=Close[pos]-Close[pos+Length-1];
      pos--;
   } 
   ...
   while(pos>=0)
   {
      if (smoothing.GetValue(pos, MA))
      {
         ATR=iATR(NULL, 0, Length, pos);
         LWTI[pos] = ATR!=0 ? 50*MA/ATR+50 : 0;
      }
      pos--;
   }  
   return(0);
