...
#endif
input int close_trade_before_close = 5; // Seconds till the candle close to close the trade
...
class CloseTradeBeforeCandleClose : public AOrderAction
{
   ActionOnConditionLogic* _actions;
public:
   CloseTradeBeforeCandleClose(ActionOnConditionLogic* actions)
   {
      _actions = actions;
   }
   ...
   virtual bool DoAction()
   {
      ...
      _actions.AddActionOnCondition(action, new SecondsBeforeCandleCloseCondition(_Symbol, (ENUM_TIMEFRAMES)_Period, close_trade_before_close));
      action.Release();
      return true;
   }
};
...
TradingController *CreateController(const string symbol, const ENUM_TIMEFRAMES timeframe, string &error)
{
   ...
   CloseTradeBeforeCandleClose* closeAction = new CloseTradeBeforeCandleClose(actions);
   controller.AddOrderAction(closeAction);
   closeAction.Release();
