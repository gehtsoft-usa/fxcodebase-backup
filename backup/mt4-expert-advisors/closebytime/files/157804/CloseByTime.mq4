...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=75495
...
input bool   close_by_strategy_on = false;                   // Close positions by time:
input string close_tm             = "00:00";                 // Close Time:
input bool   only_close_winners   = true;                    // Only Close Winners?
  ...
  public:
    bool evaluate()
    {
        ...
        if (OrderType() == OP_BUY) {
            ...
            datetime userTm = StringToTime(close_tm);
            if (TimeCurrent() >= userTm) {
                if (!only_close_winners) {
                    r = true;
                }
                if (only_close_winners == true && OrderProfit() > 0) {
                    r = true;
                }
            }
        }
...
void showTimers()
{
    ...
    datetime userTm = StringToTime(close_tm);
    string msg = "Current Time: " + TimeToString(tm, TIME_MINUTES) + "\n" + "Time to Close: " +  TimeToString(userTm, TIME_MINUTES);
    Comment(msg);
}
