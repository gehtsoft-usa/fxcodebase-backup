...
//http://fxcodebase.com/code/viewtopic.php?f=38&t=70580
...
input int ma_periods = 5; // MA period
...
int init()
{
   ...
   IndicatorBuffers(12);
   IndicatorObjPrefix = GenerateIndicatorPrefix("mabba");
   IndicatorShortName("MA BB Alert");
   ...
   SetIndexBuffer(id, mal);
   SetIndexLabel(id, "MA Low");
   ++id;
   SetIndexStyle(id, DRAW_LINE);
   SetIndexBuffer(id, mah);
   SetIndexLabel(id, "MA High");
   ++id;
   SetIndexStyle(id, DRAW_LINE);
   SetIndexBuffer(id, bbt);
   SetIndexLabel(id, "BB Top");
   ++id;
   SetIndexStyle(id, DRAW_LINE);
   SetIndexBuffer(id, bbb);
   SetIndexLabel(id, "BB Bottom");
   ++id;
...
int start()
{
   ...
   {
      ArrayInitialize(bbb, EMPTY_VALUE);
      ArrayInitialize(bbt, EMPTY_VALUE);
   ...
   {
      mal[pos] = iMA(_Symbol, _Period, ma_periods, 0, MODE_SMA, PRICE_LOW, pos);
      mah[pos] = iMA(_Symbol, _Period, ma_periods, 0, MODE_SMA, PRICE_HIGH, pos);
      bbt[pos] = iBands(_Symbol, _Period, bb_period, bb_deviation, bb_shift, bb_price, MODE_UPPER, pos);
      bbb[pos] = iBands(_Symbol, _Period, bb_period, bb_deviation, bb_shift, bb_price, MODE_LOWER, pos);
      if (customStream != NULL)
      {
