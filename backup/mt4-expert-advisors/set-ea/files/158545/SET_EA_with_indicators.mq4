//Available @  https://fxcodebase.com/code/viewtopic.php?f=38&p=158417#p158417
...
#property strict
#resource "\\Indicators\\[SHK]_Schaff_Trend_Cycle_(STC).ex4" 
#resource "\\Indicators\\traders-dynamic-index.ex4" 
...
input int RSI_Period = 13;        //8-25
input int RSI_Price = MODE_CLOSE; //0-6
input int Volatility_Band = 34;   //20-40
input int RSI_Price_Line = 2;
input int RSI_Price_Type = MODE_SMA; //0-3
input int Trade_Signal_Line = 7;
input int Trade_Signal_Type = MODE_SMA; //0-3
...
input int BL1 = 50; // Buy Level
input int SL1 = 50; // Sell Level
input int BL2 = 50; // Buy Level 2
input int SL2 = 50; // Sell Level 2

input int ma_length = 200; // MA length
input ENUM_MA_METHOD ma_type = MODE_SMA; // MA type

input int use_filter = true; // Use MVA Filter
input int fma_length = 200; // MVA Filter Period
input ENUM_MA_METHOD fma_type = MODE_SMA; // MVA Filter Method
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      ...
      double maValue = iMA(_symbol, _timeframe, ma_length, 0, ma_type, PRICE_CLOSE, period);
      double fmaValue = iMA(_symbol, _timeframe, fma_length, 0, fma_type, PRICE_CLOSE, period);
      // double Schaff = iCustom(_symbol, _timeframe, "[SHK]_Schaff_Trend_Cycle_(STC)", EEEEEE, BBBB, BBBBB, AAA, 0, period);
      // double value=iCustom(_Symbol,_Period,"::Indicators\\SampleIndicator.ex4",0,0); 
      double Schaff = iCustom(_symbol, _timeframe, "::Indicators\\[SHK]_Schaff_Trend_Cycle_(STC).ex4", EEEEEE, BBBB, BBBBB, AAA, 0, period);
      // double RSI_P = iCustom(_symbol, _timeframe, "traders-dynamic-index", source_mode, channel_mode, RSI_Period, RSI_Price, Volatility_Band, RSI_Price_Line, RSI_Price_Type, Trade_Signal_Line, Trade_Signal_Type, StdDev,
      double RSI_P = iCustom(_symbol, _timeframe, "::Indicators\\traders-dynamic-index.ex4", source_mode, channel_mode, RSI_Period, RSI_Price, Volatility_Band, RSI_Price_Line, RSI_Price_Type, Trade_Signal_Line, Trade_Signal_Type, StdDev,
         4, period + 1);
      // double MB = iCustom(_symbol, _timeframe, "traders-dynamic-index", source_mode, channel_mode, RSI_Period, RSI_Price, Volatility_Band, RSI_Price_Line, RSI_Price_Type, Trade_Signal_Line, Trade_Signal_Type, StdDev,
      double MB = iCustom(_symbol, _timeframe, "::Indicators\\traders-dynamic-index.ex4", source_mode, channel_mode, RSI_Period, RSI_Price, Volatility_Band, RSI_Price_Line, RSI_Price_Type, Trade_Signal_Line, Trade_Signal_Type, StdDev,
         2, period + 1);
      // double TS = iCustom(_symbol, _timeframe, "traders-dynamic-index", source_mode, channel_mode, RSI_Period, RSI_Price, Volatility_Band, RSI_Price_Line, RSI_Price_Type, Trade_Signal_Line, Trade_Signal_Type, StdDev,
      double TS = iCustom(_symbol, _timeframe, "::Indicators\\traders-dynamic-index.ex4", source_mode, channel_mode, RSI_Period, RSI_Price, Volatility_Band, RSI_Price_Line, RSI_Price_Type, Trade_Signal_Line, Trade_Signal_Type, StdDev,
         5, period);
      return close < maValue && Schaff > BL1 && RSI_P <= MB && TS > BL2 && (!use_filter || close > fmaValue);
   }
};
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      ...
      double TS = iCustom(_symbol, _timeframe, "::Indicators\\traders-dynamic-index.ex4", source_mode, channel_mode, RSI_Period, RSI_Price, Volatility_Band, RSI_Price_Line, RSI_Price_Type, Trade_Signal_Line, Trade_Signal_Type, StdDev,
         5, period);
      return close > maValue && Schaff < SL1 && RSI_P >= MB && MB < SL2 && (!use_filter || close < fmaValue);
   }
};
...
int OnInit()
{
   // double temp = iCustom(NULL, 0, "traders-dynamic-index", 0, 0);
   // if (GetLastError() == ERR_INDICATOR_CANNOT_LOAD)
   // {
   //    Alert("Please, install the 'traders-dynamic-index' indicator (http://fxcodebase.com/code/viewtopic.php?f=38&t=70751)");
   //    return INIT_FAILED;
   // }
   // temp = iCustom(NULL, 0, "[SHK]_Schaff_Trend_Cycle_(STC)", 0, 0);
   // if (GetLastError() == ERR_INDICATOR_CANNOT_LOAD)
   // {
   //    Alert("Please, install the '[SHK]_Schaff_Trend_Cycle_(STC)' indicator (http://fxcodebase.com/code/viewtopic.php?f=38&t=74157)");
   //    return INIT_FAILED;
   // }
