...
// http://fxcodebase.com/code/viewtopic.php?f=38&t=70609
...
input int            ma1_period  = 100;       // MA 1 Period
input ENUM_MA_METHOD ma2_method  = MODE_SMA;  // MA 2 Method
input int            ma2_period  = 130;       // MA 2 Period
input ENUM_MA_METHOD ma3_method  = MODE_SMA;  // MA 3 Method
input int            ma3_period  = 200;       // MA 3 Period
input int            ob          = 70;        // Overbought Level (OB)
input int            os          = 30;        // Oversold Level (OS)
...
input SSDPrice ssd_price = SSDHL;  // Price Field
...
input int qntCandlesToExit = 0;  // Quantity of Candle to close a loser position:
...
input double         take_profit_value             = 20;          // Take profit value
...
#endif
input int    close_day  = 1;          // Day of the month to close all positions
input string close_time = "000000";   // Time in hhmmss format to close all positions
...
{
  public:
   virtual void   AddRef()                                             = 0;  // add reference
...
class OrdersIterator
{
  ...
  private:
   bool PassFilter()
   {
      if (_useMagicNumber && OrderMagicNumber() != _magicNumber) return false;
      if (_useOrderType && OrderType() != _orderType) return false;
      if (_trades && !IsTrade()) return false;
      if (_orders && IsTrade()) return false;
      if (_useSymbol && OrderSymbol() != _symbol) return false;
      if (_useProfit)
      {
         ...
         case ERR_INVALID_STOPS: {
            ...
            else
            {
               ...
               if (rateDistance < minStopDistancePoints)
                  error = "Distance to the pending order rate is too close: " + DoubleToStr(rateDistance, 1) + ". Min. allowed distance: " + IntegerToString(minStopDistancePoints);
               else
      ...
      if (order == -1)
      {
         ...
         switch (error)
         {
            ...
            case ERR_INVALID_TRADE_PARAMETERS:
               errorMessage = "Incorrect trade parameters. Symbol: " + _instrument + " Order type: " + IntegerToString(orderType) + " Amount: " + DoubleToString(_amount) + " Rate: " + DoubleToString(rate) + " Slippage: " + DoubleToString(_slippage) + " SL: " + DoubleToString(sl) + " TP: " + DoubleToString(tp) + " Comment: " + _comment == NULL ? "" : _comment + " Magic number: " + IntegerToString(_magicNumber);
               break;
            default:
...
class TradingController
{
  ...
  public:
   ...
   ~TradingController()
   {
      ...
      if (_exitLongCondition != NULL) _exitLongCondition.Release();
      if (_exitShortCondition != NULL) _exitShortCondition.Release();
      delete _calculator;
      delete _signaler;
   }
  ...
  private:
   ...
   void DoExitLogic(int exitTradePeriod, datetime date, string& longLog, string& shortLog)
   {
      ...
      //--- cav
      IAction* orderAction = new CloseLoserTradeByBarsAction(_Symbol, magic_number);
      orderAction.DoAction(_Symbol, _Period);
      delete orderAction;
      //--- 
...
//--- cav
//+------------------------------------------------------------------+
class CloseLoserTradeByBarsAction : public AOrderAction
{
   // IOrder* _order;
   // int     _slippagePoints;
   // double  _toClose;
  ...
  public:
   // CloseLoserTradeByBarsAction(IOrder* order, int slippagePoints)
   CloseLoserTradeByBarsAction(string symbol, int magicNumber)
   {
      _symbol      = symbol;
      _magicNumber = magicNumber;
   }

   ~CloseLoserTradeByBarsAction()
   {
      // _order.Release();
   }
   ...
   virtual bool DoAction(const int period, const datetime date)
   {
      if (qntCandlesToExit <= 0) return false;
      ...
      for (int i = OrdersTotal() - 1; i >= 0; i--)
      {
         if (OrderSelect(i, SELECT_BY_POS, MODE_TRADES) && OrderSymbol() == _Symbol && OrderMagicNumber() == _magicNumber)
         {
            // while (trades.Next())
            // {
            // if (!trades.Select() || OrderCloseTime() != 0 || OrderProfit() > 0)
            ...
            Print(__FUNCTION__, " ", "_Period", " ", _Period);
            int bars = iBarShift(_Symbol, _Period, opTime, true);

            Print(__FUNCTION__, " ", "opTime", " ", opTime);
            Print(__FUNCTION__, " ", "bars", " ", bars);
            Print(__FUNCTION__, " ", "OrderTicket()", " ", OrderTicket());

            if (bars >= qntCandlesToExit)
            {
               int    orderType = OrderType();
               string error;
               double price = orderType == OP_BUY ? InstrumentInfo::GetBid(OrderSymbol()) : InstrumentInfo::GetAsk(OrderSymbol());
               if (!TradingCommands::CloseCurrentOrder(price, 30, error))
               {
                  Print("Position close error: " + error);
                  return false;
               }
...
class PivotLongCondition : public ACondition
{
  public:
   PivotLongCondition(const string symbol, ENUM_TIMEFRAMES timeframe)
       : ACondition(symbol, timeframe)
   {
   }
   ...
   bool IsPass(const int period, const datetime date)
   {
      ...
      return iClose(_symbol, _timeframe, period) > p;
   }
};
class MA1LongCondition : public ACondition
{
  public:
   MA1LongCondition(const string symbol, ENUM_TIMEFRAMES timeframe)
       : ACondition(symbol, timeframe)
   {
   }
   ...
   bool IsPass(const int period, const datetime date)
   {
      double maValue = iMA(_symbol, _timeframe, ma1_period, 0, ma1_method, PRICE_CLOSE, period);
      return iClose(_symbol, _timeframe, period) > maValue;
   }
};
class MA1MA2LongCondition : public ACondition
{
  public:
   MA1MA2LongCondition(const string symbol, ENUM_TIMEFRAMES timeframe)
       : ACondition(symbol, timeframe)
   {
   }
   ...
   bool IsPass(const int period, const datetime date)
   {
      double ma1Value = iMA(_symbol, _timeframe, ma1_period, 0, ma1_method, PRICE_CLOSE, period);
      double ma2Value = iMA(_symbol, _timeframe, ma2_period, 0, ma2_method, PRICE_CLOSE, period);
      return ma1Value > ma2Value;
   }
};
class MA2MA3LongCondition : public ACondition
{
  public:
   MA2MA3LongCondition(const string symbol, ENUM_TIMEFRAMES timeframe)
       : ACondition(symbol, timeframe)
   {
   }
   ...
   bool IsPass(const int period, const datetime date)
   {
      double ma3Value = iMA(_symbol, _timeframe, ma3_period, 0, ma3_method, PRICE_CLOSE, period);
      double ma2Value = iMA(_symbol, _timeframe, ma2_period, 0, ma2_method, PRICE_CLOSE, period);
      return ma2Value > ma3Value;
   }
};

class SSDLongCondition : public ACondition
{
  public:
   SSDLongCondition(const string symbol, ENUM_TIMEFRAMES timeframe)
       : ACondition(symbol, timeframe)
   {
   }
   ...
   bool IsPass(const int period, const datetime date)
   {
      double stochValue = iStochastic(_symbol, _timeframe, ssd_k, ssd_d, ssd_slowing, MODE_SMA, (int)ssd_price, MODE_MAIN, period);
      return stochValue < os;
   }
};
class PivotShortCondition : public ACondition
{
  public:
   PivotShortCondition(const string symbol, ENUM_TIMEFRAMES timeframe)
       : ACondition(symbol, timeframe)
   {
   }
...
class MA1ShortCondition : public ACondition
{
  public:
   MA1ShortCondition(const string symbol, ENUM_TIMEFRAMES timeframe)
       : ACondition(symbol, timeframe)
   {
   }
...
class MA1MA2ShortCondition : public ACondition
{
  public:
   MA1MA2ShortCondition(const string symbol, ENUM_TIMEFRAMES timeframe)
       : ACondition(symbol, timeframe)
   {
   }
...
class MA2MA3ShortCondition : public ACondition
{
  public:
   MA2MA3ShortCondition(const string symbol, ENUM_TIMEFRAMES timeframe)
       : ACondition(symbol, timeframe)
   {
   }
...
class SSDShortCondition : public ACondition
{
  public:
   SSDShortCondition(const string symbol, ENUM_TIMEFRAMES timeframe)
       : ACondition(symbol, timeframe)
   {
   }
...
class ExitLongCondition : public ACondition
{
  public:
   ...
   //TODO: implement
   /* 
      En un trade perdedor tienen que haber pasado x velas que el usuario ingresa
      - por lo tanto: 
      .      tengo que saber que el trade está perdedor      
      .      tengo que contar las velas dede la actual hasta la entrada del trade
      .      encontrar la vela de entrada del trade: buscar la hora, 
      */

   bool IsPass(const int period, const datetime date)
   // bool IsPass(const int period, const datetime date, IOrder* order)
   {
      // if (qntCandlesToExit > 0)
      // {
      //    order.Select();
      //    datetime opTime = OrderOpenTime();

      //    int bars = iBarShift(_Symbol, period, opTime, false);
      //    Print(__FUNCTION__, " ", "bars", " ", bars);
      //    return bars >= qntCandlesToExit;
      // }

      // datetime opTime = trades.GetOpenTime();
      // for(int i=OrdersTotal()-1;i>=0;i--)
      // {
      //    // if(OrderSelect(i,SELECT_BY_POS, MODE_TRADES) && OrderSymbol() == _Symbol && OrderMagicNumber() == magic_number && OrderProfit() < 0 )
      //    if(OrderSelect(i,SELECT_BY_POS, MODE_TRADES) && OrderSymbol() == _Symbol && OrderMagicNumber() == magic_number && OrderType()==OP_BUY )
      //    {
      //       // datetime opTime = OrderOpenTime();
      //       int bars = iBarShift(_Symbol, period, OrderOpenTime(), false);
      //       Print(__FUNCTION__," ","bars"," ",bars);
      //       return bars >= qntCandlesToExit && qntCandlesToExit > 0;
      //    }
      // }
...
ICondition* CreateLongCondition(string symbol, ENUM_TIMEFRAMES timeframe)
{
   ...
   condition.Add(new SSDLongCondition(symbol, timeframe), false);
...
ICondition* CreateLongFilterCondition(string symbol, ENUM_TIMEFRAMES timeframe)
{
   ...
   AndCondition* condition = new AndCondition();
   condition.Add(new MA2MA3LongCondition(symbol, timeframe), false);
   condition.Add(new MA1MA2LongCondition(symbol, timeframe), false);
   condition.Add(new MA1LongCondition(symbol, timeframe), false);
   condition.Add(new PivotLongCondition(symbol, timeframe), false);
   return condition;
}
...
ICondition* CreateShortCondition(string symbol, ENUM_TIMEFRAMES timeframe)
{
   ...
   AndCondition* condition = new AndCondition();
   condition.Add(new SSDShortCondition(symbol, timeframe), false);
#ifdef ACT_ON_SWITCH_CONDITION
   ActOnSwitchCondition* switchCondition = new ActOnSwitchCondition(symbol, timeframe, (ICondition*)condition);
   condition.Release();
   return switchCondition;
...
ICondition* CreateShortFilterCondition(string symbol, ENUM_TIMEFRAMES timeframe)
{
   ...
   AndCondition* condition = new AndCondition();
   condition.Add(new MA2MA3ShortCondition(symbol, timeframe), false);
   condition.Add(new MA1MA2ShortCondition(symbol, timeframe), false);
   condition.Add(new MA1ShortCondition(symbol, timeframe), false);
   condition.Add(new PivotShortCondition(symbol, timeframe), false);
   return condition;
}
...
TradingController* CreateController(const string symbol, const ENUM_TIMEFRAMES timeframe, string algoId, string& error)
{
   ...
   if (tradingTimeCondition == NULL)
      return NULL;
#endif
   int closeTime = ParseTime(close_time, error);
   if (closeTime == -1)
   {
   ...
   DayTimeCondition* condition = new DayTimeCondition(close_day, closeTime, 60);
