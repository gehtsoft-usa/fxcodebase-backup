...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=75202&p=157438#p157438
...
input bool   close_by_strategy_on = true;                    // Close positions:
input string uSym                 = "";                      // Symbol (Empty = Current Symbol) :
input int    uMagic               = -1;                      // Magic: (<0 not consider):
input double uCoef                = 5;                       // Coeficient to close:
...
class ConditionCloseBuy : public iConditions
{
  public:
    bool evaluate()
    {
        ...
        bool okSymbol = true;
        if (uSym != "" && OrderSymbol() != uSym) {
            okSymbol = false;
        }
        bool okMagic = true;
        if (uMagic > 0 && OrderMagicNumber() != uMagic) {
            okMagic = false;
        }

        double tradeProfit = OrderProfit() + OrderSwap() + OrderCommission();
        double limit       = OrderLots() * uCoef * 100;

        if (OrderType() == OP_BUY && okSymbol == true && okMagic==true) {
            return (tradeProfit >= limit);
        }
...
class ConditionCloseSell : public iConditions
{
  public:
    bool evaluate()
    {
        ...
        if (OrderType() == OP_SELL && okSymbol == true && okMagic == true) {
            return (tradeProfit >= limit);
        }
...
input bool         BBFilterOn       = true;                               // Close by BB?
input double       bb_coef          = 0.5;                                // Coeficient (limit = band * coef):
...
class ConditionBB_Buy : public iConditions
{
  public:
    bool evaluate()
    {
        ...
        // abajo de la main
        return iClose(NULL, 0, 0) >= bb(bu_up, 1) * bb_coef;
    }
};
...
class ConditionBB_Sell : public iConditions
{
  public:
    bool evaluate()
    {
        ...
        // arriba de la main
        return iClose(NULL, 0, 0) <= bb(bu_low, 1) * bb_coef;
    }
};
...
void oninit_close_by_bb()
{
    if(!BBFilterOn)return;

    stCloseBuy.addCondition(&cdBB_Buy);
    stCloseSell.addCondition(&cdBB_Sell);
}
