...
// http://fxcodebase.com/code/viewtopic.php?f=38&t=71290
...
input double Period1=0; 
input double Period2=0; 
input double Period3=200; 
input string   Dev_Step_1="1,3";
input string   Dev_Step_2="8,5";
input string   Dev_Step_3="21,12";
...
input ENUM_APPLIED_PRICE bb_price = PRICE_OPEN; // BB Price type
...
int CreateAlert(int id, ENUM_TIMEFRAMES tf, color upColor, color downColor)
{
   ...
   upCondition.Add(new TRUpCondition(_Symbol, tf), false);
   upCondition.Add(new BBUpCondition(_Symbol, tf), false);
   ...
   downCondition.Add(new TRDownCondition(_Symbol, tf), false);
   downCondition.Add(new BBDownCondition(_Symbol, tf), false);
...
class TrensReversStream : public AStream
{
   double _buffer[];
public:
   TrensReversStream(const string symbol, ENUM_TIMEFRAMES timeframe)
      :AStream(symbol, timeframe)
   {
   }
   ...
   bool GetValue(const int period, double &val)
   {
      ...
      double valueBuy = iCustom(_symbol, _timeframe, "TREND-REVERS", Period1, Period2, Period3, 
         Dev_Step_1, Dev_Step_2, Dev_Step_3, Symbol_1_Kod, Symbol_2_Kod, Symbol_3_Kod, 4, period);
      if (valueBuy != 0 && valueBuy != EMPTY_VALUE)
      {
         _buffer[index] = 1;
         val = _buffer[index];
         return true;
      }
      double valueSell = iCustom(_symbol, _timeframe, "TREND-REVERS", Period1, Period2, Period3, 
         Dev_Step_1, Dev_Step_2, Dev_Step_3, Symbol_1_Kod, Symbol_2_Kod, Symbol_3_Kod, 5, period);
      if (valueSell != 0 && valueSell != EMPTY_VALUE)
      {
         _buffer[index] = -1;
         val = _buffer[index];
         return true;
...
TrensReversStream* _trStream;

class TRSimpleUpCondition : public ACondition
{
public:
   TRSimpleUpCondition(const string symbol, ENUM_TIMEFRAMES timeframe)
      :ACondition(symbol, timeframe)
   {
   ...
   {
      double val;
      if (!_trStream.GetValue(period, val))
      {
         return false;
      }
      return val == 1;
...
class TRUpCondition : public ACondition
{
public:
   TRUpCondition(const string symbol, ENUM_TIMEFRAMES timeframe)
      :ACondition(symbol, timeframe)
   {
...
class TRDownCondition : public ACondition
{
public:
   TRDownCondition(const string symbol, ENUM_TIMEFRAMES timeframe)
      :ACondition(symbol, timeframe)
   {
   ...
   bool IsPass(const int period, const datetime date)
   {
      for (int i = period; i < iBars(_symbol, _timeframe) - 1; ++i)
      {
         double valueBuy = iCustom(_symbol, _timeframe, "TREND-REVERS", Period1, Period2, Period3, 
            Dev_Step_1, Dev_Step_2, Dev_Step_3, Symbol_1_Kod, Symbol_2_Kod, Symbol_3_Kod, 4, i);
         if (valueBuy != 0 && valueBuy != EMPTY_VALUE)
         {
            return false;
         }
         double valueSell = iCustom(_symbol, _timeframe, "TREND-REVERS", Period1, Period2, Period3, 
            Dev_Step_1, Dev_Step_2, Dev_Step_3, Symbol_1_Kod, Symbol_2_Kod, Symbol_3_Kod, 5, i);
         if (valueSell != 0 && valueSell != EMPTY_VALUE)
         {
            return true;
         }
...
class BBUpCondition : public ACondition
{
public:
   BBUpCondition(const string symbol, ENUM_TIMEFRAMES timeframe)
      :ACondition(symbol, timeframe)
   {
   ...
   bool IsPass(const int period, const datetime date)
   {
      ...
      return bbValue < iClose(_symbol, _timeframe, period);
   }
};

class BBDownCondition : public ACondition
{
public:
   BBDownCondition(const string symbol, ENUM_TIMEFRAMES timeframe)
      :ACondition(symbol, timeframe)
   {
...
int init()
{
   ...
   double temp = iCustom(NULL, 0, "TREND-REVERS", Period1, Period2, Period3, 
         Dev_Step_1, Dev_Step_2, Dev_Step_3, Symbol_1_Kod, Symbol_2_Kod, Symbol_3_Kod, 0);
   if (GetLastError() == ERR_INDICATOR_CANNOT_LOAD)
   {
      Alert("Please, install the 'TREND-REVERS' indicator");
      return INIT_FAILED;
   }

   IndicatorBuffers(8);
   IndicatorObjPrefix = GenerateIndicatorPrefix("trbbi");
   IndicatorShortName("Trend Reversal + BB");
   ...
   _trStream = new TrensReversStream(_Symbol, (ENUM_TIMEFRAMES)_Period);
