...
input const string indicator_file = "Breakout_Indicator";              // Indicator File:
//---
int  bufferToBuy              = 0;
int  bufferToSell             = 0;
  ...
  public:
    bool evaluate()
    {
        for (int i = 1; i <= candles_back_for_signals; i++) {
            ...
            if (iClose(NULL, 0, 1) > indi && iOpen(NULL, 0, 1) < indi && indi != lastSignal) {
                lastSignal = indi;
                return true;
            }
        }
...
//Available @  https://fxcodebase.com/code/viewtopic.php?f=38&t=75453
