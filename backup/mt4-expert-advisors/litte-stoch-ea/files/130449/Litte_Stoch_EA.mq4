...
input bool use_stoch_filter = true; // Use stochastic filter
input int K_periods = 5; // Stochastic K periods
input int D_periods = 3; // Stochastic D periods
input int Slowing = 3; // Stochastic Slowing
input int AveragePeriod = 3; // Averages period

input bool use_macd_filter = true; // Use MACD filter
input int FastEMAPeriod = 12; // MACD Fast EMA period
input int SlowEMAPeriod = 26; // MACD Slow EMA period
...
public:
   LongCondition(const string symbol, ENUM_TIMEFRAMES timeframe)
      :ACondition(symbol, timeframe)
   {
      IStream* stoch = new StochasticStream(symbol, timeframe, K_periods, D_periods, Slowing, 1);
      _stream = new SmaOnStream(stoch, AveragePeriod);
      stoch.Release();
   }
   ...
   {
      double value;
      if (!_stream.GetValue(period, value))
         return false;
      return value > 50;
   }
};
...
ICondition* CreateLongCondition(string symbol, ENUM_TIMEFRAMES timeframe)
{
   ...
   condition.Add(new PSARBelowPriceCondition(symbol, timeframe, Step, Max), false);
...
ICondition* CreateLongFilterCondition(string symbol, ENUM_TIMEFRAMES timeframe)
{
   ...
   if (use_macd_filter)
   {
      condition.Add(new MACDAboveLevelCondition(symbol, timeframe, K_periods, D_periods, Slowing, 1, 0), false);
   }
   if (use_stoch_filter)
   {
...
ICondition* CreateShortCondition(string symbol, ENUM_TIMEFRAMES timeframe)
{
   ...
   AndCondition* condition = new AndCondition();
   condition.Add(new PSARAbovePriceCondition(symbol, timeframe, Step, Max), false);
   #ifdef ACT_ON_SWITCH_CONDITION
      ActOnSwitchCondition* switchCondition = new ActOnSwitchCondition(symbol, timeframe, (ICondition*) condition);
      condition.Release();
      return switchCondition;
...
ICondition* CreateShortFilterCondition(string symbol, ENUM_TIMEFRAMES timeframe)
{
   ...
   if (use_macd_filter)
   {
      condition.Add(new MACDBelowLevelCondition(symbol, timeframe, K_periods, D_periods, Slowing, 1, 0), false);
   }
   if (use_stoch_filter)
   {
