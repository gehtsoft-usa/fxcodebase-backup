...
//https://fxcodebase.com/code/viewtopic.php?f=38&t=71172
...
input string precision_trend_grouop = ""; // Precision Trend Settings
input int pt_period = 30; // Average Period
input double pt_sensitivity = 3.0; // Sensitivity

input string supertred_group = ""; // Supertrend Settings
input int                st_period       = 10; // Period
input ENUM_APPLIED_PRICE st_appliedPrice = PRICE_CLOSE; // Applied Price
input double             st_multiplier   = 3.0; // Multiplier

input	string	__________FreeScalper_Setting__________ = ""; // FreeScalper Setting
input	int			FreeScalper_Length = 15; // Length

input	string							__________RSIFilter_Setting__________ = ""; // RSIFilter Setting
input	int									RSIFilter_RSIPeriod = 8; // Period
input	ENUM_APPLIED_PRICE	RSIFilter_RSIPrice = PRICE_TYPICAL; // Price

input	string	__________BBStops_Setting__________ = ""; // BBStops Setting
input	int			BBStops_BandsPeriod = 21; // Period
input	double	BBStops_BandsDeviation = 1.0; // Deviations

input	string	__________HeikenAshi_Setting__________ = ""; // HeikenAshi Setting
input	int			HeikenAshi_SmoothingMethod = 24; // Smoothing Method
input	int			HeikenAshi_SmoothingPeriod = 14; // Smoothing Period

input	string							__________MAAngle_Setting__________ = ""; // MAAngle Setting
input	ENUM_MA_METHOD			MAAngle_MAMode = MODE_SMMA; // MA Mode
input	int									MAAngle_MAPeriod = 20; // MA Period
input	ENUM_APPLIED_PRICE	MAAngle_Price = PRICE_CLOSE; // Price
input	double							MAAngle_AngleTreshold = 0; // Angle Threshold
input	int									MAAngle_StartMAShift = 6; // Start MA Shift
input	int									MAAngle_EndMAShift = 0; // End MA Shift
...
public:
   ...
   {
      int count = 0;
      double valUp_FreeScalper = iCustom(_symbol, _timeframe, "freescalperindicator", FreeScalper_Length, 0, period);
      if ((valUp_FreeScalper != EMPTY_VALUE) && (valUp_FreeScalper > 0))
      {
         count += 1;
      }
      double valUp_PrecisionTrend = iCustom(_symbol, _timeframe, "precision trend 2.2 (histo+alerts+lines)", _timeframe, pt_period, pt_sensitivity, 0, period);
      if ((valUp_PrecisionTrend != EMPTY_VALUE) && (valUp_PrecisionTrend > 0))
      {
         count += 1;
      }
      double valUp_SuperTrend = iCustom(_symbol, _timeframe, "supertrend_nrp_new_mtf_2_histo", st_period, st_appliedPrice, st_multiplier, 0, period);
      if ((valUp_SuperTrend != EMPTY_VALUE) && (valUp_SuperTrend > 0))
      {
         count += 1;
      }
      double valUp_RSIFilter = iCustom(_symbol, _timeframe, "RSI Filter v1.32", _timeframe, RSIFilter_RSIPeriod, RSIFilter_RSIPrice, 0, period);
      if ((valUp_RSIFilter != EMPTY_VALUE) && (valUp_RSIFilter > 0))
      {
         count += 1;
      }
      double valUp_BBStops = iCustom(_symbol, _timeframe, "BB stops (new format - histo) 1.3", PERIOD_CURRENT, BBStops_BandsPeriod, BBStops_BandsDeviation, 0, period);
      if ((valUp_BBStops != EMPTY_VALUE) && (valUp_BBStops > 0))
      {
         count += 1;
      }
      double valUp_HeikenAshi = iCustom(_symbol, _timeframe, "heiken ashi averages histo", PERIOD_CURRENT, false, HeikenAshi_SmoothingMethod, HeikenAshi_SmoothingPeriod, 0, period);
      if ((valUp_HeikenAshi != EMPTY_VALUE) && (valUp_HeikenAshi > 0))
      {
         count += 1;
      }
      double valUp_MAAngle = iCustom(_symbol, _timeframe, "MAAngle_alerts_v1", MAAngle_MAMode, MAAngle_MAPeriod, MAAngle_Price, MAAngle_AngleTreshold, MAAngle_StartMAShift, MAAngle_EndMAShift, 0, period);
      if ((valUp_MAAngle != EMPTY_VALUE) && (valUp_MAAngle > 0))
      {
         count += 1;
      }
...
public:
   ...
   virtual ICell* Create(const string id, const int x, const int y, ENUM_BASE_CORNER corner, const string symbol, const ENUM_TIMEFRAMES timeframe, bool showHistorical)
   {
      ...
      ICondition* downCondition = new NotCondition(upCondition);
