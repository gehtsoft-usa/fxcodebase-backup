...
void TrendControl()
{
    ...
    if((openBuys + openSells) == 0)
        limitTrades.ResetCounter();
...
double distanceBtwPrices()
{
    return fabs(allPrices[1] - allPrices[0]);
}

double UpPrice()
{
    takeAllPrices();
    for(int i = 1; i < ArraySize(allPrices); i++) {
...
{
    takeAllPrices();
    for(int i = 0; i < ArraySize(allPrices); i++) {
    ...
    bool evaluate()
    {
        ...
        for(int i=OrdersTotal()-1;i>=0;i--)
        {
           if(OrderSelect(i,SELECT_BY_POS) && OrderSymbol() == _Symbol && OrderMagicNumber()== magico)
           {
               ...
               if(Ask < OrderOpenPrice() + (distanceBtwPrices()*0.50) && Ask > OrderOpenPrice() - (distanceBtwPrices()*0.50))
                   return false;
    ...
    bool evaluate()
    {
        ...
        for(int i = OrdersTotal() - 1; i >= 0; i--)
        {
                ...
                && (OrderType() == OP_BUY || OrderType() == OP_SELL))
            {
                ...
                Print(__FUNCTION__, " _countAcum: ", _countAcum);
                Print(__FUNCTION__, " count: ", _count);
...
double Price(string direction, int pips = 0, string _symbol = "")
{
    ...
    if(direction == "buy")
    {
        ...
        if(modeEntry == PriceLine)
        {
            ...
            if(mode == oneSide && direction == 1)
                price = Ask;

                Print(__FUNCTION__, " BUY: price: ", price);
        }
    ...
    if(direction == "sell")
    {
        ...
        if(modeEntry == PriceLine)
        {
            ...
            Print(__FUNCTION__, " SELL: price: ", price);
        }
