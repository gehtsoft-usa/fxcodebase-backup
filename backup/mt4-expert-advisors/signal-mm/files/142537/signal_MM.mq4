...
//https://fxcodebase.com/code/viewtopic.php?f=38&t=71277
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      double ma1 = iMA(_symbol, _timeframe, ma1_period, 0, ma1_method, PRICE_CLOSE, period);
      double ma2 = iMA(_symbol, _timeframe, ma2_period, 0, ma2_method, PRICE_CLOSE, period);
      double open = iOpen(_symbol, _timeframe, period);
      double close = iClose(_symbol, _timeframe, period);
      return open < ma1 
         && open < ma2
         && close > ma1 
...
int init()
{
   ...
   IndicatorBuffers(8);
   IndicatorObjPrefix = GenerateIndicatorPrefix("signal MM");
   IndicatorShortName("signal MM");
   mainSignaler = new Signaler();
   mainSignaler.SetMessagePrefix(_Symbol + "/" + TimeframeToString((ENUM_TIMEFRAMES)_Period) + ": ");

   int id = 0;

   SetIndexBuffer(id, avg1);
   ++id;
   SetIndexBuffer(id, avg2);
   ++id;
...
int start()
{
   ...
   {
      ArrayInitialize(avg1, EMPTY_VALUE);
      ArrayInitialize(avg2, EMPTY_VALUE);
      if (customStream != NULL)
      {
         customStream.Init();
      }
   ...
   {
      avg1[pos] = iMA(_Symbol, 0, ma1_period, 0, ma1_method, PRICE_CLOSE, pos);
      avg2[pos] = iMA(_Symbol, 0, ma2_period, 0, ma2_method, PRICE_CLOSE, pos);
      if (customStream != NULL)
      {
