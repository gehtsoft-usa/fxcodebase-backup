...
bool DefineAndShowDivergence(ENUM_EXTREMUM_TYPE extremumType, int rightBarIndex, int leftBarIndex, int priceRightBarIndex, int priceLeftBarIndex)
{
   ...
   ResetLastError();
   string id = IndicatorObjPrefix + TimeToString(Time[leftBarIndex]);
   double price = code == 218 ? High[priceRightBarIndex] : Low[priceRightBarIndex];
   if (ObjectFind(0, id) == -1)
   {
      if (!ObjectCreate(0, id, OBJ_TEXT, 0, Time[priceRightBarIndex], price))
      {
   ...
   ObjectSetInteger(0, id, OBJPROP_TIME, Time[priceRightBarIndex]);
