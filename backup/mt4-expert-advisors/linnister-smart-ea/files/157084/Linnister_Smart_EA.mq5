...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=75312
...
input const string indicator_file = "Alpha Trend";                          // Indicator File:
input bool alpha_trend_on = true; // Alpha Trend Strategy On?
//---
...
input const string indicator_file2 = "Extreme_Spike";                          // Indicator File:
input bool extreme_spike_on = true; // Extreme Spike Strategy On?
//---
int  bufferToBuy2              = 1;
int  bufferToSell2             = 0;
int  candles_back_for_signals2 = 1;
//---
int  handle2 = 0;
void setHandle_ExtremeSpike() { handle2 = iCustom(Symbol(), NULL, indicator_file2); }

double Indi_ExtremeSpike(int buffer, int candle = 1)
{
    double value[1];
    int    copy = CopyBuffer(handle2, buffer, candle, 1, value);
...
class ConditionIndicatorBuy_extreme_spike : public iConditions
{
    double lastSignal;
  ...
  public:
    bool evaluate()
    {
        for (int i = 1; i <= candles_back_for_signals2; i++) {
            double indi = Indi_ExtremeSpike(bufferToBuy2, i);
...
ConditionIndicatorBuy_extreme_spike conditionIndicatorBuy_extreme_spike;

class ConditionIndicatorSell_extreme_spike : public iConditions
{
    double lastSignal;
  ...
  public:
    ...
    {
        for (int i = 1; i <= candles_back_for_signals2; i++) {
            double indi = Indi_ExtremeSpike(bufferToSell2, i);
            if (indi > 0 && indi != EMPTY_VALUE) {
                lastSignal = indi;
                return true;
            }
...
ConditionIndicatorSell_extreme_spike conditionIndicatorSell_extreme_spike;

Strategy LongStrategy_extreme_spike;
Strategy ShortStrategy_extreme_spike;
...
bool OnInit_CustomIndicator()
{
    ...
    setHandle_ExtremeSpike();
    if(extreme_spike_on){
        LongStrategy_extreme_spike.set(&conditionIndicatorBuy_extreme_spike, &buy);
        ShortStrategy_extreme_spike.set(&conditionIndicatorSell_extreme_spike, &sell);
    }
...
input bool   close_on_opposite_by_alpha         = false;                     // Close On Opposite Signal Alpha Trend:
input bool   close_on_opposite_by_extreme_spike = false;                     // Close On Opposite by Extreme Spike:

ConditionIndicatorSell cd_close_buy_by_alpha;
ConditionIndicatorBuy  cd_close_sell_by_alpha;
ConditionIndicatorSell_extreme_spike cd_close_buy_extreme_spike;
ConditionIndicatorBuy_extreme_spike  cd_close_sell_extreme_spike;

Strategy closeOppositeBuy_alpha; 
Strategy closeOppositeSell_alpha;
Strategy closeOppositeBuy_extreme_spike; 
Strategy closeOppositeSell_extreme_spike;

void oninit_close_on_opposite()
{
    closeOppositeBuy_alpha.set(&cd_close_buy_by_alpha, &acCloseBuy);
    closeOppositeSell_alpha.set(&cd_close_sell_by_alpha, &acCloseSell);
    closeOppositeBuy_extreme_spike.set(&cd_close_buy_extreme_spike, &acCloseBuy);
    closeOppositeSell_extreme_spike.set(&cd_close_sell_extreme_spike, &acCloseSell);
    
    if (close_on_opposite_by_alpha) {
        Trader.management.add(&closeOppositeBuy_alpha);
        Trader.management.add(&closeOppositeSell_alpha);
    }
    
    if (close_on_opposite_by_extreme_spike) {
        Trader.management.add(&closeOppositeBuy_extreme_spike);
        Trader.management.add(&closeOppositeSell_extreme_spike);
    }
}
