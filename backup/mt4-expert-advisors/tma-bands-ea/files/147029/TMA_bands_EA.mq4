...
// ------------------------------------------------------------------
input uint       uInpPeriodTMA     = 56;         // TMA period
input BANDS_MODE ubands_mode       = PIPS_MODE;  // Bands Mode:
input uint       uInpPeriodATR     = 100;        // ATR period
input double     uInpMultiplierATR = 2.0;        // ATR multiplier
input int        uInpPipsDistance  = 12;         // PIPS Distance
input double     uInpThreshold     = 0.5;        // Trend threshold
// ------------------------------------------------------------------
input string       T0                      = "== Trade Setup ==";  // == Trade Setup ==
input int          MaxNumberOfPositions    = 8;                    // Max Number of positions in any direction:
input double       uMaxDailyLoss           = 1.8;                  // Max Daily Loss Balance %
input ModeCalcLots modeCalcLots            = FixLots;              // Mode to Calc Lots:
double       userMoney               = 10;                   // Setup Lots by "Money":
double       userBalancePerTotal     = 2;                    // Setup Lots by "Account Percent" %:
double             userBalancePer          = userBalancePerTotal / MaxNumberOfPositions;
...
input CloseAllMode closeBy                 = CloseByAccountPercent;      // Close All Mode:
input double       closeAllMoney           = 100;                        // Close by Money $ (zero to off):
input double       closeAllMoneyLoss       = -100;                       // Close by Money Lossing $ (zero to off):
...
int                GridUser_maxCount       = MaxNumberOfPositions;       // Max attempts:
double             GridUser_maxLot         = 1000;                       // Max lot value:
input double       GridUser_multiplier     = 1;                          // Lot Multiplier:
input int          GridUser_gap            = 30;                         // Gap betwen orders (pips):
bool               closeGridOn             = false;                      // Use Close Grid?
...
class ByPercent : public iLevels
{
  string _symbol;
  string _side;
  int    _pips;
 ...
 public:
  ByPercent(string inpSymbol, string inpSide, double inpPercent, string inpMode)
  {
    _percent = inpPercent;
    // _pips   = inpPips;
    _symbol = inpSymbol;
  ...
  double calculateLevel()
  {
    ...
    double bid      = SymbolInfoDouble(_symbol, SYMBOL_BID);
    double mPoint   = MarketInfo(_symbol, MODE_POINT);
    double newPrice = NormalizeDouble(ask * (1 + _percent / 100), _Digits);
    double distance = NormalizeDouble(fabs(newPrice - ask), _Digits);
    _pips           = distance / (_Point * 10);
    double result   = 0;
...
ConcurrentConditions conditionsToTrade;
...
ConditionCountOrders* countOpenTrades;

class ProfitCondition : public iConditions
{
  int    _magic;
  double _limit;

 public:
  ProfitCondition(double limit, int mag) : _limit(limit), _magic(mag) { ; }
  ~ProfitCondition() { ; }

  bool evaluate()
  {
    // TODO: Profit condition
    if (TodayProfit() >= _limit)
    {
      return false;
    }
    return true;
...
ProfitCondition* conditionDailyLoss;
...
class BUYcondition1 : public iConditions
{
 public:
  bool evaluate()
  {
    ...
    double Signal = iCustom(NULL, 0, "TMA_bands.ex4", uInpPeriodTMA, ubands_mode, uInpPeriodATR, uInpMultiplierATR, uInpPipsDistance, uInpThreshold, 6, 0);
...
class SELLcondition1 : public iConditions
{
 public:
  bool evaluate()
  {
    ...
    double Signal = iCustom(NULL, 0, "TMA_bands", uInpPeriodTMA, ubands_mode, uInpPeriodATR, uInpMultiplierATR, uInpPipsDistance, uInpThreshold, 7, 0);
    if (Signal != EMPTY_VALUE)
    {
      return true;
    }
...
int OnInit()
{
  ...
  conditionsToTrade.AddCondition(countOpenTrades = new ConditionCountOrders(MaxNumberOfPositions, magico));

  double moneyDailyLimit = AccountInfoDouble(ACCOUNT_BALANCE) * uMaxDailyLoss / 100;
  conditionsToTrade.AddCondition(conditionDailyLoss = new ProfitCondition(moneyDailyLimit, magico));
  ...
  // conditionsToBuy.AddCondition(countBuys = new ConditionCountBuys(MaxNumberOfPositions));
  ...
  // conditionsToSell.AddCondition(countSells = new ConditionCountSells(MaxNumberOfPositions));
...
void OnTick()
{
  ...
  // ------------------------------------------------------------------
  if (conditionsToTrade.EvaluateConditions())
  {
  ...
  if (stopLossOn)
    switch (modeSL)
    {
      ...
      case PipsFromOpenCandle:
        ...
        break;
      case bypercent:
        Print("Tomé el BY PERCENT SL");
        levelSL = new Levels(new ByPercent(_Symbol, side, uPercent_SL, "SL"));
        Print("levelSL: ", levelSL);
        result = levelSL.calculateLevel();
        Print("result: ", result);
        break;
    }
  ...
  if (takeProfitOn)
    switch (modeTP)
    {
      ...
      case PipsFromOpenCandle:
        ...
        break;
      case bypercent:
        Print("Tomé el BY PERCENT TP");
        levelTP = new Levels(new ByPercent(_Symbol, side, uPercent_TP, "TP"));
        Print("levelTP: ", levelTP);
        result = levelTP.calculateLevel();
        Print("result: ", result);
        break;
    }
...
double Lots()
{
  ...
  {
    // case Money:
		//   if (CheckPointer(levelSL) == POINTER_INVALID)
    //   {
    //     Alert("To use Lots By Money set the Stop Loss");
    //     break;
    //   }
    ...
    case AccountPercent:
      if (CheckPointer(levelSL) == POINTER_INVALID)
      {
        Alert("To use Lots By Account Percent set the Stop Loss");
        break;
      }
