...
// https://fxcodebase.com/code/viewtopic.php?f=38&p=149472#p149472
...
input int                maxLookback  = 500;   // Maximum lookback period for the indicator
input bool               ma_Filter1   = false;
input int                ma_Period1   = 200;
input ENUM_MA_METHOD     ma_Mode1     = MODE_EMA;
input ENUM_APPLIED_PRICE ma_Price1    = PRICE_CLOSE;
input bool               ma_Filter2   = false;
input int                ma_Period2   = 800;
input ENUM_MA_METHOD     ma_Mode2     = MODE_EMA;
         ...
         for(j = 0; j < ArraySize(pairList); j++)
           {
            ...
            string label  = (val0 == 1) ? "Up" : (val0 == -1) ? "Down" :
                            "-";
            state        += (label == "Up") ? "b" : (label == "Down") ? "s" :
                            "-";
            color  clr    = (val0 == 1) ? Confirmed_Up_Color : (val0 == -1) ? Confirmed_Dn_Color :
                            Neutral_Color;
...
//|                                                                  |
//+------------------------------------------------------------------+
int indCalc(string sym, ENUM_TIMEFRAMES tf)
  {
   int dir = 0;
#define PARAM sym, tf, "TREND-REVERS", 0, 0, Period3, "", "", Dev_Step_3
   for(int i = 0; i < MathMin(iBars(sym, tf) - 2, maxLookback); i++)
     {
      if(iCustom(PARAM, 4, i) > 0 && iCustom(PARAM, 4, i) != EMPTY_VALUE)
        {
         dir = 1;
         break;
        }
      if(iCustom(PARAM, 5, i) > 0 && iCustom(PARAM, 5, i) != EMPTY_VALUE)
        {
         dir = -1;
         break;
        }
   ...
   if(dir == 1)
     {
      if((ma_Filter1 && iMA(sym, tf, ma_Period1, 0, ma_Mode1, ma_Price1, 0) < iClose(sym, tf, 0)) || !ma_Filter1)
         dir = 1;
      else
         return 0;
      if((ma_Filter2 && iMA(sym, tf, ma_Period2, 0, ma_Mode2, ma_Price2, 0) < iClose(sym, tf, 0)) || !ma_Filter2)
         dir = 1;
      else
         return 0;
      if((bb_Filter && iBands(sym, tf, bb_Period, bb_Deviation, 0, bb_Price, MODE_MAIN, 0) < iClose(sym, tf, 0)) || !bb_Filter)
         dir = 1;
      else
         return 0;
     }
...
void Notify(int type)
  {
   ...
   switch(type)
     {
      case 1:
         text += " UP / DOWN states changes before bar closes - " + _Symbol + " " + GetTimeFrame(_Period);
         break;
      case 11:
         text += " UP / DOWN states changed after bar closed - " + _Symbol + " " + GetTimeFrame(_Period);
         break;
     }
   text += " ";
