...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=73895
...
input bool magicfilterOn = true; // Magic Filter On:
...
input closeAllBy maxDailyPLCalc = 0;         // Max daily PL calculation mode
input double maxDailyLoss = 0;               // Max loss per day (0=unlimited)
input double maxDailyWin = 0;                // Max win per day (0=unlimited)
#ifdef SPREAD_FILTER
   ...
   if(dir == 1 || dir == 2)
     {
      ...
      if(lotMode == 1)
        {
         risk =  AccountInfoDouble(ACCOUNT_EQUITY) * LotVol / 100;
         final_lot = risk / distance / tickValue;
        }
      if(lotMode == 2)
        {
         risk =  AccountInfoDouble(ACCOUNT_BALANCE) * LotVol / 100;
         final_lot = risk / distance / tickValue;
        }
      if(lotMode == 3)
        {
      ...
      if(lotMode == 4)
        {
         final_lot =  LotVol / marginReq;
        }
      if(lotMode == 5)
        {
         final_lot = (AccountInfoDouble(ACCOUNT_EQUITY) * LotVol / 100) / marginReq;
        }
     }
   ...
   for(int pos = OrdersTotal() - 1; pos >= 0 ; pos--)
     {
      ...
      int dir = OrderType() == 0 ? 1 : 2;
      double newLot = calcLot(dir, OrderSymbol(), SL);
      if(currentLot != newLot)
        {
         if(dir == 1)
           {
            succ = OrderClose(ticket, currentLot, Bid, 1000);
            if(succ)
               OrderSend(sym, OP_BUY, newLot, Ask, 1000, SL, TP, "modified", magico);
           }
         if(dir == 2)
           {
            succ = OrderClose(ticket, currentLot, Ask, 1000);
            if(succ)
               OrderSend(sym, OP_SELL, newLot, Bid, 1000, SL, TP, "modified", magico);
           }
        }
     }
  }
...
// ——————————————————————————————————————————————————————————————————
   ...
   if(maxDailyPLCalc > 0)
     {
      double PL;
      PL += PLOfPositionsHistory(maxDailyPLCalc, 0, Symbol(), iTime(Symbol(), PERIOD_D1, 0));
      PL += PLOfPositions(maxDailyPLCalc, 0, Symbol());
      Print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! "+PL);
      if(PL >= maxDailyWin || PL <= maxDailyLoss * (-1))
        {
         closeAllPos(0,Symbol());
         return;
        }
     }
   ...
   if(OrderSelect(tk, SELECT_BY_TICKET))
     {
      ...
      double newSl = SL(side, OrderOpenPrice());
      double newTp = TP(side, OrderOpenPrice());
      r = OrderModify(tk, OrderOpenPrice(), newSl, newTp, OrderExpiration(), clrNONE);
     }
   return r;
  }
   ...
   for(int pos = OrdersTotal() - 1; pos >= 0 ; pos--)
     {
      ...
      if((ordersym == sym || sym == "") && ((OrderMagicNumber() == (int)magico && magico > 0) || magico == 0))
        {
         if(OrderType() == OP_BUY)
            if(dir == 1 || dir == 0)
              {
   ...
   for(int pos = OrdersHistoryTotal() - 1; pos >= 0 ; pos--)
     {
      ...
      string comment = OrderComment();
      if((ordersym == sym || sym == "") && ((OrderMagicNumber() == (int)magico && magico > 0) || magico == 0) && openTime >= from)
        {
         if(OrderType() == OP_BUY)
            if(dir == 1 || dir == 0)
              {
...
//|                                                                  |
//+------------------------------------------------------------------+
void closeAllPos(int dir, string sym)
  {
   int succ;
   string commentPart = "";
   for(int pos = OrdersTotal() - 1; pos >= 0 ; pos--)
     {
      ...
      RefreshRates();
      ResetLastError();
      if((symbol == sym || sym == "") && ((OrderMagicNumber() == (int)magico && magico > 0) || magico == 0))
        {
