...
//http://fxcodebase.com/code/viewtopic.php?f=38&t=70743
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      return iClose(_symbol, _timeframe, period) > iHigh(_symbol, _timeframe, period) 
         && iClose(_symbol, _timeframe, period - 1) <= iHigh(_symbol, _timeframe, period - 1);
   }
};
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      return iClose(_symbol, _timeframe, period) < iLow(_symbol, _timeframe, period) 
         && iClose(_symbol, _timeframe, period - 1) >= iLow(_symbol, _timeframe, period - 1);
   }
};
...
int init()
{
   ...
   IndicatorObjPrefix = GenerateIndicatorPrefix("dncs");
   ...
   }
   {
      ...
      id = CreateAlert(id, upCondition, NULL, 217, "Up " + TimeframeToString((ENUM_TIMEFRAMES)_Period), up_color, PriceHigh, 1);
      id = CreateAlert(id, downCondition, NULL, 218, "Down " + TimeframeToString((ENUM_TIMEFRAMES)_Period), down_color, PriceLow, -1);
      upCondition.Release();
