...
//https://fxcodebase.com/code/viewtopic.php?f=38&t=76105
...
#property description "This EA request to have the indicator in the chart"
// #property description "in the folder: MQL4/Indicators"
// ------------------------------------------------------------------

input string       Tcustom        = "==== Indicator Setup ====";           // ————————————————————————
input const string indicator_file = "LuxAlgo - Swing Failure Pattern.ex4"; // Indicator File:
//---
int  bufferToBuy              = 0;
int  bufferToSell             = 1;
...
bool ArrowUpInLastClosedCandle()
{
    datetime lastCandleTime = iTime(NULL, 0, 1); // Última vela cerrada
    int      total          = ObjectsTotal();

    for (int i = total-1; i >= 0; i--) {
        string name = ObjectName(i);
        if (StringFind(name, "Swing Failure", 0) >= 0) // Comienza con "Swing Failure"
        {
            datetime objTime = ObjectGet(name, OBJPROP_TIME1);
            if (objTime < lastCandleTime) continue; // Si el tiempo del objeto es anterior al de la última vela cerrada, lo ignoramos
            string tx = ObjectGetString(0, name, OBJPROP_TEXT);
            if (tx == "é") {
                return true;
...
bool ArrowDnInLastClosedCandle()
{
    datetime lastCandleTime = iTime(NULL, 0, 1); // Última vela cerrada
    int      total          = ObjectsTotal();
...
class ConditionIndicatorBuy : public iConditions
{
  ...
  public:
    bool evaluate()
    {
            return ArrowUpInLastClosedCandle();
    }
};
...
class ConditionIndicatorSell : public iConditions
{
  ...
  public:
    bool evaluate()
    {
        return ArrowDnInLastClosedCandle();
    }
};
