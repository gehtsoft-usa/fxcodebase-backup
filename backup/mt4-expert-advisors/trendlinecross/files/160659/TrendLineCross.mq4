...
Version:     
Date:        
Repository:  Available @ https://fxcodebase.com/code/viewtopic.php?f=38&t=76333
License:     GNU
*/
...
{
    ArrowUp[1]=EMPTY_VALUE;
    ArrowDn[1]=EMPTY_VALUE;

    if (PriceCrossedTrendline() == 1) {
        ArrowUp[1] = Low[1];
...
int PriceCrossedTrendline(int shift = 0)
{
    ...
    for (int i = 0; i < total; i++) {
        ...
        if (ObjectGetInteger(0, objName, OBJPROP_TYPE) == OBJ_TREND) {
            trendlineName = objName;
        }
        if(trendlineName!="")
        {
            double TrendPrice = ObjectGetValueByShift(trendlineName, 1);
            double closeNow  = iClose(Symbol(), 0, 1);
            double closePrev = iClose(Symbol(), 0, 2);
            if (closeNow > closePrev) {
                if (closePrev < TrendPrice && closeNow > TrendPrice) return 1;
            }
            if (closeNow < closePrev) {
                if (closePrev > TrendPrice && closeNow < TrendPrice) return -1;
            }
