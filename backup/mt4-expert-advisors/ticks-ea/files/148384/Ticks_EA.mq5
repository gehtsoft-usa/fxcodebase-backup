...
// http://fxcodebase.com/code/viewtopic.php?f=38&t=71038
...
input int ticks_threshold = 5; // Ticks threshold
input int seconds = 1; // Seconds
...
input bool OrderFilling = false;             // Order filling (enable if get "Unsupported filling mode" error)
...
double ticks_history[];
datetime ticks_history_date[];
...
public:
   ...
   virtual bool IsPass(const int period, const datetime date)
   {
      if (ArraySize(ticks_history) == 0)
      {
         return false;
      }
      double point = SymbolInfoDouble(_Symbol, SYMBOL_POINT);
      return (iClose(_symbol, _timeframe, 0) - ticks_history[0]) / point >= ticks_threshold;
   }
};
...
public:
   ...
   virtual bool IsPass(const int period, const datetime date)
   {
      ...
      double point = SymbolInfoDouble(_Symbol, SYMBOL_POINT);
      return (ticks_history[0] - iClose(_symbol, _timeframe, 0)) / point >= ticks_threshold;
   }
};
...
{
   int toRemove = 0;
   datetime currentTime = (datetime)SymbolInfoInteger(Symbol(), SYMBOL_TIME);
   for (int i = 0; i < ArraySize(ticks_history_date); ++i)
   {
      if (currentTime - ticks_history_date[i] > seconds)
      {
         toRemove++;
      }
      else
      {
         ticks_history_date[i - toRemove] = ticks_history_date[i];
         ticks_history[i - toRemove] = ticks_history[i];
      }
   }
   int size = ArraySize(ticks_history);
   ArrayResize(ticks_history, size - toRemove + 1);
   ArrayResize(ticks_history_date, size - toRemove + 1);
   ticks_history[size - toRemove] = iClose(_Symbol, (ENUM_TIMEFRAMES)_Period, 0);
   ticks_history_date[size - toRemove] = currentTime;
