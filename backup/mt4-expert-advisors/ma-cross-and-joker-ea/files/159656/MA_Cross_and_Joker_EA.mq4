...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=76050
...
#define Section_Ema_Cross
#ifdef Section_Ema_Cross
// Mark: Section_Ema_Cross

input string             ma_title          = "== Moving Average 1 Setup =="; // ————————————————————————
input int                ma1_period        = 10;                             // Periods:
...
input int                ma2_period        = 20;                             // Periods:
...
input string maContext_title   = "== Moving Average Context Setup =="; // ————————————————————————
input bool   maContext_on      = true;                                 // Context MA On
input int    maContext_period1 = 20;                                   // Periods:
input int    maContext_period2 = 34;                                   // Periods:
input int    maContext_period3 = 55;                                   // Periods:
input int    maContext_period4 = 100;                                  // Periods:

double ma3(int shift) { return iMA(NULL, 0, maContext_period1, 0, MODE_EMA, PRICE_CLOSE, shift); }
double ma4(int shift) { return iMA(NULL, 0, maContext_period2, 0, MODE_EMA, PRICE_CLOSE, shift); }
double ma5(int shift) { return iMA(NULL, 0, maContext_period3, 0, MODE_EMA, PRICE_CLOSE, shift); }
double ma6(int shift) { return iMA(NULL, 0, maContext_period4, 0, MODE_EMA, PRICE_CLOSE, shift); }

class ConditionMaCrossUp : public iConditions
{
  public:
    bool evaluate() { return ma1(1) > ma2(1) && ma1(2) <= ma2(2); }
};
ConditionMaCrossUp cdMaCrossUp;

class ConditionMaCrossDn : public iConditions
{
  public:
    bool evaluate() { return ma1(1) < ma2(1) && ma1(2) >= ma2(2); }
};
ConditionMaCrossDn cdMaCrossDn;

class ConditionContextUp : public iConditions
{
  public:
    bool evaluate() { return (ma3(1) > ma4(1) && ma4(1) > ma5(1) && ma5(1) > ma6(1)); }
};
ConditionContextUp cdContextUp;
class ConditionContextDn : public iConditions
{
  public:
    bool evaluate() { return (ma3(1) < ma4(1) && ma4(1) < ma5(1) && ma5(1) < ma6(1)); }
};
ConditionContextDn cdContextDn;

// void MaCrossStrategy_Iniciate()
// {
//     LongStrategy.set(&cdMaCrossUp, &buy);
//     ShortStrategyMaCross.set(&cdMaCrossDn, &sell);
//     if (maContext_on) {
//         LongStrategyMaCross.addCondition(&cdContextUp);
//         ShortStrategyMaCross.addCondition(&cdContextDn);
//     }
//     Trader.strategys.add(&LongStrategyMaCross);
//     Trader.strategys.add(&ShortStrategyMaCross);
// }
...
{
    LongStrategy.set(&cdMaCrossUp, &buy);
    ShortStrategy.set(&cdMaCrossDn, &sell);

    if (maContext_on) {
        LongStrategy.addCondition(&cdContextUp);
        ShortStrategy.addCondition(&cdContextDn);
    }
...
input const string    indicator_file = "JokerFilter";               // Joker File:
input bool            indicator_on   = true;                        // Joker On/Off
input ENUM_TIMEFRAMES indicator_tf   = PERIOD_M5;                   // Joker TimeFrame:
input int             smooth_period  = 5;                           // Joker Smooth Period:
input int             phase_period   = 0;                           // Joker Phase Period:
input bool            indicator2_on   = true;                        // Joker2 On/Off
input ENUM_TIMEFRAMES indicator2_tf   = PERIOD_M15;                   // Joker2 TimeFrame:
//---
int  bufferToBuy              = 0;
int  bufferToSell             = 1;
...
//---
double Indi(int buffer, int candle = 1) { return iCustom(NULL, indicator_tf, indicator_file, smooth_period, phase_period, buffer, candle); }
double Indi2(int buffer, int candle = 1) { return iCustom(NULL, indicator2_tf, indicator_file, smooth_period, phase_period, buffer, candle); }
    ...
    bool evaluate()
    {
        for (int i = 1; i <= candles_back_for_signals; i++) {
            ...
            if (indi > 0 && indi != EMPTY_VALUE) {
                if (indicator2_on) {
                    double indi2 = Indi2(bufferToBuy, i);
                    if (indi2 > 0 && indi2 != EMPTY_VALUE) {
                        return true;
                    }
                } else {
                    return true;
  ...
  public:
    bool evaluate()
    {
        for (int i = 1; i <= candles_back_for_signals; i++) {
            ...
            if (indi > 0 && indi != EMPTY_VALUE) {
                if (indicator2_on) {
                    double indi2 = Indi2(bufferToSell, i);
                    if (indi2 > 0 && indi2 != EMPTY_VALUE) {
                        return true;
                    }
                } else {
