...
// http://fxcodebase.com/code/viewtopic.php?f=38&t=70104
...
input int    Amplitude           = 2.0;  // Amplitude
input ENUM_TIMEFRAMES long_tf = PERIOD_CURRENT; // Long timeframe
...
input TradingSide trading_side = BothSides; // What trades should be taken
   ...
   input bool close_on_opposite_short = true; // Close on opposite signal (short)
   input bool close_on_opposite_long = true; // Close on opposite signal (long)
#else
...
class LongConfirmCondition : public ACondition
{
public:
   LongConfirmCondition(const string symbol, ENUM_TIMEFRAMES timeframe)
      :ACondition(symbol, timeframe)
   {
   ...
   bool IsPass(const int period, const datetime date)
   {
      return iCustom(_symbol, long_tf, "HalfTmtf", "Current time frame", Amplitude, 1, 0) != 0;
   }
};

class ShortConfirmCondition : public ACondition
{
public:
   ShortConfirmCondition(const string symbol, ENUM_TIMEFRAMES timeframe)
      :ACondition(symbol, timeframe)
   {
   ...
   bool IsPass(const int period, const datetime date)
   {
      return iCustom(_symbol, long_tf, "HalfTmtf", "Current time frame", Amplitude, 0, 0) != 0;
   }
};
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      double value0 = iCustom(_symbol, _timeframe, "HalfTmtf", "Current time frame", Amplitude, 1, period);
      double value1 = iCustom(_symbol, _timeframe, "HalfTmtf", "Current time frame", Amplitude, 0, period);
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      double value0 = iCustom(_symbol, _timeframe, "HalfTmtf", "Current time frame", Amplitude, 0, period);
      double value1 = iCustom(_symbol, _timeframe, "HalfTmtf", "Current time frame", Amplitude, 1, period);
      return value0 != 0 && value1 == 0;
   }
};
...
ICondition* CreateLongFilterCondition(string symbol, ENUM_TIMEFRAMES timeframe)
{
   ...
   AndCondition* condition = new AndCondition();
   condition.Add(new LongConfirmCondition(symbol, timeframe), false);
   return condition;
}
...
ICondition* CreateShortFilterCondition(string symbol, ENUM_TIMEFRAMES timeframe)
{
   ...
   AndCondition* condition = new AndCondition();
   condition.Add(new ShortConfirmCondition(symbol, timeframe), false);
   return condition;
}
...
{
   AndCondition* condition = new AndCondition();
   if (close_on_opposite_long)
   {
      condition.Add(new ShortCondition(symbol, long_tf), false);
   }
   if (close_on_opposite_short)
   {
      condition.Add(new ShortCondition(symbol, timeframe), false);
   }
   #ifdef ACT_ON_SWITCH_CONDITION
...
ICondition* CreateExitShortCondition(string symbol, ENUM_TIMEFRAMES timeframe)
{
   ...
   if (close_on_opposite_long)
   {
      condition.Add(new LongCondition(symbol, long_tf), false);
   }
   if (close_on_opposite_short)
   {
      condition.Add(new LongCondition(symbol, timeframe), false);
...
TradingController *CreateController(const string symbol, const ENUM_TIMEFRAMES timeframe, string &error)
{
   ...
   ICloseOnOppositeStrategy* closeOnOpposite = new DontCloseOnOppositeStrategy();
...
int OnInit()
{
   double temp = iCustom(0, 0, "HalfTmtf", "Current time frame", Amplitude, 1, 0);
   if (GetLastError() == ERR_INDICATOR_CANNOT_LOAD)
   {
      Alert("Please, install the 'HalfTmtf' indicator");
      return INIT_FAILED;
   }
   #ifdef SHOW_ACCOUNT_STAT
      stats = NULL;
