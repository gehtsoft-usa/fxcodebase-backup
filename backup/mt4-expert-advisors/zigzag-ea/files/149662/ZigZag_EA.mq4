...
#resource "\\Indicators\\ZIG ZAG NRP.ex4" 
#property strict

string file_custom_indicator = "::Indicators\\ZIG ZAG NRP.ex4";
...
input string Ti       = "== Indicator Setup ==";  // ————————————
input bool            mtfOn    = true;                     // Filter by Major Time Frame:
input ENUM_TIMEFRAMES supTF = PERIOD_H4; // Major TimeFrame:
input int Candle = 480;
input int Length = 4;
...
class BUYcondition1 : public iConditions
{
 public:
  bool evaluate()
  {
	...
	return iHigh(NULL,supTF,1) < iHigh(NULL,supTF,0);
  }
};
...
class BUYcondition2 : public iConditions
{
 public:
  bool evaluate()
  {
	...
	return iHigh(NULL,0,1) < iHigh(NULL,0,0);
	
	  // double indi1 = 0;
    // double indi2 = 0;
    // int    pp    = 0;

    // for (int i = 1; i < 100 && pp <= 1 ; i++)
    // {
    //   double value = iCustom(NULL, 0, file_custom_indicator, Candle, Length, 0, i);
    //   if (value == EMPTY_VALUE || value == 0) continue;
      
		// 	if (indi1 == 0) {
		// 		indi1 = value;
    //     pp++;
	
		//     Print(__FUNCTION__," indi1: ",indi1);
		// 		continue;
	  //     }
		...
		// 		Print(__FUNCTION__," indi2: ",indi2);
		// 		continue;
		// 	}
    // }
    // return indi1 < indi2;
...
class ConditionToCloseBuy : public iConditions
{
 public:
  bool evaluate()
  {
    ...
    {
			if(mtfOn){ return sellCondition1.evaluate() && sellCondition2.evaluate();}
...
class ConditionToCloseSell : public iConditions
{
 public:
  bool evaluate()
  {
    if (closeAllInOpositeSignal)
    {
			if(mtfOn){ return buyCondition1.evaluate() && buyCondition2.evaluate();}
...
//--- get custom indicator value 
  //  double value=iCustom(_Symbol,_Period,"::Indicators\\ZIG ZAG NRP.ex4",0,0); 
  //  double value=iCustom(_Symbol,_Period,file_custom_indicator,0,0); 
  ...
  if(mtfOn) conditionsToBuy.AddCondition(buyCondition1 = new BUYcondition1());
  ...
  if(mtfOn) conditionsToSell.AddCondition(sellCondition1 = new SELLcondition1());
