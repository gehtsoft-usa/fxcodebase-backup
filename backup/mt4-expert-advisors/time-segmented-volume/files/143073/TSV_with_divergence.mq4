...
//https://fxcodebase.com/code/posting.php?mode=reply&f=38&t=65882
...
input int ma_period = 14; // Avg Period
input ENUM_MA_METHOD ma_method = MODE_SMA; // Avg Method
...
int CreateAlert(int id, ENUM_TIMEFRAMES tf, color upColor, color downColor)
{
   ...
   upCondition.Add(new RegularBullishDivergenceCondition(TSV, _Symbol, tf), false);
   ...
   downCondition.Add(new RegularBearishDivergenceCondition(TSV, _Symbol, tf), false);
   ...
   upCondition.Add(new HiddenBullishDivergenceCondition(TSV, _Symbol, tf), false);
   upAction = NULL;
   if (filter_consecutive)
   {
      upCondition.Add(new LastSignalNotCondition(1), false);
   ...
   downCondition.Add(new HiddenBearishDivergenceCondition(TSV, _Symbol, tf), false);
   downAction = NULL;
   if (filter_consecutive)
   {
      downCondition.Add(new LastSignalNotCondition(-1), false);
...
int init()
{
   ...
   IndicatorBuffers(11);
   IndicatorObjPrefix = GenerateIndicatorPrefix("tsvd");
   IndicatorShortName("TSV with divergence");
   mainSignaler = new Signaler();
   mainSignaler.SetMessagePrefix(_Symbol + "/" + TimeframeToString((ENUM_TIMEFRAMES)_Period) + ": ");

   int id = 0;
   TSV = new ColoredStream(_Symbol, (ENUM_TIMEFRAMES)_Period);
   id = TSV.RegisterStream(id, Green);
   id = TSV.RegisterStream(id, Red);
   id = TSV.RegisterInternalStream(id);
                ...
                const int &spread[])
{
   ...
   for (int pos = MathMin(bars_limit, rates_total - 1 - MathMax(prev_calculated - 1, toSkip)); pos >= 0 && !IsStopped(); --pos)
   {
      ...
      {
         if (close[pos + i] > close[pos + 1 + i])
         {
            tsv = tick_volume[pos + i] * (close[pos + i] - close[pos + 1 + i]);
         }
         else if (close[pos + i] < close[pos + 1 + i])
         {
            tsv = (-1) * tick_volume[pos + i] * (close[pos + 1 + i] - close[pos + i]);
         }
         else
         {
            tsv = 0;
      ...
      TSV.Set(tsv, pos, tsv > TSV._data[pos + 1] ? 0 : 1);

      if (customStream != NULL)
      {
         customStream.SetValue(pos, TSV._data[pos]);
      }
