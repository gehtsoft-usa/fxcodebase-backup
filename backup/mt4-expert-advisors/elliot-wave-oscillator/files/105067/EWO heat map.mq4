...
input color up_up_color = 0x00FF00; // Up-Up color
input color up_dn_color = 0x008800; // Up-Down color
input color dn_up_color = 0x000088; // Down-Up color
input color dn_dn_color = 0x0000FF; // Up-Down color
...
int init()
{
   ...
   if (symbol1 != "")
   {
      id = Add(symbol1, id, index);
      --index;
   }
   if (symbol2 != "")
   {
      id = Add(symbol2, id, index);
      --index;
   }
   if (symbol3 != "")
   {
      id = Add(symbol3, id, index);
      --index;
   }
   if (symbol4 != "")
   {
      id = Add(symbol4, id, index);
      --index;
   }
   if (symbol5 != "")
   {
      id = Add(symbol5, id, index);
      --index;
   }
...
int Add(string symbol, int id, int index)
{
   MultiHeatMapValueCalculator* calc = new MultiHeatMapValueCalculator(index + 1);
   conditions[index] = calc;
   ICondition* upUpCondition = new UpUpCondition(symbol, (ENUM_TIMEFRAMES)_Period);
   ICondition* upDownCondition = new UpDownCondition(symbol, (ENUM_TIMEFRAMES)_Period);
   ICondition* downUpCondition = new DownUpCondition(symbol, (ENUM_TIMEFRAMES)_Period);
   ICondition* downDownCondition = new DownDownCondition(symbol, (ENUM_TIMEFRAMES)_Period);
   id = calc.RegisterStreams(id, up_up_color, upUpCondition, symbol + " UP-UP");
   id = calc.RegisterStreams(id, up_dn_color, upDownCondition, symbol + " UP-DN");
   id = calc.RegisterStreams(id, dn_up_color, downUpCondition, symbol + " DN-UP");
   id = calc.RegisterStreams(id, dn_dn_color, downDownCondition, symbol + " DN-DN");
