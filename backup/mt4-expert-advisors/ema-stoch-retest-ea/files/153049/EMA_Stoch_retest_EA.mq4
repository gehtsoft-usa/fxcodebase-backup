...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=74286
...
enum ModeLevelsTP {
    ...
    TPBySLMultiplier // SL Multiplied
};
enum TSLMode {
    byPips,  // By Pips
...
input ModeLevelsTP modeTP = TPBySLMultiplier;         // Mode Take Profit:
input double slMultiplier = 2; // SL multiplier:
...
input ModeLevelsSL modeSL = SLByATR;            // Mode Stop Loss:
...
input string             IemaFast = "== Moving Average Setup ==";  // ————————————
input int                maFastPeriod = 50;                            // Period
...
input string             IemaSlow = "== Moving Average Setup ==";  // ————————————
input int                maSlowPeriod = 150;                            // Period
...
#define stochastic
#ifdef stochastic
enum ENUM_PRICE_FIELD {
    LowHigh,    // Low/High
    CloseClose  // Close/Close
};
input string           Istoch = "== Stochastic Setup ==";  // == Stochastic Setup ==
input int              kPeriod = 5;                         // K% Period
input int              dPeriod = 5;                         // D% Period
input int              slowing = 3;                         // Slowing
input ENUM_MA_METHOD   stMethod = MODE_SMA;                  // Ma Method
input ENUM_PRICE_FIELD stPriceField = LowHigh;                   // Price Field
input double st_level_up = 80;  // Over Bougth Level
input double st_level_dn = 20;  // Over Sold Level

class Stoch
{
    string _symbol;
    int    _tf;
    double _levelUp, _levelDn;
    ...
    struct StochParameters
    {
        ...
        int setup2;  // D% Period
        int setup3;  // Slowing
        int setup4;  // Ma Method
        int setup5;  // Price Field
    };
    StochParameters _setup;
    ...
    Stoch(string Symbol, int TimeFrame)
    {
        _symbol = Symbol;
        _tf = TimeFrame;
    }
    ...
    void setSetup(int set1, int set2, int set3, ENUM_MA_METHOD set4, ENUM_PRICE_FIELD set5, double levelUp, double levelDn)
    {
        _setup.setup1 = set1;
        _setup.setup2 = set2;
        _setup.setup3 = set3;
    ...
    {
        // return iStochastic(_symbol, _tf, kPeriod,dPeriod, slowing, stMethod,stPriceField,shift };
        return iStochastic(_symbol, _tf,
                           _setup.setup1,
                           _setup.setup2,
                           _setup.setup3,
                           _setup.setup4,
    ...
    {
        return main(shift) >= _levelUp;
    }
    bool isOverSold(int shift)
    {
        return main(shift) <= _levelDn;
    }

    bool isCrossUp(int shift)
    {
        return main(shift) > signal(shift) && main(shift + 1) <= signal(shift + 1);
    }
    bool isCrossDn(int shift)
    {
        return main(shift) < signal(shift) && main(shift + 1) >= signal(shift + 1);
    }
...
class ByATR : public iLevels
{
    ...
    double calculateLevel()
    {
        ...
        if(_side == "buy") { return ask + distance; }
        if(_side == "sell") { return bid - distance; }
        return -1;
    }
...
class BUYcondition1 : public iConditions
{
    ...
    bool evaluate()
    {
        ...
        //   return iClose(NULL, 0, 1) > maFast.index(1);
        return maFast.index(1) > maSlow.index(1);
...
class BUYcondition3 : public iConditions
{
    ...
    bool evaluate()
    {
        ...
        return stoch.isOverSold(1) && stoch.isCrossDn(1);
        //   return false;
    }
};
...
class SELLcondition3 : public iConditions
{
    ...
    bool evaluate()
    {
        ...
        return stoch.isOverBougth(1) && stoch.isCrossUp(1);
        // return false;
    }
};
...
int OnInit()
{
    ...
    // NOTE: OnInit stoch
    stoch = new Stoch();
    stoch.setSetup(kPeriod, dPeriod, slowing, stMethod, stPriceField, st_level_up, st_level_dn);
#endif
    ...
    if(stopLossOn)
        switch(modeSL)
        {
            ...
            case SLPipsFromOpenCandle:
                ...
                break;
            case SLByATR:
                levelSL = new Levels(new ByATR(_Symbol, side, "SL", PERIOD_CURRENT, ATRPeriods, 0, ATRMultiplier));
                result = levelSL.calculateLevel();
                break;
                // ByATR(string inpSymbol, string inpSide, string inpMode, ENUM_TIMEFRAMES inpATRTf, int inpATRPeriod, int inpATRShift, double inpATRmultiplier)
        }
...
void TakeProfitControl()
{
    double newtp = 0;
    if(modeTP == TPBySLMultiplier)
    {
        for(int i = OrdersTotal() - 1;i >= 0;i--)
        {
            if(OrderSelect(i, SELECT_BY_POS) && OrderSymbol() == _Symbol && OrderMagicNumber() == magico)
            {
                if(OrderTakeProfit() == 0 && OrderStopLoss() != 0)
                {
                    ...
                    if(OrderType() == OP_BUY) { newtp = OrderOpenPrice() + (slDistance * slMultiplier); }
                    if(OrderType() == OP_SELL) { newtp = OrderOpenPrice() - (slDistance * slMultiplier); }
