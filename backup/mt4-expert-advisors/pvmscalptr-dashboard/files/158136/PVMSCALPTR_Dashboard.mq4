...
public:
   ...
   {
      double value = iCustom(_symbol, _timeframe, "PVMSCALPTR", W1, W2, 0, period);
      return value > 0;
   }
};
...
public:
   ...
   virtual bool IsPass(const int period, const datetime date)
   {
      double value = iCustom(_symbol, _timeframe, "PVMSCALPTR", W1, W2, 1, period);
      return value > 0;
   }
};
...
string IndicatorName = "PVMSCALPTRD";
int init()
{
   double temp = iCustom(NULL, 0, "PVMSCALPTR", 0, 0);
   if (GetLastError() == ERR_INDICATOR_CANNOT_LOAD)
   {
      Alert("Please, install the 'PVMSCALPTR' indicator");
      return INIT_FAILED;
   }
   ...
   IndicatorObjPrefix = GenerateIndicatorPrefix("PVMSCALPTRD");
