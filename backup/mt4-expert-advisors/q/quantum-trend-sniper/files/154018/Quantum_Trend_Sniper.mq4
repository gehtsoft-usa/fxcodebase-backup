...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=74527
...
string file_custom_indicator = "Quantum Trend Sniper Indicator MT4";
int    bufferToBuy           = 0;
int    bufferToSell          = 1;

input string Tindi           = "== Indicator Setup =="; // ————————————
input int    InpPeriod       = 14;                      // Period
input int    InpAppliedPrice = 0;                       // Applied Price
...
input bool   UseTPLevels     = true;                    // Use TP Levels
input double TP1             = 20.0;                    // TP1
...
class BUYcondition1 : public iConditions
{
  ...
  public:
    ...
    {
        int  barcount      = 10;
        long lastArrowSell = 0, lastArrowBuy = 0;
        for (int i = ObjectsTotal() - 1; i >= 0; i--) {
            string name = ObjectName(0, i, 0, OBJ_ARROW);
            if (StringFind("rsidiv", name, 0)) {
                if (ObjectGetInteger(0, name, OBJPROP_TIME) > lastArrowSell && ObjectGetInteger(0, name, OBJPROP_ARROWCODE) == 242)
                    lastArrowSell = ObjectGetInteger(0, name, OBJPROP_TIME);
                if (ObjectGetInteger(0, name, OBJPROP_TIME) > lastArrowBuy && ObjectGetInteger(0, name, OBJPROP_ARROWCODE) == 241)
                    lastArrowBuy = ObjectGetInteger(0, name, OBJPROP_TIME);
            }
        }

        if (lastArrowBuy >= iTime(Symbol(), Period(), barcount) && lastArrowBuy <= iTime(Symbol(), Period(), 0) && lastArrowBuy > lastArrowSell && lastSignal != lastArrowBuy ) {
            lastSignal = lastArrowBuy;
            return true;
        }
        return false;
    }
...
class SELLcondition1 : public iConditions
{
  ...
  public:
    bool evaluate()
    {
        ...
        if (lastArrowSell >= iTime(Symbol(), Period(), barcount) && lastArrowSell <= iTime(Symbol(), Period(), 0) && lastArrowSell > lastArrowBuy && lastSignal != lastArrowSell ) {
            lastSignal = lastArrowSell;
            return true;
        }
        return false;
    }
...
// #ifdef CONTROL_CUSTOM_INDICATOR_FILE
    ...
    // if (GetLastError() == ERR_INDICATOR_CANNOT_LOAD) {
        string txt = "To work correctly this EA need the indicator \"" + file_custom_indicator + "\" in the Chart";
