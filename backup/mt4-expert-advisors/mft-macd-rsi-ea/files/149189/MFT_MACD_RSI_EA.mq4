...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=73250
...
// ------------------------------------------------------------------
input string T10           = "== MACD1 Setup ==";  // == MACD1 Setup ==
input int    uTF0          = 0;                    // Time Frame Current
input int    uFastEma0     = 12;                   // Fast Ema:
input int    uSlowEma0     = 26;                   // Slow Ema:
input int    uSignalEma0   = 9;                    // Slow Ema:
input int    uApliedPrice0 = 0;                    // Aplied Price:
input string T11           = "== MACD2 Setup ==";  // == MACD2 Setup ==
input bool   TF1_ON        = true;                 // TF2 ON:
input int    uTF1          = 60;                   // Time Frame:
input int    uFastEma1     = 12;                   // Fast Ema:
input int    uSlowEma1     = 26;                   // Slow Ema:
input int    uSignalEma1   = 9;                    // Slow Ema:
input int    uApliedPrice1 = 0;                    // Aplied Price:
input string T12           = "== MACD3 Setup ==";  // == MACD3 Setup ==
input bool   TF2_ON        = true;                 // TF3 ON:
input int    uTF2          = 240;                  // Time Frame:
input int    uFastEma2     = 12;                   // Fast Ema:
input int    uSlowEma2     = 26;                   // Slow Ema:
input int    uSignalEma2   = 9;                    // Slow Ema:
input int    uApliedPrice2 = 0;                    // Aplied Price:
input string T13           = "== MACD4 Setup ==";  // == MACD4 Setup ==
input bool   TF3_ON        = true;                 // TF4 ON:
input int    uTF3          = 1440;                 // Time Frame:
input int    uFastEma3     = 12;                   // Fast Ema:
input int    uSlowEma3     = 26;                   // Slow Ema:
input int    uSignalEma3   = 9;                    // Slow Ema:
input int    uApliedPrice3 = 0;                    // Aplied Price:
input string             Irsi            = "== RSI Setup ==";  // == RSI Setup ==
input int                rsiPeriod       = 14;                 // Period
input ENUM_APPLIED_PRICE rsiAppliedPrice = PRICE_CLOSE;        // Applied Price
input double             rsiLevelUp      = 60;                 // RSI Level Over Bougth
input double             rsiLevelDn      = 40;                 // RSI Level Over Sold
...
class BUYcondition1 : public iConditions
{
 public:
  bool evaluate()
  {
    ...
    // if (macdTrendUp()== true && crossUp()==true) { return true; }
    if (macdTrendUp()== true) { return true; }
 ...
 public:
  bool evaluate()
  {
    // TODO: condition Buy 2
		return rsi.CrossAbove_LevelDn(1);
  }
};
 ...
 public:
  bool evaluate()
  {
    // TODO: condition sell 1
		// if (macdTrendDn()== true && crossDn()==true) { return true; }
		if (macdTrendDn()== true) { return true; }
    return false;
  }
};
 ...
 public:
  bool evaluate()
  {
    // TODO: condition sell 2
		return rsi.CrossBelow_LevelUp(1);
  }
};
...
bool macdTrendUp()
{
  ...
  int trend2 = 2;
  int trend3 = 2;
  if (TF1_ON) trend1 = trendTF1();
  if (TF2_ON) trend2 = trendTF2();
  if (TF3_ON) trend3 = trendTF3();
  if ((trend1 == 0 || trend1 == 2) && (trend2 == 0 || trend2 == 2) && (trend3 == 0 || trend3 == 2)) return true;
...
bool macdTrendDn()
{
  ...
  if (TF2_ON) trend2 = trendTF2();
  if (TF3_ON) trend3 = trendTF3();
  if ((trend1 == 1 || trend1 == 2) && (trend2 == 1 || trend2 == 2) && (trend3 == 1 || trend3 == 2)) return true;

  return false;
}
int trendTF1()
{
  double main1   = iMACD(NULL, uTF1, uFastEma1, uSlowEma1, uSignalEma1, uApliedPrice1, 0, 1);
  double signal1 = iMACD(NULL, uTF1, uFastEma1, uSlowEma1, uSignalEma1, uApliedPrice1, 1, 1);

  if (main1 > 0 && main1 > signal1) return 0;
  if (main1 < 0 && main1 < signal1) return 1;

  return -1;
}
int trendTF2()
{
  double main2   = iMACD(NULL, uTF2, uFastEma2, uSlowEma2, uSignalEma2, uApliedPrice2, 0, 1);
  double signal2 = iMACD(NULL, uTF2, uFastEma2, uSlowEma2, uSignalEma2, uApliedPrice2, 1, 1);

  if (main2 > 0 && main2 > signal2) return 0;
  if (main2 < 0 && main2 < signal2) return 1;

  return -1;
}
int trendTF3()
{
  double main3   = iMACD(NULL, uTF3, uFastEma3, uSlowEma3, uSignalEma3, uApliedPrice3, 0, 1);
  double signal3 = iMACD(NULL, uTF3, uFastEma3, uSlowEma3, uSignalEma3, uApliedPrice3, 1, 1);

  if (main3 > 0 && main3 > signal3) return 0;
  if (main3 < 0 && main3 < signal3) return 1;

  return -1;
}
bool crossUp()
{
	double main0   = iMACD(NULL, uTF0, uFastEma0, uSlowEma0, uSignalEma0, uApliedPrice0, 0, 1);
  double signal0 = iMACD(NULL, uTF0, uFastEma0, uSlowEma0, uSignalEma0, uApliedPrice0, 1, 1);
	double prev_main0   = iMACD(NULL, uTF0, uFastEma0, uSlowEma0, uSignalEma0, uApliedPrice0, 0, 2);
  double prev_signal0 = iMACD(NULL, uTF0, uFastEma0, uSlowEma0, uSignalEma0, uApliedPrice0, 1, 2);

	if(main0 > signal0 && prev_main0 < prev_signal0) return true;
