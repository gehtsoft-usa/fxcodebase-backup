...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=74072
...
#endif
input string Tindi = "== SAR Indicator Setup ==";  // ————————————
input ENUM_TIMEFRAMES sar_timeframe = PERIOD_CURRENT; // Time Frames:
input double          sar_step = 0.02; // Step:
input double          sar_maximum = 0.2;// Maximum:
...
#endif
#ifdef GRID_ON
input string tGrid               = "== Averaging Setup ==";  // ————————————
input bool   GridON              = true;               // Averaging On:
...
input int    GridUser_gap        = 20;                  // Gap betwen orders (pips):
input bool   closeGridOn         = true;                // Use Close Grid?
input double closeGridTP         = 100;                 // Take Profit Grid $ (zero to deactive)
input double closeGridSL = -100;                // Stop Loss Grid -$ (zero to deactive)

input string trzone               = "== RecoveryZone Setup ==";  // == RecoveryZone Setup ==
input bool   rzoneON              = true;                // Use RecoveryZone:
input int    rzoneUser_maxCount   = 5;                   // Max attempts:
input double rzoneUser_maxLot     = 10;                  // Max lot value:
input double rzoneUser_multiplier = 1.5;                 // Multiplier:
input int    rzoneUser_gap        = 30;                  // Gap betwen orders (pips):
input bool   closerzoneON         = true;                // Use Close RecoveryZone?
input double closerzoneTP         = 5;                 // Take Profit RecoveryZone $ (zero to deactive)
input double closerzoneSL         = 0;                // Stop Loss RecoveryZone -$ (zero to deactive)
...
class RecoveryZone
{
 ...
 public:
  RecoveryZone(string Symbol, string Side, double LastPrice, double Gap, double Multiplier, int MaxQnt, double MaxLot, double InitialLot, int magic, bool simbolFilterOn = true, bool magicFilterOn = true)
  {
    _symbol     = Symbol;
    _side       = Side;
    _lastPrice  = LastPrice;
...
RecoveryZone* rzoneBuy;
RecoveryZone* rzoneSell;
 ...
 public:
  bool evaluate()
  {
    // TODO: condition Buy 1
      ...
      return
          iClose(NULL, 0, 1) > iSAR(NULL, sar_timeframe, sar_step, sar_maximum, 1) &&
          iClose(NULL, 0, 2) < iSAR(NULL, sar_timeframe, sar_step, sar_maximum, 2);
  }
};
...
// NOTE: Recovery Zone ontick  
  if(rzoneON == true && CheckPointer(rzoneSell) != POINTER_INVALID)
  {
    rzoneSell.doGrid();
    mainOrders.GetMarketOrders();
  }
  if (rzoneON == true && CheckPointer(rzoneBuy) != POINTER_INVALID)
  {
    rzoneBuy.doGrid();
    mainOrders.GetMarketOrders();
  }
  if (rzoneON == true && closerzoneON == true)
  {
    doCloserzoneControl();
  }
  ...
  if (!uTradeReverse)
  {
    if (conditionsToBuy.EvaluateConditions())
    {
      ...
      if (actionSendOrder.doAction())
      {
        ...
        if (rzoneON == true && CheckPointer(rzoneBuy) == POINTER_INVALID)
        {
            double iniPrice = NormalizeDouble(mainOrders.last().price() - ((GridUser_gap * 10 * _Point) * (GridUser_maxCount+1)), _Digits);
            rzoneBuy = new RecoveryZone(_Symbol, "buy", iniPrice , rzoneUser_gap, rzoneUser_multiplier, rzoneUser_maxCount, rzoneUser_maxLot, mainOrders.last().lot(), magico);
        //   conditionsToBuy.AddCondition(gridActiveCondition = new ConditionGridActive(rzoneBuy));
        }
    ...
    if (conditionsToSell.EvaluateConditions())
    {
      ...
      if (actionSendOrder.doAction())
      {
        ...
        {
            double iniPrice = NormalizeDouble(mainOrders.last().price() + ((GridUser_gap * 10 * _Point) * (GridUser_maxCount+1)), _Digits);
            rzoneBuy = new RecoveryZone(_Symbol, "sell", iniPrice, rzoneUser_gap, rzoneUser_multiplier, rzoneUser_maxCount, rzoneUser_maxLot, mainOrders.last().lot(), magico);
            //   conditionsToSell.AddCondition(gridActiveCondition = new ConditionGridActive(rzoneSell));
        }
...
// void CheckearOrdernesyGenerarZones()
// {
//   if (mainOrders.qnt() == 1)
//   {
//     if (mainOrders.last().type() == OP_BUY && CheckPointer(gridBuy) == POINTER_INVALID)
...
void deleterzone()
{
  if (CheckPointer(rzoneSell) != POINTER_INVALID) { delete rzoneSell; }
  if (CheckPointer(rzoneBuy) != POINTER_INVALID) { delete rzoneBuy; }
}
...
void doCloserzoneControl()
{   
   if(closerzoneTP>0)
   {
      if(CheckPointer(rzoneBuy) != POINTER_INVALID)
      if(rzoneBuy.profit() >= closerzoneTP) { rzoneBuy.closeGrid(); delete rzoneBuy; }
      if(CheckPointer(rzoneSell) != POINTER_INVALID)
      if(rzoneSell.profit() >= closerzoneTP){ rzoneSell.closeGrid(); delete rzoneSell; }
   }

   if(closerzoneSL<0)
   {
      if(CheckPointer(rzoneBuy) != POINTER_INVALID)
      if(rzoneBuy.profit() <= closerzoneSL) {rzoneBuy.closeGrid(); delete rzoneBuy; }
      if(CheckPointer(rzoneSell) != POINTER_INVALID)
      if(rzoneSell.profit() <= closerzoneSL) {rzoneSell.closeGrid(); delete rzoneSell; }
   }
}
// clang-format on
