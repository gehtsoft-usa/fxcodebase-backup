...
void OnTick()
{
 ...
 HA_O=iCustom(NULL, 0, "Heiken Ashi", 0, 0, 0, 0, 2, 1);   
 HA_C=iCustom(NULL, 0, "Heiken Ashi", 0, 0, 0, 0, 3, 1);   
 ...
 if ((Signal_Type==0 && HA_C>HA_O) || (Signal_Type==1 && HA_O>HA_C))
 {
  _Alert("Buy");
  CO=CalculateOrders();
  if (CO<=0)
 ...
 if ((Signal_Type==1 && HA_O<HA_C) || (Signal_Type==0 && HA_C<HA_O))
 {
  _Alert("Sell");
  CO=CalculateOrders();
  if (CO>=0)
