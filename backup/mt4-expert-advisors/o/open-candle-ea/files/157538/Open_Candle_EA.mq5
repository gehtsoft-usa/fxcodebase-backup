...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=75360&p=157538#p157538
...
// Mark: DailyLimits:
input string Tlimits            = "==== Loss limits setup ===="; // —————————————————
input bool   lossLimitOn        = false;                         // Loss Limit On:
input double lossLimit          = -1000;                         // Max Daily Loss $:
input bool   uConsiderFloatting = true;                          // Consider Floating:
input bool   uCloseAllWhenLimit = true;                          // Close All when reach the limit:
bool         lossLimitPercentOn = false;                         // Loss Limit by account percent On:
double       lossLimitPercent   = -1;                            // Loss Limit Percent:
...
class ProfitLossLimits : public iConditions
{
    Stats  _stats;
    bool   _botState;
  ...
  public:
    ...
    void set(double LossLimit, double WinLimit, double LossPercent, double WinPercent, bool CtrlLosses = true, bool CtrlWin = true, bool CtrlLossesPercent = true, bool CtrlWinPercent = true,
             bool considerFloating = true, bool closeallon = true)
    {
...
ProfitLossLimits cdLimits;

void OnInitDailyLimits()
{
    if (!lossLimitOn) return;
    cdLimits.set(lossLimit, winLimit, lossLimitPercent, winLimitPercent, lossLimitOn, winLimitOn, lossLimitPercentOn, winLimitPercentOn, uConsiderFloatting);
    Trader.filters.add(&cdLimits);
}
