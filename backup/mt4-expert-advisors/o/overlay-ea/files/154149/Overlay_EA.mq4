...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=74555
...
string file_custom_indicator = "StaticOverlay";

input string Tindi         = "== Indicator Setup =="; // ————————————
input string sSymbol       = "EURCHF";
input int    iPeriod       = 100;
input bool   bMirror       = true;
input bool   bUseAutoCorr  = true;
input int    uPointsBtween = 300; // Distance Between Pairs:

double Indi(int candle = 0, int buffer = 0)
{
    // TODO: copiar la lista de INPUTS para custom indicator:
    return iCustom(NULL, 0, file_custom_indicator,
                   sSymbol, iPeriod, bMirror, bUseAutoCorr,
                   buffer, candle);
}
#endif
...
#ifdef SYMBOLS
// ------------------------------------------------------------------
input string TSymbols = "== Symbols Setup =="; // ————————————
...
string sym = (string)_Symbol+","+sSymbol;
OrdersList mainOrders(filterMagicsOn, (string)magico, filterSymbolsOn, sym);
...
class BUYcondition1 : public iConditions
{
  public:
    bool evaluate()
    {
        ...
        double precioA = SymbolInfoDouble(Symbol(), SYMBOL_BID);
        double precioB = Indi(0);

        // Print(__FUNCTION__, " Distancia(precioA, precioB) ", Distancia(precioA, precioB, "", "points"));

        if (Distancia(precioA, precioB, "", "points") > uPointsBtween) {
            if (precioA < precioB) {
                buySymbol  = Symbol();
                sellSymbol = sSymbol;
            }
...
void OnTick()
{
    ...
    if (!uTradeReverse) {
        if (conditionsToBuy.EvaluateConditions()) {
            if (modeEntry == Market) {
                actionSendOrder = new SendNewOrder("buy", Lots(), buySymbol, 0, SL("buy"), TP("buy"), magico);
            } else {
            ...
            actionSendOrder = new SendNewOrder("sell", Lots(), sellSymbol, 0, SL("sell"), TP("sell"), magico);
            if (actionSendOrder.doAction()) {
                mainOrders.GetMarketOrders();
            }
            delete actionSendOrder;
