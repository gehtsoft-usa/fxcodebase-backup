...
input int    MartingaleUser_maxCount   = 10;                      // Max attempts:
...
MartingaleIncrementLot martingale(magic_number, _Symbol);
#endif
...
// NOTE: atr
  public:
    ...
    virtual double GetValue(const int period, double entryPrice)
    {
        ...
        double slBuy = NormalizeDouble(entryPrice - atrValue, _Digits);
        double slSell = NormalizeDouble(entryPrice + atrValue, _Digits);
        Print(__FUNCTION__," slSell: ",slSell);
        return _isBuy ? slBuy : slSell;
    }
};
#endif
...
class ProfitInRangeCondition : public AConditionBase
{
  ...
  public:
    ...
    virtual bool IsPass(const int period, const datetime date)
    {
        ...
        if (closePrice == 0)
            closePrice = iClose(symbol, PERIOD_M5, 1);
        if (closePrice == 0)
            closePrice = iClose(symbol, PERIOD_M15, 1);
        if (closePrice == 0)
            closePrice = iClose(symbol, PERIOD_M30, 1);
        if (closePrice == 0)
            closePrice = iClose(symbol, PERIOD_H1, 1);
        if (closePrice == 0)
            closePrice = iClose(symbol, PERIOD_H4, 1);
...
ICondition* CreateLongCondition(string symbol, ENUM_TIMEFRAMES timeframe)
{
    ...
    if (SAROn)
        condition.Add(new SARLongCondition(symbol, timeframe), false);
...
ICondition* CreateLongFilterCondition(string symbol, ENUM_TIMEFRAMES timeframe)
{
    ...
    AndCondition* condition = new AndCondition();
    if (StochOn)
        condition.Add(new StochLongCondition(symbol, timeframe), false);
    if (RSIOn)
        condition.Add(new RSILongCondition(symbol, timeframe), false);
    if (CCIOn)
        condition.Add(new CCILongCondition(symbol, timeframe), false);
    return condition;
}
...
ICondition* CreateShortCondition(string symbol, ENUM_TIMEFRAMES timeframe)
{
    ...
    if (SAROn)
        condition.Add(new SARShortCondition(symbol, timeframe), false);
#ifdef ACT_ON_SWITCH_CONDITION
    ActOnSwitchCondition* switchCondition = new ActOnSwitchCondition(symbol, timeframe, (ICondition*)condition);
    condition.Release();
    return switchCondition;
...
ICondition* CreateShortFilterCondition(string symbol, ENUM_TIMEFRAMES timeframe)
{
    ...
    AndCondition* condition = new AndCondition();
    if (StochOn)
        condition.Add(new StochShortCondition(symbol, timeframe), false);
    if (RSIOn)
        condition.Add(new RSIShortCondition(symbol, timeframe), false);
    if (CCIOn)
        condition.Add(new CCIShortCondition(symbol, timeframe), false);
    return condition;
}
