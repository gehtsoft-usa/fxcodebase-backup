//Available @   https://fxcodebase.com/code/viewtopic.php?f=38&t=74672
...
#define EA_NAME "OGT Zone Recovery v1.4.0"
...
void OnTick()
{
   ...
   if(gCurrentDirection != 0)
   {
      ...
      double tradeCosts = totalSpreadCosts+totalCommision; // spread and commision
      ...
      {
         CloseOrdersAndReset();
         Print("Orders closed, profit target of: ", DoubleToStr(profitTarget, 2), "$ exceeded at: ", DoubleToStr(currentProfit, 2), "$, (", tradeCosts, "$)");        
      }
      if(netLotSize != 0)
      {
   ...
   // check if the current direction is buy and the bid price (sell stop has opened) is below the recovery line
      if(gCurrentDirection == 1)
      {
         ...
         if(OrderSelect(gSellStopTicket, SELECT_BY_TICKET))
         {
            ...
            {
               Print("Recovery Sell Stop has been opene, initiating recovery...");
               ...
               if((MaxTrades > 0 && positions < MaxTrades)||MaxTrades == 0) // check we've not exceeded the max trades
               {
