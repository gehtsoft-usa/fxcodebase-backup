...
//http://fxcodebase.com/code/viewtopic.php?f=38&t=70560
...
input bool useRes = true; // Use Alternate Resolution?
input int intRs_ = 6; // Multiplier for Alernate Resolution
input ENUM_MA_METHOD basisType = MODE_SMA; // MA Type
input int basisLen = 9; // MA Period
input double multi = 0.5; // Divergence Channel Width Factor (Stddev)
input bool uHid = false; // Show Hidden Divergence
input bool uReg = false; // Show Regular Divergence
...
int CreateAlert(int id, ICondition* upCondition, IAction* upAction, ICondition* downCondition, IAction* downAction, int upCode, int downCode, string upMessage = "Up", string downMessage = "Down")
{
   ...
   switch (Type)
   {
      case Arrows:
         {
            id = conditions[size].RegisterStreams(id, upMessage, upCode, up_color, pcd);
            id = conditions[size + 1].RegisterStreams(id, downMessage, downCode, down_color, pcd);
         }
         break;
...
public:
   ...
   {
      double closeSeries = iMA(_Symbol, _Period, basisLen, 0, basisType, PRICE_CLOSE, period);
      double openSeries = iMA(_Symbol, _Period, basisLen, 0, basisType, PRICE_OPEN, period);
      double closeOpenAvg = (closeSeries + openSeries) / 2;

      double closeSeriesAlt = iMA(_Symbol, _Period, basisLen * intRes, 0, basisType, PRICE_CLOSE, period);
      double openSeriesAlt = iMA(_Symbol, _Period, basisLen * intRes, 0, basisType, PRICE_OPEN, period);
      double closeSeriesAlt1 = iMA(_Symbol, _Period, basisLen * intRes, 0, basisType, PRICE_CLOSE, period + 1);
      double openSeriesAlt1 = iMA(_Symbol, _Period, basisLen * intRes, 0, basisType, PRICE_OPEN, period + 1);
      return closeSeriesAlt > openSeriesAlt && closeSeriesAlt1 <= openSeriesAlt1;
   }
};
...
class RegDivUpCondition : public ACondition
{
public:
   RegDivUpCondition(const string symbol, ENUM_TIMEFRAMES timeframe)
      :ACondition(symbol, timeframe)
   {
   ...
   bool IsPass(const int period, const datetime date)
   {
      return fractal_bot_rsi[period] != EMPTY_VALUE && iLow(_symbol, _timeframe, period + 2) < rsi_low_price._stream[period] && pcd._data[period + 2] > rsi_low_prev._stream[period] && pcd._data[period] < 0;
   }
};

class RegDivDownCondition : public ACondition
{
public:
   RegDivDownCondition(const string symbol, ENUM_TIMEFRAMES timeframe)
      :ACondition(symbol, timeframe)
   {
   ...
   bool IsPass(const int period, const datetime date)
   {
      return fractal_top_rsi[period] != EMPTY_VALUE && iHigh(_symbol, _timeframe, period + 2) > rsi_high_price._stream[period] && pcd._data[period + 2] < rsi_high_prev._stream[period] && pcd._data[period] > 0;
   }
};

class HidDivUpCondition : public ACondition
{
public:
   HidDivUpCondition(const string symbol, ENUM_TIMEFRAMES timeframe)
      :ACondition(symbol, timeframe)
   {
...
class HidDivDownCondition : public ACondition
{
public:
   HidDivDownCondition(const string symbol, ENUM_TIMEFRAMES timeframe)
      :ACondition(symbol, timeframe)
   {
...
int intRes;
double fractal_top_rsi[];
double fractal_bot_rsi[];
ColoredStream* pcd;
int init()
{
   ...
   IndicatorBuffers(33);
   IndicatorObjPrefix = GenerateIndicatorPrefix("occnrp");
   IndicatorShortName("OCC Alerts NRP");
   mainSignaler = new Signaler();
   mainSignaler.SetMessagePrefix(_Symbol + "/" + TimeframeToString((ENUM_TIMEFRAMES)_Period) + ": ");

   int id = 0;
   pcd = new ColoredStream(_Symbol, (ENUM_TIMEFRAMES)_Period);
   id = pcd.RegisterStream(id, Green, "OCC Difference Factor");
   id = pcd.RegisterStream(id, Red, "OCC Difference Factor");
   ...
   {
      ICondition* upCondition = (ICondition*) new RegDivUpCondition(_Symbol, (ENUM_TIMEFRAMES)_Period);
      ICondition* downCondition = (ICondition*) new RegDivDownCondition(_Symbol, (ENUM_TIMEFRAMES)_Period);
      id = CreateAlert(id, upCondition, NULL, downCondition, NULL, 217, 218);
   }
   {
      ICondition* upCondition = (ICondition*) new HidDivUpCondition(_Symbol, (ENUM_TIMEFRAMES)_Period);
      ICondition* downCondition = (ICondition*) new HidDivDownCondition(_Symbol, (ENUM_TIMEFRAMES)_Period);
      id = CreateAlert(id, upCondition, NULL, downCondition, NULL, 217, 218);
   }

   SetIndexStyle(id, DRAW_NONE);
   SetIndexBuffer(id, fractal_top_rsi);
   ++id;
   SetIndexStyle(id, DRAW_NONE);
   SetIndexBuffer(id, fractal_bot_rsi);
   ++id;
   id = pcd.RegisterInternalStream(id);
   rsi_high_prev = new ValueWhenSimpleStream(_Symbol, (ENUM_TIMEFRAMES)_Period, 1);
   id = rsi_high_prev.RegisterInternalStream(id);
   rsi_high_price = new ValueWhenSimpleStream(_Symbol, (ENUM_TIMEFRAMES)_Period, 1);
   id = rsi_high_price.RegisterInternalStream(id);
   rsi_low_prev = new ValueWhenSimpleStream(_Symbol, (ENUM_TIMEFRAMES)_Period, 1);
   id = rsi_low_prev.RegisterInternalStream(id);
   rsi_low_price = new ValueWhenSimpleStream(_Symbol, (ENUM_TIMEFRAMES)_Period, 1);
   id = rsi_low_price.RegisterInternalStream(id);

   return 0;
}
ValueWhenSimpleStream* rsi_high_prev;
ValueWhenSimpleStream* rsi_high_price;
ValueWhenSimpleStream* rsi_low_prev;
ValueWhenSimpleStream* rsi_low_price;

int deinit()
{
   rsi_high_prev.Release();
   rsi_high_prev = NULL;
   rsi_high_price.Release();
   rsi_high_price = NULL;
   rsi_low_prev.Release();
   rsi_low_prev = NULL;
   rsi_low_price.Release();
   rsi_low_price = NULL;
...
int start()
{
   ...
   {
      double closeSeries = iMA(_Symbol, _Period, basisLen, 0, basisType, PRICE_CLOSE, pos);
      double openSeries = iMA(_Symbol, _Period, basisLen, 0, basisType, PRICE_OPEN, pos);
      double closeOpenAvg = (closeSeries + openSeries) / 2;

      double closeSeriesAlt = iMA(_Symbol, _Period, basisLen * intRes, 0, basisType, PRICE_CLOSE, pos);
      double openSeriesAlt = iMA(_Symbol, _Period, basisLen * intRes, 0, basisType, PRICE_OPEN, pos);
      double closeSeriesAlt1 = iMA(_Symbol, _Period, basisLen * intRes, 0, basisType, PRICE_CLOSE, pos + 1);
      double openSeriesAlt1 = iMA(_Symbol, _Period, basisLen * intRes, 0, basisType, PRICE_OPEN, pos + 1);
      
      bool longCond = closeSeriesAlt > openSeriesAlt && closeSeriesAlt1 <= openSeriesAlt1;
      bool shortCond = closeSeriesAlt < openSeriesAlt && closeSeriesAlt1 >= openSeriesAlt1;
      double diff = closeSeriesAlt - openSeriesAlt;
      pcd.Set(closeOpenAvg == 0 ? 0 : 50000.0 * diff / closeOpenAvg, pos, closeSeriesAlt > openSeriesAlt ? 0 : 1);
      bool f_top_fractal = pcd._data[pos + 4] < pcd._data[pos + 2] && pcd._data[pos + 3] < pcd._data[pos + 2] && pcd._data[pos + 2] > pcd._data[pos + 1] && pcd._data[pos + 2] > pcd._data[pos];
      bool f_bot_fractal = pcd._data[pos + 4] > pcd._data[pos + 2] && pcd._data[pos + 3] > pcd._data[pos + 2] && pcd._data[pos + 2] < pcd._data[pos + 1] && pcd._data[pos + 2] < pcd._data[pos];
      bool f_fractalize = f_top_fractal ? 1 : (f_bot_fractal ? -1 : 0);
      
      fractal_top_rsi[pos] = f_fractalize > 0 ? pcd._data[pos + 2] : EMPTY_VALUE;
      fractal_bot_rsi[pos] = f_fractalize < 0 ? pcd._data[pos + 2] : EMPTY_VALUE;
      rsi_high_prev.Update(pos, Time[pos], fractal_top_rsi[pos] != EMPTY_VALUE, pcd._data[pos + 2]);
      rsi_high_price.Update(pos, Time[pos], fractal_top_rsi[pos] != EMPTY_VALUE, High[pos + 2]);
      rsi_low_prev.Update(pos, Time[pos], fractal_bot_rsi[pos] != EMPTY_VALUE, pcd._data[pos + 2]);
      rsi_low_price.Update(pos, Time[pos], fractal_bot_rsi[pos] != EMPTY_VALUE, Low[pos + 2]);
