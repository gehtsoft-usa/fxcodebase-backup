...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=72477
...
#property indicator_label1 "Line Fast"
#property indicator_type1  DRAW_LINE
#property indicator_color1 clrGreen
#property indicator_style1 STYLE_SOLID
#property indicator_width1 2
#property indicator_label2 "Line Slow"
...
double Trena[];
// ------------------------------------------------------------------
input string Tlines                = "== Lines Setup ==";    // === Lines Setup ===
input int    uSlowPer              = 7;                      // Slow Length
input double uSlowPips             = 0;                      // Slow Pips displacement
input int    uFastPer              = 3;                      // Fast Length
input double uFastPips             = 0;                      // Fast Pips displacement
input string Tstyle                = "== Style ==";          // === Style ===
input bool   LinesOn               = true;                   // Line On?
input color  LineSlowClr           = clrLimeGreen;           // Line Slow Color:
input color  LineFastClr           = clrRed;                 // Line Fast Color:
...
input ENUM_TIMEFRAMES btf = PERIOD_CURRENT; // Bigger timeframe
// ------------------------------------------------------------------
...
int OnInit()
{
  ...
  SetIndexStyle(0, DRAW_LINE, EMPTY, 2, LineFastClr);
  SetIndexBuffer(1, LineSlow, INDICATOR_DATA);
  SetIndexStyle(1, DRAW_LINE, EMPTY, 2, LineSlowClr);
  if (!LinesOn)
  {
...
{
  // clang-format off
  int i = rates_total - prev_calculated - uSlowPer;
  ...
  for (; i > 0; i--)
  {
    ...
    {
      int btfPos = iBarShift(_Symbol, btf, time[i]);
      int prevBtfPos = iBarShift(_Symbol, btf, time[i + 1]);
      if (btfPos == -1)
      {
        continue;
      }
      LineFast[i] = iCustom(_Symbol, btf, "PerfectTrendLine", "", uSlowPer, uSlowPips, uFastPer, uFastPips, 0, btfPos);
      LineSlow[i] = iCustom(_Symbol, btf, "PerfectTrendLine", "", uSlowPer, uSlowPips, uFastPer, uFastPips, 1, btfPos);
      if (prevBtfPos != btfPos)
      {
        ArrowUp[i] = iCustom(_Symbol, btf, "PerfectTrendLine", "", uSlowPer, uSlowPips, uFastPer, uFastPips, 2, btfPos);
        ArrowDn[i] = iCustom(_Symbol, btf, "PerfectTrendLine", "", uSlowPer, uSlowPips, uFastPer, uFastPips, 3, btfPos);
      }
      continue;
    }
    
    double maxFast = high[iHighest(NULL, 0, MODE_HIGH, uFastPer, i)] + uFastPips*_Point;
    double minFast = low[iLowest(NULL, 0, MODE_LOW, uFastPer, i)] - uFastPips*_Point;

    double maxSlow = high[iHighest(NULL, 0, MODE_HIGH, uSlowPer, i)] + uSlowPips*_Point;
    double minSlow = low[iLowest(NULL, 0, MODE_LOW, uSlowPer, i)] - uSlowPips*_Point;
    
		if (close[i] > LineFast[i + 1]) { LineFast[i] = minFast; } else { LineFast[i] = maxFast; }
    if (close[i] > LineSlow[i + 1]) { LineSlow[i] = minSlow; } else { LineSlow[i] = maxSlow; }

    Trend[i] = Trend[i+1];
    if (close[i] > LineFast[i] && close[i] > LineSlow[i]) { Trend[i] = 1; }
    if (close[i] < LineFast[i] && close[i] < LineSlow[i]) { Trend[i] = -1; }
    ...
    {
      ArrowUp[i] = LineSlow[i] - 100 * _Point;
      if (newCandle.IsNewCandle()) Notifications(0);
    }

    if (Trend[i] != Trend[i + 1] && Trend[i] == -1)
    {
      ArrowDn[i] = LineSlow[i] + 100 * _Point;
