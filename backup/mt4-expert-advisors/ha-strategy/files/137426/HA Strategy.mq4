...
input int ha_bars = 3; // HA bars
input bool use_mva = true; // Use MVA
input int mva_period = 100; // MVA period
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      for (int i = 0; i < ha_bars; ++i)
      {
         if (iClose(_symbol, _timeframe, period + i) < iOpen(_symbol, _timeframe, period + i))
         {
            return false;
         }
      }
...
class MVALongCondition : public ACondition
{
public:
   MVALongCondition(const string symbol, ENUM_TIMEFRAMES timeframe)
      :ACondition(symbol, timeframe)
   {
   ...
   bool IsPass(const int period, const datetime date)
   {
      double maValue = iMA(_symbol, _timeframe, mva_period, 0, MODE_SMA, PRICE_CLOSE, period);
...
class MVAShortCondition : public ACondition
{
public:
   MVAShortCondition(const string symbol, ENUM_TIMEFRAMES timeframe)
      :ACondition(symbol, timeframe)
   {
...
ICondition* CreateLongCondition(string symbol, ENUM_TIMEFRAMES timeframe)
{
   ...
   if (use_mva)
   {
      condition.Add(new MVALongCondition(symbol, timeframe), false);
   }
...
ICondition* CreateShortCondition(string symbol, ENUM_TIMEFRAMES timeframe)
{
   ...
   if (use_mva)
   {
      condition.Add(new MVAShortCondition(symbol, timeframe), false);
   }
   #ifdef ACT_ON_SWITCH_CONDITION
      ActOnSwitchCondition* switchCondition = new ActOnSwitchCondition(symbol, timeframe, (ICondition*) condition);
      condition.Release();
