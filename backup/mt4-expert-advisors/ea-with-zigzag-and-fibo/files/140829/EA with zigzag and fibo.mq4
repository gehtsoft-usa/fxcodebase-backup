...
//http://fxcodebase.com/code/viewtopic.php?f=38&t=70940
...
input double open_level = 0.382; // Open level
...
enum TrailingTargetType
{
   ...
   input StopLossType stop_loss_type = SLCustom; // Stop loss type
   input double stop_loss_value = 1; // Stop loss value
...
public:
   ...
   {
      double from, to;
      if (!GetFromTo(period, from, to))
      {
         return 0;
      }
      double level = from + (to - from) * stop_loss_value;
      return level;
   }
};
#endif
...
public:
   ...
   virtual void GetTakeProfit(const int period, const double entryPrice, double stopLoss, double amount, double& takeProfit)
   {
      ...
      takeProfit = from + (to - from) * stop_loss_value;
   }
};
#endif

bool GetFromTo(int period, double& from, double& to)
{
   from = 0;
   to = 0;
   for (int i = period; i < 1000; ++i)
   {
      double value = iCustom(_Symbol, (ENUM_TIMEFRAMES)_Period, "ZigZag", ZZLength, ZZPatternLength, ZZPatternWidth, 0, period);
...
public:
   ...
   {
      double from, to;
      if (!GetFromTo(period, from, to) || from < to)
      {
         return false;
      }
      double level = from + (to - from) * open_level;
      return iClose(_symbol, _timeframe, period) > level;
   } 
};
