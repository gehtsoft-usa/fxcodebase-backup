...
//https://fxcodebase.com/code/viewtopic.php?f=38&p=156464#p156464
...
enum TradeSide { buy,
                 sell };
TradeSide uSide        = buy;                  // Select Side:
input double    uEntryFibo1  = 23.6;                 // Entry Level 1:
input double    uEntryFibo2  = 38.2;                 // Entry Level 2:
input double    uEntryFibo3  = 50.0;                 // Entry Level 3:
input double    uEntryFibo4  = 61.8;                 // Entry Level 4:
input bool      stopLossOn   = true;                 // Stop Loss On:
input double    uSlFibo      = 0;                  // SL Level:
input bool      takeProfitOn = true;                 // Take Profit On:
input double    uTPFibo      = 100;                // TP level:
input string    T0           = "== Trade Setup ==";  // == Trade Setup ==
#ifdef MAX_TRADES_AT_SAME_TIME
input int uMaxTrades = 4;  // Max Trades At Same Time:
#endif
...
string             T01            = "- Take Profit -";  // Setup Hard Take Profit
// bool         takeProfitOn   = true;               // Take Profit On:
ModeLevels modeTP      = Fibo;             // Mode Take Profit:
int        userTPpips  = 20;               // Pips TP
double     userTPmoney = 15;               // Money TP
string     T02         = "- Stop Loss -";  // Setup hard Stop Loss
// bool         stopLossOn     = true;               // Stop Loss On:
ModeLevels modeSL      = Fibo;  // Mode Stop Loss:
...
class SendNewOrder : public iActions
{
 ...
 public:
  ...
  bool doAction()
  {
    ...
    // Mark: Error!
    Print("TYPE: ", newOrder.type());
    Print("SL: ", newOrder.sl());
    Print("TP: ", newOrder.tp());
...
class BUYcondition1 : public iConditions
{
 public:
  bool evaluate()
  {
    ...
    // if (uSide == buy)
    if(zz.price(1) > zz.price(3) &&  zz.price(3) > zz.price(2))
    {
      if(Bid <= FiboZZ(uEntryFibo1) && Ask > FiboZZ(uEntryFibo1))return true;
      if(Bid <= FiboZZ(uEntryFibo2) && Ask > FiboZZ(uEntryFibo2))return true;
      if(Bid <= FiboZZ(uEntryFibo3) && Ask > FiboZZ(uEntryFibo3))return true;
      if(Bid <= FiboZZ(uEntryFibo4) && Ask > FiboZZ(uEntryFibo4))return true;
      
      return zz.price(1) > zz.price(3) &&  zz.price(3) > zz.price(2);
...
// Mark: sl
      case Fibo:
        result = TPSL_Fibo(uTPFibo, side, "sl");
        break;
    }
  return result;
}
...
double TP(string side, double price = 0)
{
  ...
  if (takeProfitOn)
    switch (modeTP)
    {
      ...
      case Fibo:
        result = TPSL_Fibo(uTPFibo, side, "tp");
        break;
    }
  return result;
}
...
double TPSL_Fibo(double level, string side, string request)
{
  double distance      = fabs(zz.price(1) - zz.price(2));  // 100% distance
  double percentPoints = NormalizeDouble(distance * level / 100, _Digits);
  // double basePrice     = zz.price(1) > zz.price(2) ? zz.price(1) : zz.price(2);  // de base tomo el mayor
  double basePrice     = fmin(zz.price(1) , zz.price(2));  // base el mayor
  double tp, sl;

  if (side == "buy")
  {
    basePrice = fmin(zz.price(1) , zz.price(2)); 
    tp = basePrice + percentPoints;
    sl = basePrice - percentPoints;
  }
  if (side == "sell")
  {
    basePrice = fmax(zz.price(1) , zz.price(2)); 
    tp = basePrice - percentPoints;
    sl = basePrice + percentPoints;
  }
  
  // tp = side=="buy"? basePrice - percentPoints :  basePrice + percentPoints;
  // double sl = side=="buy"? basePrice + percentPoints :  basePrice - percentPoints;
