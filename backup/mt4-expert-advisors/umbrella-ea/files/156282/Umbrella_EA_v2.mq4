...
input double       grid_close_profit = 50;                   // Profit to close (0 = off):
input double       grid_close_loss   = 50;                   // Loss to close (0 = off):
...
#define Section_DrawDown_Filter
#ifdef Section_DrawDown_Filter
// Mark: Section_DrawDown_Filter

input string tddfilter         = "== Drawdown filter =="; // ————————————————————————
input bool   dd_filter_on      = false;                   // Drawdown filter on:
input bool   dd_filter_on_grid = false;                   // Aplly to grid ?:
input double dd_limit_money    = 100;                     // Money Limit (0 off):
input double dd_limit_percent  = 1;                       // % Account Limit(0 off):

class ConditionDrawDown_Filter : public iConditions
{
  public:
    bool evaluate()
    {
        ...
        double per   = NormalizeDouble(dd_limit_percent / 100 * AccountBalance(), 2);
        double limit = dd_limit_money == 0 ? per : per == 0 ? dd_limit_money : 0;
        if (per != 0 && dd_limit_money != 0) {
            limit = fmin(per, dd_limit_money);
        }

        for (int i = OrdersTotal() - 1; i >= 0; i--) {
            if (OrderSelect(i, SELECT_BY_POS) && OrderSymbol() == _Symbol && OrderMagicNumber() == magico) {
                dd_current += OrderProfit() + OrderCommission() + OrderSwap();
            }
        }

        if (fabs(dd_current) >= limit && dd_current < 0) {
            r = false;
        }
...
ConditionDrawDown_Filter cdDrawDown_Filter;

void OnInit_DrawDown_Filter()
{
    if (dd_filter_on) {
        Trader.filters.add(&cdDrawDown_Filter);
    }
    if (dd_filter_on_grid) {
        stGrid.addCondition(&cdDrawDown_Filter);
    }
}
...
input const string indicator_file = "Umbrella";                  // Indicator File:
//---
int  bufferToBuy              = 0;
int  bufferToSell             = 1;
