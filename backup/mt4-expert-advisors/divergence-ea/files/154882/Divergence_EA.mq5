...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=74746
...
input ENUM_APPLIED_PRICE maFast_AppliedPrice   = PRICE_OPEN;                          // Applied Price
...
input int                maSlow_Period         = 88;                                   // Period
int                      maSlow_Shift          = 0;                                    // Ma Shift
input ENUM_MA_METHOD     maSlow_Method         = MODE_EMA;                             // Method
input ENUM_APPLIED_PRICE maSlow_AppliedPrice   = PRICE_OPEN;                          // Applied Price
...
extern double DVBuySell=0.0008;// Divergence Enter:
extern double DVStayOut=0.0045;// Divergence Stay Out:
...
class BUYcondition1 : public iConditions
{
 public:
  bool evaluate()
  {
    ...
    double d = divergence();
    return ( d >=DVBuySell && d <=DVStayOut);
...
class SELLcondition1 : public iConditions
{
 public:
  bool evaluate()
  {
    ...
    double d = divergence();
    return ( d <= DVBuySell *-1 && d >= DVStayOut*-1);
  }
};
...
double divergence()
{
   ...
   double dv1, dv2;
    ...
    maF1 = emaFast.index(0);
    maS1 = emaSlow.index(0);
    
    maF2 = emaFast.index(1);
    maS2 = emaSlow.index(1);
