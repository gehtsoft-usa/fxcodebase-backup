//Available @  https://fxcodebase.com/code/viewtopic.php?f=38&t=76280
...
extern bool   fromMarketWatch = false; // Get symbols from Market Watch?
extern string Symbols = "AUDCAD;AUDCHF;AUDJPY;AUDNZD;AUDUSD;*;CADCHF;CADJPY;CHFJPY;*;EURAUD;EURCAD;EURCHF;EURGBP;EURJPY;EURNZD;EURUSD;*;GBPAUD;GBPCAD;GBPCHF;GBPJPY;GBPNZD;GBPUSD;*;NZDCAD;NZDCHF;"
                        "NZDJPY;NZDUSD;*;USDCAD;USDCHF;USDJPY;*;XAUUSD;XAGUSD"; // List of symbols (separated by ";"
...
extern int    XShift      = 5;                   //  Horizontal shift
extern int    YShift      = 100;                 // Vertical shift
...
extern color  Tncolor     = clrWhite;            // Text color - normal
...
#define Section_SymbolList
#ifdef Section_SymbolList
// Mark: Section_SymbolList
...
int OnInit()
{
    ...
    if (!fromMarketWatch) {
        ...
        while (i > 0) {
            ...
            if (current != "*") current = StringConcatenate(prefix, current, suffix);
        ...
        list.getSymbolsFromMarketWatch();
        ArrayResize(aSymbols, list.qnt());

        for (int i = list.ini(); i < list.end(); i = list.next())
            aSymbols[i] = list.currentSymbol();
    }
...
{
    if (ObjectFind(name) != 0) ObjectCreate(0, name, OBJ_BUTTON, WindowIndex, 0, 0, 0);
...
string sTfTable[] = {"M1", "M5", "M15", "M30", "H1"};
int    iTfTable[] = {1, 5, 15, 30, 60};
