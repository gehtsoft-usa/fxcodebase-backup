...
Name:        Combine_two_indicators_into_one_WithButton_v2
Version:     1.00
Date:        2025
...
void start(int prev_calculated)
  {
   ...
   if(prev_calculated > Length + maPeriod + 50)
      start = prev_calculated - Length - maPeriod - 50;
   else
      start = Length + 1;
   ...
   if(start > Length + 1)
     {
      int prev_pos = start - 1;
      if(UpTrendBuffer[prev_pos] > 0 || UpTrendBuffer[prev_pos] == -1.0)
         trend = 1;
      else
         if(DownTrendBuffer[prev_pos] > 0 || DownTrendBuffer[prev_pos] == -1.0)
            trend = -1;
         else
            trend = 0;
     }
   for(int i = start; i < iBars(Symbol(), Period()) && !IsStopped(); i++)
     {
      ...
      if(Close[i] < smin[i - 1])
         trend = -1;
      if(trend > 0 && smin[i] < smin[i - 1])
         smin[i] = smin[i - 1];
      if(trend < 0 && smax[i] > smax[i - 1])
         smax[i] = smax[i - 1];
      bsmax[i] = smax[i] + 0.5 * (MoneyRisk - 1) * (smax[i] - smin[i]);
      bsmin[i] = smin[i] - 0.5 * (MoneyRisk - 1) * (smax[i] - smin[i]);
      if(trend > 0 && bsmin[i] < bsmin[i - 1])
         bsmin[i] = bsmin[i - 1];
      if(trend < 0 && bsmax[i] > bsmax[i - 1])
         bsmax[i] = bsmax[i - 1];
      if(trend > 0)
        {
