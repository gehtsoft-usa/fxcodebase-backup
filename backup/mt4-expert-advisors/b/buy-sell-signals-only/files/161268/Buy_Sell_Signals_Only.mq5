...
Version:     
Date:        
Repository:  Available @ https://fxcodebase.com/code/viewtopic.php?f=38&t=76448
License:     GNU
*/
                ...
                const int &spread[])
{
   ...
   for (int pos = MathMax(rates_total - 1 - bars_limit, MathMax(first, prev_calculated - 1)); pos < rates_total; ++pos)
   {
      ...
      if (plot1_location == "absolute") plot1[pos] = (buySignal ? up[pos] : EMPTY_VALUE);
      else if (plot1_location == "abovebar" || plot1_location == "top")
      {
         int plotshape1_condition = NumberToBool((buySignal ? up[pos] : EMPTY_VALUE));
         if (plotshape1_condition == true) { if (pos - 1 >= 0) plot1[pos] = high[pos - 1]; }
      }
      else if (plot1_location == "belowbar" || plot1_location == "bottom")
      {
         int plotshape1_condition = NumberToBool((buySignal ? up[pos] : EMPTY_VALUE));
         if (plotshape1_condition == true) { if (pos - 1 >= 0) plot1[pos] = low[pos - 1]; }
      }
      string plot2_location = "abovebar";
      if (plot2_location == "absolute") plot2[pos] = (sellSignal ? dn[pos] : EMPTY_VALUE);
      else if (plot2_location == "abovebar" || plot2_location == "top")
      {
         int plotshape2_condition = NumberToBool((sellSignal ? dn[pos] : EMPTY_VALUE));
         if (plotshape2_condition == true) { if (pos - 1 >= 0) plot2[pos] = high[pos - 1]; }
      }
      else if (plot2_location == "belowbar" || plot2_location == "bottom")
      {
         int plotshape2_condition = NumberToBool((sellSignal ? dn[pos] : EMPTY_VALUE));
         if (plotshape2_condition == true) { if (pos - 1 >= 0) plot2[pos] = low[pos - 1]; }
      }
      if (buySignal) { _signaler.SendNotifications("Buy", "{{ticker}} - BUY Signal on {{interval}}"); }
      if (sellSignal) { _signaler.SendNotifications("Sell", "{{ticker}} - SELL Signal on {{interval}}"); }
   }
   return rates_total;
}
//── Project ─────────────────────────────────────────────────────────────────────
