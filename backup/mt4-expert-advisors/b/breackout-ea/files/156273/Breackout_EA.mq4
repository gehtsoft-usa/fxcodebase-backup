...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=75106
...
input string timeStart      = "00:00:00";               // Work hours Start GMT
input string timeEnd        = "23:59:59";               //  Work hours End GMT
input bool closealliftimeend = true; // Close all if time end?
#endif
...
input string hrIni         = "00:00"; // Time Start Range:
input string hrFin         = "04:00"; // Time End Range:
input string hrEndBreckout = "10:00"; // Time End to Valid Breckout:
datetime     TmStartBox;
datetime     TmEndBox;
datetime     TmValidBreackout;
datetime     today;

void getHoras()
{
    datetime tmStartToday = iTime(NULL, PERIOD_D1, 0);
    if (tmStartToday != today) {
        today = tmStartToday;

        TmStartBox       = StringToTime(hrIni);
        TmEndBox         = StringToTime(hrFin);
        TmValidBreackout = StringToTime(hrEndBreckout);

        Print(__FUNCTION__, " ", "TmStartBox", " ", TmStartBox);
        Print(__FUNCTION__, " ", "TmEndBox", " ", TmEndBox);
        Print(__FUNCTION__, " ", "TmValidBreackout", " ", TmValidBreackout);
    }
}

void UpdateBox()
{
    int minutos = ((int)TmEndBox - (int)TmStartBox) / 60;
    int barIni  = iBarShift(NULL, 1, TmEndBox);
...
void DrawBox()
{
    ...
    ObjectDelete(0, "Box");
    ObjectCreate(0, "Box", OBJ_RECTANGLE, 0, TmStartBox, MaxCaja, TmEndBox, MinCaja);
    ObjectSetInteger(0, "Box", OBJPROP_COLOR, RoyalBlue);
...
#define Section_DeletePendingsaccordingtoTime

#ifdef Section_DeletePendingsaccordingtoTime
// Mark: Section_DeletePendingsaccordingtoTime

class ConditionDeletePendingsaccordingtoTime : public iConditions
{
  public:
    bool evaluate()
    {
        datetime timeToDeletePendings = convertTime(timeEnd);
        if (TimeCurrent() >= timeToDeletePendings) {
            return true;
        }
        return false;
    }
};
ConditionDeletePendingsaccordingtoTime cdDeletePendingsaccordingtoTime;

class ActionDeletePendingsaccordingtoTime : public iActions
{
  public:
    bool execute()
    {
        for (int i = OrdersTotal() - 1; i >= 0; i--) {
            if (OrderSelect(i, SELECT_BY_POS) && OrderSymbol() == _Symbol && OrderMagicNumber() == magico) {
                if (OrderType() != OP_BUY && OrderType() != OP_SELL && OrderComment() != "hedge") OrderDelete(OrderTicket());
            }
        }
        return true;
    }
};
ActionDeletePendingsaccordingtoTime acDeletePendings;

Strategy stDeletePendingsaccordingtoTime;

void OnInit_DeletePendingsaccordingtoTime()
{
    stDeletePendingsaccordingtoTime.set(&cdDeletePendingsaccordingtoTime, &acDeletePendings);
    Trader.strategys.add(&stDeletePendingsaccordingtoTime);
}

#endif

#define Section_CloseAllIfTakeprofit
#ifdef Section_CloseAllIfTakeprofit
// Mark: Section_CloseAllIfTakeprofit

class ConditionCloseAllIfTakeprofit : public iConditions
{
  public:
    bool evaluate()
    {
        if (OrderSelect(OrdersHistoryTotal() - 1, SELECT_BY_POS, MODE_HISTORY) && OrderSymbol() == _Symbol && OrderMagicNumber() == magico) {
            if (OrderOpenTime() >= TmEndBox && OrderOpenTime() <= convertTime(timeEnd)) {
                if (OrderProfit() > 0) {
                    Print(" tk ", OrderTicket(), " >>> was tp , with profit: ", OrderProfit());
                    return true;
                }
            }
        }
...
ConditionCloseAllIfTakeprofit cdCloseAllIfTakeprofit;
...
class ActionDeletePendingsHedge : public iActions
{
  public:
    bool execute()
    {
        for (int i = OrdersTotal() - 1; i >= 0; i--) {
            if (OrderSelect(i, SELECT_BY_POS) && OrderSymbol() == _Symbol && OrderMagicNumber() == magico) {
                if (OrderType() != OP_BUY && OrderType() != OP_SELL && OrderComment() == "hedge") 
...
ActionDeletePendingsHedge acDeletePendingsHedge;

Strategy stCloseAllIfTakeprofit;

void OnInit_CloseAllIfTakeprofit()
{
    stCloseAllIfTakeprofit.set(&cdCloseAllIfTakeprofit, &acCloseAll);
    stCloseAllIfTakeprofit.addAction(&acDeletePendingsHedge);
    Trader.strategys.add(&stCloseAllIfTakeprofit);
}

#endif

#define Section_CloseAllTimeOver
#ifdef Section_CloseAllTimeOver
// Mark: Section_CloseAllTimeOver

class ConditionCloseAllTimeOver : public iConditions
{
  public:
    bool evaluate() { return (TimeCurrent() >= convertTime(timeEnd)); }
};
ConditionCloseAllTimeOver cdCloseAllTimeOver;

Strategy stCloseAllTimeOver;

void OnInit_CloseAllTimeOver()
{
    if(!closealliftimeend) return;
    
    stCloseAllTimeOver.set(&cdCloseAllTimeOver, &acCloseAll);
    stCloseAllTimeOver.addAction(&acDeletePendingsHedge);
    Trader.strategys.add(&stCloseAllTimeOver);
}
...
class ConditionNewTradeSell : public iConditions
{
  ...
  public:
    bool evaluate()
    {
        ...
        for (int i = OrdersTotal() - 1; i >= 0; i--) {
            if (OrderSelect(i, SELECT_BY_POS) && OrderSymbol() == _Symbol && OrderMagicNumber() == magico) {
                if (OrderType() == OP_SELL && OrderComment() == "") count++;
            }
        }
...
void OnInit_Hedge()
{
    stHedgeBuy.set(&cdNewTradeBuy, &acDeletePendings);
    stHedgeBuy.addAction(&sellhedge); // abrir sell hedge
    stHedgeSell.set(&cdNewTradeSell, &acDeletePendings);
    stHedgeSell.addAction(&buyhedge); // abrir buy hedge
...
class BUYcondition1 : public iConditions
{
  public:
    bool evaluate()
    {
        ...
        // no hay pendientes ni abiertos, ni se abrió ninguno durante la ventana de itmepo
        int open_trades = 0;
        for (int i = OrdersTotal() - 1; i >= 0; i--) {
            if (OrderSelect(i, SELECT_BY_POS) && OrderSymbol() == _Symbol && OrderMagicNumber() == magico) {
                open_trades++;
...
class ConditionNoTrades : public iConditions
{
  public:
    bool evaluate()
    {
        ...
        for (int i = OrdersHistoryTotal() - 1; i >= 0; i--) {
            if (OrderSelect(i, SELECT_BY_POS, MODE_HISTORY) && OrderSymbol() == _Symbol && OrderMagicNumber() == magico) {
                // if (OrderOpenTime() >= convertTime(timeStart) && OrderOpenTime() <= convertTime(timeEnd)) {
                if (OrderOpenTime() >= TmEndBox && OrderOpenTime() <= convertTime(timeEnd)) {
                    return false;
                }
            }
...
ConditionNoTrades cdNoTradesOpen;

class ConditionStartBOWindow : public iConditions
{
    public:
    bool evaluate()
    {
...
ConditionStartBOWindow cdStartBOWindow;


datetime convertTime(string time)
{
    MqlDateTime tm;
    TimeToStruct(iTime(NULL, PERIOD_D1, 0), tm);
    int hh  = (int)StringSubstr(time, 0, 2);
    int mm  = (int)StringSubstr(time, 3, 2);
    tm.hour = hh;
    tm.min  = mm;
...
{
    SendPendingsStrategy.set(&cdNoTradesOpen, &buy);
    SendPendingsStrategy.addCondition(&cdStartBOWindow);
    SendPendingsStrategy.addAction(&sell);
    ...
    Trader.strategys.add(&SendPendingsStrategy);
...
int OnInit()
{
    ...
    OnInit_DeletePendingsaccordingtoTime();
    OnInit_Hedge();
    OnInit_CloseAllIfTakeprofit();
    OnInit_CloseAllTimeOver();
