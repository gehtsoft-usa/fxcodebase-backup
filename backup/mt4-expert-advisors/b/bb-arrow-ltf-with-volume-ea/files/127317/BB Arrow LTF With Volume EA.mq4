...
// http://fxcodebase.com/code/viewtopic.php?f=38&t=68656
...
input int        Bollinger_Bands_Periods       = 20; // BB periods
input double     Bollinger_Bands_Deviations    = 2; // BB Deviations
input double max_spread = 3; // Max spread, pips

extern string GeneralSection = ""; // == General ==
TradingMode trade_live = TradingModeLive; // Trade live?
...
STOP_LOSS_FEATURE StopLimitType stop_loss_type = StopLimitPips; // Stop loss type
STOP_LOSS_FEATURE double stop_loss_value            = 50; // Stop loss value
...
TAKE_PROFIT_FEATURE StopLimitType take_profit_type = StopLimitPips; // Take profit type
TAKE_PROFIT_FEATURE double take_profit_value           = 50; // Take profit value
enum DayOfWeek
{
...
public:
   ...
   bool IsPass(const int period)
   {
      ...
      return lowerValue0 > close0 && lowerValue1 <= close1 && volume0 > volume1 && _instrument.GetSpread() < max_spread;
   }
};
...
public:
   ...
   bool IsPass(const int period)
   {
      ...
      long volume0 = iVolume(_symbol, _timeframe, period);
      long volume1 = iVolume(_symbol, _timeframe, period + 1);
      return upperValue0 < close0 && upperValue1 >= close1 && volume0 < volume1 && _instrument.GetSpread() < max_spread;
   }
};
...
public:
   LongMoneyManagementStrategy(TradeCalculator *calculator, PositionSizeType lotsType, double lots
      ...
      : AMoneyManagementStrategy(calculator, lotsType, lots, stopLossType, stopLoss, takeProfitType, takeProfit, 0)
   {
      _timeframe = timeframe;
   }
   ...
   {
      stopLoss = iLow(_calculator.GetSymbol(), _timeframe, period);
...
public:
   ...
   void Get(const int period, const double entryPrice, double &amount, double &stopLoss, double &takeProfit)
   {
      stopLoss = iHigh(_calculator.GetSymbol(), _timeframe, period);
      if (_lotsType == PositionSizeRisk)
...
public:
   ...
   void DoTrading()
   {
      ...
      if (_lastBarDate != current_time)
      {
         ...
         _entryStrategy.Exit(BuySide);
         _entryStrategy.Exit(SellSide);
      }
...
TradingController *CreateController(const string symbol, const ENUM_TIMEFRAMES timeframe, string &error)
{
   ...
   IMoneyManagementStrategy *longMoneyManagement = new LongMoneyManagementStrategy(tradeCalculator, lots_type, lots_value, stop_loss_type, stop_loss_value, take_profit_type, take_profit_value, timeframe);
   IMoneyManagementStrategy *shortMoneyManagement = new ShortMoneyManagementStrategy(tradeCalculator, lots_type, lots_value, stop_loss_type, stop_loss_value, take_profit_type, take_profit_value, timeframe);
