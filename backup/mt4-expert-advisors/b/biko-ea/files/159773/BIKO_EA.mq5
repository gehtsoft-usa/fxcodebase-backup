...
// https://fxcodebase.com/code/viewtopic.php?f=38&p=159795#p159795
...
input const bool   grid_on           = true;                 // Grid ON:
input double       grid_m            = 1.5;                  // Grid Lot multiplier:
input double       grid_close_profit = 5;                    // Profit to close grid (0 = off):
input double       grid_close_loss   = 0;                    // Loss to close grid (0 = off):
input const double grid_step_first   = 10;                   // Grid First Step (pips):
input const double grid_step         = 5;                   // Grid Step (pips):
...
class ActionGrid : public iActions
{
  public:
    ...
    bool execute()
    {
        ...
        double first_step = grid_step_first * mPoints * 10;
        ...
        if (OrderType == POSITION_TYPE_BUY) {
            for (int n = 1; n <= grid_max_steps; n++) {
                double last_step = open_price - first_step - (step * n);
                if (Ask < open_price - first_step - (step * (n + 1))) continue;
        ...
        if (OrderType == POSITION_TYPE_SELL) {
            for (int n = 1; n <= grid_max_steps; n++) {
                double last_step = open_price + first_step + (step * n);
                if (Bid > open_price + first_step + (step * (n + 1))) continue;
...
input double        uLotsValue = 0.02;                        // value to calculate Lots:
...
input bool               cciOn            = false;             // Use CCI to entry?
input int                cci_Period       = 14;                // Period
input ENUM_APPLIED_PRICE cci_AppliedPrice = PRICE_TYPICAL;     // Applied Price
input double            ccimax           = 100;
input double            ccimin           = -100;

int  handle_cci = 0;
void setHandle_cci() { handle_cci = iCCI(NULL, 0, cci_Period, cci_AppliedPrice); }

double CCI(int candle = 1)
{
    double value[1];
    // int    shift = Bars(_Symbol, _Period) - candle;
    int shift = candle;
    int copy  = CopyBuffer(handle_cci, 0, shift, 1, value);
...
int    count_max_positions = 2;                           // # Total positions:
