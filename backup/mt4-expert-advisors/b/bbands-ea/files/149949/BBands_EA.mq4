...
string file_custom_indicator = "BBands_Stop_v1.ex4";
...
input string             Tindi          = "== BBands Setup ==";          // ————————————
input ENUM_TIMEFRAMES    bbands_tf      = PERIOD_CURRENT;                // TimeFrame:
...
input bool               maOn           = true;                          // Ema On?:
input ENUM_TIMEFRAMES    ma_tf          = PERIOD_CURRENT;                // TimeFrame:
...
input bool   allowLong  = true;                 // Allow Longs:
input bool   allowShort = true;                 // Allow Shorts:
...
input bool   closeallfriday = true;        // Close All In Friday?
input string closealltime   = "23:59:59";  // Time to Close all
...
class BUYcondition1 : public iConditions
{
 public:
  bool evaluate()
  {
    ...
    // return if(iCustom(NULL, bbands_tf, file_custom_indicator, Length, Deviation, MoneyRisk, Signal, 2, 1) > 0

    double currentSignal = iCustom(NULL, bbands_tf, file_custom_indicator, Length, Deviation, MoneyRisk, Signal, 2, 1);
...
class SELLcondition1 : public iConditions
{
 public:
  bool evaluate()
  {
    ...
    double currentSignal = iCustom(NULL, bbands_tf, file_custom_indicator, Length, Deviation, MoneyRisk, Signal, 3, 1);
    if (currentSignal > 0 && currentSignal != lastsignal)
    {
      lastsignal = currentSignal;
      return true;
...
class ConditionToCloseBuy : public iConditions
{
 public:
  bool evaluate()
  {
    ...
    {
      if (maOn) return sellCondition1.evaluate() && sellCondition2.evaluate();
...
class ConditionToCloseSell : public iConditions
{
 public:
  bool evaluate()
  {
    if (closeAllInOpositeSignal)
    {
      if (maOn) return buyCondition1.evaluate() && buyCondition2.evaluate();
...
int OnInit()
{
  ...
  if (maOn) conditionsToBuy.AddCondition(buyCondition2 = new BUYcondition2());
  ...
  if (maOn) conditionsToSell.AddCondition(sellCondition2 = new SELLcondition2());
...
//--- SESSIONS CONTROL:
  ...
  ma = new MovingAverage(_Symbol, ma_tf);
...
void OnTick()
{
  ...
  // close all in friday
  if (closeallfriday == true && DayOfWeek() == 5)
  {
    if (timeToCloseInFriday())
    {
      closeAll();
    }
  }
...
bool timeToCloseInFriday()
{
  int _timeZone = uTimeZone * 60 * 60;
  int current   = (TimeHour(TimeGMT() + _timeZone) * 3600) + (TimeMinute(TimeGMT() + _timeZone) * 60);

	if(DayOfWeek()==5)
  if (current >= secondsFromZeroHour(closealltime))
  {
    return true;
  }
