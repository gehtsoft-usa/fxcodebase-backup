...
Version:     
Date:        
Repository:  Available @ https://fxcodebase.com/code/viewtopic.php?f=38&t=76350
License:     GNU
*/
...
// ------------------------------------------------------------------
int d_seconds = 3; // Delay in seconds

input int    periods       = 20;
input double desviation    = 2;
input int    shift         = 0;
                ...
                const long &tick_volume[], const long &volume[], const int &spread[])
{
    ...
    for (i = start; i >= 0; i--) {
        ...
        if (haveSignalUp(i)) {
            if (firstTm == 0) firstTm = Time[i];
            if (firstTm != 0 && Time[i] - firstTm >= d_seconds) {
                ArrowUp[i] = Low[i];
                firstTm    = 0;
                notify(0);
            }
...
double valueLineUp(int i) { return iBands(NULL, 0, periods, desviation, shift, applied_price, MODE_UPPER, i); }

double valueLineDn(int i) { return iBands(NULL, 0, periods, desviation, shift, applied_price, MODE_LOWER, i); }

bool haveSignalUp(int i)
{
    // TODO: signal up
    return (iLow(NULL, 0, i) < LineDn[i]);
}

bool haveSignalDown(int i)
{
    // TODO: signal down
    return (iHigh(NULL, 0, i) > LineUp[i]);
}
