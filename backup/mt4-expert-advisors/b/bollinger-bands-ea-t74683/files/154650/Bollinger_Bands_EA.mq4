...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=74683
...
input string Tbands = "== Bands Setup =="; // ————————————
input double bb_period = 20; // Periods:
input double bb_desv = 2; // Desviation:
input ENUM_APPLIED_PRICE bb_applied = PRICE_CLOSE; // Applie price:
int main = 0;
int upper = 1;
int lower = 2;
...
input double       closeAllMoney           = 10;                       // Close by Money $ (0 off):
input double       closeAllMoneyLoss       = -1000;                    // Close by Money Lossing $ (0 off):
...
class BUYcondition1 : public iConditions
{
  public:
    bool evaluate()
    {
        ...
        double indi = iBands(NULL, 0, bb_period, bb_desv, 0, bb_applied, lower, 1);
        double cl=iClose(NULL,0,1);
        double op=iOpen(NULL,0,1);
        return op>indi && cl<indi;
    }
};
...
class SELLcondition1 : public iConditions
{
  public:
    bool evaluate()
    {
        double indi = iBands(NULL, 0, bb_period, bb_desv, 0, bb_applied, upper, 1);
        double cl=iClose(NULL,0,1);
        double op=iOpen(NULL,0,1);
        return op<indi && cl>indi;
    }
