...
// http://fxcodebase.com/code/viewtopic.php?f=38&t=68839
...
input int        Bollinger_Bands_Periods       = 20; // BB Period
input double     Bollinger_Bands_Deviations    = 2; // BB Deviation
input ENUM_APPLIED_PRICE bb_price = PRICE_CLOSE; // BB Price
input int ma_period = 7; // MA period
input ENUM_MA_METHOD ma_method = MODE_SMA; // MA Smoothing
...
public:
   ...
   bool IsPass(const int period)
   {
      double bbValue0 = iBands(_symbol, _timeframe, Bollinger_Bands_Periods, Bollinger_Bands_Deviations, 0, bb_price, MODE_UPPER, period);
      double bbValue1 = iBands(_symbol, _timeframe, Bollinger_Bands_Periods, Bollinger_Bands_Deviations, 0, bb_price, MODE_UPPER, period + 1);
      
      double maValue0 = iMA(_symbol, _timeframe, ma_period, 0, ma_method, ma_price, period);
      double maValue1 = iMA(_symbol, _timeframe, ma_period, 0, ma_method, ma_price, period + 1);
      return maValue0 > bbValue0 && maValue1 <= bbValue1;
   }
};
...
public:
   ...
   bool IsPass(const int period)
   {
      double bbValue0 = iBands(_symbol, _timeframe, Bollinger_Bands_Periods, Bollinger_Bands_Deviations, 0, bb_price, MODE_LOWER, period);
      double bbValue1 = iBands(_symbol, _timeframe, Bollinger_Bands_Periods, Bollinger_Bands_Deviations, 0, bb_price, MODE_LOWER, period + 1);
...
int init()
{
   ...
   IndicatorName = GenerateIndicatorName("BB MA Cross");
   ...
   SetIndexLabel(0, "BB Top");

   SetIndexStyle(1, DRAW_LINE);
   SetIndexBuffer(1, bb_bottom);
   SetIndexLabel(1, "BB Bottom");
...
int start()
{
   ...
   {
      bb_top[pos] = iBands(_Symbol, _Period, Bollinger_Bands_Periods, Bollinger_Bands_Deviations, 0, bb_price, MODE_UPPER, pos);
      bb_bottom[pos] = iBands(_Symbol, _Period, Bollinger_Bands_Periods, Bollinger_Bands_Deviations, 0, bb_price, MODE_LOWER, pos);
      ma[pos] = iMA(_Symbol, _Period, ma_period, 0, ma_method, ma_price, pos);
      up.Update(pos);
      down.Update(pos);
   } 
   return 0;
