...
// http://fxcodebase.com/code/viewtopic.php?f=38&t=71352
...
input HistMode mode = ModeAbsolute; // Mode
enum SingalMode
{
   SingalModeLive, // Live
...
input double X = 2.0; // Bull/Bear multiplier
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      return up[period] >= MathAbs(down[period]) * X;
   }
};
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      return MathAbs(down[period]) >= up[period] * X;
   }
};
...
int init()
{
   IndicatorObjPrefix = GenerateIndicatorPrefix("bvbi");
   IndicatorShortName("Bull vs Bear Indicator");
                ...
                const int &spread[])
{
   ...
   for (int pos = MathMin(bars_limit, rates_total - 1 - MathMax(prev_calculated - 1, toSkip)); pos >= 0 && !IsStopped(); --pos)
   {
      ...
      {
         up[pos] = (close[pos] - low[pos]) / pipSize;
         down[pos] = (close[pos] - high[pos]) / pipSize;
      }
      else
      {
         up[pos] = (close[pos] - low[pos]) / close[pos];
         down[pos] = (close[pos] - high[pos]) / close[pos];
      }
      if (customStream != NULL)
      {
