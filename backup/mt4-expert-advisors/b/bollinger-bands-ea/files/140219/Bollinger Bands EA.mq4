...
// http://fxcodebase.com/code/viewtopic.php?f=38&t=70826
...
input double dist = 5; // Distance to order, pips
...
public:
   ...
   public:
      ...
      bool GetValue(const int period, double &val)
      {
         ...
         val = iClose(_symbol, _timeframe, period) + dist * pipSize;
         return true;
      }
   };
...
class EntryOrderCondition : public AConditionBase
{
   IOrder* _order;
public:
   EntryOrderCondition(IOrder* order)
   {
      _order = order;
      _order.AddRef();
   }
...
class KeepDistanceAction : public AAction
{
   IOrder* _order;
public:
   KeepDistanceAction(IOrder* order)
   {
      _order = order;
      _order.AddRef();
   }
   ...
   virtual bool DoAction(const int period, const datetime date)
   {
      ...
      int mult = digits == 3 || digits == 5 ? 10 : 1;
      double pipSize = point * mult;
      if (MathAbs(OrderOpenPrice() - iClose(_Symbol, PERIOD_M1, 0)) / pipSize > dist)
      {
         if (OrderOpenPrice() > iClose(_Symbol, PERIOD_M1, 0))
         {
            OrderModify(OrderTicket(), iClose(_Symbol, PERIOD_M1, 0) + dist * pipSize, OrderStopLoss(), OrderTakeProfit(), 0, CLR_NONE);
         }
         else
         {
            OrderModify(OrderTicket(), iClose(_Symbol, PERIOD_M1, 0) - dist * pipSize, OrderStopLoss(), OrderTakeProfit(), 0, CLR_NONE);
...
class MoveOrderAction : public AOrderAction
{
   ActionOnConditionLogic* _actions;
public:
   MoveOrderAction(ActionOnConditionLogic* actions)
   {
      _actions = actions;
   }
   ...
   virtual bool DoAction(const int period, const datetime date)
   {
      ...
      EntryOrderCondition* condition = new EntryOrderCondition(order);
      IAction* action = new KeepDistanceAction(order);
      order.Release();
      _actions.AddActionOnCondition(action, condition);
      condition.Release();
      action.Release();
...
TradingController *CreateController(const string symbol, const ENUM_TIMEFRAMES timeframe, string algoId, string &error)
{
   ...
   MoveOrderAction* orderAction = new MoveOrderAction(actions);
   orderAction.RestoreActions(_Symbol, magic_number);
   orderHandlers.AddOrderAction(orderAction);
   orderAction.Release();
   #ifdef TWO_LEVEL_TP
