...
//http://fxcodebase.com/code/viewtopic.php?f=38&t=69228
...
input int periods = 20; // Periods back
input double dev = 2; // Deviations
input color clr = Red; // Area color
...
int init()
{
   IndicatorName = GenerateIndicatorName("BoxPriceStDev");
   IndicatorObjPrefix = "__" + IndicatorName + "__";
   IndicatorShortName(IndicatorName);

   source = new PriceStream(_Symbol, (ENUM_TIMEFRAMES)_Period, PriceClose);
   stDev = new StDevStream(source, periods);
...
{
   double val;
   if (stDev.GetValue(0, val))
   {
      double top = Close[0] + val * 2;
      double bottom = Close[0] - val * 2;
      ResetLastError();
      string id = IndicatorObjPrefix + "idValue";
      if (ObjectFind(0, id) == -1)
      {
         if (!ObjectCreate(0, id, OBJ_RECTANGLE, 0, Time[periods], top, Time[0], bottom))
         {
      ...
      ObjectSetDouble(0, id, OBJPROP_PRICE1, top);
      ObjectSetDouble(0, id, OBJPROP_PRICE2, bottom);
      ObjectSetInteger(0, id, OBJPROP_TIME1, Time[periods]);
