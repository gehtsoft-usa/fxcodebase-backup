...
enum e_cycles{ Min_5=1, Min_15=2, Min_30=3, Min_60=4, Min_240=5, Daily=6, Weekly=7, Monthly=8, Quoter=9 };
...
int start()
{
   ...
   switch (BTF)
   {
      ...
      case Quoter:
         ...
         periods = 3;
         Minutes = 43200;
         int max = iBars(NULL, PERIOD_MN1);
         while (start < max - 1)
         {
            datetime dt = iTime(NULL, PERIOD_MN1, start);
            MqlDateTime time;
            if (!TimeToStruct(dt, time) || time.mon == 1)
               break;
            ++start;
         }
         start -= 3;
