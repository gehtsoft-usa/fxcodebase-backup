...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=74761
...
string file_custom_indicator = "Indicador_TV_to_MT4";
int    bufferToBuy           = 0;
int    bufferToSell          = 1;
...
{
    return iCustom(NULL, 0, file_custom_indicator, m, atrPeriods,
                   buffer, candle);
}
...
int trend()
{
    ...
    if(up>0 && up != EMPTY_VALUE) cTrend = 1;        
    if(dn>0 && dn != EMPTY_VALUE) cTrend = -1;        
...
input int uMaxTrades = 1000; // Max Trades At Same Time:
#endif
