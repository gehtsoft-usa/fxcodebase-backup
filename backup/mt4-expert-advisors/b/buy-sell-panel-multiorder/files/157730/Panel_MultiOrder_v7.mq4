...
extern bool  TslON              = false;                       // TSL ON:
...
bool OnInit_GUI()
{
    ...
    {
        res = gui.Create(0, "Multi Order Panel NRH",10, 150, 10, 350, 300);
        if(res) gui.Run();
    }
    return res;
}
...
class BuyAction : public iActions
{
    bool doAction()
    {
        ...
        while(n < userNoTrades)
        {
            ...
            tp = TP("buy", pr);
            SendOrder = new SendNewOrder("buy", Lots(), "", 0, SL("buy", pr), tp, magico);
...
class SellAction : public iActions
{
    bool doAction()
    {
        ...
        while(n < userNoTrades)
        {
            ...
            SendNewOrder* SendOrder;
            SendOrder = new SendNewOrder("sell", Lots(), "", 0, SL("sell", pr), tp, magico);
            result = SendOrder.doAction();
            n++;
            delete SendOrder;            
        }
...
class GUI : public CAppDialog
{
    ...
    bool Create(const long chart, const string name, const int subwin, const int x1, const int y1, const int x2, const int y2)
    {
        ...
        if(!Create_label("Lots", Col(1), Row(0), _high, _width, lb5)) return false;
        ...
        if(!Create_button("Buy Stop",  Col(0), Row(3), _high, _width, bt6)) return false;
        if(!Create_button("Sell Stop", Col(1), Row(3), _high, _width, bt7)) return false;
        ...
        if(!Create_button("CLOSE ALL", Col(0), Row(7), _high, _width*2.14, bt3)) return false;
        ...
        if(!Create_Check("TSL ON", Col(0), Row(10), _high, _width, chTslOn)) return false;
    ...
    void HoverEvents(const int id, const long& lparam, const double& dparam, const string& sparam)
    {
        ...
        if(bt6.IsActive()) bt6.ColorBackground(Lime); else bt6.ColorBackground(CONTROLS_BUTTON_COLOR_BG);
        if(bt7.IsActive()) bt7.ColorBackground(Red); else bt7.ColorBackground(CONTROLS_BUTTON_COLOR_BG);
    ...
    void OnEndEdit_editLots() { userLotSize = (double)editLots.Text(); Print(userLotSize); }
    ...
    void OnEndEdit_NewSl() { 
      // recorrer las ordenes y cambiarle los sl
      for(int i=OrdersTotal()-1;i>=0;i--) {
          if(OrderSelect(i,SELECT_BY_POS) && OrderSymbol() == _Symbol && OrderMagicNumber()== magico)
          {
            if(OrderType()==OP_BUY || OrderType()==OP_BUYLIMIT || OrderType()==OP_BUYSTOP) {
                double newSl = OrderOpenPrice() - ((int)eNewSl.Text() *10* _Point);
    ...
    {
      TslON=!TslON;
      Print("line: ",__LINE__," TslON: ",TslON);
    }
...
ON_EVENT(ON_END_EDIT, eNewSl, OnEndEdit_NewSl)
...
gui.setButton6Action(&btnBuyStopAction);
gui.setButton7Action(&btnSellStopAction);
