...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=74682
...
input uint RISK=3; // Risk
input uint SSP =9;  // SSP


input string TindicatorHigherTF = "== Indicator Higher TF ==";  // ————————————
input bool htf_on = false; // Filter Higher TF ON?
input ENUM_TIMEFRAMES htf; // Higher Time Frame:
input uint htf_RISK = 3;     // Risk Higher TF
input uint htf_SSP  = 9;     // SSP Higher TF
int   htf_handle    = 0;

string file_custom_indicator = "BykovTrendAlert";
int    handle                = 0;
int buyBuffer = 1;
int sellBuffer = 0;
...
void setHandle()
{
  handle = iCustom(Symbol(), NULL, file_custom_indicator, RISK,SSP);
  if(htf_on) htf_handle = iCustom(Symbol(), htf, file_custom_indicator, htf_RISK, htf_SSP);
}
...
double custom_indicator_htf()
{
  double lastSignal = EMPTY_VALUE;
  double value[1];
  int i = 1;
  
  while (lastSignal == EMPTY_VALUE) {
    if(CopyBuffer(htf_handle, buyBuffer, i, 1, value) > 0) {
      if (value[0]>0) {
        lastSignal = 1;
        break;
      }
    }
    if(CopyBuffer(htf_handle, sellBuffer, i, 1, value) > 0) {
    if (value[0]>0) {
        lastSignal = -1;
        break;
      }
...
string             Iema1                 = "== Moving Average Fast Setup ==";    // == Moving Average Setup ==
int                maFast_Period         = 10;                                   // Period
int                      maFast_Shift          = 0;                                    // Ma Shift
ENUM_MA_METHOD     maFast_Method         = MODE_EMA;                             // Method
ENUM_APPLIED_PRICE maFast_AppliedPrice   = PRICE_CLOSE;                          // Applied Price
string             Iema2                 = "== Moving Average Medium Setup ==";  // == Moving Average Setup ==
int                maMedium_Period       = 50;                                   // Period
int                      maMedium_Shift        = 0;                                    // Ma Shift
ENUM_MA_METHOD     maMedium_Method       = MODE_EMA;                             // Method
ENUM_APPLIED_PRICE maMedium_AppliedPrice = PRICE_CLOSE;                          // Applied Price
...
input string st = "== Global Close =="; // ————————————
globalPL globalCalcMode = 0;           // Global Close calculate with the following positions
...
class BUYcondition1 : public iConditions
{
 public:
  bool evaluate()
  {
    ...
    double indi = custom_indicator(buyBuffer, 1);
...
class BUYcondition3 : public iConditions
{
 public:
  bool evaluate()
  {
    ...
    double indi = custom_indicator_htf();
    return indi != EMPTY_VALUE && indi >0;
  }
};
...
class SELLcondition1 : public iConditions
{
 public:
  bool evaluate()
  {
    ...
    // return iOpen(NULL, 0, 1) > iClose(NULL, 0, 1);
    double indi = custom_indicator(sellBuffer, 1);
    return indi != EMPTY_VALUE && indi >0;
    // return false;
  }
};
...
int OnInit()
{
    ...
    if(htf_on) conditionsToBuy.AddCondition(buyCondition3 = new BUYcondition3());
    ...
    if(htf_on) conditionsToSell.AddCondition(sellCondition3 = new SELLcondition3());
      ...
      if((ticket = PositionGetTicket(pos)) > 0)
        {
         ...
         if((PositionGetString(POSITION_SYMBOL) ==  sym || sym == "") && PositionGetInteger(POSITION_MAGIC) == (int)magico && magico > 0)
           {
            if(dir == 0 || (dir == 1 && PositionGetInteger(POSITION_TYPE) == POSITION_TYPE_BUY) || (dir == 2 && PositionGetInteger(POSITION_TYPE) == POSITION_TYPE_SELL))
              {
               ...
               if(!succ)
                 {
                  Alert("OrderClose on " + sym + " failed with error #", result.retcode);
                 }
               else
                 {
#ifdef ALERT_FUNCTIONS
                  Notifications(3, sym, result.price, PositionGetDouble(POSITION_SL), PositionGetDouble(POSITION_TP), PositionGetString(POSITION_COMMENT));
#endif
                 }
              }
           }
      ...
      if((ticket = PositionGetTicket(pos)) > 0)
        {
         ...
         if(StringFind(comment, "_gr#", 0) >= 0  && (globalCalcMode == 0 || globalCalcMode == 2))
            continue;
         if((ordersym == sym || sym == "") && PositionGetInteger(POSITION_MAGIC) == (int)magico && magico > 0)
           {
