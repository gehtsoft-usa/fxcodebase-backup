...
// http://fxcodebase.com/code/viewtopic.php?f=38&t=68912
//+------------------------------------------------------------------+
...
public:
   ...
   {
      double value = iCustom(_symbol, _timeframe, "BUY-SELL.SIGNAL", FastEMA, SlowEMA, RSIPeriod, false, 2, period);
      return value > 0;
   }
};
...
public:
   ...
   virtual bool IsPass(const int period)
   {
      double value = iCustom(_symbol, _timeframe, "BUY-SELL.SIGNAL", FastEMA, SlowEMA, RSIPeriod, false, 3, period);
      return value > 0;
   }
};
...
int init()
{
   ...
   double temp = iCustom(NULL, 0, "BUY-SELL.SIGNAL", 0, 0);
   if (GetLastError() == ERR_INDICATOR_CANNOT_LOAD)
   {
      Alert("Please, install the 'BUY-SELL.SIGNAL' indicator");
      return INIT_FAILED;
   }

   IndicatorName = GenerateIndicatorName("BUY-SELL SIGNAL Dashboard");
