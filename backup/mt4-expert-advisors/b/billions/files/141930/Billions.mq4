...
//https://fxcodebase.com/code/viewtopic.php?f=38&t=71176
 ...
 * Version history:
 *
 * V1.3c: (modify by Franky)
 *     - time is in minutes format (due to testing window start / range in StrategyTester)
 *     - modified default vaues of StickBoxToLatestExtreme, TP3Factor, TP5Factor, MaxLossesAllowedPerSession
 *
 * V1.3b:
...
// based on by Squalou http://www.forexfactory.com/showthread.php?t=247220
...
extern int     SessionStart    = 360;    // time for start of price establishment window
extern int     SessionLength  = 119;    // time for end of price establishment window
extern int     SessionEnd = 120;   // end of daily session; tomorrow is another day!
...
extern bool       StickBoxToLatestExtreme = true;  // (applies when "LimitBoxToMaxSize" is true) when true, the box will "stick" to the box high or low, whichever comes last; else it will be centered on the EMA(box_time_range) value;
input int tp_shift = 3; // Take profit shift, pips
...
extern double     TP3Factor    = 0.0;      // if >=10, this will be taken as FIXED PIPs rather than a factor of the box size; if 0, Trades 2&3 will NOT be sent (only Trade 1);
       double     TP4Factor; // set to half-way between TP3Factor and TP5Factor;
extern double     TP5Factor    = 0.0;// TP4 and TP5 targets are OPTIONAL: set TP5Factor=0 to allow only up to TP3 target;
...
extern int        MaxLossesAllowedPerSession=2;
...
string StartTime, EndTime, SessionEndTime;
...
//+------------------------------------------------------------------+
{
  ...
  StartTime = IntegerToString(SessionStart / 60, 2, '0') + ":" + IntegerToString(SessionStart % 60, 2, '0');
  int boxEnd = SessionStart + SessionLength;
  EndTime = IntegerToString(boxEnd / 60, 2, '0') + ":" + IntegerToString(boxEnd % 60, 2, '0');
  SessionEndTime = IntegerToString(SessionEnd / 60, 2, '0') + ":" + IntegerToString(SessionEnd % 60, 2, '0');

  Print(StartTime + " - " + EndTime + " / " + SessionEndTime);
