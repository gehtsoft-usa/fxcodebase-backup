...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=74458
...
input string hrIni = "06:00";   // Time Start register Max-Min box:
input string hrFin = "08:00";    // Time End register Max-Min box:
input bool   ctrlVentana = true; // Control Breackout Time?
input string hrFinVentana = "10:00";    // End Time Allow Trades:
string hrDeletePendings = "20:00";    // Time to Delete Pendings:
datetime     HoraInicio;
datetime     HoraFin;
datetime     HoraFinVentana;
datetime     HoraDeletePendings;
bool   ctrlSize = false;     // Control Max range?
int    maxSize = 500;       // Max range:
...
Modos ModoTP = Puntos;
Modos ModoSL = Puntos;
double TpPer = 100;// Fibo level Take Profit:
double SlPer = 0;  // Fibo Level Stop Loss:
input int    TpPuntos = 50;// Pips Take Profit:
input int    SlPuntos = 50;// Pips Stop Loss:

// Fibos to Send Pending Orders:
double fiboEntry1 = 0; // Fibo Entry 1:
double fiboEntry2 = 61.8; // Fibo Entry 2:
double fiboEntry3 = 100; // Fibo Entry 3:


//--- Modos de Rompimiento:
enum ModosRompi { CruceLimite, VelaCerrada };
ModosRompi TipoRompi = CruceLimite;
bool       ctrlDistancia = false; // Control Breakout distance?
int        extraPips = 0;    // Minimum Pips to consider Breakout:
double     distanciaMax = 50000;    // Maximum distance Breakout:
...
// Colores:
color clrCaja = Crimson;  // Color Caja:
int   grosorCaja = 1;     // Grosor Borde Caja:
bool  rellenoCaja = false; // Relleno Caja:
color clrLineas = Black;     // Color Lineas:
...
//--- Version 2.0
input int userQntMax = 1;// Max Trades At Same Time:
struct Trade
{
