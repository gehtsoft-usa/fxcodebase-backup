...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=73265
...
string file_custom_indicator = "BuySel Alert - Indicator.ex4";
...
int     tolerance   = 3;                    // Tolerance Candles Back for signal retracement:
...
input int    userTslInitialStep = 10;                          // TSL Initial Step:
...
class BUYcondition1 : public iConditions
{
 ...
 public:
  bool evaluate()
  {
		double startPrice = ObjectGetDouble(0, "Start line", OBJPROP_PRICE);
		double stopPrice = ObjectGetDouble(0, "Stop line", OBJPROP_PRICE);
    
		// TODO: condition Buy 1
		// - "Start line" tiene un precio mayor que "Stop line"
		if(stopPrice > startPrice)return false;
		
		// - la vela anterior corta a la linea start
		if(iClose(NULL,0,2) < startPrice && iClose(NULL,0,1) > startPrice && startPrice != _lastSignal)
		{
			  _lastSignal = startPrice;
				return true;
		}
    return false;
  }
...
class BUYcondition2 : public iConditions
{
 ...
 public:
  bool evaluate()
  {
		...
		// - la vela anterior corta a la linea start
    bool haveCut=false;
    if (iClose(NULL, 0, 2) < startPrice && iClose(NULL, 0, 1) > startPrice) haveCut = true;
    if (iLow(NULL, 0, 1) < startPrice && iClose(NULL, 0, 1) > startPrice) haveCut = true;

    if (haveCut && startPrice != _lastSignal)
    {
        _lastSignal = startPrice;
				return true;
		}
...
class SELLcondition1 : public iConditions
{
 ...
 public:
  bool evaluate()
  {
		...
		// - "Start line" tiene un precio Menor que "Stop line"
		bool haveCut=false;
    if (iClose(NULL, 0, 2) > startPrice && iClose(NULL, 0, 1) < startPrice) haveCut = true;
    if (iHigh(NULL, 0, 1) > startPrice && iClose(NULL, 0, 1) < startPrice) haveCut = true;
...
int OnInit()
{
  ...
  double temp = iCustom(NULL, 0, file_custom_indicator,barn, Length, 0, 0);
  if (GetLastError() == ERR_INDICATOR_CANNOT_LOAD)
  {
    Alert("Please, install the: " + file_custom_indicator + " indicator in the Folder MQL4/Indicators");
    return INIT_FAILED;
  }
#endif
...
if(IsTesting())
{
  ...
  iCustom(NULL, 0, file_custom_indicator, barn, Length, 0, 1);
