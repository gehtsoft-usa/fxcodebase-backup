// Id: 13764
//+------------------------------------------------------------------+
//|                               MTF MCP Fractal_Bar_Indicator.mq4  |
...
int init()
  {
   ...
   IndicatorName = GenerateIndicatorName("MTF_MCP_Fractal_Bar_Indicator");
  ...
  { 
	...
	double Bottom=-1;
			 ...
			 {   
				  ...
				  if (iHigh(PAIR[y],iTF[x],pos+2)>iHigh(PAIR[y],iTF[x],pos+4) && iHigh(PAIR[y],iTF[x],pos+2)>iHigh(PAIR[y],iTF[x],pos+3) && iHigh(PAIR[y],iTF[x],pos+2)>iHigh(PAIR[y],iTF[x],pos+1) && iHigh(PAIR[y],iTF[x],pos+2)>iHigh(PAIR[y],iTF[x],pos))
				  {
				   Top = iHigh(PAIR[y],iTF[x],pos+2) ;
				  }
				  
				  if ( iLow(PAIR[y],iTF[x],pos+2)< iLow(PAIR[y],iTF[x],pos+4) &&  iLow(PAIR[y],iTF[x],pos+2)< iLow(PAIR[y],iTF[x],pos+3) && iLow(PAIR[y],iTF[x],pos+2)< iLow(PAIR[y],iTF[x],pos+1) && iLow(PAIR[y],iTF[x],pos+2)< iLow(PAIR[y],iTF[x],pos))
				  {
				     Bottom = iLow(PAIR[y],iTF[x],pos+2) ;  
				  }
			  ...
			  else
			  {
			     ...
			     if (iClose(PAIR[y],iTF[x],0)> Top)
                 {
				  return (1);
                 }	

                  if (iClose(PAIR[y],iTF[x],0)< Bottom)
                 {
				  return (-1);
                 }					 
			     else
