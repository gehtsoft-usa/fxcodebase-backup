//Available @  https://fxcodebase.com/code/viewtopic.php?f=38&t=75538
...
input int param1 = 10; // SuperTrend ATR Period
input double param2 = 1.7; // SuperTrend Multiplier
input int param3 = 0; // SuperTrend Timeframe
input int bars_limit = 1000; // Bars limit
input ENUM_TIMEFRAMES htf = PERIOD_CURRENT; // Higher timeframe
int SuperTrend_Period;
double SuperTrend_Multiplier;
int SuperTrend_TimeFrame;
ATRStream* atr1;
double gadSuperTrend[];
double gadSuperTrend_DEFAULT_VALUE;
ColoredPlot* plot1;
...
void OnInit()
  {
   if(htf != PERIOD_CURRENT)
     {
      htf_indi = iCustom(_Symbol, htf, "XSuperTrend_Tape_v2", param1, param2, param3);
     }
   else
     {
      htf_indi = 0;
     }
     SetIndexBuffer(0, upB, INDICATOR_DATA);
   SetIndexBuffer(1, doB, INDICATOR_DATA);
   int id = 2;
   SuperTrend_Period = param1;
   SuperTrend_Multiplier = param2;
   SuperTrend_TimeFrame = param3;
   atr1 = new ATRStream(SuperTrend_Period);
   plot1 = new ColoredPlot(id);
   plot1.AddColor(Green);
   plot1.AddColor(Red);
   plot1.AddColor(Gray);
   plot1.SetOffset(0);
   id = plot1.RegisterStreams(id);
   IndicatorObjPrefix = GenerateIndicatorPrefix("");
   SetIndexBuffer(id++, gadSuperTrend, INDICATOR_CALCULATIONS);
   id = plot1.RegisterInternalStreams(id);
    
   IndicatorSetString(INDICATOR_SHORTNAME, "xSuperTrend Tape");
...
int OnCalculate(const int rates_total,
                ...
                const int &spread[])
  {
   if(prev_calculated <= 0 || prev_calculated > rates_total)
     {
      gadSuperTrend_DEFAULT_VALUE = 0.0;
      ArrayInitialize(gadSuperTrend, gadSuperTrend_DEFAULT_VALUE);
      plot1.Init();
     }
   int first = 0;
   for(int pos = MathMax(rates_total - 1 - bars_limit, MathMax(first, prev_calculated - 1)); pos < rates_total; ++pos)
     {
      ...
      if(htf_indi != 0)
        {
         ...
         double clr[1];
         if(CopyBuffer(htf_indi, 1, btfPos, 1, clr) != 1)
           {
            return 0;
           }
         plot1.Set(pos, value[0], clr[0] == 0 ? Green : Red);
         continue;
        }
      gadSuperTrend[pos] = pos > 0 ? gadSuperTrend[pos - 1] : 0.0;
      double atr1Value[1];
      if(!atr1.GetValues(pos, 1, atr1Value))
        {
         atr1Value[0] = EMPTY_VALUE;
        }
      double dAtr = atr1Value[0];
      double dUpperLevel = SafePlus(SafeDivide((high[pos] + low[pos]), 2), SafeMultiply(SuperTrend_Multiplier, dAtr));
      double dLowerLevel = SafeMinus(SafeDivide((high[pos] + low[pos]), 2), SafeMultiply(SuperTrend_Multiplier, dAtr));
      if(pos - 1 < 0)
        {
         continue;
        }
      ...
      if((SafeGreater(close[pos], gadSuperTrend[pos - 1]) && SafeLE(close[pos - 1], gadSuperTrend[pos - 1])))
        {
         SetStream(gadSuperTrend, pos, dLowerLevel, gadSuperTrend_DEFAULT_VALUE);
        }
      else
        {
         if(pos - 1 < 0)
         ...
         if((SafeLess(close[pos], gadSuperTrend[pos - 1]) && SafeGE(close[pos - 1], gadSuperTrend[pos - 1])))
           {
            SetStream(gadSuperTrend, pos, dUpperLevel, gadSuperTrend_DEFAULT_VALUE);
           }
         else
           {
            if(pos - 1 < 0)
            ...
            if((SafeLess(gadSuperTrend[pos - 1], dLowerLevel)))
              {
               SetStream(gadSuperTrend, pos, dLowerLevel, gadSuperTrend_DEFAULT_VALUE);
              }
            else
              {
               ...
               if((SafeGreater(gadSuperTrend[pos - 1], dUpperLevel)))
                 {
                  SetStream(gadSuperTrend, pos, dUpperLevel, gadSuperTrend_DEFAULT_VALUE);
                 }
              }
      ...
      double plot1Value = plot1.Set(pos, 1, (((close[pos] > gadSuperTrend[pos]) || ((close[pos] == gadSuperTrend[pos]) && SafeGreater(close[pos - 1], gadSuperTrend[pos - 1]))) ? Green : (((close[pos] < gadSuperTrend[pos]) || ((close[pos] == gadSuperTrend[pos]) && SafeLess(close[pos - 1], gadSuperTrend[pos - 1]))) ? Red : Gray)));
      upB[pos] = gadSuperTrend[pos] < close[pos] ? 1 : 0;
      doB[pos] = gadSuperTrend[pos] > close[pos] ? 1 : 0;
     }
   return rates_total;
  }
//Available @  https://fxcodebase.com/code/viewtopic.php?f=38&t=75538
