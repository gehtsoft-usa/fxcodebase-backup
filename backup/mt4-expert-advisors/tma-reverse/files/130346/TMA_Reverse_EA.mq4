...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      double value = iCustom(_symbol, _timeframe, "TMA Reverse", HalfLength, AtrLength, AtrMultiplier, 1, period);
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      double value = iCustom(_symbol, _timeframe, "TMA Reverse", HalfLength, AtrLength, AtrMultiplier, 2, period);
      return iClose(_symbol, _timeframe, period) < value;
   }
};
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      double value = iCustom(_symbol, _timeframe, "TMA Reverse", HalfLength, AtrLength, AtrMultiplier, 0, period);
      return iClose(_symbol, _timeframe, period) > value;
   }
};
...
int OnInit()
{
   double temp = iCustom(NULL, 0, "TMA Reverse", 0, 0);
   if (GetLastError() == ERR_INDICATOR_CANNOT_LOAD)
   {
      Alert("Please, install the 'TMA Reverse' indicator");
      return INIT_FAILED;
   }
   #ifdef SHOW_ACCOUNT_STAT
      stats = NULL;
