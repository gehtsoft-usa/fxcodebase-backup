...
//https://fxcodebase.com/code/viewtopic.php?f=38&t=71442
...
int init()
{
   IndicatorObjPrefix = GenerateIndicatorPrefix("fco");
   IndicatorShortName("FCO");

   IndicatorBuffers(4);
   stream = new ColoredStream(_Symbol, (ENUM_TIMEFRAMES)_Period);
   stream.RegisterStream(0, clrSilver);
   stream.RegisterStream(1, clrLimeGreen);
   stream.RegisterStream(2, clrOrchid);
   stream.RegisterInternalStream(3);
                ...
                const int &spread[])
{
   ...
   {
      double fru = iFractals(_Symbol, _Period, MODE_UPPER, pos);
      double frd = iFractals(_Symbol, _Period, MODE_LOWER, pos);
      if (fru != EMPTY_VALUE && fru != 0)
      {
         stream.Set(1, pos, 1);
      } 
      else if (frd != EMPTY_VALUE && frd != 0)
      {
         stream.Set(-1, pos, 2);
      }
      else
