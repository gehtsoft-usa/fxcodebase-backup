...
string file_custom_indicator = "Pinbar_Magic_Indicator";
int    handle                = 0;
...
void setHandle()
{
    handle = iCustom(Symbol(), NULL, file_custom_indicator, Iema1, maFast_Period, maFast_Method, maFast_AppliedPrice, Iema2, maMedium_Period, maMedium_Method, maMedium_AppliedPrice, Iema3,
                     maSlow_Period, maSlow_Method, maSlow_AppliedPrice);
}
...
enum CloseAllMode { CloseByMoney, CloseByAccountPercent, CloseByPips };
enum ModeCalcLots { Money, AccountPercent, FixLots };
class LotCalculator
{
    double _tickValue;
    long   _modeCalc;
...
class MovingAverage
{
  ...
  public:
    ...
    void setHandle() { _handle = iMA(_symbol, _tf, _setup.setup0, _setup.setup1, _setup.setup2, _setup.setup3); }
...
class ADX
{
  ...
  public:
    ...
    void setHandle() { _handle = iADX(_symbol, _tf, _setup.setup0, _setup.setup1); }
    ...
    // LINES:
    double Main(int shift) { return calculate(0, shift); }
    double PlusDi(int shift) { return calculate(1, shift); }
    double MinusDi(int shift) { return calculate(2, shift); }
...
input TSLMode      userTslMode        = byPips;                       // TSL Mode:
input int          tsl_delay          = 30;                           // TSL Delay Seconds:
...
input string       tTslBypercent      = "-- TSL By Percent Setup --"; // -- TSL By Percent Setup --
input double       uTSL_per_Initial   = 1;                            // TSL % Initial Step:
input double       uTSL_per_Step      = 1;                            // TSL % Step:
input double       uTSL_per_Distance  = 3;                            // TSL % Distance:
...
class ScheduleController
{
  ...
  public:
    ...
    void setTimeZone(int hs) { _timeZone = hs * 60 * 60; }
...
class Order
{
  ...
  public:
    Order(long id, string symbol, double price, double sl, double tp, double lot, ENUM_ORDER_TYPE type, long magic, string comment, string strategy, datetime expireTime, datetime signalTime,
          double profit, bool bkvWasDoIt, int countPartials)
    ...
    Order* magic(long magic) { _magic = magic; return &this; }
...
class OrdersList
{
    ...
    Order *index(int in) { return GetPointer(orders[in]); }

    void closeAllInList()
    {
        for (int x = PositionsTotal(); x >= 0; x--) {
            ulong tk = PositionGetTicket(x);
...
class TslByPercent : public iTSL
{
    double _InitialStep;
    double _TslStep;
    double _Distance;
  ...
  public:
    TslByPercent(double InitialStep, double TslStep, double Distance)
    {
    ...
    void setInitialStep(Order *order)
    {
        double percentToMove = 1 + (_InitialStep / 100);

        if (order.type() == ORDER_TYPE_SELL) {
            percentToMove = 1 - (_InitialStep / 100);
        }  

        double initial_price = order.price() * percentToMove;
        Print("initial_price:", initial_price);
        order.tslNext(initial_price);
    }

    void setNextStep(Order *order)
    {
        double percentToMove = 1 + (_TslStep / 100);

        if (order.type() == ORDER_TYPE_SELL) {
            percentToMove = 1 - (_InitialStep / 100);
        }
        
        double next_price = order.tslNext() * percentToMove;
        Print("next_price:", next_price);  
        order.tslNext(next_price);
    }

    double newSL(Order *order)
    {
        double pointsToMove = Ask() * (1 + (_Distance /100));
        double newSl        = order.sl();
        
        if (order.type() == ORDER_TYPE_BUY) {
            double target = order.tslNext() * (1 - (_Distance /100));
            if (target > order.sl()) {
                newSl = target;
                Print("newSl: ", newSl);
            }
  ...
  public:
    TrailingStop(OrdersList *ordersList, TSLMode mode)
    {
        ...
        case byPips:
            ...
            break;
        case byPercent:
            _TslMode = new TslByPercent(uTSL_per_Initial, uTSL_per_Step, uTSL_per_Distance);
            break;            
        }
    }
    ~TrailingStop()
    ...
    void moveSL(ulong tk, double newSl)
    {
        ...
        datetime tm = PositionGetInteger(POSITION_TIME) + tsl_delay;
        if (TimeCurrent() < tm)
        {
            Print("waiting time...", tm, " current time: ", TimeCurrent());
            return;
        }    
...
class SendNewOrder : public iActions
{
  ...
  public:
    SendNewOrder(string side, double lots, string symbol = "", double price = 0, double sl = 0, double tp = 0, int magic = 0, string coment = "", datetime expire = 0)
    {
        ...
        newOrder.id(0).symbol(_symbol).type(_type).price(_price).sl(sl).tp(tp).lot(lots).magic(magic).comment(coment).expireTime(expire).profit(0);
    }
...
class MoveSL : public iActions
{
  ...
  public:
    ...
    bool moveSL(long tk, double newSl)
    {
...
class BUYcondition1 : public iConditions
{
  public:
    bool evaluate()
    {
        ...
        return custom_indicator(3, 2) != EMPTY_VALUE;
...
class ConditionCountBuys : public iConditions
{
  ...
  public:
    ConditionCountBuys(int maxBuys, int magico_, ENUM_ORDER_TYPE type)
    {
        _maxBuys = maxBuys;
        _magic   = magico_;
        _type    = type;
...
class SELLcondition1 : public iConditions
{
  public:
    bool evaluate()
    {
        ...
        // return iOpen(NULL, 0, 1) > iClose(NULL, 0, 1);
        return custom_indicator(4, 2) != EMPTY_VALUE;
        // return false;
    }
};
...
class ConditionCountSells : public iConditions
{
  ...
  public:
    ConditionCountSells(int maxSells, int magi, ENUM_ORDER_TYPE type)
    {
        _maxSells = maxSells;
        _magic    = magi;
        _type     = type;
...
double index(int handle_, int buffer, int shift)
{
    double value[1];
    int    qnt = CopyBuffer(handle_, buffer, shift, 1, value);
