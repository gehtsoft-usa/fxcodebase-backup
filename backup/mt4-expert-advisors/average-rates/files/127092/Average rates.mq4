...
input ShowMethod Method = MethodLine; // Index Method
...
public:
   ...
   {
      SetIndexStyle(id + 0, DRAW_LINE, STYLE_SOLID, 5, clr);
      SetIndexBuffer(id + 0, OpenStream);
      SetIndexLabel(id + 0, "Open");
      SetIndexStyle(id + 1, DRAW_LINE, STYLE_SOLID, 5, clr);
      SetIndexBuffer(id + 1, CloseStream);
      SetIndexLabel(id + 1, "Close");
      SetIndexStyle(id + 2, DRAW_LINE, STYLE_SOLID, 1, clr);
      SetIndexBuffer(id + 2, HighStream);
      SetIndexLabel(id + 2, "High");
      SetIndexStyle(id + 3, DRAW_LINE, STYLE_SOLID, 1, clr);
...
public:
   ...
   double Weight;
   double InitialValue;
   Instrument(string symbol, double weight)
   {
      Symbol = symbol;
      Weight = weight;
      InitialValue = 1;
...
int init()
{
   IndicatorShortName("Average rates");
   IndicatorDigits(Digits);

   if (Method == MethodLine)
   {
   ...
   {
      int id = Up.RegisterStreams(0, LineColor);
   }
   if (On1)
   {
      ...
      items[size] = new Instrument(Select1, index == 0 ? Weight1 : -Weight1);
   }
   if (On2)
   {
      int size = ArraySize(items);
      ...
      items[size] = new Instrument(Select2, index == 0 ? Weight2 : -Weight2);
   }
   if (On3)
   {
      int size = ArraySize(items);
      ...
      items[size] = new Instrument(Select3, index == 0 ? Weight3 : -Weight3);
   }
   if (On4)
   {
      int size = ArraySize(items);
      ...
      items[size] = new Instrument(Select4, index == 0 ? Weight4 : -Weight4);
   }
   if (On5)
   {
      int size = ArraySize(items);
      ...
      items[size] = new Instrument(Select5, index == 0 ? Weight5 : -Weight5);
   }
   if (On6)
   {
      int size = ArraySize(items);
      ...
      items[size] = new Instrument(Select6, index == 0 ? Weight6 : -Weight6);
   }
   if (On7)
   {
      int size = ArraySize(items);
      ...
      items[size] = new Instrument(Select7, index == 0 ? Weight7 : -Weight7);
   }
...
int start()
{
   ...
   {
      if (Method == MethodCandle)
         Up.Clear(pos);

      double NumeratorClose = 100;
      double NumeratorOpen = 100;
      double NumeratorHigh = 100;
      double NumeratorLow = 100;
      ...
      if (pos == Bars - 1)
      {
         for (int i = 0; i < ArraySize(items); ++i)
         {
            ...
            if (index < 0)
               continue;
            item.InitialValue = iClose(item.Symbol, _Period, index);
         }
      }
      else
      {
         ...
         for (int i = 0; i < N; ++i)
         {
            ...
            double open = iOpen(item.Symbol, _Period, index);
            double high = iHigh(item.Symbol, _Period, index);
            double low = iLow(item.Symbol, _Period, index);
            NumeratorClose = NumeratorClose * MathPow(close / item.InitialValue, item.Weight / N);
            NumeratorOpen = NumeratorOpen * MathPow(open / item.InitialValue, item.Weight / N);
            NumeratorHigh = NumeratorHigh * MathPow(high / item.InitialValue, item.Weight / N);
            NumeratorLow = NumeratorLow * MathPow(low / item.InitialValue, item.Weight / N);
         }
      }
      if (Method == MethodCandle)
         Up.Set(pos, NumeratorOpen * SCALEFACTOR, NumeratorHigh * SCALEFACTOR, NumeratorLow * SCALEFACTOR, NumeratorClose * SCALEFACTOR);
      else
         out[pos] = NumeratorClose * SCALEFACTOR;
   }
   return 0;
}
