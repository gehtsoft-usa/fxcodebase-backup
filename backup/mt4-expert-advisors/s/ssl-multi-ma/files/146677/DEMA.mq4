...
int start()
{
  ...
  if (AppliedPrice != PRICE_HIGH && AppliedPrice != PRICE_LOW)
  {
  ...
  if (AppliedPrice == PRICE_HIGH)
  {
    if (IndicatorCounted() == 0)
    {
      Buffer[limit]  = High[limit];
      lastEMA        = High[limit];
      lastEMA_of_EMA = High[limit];
      limit--;
    }
    //----
    //	Calculate old bars (not the latest), if necessary
    for (int i = limit; i > 0; i--)
    {
      lastEMA        = weight * High[i] + (1.0 - weight) * lastEMA;
      lastEMA_of_EMA = weight * lastEMA + (1.0 - weight) * lastEMA_of_EMA;
      Buffer[i]      = 2.0 * lastEMA - lastEMA_of_EMA;
    }
    //----
    //	(Re)calculate current bar
    double EMA        = weight * High[0] + (1.0 - weight) * lastEMA,
           EMA_of_EMA = weight * EMA + (1.0 - weight) * lastEMA_of_EMA;
    Buffer[0]         = 2.0 * EMA - EMA_of_EMA;
    //----
    return (0);
  ...
  if (AppliedPrice == PRICE_LOW)
  {
    if (IndicatorCounted() == 0)
    {
      Buffer[limit]  = Low[limit];
      lastEMA        = Low[limit];
      lastEMA_of_EMA = Low[limit];
      limit--;
    }
    //----
    //	Calculate old bars (not the latest), if necessary
    for (int i = limit; i > 0; i--)
    {
      lastEMA        = weight * Low[i] + (1.0 - weight) * lastEMA;
      lastEMA_of_EMA = weight * lastEMA + (1.0 - weight) * lastEMA_of_EMA;
      Buffer[i]      = 2.0 * lastEMA - lastEMA_of_EMA;
    }
    //----
    //	(Re)calculate current bar
    double EMA        = weight * Low[0] + (1.0 - weight) * lastEMA,
           EMA_of_EMA = weight * EMA + (1.0 - weight) * lastEMA_of_EMA;
    Buffer[0]         = 2.0 * EMA - EMA_of_EMA;
    //----
    return (0);
