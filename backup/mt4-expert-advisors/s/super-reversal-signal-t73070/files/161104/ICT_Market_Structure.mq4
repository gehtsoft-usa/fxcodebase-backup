...
Version:     
Date:        
Repository:  Available @ https://fxcodebase.com/code/viewtopic.php?f=38&p=160829#p160829
License:     GNU
*/
...
input bool   zzOn                  = false;                 // Draw MS Line?
...
input color Text_color_Top    = LimeGreen; // Text color Top
input color Text_color_Bottom = Red;       // Text color Bottom
                ...
                const long &tick_volume[], const long &volume[], const int &spread[])
{
    ...
    if (newCandle.IsNewCandle()) {
        // Only process the last closed candle (i=1) to avoid repaint
        i = 1;
        LoadPoints(i, 5);
        LabelLastPoint();
        FindBMS(i);
...
double Value(int i, int shift, bool candle = false)
{
    ...
    int    arraySize = ArraySize(ExtZigzagBuffer);

    while (count != shift && i < arraySize) {
...
void LabelLastPoint()
{
    // Only draw labels if we have enough points and they are valid
    if (ArraySize(points) < 4) return;
    
    // Check if the first point (most recent) has valid data
    if (price(1) == 0 || price(2) == 0 || price(3) == 0) return;
...
double lastPriceUp, lastPriceDn;
void FindBMS(int i)
{
    if (i == 0) return;
    ...
    if (price(2) > price(4) && price(3) > price(5) && price(3) < price(4)) {
        if (price(1) < price(2) && price(1) < price(3)) {
            ...
            if(points[1].price != lastPriceDn)
            {
              lastPriceDn = points[1].price;
              BMSdn[i] = points[1].price;
            }
    ...
    if (price(2) < price(4) && price(3) < price(5) && price(3) > price(4)) {
        if (price(1) > price(2) && price(1) > price(3)) {
            DrawLabel(1, "up", "BOS");
            int _bar    = points[1].candle;
            if(points[1].price != lastPriceUp)
            {
              lastPriceUp = points[1].price;
              BMSup[i] = points[1].price;
            }
