...
//http://fxcodebase.com/code/viewtopic.php?f=38&t=70616
...
public:
   ...
   virtual double GetValue(const int period, double entryPrice)
   {
      if (_isBuy)
      {
         ...
         {
            double value = iCustom(_symbol, _timeframe, "Swing_ZZ", minBars, 2, period + i);
      ...
      else
      {
         for (int i = 0; i < 1000; ++i)
         {
            double value = iCustom(_symbol, _timeframe, "Swing_ZZ", minBars, 1, period + i);
            if (value != EMPTY_VALUE)
            {
               return value;
            }
...
input int K_periods = 5; // Stoch K
input int D_periods = 3; // Stoch D
input int Slowing = 3; // Stoch Slowing
input int stoch_os = 20; // Stoch oversold level
input int stoch_ob = 80; // Stoch overbought level
input int minBars = 2; // Min bars
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      ...
      return stochValue > stoch_os;
   }
};

class TrendLongCondition : public ACondition
{
public:
   TrendLongCondition(const string symbol, ENUM_TIMEFRAMES timeframe)
      :ACondition(symbol, timeframe)
   {
   ...
   bool IsPass(const int period, const datetime date)
   {
      double value = iCustom(_symbol, _timeframe, "Swing_ZZ", minBars, 0, period);
...
public:
   ...
   {
      double stochValue = iStochastic(_symbol, _timeframe, K_periods, D_periods, Slowing, MODE_SMA, 0, MODE_MAIN, period);
      return stochValue < stoch_ob;
   }
};

class TrendShortCondition : public ACondition
{
public:
   TrendShortCondition(const string symbol, ENUM_TIMEFRAMES timeframe)
      :ACondition(symbol, timeframe)
   {
...
ICondition* CreateLongCondition(string symbol, ENUM_TIMEFRAMES timeframe)
{
   ...
   condition.Add(new TrendLongCondition(symbol, timeframe), false);
...
ICondition* CreateShortCondition(string symbol, ENUM_TIMEFRAMES timeframe)
{
   ...
   condition.Add(new TrendShortCondition(symbol, timeframe), false);
   #ifdef ACT_ON_SWITCH_CONDITION
      ActOnSwitchCondition* switchCondition = new ActOnSwitchCondition(symbol, timeframe, (ICondition*) condition);
      condition.Release();
      return switchCondition;
...
int OnInit()
{
   double temp = iCustom(NULL, 0, "Swing_ZZ", minBars, 0, 0);
   if (GetLastError() == ERR_INDICATOR_CANNOT_LOAD)
   {
      Alert("Please, install the 'Swing_ZZ' indicator");
      return INIT_FAILED;
   }
   #ifdef SHOW_ACCOUNT_STAT
      stats = NULL;
