...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=73939
...
ActionCloseAll actionCloseAll(magico);
...
bool OnInit_GUI()
{
    ...
    if(gui.reason() != REASON_CHARTCHANGE && gui.reason() != REASON_TEMPLATE && gui.reason() != REASON_PARAMETERS)
    {
        ...
        res = gui.Create(0, "Panel Buy - Sell", 0, 400, 0, 590, altoPanel);
        if(res) gui.Run();
    }
    return res;
}
...
class BuyAction : public iActions
{
    bool doAction()
    {
        ...
        string ComentFromPanel = gui.eComent.Text() != "" ? gui.eComent.Text() : "";
        SendOrder = new SendNewOrder("buy", Lots(), "", pr, SL("buy", pr), TP("buy", pr), magico, ComentFromPanel);
...
class SellAction : public iActions
{
    ...
    {
        SendNewOrder* SendOrder;
        int distance = (int) gui.edit2.Text();
        double pr = distance == 0 ? 0 : Price("sell", distance, _Symbol);

        string ComentFromPanel = gui.eComent.Text() != "" ? gui.eComent.Text() : "";
        SendOrder = new SendNewOrder("sell", Lots(), "", pr, SL("sell", pr), TP("sell", pr), magico,ComentFromPanel);
        bool result = SendOrder.doAction();
        delete SendOrder;
        return result;
    }
...
class GUI : public CAppDialog
{
    ...
    iActions* buttonAction_CloseBuy;
    iActions* buttonAction_CloseSell;
    iActions* buttonAction_DeleteBuy;
    iActions* buttonAction_DeleteSell;
    iActions* buttonAction_DeleteAll;
    ...
    ~GUI()
    {
        ...
        delete buttonAction_CloseSell;
        delete buttonAction_CloseBuy;
        delete buttonAction_DeleteSell;
        delete buttonAction_DeleteBuy;
        delete buttonAction_DeleteAll;
    }


    CLabel  lbLots, lbSl, lbTp;
    CButton bt1, bt2, bt3, btCloseSell, btCloseBuy, btDeleteSell, btDeleteBuy, btDeleteAll;
    CEdit   edit1, edit2, eLots, eSl, eTp, eComent;

    void setButton1Action(iActions* action)       { button1Action = action; }
    void setButton2Action(iActions* action)       { button2Action = action; }
    void setButtonClosAllAction(iActions* action) { button3Action = action; }
    void setButtonCloseBuy(iActions* action)      { buttonAction_CloseBuy = action; }
    void setButtonCloseSell(iActions* action)     { buttonAction_CloseSell = action; }
    void setButtonDeleteBuy(iActions* action)     { buttonAction_DeleteBuy = action; }
    void setButtonDeleteSell(iActions* action)    { buttonAction_DeleteSell = action; }
    void setButtonDeleteAll(iActions* action)     { buttonAction_DeleteAll = action; }
    ...
    bool Create(const long chart, const string name, const int subwin, const int x1, const int y1, const int x2, const int y2)
    {
        ...
        if(!Create_button("BUY",          Col(0), Row(3), _high, _width, bt1)) return false;
        if(!Create_button("SELL",         Col(0), Row(4), _high, _width, bt2)) return false;
        if(!Create_button("Close Buys",   Col(0), Row(5), _high, _width, btCloseBuy)) return false;
        if(!Create_button("Close Sells",  Col(1), Row(5), _high, _width, btCloseSell)) return false;
        if(!Create_button("Close All",    Col(0), Row(6), _high, _widthFull, bt3)) return false;
        if(!Create_button("Delete Buys",  Col(0), Row(7), _high, _width, btDeleteBuy)) return false;
        if(!Create_button("Delete Sells", Col(1), Row(7), _high, _width, btDeleteSell)) return false;
        if(!Create_button("Delete All",   Col(0), Row(8), _high, _widthFull, btDeleteAll)) return false;

        if(!Create_Edit("Pips buy",       Col(1), Row(3), _high, _width, edit1)) return false;
        if(!Create_Edit("Pips sell",      Col(1), Row(4), _high, _width, edit2)) return false;

        if(!Create_Edit("eLots",         Col(1), Row(0), _high, _width, eLots)) return false;
        if(!Create_Edit("eSL",           Col(1), Row(1), _high, _width, eSl)) return false;
        if(!Create_Edit("eTP",           Col(1), Row(2), _high, _width, eTp)) return false;
        if(!Create_Edit("eComent", Col(0), Row(9), _high, _widthFull, eComent)) return false;
        eComent.Text("Simple Pannel");

        if(!Create_label("Lots:",        Col(0), Row(0), _high, _width, lbLots)) return false;
        if(!Create_label("SL (points):", Col(0), Row(1), _high, _width, lbSl)) return false;
        if(!Create_label("TP (points):", Col(0), Row(2), _high, _width, lbTp)) return false;
    ...
    void HoverEvents(const int id, const long& lparam, const double& dparam, const string& sparam)
    {
        ...
        if(btCloseBuy.IsActive()) btCloseBuy.ColorBackground(RoyalBlue); else btCloseBuy.ColorBackground(CONTROLS_BUTTON_COLOR_BG);        
        if(btCloseSell.IsActive()) btCloseSell.ColorBackground(RoyalBlue); else btCloseSell.ColorBackground(CONTROLS_BUTTON_COLOR_BG);
        if(btDeleteBuy.IsActive()) btDeleteBuy.ColorBackground(RoyalBlue); else btDeleteBuy.ColorBackground(CONTROLS_BUTTON_COLOR_BG);        
        if(btDeleteSell.IsActive()) btDeleteSell.ColorBackground(RoyalBlue); else btDeleteSell.ColorBackground(CONTROLS_BUTTON_COLOR_BG);
        if(btDeleteAll.IsActive()) btDeleteAll.ColorBackground(RoyalBlue); else btDeleteAll.ColorBackground(CONTROLS_BUTTON_COLOR_BG);
    }
    ...
    // void OnClick_button4() { buttonAction_CloseBuy.doAction(); }
    // void OnClick_button5() { buttonAction_CloseSell.doAction(); }
    // void OnClick_button6() { buttonAction_DeleteBuy.doAction(); }
    // void OnClick_button7() { buttonAction_DeleteSell.doAction(); }
    // void OnClick_button8() { buttonAction_DeleteAll.doAction(); }
...
ON_EVENT(ON_CLICK, btCloseBuy,   buttonAction_CloseBuy.doAction    )
ON_EVENT(ON_CLICK, btCloseSell,  buttonAction_CloseSell.doAction   )
ON_EVENT(ON_CLICK, btDeleteBuy,  buttonAction_DeleteBuy.doAction   )
ON_EVENT(ON_CLICK, btDeleteSell, buttonAction_DeleteSell.doAction  )
ON_EVENT(ON_CLICK, btDeleteAll,  buttonAction_DeleteAll.doAction   )
...
ActionCloseOrdersByType actionClosebuys("buy", magico);
ActionCloseOrdersByType actionClosesells("sell", magico);
...
ActionDeletePendings actDeletePendingsBuys("buy", magico);
ActionDeletePendings actDeletePendingsSells("sell", magico);
ActionDeletePendings actDeletePendings("all", magico);
...
gui.setButtonClosAllAction(&actionCloseAll);
gui.setButtonCloseBuy(&actionClosebuys);
gui.setButtonCloseSell(&actionClosesells);
gui.setButtonDeleteBuy(&actDeletePendingsBuys);
gui.setButtonDeleteSell(&actDeletePendingsSells);
gui.setButtonDeleteAll(&actDeletePendings);
...
double SL(string side, double price = 0)
{
    ...
    int uPips = userSLpips;
    
    double PipsFromPanel = (double) gui.eSl.Text();
    if(PipsFromPanel != 0) uPips = PipsFromPanel;

    if(stopLossOn)
    switch (modeSL)
    {
      case FixPips:
        levelSL = new Levels(new ByFixPips(_Symbol, side, uPips, "SL", price));
      ...
      case PipsFromOpenCandle:
        levelSL = new Levels(new ByPipsFromCandle(_Symbol, side, uPips, "SL", 0, 1));
        result  = levelSL.calculateLevel();
    }
  return result;
}
double TP(string side, double price = 0)
{
    ...
    int uPips = userTPpips;
    double PipsFromPanel = (double) gui.eTp.Text();
    if(PipsFromPanel != 0) uPips = PipsFromPanel;
    ...
    {
      case FixPips:
        levelTP = new Levels(new ByFixPips(_Symbol, side, uPips, "TP", price));
      ...
      case byMoney:
        ...
        break;
      case PipsFromOpenCandle:
        levelSL = new Levels(new ByPipsFromCandle(_Symbol, side, uPips, "TP", 0, 1));
        result  = levelSL.calculateLevel();
    }
  return result;
}
double Lots()
{
  ...
  double lotsFromPanel = (double)gui.eLots.Text();
  if(lotsFromPanel != 0) lots = lotsFromPanel;
