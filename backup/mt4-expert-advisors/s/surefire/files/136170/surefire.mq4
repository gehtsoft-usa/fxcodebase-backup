...
void OpenEntryOrder()
{
   ...
   double pipSize = instrument.GetPipSize();
   ...
   while (it.Next())
   {
      ...
      if (it.IsBuy())
      {
         pl = (OrderOpenPrice() - MathMin(original_stop, original_limit)) * OrderLots();
      }
      else
      {
         pl = (MathMax(original_stop, original_limit) - OrderOpenPrice()) * OrderLots();
      }
      total_profit += pl * mult / pipSize;
   }
   double order_amount = instrument.NormalizeLots(MathMax(0, -total_profit / limit) + amount);
...
void OnTick()
{
   ...
   if (trade == 0)
   {
      ...
      builder.SetInstrument(_Symbol)
         ...
         .SetStopLoss(direction ? Ask - stop * pipSize : Bid + stop * pipSize)
   ...
   if (original_rate == 0)
   {
      if (OrderSelect(trade, SELECT_BY_TICKET, MODE_TRADES))
      {
         ...
         if (last_side)
         {
            opposite_rate = original_rate - distance * pipSize;
         }
         else
         {
            opposite_rate = original_rate + distance * pipSize;
