...
//https://fxcodebase.com/code/viewtopic.php?f=38&t=71406
...
int CreateAlert1(int id, ENUM_TIMEFRAMES tf, color upColor, color downColor)
{
   AndCondition* upCondition = new AndCondition();
   upCondition.Add(new CloseCULCondition(_Symbol, tf), false);
   ICondition* oscCondition = new OscCondition(_Symbol, tf);
   upCondition.Add(oscCondition, true);
   ICondition* price = new Price1Condition(_Symbol, tf);
   upCondition.Add(new NotCondition(price), false);
   price.Release();
   ...
   downCondition.Add(new CloseCOHCondition(_Symbol, tf), false);
   downCondition.Add(oscCondition, true);
   oscCondition.Release();
   price = new Price2Condition(_Symbol, tf);
   upCondition.Add(new NotCondition(price), false);
   price.Release();
...
int CreateAlert2(int id, ENUM_TIMEFRAMES tf, color upColor, color downColor)
{
   AndCondition* upCondition = new AndCondition();
   upCondition.Add(new CloseCOHCondition(_Symbol, tf), false);
   upCondition.Add(new Price2Condition(_Symbol, tf), false);
   SetLastSignalAction* upAction = NULL;
   if (filter_consecutive)
   {
      upCondition.Add(new LastSignalNotCondition(1), false);
   ...
   AndCondition* downCondition = new AndCondition();
   downCondition.Add(new CloseCULCondition(_Symbol, tf), false);
   downCondition.Add(new Price1Condition(_Symbol, tf), false);
   SetLastSignalAction* downAction = NULL;
   if (filter_consecutive)
   {
      downCondition.Add(new LastSignalNotCondition(-1), false);
...
int CreateAlert3(int id, ENUM_TIMEFRAMES tf, color upColor, color downColor)
{
   AndCondition* upCondition = new AndCondition();
   upCondition.Add(new CloseCULCondition(_Symbol, tf), false);
   upCondition.Add(new OscCondition(_Symbol, tf), false);
   SetLastSignalAction* upAction = NULL;
   if (filter_consecutive)
   {
      upCondition.Add(new LastSignalNotCondition(1), false);
   ...
   AndCondition* downCondition = new AndCondition();
   downCondition.Add(new CloseCOHCondition(_Symbol, tf), false);
   downCondition.Add(new OscCondition(_Symbol, tf), false);
   SetLastSignalAction* downAction = NULL;
   if (filter_consecutive)
   {
      downCondition.Add(new LastSignalNotCondition(-1), false);
...
class CloseCULCondition : public ACondition
{
public:
   CloseCULCondition(const string symbol, ENUM_TIMEFRAMES timeframe)
      :ACondition(symbol, timeframe)
   {
   ...
   bool IsPass(const int period, const datetime date)
   {
      return iClose(_symbol, _timeframe, period) < plot2[period];
   }
};
class CloseCOHCondition : public ACondition
{
public:
   CloseCOHCondition(const string symbol, ENUM_TIMEFRAMES timeframe)
      :ACondition(symbol, timeframe)
   {
   ...
   bool IsPass(const int period, const datetime date)
   {
      return iClose(_symbol, _timeframe, period) > plot1[period];
   }
};
class OscCondition : public ACondition
{
public:
   OscCondition(const string symbol, ENUM_TIMEFRAMES timeframe)
      :ACondition(symbol, timeframe)
   {
   ...
   bool IsPass(const int period, const datetime date)
   {
      return osc[period] > volumeThresh;
   }
};
class Price1Condition : public ACondition
{
public:
   Price1Condition(const string symbol, ENUM_TIMEFRAMES timeframe)
      :ACondition(symbol, timeframe)
   {
   ...
   bool IsPass(const int period, const datetime date)
   {
      return iOpen(_symbol, _timeframe, period) - iClose(_symbol, _timeframe, period) < iHigh(_symbol, _timeframe, period) - iOpen(_symbol, _timeframe, period);
   }
};
class Price2Condition : public ACondition
{
public:
   Price2Condition(const string symbol, ENUM_TIMEFRAMES timeframe)
      :ACondition(symbol, timeframe)
   {
   ...
   bool IsPass(const int period, const datetime date)
   {
      return iOpen(_symbol, _timeframe, period) - iLow(_symbol, _timeframe, period) < iClose(_symbol, _timeframe, period) - iOpen(_symbol, _timeframe, period);
   }
};
...
int init()
{
   ...
   IndicatorObjPrefix = GenerateIndicatorPrefix(" Support and Resistance Levels with Breaks");
   IndicatorShortName(" Support and Resistance Levels with Breaks");
   IndicatorBuffers(28);
   int id = 0;
   ...
   pivothigh1 = new PivotHighStream(pivothigh1Source, leftBars, rightBars);
   fixnan2 = new FixnanStream(pivothigh1);
   pivotlow3Source = new SimplePriceStream(_Symbol, (ENUM_TIMEFRAMES)_Period, PriceLow);
   pivotlow3 = new PivotLowStream(pivotlow3Source, leftBars, rightBars);
   fixnan4 = new FixnanStream(pivotlow3);
   SetIndexStyle(id, DRAW_NONE);
   ...
   {
      id = CreateAlert1(id, (ENUM_TIMEFRAMES)_Period, up_color, down_color);
   }
   {
      id = CreateAlert2(id, (ENUM_TIMEFRAMES)_Period, up_color, down_color);
   }
   {
      id = CreateAlert3(id, (ENUM_TIMEFRAMES)_Period, up_color, down_color);
   }
                ...
                const int &spread[])
{
   ...
   for (int pos = MathMin(bars_limit, rates_total - 1 - MathMax(prev_calculated - 1, toSkip)); pos >= 0 && !IsStopped(); --pos)
   {
      ...
      double highUsePivot;
      if (!fixnan2.GetValue(pos, highUsePivot))
      {
         continue;
      }
      double lowUsePivot;
      if (!fixnan4.GetValue(pos, lowUsePivot))
      {
         continue;
      }
      plot1[pos] = highUsePivot;
      plot2[pos] = lowUsePivot;

      double short_ = iMAOnArray(vol, 0, 5, 0, MODE_EMA, pos);
      double long_ = iMAOnArray(vol, 0, 10, 0, MODE_EMA, pos);
      osc[pos] = 100 * (short_ - long_) / long_;
      if (customStream != NULL)
      {
