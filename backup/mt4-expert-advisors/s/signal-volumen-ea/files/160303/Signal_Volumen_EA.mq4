...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=76256
...
input double         uStopLossValue = 10;                // SL value (pips):
...
class BUYcondition1 : public iConditions
{
  public:
    bool evaluate()
    {
        ...
        if(iHigh(NULL, 0, 3) >= iHigh(NULL, 0, 2) || iHigh(NULL, 0, 2) >= iHigh(NULL, 0, 1))
            return false;
...
class SELLcondition1 : public iConditions
{
  public:
    bool evaluate()
    {
        ...
        if(iLow(NULL, 0, 3) <= iLow(NULL, 0, 2) || iLow(NULL, 0, 2) <= iLow(NULL, 0, 1))
            return false;
...
class ConditionBullTrend : public iConditions
{
    public:
    bool evaluate()
    {
        double close1    = iClose(NULL, 0, 1);
        double ema50_15m = iMA(NULL, PERIOD_M15, 50, 0, MODE_EMA, PRICE_CLOSE, 1);
        double ema50_H1  = iMA(NULL, PERIOD_H1, 50, 0, MODE_EMA, PRICE_CLOSE, 1);

        return (close1 > ema50_15m && close1 > ema50_H1 && ema50_15m > ema50_H1);
    }
};
ConditionBullTrend cdBullTrend;

class ConditionDownTrend : public iConditions
{
    public:
    bool evaluate()
    {
...
ConditionDownTrend cdDownTrend;
...
void BasicStrategy_Iniciate()
{
    ...
    LongStrategy.addCondition(&cdBullTrend);
    ShortStrategy.set(&conditionSell, &sell);
    ShortStrategy.addCondition(&cdDownTrend);
