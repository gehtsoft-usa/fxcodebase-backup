//Available @ https://fxcodebase.com/code/viewtopic.php?f=38&t=74374
...
input string IndicatorSection = ""; // == Indicator parameters ==
input int indi_period = 21; // period
input int indi_method = 3; // method
input int indi_price = 0; // price
input int indi_ATR = 1000; // ATR
input double indi_distance = 2.0; // distance

input string GeneralSection = ""; // == General ==
input string symbols = "EURUSD"; // Symbols to trade
...
input TradingMode entry_logic = TradingModeOnBarClose; // Entry logic
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      double value = iCustom(_Symbol, _Period, "SonicBlast", indi_period, indi_method, 
         indi_price, indi_ATR, 7, entry_logic == TradingModeLive ? 0 : 1);
...
public:
   ...
   {
      double value = iCustom(_Symbol, _Period, "SonicBlast", indi_period, indi_method, 
         indi_price, indi_ATR, 8, entry_logic == TradingModeLive ? 0 : 1);
      return value != 0 && value != EMPTY_VALUE;
   }
};
...
int OnInit()
{
   ...
   double temp = iCustom(NULL, 0, "SonicBlast", 0, 0);
   if (GetLastError() == ERR_INDICATOR_CANNOT_LOAD)
   {
      Alert("Please, install the 'SonicBlast' indicator");
      return INIT_FAILED;
   }
