...
// ------------------------------------------------------------------
input string IssrcSlow         = "== SSRC Slow Setup ==";  // == SSRC Slow Setup ==
bool         majorSignalON     = true;                     // Use Major Trend:
input int    slow_TotalBars    = 700;                      // TotalBars
input int    slow_SnakeRange   = 100;                      // SnakeRange
input int    slow_FilterPeriod = 100;                      // FilterPeriod
input double slow_MartFiltr    = 2;                        // MartFiltr
input int    slow_PriceConst   = 6;                        // PriceConst
input double majorTrendLevel   = 0.65;                     // Major Trend level cross signal
input string IssrcFast         = "== SSRC Fast Setup ==";  // == SSRC Fast Setup ==
bool         minorSignalON     = true;                     // Use Major Trend:
input int    fast_TotalBars    = 700;                      // TotalBars
input int    fast_SnakeRange   = 50;                       // SnakeRange
input int    fast_FilterPeriod = 100;                      // FilterPeriod
input double fast_MartFiltr    = 2;                        // MartFiltr
input int    fast_PriceConst   = 6;                        // PriceConst
input double minorTrendLevel   = 0.75;                     // Minor Trend level cross signal

input string tHeikenAshi = "== Heiken Ashi Setup ==";  // == Heiken Ashi Setup ==
input bool haFilterOn = false; // Heiken Ashi Filter On?
...
class SSRC : public iIndicators
{
   ...
   {
      int    setup0;  // TotalBars
      int    setup1;  // SnakeRange
      int    setup2;  // FilterPeriod
      double setup3;  // MartFiltr
      int    setup4;  // PriceConst
   };
   CustomParameters _setup;

  public:
   SSRC(string Symbol, int TimeFrame)
   {
      _symbol = Symbol;
      Print(__FUNCTION__," ","_symbol"," ",_symbol);
      _tf     = TimeFrame;
   ...
   SSRC* file(string setfile)
   {
      _file = setfile;
      return &this;
   }

   void setSetup(int set0, int set1, int set2, double set3, int set4)
   {
      _setup.setup0 = set0;
      _setup.setup1 = set1;
      _setup.setup2 = set2;
   ...
   {
      double value = iCustom(_symbol, _tf, _file, _setup.setup0, _setup.setup1, _setup.setup2,_setup.setup3,_setup.setup4, buffer, shift);
...
SSRC ssrcSlow(Symbol(), Period());
SSRC ssrcFast(Symbol(), Period());
...
class OrdersList
{
  ...
  public:
   ...
   int qnt()
   {
	...
	int      type(int index)           { if (notOverFlow(index)) { return orders[index].type(); } return -1; }
...
strategy buy:
1. when minor trend cross up level -0.75 
2. same time major trend cross up level -0.65 
3. heikin ashi candle colour is white (si buffer2 < buffer3)
...
class BUYcondition1 : public iConditions
{
  public:
   bool evaluate()
   {
      if (!majorSignalON) { return true; }

      if (ssrcSlow.calculate(0, 1) < majorTrendLevel) 
      {
         return true;
      }
...
class BUYcondition2 : public iConditions
{
  public:
   bool evaluate()
   {
      if (!minorSignalON) { return true; }
      if (ssrcFast.calculate(0, 1) > minorTrendLevel)
      {
         return true;
      }
      return false;
...
class BUYcondition3 : public iConditions
{
  public:
   bool evaluate()
   {
       if(!haFilterOn)return true;
       
       double open = iCustom(Symbol(), Period(), "Heiken Ashi.ex4", 2, 0);
      double close = iCustom(Symbol(),Period(),"Heiken Ashi.ex4",3,0);
		if(open < close)
		{
			return true;
		}
...
class SELLcondition1 : public iConditions
{
  public:
   ...
   {
      if (!majorSignalON) { return true; }
      if(ssrcSlow.calculate(0, 1) > majorTrendLevel) { return true; }
      return false;
   }
};
...
class SELLcondition2 : public iConditions
{
  public:
   bool evaluate()
   {
      ...
      if (ssrcFast.calculate(0, 1) < minorTrendLevel) { return true; }
      return false;
   }
};
...
class SELLcondition3 : public iConditions
{
  public:
   bool evaluate()
   {
       ...
       double open = iCustom(Symbol(), Period(), "Heiken Ashi.ex4", 2, 1);
       double close = iCustom(Symbol(),Period(),"Heiken Ashi.ex4",3,1);
  ...
  public:
   bool evaluate()
   {
      if (closeAllInOpositeSignal)
      {
         ...
         return ssrcSlow.calculate(0, 1) > majorTrendLevel;
      }
...
int OnInit()
{
   ...
   // NOTE: CONDITIONS TO OPEN TRADES (OnInit)
   //--- buys:
