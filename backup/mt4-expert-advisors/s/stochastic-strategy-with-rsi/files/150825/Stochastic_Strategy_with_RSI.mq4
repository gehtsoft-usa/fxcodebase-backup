...
//https://fxcodebase.com/code/viewtopic.php?f=38&t=71932
...
// NOTE: indicators setup

input string             Iema                    = "== Stochastic Setup ==";   // == Stochastic Setup ==
input double             stochLevelOB            = 80;                         // Stoch Level Over Bought:
input double             stochLevelOS            = 20;                         // Stoch Level Over Sold:
...
input double             RsiLevelBuy             = 50;                         // RSI Level to Buy ,over than:
input double             RsiLevelSell            = 50;                         // RSI Level to Sell ,under than:
...
class Stoch : public iIndicators
{
   string _symbol;
   int    _tf;
...
class RSI : public iIndicators
{
  ...
  public:
   ...
   RSI(string Symbol, int TimeFrame)
   {
      _symbol = Symbol;
      _tf     = TimeFrame;
      // setSetup(int Period, int AppliedPrice);
   }
   ~RSI() { ; }

   void setSetup(
       int                set0,
       ENUM_APPLIED_PRICE set1)
   {
      _setup.setup0 = set0;
      _setup.setup1 = (int)set1;
   }
...
class BUYcondition2 : public iConditions
{
  public:
   bool evaluate()
   {
      // TODO: condition Buy 2 . cualquiera de las dos lineas cruzó hacia abajo el level 20
      if ((stoch.main(1) < stochLevelOS && stoch.main(2) > stochLevelOS) || (stoch.signal(1) < stochLevelOS && stoch.signal(2) > stochLevelOS))
      {
         return true;
      } 
      // else if ((stoch.main(1) < stochLevelOS || stoch.signal(1) < stochLevelOS) && ((stoch.main(1) > stoch.signal(1)) && (stoch.main(2) < stoch.signal(2))))
      // {
      //    return true;
      // }
      return false;
...
class BUYcondition3 : public iConditions
{
  public:
   bool evaluate()
   {
      // TODO: condition Buy 3. cruce alcista
      if((stoch.main(1) > stoch.signal(1)) && (stoch.main(2) < stoch.signal(2)))
      {
         return true;
      }
      return false;
...
class SELLcondition2 : public iConditions
{
  public:
   bool evaluate()
   {
      // TODO: condition sell 2. cualquiera de las dos lineas cruzó hacia arriba el level 80
      if ((stoch.main(1) > stochLevelOB && stoch.main(2) < stochLevelOB) || (stoch.signal(1) > stochLevelOB && stoch.signal(2) < stochLevelOB))
      {
         return true;
      } 

      // else if ((stoch.main(1) > stochLevelOB || stoch.signal(1) > stochLevelOB) && ((stoch.main(1) < stoch.signal(1)) && (stoch.main(2) > stoch.signal(2))))
      // {
      //    return true;
      // }
      return false;
...
class SELLcondition3 : public iConditions
{
  public:
   bool evaluate()
   {
      // TODO: condition sell 3. cuce bajista
   if((stoch.main(1) < stoch.signal(1)) && (stoch.main(2) > stoch.signal(2)))
   {
      return true;
   }
...
int OnInit()
{
   ...
   rsi.setSetup(rsiPeriod, rsiAppliedPrice);
