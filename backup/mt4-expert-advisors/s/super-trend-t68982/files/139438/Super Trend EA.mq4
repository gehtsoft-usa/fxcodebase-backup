...
enum TakeProfitType
{
   ...
   input double take_profit_value_1 = 10; // Take profit value #1
   input double take_profit_1_close = 50; // % of close for TP #1
   input double take_profit_value = 20; // Take profit value #2
...
IStopLossStrategy* CreateStopLossStrategy(TradingCalculator* tradingCalculator, string symbol,
   ENUM_TIMEFRAMES timeframe, bool isBuy, StopLossType stopLossType, double stopLossValue)
{
   switch (stopLossType)
   {
      case SLDoNotUse:
...
MoneyManagementStrategy* CreateMoneyManagementStrategy(TradingCalculator* tradingCalculator, string symbol,
   ...
   StopLossType stopLossType, double stopLossValue, TakeProfitType takeProfitType, double takeProfitValue,
   double takeProfitAtrMultiplicator)
{
   ...
   {
      case PositionSizeRisk:
         stopLoss = CreateStopLossStrategy(tradingCalculator, symbol, timeframe, isBuy, stopLossType, stopLossValue);
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      double value = iCustom(_symbol, _timeframe, "Super_trend", inpPeriod, inpPrice, inpAtrPeriod, 2, period);
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      double value = iCustom(_symbol, _timeframe, "Super_trend", inpPeriod, inpPrice, inpAtrPeriod, 1, period);
      return value != 0 && value != EMPTY_VALUE;
   }
};
...
TradingController *CreateController(const string symbol, const ENUM_TIMEFRAMES timeframe, string &error)
{
   ...
   switch (take_profit_type)
   {
      ...
      switch (trailing_type)
      {
         ...
         case TrailingATR:
            {
               CreateATRTrailingAction* trailingAction = new CreateATRTrailingAction(symbol, timeframe, trailing_start, trailing_step, actions);
               orderHandlers.AddOrderAction(trailingAction);
               trailingAction.Release();
            }
            break;
...
int OnInit()
{
      ...
      stats = NULL;
   #endif
   double temp = iCustom(NULL, 0, "Super_trend", inpPeriod, inpPrice, inpAtrPeriod, 0, 0);
   if (GetLastError() == ERR_INDICATOR_CANNOT_LOAD)
   {
      Alert("Please, install the 'Super_trend' indicator");
      return INIT_FAILED;
   }
