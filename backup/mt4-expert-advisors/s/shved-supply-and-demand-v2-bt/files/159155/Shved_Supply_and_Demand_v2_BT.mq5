//Available @ https://fxcodebase.com/code/viewtopic.php?f=38&t=75912
   ...
   for(int i = 0; i < zone_count; i++)
     {
      if(zone_strength[i] == ZONE_VERIFIED && zone_show_verified == false)
         continue;
      if(zone_strength[i] == ZONE_PROVEN && zone_show_proven == false)
         continue;
         ...
         for(i = 0; i < merge_count; i++)
           {
            ...
            if(temp_hits[target] == 0 && temp_turn[target] == false)
              {
               temp_hits[target] = 1;
               if(temp_strength[target] < ZONE_VERIFIED && zone_show_verified)
      ...
      if(zone_type[i] == ZONE_SUPPORT)
        {
         ...
         if(zone_strength[i] == ZONE_TURNCOAT)
            ...
            if(zone_strength[i] == ZONE_PROVEN && zone_show_proven)
               ObjectSetInteger(0, s, OBJPROP_COLOR, color_support_proven);
            else
               if(zone_strength[i] == ZONE_VERIFIED && zone_show_verified)
