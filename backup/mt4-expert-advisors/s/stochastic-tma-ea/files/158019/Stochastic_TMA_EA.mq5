...
input    string  s1  = "==============================Setting   of  Moving Average ==============================" ;
input  int  MA_PERIOD   =  10  ;
input    ENUM_MA_METHOD  MA_METHOD   = MODE_SMA;
input   ENUM_APPLIED_PRICE   MA_APPLIED_PERIOD   =   PRICE_CLOSE;

input string   LabelSto            = "====================Stochastic Settings=================";
...
input    ENUM_MA_METHOD  STOCHASTIC_MA_METHOD   = MODE_SMA;  //  Stochastic  MA Method
...
public:
   ...
   {
      refs = 1;
      tma = iCustom(symbol, timeframe, "TMA+CG mladen", HalfLength, Price, BandsDeviations);
      if (tma == INVALID_HANDLE)
      {
         Print("Please, install TMA+CG mladen");
      }
      ma = iMA(symbol, timeframe, MA_PERIOD, 0, MA_METHOD, MA_APPLIED_PERIOD);
      stoch = iStochastic(symbol, timeframe, KPeriod, DPeriod, Slowing, STOCHASTIC_MA_METHOD, STO_CLOSECLOSE);
   }
   ~EntryStreamData()
   {
      IndicatorRelease(tma);
      IndicatorRelease(ma);
   ...
   double GetStochD(int period)
   {
      double buffer[1];
      if (CopyBuffer(stoch, 0, period, 1, buffer) == 1)
      {
         return buffer[0];
      }
      return EMPTY_VALUE;
   }
   double GetStochK(int period)
   {
      double buffer[1];
      if (CopyBuffer(stoch, 1, period, 1, buffer) == 1)
      {
         return buffer[0];
      }
      return EMPTY_VALUE;
   ...
   double GetTMA(int period)
   {
      double buffer[1];
      if (CopyBuffer(tma, 0, period, 1, buffer) == 1)
      {
         return buffer[0];
      }
      return EMPTY_VALUE;
...
public:
   ...
   virtual bool IsPass(const int period, const datetime date)
   {
      return data.GetStochK(period) > data.GetStochD(period);
   }
};
class EntryLongFilterCondition : public ACondition
{
   EntryStreamData* data;
public:
   EntryLongFilterCondition(string symbol, ENUM_TIMEFRAMES timeframe, EntryStreamData* data)
      :ACondition(symbol, timeframe)
   {
      this.data = data;
      data.AddRef();
   }
   ~EntryLongFilterCondition()
   {
      data.Release();
   }
   ...
   virtual bool IsPass(const int period, const datetime date)
   {
      ...
      return data.GetStochK(period) < stochastic_buy_level
         && price >= data.GetMA(period)
         && data.GetTMA(period) >= price;
   }
};
...
class EntryShortFilterCondition : public ACondition
{
   EntryStreamData* data;
public:
   EntryShortFilterCondition(string symbol, ENUM_TIMEFRAMES timeframe, EntryStreamData* data)
      :ACondition(symbol, timeframe)
   {
      this.data = data;
      data.AddRef();
   }
   ~EntryShortFilterCondition()
   {
      data.Release();
   }
   ...
   {
      double price = iClose(_symbol, _timeframe, period);
      return data.GetStochK(period) > stochastic_sell_level
         && price <= data.GetMA(period)
         && data.GetTMA(period) <= price;
   }
};
...
ICondition* CreateLongFilterCondition(string symbol, ENUM_TIMEFRAMES timeframe, EntryStreamData* data)
{
   ...
   AndCondition* condition = new AndCondition();
   condition.Add(new EntryLongFilterCondition(symbol, timeframe, data), false);
   return condition;
}
...
ICondition* CreateShortFilterCondition(string symbol, ENUM_TIMEFRAMES timeframe, EntryStreamData* data)
{
   ...
   AndCondition* condition = new AndCondition();
   condition.Add(new EntryShortFilterCondition(symbol, timeframe, data), false);
   return condition;
}
