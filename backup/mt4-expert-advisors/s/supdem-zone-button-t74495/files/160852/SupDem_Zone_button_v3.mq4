...
Version:     
Date:        
Repository:  Available @ https://fxcodebase.com/code/viewtopic.php?f=38&p=160736#p160736
License:     GNU
*/
...
int init()
{
    ...
    if(GlobalVariableGet(IndicatorName + "_visibility", val))
        ...
        // put init() here
        SetIndexBuffer(0, BuferDn); // Pone un OB arriba del precio actual
...
int start()
{
    ...
    if(NewBar() == true) {
        ...
        GetValid();
        Draw();
        FindRectanglesAboveBelowPrice(PriceZoneUP, PriceZoneDN);
        if(Low[0] < PriceZoneDN && Close[0] > PriceZoneDN) 
        {
            ArrowUp[0] = Low[0];             
            Notifications(1);
        }    
        if(High[0] >= PriceZoneUP && Close[0] < PriceZoneUP) 
        {
            ArrowDn[0] = High[0];
            Notifications(0);
        }    
...
void FindRectanglesAboveBelowPrice(double &PriceZoneUP, double &PriceZoneDN)
{
    PriceZoneUP = 0.0;
    PriceZoneDN = 0.0;
    ...
    for(int i = 0; i < total; i++)
    {
        ...
        if(ObjectGetInteger(0, objName, OBJPROP_TYPE) == OBJ_RECTANGLE)
        {
            ...
            // Rectángulo arriba del precio actual
            if(rectLow > price && rectLow < minAbove)
            {
                minAbove = rectLow;
                PriceZoneUP = rectLow;
                Print("line: ",__LINE__," PriceZoneUP: ",PriceZoneUP);
            }
            // Rectángulo abajo del precio actual
            if(rectHigh < price && rectHigh > maxBelow)
            {
                maxBelow = rectHigh;
                PriceZoneDN = rectHigh;
                Print("line: ",__LINE__," PriceZoneDN: ",PriceZoneDN);
            }
        }
    }
}
