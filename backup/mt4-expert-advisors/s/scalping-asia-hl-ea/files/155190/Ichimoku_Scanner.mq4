//Available @  https://fxcodebase.com/code/viewtopic.php?f=38&t=74825
...
// ------------------------------------------------------------------
input string       Ttf = "== Time Frames ==";      // Select Time Frames
input bool USE_M1 = true; // USE M1 ?
...
void PrintArray(ENUM_TIMEFRAMES &array[])
{
      for (int i = 0; i < ArraySize(array); i++) {
          Print("TF ", i," ",GetTimeFrame(array[i]));
      }
}
  ...
  public:
    ...
    void Create()
    {
        ...
        for (int i = 0; i < _nRows; i++) {
            ...
            // TODO: nro de tf que seleciono el user
            for (int j = 1; j < ArraySize(TF) + 1; j++) {
                _rows[i, j].Name("celda_" + i + "_" + j);
                _rows[i, j].Text("");
                if (i > 0)
                    _rows[i, j].Font("Wingdings 3", 8);
    ...
    // TODO: los headers dependen de los tf que se selccionen
    ...
    void setValues(int rowNumber, string& values[])
    {
        for (int j = 0; j < ArraySize(values); j++) {
            ...
            if (values[j] == 0) {
                ...
                _rows[rowNumber + 1, j + 1].Font("Wingdings", 8);
                _rows[rowNumber + 1, j + 1].Text("l");
            ...
            // PullBack Format
            if (values[j] == 2) {
                _rows[rowNumber + 1, j + 1].Font("Wingdings 3");
                _rows[rowNumber + 1, j + 1].Text("v");
...
void addTF(ENUM_TIMEFRAMES newtf)
{
    int t = ArraySize(TF);
    if (ArrayResize(TF, t + 1)) {
        TF[t] = newtf;
    }

    t = ArraySize(headers);
    if (t == 0) {
        if (ArrayResize(headers, t + 2)) {
            headers[t]     = " ";
            headers[t + 1] = GetTimeFrame(newtf);
        }
    } else if (ArrayResize(headers, t + 1)) {
        headers[t] = GetTimeFrame(newtf);
    }
    // PrintArray(headers);
    // PrintArray(TF);
}
...
{
    for (int i = 0; i < ArraySize(ichimokus); i++) {
        // TODO: los values a levantar depende de la cantidad de tf
        int    t = ArraySize(TF);
        string values[];
        ArrayResize(values, t);

        for (int j = 0; j < ArraySize(TF); j++) {

            int currentShift = liveMode == true ? 0 : 1;
            int previusShift = liveMode == true ? 1 : 2;

            // double tenkan = ichimokus[i].Tenkansen(TF[j], currentShift);
            double kinjun = ichimokus[i].Kijunsen(TF[j], currentShift);

            // double prev_tenkan = ichimokus[i].Tenkansen(TF[j], previusShift);
            double prev_kinjun = ichimokus[i].Kijunsen(TF[j], previusShift);

            int trend = -1;

            // Up
            // when the price is above the tenkan,
            double ask = SymbolInfoDouble(ichimokus[i].symbol(), SYMBOL_ASK);

            // the tenkan above the kijun
            // if(ask > tenkan && tenkan > kinjun && tenkan > prev_tenkan && kinjun > prev_kinjun)
            if (kinjun == prev_kinjun) {
                trend = 0;
            }
            // Down
            // if(ask < tenkan && tenkan < kinjun && tenkan < prev_tenkan && kinjun < prev_kinjun)
