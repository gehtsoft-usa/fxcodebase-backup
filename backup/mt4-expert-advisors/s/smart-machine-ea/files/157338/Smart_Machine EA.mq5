//Available @  https://fxcodebase.com/code/viewtopic.php?f=38&t=75371
...
double       grid_close_profit = 0;                  // Profit to close grid (0 = off):
double       grid_close_loss   = 0;                    // Loss to close grid (0 = off):
...
#define Section_CloseByProfit
#ifdef Section_CloseByProfit
// Mark: Section_CloseByProfit

input string tclosebyprofit  = "== Close All by profit =="; // ————————————————————————
input bool   tpbyprofit_on   = false;                       // Close by profit ON:
input double tpbyprofit      = 10;                          // Profit (zero = off):
input double tpbyloss        = 0;                          // Loss (zero = off):


class ConditionCloseByProfit : public iConditions
{
  public:
    bool evaluate()
    {

        if(!tpbyprofit_on) return false;
        ...
        for (int i = PositionsTotal(); i >= 0; i--) {
            ...
            if (PositionGetSymbol(i) == Symbol() && PositionGetInteger(POSITION_MAGIC) == magico) {
                ...
                Print("line: ",__LINE__," profit: ",profit);
            }
        }
        
        if(tpbyprofit > 0 && profit >= tpbyprofit) return true;
        if(tpbyloss > 0 && -profit >= tpbyloss) return true;
        
        return false;
    }
};
ConditionCloseByProfit cdCloseByProfit;

Strategy stCloseByProfit;

void OnInit_CloseByProfit()
{
    stCloseByProfit.set(&cdCloseByProfit, &acCloseAll);
    Trader.strategysWithOutFilters.add(&stCloseByProfit);
}
