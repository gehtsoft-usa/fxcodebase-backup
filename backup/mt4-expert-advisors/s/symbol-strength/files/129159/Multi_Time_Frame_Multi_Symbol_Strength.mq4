...
int start()
{
   ...
   while (pos >= 0)
   {
      int indexPos = timeframe == PERIOD_CURRENT || timeframe == (ENUM_TIMEFRAMES)_Period || pos == 0
         ? pos
         : iBarShift(_Symbol, timeframe, Time[pos]);
      for (int i = 0; i < ArraySize(lines); ++i)
      {
         ...
         for (int index = 0; index < symbolsCount; index++)
         {
            ...
            double day_high = iHigh(symbolData.Symbol, timeframe, indexPos);
            double day_low = iLow(symbolData.Symbol, timeframe, indexPos);
            double curr_bid = iClose(symbolData.Symbol, timeframe, indexPos);
