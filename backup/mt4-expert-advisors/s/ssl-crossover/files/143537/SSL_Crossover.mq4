...
//https://fxcodebase.com/code/viewtopic.php?f=38&t=71493
...
#property indicator_label1 "Down SSL"
#property indicator_type1 DRAW_LINE
#property indicator_color1 Blue
#property indicator_style1 STYLE_SOLID
#property indicator_width1 1
#property indicator_label2 "Up SSL"
...
input int sslLen = 10; // SSL Length
input int bars_limit = 100000; // Bars limit
double Hlv[], sslDown[], sslUpR[];
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      return sslDown[period] > sslUpR[period];
   }
};
...
int init()
{
   ...
   IndicatorObjPrefix = GenerateIndicatorPrefix("SSL-CROSS");
   IndicatorShortName("SSL Crossover + MA choice");
   IndicatorBuffers(3 + 8);
   SetIndexBuffer(0, sslDown);
   SetIndexBuffer(1, sslUpR);
                ...
                const int &spread[])
{
   ...
   {
      ArrayInitialize(sslDown, EMPTY_VALUE);
      ArrayInitialize(sslUpR, EMPTY_VALUE);
   ...
   {
      double smaHigh = iMA(_Symbol, _Period, sslLen, 0, maType, PRICE_HIGH, pos);
      double smaLow = iMA(_Symbol, _Period, sslLen, 0, maType, PRICE_LOW, pos);
      Hlv[pos] = ((close[pos] > smaHigh) ? 1 : ((close[pos] < smaLow) ? (-1) : Hlv[pos + 1]));
      sslUpR[pos] = ((Hlv[pos] < 0) ? smaHigh : smaLow);
      sslDown[pos] = ((Hlv[pos] < 0) ? smaLow : smaHigh);
