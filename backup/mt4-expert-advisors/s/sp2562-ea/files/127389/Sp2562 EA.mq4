...
// http://fxcodebase.com/code/viewtopic.php?f=38&t=68673
...
extern int sma_Fast_MA_Period = 1;
extern int sma_Slow_MA_Period = 34;
extern int sma_Signal_period = 5;

extern int     sar_Price             =     0;    //Price Mode (0...6)
extern int     sar_HiLoMode          =     0;    //0-off,1-on 
extern double  sar_StartAF           =  0.02;    //Start value of Acceleration Factor
extern double  sar_MinStep           =  0.00;    //Minimum Acceleration Factor increment 
extern double  sar_MaxStep           =  0.02;    //Maximum Acceleration Factor increment 
extern double  sar_MaxAF             =   0.2;    //Maximum value of Acceleration Factor   
extern int     sar_AdaptiveMode      =     1;    //0-off,1-Kaufman,2-Ehlers
extern int     sar_AdaptiveSmooth    =     5;    //Adaptive Smoothing Period 
extern double  sar_Filter            =   0.0;    //Filter in pips
extern double  sar_MinChange         =   0.0;    //Min Change in pips  
extern double  sar_IndicatorValue    =     1;    //Indicator Value (ex.1.0)
...
public:
   ...
   {
      double smaValue = iCustom(_symbol, _timeframe, "SMA CrossOver_Justin_Alert", false, false, 
         sma_Fast_MA_Period, sma_Slow_MA_Period, sma_Signal_period, 1, period);
      double sar = iCustom(_symbol, _timeframe, "AdaptiveParabolic Histo_v2.1", _timeframe, 
         sar_Price, sar_HiLoMode, sar_StartAF, sar_MinStep, sar_MaxStep, sar_MaxAF, 
         sar_AdaptiveMode, sar_AdaptiveSmooth, sar_Filter, sar_MinChange, sar_IndicatorValue, 0, period);
      return smaValue > 0 && sar > 0;
   }
};
...
public:
   ...
   {
      double smaValue = iCustom(_symbol, _timeframe, "SMA CrossOver_Justin_Alert", false, false, 
         sma_Fast_MA_Period, sma_Slow_MA_Period, sma_Signal_period, 0, period);
      double sar = iCustom(_symbol, _timeframe, "AdaptiveParabolic Histo_v2.1", _timeframe, 
         sar_Price, sar_HiLoMode, sar_StartAF, sar_MinStep, sar_MaxStep, sar_MaxAF, 
         sar_AdaptiveMode, sar_AdaptiveSmooth, sar_Filter, sar_MinChange, sar_IndicatorValue, 1, period);
...
int OnInit()
{
   double temp = iCustom(NULL, 0, "SMA CrossOver_Justin_Alert", 0, 0);
   if (GetLastError() == ERR_INDICATOR_CANNOT_LOAD)
   {
      Alert("Please, install the 'SMA CrossOver_Justin_Alert' indicator");
      return INIT_FAILED;
   }
   temp = iCustom(NULL, 0, "AdaptiveParabolic Histo_v2.1", 0, 0);
   if (GetLastError() == ERR_INDICATOR_CANNOT_LOAD)
   {
      Alert("Please, install the 'AdaptiveParabolic Histo_v2.1' indicator");
      return INIT_FAILED;
   }
