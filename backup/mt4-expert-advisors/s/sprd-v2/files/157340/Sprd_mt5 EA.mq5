...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=75127&p=157340#p157340
...
input bool   closeAll_on  = true;                   // Close All on?
input double profit_limit = 50;                     // Profit Limit (0 = off)
input double loss_limit   = 0;                     // Loss Limit (0 = off)
...
class ConditionCloseByMoney : public iConditions
{
  public:
    bool evaluate()
    {
        ...
        if (current_profit >= profit_limit && profit_limit > 0) return true;
        if (current_profit < 0 && -current_profit >= loss_limit && loss_limit > 0) return true;
        return false;
    }
};
...
input const double grid_step      = 20;                   // Grid Step:
...
BUYcondition1 conditionNoTrades;

class ActionGrids : public iActions
{
  public:
    bool execute()
    {
        ...
        for (int i = 1; i <= grid_max_steps; i++) {
            double price = Ask + (grid_step * 10 * _Point) * i;
            double vol   = uLotsValue * grid_m * i;
            int    r     = trade.BuyStop(vol, price, _Symbol, 0, 0);
        }
        for (int i = 1; i <= grid_max_steps; i++) {
            double price = Bid - (grid_step * 10 * _Point) * i;
            double vol   = uLotsValue * grid_m * i;
            int    r     = trade.SellStop(vol, price, _Symbol, 0, 0);
        }
...
Strategy GridsStrategy;

void BasicStrategy_Iniciate()
{
    GridsStrategy.set(&conditionNoTrades, &acGrids);
    Trader.strategys.add(&GridsStrategy);
}
