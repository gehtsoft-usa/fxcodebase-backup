...
//https://fxcodebase.com/code/viewtopic.php?f=38&t=72343
...
// #include <FrameWork/DrawObjects/DrawHLines.mqh>
class HLine
{
  string   _name;
  datetime _iniTm;
...
HLine* clr(color clr)
{
  ...
  void changeColor(color clr)
  {
	...
	bool isSelected()
	{
  	return (bool)ObjectGetInteger(0, _name, OBJPROP_SELECTED);
	}
...
input string T1                    = "== Price Alert ==";    // -----------------------
input double alertPrice            = 1.2;                    // Alert Price
input color  activeClr             = clrGreen;               // Color when alert is active
input color  inactiveClr           = clrRed;                 // Color when alert is inactive
...
//+------------------------------------------------------------------+
extern string    te                = "== Telegram Inputs == ";                          //----------------------
...
string           InpTemplates      = "Gris;ADX;BollingerBands;Momentum";                // List of Templates:
string           temple;                                                                // Template to telegram photo:
string           InpChannelName = "ttrPruebas";                                         // Channel Name:
input long       Channel_id     = -1001779335873;                                       // Channel ID:
input bool       sendPic        = false;                                                // Send Pics by Telegram:
input string     userMsg        = "the alert price was reached ";                       // User Message:
//---
CComment            commentA;
CMyBot              bot;
...
int OnInit()
{
  ...
  line = new HLine("Alert", 0, alertPrice, activeClr);
  line.draw();

  //+------------------------------------------------------------------+
  // TELEGRAM ON INIT *****
  //+------------------------------------------------------------------+
  //--- stop working in tester mode
  ...
  if (ChartGetInteger(0, CHART_SHOW_ONE_CLICK))
    y = 500;
  commentA.Create("myPanel", 900, 420);
  commentA.SetColor(clrDimGray, clrBlack, 200);
...
void setSide()
{
  // tomar el precio de la linea
  // si el precio de la linea es superior a bid y ask, side: up
  // si el precio de la linea es inferior a bid y ask, side: down
  // si el side es up: alerta cuando el precio pasa hacia arriba
  // si el side es dn:alerta cuando el precio pasa hacia abajo
...
double CurrentPrice()
{
  if (line.isSelected()) return 0;

  double currPrice = NormalizeDouble(line.linePrice(),_Digits);
  if (currPrice != line.price())
  {
    line.price(currPrice);
    alertOn = true;
    ObjectSetInteger(0, "Alert", OBJPROP_COLOR, activeClr);
  }
  return currPrice;
}
...
void checkAlert()
{
  ...
  if (side == up)
  {
    if (Bid() >= CurrentPrice())
    {
      // SendAlert();
      alertOn = false;
      ObjectSetInteger(0, "Alert", OBJPROP_COLOR, inactiveClr);
      string msg = "Price Reached: " + (string)NormalizeDouble(line.linePrice(),_Digits) + " " + _Symbol + "\n" + userMsg;

      Print(msg);
      if (telegramOn) EnviarTelegram(msg);
      if (NotificationsOn) Notifications(msg);
    }
  }
  if (side == down)
  {
...
void lineColor()
{
  if (alertOn == true)
  {
    line.changeColor(activeClr);
  } else
  {
    line.changeColor(inactiveClr);
  }
}
...
void EnviarTelegram(string msg)
{
  if (sendPic)
  {
    bot.SendScreenShot(Channel_id, Symbol(), 0, temple);
  }
  int error = bot.SendMessage(Channel_id, msg);
  if (error != 0)
  {
    Print("error al enaviar telegram: ", error);
  }
}
