...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=71475
...
class UpSTAlertCondition : public ACondition
{
public:
   UpSTAlertCondition(const string symbol, ENUM_TIMEFRAMES timeframe)
      :ACondition(symbol, timeframe)
   {
   ...
   {
      double buffer[1];
      if (CopyBuffer(st, 0, period, 1, buffer) != 1)
      {
         return false;
      }
      return buffer[0] < iLow(_symbol, _timeframe, period);
   }
};

class DownSTAlertCondition : public ACondition
{
public:
   DownSTAlertCondition(const string symbol, ENUM_TIMEFRAMES timeframe)
      :ACondition(symbol, timeframe)
   {
   ...
   bool IsPass(const int period, const datetime date)
   {
      ...
      return buffer[0] > iHigh(_symbol, _timeframe, period);
   }
};

class UpMAAlertCondition : public ACondition
{
public:
   UpMAAlertCondition(const string symbol, ENUM_TIMEFRAMES timeframe)
      :ACondition(symbol, timeframe)
   {
   ...
   {
      double buffer[1];
      if (CopyBuffer(ma, 0, period, 1, buffer) != 1)
      {
         return false;
      }
      return buffer[0] < iLow(_symbol, _timeframe, period);
...
class DownMAAlertCondition : public ACondition
{
public:
   DownMAAlertCondition(const string symbol, ENUM_TIMEFRAMES timeframe)
      :ACondition(symbol, timeframe)
   {
...
class UpDirAlertCondition : public ACondition
{
public:
   UpDirAlertCondition(const string symbol, ENUM_TIMEFRAMES timeframe)
      :ACondition(symbol, timeframe)
   {
...
class DownDirAlertCondition : public ACondition
{
public:
   DownDirAlertCondition(const string symbol, ENUM_TIMEFRAMES timeframe)
      :ACondition(symbol, timeframe)
   {
...
int OnInit(void)
{
   st = iCustom(_Symbol, _Period, "Super_Trend", inpPeriod, inpPrice, inpAtrPeriod);
   ma = iMA(_Symbol, _Period, ma_period, 0, ma_method, PRICE_CLOSE);
   IndicatorObjPrefix = GenerateIndicatorPrefix("stmas");
   IndicatorSetString(INDICATOR_SHORTNAME, "stmas");
   ...
   {
      AndCondition* upConditionAnd = new AndCondition();
      AndCondition* downConditionAnd = new AndCondition();
      ICondition* upCondition = upConditionAnd;
      ICondition* downCondition = downConditionAnd;
      upConditionAnd.Add(new UpSTAlertCondition(_Symbol, timeframe), false);
      upConditionAnd.Add(new UpMAAlertCondition(_Symbol, timeframe), false);
      upConditionAnd.Add(new UpDirAlertCondition(_Symbol, timeframe), false);
      downConditionAnd.Add(new DownSTAlertCondition(_Symbol, timeframe), false);
      downConditionAnd.Add(new DownMAAlertCondition(_Symbol, timeframe), false);
      downConditionAnd.Add(new DownDirAlertCondition(_Symbol, timeframe), false);
      #ifdef ACT_ON_SWITCH
