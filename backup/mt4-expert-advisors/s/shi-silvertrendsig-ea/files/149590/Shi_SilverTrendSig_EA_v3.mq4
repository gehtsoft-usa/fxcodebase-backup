...
string file_custom_indicator = "Shi_SilverTrendSig-Indicator.ex4";
...
input int     Otstup     = 30;
input double  Per        = 9;
input int     tolerance  = 1;                    // Tolerance Candles Back for signal retracement:
...
class BUYcondition1 : public iConditions
{
 ...
 public:
  bool evaluate()
  {
    ...
    {	
			arrow = iCustom(NULL, 0, file_custom_indicator, AllBars, Otstup, Per, 1, i);
    	if(arrow != EMPTY_VALUE && arrow!= _lastSignal) 
			{
        _lastSignal = arrow;
				return true;
      }
...
class SELLcondition1 : public iConditions
{
 ...
 public:
  bool evaluate()
  {
    ...
    for (int i = 1; i <= tolerance; i++)
    {	
			arrow = iCustom(NULL, 0, file_custom_indicator, AllBars, Otstup, Per, 0, i);
    	if(arrow != EMPTY_VALUE && arrow!= _lastSignal)
			{
        _lastSignal = arrow;
				return true;
...
class ConditionToCloseBuy : public iConditions
{
 public:
  bool evaluate()
  {
    ...
    {
      return sellCondition2.evaluate();
    }
...
int OnInit()
{
  ...
  buyCondition2 = new BUYcondition2();
  ...
  sellCondition2 = new SELLcondition2();
...
// NOTE: martingale tick
if(martingaleOn)
if(martin.lastWasLoss()==true && OrdersTotal()==0)
{
	// necesito la direction del ultimo
    ...
    string type = lastType == OP_BUY ? "buy" : "sell";
    
		// calcular el nuevo lotaje y manda la orden:
    actionSendOrder = new SendNewOrder(type, martin.lots(Lots()), "", 0, SL(type), TP(type), magico);
		if (actionSendOrder.doAction())
		{
      if (type == "buy") Notifications(0); else Notifications(1);
