...
// http://fxcodebase.com/code/viewtopic.php?f=38&t=68696
...
input string Indi1Params = ""; // == Semaphor 3Level ZZ parameters ==
input double indi1_Period_1    = 5; // Period 1
input double indi1_Period_2    = 13; // Period 2
input double indi1_Period_3    = 34; // Period 3
input int    indi1_Deviation_1 = 1; // Deviation 1
input int    indi1_Step_1      = 3; // Step 1
input int    indi1_Deviation_2 = 8; // Deviation 2
input int    indi1_Step_2      = 5; // Step 2
input int    indi1_Deviation_3 = 21; // Deviation 3
input int    indi1_Step_3      = 12; // Step 3

input string Indi2Params = ""; // == zig-zag-buy-sell parameters == 
input int Indi2KeelOver=55;
input int Indi2Slalom=17;

input string Indi3Params = "";// == QQE with Alerts parameters == 
input int Indi3SF=5;
...
public:
   ...
   {
      double value0_0 = iCustom(_symbol, _timeframe, "QQE with Alerts", Indi3SF, 0, period);
      double value1_0 = iCustom(_symbol, _timeframe, "QQE with Alerts", Indi3SF, 1, period);
      double value0_1 = iCustom(_symbol, _timeframe, "QQE with Alerts", Indi3SF, 0, period + 1);
      double value1_1 = iCustom(_symbol, _timeframe, "QQE with Alerts", Indi3SF, 1, period + 1);
      if (value0_0 < value1_0 || value0_1 > value1_1)
         return false;
      ...
      for (int i = period; i < 1000; ++i)
      {
         if (semaphoreIndex == -1)
         {
            double valueSell = iCustom(_symbol, _timeframe, "Semaphor_3Level_ZZ", indi1_Period_1, indi1_Period_2, indi1_Period_3,
               indi1_Deviation_1, indi1_Step_1, indi1_Deviation_2, indi1_Step_2, indi1_Deviation_3, indi1_Step_3, 5, period + i);
            if (valueSell != 0.0)
               return false;
            double valueBuy = iCustom(_symbol, _timeframe, "Semaphor_3Level_ZZ", indi1_Period_1, indi1_Period_2, indi1_Period_3,
               indi1_Deviation_1, indi1_Step_1, indi1_Deviation_2, indi1_Step_2, indi1_Deviation_3, indi1_Step_3, 4, period + i);
            if (valueBuy != 0.0)
               semaphoreIndex = i;
         }
         if (zzIndex == -1)
         {
            double zzValueSell = iCustom(_symbol, _timeframe, "zig-zag-buy-sell", Indi2KeelOver, Indi2Slalom, 6, period + i);
            if (zzValueSell != 0.0)
               return false;
            double zzValueBuy = iCustom(_symbol, _timeframe, "zig-zag-buy-sell", Indi2KeelOver, Indi2Slalom, 5, period + i);
            if (zzValueBuy != 0.0)
               zzIndex = i;
         }
         if (semaphoreIndex != -1 && zzIndex != -1)
            break;
      }
      
      return semaphoreIndex != -1 && zzIndex != -1 && semaphoreIndex >= zzIndex;
   }
};
...
int OnInit()
{
   double temp = iCustom(NULL, 0, "QQE with Alerts", 0, 0);
   if (GetLastError() == ERR_INDICATOR_CANNOT_LOAD)
   {
      Alert("Please, install the 'QQE with Alerts' indicator");
      return INIT_FAILED;
   }
   ...
   temp = iCustom(NULL, 0, "zig-zag-buy-sell", 0, 0);
   if (GetLastError() == ERR_INDICATOR_CANNOT_LOAD)
   {
      Alert("Please, install the 'zig-zag-buy-sell' indicator");
      return INIT_FAILED;
   }
