...
// http://fxcodebase.com/code/viewtopic.php?f=38&t=70867
...
input string session_open = "090000"; // Session open in hhmmss format
input int bars_to_take = 3; // Bars to take
...
#else
   StopLossType stop_loss_type = SLCustom; // Stop loss type
...
public:
   ...
   {
      double high, low;
      GetRange(_symbol, _timeframe, high, low);
...
void GetRange(string symbol, ENUM_TIMEFRAMES timeframe, double& high, double& low)
{
   string error;
   int sessionOpen = ParseTime(session_open, error);
   int i = 0;
   int time = ConvertTime(iTime(symbol, timeframe, i));
   if (time == 0)
   {
      time = 24 * 60;
   }
   while (i < Bars && time > sessionOpen)
   {
      ++i;
      time = ConvertTime(iTime(symbol, timeframe, i));
   }
   int highestIndex = iHighest(symbol, timeframe, MODE_HIGH, bars_to_take, 0);
   high = iHigh(symbol, timeframe, highestIndex);
   int lowestIndex = iLowest(symbol, timeframe, MODE_LOW, bars_to_take, 0);
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      double high, low;
      GetRange(_symbol, _timeframe, high, low);
      return iClose(_symbol, _timeframe, period) > high;
   }
};
