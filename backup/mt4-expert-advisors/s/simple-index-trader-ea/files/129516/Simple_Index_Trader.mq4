...
// http://fxcodebase.com/code/viewtopic.php?f=38&t=69077
...
input ENUM_MA_METHOD avg_method = MODE_SMA; // Smoothing method
input int avg_period = 200; // Average period
...
ICondition* CreateLongCondition(string symbol, ENUM_TIMEFRAMES timeframe)
{
   ...
   condition.Add(new MABelowPriceCondition(symbol, timeframe, avg_method, avg_period), false);
   condition.Add(new MACDAboveLevelCondition(symbol, timeframe, Short_Length, Long_Length, Signal_Length, MODE_MAIN, 0), false);
...
ICondition* CreateShortCondition(string symbol, ENUM_TIMEFRAMES timeframe)
{
   ...
   AndCondition* condition = new AndCondition();
   condition.Add(new MAAbovePriceCondition(symbol, timeframe, avg_method, avg_period), false);
   condition.Add(new MACDBelowLevelCondition(symbol, timeframe, Short_Length, Long_Length, Signal_Length, MODE_MAIN, 0), false);
#ifdef ACT_ON_SWITCH_CONDITION
   return (ICondition*) new ActOnSwitchCondition(symbol, timeframe, (ICondition*) condition);
#else 
   return (ICondition*) condition;
