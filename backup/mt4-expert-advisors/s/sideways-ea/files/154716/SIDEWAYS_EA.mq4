//Available @  https://fxcodebase.com/code/viewtopic.php?f=38&t=74664
...
#endif
input bool ECN = false; // ECN Broker:
bool         uTradeReverse = false;         // Trade Reverse:
input int    magico        = 2024;          // Magic Number:
input double minDistance   = 100; // Min Distance to Breakout (in points):
input int    candlesBack   = 48; // Candles Back to High Low:
...
input int        userSLpips   = 30;                // Pips SL
...
int searchSignals()
{
    ...
    h=iHigh(NULL, 0, iHighest(NULL,0,MODE_HIGH,candlesBack,2));
    l=iLow(NULL, 0, iLowest(NULL,0,MODE_LOW,candlesBack,2));

    lineUp.price(h).width(2).clr(DodgerBlue).redraw();
    lineDn.price(l).width(2).clr(Crimson).redraw();

    if(iClose(NULL,0,1)+minDistance*_Point > h) return 1;
    if(iClose(NULL,0,1)+minDistance*_Point < l) return 2;
return 0;
}
...
class SendNewOrder : public iActions
{
  ...
  public:
    ...
    bool doAction()
    {
        double tp = newOrder.tp();
        double sl = newOrder.sl();
        int tk;
        if(!ECN){
            tk = OrderSend(newOrder.symbol(), newOrder.type(), newOrder.lot(), newOrder.price(), 10000, sl, tp, newOrder.comment(), newOrder.magic(), newOrder.expireTime(), clrNONE);
        }
        if(ECN){
            tk = OrderSend(newOrder.symbol(), newOrder.type(), newOrder.lot(), newOrder.price(), 10000, 0, 0, newOrder.comment(), newOrder.magic(), newOrder.expireTime(), clrNONE);
            if(OrderSelect(tk, SELECT_BY_TICKET)==true) 
                OrderModify(tk, OrderOpenPrice(), sl, tp, 0, CLR_NONE);
        }
...
//--- SESSIONS CONTROL:
    ...
    lineUp = new Line("lineUp");
    lineDn = new Line("lineDn");
