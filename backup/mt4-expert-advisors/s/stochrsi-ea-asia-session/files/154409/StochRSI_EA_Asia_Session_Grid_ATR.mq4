...
#property description "Note: GRID: if what to use grid, don't use Stop Loss"
#property strict
...
// int bufferToBuy = ;
// int bufferToSell = ;
string file_custom_indicator = "Stoch RSI";

input string             Tindi               = "== Indicator Setup =="; // ————————————
input ENUM_TIMEFRAMES    StochTF             = PERIOD_M15;              // Time Frame Stoch
...
double main(int candle)
{
    return iCustom(NULL, StochTF, file_custom_indicator, InpStockKPeriod, InpStockDPeriod, InpRSIPeriod, InpStochastikPeriod, InpRSIAppliedPrice,
                   0, candle);
}
double signal(int candle)
{
    return iCustom(NULL, StochTF, file_custom_indicator, InpStockKPeriod, InpStockDPeriod, InpRSIPeriod, InpStochastikPeriod, InpRSIAppliedPrice,
                   1, candle);
}
...
// CONTROL PARA QUE HAGA UN TRADE AL DÍA
// cuando abra un trade se pone en false,
// cuando cambie el timer a un nuevo día, se pone en true
bool allowToTrade = true;
...
input ModeLevels      modeTP          = byATR;                 // Mode Take Profit:
input double          ATRmultiplierTP = 4;                     // ATR multiplier TP
...
input ModeLevels      modeSL          = byATR;                 // Mode Stop Loss:
input double          ATRmultiplierSL = 2;                     // ATR multiplier SL
input int             userSLpips      = 20;                    // Pips SL
double                userSLmoney     = 15;                    // Money SL
input string          Tatr            = "= ATR for TP & SL ="; // ————————————
...
input enum_ModeGrid       modeGrid            = grid_byATR;         // Mode Grid:
input int                 grid_ATR_periods    = 14;                 // ATR Periods
input ENUM_TIMEFRAMES     grid_ATR_tf         = PERIOD_M15;
input int                 grid_ATR_multi      = 4; // ATR Multiplier
...
#ifdef TIMER_MINI
input string T1        = "== Trading Sessions =="; // ————————————
input string timeStart = "07:00:00";               // Time Start GMT
input string timeEnd   = "09:30:00";               // Time End GMT
#endif
#ifdef TIMER_OFF
...
class Grid
{
  ...
  public:
    ...
    double nextPrice(double inpLastPrice)
    {
        ...
        if (grid_byATR) {
            double atr = NormalizeDouble(iATR(_Symbol, grid_ATR_tf, grid_ATR_periods, 1) * grid_ATR_multi, _Digits);

            if (_side == "buy")
                _nextPrice = inpLastPrice - (atr);
            if (_side == "sell")
                _nextPrice = inpLastPrice + (atr);
        }
    ...
    {
        if (StringFind(txt, "sl") > 0)
            return true;
        if (StringFind(txt, "tp") > 0)
            return true;
        return false;
    }
...
class ByATR : public iLevels
{
  ...
  public:
    ByATR(string Symbol, string Side, string Mode, ENUM_TIMEFRAMES ATRTf, int ATRPeriod, int ATRShift, double ATRmultiplier)
    {
        _symbol = Symbol;
        _side   = Side;
        _mode   = Mode;
...
void OnTick()
{
    ...
    if (GridON == true && CheckPointer(gridSell) != POINTER_INVALID) {
        gridSell.doGrid();
        mainOrders.GetMarketOrders();
        if (gridSell.checkIfCloseByTpSl()) {
            delete gridSell;
        }
    }
...
double SL(string side, double price = 0)
{
    ...
    if (stopLossOn)
        ...
        case byATR:
            levelSL = new Levels(new ByATR(_Symbol, side, "SL", ATRTf, ATRPeriod, ATRShift, ATRmultiplierSL));
            result  = levelSL.calculateLevel();
            break;
...
double TP(string side, double price = 0)
{
    ...
    if (takeProfitOn)
        ...
        case byATR:
            levelTP = new Levels(new ByATR(_Symbol, side, "TP", ATRTf, ATRPeriod, ATRShift, ATRmultiplierTP));
            result  = levelTP.calculateLevel();
            break;
