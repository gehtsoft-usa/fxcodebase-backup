...
int  handle_sar = 0;
void setHandle() { handle_sar = iSAR(NULL, 0, sar_step, sar_max); }

double sar(int candle = 1)
{
    double value[1];
    // int    shift = Bars(_Symbol, _Period) - candle;
    int shift = candle;
    int copy  = CopyBuffer(handle_sar, 0, shift, 1, value);
...
class ConditionSAR_Buy : public iConditions {
   public:
    bool evaluate() { return sar(1) < iLow(NULL, 0, 1) && sar(2) > iHigh(NULL, 0, 2); }
};
ConditionSAR_Buy cdSAR_Buy;

class ConditionSAR_Sell : public iConditions {
   public:
    bool evaluate() { return sar(1) > iHigh(NULL, 0, 1) && sar(2) < iLow(NULL, 0, 2); }
};
