...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=75336
...
input string TBox      = "==== Range Setup ===="; // —————————————————
RangeMode    rangeMode = rm_Auto;                 // Range Mode:
int          linesGap  = 50;                      // Initial Pips for lines:
input string hrIni     = "00:00";                 // Time GMT Start Range:
input string hrFin     = "06:00";                 // Time GMT End Range:
input color  clrCaja   = C'90,90,90';            // Range Color:

input bool OneOperationAtDay = false; // One Initial Trade at Day:
input bool OneTradeByCandle  = true;  // One Trade by Candle:

int      grosorCaja   = 1;        // Box Border :
bool     rellenoCaja  = true;     // Relleno Caja:
color    clrLineas    = DarkGray; // Color Lineas:
...
datetime diaActual;
bool     habilitado = false; // bandera para bloquear
...
input bool   partialCloseOn            = true;                  // Partial Close On:
input double userPartialClosePercent   = 50;                    // Partial Close Percent:
input double userPartialClosePips      = 5;                     // Partial Close Pips:
input int    uQntPartials              = 1;                     // Partials to take:
input bool   doBreakevenOnPartialClose = true;                  // do Breakeven on Partial Close?:
#else
...
double     userPartialClosePips      = 15;                    // Partial Close Pips:
int        uQntPartials              = 1;                     // Partials to take:
input bool doBreakevenOnPartialClose = false;                 // do Breakeven on Partial Close?:
#endif
#ifdef BREAKEVEN_ON
...
input ENUM_TIMEFRAMES utf = PERIOD_H1;               // TF:
#endif
input string             IemaSlow             = "== Moving Average Slow Setup ==";   // ————————————
input int                maSlowPeriod         = 55;                                  // Period
...
class Order
{
  ...
  public:
	...
	Order* type(int type){_type=(ENUM_ORDER_TYPE)type; return &this;}
...
class ScheduleController
{
    ...
    int      _currentDay;
    int      _timeZone; // modificador para ajustar GMT
...
class BUYcondition1 : public iConditions
{
  public:
    bool evaluate()
    {
        ...
        if (maSlow.index(0) < maSlow.index(2))
            return false;

        if (rangeMode == rm_Auto) {
            if (OneOperationAtDay == true && hizoOperacionesHoy())
                return false;
            if (OneTradeByCandle == true && tradeOnThisCandle())
                return false;
            // if (!Habilitado()) return false;

            double op     = iOpen(NULL, 0, 0);
            bool   filter = Ask >= HigherPrice && op < HigherPrice && HigherPrice > 0;
            bool   time   = iTime(NULL, 0, 0) > HoraFin;
  ...
  public:
    bool evaluate()
    {
        ...
        if (rangeMode == rm_Auto) {
            ...
            double op     = iOpen(NULL, 0, 0);
            bool   filter = Bid <= LowerPrice && op > LowerPrice && LowerPrice > 0;
            bool   time   = iTime(NULL, 0, 0) > HoraFin;
...
int OnInit()
{
    ...
    Print("ActualizarCaja: ", ActualizarCaja);
...
//--- SESSIONS CONTROL:
    ...
    maSlow = new MovingAverage(_Symbol, utf);
...
double Price(string direction, double pips = 0, string _symbol = "")
{
...
void RegistrarMaxMin()
{
    ...
    if (rangeMode == rm_Auto) {
        double ctrlMax = HigherPrice; // es el máximo de la caja antes de aplicar la función. Lo uso como punto de control, si los valores cambian imprime, sino no

        // if (TimeGMT() > HoraFin && ActualizarCaja == true) {
        if (iTime(NULL, 1, 0) > HoraFin && ActualizarCaja == true) {
            // buscar max y min:
            ...
            for (int i = barIni; i < barIni + minutos; i++) {
                if (HigherPrice == 0 || iHigh(NULL, 1, i) > HigherPrice) {
                    HigherPrice = iHigh(NULL, 1, i);
                }
                if (LowerPrice == 0 || iLow(NULL, 1, i) < LowerPrice) {
                    LowerPrice = iLow(NULL, 1, i);
                }
            }
            // NOTE: actualizar caja
            ActualizarCaja = false;
            DrawBox();
        }

        if (HigherPrice != ctrlMax) {
            Print(__FUNCTION__, " ", "HigherPrice", " ", HigherPrice);
            Print(__FUNCTION__, " ", "LowerPrice", " ", LowerPrice);
        }
    }

    if (rangeMode == rm_Mannual) {
        double Top_Price=0;
        if (ObjectFind(0, "TOP_RANGE") >= 0) {
            ObjectGetDouble(0, "TOP_RANGE", OBJPROP_PRICE, 0, Top_Price);
            Top_Price = NormalizeDouble(Top_Price, _Digits);
        }
        double Bottom_Price=0;
        if (ObjectFind(0, "BOTTOM_RANGE") >= 0) {
            ObjectGetDouble(0, "BOTTOM_RANGE", OBJPROP_PRICE, 0, Bottom_Price);
            Bottom_Price = NormalizeDouble(Bottom_Price, _Digits);
        }

        HigherPrice = Top_Price;
        LowerPrice  = Bottom_Price;

        // Print("Top_Price: ", Top_Price, " Bottom_Price: ", Bottom_Price);
    }
}
...
void DrawLines()
{
    ...
    for (int i = 0; i < 2; i++) {
        string _name  = i == 0 ? "TOP_RANGE" : "BOTTOM_RANGE";
        double _price = i == 0 ? Ask + (linesGap * 10 * _Point) : Ask - (linesGap * 10 * _Point);
        _price        = NormalizeDouble(_price, _Digits);
        color clr     = i == 0 ? RoyalBlue : Pink;

        // Print(__FUNCTION__, " _price: ", _price);
        ...
        // ObjectSetInteger(0, _name, OBJPROP_STYLE,);
    }
}
...
void CleanChart()
{
    ...
    ObjectDelete(0, "Max");
    ObjectDelete(0, "Min");
    ObjectDelete(0, "Average_Price_Line_Sell_" + Symbol());
    ObjectDelete(0, "Average_Price_Line_Buy_" + Symbol());
}

bool tradeOnThisCandle()
{
    // si en las abiertas tengo una orden en esta vela
    for (int i = OrdersTotal() - 1; i >= 0; i--) {
    ...
    // chequeo si en el historial hay alguna que se haya abierto en esta vela
    for (int i = OrdersHistoryTotal() - 1; i >= 0; i--) {
