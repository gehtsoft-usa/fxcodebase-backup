...
string             Istoch          = "== Stochastic Setup ==";      // ————————————
int                kPeriod         = 5;                             // K% Period
int                dPeriod         = 3;                             // D% Period
int                slowing         = 3;                             // Slowing
ENUM_MA_METHOD     stMethod        = MODE_SMA;                      // Ma Method
ENUM_PRICE_FIELD   stPriceField    = LowHigh;                       // Price Field
input string             IMACD           = "== MACD Setup ==";            // ————————————
...
input string             Ibb             = "== BB Setup ==";              // ————————————
input int                bbPeriods       = 60;                            // Periods
...
class BUYcondition1 : public iConditions
{
 public:
  bool evaluate()
  {
		...
		// and source.low[period - 1] <= BB.BL[period - 1] 
		// and source.close[period] > MVA.DATA[period] 
		// and MACD.MACD[period] > 0 and MACD2.MACD[MACD2_L] > 0 then BUY();
    
		// TODO: condition Buy 1
		// double bbInf = iBands(NULL, 0, bbPeriods, bbDesviation, 0, PRICE_CLOSE, 2, 0);
		// double bbSup = iBands(NULL, 0, bbPeriods, bbDesviation, 0, PRICE_CLOSE, 1, 0);
		// double bbInf1 = iBands(NULL, 0, bbPeriods, bbDesviation, 0, PRICE_CLOSE, 2, 1);
		// double bbSup1 = iBands(NULL, 0, bbPeriods, bbDesviation, 0, PRICE_CLOSE, 1, 1);    
    ...
    return cl > op && cl > ma.index(1) && macd.macd_line(1) > 0 && macd.signal_line(1) > 0;
...
class BUYcondition2 : public iConditions
{
 public:
  bool evaluate()
  {
	...
	double cl  = iClose(NULL, 0, 1);
	double lo1 = iLow(NULL, 0, 2);

	double bbSup = iBands(NULL, 0, bbPeriods, bbDesviation, 0, PRICE_CLOSE, 1, 1);
	double bbInf1 = iBands(NULL, 0, bbPeriods, bbDesviation, 0, PRICE_CLOSE, 2, 2);

	return lo1 < bbInf1 || cl > bbSup;
	}
};
 ...
 public:
  bool evaluate()
  {
	// TODO: condition sell 2
		double cl  = iClose(NULL, 0, 1);
		double hi1 = iHigh(NULL, 0, 2);

		double bbInf = iBands(NULL, 0, bbPeriods, bbDesviation, 0, PRICE_CLOSE, 2, 1);
		double bbSup1 = iBands(NULL, 0, bbPeriods, bbDesviation, 0, PRICE_CLOSE, 1, 2);

		return hi1 > bbSup1 || cl < bbInf;
  }
};
