...
// http://fxcodebase.com/code/viewtopic.php?f=38&t=68855
...
input int stoch_k = 5; // Stochastic K-period (number of bars for calculations) 
input int stoch_d = 3; // Stochastic D-period (period of first smoothing) 
input int stoch_slowing = 3; // Stochastic final smoothing 
input double oversold_level = 20; // Oversold level
input double overbought_level = 80; // Overbought level
...
public:
   ...
   bool IsPass(const int period)
   {
      if (period == 0)
         return false;
      return signalLine[period] > overbought_level && signalLine[period - 1] <= overbought_level;
   }
};
...
public:
   ...
   bool IsPass(const int period)
   {
      if (period == 0)
         return false;
      return signalLine[period] < oversold_level && signalLine[period - 1] >= oversold_level;
   }
};
...
int OnInit(void)
{
   IndicatorName = GenerateIndicatorName("Stochastic Alert");
   ...
   stoch = iStochastic(_Symbol, timeframe, stoch_k, stoch_d, stoch_slowing, MODE_SMA, STO_LOWHIGH);
...
)
{
   for (int pos = prev_calculated; pos < rates_total; ++pos)
   {
      ...
      double buffer[1];
      if (CopyBuffer(stoch, MAIN_LINE, oldIndex, 1, buffer) == 1)
         mainLine[pos] = buffer[0];
      if (CopyBuffer(stoch, SIGNAL_LINE, oldIndex, 1, buffer) == 1)
         signalLine[pos] = buffer[0];
      up.Update(pos);
      down.Update(pos);
   }
   return rates_total;
