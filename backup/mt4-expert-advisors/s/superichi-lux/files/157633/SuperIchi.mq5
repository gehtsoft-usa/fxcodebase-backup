//Available @  https://fxcodebase.com/code/viewtopic.php?f=38&p=155588#p155588
...
public:
   ...
   bool GetValue(const int pos, const int oldPos, double &__out1)
   {
      ...
      double multValue[1];
      if (!mult.GetValues(pos, 1, multValue)) { multValue[0] = EMPTY_VALUE; }
      ...
      int cross2Value[1];
      if (!cross2.GetValues(pos, 1, cross2Value)) { cross2Value[0] = (-1); }
      if (pos - 1 < 0) { return false; }
      if (pos - 1 < 0) { return false; }
                ...
                const int &spread[])
{
   ...
   for (int pos = MathMax(rates_total - 1 - bars_limit, MathMax(first, prev_calculated - 1)); pos < rates_total; ++pos)
   {
      ...
      if (!avg__custom_fS_i_fS1.GetValue(pos, oldPos, avg__custom_fS_i_fS1Value)) { avg__custom_fS_i_fS1Value = EMPTY_VALUE; }
      ...
      if (!avg__custom_fS_i_fS2.GetValue(pos, oldPos, avg__custom_fS_i_fS2Value)) { avg__custom_fS_i_fS2Value = EMPTY_VALUE; }
      ...
      if (!avg__custom_fS_i_fS3.GetValue(pos, oldPos, avg__custom_fS_i_fS3Value)) { avg__custom_fS_i_fS3Value = EMPTY_VALUE; }
