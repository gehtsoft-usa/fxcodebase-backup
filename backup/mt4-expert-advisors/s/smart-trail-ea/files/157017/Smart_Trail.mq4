...
// https://fxcodebase.com/code/viewtopic.php?f=38&t=75286
...
input double atr_multi  = 4;               // ATR multiplier
input double smooth_per = 4;               // Smoothing

double factor = 1.07; // Factor
// ------------------------------------------------------------------
...
int OnInit()
{
    ...
    SetIndexStyle(0, DRAW_LINE, STYLE_SOLID, 1, LineDnClr);
    SetIndexLabel(0, "Line Up");

    SetIndexBuffer(1, bottomLine, INDICATOR_DATA);
    SetIndexStyle(1, DRAW_LINE, STYLE_SOLID, 1, LineUpClr);
                ...
                const long &tick_volume[], const long &volume[], const int &spread[])
{
    ...
    for (i = start; i >= 0; i--) {
        ...
        double atrm = atr * atr_multi * factor;
        ...
        if (dir[i] == 1) {            
            double bottom_sma = 0;
            for (int j = 1; j <= smooth_per; j++) { 
                if (longStop[i+j] != EMPTY_VALUE)
                    bottom_sma += longStop[i+j]; 
            }            
            // bottomLine[i] = longStop[i];
            bottomLine[i] = bottom_sma/smooth_per;            
            topLine[i]    = EMPTY_VALUE;
        }
        if (dir[i] == -1) {
            double top_sma = 0;
            for (int j = 1; j <= smooth_per; j++) { 
                if (shortStop[i+j] != EMPTY_VALUE)
                top_sma += shortStop[i+j]; 
            }
            // topLine[i]    = shortStop[i];
            topLine[i]    = top_sma/smooth_per;            
