...
//http://fxcodebase.com/code/viewtopic.php?f=38&t=69999
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      double maValue = iMA(_symbol, _timeframe, ma_period, 0, ma_method, PRICE_CLOSE, period);
...
class VPCLongCondition : public ACondition
{
public:
   VPCLongCondition(const string symbol, ENUM_TIMEFRAMES timeframe)
      :ACondition(symbol, timeframe)
   {
   ...
   bool IsPass(const int period, const datetime date)
   {
      double vpc = iCustom(_symbol, _timeframe, "Volume Price Change", vpc_length, vpc_price_smoothing, vpc_signal_smoothing, vpc_overbought, vpc_oversold, 0, period);
      double signal = iCustom(_symbol, _timeframe, "Volume Price Change", vpc_length, vpc_price_smoothing, vpc_signal_smoothing, vpc_overbought, vpc_oversold, 1, period);
      return vpc > signal;
   }
};

class VPCShortCondition : public ACondition
{
public:
   VPCShortCondition(const string symbol, ENUM_TIMEFRAMES timeframe)
      :ACondition(symbol, timeframe)
   {
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      double k = iStochastic(_symbol, _timeframe, K_periods, D_periods, Slowing, MODE_SMA, 0, MODE_MAIN, period);
      double d = iStochastic(_symbol, _timeframe, K_periods, D_periods, Slowing, MODE_SMA, 0, MODE_SIGNAL, period);
      return k > d && k < OS_level;
   }
};
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      double k = iStochastic(_symbol, _timeframe, K_periods, D_periods, Slowing, MODE_SMA, 0, MODE_MAIN, period);
      double d = iStochastic(_symbol, _timeframe, K_periods, D_periods, Slowing, MODE_SMA, 0, MODE_SIGNAL, period);
      return k < d && k > OB_level;
   }
};

class BaseMALongCondition : public ACondition
{
public:
   BaseMALongCondition(const string symbol, ENUM_TIMEFRAMES timeframe)
      :ACondition(symbol, timeframe)
   {
   ...
   bool IsPass(const int period, const datetime date)
   {
      double maValue = iMA(_symbol, trend_td, base_ma_period, 0, base_ma_method, PRICE_CLOSE, 0);
      return iClose(_symbol, _timeframe, period) > maValue;
   }
};

class BaseMAShortCondition : public ACondition
{
public:
   BaseMAShortCondition(const string symbol, ENUM_TIMEFRAMES timeframe)
      :ACondition(symbol, timeframe)
   {
...
class ATRPeriodLongCondition : public ACondition
{
public:
   ATRPeriodLongCondition(const string symbol, ENUM_TIMEFRAMES timeframe)
      :ACondition(symbol, timeframe)
   {
   ...
   bool IsPass(const int period, const datetime date)
   {
      double maValue = iMA(_symbol, trend_td, base_ma_period, 0, base_ma_method, PRICE_CLOSE, period);
      double atrValue = iATR(_symbol, _timeframe, atr_period, period);
      return iClose(_symbol, _timeframe, period) > maValue + atrValue * atr_mult;
   }
};

class ATRPeriodShortCondition : public ACondition
{
public:
   ATRPeriodShortCondition(const string symbol, ENUM_TIMEFRAMES timeframe)
      :ACondition(symbol, timeframe)
   {
...
input int vpc_length = 15; // VPC Period
input int vpc_price_smoothing = 15; // VPC Price Smoothing
input int vpc_signal_smoothing = 15; // VPC Signal Smoothing
input int vpc_overbought = 80; // VPC Overbought Level
input int vpc_oversold = 20; // VPC Oversold Level
...
input double OB_level = 20; // Overbought level
input ENUM_TIMEFRAMES trend_td = PERIOD_CURRENT; // Timeframe
input int base_ma_period = 14; // Base MA Period
input ENUM_MA_METHOD base_ma_method = MODE_SMA; // Base Smoothing method
input bool use_atr_period = true; // Use price/ATR rule
input int atr_period = 14; // ATR Period
input double atr_mult = 1.0; // ATR Multiplicator
input bool use_atr_ma = true; // Use ma/ATR rule
class ATRMALongCondition : public ACondition
{
public:
   ATRMALongCondition(const string symbol, ENUM_TIMEFRAMES timeframe)
      :ACondition(symbol, timeframe)
   {
...
class ATRMAShortCondition : public ACondition
{
public:
   ATRMAShortCondition(const string symbol, ENUM_TIMEFRAMES timeframe)
      :ACondition(symbol, timeframe)
   {
...
ICondition* CreateLongFilterCondition(string symbol, ENUM_TIMEFRAMES timeframe)
{
   ...
   AndCondition* condition = new AndCondition();
   condition.Add(new VPCLongCondition(symbol, timeframe), false);
   condition.Add(new SSDLongCondition(symbol, timeframe), false);
   condition.Add(new BaseMALongCondition(symbol, timeframe), false);
   if (use_atr_period)
   {
      condition.Add(new ATRPeriodLongCondition(symbol, timeframe), false);
   }
   if (use_atr_ma)
   {
      condition.Add(new ATRMALongCondition(symbol, timeframe), false);
   }
   return condition;
}
...
ICondition* CreateShortFilterCondition(string symbol, ENUM_TIMEFRAMES timeframe)
{
   ...
   AndCondition* condition = new AndCondition();
   condition.Add(new VPCShortCondition(symbol, timeframe), false);
   condition.Add(new SSDShortCondition(symbol, timeframe), false);
   condition.Add(new BaseMAShortCondition(symbol, timeframe), false);
   if (use_atr_period)
   {
      condition.Add(new ATRPeriodShortCondition(symbol, timeframe), false);
   }
   if (use_atr_ma)
   {
      condition.Add(new ATRMAShortCondition(symbol, timeframe), false);
   }
   return condition;
}
...
int OnInit()
{
      ...
      stats = NULL;
   #endif
   double temp = iCustom(NULL, 0, "Volume Price Change", vpc_length, vpc_price_smoothing, vpc_signal_smoothing, vpc_overbought, vpc_oversold, 0, 0);
