...
//https://fxcodebase.com/code/viewtopic.php?f=38&t=71284
...
input int mainperiod = 100; // Main Period
input int signalperiod = 7; // Signal Period
...
int init()
{
   ...
   IndicatorObjPrefix = GenerateIndicatorPrefix("sl");
   IndicatorShortName("Scalping Line");
   IndicatorBuffers(11);
                ...
                const int &spread[])
{
   ...
   {
      emasrc[pos] = iMA(_Symbol, _Period, MathCeil(mainperiod / 2), 0, MODE_SMA, src, pos);
      double MA = iMAOnArray(emasrc, 0, MathFloor(mainperiod / 2) + 1, 0, MODE_SMA, pos);
      double ssMA = MA > close[pos] + MA * percent / 100 ? MA : MA < close[pos] - MA * percent / 100 ? MA : close[pos];
      double signalline = iMA(_Symbol, _Period, signalperiod, 0, MODE_SMA, PRICE_CLOSE, pos);
      plot1[pos] = signalline - ssMA;
      plot2[pos] = 0;

      if (customStream != NULL)
      {
         customStream.SetValue(pos, plot1[pos]);
      }
