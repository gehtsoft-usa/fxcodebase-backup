...
input bool Trend = true; // Use Trend Filter
input bool Change = true; // Only on subtrend change
input ENUM_TIMEFRAMES TF1 = PERIOD_CURRENT; // Timeframe 1
input int N1 = 7; // Number of periods
input double M1 = 2.5; // Multiplier
input ENUM_TIMEFRAMES TF2 = PERIOD_CURRENT; // Timeframe 2
input int N2 = 7; // Number of periods
input double M2 = 2.5; // Multiplier
...
class SupertrendUpCondition : public ACondition
{
   int _period;
   double _mult;
public:
   SupertrendUpCondition(const string symbol, ENUM_TIMEFRAMES timeframe, int period, double mult)
      :ACondition(symbol, timeframe)
   {
      _period = period;
      _mult = mult;
   ...
   bool IsPass(const int period, const datetime date)
   {
      double val = iCustom(_symbol, _timeframe, "supertrend", _period, _mult, 0, period);
      return val != EMPTY_VALUE;
   }
};

class SupertrendDownCondition : public ACondition
{
   int _period;
   double _mult;
public:
   SupertrendDownCondition(const string symbol, ENUM_TIMEFRAMES timeframe, int period, double mult)
      :ACondition(symbol, timeframe)
   {
      _period = period;
      _mult = mult;
   ...
   bool IsPass(const int period, const datetime date)
   {
      double val = iCustom(_symbol, _timeframe, "supertrend", _period, _mult, 1, 0);
      return val != EMPTY_VALUE;
   }
};
...
ICondition* CreateLongCondition(string symbol, ENUM_TIMEFRAMES timeframe)
{
   ...
   condition.Add(new SupertrendUpCondition(symbol, TF1, N1, M1), false);
...
ICondition* CreateLongFilterCondition(string symbol, ENUM_TIMEFRAMES timeframe)
{
   ...
   return new SupertrendUpCondition(symbol, TF2, N2, M2);
}
...
ICondition* CreateShortCondition(string symbol, ENUM_TIMEFRAMES timeframe)
{
   ...
   AndCondition* condition = new AndCondition();
   condition.Add(new SupertrendDownCondition(symbol, TF1, N1, M1), false);
   #ifdef ACT_ON_SWITCH_CONDITION
      if (!Change)
      {
         return condition;
...
ICondition* CreateShortFilterCondition(string symbol, ENUM_TIMEFRAMES timeframe)
{
   ...
   return new SupertrendDownCondition(symbol, TF2, N2, M2);
}
...
int OnInit()
{
   double temp = iCustom(NULL, 0, "supertrend", 0, 0);
