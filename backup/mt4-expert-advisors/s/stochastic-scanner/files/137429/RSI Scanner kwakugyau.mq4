...
input int buy_level_1 = 16; // Buy level 1
input int buy_level_2 = 25; // Buy level 2
input int sell_level_1 = 75; // Sell level 1
input int sell_level_2 = 82; // Sell level 2
enum DisplayMode
{
   Vertical,
   Horizontal
...
public:
   ...
   {
      double rsi0 = iRSI(_symbol, _timeframe, rsi_period, PRICE_CLOSE, period);
      double rsi1 = iRSI(_symbol, _timeframe, rsi_period, PRICE_CLOSE, period + 1);
      return (rsi0 >= buy_level_1 && rsi1 < buy_level_1)
         || (rsi0 >= buy_level_2 && rsi1 < buy_level_2);
   }
};
...
public:
   ...
   virtual bool IsPass(const int period, const datetime date)
   {
      double rsi0 = iRSI(_symbol, _timeframe, rsi_period, PRICE_CLOSE, period);
      double rsi1 = iRSI(_symbol, _timeframe, rsi_period, PRICE_CLOSE, period + 1);
      return (rsi0 <= sell_level_1 && rsi1 > sell_level_1)
         || (rsi0 <= sell_level_2 && rsi1 > sell_level_2);
   }
};
...
class TextFormatter : public AValueFormatter
{
   string _text;
   color _clr;
   color _bgClr;
   ...
   ENUM_TIMEFRAMES _timeframe;
public:
   TextFormatter(string symbol, ENUM_TIMEFRAMES timeframe, string text, color clr, color bgClr)
   {
      _symbol = symbol;
      _timeframe = timeframe;
      _bgClr = bgClr;
   ...
   virtual string FormatItem(const int period, const datetime date, color& clr, color& bgColor)
   {
      ...
      bgColor = _bgClr;
      double rsi0 = iRSI(_symbol, _timeframe, rsi_period, PRICE_CLOSE, period);
      string rsi = DoubleToString(rsi0, 2);
      return _text + ". RSI: " + rsi;
   }
};
#endif
...
public:
   ...
   virtual ICell* Create(const string id, const int x, const int y, ENUM_BASE_CORNER corner, const string symbol, const ENUM_TIMEFRAMES timeframe)
   {
      ...
      IValueFormatter* alertUpValue = new TextFormatter(symbol, timeframe, "Buy", GetTextColor(_upColor), GetBackgroundColor(_upColor));
      cell.AddCondition(upCondition, upValue, historyUpValue, alertUpValue);
      upCondition.Release();
      upValue.Release();
      ...
      IValueFormatter* alertDownValue = new TextFormatter(symbol, timeframe, "Buy", GetTextColor(_upColor), GetBackgroundColor(_upColor));
      cell.AddCondition(downCondition, downValue, historyDownValue, alertDownValue);
...
string IndicatorName = "RSI Scanner";
int init()
{
   ...
   IndicatorObjPrefix = GenerateIndicatorPrefix("RSI Scanner");
