...
private:
   ...
   void DrawItem(string text, color textColor, color bgColor)
   {
      ...
      else
      {
         ...
         ObjectSetInteger(0, id, OBJPROP_XSIZE, MathMax(min_button_width, width + 5));
...
public:
   ...
   virtual ICell* Create(const string id, const int x, const int y, const string symbol, const ENUM_TIMEFRAMES timeframe)
   {
      ...
      TrendValueCell* cell = new TrendValueCell(id, x, y, symbol, timeframe, _alertShift, defaultValue, output_mode);
      defaultValue.Release();

      ICondition* upCondition = new UpCondition(symbol, timeframe);
      IValueFormatter* upValue = new FixedTextFormatter("OB", output_mode == OutputLabels ? _upColor : button_text_color, output_mode != OutputLabels ? _upColor : button_text_color);
      cell.AddCondition(upCondition, upValue, NULL, upValue);
      upCondition.Release();
      upValue.Release();

      ICondition* downCondition = new DownCondition(symbol, timeframe);
      IValueFormatter* downValue = new FixedTextFormatter("OS", output_mode == OutputLabels ? _downColor : button_text_color, output_mode != OutputLabels ? _downColor : button_text_color);
      cell.AddCondition(downCondition, downValue, NULL, upValue);
