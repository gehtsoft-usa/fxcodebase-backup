...
//An alert sounds when these two conditions are met: when the stochastic indicator (14,3,3) crosses in oversold and the price is above the moving average simple 70
//And another one sounds : When the stochastic indicator (14,3,3 )crosses in overbought and the price is below the moving average 70
...
input int ma_period = 70; // MA Period
...
public:
   ...
   virtual bool IsPass(const int period, const datetime date)
   {
      ...
      double ma = iMA(_symbol, _timeframe, ma_period, 0, ma_method, PRICE_CLOSE, period);
      double stoch0 = iStochastic(_symbol, _timeframe, K_periods, D_periods, Slowing, MODE_SMA, 0, MODE_MAIN, period);
      double stoch1 = iStochastic(_symbol, _timeframe, K_periods, D_periods, Slowing, MODE_SMA, 0, MODE_MAIN, period + 1);
      return ma > close && stoch0 <= os_level && stoch1 > os_level;
   }
};
...
public:
   ...
   virtual bool IsPass(const int period, const datetime date)
   {
      ...
      double stoch0 = iStochastic(_symbol, _timeframe, K_periods, D_periods, Slowing, MODE_SMA, 0, MODE_MAIN, period);
      double stoch1 = iStochastic(_symbol, _timeframe, K_periods, D_periods, Slowing, MODE_SMA, 0, MODE_MAIN, period + 1);
      return ma < close && stoch0 >= ob_level && stoch1 < ob_level;
   }
};
...
int init()
{
   ...
   IndicatorObjPrefix = GenerateIndicatorPrefix("smaa");
   IndicatorShortName("Stochastic MA Alert");
...
{
   //handleButtonClicks();
   WindowNumber = MathMax(0, WindowFind("Stochastic MA Alert"));
   grid.HandleButtonClicks();
   grid.Draw();
