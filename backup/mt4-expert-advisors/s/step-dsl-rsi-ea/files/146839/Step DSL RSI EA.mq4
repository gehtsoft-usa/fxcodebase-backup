...
// //https://fxcodebase.com/code/viewtopic.php?f=38&t=72541
...
enum eRSI_TYPE {
  rsi_regular,  // Regular RSI
  rsi_slow,     // Slow RSI
  rsi_rapid,    // Rapid RSI
  rsi_harris,   // Harris RSI
  rsi_rsx,      // RSX
  rsi_cutlers,  // Cuttlers RSI
};
...
enum eRSI_Price {
  ...
  typical,   // Typical
  weighted,  // Weighted
  Average1,  // Average (High+Open+Low+Close)/4
  Average2,  // Average median body (open+close)/2
  Trend1,    // Trend biased price
  Trend2,    // Trend biased (extreme) price

  HeinkenAshi_Close,     // Heiken ashi Close
  HeinkenAshi_Open,      // Heiken ashi Open
  HeinkenAshi_High,      // Heiken ashi High
  HeinkenAshi_Low,       // Heiken ashi Low
  HeinkenAshi_Median,    // Heiken ashi Median
  HeinkenAshi_Typical,   // Heiken ashi Typical
  HeinkenAshi_Weighted,  // Heiken ashi Weighted
  HeinkenAshi_Average1,  // Heiken ashi Average
  HeinkenAshi_Average2,  // Heiken ashi Average median body
  HeinkenAshi_Trend1,    // Heiken ashi Trend biased price
  HeinkenAshi_Trend2,    // Heiken ashi Trend biased (extreme) price

  HeinkenAshi_BF_Close,     // Heiken ashi (better formula) Close
  HeinkenAshi_BF_Open,      // Heiken ashi (better formula) Open
  HeinkenAshi_BF_High,      // Heiken ashi (better formula) High
  HeinkenAshi_BF_Low,       // Heiken ashi (better formula) Low
  HeinkenAshi_BF_Median,    // Heiken ashi (better formula) Median
  HeinkenAshi_BF_Typical,   // Heiken ashi (better formula) Typical
  HeinkenAshi_BF_Weighted,  // Heiken ashi (better formula) Weighted
  HeinkenAshi_BF_Average1,  // Heiken ashi (better formula) Average
  HeinkenAshi_BF_Average2,  // Heiken ashi (better formula) Average median body
  HeinkenAshi_BF_Trend1,    // Heiken ashi (better formula) Trend biased price
  HeinkenAshi_BF_Trend2,    // Heiken ashi (better formula) Trend biased (extreme) price
};

// ------------------------------------------------------------------
input string      Tindi            = "== Indicator Setup ==";  // == Indicator Setup ==
input eTimeFrames TimeFrame        = 0;
input eRSI_TYPE   RsiType          = 0;    // Rsi Type
input int         RsiPeriod        = 14;   // Rsi Period
input eRSI_Price  RsiPrice         = 0;    // Rsi Price
input double      RsiSignal        = 9.0;  // Rsi Signal
input bool        Smoothed         = true;
input int         Length           = 15;
input double      Phase            = 0.0;
input bool        Double           = false;
input double      Sensitivity      = 4.0;
input double      StepSize         = 5.0;  // Step Size
input int         colorOn          = 2;    // Color On
input bool        Interpolate      = true;
input bool        alertsOn         = false;               // Alerts On
input bool        alertsOnCurrent  = false;               // Alerts On Current
input bool        alertsMessage    = false;               // Alerts Message
input bool        alertsSound      = false;               // Alerts Sound
input bool        alertsNotify     = false;               // Alerts Notify
...
input string      arrowsIdentifier = "step rsi Arrows1";  // Arrows Identifier
input bool        arrowsOnNewest   = false;               // Arrows On Newest
input int         arrowsUpperGap   = 4;                   // Arrows Upper Gap
input int         arrowsLowerGap   = 5;                   // Arrows Lower Gap
input color       arrowsUpColor    = 7451452;             // Arrows Up Color
input color       arrowsDnColor    = 3937500;             // Arrows Dn Color
input int         arrowsUpCode     = 159;                 // Arrows Up Code
input int         arrowsDnCode     = 159;                 // Arrows Dn Code
input int         arrowsUpSize     = 2;                   // Arrows Up Size
input int         arrowsDnSize     = 2;                   // Arrows Dn Size
// ------------------------------------------------------------------
 ...
 public:
  bool evaluate()
  {
    // TODO: condition Buy 1
		if( indi(3,1) != EMPTY_VALUE && indi(3,2)==EMPTY_VALUE )
		{
			return true;
		}
    return false;
 ...
 public:
  bool evaluate()
  {
    // TODO: condition sell 1
		if( indi(5,1) != EMPTY_VALUE && indi(5,2)==EMPTY_VALUE )
		{
			return true;
		}
    return false;
...
double indi(int buffer, int n)
{
	
	return iCustom(NULL, 0, "Step DSL RSI",
                         TimeFrame,
                         RsiType,
                         RsiPeriod,
                         RsiPrice,
