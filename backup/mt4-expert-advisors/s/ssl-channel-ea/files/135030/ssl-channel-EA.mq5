...
//http://fxcodebase.com/code/viewtopic.php?f=38&t=69212
...
input double take_profit_atr_multiplicator = 1; // Take profit ATR multiplicator
...
public:
   ...
   {
      double first[1];
      if (CopyBuffer(_indi, 0, period, 1, first) != 1)
      {
         return false;
      }
      double second[1];
      if (CopyBuffer(_indi, 1, period, 1, second) != 1)
      {
         return false;
      }
...
int OnInit()
{
   ...
   _indi = iCustom(_Symbol, _Period, "ssl-channel-chart-indicator", Lb);
