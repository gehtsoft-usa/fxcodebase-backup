//Available @  https://fxcodebase.com/code/viewtopic.php?f=38&t=74930
...
input int    sl_candles_shift    = 10; // Candles Shift (high/low)
...
input string       tindifiles      = "== Indicators Files ==";               // ————————————————————————
input const string indicator_file1 = "LunaM1.ex4";                           // Indicator File:
input const string indicator_file2 = "LunaM2.ex4";                           // Indicator File:
input const string indicator_file3 = "LunaM3.ex4";                           // Indicator File:
input const string indicator_file4 = "100% Non Repaint Indicator V22.0.ex4"; // Indicator File:
input string       tindiEntrys     = "== Indicators Entrys ==";              // ————————————————————————
input const bool   entry_luna1_on  = true;                                   // Entry by LunaM1 on:
input const bool   entry_luna2_on  = true;                                   // Entry by LunaM2 on:
input const bool   entry_luna3_on  = true;                                   // Entry by LunaM3 on:
input const bool   entry_nrp_i_on  = true;                                   // Entry by 100% Non Repaint Indicator on:
...
double             luna1(int buffer, int candle = 1) { return iCustom(NULL, 0, indicator_file1, buffer, candle); }
double             luna2(int buffer, int candle = 1) { return iCustom(NULL, 0, indicator_file2, buffer, candle); }
double             luna3(int buffer, int candle = 1) { return iCustom(NULL, 0, indicator_file3, buffer, candle); }
double             nrp_i(int buffer, int candle = 1) { return iCustom(NULL, 0, indicator_file4, buffer, candle); }


class ConditionLuna1Buy : public iConditions
{
  public:
    bool evaluate()
    {
        double indi = luna1(luna_buBuy, 1);
        return indi > 0 && indi != EMPTY_VALUE;
    }
};
ConditionLuna1Buy cluna1Buy;
class ConditionLuna1Sell : public iConditions
{
  public:
    bool evaluate()
    {
        double indi = luna1(luna_buSel, 1);
        return indi > 0 && indi != EMPTY_VALUE;
    }
};
ConditionLuna1Sell cluna1Sell;
Strategy           st_luna1_long(&cluna1Buy, &buy);
Strategy           st_luna1_short(&cluna1Sell, &sell);
class Conditionluna2Buy : public iConditions
{
  public:
    bool evaluate()
    {
        double indi = luna2(luna_buBuy, 1);
        return indi > 0 && indi != EMPTY_VALUE;
    }
};
Conditionluna2Buy cluna2Buy;
class Conditionluna2Sell : public iConditions
{
  public:
    bool evaluate()
    {
        double indi = luna2(luna_buSel, 1);
        return indi > 0 && indi != EMPTY_VALUE;
    }
};
Conditionluna2Sell cluna2Sell;
Strategy           st_luna2_long(&cluna2Buy, &buy);
Strategy           st_luna2_short(&cluna2Sell, &sell);
class Conditionluna3Buy : public iConditions
{
  public:
    bool evaluate()
    {
        double indi = luna3(luna_buBuy, 1);
        return indi > 0 && indi != EMPTY_VALUE;
    }
};
Conditionluna3Buy cluna3Buy;
class Conditionluna3Sell : public iConditions
{
  public:
    bool evaluate()
    {
        double indi = luna3(luna_buSel, 1);
        return indi > 0 && indi != EMPTY_VALUE;
    }
};
Conditionluna3Sell cluna3Sell;
Strategy           st_luna3_long(&cluna3Buy, &buy);
Strategy           st_luna3_short(&cluna3Sell, &sell);

class ConditionNrpBuy : public iConditions
{
  public:
    bool evaluate()
    {
        double indi = nrp_i(nrp__buBuy, 1);
        return indi > 0 && indi != EMPTY_VALUE;
    }
};
ConditionNrpBuy cNrpBuy;
class ConditionNrpSell : public iConditions
{
  public:
    bool evaluate()
    {
        double indi = nrp_i(nrp__buSel, 1);
        return indi > 0 && indi != EMPTY_VALUE;
    }
};
ConditionNrpSell cNrpSell;

Strategy         st_nrp_long(&cNrpBuy, &buy);
Strategy         st_nrp_short(&cNrpSell, &sell);

Strategy         closeopposite_luna1_buy(&cluna1Sell, &acCloseBuy);
Strategy         closeopposite_luna1_sel(&cluna1Buy, &acCloseSell);
Strategy         closeopposite_luna2_buy(&cluna2Sell, &acCloseBuy);
Strategy         closeopposite_luna2_sel(&cluna2Buy, &acCloseSell);
Strategy         closeopposite_luna3_buy(&cluna3Sell, &acCloseBuy);
Strategy         closeopposite_luna3_sel(&cluna3Buy, &acCloseSell);
Strategy         closeopposite_nrp_buy(&cNrpSell, &acCloseBuy);
Strategy         closeopposite_nrp_sel(&cNrpBuy, &acCloseSell);

bool             OnInit_CustomIndicator()
{
    if (entry_luna1_on) {
        double temp1 = iCustom(NULL, 0, indicator_file1, 0, 0);
        if (GetLastError() == ERR_INDICATOR_CANNOT_LOAD) {
            MessageBox("THIS EA NEED AN INDICATOR\n install the file:\n" + indicator_file1 + "\ninto the folder:\nMQL4/Indicators.", "Important Information", MB_ICONINFORMATION);
            Alert("THIS EA NEED AN INDICATOR, install the file: " + indicator_file1 + " into the folder: MQL4/Indicators.");
            indicator_init = false;
        }

        if (!indicator_init)
            return false;
        Trader.strategys.add(&st_luna1_long);
        Trader.strategys.add(&st_luna1_short);
        if (close_on_opposite1_on) {
            Trader.management.add(&closeopposite_luna1_buy);
            Trader.management.add(&closeopposite_luna1_sel);
        }
    }

    if (entry_luna2_on) {
        double temp2 = iCustom(NULL, 0, indicator_file2, 0, 0);
        if (GetLastError() == ERR_INDICATOR_CANNOT_LOAD) {
            MessageBox("THIS EA NEED AN INDICATOR\n install the file:\n" + indicator_file2 + "\ninto the folder:\nMQL4/Indicators.", "Important Information", MB_ICONINFORMATION);
            Alert("THIS EA NEED AN INDICATOR, install the file: " + indicator_file2 + " into the folder: MQL4/Indicators.");
            indicator_init = false;
        }

        if (!indicator_init)
            return false;
        Trader.strategys.add(&st_luna2_long);
        Trader.strategys.add(&st_luna2_short);

        if (close_on_opposite2_on) {
            Trader.management.add(&closeopposite_luna2_buy);
            Trader.management.add(&closeopposite_luna2_sel);
        }
    }

    if (entry_luna3_on) {
        double temp3 = iCustom(NULL, 0, indicator_file3, 0, 0);
        if (GetLastError() == ERR_INDICATOR_CANNOT_LOAD) {
            MessageBox("THIS EA NEED AN INDICATOR\n install the file:\n" + indicator_file3 + "\ninto the folder:\nMQL4/Indicators.", "Important Information", MB_ICONINFORMATION);
            Alert("THIS EA NEED AN INDICATOR, install the file: " + indicator_file3 + " into the folder: MQL4/Indicators.");
            indicator_init = false;
        }

        if (!indicator_init)
            return false;
        Trader.strategys.add(&st_luna3_long);
        Trader.strategys.add(&st_luna3_short);
        
        if (close_on_opposite3_on) {
            Trader.management.add(&closeopposite_luna3_buy);
            Trader.management.add(&closeopposite_luna3_sel);
        }
    }

    if (entry_nrp_i_on) {
        double temp4 = iCustom(NULL, 0, indicator_file4, 0, 0);
        if (GetLastError() == ERR_INDICATOR_CANNOT_LOAD) {
            MessageBox("THIS EA NEED AN INDICATOR\n install the file:\n" + indicator_file4 + "\ninto the folder:\nMQL4/Indicators.", "Important Information", MB_ICONINFORMATION);
            Alert("THIS EA NEED AN INDICATOR, install the file: " + indicator_file4 + " into the folder: MQL4/Indicators.");
            indicator_init = false;
        }

        if (!indicator_init)
            return false;
        Trader.strategys.add(&st_nrp_long);
        Trader.strategys.add(&st_nrp_short);

        if (close_on_opposite4_on) {
            Trader.management.add(&closeopposite_nrp_buy);
            Trader.management.add(&closeopposite_nrp_sel);
        }
    }
...
input bool   close_on_opposite1_on = false;                         // Close On Opposite Luna1:
input bool   close_on_opposite2_on = false;                         // Close On Opposite Luna2:
input bool   close_on_opposite3_on = false;                         // Close On Opposite Luna3:
input bool   close_on_opposite4_on = false;                         // Close On Opposite 100% nrp indicator:
...
class ConditionCloseOnOppositeBuy : public iConditions
{
  public:
    bool evaluate()
    {
        bool r = false;
        if (OrderType() == OP_BUY) {
            // TODO: condición de apertura de Sell
        }
        return r;
    }
};
...
class ConditionCloseOnOppositeSell : public iConditions
{
  public:
    bool evaluate()
    {
        bool r = false;
        if (OrderType() == OP_SELL) {
            // TODO: condición de apertura de BUY
        }
        return r;
    }
};
...
void                         oninit_close_on_opposite()
{
    // if (close_on_opposite_on) {
        // Trader.management.add(&closeopposite_luna1_buy);
        // Trader.management.add(&cluna1Sell);
    // }
}
...
void                    oninit_counters_filters()
{
    ...
    st_luna1_long.addCondition(&cCountBuys);
    st_luna1_short.addCondition(&cCountSells);
    st_luna2_long.addCondition(&cCountBuys);
    st_luna2_short.addCondition(&cCountSells);
    st_luna3_long.addCondition(&cCountBuys);
    st_luna3_short.addCondition(&cCountSells);
    st_nrp_long.addCondition(&cCountBuys);
    st_nrp_short.addCondition(&cCountSells);
