...
//http://fxcodebase.com/code/viewtopic.php?f=38&t=69851
...
#property description "ZigZag-Integer BTF"
...
input int HistoryLimit = 500;
input bool Label = true;
input ENUM_BASE_CORNER DisplayLocation = CORNER_RIGHT_LOWER; //Display location
input int x_shift = 5; // X shift
input int y_shift = 5; // Y shift
...
int init()
{
   IndicatorObjPrefix = GenerateIndicatorPrefix("zzibtf");
   IndicatorShortName("ZigZag-Integer BTF");
                ...
                const int& spread[])
{
   ...
   for (int period = limit; period >= 0; period--)
   {
      ...
      {
         int index = iBarShift(_Symbol, tf, time[period]);
         if (index <= 0)
         {
            continue;
         }
         Zig[period] = iCustom(_Symbol, tf, "ZigZag_Integer_BTF", Depth, Deviation, Backstep, Average, 0, index);
         Zag[period] = iCustom(_Symbol, tf, "ZigZag_Integer_BTF", Depth, Deviation, Backstep, Average, 1, index);
      }
   }
   DRAW();
   return rates_total;
...
void DRAW()
{
   ...
   for (int i = MathMin(Bars + 2, HistoryLimit); i >= 1; --i)
   {
      if (Zag[i] != EMPTY_VALUE && Zag[i - 1] == EMPTY_VALUE)
      {
         ...
         CreateLabel(IndicatorObjPrefix + IntegerToString(peak_id++), Time[i], value + number / 10, IntegerToString(number / pipSize), clrGray, 10);
         ArrayResize(numbers, ++count);
         numbers[count - 1] = number / pipSize;
         prev_peak = value;
      }
   ...
   double summ = 0;
   for (i = count - 2; i >= MathMax(0, count - 2 - Average); --i)
   {
      summ += numbers[i];
   }
   string name = IndicatorObjPrefix + "Avg";
   ObjectCreate(0, name, OBJ_LABEL, 0, 0, 0);
   ObjectSetInteger(0, name, OBJPROP_CORNER, DisplayLocation);
   ObjectSetInteger(0, name, OBJPROP_XDISTANCE, x_shift);
   ObjectSetInteger(0, name, OBJPROP_YDISTANCE, y_shift);
   ...
   ObjectSetText(name, "ZigZag Avg: " + DoubleToString(summ / Average, 2) + " pips", 12, "Arial", clrGray);
}
