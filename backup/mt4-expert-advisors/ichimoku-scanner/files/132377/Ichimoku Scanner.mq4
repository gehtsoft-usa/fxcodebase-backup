...
// More information about this indicator can be found at:
// http://fxcodebase.com/code/viewtopic.php?f=28&t=68827
...
public:
   ...
   virtual void Draw()
   { 
      int direction = GetDirection(_alertShift);
      if (direction == 0 || TimeCurrent() - _lastSignalDate >= expiration_min * 60)
      {
         if (direction == 0)
         {
...
private:
   ...
   {
      datetime date = iTime(_symbol, _timeframe, index);
      if (_upCondition.IsPass(index, date))
         return ENTER_BUY_SIGNAL;
      if (_downCondition.IsPass(index, date))
