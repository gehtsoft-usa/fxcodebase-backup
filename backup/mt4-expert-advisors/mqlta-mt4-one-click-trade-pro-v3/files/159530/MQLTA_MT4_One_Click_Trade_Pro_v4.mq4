//Available @  https://fxcodebase.com/code/viewtopic.php?f=38&t=75972
...
enum DEF_PERCCALC
{
    ...
    MonetaryValue = 3,  // MONETARY VALUE
};
...
input double DefaultTPRR = 2;                               // Default Take Profit by R:R
double TPRRvalue = DefaultTPRR;
double TPPRR_value = DefaultTPRR;
input int DefaultMagic = 0;                              // Default Magic Number
input double uBreakevenPoints = 0;                        // Breakeven Points
input double uBreakevenRR = 0;                            // Breakeven R:R
double BreakevenPoints = uBreakevenPoints;
double BreakevenRR = uBreakevenRR;

input string DefaultComment = "";                        // Default Comment
input double DefaultRiskPerc = 2;                        // Default % of Risk or Monetary Value
    ...
    if (BreakevenPoints > 0) {
        for (int i = 0; i < OrdersTotal(); i++) {
            if (OrderSelect(i, SELECT_BY_POS, MODE_TRADES)) {
                if(OrderMagicNumber() != CurrMagic) continue;
                if (OrderType() == OP_BUY) {
                    double currdistance = (Bid - OrderOpenPrice())/Point;
                    if ((currdistance > uBreakevenPoints) && (OrderStopLoss() < OrderOpenPrice())) {
                        int r = OrderModify(OrderTicket(), OrderOpenPrice(), OrderOpenPrice(), 0, 0, clrNONE);
                    }
                }
                else if (OrderType() == OP_SELL) {
                    double currdistance = -(Ask - OrderOpenPrice())/Point;
                    if ((currdistance > BreakevenPoints) && (OrderStopLoss() > OrderOpenPrice())) {
                        int r = OrderModify(OrderTicket(), OrderOpenPrice(), OrderOpenPrice(), 0, 0, clrNONE);
                    }
                }
            }
...
// uBreakevenRR is used to set the breakeven point based on the risk-reward ratio.
    if (BreakevenRR > 0) {
        for (int i = 0; i < OrdersTotal(); i++) {
            if (OrderSelect(i, SELECT_BY_POS, MODE_TRADES)) {
                if(OrderMagicNumber() != CurrMagic) continue;
                double profit = fabs(Bid - OrderOpenPrice());
                double risk = fabs((OrderOpenPrice() - OrderStopLoss()));
                  ...
                  const string &sparam)
{
    ...
    else if (id == CHARTEVENT_OBJECT_ENDEDIT)
    {
        ...
        // Mark: realizar cambio en variable cuando cambia el RR del panel
        else if ((sparam == NewOrderTPRRe) || (sparam == NewPendingOppTPRRe))
        {
            ChangeTPRR();
        }
...
void ExecuteOrder(int Operation)
{
    ...
    if(TPRRvalue > 0 && TPRRvalue != EMPTY_VALUE && SLPrice >0)
    {
        double dist = fabs(Ask - SLPrice);
        dist *= TPRRvalue;

        if ((Operation == OP_BUY) || (Operation == OP_BUYLIMIT) || (Operation == OP_BUYSTOP))
        {
            TPPrice = OpenPrice + dist;
        } 
        if ((Operation == OP_SELL) || (Operation == OP_SELLLIMIT) || (Operation == OP_SELLSTOP))
        {
            TPPrice = OpenPrice - dist;
        } 
    }
...
string NewOrderTPRR = IndicatorName + "-NO-TPPRR";
string NewOrderTPRRe = IndicatorName + "-NO-TPPRRe";
...
string BKVinRREinPoints = IndicatorName + "-NO-BKVinRREinPoints";
string BKVinRREinPointsE = IndicatorName + "-NO-BKVinRREinPointsE";
string BKVinRREinRR = IndicatorName + "-NO-BKVinRREinRR";
string BKVinRREinRRE = IndicatorName + "-NO-BKVinRREinRREE";
...
void ShowNewOrder()
{
    ...
    ObjectCreate(0,     NewOrderTPRR, OBJ_EDIT, 0, 0, 0);
    ObjectSet(          NewOrderTPRR, OBJPROP_XDISTANCE, NewOrderXoff + 2);
    ObjectSet(          NewOrderTPRR, OBJPROP_YDISTANCE, NewOrderYoff + 2 + (NewOrderLabelY + 1) * j);
    ObjectSetInteger(0, NewOrderTPRR, OBJPROP_XSIZE, NewOrderDoubleX);
    ObjectSetInteger(0, NewOrderTPRR, OBJPROP_YSIZE, NewOrderLabelY);
    ObjectSetInteger(0, NewOrderTPRR, OBJPROP_BORDER_TYPE, BORDER_FLAT);
    ObjectSetInteger(0, NewOrderTPRR, OBJPROP_STATE, false);
    ObjectSetInteger(0, NewOrderTPRR, OBJPROP_HIDDEN, true);
    ObjectSetInteger(0, NewOrderTPRR, OBJPROP_READONLY, true);
    ObjectSetInteger(0, NewOrderTPRR, OBJPROP_FONTSIZE, NOFontSize);
    ObjectSetString(0,  NewOrderTPRR, OBJPROP_TOOLTIP, "Take Profit by R:R");
    ObjectSetInteger(0, NewOrderTPRR, OBJPROP_ALIGN, ALIGN_CENTER);
    ObjectSetString(0,  NewOrderTPRR, OBJPROP_FONT, NOFont);
    ObjectSetString(0,  NewOrderTPRR, OBJPROP_TEXT, "TP by R:R");
    ObjectSet(          NewOrderTPRR, OBJPROP_SELECTABLE, false);
    ObjectSetInteger(0, NewOrderTPRR, OBJPROP_COLOR, clrBlack);

    ObjectCreate(0,     NewOrderTPRRe, OBJ_EDIT, 0, 0, 0);
    ObjectSet(          NewOrderTPRRe, OBJPROP_XDISTANCE, NewOrderXoff + (NewOrderDoubleX + 2) + 2);
    ObjectSet(          NewOrderTPRRe, OBJPROP_YDISTANCE, NewOrderYoff + 2 + (NewOrderLabelY + 1) * j);
    ObjectSetInteger(0, NewOrderTPRRe, OBJPROP_XSIZE, NewOrderDoubleX);
    ObjectSetInteger(0, NewOrderTPRRe, OBJPROP_YSIZE, NewOrderLabelY);
    ObjectSetInteger(0, NewOrderTPRRe, OBJPROP_BORDER_TYPE, BORDER_FLAT);
    ObjectSetInteger(0, NewOrderTPRRe, OBJPROP_STATE, false);
    ObjectSetInteger(0, NewOrderTPRRe, OBJPROP_HIDDEN, true);
    ObjectSetInteger(0, NewOrderTPRRe, OBJPROP_READONLY, false);
    ObjectSetInteger(0, NewOrderTPRRe, OBJPROP_FONTSIZE, NOFontSize);
    ObjectSetString(0,  NewOrderTPRRe, OBJPROP_TOOLTIP, "Take Profit by R:R");
    ObjectSetInteger(0, NewOrderTPRRe, OBJPROP_ALIGN, ALIGN_CENTER);
    ObjectSetString(0,  NewOrderTPRRe, OBJPROP_FONT, NOFont);
    ObjectSetString(0,  NewOrderTPRRe, OBJPROP_TEXT, (string)TPRRvalue);
    ObjectSet(          NewOrderTPRRe, OBJPROP_SELECTABLE, false);
    ObjectSetInteger(0, NewOrderTPRRe, OBJPROP_COLOR, clrBlack);
    j++;
...
// --------------------------------------------------------------
// Breakeven by points and By R:R
// --------------------------------------------------------------
    ObjectCreate(0, BKVinRREinPoints, OBJ_EDIT, 0, 0, 0);
    ObjectSet(BKVinRREinPoints, OBJPROP_XDISTANCE, NewOrderXoff + 2);
    ObjectSet(BKVinRREinPoints, OBJPROP_YDISTANCE, NewOrderYoff + 2 + (NewOrderLabelY + 1) * j);
    ObjectSetInteger(0, BKVinRREinPoints, OBJPROP_XSIZE, NewOrderDoubleX);
    ObjectSetInteger(0, BKVinRREinPoints, OBJPROP_YSIZE, NewOrderLabelY);
    ObjectSetInteger(0, BKVinRREinPoints, OBJPROP_BORDER_TYPE, BORDER_FLAT);
    ObjectSetInteger(0, BKVinRREinPoints, OBJPROP_STATE, false);
    ObjectSetInteger(0, BKVinRREinPoints, OBJPROP_HIDDEN, true);
    ObjectSetInteger(0, BKVinRREinPoints, OBJPROP_READONLY, true);
    ObjectSetInteger(0, BKVinRREinPoints, OBJPROP_FONTSIZE, NOFontSize);
    ObjectSetString(0, BKVinRREinPoints, OBJPROP_TOOLTIP, "Breakeven by Points");
    ObjectSetInteger(0, BKVinRREinPoints, OBJPROP_ALIGN, ALIGN_CENTER);
    ObjectSetString(0, BKVinRREinPoints, OBJPROP_FONT, NOFont);
    ObjectSetString(0, BKVinRREinPoints, OBJPROP_TEXT, "Breakeven Points");
    ObjectSet(BKVinRREinPoints, OBJPROP_SELECTABLE, false);
    ObjectSetInteger(0, BKVinRREinPoints, OBJPROP_COLOR, clrBlack);

    ObjectCreate(0, BKVinRREinPointsE, OBJ_EDIT, 0, 0, 0);
    ObjectSet(BKVinRREinPointsE, OBJPROP_XDISTANCE, NewOrderXoff + (NewOrderDoubleX + 2) + 2);
    ObjectSet(BKVinRREinPointsE, OBJPROP_YDISTANCE, NewOrderYoff + 2 + (NewOrderLabelY + 1) * j);
    ObjectSetInteger(0, BKVinRREinPointsE, OBJPROP_XSIZE, NewOrderDoubleX);
    ObjectSetInteger(0, BKVinRREinPointsE, OBJPROP_YSIZE, NewOrderLabelY);
    ObjectSetInteger(0, BKVinRREinPointsE, OBJPROP_BORDER_TYPE, BORDER_FLAT);
    ObjectSetInteger(0, BKVinRREinPointsE, OBJPROP_STATE, false);
    ObjectSetInteger(0, BKVinRREinPointsE, OBJPROP_HIDDEN, true);
    ObjectSetInteger(0, BKVinRREinPointsE, OBJPROP_READONLY, false);
    ObjectSetInteger(0, BKVinRREinPointsE, OBJPROP_FONTSIZE, NOFontSize);
    ObjectSetString(0, BKVinRREinPointsE, OBJPROP_TOOLTIP, "Breakeven Points - Click to change");
    ObjectSetInteger(0, BKVinRREinPointsE, OBJPROP_ALIGN, ALIGN_CENTER);
    ObjectSetString(0, BKVinRREinPointsE, OBJPROP_FONT, NOFont);
    ObjectSetString(0, BKVinRREinPointsE, OBJPROP_TEXT, (string)BreakevenPoints);
    ObjectSet(BKVinRREinPointsE, OBJPROP_SELECTABLE, false);
    ObjectSetInteger(0, BKVinRREinPointsE, OBJPROP_COLOR, clrBlack);
    j++;
    
    ObjectCreate(0, BKVinRREinRR, OBJ_EDIT, 0, 0, 0);
    ObjectSet(BKVinRREinRR, OBJPROP_XDISTANCE, NewOrderXoff + 2);
    ObjectSet(BKVinRREinRR, OBJPROP_YDISTANCE, NewOrderYoff + 2 + (NewOrderLabelY + 1) * j);
    ObjectSetInteger(0, BKVinRREinRR, OBJPROP_XSIZE, NewOrderDoubleX);
    ObjectSetInteger(0, BKVinRREinRR, OBJPROP_YSIZE, NewOrderLabelY);
    ObjectSetInteger(0, BKVinRREinRR, OBJPROP_BORDER_TYPE, BORDER_FLAT);
    ObjectSetInteger(0, BKVinRREinRR, OBJPROP_STATE, false);
    ObjectSetInteger(0, BKVinRREinRR, OBJPROP_HIDDEN, true);
    ObjectSetInteger(0, BKVinRREinRR, OBJPROP_READONLY, true);
    ObjectSetInteger(0, BKVinRREinRR, OBJPROP_FONTSIZE, NOFontSize);
    ObjectSetString(0, BKVinRREinRR, OBJPROP_TOOLTIP, "Breakeven by R:R");
    ObjectSetInteger(0, BKVinRREinRR, OBJPROP_ALIGN, ALIGN_CENTER);
    ObjectSetString(0, BKVinRREinRR, OBJPROP_FONT, NOFont);
    ObjectSetString(0, BKVinRREinRR, OBJPROP_TEXT, "Breakeven R:R");
    ObjectSet(BKVinRREinRR, OBJPROP_SELECTABLE, false);
    ObjectSetInteger(0, BKVinRREinRR, OBJPROP_COLOR, clrBlack);

    ObjectCreate(0, BKVinRREinRRE, OBJ_EDIT, 0, 0, 0);
    ObjectSet(BKVinRREinRRE, OBJPROP_XDISTANCE, NewOrderXoff + (NewOrderDoubleX + 2) + 2);
    ObjectSet(BKVinRREinRRE, OBJPROP_YDISTANCE, NewOrderYoff + 2 + (NewOrderLabelY + 1) * j);
    ObjectSetInteger(0, BKVinRREinRRE, OBJPROP_XSIZE, NewOrderDoubleX);
    ObjectSetInteger(0, BKVinRREinRRE, OBJPROP_YSIZE, NewOrderLabelY);
    ObjectSetInteger(0, BKVinRREinRRE, OBJPROP_BORDER_TYPE, BORDER_FLAT);
    ObjectSetInteger(0, BKVinRREinRRE, OBJPROP_STATE, false);
    ObjectSetInteger(0, BKVinRREinRRE, OBJPROP_HIDDEN, true);
    ObjectSetInteger(0, BKVinRREinRRE, OBJPROP_READONLY, false);
    ObjectSetInteger(0, BKVinRREinRRE, OBJPROP_FONTSIZE, NOFontSize);
    ObjectSetString(0, BKVinRREinRRE, OBJPROP_TOOLTIP, "Breakeven R:R - Click to change");
    ObjectSetInteger(0, BKVinRREinRRE, OBJPROP_ALIGN, ALIGN_CENTER);
    ObjectSetString(0, BKVinRREinRRE, OBJPROP_FONT, NOFont);
    ObjectSetString(0, BKVinRREinRRE, OBJPROP_TEXT, (string)BreakevenRR);
    ObjectSet(BKVinRREinRRE, OBJPROP_SELECTABLE, false);
    ObjectSetInteger(0, BKVinRREinRRE, OBJPROP_COLOR, clrBlack);
    j++;
...
void ChangeTPRR()
{
    if (NewOrderPanelIsOpen)
    {
        TPRRvalue = StringToDouble(ObjectGetString(0, NewOrderTPRRe, OBJPROP_TEXT));
        Print("line: ",__LINE__," TPRRvalue: ",TPRRvalue);
        // CurrTPPts = StringToDouble(ObjectGetString(0, NewOrderTPPtsE, OBJPROP_TEXT));
        ShowNewOrder();
    }
    if (NewPendingOppPanelIsOpen)
    {
        TPPRR_value = StringToDouble(ObjectGetString(0, NewPendingOppTPRRe, OBJPROP_TEXT));
        Print("line: ",__LINE__," : ",TPPRR_value);
        // CurrTPPts = StringToDouble(ObjectGetString(0, NewPendingOppTPPtsE, OBJPROP_TEXT));
        ShowNewPendingOpp();
    }
}
...
string NewPendingOppTPRR = IndicatorName + "-NPO-TPPendingRR";
string NewPendingOppTPRRe = IndicatorName + "-NPO-TPPendingRRe";
...
void ShowNewPendingOpp()
{
    ...
    // Mark: new TP by RR en pending orders
    ObjectCreate(0,     NewPendingOppTPRR, OBJ_EDIT, 0, 0, 0);
    ObjectSet(          NewPendingOppTPRR, OBJPROP_XDISTANCE, NewPendingOppXoff + 2);
    ObjectSet(          NewPendingOppTPRR, OBJPROP_YDISTANCE, NewPendingOppYoff + 2 + (NewPendingOppLabelY + 1) * j);
    ObjectSetInteger(0, NewPendingOppTPRR, OBJPROP_XSIZE, NewPendingOppDoubleX);
    ObjectSetInteger(0, NewPendingOppTPRR, OBJPROP_YSIZE, NewPendingOppLabelY);
    ObjectSetInteger(0, NewPendingOppTPRR, OBJPROP_BORDER_TYPE, BORDER_FLAT);
    ObjectSetInteger(0, NewPendingOppTPRR, OBJPROP_STATE, false);
    ObjectSetInteger(0, NewPendingOppTPRR, OBJPROP_HIDDEN, true);
    ObjectSetInteger(0, NewPendingOppTPRR, OBJPROP_READONLY, true);
    ObjectSetInteger(0, NewPendingOppTPRR, OBJPROP_FONTSIZE, NOFontSize);
    ObjectSetString(0,  NewPendingOppTPRR, OBJPROP_TOOLTIP, "Take Profit by R:R");
    ObjectSetInteger(0, NewPendingOppTPRR, OBJPROP_ALIGN, ALIGN_CENTER);
    ObjectSetString(0,  NewPendingOppTPRR, OBJPROP_FONT, NOFont);
    ObjectSetString(0,  NewPendingOppTPRR, OBJPROP_TEXT, "TP by R:R");
    ObjectSet(          NewPendingOppTPRR, OBJPROP_SELECTABLE, false);
    ObjectSetInteger(0, NewPendingOppTPRR, OBJPROP_COLOR, clrBlack);

    ObjectCreate(0,     NewPendingOppTPRRe, OBJ_EDIT, 0, 0, 0);
    ObjectSet(          NewPendingOppTPRRe, OBJPROP_XDISTANCE, NewPendingOppXoff + (NewPendingOppDoubleX + 2) + 2);
    ObjectSet(          NewPendingOppTPRRe, OBJPROP_YDISTANCE, NewPendingOppYoff + 2 + (NewPendingOppLabelY + 1) * j);
    ObjectSetInteger(0, NewPendingOppTPRRe, OBJPROP_XSIZE, NewPendingOppDoubleX);
    ObjectSetInteger(0, NewPendingOppTPRRe, OBJPROP_YSIZE, NewPendingOppLabelY);
    ObjectSetInteger(0, NewPendingOppTPRRe, OBJPROP_BORDER_TYPE, BORDER_FLAT);
    ObjectSetInteger(0, NewPendingOppTPRRe, OBJPROP_STATE, false);
    ObjectSetInteger(0, NewPendingOppTPRRe, OBJPROP_HIDDEN, true);
    ObjectSetInteger(0, NewPendingOppTPRRe, OBJPROP_READONLY, false);
    ObjectSetInteger(0, NewPendingOppTPRRe, OBJPROP_FONTSIZE, NOFontSize);
    ObjectSetString(0,  NewPendingOppTPRRe, OBJPROP_TOOLTIP, "Take Profit by R:R");
    ObjectSetInteger(0, NewPendingOppTPRRe, OBJPROP_ALIGN, ALIGN_CENTER);
    ObjectSetString(0,  NewPendingOppTPRRe, OBJPROP_FONT, NOFont);
    ObjectSetString(0,  NewPendingOppTPRRe, OBJPROP_TEXT,  (string)TPPRR_value);
    ObjectSet(          NewPendingOppTPRRe, OBJPROP_SELECTABLE, false);
    ObjectSetInteger(0, NewPendingOppTPRRe, OBJPROP_COLOR, clrBlack);
    j++;
...
void ShowSettings()
{
	...
	else if (RiskBase == MonetaryValue) TextRiskBase = "MONETARY VALUE";
...
void ChangeRiskBase()
{
    ...
    if (Tmp == "FREE MARGIN")
    {
        ObjectSetString(0, SettingsRiskBaseE, OBJPROP_TEXT, "MONETARY VALUE");
        return;
    }
    if (Tmp == "MONETARY VALUE")
    {
...
void SaveSettingsChanges()
{
	...
	else if (StringFind(SettingsRiskBaseTmp, "MONETARY VALUE") >= 0) RiskBase = MonetaryValue;
    ShowSettings();
}
...
double RecommendedSize()
{
	...
	else if (RiskBase == MonetaryValue) Base = 100;
    double Points = MarketInfo(Symbol(), MODE_POINT);
