...
// http://fxcodebase.com/code/viewtopic.php?f=38&t=69245
//+------------------------------------------------------------------------------------------------+
...
input int Length = 10; // RVI period

input int cciPeriod = 20; // CCI Periods:
input int cciBuyLevel = -100; // CCI Level Long Filter:
input int cciSellLevel = 100; // CCI Level Short Filter:
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      double value0_0 = iCustom(_symbol, _timeframe, "ERVI", Length, 0, period);
      double value1_0 = iCustom(_symbol, _timeframe, "ERVI", Length, 1, period);
      double value0_1 = iCustom(_symbol, _timeframe, "ERVI", Length, 0, period + 1);
      double value1_1 = iCustom(_symbol, _timeframe, "ERVI", Length, 1, period + 1);
      
      // NOTE: CCI long Filter
      double cci = iCCI(NULL, 0, cciPeriod, PRICE_CLOSE,0); 
      
      return value0_0 >= value1_0 && value0_1 < value1_1 && cci < cciBuyLevel;
   }
};
...
public:
   ...
   bool IsPass(const int period, const datetime date)
   {
      ...
      // NOTE: CCI short Filter
      double cci = iCCI(NULL, 0, cciPeriod, PRICE_CLOSE,0); 

      return value0_0 <= value1_0 && value0_1 > value1_1 && cci > cciSellLevel;
   }
};
...
int OnInit()
{
   double temp = iCustom(NULL, 0, "ERVI", 0, 0);
   if (GetLastError() == ERR_INDICATOR_CANNOT_LOAD)
   {
      Alert("Please, install the 'ERVI' indicator");
      return INIT_FAILED;
   }
   #ifdef SHOW_ACCOUNT_STAT
      stats = NULL;
