//Available @  https://fxcodebase.com/code/viewtopic.php?f=38&t=75477
...
input const string indicator_file = "TBF";                          // Indicator File:
//---
int  bufferToBuy              = 5;
int  bufferToStrongBuy        = 7;
int  bufferToSell             = 6;
int  bufferToStrongSell       = 8;
  ...
  public:
    bool evaluate()
    {
        for (int i = 1; i <= candles_back_for_signals; i++) {
            ...
            double indi2 = Indi(bufferToStrongBuy, i);
            if (indi > 0 && indi != EMPTY_VALUE && indi != lastSignal) {
                lastSignal = indi;
                return true;
            }
            if (indi2 > 0 && indi2 != EMPTY_VALUE && indi2 != lastSignal) {
                lastSignal = indi2;
                return true;
            }
        }
  ...
  public:
    bool evaluate()
    {
        for (int i = 1; i <= candles_back_for_signals; i++) {
            ...
            double indi2 = Indi(bufferToStrongSell, i);
            if (indi > 0 && indi != EMPTY_VALUE && indi != lastSignal) {
                lastSignal = indi;
                return true;
            }
...
input bool filterByEmaOn = true; // Filter by EMA on?
input int                ma1_period        = 38;                             // Periods:
...
input int                ma2_period        = 62;                             // Periods:
...
void oninit_filterByEma()
{
    if(!filterByEmaOn) return;
