...
function Init()
    indicator:name("ZigZag with counter_v6");
    indicator:description("ZigZag with counts of Bars, Pips, Ticks Volumes, Swing % and Fibonacci Levels");
    ...
    indicator.parameters:addColor("Text_color_Top", "Top Label Text color", "Top Label Text color", core.rgb(128, 255, 0));
    indicator.parameters:addColor("Text_color_Bottom", "Bottom Label Text color", "Bottom Label Text color", core.rgb(255, 128, 255));
    indicator.parameters:addInteger("TextSize", "Text size", "Text size", 9);
end
...
function Prepare(nameOnly)
    ...
    --TextBuff 	= instance:createTextOutput ("Text", "Text", "Arial", instance.parameters.TextSize, core.H_Center, core.V_Top, instance.parameters.Text_color, first);
    ...
    -- Adapt the label's offset in relation with the choosen TimeFrame
--~     if TF_Bar == "t1" or TF_Bar == "m1" or TF_Bar == "m5" or TF_Bar == "m15" or TF_Bar == "m30" then
--~ 	TopPos = 1*pipSize;	BottomPos = 1*pipSize;
--~     elseif TF_Bar == "H1" or TF_Bar == "H2" or TF_Bar == "H3" or TF_Bar == "H4" or TF_Bar == "H6" or TF_Bar == "H8" then
--~ 	TopPos = 5*pipSize;	BottomPos = 5*pipSize;
--~     else
--~ 	TopPos = 7*pipSize;	BottomPos = 7*pipSize;
--~     end

--~    TopPos = 3*pipSize;	BottomPos = 3*pipSize;
    
--~   TopPos = 3;	BottomPos = 3;
--~     TopPos = 1*pipSize;	
--~     BottomPos = 1*pipSize;
...
function TextFormat(bars, pips, pips2, tickvol, percent1)	-- called by lines 256,  262, 272, 288, 298 and 304
	...
	for i = start, period, 1 do
		...
		elseif searchMode == searchPeak then
			if (LowMap[i] ~= 0 and LowMap[i] < last_peak) then
				...
				if prev_peak ~= nil then
					if Peak[prev_peak] > LowMap[i] then
						...
						TextBuff_Bottom:set(i, LowMap[i], TextFormat(i-prev_peak, Peak[prev_peak]-LowMap[i], LowMap[pp_peak]-Peak[prev_peak], RegisterTicksVolume(prev_peak, i), "- " .. string.format("%.4f", (Peak[prev_peak]-LowMap[i])/Peak[prev_peak]*100)));	
					else
			...
			if HighMap[i] ~= 0 and LowMap[i] == 0 then
				...
				TextBuff_Top:set(i, HighMap[i], TextFormat(i-last_peak_i, HighMap[i]-last_peak, Peak[last_peak_i]-HighMap[pp_peak], RegisterTicksVolume(last_peak_i, i), "+ " .. string.format("%.4f", ((HighMap[i] / last_peak)-1)*100)));
			...
			elseif searchMode == searchLawn then
				if (HighMap[i] ~= 0 and HighMap[i] > last_peak) then
					...
					if prev_peak ~= nil then
						...
						TextBuff_Top:set(i, HighMap[i], TextFormat(i-prev_peak, HighMap[i]-Peak[prev_peak], Peak[prev_peak]-HighMap[pp_peak], RegisterTicksVolume(prev_peak, i), "+ " .. string.format("%.4f", ((HighMap[i] / Peak[prev_peak])-1)*100)));
					end
					ReplaceLastPeak(i, searchMode, last_peak);
				end
				if LowMap[i] ~= 0 and HighMap[i] == 0 then
					if  last_peak > LowMap[i] then
						...
						TextBuff_Bottom:set(i, LowMap[i], TextFormat(i-last_peak_i, last_peak-LowMap[i], LowMap[pp_peak]-Peak[last_peak_i], RegisterTicksVolume(last_peak_i, i), "- " .. string.format("%.4f", (last_peak-LowMap[i])/last_peak*100)));
					else
