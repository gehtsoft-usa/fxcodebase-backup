...
--Modified for printint text with "RETURN" & "NEW LINE" in accordance with the LUA Language ; with also TopPos and BottomPos (see function Update() below) 
 if math.abs(pips)>math.abs(pips2) then
  return "" .. math.abs(bars)+1 .. " bars" .. "\r\n" .. math.floor(math.abs(pips)/pipSize*10+0.5)/10 .. " pips";
 else
  return "" .. math.abs(bars)+1 .. " bars" .. "\r\n" .. math.floor(math.abs(pips)/pipSize*10+0.5)/10 .. " pips" .. "\r\n" .. " " .. math.floor(math.abs(pips/pips2)*1000+0.5)/1000;
 end 
end
        ...
        for i = start, period, 1 do
            ...
            elseif searchMode == searchPeak then
                if (LowMap[i] ~= 0 and LowMap[i] < last_peak) then
                    ...
                    if prev_peak ~= nil then
                        if Peak[prev_peak] > LowMap[i] then
                            ...
                            pp_peak=GetPeak(-2);
                            TextBuff:set(i, LowMap[i]-BottomPos, TextFormat(i-prev_peak, Peak[prev_peak]-LowMap[i], LowMap[pp_peak]-Peak[prev_peak]));
                        else
                ...
                if HighMap[i] ~= 0 and LowMap[i] == 0 then
                    ...
                    pp_peak=GetPeak(-1);
                    TextBuff:set(i, HighMap[i]+TopPos, TextFormat(i-last_peak_i, HighMap[i]-last_peak, Peak[last_peak_i]-HighMap[pp_peak]));
            ...
            elseif searchMode == searchLawn then
                if (HighMap[i] ~= 0 and HighMap[i] > last_peak) then
                    ...
                    if prev_peak ~= nil then
                        ...
                        DeleteLabels(prev_peak+1, i);
                        pp_peak=GetPeak(-2);
                        TextBuff:set(i, HighMap[i]+TopPos, TextFormat(i-prev_peak, HighMap[i]-Peak[prev_peak], Peak[prev_peak]-HighMap[pp_peak]));
                    end
                    ReplaceLastPeak(i, searchMode, last_peak);
                end
                if LowMap[i] ~= 0 and HighMap[i] == 0 then
                    if  last_peak > LowMap[i] then
                        ...
                        DeleteLabels(last_peak_i+1, i);
                        pp_peak=GetPeak(-1);
                        TextBuff:set(i, LowMap[i]-BottomPos, TextFormat(i-last_peak_i, last_peak-LowMap[i], LowMap[pp_peak]-Peak[last_peak_i]));
                    else
