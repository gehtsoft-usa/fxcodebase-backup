...
function Init()
    ...
    indicator.parameters:addBoolean("LabelFibo", "Fibonacci Levels", "Select true to see the Fibonacci levels in the label and false to not see them", true);
	 ...
	 indicator.parameters:addBoolean("VolumeBar", "Volume/Bar", "Select true to see the Volume/Bar in the label and false to not see them", true);
    indicator.parameters:addBoolean("VolumePip", "Volume/Pip", "Select true to see the Volume/Pip in the label and false to not see them", true);
    ...
    indicator.parameters:addGroup("Displaying Style");
...
local DisplayVolumeBar;
local DisplayVolumePip
...
function Prepare(nameOnly)
	...
	DisplayVolumeBar = instance.parameters.VolumeBar;
    DisplayVolumePip	= instance.parameters.VolumePip;
	
	assert(not(DisplayBars==false and DisplayPips==false and DisplayVols==false and DisplaySwng==false and DisplayHLp==false and DisplayFibo==false and DisplayVolumeBar==false and DisplayVolumePip==false), "At least one information must be turned on");
...
function TextFormat(type, bars, pips, pips2, tickvol, percent1, HLprice)	-- called by lines at end of code
	...
	if DisplayBars then TextLabel 	= TextLabel .. "Bars " .. type .. ": " .. math.abs(bars)+1; end
	...
	if DisplayVolumeBar then TextLabel 	= TextLabel .. "\r\n" .."Volume/Bar: " .. math.floor((tickvol/(math.abs(bars)+1))+0.5); end	
	if DisplayVolumePip then TextLabel 	= TextLabel .. "\r\n" .. "Volume/Pip: " ..math.floor((tickvol/(math.abs(pips)/pipSize))+0.5); end
	...
	for i = start, period, 1 do
		...
		elseif searchMode == searchPeak then
			if (LowMap[i] ~= 0 and LowMap[i] < last_peak) then
				...
				if prev_peak ~= nil then
					if Peak[prev_peak] > LowMap[i] then
						...
						pp_peak=GetPeak(-2);
						TextBuff_Bottom:set(i, LowMap[i]-V_Shift, TextFormat("LL", i-prev_peak, Peak[prev_peak]-LowMap[i], LowMap[pp_peak]-Peak[prev_peak], RegisterTicksVolume(prev_peak, i), "- " .. string.format("%.4f", (Peak[prev_peak]-LowMap[i])/Peak[prev_peak]*100), "Lowest = " .. string.format("%5." .. PricePrecision .. "f", LowMap[i])));	
					else
			...
			if HighMap[i] ~= 0 and LowMap[i] == 0 then
				...
				pp_peak=GetPeak(-1);
				TextBuff_Top:set(i, HighMap[i]+V_Shift, TextFormat("HH", i-last_peak_i, HighMap[i]-last_peak, Peak[last_peak_i]-HighMap[pp_peak], RegisterTicksVolume(last_peak_i, i), "+ " .. string.format("%.4f", ((HighMap[i] / last_peak)-1)*100), "Highest = " .. string.format("%5." .. PricePrecision .. "f", HighMap[i])));
			...
			elseif searchMode == searchLawn then
				if (HighMap[i] ~= 0 and HighMap[i] > last_peak) then
					...
					if prev_peak ~= nil then
						...
						DeleteLabels(prev_peak+1, i);
						pp_peak=GetPeak(-2);
						TextBuff_Top:set(i, HighMap[i]+V_Shift, TextFormat("HH", i-prev_peak, HighMap[i]-Peak[prev_peak], Peak[prev_peak]-HighMap[pp_peak], RegisterTicksVolume(prev_peak, i), "+ " .. string.format("%.4f", ((HighMap[i] / Peak[prev_peak])-1)*100), "Highest = " .. string.format("%5." .. PricePrecision .. "f", HighMap[i])));
					end
					ReplaceLastPeak(i, searchMode, last_peak);
				end
				if LowMap[i] ~= 0 and HighMap[i] == 0 then
					if  last_peak > LowMap[i] then
						...
						DeleteLabels(last_peak_i+1, i);
						pp_peak=GetPeak(-1);
						TextBuff_Bottom:set(i, LowMap[i]-V_Shift, TextFormat("LL", i-last_peak_i, last_peak-LowMap[i], LowMap[pp_peak]-Peak[last_peak_i], RegisterTicksVolume(last_peak_i, i), "- " .. string.format("%.4f", (last_peak-LowMap[i])/last_peak*100), "Lowest = " .. string.format("%5." .. PricePrecision .. "f", LowMap[i])));
					else
