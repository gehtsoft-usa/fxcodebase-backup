...
-- http://fxcodebase.com/code/viewtopic.php?f=17&t=66825
...
function Init()
    indicator:name("ZigZag Threshold");
    indicator:description("v1 2018-10-13");
    ...
    indicator.parameters:addInteger("min_threshold", "Threshold, %", "", 33);
    ...
    if period >= Depth then
        ...
        if val == lastlow then
            ...
            if (source.low[period] - val) > (source:pipSize() * Deviation) then
                val = nil;
            elseif (highIndex ~= -1 and lowIndex ~= -1 and (HighMap[highIndex] - val) < (HighMap[highIndex] - LowMap[lowIndex]) * min_threshold / 100) then
                val = nil;
            else
        ...
        if val == lasthigh then
            ...
            if (val - source.high[period]) > (source:pipSize() * Deviation) then
                val = nil;
            elseif (highIndex ~= -1 and lowIndex ~= -1 and (val - LowMap[lowIndex]) < (HighMap[highIndex] - LowMap[lowIndex]) * min_threshold / 100) then
                val = nil;
            else
                -- check for the previous backstep lows
                for i = period - 1, period - Backstep + 1, -1 do
