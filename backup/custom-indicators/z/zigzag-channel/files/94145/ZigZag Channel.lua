...
function Draw(stage, context)
    ...
    Lines(Up, Down, context)
end

function Lines(Up, Down, context)
    if Up == nil or Down == nil or table.getn(Up) < 2 or table.getn(Down) < 2 then
        return
    end

    local tx1, x = context:positionOfBar(Up[1])
    local tx2, x = context:positionOfBar(Up[2])
    local bx1, x = context:positionOfBar(Down[1])
    local bx2, x = context:positionOfBar(Down[2])

    local visible, ty1 = context:pointOfPrice(source.high[Up[1]])
    local visible, ty2 = context:pointOfPrice(source.high[Up[2]])

    local visible, by1 = context:pointOfPrice(source.low[Down[1]])
    local visible, by2 = context:pointOfPrice(source.low[Down[2]])

    local itx1, ity1, itx2, ity2 =
    ...
    context:drawLine(2, x3, y3, x4, y4)
end
