...
function Prepare(nameOnly) 
    ...
    for i=1, Lookback, 1 do
	    Top[i] = instance:addStream("Top " .. i, core.Line, name, "Top " .. i, UpColor, first);
        Top[i]:setWidth(Width);
        Top[i]:setStyle(Style);
	
	    Bottom[i] = instance:addStream("Bottom " .. i, core.Line, name, "Bottom " .. i, DownColor, first);
        Bottom[i]:setWidth(Width);
        Bottom[i]:setStyle(Style);
    end
...
local a2, c2= math2d.lineEquation (bx1, by1, bx2, by2);
 ...
 local Y2= GetYofABCLine(source:size()-1, a2, c2);
	...
	core.drawLine (Top[index], core.range(tx2, source:size()-1), ty2, tx2, Y1, source:size()-1, UpColor)
    core.drawLine (Bottom[index], core.range(bx2, source:size()-1), by2, bx2, Y2, source:size()-1, DownColor)	
	
	for period =tx2-1, first , -1 do
	if Top[index][period]~= nil then
	Top[index][period]= nil;
	else
	break;
	end
	...
	if Bottom[index][period]~= nil then
	Bottom[index][period]= nil;
	else
	break;
	end
