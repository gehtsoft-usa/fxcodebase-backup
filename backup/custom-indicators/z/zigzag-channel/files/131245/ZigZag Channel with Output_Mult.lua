...
function RegisterUp(context, period, hist)
    ...
    local _, y = context:pointOfPrice(bid_source.high[bid_period]);
...
function RegisterDown(context, period, hist)
    ...
    local _, y = context:pointOfPrice(ask_source.low[ask_period]);
    local x = context:positionOfBar(period);
    if hist then
        ...
        if last_peak_2 ~= nil then
            if searchMode == -1 then
                ...
                then
                    ...
                    local _, y1 = context:pointOfPrice(bid_source.high[bid_last_peakPeriod_2]);
                    ...
                    local _, y2 = context:pointOfPrice(bid_source.high[bid_peakPeriod]);
                ...
                then
                    ...
                    local _, y1 = context:pointOfPrice(ask_source.low[ask_last_peakPeriod_2]);
                    local x1 = context:positionOfBar(last_peakPeriod_2);
                    local ask_peakPeriod = core.findDate(ask_source, source:date(peakPeriod), false);
                    local _, y2 = context:pointOfPrice(ask_source.low[ask_peakPeriod]);
                    local x2 = context:positionOfBar(peakPeriod);
