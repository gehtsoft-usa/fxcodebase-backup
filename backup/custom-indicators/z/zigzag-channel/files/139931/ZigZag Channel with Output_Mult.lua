...
function Init()
    ...
    indicator.parameters:addDouble("extension_1u", "Up Extension 1, %", "", 1.272)
    indicator.parameters:addDouble("extension_2u", "Up Extension 2, %", "", 1.5);
    indicator.parameters:addDouble("extension_3u", "Up Extension 3, %", "", 1.618);
    indicator.parameters:addDouble("extension_1d", "Down Extension 1, %", "", 1.272)
    indicator.parameters:addDouble("extension_2d", "Down Extension 2, %", "", 1.5);
    indicator.parameters:addDouble("extension_3d", "Down Extension 3, %", "", 1.618);
    indicator.parameters:addColor("extension_up_color", "Extension Up Color", "Color", core.colors().Red);
    indicator.parameters:addInteger("extension_up_width", "Extension Up Width", "Width", 1, 1, 5);
    indicator.parameters:addInteger("extension_up_style", "Extension Up Style", "Style", core.LINE_SOLID);
    indicator.parameters:setFlag("extension_up_style", core.FLAG_LINE_STYLE);
    indicator.parameters:addColor("extension_down_color", "Extension Down Color", "Color", core.colors().Blue);
    indicator.parameters:addInteger("extension_down_width", "Extension Down Width", "Width", 1, 1, 5);
    indicator.parameters:addInteger("extension_down_style", "Extension Down Style", "Style", core.LINE_SOLID);
    indicator.parameters:setFlag("extension_down_style", core.FLAG_LINE_STYLE);

    indicator.parameters:addColor("swing_top_color", "Swing Top Color", "Color", core.colors().Red);
    indicator.parameters:addInteger("swing_top_width", "Swing Top Width", "Width", 1, 1, 5);
    indicator.parameters:addInteger("swing_top_style", "Swing Top Style", "Style", core.LINE_SOLID);
    indicator.parameters:setFlag("swing_top_style", core.FLAG_LINE_STYLE);

    indicator.parameters:addColor("swing_middle_color", "Swing Middle Color", "Color", core.colors().Black);
    indicator.parameters:addInteger("swing_middle_width", "Swing Middle Width", "Width", 1, 1, 5);
    indicator.parameters:addInteger("swing_middle_style", "Swing Middle Style", "Style", core.LINE_SOLID);
    indicator.parameters:setFlag("swing_middle_style", core.FLAG_LINE_STYLE);

    indicator.parameters:addColor("swing_bottom_color", "Swing Bottom Color", "Color", core.colors().Green);
    indicator.parameters:addInteger("swing_bottom_width", "Swing Bottom Width", "Width", 1, 1, 5);
    indicator.parameters:addInteger("swing_bottom_style", "Swing Bottom Style", "Style", core.LINE_SOLID);
    indicator.parameters:setFlag("swing_bottom_style", core.FLAG_LINE_STYLE);
...
local SWING_UP_PEN = 6;
local SWING_DOWN_PEN = 11;
local SWING_MIDDLE_PEN = 12;
local EXTENSION_UP_PEN = 13;
local EXTENSION_DOWN_PEN = 14;
local FONT = 7;
...
local extension_1u, extension_2u, extension_3u
local extension_1d, extension_2d, extension_3d
local init = false;
function Draw(stage, context) 
    ...
    if not init then
        ...
        context:createPen(SWING_UP_PEN, context:convertPenStyle(instance.parameters.swing_top_style), instance.parameters.swing_top_width, instance.parameters.swing_top_color);
        context:createPen(SWING_DOWN_PEN, context:convertPenStyle(instance.parameters.swing_bottom_style), instance.parameters.swing_bottom_width, instance.parameters.swing_bottom_color);
        context:createPen(SWING_MIDDLE_PEN, context:convertPenStyle(instance.parameters.swing_middle_style), instance.parameters.swing_middle_width, instance.parameters.swing_middle_color);
        context:createPen(EXTENSION_UP_PEN, context:convertPenStyle(instance.parameters.extension_up_style), instance.parameters.extension_up_width, instance.parameters.extension_up_color);
        context:createPen(EXTENSION_DOWN_PEN, context:convertPenStyle(instance.parameters.extension_down_style), instance.parameters.extension_down_width, instance.parameters.extension_down_color);
        extension_1u = instance.parameters.extension_1u;
        extension_2u = instance.parameters.extension_2u;
        extension_3u = instance.parameters.extension_3u;
        extension_1d = instance.parameters.extension_1d;
        extension_2d = instance.parameters.extension_2d;
        extension_3d = instance.parameters.extension_3d;
        context:createFont(FONT, "Arial", 0, context:pixelsToPoints(instance.parameters.font_size), 0);
    end
        ...
        if last_peak_2 ~= nil then
            if searchMode == -1 then
                ...
                if source.high[peakPeriod] > source.high[last_peakPeriod_2] 
                    ...
                    and (current_pullback * 100 >= pullback or (lines < 2 and not use_poolback))
                then
    ...
    local top_y = math.max(y1, y2);
    local bottom_y = math.min(y1, y2);

    local up = math.max(peak1, peak2);
    local down = math.min(peak1, peak2);
    local range = up - down;

    local _, extension_y_1 = context:pointOfPrice(up + range * (extension_1u - 1));
    local _, extension_y_2 = context:pointOfPrice(up + range * (extension_2u - 1));
    local _, extension_y_3 = context:pointOfPrice(up + range * (extension_3u - 1));
    context:drawLine(EXTENSION_UP_PEN, x, extension_y_1, context:right(), extension_y_1);
    context:drawLine(EXTENSION_UP_PEN, x, extension_y_2, context:right(), extension_y_2);
    context:drawLine(EXTENSION_UP_PEN, x, extension_y_3, context:right(), extension_y_3);

    context:drawLine(SWING_UP_PEN, x, top_y, context:right(), top_y);
    context:drawLine(SWING_MIDDLE_PEN, x, (y2 + y1) / 2, context:right(), (y2 + y1) / 2);
    context:drawLine(SWING_DOWN_PEN, x, bottom_y, context:right(), bottom_y);

    local _, extension_y_1 = context:pointOfPrice(down - range * (extension_1d - 1));
    local _, extension_y_2 = context:pointOfPrice(down - range * (extension_2d - 1));
    local _, extension_y_3 = context:pointOfPrice(down - range * (extension_3d - 1));
    context:drawLine(EXTENSION_DOWN_PEN, x, extension_y_1, context:right(), extension_y_1);
    context:drawLine(EXTENSION_DOWN_PEN, x, extension_y_2, context:right(), extension_y_2);
    context:drawLine(EXTENSION_DOWN_PEN, x, extension_y_3, context:right(), extension_y_3);

    local distance = win32.formatNumber((peak1 - peak2) / source:pipSize(), false, 1);
...
function CreateZigZag(stream, Depth, Deviation, Backstep, ZigC, ZagC)
    ...
    function zz:Calc(period)
        ...
        local val = mathex.min(source.open, range, period)
        if val ~= self.lastlow then
            self.lastlow = val
            if (source.open[period] - val) > (source:pipSize() * self.Deviation) then
                val = nil
            else
            ...
            if source.open[period] == val then
                self.LowMap[period] = val
            else
                self.LowMap[period] = 0
            end
        end
        val = mathex.max(source.open, range, period)
        if val ~= lasthigh then
            self.lasthigh = val
            if (val - source.open[period]) > (source:pipSize() * self.Deviation) then
                val = nil
            else
