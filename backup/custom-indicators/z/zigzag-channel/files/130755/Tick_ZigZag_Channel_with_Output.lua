...
{ 
    {
        ...
        UpCondition = function (period)
            return (Top[period] - Bottom[period]) / source:pipSize() >= threshold;
        end,
        DownCondition = function (period)
            return (Bottom[period] - Top[period]) / source:pipSize() >= threshold;
        end,
...
function Init()
    ...
    indicator.parameters:addDouble("threshold", "Alert threshold", "", 0)
end
...
function Prepare(nameOnly)
    ...
    if source:isBid() then
        bid = source;
        ask = core.host:execute("getHistory", 2, source:instrument(), source:barSize(), source:date(0), 0, false);
    else
        ask = source;
        bid = core.host:execute("getHistory", 2, source:instrument(), source:barSize(), source:date(0), 0, true);
    end
...
function Update(period, mode)
    ...
    local bid_period = core.findDate(bid, source:date(period), false);
    if bid_period == -1 then
        return;
    end
    local ask_period = core.findDate(ask, source:date(period), false);
    if ask_period == -1 then
        return;
    end
    ...
    if ask_period <= Depth then
        return;
    end
    -- fill high/low maps
    -- get the lowest low for the last depth periods
    local val = mathex.min(ask, ask_period - Depth + 1, ask_period)
    if val == lastlow then
        ...
        if (ask[ask_period] - val) > (source:pipSize() * Deviation) then
            val = nil
        else
    ...
    if ask[ask_period] == val then
        LowMap[period] = val
    else
        LowMap[period] = 0
    end
    if bid_period <= Depth then
        return;
    end
    -- get the lowest low for the last depth periods
    val = mathex.max(bid, bid_period - Depth + 1, bid_period)
    if val == lasthigh then
        ...
        lasthigh = val
        -- if current low is higher for more than Deviation pips, ignore
        if (val - bid[bid_period]) > (source:pipSize() * Deviation) then
            val = nil
        else
            -- check for the previous backstep lows
            for i = period - 1, period - Backstep + 1, -1 do
    ...
    for i = source:size() - 2, first, -1 do
        if (SearchMode[i] ~= searchMode and SearchMode[i] ~= 0) then
            ...
            if Total == Period + 4 then
                DrawLine(Up[1], Up[2], Top, UpColor, true)
                DrawLine(Down[1], Down[2], Bottom, DownColor, false)
                break
            end
        end
    end
...
function DrawLine(x1, x2, target, color, isBid)
    local y1, y2;
    if isBid then
        local xx = core.findDate(bid, source:date(x1), false);
        y1 = bid[xx];
        xx = core.findDate(bid, source:date(x2), false);
        y2 = bid[xx];
    else
        local xx = core.findDate(ask, source:date(x1), false);
        y1 = ask[xx];
        xx = core.findDate(ask, source:date(x2), false);
        y2 = ask[xx];
    end
