...
function Update(period, mode)
    ...
    local prev_up;
    local prev_down;
    local up_drawn = false;
    local down_drawn = false;
    local peaks = zz:EnumPeaks();
    while (peaks:Next()) do
        local peakPeriod, peak, searchMode = peaks:GetData();
        if searchMode == -1 then
            if prev_up ~= nil and source[peakPeriod] > source[prev_up] and not up_drawn then
                up_drawn = true;
                LineUp(period, prev_up, peakPeriod);
            end
            prev_up = peakPeriod;
        else
            if prev_down ~= nil and source[peakPeriod] < source[prev_down] and not down_drawn then
                down_drawn = true;
                LineDown(period, prev_down, peakPeriod);
            end
            prev_down = peakPeriod;
        end
        if up_drawn and down_drawn then
            break
        end
    end
end
...
function LineUp(period, tx1, tx2)
    local ty1 = source[tx1]
    local ty2 = source[tx2]
...
function LineDown(period, bx1, bx2)
    local by1 = source[bx1]
    local by2 = source[bx2]
