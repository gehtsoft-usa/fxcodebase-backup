-- Available @ https://fxcodebase.com/code/viewtopic.php?f=17&t=75995
...
function Init()
    ...
    indicator.parameters:addColor("Histogram_color_UP", "Histogram Color", "", core.rgb(0, 128, 0))
    indicator.parameters:addColor("Histogram_color_DN", "Histogram Color", "", core.rgb(128, 0, 0))
end
...
function Prepare(nameOnly)
    ...
    hist = instance:addStream("Histogram", core.Bar, "Histogram", "Histogram", instance.parameters.Histogram_color_UP, first, 0);
...
function Update(period, mode)
    ...
    while enum:Next() do
        local peak_period, peak_value, search_mode = enum:GetData();
        if peak_period ~= nil then
            if search_mode == 1 then
                hist[period] = peak_value;
                hist:setColor(period, instance.parameters.Histogram_color_UP);
            else
                hist[period] = -peak_value;
                hist:setColor(period, instance.parameters.Histogram_color_DN);
            end
            break;
        end
    end
