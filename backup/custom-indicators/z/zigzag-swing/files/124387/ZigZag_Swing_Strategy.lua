...
--+------------------------------------------------------------------+
function Init()
    strategy:name("ZigZag swing strategy")
    strategy:setTag("NonOptimizableParameters", "Email,SendEmail,SoundFile,RecurrentSound,PlaySound,ShowAlert")
    strategy:description("ZigZag swing strategy")

    strategy.parameters:addGroup("Parameters")
    strategy.parameters:addInteger("Depth", "Depth", "", 12)
    strategy.parameters:addInteger("Deviation", "Deviation", "", 5)
    strategy.parameters:addInteger("Backstep", "Backstep", "", 3)
...
function Prepare()
    ...
    assert(
        core.indicators:findIndicator("ZIGZAG_OSCILLATOR") ~= nil,
        "ZIGZAG_OSCILLATOR" .. " indicator must be installed"
    )
    ZZ_Osc =
...
function ExtUpdate(id, source, period)
    ZZ_Osc:update(core.UpdateLast)

    if (ZZ_Osc.DATA:first() > (period - 1)) then
        return
    end
    ...
    if ZZ_Osc.DATA[period - 1] < 0 and ZZ_Osc.DATA[period] > 0 then
