...
-- http://fxcodebase.com/code/viewtopic.php?f=17&t=69056
...
function Init()
    ...
    indicator.parameters:addDouble("min_threshold", "Threshold, %", "", 0.1);
...
function Update(period, mode)
    ...
    if period >= Depth then
        ...
        if val == lastlow then
            ...
            elseif highIndex ~= -1 
                and lowIndex ~= -1 
                and 100 * (HighMap[highIndex] - val) / val < min_threshold
            then
                val = nil;
            else
        ...
        if val == lasthigh then
            ...
            elseif highIndex ~= -1 
                and lowIndex ~= -1 
                and 100 * (val - LowMap[lowIndex]) < min_threshold
            then
                val = nil;
            else
                -- check for the previous backstep lows
