...
local Modules = {};
function Init()
    indicator:name("Lorepiu Buy-Sell Limit Order");
    ...
    indicator.parameters:addInteger("x", "X, %", "", 30);
end
...
function Prepare(nameOnly)
    ...
    core.host:execute("addCommand", 1, "Open Buy-Sell Orders", "Open Buy-Sell Orders");
end
...
function AsyncOperationFinished(cookie, successful, message, message1, message2)
    if cookie == 1 then
        local range = source.high[NOW - 1] - source.low[NOW - 1];
        trading:EntryOrder(source:instrument())
            :SetAccountID(instance.parameters.Account)
            :SetAmount(instance.parameters.Amount)
            :SetSide("B") 
            :SetRate(source.open[NOW] + range * x / 100) 
            :Execute();
        trading:EntryOrder(source:instrument())
            :SetAccountID(instance.parameters.Account)
            :SetAmount(instance.parameters.Amount)
