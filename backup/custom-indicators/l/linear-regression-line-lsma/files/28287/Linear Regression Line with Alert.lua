...
function Init()
    indicator:name("Linear Regression Line with Alert");
    indicator:description("Linear Regression Line with Alert");
	...
	Parameters (1, "Price / Linear Regression");	
	Parameters (2, "Linear Regression Slope");
end
...
function Activate (id, period)
	  ...
	  if id == 1  and ON[id]  then
			...
			if  source[period] > LRL[period] 
			and   source[period-1] <= LRL[period-1] 
			then
			           
						    
             core.host:execute("drawLabel1", source:serial(period), source:date(period),  core.CR_CHART, LRL[period], core.CR_CHART, core.H_Center, core.V_Bottom, font, UpTrendColor, "\225");
			...
			elseif  source[period] < LRL[period] 
			and   source[period-1] >= LRL[period-1] 
            then			
			
			            			 
			               core.host:execute("drawLabel1", source:serial(period), source:date(period),  core.CR_CHART, LRL[period], core.CR_CHART, core.H_Center, core.V_Top, font, DownTrendColor, "\226");						   
	   ...
	   if id == 2  and ON[id]  then
			...
			elseif  LRL[period]  < LRL[period-1] 
			and LRL[period-1]  >= LRL[period-2] 
            then			
