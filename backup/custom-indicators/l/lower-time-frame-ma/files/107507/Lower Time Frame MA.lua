...
-- http://fxcodebase.com/code/viewtopic.php?f=17&t=63739
...
function Init()
    indicator:name("Lower Time Frame MA");
...
local Number=0;
 ...
 function Prepare(nameOnly)   
    ...
    assert(core.indicators:findIndicator(Method) ~= nil, Method .. " indicator must be installed");
	 Temp1= core.indicators:create(Method, source.close ,  Period);  				
	 ifirst= Temp1.DATA:first()*2;
	
	 for i = 1, Number ,  1 do
	 SourceData[i]  = core.host:execute("getSyncHistory",  source:instrument(),  TF[i], source:isBid(),math.min(300,ifirst), 2000 + i , 1000 +i);	 	 
	 loading[i]  = true;  	 
	 Indicator[i] = core.indicators:create(Method, SourceData[i].close ,  Period);   
	 end	
