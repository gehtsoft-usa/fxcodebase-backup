...
function Init()
    indicator:name("Generic Laguerre");
    ...
    indicator.parameters:addInteger("Repetition", "Repetition Number", "", 4);	
    ...
    indicator.parameters:addDouble("Level1", "1. Level","", 0.8);
	indicator.parameters:addDouble("Level2", "2. Level","", 0.5); 
	indicator.parameters:addDouble("Level3", "3. Level","", 0.2);	
...
-- Routine
 function Prepare(nameOnly)   
    ...
    local name = profile:id() .. "(" ..  instance.source:name().. "," ..  Gamma .. "," ..  Repetition .. ")";
    instance:name(name); 
...
function Update(period, mode) 
	...
	return;
		...
		for i= 1, Repetition, 1 do
			if i==1 then
			L[i][period] = (1 - Gamma) * source[period] + Gamma * L[i][period-1]
			else
			L[i][period] = -Gamma * L[i-1][period] + L[i-1][period-1] + Gamma * L[i][period-1];
			end
		end
		...
		for i= 1, Repetition-1, 1 do
			
			if (L[i][period] >= L[i+1][period]) then
			CU = CU +  L[i][period] - L[i+1][period];
			else
			CD = CD +  L[i+1][period] - L[i][period];
			end
		end
