...
{ 
    {
        ...
        UpCondition = function (period)
            ...
            return core.crossesOver(source.close, max, period);
        end,
        DownCondition = function (period)
            ...
            return core.crossesUnder(source.close, min, period);
        end,
...
function Prepare(nameOnly)
	...
	local name =
		profile:id() ..
		"(" .. source:name() .. ", " .. tostring(Buy) .. ", " .. tostring(Sell) .. ", " .. tostring(Type) .. ")"
	instance:name(name)
...
function indi_alerts:Prepare()
    ...
    for i = 1, 100 do 
        ...
        function alert:UpAlert(source, period, level, historical_period)
            if self.FilterConsecutive then
                local last_signal = self:GetLast(period - 1);
                core.host:trace(core.formatDate(source:date(period)) .. " " .. tostring(last_signal))
                if last_signal == 1 then
                    return;
                end
            end
