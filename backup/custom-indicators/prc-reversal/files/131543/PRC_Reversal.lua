...
--http://fxcodebase.com/code/viewtopic.php?f=17&t=69464
...
{ 
    {
        ...
        UpCondition = function (period)
            ...
            local sto_pass = false;
            for i = 0, 2 do
                if not ssd.K:hasData(period - i) then
                    return false;
                end
                if ssd.K[period - i] < 20 then
                    sto_pass = true;
                end
            end
            if not sto_pass then
                return false;
            end
            local lowest3 = mathex.min(source, period - 3, period);
            local lowest50_1 = mathex.min(source, period - 51, period - 1);
            local lowest50_2 = mathex.min(source, period - 52, period - 2);
            local lowest50_3 = mathex.min(source, period - 53, period - 3);
            return source.close[period - 1] < source.open[period - 1] 
                and source.close[period] > source.open[period]
                and source.close[period] > source.open[period - 1]
                and (lowest3 < lowest50_1 or lowest3 < lowest50_2 or lowest3 < lowest50_3)
        end,
        DownCondition = function (period)
            ...
            local highest3 = mathex.max(source, period - 3, period);
            local highest50_1 = mathex.min(source, period - 51, period - 1);
            local highest50_2 = mathex.min(source, period - 52, period - 2);
            local highest50_3 = mathex.min(source, period - 53, period - 3);
            return source.close[period - 1] > source.open[period - 1] 
                and source.close[period] < source.open[period]
                and source.close[period] < source.open[period - 1]
                and (highest3 > highest50_1 or highest3 > highest50_2 or highest3 > highest50_3)
        end,
...
function Init()
    indicator:name("PRC Seversal");
    indicator:description("");
    indicator:requiredSource(core.Bar);
    indicator:type(core.Indicator);
    
	indicator.parameters:addInteger("stoch_k", "Periods for %K", "", 10, 2, 1000);
    indicator.parameters:addInteger("stoch_sd", "%D slowing periods", "", 5, 2, 1000);
    indicator.parameters:addInteger("stoch_d", "Periods for %D", "", 5, 2, 1000);
