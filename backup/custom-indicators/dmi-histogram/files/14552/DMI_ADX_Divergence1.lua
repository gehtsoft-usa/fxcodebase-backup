...
function Init()
    indicator:name("DMI_ADX Divergence Trend");
    indicator:description("Shows DMI_ADX Divergence trends");
...
function Prepare(nameOnly)
	...
	assert(core.indicators:findIndicator("DMI_ADX_DIVERGENCE") ~= nil, "Please, download and install DMI_ADX_DIVERGENCE.LUA indicator");    
	
    DMI_ADX = core.indicators:create("DMI_ADX_DIVERGENCE", source, instance.parameters.Period, false);
...
function Update(period, mode)
    ...
    if DMI_ADX:getStream(1):hasData(period - 2) then
        l = math.abs(DMI_ADX:getStream(1)[period - 2]);
    ...
    if DMI_ADX:getStream(2):hasData(period - 2) then
        l = math.abs(DMI_ADX:getStream(2)[period - 2]);
        local prev = period - 2 - l;
        line_id = line_id + 1;
