-- Available @ https://fxcodebase.com/code/viewtopic.php?f=17&t=954&start=40
...
local indi_id = "3_LEVEL_ZZ_SEMAFOR";
...
function CreateIndicators(source)
    ...
    p:setInteger("Depth1", instance.parameters.Depth1);
    p:setInteger("Deviation1", instance.parameters.Deviation1);
    p:setInteger("Backstep1", instance.parameters.Backstep1);
    p:setInteger("Depth2", instance.parameters.Depth2);
    p:setInteger("Deviation2", instance.parameters.Deviation2);
    p:setInteger("Backstep2", instance.parameters.Backstep2);
...
function GetLastSignal(indi, source)
    indi[1]:update();
    local up1 = indi[1]:getTextOutput(0);
    local down1 = indi[1]:getTextOutput(1);
    local up2 = indi[1]:getTextOutput(2);
    local down2 = indi[1]:getTextOutput(3);
    local up3 = indi[1]:getTextOutput(4);
    local down3 = indi[1]:getTextOutput(5);
    core.host:trace(up1:size() .. tostring (source:size()));
    for i = 0, up1:size() - 1 do
        if up3:hasData(NOW - i) then
            level, label = up3:get(NOW - i);
            return 1, source:date(NOW - i), label;
        end
        if down3:hasData(NOW - i) then
            level, label = down3:get(NOW - i);
            return -1, source:date(NOW - i), label;
        end
        if up2:hasData(NOW - i) then
            level, label = up2:get(NOW - i);
            return 1, source:date(NOW - i), label;
        end
        if down2:hasData(NOW - i) then
            level, label = down2:get(NOW - i);
            return -1, source:date(NOW - i), label;
        end
        if up1:hasData(NOW - i) then
            level, label = up1:get(NOW - i);
            return 1, source:date(NOW - i), label;
        end
        if down1:hasData(NOW - i) then
            level, label = down1:get(NOW - i);
            return -1, source:date(NOW - i), label;
        end
    end
    return 0, nil, "-";
...
function DrawSignal(symbol, context)
    ...
    CellsBuilder:Add(FONT, symbol.Text, color, column, row, context.CENTER, backgound, GRID_PEN, true, false);
