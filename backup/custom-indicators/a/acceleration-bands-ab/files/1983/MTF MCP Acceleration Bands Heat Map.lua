...
--+------------------------------------------------------------------+
 ...
 function Add(id, TF,Flag, Instrument )
	...
	indicator.parameters:addInteger("Period".. id, "Number of periods", "", 20, 1, 200);
    
    indicator.parameters:addDouble("Factor".. id, "Factor", "Factor", 0.001);
end

 
 
function Init()
    indicator:name("MTF MCP Acceleration Bands Heat Map");
...
function Prepare(nameOnly) 
	...
	assert(core.indicators:findIndicator("AB") ~= nil, "Please, download and install AB.LUA indicator");    
	...
	s2, e2 = core.getcandle(iTF[i], 0, 0, 0);
	 ...
	 Number=Number+1;
		  ...
		  Factor[Number]= instance.parameters:getDouble("Factor" .. i);
	 ...
	 Label[Number]="";
                ...
                Label[Number]=Label[Number] .. " - " ..  TF[Number];				
			  	...
			  	Temp1= core.indicators:create("AB", source ,  Period[Number],Factor[Number]);  				
				ifirst= Temp1.DATA:first()*2;
				 ...
				 loading[Number]  = true;  	 
				 Indicator[Number] = core.indicators:create("AB", SourceData[Number] ,  Period[Number] ,Factor[Number]);   
...
function Draw (stage, context)
   ...
   context:setClipRectangle(context:left(), context:top(), context:right(), context:bottom());
        ...
        if not init then
			...
			context:createPen (51, context.SOLID, 1, OBOS)       
			   ...
			   x0, x1, x2 = context:positionOfBar (i);
			    ...
			    for j= 1, Number , 1 do
				  ...
				  if p~= false then
										...
										if Indicator[j].Central:hasData(p)   then 
										
												    if SourceData[j].close[p] > Indicator[j].Central[p]   then		 
													...
													if SourceData[j].close[p] >  Indicator[j].Top[p]
													or  SourceData[j].close[p] <  Indicator[j].Bottom[p]
													then
													C1=51;
													end
