...
function Init()
    ...
    indi_alerts:AddAlert("Middle line cross");
    indicator.parameters:addBoolean("mid_line_outside_channel_alert", "Alert when outside channel only", "", false);
end
...
local transparency;
local mid_line_outside_channel_alert;
...
function Prepare(nameOnly)
    ...
    mid_line_outside_channel_alert = instance.parameters.mid_line_outside_channel_alert;
...
function Activate(alert, period, historical_period)
    ...
    if alert.id == 1 and alert.ON then
        if core.crossesOver(source.close, Out[4], period)
            and (not mid_line_outside_channel_alert or Out[4][period] > math.max(Out[1][period], Out[2][period]))
        then
            alert:UpAlert(source, period, alert.Label .. " up", source.high[period], historical_period);
        elseif core.crossesUnder(source.close, Out[4], period)
            and (not mid_line_outside_channel_alert or Out[4][period] < math.min(Out[1][period], Out[2][period]))
        then
            alert:DownAlert(source, period, alert.Label .. " down", source.low[period], historical_period);
        end
    end
