...
local STRATEGY_NAME = "A WAVE Strategy";
function CreateParameters() 
	strategy.parameters:addInteger("Period", "Period", "", 14);
	strategy.parameters:addInteger("Period1", "1. EMA Period", "", 13);
	strategy.parameters:addInteger("Period2", "2. EMA Period", "", 2);
	strategy.parameters:addInteger("Period3", "3. EMA Period", "", 5);
end

local indi;
function CreateEntryIndicators(source)
    local profile = core.indicators:findIndicator("A WAVE INDICATOR");
    assert(profile ~= nil, "Please, download and install " .. "A WAVE INDICATOR" .. ".LUA indicator");
    local indicatorParams = profile:parameters();
    indi = core.indicators:create("A WAVE INDICATOR", source, instance.parameters.Period, instance.parameters.Period1, instance.parameters.Period2, instance.parameters.Period3);
end
...
function IsEntryLong(source, period)
    return core.crossesOver(indi.closeup, indi.Signal, period);
end
function IsEntryShort(source, period)
    return core.crossesUnder(indi.closeup, indi.Signal, period);
end
