...
function Init()
    indicator:name("FairValue of AD");
    indicator:description("Difference between Price Normalized AD and Price ");
...
function Prepare(nameOnly) 
   ...
   Label="Trade Station";
    ...
    if   (nameOnly) then
		...
		FairValue = instance:addStream("FairValue", core.Line, name, "", instance.parameters.Color, first);
		FairValue:setWidth(instance.parameters.width);
        FairValue:setStyle(instance.parameters.style);
		
		FairValue:setPrecision(math.max(2, instance.source:getPrecision()));
...
function Update(period, mode)
	...
	return;
		...
		FairValue[period]= AD[period]-source.close[period];
