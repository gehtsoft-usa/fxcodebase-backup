...
function Init()
    indicator:name("Adaptive Exponential Moving Average");
...
function Update(period, mode)
	...
	local   MLTP2=math.abs((source.close[period]-min)-(max-source.close[period]))/ (max-min);
	local Rate=MLTP1*(1+MLTP2);	
	
	if period <= Period1 then
	MA[period]=mathex.avg(source.close, period-Period1+1, period);
	else
	MA[period]= MA[period-1]+Rate*(source.close[period-1]-MA[period-1]);
