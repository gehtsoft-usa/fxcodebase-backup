...
-- Routine
 function Prepare(nameOnly) 
	...
	H= false;
	S= false;	
	OUT = instance:addStream("MACD", core.Bar, name .. ".MACD", "MACD", instance.parameters.MACD_color, firstPeriodMACD);
	...
	if Type == "Absolute" then
				...
				if (period >= firstPeriodMACD) then
					...
					OUT[period]=MACD[period];	
								...
								OUT:setColor(period, instance.parameters.UPHISTOGRAM_color);                     
								else
								OUT:setColor(period, instance.parameters.DOWNHISTOGRAM_color); 
								end		
						end  	 					
					end 
				end
