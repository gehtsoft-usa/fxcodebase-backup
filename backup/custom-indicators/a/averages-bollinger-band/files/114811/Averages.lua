...
--
--
function CMA2Init(source, n)
    local p = {};
        
    p.first = source:first();
    p.n = n;
	p.raw=instance:addInternalStream(source:first(), n/2+1);
    p.source = source;
	
	return p;
end

function CMA2Update(params, period, mode)
   
    local i;
    if period < params.source:size() -1 then
        params.raw[period]= params.source[period];	 
    else	
        for  i = period+1, (period+1+params.n/2), 1 do
            params.raw[i] = params.source[period]; 
        end	
	    params.raw[period] = params.source[period];   
	    params.raw[period-1] = params.source[period-1]; 	
	end
  
    if period < (params.first + params.n/2) or not params.source:hasData(period) then
        return;
	end

    if period < params.first + params.n + params.n/2 then
        return;
    end
   
    if period < params.source:size() - 1 then
        params.buffer[period - (params.n /2)] = mathex.avg(params.raw, period - params.n+1, period);
    else

        for i= 0, (params.n/2), 1 do
            params.buffer[period - (params.n /2)+i] =  mathex.avg(params.raw, period - params.n + 1 + i, period+i);
        end
    end
end
