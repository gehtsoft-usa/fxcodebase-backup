...
--http://fxcodebase.com/code/viewtopic.php?f=17&t=70963
...
local source, tr_rma, _plusDM, _minusDM, _plusDM_rma, _minusDM_rma, f_sig, f_sig_source, i_hlRange, i_hlTrend, diPlus, diMinus, sig;
local condition, dot;
local indi_alerts = {};
...
{ 
    {
        ...
        UpCondition = function (period)
            return condition[period - 1] ~= 0.5 and condition[period] == 0.5;
        end,
    ...
    },
    {
        Stage = 102,
        UpCondition = function (period)
            return condition[period - 1] ~= 1 and condition[period] == 1;
        end,
        DownCondition = function (period)
            return condition[period - 1] ~= -1 and condition[period] == -1;
        end,
...
function Init()
    indicator:name("ADX Di+ Di- [Gu5]");
    indicator:description("ADX Di+ Di- [Gu5]");
    indicator:requiredSource(core.Bar);
    indicator:type(core.Oscillator);

    indicator.parameters:addInteger("i_f_sigLen", "ADX Smoothing", "", 14)
    indicator.parameters:addInteger("i_diLen", "DI Length", "", 14)
    indicator.parameters:addInteger("i_hlRange", "Level Range", "", 20)
    indicator.parameters:addInteger("i_hlTrend", "Level Trend", "", 35)
...
function Prepare(nameOnly)
    ...
    i_hlRange = instance.parameters.i_hlRange;
    i_hlTrend = instance.parameters.i_hlTrend;
    tr_rma = core.indicators:create("ATR", source, instance.parameters.i_diLen);
    _plusDM = instance:addInternalStream(0, 0);
    _minusDM = instance:addInternalStream(0, 0);
    sig = instance:addStream("ADX", core.Line, "ADX", "ADX", core.rgb(0, 0, 0), 0, 0);
    sig:setWidth(3);
    sig:addLevel(i_hlRange, core.LINE_DOT, 1, c_gray);
    sig:addLevel(i_hlTrend, core.LINE_DOT, 1, c_gray);

    diPlus = instance:addStream("+DI", core.Line, "+DI", "+DI", core.colors().Green, 0, 0);
    diMinus = instance:addStream("-DI", core.Line, "-DI", "-DI", core.colors().Red, 0, 0);
    dot = instance:addStream("CrossDi", core.Dot, "Cross Di", "Cross Di", core.colors().Black, 0, 0);
    dot:setWidth(2);
    instance:createChannelGroup("di", "di", diPlus, diMinus, core.rgb(0, 0, 0), 128, true);

    condition = instance:addInternalStream(0, 0);
    _plusDM_rma = core.indicators:create("MVA", _plusDM, instance.parameters.i_diLen);
    _minusDM_rma = core.indicators:create("MVA", _minusDM, instance.parameters.i_diLen);
    f_sig_source = instance:addInternalStream(0, 0);
    f_sig = core.indicators:create("MVA", f_sig_source, instance.parameters.i_f_sigLen);
    indi_alerts:Prepare();
...
function change(period, stream)
    if period == 0 or not stream:hasData(period - 1) then
        return nil;
    end
    return stream:tick(period) - stream:tick(period - 1);
end
...
function Update(period, mode)
    ...
    local _up = change(period, source.high);
    local _down = change(period, source.low);
    if (_down == nil) then
        return;
    end
    _down = -_down;

    _plusDM[period] = na(_up) and nil or ((_up > _down and _up > 0) and _up or 0)
    _minusDM[period] = na(_down) and nil or ((_down > _up and _down > 0) and _down or 0)
    _plusDM_rma:update(mode);
    _minusDM_rma:update(mode);
    _trueRange = tr_rma.DATA[period];
    if (not _plusDM_rma.DATA:hasData(period)) or not tr_rma.DATA:hasData(period) or _trueRange == 0 then
        return;
    end
    diPlus[period] = 100 * _plusDM_rma.DATA[period] / _trueRange
    diMinus[period] = 100 * _minusDM_rma.DATA[period] / _trueRange

    sum = diPlus[period] + diMinus[period]
    f_sig_source[period] = math.abs(diPlus[period] - diMinus[period]) / (sum == 0 and 1 or sum);
    f_sig:update(mode);
    sig[period] = 100 * f_sig.DATA[period];

    range = sig[period] <= i_hlRange
    diUp = diPlus[period] >= diMinus[period]
    diUp_Cross = core.crossesOver(diPlus, diMinus, period);
    diUpStrong = diPlus[period] >= i_hlTrend
    diDn = diMinus[period] > diPlus[period]
    diDn_Cross = core.crossesUnder(diPlus, diMinus, period);
    diDnStrong = diMinus[period] > i_hlTrend

    crossDi = core.crosses(diPlus, diMinus, period);
    if (crossDi) then
        dot[period] = diPlus[period];
    end
    sigUp = sig[period] > sig[period - 1]
    sigDir = (sig[period] > sig[period - 1] and diUp and not range) and 1 or ((sig[period] > sig[period - 1] and diDn and not range) and -1 or 0)

    condition[period] = 0.0
    buyWeakCond = (not range and diUp_Cross and sigUp) or (not range and diUp and sigUp and sig[period] > i_hlRange and (sig[period - 1] <= i_hlRange)) 
    sellWeakCond = (not range and diDn_Cross and sigUp) or (not range and diDn and sigUp and sig[period] > i_hlRange and (sig[period - 1] <= i_hlRange))
    buyStrongCon = not range and diUp and sigUp and diUpStrong 
    sellStrongCo = not range and diDn and sigUp and diDnStrong
    closeCond = (crossDi and diPlus[period - 1] >= diMinus[period - 1]) or (crossDi and diPlus[period - 1] < diMinus[period - 1]) or (range and not (sig[period - 1] <= i_hlRange))

    if (condition[period - 1] ~= 1 and buyStrongCon) then
        condition[period] = 1;
    elseif (condition[period - 1] ~= -1 and sellStrongCo) then
        condition[period] = -1;
    elseif (condition[period - 1] ~= 0.5 and buyWeakCond) then
        condition[period] = 0.5;
    elseif (condition[period - 1] ~= -0.5 and sellWeakCond) then
        condition[period] = -0.5;
    elseif condition[period - 1] ~= 0 and closeCond then
        condition[period] = 0;
    else
        condition[period] = condition[period - 1]; 
    end

    c_orange = core.colors().Orange
    c_green = core.colors().Green
    c_greenLight = core.colors().Lime
    c_green100 = core.colors().Green
    c_red = core.colors().Red
    c_redLight = core.colors().Pink
    c_red100 = core.colors().Red
    c_white = core.colors().White
    c_gray = core.colors().Gray

    if (range) then
        sig:setColor(period, c_orange)
    elseif sigUp and diUp then
        sig:setColor(period, c_green100)
    elseif not sigUp and diUp then
        sig:setColor(period, c_greenLight)
    elseif sigUp and diDn then
        sig:setColor(period, c_red100)
    elseif not sigUp and diDn then
        sig:setColor(period, c_redLight)
    end
    if (range) then
        diPlus:setColor(period, c_orange)
    elseif not range and diUp and diUpStrong then
        diPlus:setColor(period, c_green)
    elseif not range and diUp and not diUpStrong then
        diPlus:setColor(period, c_greenLight)
    elseif not range and diDn and diDnStrong then
        diPlus:setColor(period, c_red)
    elseif not range and diDn and not diDnStrong then
        diPlus:setColor(period, c_redLight)
    end
