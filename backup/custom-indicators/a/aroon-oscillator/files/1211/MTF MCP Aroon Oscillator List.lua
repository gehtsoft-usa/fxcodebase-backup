...
function Init()
    indicator:name("MTF MCP Aroon Oscillator List");
    indicator:description("MTF MCP Aroon List");
	...
	indicator.parameters:addDouble("OB", "OB Level ", "OB Level", 50);
	indicator.parameters:addDouble("OS", "OS Level ", "OS Level", -50);
	...
	indicator.parameters:addColor("OBC", "OB Color", "OB Color", core.rgb(128, 128,128)); 
	indicator.parameters:addColor("OSC", "OS Color", "OS Color", core.rgb(128, 128,128)); 
...
function Prepare(nameOnly) 
	...
	assert(core.indicators:findIndicator("AROON OSCILLATOR") ~= nil, "Please, download and install AROON OSCILLATOR.LUA indicator");  
	 ...
	 for i = 1, Count, 1 do	
      ...
      AO[i]={};
		   ...
		   for j = 1, Num, 1 do	
			...
			Temp= core.indicators:create("AROON OSCILLATOR", source, Period  );
			first =  Temp.DATA:first();  
		   Source[i][j]= core.host:execute("getSyncHistory", Pair[i], TF[j], source:isBid(),math.min(first*2),20000 + ID , 10000 +ID);
		   loading [i][j]=true;
		   
		   AO [i][j]= core.indicators:create("AROON OSCILLATOR", Source[i][j], Period);
...
function Draw(stage, context)
	...
	for i = 1, Count, 1 do 
	 ...
	 if Loading then
        ...
        if not init then
           ...
           context:createSolidBrush(4,OBC); 
		   context:createSolidBrush(5,OSC); 
 ...
 function Calculate (context,i, j )
     ...
     if  not   AO[i][j].DATA:hasData(AO[i][j].DATA:size()-1)
     or  not   AO[i][j].DATA:hasData(AO[i][j].DATA:size()-2)  
	...
	--local Symbol="\243";
    local color=Neutral;	
	local Value=AO[i][j].DATA[AO[i][j].DATA:size()-1];
	local Symbol=string.format("%." .. 2 .. "f",Value);
	local Background=-1;   
		   if  AO[i][j].DATA[AO[i][j].DATA:size()-1] >  AO[i][j].DATA[AO[i][j].DATA:size()-2] then
		  -- Symbol="\200";
		   color=UpColor;
		   elseif  AO[i][j].DATA[AO[i][j].DATA:size()-1] <  AO[i][j].DATA[AO[i][j].DATA:size()-2] then
		   --Symbol="\241";
		   color=DownColor;
	        else
	     -- Symbol="\243";
	  ...
	  elseif Value < OS then
	   Background= 5;
		...
		context:drawRectangle( -1, 3 , x1, y1, x2, y2, transparency);	
		 ...
		 context:drawText (7,  Symbol, color, -1, x1 +(x2-x1)/2-width/2, y1, x1+(x2-x1)/2 +width/2, y2, 0 );	
