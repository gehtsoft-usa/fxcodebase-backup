...
function Update(period, mode)
	...
	or loading
	or not period == source:size()-1 
	then
    return;
    end	
	
	start_date,end_date	= core.getcandle(TF, core.now(), core.host:execute("getTradingDayOffset"), core.host:execute("getTradingWeekOffset"));
	  ...
	  local ADR=Calculate();
	   ...
	   local  MAX = source.open[period] +  ADR*Multiplier;
	   local  MIN = source.open[period] -  ADR*Multiplier; 
	
		if Label then 					 
			core.host:execute ("drawLabel", 1, end_date , MAX  , "ADR : ".. win32.formatNumber( ADR/source:pipSize(), false,2) );  
        end
		
		
		if Show then
		 core.host:execute ("drawLine", 2,  start_date, MAX, end_date, MAX, color, style, width, string.format("%." .. 2 .. "f", MAX));
		 core.host:execute ("drawLine", 3,  start_date, MIN, end_date, MIN,  color, style, width, string.format("%." .. 2 .. "f",MIN));
	    end
