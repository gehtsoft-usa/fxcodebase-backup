...
-- http://fxcodebase.com/code/viewtopic.php?f=17&t=63112&start=20
...
function CreateSessionParams(name, id, from, to, color)
    ...
    indicator.parameters:addBoolean(id .. "_show_average_range", "Show average range", "", false);
    indicator.parameters:addInteger(id .. "_average_range_period", "Average range period", "", 3);
end
...
function Init()
    ...
    indicator.parameters:addBoolean("show_session_name", "Show session name", "", true);
    indicator.parameters:addBoolean("show_data_in_toolip", "Show data in tooltip", "", false);
    indicator.parameters:addBoolean("show_session_high", "Show session high", "", true);
    indicator.parameters:addBoolean("show_session_low", "Show session low", "", true);
...
local pipsize;
local show_session_name, show_session_high, show_session_low, S_OD, S_OH, show_data_in_toolip;
local S_SE, S_TR;
local barsize;
...
function Prepare(nameOnly)
    ...
    show_session_name = instance.parameters.show_session_name;
    show_data_in_toolip = instance.parameters.show_data_in_toolip;
    show_session_high = instance.parameters.show_session_high;
    show_session_low = instance.parameters.show_session_low;
...
function Process(session, period)
    ...
    if date >= sfrom and date < sto_eod then
        ...
        if t >= 0 and t <= period and session.labels then
            ...
            local hl, ll;
            if show_session_high or S_OH or S_OD then
                hl = "";
                if show_session_high then
                    hl = hl .. hi;
                    namesuffix = namesuffix .. NEW_LINE;
                end
                if S_OD then
                    if hl ~= "" then
                        hl = hl .. NEW_LINE;
                ...
                if session.avgIndi ~= nil then
                    local index = core.findDate(session.avgIndi.DATA, source:date(period), false);
                    if index ~= -1 then
                        if hl ~= "" then
                            hl = hl .. NEW_LINE;
                        end
                        hl = hl .. "(AVG " .. math.floor(session.avgIndi.DATA[index] * 10 + 0.5) / 10 .. ")";
                        namesuffix = namesuffix .. NEW_LINE;
                    end
                end
                if not(show_data_in_toolip) then
                    session.LH:set(t, hi, hl);
                end
            end
            cc = 0;
            if show_session_low or S_OD then
                ll = "";
                if show_session_low then
                    ll = ll .. lo;
                    cc = cc + 1;
                end
                if S_OD then
            ...
            if show_session_name then
                if show_data_in_toolip then
                    session.LN:set(t, hi, session.name, hl .. NEW_LINE .. ll);
                else
...
function CreateSession(id, name, iname)
    ...
    local session = { };
    if instance.parameters:getBoolean(id .. "_show_average_range") then
        local profile = core.indicators:findIndicator("TRADE SESSION RANGE");
        assert(profile ~= nil, "Please, download and install " .. "TRADE SESSION RANGE" .. ".LUA indicator");
        local indicatorParams = profile:parameters();
        indicatorParams:setString("Start", from_str);
        indicatorParams:setString("End", to_str);
        if sources.items["D1"] == nil then
            sources:Request("D1", source, "D1");
        end
        session.avgSource = sources.items["D1"];
        session.rangeIndi = core.indicators:create("TRADE SESSION RANGE", session.avgSource, indicatorParams)
        session.avgIndi = core.indicators:create("MVA", session.rangeIndi.DATA, instance.parameters:getInteger(id .. "_average_range_period"))
    end
    function session:Update(period, mode)
        if self.rangeIndi ~= nil then
            self.rangeIndi:update(mode);
            self.avgIndi:update(mode);
        end
    end
    local n;
