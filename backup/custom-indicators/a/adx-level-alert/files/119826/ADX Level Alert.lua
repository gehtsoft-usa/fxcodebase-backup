...
-- http://fxcodebase.com/code/viewtopic.php?f=17&t=66245
...
function Init()
    indicator:name("ADX Level Alert");
   ...
   indicator:requiredSource(core.Bar);
	...
	indicator.parameters:addDouble("Level3", "3. Level","", 40); 
...
-- Routine
 function Prepare(nameOnly)   
    ...
    Indicator = core.indicators:create("ADX", source, Period1);	
	first=Indicator.DATA:first();

    ADX = instance:addStream("ADX", core.Line, name .. ".ADX", "ADX", instance.parameters.color1,  first);
	...
	if ON[1] then
	ADX:addLevel(Level[1], instance.parameters.level_overboughtsold_style, instance.parameters.level_overboughtsold_width, instance.parameters.level_overboughtsold_color);
	end
	if ON[2] then
	ADX:addLevel(Level[2],  instance.parameters.level_overboughtsold_style, instance.parameters.level_overboughtsold_width, instance.parameters.level_overboughtsold_color);   
	end
	if ON[3] then
	ADX:addLevel(Level[3],  instance.parameters.level_overboughtsold_style, instance.parameters.level_overboughtsold_width, instance.parameters.level_overboughtsold_color); 
    end
...
function Activate (id, period)
   ...
   Alert[id][period]=0;
	  ...
	  if  ON[id]  then
			...
			if  Indicator.DATA[period] > Level[id] 
			and   Indicator.DATA[period-1] <= Level[id] 
			then
			...
			elseif  Indicator.DATA[period] < Level[id] 
			and   Indicator.DATA[period-1] >= Level[id] 
            then			
