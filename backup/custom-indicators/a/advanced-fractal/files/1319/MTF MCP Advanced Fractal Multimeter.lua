...
function Init()
    indicator:name("MTF MCP Advanced Fractal Multimeter");
    ...
    indicator.parameters:addInteger("Frame", "Number of fractals)", "Number of fractals", 5, 3,99);
...
-- Routine
 function Prepare(nameOnly)   
   ...
   assert(core.indicators:findIndicator("AFBSR") ~= nil, "Please, download and install AFBSR.LUA indicator");	
    ...
    for i = 1, Number, 1 do	
		...
		Indicator[i] = core.indicators:create("AFBSR", SourceData[i], Frame  );
    ...
    for j = 1, Number, 1 do			
		...
		for i = 1, Number, 1 do 
			 ...
			 LabelText="";
				...
				if  Indicator[i].R:hasData(Indicator[i].R:size()-1) then		   
							if  SourceData[i].close[SourceData[i].close:size()-1]  > Indicator[i].R[Indicator[i].R:size()-1] then
							Color1=UP;
							elseif  SourceData[i].close[SourceData[i].close:size()-1]  <   Indicator[i].S[Indicator[i].S:size()-1] then
							Color1=DN;
							else
							Color1=NO;
							end	
							...
							LabelText= (SourceData[i].close[SourceData[i].close:size()-1]  - Indicator[i].S[Indicator[i].S:size()-1] )/((Indicator[i].R[Indicator[i].R:size()-1]-Indicator[i].S[Indicator[i].S:size()-1])/100)			
