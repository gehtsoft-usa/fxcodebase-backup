...
function Parameters (id , FRAME, flag )
    ...
    indicator.parameters:setFlag("TF"..id, core.FLAG_PERIODS);
     ...
     indicator.parameters:addInteger("SM1".. id, "Smoothing periods 1", "", 14, 1, 100);
    indicator.parameters:addInteger("SM2".. id, "Smoothing periods 2", "", 14, 1, 100);
	indicator.parameters:addDouble("Level".. id, "Level", "Level", 20, 1, 100);
end
...
function Prepare(nameOnly)
	...
	for i = 1 , 13 , 1 do   
       ...
       SM1[Num]=  instance.parameters:getInteger ("SM1"..i); 
	   SM2[Num]=  instance.parameters:getInteger ("SM2"..i); 
	   Level[Num]=  instance.parameters:getDouble ("Level"..i); 
	...
	for j = 1, Count, 1 do
		 ...
		 for i = 1, Num, 1 do	
		      ...
		      Test = core.indicators:create("VTADX", source   ,SM1[i], SM2[i]);   
	          ...
	          first= Test.DATA:first();
			   ...
			   Indicator[j][i] = core.indicators:create("VTADX", SourceData[j][i], SM1[i], SM2[i]  );
        ...
        context:createFont (8, "Arial",iwidth, iheight , context.UNDERLINE);
		        ...
		        for i = 1, Count, 1 do
						 for j = 1, Num, 1 do	
                            ...
                            if Indicator[i][j].DATA~= nil and   Indicator[i][j].DATA:hasData(Indicator[i][j].DATA:size()-1) then
                            y1=bottom -(i+1)*yGap;	
							x1=right -(j+1)*xGap;
							x2=right -(j )*xGap;
						 ...
						 if j== Num then 
							...
							context:drawText (7,  TF[j], Color, -1, x1+xGap  , y1-height+yGap ,x2+xGap ,  y1+yGap , context.CENTER   );	
								 ...
								 Text= win32.formatNumber(Indicator[i][j].DATA[Indicator[i][j].DATA:size()-1], false, 2);		
								 
								 
								 
								 
								 
								 if Indicator[i][j].DATA[Indicator[i][j].DATA:size()-1] > Indicator[i][j].DATA[Indicator[i][j].DATA:size()-2]
								 then
								 
									 if Indicator[i][j].DATA[Indicator[i][j].DATA:size()-1] < Level[j] then
									 ...
									 context:drawText (8, Text, Up, -1, x1+xGap  , y1-height+yGap*2 ,x2+xGap ,  y1+yGap*2 , context.CENTER   );	
									 end
									   ...
									   context:drawText (7, Text,Down, -1, x1+xGap  , y1-height+yGap*2 ,x2+xGap ,  y1+yGap*2 , context.CENTER   );	
									    ...
									    width, height = context:measureText (8, Text, 0);	
									   context:drawText (8, Text,Down, -1, x1+xGap  , y1-height+yGap*2 ,x2+xGap ,  y1+yGap*2 , context.CENTER   );		
									   end
								 end
