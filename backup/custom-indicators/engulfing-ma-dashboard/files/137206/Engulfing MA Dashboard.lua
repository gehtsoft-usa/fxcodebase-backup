...
--http://fxcodebase.com/code/viewtopic.php?f=17&t=70357
...
local indi_name = "Engulfing MA Dashboard";
...
function CreateIndicators(source)
    ...
    indicators[#indicators + 1] = CreateAverages(instance.parameters.period89, instance.parameters.method89, source);
    indicators[#indicators + 1] = CreateAverages(instance.parameters.period18, instance.parameters.method18, source);

    return indicators;
end

function CreateParameters()
    indicator.parameters:addInteger("period89", "Period 89", "", 89);
    AddAverages("method89", "Method 89", "MVA");
    indicator.parameters:addInteger("period18", "Period 18", "", 18);
    AddAverages("method18", "Method 18", "MVA");
end

function GetLastSignal(indi, source)
    if indi[1].DATA[NOW] > indi[2].DATA[NOW]
        and source.open[NOW] > indi[1].DATA[NOW]
        and source.close[NOW] < indi[2].DATA[NOW]
        and source.open[NOW] <= source.close[NOW - 1]
        and source.close[NOW] >= source.open[NOW - 1]
    then
        return 1, "B"
    end
    if indi[1].DATA[NOW] < indi[2].DATA[NOW]
