...
local indi_name = "Ranking Dashboard";
...
function CreateParameters()
    indicator.parameters:addString("indi", "Indicator", "", "RSI");
    indicator.parameters:setFlag("indi", core.FLAG_ONLYOSCILLATORS);
    indicator.parameters:addInteger("stream_index", "Stream Index", "", 1);
end

function GetSignal(indicators, source)
    local signal = {};
    signal.Label = source:instrument();
    signal.Value = indicators[1]:getStream(stream_index - 1)[NOW];
    signal.IsUp = true;
    signal.ValueLabel = win32.formatNumber(signal.Value, false, 1);
    signal.IsHistoricalUp = indicators[1]:getStream(stream_index - 1)[NOW] > indicators[1]:getStream(stream_index - 1)[NOW - 1];
    return signal;
end
...
function Prepare(nameOnly)
    ...
    if instance.parameters.all_instruments then
        ...
        local row = enum:next();
        while (row ~= nil) do
            PrepareInstrument(row.Instrument, core.colors().Green);
            row = enum:next();
        end
    else
        for i = 1, 20, 1 do
    ...
    if not init then
        ...
        for i, symbol in ipairs(items) do
            if instrument_bg_brushes[symbol.SymbolIndex] == nil then
                if color_brushes[symbol.BGColor] == nil then
                    color_brushes[symbol.BGColor] = LAST_PEN + i;
                    ...
                    instrument_bg_brushes[symbol.SymbolIndex] = color_brushes[symbol.BGColor];
                end
            end
...
function GetRank(tf)
    ...
    local message = "";
    if symbolsToDraw[1].LastRank ~= 1 then
        symbolsToDraw[1].LastRank = 1;
        message = message .. symbolsToDraw[1].Pair .. " Rank 1 - " .. tf;
    end
    if symbolsToDraw[2].LastRank ~= 2 then
        symbolsToDraw[2].LastRank = 2;
        if message ~= "" then
            message = message .. "\r\n";
        end
        message = message .. symbolsToDraw[2].Pair .. " Rank 2 - " .. tf;
    end
    if symbolsToDraw[3].LastRank ~= 3 then
        symbolsToDraw[3].LastRank = 3;
        if message ~= "" then
            message = message .. "\r\n";
        end
        message = message .. symbolsToDraw[3].Pair .. " Rank 3 - " .. tf;
    end
    return message
end
