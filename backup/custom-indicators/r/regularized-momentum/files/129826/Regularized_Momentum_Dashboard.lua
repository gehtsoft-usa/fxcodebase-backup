...
local indi_name = "Regularized Momentum";
local indi_id = "REGULARIZED MOMENTUM";
...
function CreateIndicators(source)
    ...
    p:setInteger("Length", instance.parameters.Length);
    p:setInteger("MinMaxPeriod", instance.parameters.MinMaxPeriod);
    p:setInteger("Multiplier", instance.parameters.Multiplier);
    p:setDouble("Lambda", instance.parameters.Lambda);
    p:setDouble("LevelUp", instance.parameters.LevelUp);
    p:setDouble("LevelDown", instance.parameters.LevelDown);
...
function GetLastSignal(indi, source)
    for i = 0, indi[1].Oscillator:size() - 3 do
        local up = indi[1].Oscillator[NOW - i] > indi[1].Oscillator[NOW - i - 1];
        local up2 = indi[1].Oscillator[NOW - i - 1] > indi[1].Oscillator[NOW - i - 2];
        if up ~= up2 then
            if up then
...
function Init()
    ...
    for i = 1, #timeframes_list do
        AddTimeFrame(i, timeframes_list[i], "display");
    end
...
function AddTimeFrame(id, FRAME, DEFAULT)
	...
	indicator.parameters:addString(paramId, FRAME, "", "disabled"); 
	end
