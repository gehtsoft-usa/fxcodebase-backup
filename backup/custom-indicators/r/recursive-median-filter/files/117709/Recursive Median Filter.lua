...
function Init()
    indicator:name("Recursive Median Filter");
    indicator:description("");
    indicator:requiredSource(core.Tick);
    indicator:type(core.Indicator);
	
	indicator.parameters:addGroup("Calculation"); 
    indicator.parameters:addInteger("Period1", "EMA Period", "", 14, 2, 2000);
	 indicator.parameters:addInteger("Period2", "Median Period", "", 5, 2, 2000);
...
function Update(period, mode)
    ...
    --Recursive Median (EMA of a 5 bar Median filter)	
	 local Median = mathex.median_s(source, period - Period2 + 1, period);
