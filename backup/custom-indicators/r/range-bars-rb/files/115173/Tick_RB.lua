...
function Init()
    indicator:name("Tick Range indicator");
    indicator:description("Tick Range indicator");
...
function Prepare(nameOnly) 
    ...
    InitCollection();
    local name = profile:id() .. "(" .. source:name() .. "," .. RangeSize .. " )";
    instance:name(name);
...
function GetPrice(index, date)
    ...
    if t.data == nil then
        ...
        t.data = host:execute("getHistory", index, t.instrument, "t1", from, to, source:isBid());
        t.rqfrom = from;
        t.rqto = to;
        t.loading = true;
        loading = true;
    ...
    return t.data[p], t.data[p], t.data[p], t.data[p];
end
...
function Update(period, mode)
    ...
    if absent==false then
     local t=data[1].data;
     Count_t=first;
     open_t[Count_t]=t[t:first()];
     high_t[Count_t]=t[t:first()];
     low_t[Count_t]=t[t:first()];
     close_t[Count_t]=t[t:first()];
     ...
     for i=t:first()+1,t:size()-1,1 do
       if high_t[Count_t]-low_t[Count_t]>=RangeSize*t:pipSize() then
        ...
        high_t[Count_t]=math.max(high_t[Count_t],t[i]);
        low_t[Count_t]=math.min(low_t[Count_t],t[i]);
        close_t[Count_t]=t[i];
       end
     end 
