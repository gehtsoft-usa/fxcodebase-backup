...
function Init()
    indicator:name("Averaged Relative Volume");
...
function Update(period, mode)
	    ...
	    if period < PeriodSize then
		return;
		end
		
		Average[period]= mathex.avg(source.volume, period-PeriodSize+1, period);
		
		local Sum=0;
		local Count=0;
		for i= period, PeriodSize, -PeriodSize  do
		...
		Volume[period]= (Average[period]-(Sum/Count));
