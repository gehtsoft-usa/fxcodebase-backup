...
-- http://fxcodebase.com/code/viewtopic.php?f=17&t=65777
...
function Init()
   ...
   indicator:requiredSource(core.Bar);
    ...
    indicator.parameters:addInteger("Period1", "1. RSI Period", "", 2, 2, 2000); 
	indicator.parameters:addInteger("Period2", "2. RSI Period", "", 15, 2, 2000); 
    indicator.parameters:addInteger("Period3", "3. RSI Period", "", 60, 2, 2000);  
	
	indicator.parameters:addDouble("OB", "OB Level", "", 70, 0, 100); 
    indicator.parameters:addDouble("OS", "OS Level", "", 30, 0, 100); 
...
local first;
local source = nil;
local RSI1, RSI2,RSI3, Period1, Period2, Period3,OB,OS;
...
-- Routine
 function Prepare(nameOnly)   
	...
	RSI3 = core.indicators:create("RSI", source.close, Period3);
	
	first=math.max(RSI1.DATA:first(),RSI2.DATA:first() ,RSI3.DATA:first() );
...
function Activate (id, period)
   ...
   Alert[id][period]=0;
	  ...
	  if id == 1  and ON[id]  then
			...
			if  RSI1.DATA[period] > OB
			and   RSI1.DATA[period-1] <= OB
			and    RSI2.DATA[period] < 50
			and    RSI3.DATA[period] < 50
			then
			...
			elseif  RSI1.DATA[period] < OS
			and   RSI1.DATA[period-1] >= OS
			and    RSI2.DATA[period] > 50
			and    RSI3.DATA[period] > 50
			then		
