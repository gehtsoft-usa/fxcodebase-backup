...
--+------------------------------------------------------------------+
 function Add(id, TF,Flag, Instrument )
	...
	indicator.parameters:addInteger("Steps"..id, "Steps", "Steps",10);
	
	indicator.parameters:addInteger("Period1"..id, "1. Smoothing Period", "Period", 30);
	indicator.parameters:addInteger("Period2"..id, "2. Smoothing Period", "Period", 30);
	indicator.parameters:addInteger("Period0"..id, "Base Period", "Period", 2);
...
function Init()
    indicator:name("MTF MCP Advanced RMO Heat Map");
...
function Prepare(nameOnly)
	...
	assert(core.indicators:findIndicator("ADVANCED RMO") ~= nil, "Please, download and install ADVANCED RMO.LUA indicator");    
	...
	s2, e2 = core.getcandle(iTF[i], 0, 0, 0);
	 ...
	 Label[Number]="";
                ...
                Label[Number]=Label[Number] .. " - " ..  TF[Number];				
			  	...
			  	Temp1= core.indicators:create("ADVANCED RMO", source.close ,  Steps[Number],  Period1[Number],  Period2[Number],  Period0[Number]);  				
				ifirst= Temp1.DATA:first()*2;
				 ...
				 loading[Number]  = true;  	 
				 Indicator[Number] = core.indicators:create("ADVANCED RMO", SourceData[Number].close ,    Steps[Number],  Period1[Number],  Period2[Number],  Period0[Number]);   
			   ...
			   x0, x1, x2 = context:positionOfBar (i);
			    ...
			    for j= 1, Number , 1 do
				  ...
				  if p~= false then
										...
										if Indicator[j].S1:hasData(p) and Indicator[j].S2:hasData(p-1) then 
										
												    if Indicator[j].S1[p] > Indicator[j].S2[p] then		 
