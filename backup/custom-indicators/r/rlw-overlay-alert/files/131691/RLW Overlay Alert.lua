...
{ 
    {
        ...
        UpCondition = function (period)
            return core.crossesOver(source.close, rlw, period);
        end,
        DownCondition = function (period)
            return core.crossesUnder(source.close, rlw, period);
        end,
...
function Init()
    indicator:name("RLW Overlay Alert");
    indicator:description("");
    indicator:requiredSource(core.Bar);
    indicator:type(core.Indicator);
    
    indicator.parameters:addInteger("rlw_period", "RLW Period", "", 14);
    indicator.parameters:addDouble("center_price", "Center Price", "", 0);
    indicator.parameters:setFlag("center_price", core.FLAG_PRICE);
    indicator.parameters:addDouble("scale_pips", "Scale, pips", "", 50);

    indicator.parameters:addColor("rlw_color", "RLW Color", "Color", core.colors().Red);
    indicator.parameters:addInteger("rlw_width", "RLW Width", "Width", 1, 1, 5);
    indicator.parameters:addInteger("rlw_style", "RLW Style", "Style", core.LINE_SOLID);
    indicator.parameters:setFlag("rlw_style", core.FLAG_LINE_STYLE);
...
local center_price, scale_pips, indi;
function Prepare(nameOnly)
    ...
    center_price = instance.parameters.center_price;
    scale_pips = instance.parameters.scale_pips;
    indi = core.indicators:create("RLW", source, instance.parameters.rlw_period);
    rlw = instance:addStream("RLW", core.Line, "RLW", "RLW", instance.parameters.rlw_color, 0, 0);
    rlw:setWidth(instance.parameters.rlw_width);
    rlw:setStyle(instance.parameters.rlw_style);
    indi_alerts:Prepare();
...
function Update(period, mode)
    ...
    rlw[period] = center_price + scale_pips * (indi.DATA[period] + 50) / 100 * source:pipSize();
