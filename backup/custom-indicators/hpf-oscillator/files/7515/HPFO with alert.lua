...
function Init()
    ...
    indicator.parameters:addBoolean("Live_param", "Calculate Last Candle", "", false)
    ...
    indi_alerts:AddAlert("Color change");
    indi_alerts:AddAlert("Zero cross");
end
...
function Prepare(nameOnly)
    ...
    Live = instance.parameters.Live_param
...
function Activate(alert, period, historical_period)
    ...
    elseif alert.id == 2 and period > 0 then
        if core.crossesOver(HPF, 0, period) then
            alert:UpAlert(source, period, alert.Label .. ". Bull pattern", source.high[period], historical_period);
        elseif core.crossesUnder(HPF, 9, period) then
            alert:DownAlert(source, period, alert.Label .. ". Bear pattern", source.low[period], historical_period);
        end
    end
