...
function Prepare(nameOnly)
	...
	core.host:execute("addCommand", 3, "Set 3. Zone Point", "")
	core.host:execute("addCommand", 4, "Set 4. Zone Point", "")
...
function Draw(stage, context)
	...
	local DL = db:get("DL", 0)
	local DD = db:get("DD", 0)
	...
	local p4 = core.findDate(source, DD, false)

	local x1, y1 = Add(context, AL, p1)
	local x2, y2 = Add(context, BL, p2)
	local x3, y3 = Add(context, CL, p3)
	local x4, y4 = Add(context, DL, p4)

	if x1 == nil or x2 == nil or x3 == nil or x4 == nil or AD == 0 or BD == 0 or CD == 0 or DD == 0 then
		return
	end
...
function AsyncOperationFinished(cookie, success, message)
	...
	elseif cookie == 4 then
		db:put("DL", tostring(level))
		db:put("DD", tostring(date))
