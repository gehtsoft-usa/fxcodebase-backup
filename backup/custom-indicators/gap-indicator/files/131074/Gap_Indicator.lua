...
--http://fxcodebase.com/code/viewtopic.php?f=17&t=69377
...
{ 
    {
        ...
        UpCondition = function (period)
            ...
            return (source.open[period] - source.close[period - 1]) / source:pipSize() >= min_gap;
        end,
        DownCondition = function (period)
            if period == 0 then
                return false;
            end
            return (source.close[period - 1] - source.open[period]) / source:pipSize() >= min_gap;
        end,
...
function Init()
    indicator:name("Gap Indicator");
    indicator:description("");
    indicator:requiredSource(core.Bar);
    indicator:type(core.Indicator);
    
    indicator.parameters:addDouble("min_gap", "Min. Gap", "", 1);
...
function Prepare(nameOnly)
    ...
    min_gap = instance.parameters.min_gap;
    indi_alerts:Prepare();
