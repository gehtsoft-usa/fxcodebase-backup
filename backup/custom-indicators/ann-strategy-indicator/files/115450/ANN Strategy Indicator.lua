...
-- http://fxcodebase.com/code/viewtopic.php?f=17&t=65182
...
function Init()
    indicator:name("ANN Strategy Indicator");
...
function Prepare(nameOnly)
	...
	Down = instance.parameters.Down;
	  ...
	  local name = profile:id() .. "(" .. source:name() .. ", " .. tostring(threshold).. ", " .. tostring(TF) .. ")";
    instance:name(name);
	...
	loading=true;
        ...
        Signal = instance:addStream("Signal", core.Bar, name, "Signal", Up, source:first());
...
function PineActivationFunctionLinear(v) 
return (math.exp(v) - math.exp(-v))/(math.exp(v) + math.exp(-v))
end

function PineActivationFunctionTanh(v)
return (math.exp(v) - math.exp(-v))/(math.exp(v) + math.exp(-v))
end

function getDiff(period)

local yesterday=(SourceData.open[period-1]+SourceData.close[period-1]+SourceData.high[period-1]+ SourceData.low[period-1])/4;
local today=(SourceData.open[period]+SourceData.close[period]+SourceData.high[period]+ SourceData.low[period])/4;
local delta=today-yesterday
local percentage=delta/yesterday
...
local X= PineActivationFunctionLinear(getDiff(p));	
