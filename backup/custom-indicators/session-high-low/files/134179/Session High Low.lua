...
--http://fxcodebase.com/code/viewtopic.php?f=17&t=69898
...
function Init()
    ...
    indicator.parameters:addDouble("level_1", "Level 1, %", "", 50);
    indicator.parameters:addDouble("level_2", "Level 2, %", "", 100);
    indicator.parameters:addDouble("level_3", "Level 3, %", "", 150);

    indicator.parameters:addColor("up1_color", "Up Level 1 Color", "Color", core.colors().Red);
    indicator.parameters:addInteger("up1_width", "Up Level 1 Width", "Width", 1, 1, 5);
    indicator.parameters:addInteger("up1_style", "Up Level 1 Style", "Style", core.LINE_SOLID);
    indicator.parameters:setFlag("up1_style", core.FLAG_LINE_STYLE);
    indicator.parameters:addColor("up2_color", "Up Level 2 Color", "Color", core.colors().Red);
    indicator.parameters:addInteger("up2_width", "Up Level 2 Width", "Width", 1, 1, 5);
    indicator.parameters:addInteger("up2_style", "Up Level 2 Style", "Style", core.LINE_SOLID);
    indicator.parameters:setFlag("up2_style", core.FLAG_LINE_STYLE);
    indicator.parameters:addColor("up3_color", "Up Level 3 Color", "Color", core.colors().Red);
    indicator.parameters:addInteger("up3_width", "Up Level 3 Width", "Width", 1, 1, 5);
    indicator.parameters:addInteger("up3_style", "Up Level 3 Style", "Style", core.LINE_SOLID);
    indicator.parameters:setFlag("up3_style", core.FLAG_LINE_STYLE);
    indicator.parameters:addColor("dn1_color", "Down Level 1 Color", "Color", core.colors().Red);
    indicator.parameters:addInteger("dn1_width", "Down Level 1 Width", "Width", 1, 1, 5);
    indicator.parameters:addInteger("dn1_style", "Down Level 1 Style", "Style", core.LINE_SOLID);
    indicator.parameters:setFlag("dn1_style", core.FLAG_LINE_STYLE);
    indicator.parameters:addColor("dn2_color", "Down Level 2 Color", "Color", core.colors().Red);
    indicator.parameters:addInteger("dn2_width", "Down Level 2 Width", "Width", 1, 1, 5);
    indicator.parameters:addInteger("dn2_style", "Down Level 2 Style", "Style", core.LINE_SOLID);
    indicator.parameters:setFlag("dn2_style", core.FLAG_LINE_STYLE);
    indicator.parameters:addColor("dn3_color", "Down Level 3 Color", "Color", core.colors().Red);
    indicator.parameters:addInteger("dn3_width", "Down Level 3 Width", "Width", 1, 1, 5);
    indicator.parameters:addInteger("dn3_style", "Down Level 3 Style", "Style", core.LINE_SOLID);
    indicator.parameters:setFlag("dn3_style", core.FLAG_LINE_STYLE);
...
function Prepare(nameOnly)
    ...
    barsize = (e - s) / 86400;
     ...
     if instance.parameters.NY_S then
        -- 8 am to 5 pm
        new_york = CreateSession(NY_Shift, 9, instance.parameters.NY_C, "NY", name, instance.parameters.NY_L
            , instance.parameters.NY_show_median_line, instance.parameters.NY_extend);
    end
    if instance.parameters.LO_S then
        -- 3 am to 12 pm
        london = CreateSession(LO_Shift, 9, instance.parameters.LO_C, "Lon", name, instance.parameters.LO_L
            , instance.parameters.LO_show_median_line, instance.parameters.LO_extend);
    end
    if instance.parameters.TO_S then
        -- 7 pm of yesterday to 4 am of today
        tokyo = CreateSession(TO_Shift, 9, instance.parameters.TO_C, "Tok", name, instance.parameters.TO_L
            , instance.parameters.TO_show_median_line, instance.parameters.TO_extend);
    end
    if instance.parameters.SY_S then
        -- 5 pm of yesterday to 2 am of today
        sydney = CreateSession(SY_Shift, 9, instance.parameters.SY_C, "Syd", name, instance.parameters.SY_L
            , instance.parameters.SY_show_median_line, instance.parameters.SY_extend);
    end
...
function SetHighLow(session, period, hi, lo, sfrom)
    ...
    session.up1[period] = session.high[period] + (session.high[period] - session.low[period]) * instance.parameters.level_1 / 100.0;
    session.up2[period] = session.high[period] + (session.high[period] - session.low[period]) * instance.parameters.level_2 / 100.0;
    session.up3[period] = session.high[period] + (session.high[period] - session.low[period]) * instance.parameters.level_3 / 100.0;
    session.dn1[period] = session.low[period] - (session.high[period] - session.low[period]) * instance.parameters.level_1 / 100.0;
    session.dn2[period] = session.low[period] - (session.high[period] - session.low[period]) * instance.parameters.level_2 / 100.0;
    session.dn3[period] = session.low[period] - (session.high[period] - session.low[period]) * instance.parameters.level_3 / 100.0;
    while period > 1 and session.begin[period] == session.begin[period - 1] do
        if session.high[period] ~= hi or session.low[period] ~= low then
...
function CreateSession(from, len, color, name, iname, labels, show_median_line, extend)
    ...
    session.up1 = instance:addStream(name .. "1u", core.Line, "Level 1", "Level 1", instance.parameters.up1_color, first, 0);
    session.up1:setWidth(instance.parameters.up1_width);
    session.up1:setStyle(instance.parameters.up1_style);
    session.up2 = instance:addStream(name .. "2u", core.Line, "Level 2", "Level 2", instance.parameters.up2_color, first, 0);
    session.up2:setWidth(instance.parameters.up2_width);
    session.up2:setStyle(instance.parameters.up2_style);
    session.up3 = instance:addStream(name .. "3u", core.Line, "Level 3", "Level 3", instance.parameters.up3_color, first, 0);
    session.up3:setWidth(instance.parameters.up3_width);
    session.up3:setStyle(instance.parameters.up3_style);
    session.dn1 = instance:addStream(name .. "1d", core.Line, "Level 1", "Level 1", instance.parameters.dn1_color, first, 0);
    session.dn1:setWidth(instance.parameters.dn1_width);
    session.dn1:setStyle(instance.parameters.dn1_style);
    session.dn2 = instance:addStream(name .. "2d", core.Line, "Level 2", "Level 2", instance.parameters.dn2_color, first, 0);
    session.dn2:setWidth(instance.parameters.dn2_width);
    session.dn2:setStyle(instance.parameters.dn2_style);
    session.dn3 = instance:addStream(name .. "3d", core.Line, "Level 3", "Level 3", instance.parameters.dn3_color, first, 0);
    session.dn3:setWidth(instance.parameters.dn3_width);
    session.dn3:setStyle(instance.parameters.dn3_style);
