...
function Init()
    indicator:name("MTF MCP Change Heat Map");
			   ...
			   x0, x1, x2 = context:positionOfBar (i);
			    ...
			    for j= 1, Number , 1 do
				  ...
				  if p~= false then
										...
										if SourceData[j].close:hasData(p) and SourceData[j].close:hasData(p-Period[j]) then 
										
												    if SourceData[j].close[p] > SourceData[j].close[p-Period[j]] then		 
															 
															C2=12;
															C1=11;
													elseif SourceData[j].close[p] < SourceData[j].close[p-Period[j]] then		
															C2=22;
															C1=21;
