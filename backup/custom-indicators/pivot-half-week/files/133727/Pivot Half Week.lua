...
function Init()
    indicator:name("Pivot Half Week");
...
function GetDates(period)
    local s, e = core.getcandle("W1", source:date(period), offset, weekoffset);
    local m = (s + e) / 2;
    if m < source:date(period) then
        local start_period = core.findDate(ref, s, false);
        local end_period = core.findDate(ref, m, false);
        return s, m, start_period, end_period;
    end
    local start_period = core.findDate(ref, m, false);
    local end_period = core.findDate(ref, e, false);
    return m, e, start_period, end_period;
end
...
function Update(period, mode)
    ...
    if start_date == nil or start_period < 0 or end_period < 0 then
        return;
    end
