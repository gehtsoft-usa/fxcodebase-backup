...
function Init()
    indicator:name("Market Range Overview");
...
-- indicator.parameters:addString("Method", "Method", "", "Percentage");
...
--    indicator.parameters:addStringAlternative("Method", "Pip", "", "Pip");
    ...
    indicator.parameters:addStringAlternative("Base", "All", "", "All");	
...
-- Routine
 function Prepare(nameOnly)   
	...
	if InstrumentType~=0 and InstrumentType~=1 then
	Major= false;  
	end
    ...
    local ID=0;
        ...
        for i = 1, InstrumentCount, 1 do
                ...
                ID = ID + 1;  
                Source[i] = core.host:execute("getHistory", HISTORY_LOADING_ID + ID, InstrumentList[i], "M1", 0, 0, true);					
                loading[i] = true; 
...
function getInstrumentList()
    ...
    while row ~= nil do
	  ...
	  if  row.InstrumentType == InstrumentType or InstrumentType== 0   then
		...
		if  (crncy1== Base or  crncy2== Base  or not Use_Base) 
...
function CalculatePercentageChange (i, id)
   ...
   local date=core.dateToTable (Source[i]:date(Source[i].close:size()-1));
 
  local S= core.datetime (date.year , 1, 1, 1, 1, 1)
  local E=Source[i]:date(Source[i].close:size()-1); 
  
  local s= core.findDate(Source[i], S, false);
  local e= core.findDate(Source[i], E, false); 
	
	 
      local   Min,Max=mathex.minmax(Source[i], s, e);	 
	  
   
	  Change[i]=  (Source[i].close[Source[i].close:size()-1]-Min)/  (Max-Min)*100;
