...
function Init()
    ...
    indicator.parameters:addInteger("K", "K Period","", 5, 1, 1000);
    indicator.parameters:addInteger("D", "D period","", 5, 1, 1000);
...
function Prepare(nameOnly) 
   ...
   OS=instance.parameters.oversold;
    ...
    sfk = core.indicators:create("SFK", source, k, d);
...
function Update(period, mode)
    ...
    if period >= kFirst then
        K_Line[period] = sfk.K[period];
    end
    if period >= dFirst then
        D_Line[period] = sfk.D[period];
    end
...
function Activate (id, period)
   ...
   Alert[id][period]=0;
	  ...
	  if id == 1  and ON[id]  then
			...
			if  K_Line[period] > D_Line[period] 
			and   K_Line[period-1] <= D_Line[period-1] 
			then
			           
						    
         
                Alert[id][period]= 1;	
 				AlertLevel[id][period]= D_Line[period] 
			...
			elseif  K_Line[period] < D_Line[period] 
			and   K_Line[period-1] >= D_Line[period-1] 
            then			
