...
function Init()
    indicator:name("MTF MCP GHLA Heat Map");
	...
	s2, e2 = core.getcandle(iTF[i], 0, 0, 0);
	 ...
	 Label[Number]="";
                ...
                Label[Number]=Label[Number] .. " - " ..  TF[Number];				
			  	...
			  	Temp1= core.indicators:create("GHLA", source ,  Period[Number]);  				
				ifirst= Temp1.DATA:first()*2;
				 ...
				 loading[Number]  = true;  	 
				 Indicator[Number] = core.indicators:create("GHLA", SourceData[Number] ,  Period[Number]);   
			    ...
			    for j= 1, Number , 1 do
				  ...
				  if p~= false then
										...
										if Indicator[j].DATA:hasData(p)   then 
												    ...
												    elseif SourceData[j].close[p] < Indicator[j].DATA[p] then		   
