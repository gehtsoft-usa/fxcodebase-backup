...
function Init()
    indicator:name("MTF MCP Wide Body Range list");
    indicator:description("MTF MCP Wide Body Range list");
	...
	if   (nameOnly) then
	 ...
	 for i = 1, Count, 1 do	
	  ...
	  loading[i] ={};  
		   ...
		   Source[i][j]= core.host:execute("getSyncHistory", Pair[i], TF[j], source:isBid(),math.min(300,Period*2),20000 + ID , 10000 +ID);
		   loading [i][j]=true;
    ...
    local SymbolColor=Neutral;	
	 ...
	 local period =Source[i][j].close:size()-1;
		...
		for k= 1 ,Period, 1   do
			...
			Max=math.max( Max,(Source[i][j].high[period-k]-Source[i][j].low[period-k]));	
			 	
        end		
		
	 
		
		if  (Source[i][j].high[period]-Source[i][j].low[period]) > Max then
		...
		if Symbol~= nil then
		 width, height = context:measureText (8, Symbol , context.CENTER  );  
		  context:drawText (8,  Symbol, SymbolColor, -1, x1+xGap  , y1+yGap, x2+xGap  , y1+yGap+height, context.CENTER   );	
		end
