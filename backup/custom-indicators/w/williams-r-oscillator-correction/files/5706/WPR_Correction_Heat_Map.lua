...
-- http://fxcodebase.com/code/viewtopic.php?f=17&t=2562
...
--+------------------------------------------------------------------+
 ...
 function Add(id, TF,Flag, Instrument )
    ...
    indicator.parameters:addInteger("WPR_Period" .. id, "WPR_Period " .. id .. " parameter", "", 14);
    indicator.parameters:addDouble("WPR_Level" .. id, "WPR_Level " .. id .. " parameter", "", -50);
    indicator.parameters:addString("MA_Method" .. id, "MA_Method" .. id, "", "MVA");
    indicator.parameters:addStringAlternative("MA_Method" .. id, "MVA", "", "MVA");
    indicator.parameters:addStringAlternative("MA_Method" .. id, "EMA", "", "EMA");
    indicator.parameters:addStringAlternative("MA_Method" .. id, "KAMA", "", "KAMA");
    indicator.parameters:addStringAlternative("MA_Method" .. id, "LWMA", "", "LWMA");
    indicator.parameters:addStringAlternative("MA_Method" .. id, "TMA", "", "TMA");
    indicator.parameters:addInteger("MA_Period" .. id, "MA_Period" .. id, "", 10);
...
function Init()
    indicator:name("MTF MCP Ehlers Heat Map");
...
function Prepare(nameOnly)
   ...
   Last=nil;
    ...
    assert(core.indicators:findIndicator("WILLIAMSPERCENTRANGE") ~= nil, "Please, download and install WILLIAMSPERCENTRANGE.LUA indicator");   
	...
	s2, e2 = core.getcandle(iTF[i], 0, 0, 0);
	 ...
	 Number=Number+1;
			...
			WPR_Period[Number]= instance.parameters:getInteger("WPR_Period" .. i);
			WPR_Level[Number]= instance.parameters:getDouble("WPR_Level" .. i);
			MA_Method[Number]= instance.parameters:getString("MA_Method" .. i);
			MA_Period[Number]= instance.parameters:getInteger("MA_Period" .. i);  
	 ...
	 Label[Number]="";
                ...
                Label[Number]=Label[Number] .. " - " ..  TF[Number];				
			  	...
			  	Temp1= core.indicators:create("WILLIAMSPERCENTRANGE", source ,  WPR_Period[Number]);  	
    assert(core.indicators:findIndicator(MA_Method[Number]) ~= nil, MA_Method[Number] .. " indicator must be installed");
                Temp2= core.indicators:create(MA_Method[Number],Temp1.DATA ,  MA_Period[Number]); 				
				ifirst= math.max(Temp1.DATA:first(), Temp2.DATA:first())*2;
				 ...
				 loading[Number]  = true;  	 
				 WPR[Number]= core.indicators:create("WILLIAMSPERCENTRANGE", SourceData[Number] ,  WPR_Period[Number]);  	
                 MA[Number]= core.indicators:create(MA_Method[Number],WPR[Number].DATA ,  MA_Period[Number]); 				
			   ...
			   x0, x1, x2 = context:positionOfBar (i);
			    ...
			    for j= 1, Number , 1 do
				  ...
				  if p~= false then
				                    ...
				                    if WPR[j].DATA[p]>= WPR_Level[j] then
										  if WPR[j].DATA[p]>MA[j].DATA[p] then
										  C2=12;
										  C1=11;
										  else
										  C2=22;
