...
--http://fxcodebase.com/code/viewtopic.php?f=17&t=70769
...
{ 
    {
        ...
        UpCondition = function (period)
            ...
            local period5, peak5, type5 = peaks:GetData();
            if not peaks:Next() or peak5 == nil then
                return false;
            end
            local period4, peak4, type4 = peaks:GetData();
            if not peaks:Next() or peak4 == nil then
                return false;
            end
            local period3, peak3, type3 = peaks:GetData();
            if not peaks:Next() or peak3 == nil then
                return false;
            end
            if not peaks:Next() then
                return false;
            end
            local period1, peak1, type1 = peaks:GetData();
            if peak4 < peak3 or peak1 == nil then
                return false;
            end
            local a, c = math2d.lineEquation(period1, peak1, period3, peak3);
            local projected5 = a * period5 + c;
            return projected5 > source.close[period]
        end,
    ...
    },
    {
        ...
        UpCondition = function (period)
            ...
            local period6, peak6, type6 = peaks:GetData();
            if not peaks:Next() or peak6 == nil then
                return false;
            end
            local period5, peak5, type5 = peaks:GetData();
            if not peaks:Next() or peak5 == nil then
            ...
            local a, c = math2d.lineEquation(period1, peak1, period4, peak4);
            local projected6 = a * period6 + c;
            return projected6 < source.close[period]
        end,
        DownCondition = function (period)
            local peaks = zz:EnumPeaks();
            if not peaks:Next() then
