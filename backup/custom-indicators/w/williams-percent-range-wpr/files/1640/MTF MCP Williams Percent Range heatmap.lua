...
function Init()
    indicator:name("MTF MCP Williams Percent Range heatmap");
	...
	s2, e2 = core.getcandle(iTF[i], 0, 0, 0);
	 ...
	 Label[Number]="";
                ...
                Label[Number]=Label[Number] .. " - " ..  TF[Number];				
			  	...
			  	Temp1= core.indicators:create("WILLIAMSPERCENTRANGE", source ,  Period[Number]);  				
				ifirst= Temp1.DATA:first()*2+1;
			
				
				   Id=Id+1;
				 SourceData[Number]  = core.host:execute("getSyncHistory",  Instrument[Number],  TF[Number], source:isBid(),ifirst, 2000 + Id , 1000 +Id);	 	 
				 loading[Number]  = true;  	 
				 Indicator[Number] = core.indicators:create("WILLIAMSPERCENTRANGE", SourceData[Number] ,  Period[Number]);   
...
function Draw (stage, context)
   ...
   context:setClipRectangle(context:left(), context:top(), context:right(), context:bottom());
        ...
        if not init then
			...
			context:createPen (51, context.SOLID, 2, OB)     
			context:createPen (61, context.SOLID, 2, OS)     
			    ...
			    for j= 1, Number , 1 do
				  ...
				  if p~= false then
										...
										if Indicator[j].DATA:hasData(p) and Indicator[j].DATA:hasData(p-1) then 
												   ...
												   if Indicator[j].DATA[p]	 >  -20 then												
												   C1=  51;
                                                   elseif Indicator[j].DATA[p]	< -80 then	
                                                   C1=  61;
                                                    end													  
