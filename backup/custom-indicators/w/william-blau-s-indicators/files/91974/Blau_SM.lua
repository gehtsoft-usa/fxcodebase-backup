...
--+------------------------------------------------------------------+
function Init()
    indicator:name("William Blau Stochastic Momentum");
    indicator:description("William Blau Stochastic Momentum");
...
function Prepare(nameOnly)  
    ...
    Blau_SM = instance:addStream("Blau_SM", core.Line, name .. ".Blau_SM", "Blau_SM", instance.parameters.clr, EMA3.DATA:first());
    Blau_SM:setWidth(instance.parameters.widthLinReg);
    Blau_SM:setStyle(instance.parameters.styleLinReg);
	
	Blau_SM:setPrecision(math.max(2, instance.source:getPrecision()));
end
...
function Update(period, mode)
   if period>first+Period then
    ...
    EMA2:update(mode);
    EMA3:update(mode);
    Blau_SM[period]=EMA3.DATA[period];
   end 
end
