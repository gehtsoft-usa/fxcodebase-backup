...
--+------------------------------------------------------------------+
function Init()
    indicator:name("William Blau Stochastic Momentum oscillator");
    indicator:description("William Blau Stochastic Momentum oscillator");
...
local Half_HH;
local HH;
local Half_HH_EMA1, Half_HH_EMA2, Half_HH_EMA3;
local HH_EMA1, HH_EMA2, HH_EMA3;
local TStoch_EMA;
local Blau_SM_Stoch=nil;
local Signal=nil;
...
function Prepare(nameOnly)  
    ...
    Half_HH_EMA1 = core.indicators:create("EMA", Half_HH, Smooth_Period1);
    Half_HH_EMA2 = core.indicators:create("EMA", Half_HH_EMA1.DATA, Smooth_Period2);
    Half_HH_EMA3 = core.indicators:create("EMA", Half_HH_EMA2.DATA, Smooth_Period3);
    ...
    Blau_SM_Stoch = instance:addStream("Blau_SM_Stoch", core.Bar, name .. ".Blau_SM_Stoch", "Blau_SM_Stoch", instance.parameters.clr, Half_HH_EMA3.DATA:first());
    TStoch_EMA = core.indicators:create("EMA", Blau_SM_Stoch, Signal_Period);
	...
	Blau_SM_Stoch:setPrecision(math.max(2, instance.source:getPrecision()));
...
function Update(period, mode)
   if period>first+Period then
    ...
    Half_HH_EMA1:update(mode);
    Half_HH_EMA2:update(mode);
    Half_HH_EMA3:update(mode);
    HH_EMA1:update(mode);
    HH_EMA2:update(mode);
    HH_EMA3:update(mode);
    if Half_HH_EMA3.DATA[period]>0 then
     Blau_SM_Stoch[period]=100*HH_EMA3.DATA[period]/Half_HH_EMA3.DATA[period];
    else
     Blau_SM_Stoch[period]=0;
    end
