...
function Init()
    ...
    local i
    local Level = {0, 23.6, 38.2, 50, 61.8, 76.4, 100, 11.8, 88.6}
    for i = 1, 9, 1 do
        if i > 7 then
            indicator.parameters:addGroup(i - 7 .. ". User defined Level")
            indicator.parameters:addBoolean("On" .. i, "Show ", "", false)
        else
    ...
    indicator.parameters:addDouble("amount", "Trade Amount", "", 1);
    indicator.parameters:addBoolean("allow_trade", "Allow strategy to trade", "", false);
    indicator.parameters:setFlag("allow_trade", core.FLAG_ALLOW_TRADE);
    indicator.parameters:addString("account", "Account to trade on", "", "");
    indicator.parameters:setFlag("account", core.FLAG_ACCOUNT);
    indicator.parameters:addString("allow_side", "Side", "", "B")
    indicator.parameters:addStringAlternative("allow_side", "Long/buy", "", "B")
    indicator.parameters:addStringAlternative("allow_side", "Short/sell", "", "S")
    ...
    indicator.parameters:addBoolean("Trend", "Show Trend Line ", "", true)
...
function Update(period) for _, module in pairs(Modules) do if module.ExtUpdate ~= nil then module:ExtUpdate(0, source, period); end end end
...
function Draw(stage, context)
    if stage == 0 then
        if not init then
            context:createPen(
                1,
                context:convertPenStyle(instance.parameters.LineStyle),
                instance.parameters.LineWidth,
                instance.parameters.LineClr
            )
            context:createPen(
                2,
                context:convertPenStyle(instance.parameters.FiboStyle),
                instance.parameters.FiboWidth,
                instance.parameters.UpFiboClr
            )
            context:createPen(
                3,
                context:convertPenStyle(instance.parameters.FiboStyle),
                instance.parameters.FiboWidth,
                instance.parameters.DnFiboClr
            )
        ...
        if firstBar < lastBar then
            ...
            for i = 1, 9, 1 do
                if instance.parameters:getBoolean("On" .. i) then
                    DrawLevel(
                        ...
                        Level0,
                        Level100,
                        instance.parameters:getDouble("Level" .. i),
                        P0,
                        P100,
                        X,
                        Pen,
            ...
            if not traded and instance.parameters.allow_trade then
                traded = true;
                local command = trading:MarketOrder(source:instrument());
                command:SetAccountID(instance.parameters.account)
                    :SetAmount(instance.parameters.amount);
                if MinPos < MaxPos then
                    command:SetStop(math.min(Min, Max)):SetLimit(math.max(Min, Max)):SetSide("B");
                else
                    command:SetStop(math.max(Min, Max)):SetLimit(math.min(Min, Max)):SetSide("S");
                end
