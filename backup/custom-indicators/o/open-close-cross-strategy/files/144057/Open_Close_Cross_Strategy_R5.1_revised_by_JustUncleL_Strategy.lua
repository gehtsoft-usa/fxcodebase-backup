...
--Open_Close_Cross_Strategy_R5.1_revised_by_JustUncleL
local STRATEGY_NAME = "Open Close Cross Strategy R5.1 revised by JustUncleL Strategy";
function CreateParameters() 
    strategy.parameters:addInteger("basis_period", "MA Period", "", 8);
    AddAverages("basis_method", "MA Method", "MVA");
    strategy.parameters:addInteger("delayOffset", "Delay Open/Close MA (Forces Non-Repainting)", "", 0);
end

local indi;
function CreateEntryIndicators(source)
    local profile = core.indicators:findIndicator("OPEN_CLOSE_CROSS_STRATEGY_R5.1_REVISED_BY_JUSTUNCLEL");
    assert(profile ~= nil, "Please, download and install " .. "OPEN_CLOSE_CROSS_STRATEGY_R5.1_REVISED_BY_JUSTUNCLEL" .. ".LUA indicator");
    indi = core.indicators:create("OPEN_CLOSE_CROSS_STRATEGY_R5.1_REVISED_BY_JUSTUNCLEL", source, 
        instance.parameters.basis_period, instance.parameters.basis_method, instance.parameters.delayOffset);
end
