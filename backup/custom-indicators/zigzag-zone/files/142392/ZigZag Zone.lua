...
-- https://fxcodebase.com/code/viewtopic.php?f=17&t=71253
...
function Draw(stage, context)
	...
	for i= 1, table.getn(UpDown)-2, 1  do
	
	    Color=-1;
	
	    if out[UpDown[i]]< out[UpDown[i+1]]
		and out[UpDown[i]]> out[UpDown[i+2]]
	    then
		Color=1;
		end
		if out[UpDown[i]]> out[UpDown[i+1]]
		...
		if Color~=-1 then
		    ...
		    price1=  out[UpDown[i]];
			price2=  out[UpDown[i+1]];
			visible, y1= context:pointOfPrice (math.max(price1, price2));
			visible, y2= context:pointOfPrice (math.min(price1, price2));
			x1, x, x = context:positionOfBar (UpDown[i+1]);
			x2, x, x = context:positionOfBar (UpDown[i]);
			context:drawRectangle (-1,Color, x1,y1, x2, y2, transparency);
