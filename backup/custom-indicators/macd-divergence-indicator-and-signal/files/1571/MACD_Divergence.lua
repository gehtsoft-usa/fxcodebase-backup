...
function Prepare(nameOnly)
    ...
    D = instance:addStream("MACD", core.Line, name .. ".MACD", "MACD", instance.parameters.D_color, first, -1);
...
function processBullish(period)
    if isTrough(period) then
        ...
        if prev ~= nil then
            if MACD.DATA[curr] > MACD.DATA[prev] and source.low[curr] < source.low[prev] then
                if I then
                    DN:set(curr, MACD.DATA[curr], "\225", "Classic bullish");
                    line_id = line_id + 1;
                    core.host:execute("drawLine", line_id, source:date(prev), MACD.DATA[prev], source:date(curr), MACD.DATA[curr], DN_color);
                else
                    DN[period] = curr - prev;
                end
            elseif MACD.DATA[curr] < MACD.DATA[prev] and source.low[curr] > source.low[prev] then
                if I then
                    DN:set(curr, MACD.DATA[curr], "\225", "Reversal bullish");
                    line_id = line_id + 1;
                    core.host:execute("drawLine", line_id, source:date(prev), MACD.DATA[prev], source:date(curr), MACD.DATA[curr], DN_color);
                else
                    DN[period] = -(curr - prev);
...
function isTrough(period)
    local i;
    if MACD.DATA[period] < 0 and MACD.DATA[period] < MACD.DATA[period - 1] and MACD.DATA[period] < MACD.DATA[period + 1] then
        for i = period - 1, first, -1 do
            if MACD.DATA[i] > 0 then
                return true;
            elseif MACD.DATA[period] > MACD.DATA[i] then
                return false;
            end
        end
    end
...
function prevTrough(period)
    ...
    for i = period - 5, first, -1 do
        if MACD.DATA[i] <= MACD.DATA[i - 1] and MACD.DATA[i] < MACD.DATA[i - 2] and
           MACD.DATA[i] <= MACD.DATA[i + 1] and MACD.DATA[i] < MACD.DATA[i + 2] then
           return i;
        end
    end
    return nil;
...
function processBearish(period)
    if isPeak(period) then
        ...
        if prev ~= nil then
            if MACD.DATA[curr] < MACD.DATA[prev] and source.high[curr] > source.high[prev] then
                if I then
                    UP:set(curr, MACD.DATA[curr], "\226", "Classic bearish");
                    line_id = line_id + 1;
                    core.host:execute("drawLine", line_id, source:date(prev), MACD.DATA[prev], source:date(curr), MACD.DATA[curr], UP_color);
                else
                    UP[period] = curr - prev;
                end
            elseif MACD.DATA[curr] > MACD.DATA[prev] and source.high[curr] < source.high[prev] then
                if I then
                    UP:set(curr, MACD.DATA[curr], "\226", "Reversal bearish");
                    line_id = line_id + 1;
                    core.host:execute("drawLine", line_id, source:date(prev), MACD.DATA[prev], source:date(curr), MACD.DATA[curr], UP_color);
                else
                    UP[period] = -(curr - prev);
