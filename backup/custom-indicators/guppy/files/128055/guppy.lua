...
-- http://fxcodebase.com/code/viewtopic.php?f=17&t=68817
...
function Init()
    indicator:name("Guppy");
    indicator:description("");
    indicator:requiredSource(core.Bar);
    indicator:type(core.Indicator);

    indi_alerts:AddParameters(indicator.parameters);
    indi_alerts:AddAlert("Buy/Sell");
    indi_alerts:AddAlert("Buy/Sell Break");

    indicator.parameters:addColor("fast_color", "Fast Color", "Fast Color", core.rgb(255, 0, 0));
    ...
    indicator.parameters:addColor("slow_color", "Slow Color", "Slow Color", core.rgb(0, 255, 0));
...
local emaF1, emaF2, emaF3, emaF4, emaF5, emaF6, emaF7, emaF8, emaF9, emaF10, emaF11, ema200;
local emaS1, emaS2, emaS3, emaS4, emaS5, emaS6, emaS7, emaS8, emaS9, emaS10, emaS11, emaS12, emaS13, emaS14, emaS15, emaS16;

function CreateStream(period, is_fast)
    local streamsData = {};
    local indi = core.indicators:create("EMA", source, period);
    streamsData.indi = indi;
    if is_fast then
        streamsData.stream = instance:addStream("data" .. period, core.Line, "EMA " .. period, "EMA " .. period, instance.parameters.fast_color, 0, 0);
        streamsData.stream:setWidth(instance.parameters.fast_width);
        streamsData.stream:setStyle(instance.parameters.fast_style);
    else
        streamsData.stream = instance:addStream("data" .. period, core.Line, "EMA " .. period, "EMA " .. period, instance.parameters.slow_color, 0, 0);
        streamsData.stream:setWidth(instance.parameters.slow_width);
        streamsData.stream:setStyle(instance.parameters.slow_style);
    end
    return streamsData;
end
...
function Prepare(nameOnly)
    ...
    fast[#fast + 1] = CreateStream(3, true);
    fast[#fast + 1] = CreateStream(5, true);
    fast[#fast + 1] = CreateStream(7, true);
    fast[#fast + 1] = CreateStream(9, true);
    fast[#fast + 1] = CreateStream(11, true);
    fast[#fast + 1] = CreateStream(13, true);
    fast[#fast + 1] = CreateStream(15, true);
    fast[#fast + 1] = CreateStream(17, true);
    fast[#fast + 1] = CreateStream(19, true);
    fast[#fast + 1] = CreateStream(21, true);
    fast[#fast + 1] = CreateStream(23, true);

    slow[#slow + 1] = CreateStream(25, false);
    slow[#slow + 1] = CreateStream(28, false);
    slow[#slow + 1] = CreateStream(31, false);
    slow[#slow + 1] = CreateStream(34, false);
    slow[#slow + 1] = CreateStream(37, false);
    slow[#slow + 1] = CreateStream(40, false);
    slow[#slow + 1] = CreateStream(43, false);
    slow[#slow + 1] = CreateStream(46, false);
    slow[#slow + 1] = CreateStream(49, false);
    slow[#slow + 1] = CreateStream(52, false);
    slow[#slow + 1] = CreateStream(55, false);
    slow[#slow + 1] = CreateStream(58, false);
    slow[#slow + 1] = CreateStream(61, false);
    slow[#slow + 1] = CreateStream(64, false);
    slow[#slow + 1] = CreateStream(67, false);
    slow[#slow + 1] = CreateStream(70, false);
    ema200 = core.indicators:create("EMA", source, 200);
...
function Update(period, mode)
    for _, ema in pairs(fast) do
        ema.indi:update(mode);
        if not ema.indi.DATA:hasData(period) then
            return;
        end
        ema.stream[period] = ema.indi.DATA[period];
    end
    for _, ema in pairs(slow) do
        ema.indi:update(mode);
        if not ema.indi.DATA:hasData(period) then
            return;
        end
...
function CalcFast(period)
    ...
    for _, ema in pairs(fast) do
        ...
        emafast = emafast + ema.indi.DATA[period];
        if last ~= nil and colfastL and last <= ema.indi.DATA[period] then
            colfastL = false;
        end
        if last ~= nil and colfastS and last >= ema.indi.DATA[period] then
            colfastS = false;
        end
        last = ema.indi.DATA[period];
    end
    return emafast / #fast, colfastL, colfastS;
end

function CalcSlow(period)
    local emaslow = 0;
    local colslowL = true;
    local colslowS = true;
    local last = nil;
    for _, ema in pairs(slow) do
        ...
        emaslow = emaslow + ema.indi.DATA[period];
        if last ~= nil and colslowL and last <= ema.indi.DATA[period] then
            colslowL = false;
        end
        if last ~= nil and colslowS and last >= ema.indi.DATA[period] then
            colslowS = false;
        end
        last = ema.indi.DATA[period];
    end
    return emaslow / #slow, colslowL, colslowS;
end

function Buy1(period)
    local emafast, colfastL, colfastS = CalcFast(period);
    if emafast == nil then
        return false;
    end
    local emaslow, colslowL, colslowS = CalcSlow(period);
    if emaslow == nil then
        return false;
    end
    return emafast > emaslow and not colslowS and colfastL and colslowL and emafast>ema200.DATA[period];
end

function Sell1(period)
    local emafast, colfastL, colfastS = CalcFast(period);
    if emafast == nil then
        return false;
    end
    ...
    return emafast < emaslow and not colslowL and colfastS and colslowS and emafast<ema200.DATA[period];
end

function Buy2(period)
    local emafast, colfastL, colfastS = CalcFast(period);
    if emafast == nil then
        return false;
    end
    ...
    return emafast > emaslow and not colslowS and emafast>ema200.DATA[period];
end

function Sell2(period)
    local emafast, colfastL, colfastS = CalcFast(period);
    if emafast == nil then
        return false;
    end
    ...
    return emafast < emaslow and not colslowL and emafast<ema200.DATA[period];
end
