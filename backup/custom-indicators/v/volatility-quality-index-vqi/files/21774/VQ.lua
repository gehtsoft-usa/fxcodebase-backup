...
function Prepare(nameOnly)
    ...
    local name = profile:id() .. "(" .. source:name() .. ", " .. tostring(Method)  .. ", " .. tostring(Length) .. ", " .. tostring(Smoothing).. ", " .. tostring(Filter).. ")";
    instance:name(name);

	 first = MH.DATA:first();
	
    if (not (nameOnly)) then
        SumVQ = instance:addStream("VQ", core.Line, name, "VQ", UP, first);
...
function Update(period, mode)
	...
	SumVQ[period] = SumVQ[period-1] + VQ[period];
	
	if (Filter > 0) then
		if (math.abs(SumVQ[period] - SumVQ[period-1]) < Filter * source:pipSize()) then
		SumVQ[period] = SumVQ[period-1];
		end
	end
