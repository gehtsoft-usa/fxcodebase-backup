...
local pipSize
local format
local zz, x_color, xy_color, y_color, x, y;
...
function Update(period, mode)
    ...
    local week_start, week_end = core.getcandle("W1", source:date(period), day_offset, week_offset);
    local last_week_start = week_start - (week_end - week_start);
    ...
    while peaks:Next() do
        ...
        lastDirection = direction;
        if source:date(lastPeriod) <= last_week_start then
            break;
        end
    end
