...
function Init()
    indicator:name("MTF MCP Market Facilitation Index List");
    indicator:description("MTF MCP Market Facilitation Index List");
    ...
    indicator.parameters:addColor("Color1", "MFI Up/Volume Up Color", "", colors.Lime);
    indicator.parameters:addColor("Color2", "MFI Down/Volume Down Color", "", colors.SaddleBrown);
    indicator.parameters:addColor("Color3", "MFI Up/Volume Down Color", "", colors.Blue);
    indicator.parameters:addColor("Color4", "MFI Down/Volume Up Color", "", colors.Pink);
...
local Select;
local SelectColor;
 local Range, Color1, Color2, Color3, Color4;
local Indicator={};
...
function Prepare(nameOnly)  
	...
	assert(core.indicators:findIndicator("BWMFI") ~= nil, "Please, download and install BWMFI.LUA indicator");    
	 ...
	 for i = 1, Count, 1 do	
      ...
      Indicator[i]={};
		   ...
		   for j = 1, Num, 1 do	
			...
			Temp= core.indicators:create("BWMFI", source  ,Range,Color1, Color2,Color3,Color4 );
		   ...
		   Indicator [i][j]= core.indicators:create("BWMFI", Source[i][j] ,  Range,Color1, Color2,Color3,Color4);
...
function Draw(stage, context)
 ...
 function Calculate (context,i, j )
	...
	local SymbolColor  = Indicator[i][j].DATA:colorI(Indicator[i][j].DATA:size()-1); 
	local Symbol=nil; 
		 
		  if Source[i][j].close [Source[i][j].close:size()-1] >  Source[i][j].open [Source[i][j].open:size()-1]  then
		  Symbol="\233";  
		  elseif Source[i][j].close [Source[i][j].close:size()-1] < Source[i][j].open [Source[i][j].open:size()-1]  then
		  Symbol="\234";  
		  else 
		  Symbol="\108"; 
		  end
