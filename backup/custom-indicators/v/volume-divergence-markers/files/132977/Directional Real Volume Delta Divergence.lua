...
{ 
    {
        ...
        UpCondition = function (period)
            ...
            return V[period] > 0 and ( source.close[period] - source.open[period]) < 0  and Signal[period]==1;
        end,
...
function Init()
    indicator:name("Directional Real Volume Delta");
    ...
    indicator.parameters:addInteger("Divergence_Threshold", "Divergence_Threshold", "", 0);
    indicator.parameters:addColor("clrUp", "Up Indicator Color", "", core.rgb(0, 255, 0));
    indicator.parameters:addColor("clrDown", "Down Indicator Color", "", core.rgb(255, 0, 0));
	  indicator.parameters:addColor("clrNeutral", "Neutral Indicator Color", "", core.rgb(0, 0, 255));
...
function Prepare(nameOnly)
	...
	Divergence_Threshold = instance.parameters.Divergence_Threshold;
    ...
    V = instance:addStream("Rate", core.Bar, name, "Rate", instance.parameters.clrUp, 0);
...
function Update(period, mode)
    ...
    if buy_stream:hasData(period) and sell_stream:hasData(period) then
        if (sell_stream:tick(period) < 0) then
			...
			if V[period] > Divergence_Threshold then
			    V:setColor(period, instance.parameters.clrUp);
				Signal[period]=1;
			elseif V[period] < - Divergence_Threshold then
			    V:setColor(period, instance.parameters.clrDown);
				Signal[period]=-1;
			else	
				 V:setColor(period, instance.parameters.clrNeutral);
				 Signal[period]=0;
			end
        end
    end
