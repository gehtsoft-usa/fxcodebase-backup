...
local vpc, signal, indi_source, UP_color, DN_color, hist
local UP, UPDOWN, DOWNUP, DOWN
...
{ 
    {
        ...
        UpCondition = function (period)
            if period < 0 or not vpc:hasData(period - 1) then
                return false;
            end
            return core.crossesOver(vpc, signal, period);
        end,
        DownCondition = function (period)
            if period < 0 or not vpc:hasData(period - 1) then
                return false;
            end
            return core.crossesUnder(vpc, signal, period);
        end,
...
function Prepare(nameOnly)
	...
	if signal_smoothing > 1 then
        ...
        hist = instance:addStream("hist", core.Bar, "HIST", "HIST", UP, first, 0)
	end
...
function Update(period, mode)
	...
	if signal_smoothing > 1 then
        ...
        hist[period] = vpc[period] - signal[period];
        if hist[period] > 0 then
            if hist[period] > hist[period - 1] then
                hist:setColor(period, UP);
            else
                hist:setColor(period, UPDOWN);
            end
        else
            if hist[period] < hist[period - 1] then
                hist:setColor(period, DOWN);
            else
                hist:setColor(period, DOWNUP);
            end
        end
	end
