...
function Init()
    indicator:name("emv");
    indicator:description("Arm's Ease of Movement Value with Real volume/Transactions");
...
local lowP = source.low[period-1];
  ...
  Main[period] = (((highCurr + lowCurr)/2) - ((highP + lowP)/2)) / ((Ind.DATA[period])/( highCurr - lowCurr)) ; 
