...
--http://fxcodebase.com/code/viewtopic.php?f=17&t=69389
...
{ 
    {
        ...
        UpCondition = function (period)
            ...
            return trend[period] == 1 and trend[period - 1] ~= 1;
        end,
        ...
        FormatMessage = function(source, period, level, label, isUp)
            return string.format(
                ...
                source:instrument(), 
                source:barSize(), 
                win32.formatNumber(source[NOW], false, source:getPrecision()), 
...
function Init()
    indicator:name("Volatility Adjusted MACD");
    indicator:description("");
    indicator:requiredSource(core.Tick);
    indicator:type(core.Oscillator);
    indicator.parameters:addInteger("FastEmaPeriod", "Fast EMA period", "", 12);
    indicator.parameters:addInteger("SlowEmaPeriod", "Slow EMA period", "", 26);
    indicator.parameters:addInteger("SignalEmaPeriod", "Signal EMA period", "", 9);
    indicator.parameters:addBoolean("AdjustSignal", "Adjust Signal?", "", true)
    indicator.parameters:addInteger("VolatilityPeriod", "Volatility period", "", 5);
    indicator.parameters:addInteger("VolatilitySmooth", "Volatility smooth", "", 10);

    indicator.parameters:addString("mode", "Alerts mode", "", "Sign");
    indicator.parameters:addStringAlternative("mode", "Sign", "", "Sign");
    indicator.parameters:addStringAlternative("mode", "Zero", "", "Zero");
    indicator.parameters:addStringAlternative("mode", "Slope", "", "Def");

    indicator.parameters:addColor("macd_color_down", "Down MACD Color", "Color", core.colors().Red);
    indicator.parameters:addColor("macd_color_up", "Up MACD Color", "Color", core.colors().Green);
    ...
    indicator.parameters:addColor("sign_color", "Sign Color", "Color", core.colors().Yellow);
    indicator.parameters:addInteger("sign_width", "Sign Width", "Width", 1, 1, 5);
    indicator.parameters:addInteger("sign_style", "Sign Style", "Style", core.LINE_SOLID);
    indicator.parameters:setFlag("sign_style", core.FLAG_LINE_STYLE);
...
local source, macd, sign, emaf, emas, sum, price, FastEmaPeriod, SlowEmaPeriod, SignalEmaPeriod, AdjustSignal, VolatilityPeriod, VolatilitySmooth, alert_mode;
local macd_color_down, macd_color_up;
function Prepare(nameOnly)
    ...
    alert_mode = instance.parameters.mode;
    macd_color_down = instance.parameters.macd_color_down;
    macd_color_up = instance.parameters.macd_color_up;
    VolatilitySmooth = instance.parameters.VolatilitySmooth;
    VolatilityPeriod = instance.parameters.VolatilityPeriod;
    AdjustSignal = instance.parameters.AdjustSignal;
    SignalEmaPeriod = instance.parameters.SignalEmaPeriod;
    SlowEmaPeriod = instance.parameters.SlowEmaPeriod;
    FastEmaPeriod = instance.parameters.FastEmaPeriod;
    macd = instance:addStream("MACD", core.Line, "MACD", "MACD", instance.parameters.macd_color_up, 0);
    macd:setWidth(instance.parameters.macd_width);
    macd:setStyle(instance.parameters.macd_style);

    sign = instance:addStream("Sign", core.Line, "Sign", "Sign", instance.parameters.sign_color, 0);
    sign:setWidth(instance.parameters.sign_width);
    sign:setStyle(instance.parameters.sign_style);

    emaf = instance:addInternalStream(0, 0);
    emas = instance:addInternalStream(0, 0);
    sum = instance:addInternalStream(0, 0);
    price = instance:addInternalStream(0, 0);
    trend = instance:addInternalStream(0, 0);

    mva = core.indicators:create("MVA", sum, VolatilitySmooth);
    indi_alerts:Prepare();
...
function Update(period, mode)
    ...
    price[period] = source[period] - source[period - 1];
    if period < source:first() + VolatilityPeriod + 1 then
        return;
    end
    sum[period] = mathex.sum(price, period - VolatilityPeriod, period);
    mva:update(mode);
    if not mva.DATA:hasData(period) then
        return;
    end
    
    local avg = mva.DATA[period];

    local fastPer = FastEmaPeriod;
    local slowPer = SlowEmaPeriod;
    local signPer = SignalEmaPeriod;
    if avg ~= 0 and sum[period] ~= 0 then
        fastPer = math.max(FastEmaPeriod / (sum[period] / avg), FastEmaPeriod / 2.0);
        slowPer = math.max(SlowEmaPeriod / (sum[period] / avg), FastEmaPeriod / 2.0);
        if AdjustSignal then
            signPer = math.max(SignalEmaPeriod / (sum[period] / avg), SignalEmaPeriod / 2.0);
        end
    end
    local alphaf = 2.0 / (1.0 + fastPer);
    local alphas = 2.0 / (1.0 + slowPer);
    local alphar = 2.0 / (1.0 + signPer);
    if emaf:hasData(period - 1) then
        emaf[period] = emaf[period - 1] + alphaf * (source[period] - emaf[period - 1]);
    else
        emaf[period] = alphaf * source[period];
    end
    if emas:hasData(period - 1) then
        emas[period] = emas[period - 1] + alphas * (source[period] - emas[period - 1]);
    else
        emas[period] = alphas * source[period];
    end
    macd[period] = emaf[period] - emas[period];
    if sign:hasData(period - 1) then
        sign[period] = sign[period - 1] + alphar * (macd[period] - sign[period - 1]);
    else
        sign[period] = alphar * macd[period];
    end
    if alert_mode == "Sign" then
        if macd[period] > sign[period] then
            trend[period] = 1;
            macd:setColor(period, macd_color_up);
        else
            trend[period] = -1;
            macd:setColor(period, macd_color_down);
        end
    elseif alert_mode == "Zero" then
        ...
        if macd:hasData(period - 1) and macd[period] > macd[period - 1] then
            trend[period] = 1;
            macd:setColor(period, macd_color_up);
        else
            trend[period] = -1;
...
function Activate(alert, period, historical_period)
    ...
    if alert.UpCondition(period) and (not alert.OnChange or not alert.UpCondition(period - 1)) then
        alert:UpAlert(source, period, source[period], historical_period);
    elseif alert.DownCondition(period) and (not alert.OnChange or not alert.DownCondition(period - 1)) then
        alert:DownAlert(source, period, source[period], historical_period);
    end
