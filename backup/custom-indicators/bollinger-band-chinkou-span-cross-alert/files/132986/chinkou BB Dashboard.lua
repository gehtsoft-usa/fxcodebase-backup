...
function GetLastSignal(indi, source, source2)
    ...
    if index >= 0 then
        if core.crossesOver(indi[1].CS, indi[2].AL, source:size() - 1 - instance.parameters.KijunSenPeriod, index) then
            return 1, "B";
        end
        if core.crossesUnder(indi[1].CS, indi[2].AL, source:size() - 1 - instance.parameters.KijunSenPeriod, index) then
            return -1, "S";
        end
    end
    return 0, "-";
