...
local indi_name = "Chikou BB Dashboard";
local indi_version = "1";

function CreateIndicators(source, source2)
    local indicators = {};
    indicators[#indicators + 1] = core.indicators:create("ICH", source, instance.parameters.TenkanSenPeriod, instance.parameters.KijunSenPeriod, instance.parameters.SenkouSpanPeriod);
    indicators[#indicators + 1] = core.indicators:create("BB", source2, instance.parameters.bb_period, instance.parameters.bb_deviations);
    return indicators;
end

function CreateParameters()
    indicator.parameters:addString("ich_tf", "Ichimoku Timeframe", "", "m15");
    indicator.parameters:setFlag("ich_tf", core.FLAG_BARPERIODS);
    indicator.parameters:addString("bb_tf", "BB Timeframe", "", "H1");
    indicator.parameters:setFlag("bb_tf", core.FLAG_BARPERIODS);
...
function GetLastSignal(indi, source, source2)
    if (source:size() < instance.parameters.KijunSenPeriod + 1) then
        return 0, "-";
    end
    local index = core.findDate(source2, source:date(NOW - instance.parameters.KijunSenPeriod), false);
    if index >= 0 then
        if core.crossesOver(indi[1].CS, indi[2].TL, source:size() - 1 - instance.parameters.KijunSenPeriod, index) then
            return 1, "B";
        end
        if core.crossesUnder(indi[1].CS, indi[2].BL, source:size() - 1 - instance.parameters.KijunSenPeriod, index) then
            return -1, "S";
        end
    end
    return 0, "-";
...
function PrepareInstrument(instrument)
    ...
    symbol.TF = instance.parameters.ich_tf;
    symbol.TF2 = instance.parameters.bb_tf;
    symbol.LoadingId = last_id + 1;
    symbol.LoadedId = last_id + 2;
    symbol.LoadingId2 = last_id + 3;
    symbol.LoadedId2 = last_id + 4;
    symbol.Loading = true;
    symbol.Loading2 = true;
    ...
    function symbol:DoLoad()
        ...
        self.Source2 = core.host:execute("getSyncHistory", self.Pair, self.TF2, instance.source:isBid(), 300, self.LoadedId2, self.LoadingId2);
        self.Indicators = CreateIndicators(self.Source, self.Source2);
    end
...
function UpdateData()
    for _, symbol in ipairs(items) do
        if symbol.Indicators ~= nil then
            ...
            local signal, label = GetLastSignal(symbol.Indicators, symbol.Source, symbol.Source2);
            symbol.Signal = signal;
            symbol.Text = label;
            if signal ~= 0 and symbol.last_alert ~= symbol.Source:date(NOW) then
                Signal(symbol.Source:instrument() .. "(" .. symbol.Source:barSize() .. "): " .. symbol.Text);
                symbol.last_alert = symbol.Source:date(NOW);
            end
		else
            symbol.Text = "...";
        end
    ...
    if cookie == TIMER_ID then
        ...
        for _, symbol in ipairs(items) do
            ...
            elseif cookie == symbol.LoadedId then
                ...
                return;
            elseif cookie == symbol.LoadingId2 then
                symbol.Loading2 = true;
                return;
            elseif cookie == symbol.LoadedId2 then
                symbol.Loading2 = false;
                return;
            end
        end
    end
