...
function Init()
    indicator:name("VAMA Moving Average Convergence/Divergence with Real volume/Transactions");
...
function Update(period, mode)
    ...
    PriceTimesVolume[period]=source[Price][period] *Ind.DATA[period];
    
	local EMAS=mathex.sum(PriceTimesVolume, period -SN+1, period)/mathex.sum(Ind.DATA, period -SN+1, period);
	local EMAL=mathex.sum(PriceTimesVolume, period -LN+1, period)/mathex.sum(Ind.DATA, period -LN+1, period);
	...
	SignalTimesVolume[period]=	MACD[period] *Ind.DATA[period];

	local MVAI=mathex.sum(SignalTimesVolume, period -IN+1, period)/mathex.sum(Ind.DATA, period -IN+1, period);		
