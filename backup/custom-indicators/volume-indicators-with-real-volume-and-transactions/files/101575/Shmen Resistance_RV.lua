...
function Init()
    indicator:name("Shmen Resistance with Real volume/Transactions");
    indicator:description("Shmen Resistance with Real volume/Transactions");
	...
	indicator.parameters:addBoolean("RPath", "RPath", "", true);
    indicator.parameters:addDouble("MinPips", "MinPips", "", 5);	
    indicator.parameters:addDouble("MinVol", "MinVol", "", 5);	
...
function Prepare(nameOnly)
	...
	MinPips = instance.parameters.MinPips;
	MinVol = instance.parameters.MinVol;
	RPath = instance.parameters.RPath;
...
function Update(period, mode)
	...
	if Method == "High/Low" then
	 Dif=(source.high[period]-source.low[period])/source:pipSize();
	elseif Method == "Open/Close" then
	Dif=math.abs(source.close[period]-source.open[period])/source:pipSize();
	end
	 
      if (Dif < MinPips) then Dif = MinPips; end
	  
      local Volume= Ind.DATA[period];
	  
      if (Volume < MinVol) then Volume = MinVol; end
      if (RPath) then Ratio[period] = Dif/Volume; else Ratio[period] = Volume/Dif; end
