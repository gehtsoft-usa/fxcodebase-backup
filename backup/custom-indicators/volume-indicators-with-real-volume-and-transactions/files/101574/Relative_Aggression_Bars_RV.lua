...
function Init()
    indicator:name("Relative Aggression Bars with Real volume/Transactions");
...
function VOLUME (period, mode)
   ...
   local max= MAX(period, Ind.DATA);	 	 
	  
	  local value = math.abs(Ind.DATA[period] - Ind.DATA[period-1]);
...
function AVOLUME (period, mode)
   ...
   local max= mathex.max(Ind.DATA, period-Period+1, period);	 	 
	  
	  local value = Ind.DATA[period];
	  Draw(value ,max,  period);
