...
function Update(period, mode)
   if period>first then
    ...
    Cost[period]=Ind.DATA[period]*(2*source.close[period]+source.high[period]+source.low[period])/4;
    if period>first+math.max(Average, Periods) then
     local SumCost=mathex.sum(Cost, core.rangeTo(period, Average));
     local SumVolume=mathex.sum(Ind.DATA, core.rangeTo(period, Average));
     if SumVolume~=0 then
