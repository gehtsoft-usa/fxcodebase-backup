...
function Init()
    indicator:name("Wyckoff PV Histogram with Real volume/Transactions");
    indicator:description("Wyckoff PV Histogram with Real volume/Transactions");
...
function Update(period, mode)
	...
	if  (source.high[period]-source.low[period]) >= (source.high[period-1]-source.low[period]-1)  and Ind.DATA[period] >  Ind.DATA[period-1] then
	HVWS= true;
	else
	HVWS= false;
	end
