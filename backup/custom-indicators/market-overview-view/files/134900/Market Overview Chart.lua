...
function getInstrumentList()
    ...
    while row ~= nil do
		...
		second[count]=crncy2;
			...
			inverse[count]=0; -- Do NOT invert the value	
			...
			elseif crncy2== Base then
				...
				if Use_Base then
				inverse[count]=1;-- Invert the value	
				else
				inverse[count]=0;	
				end			
...
function CalculatePercentageChange (i, id)
	...
	local Value; 
	  ...
	  Value=  (Source[i].close[Source[i].close:size()-1]-Source[i].open[Source[i].open:size()-1])/ (Source[i].open[Source[i].open:size()-1]/100);
	  else
	  Value=  (Source[i].close[Source[i].close:size()-1]-Source[i].open[Source[i].open:size()-1])/  Point[i];
	  end
					...
					for i= 1, InstrumentCount,1 do 	
					   ...
					   for i= 1, InstrumentCount,1 do 	
									...
									x2=x1+x_cell- (x_cell/100)*Spacing;
									 ...
									 width, height = context:measureText (1, "XXX", context.CENTER  ); 
									 height1= height
									...
									--ChangeKey[i] will go from 1... InstrumentCount
									--i will go from 1... InstrumentCount		
                                   -- ChangeKey[i] will be sorted									
