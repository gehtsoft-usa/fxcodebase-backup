...
function Activate (id, period)
   ...
   Alert[id][period]=0;
	  ...
	  if id == 1  and ON[id]  then
			...
			if  source.close[period] > NR_Super_Trend[period] 
			and   source.close[period-1] <= NR_Super_Trend[period-1] 
			then
			           
						    
         
                Alert[id][period]= 1;	
 				AlertLevel[id][period]= NR_Super_Trend[period] 
			...
			elseif  source.close[period] < NR_Super_Trend[period] 
			and   source.close[period-1] >= NR_Super_Trend[period-1] 
            then			
