...
-- http://fxcodebase.com/code/viewtopic.php?f=17&t=8084
-- Id: 4970
...
function Add(id, TF, Flag, Instrument)
	...
	indicator.parameters:addInteger("ATRP" .. id, "ATR Period ", "", 12)
	indicator.parameters:addInteger("MVAP" .. id, "MVA Period ", "", 14)
end

function Init()
	indicator:name("New Trend Heat Map")
...
function Prepare(nameOnly)
	...
	for i = 1, 13, 1 do
		...
		if instance.parameters:getBoolean("On" .. i) and (e1 - s1) <= (e2 - s2) then
			...
			ATRP[Number] = instance.parameters:getInteger("ATRP" .. i)
			MVAP[Number] = instance.parameters:getInteger("MVAP" .. i)
			...
			Temp1 = core.indicators:create("NEW TREND", source, ATRP[Number], MVAP[Number], true)
			...
			loading[Number] = true
			Indicator[Number] = core.indicators:create("NEW TREND", SourceData[Number], ATRP[Number], MVAP[Number], true)
		end
	end
