-- Available @ https://fxcodebase.com/code/viewtopic.php?f=17&t=74335
...
{ 
    {
        ...
        UpCondition = function (period)
            return h:hasData(period);
        end,
        DownCondition = function (period)
            return l:hasData(period);
        end,
...
function Init()
    indicator:name("Neutrino Trader");
...
function IsNewLow(period, ll)
    last_low = l:getBookmark(1);
    if last_low == -1 then
        return true;
    end
    if ll == source.low[last_low] then
        return false;
    end
    if source.low[period] - ll > threshold * source:pipSize() then
        return false;
    end
    return true;
end

function IsNewHigh(period, hh)
    last_high = h:getBookmark(1);
    if last_high == -1 then
        return true;
    end
    if hh == source.high[last_high] then
        return false;
    end
    if hh - source.high[period] > threshold * source:pipSize() then
        return false;
    end
    return true;
end
...
function Update(period, mode)
    ...
    local ll = mathex.min(source.low, period - length, period);
    if IsNewLow(period, ll) then
        l:setBookmark(1, period);
        l[period] = ll;
        for i = period - 1, period - lookback - 1, -1 do
            if l:hasData(i) and l[i] > ll then
                l:setNoData(i);
                indi_alerts.Alerts[1].Alert:setNoData(i);
                indi_alerts.Alerts[1]:Clear(i);
            end
        end
    end

    local hh = mathex.max(source.high, period - length, period);
    if IsNewHigh(period, hh) then
        h:setBookmark(1, period);
        h[period] = hh;
        for i = period - 1, period - lookback - 1, -1 do
            if h:hasData(i) and h[i] < hh then
                h:setNoData(i);
                indi_alerts.Alerts[1].Alert:setNoData(i);
                indi_alerts.Alerts[1]:Clear(i);
            end
...
function indi_alerts:Prepare()
    ...
    for i = 1, 100 do 
        ...
        function alert:Clear(period)
            ...
            while (bookmark ~= -1 and bookmark > period) do
                i = i + 1;
                bookmark = self.Alert:getBookmark(i);
            end
