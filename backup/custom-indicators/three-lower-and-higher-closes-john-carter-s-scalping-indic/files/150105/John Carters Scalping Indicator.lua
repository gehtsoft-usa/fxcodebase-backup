-- Available @ https://fxcodebase.com/code/viewtopic.php?f=17&t=73515
...
function Init()
    indicator:name("John Carter’s Scalping Indicator");
...
function Update(period, mode)
    ...
    if Close[period] < Close[period-1] and Close[period-1] < Close[period-2] and (Close[period] < Low[period-2] and Close[period-1] <Low[period-2]) then
	BearCondition = true;
	else
	BearCondition = false;
    end	
    ...
    if Close[period] > Close[period-1] and Close[period-1] > Close[period-2] and (Close[period] > High[period-2] and Close[period-1] > High[period-2]) then	  
	BullCondition = true;
	else
	BullCondition = false;
    end	
