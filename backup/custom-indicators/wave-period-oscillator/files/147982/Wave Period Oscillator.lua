...
-- https://fxcodebase.com/code/viewtopic.php?f=17&t=72858
...
function Init()
    indicator:name("Wave Period Oscillator");
	...
	indicator.parameters:addDouble("Level4", "4. Level","", 0);	
...
-- Routine
 function Prepare(nameOnly)   
	...
	Ti = instance:addInternalStream(0, 0);
	Indicator= core.indicators:create(Method, Ti, Period); 
	...
	Line:addLevel(instance.parameters.Level4, instance.parameters.level_overboughtsold_style, instance.parameters.level_overboughtsold_width, instance.parameters.level_overboughtsold_color);
...
function Update(period, mode)
	...
	local sinwt= source.close[period] / source.high[period]
	local sinsq= sinwt * sinwt
	local cossq= 1 - sinsq	
	local coswt= math.sqrt(cossq)
	local Angle=  math.atan(sinwt / coswt)
	local Rad= 3.14 * Angle / 180
	local Tt= 6.28 / Rad
