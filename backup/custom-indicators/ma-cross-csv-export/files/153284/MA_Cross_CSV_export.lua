-- Available @ https://fxcodebase.com/code/viewtopic.php?f=17&t=74337
...
{ 
    {
        ...
        UpCondition = function (period)
            if period < 1 or not ma1.DATA:hasData(period - 1) or not ma2.DATA:hasData(period - 1) then
                return false;
            end
...
function Init()
    indicator:name("MA Cross CSV export");
    ...
    indicator.parameters:addFile("export_file", "Path to export file", "", core.app_path() .. "\\1.csv");
end
...
function Export(period)
    f = io.open(export_file, "a")
    core.host:trace(tostring(f));
	if f ~= nil then
        local date = source:date(period);
        local dateTable = core.dateToTable(date);
        f:write(string.format("%02i/%02i/%04i;%02i:%02i:%02i;\n", dateTable.month, dateTable.day, dateTable.year,
