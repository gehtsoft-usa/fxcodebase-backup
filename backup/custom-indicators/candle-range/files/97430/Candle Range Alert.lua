...
function Init()
    ...
    indicator.parameters:addColor("UpC", "Up Trend Color","Up Trend Color", core.COLOR_UPCANDLE);
	indicator.parameters:addColor("DownC", "Down Trend Color","Down Trend Color", core.COLOR_DOWNCANDLE);
	...
	indicator.parameters:addBoolean("Ignore", "Ignore smaller range alert" , "Ignore", true);	
    ...
    indicator.parameters:setFlag("Email", core.FLAG_EMAIL);
	 ...
	 indicator.parameters:addString("ON" , "Alert Selector",  "Alert" , "Bar Range");
	 indicator.parameters:addStringAlternative("ON", "Bar Range", "", "Bar Range");
	indicator.parameters:addStringAlternative("ON", "Bar Range/ Signal Line", "", "Bar Range/ Signal Line");   
	
	Parameters (1, "Bar Range");	
	Parameters (2, "Bar Range/ Signal Line");
...
function Prepare(nameOnly)
	...
	if   (nameOnly) then
        ...
        Range = instance:addStream("Range", core.Bar, name, "Range",UpC, first);
...
function Update(period,mode)
	...
	Range:setColor(period, UpC);
	else
	Range:setColor(period,  DownC);
	end
...
function Activate (id, period)
  ...
  --Parameters (1, "Bar Range");	
	--Parameters (2, "Bar Range/ Signal Line");
	  ...
	  if id == 1  and ON=="Bar Range"  then
	  
	       
			if  Range[period] > Range[period-1] 			 
			then
			 ...
			 D[id] = nil;
							  ...
							  SoundAlert(Up[id]);
							  EmailAlert(  Label[id], " Range Extension ", period);
							  SendAlert("Range Extension");  
							        
									Pop(Label[id], " Range Extension " );  	
            ...
            then			
			               ...
			               core.host:execute("drawLabel1", source:serial(period), source:date(period),  core.CR_CHART, 0, core.CR_CHART, core.H_Center, core.V_Bottom, font, DownTrendColor, "\226");						   
		     ...
		     U[id] = nil;
							 ...
							 D[id]=source:serial(period);
							        if not Ignore then
							        SoundAlert(Down[id]);			 
							        EmailAlert( Label[id] , " Range Contraction", period);
									Pop(Label[id], " Range Contraction " );  	
								    SendAlert(" Range Contraction");
							        end
			                  end			   
	         end
	  ...
	  if id == 2  and   ON~="Bar Range" then
	  
	       
			if  Range[period] > Signal[period] 
			and   Range[period-1] <= Signal[period-1] 
			then
			...
			elseif  Range[period] < Signal[period] 
			and   Range[period-1] >= Signal[period-1] 
            then			
