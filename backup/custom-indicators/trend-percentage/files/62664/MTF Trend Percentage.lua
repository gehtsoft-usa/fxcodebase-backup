...
function Init()
    indicator:name("MTF Trend Percentage");
    ...
    indicator.parameters:addColor("UP", "Color for Bull", "", core.rgb(0, 255, 0));
    indicator.parameters:addColor("DN", "Color for Bear", "", core.rgb(255, 0, 0));	
...
function Prepare(nameOnly)
    ...
    assert(core.indicators:findIndicator("TREND PERCENTAGE") ~= nil, "Please, download and installTREND PERCENTAGE.LUA indicator");
    ...
    for i = 1, Number, 1 do	 
	    	...
	    	local Test = core.indicators:create("TREND PERCENTAGE", source.close , Period[i]);   
		...
		Indicator[i] = core.indicators:create("TREND PERCENTAGE", SourceData[i].close, Period[i]);
		loading[i]= true;		
    ...
    for j = 1, Number, 1 do			
      ...
      -- context:createFont (1, "Wingdings", (xCell/10)*(Size/100)*2, yCell*(Size/100), 0);	
		...
		for i = 1, Number, 1 do 	
		    ...
		    if  Indicator[i].Bull:hasData( Indicator[i].Bull:size()-1) then
			...
			--
			 local Value1= string.format("%." .. 2 .. "f", Indicator[i].Bull[Indicator[i].Bull:size()-1] );
			 local Value2= string.format("%." .. 2 .. "f", Indicator[i].Bear[Indicator[i].Bear:size()-1] );
			
             width, height = context:measureText (2, Value1, 0)			
			 context:drawText (2,  Value1, UP, -1, context:right ()-(i)*xCell    , context:top ()+VShift+yCell*2, context:right ()-(i-1)*xCell + width, context:top ()+VShift+yCell*2 + height, 0);
			 
			 
			   width, height = context:measureText (2, Value2, 0)			
			 context:drawText (2,  Value2, DN, -1, context:right ()-(i)*xCell    , context:top ()+VShift+yCell*3, context:right ()-(i-1)*xCell + width, context:top ()+VShift+yCell*3 + height, 0);
