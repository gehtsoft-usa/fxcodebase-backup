...
-- http://fxcodebase.com/code/viewtopic.php?f=17&t=9358
...
function Prepare(nameOnly)
    ...
    local name = profile:id() .. "(" .. source:name() .. ", " .. tostring(RP) .. ", " .. tostring(Method) .. ")";
    instance:name(name);
...
function Update(period, mode)
   ...
   local diff = source[period] - source[period - 1];
	...
	if period < first +RP  or not source:hasData(period) then
	return;
	end	
