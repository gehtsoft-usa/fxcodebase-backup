...
function Init()
	...
	indicator.parameters:addDouble("OB", "OB Level ", "", 85);
	indicator.parameters:addDouble("OS", "OS Level ", "", 15);
	...
	for i = 1, Num, 1  do
				...
				if  Indicator[j][i].DATA:hasData(Indicator[j][i].DATA:size()-1) and Indicator[j][i].DATA:hasData(Indicator[j][i].DATA:size()-2) then					
						...
						elseif Indicator[j][i].DATA[Indicator[j][i].DATA:size()-1] <  Indicator[j][i].DATA[Indicator[j][i].DATA:size()-2] then
						 ...
						 if Mode[i]	 =="Arrows" then
							 if Indicator[j][i].DATA[Indicator[j][i].DATA:size()-1] > OB then
							 Color = Up;
							 elseif Indicator[j][i].DATA[Indicator[j][i].DATA:size()-1]  < OS then
							  Color = Down;	
							 else
							  Color = No;	
							 end  
