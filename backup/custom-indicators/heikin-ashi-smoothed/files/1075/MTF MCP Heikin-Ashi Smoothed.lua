...
function AddTimeFrame(id , FRAME , DEFAULT  )
    ...
    indicator.parameters:addStringAlternative("Method1"..id, "Vidya (1995)*", "", "VIDYA");
    indicator.parameters:addStringAlternative("Method1"..id, "Vidya (1992)*", "", "VIDYA92");
    indicator.parameters:addStringAlternative("Method1"..id, "Wilders*", "", "WMA");
    ...
    indicator.parameters:addStringAlternative("Method2"..id, "Vidya (1995)*", "", "VIDYA");
    indicator.parameters:addStringAlternative("Method2"..id, "Vidya (1992)*", "", "VIDYA92");
    indicator.parameters:addStringAlternative("Method2"..id, "Wilders*", "", "WMA");
	 ...
	 for i = 1, Count, 1 do	
      ...
      HA[i]={};
		   ...
		   for j = 1, Num, 1 do	
			...
			Temp= core.indicators:create("HASM", source, Method1[j], N1[j], Method2[j], N2[j]   );
			first =  Temp.DATA:first();  
		   Source[i][j]= core.host:execute("getSyncHistory", Pair[i], TF[j], source:isBid(),first+1,20000 + ID , 10000 +ID);
		   loading [i][j]=true;
		   
		   HA [i][j]= core.indicators:create(  "HASM", Source[i][j], Method1[j], N1[j], Method2[j], N2[j] );
