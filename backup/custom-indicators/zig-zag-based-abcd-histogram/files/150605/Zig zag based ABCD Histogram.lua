...
function Init()
    ...
    indicator.parameters:addColor("color4", "AD Line Color", "", core.rgb(128, 128, 128));
end
...
-- Routine
 function Prepare(nameOnly)    
	...
	AD = instance:addStream("AD" , core.Line, " AD"," AD",instance.parameters.color4, first); 
    AD:setPrecision(math.max(2, source:getPrecision()));		
end
...
function Update(period, mode)
	...
	FirstLoad=false;
			...
			for i= first, source:size()-1, 1 do
				  ...
				  AD[i]= ZigZag.DATA[A]-ZigZag.DATA[D];	
					  if Absolute  then
					  AB[i]=math.abs(AB[i])
					  BC[i]=math.abs(BC[i])
					  CD[i]=math.abs(CD[i])
