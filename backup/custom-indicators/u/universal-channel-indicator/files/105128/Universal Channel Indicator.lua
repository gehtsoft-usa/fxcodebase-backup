...
function Prepare(nameOnly)
	...
	MA[i] = core.indicators:create(MA_Method[i], source, MA_Period[i]);	
	end
...
function Update(period,mode)
	...
	for i = 1, 3 do
        	...
        	if Method == "Donchian" then
				if (ac) then
					du[i][period] =  mathex.max (source, period -Donchian_Period[i] +1  , period);
					dn[i][period] = mathex.min (source, period -Donchian_Period[i] +1  , period);
				else
					 du[i][period] =  mathex.max (source, period -Donchian_Period[i] +1-1  , period-1);
					 dn[i][period] = mathex.min (source, period -Donchian_Period[i] +1-1  , period-1);
				end
			...
			elseif Method == "Pip" then
			 ...
			 elseif Method == "Deviation" then
			    ...
			    if (ac) then
				    iDev=mathex.stdev(source, period-Dev[i]+1, period);
					du[i][period] =  MA[i].DATA[period]+Multi[i]*iDev;
					dn[i][period] =  MA[i].DATA[period]-Multi[i]*iDev;
				else 
				     iDev=mathex.stdev(source, period-1-Dev[i]+1, period-1);
