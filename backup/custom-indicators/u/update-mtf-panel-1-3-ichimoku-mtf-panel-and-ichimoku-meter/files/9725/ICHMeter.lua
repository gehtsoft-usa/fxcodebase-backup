...
function Init()
    indicator:name("Ichimoku Signal Panel V1.1");
...
function Prepare()
	...
	if instance.parameters.eIch then
		...
		sSL:setWidth(instance.parameters.widthSL);
		sSL:setStyle(instance.parameters.styleSL);
		sTL = instance:addStream("TL", core.Line, name .. ".TL", "TL", instance.parameters.clrKS, firstPeriod + Y - 1)
		sTL:setWidth(instance.parameters.widthTL);
		sTL:setStyle(instance.parameters.styleTL);
		sCS = instance:addStream("CS", core.Line, name .. ".CS", "CS", instance.parameters.clrCS, firstPeriod, -Y)
		sCS:setWidth(instance.parameters.widthCS);
		sCS:setStyle(instance.parameters.styleCS);
		sSA = instance:addStream("SA", core.Line, name .. ".SA", "SA", instance.parameters.clrSSA, math.max(sSL:first(), sTL:first()), Y)
		sSA:setWidth(instance.parameters.widthSSA);
		sSA:setStyle(instance.parameters.styleSSA);
		sSB = instance:addStream("SB", core.Line, name .. ".SB", "SB", instance.parameters.clrSSB, firstPeriod + Z - 1, Y)
		sSB:setWidth(instance.parameters.widthSSB);
		sSB:setStyle(instance.parameters.styleSSB);
    ...
    drwText("-------Ichimoku Meter 1.1 ------------------------" ,1,-250,0,instance.parameters.TC,sF);
...
function getSignal(period)
    ...
    elseif 	source.close[period] < SA[period] and source.close[period] < SB[period] then
			S.PA = "Bearish";
			PAdirection = sDN;
    ...
    if 		SL[period-2]<=TL[period-2] and SL[period-1]>TL[period-1] then 
        if 		source.close[period-1] < SA[period-1] and source.close[period-1] < SB[period-1] then
				...
				TSdirection= aUP;
        elseif 	source.close[period-1] > SA[period-1] and source.close[period-1] > SB[period-1] then
...
----------------------------------------------------
	...
	if 		string.find(S.PA,"Bullish") 		~= 		nil 																						then 	-- Bullish
		-- Chikou 
		if 	CS_Signal == "Bullish"						then    																					end
		
	elseif 	string.find(S.PA,"Bearish") 		~= 		nil 																						then 	-- Bearish
		-- Chikou 
		if 	CS_Signal == "Bearish"						then    																					end
