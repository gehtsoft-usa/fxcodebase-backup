...
function Init()
    indicator:name("MTF MCP Ulcer Index List");
...
function Parameters (id , FRAME, flag )
	...
	indicator.parameters:addBoolean("Inverse"..id, "Inverse pair", "", false);	
...
function Prepare(nameOnly)
	...
	for i = 1 , 13 , 1 do   
	   ...
	   iType[Num]=  instance.parameters:getString ("Type"..i);
	   Inverse[Num]=  instance.parameters:getBoolean ("Inverse"..i);
	...
	assert(core.indicators:findIndicator("UI") ~= nil, "Please, download and install UI.LUA indicator");
	...
	for j = 1, Count, 1 do
		 ...
		 for i = 1, Num, 1 do	
		      ...
		      Test = core.indicators:create("UI", source.close   ,Period[i], Inverse[i]);   
	          ...
	          first= Test.DATA:first();
			   ...
			   Indicator[j][i] = core.indicators:create("UI", SourceData[j][i].close,Period[i], Inverse[i]  );
        ...
        context:createFont (8, "Wingdings",iwidth, iheight , 0);
		        ...
		        for i = 1, Count, 1 do
						 for j = 1, Num, 1 do	
                             ...
                             if Indicator[i][j].DATA:hasData( Indicator[i][j].DATA:size()-1)
							 and Indicator[i][j].DATA:hasData( Indicator[i][j].DATA:size()-2)
							 then
						   ...
						   if j== Num then 
							...
							context:drawText (7,  TF[j], Color, -1, x1+xGap  , y1-height+yGap ,x2+xGap ,  y1+yGap , context.CENTER   );	
						         ...
						         if iType[j]~= "Trend" then
								 Text= win32.formatNumber(  Indicator[i][j].DATA[ Indicator[i][j].DATA:size()-1], false, 5);


                                 width, height = context:measureText (7, Text, 0);	
								 context:drawText (7, Text, iColor, -1, x1+xGap  , y1-height+yGap*2 ,x2+xGap ,  y1+yGap*2 , context.CENTER   );			
								                        ...
								                        context:drawText (8, Text, iColor, -1, x1+xGap  , y1-height+yGap*2 ,x2+xGap ,  y1+yGap*2 , context.CENTER   );											 
