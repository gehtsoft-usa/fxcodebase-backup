...
function Init()
    ...
    indicator.parameters:addColor("clrUP3", "UP3 Color", "", core.rgb(64, 255, 64));
    indicator.parameters:addColor("clrDN3", "DN3 Color", "", core.rgb(255, 64, 64));
    indicator.parameters:addColor("clrUP2", "UP2 Color", "", core.rgb(170, 255, 170));
    indicator.parameters:addColor("clrDN2", "DN2 Color", "", core.rgb(255, 170, 170));
    indicator.parameters:addColor("clrUP1", "UP1 Color", "", core.rgb(196, 224, 100));
    indicator.parameters:addColor("clrDN1", "DN1 Color", "", core.rgb(224, 196, 100));
...
function Update(period, mode)
    ...
    if stream == nil then
        ...
        M3:update(mode);
        M4:update(mode);
        --set hasdata falg for external check
        TSN[0] = 0;
...
function GetTrendStatus(tStatus, pMASL, period)
    ...
    if MAS4[period]<pMASL*-1 and core.crossesUnder(MA1, MA2, period) then tStatus=sadn; end
    if MAS4[period]>pMASL and core.crossesOver(MA1, MA2, period) then tStatus=saup; end

    --phase to trend decelerate : straddle
    if MAS4[period]<pMASL*-1 and core.crossesOver(MA1, MA2, period) then tStatus=sddn; end
    if MAS4[period]>pMASL and core.crossesUnder(MA1, MA2, period) then tStatus=sdup; end
    ...
    then if MAS4[period]<pMASL*-1 then tStatus=sddn; else tStatus=swdn; end end
    ...
    then if MAS4[period]>pMASL then tStatus=sdup; else tStatus=swup; end end
    
    --phase to whipsaw : strangle
    if core.crossesUnder(MAS4, pMASL, period) then tStatus=swdn; end
    if core.crossesOver(MAS4, pMASL*-1, period) then tStatus=swup; end

    --phase for trend start
    if core.crossesUnder(MAS4, pMASL*-1, period) then if MA1[period]<MA2[period] then tStatus=sadn else tStatus=sddn end end
    if core.crossesOver(MAS4, pMASL, period) then if MA1[period]>MA2[period] then tStatus=saup else tStatus=sdup end end
