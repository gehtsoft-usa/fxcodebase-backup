...
function Init()
    ...
    indicator.parameters:addString("Lines", "List of lines between high and low", "", "0.2, 0.5, 0.8");
    indicator.parameters:addColor("LinesColor", "The lines color", "", core.rgb(255, 255, 0));
end
...
function Prepare(nameOnly)
    ...
    LList, LCount=core.parseCsv(instance.parameters.Lines);
    for i=0, LCount-1, 1 do
        LList[i]=tonumber(LList[i]);
    end
    ...
    if LCount>0 then
        for i=0, LCount-1, 1 do
            MLines[i]=instance:addStream("ML" .. i, core.Line, name .. "." .. "ML" .. i, "ML" .. i, instance.parameters.LinesColor, first);
        end
    end
end
    ...
    if period >= first then
        ...
        elseif curr == BE_BOX then
            if H:hasData(period - 1) then
                ...
                for ii=0, LCount-1, 1 do
                    MLines[ii][period]=L[period]+(H[period]-L[period])*LList[ii];
                end
            end
        elseif curr == BE_PERIOD then
            ...
            if H:hasData(period - 1) and
               ...
               L[period - 1] == l then
                ...
                PL[period] = l;
                for ii=0, LCount-1, 1 do
                    MLines[ii][period]=l+(h-l)*LList[ii];
                end
            else
                for i = p, period, 1 do
                    H[i] = h;
