...
--http://fxcodebase.com/code/viewtopic.php?f=17&t=70995
...
{ 
    {
        ...
        UpCondition = function (period)
            if period < 1 or not Alligator.Jaw:hasData(period - 1) then
                return false;
            end
            local Range = (source.close[period] - source.low[period]) / ((source.high[period] - source.low[period]) / 100);
            return source.low[period] < source.low[period - 1]
                and Range > 50
        ...
        DownCondition = function (period)
            ...
            local Range = (source.close[period] - source.low[period]) / ((source.high[period] - source.low[period]) / 100);
            return source.high[period] > source.high[period - 1]
                and Range < 50
...
function Init()
    indicator:name("Bill Williams Profitunity system");
...
function Prepare(nameOnly)
    ...
    Alligator = core.indicators:create("ALLIGATOR", source, instance.parameters.JawN, instance.parameters.JawS, instance.parameters.TeethN, 
        instance.parameters.TeethS, instance.parameters.LipsN, instance.parameters.LipsS, instance.parameters.MTH);
    indi_alerts:Prepare();
