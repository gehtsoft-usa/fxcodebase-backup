...
-- http://fxcodebase.com/code/viewtopic.php?f=17&t=761
...
function Update(period, mode)
    ...
    if bf_data == nil then
        ...
        RSI = core.indicators:create("STOCHRSI", bf_data.close, N, K, KS, D);
        return ;
    end
    ...
    if RSI:getStream(0):hasData(p) then
        SK[period] = RSI:getStream(0)[p];
    end
    if RSI:getStream(1):hasData(p) then
        SD[period] = RSI:getStream(1)[p];
    end
end
