...
-- http://fxcodebase.com/code/viewtopic.php?f=17&t=66861
...
function Activate(alert, period, historical_period)
    ...
    if alert.id == 1 and alert.ON and period >= 2 and ma.DATA:hasData(period - 2) then
        if ma.DATA[period - 2] < ma.DATA[period - 1] 
            and core.crossesOver(source.close, ma.DATA, period - 1)
        ...
        elseif ma.DATA[period - 2] > ma.DATA[period - 1]
            and core.crossesUnder(source.close, ma.DATA, period - 1)
            and source.open[period - 1] > source.close[period - 1]
