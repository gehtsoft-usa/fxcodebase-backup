...
-- http://fxcodebase.com/code/viewtopic.php?f=17&t=65005
...
function Prepare(nameOnly)
	...
	if (PeriodBegin < PeriodEnd) then
    	...
    	if (PeriodBegin > PeriodEnd) then
            assert(BoxEnd < PeriodBegin and BoxEnd > PeriodEnd, "The period of Box end must be after the period end and before period start");
        end
    end
...
function BelongsTo(time)
    ...
    if PeriodBegin < PeriodEnd then
        ...
        if (t >= 0 and t <= PeriodEnd) or (t >= PeriodBegin and t <= 86400) then
            return BE_PERIOD;
        elseif t >= PeriodEnd and t < BoxEnd then
            return BE_BOX;
        else
