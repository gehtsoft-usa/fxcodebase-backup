...
-- http://fxcodebase.com/code/viewtopic.php?f=17&t=3169&sid=3e0f87a256b3e1ed0bd16e1fec0c0a6c
...
function Init()
    indicator:name("MTF MCP ImpulseBar Dashboard");
    indicator:description("MTF MCP ImpulseBar Dashboard");
...
local FastEMA, SlowEMA, SignalEMA, app_price;
...
function Prepare(nameOnly)
	...
	assert(core.indicators:findIndicator("IMPULSEBAR") ~= nil, "Please, download and install IMPULSEBAR.LUA indicator");   
	 ...
	 for i = 1, Count, 1 do	
      ...
      Indicator[i]={};
		   ...
		   for j = 1, Num, 1 do	
			...
			Temp= core.indicators:create("IMPULSEBAR", source  ,FastEMA, SlowEMA, SignalEMA, app_price );
			first =  Temp.DATA:first()*2;  
		   Source[i][j]= core.host:execute("getSyncHistory", Pair[i], TF[j], source:isBid(),math.max(300,first),20000 + ID , 10000 +ID);
		   loading [i][j]=true;
		   
		   Indicator [i][j]= core.indicators:create("IMPULSEBAR", Source[i][j] , FastEMA, SlowEMA, SignalEMA, app_price,Up,Down,Neutral);
...
function Draw(stage, context)
 ...
 function Calculate (context,i, j )
    ...
    local SymbolColor=Neutral;	
	     ...
	     -- Indicator[i][j].DATA:colorI(Indicator[i][j].DATA:size()-1) ==  Up then
		 
		 
		  Symbol="\108"; 
		  SymbolColor= Indicator[i][j].DATA:colorI(Indicator[i][j].DATA:size()-1);
