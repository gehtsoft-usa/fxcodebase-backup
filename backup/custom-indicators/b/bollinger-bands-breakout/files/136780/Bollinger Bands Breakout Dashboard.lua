...
local indi_id = "BOLLINGER BANDS BREAKOUT WITH ALERT";
...
function CreateIndicators(source)
    ...
    p:setBoolean("strategy_mode", true);
...
function GetLastSignal(indi, source)
    local up = indi[1].signal_1;
    local down = indi[1].signal_2;
    for i = 0, up:size() - 1 do
        if up[NOW - i] == 1 then
            return 1, "B";
        end
        if down[NOW - i] == 1 then
            return -1, "S";
        end
    end
    return 0, "-";
