...
{ 
    {
        ...
        UpCondition = function (period)
            ...
            return MACD[period] > MACD[period - 1] and MACD[period - 1] <= MACD[period - 2];
        end,
        ...
        FormatMessage = function(source, period, level, label, isUp, metadata)
            return string.format(
                ...
                "Price: %s\013\010" .. 
                "Date: %s", 
                isUp and (label .. " Green") or (label .. " Red"), 
                source:instrument(), 
                source:barSize(), 
    ...
    },
    {
        ...
        UpCondition = function (period)
            ...
            return core.crossesOver(MACD, Top, period);
        end,
        DownCondition = function (period)
            if period <= first+ Period3 then
                return false;
            end
            return core.crossesUnder(MACD, Top, period);
        end,
        FormatMessage = function(source, period, level, label, isUp, metadata)
            return string.format(
                "Label: %s\013\010" ..
                ...
                "Price: %s\013\010" .. 
                "Date: %s", 
                isUp and (label .. " Crossed over top") or (label .. " Crossed under top"), 
                source:instrument(), 
                source:barSize(), 
                win32.formatNumber(source:isBar() and source.close[NOW] or source[NOW], false, source:getPrecision()), 
                core.formatDate(core.now()));
    ...
    },
    {
        ...
        UpCondition = function (period)
            ...
            return core.crossesOver(MACD, Bottom, period);
        end,
        DownCondition = function (period)
            if period <= first+ Period3 then
                return false;
            end
            return core.crossesUnder(MACD, Bottom, period);
        end,
        FormatMessage = function(source, period, level, label, isUp, metadata)
            return string.format(
                "Label: %s\013\010" ..
                ...
                "Price: %s\013\010" .. 
                "Date: %s", 
                isUp and (label .. " Crossed over bottom") or (label .. " Crossed under bottom"), 
                source:instrument(), 
                source:barSize(), 
                win32.formatNumber(source:isBar() and source.close[NOW] or source[NOW], false, source:getPrecision()), 
                core.formatDate(core.now()));
...
function Prepare(nameOnly)
    ...
    local Parameters =
        Period1 ..
        ", " ..
            Method1 .. ", " .. Period2 .. ", " .. Method2 .. ", " .. Period3 .. ", " .. Method3 .. ", " .. Multiplier
...
function Activate(alert, period, historical_period)
    ...
    if isUp then
        ...
        if source:isBar() then
            alert:UpAlert(source, period, MACD[period], historical_period, upMetadata);
        else
            alert:UpAlert(source, period, MACD[period], historical_period, upMetadata);
        end
        return;
    ...
    if isDown then
        ...
        if source:isBar() then
            alert:DownAlert(source, period, MACD[period], historical_period, downMetadata);
        else
            alert:DownAlert(source, period, MACD[period], historical_period, downMetadata);
        end
    end
