...
--http://fxcodebase.com/code/viewtopic.php?f=17&t=70419
...
local indi_name = "MA Cross Dashboard";
...
function CreateIndicators(source)
    ...
    indicators[#indicators + 1] = CreateAverages(instance.parameters.ma1_period, instance.parameters.ma1_method, source);
    indicators[#indicators + 1] = CreateAverages(instance.parameters.ma2_period, instance.parameters.ma2_method, source);
...
function GetLastSignal(indi, source)
    for i = 0, source:size() do
        if core.crossesOver(indi[1].DATA, indi[2].DATA, NOW - i) then
            return 1, source:date(NOW - i), "MA1 > MA2";
        elseif core.crossesUnder(indi[1].DATA, indi[2].DATA, NOW - i) then
            return -1, source:date(NOW - i), "MA1 < MA2";
        end
    end
    return 0, nil, "-";
