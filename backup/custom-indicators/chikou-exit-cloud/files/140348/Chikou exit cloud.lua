...
-- http://fxcodebase.com/code/viewtopic.php?f=17&t=70854
-- More information about this indicator can be found at:
--http://fxcodebase.com/code/viewtopic.php?f=17&t=70854
...
{ 
    {
        ...
        UpCondition = function (period)
            if not ich.CS:hasData(period) or not ich.SA:hasData(period) or not ich.SB:hasData(period) then
                return false;
            end
            if ich.CS[period] <= source.close[period] then
                return false;
            end
            return (core.crossesOver(ich.CS, ich.SB, period) and ich.CS[period] > ich.SA[period]);
        end,
        DownCondition = function (period)
            ...
            return (core.crossesUnder(ich.CS, ich.SA, period) and ich.CS[period] < ich.SB[period]);
        end,
...
function Init()
    indicator:name("Chikou Exit Cloud");
...
function Prepare(nameOnly)
    ...
    ich = core.indicators:create("ICH", source, instance.parameters.TenkanSenPeriod, instance.parameters.KijunSenPeriod, instance.parameters.SenkouSpanPeriod)
...
function Activate(alert, period, historical_period)
    ...
    if isUp then
        ...
        if source:isBar() then
            alert:UpAlert(source, period, ich.CS[period], historical_period, upMetadata);
        else
            alert:UpAlert(source, period, ich.CS[period], historical_period, upMetadata);
        end
        return;
    ...
    if isDown then
        ...
        if source:isBar() then
            alert:DownAlert(source, period, ich.CS[period], historical_period, downMetadata);
        else
            alert:DownAlert(source, period, ich.CS[period], historical_period, downMetadata);
        end
    end
