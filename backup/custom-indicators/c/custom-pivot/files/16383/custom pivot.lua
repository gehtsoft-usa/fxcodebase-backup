...
-- http://fxcodebase.com/code/viewtopic.php?f=17&t=7345
...
function Init()
	...
	indicator.parameters:addGroup("MIDPOINT"); 
	...
	indicator.parameters:addGroup("TC/BC");
	
	indicator.parameters:addBoolean("showTC", "Show TC", "", true);
    indicator.parameters:addColor("clrTC", "TC Color", "", core.rgb(128, 128, 128));
    indicator.parameters:addInteger("widthTC", "TC Line Width", "", 3, 1, 5);
    indicator.parameters:addInteger("styleTC","TC Line Style", "", core.LINE_SOLID);
    indicator.parameters:setFlag("styleTC", core.FLAG_LEVEL_STYLE);
	
	indicator.parameters:addBoolean("showBC", "Show BC", "", true);
    indicator.parameters:addColor("clrBC", "BC Color", "", core.rgb(128, 128, 128));
    indicator.parameters:addInteger("widthBC", "BC Line Width", "", 3, 1, 5);
    indicator.parameters:addInteger("styleBC","BC Line Style", "", core.LINE_SOLID);
    indicator.parameters:setFlag("styleBC", core.FLAG_LEVEL_STYLE);
end
...
function Prepare(onlyName)
	...
	style[TC] = instance.parameters.styleTC;
    style[BC] = instance.parameters.styleBC;
	clr[TC] = instance.parameters.clrTC;
    clr[BC] = instance.parameters.clrBC;
	show[TC] = instance.parameters.showTC;
    show[BC] = instance.parameters.showBC;
	width[TC] = instance.parameters.widthTC;
    width[BC] = instance.parameters.widthBC;
...
function ShowLevels(data, period)
    ...
    if LabelLoc == O_BEG or LabelLoc == O_BOTH then
        host:execute("drawLabel", PID + 1000, d1, P[period], name[RP]);
    end

    for i = S1, BC, 1 do
        -- if  data[i] ~= 0 then
		 if data[i] ~= 0  and show[i] then
            ...
            if LabelLoc == O_BEG or LabelLoc == O_BOTH then
                host:execute("drawLabel", i + 1000, d1, data[i], name[i]);
            end
       -- else
            --host:execute("removeLine", i);
          --  host:execute("removeLabel", i);
           -- host:execute("removeLabel", i + 1000);
        end
    end
...
function ShowMPP(i, d1, d2, p1, p2, l)
    if p1 ~= 0 and p2 ~= 0 then
        local p = (p1 + p2) / 2;
        host:execute("drawLine", PID + 100 + i, d1, p, d2, p, clrMP, styleMP, widthMP, l .. "(" .. round(p, source:getPrecision()) .. ")");
        if LabelLoc == O_END or LabelLoc == O_BOTH then
            host:execute("drawLabel", PID + 100 + i, d2, p, l);
        end
        if LabelLoc == O_BEG or LabelLoc == O_BOTH then
            host:execute("drawLabel", PID + 1100 + i, d1, p, l);
        end
    end
end
