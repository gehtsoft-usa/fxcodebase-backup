...
function Init()
    indicator:name("Conditional Consecutive Candle Count");
    indicator:description("Conditional Consecutive Candle Count");
    indicator:requiredSource(core.Tick);
    indicator:type(core.Indicator);
    indicator.parameters:addGroup("Calculation");
    indicator.parameters:addInteger("Period", "Period", "Period", 0);
	indicator.parameters:addInteger("Minimum", "", "Minimum Period", 5);
...
function Update(period)
   ...
   if Count[period]==0 and Count[period-1]> Minimum then 
   Up[period]=1;
   elseif Count[period]==0 and Count[period-1]< - Minimum then    
   Down[period]=1;
   end
	 ...
	 if period == source:size()-1 then
	   if Period == 0 then
	   max = mathex.sum(Up, source:first(), source:size()-1);
	   min = mathex.sum(Down, source:first(), source:size()-1);
	   else
	   max = mathex.sum(Up,  math.max(source:first(), (source:size()-1-Period+1)), source:size()-1);
	   min = mathex.sum(Down,  math.max(source:first(), (source:size()-1-Period+1)), source:size()-1);	   
	   end
	 end
