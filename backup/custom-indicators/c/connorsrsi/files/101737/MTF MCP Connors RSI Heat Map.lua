...
--+------------------------------------------------------------------+
 ...
 function Add(id, TF,Flag, Instrument )
	...
	indicator.parameters:addInteger("LenRSI".. id, "RSI Closes Length", "RSI Closes Length", 3);
    indicator.parameters:addInteger("LenUD".. id, "RSI UpClose Length", "RSI UpClose Length", 2);
    indicator.parameters:addInteger("LenRank".. id, "PerecentRank Length", "PerecentRank Length", 100);
end
 
 
function Init()
    indicator:name("MTF MCP Connors RSI Heat Map");
	...
	indicator.parameters:addColor("Over", "Overbought/Oversold Color","", core.rgb(0, 0, 255));
...
local Indicator = {} ;
 ...
 function Prepare(nameOnly)   
	...
	assert(core.indicators:findIndicator("CONNORSRSI") ~= nil, "Please, download and install CONNORSRSI.LUA indicator");    
	...
	s2, e2 = core.getcandle(iTF[i], 0, 0, 0);
	 ...
	 Number=Number+1;
		  ...
		  LenRSI[Number]= instance.parameters:getInteger("LenRSI" .. i);
		  LenUD[Number]= instance.parameters:getInteger("LenUD" .. i);
		  LenRank[Number]= instance.parameters:getInteger("LenRank" .. i);
	 ...
	 Label[Number]="";
                ...
                Label[Number]=Label[Number] .. " - " ..  TF[Number];				
			  	...
			  	Temp1= core.indicators:create("CONNORSRSI", source.close ,   LenRSI[Number],  LenUD[Number], LenRank[Number] );  				
				ifirst= Temp1.DATA:first()*2;
				 ...
				 loading[Number]  = true;  	 
				 Indicator[Number] = core.indicators:create("CONNORSRSI", SourceData[Number].close ,  LenRSI[Number],  LenUD[Number], LenRank[Number]);   
...
function Draw (stage, context)
   ...
   context:setClipRectangle(context:left(), context:top(), context:right(), context:bottom());
        ...
        if not init then
			...
			context:createPen (51, context.SOLID, 3, Over)       
			    ...
			    for j= 1, Number , 1 do
				  ...
				  if p~= false then
										...
										if Indicator[j].DATA:hasData(p) and Indicator[j].DATA:hasData(p-1) then 
											       ...
											       if Indicator[j].DATA[p] > overbought 
												   or  Indicator[j].DATA[p] < oversold
												   then
												   C1=51;
												   end
