...
--+------------------------------------------------------------------+
function Init()
    indicator:name("Currency pair Exposure");
...
return;
 ...
 function AsyncOperationFinished(cookie)
    ...
    while row ~= nil do
	    ...
	    if ValueLong[row.Instrument]== nil 
		then
		WeHave=WeHave+1;
		Index[WeHave]=row.Instrument;
		ValueLong[row.Instrument]=0;
		ValueShort[row.Instrument]=0; 
		end
	 
     
		if row.BS == "B" then	
			ValueLong[row.Instrument]=ValueLong[row.Instrument]+ row.Lot;
            Maximum=math.max(Maximum, ValueLong[row.Instrument]);
        end
		
		if row.BS == "S" then					
			ValueShort[row.Instrument]=ValueShort[row.Instrument]+ row.Lot;
			Maximum=math.max(Maximum, ValueShort[row.Instrument]);
        end
