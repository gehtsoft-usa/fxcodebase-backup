...
-- http://fxcodebase.com/code/viewtopic.php?f=17&t=64819
-- Id: 18492
...
function Init()
    indicator:name("Candle-range indicator with alert");
   ...
   indicator:requiredSource(core.Bar);
	...
	indicator.parameters:addString("Direction", "2. MA Method", "Method" , "Both");
    indicator.parameters:addStringAlternative("Direction", "Up", "Direction" , "Up");
    indicator.parameters:addStringAlternative("Direction", "Down", "Direction" , "Down");
	indicator.parameters:addStringAlternative("Direction", "Both", "Direction" , "Both");
	
    indicator.parameters:addDouble("Range", "Minimum Range for Candle Range", "", 50, 0, 1000000 );
	indicator.parameters:addDouble("Max", "Maximum Range for Alert", "", 1000000, 0, 1000000 );
...
function Prepare(nameOnly)   
    ...
    local name = profile:id() .. "(" .. source:name() .. ", " .. Range .. ")";
	instance:name(name);
	if nameOnly then
		return;
	end
...
function Activate (id, period)
   ...
   Alert[id][period]=0;
	  ...
	  if id == 1  and ON[id]  then
	       ...
	       --Max
			if   math.abs(source.open[period]-source.close[period])/source:pipSize() > Range
			and  math.abs(source.open[period]-source.close[period])/source:pipSize() < Max
			...
			elseif   math.abs(source.open[period]-source.close[period])/source:pipSize() > Range
			and  math.abs(source.open[period]-source.close[period])/source:pipSize() < Max
			and source.open[period]>source.close[period] 
            then			
