...
function Init()
    indicator:name("Custom Pattern 3");
...
function Update(period, mode)
	...
	local Candle1=period-2;
	local Candle2=period-1;
	local Candle3=period;
	...
	if  Bottom[Candle1]>  50
	then
	C1=-1;
	end
	
	if  source.high[Candle2]< source.high[Candle1]
	and  source.low[Candle2]< source.low[Candle1]
	and math.max(source.open[Candle2],source.close[Candle2]) < source.high[Candle1]
	and math.min(source.open[Candle2],source.close[Candle2]) > source.low[Candle1]
	then
	C2=1;
	end
	
	if  source.low[Candle2]> source.low[Candle1]
