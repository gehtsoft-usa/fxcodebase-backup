...
function Init()
	indicator:name("Consecutive candle range Tick Version")
...
function Update(period)
	...
	if source[period] > source[period - 1] then
		if source[Count[period - 1]] < source[Count[period - 1] - 1] then
			Count[period] = period
		end
	...
	for i = firstBar, lastBar, 1 do
		if (i - Count[i] + 1) >= Period and Count[i + 1] == i + 1 then
			if source[i] > source[i - 1] then
				...
				visible, y1 = context:pointOfPrice(source[i])
				visible, y2 = context:pointOfPrice(source[Count[i]])
				...
				visible, y1 = context:pointOfPrice(source[Count[i]])
