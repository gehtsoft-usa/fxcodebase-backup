...
function Init()
    ...
    indicator.parameters:addBoolean("ShowHL", "Show session high/low", "", false);
    indicator.parameters:addInteger("FS", "Font Size", "", 8, 4, 24);
end
...
function Process(session, period)
    ...
    if date >= sfrom and date < sto then
        ...
        if hi ~= 0 then
            ...
            if t >= 0 and t <= period then
                session.LN:set(t, hi, session.name .. "\013\010");
                if ShowHL then
                    local open = source.open[t];
                    session.LH:set(t, hi, "" .. hi .. "(" .. math.floor((hi - open) / pipsize * 10 + 0.5) / 10 .. ")");
                    session.LL:set(t, lo, "" .. lo .. "(" .. math.floor((lo - open) / pipsize * 10 + 0.5) / 10 .. ")");
                end
            end
...
function CreateSession(from, len, color, name, iname)
    ...
    if ShowHL then
        session.LH = instance:createTextOutput(name .. "_HL", name .. "_HL", "Arial", instance.parameters.FS, core.H_Right, core.V_Top, color, 0);
        session.LL = instance:createTextOutput(name .. "_LL", name .. "_LL", "Arial", instance.parameters.FS, core.H_Right, core.V_Bottom, color, 0);
    end
    session.LN = instance:createTextOutput(name .. "_LN", name .. "_LN", "Arial", instance.parameters.FS, core.H_Right, core.V_Top, color, 0);
    return session;
end
