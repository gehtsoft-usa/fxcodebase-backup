...
function Init()
    indicator:name("MTF MCP Tow Lines");
    indicator:description("MTF MCP Tow Lines");
    ...
    indicator.parameters:addString("Mode", "Calculation Mode", "Calculation Mode" , "AB"); 
    indicator.parameters:addStringAlternative("Mode", "AB", "AB" , "AB");
	indicator.parameters:addStringAlternative("Mode", "CA", "CA" , "CA");
	indicator.parameters:addStringAlternative("Mode", "CB", "CB" , "CB");
...
function AddTimeFrame(id , FRAME , DEFAULT  )
    ...
    indicator.parameters:addInteger("PeriodA"..id, "A Line Period", "Period",20, 1,2000);
	indicator.parameters:addInteger("PeriodB"..id, "B Line Period", "Period",6, 1,2000);
	indicator.parameters:addInteger("PeriodC"..id, "C Line Period", "Period",5, 1,2000);
...
function Prepare(nameOnly) 
	...
	Num=0;
		for i = 1 , 13 , 1 do  
		   ...
		   if Use[i] then
			...
			PeriodA[Num]=instance.parameters:getInteger("PeriodA" .. i);
			PeriodB[Num]=instance.parameters:getInteger("PeriodB" .. i);
			PeriodC[Num]=instance.parameters:getInteger("PeriodC" .. i);
			TF[Num]=  iTF[i];	
    ...
    assert(core.indicators:findIndicator("TOW LINES") ~= nil, "TOW LINES" .. " indicator must be installed");
			Temp= core.indicators:create("TOW LINES", source, PeriodA[j],PeriodB[j],PeriodC[j]);
			first = math.max( Temp.C:first(), Temp.B:first(), Temp.A:first())   
		   Source[i][j]= core.host:execute("getSyncHistory", Pair[i], TF[j], source:isBid(),first+1,20000 + ID , 10000 +ID);
		   loading [i][j]=true;
		   
		   Tow [i][j]= core.indicators:create("TOW LINES", Source[i][j], PeriodA[j],PeriodB[j],PeriodC[j]);
...
function AsyncOperationFinished(cookie)
 ...
 local ID=0;
	...
	if not FLAG and cookie== 1 then
		...
		Tow[i][j]:update(core.UpdateLast);   
...
function Draw(stage, context)
 ...
 function Calculate (context,i, j )
     ...
     if  not   Tow[i][j].A:hasData(Tow[i][j].A:size()-1)
     or  not  Tow[i][j].A:hasData(Tow[i][j].A:size()-2)  
	 or  not   Tow[i][j].B:hasData(Tow[i][j].B:size()-1)
	 or  not   Tow[i][j].C:hasData(Tow[i][j].C:size()-1)  
     then
     return;
     end
    ...
    local color=Neutral;	
	 ...
	 if Tow[i][j].A[Tow[i][j].A:size()-1] >  Tow[i][j].A[Tow[i][j].A:size()-2] then
	  Symbol="\233";
	  elseif Tow[i][j].A[Tow[i][j].A:size()-1] <  Tow[i][j].A[Tow[i][j].A:size()-2] then
	    Symbol="\234";
	 else
	     Symbol="\149";
	  end
	...
	if Mode== "AB" then	  
	    if Tow[i][j].A[Tow[i][j].A:size()-1] >  Tow[i][j].B[Tow[i][j].B:size()-1] then
	   color=UpColor;
	  elseif Tow[i][j].A[Tow[i][j].A:size()-1] <  Tow[i][j].B[Tow[i][j].B:size()-1] then
	    color=DownColor;
	  end
	  
	elseif Mode== "CA" then
	
	     if Tow[i][j].C[Tow[i][j].C:size()-1] >  Tow[i][j].A[Tow[i][j].A:size()-1] then
	    color=UpColor;
	    elseif Tow[i][j].C[Tow[i][j].C:size()-1] <  Tow[i][j].A[Tow[i][j].A:size()-1] then
	    color=DownColor;
        end		
		
	elseif Mode== "CB" then 
	     if Tow[i][j].C[Tow[i][j].C:size()-1] >  Tow[i][j].B[Tow[i][j].B:size()-1] then
	    color=UpColor;
	    elseif Tow[i][j].C[Tow[i][j].C:size()-1] <  Tow[i][j].B[Tow[i][j].B:size()-1] then
	    color=DownColor;
		end
	end
