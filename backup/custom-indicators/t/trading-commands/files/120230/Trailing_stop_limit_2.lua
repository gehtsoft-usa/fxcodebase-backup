...
function Init()
    ...
    strategy:description("v 3 2018-07-30");
...
function Prepare(nameOnly)
    ...
    source = ExtSubscribe(1, trade.Instrument, "t1", true);
    source_m1 = ExtSubscribe(2, trade.Instrument, "m1", trade.BS == "B" and true or false, "bar");

    for i = 1, 5 do
        local level = {};
        if instance.parameters:getBoolean("use_pips" .. i) then
            if trade.BS == "B" then
                level.rate = trade.Open + instance.parameters:getDouble("level_pips" .. i) * source:pipSize();
            else
                level.rate = trade.Open - instance.parameters:getDouble("level_pips" .. i) * source:pipSize();
            end
        else
        ...
        if action == "stop" or action == "stop_close" then
            ...
            if level.stop_in_pips then
                if trade.BS == "B" then
                    level.stop = trade.Open + instance.parameters:getDouble("stop_" .. i) * source:pipSize();
                else
                    level.stop = trade.Open - instance.parameters:getDouble("stop_" .. i) * source:pipSize();
                end
            else
...
function ExtUpdate(id, source, period)
    ...
    for _, level in ipairs(levels) do
        if core.crosses(source_m1.close, level.rate, source_m1:size() - 1) then
