...
return;
	...
	id =0;
	Draw (1, "Time Frame : " ..tostring( BS) , 10, Size, Placement, Label);
	
	local Color;
	if Net > 0 then
	Color=instance.parameters.Up;
	elseif Net < 0 then
	Color=instance.parameters.Dn;	
	end
	
	--Draw (2, "Net Change (Pips) "  , 10, Size, Placement, Label);
	Draw (2, tostring( string.format("%." .. 2 .. "f", Net)) .. " Pips" , 10, Size, Placement, Color);
	--Draw (4, "Percent Change (%) "  .." Pips"  , 10, Size, Placement, Label) ;
	Draw (3, tostring( string.format("%." .. 2 .. "f", Percent))  .." %"  , 10, Size, Placement, Color) 
...
function Draw (line, value , x, y, Pos, color)
	...
	core.host:execute("drawLabel1",id,  X, POZ["X"], Y, POZ["Y"], POZ["H"], POZ["V"],  font, color,   Out  );
