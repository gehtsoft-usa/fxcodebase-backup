...
-- https://fxcodebase.com/code/viewtopic.php?f=17&t=73624
...
function Init()
    ...
    indicator.parameters:addDouble("Level1", "1. Level", "", 35, 1, 2000);
    indicator.parameters:addDouble("Level2", "2. Level", "", 30, 1, 2000);	
...
-- Routine
 function Prepare(nameOnly)   
    ...
    local MxP=math.max(Period1,Period2,Period3);
	local MnP=math.min(Period1,Period2,Period3);
	if(MxP==Period1)then 
	MdP= math.max(Period2,Period3);
	elseif(MxP==Period2) then
	MdP=math.max(Period1,Period3);	
	else 
	MdP=math.max(Period2,Period1);
	end
  
	Indicator1= core.indicators:create("ADX", source, MnP);
	Indicator2= core.indicators:create("ADX", source, MdP);
	Indicator3= core.indicators:create("ADX", source, MxP);
	Indicator4= core.indicators:create("DMI", source, MnP);	
...
function Update(period, mode)
	...
	if(Indicator1.DATA[period-1]<Indicator1.DATA[period])then f1=true; end
	if(Indicator2.DATA[period-1]<Indicator2.DATA[period])then f2=true; end
	if(Indicator3.DATA[period-1]<Indicator3.DATA[period])then f3=true; end	
	  
	if(f1 and f2 and f3 and Indicator1.DATA[period]>Level1 and Indicator2.DATA[period]>Level2) then
    
	Signal[period]=1;	
	
			if Indicator4.DIP[period] > Indicator4.DIM[period]  and Signal[period-1] == 0 then		
			Bar[period]= 1;
			up:set(period, source.low[period], "\217");				
			elseif Indicator4.DIP[period] < Indicator4.DIM[period]  and Signal[period-1] == 0 then	
