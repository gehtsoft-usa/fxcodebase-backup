...
-- https://fxcodebase.com/code/viewtopic.php?f=17&t=72191
...
function Init()
    ...
    indicator.parameters:addInteger("Period1", "Fast MA", "", 8, 1, 2000);
    indicator.parameters:addInteger("Period2", "Mid MA", "", 40, 1, 2000);
    indicator.parameters:addInteger("Period3", "Slow MA", "", 200, 1, 2000);
...
-- Routine
 function Prepare(nameOnly)   
    ...
    Trend = instance:addStream("Trend", core.Bar, name, "Trend", instance.parameters.Up, first );
...
function Update(period, mode)
	...
	Trend:setColor(period,  instance.parameters.Neutral);	
	end
	
	Trend[period]= 
	((Indicator1.DATA[period] -  Indicator2.DATA[period])/source:pipSize())*(Period1/Period2)
	+
	((Indicator2.DATA[period] -  Indicator3.DATA[period])/source:pipSize())*(Period2/Period3);
 
	
	if Absolute then
	Trend[period]=math.abs(Trend[period]);
	end
