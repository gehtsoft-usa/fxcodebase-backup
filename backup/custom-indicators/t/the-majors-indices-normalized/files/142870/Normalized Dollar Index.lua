...
-- https://fxcodebase.com/code/viewtopic.php?f=17&t=71353
...
function Init()
    indicator:name("Normalized Dollar Index");
    indicator:description("Normalized Dollar Index");
...
function Prepare(nameOnly)
	...
	if Method == "Line" then
	Close = instance:addStream("Out", core.Line, "", label[5], instance.parameters.color1, first);
    ...
    Low = instance:addInternalStream(0, 0);
    Close = instance:addInternalStream(0, 0);
	 instance:createCandleGroup(label[5], label[5], Open, High, Low, Close);
	 end
	...
	return;
	 ...
	 local p={}
       ...
       p[2]= Initialization(period,2)	
        ...
        local close_min,close_max=mathex.minmax(close, period-Period+1, period);	
		Close[period]=(close[period]-close_min)/(close_max-close_min)/100;
		
	    if Method ~= "Line" then
		local high_min,high_max=mathex.minmax(high, period-Period+1, period);	
		local low_min,low_max=mathex.minmax(low, period-Period+1, period);	
		local open_min,open_max=mathex.minmax(open, period-Period+1, period);
		 
		
		Open[period]=(open[period]-open_min)/(open_max-open_min)/100;
		High[period]=(high[period]-high_min)/(high_max-high_min)/100;
		Low[period]=(low[period]-low_min)/(low_max-low_min)/100;
		end			 
