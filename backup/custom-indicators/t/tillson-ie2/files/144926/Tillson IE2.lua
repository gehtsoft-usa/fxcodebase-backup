...
function Update(period, mode)
    ...
    local ma = mathex.avg(source, period-length+1, period);
	a[period] = mathex.lreg(source, period-length+1, period);
    local m = a[period] -  a[period-1] + ma;
    Line[period] = (m + a[period]) / 2
	
	local slo = source[period] - Line[period];
