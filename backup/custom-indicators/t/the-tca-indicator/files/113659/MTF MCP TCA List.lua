...
-- http://fxcodebase.com/code/viewtopic.php?f=17&t=3393
...
function Init()
    ...
    indicator.parameters:addInteger("M", "Periods for Smooth", "", 7);
    indicator.parameters:addInteger("D", "Distance from center", "", 100);
    indicator.parameters:addInteger("C", "Coefficient percentage", "", 100);
	
	 indicator.parameters:addDouble("BL", "Buy Level", "", -20);
	 indicator.parameters:addDouble("SL", "Sell Level", "", 20);
	 ...
	 for i = 1, Count, 1 do	
      ...
      Indicator[i]={};
		   ...
		   for j = 1, Num, 1 do	
			...
			Temp= core.indicators:create("TCA", source  ,N,M,D,C );
		   ...
		   Indicator [i][j]= core.indicators:create("TCA", Source[i][j] , N,M,D,C);
...
function Draw(stage, context)
 ...
 function Calculate (context,i, j )
     ...
     if  not   Indicator[i][j].M:hasData(Indicator[i][j].M:size()-1)
     or  not   Indicator[i][j].M:hasData(Indicator[i][j].M:size()-2)  
	...
	yellow dot : otherwice.
		  ...
		  if Indicator[i][j].S [Indicator[i][j].S:size()-1] < BL 
		  and Indicator[i][j].T [Indicator[i][j].T:size()-1]> Indicator[i][j].M [Indicator[i][j].M:size()-1]
		  then
		  Symbol="\108"; 
		  SymbolColor=Up;
		  elseif Indicator[i][j].S [Indicator[i][j].S:size()-1] > SL 
		  and Indicator[i][j].T [Indicator[i][j].T:size()-1]< Indicator[i][j].M [Indicator[i][j].M:size()-1]
		  then
		  Symbol="\108"; 
		  SymbolColor=Down;
