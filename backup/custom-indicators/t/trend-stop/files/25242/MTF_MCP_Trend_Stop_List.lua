...
function Init()
	indicator:name("Multi Time Frame, Multi Currency Pairs Trend Stop List")
...
function AddTimeFrame(id, FRAME, DEFAULT)
	...
	indicator.parameters:addInteger("iPeriod" .. id, "Period", "Period", 10)
	indicator.parameters:addString("iType" .. id, "Triger" .. id, "", "Close")
	indicator.parameters:addStringAlternative("iType" .. id, "Close", "", "Close")
	indicator.parameters:addStringAlternative("iType" .. id, "High/Low", "", "High")
end
...
function Prepare(nameOnly)
	...
	for i = 1, 13, 1 do
		...
		if Use[i] then
			Num = Num + 1
			iPeriod[Num] = instance.parameters:getInteger("iPeriod" .. i)
	...
	for i = 1, Count, 1 do
		...
		for j = 1, Num, 1 do
			...
			Temp = core.indicators:create("TRENDSTOP", source, iPeriod[i], iType[i])
			first = Temp.DATA:first() * 2
			Source[i][j] = core.host:execute("getSyncHistory", Pair[i], TF[j], source:isBid(), first + 1, 20000 + ID, 10000 + ID)
			loading[i][j] = true

			Indicator[i][j] = core.indicators:create("TRENDSTOP", Source[i][j], iPeriod[i], iType[i])
		end
	end
