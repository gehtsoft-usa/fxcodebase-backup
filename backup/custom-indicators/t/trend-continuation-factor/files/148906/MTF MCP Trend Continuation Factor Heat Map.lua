...
-- https://fxcodebase.com/code/viewtopic.php?f=17&t=23551
...
--+------------------------------------------------------------------------------------------------+
 ...
 function Add(id, TF,Flag, Instrument )
	...
	indicator.parameters:addInteger("Period1".. id, "1. Period", "", 1, 1, 200);
	indicator.parameters:addInteger("Period2".. id, "2. Period", "", 35, 1, 200); 
end

 
 
function Init()
    indicator:name("MTF MCP Trend Continuation Factor Heat Map");
...
function Prepare(nameOnly)
	...
	assert(core.indicators:findIndicator("TCF") ~= nil, "Please, download and install TCF.LUA indicator");    
	...
	s2, e2 = core.getcandle(iTF[i], 0, 0, 0);
	 ...
	 Label[Number]="";
                ...
                Label[Number]=Label[Number] .. " - " ..  TF[Number];				
				 ...
				 loading[Number]  = true;  	 
				 Indicator[Number] = core.indicators:create("TCF", SourceData[Number].close ,  Period1[Number] ,  Period2[Number]);   
			    ...
			    for j= 1, Number , 1 do
				  ...
				  if p~= false then
										...
										if Indicator[j].DATA:hasData(p) and Indicator[j].DATA:hasData(p-1) then 
														...
														if Indicator[j].PTCF[p] > Indicator[j].NTCF[p] then		 
																if Indicator[j].PTCF[p] > Indicator[j].PTCF[p-1] then	
																C2=12;
																C1=11;
																else
																C2=22;
																...
																if Indicator[j].NTCF[p] > Indicator[j].NTCF[p-1] then	
																C2=32;
																C1=31;
																else
																C2=42;
