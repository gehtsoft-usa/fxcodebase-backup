...
function Init()
	...
	indicator.parameters:addInteger("AlertWidth", "Line width", "", 3, 1, 5);
...
function Prepare(nameOnly)
    ...
    assert(core.indicators:findIndicator(MS) ~= nil, MS .. " indicator must be installed");
        ...
        TTMS = instance:addStream("TTMS", core.Line, name, "TTMS", instance.parameters.Up, math.max(Period, NM, NB));
    ...
    Alert:setPrecision(math.max(2, instance.source:getPrecision()));
		Alert:setWidth(instance.parameters.AlertWidth);
        --Aert:setStyle(instance.parameters.AlertStyle);
    end
end
