...
function Init()
    indicator:name("Smoothed True MFI indicator");
...
function Prepare(nameOnly) 
	...
	PMA = core.indicators:create(Method, source.close, N);
...
function Update(period, mode)
    ...
    if period  < PMA.DATA:first() then
	return;
	end
	
	 PMA:update(mode);  
	     local rel=PMA.DATA[period]-PMA.DATA[period-3]+8*PMA.DATA[period-1]-8*PMA.DATA[period-2];
        if rel > 0 then
             pos =  source.volume[period] +2* source.volume[period-1]+2*source.volume[period-2]+source.volume[period-3] ;
        else 
             neg =   source.volume[period] +2* source.volume[period-1]+2*source.volume[period-2]+source.volume[period-3] ;
        end
