...
-- A: active; B: base
-- A: led; B: sine
-- N: wave look back; C: envelope width by average range 

function Init()
    indicator:name("RichardTaoDirectionalLine_envelope");
...
function Prepare()
    ...
    if oPara.pIND and oPara.pTHS then
        ...
        PRP[1].ln2 = instance:addStream("B", core.Line, name .. ".B", "B", instance.parameters.clrM, 0); 
        PRP[1].led = instance:addStream("A", core.Line, name .. ".A", "A", instance.parameters.clrB, 0); 
        PRP[1].sin = instance:addStream("B", core.Line, name .. ".B", "B", instance.parameters.clrP, 0); 
...
function Update(period, mode)
    ...
    and iperiod > oPara.pBGN then
        ...
        speriod = PRA.sperd;  
        PRA.reg[iperiod] = CalcEma(oPara.pNBK, PRA.reg[iperiod-1], SRS.close[speriod]);
        PRA.ln1[iperiod] = PRA.reg[iperiod]+PRA.sunit*oPara.pCHK;
        PRA.ln2[iperiod] = PRA.reg[iperiod]-PRA.sunit*oPara.pCHK;
        PRA.ledw1, PRA.sinw1 = CalcCsw(50, SRS, speriod);
        PRA.ledw2, PRA.sinw2 = CalcCsw(oPara.pNBK, SRS, speriod);
        PRA.led[iperiod] = PRA.reg[iperiod]+PRA.sunit*oPara.pCHK * PRA.ledw2;
        PRA.sin[iperiod] = PRA.reg[iperiod]+PRA.sunit*oPara.pCHK * PRA.sinw2;
    end
...
function CalcCsw(rPeriod, sourcep, iperiod)
    local tpval = 0;
    local realP = 0;
    local imagP = 0;
    local phase = 90;
    for i = 0, rPeriod-1 do
        tpval = (sourcep.close[iperiod-i]);
    ...
    return math.sin(math.rad(phase + 45)), math.sin(math.rad(phase));
end
