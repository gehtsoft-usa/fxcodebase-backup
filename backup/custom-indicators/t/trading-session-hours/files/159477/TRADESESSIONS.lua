...
-- https://fxcodebase.com/code/viewtopic.php?f=17&t=1974&p=159477#p159477
...
function Init()
    ...
    indicator.parameters:addString("calc_type", "Calculation type", "", "HL");
    indicator.parameters:addStringAlternative("calc_type", "High/Low", "", "HL");
    indicator.parameters:addStringAlternative("calc_type", "Open/Close", "", "OC");
    indicator.parameters:addStringAlternative("calc_type", "High/Close", "", "HC");
    indicator.parameters:addStringAlternative("calc_type", "Low/Close", "", "LC");
...
function Prepare(nameOnly)
    ...
    calc_type = instance.parameters.calc_type;
...
function Process(session, period)
    ...
    if date >= sfrom and date < sto_eod then
        ...
        if calc_type == "OC" then
            local from_index = core.findDate(ref, sfrom, false);
            local to_index = core.findDate(ref, sto, false) - 1;
            lo = math.min(ref.open[from_index], ref.close[to_index]);
            hi = math.max(ref.open[from_index], ref.close[to_index]);
        elseif calc_type == "HC" then
            local to_index = core.findDate(ref, sto, false) - 1;
            lo = ref.close[to_index];
        elseif calc_type == "LC" then
            local to_index = core.findDate(ref, sto, false) - 1;
            hi = ref.close[to_index];
        end
        if date == sfrom then
