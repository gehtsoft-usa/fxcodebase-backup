...
{
    {
        ...
        UpCondition = function(period)
            return Slow[period] < Fast[period] and Slow[period - 1] >= Fast[period - 1];
        end,
        ...
        FormatMessage = function(source, period, level, label, isUp)
            return string.format(
                ...
                price_source:instrument(), 
                source:barSize(), 
                win32.formatNumber(price_source.close[NOW], false, price_source:getPrecision()), 
...
function Init()
    indicator:name("Tick Two Averages Cross Alert (steveped)")
...
function Prepare(nameOnly)
    ...
    local name =
        ...
        "(" ..
            source:name() ..
                ", " .. Method1 .. ", " .. Period1 .. ", " .. Method2 .. ", " .. Period2 .. ")"
    instance:name(name)
    ...
    price_source = sources:Request(1, source, source:barSize());
