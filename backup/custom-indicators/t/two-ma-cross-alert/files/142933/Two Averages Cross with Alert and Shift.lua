...
function Init()
    indicator:name("Two Averages Cross Alert");
	...
	indicator.parameters:addInteger("H_Shift_Fast", "1. Line Shift period", "", 0);
	indicator.parameters:addInteger("H_Shift_Slow", "2. Line Shift period", "", 0);
	indicator.parameters:addInteger("V_Shift_Fast", "1. Line Vertical Shift (in Pips)", "", 0);
	indicator.parameters:addInteger("V_Shift_Slow", "2. Line Vertical Shift (in Pips)", "", 0);
...
local V_Shift_Fast, V_Shift_Slow, H_Shift_Fast, H_Shift_Slow, FIRST_Period_1, FIRST_Period_2  ;
-- Routine
function Prepare(nameOnly) 
	...
	H_Shift_Fast= instance.parameters.H_Shift_Fast;
	H_Shift_Slow= instance.parameters.H_Shift_Slow;
	V_Shift_Fast= instance.parameters.V_Shift_Fast;
	V_Shift_Slow= instance.parameters.V_Shift_Slow;

   
    source = instance.source;

    -- Base name of the indicator.
    local name = profile:id() .. "(" .. source:name().. ", " .. Type.. ", " .. Price1.. ", " .. Method1 .. ", " .. Period1 .. ", " .. Price2 .. ", " .. Method2 .. ", " .. Period2 .. ")";
    instance:name(name);
    ...
    fast = core.indicators:create("AVERAGES", source[Price1], Method1, Period1, false);
    slow = core.indicators:create("AVERAGES", source[Price2], Method2, Period2, false);

    first = math.max(slow.DATA:first(), fast.DATA:first());
	
	if  H_Shift_Fast > 0 then
	FIRST_Period_1= first + H_Shift_Fast;
	else
	FIRST_Period_1= first;
	end
	
	if  H_Shift_Slow > 0 then
	FIRST_Period_2= first + H_Shift_Slow;
	else
	FIRST_Period_2= first;
	end
	
	
	Fast = instance:addStream("Fast", core.Line, name .. ".Fast", "Fast", instance.parameters.color1,  FIRST_Period_1, H_Shift_Fast);
    Fast:setWidth(instance.parameters.width1);
    Fast:setStyle(instance.parameters.style1);
	
	
    Slow = instance:addStream("Slow", core.Line, name .. ".Slow", "Slow", instance.parameters.color2,  FIRST_Period_2, H_Shift_Slow);
...
function Calculate(period, mode)
	...
	local p= period+H_Shift_Fast;	
	if  H_Shift_Fast < 0 then	
	
	   if  period+H_Shift_Fast < FIRST_Period_1 then
	   return;
	   end
	
	end 
	Fast[p] = fast.DATA[period]+V_Shift_Fast*source:pipSize();

	local p= period+H_Shift_Slow;	
	if  H_Shift_Slow < 0 then	
	
	   if  period+H_Shift_Slow < FIRST_Period_2 then
	   return;
	   end
	
	end 
	
    Slow[p] = slow.DATA[period]+V_Shift_Slow*source:pipSize();
 end
...
function Activate(id, period)
    ...
    if id == 1 and ON[id] then
        if Slow[period_slow] < Fast[period_fast] and Slow[period_slow - 1] >= Fast[period_fast - 1] then
            if Type ~= "Short" then    
                up[id]:set(period, Slow[period_slow], "\108");
            end
            D[id] = nil;
        ...
        elseif Slow[period_slow] > Fast[period_fast] and Slow[period_slow - 1] <= Fast[period_fast - 1] then
            if Type ~= "Long" then                 
                down[id]:set(period, Slow[period_slow], "\108");
            end                           
            U[id] = nil;
...
function EmailAlert(label, Subjec )
