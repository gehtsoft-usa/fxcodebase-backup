...
--+------------------------------------------------------------------------------------------------+
 ...
 function Add(id, TF,Flag, Instrument )
	...
	indicator.parameters:addInteger("Period".. id, "Number of periods", "", 15, 1, 200);

	indicator.parameters:addString("MA_Method".. id, "MA Method", "Method" , "MVA");
    indicator.parameters:addStringAlternative("MA_Method".. id, "MVA", "MVA" , "MVA");
    indicator.parameters:addStringAlternative("MA_Method".. id, "EMA", "EMA" , "EMA");
     indicator.parameters:addStringAlternative("MA_Method".. id, "LWMA", "LWMA" , "LWMA");
    indicator.parameters:addStringAlternative("MA_Method".. id, "TMA", "TMA" , "TMA");
    indicator.parameters:addStringAlternative("MA_Method".. id, "SMMA", "SMMA" , "SMMA");
    indicator.parameters:addStringAlternative("MA_Method".. id, "KAMA", "KAMA" , "KAMA");
    indicator.parameters:addStringAlternative("MA_Method".. id, "VIDYA", "VIDYA" , "VIDYA");
    indicator.parameters:addStringAlternative("MA_Method".. id, "WMA", "WMA" , "WMA");    
end

 
 
function Init()
    indicator:name("MTF MCP TSR Big Trend Heat Map");
	...
	s2, e2 = core.getcandle(iTF[i], 0, 0, 0);
	 ...
	 Label[Number]="";
                ...
                Label[Number]=Label[Number] .. " - " ..  TF[Number];				
			  	...
			  	Temp1= core.indicators:create("TSR BIG TREND", source.close ,  Period[Number]);  				
				ifirst= Temp1.DATA:first()*2;
				 ...
				 loading[Number]  = true;  	 
				 Indicator[Number] = core.indicators:create("TSR BIG TREND", SourceData[Number].close ,  Period[Number] ,  MA_Method[Number]);   
			    ...
			    for j= 1, Number , 1 do
				  ...
				  if p~= false then
										...
										if Indicator[j].DATA:hasData(p) and Indicator[j].DATA:hasData(p-1) then 
														...
														if SourceData[j].close[p] > Indicator[j].DATA[p] then		 
