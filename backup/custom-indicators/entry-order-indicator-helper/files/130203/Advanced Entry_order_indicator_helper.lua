...
-- http://fxcodebase.com/code/viewtopic.php?f=17&t=65479
...
function Init()
    indicator:name("Advanced Entry_order_indicator_helper")
    indicator:description("Entry order indicator helper")
    indicator:requiredSource(core.Bar)
    indicator:type(core.Indicator)

    indicator.parameters:addInteger("distance", "Distance to Orders", "", 10)

    indicator.parameters:addInteger("stop_level", "Stop Level, in pips", "", 10)
    indicator.parameters:addBoolean("trailing_stop", "Trailing Stop Order", "", false)
    indicator.parameters:addInteger("trailing", "Trailing, in pips", "", 1)
    indicator.parameters:addInteger("limit_level", "Limit Level, in pips", "", 10)

    indicator.parameters:addBoolean("use_breakeven", "Use Breakeven", "", false);
    indicator.parameters:addDouble("breakeven_when", "Breakeven Activation Value, in pips", "", 10);
    indicator.parameters:addDouble("breakeven_to", "Breakeven To, in pips", "", 0);
    indicator.parameters:addString("breakeven_trailing", "Trailing after breakeven", "", "default");
    indicator.parameters:addStringAlternative("breakeven_trailing", "Do not change", "", "default");
    indicator.parameters:addStringAlternative("breakeven_trailing", "Set trailing", "", "set");
    indicator.parameters:addBoolean("ignore_mandatory_closing", "Ignore mandatory closing after breakeven", "", true);
    indicator.parameters:addBoolean("use_reentry", "Use re-entry", "", false);
    indicator.parameters:addDouble("reentry_pips", "Re-entry PL, pips", "", 15);
    indicator.parameters:addDouble("reentry_mult", "Re-entry lot multiplier", "", 1.5);

    indicator.parameters:addString("execute_type", "When to Execute", "", "command")
    indicator.parameters:addStringAlternative("execute_type", "Now", " ", "now")
    indicator.parameters:addStringAlternative("execute_type", "By Command", "", "command")
    indicator.parameters:addStringAlternative("execute_type", "By Time", " ", "time")
    indicator.parameters:addStringAlternative("execute_type", "By Date/Time", " ", "date")

    indicator.parameters:addString("execute_time", "Execute Time", "", "23:59:59")
    indicator.parameters:addDate("execute_date", "Execute Date", "", 0)
    indicator.parameters:setFlag("execute_date", core.FLAG_DATETIME);
    ...
    indicator.parameters:addInteger("remove_orders_timeout", "Remove Orders After, s", "", 60)
    indicator.parameters:addInteger("close_trades_timeout", "Close Trades After, s", "", 60)

    indicator.parameters:addString("account", "Account to trade on", "", "")
    indicator.parameters:setFlag("account", core.FLAG_ACCOUNT)
    indicator.parameters:addInteger("amount", "Trade Amount in Lots", "", 1)
    indicator.parameters:addInteger("ValidInterval", "Valid interval for operation in second", "", 60)

    indicator.parameters:addString("custom_id", "Custom ID", "", "42")
end

local EXECUTE_COMMAND_ID = 41
local distance
local stop_level
local trailing_stop
local trailing
local limit_level
local execute_type
local execute_time, execute_date
local remove_orders_timeout
local close_trades_timeout
local time
local amount
local account_id
local source
...
local use_breakeven, breakeven_when, breakeven_to, breakeven_trailing, ignore_mandatory_closing;
local use_reentry, reentry_pips, reentry_mult
function Prepare(nameOnly)
    ...
    use_reentry = instance.parameters.use_reentry;
    reentry_pips = instance.parameters.reentry_pips;
    reentry_mult = instance.parameters.reentry_mult;
    ...
    breakeven_trailing = instance.parameters.breakeven_trailing;
    ignore_mandatory_closing = instance.parameters.ignore_mandatory_closing;
    ...
    stop_level = instance.parameters.stop_level
    trailing_stop = instance.parameters.trailing_stop
    trailing = instance.parameters.trailing
    limit_level = instance.parameters.limit_level
    execute_type = instance.parameters.execute_type
    execute_time = instance.parameters.execute_time
    execute_date = instance.parameters.execute_date;
    remove_orders_timeout = instance.parameters.remove_orders_timeout
    close_trades_timeout = instance.parameters.close_trades_timeout
    time = ParseTime(instance.parameters.execute_time)
    account_id = instance.parameters.account
    ...
    core.host:execute("addCommand", EXECUTE_COMMAND_ID, "Execute orders")
...
function Update(period)
    for _, module in pairs(Modules) do 
        module:ExtUpdate(0, source, period);
    end
    if order1 ~= nil then
        if order1:GetTrade() ~= nil then
            trading:DeleteOrder(order2:GetOrder());
            order1 = nil;
            order2 = nil;
        elseif order2:GetTrade() ~= nil then
            trading:DeleteOrder(order1:GetOrder());
            order1 = nil;
            order2 = nil;
        end
    end
    if not executed and execute_type == "now" then
        ExecuteOrders()
    end
end
...
function TimeToAction(time)
    ...
    return now >= time and now < time + (ValidInterval / 86400.0)
end
...
function GetDate()
    local now = core.host:execute("getServerTime")
    return core.host:execute("convertTime", core.TZ_EST, ToTime, now)
end
...
function AsyncOperationFinished(cookie, success, message, message1, message2)
    ...
    if cookie == EXECUTE_COMMAND_ID then
        ExecuteOrders()
    elseif cookie == 1 then
        ...
        if not executed then
            if execute_type == "now" 
                or (execute_type == "time" and TimeToAction(time)) 
                or (execute_type == "date" and execute_date < GetDate())
            then
                ExecuteOrders()
            end
        else
            local now = core.host:execute("getServerTime")
            now = now - math.floor(now)
            local from = last_executed + remove_orders_timeout / 86400.0
            local to = from + ValidInterval / 86400.0
            if not removed and remove_orders_timeout > 0 and InRange(now, from, to) then
                removed = true
                trading:FindOrder()
                    :WhenInstrument(instance.source:instrument())
                    :WhenCustomID(custom_id)
                    :Do(
                        function (order)
                            if order.Type == "LE" or order.Type == "SE" then
                                trading:DeleteOrder(order);
                            end
                        end
                    );
            end
            local from = last_executed + close_trades_timeout / 86400.0
            local to = from + ValidInterval / 86400.0
            if not closed and close_trades_timeout > 0 and InRange(now, from, to) then
                closed = true
                ...
                while (row ~= nil) do
                    trading:FindTrade()
                        :WhenAccountID(row.AccountID)
                        :WhenInstrument(instance.source:instrument())
                        :WhenCustomID(custom_id)
                        :Do(function (trade)
                            if ignore_mandatory_closing then
                                for i, controller in ipairs(breakeven._controllers) do
                                    if controller.TradeID == trade.TradeID then
                                        if controller._executed then
                                            return;
                                        end
                                        break;
                                    end
...
function OpenMartingalePosition(side, lot, last_trade)
    local command = trading:MarketOrder(last_trade.Instrument)
        :SetSide(side)
        :SetAccountID(last_trade.AccountID)
        :SetPipLimit(nil, limit_level)
        :SetPipStop(nil, stop_level, trailing_stop and trailing or nil)
        :SetAmount(lot)
    ...
    if use_breakeven then
        local controller = breakeven:CreateController()
            ...
            :SetTo(breakeven_to);
        if breakeven_trailing == "set" then
            controller:SetTrailing(trailing_stop);
        end
    end
    return result;
end
...
function ExecuteOrders()
    ...
    last_executed = core.host:execute("getServerTime")
    last_executed = last_executed - math.floor(last_executed)
    
    order1 = trading:EntryOrder(source:instrument())
        :SetAccountID(account_id)
        :SetAmount(amount)
        :SetSide("B")
        :SetRate(source.close[NOW] + distance * source:pipSize())
        :SetPipLimit(nil, limit_level)
        :SetPipStop(nil, stop_level, trailing_stop and trailing or nil)
        :SetCustomID(custom_id)
        :Execute();

    order2 = trading:EntryOrder(source:instrument())
        :SetAccountID(account_id)
        :SetAmount(amount)
        :SetSide("S")
        :SetRate(source.close[NOW] - distance * source:pipSize())
    ...
    if use_breakeven then
        local controller = breakeven:CreateController()
            :SetRequestID(order1.RequestID)
            :SetWhen(breakeven_when)
            :SetTo(breakeven_to);
        if breakeven_trailing == "set" then
            controller:SetTrailing(trailing_stop);
        end
        controller = breakeven:CreateController()
            :SetRequestID(order2.RequestID)
            :SetWhen(breakeven_when)
            :SetTo(breakeven_to);
        if breakeven_trailing == "set" then
            controller:SetTrailing(trailing_stop);
    ...
    if use_reentry then
        breakeven:CreateMartingale(OpenMartingalePosition)
            :SetRequestID(order1.RequestID)
            :SetStep(-reentry_pips)
            :SetLotSizingValue(reentry_mult);
        breakeven:CreateMartingale(OpenMartingalePosition)
            :SetRequestID(order2.RequestID)
            :SetStep(-reentry_pips)
            :SetLotSizingValue(reentry_mult);
