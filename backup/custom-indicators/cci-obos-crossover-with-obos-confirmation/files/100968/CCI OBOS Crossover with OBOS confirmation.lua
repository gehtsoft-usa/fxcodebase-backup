...
-- http://fxcodebase.com/code/viewtopic.php?f=17&t=62318
...
function Init()
    indicator:name("CCI OBOS Crossover with OBOS confirmation");
...
function Prepare(nameOnly)
	...
	assert(core.indicators:findIndicator("OBOS") ~= nil, "Please, download and install OBOS.LUA indicator");     
	cci= core.indicators:create("CCI",  source,  CCI_Period);
    OBOS= core.indicators:create("OBOS",  source,  Period,UP_color, DN_color);	  
...
function Update(period,mode)
    ...
    if period < first  then
      ...
      and OBOS.open:colorI(period) == UP_color
	  then
	...
	and OBOS.open:colorI(period) == DN_color
	then 
