...
function CreateZigZag(stream, Depth, Deviation, Backstep, ZigC, ZagC)
    ...
    function zz:RegisterPeak(period, mode, peak)
		...
		if mode ~= 1 then
			TextBuff_Top:set(period, peak + V_Shift,
				TextFormat(period - second, self.Peak[period] - self.Peak[second], self.Peak[second] - self.Peak[third], 
					RegisterTicksVolume(second, period), "+ " .. string.format("%.4f", ((self.Peak[period] / self.Peak[second]) - 1) * 100), 
						"Highest = " .. string.format("%5." .. source:getPrecision() .. "f", self.Peak[period])));
		else
			TextBuff_Bottom:set(period, peak - V_Shift,
				TextFormat(period - second, self.Peak[second] - self.Peak[period], self.Peak[third] - self.Peak[second], 
					RegisterTicksVolume(second, period), "- " .. string.format("%.4f", (self.Peak[second] - self.Peak[period]) / self.Peak[second] * 100), 
						"Lowest = " .. string.format("%5." .. source:getPrecision() .. "f", self.Peak[period])));
		end
    end
