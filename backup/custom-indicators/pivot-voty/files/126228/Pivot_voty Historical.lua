...
-- http://fxcodebase.com/code/viewtopic.php?f=17&t=68434
...
function Init()
    ...
    indicator.parameters:addBoolean("historical_mode", "Show history", "", true);
...
function Prepare(nameOnly)
    historical_mode = instance.parameters.historical_mode;
    indi_alerts:Prepare();
...
function Update(period, mode)
    ...
    if historical_mode then
        core.host:trace(core.formatDate(tf_source:date(WeeklyBar)))
        local toIndex = core.findDate(source, tf_source:date(WeeklyBar + 1), false);
        local fromIndex = core.findDate(source, tf_source:date(WeeklyBar), false);
        for i = toIndex - 1, math.max(0, fromIndex), -1 do
            Pivot:setNoData(i);
            Res38:setNoData(i);
            Supp38:setNoData(i);
            Res61:setNoData(i);
