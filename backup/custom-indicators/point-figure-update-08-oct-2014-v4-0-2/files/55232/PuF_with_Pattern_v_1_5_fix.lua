function Init()
    indicator:name("P&F with Pattern on Chart indicator V1.5 fix");
    indicator:description("Point & Figure indicator V1.5 fix");
	...
	indicator.parameters:addGroup("TrendLines");
	indicator.parameters:addBoolean("TrendLines", "Show TrendLines", "Show Trendlines" ,  true);
	indicator.parameters:addColor("Tup", "Color of Up TrendLine", "", core.rgb( 0, 64, 0));
	indicator.parameters:addColor("Tdn", "Color of Down TrendLine", "", core.rgb( 128, 0, 0));
...
local Tdn,TdnS;
function Prepare()
	TrendLines		= instance.parameters.TrendLines;
    ...
    SigTXT = instance:createTextOutput("Sig", "Sig", "Consolas", 10, core.H_Center, core.V_Center, instance.parameters.TXT);
	if TrendLines then
		Tup 	= instance:createTextOutput("Bearish Resistance Line", "Bearish Resistance Line", "Consolas", 10, core.H_Center, core.V_Center, instance.parameters.Tup);
		Tdn 	= instance:createTextOutput("Bullish Support Line", "Bullish Support Line", "Consolas", 10, core.H_Center, core.V_Center, instance.parameters.Tdn);
		Up["Breakout"]    = instance:addStream("UpBreakout", core.Bar,     name, "", core.rgb(0, 255, 0), first);
		Down["Breakout"]  = instance:addStream("DownBreakout", core.Bar,   name, "", core.rgb(0, 255, 0), first);	
		instance:createFromToBarGroup ("", "BreakoutSignal", Up["Breakout"], Down["Breakout"], Bull);
...
function Update(period, mode)
	if mode == core.UpdateAll then
		drwText("Reset Indicator recieving more data",1,10,00,instance.parameters.TXT,sF);
		initFirst = true;
		return;
	end
	if (period < first) or (period ~= ( source:size()-1 )) then
		drwText("Waiting for loading history finished",1,10,00,instance.parameters.TXT,sF);
		return;
	end
    ...
    if Type == "ATR" and TEST  then
        ...
        if not ATR.DATA:hasData(period) then
			drwText("ATR has no data",1,10,00,instance.parameters.TXT,sF);
            return;
		end
        TEST = false;
	...
	-- New Code ----------------------------------------------------------------------------------------------
	if 	mode == core.UpdateLast and period >= first+3 then
		if initFirst then
...
function drawDebug(i)
	if not Debug then return; end
            ...
            local TXTs = "";
			local fDigits = "%." .. tostring(source:getPrecision()-1) .. "f";
			if Type == "Pips" then
            ...
            if pf.swing == "Up" then
                ...
                TXTs = TXTs ..  " Column has    : " .. rows .. " of Xs" .. "\n" .. 
                                " Flag          : " .. pf.flag  .. "\n" .. 
                                " Column Close  : " .. string.format(fDigits,PFClose) ..  "\n" ..
								" Next UP       : " .. string.format(fDigits,PFClose + BS*point) ..    " Pips to Next : " .. string.format("%.1f",((PFClose + BS*point) - source.close[i])/point) .. "\n" ..
                                " Next RevDown  : " .. string.format(fDigits,PFClose - BS*point*RS) .. " Pips to Next : " .. string.format("%.1f",(source.close[i] - (PFClose - BS*point*RS))/point) .. "\n";
            end
            if pf.swing == "Down" then
                ...
                TXTs = TXTs ..  " Column has    : " .. rows .. " of Os" .. "\n" .. 
                                " Flag          : " .. pf.flag  .. "\n" .. 
                                " Column Close  : " .. string.format(fDigits,PFClose) ..  "\n" ..
								" Next DN       : " .. string.format(fDigits,PFClose - BS*point) ..    " Pips to Next : " .. string.format("%.1f",(source.close[i] - (PFClose - BS*point))/point) .. "\n" ..
                                " Next RevUp    : " .. string.format(fDigits,PFClose + BS*point*RS) .. " Pips to Next : " .. string.format("%.1f",((PFClose + BS*point*RS) - source.close[i])/point) .. "\n";
            end
                TXTs = TXTs ..  " Source Low    : " .. string.format(fDigits,source_low[i])  ..   "\n" ..
                                " Source High   : " .. string.format(fDigits,source_high[i]) ..   "\n" ..
...
function drawTrendLines(period)
	if not TrendLines then return; end
	local z;
	if pf.period <= 10 then
		return;
	end
	local up={startPoint1 = nil,startPoint2=nil,Level=nil};
	local dn={startPoint1 = nil,startPoint2=nil,Level=nil};
	for z=period-pf.period+1,period,1 do
		if close[z] == nil then return; end
		Tup:setNoData(z);
		Tdn:setNoData(z);
		--TupS:setNoData(z);
		...
		Up["Breakout"][z]=nil;
		Down["Breakout"][z]=nil;
		if open[z] > close[z-1] then -- X Candle 
			-- Downside Breakout Below a Bullish Support Line  --------------------------------------------------------------
			if up.startPoint1 ~= nil and z ~= up.startPoint1 and up.startPoint2 ~= nil and up.startPoint2 ~= z and up.Level ~= nil then
				up.Level = up.Level + BS*point;
				up.startPoint2 = z;
				Tup:set(z,up.Level,"+","Up TrendLine");
			end
			-- Downside Breakout Below a Bullish Support Line  --------------------------------------------------------------
			-----------------------------------------------------------------------------------------------------------------
			-----------------------------------------------------------------------------------------------------------------
			-- Upside Breakout above a Bearish Resistance Line --------------------------------------------------------------
			if dn.startPoint1 == nil then
				dn.startPoint1 = z;
				dn.Level = close[z] + BS*point;
			elseif dn.startPoint1 ~= nil and z ~= dn.startPoint1 and dn.startPoint2 == nil and dn.Level ~= nil then
				dn.Level = dn.Level - BS*point;
				if  close[z] < dn.Level then
					dn.startPoint2 = z;
					local i;
					dn.Level = close[dn.startPoint1] + 2*BS*point;
					for i = dn.startPoint1,dn.startPoint2,1 do
						dn.Level = dn.Level - BS*point;
						Tdn:set(i, dn.Level,"+","Dn TrendLine");
					end
				else
					dn.startPoint1 = z;
					dn.startPoint2 = nil;
			...
			elseif dn.startPoint1 ~= nil and dn.startPoint2 ~= nil and dn.startPoint2 ~= z and dn.Level ~= nil then
				dn.startPoint2 = z;
				dn.Level = dn.Level - BS*point;
				if close[z] <= dn.Level then
					Tdn:set(z,dn.Level,"+","Dn TrendLine");
				elseif close[z] > dn.Level then
					-- TrendLine Break Sell Signal
					Tdn:set(z,dn.Level,"B","Dn TrendLine Break Buy Signal");
					Up["Breakout"][z] = dn.Level + BS*point;
                    Down["Breakout"][z] = dn.Level - BS*point;
                    Up["Breakout"]:setColor(z,Bull);
                    Down["Breakout"]:setColor(z,Bull);
					dn.startPoint1 = nil;
					dn.startPoint2 = nil;
					dn.Level = nil;
				end
			...
			-- Upside Breakout above a Bearish Resistance Line --------------------------------------------------------------
			-----------------------------------------------------------------------------------------------------------------
		elseif open[z] < close[z-1]  then -- O Candle
			-- Upside Breakout above a Bearish Resistance Line --------------------------------------------------------------
			if dn.startPoint1 ~= nil and dn.startPoint2 ~= nil and dn.startPoint2 ~= z and dn.Level ~= nil then
				dn.Level = dn.Level - BS*point;
				dn.startPoint2 = z;
				Tdn:set(z,dn.Level,"+","Dn TrendLine");
			end
			...
			-----------------------------------------------------------------------------------------------------------------
			-- Downside Breakout Below a Bullish Support Line ---------------------------------------------------------------
			if up.startPoint1 == nil then
				up.startPoint1 = z;
				up.Level = close[z] - BS*point;
			elseif up.startPoint1 ~= nil and z ~= up.startPoint1 and up.startPoint2 == nil and up.Level ~= nil then
				up.Level = up.Level + BS*point;
				if  close[z] > up.Level  then
					up.startPoint2 = z;
					local i;
					up.Level = close[up.startPoint1] - 2*BS*point;
					for i = up.startPoint1,up.startPoint2,1 do
						up.Level = up.Level + BS*point;
						Tup:set(i, up.Level,"+","Dn TrendLine");
					end
				else
					up.startPoint1 = z;
					up.startPoint2 = nil;
			...
			elseif up.startPoint1 ~= nil and up.startPoint2 ~= nil and up.startPoint2 ~= z and up.Level ~= nil then
				up.startPoint2 = z;
				up.Level = up.Level + BS*point;
				if close[z] >= up.Level then
					Tup:set(z,up.Level,"+","Dn TrendLine");
				elseif close[z] < up.Level then
					-- TrendLine Break Sell Signal
					Tup:set(z,up.Level,"S","Up TrendLine Break Sell Signal");
					Up["Breakout"][z] = up.Level + BS*point;
                    Down["Breakout"][z] = up.Level - BS*point ;
                    Up["Breakout"]:setColor(z,Bear);
                    Down["Breakout"]:setColor(z,Bear);
					up.startPoint1 = nil;
					up.startPoint2 = nil;
					up.Level = nil;
				end
