...
function Init()
	...
	indicator.parameters:addInteger("LookBack", "Look Back Period", "", 0);
    ...
    for i = 1, 13,1 do
        ...
        indicator.parameters:addBoolean("ShowO"..i , "Show Open " ..  TF[i] .. " Frame", "", false);
        ...
        indicator.parameters:addColor("Open"..i, "Open Color", "", core.rgb(0, 0, 0));
 ...
 function Prepare(nameOnly) 
    ...
    for j = 1, 13, 1 do
        ...
        if ((e-s)< (e1-s1)) and On[j] then                
            ...
            ShowO[Num] = instance.parameters:getBoolean("ShowO" .. j);
            ...
            Open[Num] = instance.parameters:getDouble("Open" .. j);
...
function Draw(stage, context )
    ...
    First=math.max(source:first(), context:firstBar (), source:size()-1 - LookBack);
	end
    ...
    for j = 1, Num, 1 do
        ...
        if Now~= false then
            if  Type~= "Labels"  then
                ...
                if ShowO[j] then
                    if Method ~= "Previous" then
                        core.host:execute("drawLine", Id(), s1, SourceData[j].open[Now], e1, SourceData[j].open[Now],Open[j], CurrentStyle[j], width[j]);
                    end
                    if Method ~= "Current" then
                        core.host:execute("drawLine", Id(), s1, SourceData[j].open[Now-1], e1, SourceData[j].open[Now-1],Open[j], PreviousStyle[j], width[j]);
                    end
                end
                if Show[j] then
                    if Method ~= "Previous" then
