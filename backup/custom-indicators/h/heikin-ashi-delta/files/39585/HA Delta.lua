...
function Init()
	...
	indicator.parameters:addInteger("Shift", "Shift", "", 1, 0, 1000);
...
function Prepare(nameOnly)
   ...
   Shift=instance.parameters.Shift;
    ...
    local name = profile:id() .. "(" .. source:name().. ", " .. Shift  .. ")";
    instance:name(name);	
	

    if (not (nameOnly)) then
	    HA= core.indicators:create("HA", source);
        Delta = instance:addStream("Delta", core.Line, name .. ".Delta", "Delta",instance.parameters.Delta,  source:first()  + Shift);
...
function Update(period, mode)
		 ...
		 if period < HA.DATA:first()  + Shift then  
		 return;
		 end
		 
	     Delta[period]= HA.close[period]- HA.open[period-Shift];	 
