...
function Init()
    ...
    indicator.parameters:addBoolean("OC", "Show open/close band", "Choose yes to show open/close band instead of high/low band", false);
...
function Prepare()
    ...
    local name = profile:id() .. "(" .. source:name() .. "," .. BS .. "," .. YTDn .. "," .. OCn .. ")";
    instance:name(name);
...
function Update(period, mode)
    ...
    if OC then
        H[period] = math.max(hilo_data.open[hilo_i], hilo_data.close[hilo_i]);
        L[period] = math.min(hilo_data.open[hilo_i], hilo_data.close[hilo_i]);
    else
        ...
        if candles:hasData(period - 1) and candles[period - 1] == hilo_candle and
           ((H[period - 1] ~= H[period]) or (L[period - 1] ~= L[period]))  then
            ...
            while i > 0 and candles:hasData(i) and candles[i] == hilo_candle do
                if OC then
                    H[i] = math.max(hilo_data.open[hilo_i], hilo_data.close[hilo_i]);
                    L[i] = math.min(hilo_data.open[hilo_i], hilo_data.close[hilo_i]);
                else
