...
function Init()
    indicator:name("Oscillator High Volume Bar Breakout");
   ...
   indicator.parameters:addInteger("Number", "Indicator Output Number", "", 1,1,100);	
...
-- Routine
 function Prepare(nameOnly)   
	...
	Indicator = iprofile:createInstance(source, iparams);
     ...
     first=Indicator:getStream (Number):first()+Period;
							   
	Count= (Indicator:getStreamCount ()-1); 
 	if Number >  Count   then
	error(INDICATOR .. " Only Have " .. (Count+1) .." Output Streams" );
	Number=Count;
    end
...
function Update(period, mode)
    ...
    local min1, max1= mathex.minmax(source.volume, period-Period+1, period);
    local min2, max2= mathex.minmax(Indicator:getStream (Number), period-Period+1, period);	
    ...
    if max1==source.volume[period] then 
	High[period]=max2;
	Low[period]=min2;		
	end
