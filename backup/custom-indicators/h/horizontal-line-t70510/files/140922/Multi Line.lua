...
function Draw(stage, context)
	...
	Value = DrawLine(context, source[last], source[source:size() - 1], last);
	core.host:execute("setStatus", Value)
	for i = 1, lines, 1 do
		DrawLine(context, source[last] + distance * source:pipSize() * i, source[source:size() - 1] + distance * source:pipSize() * i, last);
		DrawLine(context, source[last] - distance * source:pipSize() * i, source[source:size() - 1] - distance * source:pipSize() * i, last);
	end
end
