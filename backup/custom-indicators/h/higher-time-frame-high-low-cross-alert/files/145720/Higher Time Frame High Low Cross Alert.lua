...
-- https://fxcodebase.com/code/viewtopic.php?f=17&t=72096
...
function Init()
   ...
   indicator:requiredSource(core.Bar);
	...
	indicator.parameters:addInteger("Delta", "Delta (In Pips)", "", 10);
...
function Signal_Logic (id, period)
	    ...
	    if id== 1  then   
			if  source.close[period] > (SourceData.high[p-1]+Delta*source:pipSize())
            and  source.close[period-1] <= (SourceData.high[p-1]+Delta*source:pipSize())
			then
			...
			elseif  source.close[period] < (SourceData.high[p-1] + Delta*source:pipSize())
            then			
		...
		if id== 2   then  
			if  source.close[period] < (SourceData.low[p-1] - Delta*source:pipSize())
            and  source.close[period-1] >= (SourceData.low[p-1] -Delta*source:pipSize())
			then
			...
			elseif source.close[period] > (SourceData.low[p-1] - Delta*source:pipSize())
            then			
