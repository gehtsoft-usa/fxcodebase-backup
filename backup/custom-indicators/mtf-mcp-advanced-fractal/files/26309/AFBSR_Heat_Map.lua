...
--+------------------------------------------------------------------------------------------------+
 ...
 function Add(id, TF,Flag, Instrument )
	...
	indicator.parameters:addInteger("Period".. id, "Number of periods", "", 5, 1, 200);
	...
	s2, e2 = core.getcandle(iTF[i], 0, 0, 0);
	 ...
	 Label[Number]="";
                ...
                Label[Number]=Label[Number] .. " - " ..  TF[Number];				
				 ...
				 loading[Number]  = true;  	 
				 Indicator[Number] = core.indicators:create("AFBSR",  SourceData[Number] ,  Period[Number]);  	
			   ...
			   x0, x1, x2 = context:positionOfBar (i);
			    ...
			    for j= 1, Number , 1 do
				  ...
				  if p~= false then
					 ...
					 Support,Resistance = Last( j, p );			
										if Indicator[j].DATA:hasData(p) and Indicator[j].DATA:hasData(p-1) and Support~= nil  and Resistance ~= nil then 
										 
										 
										
										 
										 
												    if Support  > Resistance then	 
														 
															C2=12;
															C1=11;
															 	
												 
												    elseif Support  < Resistance then  
													        C2=22;
															C1=21;
													else
                                                            C2=32;
...
for i = p , Period[j] , -1 do


        if Indicator[j].R:hasData(i) and Indicator[j].S:hasData(i) then
