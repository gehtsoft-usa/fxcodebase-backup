...
--+------------------------------------------------------------------+
function Init()
    indicator:name("William Blau Ergodic MDI oscillator");
    indicator:description("William Blau Ergodic MDI oscillator");
...
function Prepare(nameOnly)  
    ...
    Blau_EMDI = instance:addStream("Blau_EMDI", core.Bar, name .. ".Blau_EMDI", "Blau_EMDI", instance.parameters.clr, EMA3.DATA:first());
    Signal_EMA = core.indicators:create("EMA", Blau_EMDI, Signal_Period);
	...
	Blau_EMDI:setPrecision(math.max(2, instance.source:getPrecision()));
...
function Update(period, mode)
   if period>first then
    ...
    EMA2:update(mode);
    EMA3:update(mode);
    Blau_EMDI[period]=EMA3.DATA[period];
