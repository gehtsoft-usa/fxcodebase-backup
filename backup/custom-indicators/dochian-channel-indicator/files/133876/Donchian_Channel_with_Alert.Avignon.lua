...
-- http://fxcodebase.com/code/viewtopic.php?f=17&t=20&start=60
...
{ 
    {
		...
		UpCondition = function (period)
			...
			return core.crossesOver(source.close, Out[1], period) and not instance.parameters.current_period;
        end,
		DownCondition = function (period)
			if period < 1 then
				return false;
			end
			return core.crossesUnder(source.close, Out[1], period);
        end,
        OnChange = false,
        Name = "Upper Outer Line"
	},
	{
        Stage = 2,
		UpCondition = function (period)
			...
			return core.crossesOver(source.close, Out[2], period) and not instance.parameters.current_period;
        end,
		DownCondition = function (period)
			if period < 1 then
				return false;
			end
			return core.crossesUnder(source.close, Out[2], period);
        end,
        OnChange = false,
        Name = "Upper Inner Line"
    },
	{
        Stage = 2,
		UpCondition = function (period)
			...
			return core.crossesOver(source.close, Out[3], period);
        end,
		DownCondition = function (period)
			if period < 1 then
				return false;
			end
			return core.crossesUnder(source.close, Out[3], period);
        end,
        OnChange = false,
        Name = "Central Line"
    },
	{
		...
		UpCondition = function (period)
			...
			return core.crossesOver(source.close, Out[4], period);
        end,
		DownCondition = function (period)
			if period < 1 then
				return false;
			end
			return core.crossesUnder(source.close, Out[4], period) and not instance.parameters.current_period;
        end,
        OnChange = false,
        Name = "Lower Inner Line"
    },
	{
        Stage = 2,
		UpCondition = function (period)
			...
			return core.crossesOver(source.close, Out[5], period);
        end,
		DownCondition = function (period)
			if period < 1 then
				return false;
			end
			return core.crossesUnder(source.close, Out[5], period) and not instance.parameters.current_period;
        end,
        OnChange = false,
        Name = "Lower Outer Line"
    }
};
...
function Init()
    ...
    indicator.parameters:addBoolean("current_period", "Analyse the current period", "", true);
