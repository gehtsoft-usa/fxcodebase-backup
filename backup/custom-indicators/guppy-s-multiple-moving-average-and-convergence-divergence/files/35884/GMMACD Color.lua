...
function Init()
	...
	indicator.parameters:addGroup("Level Lines  Style");
	indicator.parameters:addBoolean("Last", "Use Last Value Line", "", false);
    indicator.parameters:addColor("color", "Color of Upper Band Line", "", core.rgb(255, 0, 0));
	indicator.parameters:addInteger("width", "Line Width", "", 1, 1, 5);
	 indicator.parameters:setFlag("width", core.FLAG_LEVEL_STYLE);
...
function Update(period, mode)
	...
	return;
		...
		return;
		 ...
		 if Last and  period == source:size()-1 then		
		 core.host:execute("drawLine", 1, source:date(first), out[period], source:date(period-1), out[period], instance.parameters.color, instance.parameters.width, instance.parameters.style);
		 end
