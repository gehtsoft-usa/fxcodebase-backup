...
function Init()
    indicator:name("MTF MCP Detrended Ehlers Leading Indicator Heat Map");
	...
	s2, e2 = core.getcandle(iTF[i], 0, 0, 0);
	 ...
	 Label[Number]="";
                ...
                Label[Number]=Label[Number] .. " - " ..  TF[Number];				
			  	...
			  	Temp1= core.indicators:create("DETRENDED EHLERS LEADING INDICATOR", source ,  Period[Number]);  				
				ifirst= Temp1.DATA:first()*2;
				 ...
				 loading[Number]  = true;  	 
				 Indicator[Number] = core.indicators:create("DETRENDED EHLERS LEADING INDICATOR", SourceData[Number] ,  Period[Number]);   
			   ...
			   x0, x1, x2 = context:positionOfBar (i);
			    ...
			    for j= 1, Number , 1 do
				  ...
				  if p~= false then
										...
										if Indicator[j].Line1:hasData(p) and Indicator[j].Line1:hasData(p-1) 
										and Indicator[j].Line2:hasData(p) and Indicator[j].Line2:hasData(p-1) 
										then 
										
										
										            Delta1= Indicator[j].Line1[p] -Indicator[j].Line2[p];  
										            Delta2= Indicator[j].Line1[p-1] -Indicator[j].Line2[p-1]; 
													
												    if Indicator[j].Line1[p] > Indicator[j].Line2[p] then		 
