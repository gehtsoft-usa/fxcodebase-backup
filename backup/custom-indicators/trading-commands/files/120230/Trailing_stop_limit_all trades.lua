...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=65947
-- Id: 
...
function Init()
    strategy:name("Trailing Stop/Limit");
    strategy:description("v 34 2018-08-02");
    ...
    strategy.parameters:addBoolean("monitor_mode", "Monitor all new trades", "", false);
...
function AddSL(i)
    ...
    strategy.parameters:addString("action" .. i, "Action", "", "no");
    strategy.parameters:addStringAlternative("action" .. i, "No action", "", "no");
    strategy.parameters:addStringAlternative("action" .. i, "Partial close", "", "close");
    strategy.parameters:addStringAlternative("action" .. i, "Move stop", "", "stop");
    strategy.parameters:addStringAlternative("action" .. i, "Partial close and move stop", "", "stop_close");
    strategy.parameters:addBoolean("use_pips" .. i, "Use level in pips", "", false);
    strategy.parameters:addDouble("level_pips" .. i, "Level", "Negative for stops", (i + 1) * 5);
    strategy.parameters:addInteger("amount_" .. i, "Amount for Partial Close", "", 1);
    strategy.parameters:addString("amount_size_selector_" .. i, "Amount Type Selector", "", "Lots");
    strategy.parameters:addStringAlternative("amount_size_selector_" .. i, "Lots", "", "Lots");
    strategy.parameters:addStringAlternative("amount_size_selector_" .. i, "Percent Of Trade", "", "Percent");
    strategy.parameters:addBoolean("stop_in_pips" .. i, "Use stop in pips", "", false);
    strategy.parameters:addDouble("stop_" .. i, "Stop level", "", i * 5);
    strategy.parameters:addString("trailing_" .. i, "Trailing stop order", "", "no");
    strategy.parameters:addStringAlternative("trailing_" .. i, "No trailing", "", "no");
    strategy.parameters:addStringAlternative("trailing_" .. i, "Dynamic", "", "dynamic");
    strategy.parameters:addStringAlternative("trailing_" .. i, "Fixed", "", "fixed");
    strategy.parameters:addInteger("trailing_value_" .. i, "Fixed trailing in pips", "10 or greater", 10, 10, 10000);
end
...
function CreateLevels(controller, trade)
    controller.levels = {};
    for i = 1, 5 do
        local level = {};
        if instance.parameters:getBoolean("use_pips" .. i) then
            if trade.BS == "B" then
                level.rate = trade.Open + instance.parameters:getDouble("level_pips" .. i) * source_m1[trade.Instrument]:pipSize();
            else
                level.rate = trade.Open - instance.parameters:getDouble("level_pips" .. i) * source_m1[trade.Instrument]:pipSize();
            end
        else
            level.rate = instance.parameters:getDouble("level_pips" .. i);
        end
        local action = instance.parameters:getString("action" .. i);
        if action == "close" or action == "stop_close" then
            if instance.parameters:getString("amount_size_selector_" .. i) == "Lots" then
                level.amount = instance.parameters:getInteger("amount_" .. i) * core.host:execute("getTradingProperty", "baseUnitSize", trade.Instrument, trade.AccountID);
            else
                level.amount = trade.Lot * instance.parameters:getInteger("amount_" .. i)/100;
            end
        else
            level.done = true;
            level.close_command = {};
            level.close_command.Finished = true;
        end

        if action == "stop" or action == "stop_close" then
            level.stop_in_pips = instance.parameters:getBoolean("stop_in_pips" .. i);
            if level.stop_in_pips then
                if trade.BS == "B" then
                    level.stop = trade.Open + instance.parameters:getDouble("stop_" .. i) * source_m1[trade.Instrument]:pipSize();
                else
                    level.stop = trade.Open - instance.parameters:getDouble("stop_" .. i) * source_m1[trade.Instrument]:pipSize();
                end
            else
                level.stop = instance.parameters:getDouble("stop_" .. i);
            end
            local trailing = instance.parameters:getDouble("stop_" .. i);
            if trailing == "dynamic" then
                level.stop_trailing = 1;
            elseif trailing == "fixed" then
                level.stop_trailing = instance.parameters:getDouble("trailing_value_" .. i);
            end
        else
            level.stop_moved = true;
        end
        controller.levels[#controller.levels + 1] = level;
    end
end

function CreateController(trade_id)
    core.host:trace("Start monitoring of " .. trade_id);
    local controller = {};
    controller.trade_id = trade_id;
    controllers[#controllers + 1] = controller;
end

local TRADES_UPDATE = 1;
local OFFERS_UPDATE = 2;
local last_index = 2;
function Prepare(nameOnly)
    ...
    core.host:execute("subscribeTradeEvents", OFFERS_UPDATE, "offers");
    monitor_mode = instance.parameters.monitor_mode;
    if not monitor_mode then
        trade_id = instance.parameters.Trade;
        local trade = core.host:findTable("trades"):find("TradeID", trade_id);
        assert(trade ~= nil, "Trade can not be found")
        CreateController(trade_id);
    else
...
function ExtAsyncOperationFinished(cookie, success, message, message1, message2)
    ...
    elseif cookie == OFFERS_UPDATE then
        for _, controller in ipairs(controllers) do
            local trade = core.host:findTable("trades"):find("TradeID", controller.trade_id);
            if trade ~= nil then
                if source_m1[trade.Instrument] == nil then
                    source_m1[trade.Instrument] = ExtSubscribe(last_index + 1, trade.Instrument, "m1", trade.BS == "B" and true or false, "close");
                    last_index = last_index + 1;
                end
                if controller.levels == nil then
                    CreateLevels(controller, trade);
                end
                if source_m1[trade.Instrument]:size() > 0 then
                    for _, level in ipairs(controller.levels) do
                        if core.crosses(source_m1[trade.Instrument], level.rate, NOW) then
                            if level.done == nil then
                                level.done = true;
                                level.close_command = trading:ParialClose(trade, level.amount);
                            elseif level.stop_moved == nil and level.close_command.Finished then
                                core.host:trace("Moving to ".. level.stop .. " after cross of " .. level.rate);
                                level.stop_moved = true;
                                trading:MoveStop(trade, level.stop, level.stop_trailing);
                            end
                        end
                    end
                end
