...
function Init()
    indicator:name("MTF MCP Strength Index");
...
-- Routine
 function Prepare(nameOnly)   
	...
	loading[j]={};
	   ...
	   for i = 1, Count, 1 do		
		 k=k+1;
		 SourceData[j][i] = core.host:execute("getSyncHistory", List[i], TF[j], source:isBid(), 1 , 200+k , 100+k);
		 loading[j][i] = true;  	 
...
local iCount=0;
	...
	for i = 1, 8, 1 do
		...
		iCount=iCount+1;
	 	...
	 	core.host:execute("drawLabel1", id, Size*5+(iCount-1)*Size*5 ,  core.CR_LEFT, Size  +Shift, core.CR_TOP, core.H_Left, core.V_Center, Bold, Color,  Pair[i]);	
       ...
       local Data;
		...
		Data= Num[i];	
			...
			core.host:execute("drawLabel1", id, Size*5+(iCount-1)*Size*5 ,  core.CR_LEFT, Size*2+(j-1)* Size  +Shift, core.CR_TOP, core.H_Left, core.V_Center, Bold, Color,  	string.format("%." .. 2 .. "f", Data)	);			  
			id = id+1;	
			 
		end	
	end	
		
			core.host:execute("drawLabel1", id, Size*2 ,  core.CR_LEFT, Size*2+ (j-1)*Size , core.CR_TOP, core.H_Left, core.V_Center, Bold, Color,  TF[j]);	
			id = id+1;	
        ...
        if On[k] and SourceData[j][i]["close"]:hasData(SourceData[j][i]["close"]:size()-1) then
				 ...
				 crncy1, crncy2 = string.match(List[i], pauto);
					...
					if crncy1 == Pair[k] then
							...
							Num[k] = Num[k] + (( math.abs(SourceData[j][i]["close"][SourceData[j][i]["close"]:size()-1] -  SourceData[j][i]["open"][SourceData[j][i]["open"]:size()-1])) /PS[i]);
							else
							Num[k] =  Num[k] - (( math.abs(SourceData[j][i]["close"][SourceData[j][i]["close"]:size()-1] -  SourceData[j][i]["open"][SourceData[j][i]["open"]:size()-1])) /PS[i]);
							end
