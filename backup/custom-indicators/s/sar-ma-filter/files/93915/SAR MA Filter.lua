...
function Init()
    indicator:name("SAR MA Filter");
    indicator:description("SAR MA Helper");
...
-- Routine
 function Prepare(nameOnly)
   ...
   local name = profile:id() .. "(" .. source:name() .. ")";
	...
	Indicator[1] = core.indicators:create(Method1, source.close,Period1);
    assert(core.indicators:findIndicator(Method2) ~= nil, Method2 .. " indicator must be installed");
	Indicator[2] = core.indicators:create(Method2, source.close,Period2);
...
function Calculation1( period )
   ...
   if Indicator[1].DATA[period]> Indicator[2].DATA[period] 
    and  sar.DN:hasData(period) 
   then
   Flag[period]=1;
   elseif Indicator[1].DATA[period]< Indicator[2].DATA[period] 
   and  sar.UP:hasData(period)   
   then
   Flag[period]=-1;
   else
   Flag[period]=0;
	...
	up:set(period, source.low[period], "\254");	
	end
	
	if Flag[period]== -1 and  Flag[period-1]~=  -1 then 
	down:set(period, source.high[period], "\254");	
	end 
	
	if Flag[period]== 0 and  Flag[period-1]~=  0 then 
		 if Flag[period-1]== -1 then
		  closedown:set(period, source.low[period], "\253");	
		 end
		 if Flag[period-1]==  1 then
	     closeup:set(period, source.high[period], "\253");	
		 end
	end 
...
function Calculation2(period )    
   ...
   if (Indicator[1].DATA[period]> Indicator[2].DATA[period]   and  sar.DN:hasData(period) )
   then
   Flag[period]=1;
   elseif (Indicator[1].DATA[period]< Indicator[2].DATA[period]  and  sar.UP:hasData(period) )   
   then
   Flag[period]=-1;
   else
