...
local Indicator = {}
local source
function GetCount(period)
	local positive = 0;
	local negative = 0;
	for j = 1, Number, 1 do
		local index = core.findDate(SourceData[j], source:date(period), false);
		if index < 0 or not Indicator[j].DATA:hasData(index) then
			return 0, 0;
		end
		if Indicator[j].DATA[index] < SourceData[j].close[index] then
			positive = positive + 1;
		elseif Indicator[j].DATA[index] > SourceData[j].close[index] then
			negative = negative + 1;
		end
	end
...
{ 
    {
        ...
        UpCondition = function (period)
			local positive = GetCount(period)
			return positive >= alert_threshold;
        end,
        DownCondition = function (period)
			local _, negative = GetCount(period)
			return negative >= alert_threshold;
        end,
...
function Add(id, TF, Flag, Instrument)
	...
	indicator.parameters:addInteger("FM" .. id, "Line " .. id .. ". Period", "", 10)
	indicator.parameters:addDouble("MP" .. id, "Line " .. id .. ". Multiplier", "", 1.5)
end

function Init()
	indicator:name("SuperTrend_Heat_Map")
	...
	indicator.parameters:addDouble("_Size", "Font Size (%)", "", 90, 50, 200)

	indi_alerts:AddParameters(indicator.parameters);
	indicator.parameters:addInteger("alert_threshold", "Alert Threshold", "", 10);
...
function Prepare(nameOnly)
	alert_threshold = instance.parameters.alert_threshold;
	...
	host = core.host
	Size = instance.parameters._Size
	...
	for i = 1, 13, 1 do
		...
		if instance.parameters:getBoolean("On" .. i) and (e1 - s1) <= (e2 - s2) then
			...
			MP[Number] = instance.parameters:getDouble("MP" .. i)
			...
			Temp1 = core.indicators:create("SUPERTREND", source, FM[Number], MP[Number])
			...
			loading[Number] = true
			Indicator[Number] = core.indicators:create("SUPERTREND", SourceData[Number], FM[Number], MP[Number])
		end
	end
		...
		for j = 1, Number, 1 do
			...
			if p ~= false then
				if Indicator[j].DATA:hasData(p) then
					if Indicator[j].DATA[p] < SourceData[j].close[p] then
						C2 = 12
						C1 = 11
					elseif Indicator[j].DATA[p] > SourceData[j].close[p] then
						C2 = 22
						C1 = 21
					else
						C2 = 32
