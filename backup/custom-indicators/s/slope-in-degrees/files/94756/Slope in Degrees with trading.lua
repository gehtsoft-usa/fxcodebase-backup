...
function Init()
    indicator:name("Slope in Degrees with trading")
    ...
    indicator.parameters:addInteger("buy_level", "Buy level", "", 0);
    indicator.parameters:addInteger("sell_level", "Sell level", "", 0);
...
function Draw(stage, context)
    ...
    if not init then
        ...
        context:createPen(
            6,
            context:convertPenStyle(instance.parameters.level_overboughtsold_style),
...
function AsyncOperationFinished(cookie, success, message, message1, message2) for _, module in pairs(Modules) do if module.AsyncOperationFinished ~= nil then module:AsyncOperationFinished(cookie, success, message, message1, message2); end end end
...
function Update(period)
    ...
    if Slope:hasData(period - 1) and Slope:hasData(period - 2) and core.crossesUnder(Slope, -buy_level, period - 1) then
    ...
    elseif Slope:hasData(period - 1) and Slope:hasData(period - 2) and core.crossesOver(Slope, -sell_level, period - 1) then
        if instance.parameters.allow_trade then
            if instance.parameters.close_on_opposite then
                trading:FindTrade()
                    :WhenSide("B")
