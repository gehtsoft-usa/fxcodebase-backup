...
function Init()
    ...
    Parameters(2, "K/OB Line")
    Parameters(3, "K/OS Line")
    Parameters(4, "K/Central Line")
    Parameters(5, "In Zone K/D Line")
end
...
function Activate(id, period)
    ...
    if id == 1 and ON[id] then
        if kLine[period] > dLine[period] and kLine[period - 1] <= dLine[period - 1] then
        ...
        elseif kLine[period] < dLine[period] and kLine[period - 1] >= dLine[period - 1] then
    ...
    if id == 2 and ON[id] then
        if kLine[period] > OB and kLine[period - 1] <= OB then
        ...
        elseif kLine[period] < OB and kLine[period - 1] >= OB then
    ...
    if id == 3 and ON[id] then
        if kLine[period] > OS and kLine[period - 1] <= OS then
        ...
        elseif kLine[period] < OS and kLine[period - 1] >= OS then
    ...
    if id == 4 and ON[id] then
        if kLine[period] > 50 and kLine[period - 1] <= 50 then
        ...
        elseif kLine[period] < 50 and kLine[period - 1] >= 50 then
    ...
    if id == 5 and ON[id] then
        if kLine[period] > dLine[period] and kLine[period - 1] <= dLine[period - 1] and (dLine[period] > OB or dLine[period] < OS) then
            up[id]:set(period, dLine[period], "\108");
            D[id] = nil;
            if U[id] ~= source:serial(period) and period == source:size() - 1 - Shift and not FIRST then
                U[id] = source:serial(period);
        ...
        elseif kLine[period] < dLine[period] and kLine[period - 1] >= dLine[period - 1] and (dLine[period] > OB or dLine[period] < OS) then
            down[id]:set(period, dLine[period], "\108");
            U[id] = nil;
            if D[id] ~= source:serial(period) and period == source:size() - 1 - Shift and not FIRST then
                D[id] = source:serial(period);
