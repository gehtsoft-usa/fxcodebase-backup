...
local VERSION_TXT = "v" .. VERSION .. "." .. REVISION .. " added: Sum Pips, Sum P/L, Harmonic Mean, Overlay, % Change, Basket Options, Code Tidy";
...
function Init()
    ...
    indicator.parameters:addInteger("lookback", "Lookback Last N Bars", "normalisation period for overlay and percentage change", 100, 0, 1000);
...
function Prepare(nameOnly)   
    ...
    for i = 1, MAX_INSTRUMENTS, 1 do
        if polarity[i] ~= "Disabled" and polarity[i] ~= "" and instrument[i] ~= "" then
            ...
            if 1 / point_size[i] > max_dps then max_dps = 1 / point_size[i] end
        end
    end
    ...
    if instance.parameters.overlay_source == true then
...
function Update(period, mode)
    ...
    if offset < first then offset = first end
    ...
    if instance.parameters.overlay_source == true and period == source:size() - 1 then
        local i;
        local min1, max1;
        local min2, max2;
