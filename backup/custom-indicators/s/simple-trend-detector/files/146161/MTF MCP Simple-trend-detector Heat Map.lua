...
--+------------------------------------------------------------------------------------------------+
 ...
 function Add(id, TF,Flag, Instrument )
	...
	indicator.parameters:addInteger("Period".. id, "Period", "", 15, 1, 2000);
 	indicator.parameters:addInteger("Signal".. id, "Signal MA Period", "", 15, 1, 2000);   
end

 
 
function Init()
    indicator:name("Simple-trend-detector Heat Map");
...
local MA = {} ; 
 function Prepare(nameOnly)   
   ...
   instance:ownerDrawn(true);
	...
	assert(core.indicators:findIndicator("SIMPLE-TREND-DETECTOR") ~= nil, "Please, download and install SIMPLE-TREND-DETECTOR.LUA indicator");    
	...
	s2, e2 = core.getcandle(iTF[i], 0, 0, 0);
	 ...
	 Number=Number+1;
	      ...
	      Signal[Number]= instance.parameters:getInteger("Signal" .. i);
	 ...
	 Label[Number]="";
                ...
                Label[Number]=Label[Number] .. " - " ..  TF[Number];				
				 ...
				 loading[Number]  = true;  	 
				 Indicator[Number] = core.indicators:create("SIMPLE-TREND-DETECTOR", SourceData[Number] ,  Period[Number]);   
				 MA[Number] = core.indicators:create("MVA", Indicator[Number].DATA ,  Signal[Number]);  			 
			   ...
			   x0, x1, x2 = context:positionOfBar (i);
			    ...
			    for j= 1, Number , 1 do
				  ...
				  if p~= false then
										...
										if Indicator[j].DATA:hasData(p) and Indicator[j].DATA:hasData(p-1) 
										and MA[j].DATA:hasData(p) and MA[j].DATA:hasData(p-1) 
										then 
												    ...
												    if Indicator[j].DATA[p] > MA[j].DATA[p] then		 
												       ...
												       if Indicator[j].DATA[p] > 0.7 then	
													   C1=13;
													   elseif Indicator[j].DATA[p] < 0.3 then	
													   C1=14
													   end
