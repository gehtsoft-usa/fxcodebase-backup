...
function Init()
    indicator:name("SAR Price Cross Alert");
   ...
   if Live~= "Live" then
	...
	Shift=1;
	  ...
	  if id == 1  and ON[id]  then
			...
			if sar.DN:hasData(period) 
			and source.open[period]>= sar.DN[period] 
			and  source.close[period] < sar.DN[period] 
			then	   
			...
			elseif sar.UP:hasData(period) 
	        and source.open[period]<= sar.UP[period] 
			and  source.close[period] > sar.UP[period]    
            then			   
