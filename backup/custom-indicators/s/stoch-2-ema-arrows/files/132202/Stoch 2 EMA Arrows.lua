...
--http://fxcodebase.com/code/viewtopic.php?f=17&t=69567
...
{ 
    {
        ...
        UpCondition = function (period)
            if not ssd.D:hasData(period - 1) or not ma1.DATA:hasData(period - 1) or not ma2.DATA:hasData(period - 1) then
                return false;
            end
            return ma1.DATA[period] > ma2.DATA[period] and ssd.K[period] > ssd.D[period];
        end,
...
function Init()
    indicator:name("Stochastics + 2 EMA Arrows");
    ...
    indicator.parameters:addInteger("ma1_period", "MA1 Period", "", 14);
    AddAverages("ma1_method", "MA1 Method", "MVA");

    indicator.parameters:addInteger("ma2_period", "MA2 Period", "", 7);
