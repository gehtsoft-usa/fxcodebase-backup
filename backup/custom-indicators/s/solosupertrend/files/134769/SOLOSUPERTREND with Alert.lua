...
function Prepare(nameOnly)
    ...
    SOLOST = instance:addStream("SOLOST", core.Line, "SOLOST", "SOLOST", instance.parameters.SOLOST_color_Up, atr.DATA:first() + 1, 0);
...
function Activate (id, period)
   ...
   Alert[id][period]=0;
	  ...
	  if id == 2  and ON[id]  then
			...
			if  source.close[period] > SOLOST[period] 
			and   source.close[period-1] <= SOLOST[period-1] 
			then
			...
			elseif  source.close[period] < SOLOST[period] 
			and   source.close[period-1] >= SOLOST[period-1] 
            then			
