...
function Init()
	...
	indicator.parameters:addColor("Top", "Color of Cross Over", "Color of Cross Over", core.rgb(0,255, 0));	
	indicator.parameters:addColor("Bottom", "Color of Cross Cross Over", "Color of Cross Under", core.rgb(255,0, 0));
...
function Update(period, mode)
 ...
 local Flag= Calculation(period, mode);
  ...
  local i;
   ...
   if ((Cross and not core.crosses (source.close, ma.DATA,period)) )  then
   return;
   end
...
function  Initialization ()
    ...
    RecurrentSound = instance.parameters.RecurrentSound;
	   ...
	   if    Cross then
					...
					up  = instance:createTextOutput ("Up" ,"Up", "Wingdings", Size, core.H_Center, core.V_Bottom,Top , 0);
					down  = instance:createTextOutput ("Down", "Down", "Wingdings", Size, core.H_Center, core.V_Top,Bottom , 0); 
   ...
   if Live~= "Live" then
	...
	iLabel= Label[id];
	  ...
	  if id == 1  and ON[id]  then
			...
			if  Signal[period]== 1
			and source.volume[period]~=0 		
            and ( cross== true or cross== false or not Cross)			
			then
