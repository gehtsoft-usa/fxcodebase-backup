...
--+------------------------------------------------------------------+
 ...
 function Add(id, TF,Flag, Instrument )
	...
	indicator.parameters:addInteger("Length" .. id, "Length", "", 20, 1, 2000);
	indicator.parameters:addBoolean("Absolute" .. id, "Absolute", "", false);
...
function Init()
    indicator:name("Sell Relative Strength Index Heat Map");
	...
	indicator.parameters:addColor("NeutralUp", "Up in Neutral Trend Color","", core.rgb(0, 0, 200));
	indicator.parameters:addColor("NeutralDown", "Down in Neutral Trend Color","", core.rgb(0, 0, 255));
...
function Prepare(nameOnly)
	...
	assert(core.indicators:findIndicator("SELL RELATIVE STRENGTH INDEX") ~= nil, "Please, download and install SELL RELATIVE STRENGTH INDEX.LUA indicator");    
	...
	s2, e2 = core.getcandle(iTF[i], 0, 0, 0);
	 ...
	 Number=Number+1;
		  ...
		  Absolute[Number]= instance.parameters:getBoolean("Absolute" .. i);
	 ...
	 Label[Number]="";
                ...
                Label[Number]=Label[Number] .. " - " ..  TF[Number];				
			  	...
			  	Temp1= core.indicators:create("SELL RELATIVE STRENGTH INDEX", source ,  Length[Number], Absolute[Number]);  				
				ifirst= Temp1.DATA:first()*2;
				 ...
				 loading[Number]  = true;  	 
				 Indicator[Number] = core.indicators:create("SELL RELATIVE STRENGTH INDEX", SourceData[Number] ,  Length[Number], Absolute[Number]);   
				 ...
				 srsi:setColor(period, instance.parameters.Down);
				  ...
				  if p~= false then
										...
										if Indicator[j].DATA:hasData(p) and Indicator[j].DATA:hasData(p-1) then 
												    ...
												    if Indicator[j].DATA[p]   >= -0.05 and Indicator[j].DATA[p]  <= 0.05 then		 
