...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=69171
...
local STRATEGY_NAME = "Support_Resistance_Lines_strategy";
...
function CreateParameters() 
	strategy.parameters:addInteger("LookBack", "Look Back Period", "Period", 1000)
	strategy.parameters:addString("Find", "Search for ", "Search for ", "Both")
	strategy.parameters:addStringAlternative("Find", "Support", "", "Support")
	strategy.parameters:addStringAlternative("Find", "Resistance", "", "Resistance")
	strategy.parameters:addStringAlternative("Find", "Both", "", "Both")

	strategy.parameters:addString("Method", "Validation Method", "Validation", "High/Low")
	strategy.parameters:addStringAlternative("Method", "High/Low", "High/Low", "High/Low")
	strategy.parameters:addStringAlternative("Method", "Open/Close", "Open/Close", "Open/Close")

	strategy.parameters:addString("Zone", "Zone Method", "Zone Method", "Narrow")
	strategy.parameters:addStringAlternative("Zone", "Narrow", "Narrow", "Narrow")
	strategy.parameters:addStringAlternative("Zone", "Wide", "Wide", "Wide")

	strategy.parameters:addInteger("Tolerance", "Tolerance", "Tolerance", 0, 0, 10)

	strategy.parameters:addString("MA_Method", "MA Method", "MA Method", "MVA")
	...
	strategy.parameters:addInteger("MA_Period", "MA period", "MA period", 10)
end
local indi, LookBack, Tolerance;
function CreateEntryindicators(source)
    LookBack = instance.parameters.LookBack;
    Tolerance = instance.parameters.Tolerance;
    local profile = core.indicators:findIndicator("SUPPORT_RESISTANCE_LINES");
    assert(profile ~= nil, "Please, download and install " .. "SUPPORT_RESISTANCE_LINES" .. ".LUA strategy");
    local strategyParams = profile:parameters();
    strategyParams:setBoolean("strategy_mode", true);
    strategyParams:setInteger("LookBack", instance.parameters.LookBack);
    strategyParams:setInteger("Tolerance", instance.parameters.Tolerance);
    strategyParams:setInteger("MA_Period", instance.parameters.MA_Period);
    strategyParams:setString("Find", instance.parameters.Find);
    strategyParams:setString("Method", instance.parameters.Method);
    strategyParams:setString("Zone", instance.parameters.Zone);
    strategyParams:setString("MA_Method", instance.parameters.MA_Method);
    indi = core.indicators:create("SUPPORT_RESISTANCE_LINES", source, strategyParams)
end
...
function CreateCustomActions()
    ...
    enterLongAction.IsPass = function (source, period, periodFromLast, data)
        ...
        for i = 0, LookBack - 1 do
            if indi.up2:hasData(period - i) and indi.up1:hasData(period - i) then
                if indi.up2[period - i] >= -Tolerance and indi.up2[period - i] ~= nil then
                    count = count + 1
                    if count == 2 then
                        break;
                    end
            ...
            if indi.down2:hasData(period - i) and indi.down1:hasData(period - i) then
                if indi.down2[period - i] >= -Tolerance and indi.down2[period - i] ~= nil then
                    return false;
                end
            end
        end
        return indi.signals[period] == 1 and count == 2;
    end
