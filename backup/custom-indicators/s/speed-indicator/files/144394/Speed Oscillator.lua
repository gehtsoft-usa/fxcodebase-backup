...
function Init()
    indicator:name("Speed Oscillator");
...
function Update(period, mode)
	...
	return;
		...
		for i= period, first, -1 do
			...
			pos_sum=pos_sum+ math.abs(Dif[i]);
			elseif  Dif[i] < 0 then
			neg_count=neg_count+1;
			neg_sum=neg_sum+ math.abs(Dif[i]);			
			end		
        ...
        local PosBuffer  = pos_sum/Period2;
        local NegBuffer  = neg_sum/Period2;
		  
        if Dif[period] > 0 then
        Oscillator[period] =  math.abs(Dif[period]) / PosBuffer;
        elseif Dif[period] < 0 then  
        Oscillator[period] =  math.abs(Dif[period]) / NegBuffer;		
        else
        Oscillator[period]=0;
        end   		
end
