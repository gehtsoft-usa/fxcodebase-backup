...
-- http://fxcodebase.com/code/viewtopic.php?f=17&t=63442&p=106099#p106099
...
function calcValue( Index, period)
   ...
   return;
    ...
    if period== 1 then 	
		...
		volume[Index] = 1; 
		Expires= History:date(1)+ OneSecond*Step;
		LastIndex=period;
	else 
		
	 
				
				if History:date(period) >  Expires   then
					...
					volume[Index] = 1; 
			        LastIndex=period;
					Expires= History:date(period)+ OneSecond*Step;
				else
