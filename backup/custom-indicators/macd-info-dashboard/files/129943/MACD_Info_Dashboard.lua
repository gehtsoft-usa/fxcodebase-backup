...
-- http://fxcodebase.com/code/viewtopic.php?f=17&t=69159
...
-- Light dashboard template v.1.0 (MODIFIED)
...
local indi_name = "MACD Info Dashboard";
local indi_id = "MACD";
...
function CreateIndicators(source)
    ...
    p:setInteger("fast", instance.parameters.fast);
    p:setInteger("slow", instance.parameters.slow);
    p:setInteger("signal_smoothing", instance.parameters.signal_smoothing);
...
function CreateParameters()
    indicator.parameters:addInteger("fast", "MACD Fast Period", "", 12);
    indicator.parameters:addInteger("slow", "MACD Slow Period", "", 26);
    indicator.parameters:addInteger("signal_smoothing", "MACD Signal Smoothing", "", 9);
end
...
function GetTableIndex(symbol)
    if grid_mode == "h" then
        return (symbol.TimeframeIndex + 1) * 3, symbol.SymbolIndex + 1;
    end

    return (symbol.SymbolIndex + 1) * 3, symbol.TimeframeIndex + 1;
end

function DrawSignal(symbol, context)
    if symbol.Text1 == nil then
        return;
    end
    local row, column = GetTableIndex(symbol);
    CellsBuilder:Add(FONT_TEXT, symbol.Text1, text_color, column, row, context.CENTER, backgound, GRID_PEN, true, false);
    CellsBuilder:Add(FONT_TEXT, symbol.Text2, text_color, column, row + 1, context.CENTER, backgound, GRID_PEN, false, false);
    CellsBuilder:Add(FONT_TEXT, symbol.Text3, text_color, column, row + 2, context.CENTER, backgound, GRID_PEN, false, true);
end
function Draw(stage, context) 
    ...
    for i = 1, #timeframes do
        if grid_mode == "h" then
            CellsBuilder:Add(FONT_TEXT, timeframes[i], text_color, 1, (i + 1) * 3 + 1, context.CENTER);
        else
    ...
    for i = 1, #instruments do
        if grid_mode == "h" then
            ...
            CellsBuilder:Add(FONT_TEXT, instruments[i], text_color, 1, (i + 1) * 3 + 1, context.CENTER);
        end
    end
...
function GetPeriodSinceCross(macd)
    local count = macd.MACD:size() - 1;
    for i = 0, count - 1 do
        if core.crosses(macd.MACD, macd.SIGNAL, count - i) then
            return i
        end
    end
    return nil;
...
function ForecastCross(macd)
    local macd_change = macd.MACD[NOW - 1] - macd.MACD[NOW];
    local signal_change = macd.SIGNAL[NOW - 1] - macd.SIGNAL[NOW];
    if macd.MACD[NOW] > macd.SIGNAL[NOW] and macd_change >= signal_change then
        return nil;
    end
    if macd.MACD[NOW] < macd.SIGNAL[NOW] and macd_change <= signal_change then
        return nil;
    end
    local diff = macd.SIGNAL[NOW] - macd.MACD[NOW];
    local periods = diff / (macd_change - signal_change);
    if periods - math.floor(periods) ~= periods then
        periods = periods + 1;
    end
...
function UpdateData()
    for _, symbol in ipairs(items) do
        if symbol.Indicators ~= nil and not symbol.Loading then
            ...
            if symbol.Indicators[1].MACD[NOW] > symbol.Indicators[1].SIGNAL[NOW] then
                symbol.Text1 = "MACD > Signal";
            else
                symbol.Text1 = "MACD < Signal";
            end
            local periods_from_cross = GetPeriodSinceCross(symbol.Indicators[1]);
            if periods_from_cross == nil then
                symbol.Text2 = "X";
            else
                symbol.Text2 = tostring(periods_from_cross);
            end
            local next_cross_in_periods = ForecastCross(symbol.Indicators[1]);
            if next_cross_in_periods == nil then
                symbol.Text3 = "X";
            else
                symbol.Text3 = tostring(next_cross_in_periods);
            end
		else
            symbol.Text1 = "...";
            symbol.Text2 = "...";
