...
function Init()
    indicator:name("MTF_MCP Colored RSI Moving Average");
...
local RSI_Period, MA_Period, MA_Method;
 ...
 function Prepare(nameOnly)   
    ...
    local name = profile:id() .. "(" ..  instance.source:name().. ", " ..  RSI_Period.. ", " ..  MA_Period.. ", " ..  MA_Method .. ")";
    instance:name(name); 
	...
	assert(core.indicators:findIndicator("COLORED RSI MOVING AVERAGE") ~= nil, "Please, download and install COLORED RSI MOVING AVERAGE.LUA indicator");    
    ...
    for i = 1, Number, 1 do	 
	    	...
	    	local Test = core.indicators:create("COLORED RSI MOVING AVERAGE", source.close , RSI_Period, MA_Period, MA_Method, false);   
		...
		Indicator[i] = core.indicators:create("COLORED RSI MOVING AVERAGE", SourceData[i].close, RSI_Period, MA_Period, MA_Method, false, NO , UP, DN);
		loading[i]= true;		
...
function Add(context, i )
        ...
        if not  Indicator[i].MA:hasData(Indicator[i].MA:size()-1) then
		return;
		end
        ...
        context:createFont (1, "Wingdings", (xCell/10)*(Size/100)*2, yCell*(Size/100), 0);	
			...
			if Indicator[i].MA:colorI(Indicator[i].MA:size()-1) == UP  
			then
			Color=UP;			
			elseif Indicator[i].MA:colorI(Indicator[i].MA:size()-1) == DN  
			then
			Color=DN; 
			else 
			Color=NO;
			...
			if Indicator[i].MA[Indicator[i].MA:size()-1] > Indicator[i].MA[Indicator[i].MA:size()-2]
			then
			Symbol="\217";
			elseif Indicator[i].MA[Indicator[i].MA:size()-1]< Indicator[i].MA[Indicator[i].MA:size()-2]
			then
			Symbol="\218";
			else
			Symbol="\160";
		    ...
		    context:drawText (2, tostring(TF[i]), Label, -1, context:right ()-(i)*xCell    , context:top ()+VShift+yCell, context:right ()-(i-1)*xCell + width, context:top ()+VShift+yCell + height, 0);
			 ...
			 context:drawText (2,  Value, Label, -1, context:right ()-(i)*xCell    , context:top ()+VShift+yCell*3, context:right ()-(i-1)*xCell + width, context:top ()+VShift+yCell*3 + height, 0);
