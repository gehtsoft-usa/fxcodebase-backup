...
function Init()
    indicator:name("Tick-Chart-Generic Four  Line Indicator Cross");
...
function Prepare(nameOnly)
	...
	Signal[i]= instance:addInternalStream(0, 0);	 
		...
		if  iprofile1:requiredSource() == core.Tick then			
			...
			assert( false, "1. Line Required Source is Bar, only Ticks Source are allowed.");
		end
		...
		if  iprofile2:requiredSource() == core.Tick then			
			...
			assert( false, "2. Line Required Source is Bar, only Ticks Source are allowed.");
		end
		
		if  iprofile3:requiredSource() == core.Tick then			
			Indicator3 = iprofile3:createInstance( source, iparams3);
		else
			assert( false, "3. Line Required Source is Bar, only Ticks Source are allowed.");
		end
		...
		if  iprofile4:requiredSource() == core.Tick then			
			...
			assert( false, "4. Line Required Source is Bar, only Ticks Source are allowed.");
		end
