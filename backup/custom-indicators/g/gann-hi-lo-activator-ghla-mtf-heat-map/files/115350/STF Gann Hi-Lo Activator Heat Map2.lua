...
function Init()
 ...
 -- indicator.parameters:addDouble("VSpace", "Vertical Spacing (%)","",50, 0, 100);
   ...
   indicator.parameters:addDouble("VSize", "Vertical size (%)","",10, 1, 100);
end
...
--local Number=1;
 function Prepare(nameOnly)   
	...
	VSize=instance.parameters.VSize;
      ...
      -- local VS=(Size*VSpace);
       ...
       for i= first, last, 1 do	   
			   ...
			   x0, x1, x2 = context:positionOfBar (i);
					...
					local middle, range=(top+bottom)/2, bottom-top;
					range=range*VSize/100;
					top=middle-range/2;
					bottom=middle+range/2;
					
					 
						
						  if GHLA.DATA:hasData(i) then
							  if source.close[i]> GHLA.DATA[i] then
							  context:drawRectangle (11, 12, x1+(x2-x1)*HS,  top, x2-(x2-x1)*HS,  bottom);
							  elseif source.close[i]< GHLA.DATA[i] then
							  context:drawRectangle (21, 22, x1+(x2-x1)*HS,  top, x2-(x2-x1)*HS, bottom);
							  else
							  context:drawRectangle (31, 32, x1+(x2-x1)*HS,  top, x2-(x2-x1)*HS, bottom);
							  end
						  else
						       context:drawRectangle (1, 2, x1+(x2-x1)*HS, top, x2-(x2-x1)*HS, bottom);
						  end
