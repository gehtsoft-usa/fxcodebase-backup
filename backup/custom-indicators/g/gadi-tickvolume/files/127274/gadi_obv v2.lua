...
function Init()
    ...
    InitStyle("3", "Line 2 Projection", core.rgb(0, 0, 255), core.LINE_DOT, 1);
    InitStyle("4", "Line 3", core.rgb(0, 0, 255), core.LINE_SOLID, 1);
    InitStyle("5", "Line 3 Projection", core.rgb(0, 0, 255), core.LINE_DOT, 1);

    indi_alerts:AddParameters(indicator.parameters);
    indi_alerts:AddAlert("Top Alert");
    indi_alerts:AddAlert("Bottom Alert");
end
...
local out1;
local out2;
local out2_projection;
local out3;
local out3_projection;
function Prepare(nameOnly)
    ...
    out2_projection = instance:addStream("out2_projection", core.Line, "Channel High", "Channel High", GetColor("3"), 0);
    out2_projection:setPrecision(math.max(2, instance.source:getPrecision()));
    SetStyle("3", out2_projection);

    out3 = instance:addStream("out3", core.Line, "Channel Low", "Channel Low", GetColor("4"), 0);
    out3:setPrecision(math.max(2, instance.source:getPrecision()));
    SetStyle("4", out3);

    out3_projection = instance:addStream("out3_projection", core.Line, "Channel Low", "Channel Low", GetColor("5"), 0);
    out3_projection:setPrecision(math.max(2, instance.source:getPrecision()));
    SetStyle("5", out3_projection);
end
...
function Update(period, mode)
    ...
    if DrawChannel then
        out2_projection[period] = out2_projection[period - 1];
        out3_projection[period] = out3_projection[period - 1];
        ...
        core.drawLine(out2_projection, range, max, periodStart, max, periodEnd);
        core.drawLine(out3_projection, range, min, periodStart, min, periodEnd);
    end
...
function Activate(alert, period, historical_period)
    ...
    if alert.id == 1 then
        if core.crossesOver(out1, out2_projection, period) then
            alert:UpAlert(source, period, alert.Label .. ". Bull pattern", source.high[period], historical_period);
        elseif core.crossesUnder(out1, out2_projection, period) then
            alert:DownAlert(source, period, alert.Label .. ". Bear pattern", source.low[period], historical_period);
        end
    end
    if alert.id == 2 then
        if core.crossesOver(out1, out3_projection, period) then
            alert:UpAlert(source, period, alert.Label .. ". Bull pattern", source.high[period], historical_period);
        elseif core.crossesUnder(out1, out3_projection, period) then
            alert:DownAlert(source, period, alert.Label .. ". Bear pattern", source.low[period], historical_period);
        end
    end
