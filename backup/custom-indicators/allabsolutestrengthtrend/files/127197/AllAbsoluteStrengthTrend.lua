...
-- http://fxcodebase.com/code/viewtopic.php?f=17&t=68620
...
function Init()
    indicator:name("All Absolute Strength Trend");
    indicator:description("");
    indicator:requiredSource(core.Bar);
    indicator:type(core.Indicator);

    indicator.parameters:addString("MathMode", "Math method", "", "Stoch");
    indicator.parameters:addStringAlternative("MathMode", "RSI", "", "RSI");
    indicator.parameters:addStringAlternative("MathMode", "Stoch", "", "Stoch");
    indicator.parameters:addStringAlternative("MathMode", "DMI", "", "DMI");
    indicator.parameters:addStringAlternative("MathMode", "MACD", "", "MACD");
    
    indicator.parameters:addInteger("Length", "Evaluation period", "", 10);
    indicator.parameters:addInteger("PreSmooth", "Pre-Smoothing period", "", 1);
    indicator.parameters:addInteger("Smooth", "Smoothing period", "", 10);
    AddAverages("MA_Method", "MA Method", "MVA");
    
    indicator.parameters:addString("LevelsMode", "Levels Mode", "", "obos");
    indicator.parameters:addStringAlternative("LevelsMode", "Standard OverBought/OverSold", "", "obos");
    indicator.parameters:addStringAlternative("LevelsMode", "StdDev Bands", "", "StdDev");
    indicator.parameters:addStringAlternative("LevelsMode", "High/Low Channel", "", "hilo");

    indicator.parameters:addDouble("StrengthLevel", "Strength Level", "", 70);
    indicator.parameters:addDouble("WeaknessLevel", "Weakness Level", "", 30);
    indicator.parameters:addInteger("LookBackPeriod", "LookBack Period for LevelsMode=1,2", "", 20);
    indicator.parameters:addDouble("UpperMultiplier", "Upper Band Multiplier", "", 1);
    indicator.parameters:addDouble("LowerMultiplier", "Lower Band Multiplier", "", 1);
    ...
    indicator.parameters:addInteger("HL", "Transparency (%)", "", 95, 0, 100);

    indi_alerts:AddParameters(indicator.parameters);
    indi_alerts:AddAlert("Alert Signal");
end
...
local MathMode, Length, LevelsMode, StrengthLevel, WeaknessLevel;
local mMA, mLo, atr;
local lbulls, lbears, Bulls, Bears, bulls, bears, trend, Smooth, upTrend, dnTrend, up_color, dn_color
function Prepare(nameOnly)
    ...
    MathMode = instance.parameters.MathMode;
    Length = instance.parameters.Length;
    LevelsMode = instance.parameters.LevelsMode;
    StrengthLevel = instance.parameters.StrengthLevel;
    WeaknessLevel = instance.parameters.WeaknessLevel;
    ...
    mMA = CreateAverates(instance.parameters.MA_Method, source, instance.parameters.PreSmooth + 2);
    mLo = CreateAverates(instance.parameters.MA_Method, source, instance.parameters.PreSmooth + Length + 2);

    bulls = instance:addInternalStream(0, 0);
    bears = instance:addInternalStream(0, 0);
    trend = instance:addInternalStream(0, 0);

    lbulls = CreateAverates(instance.parameters.MA_Method, bulls, instance.parameters.PreSmooth + Length + 2);
    lbears = CreateAverates(instance.parameters.MA_Method, bears, instance.parameters.PreSmooth + Length + 2);

    Bulls = CreateAverates(instance.parameters.MA_Method, lbulls.DATA, instance.parameters.PreSmooth + Length + Smooth + 2);      
    Bears = CreateAverates(instance.parameters.MA_Method, lbears.DATA, instance.parameters.PreSmooth + Length + Smooth + 2);
    atr = core.indicators:create("ATR", source, 10);

    upTrend = instance:addInternalStream(0, 0);
    dnTrend = instance:addInternalStream(0, 0);
    instance:createChannelGroup("trend", "Trend", upTrend, dnTrend, up_color, 100 - instance.parameters.HL);
end
...
function Update(period, mode)
    ...
    if (MathMode == "RSI") then
        bulls[period] = 0.5*(math.abs(mMA.DATA[period] - mMA.DATA[period - 1]) + (mMA.DATA[period] - mMA.DATA[period - 1]));
        bears[period] = 0.5*(math.abs(mMA.DATA[period] - mMA.DATA[period - 1]) - (mMA.DATA[period] - mMA.DATA[period - 1]));
    elseif (MathMode == "Stoch") then
        local up = 0;      
        local dn = 10000000000;
        for i = 0, Length - 1 do
            up = math.max(up, source.high[period - i]);
            dn = math.min(dn, source.low[period - i]);
        end
        bulls[period] = mMA.DATA[period] - dn;
        bears[period] = up - mMA.DATA[period];
    elseif(MathMode == "DMI") then
        bulls[period] = math.max(0,0.5*(math.abs(mMA.DATA[period] - mMA.DATA[period - 1]) + (mMA.DATA[period] - mMA.DATA[period - 1])));
        bears[period] = math.max(0,0.5*(math.abs(mLo.DATA[period - 1] - mLo.DATA[period]) + (mLo.DATA[period - 1] - mLo.DATA[period])));

        if (bulls[period] > bears[period]) then
            bears[period] = 0;
        elseif(bulls[period] < bears[period]) then
            bulls[period] = 0;
        else 
            bulls[period] = 0;
            bears[period] = 0;
        end
    elseif(MathMode == "MACD") then
        bulls[period] = 0.5*(math.abs(mMA.DATA[period] - mLo.DATA[period]) + (mMA.DATA[period] - mLo.DATA[period]));
        bears[period] = 0.5*(math.abs(mMA.DATA[period] - mLo.DATA[period]) - (mMA.DATA[period] - mLo.DATA[period]));
    end
    
    local len = Length;
    if (MathMode == "Stoch" or MathMode == "MACD") then
        len = 1;
    end
    lbulls:update(mode);
    lbears:update(mode);
    ...
    local strength, weakness;
    if (LevelsMode == "obos") then
        strength = StrengthLevel / 100 * (Bulls.DATA[period] + Bears.DATA[period]);
        weakness = WeaknessLevel / 100 * (Bulls.DATA[period] + Bears.DATA[period]);
    else
        HiArray[period] = math.max(Bulls.DATA[period], Bears.DATA[period]);  
        LoArray[period] = math.min(Bulls.DATA[period], Bears.DATA[period]);

        if (LevelsMode == "StdDev") then
            strength = mathex.avg(HiArray, period - LookBackPeriod, period) + UpperMultiplier * mathex.stdev(HiArray, period - LookBackPeriod, period);
            weakness = mathex.avg(LoArray, period - LookBackPeriod, period) + LowerMultiplier * mathex.stdev(LoArray, period - LookBackPeriod, period);
        elseif (LevelsMode == "hilo") then
            dnPivot, upPivot = getPivots(HiArray, LoArray, period);
    
            strength = upPivot - (upPivot - dnPivot) * (1 - StrengthLevel / 100);
            weakness = dnPivot + (upPivot - dnPivot) * WeaknessLevel / 100;
        end
    end

    trend[period] = trend[period - 1];
    if (Bulls.DATA[period] > strength and Bulls.DATA[period] > Bears.DATA[period]) then
        trend[period] = 1;
    elseif (Bears.DATA[period] > strength and Bulls.DATA[period] < Bears.DATA[period]) then
        trend[period] =-1;
    end

    local delta = 0.5 * atr.DATA[period];

    if (trend[period] > 0) then
        if (Bulls.DATA[period] > Bears.DATA[period]) then
            upTrend[period] = math.max(source.open[period], source.close[period]);
            dnTrend[period] = math.min(source.open[period], source.close[period]);
            upTrend:setColor(period, up_color);
        end
    elseif (trend[period] < 0) then
        if (Bears.DATA[period] > Bulls.DATA[period]) then
            upTrend[period] = math.min(source.open[period], source.close[period]);
            dnTrend[period] = math.max(source.open[period], source.close[period]);
            upTrend:setColor(period, dn_color);
        end
    end
...
function getPivots(hi, lo, period)
    local max, imax = mathex.max(hi, period - LookBackPeriod, period);
    local min, imin = mathex.min(lo, period - LookBackPeriod, period);
    return min, max;  
end
