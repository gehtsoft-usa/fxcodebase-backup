...
-- https://fxcodebase.com/code/viewtopic.php?f=17&t=71971
...
-- Routine
 function Prepare(nameOnly)   
    ...
    Max = instance:addStream("Max", core.Line, name, "Max", instance.parameters.color1, first+Period );
    ...
    Min = instance:addStream("Min", core.Line, name, "Min", instance.parameters.color2, first+Period );
...
function Update(period, mode)
	 ...
	 Line[period]=Line[period-1]+source.volume[period]; 
	...
	local min, max=mathex.minmax(Line , period-Period+1, period);
	Max[period]=max;
	Min[period]=min;
	
	Top[period] = Max[period]-(Max[period]-Min[period])/5.0;
    Bottom[period] = Min[period]+(Max[period]-Min[period])/5.0;
