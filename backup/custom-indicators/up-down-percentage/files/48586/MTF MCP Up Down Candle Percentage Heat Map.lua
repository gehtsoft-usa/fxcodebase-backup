...
function Init()
    indicator:name("MTF MCP Up Down Candle Percentage Heat Map");
...
function Prepare(nameOnly)  
	...
	assert(core.indicators:findIndicator("UDP") ~= nil, "Please, download and install UDP.LUA indicator");    
	...
	s2, e2 = core.getcandle(iTF[i], 0, 0, 0);
	 ...
	 Label[Number]="";
                ...
                Label[Number]=Label[Number] .. " - " ..  TF[Number];				
			  	...
			  	Temp1= core.indicators:create("UDP", source.close ,  Period[Number]);  				
				ifirst= Temp1.DATA:first()*2;
				 ...
				 loading[Number]  = true;  	 
				 Indicator[Number] = core.indicators:create("UDP", SourceData[Number].close ,  Period[Number]);   
			    ...
			    for j= 1, Number , 1 do
				  ...
				  if p~= false then
										...
										if Indicator[j].DATA:hasData(p)   then 
												    ...
												    if Indicator[j].UP[p] > Indicator[j].DOWN[p] then		 
															 
															C2=12;
															C1=11;
														 
														
												    elseif Indicator[j].UP[p] < Indicator[j].DOWN[p] then		   
													      	C2=22;
															C1=21
													else
															C2=32;
