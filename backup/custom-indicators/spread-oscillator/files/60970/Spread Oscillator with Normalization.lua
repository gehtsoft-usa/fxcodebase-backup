...
function Init()
    indicator:name("Spread Oscillator with Normalization");
    ...
    indicator.parameters:addDouble("Zero", "Zero Level","", 50);
...
function Prepare(nameOnly)
	...
	TWO = core.indicators:create("EMA", Raw, Two);
	   ...
	   rEMA = instance:addInternalStream(0, 0);
        ...
        EMA = instance:addStream("ST", core.Line, name, "Spread Oscillator", instance.parameters.EMA_color, math.max(ONE.DATA:first(),TWO.DATA:first()) +Period);
...
function Update(period, mode)
      ...
      rEMA[period]=TWO.DATA[period]-ONE.DATA[period];
	  
	  
	  if period < ONE.DATA:first() +Period
		or  period < TWO.DATA:first()+Period
		then
		return;
		end
	  ...
	  local min, max;
		min,max=mathex.minmax(rEMA, period-Period+1, period);
		EMA[period] = ((rEMA[period] - min)/ (max-min))*100; 		
end
