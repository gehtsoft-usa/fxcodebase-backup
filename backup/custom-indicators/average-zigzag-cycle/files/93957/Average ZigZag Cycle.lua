...
-- http://fxcodebase.com/code/viewtopic.php?f=17&t=60681
...
function Init()
    indicator:name("Average ZigZag Cycle");
	...
	indicator.parameters:addGroup("Average Calculation");
...
function Draw (stage, context)
   ...
   if Init  then
   context:createFont (1, "Arial", instance.parameters.TextSize,  instance.parameters.TextSize, 0);
   Init=false;
   end
    local iFirst = math.max(first, context:firstBar ());
    local iLast = math.min (context:lastBar (), source:size()-2);
	   ...
	   for i= iFirst, iLast, 1 do
		...
		txt1,txt2= Calculate(i);		 
		 width1, height1 = context:measureText (1,txt1 , 0);
		 width2, height2 = context:measureText (1,txt2 , 0);
		context:drawText (1,  txt1 , instance.parameters.Text_color, -1, x1-width1/2, y1-height1, x1+width1/2, y1, 0 );	
        context:drawText (1,  txt2 , instance.parameters.Text_color, -1, x1-width1/2, y1-height1-height2, x1+width2/2, y1-height1, 0 );		
		elseif Flag[i] == -1 then
		txt1, txt2= Calculate(i);
		visible, y1=  context:pointOfPrice (source.low[i]);	
		 width1, height1 = context:measureText (1,txt1 , 0);
		 width2, height2 = context:measureText (1,txt2 , 0);
		context:drawText (1,  txt1 , instance.parameters.Text_color, -1, x1-width1/2, y1, x1+width1/2, y1+height1, 0 );	
        context:drawText (1,  txt2 , instance.parameters.Text_color, -1, x1-width1/2, y1+height1, x1+width2/2, y1+height1+height2, 0 );		
		end
	end	
end	 
...
for i = period, first, -1 do	 
	...
	break;
		...
		return "Av: " .. string.format(format, Sum/Period) ,  " Ac: ".. Actual ;
