...
-- https://fxcodebase.com/code/viewtopic.php?f=17&t=72419
...
function Init()
    indicator:name("Adaptive Stochastic");
...
function Update(period, mode)
	...
	Smooth[period] = (4*source.close[period] + 3*source.close[period-1] + 2*source.close[period-2] + source.close[period-3]) / 10; 
    ...
    local min,max=mathex.minmax(source, period -Length +1 , period);
		...
		Line[period] = (source.close[period]-min)/(max-min)*100;
