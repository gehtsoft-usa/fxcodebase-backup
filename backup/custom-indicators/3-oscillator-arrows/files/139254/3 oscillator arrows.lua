...
--http://fxcodebase.com/code/viewtopic.php?f=17&t=70677
...
{ 
    {
        ...
        UpCondition = function (period)
            return di.DATA[period] > 0 and cmf.DATA[period] > 0 and src.DATA[period] > 0;
        end,
...
function Init()
    indicator:name("3 oscillator arrows");
    indicator:description("");
    indicator:requiredSource(core.Bar);
    indicator:type(core.Indicator);

    indicator.parameters:addGroup("Demand Index");
    indicator.parameters:addInteger("di_Length", "Period", "", 5);

    indicator.parameters:addGroup("CMF");
    indicator.parameters:addInteger("cmf_N", "Periods", "", 21, 1, 1000);
    indicator.parameters:addString("cmf_Method", "Method","", "CI");
    indicator.parameters:addStringAlternative("cmf_Method", "Method_Classic", "", "CS");
    indicator.parameters:addStringAlternative("cmf_Method", "Method_ClassicIncremental", "", "CI");
    indicator.parameters:addStringAlternative("cmf_Method", "TradeStation", "", "TS");

    indicator.parameters:addGroup("Spearman Rank Correlation");
    indicator.parameters:addInteger("src_Range", "Range", "", 14);
...
function Prepare(nameOnly)
    ...
    local profile = core.indicators:findIndicator("DEMAND INDEX");
    assert(profile ~= nil, "Please, download and install " .. "DEMAND INDEX" .. ".LUA indicator");
    di = core.indicators:create("DEMAND INDEX", source, instance.parameters.di_Length);

    local profile = core.indicators:findIndicator("CMF");
    assert(profile ~= nil, "Please, download and install " .. "CMF" .. ".LUA indicator");
    cmf = core.indicators:create("CMF", source, instance.parameters.cmf_N, instance.parameters.cmf_Method);

    local profile = core.indicators:findIndicator("SPEARMAN_RANK_CORRELATION");
    assert(profile ~= nil, "Please, download and install " .. "SPEARMAN_RANK_CORRELATION" .. ".LUA indicator");
    src = core.indicators:create("SPEARMAN_RANK_CORRELATION", source, instance.parameters.src_Range);
    indi_alerts:Prepare();
