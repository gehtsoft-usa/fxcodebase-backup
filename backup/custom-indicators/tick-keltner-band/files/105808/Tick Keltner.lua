...
-- http://fxcodebase.com/code/viewtopic.php?f=17&t=63381
...
function Init()
    indicator:name("Tick Keltner Band");
    ...
    indicator.parameters:addStringAlternative("SRC", "Median (Close + Previous Close)/2", "", "M1");
...
function Prepare(nameOnly) 
	...
	assert(core.indicators:findIndicator("TICK ADX") ~= nil, "Please, download and install TICK ADX.LUA indicator"); 
    ...
    elseif MV == "ATR" then
        name = name .. "ATR(" .. NB .. "), "
        ATR = core.indicators:create("TICK ADX", source, NB);
...
function Update(period, mode)
    if period >= source:first() then
        if SRC == "M1" then
            AS[period] = (source[period] + source[period-1]) / 2;
        elseif SRC == "M2" then
            AS[period] = (source[period] + source[period-1] ) / 1;
        end
        -- update the source smoothing indicator
        MI:update(mode);

        if MV == "AHL" then
            VM1[period] = source[period] - source[period-1];
            VMI:update(mode);
        elseif MV == "ATR" then
            ATR:update(mode);
        end
