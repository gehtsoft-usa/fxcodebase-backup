...
function Init()
    indicator:name("Time correlation indicator 2");
    indicator:description("Time correlation indicator 2");
    ...
    if LastTime~=source:date(period) then
     ...
     for i2=first,period-1,1 do
      ...
      if source.open[i2]>source.close[i2] then
       NegativeArray[f]=NegativeArray[f]+source.open[i2]-source.close[i2];
      elseif source.open[i2]<source.close[i2] then
       PositiveArray[f]=PositiveArray[f]+source.close[i2]-source.open[i2];
      end
     end 
