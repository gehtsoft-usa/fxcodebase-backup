...
--http://fxcodebase.com/code/viewtopic.php?f=17&t=70787
...
local wma2, wma, n, wmar, diff, n1, n2;
local source;
local indi_alerts = {};
...
{ 
    {
        ...
        UpCondition = function (period)
            if period == 0 or not n1:hasData(period - 1) or not n2:hasData(period - 1) then
                return false;
            end
            return core.crossesOver(n1, n2, period);
        end,
        DownCondition = function (period)
            if period == 0 or not n1:hasData(period - 1) or not n2:hasData(period - 1) then
                return false;
            end
            return core.crossesUnder(n1, n2, period);
        end,
...
function Init()
    indicator:name("Double Hull MA Cross");
    indicator:description("");
    indicator:requiredSource(core.Bar);
    indicator:type(core.Indicator);
    indicator.parameters:addInteger("n", "Period", "", 7);

    indicator.parameters:addColor("n1_color", "N1 Color", "Color", core.colors().Red);
    indicator.parameters:addInteger("n1_width", "N1 Width", "Width", 1, 1, 5);
    indicator.parameters:addInteger("n1_style", "N1 Style", "Style", core.LINE_SOLID);
    indicator.parameters:setFlag("n1_style", core.FLAG_LINE_STYLE);
    indicator.parameters:addColor("n2_color", "N2 Color", "Color", core.colors().Green);
    indicator.parameters:addInteger("n2_width", "N2 Width", "Width", 1, 1, 5);
    indicator.parameters:addInteger("n2_style", "N2 Style", "Style", core.LINE_SOLID);
    indicator.parameters:setFlag("n2_style", core.FLAG_LINE_STYLE);
...
function Prepare(nameOnly)
    ...
    wma2 = core.indicators:create("WMA", source, math.floor(n / 2 + 0.5));
    wma = core.indicators:create("WMA", source, n);
    diff = instance:addInternalStream(0, 0);
    wmar = core.indicators:create("WMA", diff, math.floor(math.sqrt(n) + 0.5));

    n1 = instance:addStream("n1", core.Line, "n1", "n1", instance.parameters.n1_color, 0, 0);
    n1:setWidth(instance.parameters.n1_width);
    n1:setStyle(instance.parameters.n1_style);
    n2 = instance:addStream("n2", core.Line, "n2", "n2", instance.parameters.n2_color, 0, 0);
...
function Update(period, mode)
    wma:update(mode);
    wma2:update(mode);
    if not wma.DATA:hasData(period) or not wma2.DATA:hasData(period) then
        return;
    end
    n2ma = 2 * wma2.DATA[period];
    nma = wma.DATA[period];
    diff[period] = n2ma - nma;
    wmar:update(mode);

    n1[period] = wmar.DATA[period];
    n2[period] = wmar.DATA[period - 1];
