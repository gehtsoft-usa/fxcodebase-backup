...
function Init()
    indicator:name("MTF MCP Simplified Pin Bar List");
    indicator:description("MTF MCP Simplified Pin Bar List");
...
--local SelectColor;
 ...
 local Indicator={};
 local BB,BP, NB,Filter, Period, Method, RangeFilter, RangePeriod,MinRange , RangeMethod;
...
-- Routine
 function Prepare(nameOnly)   
	...
	RangeMethod= instance.parameters.RangeMethod;
     ...
     assert(core.indicators:findIndicator("SIMPLIFIED PIN BAR") ~= nil, "Please, download and install SIMPLIFIED PIN BAR.LUA indicator");  
	 ...
	 for i = 1, Count, 1 do	
      ...
      Indicator[i]={};
		   ...
		   for j = 1, Num, 1 do	
			...
			Temp= core.indicators:create("RSI", source.close ,Period );
			first =  Temp.DATA:first()*2;  
		   Source[i][j]= core.host:execute("getSyncHistory", Pair[i], TF[j], source:isBid(),first+1,20000 + ID , 10000 +ID);
		   loading [i][j]=true;
		   
		   Indicator [i][j]= core.indicators:create("SIMPLIFIED PIN BAR", Source[i][j], BB,BP, NB,Filter, Period, Method, RangeFilter, RangePeriod,MinRange , RangeMethod, true);
