...
function Init()
    ...
    Parameters(8, "Kijun and Tenkan/Cloud")
end
...
function Logic(id, Data, period, DataLine)
    ...
    if period < Data:first() or period > Data:size() - 1 then
        return
    end

    if id == 6 or id == 7 then
        if Data[period] > DataLine[period] and Data[period - 1] <= DataLine[period - 1] then
        ...
        elseif Data[period] < DataLine[period] and Data[period - 1] >= DataLine[period - 1] then
        ...
        ----------------
        if source.close[period] > Data[period] and source.close[period - 1] <= Data[period - 1] then
        ...
        elseif source.close[period] < Data[period] and source.close[period - 1] >= Data[period - 1] then
...
function LogicAdvanced(id, SL, TL, period, SA, SB)
    ...
    if math.max(SA[period], SB[period]) < SL[period] and  math.max(SA[period-1], SB[period-1]) >= SL[period-1] and
       math.max(SA[period], SB[period]) < TL[period] and  math.max(SA[period-1], SB[period-1]) >= TL[period-1] then

        up[id]:set(period, math.max(SA[period], SB[period]), "\108")
        D[id] = nil
    ...
    elseif SL[period] < math.min(SA[period], SB[period])  and  SL[period-1] >= math.min(SA[period-1], SB[period-1]) and
           TL[period] < math.min(SA[period], SB[period])  and  TL[period-1] >= math.min(SA[period-1], SB[period-1]) then

        down[id]:set(period, math.min(SA[period], SB[period]), "\108")
        U[id] = nil
