...
--http://fxcodebase.com/code/viewtopic.php?f=17&t=70139
...
{ 
    {
        ...
        UpCondition = function (period)
            ...
            return core.crossesUnder(source.close, bb.TL, period);
        end,
        DownCondition = function (period)
            if period == 0 then
                return false;
            end
            return core.crossesOver(source.close, bb.BL, period);
        end,
...
local tl, bl;
function Init()
    indicator:name("BB Delayed Alert");
    ...
    indicator.parameters:addColor("tl_color", "TL Color", "Color", core.colors().Red);
    indicator.parameters:addInteger("tl_width", "TL Width", "Width", 1, 1, 5);
    indicator.parameters:addInteger("tl_style", "TL Style", "Style", core.LINE_SOLID);
    indicator.parameters:setFlag("tl_style", core.FLAG_LINE_STYLE);
    indicator.parameters:addColor("bl_color", "BL Color", "Color", core.colors().Red);
    indicator.parameters:addInteger("bl_width", "BL Width", "Width", 1, 1, 5);
    indicator.parameters:addInteger("bl_style", "BL Style", "Style", core.LINE_SOLID);
    indicator.parameters:setFlag("bl_style", core.FLAG_LINE_STYLE);
...
function Prepare(nameOnly)
    ...
    tl = instance:addStream("TL", core.Line, "TL", "TL", instance.parameters.tl_color, 0, 0);
    tl:setWidth(instance.parameters.tl_width);
    tl:setStyle(instance.parameters.tl_style);
    bl = instance:addStream("BL", core.Line, "BL", "BL", instance.parameters.bl_color, 0, 0);
    bl:setWidth(instance.parameters.bl_width);
    bl:setStyle(instance.parameters.bl_style);
...
function indi_alerts:AddParameters(parameters)
    ...
    indicator.parameters:addInteger("delay", "Delay in seconds", "", 3);
...
indi_alerts.SignalsToSend = {};
function indi_alerts:AsyncOperationFinished(cookie, success, message, message1, message2)
    if cookie == self._advanced_alert_timer then 
        if #self._alerts > 0 then
            if self._advanced_alert_key == nil then return; end 
            local data = self:ArrayToJSON(self._alerts); 
            self._alerts = {}; 
            local req = http_lua.createRequest(); 
            local query = string.format('{"Key":"%s","StrategyName":"%s","Platform":"FXTS2","Notifications":%s}', self._advanced_alert_key, string.gsub(self.StrategyName or "", '"', '\\"'), data); req:setRequestHeader("Content-Type", "application/json"); 
            req:setRequestHeader("Content-Length", tostring(string.len(query))); req:start("http://profitrobots.com/api/v1/notification", "POST", query); 
        end
        local time = core.host:execute("getServerTime");
        for i = #self.SignalsToSend, 1, -1 do
            local signal = self.SignalsToSend[i];
            if signal.SendAfter < time then
                indi_alerts:SoundAlert(signal.Sound);
                indi_alerts:EmailAlert(signal.Label, signal.Text, signal.Period);
                indi_alerts:SendAlert(signal.Label, signal.Text, signal.Period);
                if indi_alerts.Show then
                    indi_alerts:Pop(signal.Label, signal.Text);
                end
                table.remove(self.SignalsToSend, i);
            end
        end
    end 
end
        ...
        function alert:DownAlert(source, period, level, historical_period, metadata)
            ...
            if self.D ~= source:date(period) and period == source:size() - 1 - shift then
                ...
                if not historical_period then
                    local signal = {};
                    signal.SendAfter = core.host:execute("getServerTime") + instance.parameters.delay / 86400.0;
                    signal.Sound = self.Down;
                    signal.Label = self.Label;
                    signal.Text = text;
                    signal.Period = period;
                    indi_alerts.SignalsToSend[#indi_alerts.SignalsToSend + 1] = signal;
                end
            end
        end
