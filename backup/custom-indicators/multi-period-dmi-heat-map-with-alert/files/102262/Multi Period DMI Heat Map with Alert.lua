	...
	AlertNumber=0;
	 ...
	 Label[Number]="";
				...
				Label[Number]=Period[Number]; 
				 ...
				 DMI[Number] = core.indicators:create("DMI", source ,  Period[Number]);   
			   ...
			   x0, x1, x2 = context:positionOfBar (i);
			    ...
			    for j= 1, Number , 1 do
				  p=i;
										...
										if DMI[j].DIP:hasData(p) and DMI[j].DIP:hasData(p-1)										
										then 
										
										            
												    if DMI[j].DIP[p] >  DMI[j].DIM[p]  then		 
															if  DMI[j].DIP[p] >  Level[j] or not Use[j] then	
															    if   DMI[j].DIP[p] > DMI[j].DIP[p-1]   then
																C1=11
																C2=12;
																else
																C1=21;
													        ...
													        if  DMI[j].DIM[p] >  Level[j]  or not Use[j] then	
																if DMI[j].DIM[p] > DMI[j].DIM[p-1] then	
																C1=31;
																C2=32;
																else
																C1=41;
...
function Evaluate(j)	
	 ...
	 local   p= source:size()-1;
		...
		and DMI[j].DIP[p]> Level[j]
		then 
		Count[j]= 1;
		elseif DMI[j].DIP[p] < DMI[j].DIM[p]		
		and DMI[j].DIM[p]>Level[j]
		then 
		Count[j]= -1;
		end
