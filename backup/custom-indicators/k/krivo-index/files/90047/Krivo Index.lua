...
function Update(period, mode)
	...
	Sorted=BubbleSort(Num, 2,8, 3)
	--function BubbleSort(Data, index,columns, raw )
...
local iCount=0;
	...
	for i = 1, 8, 1 do
		...
		iCount=iCount+1;
	 	core.host:execute("drawLabel1", id, Size*10+(iCount-1)*Size*6  ,  core.CR_LEFT, Size*2  +Shift, core.CR_TOP, core.H_Left, core.V_Center, Bold, Color,  Sorted[1][i]);	
       ...
       local Data;
		...
		Data= Sorted[2][i];	
				...
				core.host:execute("drawLabel1", id, Size*10+(iCount-1)*Size*6  ,  core.CR_LEFT,   Size*4  +Shift, core.CR_TOP, core.H_Left, core.V_Center, Bold, Color,  "No Data"	);			  
			    id = id+1;	
				else
			
					 if Rainbow then					 
					core.host:execute("drawLabel1", id, Size*10+(iCount-1)*Size*6 ,  core.CR_LEFT,   Size*4  +Shift, core.CR_TOP, core.H_Left, core.V_Center, Bold,  Coloring (( 100/8)*i, 50),  	string.format("%." .. 2 .. "f", Data)	);			  
					id = id+1;	
					else 
					core.host:execute("drawLabel1", id, Size*10+(iCount-1)*Size*6  ,  core.CR_LEFT,  Size*4  +Shift, core.CR_TOP, core.H_Left, core.V_Center, Bold, Color,  	string.format("%." .. 2 .. "f", Data)	);			  
					id = id+1;	
					 end
				end
		end	
        ...
        if On[j] then
				 ...
				 crncy1, crncy2 = string.match(List[i], pauto);
					...
					if crncy1 == Pair[j] then
							...
							if SourceData[i].close[SourceData[i][Price]:size()-1] > Indicator[i].DATA[Indicator [i].DATA:size()-1] then
							Num[2][j] = Num[2][j] + 1;
							else
							Num[2][j] =  Num[2][j] - 1;
							end
