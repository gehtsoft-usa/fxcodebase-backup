...
function Init()
    indicator:name("Krivo Index Candles");
	...
	indicator.parameters:addDouble("Gate", "Gate", " " , 50);
...
function Prepare(nameOnly)
	...
	crn1, crn2 = string.match(source:instrument(), pauto);
end
...
function Update(period, mode)
    ...
    local res=0;
    for i = 1, 8, 1 do	
    	if Num[1][i]==crn1 then
    		res=res+Krivo[i][period];
    	elseif Num[1][i]==crn2 then
    		res=res-Krivo[i][period];
    	end
    end
	...
	if Buff[period]>instance.parameters.Gate then
		open:setColor(period, instance.parameters.Up);	
	elseif Buff[period]<-instance.parameters.Gate then
