...
--http://fxcodebase.com/code/viewtopic.php?f=17&t=69801
...
local source, KnoxRSI, high_prev_knox, high_price_knox, low_prev_knox, low_price_knox, macd;
local fast_prc_k, slow_prc_k, stoch_os, stoch_ob;

function f_top_fractal(_src, period)
    return _src[period - 4] < _src[period - 2] and _src[period - 3] < _src[period - 2] and _src[period - 2] > _src[period - 1] and _src[period - 2] > _src[period - 0];
end
function f_bot_fractal(_src, period)
    return _src[period - 4] > _src[period - 2] and _src[period - 3] > _src[period - 2] and _src[period - 2] < _src[period - 1] and _src[period - 2] < _src[period - 0];
end
function f_fractalize(_src, period)
    if f_top_fractal(_src, period) then
        return 1;
    end
    if f_bot_fractal(_src, period) then
        return -1;
    end
    return 0;
end
...
{ 
    {
        ...
        UpCondition = function (period)
            if period < 4 or not KnoxMOM.DATA:hasData(period - 4) then
                return false;
            end
            return f_fractalize(KnoxMOM.DATA, period) > 0
                and KnoxRSI.DATA[period] > 70
                and source.high[period - 2] > high_price_knox[period - 2]
                and KnoxMOM.DATA[period2] < high_prev_knox[period - 2];
        end,
        DownCondition = function (period)
            ...
            return f_fractalize(KnoxMOM.DATA, period) < 0 
                and KnoxRSI.DATA[period] < 30
                and source.low[period - 2] < low_price_knox[period - 2]
                and KnoxMOM.DATA[period - 2] > low_prev_knox[period - 2];
        end,
        ...
        OnChange = true,
        Name = "Knoxville - Triangle Divergence"
    },
    {
        Stage = 2,
        UpCondition = function (period)
            if period < 4 or not macd.MACD:hasData(period - 1) or not slow_prc_k.DATA:hasData(period) then
                return false;
            end
            return core.crossesOver(macd.MACD, 0, period)
                and slow_prc_k.DATA[period] < stoch_os;
        end,
        DownCondition = function (period)
            if period < 4 or not macd.MACD:hasData(period - 1) or not slow_prc_k.DATA:hasData(period) then
                return false;
            end
            return core.crossesUnder(macd.MACD, 0, period)
                and slow_prc_k.DATA[period] > stoch_ob;
        end,
        FormatMessage = function(source, period, level, label, isUp, metadata)
            return string.format(
                "Label: %s\013\010" ..
...
function Init()
    indicator:name("Knoxville Divergence");
    indicator:description("");
    indicator:requiredSource(core.Bar);
    indicator:type(core.Indicator);
    
    indicator.parameters:addInteger("KnoxRSILength", "Knoxville RSI", "", 17);
    indicator.parameters:addInteger("KnoxMOMLength", "Knoxville Mom", "", 20);
    indicator.parameters:addBoolean("KnoxDivergence", "Knoxville Divergences", "", true);
    indicator.parameters:addBoolean("KnoxTriangle", "Knoxville Divergence Triangles", "", true);

    indicator.parameters:addBoolean("ReversalTabs", "Reversal Tabs", "", true);

    indicator.parameters:addInteger("macd_fast_period", "Reversal Tabs - MACD Fast Period", "", 12);
    indicator.parameters:addInteger("macd_slow_period", "Reversal Tabs - MACD Slow Period", "", 26);
    indicator.parameters:addInteger("macd_signal_period", "Reversal Tabs - MACD Signal Period", "", 9);
    indicator.parameters:addInteger("stoch_period", "Reversal Tabs - Stochastic RSI Period", "", 70);
    indicator.parameters:addInteger("prc_k_period", "Reversal Tabs - %K Period", "", 30);
    indicator.parameters:addInteger("prc_d_period", "Reversal Tabs - %D Period", "", 30);
    indicator.parameters:addInteger("stoch_ob", "Reversal Tabs - Stochastic Overbought Level", "", 70);
    indicator.parameters:addInteger("stoch_os", "Reversal Tabs - Stochastic Oversold Level", "", 30);

    indicator.parameters:addColor("high_price_knox_color", "Bearish Divergence Lines Color", "Color", core.colors().Red);
    indicator.parameters:addInteger("high_price_knox_width", "Bearish Divergence Lines Width", "Width", 1, 1, 5);
    indicator.parameters:addInteger("high_price_knox_style", "Bearish Divergence Lines Style", "Style", core.LINE_SOLID);
    indicator.parameters:setFlag("high_price_knox_style", core.FLAG_LINE_STYLE);

    indicator.parameters:addColor("low_price_knox_color", "Bullish Divergence Lines Color", "Color", core.colors().Red);
    indicator.parameters:addInteger("low_price_knox_width", "Bullish Divergence Lines Width", "Width", 1, 1, 5);
    indicator.parameters:addInteger("low_price_knox_style", "Bullish Divergence Lines Style", "Style", core.LINE_SOLID);
    indicator.parameters:setFlag("low_price_knox_style", core.FLAG_LINE_STYLE);
...
function Prepare(nameOnly)
    ...
    stoch_os = instance.parameters.stoch_os;
    stoch_ob = instance.parameters.stoch_ob;
    KnoxRSI = core.indicators:create("RSI", source, instance.parameters.KnoxRSILength);
    local profile = core.indicators:findIndicator("MOMENTUM");
    assert(profile ~= nil, "Please, download and install " .. "MOMENTUM" .. ".LUA indicator");
    KnoxMOM = core.indicators:create("MOMENTUM", source, instance.parameters.KnoxMOMLength)

    high_prev_knox = instance:addInternalStream(0, 0);
    
    low_prev_knox = instance:addInternalStream(0, 0);
    if instance.parameters.KnoxDivergence then
        high_price_knox = instance:addStream("high_price_knox", core.Line, "Knoxville - Bearish Divergence Lines", "Knoxville - Bearish Divergence Lines", instance.parameters.high_price_knox_color, 0, -2);
        high_price_knox:setWidth(instance.parameters.high_price_knox_width);
        high_price_knox:setStyle(instance.parameters.high_price_knox_style);

        low_price_knox = instance:addStream("low_price_knox", core.Line, "Knoxville - Bullish Divergence Lines", "Knoxville - Bullish Divergence Lines", instance.parameters.low_price_knox_color, 0, -2);
        low_price_knox:setWidth(instance.parameters.low_price_knox_width);
        low_price_knox:setStyle(instance.parameters.low_price_knox_style);
    else
        high_price_knox = instance:addInternalStream(0, -2);
        low_price_knox = instance:addInternalStream(0, -2);
    end
    indi_alerts:Prepare();
    ...
    macd = core.indicators:create("MACD", source, instance.parameters.macd_fast_period, instance.parameters.macd_slow_period, instance.parameters.macd_signal_period)
    fast_prc_k = instance:addInternalStream(0, 0);
    stoch_period = instance.parameters.stoch_period;
    slow_prc_k = core.indicators:create("MVA", fast_prc_k, instance.parameters.prc_k_period);
end
...
function Update(period, mode)
    ...
    KnoxRSI:update(mode);
    KnoxMOM:update(mode);
    if period < 4 or not KnoxMOM.DATA:hasData(period - 4) or period < stoch_period then
        return;
    end
    local ll, hh = mathex.minmax(source, period - stoch_period, period);
    fast_prc_k[period] = (hh - ll) == 0 and 0 or 100 * (source.close[period] - ll) / (hh - ll);
    slow_prc_k:update(mode);
    
    local fractal_top_knox;
    if f_fractalize(KnoxMOM.DATA, period) > 0 then
        fractal_top_knox = KnoxMOM.DATA[period - 2];
        high_prev_knox[period - 2] = KnoxMOM.DATA[period - 2];
        high_price_knox[period - 2] = source.high[period - 2];
    else
        high_prev_knox[period - 2] = high_prev_knox[period - 3];
        high_price_knox[period - 2] = high_price_knox[period - 3];
    end
    local fractal_bot_knox;
    if f_fractalize(KnoxMOM.DATA, period) < 0 then
        fractal_bot_knox = KnoxMOM.DATA[period - 2];
        low_prev_knox[period - 2] = KnoxMOM.DATA[period - 2];
        low_price_knox[period - 2] = source.low[period - 2];
    else
        low_prev_knox[period - 2] = low_prev_knox[period - 3];
        low_price_knox[period - 2] = low_price_knox[period - 3];
    end
