...
local indi_name = "MTF MCP Multi Indicator List";
...
function CreateIndicators(source)
    ...
    local profile = core.indicators:findIndicator(instance.parameters:getString("Method"))
	local params = instance.parameters:getCustomParameters("Method")
	if profile:requiredSource() == core.Tick then
		indicators[#indicators + 1] = profile:createInstance(source.close, params);
    else
	    indicators[#indicators + 1] = profile:createInstance(source, params);
        indi:createInstance(source, p);
    end
...
function CreateParameters()
	indicator.parameters:addString("Method", "Indicator", "", "MVA")
	indicator.parameters:setFlag("Method", core.FLAG_INDICATOR)
end

function GetLastSignal(indi, source)
	if indi[1]:getStream(0):size() == 0 then
		return;
	end
    return indi[1]:getStream(0)[NOW], win32.formatNumber(indi[1]:getStream(0)[NOW], false, indi[1]:getStream(0):getDisplayPrecision());
end
