...
-- http://fxcodebase.com/code/viewtopic.php?f=17&t=69344
-- More information about this indicator can be found at:
--http://fxcodebase.com/code/viewtopic.php?f=17&t=69344
...
function Init()
    ...
    indicator.parameters:addString("StartTime", "Start Time for Trading", "", "03:30:00");
    indicator.parameters:addString("StopTime", "Stop Time for Trading", "", "10:00:00");
...
function GetDates(period)
    local initial_date = math.floor(source:date(period)) + OpenTime;
    for i = period - 1, 0, -1 do
        if initial_date ~= math.floor(source:date(i)) + OpenTime then
            local start_date = math.floor(source:date(i)) + OpenTime;
            local end_date = math.floor(source:date(i)) + CloseTime;
            if end_date < start_date then
                start_date = start_date - 1;
            end
            local start_period = core.findDate(ref, start_date, false);
            local end_period = core.findDate(ref, end_date, false);
            if start_period < 0 or end_period < 0 then
                return nil;
            end
            if start_period ~= end_period then
                return start_date, end_date, start_period, end_period;
            end
        end
    end
    return nil;
...
function Update(period, mode)
    ...
    local start_date, end_date, start_period, end_period = GetDates(period);
