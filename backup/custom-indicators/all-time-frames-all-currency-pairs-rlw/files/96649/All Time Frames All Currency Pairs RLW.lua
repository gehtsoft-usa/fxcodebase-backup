...
-- http://fxcodebase.com/code/viewtopic.php?f=17&t=61353
...
function Init()
    indicator:name("All Time Frames All Currency Pairs RLW");
    indicator:description("All Time Frames All Currency Pairs RLW");
	...
	indicator.parameters:addDouble("OB", "Overbought Level", "", -20 , -100, 0);
	indicator.parameters:addDouble("OS", "Oversold Level", "", -80 , 100, 0);
	...
	indicator.parameters:addInteger("Size", "Font Size (As % of Cell)", "", 55 , 0, 100);
	 ...
	 for i = 1, Count, 1 do	
      ...
      RLW[i]={};
		   ...
		   for j = 1, Num, 1 do	
			...
			Temp= core.indicators:create("RLW", source, Period   );
		   ...
		   RLW [i][j]= core.indicators:create("RLW", Source[i][j], Period);
...
function AsyncOperationFinished(cookie)
 ...
 local ID=0;
	...
	if not FLAG and cookie== 1 then
		...
		RLW[i][j]:update(core.UpdateLast);   
                 if loading [i][j] then
				 Loading= true; 
				 end
		end		 
...
function Draw(stage, context)
 ...
 function Calculate (context,i, j )
     ...
     if  not   RLW[i][j].DATA:hasData(RLW[i][j].DATA:size()-1)
     or  not   RLW[i][j].DATA:hasData(RLW[i][j].DATA:size()-2)  
	...
	local Symbol=string.format("%." .. 2 .. "f", RLW[i][j].DATA[RLW[i][j].DATA:size()-1]);
    local color1=Neutral;
    local color2=-1;	
	
	 if RLW[i][j].DATA[RLW[i][j].DATA:size()-1] >  RLW[i][j].DATA[RLW[i][j].DATA:size()-2] then
	   
		   
	  color1=UpColor;
	  elseif RLW[i][j].DATA[RLW[i][j].DATA:size()-1] <  RLW[i][j].DATA[RLW[i][j].DATA:size()-2]  then
	   ...
	   if RLW[i][j].DATA[RLW[i][j].DATA:size()-1] >  OB then
	   
		   
	  color2=OBColor;
	  elseif RLW[i][j].DATA[RLW[i][j].DATA:size()-1] < OS  then
