...
-- Routine
 function Prepare(nameOnly) 
   ...
   -- Dn = instance:addStream("Dn", core.Line, name .. ".Dn", "Dn", instance.parameters.Dn_color, first);
	...
	CE = instance:addStream("ChandelierExit", core.Line, name .. ".ChandelierExit", "ChandelierExit", instance.parameters.Up_color, first);
	CE:setWidth(instance.parameters.width);
    CE:setStyle(instance.parameters.style);
...
function Update(period, mode)
	...
	Trend[period]=Trend[period-1];
		...
		CE:setColor(period, instance.parameters.Up_color);
		else
		CE:setColor(period, instance.parameters.Dn_color);
		end
		
		if Trend[period]~= Trend[period-1] then
		CE:setBreak (period, true);
		end
