...
-- http://fxcodebase.com/code/viewtopic.php?f=17&t=67281
...
local indi_name = "Ichimoku Switch Dashboard";
local indi_id = "ICH";
...
function CreateIndicators(source)
    ...
    indicators[#indicators + 1] = core.indicators:create("ICH", source, 
...
function GetLastSignal(indi, source)
    for i = 0, indi[1].SA:size() - 1 do
        if core.crossesOver(indi[1].SA, indi[1].SB, NOW - i) then
            return 1, source:date(NOW - i), "B";
        end
        if core.crossesUnder(indi[1].SA, indi[1].SB, NOW - i) then
