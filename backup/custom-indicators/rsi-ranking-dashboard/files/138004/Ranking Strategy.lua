...
local STRATEGY_NAME = "Ranking Strategy";
function CreateParameters() 
    strategy.parameters:addString("indi", "Indicator", "", "RSI");
    strategy.parameters:setFlag("indi", core.FLAG_ONLYOSCILLATORS);
    strategy.parameters:addInteger("stream_index", "Stream Index", "", 1);
end

local indicators = {};
local stream_index;
function CreateEntryIndicators(source)
    stream_index = instance.parameters.stream_index;
    local profile = core.indicators:findIndicator(instance.parameters:getString("indi"))
    local params = instance.parameters:getCustomParameters("indi")
    if profile:requiredSource() == core.Tick then
        indicators[#indicators + 1] = core.indicators:create(instance.parameters:getString("indi"), source.close, params)
    else
        indicators[#indicators + 1] = core.indicators:create(instance.parameters:getString("indi"), source, params)
    end
end
...
local MAIN_SOURCE_ID = 3;
local TICK_SOURCE_ID = 2;
local MANDATORY_CLOSE_TIMER_ID = 1;
local entry_source_id;
local main_sources = {};
local base_size, Account, Amount, amount_type, AllowTrade, custom_id, AllowedSide;
...
function Prepare(nameOnly)
    ...
    local row = enum:next();
    while (row ~= nil) do
        local new_source = ExtSubscribe(MAIN_SOURCE_ID + #main_sources, row.Instrument, instance.parameters.timeframe, instance.parameters.type, "bar");
        main_sources[#main_sources + 1] = new_source
        CreateEntryIndicators(new_source);
        row = enum:next();
    end
...
local last_entry_buy, last_entry_sell;
function ExtUpdate(id, source, period)
    ...
    local shift = entry_execution_type == "Live" and 0 or 1;
    local max_val;
    local max_symbol;
    local min_val;
    local min_symbol;
    for i, indicator in ipairs(indicators) do
        local stream = indicator:getStream(stream_index - 1);
        if stream:size() == 0 or not stream:hasData(NOW - shift) then
            return;
        end
        if max_val == nil or max_val < stream[NOW - shift] then
            max_val = stream[NOW - shift];
            max_symbol = stream:instrument();
        end
        if min_val == nil or min_val > stream[NOW - shift] then
            min_val = stream[NOW - shift];
            min_symbol = stream:instrument();
        end
    end
    if last_entry_buy ~= min_symbol and not PositionsLimitHit() then
        if AllowTrade then
            OpenTrade("B", min_symbol);
        end
        --Signal("Entry long", main_source);
        last_entry_buy = min_symbol;
    end
    if last_entry_sell ~= max_symbol and not PositionsLimitHit() then
        if AllowTrade then
            OpenTrade("S", max_symbol);
        end
        --Signal("Entry short", main_source);
        last_entry_sell = max_symbol;
    end
end
...
function OpenTrade(side, symbol)
    ...
    base_size = core.host:execute("getTradingProperty", "baseUnitSize", symbol, Account);
