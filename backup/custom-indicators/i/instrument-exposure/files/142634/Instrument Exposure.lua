...
-- https://fxcodebase.com/code/viewtopic.php?f=17&t=71304
...
function Init()
    indicator:name("Instrument Exposure");
	...
	indicator.parameters:addString("BuySell", " BuySell","" , "Any");
    indicator.parameters:addStringAlternative("BuySell", "Any", "Any" , "Any");
    indicator.parameters:addStringAlternative("BuySell", "Long", "Long" , "B"); 
    indicator.parameters:addStringAlternative("BuySell", "Short", "Short" , "S"); 
...
function AsyncOperationFinished(cookie)
    ...
    if cookie== 1 then
			...
			while row ~= nil do
				...
				and ((BuySell~="Any" and row.BS == BuySell) or BuySell=="Any"  ) then
					count = count + 1;
					TS=TS+row.Lot*BaseSize ;
				end
...
function Draw(stage, context)
 ...
 return;
	...
	local Text1=  "Number of Position : " .. tostring(NumberOfPositions)
   ...
   local Text1=  "Total Size : " .. tostring(TotalSize);
   local i=2;	 
   width, height = context:measureText (1, Text1, 0);
   context:drawText (1,  Text1, Label, -1,  iX(context,width,0,1) ,  iY(context,height,i,0) ,iX(context,width,0,2),iY(context,height,i,1), 0 );	
