...
function Activate (id, period)
    ...
    if id == 8 and ON[id] then
        LogicAdvanced(id, CS, period, SA, SB)
    end
...
function LogicAdvanced(id, CS, period, SA, SB)
    local Shift = 0
    ...
    if (period < CS:first() or period > CS:size() - 1) or
        not SA:hasData(period-1) or  not SB:hasData(period-1)then
        return
    end

    if math.max(SA[period], SB[period]) < CS[period] and  math.max(SA[period-1], SB[period-1]) >= CS[period-1] then
    ...
    elseif CS[period] < math.min(SA[period], SB[period])  and  CS[period-1] >= math.min(SA[period-1], SB[period-1]) then
