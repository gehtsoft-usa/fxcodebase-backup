...
-- http://fxcodebase.com/code/viewtopic.php?f=17&t=68388
...
local indi_name = "Indicator Match Price Dashboard";
...
function CreateIndicators(source)
    ...
    assert(core.indicators:findIndicator(instance.parameters.indi) ~= nil, instance.parameters.indi .. " indicator must be installed");
    indicators[#indicators + 1] = core.indicators:create(instance.parameters.indi, source, instance.parameters:getCustomParameters("indi"));
...
function CreateParameters(parameters)
    ...
    indicator.parameters:addString("indi", "Indicator", "", "EMA");
    indicator.parameters:setFlag("indi", core.FLAG_ONLYINDICATORS);
end

function GetLastSignal(indi, source)
    local pipsize = source:pipSize();
    local start = live and 0 or 1;
    for i = start, indi[1].DATA:size() - 1 do
        local indiPrice = math.floor(indi[1].DATA[NOW - i] / pipsize);
        local sourcePrice = math.floor(source.close[NOW - i] / pipsize);
        if indiPrice == sourcePrice then
