-- Available @ https://fxcodebase.com/code/viewtopic.php?f=17&t=76006
-- +------------------------------------------------------------------------------------------------+
...
{ 
    {
        ...
        UpCondition = function (period)
            local date = ST1.DATA:date(period);
            local btfPeriod = core.findDate(ST2.DATA, date, false);
            return ST1.DATA:colorI(period) == core.rgb(0, 255, 0) and ST1.DATA:colorI(period - 1) ~= core.rgb(0, 255, 0) and
                ST2.DATA:colorI(btfPeriod) == core.rgb(0, 255, 0) and
                ST2.DATA:colorI(btfPeriod - 1) ~= core.rgb(0, 255, 0);
        end,
        DownCondition = function (period)
            local date = ST1.DATA:date(period);
            local btfPeriod = core.findDate(ST2.DATA, date, false);
            return ST1.DATA:colorI(period) == core.rgb(255, 0, 0) and ST1.DATA:colorI(period - 1) ~= core.rgb(255, 0, 0) and
                ST2.DATA:colorI(btfPeriod) == core.rgb(255, 0, 0) and
                ST2.DATA:colorI(btfPeriod - 1) ~= core.rgb(255, 0, 0);
        end,
...
function Init()
    indicator:name("Two Time Frame Super Trend");
    indicator:description("");
    indicator:requiredSource(core.Bar);
    indicator:type(core.Indicator);
    
    indicator.parameters:addInteger("N1", "Number of periods", "", 10)
    indicator.parameters:addDouble("M1", "Multiplier", "", 1.5)
    indicator.parameters:addGroup("2. Time Fame Calculation")
    indicator.parameters:addString("TF2", "Time frame", "", "m15")
    indicator.parameters:setFlag("TF2", core.FLAG_PERIODS)
    indicator.parameters:addInteger("N2", "Number of periods", "", 10)
    indicator.parameters:addDouble("M2", "Multiplier", "", 1.5)
...
function Prepare(nameOnly)
    ...
    ST1 = core.indicators:create("ST", source, instance.parameters.N1, instance.parameters.M1, core.rgb(0, 255, 0), core.rgb(255, 0, 0))

    Source2 = sources:Request(2, source, instance.parameters.TF2)
    ST2 = core.indicators:create("ST", Source2, instance.parameters.N2, instance.parameters.M2, core.rgb(0, 255, 0), core.rgb(255, 0, 0))
