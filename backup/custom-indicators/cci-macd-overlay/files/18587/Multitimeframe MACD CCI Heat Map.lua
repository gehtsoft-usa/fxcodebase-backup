...
--+------------------------------------------------------------------+
 ...
 function Add(id, TF,Flag, Instrument )
	...
	indicator.parameters:addGroup(id .. ". CCI Calculation");
    indicator.parameters:addInteger("CN"..id, "CCI Period", "", 14, 2, 1000);

	indicator.parameters:addGroup(id .. ". MACD Calculation");
	indicator.parameters:addInteger("SN"..id, "Short Period", "", 12, 1, 2000);
    indicator.parameters:addInteger("LN"..id, "Long Period", "", 26, 1, 2000);
    indicator.parameters:addInteger("IN"..id, "Signal Period", "", 9, 1, 2000);	
...
function Init()
    indicator:name("Multitimeframe MACD CCI Heat Map");
   ...
   Last=nil;
	...
	s2, e2 = core.getcandle(iTF[i], 0, 0, 0);
	 ...
	 Number=Number+1;
			...
			CN[Number]=instance.parameters:getInteger("CN".. i);
	 ...
	 Label[Number]="";
                ...
                Label[Number]=Label[Number] .. " - " ..  TF[Number];				
			  	...
			  	Temp1= core.indicators:create("MACD", source[PriceType] ,   SN[Number],  LN[Number], IN[Number]);  
                Temp2= core.indicators:create("CCI", source ,  CN[Number]);  					
				ifirst= math.max(Temp1.DATA:first(), Temp1.DATA:first())*2;
			   ...
			   MACD[Number] = core.indicators:create("MACD", SourceData[Number][PriceType],  SN[Number],  LN[Number], IN[Number]  );		
		       CCI[Number] = core.indicators:create("CCI", SourceData[Number] , CN[Number]  );
			   ...
			   x0, x1, x2 = context:positionOfBar (i);
			    ...
			    for j= 1, Number , 1 do
				  ...
				  if p~= false then
						...
						--if s.MACD.MACD[p] >  s.MACD.SIGNAL[p]
					--and   s.CCI.DATA[p] >  0
								
										if MACD[j].DATA:hasData(p) and CCI[j].DATA:hasData(p) then 
										
												    if MACD[j].MACD[p] > MACD[j].SIGNAL[p]
													and CCI[j].DATA[p] > 0
													then		 
												    ...
												    elseif MACD[j].MACD[p] < MACD[j].SIGNAL[p]
													and CCI[j].DATA[p] < 0 
													 then       
