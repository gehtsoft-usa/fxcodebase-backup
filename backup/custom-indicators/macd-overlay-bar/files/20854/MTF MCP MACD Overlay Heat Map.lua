...
--+------------------------------------------------------------------+
 ...
 function Add(id, TF,Flag, Instrument )
    ...
    indicator.parameters:addString("Method" .. id, "Indicator Method", "Method" , "MACD / Zero & MACD Slope");
    indicator.parameters:addStringAlternative("Method" .. id, "MACD / Zero & MACD Slope", "MACD / Zero & MACD Slope" , "MACD / Zero & MACD Slope");
    indicator.parameters:addStringAlternative("Method" .. id, "MACD / Signal", "MACD / Signal" , "MACD / Signal");
    indicator.parameters:addStringAlternative("Method" .. id, "Histogram/Zero & Histogram Slope", "Histogram/Zero & Histogram Slope" , "Histogram/Zero & Histogram Slope");
    indicator.parameters:addStringAlternative("Method" .. id, "MACD/Signal & MACD/Zero", "MACD/Signal & MACD/Zero" , "MACD/Signal & MACD/Zero");
...
function Init()
    indicator:name("MTF MCP MACD Overlay Heat Map");
 ...
 function Prepare(nameOnly)   
   ...
   instance:ownerDrawn(true);
	...
	s2, e2 = core.getcandle(iTF[i], 0, 0, 0);
	 ...
	 SignalMethod[Number]= instance.parameters:getString("Method" .. i);
	 ...
	 Label[Number]="";
                ...
                Label[Number]=Label[Number] .. " - " ..  TF[Number];				
			  	...
			  	Temp1= core.indicators:create("MACD", source [Price[Number]],SN[Number],LN[Number],IN[Number]);  				
				ifirst= Temp1.DATA:first()*2;
				 ...
				 loading[Number]  = true;  	 
				 MACD[Number] = core.indicators:create("MACD", SourceData[Number][Price[Number]],SN[Number],LN[Number],IN[Number]);   
			   ...
			   x0, x1, x2 = context:positionOfBar (i);
			    ...
			    for j= 1, Number , 1 do
				  ...
				  if p~= false then
										...
										if MACD[j].HISTOGRAM:hasData(p) and MACD[j].HISTOGRAM:hasData(p-1) then 
										
												     
													 
													 ------------------------------
												if SignalMethod[j]== "MACD / Zero & MACD Slope" then
														if MACD[j].MACD[p] > 0  then
														 if MACD[j].MACD[p] > MACD[j].MACD[p-1] then
														   C1=11; C2=12;		
														 else
														     C1=21; C2=22;		
														 end
												...
												if SignalMethod[j]== "MACD / Signal" then
														if MACD[j].MACD[p] > MACD[j].SIGNAL[p] then
														   C1=11; C2=12;		
														else
														    C1=41; C2=42;		
														end
												...
												if SignalMethod[j]== "Histogram/Zero & Histogram Slope" then
														if MACD[j].HISTOGRAM[p] > 0 then
															  if MACD[j].HISTOGRAM[p] > MACD[j].HISTOGRAM[p-1] then
															   C1=11; C2=12;		
															 else
															 C1=21; C2=22;		
															 end
												...
												if SignalMethod[j]== "MACD/Signal & MACD/Zero" then
													if MACD[j].MACD[p] > MACD[j].SIGNAL[p]  then
