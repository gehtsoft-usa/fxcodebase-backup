...
-- https://fxcodebase.com/code/viewtopic.php?f=17&t=71853
...
function Init()
    indicator:name("Optimal Weighted Moving Average");
...
-- Routine
 function Prepare(nameOnly)   
    ...
    Line = instance:addStream("Line", core.Line, name, "Line", instance.parameters.Up, source:first() );
...
function Update(period, mode)
	...
	local corr = mathex.correl (source, Line, period-Length+1, period, period-Length+1, period)

    local sum = 0.0;
	local weightSum = 0.0;	
 
    for i = 0 ,  Length - 1, 1 do	
    weight = math.pow(Length - i, corr)
	...
	Line[period]= sum / weightSum;
	
	if source[period]> Line[period] then
