...
function Draw(stage, context)
    ...
    FontSize=math.min(xGap/2, yGap);

	context:createFont(7, "Arial", FontSize, FontSize, 0)
...
function Calculate(context, i ,j )
	...
	if i == Count then
		...
		context:drawRectangle(Edge, 1, x1+Shift, y1, x2+Shift, y2, transparency)
		elseif Source[i].close[period]< Source[i].open[period] then
		context:drawRectangle(Edge, 2, x1+Shift, y1, x2+Shift, y2, transparency)
		else
		context:drawRectangle(Edge, 3, x1+Shift, y1, x2+Shift, y2, transparency)
		end
