...
function Prepare(nameOnly)   
    ...
    Buff1 = instance:addStream("Buff1", core.Line, name .. ".Buff1", "Buff1", instance.parameters.clr_buff1, first+4);
    Buff1:setPrecision(math.max(2, instance.source:getPrecision()));
    Buff2 = instance:addStream("Buff2", core.Line, name .. ".Buff2", "Buff2", instance.parameters.clr_buff2, first+4);	
...
function Update(period, mode)
    ...
    if (period>first+4) then
     ...
     if period<first+8 then
      Buff1[period]=(source[period]-2.*source[period-1]+source[period-2])/4.;
     else
