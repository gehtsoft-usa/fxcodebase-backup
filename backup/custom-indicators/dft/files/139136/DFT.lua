...
function Prepare(nameOnly)
    ...
    leh = instance:addInternalStream(source:first() + 3, 0);
    lel = instance:addInternalStream(source:first() + 3, 0);
    hbtrue = instance:addInternalStream(source:first() + 3, 0);
    hftrue = instance:addInternalStream(source:first() + 3, 0);
    lftrue = instance:addInternalStream(source:first() + 3, 0);
    atr = core.indicators:create("ATR", source, per);
    local profile = core.indicators:findIndicator("DNC");
    assert(profile ~= nil, "Please, download and install " .. "DNC" .. ".LUA indicator");
    dnc = core.indicators:create("DNC", source, per, "HighLow");
...
function evpup(period)
    return source.high[period] > dnc.DU[period - 1];
end

function evpdown(period)
    return source.low[period] < dnc.DN[period - 1];
end
...
function Update(period, mode)
    ...
    phb[period] = dnc.DU[period]; 
    plb[period] = dnc.DN[period]; 
    local dist = dnc.DU[period] - dnc.DN[period];
    pchf[period] = dnc.DU[period] - dist * 0.382;
    pclf[period] = dnc.DU[period] - dist * 0.618;
    hbtrue[period] = 1;
    local tol = atr.DATA[period] * 0.2;
    local evhbstart = dnc.DU[period - 3] == dnc.DU[period - 2] and dnc.DU[period - 2] == dnc.DU[period - 1] and evpup(period);
    local evlbstart = dnc.DN[period - 3] == dnc.DN[period - 2] and dnc.DN[period - 2] == dnc.DN[period - 1] and evpdown(period);
    evhb = evhbstart or source.high[period - 1] == dnc.DU[period];
    local hf = dnc.DU[period] - dist * 0.236;
    leh[period] = leh[period - 1];
    if evhb then
        leh[period] = hf;
    end
    
    evlb = evlbstart or source.low[period - 1] == dnc.DN[period];
    local lf = dnc.DU[period] - dist * 0.764;
    ...
    if hbdtrue then
        schigh[period] = dnc.DU[period]
        sclow[period] = hf
        if (evpup(period) or evpup(period - 1)) then
            prcahigh = dnc.DU[period] + pred;
            ps9:set(period, dnc.DU[period]+tol, "\116");
        end
        ps5:set(period, dnc.DU[period], "\161");
        phf[period] = hf;
    elseif lbdtrue then
        schigh[period] = lf
        sclow[period] = dnc.DN[period];
        if (evpdown(period) or evpdown(period - 1)) then
            prcalow = dnc.DN[period] - pred;
            ps10:set(period, dnc.DN[period]-tol, "\116");
        end
        ps6:set(period, dnc.DN[period], "\161");
        plf[period] = lf;
