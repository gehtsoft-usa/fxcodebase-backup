...
function Init()
    indicator:name("Firestorm Trend Chaser");
	...
	indicator.parameters:addBoolean("Show", "Show MAs", "Show", true);	
...
-- Routine
 function Prepare(nameOnly)   
	...
	Count=Count+1;
	Indicator[i]= core.indicators:create(avg_type, source.close, annual_avg-(i-1)*month_step_down);
	end

	ATR= core.indicators:create("ATR", source, Periods);
...
function Update(period, mode)
	...
	up[period] = source.close[period] - Multiplier * ATR.DATA[period]; 
	if  source.close[period-1] > up[period-1] then
	up[period]=math.max(up[period], up[period-1])
	end
	
	dn[period] = source.close[period] + Multiplier * ATR.DATA[period]; 
	...
	if trend[period] == -1 and source.close[period] > dn[period-1]  then
	trend[period]=1;
	elseif trend[period] == 1 and source.close[period] < up[period-1]  then
	trend[period]=-1;
	end
