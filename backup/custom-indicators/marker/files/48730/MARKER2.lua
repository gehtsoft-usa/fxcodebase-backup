...
function Init()
	...
	indicator.parameters:addBoolean("ShowLine", "Show vertical line", "", true);
	indicator.parameters:addColor("LineColor", "Vertical line color", "", core.rgb(255, 255, 0));
			...
			if Mode == "Trailing" then
						...
						if source:serial(source:size()-1) ~= LAST  and period == source:size()-1 then
						   ...
						   LAST  = source:serial(source:size()-1);
								...
								if ShowLine then
								        core.host:execute ("drawLine", 1, source:date(period-LookBack), 0, source:date(period-LookBack), 1000, instance.parameters.LineColor, instance.parameters.styleLinReg, instance.parameters.widthLinReg);
								end
