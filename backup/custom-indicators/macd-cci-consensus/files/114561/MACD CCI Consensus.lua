...
local first;
local source = nil;
local open, close, buy, sell, neutral;
local Parameters={}; 
local Indicator={};
...
-- Routine
 function Prepare(nameOnly)   
   ...
   if   (nameOnly) then
    ...
    buy = instance:createTextOutput ("buy", "buy", "Wingdings", Size, core.H_Center, core.V_Top, instance.parameters.Up, first);
    core.host:execute ("attachTextToChart", "buy");
    
    sell = instance:createTextOutput ("sell", "sell", "Wingdings", Size, core.H_Center, core.V_Bottom, instance.parameters.Dn, first);
    core.host:execute ("attachTextToChart", "sell");

    neutral = instance:createTextOutput ("neutral", "neutral", "Wingdings 2", Size, core.H_Center, core.V_Top, instance.parameters.No, first);
    core.host:execute ("attachTextToChart", "neutral");
...
function Update(period, mode)
    ...
    CurrentColor = instance.parameters.No
	...
	local TWO=nil;
		...
		then
            open:setColor(period, instance.parameters.No);	   
		elseif (ONE == true or not One)and (TWO  == true  or not Two) then		
            open:setColor(period, instance.parameters.Up);
        elseif(ONE == false or not One) and  (TWO == false or not Two) then
            open:setColor(period, instance.parameters.Dn);
		else
            open:setColor(period, instance.parameters.No);			
		end 
		 
		neutral:setNoData (period-1);
		buy:setNoData (period-1);
		sell:setNoData (period-1);
		
        if open:color(period) ~= open:color(period - 1) then
            local color = open:color(period);
            if (color == instance.parameters.No) then                
                neutral:set(period -1, source.high[period - 1], "\205");               
            elseif (color == instance.parameters.Up) then
                buy:set(period - 1, source.high[period - 1], "\226");
            elseif (color == instance.parameters.Dn) then
                sell:set(period - 1, source.low[period - 1], "\225");
            end            
        end 
end
