...
function Prepare(nameOnly)
   ...
   -- D = instance:addStream("D", core.Bar, name .. ".D", "D", instance.parameters.D_color, first);
    ...
    H = instance:addStream("H", core.Bar, name .. ".H", "H", instance.parameters.U_color, first);
  ...
  --  F = instance:addInternalStream(first, 0);
    ...
    IMA1 = core.indicators:create(MA1M, H, MA1N);
...
function Update(period, mode)
    if period >= first then
        ...
        O[period] = v;
        H[period] = 0.5 * math.log((1 + v) / (1 - v)) + 0.5 * F1;
       
        if  H[period] > 0 then
        H:setColor(period, instance.parameters.U_color);  
        else
        H:setColor(period, instance.parameters.D_color); 
        end
    end
