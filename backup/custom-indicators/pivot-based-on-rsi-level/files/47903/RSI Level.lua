...
function Init()
    indicator:name("Waddah Attar RSI Level");
...
function Update(period, mode)
	   ...
	   Indicator:update(mode); 	
		...
		local i;
			...
			for i = 1, 11, 1 do
			RSI[i][period] = ((((i*10) - rsi1)*dc) / drsi) + c1;
			end
		 ...
		 for i = 1, 11, 1 do
		 host:execute("drawLabel", i, source:date(period) + Size*5, RSI[i][period],  tostring( ((i-1)*10) .."   ("..string.format("%." .. source:getPrecision () .. "f", RSI[i][period])) .. ")" ); 
		 end
