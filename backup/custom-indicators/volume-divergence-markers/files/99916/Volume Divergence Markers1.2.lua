	...
	return;
	  ...
	  return;
	    ...
	    if   CalculateVolume (period,dir) then 
             core.host:execute("drawLabel1", source:serial(period), source:date(period),core.CR_CHART, source.close[period] , core.CR_CHART, core.H_Center, core.V_Center,   font , color, "\226"); 
		else
             core.host:execute("drawLabel1", source:serial(period), source:date(period),core.CR_CHART, source.close[period] , core.CR_CHART, core.H_Center, core.V_Center,   font , color, "\225"); 
		end

	    if  CalculateVolume (period,dir) == nil then
	    core.host:execute ("removeLabel", source:serial(period));
	    return;
	    end
