...
function Prepare(nameOnly)
    ...
    local name = profile:id() .. "(" .. source:name() .. ", " .. instance.parameters.SmoothPeriod .. ", " .. instance.parameters.ROCPeriod .. ", " .. instance.parameters.TypeSmooth .. ")";
    instance:name(name);
    if nameOnly then
        return;
    end
    HL = instance:addInternalStream(0, 0);
    if instance.parameters.TypeSmooth==0 then
     MA = core.indicators:create("MVA", HL, instance.parameters.SmoothPeriod);
    else
     MA = core.indicators:create("EMA", HL, instance.parameters.SmoothPeriod);
    end 
