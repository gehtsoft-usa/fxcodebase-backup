	 ...
	 for i = 1, Count, 1 do	
      ...
      Indicator[i]={};
		   ...
		   for j = 1, Num, 1 do	
			...
			Temp= core.indicators:create("MACD", source[Price], MACD_Fast, MACD_Slow, MACD_MA, Up, Down, Neutral);
			first =  Temp.HISTOGRAM:first();  
		   Source[i][j]= core.host:execute("getSyncHistory", Pair[i], TF[j], source:isBid(),math.min(first*2,300),20000 + ID , 10000 +ID);
		   loading [i][j]=true;
		   
		   Indicator [i][j]= core.indicators:create("MACD", Source[i][j][Price], MACD_Fast, MACD_Slow, MACD_MA, Up, Down, Neutral);
...
function Draw(stage, context)
 ...
 function Calculate (context,i, j )
     ...
     if  not   Indicator[i][j].SIGNAL:hasData(Indicator[i][j].SIGNAL:size()-1)
     or  not   Indicator[i][j].SIGNAL:hasData(Indicator[i][j].SIGNAL:size()-2)  
    ...
    local color=Neutral;	
	      ...
	      if Indicator[i][j].SIGNAL[Indicator[i][j].SIGNAL:size()-1]< Indicator[i][j].MACD[Indicator[i][j].MACD:size()-1]
		  and Indicator[i][j].MACD[Indicator[i][j].MACD:size()-1] > 0 
		  then
		  Symbol="\225"; 
		  color=Up;
		  elseif Indicator[i][j].SIGNAL[Indicator[i][j].SIGNAL:size()-1]> Indicator[i][j].MACD[Indicator[i][j].MACD:size()-1]
		  and Indicator[i][j].MACD[Indicator[i][j].MACD:size()-1] < 0 
		  then
		   color=Down;
		  Symbol="\226"; 
