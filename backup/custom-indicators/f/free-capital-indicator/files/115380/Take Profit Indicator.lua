...
local LimitPips = 0
local LimitAccountCurrency = 0

function AsyncOperationFinished(cookie)
    if cookie == 1 then
        LimitPips, LimitAccountCurrency = Stop_Calculation()
    end
end
...
function Stop_Calculation()
    ...
    while row ~= nil do
        if row.AccountID == Account then
            ...
            if row.Limit ~= 0 then
                Pips = Pips + (math.abs(row.Open - row.Limit) / Point)
                Currency = Currency + (row.Lot / base_size) * (math.abs(row.Open - row.Limit) / Point) * PipCost
            end
        end
...
function Draw(stage, context)
    ...
    local Text1 =
        "Pips : " ..
        win32.formatNumber(LimitPips, false, 2) ..
            " Account Currency : " .. win32.formatNumber(LimitAccountCurrency, false, 2)
