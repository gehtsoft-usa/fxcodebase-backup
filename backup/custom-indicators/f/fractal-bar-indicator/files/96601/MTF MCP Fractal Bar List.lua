	 ...
	 for i = 1, Count, 1 do	
      ...
      Indicator[i]={};
		   ...
		   for j = 1, Num, 1 do	
			...
			Temp= core.indicators:create("FRACTAL BAR INDICATOR", source , Up,Down,Neutral, true );
			first =  Temp.DATA:first();  
		   Source[i][j]= core.host:execute("getSyncHistory", Pair[i], TF[j], source:isBid(),first+1,20000 + ID , 10000 +ID);
		   loading [i][j]=true;
		   
		   Indicator [i][j]= core.indicators:create("FRACTAL BAR INDICATOR", Source[i][j], Up,Down,Neutral, true);
