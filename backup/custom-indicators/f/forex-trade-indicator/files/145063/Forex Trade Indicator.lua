...
-- https://fxcodebase.com/code/viewtopic.php?f=17&t=71890
...
function Init()
    ...
    indicator.parameters:addInteger("nPeriod", "nPeriod", "", 9, 1, 2000);
    indicator.parameters:addDouble("factor_1", "factor_1 MA", "", 0.6666666, 0, 2000);
    indicator.parameters:addDouble("factor_2", "factor_2", "", 0.3333333 , 0, 2000);	
...
local first;
local source = nil;
local nPeriod, factor_1,factor_2; 
local Indicator;
...
-- Routine
 function Prepare(nameOnly)   
    ...
    local name = profile:id() .. "(" ..  instance.source:name().. "," ..  nPeriod.. "," ..  factor_1 .. "," ..  factor_2 .. ")";
    instance:name(name); 
...
function Update(period, mode)
	 ...
	 local min,max=mathex.minmax(source, period-nPeriod+1, period);
         ...
         if (max-min ~= 0) then RSV = (source.close[period]-min)/(max-min)*100; else RSV = 50; end
		 
		 

         if (period == first) then
            Line1[period] = factor_1 * 50 + factor_2 * RSV;
            Line2[period] = factor_1 * 50 + factor_2 * Line1[period];
         else  
            Line1[period] = factor_1 * Line1[period-1] + factor_2 * RSV;
            Line2[period] = factor_1 * Line2[period-1] + factor_2 * Line1[period];
         end
		 
         Line3[period] = 3 * Line2[period] - 2 * Line1[period];
