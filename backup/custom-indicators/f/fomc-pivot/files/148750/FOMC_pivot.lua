    ...
    for i = instance.source:size() - 1, 0, -1 do
        ...
        if rate ~= nil then
            ...
            elseif last_rate ~= rate then
                local to_x = context:positionOfBar(last_period);
                ...
                context:drawLine(main_pen, to_x, y, from_x, y);
				if Vertical then
                context:drawLine(main_pen, from_x , context:top (), from_x , context:bottom ());
				end
                count = count + 1;
