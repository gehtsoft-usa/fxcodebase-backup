-- Available @ http://fxcodebase.com/code/viewtopic.php?f=17&t=64929
...
function Init()
    indicator:name("Custom Time Frame Candle Tick View");
    indicator:description("A chart will add a new Candle After passing of Set Number of Base Tick Time Frame elements.");
    ...
    indicator.parameters:addInteger("BarSize", "Size of bar in second", "", 10);
    ...
    indicator.parameters:addDate("from", "From","", -10);
...
function Prepare(onlyName)
    ...
    FIRST=true;
    BarSizeSec = instance.parameters.BarSize;
	TF = instance.parameters.TF;
	Instrument = instance.parameters.Instrument;

    local name = profile:id().. ", " .. Instrument .. ", " .. BarSizeSec .. "T" 
    instance:name(name);
    ...
    instance:createCandleGroup("candle", "candle", open, high, low, close, TF);
    firstBarSec = 0;
end
...
function calcValue( Index, period)
    ...
    local currentBarSec = math.floor(History:date(period)*86400);        
    local diff =  currentBarSec - firstBarSec;
	
    if diff >= BarSizeSec  then    
        NEW=true;
	end
    ...
    if  NEW then   
        ...
        Index = Index + 1;
        firstBarSec = math.floor(History:date(period)*86400); 
