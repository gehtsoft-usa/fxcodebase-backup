...
{ 
    {
		...
		UpCondition = function (period)
			return period > 0 and core.crossesOver(Bid, Ask, period);
        end,
		DownCondition = function (period)
			return period > 0 and core.crossesUnder(Bid, Ask, period);
        end,
...
function Update(period, mode)
	...
	Bid[period] = (source.volume[period] / (2 - Oscillator[period]))
	Ask[period] = (source.volume[period] - Bid[period])
