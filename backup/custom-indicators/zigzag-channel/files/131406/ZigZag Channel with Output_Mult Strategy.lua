...
local STRATEGY_NAME = "ZigZag Channel with Output_Mult Strategy";
function CreateParameters() 
    ...
    strategy.parameters:addInteger("Period", "Period", "Period", 1)
    strategy.parameters:addBoolean("use_poolback", "Use poolback", "", true);
    strategy.parameters:addDouble("pullback", "Pullback, %", "", 10);
end

local indi;
function CreateEntryIndicators(source)
    local profile = core.indicators:findIndicator("ZIGZAG CHANNEL WITH OUTPUT_MULT WITH ARROWS");
    assert(profile ~= nil, "Please, download and install " .. "ZIGZAG CHANNEL WITH OUTPUT_MULT WITH ARROWS" .. ".LUA indicator");
    local indicatorParams = profile:parameters();
    indicatorParams:setInteger("Depth", instance.parameters.Depth);
    indicatorParams:setInteger("Deviation", instance.parameters.Deviation);
    indicatorParams:setInteger("Backstep", instance.parameters.Backstep);
    indicatorParams:setInteger("Period", instance.parameters.Period);
    indicatorParams:setDouble("pullback", instance.parameters.pullback);
    indicatorParams:setBoolean("use_poolback", instance.parameters.use_poolback);
    indicatorParams:setBoolean("strategy_output", true);
    indi = core.indicators:create("ZIGZAG CHANNEL WITH OUTPUT_MULT WITH ARROWS", source, indicatorParams)
end
...
function IsEntryLong(source, period)
    return indi.strat_signal_1[period - 1] == 1;
end
