...
function Init()
    ...
    indicator.parameters:addColor("trend_color", "Trend Color", "Color", core.colors().Red);
    indicator.parameters:addInteger("trend_width", "Trend Width", "Width", 1, 1, 5);
    indicator.parameters:addInteger("trend_style", "Trend Style", "Style", core.LINE_DASH);
    indicator.parameters:setFlag("trend_style", core.FLAG_LINE_STYLE);
        ...
        if last_peak_2 ~= nil then
            if searchMode == -1 then
                ...
                if source.high[peakPeriod] > source.high[last_peakPeriod_2] 
                    ...
                    and current_pullback * 100 >= pullback
                then
                    RegisterUp(context, peakPeriod, lines >= 2);
                    local _, y1 = context:pointOfPrice(source.high[last_peakPeriod_2]);
                    local x1 = context:positionOfBar(last_peakPeriod_2);
                    local _, y2 = context:pointOfPrice(source.high[peakPeriod]);
                    local x2 = context:positionOfBar(peakPeriod);
                    ExtendLine(context, TREND_LINE, x2, y2, x1, y1);
                    lines = lines + 1;
                end
            else
                ...
                then
                    ...
                    local _, y1 = context:pointOfPrice(source.low[last_peakPeriod_2]);
                    local x1 = context:positionOfBar(last_peakPeriod_2);
                    local _, y2 = context:pointOfPrice(source.low[peakPeriod]);
                    local x2 = context:positionOfBar(peakPeriod);
                    ExtendLine(context, TREND_LINE, x2, y2, x1, y1);
                    lines = lines + 1;
                end
