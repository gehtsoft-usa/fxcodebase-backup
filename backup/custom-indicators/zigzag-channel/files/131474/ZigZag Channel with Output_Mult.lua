...
function Init()
    ...
    indicator.parameters:addDouble("extension_up", "Extension Up, %", "", 3)
    ...
    indicator.parameters:addDouble("extension_down", "Extension Down, %", "", 3);
...
local extension_up, extension_down
local init = false;
function Draw(stage, context) 
    ...
    if not init then
        ...
        extension_up = instance.parameters.extension_up;
        extension_down = instance.parameters.extension_down;
    ...
    local _, extension_y_up = context:pointOfPrice(math.max(peak1, peak2) * (1 + extension_up / 100));
    local _, extension_y_down = context:pointOfPrice(math.min(peak1, peak2) * (1 - extension_down / 100));

    context:drawLine(EXTENSION_UP_PEN, x, extension_y_up, context:right(), extension_y_up);
    ...
    context:drawLine(EXTENSION_DOWN_PEN, x, extension_y_down, context:right(), extension_y_down);
