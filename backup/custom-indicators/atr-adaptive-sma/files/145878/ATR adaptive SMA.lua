...
-- https://fxcodebase.com/code/viewtopic.php?f=17&t=72139
...
function Init()
    indicator:name("ATR adaptive SMA");
...
-- Routine
 function Prepare(nameOnly)   
    ...
    local name = profile:id() .. "(" ..  instance.source:name().. "," ..  Period .. "," ..  Price .. ")";
    instance:name(name); 
...
function Update(period, mode)
	...
	if max > ATR.DATA[period] then
	Max=max 
	else
	Max=ATR.DATA[period]
	end
	
	if min < ATR.DATA[period] then
	Min=min 
	else
	Min=ATR.DATA[period]
	end
	...
	local PERIOD =  Period*(Coeff+1.0)/2.0;
	
	 if period <= first+Period+PERIOD then
	 return;
	 end
	...
	Line[period]= mathex.avg(source[Price], period-PERIOD+1, period);
