...
-- https://fxcodebase.com/code/viewtopic.php?f=17&t=72887
...
function Init()
    indicator:name("Trader Oscillator");
    ...
    indicator.parameters:addInteger("Period2", "2. Period", "", 13, 1, 2000);
    indicator.parameters:addInteger("Period3", "3. Period", "", 8, 1, 2000);
    indicator.parameters:addInteger("Period4", "4. Period", "", 8, 1, 2000);	
...
-- Routine
 function Prepare(nameOnly)   
    ...
    Line1 = instance:addStream("Line1", core.Line, name, "1. Line", instance.parameters.color1, first + Period2+ Period3  );
    ...
    Line2 = instance:addStream("Line2", core.Line, name, "2. Line", instance.parameters.color2, first + Period2+ Period3+ Period4  );
...
function Update(period, mode)
	...
	local min, max = mathex.minmax(RSI.DATA, period-Period2+1, period); 
	 
	Data[period] = 100 * (RSI.DATA[period] - min) / (max - min);
  
	 if period <= first + Period2+ Period3  then
	 return;
	 end
	 
	 Line1[period] =  mathex.avg(Data, period-Period3+1, period); 

	 if period <= first + Period2+ Period3+ Period4 then
	 return;
	 end	 
	  	
	Line2[period] = mathex.avg(Line1, period-Period4+1, period); 
