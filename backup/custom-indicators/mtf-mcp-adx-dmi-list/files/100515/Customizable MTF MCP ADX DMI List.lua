...
-- http://fxcodebase.com/code/viewtopic.php?f=17&t=62230
...
function AddTimeFrame(id , FRAME , DEFAULT  )
	...
	indicator.parameters:addInteger("ADX_Period" ..id, "ADX Period", "", 20);
	indicator.parameters:addInteger("DMI_Period"..id , "DMI Period", "", 20);
	indicator.parameters:addDouble("Entry" ..id, "Entry Level", "", 30);
	indicator.parameters:addDouble("Exit" ..id, "Exit Level", "", 45);
...
function Prepare(nameOnly)
	...
	Num=0;
		for i = 1 , 13 , 1 do  
		   ...
		   if Use[i] then
			Num=Num+1;
			ADX_Period[Num]= instance.parameters:getInteger("ADX_Period" .. i);
			DMI_Period[Num]= instance.parameters:getInteger("DMI_Period" .. i);
			Entry[Num]= instance.parameters:getDouble("Entry" .. i);
			Exit[Num]= instance.parameters:getDouble("Exit" .. i);
			TF[Num]=  iTF[i];	
	 ...
	 for i = 1, Count, 1 do	
      ...
      DMI[i]={};
		   ...
		   for j = 1, Num, 1 do	
			...
			Temp1= core.indicators:create("ADX", source ,ADX_Period[j] );
			Temp2= core.indicators:create("DMI", source ,DMI_Period[j] );
			first =  math.max(Temp1.DATA:first(), Temp2.DATA:first())*2;  
		   Source[i][j]= core.host:execute("getSyncHistory", Pair[i], TF[j], source:isBid(),math.min(300,first),20000 + ID , 10000 +ID);
		   loading [i][j]=true;
		   
		   ADX [i][j]= core.indicators:create("ADX", Source[i][j] , ADX_Period[j]);
		   DMI [i][j]= core.indicators:create("DMI", Source[i][j] , DMI_Period[j]); 		  
		   end
	 end 
...
function Draw(stage, context)
 ...
 function Calculate (context,i, j )
     ...
     if  not   DMI[i][j].DATA:hasData(DMI[i][j].DATA:size()-1)
     or  not   DMI[i][j].DATA:hasData(DMI[i][j].DATA:size()-2)  
     or  not   ADX[i][j].DATA:hasData(ADX[i][j].DATA:size()-1)
     or  not   ADX[i][j].DATA:hasData(ADX[i][j].DATA:size()-2)  
     then
     return;
     end
    ...
    YELLOW DOT: OTHER WICE
		  ...
		  if ADX[i][j].DATA [ADX[i][j].DATA:size()-1] >  ADX[i][j].DATA [ADX[i][j].DATA:size()-2]  
		  and ADX[i][j].DATA [ADX[i][j].DATA:size()-1]  > Entry[j]
		  and ADX[i][j].DATA [ADX[i][j].DATA:size()-1] < Exit[j]
		  and  DMI[i][j].DIP [DMI[i][j].DIP:size()-1] >  DMI[i][j].DIM [DMI[i][j].DIM:size()-1] 
		  then
		  Symbol="\225"; 
		  SymbolColor=Up;
		  elseif  ADX[i][j].DATA [ADX[i][j].DATA:size()-1] >  ADX[i][j].DATA [ADX[i][j].DATA:size()-2] 
		  and ADX[i][j].DATA [ADX[i][j].DATA:size()-1]  > Entry[j]
		  and ADX[i][j].DATA [ADX[i][j].DATA:size()-1] < Exit[j]
		  and  DMI[i][j].DIP [DMI[i][j].DIP:size()-1] <  DMI[i][j].DIM [DMI[i][j].DIM:size()-1] 
		  then
