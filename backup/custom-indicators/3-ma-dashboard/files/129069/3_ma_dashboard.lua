...
-- http://fxcodebase.com/code/viewtopic.php?f=17&t=68996
...
local indi_name = "3 ma dashboard";
...
function CreateIndicators(source)
    ...
    indicators[#indicators + 1] = CreateAverages(instance.parameters.ma_1_method, source, instance.parameters.ma_1_period);
    indicators[#indicators + 1] = CreateAverages(instance.parameters.ma_2_method, source, instance.parameters.ma_2_period);
    indicators[#indicators + 1] = CreateAverages(instance.parameters.ma_3_method, source, instance.parameters.ma_3_period);

    return indicators;
end

function CreateParameters()
    indicator.parameters:addGroup("MA 1");
    AddAverages("ma_1_method", "Method", "MVA");
    indicator.parameters:addInteger("ma_1_period", "Period", "", 200);
    indicator.parameters:addGroup("MA 2");
    AddAverages("ma_2_method", "Method", "EMA");
    indicator.parameters:addInteger("ma_2_period", "Period", "", 49);
    indicator.parameters:addGroup("MA 3");
    AddAverages("ma_3_method", "Method", "EMA");
    indicator.parameters:addInteger("ma_3_period", "Period", "", 13);
end

function GetLastSignal(indi, source)
    for i = 0, source:size() - 1 do
        if indi[2].DATA[NOW - i] < indi[1].DATA[NOW - i] 
            and indi[3].DATA[NOW - i] < indi[1].DATA[NOW - i] 
            and source.close[NOW - i] > indi[1].DATA[NOW - i]
        then
