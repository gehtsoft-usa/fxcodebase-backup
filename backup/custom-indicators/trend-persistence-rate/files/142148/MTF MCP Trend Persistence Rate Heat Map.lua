...
--+------------------------------------------------------------------------------------------------+
 ...
 function Add(id, TF,Flag, Instrument )
	...
	indicator.parameters:addInteger("TPRPer".. id, "TPR Period", "", 20, 1, 2000);
    indicator.parameters:addInteger("SMAPer".. id, "SMA Period", "", 5, 1, 2000); 
	indicator.parameters:addDouble("Threshold".. id, "Threshold", "", 1); 
    indicator.parameters:addInteger("MULT".. id, "Multiplier", "", 10); 
	 indicator.parameters:addInteger("offset".. id, "offset", "", 0); 
end

 
 
function Init()
    indicator:name("MTF MCP Trend Persistence Rate Heat Map");
...
function Prepare(nameOnly)
	...
	assert(core.indicators:findIndicator("TREND PERSISTENCE RATE") ~= nil, "Please, download and install TREND PERSISTENCE RATE.LUA indicator");    
	...
	s2, e2 = core.getcandle(iTF[i], 0, 0, 0);
	 ...
	 TPRPer[Number]= instance.parameters:getInteger("TPRPer" .. i);
	 SMAPer[Number]= instance.parameters:getInteger("SMAPer" .. i);
	 Threshold[Number]= instance.parameters:getDouble("Threshold" .. i);
	 MULT[Number]= instance.parameters:getInteger("MULT" .. i);
	 offset[Number]= instance.parameters:getInteger("offset" .. i);
	 ...
	 Label[Number]="";
                ...
                Label[Number]=Label[Number] .. " - " ..  TF[Number];				
			  	...
			  	Temp1= core.indicators:create("TREND PERSISTENCE RATE", source.close ,  TPRPer[1], SMAPer[1],Threshold[1],  MULT[1],  offset[1]);  				
				ifirst= Temp1.DATA:first()*2;
				 ...
				 loading[Number]  = true;  	 
				 Indicator[Number] = core.indicators:create("TREND PERSISTENCE RATE", SourceData[Number].close ,  TPRPer[Number], SMAPer[Number],Threshold[Number],  MULT[Number],  offset[Number]);   
