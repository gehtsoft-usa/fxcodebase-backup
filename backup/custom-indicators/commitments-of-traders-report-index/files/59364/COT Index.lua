...
-- http://fxcodebase.com/code/viewtopic.php?f=17&t=34918
...
function Init()
    indicator:name("COT Index");
    indicator:description("Commitments of Traders Report Index");
      ...
      if idx > 0 then
        ...
        if  segment == "Noncommercial" then
          ...
          long, short = data.cl, data.cs;
			  ...
			  local min,max;
			   NP[period] = (long - short);
			  min,max= mathex.minmax(NP, period-Period+1, period);
			  
				  if (max-min)~=  0 then
				  COT[period] =(NP[period]-min)/(max-min); 
				  else
				  COT[period] =0;
				  end
            else
