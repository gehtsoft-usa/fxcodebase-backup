...
function AddParam(id, frame , Flag)
	...
	indicator.parameters:addDouble("Step"..id, "SAR Step","", 0.02, 0.001, 1);
    indicator.parameters:addDouble("Max"..id, "SAR Max", "", 0.2, 0.001, 10);
		
	indicator.parameters:addDouble("DMI_Period"..id, "Period","", 14, 1, 1000);	
end



function Init()
    indicator:name("MTF_MCP TrendBar");
...
function Prepare(nameOnly)
    ...
    for i = 1, Number, 1 do	 
	    	 ...
	    	 Test1 = core.indicators:create("SAR", source , instance.parameters:getDouble ("Step"..i) , instance.parameters:getDouble("Max"..i));
             Test2 = core.indicators:create("DMI", source , instance.parameters:getInteger ("DMI_Period"..i));	
	         first=  math.max(Test1.DATA:first(), Test2.DATA:first())*2 ; 		  
			  ...
			  SAR[i] = core.indicators:create("SAR", SourceData[i] , instance.parameters:getDouble ("Step"..i) , instance.parameters:getDouble("Max"..i));
              DMI[i] = core.indicators:create("DMI", SourceData[i] , instance.parameters:getInteger ("DMI_Period"..i));	
		      loading[i]= true;		
...
function Draw(stage, context)
	...
	local Symbol="\149";
		...
		for i = 1, Number, 1 do 	
			...
			if  DMI[i].DIP[DMI[i].DIP:size()-1]  > DMI[i].DIM[DMI[i].DIM:size()-1]
			and SAR[i].DN[SAR[i].DN:size()-1] < SourceData[i].close[SourceData[i].close:size()-1] 
			and SAR[i].UP[SAR[i].UP:size()-1] ==0   
			then
			Color=UP;
			Symbol="\217";
			elseif DMI[i].DIP[DMI[i].DIP:size()-1] < DMI[i].DIM[DMI[i].DIM:size()-1] 
			and SAR[i].UP[SAR[i].UP:size()-1] > SourceData[i].close[SourceData[i].close:size()-1] 
			and SAR[i].DN[SAR[i].DN:size()-1] ==0 
			then
			Color=DN;
			Symbol="\218";
			else
