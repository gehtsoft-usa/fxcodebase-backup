...
function Init()
    indicator:name("Stochastic Rainbow indicator");
    indicator:description("Stochastic Rainbow indicator");
    indicator:requiredSource(core.Bar);
    indicator:type(core.Oscillator);

    indicator.parameters:addGroup("Calculation");
    indicator.parameters:addInteger("K_Period", "K_Period", "", 1);
    indicator.parameters:addInteger("D_Period", "D_Period", "", 3);
...
function Prepare(nameOnly)
    ...
    local name = profile:id() .. "(" .. source:name() .. ", " .. instance.parameters.K_Period .. ", " .. instance.parameters.D_Period .. ")";
    instance:name(name);
    if nameOnly then
        return;
    end
    for i=1,8,1 do
     Ind_St[i]=core.indicators:create("STOCHASTIC", source, K_Period*(i+2), 3, D_Period*(i+2),"MVA","MVA");
     St[i]=instance:addStream("St" .. i, core.Line, name .. ".St" .. i, "St" .. i, instance.parameters:getColor("clr" .. i), Ind_St[i].D:first());
