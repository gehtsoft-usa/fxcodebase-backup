...
function Init()
    indicator:name("MTF MCP MACD Heat Map");
	...
	Mode[Number]=instance.parameters:getInteger("Mode" .. i);
	 ...
	 Label[Number]="";
                ...
                Label[Number]=Label[Number] .. " - " ..  TF[Number];				
			  	...
			  	Temp1= core.indicators:create("MACD", source.close ,  SN[Number],   LN[Number],   IN[Number]);  				
				ifirst= Temp1.DATA:first()*2;
				 ...
				 loading[Number]  = true;  	 
				 Indicator[Number] = core.indicators:create("MACD", SourceData[Number] , SN[Number],   LN[Number],   IN[Number]);   
			   ...
			   x0, x1, x2 = context:positionOfBar (i);
			    ...
			    for j= 1, Number , 1 do
				  ...
				  if p~= false then
										...
										if Indicator[j].MACD:hasData(p) and Indicator[j].SIGNAL:hasData(p) then 
												   ...
												   if Mode[j] == 1 then
																...
																C1=11; C2=12;		
																elseif Indicator[j].MACD[p]  < Indicator[j].SIGNAL[p] then
