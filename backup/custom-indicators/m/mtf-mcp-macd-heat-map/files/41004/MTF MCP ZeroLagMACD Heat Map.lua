...
-- http://fxcodebase.com/code/viewtopic.php?f=17&t=23823
...
--+------------------------------------------------------------------+
 ...
 function Add(id, TF,Flag, Instrument )
	...
	indicator.parameters:addInteger("SN".. id, "Short Period", "", 12, 2, 1000);
    indicator.parameters:addInteger("LN".. id, "Long Period", "", 26, 2, 1000);
    indicator.parameters:addInteger("IN".. id, " Signal line Period", "", 9, 2, 1000);
	
	indicator.parameters:addInteger("Mode".. id, "Calculation Mode", "", 1);
    indicator.parameters:addIntegerAlternative("Mode".. id, "MACD / Signal", "", 1);
    indicator.parameters:addIntegerAlternative("Mode".. id, "MACD / Zero", "", 2);
    indicator.parameters:addIntegerAlternative("Mode".. id, "Histogram / Zero", "", 3);
...
function Init()
    indicator:name("MTF MCP ZeroLagMACD Heat Map");
	...
	Mode[Number]=instance.parameters:getInteger("Mode" .. i);
	 ...
	 Label[Number]="";
				...
				assert(core.indicators:findIndicator("ZEROLAGMACD") ~= nil, "Please, download and install ZEROLAGMACD.LUA indicator");    
			  	...
			  	Temp1= core.indicators:create("ZEROLAGMACD", source.close ,  SN[Number],   LN[Number],   IN[Number]);  				
				ifirst= Temp1.DATA:first()*2;
				 ...
				 loading[Number]  = true;  	 
				 Indicator[Number] = core.indicators:create("ZEROLAGMACD", SourceData[Number] , SN[Number],   LN[Number],   IN[Number]);   
			   ...
			   x0, x1, x2 = context:positionOfBar (i);
			    ...
			    for j= 1, Number , 1 do
				  ...
				  if p~= false then
										...
										if Indicator[j].MACD:hasData(p) and Indicator[j].SIG:hasData(p) then 
												   ...
												   if Mode[j] == 1 then
																...
																if Indicator[j].MACD[p]  > Indicator[j].SIG[p] then
																 
																C1=11; C2=12;		
																elseif Indicator[j].MACD[p]  < Indicator[j].SIG[p] then
													  ...
													  elseif Mode[j] == 2 then
																...
																C1=11; C2=12;	
																elseif Indicator[j].MACD[p]  < 0 then
													  ...
													  elseif Mode[j] == 3 then
																...
																if Indicator[j].HISTOGRAM[p]  > 0 then
																 C1=11; C2=12;	
																elseif Indicator[j].HISTOGRAM[p]  < 0 then
