...
function Init()
    indicator:name("Multi currency pair, Multi Time Frame, OBOS Dashboard");
    indicator:description("Multi currency pair, Multi Time Frame, OBOS Dashboard");
        ...
        context:createFont (8, "Wingdings",iwidth, iheight , 0);
		        ...
		        for i = 1, Count, 1 do
                         ...
                         if OBOS[i][j].DATA:hasData(OBOS[i][j].DATA:size()-1)   then        
								local BarColor=	  OBOS[i][j].DATA:colorI(OBOS[i][j].DATA:size()-1);
						 
								 if BarColor== Up then
								 Alert= "\230";					
								 elseif BarColor== Down then
								 Alert= "\228";
								else
								  Alert= "\113";
								 end
								 ...
								 context:drawText (8,  Alert, BarColor, -1, x1+xGap  , y1-height+yGap*2 ,x2+xGap ,  y1+yGap*2 , context.CENTER   );	
