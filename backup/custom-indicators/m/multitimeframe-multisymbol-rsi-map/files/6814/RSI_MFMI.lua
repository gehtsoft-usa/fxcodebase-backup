...
-- http://fxcodebase.com/code/viewtopic.php?f=17&t=2972
...
function Init()
    indicator:name("MTF MCP RSI List");
    indicator:description("MTF MCP RSI List");
     indicator:requiredSource(core.Bar);
    indicator:type(core.Oscillator);
  
    indicator.parameters:addGroup("Calculation");
	
    indicator.parameters:addInteger("RSI_Period", "Period of RSI", "", 3);
    indicator.parameters:addInteger("RSI_Step", "Step between two values of RSI", "", 1);
    indicator.parameters:addInteger("Step", "Step between symbols of indicator", "", 1);
    indicator.parameters:addDouble("Difference", "Difference between RSI value for UP/DN", "", 2.5);
    indicator.parameters:addDouble("SDifference", "Difference between RSI value for strong UP/DN", "", 4.75);
	...
	indicator.parameters:addColor("Up", "Up Trend Color","", core.rgb(0, 128, 0));
	indicator.parameters:addColor("Down", "Down Trend Color","", core.rgb(128, 0, 0));
	
	indicator.parameters:addColor("sUp", "Strong Up Trend Color","", core.rgb(0, 255, 0));
	indicator.parameters:addColor("sDown", "Strong Down Trend Color","", core.rgb(255, 0, 0));
...
local sUp,  sDown; 
local Price, RSI_Period, RSI_Step, Difference, SDifference;
...
-- Routine
 function Prepare(nameOnly)   
	...
	sDown= instance.parameters.sDown;
	Neutral= instance.parameters.Neutral; 
	source = instance.source; 
	
	Price= instance.parameters.Price;
	RSI_Period= instance.parameters.RSI_Period;
	RSI_Step= instance.parameters.RSI_Step;
	Difference= instance.parameters.Difference;
	SDifference= instance.parameters.SDifference;
	 ...
	 for i = 1, Count, 1 do	
      ...
      Indicator[i]={};
		   ...
		   for j = 1, Num, 1 do	
			...
			Temp= core.indicators:create("RSI", source.close ,RSI_Period );
			first =  (Temp.DATA:first()*2)+RSI_Step;  
		   Source[i][j]= core.host:execute("getSyncHistory", Pair[i], TF[j], source:isBid(),first+1,20000 + ID , 10000 +ID);
		   loading [i][j]=true;
		   
		   Indicator [i][j]= core.indicators:create("RSI", Source[i][j][Price], RSI_Period);
...
function Draw(stage, context)
 ...
 function Calculate (context,i, j )
		...
		x2=left +(j )*xGap;
	   	...
	   	if j== 1 then 
		width, height = context:measureText (20, Pair[i], context.CENTER  ); 
		context:drawText (20,Pair[i], Color, -1, x1 , y1, x2, y2, context.CENTER   );	
		end
		
		
 	  
	    if i== Count then 
		width, height = context:measureText (20, TF[j], 0); 
		context:drawText (20,  TF[j], Color, -1, x1+xGap , y1-yGap,x2+xGap,  y2-yGap, context.CENTER   );	
		end
		
	 
     if  not   Indicator[i][j].DATA:hasData(Indicator[i][j].DATA:size()-1)
     or  not   Indicator[i][j].DATA:hasData(Indicator[i][j].DATA:size()-1-RSI_Step)  
	...
	local  Value=  math.abs(Indicator[i][j].DATA[Indicator[i][j].DATA:size()-1]-Indicator[i][j].DATA[Indicator[i][j].DATA:size()-1-RSI_Step]);
		 
           
		 if Indicator[i][j].DATA[Indicator[i][j].DATA:size()-1] > Indicator[i][j].DATA[Indicator[i][j].DATA:size()-1-RSI_Step] then
		      
 		   
		      if Value > SDifference then
			  SymbolColor=sUp;
			   Symbol="\233";
			  elseif Value > Difference then
			  SymbolColor=Up;
			   Symbol="\236";
			  else
			  SymbolColor=Neutral;
		...
		context:createFont (20, "Arial",iwidth, iheight , context.ITALIC);
        iwidth = ( xGap  /100)*Size ;		
		context:createFont (21, "Wingdings",iwidth, iheight , context.CENTER  ); 
		
	
		
 
		if Symbol~= nil then
		 width, height = context:measureText (21, Symbol , context.CENTER  );  
		  context:drawText (21,  Symbol, SymbolColor, -1, x1+xGap , y1, x2+xGap  , y2 , context.CENTER   );	
		end
