...
-- Routine
 function Prepare(nameOnly)  
	...
	residual6 = instance:addInternalStream(residual12mva.DATA:first());
		...
		if S1Flag then	
	S1 = instance:addStream("firstMVA2001", core.Line, "1. " .. Method1, "1. " .. Method1, instance.parameters.Color1, MVA200.DATA:first());
	...
	if S2Flag then
	S2 = instance:addStream("secondMVA1001", core.Line, "2, " .. Method1, "2. " .. Method1, instance.parameters.Color2, residual200mva.DATA:first());
	...
	if S3Flag then
	S3 = instance:addStream("thirdMVA1001", core.Line, "3. " .. Method1, "3. " .. Method1, instance.parameters.Color3, residual100mva.DATA:first());
	...
	if S4Flag then
	S4 = instance:addStream("fourthMVA1001", core.Line, "4. " .. Method1, "4. " .. Method1, instance.parameters.Color4, residual50mva.DATA:first());
	...
	if S5Flag then 
	S5 = instance:addStream("fifthMVA1001", core.Line, "5. " .. Method1, "5. " .. Method1, instance.parameters.Color5, residual25mva.DATA:first());
	...
	if S6Flag then 
	S6 =  instance:addStream("sixthMVA1001", core.Line, "6. " .. Method1, "6. " .. Method1, instance.parameters.Color6, residual12mva.DATA:first());
...
function Update(period, mode)
	...
	if( period >= residual200mva.DATA:first() ) then
		...
		S2[period] = MVA200.DATA[period] + residual200mva.DATA[period];
	end
	...
	if( period >= residual100mva.DATA:first() ) then		
		...
		S3[period] = S2[period] + residual100mva.DATA[period];
	end
	...
	if( period >= residual50mva.DATA:first() ) then
		...
		S4[period] = S3[period] + residual50mva.DATA[period];
	end	
	...
	if( period >= residual25mva.DATA:first() ) then
		...
		S5[period] = S4[period] + residual25mva.DATA[period]; 
	end
	
	if( period >= residual12mva.DATA:first() ) then
	S6[period] = S5[period] + residual12mva.DATA[period]; 
	end
	
	
	 BUFFER[period]= (S1[period]+S2[period]+S3[period]+S4[period]+S5[period]+S6[period])/6;	
