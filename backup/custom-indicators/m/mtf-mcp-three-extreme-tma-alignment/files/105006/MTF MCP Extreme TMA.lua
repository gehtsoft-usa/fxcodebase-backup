...
function Init()
    indicator:name("MTF MCP Extreme TMA");
    indicator:description("MTF MCP Extreme TMA");
	...
	indicator.parameters:addStringAlternative("Type", "All currency pair", "All currency pair" , "All currency pair");
     indicator.parameters:addBoolean("Sort", "Sort By Time Frame", "", true); 
                   ...
                   SortFlag= true		
                    while SortFlag do  
						   ...
						   for j = 2, Num, 1 do 
								  ...
								  p1=New[i][j];
								  p2=New[i][j-1];
								  x1=Indicator1[i][p1].DATA:size()-1
								  x2=Indicator1[i][p2].DATA:size()-1
								  
										if
										Indicator1[i][p1].DATA[x1] <Indicator1[i][p2].DATA[x2] 
										then	
										  SortFlag=true;						
										  Temp= p2;						  
										  New[i][j-1]= p1;
	 ...
	 for i = 1, Count, 1 do	
	  ...
	  Index[i]={};
		   ...
		   for j = 1, Num, 1 do	
			...
			Test1 =   core.indicators:create("EXTREME_TMA_LINE", source, Period1[j],Period1[j],2, TrendThreshold[j], true, Redraw[j],Neutral,Up, Down);			
			first =  Test1.DATA:first()*3+1;  
			
		   Source[i][j]= core.host:execute("getSyncHistory", Pair[i], TF[j], source:isBid(),math.min(300,first),20000 + ID , 10000 +ID);
		   loading [i][j]=true; 
		   Indicator1[i][j] =   core.indicators:create("EXTREME_TMA_LINE", Source[i][j], Period1[j],Period1[j],2, TrendThreshold[j], true, Redraw[j],Neutral,Up, Down);
		   Index[i][j]=j;
		   end
	 end 
...
function Draw(stage, context)
	...
	for i = 1, Count, 1 do 
	 ...
	 if Loading then
		...
		xGap=  (right-left)/(Count);
		yGap=  (context:bottom()-context:top())/(Num+2);
		top=context:top();
 ...
 function Calculate (context,i, j )
	   ...
	   if j==Num then 
		...
		context:drawText (7,Pair[i], Color, -1, x1 , y1, x1+width,y1+height, context.LEFT   );	
      end
		
	  
     
	local Style=tostring(TF[ Sorted[i][j]]);
	SymbolColor =  Indicator1[i][Sorted[i][j]].DATA:colorI(Indicator1[i][Sorted[i][j]].DATA:size()-1) ;	
	
	width, height = context:measureText (7, Style , context.CENTER  );  
	context:drawText (7,  Style, SymbolColor, -1, x1 , y1+yGap, x1+width  , y1+yGap+height, context.CENTER   );	
