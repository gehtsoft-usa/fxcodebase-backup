...
function Init()
    indicator:name("Mansfield Relative Performance indicator");
    indicator:description("Mansfield Relative Performance indicator");
	...
	indicator.parameters:addInteger("Period" ,"MA Period", "", 200);
...
-- Routine
 function Prepare(nameOnly)   
    ...
    local name = profile:id() .. "(" ..  instance.source:name().. ", " .. Period .. ", " .. Instrument[1] .. ", " ..  Instrument[2]   .. ")";
    instance:name(name); 
    ...
    RP= instance:addInternalStream(0, 0)
	
    MRP = instance:addStream("MRP" , core.Line, " MRP "," MRP ",instance.parameters.color, source:first());
    MRP:setPrecision(math.max(2, instance.source:getPrecision()));
	MRP:setWidth(instance.parameters.width);
    MRP:setStyle(instance.parameters.style);
end
...
function Update(period, mode)
	...
	MRP[period]= mathex.avg(RP, period-Period+1, period);
