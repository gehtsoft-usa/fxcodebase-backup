...
function Init()
    indicator:name("Absolute Moving Average Relative Position");
...
function Update(period, mode)
	...
	LONG:update(mode);
	     ...
	     if Short < Long then
		   RAW[period] = (SHORT.DATA[period]-LONG.DATA[period])/source:pipSize();
		   else
		   RAW[period] = (LONG.DATA[period]-SHORT.DATA[period])/source:pipSize();
          end
	...
	return;
		   ...
		   local Range = math.max(PlusRange , MinusRange);
		          ...
		          for i = first ,source:size()-1, 1 do
  				       ...
  				       DMS[i] =(RAW[i]/ (Range/100))--* (RAW[i]/100);
