...
function Init()
	indicator:name("All Time Frames All Currency Moving Averages Scalping Tool Oscillator View")
	indicator:description("All Time Frames All Currency Moving Averages Scalping Tool Oscillator View")
...
--	Moving Averages Scalping Tool Oscillator
	instance:initView("Moving Averages Scalping Tool", 2, 0.01, true, true)
	...
	assert(core.indicators:findIndicator("MOVING AVERAGES SCALPING TOOL OSCILLATOR") ~= nil, "MOVING AVERAGES SCALPING TOOL OSCILLATOR indicator must be installed");
	...
	for i = 1, Count, 1 do
		...
		for j = 1, Num, 1 do
			...
			Indicator[i][j] = core.indicators:create("MOVING AVERAGES SCALPING TOOL OSCILLATOR", Source[i][j][Price] , instance.parameters.Period1, instance.parameters.Period2, instance.parameters.Period3, instance.parameters.Method, instance.parameters.Off)
		end
	end
...
function Calculate(context, i, j)
	if not Indicator[i][j].DATA:hasData(Indicator[i][j].DATA:size() - 1) or not Indicator[i][j].DATA:hasData(Indicator[i][j].DATA:size() - 2) then
		return
	end
	...
	if Indicator[i][j].DATA[Indicator[i][j].DATA:size() - 1]== 1 then
		...
		if Indicator[i][j].DATA[Indicator[i][j].DATA:size() - 2] ~= Indicator[i][j].DATA[Indicator[i][j].DATA:size() - 1]  then
		color2 = DownColor
		end
	...
	context:drawText(8, Symbol, color1, color2, x1 + (x2 - x1) / 2 - width / 2, y1, x1 + (x2 - x1) / 2 + width / 2, y2, 0)
end
