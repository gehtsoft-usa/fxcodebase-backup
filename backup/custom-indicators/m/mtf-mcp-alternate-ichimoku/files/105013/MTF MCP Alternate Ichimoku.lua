...
-- http://fxcodebase.com/code/viewtopic.php?f=17&t=63193
...
function Init()
    indicator:name("MTF MCP Alternate Ichimoku");
    indicator:description("MTF MCP Alternate Ichimoku");
	 ...
	 for i = 1, Count, 1 do	
      ...
      Indicator[i]={};
		   ...
		   for j = 1, Num, 1 do	
			...
			Test1 =   core.indicators:create("ALTERNATE ICHIMOKU", source, SSP,SSK);
		   ...
		   loading [i][j]=true; 
		   Indicator[i][j] =   core.indicators:create("ALTERNATE ICHIMOKU", Source[i][j], SSP,SSK);
		   end
	 end 
...
function Draw(stage, context)
 ...
 function Calculate (context,i, j )
      ...
      if not Indicator[i][j].SA:hasData(Indicator[i][j].SA:size()-1)
	  or not Indicator[i][j].SL:hasData(Indicator[i][j].SL:size()-1)
      then
	  return;
	  end
		...
		--[[
			...
			2. GREEN UPPER ARROW WITH DOT --- SA> SB AND PRICE<SA AND PRICE>SL(PRICE WITH IN BULLISH CLOUD BUT ABOVE SL LINE)
    ...
    local SymbolColor=Neutral;	
	      	   ...
	      	   if Indicator[i][j].SA[Indicator[i][j].SA:size()-1] >  Indicator[i][j].SB[Indicator[i][j].SB:size()-1] 
			   and  Source[i][j].close[ Source[i][j].close:size()-1] >  Indicator[i][j].SA[Indicator[i][j].SA:size()-1] 
			   then
			   	                            SymbolColor = Up;
											Style= "\225";
			   elseif Indicator[i][j].SA[Indicator[i][j].SA:size()-1] >  Indicator[i][j].SB[Indicator[i][j].SB:size()-1] 
			   and  Source[i][j].close[ Source[i][j].close:size()-1] <  Indicator[i][j].SA[Indicator[i][j].SA:size()-1] 
			   and  Source[i][j].close[ Source[i][j].close:size()-1] >  Indicator[i][j].SL[Indicator[i][j].SL:size()-1] 
			   then						
