...
function Init()
    indicator:name("MACD Divergence Trend");
...
function Prepare(nameOnly)
    ...
    assert(core.indicators:findIndicator("MACD_H_DIVERGENCE") ~= nil, "Please, download and install MACD_H_DIVERGENCE.LUA indicator");   
    MACD = core.indicators:create("MACD_H_DIVERGENCE", source, instance.parameters.MACD_Short,instance.parameters.MACD_Long,instance.parameters.MACD_Signal, false);
...
function Update(period, mode)
    ...
    if MACD:getStream(1):hasData(period - 2) then
        ...
        line_id = line_id + 1;
        core.host:execute("drawLine", line_id, source:date(prev), source.high[prev], source:date(period - 2), source.high[period - 2], UP_color, instance.parameters.styleLinReg, instance.parameters.widthLinReg);
    end
    if MACD:getStream(2):hasData(period - 2) then
        l = math.abs(MACD:getStream(2)[period - 2]);
        local prev = period - 2 - l;
        line_id = line_id + 1;
        core.host:execute("drawLine", line_id, source:date(prev), source.low[prev], source:date(period - 2), source.low[period - 2], DN_color, instance.parameters.styleLinReg, instance.parameters.widthLinReg);
    end
end
