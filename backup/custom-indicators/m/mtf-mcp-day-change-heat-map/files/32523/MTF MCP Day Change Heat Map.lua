...
-- http://fxcodebase.com/code/viewtopic.php?f=17&t=17971
...
--indicator.parameters:addString("Mode".. id, "The Indicator Mode", "", "N");
  --  indicator.parameters:addStringAlternative("Mode".. id, "Net Change", "", "N");
  --  indicator.parameters:addStringAlternative("Mode".. id, "Percent Change", "", "P");
...
function Init()
    indicator:name("MTF MCP Day Change Heat Map");
...
function Prepare(nameOnly)  
   ...
   assert(core.indicators:findIndicator("DAY_CHANGE") ~= nil, "Please, download and install DAY_CHANGE.LUA indicator");       
	...
	s2, e2 = core.getcandle(iTF[i], 0, 0, 0);
	 ...
	 if  instance.parameters:getBoolean("On" .. i) 
	 and (e1 - s1) <= (e2 - s2)
	 and (e2 - s2) <= (e3 - s3)
	 then
	 Number=Number+1;
	 
	 -- Mode[Number]= instance.parameters:getString("Mode" .. i);
	 ...
	 Label[Number]="";
                ...
                Label[Number]=Label[Number] .. " - " ..  TF[Number];				
			  	...
			  	Temp1= core.indicators:create("DAY_CHANGE",  source, "N");       
				ifirst= Temp1.DATA:first()*2;
				 ...
				 loading[Number]  = true;  	 
				 Indicator[Number] = core.indicators:create("DAY_CHANGE",  SourceData[Number], "N");          
			   ...
			   x0, x1, x2 = context:positionOfBar (i);
			    ...
			    for j= 1, Number , 1 do
				  ...
				  if p~= false then
										...
										if Indicator[j]:getStream(0):hasData(p) then 
										
												    if Indicator[j]:getStream(0)[p]  > 0 then		 
