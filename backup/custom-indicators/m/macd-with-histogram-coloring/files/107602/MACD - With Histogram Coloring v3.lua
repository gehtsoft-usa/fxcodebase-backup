...
function Init()
   ...
   indicator.parameters:addGroup("Calculation");
    ...
    indicator.parameters:addColor("HISTOGRAM_colorUpUp", "Up in Up Trend  Histogram", "Up Histogram", core.rgb(0, 255, 0));
	indicator.parameters:addColor("HISTOGRAM_colorDownUp", "Down in Up Trend  Histogram", "Up Histogram", core.rgb(0, 200, 0));
	indicator.parameters:addColor("HISTOGRAM_colorUpDown", "Up in Down Trend Histogram", "Down Histogram", core.rgb(255, 0, 0));
    indicator.parameters:addColor("HISTOGRAM_colorDownDown", "Down in Down Trend Histogram", "Down Histogram", core.rgb(200, 0, 0));
...
function Prepare()
	...
	HISTOGRAM = instance:addStream( "HISTOGRAM", core.Bar, name .. ".HISTOGRAM", "HISTOGRAM", instance.parameters.HISTOGRAM_colorUpUp, firstPeriodMACD +  IN);
		 ...
		 HISTOGRAM[period] = (MACD[period] - SIGNAL[period])*Multiplier;
		   ...
		   if HISTOGRAM[period]>0 then
					if( INHIST[period] > INHIST[period - 1]) then
					 HISTOGRAM:setColor(period, instance.parameters.HISTOGRAM_colorUpUp);  
					else
					  HISTOGRAM:setColor(period, instance.parameters.HISTOGRAM_colorDownUp);  
					end
			
			else
					  if( INHIST[period] > INHIST[period - 1]) then
					 HISTOGRAM:setColor(period, instance.parameters.HISTOGRAM_colorUpDown);  
					else
					  HISTOGRAM:setColor(period, instance.parameters.HISTOGRAM_colorDownDown);  
					end
			end
