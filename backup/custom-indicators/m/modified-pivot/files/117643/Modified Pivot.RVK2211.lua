...
function Prepare(onlyName)
    ...
    ref = core.host:execute("getSyncHistory", source:instrument(), "m1", source:isBid(), 10, 100, 101);
    loading = true;
end
...
function Update(period, mode)
    ...
    local startDate = date - 1 + OpenTime;
    local endDate = date - 1 + CloseTime;
    local startIndex = core.findDate(ref, startDate, false);
    local endIndex = core.findDate(ref, endDate, false);
    if startIndex == endIndex or startIndex == -1 then
        return;
    end
    local periodOpen = ref.open[startIndex];
    local periodClose = ref.close[endIndex];
    local periodHigh = mathex.max(ref.high, startIndex, endIndex);
    local periodLow = mathex.min(ref.low, startIndex, endIndex);

    if CalcMode == O_PIVOT or CalcMode == O_FIB or CalcMode == O_FLOOR then
        P[period] = (periodHigh + periodClose + periodLow) / 3;
    elseif CalcMode == O_CAM then
        P[period] = periodClose;
    elseif CalcMode == O_FIBR then
        P[period] = (periodHigh + periodLow) / 2;
    elseif CalcMode == O_WOOD then
        local open;
        if (endDate == ref:size() - 1) then
            -- for a live day take close as open of the next period
            open = periodOpen;
        else
            local startDate = date + OpenTime;
            local endDate = date + CloseTime;
            local startIndex = core.findDate(ref, startDate, false);
            local endIndex = core.findDate(ref, endDate, false);
            if startIndex == endIndex then
                return;
            end
            open = ref.open[startIndex];
        end
        P[period] = (periodHigh + periodLow + open * 2 ) / 4;
    end
    H[period] = periodHigh;
    L[period] = periodLow;
...
function ShowLevels(data, period)
    ...
    d1 = math.floor(source:date(period)) + OpenTime;
    d2 = math.floor(source:date(period)) + CloseTime;
