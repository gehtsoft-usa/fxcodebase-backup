...
-- http://fxcodebase.com/code/viewtopic.php?f=17&t=66851
...
function Init()
    indicator:name("Maximum Profit Drawdown Delta");
...
function Update(period, mode)
	...
	local Max2=0;
     for i=0, Period, 1 do
		 ...
		 Max1=math.max(Max1,(source.high[period]-source.low[period+i]));
		 Max2=math.max(Max2,(source.high[period+i]-source.low[period]));
		 end
	 end	
	
	 Oscillator[period+shift]=(Max2-Max1)/source:pipSize(); 
...
function New(period)
   ...
   local Max2=0; 
	 ...
	 Max1=math.max(Max1,(source.high[period]-source.low[i]));
	 Max2=math.max(Max2,(source.high[i]-source.low[period]));
	 end
