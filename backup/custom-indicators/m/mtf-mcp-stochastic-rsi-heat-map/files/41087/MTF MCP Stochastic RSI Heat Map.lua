...
--http://fxcodebase.com/code/viewtopic.php?f=17&t=23904
...
--+------------------------------------------------------------------+
 ...
 function Add(id, TF,Flag, Instrument )
	...
	indicator.parameters:addInteger("Mode".. id, "Calculation Mode", "", 1);
    indicator.parameters:addIntegerAlternative("Mode".. id, "K / D", "", 1);
    indicator.parameters:addIntegerAlternative("Mode".. id, "K / Cental Line", "", 2);
...
function Init()
    indicator:name("MTF MCP Stochastic RSI Heat Map");
	...
	indicator.parameters:addColor("UpNeutral", "Up in Neutal Color","", core.rgb(128, 128, 128));
	indicator.parameters:addColor("DownNeutral", "Down in Neutal Color","", core.rgb(100, 100, 100));
	
	indicator.parameters:addColor("OB", "OB Color","", core.rgb(0,0,255));
	indicator.parameters:addColor("OS", "OS Color","", core.rgb(0,0,255));
...
local UpUp, DownDown, UpNeutral;
local UpDown, DownUp, DownNeutral;
local OB,OS;
local Instrument={};
...
function Prepare(nameOnly) 
	...
	s2, e2 = core.getcandle(iTF[i], 0, 0, 0);
	 ...
	 Number=Number+1;
		  ...
		  Mode[Number]= instance.parameters:getInteger("Mode" .. i);
	 ...
	 Label[Number]="";
                ...
                Label[Number]=Label[Number] .. " - " ..  TF[Number];				
			  	...
			  	Temp1= core.indicators:create("STOCHRSI", source.close,  N[Number],K[Number], KS[Number], D[Number]);  	
				ifirst= Temp1.DATA:first()*2;
				 ...
				 loading[Number]  = true;  	 
				Indicator[Number] = core.indicators:create("STOCHRSI", SourceData[Number][Price[Number]],  N[Number],K[Number], KS[Number], D[Number]);   
...
function Draw (stage, context)
   ...
   context:setClipRectangle(context:left(), context:top(), context:right(), context:bottom());
        ...
        if not init then
			...
			context:createPen (51, context.SOLID, 1, UpNeutral)       
			context:createSolidBrush(52,UpNeutral);
			
			context:createPen (61, context.SOLID, 1, DownNeutral)       
			context:createSolidBrush(62,DownNeutral);
			
			context:createPen (53, context.SOLID, 1, OB)    
			
			context:createPen (54, context.SOLID, 1, OS)    
			   ...
			   x0, x1, x2 = context:positionOfBar (i);
				...
				if p~= false then
								 ...
								 if Mode[j] == 1 then
										...
										if Indicator[j].K:hasData(p) and Indicator[j].K:hasData(p-1)  and Indicator[j].D:hasData(p) then 
										
										
												if Indicator[j].K[p]  > Indicator[j].D[p] then 
												
												    if Indicator[j].K[p] > Indicator[j].K[p-1] then
													C2=12;
													C1=11;
													else
													C2=22;
												...
												elseif Indicator[j].K[p]  < Indicator[j].D[p] then
												    ...
												    if  Indicator[j].K[p]> 80 then
													C1=53
												   elseif  Indicator[j].K[p]< 20 then
													 C1=54;
									                end
									   else		
									   C1=1; C2=2;										   
								  ...
								  elseif Mode[j]== 2 then
								  
								  
											if Indicator[j].K:hasData(p) and Indicator[j].K:hasData(p-1)  then 
													if Indicator[j].K[p]  > 50 then
															if Indicator[j].K[p] > Indicator[j].K[p-1] then
															C2=12;
															C1=11;
															else
													...
													elseif Indicator[j].K[p]  < 50 then
