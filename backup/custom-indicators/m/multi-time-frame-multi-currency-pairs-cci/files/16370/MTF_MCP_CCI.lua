...
-- http://fxcodebase.com/code/viewtopic.php?f=17&t=7337
...
function Prepare(nameOnly)
    ...
    for i = 1, Number, 1 do	 
	    	...
	    	local Test = core.indicators:create("CCI", source ,Period[i]);   
	        first= Test.DATA:first()*2 ; 		  
	  	
	  
	    SourceData[i] = core.host:execute("getSyncHistory", Pair[i], TF[i], source:isBid(),  math.min(first,300) , 200+i, 100+i);	   
		Indicator[i] = core.indicators:create("CCI", SourceData[i], Period[i]);
		loading[i]= true;		
...
function Draw(stage, context)
	...
	local Symbol="\149";
		...
		for i = 1, Number, 1 do 	
			...
			context:createFont (11, "Arial", (xCell/10)*(Size/100)*2, yCell*(Size/100), 0);	
		    ...
		    context:drawText (2, tostring(TF[i]), Label, -1, context:right ()-(i)*xCell    , context:top ()+yCell, context:right ()-(i-1)*xCell + width, context:top ()+yCell + height, 0);
			  ...
			  local Symbol="";
								   ...
								   if Indicator[i].DATA[Indicator[i].DATA:size()-1] > 0 then
								   Symbol= "+"
								   else
								   Symbol= "-"
								   end
			 ...
			 if Style == "Simple" then 
								  ...
								  if  Indicator[i].DATA[Indicator[i].DATA:size()-1] > 100 
								  or   Indicator[i].DATA[Indicator[i].DATA:size()-1] < -100 
								  then
								  Symbol="("..Symbol..")";
								  end
			 ...
			 context:drawText (11,  tostring(Symbol), Color1, -1, context:right ()-(i)*xCell    , context:top ()+yCell*2, context:right ()-(i-1)*xCell + width, context:top ()+yCell*2 + height, 0);
			 else
			 ...
			 context:drawText (2,  Value, Color1, -1, context:right ()-(i)*xCell    , context:top ()+yCell*3, context:right ()-(i-1)*xCell + width, context:top ()+yCell*3 + height, 0);
