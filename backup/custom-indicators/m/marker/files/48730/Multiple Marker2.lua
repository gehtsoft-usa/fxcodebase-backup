...
function AddStyle(id)
	...
	indicator.parameters:addBoolean("ShowLine"..id, "Show vertical line", "", true);
	indicator.parameters:addColor("LineColor"..id, "Vertical line color", "", core.rgb(255, 255, 0));
        indicator.parameters:addInteger("widthLinReg"..id, "Line width", "Line width", 1, 1, 5);
        indicator.parameters:addInteger("styleLinReg"..id, "Line style", "Line style", core.LINE_SOLID);
        indicator.parameters:setFlag("styleLinReg"..id, core.FLAG_LINE_STYLE);
						...
						if source:serial(source:size()-1) ~= LAST[id]  and period == source:size()-1 then
						   ...
						   if LookBack[id] ~= 0  and LookBack[id]+1 < period  then
						      ...
						      LAST[id]  = source:serial(source:size()-1);
								...
								if instance.parameters:getBoolean("ShowLine" .. id) then
								        core.host:execute ("drawLine", id, source:date(period-LookBack[id]), 0, source:date(period-LookBack[id]), 1000, instance.parameters:getInteger("LineColor" .. id), instance.parameters:getInteger("styleLinReg" .. id), instance.parameters:getInteger("widthLinReg" .. id));
								end
