...
function Draw(stage, context)
    ...
    local Last = math.min(context:lastBar(), source:size() - 2)
...
function isMasterCandle(period)
    ...
    if math.max(source.close[period + 1], source.open[period + 1]) > CandleTop or math.min(source.close[period + 1], source.open[period + 1]) < CandleBottom then
        Flag = false
    end
