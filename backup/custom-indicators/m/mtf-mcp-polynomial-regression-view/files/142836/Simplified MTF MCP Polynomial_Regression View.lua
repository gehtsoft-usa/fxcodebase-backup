...
function Init()
    indicator:name("Simplified MTF MCP  Polynomial_Regression View")
    indicator:description("Simplified MTF MCP  Polynomial_Regression View")
    ...
    indicator.parameters:addBoolean("Skip" , "Skip Instruments without Indication", "", true)	
    indicator.parameters:addBoolean("CutOff" , "CutOff", "", true)		
	indicator.parameters:addDouble("CutOffValue", "CutOff Value", "", 10);
...
function Logic()
	 CountOf=0;
		...
		ShowTheLabel[i][j]=0; 	
						   ...
						   if  Indicator[i][j].BuffBandUp:hasData(Indicator[i][j].BuffBandUp:size() - 1)  
						   and Indicator[i][j].BuffBandDn:hasData(Indicator[i][j].BuffBandDn:size() - 1)
						   then
	   
							 Value1[i][j] = (Source[i][j].close[Source[i][j].close:size()-1] -Indicator[i][j].BuffBandDn[Indicator[i][j].BuffBandDn:size() - 1]) /
							 ( (Indicator[i][j].BuffBandUp[Indicator[i][j].BuffBandUp:size() - 1]-Indicator[i][j].BuffBandDn[Indicator[i][j].BuffBandDn:size() - 1])/100)
								
								 Value2[i][j] = (Source[i][j].close[Source[i][j].close:size()-2] -Indicator[i][j].BuffBandDn[Indicator[i][j].BuffBandDn:size() - 2]) /
