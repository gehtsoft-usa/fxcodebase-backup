...
--+------------------------------------------------------------------+
 ...
 function Add(id, TF,Flag, Instrument )
	...
	indicator.parameters:addInteger("Period".. id, "Number of periods", "", 12, 1, 2000);
...
function Init()
    indicator:name("MTF MCP Random Walk Index Heat Map");
...
local Indicator = {} ;
 ...
 function Prepare(nameOnly) 
  ...
  assert(core.indicators:findIndicator("RWI") ~= nil, "Please, download and install RWI.LUA indicator"); 
	...
	s2, e2 = core.getcandle(iTF[i], 0, 0, 0);
	 ...
	 Label[Number]="";
                ...
                Label[Number]=Label[Number] .. " - " ..  TF[Number];				
			  	...
			  	Temp1= core.indicators:create("RWI", source ,  Period[Number]);  				
				ifirst= Temp1.DATA:first()*2;
				 ...
				 loading[Number]  = true;  	 
				 Indicator[Number] = core.indicators:create("RWI", SourceData[Number] ,  Period[Number]);   
			   ...
			   x0, x1, x2 = context:positionOfBar (i);
			    ...
			    for j= 1, Number , 1 do
				  ...
				  if p~= false then
										...
										if Indicator[j]:getStream(0):hasData(p) and Indicator[j]:getStream(1):hasData(p) then 
										
												    if Indicator[j]:getStream(0)[p]  > Indicator[j]:getStream(1)[p] then		 
