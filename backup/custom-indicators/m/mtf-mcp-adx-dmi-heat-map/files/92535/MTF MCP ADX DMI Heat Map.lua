...
-- http://fxcodebase.com/code/viewtopic.php?f=17&t=60284
...
--+------------------------------------------------------------------+
 ...
 function Add(id, TF,Flag, Instrument )
	...
	indicator.parameters:addInteger("DMIPeriod" .. id, "DMI Period", "", 14);
	
	indicator.parameters:addString("Method".. id, "Method", "Method" , "Both");
    indicator.parameters:addStringAlternative("Method".. id, "ADX", "ADX" , "ADX");
    indicator.parameters:addStringAlternative("Method".. id, "DMI", "DMI" , "DMI");
	indicator.parameters:addStringAlternative("Method".. id, "ADX+DMI", "ADX+DMI" , "Both"); 
	indicator.parameters:addStringAlternative("Method".. id, "ADX + ADX Slope + DMI ", "ADX + Slope" , "Both + Slope");
...
function Init()
    indicator:name("MTF MCP ADX DMI Heat Map");
			   ...
			   x0, x1, x2 = context:positionOfBar (i);
			    ...
			    for j= 1, Number , 1 do
				  ...
				  if p~= false then
										...
										if ADX[j].DATA:hasData(p) 
										and DMI[j].DATA:hasData(p) 
										then 
										
													if SignalMethod[j] == "ADX" then 
													 
																if ADX[j].DATA[p] >   ADXLevel[j] 
																then
																C1=11; C2=12;		 
																elseif  ADX[j].DATA[p] <  ADXLevel[j]
																then
															   C1=21; C2=22;		
																else
																   C1=31; C2=32;		
													 ...
													 elseif SignalMethod[j] == "DMI" 
																then 
																if DMI[j].DIP[p] >  DMI[j].DIM[p] 
																then
																 C1=11; C2=12;		
																elseif  DMI[j].DIP[p] <  DMI[j].DIM[p] 
																then
															    C1=21; C2=22;	
																else
																 	  C1=31; C2=32;				
																end 
													 elseif SignalMethod[j] == "Both"
																 then 
																 if DMI[j].DIP[p] >  DMI[j].DIM[p] 
																 and    ADX[j].DATA[p] >   ADXLevel[j]
																 then
																 C1=11; C2=12;		
																elseif  DMI[j].DIP[p] <  DMI[j].DIM[p]
																and    ADX[j].DATA[p] >   ADXLevel[j] 
																...
																C1=21; C2=22;	  
																 ...
																 if DMI[j].DIP[p] >  DMI[j].DIM[p] 
																 and    ADX[j].DATA[p] >   ADXLevel[j] 
																 and    ADX[j].DATA[p] >    ADX[j].DATA[p-1]
																 then
																 C1=11; C2=12;		
