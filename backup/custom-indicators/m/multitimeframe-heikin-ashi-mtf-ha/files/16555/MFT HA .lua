...
function Init()
	...
	indicator.parameters:addColor("Contrary", "Price Bar Contrary Color","", core.rgb(0, 0, 255));
...
local host;
local RSI={}; 
local Up, Down,Neutral,Contrary ;
local Instrument={};
...
function Prepare(nameOnly)   
	...
	Contrary=instance.parameters.Contrary;
	...
	s2, e2 = core.getcandle(iTF[i], 0, 0, 0);
	 ...
	 Label[Number]="";
				...
				ifirst= Temp1.DATA:first()*2;
				 ...
				 loading[Number]  = true;  	 
				 Indicator[Number] = core.indicators:create("HA", SourceData[Number] );   
...
function Draw (stage, context)
   ...
   context:setClipRectangle(context:left(), context:top(), context:right(), context:bottom());
        ...
        if not init then
			...
			context:createPen (41, context.SOLID, 3, Contrary)       
			    ...
			    for j= 1, Number , 1 do
				  ...
				  if p~= false then
								    ...
								    if Indicator[j].DATA:hasData(p)   then 
												           ...
												           if Indicator[j].close[p] > Indicator[j].open[p] then	 
															C2=12;
															C1=11;	
															...
															elseif Indicator[j].close[p] < Indicator[j].open[p] then	 
															C2=22;
															C1=21;
