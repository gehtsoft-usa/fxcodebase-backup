...
function Init()
	...
	indicator.parameters:addDouble("profile_mult", "Profile Multiplier", "", 1);
...
function Draw(stage, context)
	...
	if not init then
		...
		profile_mult = instance.parameters.profile_mult;
	...
	for _, data in pairs(dates) do
		...
		for count, profile in ipairs(data.Profile) do
			...
			if not Hide and profile > 0 then
				local coeff = (profile / data.Total) * profile_mult;
