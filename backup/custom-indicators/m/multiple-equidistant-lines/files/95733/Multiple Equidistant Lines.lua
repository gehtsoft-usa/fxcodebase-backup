...
function Init()
   ...
   indicator.parameters:addIntegerAlternative("Calculation", "Standard deviation", "Standard deviation" , 4);
	...
	indicator.parameters:addInteger("points", "Font Size","", 10);
...
function Prepare(nameOnly)
    ...
    local name = profile:id() .. "(" .. source:name() ..", " .. ID ..")";
	instance:name(name);
	if nameOnly then
		return;
	end
	...
	core.host:execute("addCommand", 1, "Set Level");
...
function Draw(stage, context)
	...
	init=false;
	context:createFont (2, "Arial", context:pointsToPixels (points), context:pointsToPixels (points), 0)
	end
	
    local iLevel= tostring( db:get("Level", 0 ));
    ...
    for i=1, 20, 1 do  
              if Calculation  == 1 then
                ...
                Value=ATR.DATA[source:size()-1]*Multiplier[i]; 
				elseif Calculation  == 4 then			
                Value= mathex.stdev (source.close, source:size()-1-Helper+1, source:size()-1)*Multiplier[i];		 
				end
				--21
		...
		for i=1, 20, 1 do  
			...
			context:drawText (2,Text1,Label , -1,context:right()-width1, y1-height1, context:right(), y1, 0);
			context:drawText (2,Text2,Label , -1,context:right()-width2, y2-height2, context:right(), y2, 0);
			end
