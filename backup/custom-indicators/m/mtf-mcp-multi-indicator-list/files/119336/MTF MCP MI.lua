...
function Update(period, mode)
 ...
 return
  ...
  core.host:execute("drawLabel1", id, Size*10 ,  core.CR_LEFT, Size*3+(j)*Size*1.5 +Shift  , core.CR_TOP, core.H_Left, core.V_Center, Bold, LabelColor,  Pair[j]);			  
  id = id+1;	
...
--close
				...
				if  Indicator[j][i]:getStream(0):hasData(Indicator[j][i]:getStream(0):size()-1) and Indicator[j][i]:getStream(0):hasData(Indicator[j][i]:getStream(0):size()-2) and SC[i] > 0 then					
						...
						Style = Indicator[j][i]:getStream(0)[Indicator[j][i]:getStream(0):size()-1];			
						 ...
						 if Indicator[j][i]:getStream(0)[Indicator[j][i]:getStream(0):size()-1] > 0 then
										
											
											Color = Up;
											Style= "\225";
						elseif Indicator[j][i]:getStream(0)[Indicator[j][i]:getStream(0):size()-1] < 0 then
