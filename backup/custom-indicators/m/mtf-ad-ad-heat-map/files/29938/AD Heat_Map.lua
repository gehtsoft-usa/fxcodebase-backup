...
function Add(id, TF, Flag, Instrument)
	...
	indicator.parameters:addString("iMethod" .. id, "Calculation Mode", "", "CI")
	indicator.parameters:addStringAlternative("iMethod" .. id, "Classic", "", "CS")
	indicator.parameters:addStringAlternative("iMethod" .. id, "Classic Incremental", "", "CI")
	indicator.parameters:addStringAlternative("iMethod" .. id, "Trade Station", "", "TS")
end

function Init()
	indicator:name("AD Heat_Map")
	...
	indicator.parameters:addColor("Neutral", "Neutral Trend Color", "", core.rgb(255, 0, 0))
...
function Prepare(nameOnly)
	...
	for i = 1, 13, 1 do
		...
		if instance.parameters:getBoolean("On" .. i) and (e1 - s1) <= (e2 - s2) then
			...
			iMethod[Number] = instance.parameters:getString("iMethod" .. i)
			...
			Temp1 = core.indicators:create("AD", source, iMethod[Number])
			...
			loading[Number] = true
			Indicator[Number] = core.indicators:create("AD", SourceData[Number], iMethod[Number])
		end
	end
