...
-- http://fxcodebase.com/code/viewtopic.php?f=17&t=61481
...
function Init()
    indicator:name("MTF MCP Aroon Indicator List");
	...
	indicator.parameters:addColor("NeutralColor", "Neutral Color", "Neutral Color", core.rgb(128, 128,128));
	 ...
	 for i = 1, Count, 1 do	
      ...
      AO[i]={};
		   ...
		   for j = 1, Num, 1 do	
			...
			Temp= core.indicators:create("AROON", source, Period  );
		   ...
		   AO [i][j]= core.indicators:create("AROON", Source[i][j], Period);
...
function Draw(stage, context)
 ...
 function Calculate (context,i, j )
     ...
     if  not   AO[i][j].UP:hasData(AO[i][j].UP:size()-1)
     or  not   AO[i][j].DOWN:hasData(AO[i][j].DOWN:size()-2)  
    ...
    local color=Neutral;	
		   ...
		   if AO[i][j].UP[AO[i][j].UP:size()-1] >  AO[i][j].DOWN[AO[i][j].DOWN:size()-1] and AO[i][j].UP[AO[i][j].UP:size()-2] <=  AO[i][j].DOWN[AO[i][j].DOWN:size()-2]then
		   Symbol="\200";
		   color=UpColor;
		   elseif  AO[i][j].UP[AO[i][j].UP:size()-1] <  AO[i][j].DOWN[AO[i][j].DOWN:size()-1] and AO[i][j].UP[AO[i][j].UP:size()-2] >=  AO[i][j].DOWN[AO[i][j].DOWN:size()-2] then
		   Symbol="\201";
		   color=DownColor;
	       elseif AO[i][j].UP[AO[i][j].UP:size()-1] >  AO[i][j].DOWN[AO[i][j].DOWN:size()-1] then
	      Symbol="\241";
		  color=UpColor;
	  ...
	  if  Type == "Chart" then
		...
		context:createFont (7, "Arial",iwidth/7, iheight , context.ITALIC); 
		...
		elseif  Type == "Chart"  then 
			if i== Count then
			width, height = context:measureText (7, TF[j], 0); 
			context:drawText (7,  TF[j], Color, -1, x1 +(x2-x1)/2-width/2, y1 , x1+(x2-x1)/2 +width/2, y1+height , 0 );	
			end
		width, height = context:measureText (8, Symbol , 0);  
		 context:drawText (8,  Symbol, color, -1, x1 +(x2-x1)/2-width/2, y2, x1+(x2-x1)/2 +width/2, y2+height, 0 );	
