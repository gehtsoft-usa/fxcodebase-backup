...
function Init()
    indicator:name("MTF MCP Murreys Math Oscillator");
    indicator:description("MTF MCP Murreys Math Oscillator");
     indicator:requiredSource(core.Bar);
    indicator:type(core.Oscillator);
  
  
    indicator.parameters:addGroup("Calculation");
	indicator.parameters:addInteger("LookBackLength", "Look back Length", "Look back Length", 100);
    indicator.parameters:addDouble("Multiplier", "Multiplier", "Multiplier", 0.125);
	...
	indicator.parameters:addColor("P1", "1.Positive Zone Color", "Zone Color", core.rgb(173, 255, 47));
	indicator.parameters:addColor("P2", "2. Positive Zone Color", "Zone Color", core.rgb(50, 205, 50));
	indicator.parameters:addColor("P3", "3. Positive Zone Color", "Zone Color", core.rgb(60, 179, 113));
	indicator.parameters:addColor("P4", "4. Positive Zone Color", "Zone Color", core.rgb(0, 128, 0));
	indicator.parameters:addColor("P5", "1. Negative Zone Color", "Zone Color", core.rgb(205, 92, 92));
	indicator.parameters:addColor("P6", "2. Negative Zone Color", "Zone Color", core.rgb(250, 128, 114));
	indicator.parameters:addColor("P7", "3. Negative Zone Color", "Zone Color", core.rgb(255, 160, 122));
	indicator.parameters:addColor("P8", "4. Negative Zone Color", "Zone Color", core.rgb(255, 0, 0));
	indicator.parameters:addColor("P9", "Neutral Color", "Neutral Color", core.rgb(0, 0, 288));
...
local LookBackLength;
local Multiplier;
  
local Indicator={};
local P1, P2, P3, P4, P5, P5, P6, P7, P8,P9;
...
-- Routine
 function Prepare(nameOnly)   
	...
	assert(core.indicators:findIndicator("MURREYS MATH OSCILLATOR") ~= nil, "Please, download and install MURREYS MATH OSCILLATOR.LUA indicator");    
	 ...
	 for i = 1, Count, 1 do	
      ...
      Indicator[i]={};
		   ...
		   for j = 1, Num, 1 do	
			...
			Temp= core.indicators:create("MURREYS MATH OSCILLATOR", source , LookBackLength, Multiplier );
		   ...
		   Indicator [i][j]= core.indicators:create("MURREYS MATH OSCILLATOR", Source[i][j], LookBackLength, Multiplier , false  , P1, P2,P3,P4,P5,P6,P7,P8,P9);
...
function Draw(stage, context)
 ...
 function Calculate (context,i, j )
     ...
     return;
		...
		local Color= Indicator[i][j].DATA:colorI(Indicator[i][j].DATA:size()-1);
		local Value= win32.formatNumber(Indicator[i][j].DATA[Indicator[i][j].DATA:size()-1], false,2);
          
		width, height = context:measureText (7, Value , context.CENTER  );  
		context:drawText (7,  Value, Color, -1, x1+xGap , y1+yGap, x2+xGap  , y2+yGap, context.CENTER   );	
