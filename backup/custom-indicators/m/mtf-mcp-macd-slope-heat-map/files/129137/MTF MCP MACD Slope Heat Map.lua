...
-- http://fxcodebase.com/code/viewtopic.php?f=17&t=69003
...
--+------------------------------------------------------------------+
 ...
 function Add(id, TF,Flag, Instrument )
	...
	indicator.parameters:addInteger("Period1".. id, "Fast Period", "", 12, 1, 2000);
	indicator.parameters:addInteger("Period2".. id, "Fast Period", "", 26, 1, 2000);
	indicator.parameters:addInteger("Period3".. id, "Fast Period", "", 9, 1, 2000);
...
function Init()
    indicator:name("MTF MCP MACD Slope Heat Map");
	...
	indicator.parameters:addString("Type", "Signal Type", "Method" , "MACD_Slope");
    indicator.parameters:addStringAlternative("Type", "MACD_Slope", "MACD_Slope" , "MACD_Slope");
    indicator.parameters:addStringAlternative("Type", "Signal_Slope", "Signal_Slope" , "Signal_Slope");
	indicator.parameters:addStringAlternative("Type", "Histogram_Slope", "Histogram_Slope" , "Histogram_Slope");
	...
	s2, e2 = core.getcandle(iTF[i], 0, 0, 0);
	 ...
	 Label[Number]="";
                ...
                Label[Number]=Label[Number] .. " - " ..  TF[Number];				
			  	...
			  	Temp1= core.indicators:create("MACD", source.close ,  Period1[Number],  Period2[Number],  Period3[Number]);  				
				ifirst= Temp1.DATA:first()*2;
				 ...
				 loading[Number]  = true;  	 
				 Indicator[Number] = core.indicators:create("MACD", SourceData[Number].close ,  Period1[Number],  Period2[Number],  Period3[Number]);   
			   ...
			   x0, x1, x2 = context:positionOfBar (i);
			    ...
			    for j= 1, Number , 1 do
				  ...
				  if p~= false then
							...
							if Type == "MACD_Slope"	 then
										if Indicator[j].MACD:hasData(p) and Indicator[j].MACD:hasData(p-1) then 
												    ...
												    elseif Indicator[j].MACD[p] < Indicator[j].MACD[p-1]then	
															C2=22;
															C1=21;												
							 ...
							 elseif Type == "Signal_Slope"	 then
										if Indicator[j].SIGNAL:hasData(p) and Indicator[j].SIGNAL:hasData(p-1) then 
										
												    if Indicator[j].SIGNAL[p] > Indicator[j].SIGNAL[p-1] then		 
														 
															C2=12;
															C1=11; 
												 
												    elseif Indicator[j].SIGNAL[p] < Indicator[j].SIGNAL[p-1]then	
															C2=22;
															C1=21;												
									   ...
									   C1=1; C2=2;										   
										...
										if Indicator[j].HISTOGRAM:hasData(p) and Indicator[j].HISTOGRAM:hasData(p-1) then 
												    ...
												    elseif Indicator[j].HISTOGRAM[p] < Indicator[j].HISTOGRAM[p-1]then	
															C2=22;
															C1=21;												
