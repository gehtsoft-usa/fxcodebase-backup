...
-- http://fxcodebase.com/code/viewtopic.php?f=17&t=65098
...
function Init()
    indicator:name("MTF MCP  Multi Wave Analysis Heat Map");
	...
	indicator.parameters:addInteger("Selector", "MA Selector", "Selector" , 6);
    indicator.parameters:addIntegerAlternative("Selector", "1. MA", "" , 1);
    indicator.parameters:addIntegerAlternative("Selector", "2. MA", "" , 2);
    indicator.parameters:addIntegerAlternative("Selector", "3. MA", "" , 3);
    indicator.parameters:addIntegerAlternative("Selector", "4. MA", "" , 4);
    indicator.parameters:addIntegerAlternative("Selector", "5. MA", "" , 5);
    indicator.parameters:addIntegerAlternative("Selector", "6. MA", "" , 6);
    indicator.parameters:addIntegerAlternative("Selector", "Average", "" , 7);
...
local Period1, Period2,Period3,Period4,Period5,Period6,Frame;
local Selector;
local Indicator = {} ;
...
function Prepare(nameOnly)   
	...
	assert(core.indicators:findIndicator("MWA") ~= nil, "Please, download and install MWA.LUA indicator");    
	...
	s2, e2 = core.getcandle(iTF[i], 0, 0, 0);
	 ...
	 Label[Number]="";
                ...
                Label[Number]=Label[Number] .. " - " ..  TF[Number];				
			  	...
			  	Temp1= core.indicators:create("MWA", source.close ,  true, true, true, true, true, true, true, Method1, Period1, Period2,   Period3,   Period4,   Period5,   Period6, Method2,Frame    );  				
				ifirst= Temp1.OUT:first()*2;
				 ...
				 loading[Number]  = true;  	 
				 Indicator[Number] = core.indicators:create("MWA", SourceData[Number].close ,  true, true, true, true, true, true, true, Method1, Period1, Period2,   Period3,   Period4,   Period5,   Period6, Method2,Frame );   
			   ...
			   x0, x1, x2 = context:positionOfBar (i);
			    ...
			    for j= 1, Number , 1 do
				  ...
				  if p~= false then
										...
										if Indicator[j]:getStream(Selector-1):hasData(p)  then 
										
												    if SourceData[j].close[p] > Indicator[j]:getStream(Selector-1)[p] then		 
