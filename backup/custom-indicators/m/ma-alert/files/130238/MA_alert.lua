...
--http://fxcodebase.com/code/viewtopic.php?f=17&t=69230
...
{ 
    {
        ...
        UpCondition = function (period)
            if period == 0 or not ma.DATA:hasData(period - 1) then
                return false;
            end
            return source[period] + distance * source:pipSize() >= ma.DATA[period] 
                and source[period - 1] + distance * source:pipSize() < ma.DATA[period - 1];
        end,
...
function Init()
    ...
    indicator.parameters:addInteger("distance", "Distance to MA", "", 10);

    indicator.parameters:addColor("ma_color", "MA Color", "MA Color", core.colors().Red);
    indicator.parameters:addInteger("ma_width", "MA Width", "MA Width", 1, 1, 5);
    indicator.parameters:addInteger("ma_style", "MA Style", "MA Style", core.LINE_SOLID);
...
function Prepare(nameOnly)
    ...
    out = instance:addStream("MA", core.Line, "MA", "MA", instance.parameters.ma_color, 0, 0);
    out:setWidth(instance.parameters.ma_width);
    out:setStyle(instance.parameters.ma_style);
end
