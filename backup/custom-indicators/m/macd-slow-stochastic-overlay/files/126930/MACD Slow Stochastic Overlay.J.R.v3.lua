...
function Init()
    ...
    indi_alerts:AddAlert("RSI/Overbought");
    indi_alerts:AddAlert("RSI/Oversold");
end
...
function Activate(alert, period, historical_period)
	...
	if alert.id == 1 then
        if core.crossesOver(rsi.DATA, rsi_ob_level, p3) then
    ...
    if alert.id == 2 then
        if core.crossesOver(rsi.DATA, rsi_os_level, p3) then
            alert:UpAlert(source, period, alert.Label .. ". Cross over", source.high[period], historical_period);
		elseif core.crossesUnder(rsi.DATA, rsi_os_level, p3) then
            alert:DownAlert(source, period, alert.Label .. ". Cross under", source.low[period], historical_period);
        end
	end
