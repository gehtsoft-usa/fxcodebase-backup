...
function Init()
    indicator:name("MTF MCP MI Change Heat Map");
	...
	s2, e2 = core.getcandle(iTF[i], 0, 0, 0);
	 ...
	 Label[Number]="";
                ...
                Label[Number]=Label[Number] .. " - " ..  TF[Number];				
				 ...
				 Id=Id+1;
				SourceData[Number]  = core.host:execute("getSyncHistory",  Instrument[Number],  TF[Number], source:isBid(), math.min(300,Period[Number]*2), 2000 + Id , 1000 +Id);	
				loading[Number]  = true;				
                
				iprofile[Number] = core.indicators:findIndicator( instance.parameters:getString("Indicator"));  	
				iparams[Number] = instance.parameters:getCustomParameters("Indicator");
									   
							if  iprofile[Number]:requiredSource() == core.Tick then			
								 Indicator[Number] = iprofile[Number]:createInstance( SourceData[Number].close, iparams[Number]);
							else
								 Indicator[Number] = iprofile[Number]:createInstance(SourceData[Number], iparams[Number]);
							end
							
							StreamCount= Indicator[Number]:getStreamCount ();
							
							if StreamNumber >= StreamCount then
							IndicatorStreamNumber[Number]  = StreamCount;
							assert( false, "Incorrect index of stream. The indicator has only ".. StreamCount  .. " stream(s).");
							else
							IndicatorStreamNumber[Number] = StreamNumber;
							end
						 
						  
					 
						 
							Index[Number]=  Indicator[Number]:getStream (IndicatorStreamNumber[Number]);			
			   ...
			   x0, x1, x2 = context:positionOfBar (i);
			    ...
			    for j= 1, Number , 1 do
				  ...
				  if p~= false then
										...
										if Index[j]:hasData(p) and Index[j]:hasData(p-Period[j]) then 
										
												    if Index[j][p] > Index[j][p-Period[j]] then		 
															 
															C2=12;
															C1=11;
													elseif Index[j][p] < Index[j][p-Period[j]] then		
															C2=22;
															C1=21;
