...
function Init()
    indicator:name("Averages indicator with horizontal shift");
    indicator:description("Averages indicator with horizontal shift");
    ...
    indicator.parameters:addInteger("Shift", "Shift (in bars)", "", 0);
...
function Prepare(onlyName)
    ...
    name = profile:id() .. "(" .. source:name() .. "," .. instance.parameters.Method .. "," .. Period .. "," .. Shift .. ")";
    instance:name(name);
    if onlyName then
        return ;
    end
    ...
    MainBuff = instance:addStream("MainBuff", core.Line, name .. ".MA", "MA", instance.parameters.MainClr, updateParams.first+Shift,Shift);
    ...
    if period >= first then
        ...
        if ColorMode then
            if MainBuff[period] > MainBuff[period - 1] then
                MainBuff:setColor(period+Shift, UPclr);
            elseif MainBuff[period] < MainBuff[period - 1] then
                MainBuff:setColor(period+Shift, DNclr);
            end
        end
    end
end
...
function MVAUpdate(params, period, mode)
    params.buffer[period+params.shift] = mathex.avg(params.source, period - params.offset, period);
end
...
function EMAUpdate(params, period, mode)
    if period == params.first then
        params.buffer[period+params.shift] = params.source[period];
    else
        params.buffer[period+params.shift] = (1 - params.k) * params.buffer[period - 1+params.shift] + params.k * params.source[period];
    end
end
...
function KAMAInit(source, n)
    local p = {};
    p.first = source:first()+1;
    p.first1 = source:first()+n;
    p.shift=Shift;
    p.n=n;
    p.source = source;
    p.INT = instance:addInternalStream(p.first, 0);
    return p;
end

function KAMAUpdate(params, period, mode)
    params.INT[period] = math.abs(params.source[period]-params.source[period-1]);
    params.buffer[period+params.shift]=nil;
    if period>=params.first1 then
        local value=0;
        if period==params.first1 then
         value=params.source[period];
        else
         value=params.buffer[period-1+params.shift];
        end
        local er=0;
        er=core.sum(params.INT,period-params.n+1,period);
        if er~=0 then
         er=math.abs(params.source[period]-params.source[period-params.n+1])/er;
        end
        local sc=er*0.6015+0.0645;
        sc=sc*sc;
        params.buffer[period+params.shift]=value+sc*(params.source[period]-value);
    end
end
...
function LWMAUpdate(params, period, mode)
    params.buffer[period+params.shift] = mathex.lwma(params.source, period - params.offset, period);
end
...
function WilderUpdate(params, period, mode)
    if period == params.first then
        params.buffer[period+params.shift] = mathex.avg(source, period - params.n + 1, period);
    else
        params.buffer[period+params.shift] = ((params.source[period] - params.buffer[period - 1+params.shift]) * params.k) + params.buffer[period - 1+params.shift];
    end
end
...
function SMMAUpdate(params, period, mode)
    if period == params.first then
        params.buffer[period+params.shift] = mathex.avg(params.source, period - params.n + 1, period);
    else
        params.buffer[period+params.shift] = (params.buffer[period - 1+params.shift] * (params.n - 1) + params.source[period]) / params.n;
    end
end
...
function GeoMeanUpdate(params, period, mode)
    ...
    params.buffer[period+params.shift] = math.pow(s, params.exp);
end
...
function SineWMAUpdate(params, period, mode)
    ...
    params.buffer[period+params.shift] = sum;
end
...
function TriMAUpdate(params, period, mode)
    ...
    params.buffer[period+params.shift] = mathex.avg(params.mabuffer, period - off, period);
end
...
function LSMAUpdate(params, period, mode)
    params.buffer[period+params.shift] = mathex.lreg(params.source, period - params.offset, period);
end
...
function HMAUpdate(params, period, mode)
    ...
    params.buffer[period+params.shift] = mathex.lwma(params.tmp, period - params.len1 + 1, period);
end
...
function ZeroLagEMAUpdate(params, period, mode)
    if period == params.first then
        ...
        params.buffer[period+params.shift] = params.alpha * (2 * params.source[period] - params.source[period - params.lag]) +
                                (1 - params.alpha) * params.buffer[period - 1+params.shift];
    end
end
...
function DEMAUpdate(params, period, mode)
    if period == params.first then
        ...
        params.buffer[period+params.shift] = 2 * ema[period] - ema2[period];
    end
end
...
function ITrendUpdate(params, period, mode)
    ...
    if period <= params.first7 then
        params.buffer[period+params.shift] = (src[period] + 2 * src[period - 1] + src[period - 2]) / 4;
    else
        params.buffer[period+params.shift] = params.k1 * src[period] + params.k2 * src[period - 1] - params.k3 * src[period - 2] +
                                params.k4 * params.buffer[period - 1+params.shift] - params.k5 * params.buffer[period - 2+params.shift];
    end
end
...
function MedianUpdate(params, period, mode)
    ...
    table.sort(arr);
    if params.even then
        params.buffer[period+params.shift] = arr[params.middle];
    else
        params.buffer[period+params.shift] = (arr[params.middle] + arr[params.middle + 1]) / 2;
    end
end
...
function REMAUpdate(params, period, mode)
    if period <= params.first3 then
        ...
        local rp = params.buffer[period - 1+params.shift];
        local rpp = params.buffer[period - 2+params.shift];
        params.buffer[period+params.shift] = (params.k * params.source[period] + (1 - params.k) * rp + 0.5 * (2 * rp - rpp)) / 1.5;
    end
end
...
function ILRSUpdate(params, period, mode)
    ...
    params.buffer[period+params.shift] = mathex.lregSlope(params.source, from, period) + mathex.avg(params.source, from, period);
end
...
function IE_2Update(params, period, mode)
    local from = period - params.offset;
    params.buffer[period+params.shift] = (mathex.lregSlope(params.source, from, period) + mathex.avg(params.source, from, period) + mathex.lreg(params.source, from, period)) / 2;
end
...
function TriMAgenUpdate(params, period, mode)
    ...
    params.buffer[period+params.shift] = mathex.avg(params.mabuffer, period - params.offset2, period);
end
...
function JSmoothUpdate(params, period, mode)
    if period < params.first3 then
        ...
        params.a4[period]     = (params.a3[period] - params.buffer[period - 1+params.shift]) * params.alpha1_2 + params.alpha_2 * params.a4[period - 1];
        params.buffer[period+params.shift] = params.buffer[period - 1+params.shift] + params.a4[period];
    end
end
