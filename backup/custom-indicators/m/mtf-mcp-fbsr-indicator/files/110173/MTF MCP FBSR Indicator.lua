...
-- http://fxcodebase.com/code/viewtopic.php?f=17&t=64230&p=110173#p110173
...
function Init()
    indicator:name("MTF MCP FBSR Indicator ");
...
-- Routine
 function Prepare(nameOnly)   
	...
	Color= instance.parameters.Color; 
	 ...
	 assert(core.indicators:findIndicator("FBSR") ~= nil, "Please, download and install FBSR.LUA indicator");
...
function Draw(stage, context)
 ...
 function Calculate (context,i, j )
     ...
     if  ( not   Indicator[i][j].R:hasData(Indicator[i][j].R:size()-1)
	 and  not   Indicator[i][j].S:hasData(Indicator[i][j].S:size()-1)
	 )     
     then
     return;
     end
    ...
    local SymbolColor=Neutral;	
	   ...
	   --"\32";
		  ...
		  if Source[i][j].close[ Source[i][j].close:size()-1] > Indicator[i][j].R [Indicator[i][j].R:size()-1] 
		  then
		  Symbol="\225"; 
		  SymbolColor=Up;
			   if Indicator[i][j].R:hasData(Indicator[i][j].R:size()-2) and Source[i][j].close[ Source[i][j].close:size()-2] <= Indicator[i][j].R [Indicator[i][j].R:size()-2]   then
			   Symbol=  "\236";
			   end
		  
		  elseif Source[i][j].close[Source[i][j].close:size()-1] < Indicator[i][j].S [Indicator[i][j].S:size()-1] 
		  then
		  Symbol="\226"; 
		  SymbolColor=Down;
		  
			  if Indicator[i][j].S:hasData(Indicator[i][j].S:size()-2) and Source[i][j].close[Source[i][j].close:size()-2] >= Indicator[i][j].S [Indicator[i][j].S:size()-2]   then
			  Symbol=   "\238";
			  end
		  else
		  SymbolColor=Neutral;
