...
-- http://fxcodebase.com/code/viewtopic.php?f=17&t=68913
...
local source;
local out, up_color, down_color, mid1_color, mid2_color;
local atr, ma, Keltner_Deviation, ml, BB_Length, BB_Deviation;
local momHist, momPeriod, momHistVal, dots, up_dots_color, down_dots_color;
local alerts = 
{ 
    {
        Stage = 102,
        UpCondition = function (period)
            local H = ma.DATA[period] + atr.DATA[period] * Keltner_Deviation;
            local L = ma.DATA[period] - atr.DATA[period] * Keltner_Deviation;
            local D = mathex.stdev(source.close, period - BB_Length, period);
            local TL = ml.DATA[period] + BB_Deviation * D;
            local BL = ml.DATA[period] - BB_Deviation * D;
            return TL > H and momHistVal.DATA[period] > 0;
        end,
...
function Init()
    indicator:name("TTMS (logicgate)");
    indicator:description("");
    indicator:requiredSource(core.Bar);
    indicator:type(core.Oscillator);
    
    indicator.parameters:addInteger("BB_Length", "BB_Length", "", 20);
    indicator.parameters:addDouble("BB_Deviation", "BB_Deviation", "", 2);
    indicator.parameters:addInteger("Keltner_Length", "Keltner_Length", "", 20);
    indicator.parameters:addInteger("Keltner_Smooth_Length", "Keltner_Smooth_Length", "", 20);
    AddAverages("Keltner_Smooth_Method", "", "MVA");
    indicator.parameters:addInteger("Keltner_Deviation", "Keltner_Deviation", "", 2);
    indicator.parameters:addInteger("momPeriod", "Momemtum Period", "", 12);
    indicator.parameters:addInteger("momEMA", "Momentum EMA Period", "", 5);

    indicator.parameters:addColor("up_color", "Momentum Up", "", core.colors().Blue);
    indicator.parameters:addColor("down_color", "Momentum Down", "", core.colors().Orange);
    indicator.parameters:addColor("mid1_color", "Momentum Mid", "", core.colors().Brown);
    indicator.parameters:addColor("mid2_color", "Momentum Mid", "", core.colors().LightBlue);

    indicator.parameters:addColor("up_dots_color", "Up dots color", "", core.colors().Green);
    indicator.parameters:addColor("down_dots_color", "Down dots color", "", core.colors().Red);
    indicator.parameters:addInteger("dots_size", "Dots size", "", 1, 1, 5)
...
function Prepare(nameOnly)
    ...
    mid1_color = instance.parameters.mid1_color;
    mid2_color = instance.parameters.mid2_color;
    Keltner_Deviation = instance.parameters.Keltner_Deviation;
    BB_Length = instance.parameters.BB_Length;
    BB_Deviation = instance.parameters.BB_Deviation;
    momPeriod = instance.parameters.momPeriod;
    up_dots_color = instance.parameters.up_dots_color;
    down_dots_color = instance.parameters.down_dots_color;

    atr = core.indicators:create("ATR", source, instance.parameters.Keltner_Smooth_Length);
    ma = CreateAverages(instance.parameters.Keltner_Smooth_Method, source, instance.parameters.Keltner_Length);
    ml = core.indicators:create("MVA", source, BB_Length);
    momHist = instance:addInternalStream(0, 0);
    momHistVal = core.indicators:create("EMA", momHist, instance.parameters.momEMA);

    dots = instance:addStream("dots", core.Dot, "Dots", "Dots", up_dots_color, 0, 0);
    dots:setPrecision(math.max(2, instance.source:getPrecision()));
    dots:setWidth(instance.parameters.dots_size);

    out = instance:addStream("out", core.Bar, "Momentum", "Momentum", up_color, 0, 0);
...
function Update(period, mode)
    ...
    ma:update(mode);
    ml:update(mode);
    if not ma.DATA:hasData(period) or not atr.DATA:hasData(period) then
        return;
    end
    if not ml.DATA:hasData(period) then
        return;
    end
    momHist[period] = source.close[period] - source.close[period - momPeriod];
    momHistVal:update(mode);
    if not momHistVal.DATA:hasData(period) then
        return;
    end
    out[period] = momHistVal.DATA[period];
    
    local buy = alerts[1].UpCondition(period);
    local sell = alerts[1].DownCondition(period);
    if buy then
        out:setColor(period, up_color);
    elseif sell then
        out:setColor(period, down_color);
    elseif out[period] > 0 then
        out:setColor(period, mid1_color);
    else
        out:setColor(period, mid2_color);
    end

    dots[period] = 0;
    if (buy and not sell) then
        dots:setColor(period, up_dots_color)
    elseif (not buy and sell) then
        dots:setColor(period, down_dots_color)
    else
        dots:setColor(period, dots:colorI(period - 1));
    end
