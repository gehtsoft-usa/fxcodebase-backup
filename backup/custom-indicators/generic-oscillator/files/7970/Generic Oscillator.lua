...
function Init()
    indicator:name("Generic Oscillator");
    indicator:description("Generic Oscillator");
...
local MA_Period, MA_Method,Type;
...
function Prepare(nameOnly)
	...
	local name = profile:id() .. "(" .. source:name() .. ", " .. MA_Period ..", " ..MA_Method.. ")";
    instance:name(name);
    if nameOnly then
        return;
    end
...
function Update(period, mode)
	...
	return;
	 ...
	 if not  DATA.DATA:hasData(period) then
	 return;
	 end
	
        Out[period] = source[period]  - DATA.DATA[period];
