...
-- A: active; B: base
-- A: nature moving average; B: simple moving average
-- N: moving average look back; C: nature moving average step

function Init()
    indicator:name("RichardTaoDirectionalLine_nature");
...
function Update(period, mode)
    ...
    and iperiod > oPara.pBGN then
        ...
        speriod = PRA.sperd;  
        PRA.ln1[iperiod] = CalcSna(oPara.pNBK, oPara.pCHK, PRA.ln1[iperiod-1], PRA.sunit, SRS.close, speriod);
        PRA.ln2[iperiod] = CalcSma(oPara.pNBK, 0, SRS.close, speriod);
        PRA.ln1:setColor(iperiod, Iif(PRA.ln1[iperiod]-PRA.ln1[iperiod-1]<0, instance.parameters.clrM, instance.parameters.clrC));
    end
...
function SetParameter(oPara, sourcep, iperiod)
    ...
    oPara.pCHK = instance.parameters.C;
        --oPara.pCHK = 0.5, 0.2-1.0
    if oPara.pNBK == 0 then oPara.pNBK = 50; end
    oPara.pTYP = 5;     
