...
-- https://fxcodebase.com/code/viewtopic.php?f=17&t=72097
...
function Init()
    indicator:name("100-pips-indikator~");
    ...
    indicator.parameters:addInteger("Gi_76", "Period", "", 40, 1, 2000);
...
-- Routine
 function Prepare(nameOnly)   
	...
	Gi_76=instance.parameters.Gi_76;
	source = instance.source
 
    local name = profile:id() .. "(" ..  instance.source:name().. "," ..  Gi_76  .. ")";
    instance:name(name); 
	...
	Indicator1= core.indicators:create("LWMA", source, math.floor(Gi_76/2));
	Indicator2= core.indicators:create("LWMA", source, Gi_76);	
	...
	Indicator3= core.indicators:create("LWMA", Line1, math.floor(math.sqrt(Gi_76)));			
...
function Update(period, mode)
	 ...
	 Line1[period]= Indicator1.DATA[period]-Indicator2.DATA[period];

 	Indicator3:update(mode); 
	  
 	 if period <= first + math.floor(Gi_76)  then
	 return;
	 end
	...
	Line2[period]= Indicator3.DATA[period];
