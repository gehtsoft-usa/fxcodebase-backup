...
function Signal_Logic (id, period)
	  ...
	  --  if not Show_Unconfirmed then
	   -- Alert[id][period]= 0;	
	    ...
	    if id== 1  then   
			if  QQE[period] > TS1[period] 
			and   QQE[period-1] <= TS1[period-1] 
			then
	    ...
	    --id 1 will define Cross Over Alert
	    if id== 3  then   
			if  QQE[period] > TS2[period] 
			and   QQE[period-1] <= TS2[period-1] 
			then
