...
function Init()
    indicator:name("MTF MCP Engulfing Signal Dashboard");
	...
	for j = 1, Count, 1 do
		 ...
		 for i = 1, Num, 1 do	
			   ...
			   loading[j][i] = true;  
                ...
                Indicator[j][i] =core.indicators:create("ENGULFING SIGNAL", SourceData[j][i],  true, HL)
...
function FindLast(j,i)
		...
		local IndexOfLast= Indicator[j][i].DATA:size()-1
		
			      for k = Indicator[j][i].DATA:size()-1, 0, - 1 do
				   
				   
							if  Indicator[j][i].DATA[k] ==1  then
