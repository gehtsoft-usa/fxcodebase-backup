...
function Init()
	...
	Parameters (4, "Central Line Slope");
end
...
function Prepare(nameOnly)
	...
	Signal = instance:addInternalStream(first, 0);
...
function Update(period, mode)
   ...
   if (period<first+2*TMA_Period) then
   return;
   end
   ...
   if Last < first+2*TMA_Period+1 then
   return;
   end
   ...
   if Live~= "Live" then
	...
	Shift=1;
	  ...
	  if id == 1  and ON[id]  then
			...
			if  source.close[period] > TMA[period] 
			and   source.close[period-1] <= TMA[period-1] 
			then
			           
						    
             core.host:execute("drawLabel1", source:serial(period), source:date(period),  core.CR_CHART, TMA[period], core.CR_CHART, core.H_Center, core.V_Bottom, font, UpTrendColor, "\225");
			...
			elseif  source.close[period] < TMA[period] 
			and   source.close[period-1] >= TMA[period-1] 
            then			
			
			            			 
			               core.host:execute("drawLabel1", source:serial(period), source:date(period),  core.CR_CHART, TMA[period], core.CR_CHART, core.H_Center, core.V_Top, font, DownTrendColor, "\226");						   
	   ...
	   if id == 2  and ON[id]  then
			...
			if  source.close[period] > Upper[period] 
			and   source.close[period-1] <= Upper[period-1] 
			then
			           
						    
             core.host:execute("drawLabel1", source:serial(period), source:date(period),  core.CR_CHART, Upper[period], core.CR_CHART, core.H_Center, core.V_Bottom, font, UpTrendColor, "\225");
			...
			elseif  source.close[period] < Upper[period] 
			and   source.close[period-1] >= Upper[period-1] 
            then			
			
			            			 
			               core.host:execute("drawLabel1", source:serial(period), source:date(period),  core.CR_CHART, Upper[period], core.CR_CHART, core.H_Center, core.V_Top, font, DownTrendColor, "\226");						   
	   ...
	   if id == 3  and ON[id]  then
			...
			if  source.close[period] > Lower[period] 
			and   source.close[period-1] <= Lower[period-1] 
			then
			           
						    
             core.host:execute("drawLabel1", source:serial(period), source:date(period),  core.CR_CHART, Lower[period], core.CR_CHART, core.H_Center, core.V_Bottom, font, UpTrendColor, "\225");
			...
			elseif  source.close[period] < Lower[period] 
			and   source.close[period-1] >= Lower[period-1] 
            then			
			
			            			 
			               core.host:execute("drawLabel1", source:serial(period), source:date(period),  core.CR_CHART, Lower[period], core.CR_CHART, core.H_Center, core.V_Top, font, DownTrendColor, "\226");						   
