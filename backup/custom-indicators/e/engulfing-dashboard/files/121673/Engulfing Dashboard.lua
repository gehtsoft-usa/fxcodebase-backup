...
function Init()
    indicator:name("Engilfing Dashboard");
...
function Prepare(nameOnly)
	...
	assert(core.indicators:findIndicator("ENGULFING") ~= nil, "Please, download and install ENGULFING.LUA indicator");
    local last_id = 0;
    local symbol_index = 1;
    for i = 1, 20, 1 do
        Dodaj = instance.parameters:getBoolean("Dodaj" .. i);
        if Dodaj then
            ...
            for ii = 1, #iTF do
                ...
                if use then
                    local symbol = {};
                    symbol.Pair = instance.parameters:getString("Pair" .. i);
                    ...
                    symbol.Loading = true;
                    symbol.SymbolIndex = symbol_index;
                    symbol.TimeframeIndex = timeframe_index;
                    symbol.Indicator = core.indicators:create("ENGULFING", symbol.Source);
                    last_id = last_id + 2;
            ...
            instruments[symbol_index] = instance.parameters:getString("Pair" .. i);
...
function FormatTime(time)
    ...
    if (diff_date.hour > 0) then
        return diff_date.hour .. " hour(s) ago";
    end
    if (diff_date.min > 0) then
        return diff_date.min .. " minute(s) ago";
    end
    return "now";
end
...
function Draw(stage, context) 
    ...
    local title = "Engulfing Dashboard";
    local title_w, title_h = context:measureText(FONT_TEXT, title, 0);
    CellsBuilder:Clear(context);
    for i = 1, #instruments do
        CellsBuilder:Add(FONT_TEXT, instruments[i], text_color, 1, (i + 1) * 2, context.LEFT);
    end
    ...
    for _, symbol in ipairs(symbols) do
        if not symbol.Loading then
            ...
            local row = (symbol.SymbolIndex + 1) * 2;
            if signal == 0 then
                CellsBuilder:Add(FONT_TEXT, "-", text_color, symbol.TimeframeIndex + 1, row, context.CENTER);
            elseif signal == 1 then
                CellsBuilder:Add(FONT, "\233", instance.parameters.up_color, symbol.TimeframeIndex + 1, row, context.CENTER);
                CellsBuilder:Add(FONT_TEXT, FormatTime(time), text_color, symbol.TimeframeIndex + 1, row + 1, context.CENTER);
            else
                CellsBuilder:Add(FONT, "\234", instance.parameters.dn_color, symbol.TimeframeIndex + 1, row, context.CENTER);
                CellsBuilder:Add(FONT_TEXT, FormatTime(time), text_color, symbol.TimeframeIndex + 1, row + 1, context.CENTER);
            end
        end
    end
