...
function Init()
    indicator:name("Asymmetric Ease of move");
    indicator:description("");
    indicator:requiredSource(core.Tick);
    indicator:type(core.Oscillator);
	
	indicator.parameters:addGroup("Calculation"); 
    indicator.parameters:addInteger("Period", "Period", "", 7, 2, 2000);
...
function Update(period, mode)
	...
	Div1=mathex.sum(CountUp, period-Period+1, period)
	Div2=mathex.sum(CountDown, period-Period+1, period)
		if Div1 ~=0 then
		
			if Div1 > Div2 then
			AverageUp[period]=mathex.avg(Up, period-Period*2+1, period)/(mathex.sum(Up, period-Period*2+1, period)/(Div1*2));
			else
			AverageUp[period]=mathex.avg(Up, period-Period+1, period)/(mathex.sum(Up, period-Period+1, period)/Div1);
			end
		end
		...
		if Div2 ~=0 then
			...
			if Div1 < Div2 then
			AverageDown[period]=mathex.avg(Down, period-Period*2+1, period)/(mathex.sum(Down, period-Period*2+1, period)/(Div2*2));
			else
			AverageDown[period]=mathex.avg(Down, period-Period+1, period)/(mathex.sum(Down, period-Period+1, period)/Div2);
			end
		end
