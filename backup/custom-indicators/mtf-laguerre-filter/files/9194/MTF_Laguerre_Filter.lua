...
-- http://fxcodebase.com/code/viewtopic.php?f=17&t=3779
...
function AddParam(id, frame , Flag)
    ...
    indicator.parameters:setFlag("Pair" .. id, core.FLAG_INSTRUMENTS);
	  ...
	  indicator.parameters:addDouble("GAMMA"..id, "Gamma", "", 0.7, -0.5, 1.05);

end


function Init()
    indicator:name("MTF_MCP Laguerre Filter");
    ...
    indicator.parameters:addColor("DN", "Color for Down Trend", "", core.rgb(255, 0, 0));	
	 ...
	 indicator.parameters:addColor("Above", "Color for  Above Price", "", core.rgb(0, 255, 0));
    indicator.parameters:addColor("Below", "Color for Below Price ", "", core.rgb(255, 0, 0));	
...
function Prepare(nameOnly)
	...
	Above=instance.parameters.Above;
	Below=instance.parameters.Below;
    ...
    for i = 1, Count, 1 do
	    ...
	    USE[i]=instance.parameters:getBoolean("USE" .. i);
		 ...
		 Number=Number+1;
		     ...
		     GAMMA[Number]=instance.parameters:getDouble (" GAMMA"..i);		 
   ...
   assert(core.indicators:findIndicator("LAGUERRE_FILTER") ~= nil, "Please, download and install LAGUERRE_FILTER.LUA indicator");
    ...
    for i = 1, Number, 1 do	 
	    	...
	    	local Test = core.indicators:create("LAGUERRE_FILTER", source  , GAMMA[i]);   
	        first= Test.DATA:first()*2 ; 		  
	  	
	  
	    SourceData[i] = core.host:execute("getSyncHistory", Pair[i], TF[i], source:isBid(),  first , 200+i, 100+i);	   
		Indicator[i] = core.indicators:create("LAGUERRE_FILTER", SourceData[i],GAMMA[i]);
		loading[i]= true;		
	...
	local Symbol="\149";
		...
		for i = 1, Number, 1 do 	
		    ...
		    context:drawText (2, tostring(TF[i]), Label, -1, context:right ()-(i)*xCell    , context:top ()+yCell, context:right ()-(i-1)*xCell + width, context:top ()+yCell + height, 0);
			 ...
			 if Indicator[i].DATA[Indicator[i].DATA:size()-1] >  SourceData[i].close[SourceData[i].close:size()-1] then
			 context:drawText (2,  Value, Above, -1, context:right ()-(i)*xCell    , context:top ()+yCell*3, context:right ()-(i-1)*xCell + width, context:top ()+yCell*3 + height, 0);
			else
			 context:drawText (2,  Value, Below, -1, context:right ()-(i)*xCell    , context:top ()+yCell*3, context:right ()-(i-1)*xCell + width, context:top ()+yCell*3 + height, 0);
			end
