...
function Draw()
	...
	id=id+1;
	core.host:execute("drawLine", id, source:date(first), source.high[i], source:date(source:size()-1),  source.high[i], up, instance.parameters.style, instance.parameters.width,  win32.formatNumber(source.high[i], false, source:getPrecision()));
	id=id+1;
    core.host:execute("drawLine", id, source:date(first), source.low[i], source:date(source:size()-1), source.low[i], down, instance.parameters.style, instance.parameters.width,  win32.formatNumber(source.low[i], false, source:getPrecision()));
	end 
