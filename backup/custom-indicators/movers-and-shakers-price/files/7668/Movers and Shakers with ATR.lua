...
function Init()
	...
	indicator.parameters:addStringAlternative("Mode", "ATR", "ATR" , "ATR");
...
function AddTimeFrame(id , FRAME , DEFAULT  )
    ...
    indicator.parameters:addInteger("Period"..id, "Period", "Period",0);
	 indicator.parameters:addInteger("ATRPeriod"..id, "ATR Period", "Period",14);
end	
	...
	Num=0;
		for i = 1 , 13 , 1 do  
		   ...
		   if Use[i] then
			...
			ATRPeriod[Num]=instance.parameters:getInteger("ATRPeriod" .. i);
			TF[Num]=  iTF[i];	
	...
	Color= instance.parameters.Color; 
	 ...
	 for i = 1, Count, 1 do	
	  ...
	  Value[i][Num+1]=Pair[i];
		   ...
		   for j = 1, Num, 1 do	
		    ID=ID+1;  
		   Source[i][j]= core.host:execute("getSyncHistory", Pair[i], TF[j], source:isBid(), math.min(300 , Period[j]),20000 + ID , 10000 +ID);
		   
		   if Mode=="ATR" then
		    Indicator[i][j]  = core.indicators:create("ATR", Source[i][j], ATRPeriod[j]);
		  end
...
function ReCalculate (context,i, j)
		...
		context:drawRectangle( -1, 3 , x1, y1, x2, y2, transparency);	
			...
			if Show then
			    ...
			    if Mode == "ATR" then
				 width, height = context:measureText (7, string.format("%." .. 5 .. "f", Sorted  [i][j]), context.ITALIC);  
				context:drawText (7,  string.format("%." .. 5 .. "f", Sorted  [i][j]), color, -1, x1, y2-height, x1+width, y2, 0 );
				else
...
function Initialize (context,i )
 ...
 function Calculate (i, j )
     ...
     or not Source[i][j].close:hasData(Source[i][j].close:size()-1-Period[j])
     then
     return;
     end
	 
	if Mode== "ATR" then
	       Value[i][j]=  Indicator[i][j].DATA[Indicator[i][j].DATA:size()-1];
     elseif Mode== "Pip" then
	            Value [i][j]= (Source[i][j].close[Source[i][j].close:size()-1]  - Source[i][j].open[Source[i][j].open:size()-1-Period[j]]  )/Point[i];
				
	   elseif Mode== "Percentage" then
	            Value  [i][j]= (Source[i][j].close[Source[i][j].close:size()-1]  - Source[i][j].open[Source[i][j].open:size()-1-Period[j]]  )/ (Source[i][j].open[Source[i][j].open:size()-1-Period[j]]/100);			
