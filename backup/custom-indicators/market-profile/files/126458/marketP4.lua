...
function Init()
    ...
    indicator.parameters:addString("EndTime", "End Time for Trading", "", "19:00:00")
end
...
local time_shift;
local OpenTime, CloseTime, time_diff;
...
function Prepare(nameOnly)
    ...
    OpenTime, valid = ParseTime(instance.parameters.StartTime)
    assert(valid, "Time " .. instance.parameters.StartTime .. " is invalid")
    CloseTime, valid = ParseTime(instance.parameters.EndTime)
    assert(valid, "Time " .. instance.parameters.EndTime .. " is invalid")
...
function FixEndTime(date)
    local ts_date = core.host:execute("convertTime", core.TZ_EST, core.TZ_TS, date);
    return core.host:execute("convertTime", core.TZ_TS, core.TZ_EST, math.floor(ts_date) + CloseTime);
end