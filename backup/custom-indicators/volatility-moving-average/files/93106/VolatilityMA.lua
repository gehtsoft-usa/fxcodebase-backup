...
function Update(period)
	...
	return;
		...
		local  Average = mathex.avg(Raw1, period-Lookback+1, period);
       
	    local j; 
		local  Variance=0;
		
		for j = 0, Lookback, 1 do			 
		Variance =Variance+ math.pow( (Raw2[period-j] - Average),2);				
		end
					
		local  sDev = math.sqrt( Variance/Lookback );
