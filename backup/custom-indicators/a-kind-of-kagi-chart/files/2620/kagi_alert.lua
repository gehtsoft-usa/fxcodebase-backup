...
function Update(period)
    ...
    local lastAddedUL = nil;
    local lastAddedDL = nil;
    for i = 0, count, 1 do
        if p >= first then
            if i > 1 then
                ...
                if kagi[i] > kagi[i - 1] then
                    if i > 2 and kagi[i] > kagi[i - 2] then
                        ...
                        if lastAddedUL ~= nil and lastAddedUL ~= 1 then 
                           SendAlert("Upper line change thin to thick")
			   SoundAlert(SoundFile)
                        end  
                        lastAddedUL = 1;
                    else
                        UL:set(p, kagi[i], "\226", label);
                        if lastAddedUL ~= nil and lastAddedUL ~= 0 then 
                           SendAlert("Upper line change thick to thin")
			   SoundAlert(SoundFile)
                        end  
                        lastAddedUL = 0;
                    end
                    ...
                    if i > 2 and kagi[i] < kagi[i - 2] then
                        ...
                        if lastAddedDL ~= nil and lastAddedDL ~= 1 then 
                           SendAlert("Bottom line change thin to thick")
			   SoundAlert(SoundFile)
                        end  
                        lastAddedDL = 1;
                    else
                        DL:set(p, kagi[i], "\225", label);
                        if lastAddedDL ~= nil and lastAddedDL ~= 0 then 
                           SendAlert("Bottom line change thick to thin")
			   SoundAlert(SoundFile)
                         end  
                        lastAddedDL = 0;
                    end
