...
function Prepare(onlyName)
   ...
   if instance.parameters.ShowMode == "TODAY" then
    ...
    if onlyName then
        assert(l1 <= l2, "Incorrect timeframe");
        return;
    end
    ...
    ref = core.host:execute("getSyncHistory", source:instrument(), "H1", source:isBid(), 10, 100, 101);
...
function Update(period, mode)
    ...
    local candle = core.getcandle(BS, source:date(period), firstHour, weekoffset);
    local candle_start, candle_end = core.getcandle(BS, candle - 1, firstHour, weekoffset);
    if candle_start < ref:date(0) then
        return;
    end
    local start_index = core.findDate(ref, candle_start, false);
    if start_index == -1 then
        return;
    end
    core.host:trace('3');
    local next_start_index = core.findDate(ref, candle_end, false);
    if next_start_index == -1 or start_index == next_start_index then
        core.host:trace(tostring(next_start_index) .. ", " .. start_index)
        return;
    end
    core.host:trace('4');
    local end_index = next_start_index - 1;
    local low, high = mathex.minmax(ref, start_index, end_index);
    core.host:trace('5');
    if CalcMode == O_PIVOT or CalcMode == O_FIB or CalcMode == O_FLOOR then
        P[period] = (high + ref.close[end_index] + low) / 3;
    elseif CalcMode == O_CAM then
        P[period] = ref.close[end_index];
    elseif CalcMode == O_FIBR then
        P[period] = (high + low) / 2;
    elseif CalcMode == O_WOOD then
        P[period] = (high + low + ref.open[next_start_index] * 2) / 4;
    end
    core.host:trace('6');
    H[period] = high;
    L[period] = low;
...
function ShowLevels(data, period)
    local d1, d2 = core.getcandle(BS, source:date(period), firstHour, weekoffset);
