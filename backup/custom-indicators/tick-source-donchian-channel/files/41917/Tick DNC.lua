...
-- http://fxcodebase.com/code/viewtopic.php?f=17&t=24360
...
function Update(period)
    if (period >= first) then
        local range;
        if (ac) then
            du[period] =  mathex.max (source, period -n +1  , period);
            dn[period] = mathex.min (source, period -n +1  , period);
        else
             du[period] =  mathex.max (source, period -n +1-1  , period-1);
            dn[period] = mathex.min (source, period -n +1-1  , period-1);
        end
