...
function Init()
    indicator:name("Candle meter overlay")
...
function Prepare(nameOnly)
    ...
    CandleMeter = instance:addInternalStream(0, 0);
end
...
function Update(period, mode)
    ...
    local Direction
    if
        source.close[period] > source.open[period] or
...
function Activate(alert, period, historical_period)
    ...
    if alert.id == 1 then
        if period > 0 and core.crossesOver(CandleMeter, 0, period) then
            alert:UpAlert(source, period, alert.Label .. ". Bull pattern", source.high[period], historical_period);
        elseif period > 0 and core.crossesUnder(CandleMeter, 0, period) then
