...
function Init()
    ...
    indicator.parameters:addInteger("RESET", "Number of Rows", "", 2);
...
function Calculate(period, mode)
    ...
    for j = 1, Count, 1 do
        ...
        HShift[j] = Size * 13 + (Corection - 1) * Size * 8;
        VShift[j] = Size * 2 + (Reset - 1) * Size * 5;
        core.host:execute("drawLabel1", id, HShift[j], core.CR_LEFT, Shift + VShift[j], core.CR_TOP, core.H_Left, core.V_Center, Bold, LabelColor, Pair[j]);
        id = id + 1;
        for i = 1, Num, 1 do
            core.host:execute("drawLabel1", id, HShift[j] + Size * 2, core.CR_LEFT, Shift + Size * i + VShift[j], core.CR_TOP, core.H_Left, core.V_Center, Bold, LabelColor, TF[i]);
            id = id + 1;
        end
    end
    for j = 1, Count, 1 do
    ...
    for i = 1, Count, 1  do
        ...
        if Num ~= 0 then
            core.host:execute("drawLabel1", id, HShift[i], core.CR_LEFT, Shift + Size * 2 + VShift[i], core.CR_TOP, core.H_Left, core.V_Center, Bold, C, L);
            id = id + 1;
        end
    end
end
...
function Draw(p, i, j)
    if Indicator[j][i].DATA:hasData(Indicator[j][i].DATA:size() - 1 - p) and SourceData[j][i].close:hasData(SourceData[j][i].close:size() - 1 - p) then
        ...
        if Style1 ~= nil then
            core.host:execute("drawLabel1", id, HShift[j] + Size * 3, core.CR_LEFT, Shift + Size * i + VShift[j], core.CR_TOP, core.H_Left, core.V_Center, Wingdings, Color1, Style1);
            id = id + 1;
        end
    end
end
    ...
    for j = 1, Count, 1 do
        for i = 1, Num, 1 do
            ...
            elseif cookie == (2000 + ID) then
                loading[j][i] = false;
                instance:updateFrom(source:size() - 1 - first);
            end
        end
    end
