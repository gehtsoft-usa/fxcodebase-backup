...
-- http://fxcodebase.com/code/viewtopic.php?f=17&t=66879
	 ...
	 if stage~= 2 then
	  return;
        ...
        if not init then
		   ...
		   context:createPen (11, context:convertPenStyle (instance.parameters.style1), context:pointsToPixels (instance.parameters.width1), instance.parameters.color1)
		   context:createPen (12, context:convertPenStyle (instance.parameters.style2), context:pointsToPixels (instance.parameters.width2), instance.parameters.color2)
		   context:createPen (13, context:convertPenStyle (instance.parameters.style3), context:pointsToPixels (instance.parameters.width3), instance.parameters.color3)
		...
		for period= math.max(context:firstBar (),source:first()), math.min( context:lastBar (), source:size()-1), 1 do
		 ...
		 for LevelId = 1 , Number ,  1 do
		 
		       if ON[LevelId] then
			   visible, y =  context:pointOfPrice (Level[LevelId]);
			   context:drawLine (10+LevelId, context:left (), y, context:right (), y);
			   end
			   
			   if Alert[LevelId]:hasData(period) then
				 
				if Alert[LevelId][period]== 1 then
				visible, y = context:pointOfPrice (AlertLevel[LevelId][period]);
				
				  width, height = context:measureText (1,  "\225", 0);
				  context:drawText (1,   "\225", UpTrendColor, -1,  x-width/2 ,  y , x+width/2 , y+height, 0 );	
	   
				elseif Alert[LevelId][period]== -1 then
				visible, y = context:pointOfPrice (AlertLevel[LevelId][period]);
...
function Activate (id, period)
   ...
   Alert[id][period]=0;
	  ...
	  if  ON[id]  then
			...
			if  source[period] > Level[id] 
			and   source[period-1] <= Level[id] 
			then
			           
						    
         
                Alert[id][period]= 1;	
 				AlertLevel[id][period]= Level[id] 
