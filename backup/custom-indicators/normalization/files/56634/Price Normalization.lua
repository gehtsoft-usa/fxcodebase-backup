...
function Prepare(nameOnly)
    ...
    local name = profile:id() .. "(" .. source:name()  .. ", " .. tostring(Price) .. ", " .. tostring(CP) .. ", " .. tostring(Period) .. ")";
    instance:name(name);
	...
	SourceData = core.host:execute("getSyncHistory", CP,source:barSize(), source:isBid(), Period, 100, 101);
	loading = true;
...
function Update(period, mode)
	 ...
	 local p2 =  Initialization(period-Period+1) ;
	    ...
	    if not p1 or not p2 or  period < first	 
		then
		return;
		end
		...
		local min1, max1;
		local min2, max2;
		min1,max1=mathex.minmax(source[Price],period-Period+1, period);
		min2,max2=mathex.minmax(SourceData[Price],p2, p1);
		Normalization[period] = ((SourceData[Price][p1] - min2)/ (max2-min2))*(max1-min1) +min1
