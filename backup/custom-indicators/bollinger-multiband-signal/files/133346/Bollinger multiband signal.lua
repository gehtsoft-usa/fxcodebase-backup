...
--http://fxcodebase.com/code/viewtopic.php?f=17&t=69762
...
{ 
    {
        ...
        UpCondition = function (period)
            for i, line in ipairs(lines) do
                if core.crossesOver(source.close, line.stream, period) then
                    return true, line.val;
                end
            end
            return false;
        end,
        DownCondition = function (period)
            for i, line in ipairs(lines) do
                if core.crossesUnder(source.close, line.stream, period) then
                    return true, line.val;
                end
            end
            return false;
...
function Init()
    indicator:name("Bollinger multiband signal");
    ...
    indicator.parameters:addDouble("step", "Step", "", 0.125);
    indicator.parameters:addInteger("count", "Count", "", 24);
    indicator.parameters:addColor("from_color", "From color", "", core.colors().Red);
    indicator.parameters:addColor("to_color", "To color", "", core.colors().Green);
...
function GetColor(from, to, part)
    local r_from = from % 256;
    local g_from = (from / 256) % 256;
    local b_from = (from / 256 / 256) % 256;
    local r_to = to % 256;
    local g_to = (to / 256) % 256;
    local b_to = (to / 256 / 256) % 256;
    return core.rgb(r_from + (r_to - r_from) * part, g_from + (g_to - g_from) * part, b_from + (b_to - b_from) * part);
end
...
function Prepare(nameOnly)
    ...
    local level = 0;
    from_color = instance.parameters.from_color;
    to_color = instance.parameters.to_color;
    count = instance.parameters.count;
    for i = 1, count do
        level = level + instance.parameters.step;
        local line = {};
        line.val = level;
        line.stream = instance:addStream("lvl_up_" .. i, core.Line, "+" .. line.val, "+" .. line.val, GetColor(from_color, to_color, i / count), 0, 0);
        lines[#lines + 1] = line;
        local line = {};
        line.val = -level;
        line.stream = instance:addStream("lvl_dn_" .. i, core.Line, "-" .. line.val, "-" .. line.val, GetColor(from_color, to_color, i / count), 0, 0);
        lines[#lines + 1] = line;
    end
    bb = core.indicators:create("BB", source, instance.parameters.bb_period, 1);
    indi_alerts:Prepare();
...
function Update(period, mode)
    bb:update(mode);
    if not bb.DATA:hasData(period) then
        return;
    end
    for i, line in ipairs(lines) do
        line.stream[period] = bb.AL[period] + (bb.TL[period] - bb.AL[period]) * line.val;
    end
...
function Activate(alert, period, historical_period)
    ...
    if isUp then
        if alert.OnChange then
            ...
            if isUpPrev and upMetadata ~= _ then
                return;
            end
        end
    ...
    if isDown then
        if alert.OnChange then
            ...
            if isDownPrev and downMetadata ~= _ then
                return;
            end
        end
