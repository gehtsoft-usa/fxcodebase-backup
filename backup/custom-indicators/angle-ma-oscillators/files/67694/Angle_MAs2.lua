...
function Init()
    ...
    indicator.parameters:addInteger("SmoothPeriod", "Smooth period", "", 4);
...
function Prepare(nameOnly)
    ...
    local name = profile:id() .. "(" .. source:name() .. ", " .. instance.parameters.Method .. ", " .. instance.parameters.Period .. ", " .. instance.parameters.SmoothPeriod .. ", " .. instance.parameters.Coeff .. ", " .. instance.parameters.VisualFactor .. ")";
    instance:name(name);
    if nameOnly then
        return;
    end
    A_MA=instance:addInternalStream(0, 0);
	
	assert(core.indicators:findIndicator("AVERAGES") ~= nil, "Please, download and install AVERAGES.LUA indicator");    
    MA = core.indicators:create("AVERAGES", source, Method, Period, false);
	
	first = MA.DATA:first();
    EMA = core.indicators:create("EMA", A_MA, SmoothPeriod);
    AngleMA = instance:addStream("AngleMA", core.Line, name .. ".AngleMA", "AngleMA", instance.parameters.clr, first+SmoothPeriod);
...
function Update(period, mode)
   if period>first then
    ...
    A_MA[period]=math.atan(tang)/math.pi*180*VisualFactor;
    EMA:update(mode);
	if  period > first+SmoothPeriod then
    AngleMA[period]=EMA.DATA[period];
	end
   end 
end
