...
function FindLast(period)
		...
		for i = period-Period+1, source:size()-1, 1 do 
		    ...
		    if ZigZag.DATA:hasData(i) then
                    ...
                    if ReturnHigh==0 then
							...
							if   source.high[i]== ZigZag.DATA[i] then
					...
					if  ReturnLow==0 then		
							if    source.low[i]== ZigZag.DATA[i] then
							 ReturnLow= IsItDown(i);
...
local Return =x;
	...
	for i = x, source:size()-1,1 do
	
		 if source.high[i]== ZigZag.DATA[i]
		and Return==0
		then
		Return=i;
		elseif source.high[i]>source.high[Return] then
		Return=i;
		end	
	end
...
local Return = 0;
	...
	for i = x, source:size()-1,1 do
		...
		if source.low[i]== ZigZag.DATA[i]
		and Return==0
		then
		Return=i;
		elseif source.low[i]<source.low[Return] then
		Return=i;
		end	
