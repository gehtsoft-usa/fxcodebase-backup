...
--+------------------------------------------------------------------+
function Init()
    indicator:name("Stochastic RSI MT1");
	...
	indicator.parameters:addInteger("D", "%D Slowing Periods", "", 9, 1, 200);
	...
	indicator.parameters:addDouble("central","Central Level","", 50);
...
function Prepare(nameOnly)
    ...
    RSI = core.indicators:create("RSI", source, N);
	 ...
	 firstD  = RSI.DATA:first()+K+D; 
   
    SK = instance:addStream("K", core.Line, name .. ".K", "K", instance.parameters.K_color, first);
...
function Update(period, mode)
	...
	return;
        ...
        if (min == max) then
            ...
            SK[period] = ((RSI.DATA[period] - min) / (max - min)) * 100;
        end  
	...
	return;
        ...
        SD[period] = mathex.avg ( SK, period - D +1 , period);
