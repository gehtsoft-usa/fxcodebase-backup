...
function Prepare(nameOnly)
    ...
    draw_line = instance.parameters.draw_line;
    ...
    if Lines then
        DPO = instance:addStream("DPO", core.Line, name .. ".DPO", "DPO", instance.parameters.clrDPO, 0 )
...
function IsSignalReset(period)
    if period == nil then 
        return true;
    end
    return period < DPO:getBookmark(1);
end
...
function Activate(alert, period, historical_period)
    ...
    if alert.id == 1 then
        ...
        if DPO[period] > buy_level and (IsSignalReset(last_period) or last_signal ~= 1 or (last_signal ~= nil and DPO[period] > last_level)) then
            alert:UpAlert(source, period, alert.Label .. ". Crossing over", DPO[period], historical_period);
        end
        if DPO[period] < sell_level and (IsSignalReset(last_period) or last_signal ~= -1 or (last_signal ~= nil and DPO[period] < last_level)) then
            alert:DownAlert(source, period, alert.Label .. ". Crossing under", DPO[period], historical_period);
        end
        if core.crosses(DPO, 0, period) then
            DPO:setBookmark(1, period);
        end
    end
...
function indi_alerts:DrawAlert(context, alert, period)
    ...
    elseif alert.Alert[period] == -1 then
        ...
        if alert.DrawingMode == "arrows" then
            visible, y = context:pointOfPrice(alert.AlertLevel[period]);
            core.host:trace(alert.AlertLevel[period]);
            core.host:trace(y);
...
function indi_alerts:Prepare()
    ...
    alert.Alert:setPrecision(math.max(2, instance.source:getPrecision()));
        ...
        function alert:DownAlert(source, period, text, level, historical_period)
            ...
            local zero_cross = DPO:getBookmark(1);
            if last_signal == -1 and last_period > zero_cross then
