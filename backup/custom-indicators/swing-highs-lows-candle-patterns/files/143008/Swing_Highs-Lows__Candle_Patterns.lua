...
--https://fxcodebase.com/code/viewtopic.php?f=17&t=71378
...
local hammer, ihammer, bulleng, hanging, shooting, beareng;
local indi_alerts = {};
...
{ 
    {
        ...
        FormatMessage = function(source, period, level, label, isUp, metadata)
            return string.format(
                ...
                "Price: %s\013\010" .. 
                "Date: %s", 
                isUp and (label .. " Hammer") or (label .. " Inverse Hammer"), 
                source:instrument(), 
                source:barSize(), 
    ...
    },
    {
        ...
        FormatMessage = function(source, period, level, label, isUp, metadata)
            return string.format(
                ...
                "Price: %s\013\010" .. 
                "Date: %s", 
                isUp and (label .. " Bullish Engulfing") or (label .. " Bearish Engulfing"), 
                source:instrument(), 
                source:barSize(), 
                win32.formatNumber(source:isBar() and source.close[NOW] or source[NOW], false, source:getPrecision()), 
                core.formatDate(core.now()));
    ...
    },
    {
        ...
        FormatMessage = function(source, period, level, label, isUp, metadata)
            return string.format(
                ...
                "Price: %s\013\010" .. 
                "Date: %s", 
                isUp and (label .. " Hanging Man") or (label .. " Shooting Star"), 
                source:instrument(), 
                source:barSize(), 
                win32.formatNumber(source:isBar() and source.close[NOW] or source[NOW], false, source:getPrecision()), 
                core.formatDate(core.now()));
...
function Init()
    indicator:name("Swing Highs/Lows & Candle Patterns");
    indicator:description("Swing Highs/Lows & Candle Patterns");
...
function pivothigh(period, _stream, leftbars, rightbars)
    if period - rightbars - leftbars - 1 < 0 or not _stream:hasData(period - rightbars) then
        return nil;
    end
    local ref = _stream:tick(period - rightbars);
    for i = period - rightbars - 1 - leftbars, period - rightbars - 1 do
        if _stream:hasData(i) or _stream:tick(period - rightbars) >= ref then
            return true;
        end
    end
    for i = period - leftbars, period do
        if _stream:hasData(i) or _stream:tick(period - rightbars) >= ref then
            return true;
        end
    end
...
function pivotlow(period, _stream, leftbars, rightbars)
    if period - rightbars - leftbars - 1 < 0 or not _stream:hasData(period - rightbars) then
        return nil;
    end
    local ref = _stream:tick(period - rightbars);
...
function CreateValueWhen()
    local value_when =
    {
        _stream = instance:addInternalStream(0, 0),
        _count = 0
    };
    function value_when:set(period, condition, value, occurrence)
...
function Update(period, mode)
    ...
    o[period] = source.open[period - length];
    h = source.high[period - length];
    l = source.low[period - length];
    c[period] = source.close[period - length];
    if period < length * 2 + 1 then
        return;
    end
    ph = pivothigh(period, source.close, length, length);
    pl = pivotlow(period, source.open, length, length);
    valH = vars["__valuewhen1"]:set(period, ph ~= nil, c[period], 0);
    valL = vars["__valuewhen2"]:set(period, pl ~= nil, c[period], 0);
    valpH = vars["__valuewhen3"]:set(period, ph ~= nil, c[period], 1);
    valpL = vars["__valuewhen4"]:set(period, pl ~= nil, c[period], 1);
    d = math.abs(c[period] - o[period]);
    hammer = (pl and ((math.min(o[period], c[period]) - l > d) and (h - math.max(c[period], o[period]) < d)));
    ihammer = (pl and ((h - math.max(c[period], o[period]) > d) and (math.min(c[period], o[period]) - l < d)));
    bulleng = ((c[period] > o[period]) and ((c[period - 1] < o[period - 1]) and ((c[period] > o[period - 1]) and (o[period] < c[period - 1]))));
    hanging = (ph and ((math.min(c[period], o[period]) - l > d) and (h - math.max(o[period], c[period]) < d)));
    shooting = (ph and ((h - math.max(o[period], c[period]) > d) and (math.min(c[period], o[period]) - l < d)));
    beareng = ((c[period] > o[period]) and ((c[period - 1] < o[period - 1]) and ((c[period] > o[period - 1]) and (o[period] < c[period - 1]))));
