...
-- http://fxcodebase.com/code/viewtopic.php?f=17&t=7635&p=115447#p115447
...
--+------------------------------------------------------------------+
 ...
 function Add(id, TF,Flag, Instrument )
    ...
    indicator.parameters:addInteger("Period1" .. id, "ADX Period", "", 14);	
    indicator.parameters:addDouble("ADX_Alpha1" .. id, "ADX Alpha", "", 0.25);
    indicator.parameters:addDouble("Level" .. id, "ADX Level", "", 20);
	
	 indicator.parameters:addDouble("ADX_Alpha2" .. id, "DMI Alpha", "", 0.33);
	
	indicator.parameters:addGroup("DMI Calculation");
	indicator.parameters:addInteger("Period2" .. id, "DMI Period", "", 14);
	indicator.parameters:addDouble("DMI_Alpha1" .. id, "ADX Alpha", "", 0.25);
    indicator.parameters:addDouble("DMI_Alpha2" .. id, "DMI Alpha", "", 0.33);
...
function Init()
    indicator:name("MTF MCP  Custom Smoothed ADX Heat Map");
	...
	indicator.parameters:addColor("Strong", "Strong Trend Color","", core.rgb(0, 0, 255));
...
local Indicator = {} ;
 ...
 function Prepare(nameOnly) 
	...
	assert(core.indicators:findIndicator("CUSTOM SMOOTHED_ADX") ~= nil, "Please, download and install CUSTOM SMOOTHED_ADX.LUA indicator");    
	...
	s2, e2 = core.getcandle(iTF[i], 0, 0, 0);
	 ...
	 Number=Number+1;
		...
		Period1[Number]= instance.parameters:getDouble("Period1" .. i);
		ADX_Alpha1[Number]= instance.parameters:getDouble("ADX_Alpha1" .. i);
		ADX_Alpha2[Number]= instance.parameters:getDouble("ADX_Alpha2" .. i);
		Period2[Number]= instance.parameters:getDouble("Period2" .. i);
		DMI_Alpha1[Number]= instance.parameters:getDouble("DMI_Alpha1" .. i);
		DMI_Alpha2[Number]= instance.parameters:getDouble("DMI_Alpha2" .. i);
	 ...
	 Label[Number]="";
                ...
                Label[Number]=Label[Number] .. " - " ..  TF[Number];				
			  	...
			  	Temp1= core.indicators:create("CUSTOM SMOOTHED_ADX", source ,  Period1[Number],ADX_Alpha1[Number],ADX_Alpha2[Number],Period2[Number],DMI_Alpha1[Number],DMI_Alpha2[Number] );  				
				ifirst= Temp1.DATA:first()*2;
				 ...
				 loading[Number]  = true;  	 
				 Indicator[Number] = core.indicators:create("CUSTOM SMOOTHED_ADX", SourceData[Number]  ,  Period1[Number],ADX_Alpha1[Number],ADX_Alpha2[Number],Period2[Number],DMI_Alpha1[Number],DMI_Alpha2[Number]);   
...
function Draw (stage, context)
   ...
   context:setClipRectangle(context:left(), context:top(), context:right(), context:bottom());
        ...
        if not init then
			...
			context:createPen (31, context.SOLID, 3, Strong)       
			context:createSolidBrush(32, Strong);
			   ...
			   x0, x1, x2 = context:positionOfBar (i);
			    ...
			    for j= 1, Number , 1 do
				  ...
				  if p~= false then
										...
										if Indicator[j].DIP:hasData(p) and Indicator[j].DIP:hasData(p-1) and Indicator[j].ADX:hasData(p) then 
												    ...
												    if Indicator[j].ADX[p] > Level[j] then	
											        C1=31;
											        end
