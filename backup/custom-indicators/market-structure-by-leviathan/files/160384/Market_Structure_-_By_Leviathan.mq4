//Available @  https://fxcodebase.com/code/viewtopic.php?f=17&p=154263#p154263
...
#endif
input int param1 = 20; // Swing Length
input string param2 = "Candle Close"; // BOS Confirmation
input bool param3 = false; // Show CHoCH
input bool param4 = true; // Show Swing Points
input bool param5 = false; // Show 0.5 Retracement Level
input color param6 = ColorRGB(41, 39, 176, 0); // Color
input string param7 = "Solid"; // Line Style
input int param8 = 1; // Width
input color param9 = ColorRGB(112, 114, 119, 0); // Color
input string param10 = "Dashed"; // Line Style
input int param11 = 1; // Width
...
double prevHigh[];
double prevHigh_DEFAULT_VALUE;
double prevLow[];
double prevLow_DEFAULT_VALUE;
double prevHighIndex[];
double prevHighIndex_DEFAULT_VALUE;
double prevLowIndex[];
double prevLowIndex_DEFAULT_VALUE;
double highActive[];
double highActive_DEFAULT_VALUE;
double lowActive[];
double lowActive_DEFAULT_VALUE;
double prevSwing[];
double prevSwing_DEFAULT_VALUE;
double prevBreakoutDir[];
double prevBreakoutDir_DEFAULT_VALUE;
class lineStyle_sStream
{
   string x;
   bool _initialized;
   string IndicatorObjPrefix;
public:
   lineStyle_sStream(string x, string indicatorObjPrefix)
   {
      _initialized = false;
   ...
   bool GetValue(const int pos, string &__out1)
   {
      ...
      {
         switch1Value1 = "dashed";
      }
      else if ((x == "Dotted"))
      {
         switch1Value1 = "dotted";
      }
      __out1 = switch1Value1;
      return true;
   }
};
lineStyle_sStream* lineStyle_s1;
lineStyle_sStream* lineStyle_s2;
lineStyle_sStream* lineStyle_s3;
lineStyle_sStream* lineStyle_s4;
...
int init()
{
   ...
   IndicatorShortName("Market Structure - By Leviathan");
   SetIndexBuffer(id++, prevHigh);
   SetIndexBuffer(id++, prevLow);
   SetIndexBuffer(id++, prevHighIndex);
   SetIndexBuffer(id++, prevLowIndex);
   SetIndexBuffer(id++, highActive);
   SetIndexBuffer(id++, lowActive);
   SetIndexBuffer(id++, prevSwing);
   SetIndexBuffer(id++, prevBreakoutDir);
   lineStyle_s1 = new lineStyle_sStream(halfStyle, IndicatorObjPrefix + "_1");
   id = lineStyle_s1.Init(id);
   lineStyle_s2 = new lineStyle_sStream(halfStyle, IndicatorObjPrefix + "_2");
   id = lineStyle_s2.Init(id);
   lineStyle_s3 = new lineStyle_sStream(bosStyle, IndicatorObjPrefix + "_3");
   id = lineStyle_s3.Init(id);
   lineStyle_s4 = new lineStyle_sStream(bosStyle, IndicatorObjPrefix + "_4");
   id = lineStyle_s4.Init(id);
                ...
                const int &spread[])
{
   if (prev_calculated <= 0 || prev_calculated > rates_total)
   {
      ...
      prevHigh_DEFAULT_VALUE = EMPTY_VALUE;
      ArrayInitialize(prevHigh, prevHigh_DEFAULT_VALUE);
      prevLow_DEFAULT_VALUE = EMPTY_VALUE;
      ArrayInitialize(prevLow, prevLow_DEFAULT_VALUE);
      prevHighIndex_DEFAULT_VALUE = INT_MIN;
      ArrayInitialize(prevHighIndex, prevHighIndex_DEFAULT_VALUE);
      prevLowIndex_DEFAULT_VALUE = INT_MIN;
      ArrayInitialize(prevLowIndex, prevLowIndex_DEFAULT_VALUE);
      highActive_DEFAULT_VALUE = false;
      ArrayInitialize(highActive, highActive_DEFAULT_VALUE);
      lowActive_DEFAULT_VALUE = false;
      ArrayInitialize(lowActive, lowActive_DEFAULT_VALUE);
      prevSwing_DEFAULT_VALUE = 0;
      ArrayInitialize(prevSwing, prevSwing_DEFAULT_VALUE);
      prevBreakoutDir_DEFAULT_VALUE = 0;
      ArrayInitialize(prevBreakoutDir, prevBreakoutDir_DEFAULT_VALUE);
      lineStyle_s1.Clear();
      lineStyle_s2.Clear();
      lineStyle_s3.Clear();
      lineStyle_s4.Clear();
   ...
   {
      prevHigh[pos] = pos < (rates_total - 1) ? prevHigh[pos + 1] : EMPTY_VALUE;
      prevLow[pos] = pos < (rates_total - 1) ? prevLow[pos + 1] : EMPTY_VALUE;
      prevHighIndex[pos] = pos < (rates_total - 1) ? prevHighIndex[pos + 1] : INT_MIN;
      prevLowIndex[pos] = pos < (rates_total - 1) ? prevLowIndex[pos + 1] : INT_MIN;
      highActive[pos] = pos < (rates_total - 1) ? highActive[pos + 1] : false;
      lowActive[pos] = pos < (rates_total - 1) ? lowActive[pos + 1] : false;
      prevSwing[pos] = pos < (rates_total - 1) ? prevSwing[pos + 1] : 0;
      prevBreakoutDir[pos] = pos < (rates_total - 1) ? prevBreakoutDir[pos + 1] : 0;
      uint CLEAR = ColorRGB(0, 0, 0, 100);
      highestpivot1Source.SetValue(pos, high[pos]);
      double highestpivot1Value;
      if (!PivotHighStream::GetValue(pos, highestpivot1Value, highestpivot1Source, swingSize, swingSize)) { highestpivot1Value = EMPTY_VALUE; }
      double pivHi = highestpivot1Value;
      lowestpivot1Source.SetValue(pos, low[pos]);
      double lowestpivot1Value;
      if (!PivotLowStream::GetValue(pos, lowestpivot1Value, lowestpivot1Source, swingSize, swingSize)) { lowestpivot1Value = EMPTY_VALUE; }
      double pivLo = lowestpivot1Value;
      int hh = false;
      int lh = false;
      int hl = false;
      int ll = false;
      if (!((pivHi) == EMPTY_VALUE))
      {
         if (SafeGE(pivHi, prevHigh[pos]))
         {
            hh = true;
            SetStream(prevSwing, pos, 2, prevSwing_DEFAULT_VALUE);
         }
         else
         {
            lh = true;
            SetStream(prevSwing, pos, 1, prevSwing_DEFAULT_VALUE);
         }
         SetStream(prevHigh, pos, pivHi, prevHigh_DEFAULT_VALUE);
         SetStream(highActive, pos, true, highActive_DEFAULT_VALUE);
         SetStream(prevHighIndex, pos, ((rates_total - 1) - pos) - swingSize, prevHighIndex_DEFAULT_VALUE);
      }
      if (!((pivLo) == EMPTY_VALUE))
      {
         if (SafeGE(pivLo, prevLow[pos]))
         {
            hl = true;
            SetStream(prevSwing, pos, (-1), prevSwing_DEFAULT_VALUE);
         }
         ...
         SetStream(prevLow, pos, pivLo, prevLow_DEFAULT_VALUE);
         SetStream(lowActive, pos, true, lowActive_DEFAULT_VALUE);
         SetStream(prevLowIndex, pos, ((rates_total - 1) - pos) - swingSize, prevLowIndex_DEFAULT_VALUE);
      }
      int highBroken = false;
      int lowBroken = false;
      double highSrc = ((bosConfType == "Candle Close") ? close[pos] : high[pos]);
      double lowSrc = ((bosConfType == "Candle Close") ? close[pos] : low[pos]);
      if (SafeGreater(highSrc, prevHigh[pos]) && highActive[pos])
      {
         highBroken = true;
         SetStream(highActive, pos, false, highActive_DEFAULT_VALUE);
      }
      if (SafeLess(lowSrc, prevLow[pos]) && lowActive[pos])
      {
         lowBroken = true;
         SetStream(lowActive, pos, false, lowActive_DEFAULT_VALUE);
      }
      if (hh && showSwing)
      {
         LabelsCollection::Create(IndicatorObjPrefix + "label_1_id", ((rates_total - 1) - pos) - swingSize, pivHi, time[pos]).SetColor(CLEAR).SetText("HH").SetTextColor(ChartGetInteger(0, CHART_COLOR_FOREGROUND)).SetStyle("down").SetSize("normal").SetYLoc("price").SetTextAlign("center");
         if (pos + 1 > (rates_total - 1)) { continue; }
         if ((prevSwing[pos + 1] == (-1)) && showHalf)
         {
            string lineStyle_s1Value;
            if (!lineStyle_s1.GetValue(pos, lineStyle_s1Value)) { lineStyle_s1Value = NULL; }
            LinesCollection::Create(IndicatorObjPrefix + "line_1_id", prevLowIndex[pos], SafeDivide((SafePlus(prevLow[pos], pivHi)), 2), ((rates_total - 1) - pos) - swingSize, SafeDivide((SafePlus(prevLow[pos], pivHi)), 2), time[pos]).SetColor(halfColor).SetWidth(halfWidth).SetStyle(lineStyle_s1Value).SetExtend("none");
         }
      }
      if (lh && showSwing)
      {
         LabelsCollection::Create(IndicatorObjPrefix + "label_2_id", ((rates_total - 1) - pos) - swingSize, pivHi, time[pos]).SetColor(CLEAR).SetText("LH").SetTextColor(ChartGetInteger(0, CHART_COLOR_FOREGROUND)).SetStyle("down").SetSize("normal").SetYLoc("price").SetTextAlign("center");
      }
      if (hl && showSwing)
      {
         LabelsCollection::Create(IndicatorObjPrefix + "label_3_id", ((rates_total - 1) - pos) - swingSize, pivLo, time[pos]).SetColor(CLEAR).SetText("HL").SetTextColor(ChartGetInteger(0, CHART_COLOR_FOREGROUND)).SetStyle("up").SetSize("normal").SetYLoc("price").SetTextAlign("center");
      }
      if (ll && showSwing)
      {
         LabelsCollection::Create(IndicatorObjPrefix + "label_4_id", ((rates_total - 1) - pos) - swingSize, pivLo, time[pos]).SetColor(CLEAR).SetText("LL").SetTextColor(ChartGetInteger(0, CHART_COLOR_FOREGROUND)).SetStyle("up").SetSize("normal").SetYLoc("price").SetTextAlign("center");
         if (pos + 1 > (rates_total - 1)) { continue; }
         if ((prevSwing[pos + 1] == 1) && showHalf)
         {
            string lineStyle_s2Value;
            if (!lineStyle_s2.GetValue(pos, lineStyle_s2Value)) { lineStyle_s2Value = NULL; }
            LinesCollection::Create(IndicatorObjPrefix + "line_2_id", prevHighIndex[pos], SafeDivide((SafePlus(prevHigh[pos], pivLo)), 2), ((rates_total - 1) - pos) - swingSize, SafeDivide((SafePlus(prevHigh[pos], pivLo)), 2), time[pos]).SetColor(halfColor).SetWidth(halfWidth).SetStyle(lineStyle_s2Value).SetExtend("none");
         }
      }
      if (highBroken)
      {
         string lineStyle_s3Value;
         if (!lineStyle_s3.GetValue(pos, lineStyle_s3Value)) { lineStyle_s3Value = NULL; }
         LinesCollection::Create(IndicatorObjPrefix + "line_3_id", prevHighIndex[pos], prevHigh[pos], ((rates_total - 1) - pos), prevHigh[pos], time[pos]).SetColor(bosColor).SetWidth(bosWidth).SetStyle(lineStyle_s3Value).SetExtend("none");
         LabelsCollection::Create(IndicatorObjPrefix + "label_5_id", SafeMathFloor(SafeMinus(((rates_total - 1) - pos), SafeDivide((SafeMinus(((rates_total - 1) - pos), prevHighIndex[pos])), 2))), prevHigh[pos], time[pos]).SetColor(CLEAR).SetText(((prevBreakoutDir[pos] == (-1)) && choch ? "CHoCH" : "BOS")).SetTextColor(bosColor).SetStyle("down").SetSize("tiny").SetYLoc("price").SetTextAlign("center");
         SetStream(prevBreakoutDir, pos, 1, prevBreakoutDir_DEFAULT_VALUE);
      }
      if (lowBroken)
      {
         string lineStyle_s4Value;
         if (!lineStyle_s4.GetValue(pos, lineStyle_s4Value)) { lineStyle_s4Value = NULL; }
         LinesCollection::Create(IndicatorObjPrefix + "line_4_id", prevLowIndex[pos], prevLow[pos], ((rates_total - 1) - pos), prevLow[pos], time[pos]).SetColor(bosColor).SetWidth(bosWidth).SetStyle(lineStyle_s4Value).SetExtend("none");
         LabelsCollection::Create(IndicatorObjPrefix + "label_6_id", SafeMathFloor(SafeMinus(((rates_total - 1) - pos), SafeDivide((SafeMinus(((rates_total - 1) - pos), prevLowIndex[pos])), 2))), prevLow[pos], time[pos]).SetColor(CLEAR).SetText(((prevBreakoutDir[pos] == 1) && choch ? "CHoCH" : "BOS")).SetTextColor(bosColor).SetStyle("up").SetSize("tiny").SetYLoc("price").SetTextAlign("center");
         SetStream(prevBreakoutDir, pos, (-1), prevBreakoutDir_DEFAULT_VALUE);
      }
   }
