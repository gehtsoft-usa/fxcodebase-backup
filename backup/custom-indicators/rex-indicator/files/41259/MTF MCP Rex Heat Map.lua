...
--+------------------------------------------------------------------+
 ...
 function Add(id, TF,Flag, Instrument )
    ...
    indicator.parameters:addInteger("SP" .. id, "Smoothing Period", "Smoothing Period", 14);
    indicator.parameters:addInteger("SS" .. id, "Signal Period", "Signal Period", 14);
	indicator.parameters:addInteger("PS" .. id, "Post-Smoothing Period", "Smoothing Period", 1);
...
function Init()
    indicator:name("MTF MCP Rex Heat Map");
...
function Prepare(nameOnly)
	...
	s2, e2 = core.getcandle(iTF[i], 0, 0, 0);
	 ...
	 Number=Number+1;
		  ...
		  SP[Number]= instance.parameters:getInteger("SP" .. i);
          SS[Number]= instance.parameters:getInteger("SS" .. i);
          PS[Number]= instance.parameters:getInteger("PS" .. i);
	 ...
	 Label[Number]="";
                ...
                Label[Number]=Label[Number] .. " - " ..  TF[Number];				
			  	...
			  	Temp1= core.indicators:create("REX", source ,  SP[Number],   SS[Number]);  				
				ifirst= Temp1.DATA:first()*2;
				 ...
				 loading[Number]  = true;  	 
				 Indicator[Number] = core.indicators:create("REX", SourceData[Number] ,   SP[Number],   SS[Number]);   
			     MA[Number] = core.indicators:create("MVA", Indicator[Number].DATA ,   PS[Number]);   
			   ...
			   x0, x1, x2 = context:positionOfBar (i);
			    ...
			    for j= 1, Number , 1 do
				  ...
				  if p~= false then
										...
										if Indicator[j].DATA:hasData(p) and Indicator[j].DATA:hasData(p-1) and Indicator[j].SIGNAL:hasData(p-1) and MA[j].DATA:hasData(p)  then 
										
												    if MA[j].DATA[p] > Indicator[j].SIGNAL[p] then		 
