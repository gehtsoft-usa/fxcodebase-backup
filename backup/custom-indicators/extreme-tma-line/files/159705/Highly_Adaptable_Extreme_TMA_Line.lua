-- Available @ https://fxcodebase.com/code/viewtopic.php?f=17&p=159663#p159663 
...
{ 
    {
        ...
        UpCondition = function (period)
            if instance.parameters.Action1 == "NO" or period < 2 or not indi.TMA:hasData(period - 1) then
                return false;
            end
            return core.crossesOver(source.close, indi.TMA, period)
        end,
        DownCondition = function (period)
            if instance.parameters.Action1 == "NO" or period < 2 or not indi.TMA:hasData(period - 1) then
                return false;
            end
            return core.crossesUnder(source.close, indi.TMA, period)
        end,
    ...
    },
    {
        Stage = 2,
        UpCondition = function (period)
            if instance.parameters.Action2 == "NO" or period < 2 or not indi.Upper:hasData(period - 1) then
                return false;
            end
            return core.crossesOver(source.close, indi.Upper, period)
        end,
        DownCondition = function (period)
            if instance.parameters.Action2 == "NO" or period < 2 or not indi.Upper:hasData(period - 1) then
                return false;
            end
            return core.crossesUnder(source.close, indi.Upper, period)
        end,
        FormatMessage = function(source, period, level, label, isUp, metadata)
            return string.format(
                "Label: %s\013\010" ..
        ...
        FilterConsecutive = false,
        OnChange = true,
        Name = "Cross Upper TMA"
    },
    {
        Stage = 2,
        UpCondition = function (period)
            if instance.parameters.Action3 == "NO" or period < 2 or not indi.Lower:hasData(period - 1) then
                return false;
            end
            return core.crossesOver(source.close, indi.Lower, period)
        end,
        DownCondition = function (period)
            if instance.parameters.Action3 == "NO" or period < 2 or not indi.Lower:hasData(period - 1) then
                return false;
            end
            return core.crossesUnder(source.close, indi.Lower, period)
        end,
        FormatMessage = function(source, period, level, label, isUp, metadata)
            return string.format(
                "Label: %s\013\010" ..
        ...
        FilterConsecutive = false,
        OnChange = true,
        Name = "Cross Lower TMA"
    }
};

function Init()
    indicator:name("Highly Adaptable Extreme TMA Line");
    indicator:description("");
    indicator:requiredSource(core.Bar);
    indicator:type(core.Indicator);
    
    AddAction(1, "Cross TMA");
    AddAction(2, "Cross Upper TMA");
    AddAction(3, "Cross Lower TMA");
...
function AddAction(id, name)
    indicator.parameters:addString("Action" .. id, name, "", "Signal")
    indicator.parameters:addStringAlternative("Action" .. id, "No Action", "", "NO")
    indicator.parameters:addStringAlternative("Action" .. id, "Signal", "", "Signal")
end
...
function Prepare(nameOnly)
    ...
    indi = core.indicators:create("EXTREME_TMA_LINE", source, indicatorParams)
