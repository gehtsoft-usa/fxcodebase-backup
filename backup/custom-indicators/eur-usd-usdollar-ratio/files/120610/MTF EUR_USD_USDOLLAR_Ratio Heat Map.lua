...
function Init()
    indicator:name("MTF EUR_USD_USDOLLAR_Ratio Heat Map");
...
function Prepare(nameOnly) 
	...
	local ifirst;
	 ...
	 assert(core.indicators:findIndicator("EUR_USD_USDOLLAR_RATIO") ~= nil, "Please, download and install EUR_USD_USDOLLAR_RATIO.LUA indicator");    
	...
	s2, e2 = core.getcandle(iTF[i], 0, 0, 0);
	 ...
	 Number=Number+1;
	      ...
	      Period[Number]= instance.parameters:getInteger("Period" .. i);
		       ...
		       Temp1= core.indicators:create("EUR_USD_USDOLLAR_RATIO", source ,  Period[Number]);  				
				ifirst= Temp1.DATA:first()*2;
				
				   Id=Id+1;
			 
				 SourceData[Number]  = core.host:execute("getSyncHistory",  source:instrument(),  TF[Number], source:isBid(),math.min(300,ifirst), 2000 + Id , 1000 +Id);	 	 
				 loading[Number]  = true;  	 
				 Indicator[Number] = core.indicators:create("EUR_USD_USDOLLAR_RATIO", SourceData[Number] ,  Period[Number]);   
			   ...
			   x0, x1, x2 = context:positionOfBar (i);
			    ...
			    for j= 1, Number , 1 do
				  ...
				  if p~= false then
										...
										if Indicator[j].MA:hasData(p) and Indicator[j].MA:hasData(p-1) then 
										
												    if Indicator[j].EURUSDX[p] > Indicator[j].MA[p] then		 
															if Indicator[j].EURUSDX[p] > Indicator[j].EURUSDX[p-1] then	
															C2=12;
															C1=11;
															else
															C2=22;
