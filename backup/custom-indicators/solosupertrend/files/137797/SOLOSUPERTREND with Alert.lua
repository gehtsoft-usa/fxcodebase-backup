...
function Init()
	...
	indicator.parameters:addString("mode", "Mode", "", "factor");
	indicator.parameters:addStringAlternative("mode", "Factor", "", "factor");
	indicator.parameters:addStringAlternative("mode", "Percentage", "", "%");
	indicator.parameters:addStringAlternative("mode", "Pips", "", "pips");
	indicator.parameters:addDouble("Factor", "Factor", "", 1.0)
	indicator.parameters:addDouble("percentage", "Percentage", "", 100)
	indicator.parameters:addDouble("pips", "Pips", "", 100)
...
function Prepare(nameOnly)
	...
	calcmode = instance.parameters.mode;
...
function Update(period, mode)
	...
	if calcmode == "factor" then
		Up = source.close[period] - Factor * atr.DATA[period - 1]
		Dn = source.close[period] + Factor * atr.DATA[period - 1]
	elseif calcmode == "%" then
		Up = source.close[period] - percentage * atr.DATA[period - 1] / 100
		Dn = source.close[period] + percentage * atr.DATA[period - 1] / 100
	elseif calcmode == "pips" then
		Up = source.close[period] - pips * source:pipSize()
		Dn = source.close[period] + pips * source:pipSize()
	end
