...
function Init()
    indicator:name("AO Divergence Trend");
    indicator:description("Shows AO Divergence trends");
...
function Prepare(nameOnly)
	...
	assert(core.indicators:findIndicator("AO_DIVERGENCE") ~= nil, "Please, download and install AO_DIVERGENCE.LUA indicator"); 
    local name = profile:id() .. "(" .. source:name() .. ", " .. instance.parameters.AO_Short .. ", " .. instance.parameters.AO_Long .. ")";
    instance:name(name);
    if nameOnly then
        return;
    end
    AO = core.indicators:create("AO_DIVERGENCE", source, instance.parameters.AO_Short,instance.parameters.AO_Long, false);
    first = AO.DATA:first();
...
function Update(period, mode)
    ...
    if AO:getStream(1):hasData(period - 2) then
        l = math.abs(AO:getStream(1)[period - 2]);
    ...
    if AO:getStream(2):hasData(period - 2) then
        l = math.abs(AO:getStream(2)[period - 2]);
        local prev = period - 2 - l;
        line_id = line_id + 1;
