	...
	s2, e2 = core.getcandle(iTF[i], 0, 0, 0);
	 ...
	 Label[Number]="";
				...
				ifirst= Temp1.DATA:first();
				 ...
				 loading[Number]  = true;  	 
				 DMI[Number] = core.indicators:create("DMI", SourceData[Number] ,  Period);   
			     ADX[Number] = core.indicators:create("ADX", SourceData[Number] ,  ADXPeriod); 
			   ...
			   x0, x1, x2 = context:positionOfBar (i);
			    ...
			    for j= 1, Number , 1 do
				  ...
				  if p~= false then
										...
										if DMI[j].DIP:hasData(p) and DMI[j].DIP:hasData(p-1) then 
													...
													if ShowADX then 
													      if ADX[j].DATA[p] > Level then		 
																	if ADX[j].DATA[p] > ADX[j].DATA[p-1] then																
																	C1=11;
																	else
...
function Evaluate(j)	
	 ...
	 local   p=Initialization((source:size()-1),j);
		...
		if DMI[j].DIP[p] > DMI[j].DIM[p] then 
		Count[j]= 1;
		elseif DMI[j].DIP[p] < DMI[j].DIM[p] then  
		Count[j]= -1;
		end
