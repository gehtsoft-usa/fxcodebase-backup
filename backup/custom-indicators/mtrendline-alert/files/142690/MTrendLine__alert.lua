...
--https://fxcodebase.com/code/viewtopic.php?f=17&t=71321
...
{ 
    {
        ...
        UpCondition = function (period)
            if not lines[1].use or lines[1].rate == nil or source:date(period) < lines[1].date then
                return false;
            end
            return source.close[period] > lines[1].rate;
        end,
    ...
    },
    {
        Stage = 2,
        UpCondition = function (period)
            if not lines[2].use or lines[2].rate == nil or source:date(period) < lines[2].date then
                return false;
            end
            return source.close[period] > lines[2].rate;
        end,
        DownCondition = function (period)
            if not lines[2].use or lines[2].rate == nil or source:date(period) < lines[2].date then
                return false;
    ...
    },
    {
        Stage = 2,
        UpCondition = function (period)
            if not lines[3].use or lines[3].rate == nil or source:date(period) < lines[3].date then
                return false;
            end
            return source.close[period] > lines[3].rate;
        end,
        DownCondition = function (period)
            if not lines[3].use or lines[3].rate == nil or source:date(period) < lines[3].date then
                return false;
    ...
    },
    {
        Stage = 2,
        UpCondition = function (period)
            if not lines[4].use or lines[4].rate == nil or source:date(period) < lines[4].date then
                return false;
            end
            return source.close[period] > lines[4].rate;
        end,
        DownCondition = function (period)
            if not lines[4].use or lines[4].rate == nil or source:date(period) < lines[4].date then
                return false;
...
function Init()
    indicator:name("MTrendLine Alert");
    indicator:description("");
    indicator:requiredSource(core.Bar);
    indicator:type(core.Indicator);

    indicator.parameters:addBoolean("TrendLine1_Red", "Trend Line 1", "", true);
    indicator.parameters:addBoolean("TrendLine2_Blue", "Trend Line 2", "", true);
    indicator.parameters:addBoolean("TrendLine_3", "Trend Line 3", "", true);
    indicator.parameters:addBoolean("TrendLine_4", "Trend Line 4", "", true);
...
function Prepare(nameOnly)
    ...
    lines[1] = { use = instance.parameters.TrendLine1_Red };
    lines[2] = { use = instance.parameters.TrendLine2_Blue };
    lines[3] = { use = instance.parameters.TrendLine_3 };
    lines[4] = { use = instance.parameters.TrendLine_4 };
    indi_alerts:Prepare();
...
function Draw(stage, context) 
    ...
    if not init then
        lines[1].pen = line_1_pen;
        context:createPen(line_1_pen, context.SOLID, 1, core.colors().Red);
        lines[2].pen = line_2_pen;
        context:createPen(line_2_pen, context.SOLID, 1, core.colors().Green);
        lines[3].pen = line_3_pen;
        context:createPen(line_3_pen, context.SOLID, 1, core.colors().Magenta);
        lines[4].pen = line_4_pen;
        context:createPen(line_4_pen, context.SOLID, 1, core.colors().Magenta);
        init = true;
    end
    for i, line in ipairs(lines) do
        if line.use then
            ...
            local _, y = context:pointOfPrice(line.rate);
            context:drawLine(line.pen, x1, y, x2, y);
        end
    end
end

function Update(period, mode)
    if period == source:size() - 1 and line_1_rate == nil then
        lines[1].rate = core.host:execute("getBidPrice").close[NOW] + 25 * source:pipSize();
        lines[2].rate = core.host:execute("getBidPrice").close[NOW] - 25 * source:pipSize();
        lines[3].rate = core.host:execute("getBidPrice").close[NOW] + 50 * source:pipSize();
        lines[4].rate = core.host:execute("getBidPrice").close[NOW] - 50 * source:pipSize();
        lines[1].date = source:date(NOW - 12);
        lines[2].date = source:date(NOW - 12);
        lines[3].date = source:date(NOW - 12);
        lines[4].date = source:date(NOW - 12);
    end
