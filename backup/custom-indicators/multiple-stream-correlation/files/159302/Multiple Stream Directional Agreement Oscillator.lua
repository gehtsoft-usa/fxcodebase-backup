...
function Init()
    indicator:name("Multiple Stream Directional Agreement Oscillator");
    indicator:description("Percentage of candles in the same direction between Multiple instruments or Multiple price series.");
...
function Prepare(nameOnly)
	...
	local Last;
	for i = 1, Count, 1 do			
		if Instrument[i] == source:instrument() then
		...
		if Instrument[i] == BaseInstrument  then
		Last=i;
		end
	...
	corrStream[Last] = instance:addStream(Instrument[Last] , core.Line, Instrument[Last],  Instrument[Last], instance.parameters.Color1, first);
    corrStream[Last]:setWidth(5);
    corrStream[Last]:setStyle(instance.parameters.Style);
    corrStream[Last]:setPrecision(math.max(2, instance.source:getPrecision()));		
end
	...
	return;
		...
		local Number=0;
			...
			for i= 1, Count, 1 do
			    Number=0;
				for j= 1, Period, 1 do				
				if (source.close[period-j+1] > source.open[period-j+1] 
				and  Source[i].close[p[i]-j+1] > Source[i].open[p[i]-j+1] )
				or (source.close[period-j+1] < source.open[period-j+1] 
				and  Source[i].close[p[i]-j+1] < Source[i].open[p[i]-j+1] )
				then
				Number=Number+1;
				end
				
				corrStream[i][period]=(Number/Period)*100	
			end
