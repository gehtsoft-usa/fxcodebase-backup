...
local STRATEGY_NAME = "ZigZag with Fibonacci Strategy (Kelly123)";
...
function CreateParameters() 
    strategy.parameters:addInteger("loopback", "Loopback", "", 100);

    strategy.parameters:addGroup("Lower timeframe");
    strategy.parameters:addBoolean("low_use", "Use this timeframe", "", true);
	strategy.parameters:addInteger("low_k", "Number of periods for %K", "The number of periods for %K.", 10, 2, 1000);
    strategy.parameters:addInteger("low_sd", "%D slowing periods", "The number of periods for slow %D.", 5, 2, 1000);
    strategy.parameters:addInteger("low_d", "Number of periods for %D", "The number of periods for %D.", 5, 2, 1000);
    strategy.parameters:addInteger("low_os", "Oversold level", "", 15);
    strategy.parameters:addInteger("low_ob", "Overbought level", "", 85);
    strategy.parameters:addInteger("low_ema", "EMA period", "", 7);

    strategy.parameters:addGroup("Middle timeframe");
    strategy.parameters:addBoolean("mid_use", "Use this timeframe", "", true);
	strategy.parameters:addInteger("mid_k", "Number of periods for %K", "The number of periods for %K.", 10, 2, 1000);
    strategy.parameters:addInteger("mid_sd", "%D slowing periods", "The number of periods for slow %D.", 5, 2, 1000);
    strategy.parameters:addInteger("mid_d", "Number of periods for %D", "The number of periods for %D.", 5, 2, 1000);
    strategy.parameters:addInteger("mid_os", "Oversold level", "", 15);
    strategy.parameters:addInteger("mid_ob", "Overbought level", "", 85);
    strategy.parameters:addInteger("mid_ema", "EMA period", "", 7);
    ...
    strategy.parameters:addInteger("mid_Backstep", "Backstep", "The minimal amount of bars between maximums/minimums", 3)

    strategy.parameters:addGroup("High timeframe");
    strategy.parameters:addBoolean("high_use", "Use this timeframe", "", true);
	strategy.parameters:addInteger("high_k", "Number of periods for %K", "The number of periods for %K.", 10, 2, 1000);
    strategy.parameters:addInteger("high_sd", "%D slowing periods", "The number of periods for slow %D.", 5, 2, 1000);
    strategy.parameters:addInteger("high_d", "Number of periods for %D", "The number of periods for %D.", 5, 2, 1000);
    strategy.parameters:addInteger("high_os", "Oversold level", "", 15);
    strategy.parameters:addInteger("high_ob", "Overbought level", "", 85);
    strategy.parameters:addInteger("high_ema", "EMA period", "", 7);
    strategy.parameters:addInteger(
        "high_Depth",
        "Depth",
        "the minimal amount of bars where there will not be the second maximum",
    ...
    strategy.parameters:addInteger("high_Backstep", "Backstep", "The minimal amount of bars between maximums/minimums", 3)
end
...
function CreateIndi(prefix, source, withzz)
    if not instance.parameters:getBoolean(prefix .. "_use") then
        return nil;
    end
    local data = {};
    local k = instance.parameters:getInteger(prefix .. "_k");
    local sd = instance.parameters:getInteger(prefix .. "_sd");
    local d = instance.parameters:getInteger(prefix .. "_d");
    data.ssd = core.indicators:create("SSD", source, k, sd, d);
    data.ssd_os = instance.parameters:getInteger(prefix .. "_os");
    data.ssd_ob = instance.parameters:getInteger(prefix .. "_ob");
    data.ema = core.indicators:create("EMA", source, instance.parameters:getInteger(prefix .. "ema"));
    if withzz then
        local Depth = instance.parameters:getInteger(prefix .. "_Depth");
        local Deviation = instance.parameters:getInteger(prefix .. "_Deviation");
        local Backstep = instance.parameters:getInteger(prefix .. "_Backstep");
        data.zz = core.indicators:create("ZIGZAG", source, Depth, Deviation, Backstep)
    end
    function data:update()
        self.ssd:update(core.UpdateLast);
        self.ema:update(core.UpdateLast);
        if self.zz ~= nil then
            self.zz:update(core.UpdateLast);
        end
    end
    return data;
end
...
function CreateIndicators(source)
    ...
    low_data = CreateIndi("low", source, false);
    mid_data = CreateIndi("mid", source, true);
    high_data = CreateIndi("high", source, true);
end

function UpdateIndicators()
    if low_data ~= nil then
        low_data:update();
    end
    if mid_data ~= nil then
        mid_data:update();
    end
    if high_data ~= nil then
        high_data:update();
    end
end
...
function GetNextCrest(Zig, index)
    ...
    if not Zig.DATA:hasData(index - 1) then
        return 0;
    end
...
function IsABCUp(Zig, period)
    index = GetNextCrest(Zig, period);
    ...
    return d < b and c < a, a, a_index, b, b_index, c, c_index, d, d_index;
end

function IsABCDown(Zig, period)
    index = GetNextCrest(Zig, period);
    if index == Zig.DATA:size() - 1 then
        return false;
    end
...
function IsABCDUpSignal(source, data, period)
    if data == nil then
        return false;
    end
    local isUp, a, ai, b, bi, c, ci, d, di = IsABCUp(data.zz, period);
    if not isUp then
        return false;
    end
    for i = ci, di do
        if i == 0 or not data.ssd.D:hasData(i - 1) then
            return false;
        end
        if core.crossesOver(data.ssd.K, data.ssd.D, i)
            and data.ssd.K[i] < data.ssd_os
            and source.close[i] > data.ema.DATA[i]
        then
            return true;
        end
    end
...
function IsABCDDownSignal(source, data, period)
    if data == nil then
        return false;
    end
    local isDown, a, ai, b, bi, c, ci, d, di = IsABCDown(data.zz, period);
    if not isUp then
        return false;
    end
    for i = ci, di do
        ...
        if core.crossesUnder(data.ssd.K, data.ssd.D, i)
            and data.ssd.K[i] > data.ssd_ob
            and source.close[i] < data.ema.DATA[i]
        then
            return true;
        end
...
function CreateCustomActions()
    ...
    enterLongAction.IsPass = function (source, period, periodFromLast)
        if low_data == nil and mid_data == nil and high_data == nil then
            return false;
        end
        if low_data ~= nil then
            if not low_data.ssd.D:hasData(period - 1)
                or not core.crossesOver(low_data.ssd.K, low_data.ssd.D, period)
                or low_data.ssd.K[period] >= low_data.ssd_os
            then
                return false;
            end
        end
        local min = mathex.min(source, period - loopback, period);
        if min >= source.low[period] then
            return false;
        end
        if not IsABCDUpSignal(source, mid_data, period) or not IsABCDUpSignal(source, high_data, period) then
            return false;
        end
    ...
    enterShortAction.IsPass = function (source, period, periodFromLast)
        if low_data == nil and mid_data == nil and high_data == nil then
            return false;
        end
        if low_data ~= nil then
            if not low_data.ssd.D:hasData(period - 1)
                or not core.crossesUnder(low_data.ssd.K, low_data.ssd.D, period)
                or low_data.ssd.K[period] <= low_data.ssd_ob
            then
                return false;
            end
        end
        local max = mathex.max(source, period - loopback, period);
        if max <= source.high[period] then
            return false;
        end
        if not IsABCDDownSignal(source, mid_data, period) or not IsABCDDownSignal(source, high_data, period) then
            return false;
        end
...
function Prepare(name_only)
    ...
    if PositionsCount == 1 then
        buy_positions[#buy_positions + 1] = CreatePositionStrategy(trading_logic.MainSource, "B", "");
        sell_positions[#sell_positions + 1] = CreatePositionStrategy(trading_logic.MainSource, "S", "");
    else
        for i = 1, PositionsCount do
            if instance.parameters:getBoolean("use_position_" .. i) then
                buy_positions[#buy_positions + 1] = CreatePositionStrategy(trading_logic.MainSource, "B", "_" .. i);
                sell_positions[#sell_positions + 1] = CreatePositionStrategy(trading_logic.MainSource, "S", "_" .. i);
            end
        end
    end
end
...
function CreatePositionStrategy(source, side, id)
    ...
    function position_strategy:Open(period)
        local rate = GetEntryRate(self.Source, self.Side, period);
        local command;
        if rate == nil then
...
function EntryFunction(source, period)
    ...
    for _, action in ipairs(EntryActions) do
        if action.IsPass(source, period, periodFromLast) and (not action.ActOnSwitch or not action.IsPass(source, period - 1, periodFromLast - 1)) then
...
function trading_logic:Prepare(name_only)
    ...
    self.MainSource = ExtSubscribe1(self._ids_start + 2, nil, instance.parameters.timeframe, self.HistoryPreloadBars, instance.parameters.is_bid, "bar");
    if self.RequestBidAsk then
        if instance.parameters.is_bid then
            self.MainSourceBid = self.MainSource;
            self.MainSourceAsk = ExtSubscribe1(self._ids_start + 3, nil, instance.parameters.timeframe, self.HistoryPreloadBars, not instance.parameters.is_bid, "bar");
        else
            self.MainSourceAsk = self.MainSource;
            self.MainSourceBid = ExtSubscribe1(self._ids_start + 3, nil, instance.parameters.timeframe, self.HistoryPreloadBars, not instance.parameters.is_bid, "bar");
        end
    end
end
