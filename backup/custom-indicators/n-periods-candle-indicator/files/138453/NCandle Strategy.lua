...
local STRATEGY_NAME = "NCandle Strategy";
function CreateParameters() 
    ...
    strategy.parameters:addInteger("N", "N", "", 10);
    strategy.parameters:addBoolean("IncludeCurrentBar", "Include current bar", "", true);
	strategy.parameters:addBoolean("Ignore", "Ignore the inside bar", "", false);
end

local MACD, ncandle;
function CreateEntryIndicators(source)
    MACD = core.indicators:create("MACD", source, instance.parameters.SN, instance.parameters.LN, instance.parameters.IN);
    local profile = core.indicators:findIndicator("MODIFIED NCANDLE");
    assert(profile ~= nil, "Please, download and install " .. "MODIFIED NCANDLE" .. ".LUA indicator");
    ncandle = core.indicators:create("MODIFIED NCANDLE", source, instance.parameters.N, instance.parameters.IncludeCurrentBar, instance.parameters.Ignore);
end

function UpdateIndicators()
    MACD:update(core.UpdateLast);
    ncandle:update(core.UpdateLast);
end
...
function IsEntryLong(source, period)
    if not ncandle.DATA:hasData(period - 1) or not MACD.DATA:hasData(period - 1) then
        return false;
    end
    return core.crossesUnderOrTouch(source, ncandle.Low, period)
        and MACD.MACD[period] < 0 and MACD.MACD[period] > MACD.SIGNAL[period];
end
function IsEntryShort(source, period)
    return core.crossesOverOrTouch(source, ncandle.High, period)
        and MACD.MACD[period] > 0 and MACD.MACD[period] < MACD.SIGNAL[period];
end
function IsExitLong(source, period)
    return core.crossesUnder(MACD.MACD, MACD.SIGNAL, period);
end
function IsExitShort(source, period)
    return core.crossesOver(MACD.MACD, MACD.SIGNAL, period);
end
