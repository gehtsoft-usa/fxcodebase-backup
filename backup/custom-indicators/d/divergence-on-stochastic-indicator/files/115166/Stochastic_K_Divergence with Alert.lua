...
local Alert_Name={ "Divergence" ,"Divergence", "Divergence", "Divergence"};
local Signal_Name={"Classic bullish", "Classic bearish", "Reversal bullish", "Reversal bearish"};
--//////////////////////////////////////////////////////////////////////////////
--\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
...
--\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\
	...
	indicator.parameters:addString("Signal_Execution", "Signal Execution", "", "Live");
...
function Parameters ( id, Label1,Label2,internal_color )
   ...
   indicator.parameters:addGroup(id .. ". " ..Label1 .. " Alert");
...
--\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\
    ...
    core.host:execute("subscribeTradeEvents", 10, "offers");
    end
    ...
    core.host:execute ("setTimer", 9 , Timer);
	end
			
end 

function ReleaseInstance()
core.host:execute ("killTimer", 9 );
end 
...
function Update(period, mode)
    ...
    pperiod = period;
    if  pperiod1 ~= source:serial(period) then
...
function AsyncOperationFinished (cookie, success, message)
    ...
    if cookie~=10--subscribeTradeEvents
	and cookie~= 9 --Timer
	then
	return;
	end
        ...
        if prev ~= nil then
            if Stochastic.K[curr] > Stochastic.K[prev] and source.low[curr] < source.low[prev] then
                --if I then
                    ...
                    core.host:execute("drawLine", source:serial(curr), source:date(prev), Stochastic.K[prev], source:date(curr), Stochastic.K[curr], DN_color);
                    Signal[curr]=1;     
            --else
            --        DN[period] = curr - prev;
            --    end
        ...
        if prev ~= nil then
            if Stochastic.K[curr] < Stochastic.K[prev] and source.high[curr] > source.high[prev] then
              -- if I then
                    ...
                    core.host:execute("drawLine",  source:serial(curr), source:date(prev), Stochastic.K[prev], source:date(curr), Stochastic.K[curr], UP_color);
                    Signal[curr]=2; 
				--else
                    UP[period] = curr - prev;
                --end
