...
function Init()
    indicator:name("Smoothed Detrended Price Oscillator (DPO)");
    indicator:description("Smoothed Detrended Price Oscillator (DPO)");
    indicator:requiredSource(core.Tick);
    indicator:type(core.Oscillator);
    indicator.parameters:addGroup("Calculation");
    indicator.parameters:addInteger("N", "Period", "Period", 14);
	indicator.parameters:addInteger("Smoothing_Period", "Smoothing Period", "Period", 14);
...
local Smoothing;
 function Prepare(nameOnly)   
	...
	Smoothing= core.indicators:create("MVA", Raw, Smoothing_Period);
	
	first = Smoothing.DATA:first()+iShift;
  
    DPO = instance:addStream("DPO", core.Line, name .. ".DPO", "DPO", instance.parameters.clrDPO, first+iShift);
...
function Update(period, mode)
	...
	return;
     ...
     Raw[period]=source[period]-MA.DATA[period-iShift];
	 ...
	 DPO[period]= Smoothing.DATA[period];
