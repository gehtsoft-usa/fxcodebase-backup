...
function Init()
    ...
    indi_alerts:AddAlert("Buy level");
    indi_alerts:AddAlert("Sell level");
end
...
function Activate(alert, period, historical_period)
    ...
    if alert.id == 1 then
        if core.crossesOver(DPO, buy_level, period) then
            alert:UpAlert(source, period, alert.Label .. ". Crossing over", DPO[period], historical_period);
        elseif core.crossesUnder(DPO, buy_level, period) then
            alert:DownAlert(source, period, alert.Label .. ". Crossing under", DPO[period], historical_period);
        end
    end
    if alert.id == 2 then
        if core.crossesOver(DPO, sell_level, period) then
            alert:UpAlert(source, period, alert.Label .. ". Crossing over", DPO[period], historical_period);
        elseif core.crossesUnder(DPO, sell_level, period) then
            alert:DownAlert(source, period, alert.Label .. ". Crossing under", DPO[period], historical_period);
        end
    end
