...
function Init()
    ...
    indicator.parameters:addDouble("shift", "Shift", "", 3);
...
function Update(period)
    if (period >= first) then
        ...
        if shift_type == "pips" then
            du[period] = core.max(source.high, range) + shift * source:pipSize();
            dn[period] = core.min(source.low, range) - shift * source:pipSize();
        else
            du[period] = core.max(source.high, range) * (1 + shift / 100);
            dn[period] = core.min(source.low, range) / (1 + shift / 100);
        end
        if (sm) then
    ...
    local Width = (du[period] - dn[period])

    local Value = " Channel width " .. string.format("%." .. 1 .. "f", Width / source:pipSize()) .. " Pips"
    Value = Value .. " Channel width " .. string.format("%." .. 4 .. "f", Width / ((dn[period] + Width) / 100)) .. " %"
