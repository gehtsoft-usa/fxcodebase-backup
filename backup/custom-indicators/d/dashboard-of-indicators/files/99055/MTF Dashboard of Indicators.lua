...
function Prepare(nameOnly)   
    ...
    assert(core.indicators:findIndicator(IndicatorList[j]) ~= nil, IndicatorList[j] .. " indicator must be installed");
	 ...
	 for i = 1, Count, 1 do	
	  ...
	  Source[i]= core.host:execute("getSyncHistory", source:instrument(), TF[i], source:isBid(),first,20000 + i , 10000 +i);
	  loading [i]=true;
 ...
 function Calculate (context,i, j )
	...
	Indicator[i][j]:update(core.UpdateLast); 
     ...
     return;
		...
		context:drawText (8, TF[i], Color, -1, x1, y0-height/2, x2, y0+height/2, context.CENTER, 0);
		end
