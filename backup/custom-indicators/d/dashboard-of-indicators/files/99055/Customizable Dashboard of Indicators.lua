			    ...
			    for j= 1 , 10, 1 do
					  if instance.parameters:getBoolean ("On"..j) then						
						iNum=iNum+1;						 
							   ...
							   Indicator[i][iNum] = iprofile[i][iNum]:createInstance(Source[i].close, iparams[i][iNum]);
							   else
 ...
 function Calculate (context,i, j )
	...
	Indicator[i][j]:update(core.UpdateLast); 
     ...
     return;
		...
		context:drawText (8, Pair[i], Color, -1, x1, y1+yGap , x2, y1+height+yGap, context.CENTER, 0);
		end
