...
function GetLastSignal(indi, source)
    for i = instance.parameters.MinBars, source:size() - 1 do
        if indi[1].DoubleTop:hasData(NOW - i) then
            if source.close[NOW] < source.high[NOW - i + instance.parameters.MinBars] then
                return 1, source:date(NOW - i + instance.parameters.MinBars), "PASS";
            else
                return 1, source:date(NOW - i + instance.parameters.MinBars), "FAIL";
            end
        end
        if indi[1].DoubleBottom:hasData(NOW - i) then
            if source.close[NOW] > source.low[NOW - i + instance.parameters.MinBars] then
                return -1, source:date(NOW - i + instance.parameters.MinBars), "PASS";
            else
                return -1, source:date(NOW - i + instance.parameters.MinBars), "FAIL";
            end
