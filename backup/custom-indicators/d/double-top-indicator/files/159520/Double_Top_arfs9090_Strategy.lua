...
local STRATEGY_NAME = "Double Top arfs9090 Strategy";
...
function CreateParameters() 
    strategy.parameters:addDouble("MinHeight", "Minimum Height", "Minimum height of the pattern in pips", 10, 1, 1000);
    strategy.parameters:addDouble("MaxDist", "Maximum Distance", "Maximum distance between tops in pips", 20, 1, 1000);
    strategy.parameters:addInteger("MinBars", "Minimum Bars", "Minimum number of bars between tops", 3, 1, 100);
...
function CreateEntryIndicators(source)
    local profile = core.indicators:findIndicator("DOUBLE_TOP");
    assert(profile ~= nil, "Please, download and install " .. "DOUBLE_TOP" .. ".LUA indicator");
    local indicatorParams = profile:parameters();
    indicatorParams:setInteger("MinHeight", instance.parameters.MinHeight);
    indicatorParams:setInteger("MaxDist", instance.parameters.MaxDist);
    indicatorParams:setInteger("MinBars", instance.parameters.MinBars);
    indi = core.indicators:create("DOUBLE_TOP", source, indicatorParams)
...
function FindLastDoubleTop(source, period)
    for i = 0, period do
        if indi.Top:hasData(period - i) then
            return indi.Top[period - i], period - i;
        end
    end
    return nil, nil;
end
function FindLastDoubleBottom(source, period)
    for i = 0, period do
        if indi.Bottom:hasData(period - i) then
            return indi.Bottom[period - i], period - i;
        end
    end
    return nil, nil;
end
function IsEntryLong(source, period)
    local last, last_period = FindLastDoubleTop(source, period);
    if last == nil or ma.DATA[last_period] < last then
        return false;
    end
    local prev_last, prev_last_period = FindLastDoubleTop(source, last_period - 1);
    return prev_last ~= nil and prev_last <= last and ma.DATA[prev_last_period] < prev_last;
end
function IsEntryShort(source, period)
    local last, last_period = FindLastDoubleBottom(source, period);
    if last == nil or ma.DATA[last_period] > last then
        return false;
    end
    local prev_last, prev_last_period = FindLastDoubleBottom(source, last_period - 1);
    return prev_last ~= nil and prev_last >= last and ma.DATA[prev_last_period] > prev_last;
end
