...
local indi_name = "Double_Top Dashboard";
local indi_id = "DOUBLE_TOP ALERT EXACT";
...
function CreateIndicators(source)
    ...
    p:setInteger("MinHeight", instance.parameters.MinHeight);
    p:setInteger("MaxDist", instance.parameters.MaxDist);
    p:setInteger("MinBars", instance.parameters.MinBars);
...
function GetLastSignal(indi, source)
    for i = instance.parameters.MinBars, source:size() - 1 do
        if indi[1].DoubleTop:hasData(NOW - i) then
            return 1, source:date(NOW - i + instance.parameters.MinBars), "Double Top";
        end
        if indi[1].DoubleBottom:hasData(NOW - i) then
            return -1, source:date(NOW - i + instance.parameters.MinBars), "Double Bottom";
        end
        if indi[1].Top:hasData(NOW - i) then
            return 1, source:date(NOW - i + instance.parameters.MinBars), "Top";
        end
        if indi[1].Bottom:hasData(NOW - i) then
            return -1, source:date(NOW - i + instance.parameters.MinBars), "Bottom";
        end
    end
    return 0, nil, "-";
end
