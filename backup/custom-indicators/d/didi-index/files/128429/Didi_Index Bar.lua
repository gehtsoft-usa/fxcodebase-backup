...
function Init()
    ...
    indicator.parameters:addColor("C_Up", "Curta up color", "Curta Up color", core.rgb(0, 255, 0));
	indicator.parameters:addColor("C_Down", "Curta down color", "Curta Down color", core.rgb(255, 0, 0));
...
function Prepare(nameOnly)
    ...
    Curta = instance:addStream("Curta", core.Bar, name .. ".Curta", "Curta", instance.parameters.C_Up, first);
...
function Update(period, mode)
   if period<first then
    ...
    if Med>0 then
     Curta[period]=(C_MA.DATA[period]/Med)-1;
     
     Longa[period]=(L_MA.DATA[period]/Med)-1;
    end
	
	if Curta[period]> 0 then
	Curta:setColor(period, instance.parameters.C_Up);
	else
	Curta:setColor(period, instance.parameters.C_Down);
	end
