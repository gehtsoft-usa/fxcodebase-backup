...
function Init()
    indicator:name("Distinct Trend Strength");
...
function Update(period, mode)
    ...
    if source[period] > Indicator[math.floor(Number/2)].DATA[period]  then
	
				local TheBottom=Number;
				for i= Number -1, 2, -1 do 
				TheBottom=i;	
					if 	Indicator[i].DATA[period] >Indicator[i+1].DATA[period] 
					and Indicator[i].DATA[period] <= Indicator[i-1].DATA[period] 
					then
					break
					end
