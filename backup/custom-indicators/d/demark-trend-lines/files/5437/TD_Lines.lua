   ...
   if (period>first) then
    ...
    if H1~=nil and H2~=nil then
	 ...
	 One, Two= getline(H1, up[H1], H2, up[H2]);
	        ...
	        core.host:execute("drawLine", 5, source:date(period), TOP[period] , source:date(period)+SIZE * Number,  One * (period+Number) + Two ,instance.parameters.clrUP,instance.parameters.Extension,instance.parameters.widthLinReg);
    ...
    core.drawLine(BOTTOM, core.range(L2, period), down[L2]-0.5*(down[L1]-down[L2])/(L1-L2), L2,  down[L2]+(period-L2-0.5)*(down[L1]-down[L2])/(L1-L2), period);
     ...
     One, Two= getline(L1, down[L1], L2, down[L2]);
	        ...
	        core.host:execute("drawLine", 6, source:date(period), BOTTOM[period] , source:date(period)+SIZE * Number,  One * (period+Number) + Two ,instance.parameters.clrDN,instance.parameters.Extension,instance.parameters.widthLinReg);
