...
-- http://fxcodebase.com/code/viewtopic.php?f=17&t=69141
...
{ 
    {
        ...
        UpCondition = function (period)
            ...
            local top0 = math.max(indi.Jaw[period], indi.Lips[period], indi.Teeth[period]);
            local top1 = math.max(indi.Jaw[period - 1], indi.Lips[period - 1], indi.Teeth[period - 1]);
            return source.close[period] >= top0 and source.close[period - 1] < top1;
        end,
        DownCondition = function (period)
            ...
            local bottom0 = math.min(indi.Jaw[period], indi.Lips[period], indi.Teeth[period]);
            local bottom1 = math.min(indi.Jaw[period - 1], indi.Lips[period - 1], indi.Teeth[period - 1]);
            return source.close[period] <= bottom0 and source.close[period - 1] > bottom1;
        end,
