...
function Init()
    indicator:name("Other Timeframe Color SAR")
...
function Update(period)
    ...
    high = btf_source.high[p]
    low = btf_source.low[p]
    prevHigh = btf_source.high[p - 1]
    prevLow = btf_source.low[p - 1]
    if (period == first or prev_period == -1) then
        ...
        parOp[period] = parOp[prev_period]
        position[period] = position[prev_period]
        tradeHigh[period] = tradeHigh[prev_period]
        tradeLow[period] = tradeLow[prev_period]
        af[period] = af[prev_period]
    end
    ...
    if position[period] == 1 then
        if (low < parOp[period]) then
            ...
            if (tradeHigh[period] > tradeHigh[prev_period] and af[period] < maxVal) then
        ...
        if (high > parOp[period]) then
            ...
            SAR[period] = parOp[period]
            if (tradeLow[period] < tradeLow[prev_period] and af[period] < maxVal) then
                af[period] = af[period] + quant
                if af[period] > maxVal then
                    af[period] = maxVal
                end
    ...
    if p ~= 0 then
        if position[p] == 1 then
            ...
            if LabelBackground then
                ...
                label2.Text = string.format("Stop : %s (%s)", win32.formatNumber(SAR[p], false, source:getPrecision()), win32.formatNumber(math.abs(SAR[p] - source.low[p]) / source:pipSize(), false, 1))
            ...
            elseif ShowLabel ~= "no" then
                ...
                local label2 = string.format("Stop : %s (%s)", win32.formatNumber(SAR[p], false, source:getPrecision()), win32.formatNumber(math.abs(SAR[p] - source.low[p]) / source:pipSize(), false, 1))
