...
function Init()
    indicator:name("Tick Price Change");
...
-- Routine
 function Prepare(nameOnly) 
    ...
    EMA1 = core.indicators:create("EMA", source , price_smoothing); 
	 end 
...
function Update(period, mode)
	...
	if period < EMA1.DATA:first() then
	return;
	end
	
	vpc[period] = (EMA1.DATA[period]- EMA1.DATA[period-length+1])  ;
	else
	 
	vpc[period] = (source [period]- source [period-length+1])  ;
	end
