...
-- http://fxcodebase.com/code/viewtopic.php?f=17&t=68570
...
function AddLevel(id, value)
    tool.parameters:addGroup("Level #" .. id)
    tool.parameters:addBoolean("use_" .. id, "Use level", "", true);
    tool.parameters:addDouble("level_" .. id, "Level", "", value);
    tool.parameters:addColor("color_" .. id, "Color", "", core.rgb(255, 0, 0));
    tool.parameters:addInteger("width_" .. id, "Width", "", 1);
    tool.parameters:addInteger("style_" .. id, "Style", "", core.LINE_SOLID);
    tool.parameters:setFlag("style_" .. id, core.FLAG_LINE_STYLE);

    tool.parameters:addString("order_type_" .. id, "Order type", "", "no");
    tool.parameters:addStringAlternative("order_type_" .. id, "No order", "", "no");
    tool.parameters:addStringAlternative("order_type_" .. id, "Buy", "", "B");
    tool.parameters:addStringAlternative("order_type_" .. id, "Sell", "", "S");
    tool.parameters:addInteger("lots_" .. id, "Lots", "", 1, 1, 1000000);
    tool.parameters:addString("stop_type" .. id, "Stop Order", "", "no");
    tool.parameters:addStringAlternative("stop_type" .. id, "No stop", "", "no");
    tool.parameters:addStringAlternative("stop_type" .. id, "In Pips", "", "pips");
    tool.parameters:addDouble("stop" .. id, "Stop Value", "", 30);
    tool.parameters:addString("limit_type" .. id, "Limit Order", "", "no");
    tool.parameters:addStringAlternative("limit_type" .. id, "No limit", "", "no");
    tool.parameters:addStringAlternative("limit_type" .. id, "In Pips", "", "pips");
    tool.parameters:addDouble("limit" .. id, "Limit Value", "", 30);
end

function Init()
    tool:name("Fibonacci Order system");
    tool:description("");
    tool:icon("");
...
function UpdateLevels()
    ...
    for id = 1, 8 do
        if (instance.parameters:getBoolean("use_" .. id)) then
            ...
            level.color = instance.parameters:getColor("color_" .. id);
            level.width = instance.parameters:getInteger("width_" .. id);
            level.style = instance.parameters:getInteger("style_" .. id);
            level.order_type = instance.parameters:getString("order_type_" .. id);
            level.lots = instance.parameters:getInteger("lots_" .. id);
            level.stop_type = instance.parameters:getString("stop_type" .. id);
            level.stop = instance.parameters:getDouble("stop" .. id);
            level.limit_type = instance.parameters:getString("limit_type" .. id);
            level.limit = instance.parameters:getDouble("limit" .. id);
    ...
    if stage == 2 then
        if not init then
            ...
            for i, level in ipairs(levels) do
                level.pen_id = i + 1;
                context:createPen(level.pen_id, context:convertPenStyle(level.style), level.width, level.color);
            end
            init = true;
        end
        local pane = core.host.Window.CurrentPane;
        ...
        local range = instance.parameters.start_rate - instance.parameters.end_rate;
        for i, level in ipairs(levels) do
            local visible, y = context:pointOfPrice(instance.parameters.start_rate - range * level.value);
            context:drawLine(level.pen_id, x1, y, x2, y);
        end
...
function Prepare(onlyName)
    ...
    local range = instance.parameters.start_rate - instance.parameters.end_rate;
    for i, level in ipairs(levels) do
        if level.order_type ~= "no" then
            local command = trading:EntryOrder(stream:instrument())
                :SetAccountID(instance.parameters.account)
                :SetAmount(level.lots)
                :SetSide(level.order_type)
                :SetRate(instance.parameters.start_rate - range * level.value);
            if level.stop_type == "pips" then
                command:SetPipStop(nil, level.stop, nil);
            end
            if level.limit_type == "pips" then
                command:SetPipLimit(nil, level.limit);
            end
            command:Execute();
        end
    end
...
function ChangeParameters()
    ...
    for id = 1, 8 do
        instance.parameters.setBoolean(instnace.parameters:getBoolean("use_" .. id));
        instance.parameters.setDouble(instnace.parameters:getDouble("stop" .. id));
        instance.parameters.setDouble(instnace.parameters:getDouble("level_" .. id));
        instance.parameters.setDouble(instnace.parameters:getDouble("limit" .. id));
        instance.parameters.setColor(instnace.parameters:getColor("color_" .. id));
        instance.parameters.setString(instnace.parameters:getString("order_type_" .. id));
        instance.parameters.setString(instnace.parameters:getString("stop_type" .. id));
        instance.parameters.setString(instnace.parameters:getString("limit_type" .. id));
        instance.parameters.setInteger(instnace.parameters:getIntege("width_" .. id));
        instance.parameters.setInteger(instnace.parameters:getIntege("style_" .. id));
        instance.parameters.setInteger(instnace.parameters:getIntege("lots_" .. id));
    end
    Prepare(false);
end
