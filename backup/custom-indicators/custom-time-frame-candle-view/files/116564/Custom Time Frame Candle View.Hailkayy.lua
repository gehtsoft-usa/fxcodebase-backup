...
-- http://fxcodebase.com/code/viewtopic.php?f=17&t=60537&p=116564#p116564
...
function calcValue(Index, period)
    ...
    local date, e = core.getcandle(TF, History:date(period), trading_day_offset, trading_week_offset);
    local period_lenth = (e - date) * Step;

    if period == 1 then
        local date_start = date + time_shift;
