...
{ 
    {
        ...
        UpCondition = function (period)
            return (source.close[period] - source.open[period]) / source:pipSize() >= move;
        end,
        DownCondition = function (period)
            return (source.open[period] - source.close[period]) / source:pipSize() >= move;
        end,
...
function Init()
    indicator:name("Price change alert");
    indicator:description("");
    indicator:requiredSource(core.Bar);
    indicator:type(core.Indicator);
    
    indicator.parameters:addInteger("move", "Move, pips", "", 10);
