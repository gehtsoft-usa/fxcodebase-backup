...
function DrawLevels(High, Low, Close, context, shift, index)
    if Close == -1 then
        return
    end
...
function Draw(stage, context)
    ...
    if ShowMode == "LAST" then
        local High, Low, Close = Find(1)
        DrawLevels(High, Low, Close, context, 1, 0)
    elseif ShowMode == "CURR" then
        local High, Low, Close = Find(0)
        DrawLevels(High, Low, Close, context, 0, 0)
    elseif ShowMode == "HIST" then
        for i = 0, hist_count - 1 do
            local High, Low, Close = Find(i)
            DrawLevels(High, Low, Close, context, i, i)
        end
    end
end
