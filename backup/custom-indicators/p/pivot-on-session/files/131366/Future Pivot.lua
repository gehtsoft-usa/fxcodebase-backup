...
-- http://fxcodebase.com/code/viewtopic.php?f=17&t=69384
...
function Init()
    ...
    indicator.parameters:addInteger("fstyleP", string.format("%s style", "Pivot (Future)"),
        string.format("The style of the %s.", "pivot line"), core.LINE_DASH);
    indicator.parameters:setFlag("fstyleP", core.FLAG_LEVEL_STYLE);
    ...
    indicator.parameters:addInteger("fstyleS1", string.format("%s style", "S1 (Future)"),
        string.format("The style of the %s.", "first support line"), core.LINE_DASH);
    indicator.parameters:setFlag("fstyleS1", core.FLAG_LEVEL_STYLE);
    ...
    indicator.parameters:addInteger("fstyleS2", string.format("%s style", "S2 (Future)"),
        string.format("The style of the %s.", "second support line"), core.LINE_DASH);
    indicator.parameters:setFlag("fstyleS2", core.FLAG_LEVEL_STYLE);
    ...
    indicator.parameters:addInteger("fstyleS3", string.format("%s style", "S3 (Future)"),
        string.format("The style of the %s.", "third support line"), core.LINE_DASH);
    indicator.parameters:setFlag("fstyleS3", core.FLAG_LEVEL_STYLE);
    ...
    indicator.parameters:addInteger("fstyleS4", string.format("%s style", "S4 (Future)"),
        string.format("The style of the %s.", "fourth support line"), core.LINE_DASH);
    indicator.parameters:setFlag("fstyleS4", core.FLAG_LEVEL_STYLE);
    ...
    indicator.parameters:addInteger("fstyleR1", string.format("%s style", "R1 (Future)"),
        string.format("The style of the %s.", "first resistance line"), core.LINE_DASH);
    indicator.parameters:setFlag("fstyleR1", core.FLAG_LEVEL_STYLE);
    ...
    indicator.parameters:addInteger("fstyleR2", string.format("%s style", "R2 (Future)"),
        string.format("The style of the %s.", "second resistance line"), core.LINE_DASH);
    indicator.parameters:setFlag("fstyleR2", core.FLAG_LEVEL_STYLE);
    ...
    indicator.parameters:addInteger("fstyleR3", string.format("%s style", "R3 (Future)"),
        string.format("The style of the %s.", "third resistance line"), core.LINE_DASH);
    indicator.parameters:setFlag("fstyleR3", core.FLAG_LEVEL_STYLE);
    ...
    indicator.parameters:addInteger("fstyleR4", string.format("%s style", "R4 (Future)"),
        string.format("The style of the %s.", "fourth resistance line"), core.LINE_DASH);
    indicator.parameters:setFlag("fstyleR4", core.FLAG_LEVEL_STYLE);
    ...
    indicator.parameters:addInteger("fstyleMP", string.format("%s style", "Midpoint lines (Future)"),
        string.format("The style of the %s.", "midpoint lines"), core.LINE_DASH);
    indicator.parameters:setFlag("fstyleMP", core.FLAG_LEVEL_STYLE);
end
...
function Prepare(nameOnly) 
    ...
    fstyleP = instance.parameters.fstyleP;
    ...
    fstyleMP = instance.parameters.fstyleMP;
    ...
    fstyle[S1] = instance.parameters.fstyleS1;
    fstyle[S2] = instance.parameters.fstyleS2;
    fstyle[S3] = instance.parameters.fstyleS3;
    fstyle[S4] = instance.parameters.fstyleS4;
    fstyle[R1] = instance.parameters.fstyleR1;
    fstyle[R2] = instance.parameters.fstyleR2;
    fstyle[R3] = instance.parameters.fstyleR3;
    fstyle[R4] = instance.parameters.fstyleR4;
    ...
    if ShowMode == O_HIST then
        P_out = instance:addStream("P", core.Line, sname .. "." .. "P", "P", clrP, 0);
        P_out:setWidth(widthP);
        P_out:setStyle(styleP);
    else
    ...
    ref = core.host:execute("getSyncHistory", source:instrument(), "m30", source:isBid(), 10, 100, 101);   
    loading = true;
...
function DrawLabel(context, id, x, i)
    local level;
    if id == RP then
        level = P[i];
    elseif stream[id] == nil then
        return;
    else
        level = stream[id][i];
    end
    local _, y = context:pointOfPrice(level);
    local w, h = context:measureText(FONT, name[id], 0);
    core.host:trace(level .. " " .. h);
    context:drawText(FONT, name[id], core.COLOR_LABEL, -1, x - w, y - h, x, y, 0);
end
...
function Draw(stage, context)
    ...
    local start_date, end_date = core.getcandle("D1", source:date(NOW), offset, weekoffset);
    local start = core.findDate(source, start_date, false) - 1;
    if start < 0 or not P:hasData(start) then
        return;
    end
    local x = context:positionOfBar(start);
    DrawLabel(context, RP, x, start);
    DrawLabel(context, R1, x, start);
    DrawLabel(context, R2, x, start);
    DrawLabel(context, R3, x, start);
    DrawLabel(context, R4, x, start);
    DrawLabel(context, S1, x, start);
    DrawLabel(context, S2, x, start);
    DrawLabel(context, S3, x, start);
    DrawLabel(context, S4, x, start);
end
...
function Update(period, mode)
    if loading or ref:size() == 0 then
        return;
    end
    local start_date, end_date = core.getcandle("D1", source:date(period), offset, weekoffset);
    local start_period = core.findDate(ref, start_date, false);
    if start_period < 0 then
        return;
    end
    local end_period = core.findDate(ref, end_date, false);

    local ll, hh = mathex.minmax(ref, start_period, end_period);
    if CalcMode == O_PIVOT or CalcMode == O_FIB or CalcMode == O_FLOOR then
        P[period] = (hh + ref.close[end_period] + ll) / 3;
    elseif CalcMode == O_CAM then
        P[period] = ref.close[end_period];
    elseif CalcMode == O_FIBR then
        P[period] = (hh + ll) / 2;
    elseif CalcMode == O_WOOD then
        local open;
        if (start_period == ref:size() - 1) then
            -- for a live day take close as open of the next period
            open = ref.open[start_period];
        else
            open = ref.open[start_period + 1];
        end
        P[period] = (hh + ll + open * 2 ) / 4;
    end
    H[period] = hh;
    L[period] = ll;
    ...
    if ShowMode == O_HIST then
        if source:date(NOW) > end_date then
            P_out[period] = P[period];
            local nb = P:hasData(period - 1) and math.abs(P[period - 1] - P[period]) > eps and not(SameSizeBar);
            for i = S1, R4, 1 do
            ...
            local start = math.max(0, core.findDate(source, start_date, false));
            if start > 1 and  not P_out:hasData(start - 1) then
                local end_period = math.max(0, core.findDate(source, end_date, false));
                for ii = start, end_period do
                    P_out[ii] = P[ii];
                    local nb = P:hasData(ii - 1) and math.abs(P[ii - 1] - P[ii]) > eps and not(SameSizeBar);
                    for i = S1, R4, 1 do
                        if show[i] and d[i] ~= 0 then
                            stream[i][ii] = d[i];
                            if nb then
                                stream[i]:setBreak(ii, true);
                            end
                        end
                    end
                end
    ...
    ShowLevels(d, period, start_date, end_date);
end
...
function ShowLevels(data, period, start, stop)
    local d1 = start;
    local d2 = source:date(NOW);
    local sp = styleP;
    if period == source:size() - 1 then
        sp = fstyleP;
    end

    host:execute("drawLine", PID, d1, P[period], d2, P[period], clrP, sp, sp, "P(" .. round(P[period], source:getPrecision()) .. ")");
    ...
    for i = S1, R4, 1 do
        if show[i] and data[i] ~= 0 then
            ...
            host:execute("drawLine", i, d1, data[i], d2, data[i], clr[i], sp, width[i], name[i] .. "(" .. round(data[i], source:getPrecision()) .. ")");
            if LabelLoc == O_END or LabelLoc == O_BOTH then
    ...
    if ShowMP then
        ShowMPP(0, d1, d2, d[S2], d[S3], "M0", period);
        ShowMPP(1, d1, d2, d[S1], d[S2], "M1", period);
        ShowMPP(2, d1, d2, P[period], d[S1], "M2", period);
        ShowMPP(3, d1, d2, P[period], d[R1], "M3", period);
        ShowMPP(4, d1, d2, d[R1], d[R2], "M4", period);
        ShowMPP(5, d1, d2, d[R2], d[R3], "M5", period);
    end
end

function ShowMPP(i, d1, d2, p1, p2, l, period)
