...
-- http://fxcodebase.com/code/viewtopic.php?f=17&t=69251
...
function Init()
    ...
    AddAction("1_o", "Action On Cross Over 1");
    AddAction("1_u", "Action On Cross Under 1");
    AddParam(2, true, 50, core.rgb(0, 0, 255), 1, core.LINE_DASH)
    AddAction("2_o", "Action On Cross Over 2");
    AddAction("2_u", "Action On Cross Under 2");
    AddParam(3, true, 17, core.rgb(128, 128, 0), 1, core.LINE_DOT)
    AddAction("3_o", "Action On Cross Over 3");
    AddAction("3_u", "Action On Cross Under 3");
    AddParam(4, true, 83, core.rgb(0, 255, 0), 1, core.LINE_SOLID)
    AddAction("4_o", "Action On Cross Over 4");
    AddAction("4_u", "Action On Cross Under 4");
    AddParam(5, true, 67, core.rgb(0, 128, 128), 1, core.LINE_DASH)
    AddAction("5_o", "Action On Cross Over 5");
    AddAction("5_u", "Action On Cross Under 5");
    AddParam(6, true, 33, core.rgb(255, 255, 0), 1, core.LINE_DOT)
    AddAction("6_o", "Action On Cross Over 6");
    AddAction("6_u", "Action On Cross Under 6");

    indicator.parameters:addBoolean("use_ma_filter", "Use MA Filter", "", true);
    indicator.parameters:addString("ma_tf", "Timeframe for MA filter", "", "m5");
    indicator.parameters:setFlag("ma_tf", core.FLAG_PERIODS);
    indicator.parameters:addInteger("ma_filter_period", "Filter MA Period", "", 14);
    AddAverages("ma_filter_method", "Filter MA Method", "MVA");

    trading:Init(indicator.parameters, 1);
    indicator.parameters:addString("entry_execution_type", "Entry Execution Type", "Once per bar close or on every tick", "EndOfTurn");
    indicator.parameters:addStringAlternative("entry_execution_type", "End of Turn", "", "EndOfTurn");
    indicator.parameters:addStringAlternative("entry_execution_type", "Live", "", "Live");
    indicator.parameters:addString("exit_execution_type", "Exit Execution Type", "Once per bar close or on every tick", "EndOfTurn");
    indicator.parameters:addStringAlternative("exit_execution_type", "End of Turn", "", "EndOfTurn");
    indicator.parameters:addStringAlternative("exit_execution_type", "Live", "", "Live");
...
local source = nil
local levels = {};
local entry_execution_type, exit_execution_type;
local ma_filter;
local custom_id;

function CloseAll(source, period)
    signaler:Signal("Close all");
    local closedCount = 0;
...
function CreateAction(id, source)
    ...
    elseif actionType == "SELL" then
        ...
        action.ExecuteData = CreateSellPositions(source);
    elseif actionType == "BUY" then
        action.Execute = GoLong;
        action.ExecuteData = CreateBuyPositions(source);
...
function GoLong(source, period, positions, log)
    if ma_filter ~= nil and ma_filter.DATA[NOW] <= source.close[period] then
        return;
    end
    signaler:Signal("Open long");
    if instance.parameters.allow_trade then
        DoCloseOnOpposite("B");
...
function Prepare(nameOnly)
    ...
    custom_id = profile:id() .. instance.source:name();
    ...
    exit_execution_type = instance.parameters.exit_execution_type;
    if instance.parameters.use_ma_filter then
        ma_source = sources:Request(1, source, instance.parameters.ma_tf);
        ma_filter = CreateAverages(instance.parameters.ma_filter_period, instance.parameters.ma_filter_method, ma_source);
    end

    for i = 1, 6 do
        if instance.parameters:getBoolean("Enable" .. i) then
            local level = {};
            level.Value = instance.parameters:getInteger("Level" .. i);

            local action, is_entry = CreateAction(i .. "_o", source);
            level.CrossOverAction = action;
            level.CrossOverIsEntry = is_entry
            
            local action, is_entry = CreateAction(i .. "_u", source);
            level.CrossUnderAction = action;
            level.CrossUnderIsEntry = is_entry
...
local levels_lines = {};
local last_entry_serial;
local last_exit_serial;
...
function Update(period, mode)
    ...
    if ma_filter ~= nil then
        ma_filter:update(mode);
    end
    for _, module in pairs(Modules) do if module.ExtUpdate ~= nil then module:ExtUpdate(0, source, period); end end 
    if period ~= source:size() - 1 then
        return;
    end
    if entry_execution_type == "Live" or last_entry_serial ~= source:serial(period) then
        if entry_execution_type ~= "Live" then
            period = period - 1;
        end
        for i, level in ipairs(levels) do
            if level.Lines ~= nil then
                for x, line in ipairs(level.Lines) do
                    if level.CrossOverAction ~= nil 
                        and level.CrossOverIsEntry 
                        and core.crossesOver(source.close, line, period) 
                        and level.LastExecuted ~= source:serial(period)
                    then
                        level.CrossOverAction.Execute(source, period, level.CrossOverAction.ExecuteData);
                        level.LastExecuted = source:serial(period);
                    end
                    if level.CrossUnderAction ~= nil 
                        and level.CrossUnderIsEntry 
                        and core.crossesUnder(source.close, line, period) 
                        and level.LastExecuted ~= source:serial(period)
                    then
                        level.CrossUnderAction.Execute(source, period, level.CrossUnderAction.ExecuteData);
                        level.LastExecuted = source:serial(period);
                    end
                end
            end
        end
        last_entry_serial = source:serial(period);
    end
    if exit_execution_type == "Live" or last_exit_serial ~= source:serial(period) then
        if entry_execution_type ~= "Live" then
            period = period - 1;
        end
        for i, level in ipairs(levels) do
            if level.Lines ~= nil then
                for x, line in ipairs(level.Lines) do
                    if level.CrossOverAction ~= nil and not level.CrossOverIsEntry and core.crossesOver(source.close, line, period) then
                        level.CrossOverAction.Execute(source, period, level.CrossOverAction.ExecuteData);
                    end
                    if level.CrossUnderAction ~= nil and not level.CrossUnderIsEntry and core.crossesUnder(source.close, line, period) then
                        level.CrossUnderAction.Execute(source, period, level.CrossUnderAction.ExecuteData);
                    end
                end
            end
        end
        last_exit_serial = source:serial(period);
    end
end
...
function Draw(stage, context)
    ...
    if stage == 0 then
        if not init then
            for i, level in ipairs(levels) do
                context:createPen(i, context:convertPenStyle(level.Style), level.Width, level.Color);
            end
            if FontSize == 0 then
        ...
        local w, h
        for i, level in ipairs(levels) do
            Level = level.Value * pipSize
            level.Lines = {};
            MaxLevel = math.ceil((topPrice - Level) / (Dig * pipSize)) * (Dig * pipSize) + Level
            MinLevel = math.floor((bottomPrice - Level) / (Dig * pipSize)) * (Dig * pipSize) + Level
            iLevel = MinLevel
            local lines = {};
            while iLevel <= MaxLevel do
                v, y = context:pointOfPrice(iLevel)
                level.Lines[#level.Lines + 1] = iLevel;
