--[[
	...
	Changelog 4.0.2
	- PNF drawing change if not EndofTheDay and TF > t1 
		- At X or O columns new highs or lows will draw immediately, reversals only at new Source Candles
	- forcast Signal only, if EndOfTheDay == true and instance.parameters.TF ~= "t1"
	- added Trendlines to line mode(if zoom out to high to draw X and O's)
...
function Init()
	...
	indicator.parameters:addBoolean("bBuRBeS45", "Bull Resistance Bear Support", "Show bullish resistance and bearish support line", true);
	-- 22.5° Trendlines
	...
	indicator.parameters:addString("enDTB", "Double Top and Bottom", "Enable Double Top or Bottom","true");
	indicator.parameters:addStringAlternative("enDTB", "Enabled", "", "true");
	indicator.parameters:addStringAlternative("enDTB", "Disabled", "", "false");
	indicator.parameters:addStringAlternative("enDTB", "Enabled with H", "", "true with H");
	indicator.parameters:addStringAlternative("enDTB", "Enabled with V", "", "true with V");
	indicator.parameters:addStringAlternative("enDTB", "Enabled with H and V", "", "true with H and V");
	
	indicator.parameters:addString("enTTB", "Triple Top and Bottom", "Enable Triple Top or Bottom","true");
	indicator.parameters:addStringAlternative("enTTB", "Enabled", "", "true");
	indicator.parameters:addStringAlternative("enTTB", "Disabled", "", "false");
	indicator.parameters:addStringAlternative("enTTB", "Enabled with H", "", "true with H");
	indicator.parameters:addStringAlternative("enTTB", "Enabled with V", "", "true with V");
	indicator.parameters:addStringAlternative("enTTB", "Enabled with H and V", "", "true with H and V");
	
	indicator.parameters:addString("enShakeOut", "ShakeOut", "Enable Bull or Bear ShakeOut","true");
	indicator.parameters:addStringAlternative("enShakeOut", "Enabled", "", "true");
	indicator.parameters:addStringAlternative("enShakeOut", "Disabled", "", "false");
	indicator.parameters:addStringAlternative("enShakeOut", "Enabled with H", "", "true with H");
	indicator.parameters:addStringAlternative("enShakeOut", "Enabled with V", "", "true with V");
	indicator.parameters:addStringAlternative("enShakeOut", "Enabled with H and V", "", "true with H and V");
	
	indicator.parameters:addString("enCatapult", "Catapult", "Enable Bull or Bear Catapult","true");
	indicator.parameters:addStringAlternative("enCatapult", "Enabled", "", "true");
	indicator.parameters:addStringAlternative("enCatapult", "Disabled", "", "false");
	indicator.parameters:addStringAlternative("enCatapult", "Enabled with H", "", "true with H");
	indicator.parameters:addStringAlternative("enCatapult", "Enabled with V", "", "true with V");
	indicator.parameters:addStringAlternative("enCatapult", "Enabled with H and V", "", "true with H and V");
	
	indicator.parameters:addString("enTriangle", "Symmetrical Triangle", "Enable Bull or Bear Triangle","true");
	indicator.parameters:addStringAlternative("enTriangle", "Enabled", "", "true");
	indicator.parameters:addStringAlternative("enTriangle", "Disabled", "", "false");
	indicator.parameters:addStringAlternative("enTriangle", "Enabled with H", "", "true with H");
	indicator.parameters:addStringAlternative("enTriangle", "Enabled with V", "", "true with V");
	indicator.parameters:addStringAlternative("enTriangle", "Enabled with H and V", "", "true with H and V");
	
	indicator.parameters:addString("enFlag", "Flag", "Enable Bull or Bear Signal Reversed", "true");
	indicator.parameters:addStringAlternative("enFlag", "Enabled", "", "true");
	indicator.parameters:addStringAlternative("enFlag", "Disabled", "", "false");
	indicator.parameters:addStringAlternative("enFlag", "Enabled with H", "", "true with H");
	indicator.parameters:addStringAlternative("enFlag", "Enabled with V", "", "true with V");
	indicator.parameters:addStringAlternative("enFlag", "Enabled with H and V", "", "true with H and V");
	
	indicator.parameters:addString("enTL", "Trendline break", "Enable Bull or Bear TrendLine breakout","true");
	indicator.parameters:addStringAlternative("enTL", "Enabled", "", "true");
	indicator.parameters:addStringAlternative("enTL", "Disabled", "", "false");
	indicator.parameters:addStringAlternative("enTL", "Enabled with H", "", "true with H");
	indicator.parameters:addStringAlternative("enTL", "Enabled with V", "", "true with V");
	indicator.parameters:addStringAlternative("enTL", "Enabled with H and V", "", "true with H and V");
	-----------------------------------------------------------------------------------------------------------
	...
	indicator.parameters:addDouble("tHMaxRatio", "Horizontal Count max Ratio", "min 0.1 max 100", 4.0, 0.1, 100);
	indicator.parameters:addBoolean("fHorizontal", 	"Only if trend changed", "Show Horizontal Count only if the trend changed occured in the range" ,  true);
...
-------------------------------------------------------------------------------------------------------------------------------------------------------
function addVerticalCount(entry)
	local period = open:size() - 1;
	local e = entry;
	if lastVTarget ~= period then
		if direction[period] > 0 then
			local t = low[period-2] + volume[period-2]*BS*RS;
			local s = low[period-3] - BS;
			local e = entry;
			local tbl = {t=t,e=e,s=s,d="UP",p=period-2,p2=period,a=true,sl = false,f=0};
			table.insert(TA["V"],tbl);
		elseif direction[period] < 0 then
			local t = high[period-2] - volume[period-2]*BS*RS;
			local s = high[period-3] + BS;
			local e = entry;
			local tbl = {t=t,e=e,s=s,d="DN",p=period-2,p2=period,a=true,sl = false,f=0};
			table.insert(TA["V"],tbl);
		end
		lastVTarget = period;
	end
			...
			if isSmaller(high[period-i],l) or isGreater(low[period-i],h) then
				...
				if direction[period] > 0 then
					...
					if  t > high[period] then
						...
						if risk < instance.parameters.tHMinRatio or risk > instance.parameters.tHMaxRatio then break; end
						local tbl = {t=t,e=entry,s=mathex.min(low, period - i + 1, period)-BS,r=risk,d="UP",p=period,p2=period-pos,f=0};
						table.insert(TA["H"],tbl);
						SendAlert("Buy Horizontal Count - Entry:" .. tbl.e .. " Limit:" .. tbl.t .. " Stop:" .. tbl.s,entry)
						lastHTarget = period 
					end
				elseif direction[period] < 0 then
					...
					if  t < low[period] then
						...
						table.insert(TA["H"],tbl);
						SendAlert("Sell Horizontal Count - Entry:" .. tbl.e .. " Limit:" .. tbl.t .. " Stop:" .. tbl.s,entry)
						lastHTarget = period 
					end
				end
				break;
...
function FindWalls()
	...
	local period = open:size() - 1;
	if period < 3 then return end
	if direction[period] < 0 then
		if high[period-2] - BS > high[period-1] then
			if WallO[period-2] == nil then
				WallO[period-2] = {entry=high[period-1] + BS,a=true}
			end
		end
		
	elseif direction[period] > 0 then
		if low[period-2] + BS < low[period-1] then
			if WallX[period-2] == nil then
				WallX[period-2] = {entry=low[period-1] - BS,a=true}
			end
		end
	end
end
...
function CheckWalls()
	...
	local period = open:size() - 1;
	local allowed = false;
	if instance.parameters.fHorizontal then
		if direction[period] > 0 and TL[45].dnOut[pos] ~= 0 and TL[45].upOut[period] ~= 0 then
			allowed = true;
		end
		if direction[period] < 0 and TL[45].upOut[pos] ~= 0 and TL[45].dnOut[period] ~= 0 then
			allowed = true;
		end
	else
		allowed = true;
	end
	for pos,val in pairs(WallO) do 
		if direction[period] > 0 and val.a == true then
			val.entry  = mathex.max(high,pos+1,period-1) + BS
			if isGreaterOrEqual(close[period] , val.entry) and LastCohen ~= period and allowed then
				local t = val.entry  + (period - pos + 1)*BS*RS;-- mathex.max(high, period - i + 1, period) - t;
				local s = mathex.min(low, pos , period)-BS
				local risk2 = (t-val.entry) / (val.entry-s);
				if risk2 > instance.parameters.tHMinRatio and risk2 < instance.parameters.tHMaxRatio then 
					local tbl = {t=t,e=val.entry,s=s,r=risk2,d="UP",p=period,p2=pos,f=0};
					table.insert(TA["H"],tbl);
					if isGreaterOrEqual(high[pos],close[period]) then
						WallO[pos].entry = close[period] + BS
					end
					LastCohen = period;
				end
			end
			if close[period] > open[pos] or period - pos > 24 then
				WallO[pos].a = false
			end
		end
	end
	for pos,val in pairs(WallX) do 
		if direction[period] < 0 and val.a == true then
			val.entry  = mathex.min(low,pos+1,period-1) - BS
			if isSmallerOrEqual(close[period] , val.entry) and LastCohen ~= period and allowed then
				local t = val.entry  - (period - pos + 1)*BS*RS;-- mathex.max(high, period - i + 1, period) - t;
				local s = mathex.max(high, pos, period)+BS
				local risk2 = (val.entry-t) / (s-val.entry);
				if risk2 > instance.parameters.tHMinRatio and risk2 < instance.parameters.tHMaxRatio then 
					local tbl = {t=t,e=val.entry,s=s,r=risk2,d="DN",p=period,p2=pos,f=0};
					table.insert(TA["H"],tbl);
					if isSmallerOrEqual(low[pos],close[period]) then
						WallX[pos].entry = close[period] + BS
					end
					LastCohen = period;
				end
			end
	...
	if isEqual(direction[period],1) then
		...
		if  pos1 ~= nil then
			if isGreater(high[period],high[pos1]) and isGreater(low[period-1],low[period-3]) then
				...
				PA["TTB"][period] = tbl;
				if instance.parameters.enTTB ~= "false" then 
					SendAlert(stType,PA["TTB"][period].entry);
					if instance.parameters.enTTB == "true with H and V" then 
						HorizontalCount(PA["TTB"][period].entry);
						addVerticalCount(PA["TTB"][period].entry);
					elseif instance.parameters.enTTB == "true with H" then 
						HorizontalCount(PA["TTB"][period].entry);
					elseif instance.parameters.enTTB == "true with V" then 
						addVerticalCount(PA["TTB"][period].entry);
					end
				end
				return true;
	...
	if PA["SBS"][period] ~= nil then
		if isEqual(direction[period],1) then
			...
			if isGreaterOrEqual(low[period-1],low[period-3]) then
				...
				PA["DTB"][period] = tbl;
				if instance.parameters.enDTB ~= "false" and PA["DTB"][period].fin then 
					SendAlert("Double Top",PA["DTB"][period].entry);
					if instance.parameters.enDTB == "true with H and V" then 
						HorizontalCount(PA["DTB"][period].entry);
						addVerticalCount(PA["DTB"][period].entry);
					elseif instance.parameters.enDTB == "true with H" then 
						HorizontalCount(PA["DTB"][period].entry);
					elseif instance.parameters.enDTB == "true with V" then 
						addVerticalCount(PA["DTB"][period].entry);
					end
				end
			end
...
function findShakeOut()
	...
	if direction[period] == 1 and TL[45].upOut[period] ~= nil and TL[45].upOut[period] ~= 0 then
		if isGreaterOrEqual(volume[period],3) and isEqual(high[period-4],high[period-2]) and isGreaterOrEqual(volume[period-3],volume[period-1]-3) and
			...
			if instance.parameters.enShakeOut == "true with H and V" then 
				HorizontalCount(PA["SHA"][period].entry);
				addVerticalCount(PA["SHA"][period].entry);
			elseif instance.parameters.enShakeOut == "true with H" then 
				HorizontalCount(PA["SHA"][period].entry);
			elseif instance.parameters.enShakeOut == "true with V" then 
				addVerticalCount(PA["SHA"][period].entry);
			end
	...
	if PA["SBS"][period] ~= nil and PA["TTB"][period-2] ~= nil and PA["TTB"][period-2].fin then
		if isEqual(direction[period],1) then
			...
			if PA["CAT"][period].fin then 
				...
				if instance.parameters.enCatapult == "true with H and V" then 
					HorizontalCount(PA["CAT"][period].entry);
					addVerticalCount(PA["CAT"][period].entry);
				elseif instance.parameters.enCatapult == "true with H" then 
					HorizontalCount(PA["CAT"][period].entry);
				elseif instance.parameters.enCatapult == "true with V" then 
					addVerticalCount(PA["CAT"][period].entry);
				end
			end
			return;
	...
	if PA["SBS"][period] ~= nil then
		if isEqual(direction[period],1) then
			...
			if 	(isEqual(high[period-4]-high[period-2],low[period-1]-low[period-3])) and isGreater(high[period-4]-high[period-2],0) then
				...
				PA["TRI"][period] = tbl;
				if PA["TRI"][period].fin then 
					SendAlert("Bullish Breakout of a Symmetrical Triangle",PA["TRI"][period].entry);
					if instance.parameters.enTriangle == "true with H and V" then 
						HorizontalCount(PA["TRI"][period].entry);
						addVerticalCount(PA["TRI"][period].entry);
					elseif instance.parameters.enTriangle == "true with H" then 
						HorizontalCount(PA["TRI"][period].entry);
					elseif instance.parameters.enTriangle == "true with V" then 
						addVerticalCount(PA["TRI"][period].entry);
					end
				end
			end
		...
		if isEqual(direction[period],1) then
			...
			if 	(isEqual(high[period-4]-high[period-2],low[period-3]-low[period-1])) and isGreater(high[period-4]-high[period-2],0) then
				...
				if PA["FLA"][period].fin then 
					...
					if instance.parameters.enFlag == "true with H and V" then 
						HorizontalCount(PA["FLA"][period].entry);
						addVerticalCount(PA["FLA"][period].entry);
					elseif instance.parameters.enFlag == "true with H" then 
						HorizontalCount(PA["FLA"][period].entry);
					elseif instance.parameters.enFlag == "true with V" then 
						addVerticalCount(PA["FLA"][period].entry);
					end
				end
			end
...
local oTL = {}
oTL[45] = {up={},dn={}}
function findBullishResistanceBearishSupport(type)
	if instance.parameters.bBuRBeS45 == false and type == 45 then return end
	local period = open:size() - 1
	if period < 3 or RS < 1 then return; end
	-- Find new other trendlines
	if PA["SBS"][period] ~= nil then
		if PA["SBS"][period].fin then 
			if PA["SBS"][period].type == "Simple Bull" then
				-- Buy Signal find Bullisch Resitance Line 
				-- Find Wall of O's
				for i = 1 , period , 2 do
					if isGreater(high[period-i]-BS, PA["SBS"][period].entry) and direction[period-i] < 0 then
						local v = mathex.max(high, period - i + 1, period);
						local tbl = {broken = false,sPoint=period-i+1,ePoint=period,brokenvalue = v,isVisable=false,isTouched=false,ad=0}
						if oTL[type].up[tbl.sPoint] == nil then
							oTL[type].up[tbl.sPoint] = tbl;
						end
						break;
					end
				end
			elseif PA["SBS"][period].type == "Simple Bear" then
				-- Sell Signal find Bearisch Support Line 
				-- Find Wall of X's
				for i = 1 , period , 2 do
					if isSmaller(low[period-i]+BS, PA["SBS"][period].entry) and direction[period-i] > 0  then
					--if isGreater(high[period-i]-BS, PA["SBS"][period].entry) and direction[period-i] > 0 then
						local v = mathex.min(low, period - i + 1, period);
						local tbl = {broken = false,sPoint=period-i+1,ePoint=period,brokenvalue = v,isVisable=false,isTouched=false,ad=0}
						if oTL[type].dn[tbl.sPoint] == nil then
							oTL[type].dn[tbl.sPoint] = tbl;
						end
						break;
					end
				end
	...
	LastUVisable[4] = nil;
	for i = 0 , period do
		local val = oTL[type].up[period-i];
		if val ~= nil then
			if val.isTouched == false then
				val.ePoint = period;
				local fromLevel = close[val.sPoint] + BS;
				local toLevel = (fromLevel + ((val.ePoint)-val.sPoint)*BS);-- - adapt;
				if isGreaterOrEqual(high[val.ePoint],toLevel) then
					oTL[type].up[period-i].isTouched = true;
				end
			else
				if val.broken == false then
					val.ePoint = period;
					local fromLevel = close[val.sPoint] + BS;
					local toLevel = (fromLevel + ((val.ePoint-1)-val.sPoint)*BS);-- - adapt;
					if isGreaterOrEqual(low[val.ePoint-1],toLevel) then
						oTL[type].up[period-i].broken=true
						oTL[type].up[period-i].isVisable = true;
					end
				end
			end		
			if LastUVisable[1] == nil and not val.broken then
				LastUVisable[1] = period-i;
				val.isVisable = true;
			elseif LastUVisable[2] == nil and not val.broken then
				LastUVisable[2] = period-i;
				val.isVisable = true;
			elseif LastUVisable[3] == nil and not val.broken then
				LastUVisable[3] = period-i;
				val.isVisable = true;
			elseif LastUVisable[4] == nil and not val.broken then
				LastUVisable[4] = period-i;
				val.isVisable = true;
			elseif not val.broken then
				val.isVisable = false;
			end
		end
	end
	...
	LastDVisable[4] = nil;
	for i = 0 , period do
		local val = oTL[type].dn[period-i];
		if val ~= nil then
			if val.isTouched == false then
				val.ePoint = period;
				local fromLevel = close[val.sPoint] - BS ;
				local toLevel = (fromLevel - ((val.ePoint-1)-val.sPoint)*BS);-- + adapt;
				if isSmallerOrEqual(low[val.ePoint],toLevel) then
					oTL[type].dn[period-i].isTouched = true;
				end
			else
				if val.broken == false then
					val.ePoint = period;
					local fromLevel = close[val.sPoint] - BS;
					local toLevel = (fromLevel - ((val.ePoint-1)-val.sPoint)*BS);-- - adapt;
					if isSmallerOrEqual(high[val.ePoint-1],toLevel) then
						oTL[type].dn[period-i].broken=true
						oTL[type].dn[period-i].isVisable = true;
					end
				end
			end
			if LastDVisable[1] == nil and not val.broken then
				LastDVisable[1] = period-i;
				val.isVisable = true;
			elseif LastDVisable[2] == nil and not val.broken then
				LastDVisable[2] = period-i;
				val.isVisable = true;
			elseif LastDVisable[3] == nil and not val.broken then
				LastDVisable[3] = period-i;
				val.isVisable = true;
			elseif LastDVisable[4] == nil and not val.broken then
				LastDVisable[4] = period-i;
				val.isVisable = true;
			elseif not val.broken then
				val.isVisable = false;
			end
			...
			if TL[type].Confirm == "CO" then
				...
				elseif val.broken and val.confirmed == false then
					if isGreaterOrEqual(low[period],toLevel) and isSmaller(open[period],close[period]) then
						...
						if val == TL[type].lastTrendLine and instance.parameters.enTL ~= "false" and TL[type].On then 
							TL[type].Long[period]=open[period]+(RS-1)*BS; 
							SendAlert(type .. " Trendline break",TL[type].Long[period]);
							if instance.parameters.enTL == "true with H and V" then 
								HorizontalCount(TL[type].Long[period]);
								addVerticalCount(TL[type].Long[period]);
							elseif instance.parameters.enTL == "true with H" then 
								HorizontalCount(TL[type].Long[period]);
							elseif instance.parameters.enTL == "true with V" then 
								addVerticalCount(TL[type].Long[period]);
							end
						end
					end
			...
			if TL[type].Confirm == "CO" then
				...
				elseif val.broken and val.confirmed == false then
					if isSmallerOrEqual(high[period],toLevel) and isGreater(open[period],close[period]) then
						...
						if val == TL[type].lastTrendLine and instance.parameters.enTL ~= "false" then 
							TL[type].Short[period]=open[period]-(RS-1)*BS;
							SendAlert(type .. " Trendline break",TL[type].Short[period]); 
							if instance.parameters.enTL == "true with H and V" then 
								HorizontalCount(TL[type].Short[period]);
								addVerticalCount(TL[type].Short[period]);
							elseif instance.parameters.enTL == "true with H" then 
								HorizontalCount(TL[type].Short[period]);
							elseif instance.parameters.enTL == "true with V" then 
								addVerticalCount(TL[type].Short[period]);
...
function AsyncOperationFinished(cookie, success, message, message1, message2)
    ...
    elseif cookie == 2000 then
        ...
        if message == offer and loading == false then
			...
			local Time ;
			if frame == "t1" then
				-- Tick Source update immediately
				...
				-- no Tick Source Source Timeframe >= m1
				if not EndOfTheDay then
					-- update high and lows immediately , reversals only with new source candle
					Time = core.host:findTable("offers"):find("OfferID", offer).Time;
					if lastSerial == history:serial(i) and lastSerial ~= nil then
						if direction[NOW] > 0 then
							if RS < 2 then
								proccesWyckoff(i,Time,his.high[i],his.high[i]);
							else
								proccesCandle(i,Time,his.high[i],his.high[i]);
							end
						elseif direction[NOW] < 0 then
							if RS < 2 then
								proccesWyckoff(i,Time,his.low[i],his.low[i]);
							else
								proccesCandle(i,Time,his.low[i],his.low[i]);
							end
						elseif isEqual(direction[NOW],0) then
							if RS < 2 then
								proccesWyckoff(i,Time,his.high[i],his.low[i]);
							else
								proccesCandle(i,Time,his.high[i],his.low[i]);
					...
					elseif lastSerial ~= history:serial(i) and lastSerial ~= nil then
						if RS < 2 then
							proccesWyckoff(i,Time,his.high[i],his.low[i]);
						else
							proccesCandle(i,Time,his.high[i],his.low[i]);
					...
					-- update only with new source candle
					if lastSerial ~= history:serial(i) and lastSerial ~= nil then
...
function proccesCandle(period,time,source_high,source_low)
	...
	if updateflag == true then
		...
		findTrendLine(45);
		findTrendLine(22);
		findBullishResistanceBearishSupport(45)
		findDoubleTB();
		findTripleTB();
		findTriangle();
		findCatapult();
...
function drawUsingFont(context, Xw, Xh,cellW,cellH)
	...
	while true do
		...
		local txt = string.format("H-Pips = %.1f  : H-WIN: %i , H-LOST: %i | V-Pips = %.1f  : V-WIN: %i , V-LOST: %i",TA["H_SUM"],Hwins , Hlost,TA["V_SUM"],Vwins , Vlost);
...
-------------------------------------------------------------------------------------------------------------------------------------------------------
function drawOtherTrendLines(context)
		for pos,val in pairs(oTL[45].dn) do
			if val.isVisable == true then 
				local adapt = 0;--val.ad*BS;
				local fromLevel = close[val.sPoint] - BS + adapt;
				local toLevel = (fromLevel - (val.ePoint-val.sPoint)*BS);-- + adapt;
		...
		for pos,val in pairs(oTL[45].up) do
			if val.isVisable == true then  
				local adapt = 0--val.ad*BS;
				local fromLevel = close[val.sPoint] + BS - adapt;
				local toLevel = (fromLevel + (val.ePoint-val.sPoint)*BS);-- - adapt;
