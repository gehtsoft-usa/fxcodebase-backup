...
function Init()
    ...
    indicator.parameters:addDate("from", "Date From", "", -100);
	-- Point and Figure calculation settings
	indicator.parameters:addGroup("PF - Calculation");
	indicator.parameters:addString("method", "Method", "Defines whether fixed BoxSize or ATR","ATR");
	indicator.parameters:addStringAlternative("method", "ATR Method", "", "ATR");
	indicator.parameters:addStringAlternative("method", "PIP Method", "", "PIP");
...
function AsyncOperationFinished(cookie, success, message, message1, message2)
    if cookie == 999 then
		...
		BS = math.floor(ATR.DATA[historyD1:size()-2]*PP/point)*point;
		lastSerialD1 = historyD1:serial(NOW);
		loading 	= true;
		history 	= core.host:execute("getHistory", 1000, instrument, frame, math.floor(instance.parameters.from), 0, bid);
