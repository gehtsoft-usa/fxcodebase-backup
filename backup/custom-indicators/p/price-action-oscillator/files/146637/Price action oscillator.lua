...
-- https://fxcodebase.com/code/viewtopic.php?f=17&t=72466
...
function Init()
    indicator:name("Price action oscillator");
    ...
    indicator.parameters:addInteger("CalcPeriod", "Period", "", 50, 1, 2000);
...
-- Routine
 function Prepare(nameOnly)   
	...
	CalcPeriod=instance.parameters.CalcPeriod;
	Method=instance.parameters.Method;
	source = instance.source
 
    local name = profile:id() .. "(" ..  instance.source:name().. "," ..  CalcPeriod.. "," ..  Method  .. ")";
    instance:name(name); 
	...
	Indicator= core.indicators:create(Method, source.close, CalcPeriod );
...
function Update(period, mode)
	...
	Line[period] = source.close[period]+source.open[period]-2*Indicator.DATA[period]; 
 
	
	Signal[period]= source.open[period]-Indicator.DATA[period];
	
   if Line[period] > Signal[period] then	
