...
function Init()
	indicator:name("Previous Period_Tails")
	indicator:description("Previous Period_Tails")
...
function AddTimeFrame(id, FRAME, DEFAULT)
	...
	indicator.parameters:addColor("Color" .. id, "Color", "", core.rgb(0 + (255 / 13) * id, 255 - (255 / 13) * id, 0))
end
...
function Prepare(nameOnly)
	...
	for i = 1, 13, 1 do
		...
		if Use[i] then
			Num = Num + 1
			Color[Num] = instance.parameters:getColor("Color" .. i)
			TF[Num] = iTF[i]
		end
	end
...
function Draw(stage, context)
	...
	if not init then
		for i = 1, Num, 1 do
			context:createPen(1, context.SOLID, 3, Color[i])
			context:createSolidBrush(2, Color[i])
		end
