...
-- https://fxcodebase.com/code/viewtopic.php?f=17&t=73009
...
function Init()
    indicator:name("PopGun Indicator");
    ...
    indicator.parameters:addInteger("Percentrange", "Percentrange", "", 25);	
...
-- Routine
 function Prepare(nameOnly)   
	...
	Percentrange=instance.parameters.Percentrange;
...
function Update(period, mode)
	...
	Support = (source.high[period]-source.low[period])*Percentrange/100
	Resistance = (source.high[period]-source.low[period])*Percentrange/100	
	
     
     if source.high[period-2]>source.high[period-1] 
	 and source.low[period-2]<source.low[period-1] 
	 ...
	 if PG and source.close[period] >source.open[period] then
	 PGLong=true;
	 else
	 PGLong=false;
	 end
