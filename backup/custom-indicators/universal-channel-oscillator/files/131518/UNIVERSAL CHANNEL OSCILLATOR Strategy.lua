...
function Prepare(nameOnly)
    ...
    assert(
        core.indicators:findIndicator("UNIVERSAL CHANNEL OSCILLATOR") ~= nil,
        "Please, download and install UNIVERSAL CHANNEL OSCILLATOR.LUA indicator"
    )
...
function ExitFunction(id, now, period)
    ...
    if core.crosses(Indicator.Buffer4, Indicator.UniAvgline, period) then
        if Direction then
...
function EntryFunction(id, now, period)
    ...
    if
        Indicator.Buffer4[period] > Indicator.UniAvgline[period] and
            Indicator.Buffer4[period - 1] <= Indicator.UniAvgline[period - 1]
     then
        if Direction then
            BUY()
        else
