...
-- https://fxcodebase.com/code/viewtopic.php?f=17&t=72179
...
function Init()
    ...
    indicator.parameters:addInteger("Period2", "2. Period", "", 12, 1, 2000);
    indicator.parameters:addInteger("Period3", "Signal Period", "", 5, 1, 2000);	
...
-- Routine
 function Prepare(nameOnly)   
	...
	DownTicks = instance:addInternalStream(0, 0); 
	
	
	EMA1= core.indicators:create("EMA", UpTicks, Period1);
	EMA2= core.indicators:create("EMA", DownTicks, Period1);	
	EMA3= core.indicators:create("EMA", EMA1.DATA, Period2);
	EMA4= core.indicators:create("EMA", EMA2.DATA, Period2);	 
	
    Line1 = instance:addStream("Line1", core.Line, name, "1. Line", instance.parameters.color1, first +Period1 +Period2 );
	...
	EMA5= core.indicators:create("EMA", Line1, Period3);	

	
    Line2 = instance:addStream("Line2", core.Line, name, "2. Line", instance.parameters.color2, first +Period1 +Period2+Period3 );
...
function Update(period, mode)
	...
	Line1[period]=100.0*(EMA3.DATA[period]-EMA4.DATA[period])/(EMA3.DATA[period]+EMA4.DATA[period]); 
	
	EMA5:update(mode); 
	
	if period <= EMA5.DATA:first() then
	return;
	end


	Line2[period]=EMA5.DATA[period]; 
end