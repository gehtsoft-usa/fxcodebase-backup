...
function Init()
    indicator:name("Current Trend Exhaustion");
...
-- Routine
 function Prepare(nameOnly)   
	...
	Line2 = instance:addStream("Line2" , core.Line, " Line2"," Line2",instance.parameters.color2, first+Length );
...
function Update(period, mode)
	...
	local CountLow=mathex.sum(Low,period-Length+1, period );	
	local CountUp=mathex.sum(Up,period-Length+1, period );
	local CountDown=mathex.sum(Down,period-Length+1, period );	
    ...
    Line1[period]= CountHigh/CountUp; 
 
	
    Line2[period]=CountLow/CountDown;
