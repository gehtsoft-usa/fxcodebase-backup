...
-- http://fxcodebase.com/code/viewtopic.php?f=17&t=68422
...
function Init()
    indicator:name("Other Timeframe PMO")
...
function Prepare(nameOnly)
    ...
    Indicator = core.indicators:create("PMO", btf_source.close, one, two, Period)
...
function Update(period)
    ...
    if (period == first or prev_period == -1) then
        ...
        position[period] = position[prev_period]
    end
    if position[period] ~= -1 and core.crossesUnder(Indicator.PMO, Indicator.SIGNAL, period) then
        position[period] = -1
    elseif position[period] ~= 1 and core.crossesOver(Indicator.PMO, Indicator.SIGNAL, period) then
        position[period] = 1
    end
    ...
    if p ~= 0 then
        ...
        if position[p] == 1 then
            ...
            --Stop Line
            core.host:execute("drawLine", New(p, 2), source:date(p), signalVal, source:date(period), signalVal, StopLineColor, style2, width2, win32.formatNumber(signalVal, false, source:getPrecision()))
            ...
            if LabelBackground then
                ...
                label2.Text = string.format("Stop : %s (%s)", win32.formatNumber(signalVal, false, source:getPrecision()), win32.formatNumber(math.abs(signalVal - source.low[p]) / source:pipSize(), false, 1))
            ...
            elseif ShowLabel ~= "no" then
                ...
                local label2 = string.format("Stop : %s (%s)", win32.formatNumber(signalVal, false, source:getPrecision()), win32.formatNumber(math.abs(signalVal - source.low[p]) / source:pipSize(), false, 1))
                ...
                core.host:execute("drawLabel1", New(p, 8), date, core.CR_CHART, signalVal, core.CR_CHART, h_pos, core.V_Top, font, LabelColor, label2)
            ...
            --Stop Line
            core.host:execute("drawLine", New(p, 14), source:date(p), signalVal, source:date(period), signalVal, StopLineColor, style2, width2, win32.formatNumber(signalVal, false, source:getPrecision()))
            ...
            elseif ShowLabel ~= "no" then
                ...
                core.host:execute("drawLabel1", New(p, 20), date, core.CR_CHART, signalVal, core.CR_CHART, h_pos, core.V_Top, font, LabelColor, label2)
