...
function Prepare(nameOnly)
	...
	_period = instance.parameters.Period;
...
function Calculate(i, j)
	...
	local Num = 0
	local Sum = 0
	for ii = 0, math.min(Source[i][j]:size() - 1, _period - 1) do
		Sum = Sum + Source[i][j].high[NOW - ii] - Source[i][j].low[NOW - ii];
		Num = Num + 1;
	end
	if Num == 0 then
		return;
	end
	Value[i][j] = (Sum / Num) / Source[i][j]:pipSize();
end
