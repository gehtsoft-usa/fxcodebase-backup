...
function Init()
    ...
    indicator:description("Minor cycle index");
    ...
    indicator.parameters:addInteger("SummarizingPeriod", "Summarizing Period", "Summarizing Period", 50);
	 ...
	 indicator.parameters:addInteger("Short", "Short MA Period", "Short MA Period", 50); 
	 indicator.parameters:addInteger("Long", "Long MA Period", "Long MA Period", 100);
...
--[[
igom:=FmlVar(�_sac-sigom�,�sigom�);
yi:=100*(Ref(Mov(C,50,S),25)-Ref(Mov(C,100,S),50))/
Ref(Mov(C,100,S),50);
yin:=100*yi/sigom; yin;
