...
-- https://fxcodebase.com/code/viewtopic.php?f=17&t=3880
...
function Init()
    indicator:name("MTF MCP iTrend Heat Map");
 ...
 function Prepare(nameOnly)   
   ...
   instance:ownerDrawn(true);
    ...
    Power_Mode = instance.parameters.Power_Mode;
				 ...
				 loading[Number]  = true;  	 
				 Indicator[Number] = core.indicators:create("ITREND", SourceData[Number] ,  	Price_Mode,Bollinger_Mode,Bollinger_Price,	Bollinger_Period,     Bollinger_Deviation,     Power_Price, Power_Period     ,     Power_Mode  );   
...
function Draw (stage, context)
   ...
   context:setClipRectangle(context:left(), context:top(), context:right(), context:bottom());
        ...
        if not init then
			...
			-- context:createPen (13, context.SOLID, 1, OB)   	
			-- context:createPen (14, context.SOLID, 1, OS) 			 
            init = true;
        end
			   ...
			   x0, x1, x2 = context:positionOfBar (i);
			    ...
			    for j= 1, Number , 1 do
				  ...
				  if p~= false then
										...
										if Indicator[j].Power:hasData(p) and Indicator[j].Power:hasData(p-1) 
										and Indicator[j].Bollinger:hasData(p) and Indicator[j].Bollinger:hasData(p-1) 
										then 
										
												    if Indicator[j].Bollinger[p] > Indicator[j].Power[p] then		 
															if Indicator[j].Bollinger[p] > Indicator[j].Bollinger[p-1] then	
															C2=12;
															C1=11;
															else
															C2=22;
												    ...
												    elseif Indicator[j].Power[p] > Indicator[j].Bollinger[p] then  
													        if Indicator[j].Power[p] > Indicator[j].Power[p-1] then	
															C2=32;
															C1=31;
															else
															C2=42;
