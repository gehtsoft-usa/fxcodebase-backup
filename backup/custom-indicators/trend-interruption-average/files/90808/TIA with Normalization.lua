...
function Init()
 ...
 indicator.parameters:addStringAlternative("Method", "LWMA", "LWMA" , "LWMA");
   ...
   indicator.parameters:addInteger("style3", "Line style", "", core.LINE_SOLID);
	...
	indicator.parameters:addString("Type", " DifferenceType", "", "Line");
...
function Prepare(nameOnly)
	...
	down = core.indicators:create(Method, d, Period);
	 ...
	 first =NormalizationPeriod + math.max(up.DATA:first(), down.DATA:first());
	 
	 
	 Up_Raw = instance:addInternalStream(0, 0);
	 Down_Raw = instance:addInternalStream(0, 0);
	 Difference_Raw = instance:addInternalStream(0, 0);
    ...
    Up:setPrecision(math.max(2, instance.source:getPrecision()));
		...
		if L2 then
        Down = instance:addStream("Dowm", core.Line, name .. ".Down", "Down", instance.parameters.Dowm_color, first);
...
function Update(period, mode)
	...
	return;
        ...
        Up_Raw[period] = up.DATA[period];
        Down_Raw[period] = down.DATA[period];		
        Difference_Raw[period] = Up_Raw[period]-Down_Raw[period] ;
		
		
		if period < first+NormalizationPeriod then
	return;
	end
		...
		min1,max1=mathex.minmax(source,period-NormalizationPeriod+1, period);
		min2,max2=mathex.minmax(Up_Raw,period-NormalizationPeriod+1, period);
		min3,max3=mathex.minmax(Down_Raw,period-NormalizationPeriod+1, period);
		min4,max4=mathex.minmax(Difference_Raw,period-NormalizationPeriod+1, period);
		
	 
        Up[period] =  ((Up_Raw[period] - min2)/ (max2-min2))*(max1-min1) +min1  ;
        Down[period]=  ((Down_Raw[period] - min3)/ (max3-min3))*(max1-min1) +min1 ;
        Difference[period]=  ((Difference_Raw[period] - min4)/ (max4-min4))*(max1-min1) +min1 ;
end
