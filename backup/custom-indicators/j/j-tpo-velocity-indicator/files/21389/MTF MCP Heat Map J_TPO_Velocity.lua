...
-- https://fxcodebase.com/code/viewtopic.php?f=17&t=10251
...
function Init()
    indicator:name("MTF MCP Heat Map J_TPO_Velocity");
    ...
    indicator.parameters:addColor("Color1", "NE Color", "NE Color", core.rgb(128, 128, 128));
    indicator.parameters:addColor("Color2", "UP UP Color", "UP UP Color", core.rgb(0, 255, 0));
    indicator.parameters:addColor("Color3", "UP DN Color", "UP DN Color", core.rgb(0, 128, 0));
    indicator.parameters:addColor("Color4", "DN UP Color", "DN UP Color", core.rgb(128, 0, 0));
    indicator.parameters:addColor("Color5", "DN DN Color", "DN DN Color", core.rgb(255, 0, 0));
...
local Indicator = {} ;
 ...
 function Prepare(nameOnly) 
	...
	assert(core.indicators:findIndicator("J_TPO_VELOCITY") ~= nil, "Please, download and install J_TPO_VELOCITY.LUA indicator");    
	...
	s2, e2 = core.getcandle(iTF[i], 0, 0, 0);
	 ...
	 Label[Number]="";
                ...
                Label[Number]=Label[Number] .. " - " ..  TF[Number];				
			  	...
			  	Temp1= core.indicators:create("J_TPO_VELOCITY", source ,  Period[Number]);  				
				ifirst= Temp1.DATA:first()*2;
				 ...
				 loading[Number]  = true;  	 
				 Indicator[Number] = core.indicators:create("J_TPO_VELOCITY", SourceData[Number] ,  Period[Number]);   
...
function Draw (stage, context)
   ...
   context:setClipRectangle(context:left(), context:top(), context:right(), context:bottom());
        ...
        if not init then
			...
			context:createPen (1, context.SOLID, 3, instance.parameters.Color1)       
			context:createSolidBrush(2, instance.parameters.Color1);
			
			context:createPen (11, context.SOLID, 3, instance.parameters.Color2)       
			context:createSolidBrush(12, instance.parameters.Color2);
			
			context:createPen (21, context.SOLID, 3, instance.parameters.Color3)       
			context:createSolidBrush(22, instance.parameters.Color3);
		
			
			context:createPen (31, context.SOLID, 3, instance.parameters.Color4)       
			context:createSolidBrush(32, instance.parameters.Color4);
			
			context:createPen (41, context.SOLID, 3, instance.parameters.Color5)       
			context:createSolidBrush(42, instance.parameters.Color5);
			    ...
			    for j= 1, Number , 1 do
				  ...
				  if p~= false then
										...
										if Indicator[j].DATA:hasData(p) and  Indicator[j].DATA:hasData(p-1)   then 
												    ...
												    if Indicator[j].DATA[p] > 0 then	
                                                         if Indicator[j].DATA[p] > Indicator[j].DATA[p-1] then
														 C1=11; C2=12;	
														 else
														 C1=21; C2=22;	
														 end
											        elseif Indicator[j].DATA[p] < 0 then	
   													     if Indicator[j].DATA[p] > Indicator[j].DATA[p-1] then 
														 C1=31; C2=32;	
														 else
														 C1=41; C2=42;	
