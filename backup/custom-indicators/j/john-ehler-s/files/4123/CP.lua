...
function Init()
    indicator:name("Cycle Period");
    indicator:description("Cycle Period");
...
function Prepare()
	...
	Frame:setWidth(instance.parameters.width);
    Frame:setStyle(instance.parameters.style);
end

-- Indicator calculation routine
-- TODO: Add your code for calculation output values
function Update(period)
    if period >= 7 and source:hasData(period) then
	
	--Detrend Price
	Value3[period] = source.median[period] - source.median[period-7];
	--Frame[period]=0;
	--InPhase[period]=0;
	--Quadrature[period]=0;
