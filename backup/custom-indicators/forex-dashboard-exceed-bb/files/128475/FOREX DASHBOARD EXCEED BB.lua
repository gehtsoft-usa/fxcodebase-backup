...
-- http://fxcodebase.com/code/viewtopic.php?f=17&t=68490
...
local indi_name = "FOREX DASHBOARD EXCEED BB";
local indi_id = "BB";
...
function CreateIndicators(source)
    ...
    p:setInteger("Period", instance.parameters.Period);
    p:setDouble("Deviation", instance.parameters.Deviation);
...
function CreateParameters()
	...
	indicator.parameters:addDouble("Deviation", "Number of Standard Deviation", "Number of Standard Deviation", 2);
end

function GetLastSignal(indi, source)
    if core.crossesOver(source.close, indi[1].TL, indi[1].TL:size() - 1) then
        return 1, source:date(NOW), "B";
    end
    if core.crossesUnder(source.close, indi[1].BL, indi[1].BL:size() - 1) then
        return -1, source:date(NOW), "S";
    end
    return 0, nil, "-";
end
