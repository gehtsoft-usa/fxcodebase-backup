...
function Update(period, mode)
    ...
    if period == source:size() - 1 and last_signal ~= source:serial(period) then
		if core.crossesOver(macd_ma.DATA, signal_ma.DATA, period) then
			signaler:Signal("MACD crossed over Signal");
			last_signal = source:serial(period);
		elseif core.crossesUnder(macd_ma.DATA, signal_ma.DATA, period) then
			signaler:Signal("MACD crossed under Signal");
			last_signal = source:serial(period);
		end
	end
end
