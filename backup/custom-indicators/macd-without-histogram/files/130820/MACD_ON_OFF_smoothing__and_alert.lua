...
local macd_ma, signal_ma, macd_ma_out, signal_ma_out;
...
{ 
    {
        ...
        UpCondition = function (period)
            if period == 0 or not macd_ma.DATA:hasData(period - 1) or not signal_ma.DATA:hasData(period - 1) then
                return false;
            end
            return core.crossesOver(macd_ma.DATA, signal_ma.DATA, period);
        end,
        DownCondition = function (period)
            if period == 0 or not macd_ma.DATA:hasData(period - 1) or not signal_ma.DATA:hasData(period - 1) then
                return false;
            end
            return core.crossesUnder(macd_ma.DATA, signal_ma.DATA, period);
        end,
...
function Init()
    ...
    indicator.parameters:addInteger("macd_period", "MACD MA Period", "", 14);
    AddAverages("macd_method", "MACD MA Method", "MVA");

    indicator.parameters:addInteger("signal_period", "Signal MA Period", "", 14);
    AddAverages("signal_method", "Signal MA Method", "MVA");
	...
	indicator.parameters:addColor("MACD_ma_color", "MACD MA color", "", core.rgb(255, 0, 0))
	indicator.parameters:addColor("SIGNAL_ma_color", "Signal MA color", "", core.rgb(0, 0, 255))
...
function Prepare(nameOnly)
    ...
    signal_ma_out = instance:addStream("SignalMA", core.Line, name .. ".Signal MA", "Signal MA", instance.parameters.SIGNAL_ma_color, 0)
    signal_ma_out:setPrecision(math.max(2, instance.source:getPrecision()))
    macd_ma_out = instance:addStream("macdMA", core.Line, name .. ".MACD MA", "MACD MA", instance.parameters.MACD_ma_color, 0)
    macd_ma_out:setPrecision(math.max(2, instance.source:getPrecision()))
    ...
    macd_ma = CreateAverages(instance.parameters.macd_period, instance.parameters.macd_method, OUT);
    signal_ma = CreateAverages(instance.parameters.signal_period, instance.parameters.signal_method, SIGNALOUT);
end
...
function Update(period, mode)
    ...
    macd_ma_out[period] = macd_ma.DATA[period];
    signal_ma_out[period] = signal_ma.DATA[period];
