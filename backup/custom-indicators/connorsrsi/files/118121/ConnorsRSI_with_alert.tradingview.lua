...
-- Routine
 function Prepare(nameOnly)   
    ...
    RSI3 = instance:addInternalStream(first1, 0);
    first=math.max(RSI1.DATA:first(), RSI2.DATA:first(), LenRank + 1)
    crsi = instance:addStream("crsi", core.Line, name, "crsi", instance.parameters.crsi_color, first);
    roc = instance:addInternalStream(0, 0);
	--= instance:addStream("roc", core.Line, name, "roc", instance.parameters.crsi_color, first);
...
function Update(period,mode)
    ...
    RSI3[period] = (source.close[period] / source.close[period - 1] - 1) * 100.0;
    roc[period] = RSI3[period];
    
    crsi[period] = (RSI1.DATA[period] + RSI2.DATA[period] + PercentRank(RSI3, LenRank, period)) / 3;
