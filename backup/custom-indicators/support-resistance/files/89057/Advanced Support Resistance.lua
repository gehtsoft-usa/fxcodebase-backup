...
function Draw(stage, context)
	...
	for period = Start - 1, source:size() - 1, 1 do
		...
		if Find ~= "Support" and max ~= nil then
			visible1, y1 = context:pointOfPrice(source.high[period] + SIZE)
			visible2, y2 = context:pointOfPrice(source.high[period] - SIZE)
			visible3, y3 = context:pointOfPrice(source.high[period])
			...
			context:drawLine(5, x1, y3, x2, y3, 0)

			if x2 >= LastValid then
				Index = Index + 1
				Top[Index] = source.high[period] + SIZE
				Bottom[Index] = source.high[period] - SIZE
			end
		end

		if Find ~= "Resistance" and min ~= nil then
			visible1, y1 = context:pointOfPrice(source.low[period] + SIZE)
			visible2, y2 = context:pointOfPrice(source.low[period] - SIZE)
			visible3, y3 = context:pointOfPrice(source.low[period])
			...
			if x2 >= LastValid then
				Index = Index + 1
				Top[Index] = source.low[period] + SIZE
				Bottom[Index] = source.low[period] - SIZE
			end
		end
	end
	...
	if flag then
		for i = period + 1, ref_period, 1 do
			if Price[i] > Price[period] then
				...
				RETURN = i
				Number = Number + 1
				if Price[i] >= source.high[period] - SIZE and Price[i] <= source.high[period] + SIZE then
					Q = Q + 1
				end
			end
		end
	...
	if not flag then
		for i = period + 1, ref_period, 1 do
			if Price[i] < source.low[period] then
				...
				RETURN = i
				Number = Number + 1
				if Price[i] >= source.low[period] - SIZE and Price[i] <= source.low[period] + SIZE then
					Q = Q + 1
				end
			end
		end
