...
-- http://fxcodebase.com/code/viewtopic.php?f=17&t=71607
...
local source, bb, macd, ma1, ma2, cci, dmi, rsi, ssd;
...
{ 
    {
        ...
        UpCondition = function (period)
            local count = 0;
            if source.close[period] > bb.AL[period] then
                count = count + 1;
            end
            ...
            if rsi.DATA[period] > 0 then
                count = count + 1;
            end
            if ma1.DATA[period] > ma2.DATA[period] then
                count = count + 1;
            end
            if ssd.D[period] > 50 then
                count = count + 1;
            end
            return count >= 5;
        end,
...
function Init()
    indicator:name("6 Indicator Condluence");
    indicator:description("");
    indicator:requiredSource(core.Bar);
    indicator:type(core.Indicator);
    
    indicator.parameters:addInteger("bb_period", "BB period", "", 20);
    indicator.parameters:addDouble("bb_deviations", "BB deviations", "", 2.0);
    ...
    indicator.parameters:addInteger("ma1_period", "MA 1 Period", "", 14);
    AddAverages("ma1_method", "MA 1 Method", "MVA");

    indicator.parameters:addInteger("ma2_period", "MA 2 Period", "", 21);
	...
	indicator.parameters:addInteger("ssd_K", "Number of periods for %K", "", 5, 2, 1000);
    indicator.parameters:addInteger("ssd_SD", "%D slowing periods", "", 3, 2, 1000);
    indicator.parameters:addInteger("ssd_D", "The number of periods for %D.", "", 3, 2, 1000);
    ...
    indicator.parameters:addColor("entry_color", "Entry Color", "Color", core.colors().Red);
    indicator.parameters:addInteger("entry_width", "Entry Width", "Width", 1, 1, 5);
    indicator.parameters:addInteger("entry_style", "Entry Style", "Style", core.LINE_SOLID);
    indicator.parameters:setFlag("entry_style", core.FLAG_LINE_STYLE);
end
...
function Prepare(nameOnly)
    ...
    macd = core.indicators:create("MACD", source, instance.parameters.fast, instance.parameters.slow, instance.parameters.signal_smoothing);
    ...
    dmi = core.indicators:create("DMI", source, instance.parameters.dmi_period);
...
function FindLastSignal()
    ...
    for i = source:size() - 1, 0, -1 do
        for idx, level in ipairs(indi_alerts.Alerts) do
            if level.Alert:hasData(i) then
                if last == nil or last == level.Alert[i] then
                    last = level.Alert[i];
                    index = i;
                else
                    return index;
...
function Draw(stage, context) 
    ...
    if LINE_ID == nil then
        LINE_ID = indi_alerts.NextId + 1;
        context:createPen(LINE_ID, context:convertPenStyle(instance.parameters.entry_style), instance.parameters.entry_width, instance.parameters.entry_color);
    end
    local period = FindLastSignal();
    if period == nil then
        return;
    end
    local x_center, x_left, x_right = context:positionOfBar(period);
    local _, y = context:pointOfPrice(source.close[period]);
    context:drawLine(LINE_ID, x_center, y, context:right(), y);
end
