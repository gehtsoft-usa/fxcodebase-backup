...
function Init()
	...
	indicator.parameters:addString("ShowX", "Show", "", "B");
	indicator.parameters:addStringAlternative("ShowX", "Outer", "", "O");
    indicator.parameters:addStringAlternative("ShowX", "Inner", "", "I");
    indicator.parameters:addStringAlternative("ShowX", "Both", "", "B");
	  ...
	  return;
		...
		for period= math.max(context:firstBar (),source:first()), math.min( context:lastBar (), source:size()-1), 1 do
		 ...
		 for Level = 1 , Number ,  1 do
		   if TrendDirection:hasData(period) then
...
function Activate (id, period)
	  ...
	  if id == 1  and ON[id]  then
			...
			if  TrendDirection[period]==1 
			and  TrendDirection[period-1]~=1 
			then
			           
						    
         
               
 				AlertLevel[id][period]=  Inner[period];
			...
			elseif  TrendDirection[period]==-1 
			and  TrendDirection[period-1]~=-1 
            then			
