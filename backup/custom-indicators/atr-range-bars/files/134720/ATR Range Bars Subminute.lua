...
function Init()
    ...
    indicator.parameters:addInteger("atr_frame", "Number Of seconds", "", 10)
    ...
    indicator.parameters:addBoolean("use_ma_atr", "Use MA/ATR Rule", "", true);
    
    indicator.parameters:addGroup("Range")
    indicator.parameters:addDate("from_atr", "ATR Date From", "", -1000)
...
function Prepare(onlyName)
    ...
    loading = true
    local profile = core.indicators:findIndicator("SUB MINUTE CANDLE VIEW");
    assert(profile ~= nil, "Please, download and install " .. "SUB MINUTE CANDLE VIEW" .. ".LUA indicator");
    local indicatorParams = profile:parameters();
    indicatorParams:setInteger("Step", instance.parameters.atr_frame);
    indicatorParams:setString("Instrument", instrument);
    indicatorParams:setDate("from", core.now() - 5000);
    indi = core.indicators:create("SUB MINUTE CANDLE VIEW", source, indicatorParams)
    history = indi:getCandleOutput(0)
    atr_source = indi:getCandleOutput(0);
...
function handleHistory()
    ...
    for i = 1, s, 1 do
        ...
        if index >= 0 then
            local threshold = atr.DATA[index];
            if use_ma_atr then
                threshold = math.min(atr.DATA[index], ma.DATA[index]);
            end
            if high[current] - low[current] >= threshold then
                current = current + 1
...
function handleUpdate()
    if not loading and history:size() > 0 then
        ...
        local index = core.findDate(ma.DATA, source:date(i), false);
        if index >= 0 then
            local threshold = math.max(getTrueRange(index), atr.DATA[index]);
            if use_ma_atr then
                threshold = math.min(atr.DATA[index], ma.DATA[index]);
            end
            if high[current] - low[current] >= threshold then
