...
local Alert_Name={ "Top Line" ,"Top Line" ,  "Bottom Line" ,  "Bottom Line" };
...
function Init()
    indicator:name("TMA Band MA Cross Alert");
    ...
    indicator.parameters:addColor("color", "Color", "Color", core.colors().Gray);
...
local source, middle, lower, upper, HalfLength, atr, AtrMultiplier,Period;
function Prepare(nameOnly)
    ...
    line = instance:addStream("line", core.Line, "line", "line", instance.parameters.color, 0);
...
function Signal_Logic (id, period)
	  ...
	  --  if not Show_Unconfirmed then
	   -- Alert[id][period]= 0;	
	    ...
	    if id== 1  then   
			if  line[period] > upper[period] 
			and   line[period-1] <= upper[period-1] 
			then
			           
						    
         
                Alert[id][period]= 1;	
 				AlertLevel[id][period]= upper[period] 
				Alignment[id][period]= -1;		   
					
						   							  
			elseif  line[period] < upper[period] 
			and   line[period-1] >= upper[period-1] 
            then			
	    ...
	    --id 1 will define Cross Over Alert
	    if id== 3  then   
			if  line[period] > lower[period] 
			and   line[period-1] <= lower[period-1] 
			then
			           
						    
         
                Alert[id][period]= 1;	
 				AlertLevel[id][period]= lower[period] 
				Alignment[id][period]= -1;		   
					
						   							  
			elseif  line[period] < lower[period] 
			and   line[period-1] >= lower[period-1] 
            then			
			--We will reset CrossOver Alert 
