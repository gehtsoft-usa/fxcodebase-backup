...
function Init()
    indicator:name("Donchian Bias Screener");
...
-- Routine
 function Prepare(nameOnly)   
	...
	MVA = core.indicators:create("MVA", source.close, Period2);	
    first=math.max(MVA.DATA:first(),Period1);	
...
function Draw(stage, context)
	  ...
	  return;
        ...
        if not init then
           context:createFont (1, "Wingdings", context:pointsToPixels (Size*3), context:pointsToPixels (Size*3), 0);
	...
	if  source.close[period-1]> Max[period-1]
	then
	Color=Up;
	Trend=Trend.. " Up "
	TrendSign="\233"	
	elseif source.close[period-1]< Min[period-1]	
	then
	Color=Down;
	TrendSign="\234"	
	Trend=Trend.. " Down "	
    ...
    if  MVA.DATA[period-1] >  Max[period-1] then 
	Trend=Trend.. " Overbought" 	
	elseif MVA.DATA[period-1] <  Min[period-1] then
   ...
   context:drawText (1,   TrendSign, Color, -1,   context:right()-100-width2 ,  context:top() +100+height1*ShiftY +height1  ,  context:right()-100,  context:top()+100+height1*ShiftY +height1 +height2 , 0 );	
   
   
   local Percentage= (source.close[period] - Min[period-1])/((Max[period-1]-Min[period-1])/100)
   Percentage = win32.formatNumber(Percentage, false, 0);
   Percentage=Percentage.. " %"
	
   width3, height3 = context:measureText (2, Percentage, 0);
   context:drawText (2,   Percentage, Color, -1,   context:right()-100-width3 , context:top()+100+height1*ShiftY +height1 +height2 ,  context:right()-100,   context:top()+100+height1*ShiftY +height1 +height2 + height3 , 0 );	 
end		
