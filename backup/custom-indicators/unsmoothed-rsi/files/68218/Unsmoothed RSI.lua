...
function Prepare(nameOnly)
    ...
    first= source:first()+RP-1;
	 
    local name = profile:id() .. "(" .. source:name() .. ", " .. RP   .. ")";
    instance:name(name);
 ...
 function Calculate(period )
	...
	local i = 0;
		...
		local P,N;
		P= mathex.sum(pos, period-RP+1, period);
        N= mathex.sum(neg, period-RP+1, period); 
        if (N == 0) then
            RSI[period] = 0;
        else
            RSI[period] = 100 - (100 / (1 + P / N));
        end
