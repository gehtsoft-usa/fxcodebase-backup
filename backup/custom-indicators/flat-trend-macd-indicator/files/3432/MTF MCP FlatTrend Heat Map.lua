...
--+------------------------------------------------------------------+
 ...
 function Add(id, TF,Flag, Instrument )
	...
	indicator.parameters:addInteger("MACD_Fast".. id, "MACD_Fast", "Period for fast MACD", 12);
    indicator.parameters:addInteger("MACD_Slow".. id, "MACD_Slow", "Period for slow MACD", 26);
    indicator.parameters:addInteger("MACD_MA".. id, "MACD_MA", "Period for signal MACD", 9);
    
end


function Init()
    indicator:name("MTF MCP FlatTrend Heat Map");
	...
	indicator.parameters:addColor("NeutralUp", "Up in Neutral Trend Color","", core.rgb(0, 0, 255));
	indicator.parameters:addColor("NeutralDown", "Down in Neutral Trend Color","", core.rgb(0, 0, 200));
   ...
   Last=nil;
	...
	s2, e2 = core.getcandle(iTF[i], 0, 0, 0);
	 ...
	 Number=Number+1;
		  ...
		  MACD_Fast[Number]= instance.parameters:getInteger("MACD_Fast" .. i);
          MACD_Slow[Number]= instance.parameters:getInteger("MACD_Slow" .. i);
          MACD_MA[Number]= instance.parameters:getInteger("MACD_MA" .. i);
	 ...
	 Label[Number]="";
			...
			--	end
                ...
                Label[Number]=Label[Number] .. " - " ..  TF[Number];				
			  	...
			  	Temp1= core.indicators:create("MACD", source.close , MACD_Fast[Number], MACD_Slow[Number],  MACD_MA[Number]);  				
				ifirst= Temp1.SIGNAL:first();
			
				
				   Id=Id+1;
				 SourceData[Number]  = core.host:execute("getSyncHistory",  Instrument[Number],  TF[Number], source:isBid(),math.min(ifirst*2,300), 2000 + Id , 1000 +Id);	 	 
				 loading[Number]  = true;  	 
				 Indicator[Number] = core.indicators:create("MACD", SourceData[Number].close ,   MACD_Fast[Number], MACD_Slow[Number],  MACD_MA[Number]);   
...
function Draw (stage, context)
   ...
   context:setClipRectangle(context:left(), context:top(), context:right(), context:bottom());
        ...
        if not init then
			...
			context:createPen (61, context.SOLID, 1, NeutralDown)       
			    ...
			    for j= 1, Number , 1 do
				  ...
				  if p~= false then
										...
										if Indicator[j].SIGNAL:hasData(p) and Indicator[j].SIGNAL:hasData(p-1) then 
												    ...
												    if Indicator[j].MACD[p] > Indicator[j].SIGNAL[p] and  Indicator[j].MACD[p] > 0  then		 
												    ...
												    elseif Indicator[j].MACD[p] < Indicator[j].SIGNAL[p] and  Indicator[j].MACD[p] < 0  then	  
													        if Indicator[j].DATA[p] > Indicator[j].DATA[p-1] then	
															C2=32;
															C1=31;
															else
