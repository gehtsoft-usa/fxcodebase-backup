...
function Prepare()
    ...
    if instance.parameters.ShowProject then 
        ...
        WPT = instance:addStream("WPT", core.Line, name .. ".WPT", "WPT", instance.parameters.clrWPL, 0, PX);
...
function Update(period, mode)
    ...
    if (period == source:size() - 1) and (mode ~= core.UpdateLast) then
        if instance.parameters.ShowWaveNo then 
            ...
            for iperiod = winfo.prevwfirst+1, period do
                if EWN:hasData(iperiod) and ZZW[iperiod]>0 then
        ...
        if instance.parameters.ShowProject then
            for i = 1, period+PX do
                if PPJ[i]>0 and i>forward[nfor-1]
                and (PPJ[i]-ZZW[forward[nfor-1]])*(ZZW[forward[nfor]]-ZZW[forward[nfor-1]])>0 
            ...
            rangeT = core.range(forward[nfor-1], proj.expos);
            core.drawLine(WPT, rangeT, ZZW[forward[nfor-1]], forward[nfor-1], PPJ[proj.expos], proj.expos);
        end     --end of project
...
function GetEwn(ZZP, forward, ntolw, nforw, nupdown, sunit, period)
    ...
    for i = 1, nforw do     
        ...
        elseif math.fmod(i, 2) == 0 and math.fmod(num, 2) == 1 then
            if num == 1 then
                if WaveLength(ZZP,sunit,forward[i-1],forward[i]) < WaveLength(ZZP,sunit,forward[i-2],forward[i-1])*0.91 then
                    wnp[num] = forward[i-1];
                    num = num + 1;
                end
            elseif num == 3 then
...
function GetProject(ZZP, EWN, forward, nforw, patt, sourcep, period)
    ...
    if nforw > 2 then
        ...
        proj.ret = GetCompProj(proj.wdiffv, proj.wendpeak, -1, proj.r3, proj.r5, proj.r6);
        proj.ext = GetCompProj(proj.wdiffv, proj.wendpeak, -1, proj.e1, proj.e6, proj.e2);
    end
    ...
    if math.fmod(thisw, 2)==1 and thisw==prevw and proj.isdire<0 and nforw>2 then
        ...
        proj.ppj = GetCompProj(proj.wdiffv, proj.wendpeak, -1, proj.p1, proj.p2, proj.ret);
    end
    if math.fmod(thisw, 2)==1 and thisw==prevw and proj.isdire>0 and nforw>2 then
        ...
        proj.ppj = GetCompProj(proj.wdiffv, proj.wendpeak, -1, proj.p1, proj.p2, proj.p3);
    end
    if math.abs(prevw)==1 and math.abs(thisw)==2 and proj.isdire<0 then
        ...
        proj.ppj = GetCompProj(proj.wdiffv, proj.wendpeak, -1, proj.ppj, proj.bbr, 0);
    end
    ...
    if math.abs(prevw)==2 and math.abs(thisw)==3 then
        ...
        proj.ppj = GetCompProj(proj.wdiffv, proj.wendpeak, -1, proj.p1, proj.p2, proj.p3);
        proj.p3 = GetFactorValue(proj.pwstart, proj.pwfinish, proj.wstartval, 2.0);
        proj.ppj = GetCompProj(proj.wdiffv, proj.wendpeak, -1, proj.ppj, proj.p3, proj.ext);
    end
    if math.abs(prevw)>2 and math.abs(thisw)>math.abs(prevw) and proj.isdire<0 then
        proj.p1 = GetFactorValue(ZZP[forward[nforw-2]], ZZP[forward[nforw-1]], proj.wstartval, -0.382);
        proj.bbr = GetBBresist(BBL, BTL, PX);
        proj.ppj = GetCompProj(proj.wdiffv, proj.wendpeak, -1, proj.p1, proj.bbr, 0);
...
function GetCompProj(wdiffv, wendpeak, minmax, p1, p2, p3)
    local pp = 0;
    local buff = sunit*4;
    if wdiffv<0 then 
        if minmax<0 then
