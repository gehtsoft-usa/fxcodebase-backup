...
function Init()
    indicator:name("CCI BB Signal with SAR targets");
    ...
    indicator.parameters:addDouble("BB_Deviation", "Deviation", "", 2 );  
...
function Prepare(nameOnly)   
	...
	first=math.max(CCI.DATA:first(),BB.DATA:first(),sar.DATA:first()) 
...
function Update(period)
	...
	if CCI.DATA[period-1]< -100
	and source.close[period] > BB.BL[period]
	and source.close[period-1] <= BB.BL[period-1]	
	then
	position[period]=1;
	end
	
	
 	if CCI.DATA[period]> 100
	and source.close[period] < BB.TL[period]
    ...
    then
        Alert[1][period]=1;
        AlertLevel[1][period]=  BB.BL[period] 
    elseif position[period]== -1
        and  position[period-1]~= -1 
    then
        Alert[1][period]=-1;
        AlertLevel[1][period]=  BB.TL[period] 
    end
