...
--http://fxcodebase.com/code/viewtopic.php?f=17&t=69303
...
local source, m_rapida, m_lenta, bbmacd, Avg, SDev, stdv, banda_supe, banda_inf, bbmacd_color_1, bbmacd_color_2, bbmacd_color_3;
local indi_alerts = {};
...
{ 
    {
        ...
        UpCondition = function (period)
            return core.crossesOver(bbmacd, banda_supe, period);
        end,
        DownCondition = function (period)
            return core.crossesUnder(bbmacd, banda_inf, period);
        end,
...
function Init()
    indicator:name("Bollinguer sobre Macd");
    indicator:description("");
    indicator:requiredSource(core.Bar);
    indicator:type(core.Oscillator);
    
    indicator.parameters:addInteger("rapida", "Media Rapida", "", 8);
    indicator.parameters:addInteger("lenta", "Media Lenta", "", 26);
    indicator.parameters:addDouble("stdv", "Stdv", "", 0.8);

    indicator.parameters:addColor("bbmacd_color_1", "BB MACD Color 1", "Color", core.colors().Red);
    indicator.parameters:addColor("bbmacd_color_2", "BB MACD Color 2", "Color", core.colors().Green);
    indicator.parameters:addColor("bbmacd_color_3", "BB MACD Color 3", "Color", core.colors().Blue);
    indicator.parameters:addInteger("bbmacd_width", "BB MACD Width", "Width", 1, 1, 5);
    indicator.parameters:addInteger("bbmacd_style", "BB MACD Style", "Style", core.LINE_SOLID);
    indicator.parameters:setFlag("bbmacd_style", core.FLAG_LINE_STYLE);

    indicator.parameters:addColor("banda_supe_color", "Banda Superior Color", "Color", core.colors().Green);
    indicator.parameters:addInteger("banda_supe_width", "Banda Superior Width", "Width", 1, 1, 5);
    indicator.parameters:addInteger("banda_supe_style", "Banda Superior Style", "Style", core.LINE_SOLID);
    indicator.parameters:setFlag("banda_supe_style", core.FLAG_LINE_STYLE);

    indicator.parameters:addColor("banda_inf_color", "Banda Inferior Color", "Color", core.colors().Blue);
    indicator.parameters:addInteger("banda_inf_width", "Banda Inferior Width", "Width", 1, 1, 5);
    indicator.parameters:addInteger("banda_inf_style", "Banda Inferior Style", "Style", core.LINE_SOLID);
    indicator.parameters:setFlag("banda_inf_style", core.FLAG_LINE_STYLE);
...
function Prepare(nameOnly)
    ...
    bbmacd_color_1 = instance.parameters.bbmacd_color_1;
    bbmacd_color_2 = instance.parameters.bbmacd_color_2;
    bbmacd_color_3 = instance.parameters.bbmacd_color_3;
    stdv = instance.parameters.stdv;
    m_rapida = core.indicators:create("EMA", source.close, instance.parameters.rapida);
    m_lenta = core.indicators:create("EMA", source.close, instance.parameters.lenta);
    
    bbmacd = instance:addStream("BBMACD", core.Line, "BB MACD", "BB MACD", instance.parameters.bbmacd_color_1, 0, 0);
    bbmacd:setWidth(instance.parameters.bbmacd_width);
    bbmacd:setStyle(instance.parameters.bbmacd_style);

    banda_supe = instance:addStream("banda_supe", core.Line, "banda_supe", "banda_supe", instance.parameters.banda_supe_color, 0, 0);
    banda_supe:setWidth(instance.parameters.banda_supe_width);
    banda_supe:setStyle(instance.parameters.banda_supe_style);

    banda_inf = instance:addStream("banda_inf", core.Line, "banda_inf", "banda_inf", instance.parameters.banda_inf_color, 0, 0);
    banda_inf:setWidth(instance.parameters.banda_inf_width);
    banda_inf:setStyle(instance.parameters.banda_inf_style);
    
    Avg = core.indicators:create("EMA", bbmacd, 9);

    instance:createChannelGroup("ab", "ab", banda_supe, banda_inf, bbmacd_color_1, 50, true);
end

function Draw(stage, context) indi_alerts:Draw(stage, context, source); end

function Update(period, mode)
    m_rapida:update(mode);
    m_lenta:update(mode);
    Avg:update(mode);
    if not m_rapida.DATA:hasData(period) or not m_lenta.DATA:hasData(period) then
        return;
    end
    bbmacd[period] = m_rapida.DATA[period] - m_lenta.DATA[period];
    if period < 9 or not bbmacd:hasData(period - 9) then
        return;
    end

    SDev = mathex.stdev(bbmacd, period - 9, period);
    banda_supe[period] = Avg.DATA[period] + stdv * SDev;
    banda_inf[period] = Avg.DATA[period] - stdv * SDev;
    if bbmacd[period] < banda_inf[period] then
        bbmacd:setColor(period, bbmacd_color_1);
        banda_supe:setColor(period, bbmacd_color_1);
    elseif bbmacd[period] > banda_supe[period] then
        bbmacd:setColor(period, bbmacd_color_2);
        banda_supe:setColor(period, bbmacd_color_2);
    else
        bbmacd:setColor(period, bbmacd_color_3);
        banda_supe:setColor(period, bbmacd_color_3);
    end
