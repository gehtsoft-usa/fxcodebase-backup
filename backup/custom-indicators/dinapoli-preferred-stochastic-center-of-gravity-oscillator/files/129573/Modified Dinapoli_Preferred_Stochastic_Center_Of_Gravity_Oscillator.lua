...
function Init()
    ...
    indicator.parameters:addGroup("CCI MA Difference")
    ...
    indicator.parameters:addInteger("ema_Period", "EMA Period", "Period", 14);

    indicator.parameters:addGroup("KRI");
    indicator.parameters:addInteger("KRI_period", "Period", "", 10);
    indicator.parameters:addInteger("KRI_ema_period", "EMA Period", "", 10);
...
local Indicator1, FIR_N, S_N, PM, cci, cci_ema, kri, kri_ema
...
function Prepare(nameOnly)
    ...
    local profile = core.indicators:findIndicator("CCI MA DIFFERENCE");
    assert(profile ~= nil, "Please, download and install " .. "CCI MA DIFFERENCE" .. ".LUA indicator");
    local indicatorParams = profile:parameters();
    cci = core.indicators:create("CCI MA DIFFERENCE", source, instance.parameters.CCI_Period, instance.parameters.MA_Period,
        instance.parameters.MA_Method);

    cci_ema = core.indicators:create("EMA", cci.DATA, instance.parameters.ema_Period);

    kri = core.indicators:create("KRI", source, instance.parameters.KRI_period);
    kri_ema = core.indicators:create("EMA", kri.DATA, instance.parameters.KRI_ema_period);
...
function Update(period, mode)
    ...
    if Indicator1.CG[period] > Indicator1.SIG[period] 
        ...
        and cci.DATA[period] > cci_ema.DATA[period]
        and kri.DATA[period] > kri_ema.DATA[period]
    then
        Oscillator[period] = 1
