...
function Init()
    ...
    indicator.parameters:addInteger("FontSize", "Size", "", 10);
...
if not init then
init=true;
context:createFont (10, "Verdana", context:pointsToPixels (FontSize), context:pointsToPixels (FontSize), context.CENTER)
...
for period= math.max(source:first(),context:firstBar ()),math.min(context:lastBar (), source:size()-1)  , 1 do
   ...
   if Last[period]== 1    then
			...
			width, height =context:measureText (10, Note, context.CENTER)
			context:drawText (10, Note, clrUP, -1, x -width/2, y-height, x +width/2, y , context.CENTER );
   ...
   elseif Last[period]== -1   then
			...
			context:drawText (10, Note, clrDN, -1, x -width/2, y, x +width/2, y+height , context.CENTER );
