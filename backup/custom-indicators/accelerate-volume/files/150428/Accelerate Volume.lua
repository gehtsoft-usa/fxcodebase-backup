...
function Init()
    indicator:name("Accelerate Volume");
    ...
    indicator.parameters:addStringAlternative("Method", "WMA", "WMA" , "WMA");	
	 ...
	 indicator.parameters:addColor("color2", "Bar Color", "", core.rgb(255, 0, 0)); 
	 indicator.parameters:addColor("color3", "MACD Color", "", core.rgb(0, 0, 255)); 	 
end
...
-- Routine
 function Prepare(nameOnly)   
	...
	BuyF= core.indicators:create(Method, VolBuyGreen, PeriodFast);
	SellF= core.indicators:create(Method, VolSellRed, PeriodFast);		

	BuyS= core.indicators:create(Method, VolBuyGreen, PeriodSlow);
	SellS= core.indicators:create(Method, VolSellRed, PeriodSlow);

	VolumeF= core.indicators:create(Method, source.volume, PeriodFast);		 
	VolumeS= core.indicators:create(Method, source.volume, PeriodSlow);	


    AverageVolBuy = instance:addStream("AverageVolBuy", core.Line, name, "AverageVolBuy", instance.parameters.color1, first );
    AverageVolBuy:setPrecision(math.max(2, instance.source:getPrecision()));  

    AverageVolSell = instance:addStream("AverageVolSell", core.Line, name, "AverageVolSell", instance.parameters.color2, first );
    AverageVolSell:setPrecision(math.max(2, instance.source:getPrecision())); 
	
	MACDVolume = instance:addStream("MACDVolume", core.Line, name, "MACDVolume", instance.parameters.color3, first );
    MACDVolume:setPrecision(math.max(2, instance.source:getPrecision())); 
...
function Update(period, mode)
	...
	VolumeF:update(mode); 	
	VolumeS:update(mode);	
	if period <= first + math.max(PeriodFast, PeriodSlow)
	or  not source:hasData(period) 
	then
	return;
	end
	...
	AverageVolBuy[period]=BuyF.DATA[period]-BuyS.DATA[period]
    AverageVolSell[period]=SellF.DATA[period]-SellS.DATA[period]
  	
 
    MACDVolume[period] = VolumeF.DATA[period] - VolumeS.DATA[period]
end
