...
-- http://fxcodebase.com/code/viewtopic.php?f=17&t=54880
...
function Init()
    indicator:name("Drive");
    indicator:description("Drive");
...
U[period]= ((source.high[period]-source.open[period])+(source.close[period]-source.low[period]))/2;
D[period]= ((source.open[period]-source.low[period])+(source.high[period]-source.close[period]))/2;
	...
	return;
        ...
        Up[period] = UP.DATA[period]/source:pipSize();
        Down[period] = DOWN.DATA[period]/source:pipSize();
