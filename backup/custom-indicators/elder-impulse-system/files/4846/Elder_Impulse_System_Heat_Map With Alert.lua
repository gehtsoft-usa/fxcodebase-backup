...
--+------------------------------------------------------------------+
 ...
 function Add(id, TF,Flag, Instrument )
    ...
    indicator.parameters:setFlag("Instrument" .. id, core.FLAG_INSTRUMENTS);
		  ...
		  indicator.parameters:addInteger("EMA".. id, "EMA periods for study", "", 13, 1, 100);
    indicator.parameters:addInteger("MACDF".. id, "MACD periods fast", "", 12, 1, 100);
    indicator.parameters:addInteger("MACDS".. id, "MACD periods slow", "", 26, 1, 100);
...
function Init()
    indicator:name("Elder_Impulse_System_Heat_Map With Alert");
...
indicator.parameters:addInteger("AlertNumber", "Alert Triger Level", "", 3 , 1, 13);	
   ...
   Last=nil;
	...
	local ifirst;
	 ...
	 for i = 1, 13, 1 do
   	...
   	assert(core.indicators:findIndicator("EIS") ~= nil, "Please, download and install EIS.LUA indicator");     
   
	AlertNumber=instance.parameters.AlertNumber;
	  for i = 1, 13, 1 do
	s2, e2 = core.getcandle(iTF[i], 0, 0, 0);
	 ...
	 Number=Number+1;
	       ...
	       EMA[Number]= instance.parameters:getInteger("EMA" .. i);
			MACDF[Number]= instance.parameters:getInteger("MACDF" .. i);
			MACDS[Number]= instance.parameters:getInteger("MACDS" .. i);
	 ...
	 Label[Number]="";
                ...
                Label[Number]=Label[Number] .. " - " ..  TF[Number];				
			  	...
			  	Temp1= core.indicators:create("EIS", source.close ,  EMA[Number],MACDF[Number], MACDS[Number]);  				
				ifirst= Temp1.DATA:first()*2;
				 ...
				 loading[Number]  = true;  	 
				  Indicator[Number] = core.indicators:create("EIS", SourceData[Number].close ,  EMA[Number],MACDF[Number], MACDS[Number], Up, Down, Neutral);
			    ...
			    for j= 1, Number , 1 do
				  ...
				  if p~= false then
										...
										if Indicator[j].DATA:hasData(p) and Indicator[j].DATA:hasData(p-1) then 
												  ...
												  if Indicator[j].DATA:colorI(p) ==  Up then		 
														 
															C2=12;
															 
														 
												 
												    elseif Indicator[j].DATA:colorI(p) ==  Down then	
                                                       ...
                                                       if Indicator[j].DATA:colorI(p-1) ==  Up then		 
														 
													 
															C1=11;
														 
												 
												    elseif Indicator[j].DATA:colorI(p-1) ==  Down then	
...
function Evaluate(j)	
	 ...
	 local   p=Initialization((source:size()-1),j);
		 ...
		 if not Indicator[j].DATA:hasData(p) 
		 or not p
		 then
		 return;
		 end
		 
		if Indicator[j].DATA:colorI(p) ==  Up
		and Indicator[j].DATA:colorI(p-1) ~=  Up
		then 
		Count= Count+1;
		elseif Indicator[j].DATA:colorI(p) ==  Down
		and Indicator[j].DATA:colorI(p-1) ~=  Down
		then
		Count= Count-1;  
		end
