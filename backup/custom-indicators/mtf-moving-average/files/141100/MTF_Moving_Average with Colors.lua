...
function Init()
	...
	indicator.parameters:addColor("Up_Color", "Up Fill color", "", core.rgb(0, 255, 0));
	indicator.parameters:addColor("Down_Color", "Down Fill color", "", core.rgb(255, 0, 0));	 
...
function Draw(stage, context)
    ...
    if not init then
        init = true
			...
			context:createSolidBrush (11, Up_Color);
			context:createSolidBrush (12, Down_Color);
    ...
    if y1 ~= nil and y2 ~= nil then
	    if y1 > y2 then
        context:drawRectangle(-1, 11 , context:left(), y1, context:right(), y2, transparency);
		else
		context:drawRectangle(-1, 12 , context:left(), y1, context:right(), y2, transparency);
		end
    end
end
