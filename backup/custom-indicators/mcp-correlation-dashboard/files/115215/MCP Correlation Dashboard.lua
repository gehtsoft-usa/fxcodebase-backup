...
function Init()
	...
	indicator.parameters:addDouble("MaxCorr", "Max. correlation", "", 0.8);
...
function Prepare(nameOnly)
    MaxCorr= instance.parameters.MaxCorr;
...
function IsSymbEqual(S1, S2)
	if S1:instrument()==S2:instrument() then
		return true;
	else
		return false;
	end
 ...
 function Calculate (context,i, j )
	...
	local Value1;
	      ...
	      if Type== "Chart" then
	      	if IsSymbEqual(Source[Count+1], Source[j]) then
	      		Value1=1000;
	      	else
	      	...
	      	if IsSymbEqual(Source[i], Source[j]) then
	      		Value1=1000;
	      	else
		  ...
		  if Value1>10 or math.abs(Value1)>=MaxCorr then
		  	Symbol1="";
		  else
