...
-- https://fxcodebase.com/code/viewtopic.php?f=17&t=71148
...
local STRATEGY_NAME = "3 level zz semafor and macd overlay Strategy";
...
local zz_indi, Indicator;
function CreateEntryIndicators(source)
    local indi = core.indicators:findIndicator("3_LEVEL_ZZ_SEMAFOR");
    assert(indi ~= nil, "Please, download and install " .. "3_LEVEL_ZZ_SEMAFOR" .. ".LUA indicator");
    ...
    local profile = core.indicators:findIndicator("MACD OVERLAY");
    assert(profile ~= nil, "Please, download and install " .. "MACD OVERLAY" .. ".LUA indicator");
    Indicator = core.indicators:create("MACD OVERLAY", source, instance.parameters.Method,
        instance.parameters.SN,
        instance.parameters.LN,
...
function IsEntryLong(source, period)
    ...
    return GetLastSignal(zz_indi) == -1 and Indicator.DATA:colorI(period) == core.rgb(0, 200, 0) and Indicator.DATA:colorI(period - 1) ~= core.rgb(0, 200, 0);
end
function IsEntryShort(source, period)
    if not Indicator.DATA:hasData(period - 1) then
        return false;
    end
    return GetLastSignal(zz_indi) == 1 and Indicator.DATA:colorI(period) == core.rgb(200, 0, 0) and Indicator.DATA:colorI(period - 1) ~= core.rgb(200, 0, 0);
end
