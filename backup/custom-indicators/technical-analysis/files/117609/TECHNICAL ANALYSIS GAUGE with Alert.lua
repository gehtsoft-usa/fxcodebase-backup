...
local last_state = {};
last_state["Osicllators"] = "";
last_state["Indicators"] = "";
local SendEmail;
local Email;
local PlaySound;
local SoundFile;
...
function AsyncOperationFinished(cookie)
    if cookie== 1 then
        Alarm(oscillators, "Osicllators");
        Alarm(indicators, "Indicators");    
    end
 end
 
function Alarm(array, caption)
    local curent_state = calculateState(array);
    if (last_state[caption] ~= curent_state) then
        if curent_state == "StrongSell" then
            Signal("StrongSell" .. " " .. caption)
        elseif curent_state == "StrongBuy" then
            Signal("StrongBuy" .. " " .. caption)
        end
        last_state[caption] = curent_state; 
    end        
end
 
function calculateState(array)
...
function Signal(Label)
    if ShowAlert then
        terminal:alertMessage(instance.source:instrument(), instance.source[NOW],  Label, instance.source:date(NOW));
    end
    ...
    if Email ~= nil then
        terminal:alertEmail(Email, Label, profile:id() .. "(" .. instance.source:instrument() .. ")" .. instance.source[NOW]..", " .. Label..", " .. instance.source:date(NOW));
    end
end