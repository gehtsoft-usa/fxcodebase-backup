-- Available @ https://fxcodebase.com/code/viewtopic.php?f=17&t=75731
...
function Init()
    ...
    indicator.parameters:addInteger("N", "N", "", 4);
    ...
    indicator.parameters:addColor("support_color", "Support color", "", core.rgb(0, 255, 0))
    indicator.parameters:addColor("resistance_color", "Resistance color", "", core.rgb(255, 0, 0))
    indicator.parameters:addColor("middle_color", "Resistance color", "", core.rgb(0, 0, 255))
end
...
function Draw(stage, context)
    ...
    if not init then
        init = true;
        context:createPen(SUP, context.SOLID, 1, instance.parameters.support_color);
        context:createPen(RES, context.SOLID, 1, instance.parameters.resistance_color);
        context:createPen(MID, context.SOLID, 1, instance.parameters.middle_color);
    end
    local count = 0;
    ...
    local hh;
    local last_period;
    while peaks:Next() and count < N do
        local period, value, next_side = peaks:GetData();
        if ll == nil or ll > value then
            ll = value;
        end
        if hh == nil or hh < value then
            hh = value;
        end
        last_period = period;
        count = count + 1;
    end
    if ll == nil or hh == nil then
        return;
    end

    local x1 = context:positionOfBar(last_period);
    local _, sup_level = context:pointOfPrice(ll);
    context:drawLine(SUP, x1, sup_level, context:right(), sup_level)
    local _, res_level = context:pointOfPrice(hh);
    context:drawLine(RES, x1, res_level, context:right(), res_level)
    local _, mid_level = context:pointOfPrice((hh + ll) / 2);
    context:drawLine(MID, x1, mid_level, context:right(), mid_level)
end
