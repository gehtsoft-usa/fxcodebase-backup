...
function Update(period, mode)
	...
	if
		source[period] > MA1.DATA[period] and (MA1.DATA[period] > MA1.DATA[period - 1] or Off) and
			MA2.DATA[period] > MA2.DATA[period - 1] and
			MA3.DATA[period] > MA3.DATA[period - 1] and
			(MA1.DATA[period] > MA2.DATA[period] or Off) and
			MA2.DATA[period] > MA3.DATA[period]
	 then
		Trend[period] = 1
	elseif
		source[period] < MA1.DATA[period] and (MA1.DATA[period] < MA1.DATA[period - 1] or Off) and
...
function Draw(stage, context)
	...
	x, x, x2 = context:positionOfBar(Trend:size() - 1)
	...
	if Off then
		if Trend[NOW] == Trend[LastCross] and Trend[NOW] == 1 then
			C1 = 1
			C2 = 2
		...
		elseif Trend[NOW] == Trend[LastCross] and Trend[NOW] == -1 then
			C1 = 1
			C2 = 2
			context:drawRectangle(-1, C1, x1, y1 + math.abs(y2 - y3) * Ratio, x2, y1, Transparency)
			context:drawRectangle(-1, C2, x1, y3, x2, y1, Transparency)
