...
function Init()
    indicator:name("FTLM-STLM Components");
    indicator:description("FTLM-STLM Components");
    indicator:requiredSource(core.Tick);
    indicator:type(core.Oscillator);
    indicator.parameters:addGroup("Calculation"); 
	indicator.parameters:addString("Method", "Selector", "Selector" , "FTLM");
    indicator.parameters:addStringAlternative("Method", "FTLM", "FTLM" , "FTLM");
    indicator.parameters:addStringAlternative("Method", "STLM", "STLM" , "STLM");
    indicator.parameters:addStringAlternative("Method", "FATL", "FATL" , "FATL");
	indicator.parameters:addStringAlternative("Method", "SATL", "SATL" , "SATL");
	indicator.parameters:addStringAlternative("Method", "RFTL", "RFTL" , "RFTL");
	indicator.parameters:addStringAlternative("Method", "RSTL", "RSTL" , "RSTL");
	indicator.parameters:addStringAlternative("Method", "RBCI", "RBCI" , "RBCI");
	indicator.parameters:addStringAlternative("Method", "PCCI", "PCCI" , "PCCI");
...
function Prepare(nameOnly)
    ...
    local name = profile:id() .. "(" .. price:name().. ", " .. Method   .. ")";
    instance:name(name);

    if (not (nameOnly)) then
        Out = instance:addStream("Out", core.Line, name .. ".".. Method,  Method, instance.parameters.color, first);
...
function Update(period)
	...
	value1=value1
	 ...
	 if Method == "FTLM" then
        Out[period]= value3-value4;
	 elseif Method == "STLM" then	
        Out[period] = value1-value2;
		
	 elseif Method == "FATL" then
        Out[period]= value3 ;
	 elseif Method == "SATL" then	
        Out[period] = value1;

     elseif Method == "RFTL" then
        Out[period]= value4;
	 elseif Method == "RSTL" then	
        Out[period] = value2;
    elseif Method == "RBCI" then 
		Out[period] = value3 - value1
     elseif Method == "PCCI" then	 
		Out[period] = price [period] - value3;
	 end
end
