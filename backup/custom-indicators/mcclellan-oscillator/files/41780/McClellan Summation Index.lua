...
function Init()
    ...
    indicator:name("McClellan Summation Index");
    indicator:description("McClellan Summation Index");
    indicator:requiredSource(core.Bar);
    indicator:type(core.Oscillator);
	
	indicator.parameters:addGroup("Calculation"); 	 
	 indicator.parameters:addInteger("SP", "Short Period", "Short Period", 19);
	indicator.parameters:addInteger("LP", "Long Period", "Long Period", 39);
    ...
    indicator.parameters:addColor("A_color", "Color of McClellan Oscillator", "Color of McClellan Oscillator", core.rgb(0, 255, 0));
...
function Prepare(nameOnly)
    ...
    if (not (nameOnly)) then
		...
		SD= core.indicators:create("EMA", RANA, SP);
		LD= core.indicators:create("EMA", RANA, LP);
		McClellan = instance:addStream("MCCLELLAN", core.Line, name, "MCCLELLAN", instance.parameters.A_color, math.max(SD.DATA:first(), LD.DATA:first()));
		McClellan:setPrecision (2);
		McClellan:setWidth(instance.parameters.width);
        McClellan:setStyle(instance.parameters.style);
...
function Update(period, mode)
    ...
    local F=0;	
	  ...
	  RANA[period]=0;	
		...
		if period <  math.max(SD.DATA:first(), LD.DATA:first()) then
		 RANA[period]=0;
		return;
		end
		
		McClellan [period]= McClellan [period-1] + ( SD.DATA[period]-LD.DATA[period]);
end
