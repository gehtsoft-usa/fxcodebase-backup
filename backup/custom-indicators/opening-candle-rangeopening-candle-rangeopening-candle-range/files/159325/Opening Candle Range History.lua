...
-- +-----------------+----------------------+-------------------------------------------------------+ 
function Init()
    indicator:name("Opening Candle Range History");
    indicator:description("Opening Candle Range History");
...
function Loop (p, context)

  
	local date= Source:date(p);
	
	if   context:positionOfDate(date) < context:left () then
	return;
	end
   ...
   if p == Source:size()-1 and  source:date(source:size()-1)< startdate  then 
