...
function Init()
    indicator:name("MTF MCP Trend filter");
    indicator:description("MTF MCP Trend filter");
	...
	indicator.parameters:addInteger("transparency", "Fill Transparency", "0 - opaque, 100 - transparent", 60, 0, 100);
...
function Prepare(nameOnly)
	...
	assert(core.indicators:findIndicator("TREND FILTER") ~= nil, "Please, download and install TREND FILTER.LUA indicator");
	 ...
	 for i = 1, Count, 1 do	
	  ...
	  loading[i] ={};  
       Indicator[i]={};
		   ...
		   Temp= core.indicators:create("TREND FILTER", source ,cp1, cp2 );
		   ...
		   loading [i][j]=true;
		    ...
		    Indicator [i][j]= core.indicators:create("TREND FILTER", Source[i][j] , cp1, cp2,color1, color2, color3, color4, color5);
...
function Draw(stage, context)
 ...
 function Calculate (context,i, j )
		...
		context:drawText (7,Pair[i], Color, -1, x1 , y2, x1+width, y2+height, context.CENTER   );	
		end
		...
		context:drawText (7,  TF[j], Color, -1, x1+xGap , y1,x1+xGap+width,  y1+height, context.CENTER   );	
		end
    ...
    local SymbolColor=Neutral;	
	  ...
	  local period =Indicator[i][j].DATA:size()-1; 
	...
	--Color
	   local period =Indicator[i][j].DATA:size()-1; 
		...
		if  (Source[i][j].close[period]>Source[i][j].open[period])   then
		 Symbol="\236";
		elseif  (Source[i][j].close[period]<Source[i][j].open[period])   then 
		Symbol="\238";
		else
		Symbol="\160";
		end
		 
		SymbolColor=Indicator[i][j].DATA:colorI(period); 
		...
		if Symbol~= nil then
		 width, height = context:measureText (8, Symbol , context.CENTER  );  
		  context:drawText (8,  Symbol, SymbolColor, -1, x1+xGap  , y1+yGap, x1+xGap+width  , y1+yGap+height, context.CENTER   );	
		end
