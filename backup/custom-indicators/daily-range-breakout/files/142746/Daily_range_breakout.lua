...
--https://fxcodebase.com/code/viewtopic.php?f=17&t=71334
...
{ 
    {
        ...
        UpCondition = function (period)
            local index = core.findDate(source_d1, source:date(period), false) - 1;
            if index < 0 then
                return false;
            end
            return source_d1.high[index] <= source.close[period];
        end,
        DownCondition = function (period)
            ...
            return source_d1.low[index] >= source.close[period];
        end,
...
function Init()
    indicator:name("Daily range breakout");
...
function Prepare(nameOnly)
    ...
    source_d1 = sources:Request(1, source, "D1");
end
