...
function Draw(stage, context)
	...
	local visible;
	local FIRST;
	local l1x1, l1y1, l1x2, l1y2, l2x1, l2y1, l2x2, l2y2;
    ...
    for i= source:size()-1, FIRST, -1 do	
		 ...
		 if core.crosses (short.DATA, long.DATA, i) then
		   ...
		   l1x1, x2, x3 = context:positionOfBar  (i-1)
			visible,l1y1 = context:pointOfPrice (short.DATA[i-1]);
			l1x2, x2, x3 = context:positionOfBar  (i)
			visible,l1y2 = context:pointOfPrice (short.DATA[i]);
			l2x1, x2, x3 = context:positionOfBar  (i-1)
			visible,l2y1 = context:pointOfPrice (long.DATA[i-1]);
			l2x2, x2, x3 = context:positionOfBar  (i) 
			visible,l2y2 = context:pointOfPrice (long.DATA[i]);
