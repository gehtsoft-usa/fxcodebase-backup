...
function Init()
    indicator:name("MTF MCP Awesome Modified List");
    indicator:description("MTF MCP Awesome Modified List");
...
local TF={};
 local Short_Period, Medium_Period, Long_Period, AO_Fast_Period, AO_Slow_Period;
...
function Prepare(nameOnly)
	...
	assert(core.indicators:findIndicator("AWESOMEMOD") ~= nil, "Please, download and install AWESOMEMOD.LUA indicator");
	...
	Color= instance.parameters.Color; 
	 ...
	 for i = 1, Count, 1 do	
      ...
      Indicator[i]={};
		   ...
		   Indicator [i][j]= core.indicators:create("AWESOMEMOD", Source[i][j].close ,Short_Period, Medium_Period, Long_Period, AO_Fast_Period, AO_Slow_Period );
...
function Draw(stage, context)
 ...
 function Calculate (context,i, j )
    ...
    local SymbolColor=Neutral;	
		  ...
		  if Indicator[i][j].AO_Slow [Indicator[i][j].AO_Slow:size()-1] > 0
		  and Indicator[i][j].AO_Fast [Indicator[i][j].AO_Fast:size()-1] > 0
		  then
		  Symbol="\225"; 
		  SymbolColor=Up;
		  elseif   Indicator[i][j].AO_Slow [Indicator[i][j].AO_Slow:size()-1] < 0
		  and Indicator[i][j].AO_Fast [Indicator[i][j].AO_Fast:size()-1] < 0
		  then
		  Symbol="\226"; 
		  SymbolColor=Down;
