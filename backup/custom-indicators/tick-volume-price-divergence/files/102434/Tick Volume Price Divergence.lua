...
-- http://fxcodebase.com/code/viewtopic.php?f=17&t=62684
...
function Init()
    indicator:name("Tick Volume Price Divergence");
...
function Prepare(nameOnly)
    ...
    D = instance:addStream("Volume", core.Bar, name .. "." .. " Tick Volume", " Tick Volume", instance.parameters.D_color, first, -1);
...
function processBullish(period)
    if isTrough(period) then
        ...
        if prev ~= nil then
            if source.volume[curr] > source.volume[prev] and source.low[curr] < source.low[prev] then
                if I then
                    DN:set(curr, source.volume[curr], "\225", "Classic bullish");
                    line_id = line_id + 1;
                    core.host:execute("drawLine", line_id, source:date(prev), source.volume[prev], source:date(curr), source.volume[curr], DN_color);
                else
                    DN[period] = curr - prev;
                end
            elseif source.volume[curr] < source.volume[prev] and source.low[curr] > source.low[prev] then
                if I then
                    DN:set(curr, source.volume[curr], "\225", "Reversal bullish");
                    line_id = line_id + 1;
                    core.host:execute("drawLine", line_id, source:date(prev), source.volume[prev], source:date(curr), source.volume[curr], DN_color);
                else
                    DN[period] = -(curr - prev);
...
function isTrough(period)
    local i;
    if source.volume[period] < source.volume[period - 1] and source.volume[period] < source.volume[period + 1] then
        for i = period - 1, period-Frames, -1 do
            if i<=first then
             return true;
            end
            if source.volume[period]>source.volume[i] then
             return false;
            end
        end
        return true;
...
function prevTrough(period)
    ...
    for i = period - 5, first, -1 do
        if source.volume[i] <= source.volume[i - 1] and source.volume[i] < source.volume[i - 2] and
           source.volume[i] <= source.volume[i + 1] and source.volume[i] < source.volume[i + 2] then
           return i;
        end
    end
    return nil;
...
function processBearish(period)
    if isPeak(period) then
        ...
        if prev ~= nil then
            if source.volume[curr] < source.volume[prev] and source.high[curr] > source.high[prev] then
                if I then
                    UP:set(curr, source.volume[curr], "\226", "Classic bearish");
                    line_id = line_id + 1;
                    core.host:execute("drawLine", line_id, source:date(prev), source.volume[prev], source:date(curr), source.volume[curr], UP_color);
                else
                    UP[period] = curr - prev;
                end
            elseif source.volume[curr] > source.volume[prev] and source.high[curr] < source.high[prev] then
                if I then
                    UP:set(curr, source.volume[curr], "\226", "Reversal bearish");
                    line_id = line_id + 1;
                    core.host:execute("drawLine", line_id, source:date(prev), source.volume[prev], source:date(curr), source.volume[curr], UP_color);
                else
                    UP[period] = -(curr - prev);
